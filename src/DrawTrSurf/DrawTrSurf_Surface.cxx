...
// purpose  :
//=======================================================================
DrawTrSurf_Surface::DrawTrSurf_Surface(const Handle(Geom_Surface) & S) : DrawTrSurf_Drawable(16, 0.01, 1) {
    surf = S;
...
// purpose  :
//=======================================================================
DrawTrSurf_Surface::DrawTrSurf_Surface(const Handle(Geom_Surface) & S, const Standard_Integer Nu,
                                       const Standard_Integer Nv, const Draw_Color& BoundsColor,
                                       const Draw_Color& IsosColor, const Standard_Integer Discret,
...
void DrawTrSurf_Surface::DrawOn(Draw_Display& dis, const Standard_Boolean Iso) const {
    ...
    // draw marker
    DrawIsoCurveOn(C, GeomAbs_IsoU, UFirst + (ULast - UFirst) / 10., VFirst, VFirst + (VLast - VFirst) / 10., dis);
}
...
Handle(Draw_Drawable3D) DrawTrSurf_Surface::Copy() const {
    Handle(DrawTrSurf_Surface) DS =
        new DrawTrSurf_Surface(Handle(Geom_Surface)::DownCast(surf->Copy()), nbUIsos, nbVIsos, boundsLook, isosLook,
...
Handle(Draw_Drawable3D) DrawTrSurf_Surface::Restore(std::istream& theStream) {
    ...
    Handle(DrawTrSurf_Surface) aDrawSurface =
        new DrawTrSurf_Surface(aGeomSurface, aParams.NbUIsos, aParams.NbVIsos, aParams.BoundsColor, aParams.IsosColor,
