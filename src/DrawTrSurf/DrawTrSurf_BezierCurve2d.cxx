...
void DrawTrSurf_BezierCurve2d::FindPole(const Standard_Real X, const Standard_Real Y, const Draw_Display& D,
    ...
    while (Index <= NbPoles) {
        ...
        if (D.Project(P).Distance(p1) <= Prec) return;
        Index++;
    }
    Index = 0;
}
...
Handle(Draw_Drawable3D) DrawTrSurf_BezierCurve2d::Copy() const {
    Handle(DrawTrSurf_BezierCurve2d) DC = new DrawTrSurf_BezierCurve2d(
        Handle(Geom2d_BezierCurve)::DownCast(curv->Copy()), look, polesLook, drawPoles, GetDiscretisation());
...
Handle(Draw_Drawable3D) DrawTrSurf_BezierCurve2d::Restore(Standard_IStream& theStream) {
    ...
    Handle(DrawTrSurf_BezierCurve2d) aDrawCurve = new DrawTrSurf_BezierCurve2d(
        aGeomCurve, aParams.CurvColor, aParams.PolesColor, aParams.IsShowPoles, aParams.Discret);
    return aDrawCurve;
}
