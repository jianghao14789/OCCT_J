...
DrawTrSurf_Curve::DrawTrSurf_Curve(const Handle(Geom_Curve) & C, const Standard_Boolean DispOrigin)
    : DrawTrSurf_Drawable(16, 0.01, 1), curv(C), look(Draw_vert), disporigin(DispOrigin),
      dispcurvradius(Standard_False), radiusmax(1.0e3), radiusratio(0.1) {
    //
}
...
DrawTrSurf_Curve::DrawTrSurf_Curve(const Handle(Geom_Curve) & C, const Draw_Color& aColor,
    ...
    : DrawTrSurf_Drawable(Discret, Deflection, DrawMode), curv(C), look(aColor), disporigin(DispOrigin),
      dispcurvradius(DispCurvRadius), radiusmax(RadiusMax), radiusratio(RadiusRatio) {
    //
}
...
Handle(Draw_Drawable3D) DrawTrSurf_Curve::Copy() const {
    Handle(DrawTrSurf_Curve) DC = new DrawTrSurf_Curve(Handle(Geom_Curve)::DownCast(curv->Copy()), look,
...
Handle(Draw_Drawable3D) DrawTrSurf_Curve::Restore(Standard_IStream& theStream) {
    ...
    Handle(DrawTrSurf_Curve) aDrawCurve =
        new DrawTrSurf_Curve(aGeomCurve, aParams.CurvColor, aParams.Discret, aParams.Deflection, aParams.DrawMode);
    return aDrawCurve;
}
