...
DrawTrSurf_BSplineSurface::DrawTrSurf_BSplineSurface(const Handle(Geom_BSplineSurface) & S,
                                                     ...
                                                     const Standard_Integer KnotsSize, const Standard_Boolean ShowPoles,
                                                     const Standard_Boolean ShowKnots, const Standard_Integer Discret,
                                                     const Standard_Real Deflection, const Standard_Integer DrawMode)
    : DrawTrSurf_Surface(S, S->NbUKnots() - 2, S->NbVKnots() - 2, BoundsColor, IsosColor, Discret, Deflection,
                         DrawMode) {
    knotsIsos = Standard_True;
    drawPoles = ShowPoles;
    drawKnots = ShowKnots;
...
DrawTrSurf_BSplineSurface::DrawTrSurf_BSplineSurface(const Handle(Geom_BSplineSurface) & S,
                                                     ...
                                                     const Standard_Boolean ShowKnots, const Standard_Integer Discret,
                                                     const Standard_Real Deflection, const Standard_Integer DrawMode)
    : DrawTrSurf_Surface(S, Abs(NbUIsos), Abs(NbVIsos), BoundsColor, IsosColor, Discret, Deflection, DrawMode) {
...
void DrawTrSurf_BSplineSurface::FindPole(const Standard_Real X, const Standard_Real Y, const Draw_Display& D,
                                         const Standard_Real XPrec, Standard_Integer& UIndex,
...
void DrawTrSurf_BSplineSurface::FindUKnot(const Standard_Real X, const Standard_Real Y, const Draw_Display& D,
                                          const Standard_Real Prec, Standard_Integer& UIndex) const {
    Handle(Geom_BSplineSurface) bs = Handle(Geom_BSplineSurface)::DownCast(surf);
    gp_Pnt2d p1(X, Y);
    UIndex++;
...
void DrawTrSurf_BSplineSurface::FindVKnot(const Standard_Real X, const Standard_Real Y, const Draw_Display& D,
                                          const Standard_Real Prec, Standard_Integer& VIndex) const {
    Handle(Geom_BSplineSurface) bs = Handle(Geom_BSplineSurface)::DownCast(surf);
    gp_Pnt2d p1(X, Y);
    VIndex++;
...
Handle(Draw_Drawable3D) DrawTrSurf_BSplineSurface::Copy() const {
    ...
    if (!knotsIsos) {
        DS = new DrawTrSurf_BSplineSurface(Handle(Geom_BSplineSurface)::DownCast(surf->Copy()), nbUIsos, nbVIsos,
                                           boundsLook, isosLook, polesLook, knotsLook, knotsForm, knotsDim, drawPoles,
                                           drawKnots, GetDiscretisation(), GetDeflection(), GetDrawMode());
    } else {
        DS = new DrawTrSurf_BSplineSurface(Handle(Geom_BSplineSurface)::DownCast(surf->Copy()), boundsLook, isosLook,
                                           polesLook, knotsLook, knotsForm, knotsDim, drawPoles, drawKnots,
...
Handle(Draw_Drawable3D) DrawTrSurf_BSplineSurface::Restore(Standard_IStream& theStream) {
    ...
    if (!aParams.NeedKnotsIsos) {
        aDrawSurface = new DrawTrSurf_BSplineSurface(
            aGeomSurface, aParams.NbUIsos, aParams.NbVIsos, aParams.BoundsColor, aParams.IsosColor, aParams.PolesColor,
            aParams.KnotsColor, aParams.KnotsMarker, aParams.KnotsSize, aParams.IsShowPoles, aParams.IsShowKnots,
            aParams.Discret, aParams.Deflection, aParams.DrawMode);
    } else {
        aDrawSurface = new DrawTrSurf_BSplineSurface(aGeomSurface, aParams.BoundsColor, aParams.IsosColor,
