...
//                   CC1 = C1
//=======================================================================
static Standard_Boolean Arrange(const Handle(Geom_BSplineCurve) & C1, const Handle(Geom_BSplineCurve) & C2,
                                const Handle(Geom_BSplineCurve) & C3, const Handle(Geom_BSplineCurve) & C4,
                                Handle(Geom_BSplineCurve) & CC1, Handle(Geom_BSplineCurve) & CC2,
                                Handle(Geom_BSplineCurve) & CC3, Handle(Geom_BSplineCurve) & CC4,
...
static Standard_Integer SetSameDistribution(Handle(Geom_BSplineCurve) & C1, Handle(Geom_BSplineCurve) & C2) {
    ...
    if (BSplCLib::PrepareInsertKnots(C1->Degree(), Standard_False, K1, M1, K2, &M2, NP, NK, Precision::PConfusion(),
                                     Standard_False)) {
        ...
        BSplCLib::InsertKnots(C1->Degree(), Standard_False, P1, &W1, K1, M1, K2, &M2, NewP, &NewW, NewK, NewM,
                              Precision::PConfusion(), Standard_False);
        if (C1->IsRational()) {
            C1 = new Geom_BSplineCurve(NewP, NewW, NewK, NewM, C1->Degree());
        } else {
            C1 = new Geom_BSplineCurve(NewP, NewK, NewM, C1->Degree());
        }
        BSplCLib::InsertKnots(C2->Degree(), Standard_False, P2, &W2, K2, M2, K1, &M1, NewP, &NewW, NewK, NewM,
                              Precision::PConfusion(), Standard_False);
        if (C2->IsRational()) {
...
GeomFill_BSplineCurves::GeomFill_BSplineCurves(const Handle(Geom_BSplineCurve) & C1,
                                               ...
                                               const Handle(Geom_BSplineCurve) & C4, const GeomFill_FillingStyle Type) {
...
GeomFill_BSplineCurves::GeomFill_BSplineCurves(const Handle(Geom_BSplineCurve) & C1,
                                               const Handle(Geom_BSplineCurve) & C2,
                                               const Handle(Geom_BSplineCurve) & C3, const GeomFill_FillingStyle Type) {
    Init(C1, C2, C3, Type);
}
...
GeomFill_BSplineCurves::GeomFill_BSplineCurves(const Handle(Geom_BSplineCurve) & C1,
                                               const Handle(Geom_BSplineCurve) & C2, const GeomFill_FillingStyle Type) {
    Init(C1, C2, Type);
}
...
void GeomFill_BSplineCurves::Init(const Handle(Geom_BSplineCurve) & C1, const Handle(Geom_BSplineCurve) & C2,
                                  const Handle(Geom_BSplineCurve) & C3, const Handle(Geom_BSplineCurve) & C4,
    ...
    Standard_Boolean isRat = (CC1->IsRational() || CC2->IsRational() || CC3->IsRational() || CC4->IsRational());
    ...
    if (Caro.isRational()) {
        ...
        mySurface =
            new Geom_BSplineSurface(Poles, Weights, UKnots, VKnots, UMults, VMults, CC1->Degree(), CC2->Degree());
    } else {
        mySurface = new Geom_BSplineSurface(Poles, UKnots, VKnots, UMults, VMults, CC1->Degree(), CC2->Degree());
    }
}
...
void GeomFill_BSplineCurves::Init(const Handle(Geom_BSplineCurve) & C1, const Handle(Geom_BSplineCurve) & C2,
    ...
    if (Type != GeomFill_CurvedStyle) {
        ...
        if (isRat) {
            ...
            mySurface = new Geom_BSplineSurface(Poles, Weights, UKnots, VKnots, UMults, VMults, CC1->Degree(), 1,
                                                CC1->IsPeriodic(), Standard_False);
        } else {
        ...
        if (!IsOK) throw Standard_OutOfRange("GeomFill_BSplineCurves: Courbes non jointives");
        ...
        if (Caro.isRational()) {
            TColStd_Array2OfReal Weights(1, NbUPoles, 1, NbVPoles);
            Caro.Weights(Weights);
            mySurface = new Geom_BSplineSurface(Poles, Weights, UKnots, VKnots, UMults, VMults, Deg1, Deg2,
                                                Standard_False, Standard_False);
        } else {
            mySurface = new Geom_BSplineSurface(Poles, UKnots, VKnots, UMults, VMults, Deg1, Deg2, Standard_False,
                                                Standard_False);
        }
    }
}
