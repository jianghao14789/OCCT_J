...
#endif
#ifdef OCCT_DEBUG
Standard_IMPORT void Law_draw1dcurve(const TColStd_Array1OfReal& pol, const TColStd_Array1OfReal& knots,
                                     const TColStd_Array1OfInteger& mults, const Standard_Integer deg,
                                     const gp_Vec2d& tra, const Standard_Real scal);
Standard_IMPORT void Law_draw1dcurve(const Handle(Law_BSpline) & bs, const gp_Vec2d& tra, const Standard_Real scal);
...
static Standard_Integer inqadd(const Standard_Real d1, const Standard_Real d2, Standard_Real* k, Standard_Integer* m,
                               const Standard_Integer deg, const Standard_Real tolk) {
...
static void sortbounds(const Standard_Integer nb, Handle(GeomFill_Boundary) * bound, Standard_Boolean* rev,
...
static void coonscnd(const Standard_Integer nb, Handle(GeomFill_Boundary) * bound, Standard_Boolean* rev,
                     GeomFill_CornerState* stat,
                     //		     Handle(GeomFill_TgtField)* tga,
                     Handle(GeomFill_TgtField) *, Standard_Real* mintg) {
...
static void killcorners(const Standard_Integer nb, Handle(GeomFill_Boundary) * bound, Standard_Boolean* rev,
                        Standard_Boolean* nrev, GeomFill_CornerState* stat, Handle(GeomFill_TgtField) * tga) {
    Standard_Integer i;
...
public:
    GeomFill_ConstrainedFilling_Eval(GeomFill_ConstrainedFilling& theTool) : curfil(theTool) {}
...
void GeomFill_ConstrainedFilling_Eval::Evaluate(Standard_Integer*, /*Dimension*/
                                                Standard_Real /*StartEnd*/[2], Standard_Real* Parameter,
                                                Standard_Integer* DerivativeRequest,
                                                Standard_Real* Result, // [Dimension]
                                                Standard_Integer* ErrorCode) {
...
GeomFill_ConstrainedFilling::GeomFill_ConstrainedFilling(const Standard_Integer MaxDeg, const Standard_Integer MaxSeg)
    : degmax(MaxDeg), segmax(MaxSeg), appdone(Standard_False), nbd3(0) {
...
void GeomFill_ConstrainedFilling::Init(const Handle(GeomFill_Boundary) & B1, const Handle(GeomFill_Boundary) & B2,
                                       const Handle(GeomFill_Boundary) & B3, const Standard_Boolean NoCheck) {
#ifdef OCCT_DEBUG
    totclock.Reset();
    parclock.Reset();
    appclock.Reset();
    ...
    if (bound[0]->HasNormals()) ttgalg[0] = tgalg[0] = new GeomFill_TgtOnCoons(ptch, 0);
    if (bound[1]->HasNormals()) ttgalg[1] = tgalg[1] = new GeomFill_TgtOnCoons(ptch, 1);
    if (bound[2]->HasNormals()) ttgalg[2] = tgalg[3] = new GeomFill_TgtOnCoons(ptch, 3);
...
void GeomFill_ConstrainedFilling::Init(const Handle(GeomFill_Boundary) & B1, const Handle(GeomFill_Boundary) & B2,
                                       const Handle(GeomFill_Boundary) & B3, const Handle(GeomFill_Boundary) & B4,
...
void GeomFill_ConstrainedFilling::SetDomain(const Standard_Real l, const Handle(GeomFill_BoundWithSurf) & B) {
    if (B == ptch->Bound(0))
        dom[0] = Min(1., Abs(l));
...
void GeomFill_ConstrainedFilling::PerformApprox() {
    ...
    AdvApprox_ApproxAFunction app(0, 0, nbd3, tol1d, tol2d, tol3d, f, l, GeomAbs_C1, degmax, segmax, ev);
...
void GeomFill_ConstrainedFilling::MatchKnots() {
    ...
    if (nbadd) {
        ...
        if (BSplCLib::PrepareInsertKnots(degree[1], 0, knots[1]->Array1(), mults[1]->Array1(), addk, &addm, nbnp, nbnk,
                                         tolk, 0)) {
            ...
            BSplCLib::InsertKnots(degree[1], 0, curvpol[1]->Array1(), BSplCLib::NoWeights(), knots[1]->Array1(),
                                  mults[1]->Array1(), addk, &addm, ncpol[1]->ChangeArray1(), BSplCLib::NoWeights(),
                                  nk[1]->ChangeArray1(), nm[1]->ChangeArray1(), tolk, 0);

            BSplCLib::InsertKnots(degree[1], 0, curvpol[3]->Array1(), BSplCLib::NoWeights(), knots[1]->Array1(),
                                  mults[1]->Array1(), addk, &addm, ncpol[3]->ChangeArray1(), BSplCLib::NoWeights(),
                                  nk[1]->ChangeArray1(), nm[1]->ChangeArray1(), tolk, 0);
            if (!tgtepol[1].IsNull()) {
                ntpol[1] = new TColgp_HArray1OfPnt(1, nbnp);
                BSplCLib::InsertKnots(degree[1], 0, tgtepol[1]->Array1(), BSplCLib::NoWeights(), knots[1]->Array1(),
                                      mults[1]->Array1(), addk, &addm, ntpol[1]->ChangeArray1(), BSplCLib::NoWeights(),
                                      nk[1]->ChangeArray1(), nm[1]->ChangeArray1(), tolk, 0);
            }
            if (!tgtepol[3].IsNull()) {
                ntpol[3] = new TColgp_HArray1OfPnt(1, nbnp);
                BSplCLib::InsertKnots(degree[1], 0, tgtepol[3]->Array1(), BSplCLib::NoWeights(), knots[1]->Array1(),
                                      mults[1]->Array1(), addk, &addm, ntpol[3]->ChangeArray1(), BSplCLib::NoWeights(),
                                      nk[1]->ChangeArray1(), nm[1]->ChangeArray1(), tolk, 0);
            }
        }
        if (dom[0] != 1.) {
    ...
    if (nbadd) {
        ...
        TColStd_Array1OfInteger addm(madd[0], 1, nbadd);
        Standard_Integer nbnp, nbnk;
        if (BSplCLib::PrepareInsertKnots(degree[0], 0, knots[0]->Array1(), mults[0]->Array1(), addk, &addm, nbnp, nbnk,
                                         tolk, 0)) {
            ...
            BSplCLib::InsertKnots(degree[0], 0, curvpol[0]->Array1(), BSplCLib::NoWeights(), knots[0]->Array1(),
                                  mults[0]->Array1(), addk, &addm, ncpol[0]->ChangeArray1(), BSplCLib::NoWeights(),
                                  nk[0]->ChangeArray1(), nm[0]->ChangeArray1(), tolk, 0);

            BSplCLib::InsertKnots(degree[0], 0, curvpol[2]->Array1(), BSplCLib::NoWeights(), knots[0]->Array1(),
                                  mults[0]->Array1(), addk, &addm, ncpol[2]->ChangeArray1(), BSplCLib::NoWeights(),
                                  nk[0]->ChangeArray1(), nm[0]->ChangeArray1(), tolk, 0);
            if (!tgtepol[0].IsNull()) {
                ntpol[0] = new TColgp_HArray1OfPnt(1, nbnp);
                BSplCLib::InsertKnots(degree[0], 0, tgtepol[0]->Array1(), BSplCLib::NoWeights(), knots[0]->Array1(),
                                      mults[0]->Array1(), addk, &addm, ntpol[0]->ChangeArray1(), BSplCLib::NoWeights(),
                                      nk[0]->ChangeArray1(), nm[0]->ChangeArray1(), tolk, 0);
            }
            if (!tgtepol[2].IsNull()) {
                ntpol[2] = new TColgp_HArray1OfPnt(1, nbnp);
                BSplCLib::InsertKnots(degree[0], 0, tgtepol[2]->Array1(), BSplCLib::NoWeights(), knots[0]->Array1(),
                                      mults[0]->Array1(), addk, &addm, ntpol[2]->ChangeArray1(), BSplCLib::NoWeights(),
                                      nk[0]->ChangeArray1(), nm[0]->ChangeArray1(), tolk, 0);
            }
        }
        if (dom[1] != 1.) {
...
void GeomFill_ConstrainedFilling::PerformSurface() {
    ...
    surf = new Geom_BSplineSurface(temp, nk[0]->Array1(), nk[1]->Array1(), nm[0]->Array1(), nm[1]->Array1(), degree[0],
                                   degree[1]);
}
...
Standard_Integer GeomFill_ConstrainedFilling::Eval(const Standard_Real W, const Standard_Integer Ord,
...
void GeomFill_ConstrainedFilling::CheckApprox(const Standard_Integer I) {
    ...
    for (Standard_Integer iu = 0; iu <= nbp; iu++) {
        ...
        uu /= nbp;
        pbound = bou->Value(uu);
        BSplCLib::D0(uu, 0, degree[I % 2], 0, ncpol[I]->Array1(), BSplCLib::NoWeights(), nk[I % 2]->Array1(),
                     &nm[I % 2]->Array1(), papp);
        if (donor) {
            BSplCLib::D0(uu, 0, degree[I % 2], 0, ntpol[I]->Array1(), BSplCLib::NoWeights(), nk[I % 2]->Array1(),
                         &nm[I % 2]->Array1(), pbid);
