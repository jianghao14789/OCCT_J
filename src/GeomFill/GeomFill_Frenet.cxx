...
static gp_Vec DDeriv(const gp_Vec& F, const gp_Vec& DF, const gp_Vec& D2F) {
    ...
    gp_Vec Result =
        ...
        F * ((DF.SquareMagnitude() + F * D2F - 3 * (F * DF) * (F * DF) / (Norma * Norma)) / (Norma * Norma * Norma));
    return Result;
}
...
GeomFill_Frenet::GeomFill_Frenet() : isSngl(Standard_False) {}
...
void GeomFill_Frenet::Init() {
    ...
    Standard_Real TolF = 1.0e-10, Tol = 10 * TolF, Tol2 = Tol * Tol, PTol = Precision::PConfusion();
                    ...
                    if (Ext.IsDone()) {
                        for (Standard_Integer k = 1; k <= Ext.NbExt(); k++) {
                            ...
                            if (Value2 < Tol2) {
                                ...
                                if (t - SeqArray[i](j) > PTol && SeqArray[i](j + 1) - t > PTol) SnglSeq.Append(t);
                            }
                        }
                    }
                }
...
//=======================================================================
Standard_Boolean GeomFill_Frenet::RotateTrihedron(gp_Vec& Tangent, gp_Vec& Normal, gp_Vec& BiNormal,
    ...
    const gp_Vec aV11(anAddCAng * aPx * aPx + aCAng, anAddCAng * aPx * aPy - aPz * aSAng,
                      anAddCAng * aPx * aPz + aPy * aSAng);
    const gp_Vec aV12(anAddCAng * aPx * aPx + aCAng, anAddCAng * aPx * aPy + aPz * aSAng,
                      anAddCAng * aPx * aPz - aPy * aSAng);
    const gp_Vec aV21(anAddCAng * aPx * aPy + aPz * aSAng, anAddCAng * aPy * aPy + aCAng,
                      anAddCAng * aPy * aPz - aPx * aSAng);
    const gp_Vec aV22(anAddCAng * aPx * aPy - aPz * aSAng, anAddCAng * aPy * aPy + aCAng,
                      anAddCAng * aPy * aPz + aPx * aSAng);
    const gp_Vec aV31(anAddCAng * aPx * aPz - aPy * aSAng, anAddCAng * aPy * aPz + aPx * aSAng,
                      anAddCAng * aPz * aPz + aCAng);
    const gp_Vec aV32(anAddCAng * aPx * aPz + aPy * aSAng, anAddCAng * aPy * aPz - aPx * aSAng,
                      anAddCAng * aPz * aPz + aCAng);
...
Standard_Boolean GeomFill_Frenet::D0(const Standard_Real theParam, gp_Vec& Tangent, gp_Vec& Normal, gp_Vec& BiNormal) {
    const Standard_Real aTol = gp::Resolution();
    ...
    if (IsSingular(theParam, Index))
        if (SingularD0(theParam, Index, Tangent, Normal, BiNormal, Delta)) return Standard_True;
    ...
    if (Ndu <= aTol) {
        ...
        if (theParam - anUinfium < dPar) {
            if (D0(aParam + dPar, Tangent, Normal, BiNormal) == Standard_False) return Standard_False;
        } else {
            if (D0(aParam - dPar, Tangent, Normal, BiNormal) == Standard_False) return Standard_False;
        }
...
Standard_Boolean GeomFill_Frenet::D1(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent, gp_Vec& Normal,
                                     gp_Vec& DNormal, gp_Vec& BiNormal, gp_Vec& DBiNormal) {
    Standard_Integer Index;
    Standard_Real Delta = 0.;
...
Standard_Boolean GeomFill_Frenet::D2(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent, gp_Vec& D2Tangent,
                                     gp_Vec& Normal, gp_Vec& DNormal, gp_Vec& D2Normal, gp_Vec& BiNormal,
                                     gp_Vec& DBiNormal, gp_Vec& D2BiNormal) {
    Standard_Integer Index;
    Standard_Real Delta = 0.;
    if (IsSingular(Param, Index))
        if (SingularD2(Param, Index, Tangent, DTangent, D2Tangent, Normal, DNormal, D2Normal, BiNormal, DBiNormal,
                       D2BiNormal, Delta))
            return Standard_True;
...
Standard_Boolean GeomFill_Frenet::DoSingular(const Standard_Real U, const Standard_Integer Index, gp_Vec& Tangent,
                                             gp_Vec& BiNormal, Standard_Integer& n, Standard_Integer& k,
                                             Standard_Integer& TFlag, Standard_Integer& BNFlag, Standard_Real& Delta) {
...
Standard_Boolean GeomFill_Frenet::SingularD0(const Standard_Real Param, const Standard_Integer Index, gp_Vec& Tangent,
                                             gp_Vec& Normal, gp_Vec& BiNormal, Standard_Real& Delta) {
    Standard_Integer n, k, TFlag, BNFlag;
    if (!DoSingular(Param, Index, Tangent, BiNormal, n, k, TFlag, BNFlag, Delta)) return Standard_False;
...
Standard_Boolean GeomFill_Frenet::SingularD1(const Standard_Real Param, const Standard_Integer Index, gp_Vec& Tangent,
                                             gp_Vec& DTangent, gp_Vec& Normal, gp_Vec& DNormal, gp_Vec& BiNormal,
                                             gp_Vec& DBiNormal, Standard_Real& Delta) {
    Standard_Integer n, k, TFlag, BNFlag;
    if (!DoSingular(Param, Index, Tangent, BiNormal, n, k, TFlag, BNFlag, Delta)) return Standard_False;
...
Standard_Boolean GeomFill_Frenet::SingularD2(const Standard_Real Param, const Standard_Integer Index, gp_Vec& Tangent,
                                             gp_Vec& DTangent, gp_Vec& D2Tangent, gp_Vec& Normal, gp_Vec& DNormal,
    ...
    D2F = D2Tangent.Crossed(Dtmp1) + 2 * DTangent.Crossed(Dtmp2) + Tangent.Crossed(myTrimmed->DN(Param, k + 2));
    DBiNormal = FDeriv(F, DF);
