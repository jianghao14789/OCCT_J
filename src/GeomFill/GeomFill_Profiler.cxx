...
void GeomFill_Profiler::AddCurve(const Handle(Geom_Curve) & Curve) {
    ...
    if (C.IsNull()) C = GeomConvert::CurveToBSplineCurve(Curve);
    /*
    ...
    */
    ...
    if (myIsPeriodic && !C->IsPeriodic()) myIsPeriodic = Standard_False;
}
...
Standard_Integer GeomFill_Profiler::Degree() const {
    if (!myIsDone) throw StdFail_NotDone("GeomFill_Profiler::Degree");
...
void GeomFill_Profiler::Poles(const Standard_Integer Index, TColgp_Array1OfPnt& Poles) const {
    ...
    Standard_DomainError_Raise_if(Index < 1 || Index > mySequence.Length(), "GeomFill_Profiler::Poles");
...
void GeomFill_Profiler::Weights(const Standard_Integer Index, TColStd_Array1OfReal& Weights) const {
    ...
    Standard_DomainError_Raise_if(Index < 1 || Index > mySequence.Length(), "GeomFill_Profiler::Weights");
...
void GeomFill_Profiler::KnotsAndMults(TColStd_Array1OfReal& Knots, TColStd_Array1OfInteger& Mults) const {
    if (!myIsDone) throw StdFail_NotDone("GeomFill_Profiler::Degree");

#ifndef No_Exception
    Standard_Integer n = NbKnots();
#endif
    Standard_DomainError_Raise_if(Knots.Length() != n || Mults.Length() != n, "GeomFill_Profiler::KnotsAndMults");
