...
GeomFill_DiscreteTrihedron::GeomFill_DiscreteTrihedron() : myUseFrenet(Standard_False) {
...
void GeomFill_DiscreteTrihedron::Init() {
    ...
    for (i = 1; i <= myKnots->Length(); i++) {
        ...
        if (norm < TolConf) {
            ...
            if (subdelta < Precision::PConfusion()) subdelta = myKnots->Value(i + 1) - myKnots->Value(i);
...
Standard_Boolean GeomFill_DiscreteTrihedron::D0(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& Normal,
                                                gp_Vec& BiNormal) {
    if (myUseFrenet) {
        ...
        Index = I1;
        if (Abs(Param - myKnots->Value(I2)) < TolPar) Index = I2;
        ...
        else //<Param> is between knots
        {
            ...
            if (norm < TolConf) {
                ...
                if (subdelta < Precision::PConfusion()) subdelta = myKnots->Value(Index + 1) - Param;
...
Standard_Boolean GeomFill_DiscreteTrihedron::D1(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent,
...
Standard_Boolean GeomFill_DiscreteTrihedron::D2(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent,
                                                gp_Vec& D2Tangent, gp_Vec& Normal, gp_Vec& DNormal, gp_Vec& D2Normal,
                                                gp_Vec& BiNormal, gp_Vec& DBiNormal, gp_Vec& D2BiNormal) {
    if (myUseFrenet) {
        myFrenet->D2(Param, Tangent, DTangent, D2Tangent, Normal, DNormal, D2Normal, BiNormal, DBiNormal, D2BiNormal);
    } else {
        D0(Param, Tangent, Normal, BiNormal);
