...
static Standard_Boolean CheckSense(const TColGeom_SequenceOfCurve& Seq1, TColGeom_SequenceOfCurve& Seq2) {
    // initialisation
    ...
    for (Standard_Integer iseq = 2; iseq <= Seq1.Length(); iseq++) {
        ...
        Standard_Boolean ok = Standard_True,
                         pasnul1 = (Abs(alpha1) > Precision::Confusion()) && (Abs(beta1) > Precision::Confusion()),
                         pasnul2 = (Abs(alpha2) > Precision::Confusion()) && (Abs(beta2) > Precision::Confusion()),
                         pasnul3 = (Abs(alpha3) > Precision::Confusion()) && (Abs(beta3) > Precision::Confusion());
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom_Curve) & Path, const Handle(Geom_Curve) & FirstSect,
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom2d_Curve) & Path, const Handle(Geom_Surface) & Support,
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom_Curve) & Path, const TColGeom_SequenceOfCurve& NSections)
    : myIsDone(Standard_False), myExchUV(Standard_False), myKPart(Standard_False) {
    Init();
    Init(Path, NSections);
}
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom_Curve) & Path, const Handle(Geom_Curve) & Curve1,
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom_Curve) & Path, const Handle(Geom_Curve) & Curve1,
                             const Handle(Geom_Curve) & Curve2, const Standard_Real Radius)
    : myIsDone(Standard_False), myExchUV(Standard_False), myKPart(Standard_False) {
    Init();
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Adaptor3d_Curve) & Path, const Handle(Adaptor3d_Curve) & Curve1,
                             const Handle(Adaptor3d_Curve) & Curve2, const Standard_Real Radius)
    : myIsDone(Standard_False), myExchUV(Standard_False), myKPart(Standard_False) {
    Init();
...
GeomFill_Pipe::GeomFill_Pipe(const Handle(Geom_Curve) & Path, const Handle(Adaptor3d_Curve) & Guide,
                             const Handle(Geom_Curve) & FirstSect, const Standard_Boolean byACR,
...
void GeomFill_Pipe::Init(const Handle(Geom_Curve) & Path, const Handle(Adaptor3d_Curve) & Guide,
                         const Handle(Geom_Curve) & FirstSect, const Standard_Boolean byACR,
                         const Standard_Boolean rotat)
// Path : trajectoire
// Guide : courbe guide
...
// triedre : AC pour absc. curv. ou P pour plan ortho
{
    ...
    // loi de section
    mySec = new (GeomFill_UniformSection)(Place.Section(Standard_False), myAdpPath->FirstParameter(),
    ...
    if (rotat) TheLoc->Set(mySec, rotat, myAdpPath->FirstParameter(), myAdpPath->LastParameter(), 0., angle);
    myLoc = TheLoc;
}
...
void GeomFill_Pipe::Init(const Handle(Geom_Curve) & Path, const Handle(Geom_Curve) & FirstSect,
                         const GeomFill_Trihedron Option) {
...
void GeomFill_Pipe::Init(const Handle(Geom2d_Curve) & Path, const Handle(Geom_Surface) & Support,
                         const Handle(Geom_Curve) & FirstSect) {
    Handle(Geom_Curve) Sect;
...
#endif
    mySec = new (GeomFill_UniformSection)(Sect, myAdpPath->FirstParameter(), myAdpPath->LastParameter());
}
...
void GeomFill_Pipe::Init(const Handle(Adaptor3d_Curve) & Path, const Handle(Adaptor3d_Curve) & Curve1,
                         const Handle(Adaptor3d_Curve) & Curve2, const Standard_Real Radius) {
    myType = 4;
    myError = 0;
    myRadius = Radius;
...
void GeomFill_Pipe::Perform(const Standard_Boolean WithParameters, const Standard_Boolean Polynomial) {
...
// purpose  :
//=======================================================================
void GeomFill_Pipe::Perform(const Standard_Real Tol, const Standard_Boolean Polynomial, const GeomAbs_Shape Conti,
                            const Standard_Integer DegMax, const Standard_Integer NbMaxSegment) {
    ...
    if (myType == 4) {
        if (!KPartT4()) {
            Func = new (GeomFill_CircularBlendFunc)(myAdpPath, myAdpFirstSect, myAdpLastSect, myRadius, Polynomial);

            Approx_SweepApproximation App(Func);
            App.Perform(myAdpPath->FirstParameter(), myAdpPath->LastParameter(), Tol, Tol, 0., 0.01, TheConti, DegMax,
                        NbMaxSegment);
#ifdef OCCT_DEBUG
            std::cout << "Tuyau : ";
            App.Dump(std::cout);
            ...
            if (App.IsDone()) {
                mySurface =
                    new Geom_BSplineSurface(App.SurfPoles(), App.SurfWeights(), App.SurfUKnots(), App.SurfVKnots(),
...
Standard_Boolean GeomFill_Pipe::KPartT4() {
    ...
    if (myAdpPath->GetType() == GeomAbs_Line && myAdpFirstSect->GetType() == GeomAbs_Line &&
        ...
        if (Abs(V1.Dot(D0)) > Precision::Confusion() || Abs(V2.Dot(D0)) > Precision::Confusion()) return Ok;
        ...
        if (ZRef.Dot(D0) < 0.) Axis.YReverse();
        ...
        mySurface = new Geom_RectangularTrimmedSurface(mySurface, M_PI / 2., M_PI / 2. + Alpha,
    ...
    // -----------    Cas du tore  ----------------------------------
        ...
        if (Abs(Alp0 - Alp1) > Precision::Angular() || Abs(Alp0 - Alp2) > Precision::Angular()) return Ok;
        ...
        if (!D0.IsEqual(D1, Precision::Angular()) || !D1.IsEqual(D2, Precision::Angular())) return Ok;

        // les 3 ax1 doivent etre confondus.
        gp_Lin L(A0.Axis());
        if (!L.Contains(A1.Location(), Precision::Confusion()) || !L.Contains(A2.Location(), Precision::Confusion()))
            return Ok;
        ...
        if (Abs(V1.Dot(YRef)) > Precision::Confusion() || Abs(V2.Dot(YRef)) > Precision::Confusion()) return Ok;
        ...
        mySurface = new Geom_RectangularTrimmedSurface(new Geom_ToroidalSurface(T), myAdpPath->FirstParameter(),
                                                       myAdpPath->LastParameter(), VV1, VV2);
...
void GeomFill_Pipe::ApproxSurf(const Standard_Boolean WithParameters) {
    ...
    if (!App.IsDone()) {
        ...
        mySurface = new Geom_BSplineSurface(App.SurfPoles(), App.SurfWeights(), App.SurfUKnots(), App.SurfVKnots(),
                                            App.SurfUMults(), App.SurfVMults(), App.UDegree(), App.VDegree());
        Standard_Real t2d;
