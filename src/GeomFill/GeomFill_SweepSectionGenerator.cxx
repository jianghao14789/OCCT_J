...
GeomFill_SweepSectionGenerator::GeomFill_SweepSectionGenerator()
    : myRadius(0.0), myIsDone(Standard_False), myNbSections(0), myType(-1), myPolynomial(Standard_False) {}
...
void GeomFill_SweepSectionGenerator::Init(const Handle(Geom_Curve) & Path, const Standard_Real Radius) {
...
// purpose  :
//=======================================================================
void GeomFill_SweepSectionGenerator::Init(const Handle(Geom_Curve) & Path, const Handle(Geom_Curve) & FirstSect) {
    myIsDone = Standard_False;
    myRadius = 0;
    GeomAdaptor_Curve ThePath(Path);
...
void GeomFill_SweepSectionGenerator::Init(const Handle(Adaptor3d_Curve) & Path, const Handle(Adaptor3d_Curve) & Curve1,
...
void GeomFill_SweepSectionGenerator::Perform(const Standard_Boolean Polynomial) {
    ...
    if ((myType == 1) || (myType == 4)) {
        ...
        */
        Handle(Geom_TrimmedCurve) Circ = new Geom_TrimmedCurve(new Geom_Circle(CircleAxis, myRadius), 0., 2. * M_PI);
...
void GeomFill_SweepSectionGenerator::GetShape(Standard_Integer& NbPoles, Standard_Integer& NbKnots,
                                              Standard_Integer& Degree, Standard_Integer& NbPoles2d) const {
    /*
     if ( myType == 1) {
        NbPoles   = 7;
        NbKnots   = 4;
...
Standard_Boolean GeomFill_SweepSectionGenerator::Section(const Standard_Integer P, TColgp_Array1OfPnt& Poles,
    ...
    if (l < Epsilon(1.)) return Standard_False;
...
void GeomFill_SweepSectionGenerator::Section(const Standard_Integer P, TColgp_Array1OfPnt& Poles,
    ...
    if (myType != 0) {
        ...
        if (P > 1) {
            ...
            if ((myType == 3) || (myType == 6)) {
                for (Standard_Integer i = 1; i <= myFirstSect->NbPoles(); i++) {
                    ...
                    Weigths(i) = (myNbSections - P) * myFirstSect->Weight(i) + (P - 1) * myLastSect->Weight(i);
        ...
        Standard_Real U1 = (1 - Alpha) * myAdpFirstSect->FirstParameter() + Alpha * myAdpFirstSect->LastParameter();
        ...
        Standard_Real U2 = (1 - Alpha) * myAdpLastSect->FirstParameter() + Alpha * myAdpLastSect->LastParameter();
...
const gp_Trsf& GeomFill_SweepSectionGenerator::Transformation(const Standard_Integer Index) const {
    if (Index > myTrsfs.Length()) throw Standard_RangeError("GeomFill_SweepSectionGenerator::Transformation");
...
Standard_Real GeomFill_SweepSectionGenerator::Parameter(const Standard_Integer P) const {
    if (P == 1) {
        ...
        Standard_Real U1 = myPath->FirstParameter();
        Standard_Real U2 = myPath->LastParameter();
        Standard_Real prm = ((myNbSections - P) * U1 + (P - 1) * U2) / (Standard_Real)(myNbSections - 1);
        return prm;
    }
}
