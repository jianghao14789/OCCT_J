...
void GeomFill_LocationDraft::Prepare() {
    ...
    for (ii = 1; ii <= myNbPts; ii++) {
        ...
        } else {                                // au moins un point ou il n'y a pas intersection
            Intersec = Standard_False;
        }
...
Standard_Boolean GeomFill_LocationDraft::D0(const Standard_Real Param, gp_Mat& M, gp_Vec& V,
    ...
    if (Intersec == Standard_True) {
        ...
        if (ii < 2 * myNbPts && !IsEqual(myPoles2d->Value(ii).Coord(2), Param)) {
            ...
            U0 = myPoles2d->Value(ii - 1).Coord(1) * Paramt1 + myPoles2d->Value(ii + 1).Coord(1) * t2Param;
            V0 = myPoles2d->Value(ii - 1).Coord(2) * Paramt1 + myPoles2d->Value(ii + 1).Coord(2) * t2Param;
        } // if
...
//==================================================================
Standard_Boolean GeomFill_LocationDraft::D1(const Standard_Real Param, gp_Mat& M, gp_Vec& V, gp_Mat& DM, gp_Vec& DV,
    ...
    if (Intersec == Standard_True) { // la generatrice intersecte la surface d'arret
        // la generatrice
        D = Cos(myAngle) * B + Sin(myAngle) * N;
...
//==================================================================
Standard_Boolean GeomFill_LocationDraft::D2(const Standard_Real Param, gp_Mat& M, gp_Vec& V, gp_Mat& DM, gp_Vec& DV,
                                            gp_Mat& D2M, gp_Vec& D2V, TColgp_Array1OfPnt2d& Poles2d,
...
// purpose  :
//==================================================================
void GeomFill_LocationDraft::Resolution(const Standard_Integer Index, const Standard_Real Tol, Standard_Real& TolU,
