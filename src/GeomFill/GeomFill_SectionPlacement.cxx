...
// Purpose :
//===============================================================
static void Tangente(const Adaptor3d_Curve& Path, const Standard_Real Param, gp_Pnt& P, gp_Vec& Tang) {
...
//===============================================================
static void DistMini(const Extrema_ExtPC& Ext, const Adaptor3d_Curve& C, Standard_Real& Dist, Standard_Real& Param) {
...
GeomFill_SectionPlacement::GeomFill_SectionPlacement(const Handle(GeomFill_LocationLaw) & L,
    ...
    : myLaw(L), /* myAdpSection(Section),  mySection(Section), */
      SecParam(0.0), PathParam(0.0), Dist(RealLast()), AngleMax(0.) {
    ...
    if (!myIsPoint) {
        gp_Pnt P;
        gp_Vec V;
        Tangente(myAdpSection, (myAdpSection.FirstParameter() + myAdpSection.LastParameter()) / 2, P, V);
        ...
        if (!isplan && NbPoles > 2) {
            ...
            if (myAdpSection.IsPeriodic()) {
                ...
                if (Abs(first - U1) <= Precision::PConfusion()) first = U1;
                if (Abs(last - U2) <= Precision::PConfusion()) last = U2;
            }
            ...
            if (myAdpSection.GetType() == GeomAbs_BSplineCurve) {
                ...
                if (I1 != I2) NbPnts += NbLocalPnts;
                if (I3 != I4 && first < BC->Knot(I3)) NbPnts += NbLocalPnts;
                if (!myAdpSection.IsClosed()) NbPnts++;
                ...
                if (!myAdpSection.IsClosed()) Pnts->SetValue(nb, myAdpSection.Value(last));
            } else // other type
            {
                ...
                if (!myAdpSection.IsClosed()) Pnts->SetValue(NbPnts, myAdpSection.Value(last));
            }
        ...
        myExt.Initialize(myAdpSection, myAdpSection.FirstParameter(), myAdpSection.LastParameter(),
                         Precision::Confusion());
    }
}
...
//===============================================================
void GeomFill_SectionPlacement::Perform(const Handle(Adaptor3d_Curve) & Path, const Standard_Real Tol) {
    ...
    if (myIsPoint) {
        ...
        if (isplan) {
            ...
            if (DistPlan <= IntTol) DistCenter = V1.Magnitude();
        ...
        if (!isplan) {
            ...
            if (!Trouve) {
                Extrema_ExtCC Ext(*Path, myAdpSection, Path->FirstParameter(), Path->LastParameter(),
                                  myAdpSection.FirstParameter(), myAdpSection.LastParameter(),
                                  Path->Resolution(Tol / 100), myAdpSection.Resolution(Tol / 100));
                if (Ext.IsDone()) {
                ...
                if (!Trouve) {
                    ...
                    Extrema_ExtPC PExt;
                    PExt.Initialize(*Path, Path->FirstParameter(), Path->LastParameter(), Precision::Confusion());
                    PExt.Perform(PonSec);
                    if (PExt.IsDone()) {
...
gp_Trsf GeomFill_SectionPlacement::Transformation(const Standard_Boolean WithTranslation,
    ...
    if (WithCorrection && !myIsPoint) {
        ...
        if (!isplan) throw Standard_Failure("Illegal usage: can't rotate non-planar profile");
