...
public:
    GeomFill_Sweep_Eval(GeomFill_LocFunction& theTool) : theAncore(theTool) {}
...
// Purpose :
//===============================================================
GeomFill_Sweep::GeomFill_Sweep(const Handle(GeomFill_LocationLaw) & Location, const Standard_Boolean WithKpart) {
    done = Standard_False;
...
// Purpose :
//===============================================================
void GeomFill_Sweep::SetDomain(const Standard_Real LocFirst, const Standard_Real LocLast,
                               const Standard_Real SectionFirst, const Standard_Real SectionLast) {
    First = LocFirst;
    Last = LocLast;
    SFirst = SectionFirst;
    SLast = SectionLast;
...
// Purpose :
//===============================================================
void GeomFill_Sweep::SetTolerance(const Standard_Real Tolerance3d, const Standard_Real BoundTolerance,
                                  const Standard_Real Tolerance2d, const Standard_Real ToleranceAngular) {
    Tol3d = Tolerance3d;
...
// Purpose :
//===============================================================
void GeomFill_Sweep::Build(const Handle(GeomFill_SectionLaw) & Section, const GeomFill_ApproxStyle Methode,
...
// Purpose :
//===============================================================
Standard_Boolean GeomFill_Sweep::BuildAll(const GeomAbs_Shape Continuity, const Standard_Integer Degmax,
                                          const Standard_Integer Segmax) {
    Standard_Boolean Ok = Standard_False;
    ...
    if (Approx.IsDone()) {
        ...
        mySurface = new (Geom_BSplineSurface)(Poles, Weights, UKnots, VKnots, UMults, VMults, Approx.UDegree(),
                                              Approx.VDegree(), mySec->IsUPeriodic());
        ...
        if (myCurve2d.IsNull()) {
            ...
            for (ii = ideb, kk = 1; ii <= ifin; ii++, kk++) {
                Handle(Geom2d_BSplineCurve) C = new (Geom2d_BSplineCurve)(
                    Approx.Curve2dPoles(kk), Approx.Curves2dKnots(), Approx.Curves2dMults(), Approx.Curves2dDegree());
...
// Purpose : A venir...
//===============================================================
Standard_Boolean GeomFill_Sweep::BuildProduct(const GeomAbs_Shape Continuity, const Standard_Integer Degmax,
                                              const Standard_Integer Segmax) {
    Standard_Boolean Ok = Standard_False;

    Handle(Geom_BSplineSurface) BSurf;
    BSurf = Handle(Geom_BSplineSurface)::DownCast(mySec->BSplineSurface()->Copy());
    ...
    AdvApprox_ApproxAFunction Approx(0, 0, 4, ThreeDTol, ThreeDTol, ThreeDTol, First, Last, Continuity, Degmax, Segmax,
                                     eval, Preferentiel);
#ifdef OCCT_DEBUG
    Approx.Dump(std::cout);
#endif
...
static Standard_Boolean IsSweepParallelSpine(const Handle(GeomFill_LocationLaw) & theLoc,
                                             const Handle(GeomFill_SectionLaw) & theSec, const Standard_Real theTol) {
    ...
    TfBegin.SetValues(GTfBegin(1, 1), GTfBegin(1, 2), GTfBegin(1, 3), GTfBegin(1, 4), GTfBegin(2, 1), GTfBegin(2, 2),
                      GTfBegin(2, 3), GTfBegin(2, 4), GTfBegin(3, 1), GTfBegin(3, 2), GTfBegin(3, 3), GTfBegin(3, 4));
    ...
    TfEnd.SetValues(GTfEnd(1, 1), GTfEnd(1, 2), GTfEnd(1, 3), GTfEnd(1, 4), GTfEnd(2, 1), GTfEnd(2, 2), GTfEnd(2, 3),
                    GTfEnd(2, 4), GTfEnd(3, 1), GTfEnd(3, 2), GTfEnd(3, 3), GTfEnd(3, 4));
...
Standard_Boolean GeomFill_Sweep::BuildKPart() {
    ...
    if (myLoc->IsTranslation(error)) {
        ...
        try { // Pas joli mais il n'y as pas d'autre moyens de tester SetValues
            OCC_CATCH_SIGNALS
            Tf2.SetValues(Tf(1, 1), Tf(1, 2), Tf(1, 3), Tf(1, 4), Tf(2, 1), Tf(2, 2), Tf(2, 3), Tf(2, 4), Tf(3, 1),
                          Tf(3, 2), Tf(3, 3), Tf(3, 4));
        ...
        if (mySec->IsConstant(error)) {
            ...
            if ((SectionType == GeomAbs_Line) && IsTrsf) {
                ...
                if (!IsSweepParallelSpine(myLoc, mySec, Tol)) return Standard_False;
...
#ifdef OCCT_DEBUG
        if (isUPeriodic && !mySurface->IsUPeriodic()) std::cout << "Pb de periodicite en U" << std::endl;
        if (isUPeriodic && !mySurface->IsUClosed()) std::cout << "Pb de fermeture en U" << std::endl;
        if (isVPeriodic && !mySurface->IsVPeriodic()) std::cout << "Pb de periodicite en V" << std::endl;
        if (isVPeriodic && !mySurface->IsVClosed()) std::cout << "Pb de fermeture en V" << std::endl;
#endif
    }
...
// Purpose :
//===============================================================
void GeomFill_Sweep::ErrorOnRestriction(const Standard_Boolean IsFirst, Standard_Real& UError,
...
// Purpose :
//===============================================================
void GeomFill_Sweep::ErrorOnTrace(const Standard_Integer IndexOfTrace, Standard_Real& UError,
                                  Standard_Real& VError) const {
    Standard_Integer ind = IndexOfTrace + 1;
    if (IndexOfTrace > myLoc->TraceNumber()) throw Standard_OutOfRange(" GeomFill_Sweep::ErrorOnTrace");
...
Handle(Geom2d_Curve) GeomFill_Sweep::Restriction(const Standard_Boolean IsFirst) const {
    if (IsFirst) return myCurve2d->Value(1);
...
Handle(Geom2d_Curve) GeomFill_Sweep::Trace(const Standard_Integer IndexOfTrace) const {
    Standard_Integer ind = IndexOfTrace + 1;
    if (IndexOfTrace > myLoc->TraceNumber()) throw Standard_OutOfRange(" GeomFill_Sweep::Trace");
