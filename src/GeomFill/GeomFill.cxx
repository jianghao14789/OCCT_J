...
// purpose  :
//=======================================================================
Handle(Geom_Surface) GeomFill::Surface(const Handle(Geom_Curve) & Curve1, const Handle(Geom_Curve) & Curve2)

{
    ...
    if (TheCurve1->IsKind(STANDARD_TYPE(Geom_Line)) && TheCurve2->IsKind(STANDARD_TYPE(Geom_Line)) && Trim1 && Trim2) {
        ...
        if (D1.IsParallel(D2, Precision::Angular())) {
            ...
            if (D1.IsEqual(D2, Precision::Angular())) {
                if (Abs(a1 - proj - a2) <= Precision::Confusion() && Abs(b1 - proj - b2) <= Precision::Confusion()) {
            ...
            if (D1.IsOpposite(D2, Precision::Angular())) {
                if (Abs(a1 - proj + b2) <= Precision::Confusion() && Abs(b1 - proj + a2) <= Precision::Confusion()) {
                    gp_Ax3 Ax(L1.Location(), gp_Dir(D1.Crossed(P1P2)), D1);
                    Handle(Geom_Plane) P = new Geom_Plane(Ax);
                    Standard_Real V = P1P2.Dot(Ax.YDirection());
                    Surf = new Geom_RectangularTrimmedSurface(P, a1, b1, Min(0., V), Max(0., V));
    ...
    else if (TheCurve1->IsKind(STANDARD_TYPE(Geom_Circle)) && TheCurve2->IsKind(STANDARD_TYPE(Geom_Circle))) {
...
void GeomFill::GetShape(const Standard_Real MaxAng, Standard_Integer& NbPoles, Standard_Integer& NbKnots,
                        Standard_Integer& Degree, Convert_ParameterisationType& TConv) {
    switch (TConv) {
...
void GeomFill::GetMinimalWeights(const Convert_ParameterisationType TConv, const Standard_Real MinAng,
                                 const Standard_Real MaxAng, TColStd_Array1OfReal& Weights)
...
Standard_Real GeomFill::GetTolerance(const Convert_ParameterisationType TConv, const Standard_Real AngleMin,
                                     const Standard_Real Radius, const Standard_Real AngularTol,
...
void GeomFill::GetCircle(const Convert_ParameterisationType TConv,
                         ...
                         const gp_Pnt& Center, TColgp_Array1OfPnt& Poles, TColStd_Array1OfReal& Weights) {
...
Standard_Boolean GeomFill::GetCircle(const Convert_ParameterisationType TConv, const gp_Vec& ns1, const gp_Vec& ns2,
                                     const gp_Vec& dn1w, const gp_Vec& dn2w, const gp_Vec& nplan, const gp_Vec& dnplan,
                                     const gp_Pnt& pts1, const gp_Pnt& pts2, const gp_Vec& tang1, const gp_Vec& tang2,
                                     const Standard_Real Rayon, const Standard_Real DRayon, const gp_Pnt& Center,
                                     const gp_Vec& DCenter, TColgp_Array1OfPnt& Poles, TColgp_Array1OfVec& DPoles,
    ...
    if (Abs(Sina) > Abs(Cosa)) {
        ...
        DAngle = (dnplan.Dot(ns1.Crossed(ns2)) + nplan.Dot(dn1w.Crossed(ns2) + ns1.Crossed(dn2w))) / Cosa;
    }
    ...
    switch (TConv) {
        case Convert_QuasiAngular: {
            GeomFill_QuasiAngularConvertor QConvertor;
            QConvertor.Init();
            QConvertor.Section(pts1, tang1, Center, DCenter, nplan, dnplan, Angle, DAngle, Poles, DPoles, Weights,
                               DWeights);
        ...
        default:
            // Cas rationel classique
            {
                ...
                for (i = 1, jj = low; i <= NbSpan; i++, jj += 2) {
                    ...
                    DPoles(jj + 1).SetLinearForm(Dlambda, temp, 1. - 2 * lambda, DCenter, lambda,
...
Standard_Boolean GeomFill::GetCircle(const Convert_ParameterisationType TConv, const gp_Vec& ns1, const gp_Vec& ns2,
                                     const gp_Vec& dn1w, const gp_Vec& dn2w, const gp_Vec& d2n1w, const gp_Vec& d2n2w,
                                     const gp_Vec& nplan, const gp_Vec& dnplan, const gp_Vec& d2nplan,
                                     const gp_Pnt& pts1, const gp_Pnt& pts2, const gp_Vec& tang1, const gp_Vec& tang2,
                                     const gp_Vec& Dtang1, const gp_Vec& Dtang2, const Standard_Real Rayon,
                                     const Standard_Real DRayon, const Standard_Real D2Rayon, const gp_Pnt& Center,
                                     const gp_Vec& DCenter, const gp_Vec& D2Center, TColgp_Array1OfPnt& Poles,
    ...
    if (Abs(Sina) > Abs(Cosa)) {
        ...
        D2Angle =
            ...
            aux * (dnplan.Dot(ns1.Crossed(ns2)) + nplan.Dot(dn1w.Crossed(ns2) + ns1.Crossed(dn2w))) / (Sina * Sina);
    } else {
        ...
        D2Angle = (d2nplan.Dot(ns1.Crossed(ns2)) + 2 * dnplan.Dot(temp) +
                   nplan.Dot(d2n1w.Crossed(ns2) + 2 * dn1w.Crossed(dn2w) + ns1.Crossed(d2n2w))) /
                      Cosa -
                  (dn1w.Dot(ns2) + ns1.Dot(dn2w)) * (dnplan.Dot(ns1.Crossed(ns2)) + nplan.Dot(temp)) / (Cosa * Cosa);
    }
    ...
    switch (TConv) {
        case Convert_QuasiAngular: {
            GeomFill_QuasiAngularConvertor QConvertor;
            QConvertor.Init();
            QConvertor.Section(pts1, tang1, Dtang1, Center, DCenter, D2Center, nplan, dnplan, d2nplan, Angle, DAngle,
                               D2Angle, Poles, DPoles, D2Poles, Weights, DWeights, D2Weights);
            return Standard_True;
        }
        case Convert_Polynomial: {
            GeomFill_PolynomialConvertor PConvertor;
            PConvertor.Init();
            PConvertor.Section(pts1, tang1, Dtang1, Center, DCenter, D2Center, nplan, dnplan, d2nplan, Angle, DAngle,
                               D2Angle, Poles, DPoles, D2Poles);
            Weights.Init(1);
            DWeights.Init(0);
            D2Weights.Init(0);
            return Standard_True;
        ...
        default: {
            ...
            for (i = 1, jj = low; i <= NbSpan; i++, jj += 2) {
                ...
                D2Poles(jj + 1).SetLinearForm(D2lambda, temp, 2 * Dlambda, dtemp, lambda,
