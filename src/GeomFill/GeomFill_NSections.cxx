...
static Standard_Boolean verifD1(const TColgp_Array1OfPnt& P1, const TColStd_Array1OfReal& W1,
                                const TColgp_Array1OfPnt& P2, const TColStd_Array1OfReal& W2,
                                const TColgp_Array1OfVec& DPoles, const TColStd_Array1OfReal& DWeights,
                                const Standard_Real pTol, const Standard_Real wTol, const Standard_Real pas) {
    ...
    for (ii = 1; ii <= L; ii++) {
        ...
        if (diff.Magnitude() > pTol) {
            if (Affich) {
                ...
                std::cout << "par diff finies : (" << dP.X() << " " << dP.Y() << " " << dP.Z() << ")" << std::endl;
                std::cout << "resultat obtenu : (" << DPoles(ii).X() << " " << DPoles(ii).Y() << " " << DPoles(ii).Z()
                          << ")" << std::endl;
            }
            ok = Standard_False;
        }
...
static Standard_Boolean verifD2(const TColgp_Array1OfVec& DP1, const TColStd_Array1OfReal& DW1,
                                const TColgp_Array1OfVec& DP2, const TColStd_Array1OfReal& DW2,
                                const TColgp_Array1OfVec& D2Poles, const TColStd_Array1OfReal& D2Weights,
                                const Standard_Real pTol, const Standard_Real wTol, const Standard_Real pas) {
    Standard_Boolean ok = Standard_True;
    ...
    for (ii = 1; ii <= L; ii++) {
        ...
        if (diff.Magnitude() > pTol) {
            if (Affich) {
                ...
                std::cout << "par diff finies : (" << d2P.X() << " " << d2P.Y() << " " << d2P.Z() << ")" << std::endl;
...
static void ResultEval(const Handle(Geom_BSplineSurface) & surf, const Standard_Real V, const Standard_Integer deriv,
...
GeomFill_NSections::GeomFill_NSections(const TColGeom_SequenceOfCurve& NC, const TColStd_SequenceOfReal& NP) {
    mySections = NC;
    myParams = NP;
    UFirst = 0.;
    ULast = 1.;
...
// purpose  :
//=======================================================================
GeomFill_NSections::GeomFill_NSections(const TColGeom_SequenceOfCurve& theNC, const TColStd_SequenceOfReal& theNP,
...
GeomFill_NSections::GeomFill_NSections(const TColGeom_SequenceOfCurve& NC, const TColStd_SequenceOfReal& NP,
                                       const Standard_Real UF, const Standard_Real UL, const Standard_Real VF,
...
GeomFill_NSections::GeomFill_NSections(const TColGeom_SequenceOfCurve& NC, const GeomFill_SequenceOfTrsf& Trsfs,
...
// Purpose :D0
//=======================================================
Standard_Boolean GeomFill_NSections::D0(const Standard_Real V, TColgp_Array1OfPnt& Poles,
                                        TColStd_Array1OfReal& Weights) {
    if (mySurface.IsNull()) {
        return Standard_False;
    } else {
        Handle(Geom_BSplineCurve) Curve = Handle(Geom_BSplineCurve)::DownCast(mySurface->VIso(V, Standard_False));
...
// Purpose :D1
//=======================================================
Standard_Boolean GeomFill_NSections::D1(const Standard_Real V, TColgp_Array1OfPnt& Poles, TColgp_Array1OfVec& DPoles,
...
// Purpose :D2
//=======================================================
Standard_Boolean GeomFill_NSections::D2(const Standard_Real V, TColgp_Array1OfPnt& Poles, TColgp_Array1OfVec& DPoles,
                                        TColgp_Array1OfVec& D2Poles, TColStd_Array1OfReal& Weights,
    ...
    for (ii = 1; ii <= L && (!NullWeight); ii++) {
        ...
        if (rational) {
            ...
            if (ww < EpsW) {
                ...
                D2Poles(ii).SetXYZ(
                    (D2Poles(ii).XYZ() - D2Weights(ii) * Poles(ii).Coord() - 2 * DWeights(ii) * DPoles(ii).XYZ()) / ww);
            }
        }
        indice += gap;
    }
...
void GeomFill_NSections::ComputeSurface() {
    ...
    if (myRefSurf.IsNull()) {
        ...
        BS = new Geom_BSplineSurface(anApprox.SurfPoles(), anApprox.SurfWeights(), anApprox.SurfUKnots(),
                                     anApprox.SurfVKnots(), anApprox.SurfUMults(), anApprox.SurfVMults(),
                                     anApprox.UDegree(), anApprox.VDegree(), section.IsPeriodic());
    }
        ...
        if (Abs(Ui1 - myRefSurf->UKnot(i1)) <= Precision::PConfusion()) Ui1 = myRefSurf->UKnot(i1);
        if (Abs(Ui1 - myRefSurf->UKnot(i2)) <= Precision::PConfusion()) Ui1 = myRefSurf->UKnot(i2);
        myRefSurf->LocateU(Ui2, Precision::PConfusion(), i1, i2);
        if (Abs(Ui2 - myRefSurf->UKnot(i1)) <= Precision::PConfusion()) Ui2 = myRefSurf->UKnot(i1);
        if (Abs(Ui2 - myRefSurf->UKnot(i2)) <= Precision::PConfusion()) Ui2 = myRefSurf->UKnot(i2);
...
//=======================================================
void GeomFill_NSections::SectionShape(Standard_Integer& NbPoles, Standard_Integer& NbKnots,
                                      Standard_Integer& Degree) const {
    if (mySurface.IsNull()) return;
...
void GeomFill_NSections::Knots(TColStd_Array1OfReal& TKnots) const {
    if (!mySurface.IsNull()) mySurface->UKnots(TKnots);
}
...
void GeomFill_NSections::Mults(TColStd_Array1OfInteger& TMults) const {
    if (!mySurface.IsNull()) mySurface->UMultiplicities(TMults);
}
...
Standard_Boolean GeomFill_NSections::IsRational() const {
    if (!mySurface.IsNull()) return mySurface->IsURational();
...
Standard_Boolean GeomFill_NSections::IsUPeriodic() const {
    if (!mySurface.IsNull()) return mySurface->IsUPeriodic();
...
Standard_Boolean GeomFill_NSections::IsVPeriodic() const {
    if (!mySurface.IsNull()) return mySurface->IsVPeriodic();
...
//=======================================================
void GeomFill_NSections::GetTolerance(const Standard_Real BoundTol, const Standard_Real SurfTol,
                                      //                                            const Standard_Real AngleTol,
                                      const Standard_Real, TColStd_Array1OfReal& Tol3d) const {
    Tol3d.Init(SurfTol);
    if (BoundTol < SurfTol) {
...
gp_Pnt GeomFill_NSections::BarycentreOfSurf() const {
    ...
    if (mySurface.IsNull()) return Bary;
...
Standard_Boolean GeomFill_NSections::IsConicalLaw(Standard_Real& Error) const {
    ...
    if (isconic) {
        ...
        if (isconic) {
            gp_Circ C1 = AC1.Circle();
            if (!myTrsfs.IsEmpty()) C1.Transform(myTrsfs(1).Inverted());
            gp_Circ C2 = AC2.Circle();
            if (!myTrsfs.IsEmpty()) C2.Transform(myTrsfs(2).Inverted());
            Standard_Real Tol = 1.e-7;
