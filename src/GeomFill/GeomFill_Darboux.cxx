...
//=======================================================================
static void NormalD0(const Standard_Real U, const Standard_Real V, const Handle(Adaptor3d_Surface) & Surf,
                     gp_Dir& Normal, Standard_Integer& OrderU, Standard_Integer& OrderV) {
    //  gp_Vec D1U,D1V,D2U,D2V,DUV;
    gp_Vec D1U, D1V;
    GeomAbs_Shape Cont = (Surf->UContinuity() < Surf->VContinuity()) ? (Surf->UContinuity()) : (Surf->VContinuity());
    OrderU = OrderV = 0;
#ifdef CHECK
    ...
    if (NStatus != CSLib_Defined) {
        ...
        if (NStatus != CSLib_Defined) {
            ...
            for (i = 0; i <= MaxOrder; i++)
                for (j = 0; j <= MaxOrder; j++) {
                    std::cout << "(" << i << "," << j << ") = " << DerSurf(i, j).X() << "," << DerSurf(i, j).Y() << ","
                              << DerSurf(i, j).Z() << std::endl;
                }
#endif
            throw Geom_UndefinedValue();
        }
...
//=======================================================================
static void NormalD1(const Standard_Real U, const Standard_Real V, const Handle(Adaptor3d_Surface) & Surf,
                     gp_Dir& Normal, gp_Vec& D1UNormal, gp_Vec& D1VNormal) {
#ifdef CHECK
...
//=======================================================================
static void NormalD2(const Standard_Real U, const Standard_Real V, const Handle(Adaptor3d_Surface) & Surf,
                     gp_Dir& Normal, gp_Vec& D1UNormal, gp_Vec& D1VNormal, gp_Vec& D2UNormal, gp_Vec& D2VNormal,
                     gp_Vec& D2UVNormal) {
#ifdef CHECK
    GeomAbs_Shape Cont = (Surf->Surface().UContinuity() < Surf->Surface().VContinuity())
                             ? (Surf->Surface().UContinuity())
...
Standard_Boolean GeomFill_Darboux::D1(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent, gp_Vec& Normal,
    ...
    DF = D2_2d.X() * dS_du + D2_2d.Y() * dS_dv + d2S_du * D2d.X() * D2d.X() + 2 * d2S_duv * D2d.X() * D2d.Y() +
         d2S_dv * D2d.Y() * D2d.Y();
...
Standard_Boolean GeomFill_Darboux::D2(const Standard_Real Param, gp_Vec& Tangent, gp_Vec& DTangent, gp_Vec& D2Tangent,
    ...
    mySupport->D3(C2d.X(), C2d.Y(), S, dS_du, dS_dv, d2S_du, d2S_dv, d2S_duv, d3S_du, d3S_dv, d3S_duuv, d3S_duvv);
    //  if(D2d.Magnitude() <= Precision::Confusion())
    //    DoTSingular(Param, Order);
    ...
    D2F = D3_2d.X() * dS_du + D3_2d.Y() * dS_dv + D2_2d.X() * (D2d.X() * d2S_du + D2d.Y() * d2S_duv) +
          ...
          2 * (D2d.X() * D2_2d.X() * d2S_du + D2d.Y() * D2_2d.Y() * d2S_dv +
    ...
    NormalD2(C2d.X(), C2d.Y(), mySupport, NormalDir, D1UNormal, D1VNormal, D2UNormal, D2VNormal, D2UVNormal);
    BiNormal.SetXYZ(NormalDir.XYZ());
    DBiNormal = D1UNormal * D2d.X() + D1VNormal * D2d.Y();
