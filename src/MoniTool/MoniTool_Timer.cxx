...
void MoniTool_Timer::Dump(Standard_OStream& ostr) {
    ...
    char buff[1024];
    Sprintf(buff, "Elapsed:%6.1f sec, CPU User:%9.4f sec, CPU Sys:%9.4f sec, hits: %d", elapsed, user, system, myCount);
...
Handle(MoniTool_Timer) MoniTool_Timer::Timer(const Standard_CString name) {
    ...
    if (dic.IsBound(name)) return dic.Find(name);
...
void MoniTool_Timer::ComputeAmendments() {
    ...
    std::cout << "Access: " << amAccess << ", External: " << amExternal << ", Internal: " << amInternal
              << ", Error: " << amError << std::endl;
}
...
void MoniTool_Timer::GetAmendments(Standard_Real& access, Standard_Real& internal, Standard_Real& external,
