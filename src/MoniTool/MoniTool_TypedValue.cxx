...
MoniTool_TypedValue::MoniTool_TypedValue(const Standard_CString name, const MoniTool_ValueType type,
                                         const Standard_CString init)
    : thename(name), thetype(type), thelims(0), themaxlen(0), theintlow(0), theintup(-1), therealow(0.0),
      therealup(0.0), theinterp(NULL), thesatisf(NULL), theival(0), thehval(new TCollection_HAsciiString("")) {
...
MoniTool_TypedValue::MoniTool_TypedValue(const Handle(MoniTool_TypedValue) & other)
    : thename(other->Name()), thedef(other->Definition()), thelabel(other->Label()), thetype(other->ValueType()),
      theotyp(other->ObjectType()), thelims(0), themaxlen(other->MaxLength()), theintlow(0), theintup(0), therealow(0),
      therealup(0), theunidef(other->UnitDef()), theival(other->IntegerValue()), thehval(other->HStringValue()),
...
void MoniTool_TypedValue::Internals(MoniTool_ValueInterpret& interp, MoniTool_ValueSatisfies& satisf,
                                    Standard_CString& satisname,
...
void MoniTool_TypedValue::PrintValue(Standard_OStream& S) const {
    if (IsSetValue()) {
        if (thetype == MoniTool_ValueIdent) S << " (type) " << theoval->DynamicType()->Name();
        if (!thehval.IsNull()) S << (thetype == MoniTool_ValueIdent ? " : " : "") << thehval->ToCString();
...
Standard_Boolean MoniTool_TypedValue::IntegerLimit(const Standard_Boolean max, Standard_Integer& val) const {
...
Standard_Boolean MoniTool_TypedValue::RealLimit(const Standard_Boolean max, Standard_Real& val) const {
    Standard_Boolean res = Standard_False;
    if (max) {
        res = (thelims & 2) != 0;
...
void MoniTool_TypedValue::AddEnum(const Standard_CString v1, const Standard_CString v2, const Standard_CString v3,
                                  const Standard_CString v4, const Standard_CString v5, const Standard_CString v6,
                                  const Standard_CString v7, const Standard_CString v8, const Standard_CString v9,
    ...
    if (theenums.IsNull())
        ...
        Handle(TColStd_HArray1OfAsciiString) enums = new TColStd_HArray1OfAsciiString(theintlow, theintup + 10);
...
void MoniTool_TypedValue::AddEnumValue(const Standard_CString val, const Standard_Integer num) {
    ...
    if (theenums.IsNull())
        ...
        Handle(TColStd_HArray1OfAsciiString) enums = new TColStd_HArray1OfAsciiString(theintlow, num + 1);
        for (Standard_Integer i = theintlow; i <= theintup; i++)
            enums->SetValue(i, theenums->Value(i));
        theenums = enums;
    }
...
Standard_Boolean MoniTool_TypedValue::EnumDef(Standard_Integer& startcase, Standard_Integer& endcase,
...
void MoniTool_TypedValue::SetSatisfies(const MoniTool_ValueSatisfies func, const Standard_CString name) {
    thesatisn.Clear();
    thesatisf = func;
...
Handle(TCollection_HAsciiString) MoniTool_TypedValue::Interpret(const Handle(TCollection_HAsciiString) & hval,
...
Standard_Boolean MoniTool_TypedValue::AddLib(const Handle(MoniTool_TypedValue) & tv, const Standard_CString defin) {
