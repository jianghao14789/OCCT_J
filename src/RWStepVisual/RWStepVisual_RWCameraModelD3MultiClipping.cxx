...
// purpose  :
//=======================================================================
void RWStepVisual_RWCameraModelD3MultiClipping::ReadStep(const Handle(StepData_StepReaderData) & data,
                                                         const Standard_Integer num, Handle(Interface_Check) & ach,
                                                         const Handle(StepVisual_CameraModelD3MultiClipping) &
                                                             ent) const {
    // Number of Parameter Control
    if (!data->CheckNbParams(num, 4, ach, "camera_model_d3_multi_clipping")) return;
    ...
    data->ReadEntity(num, 2, "view_reference_system", ach, STANDARD_TYPE(StepGeom_Axis2Placement3d),
    ...
    if (data->ReadSubList(num, 4, "shape_clipping", ach, nbSub)) {
        ...
        for (Standard_Integer i = 1; i <= nbElements; i++) {
            if (data->ReadEntity(nbSub, i, "shape_clipping", ach, anEnt)) aShapeClipping->SetValue(i, anEnt);
        }
    }
...
// purpose  :
//=======================================================================
void RWStepVisual_RWCameraModelD3MultiClipping::WriteStep(StepData_StepWriter& SW,
                                                          const Handle(StepVisual_CameraModelD3MultiClipping) &
                                                              ent) const {
    // Inherited field name
    SW.Send(ent->Name());
...
// purpose  :
//=======================================================================
void RWStepVisual_RWCameraModelD3MultiClipping::Share(const Handle(StepVisual_CameraModelD3MultiClipping) & ent,
