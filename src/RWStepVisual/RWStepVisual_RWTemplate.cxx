...
void RWStepVisual_RWTemplate::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
                                       Handle(Interface_Check) & ach, const Handle(StepVisual_Template) & ent) const {

    // --- Number of Parameter Control ---

    if (!data->CheckNbParams(num, 3, ach, "template")) return;
    ...
    if (data->ReadSubList(num, 2, "items", ach, nsub2)) {
        ...
        for (Standard_Integer i2 = 1; i2 <= nb2; i2++) {
            ...
            if (data->ReadEntity(nsub2, i2, "representation_item", ach, STANDARD_TYPE(StepRepr_RepresentationItem),
                                 anent2))
...
void RWStepVisual_RWTemplate::WriteStep(StepData_StepWriter& SW, const Handle(StepVisual_Template) & ent) const {
...
void RWStepVisual_RWTemplate::Share(const Handle(StepVisual_Template) & ent, Interface_EntityIterator& iter) const {
