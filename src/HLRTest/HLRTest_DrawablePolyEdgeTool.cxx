...
HLRTest_DrawablePolyEdgeTool::HLRTest_DrawablePolyEdgeTool(const Handle(HLRBRep_PolyAlgo) & Alg,
                                                           const Standard_Integer ViewId, const Standard_Boolean Debug)
    : myAlgo(Alg), myDispRg1(Standard_False), myDispRgN(Standard_False), myDispHid(Standard_False), myViewId(ViewId),
      myDebug(Debug), myHideMode(Standard_True) {
        ...
        if (myHideMode) {
            ...
            if (myDispHid) {
                ...
                for (It.Initialize(myBiPntHid); It.More(); It.Next()) {
                    ...
                    if ((!myDispRg1 && BP.Rg1Line() && !BP.OutLine()) || (!myDispRgN && BP.RgNLine() && !BP.OutLine()))
            ...
            for (myAlgo->InitShow(); myAlgo->MoreShow(); myAlgo->NextShow()) {
                ...
                Standard_Boolean todraw = Standard_True;
                if ((!myDispRg1 && reg1 && !outl) || (!myDispRgN && regn && !outl)) todraw = Standard_False;
                if (todraw) {
