...
// purpose  :
//=======================================================================
Poly_Triangulation::Poly_Triangulation() : myCachedMinMax(NULL), myDeflection(0), myPurpose(Poly_MeshPurpose_NONE) {
    //
}
...
// purpose  :
//=======================================================================
Poly_Triangulation::Poly_Triangulation(const Standard_Integer theNbNodes, const Standard_Integer theNbTriangles,
                                       const Standard_Boolean theHasUVNodes, const Standard_Boolean theHasNormals)
    : myCachedMinMax(NULL), myDeflection(0), myNodes(theNbNodes), myTriangles(1, theNbTriangles),
...
// purpose  :
//=======================================================================
Poly_Triangulation::Poly_Triangulation(const TColgp_Array1OfPnt& theNodes, const Poly_Array1OfTriangle& theTriangles)
    : myCachedMinMax(NULL), myDeflection(0), myNodes(theNodes.Length()), myTriangles(1, theTriangles.Length()),
      myPurpose(Poly_MeshPurpose_NONE) {
...
Poly_Triangulation::Poly_Triangulation(const TColgp_Array1OfPnt& theNodes, const TColgp_Array1OfPnt2d& theUVNodes,
                                       const Poly_Array1OfTriangle& theTriangles)
    : myCachedMinMax(NULL), myDeflection(0), myNodes(theNodes.Length()), myTriangles(1, theTriangles.Length()),
      myUVNodes(theNodes.Length()), myPurpose(Poly_MeshPurpose_NONE) {
    const Poly_ArrayOfNodes aNodeWrapper(theNodes.First(), theNodes.Length());
    myNodes = aNodeWrapper;
    myTriangles = theTriangles;
...
Poly_Triangulation::Poly_Triangulation(const Handle(Poly_Triangulation) & theTriangulation)
    : myCachedMinMax(NULL), myDeflection(theTriangulation->myDeflection), myNodes(theTriangulation->myNodes),
      myTriangles(theTriangulation->myTriangles), myUVNodes(theTriangulation->myUVNodes),
      myNormals(theTriangulation->myNormals), myPurpose(theTriangulation->myPurpose) {
...
void Poly_Triangulation::SetNormals(const Handle(TShort_HArray1OfShortReal) & theNormals) {
    ...
    for (Standard_Integer aNodeIter = 1; aNodeIter <= NbNodes(); ++aNodeIter) {
        ...
        gp_Vec3f aNorm(theNormals->Value(anArrayInd + 0), theNormals->Value(anArrayInd + 1),
...
// purpose  :
// =======================================================================
void Poly_Triangulation::ResizeTriangles(Standard_Integer theNbTriangles, Standard_Boolean theToCopyOld) {
...
void Poly_Triangulation::DumpJson(Standard_OStream& theOStream, Standard_Integer) const {
    ...
    if (!myUVNodes.IsEmpty()) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, myUVNodes.Size())
    if (!myNormals.IsEmpty()) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, myNormals.Size())
...
Standard_Boolean Poly_Triangulation::MinMax(Bnd_Box& theBox, const gp_Trsf& theTrsf, const bool theIsAccurate) const {
    Bnd_Box aBox;
    if (HasCachedMinMax() &&
        (!HasGeometry() || !theIsAccurate || theTrsf.Form() == gp_Identity || theTrsf.Form() == gp_Translation ||
...
// purpose  :
//=======================================================================
Handle(Poly_Triangulation) Poly_Triangulation::DetachedLoadDeferredData(const Handle(OSD_FileSystem) &
                                                                        theFileSystem) const {
    if (!HasDeferredData()) {
