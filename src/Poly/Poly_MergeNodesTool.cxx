...
static int initialNbBuckets(int theNbFacets) {
    return theNbFacets > 0 ? theNbFacets * 2 // consider ratio 1:2 (NbTriangles:MergedNodes) as expected
...
public:
    //! Constructor.
    DataMapNode(const NCollection_Vec3<float>& thePos, const NCollection_Vec3<float>& theNorm, int theItem,
                NCollection_ListNode* theNext)
        : NCollection_TListNode<int>(theItem, theNext), myKey(thePos, theNorm) {}

    //! Key.
    const Poly_MergeNodesTool::Vec3AndNormal& Key() const {
        return myKey;
    }
...
Poly_MergeNodesTool::MergedNodesMap::MergedNodesMap(const int theNbBuckets)
    : NCollection_BaseMap(theNbBuckets, true, new NCollection_IncAllocator()), myTolerance(0.0f), myInvTol(0.0f),
      myAngle(1.0f), myAngleCos(0.0f), myToMergeOpposite(false) {
    //
}
...
// =======================================================================
inline int
Poly_MergeNodesTool::MergedNodesMap::vec3iHashCode(const Poly_MergeNodesTool::MergedNodesMap::CellVec3i& theVec,
                                                   const int theUpper) {
...
// purpose  :
// =======================================================================
inline bool Poly_MergeNodesTool::MergedNodesMap::Bind(int& theIndex, bool& theIsOpposite,
    ...
    for (DataMapNode* aNodeIter = aData[aHash]; aNodeIter != NULL; aNodeIter = (DataMapNode*)aNodeIter->Next()) {
...
// purpose  :
// =======================================================================
Poly_MergeNodesTool::Poly_MergeNodesTool(const double theSmoothAngle, const double theMergeTolerance,
                                         const int theNbFacets)
    : myPolyData(new Poly_Triangulation()),
      myNodeIndexMap((theSmoothAngle > 0.0 || theMergeTolerance > 0.0) ? initialNbBuckets(theNbFacets) : 1),
      myNodeInds(0, 0, 0, -1), myTriNormal(0.0f, 0.0f, 1.0f), myUnitFactor(1.0), myNbNodes(0), myNbElems(0),
      myNbDegenElems(0), myNbMergedElems(0), myToDropDegenerative(true), myToMergeElems(false) {
...
void Poly_MergeNodesTool::PushLastElement(int theNbNodes) {
    ...
    if (myToDropDegenerative) {
        ...
        if (myNodeInds[0] == myNodeInds[1] || myNodeInds[0] == myNodeInds[2] || myNodeInds[1] == myNodeInds[2]) {
            if (theNbNodes == 4) {
                //
            } else {
                ++myNbDegenElems;
...
// purpose  :
// =======================================================================
void Poly_MergeNodesTool::AddTriangulation(const Handle(Poly_Triangulation) & theTris, const gp_Trsf& theTrsf,
...
Handle(Poly_Triangulation) Poly_MergeNodesTool::MergeNodes(const Handle(Poly_Triangulation) & theTris,
                                                           const gp_Trsf& theTrsf, const Standard_Boolean theToReverse,
                                                           const double theSmoothAngle, const double theMergeTolerance,
