...
public:
    ...
    struct Link {
        ...
        Link() : node1(0), node2(0), flags(0) {}

        Link(Standard_Integer theNode1, Standard_Integer theNode2) : node1(theNode1), node2(theNode2), flags(1) {}
    ...
    public:
        HeapOfInteger(const Standard_Integer theNbPreAllocated = 1)
            : myMap(theNbPreAllocated), myIterReady(Standard_False) {}
        ...
        void Remove(const Standard_Integer theValue) {
            if (myIterReady && myIter.More() && myIter.Key() == theValue) myIter.Next();
...
public:
    ...
    Standard_EXPORT Poly_MakeLoops(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc = 0L);

    //! It is to reset the algorithm to the initial state.
    Standard_EXPORT void Reset(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc = 0L);
...
protected:
    virtual Standard_Integer chooseLeftWay(const Standard_Integer theNode, const Standard_Integer theSegIndex,
    ...
    Link getLink(const Standard_Integer theSegIndex) const {
        ...
        if (theSegIndex < 0) aLink.Reverse();
        return aLink;
    }
#ifdef OCCT_DEBUG
...
private:
    ...
    void acceptContour(const NCollection_IndexedMap<Standard_Integer>& theContour, Standard_Integer theStartNumber);
 ...
 */
inline Standard_Boolean IsEqual(const Poly_MakeLoops::Link& theKey1, const Poly_MakeLoops::Link& theKey2) {
...
public:
    ...
    //! Constructor. If helper is NULL then the algorithm will
    //! probably return a wrong result
    Standard_EXPORT Poly_MakeLoops3D(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc);

protected:
    Standard_EXPORT virtual Standard_Integer chooseLeftWay(const Standard_Integer theNode,
...
public:
    ...
    //! Constructor. If helper is NULL then the algorithm will
    //! probably return a wrong result
    Standard_EXPORT Poly_MakeLoops2D(const Standard_Boolean theLeftWay, const Helper* theHelper,
