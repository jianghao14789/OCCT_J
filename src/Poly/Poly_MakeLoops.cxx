...
Poly_MakeLoops::Poly_MakeLoops(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc)
    : myHelper(theHelper), myAlloc(theAlloc), myMapLink(4000, myAlloc), myLoops(myAlloc), myStartIndices(4000) {}
...
void Poly_MakeLoops::Reset(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc) {
    myHelper = theHelper;
    myMapLink.Clear();
...
void Poly_MakeLoops::AddLink(const Link& theLink) {
    if (theLink.node1 == theLink.node2) return;
...
void Poly_MakeLoops::ReplaceLink(const Link& theLink, const Link& theNewLink) {
    if (theNewLink.node1 == theNewLink.node2) return;
    Standard_Integer aInd = myMapLink.Add(theLink);
    if (aInd > 0) {
        Link aLink;
...
Poly_MakeLoops::LinkFlag Poly_MakeLoops::SetLinkOrientation(const Link& theLink, const LinkFlag theOrient) {
...
Poly_MakeLoops::Link Poly_MakeLoops::FindLink(const Link& theLink) const {
    ...
    if (aInd > 0) aLink = myMapLink(aInd);
    return aLink;
}
...
Standard_Integer Poly_MakeLoops::Perform() {
    ...
    for (i = 1; i <= myMapLink.Extent(); i++) {
        ...
        if (aLink.flags & LF_Fwd) myStartIndices.Add(i);
        if (aLink.flags & LF_Rev) myStartIndices.Add(-i);
    }

#ifdef OCCT_DEBUG
    if (doDebug) showBoundaryBreaks();
#endif
    ...
    if (doDebug && nbLoopsOnPass2)
        std::cout << "MakeLoops: " << nbLoopsOnPass2 << " contours accepted on the second pass" << std::endl;
#endif

    if (!myLoops.IsEmpty()) aResult |= RC_LoopsDone;
    if (!myHangIndices.IsEmpty()) aResult |= RC_HangingLinks;
    return aResult;
}
...
Standard_Integer Poly_MakeLoops::findContour(Standard_Integer theIndexS,
    ...
    for (;;) {
        ...
        for (; itLinks.More(); itLinks.Next()) {
            ...
            if (aInd == 0 || aInd == aIndex) continue;
            ...
            if (aNode1 != aLastNode) aIndS = -aIndS;

            if (canLinkBeTaken(aIndS)) aLstIndS.Append(aIndS);
        }
...
void Poly_MakeLoops::acceptContour(const NCollection_IndexedMap<Standard_Integer>& theContour,
    ...
    for (i = theStartNumber; i <= theContour.Extent(); i++) {
        ...
        if (aIndexS < 0) aOrientedLink.Reverse();
...
Standard_Integer Poly_MakeLoops::getFirstNode(Standard_Integer theIndexS) const {
    ...
    const Link& aLink = myMapLink(aIndex);
    if (theIndexS > 0) return aLink.node1;
    return aLink.node2;
}
...
Standard_Integer Poly_MakeLoops::getLastNode(int theIndexS) const {
    Standard_Integer aIndex = Abs(theIndexS);
    const Link& aLink = myMapLink(aIndex);
    if (theIndexS > 0) return aLink.node2;
    return aLink.node1;
}
...
void Poly_MakeLoops::markHangChain(Standard_Integer theNode, Standard_Integer theIndexS) {
    ...
    for (;;) {
        ...
        for (; itLinks.More() && nEdges == 0; itLinks.Next()) {
            ...
            if (aInd == 0 || aInd == aIndex) continue;
            if ((isOut && aNode1 == aL.node1) || (!isOut && aNode1 == aL.node2)) aInd = -aInd;
            if (canLinkBeTaken(aInd)) nEdges++;
        }
        if (nEdges > 0)
            // leave this chain
            break;
        ...
        for (itLinks.Init(aNextLinks); itLinks.More(); itLinks.Next()) {
            ...
            Standard_Integer aInd = myMapLink.FindIndex(aL);
            if (aInd == 0 || aInd == aIndex) continue;
            if ((isOut && aNode1 == aL.node2) || (!isOut && aNode1 == aL.node1)) aInd = -aInd;
...
void Poly_MakeLoops::showBoundaryBreaks() const {
    ...
    for (; it.More(); it.Next()) {
        ...
        for (; itLinks.More(); itLinks.Next()) {
            ...
            if (myMapLink.FindIndex(aLink) == 0) continue;
            Standard_Integer aFlags = aLink.flags & LF_Both;
            if (aFlags && aFlags != LF_Both) {
...
Poly_MakeLoops3D::Poly_MakeLoops3D(const Helper* theHelper, const Handle(NCollection_BaseAllocator) & theAlloc)
...
Standard_Integer Poly_MakeLoops3D::chooseLeftWay(const Standard_Integer theNode, const Standard_Integer theSegIndex,
                                                 const NCollection_List<Standard_Integer>& theLstIndS) const {
    Standard_Real aAngleMin = M_PI * 2;
    gp_Dir aNormal;
    const Helper* aHelper = getHelper();
    if (!aHelper->GetNormal(theNode, aNormal)) return theLstIndS.First();

    Link aLink = getLink(theSegIndex);
    gp_Dir aTgtRef;
    if (!aHelper->GetLastTangent(aLink, aTgtRef)) return theLstIndS.First();
    ...
    for (; aItI.More(); aItI.Next()) {
        ...
        aLink = getLink(aIndS);
        gp_Dir aTgt;
        if (!aHelper->GetFirstTangent(aLink, aTgt)) continue;
        ...
        if (aAngle < 1e-4 - M_PI) aAngle = M_PI;
        if (aAngle < aAngleMin) {
            aAngleMin = aAngle;
            aResIndex = aIndS;
        }
...
Poly_MakeLoops2D::Poly_MakeLoops2D(const Standard_Boolean theLeftWay, const Helper* theHelper,
                                   const Handle(NCollection_BaseAllocator) & theAlloc)
    : Poly_MakeLoops(theHelper, theAlloc), myRightWay(!theLeftWay) {}
...
Standard_Integer Poly_MakeLoops2D::chooseLeftWay(const Standard_Integer /*theNode*/, const Standard_Integer theSegIndex,
                                                 const NCollection_List<Standard_Integer>& theLstIndS) const {
    Standard_Real aAngleMin = M_PI * 2;
    const Helper* aHelper = getHelper();
    Link aLink = getLink(theSegIndex);
