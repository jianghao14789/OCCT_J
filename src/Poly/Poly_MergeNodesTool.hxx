...
public:
    ...
    Standard_EXPORT static Handle(Poly_Triangulation)
        MergeNodes(const Handle(Poly_Triangulation) & theTris, const gp_Trsf& theTrsf,
                   const Standard_Boolean theToReverse, const double theSmoothAngle,
                   const double theMergeTolerance = 0.0, const bool theToForce = true);
...
public:
    ...
    Standard_EXPORT Poly_MergeNodesTool(const double theSmoothAngle, const double theMergeTolerance = 0.0,
...
private:
    ...
    public:
        ...
        bool HasMergeAngle() const {
            return myAngle > 0.0f;
        }

        //! Return TRUE if merge angle comparison can be skipped (angle is close to 90 degrees).
        bool ToMergeAnyAngle() const {
            return myAngleCos <= 0.01f;
        }

        //! Return TRUE if nodes with opposite normals should be merged; FALSE by default.
        bool ToMergeOpposite() const {
            return myToMergeOpposite;
        }

        //! Set if nodes with opposite normals should be merged.
        void SetMergeOpposite(bool theToMerge) {
            myToMergeOpposite = theToMerge;
        }
        ...
        bool HasMergeTolerance() const {
            return myTolerance > 0.0f;
        }
        ...
        Standard_EXPORT bool Bind(int& theIndex, bool& theIsOpposite, const NCollection_Vec3<float>& thePos,
    ...
    private:
        ...
        //! Compute hash code.
        Standard_EXPORT int hashCode(const NCollection_Vec3<float>& thePos, const NCollection_Vec3<float>& theNorm,
        ...
        //! Compare two nodes.
        Standard_EXPORT bool isEqual(const Vec3AndNormal& theKey1, const NCollection_Vec3<float>& thePos2,
                                     const NCollection_Vec3<float>& theNorm2, bool& theIsOpposite) const;
