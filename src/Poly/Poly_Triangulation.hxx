...
public:
    ...
    Standard_EXPORT Poly_Triangulation(const Standard_Integer theNbNodes, const Standard_Integer theNbTriangles,
    ...
    Standard_EXPORT Poly_Triangulation(const TColgp_Array1OfPnt& Nodes, const TColgp_Array1OfPnt2d& UVNodes,
    ...
    Standard_EXPORT Standard_Boolean HasCachedMinMax() const {
        return myCachedMinMax != NULL;
    }
    ...
    //! @return FALSE if there is no any data to extend the passed box (no both triangulation and cached min - max
    //! range).
    Standard_EXPORT Standard_Boolean MinMax(Bnd_Box& theBox, const gp_Trsf& theTrsf,
...
public:
    ...
    bool IsDoublePrecision() const {
        return myNodes.IsDoublePrecision();
    }
    ...
    Standard_EXPORT void ResizeTriangles(Standard_Integer theNbTriangles, Standard_Boolean theToCopyOld);
...
public:
    ...
    Standard_DEPRECATED("Deprecated method, SetNormal() should be used instead") Standard_EXPORT
        void SetNormals(const Handle(TShort_HArray1OfShortReal) & theNormals);

    Standard_DEPRECATED("Deprecated method, Triangle() should be used instead") const
        Poly_Array1OfTriangle& Triangles() const {
        return myTriangles;
    }
...
public: //! @name late-load deferred data interface
    ...
    virtual Standard_Boolean HasDeferredData() const {
        return NbDeferredTriangles() > 0;
    }

    //! Loads triangulation data into itself
    //! from some deferred storage using specified shared input file system.
    Standard_EXPORT virtual Standard_Boolean
    LoadDeferredData(const Handle(OSD_FileSystem) & theFileSystem = Handle(OSD_FileSystem)());

    //! Loads triangulation data into new Poly_Triangulation object
    //! from some deferred storage using specified shared input file system.
    Standard_EXPORT virtual Handle(Poly_Triangulation)
        DetachedLoadDeferredData(const Handle(OSD_FileSystem) & theFileSystem = Handle(OSD_FileSystem)()) const;
