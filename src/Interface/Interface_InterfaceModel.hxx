...
public:
    ...
    Standard_EXPORT Handle(Standard_Type)
        Type(const Handle(Standard_Transient) & ent, const Standard_Integer num = 1) const;
    ...
    //! By default, queries main report, if <semantic> is True, it
    //! queries report for semantic check
    Standard_EXPORT Handle(Interface_ReportEntity)
        ReportEntity(const Standard_Integer num, const Standard_Boolean semantic = Standard_False) const;
    ...
    Standard_EXPORT const Handle(Interface_Check) &
        Check(const Standard_Integer num, const Standard_Boolean syntactic) const;
    ...
    Standard_EXPORT void AddWithRefs(const Handle(Standard_Transient) & anent, const Handle(Interface_Protocol) & proto,
                                     const Standard_Integer level = 0, const Standard_Boolean listall = Standard_False);

    //! Same as above, but works with the Protocol of the Model
    Standard_EXPORT void AddWithRefs(const Handle(Standard_Transient) & anent, const Standard_Integer level = 0,
                                     const Standard_Boolean listall = Standard_False);

    //! Same as above, but works with an already created GeneralLib
    Standard_EXPORT void AddWithRefs(const Handle(Standard_Transient) & anent, const Interface_GeneralLib& lib,
                                     const Standard_Integer level = 0, const Standard_Boolean listall = Standard_False);
    ...
    Standard_EXPORT void ChangeOrder(const Standard_Integer oldnum, const Standard_Integer newnum,
    ...
    Standard_EXPORT void Print(const Handle(Standard_Transient) & ent, Standard_OStream& s,
    ...
    //! surprises
    Standard_EXPORT static Standard_Boolean SetTemplate(const Standard_CString name,
