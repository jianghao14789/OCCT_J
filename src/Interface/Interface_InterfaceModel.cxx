...
Interface_InterfaceModel::Interface_InterfaceModel() : haschecksem(Standard_False), isdispatch(Standard_False) {
...
void Interface_InterfaceModel::FillSemanticChecks(const Interface_CheckIterator& checks, const Standard_Boolean clear) {
...
const Handle(Interface_Check) & Interface_InterfaceModel::Check(const Standard_Integer num,
    ...
    if (!(syntactic ? thereports.IsBound(num) : therepch.IsBound(num))) return nulch();
...
void Interface_InterfaceModel::AddWithRefs(const Handle(Standard_Transient) & anent,
                                           const Handle(Interface_Protocol) & proto, const Standard_Integer level,
...
void Interface_InterfaceModel::AddWithRefs(const Handle(Standard_Transient) & anent, const Standard_Integer level,
                                           const Standard_Boolean listall) {
...
void Interface_InterfaceModel::AddWithRefs(const Handle(Standard_Transient) & anent, const Interface_GeneralLib& lib,
                                           const Standard_Integer level, const Standard_Boolean listall) {
    if (anent.IsNull()) return;
    if (theentities.FindIndex(anent) != 0) {
        if (!listall) return;
    } else
...
void Interface_InterfaceModel::ReplaceEntity(const Standard_Integer nument, const Handle(Standard_Transient) & anent) {
...
void Interface_InterfaceModel::ChangeOrder(const Standard_Integer oldnum, const Standard_Integer newnum,
...
void Interface_InterfaceModel::GetFromTransfer(const Interface_EntityIterator& aniter) {
    ...
    for (aniter.Start(); aniter.More(); aniter.Next()) {
        Handle(Standard_Transient) ent = aniter.Value();
        AddEntity(ent);
    }
}
...
const Handle(Interface_Check) & Interface_InterfaceModel::GlobalCheck(const Standard_Boolean syntactic) const {
    if (syntactic)
        return thecheckstx;
    else
        return thechecksem;
...
void Interface_InterfaceModel::Print(const Handle(Standard_Transient) & ent, Standard_OStream& S,
...
void Interface_InterfaceModel::PrintToLog(const Handle(Standard_Transient) & ent, Standard_OStream& S) const {
    PrintLabel(ent, S);
}
...
Standard_Boolean Interface_InterfaceModel::SetTemplate(const Standard_CString name,
