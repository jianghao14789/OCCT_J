...
//  #####################################################################
//  ....                        CONSTRUCTEURS                        ....
Interface_CopyTool::Interface_CopyTool(const Handle(Interface_InterfaceModel) & amodel, const Interface_GeneralLib& lib)
...
Standard_Boolean Interface_CopyTool::Copy(const Handle(Standard_Transient) & entfrom,
                                          Handle(Standard_Transient) & entto, const Standard_Boolean mapped,
    ...
    if (!res) {
        ...
        if (entfrom->DynamicType() == STANDARD_TYPE(TCollection_HAsciiString)) {
            entto = new TCollection_HAsciiString(Handle(TCollection_HAsciiString)::DownCast(entfrom)->ToCString());
            res = Standard_True;
        }
        return res;
    }
...
void Interface_CopyTool::Implied(const Handle(Standard_Transient) & entfrom, const Handle(Standard_Transient) & entto) {
    Handle(Interface_GeneralModule) module;
    Standard_Integer CN;
    if (thelib.Select(entfrom, module, CN)) module->RenewImpliedCase(CN, entfrom, entto, *this);
}
...
Handle(Standard_Transient) Interface_CopyTool::Transferred(const Handle(Standard_Transient) & ent) {
    ...
    if (nument == 0 && thelev > 100)
        throw Interface_InterfaceError("CopyTool : Transferred, Entity is not contained in Starting Model");
    if (!themap->Search(ent, res)) { // deja transfere ? sinon, le faire
        ...
        if (!Copy(ent, res, (nument != 0), themod->IsRedefinedContent(nument))) return res;
...
void Interface_CopyTool::Bind(const Handle(Standard_Transient) & ent, const Handle(Standard_Transient) & res) {
...
Standard_Integer Interface_CopyTool::LastCopiedAfter(const Standard_Integer numfrom, Handle(Standard_Transient) & ent,
                                                     Handle(Standard_Transient) & res) const {
...
void Interface_CopyTool::RenewImpliedRefs() {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (!therep->Search(ent, aRep)) {
            ...
            if (!rep.IsNull() && !rep->HasNewContent()) Implied(ent, res);
        }
    }
}
...
Interface_EntityIterator Interface_CopyTool::CompleteResult(const Standard_Boolean withreports) const {
