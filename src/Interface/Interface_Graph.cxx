...
Interface_Graph::Interface_Graph(const Handle(Interface_InterfaceModel) & amodel, const Interface_GeneralLib& /*lib*/,
                                 Standard_Boolean theModeStat)
    : themodel(amodel), thepresents("") {
    if (theModeStat) InitStats();
    Evaluate();
}

Interface_Graph::Interface_Graph(const Handle(Interface_InterfaceModel) & amodel,
                                 const Handle(Interface_Protocol) & /*protocol*/, Standard_Boolean theModeStat)
    : themodel(amodel), thepresents("")
...
Interface_Graph::Interface_Graph(const Handle(Interface_InterfaceModel) & amodel,
                                 const Handle(Interface_GTool) & /*gtool*/, Standard_Boolean theModeStat)
    : themodel(amodel), thepresents("") {
    if (theModeStat) InitStats();
    Evaluate();
}

Interface_Graph::Interface_Graph(const Handle(Interface_InterfaceModel) & amodel, Standard_Boolean theModeStat)
    : themodel(amodel), thepresents("") {
    if (theModeStat) InitStats();
    Evaluate();
}
...
Interface_Graph::Interface_Graph(const Interface_Graph& agraph, const Standard_Boolean /*copied*/)
    ...
    if (!nb) return;
    if (thestats.IsNull()) thestats = new TColStd_HArray1OfInteger(1, nb);
...
void Interface_Graph::InitStats() {
    thestats = new TColStd_HArray1OfInteger(1, themodel->NbEntities()), theflags.Initialize(themodel->NbEntities(), 2);
...
void Interface_Graph::Evaluate() {
    ...
    if (themodel->GTool().IsNull()) return;
    ...
    for (i = 1; i <= n; i++) {
        ...
        for (iter.Start(); iter.More(); iter.Next()) {
            ...
            if (entshare == ent) continue;

            Standard_Integer num = EntityNumber(entshare);

            if (!num) {
                if (!thestats.IsNull()) theflags.SetTrue(i, Graph_ShareError);
                continue;
            }
...
void Interface_Graph::SetStatus(const Standard_Integer num, const Standard_Integer stat) {
    if (!thestats.IsNull()) thestats->SetValue(num, stat);
}
...
void Interface_Graph::GetFromModel() {
    if (themodel.IsNull() || thestats.IsNull()) return; // no model ... (-> on n ira pas loin)
    theflags.Init(Standard_True, Graph_Present);
    thestats->Init(0);
}

void Interface_Graph::GetFromEntity(const Handle(Standard_Transient) & ent, const Standard_Boolean shared,
...
void Interface_Graph::GetFromIter(const Interface_EntityIterator& iter, const Standard_Integer newstat) {
    if (thestats.IsNull()) return;
    for (iter.Start(); iter.More(); iter.Next()) {
...
void Interface_Graph::GetFromIter(const Interface_EntityIterator& iter, const Standard_Integer newstat,
                                  const Standard_Integer overlapstat, const Standard_Boolean cumul) {
    if (thestats.IsNull()) return;
    for (iter.Start(); iter.More(); iter.Next()) {
        Handle(Standard_Transient) ent = iter.Value();
        Standard_Integer num = EntityNumber(ent);
...
void Interface_Graph::GetFromGraph(const Interface_Graph& agraph) {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (agraph.IsPresent(i)) GetFromEntity(agraph.Entity(i), Standard_False, agraph.Status(i));
    }
}
...
void Interface_Graph::GetFromGraph(const Interface_Graph& agraph, const Standard_Integer stat) {
    ...
    Standard_Integer nb = Size();
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (agraph.IsPresent(i) && agraph.Status(i) == stat) GetFromEntity(agraph.Entity(i), Standard_False, stat);
    }
}
...
Standard_Boolean Interface_Graph::HasShareErrors(const Handle(Standard_Transient) & ent) const {
    if (thestats.IsNull()) return Standard_False;
    Standard_Integer num = EntityNumber(ent);
...
Interface_EntityIterator Interface_Graph::Shareds(const Handle(Standard_Transient) & ent) const {
    ...
    if (themodel->IsRedefinedContent(num)) aCurEnt = themodel->ReportEntity(num)->Content();

    // if (num == 0)  throw Standard_DomainError("Interface : Shareds");
    Handle(Interface_GeneralModule) module;
    Standard_Integer CN;
    if (themodel->GTool()->Select(aCurEnt, module, CN)) module->FillShared(themodel, CN, aCurEnt, iter);
    return iter;
}
...
static void AddTypedSharings(const Handle(Standard_Transient) & ent, const Handle(Standard_Type) & type,
                             Interface_EntityIterator& iter, const Standard_Integer n, const Interface_Graph& G) {
...
Interface_EntityIterator Interface_Graph::RootEntities() const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (!thesharings->Value(i).IsEmpty()) continue;
