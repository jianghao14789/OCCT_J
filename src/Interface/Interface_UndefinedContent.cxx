...
Standard_Boolean Interface_UndefinedContent::ParamData(const Standard_Integer num, Interface_ParamType& ptype,
...
void Interface_UndefinedContent::Reservate(const Standard_Integer nb, const Standard_Integer nblit) {
    ...
    if (nblit > thenbstr) { // Reservation en Litteraux
        if (thevalues.IsNull())
            ...
            Handle(Interface_HArray1OfHAsciiString) newvalues = new Interface_HArray1OfHAsciiString(1, nbnew);
...
void Interface_UndefinedContent::AddEntity(const Interface_ParamType ptype, const Handle(Standard_Transient) & ent) {
...
void Interface_UndefinedContent::RemoveParam(const Standard_Integer num) {
    ...
    for (np = 1; np <= thenbparams; np++) {
        ...
        if (((desc >> Content_LocalShift) & Content_LocalField) == local && (desc >> Content_NumberShift) > rang)
            theparams->SetValue(np, desc - (1 << Content_NumberShift));
    }
}

void Interface_UndefinedContent::SetLiteral(const Standard_Integer num, const Interface_ParamType ptype,
                                            const Handle(TCollection_HAsciiString) & val) {
...
void Interface_UndefinedContent::SetEntity(const Standard_Integer num, const Interface_ParamType ptype,
    ...
    if (!c1ent) {
        ...
        for (i = 1; i <= thenbparams; i++) {
            desc = theparams->Value(i);
            if (((desc >> Content_LocalShift) & Content_LocalField) == 0 && (desc >> Content_NumberShift) > rang)
                theparams->SetValue(i, desc - (1 << Content_NumberShift));
        }
    ...
    desc = Standard_Integer(ptype) + (Content_LocalRef << Content_LocalShift) + (rang << Content_NumberShift);
    theparams->SetValue(num, desc);
}

void Interface_UndefinedContent::SetEntity(const Standard_Integer num, const Handle(Standard_Transient) & ent) {
