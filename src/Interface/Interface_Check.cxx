...
Standard_CString Interface_Check::CFail(const Standard_Integer num, const Standard_Boolean final) const {
...
Standard_CString Interface_Check::CWarning(const Standard_Integer num, const Standard_Boolean final) const {
...
Handle(TColStd_HSequenceOfHAsciiString) Interface_Check::Warnings(const Standard_Boolean final) const {
...
Standard_CString Interface_Check::CInfoMsg(const Standard_Integer num, const Standard_Boolean final) const {
...
Handle(TColStd_HSequenceOfHAsciiString) Interface_Check::InfoMsgs(const Standard_Boolean final) const {
...
Standard_Boolean Interface_Check::Complies(const Handle(TCollection_HAsciiString) & mess, const Standard_Integer incl,
    ...
    if (status == Interface_CheckWarning || status == Interface_CheckAny) {
        ...
        for (i = nb; i > 0; i--) {
            ...
            if (incl == 0) {
                if (mess->IsSameString(ames)) return Standard_True;
            }
            if (incl < 0) {
                if (ames->Location(mess, 1, ames->Length()) > 0) return Standard_True;
            }
            if (incl < 0) {
                if (mess->Location(mess, 1, lng) > 0) return Standard_True;
            }
        }
    }
...
Standard_Boolean Interface_Check::Remove(const Handle(TCollection_HAsciiString) & mess, const Standard_Integer incl,
...
void Interface_Check::GetAsWarning(const Handle(Interface_Check) & other, const Standard_Boolean failsonly) {
    Standard_Integer nb, i;
    if ((nb = other->NbFails()) != 0) {
        if (thewarns.IsNull()) thewarns = new TColStd_HSequenceOfHAsciiString();
        if (thewarno.IsNull()) thewarno = new TColStd_HSequenceOfHAsciiString();
...
void Interface_Check::Print(Standard_OStream& S, const Standard_Integer level, const Standard_Integer final) const {
