...
Interface_CheckTool::Interface_CheckTool(const Handle(Interface_InterfaceModel) & model,
    ...
    : thegtool(new Interface_GTool(protocol, model->NbEntities())), theshare(model, protocol) {
    thestat = 0;
}
...
void Interface_CheckTool::FillCheck(const Handle(Standard_Transient) & ent, const Interface_ShareTool& sh,
    ...
    if (theshare.Graph().HasShareErrors(ent)) ach->AddFail("** Shared Items unknown from the containing Model");
}
