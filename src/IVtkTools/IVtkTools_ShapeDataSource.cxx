...
vtkStandardNewMacro(IVtkTools_ShapeDataSource)
    ...
    : myPolyData(new IVtkVTK_ShapeData()), myIsFastTransformMode(Standard_False), myIsTransformOnly(Standard_False) {
...
void IVtkTools_ShapeDataSource::SetShape(const IVtkOCC_Shape::Handle& theOccShape) {
    myIsTransformOnly =
        myIsFastTransformMode && !myOccShape.IsNull() && theOccShape->GetShape().IsPartner(myOccShape->GetShape());
...
vtkSmartPointer<vtkPolyData> IVtkTools_ShapeDataSource::transform(vtkPolyData* theSource,
    ...
    vtkSmartPointer<vtkTransformPolyDataFilter> aTrsfFilter = vtkSmartPointer<vtkTransformPolyDataFilter>::New();
