...
vtkStandardNewMacro(IVtkTools_SubPolyDataFilter)
    ...
    : myIdsArrayName(IVtkVTK_ShapeData::ARRNAME_SUBSHAPE_IDS()), myDoFiltering(true), myToCopyNormals(true) {
    //
}
...
int IVtkTools_SubPolyDataFilter::RequestData(vtkInformation* vtkNotUsed(theRequest),
    ...
    vtkSmartPointer<vtkPolyData> anInput = vtkPolyData::SafeDownCast(anInInfo->Get(vtkDataObject::DATA_OBJECT()));

    vtkSmartPointer<vtkPolyData> anOutput = vtkPolyData::SafeDownCast(anOutInfo->Get(vtkDataObject::DATA_OBJECT()));
...
#if (VTK_MAJOR_VERSION >= 9)
        anOutput->AllocateExact(aNbVerts, aNbVertPts, aNbLines, aNbLinePts, aNbPolys, aNbPolyPts, aNbStrips,
                                aNbStripPts);
#else
