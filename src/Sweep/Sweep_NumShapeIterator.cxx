...
Sweep_NumShapeIterator::Sweep_NumShapeIterator()
    : myNumShape(0, TopAbs_SHAPE), myCurrentNumShape(0, TopAbs_SHAPE), myCurrentRange(0), myMore(Standard_False) {}
...
void Sweep_NumShapeIterator::Init(const Sweep_NumShape& aShape) {
    ...
    if (myNumShape.Type() == TopAbs_EDGE) {
        ...
        if (myMore) {
            myCurrentRange = 1;
            myCurrentNumShape = Sweep_NumShape(1, TopAbs_VERTEX, myNumShape.Closed(), Standard_False, Standard_False);
            if (nbvert == 1) {
...
void Sweep_NumShapeIterator::Next() {
    ...
    if (myMore) {
        if (myNumShape.Type() == TopAbs_EDGE) {
            myCurrentNumShape =
                Sweep_NumShape(myCurrentRange, TopAbs_VERTEX, myNumShape.Closed(), Standard_False, Standard_False);
            myCurrentOrientation = TopAbs_REVERSED;
        }
    }
}
