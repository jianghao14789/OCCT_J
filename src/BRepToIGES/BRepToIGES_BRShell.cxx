...
BRepToIGES_BRShell::BRepToIGES_BRShell(const BRepToIGES_BREntity& BR) : BRepToIGES_BREntity(BR) {}
...
Handle(IGESData_IGESEntity) BRepToIGES_BRShell ::TransferFace(const TopoDS_Face& start, const Message_ProgressRange&) {
    Handle(IGESData_IGESEntity) res;
    ...
    if (!Outer.IsNull()) {
        Handle(IGESData_IGESEntity) ICurve3d = BW.TransferWire(Outer, myface, ICurve2d, Length);
    ...
    for (Ex.Init(myface, TopAbs_WIRE); Ex.More(); Ex.Next()) {
        ...
        if (W.IsNull()) {
            ...
            Handle(IGESData_IGESEntity) ICurve3d = BW.TransferWire(W, myface, ICurve2d, Length);
            if ((!ICurve3d.IsNull()) && (!ICurve2d.IsNull())) Iprefer = 3;
            if ((!ICurve3d.IsNull()) && (ICurve2d.IsNull())) Iprefer = 2;
            if ((ICurve3d.IsNull()) && (!ICurve2d.IsNull())) Iprefer = 1;
    ...
    if ((Surf->IsKind(STANDARD_TYPE(Geom_Plane)) || Surf->IsKind(STANDARD_TYPE(Geom_CylindricalSurface)) ||
