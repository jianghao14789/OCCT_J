...
BRepToIGES_BRWire::BRepToIGES_BRWire(const BRepToIGES_BREntity& BR) : BRepToIGES_BREntity(BR) {}
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferVertex(const TopoDS_Vertex& myvertex, const TopoDS_Edge& myedge,
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferVertex(const TopoDS_Vertex& myvertex, const TopoDS_Edge& myedge,
                                                               const TopoDS_Face& myface, Standard_Real& parameter) {
    Handle(IGESData_IGESEntity) res;
    if (myvertex.IsNull()) return res;
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferVertex(const TopoDS_Vertex& myvertex, const TopoDS_Edge& myedge,
                                                               ...
                                                               const TopLoc_Location& myloc, Standard_Real& parameter) {
    Handle(IGESData_IGESEntity) res;
    if (myvertex.IsNull()) return res;
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferVertex(const TopoDS_Vertex& myvertex, const TopoDS_Face& myface,
                                                               gp_Pnt2d& mypoint) {
    Handle(IGESData_IGESEntity) res;
    if (myvertex.IsNull()) return res;
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferEdge(const TopoDS_Edge& myedge, const TopoDS_Face& myface,
                                                             const Standard_Real length,
                                                             const Standard_Boolean isBRepMode) {
    Handle(IGESData_IGESEntity) res;
    if (myedge.IsNull() || GetPCurveMode() == 0 || (!isBRepMode && BRep_Tool::Degenerated(myedge))) return res;
    ...
    if (!Curve2d.IsNull()) {
        ...
        if (analyticMode &&
            (Surf->IsKind(STANDARD_TYPE(Geom_CylindricalSurface)) || Surf->IsKind(STANDARD_TYPE(Geom_ConicalSurface))))
            myLen = M_PI / 180.;

        if (analyticMode &&
            (Surf->IsKind(STANDARD_TYPE(Geom_SphericalSurface)) || Surf->IsKind(STANDARD_TYPE(Geom_ToroidalSurface)))) {
            gp_Trsf2d trans;
        ...
        if (myedge.Orientation() == TopAbs_REVERSED) {
            Standard_Real tmpFirst = Curve2d->ReversedParameter(Last), tmpLast = Curve2d->ReversedParameter(First);
            Curve2d->Reverse();
            First = tmpFirst;
            Last = tmpLast;
        }
...
Handle(IGESData_IGESEntity) BRepToIGES_BRWire ::TransferWire(const TopoDS_Wire& mywire, const TopoDS_Face& myface,
    ...
    if (TE.More()) {
        ...
        */
        Handle(ShapeFix_Wire) aSFW = new ShapeFix_Wire(mywire, myface, Precision::Confusion());
        aSFW->FixReorder();
