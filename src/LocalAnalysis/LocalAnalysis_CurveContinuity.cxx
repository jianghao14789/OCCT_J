...
LocalAnalysis_CurveContinuity::LocalAnalysis_CurveContinuity(const Handle(Geom_Curve) & Curv1, const Standard_Real u1,
                                                             const Handle(Geom_Curve) & Curv2, const Standard_Real u2,
                                                             const GeomAbs_Shape Order, const Standard_Real Epsnul,
                                                             const Standard_Real EpsC0, const Standard_Real EpsC1,
                                                             const Standard_Real EpsC2, const Standard_Real EpsG1,
                                                             const Standard_Real EpsG2, const Standard_Real Percent,
                                                             const Standard_Real Maxlen)
    : myContC0(0.0), myContC1(0.0), myContC2(0.0), myContG1(0.0), myContG2(0.0), myCourbC1(0.0), myCourbC2(0.0),
      myG2Variation(0.0), myLambda1(0.0), myLambda2(0.0) {
    myTypeCont = Order;
    myepsnul = Epsnul;
    myMaxLon = Maxlen;
    myepsC0 = EpsC0;
...
Standard_Boolean LocalAnalysis_CurveContinuity::IsC2() const {
    ...
    if (IsC1()) {
        if ((myContC2 <= myepsC2) || (Abs(myContC2 - M_PI) <= myepsC2)) {
            ...
            if ((Abs(myLambda1 * myLambda1 - myLambda2)) <= (epsil1 * epsil1 + epsil2)) return Standard_True;
        } else
            return Standard_False;
    }
    return Standard_False;
