...
Handle(Geom_Curve) GeomTools_CurveSet::Curve(const Standard_Integer I) const {
    if (I <= 0 || I > myMap.Extent()) return Handle(Geom_Curve)();
...
static void Print(const Handle(Geom_Circle) & CC, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << CIRCLE << " ";
    else
        OS << "Circle";
...
static void Print(const Handle(Geom_Ellipse) & E, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << ELLIPSE << " ";
    else
        OS << "Ellipse";
...
static void Print(const Handle(Geom_Parabola) & P, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << PARABOLA << " ";
    else
        OS << "Parabola";
...
static void Print(const Handle(Geom_Hyperbola) & H, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << HYPERBOLA << " ";
    else
        OS << "Hyperbola";
...
static void Print(const Handle(Geom_BezierCurve) & B, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << BEZIER << " ";
    else
        OS << "BezierCurve";
...
static void Print(const Handle(Geom_BSplineCurve) & B, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << BSPLINE << " ";
    else
        OS << "BSplineCurve";
...
static void Print(const Handle(Geom_TrimmedCurve) & C, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << TRIMMED << " ";
    else
        OS << "Trimmed curve\n";
...
static void Print(const Handle(Geom_OffsetCurve) & C, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << OFFSET << " ";
    else
        OS << "OffsetCurve";
...
void GeomTools_CurveSet::PrintCurve(const Handle(Geom_Curve) & C, Standard_OStream& OS,
