...
Handle(Geom2d_Curve) GeomTools_Curve2dSet::Curve2d(const Standard_Integer I) const {
    if (I <= 0 || I > myMap.Extent()) return Handle(Geom2d_Curve)();
...
static void Print(const Handle(Geom2d_Line) & L, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << LINE << " ";
    else
        OS << "Line";
...
static void Print(const Handle(Geom2d_Circle) & C, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << CIRCLE << " ";
    else
        OS << "Circle";
...
static void Print(const Handle(Geom2d_Ellipse) & E, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << ELLIPSE << " ";
    else
        OS << "Ellipse";
...
static void Print(const Handle(Geom2d_Parabola) & P, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << PARABOLA << " ";
    else
        OS << "Parabola";
...
static void Print(const Handle(Geom2d_Hyperbola) & H, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << HYPERBOLA << " ";
    else
        OS << "Hyperbola";
...
static void Print(const Handle(Geom2d_BezierCurve) & B, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << BEZIER << " ";
    else
        OS << "BezierCurve";
...
static void Print(const Handle(Geom2d_BSplineCurve) & B, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << BSPLINE << " ";
    else
        OS << "BSplineCurve";
...
static void Print(const Handle(Geom2d_TrimmedCurve) & C, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << TRIMMED << " ";
    else
        OS << "Trimmed curve\n";
...
static void Print(const Handle(Geom2d_OffsetCurve) & C, Standard_OStream& OS, const Standard_Boolean compact) {
    if (compact)
        OS << OFFSET << " ";
    else
        OS << "OffsetCurve";
...
void GeomTools_Curve2dSet::PrintCurve2d(const Handle(Geom2d_Curve) & C, Standard_OStream& OS,
...
static Standard_IStream& operator>>(Standard_IStream& IS, Handle(Geom2d_BezierCurve) & B) {
    ...
    for (i = 1; i <= degree + 1; i++) {
        IS >> poles(i);
        if (rational) GeomTools::GetReal(IS, weights(i));
    }
