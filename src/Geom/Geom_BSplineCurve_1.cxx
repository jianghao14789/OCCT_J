...
Standard_Boolean Geom_BSplineCurve::IsG1(const Standard_Real theTf, const Standard_Real theTl,
    ...
    for (Standard_Integer aNKnot = start; aNKnot <= finish; aNKnot++) {
        ...
        if (aTpar < theTf) continue;
        if (aTpar > theTl) break;

        Standard_Integer mult = Multiplicity(aNKnot);
        if (mult < aDeg) continue;
        ...
        if (Abs(aV1.Angle(aV2)) > theAngTol) return Standard_False;
    }

    if (!IsPeriodic()) return Standard_True;
...
void Geom_BSplineCurve::D0(const Standard_Real U, gp_Pnt& P) const {
    ...
    if (aNewU < knots->Value(aSpanIndex)) aSpanIndex--;
...
void Geom_BSplineCurve::D3(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3) const {
    Standard_Integer aSpanIndex = 0;
    Standard_Real aNewU(U);
    PeriodicNormalization(aNewU);
    BSplCLib::LocateParameter(deg, knots->Array1(), &mults->Array1(), U, periodic, aSpanIndex, aNewU);
...
gp_Vec Geom_BSplineCurve::DN(const Standard_Real U, const Standard_Integer N) const {
    gp_Vec V;
    BSplCLib::DN(U, N, 0, deg, periodic, POLES, rational ? &weights->Array1() : BSplCLib::NoWeights(), FKNOTS, FMULTS,
                 V);
    return V;
}
...
void Geom_BSplineCurve::Knots(TColStd_Array1OfReal& K) const {
    Standard_DomainError_Raise_if(K.Lower() < knots->Lower() || K.Upper() > knots->Upper(), "Geom_BSplineCurve::Knots");
...
gp_Pnt Geom_BSplineCurve::LocalValue(const Standard_Real U, const Standard_Integer FromK1,
...
void Geom_BSplineCurve::LocalD0(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
                                gp_Pnt& P) const {
...
void Geom_BSplineCurve::LocalD1(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
                                gp_Pnt& P, gp_Vec& V1) const {
    ...
    BSplCLib::LocateParameter(deg, FKNOTS, U, periodic, FromK1, ToK2, index, u);
    index = BSplCLib::FlatIndex(deg, index, mults->Array1(), periodic);
    BSplCLib::D1(u, index, deg, periodic, POLES, rational ? &weights->Array1() : BSplCLib::NoWeights(), FKNOTS, FMULTS,
                 P, V1);
}
...
void Geom_BSplineCurve::LocalD2(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
    ...
    BSplCLib::LocateParameter(deg, FKNOTS, U, periodic, FromK1, ToK2, index, u);
    index = BSplCLib::FlatIndex(deg, index, mults->Array1(), periodic);
    BSplCLib::D2(u, index, deg, periodic, POLES, rational ? &weights->Array1() : BSplCLib::NoWeights(), FKNOTS, FMULTS,
                 P, V1, V2);
}
...
void Geom_BSplineCurve::LocalD3(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
    ...
    BSplCLib::LocateParameter(deg, FKNOTS, U, periodic, FromK1, ToK2, index, u);
    index = BSplCLib::FlatIndex(deg, index, mults->Array1(), periodic);
    BSplCLib::D3(u, index, deg, periodic, POLES, rational ? &weights->Array1() : BSplCLib::NoWeights(), FKNOTS, FMULTS,
                 P, V1, V2, V3);
}
...
gp_Vec Geom_BSplineCurve::LocalDN(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
    ...
    gp_Vec V;
    BSplCLib::DN(u, N, index, deg, periodic, POLES, rational ? &weights->Array1() : BSplCLib::NoWeights(), FKNOTS,
                 FMULTS, V);
    return V;
}
...
Standard_Integer Geom_BSplineCurve::Multiplicity(const Standard_Integer Index) const {
    Standard_OutOfRange_Raise_if(Index < 1 || Index > mults->Length(), "Geom_BSplineCurve::Multiplicity");
...
void Geom_BSplineCurve::Multiplicities(TColStd_Array1OfInteger& M) const {
    Standard_DimensionError_Raise_if(M.Length() != mults->Length(), "Geom_BSplineCurve::Multiplicities");
    M = mults->Array1();
}
...
const TColStd_Array1OfReal* Geom_BSplineCurve::Weights() const {
    if (IsRational()) return &weights->Array1();
...
void Geom_BSplineCurve::LocateU(const Standard_Real U, const Standard_Real ParametricTolerance, Standard_Integer& I1,
...
void Geom_BSplineCurve::Resolution(const Standard_Real Tolerance3D, Standard_Real& UTolerance) {
    if (!maxderivinvok) {
        if (periodic) {
            ...
            BSplCLib::Resolution(poles->Array1(), rational ? &weights->Array1() : BSplCLib::NoWeights(),
                                 poles->Length(), flatknots->Array1(), deg, 1., maxderivinv);
        }
        maxderivinvok = 1;
    }
...
Standard_Boolean Geom_BSplineCurve::IsEqual(const Handle(Geom_BSplineCurve) & theOther,
    ...
    if (knots.IsNull() || poles.IsNull() || mults.IsNull()) return Standard_False;
    if (deg != theOther->Degree()) return Standard_False;
    if (knots->Length() != theOther->NbKnots() || poles->Length() != theOther->NbPoles()) return Standard_False;
    ...
    for (; i <= knots->Length(); i++) {
        if (fabs(knots->Value(i) - theOther->Knot(i)) > Precision::Parametric(thePreci)) return Standard_False;
    }

    for (i = 1; i <= mults->Length(); i++) {
        if (mults->Value(i) != theOther->Multiplicity(i)) return Standard_False;
    }

    if (rational != theOther->IsRational()) return Standard_False;

    if (!rational) return Standard_True;
