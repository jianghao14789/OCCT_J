...
// purpose  :
//=======================================================================
Geom_OsculatingSurface::Geom_OsculatingSurface() : myTol(0.0), myAlong(1, 4) {
...
Geom_OsculatingSurface::Geom_OsculatingSurface(const Handle(Geom_Surface) & BS, const Standard_Real Tol)
    : myAlong(1, 4) {
    Init(BS, Tol);
}
...
void Geom_OsculatingSurface::Init(const Handle(Geom_Surface) & BS, const Standard_Real Tol) {
    ...
    if ((BS->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) || (BS->IsKind(STANDARD_TYPE(Geom_BezierSurface)))) {
        ...
        if (myAlong(1) || myAlong(2) || myAlong(3) || myAlong(4)) {
            ...
            if (BS->IsKind(STANDARD_TYPE(Geom_BezierSurface))) {
                ...
                BzS->Poles(P);
                InitSurf = new Geom_BSplineSurface(P, UKnots, VKnots, UMults, VMults, BzS->UDegree(), BzS->VDegree(),
                                                   BzS->IsUPeriodic(), BzS->IsVPeriodic());
            } else {
...
Standard_Boolean Geom_OsculatingSurface::UOscSurf(const Standard_Real U, const Standard_Real V, Standard_Boolean& t,
    ...
    if (myAlong(1) || myAlong(2)) {
        ...
        if (myBasisSurf->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) {
            ...
            if (NbVK == 2 && NV == 1)
                ...
                if (VKnots(NbVK) - V > V - VKnots(1)) isToSkipSecond = Standard_True;
        } else {
            NU = 1;
            NV = 1;
            NbVK = 2;
...
Standard_Boolean Geom_OsculatingSurface::VOscSurf(const Standard_Real U, const Standard_Real V, Standard_Boolean& t,
                                                  Handle(Geom_BSplineSurface) & L) const {
    Standard_Boolean along = Standard_False;
    if (myAlong(3) || myAlong(4)) {
        Standard_Integer NU = 1, NV = 1;
        ...
        if (myBasisSurf->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) {
            ...
            if (NbUK == 2 && NU == 1)
                // Need to find the closest end
                if (UKnots(NbUK) - U > U - UKnots(1)) isToSkipSecond = Standard_True;
        } else {
            NU = 1;
            NV = 1;
            NbUK = 2;
...
Standard_Boolean Geom_OsculatingSurface::BuildOsculatingSurface(const Standard_Real Param,
    ...
    if ((IsAlongU() && vdeg <= 1) || (IsAlongV() && udeg <= 1)) {
        ...
        if (IsAlongU() && (Param > vcacheparameter + vspanlength / 2)) vcacheparameter = vcacheparameter + vspanlength;
        if (IsAlongV() && (Param > ucacheparameter + uspanlength / 2)) ucacheparameter = ucacheparameter + uspanlength;

        BSplSLib::BuildCache(ucacheparameter, vcacheparameter, uspanlength, vspanlength, BS->IsUPeriodic(),
                             BS->IsVPeriodic(), BS->UDegree(), BS->VDegree(), ULocalIndex, VLocalIndex, UFlatKnots,
                             VFlatKnots, NewPoles, BSplSLib::NoWeights(), cachepoles, BSplSLib::NoWeights());
        ...
        Convert_GridPolynomialToPoles Data(1, 1, UContinuity, VContinuity, MaxUDegree, MaxVDegree, NumCoeffPerSurface,
                                           Coefficients, PolynomialUIntervals, PolynomialVIntervals, TrueUIntervals,
                                           TrueVIntervals);

        //      Handle(Geom_BSplineSurface) BSpl =
        BSpl = new Geom_BSplineSurface(Data.Poles()->Array2(), Data.UKnots()->Array1(), Data.VKnots()->Array1(),
                                       Data.UMultiplicities()->Array1(), Data.VMultiplicities()->Array1(),
                                       Data.UDegree(), Data.VDegree(), 0, 0);
#ifdef OCCT_DEBUG
        std::cout << "^====================================^" << std::endl << std::endl;
#endif
...
Standard_Boolean Geom_OsculatingSurface::IsQPunctual(const Handle(Geom_Surface) & S, const Standard_Real Param,
                                                     const GeomAbs_IsoType IT, const Standard_Real TolMin,
...
void Geom_OsculatingSurface::DumpJson(Standard_OStream& theOStream, Standard_Integer theDepth) const {
    ...
    if (!myOsculSurf1.IsNull()) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, myOsculSurf1->Size())
    if (!myOsculSurf2.IsNull()) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, myOsculSurf2->Size())
    if (!myKdeg.IsNull()) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, myKdeg->Size())
