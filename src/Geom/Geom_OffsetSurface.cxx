...
//                               CSLib
// Modified     15/11/96 : JPI : ajout equivalent surface pour les surfaces canoniques et modif des methodes D0 D1, ...
// UIso,VIso Modified     18/11/96 : JPI : inversion de l'offsetValue dans UReverse et Vreverse
...
Geom_OffsetSurface::Geom_OffsetSurface(const Handle(Geom_Surface) & theSurf, const Standard_Real theOffset,
...
void Geom_OffsetSurface::SetBasisSurface(const Handle(Geom_Surface) & S, const Standard_Boolean isNotCheckC0) {
    ...
    if (isC0) {
        ...
        if (isC0) throw Standard_ConstructionError("Offset with no C1 Surface");
    }
    ...
    if (equivSurf.IsNull()) myEvaluator = new GeomEvaluator_OffsetSurface(basisSurf, offsetValue, myOscSurf);
}
...
void Geom_OffsetSurface::Bounds(Standard_Real& U1, Standard_Real& U2, Standard_Real& V1, Standard_Real& V2) const {
...
#ifdef CHECK
    if (myBasisSurfContinuity == GeomAbs_C0) throw Geom_UndefinedValue();
#endif
    if (equivSurf.IsNull())
...
void Geom_OffsetSurface::D1(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V) const {
#ifdef CHECK
    if (myBasisSurfContinuity == GeomAbs_C0 || myBasisSurfContinuity == GeomAbs_C1) throw Geom_UndefinedDerivative();
#endif
    if (equivSurf.IsNull())
...
void Geom_OffsetSurface::D2(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
...
void Geom_OffsetSurface::D3(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
...
gp_Vec Geom_OffsetSurface::DN(const Standard_Real U, const Standard_Real V, const Standard_Integer Nu,
...
Handle(Geom_Curve) Geom_OffsetSurface::UIso(const Standard_Real UU) const {
    if (equivSurf.IsNull()) {
        ...
        AdvApprox_ApproxAFunction Approx(Num1, Num2, Num3, T1, T2, T3, V1, V2, Cont, MaxDeg, MaxSeg, ev);
...
Handle(Geom_Curve) Geom_OffsetSurface::VIso(const Standard_Real VV) const {
    if (equivSurf.IsNull()) {
        ...
        AdvApprox_ApproxAFunction Approx(Num1, Num2, Num3, T1, T2, T3, U1, U2, Cont, MaxDeg, MaxSeg, ev);
...
Standard_Boolean Geom_OffsetSurface::IsUClosed() const {
    ...
    if (SBasis->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface))) {
        Handle(Geom_RectangularTrimmedSurface) St = Handle(Geom_RectangularTrimmedSurface)::DownCast(SBasis);
        ...
        if (S->IsKind(STANDARD_TYPE(Geom_ElementarySurface))) {
            ...
            Handle(Geom_SurfaceOfLinearExtrusion) Extru = Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(S);
        ...
        if (SBasis->IsKind(STANDARD_TYPE(Geom_ElementarySurface))) {
            ...
            Handle(Geom_SurfaceOfLinearExtrusion) Extru = Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(SBasis);
...
void Geom_OffsetSurface::TransformParameters(Standard_Real& U, Standard_Real& V, const gp_Trsf& T) const {
...
Handle(Geom_Surface) Geom_OffsetSurface::Surface() const {
    ...
    if (TheType == STANDARD_TYPE(Geom_RectangularTrimmedSurface)) {
        Handle(Geom_RectangularTrimmedSurface) S = Handle(Geom_RectangularTrimmedSurface)::DownCast(basisSurf);
        Base = S->BasisSurface();
...

    {
        ...
        if (MinorRadius <= MajorRadius) {
            if (Axis.Direct())
                ...
                MinorRadius -= offsetValue;
            if (MinorRadius >= Tol) Result = new Geom_ToroidalSurface(Axis, MajorRadius, MinorRadius);
