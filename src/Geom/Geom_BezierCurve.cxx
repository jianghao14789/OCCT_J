...
Geom_BezierCurve::Geom_BezierCurve(const TColgp_Array1OfPnt& Poles) {
    ...
    if (nbpoles < 2 || nbpoles > (Geom_BezierCurve::MaxDegree() + 1)) throw Standard_ConstructionError();
    //  copy the poles
...
Geom_BezierCurve::Geom_BezierCurve(const TColgp_Array1OfPnt& Poles, const TColStd_Array1OfReal& Weights) {
    ...
    if (Weights.Length() != nbpoles) throw Standard_ConstructionError();
...
void Geom_BezierCurve::InsertPoleAfter(const Standard_Integer Index, const gp_Pnt& P, const Standard_Real Weight) {
    ...
    if (Index < 0 || Index > nbpoles) throw Standard_OutOfRange("Geom_BezierCurve::InsertPoleAfter");
...
void Geom_BezierCurve::RemovePole(const Standard_Integer Index) {
    ...
    if (nbpoles <= 2) throw Standard_ConstructionError("Geom_BezierCurve::RemovePole");

    if (Index < 1 || Index > nbpoles) throw Standard_OutOfRange("Geom_BezierCurve::RemovePole");
...
void Geom_BezierCurve::Segment(const Standard_Real U1, const Standard_Real U2) {
    ...
    if (IsRational()) {
        ...
        BSplCLib::BuildCache(0.0, 1.0, 0, Degree(), bidflatknots, poles->Array1(), &weights->Array1(), coeffs,
                             &wcoeffs);
...
void Geom_BezierCurve::SetPole(const Standard_Integer Index, const gp_Pnt& P) {
    if (Index < 1 || Index > NbPoles()) throw Standard_OutOfRange("Geom_BezierCurve::SetPole");
...
void Geom_BezierCurve::SetWeight(const Standard_Integer Index, const Standard_Real Weight) {
    ...
    if (Index < 1 || Index > nbpoles) throw Standard_OutOfRange("Geom_BezierCurve::SetWeight");
    if (Weight <= gp::Resolution()) throw Standard_ConstructionError("Geom_BezierCurve::SetWeight");
    ...
    if (wasrat && !Rational(cweights)) weights.Nullify();
}
...
gp_Vec Geom_BezierCurve::DN(const Standard_Real U, const Standard_Integer N) const {
    if (N < 1) throw Standard_RangeError("Geom_BezierCurve::DN");
    gp_Vec V;
...
const gp_Pnt& Geom_BezierCurve::Pole(const Standard_Integer Index) const {
    if (Index < 1 || Index > poles->Length()) throw Standard_OutOfRange("Geom_BezierCurve::Pole");
...
void Geom_BezierCurve::Poles(TColgp_Array1OfPnt& P) const {
    if (P.Length() != poles->Length()) throw Standard_DimensionError("Geom_BezierCurve::Poles");
    P = poles->Array1();
}
...
Standard_Real Geom_BezierCurve::Weight(const Standard_Integer Index) const {
    if (Index < 1 || Index > poles->Length()) throw Standard_OutOfRange("Geom_BezierCurve::Weight");
    if (IsRational())
...
void Geom_BezierCurve::Weights(TColStd_Array1OfReal& W) const {
    ...
    Standard_Integer nbpoles = NbPoles();
    if (W.Length() != nbpoles) throw Standard_DimensionError("Geom_BezierCurve::Weights");
    if (IsRational())
        W = weights->Array1();
    else {
        Standard_Integer i;
