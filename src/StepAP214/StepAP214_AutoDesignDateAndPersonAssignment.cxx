...
void StepAP214_AutoDesignDateAndPersonAssignment::Init(const Handle(StepBasic_PersonAndOrganization) &
                                                           aAssignedPersonAndOrganization,
                                                       const Handle(StepBasic_PersonAndOrganizationRole) & aRole,
                                                       const Handle(StepAP214_HArray1OfAutoDesignDateAndPersonItem) &
                                                           aItems) {
    // --- classe own fields ---
    items = aItems;
...
StepAP214_AutoDesignDateAndPersonAssignment::ItemsValue(const Standard_Integer num) const {
