...
void RWStepShape_RWFaceBound::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
                                       Handle(Interface_Check) & ach, const Handle(StepShape_FaceBound) & ent) const {
...
void RWStepShape_RWFaceBound::WriteStep(StepData_StepWriter& SW, const Handle(StepShape_FaceBound) & ent) const {
...
void RWStepShape_RWFaceBound::Check(const Handle(StepShape_FaceBound) & ent, const Interface_ShareTool& aShto,
    ...
    if (!theEL1.IsNull()) {
        ...
        for (Standard_Integer i = 1; i <= nbEdg; i++) {
            ...
            if (nbRef == 1) {
                ...
                Handle(StepShape_OrientedEdge) refOE1 = Handle(StepShape_OrientedEdge)::DownCast(myShRef.Value());
                myShRef.Next();
                Handle(StepShape_OrientedEdge) refOE2 = Handle(StepShape_OrientedEdge)::DownCast(myShRef.Value());
                if (theOE1 == refOE1)
                    theOE2 = refOE2;
                ...
                if (!sharOE2) {
                    ...
                    if (!sharEL2) {
                        ...
                        myShRef.Start();
                        Handle(StepShape_FaceBound) theFB2 = Handle(StepShape_FaceBound)::DownCast(myShRef.Value());
                        if (!theFB2.IsNull()) {
