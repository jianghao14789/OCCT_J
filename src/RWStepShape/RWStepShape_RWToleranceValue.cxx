...
void RWStepShape_RWToleranceValue::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
    ...
    if (!data->ReadEntity(num, 1, "lower_bound", ach, STANDARD_TYPE(StepBasic_MeasureWithUnit), LB)) {
        ...
        if (data->ReadEntity(num, 1, "lower_bound", ach, STANDARD_TYPE(StepRepr_MeasureRepresentationItem), aMSR) ||
    ...
    if (!LB.IsNull() && !UB.IsNull()) ach->ClearFails();
    ent->Init(LB, UB);
}
