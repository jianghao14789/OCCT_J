...
private:
    struct Approx_Data {
        Approx_Data()
            : myBezierApprox(Standard_True), Xo(0.0), Yo(0.0), Zo(0.0), U1o(0.0), V1o(0.0), U2o(0.0), V2o(0.0),
              ApproxXYZ(Standard_True), ApproxU1V1(Standard_True), ApproxU2V2(Standard_True), indicemin(0),
              indicemax(0), myNbPntMax(30), parametrization(Approx_ChordLength) {}
...
public:
    ...
    Standard_EXPORT void Perform(const Handle(Adaptor3d_Surface) & Surf1, const Handle(Adaptor3d_Surface) & Surf2,
                                 const Handle(IntPatch_WLine) & aLine, const Standard_Boolean ApproxXYZ = Standard_True,
                                 const Standard_Boolean ApproxU1V1 = Standard_True,
                                 const Standard_Boolean ApproxU2V2 = Standard_True,
                                 const Standard_Integer indicemin = 0, const Standard_Integer indicemax = 0);

    Standard_EXPORT void Perform(const Handle(IntPatch_WLine) & aLine, const Standard_Boolean ApproxXYZ = Standard_True,
                                 const Standard_Boolean ApproxU1V1 = Standard_True,
                                 const Standard_Boolean ApproxU2V2 = Standard_True,
                                 const Standard_Integer indicemin = 0, const Standard_Integer indicemax = 0);
    ...
    Standard_EXPORT void SetParameters(const Standard_Real Tol3d, const Standard_Real Tol2d,
                                       ...
                                       const Standard_Integer NbIterMax, const Standard_Integer NbPntMax = 30,
    ...
    Standard_EXPORT static void Parameters(const GeomInt_TheMultiLineOfWLApprox& Line, const Standard_Integer firstP,
                                           const Standard_Integer lastP, const Approx_ParametrizationType Par,
                                           math_Vector& TheParameters);

protected:
private:
    Standard_EXPORT void Perform(const IntSurf_Quadric& Surf1, const Handle(Adaptor3d_Surface) & Surf2,
                                 const Handle(IntPatch_WLine) & aLine, const Standard_Boolean ApproxXYZ,
                                 const Standard_Boolean ApproxU1V1, const Standard_Boolean ApproxU2V2,
                                 const Standard_Integer indicemin, const Standard_Integer indicemax,
    ...
    Standard_EXPORT void prepareDS(const Standard_Boolean theApproxXYZ, const Standard_Boolean theApproxU1V1,
                                   const Standard_Boolean theApproxU2V2, const Standard_Integer indicemin,
                                   const Standard_Integer indicemax);

    //! Build knot sequence.
    Standard_EXPORT void buildKnots(const Handle(IntPatch_WLine) & theline, const Standard_Address thePtrSVSurf);

    //! Build curve.
    Standard_EXPORT void buildCurve(const Handle(IntPatch_WLine) & theline, const Standard_Address thePtrSVSurf);
