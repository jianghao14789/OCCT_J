...
AdvApp2Var_Network::AdvApp2Var_Network(const AdvApp2Var_SequenceOfPatch& Net, const TColStd_SequenceOfReal& TheU,
...
void AdvApp2Var_Network::UpdateInU(const Standard_Real CuttingValue) {
    ...
    for (j = 1; j < myVParameters.Length(); j++) {
        ...
        Handle(AdvApp2Var_Patch) aNewPat =
            new AdvApp2Var_Patch(CuttingValue, myUParameters.Value(i + 1), myVParameters.Value(j),
                                 myVParameters.Value(j + 1), aPat->UOrder(), aPat->VOrder());
        aNewPat->ResetApprox();
...
void AdvApp2Var_Network::UpdateInV(const Standard_Real CuttingValue) {
    ...
    for (Standard_Integer i = 1; i < myUParameters.Length(); i++) {
        ...
        Handle(AdvApp2Var_Patch) aNewPat =
            new AdvApp2Var_Patch(myUParameters.Value(i), myUParameters.Value(i + 1), CuttingValue,
                                 myVParameters.Value(j + 1), Pat->UOrder(), Pat->VOrder());
        aNewPat->ResetApprox();
        myNet.InsertAfter(indice, aNewPat);
    }
}
...
void AdvApp2Var_Network::SameDegree(const Standard_Integer iu, const Standard_Integer iv, Standard_Integer& ncfu,
