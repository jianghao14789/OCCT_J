...
AdvApp2Var_ApproxAFunc2Var::AdvApp2Var_ApproxAFunc2Var(
    ...
    const Handle(TColStd_HArray1OfReal) & ThreeDTol, const Handle(TColStd_HArray2OfReal) & OneDTolFr,
    const Handle(TColStd_HArray2OfReal) & TwoDTolFr, const Handle(TColStd_HArray2OfReal) & ThreeDTolFr,
    const Standard_Real FirstInU, const Standard_Real LastInU, const Standard_Real FirstInV,
    const Standard_Real LastInV, const GeomAbs_IsoType FavorIso, const GeomAbs_Shape ContInU,
    const GeomAbs_Shape ContInV, const Standard_Integer PrecisCode, const Standard_Integer MaxDegInU,
    const Standard_Integer MaxDegInV, const Standard_Integer MaxPatch, const AdvApp2Var_EvaluatorFunc2Var& Func,
    AdvApprox_Cutting& UChoice, AdvApprox_Cutting& VChoice)
    : my1DTolerances(OneDTol), my2DTolerances(TwoDTol), my3DTolerances(ThreeDTol), my1DTolOnFront(OneDTolFr),
      my2DTolOnFront(TwoDTolFr), my3DTolOnFront(ThreeDTolFr), myFirstParInU(FirstInU), myLastParInU(LastInU),
      myFirstParInV(FirstInV), myLastParInV(LastInV), myFavoriteIso(FavorIso), myContInU(ContInU), myContInV(ContInV),
      myPrecisionCode(PrecisCode), myMaxDegInU(MaxDegInU), myMaxDegInV(MaxDegInV), myMaxPatches(MaxPatch),
      myDone(Standard_False), myHasResult(Standard_False), myDegreeInU(0), myDegreeInV(0), myCriterionError(0.0) {
...
AdvApp2Var_ApproxAFunc2Var::AdvApp2Var_ApproxAFunc2Var(
    ...
    const GeomAbs_Shape ContInV, const Standard_Integer PrecisCode, const Standard_Integer MaxDegInU,
    const Standard_Integer MaxDegInV, const Standard_Integer MaxPatch, const AdvApp2Var_EvaluatorFunc2Var& Func,
    const AdvApp2Var_Criterion& Crit, AdvApprox_Cutting& UChoice, AdvApprox_Cutting& VChoice)
    : my1DTolerances(OneDTol), my2DTolerances(TwoDTol), my3DTolerances(ThreeDTol), my1DTolOnFront(OneDTolFr),
      my2DTolOnFront(TwoDTolFr), my3DTolOnFront(ThreeDTolFr), myFirstParInU(FirstInU), myLastParInU(LastInU),
      myFirstParInV(FirstInV), myLastParInV(LastInV), myFavoriteIso(FavorIso), myContInU(ContInU), myContInV(ContInV),
      myPrecisionCode(PrecisCode), myMaxDegInU(MaxDegInU), myMaxDegInV(MaxDegInV), myMaxPatches(MaxPatch),
...
void AdvApp2Var_ApproxAFunc2Var::Init() {
    ...
    if (ndu < 2 * iu + 2) throw Standard_ConstructionError("AdvApp2Var_ApproxAFunc2Var : UMaxDegree Error");
    if (ndv < 2 * iv + 2) throw Standard_ConstructionError("AdvApp2Var_ApproxAFunc2Var : VMaxDegree Error");
    myPrecisionCode = Max(0, Min(myPrecisionCode, 3));
    AdvApp2Var_Context Conditions(ifav, iu, iv, ndu, ndv, myPrecisionCode, myNumSubSpaces[0], myNumSubSpaces[1],
                                  myNumSubSpaces[2], my1DTolerances, my2DTolerances, my3DTolerances, my1DTolOnFront,
                                  my2DTolOnFront, my3DTolOnFront);
...
void AdvApp2Var_ApproxAFunc2Var::InitGrid(const Standard_Integer NbInt) {
    ...
    Handle(AdvApp2Var_Iso) V0 = new AdvApp2Var_Iso(GeomAbs_IsoV, myFirstParInV, myFirstParInU, myLastParInU,
                                                   myFirstParInV, myLastParInV, 1, iu, iv);
    Handle(AdvApp2Var_Iso) V1 = new AdvApp2Var_Iso(GeomAbs_IsoV, myLastParInV, myFirstParInU, myLastParInU,
                                                   myFirstParInV, myLastParInV, 2, iu, iv);
    Handle(AdvApp2Var_Iso) U0 = new AdvApp2Var_Iso(GeomAbs_IsoU, myFirstParInU, myFirstParInU, myLastParInU,
                                                   myFirstParInV, myLastParInV, 3, iu, iv);
    Handle(AdvApp2Var_Iso) U1 = new AdvApp2Var_Iso(GeomAbs_IsoU, myLastParInU, myFirstParInU, myLastParInU,
                                                   myFirstParInV, myLastParInV, 4, iu, iv);
    ...
    Standard_Real deltu = (myLastParInU - myFirstParInU) / NbInt, deltv = (myLastParInV - myFirstParInV) / NbInt;
...
void AdvApp2Var_ApproxAFunc2Var::Perform(const AdvApprox_Cutting& UChoice, const AdvApprox_Cutting& VChoice,
...
void AdvApp2Var_ApproxAFunc2Var::Perform(const AdvApprox_Cutting& UChoice, const AdvApprox_Cutting& VChoice,
                                         const AdvApp2Var_EvaluatorFunc2Var& Func, const AdvApp2Var_Criterion& Crit) {
...
void AdvApp2Var_ApproxAFunc2Var::ComputePatches(const AdvApprox_Cutting& UChoice, const AdvApprox_Cutting& VChoice,
                                                const AdvApp2Var_EvaluatorFunc2Var& Func) {
    ...
    while (myResult.FirstNotApprox(FirstNA)) {
        ...
        if (((NbPatch + NbV) > myMaxPatches) && ((NbPatch + NbU) <= myMaxPatches) && (Vmore)) NumDec = 2;
...
void AdvApp2Var_ApproxAFunc2Var::ComputeConstraints(const AdvApprox_Cutting& UChoice, const AdvApprox_Cutting& VChoice,
                                                    const AdvApp2Var_EvaluatorFunc2Var& Func) {
    Standard_Real dec;
    ...
    for (Handle(AdvApp2Var_Iso) anIso = myConstraints.FirstNotApprox(ind1, ind2); !anIso.IsNull();
        ...
        anIso->MakeApprox(myConditions, myFirstParInU, myLastParInU, myFirstParInV, myLastParInV, Func, N1, N2);
...
void AdvApp2Var_ApproxAFunc2Var::Compute3DErrors() {
    ...
    if (myNumSubSpaces[2] > 0) {
        ...
        for (iesp = 1; iesp <= myNumSubSpaces[2]; iesp++) {
            ...
            if (error_max > Tol || error_U0 > F3Tol || error_U1 > F4Tol || error_V0 > F1Tol || error_V1 > F2Tol) {
...
void AdvApp2Var_ApproxAFunc2Var::ConvertBS() {
    ...
    for (SSP = 1; SSP <= myNumSubSpaces[2]; SSP++) {
        ...
        Convert_GridPolynomialToPoles CvP(myResult.NbPatchInU(), myResult.NbPatchInV(), iu, iv, myMaxDegInU,
                                          myMaxDegInV, NbCoeff, Poly, Uint1, Vint1, Uint2, Vint2);
        if (!CvP.IsDone()) {
            myDone = Standard_False;
        }
        ...
        mySurfaces->ChangeValue(SSP) = new (Geom_BSplineSurface)(
            CvP.Poles()->Array2(), CvP.UKnots()->Array1(), CvP.VKnots()->Array1(), CvP.UMultiplicities()->Array1(),
            CvP.VMultiplicities()->Array1(), CvP.UDegree(), CvP.VDegree());
    }
}
...
void AdvApp2Var_ApproxAFunc2Var::Dump(Standard_OStream& o) const {
    ...
    if (!myHasResult) {
        ...
        o << std::endl;
        o << "Degree of Bezier patches in U : " << myDegreeInU << "  in V : " << myDegreeInV << std::endl;
        o << std::endl;
