...
AdvApp2Var_Patch::AdvApp2Var_Patch()
    : myU0(0.), myU1(1.), myV0(0.), myV1(1.), myOrdInU(0), myOrdInV(0), myNbCoeffInU(0), myNbCoeffInV(0),
      myApprIsDone(Standard_False), myHasResult(Standard_False), myCutSense(0), myDiscIsDone(Standard_False),
      myCritValue(0.) {}
...
AdvApp2Var_Patch::AdvApp2Var_Patch(const Standard_Real U0, const Standard_Real U1, const Standard_Real V0,
                                   const Standard_Real V1, const Standard_Integer iu, const Standard_Integer iv)
    : myU0(U0), myU1(U1), myV0(V0), myV1(V1), myOrdInU(iu), myOrdInV(iv), myNbCoeffInU(0), myNbCoeffInV(0),
      myApprIsDone(Standard_False), myHasResult(Standard_False), myCutSense(0), myDiscIsDone(Standard_False),
      myCritValue(0.) {}
...
void AdvApp2Var_Patch::Discretise(const AdvApp2Var_Context& Conditions, const AdvApp2Var_Framework& Constraints,
    ...
    AdvApp2Var_ApproxF2var::mma2cdi_(&NDIMEN, &NBPNTU, UROOT, &NBPNTV, VROOT, &IORDRU, &IORDRV, C1, C2, C3, C4, SU0,
                                     SU1, DU0, DU1, SV0, SV1, DV0, DV1, SOSOTB, SODITB, DISOTB, DIDITB, &IERCOD);
    ...
    AdvApp2Var_ApproxF2var::mma2ds1_(&NDIMEN, UDBFN, VDBFN,
                                     /*(void *)*/ Func, &NBPNTU, &NBPNTV, UROOT, VROOT, &ISOFAV, SOSOTB, DISOTB, SODITB,
                                     DIDITB, PTS, TAB, &IERCOD);
...
void AdvApp2Var_Patch::AddConstraints(const AdvApp2Var_Context& Conditions, const AdvApp2Var_Framework& Constraints) {
    // data stored in the  Context
    ...
    Handle(TColStd_HArray1OfReal) HHERMV = new TColStd_HArray1OfReal(1, (2 * IORDRV + 2) * (2 * IORDRV + 2));
    ...
    if (IORDRV >= 0) {
        ...
        AdvApp2Var_ApproxF2var::mma2ac2_(&NDIMEN, &NDegU, &NDegV, &IORDRV, &NCFLMU, NCFV0, IsoV0, NCFV1, IsoV1, HermV,
                                         PATCAN);
    }

    // add constraints to constant U
    Handle(TColStd_HArray1OfReal) HHERMU = new TColStd_HArray1OfReal(1, (2 * IORDRU + 2) * (2 * IORDRU + 2));
    ...
    if (IORDRU >= 0) {
        ...
        AdvApp2Var_ApproxF2var::mma2ac3_(&NDIMEN, &NDegU, &NDegV, &IORDRU, &NCFLMV, NCFU0, IsoU0, NCFU1, IsoU1, HermU,
                                         PATCAN);
    }
    ...
    if (IORDRU >= 0 && IORDRV >= 0) {
        AdvApp2Var_ApproxF2var::mma2ac1_(&NDIMEN, &NDegU, &NDegV, &IORDRU, &IORDRV, C1, C2, C3, C4, HermU, HermV,
                                         PATCAN);
    }
}
...
void AdvApp2Var_Patch::MakeApprox(const AdvApp2Var_Context& Conditions, const AdvApp2Var_Framework& Constraints,
    ...
    NCOEFU = 0;
    NCOEFV = 0;
    AdvApp2Var_ApproxF2var::mma2ce1_((integer*)&NumDec, &NDIMEN, &NBSESP, &NDIMSE, &NDMINU, &NDMINV, &NDegU, &NDegV,
                                     &NDJACU, &NDJACV, &IORDRU, &IORDRV, &NBPNTU, &NBPNTV, EPSAPR, SOSOTB, DISOTB,
                                     SODITB, DIDITB, PATJAC, ERRMAX, ERRMOY, &NCOEFU, &NCOEFV, &ITYDEC, &IERCOD);
    ...
    if (ITYDEC == 0 && IERCOD <= 0) {
        ...
        AdvApp2Var_MathBase::mmfmca9_(&NJacU, &NJacV, &NDIMEN, &myNbCoeffInU, &myNbCoeffInV, &NDIMEN, PATJAC, PATJAC);
        AdvApp2Var_ApproxF2var::mma2can_(&NCFLMU, &NCFLMV, &NDIMEN, &myOrdInU, &myOrdInV, &myNbCoeffInU, &myNbCoeffInV,
        ...
        AdvApp2Var_ApproxF2var::mma2fx6_(&NCFLMU, &NCFLMV, &NDIMEN, &NBSESP, &itrois, &iun, &iun, &IORDRU, &IORDRV,
                                         EPSAPR, EPSFRO, PATCAN, ERRMAX, &myNbCoeffInU, &myNbCoeffInV);
        ...
        for (dim = 1; dim <= NDIMEN; dim++) {
            ...
            for (ii = 1; ii <= NCFLMU; ii++) {
                ...
                for (jj = 1; jj <= NCFLMV; jj++) {
                    HPAUX->SetValue(dim + NDIMEN * (jj - 1) + aIU, myEquation->Value(ii + NCFLMU * (jj - 1) + aIN));
                }
            }
        }
        myEquation = HPAUX;
...
void AdvApp2Var_Patch::ChangeDomain(const Standard_Real a, const Standard_Real b, const Standard_Real c,
...
Standard_Integer AdvApp2Var_Patch::CutSense(const AdvApp2Var_Criterion& Crit, const Standard_Integer NumDec) const {
...
void AdvApp2Var_Patch::ChangeNbCoeff(const Standard_Integer NbCoeffU, const Standard_Integer NbCoeffV) {
...
Handle(TColgp_HArray2OfPnt) AdvApp2Var_Patch::Poles(const Standard_Integer SSPIndex,
    ...
    // Conversion
    Convert_GridPolynomialToPoles Conv(Cond.ULimit() - 1, Cond.VLimit() - 1, NbCoeff, SousEquation, Intervalle,
                                       Intervalle);
