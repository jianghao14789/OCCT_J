...
Standard_Boolean ShapeAlgo_AlgoContainer::ConnectNextWire(const Handle(ShapeAnalysis_Wire) & saw,
                                                          ...
                                                          const Standard_Real maxtol, Standard_Real& distmin,
    ...
    for (Standard_Integer ipole = 1; ipole < NbPoles; ipole++) {
        if (Poles(ipole).IsEqual(Poles(ipole + 1), Precision::Confusion())) {
            if (jpole == 1) {
                ...
                Handle(Geom_BSplineCurve) newC = new Geom_BSplineCurve(newPoles, newWeigs, newKnots, newMults, deg);
                SCurve.Append(newC);
                I1 = ipole + 1;
                jpole = 1;
            }
    ...
    for (Standard_Integer itab = 1; itab <= nbcurves; itab++) {
        ...
        if (jpole > 2) {
            ...
            if (!theapp3d.IsDone()) {
                ...
                modifCurve = new Geom_BSplineCurve(tpoles, mbspc.Knots(), mbspc.Multiplicities(), mbspc.Degree());
            }
        } else {
            modifCurve = mycurve;
        }
    ...
    for (Standard_Integer ipole = 1; ipole < NbPoles; ipole++) {
        if (Poles(ipole).IsEqual(Poles(ipole + 1), Precision::PConfusion())) {
            if (jpole == 1) {
                ...
                Handle(Geom2d_BSplineCurve) newC = new Geom2d_BSplineCurve(newPoles, newWeigs, newKnots, newMults, deg);
                SCurve.Append(newC);
                I1 = ipole + 1;
                jpole = 1;
            }
    ...
    for (Standard_Integer itab = 1; itab <= nbcurves; itab++) {
        ...
        if (jpole > 10) {
            ...
            if (!theapp3d.IsDone()) {
                ...
                modifCurve = new Geom2d_BSplineCurve(tpoles, mbspc.Knots(), mbspc.Multiplicities(), mbspc.Degree());
            }
        } else {
            modifCurve = mycurve;
        }
...
TopoDS_Shape ShapeAlgo_AlgoContainer::C0ShapeToC1Shape(const TopoDS_Shape& shape, const Standard_Real tol) const {
...
Handle(Geom_BSplineSurface) ShapeAlgo_AlgoContainer::ConvertSurfaceToBSpline(const Handle(Geom_Surface) & surf,
    ...
    return ShapeConstruct::ConvertSurfaceToBSpline(surf, UF, UL, VF, VL, Precision::Confusion(), GeomAbs_C1, 100,
...
Standard_Boolean ShapeAlgo_AlgoContainer::HomoWires(const TopoDS_Wire& wireIn1, const TopoDS_Wire& wireIn2,
                                                    TopoDS_Wire& wireOut1, TopoDS_Wire& wireOut2,
    ...
    if (edge1.Orientation() == TopAbs_REVERSED) IsToReverse1 = Standard_True;
    if (edge2.Orientation() == TopAbs_REVERSED) IsToReverse2 = Standard_True;
    crv1 = BRep_Tool::Curve(edge1, loc1, first1, last1);
    crv2 = BRep_Tool::Curve(edge2, loc2, first2, last2);
    delta1 = last1 - first1;
    delta2 = last2 - first2;
    while (nbCreatedEdges < (nEdges1 + nEdges2 - 1)) { /*just a security. */
        ...
        if (iterCook && iterPerry) {
            ...
            if (Copernic.More()) Copernic.Next();
            if (!Copernic.More()) {
                wireOut1 = makeWire1.Wire();
                wireOut2 = makeWire2.Wire();
                return Standard_True; // szv#4:S4163:12Mar99 `res=` not needed
...
Standard_Boolean ShapeAlgo_AlgoContainer::C0BSplineToSequenceOfC1BSplineCurve(const Handle(Geom_BSplineCurve) & BS,
                                                                              Handle(TColGeom_HSequenceOfBoundedCurve) &
                                                                                  seqBS) const {
...
Standard_Boolean
ShapeAlgo_AlgoContainer::C0BSplineToSequenceOfC1BSplineCurve(const Handle(Geom2d_BSplineCurve) & BS,
...
void ShapeAlgo_AlgoContainer::GetFaceUVBounds(const TopoDS_Face& F, Standard_Real& Umin, Standard_Real& Umax,
...
Handle(Geom_BSplineCurve) ShapeAlgo_AlgoContainer::ConvertCurveToBSpline(
    ...
    const GeomAbs_Shape Continuity, const Standard_Integer MaxSegments, const Standard_Integer MaxDegree) const {
