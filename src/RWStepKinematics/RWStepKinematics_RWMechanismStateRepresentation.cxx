...
void RWStepKinematics_RWMechanismStateRepresentation::ReadStep(
    ...
    if (theData->ReadSubList(theNum, 2, "items", theArch, nsub)) {
        ...
        for (Standard_Integer i = 1; i <= nb; i++) {
            if (theData->ReadEntity(nsub, i, "representation_item", theArch, STANDARD_TYPE(StepRepr_RepresentationItem),
                                    anEnt))
    ...
    theData->ReadEntity(theNum, 3, "context_of_items", theArch, STANDARD_TYPE(StepRepr_RepresentationContext),
                        aContextOfItems);

    // Inherited field : represented_mechanism
    Handle(StepKinematics_MechanismRepresentation) aRepresentedMechanism;
    theData->ReadEntity(theNum, 4, "represented_mechanism", theArch,
                        STANDARD_TYPE(StepKinematics_MechanismRepresentation), aRepresentedMechanism);
...
void RWStepKinematics_RWMechanismStateRepresentation::WriteStep(
    StepData_StepWriter& theSW, const Handle(StepKinematics_MechanismStateRepresentation) & theEnt) const {
    // Inherited field : name
...
// purpose  :
//=======================================================================
void RWStepKinematics_RWMechanismStateRepresentation::Share(const Handle(StepKinematics_MechanismStateRepresentation) &
                                                                theEnt,
