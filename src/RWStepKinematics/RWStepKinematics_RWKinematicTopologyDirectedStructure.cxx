...
void RWStepKinematics_RWKinematicTopologyDirectedStructure::ReadStep(
    ...
    if (theData->ReadSubList(theNum, 2, "representation.items", theArch, sub2)) {
        ...
        for (Standard_Integer i0 = 1; i0 <= nb0; i0++) {
            ...
            theData->ReadEntity(num2, i0, "representation_item", theArch, STANDARD_TYPE(StepRepr_RepresentationItem),
                                anIt0);
    ...
    theData->ReadEntity(theNum, 3, "representation.context_of_items", theArch,
    ...
    theData->ReadEntity(theNum, 4, "parent", theArch, STANDARD_TYPE(StepKinematics_KinematicTopologyStructure),
                        aParent);

    // Initialize entity
    theEnt->Init(aRepresentation_Name, aRepresentation_Items, aRepresentation_ContextOfItems, aParent);
}
...
void RWStepKinematics_RWKinematicTopologyDirectedStructure::WriteStep(
    StepData_StepWriter& theSW, const Handle(StepKinematics_KinematicTopologyDirectedStructure) & theEnt) const {
...
void RWStepKinematics_RWKinematicTopologyDirectedStructure::Share(
    const Handle(StepKinematics_KinematicTopologyDirectedStructure) & theEnt, Interface_EntityIterator& iter) const {
