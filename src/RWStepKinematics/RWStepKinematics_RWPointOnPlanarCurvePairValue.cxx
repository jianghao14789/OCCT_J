...
void RWStepKinematics_RWPointOnPlanarCurvePairValue::ReadStep(const Handle(StepData_StepReaderData) & theData,
                                                              ...
                                                              const Handle(StepKinematics_PointOnPlanarCurvePairValue) &
                                                                  theEnt) const {
    ...
    theData->ReadEntity(theNum, 2, "pair_value.applies_to_pair", theArch, STANDARD_TYPE(StepKinematics_KinematicPair),
                        aPairValue_AppliesToPair);

    // Own fields of PointOnPlanarCurvePairValue

    Handle(StepGeom_PointOnCurve) aActualPointOnCurve;
    theData->ReadEntity(theNum, 3, "actual_point_on_curve", theArch, STANDARD_TYPE(StepGeom_PointOnCurve),
                        aActualPointOnCurve);
    ...
    // Initialize entity
    theEnt->Init(aRepresentationItem_Name, aPairValue_AppliesToPair, aActualPointOnCurve, aInputOrientation);
}
...
void RWStepKinematics_RWPointOnPlanarCurvePairValue::WriteStep(
    StepData_StepWriter& theSW, const Handle(StepKinematics_PointOnPlanarCurvePairValue) & theEnt) const {
...
// purpose  :
//=======================================================================
void RWStepKinematics_RWPointOnPlanarCurvePairValue::Share(const Handle(StepKinematics_PointOnPlanarCurvePairValue) &
                                                               theEnt,
    ...
    if (!theEnt->InputOrientation().RotationAboutDirection().IsNull()) iter.AddItem(theEnt->InputOrientation().Value());
}
