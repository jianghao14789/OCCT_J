...
void RWStepKinematics_RWSlidingCurvePairValue::ReadStep(const Handle(StepData_StepReaderData) & theData,
                                                        ...
                                                        const Handle(StepKinematics_SlidingCurvePairValue) &
                                                            theEnt) const {
    ...
    theData->ReadEntity(theNum, 3, "actual_point_on_curve1", theArch, STANDARD_TYPE(StepGeom_PointOnCurve),
                        aActualPointOnCurve1);

    Handle(StepGeom_PointOnCurve) aActualPointOnCurve2;
    theData->ReadEntity(theNum, 4, "actual_point_on_curve2", theArch, STANDARD_TYPE(StepGeom_PointOnCurve),
                        aActualPointOnCurve2);

    // Initialize entity
    theEnt->Init(aRepresentationItem_Name, aPairValue_AppliesToPair, aActualPointOnCurve1, aActualPointOnCurve2);
}
