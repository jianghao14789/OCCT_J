...
public:
    ...
    Standard_EXPORT Standard_Boolean IsCoaxial(const gp_Ax1& Other, const Standard_Real AngularTolerance,
    ...
    //! Returns True if the direction of this and another axis are parallel with same orientation or opposite
    //! orientation. That is, if the angle between the two axes is equal to 0 or Pi. Note: the tolerance criterion is
    //! given by theAngularTolerance.
    ...
    Standard_Real Angle(const gp_Ax1& theOther) const {
        return vdir.Angle(theOther.vdir);
    }
