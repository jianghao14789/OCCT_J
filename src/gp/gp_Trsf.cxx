...
gp_Trsf::gp_Trsf(const gp_Trsf2d& T)
    : scale(T.ScaleFactor()), shape(T.Form()), loc(T.TranslationPart().X(), T.TranslationPart().Y(), 0.0) {
...
void gp_Trsf::SetValues(const Standard_Real a11, const Standard_Real a12, const Standard_Real a13,
    ...
    Standard_Real As = s;
    if (As < 0) As = -As;
    Standard_ConstructionError_Raise_if(As < gp::Resolution(), "gp_Trsf::SetValues, null determinant");
    if (s > 0)
        s = Pow(s, 1. / 3.);
    else
        s = -Pow(-s, 1. / 3.);
...
void gp_Trsf::Multiply(const gp_Trsf& T) {
    if (T.shape == gp_Identity) {
        ...
        loc.Add(T.loc.Multiplied(matrix));
        matrix.Multiply(T.matrix);
    } else if ((shape == gp_CompoundTrsf || shape == gp_Rotation || shape == gp_Ax1Mirror || shape == gp_Ax2Mirror) &&
        ...
        Tloc.Multiply(scale);
        loc.Add(Tloc);
    } else if (shape == gp_Translation && (T.shape == gp_CompoundTrsf || T.shape == gp_Rotation ||
...
void gp_Trsf::DumpJson(Standard_OStream& theOStream, Standard_Integer) const {
    OCCT_DUMP_VECTOR_CLASS(theOStream, "Location", 3, loc.X(), loc.Y(), loc.Z())
        OCCT_DUMP_VECTOR_CLASS(theOStream, "Matrix", 9, matrix.Value(1, 1), matrix.Value(1, 2), matrix.Value(1, 3),
                               matrix.Value(2, 1), matrix.Value(2, 2), matrix.Value(2, 3), matrix.Value(3, 1),
                               matrix.Value(3, 2), matrix.Value(3, 3))
            OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, shape) OCCT_DUMP_FIELD_VALUE_NUMERICAL(theOStream, scale)}
...
Standard_Boolean gp_Trsf::InitFromJson(const Standard_SStream& theSStream, Standard_Integer& theStreamPos) {
    ...
    gp_XYZ anXYZLoc;
    OCCT_INIT_VECTOR_CLASS(aStreamStr, "Location", aPos, 3, &anXYZLoc.ChangeCoord(1), &anXYZLoc.ChangeCoord(2),
    ...
    OCCT_INIT_VECTOR_CLASS(aStreamStr, "Matrix", aPos, 9, &mymatrix[0][0], &mymatrix[0][1], &mymatrix[0][2],
                           &mymatrix[1][0], &mymatrix[1][1], &mymatrix[1][2], &mymatrix[2][0], &mymatrix[2][1],
                           &mymatrix[2][2])
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
