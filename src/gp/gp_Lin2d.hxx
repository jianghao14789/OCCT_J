...
public:
    ...
    gp_Lin2d(const gp_Ax2d& theA) : pos(theA) {}

    //! <theP> is the location point (origin) of the line and
    //! <theV> is the direction of the line.
    gp_Lin2d(const gp_Pnt2d& theP, const gp_Dir2d& theV) : pos(theP, theV) {}

    //! Creates the line from the equation theA*X + theB*Y + theC = 0.0 Raises ConstructionError if Sqrt(theA*theA +
    //! theB*theB) <= Resolution from gp. Raised if Sqrt(theA*theA + theB*theB) <= Resolution from gp.
