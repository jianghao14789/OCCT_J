...
IGESToBRep_IGESBoundary::IGESToBRep_IGESBoundary(const IGESToBRep_CurveAndSurface& CS) : myCS(CS) {}
...
void IGESToBRep_IGESBoundary::Init(const IGESToBRep_CurveAndSurface& CS, const Handle(IGESData_IGESEntity) & entity,
                                   const TopoDS_Face& face, const gp_Trsf2d& trans, const Standard_Real uFact,
...
Standard_Boolean IGESToBRep_IGESBoundary::Transfer(Standard_Boolean& okCurve, Standard_Boolean& okCurve3d,
    ...
    return Transfer(okCurve, okCurve3d, okCurve2d, curve3d, scurve3d, Standard_False, toreverse3d, curves2d,
                    Standard_False, number, lsewd);
}
...
Standard_Boolean IGESToBRep_IGESBoundary::Transfer(Standard_Boolean& okCurve, Standard_Boolean& okCurve3d,
                                                   ...
                                                   const Handle(IGESData_HArray1OfIGESEntity) & curves2d,
                                                   const Standard_Boolean toreverse2d, const Standard_Integer number,
                                                   Handle(ShapeExtend_WireData) & lsewd) {
    Handle(IGESData_IGESEntity) icurve3d; // temporary object
    return Transfer(okCurve, okCurve3d, okCurve2d, icurve3d, curve3d, Standard_True, Standard_False, curves2d,
                    toreverse2d, number, lsewd);
}
...
void IGESToBRep_IGESBoundary::Check(const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
...
Standard_Boolean IGESToBRep_IGESBoundary::Transfer(Standard_Boolean&, Standard_Boolean&, Standard_Boolean&,
    ...
    if (GTranslate3d && !GTranslate2d) {
        ...
        if (toreverse2d) ReverseCurves2d(Gsewd2d, myface);
        ...
        for (Standard_Integer i = 1; i <= len3d; i++) {
            ...
            if (usescurve)
                ...
                if (!shape3d.IsNull()) {
                    Lsewd3d->Add(shape3d);
                    if (toreverse3d) ReverseCurves3d(Lsewd3d);
                } else
            ...
            if (!shape2d.IsNull()) {
                Lsewd2d->Add(shape2d);
                if (toreverse2d) ReverseCurves2d(Lsewd2d, myface);
...
void IGESToBRep_IGESBoundary::ReverseCurves3d(const Handle(ShapeExtend_WireData) & sewd) {
    ...
    for (Standard_Integer i = 1; i <= sewd->NbEdges(); i++) {
        ...
        if (curve->IsPeriodic()) // #21
            ShapeBuild_Edge().MakeEdge(newedge, curve->Reversed(), L, curve->ReversedParameter(p2),
                                       curve->ReversedParameter(p1));
        else
            ShapeBuild_Edge().MakeEdge(
                newedge, curve->Reversed(), L,
...
void IGESToBRep_IGESBoundary::ReverseCurves2d(const Handle(ShapeExtend_WireData) & sewd, const TopoDS_Face& face) {
    ...
    for (Standard_Integer i = 1; i <= sewd->NbEdges(); i++) {
        ...
        if (curve->IsPeriodic())
            ShapeBuild_Edge().MakeEdge(newedge, curve->Reversed(), face, curve->ReversedParameter(p2),
                                       curve->ReversedParameter(p1));
        else
            ShapeBuild_Edge().MakeEdge(
                newedge, curve->Reversed(), face,
