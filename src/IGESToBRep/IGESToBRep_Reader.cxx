...
//  sln 11.06.2002 OCC448 : Initialize "read.onlyvisiable" parameter  to control transferring invisible sub entities
//  which logically depend on the grouping entities
...
Standard_Integer IGESToBRep_Reader::LoadFile(const Standard_CString filename) {
    if (theProc.IsNull()) theProc = new Transfer_TransientProcess;
...
//=======================================================================
void IGESToBRep_Reader::TransferRoots(const Standard_Boolean onlyvisible, const Message_ProgressRange& theProgress) {
...
//=======================================================================
Standard_Boolean IGESToBRep_Reader::Transfer(const Standard_Integer num, const Message_ProgressRange& theProgress) {
    Handle(Message_Messenger) TF = theProc->Messenger();
    theDone = Standard_False;
    if (theModel.IsNull()) {
    ...
    if (!exceptionRaised) {
        // fixing shape
        //    shape = XSAlgo::AlgoContainer()->PerformFixShape ( shape, theProc, eps*CAS.GetUnitFactor(),
        //    CAS.GetMaxTol() );

        Handle(Standard_Transient) info;
        shape =
            XSAlgo::AlgoContainer()->ProcessShape(shape, eps * CAS.GetUnitFactor(), CAS.GetMaxTol(),
                                                  "read.iges.resource.name", "read.iges.sequence", info, aPS.Next());
        if (aPS.UserBreak()) return Standard_False;
