...
IGESToBRep_CurveAndSurface::IGESToBRep_CurveAndSurface()
    : myEps(1.e-04), myEpsCoeff(1.e-06), myEpsGeom(1.e-04), myMinTol(-1.0), myMaxTol(-1.0), myModeIsTopo(Standard_True),
      myModeApprox(Standard_False), myContIsOpti(Standard_False), myUnitFactor(1.0), mySurfaceCurve(0), myContinuity(0),
      myUVResolution(0.0), myIsResolCom(Standard_False), myTP(new Transfer_TransientProcess()) {
    UpdateMinMaxTol();
}
...
IGESToBRep_CurveAndSurface::IGESToBRep_CurveAndSurface(const Standard_Real eps, const Standard_Real epsCoeff,
                                                       const Standard_Real epsGeom, const Standard_Boolean mode,
                                                       const Standard_Boolean modeapprox,
                                                       const Standard_Boolean optimized)
    : myEps(eps), myEpsCoeff(epsCoeff), myEpsGeom(epsGeom), myMinTol(-1.0), myMaxTol(-1.0), myModeIsTopo(mode),
      myModeApprox(modeapprox), myContIsOpti(optimized), myUnitFactor(1.0), mySurfaceCurve(0), myContinuity(0),
      myUVResolution(0.0), myIsResolCom(Standard_False), myTP(new Transfer_TransientProcess()) {
    UpdateMinMaxTol();
}
...
void IGESToBRep_CurveAndSurface::SetModel(const Handle(IGESData_IGESModel) & model) {
    ...
    if (unitfactor != 1.) {
        if (myTP->TraceLevel() > 2) myTP->Messenger()->SendInfo() << "UnitFactor = " << unitfactor << std::endl;
...
TopoDS_Shape IGESToBRep_CurveAndSurface::TransferGeometry(const Handle(IGESData_IGESEntity) & start,
    ...
    if (IGESToBRep::IsCurveAndSurface(start)) {
        if (onlyvisible && start->BlankStatus() == 1) return res;
        try {
            OCC_CATCH_SIGNALS
    ...
    // 308 : SubfigureDefinition
        ...
        for (Standard_Integer i = 1; i <= st308->NbEntities() && PS.More(); i++) {
            ...
            if (onlyvisible && st308->AssociatedEntity(i)->BlankStatus() == 1) continue;
        ...
        for (Standard_Integer i = 1; i <= st402f1->NbEntities() && PS.More(); i++) {
            ...
            if (onlyvisible && st402f1->Entity(i)->BlankStatus() == 1) continue;
        ...
        for (Standard_Integer i = 1; i <= st402f7->NbEntities() && PS.More(); i++) {
            ...
            if (onlyvisible && st402f7->Entity(i)->BlankStatus() == 1) continue;
...
TopoDS_Shape IGESToBRep_CurveAndSurface::GetShapeResult(const Handle(IGESData_IGESEntity) & start) const {
    ...
    if (binder->HasResult()) res = binder->Result();
    return res;
}
...
void IGESToBRep_CurveAndSurface::SetShapeResult(const Handle(IGESData_IGESEntity) & start, const TopoDS_Shape& result) {
...
TopoDS_Shape IGESToBRep_CurveAndSurface::GetShapeResult(const Handle(IGESData_IGESEntity) & start,
    ...
    if (num <= binder->NbShapes()) res = binder->Shape(num);
    return res;
}
...
void IGESToBRep_CurveAndSurface::AddShapeResult(const Handle(IGESData_IGESEntity) & start, const TopoDS_Shape& result) {
    DeclareAndCast(TransferBRep_ShapeListBinder, binder, myTP->Find(start));
    if (binder.IsNull()) {
