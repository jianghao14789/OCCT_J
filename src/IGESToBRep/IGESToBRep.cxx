...
Standard_Boolean IGESToBRep::TransferPCurve(const TopoDS_Edge& fromedge, const TopoDS_Edge& toedge,
    ...
    if (!oldpcurve.IsNull()) {
        ...
        if (toedge.Orientation() == TopAbs_FORWARD)
            B.UpdateEdge(toedge, Handle(Geom2d_Curve)::DownCast(pcurve->Copy()),
                         Handle(Geom2d_Curve)::DownCast(oldpcurve->Copy()), face, 0);
        else
            B.UpdateEdge(toedge, Handle(Geom2d_Curve)::DownCast(oldpcurve->Copy()),
