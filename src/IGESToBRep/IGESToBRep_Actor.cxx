...
Standard_Boolean IGESToBRep_Actor::Recognize(const Handle(Standard_Transient) & start) {
    ...
    if (IGESToBRep::IsCurveAndSurface(ent) ||
        ((typnum == 402 && (fornum == 1 || fornum == 7 || fornum == 14 || fornum == 15)) || (typnum == 408) ||
         (typnum == 308)))
...
Handle(Transfer_Binder) IGESToBRep_Actor::Transfer(const Handle(Standard_Transient) & start,
    ...
    if (Interface_Static::IVal("read.iges.faulty.entities") == 0 && mymodel->IsErrorEntity(anum)) return NullResult();
    TopoDS_Shape shape;
    ...
    if (IGESToBRep::IsCurveAndSurface(ent) ||
        ...
        shape = XSAlgo::AlgoContainer()->ProcessShape(shape, theeps, CAS.GetMaxTol(), "read.iges.resource.name",
                                                      "read.iges.sequence", info, aPS.Next());
