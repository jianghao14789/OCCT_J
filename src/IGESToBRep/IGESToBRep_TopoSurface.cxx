...
// purpose  :
//=======================================================================
IGESToBRep_TopoSurface::IGESToBRep_TopoSurface() : IGESToBRep_CurveAndSurface() {}
...
IGESToBRep_TopoSurface::IGESToBRep_TopoSurface(const IGESToBRep_CurveAndSurface& CS) : IGESToBRep_CurveAndSurface(CS) {}
...
IGESToBRep_TopoSurface::IGESToBRep_TopoSurface(const Standard_Real eps, const Standard_Real epsCoeff,
                                               const Standard_Real epsGeom, const Standard_Boolean mode,
                                               const Standard_Boolean modeapprox, const Standard_Boolean optimized)
...
static Standard_Boolean extractCurve3d(const TopoDS_Shape& theEdges, Handle(Geom_Curve) & theCurve) {
    ...
    if (howMuch != 1 || theCurve.IsNull()) return Standard_False;
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferTopoSurface(const Handle(IGESData_IGESEntity) &
                                                         st) { // Declaration of messages//
                                                               // DCE 22/12/98
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferTopoBasicSurface(const Handle(IGESData_IGESEntity) &
                                                              st) { // Declaration of messages//
                                                                    // DCE 22/12/98
                                                                    // Message_Msg msg1005("IGES_1005");
                                                                    ////////////////////////////
    ...
    if (st->HasTransf()) {
        gp_Trsf trsf;
        SetEpsilon(1.E-04);
        if (IGESData_ToolLocation::ConvertLocation(GetEpsilon(), st->CompoundLocation(), trsf, GetUnitFactor())) {
...
// purpose  :
//=======================================================================
static void reparamBSpline(Handle(Geom_Curve) & curve, const Standard_Real First, const Standard_Real Last) {
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferRuledSurface(const Handle(IGESGeom_RuledSurface) &
                                                          st) { // Declaration of messages//
                                                                // DCE 22/12/98
                                                                // Message_Msg msg1005("IGES_1005");
                                                                ////////////////////////////////
    ...
    if (nbEdges1 != nbEdges2) {
        ...
        if (!ShapeAlgo::AlgoContainer()->HomoWires(wire1, wire2, newWire1, newWire2, st->IsRuledByParameter())) {
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferSurfaceOfRevolution(const Handle(IGESGeom_SurfaceOfRevolution) &
                                                                 st) { // Declaration of messages//
                                                                       // DCE 22/12/98
                                                                       // Message_Msg msg1005("IGES_1005");
                                                                       ////////////////////////////////
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferTabulatedCylinder(const Handle(IGESGeom_TabulatedCylinder) &
                                                               st) { // Declaration of messages//
                                                                     // DCE 22/12/98
                                                                     // Message_Msg msg1005("IGES_1005");
                                                                     ////////////////////////////////
            ...
            if (extractCurve3d(directrix, aBasisCurve)) {
                ...
                if (!aResultSurf.IsNull()) {
                    ...
                    BRepBuilderAPI_MakeFace aMakeF(aResultSurf, aBasisCurve->FirstParameter(),
                                                   aBasisCurve->LastParameter(), 0., dir.Magnitude(),
                                                   Precision::Confusion());
                    if (aMakeF.IsDone()) res = aMakeF.Face();
                }
            }
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferOffsetSurface(const Handle(IGESGeom_OffsetSurface) &
                                                           st) { // Declaration of messages//
                                                                 // DCE 22/12/98
                                                                 // Message_Msg msg1005("IGES_1005");
                                                                 ////////////////////////////////
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferTrimmedSurface(const Handle(IGESGeom_TrimmedSurface) &
                                                            st) { // Declaration of messages//
                                                                  // DCE 22/12/98
                                                                  // Message_Msg msg1005("IGES_1005");
                                                                  ////////////////////////////////
    ...
    if (!aTransf.IsNull()) {
        ...
        if (fabs(s1 - s2) > Precision::Confusion() || fabs(s1 - s3) > Precision::Confusion()) {
            ...
            if ((tmpVal + aTrans.Modulus()) > Precision::Confusion()) {
                // not Identity
                gp_Trsf aT;
                aT.SetValues(aMat.Value(1, 1), aMat.Value(1, 2), aMat.Value(1, 3), aTrans.X(), aMat.Value(2, 1),
                             aMat.Value(2, 2), aMat.Value(2, 3), aTrans.Y(), aMat.Value(3, 1), aMat.Value(3, 2),
                             aMat.Value(3, 3), aTrans.Z());
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferBoundedSurface(const Handle(IGESGeom_BoundedSurface) &
                                                            st) { // Declaration of messages//
                                                                  // DCE 22/12/98
                                                                  // Message_Msg msg1005("IGES_1005");
                                                                  ////////////////////////////////
...
#ifdef OCCT_DEBUG
        std::cout << "Fail: IGESToBRep_TopoSurface::TransferBoundedSurface  UntrimmedSurface is translated into Null"
                  << std::endl;
#endif
        return res;
    } else {
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferPlane(const Handle(IGESGeom_Plane) &
                                                   st) { // Declaration of messages//
                                                         // DCE 22/12/98
                                                         // Message_Msg msg1005("IGES_1005");
                                                         ////////////////////////////////
...
TopoDS_Shape IGESToBRep_TopoSurface::TransferPerforate(const Handle(IGESBasic_SingleParent) &
                                                       st) { // Declaration of messages//
                                                             // DCE 22/12/98
                                                             // Message_Msg msg1005("IGES_1005");
                                                             ////////////////////////////////
...
//=======================================================================
TopoDS_Shape
IGESToBRep_TopoSurface::TransferPlaneParts(const Handle(IGESGeom_Plane) & st, gp_Pln& pln, gp_Trsf& trsf,
                                           const Standard_Boolean first) { // Declaration of messages//
                                                                           // DCE 22/12/98
                                                                           // Message_Msg msg1005("IGES_1005");
                                                                           ////////////////////////////////
    TopoDS_Shape res;
        ...
        if (crv.IsNull()) {
            ...
            if (IGESToBRep::IsTopoCurve(crv)) {
                ...
                if (crv->IsKind(STANDARD_TYPE(IGESGeom_CurveOnSurface))) {
                    ...
                    if (G.Mass() < 0) {
                        if (!st->HasBoundingCurveHole()) wire.Reverse();
...
// purpose  :
//=======================================================================
TopoDS_Shape IGESToBRep_TopoSurface::ParamSurface(const Handle(IGESData_IGESEntity) & st, gp_Trsf2d& trans,
                                                  Standard_Real& uFact) { // Declaration of messages//
                                                                          // DCE 22/12/98
                                                                          // Message_Msg msg1005("IGES_1005");
                                                                          ////////////////////////////////
    ...
    if ((Surf->IsKind(STANDARD_TYPE(Geom_CylindricalSurface))) || (Surf->IsKind(STANDARD_TYPE(Geom_ConicalSurface))) ||
        (Surf->IsKind(STANDARD_TYPE(Geom_ToroidalSurface))) || (Surf->IsKind(STANDARD_TYPE(Geom_SphericalSurface)))) {
        TopExp_Explorer TE;
        ...
        if (Surf->IsKind(STANDARD_TYPE(Geom_CylindricalSurface))) {
            ...
            ElSLib::CylinderParameters(TheCyl.Position(), TheCyl.Radius(), Curve3d->Value(First), paramu, paramv);
        }
            ...
            ElSLib::ConeParameters(TheCone.Position(), TheCone.RefRadius(), TheCone.SemiAngle(), Curve3d->Value(First),
                                   paramu, paramv);
        }
            ...
            ElSLib::TorusParameters(TheTore.Position(), TheTore.MajorRadius(), TheTore.MinorRadius(),
            ...
            ElSLib::SphereParameters(TheSphere.Position(), TheSphere.Radius(), Curve3d->Value(First), paramu, paramv);
        }
        ...
        if (st->IsKind(STANDARD_TYPE(IGESGeom_SurfaceOfRevolution))) {
            ...
            if (generatrix->IsKind(STANDARD_TYPE(IGESGeom_CircularArc))) {
                ...
                paramv -=
                    ElCLib::Parameter(gp_Circ2d(gp_Ax2d(circ->Center(), gp_Dir2d(1, 0)), circ->Radius()), startpoint);
    ...
    if (isrf->IsKind(STANDARD_TYPE(IGESSolid_ToroidalSurface))) {
        ...
        tmp.SetMirror(gp_Ax2d(gp::Origin2d(), gp_Dir2d(1., 1.)));
        trans.PreMultiply(tmp);
        if (paramv > 0) paramv = paramv * 180. / M_PI;
        cscale = M_PI / 180.;
        uscale = 1.;
    }
