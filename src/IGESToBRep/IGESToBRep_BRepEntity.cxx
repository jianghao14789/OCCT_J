...
// purpose  :
//=======================================================================
IGESToBRep_BRepEntity::IGESToBRep_BRepEntity() : IGESToBRep_CurveAndSurface() {
...
// purpose  :
//=======================================================================
IGESToBRep_BRepEntity::IGESToBRep_BRepEntity(const IGESToBRep_CurveAndSurface& CS) : IGESToBRep_CurveAndSurface(CS) {
    SetContinuity(0);
}
...
// purpose  :
//=======================================================================
IGESToBRep_BRepEntity::IGESToBRep_BRepEntity(const Standard_Real eps, const Standard_Real epsCoeff,
                ...
                if (!Sh.IsNull()) {
                    if (Sh.ShapeType() == TopAbs_EDGE) {
                        ...
                        if (V1.IsSame(V2) || dist1f + dist2l <= dist1l + dist2f + Precision::Confusion()) {
                            //: 77 if (BRepTools::Compare(V1, Vf)) //the part 'else' only if, in fact, edge should be
                            //: reversed
                            V1.Orientation(TopAbs_FORWARD);
                            B.Add(E, V1);
                            V2.Orientation(TopAbs_REVERSED);
                            B.Add(E, V2);
                            B.UpdateVertex(V1, first, E, 0.); // S4054 1.001 * dist1f //:77
                        ...
                        // modif mjm du 13/10/97 : Reverse de l`edge ?
                            ...
                            B.UpdateVertex(V2, first, E, 0.); // S4054 1.001 * dist2f //:77
                                                              // GetEpsGeom()*GetUnitFactor();
                            B.Range(E, first, last);
                        }
                        ...
                        // pas traite
                        Message_Msg Msg1325("IGES_1325"); //"Item %d of EdgeList cannot be represented by single edge
                                                          //(non-continuous or composite curve)."
                        Msg1325.Arg(inum);
...
// purpose  :
//=======================================================================
TopoDS_Shape IGESToBRep_BRepEntity::TransferLoop(const Handle(IGESSolid_Loop) & start, const TopoDS_Face& face,
        ...
        for (Standard_Integer iedge = 1; iedge <= start->NbEdges(); iedge++) {
            ...
            if (theedge.IsNull()) {
                ...
                if ((itype == 1) && (theedge->IsKind(STANDARD_TYPE(IGESSolid_VertexList)))) {
                    ...
                    if (Sh.IsNull()) {
                        continue; // skip non-translated edge hoping for the best; warning is already generated by
                                  // TransferEdge()
                    }
                    curve3d->Add(Sh);
                } else {
                ...
                Result =
                    Result & IB->Transfer(okCurve, okCurve3d, okCurve2d, curve3d, Curves2d, !orientation, iedge, lsewd);
...
TopoDS_Shape IGESToBRep_BRepEntity::TransferFace(const Handle(IGESSolid_Face) & start) {
    ...
    if (!HasShapeResult(start)) {
        ...
        // surface
        // -------
        if (surf.IsNull() || !IGESToBRep::IsTopoSurface(surf) || surf->IsKind(STANDARD_TYPE(IGESGeom_Plane)) ||
