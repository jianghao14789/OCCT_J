...
static Standard_Boolean checkBSplineSurface(IGESToBRep_BasicSurface* theSurface,
                                            ...
                                            TColStd_Array1OfReal& SUKnots, TColStd_Array1OfReal& SVKnots,
...
IGESToBRep_BasicSurface::IGESToBRep_BasicSurface() : IGESToBRep_CurveAndSurface() {
...
IGESToBRep_BasicSurface::IGESToBRep_BasicSurface(const Standard_Real eps, const Standard_Real epsCoeff,
...
Handle(Geom_Surface) IGESToBRep_BasicSurface::TransferBasicSurface(const Handle(IGESData_IGESEntity) & start) {
...
Handle(Geom_CylindricalSurface)
    IGESToBRep_BasicSurface::TransferRigthCylindricalSurface(const Handle(IGESSolid_CylindricalSurface) & start) {
...
Handle(Geom_ConicalSurface)
    IGESToBRep_BasicSurface::TransferRigthConicalSurface(const Handle(IGESSolid_ConicalSurface) & start) {
    ...
    if (radius < Precision::Confusion()) radius = 0.;
...
Handle(Geom_SphericalSurface)
    IGESToBRep_BasicSurface::TransferSphericalSurface(const Handle(IGESSolid_SphericalSurface) & start) {
...
Handle(Geom_ToroidalSurface) IGESToBRep_BasicSurface::TransferToroidalSurface(const Handle(IGESSolid_ToroidalSurface) &
                                                                              start) {
...
Handle(Geom_BSplineSurface) IGESToBRep_BasicSurface::TransferSplineSurface(const Handle(IGESGeom_SplineSurface) &
                                                                           start) {
...
Handle(Geom_BSplineSurface) IGESToBRep_BasicSurface::TransferBSplineSurface(const Handle(IGESGeom_BSplineSurface) &
                                                                            start) {
    ...
    if (start->IsPolynomial()) {
        ...
        for (i = 0; i <= start->UpperIndexU(); i++) {
            for (j = 0; j <= start->UpperIndexV(); j++) {
                polynomial = (Abs(start->Weight(i, j) - WeightReference) <= Epsilon(WeightReference)) && polynomial;
                //: 39 by abv 15.12.97
