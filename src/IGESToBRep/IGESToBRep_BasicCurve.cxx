...
static Standard_Boolean checkBSplineCurve(IGESToBRep_BasicCurve* theCurve,
                                          ...
                                          TColStd_Array1OfReal& CKnots, const TColStd_Array1OfReal& CWeights) {
...
// purpose  :
//=======================================================================
IGESToBRep_BasicCurve::IGESToBRep_BasicCurve() : IGESToBRep_CurveAndSurface() {
...
// purpose  :
//=======================================================================
IGESToBRep_BasicCurve::IGESToBRep_BasicCurve(const IGESToBRep_CurveAndSurface& CS) : IGESToBRep_CurveAndSurface(CS) {}

//=======================================================================
// function : IGESToBRep_BasicCurve
// purpose  :
//=======================================================================
IGESToBRep_BasicCurve::IGESToBRep_BasicCurve(const Standard_Real eps, const Standard_Real epsCoeff,
...
Handle(Geom_Curve) IGESToBRep_BasicCurve::TransferBasicCurve(const Handle(IGESData_IGESEntity) & start) {
...
Handle(Geom2d_Curve) IGESToBRep_BasicCurve::Transfer2dBasicCurve(const Handle(IGESData_IGESEntity) & start) {
...
Handle(Geom2d_Curve) IGESToBRep_BasicCurve::Transfer2dConicArc(const Handle(IGESGeom_ConicArc) & st) {
    ...
    if (st->IsFromEllipse()) {
        ...
        if ((Abs(a - c) <= eps2) && (Abs(b) < eps2)) {
            ...
            // #45 rln 23.11.98
            if (st->TransformedAxis().IsOpposite(st->Axis(), GetEpsilon())) res->Reverse();
    ...
    if (st->IsFromEllipse()) {
        ...
        if (!st->IsClosed()) {
            ...
            gp_Elips2d elips =
                Handle(Geom2d_Ellipse)::DownCast(res)->Elips2d(); // #45 rln (frame, majorRadius, minorRadius);
...
Handle(Geom_Curve) IGESToBRep_BasicCurve::TransferCircularArc(const Handle(IGESGeom_CircularArc) & st) {
    Handle(Geom_Curve) res;
    if (st.IsNull()) {
        Message_Msg msg1005("IGES_1005");
        SendFail(st, msg1005);
...
Handle(Geom2d_Curve) IGESToBRep_BasicCurve::Transfer2dCircularArc(const Handle(IGESGeom_CircularArc) & st) {
    Handle(Geom2d_Curve) res;
    if (st.IsNull()) {
        Message_Msg msg1005("IGES_1005");
        SendFail(st, msg1005);
...
Handle(Geom_BSplineCurve) IGESToBRep_BasicCurve::TransferSplineCurve(const Handle(IGESGeom_SplineCurve) & st) {
...
Handle(Geom2d_BSplineCurve) IGESToBRep_BasicCurve::Transfer2dSplineCurve(const Handle(IGESGeom_SplineCurve) & st) {
    ...
    SetEpsGeom(epsGeom);
    if (res3d.IsNull()) return res; // The transfer was not over the top.
...
Handle(Geom_Curve) IGESToBRep_BasicCurve::TransferBSplineCurve(const Handle(IGESGeom_BSplineCurve) & start)

{
    ...
    if (start->IsPolynomial()) {
        ...
        for (i = 0; i <= start->UpperIndex(); i++) {
            polynomial = Abs(start->Weight(i) - WeightReference) <= Epsilon(WeightReference) && polynomial;
            //: 39 by abv 15.12.97
...
#ifdef OCCT_DEBUG
            std::cout << "\n** Exception in IGESToBRep_BasicCurve::TransferBSplineCurve during creation of "
                         "Geom_BSplineCurve : ";
            anException.Print(std::cout);
#endif
            (void)anException;
    ...
    if (((Udeb - First) > -Precision::PConfusion() && (Last - Ufin) > -Precision::PConfusion()) && Udeb <= Ufin) {
        try {
            OCC_CATCH_SIGNALS
            if (Abs(Ufin - Udeb) > Precision::PConfusion()) BSplineRes->Segment(Udeb, Ufin);
            res = BSplineRes;
...
Handle(Geom2d_Curve) IGESToBRep_BasicCurve::Transfer2dBSplineCurve(const Handle(IGESGeom_BSplineCurve) & start) {
    Handle(Geom2d_Curve) res;
    if (start.IsNull()) {
        Message_Msg msg1005("IGES_1005");
        SendFail(start, msg1005);
...
Handle(Geom_Transformation) IGESToBRep_BasicCurve::TransferTransformation(const Handle(IGESGeom_TransformationMatrix) &
                                                                          start)
...
Handle(Geom_BSplineCurve) IGESToBRep_BasicCurve::TransferCopiousData(const Handle(IGESGeom_CopiousData) & start)

{
    ...
    for (i = 2; i <= NbPoints; i++) {
        ...
        if (!aPole.IsEqual(TempPole(TempIndex - 1), gp::Resolution())) TempPole.SetValue(TempIndex++, aPole);
    }
...
Handle(Geom2d_BSplineCurve) IGESToBRep_BasicCurve::Transfer2dCopiousData(const Handle(IGESGeom_CopiousData) & start) {
    ...
    if (!GetModeTransfer() && start->HasTransf())
        TempPole.SetValue(TempIndex, gp_Pnt2d(start->TransformedPoint(1).X(), start->TransformedPoint(1).Y()));
    else
    ...
    for (i = 2; i <= NbPoints; i++) {
        ...
        //    if (!aPole.IsEqual(TempPole(TempIndex-1), GetEpsCoeff())) //modified by rln 16/12/97 CSR# PRO11641 entity
        //    46GetEpsGeom()*GetUnitFactor()
        // S4054: some filter must be kept UKI60556 entity 7 (two equal points)
        if (!aPole.IsEqual(TempPole(TempIndex - 1), gp::Resolution())) TempPole.SetValue(TempIndex++, aPole);
    }
