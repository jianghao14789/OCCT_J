...
// purpose  :
//=======================================================================
IGESToBRep_TopoCurve::IGESToBRep_TopoCurve() : IGESToBRep_CurveAndSurface() {}
...
IGESToBRep_TopoCurve::IGESToBRep_TopoCurve(const IGESToBRep_TopoCurve& other) : IGESToBRep_CurveAndSurface(other) {
...
IGESToBRep_TopoCurve::IGESToBRep_TopoCurve(const IGESToBRep_CurveAndSurface& CS) : IGESToBRep_CurveAndSurface(CS) {}
...
IGESToBRep_TopoCurve::IGESToBRep_TopoCurve(const Standard_Real eps, const Standard_Real epsCoeff,
...

{
    ...
    if (GetModeTransfer() && start->HasTransf()) {
        gp_Trsf T;
        SetEpsilon(1.E-04);
        if (IGESData_ToolLocation::ConvertLocation(GetEpsilon(), start->CompoundLocation(), T, GetUnitFactor())) {
            TopLoc_Location L(T);
            V1.Move(L);
        }
    }
...
TopoDS_Shape IGESToBRep_TopoCurve::TransferCompositeCurveGeneral(const Handle(IGESGeom_CompositeCurve) & start,
                                                                 const Standard_Boolean is2d, const TopoDS_Face& face,
                                                                 const gp_Trsf2d& trans, const Standard_Real uFact)
    ...
    for (Standard_Integer i = 1; i <= start->NbCurves(); i++) {
        ...
        if (is2d && (IgesEnt->IsKind(STANDARD_TYPE(IGESGeom_Boundary)) ||
            ...
            if (!shape.IsNull()) {
                ...
                Standard_Boolean isConnected =
                    ShapeAlgo::AlgoContainer()->ConnectNextWire(saw, nextsewd, maxtol, distmin, revsewd, revnextsewd);
                if (isConnected) {
                    if (revsewd) {
                        Message_Msg Msg1051(
                            "IGES_1051"); //"All curvAll curves %dd before rank %d have been to be reversed."
...
TopoDS_Shape IGESToBRep_TopoCurve::Transfer2dCompositeCurve(const Handle(IGESGeom_CompositeCurve) & start,
                                                            const TopoDS_Face& face, const gp_Trsf2d& trans,
...
TopoDS_Shape IGESToBRep_TopoCurve::TransferCurveOnFace(TopoDS_Face& face, const Handle(IGESGeom_CurveOnSurface) & start,
                                                       const gp_Trsf2d& trans, const Standard_Real uFact,
...
TopoDS_Shape IGESToBRep_TopoCurve::TransferTopoBasicCurve(const Handle(IGESData_IGESEntity) & start) {
    ...
    //  2d representation
    if (start->IsKind(STANDARD_TYPE(IGESGeom_CircularArc)) && Handle(IGESGeom_CircularArc)::DownCast(start)->IsClosed())
...
TopoDS_Shape IGESToBRep_TopoCurve::Transfer2dTopoBasicCurve(const Handle(IGESData_IGESEntity) & start,
    ...
    for (Standard_Integer icurve = 1; icurve <= nbcurves; icurve++) {
        ...
        if (trans.Form() != gp_Identity) mycurve2d->Transform(trans);

        gp_Trsf2d ntrsf;
        if (mysurf->IsKind(STANDARD_TYPE(Geom_Plane))) ntrsf.SetScale(gp_Pnt2d(0, 0), GetUnitFactor());
        ...
        if ((mycurve2d->IsKind(STANDARD_TYPE(Geom2d_BSplineCurve))) && (mycurve2d->Continuity() < GeomAbs_C1) &&
            GetContinuity() >= 1) {
...
TopoDS_Shape IGESToBRep_TopoCurve::TransferBoundaryOnFace(TopoDS_Face& face, const Handle(IGESGeom_Boundary) & start,
                                                          const gp_Trsf2d& trans, const Standard_Real uFact) {
    TopoDS_Shape res;
    if (start.IsNull()) {
    ...
    for (Standard_Integer i = 1; i <= start->NbModelSpaceCurves(); i++) {
        ...
        Result = Result & IB->Transfer(okCurve, okCurve3d, okCurve2d, start->ModelSpaceCurve(i), start->Sense(i) == 2,
                                       Curves2d, i);
    }
