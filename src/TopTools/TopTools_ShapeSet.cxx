...
const Standard_CString TopTools_ShapeSet::THE_ASCII_VERSIONS[TopTools_FormatVersion_UPPER + 1] = {
    "", "CASCADE Topology V1, (c) Matra-Datavision", "CASCADE Topology V2, (c) Matra-Datavision",
...
// purpose  :
//=======================================================================
TopTools_ShapeSet::TopTools_ShapeSet() : myFormatNb(TopTools_FormatVersion_CURRENT) {}
...
void TopTools_ShapeSet::SetFormatNb(const Standard_Integer theFormatNb) {
    Standard_ASSERT_RETURN(theFormatNb >= TopTools_FormatVersion_LOWER && theFormatNb <= TopTools_FormatVersion_UPPER,
...
void TopTools_ShapeSet::Dump(Standard_OStream& OS) const {
    ...
    for (i = nbShapes; i >= 1; i--) {
        ...
        while (its.More()) {
            ...
            if (!sub.Location().IsIdentity()) OS << "(L" << myLocations.Index(sub.Location()) << ")";
            OS << " ";
            its.Next();
        }
        OS << "\n";
...
void TopTools_ShapeSet::Write(Standard_OStream& OS, const Message_ProgressRange& theProgress) {
    ...
    if (aPS.UserBreak()) OS << "Interrupted by the user\n";
}
...
void TopTools_ShapeSet::Read(Standard_IStream& IS, const Message_ProgressRange& theProgress) {
    ...
    for (i = 1; i <= nbShapes && aPS1.More(); i++, aPS1.Next()) {
        ...
        if (myFormatNb == TopTools_FormatVersion_VERSION_1) Check(T, S);
    ...
    if (aPS.UserBreak()) std::cout << "Interrupted by the user" << std::endl;
}
...
void TopTools_ShapeSet::Dump(const TopoDS_Shape& S, Standard_OStream& OS) const {
    ...
    if (!S.Location().IsIdentity()) OS << ", location : " << myLocations.Index(S.Location());
    OS << "\n";
}
...
void TopTools_ShapeSet::Read(TopoDS_Shape& S, Standard_IStream& IS, const Standard_Integer nbshapes) const {
    if (nbshapes < 1) {
        S = TopoDS_Shape();
        return;
    }
