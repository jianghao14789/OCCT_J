...
void IGESDimen_ToolSection::WriteOwnParams(const Handle(IGESDimen_Section) & ent, IGESData_IGESWriter& IW) const {
...
void IGESDimen_ToolSection::OwnCopy(const Handle(IGESDimen_Section) & another, const Handle(IGESDimen_Section) & ent,
...
void IGESDimen_ToolSection::OwnCheck(const Handle(IGESDimen_Section) & ent, const Interface_ShareTool&,
                                     Handle(Interface_Check) & ach) const {
    if (ent->RankLineFont() != 1) ach->AddFail("Line Font Pattern != 1");
    if (ent->Datatype() != 1) ach->AddFail("Interpretation Flag != 1");
    if (ent->NbPoints() % 2 != 0) ach->AddFail("Number of data points is not even");
}

void IGESDimen_ToolSection::OwnDump(const Handle(IGESDimen_Section) & ent, const IGESData_IGESDumper& /* dumper */,
    ...
    S << "IGESDimen_Section\n"
      ...
      << "Data Points : ";
    IGESData_DumpListXYLZ(S, level, 1, ent->NbPoints(), ent->Point, ent->Location(), ent->ZDisplacement());
    S << std::endl;
}
