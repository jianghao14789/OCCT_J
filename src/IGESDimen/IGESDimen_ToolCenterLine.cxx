...
void IGESDimen_ToolCenterLine::ReadOwnParams(const Handle(IGESDimen_CenterLine) & ent,
    ...
    if (!dataPoints.IsNull())
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadXY(PR.CurrentList(1, 2), "Data Points", tempXY)) dataPoints->SetValue(i, tempXY);
        }
...
void IGESDimen_ToolCenterLine::WriteOwnParams(const Handle(IGESDimen_CenterLine) & ent, IGESData_IGESWriter& IW) const {
...
IGESData_DirChecker IGESDimen_ToolCenterLine::DirChecker(const Handle(IGESDimen_CenterLine) & /* ent */) const {
...
void IGESDimen_ToolCenterLine::OwnCheck(const Handle(IGESDimen_CenterLine) & ent, const Interface_ShareTool&,
