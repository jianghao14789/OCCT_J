...
void IGESDimen_ToolNewDimensionedGeometry::ReadOwnParams(const Handle(IGESDimen_NewDimensionedGeometry) & ent,
    ...
    ent->Init(tempNbDimens, tempDimen, tempDimOrientFlag, tempAngle, tempGeomEnts, tempDimLocFlags, tempPoints);
}

void IGESDimen_ToolNewDimensionedGeometry::WriteOwnParams(const Handle(IGESDimen_NewDimensionedGeometry) & ent,
...
void IGESDimen_ToolNewDimensionedGeometry::OwnShared(const Handle(IGESDimen_NewDimensionedGeometry) & ent,
...
void IGESDimen_ToolNewDimensionedGeometry::OwnCopy(const Handle(IGESDimen_NewDimensionedGeometry) & another,
...
Standard_Boolean IGESDimen_ToolNewDimensionedGeometry::OwnCorrect(const Handle(IGESDimen_NewDimensionedGeometry) &
                                                                  ent) const {
    ...
    ent->Init(1, ent->DimensionEntity(), ent->DimensionOrientationFlag(), ent->AngleValue(), tempGeomEnts,
              tempDimLocFlags, tempPoints);
    return Standard_True;
}

IGESData_DirChecker
IGESDimen_ToolNewDimensionedGeometry::DirChecker(const Handle(IGESDimen_NewDimensionedGeometry) & /* ent */) const {
...
void IGESDimen_ToolNewDimensionedGeometry::OwnCheck(const Handle(IGESDimen_NewDimensionedGeometry) & ent,
                                                    const Interface_ShareTool&, Handle(Interface_Check) & ach) const {
    if (ent->NbDimensions() != 1) ach->AddFail("Number of Dimensions != 1");
    if (ent->HasTransf()) ach->AddWarning("Transformation Matrix exists, ignored");
}

void IGESDimen_ToolNewDimensionedGeometry::OwnDump(const Handle(IGESDimen_NewDimensionedGeometry) & ent,
