...
void IGESDimen_ToolRadiusDimension::ReadOwnParams(const Handle(IGESDimen_RadiusDimension) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "General Note", STANDARD_TYPE(IGESDimen_GeneralNote),
                  tempNote); // szv#4:S4163:12Mar99 `st=` not needed

    PR.ReadEntity(IR, PR.Current(), "Leader arrow", STANDARD_TYPE(IGESDimen_LeaderArrow),
    ...
    if (ent->FormNumber() == 1)
        PR.ReadEntity(IR, PR.Current(), "Leader arrow 2", STANDARD_TYPE(IGESDimen_LeaderArrow), leadArr2,
...
void IGESDimen_ToolRadiusDimension::WriteOwnParams(const Handle(IGESDimen_RadiusDimension) & ent,
    ...
    if (ent->HasLeader2() || ent->FormNumber() == 1) IW.Send(ent->Leader2());
}
...
void IGESDimen_ToolRadiusDimension::OwnCopy(const Handle(IGESDimen_RadiusDimension) & another,
    ...
    Handle(IGESDimen_LeaderArrow) leadArr2;
    if (another->HasLeader2()) leadArr2 = GetCasted(IGESDimen_LeaderArrow, TC.Transferred(another->Leader2()));
    ent->Init(tempNote, leadArr, arcCenter, leadArr2);
    ent->InitForm(another->FormNumber());
}

IGESData_DirChecker
IGESDimen_ToolRadiusDimension::DirChecker(const Handle(IGESDimen_RadiusDimension) & /* ent */) const {
...
void IGESDimen_ToolRadiusDimension::OwnCheck(const Handle(IGESDimen_RadiusDimension) & ent, const Interface_ShareTool&,
