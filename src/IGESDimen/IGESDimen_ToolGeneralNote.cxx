...
void IGESDimen_ToolGeneralNote::ReadOwnParams(const Handle(IGESDimen_GeneralNote) & ent,
    ...
    if (!(nbChars.IsNull() || boxWidths.IsNull() || boxHeights.IsNull() || fontCodes.IsNull() ||
          fontEntities.IsNull() || slantAngles.IsNull() || rotationAngles.IsNull() || mirrorFlags.IsNull() ||
          rotateFlags.IsNull() || startPoints.IsNull() || texts.IsNull())) {
        DirChecker(ent).CheckTypeAndForm(PR.CCheck(), ent);
        ent->Init(nbChars, boxWidths, boxHeights, fontCodes, fontEntities, slantAngles, rotationAngles, mirrorFlags,
...
void IGESDimen_ToolGeneralNote::OwnShared(const Handle(IGESDimen_GeneralNote) & ent,
    ...
    Standard_Integer nbval = ent->NbStrings();
    for (Standard_Integer i = 1; i <= nbval; i++) {
        if (ent->IsFontEntity(i)) iter.GetOneItem(ent->FontEntity(i));
    }
}
...
IGESData_DirChecker IGESDimen_ToolGeneralNote::DirChecker(const Handle(IGESDimen_GeneralNote) & /* ent */) const {
...
void IGESDimen_ToolGeneralNote::OwnCheck(const Handle(IGESDimen_GeneralNote) & ent, const Interface_ShareTool&,
...
void IGESDimen_ToolGeneralNote::OwnDump(const Handle(IGESDimen_GeneralNote) & ent, const IGESData_IGESDumper& dumper,
