...
IGESData_DirChecker IGESDimen_ToolGeneralSymbol::DirChecker(const Handle(IGESDimen_GeneralSymbol) & /* ent */) const {
...
void IGESDimen_ToolGeneralSymbol::OwnCheck(const Handle(IGESDimen_GeneralSymbol) & ent, const Interface_ShareTool&,
                                           Handle(Interface_Check) & ach) const {
    if ((ent->FormNumber() < 0 || ent->FormNumber() > 3) && (ent->FormNumber() < 5001 || ent->FormNumber() > 9999))
        ach->AddFail("Invalid Form Number");
    if (ent->FormNumber() != 0)
        if (ent->Note().IsNull()) ach->AddFail("No General Note defined for form number non 0");
}
