...
void IGESDimen_ToolLeaderArrow::ReadOwnParams(const Handle(IGESDimen_LeaderArrow) & ent,
    ...
    if (!segmentTails.IsNull()) {
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadXY(PR.CurrentList(1, 2), "Segment Co-ords.", tempXY)) segmentTails->SetValue(i, tempXY);
        }
...
IGESData_DirChecker IGESDimen_ToolLeaderArrow::DirChecker(const Handle(IGESDimen_LeaderArrow) & /* ent */) const {
...
void IGESDimen_ToolLeaderArrow::OwnCheck(const Handle(IGESDimen_LeaderArrow) & /* ent */, const Interface_ShareTool&,
...
void IGESDimen_ToolLeaderArrow::OwnDump(const Handle(IGESDimen_LeaderArrow) & ent,
    ...
    S << "\nSegment Tails : ";
    IGESData_DumpListXYLZ(S, level, 1, ent->NbSegments(), ent->SegmentTail, ent->Location(), ent->ZDepth());
    S << std::endl;
}
