...
Standard_Boolean IGESDimen_ToolWitnessLine::OwnCorrect(const Handle(IGESDimen_WitnessLine) & ent) const {
...
IGESData_DirChecker IGESDimen_ToolWitnessLine::DirChecker(const Handle(IGESDimen_WitnessLine) & /* ent */) const {
...
void IGESDimen_ToolWitnessLine::OwnCheck(const Handle(IGESDimen_WitnessLine) & ent, const Interface_ShareTool&,
                                         Handle(Interface_Check) & ach) const {
    if (ent->RankLineFont() != 1) ach->AddFail("Line Font Pattern != 1");
    if (ent->Datatype() != 1) ach->AddFail("Interpretation Flag != 1");
    if (ent->NbPoints() < 3) ach->AddFail("Number of data points < 3");
    if (ent->NbPoints() % 2 == 0) ach->AddFail("Number of data points is not odd");
}
