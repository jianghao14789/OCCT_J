...
void IGESDimen_FlagNote::Init(const gp_XYZ& leftCorner, const Standard_Real anAngle,
                              const Handle(IGESDimen_GeneralNote) & aNote,
                              const Handle(IGESDimen_HArray1OfLeaderArrow) & someLeaders) {
    if (!someLeaders.IsNull())
        if (someLeaders->Lower() != 1) throw Standard_DimensionMismatch("IGESDimen_FlagNote : Init");
...
Standard_Real IGESDimen_FlagNote::CharacterHeight() const {
    ...
    for (Standard_Integer i = 2; i <= theNote->NbStrings(); i++) {
        if (Max < theNote->BoxHeight(i)) Max = theNote->BoxHeight(i);
    }
    return (Max);
}
