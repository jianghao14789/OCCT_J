...
void IGESDimen_SectionedArea::Init(const Handle(IGESData_IGESEntity) & aCurve, const Standard_Integer aPattern,
                                   const gp_XYZ& aPoint, const Standard_Real aDistance, const Standard_Real anAngle,
                                   const Handle(IGESData_HArray1OfIGESEntity) & someIslands) {
    if (!someIslands.IsNull())
        if (someIslands->Lower() != 1) throw Standard_DimensionMismatch("IGESDimen_SectionedArea : Init");
