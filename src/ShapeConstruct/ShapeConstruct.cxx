...
// purpose  :
//=======================================================================
Handle(Geom_BSplineCurve) ShapeConstruct::ConvertCurveToBSpline(
    const Handle(Geom_Curve) & C3D, const Standard_Real First, const Standard_Real Last, const Standard_Real Tol3d,
    const GeomAbs_Shape Continuity, const Standard_Integer MaxSegments, const Standard_Integer MaxDegree) {
    Standard_Integer MaxDeg = MaxDegree;
    Handle(Geom_BSplineCurve) aBSpline;
    if (C3D->IsKind(STANDARD_TYPE(Geom_BSplineCurve)))
        aBSpline = Handle(Geom_BSplineCurve)::DownCast(C3D);
    else {
        if (C3D->IsKind(STANDARD_TYPE(Geom_Conic))) MaxDeg = Min(MaxDeg, 6);
...
Handle(Geom2d_BSplineCurve) ShapeConstruct::ConvertCurveToBSpline(
    const Handle(Geom2d_Curve) & C2D, const Standard_Real First, const Standard_Real Last, const Standard_Real Tol2d,
    const GeomAbs_Shape Continuity, const Standard_Integer MaxSegments, const Standard_Integer MaxDegree) {
...
Handle(Geom_BSplineSurface) ShapeConstruct::ConvertSurfaceToBSpline(
    const Handle(Geom_Surface) & surf, const Standard_Real UF, const Standard_Real UL, const Standard_Real VF,
    const Standard_Real VL, const Standard_Real Tol3d, const GeomAbs_Shape Continuity,
    const Standard_Integer MaxSegments, const Standard_Integer MaxDegree) {
    ...
    if (S->IsKind(STANDARD_TYPE(Geom_ElementarySurface))) {
        Handle(Geom_RectangularTrimmedSurface) aRTS = new Geom_RectangularTrimmedSurface(S, UF, UL, VF, VL);
    ...
    if (S->IsKind(STANDARD_TYPE(Geom_SurfaceOfLinearExtrusion))) {
        ...
        Handle(Geom_BSplineSurface) bspline = new Geom_BSplineSurface(
            resPoles, resWeigth, knots, vknots, mults, vmults, bspl->Degree(), 1, bspl->IsPeriodic(), Standard_False);
        return bspline;
    }
    ...
    for (Standard_Integer cnt = (Continuity > GeomAbs_C3 ? GeomAbs_C3 : Continuity); cnt >= 0;) {
        try {
            ...
            if (anApprox.MaxError() <= Tol3d && Done) {
                ...
                if (anApprox.HasResult()) errSpl = anApprox.Surface();
#ifdef OCCT_DEBUG
...
Standard_Boolean ShapeConstruct::JoinPCurves(const Handle(TopTools_HSequenceOfShape) & edges,
                                             const TopoDS_Face& theFace, TopoDS_Edge& theEdge) {
    ...
    try {
        ...
        if (aGeomSurf->IsKind(STANDARD_TYPE(Geom_Plane))) return Standard_True;
        ...
        for (; i <= edges->Length(); i++) {
            ...
            if (!sae.PCurve(Edge, theFace, c2d, first, last, Standard_False)) break;
            ...
            if (i == 1) {
                ...
                if (!JoinCurves(aCrvRes1, c2d, resOrient, Edge.Orientation(), newf, newl, first, last, newCrv, isRev1,
                                isRev2))
                    break;
                ...
                if (IsEdgeSeam) {
                    ...
                    if (!JoinCurves(aCrvRes2, c2d2, resOrient, Edge.Orientation(), newf2, newl2, first2, last2, newCrv2,
                                    isRev1, isRev2))
                        break;
                    aCrvRes2 = newCrv2;
                }
...
static inline HCurve GetCurveCopy(const HCurve& curve, Standard_Real& first, Standard_Real& last,
...
static inline void GetReversedParameters(const HPoint& p11, const HPoint& p12, const HPoint& p21, const HPoint& p22,
                                         Standard_Boolean& isRev1, Standard_Boolean& isRev2) {
...
Standard_Boolean ShapeConstruct::JoinCurves(const Handle(Geom_Curve) & ac3d1, const Handle(Geom_Curve) & ac3d2,
                                            const TopAbs_Orientation Orient1, const TopAbs_Orientation Orient2,
                                            Standard_Real& first1, Standard_Real& last1, Standard_Real& first2,
                                            Standard_Real& last2, Handle(Geom_Curve) & c3dOut, Standard_Boolean& isRev1,
...
Standard_Boolean ShapeConstruct::JoinCurves(const Handle(Geom2d_Curve) & aC2d1, const Handle(Geom2d_Curve) & aC2d2,
                                            const TopAbs_Orientation Orient1, const TopAbs_Orientation Orient2,
                                            Standard_Real& first1, Standard_Real& last1, Standard_Real& first2,
                                            Standard_Real& last2, Handle(Geom2d_Curve) & C2dOut,
                                            Standard_Boolean& isRev1, Standard_Boolean& isRev2,
    ...
    Handle(Geom_BSplineCurve) bspl1 = Handle(Geom_BSplineCurve)::DownCast(GeomAPI::To3d(bsplc12d, vPln));
    Handle(Geom_BSplineCurve) bspl2 = Handle(Geom_BSplineCurve)::DownCast(GeomAPI::To3d(bsplc22d, vPln));
