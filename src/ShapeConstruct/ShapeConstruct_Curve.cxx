...
// purpose  :
//=======================================================================
Standard_Boolean ShapeConstruct_Curve::AdjustCurve(const Handle(Geom_Curve) & C3D, const gp_Pnt& P1, const gp_Pnt& P2,
                                                   const Standard_Boolean take1, const Standard_Boolean take2) const {
...
Standard_Boolean ShapeConstruct_Curve::AdjustCurveSegment(const Handle(Geom_Curve) & C3D, const gp_Pnt& P1,
                                                          const gp_Pnt& P2, const Standard_Real U1,
...
Standard_Boolean ShapeConstruct_Curve::AdjustCurve2d(const Handle(Geom2d_Curve) & C2D, const gp_Pnt2d& P1,
                                                     const gp_Pnt2d& P2, const Standard_Boolean take1,
...
Handle(Geom_BSplineCurve) ShapeConstruct_Curve::ConvertToBSpline(const Handle(Geom_Curve) & C,
    ...
    try {
        ...
        if (Conv.IsDone() || Conv.HasResult()) bspl = Conv.Curve();
    } catch (Standard_Failure const& anException) {
#ifdef OCCT_DEBUG
...
Handle(Geom2d_BSplineCurve) ShapeConstruct_Curve::ConvertToBSpline(const Handle(Geom2d_Curve) & C,
    ...
    try {
        OCC_CATCH_SIGNALS
        Approx_Curve2d Conv(new Geom2dAdaptor_Curve(newc, first, last), first, last, prec, prec, GeomAbs_C1, 9, 1000);
        if (Conv.IsDone() || Conv.HasResult()) bspl = Conv.Curve();
    } catch (Standard_Failure const& anException) {
#ifdef OCCT_DEBUG
        std::cout << "Warning: ShapeConstruct_Curve::ConvertToBSpline(): Exception in Approx_Curve3d: ";
