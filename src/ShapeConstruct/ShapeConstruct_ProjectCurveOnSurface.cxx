...
static void AdjustSecondPointToFirstPoint(const gp_Pnt2d& theFirstPoint, gp_Pnt2d& theSecondPoint,
                                          const Handle(Geom_Surface) & theSurf) {
    if (theSurf->IsUPeriodic()) {
        Standard_Real UPeriod = theSurf->UPeriod();
        Standard_Real NewU =
            ElCLib::InPeriod(theSecondPoint.X(), theFirstPoint.X() - UPeriod / 2, theFirstPoint.X() + UPeriod / 2);
        theSecondPoint.SetX(NewU);
    }
    if (theSurf->IsVPeriodic()) {
        Standard_Real VPeriod = theSurf->VPeriod();
        Standard_Real NewV =
            ElCLib::InPeriod(theSecondPoint.Y(), theFirstPoint.Y() - VPeriod / 2, theFirstPoint.Y() + VPeriod / 2);
...
// purpose  :
//=======================================================================
Standard_Boolean ShapeConstruct_ProjectCurveOnSurface::Perform(Handle(Geom_Curve) & c3d, const Standard_Real First,
                                                               const Standard_Real Last, Handle(Geom2d_Curve) & c2d,
    ...
    if (!c3d->IsKind(STANDARD_TYPE(Geom_BoundedCurve))) crv3dtrim = new Geom_TrimmedCurve(c3d, First, Last);
    ...
    if (!bspl.IsNull()) {
        ...
        while (nbPini < minPnt)
            nbPini += NCONTROL - 1;
#ifdef OCCT_DEBUG
        if (nbPini > NCONTROL) std::cout << "Warning: number of points for projecting is " << nbPini << std::endl;
#endif
    }
    ...
    if (!bspl.IsNull()) {
        ...
        for (; anIdx <= bspl->NbKnots() && aFirstParam < Last; anIdx++) {
            ...
            if (Abs(aCoeff) > gp::Resolution()) aKnotCoeffs.Append(aCoeff);
    ...
    ApproxPCurve(nbrPnt, c3d, TolFirst, TolLast, points, params, pnt2d, c2d); // szv#4:S4163:12Mar99 OK not needed
...
Standard_Boolean ShapeConstruct_ProjectCurveOnSurface::PerformByProjLib(
    Handle(Geom_Curve) & c3d, const Standard_Real First, const Standard_Real Last, Handle(Geom2d_Curve) & c2d,
    const GeomAbs_Shape /*continuity*/, const Standard_Integer /*maxdeg */, const Standard_Integer /*nbinterval */) {
...
Handle(Geom2d_Curve) ShapeConstruct_ProjectCurveOnSurface::ProjectAnalytic(const Handle(Geom_Curve) & c3d) const {
    ...
    if (Plane.IsNull()) {
        Handle(Geom_RectangularTrimmedSurface) RTS = Handle(Geom_RectangularTrimmedSurface)::DownCast(surf);
        if (!RTS.IsNull())
            Plane = Handle(Geom_Plane)::DownCast(RTS->BasisSurface());
        else {
            Handle(Geom_OffsetSurface) OS = Handle(Geom_OffsetSurface)::DownCast(surf);
            if (!OS.IsNull()) Plane = Handle(Geom_Plane)::DownCast(OS->BasisSurface());
        }
    }
    if (!Plane.IsNull()) {
                                              ...
                                              Standard_Real theSavedParam)    // Param from cache to choose period
{
    ...
    if (aMaxParam - aSavedParam < Precision::PConfusion() || aSavedParam - aMinParam < Precision::PConfusion()) {
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        ...
        if (isIsoLine) {
            ...
            if (aMaxParam - aParam < Precision::PConfusion()) aParam = aMaxParam;
            if (aParam - aMinParam < Precision::PConfusion()) aParam = aMinParam;
        }
    ...
    if (!isJump) return Standard_False;
...
Handle(Geom2d_Curve) ShapeConstruct_ProjectCurveOnSurface::getLine(const TColgp_SequenceOfPnt& thepoints,
                                                                   const TColStd_SequenceOfReal& theparams,
                                                                   TColgp_SequenceOfPnt2d& thePnt2ds,
                                                                   Standard_Real theTol, Standard_Boolean& isRecompute,
    ...
    if (aTol2 < Precision::SquareConfusion()) aTol2 = Precision::SquareConfusion();
    ...
    for (; i < 4; i += 3) {
        ...
        for (j = 0; j < myNbCashe; j++) {
            if (myCashe3d[j].SquareDistance(aP[i]) < aTol2) {
                ...
                if (i == 0) isFromCashe = Standard_True;
                break;
            }
        }

        if (j >= myNbCashe) aP2d[i] = mySurf->ValueOfUV(aP[i], theTol);

        Standard_Real aDist = mySurf->Gap();
        Standard_Real aCurDist = aDist * aDist;
        if (aTol2 < aDist * aDist) aTol2 = aCurDist;
    }
    ...
    if (isPeriodicU || isPeriodicV) {
        ...
        if (isPeriodicU) {
            isRecompute = fixPeriodictyTroubles(&aP2d[0], 1 /* X Coord */, mySurf->Surface()->UPeriod(), aSavedPointNum,
                                                aSavedPoint.X());
        }

        if (isPeriodicV) {
            isRecompute = fixPeriodictyTroubles(&aP2d[0], 2 /* Y Coord */, mySurf->Surface()->VPeriod(), aSavedPointNum,
                                                aSavedPoint.Y());
        }
    }
    ...
    if (Abs(dPar) < Precision::PConfusion()) return 0;
    ...
    if (isNormalCheck) {
        for (i = 1; i <= nb; i++) {
            ...
            if (aDist > theTol) return 0;
        }
    }
    if (!isNormalCheck) {
        Standard_Real aFirstPointDist = mySurf->Surface()->Value(aP2d[0].X(), aP2d[0].Y()).SquareDistance(thepoints(1));
        ...
        for (i = 2; i < nb; i++) {
            ...
            if (Abs(aFirstPointDist - aDist1) > aTol2) return 0;
        }
    }
...
Standard_Boolean
ShapeConstruct_ProjectCurveOnSurface::ApproxPCurve(const Standard_Integer nbrPnt, const Handle(Geom_Curve) & c3d,
                                                   const Standard_Real TolFirst, const Standard_Real TolLast,
                                                   TColgp_SequenceOfPnt& points, TColStd_SequenceOfReal& params,
                                                   TColgp_SequenceOfPnt2d& pnt2d, Handle(Geom2d_Curve) & c2d) {
    ...
    isoParam = IsAnIsoparametric(nbrPnt, points, params, isoTypeU, p1OnIso, valueP1, p2OnIso, valueP2, isoPar2d3d, cIso,
                                 t1, t2, pout);
    ...
    if (myNbCashe > 0 && myCashe3d[0].Distance(points(1)) > myCashe3d[0].Distance(points(nbrPnt)))
        ...
        if (myCashe3d[0].Distance(points(nbrPnt)) < Precision::Confusion()) ChangeCycle = Standard_True;
        ...
        if (isoParam) {
            ...
            if (isoPar2d3d) {
                ...
                if (aPntIndex == 1)
                    ...
                    tPar = pout(aPntIndex);
                    //: S4030  ShapeAnalysis_Curve().Project (cIso,p3d,myPreci,pt,tPar,Cf,Cl); //szv#4:S4163:12Mar99
                    //: `dist=` not needed
                }
            }
    ...
    for (Standard_Integer i = 1;; i++) {
        ...
        if (!mySurf->Singularity(i, aPreci, aP3d, aFirstP2d, aLastP2d, aFirstPar, aLastPar, IsUiso)) break;
    ...
    if (mySurf->IsUClosed(myPreci) || needResolveUJump) { // #78 rln 12.03.99 S4135
        ...
        for (Standard_Integer aPntIter = 2; aPntIter <= pnt2d.Length(); ++aPntIter) {
            ...
            if (dist2d > (Up / 2)) {
                InsertAdditionalPointOrAdjust(ToAdjust, 1, Up, TolOnUPeriod, CurX, prevX, c3d, aPntIter, points, params,
                                              pnt2d);
            }
            prevX = CurX;
            if (minX > CurX)
    ...
    if (mySurf->IsVClosed(myPreci) || needResolveVJump ||
        ...
        for (Standard_Integer aPntIter = 2; aPntIter <= pnt2d.Length(); ++aPntIter) {
            ...
            if (dist2d > (Vp / 2)) {
                InsertAdditionalPointOrAdjust(ToAdjust, 2, Vp, TolOnVPeriod, CurY, prevY, c3d, aPntIter, points, params,
                                              pnt2d);
            }
            prevY = CurY;
            if (minY > CurY)
        ...
        if (mySurf->IsUClosed(myPreci)) {                    // #78 rln 12.03.99 S4135
            ...
            if (mySurf->NbSingularities(myPreci) > 0) {      // rln S4135
                ...
                for (ind = 1; ind <= pnt2d.Length(); ind++) {
                    ...
                    if (mySurf->IsDegenerated(points(ind), Precision::Confusion())) continue;
                    OnBound = (Abs(Abs(CurX - 0.5 * (ul + uf)) - Up / 2) <= Precision::PConfusion());
                    if (!start && Abs(Abs(CurX - PrevX) - Up / 2) <= 0.01 * Up) break;
            ...
            if (mySurf->NbSingularities(myPreci) > 0) {      // rln S4135
                ...
                for (ind = 1; ind <= pnt2d.Length(); ind++) {
                    ...
                    if (!start && Abs(Abs(CurY - PrevY) - Vp / 2) <= 0.01 * Vp) break;
                    start = Standard_False;
                    PrevY = CurY;
                    PrevOnBound = OnBound;
                }
...
Handle(Geom2d_Curve) ShapeConstruct_ProjectCurveOnSurface::ApproximatePCurve(
    const Standard_Integer /*nbrPnt*/, Handle(TColgp_HArray1OfPnt2d) & points2d, Handle(TColStd_HArray1OfReal) & params,
    ...
    try {
        ...
        anException.Print(std::cout);
        std::cout << "Pb Geom2dAPI_Approximate, tol2d=" << theTolerance2d << " NbParams=" << nbp << " NbPnts=" << nb2
                  << std::endl;
//     if (nb2 > nbp) nb2 = nbp;
...
Handle(Geom2d_Curve) ShapeConstruct_ProjectCurveOnSurface::InterpolatePCurve(
    ...
    try {
        ...
        anException.Print(std::cout);
        std::cout << "Pb Geom2dAPI_Interpolate, tol2d=" << theTolerance2d << " NbParams=" << nbp << " NbPnts=" << nb2
                  << std::endl;
//     if (nb2 > nbp) nb2 = nbp;
//     Standard_Real rbp,rb2; rbp = nbp; rb2 = nb2;
// //    dbl.AddString ("NbP2d/NbParams puis  X Y Param -> mini");
...
void ShapeConstruct_ProjectCurveOnSurface::CorrectExtremity(
    const Handle(Geom_Curve) & theC3d, const TColStd_SequenceOfReal& theParams, TColgp_SequenceOfPnt2d& thePnt2d,
    const Standard_Boolean theIsFirstPoint, const gp_Pnt2d& thePointOnIsoLine, const Standard_Boolean theIsUiso) {
    ...
    Standard_Boolean IsPeriodic = (theIsUiso) ? mySurf->Surface()->IsVPeriodic() : mySurf->Surface()->IsUPeriodic();
    ...
    for (;;) {
        ...
        if (Abs(SecondParam - FirstParam) <= 2 * Precision::PConfusion()) break;
        gp_Pnt aP3d;
        theC3d->D0(SecondParam, aP3d);
        SecondPointOfLine = mySurf->NextValueOfUV(FirstPointOfLine, aP3d, myPreci, Precision::Confusion());
        if (IsPeriodic) AdjustSecondPointToFirstPoint(FirstPointOfLine, SecondPointOfLine, mySurf->Surface());
...
void ShapeConstruct_ProjectCurveOnSurface::InsertAdditionalPointOrAdjust(
    Standard_Boolean& ToAdjust, const Standard_Integer theIndCoord, const Standard_Real Period,
    const Standard_Real TolOnPeriod, Standard_Real& CurCoord, const Standard_Real prevCoord,
    const Handle(Geom_Curve) & c3d, Standard_Integer& theIndex, TColgp_SequenceOfPnt& points,
    TColStd_SequenceOfReal& params, TColgp_SequenceOfPnt2d& pnt2d) {
    Standard_Real CorrectedCurCoord = ElCLib::InPeriod(CurCoord, prevCoord - Period / 2, prevCoord + Period / 2);
    if (!ToAdjust) {
...
void ShapeConstruct_ProjectCurveOnSurface::CheckPoints(Handle(TColgp_HArray1OfPnt) & points,
    ...
    for (i = firstElem + 1; i <= lastElem; i++) {
        ...
        if (CurDist2 < gp::Resolution()) { // test 0
            ...
            if (CurDist2 < DistMin2) DistMin2 = CurDist2;
...
void ShapeConstruct_ProjectCurveOnSurface::CheckPoints2d(Handle(TColgp_HArray1OfPnt2d) & points,
    ...
    if (DistMin2 < RealLast()) preci = 0.9 * Sqrt(DistMin2);
    if (nbPntDropped == 0) return;
    ...
    for (i = firstElem; i <= lastElem; i++) {
        if (tmpParam.Value(i) == 1) {
#ifdef OCCT_DEBUG
            std::cout << "Point " << i << " : " << points->Value(i).X() << " " << points->Value(i).Y() << " at param "
                      << params->Value(i) << std::endl;
#endif
            newPnts->SetValue(newCurr, points->Value(i));
            newParams->SetValue(newCurr, params->Value(i));
            newCurr++;
...
#ifdef OCCT_DEBUG
            std::cout << "Removed " << i << " : " << points->Value(i).X() << " " << points->Value(i).Y() << " at param "
                      << params->Value(i) << std::endl;
#endif
        }
    }
...
Standard_Boolean ShapeConstruct_ProjectCurveOnSurface::IsAnIsoparametric(
    const Standard_Integer nbrPnt, const TColgp_SequenceOfPnt& points, const TColStd_SequenceOfReal& params,
    Standard_Boolean& isoTypeU, Standard_Boolean& p1OnIso, gp_Pnt2d& valueP1, Standard_Boolean& p2OnIso,
    gp_Pnt2d& valueP2, Standard_Boolean& isoPar2d3d, Handle(Geom_Curve) & cIso, Standard_Real& t1, Standard_Real& t2,
    ...
    try { // RAJOUT
        ...
        for (Standard_Integer j = 1; (j <= 4) /*&& !isoParam*/; j++) {
            ...
            if (mp[0] > 0 && mp[1] > 0 && Abs(tp[0] - tp[1]) < Precision::PConfusion()) continue;

            if (mp[0] > 0 && (!p1OnIso || currd2[0] < mind2[0])) {
                p1OnIso = Standard_True;
                mind2[0] = currd2[0]; // LP2.stp #105899: FLT_INVALID_OPERATION on Windows 7 VC 9 Release mode on the
                                      // whole file
                if (isoU)
            ...
            if (isoPar2d3d)
                ...
                for (Standard_Integer i = 2; i < nbrPnt && isoByDistance; i++) {
                    Standard_Real dist = sac.NextProject(
                        prevParam, cIso, points(i), prec, pt, t, Cf, Cl,
                        Standard_False); //: j8 abv 10.12.98: TR10 r0501_db.stp #9423: avoid adjusting to ends
                    prevParam = t;
                    pout(i) = t;
                    if ((dist > prec) || (t < Cf) || (t > Cl)) isoByDistance = Standard_False;
                }
...
//=======================================================================
 ...
 gp_Pnt2d ShapeConstruct_ProjectCurveOnSurface::BestExtremum(const gp_Pnt2d& P2iso,const gp_Pnt& P3ext,const gp_Pnt&
P3next) const
{
