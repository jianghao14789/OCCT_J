...
Standard_Integer Contap_HContTool::NbSamplesV(const Handle(Adaptor3d_Surface) & S, const Standard_Real,
                                              const Standard_Real) {
...
Standard_Integer Contap_HContTool::NbSamplesU(const Handle(Adaptor3d_Surface) & S, const Standard_Real,
                                              const Standard_Real) {
    Standard_Integer nbs;
    GeomAbs_SurfaceType typS = S->GetType();
    switch (typS) {
...
void Contap_HContTool::SamplePoint(const Handle(Adaptor3d_Surface) & S, const Standard_Integer Index, Standard_Real& U,
                                   Standard_Real& V) {
    if (S->GetType() == GeomAbs_BSplineSurface) {
...
void Contap_HContTool::Bounds(const Handle(Adaptor2d_Curve2d) & A, Standard_Real& Ufirst, Standard_Real& Ulast) {
    Ufirst = A->FirstParameter();
    Ulast = A->LastParameter();
}

Standard_Boolean Contap_HContTool::Project(const Handle(Adaptor2d_Curve2d) & C, const gp_Pnt2d& P,
...
Standard_Real Contap_HContTool::Tolerance(const Handle(Adaptor3d_HVertex) & V, const Handle(Adaptor2d_Curve2d) & C) {
    //  return BRepAdaptor2d_Curve2dTool::Resolution(C,BRep_Tool::Tolerance(V));
    return V->Resolution(C);
}

Standard_Real Contap_HContTool::Parameter(const Handle(Adaptor3d_HVertex) & V, const Handle(Adaptor2d_Curve2d) & C) {
...
void Contap_HContTool::Value(const Handle(Adaptor2d_Curve2d) &, const Standard_Integer, gp_Pnt&, Standard_Real&,
                             Standard_Real&) {
    throw Standard_OutOfRange();
}

Standard_Boolean Contap_HContTool::IsVertex(const Handle(Adaptor2d_Curve2d) &, const Standard_Integer) {
    return Standard_False;
}

void Contap_HContTool::Vertex(const Handle(Adaptor2d_Curve2d) &, const Standard_Integer, Handle(Adaptor3d_HVertex) &) {
    throw Standard_OutOfRange();
}
...
Standard_Boolean Contap_HContTool::HasFirstPoint(const Handle(Adaptor2d_Curve2d) &, const Standard_Integer,
                                                 Standard_Integer&) {
    throw Standard_OutOfRange();
}

Standard_Boolean Contap_HContTool::HasLastPoint(const Handle(Adaptor2d_Curve2d) &, const Standard_Integer,
                                                Standard_Integer&) {
    throw Standard_OutOfRange();
}
