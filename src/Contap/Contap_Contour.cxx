...
Contap_Contour::Contap_Contour() : done(Standard_False), modeset(Standard_False) {}
...
Contap_Contour::Contap_Contour(const Handle(Adaptor3d_Surface) & Surf, const Handle(Adaptor3d_TopolTool) & Domain,
...
Contap_Contour::Contap_Contour(const Handle(Adaptor3d_Surface) & Surf, const Handle(Adaptor3d_TopolTool) & Domain,
                               const gp_Vec& Direction, const Standard_Real Angle)
    :
...
void Contap_Contour::Perform(const Handle(Adaptor3d_Surface) & Surf, const Handle(Adaptor3d_TopolTool) & Domain) {
    if (!modeset) {
...
static IntSurf_TypeTrans ComputeTransitionOnLine(Contap_SurfFunction&, const Standard_Real, const Standard_Real,
                                                 const gp_Vec&);
...
static void ComputeInternalPoints(Contap_Line& Line, Contap_SurfFunction&, const Standard_Real ureso,
                                  const Standard_Real vreso);

static void ComputeInternalPointsOnRstr(Contap_Line&, const Standard_Real, const Standard_Real, Contap_SurfFunction&);

static void ProcessSegments(const Contap_TheSearch&, Contap_TheSequenceOfLine&, const Standard_Real,
                            Contap_SurfFunction&, const Handle(Adaptor3d_TopolTool) &);
...
static void LineConstructor(Contap_TheSequenceOfLine& slin, const Handle(Adaptor3d_TopolTool) & Domain, Contap_Line& L,
            ...
            if (firstp != lastp) {
                ...
                if (in2 == TopAbs_OUT) {
                    ...
                    for (Standard_Integer j = firstp + 1; j <= lastp; j++) {
                        ...
                        if (aSqDist > gp::Resolution()) LineOn2S->Add(L.Point(j));
                    }
                    if (LineOn2S->NbPoints() < 2) continue;
                    Contap_Line Line;
...
static void KeepInsidePoints(const Contap_TheSearchInside& solins, const Contap_TheSearch& solrst,
                             Contap_SurfFunction& Func, IntSurf_SequenceOfInteriorPoint& seqpins)
...
static void ComputeTangency(const Contap_TheSearch& solrst, const Handle(Adaptor3d_TopolTool) & Domain,
                            Contap_SurfFunction& Func, IntSurf_SequenceOfPathPoint& seqpdep,
    ...
    for (i = 1; i <= NbPoints; i++) {
        ...
        if (Destination(i) == 0) {
            ...
            for (Standard_Integer restriction = 1;
                 SurUneRestrictionSolution == Standard_False && restriction <= solrst.NbSegments(); restriction++) {
...
IntSurf_TypeTrans ComputeTransitionOnLine(Contap_SurfFunction& SFunc, const Standard_Real u, const Standard_Real v,
                                          const gp_Vec& tgline) {
    gp_Vec d1u, d1v;
    gp_Pnt pntbid;
    // gp_Vec tglineuv;
...
void ProcessSegments(const Contap_TheSearch& solrst, Contap_TheSequenceOfLine& slin, const Standard_Real TolArc,
                     Contap_SurfFunction& SFunc, const Handle(Adaptor3d_TopolTool) & Domain)
...
void ComputeInternalPointsOnRstr(Contap_Line& Line, const Standard_Real Paramf, const Standard_Real Paraml,
...
void ComputeInternalPoints(Contap_Line& Line, Contap_SurfFunction& SFunc, const Standard_Real ureso,
                           const Standard_Real vreso)
                ...
                for (i = NbPointRst; i >= 1; i--) {
                    if (Destination(i) == indlast) {
                        ...
                        if (!iwline->IsTangentAtEnd()) {
                            ...
                            tgtrst = d2d.X() * d1u;
                            tgtrst.Add(d2d.Y() * d1v);
                            IntSurf::MakeTransition(PPoint.Direction3d().Reversed(), tgtrst, normale, TLine, TArc);
                        } else {
                            TLine.SetValue();
                            TArc.SetValue();
                        }
                            ...
                            if (slin(k).TypeContour() == Contap_Restriction) {
                                ...
                                if (projok) {
                                    ...
                                    if (dist <= Preci) {
                                        ...
                                        Contap_SurfProps::DerivAndNorm(Surf, Ptproj.X(), Ptproj.Y(), ptonsurf, d1u, d1v,
                                                                       normale);
...
static Standard_Boolean FindLine(Contap_Line& Line, const Handle(Adaptor3d_Surface) & Surf, const gp_Pnt2d& Pt2d,
                                 gp_Pnt& Ptref, Standard_Real& Paramin, gp_Vec& Tgmin, gp_Vec& Norm) {
    //  Standard_Integer i;
    gp_Pnt pt, ptmin;
    gp_Vec tg;
...
static void PutPointsOnLine(const Contap_TheSearch& solrst, const Handle(Adaptor3d_Surface) & Surf,
    ...
    switch (typS) {
        ...
        case GeomAbs_Sphere: {
            switch (TypeFunc) {
                ...
                case Contap_DraftStd: {
                    contana.Perform(Adaptor3d_HSurfaceTool::Sphere(Surf), mySFunc.Direction(), mySFunc.Angle());
                } break;
                case Contap_DraftPrs:
                default: {
                }
        ...
        case GeomAbs_Cylinder: {
            switch (TypeFunc) {
                ...
                case Contap_DraftStd: {
                    contana.Perform(Adaptor3d_HSurfaceTool::Cylinder(Surf), mySFunc.Direction(), mySFunc.Angle());
                } break;
                case Contap_DraftPrs:
                default: {
                }
