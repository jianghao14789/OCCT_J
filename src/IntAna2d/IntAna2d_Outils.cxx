...
MyDirectPolynomialRoots::MyDirectPolynomialRoots(const Standard_Real A4, const Standard_Real A3, const Standard_Real A2,
                                                 const Standard_Real A1, const Standard_Real A0) {
    ...
    if (MATH_A43210.IsDone()) {
        ...
        if (nbp & 1) PbPossible = Standard_True;
    } else {
        PbPossible = Standard_True;
    }
...
MyDirectPolynomialRoots::MyDirectPolynomialRoots(const Standard_Real A2, const Standard_Real A1,
...
Standard_Boolean Points_Confondus(const Standard_Real x1, const Standard_Real y1, const Standard_Real x2,
...
void Traitement_Points_Confondus(Standard_Integer& nb_pts, IntAna2d_IntPoint* pts) {
    ...
    for (i = nb_pts; i > 1; i--) {
        ...
        for (j = i - 1; (j > 0) && Non_Egalite; j--) {
            ...
            if (Points_Confondus(pts[i - 1].Value().X(), pts[i - 1].Value().Y(), pts[j - 1].Value().X(),
                                 pts[j - 1].Value().Y())) {
                Standard_Integer k;
