...
Standard_EXPORT void debfillonfemess(const Standard_Integer iFOR, const Standard_Integer iEG,
                                     const TopOpeBRepBuild_PBuilder& PB, const TopOpeBRepBuild_PWireEdgeSet& PWES,
                                     const TCollection_AsciiString& str) {
    PB->GdumpSHASTA(iEG, TopAbs_ON, *PWES, str);
    debfillonfemess(iFOR, iEG);
}
Standard_EXPORT void debaddpwes(const Standard_Integer iFOR, const TopAbs_State TB1, const Standard_Integer iEG,
...
Standard_EXPORT Standard_Boolean DEBTEFOR(const TopOpeBRepBuild_Builder& B, const Standard_Integer iFOR,
...
// purpose  :
//=======================================================================
TopOpeBRepBuild_BuilderON::TopOpeBRepBuild_BuilderON(const TopOpeBRepBuild_PBuilder& PB, const TopoDS_Shape& FOR,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_BuilderON::Perform(const TopOpeBRepBuild_PBuilder& PB, const TopoDS_Shape& FOR,
                                        const TopOpeBRepBuild_PGTopo& PG, const TopOpeBRepTool_Plos& PLSclass,
                                        const TopOpeBRepBuild_PWireEdgeSet& PWES) {
    myPB = PB;
    myFace = FOR;
    myPG = PG;
...
#endif
                             const TopOpeBRepDS_Transition& TFE, const TopAbs_State TB1, const TopAbs_State)
//------------------------------------------------------
{
    ...
    if (tFFEG) debfillonfemess(iFF, iEG);
    Standard_Boolean tFSEG = DEBTEFOR(B, iFS, iEG);
    if (tFSEG) debfillonfemess(iFS, iEG);
#endif
...
static Standard_Integer FUN_findeSD(const TopOpeBRepDS_DataStructure& BDS, const TopoDS_Edge& EspON,
                                    const TopoDS_Edge& EG, const TopoDS_Face& FOR, TopAbs_Orientation& oeSD,
...
static Standard_Boolean ComputeFaceCrvtInSec(const TopoDS_Face& aFace, const gp_Pnt2d& aP2d, const gp_Dir& aSecDir,
                                             Standard_Real& aCrvt) {
...
void TopOpeBRepBuild_BuilderON::GFillONPartsWES2(const Handle(TopOpeBRepDS_Interference) & I,
    ...
    Standard_Boolean isclosedFF =
        FUN_tool_IsClosingE(EG, FOR, FOR);                         // xpu240898 : cto900J5 faulty yapc2(FOR17,FS18,EG15)
...
//  Standard_Boolean opec21 = myPB->Opec21();
    ...
    if (yap0) {
        ...
        gp_Dir ntFOR(ngFOR);
        if (M_REVERSED(oFOR)) ntFOR.Reverse();
    ...
    if (yapc2) {
        ...
        if (tEFOR) {
            if (!isclosedFF)
                ...
                debaddpwes(iFOR, TB1, iEG, TopAbs_REVERSED, myPB, myPWES, "yapc2 GFillON closed",
                           "WES+ EspON INTERNAL");
            }
        }
        if (tE) {
        ...
        if (!addFORREV) {
            ...
            if (TopOpeBRepBuild_GetcontextNOSG())
                ...
                reverse = Standard_False; // we exclude this line from #ifdef OCCT_DEBUG because
                                          // in optimised mode this line will never be included , and that follows to
                                          // regressions MZV-12-05-2000
#endif
    ...
    if (yap1) {
        ...
        for (; itssi.More(); itssi.Next()) {
            Handle(TopOpeBRepDS_ShapeShapeInterference)
                ssi(Handle(TopOpeBRepDS_ShapeShapeInterference)::DownCast(itssi.Value()));
...
//    Standard_Integer rkToFill = BDS.AncestorRank(myFace);
        ...
        Standard_Boolean ok = FUN_ds_shareG(myPB->DataStructure(), iFOR, iFCX, GI, TopoDS::Edge(EspON), shareG);
