...
#include <stdio.h>
static void BuildPath(const TopoDS_Vertex& myVertex0, const TopoDS_Edge& myEdge, const TopoDS_Vertex& myVertex,
                      const Standard_Integer aNbEdges, TopOpeBRepBuild_IndexedDataMapOfShapeVertexInfo& M,
                      Standard_Integer anEdgesCount, Standard_Integer& aBreakFlag, TopTools_ListOfShape& myResList);
...
// purpose:
//=======================================================================
void BuildPath(const TopoDS_Vertex& myVertex0, const TopoDS_Edge& myInputEdge, const TopoDS_Vertex& myInputVertex,
               const Standard_Integer aNbEdges, TopOpeBRepBuild_IndexedDataMapOfShapeVertexInfo& M,
               Standard_Integer anEdgesCount, Standard_Integer& aBreakFlag, TopTools_ListOfShape& myResList) {
    ...
    for (j = 1; j <= aNbCases; j++) {
        ...
        if (!aFoundOut) { // FondOut=0 TUPICK
            ...
            anEdgesCount++;
            myVertex =
                (myEdge.Orientation() == TopAbs_FORWARD) ? TopExp::LastVertex(myEdge) : TopExp::FirstVertex(myEdge);
...
void TopOpeBRepBuild_Tools2d::MakeMapOfShapeVertexInfo(const TopoDS_Wire& aWire,
    ...
    for (; exa.More(); exa.Next()) {
        ...
        for (; exs.More(); exs.Next()) {
            ...
            if (!index) index = M.Add(aVertex, empty);
