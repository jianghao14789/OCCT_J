...
//-----------------------------------------------------------------------
static Standard_Boolean FUN_PinC(const gp_Pnt& P, const Handle(Geom_Curve) & C, const Standard_Real pmin,
...
static void FUN_selLEE(TopTools_ListOfShape& LE, const TopoDS_Edge& E, const TopAbs_State sta,
    ...
    for (i = 1; i <= n; i++) { // 2
        ...
        for (TopTools_ListIteratorOfListOfShape itLESD(LESD); itLESD.More(); itLESD.Next()) { // 2.1
            ...
            if (tSPS) {
                ...
                std::cout << std::endl;
                if (DEBiESD == 1) std::cout << stru << std::endl;
                DEBiESD++;
        ...
        for (; it.More(); it.Next()) {
            ...
            for (; itlon.More(); itlon.Next()) {
                ...
                if (TopOpeBRepTool_GettraceC2D()) {
                    std::cout << "\n#TopOpeBRepBuild_Builder::SSE : hasPC = 0 ES" << i << " E" << idebE << " sur F"
                              << iF << std::endl;
                ...
                Standard_Real tolpc;
                MGhc2 PC = FC2D_CurveOnSurface(eon, F, esdF, f, l, tolpc, Standard_True); // xpu051198 :PRO15049
                hasPC = (!PC.IsNull());
...
//    Standard_Integer non = LON.Extent(); // DEB
        ...
        for (; exv.More(); exv.Next()) {
            ...
            if (iv != 0) hsd = myDataStructure->HasSameDomain(v);
            if (hsd) ivref = myDataStructure->SameDomainReference(v);
...
void TopOpeBRepBuild_Builder::FillSecEdgeAncestorMap(const Standard_Integer aShapeRank,
    ...
    for (i = 1; i <= nse; i++) {
        const TopoDS_Shape& es = BDS.SectionEdge(i);
        if (es.IsNull() || ShapeRank(es) != aShapeRank) continue;
        ...
        for (Standard_Integer j = 0; j < 3; j++) {
            ...
            if (IsSplit(es, states[j])) {
                ...
                for (; it.More(); it.Next()) {
                    ...
                    if (aMapON.Contains(aS)) anAncMap.Bind(aS, es);
                }
            }
        }
    }
