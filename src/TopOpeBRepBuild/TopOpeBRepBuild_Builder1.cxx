...
// purpose  :
//=======================================================================
TopOpeBRepBuild_Builder1::TopOpeBRepBuild_Builder1(const TopOpeBRepDS_BuildTool& BT) : TopOpeBRepBuild_Builder(BT) {
    mySameDomMap.Clear();
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::Perform(const Handle(TopOpeBRepDS_HDataStructure) & HDS, const TopoDS_Shape& S1,
    ...
    if ((myIsKPart == 1) || (myIsKPart == 5)) myIsKPart = 4;
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillSolidSFS(const TopoDS_Shape& SO1, const TopTools_ListOfShape& LSO2,
                                             const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    // modified by NIZHNY-MZV  Wed Apr 19 17:23:12 2000
    // see the comments at the top of file about this global variable
    if (!GLOBAL_USE_NEW_BUILDER) {
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillShellSFS(const TopoDS_Shape& SH, const TopTools_ListOfShape& LSO2,
                                             const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    // modified by NIZHNY-MZV  Wed Apr 19 17:23:12 2000
    // see the comments at the top of file about this global variable
    if (!GLOBAL_USE_NEW_BUILDER) {
    ...
    for (exFace.Init(SH1, TopAbs_FACE); exFace.More(); exFace.Next()) {
        ...
        if (hsd && !mySameDomMap.Contains(FOR)) GFillFaceSameDomSFS(FOR, LSO2, G1, SFS);
    }

    // 2 Process all other faces
    for (exFace.Init(SH1, TopAbs_FACE); exFace.More(); exFace.Next()) {
        TopoDS_Shape FOR = exFace.Current();
        if (!myDataStructure->HasShape(FOR) || myDataStructure->HasSameDomain(FOR)) continue;
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillFaceNotSameDomSFS(const TopoDS_Shape& FOR, const TopTools_ListOfShape& LSO2,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillFaceNotSameDomWES(const TopoDS_Shape& FOR1, const TopTools_ListOfShape& LFclass,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillWireNotSameDomWES(const TopoDS_Shape& W, const TopTools_ListOfShape& LSclass,
                                                      const TopOpeBRepBuild_GTopo& G1,
                                                      TopOpeBRepBuild_WireEdgeSet& WES) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillEdgeNotSameDomWES(const TopoDS_Shape& EOR, const TopTools_ListOfShape& /*LSclass*/,
                                                      const TopOpeBRepBuild_GTopo& G1,
                                                      TopOpeBRepBuild_WireEdgeSet& WES) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillFaceSameDomSFS(const TopoDS_Shape& FOR, const TopTools_ListOfShape& LSO2,
                                                   const TopOpeBRepBuild_GTopo& Gin,
                                                   TopOpeBRepBuild_ShellFaceSet& SFS) {
    myProcessedPartsOut2d.Clear();
    myProcessedPartsON2d.Clear();
    ...
    if (myDataStructure->DS().AncestorRank(FOR) != 1) return;
    ...
    for (; LOFit.More(); LOFit.Next()) {
        ...
        if (OrigRev) aFace.Reverse();

        if (rev) aFace.Reverse();
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillFaceSameDomWES(const TopoDS_Shape& FOR1, const TopTools_ListOfShape& /*LFclass*/,
                                                   const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_WireEdgeSet& WES) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
    ...
    for (i = 1; i <= nF; i++) {
        ...
        if (iref == 1) { // object
            ...
            TB = TB2;
            if (RevOri) GFTW = G1.CopyPermuted();
        }
            ...
            if (!hasshape && (shSt != TopAbs_ON)) {
                ...
                if (keep) {
                    ...
                    if (myBaseFaceToFill != mySDFaceToFill)
                        TopOpeBRepBuild_Tools::UpdatePCurves(TopoDS::Wire(W), TopoDS::Face(mySDFaceToFill),
                    ...
                    for (; we.More(); we.Next()) {
                        ...
                        OrientateEdgeOnFace(EOR, TopoDS::Face(myBaseFaceToFill), TopoDS::Face(mySDFaceToFill), G1,
                                            stateOfFaceOri);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillWireSameDomWES(const TopoDS_Shape& W, const TopTools_ListOfShape& LSclass,
                                                   const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_WireEdgeSet& WES) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
        ...
        if (!hasshape) {
            ...
            if (keep) {
                ...
                if (mySDFaceToFill != myBaseFaceToFill) {
                    TopOpeBRepBuild_Tools::UpdateEdgeOnFace(TopoDS::Edge(EOR), TopoDS::Face(mySDFaceToFill),
                                                            TopoDS::Face(myBaseFaceToFill));
                } else {
                    mySourceShapes.Add(EOR);
                }
                ...
                OrientateEdgeOnFace(TopoDS::Edge(EOR), TopoDS::Face(myBaseFaceToFill), TopoDS::Face(mySDFaceToFill), G1,
                                    stateOfFaceOri);
                myMapOfEdgeWithFaceState.Bind(EOR, stateOfFaceOri);
                WES.AddElement(EOR);
            }
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GFillEdgeSameDomWES(const TopoDS_Shape& EOR, const TopTools_ListOfShape& LSclass,
                                                   const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_WireEdgeSet& WES) {
    ...
    for (; it.More(); it.Next()) {
        ...
        if (mySDFaceToFill != myBaseFaceToFill) {
            TopOpeBRepBuild_Tools::UpdateEdgeOnFace(newE, TopoDS::Face(mySDFaceToFill), TopoDS::Face(myBaseFaceToFill));
        } else {
            mySourceShapes.Add(newE);
        }

        Standard_Boolean stateOfFaceOri = Standard_False;
        OrientateEdgeOnFace(newE, TopoDS::Face(myBaseFaceToFill), TopoDS::Face(mySDFaceToFill), G1, stateOfFaceOri);
    ...
    for (; it.More(); it.Next()) {
        ...
        if (myDataStructure->HasSameDomain(aSplitPart) || myDataStructure->HasSameDomain(EOR)) {
            ...
            if (myProcessedPartsOut2d.Contains(eON)) continue;
        ...
        } else { // if face has more than one same domain faces we need to detect for each part complement same domain
                 // face
        ...
        if (aState == TopAbs_ON || aState == TopAbs_UNKNOWN) continue;
        ...
        if (aState == TopAbs_OUT || aSDShape.IsNull()) {
            ...
            if (!aAdjSDFace.IsNull()) {
                ...
                if (aSDToAdjFace.Orientation() == TopAbs_REVERSED) aN2.Reverse();

                TopOpeBRepBuild_Tools::GetNormalToFaceOnEdge(TopoDS::Face(aAdjSDFace), aSplitP, aN3);
                if (aAdjSDFace.Orientation() == TopAbs_REVERSED) aN3.Reverse();
                ...
                if (mySDFaceToFill.Orientation() == TopAbs_REVERSED) aN1.Reverse();
                scalarPr = aBiN * aN1;
            ...
            if (keep) {
                if (mySDFaceToFill != myBaseFaceToFill) {
                    TopOpeBRepBuild_Tools::UpdateEdgeOnFace(aSplitPart, TopoDS::Face(mySDFaceToFill),
                                                            TopoDS::Face(myBaseFaceToFill));
                } else {
                ...
                Standard_Boolean stateOfFaceOri = Standard_False;
                OrientateEdgeOnFace(aSplitPart, TopoDS::Face(myBaseFaceToFill), TopoDS::Face(mySDFaceToFill), G1,
                                    stateOfFaceOri);
        ...
        Standard_Boolean keep = Standard_False;
        PerformPieceIn2D(aSplitPart, TopoDS::Edge(EOR), TopoDS::Face(mySDFaceToFill), aSDFace, G1, keep);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::PerformONParts(const TopoDS_Shape& FOR1, const TopTools_IndexedMapOfShape& /*SDFaces*/,
                                              const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_WireEdgeSet& WES) {
    ...
    for (TopOpeBRepDS_ListIteratorOfListOfInterference ILI(LI); ILI.More(); ILI.Next()) {
        ...
        for (TopTools_ListIteratorOfListOfShape itON(splON); itON.More(); itON.Next()) {
            ...
            if (!(aState == TopAbs_IN || aState == TopAbs_ON)) continue;
            ...
            for (TopTools_ListIteratorOfListOfShape it(EdgeFaces); it.More(); it.Next()) {
                ...
                for (; Exp.More(); Exp.Next()) {
                    EOR = Exp.Current();
                    if (EG.IsSame(EOR)) break;
                }

                if (EOR.IsNull()) continue;
                ...
                if (FOR.Orientation() == TopAbs_REVERSED) aN2.Reverse();
                ...
                if (aSDFace.Orientation() == TopAbs_REVERSED) aN3.Reverse();
                keep = Standard_False;
                aBiN = aTg ^ aN2;
                Standard_Real scalarPr = 0.;
                scalarPr = aBiN * aN3;
                ...
                if (fabs(scalarPr) <= 1e-10) { // try to step inside
                    ...
                    aBiN = aTg ^ aN2;
                    scalarPr = aBiN * aN3;
                    if (fabs(scalarPr) <= 1e-10) continue;
                }
                TopAbs_State aPartState = (scalarPr > 0) ? TopAbs_IN : TopAbs_OUT;
            ...
            if (keep) {
                ...
                if (aSDFace.Orientation() == TopAbs_REVERSED) aNsf.Reverse();
                TopOpeBRepBuild_Tools::GetNormalToFaceOnEdge(TopoDS::Face(myBaseFaceToFill), TopoDS::Edge(newE),
                                                             OrigNormalbf);
                aNbf = OrigNormalbf;
                if (myBaseFaceToFill.Orientation() == TopAbs_REVERSED) aNbf.Reverse();
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::GWESMakeFaces(const TopoDS_Shape& FF, TopOpeBRepBuild_WireEdgeSet& WES,
                                             TopTools_ListOfShape& LOF) {
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::PerformPieceIn2D(const TopoDS_Edge& EdgeToPerform, const TopoDS_Edge& EOR,
                                                const TopoDS_Face& edgeFace, const TopoDS_Face& toFace,
                                                const TopOpeBRepBuild_GTopo& G1, Standard_Boolean& keep) {
    keep = Standard_False;
    ...
    if (aFEL.Extent() <= 2) { // we don't compute adjacent if we have more than one adjacent face
        for (; aEFIt.More(); aEFIt.Next()) {
            if (edgeFace.IsSame(aEFIt.Value()))
                ...
                if (aAdjF.Orientation() == TopAbs_REVERSED) aN1.Reverse();
                scalarPr = aBiN * aN1;
    ...
    if (fabs(scalarPr) <= 1e-10) {
        ...
        if (Opec12() || Opec21()) keep = aN3 * aN2 < 0;
        if (Opecom()) keep = aN3 * aN2 > 0;
    }
}
...
// purpose  :
//=======================================================================
Standard_Integer TopOpeBRepBuild_Builder1::PerformPieceOn2D(const TopoDS_Shape& aPieceObj, const TopoDS_Shape& aFaceObj,
    ...
    if (!myDataStructure->HasSameDomain(aFaceObj)) return -1;
    // Main DataStructure
            ...
            for (j = 1; anItE.More(); anItE.Next(), j++) {
                ...
                if (anEdgesToolMap.Contains(anEdgeTool)) {
                    ...
                    for (anExpEdges.Init(aFaceTool, TopAbs_EDGE); anExpEdges.More(); anExpEdges.Next()) {
                        ...
                        const TopOpeBRepDS_ShapeWithState& aSWSTool = aMapOfShapeWithStateTool.FindFromKey(anEdgeTool);
                        ...
                        for (k = 1; anItTool.More(); anItTool.Next(), k++) {
                            ...
                            aIsSameCnd = IsDegFlag ? TopOpeBRepBuild_Tools::IsDegEdgesTheSame(aPieceObj, aPieceTool)
    ...
    if (flag > 1) {
        if (aCasesMap.Contains(14) && aCasesMap.Contains(12) && Opefus()) aListOfPieces.Clear();
        // eap 30 May occ417, add :
        if (aCasesMap.Contains(11) && aCasesMap.Contains(13) && (Opec12() || Opec21())) aListOfPieces.Clear();
    }
    return flag; // Ok
}
...
Standard_Integer TopOpeBRepBuild_Builder1::TwoPiecesON(const TopTools_SequenceOfShape& aSeq,
    ...
    //
    Standard_Boolean IsFacesDifOriented, IsEdgesRevSense, anAd1 = Standard_False, anAd2 = Standard_False,
    ...
    if (aFObj.Orientation() == TopAbs_REVERSED) anyN.Reverse();
    gp_Dir aDNObj(anyN);

    //
    TopOpeBRepBuild_Tools::GetNormalToFaceOnEdge(aFTool, anEdgeTool, anyN);
    if (aFTool.Orientation() == TopAbs_REVERSED) anyN.Reverse();
    gp_Dir aDNTool(anyN);
    ...
    if (anAd1 && anAd2) {
        ...
        if (anAdjFaceObj.Orientation() == TopAbs_REVERSED) anyN.Reverse();
        gp_Dir aDNAObj(anyN);

        TopOpeBRepBuild_Tools::GetNormalToFaceOnEdge(TopoDS::Face(anAdjFaceTool), anEdgeTool, anyN);
        if (anAdjFaceTool.Orientation() == TopAbs_REVERSED) anyN.Reverse();
...
Standard_Integer TopOpeBRepBuild_Builder1::IsSame2d(const TopTools_SequenceOfShape& aSeq,
    ...
    if (!BRep_Tool::IsClosed(anEdgeObj) || !BRep_Tool::IsClosed(anEdgeTool)) return 1;
    ...
    if (aTranslateV.Magnitude() >= gp_Resolution) {
        ...
        if (anEObj.Orientation() == TopAbs_REVERSED) aOVec.Reverse();
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder1::OrientateEdgeOnFace(TopoDS_Edge& EdgeToPerform, const TopoDS_Face& baseFace,
                                                   const TopoDS_Face& edgeFace, const TopOpeBRepBuild_GTopo& G1,
    ...
    if (faceOri == TopAbs_REVERSED) aN1.Reverse();

    TopOpeBRepBuild_Tools::GetNormalToFaceOnEdge(baseFace, EdgeToPerform, aN2);
    if (baseOri == TopAbs_REVERSED) aN2.Reverse();

    if (aN1 * aN2 < 0) stateOfFaceOri = Standard_True;

    if (faceRef == 2) // for tool we need to reverse face in cut
        if (Opec12() || Opec21()) stateOfFaceOri = !stateOfFaceOri;

    // orientate edge with neworiE
    EdgeToPerform.Orientation(neworiE);

    if (currOri != baseOri) EdgeToPerform.Reverse();

    if (stateOfFaceOri) EdgeToPerform.Reverse();
}
...
static TopAbs_State ClassifyEdgeToFaceByOnePoint(const TopoDS_Edge& E, const TopoDS_Face& F) {
    ...
    if (C2D.IsNull()) return TopAbs_UNKNOWN;
