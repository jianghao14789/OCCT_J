...
static void GroupShape(TopTools_ListOfShape&, Standard_Boolean, TopTools_DataMapOfShapeListOfShape&);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_FuseFace::Init(const TopTools_ListOfShape& LIF, const TopTools_ListOfShape& LRF,
...
#ifdef OCCT_DEBUG
                        if (trc)
                            std::cout << " TopOpeBRepBuild_FuseFace::PerformFace : Empty list of edges" << std::endl;
#endif
                        myModified = Standard_False;
                        myDone = Standard_True;
                        myLFF = myLRF;
...
static void GroupShape(TopTools_ListOfShape& mylist, Standard_Boolean Keep_Edge,
...
#ifdef OCCT_DEBUG
            if (trc)
                std::cout << " TopOpeBRepBuild_FuseFace::MakeEdge : Failure in reconstructing new edge" << std::endl;
#endif
            return;
        }
...
Standard_Boolean SameSupport(const TopoDS_Edge& E1, const TopoDS_Edge& E2) {
    ...
    if (typC1 == STANDARD_TYPE(Geom_Line)) {
        ...
        if (Abs(li1.Angle(li2)) <= tolang && li1.Location().SquareDistance(li2.Location()) <= tollin * tollin) {
