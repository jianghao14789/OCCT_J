...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GWESMakeFaces(const TopoDS_Shape& FF, TopOpeBRepBuild_WireEdgeSet& WES,
...
static Standard_Boolean FUN_purgeFon1nonoriE(const TopoDS_Shape& newFace) {
    ...
    if (nE == 1) {
        ...
        if (BRep_Tool::Degenerated(theEdge)) return Standard_True;
        /////////////////////////////////////////////
    }
...
//---------------------------------------------------------------------------
static TopAbs_Orientation FUN_ReOrientIntExtEdge(const TopoDS_Edge& FRE, TopAbs_Orientation OFRE,
...
static Standard_Integer FUN_CheckORI(TopAbs_Orientation O1, TopAbs_Orientation O2) {
    ...
    if ((O1 == TopAbs_INTERNAL || O1 == TopAbs_EXTERNAL) && (O2 == TopAbs_INTERNAL || O2 == TopAbs_EXTERNAL))
        result = 0;
    else if ((O1 == TopAbs_INTERNAL || O1 == TopAbs_EXTERNAL) && (O2 == TopAbs_FORWARD || O2 == TopAbs_REVERSED))
        result = 1;
    else if ((O1 == TopAbs_FORWARD || O1 == TopAbs_REVERSED) && (O2 == TopAbs_INTERNAL || O2 == TopAbs_EXTERNAL))
        result = 2;
    else
        result = 4;
    return result;
...
void TopOpeBRepBuild_Builder::GFABUMakeFaces(const TopoDS_Shape& FF, TopOpeBRepBuild_FaceBuilder& FABU,
    ...
    for (; FABU.MoreFace(); FABU.NextFace()) {
            ...
            if (isold) {
                ...
                for (; FABU.MoreEdge(); FABU.NextEdge()) {
                    ...
                    if (oE == TopAbs_INTERNAL)
                        ...
                        neEXTERNAL++;
                    else if (oE == TopAbs_FORWARD)
                        neFORWARD++;
                    else if (oE == TopAbs_REVERSED)
                        neREVERSED++;
                    ...
                    {                                                                  // jyl980402+
                        ...
                        C2D =
                            FC2D_CurveOnSurface(newEdge, newFace, f2, l2, tolpc, Standard_True); // xpu051198 (CTS21701)
                        if (C2D.IsNull())
                            throw Standard_ProgramError(
                                "TopOpeBRepBuild_Builder::GFABUMakeFaces null PC"); // jyl980402+
                        Standard_Real tol = Max(tolE, tolpc);                       // jyl980402+
                        BRep_Builder BB_PC;
                        BB_PC.UpdateEdge(newEdge, C2D, newFace, tol); // jyl980402+
                    } // jyl980402+
                } // FABU.MoreEdge()
                ...
                if ((neINTERNAL == 0 && neEXTERNAL == 0) || (ne == neINTERNAL || ne == neEXTERNAL)) {
                    ...
                    if (nbw == 1 && ne == 2) {
                        ...
                        if (nEdge1.IsSame(nEdge2)) return;
                    }
                    ...
                    // checking: wire is closed and regular. If wire is not close or not regular: vertex has only the
                    // one edge or vetrex has more then two shared edges, we don't modify it.
        ...
        Standard_Boolean istrim = (T == STANDARD_TYPE(Geom_RectangularTrimmedSurface));
        if (istrim) {
            Handle(Geom_RectangularTrimmedSurface) hrts = Handle(Geom_RectangularTrimmedSurface)::DownCast(Surf);
