...
void TopOpeBRepBuild_Area1dBuilder::InitAreaBuilder(TopOpeBRepBuild_LoopSet& LS, TopOpeBRepBuild_LoopClassifier& LC,
                                                    const Standard_Boolean ForceClass) {
    TopAbs_State state;
            ...
            if (Loopinside) {
                ...
                while (LoopIter.More()) {
                    ...
                    if (loopoutside) {
                        ...
                        ADD_Loop_TO_LISTOFLoop(curL, removedLoops, (void*)("loopoutside = 1, area = removedLoops"));

                        allShape = allShape && curL->IsShape();
                        REM_Loop_FROM_LISTOFLoop(LoopIter, AreaIter.Value(), (void*)("loop of cur. area, cur. area"));
                    } else {
                        LoopIter.Next();
                    }
                }
                ...
                if (!removedLoops.IsEmpty()) {
                    if (allShape) {
                        ADD_LISTOFLoop_TO_LISTOFLoop(removedLoops, boundaryloops, (void*)("allShape = 1"),
                                                     (void*)("removedLoops"), (void*)("boundaryloops"));
                    } else {
                        // make a new area with the removed loops
                        TopOpeBRepBuild_ListOfLoop thelist;
                        myArea.Append(thelist);
                        ADD_LISTOFLoop_TO_LISTOFLoop(removedLoops, myArea.Last(), (void*)("allShape = 0"),
                                                     (void*)("removedLoops"), (void*)("new area"));
                    }
                }
            } // Loopinside == True
                ...
                while (LoopIter.More()) {
                    ...
                    if (ashapeinside && ablockinside) {
                        const Handle(TopOpeBRepBuild_Loop) & curL = LoopIter.Value();
                        ADD_Loop_TO_LISTOFLoop(curL, newArea0, (void*)("ashapeinside && ablockinside, new area"));
...
void TopOpeBRepBuild_Area1dBuilder::REM_Loop_FROM_LISTOFLoop(TopOpeBRepBuild_ListIteratorOfListOfLoop& ITA,
...
#ifdef OCCT_DEBUG
                                                                 const Standard_Address ss, const Standard_Address ss1,
                                                                 const Standard_Address ss2) const
#else
                                                                 const Standard_Address, const Standard_Address,
                                                                 const Standard_Address) const
#endif
{
#ifdef OCCT_DEBUG
