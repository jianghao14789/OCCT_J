...
TopOpeBRepBuild_PaveSet::TopOpeBRepBuild_PaveSet(const TopoDS_Shape& E)
    : myEdge(TopoDS::Edge(E)), myHasEqualParameters(Standard_False), myClosed(Standard_False),
      myPrepareDone(Standard_False), myRemovePV(Standard_True) {}
...
void TopOpeBRepBuild_PaveSet::SortPave(const TopOpeBRepBuild_ListOfPave& List, TopOpeBRepBuild_ListOfPave& SortedList) {
...
void TopOpeBRepBuild_PaveSet::Prepare() {
    ...
    if (trc) {
        ...
        for (; itd.More(); itd.Next()) {
            ...
            TopoDS_Vertex VSD;
            if (hasVSD) VSD = TopoDS::Vertex(PV->SameDomain());
