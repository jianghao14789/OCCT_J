...
Standard_EXPORT Standard_Boolean FUNKP_KPiskolesh(const TopOpeBRepBuild_Builder& BU,
                                                  const TopOpeBRepDS_DataStructure& BDS, const TopoDS_Shape& Sarg,
                                                  TopTools_ListOfShape& lShsd, TopTools_ListOfShape& lfhsd);
...
static void LocalKPisdisjanalyse(const TopAbs_State Stsol1, const TopAbs_State Stsol2,
                                 const TopOpeBRepBuild_KPart_Operation& theOperation, Standard_Integer& ires,
                                 Standard_Integer& icla1, Standard_Integer& icla2);

static TopoDS_Solid BuildNewSolid(const TopoDS_Solid& sol1, const TopoDS_Solid& sol2, const TopAbs_State stsol1,
                                  const TopAbs_State stsol2, const Standard_Integer ires, const Standard_Integer icla1,
                                  const Standard_Integer icla2, const TopAbs_State theState1,
...
static Standard_Integer FUN_comparekoletgesh(TopOpeBRepTool_ShapeClassifier& SC, const TopoDS_Shape& sh1,
                                             const TopoDS_Shape& sh2, const TopoDS_Shape& fkole1, const TopoDS_Shape&)
...
static Standard_Boolean FUN_updatev(const Handle(TopOpeBRepDS_HDataStructure) & HDS, TopoDS_Edge& newed,
                                    const TopoDS_Vertex& v, const TopAbs_Orientation oriv, const Standard_Real parv,
...
static Standard_Boolean FUN_makefaces(const TopOpeBRepBuild_Builder& BU,
                                      ...
                                      TopTools_DataMapOfShapeListOfShape& EnewE, const TopoDS_Shape& f,
                                      const TopoDS_Shape&, TopTools_ListOfShape& newfaces)
...
static Standard_Boolean FUN_rebuildfc(const TopOpeBRepBuild_Builder& BU,
                                      const Handle(TopOpeBRepDS_HDataStructure) & HDS, const TopAbs_State,
                                      const TopoDS_Shape& Fk, TopTools_DataMapOfShapeListOfShape& EnewE,
...
void TopOpeBRepBuild_Builder::MergeKPartiskoletge() {
    ...
    if (ires == RESNULL) {
        ...
        TopTools_IndexedDataMapOfShapeListOfShape
            fcnewfcSMA; // faces connexed to fSMA built up with the split of outerwSMA
        TopTools_IndexedDataMapOfShapeListOfShape
            fcnewfcGRE; // faces connexed to fGRE built up with the split of outerwGRE
...
void TopOpeBRepBuild_Builder::MergeKPartisdisj() {
    ...
    for (Standard_Integer i = TopAbs_SHELL; i <= TopAbs_VERTEX && !hasnotsol1 && !hasnotsol2; i++) {
        ...
        if (anExp.More()) hasnotsol1 = Standard_True;
        anExp.Init(myShape2, (TopAbs_ShapeEnum)i, TopAbs_SOLID);
        if (anExp.More()) hasnotsol2 = Standard_True;
    }
    ...
    if (soldisj) {
        ...
        if (aMapOfSolid1.Extent() > 1 || aMapOfSolid2.Extent() > 1) {
            ...
            if (Opefus()) {
                if (!disjPerformFuse(aMapOfSolid1, aMapOfSolid2, aMapOfResult)) return;
            } else if (Opec12()) {
                if (!disjPerformCut(aMapOfSolid1, aMapOfSolid2, aMapOfResult)) return;
            } else if (Opec21()) {
                if (!disjPerformCut(aMapOfSolid2, aMapOfSolid1, aMapOfResult)) return;
            } else if (Opecom()) {
                if (!disjPerformCommon(aMapOfSolid1, aMapOfSolid2, aMapOfResult)) return;
            }
...
// prequesitory : all interferences attached to <f1> are SSI
{
    ...
    for (; it1.More(); it1.Next()) {
        Handle(TopOpeBRepDS_ShapeShapeInterference)
            SSI1(Handle(TopOpeBRepDS_ShapeShapeInterference)::DownCast(it1.Value()));
...
Standard_Integer TopOpeBRepBuild_Builder::KPlhg(const TopoDS_Shape& S, const TopAbs_ShapeEnum T,
...
Standard_Integer TopOpeBRepBuild_Builder::KPlhsd(const TopoDS_Shape& S, const TopAbs_ShapeEnum T,
                                                 TopTools_ListOfShape& L) const {
    Standard_Integer n = 0;
    L.Clear();
...
TopAbs_State TopOpeBRepBuild_Builder::KPclasSS(const TopoDS_Shape& S1, const TopTools_ListOfShape& exLS1,
...
TopAbs_State TopOpeBRepBuild_Builder::KPclasSS(const TopoDS_Shape& S1, const TopoDS_Shape& exS1,
                                               const TopoDS_Shape& S2) {
...
Standard_Boolean TopOpeBRepBuild_Builder::KPiskoletgesh(const TopoDS_Shape& Sarg, TopTools_ListOfShape& lShsd,
...
void TopOpeBRepBuild_Builder::KPiskoletgeanalyse(const TopOpeBRepDS_Config config2, const TopAbs_State Stsol1,
                                                 const TopAbs_State Stsol2, Standard_Integer& ires) const {
    // -----------------------------------------------------------------------------
...
void TopOpeBRepBuild_Builder::KPisdisjanalyse(const TopAbs_State Stsol1, const TopAbs_State Stsol2,
                                              Standard_Integer& ires, Standard_Integer& icla1,
...
void TopOpeBRepBuild_Builder::KPclassFF(const TopoDS_Shape& F1, const TopoDS_Shape& F2, TopAbs_State& St1,
                                        TopAbs_State& St2) {
    St1 = KPclassF(F1, F2);
    St2 = KPclassF(F2, F1);
...
Standard_Boolean TopOpeBRepBuild_Builder::KPiskoleFF(const TopoDS_Shape& F1, const TopoDS_Shape& F2, TopAbs_State& St1,
                                                     TopAbs_State& St2) {
#ifdef OCCT_DEBUG
    Standard_Integer iF1;
    Standard_Boolean tSPS1 = GtraceSPS(F1, iF1);
...
static Standard_Boolean disjPerformFuse(const TopTools_IndexedMapOfShape& theMapOfSolid1,
    ...
    for (i = 1; i <= aMapOfSolid.Extent(); i++) {
        ...
        if (localshape1.ShapeType() != TopAbs_SOLID) return Standard_False;

        sol1 = TopoDS::Solid(localshape1);
        acurrentsolid = sol1;
        if (aMapOfUsedSolids.Contains(localshape1)) continue;
        ...
        while (j <= aMapOfSolid.Extent() && (acheckiterator <= aMaxNumberOfIterations)) {
            ...
            if (localshape2.ShapeType() != TopAbs_SOLID) return Standard_False;
            ...
            if (ires == RESSHAPE12) continue;

            if (ires == RESNEWSHA1 || ires == RESNEWSHA2) {
                TopoDS_Solid newsol =
                    BuildNewSolid(acurrentsolid, sol2, stsol1, stsol2, ires, icla1, icla2, TopAbs_OUT, TopAbs_OUT);
                ...
                if (acurrentsolid.IsNull()) return Standard_False;
            }
        } // end while(j)
    ...
    for (Standard_Integer i = 1; i <= aMapOfSeparatedSolid1.Extent(); i++) {
        ...
        for (Standard_Integer j = 1; j <= aMapOfSeparatedSolid2.Extent() && !NullResult; j++) {
            ...
            switch (ires) {
                ...
                case RESNEWSHA2: {
                    TopoDS_Solid newsol =
                        BuildNewSolid(acurrentsolid, sol2, stsol1, stsol2, ires, icla1, icla2, TopAbs_OUT, TopAbs_IN);
                    acurrentsolid = newsol;
                    break;
                }
                default:
