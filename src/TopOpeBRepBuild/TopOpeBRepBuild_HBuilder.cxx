...
TopOpeBRepBuild_HBuilder::TopOpeBRepBuild_HBuilder(const TopOpeBRepDS_BuildTool& BT)
    : myBuilder(BT), myMakeEdgeAncestorIsDone(Standard_False), myMakeCurveAncestorIsDone(Standard_False),
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_HBuilder::Perform(const Handle(TopOpeBRepDS_HDataStructure) & HDS, const TopoDS_Shape& S1,
...
void TopOpeBRepBuild_HBuilder::MergeShapes(const TopoDS_Shape& S1, const TopAbs_State ToBuild1, const TopoDS_Shape& S2,
...
void TopOpeBRepBuild_HBuilder::MergeSolids(const TopoDS_Shape& S1, const TopAbs_State ToBuild1, const TopoDS_Shape& S2,
                                           const TopAbs_State ToBuild2) {
...
void TopOpeBRepBuild_HBuilder::MakeEdgeAncestorMap() {
    if (myMakeEdgeAncestorIsDone) return;
    ...
    for (; it.More(); it.Next()) {
        ...
        if (re == 1)
            for (; its.More(); its.Next()) {
                ...
                if (!mySectEdgeDSEdges1.IsBound(SecEdg)) mySectEdgeDSEdges1.Bind(SecEdg, ei);
            }
        else if (re == 2)
            for (; its.More(); its.Next()) {
                TopoDS_Shape& SecEdg = its.Value();
                if (!mySectEdgeDSEdges2.IsBound(SecEdg)) mySectEdgeDSEdges2.Bind(SecEdg, ei);
            }
    }
...
Standard_Integer TopOpeBRepBuild_HBuilder::GetDSEdgeFromSectEdge(const TopoDS_Shape& E, const Standard_Integer rank) {
    if (!myMakeEdgeAncestorIsDone) MakeEdgeAncestorMap();

    Standard_Integer i = 0;

    if (rank == 1) {
        if (mySectEdgeDSEdges1.IsBound(E)) i = mySectEdgeDSEdges1.Find(E);
    }

    if (rank == 2) {
        if (mySectEdgeDSEdges2.IsBound(E)) i = mySectEdgeDSEdges2.Find(E);
    }
    return i;
}
...
void TopOpeBRepBuild_HBuilder::MakeCurveAncestorMap() {
    if (myMakeCurveAncestorIsDone) return;
...
Standard_Integer TopOpeBRepBuild_HBuilder::GetDSCurveFromSectEdge(const TopoDS_Shape& SectEdge) {
    Standard_Integer i = 0;
    if (!myMakeCurveAncestorIsDone) MakeCurveAncestorMap();
...
Standard_Integer TopOpeBRepBuild_HBuilder::GetDSPointFromNewVertex(const TopoDS_Shape& NewVert) {
    if (!myMakePointAncestorIsDone) {
        ...
        for (i = 1; i <= NbPoint; i++) {
            ...
            if (!Vertex.IsNull()) myNewVertexDSPoint.Bind(Vertex, i);
        }
    }

    Standard_Integer iPnt = 0;
    if (myNewVertexDSPoint.IsBound(NewVert)) iPnt = myNewVertexDSPoint.Find(NewVert);
    return iPnt;
}
...
Standard_Boolean TopOpeBRepBuild_HBuilder::EdgeCurveAncestors(const TopoDS_Shape& E, TopoDS_Shape& F1, TopoDS_Shape& F2,
                                                              Standard_Integer& IC) {
    F1.Nullify();
    F2.Nullify();
    IC = 0;
...
Standard_Boolean TopOpeBRepBuild_HBuilder::EdgeSectionAncestors(const TopoDS_Shape& E, TopTools_ListOfShape& LF1,
                                                                TopTools_ListOfShape& LF2, TopTools_ListOfShape& LE1,
    ...
    if (ie1) LE1.Append(DS.Shape(ie1));
    if (ie2) LE2.Append(DS.Shape(ie2));
