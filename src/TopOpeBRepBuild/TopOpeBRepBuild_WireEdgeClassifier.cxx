...
TopAbs_State TopOpeBRepBuild_WireEdgeClassifier::Compare(const Handle(TopOpeBRepBuild_Loop) & L1,
    ...
    if (isshape2 && isshape1) { // L1 is Shape , L2 is Shape
        ...
        if (state == TopAbs_UNKNOWN) {
            ...
            if (s1.IsNull()) return state;
            TopoDS_Shape s2 = LoopToShape(L2);
            if (s2.IsNull()) return state;
...
TopAbs_State TopOpeBRepBuild_WireEdgeClassifier::CompareShapes(const TopoDS_Shape& B1, const TopoDS_Shape& B2) {
...
TopAbs_State TopOpeBRepBuild_WireEdgeClassifier::CompareElementToShape(const TopoDS_Shape& EE, const TopoDS_Shape& B) {
...
void TopOpeBRepBuild_WireEdgeClassifier::ResetElement(const TopoDS_Shape& EE) {
    ...
    gp_Pnt P3D;
    if (!C3D.IsNull()) P3D = C3D->Value(par);
#endif
