...
Standard_EXPORT void TopOpeBRepDS_SetThePCurve(const BRep_Builder& B, TopoDS_Edge& E, const TopoDS_Face& F,
                                               const TopAbs_Orientation O, const Handle(Geom2d_Curve) & C);
// Standard_IMPORT Standard_Integer FUN_tool_outofUVbounds
//(const TopoDS_Face& fF,const TopoDS_Edge& E,Standard_Real& splitpar);

//---------------------------------------------
static Standard_Integer FUN_getG(const gp_Pnt P, const TopOpeBRepDS_ListOfInterference& LI,
                                 const Handle(TopOpeBRepDS_HDataStructure) HDS, Standard_Integer& iEinterf)
//---------------------------------------------
{
...
static Standard_Boolean FUN_EPIforEvisoONperiodicF(const TopoDS_Edge& E, const TopoDS_Face& F,
    ...
    Handle(TopOpeBRepDS_CurvePointInterference) CPI =
        new TopOpeBRepDS_CurvePointInterference(T, TopOpeBRepDS_EDGE, iEinterf, TopOpeBRepDS_POINT, iG, parone);
    loCPI.Append(CPI);
...
(const TopoDS_Edge& E, TopoDS_Vertex& V2pi, const Standard_Real& paronE,const TopoDS_Face& F, TopTools_ListOfShape&
losplits)
{
...
//---------------------------------------------
static void FUN_getEPIonEds(const TopoDS_Shape& FOR, const Handle(TopOpeBRepDS_HDataStructure) & HDS,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GMergeSolids(const TopTools_ListOfShape& LSO1, const TopTools_ListOfShape& LSO2,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GFillSolidsSFS(const TopTools_ListOfShape& LS1, const TopTools_ListOfShape& LS2,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GFillSolidSFS(const TopoDS_Shape& SO1, const TopTools_ListOfShape& LSO2,
                                            const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GFillShellSFS(const TopoDS_Shape& SH, const TopTools_ListOfShape& LSO2,
                                            const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
...
// ----------------------------------------------------------------------
static void FUNBUILD_MAPSUBSHAPES(const TopoDS_Shape& S, const TopAbs_ShapeEnum T, TopTools_IndexedMapOfShape& _IM) {
    TopExp::MapShapes(S, T, _IM);
}

// ----------------------------------------------------------------------
static void FUNBUILD_MAPSUBSHAPES(const TopTools_ListOfShape& LOFS, const TopAbs_ShapeEnum T,
...
// ----------------------------------------------------------------------
static void FUNBUILD_MAPANCSPLSHAPES(TopOpeBRepBuild_Builder& B, const TopoDS_Shape& S, const TopAbs_State STATE,
...
// ----------------------------------------------------------------------
static void FUNBUILD_MAPANCSPLSHAPES(TopOpeBRepBuild_Builder& B, const TopoDS_Shape& S,
                                     TopTools_IndexedDataMapOfShapeListOfShape& _IDM) {
...
// ----------------------------------------------------------------------
static void FUNBUILD_MAPANCSPLSHAPES(TopOpeBRepBuild_Builder& B, const TopTools_IndexedMapOfShape& M,
                                     TopTools_IndexedDataMapOfShapeListOfShape& _IDM) {
...
// ----------------------------------------------------------------------
Standard_EXPORT void FUNBUILD_ANCESTORRANKPREPARE(TopOpeBRepBuild_Builder& B, const TopTools_ListOfShape& LF1,
                                                  const TopTools_ListOfShape& LF2, const TopOpeBRepDS_Config CONF1,
...
// ----------------------------------------------------------------------
Standard_EXPORT void FUNBUILD_ANCESTORRANKGET(TopOpeBRepBuild_Builder& /*B*/, const TopoDS_Shape& f,
                                              Standard_Boolean& of1, Standard_Boolean& of2) {
...
// ----------------------------------------------------------------------
Standard_EXPORT void FUNBUILD_ORIENTLOFS(TopOpeBRepBuild_Builder& B, const TopAbs_State TB1, const TopAbs_State TB2,
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GSplitFaceSFS(const TopoDS_Shape& FOR, const TopTools_ListOfShape& LSclass,
                                            const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    TopAbs_State TB1, TB2;
    G1.StatesON(TB1, TB2);
    Standard_Boolean RevOri1 = G1.IsToReverse1();
...
//=======================================================================
void TopOpeBRepBuild_Builder::GMergeFaceSFS(const TopoDS_Shape& FOR, const TopOpeBRepBuild_GTopo& G1,
                                            TopOpeBRepBuild_ShellFaceSet& SFS) {
#ifdef OCCT_DEBUG
    Standard_Integer iFOR;
...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GSplitFace(const TopoDS_Shape& FOR, const TopOpeBRepBuild_GTopo& GG1,
...
void TopOpeBRepBuild_Builder::AddONPatchesSFS(const TopOpeBRepBuild_GTopo& G1, TopOpeBRepBuild_ShellFaceSet& SFS) {
    ...
    for (i = 1; i <= myONFacesMap.Extent(); i++) {
        ...
        if (myDataStructure->DS().AncestorRank(aFAnc1) == 1) {
            ...
            for (k = 1; k <= aMapE1.Extent() && aChkEdge.IsNull(); k++) {
                ...
                if (!BRep_Tool::Degenerated(aE)) aChkEdge = aE;
            }
            ...
            aN1 /= norm;
            if (aFace1.Orientation() == TopAbs_REVERSED) aN1.Reverse();
            ...
            for (j = i + 1; j <= myONFacesMap.Extent() && ok; j++) {
                ...
                if (myDataStructure->DS().AncestorRank(aFAnc2) == 2) {
                    ...
                    for (k = 1; k <= aMapE2.Extent() && sameBnd; k++)
                        if (!aMapE1.Contains(aMapE2(k))) sameBnd = Standard_False;
                    if (!sameBnd) continue;
                    ...
                    aN2 /= norm;
                    if (aFace2.Orientation() == TopAbs_REVERSED) aN2.Reverse();
...
static Standard_Boolean AreFacesCoincideInArea(const TopoDS_Shape& theBaseFace, const TopoDS_Shape& theFace,
                                               const TopoDS_Shape& theEdge, const TopTools_ListOfShape& allEdges,
    ...
    if (anEdgeOri != TopAbs_FORWARD && anEdgeOri != TopAbs_REVERSED) return Standard_False;
    ...
    gp_Vec2d d1;
    PCref->D1(pm, pt, d1);
    if (d1.Magnitude() < gp::Resolution()) return Standard_False;
    ...
    for (; it.More() && !isError; it.Next()) {
        ...
        if (aE.IsEqual(anEdge) ||
            (aE.Orientation() != TopAbs_FORWARD && aE.Orientation() != TopAbs_REVERSED && aE.IsSame(anEdge)))
        ...
        if (anInter.IsDone()) {
            ...
            for (i = 1; i <= anInter.NbSegments() && !isError; i++) {
                ...
                Standard_Real pLinF = aIS.HasFirstPoint() ? aIS.FirstPoint().ParamOnFirst() : -Precision::Infinite();
                Standard_Real pLinL = aIS.HasLastPoint() ? aIS.LastPoint().ParamOnFirst() : Precision::Infinite();
...
void TopOpeBRepBuild_Builder::FillOnPatches(const TopTools_ListOfShape& anEdgesON, const TopoDS_Shape& aBaseFace,
    ...
    if (!aMapON.IsEmpty()) FillSecEdgeAncestorMap(rankOpp, aMapON, anAncMap);
    if (!anAncMap.IsEmpty()) {
        for (it.Initialize(anEdgesON); it.More(); it.Next()) {
            ...
            if (anAncMap.IsBound(aE) && !avoidMap.Contains(aE)) {
                ...
                for (itF.Initialize(aFacesToCheck); itF.More() && !isOnFace; itF.Next()) {
                    ...
                    if (aE.Orientation() != TopAbs_FORWARD && aE.Orientation() != TopAbs_REVERSED) {
                        ok = AreFacesCoincideInArea(aBaseFace, aF, aE.Oriented(TopAbs_FORWARD), allEdges, isSameOri);
                        ok = ok ||
                             AreFacesCoincideInArea(aBaseFace, aF, aE.Oriented(TopAbs_REVERSED), allEdges, isSameOri);
                    } else
                ...
                if (isOnFace) WES.AddStartElement(aE);
            }
        }
    }
    ...
    anAncMap.Clear();
    if (!aMapON1.IsEmpty()) FillSecEdgeAncestorMap(rankBF, aMapON1, anAncMap);
        ...
        for (i = 1; i <= myONElemMap.Extent(); i++) {
            ...
            if (aE.ShapeType() == TopAbs_WIRE) {
                ...
                for (itW.Initialize(aE); itW.More() && anEdge.IsNull(); itW.Next()) {
                    ...
                    if (!BRep_Tool::Degenerated(e)) anEdge = anAncE = e;
                }
...
void TopOpeBRepBuild_Builder::FindFacesTouchingEdge(const TopoDS_Shape& aFace, const TopoDS_Shape& anEdge,
