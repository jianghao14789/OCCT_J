...
TopOpeBRepBuild_PaveClassifier::TopOpeBRepBuild_PaveClassifier(const TopoDS_Shape& E)
    : myEdgePeriodic(Standard_False), mySameParameters(Standard_False), myClosedVertices(Standard_False) {
...
Standard_Real TopOpeBRepBuild_PaveClassifier::AdjustCase(const Standard_Real p1, const TopAbs_Orientation o,
                                                         const Standard_Real first, const Standard_Real period,
                                                         const Standard_Real tol, Standard_Integer& cas) {
    Standard_Real p2;
...
#ifdef OCCT_DEBUG
    if (TopOpeBRepTool_GettraceVC()) std::cout << std::endl << "VC : set first parameter " << myFirst << std::endl;
#endif
}
