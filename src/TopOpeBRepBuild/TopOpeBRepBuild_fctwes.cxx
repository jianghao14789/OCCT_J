...
// purpose  :
//=======================================================================
void TopOpeBRepBuild_Builder::GFillCurveTopologyWES(const TopoDS_Shape& F1, const TopOpeBRepBuild_GTopo& G1,
    ...
    for (; FCit.More(); FCit.Next()) {
        if (ShapeInterf != TopAbs_SHAPE) {
            ...
            // see comment above
            if (aSDMap.Contains(SS)) continue;
...
//  Standard_Boolean FTFeqWESF = myFaceReference.IsEqual(WESF);
    ...
    for (; Iti.More(); Iti.Next()) {
        ...
        if (!EhasPConFTF) myBuildTool.PCurve(FTF, E, PC);
...
//      Standard_Real tol = Max(tolE,tolpc);
            ...
            if (!EhasPConWESF)
                std::cout << "TopOpeBRepBuild_Builder::GFillCurveTopologyWES : Null PCurve on F" << iWESF << std::endl;
#endif
        }

#ifdef OCCT_DEBUG
        if (tSPS)
            debaddpwes(iWESF, TB, iG, neworiE, (TopOpeBRepBuild_Builder* const)this, &WES, "GFillCurveTopology ",
                       "WES+ EofC ");
#endif
