...
//==================================================================================
void IntWalk_PWalking::ComputePasInit(const Standard_Real theDeltaU1, const Standard_Real theDeltaV1,
                                      const Standard_Real theDeltaU2, const Standard_Real theDeltaV2) {
...
//=======================================================================
static void IsParallel(const Handle(IntSurf_LineOn2S) & theLine, const Standard_Boolean theCheckSurf1,
                       const Standard_Real theToler, Standard_Boolean& theIsUparallel,
    ...
    for (Standard_Integer aNum = 1; aNum <= aNbPoints; aNum++, aNPoint += aStep) {
        ...
        if (aNPoint > aNbLinePnts) aNPoint = aNbLinePnts;
...
//=======================================================================
static Standard_Boolean AdjustToDomain(const Standard_Integer theNbElem, Standard_Real* theParam,
...
// purpose  :
//==================================================================================
IntWalk_PWalking::IntWalk_PWalking(const Handle(Adaptor3d_Surface) & Caro1, const Handle(Adaptor3d_Surface) & Caro2,
                                   const Standard_Real TolTangency, const Standard_Real Epsilon,
                                   const Standard_Real Deflection, const Standard_Real Increment)
    :

      done(Standard_True), close(Standard_False), tgfirst(Standard_False), tglast(Standard_False), myTangentIdx(0),
      fleche(Deflection), pasMax(0.0), tolconf(Epsilon), myTolTang(TolTangency), sensCheminement(1),
      previoustg(Standard_False), myIntersectionOn2S(Caro1, Caro2, TolTangency), STATIC_BLOCAGE_SUR_PAS_TROP_GRAND(0),
...
IntWalk_PWalking::IntWalk_PWalking(const Handle(Adaptor3d_Surface) & Caro1, const Handle(Adaptor3d_Surface) & Caro2,
                                   const Standard_Real TolTangency, const Standard_Real Epsilon,
                                   const Standard_Real Deflection, const Standard_Real Increment,
                                   const Standard_Real U1, const Standard_Real V1, const Standard_Real U2,
                                   const Standard_Real V2)
    :

      done(Standard_True), close(Standard_False), fleche(Deflection), tolconf(Epsilon), myTolTang(TolTangency),
      sensCheminement(1), myIntersectionOn2S(Caro1, Caro2, TolTangency), STATIC_BLOCAGE_SUR_PAS_TROP_GRAND(0),
      STATIC_PRECEDENT_INFLEXION(0) {
    Standard_Real KELARG = 20.;
    //
    pasMax = Increment * 0.2; //-- June 25 99 after problems with precision
...
// purpose  :
//==================================================================================
Standard_Boolean IntWalk_PWalking::PerformFirstPoint(const TColStd_Array1OfReal& ParDep, IntSurf_PntOn2S& FirstPoint) {
    sensCheminement = 1;
...
//=======================================================================
static Standard_Real SQDistPointSurface(const gp_Pnt& thePnt, const Adaptor3d_Surface& theSurf,
    ...
    if (!aExtPS.IsDone()) return RealLast();
...
static Standard_Boolean IsTangentExtCheck(const Handle(Adaptor3d_Surface) & theSurf1,
                                          const Handle(Adaptor3d_Surface) & theSurf2, const Standard_Real theU10,
                                          const Standard_Real theV10, const Standard_Real theU20,
                                          const Standard_Real theV20, const Standard_Real theToler,
                                          const Standard_Real theArrStep[]) {
    {
        ...
        const Standard_Real aDP = aN1.Dot(aN2), aSQ1 = aN1.SquareMagnitude(), aSQ2 = aN2.SquareMagnitude();

        if ((aSQ1 < RealSmall()) || (aSQ2 < RealSmall())) return Standard_True; // Tangent

        if (aDP * aDP < 0.9998 * aSQ1 * aSQ2) { // cos(ang N1<->N2) < 0.9999
    ...
    const Standard_Real aParUS1[aNbItems] = {theU10 + theArrStep[0], theU10 - theArrStep[0], theU10, theU10};
    const Standard_Real aParVS1[aNbItems] = {theV10, theV10, theV10 + theArrStep[1], theV10 - theArrStep[1]};
    const Standard_Real aParUS2[aNbItems] = {theU20 + theArrStep[2], theU20 - theArrStep[2], theU20, theU20};
    const Standard_Real aParVS2[aNbItems] = {theV20, theV20, theV20 + theArrStep[3], theV20 - theArrStep[3]};
    ...
    for (Standard_Integer i = 0; i < aNbItems; i++) {
        ...
        if (aSqDist > aSQToler) return Standard_False;
    }
...
// purpose  :
//==================================================================================
void IntWalk_PWalking::Perform(const TColStd_Array1OfReal& ParDep, const Standard_Real u1min, const Standard_Real v1min,
                               const Standard_Real u2min, const Standard_Real v2min, const Standard_Real u1max,
                               const Standard_Real v1max, const Standard_Real u2max, const Standard_Real v2max) {
    ...
    if (IsTangentExtCheck(Caro1, Caro2, Param(1), Param(2), Param(3), Param(4), myTolTang, pasuv)) return;
    ...
    const Standard_Real aTol[4] = {Epsilon(UM1 - Um1), Epsilon(VM1 - Vm1), Epsilon(UM2 - Um2), Epsilon(VM2 - Vm2)};
    ...
    while (!Arrive) // 010
    {
        ...
        do {
            ...
            if (myIntersectionOn2S.IsDone() && !myIntersectionOn2S.IsEmpty()) {
                ...
                if (aNewPnt[0] < Um1 || aNewPnt[0] > UM1 || aNewPnt[1] < Vm1 || aNewPnt[1] > VM1 || aNewPnt[2] < Um2 ||
                    aNewPnt[2] > UM2 || aNewPnt[3] < Vm2 || aNewPnt[3] > VM2) {
                ...
                switch (aStatus) // 007
                {
                    ...
                    case IntWalk_PasTropGrand: {
                        ...
                        if (LevelOfIterWithoutAppend > 5) {
                            for (Standard_Integer i = 0; i < 4; i++) {
                                if (pasSav[i] > pasInit[i]) continue;
                    ...
                    Standard_FALLTHROUGH case IntWalk_OK:
                                                ...
                                                */
                                                ...
                                                if (previoustg) {
...

                                                    {
                                                        ...
                                                        myIntersectionOn2S.Function().AuxillarSurface1()->D1(
                                                            curPntOnS1.X(), curPntOnS1.Y(), aPtemp, aDuS1, aDvS1);
                                                        myIntersectionOn2S.Function().AuxillarSurface2()->D1(
                                                            curPntOnS2.X(), curPntOnS2.Y(), aPtemp, aDuS2, aDvS2);
                                                        ...
                                                        const Standard_Real aF1 =
                                                            aF1x * aF1x + aF1y * aF1y + aF1z * aF1z;
                                                        const Standard_Real aF2 =
                                                            aF2x * aF2x + aF2y * aF2y + aF2z * aF2z;
...
Standard_Boolean IntWalk_PWalking::ExtendLineInCommonZone(const IntImp_ConstIsoparametric theChoixIso,
            ...
            switch (aStatus) {
                ...
                case IntWalk_ArretSurPoint: {
                    ...
                    if (!bStop) {
                        ...
                        if (u1 <= UM1 && u2 <= UM2 && v1 <= VM1 && v2 <= VM2 && u1 >= Um1 && u2 >= Um2 && v1 >= Vm1 &&
                            v2 >= Vm2)
                            pointisvalid = Standard_True;
                        ...
                        if (!pointisvalid) {
                            ...
                            if ((u1 > UM1) || (u1 < Um1)) pasuv[0] *= 0.5;

                            if ((v1 > VM1) || (v1 < Vm1)) pasuv[1] *= 0.5;

                            if ((u2 > UM2) || (u2 < Um2)) pasuv[2] *= 0.5;

                            if ((v2 > VM2) || (v2 < Vm2)) pasuv[3] *= 0.5;
                        }
                    } // end if(!bStop)
                    else { // if(bStop)
    ...
    for (pit = 0; !bExtendLine && (pit < 2); pit++) {
        ...
        if (((u1 - Um1) < ResoU1) || ((UM1 - u1) < ResoU1) || ((u2 - Um2) < ResoU2) || ((UM2 - u2) < ResoU2) ||
            ((v1 - Vm1) < ResoV1) || ((VM1 - v1) < ResoV1) || ((v2 - Vm2) < ResoV2) || ((VM2 - v2) < ResoV2))
        ...
        if (aStatus == IntWalk_OK) {
            ...
            if (aSeqOfNewPoint.Length() > 1) {
                ...
                aSeqOfNewPoint(aSeqOfNewPoint.Length())
                    .Parameters(LastParams.ChangeValue(0), LastParams.ChangeValue(1), LastParams.ChangeValue(2),
                ...
                if ((indexofiso == 1) || (indexofiso == 3)) anIsoDir = gp_Dir2d(1, 0);
                ...
                if (aTangentZoneDir.SquareMagnitude() > gp::Resolution()) {
                    ...
                    if (fabs(aTangentZoneDir.Angle(anIsoDir)) > piquota) {
                        ...
                        do {
                            ...
                            d1 = gp_Vec2d(gp_Pnt2d(FirstParams.Value(afirstindex), FirstParams.Value(afirstindex + 1)),
                                          gp_Pnt2d(LastParams.Value(afirstindex), LastParams.Value(afirstindex + 1)));
                            ii++;
...
//=======================================================================
Standard_Boolean IntWalk_PWalking::DistanceMinimizeByGradient(const Handle(Adaptor3d_Surface) & theASurf1,
    ...
    if (theASurf1->GetType() != GeomAbs_BezierSurface && theASurf1->GetType() != GeomAbs_BSplineSurface)
        return Standard_True;
    if (theASurf2->GetType() != GeomAbs_BezierSurface && theASurf2->GetType() != GeomAbs_BSplineSurface)
        return Standard_True;
...
//=======================================================================
Standard_Boolean IntWalk_PWalking::DistanceMinimizeByExtrema(const Handle(Adaptor3d_Surface) & theASurf,
                                                             const gp_Pnt& theP0, Standard_Real& theU0,
                                                             Standard_Real& theV0, const Standard_Real* theStep0) {
    const Standard_Real aTol = 1.0e-14;
    gp_Pnt aPS;
    ...
    do {
        ...
        if (aSQDistPrev < aTol) break;

        // Functions
        const Standard_Real aF1 = aD1Su.Dot(aVec), aF2 = aD1Sv.Dot(aVec);

        // Derivatives
        const Standard_Real aDf1u = aD2Su.Dot(aVec) + aD1Su.Dot(aD1Su), aDf1v = aD2Su.Dot(aD1Sv), aDf2u = aDf1v,
...
Standard_Boolean IntWalk_PWalking::HandleSingleSingularPoint(const Handle(Adaptor3d_Surface) & theASurf1,
    ...
    Standard_Real aLowBorder[4] = {theASurf1->FirstUParameter(), theASurf1->FirstVParameter(),
                                   theASurf2->FirstUParameter(), theASurf2->FirstVParameter()};
    Standard_Real aUppBorder[4] = {theASurf1->LastUParameter(), theASurf1->LastVParameter(),
                                   theASurf2->LastUParameter(), theASurf2->LastVParameter()};
    IntImp_ConstIsoparametric aLockedDir[4] = {IntImp_UIsoparametricOnCaro1, IntImp_VIsoparametricOnCaro1,
    ...
    for (Standard_Integer i = 1; i <= 4; ++i) {
        if (Abs(thePnt(i) - aLowBorder[i - 1]) < Precision::PConfusion() ||
            ...
            if (!anInt.IsDone()) continue;

            if (anInt.IsEmpty()) continue;
            ...
            for (k = 0; k < 2; ++k) {
                ...
                if (aMod > Precision::Confusion()) {
                    ...
                    if (Abs(aLowBorder[iu] - aPars[iu]) < aTolU) {
                        ...
                        if (aPInt.SquareDistance(aP) < aTol2) aPars[iu] = aLowBorder[iu];
                    } else if (Abs(aUppBorder[iu] - aPars[iu]) < aTolU) {
                        aP = aSurfs[k]->Value(aUppBorder[iu], aPars[iv]);
                        if (aPInt.SquareDistance(aP) < aTol2) aPars[iu] = aUppBorder[iu];
                    }
                }
                aMod = aDV.Magnitude();
                if (aMod > Precision::Confusion()) {
                    ...
                    if (Abs(aLowBorder[iv] - aPars[iv]) < aTolV) {
                        ...
                        if (aPInt.SquareDistance(aP) < aTol2) aPars[iv] = aLowBorder[iv];
                    } else if (Abs(aUppBorder[iv] - aPars[iv]) < aTolV) {
                        aP = aSurfs[k]->Value(aPars[iu], aUppBorder[iv]);
                        if (aPInt.SquareDistance(aP) < aTol2) aPars[iv] = aUppBorder[iv];
                    }
                }
            }
            //
            ...
            if (isInDomain) return Standard_True;
        }
    }
...
Standard_Boolean IntWalk_PWalking::SeekPointOnBoundary(const Handle(Adaptor3d_Surface) & theASurf1,
                                                       const Handle(Adaptor3d_Surface) & theASurf2,
                                                       const Standard_Real theU1, const Standard_Real theV1,
                                                       const Standard_Real theU2, const Standard_Real theV2,
                                                       const Standard_Boolean isTheFirst) {
    Standard_Boolean isOK = Standard_False;

    // u1, v1, u2, v2 order is used.
    const Standard_Real aLowBorder[4] = {theASurf1->FirstUParameter(), theASurf1->FirstVParameter(),
                                         theASurf2->FirstUParameter(), theASurf2->FirstVParameter()};
    const Standard_Real aUppBorder[4] = {theASurf1->LastUParameter(), theASurf1->LastVParameter(),
                                         theASurf2->LastUParameter(), theASurf2->LastVParameter()};
    ...
    do {
        ...
        if (aStatus && !AdjustToDomain(4, &aPnt(1), &aLowBorder[0], &aUppBorder[0])) break;

        aStatus = DistanceMinimizeByExtrema(theASurf1, theASurf2->Value(aPnt(3), aPnt(4)), aPnt(1), aPnt(2));
        if (aStatus && !AdjustToDomain(2, &aPnt(1), &aLowBorder[0], &aUppBorder[0])) break;

        aStatus = DistanceMinimizeByExtrema(theASurf2, theASurf1->Value(aPnt(1), aPnt(2)), aPnt(3), aPnt(4));
        if (aStatus && !AdjustToDomain(2, &aPnt(3), &aLowBorder[2], &aUppBorder[2])) break;
    ...
    if (aSingularStatus) aPnt = aSingularPnt;
    ...
    if (isTheFirst) {
        while (line->NbPoints() > 1) {
            ...
            for (++aPInd; aPInd <= aNbPnts; aPInd++) {
                ...
                if (aP1.SquareDistance(aP2) > Precision::SquareConfusion()) break;
            }
        ...
        while (line->NbPoints() > 1) {
            ...
            for (--aPInd; aPInd > 0; aPInd--) {
                ...
                if (aPCurr.SquareDistance(aPPrev) > Precision::SquareConfusion()) break;
            }
...
Standard_Boolean IntWalk_PWalking::PutToBoundary(const Handle(Adaptor3d_Surface) & theASurf1,
    ...
    if (aTol <= 2.0 * aTolMin) return hasBeenAdded;
...
Standard_Boolean IntWalk_PWalking::SeekAdditionalPoints(const Handle(Adaptor3d_Surface) & theASurf1,
    ...
    if (aNbPoints > theMinNbPoints) return Standard_True;
    ...
    while (aNbPoints < theMinNbPoints && (aNbPoints != aNbPointsPrev)) {
        ...
        for (Standard_Integer fp = 1, lp = 2; fp < aNbPoints; fp = lp + 1) {
            ...
            aPnt(1) = 0.5 * (U1f + U1l);
            if (aPnt(1) < aU1bFirst) aPnt(1) = aU1bFirst;
            if (aPnt(1) > aU1bLast) aPnt(1) = aU1bLast;

            aPnt(2) = 0.5 * (V1f + V1l);
            if (aPnt(2) < aV1bFirst) aPnt(2) = aV1bFirst;
            if (aPnt(2) > aV1bLast) aPnt(2) = aV1bLast;

            aPnt(3) = 0.5 * (U2f + U2l);
            if (aPnt(3) < aU2bFirst) aPnt(3) = aU2bFirst;
            if (aPnt(3) > aU2bLast) aPnt(3) = aU2bLast;

            aPnt(4) = 0.5 * (V2f + V2l);
            if (aPnt(4) < aV2bFirst) aPnt(4) = aV2bFirst;
            if (aPnt(4) > aV2bLast) aPnt(4) = aV2bLast;
            ...
            if (aStatus) {
                ...
                const Standard_Real aSQDist1 = aPInt.SquareDistance(aP1), aSQDist2 = aPInt.SquareDistance(aP2);
                ...
                if ((aSQDist1 < aTol) && (aSQDist2 < aTol)) {
                    ...
                    if (++aNbPoints >= theMinNbPoints) break;
                } else {
                    lp--;
                }
            }
...
void IntWalk_PWalking::RepartirOuDiviser(Standard_Boolean& DejaReparti, IntImp_ConstIsoparametric& ChoixIso,
...

{
    ...
    if (Arrive) { // restart in the other direction
      ...
      pasuv[3]=pasSav[3];
        ...
        if (pasuv[0] * 0.5 < ResoU1 && pasuv[1] * 0.5 < ResoV1 && pasuv[2] * 0.5 < ResoU2 && pasuv[3] * 0.5 < ResoV2) {
            if (!previoustg) {
...
//
{
    ...
    if (aSqDist < Precision::SquareConfusion()) {
        ...
        if (Abs(pasuv[choixIso] - pasInit[choixIso]) <= Precision::Confusion()) {
            ...
            gp_Pnt CurPnt = (choixIso == IntImp_UIsoparametricOnCaro1 || choixIso == IntImp_VIsoparametricOnCaro1)
            ...
            if (RefDist > gp::Resolution()) LocalResol = pasuv[choixIso] * tolconf / RefDist;
            if (pasuv[choixIso] < 2 * LocalResol) pasuv[choixIso] = pasInit[choixIso] = 2 * LocalResol;
        }
        ////////////////////////////////////////
    ...
    if (ResoU1 < Precision::PConfusion() || ResoV1 < Precision::PConfusion() || ResoU2 < Precision::PConfusion() ||
    ...
    if (aStatus != IntWalk_PointConfondu) {
        ...
        */
        ...
        if (aSinB2Max >= 0.0 && (aCosBetweenTangent <= 2.0 * aSinB2Max * aSinB2Max -
                                                           1.0)) { // Real deflection is greater or equal than tolconf
            aStatus = IntWalk_PasTropGrand;
        } else { // Real deflection is less than tolconf
            ...
            if (theStatus != IntWalk_PasTropGrand &&
                ((aSinB2Min < 0.0) || (aCosBetweenTangent >= 2.0 * aSinB2Min * aSinB2Min -
                                                                 1.0))) { // Real deflection is less than tolconf/2.0
...
Standard_Boolean IntWalk_PWalking::TestArret(const Standard_Boolean DejaReparti, TColStd_Array1OfReal& Param,
                                             IntImp_ConstIsoparametric& ChoixIso)
...
//
{
    ...
    if (Trouve) {
        ...
        if (!DejaReparti) { // find if line closed
            ...
            Standard_Boolean close2dS1 = (P1uvS1.XY() - PrevuvS1.XY()) * (P1uvS1.XY() - myIntersuvS1.XY()) < 0.0;
            // On S2
            ...
            Standard_Boolean close2dS2 = (P1uvS2.XY() - PrevuvS2.XY()) * (P1uvS2.XY() - myIntersuvS2.XY()) < 0.0;
