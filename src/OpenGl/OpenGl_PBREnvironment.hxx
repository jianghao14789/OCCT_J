...
//! This class contains specular and diffuse maps required for Image Base Lighting (IBL) in PBR shading model with it's
//! generation methods.
...
public:
    ...
    //!                            if theSpecMapLevelsNum less than 2 or less than Pow2Size + 1 it will be set to the
    //!                            corresponding values.
    //! @param theId OpenGl_Resource name
    //! @return handle to created PBR environment or NULL handle in case of fail
    Standard_EXPORT static Handle(OpenGl_PBREnvironment)
        Create(const Handle(OpenGl_Context) & theCtx, unsigned int thePow2Size = 9,
               unsigned int theSpecMapLevelsNum = 6, const TCollection_AsciiString& theId = "PBREnvironment");
...
public:
    ...
    //! @param theDiffMapNbSamples number of samples in Monte-Carlo integration for diffuse IBL spherical harmonics
    //! calculation
    //! @param theSpecMapNbSamples number of samples in Monte-Carlo integration for specular IBL map generation
    //! @param theProbability controls strength of samples number reducing strategy during specular IBL map baking (see
    //! 'SpecIBLMapSamplesFactor' for details) theZIsInverted and theIsTopDown can be taken from Graphic3d_CubeMap
    //! source of environment cubemap. theDiffMapNbSamples and theSpecMapNbSamples is the main parameter directly
    //! affected to performance.
    Standard_EXPORT void Bake(const Handle(OpenGl_Context) & theCtx, const Handle(OpenGl_Texture) & theEnvMap,
                              Standard_Boolean theZIsInverted = Standard_False,
                              Standard_Boolean theIsTopDown = Standard_True, Standard_Size theDiffMapNbSamples = 1024,
                              Standard_Size theSpecMapNbSamples = 256, Standard_ShortReal theProbability = 0.99f);
    ...
    //! If creation method with this values returns the PBR environment having real sizes which are equals to current
    //! ones then this method will return false. It is handful when sizes are required to be changed. If this method
    //! returns false there is no reason to recreate PBR environment in order to change sizes.
    Standard_EXPORT bool SizesAreDifferent(unsigned int thePow2Size, unsigned int theSpecMapLevelsNumber) const;
...
private:
    ...
    Standard_EXPORT OpenGl_PBREnvironment(const Handle(OpenGl_Context) & theCtx, unsigned int thePowOf2Size = 9,
...
private:
    ...
    struct BakingParams {
        ...
        BakingParams()
            : NbSpecSamples(0), NbDiffSamples(0), EnvMapSize(1024), Probability(1.0f), IsZInverted(false),
              IsTopDown(false) {}
    };
    ...
    //! Warning! Requires using of OpenGl_PBREnvironmentSentry.
    void bake(const Handle(OpenGl_Context) & theCtx, const Handle(OpenGl_Texture) & theEnvMap,
              Standard_Boolean theZIsInverted = Standard_False, Standard_Boolean theIsTopDown = Standard_True,
              Standard_Size theDiffMapNbSamples = 1024, Standard_Size theSpecMapNbSamples = 256,
...
private:
    ...
    Standard_Boolean
        myIsNeededToBeBound; //!< indicates whether IBL map's textures have to be bound or it is not obligate
    Standard_Boolean
        myCanRenderFloat; //!< indicates whether driver supports rendering into floating point texture or not
};
