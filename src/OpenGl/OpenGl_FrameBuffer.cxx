...
OpenGl_FrameBuffer::OpenGl_FrameBuffer()
    : myInitVPSizeX(0), myInitVPSizeY(0), myVPSizeX(0), myVPSizeY(0), myNbSamples(0),
      myDepthFormat(GL_DEPTH24_STENCIL8), myGlFBufferId(NO_FRAMEBUFFER), myGlColorRBufferId(NO_RENDERBUFFER),
      myGlDepthRBufferId(NO_RENDERBUFFER), myIsOwnBuffer(false), myIsOwnColor(false), myIsOwnDepth(false),
...
Standard_Boolean OpenGl_FrameBuffer::InitWrapper(const Handle(OpenGl_Context) & theGlContext,
    ...
    for (NCollection_Sequence<Handle(OpenGl_Texture)>::Iterator aColorIter(theColorTextures); aColorIter.More();
         aColorIter.Next()) {
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_FrameBuffer::Init(const Handle(OpenGl_Context) & theGlContext, const Graphic3d_Vec2i& theSize,
                                          const Standard_Integer theColorFormat, const Standard_Integer theDepthFormat,
...
Standard_Boolean OpenGl_FrameBuffer::Init(const Handle(OpenGl_Context) & theGlContext, const Graphic3d_Vec2i& theSize,
                                          const OpenGl_ColorFormats& theColorFormats,
                                          const Standard_Integer theDepthFormat, const Standard_Integer theNbSamples) {
    myColorFormats = theColorFormats;
    ...
    if (theNbSamples != 0) {
        ...
        // extensions (GL_OES_packed_depth_stencil, GL_OES_depth_texture) + GL version might be used to determine
        // supported formats instead of just trying to create such texture
        const OpenGl_TextureFormat aDepthFormat = OpenGl_TextureFormat::FindSizedFormat(theGlContext, myDepthFormat);
        if (aDepthFormat.IsValid() && !myDepthStencilTexture->Init(theGlContext, aDepthFormat,
                                                                   Graphic3d_Vec2i(aSizeX, aSizeY), Graphic3d_TOT_2D)) {
    ...
    if (myDepthStencilTexture->IsValid()) {
        ...
        if (hasDepthStencilAttach(theGlContext) && hasStencilRB) {
            ...
            theGlContext->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                                            myGlDepthRBufferId);
            if (hasStencilRB) {
                theGlContext->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                                                myGlDepthRBufferId);
            }
        }
    }
...
Standard_Boolean OpenGl_FrameBuffer::InitLazy(const Handle(OpenGl_Context) & theGlContext,
                                              ...
                                              const Standard_Integer theDepthFormat,
                                              const Standard_Integer theNbSamples) {
    if (myVPSizeX == theViewportSize.x() && myVPSizeY == theViewportSize.y() && myColorFormats == theColorFormats &&
        myDepthFormat == theDepthFormat && myNbSamples == theNbSamples) {
        return IsValid();
    }
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_FrameBuffer::InitWithRB(const Handle(OpenGl_Context) & theGlCtx, const Graphic3d_Vec2i& theSize,
                                                const Standard_Integer theColorFormat,
                                                const Standard_Integer theDepthFormat,
    ...
    theGlCtx->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER,
                                                myGlColorRBufferId);
    if (myGlDepthRBufferId != NO_RENDERBUFFER) {
        if (hasDepthStencilAttach(theGlCtx) && hasStencilRB) {
            theGlCtx->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                                        myGlDepthRBufferId);
        } else {
            theGlCtx->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER,
                                                        myGlDepthRBufferId);
            if (hasStencilRB) {
                theGlCtx->arbFBO->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                                                            myGlDepthRBufferId);
            }
        }
    }
...
Standard_Boolean OpenGl_FrameBuffer::InitWrapper(const Handle(OpenGl_Context) & theGlCtx) {
    ...
    GLint aDepthType = 0;
    GLint aDepthId = 0;
    theGlCtx->arbFBO->glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                                            GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &aColorType);
    theGlCtx->arbFBO->glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                                            GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &aDepthType);
    ...
    if (aColorType == GL_RENDERBUFFER) {
        theGlCtx->arbFBO->glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                                                GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &aColorId);
    ...
    if (aDepthType == GL_RENDERBUFFER) {
        theGlCtx->arbFBO->glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                                                GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &aDepthId);
...
// purpose  :
// =======================================================================
void OpenGl_FrameBuffer::ChangeViewport(const Standard_Integer theVPSizeX, const Standard_Integer theVPSizeY) {
...
void OpenGl_FrameBuffer::UnbindBuffer(const Handle(OpenGl_Context) & theGlCtx) {
    if (!theGlCtx->DefaultFrameBuffer().IsNull() && theGlCtx->DefaultFrameBuffer().operator->() != this) {
...
template <typename T>
inline void convertRowFromRgba(T* theRgbRow, const Image_ColorRGBA* theRgbaRow, const Standard_Size theWidth) {
...
Standard_Boolean OpenGl_FrameBuffer::BufferDump(const Handle(OpenGl_Context) & theGlCtx,
                                                const Handle(OpenGl_FrameBuffer) & theFbo, Image_PixMap& theImage,
    ...
    if (theBufferType == Graphic3d_BT_Depth && aFormat != GL_DEPTH_COMPONENT) {
        return Standard_False;
    }
#else
    (void)theBufferType;
    ...
    if (toConvRgba2Rgb) {
        ...
        for (Standard_Size aRow = 0; aRow < theImage.SizeY(); ++aRow) {
            ...
            theGlCtx->core11fwd->glReadPixels(0, GLint(theImage.SizeY() - aRow - 1), GLsizei(theImage.SizeX()), 1,
                                              aFormat, aType, aRowBuffer.ChangeData());
        ...
        for (Standard_Size aRow = 0; aRow < theImage.SizeY(); ++aRow) {
            ...
            // while order in memory depends on the flag and processed by ChangeRow() method
            theGlCtx->core11fwd->glReadPixels(0, GLint(theImage.SizeY() - aRow - 1), GLsizei(theImage.SizeX()), 1,
                                              aFormat, aType, theImage.ChangeRow(aRow));
        }
    } else {
        theGlCtx->core11fwd->glReadPixels(0, 0, GLsizei(theImage.SizeX()), GLsizei(theImage.SizeY()), aFormat, aType,
                                          theImage.ChangeData());
    }
