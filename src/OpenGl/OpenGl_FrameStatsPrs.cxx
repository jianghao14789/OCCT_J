...
OpenGl_FrameStatsPrs::OpenGl_FrameStatsPrs()
    : myStatsPrev(new OpenGl_FrameStats()), myCountersTrsfPers(new Graphic3d_TransformPers(
                                                Graphic3d_TMF_2d, Aspect_TOTP_LEFT_UPPER, Graphic3d_Vec2i(20, 20))),
      myChartTrsfPers(new Graphic3d_TransformPers(Graphic3d_TMF_2d, Aspect_TOTP_RIGHT_UPPER, Graphic3d_Vec2i(20, 20))),
      myChartVertices(new OpenGl_VertexBuffer()), myChartIndices(new OpenGl_IndexBuffer()),
...
void OpenGl_FrameStatsPrs::updateChart(const Handle(OpenGl_Workspace) & theWorkspace) {
    ...
    if (aMaxDuration <= 0.0f) {
        for (Standard_Integer aFrameIter = aStats->DataFrames().Lower(); aFrameIter <= aStats->DataFrames().Upper();
             ++aFrameIter) {
    ...
    if (myChartArray.IsNull() || myChartArray->VertexNumber() != aNbVerts || myChartArray->EdgeNumber() != aNbIndexes) {
    ...
    myChartVertices->init(aCtx, myChartArray->Attributes()->Stride, myChartArray->Attributes()->NbElements,
                          myChartArray->Attributes()->Data(), GL_UNSIGNED_BYTE, myChartArray->Attributes()->Stride);
    if (myChartArray->Indices()->Stride == 2) {
        myChartIndices->Init(aCtx, 1, myChartArray->Indices()->NbElements,
...

    {
        ...
        aParams.SetHorizontalAlignment((!myChartTrsfPers.IsNull() && myChartTrsfPers->IsTrihedronOr2d() &&
                                        (myChartTrsfPers->Corner2d() & Aspect_TOTP_RIGHT) != 0)
                                           ...
                                           : Graphic3d_HTA_LEFT);
        aParams.SetVerticalAlignment(Graphic3d_VTA_CENTER);
        TCollection_AsciiString aLabels[3] = {TCollection_AsciiString() + 0 + " ms", formatTimeMs(aMaxDuration * 0.5),
                                              formatTimeMs(aMaxDuration)};

        const float aLabX = aParams.HorizontalAlignment() == Graphic3d_HTA_RIGHT ? float(anOffset.x())
...
void OpenGl_FrameStatsPrs::Render(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    // draw counters
    {
        ...
        if (!myCountersTrsfPers.IsNull()) {
            myCountersTrsfPers->Apply(aCtx->Camera(), aCtx->ProjectionState.Current(),
                                      aCtx->WorldViewState.ChangeCurrent(), aCtx->VirtualViewport()[2],
    ...
    if (myChartIndices->IsValid() && myChartIndices->GetElemsNb() > 0) {
        ...
        if (!myChartTrsfPers.IsNull()) {
            myChartTrsfPers->Apply(aCtx->Camera(), aCtx->ProjectionState.Current(),
                                   aCtx->WorldViewState.ChangeCurrent(), aCtx->VirtualViewport()[2],
                                   aCtx->VirtualViewport()[3]);
        }
        aCtx->ApplyModelViewMatrix();
        ...
        aCtx->ShaderManager()->BindFaceProgram(Handle(OpenGl_TextureSet)(), Graphic3d_TypeOfShadingModel_Unlit,
                                               Graphic3d_AlphaMode_Blend, true, false, Handle(OpenGl_ShaderProgram)());
        ...
        myChartVertices->bindAttribute(aCtx, Graphic3d_TOA_COLOR, 4, GL_UNSIGNED_BYTE,
                                       myChartVertices->GetComponentsNb(), (void*)sizeof(Graphic3d_Vec3));

        myChartIndices->Bind(aCtx);
        aCtx->core15fwd->glDrawElements(GL_TRIANGLES, myChartIndices->GetElemsNb(), myChartIndices->GetDataType(),
                                        NULL);
        ...
        myChartLines->bindAttribute(aCtx, Graphic3d_TOA_COLOR, 4, GL_UNSIGNED_BYTE, myChartLines->GetComponentsNb(),
