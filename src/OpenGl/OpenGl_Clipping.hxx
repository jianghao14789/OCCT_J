...
public: //! @name general methods
    ...
    Standard_Boolean IsClippingOrCappingOn() const {
        return NbClippingOrCappingOn() > 0;
    }
...
public:
    ...
    Standard_Integer CappedSubPlane() const {
        return myCappedSubPlane;
    }

    //! Return TRUE if capping algorithm is in state, when all clipping planes are temporarily disabled except currently
    //! processed one.
    bool IsCappingFilterOn() const {
        return !myCappedChain.IsNull();
    }

    //! Return TRUE if capping algorithm is in state, when all clipping planes are temporarily disabled except currently
    //! processed one.
    bool IsCappingDisableAllExcept() const {
        return myCappedSubPlane > 0;
    }

    //! Return TRUE if capping algorithm is in state, when all clipping planes are enabled except currently rendered
    //! one.
...
private:
    ...
    Handle(Graphic3d_ClipPlane)
        myCappedChain; //!< chain which is either temporary disabled or the only one enabled for Capping algorithm
    Standard_Integer myCappedSubPlane; //!< sub-plane index within filtered chain; positive number for DisableAllExcept
                                       //!< and negative for EnableAllExcept
