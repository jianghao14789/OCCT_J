...
void OpenGl_Structure::renderBoundingBox(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    if (aCtx->core20fwd != NULL && aCtx->ShaderManager()->BindBoundBoxProgram()) {
        ...
        aCtx->ActiveProgram()->SetUniform(aCtx, "occBBoxCenter",
                                          Graphic3d_Vec3((float)aCenter.x(), (float)aCenter.y(), (float)aCenter.z()));
        aCtx->ActiveProgram()->SetUniform(aCtx, "occBBoxSize",
        ...
        const OpenGl_Vec3 aVerts[16] = {
            OpenGl_Vec3(aMin.x(), aMin.y(), aMin.z()), OpenGl_Vec3(aMin.x(), aMin.y(), aMax.z()),
            OpenGl_Vec3(aMin.x(), aMax.y(), aMax.z()), OpenGl_Vec3(aMin.x(), aMax.y(), aMin.z()),
            OpenGl_Vec3(aMin.x(), aMin.y(), aMin.z()), OpenGl_Vec3(aMax.x(), aMin.y(), aMin.z()),
            OpenGl_Vec3(aMax.x(), aMin.y(), aMax.z()), OpenGl_Vec3(aMax.x(), aMax.y(), aMax.z()),
            OpenGl_Vec3(aMax.x(), aMax.y(), aMin.z()), OpenGl_Vec3(aMax.x(), aMin.y(), aMin.z()),
            OpenGl_Vec3(aMax.x(), aMax.y(), aMin.z()), OpenGl_Vec3(aMin.x(), aMax.y(), aMin.z()),
            OpenGl_Vec3(aMin.x(), aMax.y(), aMax.z()), OpenGl_Vec3(aMax.x(), aMax.y(), aMax.z()),
            OpenGl_Vec3(aMax.x(), aMin.y(), aMax.z()), OpenGl_Vec3(aMin.x(), aMin.y(), aMax.z())};
...
OpenGl_Structure::OpenGl_Structure(const Handle(Graphic3d_StructureManager) & theManager)
    : Graphic3d_CStructure(theManager), myInstancedStructure(NULL), myIsRaytracable(Standard_False),
      myModificationState(0), myIsMirrored(Standard_False) {
...
void OpenGl_Structure::SetTransformation(const Handle(TopLoc_Datum3D) & theTrsf) {
    ...
    if (!myTrsf.IsNull()) {
        ...
        const Standard_Real aDet =
            aTrsf.Value(1, 1) * (aTrsf.Value(2, 2) * aTrsf.Value(3, 3) - aTrsf.Value(3, 2) * aTrsf.Value(2, 3)) -
...
void OpenGl_Structure::RemoveGroup(const Handle(Graphic3d_Group) & theGroup) {
    ...
    for (Graphic3d_SequenceOfGroup::Iterator aGroupIter(myGroups); aGroupIter.More(); aGroupIter.Next()) {
        ...
        if (aGroupIter.Value() == theGroup) {
            const Standard_Boolean wasRaytracable = static_cast<const OpenGl_Group&>(*theGroup).IsRaytracable();
...
// purpose  :
// =======================================================================
void OpenGl_Structure::renderGeometry(const Handle(OpenGl_Workspace) & theWorkspace, bool& theHasClosed) const {
...
void OpenGl_Structure::Render(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    if (aCtx->Clipping().IsClippingOrCappingOn()) {
        ...
        if (!myClipPlanes.IsNull() && myClipPlanes->ToOverrideGlobal()) {
            ...
            if (myTrsfPers->IsZoomOrRotate()) {
                ...
                for (OpenGl_ClippingIterator aPlaneIt(aCtx->Clipping()); aPlaneIt.More() && aPlaneIt.IsGlobal();
                     aPlaneIt.Next()) {
    ...
    // Apply highlight box
    if (!isClipped && !myHighlightStyle.IsNull() && myHighlightStyle->Method() == Aspect_TOHM_BOUNDBOX) {
        aCtx->ApplyModelViewMatrix();
        theWorkspace->SetHighlightStyle(myHighlightStyle);
...
void OpenGl_Structure::applyPersistence(const Handle(OpenGl_Context) & theCtx,
                                        ...
                                        const Standard_Boolean theIsLocal, Standard_Boolean& theOldCastShadows) const {
    ...
    if (theIsLocal && theTrsfPers->IsZoomOrRotate()) {
        ...
        Graphic3d_Vec4 anAnchorPoint =
            aModelWorld * Graphic3d_Vec4((Standard_ShortReal)aStartPnt.X(), (Standard_ShortReal)aStartPnt.Y(),
        ...
        theTrsfPers->Apply(theCtx->Camera(), theCtx->ProjectionState.Current(), aWorldView,
                           theCtx->VirtualViewport()[2], theCtx->VirtualViewport()[3], &aStartPnt);
    } else {
        theTrsfPers->Apply(theCtx->Camera(), theCtx->ProjectionState.Current(), aWorldView,
