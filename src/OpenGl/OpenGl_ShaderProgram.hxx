...
struct OpenGl_SetterInterface {
    ...
    virtual void Set(const Handle(OpenGl_Context) & theCtx, const Handle(Graphic3d_ShaderVariable) & theVariable,
...
public:
    ...
    //! Sets user-defined uniform variable to specified program.
    void Set(const Handle(OpenGl_Context) & theCtx, const Handle(Graphic3d_ShaderVariable) & theVariable,
...
public:
    ...
    //! Note you may safely put invalid location in functions like glUniform* - the data passed in will be silently
    //! ignored.
    //! @return true if location is not equal to -1.
    bool IsValid() const {
        return myLocation != INVALID_LOCATION;
    }
...
public:
    ...
    //! Manager has been designed to synchronize camera position, lights definition and other aspects of the program
    //! implicitly, as well as sharing same program across rendering groups.
    //!
    ...
    //!
    //! This constructor has been made public to provide more flexibility to re-use OCCT OpenGL classes without OCCT
    //! Viewer itself. If this is not the case - create the program using shared OpenGl_ShaderManager instance instead.
...
public:
    ...
    Standard_EXPORT Standard_Boolean Link(const Handle(OpenGl_Context) & theCtx, bool theIsVerbose = true);
...
public:
    ...
    Standard_EXPORT GLint GetAttributeLocation(const Handle(OpenGl_Context) & theCtx, const GLchar* theName) const;
...
public:
    ...
    Standard_EXPORT Standard_Boolean GetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
...
public:
    ...
    Standard_EXPORT Standard_Boolean GetAttribute(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
                                                  OpenGl_Vec4i& theValue) const;
...
public:
    ...
    Standard_EXPORT Standard_Boolean SetAttributeName(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
    ...
    Standard_EXPORT Standard_Boolean SetAttribute(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
                                                  GLfloat theValue);
...
public:
    ...
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLint theValue);
...
public:
    ...
    //! Wrapper for glUniform2uiv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, const GLchar* theName,
                                                const GLsizei theCount, const OpenGl_Vec2u* theValue);
...
public:
    ...
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const NCollection_Mat3<float>* theData);

    //! Specifies the value of the float uniform 4x4 matrix.
    //! Wrapper for glUniformMatrix4fv()
    bool SetUniform(const Handle(OpenGl_Context) & theCtx, const GLchar* theName, const OpenGl_Mat4& theValue,
    ...
    //! Wrapper for glUniformMatrix4fv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                const OpenGl_Mat4& theValue, GLboolean theTranspose = GL_FALSE);

    //! Specifies the value of the array of float uniform 4x4 matrices.
    //! Wrapper over glUniformMatrix4fv().
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Mat4* theData);

    //! Specifies the value of the float uniform array
    //! Wrapper over glUniform1fv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const Standard_ShortReal* theData);

    //! Specifies the value of the float2 uniform array
    //! Wrapper over glUniform2fv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec2* theData);

    //! Specifies the value of the float3 uniform array
    //! Wrapper over glUniform3fv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec3* theData);

    //! Specifies the value of the float4 uniform array
    //! Wrapper over glUniform4fv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec4* theData);

    //! Specifies the value of the integer uniform array
    //! Wrapper over glUniform1iv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const Standard_Integer* theData);

    //! Specifies the value of the int2 uniform array
    //! Wrapper over glUniform2iv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec2i* theData);

    //! Specifies the value of the int3 uniform array
    //! Wrapper over glUniform3iv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec3i* theData);

    //! Specifies the value of the int4 uniform array
    //! Wrapper over glUniform4iv()
    Standard_EXPORT Standard_Boolean SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                GLuint theCount, const OpenGl_Vec4i* theData);

public:
    //! Specifies the value of the sampler uniform variable.
    bool SetSampler(const Handle(OpenGl_Context) & theCtx, const GLchar* theName,
                    const Graphic3d_TextureUnit theTextureUnit) {
        return SetSampler(theCtx, GetUniformLocation(theCtx, theName), theTextureUnit);
    }

    //! Specifies the value of the sampler uniform variable.
    Standard_EXPORT Standard_Boolean SetSampler(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                const Graphic3d_TextureUnit theTextureUnit);

public:
    //! Update the shader program from external files (per shader stage) in the following way:
    //! 1) If external file does not exist, then it will be created (current source code will be dumped, no
    //! recompilation) and FALSE will be returned. 2) If external file exists and it has the same timestamp as
    //! myDumpDate, nothing will be done and FALSE will be returned. 3) If external file exists and it has    newer
    //! timestamp than myDumpDate, shader  will be recompiled and relinked and TRUE will be returned.
    //! @param theCtx OpenGL context bound to this working thread
    //! @param theFolder folder to store files; when unspecified, $CSF_ShadersDirectoryDump or current folder will be
    //! used instead
...
protected:
    Standard_Size
        myCurrentState[OpenGl_UniformStateType_NB]; //!< defines last modification for variables of each state type
...
template <class T> struct OpenGl_VariableSetter : public OpenGl_SetterInterface {
    virtual void Set(const Handle(OpenGl_Context) & theCtx, const Handle(Graphic3d_ShaderVariable) & theVariable,
...
template <class K, class V> struct MapListOfType {
...
template <class K, class V> MapListOfType<K, V> CreateListOf(K theKey, V theValue) {
