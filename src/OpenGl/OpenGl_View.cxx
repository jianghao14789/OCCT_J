...
static bool checkWasFailedFbo(const Handle(OpenGl_FrameBuffer) & theFboToCheck, Standard_Integer theSizeX,
                              Standard_Integer theSizeY, Standard_Integer theNbSamples) {
    return !theFboToCheck->IsValid() && theFboToCheck->GetInitVPSizeX() == theSizeX &&
           theFboToCheck->GetInitVPSizeY() == theSizeY && theFboToCheck->NbSamples() == theNbSamples;
}
...
static bool chooseOitColorConfiguration(const Handle(OpenGl_Context) & theGlContext,
                                        const Standard_Integer theConfigIndex, OpenGl_ColorFormats& theFormats) {
    theFormats.Clear();
...
OpenGl_View::OpenGl_View(const Handle(Graphic3d_StructureManager) & theMgr,
                         const Handle(OpenGl_GraphicDriver) & theDriver, const Handle(OpenGl_Caps) & theCaps,
                         OpenGl_StateCounter* theCounter)
    : Graphic3d_CView(theMgr), myDriver(theDriver.operator->()), myCaps(theCaps), myWasRedrawnGL(Standard_False),
      myToShowGradTrihedron(false), myZLayers(Structure_MAX_PRIORITY - Structure_MIN_PRIORITY + 1),
      myStateCounter(theCounter), myCurrLightSourceState(theCounter->Increment()), myLightsRevision(0),
      myLastLightSourceState(0, 0), mySRgbState(-1),
      myFboColorFormat(GL_SRGB8_ALPHA8), // note that GL_SRGB8 is not required to be renderable, unlike GL_RGB8,
                                         // GL_RGBA8, GL_SRGB8_ALPHA8
      myFboDepthFormat(GL_DEPTH24_STENCIL8), myToFlipOutput(Standard_False),
      //
      myFrameCounter(0), myHasFboBlit(Standard_True), myToDisableOIT(Standard_False),
      myToDisableOITMSAA(Standard_False), myToDisableMSAA(Standard_False), myTransientDrawToFront(Standard_True),
      myBackBufferRestored(Standard_False), myIsImmediateDrawn(Standard_False), myTextureParams(new OpenGl_Aspects()),
      myCubeMapParams(new OpenGl_Aspects()), myColoredQuadParams(new OpenGl_Aspects()),
      myPBREnvState(OpenGl_PBREnvState_NONEXISTENT), myPBREnvRequest(Standard_False),
      // ray-tracing fields initialization
      myRaytraceInitStatus(OpenGl_RT_NONE), myIsRaytraceDataValid(Standard_False),
      myIsRaytraceWarnTextures(Standard_False),
      myRaytraceBVHBuilder(new BVH_BinnedBuilder<Standard_ShortReal, 3, BVH_Constants_NbBinsBest>(
          BVH_Constants_LeafNodeSizeAverage, BVH_Constants_MaxTreeDepth, Standard_False,
          OSD_Parallel::NbLogicalProcessors() + 1)),
      myRaytraceSceneRadius(0.0f), myRaytraceSceneEpsilon(1.0e-6f), myToUpdateEnvironmentMap(Standard_False),
      myRaytraceLayerListState(0), myPrevCameraApertureRadius(0.f), myPrevCameraFocalPlaneDist(0.f) {
...
// purpose  :
// =======================================================================
void OpenGl_View::SetWindow(const Handle(Aspect_Window) & theWindow, const Aspect_RenderingContext theContext) {
    myWindow = myDriver->CreateRenderWindow(theWindow, theContext);
    Standard_ASSERT_RAISE(!myWindow.IsNull(), "OpenGl_View::SetWindow, "
...
static void SetMinMaxValuesCallback(Graphic3d_CView* theView) {
    ...
    if (!aBox.IsVoid()) {
        ...
        Graphic3d_Vec3 aMinVec((Standard_ShortReal)aMin.X(), (Standard_ShortReal)aMin.Y(),
                               (Standard_ShortReal)aMin.Z());
        Graphic3d_Vec3 aMaxVec((Standard_ShortReal)aMax.X(), (Standard_ShortReal)aMax.Y(),
...
Standard_Boolean OpenGl_View::BufferDump(Image_PixMap& theImage, const Graphic3d_BufferType& theBufferType) {
    ...
    if (!myRaytraceParameters.AdaptiveScreenSampling) {
        return myWorkspace->BufferDump(myAccumFrames % 2 ? myRaytraceFBO2[0] : myRaytraceFBO1[0], theImage,
                                       theBufferType);
    }
...
void OpenGl_View::SetGradientBackground(const Aspect_GradientBackground& theBackground) {
    ...
    myBackgrounds[Graphic3d_TOB_GRADIENT]->SetGradientParameters(aColor1, aColor2,
...
void OpenGl_View::SetBackgroundImage(const Handle(Graphic3d_TextureMap) & theTextureMap,
    ...
    if (theToUpdatePBREnv) {
        ...
        const TCollection_AsciiString anIdNew = !aCubeMap.IsNull() ? aCubeMap->GetId() : TCollection_AsciiString();
        if (anIdNew != anIdOld) {
...
void OpenGl_View::SetImageBasedLighting(Standard_Boolean theToEnableIBL) {
    ...
    const TCollection_AsciiString anIdOld = !myCubeMapIBL.IsNull() ? myCubeMapIBL->GetId() : TCollection_AsciiString();
...
// purpose  :
//=======================================================================
void OpenGl_View::InsertLayerBefore(const Graphic3d_ZLayerId theLayerId, const Graphic3d_ZLayerSettings& theSettings,
...
// purpose  :
//=======================================================================
void OpenGl_View::InsertLayerAfter(const Graphic3d_ZLayerId theLayerId, const Graphic3d_ZLayerSettings& theSettings,
...
// purpose  :
//=======================================================================
void OpenGl_View::SetZLayerSettings(const Graphic3d_ZLayerId theLayerId, const Graphic3d_ZLayerSettings& theSettings) {
...
// purpose  :
//=======================================================================
Handle(Standard_Transient) OpenGl_View::FBOCreate(const Standard_Integer theWidth, const Standard_Integer theHeight) {
...
// purpose  :
//=======================================================================
void OpenGl_View::FBOGetDimensions(const Handle(Standard_Transient) & theFbo, Standard_Integer& theWidth,
                                   Standard_Integer& theHeight, Standard_Integer& theWidthMax,
...
// purpose  :
//=======================================================================
void OpenGl_View::FBOChangeViewport(const Handle(Standard_Transient) & theFbo, const Standard_Integer theWidth,
...
void OpenGl_View::drawBackground(const Handle(OpenGl_Workspace) & theWorkspace,
    ...
    if (myBackgroundType == Graphic3d_TOB_CUBEMAP) {
        myCubeMapParams->Aspect()->ShaderProgram()->PushVariableInt("uZCoeff",
                                                                    myCubeMapBackground->ZIsInverted() ? -1 : 1);
        myCubeMapParams->Aspect()->ShaderProgram()->PushVariableInt("uYCoeff",
        ...
        if (myBackgrounds[Graphic3d_TOB_TEXTURE]->IsDefined() && myTextureParams->Aspect()->ToMapTexture()) {
...
bool OpenGl_View::prepareFrameBuffers(Graphic3d_Camera::Projection& theProj) {
    ...
    const bool toInitImmediateFbo =
        myTransientDrawToFront && (!aCtx->caps->useSystemBuffer || (toUseOit && HasImmediateStructures()));

    if (aFrameBuffer == NULL && !aCtx->DefaultFrameBuffer().IsNull() && aCtx->DefaultFrameBuffer()->IsValid()) {
        aFrameBuffer = aCtx->DefaultFrameBuffer().operator->();
    }

    if (myHasFboBlit && (myTransientDrawToFront || theProj == Graphic3d_Camera::Projection_Stereo || aNbSamples != 0 ||
                         toUseOit || aSizeX != aRendSize.x())) {
        if (myMainSceneFbos[0]->GetVPSize() != aRendSize || myMainSceneFbos[0]->NbSamples() != aNbSamples) {
        ...
        if (myMainSceneFbos[0]->IsValid() && (toInitImmediateFbo || myImmediateSceneFbos[0]->IsValid())) {
            ...
            if (!myImmediateSceneFbos[0]->InitLazy(aCtx, *myMainSceneFbos[0]) && !wasFailedImm0) {
    ...
    if (theProj == Graphic3d_Camera::Projection_Stereo && IsActiveXR() && myMainSceneFbos[0]->IsValid()) {
        if (aNbSamples != 0 || aSizeX != aRendSize.x()) {
            hasXRBlitFbo =
                myXrSceneFbo->InitLazy(aCtx, Graphic3d_Vec2i(aSizeX, aSizeY), myFboColorFormat, myFboDepthFormat, 0);
            if (!hasXRBlitFbo) {
        ...
        if (!myMainSceneFbos[1]->InitLazy(aCtx, *myMainSceneFbos[0]) && !wasFailedMain1) {
        ...
        if (!myMainSceneFbos[1]->IsValid()) {
            ...
            //
        } else if (!aCtx->HasStereoBuffers() || myRenderParams.StereoMode != Graphic3d_StereoMode_QuadBuffer) {
            const bool wasFailedImm0 = checkWasFailedFbo(myImmediateSceneFbos[0], myMainSceneFbos[0]);
            ...
            if (!myImmediateSceneFbos[1]->InitLazy(aCtx, *myMainSceneFbos[0]) && !wasFailedImm1) {
                TCollection_ExtendedString aMsg = TCollection_ExtendedString() + "Error! Immediate FBO (first) " +
                                                  printFboFormat(myImmediateSceneFbos[1]) +
                                                  " initialization has failed";
                aCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH, aMsg);
            }
    ...
    if (myRenderParams.ShadingModel == Graphic3d_TypeOfShadingModel_Pbr ||
        ...
        if (!myPBREnvironment.IsNull() &&
            myPBREnvironment->SizesAreDifferent(myRenderParams.PbrEnvPow2Size, myRenderParams.PbrEnvSpecMapNbLevels)) {
        ...
        if (myPBREnvState == OpenGl_PBREnvState_NONEXISTENT && aCtx->HasPBR()) {
            myPBREnvironment = OpenGl_PBREnvironment::Create(aCtx, myRenderParams.PbrEnvPow2Size,
            ...
            if (myPBREnvState == OpenGl_PBREnvState_CREATED) {
                ...
                if (!aCtx->GetResource(THE_SHARED_ENV_LUT_KEY, anEnvLUT)) {
                    ...
                    // OES_texture_half_float_linear. OpenGL ES 3.0 allows initialization of GL_RG16F from 32-bit float
                    // data, but OpenGL ES 2.0 + OES_texture_half_float does not. Note that it is expected that GL_RG16F
                    // has enough precision for this table, so that it can be used also on desktop OpenGL.
                    ...
                    if (anImgFormat == Image_Format_RGF) {
                        ...
                        aPixMapRG.InitWrapper(Image_Format_RGF, (Standard_Byte*)Textures_EnvLUT, Textures_EnvLUTSize,
                                              Textures_EnvLUTSize);
...
#if defined(GL_ES_VERSION_2_0)
                    if (aTexFormat.IsValid() && hasHalfFloat) {
                    ...
                    if (!aTexFormat.IsValid() ||
                        !anEnvLUT->Init(aCtx, aTexFormat, Graphic3d_Vec2i((Standard_Integer)Textures_EnvLUTSize),
                                        Graphic3d_TOT_2D, aPixMap.get())) {
                        aCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH,
    ...
    if (toUseOit && myRenderParams.TransparencyMethod == Graphic3d_RTM_BLEND_OIT) {
        ...
        for (; anFboIt < 2; ++anFboIt) {
            ...
            if (aMainSceneFbo->IsValid() &&
                (aMainSceneFboOit->GetVPSize() != aRendSize || aMainSceneFboOit->NbSamples() != aNbSamples)) {
                ...
                for (;;) // seemly responding to driver limitation (GL_FRAMEBUFFER_UNSUPPORTED)
                {
                    ...
                    if (aMainSceneFboOit->Init(aCtx, aRendSize, myFboOitColorConfig,
                                               aMainSceneFbo->DepthStencilTexture(), aNbSamples)) {
                        break;
                    }
            ...
            if (anImmediateSceneFbo->IsValid() && (anImmediateSceneFboOit->GetVPSize() != aRendSize ||
    ...
    if (toUseShadowMap) {
        ...
        for (Standard_Integer aShadowIter = 0; aShadowIter < myShadowMaps->Size(); ++aShadowIter) {
            ...
            if (aShadowFbo->GetVPSizeX() != myRenderParams.ShadowMapResolution && toUseShadowMap) {
                ...
                if (!aShadowFbo->Init(aCtx, Graphic3d_Vec2i(myRenderParams.ShadowMapResolution), aDummy,
                                      myFboDepthFormat, 0)) {
...
void OpenGl_View::Redraw() {
    ...
    if (myRenderParams.Method == Graphic3d_RM_RAYTRACING && !myCaps->vboDisable && !myCaps->keepArrayData) {
    ...
    OpenGl_FrameBuffer* aFrameBuffer = myFBO.get();
    bool toSwap = aCtx->IsRender() && !aCtx->caps->buffersNoSwap && aFrameBuffer == NULL &&
    ...
    if (aProjectType == Graphic3d_Camera::Projection_Stereo) {
        OpenGl_FrameBuffer* aMainFbos[2] = {myMainSceneFbos[0]->IsValid() ? myMainSceneFbos[0].operator->() : NULL,
                                            myMainSceneFbos[1]->IsValid() ? myMainSceneFbos[1].operator->() : NULL};
        OpenGl_FrameBuffer* aMainFbosOit[2] = {myMainSceneFbosOit[0]->IsValid() ? myMainSceneFbosOit[0].operator->()
                                                                                : NULL,
        ...
        if (IsActiveXR()) {
            ...
            anImmFbosOit[0] = aMainFbosOit[0];
            anImmFbosOit[1] = aMainFbosOit[1];
        } else if (aStereoMode == Graphic3d_StereoMode_SoftPageFlip || aStereoMode == Graphic3d_StereoMode_QuadBuffer) {
            anImmFbos[0] = NULL;
            anImmFbos[1] = NULL;
            anImmFbosOit[0] = NULL;
            anImmFbosOit[1] = NULL;
...
#endif
            myXRSession->SubmitEye((void*)(size_t)anXRFbo->ColorTexture()->TextureId(), aGraphicsLib,
                                   Aspect_ColorSpace_sRGB, Aspect_Eye_Left);
        }
...
#endif
            myXRSession->SubmitEye((void*)(size_t)anXRFbo->ColorTexture()->TextureId(), aGraphicsLib,
                                   Aspect_ColorSpace_sRGB, Aspect_Eye_Right);
    ...
    if (myRenderParams.Method == Graphic3d_RM_RAYTRACING && myRenderParams.IsGlobalIlluminationEnabled) {
        myAccumFrames++;
    }
...
void OpenGl_View::RedrawImmediate() {
    if (!myWorkspace->Activate()) return;
    ...
    if (aProjectType == Graphic3d_Camera::Projection_Stereo) {
        ...
        OpenGl_FrameBuffer* anImmFbosOit[2] = {
            ...
            : myImmediateSceneFbosOit[0]->IsValid() ? myImmediateSceneFbosOit[0].operator->()
                                                    : NULL};
        if (aStereoMode == Graphic3d_StereoMode_SoftPageFlip || aStereoMode == Graphic3d_StereoMode_QuadBuffer) {
            anImmFbos[0] = NULL;
            anImmFbos[1] = NULL;
            anImmFbosOit[0] = NULL;
            anImmFbosOit[1] = NULL;
        ...
        aCtx->SetResolution(myRenderParams.Resolution, myRenderParams.ResolutionRatio(),
                            anImmFbos[0] != NULL ? myRenderParams.RenderResolutionScale : 1.0f);
        toSwap = redrawImmediate(Graphic3d_Camera::Projection_MonoLeftEye, aMainFbos[0], anImmFbos[0], anImmFbosOit[0],
                                 Standard_True) ||
                 toSwap;
        if (aStereoMode == Graphic3d_StereoMode_SoftPageFlip && toSwap && myFBO.get() == NULL &&
        ...
        aCtx->SetResolution(myRenderParams.Resolution, myRenderParams.ResolutionRatio(),
                            anImmFbos[1] != NULL ? myRenderParams.RenderResolutionScale : 1.0f);
        toSwap = redrawImmediate(Graphic3d_Camera::Projection_MonoRightEye, aMainFbos[1], anImmFbos[1], anImmFbosOit[1],
                                 Standard_True) ||
                 toSwap;
        if (anImmFbos[0] != NULL) {
            drawStereoPair(aFrameBuffer);
        ...
        aCtx->SetResolution(myRenderParams.Resolution, myRenderParams.ResolutionRatio(),
                            anImmFbo != aFrameBuffer ? myRenderParams.RenderResolutionScale : 1.0f);
        toSwap = redrawImmediate(aProjectType, aMainFbo, anImmFbo, anImmFboOit, Standard_True) || toSwap;
        if (anImmFbo != NULL && anImmFbo != aFrameBuffer) {
            blitBuffers(anImmFbo, aFrameBuffer, myToFlipOutput);
        }
    }
    ...
    if (toSwap && myFBO.get() == NULL && !aCtx->caps->buffersNoSwap) {
        aCtx->SwapBuffers();
    } else {
        aCtx->core11fwd->glFlush();
    }
...
void OpenGl_View::redraw(const Graphic3d_Camera::Projection theProjection, OpenGl_FrameBuffer* theReadDrawFbo,
    ...
    aCtx->core11fwd->glClearColor(aBgColor.r(), aBgColor.g(), aBgColor.b(),
...
// purpose  :
// =======================================================================
bool OpenGl_View::redrawImmediate(const Graphic3d_Camera::Projection theProjection, OpenGl_FrameBuffer* theReadFbo,
                                  OpenGl_FrameBuffer* theDrawFbo, OpenGl_FrameBuffer* theOitAccumFbo,
...
void OpenGl_View::renderShadowMap(const Handle(OpenGl_ShadowMap) & theShadowMap) {
    ...
    // aShadowBuffer->GetVPSizeY()); OpenGl_FrameBuffer::BufferDump (aCtx, aShadowBuffer, anImage, Graphic3d_BT_Depth);
    // anImage.Save (TCollection_AsciiString ("shadow") +
...
// purpose  :
//=======================================================================
void OpenGl_View::render(Graphic3d_Camera::Projection theProjection, OpenGl_FrameBuffer* theOutputFBO,
                         OpenGl_FrameBuffer* theOitAccumFbo, const Standard_Boolean theToDrawImmediate) {
    // ==================================
    //      Step 1: Prepare for render
    // ==================================

    const Handle(OpenGl_Context) & aContext = myWorkspace->GetGlContext();
    aContext->SetAllowSampleAlphaToCoverage(myRenderParams.ToEnableAlphaToCoverage && theOutputFBO != NULL &&
    ...
    if (StateInfo(myCurrLightSourceState, aManager->LightSourceState().Index()) != myLastLightSourceState ||
        ...
        aManager->UpdateLightSourceStateTo(aLights, SpecIBLMapLevels(),
                                           myShadowMaps->IsValid() ? myShadowMaps : Handle(OpenGl_ShadowMapArray)());
    ...
    aManager->SetShadingModel(
        OpenGl_ShaderManager::PBRShadingModelFallback(myRenderParams.ShadingModel, checkPBRAvailability()));
...
// purpose  :
//=======================================================================
void OpenGl_View::renderStructs(Graphic3d_Camera::Projection theProjection, OpenGl_FrameBuffer* theReadDrawFbo,
                                OpenGl_FrameBuffer* theOitAccumFbo, const Standard_Boolean theToDrawImmediate) {
    myZLayers.UpdateCulling(myWorkspace, theToDrawImmediate);
    if (myZLayers.NbStructures() <= 0) return;

    Handle(OpenGl_Context) aCtx = myWorkspace->GetGlContext();
    Standard_Boolean toRenderGL = theToDrawImmediate || myRenderParams.Method != Graphic3d_RM_RAYTRACING ||
                                  myRaytraceInitStatus == OpenGl_RT_FAIL || aCtx->IsFeedback();

    if (!toRenderGL) {
        const Graphic3d_Vec2i aSizeXY = theReadDrawFbo != NULL ? theReadDrawFbo->GetVPSize()
        ...
        if (!toRenderGL) {
            ...
            if (aCtx->arbFBOBlit != NULL) {
                ...
                const Standard_Integer aPrevFilter =
                    myWorkspace->RenderFilter() & ~(Standard_Integer)(OpenGl_RenderFilter_NonRaytraceableOnly);
                myWorkspace->SetRenderFilter(aPrevFilter | OpenGl_RenderFilter_NonRaytraceableOnly);
                {
                    ...
                    myZLayers.Render(myWorkspace, theToDrawImmediate, OpenGl_LF_RayTracable, theReadDrawFbo,
                                     theOitAccumFbo);
                }
...
void OpenGl_View::renderFrameStats() {
    if (myRenderParams.ToShowStats && myRenderParams.CollectedStats != Graphic3d_RenderingParams::PerfCounters_NONE) {
...
// purpose  :
//=======================================================================
void OpenGl_View::renderScene(Graphic3d_Camera::Projection theProjection, OpenGl_FrameBuffer* theReadDrawFbo,
                              OpenGl_FrameBuffer* theOitAccumFbo, const Standard_Boolean theToDrawImmediate) {
    const Handle(OpenGl_Context) & aContext = myWorkspace->GetGlContext();
...
OpenGl_VertexBuffer* OpenGl_View::initBlitQuad(const Standard_Boolean theToFlip) {
    ...
    if (!theToFlip) {
        ...
        if (!aVerts->IsValid()) {
            OpenGl_Vec4 aQuad[4] = {OpenGl_Vec4(1.0f, -1.0f, 1.0f, 0.0f), OpenGl_Vec4(1.0f, 1.0f, 1.0f, 1.0f),
                                    OpenGl_Vec4(-1.0f, -1.0f, 0.0f, 0.0f), OpenGl_Vec4(-1.0f, 1.0f, 0.0f, 1.0f)};
            aVerts->Init(myWorkspace->GetGlContext(), 4, 4, aQuad[0].GetData());
        }
    } else {
        aVerts = &myFullScreenQuadFlip;
        if (!aVerts->IsValid()) {
            OpenGl_Vec4 aQuad[4] = {OpenGl_Vec4(1.0f, -1.0f, 1.0f, 1.0f), OpenGl_Vec4(1.0f, 1.0f, 1.0f, 0.0f),
                                    OpenGl_Vec4(-1.0f, -1.0f, 0.0f, 1.0f), OpenGl_Vec4(-1.0f, 1.0f, 0.0f, 0.0f)};
            aVerts->Init(myWorkspace->GetGlContext(), 4, 4, aQuad[0].GetData());
        }
    }
    return aVerts;
...
// purpose  :
// =======================================================================
bool OpenGl_View::blitBuffers(OpenGl_FrameBuffer* theReadFbo, OpenGl_FrameBuffer* theDrawFbo,
    ...
    if (aCtx->arbFBOBlit != NULL && !toApplyGamma && theReadFbo->NbSamples() != 0) {
        ...
        aCtx->arbFBOBlit->glBlitFramebuffer(0, 0, aReadSizeX, aReadSizeY, 0, 0, aDrawSizeX, aDrawSizeY, aCopyMask,
                                            GL_NEAREST);
        ...
        if (anErr != GL_NO_ERROR) {
            ...
            TCollection_ExtendedString aMsg = TCollection_ExtendedString() + "FBO blitting has failed [Error " +
...
#if defined(GL_ES_VERSION_2_0)
        if (!aCtx->IsGlGreaterEqual(3, 0) && !aCtx->extFragDepth) {
        ...
        const Graphic3d_TypeOfTextureFilter aFilter =
            (aDrawSizeX == aReadSizeX && aDrawSizeY == aReadSizeY) ? Graphic3d_TOTF_NEAREST : Graphic3d_TOTF_BILINEAR;
        ...
        if (aVerts->IsValid() &&
            ...
            TCollection_ExtendedString aMsg = TCollection_ExtendedString() + "Error! FBO blitting has failed";
            aCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH, aMsg);
...
void OpenGl_View::drawStereoPair(OpenGl_FrameBuffer* theDrawFbo) {
    ...
    OpenGl_FrameBuffer* aPair[2] = {myImmediateSceneFbos[0]->IsValid() ? myImmediateSceneFbos[0].operator->() : NULL,
                                    myImmediateSceneFbos[1]->IsValid() ? myImmediateSceneFbos[1].operator->() : NULL};
    ...
    if (isOddY && (myRenderParams.StereoMode == Graphic3d_StereoMode_RowInterlaced ||
                   myRenderParams.StereoMode == Graphic3d_StereoMode_ChessBoard)) {
        toReverse = !toReverse;
    }
    if (isOddX && (myRenderParams.StereoMode == Graphic3d_StereoMode_ColumnInterlaced ||
                   myRenderParams.StereoMode == Graphic3d_StereoMode_ChessBoard)) {
        toReverse = !toReverse;
    }
    ...
    if (aVerts->IsValid() && aManager->BindStereoProgram(myRenderParams.StereoMode)) {
        ...
        TCollection_ExtendedString aMsg = TCollection_ExtendedString() + "Error! Anaglyph has failed";
        aCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH, aMsg);
    }
}
...
bool OpenGl_View::copyBackToFront() {
    ...
    Graphic3d_TransformUtils::Ortho2D(aProjectMat, 0.0f, static_cast<GLfloat>(myWindow->Width()), 0.0f,
...
void OpenGl_View::updatePBREnvironment(const Handle(OpenGl_Context) & theCtx) {
    ...
    if (!aGlTextureSet.IsNull() && !aGlTextureSet->IsEmpty()) {
        myPBREnvironment->Bake(theCtx, aGlTextureSet->First(), myCubeMapIBL->ZIsInverted(), myCubeMapIBL->IsTopDown(),
                               myRenderParams.PbrEnvBakingDiffNbSamples, myRenderParams.PbrEnvBakingSpecNbSamples,
