...
public: //! @name flags to disable particular functionality, should be used only for testing purposes!
    ...
    Standard_Boolean
        compressedTexturesDisable; //!< Disables uploading of compressed texture formats native to GPU (OFF by default)
    Standard_Boolean vboDisable;   //!< disallow VBO usage for debugging purposes (OFF by default)
    Standard_Boolean
        pntSpritesDisable; //!< flag permits Point Sprites usage, will significantly affect performance (OFF by default)
    Standard_Boolean keepArrayData; //!< Disables freeing CPU memory after building VBOs (OFF by default)
    Standard_Boolean
        ffpEnable; //!< Enables FFP (fixed-function pipeline), do not use built-in GLSL programs (OFF by default)
    Standard_Boolean usePolygonMode;  //!< Enables Polygon Mode instead of built-in GLSL programs (OFF by default;
                                      //!< unsupported on OpenGL ES)
    Standard_Boolean useSystemBuffer; //!< Enables usage of system backbuffer for blitting (OFF by default on desktop
                                      //!< OpenGL and ON on OpenGL ES for testing)
    Standard_Integer swapInterval;    //!< controls swap interval - 0 for VSync off and 1 for VSync on, 1 by default
    Standard_Boolean
        useZeroToOneDepth; //!< use [0, 1] depth range instead of [-1, 1] range, when possible (OFF by default)
