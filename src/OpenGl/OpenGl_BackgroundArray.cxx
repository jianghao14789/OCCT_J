...
OpenGl_BackgroundArray::OpenGl_BackgroundArray(const Graphic3d_TypeOfBackground theType)
    : OpenGl_PrimitiveArray(NULL, Graphic3d_TOPA_TRIANGLES, NULL, NULL, NULL), myType(theType),
      myFillMethod(Aspect_FM_NONE), myViewWidth(0), myViewHeight(0), myToUpdate(Standard_False) {
...
// purpose :
// =======================================================================
void OpenGl_BackgroundArray::SetGradientParameters(const Quantity_Color& theColor1, const Quantity_Color& theColor2,
...
Standard_Boolean OpenGl_BackgroundArray::createGradientArray(const Handle(OpenGl_Context) & theCtx) const {
    ...
    OpenGl_Vec2 aVertices[4] = {OpenGl_Vec2(float(myViewWidth), 0.0f),
                                ...
                                OpenGl_Vec2(0.0f, float(myViewHeight)), OpenGl_Vec2(0.0f, 0.0f)};
    ...
    switch (myGradientParams.type) {
        ...
        case Aspect_GradientFillMethod_Corner4: {
            ...
            OpenGl_Vec2 anUVs[4] = {OpenGl_Vec2(1.0f, 0.0f), OpenGl_Vec2(1.0f, 1.0f), OpenGl_Vec2(0.0f, 1.0f),
            ...
            for (Standard_Integer anIt = 0; anIt < 4; ++anIt) {
                ...
                *aVertData = aVertices[anIt];
                ...
                OpenGl_Vec2* anUvData =
                    reinterpret_cast<OpenGl_Vec2*>(myAttribs->changeValue(anIt) + myAttribs->AttributeOffset(1));
    ...
    for (Standard_Integer anIt = 0; anIt < 4; ++anIt) {
        ...
        *aVertData = aVertices[anIt];
        ...
        OpenGl_Vec3* aColorData =
            reinterpret_cast<OpenGl_Vec3*>(myAttribs->changeValue(anIt) + myAttribs->AttributeOffset(1));
        *aColorData =
            theCtx->Vec4FromQuantityColor(OpenGl_Vec4(aCorners[anIt][0], aCorners[anIt][1], aCorners[anIt][2], 1.0f))
                .rgb();
    }
        ...
        if (aCtx->Camera()->Tile().IsValid()) {
            ...
            if (myType == Graphic3d_TOB_GRADIENT) {
                ...
                aWorldView.SetColumn(
                    3, OpenGl_Vec4(-1.0f + (float)aViewSizeX / aTileSize.x() - 2.0f * aTileOffset.x() / aTileSize.x(),
