...
OpenGl_Group::OpenGl_Group(const Handle(Graphic3d_Structure) & theStruct)
    : Graphic3d_Group(theStruct), myAspects(NULL), myFirst(NULL), myLast(NULL), myIsRaytracable(Standard_False) {
...
// purpose  :
// =======================================================================
void OpenGl_Group::AddText(const Handle(Graphic3d_Text) & theTextParams, const Standard_Boolean theToEvalMinMax) {
    if (IsDeleted()) {
        return;
    }

    if (theTextParams->Height() < 2.0) {
        // TODO - this should be handled in different way (throw exception / take default text height without modifying
        // Graphic3d_Text / log warning, etc.)
        OpenGl_Structure* aStruct = GlStruct();
...
// purpose  :
// =======================================================================
void OpenGl_Group::SetFlippingOptions(const Standard_Boolean theIsEnabled, const gp_Ax2& theRefPlane) {
...
// purpose  :
// =======================================================================
bool OpenGl_Group::renderFiltered(const Handle(OpenGl_Workspace) & theWorkspace, OpenGl_Element* theElement) const {
...
void OpenGl_Group::Render(const Handle(OpenGl_Workspace) & theWorkspace) const {
    // Setup aspects
    theWorkspace->SetAllowFaceCulling(myIsClosed && !theWorkspace->GetGlContext()->Clipping().IsClippingOrCappingOn());
    ...
    // Restore aspects
    if (isAspectSet) theWorkspace->SetAspects(aBackAspects);
}
