...
template <class TheBaseClass, int NbAttributes> class OpenGl_VertexBufferT : public TheBaseClass {
public:
...
Standard_Boolean OpenGl_PrimitiveArray::initNormalVbo(const Handle(OpenGl_Context) & theCtx) const {
    ...
    if (myAttribs->NbElements != myAttribs->NbMaxElements() && myIndices.IsNull() &&
        ...
        throw Standard_ProgramError(
            "OpenGl_PrimitiveArray::buildVBO() - vertex attribute data with reserved size is not supported");
    }

    // specify data type as Byte and NbComponents as Stride, so that OpenGl_VertexBuffer::EstimatedDataSize() will
    // return correct value
    const Standard_Integer aNbVertexes =
        (isAttribMutable || !isAttribInterleaved) ? myAttribs->NbMaxElements() : myAttribs->NbElements;
    if (!myVboAttribs->init(theCtx, myAttribs->Stride, aNbVertexes, myAttribs->Data(), GL_UNSIGNED_BYTE,
                            myAttribs->Stride)) {
        ...
        if (isAttribMutable && isAttribInterleaved) {
            // for mutable interlaced array we can change dynamically number of vertexes (they will be just skipped at
            // the end of buffer); this doesn't matter in case if we have indexed array
...
Standard_Boolean OpenGl_PrimitiveArray::buildVBO(const Handle(OpenGl_Context) & theCtx,
    ...
    clearMemoryGL(theCtx);
    if (myAttribs.IsNull() || myAttribs->IsEmpty() || myAttribs->NbElements < 1 || myAttribs->NbAttributes < 1 ||
...
void OpenGl_PrimitiveArray::updateVBO(const Handle(OpenGl_Context) & theCtx) const {
    if (!myAttribs.IsNull()) {
        ...
        if (!aRange.IsEmpty() && myVboAttribs->IsValid() && !myVboAttribs->IsVirtual()) {
            ...
            theCtx->core15fwd->glBufferSubData(myVboAttribs->GetTarget(), aRange.Start, aRange.Length,
    ...
    if (!myIndices.IsNull()) {
        ...
        if (!aRange.IsEmpty() && myVboIndices->IsValid() && !myVboIndices->IsVirtual()) {
            ...
            theCtx->core15fwd->glBufferSubData(myVboIndices->GetTarget(), aRange.Start, aRange.Length,
...
void OpenGl_PrimitiveArray::drawArray(const Handle(OpenGl_Workspace) & theWorkspace,
    ...
    const GLenum aDrawMode =
        !aGlContext->ActiveProgram().IsNull() && aGlContext->ActiveProgram()->HasTessellationStage() ? GL_PATCHES
                                                                                                     : myDrawMode;
    ...
    if (!myVboIndices.IsNull()) {
        ...
        if (!myBounds.IsNull()) {
            ...
            const size_t aStride =
                myVboIndices->GetDataType() == GL_UNSIGNED_SHORT ? sizeof(unsigned short) : sizeof(unsigned int);
...
void OpenGl_PrimitiveArray::drawEdges(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    if (aGlContext->core20fwd != NULL) {
        aGlContext->ShaderManager()->BindLineProgram(Handle(OpenGl_TextureSet)(), anAspect->Aspect()->EdgeLineType(),
                                                     Graphic3d_TypeOfShadingModel_Unlit, Graphic3d_AlphaMode_Opaque,
                                                     Standard_False, anAspect->ShaderProgramRes(aGlContext));
    }
    ...
    aGlContext->SetColor4fv(theWorkspace->EdgeColor().a() >= 0.1f ? theWorkspace->EdgeColor()
    ...
    if (!myVboIndices.IsNull()) {
        ...
        if (!myBounds.IsNull()) {
            ...
            for (Standard_Integer aGroupIter = 0; aGroupIter < myBounds->NbBounds; ++aGroupIter) {
                const GLint aNbElemsInGroup = myBounds->Bounds[aGroupIter];
                aGlContext->core11fwd->glDrawElements(aDrawMode, aNbElemsInGroup, myVboIndices->GetDataType(),
                                                      anOffset);
                anOffset += aStride * aNbElemsInGroup;
            }
        }
        // draw one (or sequential) primitive by the indices
            ...
            aGlContext->core11fwd->glDrawElements(aDrawMode, myVboIndices->GetElemsNb(), myVboIndices->GetDataType(),
                                                  anOffset);
        }
        myVboIndices->Unbind(aGlContext);
    } else if (!myBounds.IsNull()) {
        ...
        aGlContext->core11fwd->glDrawArrays(
            aDrawMode, 0, !myVboAttribs.IsNull() ? myVboAttribs->GetElemsNb() : myAttribs->NbElements);
    }
...
void OpenGl_PrimitiveArray::drawMarkers(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    const GLenum aDrawMode =
        !aCtx->ActiveProgram().IsNull() && aCtx->ActiveProgram()->HasTessellationStage() ? GL_PATCHES : myDrawMode;
    if (anAspectMarker->Aspect()->MarkerType() == Aspect_TOM_POINT) {
        aCtx->SetPointSize(anAspectMarker->MarkerSize());
        aCtx->core11fwd->glDrawArrays(aDrawMode, 0,
                                      !myVboAttribs.IsNull() ? myVboAttribs->GetElemsNb() : myAttribs->NbElements);
...
OpenGl_PrimitiveArray::OpenGl_PrimitiveArray(const OpenGl_GraphicDriver* theDriver)
    ...
    : myDrawMode(DRAW_MODE_NONE), myIsFillType(Standard_False), myIsVboInit(Standard_False) {
    if (theDriver != NULL) {
...
OpenGl_PrimitiveArray::OpenGl_PrimitiveArray(const OpenGl_GraphicDriver* theDriver,
    ...
    : myIndices(theIndices), myAttribs(theAttribs), myBounds(theBounds), myDrawMode(DRAW_MODE_NONE),
      myIsFillType(Standard_False), myIsVboInit(Standard_False) {
...
// purpose  :
// =======================================================================
void OpenGl_PrimitiveArray::UpdateDrawStats(Graphic3d_FrameStatsDataTmp& theStats, bool theIsDetailed) const {
...
void OpenGl_PrimitiveArray::Render(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    if (myIsFillType) {
        ...
        if (anAspectFace->Aspect()->ToDrawEdges()) {
            ...
            if (anAspectFace->Aspect()->EdgeLineType() != Aspect_TOL_SOLID ||
                aCtx->hasGeometryStage == OpenGl_FeatureNotAvailable || aCtx->caps->usePolygonMode) {
    ...
    if (!myIsVboInit) {
        ...
        Standard_Boolean toKeepData = myDrawMode == GL_POINTS && anAspectFace->IsDisplayListSprite(aCtx);
#if defined(GL_ES_VERSION_2_0)
        processIndices(aCtx);
#endif
        buildVBO(aCtx, toKeepData);
        myIsVboInit = Standard_True;
    } else if ((!myAttribs.IsNull() && myAttribs->IsMutable()) || (!myIndices.IsNull() && myIndices->IsMutable())) {
        updateVBO(aCtx);
    }
    ...
    const bool toEnableEnvMap = !aTextureSet.IsNull() && aTextureSet == theWorkspace->EnvironmentTexture();
    if (toDrawArray) {
        ...
        switch (myDrawMode) {
            case GL_POINTS: {
                aShadingModel =
                    aCtx->ShaderManager()->ChooseMarkerShadingModel(anAspectFace->ShadingModel(), hasVertNorm);
                aCtx->ShaderManager()->BindMarkerProgram(aTextureSet, aShadingModel, Graphic3d_AlphaMode_Opaque,
            ...
            case GL_LINE_STRIP: {
                ...
                aCtx->ShaderManager()->BindLineProgram(Handle(OpenGl_TextureSet)(), anAspectFace->Aspect()->LineType(),
                                                       aShadingModel, Graphic3d_AlphaMode_Opaque, hasVertColor,
            ...
            default: {
                ...
                aCtx->ShaderManager()->BindFaceProgram(
                    aTextureSet, aShadingModel,
                    aCtx->ShaderManager()->MaterialState().HasAlphaCutoff() ? Graphic3d_AlphaMode_Mask
                                                                            : Graphic3d_AlphaMode_Opaque,
                    toDrawInteriorEdges == 1 ? anAspectFace->Aspect()->InteriorStyle() : Aspect_IS_SOLID, hasVertColor,
                    toEnableEnvMap, toDrawInteriorEdges == 1, anAspectFace->ShaderProgramRes(aCtx));
        ...
        const Graphic3d_Vec4* aFaceColors =
            !myBounds.IsNull() && !toHilight && anAspectFace->Aspect()->InteriorStyle() != Aspect_IS_HIDDENLINE
                ? myBounds->Colors
                : NULL;
        ...
        if (anAspectFace->Aspect()->ToDrawSilhouette() && aCtx->ToCullBackFaces() &&
            ...
            anOutlineProgram->SetUniform(aCtx, anOutlineProgram->GetStateLocation(OpenGl_OCCT_SILHOUETTE_THICKNESS),
                                         anEdgeWidth);
            anOutlineProgram->SetUniform(aCtx, anOutlineProgram->GetStateLocation(OpenGl_OCCT_ORTHO_SCALE),
                                         anOrthoScale);
    ...
    if (myAttribs->NbElements > std::numeric_limits<GLushort>::max()) {
        ...
        for (Standard_Integer anIdxIdx = 0; anIdxIdx < myIndices->NbElements; ++anIdxIdx) {
            ...
            memcpy(anAttribs->ChangeData() + myAttribs->Stride * anIdxIdx,
                   myAttribs->Data() + myAttribs->Stride * anIndex, myAttribs->Stride);
        }
