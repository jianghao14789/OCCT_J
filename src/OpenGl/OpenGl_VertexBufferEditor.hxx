...
template <typename theVec_t> class OpenGl_VertexBufferEditor {
...
public:
    ...
    explicit OpenGl_VertexBufferEditor(const Standard_Integer theTmpBufferLength = 0)
        : myElemFrom(0), myElemsNb(0), myTmpBuffer(0, theTmpBufferLength > 0 ? (theTmpBufferLength - 1) : 2047) {}
    ...
    OpenGl_VertexBufferEditor(theVec_t* theTmpBuffer, const Standard_Integer theTmpBufferLength)
        : myElemFrom(0), myElemsNb(0), myTmpBuffer(theTmpBuffer[0], 0, theTmpBufferLength - 1) {}
    ...
    //! @return assigned VBO
    const Handle(OpenGl_Buffer) & GetVBO() const {
        return myVbo;
    }
