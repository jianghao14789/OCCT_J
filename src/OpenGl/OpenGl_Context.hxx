...
//!
//! Functions related to specific OpenGL version or extension are grouped into structures which can be accessed as
//! fields of this class. The most simple way to check that required functionality is available - is NULL check for the
//! group:
//! @code
...
//!    For example, if algorithm requires OpenGL 2.1+, it is better to write core20fwd->glEnable() rather than
//!    core11fwd->glEnable() for uniformity.
//!  - Validate minimal requirements at initialization/creation time and omit checks within code where algorithm should
//!  be already initialized.
//!    Properly escape code incompatible with Core Profile. The simplest way to check Core Profile is "if (core11ffp ==
//!    NULL)".
//!
...
//!
//! Notice that some systems provide mechanisms to simultaneously incorporate with GL contexts with different
//! capabilities. For this reason OpenGl_Context should be initialized and used for each GL context independently.
//!
...
public:
    ...
    //!   EGLSurface theEglSurf = eglCreateWindowSurface (theEglDisp, anEglConfig, (EGLNativeWindowType
    //!   )theAspWin->NativeHandle(), NULL); EGLDisplay theEglDisp = eglGetDisplay (EGL_DEFAULT_DISPLAY); EGLContext
    //!   theEglCtx  = eglCreateContext ((EGLDisplay )theEglDisp, anEglConfig, EGL_NO_CONTEXT, anEglCtxAttribs);
    //!   Handle(OpenGl_Context) aGlCtx = new OpenGl_Context();
    //!   aGlCtx->Init ((Aspect_Drawable )theEglSurf, (Aspect_Display )theEglDisp,  (Aspect_RenderingContext
    //!   )theEglCtx);
    //! @endcode
    //!
    //! Windows (Win32):
    ...
    //! @param theContext [in] rendering context         (EGLContext | HGLRC | GLXContext | EAGLContext* |
    //! NSOpenGLContext*)
    ...
    Standard_EXPORT Standard_Boolean Init(const Aspect_Drawable theSurface, const Aspect_Display theDisplay,
    ...
    Standard_Boolean Init(EAGLContext* theGContext, const Standard_Boolean theIsCoreProfile = Standard_False) {
    ...
    Standard_Boolean Init(NSOpenGLContext* theGContext, const Standard_Boolean theIsCoreProfile = Standard_False) {
        return Init((Aspect_Drawable)0, (Aspect_Display)0, (Aspect_RenderingContext)theGContext, theIsCoreProfile);
    }
#endif
#endif
    ...
    Standard_EXPORT static void ReadGlVersion(Standard_Integer& theGlVerMajor, Standard_Integer& theGlVerMinor);
    ...
    Standard_EXPORT static Standard_Boolean CheckExtension(const char* theExtString, const char* theExtName);
    ...
    Standard_Boolean FindProcVerbose(const char*& theLastFailFuncName, const char* theFuncName,
    ...
    template <typename FuncType_t> Standard_Boolean FindProc(const char* theFuncName, FuncType_t& theFuncPtr) {
        theFuncPtr = (FuncType_t)findProc(theFuncName);
    ...
    inline Standard_Boolean IsGlGreaterEqual(const Standard_Integer theVerMajor,
        ...
        return (myGlVerMajor > theVerMajor) || (myGlVerMajor == theVerMajor && myGlVerMinor >= theVerMinor);
    }
    ...
    template <class T> void DelayedRelease(Handle(T) & theResource) {
    ...
    inline OpenGl_Clipping& ChangeClipping() {
        return myClippingState;
    }

    //! @return tool for management of clippings within this context.
    inline const OpenGl_Clipping& Clipping() const {
        return myClippingState;
    }

    //! @return tool for management of shader programs within this context.
    inline const Handle(OpenGl_ShaderManager) & ShaderManager() const {
...
public:
    ...
    OpenGl_Vec4 Vec4FromQuantityColor(const OpenGl_Vec4& theColor) const {
        return myIsSRgbActive ? Vec4LinearFromQuantityColor(theColor) : Vec4sRGBFromQuantityColor(theColor);
    }
    ...
    Graphic3d_TextureUnit PBREnvLUTTexUnit() const {
        return myPBREnvLUTTexUnit;
    }

    //! Returns texture unit where Diffuse (irradiance) IBL map's spherical harmonics coefficients is expected to be
    //! bound, or 0 if PBR is unavailable.
    Graphic3d_TextureUnit PBRDiffIBLMapSHTexUnit() const {
        return myPBRDiffIBLMapSHTexUnit;
    }
    ...
    Graphic3d_TextureUnit DepthPeelingDepthTexUnit() const {
        return myDepthPeelingDepthTexUnit;
    }
    ...
    inline bool ToUseVbo() const {
        return core15fwd != NULL && !caps->vboDisable;
    }
...
public:
    ...
    Standard_EXPORT void PushMessage(const unsigned int theSource, const unsigned int theType, const unsigned int theId,
                                     const unsigned int theSeverity, const TCollection_ExtendedString& theMessage);

    //! Adds a filter for messages with theId and theSource (GL_DEBUG_SOURCE_)
    Standard_EXPORT Standard_Boolean ExcludeMessage(const unsigned int theSource, const unsigned int theId);

    //! Removes a filter for messages with theId and theSource (GL_DEBUG_SOURCE_)
    Standard_EXPORT Standard_Boolean IncludeMessage(const unsigned int theSource, const unsigned int theId);
...
public: //! @name methods to alter or retrieve current state
    ...
    Standard_Integer DrawBuffer(Standard_Integer theIndex = 0) const {
        return theIndex >= myDrawBuffers.Lower() && theIndex <= myDrawBuffers.Upper() ? myDrawBuffers.Value(theIndex)
                                                                                      : GL_NONE;
    }
    ...
    //! - TRUE when writing into sRGB-ready window buffer (might require choosing proper pixel format on window
    //! creation).
    //! - FALSE if sRGB rendering is not supported or sRGB-not-ready window buffer is used for drawing.
    //! @param theIsFbo [in] flag indicating writing into offscreen FBO (always expected sRGB-ready when sRGB FBO is
    //! supported)
    ...
    Standard_DEPRECATED("BindTextures() with explicit GLSL program should be used instead") Handle(OpenGl_TextureSet)
        BindTextures(const Handle(OpenGl_TextureSet) & theTextures) {
    ...
    //!                         mock textures will be bound to texture units expected by GLSL program, but undefined by
    //!                         texture set
    //! @return previous texture set
    Standard_EXPORT Handle(OpenGl_TextureSet)
        BindTextures(const Handle(OpenGl_TextureSet) & theTextures, const Handle(OpenGl_ShaderProgram) & theProgram);
    ...
    CheckIsTransparent(const OpenGl_Aspects* theAspect, const Handle(Graphic3d_PresentationAttributes) & theHighlight,
                       Standard_ShortReal& theAlphaFront, Standard_ShortReal& theAlphaBack);
    ...
    //! Setup type of line.
    Standard_EXPORT void SetTypeOfLine(const Aspect_TypeOfLine theType, const Standard_ShortReal theFactor = 1.0f);

    //! Setup stipple line pattern with 1.0f factor; wrapper for glLineStipple().
    void SetLineStipple(const uint16_t thePattern) {
        SetLineStipple(1.0f, thePattern);
    }

    //! Setup type of line; wrapper for glLineStipple().
    Standard_EXPORT void SetLineStipple(const Standard_ShortReal theFactor, const uint16_t thePattern);
...
public:                                //! @name extensions
    ...
    Standard_Boolean
        hasPackRowLength; //!< supporting of GL_PACK_ROW_LENGTH   parameters (any desktop OpenGL; OpenGL ES 3.0)
    Standard_Boolean
        hasUnpackRowLength;    //!< supporting of GL_UNPACK_ROW_LENGTH parameters (any desktop OpenGL; OpenGL ES 3.0)
    Standard_Boolean hasHighp; //!< highp in GLSL ES fragment shader is supported
    Standard_Boolean hasUintIndex; //!< GLuint for index buffer is supported (always available on desktop; on OpenGL ES
                                   //!< - since 3.0 or as extension GL_OES_element_index_uint)
    Standard_Boolean
        hasTexRGBA8; //!< always available on desktop; on OpenGL ES - since 3.0 or as extension GL_OES_rgb8_rgba8
    Standard_Boolean hasTexFloatLinear; //!< texture-filterable state for 32-bit floating texture formats (always on
                                        //!< desktop, GL_OES_texture_float_linear within OpenGL ES)
    Standard_Boolean
        hasTexSRGB; //!< sRGB texture    formats (desktop OpenGL 2.1, OpenGL ES 3.0 or OpenGL ES 2.0 + GL_EXT_sRGB)
    Standard_Boolean hasFboSRGB; //!< sRGB FBO render targets (desktop OpenGL 2.1, OpenGL ES 3.0)
    Standard_Boolean
        hasSRGBControl; //!< sRGB write control (any desktop OpenGL, OpenGL ES + GL_EXT_sRGB_write_control extension)
    ...
    OpenGl_FeatureFlag hasFlatShading;   //!< Complex flag indicating support of Flat shading
                                       //!< (Graphic3d_TypeOfShadingModel_Phong) (always available on desktop; on OpenGL
                                       //!< ES - since 3.0 or as extension GL_OES_standard_derivatives)
    OpenGl_FeatureFlag hasGlslBitwiseOps; //!< GLSL supports bitwise operations; OpenGL 3.0 / OpenGL ES 3.0 (GLSL 130 /
                                          //!< GLSL ES 300) or OpenGL 2.1 + GL_EXT_gpu_shader4
    OpenGl_FeatureFlag hasDrawBuffers;    //!< Complex flag indicating support of multiple draw buffers (desktop
                                          //!< OpenGL 2.0, OpenGL ES 3.0, GL_ARB_draw_buffers, GL_EXT_draw_buffers)
    OpenGl_FeatureFlag hasFloatBuffer;    //!< Complex flag indicating support of float color buffer format (desktop
                                          //!< OpenGL 3.0, GL_ARB_color_buffer_float, GL_EXT_color_buffer_float)
    OpenGl_FeatureFlag
        hasHalfFloatBuffer; //!< Complex flag indicating support of half-float color buffer format (desktop OpenGL 3.0,
                            //!< GL_ARB_color_buffer_float, GL_EXT_color_buffer_half_float)
    OpenGl_FeatureFlag hasSampleVariables; //!< Complex flag indicating support of MSAA variables in GLSL shader
                                           //!< (desktop OpenGL 4.0, GL_ARB_sample_shading)
    OpenGl_FeatureFlag hasGeometryStage;   //!< Complex flag indicating support of Geometry shader (desktop OpenGL 3.2,
                                           //!< OpenGL ES 3.2, GL_EXT_geometry_shader)
    Standard_Boolean arbDrawBuffers;       //!< GL_ARB_draw_buffers
    Standard_Boolean arbNPTW;              //!< GL_ARB_texture_non_power_of_two
    Standard_Boolean arbTexRG;             //!< GL_ARB_texture_rg
    Standard_Boolean
        arbTexFloat; //!< GL_ARB_texture_float (on desktop OpenGL - since 3.0 or as extension GL_ARB_texture_float; on
                     //!< OpenGL ES - since 3.0); @sa hasTexFloatLinear for linear filtering support
    OpenGl_ArbSamplerObject* arbSamplerObject; //!< GL_ARB_sampler_objects (on desktop OpenGL - since 3.3 or as
                                               //!< extension GL_ARB_sampler_objects; on OpenGL ES - since 3.0)
    OpenGl_ArbTexBindless* arbTexBindless;     //!< GL_ARB_bindless_texture
    OpenGl_ArbTBO* arbTBO;           //!< GL_ARB_texture_buffer_object (on desktop OpenGL - since 3.1 or as extension
                                     //!< GL_ARB_texture_buffer_object; on OpenGL ES - since 3.2)
    Standard_Boolean arbTboRGB32;    //!< GL_ARB_texture_buffer_object_rgb32 (3-component TBO), in core since 4.0 (on
                                     //!< OpenGL ES - since 3.2)
    Standard_Boolean arbClipControl; //!< GL_ARB_clip_control, in core since 4.5
    OpenGl_ArbIns*
        arbIns; //!< GL_ARB_draw_instanced (on desktop OpenGL - since 3.1 or as extension GL_ARB_draw_instanced; on
                //!< OpenGL ES - since 3.0 or as extension GL_ANGLE_instanced_arrays to WebGL 1.0)
    OpenGl_ArbDbg* arbDbg; //!< GL_ARB_debug_output (on desktop OpenGL - since 4.3 or as extension GL_ARB_debug_output;
                           //!< on OpenGL ES - since 3.2 or as extension GL_KHR_debug)
    OpenGl_ArbFBO* arbFBO; //!< GL_ARB_framebuffer_object
    OpenGl_ArbFBOBlit* arbFBOBlit;     //!< glBlitFramebuffer function, moved out from OpenGl_ArbFBO structure for
                                       //!< compatibility with OpenGL ES 2.0
    Standard_Boolean arbSampleShading; //!< GL_ARB_sample_shading
    Standard_Boolean arbDepthClamp;    //!< GL_ARB_depth_clamp (on desktop OpenGL - since 3.2 or as extensions
                                       //!< GL_ARB_depth_clamp,NV_depth_clamp; unavailable on OpenGL ES)
    Standard_Boolean
        extFragDepth; //!< GL_EXT_frag_depth on OpenGL ES 2.0 (gl_FragDepthEXT built-in variable, before OpenGL ES 3.0)
...
private:                      // system-dependent fields
    ...
    Aspect_RenderingContext
        myGContext; //!< rendering context EGLContext | HGLRC | GLXContext | EAGLContext* | NSOpenGLContext*
...
private: // context info
    ...
    NCollection_Handle<OpenGl_GlFunctions> myFuncs;    //!< mega structure for all GL functions
    ...
    Standard_Boolean
        myHasRayTracingAdaptiveSampling; //! indicates whether adaptive screen sampling in ray tracing mode is supported
    Standard_Boolean myHasRayTracingAdaptiveSamplingAtomic; //! indicates whether atomic adaptive screen sampling in ray
                                                            //! tracing mode is supported

    Standard_Boolean myHasPBR;                //!< indicates whether PBR shading model is supported
    Graphic3d_TextureUnit myPBREnvLUTTexUnit; //!< sampler2D occEnvLUT, texture unit where environment lookup table is
                                              //!< expected to be binded (0 if PBR is not supported)
    Graphic3d_TextureUnit
        myPBRDiffIBLMapSHTexUnit; //!< sampler2D occDiffIBLMapSHCoeffs, texture unit where diffuse (irradiance) IBL
                                  //!< map's spherical harmonics coefficients is expected to  be binded
                                  //!  (0 if PBR is not supported)
    Graphic3d_TextureUnit myPBRSpecIBLMapTexUnit; //!< samplerCube occSpecIBLMap, texture unit where specular IBL map is
                                                  //!< expected to  be binded (0 if PBR is not supported)
    Graphic3d_TextureUnit myShadowMapTexUnit;     //!< sampler2D occShadowMapSampler

    Graphic3d_TextureUnit
        myDepthPeelingDepthTexUnit; //!< sampler2D occDepthPeelingDepth, texture unit for Depth Peeling lookups
    Graphic3d_TextureUnit myDepthPeelingFrontColorTexUnit; //!< sampler2D occDepthPeelingFrontColor, texture unit for
                                                           //!< Depth Peeling lookups
...
private:                                          //! @name fields tracking current state
    ...
    NCollection_Array1<Standard_Integer> myDrawBuffers; //!< current draw buffers
    unsigned int myDefaultVao;                          //!< default Vertex Array Object
    NCollection_Vec4<bool>
        myColorMask; //!< flag indicating writing into color buffer is enabled or disabled (glColorMask)
    ...
    TColStd_PackedMapOfInteger myFilters[6]; //!< messages suppressing filter (for sources from GL_DEBUG_SOURCE_API_ARB
                                             //!< to GL_DEBUG_SOURCE_OTHER_ARB)
