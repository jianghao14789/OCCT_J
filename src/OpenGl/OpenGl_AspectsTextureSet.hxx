...
public:
    ...
    const Handle(OpenGl_TextureSet) & TextureSet(const Handle(OpenGl_Context) & theCtx,
                                                 ...
                                                 const Handle(OpenGl_PointSprite) & theSpriteA, bool theToHighlight) {
        if (!myIsTextureReady) {
...
private:
    ...
    Standard_EXPORT void build(const Handle(OpenGl_Context) & theCtx, const Handle(Graphic3d_Aspects) & theAspect,
                               const Handle(OpenGl_PointSprite) & theSprite,
