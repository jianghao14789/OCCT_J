...
public:
    ...
    Graphic3d_Vec2i TileSize() const {
        return Graphic3d_Vec2i(myTileSize, myTileSize);
    }
    ...
    int NbTilesX() const {
        return (int)myTiles.SizeX;
    }

    //! Returns number of tiles in Y dimension.
    int NbTilesY() const {
        return (int)myTiles.SizeY;
    }

    //! Returns total number of tiles in viewport.
    int NbTiles() const {
        return int(myTiles.SizeX * myTiles.SizeY);
    }
    ...
    Standard_EXPORT void SetSize(const Graphic3d_RenderingParams& theParams, const Graphic3d_Vec2i& theSize);
    ...
    bool UploadSamples(const Handle(OpenGl_Context) & theContext, const Handle(OpenGl_Texture) & theSamplesTexture,
    ...
    bool UploadOffsets(const Handle(OpenGl_Context) & theContext, const Handle(OpenGl_Texture) & theOffsetsTexture,
                       const bool theAdaptive) {
...
protected:
    ...
    Standard_EXPORT void dumpMap(std::ostream& theStream, const Image_PixMapTypedData<int>& theMap,
                                 const char* theTitle) const;

protected:
    Image_PixMapTypedData<unsigned int> myTiles; //!< number of samples per tile (initially all 1)
    Image_PixMapTypedData<unsigned int>
        myTileSamples; //!< number of samples for all pixels within the tile (initially equals to Tile area)
