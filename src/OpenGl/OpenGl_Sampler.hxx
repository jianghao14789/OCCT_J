...
public:
    ...
    Standard_EXPORT Standard_Boolean Init(const Handle(OpenGl_Context) & theContext, const OpenGl_Texture& theTexture);
    ...
    Standard_EXPORT void Bind(const Handle(OpenGl_Context) & theCtx, const Graphic3d_TextureUnit theUnit);

    //! Unbinds sampler object from the given texture unit.
    Standard_EXPORT void Unbind(const Handle(OpenGl_Context) & theCtx, const Graphic3d_TextureUnit theUnit);

    //! Sets specific sampler parameter.
    void SetParameter(const Handle(OpenGl_Context) & theCtx, unsigned int theTarget, unsigned int theParam,
    ...
    bool ToUpdateParameters() const {
        return mySamplerRevision != myParams->SamplerRevision();
    }
...
protected:
    ...
    //! Sets specific sampler parameter.
    Standard_EXPORT static void setParameter(const Handle(OpenGl_Context) & theContext, OpenGl_Sampler* theSampler,
    ...
    Standard_EXPORT static void applySamplerParams(const Handle(OpenGl_Context) & theCtx,
                                                   ...
                                                   OpenGl_Sampler* theSampler, const unsigned int theTarget,
