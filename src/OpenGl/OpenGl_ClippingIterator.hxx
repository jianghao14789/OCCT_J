...
public:
    ...
    OpenGl_ClippingIterator(const OpenGl_Clipping& theClipping)
        : myDisabled(&theClipping.myDisabledPlanes), myCurrIndex(1) {
    ...
    bool More() const {
        return myIter1.More() || myIter2.More();
    }
    ...
    //! Return true if plane has been temporarily disabled either by Graphic3d_ClipPlane->IsOn() property or by
    //! temporary filter. Beware that this method does NOT handle a Chain filter for Capping algorithm
    //! OpenGl_Clipping::CappedChain()!
    bool IsDisabled() const {
        return myDisabled->Value(myCurrIndex) || !Value()->IsOn();
    }
