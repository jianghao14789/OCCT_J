...
public:
    ...
    Standard_EXPORT void Unregister(TCollection_AsciiString& theShareKey, Handle(OpenGl_ShaderProgram) & theProgram);
    ...
    Standard_Boolean BindFaceProgram(const Handle(OpenGl_TextureSet) & theTextures,
                                     ...
                                     Standard_Boolean theHasVertColor, Standard_Boolean theEnableEnvMap,
                                     const Handle(OpenGl_ShaderProgram) & theCustomProgram) {
        return BindFaceProgram(theTextures, theShadingModel, theAlphaMode, Aspect_IS_SOLID, theHasVertColor,
                               theEnableEnvMap, false, theCustomProgram);
    }

    //! Bind program for filled primitives rendering
    Standard_Boolean BindFaceProgram(const Handle(OpenGl_TextureSet) & theTextures,
                                     Graphic3d_TypeOfShadingModel theShadingModel, Graphic3d_AlphaMode theAlphaMode,
                                     Aspect_InteriorStyle theInteriorStyle, Standard_Boolean theHasVertColor,
                                     Standard_Boolean theEnableEnvMap, Standard_Boolean theEnableMeshEdges,
                                     const Handle(OpenGl_ShaderProgram) & theCustomProgram) {
        const Graphic3d_TypeOfShadingModel aShadeModelOnFace =
            theShadingModel != Graphic3d_TypeOfShadingModel_Unlit && (theTextures.IsNull() || theTextures->IsModulate())
                ? theShadingModel
        ...
        const Standard_Integer aBits = getProgramBits(theTextures, theAlphaMode, theInteriorStyle, theHasVertColor,
                                                      theEnableEnvMap, theEnableMeshEdges);
    ...
    Standard_Boolean BindLineProgram(const Handle(OpenGl_TextureSet) & theTextures, const Aspect_TypeOfLine theLineType,
                                     const Graphic3d_TypeOfShadingModel theShadingModel,
                                     const Graphic3d_AlphaMode theAlphaMode, const Standard_Boolean theHasVertColor,
                                     const Handle(OpenGl_ShaderProgram) & theCustomProgram) {
        if (!theCustomProgram.IsNull() || myContext->caps->ffpEnable) {
    ...
    Standard_Boolean BindOutlineProgram() {
        ...
        const Standard_Integer aBits = getProgramBits(Handle(OpenGl_TextureSet)(), Graphic3d_AlphaMode_Opaque,
                                                      Aspect_IS_SOLID, false, false, false);
...
public:
    ...
    void UpdateMaterialStateTo(const OpenGl_Material& theMat, const float theAlphaCutoff, const bool theToDistinguish,
...
public:
    ...
    PushState(const Handle(OpenGl_ShaderProgram) & theProgram,
...
public:
    ...
    Graphic3d_TypeOfShadingModel ChooseFaceShadingModel(Graphic3d_TypeOfShadingModel theCustomModel,
        ...
        switch (aModel) {
            ...
            case Graphic3d_TypeOfShadingModel_Pbr:
                return PBRShadingModelFallback(theHasNodalNormals ? aModel : Graphic3d_TypeOfShadingModel_PbrFacet,
                                               IsPbrAllowed());
            case Graphic3d_TypeOfShadingModel_PbrFacet:
    ...
    Graphic3d_TypeOfShadingModel ChooseLineShadingModel(Graphic3d_TypeOfShadingModel theCustomModel,
        ...
        switch (aModel) {
            ...
            case Graphic3d_TypeOfShadingModel_Pbr:
                return PBRShadingModelFallback(theHasNodalNormals ? aModel : Graphic3d_TypeOfShadingModel_Unlit,
                                               IsPbrAllowed());
            case Graphic3d_TypeOfShadingModel_PbrFacet:
                return Graphic3d_TypeOfShadingModel_Unlit;
        }
...
protected:
    ...
    //! Define program bits.
    Standard_Integer getProgramBits(const Handle(OpenGl_TextureSet) & theTextures, Graphic3d_AlphaMode theAlphaMode,
                                    Aspect_InteriorStyle theInteriorStyle, Standard_Boolean theHasVertColor,
                                    Standard_Boolean theEnableEnvMap, Standard_Boolean theEnableMeshEdges) const {
        Standard_Integer aBits = 0;
    ...
    Handle(OpenGl_ShaderProgram) &
        getStdProgram(Graphic3d_TypeOfShadingModel theShadingModel, Standard_Integer theBits) {
    ...
    Standard_Boolean prepareStdProgramLight(Handle(OpenGl_ShaderProgram) & theProgram,
                                            Graphic3d_TypeOfShadingModel theShadingModel, Standard_Integer theBits) {
        switch (theShadingModel) {
            case Graphic3d_TypeOfShadingModel_Unlit:
...
protected:
    ...
    void addClippingPlane(Standard_Integer& thePlaneId, const Graphic3d_ClipPlane& thePlane,
                          const Graphic3d_Vec4d& theEq, const Standard_Integer theChainFwd) const {
...
protected:
    ...
    NCollection_Array1<Handle(OpenGl_ShaderProgram)> myBlitPrograms[2]; //!< standard program for FBO blit emulation
    Handle(OpenGl_ShaderProgram) myBoundBoxProgram;                     //!< standard program for bounding box
    Handle(OpenGl_ShaderProgram)
        myOitCompositingProgram[2]; //!< standard program for OIT compositing (default and MSAA).
    Handle(OpenGl_ShaderProgram)
        myOitDepthPeelingBlendProgram[2]; //!< standard program for OIT Depth Peeling blend (default and MSAA)
    Handle(OpenGl_ShaderProgram)
        myOitDepthPeelingFlushProgram[2]; //!< standard program for OIT Depth Peeling flush (default and MSAA)
    OpenGl_MapOfShaderPrograms myMapOfLightPrograms; //!< map of lighting programs depending on lights configuration

    Handle(OpenGl_ShaderProgram) myPBREnvBakingProgram[3]; //!< programs for IBL maps generation used in PBR pipeline (0
                                                           //!< for Diffuse; 1 for Specular; 2 for fallback)
