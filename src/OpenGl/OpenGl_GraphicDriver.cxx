...
static int TheDoubleBuffVisual[] = {GLX_RGBA, GLX_DEPTH_SIZE, 16, GLX_STENCIL_SIZE, 1, GLX_DOUBLEBUFFER, None};
...
OpenGl_GraphicDriver::OpenGl_GraphicDriver(const Handle(Aspect_DisplayConnection) & theDisp,
    ...
    : Graphic3d_GraphicDriver(theDisp), myIsOwnContext(Standard_False), myEglDisplay(NULL), myEglContext(NULL),
      myEglConfig(NULL), myCaps(new OpenGl_Caps()), myMapOfView(1, NCollection_BaseAllocator::CommonBaseAllocator()),
...
Standard_Boolean OpenGl_GraphicDriver::InitContext() {
    ...
    if (myCaps->contextMajorVersionUpper != 2) {
        myEglContext = (Aspect_RenderingContext)eglCreateContext((EGLDisplay)myEglDisplay, myEglConfig, EGL_NO_CONTEXT,
                                                                 anEglCtxAttribs3);
    }
    ...
    // if (eglMakeCurrent ((EGLDisplay )myEglDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, (EGLContext )myEglContext) !=
    // EGL_TRUE)
    //{
...
Standard_Boolean OpenGl_GraphicDriver::InitEglContext(Aspect_Display theEglDisplay,
                                                      Aspect_RenderingContext theEglContext, void* theEglConfig) {
    ReleaseContext();
#if defined(HAVE_EGL)
#if defined(HAVE_XLIB)
    if (myDisplayConnection.IsNull()) {
...
void OpenGl_GraphicDriver::chooseVisualInfo() {
    ...
    if (myEglDisplay != EGL_NO_DISPLAY && myEglConfig != NULL &&
        eglGetConfigAttrib((EGLDisplay)myEglDisplay, myEglConfig, EGL_NATIVE_VISUAL_ID,
                           (EGLint*)&aVisInfoTmp.visualid) == EGL_TRUE) {
        int aNbVisuals = 0;
    ...
    int aDummy = 0;
    if (!XQueryExtension(aDisp, "GLX", &aDummy, &aDummy, &aDummy) || !glXQueryExtension(aDisp, &aDummy, &aDummy)) {
    ...
    const bool hasFBCfg =
        glXQueryVersion(aDisp, &aGlxMajor, &aGlxMinor) && ((aGlxMajor == 1 && aGlxMinor >= 3) || (aGlxMajor > 1));
    if (hasFBCfg) {
        int aFBCount = 0;
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_GraphicDriver::MemoryInfo(Standard_Size& theFreeBytes, TCollection_AsciiString& theInfo) const {
...
// purpose  :
// =======================================================================
void OpenGl_GraphicDriver::TextSize(const Handle(Graphic3d_CView) & theView, const Standard_CString theText,
                                    const Standard_ShortReal theHeight, Standard_ShortReal& theWidth,
                                    Standard_ShortReal& theAscent, Standard_ShortReal& theDescent) const {
    const Handle(OpenGl_Context) & aCtx = GetSharedContext();
    if (aCtx.IsNull()) {
        return;
    }
    ...
    OpenGl_Text::StringSize(aCtx, aText, aTextAspect, aHeight, theView->RenderingParams().Resolution,
                            theView->RenderingParams().FontHinting, theWidth, theAscent, theDescent);
}
...
void OpenGl_GraphicDriver::RemoveZLayer(const Graphic3d_ZLayerId theLayerId) {
    ...
    for (NCollection_DataMap<Standard_Integer, OpenGl_Structure*>::Iterator aStructIt(myMapOfStructure);
         aStructIt.More(); aStructIt.Next()) {
        OpenGl_Structure* aStruct = aStructIt.ChangeValue();
        if (aStruct->ZLayer() == theLayerId) aStruct->SetZLayer(Graphic3d_ZLayerId_Default);
    }
}
...
// purpose  :
// =======================================================================
Handle(Graphic3d_CStructure) OpenGl_GraphicDriver::CreateStructure(const Handle(Graphic3d_StructureManager) &
                                                                   theManager) {
