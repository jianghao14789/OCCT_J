...
OpenGl_RaytraceMaterial::OpenGl_RaytraceMaterial()
    : Ambient(ZERO_VEC_4F), Diffuse(ZERO_VEC_4F), Specular(ZERO_VEC_4F), Emission(ZERO_VEC_4F), Reflection(ZERO_VEC_4F),
      Refraction(ZERO_VEC_4F), Transparency(ZERO_VEC_4F) {}

// =======================================================================
// function : OpenGl_RaytraceLight
// purpose  : Creates new light source
// =======================================================================
OpenGl_RaytraceLight::OpenGl_RaytraceLight(const BVH_Vec4f& theEmission, const BVH_Vec4f& thePosition)
    : Emission(theEmission), Position(thePosition) {
    //
}
...
// =======================================================================
Standard_ShortReal OpenGl_TriangleSet::Center(const Standard_Integer theIndex, const Standard_Integer theAxis) const {
    ...
    const Standard_ShortReal aVertex0 = BVH::VecComp<Standard_ShortReal, 3>::Get(Vertices[aTriangle.x()], theAxis);
    const Standard_ShortReal aVertex1 = BVH::VecComp<Standard_ShortReal, 3>::Get(Vertices[aTriangle.y()], theAxis);
    const Standard_ShortReal aVertex2 = BVH::VecComp<Standard_ShortReal, 3>::Get(Vertices[aTriangle.z()], theAxis);
    ...
    for (Standard_Integer aX = 0; aX <= 1; ++aX) {
        for (Standard_Integer aY = 0; aY <= 1; ++aY) {
            for (Standard_Integer aZ = 0; aZ <= 1; ++aZ) {
                BVH_Vec4f aCorner =
                    aTransform->Transform() * BVH_Vec4f(aX == 0 ? aBox.CornerMin().x() : aBox.CornerMax().x(),
                                                        aY == 0 ? aBox.CornerMin().y() : aBox.CornerMax().y(),
                                                        aZ == 0 ? aBox.CornerMin().z() : aBox.CornerMax().z(), 1.f);
...
OpenGl_TriangleSet::OpenGl_TriangleSet(const Standard_Size theArrayID,
    ...
    : BVH_Triangulation<Standard_ShortReal, 3>(theBuilder), myArrayID(theArrayID) {
    //
}
...
void OpenGl_RaytraceGeometry::Clear() {
    ...
    std::vector<OpenGl_RaytraceLight, NCollection_StdAllocator<OpenGl_RaytraceLight>> anEmptySources;

    Sources.swap(anEmptySources);

    std::vector<OpenGl_RaytraceMaterial, NCollection_StdAllocator<OpenGl_RaytraceMaterial>> anEmptyMaterials;
...
struct OpenGL_BVHParallelBuilder {
    ...
    OpenGL_BVHParallelBuilder(BVH_ObjectSet<Standard_ShortReal, 3>* theSet) : Set(theSet) {
        //
    }
    ...
    void operator()(const Standard_Integer theObjectIdx) const {
        ...
        if (aTriangleSet != NULL) aTriangleSet->QuadBVH();
    }
};
...
Standard_Boolean OpenGl_RaytraceGeometry::ProcessAcceleration() {
    ...
    Standard_ASSERT_RETURN(!aBVH.IsNull(), "Error! Failed to update high-level BVH of ray-tracing scene",
                           Standard_False);
    ...
    for (Standard_Integer aNodeIdx = 0; aNodeIdx < aBVH->Length(); ++aNodeIdx) {
        if (aBVH->IsOuter(aNodeIdx)) {
            ...
            aBVH->NodeInfoBuffer()[aNodeIdx] = BVH_Vec4i(anObjectIdx + 1, // to keep leaf flag
                                                         aBvhNodesOffset, aVerticesOffset, aElementsOffset);
...
Standard_Integer OpenGl_RaytraceGeometry::AccelerationOffset(Standard_Integer theNodeIdx) {
    ...
    if (theNodeIdx >= aBVH->Length() || !aBVH->IsOuter(theNodeIdx)) return INVALID_OFFSET;
...
OpenGl_TriangleSet* OpenGl_RaytraceGeometry::TriangleSet(Standard_Integer theNodeIdx) {
    ...
    if (theNodeIdx >= aBVH->Length() || !aBVH->IsOuter(theNodeIdx)) return NULL;

    if (aBVH->NodeInfoBuffer().at(theNodeIdx).x() > myObjects.Size()) return NULL;

    return dynamic_cast<OpenGl_TriangleSet*>(myObjects(aBVH->NodeInfoBuffer().at(theNodeIdx).x() - 1).get());
}
...
Standard_Boolean OpenGl_RaytraceGeometry::AcquireTextures(const Handle(OpenGl_Context) & theContext) {
    ...
    for (NCollection_Vector<Handle(OpenGl_Texture)>::Iterator aTexSrcIter(myTextures); aTexSrcIter.More();
        ...
        if (!aTexture->Sampler()->IsValid() || !aTexture->Sampler()->IsImmutable()) {
            ...
            aHandle = theContext->arbTexBindless->glGetTextureSamplerHandleARB(aTexture->TextureId(),
