...
static void renderCappingForStructure(StencilTestSentry& theStencilSentry,
    ...
    const Standard_Integer anAnyFilter =
        aPrevFilter & ~(Standard_Integer)(OpenGl_RenderFilter_OpaqueOnly | OpenGl_RenderFilter_TransparentOnly);
    ...
    if (theStructure.InstancedStructure() != NULL) {
        renderCappingForStructure(theStencilSentry, theWorkspace, *theStructure.InstancedStructure(), theClipChain,
                                  theSubPlaneIndex, thePlane);
    }
}
} // namespace
...
void OpenGl_CappingAlgo::RenderCapping(const Handle(OpenGl_Workspace) & theWorkspace,
    ...
    for (OpenGl_ClippingIterator aCappingIt(aContext->Clipping()); aCappingIt.More(); aCappingIt.Next()) {
        ...
        for (const Graphic3d_ClipPlane* aSubPlaneIter = aClipChain.get(); aSubPlaneIter != NULL;
            ...
            renderCappingForStructure(aStencilSentry, theWorkspace, theStructure, aClipChain, aSubPlaneIndex,
                                      aPlaneRes);
