...
public:
    ...
    OpenGl_TextureSet(Standard_Integer theNbTextures)
        : myTextures(0, theNbTextures - 1), myTextureSetBits(Graphic3d_TextureSetBits_NONE) {}
    ...
    //! Return texture units declared within the program, @sa Graphic3d_TextureSetBits.
    Standard_Integer& ChangeTextureSetBits() {
        return myTextureSetBits;
    }
    ...
    Standard_Integer Lower() const {
        return myTextures.Lower();
    }

    //! Return the upper index in texture set.
    Standard_Integer Upper() const {
        return myTextures.Upper();
    }

    //! Return the first texture.
    const Handle(OpenGl_Texture) & First() const {
        return myTextures.First().Texture;
    }

    //! Return the first texture.
    Handle(OpenGl_Texture) & ChangeFirst() {
        return myTextures.ChangeFirst().Texture;
    }

    //! Return the first texture unit.
    Graphic3d_TextureUnit FirstUnit() const {
        return myTextures.First().Unit;
    }

    //! Return the last texture.
    const Handle(OpenGl_Texture) & Last() const {
        return myTextures.Last().Texture;
    }

    //! Return the last texture.
    Handle(OpenGl_Texture) & ChangeLast() {
        return myTextures.ChangeLast().Texture;
    }

    //! Return the last texture unit.
    Graphic3d_TextureUnit LastUnit() const {
        return myTextures.Last().Unit;
    }

    //! Return the last texture unit.
    Graphic3d_TextureUnit& ChangeLastUnit() {
        return myTextures.ChangeLast().Unit;
    }
