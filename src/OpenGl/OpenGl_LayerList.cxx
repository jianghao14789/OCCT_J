...
public:
    ...
    OpenGl_IndexedLayerIterator(const NCollection_List<Handle(Graphic3d_Layer)>& theSeq)
        : NCollection_List<Handle(Graphic3d_Layer)>::Iterator(theSeq), myIndex(1) {}
...
public:
    ...
    OpenGl_FilteredIndexedLayerIterator(const NCollection_List<Handle(Graphic3d_Layer)>& theSeq,
                                        Standard_Boolean theToDrawImmediate, OpenGl_LayerFilter theLayersToProcess)
        : myIter(theSeq), myLayersToProcess(theLayersToProcess), myToDrawImmediate(theToDrawImmediate) {
        next();
    }
    ...
    const OpenGl_Layer& Value() const {
        return *myIter.Value();
    }
        ...
        for (; myIter.More(); myIter.Next()) {
            ...
            switch (myLayersToProcess) {
                ...
                case OpenGl_LF_Bottom: {
                    if (aLayer->LayerId() == Graphic3d_ZLayerId_BotOSD && !aLayer->LayerSettings().IsRaytracable()) {
                        return;
                    }
                    break;
                }
...
static const Standard_Integer THE_DRAW_BUFFERS012[] = {GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT0 + 1,
...
OpenGl_LayerList::OpenGl_LayerList(const Standard_Integer theNbPriorities)
    : myBVHBuilder(
          new BVH_LinearBuilder<Standard_Real, 3>(BVH_Constants_LeafNodeSizeSingle, BVH_Constants_MaxTreeDepth)),
      myNbPriorities(theNbPriorities), myNbStructures(0), myImmediateNbStructures(0), myModifStateOfRaytraceable(0) {
    //
}
...
void OpenGl_LayerList::AddStructure(const OpenGl_Structure* theStruct, const Graphic3d_ZLayerId theLayerId,
                                    const Standard_Integer thePriority, Standard_Boolean isForChangePriority) {
...
void OpenGl_LayerList::ChangeLayer(const OpenGl_Structure* theStructure, const Graphic3d_ZLayerId theOldLayerId,
...
// purpose  :
//=======================================================================
void OpenGl_LayerList::ChangePriority(const OpenGl_Structure* theStructure, const Graphic3d_ZLayerId theLayerId,
...
void OpenGl_LayerList::SetLayerSettings(const Graphic3d_ZLayerId theLayerId,
    ...
    if (aLayer.LayerSettings().IsRaytracable() != theSettings.IsRaytracable() && aLayer.NbStructures() != 0) {
        ++myModifStateOfRaytraceable;
    }
...
void OpenGl_LayerList::renderLayer(const Handle(OpenGl_Workspace) & theWorkspace,
    ...
    const bool hasOwnLights =
        aCtx->ColorMask() && !aLayerSettings.Lights().IsNull() && aLayerSettings.Lights() != aLightsBack;
    if (hasOwnLights) {
    ...
    if (hasLocalCS) {
        ...
        //   occModelWorldMatrix - model-world matrix defining Object transformation from local coordinate system to the
        //   world coordinate system occVertex           - input vertex position
        //
        ...
        //
        // All approaches might be useful in different scenarios, but for the moment we consider the last one as main
        // scenario. Here we do the trick:
        ...
        //  - OpenGl_Structure defines Object local transformation occModelWorldMatrix with subtracted Local Origin of
        //  the Layer.
        //    This means that Object itself should be defined within either Local Transformation equal or near to Local
        //    Origin of the Layer.
        ...
        Graphic3d_TransformUtils::Translate(aWorldView, aLayerSettings.Origin().X(), aLayerSettings.Origin().Y(),
        ...
        if (!aManager->LightSourceState().ShadowMaps().IsNull()) {
            ...
            for (OpenGl_ShadowMapArray::Iterator aShadowIter(*aManager->LightSourceState().ShadowMaps());
                 aShadowIter.More(); aShadowIter.Next()) {
    ...
    const Standard_Integer aViewId = theWorkspace->View()->Identification();
    for (Graphic3d_ArrayOfIndexedMapOfStructure::Iterator aMapIter(theLayer.ArrayOfStructures()); aMapIter.More();
         aMapIter.Next()) {
...
// purpose  :
//=======================================================================
void OpenGl_LayerList::Render(const Handle(OpenGl_Workspace) & theWorkspace, const Standard_Boolean theToDrawImmediate,
                              const OpenGl_LayerFilter theLayersToProcess, OpenGl_FrameBuffer* theReadDrawFbo,
    ...
    for (OpenGl_FilteredIndexedLayerIterator aLayerIterStart(myLayers, theToDrawImmediate, theLayersToProcess);
        ...
        for (int aPassIter = toPerformDepthPrepass ? 0 : 2; aPassIter < 3; ++aPassIter) {
            if (aPassIter == 0) {
                ...
                if (isShadowMapPass) {
                    ...
                    aCtx->SetColorMask(true);
                    aCtx->ShaderManager()->UpdateLightSourceStateTo(
                        aLightsBack, theWorkspace->View()->SpecIBLMapLevels(), aShadowMaps);
                }
            ...
            for (; aLayerIter.More(); aLayerIter.Next()) {
                ...
                if (aLayer.IsCulled()) {
                    ...
                    // At this point the depth buffer may be set to clear by previous configuration of layers or
                    // configuration of the current layer. Additional rendering pass to handle transparent elements of
                    // recently drawn layers require use of current depth buffer so we put remaining layers for
                    // processing as one bunch before erasing the depth buffer.
                    if (aPassIter == 2) {
...
void OpenGl_LayerList::renderTransparent(const Handle(OpenGl_Workspace) & theWorkspace,
                                         ...
                                         OpenGl_FrameBuffer* theReadDrawFbo, OpenGl_FrameBuffer* theOitAccumFbo) const {
    ...
    if (anOitMode == Graphic3d_RTM_BLEND_OIT) {
        if (theOitAccumFbo == NULL || theOitAccumFbo->NbColorBuffers() < 2 ||
            !theOitAccumFbo->ColorTexture(0)->IsValid() || !theOitAccumFbo->ColorTexture(1)->IsValid()) {
        ...
        case Graphic3d_RTM_DEPTH_PEELING_OIT: {
            ...
            for (int aPass = 0; aPass < aNbPasses; ++aPass) {
                ...
                if (aQuadVerts->IsValid() && aManager->BindOitDepthPeelingBlendProgram(isMSAA)) {
                    ...
                    aCtx->core20fwd->glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
