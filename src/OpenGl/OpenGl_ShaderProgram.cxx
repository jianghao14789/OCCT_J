...
OpenGl_VariableSetterSelector::OpenGl_VariableSetterSelector() {
    ...
    mySetterList = OpenGl_HashMapInitializer::CreateListOf<size_t, OpenGl_SetterInterface*>(
        ...
        new OpenGl_VariableSetter<int>())(Graphic3d_UniformValueTypeID<float>::ID, new OpenGl_VariableSetter<float>())(
...
OpenGl_ShaderProgram::OpenGl_ShaderProgram(const Handle(Graphic3d_ShaderProgram) & theProxy,
    ...
    : OpenGl_NamedResource(!theProxy.IsNull() ? theProxy->GetId() : theId), myProgramID(NO_PROGRAM), myProxy(theProxy),
      myShareCount(1), myNbLightsMax(0), myNbShadowMaps(0), myNbClipPlanesMax(0), myNbFragOutputs(1),
      myTextureSetBits(Graphic3d_TextureSetBits_NONE), myOitOutput(Graphic3d_RTM_BLEND_UNORDERED),
      myHasAlphaTest(false), myHasTessShader(false) {
...
Standard_Boolean OpenGl_ShaderProgram::Initialize(const Handle(OpenGl_Context) & theCtx,
        ...
        if (theCtx->caps->glslDumpLevel) {
            ...
            if (theCtx->caps->glslDumpLevel == OpenGl_ShaderProgramDumpLevel_Short) {
                anOutputSource = aHeaderVer + (!aHeaderVer.IsEmpty() ? "\n" : "") + anExtensions + aPrecisionHeader +
                                 aHeaderType + aHeaderConstants + anIter.Value()->Source();
            }
    ...
    if (!myProxy.IsNull()) {
        for (Graphic3d_ShaderAttributeList::Iterator anAttribIter(myProxy->VertexAttributes()); anAttribIter.More();
             anAttribIter.Next()) {
...
OpenGl_ShaderUniformLocation OpenGl_ShaderProgram::GetUniformLocation(const Handle(OpenGl_Context) & theCtx,
    ...
    return OpenGl_ShaderUniformLocation(
        myProgramID != NO_PROGRAM ? theCtx->core20fwd->glGetUniformLocation(myProgramID, theName) : INVALID_LOCATION);
}
...
// =======================================================================
GLint OpenGl_ShaderProgram::GetAttributeLocation(const Handle(OpenGl_Context) & theCtx, const GLchar* theName) const {
    return myProgramID != NO_PROGRAM ? theCtx->core20fwd->glGetAttribLocation(myProgramID, theName) : INVALID_LOCATION;
}
...
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::GetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
...
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::GetAttribute(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
                                                    OpenGl_Vec4i& theValue) const {
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetAttributeName(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetAttribute(const Handle(OpenGl_Context) & theCtx, GLint theIndex,
                                                    GLfloat theValue) {
    if (myProgramID == NO_PROGRAM || theIndex == INVALID_LOCATION) {
        return Standard_False;
    }
...
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                  GLint theValue) {
    if (myProgramID == NO_PROGRAM || theLocation == INVALID_LOCATION) {
        return Standard_False;
    }
...
// purpose  :
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetUniform(const Handle(OpenGl_Context) & theCtx, const GLchar* theName,
...
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetUniform(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
                                                  const OpenGl_Mat4& theValue, GLboolean theTranspose) {
    if (myProgramID == NO_PROGRAM || theLocation == INVALID_LOCATION) {
        return Standard_False;
    }

    theCtx->core20fwd->glUniformMatrix4fv(theLocation, 1, GL_FALSE,
                                          theTranspose ? theValue.Transposed().GetData() : theValue.GetData());
    return Standard_True;
}
...
// =======================================================================
Standard_Boolean OpenGl_ShaderProgram::SetSampler(const Handle(OpenGl_Context) & theCtx, GLint theLocation,
...
Standard_Boolean OpenGl_ShaderProgram::UpdateDebugDump(const Handle(OpenGl_Context) & theCtx,
                                                       ...
                                                       Standard_Boolean theToBeautify, Standard_Boolean theToReset) {
    if (myProgramID == NO_PROGRAM) {
        return Standard_False;
    }
    ...
    for (OpenGl_ShaderList::Iterator anIter(myShaderObjects); anIter.More(); anIter.Next()) {
        if (!anIter.Value().IsNull()) {
            ...
            hasUpdates =
                anIter.ChangeValue()->updateDebugDump(theCtx, myResourceId, aFolder, theToBeautify, theToReset) ||
                hasUpdates;
        }
    }
    if (hasUpdates) {
