...
OpenGl_Font::OpenGl_Font(const Handle(Font_FTFont) & theFont, const TCollection_AsciiString& theKey)
    : myKey(theKey), myFont(theFont), myAscender(0.0f), myDescender(0.0f), myTileSizeY(0), myLastTileId(-1),
...
bool OpenGl_Font::createTexture(const Handle(OpenGl_Context) & theCtx) {
        ...
        !aTexture->Init(theCtx, aBlackImg, Graphic3d_TOT_2D, true)) // myTextureFormat
    {
        theCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH,
                            TCollection_AsciiString("New texture initialization of size ") + aTextureSizeX + "x" +
                                aTextureSizeY + " for textured font has failed.");
        return false;
    }
...
// purpose  :
// =======================================================================
bool OpenGl_Font::renderGlyph(const Handle(OpenGl_Context) & theCtx, const Standard_Utf32Char theChar) {
    ...
    theCtx->core11fwd->glTexSubImage2D(GL_TEXTURE_2D, 0, myLastTilePx.Left, myLastTilePx.Top, (GLsizei)anImg.SizeX(),
                                       (GLsizei)anImg.SizeY(), aTexture->GetFormat(), GL_UNSIGNED_BYTE, anImg.Data());
...
// purpose  :
// =======================================================================
bool OpenGl_Font::RenderGlyph(const Handle(OpenGl_Context) & theCtx, const Standard_Utf32Char theUChar,
                              Tile& theGlyph) {
