...
// purpose  :
//=======================================================================
OpenGl_TileSampler::OpenGl_TileSampler() : myLastSample(0), myScaleFactor(1.0f), myTileSize(0), myViewSize(0, 0) {
    //
}
...
void OpenGl_TileSampler::GrabVarianceMap(const Handle(OpenGl_Context) & theContext,
    ...
    for (Standard_Size aColIter = 0; aColIter < myVarianceMap.SizeX; ++aColIter) {
        for (Standard_Size aRowIter = 0; aRowIter < myVarianceMap.SizeY; ++aRowIter) {
            ...
            Standard_RangeError_Raise_if(aRawValue < 0,
...
// purpose  :
//=======================================================================
void OpenGl_TileSampler::dumpMap(std::ostream& theStream, const Image_PixMapTypedData<int>& theMap,
...
// purpose  :
//=======================================================================
void OpenGl_TileSampler::SetSize(const Graphic3d_RenderingParams& theParams, const Graphic3d_Vec2i& theSize) {
...
bool OpenGl_TileSampler::upload(const Handle(OpenGl_Context) & theContext,
                                ...
                                const Handle(OpenGl_Texture) & theOffsetsTexture, const bool theAdaptive) {
    if (myTiles.IsEmpty()) {
        return false;
    }
    ...
    if (!theSamplesTexture.IsNull()) {
        ...
        if (theSamplesTexture->SizeX() == (int)myTileSamples.SizeX &&
            ...
            theContext->core11fwd->glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (int)myTileSamples.SizeX,
                                                   (int)myTileSamples.SizeY, GL_RED_INTEGER, GL_INT,
                                                   myTileSamples.Data());
    ...
    if (!theOffsetsTexture.IsNull()) {
        if (theOffsetsTexture->SizeX() != (int)anOffsets.SizeX || theOffsetsTexture->SizeY() != (int)anOffsets.SizeY ||
            !theOffsetsTexture->IsValid()) {
            theOffsetsTexture->Release(theContext.get());
            if (!theOffsetsTexture->Init(theContext, OpenGl_TextureFormat::FindSizedFormat(theContext, GL_RG32I),
        ...
        if (theOffsetsTexture->IsValid()) {
            ...
            theContext->core11fwd->glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
#endif
            theContext->core11fwd->glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (int)anOffsets.SizeX, (int)anOffsets.SizeY,
                                                   GL_RG_INTEGER, GL_INT, anOffsets.Data());
            if (theContext->core11fwd->glGetError() != GL_NO_ERROR) {
                hasErrors = true;
                theContext->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_MEDIUM,
