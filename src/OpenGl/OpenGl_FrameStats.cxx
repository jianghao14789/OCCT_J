...
// purpose  :
// =======================================================================
void OpenGl_FrameStats::updateStatistics(const Handle(Graphic3d_CView) & theView, bool theIsImmediateOnly) {
    ...
    if (toCountStructs || (aBits & Graphic3d_RenderingParams::PerfCounters_Layers) != 0) {
        ...
        for (NCollection_List<Handle(Graphic3d_Layer)>::Iterator aLayerIter(aView->LayerList().Layers());
            ...
            if (toCountGroups) {
                updateStructures(aViewId, aLayer->CullableStructuresBVH().Structures(), toCountElems, toCountTris,
                                 toCountMem);
                updateStructures(aViewId, aLayer->CullableTrsfPersStructuresBVH().Structures(), toCountElems,
                                 toCountTris, toCountMem);
...
void OpenGl_FrameStats::updateStructures(Standard_Integer theViewId,
                                         ...
                                         Standard_Boolean theToCountElems, Standard_Boolean theToCountTris,
