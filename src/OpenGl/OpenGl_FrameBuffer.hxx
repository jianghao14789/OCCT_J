...
public:
    ...
    Standard_EXPORT static Standard_Boolean BufferDump(const Handle(OpenGl_Context) & theGlCtx,
                                                       ...
                                                       Image_PixMap& theImage, Graphic3d_BufferType theBufferType);
...
public:
    ...
    Graphic3d_Vec2i GetSize() const {
        return Graphic3d_Vec2i(GetSizeX(), GetSizeY());
    }
    ...
    Graphic3d_Vec2i GetVPSize() const {
        return Graphic3d_Vec2i(myVPSizeX, myVPSizeY);
    }
    ...
    Graphic3d_Vec2i GetInitVPSize() const {
        return Graphic3d_Vec2i(myInitVPSizeX, myInitVPSizeY);
    }
    ...
    //! @return true on success
    Standard_EXPORT Standard_Boolean Init(const Handle(OpenGl_Context) & theGlCtx, const Graphic3d_Vec2i& theSize,
    ...
    Standard_Boolean InitLazy(const Handle(OpenGl_Context) & theGlCtx, const OpenGl_FrameBuffer& theFbo) {
        return InitLazy(theGlCtx, Graphic3d_Vec2i(theFbo.myVPSizeX, theFbo.myVPSizeY), theFbo.myColorFormats,
                        theFbo.myDepthFormat, theFbo.myNbSamples);
    }
    ...
    //! @param theColorRBufferFromWindow when specified - should be ID of already initialized RB object, which will be
    //! released within this class
    Standard_EXPORT Standard_Boolean InitWithRB(const Handle(OpenGl_Context) & theGlCtx, const Graphic3d_Vec2i& theSize,
                                                const Standard_Integer theColorFormat,
                                                const Standard_Integer theDepthFormat,
    ...
    Standard_EXPORT void ChangeViewport(const Standard_Integer theVPSizeX, const Standard_Integer theVPSizeY);
...
public:
    //! Initialize FBO for rendering into single/multiple color buffer and depth textures.
    Standard_DEPRECATED("Obsolete method, use Init() taking Graphic3d_Vec2i") bool Init(
        const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theSizeX, const Standard_Integer theSizeY,
        const OpenGl_ColorFormats& theColorFormats, const Handle(OpenGl_Texture) & theDepthStencilTexture,
        const Standard_Integer theNbSamples = 0) {
        return Init(theGlCtx, Graphic3d_Vec2i(theSizeX, theSizeY), theColorFormats, theDepthStencilTexture,
                    theNbSamples);
    }
    ...
    Standard_DEPRECATED("Obsolete method, use Init() taking Graphic3d_Vec2i") bool Init(
        const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theSizeX, const Standard_Integer theSizeY,
        const OpenGl_ColorFormats& theColorFormats, const Standard_Integer theDepthFormat,
        const Standard_Integer theNbSamples = 0) {
        return Init(theGlCtx, Graphic3d_Vec2i(theSizeX, theSizeY), theColorFormats, theDepthFormat, theNbSamples);
    }

    //! (Re-)initialize FBO with specified dimensions.
    Standard_DEPRECATED("Obsolete method, use InitLazy() taking Graphic3d_Vec2i") bool InitLazy(
        const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theViewportSizeX,
        const Standard_Integer theViewportSizeY, const Standard_Integer theColorFormat,
        const Standard_Integer theDepthFormat, const Standard_Integer theNbSamples = 0) {
        return InitLazy(theGlCtx, Graphic3d_Vec2i(theViewportSizeX, theViewportSizeY), theColorFormat, theDepthFormat,
                        theNbSamples);
    }
    ...
    Standard_DEPRECATED("Obsolete method, use InitLazy() taking Graphic3d_Vec2i") bool InitLazy(
        const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theViewportSizeX,
        const Standard_Integer theViewportSizeY, const OpenGl_ColorFormats& theColorFormats,
        const Standard_Integer theDepthFormat, const Standard_Integer theNbSamples = 0) {
        return InitLazy(theGlCtx, Graphic3d_Vec2i(theViewportSizeX, theViewportSizeY), theColorFormats, theDepthFormat,
                        theNbSamples);
    }

    //! (Re-)initialize FBO with specified dimensions.
    //! The Render Buffer Objects will be used for Color, Depth and Stencil attachments (as opposite to textures).
    Standard_DEPRECATED("Obsolete method, use InitWithRB() taking Graphic3d_Vec2i") bool InitWithRB(
        const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theSizeX, const Standard_Integer theSizeY,
        const Standard_Integer theColorFormat, const Standard_Integer theDepthFormat,
        const unsigned int theColorRBufferFromWindow = 0) {
        return InitWithRB(theGlCtx, Graphic3d_Vec2i(theSizeX, theSizeY), theColorFormat, theDepthFormat,
