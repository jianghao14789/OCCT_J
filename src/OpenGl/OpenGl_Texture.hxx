...
public:
    ...
    Standard_EXPORT
    OpenGl_Texture(const TCollection_AsciiString& theResourceId = TCollection_AsciiString(),
    ...
    //! Also binds Sampler Object if it is allocated.
    Standard_EXPORT void Bind(const Handle(OpenGl_Context) & theCtx, const Graphic3d_TextureUnit theTextureUnit) const;
    ...
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theCtx, const Image_PixMap& theImage,
    ...
    //! Notice that texture will be unbound after this call.
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theCtx, const OpenGl_TextureFormat& theFormat,
    ...
    Standard_EXPORT bool InitCompressed(const Handle(OpenGl_Context) & theCtx, const Image_CompressedPixMap& theImage,
                                        const Standard_Boolean theIsColorMap);

    //! Initialize the 2D multisampling texture using glTexImage2DMultisample().
    Standard_EXPORT bool Init2DMultisample(const Handle(OpenGl_Context) & theCtx, const Standard_Integer theNbSamples,
    ...
    Standard_EXPORT bool InitRectangle(const Handle(OpenGl_Context) & theCtx, const Standard_Integer theSizeX,
    ...
    Standard_EXPORT bool Init3D(const Handle(OpenGl_Context) & theCtx, const OpenGl_TextureFormat& theFormat,
    ...
    //! Returns estimated GPU memory usage for holding data without considering overheads and allocation alignment
    //! rules.
    ...
    Standard_EXPORT bool ImageDump(Image_PixMap& theImage, const Handle(OpenGl_Context) & theCtx,
                                   Graphic3d_TextureUnit theTexUnit, Standard_Integer theLevel = 0,
                                   Standard_Integer theCubeSide = 0) const;

public:
    Standard_DEPRECATED("Deprecated method, OpenGl_TextureFormat::FindFormat() should be used "
                        "instead") static bool GetDataFormat(const Handle(OpenGl_Context) & theCtx,
                                                             const Image_Format theFormat,
                                                             Standard_Integer& theTextFormat,
                                                             unsigned int& thePixelFormat, unsigned int& theDataType) {
    ...
    Standard_DEPRECATED("Deprecated method, OpenGl_TextureFormat should be passed instead of separate "
                        "parameters") bool Init(const Handle(OpenGl_Context) & theCtx,
                                                const Standard_Integer theTextFormat, const unsigned int thePixelFormat,
                                                const unsigned int theDataType, const Standard_Integer theSizeX,
                                                const Standard_Integer theSizeY, const Graphic3d_TypeOfTexture theType,
                                                const Image_PixMap* theImage = NULL) {
    ...
    Standard_DEPRECATED("Deprecated method, theIsColorMap parameter should be explicitly specified") bool Init(
        const Handle(OpenGl_Context) & theCtx, const Image_PixMap& theImage, const Graphic3d_TypeOfTexture theType) {
        return Init(theCtx, theImage, theType, true);
    }

    Standard_DEPRECATED("Deprecated method, OpenGl_TextureFormat should be passed instead of separate "
                        "parameters") bool Init3D(const Handle(OpenGl_Context) & theCtx,
                                                  const Standard_Integer theTextFormat,
                                                  const unsigned int thePixelFormat, const unsigned int theDataType,
                                                  const Standard_Integer theSizeX, const Standard_Integer theSizeY,
                                                  const Standard_Integer theSizeZ, const void* thePixels) {
        OpenGl_TextureFormat aFormat;
        aFormat.SetInternalFormat(theTextFormat);
        aFormat.SetPixelFormat(thePixelFormat);
        aFormat.SetDataType(theDataType);
    ...
    //! If theCubeMap is not NULL then size and format will be taken from it and corresponding arguments will be
    //! ignored. Otherwise this parameters will be taken from arguments.
    ...
    Standard_EXPORT bool InitCubeMap(const Handle(OpenGl_Context) & theCtx,
                                     const Handle(Graphic3d_CubeMap) & theCubeMap, Standard_Size theSize,
                                     Image_Format theFormat, Standard_Boolean theToGenMipmap,
