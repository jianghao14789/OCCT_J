...
template <class T> class OpenGl_MatrixState {
public:
    //! Constructs matrix state object.
    OpenGl_MatrixState() : myStack(8), myStackHead(-1) {
        //
    }
    ...
    //! Sets given matrix as current.
    template <typename Other_t> void SetCurrent(const typename OpenGl::MatrixType<Other_t>::Mat4& theNewCurrent) {
    ...
    void DumpJson(Standard_OStream& theOStream, Standard_Integer theDepth = -1) const {
        (void)theDepth;
        OCCT_DUMP_FIELD_VALUES_NUMERICAL(theOStream, "myCurrent", 16, myCurrent.GetValue(0, 0),
                                         myCurrent.GetValue(0, 1), myCurrent.GetValue(0, 2), myCurrent.GetValue(0, 3),
                                         myCurrent.GetValue(1, 0), myCurrent.GetValue(1, 1), myCurrent.GetValue(1, 2),
                                         myCurrent.GetValue(1, 3), myCurrent.GetValue(2, 0), myCurrent.GetValue(2, 1),
                                         myCurrent.GetValue(2, 2), myCurrent.GetValue(2, 3), myCurrent.GetValue(3, 0),
                                         myCurrent.GetValue(3, 1), myCurrent.GetValue(3, 2), myCurrent.GetValue(3, 3))
