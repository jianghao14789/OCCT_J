...
public:
    OpenGl_PBREnvironmentSentry(const Handle(OpenGl_Context) & theCtx) : myContext(theCtx) {
        backup();
        prepare();
    }
...
Handle(OpenGl_PBREnvironment) OpenGl_PBREnvironment::Create(const Handle(OpenGl_Context) & theCtx,
                                                            unsigned int thePow2Size, unsigned int theLevelsNumber,
...
// purpose  :
// =======================================================================
OpenGl_PBREnvironment::OpenGl_PBREnvironment(const Handle(OpenGl_Context) & theCtx, unsigned int thePowOf2Size,
                                             unsigned int theSpecMapLevelsNumber, const TCollection_AsciiString& theId)
    : OpenGl_NamedResource(theId), myPow2Size(std::max(1u, thePowOf2Size)),
      mySpecMapLevelsNumber(std::max(2u, std::min(theSpecMapLevelsNumber, std::max(1u, thePowOf2Size) + 1))),
      myFBO(OpenGl_FrameBuffer::NO_FRAMEBUFFER), myIsComplete(Standard_False), myIsNeededToBeBound(Standard_True),
...
// purpose  :
// =======================================================================
void OpenGl_PBREnvironment::Clear(const Handle(OpenGl_Context) & theCtx, const Graphic3d_Vec3& theColor) {
    OpenGl_PBREnvironmentSentry aSentry(theCtx);
...
// purpose  :
// =======================================================================
void OpenGl_PBREnvironment::Bake(const Handle(OpenGl_Context) & theCtx, const Handle(OpenGl_Texture) & theEnvMap,
                                 Standard_Boolean theZIsInverted, Standard_Boolean theIsTopDown,
                                 Standard_Size theDiffMapNbSamples, Standard_Size theSpecMapNbSamples,
                                 Standard_ShortReal theProbability) {
    Standard_ProgramError_Raise_if(
        theEnvMap.IsNull(), "'Bake' function of 'OpenGl_PBREnvironment' can't work without source environment map")
        Standard_RangeError_Raise_if(
            theProbability > 1.f || theProbability < 0.f,
            "'probability' parameter in 'Bake' function of 'OpenGl_PBREnvironment' must be in range [0, 1]")
            Unbind(theCtx);
    OpenGl_PBREnvironmentSentry aSentry(theCtx);
...
// purpose  :
// =======================================================================
bool OpenGl_PBREnvironment::SizesAreDifferent(unsigned int thePow2Size, unsigned int theSpecMapLevelsNumber) const {
...
bool OpenGl_PBREnvironment::initTextures(const Handle(OpenGl_Context) & theCtx) {
    ...
    // NVIDIA's driver didn't work properly with 3 channel texture for diffuse SH coefficients so that alpha channel has
    // been added
    if (!myIBLMaps[OpenGl_TypeOfIBLMap_DiffuseSH].Init(
            theCtx, OpenGl_TextureFormat::FindFormat(theCtx, Image_Format_RGBAF, false), Graphic3d_Vec2i(9, 1),
            Graphic3d_TOT_2D)) {
        Message::SendFail() << "OpenGl_PBREnvironment, DiffuseSH texture creation failed";
        return false;
    }

    if (!myIBLMaps[OpenGl_TypeOfIBLMap_Specular].InitCubeMap(
            theCtx, Handle(Graphic3d_CubeMap)(), Standard_Size(1) << myPow2Size, Image_Format_RGB, true, false)) {
    ...
    if (!myIBLMaps[OpenGl_TypeOfIBLMap_DiffuseFallback].Init(
            theCtx, OpenGl_TextureFormat::FindFormat(theCtx, Image_Format_RGBA, false), Graphic3d_Vec2i(10, 4),
            Graphic3d_TOT_2D)) {
...
bool OpenGl_PBREnvironment::initVAO(const Handle(OpenGl_Context) & theCtx) {
    const float aVertexPos[] = {-1.f, -1.f, 0.f, 0.f, 1.f, -1.f, 0.f, 0.f, -1.f, 1.f, 0.f, 0.f, 1.f, 1.f, 0.f, 0.f};
...
bool OpenGl_PBREnvironment::processDiffIBLMap(const Handle(OpenGl_Context) & theCtx,
    ...
    if (!myCanRenderFloat) {
        anImageF.InitZero(Image_Format_RGBAF, myIBLMaps[OpenGl_TypeOfIBLMap_DiffuseSH].SizeX(),
    ...
    if (theDrawParams != NULL) {
        ...
        if (!myCanRenderFloat) {
            ...
            theCtx->core11fwd->glReadPixels(0, 0, aViewport[2], aViewport[3], GL_RGBA,
                                            myCanRenderFloat ? GL_FLOAT : GL_UNSIGNED_BYTE, anImageIn.ChangeData());
...
bool OpenGl_PBREnvironment::processSpecIBLMap(const Handle(OpenGl_Context) & theCtx,
    ...
    const OpenGl_TextureFormat aTexFormat =
        OpenGl_TextureFormat::FindSizedFormat(theCtx, myIBLMaps[OpenGl_TypeOfIBLMap_Specular].SizedFormat());
    ...
    for (int aLevelIter = mySpecMapLevelsNumber - 1;; --aLevelIter) {
        ...
        if (theDrawParams != NULL) {
            const Standard_Integer aNbSamples =
                Standard_Integer(Graphic3d_PBRMaterial::SpecIBLMapSamplesFactor(
                                     theDrawParams->Probability, aLevelIter / float(mySpecMapLevelsNumber - 1)) *
        ...
        for (Standard_Integer aSideIter = 0; aSideIter < 6; ++aSideIter) {
            theCtx->arbFBO->glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + aSideIter,
            ...
            if (!canRenderMipmaps && aLevelIter != 0) {
                ...
                theCtx->core20fwd->glCopyTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + aSideIter, aLevelIter, anIntFormat,
                                                    0, 0, (GLsizei)aSize, (GLsizei)aSize, 0);
...
bool OpenGl_PBREnvironment::checkFBOComplentess(const Handle(OpenGl_Context) & theCtx) {
    ...
    if (theCtx->arbFBO->glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        // Some WebGL 1.0 and OpenGL ES 2.0 implementations support float textures which cannot be used as render
        // targets. This capability could be exposed by WEBGL_color_buffer_float/EXT_color_buffer_float extensions, but
        // the simplest way is just to check FBO status. The fallback solution involves rendering into RGBA8 texture
        // with floating values packed, and using glReadPixels() + conversion + texture upload of computed values.
    ...
    for (Standard_Integer aSideIter = 0; aSideIter < 6; ++aSideIter) {
        for (unsigned int aLevel = 0; aLevel < mySpecMapLevelsNumber; ++aLevel) {
            ...
            if (theCtx->arbFBO->glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
                Message::SendTrace() << "OpenGl_PBREnvironment, incomplete FBO for specular map " << aSideIter << " "
                                     << aLevel;
                return false;
            }
        }
...
void OpenGl_PBREnvironment::bake(const Handle(OpenGl_Context) & theCtx, const Handle(OpenGl_Texture) & theEnvMap,
                                 Standard_Boolean theZIsInverted, Standard_Boolean theIsTopDown,
                                 Standard_Size theDiffNbSamples, Standard_Size theSpecNbSamples,
                                 Standard_ShortReal theProbability) {
    myIsNeededToBeBound = Standard_True;
    ...
    if (processSpecIBLMap(theCtx, &aDrawParams) && processDiffIBLMap(theCtx, &aDrawParams)) {
        Message::SendTrace(TCollection_AsciiString() + "IBL " + myIBLMaps[OpenGl_TypeOfIBLMap_Specular].SizeX() + "x" +
                           myIBLMaps[OpenGl_TypeOfIBLMap_Specular].SizeY() + " is baked in " + aTimer.ElapsedTime() +
                           " s");
    } else {
        theCtx->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_PERFORMANCE, 0, GL_DEBUG_SEVERITY_HIGH,
                            TCollection_AsciiString("Error: baking PBR environment ") +
                                ...
                                myIBLMaps[OpenGl_TypeOfIBLMap_Specular].SizeY() + " takes too much time!.");
