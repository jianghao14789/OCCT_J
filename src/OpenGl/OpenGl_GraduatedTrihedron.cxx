...
OpenGl_GraduatedTrihedron::OpenGl_GraduatedTrihedron()
    : myMin(0.0f, 0.0f, 0.0f), myMax(100.0f, 100.0f, 100.0f), myIsInitialized(Standard_False) {
    //
}
...
Standard_ShortReal OpenGl_GraduatedTrihedron::getNormal(const Handle(OpenGl_Context) & theContext,
    ...
    Graphic3d_TransformUtils::UnProject<Standard_ShortReal>(
        (Standard_ShortReal)aViewport[0], (Standard_ShortReal)aViewport[1], 0.0f, aModelMatrix, aProjMatrix, aViewport,
        aPoint1.x(), aPoint1.y(), aPoint1.z());

    Graphic3d_TransformUtils::UnProject<Standard_ShortReal>(
        (Standard_ShortReal)(aViewport[0] + aViewport[2]), (Standard_ShortReal)aViewport[1], 0.0f, aModelMatrix,
        aProjMatrix, aViewport, aPoint2.x(), aPoint2.y(), aPoint2.z());

    Graphic3d_TransformUtils::UnProject<Standard_ShortReal>(
        (Standard_ShortReal)aViewport[0], (Standard_ShortReal)(aViewport[1] + aViewport[3]), 0.0f, aModelMatrix,
        aProjMatrix, aViewport, aPoint3.x(), aPoint3.y(), aPoint3.z());
...
Standard_ExtCharacter OpenGl_GraduatedTrihedron::getGridAxes(const Standard_ShortReal theCorners[8],
    ...
    switch (aMaxIndex) {
        ...
        default: {
            ...
            return OYO_XYO | OYZ_XYZ | OOZ_XOZ | XOO_XYO | XOZ_XYZ | OOZ_OYZ | XOO_XOZ | XYO_XYZ | OYO_OYZ;
        }
    }
}
...
void OpenGl_GraduatedTrihedron::renderLine(const OpenGl_PrimitiveArray& theLine,
                                           const Handle(OpenGl_Workspace) & theWorkspace, const OpenGl_Mat4& theMat,
                                           const Standard_ShortReal theXt, const Standard_ShortReal theYt,
...
void OpenGl_GraduatedTrihedron::renderGridPlane(const Handle(OpenGl_Workspace) & theWorkspace,
                                                const Standard_Integer& theIndex, const GridAxes& theGridAxes,
...
void OpenGl_GraduatedTrihedron::renderAxis(const Handle(OpenGl_Workspace) & theWorkspace,
                                           const Standard_Integer& theIndex, const OpenGl_Mat4& theMat) const {
    ...
    Graphic3d_TransformPers aTransMode(
        ...
        gp_Pnt(Standard_Real(anArrowVec.x()), Standard_Real(anArrowVec.y()), Standard_Real(anArrowVec.z())));
    ...
    Graphic3d_TransformUtils::Project<Standard_ShortReal>(aEndPoint.x(), aEndPoint.y(), aEndPoint.z(), aModelMat,
                                                          aProjMat, aContext->Viewport(), aWinPoint.x(), aWinPoint.y(),
                                                          aWinPoint.z());
    ...
    Graphic3d_TransformUtils::UnProject<Standard_ShortReal>(aWinPoint.x(), aWinPoint.y(), aWinPoint.z(), aModelMat,
                                                            aProjMat, aContext->Viewport(), anArrowStart.x(),
                                                            anArrowStart.y(), anArrowStart.z());
    // Render axis line
    aModelMat = theMat;
    Graphic3d_TransformUtils::Translate(aModelMat, myMin.x(), myMin.y(), myMin.z());

    Standard_ShortReal aScaleFactor =
        ((anArrowStart - myMin) * anAxis.Direction).Modulus() / (anAxis.Direction * (myMax - myMin)).Modulus();
...
void OpenGl_GraduatedTrihedron::renderTickmarkLabels(const Handle(OpenGl_Workspace) & theWorkspace,
                                                     const OpenGl_Mat4& theMat, const Standard_Integer theIndex,
    ...
    if (aCurAspect.ToDrawTickmarks() && aCurAspect.TickmarksNumber() > 0) {
        ...
        Graphic3d_TransformUtils::Translate(aModelMat, theGridAxes.Ticks[theIndex].x(), theGridAxes.Ticks[theIndex].y(),
    ...
    if (aCurAspect.ToDrawValues() && aCurAspect.TickmarksNumber() > 0) {
        ...
        for (Standard_Integer anIt = 0; anIt <= aCurAspect.TickmarksNumber(); ++anIt) {
            ...
            OpenGl_Vec3 aPos(theGridAxes.Ticks[theIndex] + anAxis.Direction * (Standard_ShortReal)(anIt * aStep) +
                             aDir * (Standard_ShortReal)(theDpix * anOffset));
...
void OpenGl_GraduatedTrihedron::Render(const Handle(OpenGl_Workspace) & theWorkspace) const {
    ...
    if (myData.CubicAxesCallback(theWorkspace->View())) {
        if (!myAxes[0].Line.IsInitialized() || !myAxes[1].Line.IsInitialized() || !myAxes[2].Line.IsInitialized() ||
...
OpenGl_GraduatedTrihedron::Axis::Axis(const Graphic3d_GraduatedTrihedron::AxisAspect& theAspect,
    ...
    : Direction(theDirection), Tickmark(NULL), Line(NULL), Arrow(NULL) {
...
OpenGl_GraduatedTrihedron::Axis& OpenGl_GraduatedTrihedron::Axis::operator=(const Axis& theOther) {
    ...
    Line.InitBuffers(NULL, Graphic3d_TOPA_SEGMENTS, theOther.Line.Indices(), theOther.Line.Attributes(),
                     theOther.Line.Bounds());
    Tickmark.InitBuffers(NULL, Graphic3d_TOPA_SEGMENTS, theOther.Tickmark.Indices(), theOther.Tickmark.Attributes(),
                         theOther.Tickmark.Bounds());
    Arrow.InitBuffers(NULL, Graphic3d_TOPA_POLYLINES, theOther.Arrow.Indices(), theOther.Arrow.Attributes(),
...
void OpenGl_GraduatedTrihedron::Axis::InitArrow(const Handle(OpenGl_Context) & theContext,
    ...
    Arrow.InitBuffers(theContext, Graphic3d_TOPA_POLYLINES, anArray->Indices(), anArray->Attributes(),
                      anArray->Bounds());
}
...
void OpenGl_GraduatedTrihedron::Axis::InitTickmark(const Handle(OpenGl_Context) & theContext,
    ...
    Tickmark.InitBuffers(theContext, Graphic3d_TOPA_SEGMENTS, anArray->Indices(), anArray->Attributes(),
                         anArray->Bounds());
}
...
void OpenGl_GraduatedTrihedron::Axis::InitLine(const Handle(OpenGl_Context) & theContext,
    ...
    Line.InitBuffers(theContext, Graphic3d_TOPA_SEGMENTS, anArray->Indices(), anArray->Attributes(), anArray->Bounds());
}
