...
public:
    ...
    //! @param theComponentsNb [in] specifies the number of components per generic vertex attribute; must be 1, 2, 3, or
    //! 4;
    //! @param theElemsNb      [in] elements count;
    //! @param theData         [in] pointer to float data (vertices/normals etc.).
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
                              const Standard_Integer theElemsNb, const float* theData);
    ...
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
                              const Standard_Integer theElemsNb, const unsigned int* theData);
    ...
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
                              const Standard_Integer theElemsNb, const unsigned short* theData);
    ...
    Standard_EXPORT bool Init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
                              const Standard_Integer theElemsNb, const Standard_Byte* theData);
    ...
    Standard_EXPORT bool SubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                 const Standard_Integer theElemsNb, const float* theData);
    ...
    Standard_EXPORT bool GetSubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                    const Standard_Integer theElemsNb, float* theData);
    ...
    Standard_EXPORT bool GetSubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                    const Standard_Integer theElemsNb, unsigned int* theData);
    ...
    Standard_EXPORT bool GetSubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                    const Standard_Integer theElemsNb, unsigned short* theData);
    ...
    Standard_EXPORT bool GetSubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                    const Standard_Integer theElemsNb, Standard_Byte* theData);
...
public: //! @name advanced methods
    ...
    Standard_EXPORT virtual bool init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
                                      const Standard_Integer theElemsNb, const void* theData,
                                      const unsigned int theDataType, const Standard_Integer theStride);

    //! Initialize buffer with new data.
    bool init(const Handle(OpenGl_Context) & theGlCtx, const unsigned int theComponentsNb,
              const Standard_Integer theElemsNb, const void* theData, const unsigned int theDataType) {
    ...
    Standard_EXPORT virtual bool subData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                         const Standard_Integer theElemsNb, const void* theData,
                                         const unsigned int theDataType);

    //! Read back buffer sub-range.
    Standard_EXPORT virtual bool getSubData(const Handle(OpenGl_Context) & theGlCtx, const Standard_Integer theElemFrom,
                                            const Standard_Integer theElemsNb, void* theData,
                                            const unsigned int theDataType);
...
protected:
    ...
    Standard_EXPORT void BindBufferBase(const Handle(OpenGl_Context) & theGlCtx, unsigned int theIndex);

    //! Unbinds a buffer object from an indexed buffer target.
    //! Wrapper for glBindBufferBase().
    //! @param theGlCtx [in] active OpenGL context
    //! @param theIndex [in] index to bind
    Standard_EXPORT void UnbindBufferBase(const Handle(OpenGl_Context) & theGlCtx, unsigned int theIndex);
    ...
    //! @param theOffset [in] offset within the buffer (@sa GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT in case of uniform
    //! buffer)
    //! @param theSize   [in] sub-section length starting from offset
    Standard_EXPORT void BindBufferRange(const Handle(OpenGl_Context) & theGlCtx, unsigned int theIndex,
                                         const intptr_t theOffset, const size_t theSize);
