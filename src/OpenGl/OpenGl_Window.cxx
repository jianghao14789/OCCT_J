...
static void addMsgToList(TCollection_ExtendedString& theList, const TCollection_ExtendedString& theMsg) {
    if (!theList.IsEmpty()) {
        theList += ", ";
    }
    theList += theMsg;
...
OpenGl_Window::OpenGl_Window(const Handle(OpenGl_GraphicDriver) & theDriver,
                             const Handle(Aspect_Window) & thePlatformWindow, Aspect_RenderingContext theGContext,
                             const Handle(OpenGl_Caps) & theCaps, const Handle(OpenGl_Context) & theShareCtx)
    : myGlContext(new OpenGl_Context(theCaps)), myOwnGContext(theGContext == 0), myPlatformWindow(thePlatformWindow),
    ...
    if ((EGLContext)theGContext == EGL_NO_CONTEXT) {
        // EGL_KHR_gl_colorspace extension specifies if OpenGL should write into window buffer as into sRGB or RGB
        // framebuffer
        ...
        // create new surface
        anEglSurf = eglCreateWindowSurface(anEglDisplay, anEglConfig,
                                           (EGLNativeWindowType)myPlatformWindow->NativeHandle(), NULL);
        if (anEglSurf == EGL_NO_SURFACE && myPlatformWindow->NativeHandle() != 0) {
            ...
            if (anEglConfig != NULL) {
                ...
                const int aSurfAttribs[] = {EGL_WIDTH, myWidth, EGL_HEIGHT, myHeight,
                                            // EGL_KHR_gl_colorspace extension specifies if OpenGL should write into
                                            // window buffer as into sRGB or RGB framebuffer
    ...
    if (aPixelFrmtId == 0 && theCaps->contextStereo) {
        ...
        myGlContext->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_OTHER, 0, GL_DEBUG_SEVERITY_HIGH, aMsg);
    ...
    if (aGContext == NULL) {
...
#endif
            aWinTmp = CreateWindowExW(anExStyle, aClass.lpszClassName, L"OpenGl_WindowTmp",
                                      WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_DISABLED, 2, 2, 4, 4, NULL,
                                      NULL, aModule, NULL);
        }
        if (aWinTmp != NULL) {
        ...
        typedef HGLRC(WINAPI * wglCreateContextAttribsARB_t)(HDC theDevCtx, HGLRC theShareContext,
        ...
        if (aChoosePixProc != NULL) {
            const int aPixAttribs[] = {
                ...
                8,
                // WGL_EXT_colorspace extension specifies if OpenGL should write into window buffer as into sRGB or RGB
                // framebuffer
        ...
        if (aCreateCtxProc != NULL) {
            if (!theCaps->contextCompatible) {
                ...
                //  but some will not (e.g. AMD Catalyst) since WGL_ARB_create_context_profile specification allows both
                //  implementations).
            ...
            if (aGContext == NULL) {
                int aCtxAttribs[] = {
                    ...
                    WGL_CONTEXT_FLAGS_ARB, theCaps->contextDebug ? WGL_CONTEXT_DEBUG_BIT_ARB : 0, 0, 0};
                ...
                if (aGContext != NULL && !theCaps->contextCompatible) {
                    ...
                    myGlContext->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_PORTABILITY, 0,
                                             GL_DEBUG_SEVERITY_LOW, aMsg);
                }
            }
    ...
    std::unique_ptr<XVisualInfo, int (*)(void*)> aVis(
        XGetVisualInfo(aDisp, VisualIDMask | VisualScreenMask, &aVisInfo, &aNbItems), &XFree);
    int isGl = 0;
    if (aVis.get() == NULL) {
        throw Aspect_GraphicDeviceDefinitionError("OpenGl_Window::CreateWindow: XGetVisualInfo is unable to choose "
                                                  "needed configuration in existing OpenGL context. ");
        return;
    ...
    if (myOwnGContext && anFBConfig != NULL &&
        ...
        typedef GLXContext (*glXCreateContextAttribsARB_t)(Display* dpy, GLXFBConfig config, GLXContext share_context,
        ...
        if (aGContext == NULL) {
            int aCtxAttribs[] = {GLX_CONTEXT_FLAGS_ARB, theCaps->contextDebug ? GLX_CONTEXT_DEBUG_BIT_ARB : 0, 0, 0};
            isCoreProfile = Standard_False;
    ...
    if (!aList.IsEmpty()) {
        TCollection_ExtendedString aMsg =
            TCollection_ExtendedString("OpenGl_Window::CreateWindow: window Visual is incomplete: ") + aList;
        myGlContext->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_OTHER, 0, GL_DEBUG_SEVERITY_MEDIUM, aMsg);
    }
...
void OpenGl_Window::Init() {
    ...
    if ((EGLSurface)myGlContext->myWindow == EGL_NO_SURFACE) {
        ...
        // note that this code is currently never called, since eglCreatePbufferSurface() is used instead as more robust
        // solution for offscreen rendering on bugged OpenGL ES drivers
