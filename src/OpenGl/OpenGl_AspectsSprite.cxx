...
void OpenGl_AspectsSprite::UpdateRediness(const Handle(Graphic3d_Aspects) & theAspect) {
    ...
    spriteKeys(theAspect->MarkerImage(), theAspect->MarkerType(), theAspect->MarkerScale(), theAspect->ColorRGBA(),
               aSpriteKeyNew, aSpriteAKeyNew);
...
const Handle(OpenGl_PointSprite) & OpenGl_AspectsSprite::Sprite(const Handle(OpenGl_Context) & theCtx,
    ...
    if (!myIsSpriteReady) {
        build(theCtx, theAspects->MarkerImage(), theAspects->MarkerType(), theAspects->MarkerScale(),
              theAspects->ColorRGBA(), myMarkerSize);
...
void OpenGl_AspectsSprite::build(const Handle(OpenGl_Context) & theCtx,
    ...
    if (theCtx->core20fwd != NULL && (!theCtx->caps->pntSpritesDisable || theCtx->core11ffp == NULL)) {
        ...
        if (!hadAlreadyAlpha) {
            if (Handle(Image_PixMap) anImageA =
                    aSprite->GetFormat() != GL_ALPHA ? aNewMarkerImage->GetImageAlpha() : Handle(Image_PixMap)()) {
        ...
        if (aFormat.IsValid()) {
            ...
            theCtx->core11ffp->glBitmap(0, 0, 0, 0, GLfloat(-0.5f * aWidth), GLfloat(-0.5f * aHeight),
                                        NULL); // make offsets that will be added to the current raster position
            theCtx->core11ffp->glDrawPixels(GLsizei(anImage->Width()), GLsizei(anImage->Height()),
                                            aFormat.PixelFormat(), aFormat.DataType(), anImage->Data());
        ...
        if (!aFormat.IsValid() || !hadAlreadyAlpha) {
            ...
            if (Handle(TColStd_HArray1OfByte) aBitMap = aNewMarkerImage->GetBitMapArray()) {
                theCtx->core11ffp->glNewList(aBitmapList, GL_COMPILE);
                theCtx->core11ffp->glBitmap((GLsizei)aWidth, (GLsizei)aHeight, (GLfloat)(0.5f * aWidth),
                                            (GLfloat)(0.5f * aHeight), 0.f, 0.f, (const GLubyte*)&aBitMap->First());
                theCtx->core11ffp->glEndList();
            }
        }
#endif
...
// purpose  :
// =======================================================================
void OpenGl_AspectsSprite::spriteKeys(const Handle(Graphic3d_MarkerImage) & theMarkerImage, Aspect_TypeOfMarker theType,
                                      Standard_ShortReal theScale, const Graphic3d_Vec4& theColor,
                                      TCollection_AsciiString& theKey, TCollection_AsciiString& theKeyA) {
    // generate key for shared resource
