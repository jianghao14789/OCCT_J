...
public:
    ...
    XmlMDF_DerivedDriver(const Handle(TDF_Attribute) & theDerivative, const Handle(XmlMDF_ADriver) & theBaseDriver)
        : XmlMDF_ADriver(theBaseDriver->MessageDriver(), theBaseDriver->Namespace().ToCString()),
          myDerivative(theDerivative), myBaseDirver(theBaseDriver) {}
    ...
    virtual Standard_Boolean Paste(const XmlObjMgt_Persistent& theSource, const Handle(TDF_Attribute) & theTarget,
    ...
    virtual void Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
