...
// purpose  : Constructor
//=======================================================================
XmlMDF_ADriver::XmlMDF_ADriver(const Handle(Message_Messenger) & theMsgDriver, const Standard_CString theNS,
                               const Standard_CString theName)
    : myNamespace(theNS == NULL ? "" : theNS), myMessageDriver(theMsgDriver) {
    if (theNS != NULL)
        if (theNS[0] != '\0') {
            myTypeName = theNS;
            myTypeName += ':';
        }
    if (theName != NULL) myTypeName += theName;
}
