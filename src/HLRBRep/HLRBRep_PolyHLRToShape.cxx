    ...
    for (myAlgo->InitHide(); myAlgo->MoreHide(); myAlgo->NextHide()) {
        ...
        if (aD.Modulus() > 1.e-10) {
            ...
            for (It.InitVisible(status); It.MoreVisible(); It.NextVisible()) {
                ...
                myBiPntVis.Append(HLRBRep_BiPnt2D(aSta2D + sta * aD, aSta2D + end * aD, S, reg1, regn, outl, intl));
            }

            for (It.InitHidden(status); It.MoreHidden(); It.NextHidden()) {
                It.Hidden(sta, tolsta, end, tolend);
                myBiPntHid.Append(HLRBRep_BiPnt2D(aSta2D + sta * aD, aSta2D + end * aD, S, reg1, regn, outl, intl));
            }
        }
    }
}
...
TopoDS_Shape HLRBRep_PolyHLRToShape::InternalCompound(const Standard_Integer typ, const Standard_Boolean visible,
