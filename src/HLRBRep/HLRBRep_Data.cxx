...
static long unsigned Mask32[32] = {
    ...
    4194304, 8388608, 16777216, 33554432, 67108864, 134217728, 268435456, 536870912, 1073741824, 2147483648U};
...
public:
    ...
    //-- ============================================================
    void SetIntersection(Standard_Integer i0, Standard_Integer i1, const IntRes2d_IntersectionPoint& IP) {
    ...
    //-- ============================================================
    void GetSingleIntersection(Standard_Integer i0, Standard_Integer i1, Standard_Real& u, Standard_Real& v) {
        u = Get(i0, i1);
        if (u != RealLast()) {
            v = Get(i1, i0);
        } else {
...
static void AdjustParameter(HLRBRep_EdgeData* E, const Standard_Boolean h, Standard_Real& p, Standard_ShortReal& t) {
    Standard_Real p1, p2;
...
HLRBRep_Data::HLRBRep_Data(const Standard_Integer NV, const Standard_Integer NE, const Standard_Integer NF)
    : myNbVertices(NV), myNbEdges(NE), myNbFaces(NF), myEData(0, NE), myFData(0, NF), myEdgeIndices(0, NE),
      myToler((Standard_ShortReal)1e-5), myLLProps(2, Epsilon(1.)), myFLProps(2, Epsilon(1.)),
      mySLProps(2, Epsilon(1.)), myHideCount(0) {
...
void HLRBRep_Data::Write(const Handle(HLRBRep_Data) & DS, const Standard_Integer dv, const Standard_Integer de,
...
void HLRBRep_Data::Update(const HLRAlgo_Projector& P) {
    ...
    for (edge = 1; edge <= myNbEdges; edge++) {
        ...
        ed.Vertical(TotMax[0] - TotMin[0] < tol && TotMax[1] - TotMin[1] < tol && TotMax[2] - TotMin[2] < tol &&
                    TotMax[3] - TotMin[3] < tol && TotMax[4] - TotMin[4] < tol && TotMax[5] - TotMin[5] < tol &&
    ...
    for (Standard_Integer face = 1; face <= myNbFaces; face++) {
        ...
        if (!withOutL && (iFaceType == GeomAbs_Plane || iFaceType == GeomAbs_Cylinder || iFaceType == GeomAbs_Cone ||
...
void HLRBRep_Data::InitBoundSort(const HLRAlgo_EdgesBlock::MinMaxIndices& MinMaxTot, const Standard_Integer e1,
...
Standard_Boolean HLRBRep_Data::MoreEdge() {
    ...
    if (iFaceTest) {
        if (myFaceItr2.MoreEdge()) {            // all edges must be tested if
            ...
            if (!myLEDouble) myLEData->HideCount(myHideCount - 1);
            return Standard_True;
        } else {
                            ...
                            if (myIntersected) { // compute real intersection
                                ...
                                if (myLE == myFE) {
                                    ...
                                    if (su != RealLast()) {
                                        ...
                                        myIntersector.Perform(myLE, myLEData, da1, db1, myFE, myFEData, da2, db2,
                                                              mySameVertex);
...
Standard_Boolean HLRBRep_Data::RejectedInterference() {
    if (iInterf <= myNbPoints) {
        ...
        if (firstPoint) nseg++;
        Standard_Real pf =
            ((HLRBRep_Curve*)myLEGeom)->Parameter3d(myIntersector.Segment(nseg).FirstPoint().ParamOnFirst());
        Standard_Real pl =
            ((HLRBRep_Curve*)myLEGeom)->Parameter3d(myIntersector.Segment(nseg).LastPoint().ParamOnFirst());
        if (pf > pl) firstPoint = !firstPoint;

        if (firstPoint) {
            Standard_Boolean ret1 = RejectedPoint(myIntersector.Segment(nseg).FirstPoint(), TopAbs_FORWARD, nseg);
            return (ret1);
        } else {
            Standard_Boolean ret2 = RejectedPoint(myIntersector.Segment(nseg).LastPoint(), TopAbs_REVERSED, -nseg);
            return (ret2);
        }
    }
}
...
void HLRBRep_Data::LocalLEGeometry2D(const Standard_Real Param, gp_Dir2d& Tg, gp_Dir2d& Nm, Standard_Real& Cu) {
    myLLProps.SetParameter(Param);
    if (!myLLProps.IsTangentDefined()) throw Standard_Failure("HLRBRep_Data::LocalGeometry2D");
...
void HLRBRep_Data::LocalFEGeometry2D(const Standard_Integer FE, const Standard_Real Param, gp_Dir2d& Tg, gp_Dir2d& Nm,
                                     Standard_Real& Cu) {
    const HLRBRep_Curve* aCurve = &myEData(FE).ChangeGeometry();
    myFLProps.SetCurve(aCurve);
    myFLProps.SetParameter(Param);
    if (!myFLProps.IsTangentDefined()) throw Standard_Failure("HLRBRep_Data::LocalGeometry2D");
...
void HLRBRep_Data::EdgeState(const Standard_Real p1, const Standard_Real p2, TopAbs_State& stbef, TopAbs_State& staft) {
    ...
    if (HLRBRep_EdgeFaceTool::UVPoint(p2, myFEGeom, iFaceGeom, pu, pv)) {
        ...
        if (mySLProps.IsNormalDefined()) {
            ...
            V.Transform(TI);
            if (NrmFace.Dot(V) > 0.) NrmFace.Reverse();
...
Standard_Integer HLRBRep_Data::HidingStartLevel(const Standard_Integer E, const HLRBRep_EdgeData& ED,
...
static void REJECT1(const Standard_Real theDeca[], const Standard_Real theTotMin[], const Standard_Real theTotMax[],
                    const Standard_Real theSurD[], HLRAlgo_EdgesBlock::MinMaxIndices& theVertMin,
...
TopAbs_State HLRBRep_Data::Classify(const Standard_Integer E, const HLRBRep_EdgeData& ED,
                                    const Standard_Boolean LevelFlag, Standard_Integer& Level,
    ...
    if (LevelFlag) {
    ...
    #ifdef OCCT_DEBUG
        {
          ...
          for(qwe=0,qwep8=8; qwe<8; qwe++,qwep8++) {
            ...
            q1 = q>>16;
            q2 = (q& 0x0000FFFF);
            printf("\nmot: %3d    q1 = %+10d    q2=%+10d    Mask : %d",qwe,(q1>32768)? (32768-q1) : q1,(q2>32768)?
    (32768-q2) : q2,q&0x80008000);
          }
          for(qwe=0,qwep8=8; qwe<8; qwe++,qwep8++) {
            q = ((Standard_Integer*)MinMaxVert)[qwep8]- ((Standard_Integer*)iFaceMinMax)[qwe];
            q1 = q>>16;
            q2 = (q& 0x0000FFFF);
            printf("\nmot: %3d    q1 = %+10d    q2=%+10d    Mask : %d",qwe+8,(q1>32768)? (32768-q1) : q1,(q2>32768)?
    (32768-q2) : q2,q&0x80008000);
          }
          std::cout<<std::endl;
        }
        ...
        if (nbPoints > 0) {
            ...
            for (i = 1; i <= nbPoints; i++) {
                ...
                if (w < wLim) {
                    ...
                    if (PeriodU > 0.) GeomInt::AdjustPeriodic(u, UMin, UMax, PeriodU, u, aDummyShift);
                    if (PeriodV > 0.) GeomInt::AdjustPeriodic(v, VMin, VMax, PeriodV, v, aDummyShift);
...
TopAbs_State HLRBRep_Data::SimplClassify(const Standard_Integer /*E*/, const HLRBRep_EdgeData& ED,
                                         const Standard_Integer Nbp, const Standard_Real p1, const Standard_Real p2) {
    nbClassification++;
    HLRAlgo_EdgesBlock::MinMaxIndices VertMin, VertMax, MinMaxVert;
    Standard_Real TotMin[16], TotMax[16];
...
Standard_Boolean HLRBRep_Data::RejectedPoint(const IntRes2d_IntersectionPoint& PInter,
                                             const TopAbs_Orientation BoundOri, const Standard_Integer NumSeg) {
    ...
    if (iFaceTest) {
        if (myLE == myFE) {
            if (st == TopAbs_IN) ((HLRBRep_EdgeData*)myLEData)->Simple(Standard_False);
        } else {
            if (mySameVertex) {
        ...
        if (Tr2->PositionOnCurve() != IntRes2d_Middle) { // correction de la transition  sur myFE
            ...
            if (Tr2->PositionOnCurve() == IntRes2d_Head) {
                ...
                if (((HLRBRep_EdgeData*)myFEData)->VerAtSta()) {
                    ...
                    if (Tgsav.SquareMagnitude() <= DERIVEE_PREMIERE_NULLE) Tgsav = Nmsav;
                }
            } else {
    ...
    if (myFEInternal) {
        ...
        decal = 1;
        if (st == TopAbs_IN && Ori == TopAbs_FORWARD && Orie == TopAbs_FORWARD) decal = 0;
    }
...
Standard_Boolean HLRBRep_Data::SameVertex(const Standard_Boolean h1, const Standard_Boolean h2) {
    ...
    if (SameV) {
        ...
        if ((h1 && ((HLRBRep_EdgeData*)myLEData)->OutLVSta()) || (!h1 && ((HLRBRep_EdgeData*)myLEData)->OutLVEnd())) {
            if (iFaceTest || myLEInternal) otherCase = Standard_False;
        } else if (iFaceTest)
...
Standard_Boolean HLRBRep_Data::IsBadFace() const {
    if (iFaceGeom) {
        ...
        if (pGeom->IsUPeriodic()) {
            ...
            if (aPeriod * 2 < aMax - aMin) return Standard_True;
        }
