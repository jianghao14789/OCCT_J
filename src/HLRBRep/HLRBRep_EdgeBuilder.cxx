...
HLRBRep_EdgeBuilder::HLRBRep_EdgeBuilder(HLRBRep_VertexList& VList) {
    ...
    for (; VList.More(); VList.Next()) {
        ...
        V = VList.Current();
        cur = new HLRBRep_AreaLimit(V, VList.IsBoundary(), VList.IsInterference(), before, after, ebefore, eafter);
        if (myLimits.IsNull()) {
            myLimits = cur;
            last = cur;
        } else {
    ...
    if (estat == TopAbs_UNKNOWN) estat = TopAbs_IN;
    ...
    while (!cur.IsNull()) {
        if (cur->StateBefore() == TopAbs_UNKNOWN)
            ...
            stat = cur->StateAfter();
        if (cur->StateAfter() == TopAbs_UNKNOWN) cur->StateAfter(stat);
        if (cur->EdgeBefore() == TopAbs_UNKNOWN)
            cur->EdgeBefore(estat);
        else
            estat = cur->EdgeAfter();
        if (cur->EdgeAfter() == TopAbs_UNKNOWN) cur->EdgeAfter(estat);
...
void HLRBRep_EdgeBuilder::NextArea() {
    left = right;
    if (!right.IsNull()) right = right->Next();
}
...
void HLRBRep_EdgeBuilder::PreviousArea() {
    right = left;
    if (!left.IsNull()) left = left->Previous();
}
...
TopAbs_State HLRBRep_EdgeBuilder::AreaState() const {
    TopAbs_State stat = TopAbs_UNKNOWN;
    if (!left.IsNull()) stat = left->StateAfter();
    if (!right.IsNull()) stat = right->StateBefore();
    return stat;
}
...
TopAbs_State HLRBRep_EdgeBuilder::AreaEdgeState() const {
    TopAbs_State stat = TopAbs_UNKNOWN;
    if (!left.IsNull()) stat = left->EdgeAfter();
    if (!right.IsNull()) stat = right->EdgeBefore();
    return stat;
}
...
TopAbs_Orientation HLRBRep_EdgeBuilder::Orientation() const {
    if (current == 1) {
        ...
        if ((right->StateBefore() == right->StateAfter()) && (right->EdgeBefore() == right->EdgeAfter()))
            return TopAbs_INTERNAL;
        else
