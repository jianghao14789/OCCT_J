...
// Purpose  :
//=======================================================================
Handle(HLRBRep_Data) HLRBRep_ShapeToHLR::Load(const Handle(HLRTopoBRep_OutLiner) & S, const HLRAlgo_Projector& P,
                                              BRepTopAdaptor_MapOfShapeTool& MST, const Standard_Integer nbIso) {
    S->Fill(P, MST, nbIso);
    ...
    for (exshell.Init(S->OutLinedShape(), TopAbs_SHELL); exshell.More(); exshell.Next()) { // faces in a shell

        for (exface.Init(exshell.Current(), TopAbs_FACE); exface.More(); exface.Next()) {
            if (!FM.Contains(exface.Current())) FM.Add(exface.Current());
        }
    }

    for (exface.Init(S->OutLinedShape(), TopAbs_FACE, TopAbs_SHELL); exface.More();
...
void HLRBRep_ShapeToHLR::ExploreFace(const Handle(HLRTopoBRep_OutLiner) & S, const Handle(HLRBRep_Data) & DS,
                                     const TopTools_IndexedMapOfShape& FM, const TopTools_IndexedMapOfShape& EM,
                                     Standard_Integer& i, const TopoDS_Face& F, const Standard_Boolean closed) {
    i++;
    ...
    for (Ex1.Init(theFace, TopAbs_WIRE); Ex1.More(); Ex1.Next()) {
        ...
        for (Ex2.Init(Ex1.Current(), TopAbs_EDGE); Ex2.More(); Ex2.Next()) {
            ...
            if (!BRep_Tool::Degenerated(anEdge)) ne++;
        }
...
void HLRBRep_ShapeToHLR::ExploreShape(const Handle(HLRTopoBRep_OutLiner) & S, const Handle(HLRBRep_Data) & DS,
                                      const TopTools_IndexedMapOfShape& FM, const TopTools_IndexedMapOfShape& EM) {
    TopTools_MapOfShape ShapeMap;
    TopExp_Explorer exshell, exface, exedge;
    Standard_Integer i = 0;

    for (exshell.Init(S->OriginalShape(), TopAbs_SHELL); exshell.More();
    ...
    for (exface.Init(S->OriginalShape(), TopAbs_FACE, TopAbs_SHELL); exface.More();
         exface.Next()) { // faces not in a shell
        if (ShapeMap.Add(exface.Current())) {
