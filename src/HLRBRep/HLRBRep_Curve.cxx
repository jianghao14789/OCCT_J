...
Standard_Real HLRBRep_Curve::Update(Standard_Real TotMin[16], Standard_Real TotMax[16]) {
    ...
    switch (typ) {
            ...
            if (!((HLRAlgo_Projector*)myProj)->Perspective()) {
                ...
                if (D1.IsParallel(gp::DZ(), Precision::Angular()))
                    myType = GeomAbs_Circle;
                         ...
                         Precision::Angular() * 10) //*10: The minor radius of ellipse should not be too small.
                    myType = GeomAbs_OtherCurve;
                else {
        ...
        case GeomAbs_BSplineCurve:
            if (!((HLRAlgo_Projector*)myProj)->Perspective()) myType = typ;
            break;
...
void HLRBRep_Curve::Tangent(const Standard_Boolean AtStart, gp_Pnt2d& P, gp_Dir2d& D) const {
    Standard_Real U =
        AtStart ? HLRBRep_BCurveTool::FirstParameter(myCurve) : HLRBRep_BCurveTool::LastParameter(myCurve);
...
void HLRBRep_Curve::PolesAndWeights(const Handle(Geom_BSplineCurve) & aCurve, TColgp_Array1OfPnt2d& TP,
