...
HLRBRep_InternalAlgo::HLRBRep_InternalAlgo() : myDebug(Standard_False) {}
...
void HLRBRep_InternalAlgo::Load(const Handle(HLRTopoBRep_OutLiner) & S, const Handle(Standard_Transient) & SData,
...
void HLRBRep_InternalAlgo::Remove(const Standard_Integer I) {
    Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::Remove : unknown Shape");
    myShapes.Remove(I);
...
void HLRBRep_InternalAlgo::ShapeData(const Standard_Integer I, const Handle(Standard_Transient) & SData) {
    Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::ShapeData : unknown Shape");
...
HLRBRep_ShapeBounds& HLRBRep_InternalAlgo::ShapeBounds(const Standard_Integer I) {
    Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::ShapeBounds : unknown Shape");
    ...
    for (f = 1; f <= nf; f++) {
        ...
        if (visible) {
            ...
            for (faceIt.InitEdge(fd); faceIt.MoreEdge(); faceIt.NextEdge()) {
                ...
                if (edf->Selected() && !edf->Vertical()) edf->Status().ShowAll();
            }
        }
    }
}
...
void HLRBRep_InternalAlgo::Select(const Standard_Integer I) {
    if (!myDS.IsNull()) {
        Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::Select : unknown Shape");
...
void HLRBRep_InternalAlgo::ShowAll(const Standard_Integer I) {
    if (!myDS.IsNull()) {
        Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::ShowAll : unknown Shape");
...
void HLRBRep_InternalAlgo::HideAll(const Standard_Integer I) {
    if (!myDS.IsNull()) {
        Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::HideAll : unknown Shape");
...
void HLRBRep_InternalAlgo::PartialHide() {
    if (!myDS.IsNull()) {
        ...
        if (myDebug) std::cout << " Partial hiding" << std::endl << std::endl;
...
void HLRBRep_InternalAlgo::Hide() {
    if (!myDS.IsNull()) {
        ...
        if (myDebug) std::cout << " Total hiding" << std::endl;
...
void HLRBRep_InternalAlgo::Hide(const Standard_Integer I) {
    if (!myDS.IsNull()) {
        Standard_OutOfRange_Raise_if(I == 0 || I > myShapes.Length(), "HLRBRep_InternalAlgo::Hide : unknown Shape");

        if (myDebug) std::cout << " hiding the shape " << I << " by itself" << std::endl;
    ...
    if (e2 >= e1) {
        ...
        if (myDebug) {
            ...
            if (myDebug) {
                ...
                if (SideFace) std::cout << "Side      : " << std::setw(5) << nbFSide << " , ";
        ...
        */
                ...
                while (k <= ir) {
                    if (k < ir) {
                        if (Val(Index(k)) > Val(Index(k + 1)))
                            ...
                            if (Size(Index(k)) > Size(Index(k + 1))) k++;
                        }
                    }
        ...
        if (myDebug) {
            ...
            for (f = 1; f <= nf; f++) {
                HLRBRep_FaceData& fd = aFDataArray.ChangeValue(f);
                if (fd.Selected() && fd.Simple()) nbFSimp++;
            }
