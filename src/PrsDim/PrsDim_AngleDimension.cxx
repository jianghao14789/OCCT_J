...
static Standard_Boolean isSameLine(const gp_Pnt& theFirstPoint, const gp_Pnt& theCenterPoint,
...
// purpose  :
//=======================================================================
PrsDim_AngleDimension::PrsDim_AngleDimension(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge)
...
// purpose  :
//=======================================================================
PrsDim_AngleDimension::PrsDim_AngleDimension(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
...
// purpose  :
//=======================================================================
PrsDim_AngleDimension::PrsDim_AngleDimension(const TopoDS_Vertex& theFirstVertex, const TopoDS_Vertex& theSecondVertex,
...
// purpose  :
//=======================================================================
PrsDim_AngleDimension::PrsDim_AngleDimension(const TopoDS_Face& theCone) : PrsDim_Dimension(PrsDim_KOD_PLANEANGLE) {
    Init();
...
// purpose  :
//=======================================================================
PrsDim_AngleDimension::PrsDim_AngleDimension(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace)
    : PrsDim_Dimension(PrsDim_KOD_PLANEANGLE) {
    Init();
...
// purpose  :
//=======================================================================
void PrsDim_AngleDimension::SetMeasuredGeometry(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge) {
...
// purpose  :
//=======================================================================
void PrsDim_AngleDimension::SetMeasuredGeometry(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
                                                const gp_Pnt& theThirdPoint) {
...
// purpose  :
//=======================================================================
void PrsDim_AngleDimension::SetMeasuredGeometry(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace) {
...
// purpose :
//=======================================================================
gp_Pnt PrsDim_AngleDimension::GetCenterOnArc(const gp_Pnt& theFirstAttach, const gp_Pnt& theSecondAttach,
...
//=======================================================================
void PrsDim_AngleDimension::DrawArc(const Handle(Prs3d_Presentation) & thePresentation, const gp_Pnt& theFirstAttach,
                                    const gp_Pnt& theSecondAttach, const gp_Pnt& theCenter,
                                    const Standard_Real theRadius, const Standard_Integer theMode) {
    ...
    if (myType == PrsDim_TypeOfAngle_Exterior) anAngle = 2.0 * M_PI - anAngle;
...
void PrsDim_AngleDimension::DrawArcWithText(const Handle(Prs3d_Presentation) & thePresentation,
                                            const gp_Pnt& theFirstAttach, const gp_Pnt& theSecondAttach,
                                            const gp_Pnt& theCenter, const TCollection_ExtendedString& theText,
                                            const Standard_Real theTextWidth, const Standard_Integer theMode,
...
void PrsDim_AngleDimension::Compute(const Handle(PrsMgr_PresentationManager) &,
    ...
    switch (aHPosition) {
        case LabelPosition_HCenter: {
            ...
            if (isLineBreak) {
                DrawArcWithText(thePresentation, aFirstAttach, aSecondAttach, myCenterPoint, aLabelString, aLabelWidth,
                                theMode, aLabelPosition);
                break;
            }
            ...
            if (theMode == ComputeMode_All || theMode == ComputeMode_Line) {
                DrawArc(thePresentation,
                        (isArrowsExternal || !isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_First)) ? aFirstAttach
                                                                                                       : aFirstArrowEnd,
                        (isArrowsExternal || !isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_Second))
                            ? aSecondAttach
                            : aSecondArrowEnd,
                        myCenterPoint, Abs(GetFlyout()), theMode);
            }
        } break;

        case LabelPosition_Left: {
            DrawExtension(thePresentation, anExtensionSize,
                          (isArrowsExternal && isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_First)) ? aFirstArrowEnd
                                                                                                        : aFirstAttach,
    ...
    if ((theMode == ComputeMode_All || theMode == ComputeMode_Line) && aHPosition != LabelPosition_HCenter) {
        ...
        DrawArc(thePresentation,
                (isArrowsExternal || !isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_First)) ? aFirstAttach
                                                                                               : aFirstArrowEnd,
                (isArrowsExternal || !isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_Second)) ? aSecondAttach
                                                                                                : aSecondArrowEnd,
                myCenterPoint, Abs(GetFlyout()), theMode);
    }
    ...
    if ((theMode == ComputeMode_All || theMode == ComputeMode_Line) && isArrowsExternal) {
        ...
        if (aHPosition != LabelPosition_Left && isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_First)) {
            DrawExtension(thePresentation, aDimensionAspect->ArrowTailSize(), aFirstArrowEnd, aFirstExtensionDir,
                          THE_EMPTY_LABEL_STRING, THE_EMPTY_LABEL_WIDTH, theMode, LabelPosition_None);
        }

        if (aHPosition != LabelPosition_Right && isArrowVisible(PrsDim_TypeOfAngleArrowVisibility_Second)) {
            DrawExtension(thePresentation, aDimensionAspect->ArrowTailSize(), aSecondArrowEnd, aSecondExtensionDir,
                          THE_EMPTY_LABEL_STRING, THE_EMPTY_LABEL_WIDTH, theMode, LabelPosition_None);
        }
    }
...
Standard_Boolean PrsDim_AngleDimension::InitTwoEdgesAngle(gp_Pln& theComputedPlane) {
    ...
    if (!PrsDim::ComputeGeometry(aFirstEdge, aSecondEdge, aFirstCurve, aSecondCurve, aFirstPoint1, aLastPoint1,
                                 aFirstPoint2, aLastPoint2, isInfinite1, isInfinite2)) {
        return Standard_False;
    }
    ...
    if (isParallelLines) {
        ...
        mySecondPoint =
            myCenterPoint.XYZ() + (aFirstLin.Direction().IsEqual(aSecondLin.Direction(), Precision::Angular())
        ...
        // *----
        myFirstPoint =
            myCenterPoint.Distance(aFirstPoint1) > myCenterPoint.Distance(aLastPoint1) ? aFirstPoint1 : aLastPoint1;

        mySecondPoint =
            myCenterPoint.Distance(aFirstPoint2) > myCenterPoint.Distance(aLastPoint2) ? aFirstPoint2 : aLastPoint2;
    }
...
Standard_Boolean PrsDim_AngleDimension::InitTwoFacesAngle() {
    ...
    PrsDim::GetPlaneFromFace(aSecondFace, aSecondPln, aSecondBasisSurf, aSecondSurfType, aSecondOffset);
    ...
    if (aFirstSurfType == PrsDim_KOS_Plane && aSecondSurfType == PrsDim_KOS_Plane) {
        ...
        return PrsDim::InitAngleBetweenPlanarFaces(aFirstFace, aSecondFace, myCenterPoint, myFirstPoint,
                                                   mySecondPoint) &&
               IsValidPoints(myFirstPoint, myCenterPoint, mySecondPoint);
    } else {
        // Curvilinear faces angle
        return PrsDim::InitAngleBetweenCurvilinearFaces(aFirstFace, aSecondFace, aFirstSurfType, aSecondSurfType,
...
Standard_Boolean PrsDim_AngleDimension::InitTwoFacesAngle(const gp_Pnt& thePointOnFirstFace) {
    ...
    if (aFirstSurfType == PrsDim_KOS_Plane && aSecondSurfType == PrsDim_KOS_Plane) {
        ...
        Handle(Geom_Plane) aFirstPlane = Handle(Geom_Plane)::DownCast(aFirstBasisSurf);
        Handle(Geom_Plane) aSecondPlane = Handle(Geom_Plane)::DownCast(aSecondBasisSurf);
        return PrsDim::InitAngleBetweenPlanarFaces(aFirstFace, aSecondFace, myCenterPoint, myFirstPoint, mySecondPoint,
                                                   Standard_True) &&
               IsValidPoints(myFirstPoint, myCenterPoint, mySecondPoint);
    } else {
        // Curvilinear faces angle
        return PrsDim::InitAngleBetweenCurvilinearFaces(aFirstFace, aSecondFace, aFirstSurfType, aSecondSurfType,
                                                        myCenterPoint, myFirstPoint, mySecondPoint, Standard_True) &&
               IsValidPoints(myFirstPoint, myCenterPoint, mySecondPoint);
    }
}
...
Standard_Boolean PrsDim_AngleDimension::InitConeAngle() {
    ...
    if (aSurfType == PrsDim_KOS_Revolution) {
        ...
        if (aBasisCurve->DynamicType() != STANDARD_TYPE(Geom_Line)) return Standard_False;
        ...
        if (aVec1.IsParallel(aVec2, Precision::Angular()) || aVec1.IsNormal(aVec2, Precision::Angular()))
            return Standard_False;
        ...
        if (aType == STANDARD_TYPE(Geom_OffsetSurface) || anOffset > 0.01) {
            ...
            aMkFace.Build();
            if (!aMkFace.IsDone()) return Standard_False;
...
// purpose  :
//=======================================================================
Standard_Boolean PrsDim_AngleDimension::IsValidPoints(const gp_Pnt& theFirstPoint, const gp_Pnt& theCenterPoint,
    ...
    return theFirstPoint.Distance(theCenterPoint) > Precision::Confusion() &&
           ...
           gp_Vec(theCenterPoint, theFirstPoint).Angle(gp_Vec(theCenterPoint, theSecondPoint)) > Precision::Angular();
}
...
// purpose  :
//=======================================================================
void PrsDim_AngleDimension::AdjustParameters(const gp_Pnt& theTextPos, Standard_Real& theExtensionSize,
    ...
    if (aFirstDist <= aSecondDist) {
        ...
        theFlyout = aFirstTextProj.Distance(myCenterPoint.Translated(aPosFlyoutDir)) > Precision::Confusion() ? -aRadius
                                                                                                              : aRadius;
    } else {
