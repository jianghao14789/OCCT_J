...
public:
    ...
    Standard_EXPORT PrsDim_AngleDimension(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge);
    ...
    Standard_EXPORT PrsDim_AngleDimension(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
    ...
    Standard_EXPORT PrsDim_AngleDimension(const TopoDS_Vertex& theFirstVertex, const TopoDS_Vertex& theSecondVertex,
    ...
    Standard_EXPORT PrsDim_AngleDimension(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace);
...
public:
    ...
    //! @param theFirstEdge [in] the first edge.
    //! @param theSecondEdge [in] the second edge.
    Standard_EXPORT void SetMeasuredGeometry(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge);

    //! Measures angle defined by three points.
    //! @param theFirstPoint [in] the first point (point on first angle flyout).
    //! @param theSecondPoint [in] the center point of angle dimension.
    //! @param theThirdPoint [in] the second point (point on second angle flyout).
    Standard_EXPORT void SetMeasuredGeometry(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
    ...
    //! @param theSecondVertex [in] the center vertex of angle dimension.
    //! @param theThirdPoint [in] the second vertex (vertex for second angle flyout).
    Standard_EXPORT void SetMeasuredGeometry(const TopoDS_Vertex& theFirstVertex, const TopoDS_Vertex& theSecondVertex,
                                             const TopoDS_Vertex& theThirdVertex);
    ...
    //! @param theFirstFace [in] the first face.
    //! @param theSecondFace [in] the second face..
    Standard_EXPORT void SetMeasuredGeometry(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace);
...
protected:
    ...
    Standard_EXPORT gp_Pnt GetCenterOnArc(const gp_Pnt& theFirstAttach, const gp_Pnt& theSecondAttach,
    ...
    Standard_EXPORT void DrawArc(const Handle(Prs3d_Presentation) & thePresentation, const gp_Pnt& theFirstAttach,
                                 const gp_Pnt& theSecondAttach, const gp_Pnt& theCenter, const Standard_Real theRadius,
    ...
    Standard_EXPORT void AdjustParameters(const gp_Pnt& theTextPos, Standard_Real& theExtensionSize,
...
protected:
    ...
    Standard_EXPORT Standard_Boolean IsValidPoints(const gp_Pnt& theFirstPoint, const gp_Pnt& theCenterPoint,
