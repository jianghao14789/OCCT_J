...
// purpose  :
//=======================================================================
PrsDim_Chamf2dDimension::PrsDim_Chamf2dDimension(const TopoDS_Shape& aFShape, const Handle(Geom_Plane) & aPlane,
...
PrsDim_Chamf2dDimension::PrsDim_Chamf2dDimension(const TopoDS_Shape& aFShape, const Handle(Geom_Plane) & aPlane,
                                                 const Standard_Real aVal, const TCollection_ExtendedString& aText,
                                                 const gp_Pnt& aPosition, const DsgPrs_ArrowSide aSymbolPrs,
...
void PrsDim_Chamf2dDimension::Compute(const Handle(PrsMgr_PresentationManager) &,
    ...
    if (!PrsDim::ComputeGeometry(thechamfedge, gcurv, pfirst, plast)) return;
    ...
    if (myAutomaticPosition) {
        ...
        if (myIsSetBndBox) curpos = PrsDim::TranslatePointToBound(curpos, myDir, myBndBox);
    ...
    if (!myArrowSizeIsDefined) {
        ...
        if ((myVal / 4) < arrsize) arrsize = myVal / 4;
        if (arrsize > 30.)
            arrsize = 30.;
        else if (arrsize < 8.)
            arrsize = 8.;
    ...
    DsgPrs_Chamf2dPresentation::Add(aPresentation, myDrawer, myPntAttach, curpos, myText, mySymbolPrs);
}
...
void PrsDim_Chamf2dDimension::ComputeSelection(const Handle(SelectMgr_Selection) & aSelection, const Standard_Integer) {
    ...
    Handle(Select3D_SensitiveBox) box =
        new Select3D_SensitiveBox(own, myPosition.X(), myPosition.Y(), myPosition.Z(), myPosition.X() + size,
                                  myPosition.Y() + size, myPosition.Z() + size);
    aSelection->Add(box);
}
