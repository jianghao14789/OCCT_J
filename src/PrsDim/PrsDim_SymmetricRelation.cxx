...
// purpose  :
//=======================================================================
PrsDim_SymmetricRelation::PrsDim_SymmetricRelation(const TopoDS_Shape& aSymmTool, const TopoDS_Shape& FirstShape,
                                                   const TopoDS_Shape& SecondShape, const Handle(Geom_Plane) & aPlane)
    : PrsDim_Relation(), myTool(aSymmTool) {
...
void PrsDim_SymmetricRelation::Compute(const Handle(PrsMgr_PresentationManager) &,
    ...
    if (myTool.ShapeType() == TopAbs_EDGE) {
        ...
        if (PrsDim::ComputeGeometry(TopoDS::Edge(myTool), aCurve, p1, p2, extcurve, isinfinite, isonplane, myPlane)) {
            if (!extcurve.IsNull()) {
                gp_Pnt pf, pl;
                if (!isinfinite) {
                    pf = p1;
...
// purpose  :
//=======================================================================
void PrsDim_SymmetricRelation::ComputeSelection(const Handle(SelectMgr_Selection) & aSel, const Standard_Integer) {
    ...
    gp_Pnt p1, p2;
    Standard_Boolean isinfinite, isonplane;
    if (!PrsDim::ComputeGeometry(TopoDS::Edge(myTool), geom_axis, p1, p2, extcurve, isinfinite, isonplane, myPlane))
        return;
        ...
        if (cu1.GetType() == GeomAbs_Line) {
            ...
            if (!P1.IsEqual(P2, Precision::Confusion())) {
                ...
                Handle(Select3D_SensitiveBox) box =
                    new Select3D_SensitiveBox(own, myPosition.X(), myPosition.Y(), myPosition.Z(),
                                              myPosition.X() + size, myPosition.Y() + size, myPosition.Z() + size);
                aSel->Add(box);
            }
...
void PrsDim_SymmetricRelation::ComputeTwoEdgesSymmetric(const Handle(Prs3d_Presentation) & aprs) {
    ...
    if (!PrsDim::ComputeGeometry(TopoDS::Edge(myFShape), TopoDS::Edge(mySShape), myExtShape, geom1, geom2, ptat11,
                                 ptat12, ptat21, ptat22, extCurv, isInfinite1, isInfinite2, myPlane)) {
        return;
    }
    ...
    */
    ...
    if (!myArrowSizeIsDefined) myArrowSize = myFAttach.Distance(mySAttach) / 50.;
    //----------------------------------------------------
    ...
    arr->SetLength(myArrowSize);
    if (cu1.GetType() == GeomAbs_Line)
        DsgPrs_SymmetricPresentation::Add(aprs, myDrawer, myFAttach, mySAttach, myFDirAttach, laxis, myPosition);

    if (cu1.GetType() == GeomAbs_Circle)
        DsgPrs_SymmetricPresentation::Add(aprs, myDrawer, myFAttach, mySAttach, circ, laxis, myPosition);
