...
void PrsDim_EllipseRadiusDimension::ComputeCylFaceGeometry(const PrsDim_KindOfSurface aSurfType,
    ...
    if (aSurfType == PrsDim_KOS_Extrusion) {
        ...
        Axis.SetLocation(gp_Pnt((Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(aBasisSurf))->Direction().XYZ()));
        ...
        if (surf1.GetType() == GeomAbs_OffsetSurface) {
            ...
            myOffsetCurve =
                new Geom_OffsetCurve(new Geom_Ellipse(myEllipse), Offset, myPlane->Pln().Axis().Direction());
            myOffset = Offset;
