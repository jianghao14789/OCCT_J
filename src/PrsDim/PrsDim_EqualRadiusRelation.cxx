...
// purpose  :
//=======================================================================
PrsDim_EqualRadiusRelation::PrsDim_EqualRadiusRelation(const TopoDS_Edge& aFirstEdge, const TopoDS_Edge& aSecondEdge,
...
void PrsDim_EqualRadiusRelation::Compute(const Handle(PrsMgr_PresentationManager) &,
    ...
    Handle(Geom_Curve) FirstProjCurve = FirstCurve.Curve().Curve(), SecondProjCurve = SecondCurve.Curve().Curve();
    ...
    if (myAutomaticPosition) {
        ...
        if (FirstPoint2.Distance(LastPoint2) > Precision::Confusion()) {
            if (aPar > LastPar2 || aPar < FirstPar2) { // mySecondPoint is out of Arc of mySecondCircle
    ...
    if (!myArrowSizeIsDefined)
        myArrowSize = (Min(myFirstCenter.Distance(myFirstPoint), mySecondCenter.Distance(mySecondPoint))) * 0.05;
    ...
    DsgPrs_EqualRadiusPresentation::Add(aPresentation, myDrawer, myFirstCenter, mySecondCenter, myFirstPoint,
                                        mySecondPoint, myPlane);
}
...
void PrsDim_EqualRadiusRelation::ComputeSelection(const Handle(SelectMgr_Selection) & aSelection,
    ...
    if (!myAutomaticPosition) ComputeRadiusPosition();
