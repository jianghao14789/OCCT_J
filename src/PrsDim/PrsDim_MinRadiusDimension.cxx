...
// purpose  :
//=======================================================================
PrsDim_MinRadiusDimension::PrsDim_MinRadiusDimension(const TopoDS_Shape& aShape, const Standard_Real aVal,
...
PrsDim_MinRadiusDimension::PrsDim_MinRadiusDimension(const TopoDS_Shape& aShape, const Standard_Real aVal,
                                                     const TCollection_ExtendedString& aText, const gp_Pnt& aPosition,
                                                     const DsgPrs_ArrowSide aSymbolPrs, const Standard_Real anArrowSize)
    : PrsDim_EllipseRadiusDimension(aShape, aText) {
    myVal = aVal;
...
void PrsDim_MinRadiusDimension::ComputeEllipse(const Handle(Prs3d_Presentation) & aPresentation) {
    ...
    if (myAutomaticPosition) {
        ...
        if (myIsSetBndBox)
            myPosition = PrsDim::TranslatePointToBound(myPosition, gp_Dir(gp_Vec(Center, myPosition)), myBndBox);
        curPos = myPosition;
    ...
    // Presenatation
    DsgPrs_EllipseRadiusPresentation::Add(aPresentation, myDrawer, myVal, myText, curPos, myEndOfArrow, Center,
                                          Standard_False, mySymbolPrs);
}
...
void PrsDim_MinRadiusDimension::ComputeArcOfEllipse(const Handle(Prs3d_Presentation) & aPresentation) {
    ...
    if (myAutomaticPosition) {
        myEndOfArrow = PrsDim::NearestApex(myEllipse, myApexP, myApexN, myFirstPar, myLastPar, IsInDomain);
        myPosition = Center;
        myAutomaticPosition = Standard_True;
        if (myIsSetBndBox)
            myPosition = PrsDim::TranslatePointToBound(myPosition, gp_Dir(gp_Vec(Center, myPosition)), myBndBox);
    ...
    if (!myIsOffset)
        DsgPrs_EllipseRadiusPresentation::Add(aPresentation, myDrawer, myVal, myText, myEllipse, curPos, myEndOfArrow,
                                              Center, parStart, IsInDomain, Standard_True, mySymbolPrs);
    else
        DsgPrs_EllipseRadiusPresentation::Add(aPresentation, myDrawer, myVal, myText, myOffsetCurve, curPos,
                                              myEndOfArrow, Center, parStart, IsInDomain, Standard_True, mySymbolPrs);
}
...
void PrsDim_MinRadiusDimension::ComputeSelection(const Handle(SelectMgr_Selection) & aSelection,
    ...
    Handle(Select3D_SensitiveBox) box =
        new Select3D_SensitiveBox(own, AttachmentPoint.X(), AttachmentPoint.Y(), AttachmentPoint.Z(),
