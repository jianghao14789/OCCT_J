...
PrsDim_Dimension::PrsDim_Dimension(const PrsDim_KindOfDimension theType)
    : AIS_InteractiveObject(), mySelToleranceForText2d(0.0), myValueType(ValueType_Computed), myCustomValue(0.0),
      myCustomStringValue(), myIsTextPositionFixed(Standard_False), mySpecialSymbol(' '),
      myDisplaySpecialSymbol(PrsDim_DisplaySpecialSymbol_No), myGeometryType(GeometryType_UndefShapes),
      myIsPlaneCustom(Standard_False), myFlyout(0.0), myIsGeometryValid(Standard_False), myKindOfDimension(theType) {}
...
TCollection_ExtendedString PrsDim_Dimension::GetValueString(Standard_Real& theWidth) const {
    ...
    if (myDrawer->DimensionAspect()->IsText3d()) {
        ...
        Font_BRepFont aFont;
        if (aFont.FindAndInit(aTextAspect->Aspect()->Font(), aTextAspect->Aspect()->GetTextFontAspect(),
                              aTextAspect->Height(), Font_StrictLevel_Any)) {
        ...
        if (Handle(Font_FTFont) aFont =
                Font_FTFont::FindAndCreate(aTextAspect->Aspect()->Font(), aTextAspect->Aspect()->GetTextFontAspect(),
                                           aFontParams, Font_StrictLevel_Any)) {
            for (NCollection_Utf8Iter anIter = anUTFString.Iterator(); *anIter != 0;) {
                Standard_Utf32Char aCurrChar = *anIter;
                Standard_Utf32Char aNextChar = *(++anIter);
...
// purpose  :
//=======================================================================
void PrsDim_Dimension::DrawArrow(const Handle(Prs3d_Presentation) & thePresentation, const gp_Pnt& theLocation,
...
// purpose  :
//=======================================================================
void PrsDim_Dimension::drawText(const Handle(Prs3d_Presentation) & thePresentation, const gp_Pnt& theTextPos,
                                const gp_Dir& theTextDir, const TCollection_ExtendedString& theText,
...
void PrsDim_Dimension::DrawExtension(const Handle(Prs3d_Presentation) & thePresentation,
                                     const Standard_Real theExtensionSize, const gp_Pnt& theExtensionStart,
                                     const gp_Dir& theExtensionDir, const TCollection_ExtendedString& theLabelString,
                                     const Standard_Real theLabelWidth, const Standard_Integer theMode,
                                     const Standard_Integer theLabelPosition) {
    ...
    Standard_Boolean isShortLine = !myDrawer->DimensionAspect()->IsText3d() || theLabelPosition & LabelPosition_VCenter;

    // compute graphical primitives and sensitives for extension line
    gp_Pnt anExtStart = theExtensionStart;
    gp_Pnt anExtEnd = !hasLabel || isShortLine ? ElCLib::Value(theExtensionSize, anExtensionLine)
...
void PrsDim_Dimension::DrawLinearDimension(const Handle(Prs3d_Presentation) & thePresentation,
                                           const Standard_Integer theMode, const gp_Pnt& theFirstPoint,
                                           const gp_Pnt& theSecondPoint, const Standard_Boolean theIsOneSide) {
    ...
    if (IsTextPositionCustom()) {
        if (!AdjustParametersForLinear(myFixedTextPosition, theFirstPoint, theSecondPoint, anExtensionSize,
                                       aHorisontalTextPos, myFlyout, myPlane, myIsPlaneCustom)) {
            throw Standard_ProgramError("Can not adjust plane to the custom label position.");
        }
    }

    FitTextAlignmentForLinear(theFirstPoint, theSecondPoint, theIsOneSide, aHorisontalTextPos, aLabelPosition,
                              isArrowsExternal);
    ...
    switch (aLabelPosition & LabelPosition_HMask) {
        ...
        case LabelPosition_HCenter: {
            ...
            gp_Pnt aTextPos =
                IsTextPositionCustom() ? myFixedTextPosition : (aCenterLineBegin.XYZ() + aCenterLineEnd.XYZ()) * 0.5;
            ...
            if (theMode == ComputeMode_All || theMode == ComputeMode_Line) {
                ...
                {
                    DrawExtension(thePresentation, aDimensionAspect->ArrowTailSize(), aFirstArrowEnd,
                                  aFirstExtensionDir, THE_EMPTY_LABEL, 0.0, theMode, LabelPosition_None);
                    if (!theIsOneSide) {
                        DrawExtension(thePresentation, aDimensionAspect->ArrowTailSize(), aSecondArrowEnd,
                                      aSecondExtensionDir, THE_EMPTY_LABEL, 0.0, theMode, LabelPosition_None);
                    }
                }
            }
            break;
        ...
        case LabelPosition_Left: {
            // add label on dimension or extension line to presentation
            {
                ...
                DrawExtension(thePresentation, anExtensionSize, isArrowsExternal ? aFirstArrowEnd : aFirstArrowBegin,
                              aFirstExtensionDir, aLabelString, aLabelWidth, theMode, aLabelPosition);
            }
        ...
        case LabelPosition_Right: {
            ...
            DrawExtension(thePresentation, anExtensionSize, isArrowsExternal ? aSecondArrowEnd : aSecondArrowBegin,
                          aSecondExtensionDir, aLabelString, aLabelWidth, theMode, aLabelPosition);
...
void PrsDim_Dimension::ComputeLinearFlyouts(const Handle(SelectMgr_Selection) & theSelection,
                                            const Handle(SelectMgr_EntityOwner) & theOwner, const gp_Pnt& theFirstPoint,
...
//=======================================================================
Standard_Boolean PrsDim_Dimension::CircleFromPlanarFace(const TopoDS_Face& theFace, Handle(Geom_Curve) & theCurve,
                                                        gp_Pnt& theFirstPoint, gp_Pnt& theLastPoint) {
...
//=======================================================================
Standard_Boolean PrsDim_Dimension::CircleFromEdge(const TopoDS_Edge& theEdge, gp_Circ& theCircle, gp_Pnt& theFirstPoint,
                                                  gp_Pnt& theLastPoint) {
...
// purpose  :
//=======================================================================
Standard_Boolean PrsDim_Dimension::InitCircularDimension(const TopoDS_Shape& theShape, gp_Circ& theCircle,
                                                         gp_Pnt& theMiddleArcPoint, Standard_Boolean& theIsClosed) {
    gp_Pln aPln;
...
void PrsDim_Dimension::ComputeSelection(const Handle(SelectMgr_Selection) & theSelection,
    ...
    if (aSelectionMode == PrsDim_DimensionSelectionMode_All || aSelectionMode == PrsDim_DimensionSelectionMode_Text) {
        ...
        gp_Ax2 aTextAxes(mySelectionGeom.TextPos, GetPlane().Axis().Direction(), mySelectionGeom.TextDir);
        ...
        if (myDrawer->DimensionAspect()->IsText3d()) {
            ...
            Handle(Poly_Triangulation) aRectanglePoly = new Poly_Triangulation(aRectanglePoints, aTriangles);
...
// purpose  :
//=======================================================================
void PrsDim_Dimension::PointsForArrow(const gp_Pnt& thePeakPnt, const gp_Dir& theDirection, const gp_Dir& thePlane,
                                      const Standard_Real theArrowLength, const Standard_Real theArrowAngle,
                                      gp_Pnt& theSidePnt1, gp_Pnt& theSidePnt2) {
...
// purpose  :
//=======================================================================
gp_Pnt PrsDim_Dimension::GetTextPositionForLinear(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
    ...
    Standard_Boolean isArrowsExternal = Standard_False;
    FitTextAlignmentForLinear(theFirstPoint, theSecondPoint, theIsOneSide, aDimensionAspect->TextHorizontalPosition(),
    ...
    switch (aLabelPosition & LabelPosition_HMask) {
        case LabelPosition_Left: {
            ...
            Standard_Real anOffset =
                isArrowsExternal ? anExtensionSize + aDimensionAspect->ArrowAspect()->Length() : anExtensionSize;
...
// purpose  :
//=======================================================================
Standard_Boolean PrsDim_Dimension::AdjustParametersForLinear(const gp_Pnt& theTextPos, const gp_Pnt& theFirstPoint,
                                                             const gp_Pnt& theSecondPoint,
                                                             Standard_Real& theExtensionSize,
                                                             Prs3d_DimensionTextHorizontalPosition& theAlignment,
                                                             Standard_Real& theFlyout, gp_Pln& thePlane,
    ...
    gp_Pnt aTextPosProj = theFirstPoint.Translated(gp_Vec(aTargetPointsDir) * aFirstToTextVec.Magnitude() * aCos);
    ...
    if (aFlyoutVector.Magnitude() > Precision::Confusion()) {
        theFlyout = gp_Dir(aFlyoutVector).IsOpposite(aPositiveFlyout, Precision::Angular()) ? -aFlyoutVector.Magnitude()
...
// purpose  :
//=======================================================================
void PrsDim_Dimension::FitTextAlignmentForLinear(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
