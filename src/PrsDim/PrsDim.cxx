...
//              and FALSE otherwise.
//=======================================================================
Standard_Boolean PrsDim::Nearest(const Handle(Geom_Curve) & theCurve, const gp_Pnt& thePoint,
                                 const gp_Pnt& theFirstPoint, const gp_Pnt& theLastPoint, gp_Pnt& theNearestPoint) {
    ...
    if (theNearestPoint.Distance(theFirstPoint) > aLength || theNearestPoint.Distance(theLastPoint) > aLength) {
...
//=======================================================================
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theEdge, Handle(Geom_Curve) & theCurve, gp_Pnt& theFirstPnt,
                                         gp_Pnt& theLastPnt) {
...
//=======================================================================
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theEdge, Handle(Geom_Curve) & theCurve, gp_Pnt& theFirstPnt,
                                         gp_Pnt& theLastPnt, Standard_Boolean& theIsInfinite) {
...
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theEdge, Handle(Geom_Curve) & theCurve, gp_Pnt& theFirstPnt,
                                         gp_Pnt& theLastPnt, Handle(Geom_Curve) & theExtCurve,
                                         Standard_Boolean& theIsInfinite, Standard_Boolean& theIsOnPlane,
    ...
    if (Handle(Geom_Line) aLine = Handle(Geom_Line)::DownCast(theExtCurve)) {
        theIsOnPlane = thePlane->Pln().Contains(aLine->Lin(), Precision::Confusion(), Precision::Angular());
    } else if (Handle(Geom_Circle) aCircle = Handle(Geom_Circle)::DownCast(theExtCurve)) {
        gp_Ax3 aCircPos(aCircle->Position());
        theIsOnPlane = aCircPos.IsCoplanar(thePlane->Pln().Position(), Precision::Confusion(), Precision::Angular());
    }
    ...
    theCurve = GeomProjLib::ProjectOnPlane(theCurve, thePlane, thePlane->Pln().Axis().Direction(), Standard_False);
...
// purpose  :
//=======================================================================
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge,
                                         Handle(Geom_Curve) & theFirstCurve, Handle(Geom_Curve) & theSecondCurve,
                                         gp_Pnt& theFirstPnt1, gp_Pnt& theLastPnt1, gp_Pnt& theFirstPnt2,
                                         gp_Pnt& theLastPnt2, const Handle(Geom_Plane) & thePlane) {
    if (thePlane.IsNull()) {
        return Standard_False;
    }
    ...
    theFirstCurve =
        GeomProjLib::ProjectOnPlane(theFirstCurve, thePlane, thePlane->Pln().Axis().Direction(), Standard_False);

    theSecondCurve =
        GeomProjLib::ProjectOnPlane(theSecondCurve, thePlane, thePlane->Pln().Axis().Direction(), Standard_False);
...
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge,
                                         Handle(Geom_Curve) & theFirstCurve, Handle(Geom_Curve) & theSecondCurve,
                                         gp_Pnt& theFirstPnt1, gp_Pnt& theLastPnt1, gp_Pnt& theFirstPnt2,
                                         gp_Pnt& theLastPnt2, Standard_Boolean& theIsInfinite1,
...
//=======================================================================
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecondEdge,
                                         Standard_Integer& theExtIndex, Handle(Geom_Curve) & theFirstCurve,
                                         Handle(Geom_Curve) & theSecondCurve, gp_Pnt& theFirstPnt1, gp_Pnt& theLastPnt1,
                                         gp_Pnt& theFirstPnt2, gp_Pnt& theLastPnt2, Handle(Geom_Curve) & theExtCurve,
                                         Standard_Boolean& theIsInfinite1, Standard_Boolean& theIsInfinite2,
                                         const Handle(Geom_Plane) & thePlane) {
    if (thePlane.IsNull()) {
        return Standard_False;
    }
    ...
    theFirstCurve = Handle(Geom_Curve)::DownCast(aFirstAdaptor.Curve().Curve()->Transformed(aFirstAdaptor.Trsf()));
    theSecondCurve = Handle(Geom_Curve)::DownCast(aSecondAdaptor.Curve().Curve()->Transformed(aSecondAdaptor.Trsf()));
    ...
    if (!isFirstOnPlane && isSecondOnPlane) { // curve 2 only in the plane
        theExtIndex = 1;
        theExtCurve = aFirstSaved;
    } else if (isFirstOnPlane && !isSecondOnPlane) { // curve 1 only in the plane
        theExtIndex = 2;
...
//           Return TRUE if ok
//=======================================================================
Standard_Boolean PrsDim::ComputeGeomCurve(Handle(Geom_Curve) & aCurve, const Standard_Real first1,
                                          const Standard_Real last1, gp_Pnt& FirstPnt1, gp_Pnt& LastPnt1,
                                          const Handle(Geom_Plane) & aPlane, Standard_Boolean& isOnPlane) {
...
//=======================================================================
Standard_Boolean PrsDim::ComputeGeometry(const TopoDS_Vertex& aVertex, gp_Pnt& point, const Handle(Geom_Plane) & aPlane,
...
//=======================================================================
Standard_Boolean PrsDim::GetPlaneFromFace(const TopoDS_Face& aFace, gp_Pln& aPlane, Handle(Geom_Surface) & aSurf,
                                          PrsDim_KindOfSurface& aSurfType, Standard_Real& Offset)

{
    ...
    if (Result == Standard_False) {
        if (isOffset) {
            ...
            if (TheType == STANDARD_TYPE(Geom_CylindricalSurface) || TheType == STANDARD_TYPE(Geom_ConicalSurface) ||
                TheType == STANDARD_TYPE(Geom_SphericalSurface) || TheType == STANDARD_TYPE(Geom_ToroidalSurface)) {
                aSurf = Handle(Geom_OffsetSurface)::DownCast(aSurf)->Surface();
            } else {
...
// purpose  :
//=======================================================================
void PrsDim::InitFaceLength(const TopoDS_Face& theFace, gp_Pln& thePlane, Handle(Geom_Surface) & theSurface,
                            PrsDim_KindOfSurface& theSurfaceType, Standard_Real& theOffset) {
...
// purpose  :
//=======================================================================
Standard_Boolean PrsDim::InitAngleBetweenPlanarFaces(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace,
                                                     gp_Pnt& theCenter, gp_Pnt& theFirstAttach, gp_Pnt& theSecondAttach,
...
Standard_Boolean PrsDim::InitAngleBetweenCurvilinearFaces(
    const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace, const PrsDim_KindOfSurface theFirstSurfType,
    const PrsDim_KindOfSurface theSecondSurfType, gp_Pnt& theCenter, gp_Pnt& theFirstAttach, gp_Pnt& theSecondAttach,
    const Standard_Boolean theIsFirstPointSet) {
    ...
    if (theSecondSurfType == PrsDim_KOS_Cylinder) {
        ...
        Standard_Real aSecondU = aCylinder->Cylinder().XAxis().Direction().Angle(
            gce_MakeDir(ProjectPointOnLine(theFirstAttach, gp_Lin(aCylinder->Cylinder().Axis())), theFirstAttach));
    ...
    if (!(aFirstLine->Lin().Direction().IsEqual(aSecondLine->Lin().Direction(), Precision::Angular())) &&
        ...
        if (theCenter.SquareDistance(theFirstAttach) <= SquareTolerance) {
            ...
            if (theFirstSurfType == PrsDim_KOS_Cylinder) {
                ElSLib::Parameters((Handle(Geom_CylindricalSurface)::DownCast(aFirstSurf))->Cylinder(), theFirstAttach,
                                   anU, aV);

                theFirstAttach =
                    ElSLib::Value(aFirstU, aV, (Handle(Geom_CylindricalSurface)::DownCast(aFirstSurf))->Cylinder());
            } else if (theFirstSurfType == PrsDim_KOS_Cone) {
                ElSLib::Parameters((Handle(Geom_ConicalSurface)::DownCast(aFirstSurf))->Cone(), theFirstAttach, anU,
                                   aV);
                theFirstAttach =
...
// purpose  :
//=======================================================================
void PrsDim::InitLengthBetweenCurvilinearFaces(const TopoDS_Face& theFirstFace, const TopoDS_Face& theSecondFace,
                                               Handle(Geom_Surface) & theFirstSurf,
                                               Handle(Geom_Surface) & theSecondSurf, gp_Pnt& theFirstAttach,
                                               gp_Pnt& theSecondAttach, gp_Dir& theDirOnPlane) {
...
gp_Pnt PrsDim::TranslatePointToBound(const gp_Pnt& aPoint, const gp_Dir& aDir, const Bnd_Box& aBndBox) {
    if (aBndBox.IsOut(aPoint))
        ...
        for (Standard_Integer i = 1; i <= 3; i++) {
            if (Abs(Dir(i)) <= gp::Resolution()) continue;
...
Standard_Boolean PrsDim::InDomain(const Standard_Real fpar, const Standard_Real lpar, const Standard_Real para) {
    if (fpar >= 0.) {
        if (lpar > fpar)
...
Standard_Real PrsDim::DistanceFromApex(const gp_Elips& elips, const gp_Pnt& Apex, const Standard_Real par) {
    Standard_Real dist;
    Standard_Real parApex = ElCLib::Parameter(elips, Apex);
    if (parApex == 0.0 || parApex == M_PI) { // Major case
...
gp_Pnt PrsDim::NearestApex(const gp_Elips& elips, const gp_Pnt& pApex, const gp_Pnt& nApex, const Standard_Real fpara,
                           const Standard_Real lpara, Standard_Boolean& IsInDomain) {
    ...
    if (InDomain(fpara, lpara, parP))
        ...
        if (InDomain(fpara, lpara, parN))
            ...
            Standard_Real posd = Min(DistanceFromApex(elips, pApex, fpara), DistanceFromApex(elips, pApex, lpara));
            Standard_Real negd = Min(DistanceFromApex(elips, nApex, fpara), DistanceFromApex(elips, nApex, lpara));
            if (posd < negd)
                EndOfArrow = pApex;
            else
                EndOfArrow = nApex;
...
void PrsDim::ComputeProjEdgePresentation(const Handle(Prs3d_Presentation) & aPresentation,
                                         const Handle(Prs3d_Drawer) & aDrawer, const TopoDS_Edge& anEdge,
                                         const Handle(Geom_Curve) & ProjCurve, const gp_Pnt& FirstP,
                                         const gp_Pnt& LastP, const Quantity_NameOfColor aColor,
                                         const Standard_Real aWidth, const Aspect_TypeOfLine aProjTOL,
...
void PrsDim::ComputeProjVertexPresentation(const Handle(Prs3d_Presentation) & aPresentation,
                                           const Handle(Prs3d_Drawer) & aDrawer, const TopoDS_Vertex& aVertex,
                                           const gp_Pnt& ProjPoint, const Quantity_NameOfColor aColor,
                                           const Standard_Real aWidth, const Aspect_TypeOfMarker aProjTOM,
                                           const Aspect_TypeOfLine aCallTOL) {
