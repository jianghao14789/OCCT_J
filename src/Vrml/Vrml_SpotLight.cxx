...
Vrml_SpotLight::Vrml_SpotLight()
    : myOnOff(Standard_True), myIntensity(1), myColor(Quantity_NOC_WHITE), myLocation(0, 0, 1), myDirection(0, 0, -1),
      myDropOffRate(0), myCutOffAngle(0.785398) {
    //
}

Vrml_SpotLight::Vrml_SpotLight(const Standard_Boolean aOnOff, const Standard_Real aIntensity,
                               const Quantity_Color& aColor, const gp_Vec& aLocation, const gp_Vec& aDirection,
                               const Standard_Real aDropOffRate, const Standard_Real aCutOffAngle) {
    myOnOff = aOnOff;
...
Standard_OStream& Vrml_SpotLight::Print(Standard_OStream& anOStream) const {
    ...
    if (Abs(myColor.Red() - 1) > 0.0001 || Abs(myColor.Green() - 1) > 0.0001 || Abs(myColor.Blue() - 1) > 0.0001) {
    ...
    if (Abs(myLocation.X() - 0) > 0.0001 || Abs(myLocation.Y() - 0) > 0.0001 || Abs(myLocation.Z() - 1) > 0.0001) {
        anOStream << "    location\t";
        anOStream << myLocation.X() << " " << myLocation.Y() << " " << myLocation.Z() << "\n";
    }

    if (Abs(myDirection.X() - 0) > 0.0001 || Abs(myDirection.Y() - 0) > 0.0001 || Abs(myDirection.Z() + 1) > 0.0001) {
        anOStream << "    direction\t";
