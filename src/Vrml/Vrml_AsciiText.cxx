...
Vrml_AsciiText::Vrml_AsciiText(const Handle(TColStd_HArray1OfAsciiString) & aString, const Standard_Real aSpacing,
                               const Vrml_AsciiTextJustification aJustification, const Standard_Real aWidth) {
    myString = aString;
    mySpacing = aSpacing;
...
Standard_OStream& Vrml_AsciiText::Print(Standard_OStream& anOStream) const {
    ...
    if (myString->Length() != 1 || myString->Value(i) != "") {
        ...
        for (i = myString->Lower(); i <= myString->Upper(); i++) {
            ...
            if (i < myString->Length()) anOStream << ",\n\t";
        }
        anOStream << " ]\n";
    }
