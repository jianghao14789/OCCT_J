...
Standard_OStream& Vrml_Material::Print(Standard_OStream& anOStream) const {
    ...
    if (myAmbientColor->Length() != 1 || Abs(myAmbientColor->Value(myAmbientColor->Lower()).Red() - 0.2) > 0.0001 ||
        ...
        for (i = myAmbientColor->Lower(); i <= myAmbientColor->Upper(); i++) {
            ...
            if (i < myAmbientColor->Length()) anOStream << ",\n\t"; // ,,,,,,,,,,
        }
        anOStream << " ]\n";
    }

    if (myDiffuseColor->Length() != 1 || Abs(myDiffuseColor->Value(myDiffuseColor->Lower()).Red() - 0.8) > 0.0001 ||
        ...
        for (i = myDiffuseColor->Lower(); i <= myDiffuseColor->Upper(); i++) {
            ...
            anOStream << aColor_sRGB.r() << ' ' << aColor_sRGB.g() << ' ' << aColor_sRGB.b();
            if (i < myDiffuseColor->Length()) anOStream << ",\n\t";
        }
        anOStream << " ]\n";
    }

    if (mySpecularColor->Length() != 1 || Abs(mySpecularColor->Value(mySpecularColor->Lower()).Red() - 0) > 0.0001 ||
        ...
        for (i = mySpecularColor->Lower(); i <= mySpecularColor->Upper(); i++) {
            ...
            anOStream << aColor_sRGB.r() << ' ' << aColor_sRGB.g() << ' ' << aColor_sRGB.b();
            if (i < mySpecularColor->Length()) anOStream << ",\n\t";
        }
        anOStream << " ]\n";
    }

    if (myEmissiveColor->Length() != 1 || Abs(myEmissiveColor->Value(myEmissiveColor->Lower()).Red() - 0) > 0.0001 ||
        ...
        for (i = myEmissiveColor->Lower(); i <= myEmissiveColor->Upper(); i++) {
            ...
            anOStream << aColor_sRGB.r() << ' ' << aColor_sRGB.g() << ' ' << aColor_sRGB.b();
            if (i < myEmissiveColor->Length()) anOStream << ",\n\t";
        }
        anOStream << " ]\n";
    }
    ...
    if (myShininess->Length() != 1 || Abs(myShininess->Value(myShininess->Lower()) - 0.2) > 0.0001) {
        ...
        for (i = myShininess->Lower(); i <= myShininess->Upper(); i++) {
            ...
            if (i < myShininess->Length()) anOStream << ", ";
        }
        anOStream << " ]\n";
    }
    ...
    if (myTransparency->Length() != 1 || Abs(myTransparency->Value(myTransparency->Lower()) - 0) > 0.0001) {
        ...
        for (i = myTransparency->Lower(); i <= myTransparency->Upper(); i++) {
            ...
            if (i < myTransparency->Length()) anOStream << ", ";
        }
        anOStream << " ]\n";
    }
    anOStream << "}\n";
