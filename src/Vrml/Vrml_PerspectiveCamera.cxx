...
Vrml_PerspectiveCamera::Vrml_PerspectiveCamera() : myFocalDistance(5), myHeightAngle(0.785398) {
...
Vrml_PerspectiveCamera::Vrml_PerspectiveCamera(const gp_Vec& aPosition, const Vrml_SFRotation& aOrientation,
                                               const Standard_Real aFocalDistance, const Standard_Real aHeightAngle) {
...
Standard_OStream& Vrml_PerspectiveCamera::Print(Standard_OStream& anOStream) const {
    ...
    if (Abs(myPosition.X() - 0) > 0.0001 || Abs(myPosition.Y() - 0) > 0.0001 || Abs(myPosition.Z() - 1) > 0.0001) {
        anOStream << "    position\t\t";
