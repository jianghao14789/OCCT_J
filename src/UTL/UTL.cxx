...
TCollection_ExtendedString UTL::xgetenv(const Standard_CString aCString) {
    ...
    if (!theValue.IsEmpty()) x = TCollection_ExtendedString(theValue);
    return x;
}
...
TCollection_ExtendedString UTL::Extension(const TCollection_ExtendedString& aFileName) {
    ...
    if (theExtension.Value(1) == '.') theExtension.Remove(1, 1);
    return TCollection_ExtendedString(theExtension);
}

Storage_Error UTL::OpenFile(const Handle(Storage_BaseDriver) & aDriver, const TCollection_ExtendedString& aFileName,
...
TCollection_ExtendedString UTL::Value(const Handle(Resource_Manager) & aResourceManager,
    ...
    return TCollection_ExtendedString(aResourceManager->Value(aResourceNameU.ToCString()), Standard_True);
}
