...
static Standard_Character EvolutionToChar(const TNaming_Evolution theEvol) {
    switch (theEvol) {
        ...
        case TNaming_REPLACE:
            return 'M'; // for compatibility case TNaming_REPLACE      : return 'R';
        default:
...
//=======================================================================
static void TranslateTo(const TopoDS_Shape& theShape, BinObjMgt_Persistent& theResult, BinTools_ShapeSet* theShapeSet) {
...
//=======================================================================
static int TranslateFrom(const BinObjMgt_Persistent& theSource, TopoDS_Shape& theResult,
    ...
    if (aShapeID <= 0 || aShapeID > theShapeSet->NbShapes()) return 1;
    Ok = theSource >> aLocID;
    if (!Ok) return 1;
...
BinMNaming_NamedShapeDriver::BinMNaming_NamedShapeDriver(const Handle(Message_Messenger) & theMsgDriver)
    : BinMDF_ADriver(theMsgDriver, STANDARD_TYPE(TNaming_NamedShape)->Name()), myShapeSet(NULL),
      myWithTriangles(Standard_False), myWithNormals(Standard_False), myIsQuickPart(Standard_False) {}
...
Standard_Boolean BinMNaming_NamedShapeDriver::Paste(const BinObjMgt_Persistent& theSource,
    ...
    for (NCollection_List<TopoDS_Shape>::Iterator anOldIt(anOldShapes), aNewIt(aNewShapes);
         anOldIt.More() && aNewIt.More(); anOldIt.Next(), aNewIt.Next()) {
        switch (anEvol) {
            case TNaming_PRIMITIVE:
            ...
            case TNaming_SELECTED:
                ...
                break;
            case TNaming_REPLACE:
                aBuilder.Modify(anOldIt.Value(),
                                aNewIt.Value()); // for compatibility aBuilder.Replace(anOldShape, aNewShape);
                break;
            default:
                throw Standard_DomainError("TNaming_Evolution:: Evolution Unknown");
        }
...
void BinMNaming_NamedShapeDriver::Paste(const Handle(TDF_Attribute) & theSource, BinObjMgt_Persistent& theTarget,
...
void BinMNaming_NamedShapeDriver::WriteShapeSection(Standard_OStream& theOS, const Standard_Integer theDocVer,
...
void BinMNaming_NamedShapeDriver::ReadShapeSection(Standard_IStream& theIS, const Message_ProgressRange& theRange) {
    myIsQuickPart = Standard_False;
