    ...
    if (ok) {
        ...
        if (ok) {
            ...
            if (theRelocTable.GetHeaderData()->StorageVersion().IntegerValue() >= TDocStd_FormatVersion_VERSION_4) {
                ...
                if (ok) {
#ifdef OCCT_DEBUG
                    std::cout << "NamingDriver:: Retrieved Context Label = " << entry << " Ok = " << theSource.IsOK()
                              << std::endl;
#endif
                    ...
                    if (!entry.IsEmpty() && !entry.IsEqual(TCollection_AsciiString(NULL_ENTRY))) {
                        ...
                        if (!tLab.IsNull()) aName.ContextLabel(tLab);
                    }
                }
...
void BinMNaming_NamingDriver::Paste(const Handle(TDF_Attribute) & theSource, BinObjMgt_Persistent& theTarget,
    ...
    if (aNbArgs > 0) {
        ...
        for (TNaming_ListIteratorOfListOfNamedShape it(aName.Arguments()); it.More(); it.Next()) {
            ...
            if (!anArg.IsNull()) {
                ...
                if (anIndx == 0) anIndx = theRelocTable.Add(anArg);
            }
    ...
    if (!aStopNS.IsNull()) {
        ...
        if (anIndx == 0) anIndx = theRelocTable.Add(aStopNS);
    } else
        anIndx = 0;
    theTarget << anIndx;
    ...
    if (!aName.ContextLabel().IsNull()) TDF_Tool::Entry(aName.ContextLabel(), entry);
    theTarget << entry;
