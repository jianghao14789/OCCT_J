...
//! The image can be stored as path to image file, as file path with the given offset and as a data buffer of encoded
//! image.
...
public:
    ...
    Standard_EXPORT explicit Image_Texture(const TCollection_AsciiString& theFileName, int64_t theOffset,
                                           int64_t theLength);
    ...
    Standard_EXPORT virtual Handle(Image_CompressedPixMap)
        ReadCompressedImage(const Handle(Image_SupportedFormats) & theSupported) const;
...
public: //! @name hasher interface
    ...
    static int HashCode(const Handle(Image_Texture) & theTexture, const int theUpper) {
        return !theTexture.IsNull() ? TCollection_AsciiString::HashCode(theTexture->myTextureId, theUpper) : 0;
    }
...
protected:
    ...
    Standard_EXPORT virtual Handle(Image_PixMap)
        loadImageOffset(const TCollection_AsciiString& thePath, int64_t theOffset, int64_t theLength) const;

    //! Read image from buffer.
    Standard_EXPORT virtual Handle(Image_PixMap)
        loadImageBuffer(const Handle(NCollection_Buffer) & theBuffer, const TCollection_AsciiString& theId) const;
