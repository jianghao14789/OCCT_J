...
Handle(Image_CompressedPixMap) Image_DDSParser::Load(const Handle(Image_SupportedFormats) & theSupported,
                                                     ...
                                                     const Standard_Integer theFaceIndex, const int64_t theFileOffset) {
    ...
    if (theFaceIndex >= aDef->NbFaces() || aDef->FaceBytes() == 0) {
        Message::SendFail(TCollection_AsciiString("DDS Reader error - invalid face index #") + theFaceIndex +
                          " within file\n" + theFile);
        return Handle(Image_CompressedPixMap)();
    }
    ...
    if (aNbReadBytes < aDef->FaceBytes()) {
        Message::SendFail(TCollection_AsciiString("DDS Reader error - unable to read face #") + theFaceIndex +
                          " data from file\n" + theFile);
        return Handle(Image_CompressedPixMap)();
    }
...
Handle(Image_CompressedPixMap) Image_DDSParser::parseHeader(const DDSFileHeader& theHeader) {
    if (theHeader.Size != 124 || theHeader.Width == 0 || theHeader.Height == 0 || theHeader.PixelFormatDef.Size != 32) {
        return Handle(Image_CompressedPixMap)();
    }
