...
Image_VideoRecorder::Image_VideoRecorder()
    : myAVContext(NULL), myVideoStream(NULL), myVideoCodec(NULL), myFrame(NULL), myScaleCtx(NULL), myFrameCount(0) {
    myFrameRate.num = 1;
    myFrameRate.den = 30;
...
void Image_VideoRecorder::Close() {
    ...
    // Write the trailer, if any. The trailer must be written before you close the CodecContexts open when you wrote the
    // header; otherwise av_write_trailer() may try to use memory that was freed on av_codec_close().
    if (myFrameCount != 0) {
...
// purpose  :
//=============================================================================
Standard_Boolean Image_VideoRecorder::Open(const char* theFileName, const Image_VideoParams& theParams) {
#ifdef HAVE_FFMPEG
    Close();
    ...
    avformat_alloc_output_context2(&myAVContext, NULL, theParams.Format.IsEmpty() ? NULL : theParams.Format.ToCString(),
                                   theFileName);
    if (myAVContext == NULL) {
...
Standard_Boolean Image_VideoRecorder::openVideoCodec(const Image_VideoParams& theParams) {
    ...
    if (!theParams.PixelFormat.IsEmpty()) {
        ...
        if (aPixFormat == AV_PIX_FMT_NONE) {
            ::Message::SendFail(TCollection_AsciiString("Error: unknown pixel format has been specified '") +
    ...
    aResAv =
        av_image_alloc(myFrame->data, myFrame->linesize, aCodecCtx->width, aCodecCtx->height, aCodecCtx->pix_fmt, 1);
    if (aResAv < 0) {
        memset(myFrame->data, 0, sizeof(myFrame->data));
        memset(myFrame->linesize, 0, sizeof(myFrame->linesize));
        ::Message::SendFail(TCollection_AsciiString("Error: can not allocate picture ") + aCodecCtx->width + "x" +
                            aCodecCtx->height + ", " + formatAvError(aResAv));
        return Standard_False;
    }
    ...
    if (!myImgSrcRgba.InitZero(Image_Format_RGBA, aCodecCtx->width, aCodecCtx->height, aStride)) {
        ::Message::SendFail(TCollection_AsciiString("Error: can not allocate RGBA32 picture ") + aCodecCtx->width +
                            "x" + aCodecCtx->height);
        return Standard_False;
    }

    myScaleCtx = sws_getContext(aCodecCtx->width, aCodecCtx->height, AV_PIX_FMT_RGBA, aCodecCtx->width,
                                aCodecCtx->height, aCodecCtx->pix_fmt, SWS_BICUBIC, NULL, NULL, NULL);
    if (myScaleCtx == NULL) {
...
Standard_Boolean Image_VideoRecorder::writeVideoFrame(const Standard_Boolean theToFlush) {
    ...
    if (!theToFlush) {
        ...
        sws_scale(myScaleCtx, aSrcData, aSrcLinesize, 0, aCodecCtx->height, myFrame->data, myFrame->linesize);
    }
    ...
    av_init_packet(&aPacket);
    {
        ...
        if (isGotPacket) {
            ...
            aPacket.pts = av_rescale_q_rnd(aPacket.pts, aTimeBase, myVideoStream->time_base,
                                           AVRounding(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
            aPacket.dts = av_rescale_q_rnd(aPacket.dts, aTimeBase, myVideoStream->time_base,
                                           AVRounding(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
