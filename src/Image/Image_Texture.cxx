...
Image_Texture::Image_Texture(const TCollection_AsciiString& theFileName)
    : myImagePath(theFileName), myOffset(-1), myLength(-1) {
...
// Purpose  :
// ================================================================
Image_Texture::Image_Texture(const TCollection_AsciiString& theFileName, int64_t theOffset, int64_t theLength)
    : myImagePath(theFileName), myOffset(theOffset), myLength(theLength) {
    // share textures with unique file paths
    if (!theFileName.IsEmpty()) {
        char aBuff[60];
...
// Purpose  :
// ================================================================
Image_Texture::Image_Texture(const Handle(NCollection_Buffer) & theBuffer, const TCollection_AsciiString& theId)
    : myBuffer(theBuffer), myOffset(-1), myLength(-1) {
    if (!theId.IsEmpty()) {
...
// Purpose  :
// ================================================================
Handle(Image_CompressedPixMap) Image_Texture::ReadCompressedImage(const Handle(Image_SupportedFormats) &
                                                                  theSupported) const {
...
// Purpose  :
// ================================================================
Handle(Image_PixMap) Image_Texture::loadImageOffset(const TCollection_AsciiString& thePath, int64_t theOffset,
...
TCollection_AsciiString Image_Texture::ProbeImageFileFormat() const {
    ...
    if (!myBuffer.IsNull()) {
        ...
        const Handle(OSD_FileSystem) & aFileSystem = OSD_FileSystem::DefaultFileSystem();
        opencascade::std::shared_ptr<std::istream> aFileIn =
...
Standard_Boolean Image_Texture::WriteImage(const TCollection_AsciiString& theFile) {
    const Handle(OSD_FileSystem) & aFileSystem = OSD_FileSystem::DefaultFileSystem();
    opencascade::std::shared_ptr<std::ostream> aFileOut =
...
// Purpose  :
// ================================================================
Standard_Boolean Image_Texture::WriteImage(std::ostream& theStream, const TCollection_AsciiString& theFile) {
    if (!myBuffer.IsNull()) {
