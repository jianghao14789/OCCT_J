...
namespace {
#ifdef HAVE_FREEIMAGE
static Image_Format convertFromFreeFormat(FREE_IMAGE_TYPE theFormatFI, FREE_IMAGE_COLOR_TYPE theColorTypeFI,
...
public:
    //! Simulate fread().
    static unsigned int DLL_CALLCONV readProc(void* theBuffer, unsigned int theSize, unsigned int theCount,
                                              fi_handle theHandle) {
    ...
    //! Simulate fwrite().
    static unsigned int DLL_CALLCONV writeProc(void* theBuffer, unsigned int theSize, unsigned int theCount,
                                               fi_handle theHandle) {
        Image_FreeImageStream* aThis = (Image_FreeImageStream*)theHandle;
...
static WICPixelFormatGUID convertToWicFormat(Image_Format theFormat) {
    switch (theFormat) {
        ...
        case Image_Format_Alpha:
            return GUID_WICPixelFormat8bppGray; // GUID_WICPixelFormat8bppAlpha
...
// purpose  :
// =======================================================================
Image_AlienPixMap::Image_AlienPixMap() : myLibImage(NULL) {
    SetTopDown(false);
}
...
// purpose  :
// =======================================================================
bool Image_AlienPixMap::InitWrapper(Image_Format, Standard_Byte*, const Standard_Size, const Standard_Size,
                                    const Standard_Size) {
    Clear();
    return false;
}
...
// =======================================================================
#ifdef HAVE_FREEIMAGE
bool Image_AlienPixMap::InitTrash(Image_Format thePixelFormat, const Standard_Size theSizeX,
    ...
    Image_Format aFormat = convertFromFreeFormat(FreeImage_GetImageType(anImage), FreeImage_GetColorType(anImage),
    ...
    Image_PixMap::InitWrapper(aFormat, FreeImage_GetBits(anImage), FreeImage_GetWidth(anImage),
                              FreeImage_GetHeight(anImage), FreeImage_GetPitch(anImage));
    SetTopDown(false);
...
// =======================================================================
#ifdef HAVE_FREEIMAGE
bool Image_AlienPixMap::Load(const Standard_Byte* theData, Standard_Size theLength,
    ...
    if (anImage != NULL) {
        aFormat = convertFromFreeFormat(FreeImage_GetImageType(anImage), FreeImage_GetColorType(anImage),
                                        FreeImage_GetBPP(anImage));
...
bool Image_AlienPixMap::Load(const Standard_Byte* theData, Standard_Size theLength,
    ...
    if (theData != NULL) {
        ...
        if (aWicImgFactory->CreateDecoderFromStream(aWicStream.get(), NULL, WICDecodeMetadataCacheOnDemand,
        ...
        if (aWicImgFactory->CreateDecoderFromFilename(aFileNameW.ToWideString(), NULL, GENERIC_READ,
    ...
    if (aPixelFormat == Image_Format_UNKNOWN) {
        ...
        if (aWicImgFactory->CreateFormatConverter(&aWicConvertedFrame.ChangePtr()) != S_OK ||
            aWicConvertedFrame->Initialize(aWicFrameDecode.get(), convertToWicFormat(aPixelFormat),
                                           WICBitmapDitherTypeNone, NULL, 0.0f, WICBitmapPaletteTypeCustom) != S_OK) {
...
bool Image_AlienPixMap::Load(const Standard_Byte*, Standard_Size, const TCollection_AsciiString&) {
    Clear();
    Message::SendFail("Error: no image library available");
    return false;
}
...
bool Image_AlienPixMap::Save(const TCollection_AsciiString& theFileName) {
    ...
    if (aWicEncoder->CreateNewFrame(&aWicFrameEncode.ChangePtr(), NULL) != S_OK ||
        aWicFrameEncode->Initialize(NULL) != S_OK || aWicFrameEncode->SetSize((UINT)SizeX(), (UINT)SizeY()) != S_OK ||
    ...
    if (IsTopDown()) {
        if (aWicFrameEncode->WritePixels((UINT)SizeY(), (UINT)SizeRowBytes(), (UINT)SizeBytes(), (BYTE*)Data()) !=
            S_OK) {
