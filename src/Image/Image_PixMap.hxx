...
public: // high-level API
    ...
    //! @param theToLinearize [in] when TRUE, the color stored in non-linear color space (e.g. Image_Format_RGB) will be
    //! linearized
    //! @return the pixel color
    Standard_EXPORT Quantity_ColorRGBA PixelColor(const Standard_Integer theX, const Standard_Integer theY,
    ...
    //! @param theToDeLinearize [in] when TRUE, the gamma correction will be applied for storing in non-linear color
    //! space (e.g. Image_Format_RGB)
    ...
    //! @param theToDeLinearize [in] when TRUE, the gamma correction will be applied for storing in non-linear color
    //! space (e.g. Image_Format_RGB)
    Standard_EXPORT void SetPixelColor(const Standard_Integer theX, const Standard_Integer theY,
    ...
    Standard_EXPORT virtual bool InitWrapper(Image_Format thePixelFormat, Standard_Byte* theDataPtr,
                                             const Standard_Size theSizeX, const Standard_Size theSizeY,
                                             const Standard_Size theSizeRowBytes = 0);

    //! Initialize image plane with required dimensions.
    //! Memory will be left uninitialized (performance trick).
    Standard_EXPORT virtual bool InitTrash(Image_Format thePixelFormat, const Standard_Size theSizeX,
                                           const Standard_Size theSizeY, const Standard_Size theSizeRowBytes = 0);
    ...
    Standard_EXPORT bool InitZero(Image_Format thePixelFormat, const Standard_Size theSizeX,
                                  const Standard_Size theSizeY, const Standard_Size theSizeRowBytes = 0,
...
public: //! @name low-level API for batch-processing (pixels reading / comparison / modification)
    ...
    inline const ColorType_t& Value(const Standard_Size theRow, const Standard_Size theCol) const {
    ...
    //! This method does not perform any type checks - use on own risk (check Format() before)!
    template <typename ColorType_t>
    inline ColorType_t& ChangeValue(const Standard_Size theRow, const Standard_Size theCol) {
