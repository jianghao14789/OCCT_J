...
// purpose
//=========================================================================
void VrmlConverter_WFShape::Add(Standard_OStream& anOStream, const TopoDS_Shape& aShape,
        ...
        for (Tool.InitFace(); Tool.MoreFace(); Tool.NextFace()) {
            ...
            if (Tool.HasSurface()) {
                ...
                if (isoU || isoV) {
                    ...
                    VrmlConverter_WFRestrictedFace::Add(anOStream, HS, isoU, isoV, aDrawer->UIsoAspect()->Number(),
                                                        aDrawer->VIsoAspect()->Number(), aDrawer);
                }
            }
        }
    }
            ...
            for (Tool.InitFace(); Tool.MoreFace(); Tool.NextFace()) {
                ...
                if (Tool.HasSurface()) {
                    ...
                    if (isoU) {
                        S.Initialize(Tool.GetFace());
                        Handle(BRepAdaptor_Surface) HS = new BRepAdaptor_Surface(S);
                        VrmlConverter_WFRestrictedFace::Add(anOStream, HS, isoU, Standard_False,
                                                            aDrawer->UIsoAspect()->Number(), 0, aDrawer);
                    }
                }
            }
        }
            ...
            for (Tool.InitFace(); Tool.MoreFace(); Tool.NextFace()) {
                ...
                if (Tool.HasSurface()) {
                    ...
                    if (isoV) {
                        S.Initialize(Tool.GetFace());
                        Handle(BRepAdaptor_Surface) HS = new BRepAdaptor_Surface(S);
                        VrmlConverter_WFRestrictedFace::Add(anOStream, HS, Standard_False, isoV, 0,
                                                            aDrawer->VIsoAspect()->Number(), aDrawer);
                    }
                }
            }
