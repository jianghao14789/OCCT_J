...
TCollection_AsciiString XSDRAW_CommandPart(Standard_Integer argc, const char** argv, const Standard_Integer argf) {
...
static Standard_Integer igesbrep(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (ctl.IsNull()) ctl = Handle(IGESControl_Controller)::DownCast(XSDRAW::Controller());

    TCollection_AsciiString fnom, rnom;

    Standard_Boolean modfic = XSDRAW::FileAndVar(argv[1], argv[2], "IGESBREP", fnom, rnom);
    if (modfic)
        di << " File IGES to read : " << fnom.ToCString() << "\n";
    else
        di << " Model taken from the session : " << fnom.ToCString() << "\n";
    ...
    while (modepri) {
        ...
        if (modepri == 0) { // fin
            ...
            di << "spline_continuity (read) : " << Interface_Static::IVal("read.iges.bspline.continuity")
            ...
            std::cout << "spline_continuity (read) : " << Interface_Static::IVal("read.iges.bspline.continuity")
...
static Standard_Integer brepiges(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    IGESControl_Writer ICW(Interface_Static::CVal("write.iges.unit"), Interface_Static::IVal("write.iges.brep.mode"));
    ...
    for (Standard_Integer i = 1; i < n && aPS.More(); i++) {
        ...
        if (ICW.AddShape(Shape, aPS.Next()))
            npris++;
        else if (ICW.AddGeom(DrawTrSurf::GetCurve(nomvar)))
            npris++;
        else if (ICW.AddGeom(DrawTrSurf::GetSurface(nomvar)))
            npris++;
    }
    ICW.ComputeModel();
    ...
    // write file
    if (!ICW.Write(nomfic))
        di << " Error: could not write file " << nomfic;
    else
        di << " File " << nomfic << " written";
...
static Standard_Integer XSDRAWIGES_tplosttrim(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    for (Standard_Integer j = 1; j <= 3; j++) {
        ...
        if (narg == 1) k = j;
        Handle(TColStd_HSequenceOfTransient) list =
            IFSelect_Functions::GiveList(pilot->Session(), strarg.Value(k).ToCString());
        if (!list.IsNull())
            itrp.Filter(list);
        else {
...
static Standard_Integer XSDRAWIGES_TPSTAT(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (argc > 1) {
        ...
        switch (arg1[0]) {
            case 'g':
                read.PrintTransferInfo(IFSelect_FailAndWarn, IFSelect_GeneralInfo);
                break;
            case 'c':
                read.PrintTransferInfo(IFSelect_FailAndWarn, IFSelect_CountByItem);
                break;
            case 'C':
                read.PrintTransferInfo(IFSelect_FailAndWarn, IFSelect_ListByItem);
                break;
            case 'r':
                read.PrintTransferInfo(IFSelect_FailAndWarn, IFSelect_ResultCount);
                break;
            case 's':
                read.PrintTransferInfo(IFSelect_FailAndWarn, IFSelect_Mapping);
                break;
            case '?':
                mod1 = -1;
                break;
