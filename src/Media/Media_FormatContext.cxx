...
// purpose  :
// =======================================================================
Media_FormatContext::Media_FormatContext() : myFormatCtx(NULL), myPtsStartBase(0.0), myDuration(0.0) {
    //
}
...
bool Media_FormatContext::OpenInput(const TCollection_AsciiString& theInput) {
    ...
    anExt.LowerCase();
    if (anExt.EndsWith(".png") || anExt.EndsWith(".jpg") || anExt.EndsWith(".jpeg") || anExt.EndsWith(".mpo") ||
        anExt.EndsWith(".bmp") || anExt.EndsWith(".tif") || anExt.EndsWith(".tiff")) {
...
// purpose  :
// =======================================================================
TCollection_AsciiString Media_FormatContext::FormatTimeProgress(double theProgress, double theDuration) {
    ...
    if (aHours1 > 0 || aHours2 > 0) {
        Sprintf(aBuffer, "%02u:%02u:%02u / %02u:%02u:%02u", aHours1, aMinutes1, aSeconds1, aHours2, aMinutes2,
                aSeconds2);
        return aBuffer;
    }
...
// purpose  :
// =======================================================================
TCollection_AsciiString Media_FormatContext::StreamInfo(unsigned int theIndex, AVCodecContext* theCodecCtx) const {
#ifdef HAVE_FFMPEG
    ...
    if (aStream.sample_aspect_ratio.num &&
        ...
        av_reduce(&aDispAspectRatio.num, &aDispAspectRatio.den,
                  ...
                  aStream.codecpar->height * int64_t(aStream.sample_aspect_ratio.den), 1024 * 1024);
...
// purpose  :
// =======================================================================
bool Media_FormatContext::SeekStream(unsigned int theStreamId, double theSeekPts, bool theToSeekBack) {
#ifdef HAVE_FFMPEG
    ...
    Message::SendWarning(TCollection_AsciiString("Error while seeking ") + aStreamType + " stream to " + theSeekPts +
...
bool Media_FormatContext::Seek(double theSeekPts, bool theToSeekBack) {
    ...
    Message::SendWarning(TCollection_AsciiString("Disaster! Seeking to ") + theSeekPts + " [" + aFileName +
                         "] has failed.");
    return false;
#else
    (void)theSeekPts;
