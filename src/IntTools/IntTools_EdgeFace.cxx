...
static Standard_Boolean IsRadius(const BRepAdaptor_Curve& aCurve, const BRepAdaptor_Surface& aSurface,
...
Standard_Boolean IntTools_EdgeFace::IsCoincident() {
    ...
    const Standard_Integer aTreshIdxF = RealToInt((aNbSeg + 1) * 0.25), aTreshIdxL = RealToInt((aNbSeg + 1) * 0.75);
    ...
    for (i = 0; i <= aNbSeg; ++i) {
        ...
        if (((0 < i) && (i < aTreshIdxF)) || ((aTreshIdxL < i) && (i < aNbSeg))) continue;

        if (isClassified && (i != aNbSeg)) continue;
        ...
        if (aState == TopAbs_OUT) return Standard_False;

        if (i != 0) isClassified = Standard_True;
    }
    //
...
// purpose  :
//=======================================================================
Standard_Boolean IntTools_EdgeFace::IsEqDistance(const gp_Pnt& aP, const BRepAdaptor_Surface& aBAS,
                                                 const Standard_Real aTol, Standard_Real& aD) {
...
void IntTools_EdgeFace::Perform() {
    ...
    if (anIntersector.MinimalSquareDistance() < RealLast()) myMinDistance = Sqrt(anIntersector.MinimalSquareDistance());
...
// purpose  :
//=======================================================================
Standard_Boolean IsRadius(const BRepAdaptor_Curve& aCurve, const BRepAdaptor_Surface& aSurface,
                          const Standard_Real aCriteria) {
    Standard_Boolean bFlag = Standard_False;
...
// purpose  :
//=======================================================================
Standard_Integer AdaptiveDiscret(const Standard_Integer iDiscret, const BRepAdaptor_Curve& aCurve,
