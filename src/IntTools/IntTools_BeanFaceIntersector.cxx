...
static Standard_Boolean SetEmptyResultRange(const Standard_Real theParameter, IntTools_MarkedRangeSet& theMarkedRange);

static Bnd_Box GetSurfaceBox(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU,
                             const Standard_Real theLastU, const Standard_Real theFirstV, const Standard_Real theLastV,
                             const Standard_Real theTolerance, IntTools_SurfaceRangeLocalizeData& theSurfaceData);

static void ComputeGridPoints(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU,
                              const Standard_Real theLastU, const Standard_Real theFirstV, const Standard_Real theLastV,
                              const Standard_Real theTolerance, IntTools_SurfaceRangeLocalizeData& theSurfaceData);

static void BuildBox(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU,
                     const Standard_Real theLastU, const Standard_Real theFirstV, const Standard_Real theLastV,
                     IntTools_SurfaceRangeLocalizeData& theSurfaceData, Bnd_Box& theBox);
...
static void CheckSampling(const IntTools_CurveRangeSample& theCurveRange,
                          ...
                          const IntTools_SurfaceRangeLocalizeData& theSurfaceData, const Standard_Real DiffC,
                          const Standard_Real DiffU, const Standard_Real DiffV, Standard_Boolean& bAllowSamplingC,
                          Standard_Boolean& bAllowSamplingU, Standard_Boolean& bAllowSamplingV);
...
IntTools_BeanFaceIntersector::IntTools_BeanFaceIntersector()
    : myFirstParameter(0.), myLastParameter(0.), myUMinParameter(0.), myUMaxParameter(0.), myVMinParameter(0.),
      myVMaxParameter(0.), myBeanTolerance(0.), myFaceTolerance(0.), myIsDone(Standard_False),
...
// purpose:
// ==================================================================================
IntTools_BeanFaceIntersector::IntTools_BeanFaceIntersector(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace)
    : myFirstParameter(0.), myLastParameter(0.), myUMinParameter(0.), myUMaxParameter(0.), myVMinParameter(0.),
      myVMaxParameter(0.), myBeanTolerance(0.), myFaceTolerance(0.), myIsDone(Standard_False),
      myMinSqDistance(RealLast()) {
...
IntTools_BeanFaceIntersector::IntTools_BeanFaceIntersector(const BRepAdaptor_Curve& theCurve,
    ...
    : myFirstParameter(0.), myLastParameter(0.), myUMinParameter(0.), myUMaxParameter(0.), myVMinParameter(0.),
      myVMaxParameter(0.), myIsDone(Standard_False), myMinSqDistance(RealLast()) {
...
// ==================================================================================
IntTools_BeanFaceIntersector::IntTools_BeanFaceIntersector(
    const BRepAdaptor_Curve& theCurve, const BRepAdaptor_Surface& theSurface, const Standard_Real theFirstParOnCurve,
    const Standard_Real theLastParOnCurve, const Standard_Real theUMinParameter, const Standard_Real theUMaxParameter,
    const Standard_Real theVMinParameter, const Standard_Real theVMaxParameter, const Standard_Real theBeanTolerance,
    const Standard_Real theFaceTolerance)
    : myFirstParameter(theFirstParOnCurve), myLastParameter(theLastParOnCurve), myUMinParameter(theUMinParameter),
      myUMaxParameter(theUMaxParameter), myVMinParameter(theVMinParameter), myVMaxParameter(theVMaxParameter),
      myBeanTolerance(theBeanTolerance), myFaceTolerance(theFaceTolerance), myIsDone(Standard_False),
      myMinSqDistance(RealLast()) {
    myCurve = theCurve;
...
void IntTools_BeanFaceIntersector::Init(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace) {
    ...
    SetSurfaceParameters(mySurface.FirstUParameter(), mySurface.LastUParameter(), mySurface.FirstVParameter(),
...
// purpose:
// ==================================================================================
void IntTools_BeanFaceIntersector::Init(const BRepAdaptor_Curve& theCurve, const BRepAdaptor_Surface& theSurface,
                                        const Standard_Real theBeanTolerance, const Standard_Real theFaceTolerance) {
    myCurve = theCurve;
    mySurface = theSurface;
    myTrsfSurface = Handle(Geom_Surface)::DownCast(mySurface.Surface().Surface()->Transformed(mySurface.Trsf()));
...
// ==================================================================================
void IntTools_BeanFaceIntersector::Init(const BRepAdaptor_Curve& theCurve, const BRepAdaptor_Surface& theSurface,
                                        const Standard_Real theFirstParOnCurve, const Standard_Real theLastParOnCurve,
                                        const Standard_Real theUMinParameter, const Standard_Real theUMaxParameter,
                                        const Standard_Real theVMinParameter, const Standard_Real theVMaxParameter,
                                        const Standard_Real theBeanTolerance, const Standard_Real theFaceTolerance) {
    Init(theCurve, theSurface, theBeanTolerance, theFaceTolerance);
...
void IntTools_BeanFaceIntersector::Perform() {
    ...
    Standard_Boolean bLocalize = (!Precision::IsInfinite(myUMinParameter) && !Precision::IsInfinite(myUMaxParameter) &&
    ...
    bLocalize =
        ...
        (mySurface.GetType() == GeomAbs_BezierSurface || mySurface.GetType() == GeomAbs_OtherSurface ||
         (mySurface.GetType() == GeomAbs_BSplineSurface && (mySurface.UDegree() > 2 || mySurface.VDegree() > 2) &&
    ...
    for (Standard_Integer i = 1; i <= myRangeManager.Length(); i++) {
        if (myRangeManager.Flag(i) != 2) continue;
...
Standard_Real IntTools_BeanFaceIntersector::Distance(const Standard_Real theArg) {
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        Standard_Real anIsoParameter =
            (i == 0) ? myUMinParameter : ((i == 1) ? myUMaxParameter : ((i == 2) ? myVMinParameter : myVMaxParameter));
        ...
        gp_Pnt aPointMin =
            (i < 2) ? mySurface.Value(anIsoParameter, aMinParameter) : mySurface.Value(aMinParameter, anIsoParameter);
        gp_Pnt aPointMax =
            (i < 2) ? mySurface.Value(anIsoParameter, aMaxParameter) : mySurface.Value(aMaxParameter, anIsoParameter);
        gp_Pnt aPointMid =
            (i < 2) ? mySurface.Value(anIsoParameter, aMidParameter) : mySurface.Value(aMidParameter, anIsoParameter);
        ...
        if (computeisoline) {
            ...
            if (aProjectorOnCurve.NbPoints() > 0) {
                ...
                if (aDistance > aProjectorOnCurve.LowerDistance()) aDistance = aProjectorOnCurve.LowerDistance();
            }
        }
...
// purpose:
// ==================================================================================
Standard_Real IntTools_BeanFaceIntersector::Distance(const Standard_Real theArg, Standard_Real& theUParameter,
                ...
                if (UIsNotValid || VIsNotValid) {
                    ...
                    if (UIsNotValid) {
                        ...
                        if (mySurface.IsUPeriodic()) {
                            ...
                            //
                            GeomInt::AdjustPeriodic(U, myUMinParameter, myUMaxParameter, aUPeriod, aNewU, du, aEps);
                    ...
                    if (bUCorrected && VIsNotValid) {
                        ...
                        if (mySurface.IsVPeriodic()) {
                            ...
                            //
                            GeomInt::AdjustPeriodic(V, myVMinParameter, myVMaxParameter, aVPeriod, aNewV, dv, aEps);
                            solutionIsValid = Standard_True;
                            V = aNewV;
                        }
                    }
...
Standard_Boolean IntTools_BeanFaceIntersector::FastComputeAnalytic() {
    ...
    if (aST == GeomAbs_Plane) {
        ...
        if (anAngle > Precision::Angular()) return Standard_False;
    ...
    // Cylinder - Line/Circle
        ...
        if (aCT == GeomAbs_Line) {
            ...
            if (!aLin.Direction().IsParallel(aCylDir, Precision::Angular())) return Standard_False;
...
void IntTools_BeanFaceIntersector::ComputeUsingExtremum() {
    ...
    GeomAdaptor_Surface aGASurface(myTrsfSurface, myUMinParameter, myUMaxParameter, myVMinParameter, myVMaxParameter);

    for (Standard_Integer i = 1; i <= myRangeManager.Length(); i++) {

        if (myRangeManager.Flag(i) > 0) continue;
        ...
        anExtCS.Initialize(aGASurface, myUMinParameter, myUMaxParameter, myVMinParameter, myVMaxParameter, Tol, Tol);
...
void IntTools_BeanFaceIntersector::ComputeNearRangeBoundaries() {
    ...
    for (Standard_Integer i = 1; i <= myRangeManager.Length(); i++) {
        ...
        if ((i > 1) && (myRangeManager.Flag(i - 1) > 0)) continue;
        ...
        if (Distance(aParamRange.First(), U, V) < myCriteria) {
            ...
            ComputeRangeFromStartPoint(Standard_True, aParamRange.First(), U, V,
...
void IntTools_BeanFaceIntersector::ComputeRangeFromStartPoint(const Standard_Boolean ToIncreaseParameter,
    ...
    if (myRangeManager.Flag(theIndex) > 0) return;
    ...
    if (aMinDelta > aDeltaRestrictor) aMinDelta = aDeltaRestrictor * 0.5;
    ...
    while ((aDelta >= aMinDelta) && (loopcounter <= 10)) {
        ...
        if (pointfound) {
            ...
            if (BoundaryCondition && (isboundaryindex || !isvalidindex)) break;
        } else {
...
Standard_Boolean IntTools_BeanFaceIntersector::LocalizeSolutions(
    const IntTools_CurveRangeSample& theCurveRange, const Bnd_Box& theBoxCurve,
    const IntTools_SurfaceRangeSample& theSurfaceRange, const Bnd_Box& theBoxSurface,
    IntTools_CurveRangeLocalizeData& theCurveData, IntTools_SurfaceRangeLocalizeData& theSurfaceData,
    IntTools_ListOfCurveRangeSample& theListCurveRange, IntTools_ListOfSurfaceRangeSample& theListSurfaceRange) {
    ...
    // check
    CheckSampling(theCurveRange, theSurfaceRange, theCurveData, theSurfaceData, localdiffC, aLocalDiffU, aLocalDiffV,
        ...
        for (vIt = 1; vIt <= nbV; vIt++, aCurIndexV++, aPrevParV = aCurParV) {
            ...
            if (!theSurfaceData.FindBox(aNewRangeS, aBoxS)) {
                ...
                if (mySurface.GetType() == GeomAbs_BSplineSurface) {
                    ...
                    aBoxS =
                        GetSurfaceBox(aSurfBspl, aPrevParU, aCurParU, aPrevParV, aCurParV, myCriteria, theSurfaceData);
                } else {
            ...
            for (; anItI.More() && anItBox.More(); anItI.Next(), anItBox.Next()) {
                ...
                if (!bHasOutC) {
                    ...
                    if (!LocalizeSolutions(aNewRangeC, anItBox.Value(), ((bUseOldS) ? theSurfaceRange : aNewRangeS),
                                           ((bUseOldS) ? theBoxSurface : aBoxS), theCurveData, theSurfaceData,
...
Standard_Boolean IntTools_BeanFaceIntersector::ComputeLocalized() {
    ...
    if (mySurface.GetType() == GeomAbs_BSplineSurface) {
        ...
        ComputeGridPoints(aSurfBspl, myUMinParameter, myUMaxParameter, myVMinParameter, myVMaxParameter,
                          myFaceTolerance, aSurfaceData);

        if (!bFBoxFound) {
            FBox = GetSurfaceBox(aSurfBspl, myUMinParameter, myUMaxParameter, myVMinParameter, myVMaxParameter,
                                 myCriteria, aSurfaceData);
            aSurfaceData.AddBox(aSurfaceRange, FBox);
        }

    } else if (!bFBoxFound) {

        BndLib_AddSurface::Add(mySurface, myUMinParameter, myUMaxParameter, myVMinParameter, myVMaxParameter,
                               myFaceTolerance, FBox);
        aSurfaceData.AddBox(aSurfaceRange, FBox);
    }

    Bnd_Box EBox;

    BndLib_Add3dCurve::Add(*myCurve.Trim(myFirstParameter, myLastParameter, Precision::PConfusion()), myBeanTolerance,
                           EBox);
    ...
    CheckSampling(aCurveRange, aSurfaceRange, aCurveDataTmp, aSurfaceDataTmp, myLastParameter - myFirstParameter,
                  myUMaxParameter - myUMinParameter, myVMaxParameter - myVMinParameter, bAllowSamplingC,
...

    {
        ...
        if (!LocalizeSolutions(aCurveRange, EBox, aSurfaceRange, FBox, aCurveData, aSurfaceData, aListCurveRange,
                               aListSurfaceRange)) {
            aSurfaceData.ClearGrid();
        ...
        for (; anItC.More() && anItS.More(); anItC.Next(), anItS.Next()) {
            ...
            if (bAllowSamplingC) aRangeC = anItC.Value().GetRange(myFirstParameter, myLastParameter, nbSampleC);

            IntTools_Range aRangeU(myUMinParameter, myUMaxParameter);

            if (bAllowSamplingU) aRangeU = anItS.Value().GetRangeU(myUMinParameter, myUMaxParameter, nbSampleU);

            IntTools_Range aRangeV(myVMinParameter, myVMaxParameter);

            if (bAllowSamplingV) aRangeV = anItS.Value().GetRangeV(myVMinParameter, myVMaxParameter, nbSampleV);
            ...
            if (anExtremaGen.IsDone() && (anExtremaGen.NbExt() > 0)) {
                ...
                for (Standard_Integer j = 1; j <= anExtremaGen.NbExt(); j++) {
                    ...
                    if (anExtremaGen.SquareDistance(j) < myCriteria * myCriteria) {
                        ...
                        if (myCurve.IsPeriodic()) T = ElCLib::InPeriod(T, anarg1, anarg1 + myCurve.Period());
                        if (mySurface.IsUPeriodic()) U = ElCLib::InPeriod(U, parUF, parUF + mySurface.UPeriod());
                        if (mySurface.IsVPeriodic()) V = ElCLib::InPeriod(V, parVF, parVF + mySurface.VPeriod());
...
Standard_Boolean IntTools_BeanFaceIntersector::TestComputeCoinside() {
    ...
    if (Distance(cfp, U, V) > myCriteria) return Standard_False;
    ...
    if (aFoundIndex != 0) {
        if (myRangeManager.Flag(aFoundIndex) == 2) return Standard_True;
    }

    if (Distance(clp, U, V) > myCriteria) return Standard_False;
    ...
    for (i = 1; i < nbSeg; i++) {
        ...
        if (Distance(aPar, U, V) > myCriteria) return Standard_False;
...
// purpose:
// ---------------------------------------------------------------------------------
Bnd_Box GetSurfaceBox(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU,
                      const Standard_Real theLastU, const Standard_Real theFirstV, const Standard_Real theLastV,
                      const Standard_Real theTolerance, IntTools_SurfaceRangeLocalizeData& theSurfaceData) {
    Bnd_Box aTotalBox;
...
// purpose:
// ---------------------------------------------------------------------------------
void ComputeGridPoints(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU,
                       const Standard_Real theLastU, const Standard_Real theFirstV, const Standard_Real theLastV,
                       const Standard_Real theTolerance, IntTools_SurfaceRangeLocalizeData& theSurfaceData) {
    Standard_Integer i;
    Standard_Integer j;
    ...
    for (j = 0; j < 2; j++) {
        ...
        for (i = 1; i <= aNbKnots[j] && (iMin[j] == -1 || iMax[j] == -1); i++) {
            if (iMin[j] == -1 && aFpTol[j] < aKnots.Value(i)) iMin[j] = i - 1;

            iLmI = aNbKnots[j] - i + 1;

            if (iMax[j] == -1 && aLmTol[j] > aKnots.Value(iLmI)) iMax[j] = iLmI + 1;
        }

        // If indices are not found, return.
        // if (iMin[j] == -1 || iMax[j] == -1)
        // return;
        if (iMin[j] == -1) iMin[j] = 1;

        if (iMax[j] == -1) iMax[j] = aNbKnots[j];

        if (iMin[j] == 0) iMin[j] = 1;

        if (iMax[j] > aNbKnots[j]) iMax[j] = aNbKnots[j];

        if (iMax[j] < iMin[j]) return;
    ...
    if (theTolerance > aDef) aDef = theTolerance;
...
// ---------------------------------------------------------------------------------
void BuildBox(const Handle(Geom_BSplineSurface) & theSurf, const Standard_Real theFirstU, const Standard_Real theLastU,
              const Standard_Real theFirstV, const Standard_Real theLastV,
              IntTools_SurfaceRangeLocalizeData& theSurfaceData, Bnd_Box& theBox) {
    Standard_Integer i;
    Standard_Integer j;
...
static void MergeSolutions(const IntTools_ListOfCurveRangeSample& theListCurveRange,
    ...
    for (; anItS1.More(); anItS1.Next()) {
        ...
        if (aMapToAvoid.Contains(aRangeS)) continue;
...
static void CheckSampling(const IntTools_CurveRangeSample& theCurveRange,
    ...
    if ((pow((Standard_Real)theSurfaceData.GetNbSampleU(), (Standard_Real)(theSurfaceRange.GetDepthU() + 1)) >
         dLimit) ||
        ((DiffU / theSurfaceData.GetNbSampleU()) < theSurfaceData.GetMinRangeU())) {
        bAllowSamplingU = Standard_False;
    }

    if ((pow((Standard_Real)theSurfaceData.GetNbSampleV(), (Standard_Real)(theSurfaceRange.GetDepthV() + 1)) >
         dLimit) ||
