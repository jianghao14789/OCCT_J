...
static void Parameters(const Handle(GeomAdaptor_Surface) &, const Handle(GeomAdaptor_Surface) &, const gp_Pnt&,
                       Standard_Real&, Standard_Real&, Standard_Real&, Standard_Real&);

static void CorrectSurfaceBoundaries(const TopoDS_Face& theFace, const Standard_Real theTolerance,
                                     Standard_Real& theumin, Standard_Real& theumax, Standard_Real& thevmin,
                                     Standard_Real& thevmax);

static Standard_Boolean ParameterOutOfBoundary(const Standard_Real theParameter, const Handle(Geom_Curve) & theCurve,
                                               const TopoDS_Face& theFace1, const TopoDS_Face& theFace2,
                                               const Standard_Real theOtherParameter,
                                               const Standard_Boolean bIncreasePar, const Standard_Real theTol,
                                               Standard_Real& theNewParameter, const Handle(IntTools_Context) &);
...
static void PerformPlanes(const Handle(GeomAdaptor_Surface) & theS1, const Handle(GeomAdaptor_Surface) & theS2,
                          const Standard_Real TolF1, const Standard_Real TolF2, const Standard_Real TolAng,
                          const Standard_Real TolTang, const Standard_Boolean theApprox1,
                          const Standard_Boolean theApprox2, IntTools_SequenceOfCurves& theSeqOfCurve,
                          Standard_Boolean& theTangentFaces);

static Standard_Boolean ClassifyLin2d(const Handle(GeomAdaptor_Surface) & theS, const gp_Lin2d& theLin2d,
                                      const Standard_Real theTol, Standard_Real& theP1, Standard_Real& theP2);
//
static void ApproxParameters(const Handle(GeomAdaptor_Surface) & aHS1, const Handle(GeomAdaptor_Surface) & aHS2,
                             Standard_Integer& iDegMin, Standard_Integer& iNbIter, Standard_Integer& iDegMax);

static void Tolerances(const Handle(GeomAdaptor_Surface) & aHS1, const Handle(GeomAdaptor_Surface) & aHS2,
                       Standard_Real& aTolTang);

static Standard_Boolean SortTypes(const GeomAbs_SurfaceType aType1, const GeomAbs_SurfaceType aType2);
...
static Standard_Real MaxDistance(const Handle(Geom_Curve) & theC, const Standard_Real aT,
                                 GeomAPI_ProjectPointOnSurf& theProjPS);

static Standard_Real FindMaxDistance(const Handle(Geom_Curve) & theC, const Standard_Real theFirst,
                                     const Standard_Real theLast, GeomAPI_ProjectPointOnSurf& theProjPS,
                                     const Standard_Real theEps);

static Standard_Real FindMaxDistance(const Handle(Geom_Curve) & theCurve, const Standard_Real theFirst,
                                     const Standard_Real theLast, const TopoDS_Face& theFace,
                                     const Handle(IntTools_Context) & theContext);

static void CorrectPlaneBoundaries(Standard_Real& aUmin, Standard_Real& aUmax, Standard_Real& aVmin,
                                   Standard_Real& aVmax);
...
//
// =======================================================================
void IntTools_FaceFace::SetParameters(const Standard_Boolean ToApproxC3d, const Standard_Boolean ToApproxC2dOnS1,
...
static Standard_Boolean isTreatAnalityc(const BRepAdaptor_Surface& theBAS1, const BRepAdaptor_Surface& theBAS2,
...
//=======================================================================
void IntTools_FaceFace::Perform(const TopoDS_Face& aF1, const TopoDS_Face& aF2,
    ...
    if (aType1 == GeomAbs_Plane && aType2 == GeomAbs_Plane) {
        ...
        //
        PerformPlanes(myHS1, myHS2, myTolF1, myTolF2, TolAng, TolTang, myApprox1, myApprox2, mySeqOfCurve,
                      myTangentFaces);
        //
    ...
    if ((aType1 != GeomAbs_BSplineSurface) && (aType1 != GeomAbs_BezierSurface) && (aType1 != GeomAbs_OtherSurface) &&
        (aType2 != GeomAbs_BSplineSurface) && (aType2 != GeomAbs_BezierSurface) && (aType2 != GeomAbs_OtherSurface)) {
    ...
    for (i = 1; i <= aNbLin; ++i) {
        ...
        for (j = 0; j < 2; ++j) {
            ...
            if (!aC2D.IsNull()) {
                ...
                if (IntTools_Tools::ComputeTolerance(aC3D, aC2D, aS, aFirst, aLast, aD, aT, Precision::PConfusion(),
                                                     theToRunParallel)) {
                    if (aD > aTolC) {
                        aTolC = aD;
                    }
...
// purpose  :
//=======================================================================
void IntTools_FaceFace::MakeCurve(const Standard_Integer Index, const Handle(Adaptor3d_TopolTool) & dom1,
                                  const Handle(Adaptor3d_TopolTool) & dom2, const Standard_Real theToler) {
...
reapprox:;
    ...
    if (typl == IntPatch_Restriction) bAvoidLineConstructor = Standard_True;
    ...
    switch (typl) {
                ...
                if (fprm < aNul && lprm > aNul) {
                    ...
                    if ((aPeriod - fprm) > Tolpc) {
                        ...
                        if (P1.Distance(P2) > myTol) {
                            ...
                            if (ParameterOutOfBoundary(fprm, newc, myFace1, myFace2, lprm, Standard_False, myTol,
                                                       anewpar, myContext)) {
                                fprm = anewpar;
                            }
                            aSeqFprm.Append(fprm);
                    ...
                    if ((lprm - aNul) > Tolpc) {
                        ...
                        if (P1.Distance(P2) > myTol) {
                            ...
                            if (ParameterOutOfBoundary(lprm, newc, myFace1, myFace2, fprm, Standard_True, myTol,
                                                       anewpar, myContext)) {
                                lprm = anewpar;
                            }
                            aSeqFprm.Append(aNul);
        ...
        case IntPatch_Walking: {
            ...
            //
                ...
                bIsDecomposited = IntTools_WLineTool::DecompositionOfWLine(
                    WL, myHS1, myHS2, myFace1, myFace2, myLConstruct, bAvoidLineConstructor, myTol, aSeqOfL,
                ...
                for (i = 1; i <= nbiter; ++i) {
                    ...
                    aParType = ApproxInt_KnotTools::DefineParType(WL, ifprm, ilprm, anApprox, anApprox1, anApprox2);
                    if (myHS1 == myHS2) {
                        ...
                        if (reApprox && !rejectSurface)
                            ...
                            theapp3d.SetParameters(myTolApprox, tol2d, iDegMin, iDegMax, iNbIter, 30, Standard_True,
                                                   aParType);
                        }
                    }
                    ...
                    if (!theapp3d.IsDone()) {
                        ...
                        for (j = 1; j <= aNbMultiCurves; j++) {
                            if (typs1 == GeomAbs_Plane) {
                                ...
                                //
                                Handle(Geom_BSplineCurve) BS = new Geom_BSplineCurve(
                                    tpoles, mbspc.Knots(), mbspc.Multiplicities(), mbspc.Degree());
                                ...
                                if (myApprox1) {
                                    Handle(Geom2d_BSplineCurve) BS1 = new Geom2d_BSplineCurve(
                                        tpoles2d, mbspc.Knots(), mbspc.Multiplicities(), mbspc.Degree());
                                ...
                                if (myApprox2) {
                                    ...
                                    Handle(Geom2d_BSplineCurve) BS2 = new Geom2d_BSplineCurve(
                                        tpoles2d, mbspc.Knots(), mbspc.Multiplicities(), mbspc.Degree());
                            ...
                            //
                                ...
                                BS = new Geom_BSplineCurve(tpoles, mbspc.Knots(), mbspc.Multiplicities(),
                                                           mbspc.Degree());
                                GeomLib_CheckBSplineCurve Check(BS, TOLCHECK, TOLANGCHECK);
                                Check.FixTangent(Standard_True, Standard_True);
                                //
                                ...
                                if (myApprox1) {
                                    if (anApprox1) {
                                        ...
                                        mbspc.Curve(2, tpoles2d);
                                        //
                                        BS1 = new Geom2d_BSplineCurve(tpoles2d, mbspc.Knots(), mbspc.Multiplicities(),
                                                                      mbspc.Degree());
                                ...
                                if (myApprox2) {
                                    if (anApprox2) {
                                        ...
                                        BS2 = new Geom2d_BSplineCurve(tpoles2d, mbspc.Knots(), mbspc.Multiplicities(),
                                                                      mbspc.Degree());
                                        GeomLib_Check2dBSplineCurve newCheck(BS2, TOLCHECK, TOLANGCHECK);
                                        newCheck.FixTangent(Standard_True, Standard_True);
                                        //
        ...
        case IntPatch_Restriction: {
            ...
            for (Standard_Integer anInd = 0; anInd < aNbIntersSolutionsm1; anInd++) {
                ...
                if ((aParL - aParF) <= Precision::PConfusion()) {
                    ...
                    if (anInd < aNbIntersSolutionsm1 - 1) aParL = aParF;
                ...
                if (!aC2d1.IsNull()) {
                    ...
                    if (aBox1.IsOut(aPt)) continue;

                    if (myApprox1) aCurv2d1 = new Geom2d_TrimmedCurve(aC2d1, aParF, aParL);
                }

                if (!aC2d2.IsNull()) {
                    aC2d2->D0(aPar, aPt);

                    if (aBox2.IsOut(aPt)) continue;

                    if (myApprox2) aCurv2d2 = new Geom2d_TrimmedCurve(aC2d2, aParF, aParL);
                }
...
// purpose  :
//=======================================================================
void Parameters(const Handle(GeomAdaptor_Surface) & HS1, const Handle(GeomAdaptor_Surface) & HS2, const gp_Pnt& Ptref,
                Standard_Real& U1, Standard_Real& V1, Standard_Real& U2, Standard_Real& V2) {
...
//=======================================================================
Handle(Geom_Curve)
    MakeBSpline(const Handle(IntPatch_WLine) & WL, const Standard_Integer ideb, const Standard_Integer ifin) {
...
void IntTools_FaceFace::PrepareLines3D(const Standard_Boolean bToSplit) {
    ...
    //
    if ((aType1 == GeomAbs_Plane && aType2 == GeomAbs_Cone) || (aType2 == GeomAbs_Plane && aType1 == GeomAbs_Cone)) {
        if (aNbCurves == 4) {
...
void CorrectSurfaceBoundaries(const TopoDS_Face& theFace, const Standard_Real theTolerance, Standard_Real& theumin,
    ...
    if (aSurface->IsKind(STANDARD_TYPE(Geom_OffsetSurface))) {
        Handle(Geom_Surface) aBasisSurface = (Handle(Geom_OffsetSurface)::DownCast(aSurface))->BasisSurface();
    ...
    if (isuperiodic || isvperiodic) {
        ...
        if (correct) {
            ...
            if (isuperiodic && correctU) {
                if (theumin < umin) theumin = umin;
                if (theumax > umax) {
                    theumax = umax;
                }
            }
            if (isvperiodic && correctV) {
                if (thevmin < vmin) thevmin = vmin;
                if (thevmax > vmax) thevmax = vmax;
            }
        }
    }
}
...
// ------------------------------------------------------------------------------------------------
Standard_Boolean ParameterOutOfBoundary(const Standard_Real theParameter, const Handle(Geom_Curve) & theCurve,
                                        const TopoDS_Face& theFace1, const TopoDS_Face& theFace2,
                                        const Standard_Real theOtherParameter, const Standard_Boolean bIncreasePar,
                                        const Standard_Real theTol, Standard_Real& theNewParameter,
    ...
    if (iter <= 11) {
        ...
        if (bIncreasePar) {
            if (acurpar >= theOtherParameter) theNewParameter = theOtherParameter;
        } else {
            if (acurpar <= theOtherParameter) theNewParameter = theOtherParameter;
        }
    }
...
Standard_Boolean IsCurveValid(const Handle(Geom2d_Curve) & thePCurve) {
    if (thePCurve.IsNull()) return Standard_False;
...
// purpose  :
//=======================================================================
void PerformPlanes(const Handle(GeomAdaptor_Surface) & theS1, const Handle(GeomAdaptor_Surface) & theS2,
                   const Standard_Real TolF1, const Standard_Real TolF2, const Standard_Real TolAng,
                   const Standard_Real TolTang, const Standard_Boolean theApprox1, const Standard_Boolean theApprox2,
                   IntTools_SequenceOfCurves& theSeqOfCurve, Standard_Boolean& theTangentFaces) {
...
// purpose  :
//=======================================================================
static inline Standard_Boolean INTER(const Standard_Real d1, const Standard_Real d2, const Standard_Real tol) {
    return (d1 > tol && d2 < -tol) || (d1 < -tol && d2 > tol) ||
           ((d1 <= tol && d1 >= -tol) && (d2 > tol || d2 < -tol)) ||
           ((d2 <= tol && d2 >= -tol) && (d1 > tol || d1 < -tol));
}
static inline Standard_Boolean COINC(const Standard_Real d1, const Standard_Real d2, const Standard_Real tol) {
    return (d1 <= tol && d1 >= -tol) && (d2 <= tol && d2 >= -tol);
}
Standard_Boolean ClassifyLin2d(const Handle(GeomAdaptor_Surface) & theS, const gp_Lin2d& theLin2d,
                               const Standard_Real theTol, Standard_Real& theP1, Standard_Real& theP2)
...
// purpose  :
//=======================================================================
void ApproxParameters(const Handle(GeomAdaptor_Surface) & aHS1, const Handle(GeomAdaptor_Surface) & aHS2,
                      Standard_Integer& iDegMin, Standard_Integer& iDegMax, Standard_Integer& iNbIter)

{
    ...
    //
    // Cylinder/Torus
    if ((aTS1 == GeomAbs_Cylinder && aTS2 == GeomAbs_Torus) || (aTS2 == GeomAbs_Cylinder && aTS1 == GeomAbs_Torus)) {
...
// purpose  :
//=======================================================================
void Tolerances(const Handle(GeomAdaptor_Surface) & aHS1, const Handle(GeomAdaptor_Surface) & aHS2,
                Standard_Real& aTolTang) {
    GeomAbs_SurfaceType aTS1, aTS2;
    //
    aTS1 = aHS1->GetType();
...
// purpose :
//=======================================================================
Standard_Real FindMaxDistance(const Handle(Geom_Curve) & theCurve, const Standard_Real theFirst,
                              const Standard_Real theLast, const TopoDS_Face& theFace,
                              const Handle(IntTools_Context) & theContext) {
...
// purpose :
//=======================================================================
Standard_Real FindMaxDistance(const Handle(Geom_Curve) & theC, const Standard_Real theFirst,
                              const Standard_Real theLast, GeomAPI_ProjectPointOnSurf& theProjPS,
                              const Standard_Real theEps) {
...
// purpose :
//=======================================================================
Standard_Real MaxDistance(const Handle(Geom_Curve) & theC, const Standard_Real aT,
                          GeomAPI_ProjectPointOnSurf& theProjPS) {
    Standard_Real aD;
    gp_Pnt aP;
    //
...
//=======================================================================
Standard_Boolean CheckPCurve(const Handle(Geom2d_Curve) & aPC, const TopoDS_Face& aFace,
                             const Handle(IntTools_Context) & theCtx) {
...
// purpose  :
//=======================================================================
void CorrectPlaneBoundaries(Standard_Real& aUmin, Standard_Real& aUmax, Standard_Real& aVmin, Standard_Real& aVmax) {
