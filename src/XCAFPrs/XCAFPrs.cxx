...
static void fillStyleColors(XCAFPrs_Style& theStyle, const Handle(XCAFDoc_ColorTool) & theTool,
...
void XCAFPrs::CollectStyleSettings(const TDF_Label& theLabel, const TopLoc_Location& theLoc,
    ...
    // for references, first collect colors of referred shape
    {
        ...
        if (XCAFDoc_ShapeTool::GetReferredShape(theLabel, aLabelRef)) {
            ...
            if (aLayerNames->Length() == 1) {
                ...
                Quantity_ColorRGBA aColor;
                if (aColorTool->GetColor(aLayer, XCAFDoc_ColorGen, aColor)) aLayerColor = aColor;
            }
    ...
    for (TDF_LabelSequence::Iterator aLabIter(aLabSeq); aLabIter.More(); aLabIter.Next()) {
        ...
        if (aShapeTool->IsComponent(aLabel)) {
            ...
            for (TDF_AttributeSequence::Iterator aShuoAttribIter(aShuoAttribSeq); aShuoAttribIter.More();
                ...
                for (TopTools_SequenceOfShape::Iterator aShuoShapeIter (aShuoShapeSeq); aShuoShapeIter.More();
                aShuoShapeIter.Next())
                {
