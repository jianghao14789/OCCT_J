...
static XCAFPrs_Style mergedStyle(const Handle(XCAFDoc_ColorTool) & theColorTool,
                                 ...
                                 const XCAFPrs_Style& theParenStyle, const TDF_Label& theLabel,
...
XCAFPrs_DocumentExplorer::XCAFPrs_DocumentExplorer()
    : myTop(-1), myHasMore(Standard_False), myFlags(XCAFPrs_DocumentExplorerFlags_None) {
    //
}
...
// purpose  :
// =======================================================================
void XCAFPrs_DocumentExplorer::Init(const Handle(TDocStd_Document) & theDocument, const TDF_Label& theRoot,
                                    const XCAFPrs_DocumentExplorerFlags theFlags, const XCAFPrs_Style& theDefStyle) {
...
// purpose  :
// =======================================================================
void XCAFPrs_DocumentExplorer::Init(const Handle(TDocStd_Document) & theDocument, const TDF_LabelSequence& theRoots,
                                    const XCAFPrs_DocumentExplorerFlags theFlags, const XCAFPrs_Style& theDefStyle) {
...
void XCAFPrs_DocumentExplorer::initCurrent(Standard_Boolean theIsAssembly) {
    ...
    if (theIsAssembly) {
        ...
        myCurrent.Style =
            mergedStyle(myColorTool, myVisMatTool, aTopNodeInStack.Style, myCurrent.Label, myCurrent.RefLabel);
    ...
    for (;;) {
        if (myNodeStack.Value(myTop).ChildIter.More()) {
            ...
            if (!XCAFDoc_ShapeTool::IsAssembly(aRefLabel)) {
                ...
                aNodeInStack.Style =
                    mergedStyle(myColorTool, myVisMatTool, aParent.Style, aNodeInStack.Label, aNodeInStack.RefLabel);
