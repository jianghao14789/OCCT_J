...
public:
    ...
    const Quantity_Color& GetColorSurf() const {
        return myColorSurf.GetRGB();
    }

    //! Set surface color.
    void SetColorSurf(const Quantity_Color& theColor) {
        SetColorSurf(Quantity_ColorRGBA(theColor));
    }
    ...
    const Handle(Image_Texture) & BaseColorTexture() const {
        ...
        } else if (myMaterial->HasCommonMaterial() && !myMaterial->CommonMaterial().DiffuseTexture.IsNull()) {
    ...
    Standard_Boolean IsEqual(const XCAFPrs_Style& theOther) const {
        ...
        return myHasColorSurf == theOther.myHasColorSurf && myHasColorCurv == theOther.myHasColorCurv &&
               myMaterial == theOther.myMaterial && (!myHasColorSurf || myColorSurf == theOther.myColorSurf) &&
