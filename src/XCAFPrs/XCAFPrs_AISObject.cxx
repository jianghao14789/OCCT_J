...
XCAFPrs_AISObject::XCAFPrs_AISObject(const TDF_Label& theLabel)
    : AIS_ColoredShape(TopoDS_Shape()), myToSyncStyles(Standard_True) {
...
static void DisplayText(const TDF_Label& aLabel, const Handle(Prs3d_Presentation) & aPrs,
                        const Handle(Prs3d_TextAspect) & anAspect, const TopLoc_Location& aLocation) {
    // first label itself
...
void XCAFPrs_AISObject::DispatchStyles(const Standard_Boolean theToSyncStyles) {
    ...
    for (XCAFPrs_DataMapIteratorOfIndexedDataMapOfShapeStyle aStyledShapeIter(aSettings); aStyledShapeIter.More();
    ...
    for (NCollection_IndexedDataMap<XCAFPrs_Style, TopoDS_Compound, XCAFPrs_Style>::Iterator aStyleGroupIter(
             aStyleGroups);
...
// purpose  :
//=======================================================================
void XCAFPrs_AISObject::setStyleToDrawer(const Handle(Prs3d_Drawer) & theDrawer, const XCAFPrs_Style& theStyle,
