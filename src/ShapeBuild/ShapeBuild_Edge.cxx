...
// purpose  :
//=======================================================================
TopoDS_Edge ShapeBuild_Edge::CopyReplaceVertices(const TopoDS_Edge& edge, const TopoDS_Vertex& V1,
...
void ShapeBuild_Edge::CopyRanges(const TopoDS_Edge& toedge, const TopoDS_Edge& fromedge, const Standard_Real alpha,
...
void ShapeBuild_Edge::SetRange3d(const TopoDS_Edge& edge, const Standard_Real first, const Standard_Real last) const {

    for (BRep_ListIteratorOfListOfCurveRepresentation itcr((*((Handle(BRep_TEdge)*)&edge.TShape()))->ChangeCurves());
         itcr.More(); itcr.Next()) {
        Handle(BRep_GCurve) GC = Handle(BRep_GCurve)::DownCast(itcr.Value());
...
TopoDS_Edge ShapeBuild_Edge::Copy(const TopoDS_Edge& edge, const Standard_Boolean sharepcurves) const {
...
void ShapeBuild_Edge::RemovePCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surf,
...
void ShapeBuild_Edge::ReplacePCurve(const TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve,
...
static Standard_Integer CountPCurves(const TopoDS_Edge& edge, const TopoDS_Face& face) {
    ...
    for (BRep_ListIteratorOfListOfCurveRepresentation itcr((*((Handle(BRep_TEdge)*)&edge.TShape()))->ChangeCurves());
         itcr.More(); itcr.Next()) {
        Handle(BRep_GCurve) GC = Handle(BRep_GCurve)::DownCast(itcr.Value());
        if (!GC.IsNull() && GC->IsCurveOnSurface(S, l)) return (GC->IsCurveOnClosedSurface() ? 2 : 1);
    }
    return 0;
}

Standard_Boolean ShapeBuild_Edge::ReassignPCurve(const TopoDS_Edge& edge, const TopoDS_Face& old,
...
Handle(Geom2d_Curve) ShapeBuild_Edge::TransformPCurve(const Handle(Geom2d_Curve) & pcurve, const gp_Trsf2d& trans,
                                                      const Standard_Real uFact, Standard_Real& aFirst,
    ...
    if (result->IsKind(STANDARD_TYPE(Geom2d_Line))) {
        ...
        if (result->IsKind(STANDARD_TYPE(Geom2d_Conic))) {
            ...
            Handle(Geom2d_Curve) tcurve =
                new Geom2d_TrimmedCurve(result, aFirst, aLast); // protection against parabols ets
...
void ShapeBuild_Edge::MakeEdge(TopoDS_Edge& edge, const Handle(Geom_Curve) & curve, const TopLoc_Location& L,
...
void ShapeBuild_Edge::MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve, const TopoDS_Face& face,
                               const Standard_Real p1, const Standard_Real p2) const {
    TopLoc_Location L;
...
void ShapeBuild_Edge::MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve, const Handle(Geom_Surface) & S,
...
void ShapeBuild_Edge::MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve, const Handle(Geom_Surface) & S,
                               const TopLoc_Location& L, const Standard_Real p1, const Standard_Real p2) const {
