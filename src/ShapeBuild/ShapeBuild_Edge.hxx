...
public:
    ...
    Standard_EXPORT TopoDS_Edge CopyReplaceVertices(const TopoDS_Edge& edge, const TopoDS_Vertex& V1,
                                                    const TopoDS_Vertex& V2) const;

    //! Copies ranges for curve3d and all common pcurves from
    //! edge <fromedge> into edge <toedge>.
    Standard_EXPORT void CopyRanges(const TopoDS_Edge& toedge, const TopoDS_Edge& fromedge,
                                    const Standard_Real alpha = 0, const Standard_Real beta = 1) const;
    ...
    Standard_EXPORT void RemovePCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surf,
    ...
    //! replaced
    Standard_EXPORT void ReplacePCurve(const TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve,
    ...
    Standard_EXPORT Standard_Boolean ReassignPCurve(const TopoDS_Edge& edge, const TopoDS_Face& old,
    ...
    Standard_EXPORT Handle(Geom2d_Curve)
        TransformPCurve(const Handle(Geom2d_Curve) & pcurve, const gp_Trsf2d& trans, const Standard_Real uFact,
                        Standard_Real& aFirst, Standard_Real& aLast) const;
    ...
    Standard_EXPORT void MakeEdge(TopoDS_Edge& edge, const Handle(Geom_Curve) & curve, const TopLoc_Location& L,
                                  const Standard_Real p1, const Standard_Real p2) const;

    //! Makes edge with pcurve and face
    Standard_EXPORT void MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve,
                                  const TopoDS_Face& face) const;

    //! Makes edge with pcurve, face and range [p1, p2]
    Standard_EXPORT void MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve, const TopoDS_Face& face,
                                  const Standard_Real p1, const Standard_Real p2) const;
    ...
    Standard_EXPORT void MakeEdge(TopoDS_Edge& edge, const Handle(Geom2d_Curve) & pcurve,
                                  const Handle(Geom_Surface) & S, const TopLoc_Location& L, const Standard_Real p1,
