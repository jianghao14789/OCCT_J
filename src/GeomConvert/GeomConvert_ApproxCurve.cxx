...
public:
    GeomConvert_ApproxCurve_Eval(const Handle(Adaptor3d_Curve) & theFunc, Standard_Real First, Standard_Real Last)
        : fonct(theFunc) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
    }
...
void GeomConvert_ApproxCurve_Eval::Evaluate(Standard_Integer* Dimension, Standard_Real StartEnd[2],
...
GeomConvert_ApproxCurve::GeomConvert_ApproxCurve(const Handle(Geom_Curve) & Curve, const Standard_Real Tol3d,
...
GeomConvert_ApproxCurve::GeomConvert_ApproxCurve(const Handle(Adaptor3d_Curve) & Curve, const Standard_Real Tol3d,
                                                 const GeomAbs_Shape Order, const Standard_Integer MaxSegments,
                                                 const Standard_Integer MaxDegree) {
    Approximate(Curve, Tol3d, Order, MaxSegments, MaxDegree);
}

void GeomConvert_ApproxCurve::Approximate(const Handle(Adaptor3d_Curve) & theCurve, const Standard_Real theTol3d,
    ...
    AdvApprox_ApproxAFunction aApprox(Num1DSS, Num2DSS, Num3DSS, OneDTolNul, TwoDTolNul, ThreeDTol, First, Last,
