...
static Handle(Geom_BSplineCurve)
    BSplineCurveBuilder(const Handle(Geom_Conic) & TheConic, const Convert_ConicToBSplineCurve& Convert)

{
    ...
    TheCurve = new Geom_BSplineCurve(Poles, Weights, Knots, Mults, Convert.Degree(), Convert.IsPeriodic());
    gp_Trsf T;
...
Handle(Geom_BSplineCurve) GeomConvert::SplitBSplineCurve(const Handle(Geom_BSplineCurve) & C,
                                                         const Standard_Real FromU1, const Standard_Real ToU2,
...
Handle(Geom_BSplineCurve) GeomConvert::CurveToBSplineCurve(const Handle(Geom_Curve) & C,
...
public:
    GeomConvert_law_evaluator(const Handle(Geom2d_BSplineCurve) & theAncore) : myAncore(theAncore) {}
...
static Handle(Geom_BSplineCurve)
    MultNumandDenom(const Handle(Geom2d_BSplineCurve) & a, const Handle(Geom_BSplineCurve) & BS)

{
    ...
    if ((end_value - start_value) / 5 < tolerance) tolerance = (end_value - start_value) / 5;
    ...
    BSplCLib::MergeBSplineKnots(tolerance, start_value, end_value, // merge of the knots
                                a->Degree(), aKnots, aMults, BS->Degree(), BSKnots, BSMults, resNbPoles, resKnots,
                                resMults);
...
static void ReorderArrayOfG1Curves(TColGeom_Array1OfBSplineCurve& ArrayOfCurves, TColStd_Array1OfReal& ArrayOfToler,
...
void GeomConvert::ConcatG1(TColGeom_Array1OfBSplineCurve& ArrayOfCurves, const TColStd_Array1OfReal& ArrayOfToler,
                           Handle(TColGeom_HArray1OfBSplineCurve) & ArrayOfConcatenated, Standard_Boolean& ClosedG1Flag,
                           const Standard_Real ClosedTolerance)

{
    Standard_Integer nb_curve = ArrayOfCurves.Length(), nb_vertexG1 = 0, nb_group = 0, index = 0, i, ii, j, jj,
    ...
    for (i = 0; i < nb_curve; i++) {
        if (i >= 1) {
            ...
            Cont = GeomLProp::Continuity(ArrayOfCurves(i - 1), ArrayOfCurves(i), PreLast, First, Standard_True,
                                         Standard_True);
            if (Cont < GeomAbs_C0)
    ...
    if ((ClosedG1Flag) && (nb_group != 1)) { // sort of the array
        nb_group--;
        ReorderArrayOfG1Curves(ArrayOfCurves, local_tolerance, tabG1, nb_vertex_group0, ClosedTolerance);
    }
    ...
    if (nb_group == 1 && ClosedG1Flag && NeedDoubleDegRepara) {
        ...
        if (Curve1->Degree() > Geom2d_BSplineCurve::MaxDegree() / 2) ClosedG1Flag = Standard_False;
    }
        ...
        for (j = 1; j <= nb_curve - 1; j++) { // secondary loop inside each group
            ...
            if ((j == (nb_curve - 1)) && (NeedDoubleDegRepara)) {
                ...
                BSplCLib::FunctionReparameterise(ev, Curve1->Degree(), Curve1FlatKnots, Curve1Poles, FlatKnots,
                                                 2 * Curve1->Degree(), NewPoles, aStatus);
                ...
                BSplCLib::FunctionReparameterise(ev, Curve1->Degree(), Curve1FlatKnots, Curve1Weights, FlatKnots,
                                                 2 * Curve1->Degree(), NewWeights, aStatus);
            ...
            if (fusion == Standard_False) throw Standard_ConstructionError("GeomConvert Concatenation Error");
...
void GeomConvert::ConcatC1(TColGeom_Array1OfBSplineCurve& ArrayOfCurves, const TColStd_Array1OfReal& ArrayOfToler,
                           Handle(TColStd_HArray1OfInteger) & ArrayOfIndices,
                           Handle(TColGeom_HArray1OfBSplineCurve) & ArrayOfConcatenated, Standard_Boolean& ClosedG1Flag,
                           const Standard_Real ClosedTolerance) {
    ConcatC1(ArrayOfCurves, ArrayOfToler, ArrayOfIndices, ArrayOfConcatenated, ClosedG1Flag, ClosedTolerance,
             Precision::Angular());
}
//=======================================================================
// function : ConcatC1
...

{
    ...
    for (i = 0; i < nb_curve; i++) {
        if (i >= 1) {
            First = ArrayOfCurves(i)->FirstParameter();
            Cont = GeomLProp::Continuity(ArrayOfCurves(i - 1), ArrayOfCurves(i), PreLast, First, Standard_True,
                                         Standard_True, local_tolerance(i - 1), AngularTolerance);
            if (Cont < GeomAbs_C0)
                throw Standard_ConstructionError("GeomConvert curves not C0");
            else {
                if (Cont >= GeomAbs_G1)
...
void GeomConvert::C0BSplineToC1BSplineCurve(Handle(Geom_BSplineCurve) & BS, const Standard_Real tolerance,
...
void GeomConvert::C0BSplineToArrayOfC1BSplineCurve(const Handle(Geom_BSplineCurve) & BS,
                                                   Handle(TColGeom_HArray1OfBSplineCurve) & tabBS,
                                                   const Standard_Real AngularTolerance, const Standard_Real tolerance)

{
    ...
    if (nbcurveC1 > 1) {
        ...
        GeomConvert::ConcatC1(ArrayOfCurves, ArrayOfToler, ArrayOfIndices, tabBS, closed_flag, tolerance,
                              AngularTolerance);
    } else {
