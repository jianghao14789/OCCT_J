...
GeomConvert_CompCurveToBSplineCurve::GeomConvert_CompCurveToBSplineCurve(
    ...
    : myTol(Precision::Confusion()), myType(theParameterisation) {
    //
}
...
//=======================================================================
GeomConvert_CompCurveToBSplineCurve::GeomConvert_CompCurveToBSplineCurve(
    const Handle(Geom_BoundedCurve) & BasisCurve, const Convert_ParameterisationType Parameterisation)
    : myTol(Precision::Confusion()), myType(Parameterisation) {
...
Standard_Boolean GeomConvert_CompCurveToBSplineCurve::Add(const Handle(Geom_BoundedCurve) & NewCurve,
                                                          const Standard_Real Tolerance, const Standard_Boolean After,
    ...
    avant = ((myCurve->Pole(1).Distance(Bs->Pole(1)) < myTol) || (myCurve->Pole(1).Distance(Bs->Pole(LBs)) < myTol));
    apres =
        ((myCurve->Pole(LCb).Distance(Bs->Pole(1)) < myTol) || (myCurve->Pole(LCb).Distance(Bs->Pole(LBs)) < myTol));
...
void GeomConvert_CompCurveToBSplineCurve::Add(Handle(Geom_BSplineCurve) & FirstCurve,
                                              Handle(Geom_BSplineCurve) & SecondCurve, const Standard_Boolean After,
                                              const Standard_Boolean WithRatio, const Standard_Integer MinM) {
