...
void RWStepRepr_RWAssemblyComponentUsage::ReadStep(const Handle(StepData_StepReaderData) & data,
    ...
    if (data->IsParamDefined(num, 3)) {
        data->ReadString(num, 3, "product_definition_relationship.description", ach,
    ...
    data->ReadEntity(num, 4, "product_definition_relationship.relating_product_definition", ach,
    ...
    if (aProductDefinitionRelationship_RelatingProductDefinition.Value().IsNull()) {
        ...
        data->ReadEntity(num, 4, "product_definition_relationship.relating_product_definition_shape", ach,
                         STANDARD_TYPE(StepRepr_ProductDefinitionShape), aProductDefinitionShape);
        if (!aProductDefinitionShape.IsNull()) {
            aProductDefinitionRelationship_RelatingProductDefinition.SetValue(
    ...
    data->ReadEntity(num, 5, "product_definition_relationship.related_product_definition", ach,
                     aProductDefinitionRelationship_RelatedProductDefinition);
    if (aProductDefinitionRelationship_RelatedProductDefinition.Value().IsNull()) {
        Handle(StepRepr_ProductDefinitionShape) aProductDefinitionShape;
        data->ReadEntity(num, 5, "product_definition_relationship.related_product_definition_shape", ach,
                         STANDARD_TYPE(StepRepr_ProductDefinitionShape), aProductDefinitionShape);
        if (!aProductDefinitionShape.IsNull()) {
            aProductDefinitionRelationship_RelatedProductDefinition.SetValue(
                aProductDefinitionShape->Definition().ProductDefinition());
        }
    }
    ...
    // Initialize entity
    ent->Init(aProductDefinitionRelationship_Id, aProductDefinitionRelationship_Name,
              hasProductDefinitionRelationship_Description, aProductDefinitionRelationship_Description,
              aProductDefinitionRelationship_RelatingProductDefinition,
              aProductDefinitionRelationship_RelatedProductDefinition, hasReferenceDesignator, aReferenceDesignator);
}
