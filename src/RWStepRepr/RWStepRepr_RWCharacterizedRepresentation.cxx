...
// purpose  :
//=======================================================================
void RWStepRepr_RWCharacterizedRepresentation::ReadStep(const Handle(StepData_StepReaderData) & data,
                                                        const Standard_Integer num, Handle(Interface_Check) & ach,
                                                        const Handle(StepRepr_CharacterizedRepresentation) &
                                                            ent) const {
    ...
    if (data->ReadSubList(num, 3, "items", ach, nsub)) {
        ...
        for (Standard_Integer i = 1; i <= nb; i++) {
            if (data->ReadEntity(nsub, i, "representation_item", ach, STANDARD_TYPE(StepRepr_RepresentationItem),
                                 anItem))
...
// purpose  :
//=======================================================================
void RWStepRepr_RWCharacterizedRepresentation::WriteStep(StepData_StepWriter& SW,
                                                         const Handle(StepRepr_CharacterizedRepresentation) &
                                                             ent) const {
    // name
    SW.Send(ent->Name());
...
// purpose  :
//=======================================================================
void RWStepRepr_RWCharacterizedRepresentation::Share(const Handle(StepRepr_CharacterizedRepresentation) & ent,
