...
void BRepIntCurveSurface_Inter::Init(const TopoDS_Shape& theShape, const GeomAdaptor_Curve& theCurve,
...
void BRepIntCurveSurface_Inter::Init(const TopoDS_Shape& theShape, const gp_Lin& theLine, const Standard_Real theTol) {
...
void BRepIntCurveSurface_Inter::Next() {
    if (myCurrentnbpoints) myCurrentindex++;
    Find();
}
...
void BRepIntCurveSurface_Inter::Find() {
    if (myCurrentnbpoints && myCurrentindex <= myCurrentnbpoints && FindPoint()) return;
    ...
    for (; i <= myFaces.Length(); i++) {
        ...
        if (myFaceBoxes.IsNull()) myFaceBoxes = new Bnd_HArray1OfBox(1, myFaces.Length());
        ...
        Standard_Boolean isOut =
            (myCurve->GetType() == GeomAbs_Line ? aFaceBox.IsOut(myCurve->Line())
...
IntCurveSurface_IntersectionPoint BRepIntCurveSurface_Inter::Point() const {
    if (myCurrentindex == 0) throw StdFail_NotDone();
