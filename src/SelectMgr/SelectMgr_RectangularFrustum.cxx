...
// purpose  :
// =======================================================================
SelectMgr_RectangularFrustum::SelectMgr_RectangularFrustum() : myScale(1.0) {}

// =======================================================================
// function : segmentSegmentDistance
// purpose  :
// =======================================================================
void SelectMgr_RectangularFrustum::segmentSegmentDistance(const gp_Pnt& theSegPnt1, const gp_Pnt& theSegPnt2,
                                                          SelectBasics_PickResult& thePickResult) const {
    gp_XYZ anU = theSegPnt2.XYZ() - theSegPnt1.XYZ();
    gp_XYZ aV =
        myFarPickedPnt.XYZ() -
        myNearPickedPnt.XYZ(); // use unnormalized vector instead of myViewRayDir to clip solutions behind Far plane
...
// purpose  :
// =======================================================================
bool SelectMgr_RectangularFrustum::segmentPlaneIntersection(const gp_Vec& thePlane, const gp_Pnt& thePntOnPlane,
                                                            SelectBasics_PickResult& thePickResult) const {
    gp_XYZ anU =
        myFarPickedPnt.XYZ() -
        myNearPickedPnt
            .XYZ(); // use unnormalized vector instead of myViewRayDir to clip solutions behind Far plane by > 1.0 check
...
void computeFrustum(const gp_Pnt2d theMinPnt, const gp_Pnt2d& theMaxPnt,
                    const Handle(SelectMgr_FrustumBuilder) & theBuilder, gp_Pnt* theVertices, gp_Vec* theEdges) {
    // LeftTopNear
...
void SelectMgr_RectangularFrustum::cacheVertexProjections(SelectMgr_RectangularFrustum* theFrustum) const {
    if (theFrustum->Camera()->IsOrthographic()) {
        ...
        for (Standard_Integer aPlaneIdx = 0; aPlaneIdx < 5; aPlaneIdx += 2) {
            Standard_Real aProj1 =
                theFrustum->myPlanes[aPlaneIdx].XYZ().Dot(theFrustum->myVertices[aVertIdxs[aPlaneIdx]].XYZ());
            Standard_Real aProj2 =
                theFrustum->myPlanes[aPlaneIdx].XYZ().Dot(theFrustum->myVertices[aVertIdxs[aPlaneIdx + 1]].XYZ());
...
void SelectMgr_RectangularFrustum::Build() {
    Standard_ASSERT_RAISE(
        mySelectionType == SelectMgr_SelectionType_Point || mySelectionType == SelectMgr_SelectionType_Box,
    ...
    if (mySelectionType == SelectMgr_SelectionType_Point) {
        ...
        aMinPnt.SetCoord(aMousePos.X() - myPixelTolerance * 0.5, aMousePos.Y() - myPixelTolerance * 0.5);
        aMaxPnt.SetCoord(aMousePos.X() + myPixelTolerance * 0.5, aMousePos.Y() + myPixelTolerance * 0.5);
    } else {
        aMinPnt = mySelRectangle.MinPnt();
        aMaxPnt = mySelRectangle.MaxPnt();
        myNearPickedPnt =
            myBuilder->ProjectPntOnViewPlane((aMinPnt.X() + aMaxPnt.X()) * 0.5, (aMinPnt.Y() + aMaxPnt.Y()) * 0.5, 0.0);
        myFarPickedPnt =
            myBuilder->ProjectPntOnViewPlane((aMinPnt.X() + aMaxPnt.X()) * 0.5, (aMinPnt.Y() + aMaxPnt.Y()) * 0.5, 1.0);
    }
...
// =======================================================================
Handle(SelectMgr_BaseIntersector)
    SelectMgr_RectangularFrustum::ScaleAndTransform(const Standard_Integer theScaleFactor, const gp_GTrsf& theTrsf,
                                                    const Handle(SelectMgr_FrustumBuilder) & theBuilder) const {
    Standard_ASSERT_RAISE(mySelectionType == SelectMgr_SelectionType_Point ||
                              mySelectionType == SelectMgr_SelectionType_Box,
                          "Error! SelectMgr_RectangularFrustum::ScaleAndTransform() should be called after selection "
                          "frustum initialization");

    Standard_ASSERT_RAISE(theScaleFactor >= 0, "Error! Pixel tolerance for selection should not be negative");
    ...
    if (isToScale) {
        ...
        const gp_Pnt2d& aMousePos = mySelRectangle.MousePos();
        const gp_Pnt2d aMinPnt(aMousePos.X() - theScaleFactor * 0.5, aMousePos.Y() - theScaleFactor * 0.5);
        const gp_Pnt2d aMaxPnt(aMousePos.X() + theScaleFactor * 0.5, aMousePos.Y() + theScaleFactor * 0.5);
...
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsBox(const SelectMgr_Vec3& theBoxMin,
    ...
    if (!hasBoxOverlap(theBoxMin, theBoxMax)) return Standard_False;
...
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsPoint(const gp_Pnt& thePnt,
    ...
    if (!hasPointOverlap(thePnt)) return Standard_False;
...
// =======================================================================
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsSegment(const gp_Pnt& thePnt1, const gp_Pnt& thePnt2,
                                                               const SelectMgr_ViewClipRange& theClipRange,
                                                               SelectBasics_PickResult& thePickResult) const {
    Standard_ASSERT_RAISE(
        mySelectionType == SelectMgr_SelectionType_Point || mySelectionType == SelectMgr_SelectionType_Box,
    ...
    if (!hasSegmentOverlap(thePnt1, thePnt2)) return Standard_False;
...
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsPolygon(const TColgp_Array1OfPnt& theArrayOfPnts,
    ...
    if (theSensType == Select3D_TOS_BOUNDARY) {
        ...
        if (aMatchingSegmentsNb == -1) return Standard_False;
...
// =======================================================================
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsTriangle(const gp_Pnt& thePnt1, const gp_Pnt& thePnt2,
    ...
    if (theSensType == Select3D_TOS_BOUNDARY) {
        ...
        const gp_XYZ aTrEdges[3] = {thePnt2.XYZ() - thePnt1.XYZ(), thePnt3.XYZ() - thePnt2.XYZ(),
        ...
        if (aVec12.SquareMagnitude() > gp::Resolution() && myViewRayDir.IsParallel(aVec12, Precision::Angular())) {
...
// purpose  :
// =======================================================================
Standard_Boolean SelectMgr_RectangularFrustum::OverlapsSphere(const gp_Pnt& theCenter, const Standard_Real theRadius,
                                                              const SelectMgr_ViewClipRange& theClipRange,
                                                              SelectBasics_PickResult& thePickResult) const {
    Standard_ASSERT_RAISE(
        mySelectionType == SelectMgr_SelectionType_Point || mySelectionType == SelectMgr_SelectionType_Box,
...
Standard_Real SelectMgr_RectangularFrustum::DistToGeometryCenter(const gp_Pnt& theCOG) const {
    Standard_ASSERT_RAISE(mySelectionType == SelectMgr_SelectionType_Point ||
                              mySelectionType == SelectMgr_SelectionType_Box,
                          "Error! SelectMgr_RectangularFrustum::DistToGeometryCenter() should be called after "
...
void SelectMgr_RectangularFrustum::GetPlanes(NCollection_Vector<SelectMgr_Vec4>& thePlaneEquations) const {
    ...
    for (Standard_Integer aPlaneIdx = 0; aPlaneIdx < 6; ++aPlaneIdx) {
        const gp_Vec& aPlaneNorm =
            Camera()->IsOrthographic() && aPlaneIdx % 2 == 1 ? myPlanes[aPlaneIdx - 1].Reversed() : myPlanes[aPlaneIdx];
