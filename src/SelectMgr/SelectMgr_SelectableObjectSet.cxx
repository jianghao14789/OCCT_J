...
public:
    ...
    virtual Select3D_BndBox3d Box(const Standard_Integer theIndex) const Standard_OVERRIDE {
        ...
        if (aBox.IsVoid()) return Select3D_BndBox3d();
...
private:
    BVHBuilderAdaptorRegular& operator=(BVHBuilderAdaptorRegular) {
        return *this;
    }
...
public:
    ...
    BVHBuilderAdaptorPersistent(ObjectsMap& theObjects, const Handle(Graphic3d_Camera) & theCamera,
        ...
        for (Standard_Integer anI = 1; anI <= myObjects.Size(); ++anI) {
            ...
            if (!aBoundingBox.IsVoid() && !anObject->TransformPersistence().IsNull()) {
                anObject->TransformPersistence()->Apply(theCamera, theProjectionMat, theWorldViewMat, theWinSize.x(),
                                                        theWinSize.y(), aBoundingBox);
            }
            ...
            for (PrsMgr_Presentations::Iterator aPrsIter(anObject->Presentations()); aPrsIter.More(); aPrsIter.Next()) {
                ...
                for (Graphic3d_SequenceOfGroup::Iterator aGroupIter(aPrs3d->Groups()); aGroupIter.More();
                    ...
                    aGroup->TransformPersistence()->Apply(theCamera, theProjectionMat, theWorldViewMat, theWinSize.x(),
                                                          theWinSize.y(), aGroupBox);
...
private:
    BVHBuilderAdaptorPersistent& operator=(BVHBuilderAdaptorPersistent) {
        return *this;
    }
...
SelectMgr_SelectableObjectSet::SelectMgr_SelectableObjectSet() {
    ...
    myBuilder[BVHSubset_3dPersistent] =
        new BVH_LinearBuilder<Standard_Real, 3>(BVH_Constants_LeafNodeSizeSingle, BVH_Constants_MaxTreeDepth);
    myBuilder[BVHSubset_3d] = new BVH_BinnedBuilder<Standard_Real, 3, 4>(BVH_Constants_LeafNodeSizeSingle,
...
void SelectMgr_SelectableObjectSet::UpdateBVH(const Handle(Graphic3d_Camera) & theCam,
    ...
    if (!theCam.IsNull()) {
        ...
        if (!IsEmpty(BVHSubset_3dPersistent) &&
            (myIsDirty[BVHSubset_3dPersistent] || myLastViewState.IsChanged(aViewState) || isWinSizeChanged)) {
            // construct adaptor over private fields to provide direct access for the BVH builder
            BVHBuilderAdaptorPersistent anAdaptor(myObjects[BVHSubset_3dPersistent], theCam, aProjMat, aWorldViewMat,
                                                  theWinSize);
        ...
        // -----------------------------------------------------
        if (!IsEmpty(BVHSubset_2dPersistent) && (myIsDirty[BVHSubset_2dPersistent] ||
                                                 myLastViewState.IsProjectionChanged(aViewState) || isWinSizeChanged)) {
            // construct adaptor over private fields to provide direct access for the BVH builder
            BVHBuilderAdaptorPersistent anAdaptor(myObjects[BVHSubset_2dPersistent], theCam, aProjMat,
