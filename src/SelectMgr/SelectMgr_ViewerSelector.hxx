...
#undef Status
#endif
typedef NCollection_DataMap<Handle(SelectMgr_SelectableObject), Handle(SelectMgr_SensitiveEntitySet)>
    SelectMgr_MapOfObjectSensitives;
typedef NCollection_DataMap<Handle(SelectMgr_SelectableObject), Handle(SelectMgr_SensitiveEntitySet)>::Iterator
...
public:
    ...
    Standard_Integer CustomPixelTolerance() const {
        return myTolerances.CustomTolerance();
    }
    ...
    //! Return the flag determining precedence of picked depth (distance from eye to entity) over entity priority in
    //! sorted results; TRUE by default. When flag is TRUE, priority will be considered only if entities have the same
    //! depth  within the tolerance. When flag is FALSE, entities with higher priority will be in front regardless of
    //! their depth (like x-ray).
    bool ToPickClosest() const {
        return myToPreferClosest;
    }
    ...
    gp_Pnt PickedPoint(const Standard_Integer theRank) const {
        return PickedData(theRank).Point;
    }
    ...
    const Handle(Select3D_BVHBuilder3d) EntitySetBuilder() {
        return myEntitySetBuilder;
    }

    //! Sets the default builder used to construct BVH of entity set.
    //! The new builder will be also assigned for already defined objects, but computed BVH trees will not be
    //! invalidated.
    ...
    Standard_EXPORT TCollection_AsciiString Status(const Handle(SelectMgr_SelectableObject) &
...
public:
    ...
    Standard_EXPORT void Pick(const Standard_Integer theXPix, const Standard_Integer theYPix,
    ...
    Standard_EXPORT void Pick(const Standard_Integer theXPMin, const Standard_Integer theYPMin,
    ...
    Standard_EXPORT Standard_Boolean ToPixMap(Image_PixMap& theImage, const Handle(V3d_View) & theView,
...
public:
    ...
    Standard_EXPORT void DisplaySensitive(const Handle(SelectMgr_Selection) & theSel, const gp_Trsf& theTrsf,
                                          const Handle(V3d_View) & theView,
...
protected:
    ...
    //! Internal function that checks if there is possible overlap between some entity of selectable object theObject
    //! and current selecting volume.
    //! @param theObject [in] the selectable object for traversal.
    //! @param theMgr [in] the (un)transformed copy of the selecting volume manager representing active selection
    //! frustum.
    ...
    Standard_EXPORT void traverseObject(const Handle(SelectMgr_SelectableObject) & theObject,
                                        ...
                                        const Graphic3d_Mat4d& theProjectionMat, const Graphic3d_Mat4d& theWorldViewMat,
...
private:
    ...
    //! Internal function that checks if a current selecting frustum needs to be scaled and transformed for the entity
    //! and performs necessary calculations.
    void computeFrustum(const Handle(Select3D_SensitiveEntity) & theEnt,
                        const SelectMgr_SelectingVolumeManager& theMgrGlobal,
                        const SelectMgr_SelectingVolumeManager& theMgrObject, const gp_GTrsf& theInvTrsf,
                        SelectMgr_FrustumCache& theCachedMgrs, SelectMgr_SelectingVolumeManager& theResMgr);

private:
    //! Compute 3d position for detected entity.
    void updatePoint3d(SelectMgr_SortCriterion& theCriterion, const SelectBasics_PickResult& thePickResult,
                       const Handle(Select3D_SensitiveEntity) & theEntity, const gp_GTrsf& theInversedTrsf,
