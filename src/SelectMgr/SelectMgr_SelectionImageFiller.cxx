...
public:
    GeneratedEntityColorFiller(Image_PixMap& thePixMap, SelectMgr_ViewerSelector* theSelector)
        ...
        for (SelectMgr_SelectableObjectSet::Iterator anObjIter(theSelector->SelectableObjects()); anObjIter.More();
             anObjIter.Next()) {
    ...
    virtual void Fill(const Standard_Integer theCol, const Standard_Integer theRow,
...
public:
    NormalizedDepthFiller(Image_PixMap& thePixMap, SelectMgr_ViewerSelector* theSelector,
                          const Standard_Boolean theToInverse)
        : SelectMgr_SelectionImageFiller(thePixMap, theSelector), myDepthMin(RealLast()), myDepthMax(-RealLast()),
...
public:
    ...
    virtual void Fill(const Standard_Integer theCol, const Standard_Integer theRow,
        ...
        for (SelectMgr_SequenceOfSelection::Iterator aSelIter(aSelectable->Selections()); aSelIter.More();
             aSelIter.Next()) {
            const Handle(SelectMgr_Selection) & aSelection = aSelIter.Value();
            for (NCollection_Vector<Handle(SelectMgr_SensitiveEntity)>::Iterator aSelEntIter(aSelection->Entities());
                 aSelEntIter.More(); aSelEntIter.Next()) {
    ...
    virtual void Fill(const Standard_Integer theCol, const Standard_Integer theRow,
        ...
        if (thePicked <= 0 || thePicked > myMainSel->NbPicked()) {
            ...
            if (aNormal.Modulus() > 0.0f) {
                myImage->SetPixelColor(theCol, theRow,
                                       Quantity_ColorRGBA(aNormal.x() * 0.5f + 0.5f, aNormal.y() * 0.5f + 0.5f,
...
Handle(SelectMgr_SelectionImageFiller) SelectMgr_SelectionImageFiller::CreateFiller(
    Image_PixMap& thePixMap, SelectMgr_ViewerSelector* theSelector, StdSelect_TypeOfSelectionImage theType) {
    switch (theType) {
        case StdSelect_TypeOfSelectionImage_NormalizedDepth:
        case StdSelect_TypeOfSelectionImage_NormalizedDepthInverted: {
            return new NormalizedDepthFiller(thePixMap, theSelector,
