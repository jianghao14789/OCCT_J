...
public:
    CompareResults(const SelectMgr_IndexedDataMapOfOwnerCriterion& theMapOfCriterion, bool theToPreferClosest)
        : myMapOfCriterion(&theMapOfCriterion), myToPreferClosest(theToPreferClosest) {}
...
void SelectMgr_ViewerSelector::updatePoint3d(SelectMgr_SortCriterion& theCriterion,
    ...
    const Standard_Real aSensFactor = myDepthTolType == SelectMgr_TypeOfDepthTolerance_SensitivityFactor
...
SelectMgr_ViewerSelector::SelectMgr_ViewerSelector()
    : myDepthTolerance(0.0), myDepthTolType(SelectMgr_TypeOfDepthTolerance_SensitivityFactor),
      myToPreferClosest(Standard_True), myCameraScale(1.0), myToPrebuildBVH(Standard_False), myIsSorted(Standard_False),
      myIsLeftChildQueuedFirst(Standard_False) {
    myEntitySetBuilder = new BVH_BinnedBuilder<Standard_Real, 3, 4>(BVH_Constants_LeafNodeSizeSingle,
                                                                    BVH_Constants_MaxTreeDepth, Standard_True);
}
...
Standard_Boolean SelectMgr_ViewerSelector::isToScaleFrustum(const Handle(Select3D_SensitiveEntity) & theEntity) {
    return mySelectingVolumeMgr.IsScalableActiveVolume() && sensitivity(theEntity) < myTolerances.Tolerance();
}
...
Standard_Integer SelectMgr_ViewerSelector::sensitivity(const Handle(Select3D_SensitiveEntity) & theEntity) const {
    return myTolerances.IsCustomTolSet() ? theEntity->SensitivityFactor() + myTolerances.CustomTolerance()
                                         : theEntity->SensitivityFactor();
}
...
void SelectMgr_ViewerSelector::checkOverlap(const Handle(Select3D_SensitiveEntity) & theEntity,
                                            const gp_GTrsf& theInversedTrsf, SelectMgr_SelectingVolumeManager& theMgr) {
...
void SelectMgr_ViewerSelector::computeFrustum(const Handle(Select3D_SensitiveEntity) & theEnt,
                                              ...
                                              const gp_GTrsf& theInvTrsf, SelectMgr_FrustumCache& theCachedMgrs,
...
void SelectMgr_ViewerSelector::traverseObject(const Handle(SelectMgr_SelectableObject) & theObject,
    ...
    if (theObject->HasTransformation() || !theObject->TransformPersistence().IsNull()) {
        if (theObject->TransformPersistence().IsNull()) {
            ...
            const Graphic3d_Mat4d aMat = theObject->TransformPersistence()->Compute(
                theCamera, theProjectionMat, theWorldViewMat, theWinSize.x(), theWinSize.y());
            gp_GTrsf aTPers;
            aTPers.SetMat4(aMat);
    ...
    if (!hasEntityTrsfPers && !aMgr.OverlapsBox(aSensitivesTree->MinPoint(0), aSensitivesTree->MaxPoint(0))) {
        return;
    }
    ...
    if (!theObject->ClipPlanes().IsNull() && theObject->ClipPlanes()->ToOverrideGlobal()) {
        ...
        if (theObject->TransformPersistence()->IsZoomOrRotate() && !theMgr.ViewClipping().IsNull()) {
            ...
            for (Graphic3d_SequenceOfHClipPlane::Iterator aPlaneIt(*theMgr.ViewClipping()); aPlaneIt.More();
                 aPlaneIt.Next()) {
    ...
    if (!theMgr.ViewClipping().IsNull() && theMgr.GetActiveSelectionType() == SelectMgr_SelectionType_Box) {
    ...
    for (;;) {
        if (!aSensitivesTree->IsOuter(aNode)) {
            ...
            const Standard_Boolean isLeftChildIn =
                hasEntityTrsfPers ||
                aMgr.OverlapsBox(aSensitivesTree->MinPoint(aLeftChildIdx), aSensitivesTree->MaxPoint(aLeftChildIdx));
            const Standard_Boolean isRightChildIn =
                hasEntityTrsfPers ||
                aMgr.OverlapsBox(aSensitivesTree->MinPoint(aRightChildIdx), aSensitivesTree->MaxPoint(aRightChildIdx));
            ...
            if (!theMgr.ViewClipping().IsNull() && theMgr.GetActiveSelectionType() == SelectMgr_SelectionType_Box) {
                ...
                for (Graphic3d_SequenceOfHClipPlane::Iterator aPlaneIt(*aViewPlanes); aPlaneIt.More();
                    ...
                    {
                        if (aPlane->ProbeBoxTouch(aBBox)) continue;
                        aClipped = true;
                        break;
                    }
                }
            ...
            if (!aClipped) {
                ...
                for (Standard_Integer anIdx = aStartIdx; anIdx <= anEndIdx; ++anIdx) {
                    ...
                    if (!anEnt->TransformPersistence().IsNull()) {
                        ...
                        const Graphic3d_Mat4d aMat = anEnt->TransformPersistence()->Compute(
                            theCamera, theProjectionMat, theWorldViewMat, theWinSize.x(), theWinSize.y());
                        gp_GTrsf aTPers;
                        aTPers.SetMat4(aMat);
...
void SelectMgr_ViewerSelector::TraverseSensitives() {
    ...
    if (!aCamera.IsNull()) {
        ...
        myCameraScale = aCamera->IsOrthographic() ? aCamera->Scale() : 2.0 * Tan(aCamera->FOVy() * M_PI / 360.0);
    ...
    for (Standard_Integer aBVHSetIt = 0; aBVHSetIt < SelectMgr_SelectableObjectSet::BVHSubsetNb; ++aBVHSetIt) {
        ...
        if (aBVHSubset == SelectMgr_SelectableObjectSet::BVHSubset_2dPersistent) {
            ...
            aTFrustum.SetTranslationPart(
                gp_XYZ(aWorldViewMat.GetValue(0, 3), aWorldViewMat.GetValue(1, 3), aWorldViewMat.GetValue(2, 3)));
...
Standard_Boolean SelectMgr_ViewerSelector::Modes(const Handle(SelectMgr_SelectableObject) & theSelectableObject,
    ...
    for (SelectMgr_SequenceOfSelection::Iterator aSelIter(theSelectableObject->Selections()); aSelIter.More();
         aSelIter.Next()) {
...
Standard_Boolean SelectMgr_ViewerSelector::IsActive(const Handle(SelectMgr_SelectableObject) & theSelectableObject,
    ...
    if (!Contains(theSelectableObject)) return Standard_False;
...
TCollection_AsciiString SelectMgr_ViewerSelector::Status(const Handle(SelectMgr_SelectableObject) &
                                                         theSelectableObject) const {
...
void SelectMgr_ViewerSelector::ResetSelectionActivationStatus() {
    for (SelectMgr_MapOfObjectSensitivesIterator aSensitivesIter(myMapOfObjectSensitives); aSensitivesIter.More();
...
// Purpose :
//=======================================================================
void SelectMgr_ViewerSelector::Pick(const Standard_Integer theXPix, const Standard_Integer theYPix,
...
// Purpose :
//=======================================================================
void SelectMgr_ViewerSelector::Pick(const Standard_Integer theXPMin, const Standard_Integer theYPMin,
...
//=======================================================================
void SelectMgr_ViewerSelector::Pick(const TColgp_Array1OfPnt2d& thePolyline, const Handle(V3d_View) & theView) {
    updateZLayers(theView);
...
// purpose  :
//=======================================================================
Standard_Boolean SelectMgr_ViewerSelector::ToPixMap(Image_PixMap& theImage, const Handle(V3d_View) & theView,
...
void SelectMgr_ViewerSelector::DisplaySensitive(const Handle(V3d_View) & theView) {
    for (SelectMgr_SelectableObjectSet::Iterator aSelectableIt(mySelectableObjects); aSelectableIt.More();
         aSelectableIt.Next()) {
        ...
        for (SelectMgr_SequenceOfSelection::Iterator aSelIter(anObj->Selections()); aSelIter.More(); aSelIter.Next()) {
            if (aSelIter.Value()->GetSelectionState() == SelectMgr_SOS_Activated) {
                SelectMgr::ComputeSensitivePrs(aStruct, aSelIter.Value(), anObj->Transformation(),
...
// purpose  :
//=======================================================================
void SelectMgr_ViewerSelector::DisplaySensitive(const Handle(SelectMgr_Selection) & theSel, const gp_Trsf& theTrsf,
                                                const Handle(V3d_View) & theView,
