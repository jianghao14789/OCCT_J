...
// purpose  :
//=======================================================================
SelectMgr_BaseIntersector::SelectMgr_BaseIntersector() : mySelectionType(SelectMgr_SelectionType_Unknown) {
    //
}
...
// purpose  :
//=======================================================================
void SelectMgr_BaseIntersector::SetViewport(const Standard_Real, const Standard_Real, const Standard_Real,
                                            const Standard_Real) {}
...
Standard_Boolean SelectMgr_BaseIntersector::RaySphereIntersection(const gp_Pnt& theCenter,
    ...
    // to find the intersection of the ray (theLoc, theRayDir) and sphere with theCenter(x0, y0, z0) and theRadius(R),
    // you need to solve the equation (x' - x0)^2 + (y' - y0)^2 + (z' - z0)^2 = R^2, where P(x',y',z') = theLoc(x,y,z) +
    // theRayDir(vx,vy,vz) * T at the end of solving, you receive a square equation with respect to T T^2 * (vx^2 + vy^2
    // + vz^2) + 2 * T * (vx*(x - x0) + vy*(y - y0) + vz*(z - z0)) + ((x-x0)^2 + (y-y0)^2 + (z-z0)^2 -R^2) = 0 (= A*T^2
    // + K*T + C) and find T by discriminant D = K^2 - A*C
...
Standard_Boolean SelectMgr_BaseIntersector::RayCylinderIntersection(
    const Standard_Real theBottomRadius, const Standard_Real theTopRadius, const Standard_Real theHeight,
    const gp_Pnt& theLoc, const gp_Dir& theRayDir, Standard_Real& theTimeEnter, Standard_Real& theTimeLeave) const {
    ...
    if (theTopRadius != theBottomRadius) {
        ...
        const Standard_Real aK = aDir.X() * aPnt.X() / (aMaxRad * aMaxRad) + aDir.Y() * aPnt.Y() / (aMaxRad * aMaxRad) -
                                 aDir.Z() * aPnt.Z() / (aConeHeight * aConeHeight);
        const Standard_Real aC = aPnt.X() * aPnt.X() / (aMaxRad * aMaxRad) + aPnt.Y() * aPnt.Y() / (aMaxRad * aMaxRad) -
