...
SelectMgr_SelectableObject::SelectMgr_SelectableObject(const PrsMgr_TypeOfPresentation3d aTypeOfPresentation3d)
    : PrsMgr_PresentableObject(aTypeOfPresentation3d), myGlobalSelMode(0), myAutoHilight(Standard_True) {
    //
}
...
// purpose  :
//=======================================================================
Handle(Prs3d_Presentation) SelectMgr_SelectableObject::GetHilightPresentation(const Handle(PrsMgr_PresentationManager) &
                                                                              theMgr) {
...
// purpose  :
//=======================================================================
Handle(Prs3d_Presentation) SelectMgr_SelectableObject::GetSelectPresentation(const Handle(PrsMgr_PresentationManager) &
                                                                             theMgr) {
...
void SelectMgr_SelectableObject::SetZLayer(const Graphic3d_ZLayerId theLayerId) {
    ...
    if (!mySelectionPrs.IsNull()) mySelectionPrs->SetZLayer(theLayerId);

    if (!myHilightPrs.IsNull()) myHilightPrs->SetZLayer(theLayerId);
...
Bnd_Box SelectMgr_SelectableObject::BndBoxOfSelected(const Handle(SelectMgr_IndexedMapOfOwner) & theOwners) {
    if (theOwners->IsEmpty()) return Bnd_Box();

    Bnd_Box aBnd;
    for (SelectMgr_SequenceOfSelection::Iterator aSelIter(myselections); aSelIter.More(); aSelIter.Next()) {
        const Handle(SelectMgr_Selection) & aSel = aSelIter.Value();
        if (aSel->GetSelectionState() != SelectMgr_SOS_Activated) continue;
        ...
        for (NCollection_Vector<Handle(SelectMgr_SensitiveEntity)>::Iterator aSelEntIter(aSel->Entities());
            ...
            if (theOwners->Contains(anOwner)) {
                ...
                aBnd.Update(aBox.CornerMin().x(), aBox.CornerMin().y(), aBox.CornerMin().z(), aBox.CornerMax().x(),
