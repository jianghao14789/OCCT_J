...
// purpose :
//=======================================================================
SelectMgr_ToleranceMap::SelectMgr_ToleranceMap() : myLargestKey(-1), myCustomTolerance(-1) {
    //
}
...
void SelectMgr_ToleranceMap::Decrement(const Standard_Integer& theTolerance) {
    ...
    if (theTolerance == myLargestKey && *aFreq == 0) {
        myLargestKey = -1;
        for (NCollection_DataMap<Standard_Integer, Standard_Integer>::Iterator anIter(myTolerances); anIter.More();
             anIter.Next()) {
            if (anIter.Value() != 0) {
