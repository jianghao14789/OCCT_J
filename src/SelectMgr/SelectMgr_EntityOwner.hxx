...
public:
    ...
    //! the owners are selectable objects (presentations) a user can give vertex priority [3], edges [2] faces [1] shape
    //! [0], so that if during selection one vertex one edge and one face are simultaneously detected, the vertex will
    //! only be hilighted.
    ...
    //! Highlights selectable object's presentation with display mode in presentation manager with given highlight
    //! style. Also a check for auto-highlight is performed - if selectable object manages highlighting on its own,
    ...
    Standard_DEPRECATED("Deprecated method - IsSelected() should be used instead") Standard_Integer State() const {
    ...
    void State(const Standard_Integer theStatus) {
        myIsSelected = (theStatus == 1);
    }

    //! if owner is not auto hilighted, for group contains many such owners will be called one method HilightSelected of
    //! SelectableObject
    virtual Standard_Boolean IsAutoHilight() const {
        return mySelectable == NULL || mySelectable->IsAutoHilight();
    }

    //! if this method returns TRUE the owner will always call method Hilight for SelectableObject when the owner is
    //! detected. By default it always return FALSE.
...
public:
    //! Sets the selectable object.
    Standard_DEPRECATED("Deprecated method - SetSelectable() should be used instead") void Set(
        const Handle(SelectMgr_SelectableObject) & theSelObj) {
        SetSelectable(theSelObj);
    }

    //! sets the selectable priority of the owner
    Standard_DEPRECATED("Deprecated method - SetPriority() should be used instead") void Set(
...
protected:
    ...
    Standard_Boolean myFromDecomposition; //!< flag indicating this owner points to a part of object (TRUE) or to entire
                                          //!< object (FALSE)
};

DEFINE_STANDARD_HANDLE(SelectMgr_EntityOwner, Standard_Transient)

Standard_DEPRECATED("Deprecated alias - SelectMgr_EntityOwner should be used instead") typedef SelectMgr_EntityOwner
