...
TopoDS_Shape XSAlgo_AlgoContainer::ProcessShape(const TopoDS_Shape& shape, const Standard_Real Prec,
                                                const Standard_Real maxTol, const Standard_CString prscfile,
                                                const Standard_CString pseq, Handle(Standard_Transient) & info,
    ...
    if (!ShapeProcess::Perform(context, seq, theProgress)) return shape; // return original shape
...
static TopoDS_Edge MakeEdgeOnCurve(const TopoDS_Edge edge) {
    ...
    Standard_Real cf, cl;
    if (!sae.Curve3d(edge, C3d, cf, cl, Standard_False)) return result;
...
Standard_Boolean XSAlgo_AlgoContainer::CheckPCurve(const TopoDS_Edge& E, const TopoDS_Face& face,
                                                   const Standard_Real preci, const Standard_Boolean isSeam) const {
    Standard_Real w1, w2;
    ...
    if (isSeam) {
        ...
        if (!sae.PCurve(TopoDS::Edge(REdge), face, seamPC, f, l, Standard_False) || seamPC == thePC)
            seamPC = Handle(Geom2d_Curve)::DownCast(thePC->Copy());
        B.UpdateEdge(edge, thePC, seamPC, face, 0.);
    } else
        B.UpdateEdge(edge, thePC, face, 0.);
    ...
    if (SPmode) B.SameParameter(edge, Standard_False);
    ...
    if (isSeam) {
        ...
        B.UpdateEdge(E, thePC, face, tol);

    B.UpdateVertex(V1, tol);
    B.UpdateVertex(V2, tol);
    B.Range(E, face, w1, w2);
    if (BRep_Tool::SameRange(E)) B.SameRange(E, sr);
    if (BRep_Tool::SameParameter(E)) B.SameParameter(E, sp);
...
void XSAlgo_AlgoContainer::MergeTransferInfo(const Handle(Transfer_TransientProcess) & TP,
    ...
    if (map.Extent() <= 0 && (msg.IsNull() || msg->MapShape().Extent() <= 0)) return;
