...
public:
    Approx_SameParameter_Evaluator(const TColStd_Array1OfReal& theFlatKnots, const TColStd_Array1OfReal& thePoles,
...
void Approx_SameParameter_Evaluator::Evaluate(Standard_Integer*, /*Dimension*/
                                              ...
                                              Standard_Integer* DerivativeRequest, Standard_Real* Result,
    ...
    BSplCLib::Eval(*Parameter, Standard_False, *DerivativeRequest, extrap_mode[0], aDegree, FlatKnots, 1, PolesArray[0],
                   eval_result[0]);
...
// purpose  :
//=======================================================================
static void ProjectPointOnCurve(const Standard_Real InitValue, const gp_Pnt APoint, const Standard_Real Tolerance,
                                const Standard_Integer NumIteration, const Adaptor3d_Curve& Curve,
                                Standard_Boolean& Status, Standard_Real& Result) {
    ...
    do {
        ...
        if (Abs(func) < Tolerance * d1.Magnitude()) {
            ...
            if (Abs(func_derivative) > Toler) param -= func / func_derivative;
...
// purpose  :
//=======================================================================
static Standard_Real ComputeTolReached(const Handle(Adaptor3d_Curve) & c3d, const Adaptor3d_CurveOnSurface& cons,
...
//=======================================================================
static Standard_Boolean Check(const TColStd_Array1OfReal& FlatKnots, const TColStd_Array1OfReal& Poles,
                              const Standard_Integer nbp, const Standard_Real* pc3d,
                              const Handle(Adaptor3d_Curve) & c3d, const Adaptor3d_CurveOnSurface& cons,
                              Standard_Real& tol, const Standard_Real oldtol) {
    const Standard_Integer aDegree = 3;
    Standard_Integer extrap_mode[2] = {aDegree, aDegree};
    ...
    if (aParamFirst < FirstPar) aParamFirst = FirstPar;
    if (aParamLast > LastPar) aParamLast = LastPar;
    ...
    for (Standard_Integer i = 0; i <= nn; i++) {
        ...
        Standard_Real tcons;
        BSplCLib::Eval(tc3d, Standard_False, 0, extrap_mode[0], aDegree, FlatKnots, 1, (Standard_Real&)Poles(1), tcons);
    ...
    for (Standard_Integer i = Poles.Lower() + 1; i <= Poles.Upper(); ++i) {
        ...
        if (aPreviousParam > aCurrentParam) return Standard_False;
    }
...
// purpose  :
//=======================================================================
Approx_SameParameter::Approx_SameParameter(const Handle(Geom_Curve) & C3D, const Handle(Geom2d_Curve) & C2D,
                                           const Handle(Geom_Surface) & S, const Standard_Real Tol)
    : myDeltaMin(Precision::PConfusion()), mySameParameter(Standard_True), myDone(Standard_False) {
...
// purpose  :
//=======================================================================
Approx_SameParameter::Approx_SameParameter(const Handle(Adaptor3d_Curve) & C3D, const Handle(Geom2d_Curve) & C2D,
...
// purpose  :
//=======================================================================
Approx_SameParameter::Approx_SameParameter(const Handle(Adaptor3d_Curve) & C3D, const Handle(Adaptor2d_Curve2d) & C2D,
                                           const Handle(Adaptor3d_Surface) & S, const Standard_Real Tol)
    : myDeltaMin(Precision::PConfusion()), mySameParameter(Standard_True), myDone(Standard_False) {
    myC3d = C3D;
    mySurf = S;
...
void Approx_SameParameter::Build(const Standard_Real Tolerance) {
    ...
    Standard_Real qpcons[myMaxArraySize], qnewpcons[myMaxArraySize], qpc3d[myMaxArraySize], qnewpc3d[myMaxArraySize];
    ...
    do {
        ...
        interpolok = Check(FlatKnots, Poles, aData.myNbPnt + 1, aData.myPC3d, myC3d, aData.myCOnS, algtol, tolsov);
        tolsov = algtol;
        ...
        if (!interpolok) hasCountChanged = IncreaseNbPoles(Poles, FlatKnots, aData, besttol2);
...
Standard_Boolean Approx_SameParameter::CheckSameParameter(Approx_SameParameter_Data& theData,
    ...
    for (Standard_Integer ii = 1; ii < theData.myNbPnt; ii++) {
        ...
        if (!isProjOk) initp = theData.myPC3d[ii];
...
Standard_Boolean Approx_SameParameter::Interpolate(const Approx_SameParameter_Data& theData,
                                                   const Standard_Real aTangFirst, const Standard_Real aTangLast,
    ...
    BSplCLib::Interpolate(3, theFlatKnots, aParameters, ContactOrder, 1, thePoles(1), inversion_problem);
