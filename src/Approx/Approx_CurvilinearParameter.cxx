...
public:
    Approx_CurvilinearParameter_EvalCurv(const Handle(Approx_CurvlinFunc) & theFunc, Standard_Real First,
                                         Standard_Real Last)
        : fonct(theFunc) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
...
void Approx_CurvilinearParameter_EvalCurv::Evaluate(Standard_Integer* Dimension, Standard_Real* StartEnd,
                                                    Standard_Real* Param, Standard_Integer* Order,
                                                    Standard_Real* Result, Standard_Integer* ErrorCode) {
    *ErrorCode = 0;
...
Approx_CurvilinearParameter::Approx_CurvilinearParameter(const Handle(Adaptor3d_Curve) & C3D, const Standard_Real Tol,
                                                         const GeomAbs_Shape Order, const Standard_Integer MaxDegree,
                                                         const Standard_Integer MaxSegments)
    : myMaxError2d1(0.0), myMaxError2d2(0.0) {
#ifdef OCCT_DEBUG_CHRONO
    ...
    AdvApprox_ApproxAFunction aApprox(Num1DSS, Num2DSS, Num3DSS, OneDTolNul, TwoDTolNul, ThreeDTol, FirstS, LastS,
                                      Order, MaxDegree, MaxSegments, evC, CutTool);
...
public:
    Approx_CurvilinearParameter_EvalCurvOnSurf(const Handle(Approx_CurvlinFunc) & theFunc, Standard_Real First,
                                               Standard_Real Last)
        : fonct(theFunc) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
...
void Approx_CurvilinearParameter_EvalCurvOnSurf::Evaluate(Standard_Integer* Dimension, Standard_Real* StartEnd,
                                                          Standard_Real* Param, Standard_Integer* Order,
                                                          Standard_Real* Result, Standard_Integer* ErrorCode) {
    *ErrorCode = 0;
    Standard_Real S = *Param;
...
Approx_CurvilinearParameter::Approx_CurvilinearParameter(const Handle(Adaptor2d_Curve2d) & C2D,
                                                         ...
                                                         const Standard_Real Tol, const GeomAbs_Shape Order,
    ...
    AdvApprox_ApproxAFunction aApprox(Num1DSS, Num2DSS, Num3DSS, OneDTol, TwoDTolNul, ThreeDTol, FirstS, LastS, Order,
                                      MaxDegree, MaxSegments, evCOnS, CutTool);
...
public:
    Approx_CurvilinearParameter_EvalCurvOn2Surf(const Handle(Approx_CurvlinFunc) & theFunc, Standard_Real First,
                                                Standard_Real Last)
        : fonct(theFunc) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
...
void Approx_CurvilinearParameter_EvalCurvOn2Surf::Evaluate(Standard_Integer* Dimension, Standard_Real* StartEnd,
                                                           Standard_Real* Param, Standard_Integer* Order,
                                                           Standard_Real* Result, Standard_Integer* ErrorCode) {
    *ErrorCode = 0;
    Standard_Real S = *Param;
...
Approx_CurvilinearParameter::Approx_CurvilinearParameter(
    const Handle(Adaptor2d_Curve2d) & C2D1, const Handle(Adaptor3d_Surface) & Surf1,
    const Handle(Adaptor2d_Curve2d) & C2D2, const Handle(Adaptor3d_Surface) & Surf2, const Standard_Real Tol,
    const GeomAbs_Shape Order, const Standard_Integer MaxDegree, const Standard_Integer MaxSegments) {
    Standard_Integer i;
    ...
    AdvApprox_ApproxAFunction aApprox(Num1DSS, Num2DSS, Num3DSS, OneDTol, TwoDTolNul, ThreeDTol, FirstS, LastS, Order,
                                      MaxDegree, MaxSegments, evCOn2S, CutTool);
...
void Approx_CurvilinearParameter::Dump(Standard_OStream& o) const {
    ...
    if (myCase == 2 || myCase == 3) o << "myMaxError2d1 = " << myMaxError2d1 << std::endl;
    if (myCase == 3) o << "myMaxError2d2 = " << myMaxError2d2 << std::endl;
...
void Approx_CurvilinearParameter::ToleranceComputation(const Handle(Adaptor2d_Curve2d) & C2D,
                                                       ...
                                                       const Standard_Integer MaxNumber, const Standard_Real Tol,
