...
public:
    Approx_CurveOnSurface_Eval(const Handle(Adaptor3d_Curve) & theFunc, const Handle(Adaptor2d_Curve2d) & theFunc2d,
...
void Approx_CurveOnSurface_Eval::Evaluate(Standard_Integer* Dimension, Standard_Real StartEnd[2],
...
public:
    Approx_CurveOnSurface_Eval3d(const Handle(Adaptor3d_Curve) & theFunc, Standard_Real First, Standard_Real Last)
        : fonct(theFunc) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
    }
...
void Approx_CurveOnSurface_Eval3d::Evaluate(Standard_Integer* Dimension, Standard_Real StartEnd[2],
                                            Standard_Real* Param,    // Parameter at which evaluation
                                            Standard_Integer* Order, // Derivative Request
                                            Standard_Real* Result,   // [Dimension]
                                            Standard_Integer* ErrorCode) {
...
public:
    Approx_CurveOnSurface_Eval2d(const Handle(Adaptor2d_Curve2d) & theFunc2d, Standard_Real First, Standard_Real Last)
        : fonct2d(theFunc2d) {
        StartEndSav[0] = First;
        StartEndSav[1] = Last;
    }
...
void Approx_CurveOnSurface_Eval2d::Evaluate(Standard_Integer* Dimension, Standard_Real StartEnd[2],
                                            Standard_Real* Param,    // Parameter at which evaluation
                                            Standard_Integer* Order, // Derivative Request
                                            Standard_Real* Result,   // [Dimension]
                                            Standard_Integer* ErrorCode) {
...
Approx_CurveOnSurface::Approx_CurveOnSurface(const Handle(Adaptor2d_Curve2d) & C2D,
                                             const Handle(Adaptor3d_Surface) & Surf, const Standard_Real First,
                                             const Standard_Real Last, const Standard_Real Tol, const GeomAbs_Shape S,
                                             const Standard_Integer MaxDegree, const Standard_Integer MaxSegments,
                                             const Standard_Boolean only3d, const Standard_Boolean only2d)
    : myC2D(C2D), mySurf(Surf), myFirst(First), myLast(Last), myTol(Tol), myIsDone(Standard_False),
      myHasResult(Standard_False), myError3d(0.0), myError2dU(0.0), myError2dV(0.0) {
...
Approx_CurveOnSurface::Approx_CurveOnSurface(const Handle(Adaptor2d_Curve2d) & theC2D,
                                             ...
                                             const Standard_Real theLast, const Standard_Real theTol)
    : myC2D(theC2D), mySurf(theSurf), myFirst(theFirst), myLast(theLast), myTol(theTol), myIsDone(Standard_False),
      myHasResult(Standard_False), myError3d(0.0), myError2dU(0.0), myError2dV(0.0) {}
...
// purpose  :
//=============================================================================
void Approx_CurveOnSurface::Perform(const Standard_Integer theMaxSegments, const Standard_Integer theMaxDegree,
                                    const GeomAbs_Shape theContinuity, const Standard_Boolean theOnly3d,
    ...
    AdvApprox_ApproxAFunction aApprox(Num1DSS, Num2DSS, Num3DSS, OneDTol, TwoDTolNul, ThreeDTol, myFirst, myLast,
                                      theContinuity, theMaxDegree, theMaxSegments, *EvalPtr, CutTool);
...
// purpose  :
//=============================================================================
Standard_Boolean Approx_CurveOnSurface::isIsoLine(const Handle(Adaptor2d_Curve2d) theC2D, Standard_Boolean& theIsU,
                                                  Standard_Real& theParam, Standard_Boolean& theIsForward) const {
...
Standard_Boolean Approx_CurveOnSurface::buildC3dOnIsoLine(const Handle(Adaptor2d_Curve2d) theC2D,
    ...
    if (aGeomAdapter.IsNull()) return Standard_False;

    if (mySurf->GetType() == GeomAbs_Sphere) return Standard_False;
    ...
    if (!theIsForward) myCurve3d->Reverse();
    ...
    if (myError3d > myTol) return Standard_False;
