...
static void findfourpoints(const Standard_Real, Standard_Integer NInterval, const Handle(TColStd_HArray1OfReal) & Si,
                           Handle(TColStd_HArray1OfReal) & Ui, const Standard_Real prevS, const Standard_Real prevU,
                           Standard_Real* Xi, Standard_Real* Yi) {
...
Approx_CurvlinFunc::Approx_CurvlinFunc(const Handle(Adaptor3d_Curve) & C, const Standard_Real Tol)
    : myC3D(C), myCase(1), myFirstS(0), myLastS(1), myTolLen(Tol), myPrevS(0.0), myPrevU(0.0) {
    Init();
}

Approx_CurvlinFunc::Approx_CurvlinFunc(const Handle(Adaptor2d_Curve2d) & C2D, const Handle(Adaptor3d_Surface) & S,
                                       const Standard_Real Tol)
    : myC2D1(C2D), mySurf1(S), myCase(2), myFirstS(0), myLastS(1), myTolLen(Tol), myPrevS(0.0), myPrevU(0.0) {
    Init();
}

Approx_CurvlinFunc::Approx_CurvlinFunc(const Handle(Adaptor2d_Curve2d) & C2D1, const Handle(Adaptor2d_Curve2d) & C2D2,
                                       const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_Surface) & S2,
                                       const Standard_Real Tol)
    : myC2D1(C2D1), myC2D2(C2D2), mySurf1(S1), mySurf2(S2), myCase(3), myFirstS(0), myLastS(1), myTolLen(Tol),
      myPrevS(0.0), myPrevU(0.0) {
    Init();
}
...
Standard_Real Approx_CurvlinFunc::Length(Adaptor3d_Curve& C, const Standard_Real FirstU,
...
Standard_Real Approx_CurvlinFunc::GetUParameter(Adaptor3d_Curve& C, const Standard_Real S,
    ...
    if (S == 1)
        ...
        for (i = 0; i < NbInt; i++) {
            if ((InitSArray->Value(i) <= S && S < InitSArray->Value(i + 1))) break;
        }
        NInterval = i;
    }
...
Standard_Real Approx_CurvlinFunc::GetSParameter(Adaptor3d_Curve& C, const Standard_Real U,
...
Standard_Boolean Approx_CurvlinFunc::EvalCase1(const Standard_Real S, const Standard_Integer Order,
...
Standard_Boolean Approx_CurvlinFunc::EvalCase2(const Standard_Real S, const Standard_Integer Order,
                                               TColStd_Array1OfReal& Result) const {
...
Standard_Boolean Approx_CurvlinFunc::EvalCase3(const Standard_Real S, const Standard_Integer Order,
...
Standard_Boolean Approx_CurvlinFunc::EvalCurOnSur(const Standard_Real S, const Standard_Integer Order,
                                                  TColStd_Array1OfReal& Result,
                                                  const Standard_Integer NumberOfCurve) const {
