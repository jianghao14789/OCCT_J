...
//=======================================================================
static Standard_Integer NbConstraint(const AppParCurves_Constraint C1, const AppParCurves_Constraint C2) {
...
//=======================================================================
static Standard_Integer PointsByPick(Handle(AppDef_HArray1OfMultiPointConstraint) & MPC, Draw_Interpretor& di) {
...
static void PointsByFile(Handle(AppDef_HArray1OfMultiPointConstraint) & MPC,
                         Handle(AppParCurves_HArray1OfConstraintCouple) & TABofCC, std::ifstream& iFile,
                         Draw_Interpretor& di) {
...
//============================================================================
{
    ...
    if (Points->Value(1).NbPoints() == 0) {
        ...
        if (TABofCC.IsNull()) {
            ...
            if (TABofCC->Value(1).Constraint() < AppParCurves_PassPoint) TABofCC->SetValue(1, AC1);

            AppParCurves_ConstraintCouple AC2(NbPoints, AppParCurves_PassPoint);
            if (TABofCC->Value(NbPoints).Constraint() < AppParCurves_PassPoint) TABofCC->SetValue(NbPoints, AC2);
        }
        ...
        if (methode < 3) {
            ...
            degmin = Max(degmin, NbConstraint(TABofCC->Value(1).Constraint(), TABofCC->Value(NbPoints).Constraint()));

            AppDef_Compute Appr(degmin, Degree, Abs(Tolerance), Abs(Tolerance), NbIteration, Standard_False,
...
void GeomliteTest::ApproxCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("bzsmooth", "bzsmooth cname tol degree option [fic]", __FILE__, smoothingbybezier, g);
}
