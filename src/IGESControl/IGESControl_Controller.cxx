...
void IGESControl_Controller::Customise(Handle(XSControl_WorkSession) & WS) {
    ...
    if (!xmr.IsNull()) {
        ...
        Handle(IFSelect_SelectType) snosub = new IFSelect_SelectType(STANDARD_TYPE(IGESBasic_SubfigureDef));
        ...
        Handle(IFSelect_SignCounter) itfs = new IFSelect_SignCounter(itf, Standard_False, Standard_True);
...
Handle(Transfer_ActorOfTransientProcess) IGESControl_Controller::ActorRead(const Handle(Interface_InterfaceModel) &
                                                                           model) const {
