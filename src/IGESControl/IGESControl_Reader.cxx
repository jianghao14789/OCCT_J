...
void IGESControl_Reader::PrintTransferInfo(const IFSelect_PrintFail failsonly, const IFSelect_PrintCount mode) const {
    ...
    if (!model.IsNull()) {
        ...
        for (iterTrans.Start(); iterTrans.More(); iterTrans.Next()) {
            ...
            // Init for dicoCountMapping for IFSelect_Mapping
                ...
                sprintf(mess, "%d\t%d\t%s\t%s", igesEnt->TypeNumber(), igesEnt->FormNumber(), "%d",
        ...
        switch (mode) {
            ...
            case IFSelect_ListByItem: {
                ...
                NCollection_DataMap<TCollection_AsciiString, Handle(TColStd_HSequenceOfInteger)>::Iterator aMapListIter(
                    aMapList);
                for (; aMapCountIter.More() && aMapListIter.More(); aMapCountIter.Next(), aMapListIter.Next()) {
                ...
                if (nbRoots != nbResults) {
                    for (Standard_Integer i = 1; i <= nbRoots; i++) {
                        ...
                        if (!TP->IsBound(root)) {
                            ...
                            sprintf(mess, "%d\t%d \t%s\t%s", root->TypeNumber(), root->FormNumber(), "%d", "Failed");
                            // std::cout << mess << std::endl;
                            if (aMapCountMapping.IsBound(mess))
                                aMapCountMapping.ChangeFind(mess)++;
                            else
