...
//           connected.
//=======================================================================
static Standard_Boolean Connect(const Handle(ShapeAnalysis_Wire) & theSAW, const Handle(ShapeExtend_WireData) & theWD,
                                const Handle(ShapeExtend_WireData) & theNextWD, const Standard_Boolean theConnectNextWD,
                                const Standard_Real theMaxTol, Standard_Real& theDistMin,
                                Standard_Boolean& theReverseWD, Standard_Boolean& theReverseNextWD) {
    theSAW->Load(theWD);
    if (theConnectNextWD) {
    ...
    return ShapeAlgo::AlgoContainer()->ConnectNextWire(theSAW, theNextWD, theMaxTol, theDistMin, theReverseWD,
                                                       theReverseNextWD);
}
...
Standard_Boolean IGESControl_IGESBoundary::Transfer(
    Standard_Boolean& okCurve, Standard_Boolean& okCurve3d, Standard_Boolean& okCurve2d,
    const Handle(IGESData_IGESEntity) & icurve3d, const Handle(ShapeExtend_WireData) & scurve3d,
    const Standard_Boolean usescurve, const Standard_Boolean toreverse3d,
    const Handle(IGESData_HArray1OfIGESEntity) & curves2d, const Standard_Boolean toreverse2d,
    const Standard_Integer number, Handle(ShapeExtend_WireData) & Gsewd) {
    ...
    Standard_Boolean GTranslate3d = Standard_True, GTranslate2d = Standard_True, Preferred3d = Standard_True,
                     Preferred2d = Standard_True;
    ...
    if (GTranslate3d && !GTranslate2d) {
        if (usescurve) {
            ...
            if (!Sh.IsNull()) {
                ...
                if (toreverse3d) {
                    ...
                    Gsewd->Add(Sh); // Gsewd = Gsewd3d is impossible to avoid sharing of sewd (UK1.igs entity 7)
            }
        }
        ...
        for (Standard_Integer i = 1; i <= len3d; i++) {
            ...
            okCurve3d = okCurve3d && ShapeAlgo::AlgoContainer()->ConnectNextWire(saw3d, Lsewd3d, maxtol, distmin,
                                                                                 revsewd, revnextsewd);

            Handle(ShapeExtend_WireData) Lsewd2d = new ShapeExtend_WireData;
            TC.SetBadCase(Standard_False); //: 27
            TopoDS_Shape shape2d =
                TC.Transfer2dTopoCurve(Handle(IGESData_IGESEntity)::DownCast(seq2d->Value(i)), myface, mytrsf, myuFact);
            ...
            if (!shape2d.IsNull()) {
                ...
                okCurve2d = okCurve2d && ShapeAlgo::AlgoContainer()->ConnectNextWire(saw2d, Lsewd2d, maxtol, distmin,
                                                                                     revsewd, revnextsewd);
            } else
            ...
            if (LTranslate3d && LTranslate2d &&
                ((isBSpline && (Lsewd3d->NbEdges() != Lsewd2d->NbEdges())) ||
                 (!isBSpline && (Lsewd3d->NbEdges() != 1 || Lsewd2d->NbEdges() != 1)) || bad3d || bad2d)) {
...
#ifdef OCCT_DEBUG
                            std::cout
                                << "Warning: IGESToBRep_IGESBoundary: 3D and 2D curves are inconsistent; 2D is ignored"
                                << std::endl;
#endif
                        } else {
#ifdef OCCT_DEBUG
            ...
            okCurve = okCurve &&
                      ShapeAlgo::AlgoContainer()->ConnectNextWire(saw, Lsewd, maxtol, distmin, revsewd, revnextsewd);
            if (!okCurve) {
#ifdef OCCT_DEBUG
                ...
                for (Standard_Integer j = 1; j <= len3d; j++) {
                    if (usescurve)
                        ...
                        TopoDS_Shape Sh = TC.TransferTopoCurve(Handle(IGESData_IGESEntity)::DownCast(seq3d->Value(j)));
    ...
    if (number > 1) {
        okCurve =
            okCurve && Connect(saw, mysewd, Gsewd, (len3d > 1) || (len2d > 1), maxtol, distmin, revsewd, revnextsewd);
        okCurve3d = okCurve3d && Connect(saw3d, mysewd3d, Gsewd3d, len3d > 1, maxtol, distmin, revsewd, revnextsewd);
        okCurve2d = okCurve2d && Connect(saw2d, mysewd2d, Gsewd2d, len2d > 1, maxtol, distmin, revsewd, revnextsewd);
    } else {
        mysewd = Gsewd;
        mysewd3d = Gsewd3d;
        mysewd2d = Gsewd2d;
