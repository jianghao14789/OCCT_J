...
void CSLib::Normal(
    ...
    const gp_Vec& D1U, const gp_Vec& D1V, const Standard_Real SinTol, CSLib_DerivativeStatus& theStatus,
    gp_Dir& Normal) {
...
void CSLib::Normal(
    ...
    const gp_Vec& D1U, const gp_Vec& D1V, const gp_Vec& D2U, const gp_Vec& D2V, const gp_Vec& DUV,
    const Standard_Real SinTol, Standard_Boolean& Done, CSLib_NormalStatus& theStatus, gp_Dir& Normal) {
...
void CSLib::Normal(
    ...
    const gp_Vec& D1U, const gp_Vec& D1V, const Standard_Real MagTol, CSLib_NormalStatus& theStatus, gp_Dir& Normal) {
...
//
void CSLib::Normal(const Standard_Integer MaxOrder, const TColgp_Array2OfVec& DerNUV, const Standard_Real SinTol,
                   const Standard_Real U, const Standard_Real V, const Standard_Real Umin, const Standard_Real Umax,
                   const Standard_Real Vmin, const Standard_Real Vmax, CSLib_NormalStatus& theStatus, gp_Dir& Normal,
                   Standard_Integer& OrderU, Standard_Integer& OrderV) {
    ...
    if (Trouve) {
        if (Order == 0) {
            ...
            if (!definie) { // All lambda i exist
                ...
                // Find zeros of SAPS
                math_FunctionRoots FindRoots(Poly, inf, sup, 200, 1e-5, Precision::Confusion(), Precision::Confusion());
                // If there are zeros
...
//
gp_Vec CSLib::DNNUV(const Standard_Integer Nu, const Standard_Integer Nv, const TColgp_Array2OfVec& DerSurf) {
...
gp_Vec CSLib::DNNUV(const Standard_Integer Nu, const Standard_Integer Nv, const TColgp_Array2OfVec& DerSurf1,
...
//
gp_Vec CSLib::DNNormal(const Standard_Integer Nu, const Standard_Integer Nv, const TColgp_Array2OfVec& DerNUV,
                       const Standard_Integer Iduref, const Standard_Integer Idvref) {
        ...
        for (Standard_Integer Pderiv = 0; Pderiv <= Mderiv; Pderiv++) {
            ...
            if (Pderiv > Qderiv) {
                ...
                for (Standard_Integer Ideriv = 1; Ideriv < Pderiv; Ideriv++) {
                    for (Standard_Integer Jderiv = 0; Jderiv <= Qderiv; Jderiv++) {
                        Scal = Scal - PLib::Bin(Pderiv, Ideriv) * PLib::Bin(Qderiv, Jderiv) *
                ...
                for (Standard_Integer Ideriv = 1; Ideriv <= Pderiv; Ideriv++) {
                    Scal = Scal - PLib::Bin(Pderiv, Ideriv) * DerVecNor.Value(Ideriv, 0) *
                                      DerVecNor.Value(Pderiv - Ideriv, Qderiv);
                }

                for (Standard_Integer Ideriv = 0; Ideriv < Pderiv; Ideriv++) {
                    Scal = Scal - PLib::Bin(Pderiv, Ideriv) * DerVecNor.Value(Ideriv, Qderiv) *
            ...
            for (Standard_Integer Jderiv = 0; Jderiv < Qderiv; Jderiv++) {
                Dnorm = Dnorm - PLib::Bin(Qderiv + Idvref, Jderiv + Idvref) * TabNorm.Value(Pderiv, Jderiv) *
            ...
            for (Standard_Integer Jderiv = 1; Jderiv <= Qderiv; Jderiv++) {
                DerNor = DerNor - PLib::Bin(Pderiv + Iduref, Iduref) * PLib::Bin(Qderiv + Idvref, Jderiv + Idvref) *
            ...
            DerNor =
                DerNor / PLib::Bin(Pderiv + Iduref, Iduref) / PLib::Bin(Qderiv + Idvref, Idvref) / TabNorm.Value(0, 0);
