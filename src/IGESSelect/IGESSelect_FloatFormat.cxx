...
IGESSelect_FloatFormat::IGESSelect_FloatFormat()
    : thezerosup(Standard_True), themainform("%E"), theformrange("%f"), therangemin(0.1), therangemax(1000.) {}
...
void IGESSelect_FloatFormat::SetFormatForRange(const Standard_CString form, const Standard_Real R1,
...
void IGESSelect_FloatFormat::Format(Standard_Boolean& zerosup, TCollection_AsciiString& mainform,
...
void IGESSelect_FloatFormat::Perform(IFSelect_ContextWrite& /*ctx*/, IGESData_IGESWriter& writer) const {
...
TCollection_AsciiString IGESSelect_FloatFormat::Label() const {
    ...
    if (theformrange.Length() > 0) {
        ...
        Standard_Integer convlen = Interface_FloatWriter::Convert(therangemin, mess, Standard_True, therangemin / 2.,
                                                                  therangemax * 2., "%f", "%f");
        mess[convlen] = ' ';
        mess[convlen + 1] = '\0';
        lab.AssignCat(mess);
        convlen = Interface_FloatWriter::Convert(therangemax, mess, Standard_True, therangemin / 2., therangemax * 2.,
                                                 "%f", "%f");
        mess[convlen] = ':';
        mess[convlen + 1] = '\0';
        lab.AssignCat(mess);
