    ...
    switch (number) {
        ...
        case 7: { //        ****    ListS(ingle)Views
            ...
            for (Standard_Integer i = 1; i <= nb; i++) {
                std::cout << " --  Set n0 " << i << " Item=entity n0: " << WS->Model()->Number(vs->SetItem(i, listdr))
                          << "  List:";
            ...
            std::cout << " --  Remaining Entities (not yet sorted) :" << sets->NbDuplicated(0, Standard_False)
                      << std::endl;
        ...
        case 70: { //        ****    SetUseFlag
            ...
            if (argc > 2) {
                ...
                for (i = 1; i <= nb; i++) {
                    ...
                    if (!ent.IsNull())
                        ent->InitStatus(ent->BlankStatus(), ent->SubordinateStatus(), usefl, ent->HierarchyStatus());
                }
            } else {
