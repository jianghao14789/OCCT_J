...
IGESSelect_EditDirPart::IGESSelect_EditDirPart() : IFSelect_Editor(23) {
...
Handle(TCollection_HAsciiString) IGESSelect_EditDirPart::StringValue(const Handle(IFSelect_EditForm) & /*form*/,
...
Standard_Boolean IGESSelect_EditDirPart::Load(const Handle(IFSelect_EditForm) & form,
    ...
    if (iges->DefLineFont() == IGESData_DefReference) form->LoadValue(6, modl->StringLabel(iges->LineFont()));

    form->LoadValue(7, DefListName(iges->DefLevel()));
    form->LoadValue(8, new TCollection_HAsciiString(iges->Level()));
    if (iges->DefLevel() == IGESData_DefSeveral) form->LoadValue(9, modl->StringLabel(iges->LevelList()));

    form->LoadValue(10, DefListName(iges->DefView()));
    if (iges->DefView() != IGESData_DefNone) form->LoadValue(11, modl->StringLabel(iges->View()));
    ...
    if (iges->DefColor() == IGESData_DefReference) form->LoadValue(21, modl->StringLabel(iges->Color()));
...
Standard_Boolean IGESSelect_EditDirPart::Update(const Handle(IFSelect_EditForm) & form, const Standard_Integer num,
