...
IGESSelect_RebuildDrawings::IGESSelect_RebuildDrawings() : IGESSelect_ModelModifier(Standard_True) {}

void IGESSelect_RebuildDrawings::Performing(IFSelect_ContextModif& ctx, const Handle(IGESData_IGESModel) & target,
    ...
    for (Standard_Integer is = 1; is <= nbs; is++) {
        ...
        for (setl.Start(); setl.More(); setl.Next()) {
            ...
            if (!viewnew.IsNull()) ent->InitView(viewnew);
        }
    }
}
