...
namespace {
static Standard_Integer GetCircleNbPoints(const gp_Circ& theCircle, const Standard_Integer theNbPnts) {
    ...
    if (theNbPnts <= 0) return 0;

    if (theCircle.Radius() > Precision::Confusion()) return 2 * theNbPnts + 1;
...
static void initCircle(Select3D_PointData& thePolygon, const gp_Circ& theCircle, const Standard_Real theU1,
                       const Standard_Real theU2, const Standard_Integer theNbPnts) {
...
Select3D_SensitiveCircle::Select3D_SensitiveCircle(const Handle(SelectMgr_EntityOwner) & theOwnerId,
                                                   const gp_Circ& theCircle, const Standard_Boolean theIsFilled,
                                                   const Standard_Integer theNbPnts)
    : Select3D_SensitivePoly(theOwnerId, !theIsFilled, GetCircleNbPoints(theCircle, theNbPnts)), myCircle(theCircle),
      myStart(0.0), myEnd(2.0 * M_PI) {
...
//=======================================================================
Select3D_SensitiveCircle::Select3D_SensitiveCircle(const Handle(SelectMgr_EntityOwner) & theOwnerId,
                                                   const gp_Circ& theCircle, const Standard_Real theU1,
                                                   const Standard_Real theU2, const Standard_Boolean theIsFilled,
                                                   const Standard_Integer theNbPnts)
    : Select3D_SensitivePoly(theOwnerId, !theIsFilled, GetCircleNbPoints(theCircle, theNbPnts)), myCircle(theCircle),
      myStart(Min(theU1, theU2)), myEnd(Max(theU1, theU2)) {
    mySensType = theIsFilled ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY;
    myCenter3D = theCircle.Position().Location();
    if (myPolyg.Size() != 1) {
        initCircle(myPolyg, theCircle, myStart, myEnd, theNbPnts);
...
Select3D_SensitiveCircle::Select3D_SensitiveCircle(const Handle(SelectMgr_EntityOwner) & theOwnerId,
    ...
    : Select3D_SensitivePoly(theOwnerId, thePnts3d, static_cast<Standard_Boolean>(!theIsFilled)), myStart(0), myEnd(0) {
    mySensType = theIsFilled ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY;
...
Select3D_SensitiveCircle::Select3D_SensitiveCircle(const Handle(SelectMgr_EntityOwner) & theOwnerId,
                                                   ...
                                                   const Standard_Boolean theIsFilled)
    : Select3D_SensitivePoly(theOwnerId, thePnts3d, !theIsFilled), myStart(0), myEnd(0) {
    mySensType = theIsFilled ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY;
