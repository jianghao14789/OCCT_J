...
Select3D_SensitiveTriangulation::Select3D_SensitiveTriangulation(const Handle(SelectMgr_EntityOwner) & theOwnerId,
    ...
    : Select3D_SensitiveSet(theOwnerId), myTriangul(theTrg), myInitLocation(theInitLoc), myPrimitivesNb(0) {
    ...
    if (!theTrg->HasGeometry()) {
        if (myTriangul->HasCachedMinMax()) {
            aCenter =
                0.5 * (myTriangul->CachedMinMax().CornerMin().XYZ() + myTriangul->CachedMinMax().CornerMax().XYZ());
        }
    } else {
        ...
        if (!theIsInterior) {
            ...
            for (Standard_Integer aTriangleIdx = 1; aTriangleIdx <= aNbTriangles; aTriangleIdx++) {
                ...
                const gp_Pnt aTriNodes[3] = {myTriangul->Node(aTrNodeIdx[0]), myTriangul->Node(aTrNodeIdx[1]),
...
Select3D_SensitiveTriangulation::Select3D_SensitiveTriangulation(const Handle(SelectMgr_EntityOwner) & theOwnerId,
                                                                 ...
                                                                 const gp_Pnt& theCOG,
                                                                 const Standard_Boolean theIsInterior)
    : Select3D_SensitiveSet(theOwnerId), myTriangul(theTrg), myInitLocation(theInitLoc), myCDG3D(theCOG),
      myFreeEdges(theFreeEdges), myPrimitivesNb(0) {
    myInvInitLocation = myInitLocation.Transformation().Inverted();
    mySensType = theIsInterior ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY;
    if (theTrg->HasGeometry()) {
...
Select3D_BndBox3d Select3D_SensitiveTriangulation::Box(const Standard_Integer theIdx) const {
    ...
    if (mySensType == Select3D_TOS_INTERIOR) {
        ...
        aMinPnt = SelectMgr_Vec3(Min(aPnt1.X(), Min(aPnt2.X(), aPnt3.X())), Min(aPnt1.Y(), Min(aPnt2.Y(), aPnt3.Y())),
                                 Min(aPnt1.Z(), Min(aPnt2.Z(), aPnt3.Z())));
        aMaxPnt = SelectMgr_Vec3(Max(aPnt1.X(), Max(aPnt2.X(), aPnt3.X())), Max(aPnt1.Y(), Max(aPnt2.Y(), aPnt3.Y())),
        ...
        aMinPnt = SelectMgr_Vec3(Min(aNode1.X(), aNode2.X()), Min(aNode1.Y(), aNode2.Y()), Min(aNode1.Z(), aNode2.Z()));
        aMaxPnt = SelectMgr_Vec3(Max(aNode1.X(), aNode2.X()), Max(aNode1.Y(), aNode2.Y()), Max(aNode1.Z(), aNode2.Z()));
    }
...
//=======================================================================
void Select3D_SensitiveTriangulation::Swap(const Standard_Integer theIdx1, const Standard_Integer theIdx2) {
...
// purpose  :
//=======================================================================
bool Select3D_SensitiveTriangulation::LastDetectedTriangle(Poly_Triangle& theTriangle, gp_Pnt theTriNodes[3]) const {
...
Standard_Boolean Select3D_SensitiveTriangulation::elementIsInside(SelectBasics_SelectingVolumeManager& theMgr,
    ...
    if (mySensType == Select3D_TOS_BOUNDARY) {
        ...
        return theMgr.OverlapsPoint(aPnt1) && theMgr.OverlapsPoint(aPnt2) && theMgr.OverlapsPoint(aPnt3);
    }
}
...
Select3D_BndBox3d Select3D_SensitiveTriangulation::applyTransformation() {
    if (!HasInitLocation()) return myBndBox;
...
void Select3D_SensitiveTriangulation::computeBoundingBox() {
    ...
    if (myTriangul->HasCachedMinMax()) {
        ...
        myBndBox.Add(
            SelectMgr_Vec3(aCachedBox.CornerMin().X(), aCachedBox.CornerMin().Y(), aCachedBox.CornerMin().Z()));
        myBndBox.Add(
            SelectMgr_Vec3(aCachedBox.CornerMax().X(), aCachedBox.CornerMax().Y(), aCachedBox.CornerMax().Z()));
        return;
