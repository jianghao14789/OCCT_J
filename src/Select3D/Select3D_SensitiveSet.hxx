...
public:
    ...
    virtual Standard_Real Center(const Standard_Integer theIdx, const Standard_Integer theAxis) const = 0;
    ...
    Standard_Integer GetLeafNodeSize() const {
        return myContent.Builder()->LeafNodeSize();
    }
...
protected:
    ...
    //! @param theToCheckAllInside flag indicating that even with SelectMgr_SelectingVolumeManager::IsOverlapAllowed()
    //! returning FALSE
    //!        the method will return TRUE if at least one sub-element is fully inside selection volume
    //!        ::elementIsInside(); this is useful for entities allowing local selection of sub-elements using single
    //!        Owner object.
    ...
    //! @param theIsFullInside [IN] when TRUE indicates that entire BVH node is already inside selection volume (in case
    //! of rectangle selection);
    //!                             in this case algorithm might skip checking the element and just register it as
    //!                             detected
    virtual Standard_Boolean overlapsElement(SelectBasics_PickResult& thePickResult,
                                             SelectBasics_SelectingVolumeManager& theMgr, Standard_Integer theElemIdx,
    ...
    //!                             in this case algorithm might skip checking the element and just register it as
    //!                             detected
    virtual Standard_Boolean elementIsInside(SelectBasics_SelectingVolumeManager& theMgr, Standard_Integer theElemIdx,
                                             Standard_Boolean theIsFullInside) = 0;
    ...
    Standard_EXPORT Standard_Boolean processElements(SelectBasics_SelectingVolumeManager& theMgr,
                                                     Standard_Integer theFirstElem, Standard_Integer theLastElem,
...
protected:
    ...
    public:
        //! Empty constructor.
        BvhPrimitiveSet() : BVH_PrimitiveSet3d(Handle(Select3D_BVHBuilder3d)()), mySensitiveSet(NULL) {}
        ...
        const opencascade::handle<BVH_Tree<Standard_Real, 3>>& GetBVH() {
            return BVH();
        }
