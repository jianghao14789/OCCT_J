...
Select3D_SensitiveWire::Select3D_SensitiveWire(const Handle(SelectMgr_EntityOwner) & theOwnerId)
    : Select3D_SensitiveSet(theOwnerId), myCenter(0.0, 0.0, 0.0) {}
...
void Select3D_SensitiveWire::Add(const Handle(Select3D_SensitiveEntity) & theSensitive) {
    if (!theSensitive.IsNull()) myEntities.Append(theSensitive);
    ...
    if (myEntities.Length() != 1) myCenter.ChangeCoord().Divide(2.0);
...
//=======================================================================
Standard_Real Select3D_SensitiveWire::Center(const Standard_Integer theIdx, const Standard_Integer theAxis) const {
    const Standard_Integer aSensitiveIdx = myEntityIndexes.Value(theIdx);
...
Standard_Boolean Select3D_SensitiveWire::overlapsElement(SelectBasics_PickResult& thePickResult,
                                                         ...
                                                         Standard_Integer theElemIdx, Standard_Boolean) {
...
void Select3D_SensitiveWire::DumpJson(Standard_OStream& theOStream, Standard_Integer theDepth) const {
    ...
    for (NCollection_Vector<Handle(Select3D_SensitiveEntity)>::Iterator anIterator(myEntities); anIterator.More();
         anIterator.Next()) {
