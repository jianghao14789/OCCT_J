...
public:
    ...
    //! Gets group content
    const Select3D_IndexedMapOfEntity& Entities() const {
        return myEntities;
    }
    ...
    //! Returns TRUE if all sensitive entities should be checked within rectangular/polygonal selection, FALSE by
    //! default. Can be useful for sensitive entities holding detection results as class property.
...
private:
    ...
    Standard_Boolean
        myToCheckOverlapAll; //!< flag to check overlapping with all entities within rectangular/polygonal selection
