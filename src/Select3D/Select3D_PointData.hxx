...
public:
    ...
    Select3D_PointData(const Standard_Integer theNbPoints) : mynbpoints(theNbPoints) {
        if (theNbPoints <= 0) throw Standard_ConstructionError("Select3D_PointData");
    ...
    void SetPnt(const Standard_Integer theIndex, const Select3D_Pnt& theValue) {
        if (theIndex < 0 || theIndex >= mynbpoints) throw Standard_OutOfRange("Select3D_PointData::SetPnt");
    ...
    const Select3D_Pnt& Pnt(const Standard_Integer theIndex) const {
        if (theIndex < 0 || theIndex >= mynbpoints) throw Standard_OutOfRange("Select3D_PointData::Pnt");
