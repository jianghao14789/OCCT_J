...
static inline void getTriIndices(const Handle(Graphic3d_IndexBuffer) & theIndices,
                                 const Standard_Integer theIndexOffset, Standard_Integer* theNodes) {
...
struct Select3D_SensitivePrimitiveArray::Select3D_SensitivePrimitiveArray_InitFunctor {
    Select3D_SensitivePrimitiveArray_InitFunctor(Select3D_SensitivePrimitiveArray& thePrimArray,
                                                 Standard_Integer theDivStep, Standard_Boolean theToEvalMinMax)
        : myPrimArray(thePrimArray), myDivStep(theDivStep), myToEvalMinMax(theToEvalMinMax),
          myToComputeBvh(Standard_True), myNbFailures(0) {}
    void operator()(const Standard_Integer& theIndex) const {
        ...
        switch (myPrimArray.myPrimType) {
            case Graphic3d_TOPA_POINTS: {
                if (!anEntity->InitPoints(myPrimArray.myVerts, myPrimArray.myIndices, myPrimArray.myInitLocation,
                                          aLower, anUpper, myToEvalMinMax, 1)) {
            ...
            case Graphic3d_TOPA_TRIANGLES: {
                if (!anEntity->InitTriangulation(myPrimArray.myVerts, myPrimArray.myIndices, myPrimArray.myInitLocation,
                                                 aLower, anUpper, myToEvalMinMax, 1)) {
                    Standard_Atomic_Increment(&myNbFailures);
                    return;
                }
...
struct Select3D_SensitivePrimitiveArray::Select3D_SensitivePrimitiveArray_BVHFunctor {
    Select3D_SensitivePrimitiveArray_BVHFunctor(NCollection_Array1<Handle(Select3D_SensitivePrimitiveArray)>& theGroups)
        : myGroups(theGroups) {}
    void operator()(const Standard_Integer& theIndex) const {
        myGroups.ChangeValue(theIndex)->BVH();
    }
...
Select3D_SensitivePrimitiveArray::Select3D_SensitivePrimitiveArray(const Handle(SelectMgr_EntityOwner) & theOwnerId)
    : Select3D_SensitiveSet(theOwnerId), myPosData(NULL), myPosStride(Standard_Size(-1)),
      myPrimType(Graphic3d_TOPA_UNDEFINED), myIndexLower(0), myIndexUpper(0), myPatchSizeMax(1),
      myPatchDistance(ShortRealLast()), myIs3d(false), myBvhIndices(Graphic3d_Buffer::DefaultAllocator()),
      myMinDepthElem(RealLast()), myMinDepthNode(RealLast()), myMinDepthEdge(RealLast()), myDetectedElem(-1),
      myDetectedNode(-1), myDetectedEdgeNode1(-1), myDetectedEdgeNode2(-1), myToDetectElem(true), myToDetectNode(false),
...
bool Select3D_SensitivePrimitiveArray::InitTriangulation(
    const Handle(Graphic3d_Buffer) & theVerts, const Handle(Graphic3d_IndexBuffer) & theIndices,
    const TopLoc_Location& theInitLoc, const Standard_Integer theIndexLower, const Standard_Integer theIndexUpper,
    const bool theToEvalMinMax, const Standard_Integer theNbGroups) {
    MarkDirty();
    myGroups.Nullify();
...
bool Select3D_SensitivePrimitiveArray::InitPoints(const Handle(Graphic3d_Buffer) & theVerts,
                                                  ...
                                                  const Standard_Integer theIndexUpper, const bool theToEvalMinMax,
    ...
    const float aPatchSize2 = myPatchDistance < ShortRealLast() ? myPatchDistance * myPatchDistance : myPatchDistance;
    ...
    for (Standard_Integer aPointIter = 0; aPointIter < aNbPoints; ++aPointIter) {
        ...
        const Standard_Integer aPointIndex = !myIndices.IsNull() ? myIndices->Index(anIndexOffset) : anIndexOffset;
        if (myIs3d) {
...
Handle(Select3D_SensitiveEntity) Select3D_SensitivePrimitiveArray::GetConnected() {
    ...
    switch (myPrimType) {
        case Graphic3d_TOPA_POINTS: {
            aNewEntity->InitPoints(myVerts, myIndices, myInitLocation, myIndexLower, myIndexUpper, true,
                                   !myGroups.IsNull() ? myGroups->Size() : 1);
            break;
        }
        case Graphic3d_TOPA_TRIANGLES: {
            aNewEntity->InitTriangulation(myVerts, myIndices, myInitLocation, myIndexLower, myIndexUpper, true,
                                          !myGroups.IsNull() ? myGroups->Size() : 1);
            break;
        }
        default:
...
// purpose  :
// =======================================================================
void Select3D_SensitivePrimitiveArray::Swap(const Standard_Integer theIdx1, const Standard_Integer theIdx2) {
...
Standard_Boolean Select3D_SensitivePrimitiveArray::Matches(SelectBasics_SelectingVolumeManager& theMgr,
    ...
    if (myGroups.IsNull() || theMgr.GetActiveSelectionType() == SelectMgr_SelectionType_Point || !toDetectRange) {
    ...
    switch (myPrimType) {
        ...
        case Graphic3d_TOPA_TRIANGLES: {
            ...
            for (Standard_Integer anElemIter = 0; anElemIter < aPatchSize; ++anElemIter) {
                ...
                if (!theIsFullInside && (!theMgr.OverlapsPoint(aPnts[0]) || !theMgr.OverlapsPoint(aPnts[1]) ||
