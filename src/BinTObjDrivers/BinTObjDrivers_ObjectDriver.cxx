...
BinTObjDrivers_ObjectDriver::BinTObjDrivers_ObjectDriver(const Handle(Message_Messenger) & theMessageDriver)
...
Standard_Boolean BinTObjDrivers_ObjectDriver::Paste(const BinObjMgt_Persistent& theSource,
    ...
    if ((unsigned)anID > 0xffff) {
        ...
        if (anObject.IsNull()) {
            ...
            myMessageDriver->Send(TCollection_ExtendedString("TObj_TObject retrieval: wrong object type name ") +
                                      aName + ", entry " + anEntry,
                                  Message_Fail);
...
void BinTObjDrivers_ObjectDriver::Paste(const Handle(TDF_Attribute) & theSource, BinObjMgt_Persistent& theTarget,
