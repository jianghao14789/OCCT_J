...
BinTObjDrivers_IntSparseArrayDriver::BinTObjDrivers_IntSparseArrayDriver(const Handle(Message_Messenger) &
                                                                         theMessageDriver)
...
Standard_Boolean BinTObjDrivers_IntSparseArrayDriver::Paste(const BinObjMgt_Persistent& theSource,
    ...
    if (!(theSource >> anId) || anId < 0) return Standard_False;
    while (anId) {
        Standard_Integer aValue;
        if (!(theSource >> aValue) || aValue <= 0) return Standard_False;
...
void BinTObjDrivers_IntSparseArrayDriver::Paste(const Handle(TDF_Attribute) & theSource,
                                                BinObjMgt_Persistent& theTarget, BinObjMgt_SRelocationTable&) const {
