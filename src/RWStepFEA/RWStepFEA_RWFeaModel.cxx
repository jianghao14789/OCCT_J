...
void RWStepFEA_RWFeaModel::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
                                    Handle(Interface_Check) & ach, const Handle(StepFEA_FeaModel) & ent) const {
    ...
    data->ReadEntity(num, 3, "representation.context_of_items", ach, STANDARD_TYPE(StepRepr_RepresentationContext),
    ...
    // Initialize entity
    ent->Init(aRepresentation_Name, aRepresentation_Items, aRepresentation_ContextOfItems, aCreatingSoftware,
              aIntendedAnalysisCode, aDescription, aAnalysisType);
}
...
void RWStepFEA_RWFeaModel::WriteStep(StepData_StepWriter& SW, const Handle(StepFEA_FeaModel) & ent) const {
    ...
    for (Standard_Integer i4 = 1; i4 <= ent->IntendedAnalysisCode()->Length(); i4++) {
        Handle(TCollection_HAsciiString) Var0 = new TCollection_HAsciiString(ent->IntendedAnalysisCode()->Value(i4));
        SW.Send(Var0);
    }
    SW.CloseSub();
...
void RWStepFEA_RWFeaModel::Share(const Handle(StepFEA_FeaModel) & ent, Interface_EntityIterator& iter) const {
