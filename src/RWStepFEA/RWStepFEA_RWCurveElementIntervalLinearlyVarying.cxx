...
void RWStepFEA_RWCurveElementIntervalLinearlyVarying::ReadStep(
    ...
    data->ReadEntity(num, 1, "curve_element_interval.finish_position", ach, STANDARD_TYPE(StepFEA_CurveElementLocation),
                     aCurveElementInterval_FinishPosition);

    Handle(StepBasic_EulerAngles) aCurveElementInterval_EuAngles;
    data->ReadEntity(num, 2, "curve_element_interval.eu_angles", ach, STANDARD_TYPE(StepBasic_EulerAngles),
    ...
    if (data->ReadSubList(num, 3, "sections", ach, sub3)) {
        ...
        for (Standard_Integer i0 = 1; i0 <= nb0; i0++) {
            ...
            data->ReadEntity(num2, i0, "curve_element_section_definition", ach,
                             STANDARD_TYPE(StepElement_CurveElementSectionDefinition), anIt0);
...
void RWStepFEA_RWCurveElementIntervalLinearlyVarying::WriteStep(
    StepData_StepWriter& SW, const Handle(StepFEA_CurveElementIntervalLinearlyVarying) & ent) const {
...
void RWStepFEA_RWCurveElementIntervalLinearlyVarying::Share(const Handle(StepFEA_CurveElementIntervalLinearlyVarying) &
                                                                ent,
