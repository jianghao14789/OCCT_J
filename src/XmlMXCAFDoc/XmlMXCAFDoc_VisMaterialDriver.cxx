...
static void writeReal(XmlObjMgt_Persistent& theTarget, const XmlObjMgt_DOMString& theName,
                      const Standard_ShortReal theValue) {
    theTarget.Element().setAttribute(theName, TCollection_AsciiString(theValue).ToCString());
}

//! Encode short real value.
static bool readReal(const XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theName,
...
//! Encode vec3.
static void writeVec3(XmlObjMgt_Persistent& theTarget, const XmlObjMgt_DOMString& theName,
...
//! Decode vec3.
static bool readVec3(const XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theName, Graphic3d_Vec3& theVec3) {
...
//! Decode vec3.
static bool readColor(const XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theName,
...
//! Encode vec4.
static void writeVec4(XmlObjMgt_Persistent& theTarget, const XmlObjMgt_DOMString& theName,
...
//! Decode vec34
static bool readVec4(const XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theName, Graphic3d_Vec4& theVec4) {
...
//! Encode texture path.
static void writeTexture(XmlObjMgt_Persistent& theTarget, const XmlObjMgt_DOMString& theName,
                         const Handle(Image_Texture) & theImage) {
    if (!theImage.IsNull() && !theImage->FilePath().IsEmpty() && theImage->FileOffset() == -1) {
        theTarget.Element().setAttribute(theName, theImage->FilePath().ToCString());
    }
}

//! Decode texture path.
static void readTexture(const XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theName,
...
Standard_Boolean XmlMXCAFDoc_VisMaterialDriver::Paste(const XmlObjMgt_Persistent& theSource,
    ...
    aMat->SetFaceCulling(aDoubleSidedInt == 1 ? Graphic3d_TypeOfBackfacingModel_DoubleSided
                                              : (aDoubleSidedInt == 2 ? Graphic3d_TypeOfBackfacingModel_BackCulled
...
//=======================================================================
void XmlMXCAFDoc_VisMaterialDriver::Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
                                          XmlObjMgt_SRelocationTable&) const {
    Handle(XCAFDoc_VisMaterial) aMat = Handle(XCAFDoc_VisMaterial)::DownCast(theSource);
    Standard_Integer aDoubleSidedInt =
        aMat->FaceCulling() == Graphic3d_TypeOfBackfacingModel_DoubleSided
            ? 1
            : (aMat->FaceCulling() == Graphic3d_TypeOfBackfacingModel_BackCulled ? 2 : 0);
