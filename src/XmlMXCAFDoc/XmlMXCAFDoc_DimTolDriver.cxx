...
Standard_Boolean XmlMXCAFDoc_DimTolDriver::Paste(const XmlObjMgt_Persistent& theSource,
    ...
    if (aNameStr == NULL || aDescrStr == NULL) {
        TCollection_ExtendedString aMessageString("Cannot retrieve DimTol attribute name or description");
        myMessageDriver->Send(aMessageString, Message_Fail);
        return Standard_False;
    }
    ...
    if (aFirstIndex == NULL)
        ...
        TCollection_ExtendedString aMessageString("Cannot retrieve the DimTol first index for real array ");
        myMessageDriver->Send(aMessageString, Message_Fail);
        return Standard_False;
    }
    ...
    if (aLastIndex == NULL)
        ...
        TCollection_ExtendedString aMessageString("Cannot retrieve the DimTol last index for real array ");
        myMessageDriver->Send(aMessageString, Message_Fail);
        return Standard_False;
    }
    ...
    if (aLength > 0) {
        ...
        for (Standard_Integer ind = aFirstInd; ind <= aLastInd; ind++) {
            if (!XmlObjMgt::GetReal(aValueStr, aValue)) {
                TCollection_ExtendedString aMessageString = TCollection_ExtendedString("Cannot retrieve real member"
                                                                                       " for real array \"") +
                                                            aValueStr + "\"";
                myMessageDriver->Send(aMessageString, Message_Fail);
                return Standard_False;
...
//=======================================================================
void XmlMXCAFDoc_DimTolDriver::Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
