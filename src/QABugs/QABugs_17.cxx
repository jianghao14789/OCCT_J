...
static Standard_Integer BUC60842(Draw_Interpretor& di, Standard_Integer /*argc*/, const char** /*argv*/) {
    ...
    Handle(Geom_Circle) cir =
        new Geom_Circle(gp_Ax2(gp_Pnt(823.687192, 502.366825, 478.960440), gp_Dir(0.173648, 0.984808, 0.000000),
                               gp_Dir(-0.932169, 0.164367, -0.322560)),
                        50);
    Handle(Geom_Ellipse) ell =
        new Geom_Ellipse(gp_Ax2(gp_Pnt(1262.224429, 425.040878, 363.609716), gp_Dir(0.173648, 0.984808, 0.000000),
                                gp_Dir(-0.932169, 0.164367, -0.322560)),
                         150, 100);
    Handle(Geom_Plane) plne =
        new Geom_Plane(gp_Ax3(gp_Ax2(gp_Pnt(1262.224429, 425.040878, 363.609716), gp_Dir(0.173648, 0.984808, 0.000000),
                                     gp_Dir(-0.932169, 0.164367, -0.322560))));
    ...
    if (!aContext.IsNull()) {
        Handle(AIS_Shape) aisp = new AIS_Shape(BRepBuilderAPI_MakeEdge(GeomAPI::To3d(curve2d, pln)).Edge());
    ...
    if (!aContext.IsNull()) {
        Handle(AIS_Shape) aisp = new AIS_Shape(BRepBuilderAPI_MakeEdge(GeomAPI::To3d(fromcurve2d, pln)).Edge());
        aContext->Display(aisp, Standard_False);
    }
    ...
    for (i = 0; i < lintan.NbSolutions(); i++) {
        ...
        if (!aContext.IsNull()) {
            Handle(AIS_Shape) aisp = new AIS_Shape(BRepBuilderAPI_MakeEdge(GeomAPI::To3d(glin, pln)).Edge());
            aContext->Display(aisp, Standard_False);
        }
    }
...
static Standard_Integer BUC60843(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc >= 5) par1 = Draw::Atof(argv[4]);
    if (argc == 6) par2 = Draw::Atof(argv[5]);
    if (argc == 7) tol = Draw::Atof(argv[6]);
...
static Standard_Integer OCC884(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (advWA->StatusSelfIntersection(status)) di << "Info: No P Curve found in the edge\n";

    status = ShapeExtend_FAIL2;
    if (advWA->StatusSelfIntersection(status)) di << "Info: No Vertices found in the edge\n";

    status = ShapeExtend_DONE1;
    if (advWA->StatusSelfIntersection(status)) di << "Info: Self-intersection found in the edge\n";
    ...
    if (sfw->StatusSelfIntersection(status)) di << "Info: ShapeExtend_OK : No intersection found\n";
    ...
    status = ShapeExtend_FAIL2;
    if (sfw->StatusSelfIntersection(status))
        di << "Info: ShapeExtend_FAIL2 : self-intersection was found, but not fixed because of limit of increasing "
              "tolerance (MaxTolerance)\n";

    status = ShapeExtend_FAIL3;
    if (sfw->StatusSelfIntersection(status))
        di << "Info: ShapeExtend_FAIL3 : intercestion of non adjacent edges found, but not fixed because of limit of "
    ...
    if (sfw->StatusSelfIntersection(status)) di << "Info: ShapeExtend_DONE4 : pcurve(s) was(were) modified\n";
...
static Standard_Integer OCC1642(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (j = 1; j <= M.Extent(); ++j) {
        ...
        std::cout << "\n j =" << j
                  << ",  CheckSelfIntersectingEdge = " << advWA->CheckSelfIntersectingEdge(j, points2d, points3d);

        ShapeExtend_Status status = ShapeExtend_FAIL1;
        if (advWA->StatusSelfIntersection(status)) status = ShapeExtend_FAIL2;
        if (advWA->StatusSelfIntersection(status)) di << "\n No Vertices found in the edge";
    ...
    if (sfw->StatusSelfIntersection(status)) di << "\n ShapeExtend_OK : No intersection found";
    ...
    status = ShapeExtend_FAIL2;
    if (sfw->StatusSelfIntersection(status))
        di << "\n ShapeExtend_FAIL2 : self-intersection was found, but not fixed because of limit of increasing "
              "tolerance (MaxTolerance)";

    status = ShapeExtend_FAIL3;
    if (sfw->StatusSelfIntersection(status))
        di << "\n ShapeExtend_FAIL3 : intercestion of non adjacent edges found, but not fixed because of limit of "
              "increasing tolerance (MaxTolerance)";
...
void QABugs::Commands_17(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("BUC60843", "BUC60843 result_shape name_of_circle name_of_curve [ par1 [ tol ] ]", __FILE__,
                    BUC60843, group);
    ...
    theCommands.Add("OCC566", "OCC566 shape [ xmin ymin zmin xmax ymax zmax] ; print bounding box", __FILE__, OCC566,
                    group);
    theCommands.Add("OCC570", "OCC570 result", __FILE__, OCC570, group);

    theCommands.Add("OCC570mkevol", "OCC570mkevol result object (then use updatevol) [R/Q/P]; mkevol", __FILE__, MKEVOL,
                    group);
    theCommands.Add("OCC570updatevol", "OCC570updatevol edge u1 rad1 u2 rad2 ...; updatevol", __FILE__, UPDATEVOL,
                    group);
    theCommands.Add("OCC570updatevollaw", "OCC570updatevollaw edge u1 rad1 u2 rad2 ...; updatevollaw", __FILE__,
                    UPDATEVOL, group);
    theCommands.Add("OCC570buildevol", "OCC570buildevol; end of the evol fillet computation", __FILE__, BUILDEVOL,
                    group);
    ...
    theCommands.Add("OCC1642",
                    "OCC1642 FinalWare FinalFace InitWare InitFace shape FixReorder FixDegenerated FixConnected "
                    "FixSelfIntersection",
                    __FILE__, OCC1642, group);
