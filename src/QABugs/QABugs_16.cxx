...
// purpose  :
//=======================================================================
static Standard_Integer BUC60774(Draw_Interpretor& theDi, Standard_Integer theArgNb, const char** theArgv) {
    if (theArgNb != 1) {
    ...
    AIS_StatusOfPick aPickStatus = anAISContext->SelectRectangle(Graphic3d_Vec2i(aXPixMin, aYPixMin),
                                                                 Graphic3d_Vec2i(aXPixMax, aYPixMax), aV3dView);
    theDi << (aPickStatus == AIS_SOP_NothingSelected ? "status = AIS_SOP_NothingSelected : OK"
    ...
    theDi << (aPickStatus == AIS_SOP_SeveralSelected ? "status = AIS_SOP_SeveralSelected : OK"
...
static Standard_Integer OCC394(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc > k) tol = Draw::Atof(argv[k++]);

    if (argc > k) mode = Draw::Atoi(argv[k++]);

    if (argc > k) tolang = Draw::Atof(argv[k++]);
