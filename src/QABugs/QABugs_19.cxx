...
public:
    IncrementerDecrementer(Standard_Integer* theVal, Standard_Boolean thePositive)
        : myVal(theVal), myPositive(thePositive) {}
...
static Standard_Boolean OCC23774Test(const TopoDS_Face& grossPlateFace, const TopoDS_Shape& originalWire,
                                     Draw_Interpretor& di) {
    ...
    if (!distShapeShape1.IsDone()) return Standard_False;
    ...
    if (!distShapeShape2.IsDone()) return Standard_False;
...
static Standard_Integer OCC23774(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (!OCC23774Test(aFace, S2, di)) di << "Something is wrong\n";
...
static Standard_Integer OCC23952sweep(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (int i = 0; i < NBTHREADS; i++) {
        ...
        if (!aThread[i].Run(&aStorage)) di << "Error: Cannot start thread << " << i << "\n";
    }

    // check results
    for (int i = 0; i < NBTHREADS; i++) {
        Standard_Address aResult = 0;
        if (!aThread[i].Wait(aResult)) di << "Error: Failed waiting for thread << " << i << "\n";
...
static Standard_Integer OCC24005(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgv) {
    ...
    Handle(Geom_Plane)
        plane(new Geom_Plane(gp_Ax3(gp_Pnt(-72.948737453424499, 754.30437716359393, 259.52151854671678),
    ...
    Handle(Geom_CylindricalSurface)
        cylinder(new Geom_CylindricalSurface(gp_Ax3(gp_Pnt(-6.4812490053250649, 753.39408794522092, 279.16400974257465),
...
static Standard_Integer OCC24271(Draw_Interpretor& di, Standard_Integer /*theArgNb*/, const char** /*theArgVec*/) {
    // input data
...
#define QVERIFY(val1) di << "Checking " #val1 " == Standard_True" << ((val1) == Standard_True ? ": OK\n" : ": Error\n")
...
static Handle(Geom_ConicalSurface)
    CreateCone(const gp_Pnt& theLoc, const gp_Dir& theDir, const gp_Dir& theXDir, const Standard_Real theRad,
               const Standard_Real theSin, const Standard_Real theCos) {
...
static Standard_Integer OCC23972(Draw_Interpretor& /*theDI*/, Standard_Integer theNArg, const char** theArgs) {
    ...
    const Handle(Geom_Surface) aS1 =
        CreateCone(gp_Pnt(123.694345356663, 789.9, 68.15), gp_Dir(-1, 3.48029791472957e-016, -8.41302743359754e-017),
                   gp_Dir(-3.48029791472957e-016, -1, -3.17572289932207e-016), 3.28206830417112, 0.780868809443031,
                   0.624695047554424);
    const Handle(Geom_Surface) aS2 =
        CreateCone(gp_Pnt(123.694345356663, 784.9, 68.15), gp_Dir(-1, -2.5209507537117e-016, -1.49772808948866e-016),
                   gp_Dir(1.49772808948866e-016, 3.17572289932207e-016, -1), 3.28206830417112, 0.780868809443031,
                   0.624695047554424);
...
template <typename T, typename HT> static void DoIsNull(Draw_Interpretor& di) {
    HT aHandle;
...
static Standard_Integer OCC24667(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n >= 6) ForceApproxC1 = Standard_True;
...
// purpose  :
//=======================================================================
static Standard_Integer OCC24889(Draw_Interpretor& theDI, Standard_Integer /*theNArg*/, const char** /*theArgs*/) {
    // Curves
    ...
    Handle(Geom2d_TrimmedCurve) aTrim[2] = {new Geom2d_TrimmedCurve(aCircle1, 1.57079632679490, 2.97959469729228),
...
static Standard_Integer OCC25004(Draw_Interpretor& theDI, Standard_Integer /*theNArg*/, const char** /*theArgs*/) {
...
// purpose  :
//=======================================================================
static Standard_Integer OCC24925(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
...
static Standard_Integer OCC25043(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    if (theArgNb != 2) {
        ...
        for (; anCheckIter.More(); anCheckIter.Next()) {
            ...
            for (; aFaultyIter.More(); aFaultyIter.Next()) {
                ...
                for (; anExp.More() && !anIsFaultyShapeFound; anExp.Next()) {
                    if (anExp.Current().IsEqual(aFaultyShape)) anIsFaultyShapeFound = Standard_True;
                }
...
// purpose  :
//=======================================================================
static Standard_Integer OCC24606(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    if (theArgNb > 1) {
...
static Standard_Integer OCC25202(Draw_Interpretor& theDI, Standard_Integer theArgN, const char** theArgVal) {
    //  0      1    2     3     4     5     6
...
// purpose  :
//=======================================================================
static Standard_Integer OCC25340(Draw_Interpretor& /*theDI*/, Standard_Integer /*theArgNb*/,
...
public:
    //! Constructor
    ParallelTest_Saxpy(const NCollection_Array1<Standard_Real>& theX, NCollection_Array1<Standard_Real>& theY,
...
public:
    ...
    ParallelTest_SaxpyBatch(const NCollection_Array1<Standard_Real>& theX, NCollection_Array1<Standard_Real>& theY,
                            Standard_Real theScalar)
        : ParallelTest_Saxpy(theX, theY, theScalar), myNbBatches((int)Ceiling((double)theX.Size() / THE_BATCH_SIZE)) {}
...
//---------------------------------------------------------------------
static Standard_Integer OCC24826(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc != 2) {
...
public:
    ParallelTest_MatMult(const NCollection_Array2<double>& theMat1, const NCollection_Array2<double>& theMat2,
...
// purpose  :
//=======================================================================
static Standard_Integer OCC25348(Draw_Interpretor& theDI, Standard_Integer /*theArgNb*/, const char** /*theArgVec*/) {
...
static Standard_Integer OCC25446(Draw_Interpretor& theDI, Standard_Integer argc, const char** argv) {
    ...
    for (; aIt.More(); aIt.Next()) {
        ...
        aType = aS.ShapeType();
        if (!(aType == TopAbs_EDGE || aType == TopAbs_FACE || aType == TopAbs_VERTEX || aType == TopAbs_SOLID)) {
            continue;
        }
        //
...
struct OCC25545_Functor {
    OCC25545_Functor(const std::vector<TopoDS_Shape>& theShapeVec) : myShapeVec(&theShapeVec), myIsRaceDetected(0) {}
...
static Standard_Integer OCC25547(Draw_Interpretor& theDI, Standard_Integer /*argc*/, const char** /*argv*/) {
    ...
    BRepMesh_GeomTool::IntFlag aIntFlag = BRepMesh_GeomTool::IntLinLin(aRefPnts[0], aRefPnts[1], aRefPnts[2],
                                                                       aRefPnts[3], aIntPnt.ChangeCoord(), aParams);
    ...
    aIntFlag = BRepMesh_GeomTool::IntSegSeg(aRefPnts[0], aRefPnts[1], aRefPnts[2], aRefPnts[3], Standard_False,
                                            Standard_False, aIntPnt);
        ...
        for (Standard_Integer aCompGridY = 0; aCompGridY < aCompGridSize; ++aCompGridY) {
            ...
            for (Standard_Integer aBoxGridX = 0; aBoxGridX < aBoxGridSize; ++aBoxGridX) {
                for (Standard_Integer aBoxGridY = 0; aBoxGridY < aBoxGridSize; ++aBoxGridY) {
                    BRepPrimAPI_MakeBox aBox(gp_Pnt(aBoxGridX * aBoxSize, aBoxGridY * aBoxSize, 0.0), aBoxSize,
                                             aBoxSize, aBoxSize);
...
static ShapeExtend_Status getStatusGap(const Handle(ShapeFix_Wire) & theFix, const Standard_Boolean theIs3d) {
...
static Standard_Boolean inspect_point(const gp_XY& thePoint, const gp_XY& theCenter, const Standard_Real theRadius) {
...
static Standard_Integer OCC24923(Draw_Interpretor& theDI, Standard_Integer argc, const char** argv) {
    ...
    const Standard_Real aDeviation = 1. - (Standard_Real)(aPointsNb - aFailedNb) / (Standard_Real)aPointsNb;
    ...
    theDI << "Deviation of incorrect results is: " << aDeviation * 100 << "% (Max " << aMaxDeviation * 100 << "%)\n";
...
static Standard_Integer OCC25574(Draw_Interpretor& theDI, Standard_Integer /*argc*/, const char** /*argv*/) {
    ...
    gp_Mat aI;
    aI.SetIdentity();
    const char* names[] = {"Extrinsic_XYZ", "Extrinsic_XZY", "Extrinsic_YZX", "Extrinsic_YXZ", "Extrinsic_ZXY",
                           "Extrinsic_ZYX", "Intrinsic_XYZ", "Intrinsic_XZY", "Intrinsic_YZX", "Intrinsic_YXZ",
                           "Intrinsic_ZXY", "Intrinsic_ZYX", "Extrinsic_XYX", "Extrinsic_XZX", "Extrinsic_YZY",
                           "Extrinsic_YXY", "Extrinsic_ZYZ", "Extrinsic_ZXZ", "Intrinsic_XYX", "Intrinsic_XZX",
    ...
    gp_EulerSequence pairs[][2] = {{gp_Extrinsic_XYZ, gp_Intrinsic_ZYX}, {gp_Extrinsic_XZY, gp_Intrinsic_YZX},
                                   {gp_Extrinsic_YZX, gp_Intrinsic_XZY}, {gp_Extrinsic_YXZ, gp_Intrinsic_ZXY},
                                   {gp_Extrinsic_ZXY, gp_Intrinsic_YXZ}, {gp_Extrinsic_ZYX, gp_Intrinsic_XYZ}};
    for (int i = 0; i < 6; i++) {
    ...
    for (int i = gp_Extrinsic_XYZ; i <= gp_Intrinsic_ZYZ; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if ((v - v2).SquareModulus() > Precision::SquareConfusion()) {
                ...
                for (int k = 1; k <= 3; k++)
                    if (Abs(v2.Coord(k)) < Precision::Confusion()) v2.SetCoord(k, 0.);
                ...
                theDI << "rotating by angle 90 deg around "
                      << (anAxis == 0   ? "X"
                          : anAxis == 1 ? "Y"
                                        : "Z")
                      << " converts vector (" << v.X() << ", " << v.Y() << ", " << v.Z() << ") to (" << v2.X() << ", "
                      << v2.Y() << ", " << v2.Z() << ")\n";
            }
        }
    }
...
Standard_Integer OCC26446(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    Handle(Geom_BSplineCurve) aCurve1 = Handle(Geom_BSplineCurve)::DownCast(DrawTrSurf::GetCurve(a[2]));
    Handle(Geom_BSplineCurve) aCurve2 = Handle(Geom_BSplineCurve)::DownCast(DrawTrSurf::GetCurve(a[3]));
    ...
    for (; aExplorer.More(); aExplorer.Next()) {
        ...
        for (int i = 0; i < aVertexNb; i++) {
            ...
            if (aNormal.X() == 0 && aNormal.Y() == 0 && aNormal.Z() == 1) {
                ...
                theDI << "Failed. Point " << buf << ": " << aPoint.X() << " " << aPoint.Y() << " " << aPoint.Z()
                      << "\n";
...
template <int size> inline const unsigned char* SizeRef();

template <> inline const unsigned char* SizeRef<8>() {
...
template <> inline const unsigned char* SizeRef<4>() {
    static const unsigned char aSizeRef[] = {0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00,
...
static Standard_Integer OCC24537(Draw_Interpretor& theDI, Standard_Integer argc, const char** argv) {
    ...
    // 3. InverseShortReal
    const unsigned char aShortRealRef[] = {0x3F, 0x80, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x40, 0x40,
                                           0x00, 0x00, 0x40, 0x80, 0x00, 0x00, 0x40, 0xA0, 0x00, 0x00,
                                           0x40, 0xC0, 0x00, 0x00, 0x40, 0xE0, 0x00, 0x00, 0x41, 0x00,
                                           0x00, 0x00, 0x41, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    const Standard_ShortReal aShortRealArr[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 0.0f};
    if (aF.is_open()) {
        for (int i = 0; i < 10; ++i) {
    ...
    const unsigned char aRndShortRealRef[] = {0xB0, 0x89, 0x70, 0x5F, 0x00, 0x00, 0x00, 0x00, 0x30, 0x89,
                                              0x70, 0x5F, 0x3F, 0x80, 0x00, 0x00, 0x40, 0x49, 0x0E, 0x56,
                                              0xC0, 0xD6, 0x66, 0x66, 0x40, 0xD6, 0x66, 0x66, 0x42, 0xC5,
                                              0xCC, 0xCD, 0xC2, 0xC7, 0xCC, 0xCD, 0x42, 0xC7, 0xCC, 0xCD};
    const Standard_ShortReal aRndShortRealArr[] = {-1.e-9f, 0.f,  1.e-9f, 1.f,    3.1415f,
                                                   -6.7f,   6.7f, 98.9f,  -99.9f, 99.9f};
    if (aF.is_open()) {
        for (int i = 0; i < 10; ++i) {
    ...
    if (!aF.is_open() && !isErr) theDI << "Conversion was done OK";
    if (aF.is_open()) {
...
#include <vector>
static TopoDS_Shape taper(const TopoDS_Shape& shape, const TopoDS_Face& face_a, const TopoDS_Face& face_b,
                          Standard_Real angle) {
...
// purpose  :
//=======================================================================
static Standard_Integer OCC26750(Draw_Interpretor& theDI, Standard_Integer /*theNArg*/, const char** /*theArgVal*/) {
...
static Standard_Integer OCC26746(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgVal) {
    if (theNArg < 2) {
...
public:
    ...
    virtual void Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePresentation,
...
static Standard_Integer OCC27700(Draw_Interpretor& /*theDI*/, Standard_Integer /*theArgNb*/,
                                 const char** /*theArgVec*/) {
...
void QABugs::Commands_19(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("OCC24202_1", "Test Handle-based procedure", __FILE__, aClassPtr, &QABugs_HandleClass::HandleProc,
                    group);
    NCollection_Handle<QABugs_NHandleClass> aNClassPtr = new QABugs_NHandleClass();
    theCommands.Add("OCC24202_2", "Test NCollection_Handle-based procedure", __FILE__, aNClassPtr,
                    &QABugs_NHandleClass::NHandleProc, group);
    ...
    theCommands.Add(
        "OCC23945",
        "OCC23945 surfname U V X Y Z [DUX DUY DUZ DVX DVY DVZ [D2UX D2UY D2UZ D2VX D2VY D2VZ D2UVX D2UVY D2UVZ]]",
        __FILE__, OCC23945, group);
    ...
    theCommands.Add("OCC24667", "OCC24667 result Wire_spine Profile [Mode [Approx]], no args to get help", __FILE__,
                    OCC24667, group);
    ...
    theCommands.Add("OCC24826,",
                    "This test performs simple saxpy test using multiple threads.\n Usage: OCC24826 length", __FILE__,
                    OCC24826, group);
    theCommands.Add("OCC29935,",
                    "This test performs product of two square matrices using multiple threads.\n Usage: OCC29935 size",
                    __FILE__, OCC29935, group);
    theCommands.Add("OCC24606", "OCC24606 : Tests ::FitAll for V3d view ('vfit' is for NIS view)", __FILE__, OCC24606,
                    group);
    ...
    theCommands.Add("OCC26139", "OCC26139 [-boxsize value] [-boxgrid value] [-compgrid value]", __FILE__, OCC26139,
                    group);
    ...
    theCommands.Add(
        "OCC26462",
        "OCC26462: Checks the ability to manage sensitivity of a particular selection mode in local context", __FILE__,
        OCC26462, group);
    ...
    theCommands.Add("OCC27048", "OCC27048 surf U V N\nCalculate value of surface N times in the point (U, V)", __FILE__,
                    OCC27048, group);

    theCommands.Add("OCC27318", "OCC27318: Creates a box that is not listed in map of AIS objects of ViewerTest",
    ...
    theCommands.Add("OCC27700", "OCC27700: Checks drawing text after setting interior style", __FILE__, OCC27700,
                    group);
    theCommands.Add("OCC27757", "OCC27757: Creates a box that has a sphere as child object and displays it", __FILE__,
                    OCC27757, group);
    theCommands.Add("OCC27818", "OCC27818: Creates three boxes and highlights one of them with own style", __FILE__,
                    OCC27818, group);
    theCommands.Add("OCC27893", "OCC27893: Creates a box and selects it via AIS_InteractiveContext API", __FILE__,
                    OCC27893, group);
    ...
    theCommands.Add("OCC29412", "OCC29412 [nb cycles]: test display / remove of many small objects", __FILE__, OCC29412,
                    group);
    return;
}
