...
static Standard_Integer BUC60889(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc != 10) {
        di << "Usage : " << argv[0]
           << " point_1 point_2 name_of_edge bndbox_X1 bndbox_Y1 bndbox_Z1 bndbox_X2 bndbox_Y2 bndbox_Z2\n";
        return 1;
    } else {
        gp_Pnt p1, p2;
        ...
        Bnd_Box bnd_box;
        bnd_box.Update(Draw::Atof(argv[4]), Draw::Atof(argv[5]), Draw::Atof(argv[6]), Draw::Atof(argv[7]),
                       Draw::Atof(argv[8]), Draw::Atof(argv[9]));
...
static Standard_Integer BUC60852(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc != 8)
        ...
        if (shape.ShapeType() != TopAbs_EDGE)
            ...
            Bnd_Box bnd_box;
            bnd_box.Update(Draw::Atof(argv[2]), Draw::Atof(argv[3]), Draw::Atof(argv[4]), Draw::Atof(argv[5]),
                           Draw::Atof(argv[6]), Draw::Atof(argv[7]));
...
Standard_Boolean BuildWiresWithReshape(const Handle(ShapeBuild_ReShape) & theReshape,
                                       const TopTools_ListOfShape& theListOfEdges, TopTools_ListOfShape& theListOfWires,
                                       const Standard_Boolean isFixConnectedMode,
                                       const Standard_Boolean isKeepLoopsMode, const Standard_Real theTolerance) {
    ...
    if (!isDone) return Standard_False;
    ...
    for (i = 2; i <= aNbEdges; i++) {
        ...
        if (aVf.IsSame(aVlast)) {
            ...
            if (aVf.IsSame(aVl)) aCurWire.Closed(Standard_True);
...
Standard_Boolean BuildWires(const TopTools_ListOfShape& theListOfEdges, TopTools_ListOfShape& theListOfWires,
    ...
    return BuildWiresWithReshape(aReshape, theListOfEdges, theListOfWires, isFixConnectedMode, isKeepLoopsMode,
                                 theTolerance);
}
...
Standard_Boolean BuildBoundWires(const TopoDS_Shape& theShell, TopTools_ListOfShape& theListOfWires) {
    ...
    if (!isBound) return Standard_True;
...
static Standard_Integer OCC6794(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc > 1) aNb = Draw::Atoi(argv[1]);
...
static Standard_Integer OCC16485(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    // std::cout << "Resulting dimensions: Xmin = " << xmin << " , Xmax = " << xmax << " , Tolerance = " << tol <<
    // std::endl;
...
void QABugs::Commands_14(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("BUC60889",
                    "BUC60889 point_1 point_2 name_of_edge bndbox_X1 bndbox_Y1 bndbox_Z1 bndbox_X2 bndbox_Y2 bndbox_Z2",
                    __FILE__, BUC60889, group);
    theCommands.Add("BUC60852", "BUC60852 name_of_edge bndbox_X1 bndbox_Y1 bndbox_Z1 bndbox_X2 bndbox_Y2 bndbox_Z2",
                    __FILE__, BUC60852, group);
    theCommands.Add(
        "BUC60854",
        "BUC60854 result_shape name_of_shape name_of_face name_of_wire/name_of_edge [ name_of_wire/name_of_edge ... ] "
        "[ name_of_face name_of_wire/name_of_edge [ name_of_wire/name_of_edge ... ] ... ] [ @ edge_on_shape "
        "edge_on_wire [ edge_on_shape edge_on_wire ... ] ] [ # L/R ]",
        __FILE__, BUC60854, group);
    ...
    theCommands.Add("OCC2932_SetIDUAttribute", "OCC2932_SetIDUAttribute (DF, entry, oldLocalID, newLocalID)", __FILE__,
                    OCC2932_SetIDUAttribute, group);
    ...
    theCommands.Add("OCC2932_SetExpression", "OCC2932_SetExpression (DF, entry, Expression)", __FILE__,
                    OCC2932_SetExpression, group);

    theCommands.Add("OCC2932_SetRelation", "OCC2932_SetRelation (DF, entry, Relation)", __FILE__, OCC2932_SetRelation,
                    group);
