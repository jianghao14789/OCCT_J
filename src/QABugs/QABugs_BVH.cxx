...
public:
    ...
    virtual Standard_Boolean RejectNode(const BVH_Vec3d& theCornerMin, const BVH_Vec3d& theCornerMax,
...
//=======================================================================
static Standard_Integer QABVH_ShapeSelect(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc < 4) {
    ...
    if (theArgc > 4) useVoidSelector = !strcmp(theArgv[4], "-void");
...
// purpose : Implement the simplest shape's selector
//=======================================================================
class PairShapesSelector : public BVH_PairTraverse<Standard_Real, 3, BVH_BoxSet<Standard_Real, 3, TopoDS_Shape>> {
public:
    //! Constructor
    PairShapesSelector() {}

    //! Returns the selected pairs of shapes
    const NCollection_List<std::pair<TopoDS_Shape, TopoDS_Shape>>& Pairs() const {
        return myPairs;
    }

public:
    //! Defines the rules for node rejection
    virtual Standard_Boolean RejectNode(const BVH_Vec3d& theCornerMin1, const BVH_Vec3d& theCornerMax1,
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theIndex1,
        ...
        if (!aBox1.IsOut(aBox2)) {
            myPairs.Append(
                std::pair<TopoDS_Shape, TopoDS_Shape>(myBVHSet1->Element(theIndex1), myBVHSet2->Element(theIndex2)));
            return Standard_True;
        }
        return Standard_False;
    }
...
public:
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theIndex1,
        ...
        if (!aBox1.IsOut(aBox2)) {
            myPairs.Append(
                std::pair<TopoDS_Shape, TopoDS_Shape>(mySBSet1->Element(theIndex1), mySBSet2->Element(theIndex2)));
            return Standard_True;
        }
        return Standard_False;
    }
...
//=======================================================================
static Standard_Integer QABVH_PairSelect(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc < 4) {
        theDI.PrintHelp(theArgv[0]);
        return 1;
    }
...
//=======================================================================
static Standard_Real TriangleTriangleSqDistance(const BVH_Vec3d& theNode11, const BVH_Vec3d& theNode12,
                                                const BVH_Vec3d& theNode13, const BVH_Vec3d& theNode21,
                                                const BVH_Vec3d& theNode22, const BVH_Vec3d& theNode23) {
    ...
    for (Standard_Integer iT = 0; iT < 2; ++iT) {
        ...
        for (Standard_Integer iP = 0; iP < 3; ++iP) {
            aDist = BVH_Tools<Standard_Real, 3>::PointTriangleSquareDistance(
                aNodes[iT][iP], aNodes[(iT + 1) % 2][0], aNodes[(iT + 1) % 2][1], aNodes[(iT + 1) % 2][2]);
            if (aDist < aMinDist) {
                aMinDist = aDist;
                if (aMinDist == 0) return aMinDist;
            }
        }

        // projecting edges on the opposite triangle
        std::pair<BVH_Vec3d, BVH_Vec3d> anEdges[3] = {std::pair<BVH_Vec3d, BVH_Vec3d>(aNodes[iT][0], aNodes[iT][1]),
        ...
        for (Standard_Integer iE = 0; iE < 3; ++iE) {
            ...
            if (aLength < Precision::Confusion()) continue;
            anEdge /= aLength;
            ...
            for (Standard_Integer iP = 1; iP < aNbSeg; ++iP) {
                ...
                aDist = BVH_Tools<Standard_Real, 3>::PointTriangleSquareDistance(
                    aPE, aNodes[(iT + 1) % 2][0], aNodes[(iT + 1) % 2][1], aNodes[(iT + 1) % 2][2]);
                if (aDist < aMinDist) {
                    aMinDist = aDist;
                    if (aMinDist == 0) return aMinDist;
                }
...
public:
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theIndex1,
        ...
        Standard_Real aDistance = TriangleTriangleSqDistance(aTri1._Node1, aTri1._Node2, aTri1._Node3, aTri2._Node1,
                                                             aTri2._Node2, aTri2._Node3);
...
//=======================================================================
static Standard_Integer QABVH_PairDistance(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc != 3) {
        theDI.PrintHelp(theArgv[0]);
        return 1;
    }
...
public:
    QABVH_TriangleSet() : BVH_Triangulation<Standard_Real, 3>() {}
...
public:
    QABVH_Geometry() : BVH_Geometry<Standard_Real, 3>() {}
...
//=======================================================================
static Standard_Integer QABVH_DistanceField(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc < 2) {
        theDI.PrintHelp(theArgv[0]);
        return 1;
    }
...
void QABugs::Commands_BVH(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("QABVH_ShapeSelect",
                    "Tests the work of BHV_BoxSet algorithm on the simple example of selection of shapes which boxes "
                    "interfere with given box.\n"
    ...
    theCommands.Add("QABVH_PairSelect",
                    "Tests the work of BHV_BoxSet algorithm on the simple example of selection of pairs of shapes with "
                    "interfering bounding boxes.\n"
