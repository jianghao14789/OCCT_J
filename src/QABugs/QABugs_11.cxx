...
static int pipe_OCC9(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    GeomFill_Pipe aPipe(Handle(Geom_Curve)::DownCast(aCurveSeq(1)), Handle(Geom_Curve)::DownCast(aCurveSeq(2)),
                        Handle(Geom_Curve)::DownCast(aCurveSeq(3)), Draw::Atof(a[5]));
...
#define _OFFSET_TELCO_
    ...
    Standard_Real xA = 0.0, xB = 200.0, xC = 200.0, xD = 0.0, yA = 0.0, yB = 0.0, yC = 200.0, yD = 200.0, zA = 0.0,
                  zB = 0.0, zC = 0.0, zD = 0.0;
...
Standard_Integer OCC299bug(Draw_Interpretor& theDi, Standard_Integer theArgNb, const char** theArgVec) {
    if (theArgNb < 3) {
...
static Standard_Integer OCC22(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    try {
        ...
        if (argc < 5) {
            di << "OCC22 FAULTY. Use : OCC22 Result Shape CompoundOfSubshapesToBeDivided ConsiderLocation";
            return 0;
        }
        ...
        if (aSubShapesToBeDivided.IsNull()) {
            di << "OCC22 FAULTY. Compound of subshapes to be divided is not exist. Please, verify input values. \n";
            return 0;
        }
...
static Standard_Integer OCC24(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    try {
        ...
        if (argc < 6) {
            di << "OCC24 FAULTY. Use : OCC22 Result Shape CompoundOfSubshapes ResourceFileName SequenceName";
            return 0;
        }
...
static Standard_Integer OCC524(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc != 9) {
        di << "Usage : " << argv[0]
           << " LowerVector UpperVector InitialValueVector LowerRowMatrix UpperRowMatrix LowerColMatrix UpperColMatrix "
              "InitialValueMatrix\n";
        return 1;
    }
...
static Standard_Integer OCC1029_AISTransparency(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb >= 3) {
        Handle(TDocStd_Document) D;
...
static Standard_Integer OCC1031_AISMaterial(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb >= 3) {
        Handle(TDocStd_Document) D;
        if (!DDocStd::GetDocument(arg[1], D)) return 1;
        TDF_Label L;
...
static Standard_Integer OCC1032_AISWidth(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb >= 3) {
        Handle(TDocStd_Document) D;
        if (!DDocStd::GetDocument(arg[1], D)) return 1;
        TDF_Label L;
...
static Standard_Integer OCC1034_AISSelectionMode(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb >= 3) {
        Handle(TDocStd_Document) D;
        if (!DDocStd::GetDocument(arg[1], D)) return 1;
        TDF_Label L;
...
static Standard_Integer OCC1487(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (CaseNumber == 1) {
        // BRepPrimAPI_MakeCylinder o_mc2 (gp_Ax2 (gp_Pnt(21.65064, -50.0, 127.5),gp_Dir(-sin(M_PI/3), 0.0, 0.5)), 5,
        // 150);
        ...
        // BRepPrimAPI_MakeCylinder o_mc2 (gp_Ax2 (gp_Pnt(978.34936, -50.0, 127.5),gp_Dir(sin(M_PI/3), 0.0, 0.5)), 5,
        // 150);
        gp_Dir myDir_mc2(sin(M_PI / 3), 0.0, 0.5);
...
TopoDS_Shape OCC1077_cut_blend(const TopoDS_Shape& aShapeToCut, const TopoDS_Shape& aTool,
...
// TopoDS_Shape OCC1077_common_blend(const TopoDS_Shape& aShape1, const TopoDS_Shape& aShape2, const Standard_Real
// aRadius)
//{
...
TopoDS_Shape OCC1077_Bug() {
    ...
    TopoDS_Shape theCylinder1 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, 0, -10), gp_Dir(0, 0, 1)), 3, 20).Shape();
    TopoDS_Shape theCylinder2 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(-10, 0, 0), gp_Dir(1, 0, 0)), 3, 20).Shape();
    TopoDS_Shape theCylinder3 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, -10, 0), gp_Dir(0, 1, 0)), 3, 20).Shape();
...
struct TestParallelFunctor {
    ...
    Standard_Integer NbNotRaised() const {
        return myNbNotRaised;
    }
    Standard_Integer NbSigSegv() const {
        return myNbSigSegv;
    }
    Standard_Integer NbUnknown() const {
        return myNbUnknown;
    }
...
static Standard_NOINLINE Standard_Integer OCC30762(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char**) {
    if (theNbArgs != 1) {
...
static Standard_Integer OCC10138(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (label1.FindAttribute(TDataStd_RealArray::GetID(), array) && array->Lower() == LOWER && array->Upper() == UPPER)
...
static Standard_Integer OCC31965(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    ViewerTest::GetAISContext()->HilightWithColor(
        aPrs, ViewerTest::GetAISContext()->HighlightStyle(Prs3d_TypeOfHighlight_Dynamic), true);
    return 0;
}
...
int TestSetGet(const Handle(TDocStd_Document) & doc) {
    ...
    if (!doc->Main().FindAttribute(TDataStd_Tick::GetID(), tick)) return 1;
    // Forget
    doc->Main().ForgetAttribute(TDataStd_Tick::GetID());
    if (doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 2;
    doc->Main().ResumeAttribute(tick);
    if (!doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 3;
    // Forget
    doc->Main().ForgetAttribute(TDataStd_Tick::GetID());
    if (doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 2;
    ...
    if (!doc->Main().FindAttribute(TDataStd_IntegerList::GetID(), getintlist)) return 1;
    if (getintlist->First() != 1) return 2;
    if (getintlist->Last() != 3) return 3;
    ...
    if (!doc->Main().FindAttribute(TDataStd_RealList::GetID(), getdbllist)) return 1;
    if (getdbllist->First() != 1.5) return 2;
    if (getdbllist->Last() != 3.5) return 3;
    ...
    if (!doc->Main().FindAttribute(TDataStd_ExtStringList::GetID(), getstrlist)) return 1;
    if (getstrlist->First() != "Guten Tag") return 2;
    if (getstrlist->Last() != "Hello") return 3;
    ...
    for (; itr_strlist.More(); itr_strlist.Next()) {
        if (itr_strlist.Value() != "Guten Tag" && itr_strlist.Value() != "Bonjour" && itr_strlist.Value() != "Hello") {
            return 4;
        }
    }
    getstrlist->Clear();
    ...
    if (!doc->Main().FindAttribute(TDataStd_BooleanList::GetID(), getboollist)) return 1;
    if (getboollist->First() != Standard_False) return 2;
    if (getboollist->Last() != Standard_True) return 3;
    ...
    if (!doc->Main().FindAttribute(TDataStd_ReferenceList::GetID(), getreflist)) return 1;
    if (getreflist->First() != L2) return 2;
    if (getreflist->Last() != L1) return 3;
    ...
    if (!doc->Main().FindAttribute(TDataStd_BooleanArray::GetID(), getboolarr)) return 1;
    if (getboolarr->Value(12) != Standard_True) return 2;
    if (getboolarr->Value(13) != Standard_False) return 2;
    if (getboolarr->Value(14) != Standard_True) return 2;
    if (getboolarr->Value(15) != Standard_False) return 2;
    if (getboolarr->Value(16) != Standard_True) return 2;
    ...
    if (!doc->Main().FindAttribute(TDataStd_ReferenceArray::GetID(), getrefarr)) return 1;
    if (getrefarr->Value(0) != L1) return 2;
    if (getrefarr->Value(1) != L2) return 2;
    if (getrefarr->Value(2) != L3) return 2;
    if (getrefarr->Value(3) != L4) return 2;
    if (getrefarr->Value(4) != L5) return 2;
    ...
    if (!doc->Main().FindAttribute(TDataStd_ByteArray::GetID(), getbytearr)) return 1;
    if (getbytearr->Value(12) != 0) return 2;
    if (getbytearr->Value(13) != 1) return 2;
    if (getbytearr->Value(14) != 2) return 2;
    if (getbytearr->Value(15) != 3) return 2;
    if (getbytearr->Value(16) != 255) return 2;
    ...
    if (!doc->Main().FindAttribute(TDataStd_NamedData::GetID(), getnd)) return 1;
    if (!getnd->HasIntegers()) return 2;
    if (!getnd->HasInteger("Integer1")) return 3;
    if (getnd->GetInteger("Integer2") != 2) return 4;
    if (getnd->GetInteger("Integer3") != 3) return 4;
...
int TestUndoRedo(const Handle(TDocStd_Document) & doc) {
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 1;
    doc->Undo();
    if (doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 2;
    doc->Redo();
    if (!doc->Main().IsAttribute(TDataStd_Tick::GetID())) return 3;
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_IntegerList::GetID())) return 1;
    doc->Undo();
    if (!intlist->IsEmpty()) return 2;
    doc->Redo();
    if (!intlist->Extent()) return 3;
    if (intlist->First() != 0) return 4;
    if (intlist->Last() != 3) return 5;
    intlist->Clear();
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_RealList::GetID())) return 1;
    doc->Undo();
    if (!dbllist->IsEmpty()) return 2;
    doc->Redo();
    if (!dbllist->Extent()) return 3;
    if (dbllist->First() != 0.5) return 4;
    if (dbllist->Last() != 3.5) return 5;
    dbllist->Clear();
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_ExtStringList::GetID())) return 1;
    doc->Undo();
    if (!strlist->IsEmpty()) return 2;
    doc->Redo();
    if (!strlist->Extent()) return 3;
    if (strlist->First() != "Guten Tag") return 4;
    if (strlist->Last() != "Hello") return 5;
    strlist->Clear();
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_BooleanList::GetID())) return 1;
    doc->Undo();
    if (!boollist->IsEmpty()) return 2;
    doc->Redo();
    if (!boollist->Extent()) return 3;
    if (boollist->First() != Standard_False) return 4;
    if (boollist->Last() != Standard_True) return 5;
    boollist->Clear();
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_ReferenceList::GetID())) return 1;
    doc->Undo();
    if (!reflist->IsEmpty()) return 2;
    doc->Redo();
    if (!reflist->Extent()) return 3;
    if (reflist->First() != L2) return 4;
    if (reflist->Last() != L1) return 5;
    reflist->Clear();
    ...
    doc->CommitCommand();
    doc->Undo();
    if (boolarr->Value(23) != Standard_True) return 2;
    if (boolarr->Value(24) != Standard_False) return 2;
    if (boolarr->Value(25) != Standard_True) return 2;
    doc->Redo();
    if (boolarr->Value(230) != Standard_True) return 3;
    if (boolarr->Value(240) != Standard_False) return 3;
    if (boolarr->Value(250) != Standard_True) return 3;
    ...
    doc->CommitCommand();
    if (!doc->Main().IsAttribute(TDataStd_ReferenceArray::GetID())) return 1;
    doc->Undo();
    doc->Redo();
    if (refarr->Value(5) != L1) return 4;
    if (refarr->Value(6) != L2) return 4;
    if (refarr->Value(7) != L3) return 4;
    if (refarr->Value(8) != L4) return 4;
    ...
    doc->CommitCommand();
    doc->Undo();
    if (bytearr->Value(23) != 23) return 2;
    if (bytearr->Value(25) != 25) return 2;
    doc->Redo();
    if (bytearr->Value(230) != 230) return 3;
    if (bytearr->Value(250) != 250) return 3;
    ...
    doc->CommitCommand();
    doc->Undo();
    if (nd->HasStrings()) return 1;
    if (nd->HasReals()) return 1;
    if (nd->HasReal("r17")) return 2;
    if (!nd->HasBytes()) return 3;
    if (nd->GetByte("b14") != 14) return 4;
    if (nd->GetByte("b17") != 17) return 4;
    if (nd->HasByte("b18")) return 5;
    doc->Redo();
    if (!nd->HasBytes()) return 1;
    if (!nd->HasReals()) return 1;
    if (nd->GetByte("b14") != 14) return 2;
    if (nd->GetReal("r14") != 14.4) return 2;
    if (nd->GetReal("r17") != 17.7) return 2;
...
int TestCopyPaste(const Handle(TDocStd_Document) & doc) {
    ...
    if (!L2.IsAttribute(TDataStd_Tick::GetID())) return 2;
    ...
    intlist->Clear();
    intlist.Nullify();
    if (!L2.FindAttribute(TDataStd_IntegerList::GetID(), intlist)) return 2;
    if (intlist->First() != 1) return 3;
    if (intlist->Last() != 2) return 4;
    intlist->Clear();
    ...
    dbllist->Clear();
    dbllist.Nullify();
    if (!L2.FindAttribute(TDataStd_RealList::GetID(), dbllist)) return 2;
    if (dbllist->First() != 1.5) return 3;
    if (dbllist->Last() != 2.5) return 4;
    dbllist->Clear();
    ...
    strlist->Clear();
    strlist.Nullify();
    if (!L2.FindAttribute(TDataStd_ExtStringList::GetID(), strlist)) return 2;
    if (strlist->First() != "Open CASCADE") return 3;
    if (strlist->Last() != " - is the best set of libraries!") return 4;
    strlist->Clear();
    ...
    boollist->Clear();
    boollist.Nullify();
    if (!L2.FindAttribute(TDataStd_BooleanList::GetID(), boollist)) return 2;
    if (boollist->First() != Standard_False) return 3;
    if (boollist->Last() != Standard_True) return 4;
    boollist->Clear();
    ...
    reflist->Clear();
    reflist.Nullify();
    if (!L2.FindAttribute(TDataStd_ReferenceList::GetID(), reflist)) return 2;
    if (reflist->First() != L100) return 3;
    if (reflist->Last() != L101) return 4;
    reflist->Clear();
    ...
    if (!copier.IsDone()) return 1;
    boolarr.Nullify();
    if (!L2.FindAttribute(TDataStd_BooleanArray::GetID(), boolarr)) return 2;
    if (boolarr->Value(4) != Standard_True) return 3;
    if (boolarr->Value(5) != Standard_False) return 3;
    if (boolarr->Value(6) != Standard_True) return 3;
    ...
    if (!copier.IsDone()) return 1;
    refarr.Nullify();
    if (!L2.FindAttribute(TDataStd_ReferenceArray::GetID(), refarr)) return 2;
    if (refarr->Value(3) != L100) return 3;
    if (refarr->Value(4) != L101) return 3;
    ...
    if (!copier.IsDone()) return 1;
    bytearr.Nullify();
    if (!L2.FindAttribute(TDataStd_ByteArray::GetID(), bytearr)) return 2;
    if (bytearr->Value(4) != 40) return 3;
    if (bytearr->Value(6) != 60) return 3;
    ...
    if (!copier.IsDone()) return 1;
    nd.Nullify();
    if (!L2.FindAttribute(TDataStd_NamedData::GetID(), nd)) return 2;
    if (!nd->HasIntegers()) return 3;
    if (!nd->HasReals()) return 3;
    if (!nd->HasStrings()) return 3;
    if (!nd->HasBytes()) return 3;
    if (!nd->HasArraysOfIntegers()) return 3;
    if (nd->HasArraysOfReals()) return 3;
    if (!nd->HasInteger("Integer1")) return 4;
    if (nd->GetInteger("Integer1") != 11) return 4;
    if (!nd->HasReal("Real1")) return 4;
    if (nd->GetReal("Real1") != 11.1) return 4;
    if (!nd->HasString("String1")) return 4;
    if (nd->GetString("String1") != "11.11111111") return 4;
    if (!nd->HasByte("Byte1")) return 4;
    if (nd->GetByte("Byte1") != 111) return 4;
    if (!nd->HasArrayOfIntegers("Integers1")) return 4;
    const Handle(TColStd_HArray1OfInteger) & ints_arr_out = nd->GetArrayOfIntegers("Integers1");
    if (ints_arr_out.IsNull()) return 4;
    if (ints_arr_out->Value(5) != 5) return 4;

    return 0;
}

int TestOpenSave(TCollection_ExtendedString aFile1, TCollection_ExtendedString aFile2,
    ...
    if (app->SaveAs(doc_std, aFile1) != PCDM_SS_OK) return 1;
    intlist.Nullify();
    dbllist.Nullify();
    strlist.Nullify();
    boollist.Nullify();
    ...
    if (app->Open(aFile1, doc_std_open) != PCDM_RS_OK) return 2;
    if (!doc_std_open->Main().IsAttribute(TDataStd_Tick::GetID())) return 3;
    if (!doc_std_open->Main().FindAttribute(TDataStd_IntegerList::GetID(), intlist)) return 4;
    if (intlist->First() != 1) return 5;
    if (intlist->Last() != 5) return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_RealList::GetID(), dbllist)) return 4;
    if (dbllist->First() != 1.5) return 5;
    if (dbllist->Last() != 5.5) return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_ExtStringList::GetID(), strlist)) return 4;
    if (strlist->First() != "Auf") return 5;
    if (strlist->Last() != "Wiedersehen") return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_BooleanList::GetID(), boollist)) return 4;
    if (boollist->First() != Standard_False) return 5;
    if (boollist->Last() != Standard_True) return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_ReferenceList::GetID(), reflist)) return 4;
    TDF_Tool::Entry(reflist->First(), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(reflist->Last(), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_BooleanArray::GetID(), boolarr)) return 4;
    if (boolarr->Value(15) != Standard_False) return 5;
    if (boolarr->Value(16) != Standard_True) return 5;
    if (boolarr->Value(17) != Standard_True) return 5;
    if (boolarr->Value(18) != Standard_True) return 5;
    if (!doc_std_open->Main().FindAttribute(TDataStd_ReferenceArray::GetID(), refarr)) return 4;
    TDF_Tool::Entry(refarr->Value(45), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(refarr->Value(46), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_std_open->Main().FindAttribute(TDataStd_ByteArray::GetID(), bytearr)) return 4;
    if (bytearr->Value(15) != 150) return 5;
    if (bytearr->Value(16) != 160) return 5;
    if (bytearr->Value(17) != 170) return 5;
    if (bytearr->Value(18) != 180) return 5;
    if (!doc_std_open->Main().FindAttribute(TDF_Reference::GetID(), ref)) return 4;
    if (ref->Get().IsNull()) return 5;
    if (ref->Get().Tag() != 103) return 5;
    ...
    if (app->SaveAs(doc_xml, aFile2) != PCDM_SS_OK) return 1;
    intlist.Nullify();
    ref.Nullify();
    app->Close(doc_xml);
    doc_xml.Nullify();
    // if (app->Open("W:\\doc.xml", doc_xml_open) != PCDM_RS_OK)
    if (app->Open(aFile2, doc_xml_open) != PCDM_RS_OK) return 2;
    if (!doc_xml_open->Main().IsAttribute(TDataStd_Tick::GetID())) return 3;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_IntegerList::GetID(), intlist)) return 4;
    if (intlist->First() != 1) return 5;
    if (intlist->Last() != 5) return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_RealList::GetID(), dbllist)) return 4;
    if (dbllist->First() != 1.5) return 5;
    if (dbllist->Last() != 5.5) return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_ExtStringList::GetID(), strlist)) return 4;
    if (strlist->First() != "Guten ") return 5;
    if (strlist->Last() != "Tag") return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_BooleanList::GetID(), boollist)) return 4;
    if (boollist->First() != Standard_False) return 5;
    if (boollist->Last() != Standard_True) return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_ReferenceList::GetID(), reflist)) return 4;
    TDF_Tool::Entry(reflist->First(), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(reflist->Last(), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_BooleanArray::GetID(), boolarr)) return 4;
    if (boolarr->Value(15) != Standard_False) return 5;
    if (boolarr->Value(16) != Standard_True) return 5;
    if (boolarr->Value(17) != Standard_True) return 5;
    if (boolarr->Value(18) != Standard_True) return 5;
    if (boolarr->Value(19) != Standard_True) return 5;
    if (boolarr->Value(20) != Standard_True) return 5;
    if (boolarr->Value(21) != Standard_False) return 5;
    if (boolarr->Value(22) != Standard_True) return 5;
    if (boolarr->Value(23) != Standard_True) return 5;
    if (boolarr->Value(24) != Standard_True) return 5;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_ReferenceArray::GetID(), refarr)) return 4;
    TDF_Tool::Entry(refarr->Value(444), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(refarr->Value(445), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_xml_open->Main().FindAttribute(TDataStd_ByteArray::GetID(), bytearr)) return 4;
    if (bytearr->Value(15) != 0) return 5;
    if (bytearr->Value(16) != 10) return 5;
    if (bytearr->Value(17) != 100) return 5;
    if (bytearr->Value(18) != 200) return 5;
    if (bytearr->Value(19) != 250) return 5;
    if (bytearr->Value(20) != 251) return 5;
    if (bytearr->Value(21) != 252) return 5;
    if (bytearr->Value(22) != 253) return 5;
    if (bytearr->Value(23) != 254) return 5;
    if (bytearr->Value(24) != 255) return 5;
    if (!doc_xml_open->Main().FindAttribute(TDF_Reference::GetID(), ref)) return 4;
    if (ref->Get().IsNull()) return 5;
    if (ref->Get().Tag() != 103) return 5;
    ...
    if (app->SaveAs(doc_bin, aFile3) != PCDM_SS_OK) return 1;
    intlist.Nullify();
    ref.Nullify();
    app->Close(doc_bin);
    doc_bin.Nullify();
    // if (app->Open("W:\\doc.cbf", doc_bin_open) != PCDM_RS_OK)
    if (app->Open(aFile3, doc_bin_open) != PCDM_RS_OK) return 2;
    if (!doc_bin_open->Main().IsAttribute(TDataStd_Tick::GetID())) return 3;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_IntegerList::GetID(), intlist)) return 4;
    if (intlist->First() != 1) return 5;
    if (intlist->Last() != 5) return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_RealList::GetID(), dbllist)) return 4;
    if (dbllist->First() != 1.5) return 5;
    if (dbllist->Last() != 5.5) return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_ExtStringList::GetID(), strlist)) return 4;
    if (strlist->First() != "Bonjour") return 5;
    if (strlist->Last() != "Bonsoir") return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_BooleanList::GetID(), boollist)) return 4;
    if (boollist->First() != Standard_False) return 5;
    if (boollist->Last() != Standard_True) return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_ReferenceList::GetID(), reflist)) return 4;
    TDF_Tool::Entry(reflist->First(), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(reflist->Last(), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_BooleanArray::GetID(), boolarr)) return 4;
    if (boolarr->Value(15) != Standard_False) return 5;
    if (boolarr->Value(16) != Standard_True) return 5;
    if (boolarr->Value(17) != Standard_True) return 5;
    if (boolarr->Value(18) != Standard_True) return 5;
    ...
    if (boolarr->Value(23) != Standard_True) return 5;
    if (boolarr->Value(24) != Standard_True) return 5;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_ReferenceArray::GetID(), refarr)) return 4;
    TDF_Tool::Entry(refarr->Value(0), entry_first);
    if (entry1 != entry_first) return 5;
    TDF_Tool::Entry(refarr->Value(1), entry_last);
    if (entry2 != entry_last) return 6;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_ByteArray::GetID(), bytearr)) return 4;
    if (bytearr->Value(15) != 0) return 5;
    if (bytearr->Value(16) != 255) return 5;
    if (!doc_bin_open->Main().FindAttribute(TDataStd_NamedData::GetID(), nameddata)) return 4;
    if (nameddata->GetByte("A") != 12) return 5;
    if (nameddata->GetByte("B") != 234) return 5;
    if (!doc_bin_open->Main().FindAttribute(TDF_Reference::GetID(), ref)) return 4;
    if (ref->Get().IsNull()) return 5;
    if (ref->Get().Tag() != 103) return 5;
...
static Standard_Integer OCC16782(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (app.IsNull()) app = new AppStd_Application();
...
static Standard_Integer OCC12584(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (aCS->TransformPersistence().IsNull() || aCS->TransformPersistence()->Mode() != Graphic3d_TMF_2d) {
...
Standard_Integer OCC22736(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc != 9) {
        di << "Usage : " << argv[0]
           << " X_mirrorFirstPoint Y_mirrorFirstPoint X_mirrorSecondPoint Y_mirrorSecondPoint X_p1 Y_p1 X_p2 Y_p2\n";
        return 1;
    }
    ...
    if (Abs(p2.X() - p1MirrorM1.X()) > aTol) aStatus = 2;
    if (Abs(p2.Y() - p1MirrorM1.Y()) > aTol) aStatus = 3;

    gp_Pnt2d p1MirrorM1M2 = p1MirrorM1.Transformed(M2);
    if (Abs(p1.X() - p1MirrorM1M2.X()) > aTol) aStatus = 4;
    if (Abs(p1.Y() - p1MirrorM1M2.Y()) > aTol) aStatus = 5;

    // If we apply the composed transformation of the same two mirrors to a point the result is //not located on the
    // initial position.-->>ERROR
    gp_Pnt2d p1MirrorComp = p1.Transformed(Tcomp);
    if (Abs(p1.X() - p1MirrorComp.X()) > aTol) aStatus = 6;
    if (Abs(p1.Y() - p1MirrorComp.Y()) > aTol) aStatus = 7;
...
Standard_Integer OCC23429(Draw_Interpretor& /*di*/, Standard_Integer narg, const char** a) {
    ...
    if (narg == 5) Builder.Approximation(Standard_True);
    Builder.Build();
    ...
    for (; ExpSec.More(); ExpSec.Next()) {
        ...
        for (; ExpShape.More(); ExpShape.Next()) {
            ...
            if (aSurface == theSurface && aLoc == theLoc) break;
        }
...
struct Task {
    ...
    Task(const Message_ProgressRange& thePR, int theSize)
        : Range(thePR), Mat1(1, theSize, 1, theSize, 0.12345), Mat2(1, theSize, 1, theSize, 12345),
          Mat3(1, theSize, 1, theSize) {}
};
struct Functor {
    void operator()(Task& theTask) const {
        if (theTask.Range.More()) {
            if (theTask.Mat1.RowNumber() > 1) theTask.Mat3 = theTask.Mat1 * theTask.Mat2;
        }
        theTask.Range.Close();
    }
};
...
Standard_Integer OCC25748(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    Message_ProgressScope aPS(Message_ProgressIndicator::Start(aProgress), "Parallel data processing", nIter);
    ...
    if (aMatSize > 1)
        ...
        aText += " empty tasks";
    if (isProgress) aText += " with progress";
...
void QABugs::Commands_11(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("OCC9", " result path cur1 cur2 radius [tolerance]:\t test GeomFill_Pipe", __FILE__, pipe_OCC9,
                    group);
    ...
    theCommands.Add("OCC22", "OCC22 Result Shape CompoundOfSubshapesToBeDivided ConsiderLocation", __FILE__, OCC22,
                    group);
    ...
    theCommands.Add("OCC524",
                    "OCC524 LowerVector UpperVector InitialValueVector LowerRowMatrix UpperRowMatrix LowerColMatrix "
                    "UpperColMatrix InitialValueMatrix",
                    __FILE__, OCC524, group);
    ...
    theCommands.Add("OCC1029_AISTransparency", "OCC1029_AISTransparency (DOC, entry, [real])", __FILE__,
                    OCC1029_AISTransparency, group);
    theCommands.Add("OCC1031_AISMaterial", "OCC1031_AISMaterial (DOC, entry, [material])", __FILE__,
                    OCC1031_AISMaterial, group);
    ...
    theCommands.Add("OCC1034_AISSelectionMode", "OCC1034_AISSelectionMode (DOC, entry, [selectionmode])", __FILE__,
                    OCC1034_AISSelectionMode, group);

    // theCommands.Add("OCC1487", "OCC1487 CylinderVariant(=1/2) cylinder1 cylinder2 cutshape", __FILE__, OCC1487,
    // group);
    ...
    theCommands.Add(
        "OCC22736",
        "OCC22736 X_mirrorFirstPoint Y_mirrorFirstPoint X_mirrorSecondPoint Y_mirrorSecondPoint X_p1 Y_p1 X_p2 Y_p2",
        __FILE__, OCC22736, group);
    theCommands.Add("OCC22744", "OCC22744", __FILE__, OCC22744, group);
    theCommands.Add("OCC22558", "OCC22558 x_vec y_vec z_vec x_dir y_dir z_dit x_pnt y_pnt z_pnt", __FILE__, OCC22558,
                    group);
    ...
    theCommands.Add("OCC28478", "OCC28478 [nb_outer=3 [nb_inner=2] [-inf]: test progress indicator on nested cycles",
                    __FILE__, OCC28478, group);
    theCommands.Add("OCC31189", "OCC31189: check stream buffer interface of Message_Messenger", __FILE__, OCC31189,
                    group);
    theCommands.Add("OCC25748",
                    "OCC25748 [-niter val] [-matsize val] [-progr] [-parallel]\n"
                    "\t\ttest progress indicator in parallel execution",
                    __FILE__, OCC25748, group);
