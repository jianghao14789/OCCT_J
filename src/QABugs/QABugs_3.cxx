...
static Standard_Integer BUC60792(Draw_Interpretor& di, Standard_Integer /*argc*/, const char** argv) {
    ...
    if (cirtanrad.NbSolutions()) {
        for (int i = 1; i <= cirtanrad.NbSolutions(); i++) {
            ...
            if (i == 1) aContext->SetColor(ais, Quantity_NOC_GREEN, Standard_False);
            if (i == 2) aContext->SetColor(ais, Quantity_NOC_HOTPINK, Standard_False);
...
static int OCC10006(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    loft2.Build();
    if (loft1.Shape().IsNull() || loft2.Shape().IsNull()) return 1;
...
//==========================================================================
static Standard_Integer coordload(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    if (theArgsNb < 3) {
        return 1;
    }
...
static Standard_Integer TestMem(Draw_Interpretor& /*di*/, Standard_Integer /*nb*/, const char** /*arg*/) {
...
static Standard_Integer BUC60841(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    DBRep::Set("sh1", sh1);
    gp_Trsf trsf1, trsf2;
    trsf1.SetTranslation(gp_Pnt(0.000000, 700.000000, -170.000000), gp_Pnt(0.000000, 700.000000, -95.000000));
    trsf2.SetRotation(gp_Ax1(gp_Pnt(0.000000, 700.000000, -170.000000), gp_Dir(0.000000, 0.000000, 1.000000)),
                      0.436111);
...
static int BUC60910(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    Handle(TPrsStd_AISPresentation) AISP = TPrsStd_AISPresentation::Set(L, TDataXtd_Constraint::GetID());
