...
static Standard_Integer OCC332bug(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    myLaw2->Set(SpineCurve->FirstParameter(), 1.0, SpineCurve->LastParameter(),
    ...
    //    getFaces.Next();
    //  }
    if (getFaces.More()) B.Add(TubeShell, getFaces.Current().Reversed());
    ...
    //    getFaces.Next();
    //  }
    if (getFaces.More()) B.Add(TubeShell, getFaces.Current());
...
static Standard_Integer OCC544(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    // Construct the plane for the second face to sit on.
    gp_Pln circ2Plane =
        gce_MakePln(circ_center, endPoint, endPoint.Translated(major_rad * origin.YDirection())).Value();
    ...
    myLaw2->Set(SpineCurve->FirstParameter(), (radius_r + wall_thickness) / (radius_l + wall_thickness),
                SpineCurve->LastParameter(), 1.0);
...
void QABugs::Commands_13(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("OCC332", "OCC332 [wall_thickness [dia1 [dia2 [length [major_radius]]]]]", __FILE__, OCC332bug,
                    group);
