...
void UnitsAPI::CheckLoading(const UnitsAPI_SystemUnits aSystemUnits) {
    if (currentSystem != aSystemUnits || CurrentUnits.IsNull()) {
        switch (aSystemUnits) {
            ...
            Standard_FALLTHROUGH case UnitsAPI_SI:
...
Standard_Real UnitsAPI::CurrentToLS(const Standard_Real aData, const Standard_CString aQuantity) {
    ...
    if (CurrentUnits->Find(aQuantity)) {
        ...
        std::cout << "Warning: UnitsAPI,the quantity '" << aQuantity << "' does not exist in the current units system"
                  << std::endl;
    }
#endif
...
Standard_Real UnitsAPI::CurrentToAny(const Standard_Real aData, const Standard_CString aQuantity,
...
Standard_Real UnitsAPI::CurrentFromAny(const Standard_Real aData, const Standard_CString aQuantity,
                                       const Standard_CString aUnit) {
    Standard_Real aValue = aData;
    CheckLoading(UnitsAPI_DEFAULT);
    if (CurrentUnits->Find(aQuantity)) {
...
Standard_Real UnitsAPI::AnyToLS(const Standard_Real aData, const Standard_CString aUnit,
    ...
    if (quantity) {
        ...
        std::cout << "Warning: BAD Quantity returns in UnitsAPI::AnyToLS(" << aData << "," << aUnit << "," << aDim.get()
                  << ")" << std::endl;
#endif
    return aValue;
}
...
Standard_Real UnitsAPI::AnyToAny(const Standard_Real aData, const Standard_CString aUnit1,
