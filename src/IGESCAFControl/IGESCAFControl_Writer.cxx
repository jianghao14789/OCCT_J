...
void CollectShapeNames(const TDF_Label& theLabel, const TopLoc_Location& theLocation,
                       const Handle(TDataStd_Name) & thePrevName, DataMapOfShapeNames& theMapOfShapeNames) {
...
IGESCAFControl_Writer::IGESCAFControl_Writer()
    : myColorMode(Standard_True), myNameMode(Standard_True), myLayerMode(Standard_True) {}
...
Standard_Boolean IGESCAFControl_Writer::Transfer(const TDF_Label& label, const Message_ProgressRange& theProgress) {
    TDF_LabelSequence labels;
    labels.Append(label);
    return Transfer(labels, theProgress);
}
...
Standard_Boolean IGESCAFControl_Writer::Transfer(const TDF_LabelSequence& labels,
    ...
    for (Standard_Integer i = 1; i <= labels.Length() && aPS.More(); i++) {
        ...
        if (!shape.IsNull()) AddShape(shape, aPS.Next());
        //      IGESControl_Writer::Transfer ( shape );
    }

    // write colors
    if (GetColorMode()) WriteAttributes(labels);

    // write layers
    if (GetLayerMode()) WriteLayers(labels);

    // write names
    if (GetNameMode()) WriteNames(labels);
...
Standard_Boolean IGESCAFControl_Writer::Perform(const Handle(TDocStd_Document) & doc, const Standard_CString filename,
                                                const Message_ProgressRange& theProgress) {
...
void IGESCAFControl_Writer::MakeColors(const TopoDS_Shape& S, const XCAFPrs_IndexedDataMapOfShapeStyle& settings,
                                       XCAFPrs_DataMapOfStyleTransient& colors, TopTools_MapOfShape& Map,
                                       const XCAFPrs_Style& inherit) {
    ...
    if (hasColor) {
        ...
        if (!rank) {
            ...
            if (colors.IsBound(c)) {
                ...
                Handle(TCollection_HAsciiString) str = new TCollection_HAsciiString(col.StringName(col.Name()));
        ...
        if (FP->FindTypedTransient(mapper, STANDARD_TYPE(IGESData_IGESEntity), ent) ||
            ...
            if (!ent_f.IsNull()) {
                if (!ent_f->Surface().IsNull()) ent_f->Surface()->InitColor(colent, rank);
            }
        } else {
...
static void AttachLayer(const Handle(Transfer_FinderProcess) & FP, const Handle(XCAFDoc_LayerTool) & LTool,
                        const TopoDS_Shape& aSh, const Standard_Integer localIntName) {
...
static void MakeLayers(const Handle(Transfer_FinderProcess) & FP, const Handle(XCAFDoc_ShapeTool) & STool,
                       const Handle(XCAFDoc_LayerTool) & LTool, const TDF_LabelSequence& aShapeLabels,
    ...
    for (DataMapOfShapeNames::Iterator anIter(aMapOfShapeNames); anIter.More(); anIter.Next()) {
        ...
        if (aFinderProcess->FindTypedTransient(aShapeMapper, STANDARD_TYPE(IGESData_IGESEntity), anIGESEntity)) {
            ...
            for (Standard_Integer aCharPos = 1; aNameLength < 8; aCharPos++, aNameLength++) {
                anAsciiName->SetValue(aNameLength + 1, IsAnAscii(aName.Value(aCharPos))
                                                           ? (Standard_Character)aName.Value(aCharPos)
                                                           : '?');
            }
