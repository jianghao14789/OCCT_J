...
void Hatch_Hatcher::Trim(const gp_Lin2d& L, const Standard_Real Start, const Standard_Real End,
    ...
    for (iLine = 1; iLine <= myLines.Length(); iLine++) {
        ...
        if (Inters.IsDone()) {
            if (!Inters.IdenticalElements() && !Inters.ParallelElements()) {
                ...
                myLines(iLine).AddIntersection(Pinter.ParamOnFirst(), norm > 0, Index, Pinter.ParamOnSecond(), myToler);
            }
        }
    }
}
...
Standard_Integer Hatch_Hatcher::NbIntervals(const Standard_Integer I) const {
    ...
    if (l == 0)
        ...
        l = l / 2;
        if (myOrient)
            if (!myLines(I).myInters(1).myStart) l++;
    }
    return l;
}
...
void Hatch_Hatcher::StartIndex(const Standard_Integer I, const Standard_Integer J, Standard_Integer& Index,
...
void Hatch_Hatcher::EndIndex(const Standard_Integer I, const Standard_Integer J, Standard_Integer& Index,
                             Standard_Real& Par2) const {
    if (myLines(I).myInters.IsEmpty()) {
        if (J != 1) throw Standard_OutOfRange();
        Index = 0;
