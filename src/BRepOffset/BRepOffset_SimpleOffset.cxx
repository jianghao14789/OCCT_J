...
// purpose  : Constructor
//=============================================================================
BRepOffset_SimpleOffset::BRepOffset_SimpleOffset(const TopoDS_Shape& theInputShape, const Standard_Real theOffsetValue,
                                                 const Standard_Real theTolerance)
    : myOffsetValue(theOffsetValue), myTolerance(theTolerance) {
...
// purpose  :
//=============================================================================
Standard_Boolean BRepOffset_SimpleOffset::NewSurface(const TopoDS_Face& F, Handle(Geom_Surface) & S, TopLoc_Location& L,
                                                     Standard_Real& Tol, Standard_Boolean& RevWires,
                                                     Standard_Boolean& RevFace) {
    if (!myFaceInfo.IsBound(F)) return Standard_False;
...
// purpose  :
//=============================================================================
Standard_Boolean BRepOffset_SimpleOffset::NewCurve(const TopoDS_Edge& E, Handle(Geom_Curve) & C, TopLoc_Location& L,
                                                   Standard_Real& Tol) {
    if (!myEdgeInfo.IsBound(E)) return Standard_False;
...
// purpose  :
//=============================================================================
Standard_Boolean BRepOffset_SimpleOffset::NewPoint(const TopoDS_Vertex& V, gp_Pnt& P, Standard_Real& Tol) {
    if (!myVertexInfo.IsBound(V)) return Standard_False;
...
// purpose  :
//=============================================================================
Standard_Boolean BRepOffset_SimpleOffset::NewCurve2d(const TopoDS_Edge& E, const TopoDS_Face& F,
    ...
    if (myEdgeInfo.IsBound(E)) Tol = myEdgeInfo.Find(E).myTol;
...
// purpose  :
//=============================================================================
Standard_Boolean BRepOffset_SimpleOffset::NewParameter(const TopoDS_Vertex& V, const TopoDS_Edge& E, Standard_Real& P,
                                                       Standard_Real& Tol) {
    // Use original parameter.
    P = BRep_Tool::Parameter(V, E);
    Tol = BRep_Tool::Tolerance(V);

    if (myVertexInfo.IsBound(V)) Tol = myVertexInfo.Find(V).myTol;
...
// purpose  :
//=============================================================================
GeomAbs_Shape BRepOffset_SimpleOffset::Continuity(const TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2,
...
void BRepOffset_SimpleOffset::FillFaceData(const TopoDS_Face& theFace) {
    ...
    if (theFace.Orientation() == TopAbs_REVERSED) aMult = -1.0;
...
void BRepOffset_SimpleOffset::FillEdgeData(const TopoDS_Edge& theEdge,
    ...
    if (aFacesList.Size() == 0) return; // Free edges are skipped.

    // Get offset surface.
    const TopoDS_Face& aCurrFace = TopoDS::Face(aFacesList.First());

    if (!myFaceInfo.IsBound(aCurrFace)) return;
    ...
    for (; !aNED.myOffsetC.IsNull() && anIter.More(); anIter.Next()) {
        ...
        if (!myFaceInfo.IsBound(aCurFace)) continue;
...
void BRepOffset_SimpleOffset::FillVertexData(const TopoDS_Vertex& theVertex,
    ...
    if (aEdgesList.Size() == 0) return; // Free verices are skipped.
    ...
    for (; anIterEdges.More(); anIterEdges.Next()) {
        ...
        if (!myEdgeInfo.IsBound(aCurrEdge)) continue; // Skip shared edges with wrong orientation.
    ...
    for (Standard_Integer i = anOffsetPointVec.Lower(); i <= anOffsetPointVec.Upper(); ++i) {
        ...
        if (aSqDist > aSqMaxDist) aSqMaxDist = aSqDist;
    }
