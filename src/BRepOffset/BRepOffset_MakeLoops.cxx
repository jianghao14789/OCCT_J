...
void BRepOffset_MakeLoops::Build(const TopTools_ListOfShape& LF, const Handle(BRepAlgo_AsDes) & AsDes,
                                 BRepAlgo_Image& Image, BRepAlgo_Image& theImageVV,
            ...
            for (; EdExp.More(); EdExp.Next()) {
                ...
                for (; itlv.More(); itlv.Next()) {
                    ...
                    if (myVerVerMap.IsBound(V)) {
                        ...
                        if (TV->Tolerance() > NewTV->Tolerance()) NewTV->Tolerance(TV->Tolerance());
...
static Standard_Boolean IsBetweenCorks(const TopoDS_Shape& E, const Handle(BRepAlgo_AsDes) & AsDes,
...
void BRepOffset_MakeLoops::BuildOnContext(const TopTools_ListOfShape& LContext, const BRepOffset_Analyse& Analyse,
                                          const Handle(BRepAlgo_AsDes) & AsDes, BRepAlgo_Image& Image,
                                          const Standard_Boolean InSide, const Message_ProgressRange& theRange) {
    //-----------------------------------------
    // unwinding of caps.
    //-----------------------------------------
...
void BRepOffset_MakeLoops::BuildFaces(const TopTools_ListOfShape& LF, const Handle(BRepAlgo_AsDes) & AsDes,
                                      BRepAlgo_Image& Image, const Message_ProgressRange& theRange) {
