...
// purpose  :
//=======================================================================
Handle(Geom_Surface) BRepOffset::Surface(const Handle(Geom_Surface) & Surface, const Standard_Real Offset,
                                         BRepOffset_Status& theStatus, Standard_Boolean allowC0) {
    ...
    if (TheType == STANDARD_TYPE(Geom_Plane)) {
        ...
        Handle(Geom_RectangularTrimmedSurface) S = Handle(Geom_RectangularTrimmedSurface)::DownCast(Surface);
...
Handle(Geom_Surface) BRepOffset::CollapseSingularities(const Handle(Geom_Surface) & theSurface,
                                                       const TopoDS_Face& theFace, Standard_Real thePrecision) {
    ...
    if (aType == STANDARD_TYPE(Geom_BSplineSurface)) {
        ...
        for (Standard_Integer iSide = 0; iSide < 4; iSide++) {
            ...
            for (int iPole = 0; iPole < NbSteps[iSide]; iPole++) {
                aSum +=
                    aPoles(RowStart[iSide] + iPole * RowStep[iSide], ColStart[iSide] + iPole * ColStep[iSide]).XYZ();
            }
            ...
            for (int iPole = 0; iPole < NbSteps[iSide]; iPole++) {
                ...
                if (aDistCG > thePrecision) isSingular = Standard_False;
            ...
            if (isSingular) {
                ...
                for (int iPole = 0; iPole < NbSteps[iSide]; iPole++) {
                    aCopy->SetPole(RowStart[iSide] + iPole * RowStep[iSide], ColStart[iSide] + iPole * ColStep[iSide],
                                   aCenter);
                }
            }
        }

        if (!aCopy.IsNull()) return aCopy;
    }
