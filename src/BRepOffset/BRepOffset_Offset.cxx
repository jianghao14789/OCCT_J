...
static void UpdateEdge(const TopoDS_Edge& E, const Handle(Geom_Curve) & C, const TopLoc_Location& L,
...
static void UpdateEdge(const TopoDS_Edge& E, const Handle(Geom2d_Curve) & C, const TopoDS_Face& F,
                       const Standard_Real Tol) {
    // Cut curves to avoid copies in the extensions.
    BRep_Builder B;
...
static void UpdateEdge(const TopoDS_Edge& E, const Handle(Geom2d_Curve) & C1, const Handle(Geom2d_Curve) & C2,
                       const TopoDS_Face& F, const Standard_Real Tol) {
    // Cut curves to avoid copies in the extensions.
    BRep_Builder B;
...
static void ComputeCurve3d(TopoDS_Edge Edge, const Handle(Geom2d_Curve) & Curve, const Handle(Geom_Surface) & Surf,
                           const TopLoc_Location Loc, Standard_Real Tol) {
            ...
            if (D.IsParallel(gp::DX2d(), Precision::Angular())) { // Iso V.
                if (STy == GeomAbs_Sphere) {
                    ...
                    if (Abs(Abs(P.Y()) - M_PI / 2.) < Precision::PConfusion()) {
                        ...
                        if (D.IsOpposite(gp::DX2d(), Precision::Angular())) Circle->Reverse();
                ...
                if (STy == GeomAbs_Sphere) {
                    ...
                    if (D.IsOpposite(gp::DY2d(), Precision::Angular())) Circle->Reverse();
                    UpdateEdge(Edge, Circle, Loc, Tol);
                    IsComputed = Standard_True;
                } else if (STy == GeomAbs_Cylinder) {
                    gp_Cylinder Cyl = S.Cylinder();
                    ...
                    if (D.IsOpposite(gp::DY2d(), Precision::Angular())) Line->Reverse();
...
BRepOffset_Offset::BRepOffset_Offset(const TopoDS_Face& Face, const Standard_Real Offset,
                                     const Standard_Boolean OffsetOutside, const GeomAbs_JoinType JoinType) {
...
BRepOffset_Offset::BRepOffset_Offset(const TopoDS_Face& Face, const Standard_Real Offset,
                                     const TopTools_DataMapOfShapeShape& Created, const Standard_Boolean OffsetOutside,
...
BRepOffset_Offset::BRepOffset_Offset(const TopoDS_Edge& Path, const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
                                     const Standard_Real Offset, const Standard_Boolean Polynomial,
                                     const Standard_Real Tol, const GeomAbs_Shape Conti) {
...
BRepOffset_Offset::BRepOffset_Offset(const TopoDS_Edge& Path, const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
                                     const Standard_Real Offset, const TopoDS_Edge& FirstEdge,
                                     const TopoDS_Edge& LastEdge, const Standard_Boolean Polynomial,
                                     const Standard_Real Tol, const GeomAbs_Shape Conti) {
...
BRepOffset_Offset::BRepOffset_Offset(const TopoDS_Vertex& Vertex, const TopTools_ListOfShape& LEdge,
                                     const Standard_Real Offset, const Standard_Boolean Polynomial,
                                     const Standard_Real Tol, const GeomAbs_Shape Conti) {
...
void BRepOffset_Offset::Init(const TopoDS_Face& Face, const Standard_Real Offset, const Standard_Boolean OffsetOutside,
                             const GeomAbs_JoinType JoinType) {
...
void BRepOffset_Offset::Init(const TopoDS_Face& Face, const Standard_Real Offset,
                             const TopTools_DataMapOfShapeShape& Created, const Standard_Boolean OffsetOutside,
                             const GeomAbs_JoinType JoinType) {
    myShape = Face;
    Standard_Real myOffset = Offset;
    if (Face.Orientation() == TopAbs_REVERSED) myOffset *= -1.;
    ...
    if ((S->IsKind(STANDARD_TYPE(Geom_BSplineSurface)) || S->IsKind(STANDARD_TYPE(Geom_SurfaceOfLinearExtrusion)) ||
         S->IsKind(STANDARD_TYPE(Geom_SurfaceOfRevolution)) || S->IsKind(STANDARD_TYPE(Geom_OffsetSurface))) &&
        !L.IsIdentity()) {
    ...
    if (!(OffsetOutside && JoinType == GeomAbs_Arc) && (TheSurf->DynamicType() == STANDARD_TYPE(Geom_ConicalSurface) ||
        ...
        for (; Explo.More(); Explo.Next()) {
            ...
            if (BRep_Tool::Degenerated(anEdge)) {
                ...
                gp_Pnt aFPnt = S->Value(aFPnt2d.X(), aFPnt2d.Y()), aLPnt = S->Value(aLPnt2d.X(), aLPnt2d.Y());
        ...
        if (!DegEdges.IsEmpty()) {
            ...
            if (Abs(fp2d.X() - lp2d.X()) <= Precision::PConfusion()) UisoDegen = Standard_True;
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (HasSingularity && BRep_Tool::Degenerated(E)) VonDegen.Add(V1);
    ...
    for (; expw.More(); expw.Next()) {
        ...
        for (; expe.More(); expe.Next()) {
            ...
            TopoDS_Edge OE;
            if (MapSS.IsBound(E) && !VonDegen.Contains(V1) && !VonDegen.Contains(V2)) { // c`est un edge de couture
                OE = TopoDS::Edge(MapSS(E));
                TopoDS_Shape aLocalShape = E.Reversed();
                Handle(Geom2d_Curve) C2d_1 = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalShape), CurFace, f, l);
                ...
                if (VonDegen.Contains(V1)) {
                    ...
                    if (!L.IsIdentity() && !IsTransformed) P1.Transform(L.Transformation());
                    vstart = P2d1.Y();
                }
                if (VonDegen.Contains(V2)) {
                    ...
                    if (!L.IsIdentity() && !IsTransformed) P2.Transform(L.Transformation());
                    vend = P2d2.Y();
                }
                ...
                if (VonDegen.Contains(V1) || VonDegen.Contains(V2)) {
                    if (VonDegen.Contains(V1)) P2d1.SetY(vstart);
                    if (VonDegen.Contains(V2)) P2d2.SetY(vend);
                ...
                if (!BRep_Tool::Degenerated(OE)) ComputeCurve3d(OE, C2d, TheSurf, L, BRep_Tool::Tolerance(E));
                MapSS.Bind(E, OE);
            }
...
void BRepOffset_Offset::Init(const TopoDS_Edge& Path, const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
                             const Standard_Real Offset, const Standard_Boolean Polynomial, const Standard_Real Tol,
...
void BRepOffset_Offset::Init(const TopoDS_Edge& Path, const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
                             const Standard_Real Offset, const TopoDS_Edge& FirstEdge, const TopoDS_Edge& LastEdge,
                             const Standard_Boolean Polynomial, const Standard_Real Tol, const GeomAbs_Shape Conti) {
    ...
    if (!Pipe.IsDone()) throw Standard_ConstructionError("GeomFill_Pipe : Cannot make a surface");
    ...
    if (!C2is3D && !C2Denerated) myBuilder.Range(Edge2, U1, U2, Standard_True);
    ...
    if (Edge1.Orientation() == TopAbs_REVERSED) myFace.Reverse();
}
...
void BRepOffset_Offset::Init(const TopoDS_Vertex& Vertex, const TopTools_ListOfShape& LEdge, const Standard_Real Offset,
                             const Standard_Boolean Polynomial, const Standard_Real TolApp, const GeomAbs_Shape Conti) {
    myStatus = BRepOffset_Good;
    myShape = Vertex;
    ...
    switch (myShape.ShapeType()) {
        ...
        case TopAbs_EDGE:
            // have generate a pipe.
            {
                ...
                if (myFace.Orientation() == TopAbs_REVERSED) aShape.Reverse();
            }
            break;
        default:
            break;
