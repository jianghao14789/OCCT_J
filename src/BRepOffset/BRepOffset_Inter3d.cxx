...
// purpose  :
//=======================================================================
BRepOffset_Inter3d::BRepOffset_Inter3d(const Handle(BRepAlgo_AsDes) & AsDes, const TopAbs_State Side,
                                       const Standard_Real Tol)
    : myAsDes(AsDes), mySide(Side), myTol(Tol) {}
...
void BRepOffset_Inter3d::CompletInt(const TopTools_ListOfShape& SetOfFaces, const BRepAlgo_Image& InitOffsetFace,
...
void BRepOffset_Inter3d::FaceInter(const TopoDS_Face& F1, const TopoDS_Face& F2, const BRepAlgo_Image& InitOffsetFace) {
    ...
    Standard_Boolean InterPipes = (InitF2.ShapeType() == TopAbs_EDGE && InitF1.ShapeType() == TopAbs_EDGE);
    Standard_Boolean InterFaces = (InitF1.ShapeType() == TopAbs_FACE && InitF2.ShapeType() == TopAbs_FACE);
...
void BRepOffset_Inter3d::ConnexIntByArc(const TopTools_ListOfShape& /*SetOfFaces*/, const TopoDS_Shape& ShapeInit,
                                        const BRepOffset_Analyse& Analyse, const BRepAlgo_Image& InitOffsetFace,
                                        const Message_ProgressRange& theRange) {
            ...
            for (Standard_Integer i = 0; i < 2; i++) {
                if (!InitOffsetFace.HasImage(V[i])) {
                    ...
                    for (it.Initialize(Anc); it.More(); it.Next()) {
                        ...
                        if (E1.IsSame(E2) || isToSkip) continue;
                        ...
                        if (InitOffsetFace.HasImage(E2)) {
                            ...
                            if (AncE2.Extent() == 2) {
                                ...
                                if (!TangentFaces) {
                                    F2 = TopoDS::Face(InitOffsetFace.Image(InitF2).First());
                                    if (!IsDone(F1, F2)) {
                                        BRepOffset_Tool::Inter3D(F1, F2, LInt1, LInt2, mySide, NullEdge, NullFace,
                                                                 NullFace);
                                        Store(F1, F2, LInt1, LInt2);
                                    }
                                }
...
void BRepOffset_Inter3d::ConnexIntByInt(const TopoDS_Shape& SI, const BRepOffset_DataMapOfShapeOffset& MapSF,
                                        const BRepOffset_Analyse& Analyse, TopTools_DataMapOfShapeShape& MES,
                                        TopTools_DataMapOfShapeShape& Build, TopTools_ListOfShape& Failed,
                                        const Message_ProgressRange& theRange, const Standard_Boolean bIsPlanar) {
    ...
    if (bIsPlanar) {
        ...
        for (i = 1; i <= aNb; ++i) {
            ...
            if (!aFGen.IsNull()) TopExp::MapShapes(aFGen, TopAbs_EDGE, VEmap);
        }
    ...
    if (bIsPlanar) {
        ...
        NCollection_DataMap<TopoDS_Shape, TopTools_MapOfShape, TopTools_ShapeMapHasher> aDMFEI;
        {
            for (TopExp_Explorer expF(SI, TopAbs_FACE); expF.More(); expF.Next()) {
                ...
                for (TopExp_Explorer expE(aFx, TopAbs_EDGE); expE.More(); expE.Next()) {
                    ...
                    if (aEx.Orientation() != TopAbs_FORWARD && aEx.Orientation() != TopAbs_REVERSED) aMEI.Add(aEx);
                }
                if (!aMEI.IsEmpty()) aDMFEI.Bind(aFx, aMEI);
            }
        }
        ...
        for (i = aNb + 1, aNb = VEmap.Extent(); i <= aNb; ++i) {
            ...
            if (aS.ShapeType() != TopAbs_VERTEX) continue;
            ...
            TopTools_ListOfShape aLF;
            {
                ...
                for (TopTools_ListOfShape::Iterator itLE(aLE); itLE.More(); itLE.Next()) {
                    ...
                    for (TopTools_ListOfShape::Iterator itLEA(aLEA); itLEA.More(); itLEA.Next()) {
                        if (!aLF.Contains(itLEA.Value())) aLF.Append(itLEA.Value());
                    }
                }
            }
            ...
            for (; it.More(); it.Next()) {
                ...
                for (; itLE1.More(); itLE1.Next()) {
                    ...
                    if (pF1Internal && pF1Internal->Contains(aE)) break;
    ...
    for (i = 1; i <= aNb; ++i, aPSInter.Next()) {
        ...
        for (; itF1.More() && itF2.More(); itF1.Next(), itF2.Next()) {
            ...
            if (!MES.IsBound(OF1)) {
                ...
                BRepOffset_Tool::EnLargeFace(OF1, NF1, Standard_True, Standard_True, enlargeU, enlargeVfirst,
                                             enlargeVlast);
                MES.Bind(OF1, NF1);
            } else {
            ...
            if (!MES.IsBound(OF2)) {
                ...
                BRepOffset_Tool::EnLargeFace(OF2, NF2, Standard_True, Standard_True, enlargeU, enlargeVfirst,
                                             enlargeVlast);
                MES.Bind(OF2, NF2);
            } else {
...
// purpose  :
//=======================================================================
void BRepOffset_Inter3d::ContextIntByArc(const TopTools_IndexedMapOfShape& ContextFaces, const Standard_Boolean InSide,
                                         const BRepOffset_Analyse& Analyse, const BRepAlgo_Image& InitOffsetFace,
                                         BRepAlgo_Image& InitOffsetEdge, const Message_ProgressRange& theRange) {
    TopTools_ListOfShape LInt1, LInt2;
    TopTools_MapOfShape MV;
    TopExp_Explorer exp;
    TopoDS_Face OF1, OF2;
...
// purpose  :
//=======================================================================
void BRepOffset_Inter3d::Store(const TopoDS_Face& F1, const TopoDS_Face& F2, const TopTools_ListOfShape& LInt1,
