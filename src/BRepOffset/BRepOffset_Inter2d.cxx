...
static void GetEdgesOrientedInFace(const TopoDS_Shape& theShape, const TopoDS_Face& theFace,
                                   const Handle(BRepAlgo_AsDes) & theAsDes, TopTools_SequenceOfShape& theSeqEdges) {
    ...
    for (;;) {
        ...
        if (aLastVertex.IsSame(aFirstVertex)) break;

        const TopTools_ListOfShape& aElist = aVEmap.FindFromKey(aLastVertex);
        if (aElist.Extent() == 1) break;
        ...
        if (theSeqEdges.Length() == aNbEdges) break;
    }
}
...
//=======================================================================
static void Store(const TopoDS_Edge& theEdge, const TopTools_ListOfShape& theLV, const Standard_Real theTol,
                  const Standard_Boolean IsToUpdate, Handle(BRepAlgo_AsDes) theAsDes2d,
...
//=======================================================================
static void Store(const TopoDS_Edge& theE1, const TopoDS_Edge& theE2, const TopTools_ListOfShape& theLV1,
                  const TopTools_ListOfShape& theLV2, const Standard_Real theTol, Handle(BRepAlgo_AsDes) theAsDes2d,
                  TopTools_IndexedDataMapOfShapeListOfShape& theDMVV) {
...
static void EdgeInter(const TopoDS_Face& F, const BRepAdaptor_Surface& BAsurf, const TopoDS_Edge& E1,
                      const TopoDS_Edge& E2, const Handle(BRepAlgo_AsDes) & AsDes, Standard_Real Tol,
                      Standard_Boolean WithOri, TopTools_IndexedDataMapOfShapeListOfShape& aDMVV) {
#ifdef DRAW
...
#endif
                if (CrossProd > 0.) OO1 = TopAbs_FORWARD;
                CrossProd = V1or ^ V2;
                if (CrossProd > 0.) OO2 = TopAbs_FORWARD;
            }
    ...
    if (!LV1.IsEmpty()) {
        ...
        while (Purge) {
            ...
            for (it1LV1.Initialize(LV1), it1LV2.Initialize(LV2); it1LV1.More(); it1LV1.Next(), it1LV2.Next()) {
                j = 1;
...
static void RefEdgeInter(const TopoDS_Face& F, const BRepAdaptor_Surface& BAsurf, const TopoDS_Edge& E1,
                         const TopoDS_Edge& E2, const TopAbs_Orientation theOr1, const TopAbs_Orientation theOr2,
                         const Handle(BRepAlgo_AsDes) & AsDes, Standard_Real Tol, Standard_Boolean WithOri,
                         const TopoDS_Vertex& theVref, BRepAlgo_Image& theImageVV,
                         TopTools_IndexedDataMapOfShapeListOfShape& aDMVV, Standard_Boolean& theCoincide) {
#ifdef DRAW
    if (Inter2dAffichInt2d) {
        char name[256];
        sprintf(name, "E2d_%d_%d", NbF2d, NbE2d++);
    ...
    if (!Inter2d.IsDone() || !Inter2d.NbPoints()) {
        theCoincide = (Inter2d.NbSegments() && (GAC1.GetType() == GeomAbs_Line) && (GAC2.GetType() == GeomAbs_Line));
        return;
    }
    //
    for (i = 1; i <= Inter2d.NbPoints(); i++) {
    ...
    for (i = 1; i <= ResPoints.Length(); i++) {
        ...
        if (WithOri) {
            ...
            Standard_Real CrossProd = V2or ^ V1;
#ifdef OCCT_DEBUG
            if (Abs(CrossProd) <= gp::Resolution()) std::cout << std::endl << "CrossProd = " << CrossProd << std::endl;
#endif
            if (CrossProd > 0.) OO1 = TopAbs_FORWARD;
            CrossProd = V1or ^ V2;
            if (CrossProd > 0.) OO2 = TopAbs_FORWARD;
        ...
        if (theOr1 != TopAbs_EXTERNAL) OO1 = theOr1;
        if (theOr2 != TopAbs_EXTERNAL) OO2 = theOr2;
...
static Standard_Boolean ExtendPCurve(const Handle(Geom2d_Curve) & aPCurve, const Standard_Real anEf,
    ...
    if (FirstPar > anEf - a2Offset) {
        ...
        if (!aCompCurve.Add(aSegment, aTol)) return Standard_False;
    }
...
Standard_Boolean BRepOffset_Inter2d::ExtentEdge(const TopoDS_Edge& E, TopoDS_Edge& NE, const Standard_Real theOffset) {
        ...
        if (CurveRep->IsCurveOnSurface()) {
            ...
            if (theCurve->IsKind(STANDARD_TYPE(Geom2d_BoundedCurve)) &&
                ...
                if (ExtendPCurve(theCurve, anEf, anEl, a2Offset, NewPCurve)) {
                    ...
                    if (CurveRep->IsCurveOnClosedSurface()) {
                        ...
                        if (ExtendPCurve(PCurve2, anEf, anEl, a2Offset, NewPCurve)) CurveRep->PCurve2(NewPCurve);
                    }
                }
            ...
            for (i = 1; i <= BoundLines.Length(); i++) {
                ...
                if (IntCC.IsDone()) {
                    for (j = 1; j <= IntCC.NbPoints(); j++) {
                        ...
                        if (aPoint.X() >= Umin && aPoint.X() <= Umax && aPoint.Y() >= Vmin && aPoint.Y() <= Vmax)
            ...
            if (!params.IsEmpty()) {
                if (params.Length() == 1) {
                    ...
                    if (PntFirst.X() >= Umin && PntFirst.X() <= Umax && PntFirst.Y() >= Vmin && PntFirst.Y() <= Vmax) {
                        if (LastPar > params(1)) LastPar = params(1);
                    } else if (FirstPar < params(1))
                        FirstPar = params(1);
                } else {
                    Standard_Real fpar = RealLast(), lpar = RealFirst();
                    for (i = 1; i <= params.Length(); i++) {
                        if (params(i) < fpar) fpar = params(i);
                        if (params(i) > lpar) lpar = params(i);
                    }
                    if (FirstPar < fpar) FirstPar = fpar;
                    if (LastPar > lpar) LastPar = lpar;
                }
            }
            //// end of check ////
    ...
    if (NbPCurves) {
        ...
        if (!C3d.IsNull()) {
            if (MinPC->IsClosed()) {
                ...
                if (!Precision::IsInfinite(FirstParOnPC)) {
                    ...
                    if (Projector.NbPoints() > 0) f = Projector.LowerDistanceParameter();
#ifdef OCCT_DEBUG
                    else
                ...
                if (!Precision::IsInfinite(LastParOnPC)) {
                    ...
                    if (Projector.NbPoints() > 0) l = Projector.LowerDistanceParameter();
#ifdef OCCT_DEBUG
                    else
                        std::cout << "ProjectPointOnCurve not done" << std::endl;
#endif
        ...
        } else if (!BRep_Tool::Degenerated(E)) // no 3d curve
        {
            ...
            if (Precision::IsInfinite(FirstParOnPC) || Precision::IsInfinite(LastParOnPC)) {
                if (MinPC->IsInstance(STANDARD_TYPE(Geom2d_Line))) {
                    ...
                    if (MinSurf->IsInstance(STANDARD_TYPE(Geom_Plane)))
                        ...
                        if (LineDir.IsParallel(gp::DY2d(), Precision::Angular())) IsLine = Standard_True;
                    }
                    if (IsLine) {
                ...
                GeomLib::BuildCurve3d(Precision::Confusion(), ConS, FirstParOnPC, LastParOnPC, C3d, max_deviation,
                                      average_deviation, Continuity, MaxDegree, MaxSegment);
            }
            ...
            if (NbPCurves > 1)
                // BRepLib::SameParameter( NE, Precision::Confusion(), Standard_True );
                for (itr.Initialize((Handle(BRep_TEdge)::DownCast(NE.TShape()))->ChangeCurves()); itr.More();
                     itr.Next()) {
                    Handle(BRep_CurveRepresentation) CurveRep = itr.Value();
                    Standard_Real FirstPar, LastPar;
                    if (CurveRep->IsCurveOnSurface()) {
                        ...
                        if (theCurve == MinPC && theSurf == MinSurf && theLoc == MinLoc) continue;
    ...
    } else // no pcurves
    {
        ...
        if (C3d->IsKind(STANDARD_TYPE(Geom_BoundedCurve)) &&
            ...
            if (FirstPar > anEf - a2Offset) {
                ...
                if (!aCompCurve.Add(aSegment, aTol)) return Standard_True;
            }
...
static Standard_Boolean UpdateVertex(TopoDS_Vertex V, TopoDS_Edge& OE, TopoDS_Edge& NE, Standard_Real TolConf) {
...
void BRepOffset_Inter2d::Compute(const Handle(BRepAlgo_AsDes) & AsDes, const TopoDS_Face& F,
                                 const TopTools_IndexedMapOfShape& NewEdges, const Standard_Real Tol,
        ...
        while (j < i && it2LE.More()) {
            ...
            if (theEdgeIntEdges.IsBound(E1)) {
                ...
                for (; itedges.More(); itedges.Next())
                    if (E2.IsSame(itedges.Value())) ToIntersect = Standard_False;
                    ...
                    for (itedges.Initialize(aElist); itedges.More(); itedges.Next()) {
                        ...
                        if (theEdgeIntEdges.IsBound(anEdge)) {
                            ...
                            for (; itedges2.More(); itedges2.Next())
                                if (E2.IsSame(itedges2.Value())) ToIntersect = Standard_False;
                        }
                    }
                }
            }
...
Standard_Boolean BRepOffset_Inter2d::ConnexIntByInt(
    const TopoDS_Face& FI, BRepOffset_Offset& OFI, TopTools_DataMapOfShapeShape& MES,
    const TopTools_DataMapOfShapeShape& Build, const Handle(BRepAlgo_AsDes) & theAsDes,
    const Handle(BRepAlgo_AsDes) & AsDes2d, const Standard_Real Offset, const Standard_Real Tol,
    const BRepOffset_Analyse& Analyse, TopTools_IndexedMapOfShape& FacesWithVerts, BRepAlgo_Image& theImageVV,
    TopTools_DataMapOfShapeListOfShape& theEdgeIntEdges, TopTools_IndexedDataMapOfShapeListOfShape& theDMVV,
    const Message_ProgressRange& theRange) {
    ...
    for (; it.More(); it.Next()) {
        ...
        if (YaBuild) {
            for (itL.Initialize(L); itL.More(); itL.Next()) {
                ...
                if (EI.Orientation() != TopAbs_FORWARD && EI.Orientation() != TopAbs_REVERSED) continue;
    ...
    for (; exp.More(); exp.Next(), aPS.Next()) {
        ...
        if (!wexp.More()) continue; // Protection from case when explorer does not contain edges.
        ...
        while (!end) {
            ...
            if (DoInter) {
                ...
                if (aE1.Orientation() == TopAbs_REVERSED) anOr1 = TopAbs::Reverse(anOr1);
                if (aE2.Orientation() == TopAbs_REVERSED) anOr2 = TopAbs::Reverse(anOr2);

                RefEdgeInter(FIO, BAsurf, aE1, aE2, anOr1, anOr2, AsDes2d, Tol, Standard_True, Vref, theImageVV,
                             theDMVV, bCoincide);
...
void BRepOffset_Inter2d::ConnexIntByIntInVert(
    const TopoDS_Face& FI, BRepOffset_Offset& OFI, TopTools_DataMapOfShapeShape& MES,
    const TopTools_DataMapOfShapeShape& Build, const Handle(BRepAlgo_AsDes) & AsDes,
    const Handle(BRepAlgo_AsDes) & AsDes2d, const Standard_Real Tol, const BRepOffset_Analyse& Analyse,
    TopTools_IndexedDataMapOfShapeListOfShape& theDMVV, const Message_ProgressRange& theRange) {
    TopoDS_Face FIO = TopoDS::Face(OFI.Face());
    if (MES.IsBound(FIO)) FIO = TopoDS::Face(MES(FIO));
    //
        ...
        while (!end) {
            ...
            for (Exp2.Init(NE3, TopAbs_EDGE); Exp2.More(); Exp2.Next()) {
                ...
                for (Exp1.Init(NE1, TopAbs_EDGE); Exp1.More(); Exp1.Next()) {
                    ...
                    RefEdgeInter(FIO, BAsurf, aE1, aE3, anOr1, anOr2, AsDes2d, Tol, Standard_True, Vref, anEmptyImage,
                                 theDMVV, bCoincide);
                    if (bCoincide) {
                ...
                for (Exp1.Init(NE2, TopAbs_EDGE); Exp1.More(); Exp1.Next()) {
                    ...
                    BRepAlgo_Image anEmptyImage;
                    RefEdgeInter(FIO, BAsurf, aE2, aE3, anOr1, anOr2, AsDes2d, Tol, Standard_True, Vref, anEmptyImage,
                                 theDMVV, bCoincide);
                    if (bCoincide) {
                ...
                for (Exp1.Next(); Exp1.More(); Exp1.Next()) {
                    ...
                    if (aME.Contains(aE3Next)) {
                        BRepAlgo_Image anEmptyImage;
                        RefEdgeInter(FIO, BAsurf, aE3Next, aE3, anOr1, anOr2, AsDes2d, Tol, Standard_True, Vref,
                                     anEmptyImage, theDMVV, bCoincide);
                    }
                }
            }
            CurE = wexp.Current();
...
// purpose  :
//=======================================================================
static void MakeChain(const TopoDS_Shape& theV, const TopTools_IndexedDataMapOfShapeListOfShape& theDMVV,
                      TopTools_MapOfShape& theMDone, TopTools_ListOfShape& theChain) {
...
Standard_Boolean BRepOffset_Inter2d::FuseVertices(const TopTools_IndexedDataMapOfShapeListOfShape& theDMVV,
                                                  const Handle(BRepAlgo_AsDes) & theAsDes, BRepAlgo_Image& theImageVV) {
    BRep_Builder aBB;
