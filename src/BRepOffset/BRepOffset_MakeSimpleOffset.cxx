...
BRepOffset_MakeSimpleOffset::BRepOffset_MakeSimpleOffset()
    : myOffsetValue(0.), myTolerance(Precision::Confusion()), myIsBuildSolid(Standard_False), myMaxAngle(0.0),
      myError(BRepOffsetSimple_OK), myIsDone(Standard_False) {
...
BRepOffset_MakeSimpleOffset::BRepOffset_MakeSimpleOffset(const TopoDS_Shape& theInputShape,
    ...
    : myInputShape(theInputShape), myOffsetValue(theOffsetValue), myTolerance(Precision::Confusion()),
      myIsBuildSolid(Standard_False), myMaxAngle(0.0), myError(BRepOffsetSimple_OK), myIsDone(Standard_False) {
    myReShape = new ShapeBuild_ReShape();
}
...
// purpose  :
//=============================================================================
void BRepOffset_MakeSimpleOffset::Initialize(const TopoDS_Shape& theInputShape, const Standard_Real theOffsetValue) {
...
Standard_Real BRepOffset_MakeSimpleOffset::GetSafeOffset(const Standard_Real theExpectedToler) {
    if (myInputShape.IsNull()) return 0.0; // Input shape is null.
...
void BRepOffset_MakeSimpleOffset::Perform() {
    ...
    for (; anExpSE.More(); anExpSE.Next()) {
        ...
        if (!BRep_Tool::Degenerated(aCurrEdge)) continue;
    ...
    if (myIsBuildSolid && !BuildMissingWalls()) return;
...
//=============================================================================
static void tgtfaces(const TopoDS_Edge& Ed, const TopoDS_Face& F1, const TopoDS_Face& F2,
                     const Standard_Boolean couture, Standard_Real& theResAngle) {
...
TopoDS_Face BRepOffset_MakeSimpleOffset::BuildWallFace(const TopoDS_Edge& theOrigEdge) {
    ...
    if (myMapVE.IsBound(aForwardV2)) {
        ...
        if (!aME1.IsDone()) return aResFace;
        aWall1 = aME1.Edge();
    ...
    if (myMapVE.IsBound(aForwardV1)) {
        ...
        if (!aME2.IsDone()) return aResFace;
        aWall2 = aME2.Edge();
...
const TopoDS_Shape BRepOffset_MakeSimpleOffset::Modified(const TopoDS_Shape& theShape) const {
    ...
    if (aModStatus == 0) return anEmptyShape; // No modifications are applied to the shape or its sub-shapes.
