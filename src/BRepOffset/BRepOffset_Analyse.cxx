...
static void CorrectOrientationOfTangent(gp_Vec& TangVec, const TopoDS_Vertex& aVertex, const TopoDS_Edge& anEdge) {
    TopoDS_Vertex Vlast = TopExp::LastVertex(anEdge);
    if (aVertex.IsSame(Vlast)) TangVec.Reverse();
}
//=======================================================================
...
BRepOffset_Analyse::BRepOffset_Analyse() : myOffset(0.0), myDone(Standard_False) {}
...
// purpose  :
//=======================================================================
static void EdgeAnalyse(const TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2, const Standard_Real SinTol,
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::Perform(const TopoDS_Shape& S, const Standard_Real Angle,
    ...
    for (; Exp.More(); Exp.Next(), aPS.Next()) {
        ...
        if (!myMapEdgeType.IsBound(E)) {
            ...
            if (L.Extent() == 2) {
                ...
                if (myMapEdgeType(E).Last().Type() == ChFiDS_Tangential) aLETang.Append(E);
            } else if (L.Extent() == 1) {
                Standard_Real U1, U2;
...
void BRepOffset_Analyse::TreatTangentFaces(const TopTools_ListOfShape& theLE, const Message_ProgressRange& theRange) {
    ...
    if (anEdgeOffsetMap.IsEmpty()) return;
    ...
    for (TopTools_ListOfShape::Iterator itE(theLE); itE.More(); itE.Next(), aPS2.Next()) {
        ...
        if (!anEdgeOffsetMap.IsBound(aE)) continue;
        ...
        for (TopoDS_Iterator itV(aE); itV.More(); itV.Next()) {
            ...
            for (TopTools_ListOfShape::Iterator itLVA(aLVA); itLVA.More(); itLVA.Next()) {
                ...
                if (!pIntervals || pIntervals->IsEmpty()) continue;
                if (pIntervals->First().Type() == ChFiDS_Tangential) continue;

                const TopTools_ListOfShape& aLEA = Ancestors(aEA);
                for (TopTools_ListOfShape::Iterator itLEA(aLEA); itLEA.More(); itLEA.Next()) {
                    const TopoDS_Shape& aFA = itLEA.Value();
                    if (aMFence.Add(aFA)) pLFA->Append(aFA);
                }
            }
        }
    }
    ...
    for (TopTools_ListOfListOfShape::Iterator itLCB(aLCB); itLCB.More(); itLCB.Next(), aPS3.Next()) {
        ...
        for (TopTools_ListOfShape::Iterator itCB1(aCB); itCB1.More(); itCB1.Next()) {
            ...
            for (itCB2.Next(); itCB2.More(); itCB2.Next()) {
                ...
                if (aMFence.Contains(aE2)) continue;
            ...
            for (TopoDS_Iterator itE(aBlock); itE.More(); itE.Next()) {
                ...
                // Orient the face so its normal is directed to smaller offset face
                {
                    ...
                    BOPTools_AlgoTools3D::PointNearEdge(aEInF, TopoDS::Face(aFOpposite), (f + l) / 2., 1.e-5, aP2d,
                                                        aPInF);

                    gp_Vec aBN(aPOnE, aPInF);

                    if (aBN.Dot(aDN) < 0) aFNew.Reverse();
                }
                ...
                for (TopoDS_Iterator itV(aE); itV.More(); itV.Next()) {
                    ...
                    if (aDMVEMin.IsBound(aV)) {
                        ...
                        if (pSA && pSA->Extent() == 1) {
                            ...
                            if (anOriInEG == anOriInEMin) aEG.Reverse();
                        }
                    }

                    TopTools_ListOfShape& aLVA = myAncestors.ChangeFromKey(aV);
                    if (!aLVA.Contains(aEG)) aLVA.Append(aEG);
                    aFNewEdgeMap.Add(aEG);

                    TopTools_ListOfShape& aLEGA = myAncestors(myAncestors.Add(aEG, aPrismAncestors.FindFromKey(aEG)));
                    {
                ...
                if (!pEEMap) pEEMap = myReplacement.Bound(aFToRemove, TopTools_DataMapOfShapeShape());
...
// purpose  :
//=======================================================================
const TopoDS_Edge& BRepOffset_Analyse::EdgeReplacement(const TopoDS_Face& theF, const TopoDS_Edge& theE) const {
    ...
    if (myDescendants.IsEmpty() || theUpdate) {
        ...
        for (Standard_Integer i = 1; i <= aNbA; ++i) {
            ...
            for (TopTools_ListOfShape::Iterator it(aLA); it.More(); it.Next()) {
                ...
                if (!pLD) pLD = myDescendants.Bound(aSA, TopTools_ListOfShape());
                if (!pLD->Contains(aSS)) pLD->Append(aSS);
            }
        }
    }
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::Edges(const TopoDS_Vertex& V, const ChFiDS_TypeOfConcavity T, TopTools_ListOfShape& LE) const {
    LE.Clear();
    ...
    for (; it.More(); it.Next()) {
        ...
        if (pIntervals && pIntervals->Extent() > 0) {
            ...
            if (V1.IsSame(V)) {
                if (pIntervals->Last().Type() == T) LE.Append(E);
            }
            if (V2.IsSame(V)) {
                if (pIntervals->First().Type() == T) LE.Append(E);
            }
        }
    }
}
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::Edges(const TopoDS_Face& F, const ChFiDS_TypeOfConcavity T, TopTools_ListOfShape& LE) const {
    LE.Clear();
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::TangentEdges(const TopoDS_Edge& Edge, const TopoDS_Vertex& Vertex,
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::Explode(TopTools_ListOfShape& List, const ChFiDS_TypeOfConcavity T1,
...
// purpose  :
//=======================================================================
void BRepOffset_Analyse::AddFaces(const TopoDS_Face& Face, TopoDS_Compound& Co, TopTools_MapOfShape& Map,
    ...
    for (TopTools_ListOfShape::Iterator it(*pLE); it.More(); it.Next()) {
        ...
        if (!LI.IsEmpty() && LI.First().Type() == T) {
            ...
            if (L.Extent() == 2) {
                ...
                if (F1.IsSame(Face)) F1 = TopoDS::Face(L.Last());
                if (Map.Add(F1)) {
                    B.Add(Co, F1);
...
//=======================================================================
void BRepOffset_Analyse::AddFaces(const TopoDS_Face& Face, TopoDS_Compound& Co, TopTools_MapOfShape& Map,
                                  const ChFiDS_TypeOfConcavity T1, const ChFiDS_TypeOfConcavity T2) const {
    BRep_Builder B;
    const TopTools_ListOfShape* pLE = Descendants(Face);
    if (!pLE) return;
    for (TopTools_ListOfShape::Iterator it(*pLE); it.More(); it.Next()) {
