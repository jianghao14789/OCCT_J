...
static void PerformPlanes(const TopoDS_Face& theFace1, const TopoDS_Face& theFace2, const TopAbs_State theState,
                          TopTools_ListOfShape& theL1, TopTools_ListOfShape& theL2);
...
static void FindPeriod(const TopoDS_Face& F, Standard_Real& umin, Standard_Real& umax, Standard_Real& vmin,
                       Standard_Real& vmax) {
...
static void BuildPCurves(const TopoDS_Edge& E, const TopoDS_Face& F) {
    ...
    if (typS == STANDARD_TYPE(Geom_BezierSurface) || typS == STANDARD_TYPE(Geom_BSplineSurface)) {
        ...
        for (; Explo.More(); Explo.Next()) {
            ...
            if (!fextr.IsDone() || fextr.NbExt() < 1) continue;
            ...
            if (dist2min > TolProj * TolProj) continue;
            Extrema_ExtPC lextr(lpoint, aCurve);
            if (!lextr.IsDone() || lextr.NbExt() < 1) continue;
            dist2min = RealLast();
        ...
        if (!theEdge.IsNull()) {
            ...
            if (theSurf->IsUPeriodic() || theSurf->IsVPeriodic()) PutInBounds(F, E, C2d);
...
void BRepOffset_Tool::OrientSection(const TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2,
                                    TopAbs_Orientation& O1, TopAbs_Orientation& O2) {
    TopLoc_Location L;
    Standard_Real f, l;
...
// purpose  :
//=======================================================================
Standard_Boolean BRepOffset_Tool::FindCommonShapes(const TopoDS_Face& theF1, const TopoDS_Face& theF2,
                                                   TopTools_ListOfShape& theLE, TopTools_ListOfShape& theLV) {
...
// purpose  :
//=======================================================================
Standard_Boolean BRepOffset_Tool::FindCommonShapes(const TopoDS_Shape& theS1, const TopoDS_Shape& theS2,
                                                   const TopAbs_ShapeEnum theType, TopTools_ListOfShape& theLSC) {
    theLSC.Clear();
    //
...
static Standard_Boolean IsOnSurface(const Handle(Geom_Curve) & C, const Handle(Geom_Surface) & S, Standard_Real TolConf,
    ...
    switch (AS.GetType()) {
        case GeomAbs_Plane: {
            ...
            for (Standard_Integer i = 0; i < n; i++) {
                ...
                if (TolReached > TolConf) return Standard_False;
            }
            break;
        }
...
void BRepOffset_Tool::PipeInter(const TopoDS_Face& F1, const TopoDS_Face& F2, TopTools_ListOfShape& L1,
                                TopTools_ListOfShape& L2, const TopAbs_State Side) {
#ifdef DRAW
    if (AffichInter) {
        char name[256];
...
static Standard_Boolean IsAutonomVertex(const TopoDS_Shape& theVertex, const BOPDS_PDS& thePDS,
    ...
    for (Standard_Integer i = 0; i < 2; i++) {
        ...
        if (IndMap.Contains(nV)) return Standard_False;
    }
...
static Standard_Boolean AreConnex(const TopoDS_Wire& W1, const TopoDS_Wire& W2) {
    ...
    if (V11.IsSame(V21) || V11.IsSame(V22) || V12.IsSame(V21) || V12.IsSame(V22)) return Standard_True;
...
static Standard_Boolean AreClosed(const TopoDS_Edge& E1, const TopoDS_Edge& E2) {
    ...
    if ((V11.IsSame(V21) && V12.IsSame(V22)) || (V11.IsSame(V22) && V12.IsSame(V21))) return Standard_True;
...
static Standard_Boolean BSplineEdges(const TopoDS_Edge& E1, const TopoDS_Edge& E2, const Standard_Integer par1,
                                     const Standard_Integer par2, Standard_Real& angle) {
    Standard_Real first1, last1, first2, last2, Param1, Param2;

    Handle(Geom_Curve) C1 = BRep_Tool::Curve(E1, first1, last1);
    if (C1->IsInstance(STANDARD_TYPE(Geom_TrimmedCurve))) C1 = Handle(Geom_TrimmedCurve)::DownCast(C1)->BasisCurve();

    Handle(Geom_Curve) C2 = BRep_Tool::Curve(E2, first2, last2);
    if (C2->IsInstance(STANDARD_TYPE(Geom_TrimmedCurve))) C2 = Handle(Geom_TrimmedCurve)::DownCast(C2)->BasisCurve();

    if (!C1->IsInstance(STANDARD_TYPE(Geom_BSplineCurve)) || !C2->IsInstance(STANDARD_TYPE(Geom_BSplineCurve)))
        return Standard_False;
...
static void ReconstructPCurves(const TopoDS_Edge& anEdge) {
    ...
    BRep_ListIteratorOfListOfCurveRepresentation itcr((Handle(BRep_TEdge)::DownCast(anEdge.TShape()))->ChangeCurves());
...
static Handle(Geom2d_Curve)
    ConcatPCurves(const TopoDS_Edge& E1, const TopoDS_Edge& E2, const TopoDS_Face& F, const Standard_Boolean After,
                  Standard_Real& newFirst, Standard_Real& newLast) {
    ...
    if (PCurve1 == PCurve2) {
        ...
        newLast = Max(last1, last2);
               ...
               (PCurve1->IsInstance(STANDARD_TYPE(Geom2d_Line)) || PCurve1->IsKind(STANDARD_TYPE(Geom2d_Conic)))) {
        newPCurve = PCurve1;
        gp_Pnt2d P1, P2;
        ...
        if (newPCurve->Continuity() < GeomAbs_C1) {
            ...
            if (Approx2d.HasResult()) newPCurve = Approx2d.Curve();
        }
...
static TopoDS_Edge Glue(const TopoDS_Edge& E1, const TopoDS_Edge& E2, const TopoDS_Vertex& Vfirst,
                        const TopoDS_Vertex& Vlast, const Standard_Boolean After, const TopoDS_Face& F1,
                        const Standard_Boolean addPCurve1, const TopoDS_Face& F2, const Standard_Boolean addPCurve2,
    ...
    if (C1 == C2) {
        ...
        if (!Concat.Add(TC2, theGlueTol, After)) return newEdge;
        ...
        if (newCurve->Continuity() < GeomAbs_C1) {
            ...
            if (Approx3d.HasResult()) newCurve = Approx3d.Curve();
        }
...
static void CheckIntersFF(const BOPDS_PDS& pDS, const TopoDS_Edge& RefEdge, TopTools_IndexedMapOfShape& TrueEdges) {
    ...
    if (CompList.Extent() == 1)
        ...
        for (; itl.More(); itl.Next()) {
            ...
            if (!Projector.IsDone() || Projector.NbSolution() == 0) continue;
...
static TopoDS_Edge AssembleEdge(const BOPDS_PDS& pDS, const TopoDS_Face& F1, const TopoDS_Face& F2,
                                const Standard_Boolean addPCurve1, const Standard_Boolean addPCurve2,
    ...
    for (Standard_Integer j = 2; j <= EdgesForConcat.Length(); j++) {
        ...
        TopoDS_Edge NewEdge = Glue(CurEdge, anEdge, Vfirst, Vlast, After, F1, addPCurve1, F2, addPCurve2, aGlueTol);
        if (NewEdge.IsNull())
            return NullEdge;
        else
            CurEdge = NewEdge;
...
void BRepOffset_Tool::Inter3D(const TopoDS_Face& F1, const TopoDS_Face& F2, TopTools_ListOfShape& L1,
                              TopTools_ListOfShape& L2, const TopAbs_State Side, const TopoDS_Edge& RefEdge,
                              const TopoDS_Face& theRefFace1, const TopoDS_Face& theRefFace2) {
#ifdef DRAW
    if (AffichInter) {
        char name[256];
        sprintf(name, "FF_%d", NbFaces++);
    ...
    if (!RefEdge.IsNull()) CheckIntersFF(aPF.PDS(), RefEdge, TrueEdges);
    ...
    for (i = 0; i < aNb; i++) {
        ...
        for (j = 0; j < aNbCurves; j++) {
            ...
            for (; aPBIt.More(); aPBIt.Next()) {
                ...
                Standard_Integer nSect = aPB->Edge();
                const TopoDS_Edge& anEdge = *(TopoDS_Edge*)&pDS->Shape(nSect);
                if (!TrueEdges.IsEmpty() && !TrueEdges.Contains(anEdge)) continue;

                Standard_Real f, l;
                const Handle(Geom_Curve) & aC3DE = BRep_Tool::Curve(anEdge, f, l);
                Handle(Geom_TrimmedCurve) aC3DETrim;

                if (!aC3DE.IsNull()) aC3DETrim = new Geom_TrimmedCurve(aC3DE, f, l);
    ...
    if (L1.Extent() > 1 && (!isEl1 || !isEl2) && !theRefFace1.IsNull()) {
        ...
        if (!aV1.IsSame(aV2)) // only if RefEdge is open
        {
            ...
            if (aRefSurf1->IsUClosed() || aRefSurf1->IsVClosed() || aRefSurf2->IsUClosed() || aRefSurf2->IsVClosed()) {
    ...
    if (L1.Extent() > 1 && (!isEl1 || !isEl2)) {
        ...
        if (!TrueEdges.IsEmpty()) {
            ...
            while (!edges.IsEmpty()) {
                ...
                if (Candidates.IsEmpty()) {
                    ...
                    for (j = 2; j <= edges.Length(); j++) {
                        ...
                        BB.MakeWire(aWire);
                        BB.Add(aWire, anEdge);
                        if (AreConnex(resWire, aWire)) Candidates.Append(j);
                    }
            ...
            for (i = 1; i <= wseq.Length(); i++) {
                ...
                TopoDS_Edge AssembledEdge = AssembleEdge(pDS, F1, F2, addPCurve1, addPCurve2, aLocalEdgesForConcat);
                if (AssembledEdge.IsNull())
...
Standard_Boolean BRepOffset_Tool::TryProject(const TopoDS_Face& F1, const TopoDS_Face& F2,
                                             const TopTools_ListOfShape& Edges, TopTools_ListOfShape& LInt1,
                                             TopTools_ListOfShape& LInt2, const TopAbs_State Side,
...
void BRepOffset_Tool::InterOrExtent(const TopoDS_Face& F1, const TopoDS_Face& F2, TopTools_ListOfShape& L1,
                                    TopTools_ListOfShape& L2, const TopAbs_State Side) {
#ifdef DRAW
    if (AffichInter) {
        char name[256];
...
static void ExtentEdge(const TopoDS_Face& F, const TopoDS_Face& EF, const TopoDS_Edge& E, TopoDS_Edge& NE) {
    ...
    if (Type == GeomAbs_Line || Type == GeomAbs_Circle || Type == GeomAbs_Ellipse || Type == GeomAbs_Hyperbola ||
...
static Standard_Boolean ProjectVertexOnEdge(TopoDS_Vertex& V, const TopoDS_Edge& E, Standard_Real TolConf) {
#ifdef DRAW
    if (AffichExtent) {
        DBRep::Set("V", V);
        DBRep::Set("E", E);
...
void BRepOffset_Tool::Inter2d(const TopoDS_Face& F, const TopoDS_Edge& E1, const TopoDS_Edge& E2,
                              TopTools_ListOfShape& LV, const Standard_Real TolConf) {
#ifdef DRAW
    if (AffichExtent) {
        DBRep::Set("E1", E1);
        DBRep::Set("E2", E2);
...
static void SelectEdge(const TopoDS_Face& /*F*/, const TopoDS_Face& /*EF*/, const TopoDS_Edge& E,
...
static void MakeFace(const Handle(Geom_Surface) & S, const Standard_Real Um, const Standard_Real UM,
                     const Standard_Real Vm, const Standard_Real VM, const Standard_Boolean uclosed,
                     const Standard_Boolean vclosed, const Standard_Boolean isVminDegen,
                     const Standard_Boolean isVmaxDegen, TopoDS_Face& F) {
    ...
    if (theSurf->DynamicType() == STANDARD_TYPE(Geom_ConicalSurface)) {
        ...
        if (Abs(VMin - Vapex) <= Precision::Confusion()) vmindegen = Standard_True;
        if (Abs(VMax - Vapex) <= Precision::Confusion()) vmaxdegen = Standard_True;
    }
    ...
    if (vmindegen) V10 = V00;
    if (vmaxdegen) V11 = V01;

    // make the lines
    Handle(Geom2d_Line) Lumin, Lumax, Lvmin, Lvmax;
    if (!umininf) Lumin = new Geom2d_Line(gp_Pnt2d(UMin, 0), gp_Dir2d(0, 1));
    if (!umaxinf) Lumax = new Geom2d_Line(gp_Pnt2d(UMax, 0), gp_Dir2d(0, 1));
    if (!vmininf) Lvmin = new Geom2d_Line(gp_Pnt2d(0, VMin), gp_Dir2d(1, 0));
    if (!vmaxinf) Lvmax = new Geom2d_Line(gp_Pnt2d(0, VMax), gp_Dir2d(1, 0));
    ...
    if (hasiso) {
        if (!umininf) Cumin = S->UIso(UMin);
        if (!umaxinf) Cumax = S->UIso(UMax);
        if (!vmininf) {
            Cvmin = S->VIso(VMin);
            if (BRepOffset_Tool::Gabarit(Cvmin) <= TolApex) vmindegen = Standard_True;
        }
        if (!vmaxinf) {
            Cvmax = S->VIso(VMax);
            if (BRepOffset_Tool::Gabarit(Cvmax) <= TolApex) vmaxdegen = Standard_True;
        }
    }
    ...
    if (!vmininf) {
        ...
        if (vmindegen) B.Degenerated(evmin, Standard_True);
    }
    ...
    if (!vmaxinf) {
        if (vclosed)
            ...
            if (vmaxdegen) B.Degenerated(evmax, Standard_True);
        }
    }
...
static Standard_Boolean EnlargeGeometry(Handle(Geom_Surface) & S, Standard_Real& U1, Standard_Real& U2,
                                        Standard_Real& V1, Standard_Real& V2, Standard_Boolean& IsV1degen,
                                        Standard_Boolean& IsV2degen, const Standard_Real uf1, const Standard_Real uf2,
                                        const Standard_Real vf1, const Standard_Real vf2, const Standard_Real coeff,
                                        ...
                                        const Standard_Real theLenBeforeUfirst, const Standard_Real theLenAfterUlast,
                                        const Standard_Real theLenBeforeVfirst, const Standard_Real theLenAfterVlast) {
    ...
    if (S->DynamicType() == STANDARD_TYPE(Geom_RectangularTrimmedSurface)) {
        ...
        EnlargeGeometry(BS, U1, U2, V1, V2, IsV1degen, IsV2degen, uf1, uf2, vf1, vf2, coeff, theGlobalEnlargeU,
                        theGlobalEnlargeVfirst, theGlobalEnlargeVlast, theLenBeforeUfirst, theLenAfterUlast,
                        theLenBeforeVfirst, theLenAfterVlast);
        if (!theGlobalEnlargeVfirst) V1 = vf1;
        if (!theGlobalEnlargeVlast) V2 = vf2;
        ...
        SurfaceChange = EnlargeGeometry(Surf, U1, U2, V1, V2, IsV1degen, IsV2degen, uf1, uf2, vf1, vf2, coeff,
        ...
        if (Precision::IsInfinite(u1) || Precision::IsInfinite(u2)) {
            ...
            uiso1 = S->UIso(uf1);
            uiso2 = S->UIso(uf2);
            if (BRepOffset_Tool::Gabarit(uiso1) <= TolApex) enlargeUfirst = Standard_False;
            if (BRepOffset_Tool::Gabarit(uiso2) <= TolApex) enlargeUlast = Standard_False;
        }
        ...
        Standard_Boolean enlargeUfirst = enlargeU, enlargeUlast = enlargeU;
        Standard_Boolean enlargeVfirst = theGlobalEnlargeVfirst, enlargeVlast = theGlobalEnlargeVlast;
        if (S->IsUClosed()) enlargeU = Standard_False;
        if (S->IsVClosed()) enlargeV = Standard_False;
        ...
        if (gabarit_viso1 <= TolApex || gabarit_viso2 <= TolApex) enlargeU = Standard_False;
        if (gabarit_uiso1 <= TolApex || gabarit_uiso2 <= TolApex) enlargeV = Standard_False;
        ...
        if (enlargeU) {
            ...
            du_first = (theLenBeforeUfirst == -1) ? du_default : theLenBeforeUfirst;
            du_last = (theLenAfterUlast == -1) ? du_default : theLenAfterUlast;
            if (gabarit_uiso1 <= TolApex) enlargeUfirst = Standard_False;
            if (gabarit_uiso2 <= TolApex) enlargeUlast = Standard_False;
        }
        if (enlargeV) {
            gac.Load(uiso1);
            Standard_Real dv_default = GCPnts_AbscissaPoint::Length(gac) * coeff;
...
static void CompactUVBounds(const TopoDS_Face& F, Standard_Real& UMin, Standard_Real& UMax, Standard_Real& VMin,
                            Standard_Real& VMax) {
...
void BRepOffset_Tool::CheckBounds(const TopoDS_Face& F, const BRepOffset_Analyse& Analyse, Standard_Boolean& enlargeU,
                                  Standard_Boolean& enlargeVfirst, Standard_Boolean& enlargeVlast) {
                    ...
                    if (!theLine.IsNull()) {
                        ...
                        if (theDir.IsParallel(gp::DX2d(), Precision::Angular())) {
                            ...
                            if (BRep_Tool::Degenerated(anEdge)) {
                                ...
                                if (theLine->Location().Y() < Vfirst) Vfirst = theLine->Location().Y();
                                if (theLine->Location().Y() > Vlast) Vlast = theLine->Location().Y();
                            }
                        } else if (theDir.IsParallel(gp::DY2d(), Precision::Angular())) {
                            Ubound++;
                            if (theLine->Location().X() < Ufirst) Ufirst = theLine->Location().X();
                            if (theLine->Location().X() > Ulast) Ulast = theLine->Location().X();
                        }
                    }
                }
            }
    ...
    if (Ubound >= 2 || Vbound >= 2) {
        if (Ubound >= 2 && Abs(UF1 - Ufirst) <= Precision::Confusion() && Abs(UF2 - Ulast) <= Precision::Confusion())
            enlargeU = Standard_False;
        if (Vbound >= 2 && Abs(VF1 - Vfirst) <= Precision::Confusion() && Abs(VF2 - Vlast) <= Precision::Confusion()) {
            enlargeVfirst = Standard_False;
            enlargeVlast = Standard_False;
        }
    }
...
Standard_Boolean
BRepOffset_Tool::EnLargeFace(const TopoDS_Face& F, TopoDS_Face& BF, const Standard_Boolean CanExtentSurface,
                             const Standard_Boolean UpdatePCurve, const Standard_Boolean theEnlargeU,
                             const Standard_Boolean theEnlargeVfirst, const Standard_Boolean theEnlargeVlast,
                             const Standard_Integer theExtensionMode, const Standard_Real theLenBeforeUfirst,
                             const Standard_Real theLenAfterUlast, const Standard_Real theLenBeforeVfirst,
    ...
    if (CanExtentSurface) {
        SurfaceChange = EnlargeGeometry(S, UU1, UU2, VV1, VV2, isVV1degen, isVV2degen, UF1, UF2, VF1, VF2, coeff,
                                        theEnlargeU, theEnlargeVfirst, theEnlargeVlast, theLenBeforeUfirst,
                                        theLenAfterUlast, theLenBeforeVfirst, theLenAfterVlast);
    } else {
        UU1 = Max(US1, UU1);
        UU2 = Min(UU2, US2);
        VV1 = Max(VS1, VV1);
    ...
    if (!theEnlargeVfirst) VV1 = VF1;
    if (!theEnlargeVlast) VV2 = VF2;
    ...
    if (uclosed && !uperiodic && (theLenBeforeUfirst != 0. || theLenAfterUlast != 0.)) uclosed = Standard_False;
    if (vclosed && !vperiodic && (theLenBeforeVfirst != 0. && theLenAfterVlast != 0.)) vclosed = Standard_False;
...
static Standard_Boolean TryParameter(const TopoDS_Edge& OE, TopoDS_Vertex& V, const TopoDS_Edge& NE,
...
void BRepOffset_Tool::BuildNeighbour(const TopoDS_Wire& W, const TopoDS_Face& F, TopTools_DataMapOfShapeShape& NOnV1,
...
void BRepOffset_Tool::ExtentFace(const TopoDS_Face& F, TopTools_DataMapOfShapeShape& ConstShapes,
                                 TopTools_DataMapOfShapeShape& ToBuild, const TopAbs_State Side,
                                 const Standard_Real TolConf, TopoDS_Face& NF) {
#ifdef DRAW
    if (AffichInter) {
        char name[256];
        ...
        for (Standard_Integer i = 1; i <= Emap.Extent(); i++) {
            ...
            if (!C2.IsNull()) {
                ...
                if (BRep_Tool::IsClosed(CE, Fforward)) {
                    ...
                    if (!Ecs.IsNull()) B.UpdateEdge(Ecs, C2, C2R, EF, BRep_Tool::Tolerance(CE));
                } else {
                    B.UpdateEdge(CE, C2, EF, BRep_Tool::Tolerance(CE));
                    if (!Ecs.IsNull()) B.UpdateEdge(Ecs, C2, EF, BRep_Tool::Tolerance(CE));
                }
                B.Range(CE, f, l);
                if (!Ecs.IsNull()) B.Range(Ecs, f, l);
            }
        }
    }
    ...
    for (exp.Init(F.Oriented(TopAbs_FORWARD), TopAbs_WIRE); exp.More(); exp.Next()) {
        ...
        for (exp2.Init(W.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp2.More(); exp2.Next()) {
            ...
            if (ToBuild.IsBound(E)) {
                ...
                LOE.Append(E);
                BRepOffset_Tool::TryProject(TopoDS::Face(ToBuild(E)), EF, LOE, LInt2, LInt1, Side, TolConf);
                if (!LInt1.IsEmpty()) ToBuild.UnBind(E);
            }
        }
            ...
            if (Build.IsBound(E)) {
                ...
                if (Build.IsBound(NEOnV1) && (ToBuild.IsBound(E) || ToBuild.IsBound(NEOnV1))) {
                    if (E.IsSame(NEOnV1))
                        ...
                        //---------------
                        if (!Build.IsBound(V1)) {
                            Inter2d(EF, TopoDS::Edge(Build(E)), TopoDS::Edge(Build(NEOnV1)), LV,
                            ...
                            if (MVE(V1).Extent() > 2) {
                                ...
                                if (Build(E).Orientation() == TopAbs_REVERSED) V.Orientation(TopAbs_REVERSED);
                ...
                if (Build.IsBound(NEOnV2) && (ToBuild.IsBound(E) || ToBuild.IsBound(NEOnV2))) {
                    if (E.IsSame(NEOnV2))
                        ...
                        if (!Build.IsBound(V2)) {
                            Inter2d(EF, TopoDS::Edge(Build(E)), TopoDS::Edge(Build(NEOnV2)), LV,
                                    /*TolConf*/ Precision::Confusion());
                            ...
                            if (MVE(V2).Extent() > 2) {
                                ...
                                if (Build(E).Orientation() == TopAbs_REVERSED) V.Orientation(TopAbs_FORWARD);
        ...
        for (exp2.Init(W.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp2.More(); exp2.Next()) {
            ...
            if (Build.IsBound(E)) {
                ...
                U1 = BRep_Tool::Parameter(TopoDS::Vertex(aLocalVertexOrientedNV1), TopoDS::Edge(aLocalEdge));
        ...
        case TopAbs_SHELL: {
            ...
            for (Standard_Integer i = 1; i <= Map.Extent() && JeGarde; i++) {
                ...
                if (aLF.Extent() < 2) {
                    ...
                    if (!Boundary.Contains(anEdge) && !BRep_Tool::Degenerated(anEdge)) JeGarde = Standard_False;
                }
            }
            if (JeGarde) SS = S;
        } break;
...
static Standard_Boolean IsInOut(BRepTopAdaptor_FClass2d& FC, Geom2dAdaptor_Curve AC, const TopAbs_State& S) {
...
void BRepOffset_Tool::CorrectOrientation(const TopoDS_Shape& SI, const TopTools_IndexedMapOfShape& NewEdges,
                                         Handle(BRepAlgo_AsDes) & AsDes, BRepAlgo_Image& InitOffset,
...
// purpose  :
//=======================================================================
Standard_Boolean BRepOffset_Tool::CheckPlanesNormals(const TopoDS_Face& theFace1, const TopoDS_Face& theFace2,
...
// purpose  :
//=======================================================================
void PerformPlanes(const TopoDS_Face& theFace1, const TopoDS_Face& theFace2, const TopAbs_State theSide,
                   TopTools_ListOfShape& theL1, TopTools_ListOfShape& theL2) {
    theL1.Clear();
    theL2.Clear();
