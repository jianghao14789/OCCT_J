...
static TopoDS_Edge MakeEdge(const Handle(Geom2d_Curve) & theCurve, const TopoDS_Face& theFace,
                            const TopoDS_Vertex& theVFirst, const TopoDS_Vertex& theVLast);
...
static Handle(Geom2d_TrimmedCurve)
    AdjustCurveEnd(const Handle(Geom2d_BoundedCurve) & theC2d, const gp_Pnt2d theP, const Standard_Boolean isFirst);
//
//=======================================================================
...
void BRepMAT2d_Explorer::Add(const TopoDS_Wire& Spine, const TopoDS_Face& aFace, TopoDS_Face& aNewFace) {
    ...
    if (aFirstEdge.Orientation() == TopAbs_REVERSED) CT2d->Reverse();
    ...
    for (; anExp.More(); anExp.Next()) {
        ...
        if (anEdge.Orientation() == TopAbs_REVERSED) CT2d->Reverse();
    ...
    if (isModif) {
        ...
        if (myIsClosed(currentContour)) aNewWire.Closed(Standard_True);
...
TopoDS_Edge MakeEdge(const Handle(Geom2d_Curve) & theCurve, const TopoDS_Face& theFace, const TopoDS_Vertex& theVFirst,
...
Handle(Geom2d_TrimmedCurve)
    ...
    if (aType == GeomAbs_Line) {
        ...
        Handle(Geom2d_BSplineCurve) BCurve = Geom2dConvert::CurveToBSplineCurve(theC2d, Convert_QuasiAngular);
        if (isFirst) {
