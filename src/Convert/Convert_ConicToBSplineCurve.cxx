...
// purpose  :
//=======================================================================
Convert_ConicToBSplineCurve::Convert_ConicToBSplineCurve(const Standard_Integer NbPoles, const Standard_Integer NbKnots,
...
gp_Pnt2d Convert_ConicToBSplineCurve::Pole(const Standard_Integer Index) const {
    if (Index < 1 || Index > nbPoles) throw Standard_OutOfRange(" ");
...
Standard_Real Convert_ConicToBSplineCurve::Knot(const Standard_Integer Index) const {
    if (Index < 1 || Index > nbKnots) throw Standard_OutOfRange(" ");
...
void CosAndSinRationalC1(Standard_Real Parameter, const Standard_Integer EvalDegree,
                         const TColgp_Array1OfPnt2d& EvalPoles, const TColStd_Array1OfReal& EvalKnots,
                         const TColStd_Array1OfInteger* EvalMults, Standard_Real Result[2]) {
    gp_Pnt2d a_point;
    BSplCLib::D0(Parameter, 0, EvalDegree, Standard_False, EvalPoles, BSplCLib::NoWeights(), EvalKnots, EvalMults,
                 a_point);
...
void CosAndSinQuasiAngular(Standard_Real Parameter, const Standard_Integer EvalDegree,
                           ...
                           const TColStd_Array1OfInteger*, Standard_Real Result[2]) {
...
// purpose  :
//=======================================================================
void AlgorithmicCosAndSin(Standard_Integer Degree, const TColStd_Array1OfReal& FlatKnots,
                          const Standard_Integer EvalDegree, const TColgp_Array1OfPnt2d& EvalPoles,
                          const TColStd_Array1OfReal& EvalKnots, const TColStd_Array1OfInteger* EvalMults,
                          Convert_CosAndSinEvalFunction Evaluator, TColStd_Array1OfReal& CosNumerator,
                          TColStd_Array1OfReal& SinNumerator, TColStd_Array1OfReal& Denominator) {
    ...
    if (num_poles != CosNumerator.Length() || num_poles != SinNumerator.Length() || num_poles != Denominator.Length()) {
    ...
    BSplCLib::Interpolate(Degree, FlatKnots, parameters, contact_order_array, poles_array, pivot_index_problem);
...
void Convert_ConicToBSplineCurve::BuildCosAndSin(const Convert_ParameterisationType Parameterisation,
                                                 ...
                                                 Standard_Integer& Degree, Handle(TColStd_HArray1OfReal) & KnotsPtr,
                                                 Handle(TColStd_HArray1OfInteger) & MultsPtr) const {
    Standard_Real delta = ULast - UFirst, direct, inverse, value1, value2, cos_beta, sin_beta, alpha = 0, alpha_2,
                  alpha_4, tan_alpha_2, beta, p_param, q_param, param;

    Standard_Integer num_poles = 0, ii, jj, num_knots = 1, num_spans = 1, num_flat_knots, num_temp_knots,
    ...
    if (tgt_theta_flag) {
        ...
        AlgorithmicCosAndSin(Degree, flat_knots, temp_degree, temp_poles, temp_knots, &temp_mults, *EvaluatorPtr,
                             CosNumeratorPtr->ChangeArray1(), SinNumeratorPtr->ChangeArray1(),
        ...
        BuildPolynomialCosAndSin(UFirst, ULast, num_poles, CosNumeratorPtr, SinNumeratorPtr, DenominatorPtr);
    }
}
//=======================================================================
// function : BuildCosAndSin
...
void Convert_ConicToBSplineCurve::BuildCosAndSin(const Convert_ParameterisationType Parameterisation,
    ...
    Standard_Integer ii, jj, index, num_poles, num_periodic_poles, temp_degree, pivot_index_problem, num_flat_knots,
        num_knots, order;
    ...
    if (Parameterisation == Convert_TgtThetaOver2) {
        BuildCosAndSin(Convert_TgtThetaOver2_3, 0.0e0, 2 * M_PI, temp_cos_ptr, temp_sin_ptr, temp_denominator_ptr,
        ...
        first_param = 0.0e0;
        last_param = M_PI;
        BuildCosAndSin(Convert_RationalC1, first_param, last_param, temp_cos_ptr, temp_sin_ptr, temp_denominator_ptr,
                       temp_degree, temp_knots_ptr, temp_mults_ptr);
        ...
        for (ii = parameters.Lower(); ii <= parameters.Upper(); ii++) {
            ...
            BSplCLib::D0(param, 0, temp_degree, Standard_False, temp_cos_ptr->Array1(), &temp_denominator_ptr->Array1(),
                         temp_knots_ptr->Array1(), &temp_mults_ptr->Array1(), value1);

            BSplCLib::D0(param, 0, temp_degree, Standard_False, temp_sin_ptr->Array1(), &temp_denominator_ptr->Array1(),
                         temp_knots_ptr->Array1(), &temp_mults_ptr->Array1(), value2);
            BSplCLib::D0(param, 0, temp_degree, Standard_False, temp_denominator_ptr->Array1(), BSplCLib::NoWeights(),
                         temp_knots_ptr->Array1(), &temp_mults_ptr->Array1(), value3);
            contact_order_array(ii) = 0;
        ...
        BSplCLib::Interpolate(Degree, flat_knots, parameters, contact_order_array, poles_array, pivot_index_problem);
