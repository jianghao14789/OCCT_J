...
static void ComputePoles(const Standard_Real R, const Standard_Real U1, const Standard_Real U2, const Standard_Real V1,
                         const Standard_Real V2, TColgp_Array2OfPnt& Poles) {
    ...
    for (i = 1; i <= nbUSpans; i++) {
        Poles(2 * i, 1) = gp_Pnt(R * Cos(UStart + AlfaU) / Cos(AlfaU), R * Sin(UStart + AlfaU) / Cos(AlfaU), V1);
        Poles(2 * i, 2) = gp_Pnt(R * Cos(UStart + AlfaU) / Cos(AlfaU), R * Sin(UStart + AlfaU) / Cos(AlfaU), V2);
...
Convert_CylinderToBSplineSurface::Convert_CylinderToBSplineSurface(const gp_Cylinder& Cyl, const Standard_Real U1,
                                                                   const Standard_Real U2, const Standard_Real V1,
                                                                   const Standard_Real V2)
    : Convert_ElementarySurfaceToBSplineSurface(TheNbUPoles, TheNbVPoles, TheNbUKnots, TheNbVKnots, TheUDegree,
                                                TheVDegree) {
    Standard_Real deltaU = U2 - U1;
    Standard_DomainError_Raise_if((Abs(V2 - V1) <= Abs(Epsilon(V1))) || (deltaU > 2 * M_PI) || (deltaU < 0.),
...
Convert_CylinderToBSplineSurface::Convert_CylinderToBSplineSurface(const gp_Cylinder& Cyl, const Standard_Real V1,
                                                                   const Standard_Real V2)
    : Convert_ElementarySurfaceToBSplineSurface(TheNbUPoles, TheNbVPoles, TheNbUKnots, TheNbVKnots, TheUDegree,
                                                TheVDegree) {
    Standard_DomainError_Raise_if(Abs(V2 - V1) <= Abs(Epsilon(V1)), "Convert_CylinderToBSplineSurface");
