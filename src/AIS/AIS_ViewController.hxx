...
public: //! @name global parameters
    ...
    void SetOrbitAcceleration(float theRatio) {
        myOrbitAccel = theRatio;
    }

    //! Return TRUE if panning anchor point within perspective projection should be displayed in 3D Viewer; TRUE by
    //! default.
    bool ToShowPanAnchorPoint() const {
        return myToShowPanAnchorPoint;
    }
    ...
    bool ToStickToRayOnRotation() const {
        return myToStickToRayOnRotation;
    }
    ...
    void SetStickToRayOnRotation(bool theToEnable) {
        myToStickToRayOnRotation = theToEnable;
    }
...
public: //! @name keyboard input
    ...
    Standard_EXPORT virtual void KeyDown(Aspect_VKey theKey, double theTime,
...
public: //! @name mouse input
    ...
    Standard_EXPORT virtual void UpdateRubberBand(const Graphic3d_Vec2i& thePntFrom, const Graphic3d_Vec2i& thePntTo);
    ...
    Standard_EXPORT virtual bool UpdateMouseButtons(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButtons,
    ...
    //!                      but emulated from non-precise input like touch on screen
    //! @return TRUE if View should be redrawn
    Standard_EXPORT virtual bool UpdateMousePosition(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButtons,
                                                     Aspect_VKeyFlags theModifiers,
                                                     bool theIsEmulated) Standard_OVERRIDE;
    ...
    //! @return TRUE if View should be redrawn
    Standard_EXPORT virtual bool UpdateMouseClick(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButton,
...
public: //! @name multi-touch input
    ...
    void SetTouchToleranceScale(float theTolerance) {
        myTouchToleranceScale = theTolerance;
    }
    ...
    Standard_EXPORT virtual void AddTouchPoint(Standard_Size theId, const Graphic3d_Vec2d& thePnt,
    ...
    //! @param theId touch unique identifier
    //! @param thePnt touch coordinates
    Standard_EXPORT virtual void UpdateTouchPoint(Standard_Size theId, const Graphic3d_Vec2d& thePnt) Standard_OVERRIDE;
...
public: //! @name resize events
    ...
    //! Default implementation does nothing - input events are accumulated in internal buffer until explicit
    //! FlushViewEvents() call.
...
public:
    ...
    //! This method is expected to be called from UI thread.
    Standard_EXPORT virtual void UpdateViewOrientation(V3d_TypeOfOrientation theOrientation, bool theToFitAll);
...
public:
    ...
    Standard_EXPORT virtual bool PickPoint(gp_Pnt& thePnt, const Handle(AIS_InteractiveContext) & theCtx,
    ...
    //! @return TRUE if result has been found
    Standard_EXPORT virtual bool PickAxis(gp_Pnt& theTopPnt, const Handle(AIS_InteractiveContext) & theCtx,
...
public:
    ...
    //! Default implementation starts an animated transaction from the current to the target camera orientation, when
    //! specific action key was pressed. This method is expected to be called from rendering thread.
    ...
    double MinZoomDistance() const {
        return myMinCamDistance;
    }

    //! Set minimal camera distance for zoom operation.
    void SetMinZoomDistance(double theDist) {
        myMinCamDistance = theDist;
    }

    //! Handle zoom event myGL.ZoomActions.
    //! This method is expected to be called from rendering thread.
    Standard_EXPORT virtual void handleZoom(const Handle(V3d_View) & theView, const Aspect_ScrollDelta& theParams,
                                            const gp_Pnt* thePnt);
    ...
    //! @param theToLockZUp amend camera to exclude roll angle (put camera Up vector to plane containing global Z and
    //! view direction)
    Standard_EXPORT virtual void handleOrbitRotation(const Handle(V3d_View) & theView, const gp_Pnt& thePnt,
                                                     bool theToLockZUp);
    ...
    Standard_EXPORT virtual void handleViewRotation(const Handle(V3d_View) & theView, double theYawExtra,
                                                    double thePitchExtra, double theRoll, bool theToRestartOnIncrement);
...
protected:
    ...
    Standard_Boolean myToInvertPitch;  //!< flag inverting pitch direction while processing
                                       //!< Aspect_VKey_NavLookUp/Aspect_VKey_NavLookDown
    ...
    Standard_Boolean
        myToStickToRayOnRotation; //!< project picked point to ray while rotating around point; TRUE by default
...
protected:                                //! @name mouse input variables
    ...
    AIS_MouseSelectionSchemeMap myMouseSelectionSchemes; //!< map defining selection schemes bound to mouse + modifiers
...
protected:                                      //! @name multi-touch input variables
    ...
    Standard_ShortReal
        myTouchRotationThresholdPx; //!< threshold for starting one-touch rotation     gesture in pixels;  6 by default
    Standard_ShortReal myTouchZRotationThreshold; //!< threshold for starting two-touch Z-rotation   gesture in radians;
                                                  //!< 2 degrees by default
    Standard_ShortReal
        myTouchPanThresholdPx; //!< threshold for starting two-touch panning      gesture in pixels;  4 by default
    Standard_ShortReal
        myTouchZoomThresholdPx; //!< threshold for starting two-touch zoom (pitch) gesture in pixels;  6 by default
    Standard_ShortReal myTouchZoomRatio; //!< distance ratio for mapping two-touch zoom (pitch) gesture from pixels to
                                         //!< zoom; 0.13 by default
    ...
    Standard_Boolean
        myUpdateStartPointPan; //!< flag indicating that new anchor  point should be picked for starting panning gesture
    Standard_Boolean myUpdateStartPointRot;  //!< flag indicating that new gravity point should be picked for starting
                                             //!< rotation   gesture
    Standard_Boolean myUpdateStartPointZRot; //!< flag indicating that new gravity point should be picked for starting
                                             //!< Z-rotation gesture
