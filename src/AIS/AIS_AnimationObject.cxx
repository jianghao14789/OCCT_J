...
AIS_AnimationObject::AIS_AnimationObject(const TCollection_AsciiString& theAnimationName,
                                         ...
                                         const Handle(AIS_InteractiveObject) & theObject, const gp_Trsf& theTrsfStart,
                                         const gp_Trsf& theTrsfEnd)
    : AIS_Animation(theAnimationName), myContext(theContext), myObject(theObject),
...
void AIS_AnimationObject::invalidateViewer() {
    ...
    for (V3d_ListOfView::Iterator aDefViewIter = myContext->CurrentViewer()->DefinedViewIterator(); aDefViewIter.More();
         aDefViewIter.Next()) {
        ...
        if (aView->Camera()->ZFitAll(aDefViewIter.Value()->AutoZFitScaleFactor(), aMinMaxBox, aGraphicBox, aZNear,
                                     aZFar)) {
