...
AIS_AttributeFilter::AIS_AttributeFilter() : hasC(Standard_False), hasW(Standard_False) {}

AIS_AttributeFilter::AIS_AttributeFilter(const Quantity_NameOfColor aCol)
    : myCol(aCol), hasC(Standard_True), hasW(Standard_False) {}

AIS_AttributeFilter::AIS_AttributeFilter(const Standard_Real aWid)
    : myWid(aWid), hasC(Standard_False), hasW(Standard_True) {}
...
Standard_Boolean AIS_AttributeFilter::IsOk(const Handle(SelectMgr_EntityOwner) & anObj) const {
    ...
    if (aSelectable.IsNull()) return Standard_False;
    ...
    if (hasW && aSelectable->HasWidth()) okstat = (myWid == aSelectable->Width()) && okstat;
