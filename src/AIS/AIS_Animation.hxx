...
//!   AIS package provides limited number of such animation atoms - classes AIS_AnimationObject and AIS_AnimationCamera,
//!   which could be enough for defining a simple animation. In general case, application is expected defining own
//!   AIS_Animation sub-classes implementing application-specific animation logic (e.g. another interpolation or another
//!   kind of transformations - like color transition and others). The basic conception of AIS_Animation::update() is
//!   defining an exact scene state for the current presentation timestamp, providing a smooth and continuous animation
//!   well defined at any time step and in any direction. So that a time difference between two sequential drawn Viewer
//!   frames can vary from frame to frame without visual artifacts, increasing rendering framerate would not lead to
//!   animation being executed too fast and low framerate (on slow hardware) would not lead to animation played longer
//!   than defined duration. Hence, implementation should avoid usage of incremental step logic or should apply it very
//!   carefully.
//!
//! - Animation Container
...
//!   It is suggested that application would define a single AIS_Animation instance (optional sub-classing) for
//!   controlling animation playback as whole. Such controller should be filled in by other AIS_Animation as children
//!   objects, and will be managed by application by calling StartTimer(), UpdateTimer() and IsStopped() methods.
//!
//! Note, that AIS_Animation::StartTimer() defines a timer calculating an elapsed time, not a multimedia timer executing
//! Viewer updates at specific intervals! Application should avoid using implicit and immediate Viewer updates to ensure
//! that AIS_Animation::UpdateTimer() is called before each redrawing of a Viewer content. Redrawing logic should be
//! also managed at application level for managing a smooth animation (by defining a multimedia timer provided by used
//! GUI framework executing updates at desired framerate, or as continuous redraws in loop).
...
public:
    ...
    //!
    //! Note, that this method initializes a timer calculating an elapsed time (presentation timestamps within
    //! AIS_Animation::UpdateTimer()), not a multimedia timer executing Viewer updates at specific intervals! Viewer
    //! redrawing should be managed at application level, so that AIS_Animation::UpdateTimer() is called once right
    ...
    Standard_EXPORT virtual void StartTimer(const Standard_Real theStartPts, const Standard_Real thePlaySpeed,
    ...
    Standard_Real ElapsedTime() const {
        return !myTimer.IsNull() ? myTimer->ElapsedTime() : 0.0;
    }
...
public:
    ...
    bool IsStopped() {
        return myState != AnimationState_Started;
    }
