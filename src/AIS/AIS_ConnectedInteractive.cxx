...
void AIS_ConnectedInteractive::connect(const Handle(AIS_InteractiveObject) & theAnotherObj,
    ...
    if (!myReference.IsNull()) {
        if (myReference->HasInteractiveContext() &&
            ...
            myReference.Nullify();
            throw Standard_ProgramError(
                "AIS_ConnectedInteractive::Connect() - connected object should NOT be displayed in context");
        }
...
void AIS_ConnectedInteractive::ComputeSelection(const Handle(SelectMgr_Selection) & theSelection,
    ...
    for (NCollection_Vector<Handle(SelectMgr_SensitiveEntity)>::Iterator aSelEntIter(TheRefSel->Entities());
...
void AIS_ConnectedInteractive::computeSubShapeSelection(const Handle(SelectMgr_Selection) & theSelection,
    ...
    typedef NCollection_DataMap<TopoDS_Shape, SensitiveList, TopTools_OrientedShapeMapHasher> Shapes2EntitiesMap;
    ...
    for (NCollection_Vector<Handle(SelectMgr_SensitiveEntity)>::Iterator aSelEntIter(aRefSel->Entities());
         aSelEntIter.More(); aSelEntIter.Next()) {
    ...
    for (Shapes2EntitiesMap::Iterator aMapIt(aShapes2EntitiesMap); aMapIt.More(); aMapIt.Next()) {
        ...
        Handle(StdSelect_BRepOwner) anOwner =
            new StdSelect_BRepOwner(aMapIt.Key(), this, aSEList.First()->OwnerId()->Priority(), Standard_True);
