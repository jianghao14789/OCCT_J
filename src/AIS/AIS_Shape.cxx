...
AIS_Shape::AIS_Shape(const TopoDS_Shape& theShape)
    : AIS_InteractiveObject(PrsMgr_TOP_ProjectorDependent), myshape(theShape), myUVOrigin(0.0, 0.0),
      myUVRepeat(1.0, 1.0), myUVScale(1.0, 1.0), myInitAng(0.0), myCompBB(Standard_True) {
    //
}
...
// purpose  :
//=======================================================================
void AIS_Shape::Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePrs,
    ...
    switch (theMode) {
        case AIS_WireFrame: {
            ...
            try {
                ...
                Message::SendFail(
                    TCollection_AsciiString("Error: AIS_Shape::Compute() wireframe presentation builder has failed (") +
...
void AIS_Shape::computeHlrPresentation(const Handle(Graphic3d_Camera) & theProjector,
                                       const Handle(Prs3d_Presentation) & thePrs, const TopoDS_Shape& theShape,
...
void AIS_Shape::ComputeSelection(const Handle(SelectMgr_Selection) & aSelection, const Standard_Integer aMode) {
    ...
    try {
        OCC_CATCH_SIGNALS
        StdSelect_BRepSelectionTool::Load(aSelection, this, shape, TypOfSel, aDeflection, myDrawer->DeviationAngle(),
...
bool AIS_Shape::setColor(const Handle(Prs3d_Drawer) & theDrawer, const Quantity_Color& theColor) const {
...
bool AIS_Shape::setWidth(const Handle(Prs3d_Drawer) & theDrawer, const Standard_Real theLineWidth) const {
...
void AIS_Shape::UnsetWidth() {
    ...
    if (!HasColor()) {
        ...
        myDrawer->FaceBoundaryAspect()->SetWidth(
            myDrawer->HasLink() ? AIS_GraphicTool::GetLineWidth(myDrawer->Link(), AIS_TOA_FaceBoundary) : 1.);
        SynchronizeAspects();
    }
    recomputeComputed();
}
...
void AIS_Shape::setMaterial(const Handle(Prs3d_Drawer) & theDrawer, const Graphic3d_MaterialAspect& theMaterial,
                            const Standard_Boolean theToKeepColor, const Standard_Boolean theToKeepTransp) const {
...
void AIS_Shape::setTransparency(const Handle(Prs3d_Drawer) & theDrawer, const Standard_Real theValue) const {
    theDrawer->SetupOwnShadingAspect();
...
Standard_Boolean AIS_Shape::OwnDeviationAngle(Standard_Real& anAngle, Standard_Real& aPreviousAngle) const {
