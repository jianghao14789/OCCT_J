...
public: //! @name main methods
    ...
    //! Sets the texture source. <theTextureFileName> can specify path to texture image or one of the standard
    //! predefined textures. The accepted file types are those used in Image_AlienPixMap with extensions such as rgb,
    //! png, jpg and more. To specify the standard predefined texture, the <theTextureFileName> should contain integer -
    //! the Graphic3d_NameOfTexture2D enumeration index. Setting texture source using this method resets the source
    //! pixmap (if was set previously).
    ...
    //! Please note that the data should be in Bottom-Up order, the flag of Image_PixMap::IsTopDown() will be ignored by
    //! graphic driver. Setting texture source using this method resets the source by filename (if was set previously).
...
public: //! @name methods to alter texture mapping properties
    ...
    //! If other parameters (ie: scale factors, texture origin, texture repeat...) have changed, the whole presentation
    ...
    //! Sets the number of occurrences of the texture on each face. The texture itself is parameterized in (0,1) by
    //! (0,1). Each face of the shape to be textured is parameterized in UV space (Umin,Umax) by (Vmin,Vmax). If
    //! RepeatYN is set to false, texture coordinates are clamped in the range (0,1)x(0,1) of the face.
    Standard_EXPORT void SetTextureRepeat(const Standard_Boolean theToRepeat, const Standard_Real theURepeat = 1.0,
                                          const Standard_Real theVRepeat = 1.0);

    //! @return true if texture UV origin has been modified
    Standard_Boolean TextureOrigin() const {
        return myIsCustomOrigin;
    }
    ...
    //! Use this method to change the origin of the texture. The texel (0,0) will be mapped to the surface
    //! (UOrigin,VOrigin)
    Standard_EXPORT void SetTextureOrigin(const Standard_Boolean theToSetTextureOrigin,
                                          const Standard_Real theUOrigin = 0.0, const Standard_Real theVOrigin = 0.0);
    ...
    Standard_EXPORT void SetTextureScale(const Standard_Boolean theToSetTextureScale,
                                         const Standard_Real theScaleU = 1.0, const Standard_Real theScaleV = 1.0);

    //! @return true if displaying of triangles is requested
    Standard_Boolean ShowTriangles() const {
        return myToShowTriangles;
    }
