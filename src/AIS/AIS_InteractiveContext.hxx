...
//! The Interactive Context allows you to manage graphic behavior and selection of Interactive Objects in one or more
//! viewers. Class methods make this highly transparent. It is essential to remember that an Interactive Object which is
//! already known by the Interactive Context must be modified using Context methods. You can only directly call the
//! methods available for an Interactive Object if it has not been loaded into an Interactive Context.
//!
...
public: //! @name object display management
    ...
    //! The Interactive Object's default selection mode is activated if GetAutoActivateSelection() is TRUE. In general,
    //! this is 0.
    ...
    Standard_EXPORT void Display(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Integer theDispMode,
                                 const Standard_Integer theSelectionMode, const Standard_Boolean theToUpdateViewer,
    ...
    //! Hides the object. The object's presentations are simply flagged as invisible and therefore excluded from
    //! redrawing. To show hidden objects, use Display().
    Standard_EXPORT void Erase(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Boolean theToUpdateViewer);

    //! Hides all objects. The object's presentations are simply flagged as invisible and therefore excluded from
    //! redrawing. To show all hidden objects, use DisplayAll().
    ...
    //! Hides selected objects. The object's presentations are simply flagged as invisible and therefore excluded from
    //! redrawing. To show hidden objects, use Display().
    ...
    void ClearPrs(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Integer theMode,
                  const Standard_Boolean theToUpdateViewer) {
    ...
    Standard_EXPORT void Redisplay(const AIS_KindOfInteractive theTypeOfObject, const Standard_Integer theSignature,
                                   const Standard_Boolean theToUpdateViewer);
    ...
    //! Updates displayed interactive object by checking and recomputing its flagged as "to be recomputed" presentation
    //! and selection structures. This method does not force any recomputation on its own. The method recomputes
    //! selections even if they are loaded without activation in particular selector.
    Standard_EXPORT void Update(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Boolean theUpdateViewer);
...
public: //! @name highlighting management
    ...
    //! only a small set of properties derived from it's base class Graphic3d_PresentationAttributes will be actually
    //! used in most cases.
    //!
    ...
    //! @sa PrsMgr_PresentableObject::DynamicHilightAttributes() overriding Prs3d_TypeOfHighlight_Dynamic and
    //! Prs3d_TypeOfHighlight_LocalDynamic defaults on object level
    //! @sa PrsMgr_PresentableObject::HilightAttributes() overriding Prs3d_TypeOfHighlight_Selected and
    //! Prs3d_TypeOfHighlight_LocalSelected defaults on object level
...
public: //! @name object presence management (View affinity, Layer, Priority)
    ...
    //! The Z layers can be used to display temporarily presentations of some object in front of the other objects in
    //! the scene. The ids for Z layers are generated by V3d_Viewer.
    ...
    Standard_EXPORT void SetViewAffinity(const Handle(AIS_InteractiveObject) & theIObj,
                                         const Handle(V3d_View) & theView, const Standard_Boolean theIsVisible);
...
public: //! @name Display Mode management
    ...
    Standard_EXPORT void SetDisplayMode(const Standard_Integer theMode, const Standard_Boolean theToUpdateViewer);

    //! Sets the display mode of seen Interactive Objects.
    //! theMode provides the display mode index of the entity theIObj.
    Standard_EXPORT void SetDisplayMode(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Integer theMode,
                                        const Standard_Boolean theToUpdateViewer);
...
public: //! @name mouse picking logic (detection and dynamic highlighting of entities under cursor)
    ...
    //! and can be adjusted by application based on user input precision (e.g. screen pixel density, input device
    //! precision, etc.).
    ...
    //! @sa PrsMgr_PresentableObject::DynamicHilightAttributes() defining per-object dynamic highlight style of detected
    //! owners (overrides defaults)
    Standard_EXPORT AIS_StatusOfDetection MoveTo(const Standard_Integer theXPix, const Standard_Integer theYPix,
    ...
    //! If theToRedrawOnUpdate is set to false, callee should call RedrawImmediate() to highlight detected object.
    //! @sa PickingStrategy()
    Standard_EXPORT AIS_StatusOfDetection MoveTo(const gp_Ax1& theAxis, const Handle(V3d_View) & theView,
                                                 const Standard_Boolean theToRedrawOnUpdate);
    ...
    //! WARNING: Loop Method. When all the detected entities have been hilighted, the next call will hilight the first
    //! one again.
    //! @return the Rank of hilighted entity
    //! @sa HasNextDetected(), HilightPreviousDetected().
    Standard_EXPORT Standard_Integer HilightNextDetected(const Handle(V3d_View) & theView,
    ...
    Standard_EXPORT Standard_Integer HilightPreviousDetected(
        const Handle(V3d_View) & theView, const Standard_Boolean theToRedrawImmediate = Standard_True);
...
public: //! @name iteration through detected entities
    ...
    //! WARNING! This method is irrelevant to DetectedOwner() which returns last picked Owner regardless of iterator
    //! position!
...
public: //! @name Selection management
    ...
    Standard_EXPORT AIS_StatusOfPick SelectRectangle(
        const Graphic3d_Vec2i& thePntMin, const Graphic3d_Vec2i& thePntMax, const Handle(V3d_View) & theView,
    ...
    SelectPolygon(const TColgp_Array1OfPnt2d& thePolyline, const Handle(V3d_View) & theView,
                  const AIS_SelectionScheme theSelScheme = AIS_SelectionScheme_Replace);
    ...
    //! @param theSelScheme [in] selection scheme
    //! @return picking status
    Standard_EXPORT AIS_StatusOfPick SelectPoint(const Graphic3d_Vec2i& thePnt, const Handle(V3d_View) & theView,
                                                 const AIS_SelectionScheme theSelScheme = AIS_SelectionScheme_Replace);
    ...
    //! @return picking status
    //!
    //! @sa HighlightStyle() defining default highlight styles of selected owners (Prs3d_TypeOfHighlight_Selected and
    //! Prs3d_TypeOfHighlight_LocalSelected)
    //! @sa PrsMgr_PresentableObject::HilightAttributes() defining per-object highlight style of selected owners
    //! (overrides defaults)
    Standard_EXPORT AIS_StatusOfPick
    ...
    Standard_EXPORT void FitSelected(const Handle(V3d_View) & theView, const Standard_Real theMargin,
    ...
    //! Empties previous selected objects in order to get the selected objects detected by the selector using
    //! UpdateSelected.
    ...
    Handle(AIS_InteractiveObject) SelectedInteractive() const {
        return !mySelection->More() ? Handle(AIS_InteractiveObject)()
...
public: //! @name management of active Selection Modes
    ...
    //!                       default value (AIS_SelectionModesConcurrency_Multiple) means active selection modes should
    //!                       be left as is, AIS_SelectionModesConcurrency_Single can be used if only one selection mode
    //!                       is expected to be active and AIS_SelectionModesConcurrency_GlobalOrLocal can be used if
    //!                       either AIS_InteractiveObject::GlobalSelectionMode() or any combination of Local selection
    //!                       modes is acceptable; this value is considered only if theToActivate set to TRUE
    //! @param theIsForce     when set to TRUE, the display status will be ignored while activating selection mode
    Standard_EXPORT void
    SetSelectionModeActive(const Handle(AIS_InteractiveObject) & theObj, const Standard_Integer theMode,
    ...
    void Activate(const Handle(AIS_InteractiveObject) & theObj, const Standard_Integer theMode = 0,
                  const Standard_Boolean theIsForce = Standard_False) {
    ...
    Standard_EXPORT void Activate(const Standard_Integer theMode, const Standard_Boolean theIsForce = Standard_False);
    ...
    //! Deactivates all the activated selection modes of the interactive object anIobj with a given selection mode
    //! aMode.
    ...
    //! Returns a collection containing all entity owners created for the interactive object in specified selection mode
    //! (in all active modes if the Mode == -1)
...
public: //! @name Selection Filters management
    ...
    SelectMgr_PickingStrategy PickingStrategy() const {
        return myPickingStrategy;
    }

    //! Setup picking strategy - which entities detected by picking line will be accepted, considering Selection
    //! Filters. By default (SelectMgr_PickingStrategy_FirstAcceptable), Selection Filters reduce the list of entities
...
public: //! @name common properties
    ...
    //! This contains all the color and line attributes which can be used by interactive objects which do not have their
    //! own attributes.
    ...
    //! Returns the list theListOfIO of erased objects (hidden objects) particular Type WhichKind and Signature
    //! WhichSignature. By Default, WhichSignature equals 1. This means that there is a check on type only.
    ...
    //! Returns the list theListOfIO of objects with indicated display status particular Type WhichKind and Signature
    //! WhichSignature. By Default, WhichSignature equals 1. This means that there is a check on type only.
    ...
    Standard_EXPORT void ObjectsForView(AIS_ListOfInteractive& theListOfIO, const Handle(V3d_View) & theView,
...
public: //! @name common object display attributes
    ...
    //! change the current facing model apply on polygons for SetColor(), SetTransparency(), SetMaterial() methods
    //! default facing model is Aspect_TOFM_TWO_SIDE. This mean that attributes is applying both on the front and back
    //! face.
    Standard_EXPORT void SetCurrentFacingModel(const Handle(AIS_InteractiveObject) & aniobj,
    ...
    Standard_EXPORT void SetColor(const Handle(AIS_InteractiveObject) & theIObj, const Quantity_Color& theColor,
                                  const Standard_Boolean theToUpdateViewer);
    ...
    Standard_EXPORT virtual void SetWidth(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Real theValue,
                                          const Standard_Boolean theToUpdateViewer);
    ...
    Standard_EXPORT void SetTransparency(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Real theValue,
                                         const Standard_Boolean theToUpdateViewer);
    ...
    Standard_EXPORT void SetPolygonOffsets(const Handle(AIS_InteractiveObject) & theIObj,
                                           const Standard_Integer theMode, const Standard_ShortReal theFactor,
    ...
    Standard_EXPORT void PolygonOffsets(const Handle(AIS_InteractiveObject) & anObj, Standard_Integer& aMode,
                                        Standard_ShortReal& aFactor, Standard_ShortReal& aUnits) const;
...
public: //! @name trihedron display attributes
    ...
    //! Use of this function in one of your own interactive objects requires a call to the Compute function of the new
    //! class. This will recalculate the presentation for every trihedron displayed.
    Standard_EXPORT void SetTrihedronSize(const Standard_Real theSize, const Standard_Boolean theToUpdateViewer);
...
public: //! @name plane display attributes
    ...
    Standard_EXPORT void SetPlaneSize(const Standard_Real theSizeX, const Standard_Real theSizeY,
                                      const Standard_Boolean theToUpdateViewer);

    //! Sets the plane size aSize.
    Standard_EXPORT void SetPlaneSize(const Standard_Real theSize, const Standard_Boolean theToUpdateViewer);
...
public: //! @name tessellation deviation properties for automatic triangulation
    ...
    //! The deviation coefficient theCoefficient gives the highest value of the angle with which a chord can deviate
    //! from a tangent to a curve. If this limit is reached, a new triangle is begun. This deviation is absolute and is
    //! set through the method: SetMaximalChordialDeviation. The default value is 0.001. In drawing shapes, however, you
    //! are allowed to ask for a relative deviation. This deviation will be: SizeOfObject * DeviationCoefficient.
    ...
    Standard_EXPORT void SetDeviationAngle(const Handle(AIS_InteractiveObject) & theIObj, const Standard_Real theAngle,
                                           const Standard_Boolean theToUpdateViewer);
    ...
    Standard_EXPORT void SetAngleAndDeviation(const Handle(AIS_InteractiveObject) & theIObj,
                                              const Standard_Real theAngle, const Standard_Boolean theToUpdateViewer);
    ...
    //! These are used to calculate reflection of light from the surface of the object.
    //! The triangles are formed from chords of the curves in the shape.
    //! The deviation coefficient gives the highest value of the angle with which a chord can deviate from a tangent to
    //! a curve. If this limit is reached, a new triangle is begun. This deviation is absolute and is set through
    //! Prs3d_Drawer::SetMaximalChordialDeviation. The default value is 0.001. In drawing shapes, however, you are
    //! allowed to ask for a relative deviation. This deviation will be: SizeOfObject * DeviationCoefficient.
...
public:
    Standard_DEPRECATED("Deprecated method Display() with obsolete argument theToAllowDecomposition") void Display(
        const Handle(AIS_InteractiveObject) & theIObj, const Standard_Integer theDispMode,
        const Standard_Integer theSelectionMode, const Standard_Boolean theToUpdateViewer,
    ...
    Standard_DEPRECATED("Deprecated method Load() with obsolete last argument theToAllowDecomposition") void Load(
    ...
    //! Selects everything found in the bounding rectangle defined by the pixel minima and maxima, XPMin, YPMin, XPMax,
    //! and YPMax in the view. The objects detected are passed to the main viewer, which is then updated.
    Standard_DEPRECATED(
        "This method is deprecated - SelectRectangle() taking AIS_SelectionScheme_Replace should be called instead")
        Standard_EXPORT AIS_StatusOfPick
        Select(const Standard_Integer theXPMin, const Standard_Integer theYPMin, const Standard_Integer theXPMax,
               const Standard_Integer theYPMax, const Handle(V3d_View) & theView,
               const Standard_Boolean theToUpdateViewer);
    ...
    Standard_DEPRECATED(
        ...
        Standard_EXPORT AIS_StatusOfPick
        Select(const TColgp_Array1OfPnt2d& thePolyline, const Handle(V3d_View) & theView,
               const Standard_Boolean theToUpdateViewer);
    ...
    Standard_DEPRECATED(
        ...
        Standard_EXPORT AIS_StatusOfPick
        ShiftSelect(const TColgp_Array1OfPnt2d& thePolyline, const Handle(V3d_View) & theView,
                    const Standard_Boolean theToUpdateViewer);
    ...
    Standard_DEPRECATED(
        ...
        Standard_EXPORT AIS_StatusOfPick
        ShiftSelect(const Standard_Integer theXPMin, const Standard_Integer theYPMin, const Standard_Integer theXPMax,
                    const Standard_Integer theYPMax, const Handle(V3d_View) & theView,
                    const Standard_Boolean theToUpdateViewer);

public:
    //! Updates the view of the current object in open context.
    //! Objects selected when there is no open local context are called current objects; those selected in open local
    //! context, selected objects.
    Standard_DEPRECATED("Local Context is deprecated - local selection should be used without Local "
                        "Context") void SetCurrentObject(const Handle(AIS_InteractiveObject) & theIObj,
                                                         const Standard_Boolean theToUpdateViewer) {
    ...
    Standard_DEPRECATED("Local Context is deprecated - local selection should be used without Local "
                        "Context") void AddOrRemoveCurrentObject(const Handle(AIS_InteractiveObject) & theObj,
                                                                 const Standard_Boolean theIsToUpdateViewer) {
        AddOrRemoveSelected(theObj, theIsToUpdateViewer);
    }

    //! Updates the list of current objects, i.e. hilights new current objects, removes hilighting from former current
    //! objects. Objects selected when there is no open local context are called current objects; those selected in open
    //! local context, selected objects.
    Standard_DEPRECATED(
        "Local Context is deprecated - local selection should be used without Local Context") void UpdateCurrent() {
    ...
    //! context, selected objects.
    Standard_DEPRECATED(
        "Local Context is deprecated - local selection should be used without Local Context") void InitCurrent() {
        InitSelected();
    }
    ...
    //! context, selected objects.
    Standard_DEPRECATED(
        "Local Context is deprecated - local selection should be used without Local Context") void NextCurrent() {
        NextSelected();
    }
    ...
    //! context, selected objects.
    Standard_DEPRECATED("Local Context is deprecated - local selection should be used without Local "
                        "Context") void HilightCurrents(const Standard_Boolean theToUpdateViewer) {
        HilightSelected(theToUpdateViewer);
    }
    ...
    //! context, selected objects.
    Standard_DEPRECATED("Local Context is deprecated - local selection should be used without Local "
                        "Context") void UnhilightCurrents(const Standard_Boolean theToUpdateViewer) {
        UnhilightSelected(theToUpdateViewer);
    }

    //! Empties previous current objects in order to get the current objects detected by the selector using
    //! UpdateCurrent. Objects selected when there is no open local context are called current objects; those selected
    //! in open local context, selected objects.
    Standard_DEPRECATED("Local Context is deprecated - local selection should be used without Local "
                        "Context") void ClearCurrents(const Standard_Boolean theToUpdateViewer) {
...
public: //! @name sub-intensity management (deprecated)
    //! Sub-intensity allows temporary highlighting of particular objects with specified color in a manner of selection
    //! highlight, but without actual selection (e.g., global status and owner's selection state will not be updated).
    ...
    //! Highlights, and removes highlights from, the displayed object which is displayed at Neutral Point with
    //! subintensity color. Available only for active local context. There is no effect if there is no local context. If
    //! a local context is open, the presentation of the Interactive Object activates the selection mode.
...
protected: //! @name internal methods
    Standard_EXPORT void GetDefModes(const Handle(AIS_InteractiveObject) & anIobj, Standard_Integer& Dmode,
                                     Standard_Integer& HiMod, Standard_Integer& SelMode) const;
    ...
    Standard_EXPORT void ClearGlobalPrs(const Handle(AIS_InteractiveObject) & theObj, const Standard_Integer theMode,
                                        const Standard_Boolean theToUpdateViewer);
    ...
    Standard_EXPORT void highlightGlobal(const Handle(AIS_InteractiveObject) & theObj,
                                         const Handle(Prs3d_Drawer) & theStyle, const Standard_Integer theDispMode);
    ...
    const Handle(Prs3d_Drawer) &
        getHiStyle(const Handle(AIS_InteractiveObject) & theObj, const Handle(SelectMgr_EntityOwner) & theOwner) const {
        const Handle(Prs3d_Drawer) & aHiDrawer = theObj->DynamicHilightAttributes();
        if (!aHiDrawer.IsNull()) {
            return aHiDrawer;
        }

        return myStyles[!theOwner.IsNull() && theOwner->ComesFromDecomposition() ? Prs3d_TypeOfHighlight_LocalDynamic
                                                                                 : Prs3d_TypeOfHighlight_Dynamic];
    }
    ...
    const Handle(Prs3d_Drawer) & getSelStyle(const Handle(AIS_InteractiveObject) & theObj,
        ...
        return myStyles[!theOwner.IsNull() && theOwner->ComesFromDecomposition() ? Prs3d_TypeOfHighlight_LocalSelected
                                                                                 : Prs3d_TypeOfHighlight_Selected];
    }
    ...
    Standard_Integer getHilightMode(const Handle(AIS_InteractiveObject) & theObj, const Handle(Prs3d_Drawer) & theStyle,
                                    const Standard_Integer theDispMode) const {
        if (!theStyle.IsNull() && theStyle->DisplayMode() != -1 && theObj->AcceptDisplayMode(theStyle->DisplayMode())) {
    ...
    Standard_EXPORT void setObjectStatus(const Handle(AIS_InteractiveObject) & theIObj,
                                         const PrsMgr_DisplayStatus theStatus, const Standard_Integer theDispyMode,
