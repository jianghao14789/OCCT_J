...
AIS_Animation::AIS_Animation(const TCollection_AsciiString& theAnimationName)
    : myName(theAnimationName), myState(AnimationState_Stopped), myPtsStart(0.0), myOwnDuration(0.0),
...
void AIS_Animation::CopyFrom(const Handle(AIS_Animation) & theOther) {
    myAnimations.Clear();
    for (NCollection_Sequence<Handle(AIS_Animation)>::Iterator anIter(theOther->myAnimations); anIter.More();
         anIter.Next()) {
...
// purpose  :
//=============================================================================
void AIS_Animation::StartTimer(const Standard_Real theStartPts, const Standard_Real thePlaySpeed,
                               const Standard_Boolean theToUpdate, const Standard_Boolean theToStopTimer) {
    if (myTimer.IsNull()) {
...
void AIS_Animation::updateWithChildren(const AIS_AnimationProgress& thePosition) {
    ...
    for (NCollection_Sequence<Handle(AIS_Animation)>::Iterator anIter(myAnimations); anIter.More(); anIter.Next()) {
        ...
        aPosition.LocalNormalized = anAnim->HasOwnDuration() ? (aPosition.LocalPts / anAnim->OwnDuration()) : 0.0;
        aPosition.LocalNormalized = Max(0.0, aPosition.LocalNormalized);
        aPosition.LocalNormalized = Min(1.0, aPosition.LocalNormalized);
