...
AIS_ViewController::AIS_ViewController()
    : myLastEventsTime(0.0), myToAskNextFrame(false), myIsContinuousRedraw(false), myMinCamDistance(1.0),
      myRotationMode(AIS_RotationMode_BndBoxActive), myNavigationMode(AIS_NavigationMode_Orbit), myMouseAccel(1.0f),
      myOrbitAccel(1.0f), myToShowPanAnchorPoint(true), myToShowRotateCenter(true), myToLockOrbitZUp(false),
      myToInvertPitch(false), myToAllowTouchZRotation(false), myToAllowRotation(true), myToAllowPanning(true),
      myToAllowZooming(true), myToAllowZFocus(true), myToAllowHighlight(true), myToAllowDragging(true),
      myToStickToRayOnZoom(true), myToStickToRayOnRotation(true),
      //
      myWalkSpeedAbsolute(1.5f), myWalkSpeedRelative(0.1f), myThrustSpeed(0.0f), myHasThrust(false),
      //
      myViewAnimation(new AIS_AnimationCamera("AIS_ViewController_ViewAnimation", Handle(V3d_View)())),
      myObjAnimation(new AIS_Animation("AIS_ViewController_ObjectsAnimation")), myToPauseObjAnimation(false),
      myPrevMoveTo(-1, -1), myHasHlrOnBeforeRotation(false),
      //
      myXRPrsDevices(0, 0), myXRLaserTeleColor(Quantity_NOC_GREEN), myXRLaserPickColor(Quantity_NOC_BLUE),
      myXRLastTeleportHand(Aspect_XRTrackedDeviceRole_Other), myXRLastPickingHand(Aspect_XRTrackedDeviceRole_Other),
      myXRLastPickDepthLeft(Precision::Infinite()), myXRLastPickDepthRight(Precision::Infinite()),
      myXRTurnAngle(M_PI_4), myToDisplayXRAuxDevices(false), myToDisplayXRHands(true),
      //
      myMouseClickThreshold(3.0), myMouseDoubleClickInt(0.4), myScrollZoomRatio(15.0f),
      myMouseActiveGesture(AIS_MouseGesture_NONE), myMouseActiveIdleRotation(false), myMouseClickCounter(0),
      myMouseSingleButton(-1), myMouseStopDragOnUnclick(false),
      //
      myTouchToleranceScale(1.0f), myTouchClickThresholdPx(3.0f), myTouchRotationThresholdPx(6.0f),
      myTouchZRotationThreshold(float(2.0 * M_PI / 180.0)), myTouchPanThresholdPx(4.0f), myTouchZoomThresholdPx(6.0f),
      myTouchZoomRatio(0.13f),
      //
      myNbTouchesLast(0), myUpdateStartPointPan(true), myUpdateStartPointRot(true), myUpdateStartPointZRot(true),
      //
    ...
    myMouseGestureMap.Bind(Aspect_VKeyMouse_LeftButton | Aspect_VKeyFlags_ALT | Aspect_VKeyFlags_SHIFT,
    ...
    myMouseSelectionSchemes.Bind(Aspect_VKeyMouse_LeftButton | Aspect_VKeyFlags_ALT | Aspect_VKeyFlags_SHIFT,
...
void AIS_ViewController::FlushViewEvents(const Handle(AIS_InteractiveContext) & theCtx,
                                         const Handle(V3d_View) & theView, Standard_Boolean theToHandle) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::flushBuffers(const Handle(AIS_InteractiveContext) &, const Handle(V3d_View) &) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::flushGestures(const Handle(AIS_InteractiveContext) &, const Handle(V3d_View) & theView) {
    ...
    } else if (aTouchNb == 2) // pinch
    {
        ...
        Graphic3d_Vec2d aPinchCenterStart((aFrom[0].x() + aFrom[1].x()) / 2.0, (aFrom[0].y() + aFrom[1].y()) / 2.0);
...
// purpose  :
// =======================================================================
void AIS_ViewController::UpdateViewOrientation(V3d_TypeOfOrientation theOrientation, bool theToFitAll) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::SelectInViewer(const Graphic3d_Vec2i& thePnt, const AIS_SelectionScheme theScheme) {
    if (myUI.Selection.Tool != AIS_ViewSelectionTool_Picking) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::UpdateRubberBand(const Graphic3d_Vec2i& thePntFrom, const Graphic3d_Vec2i& thePntTo) {
    myUI.Selection.Tool = AIS_ViewSelectionTool_RubberBand;
    myUI.Selection.Points.Clear();
...
// purpose  :
// =======================================================================
bool AIS_ViewController::UpdateMouseClick(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButton,
                                          Aspect_VKeyFlags theModifiers, bool theIsDoubleClick) {
...
// purpose  :
// =======================================================================
bool AIS_ViewController::UpdateMouseButtons(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButtons,
    ...
    if (theButtons == Aspect_VKeyMouse_NONE && myMouseSingleButton > 0) {
        ...
        if (double(aDelta.cwiseAbs().maxComp()) < aTolClick) {
            ...
            toUpdateView =
                UpdateMouseClick(thePoint, (Aspect_VKeyMouse)myMouseSingleButton, theModifiers, isDoubleClick) ||
                toUpdateView;
        } else {
            myMouseClickTimer.Stop();
            myMouseClickCounter = 0;
    ...
    if (theIsEmulated || myNavigationMode != AIS_NavigationMode_FirstPersonWalk) {
        ...
        if (theButtons == Aspect_VKeyMouse_LeftButton && theModifiers == Aspect_VKeyFlags_NONE && myToAllowDragging) {
    ...
    if (aPrevGesture != myMouseActiveGesture) {
        if (aPrevGesture == AIS_MouseGesture_SelectRectangle || aPrevGesture == AIS_MouseGesture_SelectLasso ||
...
// purpose  :
// =======================================================================
bool AIS_ViewController::UpdateMousePosition(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButtons,
                                             Aspect_VKeyFlags theModifiers, bool theIsEmulated) {
    ...
    if (myMouseModifiers != theModifiers && UpdateMouseButtons(thePoint, theButtons, theModifiers, theIsEmulated)) {
        toUpdateView = true;
    }
    ...
    switch (myMouseActiveGesture) {
        ...
        case AIS_MouseGesture_RotateView: {
            ...
            const double aRotTol = theIsEmulated ? double(myTouchToleranceScale) * myTouchRotationThresholdPx : 0.0;
        ...
        case AIS_MouseGesture_Zoom: {
            ...
            const double aZoomTol = theIsEmulated ? double(myTouchToleranceScale) * myTouchZoomThresholdPx : 0.0;
        ...
        case AIS_MouseGesture_Pan: {
            ...
            const double aPanTol = theIsEmulated ? double(myTouchToleranceScale) * myTouchPanThresholdPx : 0.0;
...
// purpose  :
// =======================================================================
void AIS_ViewController::AddTouchPoint(Standard_Size theId, const Graphic3d_Vec2d& thePnt,
...
bool AIS_ViewController::RemoveTouchPoint(Standard_Size theId, Standard_Boolean theClearSelectPnts) {
    ...
    if (myTouchPoints.Extent() == 1) {
        ...
        if (theId == (Standard_Size)-1) {
            ...
            if (myTouchDoubleTapTimer.IsStarted() && myTouchDoubleTapTimer.ElapsedTime() <= myMouseDoubleClickInt) {
                isDoubleClick = true;
            } else {
            ...
            // emulate mouse click
            UpdateMouseClick(Graphic3d_Vec2i(myTouchClick.From), Aspect_VKeyMouse_LeftButton, Aspect_VKeyFlags_NONE,
                             isDoubleClick);
        }
    }
    myUI.IsNewGesture = true;
...
// purpose  :
// =======================================================================
void AIS_ViewController::KeyFromAxis(Aspect_VKey theNegative, Aspect_VKey thePositive, double theTime,
                                     double thePressure) {
...
// purpose  :
// =======================================================================
AIS_WalkDelta AIS_ViewController::FetchNavigationKeys(Standard_Real theCrouchRatio, Standard_Real theRunRatio) {
    AIS_WalkDelta aWalk;
    ...
    const double aRunRatio = aWalk.IsRunning() ? theRunRatio : aWalk.IsCrouching() ? theCrouchRatio : 1.0;
...
void AIS_ViewController::handlePanning(const Handle(V3d_View) & theView) {
    ...
    const gp_Pnt aViewDims =
        aCam->ViewDimensions(anEyeToPnt.Dot(aCam->Direction().XYZ())); // view dimensions at 3D point
    ...
    gp_Trsf aPanTrsf;
    const gp_Vec aCameraPan = gp_Vec(aCameraCS.XDirection()) * aDxy.x() + gp_Vec(aCameraCS.YDirection()) * aDxy.y();
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleZoom(const Handle(V3d_View) & theView, const Aspect_ScrollDelta& theParams,
                                    const gp_Pnt* thePnt) {
    if (!myToAllowZooming) {
        return;
    }
    ...
    theView->Convert(theParams.Point.x(), theParams.Point.y(), aZoomAtPointXYv.x(), aZoomAtPointXYv.y());
    ...
    if (thePnt != NULL) {
        ...
        aDxy.SetValues(anEyeToPnt.Dot(aCameraCS.XDirection().XYZ()), anEyeToPnt.Dot(aCameraCS.YDirection().XYZ()));
        ...
        const Graphic3d_Vec2d aPanFromCenterPx(double(theParams.Point.x()) - 0.5 * double(aWinSize.x()),
                                               double(aWinSize.y() - theParams.Point.y() - 1) -
                                                   0.5 * double(aWinSize.y()));
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleZFocusScroll(const Handle(V3d_View) & theView, const Aspect_ScrollDelta& theParams) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleOrbitRotation(const Handle(V3d_View) & theView, const gp_Pnt& thePnt,
                                             bool theToLockZUp) {
    if (!myToAllowRotation) {
        return;
    }
    ...
    if (myGL.OrbitRotation.ToStart) {
        ...
        // if (myRotationMode == AIS_RotationMode_BndBoxActive) theView->StartRotation (myGL.RotateAtPoint.x(),
        // myGL.RotateAtPoint.y()); theView->Rotate (0.0, 0.0, 0.0, thePnt.X(), thePnt.Y(), thePnt.Z(), true);
        ...
        aRot.GetEulerAngles(gp_YawPitchRoll, myRotateStartYawPitchRoll[0], myRotateStartYawPitchRoll[1],
    ...
    if (theToLockZUp) {
        ...
        double aYawAngleDelta =
            ((myGL.OrbitRotation.PointStart.x() - myGL.OrbitRotation.PointTo.x()) / double(aWinXY.x())) * (M_PI * 0.5);
        double aPitchAngleDelta =
            -((myGL.OrbitRotation.PointStart.y() - myGL.OrbitRotation.PointTo.y()) / double(aWinXY.y())) * (M_PI * 0.5);
        ...
        if (!theView->View()->IsActiveXR()) {
            aPitchAngleNew = Max(Min(myRotateStartYawPitchRoll[1] + aPitchAngleDelta, M_PI * 0.5 - M_PI / 180.0),
                                 -M_PI * 0.5 + M_PI / 180.0);
            aRoll = 0.0;
        }
        ...
        // default alternatives
        // if (myRotationMode == AIS_RotationMode_BndBoxActive) theView->Rotation (myGL.RotateToPoint.x(),
        // myGL.RotateToPoint.y()); theView->Rotate (aDX, aDY, aDZ, myRotatePnt3d.X(), myRotatePnt3d.Y(),
        // myRotatePnt3d.Z(), false);
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleViewRotation(const Handle(V3d_View) & theView, double theYawExtra, double thePitchExtra,
                                            double theRoll, bool theToRestartOnIncrement) {
    if (!myToAllowRotation) {
        return;
    }

    const Handle(Graphic3d_Camera) & aCam = theView->Camera();
    const bool toRotateAnyway = Abs(theYawExtra) > gp::Resolution() || Abs(thePitchExtra) > gp::Resolution() ||
    ...
    theView->Window()->Size(aWinXY.x(), aWinXY.y());
    double aYawAngleDelta =
        ((myGL.ViewRotation.PointStart.x() - myGL.ViewRotation.PointTo.x()) / double(aWinXY.x())) * (M_PI * 0.5);
    double aPitchAngleDelta =
        -((myGL.ViewRotation.PointStart.y() - myGL.ViewRotation.PointTo.y()) / double(aWinXY.y())) * (M_PI * 0.5);
    const double aPitchAngleNew = Max(Min(myRotateStartYawPitchRoll[1] + aPitchAngleDelta, M_PI * 0.5 - M_PI / 180.0),
                                      -M_PI * 0.5 + M_PI / 180.0);
    const double aYawAngleNew = myRotateStartYawPitchRoll[0] + aYawAngleDelta;
    gp_Quaternion aRot;
...
// purpose  :
// =======================================================================
bool AIS_ViewController::PickPoint(gp_Pnt& thePnt, const Handle(AIS_InteractiveContext) & theCtx,
                                   const Handle(V3d_View) & theView, const Graphic3d_Vec2i& theCursor,
...
// purpose  :
// =======================================================================
bool AIS_ViewController::PickAxis(gp_Pnt& theTopPnt, const Handle(AIS_InteractiveContext) & theCtx,
                                  const Handle(V3d_View) & theView, const gp_Ax1& theAxis) {
    ResetPreviousMoveTo();
...
// purpose  :
// =======================================================================
void AIS_ViewController::FitAllAuto(const Handle(AIS_InteractiveContext) & theCtx, const Handle(V3d_View) & theView) {
    ...
    if (aCameraSel->Center().IsEqual(aCam->Center(), aFitTol) && Abs(aCameraSel->Scale() - aCam->Scale()) < aFitTol &&
...
void AIS_ViewController::handleViewOrientationKeys(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    static const ViewKeyAction THE_VIEW_KEYS[] = {{Aspect_VKey_ViewTop, V3d_TypeOfOrientation_Zup_Top},
...
AIS_WalkDelta AIS_ViewController::handleNavigationKeys(const Handle(AIS_InteractiveContext) &,
    ...
    const double aWalkSpeed =
        myNavigationMode != AIS_NavigationMode_Orbit && myNavigationMode != AIS_NavigationMode_FirstPersonFlight
    ...
    if (aWalk.ToMove() && myToAllowPanning) {
        ...
        if (!aWalk[AIS_WalkTranslation_Side].IsEmpty()) {
            aMoveVec += aSide.XYZ() * aWalk[AIS_WalkTranslation_Side].Value * aWalk[AIS_WalkTranslation_Side].Pressure *
                        aWalkSpeed;
        }
        if (!aWalk[AIS_WalkTranslation_Up].IsEmpty()) {
            aMoveVec +=
                anUp.XYZ() * aWalk[AIS_WalkTranslation_Up].Value * aWalk[AIS_WalkTranslation_Up].Pressure * aWalkSpeed;
        }
        {
            if (aCam->IsOrthographic()) {
                if (!aWalk[AIS_WalkTranslation_Forward].IsEmpty()) {
...
void AIS_ViewController::handleCameraActions(const Handle(AIS_InteractiveContext) & theCtx,
                                             const Handle(V3d_View) & theView, const AIS_WalkDelta& theWalk) {
    // apply view actions
    ...
    if (myNavigationMode != AIS_NavigationMode_FirstPersonWalk) {
        ...
        if (myToShowPanAnchorPoint && hasPanningAnchorPoint() && myGL.Panning.ToPan && !myGL.IsNewGesture &&
    ...
    if ((myNavigationMode == AIS_NavigationMode_Orbit || myGL.OrbitRotation.ToStart || myGL.OrbitRotation.ToRotate) &&
        myToAllowRotation) {
        ...
        handleOrbitRotation(theView, aGravPnt, myToLockOrbitZUp || myNavigationMode != AIS_NavigationMode_Orbit);
    }

    if ((myNavigationMode != AIS_NavigationMode_Orbit || myGL.ViewRotation.ToStart || myGL.ViewRotation.ToRotate) &&
        myToAllowRotation) {
    ...
    if (!myGL.ZoomActions.IsEmpty()) {
        for (NCollection_Sequence<Aspect_ScrollDelta>::Iterator aZoomIter(myGL.ZoomActions); aZoomIter.More();
             aZoomIter.Next()) {
            Aspect_ScrollDelta aZoomParams = aZoomIter.Value();
            if (myToAllowZFocus && (aZoomParams.Flags & Aspect_VKeyFlags_CTRL) != 0 && theView->Camera()->IsStereo()) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleXRInput(const Handle(AIS_InteractiveContext) & theCtx, const Handle(V3d_View) & theView,
                                       const AIS_WalkDelta&) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleXRTurnPad(const Handle(AIS_InteractiveContext) &, const Handle(V3d_View) & theView) {
    ...
    for (int aHand = 0; aHand < 2; ++aHand) {
        ...
        const Handle(Aspect_XRAction) & aPadPosAct =
            theView->View()->XRSession()->GenericAction(aRole, Aspect_XRGenericAction_InputTrackPadPosition);
...
void AIS_ViewController::handleXRTeleport(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    for (int aHand = 0; aHand < 2; ++aHand) {
        ...
        if (aPadClick.IsActive && (isPressed || isClicked) && aPadPos.IsActive && aPadPos.VecXYZ.y() > 0.6f &&
            ...
            Standard_Real& aPickDepth =
                aRole == Aspect_XRTrackedDeviceRole_LeftHand ? myXRLastPickDepthLeft : myXRLastPickDepthRight;
            ...
            if (isClicked) {
                ...
                if (!Precision::IsInfinite(aPickDepth)) {
                    ...
                    if (isHorizontal) {
                        aNewEye =
                            aHandBase.TranslationPart() + aTeleDir.XYZ() * aPickDepth + anUpDir.XYZ() * aHeadHeight;
                    } else {
    ...
    if (myXRLastTeleportHand != aTeleOld) {
        if (aTeleOld != Aspect_XRTrackedDeviceRole_Other) {
            if (const Handle(Aspect_XRAction) & aHaptic =
                    theView->View()->XRSession()->GenericAction(aTeleOld, Aspect_XRGenericAction_OutputHaptic)) {
                theView->View()->XRSession()->AbortHapticVibrationAction(aHaptic);
            }
        }
        if (myXRLastTeleportHand != Aspect_XRTrackedDeviceRole_Other) {
            if (const Handle(Aspect_XRAction) & aHaptic = theView->View()->XRSession()->GenericAction(
                    myXRLastTeleportHand, Aspect_XRGenericAction_OutputHaptic)) {
    ...
    for (int aHand = 0; aHand < 2; ++aHand) {
        ...
        if (aTrigPos.IsActive && Abs(aTrigPos.VecXYZ.x()) > 0.1f) {
            ...
            if (aTrigClick.IsActive && aTrigClick.IsPressed && aTrigClick.IsChanged) {
                ...
                if (const Handle(Aspect_XRAction) & aHaptic = theView->View()->XRSession()->GenericAction(
                        myXRLastPickingHand, Aspect_XRGenericAction_OutputHaptic)) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::OnSelectionChanged(const Handle(AIS_InteractiveContext) &, const Handle(V3d_View) &) {
    //
}
...
void AIS_ViewController::OnObjectDragged(const Handle(AIS_InteractiveContext) & theCtx,
                                         const Handle(V3d_View) & theView, AIS_DragAction theAction) {
    switch (theAction) {
        ...
        case AIS_DragAction_Update: {
            ...
            myDragObject->ProcessDragging(theCtx, theView, myDragOwner, myGL.Dragging.PointStart, myGL.Dragging.PointTo,
                                          theAction);
            theView->Invalidate();
            return;
        }
...
void AIS_ViewController::contextLazyMoveTo(const Handle(AIS_InteractiveContext) & theCtx,
                                           const Handle(V3d_View) & theView, const Graphic3d_Vec2i& thePnt) {
    if (myPrevMoveTo == thePnt || myHasHlrOnBeforeRotation) // ignore highlighting in-between rotation of HLR view
    {
        return;
    }
    ...
    if (aLastPicked != aNewPicked || (!aNewPicked.IsNull() && aNewPicked->IsForcedHilight())) {
        ...
        for (V3d_ListOfViewIterator aViewIter(theView->Viewer()->ActiveViewIterator()); aViewIter.More();
             aViewIter.Next()) {
...
void AIS_ViewController::handleSelectionPick(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    if (myGL.Selection.Tool == AIS_ViewSelectionTool_Picking && !myGL.Selection.Points.IsEmpty()) {
        for (NCollection_Sequence<Graphic3d_Vec2i>::Iterator aPntIter(myGL.Selection.Points); aPntIter.More();
             aPntIter.Next()) {
    ...
    if (myGL.Selection.Tool == AIS_ViewSelectionTool_RubberBand ||
        ...
        if (!myGL.Selection.Points.IsEmpty()) {
            ...
            if (anIsRubber) {
                ...
                for (NCollection_Sequence<Graphic3d_Vec2i>::Iterator aSelIter(myGL.Selection.Points); aSelIter.More();
                     aSelIter.Next()) {
        ...
        if (theCtx->IsDisplayed(myRubberBand)) {
            theCtx->Remove(myRubberBand, false);
            {
                ...
                if (aPoints.Size() == 4 && aPoints.Value(1).x() == aPoints.Value(2).x() &&
                    aPoints.Value(3).x() == aPoints.Value(4).x() && aPoints.Value(1).y() == aPoints.Value(4).y() &&
...
void AIS_ViewController::handleDynamicHighlight(const Handle(AIS_InteractiveContext) & theCtx,
                                                const Handle(V3d_View) & theView) {
    if ((myGL.MoveTo.ToHilight || myGL.Dragging.ToStart) && myNavigationMode != AIS_NavigationMode_FirstPersonWalk) {
        const Graphic3d_Vec2i& aMoveToPnt = myGL.MoveTo.ToHilight ? myGL.MoveTo.Point : myGL.Dragging.PointStart;
        if (myGL.Dragging.ToStart && (!myGL.MoveTo.ToHilight || !myToAllowHighlight) && !HasPreviousMoveTo()) {
            ...
            if (myPrevMoveTo != aMoveToPnt ||
                (!theView->View()->IsActiveXR() &&
                 (myGL.OrbitRotation.ToRotate || myGL.ViewRotation.ToRotate || theView->IsInvalidated()))) {
                ResetPreviousMoveTo();
                contextLazyMoveTo(theCtx, theView, aMoveToPnt);
            }
    ...
    if (!myDragObject.IsNull()) {
        if (myGL.Dragging.ToAbort) {
            ...
            myGL.OrbitRotation.ToRotate = false;
            myGL.ViewRotation.ToRotate = false;
        } else if (myGL.OrbitRotation.ToRotate || myGL.ViewRotation.ToRotate) {
            OnObjectDragged(theCtx, theView, AIS_DragAction_Update);
            myGL.OrbitRotation.ToRotate = false;
            myGL.ViewRotation.ToRotate = false;
        }
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleMoveTo(const Handle(AIS_InteractiveContext) & theCtx, const Handle(V3d_View) & theView) {
...
// purpose  :
// =======================================================================
void AIS_ViewController::handleViewRedraw(const Handle(AIS_InteractiveContext) &, const Handle(V3d_View) & theView) {
...
Standard_Integer AIS_ViewController::handleXRMoveTo(const Handle(AIS_InteractiveContext) & theCtx,
                                                    const Handle(V3d_View) & theView, const gp_Trsf& thePose,
...
void AIS_ViewController::handleXRHighlight(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    if (!theCtx->DetectedOwner().IsNull() && theCtx->DetectedOwner() != aDetOld) {
        if (const Handle(Aspect_XRAction) & aHaptic =
                theView->View()->XRSession()->GenericAction(myXRLastPickingHand, Aspect_XRGenericAction_OutputHaptic)) {
            theView->View()->XRSession()->TriggerHapticVibrationAction(aHaptic, myXRPickingHaptic);
        }
    }

    Standard_Real& aPickDepth =
        myXRLastPickingHand == Aspect_XRTrackedDeviceRole_LeftHand ? myXRLastPickDepthLeft : myXRLastPickDepthRight;
    aPickDepth = Precision::Infinite();
...
void AIS_ViewController::handleXRPresentations(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    if (!theView->View()->IsActiveXR() || (!myToDisplayXRAuxDevices && !myToDisplayXRHands)) {
        for (NCollection_Array1<Handle(AIS_XRTrackedDevice)>::Iterator aPrsIter(myXRPrsDevices); aPrsIter.More();
             aPrsIter.Next()) {
    ...
    if (myXRPrsDevices.Length() != theView->View()->XRSession()->TrackedPoses().Length()) {
        ...
        myXRPrsDevices.Resize(theView->View()->XRSession()->TrackedPoses().Lower(),
                              theView->View()->XRSession()->TrackedPoses().Upper(), false);
    }
    ...
    for (Standard_Integer aDeviceIter = theView->View()->XRSession()->TrackedPoses().Lower();
         aDeviceIter <= theView->View()->XRSession()->TrackedPoses().Upper(); ++aDeviceIter) {
        ...
        if (isHand && aPosePrs->Role() == myXRLastPickingHand) {
            aLaserLen = myXRLastPickingHand == Aspect_XRTrackedDeviceRole_LeftHand ? myXRLastPickDepthLeft
            ...
            aLaserLen = myXRLastTeleportHand == Aspect_XRTrackedDeviceRole_LeftHand ? myXRLastPickDepthLeft
                                                                                    : myXRLastPickDepthRight;
            if (Precision::IsInfinite(aLaserLen)) {
                const Bnd_Box aViewBox = theView->View()->MinMaxValues(true);
                if (!aViewBox.IsVoid()) {
...
void AIS_ViewController::HandleViewEvents(const Handle(AIS_InteractiveContext) & theCtx,
    ...
    theView->View()->SynchronizeXRPosedToBaseCamera(); // handleCameraActions() may modify posed camera position - copy
                                                       // this modifications also to the base camera
