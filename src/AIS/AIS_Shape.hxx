...
//! To generate texture coordinates, appropriate shading attribute should be set before computing presentation in
//! AIS_Shaded display mode:
//! @code
...
public:
    ...
    Standard_EXPORT Standard_Boolean OwnDeviationCoefficient(Standard_Real& aCoefficient,
...
public: //! @name methods to alter texture mapping properties
    ...
    //! (0,1). Each face of the shape to be textured is parameterized in UV space (Umin,Umax) by (Vmin,Vmax).
    void SetTextureRepeatUV(const gp_Pnt2d& theRepeatUV) {
        myUVRepeat = theRepeatUV;
    }
...
protected:
    ...
    virtual void computeHLR(const Handle(Graphic3d_Camera) & theProjector, const Handle(TopLoc_Datum3D) & theTrsf,
    ...
    Standard_EXPORT void setMaterial(const Handle(Prs3d_Drawer) & theDrawer,
                                     const Graphic3d_MaterialAspect& theMaterial, const Standard_Boolean theToKeepColor,
