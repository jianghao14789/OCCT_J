...
public:
    ...
    Standard_EXPORT static Standard_Boolean FindColor(const Standard_Real theValue, const Standard_Real theMin,
                                                      const Standard_Real theMax, const Standard_Integer theColorsCount,
                                                      const Graphic3d_Vec3d& theColorHlsMin,
                                                      const Graphic3d_Vec3d& theColorHlsMax, Quantity_Color& theColor);

    //! Calculate color according passed value; returns true if value is in range or false, if isn't
    static Standard_Boolean FindColor(const Standard_Real theValue, const Standard_Real theMin,
                                      const Standard_Real theMax, const Standard_Integer theColorsCount,
                                      Quantity_Color& theColor) {
        return FindColor(theValue, theMin, theMax, theColorsCount, Graphic3d_Vec3d(230.0, 1.0, 1.0),
                         Graphic3d_Vec3d(0.0, 1.0, 1.0), theColor);
    }
...
public:
    ...
    //! Returns the user specified labels.
    const TColStd_SequenceOfExtendedString& Labels() const {
        return myLabels;
    }

    //! Sets the color scale labels.
    //! The length of the sequence should be equal to GetNumberOfIntervals() or to GetNumberOfIntervals() + 1 if
    //! IsLabelAtBorder() is true. If length of the sequence does not much the number of intervals, then these labels
    //! will be considered as "free" and will be located at the virtual intervals corresponding to the number of labels
    ...
    //! Returns the user specified colors.
    const Aspect_SequenceOfColor& GetColors() const {
        return myColors;
    }
    ...
    Standard_DEPRECATED("AIS_ColorScale::SetTitlePosition() has no effect!") void SetTitlePosition(
...
public:
    ...
    Standard_EXPORT void TextSize(const TCollection_ExtendedString& theText, const Standard_Integer theHeight,
...
private:
    ...
    Quantity_Color colorFromValue(const Standard_Real theValue, const Standard_Real theMin,
    ...
    void drawText(const Handle(Graphic3d_Group) & theGroup, const TCollection_ExtendedString& theText,
    ...
    //! Draw labels.
    void drawLabels(const Handle(Graphic3d_Group) & theGroup, const TColStd_SequenceOfExtendedString& theLabels,
                    const Standard_Integer theBarBottom, const Standard_Integer theBarHeight,
                    const Standard_Integer theMaxLabelWidth, const Standard_Integer theColorBreadth);

    //! Draw a color bar.
    void drawColorBar(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theBarBottom,
    ...
    void drawFrame(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theX, const Standard_Integer theY,
                   const Standard_Integer theWidth, const Standard_Integer theHeight, const Quantity_Color& theColor);
