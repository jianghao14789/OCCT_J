...
public: //! @name Light properties
    ...
    void SetMarkerImage(const Handle(Graphic3d_MarkerImage) & theImage, bool theIsEnabled) {
        ...
        myMarkerTypes[theIsEnabled ? 1 : 0] =
            !theImage.IsNull() ? Aspect_TOM_USERDEFINED : (theIsEnabled ? Aspect_TOM_O_POINT : Aspect_TOM_O_X);
    }
...
protected:
    ...
    //! Compute positional light source presentation as a sphere of either fixed size (no range) or of size representing
    //! a maximum range.
    ...
    Standard_EXPORT virtual void computeSpot(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode);
...
public:
    //! Main constructor.
    Standard_EXPORT AIS_LightSourceOwner(const Handle(AIS_LightSource) & theObject, Standard_Integer thePriority = 5);

    //! Handle mouse button click event.
    Standard_EXPORT virtual Standard_Boolean HandleMouseClick(const Graphic3d_Vec2i& thePoint,
                                                              Aspect_VKeyMouse theButton, Aspect_VKeyFlags theModifiers,
