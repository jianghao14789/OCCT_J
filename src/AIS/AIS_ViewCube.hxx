...
//! @code SetTransformPersistence (new Graphic3d_TransformPers (Graphic3d_TMF_TriedronPers, Aspect_TOTP_LEFT_LOWER,
//! Graphic3d_Vec2i (100, 100)); @endcode
//!
...
//! This loop allows external actions like application updating. For this purpose AIS_ViewCube has virtual interface
//! onAfterAnimation(), that is to be redefined on application level.
...
public: //! @name Geometry management API
    ...
    Standard_EXPORT void SetYup(Standard_Boolean theIsYup, Standard_Boolean theToUpdateLabels = Standard_True);
...
public: //! @name Style management API
    ...
    void SetBoxColor(const Quantity_Color& theColor) {
        if (!myDrawer->ShadingAspect()->Color().IsEqual(theColor) || !myBoxEdgeAspect->Color().IsEqual(theColor) ||
    ...
    //! Set axes labels.
    void SetAxesLabels(const TCollection_AsciiString& theX, const TCollection_AsciiString& theY,
...
public: //! @name animation methods
    ...
    //! Return TRUE if new camera Up direction should be always set to default value for a new camera Direction; FALSE
    //! by default. When this flag is FALSE, the new camera Up will be set as current Up orthogonalized to the new
    //! camera Direction, and will set to default Up on second click.
    Standard_Boolean ToResetCameraUp() const {
        return myToResetCameraUp;
    }
...
protected:
    ...
    struct IntegerHasher {
        static Standard_Integer HashCode(Standard_Integer theValue, Standard_Integer theUpper) {
            return ::HashCode(theValue, theUpper);
        }
        static Standard_Boolean IsEqual(Standard_Integer theA, Standard_Integer theB) {
            return theA == theB;
        }
    };
...
public:
    //! Main constructor.
    AIS_ViewCubeOwner(const Handle(AIS_ViewCube) & theObject, V3d_TypeOfOrientation theOrient,
    ...
    virtual Standard_Boolean IsForcedHilight() const Standard_OVERRIDE {
    ...
    virtual Standard_Boolean HandleMouseClick(const Graphic3d_Vec2i& thePoint, Aspect_VKeyMouse theButton,
                                              Aspect_VKeyFlags theModifiers, bool theIsDoubleClick) Standard_OVERRIDE {
        (void)thePoint;
        (void)theButton;
        (void)theModifiers;
