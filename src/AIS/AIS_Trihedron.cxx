...
AIS_Trihedron::AIS_Trihedron(const Handle(Geom_Axis2Placement) & theComponent)
    : myComponent(theComponent), myTrihDispMode(Prs3d_DM_WireFrame), myHasOwnSize(Standard_False),
      myHasOwnTextColor(Standard_False), myHasOwnArrowColor(Standard_False) {
...
void AIS_Trihedron::Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
    ...
    updatePrimitives(myDrawer->DatumAspect(), myTrihDispMode, anAxis.Location(), anAxis.XDirection(),
                     anAxis.YDirection(), anAxis.Direction());
...
// purpose  :
//=======================================================================
void AIS_Trihedron::ComputeSelection(const Handle(SelectMgr_Selection) & theSelection, const Standard_Integer theMode) {
    ...
    switch (theMode) {
        case AIS_TrihedronSelectionMode_EntireObject: {
            ...
            for (int aPartIter = isShadingMode ? Prs3d_DatumParts_Origin : Prs3d_DatumParts_XAxis;
                 aPartIter <= Prs3d_DatumParts_ZAxis; ++aPartIter) {
...
// purpose  :
//=======================================================================
void AIS_Trihedron::SetDatumPartColor(const Prs3d_DatumParts thePart, const Quantity_Color& theColor) {
    setOwnDatumAspect();
...
Handle(Select3D_SensitiveEntity) AIS_Trihedron::createSensitiveEntity(const Prs3d_DatumParts thePart,
    ...
    if (thePart >= Prs3d_DatumParts_XOYAxis && thePart <= Prs3d_DatumParts_XOZAxis) { // plane
...
// purpose  :
// =======================================================================
void AIS_Trihedron::updatePrimitives(const Handle(Prs3d_DatumAspect) & theAspect, Prs3d_DatumMode theMode,
                                     const gp_Pnt& theOrigin, const gp_Dir& theXDirection, const gp_Dir& theYDirection,
    ...
    for (int anAxisIter = Prs3d_DatumParts_XAxis; anAxisIter <= Prs3d_DatumParts_ZAxis; ++anAxisIter) {
        Prs3d_DatumParts aPart = (Prs3d_DatumParts)anAxisIter;
        anAxisPoints.Bind(aPart, gp_Pnt(anXYZOrigin + anAxisDirs.Find(aPart).XYZ() * theAspect->AxisLength(aPart)));
    }
        ...
        for (int aPartIter = Prs3d_DatumParts_XAxis; aPartIter <= Prs3d_DatumParts_ZAxis; ++aPartIter) {
            ...
            if (theAspect->DrawDatumPart(anArrowPart)) {
                myPrimitives[anArrowPart] = Prs3d_Arrow::DrawSegments(
                    anAxisPoints.Find(aPart), anAxisDirs.Find(aPart), theAspect->ArrowAspect()->Angle(),
            ...
            for (Standard_Integer anAxisIter = Prs3d_DatumParts_XAxis; anAxisIter <= Prs3d_DatumParts_ZAxis;
                ...
                if (theAspect->DrawDatumPart(aPart)) {
                    ...
                    myPrimitives[aPart] = Prs3d_Arrow::DrawShaded(
                        anAxis, anAxisLength * aTubeRadiusPercent,
                        aDrawArrow ? (anAxisLength - anAxisLength * aConeLengthPercent) : anAxisLength, 0.0, 0.0,
                        aNbOfFacettes);
                }

                // draw arrow
                if (aDrawArrow) {
                    myPrimitives[anArrowPart] =
                        Prs3d_Arrow::DrawShaded(anAxis, 0.0, anAxisLength, anAxisLength * aConeRadiusPercent,
