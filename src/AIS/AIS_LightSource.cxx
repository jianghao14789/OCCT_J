...
// purpose  :
// =======================================================================
AIS_LightSourceOwner::AIS_LightSourceOwner(const Handle(AIS_LightSource) & theObject, Standard_Integer thePriority)
...
// purpose  :
// =======================================================================
Standard_Boolean AIS_LightSourceOwner::HandleMouseClick(const Graphic3d_Vec2i&, Aspect_VKeyMouse theKey,
                                                        Aspect_VKeyFlags theFlags, bool) {
...
void AIS_LightSourceOwner::HilightWithColor(const Handle(PrsMgr_PresentationManager) & thePM,
    ...
    if (aLightSource->Light()->Type() == Graphic3d_TypeOfLightSource_Directional && aLightSource->myIsDraggable) {
        ...
        const Graphic3d_ZLayerId aZLayer =
            theStyle->ZLayer() != -1 ? theStyle->ZLayer()
        ...
        for (Standard_Integer aStep = 0; aStep < aNbPnts; ++aStep) {
            aCircPoints.SetValue(
                aStep, (aDetPnt.Rotated(gp_Ax1(gp::Origin(), aDirNormToPln), M_PI / 90 * (aStep - aNbPnts / 2))));
        }
...
AIS_LightSource::AIS_LightSource(const Handle(Graphic3d_CLight) & theLight)
    : myLightSource(theLight), myCodirMarkerType(Aspect_TOM_X), myOpposMarkerType(Aspect_TOM_O_POINT), mySize(50),
      myNbArrows(5), myNbSplitsQuadric(theLight->Type() == Graphic3d_TypeOfLightSource_Ambient ? 10 : 30),
      myNbSplitsArrow(20), mySensSphereArcSize(25),
      myIsZoomable(theLight->Type() == Graphic3d_TypeOfLightSource_Positional ||
                   theLight->Type() == Graphic3d_TypeOfLightSource_Spot),
      myIsDraggable(theLight->Type() == Graphic3d_TypeOfLightSource_Directional), myToDisplayName(true),
      myToDisplayRange(true), myToSwitchOnClick(true) {
    ...
    myArrowLineAspectShadow = new Graphic3d_AspectLine3d(
        Quantity_NOC_BLACK, Aspect_TOL_SOLID, theLight->Type() != Graphic3d_TypeOfLightSource_Ambient ? 3.0f : 1.0f);
...
void AIS_LightSource::updateLightAspects() {
    ...
    if (myLightSource->Type() == Graphic3d_TypeOfLightSource_Directional) {
        ...
        if (myLightSource->IsEnabled() && myLightSource->IsHeadlight() &&
            ...
            aDirMark =
                myLightSource->Direction().IsOpposite(-gp::DZ(), anAngleTol) ? myOpposMarkerType : myCodirMarkerType;
        }
        ...
        case Graphic3d_TypeOfLightSource_Ambient: {
            if (!myIsZoomable) {
                if (aTrsfPers.IsNull() || !aTrsfPers->IsTrihedronOr2d()) {
                    aTrsfPers = new Graphic3d_TransformPers(Graphic3d_TMF_TriedronPers, Aspect_TOTP_LEFT_UPPER,
                                                            Graphic3d_Vec2i(50));
                }
            } else {
                aTrsfPers.Nullify();
...
void AIS_LightSource::updateLightLocalTransformation() {
    ...
    switch (myLightSource->Type()) {
        ...
        case Graphic3d_TypeOfLightSource_Directional: {
            const gp_Pnt aLightPos =
                (myIsZoomable && !myLightSource->IsHeadlight()) ? myLightSource->DisplayPosition() : gp::Origin();
            gp_Trsf aTrsf;
...
// purpose  :
// =======================================================================
void AIS_LightSource::Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePrs,
...
// purpose  :
// =======================================================================
void AIS_LightSource::computeAmbient(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode) {
    ...
    if (theMode == 0 || theMode == 1) {
        ...
        Handle(Graphic3d_ArrayOfTriangles) aTrisArray =
            new Graphic3d_ArrayOfTriangles(aNbArrows * aCylTool.VerticesNb(), aNbArrows * aCylTool.TrianglesNb() * 3,
...
// purpose  :
// =======================================================================
void AIS_LightSource::computeDirectional(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode) {
...
// purpose  :
// =======================================================================
void AIS_LightSource::computePositional(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode) {
...
// purpose  :
// =======================================================================
void AIS_LightSource::computeSpot(const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode) {
    ...
    if (theMode == 0 && myToDisplayRange) {
        ...
        Handle(Graphic3d_ArrayOfTriangles) aSpotRangeArray =
            Prs3d_ToolCylinder::Create(aRadius, 0.0, aDistance, myNbSplitsQuadric, myNbSplitsQuadric, aTrsfCone);
...
// purpose  :
// =======================================================================
void AIS_LightSource::ComputeSelection(const Handle(SelectMgr_Selection) & theSel, const Standard_Integer theMode) {
    if (theMode != 0) {
        return;
    }
