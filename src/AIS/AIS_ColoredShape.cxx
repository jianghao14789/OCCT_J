...
// purpose  :
//=======================================================================
AIS_ColoredShape::AIS_ColoredShape(const Handle(AIS_Shape) & theShape) : AIS_Shape(theShape->Shape()) {
    // disable dedicated line aspects
    myDrawer->SetFreeBoundaryAspect(myDrawer->LineAspect());
    myDrawer->SetUnFreeBoundaryAspect(myDrawer->LineAspect());
    myDrawer->SetSeenLineAspect(myDrawer->LineAspect());
...
// purpose  :
//=======================================================================
void AIS_ColoredShape::UnsetCustomAspects(const TopoDS_Shape& theShape, const Standard_Boolean theToUnregister) {
...
void AIS_ColoredShape::Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
    ...
    dispatchColors(aBaseDrawer, myshape, aSubshapeDrawerMap, TopAbs_COMPOUND, Standard_False, aDispatchedOpened,
...
void AIS_ColoredShape::fillSubshapeDrawerMap(AIS_DataMapOfShapeDrawer& theSubshapeDrawerMap) const {
    ...
    for (AIS_DataMapOfShapeDrawer::Iterator aKeyShapeIter(myShapeColors); aKeyShapeIter.More(); aKeyShapeIter.Next()) {
...
void AIS_ColoredShape::ComputeSelection(const Handle(SelectMgr_Selection) & theSelection,
    ...
    Handle(AIS_ColoredDrawer) aBaseDrawer;
    myShapeColors.Find(myshape, aBaseDrawer);
    computeSubshapeSelection(aBaseDrawer, aSubshapeDrawerMap, myshape, aBrepOwner, theSelection, aTypOfSel, aPriority,
                             aDeflection, aDeviationAngle);
...
void AIS_ColoredShape::computeSubshapeSelection(const Handle(AIS_ColoredDrawer) & theParentDrawer,
                                                ...
                                                const TopAbs_ShapeEnum theTypOfSel, const Standard_Integer thePriority,
                                                const Standard_Real theDeflection, const Standard_Real theDeflAngle) {
    ...
    if (theTypOfSel == TopAbs_SHAPE && theShape.ShapeType() >= TopAbs_FACE) {
        StdSelect_BRepSelectionTool::ComputeSensitive(theShape, theOwner, theSelection, theDeflection, theDeflAngle,
                                                      aNbPOnEdge, aMaximalParameter, myDrawer->IsAutoTriangulation());
        return;
        ...
        StdSelect_BRepSelectionTool::ComputeSensitive(theShape, aBrepOwner, theSelection, theDeflection, theDeflAngle,
                                                      aNbPOnEdge, aMaximalParameter, myDrawer->IsAutoTriangulation());
        return;
    }
    ...
    for (TopoDS_Iterator aSubShapeIter(theShape); aSubShapeIter.More(); aSubShapeIter.Next()) {
        ...
        computeSubshapeSelection(aDrawer, theShapeDrawerMap, aSubShape, theOwner, theSelection, theTypOfSel,
                                 thePriority, theDeflection, theDeflAngle);
    }
}
        ...
        for (DataMapOfDrawerCompd::Iterator aMapIter(aDrawerShapeMap); aMapIter.More(); aMapIter.Next()) {
            ...
            if (theMode == AIS_Shaded && aShapeDraw.ShapeType() <= TopAbs_FACE && !IsInfinite()) {
                ...
                Handle(Graphic3d_ArrayOfTriangles) aTriangles =
                    StdPrs_ShadedShape::FillTriangles(aShapeDraw,
                ...
                if (aDrawer->FaceBoundaryDraw()) {
                    if (Handle(Graphic3d_ArrayOfSegments) aBndSegments = StdPrs_ShadedShape::FillFaceBoundaries(
                            aShapeDraw, aDrawer->FaceBoundaryUpperContinuity())) {
...
Standard_Boolean AIS_ColoredShape::dispatchColors(const Handle(AIS_ColoredDrawer) & theParentDrawer,
    ...
    if (aShapeType <= TopAbs_SHELL) {
        ...
        Standard_Boolean isClosedShell = theParentType == TopAbs_SOLID && aShapeType == TopAbs_SHELL &&
        ...
        for (TopoDS_Iterator aSubShapeIter(theShapeToParse); aSubShapeIter.More(); aSubShapeIter.Next()) {
            const TopoDS_Shape& aSubShape = aSubShapeIter.Value();
            if (dispatchColors(aDrawer, aSubShape, theShapeDrawerMap, aShapeType, isClosedShell,
                               theDrawerOpenedShapePerType, theDrawerClosedFaces)) {
    ...
    for (TopoDS_Iterator aSubShapeIter(theShapeToParse); aSubShapeIter.More(); aSubShapeIter.Next()) {
        const TopoDS_Shape& aSubShape = aSubShapeIter.Value();
        if (dispatchColors(aDrawer, aSubShape, theShapeDrawerMap, aShapeType, theIsParentClosed,
                           theDrawerOpenedShapePerType, theDrawerClosedFaces)) {
            isSubOverride = Standard_True;
        } else {
...
// purpose  :
//=======================================================================
void AIS_ColoredShape::bindSubShapes(AIS_DataMapOfShapeDrawer& theShapeDrawerMap, const TopoDS_Shape& theKeyShape,
