...
public:
    ...
    Standard_EXPORT void SetPart(const Standard_Integer theAxisIndex, const AIS_ManipulatorMode theMode,
...
public:
    ...
    ProcessDragging(const Handle(AIS_InteractiveContext) & theCtx, const Handle(V3d_View) & theView,
                    const Handle(SelectMgr_EntityOwner) & theOwner, const Graphic3d_Vec2i& theDragFrom,
                    const Graphic3d_Vec2i& theDragTo, const AIS_DragAction theAction) Standard_OVERRIDE;
    ...
    Standard_EXPORT void StartTransform(const Standard_Integer theX, const Standard_Integer theY,
    ...
    //! @return true if some part of manipulator is selected (transformation mode is active, and owning object can be
    //! transformed).
    ...
    Standard_Boolean HasActiveTransformation() {
        return myHasStartedTransformation;
    }
...
public: //! @name Configuration of graphical transformations
    ...
    //! @warning revise use of AdjustSize argument of of \sa AttachToObjects method
    //! when enabling zoom persistence.
    Standard_EXPORT virtual void SetTransformPersistence(const Handle(Graphic3d_TransformPers) &
                                                         theTrsfPers) Standard_OVERRIDE;
...
public:
    ...
    struct BehaviorOnTransform {
        ...
        BehaviorOnTransform()
            : FollowTranslation(Standard_True), FollowRotation(Standard_True), FollowDragging(Standard_True) {}
...
public: //! @name Presentation computation
    ...
    HilightOwnerWithColor(const Handle(PrsMgr_PresentationManager) & thePM, const Handle(Prs3d_Drawer) & theStyle,
...
protected:
    ...
    Standard_EXPORT Handle(Prs3d_Presentation)
        getHighlightPresentation(const Handle(SelectMgr_EntityOwner) & theOwner) const;

    Standard_EXPORT Handle(Graphic3d_Group)
        getGroup(const Standard_Integer theIndex, const AIS_ManipulatorMode theMode) const;
...
protected: //! @name Auxiliary classes to fill presentation with proper primitives
    ...
    public:
        Disk() : Quadric(), myInnerRad(0.0f), myOuterRad(1.0f) {}

        ~Disk() {}

        void Init(const Standard_ShortReal theInnerRadius, const Standard_ShortReal theOuterRadius,
                  const gp_Ax1& thePosition, const Standard_Integer theSlicesNb = 20,
    ...
    public:
        Sphere() : Quadric(), myRadius(1.0f) {}

        void Init(const Standard_ShortReal theRadius, const gp_Pnt& thePosition,
                  const Standard_Integer theSlicesNb = 20, const Standard_Integer theStacksNb = 20);
    ...
    public:
        Sector() : Quadric(), myRadius(0.0f) {}

        ~Sector() {}

        void Init(const Standard_ShortReal theRadius, const gp_Ax1& thePosition, const gp_Dir& theXDirection,
                  const Standard_Integer theSlicesNb = 5, const Standard_Integer theStacksNb = 5);
    ...
    class Axis {
    public:
        Axis(const gp_Ax1& theAxis = gp_Ax1(), const Quantity_Color& theColor = Quantity_Color(),
             const Standard_ShortReal theLength = 10.0f);

        void Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr, const Handle(Prs3d_Presentation) & thePrs,
                     const Handle(Prs3d_ShadingAspect) & theAspect);

        const gp_Ax1& ReferenceAxis() const {
            return myReferenceAxis;
        }
        ...
        Standard_Integer FacettesNumber() const {
            return myFacettesNumber;
        }

    public:
        const gp_Pnt& TranslatorTipPosition() const {
            return myArrowTipPos;
        }
        const Sector& DraggerSector() const {
            return mySector;
        }
        const Disk& RotatorDisk() const {
            return myCircle;
        }
        float RotatorDiskRadius() const {
            return myCircleRadius;
        }
        const Cube& ScalerCube() const {
            return myCube;
        }
        const gp_Pnt& ScalerCubePosition() const {
            return myCubePos;
        }
...
protected:
    ...
    BehaviorOnTransform
        myBehaviorOnTransform; //!< Behavior settings applied on manipulator when transforming an object.

protected: //! @name Fields for interactive transformation. Fields only for internal needs. They do not have public
           //! interface.
    NCollection_Sequence<gp_Trsf> myStartTrsfs; //!< Owning object transformation for start. It is used internally.
    Standard_Boolean
        myHasStartedTransformation; //!< Shows if transformation is processed (sequential calls of Transform()).
