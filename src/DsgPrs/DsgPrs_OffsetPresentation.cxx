...
void DsgPrs_OffsetPresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                    ...
                                    const gp_Dir& aDirection, const gp_Dir& aDirection2, const gp_Pnt& OffsetPoint) {
    ...
    if (DimNulle) {
        Prs3d_Arrow::Draw(aPresentation->CurrentGroup(), offp, L4.Direction(), LA->ArrowAspect()->Angle(),
                          LA->ArrowAspect()->Length());
        Prs3d_Arrow::Draw(aPresentation->CurrentGroup(), offp, L4.Direction().Reversed(), LA->ArrowAspect()->Angle(),
                          LA->ArrowAspect()->Length());
    } else {
...
void DsgPrs_OffsetPresentation::AddAxes(const Handle(Prs3d_Presentation) & aPresentation,
                                        ...
                                        const TCollection_ExtendedString& /*aText*/, const gp_Pnt& AttachmentPoint1,
                                        const gp_Pnt& AttachmentPoint2, const gp_Dir& aDirection,
                                        const gp_Dir& /*aDirection2*/, const gp_Pnt& OffsetPoint) {
    gp_Lin L1(AttachmentPoint1, aDirection);
    gp_Pnt Proj1 = ElCLib::Value(ElCLib::Parameter(L1, OffsetPoint), L1);
