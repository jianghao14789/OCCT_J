...
void DsgPrs_XYZAxisPresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                     const Handle(Prs3d_LineAspect) & aLineAspect, const gp_Dir& aDir,
                                     const Standard_Real aVal, const Standard_CString theText, const gp_Pnt& aPfirst,
                                     const gp_Pnt& aPlast) {
...
void DsgPrs_XYZAxisPresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                     ...
                                     const Handle(Prs3d_TextAspect) & aTextAspect, const gp_Dir& aDir,
                                     const Standard_Real aVal, const Standard_CString theText, const gp_Pnt& aPfirst,
                                     const gp_Pnt& aPlast) {
    Handle(Graphic3d_Group) G = aPresentation->CurrentGroup();
    G->SetPrimitivesAspect(aLineAspect->Aspect());
