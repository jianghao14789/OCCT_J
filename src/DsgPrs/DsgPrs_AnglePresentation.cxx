...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   const Handle(Prs3d_Drawer) & aDrawer, const Standard_Real /*aVal*/,
                                   const TCollection_ExtendedString& aText, const gp_Circ& aCircle,
                                   const gp_Pnt& aPosition, const gp_Pnt& Apex, const gp_Circ& VminCircle,
                                   const gp_Circ& VmaxCircle, const Standard_Real aArrowSize) {
    ...
    Standard_Integer i;
    Standard_Real AttParam =
        ElCLib::Parameter(aCircle2, AttachmentPnt); // must be equal to zero (look circle construction)
    ...
    if (ElCLib::Parameter(aCircle2, tmpPnt) < OppParam)
        if (2. * myCircle.Radius() > 4. * myArrowSize) IsArrowOut = Standard_False; // four times more than an arrow
                                                                                    // size
    ...
    DsgPrs::ComputeSymbol(aPresentation, aDimensionAspect, AttachmentPnt, AttachmentPnt, aDir, aDir, DsgPrs_AS_LASTAR);
    DsgPrs::ComputeSymbol(aPresentation, aDimensionAspect, OppositePnt, OppositePnt, aDir2, aDir2, DsgPrs_AS_LASTAR);
...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   const Handle(Prs3d_Drawer) & aDrawer, const Standard_Real theval,
                                   const TCollection_ExtendedString& aText, const gp_Pnt& CenterPoint,
                                   const gp_Pnt& AttachmentPoint1, const gp_Pnt& AttachmentPoint2, const gp_Dir& dir1,
                                   const gp_Dir& dir2, const gp_Dir& axisdir, const gp_Pnt& OffsetPoint) {
    char valcar[80];
...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   ...
                                   const TCollection_ExtendedString& aText, const gp_Pnt& CenterPoint,
                                   const gp_Pnt& AttachmentPoint1, const gp_Pnt& AttachmentPoint2, const gp_Dir& dir1,
                                   const gp_Dir& dir2, const gp_Dir& axisdir, const Standard_Boolean isPlane,
                                   const gp_Ax1& AxisOfSurf, const gp_Pnt& OffsetPoint,
    ...
    DsgPrs::ComputeFacesAnglePresentation(
        LA->ArrowAspect()->Length(), theval, CenterPoint, AttachmentPoint1, AttachmentPoint2, dir1, dir2, axisdir,
        isPlane, AxisOfSurf, OffsetPoint, AngleCirc, FirstParAngleCirc, LastParAngleCirc, EndOfArrow1, EndOfArrow2,
        DirOfArrow1, DirOfArrow2, ProjAttachPoint2, AttachCirc, FirstParAttachCirc, LastParAttachCirc);
...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   ...
                                   const TCollection_ExtendedString& aText, const gp_Pnt& CenterPoint,
                                   const gp_Pnt& AttachmentPoint1, const gp_Pnt& AttachmentPoint2, const gp_Dir& dir1,
                                   const gp_Dir& dir2, const gp_Pnt& OffsetPoint) {
    char valcar[80];
    sprintf(valcar, "%5.2f", theval);
...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   ...
                                   const TCollection_ExtendedString& aText, const gp_Pnt& CenterPoint,
                                   const gp_Pnt& AttachmentPoint1, const gp_Pnt& AttachmentPoint2, const gp_Dir& dir1,
                                   const gp_Dir& dir2, const gp_Pnt& OffsetPoint, const DsgPrs_ArrowSide ArrowPrs) {
    char valcar[80];
    sprintf(valcar, "%5.2f", theval);
...
void DsgPrs_AnglePresentation::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                   const Handle(Prs3d_Drawer) & aDrawer, const Standard_Real theval,
                                   const gp_Pnt& CenterPoint, const gp_Pnt& AttachmentPoint1, const gp_Ax1& theAxe,
    ...
    switch (ArrowSide) {
        case DsgPrs_AS_FIRSTAR: {
            ElCLib::D1(uc1, cer, ptarr, vecarr);
            Prs3d_Arrow::Draw(aPresentation->CurrentGroup(), ptarr, gp_Dir(-vecarr), LA->ArrowAspect()->Angle(),
                              length);
            break;
        }
