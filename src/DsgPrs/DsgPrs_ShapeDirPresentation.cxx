...
static Standard_Boolean ComputeDir(const TopoDS_Shape& shape, gp_Pnt& pt, gp_Dir& dir, const Standard_Integer mode) {
    ...
    if (shape.ShapeType() == TopAbs_EDGE) {
        ...
        if (!lProps.IsTangentDefined()) return Standard_False;
        pt = lProps.Value();
        lProps.Tangent(dir);
        ...
        if (BRep_Tool::NaturalRestriction(TopoDS::Face(shape))) {
            ...
            if (!FindPointOnFace(TopoDS::Face(shape), pt2d)) return Standard_False;
        }

        GeomLProp_SLProps lProps(surface, pt2d.X(), pt2d.Y(), 1, gp::Resolution());
        if (!lProps.IsNormalDefined()) return Standard_False;
...
void DsgPrs_ShapeDirPresentation::Add(const Handle(Prs3d_Presentation) & prs, const Handle(Prs3d_Drawer) & drawer,
                                      const TopoDS_Shape& shape, const Standard_Integer mode)

{
    ...
    if (shape.ShapeType() == TopAbs_EDGE) {
        ...
        for (anExp.Init(TopoDS::Wire(shape)); anExp.More(); anExp.Next()) {
            ...
            nb++;
            if (nb <= 3) BRepBndLib::Add(edge, box);
            aList.Append(edge);
        }
