...
public:
    Standard_EXPORT static Handle(ItemLocation)
        Translate(const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap);
    Standard_EXPORT static Handle(Datum3D)
        Translate(const Handle(TopLoc_Datum3D) & theDatum, StdObjMgt_TransientPersistentMap& theMap);
};
