...
Standard_Boolean TFunction_GraphNode::AddPrevious(const Standard_Integer funcID) {
    if (myPrevious.Contains(funcID)) return Standard_False;
...
Standard_Boolean TFunction_GraphNode::AddPrevious(const TDF_Label& func) {
    ...
    if (!scope->GetFunctions().IsBound2(func)) return Standard_False;
...
Standard_Boolean TFunction_GraphNode::AddNext(const Standard_Integer funcID) {
    if (myNext.Contains(funcID)) return Standard_False;
...
void TFunction_GraphNode::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & /*RT*/) const {
