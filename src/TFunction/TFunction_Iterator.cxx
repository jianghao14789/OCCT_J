...
void TFunction_Iterator::Init(const TDF_Label& Access) {
    ...
    for (; itrm.More(); itrm.Next()) {
        ...
        if (!graphNode->GetPrevious().IsEmpty()) continue;

        // In execution mode we consider only "not executed" functions.
        if (myUsageOfExecutionStatus && status != TFunction_ES_NotExecuted) continue;

        myCurrent.Append(L);

        // Register already passed functions
        if (!myUsageOfExecutionStatus) myPassedFunctions.Add(L);
    }
}
...
Standard_Integer TFunction_Iterator::GetMaxNbThreads() const {
    ...
    while (!fIterator.Current().IsEmpty()) {
        ...
        if (nb_threads < current.Extent()) nb_threads = current.Extent();
        fIterator.Next();
    }
...
Standard_Boolean TFunction_Iterator::More() const {
    if (myUsageOfExecutionStatus) {
        ...
        for (; itrm.More(); itrm.Next()) {
            const TDF_Label& L = itrm.Key2();
            if (GetStatus(L) == TFunction_ES_NotExecuted) return Standard_True;
        }
...
void TFunction_Iterator::Next() {
    ...
    for (; itrl.More(); itrl.Next()) {
        ...
        for (; itrm.More(); itrm.Next()) {
            ...
            if (!myUsageOfExecutionStatus && myPassedFunctions.Contains(Lnext)) continue;

            next_current.Add(Lnext);

            // Register already passed functions
            if (!myUsageOfExecutionStatus) myPassedFunctions.Add(Lnext);
        }
    }
...
void TFunction_Iterator::SetStatus(const TDF_Label& func, const TFunction_ExecutionStatus status) const {
    TFunction_IFunction iFunction(func);
...
Standard_OStream& TFunction_Iterator::Dump(Standard_OStream& anOS) const {
    ...
    if (myCurrent.IsEmpty()) return anOS;
