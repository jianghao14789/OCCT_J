...
Standard_Boolean TFunction_DriverTable::AddDriver(const Standard_GUID& guid, const Handle(TFunction_Driver) & driver,
...
Standard_Boolean TFunction_DriverTable::HasDriver(const Standard_GUID& guid, const Standard_Integer thread) const {
    if (thread == 0)
...
Standard_Boolean TFunction_DriverTable::FindDriver(const Standard_GUID& guid, Handle(TFunction_Driver) & driver,
...
Standard_Boolean TFunction_DriverTable::RemoveDriver(const Standard_GUID& guid, const Standard_Integer thread) {
    if (thread == 0)
...
void TFunction_DriverTable::Clear() {
    myDrivers.Clear();
    if (!myThreadDrivers.IsNull()) myThreadDrivers.Nullify();
}
