...
public:
    ...
    Standard_EXPORT GCPnts_TangentialDeflection(const Adaptor3d_Curve& theC, const Standard_Real theAngularDeflection,
    ...
    //! @param theMinLen [in] minimal length
    Standard_EXPORT GCPnts_TangentialDeflection(const Adaptor3d_Curve& theC, const Standard_Real theFirstParameter,
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT GCPnts_TangentialDeflection(const Adaptor2d_Curve2d& theC, const Standard_Real theAngularDeflection,
                                                const Standard_Real theCurvatureDeflection,
                                                const Standard_Integer theMinimumOfPoints = 2,
                                                const Standard_Real theUTol = 1.0e-9,
                                                const Standard_Real theMinLen = 1.0e-7);
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT GCPnts_TangentialDeflection(const Adaptor2d_Curve2d& theC, const Standard_Real theFirstParameter,
                                                const Standard_Real theLastParameter,
                                                const Standard_Real theAngularDeflection,
                                                const Standard_Real theCurvatureDeflection,
                                                const Standard_Integer theMinimumOfPoints = 2,
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT void Initialize(const Adaptor3d_Curve& theC, const Standard_Real theAngularDeflection,
                                    const Standard_Real theCurvatureDeflection,
                                    const Standard_Integer theMinimumOfPoints = 2, const Standard_Real theUTol = 1.0e-9,
                                    const Standard_Real theMinLen = 1.0e-7);
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT void Initialize(const Adaptor3d_Curve& theC, const Standard_Real theFirstParameter,
                                    const Standard_Real theLastParameter, const Standard_Real theAngularDeflection,
                                    const Standard_Real theCurvatureDeflection,
                                    const Standard_Integer theMinimumOfPoints = 2, const Standard_Real theUTol = 1.0e-9,
                                    const Standard_Real theMinLen = 1.0e-7);
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT void Initialize(const Adaptor2d_Curve2d& theC, const Standard_Real theAngularDeflection,
                                    const Standard_Real theCurvatureDeflection,
                                    const Standard_Integer theMinimumOfPoints = 2, const Standard_Real theUTol = 1.0e-9,
                                    const Standard_Real theMinLen = 1.0e-7);
    ...
    //! @param theUTol   [in] tolerance in curve parametric scope
    //! @param theMinLen [in] minimal length
    Standard_EXPORT void Initialize(const Adaptor2d_Curve2d& theC, const Standard_Real theFirstParameter,
                                    const Standard_Real theLastParameter, const Standard_Real theAngularDeflection,
                                    const Standard_Real theCurvatureDeflection,
                                    const Standard_Integer theMinimumOfPoints = 2, const Standard_Real theUTol = 1.0e-9,
                                    const Standard_Real theMinLen = 1.0e-7);
    ...
    Standard_EXPORT Standard_Integer AddPoint(const gp_Pnt& thePnt, const Standard_Real theParam,
...
private:
    ...
    void initialize(const TheCurve& theC, const Standard_Real theFirstParameter, const Standard_Real theLastParameter,
    ...
    template <class TheCurve> void PerformLinear(const TheCurve& theC);

    template <class TheCurve> void PerformCircular(const TheCurve& theC);

    //! Respecting the angle and the deflection,
    //! we impose a minimum number of points on a curve.
    template <class TheCurve> void PerformCurve(const TheCurve& theC);

    template <class TheCurve>
    void EvaluateDu(const TheCurve& theC, const Standard_Real theU, gp_Pnt& theP, Standard_Real& theDu,
                    Standard_Boolean& theNotDone) const;

    //! Estimation of maximal deflection for interval [theU1, theU2]
    template <class TheCurve>
    void EstimDefl(const TheCurve& theC, const Standard_Real theU1, const Standard_Real theU2,
