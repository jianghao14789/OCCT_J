...
TColStd_PackedMapOfInteger& TColStd_PackedMapOfInteger::Assign(const TColStd_PackedMapOfInteger& theOther) {
    if (this != &theOther) {
        Clear();
        if (!theOther.IsEmpty()) {
...
void TColStd_PackedMapOfInteger::ReSize(const Standard_Integer theNbBuckets) {
    ...
    TColStd_intMapNode** aNewData =
        (TColStd_intMapNode**)Standard::Allocate((aNewBuck + 1) * sizeof(TColStd_intMapNode*));
            ...
            while (p2) {
                if (p2->IsEqual(aKeyInt)) {
                    ...
                    if (aNewData == 0)
                        ...
                        if (aNewData != p1->Data()) p1->ChangeData() = aNewData;
...
Standard_Boolean TColStd_PackedMapOfInteger::IsEqual(const TColStd_PackedMapOfInteger& theMap) const {
    ...
    for (Standard_Integer i = 0; i <= myNbBuckets; i++) {
        ...
        while (p1 != 0L) {
            ...
            while (p2) {
                if (p2->IsEqual(aKeyInt)) {
                    if (p1->Data() != p2->Data()) return Standard_False;
                    break;
                }
                p2 = p2->Next();
            }
            // if the same block not found, maps are different
            if (p2 == 0L) return Standard_False;
...
Standard_Boolean TColStd_PackedMapOfInteger::HasIntersection(const TColStd_PackedMapOfInteger& theMap) const {
    ...
    if (myData1 == theMap.myData1) return Standard_True;
    ...
    for (Standard_Integer i = 0; i <= myNbBuckets; i++) {
        ...
        while (p1 != 0L) {
            ...
            while (p2) {
                if (p2->IsEqual(aKeyInt)) {
                    if (p1->Data() & p2->Data()) return Standard_True;
                    break;
                }
                p2 = p2->Next();
            }
