...
void Bnd_Sphere::Add(const Bnd_Sphere& theOther) {
    ...
    if (theOther.myRadius + aDist <= myRadius) return; // this sphere encloses other
...
Standard_Boolean Bnd_Sphere::IsOut(const gp_XYZ& theXYZ, Standard_Real& theMaxDist) const {
    ...
    if (aCurMinDist > theMaxDist) return Standard_True;
    if (myIsValid && aCurMaxDist < theMaxDist) theMaxDist = aCurMaxDist;
