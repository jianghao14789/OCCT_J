...
Standard_Boolean Bnd_Range::Union(const Bnd_Range& theOther) {
    ...
    if (myLast < theOther.myFirst) return Standard_False;

    if (myFirst > theOther.myLast) return Standard_False;
...
Standard_Integer Bnd_Range::IsIntersected(const Standard_Real theVal, const Standard_Real thePeriod) const {
    if (IsVoid()) return Standard_False;
    ...
    if (aPeriod <= RealSmall()) {
        ...
        if (IsEqual(aDelta, 0.0)) return 2;

        if (aDelta > 0.0) return 0;
    ...
    if (aPar1 != aPar2) { // Interval (myFirst, myLast] intersects seam-edge
        if (IsEqual(aVal2, static_cast<Standard_Real>(
                               aPar2))) { // aVal2 is an integer number => myLast lies ON the "seam-edge"
            return 2;
        }
    ...
    if (IsEqual(aVal1,
                static_cast<Standard_Real>(aPar1))) { // aVal1 is an integer number => myFirst lies ON the "seam-edge"
        return 2;
    }
...
// purpose  :
//=======================================================================
void Bnd_Range::Split(const Standard_Real theVal, NCollection_List<Bnd_Range>& theList,
