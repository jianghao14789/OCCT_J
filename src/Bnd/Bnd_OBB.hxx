...
public:
    ...
    Bnd_OBB(const gp_Pnt& theCenter, const gp_Dir& theXDirection, const gp_Dir& theYDirection,
            const gp_Dir& theZDirection, const Standard_Real theHXSize, const Standard_Real theHYSize,
            const Standard_Real theHZSize)
        : myCenter(theCenter.XYZ()), myIsAABox(Standard_False) {
    ...
    //! So that applying it to axis-aligned box ((-XHSize, -YHSize, -ZHSize), (XHSize, YHSize, ZHSize)) will produce
    //! this oriented box.
    //! @code
    //!   gp_Trsf aLoc;
    ...
    gp_Ax3 Position() const {
        return gp_Ax3(myCenter, ZDirection(), XDirection());
    }
