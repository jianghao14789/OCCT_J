...
// purpose  :
//=======================================================================
void Bnd_Box2d::Update(const Standard_Real x, const Standard_Real y, const Standard_Real X, const Standard_Real Y) {
    if (Flags & VoidMask) {
        Xmin = x;
        Ymin = y;
        Xmax = X;
...
void Bnd_Box2d::Get(Standard_Real& x, Standard_Real& y, Standard_Real& Xm, Standard_Real& Ym) const {
    if (Flags & VoidMask) throw Standard_ConstructionError("Bnd_Box is void");
...
Standard_Boolean Bnd_Box2d::IsOut(const gp_Lin2d& theL) const {
    ...
    const Standard_Real aProd[3] = {theL.Direction().XY() ^ (aCenter - theL.Location().XY()),
                                    theL.Direction().X() * aHeigh.Y(), theL.Direction().Y() * aHeigh.X()};
...
Standard_Boolean Bnd_Box2d::IsOut(const gp_Pnt2d& theP0, const gp_Pnt2d& theP1) const {
    ...
    const Standard_Real aProd[3] = {aSegDelta ^ (aCenter - theP0.XY()), aSegDelta.X() * aHeigh.Y(),
