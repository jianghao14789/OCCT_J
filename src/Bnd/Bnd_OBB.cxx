...
public:
    ...
    OBB_ExtremePointsSelector()
        : BVH_Traverse<Standard_Real, 3, BVH_BoxSet<Standard_Real, 3, gp_XYZ>, Bnd_Range>(), myPrmMin(RealLast()),
          myPrmMax(RealFirst()) {}

public: //! @name Set axis for projection
    //! Sets the axis
    void SetAxis(const gp_XYZ& theAxis) {
        myAxis = theAxis;
    }
...
public: //! @name Definition of rejection/acceptance rules
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theIndex, const Bnd_Range&) Standard_OVERRIDE {
...
public:
    ...
    OBBTool(const TColgp_Array1OfPnt& theL, const TColStd_Array1OfReal* theLT = 0,
...
protected:
    ...
    //! recomputed.
    void ProcessTriangle(const Standard_Integer theIdx1, const Standard_Integer theIdx2, const Standard_Integer theIdx3,
...
private:
    ...
    struct Params {
        Params() : _ParamMin(RealLast()), _ParamMax(RealFirst()) {}

        Params(Standard_Real theMin, Standard_Real theMax) : _ParamMin(theMin), _ParamMax(theMax) {}
    ...
    void ComputeParams(const Standard_Integer theId1, const Standard_Integer theId2, Standard_Real& theMin,
    ...
    void Project(const gp_XYZ& theAxis, Standard_Real& theMin, Standard_Real& theMax, gp_XYZ* thePntMin = 0,
                 gp_XYZ* thePntMax = 0) {
        theMin = RealLast(), theMax = RealFirst();
        ...
        if (myOptimal) {
            ...
            for (Standard_Integer iP = myPntsList.Lower(); iP <= myPntsList.Upper(); ++iP) {
                ...
                if (aPrm < theMin) {
                    theMin = aPrm;
                    if (thePntMin) *thePntMin = aPoint;
                }
                if (aPrm > theMax) {
                    theMax = aPrm;
                    if (thePntMax) *thePntMax = aPoint;
                }
            }
        }
    }
...
// purpose :
//=======================================================================
OBBTool::OBBTool(const TColgp_Array1OfPnt& theL, const TColStd_Array1OfReal* theLT, const Standard_Boolean theIsOptimal)
    : myPntsList(theL), myListOfTolers(theLT), myQualityCriterion(RealLast()), myOptimal(theIsOptimal) {
    if (myOptimal) {
...
void OBBTool::ComputeExtremePoints() {
    ...
    const gp_XYZ anInitialAxes7[myNbInitAxes] = {gp_XYZ(1.0, 0.0, 0.0),           gp_XYZ(0.0, 1.0, 0.0),
                                                 gp_XYZ(0.0, 0.0, 1.0),           gp_XYZ(1.0, 1.0, 1.0) / aSqrt3,
                                                 gp_XYZ(1.0, 1.0, -1.0) / aSqrt3, gp_XYZ(1.0, -1.0, 1.0) / aSqrt3,
    ...
    for (Standard_Integer anAxeInd = 0, aPrmInd = -1; anAxeInd < myNbInitAxes; ++anAxeInd) {
        ...
        Project(anInitialAxesArray[anAxeInd], aParams[aMinInd], aParams[aMaxInd], &myLExtremalPoints[aMinInd],
...
void OBBTool::FillToTriangle3() {
    ...
    Standard_Real aMaxSqDist = -1.0;
    for (Standard_Integer i = 0; i < myNbExtremalPoints; i++) {
        if ((i == myTriIdx[0]) || (i == myTriIdx[1])) continue;
...
void OBBTool::FillToTriangle5(const gp_XYZ& theNormal, const gp_XYZ& theBarryCenter) {
    ...
    for (Standard_Integer aPtIdx = 0; aPtIdx < myNbExtremalPoints; aPtIdx++) {
        if ((aPtIdx == myTriIdx[0]) || (aPtIdx == myTriIdx[1]) || (aPtIdx == myTriIdx[2])) continue;
    ...
    if (id3 >= 0 && aParams[0] < -Precision::Confusion()) myTriIdx[3] = id3;

    if (id4 >= 0 && aParams[1] > Precision::Confusion()) myTriIdx[4] = id4;
}
...
//=======================================================================
void OBBTool::ProcessTriangle(const Standard_Integer theIdx1, const Standard_Integer theIdx2,
                              const Standard_Integer theIdx3, const Standard_Boolean theIsBuiltTrg) {
    ...
    if (aSqMod < Precision::SquareConfusion()) return;
    ...
    if (theIsBuiltTrg) FillToTriangle5(aZAxis, myLExtremalPoints[theIdx1]);
...
void OBBTool::BuildBox(Bnd_OBB& theBox) {
    ...
    const gp_XYZ aCenter = 0.5 * ((aParams[1] + aParams[0]) * aXDir.XYZ() + (aParams[3] + aParams[2]) * aYDir.XYZ() +
...
// =======================================================================
void Bnd_OBB::ReBuild(const TColgp_Array1OfPnt& theListOfPoints, const TColStd_Array1OfReal* theListOfTolerances,
                      const Standard_Boolean theIsOptimal) {
    switch (theListOfPoints.Length()) {
        case 1:
            ProcessOnePoint(theListOfPoints.First());
            if (theListOfTolerances) Enlarge(theListOfTolerances->First());
            return;
        case 2: {
...
Standard_Boolean Bnd_OBB::IsOut(const Bnd_OBB& theOther) const {
    if (IsVoid() || theOther.IsVoid()) return Standard_True;
    ...
    for (Standard_Integer i = 0; i < 3; ++i) {
        ...
        if (aDistCC > myHDims[i] + aLSegm2) return Standard_True;
    }
    ...
    for (Standard_Integer i = 0; i < 3; ++i) {
        ...
        if (aDistCC > aLSegm1 + theOther.myHDims[i]) return Standard_True;
    }
    ...
    for (Standard_Integer i = 0; i < 3; ++i) {
        for (Standard_Integer j = 0; j < 3; ++j) {
            ...
            if (aDistCC > aLSegm1 + aLSegm2) return Standard_True;
        }
    }
...
Standard_Boolean Bnd_OBB::IsCompletelyInside(const Bnd_OBB& theOther) const {
    ...
    for (Standard_Integer i = 0; i < 8; i++) {
        if (IsOut(aVert[i])) return Standard_False;
    }
