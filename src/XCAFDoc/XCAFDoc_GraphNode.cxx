...
Handle(XCAFDoc_GraphNode) XCAFDoc_GraphNode::Set(const TDF_Label& L, const Standard_GUID& explicitID) {
    Handle(XCAFDoc_GraphNode) GN;
...
void XCAFDoc_GraphNode::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & RT) const {
...
Standard_OStream& XCAFDoc_GraphNode::Dump(Standard_OStream& anOS) const {
    ...
    if (myFathers.Length() != 0) {
        anOS << "  Fathers=";
        for (; i <= NbFathers(); i++) {
            if (!myFathers(i)->Label().IsNull()) myFathers(i)->Label().EntryDump(anOS);
            anOS << std::endl;
        }
    }
    i = 1;
    if (myChildren.Length() != 0) {
        anOS << "  Children=";
        for (; i <= NbChildren(); i++) {
            if (!myChildren(i)->Label().IsNull()) myChildren(i)->Label().EntryDump(anOS);
            anOS << std::endl;
        }
    }
    //  anOS<<std::endl;
