...
Handle(XCAFDoc_Datum) XCAFDoc_Datum::Set(const TDF_Label& theLabel, const Handle(TCollection_HAsciiString) & theName,
...
Handle(TCollection_HAsciiString) XCAFDoc_Datum::GetName() const {
    if (myName.IsNull()) return new TCollection_HAsciiString();
    return myName;
}
...
void XCAFDoc_Datum::SetObject(const Handle(XCAFDimTolObjects_DatumObject) & theObject) {
    ...
    if (theObject->GetModifiers().Length() > 0) {
        ...
        if (!aModifiers.IsNull()) aModifiers->ChangeArray(anArr);
    }
    ...
    if (theObject->IsDatumTarget()) {
        Handle(TDataStd_Integer) aType =
            TDataStd_Integer::Set(Label().FindChild(ChildLab_DTargetType), theObject->GetDatumTargetType());
        ...
        if (theObject->GetDatumTargetType() == XCAFDimTolObjects_DatumTargetType_Area) {
            ...
            if (!aR.IsNull()) aR->ChangeArray(aRArr);

            if (theObject->GetDatumTargetType() != XCAFDimTolObjects_DatumTargetType_Point) {
                Handle(TDataStd_Real) aLen =
                    TDataStd_Real::Set(Label().FindChild(ChildLab_DTargetLength), theObject->GetDatumTargetLength());
                if (theObject->GetDatumTargetType() == XCAFDimTolObjects_DatumTargetType_Rectangle)
                    Handle(TDataStd_Real) aWidth =
                        TDataStd_Real::Set(Label().FindChild(ChildLab_DTargetWidth), theObject->GetDatumTargetWidth());
            }
        }
        Handle(TDataStd_Integer) aNum =
            TDataStd_Integer::Set(Label().FindChild(ChildLab_DTargetNumber), theObject->GetDatumTargetNumber());
    }
        ...
        if (Label().FindChild(ChildLab_DTargetType).FindAttribute(TDataStd_Integer::GetID(), aDTargetType)) {
            ...
            if (anObj->GetDatumTargetType() == XCAFDimTolObjects_DatumTargetType_Area) {
                ...
                if (Label().FindChild(ChildLab_AxisLoc).FindAttribute(TDataStd_RealArray::GetID(), aLoc) &&
                    ...
                    aR->Length() == 3) {
                    gp_Pnt aL(aLoc->Value(aLoc->Lower()), aLoc->Value(aLoc->Lower() + 1),
...
void XCAFDoc_Datum::Paste(const Handle(TDF_Attribute) & theInto, const Handle(TDF_RelocationTable) & /*RT*/) const {
