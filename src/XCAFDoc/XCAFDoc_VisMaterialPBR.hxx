...
struct XCAFDoc_VisMaterialPBR {
    ...
    Handle(Image_Texture)
        MetallicRoughnessTexture;           //!< RG texture packing the metallic and roughness properties together
    Handle(Image_Texture) EmissiveTexture;  //!< RGB emissive map controls the color and intensity of the light being
    ...
    Standard_ShortReal
        Metallic; //!< metalness  (or scale factor to the texture) within range [0.0, 1.0]; 1.0 by default
    Standard_ShortReal
        Roughness; //!< roughness  (or scale factor to the texture) within range [0.0, 1.0]; 1.0 by default
    ...
    XCAFDoc_VisMaterialPBR()
        : BaseColor(1.0f, 1.0f, 1.0f, 1.0f), EmissiveFactor(0.0f, 0.0f, 0.0f), Metallic(1.0f), Roughness(1.0f),
          RefractionIndex(1.5f), IsDefined(Standard_True) {}
    ...
    Standard_Boolean IsEqual(const XCAFDoc_VisMaterialPBR& theOther) const {
        ...
        return theOther.BaseColorTexture == BaseColorTexture &&
               ...
               theOther.EmissiveTexture == EmissiveTexture && theOther.OcclusionTexture == OcclusionTexture &&
