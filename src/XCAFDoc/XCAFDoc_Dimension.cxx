...
void XCAFDoc_Dimension::SetObject(const Handle(XCAFDimTolObjects_DimensionObject) & theObject) {
    ...
    if (!theObject->GetValues().IsNull()) {
        Handle(TDataStd_RealArray) aVal =
            TDataStd_RealArray::Set(Label().FindChild(ChildLab_Value), theObject->GetValues()->Lower(),
                                    theObject->GetValues()->Lower() + theObject->GetValues()->Length() - 1);
        if (!aVal.IsNull()) aVal->ChangeArray(theObject->GetValues());
    }
    ...
    if (aF != XCAFDimTolObjects_DimensionFormVariance_None) {
        ...
        if (!aClass.IsNull()) aClass->ChangeArray(anArrI);
    }
    ...
    if (aL > 0 || aR > 0) {
        ...
        if (!aDec.IsNull()) aDec->ChangeArray(anArrI);
    }
    ...
    if (theObject->GetModifiers().Length() > 0) {
        ...
        Handle(TDataStd_IntegerArray) aModifiers =
            TDataStd_IntegerArray::Set(Label().FindChild(ChildLab_Modifiers), 1, theObject->GetModifiers().Length());
        if (!aModifiers.IsNull()) aModifiers->ChangeArray(anArrI);
    }
    ...
    if (theObject->GetType() == XCAFDimTolObjects_DimensionType_Location_Oriented) {
        ...
        if (!aDir.IsNull()) aDir->ChangeArray(anArrR);
    }
    ...
    if (theObject->HasPoint()) {
        ...
        if (!aPnt.IsNull()) aPnt->ChangeArray(aPntArr);
    }
    ...
    if (theObject->HasPlane()) {
        ...
        if (!aLoc.IsNull()) aLoc->ChangeArray(aLocArr);
        ...
        if (!aN.IsNull()) aN->ChangeArray(aNArr);
        ...
        if (!aRAtt.IsNull()) aRAtt->ChangeArray(aRArr);
    }
    ...
    if (theObject->HasDescriptions()) {
        ...
        Handle(TDataStd_ExtStringArray) aDescriptions =
            TDataStd_ExtStringArray::Set(Label().FindChild(ChildLab_Descriptions), 1, theObject->NbDescriptions());
        Handle(TDataStd_ExtStringArray) aDescriptionNames =
            TDataStd_ExtStringArray::Set(Label().FindChild(ChildLab_DescriptionNames), 1, theObject->NbDescriptions());
        if (!aDescriptions.IsNull()) aDescriptions->ChangeArray(aDescrArr);
        if (!aDescriptionNames.IsNull()) aDescriptionNames->ChangeArray(aDescrNameArr);
    }
}
...
Handle(XCAFDimTolObjects_DimensionObject) XCAFDoc_Dimension::GetObject() const {
    ...
    if (Label().FindAttribute(TDataStd_Name::GetID(), aSemanticNameAttr)) {
        ...
        if (!aName.IsEmpty()) aSemanticName = new TCollection_HAsciiString(aName);
    }
    ...
    Handle(TDataStd_RealArray) aVal;
    if (Label().FindChild(ChildLab_Value).FindAttribute(TDataStd_RealArray::GetID(), aVal) && !aVal->Array().IsNull()) {
    ...
    if (Label().FindChild(ChildLab_Path).FindAttribute(TNaming_NamedShape::GetID(), aShape) && !aShape.IsNull()) {
        anObj->SetPath(TopoDS::Edge(aShape->Get()));
    }

    Handle(TDataStd_RealArray) aDir;
    if (Label().FindChild(ChildLab_Dir).FindAttribute(TDataStd_RealArray::GetID(), aDir) && !aDir->Array().IsNull() &&
        aDir->Array()->Length() > 0) {
    ...
    if (Label().FindChild(ChildLab_PntText).FindAttribute(TDataStd_RealArray::GetID(), aPntText) &&
        aPntText->Length() == 3) {
        gp_Pnt aP(aPntText->Value(aPntText->Lower()), aPntText->Value(aPntText->Lower() + 1),
    ...
    if (aLPres.FindAttribute(TNaming_NamedShape::GetID(), aNS)) {
        ...
        if (!aPresentation.IsNull()) {
            ...
            if (aLPres.FindAttribute(TDataStd_Name::GetID(), aNameAtrr)) {
                ...
                if (!aName.IsEmpty()) aPresentName = new TCollection_HAsciiString(aName);
            }
