...
const Handle(XCAFDoc_ShapeTool) & XCAFDoc_DimTolTool::ShapeTool() {
    if (myShapeTool.IsNull()) myShapeTool = XCAFDoc_DocumentTool::ShapeTool(Label());
    return myShapeTool;
}
...
Standard_Boolean XCAFDoc_DimTolTool::FindDimTol(const Standard_Integer kind, const Handle(TColStd_HArray1OfReal) & aVal,
    ...
    for (; it.More(); it.Next()) {
        ...
        if (kind < 20) { // dimension
            for (Standard_Integer i = 1; i <= aVal->Length(); i++) {
                if (Abs(aVal->Value(i) - aVal1->Value(i)) > Precision::Confusion()) IsEqual = Standard_False;
            }
        } else if (kind < 50) { // tolerance
            if (Abs(aVal->Value(1) - aVal1->Value(1)) > Precision::Confusion()) IsEqual = Standard_False;
        }
        if (IsEqual) {
            lab = DimTolL;
            return Standard_True;
...
TDF_Label XCAFDoc_DimTolTool::FindDimTol(const Standard_Integer kind, const Handle(TColStd_HArray1OfReal) & aVal,
                                         const Handle(TCollection_HAsciiString) & aName,
...
TDF_Label XCAFDoc_DimTolTool::AddDimTol(const Standard_Integer kind, const Handle(TColStd_HArray1OfReal) & aVal,
                                        const Handle(TCollection_HAsciiString) & aName,
                                        const Handle(TCollection_HAsciiString) & aDescription) const {
    TDF_Label DimTolL;
    TDF_TagSource aTag;
...
void XCAFDoc_DimTolTool::SetDimension(const TDF_Label& theFirstL, const TDF_Label& theSecondL,
                                      const TDF_Label& theDimTolL) const {
    TDF_LabelSequence aFirstLS, aSecondLS;
    if (!theFirstL.IsNull()) aFirstLS.Append(theFirstL);
    if (!theSecondL.IsNull()) aSecondLS.Append(theSecondL);
...
void XCAFDoc_DimTolTool::SetDimension(const TDF_LabelSequence& theFirstL, const TDF_LabelSequence& theSecondL,
                                      const TDF_Label& theDimTolL) const {
    ...
    if (theDimTolL.FindAttribute(XCAFDoc::DimensionRefFirstGUID(), aChGNode)) {
        while (aChGNode->NbFathers() > 0) {
            ...
            if (aFGNode->NbChildren() == 0) aFGNode->ForgetAttribute(XCAFDoc::DimensionRefFirstGUID());
        }
    ...
    if (theDimTolL.FindAttribute(XCAFDoc::DimensionRefSecondGUID(), aChGNode)) {
        while (aChGNode->NbFathers() > 0) {
            aFGNode = aChGNode->GetFather(1);
            aFGNode->UnSetChild(aChGNode);
            if (aFGNode->NbChildren() == 0) aFGNode->ForgetAttribute(XCAFDoc::DimensionRefSecondGUID());
        }
...
void XCAFDoc_DimTolTool::SetGeomTolerance(const TDF_LabelSequence& theL, const TDF_Label& theGeomTolL) const {
    //  // set reference
    ...
    if (theGeomTolL.FindAttribute(XCAFDoc::GeomToleranceRefGUID(), aChGNode)) {
        while (aChGNode->NbFathers() > 0) {
            aFGNode = aChGNode->GetFather(1);
            aFGNode->UnSetChild(aChGNode);
            if (aFGNode->NbChildren() == 0) aFGNode->ForgetAttribute(XCAFDoc::GeomToleranceRefGUID());
        }
...
TDF_Label XCAFDoc_DimTolTool::SetDimTol(const TDF_Label& L, const Standard_Integer kind,
...
Standard_Boolean XCAFDoc_DimTolTool::GetRefShapeLabel(const TDF_Label& theL, TDF_LabelSequence& theShapeLFirst,
...
Standard_Boolean XCAFDoc_DimTolTool::GetRefGeomToleranceLabels(const TDF_Label& theShapeL,
                                                               TDF_LabelSequence& theDimTols) const {
    Handle(XCAFDoc_GraphNode) aGNode;
    if (!theShapeL.FindAttribute(XCAFDoc::GeomToleranceRefGUID(), aGNode) || aGNode->NbChildren() == 0) {
        return Standard_False;
    }
    for (Standard_Integer i = 1; i <= aGNode->NbChildren(); i++) {
        theDimTols.Append(aGNode->GetChild(i)->Label());
...
Standard_Boolean XCAFDoc_DimTolTool::GetRefDatumLabel(const TDF_Label& theShapeL, TDF_LabelSequence& theDatum) const {
    Handle(XCAFDoc_GraphNode) aGNode;
...
Standard_Boolean XCAFDoc_DimTolTool::GetDimTol(const TDF_Label& DimTolL, Standard_Integer& kind,
...
Standard_Boolean XCAFDoc_DimTolTool::FindDatum(const Handle(TCollection_HAsciiString) & aName,
    ...
    for (; it.More(); it.Next()) {
        ...
        if (!GetDatum(aLabel, aName1, aDescription1, anIdentification1)) continue;
        if (!(aName == aName1)) continue;
        if (!(aDescription == aDescription1)) continue;
...
void XCAFDoc_DimTolTool::SetDatum(const TDF_LabelSequence& theL, const TDF_Label& theDatumL) const {
    ...
    if (theDatumL.FindAttribute(XCAFDoc::DatumRefGUID(), aChGNode)) {
        while (aChGNode->NbFathers() > 0) {
            aFGNode = aChGNode->GetFather(1);
            aFGNode->UnSetChild(aChGNode);
            if (aFGNode->NbChildren() == 0) aFGNode->ForgetAttribute(XCAFDoc::DatumRefGUID());
        }
...
void XCAFDoc_DimTolTool::SetDatum(const TDF_Label& L, const TDF_Label& TolerL,
                                  const Handle(TCollection_HAsciiString) & aName,
                                  const Handle(TCollection_HAsciiString) & aDescription,
                                  const Handle(TCollection_HAsciiString) & anIdentification) const {
    TDF_Label DatumL;
...
void XCAFDoc_DimTolTool::SetDatumToGeomTol(const TDF_Label& theDatumL, const TDF_Label& theTolerL) const {
    // set reference
    Handle(XCAFDoc_GraphNode) aFGNode;
    Handle(XCAFDoc_GraphNode) aChGNode;
...
Standard_Boolean XCAFDoc_DimTolTool::GetDatum(const TDF_Label& theDatumL, Handle(TCollection_HAsciiString) & theName,
                                              Handle(TCollection_HAsciiString) & theDescription,
                                              Handle(TCollection_HAsciiString) & theIdentification) const {
    Handle(XCAFDoc_Datum) aDatumAttr;
    if (theDatumL.IsNull() || !theDatumL.FindAttribute(XCAFDoc_Datum::GetID(), aDatumAttr)) return Standard_False;
...
Standard_Boolean XCAFDoc_DimTolTool::GetDatumOfTolerLabels(const TDF_Label& theDimTolL,
    ...
    if (!theDimTolL.FindAttribute(XCAFDoc::DatumTolRefGUID(), aNode)) return Standard_False;
...
Standard_Boolean XCAFDoc_DimTolTool::GetDatumWithObjectOfTolerLabels(const TDF_Label& theDimTolL,
    ...
    for (Standard_Integer i = 1; i <= aNode->NbChildren(); i++) {
        ...
        Handle(XCAFDoc_Datum) aDatumAttr;
        if (!aDatumL.FindAttribute(XCAFDoc_Datum::GetID(), aDatumAttr)) continue;
...
Standard_Boolean XCAFDoc_DimTolTool::GetTolerOfDatumLabels(const TDF_Label& theDatumL,
    ...
    Handle(XCAFDoc_GraphNode) aNode;
    if (!theDatumL.FindAttribute(XCAFDoc::DatumTolRefGUID(), aNode)) return Standard_False;
...
void XCAFDoc_DimTolTool::GetGDTPresentations(
    NCollection_IndexedDataMap<TDF_Label, TopoDS_Shape, TDF_LabelMapHasher>& theGDTLabelToShape) const {
    ...
    for (Standard_Integer i = 1; i <= aGDTs.Length(); i++) {
        ...
        if (!aCL.FindAttribute(XCAFDoc_Dimension::GetID(), aDimAttr)) continue;
        ...
        if (!aShape.IsNull()) theGDTLabelToShape.Add(aCL, aShape);
    }
    ...
    for (Standard_Integer i = 1; i <= aGDTs.Length(); i++) {
        ...
        const TDF_Label& aCL = aGDTs.Value(i);
        if (!aCL.FindAttribute(XCAFDoc_GeomTolerance::GetID(), aGTAttr)) continue;
    ...
    for (Standard_Integer i = 1; i <= aGDTs.Length(); i++) {
        ...
        const TDF_Label& aCL = aGDTs.Value(i);
        if (!aCL.FindAttribute(XCAFDoc_Datum::GetID(), aGTAttr)) continue;
...
void XCAFDoc_DimTolTool::SetGDTPresentations(
    NCollection_IndexedDataMap<TDF_Label, TopoDS_Shape, TDF_LabelMapHasher>& theGDTLabelToPrs) {
