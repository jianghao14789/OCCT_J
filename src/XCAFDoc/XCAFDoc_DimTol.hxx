...
public:
    ...
    Standard_EXPORT static Handle(XCAFDoc_DimTol)
        Set(const TDF_Label& label, const Standard_Integer kind, const Handle(TColStd_HArray1OfReal) & aVal,
            const Handle(TCollection_HAsciiString) & aName, const Handle(TCollection_HAsciiString) & aDescription);

    Standard_EXPORT void Set(const Standard_Integer kind, const Handle(TColStd_HArray1OfReal) & aVal,
