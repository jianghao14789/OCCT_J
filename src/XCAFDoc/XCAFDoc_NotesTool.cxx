...
enum NotesTool_RootLabels { NotesTool_NotesRoot = 1, NotesTool_AnnotatedItemsRoot };
...
Standard_Integer XCAFDoc_NotesTool::NbNotes() const {
    ...
    for (TDF_ChildIterator anIter(GetNotesLabel()); anIter.More(); anIter.Next()) {
        ...
        if (!XCAFDoc_Note::Get(aLabel).IsNull()) ++nbNotes;
    }
    return nbNotes;
}
...
Standard_Integer XCAFDoc_NotesTool::NbAnnotatedItems() const {
    ...
    for (TDF_ChildIDIterator anIter(GetAnnotatedItemsLabel(), XCAFDoc_AssemblyItemRef::GetID()); anIter.More();
         anIter.Next()) {
        ++nbItems;
    }
    return nbItems;
...
void XCAFDoc_NotesTool::GetNotes(TDF_LabelSequence& theNoteLabels) const {
    for (TDF_ChildIterator anIter(GetNotesLabel()); anIter.More(); anIter.Next()) {
        const TDF_Label aLabel = anIter.Value();
        if (!XCAFDoc_Note::Get(aLabel).IsNull()) theNoteLabels.Append(aLabel);
    }
}
...
// purpose  :
// =======================================================================
TDF_Label XCAFDoc_NotesTool::FindAnnotatedItemAttr(const TDF_Label& theItemLabel, const Standard_GUID& theGUID) const {
...
Standard_Integer XCAFDoc_NotesTool::GetNotes(const XCAFDoc_AssemblyItemId& theItemId,
    ...
    if (anAnnotatedItem.IsNull()) return 0;

    Handle(XCAFDoc_GraphNode) aChild;
    if (!anAnnotatedItem.FindAttribute(XCAFDoc::NoteRefGUID(), aChild)) return 0;
...
// purpose  :
// =======================================================================
Standard_Integer XCAFDoc_NotesTool::GetAttrNotes(const XCAFDoc_AssemblyItemId& theItemId, const Standard_GUID& theGUID,
                                                 TDF_LabelSequence& theNoteLabels) const {
...
// purpose  :
// =======================================================================
Standard_Integer XCAFDoc_NotesTool::GetAttrNotes(const TDF_Label& theItemLabel, const Standard_GUID& theGUID,
                                                 TDF_LabelSequence& theNoteLabels) const {
...
Handle(XCAFDoc_AssemblyItemRef) XCAFDoc_NotesTool::AddNote(const TDF_Label& theNoteLabel,
    ...
    if (!XCAFDoc_Note::IsMine(theNoteLabel)) return anItemRef;
    ...
    if (anAnnotatedItem.IsNull()) {
        ...
        if (anAnnotatedItem.IsNull()) return anItemRef;
    }
    ...
    if (!anAnnotatedItem.FindAttribute(XCAFDoc::NoteRefGUID(), aChild)) {
        ...
        if (aChild.IsNull()) return anItemRef;
    }
    ...
    if (!anAnnotatedItem.FindAttribute(XCAFDoc_AssemblyItemRef::GetID(), anItemRef)) {
        ...
        if (anItemRef.IsNull()) return anItemRef;
    }
    ...
    if (!theNoteLabel.FindAttribute(XCAFDoc::NoteRefGUID(), aFather)) {
        ...
        if (aFather.IsNull()) return anItemRef;
    }
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveNote(const TDF_Label& theNoteLabel, const XCAFDoc_AssemblyItemId& theItemId,
    ...
    Handle(XCAFDoc_GraphNode) aFather;
    if (!theNoteLabel.FindAttribute(XCAFDoc::NoteRefGUID(), aFather)) return Standard_False;

    TDF_Label anAnnotatedItem = FindAnnotatedItem(theItemId);
    if (anAnnotatedItem.IsNull()) return Standard_False;

    Handle(XCAFDoc_GraphNode) aChild;
    if (!anAnnotatedItem.FindAttribute(XCAFDoc::NoteRefGUID(), aChild)) return Standard_False;

    aChild->UnSetFather(aFather);
    if (aChild->NbFathers() == 0) anAnnotatedItem.ForgetAllAttributes();

    if (theDelIfOrphan && aNote->IsOrphan()) DeleteNote(theNoteLabel);
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveNote(const TDF_Label& theNoteLabel, const TDF_Label& theItemLabel,
                                               Standard_Boolean theDelIfOrphan) {
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveSubshapeNote(const TDF_Label& theNoteLabel, const TDF_Label& theItemLabel,
                                                       Standard_Integer theSubshapeIndex,
                                                       Standard_Boolean theDelIfOrphan) {
...
Standard_Boolean XCAFDoc_NotesTool::RemoveAttrNote(const TDF_Label& theNoteLabel,
                                                   const XCAFDoc_AssemblyItemId& theItemId,
                                                   const Standard_GUID& theGUID, Standard_Boolean theDelIfOrphan) {
    Handle(XCAFDoc_Note) aNote = XCAFDoc_Note::Get(theNoteLabel);
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveAttrNote(const TDF_Label& theNoteLabel, const TDF_Label& theItemLabel,
                                                   const Standard_GUID& theGUID, Standard_Boolean theDelIfOrphan) {
...
Standard_Boolean XCAFDoc_NotesTool::RemoveAllNotes(const XCAFDoc_AssemblyItemId& theItemId,
    ...
    while (aChild->NbFathers() > 0) {
        ...
        if (!aNote.IsNull()) {
            ...
            if (theDelIfOrphan && aNote->IsOrphan()) DeleteNote(aFather->Label());
        }
    }
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveAllNotes(const TDF_Label& theItemLabel, Standard_Boolean theDelIfOrphan) {
...
// purpose  :
// =======================================================================
Standard_Boolean XCAFDoc_NotesTool::RemoveAllAttrNotes(const TDF_Label& theItemLabel, const Standard_GUID& theGUID,
                                                       Standard_Boolean theDelIfOrphan) {
    ...
    if (!aNote.IsNull()) {
        ...
        if (theNoteLabel.FindAttribute(XCAFDoc::NoteRefGUID(), aFather) && !aFather.IsNull()) {
            while (aFather->NbChildren() > 0) {
                ...
                aFather->UnSetChild(aChild);
                if (aChild->NbFathers() == 0) aChild->Label().ForgetAllAttributes(Standard_True);
            }
        }
...
Standard_Integer XCAFDoc_NotesTool::DeleteNotes(TDF_LabelSequence& theNoteLabels) {
    ...
    for (TDF_LabelSequence::Iterator anIter(theNoteLabels); anIter.More(); anIter.Next()) {
        if (DeleteNote(anIter.Value())) ++nbNotes;
    }
    return nbNotes;
}
...
Standard_Integer XCAFDoc_NotesTool::NbOrphanNotes() const {
    ...
    for (TDF_ChildIterator anIter(GetNotesLabel()); anIter.More(); anIter.Next()) {
        ...
        if (!aNote.IsNull() && aNote->IsOrphan()) ++nbNotes;
    }
    return nbNotes;
}
...
void XCAFDoc_NotesTool::GetOrphanNotes(TDF_LabelSequence& theNoteLabels) const {
    for (TDF_ChildIterator anIter(GetNotesLabel()); anIter.More(); anIter.Next()) {
        const TDF_Label aLabel = anIter.Value();
        Handle(XCAFDoc_Note) aNote = XCAFDoc_Note::Get(aLabel);
        if (!aNote.IsNull() && aNote->IsOrphan()) theNoteLabels.Append(aLabel);
    }
}
...
Standard_Integer XCAFDoc_NotesTool::DeleteOrphanNotes() {
    ...
    for (TDF_ChildIterator anIter(GetNotesLabel()); anIter.More(); anIter.Next()) {
        const TDF_Label aLabel = anIter.Value();
        Handle(XCAFDoc_Note) aNote = XCAFDoc_Note::Get(aLabel);
        if (!aNote.IsNull() && aNote->IsOrphan() && DeleteNote(aLabel)) ++nbNotes;
    }
    return nbNotes;
}
