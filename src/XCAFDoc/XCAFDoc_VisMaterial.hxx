...
//! It is expected to correlate with physical material properties (XCAFDoc_Material), but not necessarily (like
//! painted/polished/rusty object).
//!
...
//! PBR metallic-roughness model is closer to physical material properties, and intended to be used within
//! physically-based renderer.
//!
//! For compatibility reasons, this attribute allows defining both material models,
//! so that it is up-to Data Exchange and Application deciding which one to define and use for rendering (depending on
//! viewer capabilities). Automatic conversion from one model to another is possible, but lossy (converted material will
//! not look the same).
//!
//! Within Data Exchange, different file formats have different capabilities for storing visualization material
//! properties from simple color (STEP, IGES), to common (OBJ, glTF 1.0) and PBR (glTF 2.0). This should be taken into
//! account while defining or converting document into one or another format - material definition might be lost or
//! disturbed.
//!
...
public:
    ...
    bool IsEmpty() const {
        return !myPbrMat.IsDefined && !myCommonMat.IsDefined;
    }
    ...
    //! Set alpha mode.
    Standard_EXPORT void SetAlphaMode(Graphic3d_AlphaMode theMode, Standard_ShortReal theCutOff = 0.5f);
    ...
    Standard_DEPRECATED("Deprecated method, FaceCulling() should be used instead") Standard_Boolean
        IsDoubleSided() const {
        return myFaceCulling == Graphic3d_TypeOfBackfacingModel_DoubleSided;
    }

    Standard_DEPRECATED("Deprecated method, SetFaceCulling() should be used instead") void SetDoubleSided(
        Standard_Boolean theIsDoubleSided) {
