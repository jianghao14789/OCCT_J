...
Standard_Boolean XCAFDoc_ClippingPlaneTool::IsClippingPlane(const TDF_Label& theLabel) const {
    if (theLabel.Father() != Label()) return Standard_False;

    Handle(TDataXtd_Plane) aPlaneAttribute;
    if (!theLabel.FindAttribute(TDataXtd_Plane::GetID(), aPlaneAttribute)) return Standard_False;
...
Standard_Boolean XCAFDoc_ClippingPlaneTool::GetClippingPlane(const TDF_Label& theLabel, gp_Pln& thePlane,
    ...
    if (theLabel.FindAttribute(TDataStd_Name::GetID(), aNameAttribute)) theName = aNameAttribute->Get();
...
Standard_Boolean XCAFDoc_ClippingPlaneTool::GetClippingPlane(const TDF_Label& theLabel, gp_Pln& thePlane,
    ...
    if (!GetClippingPlane(theLabel, thePlane, anExtName, theCapping)) return Standard_False;
...
TDF_Label XCAFDoc_ClippingPlaneTool::AddClippingPlane(const gp_Pln thePlane,
    ...
    for (Standard_Integer i = 1; i <= aClippingPlanes.Length(); i++) {
        ...
        if (!aName.IsEqual(theName)) continue;
        if (aPlane.Axis().Angle(thePlane.Axis()) > Precision::Angular()) continue;
        if (aPlane.XAxis().Angle(thePlane.XAxis()) > Precision::Angular()) continue;
        if (aPlane.YAxis().Angle(thePlane.YAxis()) > Precision::Angular()) continue;
    ...
    if (!theName.IsEmpty()) TDataStd_Name::Set(aLabel, theName);
...
TDF_Label XCAFDoc_ClippingPlaneTool::AddClippingPlane(const gp_Pln thePlane, const TCollection_ExtendedString theName,
...
Standard_Boolean XCAFDoc_ClippingPlaneTool::RemoveClippingPlane(const TDF_Label& theLabel) const {
    ...
    if (!IsClippingPlane(theLabel) || theLabel.FindAttribute(XCAFDoc::ViewRefPlaneGUID(), Node)) return Standard_False;
...
void XCAFDoc_ClippingPlaneTool::UpdateClippingPlane(const TDF_Label& theLabel, const gp_Pln thePlane,
                                                    const TCollection_ExtendedString theName) const {
    if (theLabel.Father() != Label()) return;

    Handle(TDataXtd_Plane) aPlaneAttribute;
    if (!theLabel.FindAttribute(TDataXtd_Plane::GetID(), aPlaneAttribute)) return;
...
void XCAFDoc_ClippingPlaneTool::SetCapping(const TDF_Label& theClippingPlaneL, const Standard_Boolean theCapping) {
    if (theClippingPlaneL.Father() != Label()) return;
...
Standard_Boolean XCAFDoc_ClippingPlaneTool::GetCapping(const TDF_Label& theClippingPlaneL) const {
    if (theClippingPlaneL.Father() != Label()) return Standard_False;
