...
Standard_Boolean XCAFDoc_ShapeTool::Search(const TopoDS_Shape& S, TDF_Label& L, const Standard_Boolean findInstance,
    ...
    if (isLocated) {
        ...
        if (findInstance && FindShape(S, L, Standard_True)) return Standard_True;
...
Standard_Boolean XCAFDoc_ShapeTool::FindShape(const TopoDS_Shape& S, TDF_Label& L,
    ...
    if (IsTopLevel(L)) return Standard_True;
    ...
    for (; it.More(); it.Next()) {
        ...
        if (aLabel.FindAttribute(TNaming_NamedShape::GetID(), NS) && S0.IsSame(TNaming_Tool::GetShape(NS))) {
            L = aLabel;
            return Standard_True;
        }
    }
...
TDF_Label XCAFDoc_ShapeTool::FindShape(const TopoDS_Shape& S, const Standard_Boolean findInstance) const {
    TDF_Label L;
    if (FindShape(S, L, findInstance)) return L;
    return TDF_Label();
}
...
void XCAFDoc_ShapeTool::MakeReference(const TDF_Label& L, const TDF_Label& refL, const TopLoc_Location& loc) {
    ...
    if (theAutoNaming) SetLabelNameByLink(L);
}
...
TDF_Label XCAFDoc_ShapeTool::addShape(const TopoDS_Shape& S, const Standard_Boolean makeAssembly) {
    ...
    if (theAutoNaming) SetLabelNameByShape(ShapeLabel);
    ...
    if (makeAssembly && S.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (theAutoNaming) TDataStd_Name::Set(ShapeLabel, TCollection_ExtendedString("ASSEMBLY"));
...
static Standard_Boolean prepareAssembly(const TopoDS_Shape& theShape, TopoDS_Shape& theOUTShape) {
    ...
    if (theShape.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (!theOUTShape.Free()) theOUTShape.Free(Standard_True);
...
TDF_Label XCAFDoc_ShapeTool::AddShape(const TopoDS_Shape& theShape, const Standard_Boolean makeAssembly,
                                      const Standard_Boolean makePrepare) {
    // PTV 17.02.2003 to avoid components without location.
    TopoDS_Shape S = theShape;
    if (makePrepare && makeAssembly && S.ShapeType() == TopAbs_COMPOUND) prepareAssembly(theShape, S); // OCC1669
...
Standard_Boolean XCAFDoc_ShapeTool::RemoveShape(const TDF_Label& L, const Standard_Boolean removeCompletely) const {
...
Standard_Boolean XCAFDoc_ShapeTool::IsFree(const TDF_Label& L) {
    Handle(TDataStd_TreeNode) Node;
    if (!L.FindAttribute(XCAFDoc::ShapeRefGUID(), Node) || !Node->HasFirst()) return Standard_True;
...
Standard_Integer XCAFDoc_ShapeTool::GetUsers(const TDF_Label& L, TDF_LabelSequence& Labels,
...
Standard_Integer XCAFDoc_ShapeTool::NbComponents(const TDF_Label& L, const Standard_Boolean getsubchilds) {
...
Standard_Boolean XCAFDoc_ShapeTool::GetComponents(const TDF_Label& L, TDF_LabelSequence& Labels,
    ...
    for (; It.More(); It.Next()) {
        ...
        if (IsComponent(comp)) {
            if (getsubchilds) {
                TDF_Label underL;
                if (GetReferredShape(comp, underL)) GetComponents(underL, Labels, getsubchilds);
            }
            Labels.Append(comp);
        }
    }
...
TopLoc_Location XCAFDoc_ShapeTool::GetLocation(const TDF_Label& L) {
    Handle(XCAFDoc_Location) LocationAttribute;
    if (L.FindAttribute(XCAFDoc_Location::GetID(), LocationAttribute)) return LocationAttribute->Get();
...
TDF_Label XCAFDoc_ShapeTool::AddComponent(const TDF_Label& assembly, const TDF_Label& compL,
    ...
    if (GetShape(L, aShape)) {
        if (!myShapeLabels.IsBound(aShape)) myShapeLabels.Bind(aShape, L);
    }
...
TDF_Label XCAFDoc_ShapeTool::AddComponent(const TDF_Label& assembly, const TopoDS_Shape& comp,
...
Standard_Boolean XCAFDoc_ShapeTool::IsSubShape(const TDF_Label& shapeL, const TopoDS_Shape& sub) const {
...
Standard_Boolean XCAFDoc_ShapeTool::FindSubShape(const TDF_Label& shapeL, const TopoDS_Shape& sub, TDF_Label& L) const {
    ...
    for (; aChldLabIt.More(); aChldLabIt.Next()) {
        ...
        Handle(TNaming_NamedShape) NS;
        if (!aSubLabel.FindAttribute(TNaming_NamedShape::GetID(), NS)) continue;
...
Standard_Boolean XCAFDoc_ShapeTool::AddSubShape(const TDF_Label& shapeL, const TopoDS_Shape& sub,
                                                TDF_Label& addedSubShapeL) const {
    addedSubShapeL = TDF_Label();
    // Check if adding subshape is possible
    if (!IsSimpleShape(shapeL) || !IsTopLevel(shapeL)) return Standard_False;
    ...
    if (!isDefined) return Standard_False;

    // Try to find already existed subshape
    if (FindSubShape(shapeL, aSubShape, addedSubShapeL)) return Standard_False;
...
TDF_Label XCAFDoc_ShapeTool::FindMainShapeUsingMap(const TopoDS_Shape& sub) const {
    ...
    // }
    if (mySubShapes.IsBound(sub)) return mySubShapes.Find(sub);
    TDF_Label L0;
    return L0;
}
...
static void DumpAssembly(Standard_OStream& theDumpLog, const TDF_Label L, const Standard_Integer level,
    ...
    if (XCAFDoc_ShapeTool::IsAssembly(L)) {
        ...
        if (L.Father().Father().Father().IsRoot()) theDumpLog << "PART ";
    } else {
        theDumpLog << "INSTANCE ";
    }
    ...
    Handle(TDataStd_Name) Name;
    if (L.FindAttribute(TDataStd_Name::GetID(), Name)) theDumpLog << " \"" << Name->Get() << "\" ";

    if (deep) {
        theDumpLog << "(" << *(void**)&S.TShape();
        if (!S.Location().IsIdentity()) theDumpLog << ", " << *(void**)&S.Location();
        theDumpLog << ") ";
    }
    theDumpLog << std::endl;
    ...
    if (level == 0) theDumpLog << std::endl;
}
...
void XCAFDoc_ShapeTool::DumpShape(Standard_OStream& theDumpLog, const TDF_Label& L, const Standard_Integer level,
                                  const Standard_Boolean deep) {
    TopoDS_Shape S;
...
void XCAFDoc_ShapeTool::SetExternRefs(const TDF_Label& L, const TColStd_SequenceOfHAsciiString& SHAS) const {
...
void XCAFDoc_ShapeTool::GetExternRefs(const TDF_Label& L, TColStd_SequenceOfHAsciiString& SHAS) {
    ...
    for (Standard_Integer i = 1; i <= L.NbChildren(); i++) {
        ...
        if (tmplbl.FindAttribute(TDataStd_Name::GetID(), TDN)) {
            ...
            Handle(TCollection_HAsciiString) str = new TCollection_HAsciiString(TCollection_AsciiString(extstr));
            SHAS.Append(str);
        }
    }
}
...
Standard_Boolean XCAFDoc_ShapeTool::GetSHUO(const TDF_Label& SHUOLabel, Handle(XCAFDoc_GraphNode) & aSHUOAttr) {
    if (!SHUOLabel.FindAttribute(XCAFDoc::SHUORefGUID(), aSHUOAttr)) return Standard_False;
    return Standard_True;
}
...
Standard_Boolean XCAFDoc_ShapeTool::GetAllComponentSHUO(const TDF_Label& theCompLabel,
    ...
    for (; it.More(); it.Next()) {
        ...
        if (GetSHUO(L, aSHUOAttr)) theSHUOAttrs.Append(aSHUOAttr);
    }
...
Standard_Boolean XCAFDoc_ShapeTool::SetSHUO(const TDF_LabelSequence& labels,
    ...
    if (labels.Length() < 2) return Standard_False;
    // check is all labels contains components of any assemblyies
    Standard_Integer i;
    for (i = 1; i <= labels.Length(); i++)
        if (!IsComponent(labels.Value(i))) return Standard_False;
...
Standard_Boolean XCAFDoc_ShapeTool::GetSHUOUpperUsage(const TDF_Label& NextUsageL, TDF_LabelSequence& aLabels) {
    Handle(XCAFDoc_GraphNode) aNextSHUO;
    if (!GetSHUO(NextUsageL, aNextSHUO) || aNextSHUO->NbFathers() < 1) return Standard_False;
...
Standard_Boolean XCAFDoc_ShapeTool::GetSHUONextUsage(const TDF_Label& UpperUsageL, TDF_LabelSequence& aLabels) {
    Handle(XCAFDoc_GraphNode) aUpperSHUO;
    if (!GetSHUO(UpperUsageL, aUpperSHUO) || aUpperSHUO->NbChildren() < 1) return Standard_False;
    // get upper_usage SHAO
...
static Standard_Boolean checkForShape(const TopoDS_Shape& theShape, const TopoDS_Shape& theCurSh,
                                      const TDF_Label& theUserL, TDF_LabelSequence& theLabels) {
...
Standard_Boolean XCAFDoc_ShapeTool::FindComponent(const TopoDS_Shape& theShape, TDF_LabelSequence& theLabels) const {
    theLabels.Clear();
    ...
    if (aRefL.IsNull()) return Standard_False; // cannot find top-level shape.

    TDF_LabelSequence aUsers;
    ::XCAFDoc_ShapeTool::GetUsers(aRefL, aUsers);
    for (Standard_Integer i = 1; i <= aUsers.Length(); i++)
        if (checkForShape(theShape, S0, aUsers.Value(i), theLabels)) break;
...
static Standard_Boolean getShapesOfSHUO(TopLoc_IndexedMapOfLocation& theaPrevLocMap,
                                        const Handle(XCAFDoc_ShapeTool) & theSTool, const TDF_Label& theSHUOlab,
    ...
    if (aLabSeq.Length() >= 1)
        ...
        if (intMapLenght < 1) return Standard_False; // should not be, but to avoid exception...?
...
TopoDS_Shape XCAFDoc_ShapeTool::GetSHUOInstance(const Handle(XCAFDoc_GraphNode) & theSHUO) const {
    TopoDS_Shape aShape;
    if (theSHUO.IsNull()) return aShape;
    ...
    if (!loc.IsIdentity()) aPrevLocMap.Add(loc);
...
static Standard_Boolean getUsersShapesOfSHUO(TopLoc_IndexedMapOfLocation& aPrevLocMap,
                                             const Handle(XCAFDoc_ShapeTool) & STool, const TDF_Label& aSHUOlab,
                                             const TDF_Label& theUserL, TopTools_SequenceOfShape& theSHUOShapeSeq) {
    TopLoc_IndexedMapOfLocation aNewPrevLocMap;
    // get location of the assembly
...
Standard_Boolean XCAFDoc_ShapeTool::GetAllSHUOInstances(const Handle(XCAFDoc_GraphNode) & theSHUO,
    ...
    if (theSHUO.IsNull()) return Standard_False;
...
Standard_Boolean XCAFDoc_ShapeTool::FindSHUO(const TDF_LabelSequence& theLabels,
    ...
    if (!::XCAFDoc_ShapeTool::GetAllComponentSHUO(aCompLabel, SHUOAttrs)) return Standard_False;
...
Standard_Boolean XCAFDoc_ShapeTool::Expand(const TDF_Label& theShapeL) {
    if (theShapeL.IsNull() || IsAssembly(theShapeL)) return Standard_False;
...
void XCAFDoc_ShapeTool::makeSubShape(const TDF_Label& theMainShapeL, const TDF_Label& thePart,
                                     const TopoDS_Shape& theShape, const TopLoc_Location& theLoc) {
    ...
    for (; anIter.More(); anIter.Next()) {
        ...
        if (!aChildLabel.IsNull()) {
            ...
            if (isCompoundPart) aSubLoc = theLoc.Inverted() * aSubLoc;
            // make subshape
            TDF_Label aSubLabel;
...
Standard_Boolean XCAFDoc_ShapeTool::updateComponent(const TDF_Label& theItemLabel, TopoDS_Shape& theUpdatedShape,
                                                    TDF_LabelMap& theUpdated) const {
    if (!IsAssembly(theItemLabel)) return Standard_False; // Do nothing for non-assemblies
    ...
    //
    if (aNumTopoComponents != aComponentLabs.Length()) isModified = Standard_True;
    ...
    if (isModified) theUpdated.Add(theItemLabel);
...
Handle(TDataStd_NamedData) XCAFDoc_ShapeTool::GetNamedProperties(const TopoDS_Shape& theShape,
    ...
    Handle(TDataStd_NamedData) aNamedProperty;
    TDF_Label aLabel;
    if (!Search(theShape, aLabel)) return aNamedProperty;
