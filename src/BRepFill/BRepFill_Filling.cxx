...
// purpose  :
//======================================================================
BRepFill_Filling::BRepFill_Filling(const Standard_Integer Degree, const Standard_Integer NbPtsOnCur,
                                   const Standard_Integer NbIter, const Standard_Boolean Anisotropie,
                                   const Standard_Real Tol2d, const Standard_Real Tol3d, const Standard_Real TolAng,
                                   const Standard_Real TolCurv, const Standard_Integer MaxDeg,
...
// purpose  :
//======================================================================
void BRepFill_Filling::SetConstrParam(const Standard_Real Tol2d, const Standard_Real Tol3d, const Standard_Real TolAng,
...
// purpose  :
//======================================================================
void BRepFill_Filling::SetResolParam(const Standard_Integer Degree, const Standard_Integer NbPtsOnCur,
                                     const Standard_Integer NbIter, const Standard_Boolean Anisotropie) {
    myDegree = Degree;
    myNbPtsOnCur = NbPtsOnCur;
    myNbIter = NbIter;
...
// purpose  :
//======================================================================
void BRepFill_Filling::SetApproxParam(const Standard_Integer MaxDeg, const Standard_Integer MaxSegments) {
    myMaxDeg = MaxDeg;
    myMaxSegments = MaxSegments;
}
...
//======================================================================
Standard_Integer BRepFill_Filling::Add(const TopoDS_Edge& anEdge, const GeomAbs_Shape Order,
...
//======================================================================
Standard_Integer BRepFill_Filling::Add(const TopoDS_Edge& anEdge, const TopoDS_Face& Support, const GeomAbs_Shape Order,
                                       const Standard_Boolean IsBound) {
...
//======================================================================
Standard_Integer BRepFill_Filling::Add(const Standard_Real U, const Standard_Real V, const TopoDS_Face& Support,
    ...
    Handle(GeomPlate_PointConstraint) aPC =
        new GeomPlate_PointConstraint(U, V, BRep_Tool::Surface(HSurf->Face()), Order, myTol3d, myTolAng, myTolCurv);
    myPoints.Append(aPC);
    return (myBoundary.Length() + myFreeConstraints.Length() + myConstraints.Length() + myPoints.Length());
}
    ...
    for (i = 1; i <= SeqOfConstraints.Length(); i++) {
        ...
        if (CurFace.IsNull()) {
            if (CurOrder == GeomAbs_C0) {
                ...
                Constr =
                    new GeomPlate_CurveConstraint(HCurvOnSurf, CurOrder, myNbPtsOnCur, myTol3d, myTolAng, myTolCurv);
            }
        } else {
            ...
            Constr = new BRepFill_CurveConstraint(HCurvOnSurf, CurOrder, myNbPtsOnCur, myTol3d, myTolAng, myTolCurv);
        }
                ...
                for (Itl.Initialize(EdgeList); Itl.More(); Itl.Next()) {
                    ...
                    for (i = 0; i < 2; i++) {
                        for (j = 0; j < 2; j++) {
                            ...
                            if (aDist < BRep_Tool::Tolerance(V_wire[i]) && aDist < BRep_Tool::Tolerance(V_edge[j])) {
                                MW.Add(CurEdge);
                                TopoDS_Edge NewEdge = MW.Edge();
                                myOldNewMap.Bind(CurEdge.Oriented(TopAbs_FORWARD), NewEdge.Oriented(TopAbs_FORWARD));
                                EdgeList.Remove(Itl);
                                found = Standard_True;
                                break;
                            }
...
void BRepFill_Filling::FindExtremitiesOfHoles(const TopTools_ListOfShape& WireList,
    ...
    if (BRep_Tool::IsClosed(theWire)) return;
...
void BRepFill_Filling::Build() {
    myBuilder.reset(new GeomPlate_BuildPlateSurface(myDegree, myNbPtsOnCur, myNbIter, myTol2d, myTol3d, myTolAng,
                                                    myTolCurv, myAnisotropie));
    TopoDS_Edge CurEdge;
    TopoDS_Face CurFace;
    Standard_Integer i, j;
    ...
    for (j = 1; j <= myFreeConstraints.Length(); j++) {
        ...
        for (i = 1; i <= VerSeq.Length(); i += 2) {
            ...
            if (Projector.LowerDistance() > Precision::Confusion()) continue;
    ...
    if (!myIsInitFaceGiven) {
        ...
        // GeomConvert_ApproxSurface Approx( GPlate, myTol3d, GeomAbs_C1, GeomAbs_C1, myMaxDeg, myMaxDeg, myMaxSegments,
        // 1 ); Approx.Dump( std::cout );
        Surface = Approx.Surface();
    }
    ...
    for (i = 1; i <= myBoundary.Length(); i++) {
        ...
        if (myOldNewMap.IsBound(anEdge)) anEdge = TopoDS::Edge(myOldNewMap(anEdge));
...
const TopTools_ListOfShape& BRepFill_Filling::Generated(const TopoDS_Shape& S) {
    ...
    if (myOldNewMap.IsBound(S)) myGenerated.Append(myOldNewMap(S));
