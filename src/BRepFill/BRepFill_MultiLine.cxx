...
BRepFill_MultiLine::BRepFill_MultiLine(const TopoDS_Face& Face1, const TopoDS_Face& Face2, const TopoDS_Edge& Edge1,
                                       const TopoDS_Edge& Edge2, const Standard_Boolean Inv1,
                                       const Standard_Boolean Inv2, const Handle(Geom2d_Curve) & Bissec)
    : myFace1(Face1), myFace2(Face2), myBis(Bissec), myKPart(0) {
    //
    ...
    if (UU1->IsPeriodic()) {
        ElCLib::AdjustPeriodic(UU1->FirstParameter(), UU1->LastParameter(), Precision::PConfusion(), Umin, Umax);
    }
    if (VV1->IsPeriodic()) {
        ElCLib::AdjustPeriodic(VV1->FirstParameter(), VV1->LastParameter(), Precision::PConfusion(), Vmin, Vmax);
    }
    // end try duplication
    ...
    if (UU2->IsPeriodic()) {
        ElCLib::AdjustPeriodic(UU2->FirstParameter(), UU2->LastParameter(), Precision::PConfusion(), Umin, Umax);
    }
    if (VV2->IsPeriodic()) {
        ElCLib::AdjustPeriodic(VV2->FirstParameter(), VV2->LastParameter(), Precision::PConfusion(), Vmin, Vmax);
    }
    // end try duplication
    ...
    if (myBis.GetType() == GeomAbs_Line) {
        ...
        if (myIsoU1) {
            if (Abs(aPnt1.Y() - aPnt2.Y()) < eps) myKPart = 1;
        } else {
            if (Abs(aPnt1.X() - aPnt2.X()) < eps) myKPart = 1;
        }

        if (myKPart == 1) myCont = GeomAbs_G1;
...
void BRepFill_MultiLine::Curves(Handle(Geom_Curve) & Curve, Handle(Geom2d_Curve) & PCurve1,
    ...
    if (myKPart == 1) {
        ...
        Handle(Geom_TrimmedCurve) TLine = new Geom_TrimmedCurve(Line, myBis.FirstParameter(), myBis.LastParameter());
...
static gp_Pnt2d ValueOnFace(const Standard_Real U, const Geom2dAdaptor_Curve& TheBis, const Geom2dAdaptor_Curve& TheU,
                            const Geom2dAdaptor_Curve& TheV, const Standard_Boolean IsIsoU) {
...
void BRepFill_MultiLine::Value3dOnF1OnF2(const Standard_Real U, gp_Pnt& P3d, gp_Pnt2d& PF1, gp_Pnt2d& PF2) const {
...
Standard_Boolean BRepFill_MultiLine::Value(const Standard_Real theT, NCollection_Array1<gp_Pnt2d>& thePnt2d,
...
Standard_Boolean BRepFill_MultiLine::D1(const Standard_Real /*theT*/, NCollection_Array1<gp_Vec2d>& /*theVec2d*/,
