...
static void EdgesFromVertex(const TopoDS_Wire& W, const TopoDS_Vertex& V, TopoDS_Edge& E1, TopoDS_Edge& E2) {
...
static void AddNewEdge(const TopoDS_Shape& theEdge, const TopTools_DataMapOfShapeSequenceOfShape& theEdgeNewEdges,
...
static Standard_Boolean PlaneOfWire(const TopoDS_Wire& W, gp_Pln& P) {
    ...
    if (findPlanarSurf.Found()) {
        ...
        if ((Abs(RMax - R1) < Tol && Abs(RMax - R2) < Tol) || (Abs(RMax - R1) < Tol && Abs(RMax - R3) < Tol) ||
...
static void WireContinuity(const TopoDS_Wire& W, GeomAbs_Shape& contW) {
    ...
    if (!IsDegenerated) {
        ...
        for (Standard_Integer j = 1; j <= nbEdges; j++) {
            ...
            if (j == nbEdges) testconti = Curve1.Value(U1).IsEqual(Curve2.Value(U2), Eps);
...
static void TrimEdge(const TopoDS_Edge& CurrentEdge, const TColStd_SequenceOfReal& CutValues, const Standard_Real t0,
                     const Standard_Real t1, const Standard_Boolean SeqOrder, TopTools_SequenceOfShape& S)
...
static Standard_Boolean SearchRoot(const TopoDS_Vertex& V, const TopTools_DataMapOfShapeListOfShape& Map,
                                   TopoDS_Vertex& VRoot) {
    Standard_Boolean trouve = Standard_False;
    VRoot.Nullify();
...
static Standard_Boolean SearchVertex(const TopTools_ListOfShape& List, const TopoDS_Wire& W, TopoDS_Vertex& VonW) {
    Standard_Boolean trouve = Standard_False;
    VonW.Nullify();
...
static Standard_Boolean EdgeIntersectOnWire(const gp_Pnt& P1, const gp_Pnt& P2, Standard_Real percent,
                                            const TopTools_DataMapOfShapeListOfShape& Map, const TopoDS_Wire& W,
                                            TopoDS_Vertex& Vsol, TopoDS_Wire& newW,
...
static void Transform(const Standard_Boolean WithRotation, const gp_Pnt& P, const gp_Pnt& Pos1, const gp_Vec& Ax1,
                      const gp_Pnt& Pos2, const gp_Vec& Ax2, gp_Pnt& Pnew) {
...
static void BuildConnectedEdges(const TopoDS_Wire& aWire, const TopoDS_Edge& StartEdge,
                                const TopoDS_Vertex& StartVertex, TopTools_ListOfShape& ConnectedEdges) {
    ...
    for (;;) {
        ...
        if (CurVertex.IsSame(Origin)) break;
    }
}
...
BRepFill_CompatibleWires::BRepFill_CompatibleWires() : myIsDone(Standard_False) {}
...
void BRepFill_CompatibleWires::SameNumberByPolarMethod(const Standard_Boolean WithRotation) {
    ...
    for (i = ideb; i <= ifin; i++) {
        ...
        if (hasDegEdge) {
            ...
            for (anItw.Initialize(myWork(i)); anItw.More(); anItw.Next()) {
                const TopoDS_Edge& anEdge = TopoDS::Edge(anItw.Value());
                if (!BRep_Tool::Degenerated(anEdge)) aBBuilder.Add(aNewWire, anEdge);
            }
            myWork(i) = aNewWire;
        }
    }
    ...
    for (i = ifin; i > ideb; i--) {
        ...
        if (SeqV.Length() > NbMaxV) throw Standard_NoSuchObject("BRepFill::SameNumberByPolarMethod failed");
        ...
        for (ii = 1; ii <= SeqV.Length(); ii++) {
            ...
            gp_Pnt Pnew;
            Transform(WithRotation, Pi, Pos->Value(i), Axe->Value(i), Pos->Value(i - 1), Axe->Value(i - 1), Pnew);
            ...
            if (Pnew.Distance(Pos->Value(i - 1)) > Precision::Confusion()) {
                ...
                NewVertex = EdgeIntersectOnWire(Pos->Value(i - 1), Pnew, percent, RMap, TopoDS::Wire(myWork(i - 1)),
    ...
    for (i = ideb; i < ifin; i++) {
        ...
        for (ii = 1; ii <= SeqV.Length(); ii++) {
            ...
            if (intersect) {
                ...
                if (Pnew.Distance(Pos->Value(i + 1)) > Precision::Confusion()) {
                    Standard_Real percent = myPercent;
                    NewVertex = EdgeIntersectOnWire(Pos->Value(i + 1), Pnew, percent, MapVLV,
                                                    TopoDS::Wire(myWork(i + 1)), Vsol, newwire, EdgeNewEdges);
    ...
    for (i = ideb + 1; i <= ibout; i++) {
        ...
        for (; itW.More(); itW.Next()) {
            ...
            if ((V1.IsSame(VVF) && V2.IsSame(VVL)) || (V2.IsSame(VVF) && V1.IsSame(VVL))) {
                ...
                for (rang = i; rang > ideb; rang--) {
                    Transform(WithRotation, PP1, Pos->Value(rang), Axe->Value(rang), Pos->Value(rang - 1),
                              Axe->Value(rang - 1), PP1);
                    Transform(WithRotation, PP2, Pos->Value(rang), Axe->Value(rang), Pos->Value(rang - 1),
                              Axe->Value(rang - 1), PP2);
                }
        ...
        if (Esol.IsNull()) throw Standard_ConstructionError("BRepFill :: profiles are inconsistent");
        MW.Add(Esol);
...
void BRepFill_CompatibleWires::SameNumberByACR(const Standard_Boolean report) {
    ...
    if (nbmax > 1) {
        ...
        if (report || nbmin < nbmax) {
            ...
            for (i = 1; i <= nbSects; i++) {
                ...
                if (WireLen(i) > gp::Resolution()) tol /= WireLen(i);
    ...
    if (nbmax != nbmin) throw Standard_NoSuchObject("BRepFill_CompatibleWires::SameNumberByACR failed");
}
...
void BRepFill_CompatibleWires::ComputeOrigin(const Standard_Boolean /*polar*/) {
    ...
    */
    if (!allClosed) throw Standard_NoSuchObject("BRepFill_CompatibleWires::ComputeOrigin : the wires must be closed");
    ...
    */
    ...
    for (i = ideb; i <= myWork.Length(); i++) {
        ...
        if (i == ideb) {
            ...
            Transform(Standard_True, Pdeb, Pos->Value(i - 1), Axe->Value(i - 1), Pos->Value(i), Axe->Value(i), Pnew);
            Transform(Standard_True, Psuiv, Pos->Value(i - 1), Axe->Value(i - 1), Pos->Value(i), Axe->Value(i), Pnext);
            Transform(Standard_True, PPs, Pos->Value(i - 1), Axe->Value(i - 1), Pos->Value(i), Axe->Value(i), PPn);
            ...
            if (SeqV.Length() > NbMaxV) throw Standard_NoSuchObject("BRepFill::ComputeOrigin failed");
            if (!polar) {
...
void BRepFill_CompatibleWires::SearchOrigin() {
    ...
    if (!allOpen) throw Standard_NoSuchObject("BRepFill_CompatibleWires::SearchOrigin : the wires must be open");
