...
Standard_Integer DetectKPart(const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2) {
    ...
    if (degen1) {
        ...
        if (curv1.IsNull()) throw Standard_NullObject("Null 3D curve in edge");
    ...
    if (IType != 0) {
        ...
        if (degen2) {
            ...
            if (curv.IsNull()) throw Standard_NullObject("Null 3D curve in edge");
                ...
                if (AdC.GetType() != GeomAbs_Circle) {
                    ...
                    if (AdC.Circle().Axis().IsCoaxial(axe1, Precision::Angular(), Precision::Confusion())) {
                        ...
                        if (Abs(AdC.Circle().Radius() - dist1) < Precision::Confusion()) {
                            ...
                            Standard_Boolean Same, SameParametricLength = (Abs(h1 - h2) < Precision::PConfusion());
...
void CreateKPart(const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2, const Standard_Integer IType,
    ...
    if (degen1) {
        ...
        if (C1.IsNull()) throw Standard_NullObject("Null 3D curve in edge");
    ...
    if (degen2) {
        ...
        if (C2.IsNull()) throw Standard_NullObject("Null 3D curve in edge");
    ...
    if (!C1.IsNull()) aC1Adaptor.Load(C1);
    GeomAdaptor_Curve aC2Adaptor;
    if (!C2.IsNull()) aC2Adaptor.Load(C2);
...
void BRepFill_Generator::Perform() {
    ...
    for (Standard_Integer i = 1; i <= Nb - 1; i++) {
        ...
        while (tantque) {
            ...
            Standard_Boolean degen1 = BRep_Tool::Degenerated(TopoDS::Edge(ex1.Current()));
            Standard_Boolean degen2 = BRep_Tool::Degenerated(TopoDS::Edge(ex2.Current()));
            ...
            if (!(degen1 && IType == 4)) B.Add(W, Edge1);
            B.Add(W, Edge4);
            if (!(degen2 && IType == 4)) B.Add(W, Edge2.Reversed());
            B.Add(W, Edge3);
            ...
            if (IType == 0) {
                ...
                if (Periodic) {
                    ...
                    B.UpdateEdge(Edge3, new Geom2d_BezierCurve(Extrem1), new Geom2d_BezierCurve(Extrem2), Face, T);
    ...
    if (uClosed && DegenFirst && DegenLast) myShell.Closed(Standard_True);
}
