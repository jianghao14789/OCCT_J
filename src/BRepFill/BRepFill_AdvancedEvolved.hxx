...
public:
    ...
    Standard_EXPORT BRepFill_AdvancedEvolved()
        : myErrorStatus(BRepFill_AdvancedEvolved_Empty), myFuzzyValue(0.0), myIsParallel(Standard_True),
          myDebugShapesPath("C:\\Temp") {}

    Standard_EXPORT void Perform(const TopoDS_Wire& theSpine, const TopoDS_Wire& theProfile,
                                 const Standard_Real theTolerance, const Standard_Boolean theSolidReq = Standard_True);

    Standard_Boolean IsDone(unsigned int* theErrorCode = 0) const {
        if (theErrorCode) *theErrorCode = myErrorStatus;
...
protected:
    ...
    Standard_EXPORT void RemoveExcessSolids(const TopTools_ListOfShape& theLSplits, const TopoDS_Shape& theShape,
    ...
    Standard_EXPORT void GetSpineAndProfile(const TopoDS_Wire& theSpine, const TopoDS_Wire& theProfile);
    ...
    Standard_EXPORT Standard_Boolean CheckSingularityAndAdd(const TopoDS_Face& theF, const Standard_Real theFuzzyToler,
