...
static const TopoDS_Wire PutProfilAt(const TopoDS_Wire& ProfRef, const gp_Ax3& AxeRef, const TopoDS_Edge& E,
                                     const TopoDS_Face& F, const Standard_Boolean AtStart);

static void TrimFace(const TopoDS_Face& Face, TopTools_SequenceOfShape& TheEdges, TopTools_SequenceOfShape& S);

static void TrimEdge(const TopoDS_Edge& Edge, const TopTools_SequenceOfShape& TheEdgesControle,
                     TopTools_SequenceOfShape& TheVer, TColStd_SequenceOfReal& ThePar, TopTools_SequenceOfShape& S);

static Standard_Integer PosOnFace(Standard_Real d1, Standard_Real d2, Standard_Real d3);

static void ComputeIntervals(const TopTools_SequenceOfShape& VonF, const TopTools_SequenceOfShape& VOnL,
                             const TColgp_SequenceOfPnt& ParOnF, const TColgp_SequenceOfPnt& ParOnL,
                             const BRepFill_TrimSurfaceTool& Trim, const Handle(Geom2d_Curve) & Bis,
                             const TopoDS_Vertex& VS, const TopoDS_Vertex& VE, TColStd_SequenceOfReal& FirstPar,
                             TColStd_SequenceOfReal& LastPar, TopTools_SequenceOfShape& FirstV,
...
static Standard_Boolean DoubleOrNotInFace(const TopTools_SequenceOfShape& EC, const TopoDS_Vertex& V);

static void SimpleExpression(const Bisector_Bisec& B, Handle(Geom2d_Curve) & Bis);

static TopAbs_Orientation Relative(const TopoDS_Wire& W1, const TopoDS_Wire& W2, const TopoDS_Vertex& V,
                                   Standard_Boolean& Commun);

static void CutEdge(const TopoDS_Edge& E, const TopoDS_Face& F, TopTools_ListOfShape& Cuts);

static void CutEdgeProf(const TopoDS_Edge& E, const Handle(Geom_Plane) & Plane, const Handle(Geom2d_Line) & Line,
                        TopTools_ListOfShape& Cuts, TopTools_DataMapOfShapeShape& MapVerRefMoved);

static Standard_Integer VertexFromNode(const Handle(MAT_Node) & aNode, const TopoDS_Edge& E, const TopoDS_Vertex& VF,
                                       const TopoDS_Vertex& VL,
                                       BRepFill_DataMapOfNodeDataMapOfShapeShape& MapNodeVertex, TopoDS_Vertex& VS);
...
BRepFill_Evolved::BRepFill_Evolved() : myIsDone(Standard_False), mySpineType(Standard_True) {}
...
BRepFill_Evolved::BRepFill_Evolved(const TopoDS_Wire& Spine, const TopoDS_Wire& Profile, const gp_Ax3& AxeProf,
                                   const GeomAbs_JoinType Join, const Standard_Boolean Solid)
...
BRepFill_Evolved::BRepFill_Evolved(const TopoDS_Face& Spine, const TopoDS_Wire& Profile, const gp_Ax3& AxeProf,
                                   const GeomAbs_JoinType Join, const Standard_Boolean Solid)
    : myIsDone(Standard_False) {
    Perform(Spine, Profile, AxeProf, Join, Solid);
}
...
static Standard_Boolean IsVertical(const TopoDS_Edge& E) {
    ...
    if (Abs(P1.Y() - P2.Y()) < BRepFill_Confusion()) {
        ...
        if (GC->DynamicType() == STANDARD_TYPE(Geom_Line)) return Standard_True;
    }
...
void BRepFill_Evolved::Perform(const TopoDS_Wire& Spine, const TopoDS_Wire& Profile, const gp_Ax3& AxeProf,
                               const GeomAbs_JoinType Join, const Standard_Boolean Solid) {
...
void BRepFill_Evolved::Perform(const TopoDS_Face& Spine, const TopoDS_Wire& Profile, const gp_Ax3& AxeProf,
                               const GeomAbs_JoinType Join, const Standard_Boolean Solid) {
...
void BRepFill_Evolved::PrivatePerform(const TopoDS_Face& Spine, const TopoDS_Wire& Profile, const gp_Ax3& AxeProf,
                                      const GeomAbs_JoinType Join, const Standard_Boolean Solid) {
...
static void IsInversed(const TopoDS_Shape& S, const TopoDS_Edge& E1, const TopoDS_Edge& E2, Standard_Boolean* Inverse) {
...
void BRepFill_Evolved::ElementaryPerform(const TopoDS_Face& Sp, const TopoDS_Wire& Pr,
    ...
    for (Standard_Integer i = 1; i <= Locus.Graph()->NumberOfArcs(); i++) {
        ...
        for (ProfExp.Init(myProfile); ProfExp.More(); ProfExp.Next()) {
            ...
            Standard_Integer EndOnF = VertexFromNode(Node2, TopoDS::Edge(ProfExp.Current()), VF, VL, MapNodeVertex, VE);
            ...
            ComputeIntervals(VOnF, VOnL, ParOnF, ParOnL, Trim, Bis, VS, VE, FirstPar, LastPar, FirstV, LastV);
    ...
    for (ite1.Initialize(myMap); ite1.More(); ite1.Next()) {
        ...
        for (ProfExp.Init(myProfile); ProfExp.More(); ProfExp.Next()) {
            ...
            if (MapBis.IsBound(CurrentEdge)) {
                ...
                if (YaFace) {
                    ...
                    TrimEdge(CurrentEdge, MapBis(FaceControle), MapBis(CurrentEdge), MapVerPar(CurrentEdge),
                             aSeqOfShape);
        ...
        if (MapBis.IsBound(CurrentEdge)) {
            ...
            if (YaFace && myMap(CurrentSpine)(VCL).Extent() <= 1) {
                TopTools_SequenceOfShape aSeqOfShape;
                TrimEdge(CurrentEdge, MapBis(FaceControle), MapBis(CurrentEdge), MapVerPar(CurrentEdge), aSeqOfShape);
...
void BRepFill_Evolved::PlanarPerform(const TopoDS_Face& Sp, const TopoDS_Wire& Pr,
                                     const BRepMAT2d_BisectingLocus& Locus, BRepMAT2d_LinkTopoBilo& Link,
                                     const GeomAbs_JoinType Join) {
        ...
        for (Standard_Integer i = 0; i <= 1; i++) {
            if (!MapVP.IsBound(V[i])) {
                ...
                for (Exp.Init(Paral.Shape(), TopAbs_EDGE); Exp.More(); Exp.Next()) {
                    ...
                    if (!myMap.IsBound(GS)) myMap.Bind(GS, EmptyMap);
                    if (!myMap(GS).IsBound(V[i])) myMap(GS).Bind(V[i], Paral.GeneratedShapes(GS));
                }
            }
            TopoDS_Shape Rest = MapVP(V[i]);

            Standard_Boolean ToReverse = Standard_False;
            if ((IsMinV1 && (i == 1)) || (!IsMinV1 && (i == 0))) ToReverse = Standard_True;
        ...
        for (; FR.More(); FR.Next()) {
            ...
            for (Exp.Init(F, TopAbs_EDGE); Exp.More(); Exp.Next()) {
                ...
                if (OffAnc.HasAncestor(CE)) {
                    ...
                    if (!myMap.IsBound(InitE)) myMap.Bind(InitE, EmptyMap);
                    if (!myMap(InitE).IsBound(E)) myMap(InitE).Bind(E, EmptyList);
...
void BRepFill_Evolved::VerticalPerform(const TopoDS_Face& Sp, const TopoDS_Wire& Pr,
                                       const BRepMAT2d_BisectingLocus& Locus, BRepMAT2d_LinkTopoBilo& Link,
                                       const GeomAbs_JoinType Join) {
    TopoDS_Shape aLocalShape = Sp.Oriented(TopAbs_FORWARD);
    mySpine = TopoDS::Face(aLocalShape);
...
void BRepFill_Evolved::PrepareProfile(TopTools_ListOfShape& WorkProf, TopTools_DataMapOfShapeShape& MapProf) const {
...
void BRepFill_Evolved::Transfert(BRepFill_Evolved& Vevo, const TopTools_DataMapOfShapeShape& MapProf,
                                 const TopTools_DataMapOfShapeShape& MapSpine, const TopLoc_Location& LS,
                                 const TopLoc_Location& InitLS, const TopLoc_Location& InitLP) {
    //----------------------------------------------------------------
...
void BRepFill_Evolved::MakePipe(const TopoDS_Edge& SE, const gp_Ax3& AxeRef) {
    ...
    TopoDS_Wire DummyProf = PutProfilAt(TopoDS::Wire(aLocalShape), AxeRef, SE, mySpine, Standard_True);
    ...
    for (ProfExp.Init(myProfile), GenProfExp.Init(GenProf); ProfExp.More(); ProfExp.Next(), GenProfExp.Next()) {
...
void BRepFill_Evolved::MakeRevol(const TopoDS_Edge& SE, const TopoDS_Vertex& VLast, const gp_Ax3& AxeRef) {
    BRepTools_WireExplorer ProfExp;
    TopExp_Explorer FaceExp;
...
TopLoc_Location BRepFill_Evolved::FindLocation(const TopoDS_Face& Face) const {
    ...
    if (!L.IsIdentity()) S = Handle(Geom_Surface)::DownCast(S->Transformed(L.Transformation()));
            ...
            if (Continuity >= 1) {
                ...
                for (iteS.Initialize(myMap); iteS.More(); iteS.Next()) {
                    ...
                    if (myMap(SP).IsBound(V) && myMap(SP).IsBound(CurE) && myMap(SP).IsBound(PrecE)) {
                        if (!myMap(SP)(V).IsEmpty() && !myMap(SP)(CurE).IsEmpty() && !myMap(SP)(PrecE).IsEmpty())
                            B.Continuity(TopoDS::Edge(myMap(SP)(V).First()), TopoDS::Face(myMap(SP)(CurE).First()),
                                         TopoDS::Face(myMap(SP)(PrecE).First()), Continuity);
                    }
                }
            }
        }
        ...
        if (Continuity >= 1) {
            ...
            for (iteS.Initialize(myMap); iteS.More(); iteS.Next()) {
                ...
                if (myMap(SP).IsBound(VF) && myMap(SP).IsBound(CurE) && myMap(SP).IsBound(FirstE)) {
                    if (!myMap(SP)(VF).IsEmpty() && !myMap(SP)(CurE).IsEmpty() && !myMap(SP)(FirstE).IsEmpty())
                        B.Continuity(TopoDS::Edge(myMap(SP)(VF).First()), TopoDS::Face(myMap(SP)(CurE).First()),
                                     TopoDS::Face(myMap(SP)(FirstE).First()), Continuity);
                }
            }
        }
    }
...
void TrimFace(const TopoDS_Face& Face, TopTools_SequenceOfShape& TheEdges, TopTools_SequenceOfShape& S) {
...
const TopoDS_Wire PutProfilAt(const TopoDS_Wire& ProfRef, const gp_Ax3& AxeRef, const TopoDS_Edge& E,
                              const TopoDS_Face& F, const Standard_Boolean AtStart) {
    gp_Vec2d D1;
    gp_Pnt2d P;
    TopoDS_Wire Prof;
...
void TrimEdge(const TopoDS_Edge& Edge, const TopTools_SequenceOfShape& TheEdgesControle,
              TopTools_SequenceOfShape& TheVer, TColStd_SequenceOfReal& ThePar, TopTools_SequenceOfShape& S) {
    Standard_Boolean Change = Standard_True;
    BRep_Builder TheBuilder;
    S.Clear();
...
void ComputeIntervals(const TopTools_SequenceOfShape& VOnF, const TopTools_SequenceOfShape& VOnL,
                      const TColgp_SequenceOfPnt& ParOnF, const TColgp_SequenceOfPnt& ParOnL,
                      const BRepFill_TrimSurfaceTool& Trim, const Handle(Geom2d_Curve) & Bis, const TopoDS_Vertex& VS,
                      const TopoDS_Vertex& VE, TColStd_SequenceOfReal& FirstPar, TColStd_SequenceOfReal& LastPar,
                      TopTools_SequenceOfShape& FirstV, TopTools_SequenceOfShape& LastV) {
    ...
    while (IOnF <= VOnF.Length() || IOnL <= VOnL.Length()) {
        ...
        //---------------------------------------------------------
        if (IOnL > VOnL.Length() || (IOnF <= VOnF.Length() && ParOnF.Value(IOnF).X() < ParOnL.Value(IOnL).X())) {
...
Standard_Integer PosOnFace(Standard_Real d1, Standard_Real d2, Standard_Real d3) {
    if (Abs(d1 - d2) <= BRepFill_Confusion()) return 1;
    if (Abs(d1 - d3) <= BRepFill_Confusion()) return 3;

    if (d2 < d3) {
        if (d1 > (d2 + BRepFill_Confusion()) && d1 < (d3 - BRepFill_Confusion())) return 2;
    } else {
        if (d1 > (d3 + BRepFill_Confusion()) && d1 < (d2 - BRepFill_Confusion())) return 2;
    }
    return 0;
}
...
void CutEdgeProf(const TopoDS_Edge& E, const Handle(Geom_Plane) & Plane, const Handle(Geom2d_Line) & Line,
                 TopTools_ListOfShape& Cuts, TopTools_DataMapOfShapeShape& MapVerRefMoved) {
    Cuts.Clear();
...
Standard_Integer VertexFromNode(const Handle(MAT_Node) & aNode, const TopoDS_Edge& E, const TopoDS_Vertex& VF,
                                const TopoDS_Vertex& VL, BRepFill_DataMapOfNodeDataMapOfShapeShape& MapNodeVertex,
                                TopoDS_Vertex& VN) {
