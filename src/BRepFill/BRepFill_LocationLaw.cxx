...
void BRepFill_LocationLaw::TransformInCompatibleLaw(const Standard_Real TolAngular) {
    ...
    for (ipath = 2; ipath <= myLaws->Length(); ipath++) {
        ...
        if (T1.IsParallel(T2, TolAngular) && !T1.IsOpposite(T2, TolAngular)) { // Correction G0
            ToG0(M1, M2, Trsf);
        } else {
            Standard_Real alpha;
            gp_Vec cross(T1);
...
//===================================================================
void BRepFill_LocationLaw::PerformVertex(const Standard_Integer Index, const TopoDS_Vertex& Input,
                                         const Standard_Real TolMin, TopoDS_Vertex& Output,
...
// purpose  :
//=======================================================================
void BRepFill_LocationLaw::CurvilinearBounds(const Standard_Integer Index, Standard_Real& First,
...
Standard_Boolean BRepFill_LocationLaw::IsClosed() const {
    if (myPath.Closed()) return Standard_True;
...
//=======================================================================
Standard_Integer BRepFill_LocationLaw::IsG1(const Standard_Integer Index, const Standard_Real SpatialTolerance,
    ...
    if (Index > 0 && Index < myLaws->Length()) {
        ...
        myLaws->Value(Index + 1)->GetDomain(First, Last);
        if (Ok_D1) Ok_D1 = myLaws->Value(Index + 1)->D1(First, M2, V2, DM2, DV2, Bid1, Bid2);
    ...
    if (Index == 0 || Index == myLaws->Length()) {
        ...
        myLaws->Value(1)->GetDomain(First, Last);
        if (Ok_D1) myLaws->Value(1)->D1(First, M2, V2, DM2, DV2, Bid1, Bid2);
    ...
    if ((DV1.Magnitude() > EpsNul) && (DV2.Magnitude() > EpsNul) && (DV1.Angle(DV2) > AngularTolerance))
        isG1 = Standard_False;
...
// purpose  :
//=======================================================================
void BRepFill_LocationLaw::Parameter(const Standard_Real Abcissa, Standard_Integer& Index, Standard_Real& U) {
    ...
    if (Trouve) {
        ...
        if (Abcissa == myLength->Value(iedge + 1)) {
            ...
            GCPnts_AbscissaPoint AbsC(myTol, *myLaws->Value(iedge)->GetCurve(), Abcissa - myLength->Value(iedge), f);
            U = AbsC.Parameter();
        }
        Index = iedge;
    } else {
...
//=======================================================================
Standard_Real BRepFill_LocationLaw::Abscissa(const Standard_Integer Index, const Standard_Real Param) {
    GCPnts_AbscissaPoint AbsC;
    ...
    Length += AbsC.Length(*myLaws->Value(Index)->GetCurve(), myLaws->Value(Index)->GetCurve()->FirstParameter(), Param,
                          myTol);
    return Length;
}
