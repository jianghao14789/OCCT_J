...
static Handle(Geom_BSplineSurface)
    totalsurf(const TopTools_Array2OfShape& shapes, const Standard_Integer NbSects, const Standard_Integer NbEdges,
              const TColStd_SequenceOfReal& params, const Standard_Boolean w1Point, const Standard_Boolean w2Point,
              const Standard_Boolean uClosed, const Standard_Boolean vClosed, const Standard_Real myPres3d) {
    ...
    for (j = jdeb; j <= jfin; j++) {
        ...
        if (j == jfin && vClosed) {
            ...
            for (i = 2; i <= NbEdges; i++) {
                ...
                if (!Bof) Bof = CompBS.Add(curvBS, 200 * epsV, Standard_True, Standard_False, 1);
    ...
    surface = new Geom_BSplineSurface(poles, weights, anApprox.SurfUKnots(), anApprox.SurfVKnots(), Umults, Vmults,
                                      anApprox.UDegree(), anApprox.VDegree(), uperiodic, vperiodic);
    return surface;
}
...
BRepFill_NSections::BRepFill_NSections(const TopTools_SequenceOfShape& S, const Standard_Boolean Build)
...
BRepFill_NSections::BRepFill_NSections(const TopTools_SequenceOfShape& S,
                                       const GeomFill_SequenceOfTrsf& Transformations, const TColStd_SequenceOfReal& P,
                                       const Standard_Real VF, const Standard_Real VL, const Standard_Boolean Build)
...
void BRepFill_NSections::Init(const TColStd_SequenceOfReal& P, const Standard_Boolean Build) {
    ...
    mySurface =
        totalsurf(myEdges->Array2(), myShapes.Length(), NbEdge, myParams, w1Point, w2Point, uclosed, vclosed, tol);
...
// purpose  :
//=======================================================================
TopoDS_Vertex BRepFill_NSections::Vertex(const Standard_Integer Index, const Standard_Real Param) const {
    BRep_Builder B;
    TopoDS_Vertex V;
    B.MakeVertex(V);
    gp_Pnt P;
...
//=======================================================================
Standard_Real BRepFill_NSections::VertexTol(const Standard_Integer Index, const Standard_Real Param) const {
    Standard_Real Tol = Precision::Confusion();
    ...
    BS = new (Geom_BSplineCurve)(Poles->Array1(), Weigth->Array1(), Knots->Array1(), Mults->Array1(), Degree,
                                 Loi->IsUPeriodic());
...
Handle(GeomFill_SectionLaw) BRepFill_NSections::ConcatenedLaw() const {
    ...
    if (myLaws->Length() == 1)
        ...
        Law = new (GeomFill_NSections)(NCompo, myTrsfs, myParams, Ufirst, Ulast, Vfirst, Vlast, mySurface);
    }
    return Law;
}
...
// purpose  :
//=======================================================================
GeomAbs_Shape BRepFill_NSections::Continuity(const Standard_Integer Index, const Standard_Real TolAngular) const {
