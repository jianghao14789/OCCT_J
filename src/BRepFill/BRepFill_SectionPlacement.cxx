...
static Standard_Real SearchParam(const Handle(BRepFill_LocationLaw) & Law, const Standard_Integer Ind,
...
BRepFill_SectionPlacement::BRepFill_SectionPlacement(const Handle(BRepFill_LocationLaw) & Law,
                                                     const TopoDS_Shape& Section, const Standard_Boolean WithContact,
...
BRepFill_SectionPlacement::BRepFill_SectionPlacement(const Handle(BRepFill_LocationLaw) & Law,
                                                     const TopoDS_Shape& Section, const TopoDS_Shape& Vertex,
...
void BRepFill_SectionPlacement::Perform(const Standard_Boolean WithContact, const Standard_Boolean WithCorrection,
    ...
    Standard_Real theParam = Place.ParameterOnPath(), eps = Precision::PConfusion();

#ifdef OCCT_DEBUG
    if (myDebug) {
        gp_Pnt P_Path;
        P_Path = adpPath->Value(theParam);
        std::cout << "Point on Path" << P_Path.X() << ", " << P_Path.Y() << ", " << P_Path.Z() << ", " << std::endl;
    }
#endif
    ...
    if (!TheV.IsNull())
        for (Ind1 = 1; Ind1 <= myLaw->NbLaw(); Ind1++) {
            ...
            if (V1.IsSame(TheV) || V2.IsSame(TheV)) break;
        }
    ////////////////////
