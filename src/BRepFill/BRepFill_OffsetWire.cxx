...
static void QuasiFleche(const Adaptor3d_Curve& C, const Standard_Real Deflection2, const Standard_Real Udeb,
                        const gp_Pnt& Pdeb, const gp_Vec& Vdeb, const Standard_Real Ufin, const gp_Pnt& Pfin,
                        const gp_Vec& Vfin, const Standard_Integer Nbmin, const Standard_Real Eps,
                        TColStd_SequenceOfReal& Parameters, TColgp_SequenceOfPnt& Points);

static Standard_Boolean PerformCurve(TColStd_SequenceOfReal& Parameters, TColgp_SequenceOfPnt& Points,
                                     const Adaptor3d_Curve& C, const Standard_Real Deflection, const Standard_Real U1,
                                     const Standard_Real U2, const Standard_Real EPSILON, const Standard_Integer Nbmin);

static void CheckBadEdges(const TopoDS_Face& Spine, const Standard_Real Offset, const BRepMAT2d_BisectingLocus& Locus,
                          const BRepMAT2d_LinkTopoBilo& Link, TopTools_ListOfShape& BadEdges);

static Standard_Integer CutEdge(const TopoDS_Edge& E, const TopoDS_Face& F, Standard_Integer ForceCut,
                                TopTools_ListOfShape& Cuts);

static void CutCurve(const Handle(Geom2d_TrimmedCurve) & C, const Standard_Integer nbParts,
...
static Standard_Boolean VertexFromNode(const Handle(MAT_Node) & aNode, const Standard_Real Offset, gp_Pnt2d& PN,
                                       BRepFill_DataMapOfNodeShape& MapNodeVertex, TopoDS_Vertex& VN);

static void StoreInMap(const TopoDS_Shape& V1, const TopoDS_Shape& V2, TopTools_IndexedDataMapOfShapeShape& MapVV);

static void TrimEdge(const TopoDS_Edge& CurrentEdge, const TopoDS_Shape& CurrentSpine, const TopoDS_Face& AllSpine,
                     const TopTools_ListOfShape& D, TopTools_SequenceOfShape& Sv, TColStd_SequenceOfReal& MapverPar,
                     TopTools_SequenceOfShape& S, TopTools_IndexedDataMapOfShapeShape& MapVV,
                     const Standard_Integer IndOfE);

static Standard_Boolean IsInnerEdge(const TopoDS_Shape& ProE, const TopoDS_Face& AllSpine, Standard_Real& TrPar1,
...
static void MakeCircle(const TopoDS_Edge& E, const TopoDS_Vertex& V, const TopoDS_Face& F, const Standard_Real Offset,
                       BRepFill_IndexedDataMapOfOrientedShapeListOfShape& Map, const Handle(Geom_Plane) & RefPlane);

static void MakeOffset(const TopoDS_Edge& E, const TopoDS_Face& F, const Standard_Real Offset,
                       BRepFill_IndexedDataMapOfOrientedShapeListOfShape& Map, const Handle(Geom_Plane) & RefPlane,
                       const Standard_Boolean IsOpenResult, const GeomAbs_JoinType theJoinType,
...
static Standard_Boolean KPartCircle(const TopoDS_Face& mySpine, const Standard_Real myOffset,
                                    const Standard_Boolean IsOpenResult, const Standard_Real Alt, TopoDS_Shape& myShape,
...
BRepFill_OffsetWire::BRepFill_OffsetWire() : myIsOpenResult(Standard_False), myIsDone(Standard_False) {}
...
BRepFill_OffsetWire::BRepFill_OffsetWire(const TopoDS_Face& Spine, const GeomAbs_JoinType Join,
...
void BRepFill_OffsetWire::Init(const TopoDS_Face& Spine, const GeomAbs_JoinType Join,
                               const Standard_Boolean IsOpenResult) {
    myIsDone = Standard_False;
...
void Compute(const TopoDS_Face& Spine, TopoDS_Shape& aShape, BRepFill_IndexedDataMapOfOrientedShapeListOfShape& Map,
...
void BRepFill_OffsetWire::PerformWithBiLo(const TopoDS_Face& Spine, const Standard_Real Offset,
                                          const BRepMAT2d_BisectingLocus& Locus, BRepMAT2d_LinkTopoBilo& Link,
                                          const GeomAbs_JoinType Join, const Standard_Real Alt) {
    myIsDone = Standard_False;
    ...
    if (fabs(Alt) > gp::Resolution()) {
        ...
        RefPlane = Handle(Geom_Plane)::DownCast(RefPlane->Translated(anAlt * gp_Vec(RefPlane->Axis().Direction())));
    }
    ...
    for (Standard_Integer ic = 1; ic <= Locus.NumberOfContours(); ic++) {
        ...
        for (Standard_Integer ie = 1; ie <= Locus.NumberOfElts(ic); ie++) {
            ...
            if (SE.ShapeType() == TopAbs_VERTEX) {
                ...
                MakeOffset(TopoDS::Edge(SE), myWorkSpine, myOffset, myMap, RefPlane, myIsOpenResult, myJoinType, Ends);
                PE = SE;
            }
        }
    }
    ...
    if (myIsOpenResult && myJoinType == GeomAbs_Arc) {
        ...
        if (!myMap.IsEmpty() && myMap.FindKey(myMap.Extent()).ShapeType() == TopAbs_VERTEX) myMap.RemoveLast();
    }
    ...
    for (Standard_Integer i = 1; i <= Locus.Graph()->NumberOfArcs(); i++) {
        ...
        if (myJoinType == GeomAbs_Intersection) StartOnEdge = EndOnEdge = 0;

        //---------------------------------------------
        // Construction of geometries.
        //---------------------------------------------
        BRepFill_TrimEdgeTool Trim(Bisec, Locus.GeomElt(CurrentArc->FirstElement()),
                                   Locus.GeomElt(CurrentArc->SecondElement()), myOffset);

        //-----------------------------------------------------------
        // Construction of vertices on edges parallel to the spine.
        //-----------------------------------------------------------

        Trim.IntersectWith(E[0], E[1], S[0], S[1], Ends[0], Ends[1], myJoinType, myIsOpenResult, Params);
    ...
    for (j = 1; j <= myMap.Extent(); j++) {
        ...
        if (MapBis.IsBound(CurrentEdge)) {
            ...
            if (!MapBis(CurrentEdge).IsEmpty()) {
                ...
                TrimEdge(CurrentEdge, CurrentSpine, mySpine, Detromp(CurrentSpine), MapBis(CurrentEdge),
                         MapVerPar(CurrentEdge), aSeqOfShape, MapVV, IndOfE);
...
void BRepFill_OffsetWire::PrepareSpine() {
    ...
    for (TopoDS_Iterator IteF(mySpine); IteF.More(); IteF.Next()) {
        ...
        for (TopoDS_Iterator IteW(IteF.Value()); IteW.More(); IteW.Next()) {
            ...
            if (nbEdges == 2 && nbResEdges == 0) ForcedCut = 1;
...
void BRepFill_OffsetWire::UpdateDetromp(BRepFill_DataMapOfOrientedShapeListOfShape& Detromp, const TopoDS_Shape& Shape1,
                                        const TopoDS_Shape& Shape2, const TopTools_SequenceOfShape& Vertices,
                                        const TColgp_SequenceOfPnt& Params, const Bisector_Bisec& Bisec,
                                        const Standard_Boolean SOnE, const Standard_Boolean EOnE,
...
void BRepFill_OffsetWire::MakeWires() {
    ...
    for (i = 1; i <= myMap.Extent(); i++) {
        for (itl.Initialize(myMap(i)); itl.More(); itl.Next()) {
            ...
            TopExp::Vertices(E, V1, V2);
            if (V1.IsSame(V2) && IsSmallClosedEdge(E, V1)) continue; // remove small closed edges
            if (!CheckSmallParamOnEdge(E)) continue;
            if (!MVE.Contains(V1)) {
...
void BRepFill_OffsetWire::FixHoles() {
    ...
    while (!UnclosedWires.IsEmpty()) {
        ...
        if (DistF > MaxTol) IndexF = 0;
        if (DistL > MaxTol) IndexL = 0;
        TopoDS_Wire theWire;
        TopoDS_Edge theEdge;
        ...
        if (DistF <= MaxTol) {
            ...
            theEdge =
                (IsFirstF) ? TopoDS::Edge(VEmap.FindFromKey(V1).First()) : TopoDS::Edge(VEmap.FindFromKey(V2).First());
            TopoDS_Iterator it(theWire);
            for (; it.More(); it.Next()) {
                TopoDS_Edge anEdge = TopoDS::Edge(it.Value());
                if (IsFirstF) anEdge.Reverse();
                if (!anEdge.IsSame(theEdge)) BB.Add(Base, anEdge);
            }
        ...
        if (DistL <= MaxTol && IndexL != IndexF) {
            ...
            TopExp::MapShapesAndAncestors(theWire, TopAbs_VERTEX, TopAbs_EDGE, VEmap);
            theEdge =
                (IsFirstL) ? TopoDS::Edge(VEmap.FindFromKey(V1).First()) : TopoDS::Edge(VEmap.FindFromKey(V2).First());
            TopoDS_Iterator it(theWire);
            for (; it.More(); it.Next()) {
                TopoDS_Edge anEdge = TopoDS::Edge(it.Value());
        ...
        if (DistF <= MaxTol) UnclosedWires.Remove(IndexF);
        if (DistL <= MaxTol && IndexL != IndexF) {
            if (DistF <= MaxTol && IndexL > IndexF) IndexL--;
...
//=======================================================================
Standard_Integer CutEdge(const TopoDS_Edge& E, const TopoDS_Face& F, Standard_Integer ForceCut,
                         TopTools_ListOfShape& Cuts) {
    Cuts.Clear();
...
void CutCurve(const Handle(Geom2d_TrimmedCurve) & C, const Standard_Integer nbParts,
              TColGeom2d_SequenceOfCurve& theCurves) {
    ...
    for (Standard_Integer i = 1; i < nbParts; i++) {
        ...
        if (UC - UF > PTol && PC.Distance(PF) > Tol) {
            if (UL - UC < PTol || PL.Distance(PC) < Tol) continue;
...
void MakeCircle(const TopoDS_Edge& E, const TopoDS_Vertex& V, const TopoDS_Face& F, const Standard_Real Offset,
                BRepFill_IndexedDataMapOfOrientedShapeListOfShape& Map, const Handle(Geom_Plane) & RefPlane) {
...
void MakeOffset(const TopoDS_Edge& E, const TopoDS_Face& F, const Standard_Real Offset,
                BRepFill_IndexedDataMapOfOrientedShapeListOfShape& Map, const Handle(Geom_Plane) & RefPlane,
                const Standard_Boolean IsOpenResult, const GeomAbs_JoinType theJoinType, const TopoDS_Vertex* Ends) {
    Standard_Real f, l;
    ...
    if (IsOpenResult) {
        TopoDS_Vertex V1, V2;
        TopExp::Vertices(E, V1, V2);
        if (V1.IsSame(Ends[0]) || V1.IsSame(Ends[1])) ToExtendFirstPar = Standard_False;
        if (V2.IsSame(Ends[0]) || V2.IsSame(Ends[1])) ToExtendLastPar = Standard_False;
    }
    ...
    if (AC.GetType() == GeomAbs_Circle) {
        ...
        if (anOffset * Signe < AC.Circle().Radius() - Precision::Confusion()) {
            ...
            if (theJoinType == GeomAbs_Arc) {
                if (ToExtendFirstPar) f -= 0.2 * Delta;
                if (ToExtendLastPar) l += 0.2 * Delta;
...
Standard_Boolean VertexFromNode(const Handle(MAT_Node) & aNode, const Standard_Real Offset, gp_Pnt2d& PN,
                                BRepFill_DataMapOfNodeShape& MapNodeVertex, TopoDS_Vertex& VN) {
...
void StoreInMap(const TopoDS_Shape& V1, const TopoDS_Shape& V2, TopTools_IndexedDataMapOfShapeShape& MapVV) {
    ...
    for (i = 1; i <= MapVV.Extent(); i++)
        if (MapVV(i).IsSame(V1)) MapVV(i) = NewV;
...
void TrimEdge(const TopoDS_Edge& E, const TopoDS_Shape& ProE, const TopoDS_Face& AllSpine,
              const TopTools_ListOfShape& Detromp, TopTools_SequenceOfShape& TheVer, TColStd_SequenceOfReal& ThePar,
    ...
    if (!BRep_Tool::Degenerated(E)) {
        ...
        for (Standard_Integer k = 1; k < TheVer.Length(); k++) {
            if (TheVer.Value(k).IsSame(TheVer.Value(k + 1)) || Abs(ThePar.Value(k) - ThePar.Value(k + 1)) <= aParTol) {
...
static Standard_Boolean IsInnerEdge(const TopoDS_Shape& ProE, const TopoDS_Face& AllSpine, Standard_Real& TrPar1,
                                    Standard_Real& TrPar2) {
    if (ProE.ShapeType() != TopAbs_EDGE) return Standard_False;
    ...
    for (Standard_Integer i = 1; i <= VEmap.Extent(); i++) {
        ...
        if (LE.Extent() == 1 && anEdge.IsSame(LE.First())) return Standard_False;
    }
...
Standard_Boolean IsSmallClosedEdge(const TopoDS_Edge& anEdge, const TopoDS_Vertex& aVertex) {
    ...
    Handle(Geom2d_Curve) PCurve;
    Handle(BRep_CurveRepresentation) CurveRep = ((Handle(BRep_TEdge)::DownCast(anEdge.TShape()))->Curves()).First();
    ...
    if (dist1 <= theTol && dist2 <= theTol && dist3 <= theTol) return Standard_True;
...
static Standard_Boolean PerformCurve(TColStd_SequenceOfReal& Parameters,
                                     ...
                                     TColgp_SequenceOfPnt& Points, const Adaptor3d_Curve& C,
                                     const Standard_Real Deflection, const Standard_Real U1, const Standard_Real U2,
                                     const Standard_Real EPSILON, const Standard_Integer Nbmin) {
    ...
    if ((aDelta / aDist) > 5.0e-14) {
        QuasiFleche(C, Deflection * Deflection,
                    ...
                    UU1, Pdeb, Ddeb, UU2, Pfin, Dfin, Nbmin, EPSILON * EPSILON, Parameters, Points);
    }
...
static void QuasiFleche(const Adaptor3d_Curve& C,
                        ...
                        const Standard_Real Deflection2, const Standard_Real Udeb, const gp_Pnt& Pdeb,
                        const gp_Vec& Vdeb, const Standard_Real Ufin, const gp_Pnt& Pfin, const gp_Vec& Vfin,
                        const Standard_Integer Nbmin, const Standard_Real Eps, TColStd_SequenceOfReal& Parameters,
                        TColgp_SequenceOfPnt& Points) {
    ...
    if (theFleche < Deflection2) {
        ...
        QuasiFleche(C, Deflection2, Udeb, Pdeb,
                    ...
                    Vdeb, Udeb + Udelta, Pdelta, Vdelta, 3, Eps, Parameters, Points);
    }

    if (Nbmin > 2) {
        QuasiFleche(C, Deflection2, Udeb + Udelta, Pdelta,

                    Vdelta, Ufin, Pfin, Vfin, Nbmin - (Points.Length() - Ptslength), Eps, Parameters, Points);
    }
}
...
Standard_Boolean CheckSmallParamOnEdge(const TopoDS_Edge& anEdge) {
    ...
    if (!aList.IsEmpty()) {
        ...
        if (Abs(l - f) < Precision::PConfusion()) return Standard_False;
    }
    return Standard_True;
}
