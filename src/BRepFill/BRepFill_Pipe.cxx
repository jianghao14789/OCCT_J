...
// purpose:
// ---------------------------------------------------------------------------------
static Standard_Boolean UpdateMap(const TopoDS_Shape& theKey, const TopoDS_Shape& theValue,
    ...
    if (!found) aList.Append(theValue);
...
static void UpdateTolFromTopOrBottomPCurve(const TopoDS_Face& aFace, TopoDS_Edge& anEdge) {
    ...
    for (Standard_Integer i = 0; i <= NCONTROL; i++) {
        ...
        if (sqdist > TolTol) TolTol = sqdist;
    }
...
BRepFill_Pipe::BRepFill_Pipe(const TopoDS_Wire& Spine, const TopoDS_Shape& Profile, const GeomFill_Trihedron aMode,
                             const Standard_Boolean ForceApproxC1, const Standard_Boolean KPart)

{
    ...
    myMode = GeomFill_IsCorrectedFrenet;
    if (aMode == GeomFill_IsFrenet || aMode == GeomFill_IsCorrectedFrenet || aMode == GeomFill_IsDiscreteTrihedron)
        myMode = aMode;

    myContinuity = GeomAbs_C2;
    if (myMode == GeomFill_IsDiscreteTrihedron) myContinuity = GeomAbs_C0;
...
void BRepFill_Pipe::Perform(const TopoDS_Wire& Spine, const TopoDS_Shape& Profile, const Standard_Boolean /*KPart*/)

{
    ...
    fila.SetValues(M(1, 1), M(1, 2), M(1, 3), V.X(), M(2, 1), M(2, 2), M(2, 3), V.Y(), M(3, 1), M(3, 2), M(3, 3),
                   V.Z());
...
TopoDS_Face BRepFill_Pipe::Face(const TopoDS_Edge& ESpine, const TopoDS_Edge& EProfile) {
    ...
    if (BRep_Tool::Degenerated(EProfile)) return theFace;
    ...
    if (!iprof) throw Standard_DomainError("BRepFill_Pipe::Face : Edge not in the Profile");
    ...
    if (!ispin) throw Standard_DomainError("BRepFill_Pipe::Edge  : Edge not in the Spine");
...
TopoDS_Edge BRepFill_Pipe::Edge(const TopoDS_Edge& ESpine, const TopoDS_Vertex& VProfile) {
    ...
    if (!iprof) throw Standard_DomainError("BRepFill_Pipe::Edge : Vertex not in the Profile");
...
TopoDS_Shape BRepFill_Pipe::Section(const TopoDS_Vertex& VSpine) const {
    ...
    if (!ispin) throw Standard_DomainError("BRepFill_Pipe::Section  : Vertex not in the Spine");
...
TopoDS_Wire BRepFill_Pipe::PipeLine(const gp_Pnt& Point) {
    ...
    MkSw.Build(myReversedEdges, myTapes, myRails, BRepFill_Modified, myContinuity, GeomFill_Location, myDegmax,
               mySegmax);
...
TopoDS_Shape BRepFill_Pipe::MakeShape(const TopoDS_Shape& S, const TopoDS_Shape& theOriginalS,
                                      const TopoDS_Shape& FirstShape, const TopoDS_Shape& LastShape) {
    TopoDS_Shape result;
    BRep_Builder B;
...
Standard_Integer BRepFill_Pipe::FindEdge(const TopoDS_Shape& S, const TopoDS_Edge& E,
...
Standard_Integer BRepFill_Pipe::FindVertex(const TopoDS_Shape& S, const TopoDS_Vertex& V,
                                           Standard_Integer& InitialLength) const {
    Standard_Integer result = 0;
...
void BRepFill_Pipe::DefineRealSegmax() {
    ...
    if (mySegmax < RealSegmax) mySegmax = RealSegmax;
}
...
void BRepFill_Pipe::RebuildTopOrBottomFace(const TopoDS_Shape& aFace, const Standard_Boolean IsTop) const {
...
void BRepFill_Pipe::BuildHistory(const BRepFill_Sweep& theSweep, const TopoDS_Shape& theSection) {
    ...
    if (theSection.ShapeType() == TopAbs_WIRE) {
        ...
        for (inde = 0; wexp_sec.More(); wexp_sec.Next()) {
            ...
            if (myGenMap.IsBound(anEdge)) continue;
            ...
            for (Standard_Integer kk = 0; kk < 2; kk++) {
                ...
                for (jj = 1; jj <= anUEdges->UpperCol(); jj++) {
                    ...
                    if (!anUedge.IsNull()) Elist->Append(anUedge);
                }
            } // for (Standard_Integer kk = 0; kk < 2; kk++)
