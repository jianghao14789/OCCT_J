...
BRepFill_TrimEdgeTool::BRepFill_TrimEdgeTool(const Bisector_Bisec& Bisec, const Handle(Geom2d_Geometry) & S1,
                                             const Handle(Geom2d_Geometry) & S2, const Standard_Real Offset)
    : myOffset(Offset), myBisec(Bisec) {
...
static void EvalParameters(const Geom2dAdaptor_Curve& Bis, const Geom2dAdaptor_Curve& AC,
...
static void EvalParametersBis(const Geom2dAdaptor_Curve& Bis, const Geom2dAdaptor_Curve& AC,
                              TColgp_SequenceOfPnt& Params, const Standard_Real Tol) {
    Geom2dInt_GInter Intersector;
...
void BRepFill_TrimEdgeTool::IntersectWith(const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
                                          const TopoDS_Shape& InitShape1, const TopoDS_Shape& InitShape2,
                                          const TopoDS_Vertex& End1, const TopoDS_Vertex& End2,
                                          const GeomAbs_JoinType theJoinType, const Standard_Boolean IsOpenResult,
                                          TColgp_SequenceOfPnt& Params) {
    Params.Clear();
    ...
    if (NbPoints > 1 && theJoinType == GeomAbs_Intersection && InitShape1.ShapeType() != TopAbs_VERTEX &&
        ...
        if (IsOpenResult) {
            ...
            if (V1.IsSame(End1) || V1.IsSame(End2)) ToExtendFirstPar = Standard_False;
            if (V2.IsSame(End1) || V2.IsSame(End2)) ToExtendLastPar = Standard_False;
        }
    ...
    if (NbPoints > 1 && theJoinType == GeomAbs_Intersection) {
        ...
        for (i = 2; i <= NbPoints; i++)
            if (Params(i).X() < Params(imin).X()) imin = i;
...
void BRepFill_TrimEdgeTool::AddOrConfuse(const Standard_Boolean Start, const TopoDS_Edge& Edge1,
                                         const TopoDS_Edge& Edge2, TColgp_SequenceOfPnt& Params) const {
...
Standard_Boolean BRepFill_TrimEdgeTool::IsInside(const gp_Pnt2d& P) const {
    ...
    if (isPoint1)
        ...
        if (Dist > aDistMin) Dist = aDistMin;
