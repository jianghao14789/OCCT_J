...
BRepFill_Section::BRepFill_Section() : islaw(0), ispunctual(0), contact(0), correction(0) {}

BRepFill_Section::BRepFill_Section(const TopoDS_Shape& Profile, const TopoDS_Vertex& V,
                                   const Standard_Boolean WithContact, const Standard_Boolean WithCorrection)
    : vertex(V), islaw(0), ispunctual(0), contact(WithContact), correction(WithCorrection) {
...
TopoDS_Shape BRepFill_Section::ModifiedShape(const TopoDS_Shape& theShape) const {
    ...
    switch (theShape.ShapeType()) {
        case TopAbs_WIRE:
            if (theShape.IsSame(myOriginalShape)) aModifiedShape = wire;
            break;
        case TopAbs_EDGE: {
