...
static Standard_Boolean HasPCurves(const TopoDS_Edge& E) {
    ...
    BRep_ListIteratorOfListOfCurveRepresentation itcr((*((Handle(BRep_TEdge)*)&E.TShape()))->Curves());
...
//=======================================================================
static void Translate(const Handle(TopTools_HArray2OfShape) & ArrayIn, const Standard_Integer In,
                      Handle(TopTools_HArray2OfShape) & ArrayOut, const Standard_Integer Out) {
...
//=======================================================================
static Handle(Geom2d_Curve) Couture(const TopoDS_Edge& E, const Handle(Geom_Surface) & S, const TopLoc_Location& L) {
...
static Standard_Boolean CheckSameParameter(const Handle(Adaptor3d_Curve) & C3d, const Handle(Geom2d_Curve) & Pcurv,
...
static Standard_Boolean SameParameter(TopoDS_Edge& E, Handle(Geom2d_Curve) & Pcurv, const Handle(Geom_Surface) & Surf,
                                      const Standard_Real tol3d, Standard_Real& tolreached) {
    ...
    if (CheckSameParameter(HC3d, Pcurv, S, tol3d, tolreached)) return Standard_True;

    if (!HasPCurves(E)) {
        Handle(Geom2dAdaptor_Curve) HC2d = new Geom2dAdaptor_Curve(Pcurv);
        Approx_CurveOnSurface AppCurve(HC2d, S, HC2d->FirstParameter(), HC2d->LastParameter(), Precision::Confusion());
...
static void CorrectSameParameter(TopoDS_Edge& theEdge, const TopoDS_Face& theFace1, const TopoDS_Face& theFace2) {
    ...
    if (!theFace1.IsNull() && theFace1.IsSame(theFace2)) theEdge.Reverse();
    if (!theFace2.IsNull()) {
    ...
    for (Standard_Integer i = 0; i <= NCONTROL; i++) {
        ...
        for (Standard_Integer j = 0; j < 2; j++)
            if (PCurveExists[j]) {
                ...
                if (aSqDist > MaxSqDist) MaxSqDist = aSqDist;
            }
    }
...
//=======================================================================
static void BuildFace(const Handle(Geom_Surface) & S, const TopoDS_Edge& E1, const TopoDS_Edge& E2,
                      const TopoDS_Edge& E3, const TopoDS_Edge& E4, TopTools_DataMapOfShapeShape& EEmap,
                      const Standard_Boolean ExchUV, const Standard_Boolean UReverse, TopoDS_Face& F) {
    Standard_Real Tol;
    ...
    if (!IsPlan || !BRep_Tool::Degenerated(e1)) B.Add(e1);
    ...
    if (!IsPlan || !BRep_Tool::Degenerated(e2)) {
        ...
        if (!BRep_Tool::Degenerated(e2)) {
            ...
            for (Iter.Initialize(WW); Iter.More(); Iter.Next())
                NewEdge = Iter.Value();
            if (!e2.IsSame(NewEdge)) EEmap.Bind(e2, NewEdge);
        }
    }
    ...
    if (!IsPlan || !BRep_Tool::Degenerated(E)) {
        ...
        if (!BRep_Tool::Degenerated(E)) {
            ...
            for (Iter.Initialize(WW); Iter.More(); Iter.Next())
                NewEdge = Iter.Value();
            if (!E.IsSame(NewEdge)) EEmap.Bind(E, NewEdge);
        }
    }
    ...
    WW = B.Wire();
#ifdef DRAW
    if (Affich) DBRep::Set("wire-on-face", WW);
#endif
    ...
    if (IsPlan) { // Suspend representation 2d
        ...
        if (NS.Dot(NP) < 0.) thePlane->UReverse();
...
//=======================================================================
static TopoDS_Edge BuildEdge(Handle(Geom_Curve) & C3d, Handle(Geom2d_Curve) & C2d, Handle(Geom_Surface) & S,
                             const TopoDS_Vertex& VF, const TopoDS_Vertex& VL, const Standard_Real f,
                             const Standard_Real l, const Standard_Real Tol3d) {
    gp_Pnt P;
    ...
    C3d->D0(f, P);
    d = P1.Distance(P);
    if (d > Tol1) B.UpdateVertex(VF, d);

    C3d->D0(l, P);
    d = P2.Distance(P);
    if (d > Tol2) B.UpdateVertex(VL, d);
...
//=======================================================================
static Standard_Boolean Filling(const TopoDS_Shape& EF, const TopoDS_Shape& F1, const TopoDS_Shape& EL,
                                const TopoDS_Shape& F2, TopTools_DataMapOfShapeShape& EEmap, const Standard_Real Tol,
                                const gp_Ax2& Axe, const gp_Vec& TangentOnPart1, TopoDS_Edge& Aux1, TopoDS_Edge& Aux2,
                                TopoDS_Face& Result) {
    BRep_Builder B;
    // Class BRep_Tool without fields and without Constructor :
    //  BRep_Tool BT;
    ...
    if (Affich) {
        ...
        if (!E3.IsNull()) DBRep::Set("Fill_Edge3", E3);
        if (!E4.IsNull()) DBRep::Set("Fill_Edge4", E4);
    }
#endif
    ...
    const gp_Pnt aP1[] = {Prof1->Value(aPrm[0]).Transformed(aTf), Prof1->Value(aPrm[1]).Transformed(aTf),
    ...
    */
    ...
    GeomLib::SameRange(Precision::PConfusion(), C2, C2->FirstParameter(), C2->LastParameter(), f2, l2, C3);
    C2 = C3;
    ...
    */
    ...
    for (Standard_Integer i = 0; i < 3; i++) {
        ...
        if (N1.SquareMagnitude() < Precision::SquareConfusion()) continue;
        ...
        if (N2.SquareMagnitude() < Precision::SquareConfusion()) continue;
    ...
    if (Angle == RealFirst()) return Standard_False;
    ...
    if (ToReverseResult) Result.Reverse();
...
// purpose  :
//=======================================================================
static void Substitute(BRepTools_Substitution& aSubstitute, const TopoDS_Edge& Old, const TopoDS_Edge& New) {
...
//=======================================================================
static void BuildVertex(const Handle(Geom_Curve) & Iso, const Standard_Boolean isfirst, const Standard_Real First,
                        const Standard_Real Last, TopoDS_Shape& Vertex) {
    BRep_Builder B;
    Standard_Real val;
...
//=======================================================================
static TopoDS_Edge BuildEdge(const Handle(Geom_Surface) & S, const Standard_Boolean isUiso, const Standard_Real ValIso,
                             const TopoDS_Shape& VFirst, const TopoDS_Shape& VLast, const Standard_Real Tol) {
    TopoDS_Edge E;
    BRep_Builder B;
    ...
    if (sing) { // Singular case
        ...
        //	     Iso->LastParameter());
        if (fwd)
            MkE.Init(Iso, TopoDS::Vertex(VFirst), TopoDS::Vertex(VLast), Iso->FirstParameter(), Iso->LastParameter());
        else
            MkE.Init(Iso, TopoDS::Vertex(VLast), TopoDS::Vertex(VFirst), Iso->FirstParameter(), Iso->LastParameter());
...
//      DrawTrSurf::Set(name,Iso);
    ...
    if (sing) B.Range(E, S, Loc, Iso->FirstParameter(), Iso->LastParameter());
...
//=======================================================================
static void UpdateEdge(TopoDS_Edge& E, const Handle(Geom_Surface) & S, const Standard_Boolean isUiso,
...
static void ReverseModifiedEdges(TopoDS_Wire& theWire, const TopTools_MapOfShape& theEmap) {
    ...
    for (itl.Initialize(Ledges); itl.More(); itl.Next()) {
        ...
        if (theEmap.Contains(anEdge)) anEdge.Reverse();
...
BRepFill_Sweep::BRepFill_Sweep(const Handle(BRepFill_SectionLaw) & Section,
                               const Handle(BRepFill_LocationLaw) & Location, const Standard_Boolean WithKPart)
    : isDone(Standard_False), KPart(WithKPart) {
    mySec = Section;
    myLoc = Location;
...
#ifdef OCCT_DEBUG
    if (!issame) std::cout << "Sweep Warning : Edge not SameRange in the limits" << std::endl;
#endif
}
...
// purpose  :
//======================================================================
void BRepFill_Sweep::SetTolerance(const Standard_Real Tol3d, const Standard_Real BoundTol, const Standard_Real Tol2d,
...
Standard_Boolean BRepFill_Sweep::CorrectApproxParameters() {
    ...
    for (; iter.More(); iter.Next()) {
        ...
        if (aContinuity < NewCont) NewCont = aContinuity;
        if (aNbInterv > NewSegmax) NewSegmax = aNbInterv;
    }

    Standard_Boolean Corrected = Standard_False;
    if (NewCont != myContinuity || NewSegmax != mySegmax) Corrected = Standard_True;
...
Standard_Boolean BRepFill_Sweep::BuildWire(const BRepFill_TransitionStyle /*Transition*/) {
    ...
    for (ipath = 1; ipath <= NbPath; ipath++) {
        ...
        if (!Sweep.IsDone()) return Standard_False;
        S = Sweep.Surface();
        if (Sweep.ExchangeUV()) {
            if (Sweep.UReversed())
        ...
        BRepLib_MakeEdge MkE;
        MkE.Init(Iso, TopoDS::Vertex(myVEdges->Value(1, ipath)), TopoDS::Vertex(myVEdges->Value(1, ipath + 1)),
                 Iso->FirstParameter(), Iso->LastParameter());
        if (!MkE.IsDone()) { // Error of construction !!
#ifdef DRAW
            char name[100];
            sprintf(name, "firstvertex_error");
...
Standard_Boolean BRepFill_Sweep::BuildShell(const BRepFill_TransitionStyle /*Transition*/,
                                            const Standard_Integer IFirst, const Standard_Integer ILast,
                                            TopTools_MapOfShape& ReversedEdges,
                                            BRepFill_DataMapOfShapeHArray2OfShape& Tapes,
                                            BRepFill_DataMapOfShapeHArray2OfShape& Rails,
                                            const Standard_Real ExtendFirst, const Standard_Real ExtendLast) {
    ...
    for (isec = 1; isec <= NbLaw + 1; isec++) {
        ...
        if (Vertex(isec, 1).IsNull())
            B.MakeVertex(TopoDS::Vertex(Vertex(isec, 1)), S->Value(u, v), mySec->VertexTol(isec - 1, Vi(1)));
        else {
            TopLoc_Location Identity;
            Vertex(isec, 1).Location(Identity);
            B.UpdateVertex(TopoDS::Vertex(Vertex(isec, 1)), S->Value(u, v), mySec->VertexTol(isec - 1, Vi(1)));
        }
    ...
    if (!LastShape.IsNull() && (ILast == myLoc->NbLaw() + 1)) {
        ...
        for (isec = 1; isec <= NbLaw; isec++) {
            E = mySec->CurrentEdge();
            if (VEdge(isec, NbPath + 1).IsNull()) VEdge(isec, NbPath + 1) = E;
            ...
            UpdateVertex(ILast - 1, isec + 1, TabErr(isec, NbPath), Vi(NbPath + 1), Vertex(isec + 1, NbPath + 1));
        }
    ...
    for (isec = 1; isec <= NbLaw + 1; isec++) {
        ...
        if (Vertex(isec, NbPath + 1).IsNull())
            B.MakeVertex(TopoDS::Vertex(Vertex(isec, NbPath + 1)), S->Value(u, v),
                         mySec->VertexTol(isec - 1, Vi(NbPath + 1)));
        else {
            TopLoc_Location Identity;
            Vertex(isec, NbPath + 1).Location(Identity);
            B.UpdateVertex(TopoDS::Vertex(Vertex(isec, NbPath + 1)), S->Value(u, v),
                           mySec->VertexTol(isec - 1, Vi(NbPath + 1)));
        }
    } // end of for (isec=1; isec<=NbLaw+1; isec++)
        ...
        for (isec = 1; isec <= NbLaw; isec++) {
            ...
            if (isec == 1) {
                if (ipath == 1 && Vertex(1, 1).IsNull()) {
                    // All first
                    if (constSection)
                        myLoc->PerformVertex(IPath - 1, TopoDS::Vertex(SecVertex(1)), VError(1),
                                             TopoDS::Vertex(Vertex(1, 1)));
                    else
                        myLoc->PerformVertex(IPath - 1, mySec->Vertex(1, Vi(1)), mySec->VertexTol(0, Vi(1)),
                                             TopoDS::Vertex(Vertex(1, 1)));
                }
                ...
                if (vclose && (ipath == NbPath)) {
                    ...
                    if (constSection)
                        myLoc->PerformVertex(IPath, TopoDS::Vertex(SecVertex(1)), TabErr(1, ipath) + VError(1),
                                             TopoDS::Vertex(Vertex(1, ipath + 1)));
                    else
                        myLoc->PerformVertex(IPath, mySec->Vertex(1, Vi(ipath + 1)),
            ...
            if (ipath == 1) {
                if (uclose && (isec == NbLaw)) {
                    ...
                    if (constSection)
                        myLoc->PerformVertex(IPath - 1, TopoDS::Vertex(SecVertex(isec + 1)),
                                             TabErr(isec, 1) + VError(isec + 1), TopoDS::Vertex(Vertex(isec + 1, 1)));
                    else
                        myLoc->PerformVertex(IPath - 1, mySec->Vertex(isec + 1, Vi(1)),
            ...
            if (uclose && (isec == NbLaw)) {
                ...
                if (constSection)
                    myLoc->PerformVertex(IPath, TopoDS::Vertex(SecVertex(isec + 1)),
                                         TabErr(isec, ipath) + VError(isec + 1),
                                         TopoDS::Vertex(Vertex(isec + 1, ipath + 1)));
                else
                    myLoc->PerformVertex(IPath, mySec->Vertex(isec + 1, Vi(ipath + 1)),
            ...
            if (Degenerated(isec, ipath)) {
                ...
                if (isec == 1 && UEdge(1, ipath).IsNull()) {
                    if (!Vertex(1, ipath).IsSame(Vertex(1, ipath + 1))) {
                        ...
                        if (P1.Distance(P2) <= myTol3d) Vertex(1, ipath + 1) = Vertex(1, ipath);
                    }
                    UEdge(1, ipath) = BuildEdge(S, !exuv, UFirst, Vertex(1, ipath), Vertex(1, ipath + 1), myTol3d);
                } else {
                ...
                if (uclose && (isec == NbLaw)) {
                    ...
                    {
                        UEdge(isec + 1, ipath) =
                            BuildEdge(S, !exuv, ULast, Vertex(isec + 1, ipath), Vertex(isec + 1, ipath + 1), myTol3d);
                    } else {
                ...
                if (vclose && (ipath == NbPath)) {
                    ...
                    {
                        VEdge(isec, ipath + 1) =
                            BuildEdge(S, exuv, VLast, Vertex(isec, ipath + 1), Vertex(isec + 1, ipath + 1), myTol3d);
                    } else {
                    ...
                    {
                        TopoDS_Edge aNewLastEdge =
                            BuildEdge(S, exuv, VLast, Vertex(isec, ipath + 1), Vertex(isec + 1, ipath + 1), myTol3d);
                        RebuildTopOrBottomEdge(aNewLastEdge, TopoDS::Edge(VEdge(isec, ipath + 1)), ReversedEdges);
    ...
    for (ipath = 1, IPath = IFirst; ipath <= NbPath; ipath++, IPath++) {
        for (isec = 1; isec <= NbLaw; isec++) {
            if (Degenerated(isec, ipath)) {
                ...
                BuildFace(TabS(isec, ipath), TopoDS::Edge(UEdge(isec, ipath)), TopoDS::Edge(VEdge(isec, ipath)),
                          TopoDS::Edge(UEdge(isec + 1, ipath)), TopoDS::Edge(VEdge(isec, ipath + 1)), myVEdgesModified,
                          ExchUV(isec, ipath), UReverse(isec, ipath), face);
    ...
    for (ipath = 1; ipath <= NbPath; ipath++)
        for (isec = 1; isec <= NbLaw; isec++)
            if (IsBuilt(isec)) myFaces->ChangeValue(isec, ipath).Reverse();
            ...
            for (ipath = 1, IPath = IFirst; ipath <= NbPath; ipath++, IPath++) {
                ...
                if (isG1) {
                    ...
                    B.Continuity(TopoDS::Edge(myUEdges->Value(isec, IPath)),
                                 TopoDS::Face(myFaces->Value(isec - 1, IPath)), FF, GeomAbs_G1);
                }
            }
        }
        ...
        for (ipath = 1, IPath = IFirst; ipath <= NbPath + 1; ipath++, IPath++) {
            ...
            for (isec = 1; isec <= NbLaw; isec++) {
                ...
                if (isG1) {
                    ...
                    BRepLib::EncodeRegularity(anEdge, FF, TopoDS::Face(myFaces->Value(isec, IPath - 1)), myTolAngular);
                }
            }
        }
    }
...
//======================================================================
void BRepFill_Sweep::Build(TopTools_MapOfShape& ReversedEdges, BRepFill_DataMapOfShapeHArray2OfShape& Tapes,
                           BRepFill_DataMapOfShapeHArray2OfShape& Rails, const BRepFill_TransitionStyle Transition,
                           const GeomAbs_Shape Continuity, const GeomFill_ApproxStyle Approx,
                           const Standard_Integer Degmax, const Standard_Integer Segmax) {
    ...
    if (mySec->IsVertex())
        ...
        Standard_Integer NbTrous = myLoc->NbHoles(myTol3d), NbPath = myLoc->NbLaw(), NbLaw = mySec->NbLaw(), ii, jj,
                         NbPart = 1;
        ...
        if (NbPart == 1) { // This is done at once
            ...
            for (ii = 1, isDone = Standard_True; ii <= NbPart && isDone; ii++) {
                ...
                isDone = BuildShell(Transition, IFirst, ILast, ReversedEdges, Tapes, Rails,
                                    EvalExtrapol(IFirst, Transition), EvalExtrapol(ILast, Transition));
                if (IFirst > 1) {
        ...
        for (; It.More(); It.Next()) {
            ...
            if (!face.IsNull() && (face.ShapeType() == TopAbs_FACE)) B.Add(shell, face);
        }
        ...
        */
        ...
        for (ii = myFaces->LowerRow(); ii <= myFaces->UpperRow(); ii++) {
            for (jj = myFaces->LowerCol(); jj <= myFaces->UpperCol(); jj++) {
                ...
                if (!aLocalShape.IsNull() && aSubstitute.IsCopied(aLocalShape)) {
                    ...
                    if (!aList.IsEmpty()) myFaces->ChangeValue(ii, jj) = aList.First();
                }
            }
        }
        ...
        for (ii = myVEdges->LowerRow(); ii <= myVEdges->UpperRow(); ii++) {
            for (jj = myVEdges->LowerCol(); jj <= myVEdges->UpperCol(); jj++) {
                ...
                if (!aLocalShape.IsNull() && aSubstitute.IsCopied(aLocalShape)) {
                    ...
                    if (!aList.IsEmpty()) myVEdges->ChangeValue(ii, jj) = aList.First();
                }
            }
        }
        ...
        for (ii = myUEdges->LowerRow(); ii <= myUEdges->UpperRow(); ii++) {
            for (jj = myUEdges->LowerCol(); jj <= myUEdges->UpperCol(); jj++) {
                ...
                if (!aLocalShape.IsNull() && aSubstitute.IsCopied(aLocalShape)) {
                    ...
                    if (!aList.IsEmpty()) myUEdges->ChangeValue(ii, jj) = aList.First();
                }
            }
        }
        ...
        for (ii = myUEdges->LowerRow(); ii <= myUEdges->UpperRow(); ii++) {
            ...
            for (jj = myUEdges->LowerCol(); jj <= myUEdges->UpperCol(); jj++) {
                ...
                if (anEdge.IsNull() || BRep_Tool::Degenerated(anEdge)) continue;
                TopoDS_Face Face1, Face2;
                Standard_Integer i1 = ii - 1, i2 = ii;
                if (i1 == 0 && mySec->IsUClosed()) i1 = myFaces->UpperRow();
                if (i2 > myFaces->UpperRow()) i2 = 0;
                if (i1 != 0) {
                    const TopoDS_Shape& aShape1 = myFaces->Value(i1, jj);
                    if (aShape1.ShapeType() == TopAbs_FACE) Face1 = TopoDS::Face(aShape1);
                }
                if (i2 != 0) {
                    const TopoDS_Shape& aShape2 = myFaces->Value(i2, jj);
                    if (aShape2.ShapeType() == TopAbs_FACE) Face2 = TopoDS::Face(aShape2);
                }
                if (!Face1.IsNull() && !Face2.IsNull()) CorrectSameParameter(anEdge, Face1, Face2);
            }
        }
        ...
        for (ii = 1; ii <= NbLaw; ii++)
            for (jj = 1; jj <= NbPath; jj++) {
                ...
                if (!aFace.IsNull() && aFace.ShapeType() == TopAbs_FACE) BB.Add(myTapes->ChangeValue(ii), aFace);
            }
...
//======================================================================
Standard_Boolean BRepFill_Sweep::PerformCorner(const Standard_Integer Index, const BRepFill_TransitionStyle Transition,
    ...
    Handle(TopTools_HArray2OfShape) UEdges = new TopTools_HArray2OfShape(1, mySec->NbLaw() + 1, 1, myLoc->NbLaw());
        ...
        for (ii = 1; ii <= mySec->NbLaw(); ii++) {
            ...
            if (list1.Extent() == list2.Extent()) {
                ...
                for (; It1.More(); It1.Next(), It2.Next()) {
                    ...
                    // Filling
                    B = Filling(It1.Value(), myFaces->Value(ii, I1), It2.Value(), myFaces->Value(ii, I2),
                    ...
                    if (B) {
                        ...
                        if (!aVshape.IsNull()) BB.Add(aCompound, aVshape);
                        BB.Add(aCompound, FF);
...
//======================================================================
void BRepFill_Sweep::UpdateVertex(const Standard_Integer ipath, const Standard_Integer isec, const Standard_Real ErrApp,
                                  const Standard_Real Param, TopoDS_Shape& V) const {
    TopoDS_Vertex vv, TheV;
    TheV = TopoDS::Vertex(V);
    myLoc->PerformVertex(ipath, mySec->Vertex(isec, Param), ErrApp + mySec->VertexTol(isec - 1, Param), vv);
    // Class BRep_Tool without fields and without Constructor :
    //  BRep_Tool BT;
    gp_Pnt P1, P2;
    //  P1 = BT.Pnt(vv);
...
//======================================================================
void BRepFill_Sweep::RebuildTopOrBottomEdge(const TopoDS_Edge& aNewEdge, TopoDS_Edge& anEdge,
        ...
        if (!V1.IsSame(V2)) {
            ...
            if (!V1.IsSame(NewV1)) {
                if (V1.IsSame(NewV2))
                    ...
                    if (!Pnt1.IsEqual(NewPnt1, TolSum)) ToReverse = Standard_True;
                }
            }
        } else {
            ...
            if (OldD1 * NewD1 < 0.) ToReverse = Standard_True;
        }
    }
