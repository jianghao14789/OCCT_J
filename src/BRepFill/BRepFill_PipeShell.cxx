...
static Standard_Boolean BuildBoundaries(const BRepFill_Sweep& theSweep, const Handle(BRepFill_SectionLaw) & theSection,
                                        TopoDS_Shape& theBottom, TopoDS_Shape& theTop);
...
static Standard_Boolean ComputeSection(const TopoDS_Wire& W1, const TopoDS_Wire& W2, const Standard_Real p1,
                                       const Standard_Real p2, TopoDS_Wire& Wres) {
...
static void PerformTransition(const BRepFill_TransitionStyle Mode, Handle(BRepFill_LocationLaw) & Loc,
...
static Standard_Boolean PerformPlan(TopoDS_Shape& S) {
    ...
    for (; explo.More(); explo.Next()) {
        ...
        if (!BRep_Tool::Degenerated(anEdge)) isDegen = Standard_False;
    }
    if (isDegen) {
        S.Nullify();
        return Standard_True;
...
static Standard_Boolean IsSameOriented(const TopoDS_Shape& aFace, const TopoDS_Shape& aShell) {
    ...
    for (Explo.Init(AdjacentFace, TopAbs_EDGE); Explo.More(); Explo.Next()) {
        ...
        if (theEdge.IsSame(anEdge)) break;
    }

    TopAbs_Orientation Or2 = theEdge.Orientation();
    if (Or1 == Or2) return Standard_False;
    return Standard_True;
}
//=======================================================================
...
BRepFill_PipeShell::BRepFill_PipeShell(const TopoDS_Wire& Spine)
    : mySpine(Spine), myForceApproxC1(Standard_False), myIsAutomaticLaw(Standard_False),
      myTrihedron(GeomFill_IsCorrectedFrenet), myTransition(BRepFill_Modified), myStatus(GeomFill_PipeOk) {
    myLocation.Nullify();
    mySection.Nullify();
    myLaw.Nullify();
    SetTolerance();
...
//=======================================================================
void BRepFill_PipeShell::Set(const TopoDS_Wire& AuxiliarySpine, const Standard_Boolean CurvilinearEquivalence,
    ...
    if (KeepContact == BRepFill_ContactOnBorder) myIsAutomaticLaw = Standard_True;
...
#ifdef DRAW
    if (Affich) DBRep::Set("theguide", TheGuide);
#endif
...
//=======================================================================
void BRepFill_PipeShell::Add(const TopoDS_Shape& Profile, const Standard_Boolean WithContact,
...
// purpose  : Add a Section
//=======================================================================
void BRepFill_PipeShell::Add(const TopoDS_Shape& Profile, const TopoDS_Vertex& Location,
...
//=======================================================================
void BRepFill_PipeShell::SetLaw(const TopoDS_Shape& Profile, const Handle(Law_Function) & L,
                                const Standard_Boolean WithContact, const Standard_Boolean WithCorrection) {
    TopoDS_Vertex V;
    V.Nullify();
...
//=======================================================================
void BRepFill_PipeShell::SetLaw(const TopoDS_Shape& Profile, const Handle(Law_Function) & L,
                                const TopoDS_Vertex& Location, const Standard_Boolean WithContact,
                                const Standard_Boolean WithCorrection) {
    mySeq.Clear();
    BRepFill_Section S(Profile, Location, WithContact, WithCorrection);
    S.Set(Standard_True);
...
// purpose  :
//=======================================================================
void BRepFill_PipeShell::SetTolerance(const Standard_Real Tol3d, const Standard_Real BoundTol,
...
//=======================================================================
void BRepFill_PipeShell::SetTransition(const BRepFill_TransitionStyle Mode, const Standard_Real Angmin,
                                       const Standard_Real Angmax) {
    if (myTransition != Mode) mySection.Nullify(); // It is required to relocalize the sections.
    myTransition = Mode;
    angmin = Angmin;
    angmax = Angmax;
}
...
Standard_Boolean BRepFill_PipeShell::Build() {
    ...
    if (mySection->IsVClosed() && myLocation->IsClosed()) {
        ...
        if (myFirst.IsPartner(myLast)) {
            ...
            if (copy.IsDone()) myLast = copy.Shape();
        }
    ...
    if (myTrihedron == GeomFill_IsDiscreteTrihedron) theContinuity = GeomAbs_C0;
    TopTools_MapOfShape Dummy;
    BRepFill_DataMapOfShapeHArray2OfShape Dummy2;
    BRepFill_DataMapOfShapeHArray2OfShape Dummy3;
    MkSw.Build(Dummy, Dummy2, Dummy3, myTransition, theContinuity, GeomFill_Location, myMaxDegree, myMaxSegments);
    ...
    if (Ok) {
        ...
        if (mySection->IsUClosed()) {
            ...
            if (DegenFirst && DegenLast) myShape.Closed(Standard_True);
        }
...
Standard_Boolean BRepFill_PipeShell::MakeSolid() {
    if (myShape.IsNull()) throw StdFail_NotDone("PipeShell is not built");
    ...
    if (!B) {
        ...
        if (B) {
            ...
            if (B) {
                ...
                if (B) {
                    if (!myFirst.IsNull() && !IsSameOriented(myFirst, myShape)) myFirst.Reverse();
                    if (!myLast.IsNull() && !IsSameOriented(myLast, myShape)) myLast.Reverse();

                    if (!myFirst.IsNull()) BS.Add(myShape, TopoDS::Face(myFirst));
                    if (!myLast.IsNull()) BS.Add(myShape, TopoDS::Face(myLast));
...
void BRepFill_PipeShell::Prepare() {
    ...
    for (i = 2; i <= mySeq.Length() - 1; i++) {
        ...
        if (wdeg) throw Standard_Failure("Wrong usage of punctual sections");
    }
    if (mySeq.Length() <= 2) {
        Standard_Boolean wdeg = Standard_True;
    ...
    if (mySeq.Length() == 1) {
        ...
        if (mySeq(1).IsLaw()) mySection = new BRepFill_ShapeLaw(aLocalShape, myLaw);
    ...
    if ((myTrihedron == GeomFill_IsGuidePlanWithContact) || (myTrihedron == GeomFill_IsGuideACWithContact)) {
        ...
        for (Standard_Integer ipath = 1; ipath <= myLocation->NbLaw(); ipath++) {
            ...
            Loc->Set(Sec, Standard_True, fs + f * Delta, fs + l * Delta, old_angle, angle); // force the rotation
            old_angle = angle;
        }
    }

    myStatus = myLocation->GetStatus();
    if (!mySection->IsDone()) myStatus = GeomFill_PipeNotOk;
}
...
//=======================================================================
void BRepFill_PipeShell::Place(const BRepFill_Section& Sec, TopoDS_Wire& W, gp_Trsf& aTrsf, Standard_Real& param) {
    BRepFill_SectionPlacement Place(myLocation, Sec.Wire(), Sec.Vertex(), Sec.WithContact(), Sec.WithCorrection());
...
void BRepFill_PipeShell::BuildHistory(const BRepFill_Sweep& theSweep) {
    ...
    for (indw = 1; indw <= mySeq.Length(); indw++) {
        ...
        for (inde = 1; wexp_sec.More(); wexp_sec.Next()) {
            ...
            if (mySection->IsUClosed()) {
                if (UIndex[0] > mySection->NbLaw()) UIndex[0] = 1;
                if (UIndex[1] > mySection->NbLaw()) UIndex[1] = 1;
            }
            ...
            for (Standard_Integer kk = 0; kk < 2; kk++) {
                if (myGenMap.IsBound(aVertex[kk])) continue;
                ...
                for (jj = 2; jj <= SeqEdges.Length(); jj++) {
                    ...
                    else // a hole
                    {
                        ...
                        for (; itl.More(); itl.Next()) {
                            ...
                            if (Candidate.IsSame(CurEdge)) continue;
                            TopoDS_Vertex V1, V2;
    ...
    for (;;) {
        if (!wexp.More()) ToExit = Standard_True;
        ...
        if (!ToExit) {
            ...
            for (Standard_Integer i = 1; i <= aFaces->UpperRow(); i++) {
                ...
                if (aFace.ShapeType() == TopAbs_FACE) Flist->Append(aFace);
            }
        }
...
// purpose:
// ---------------------------------------------------------------------------------
Standard_Boolean BuildBoundaries(const BRepFill_Sweep& theSweep, const Handle(BRepFill_SectionLaw) & theSection,
                                 TopoDS_Shape& theBottom, TopoDS_Shape& theTop) {
    ...
    for (i = 1; i <= theSection->NbLaw(); i++) {
        ...
        if (!aBottomEdge.IsNull() && !aTopEdge.IsNull() && !aBottomEdge.IsSame(aTopEdge)) bAllSame = Standard_False;
    }
    ...
    if (bAllSame && bfoundbottom && bfoundtop) theTop = theBottom;
