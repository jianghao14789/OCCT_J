...
template <class T, int N> struct BVH_Bin {
...
public:
    ...
    BVH_BinnedBuilder(const Standard_Integer theLeafNodeSize = BVH_Constants_LeafNodeSizeDefault,
        ...
        : BVH_QueueBuilder<T, N>(theLeafNodeSize, theMaxTreeDepth, theNumOfThreads), myUseMainAxis(theDoMainSplits) {
        //
    }
...
protected:
    ...
    virtual typename BVH_QueueBuilder<T, N>::BVH_ChildNodes
    buildNode(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH, const Standard_Integer theNode) const Standard_OVERRIDE;

    //! Arranges node primitives into bins.
    virtual void getSubVolumes(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH, const Standard_Integer theNode,
                               BVH_BinVector& theBins, const Standard_Integer theAxis) const;
...
void BVH_BinnedBuilder<T, N, Bins>::getSubVolumes(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH,
                                                  const Standard_Integer theNode, BVH_BinVector& theBins,
...
Standard_Integer SplitPrimitives(BVH_Set<T, N>* theSet, const BVH_Box<T, N>& theBox, const Standard_Integer theBeg,
                                 const Standard_Integer theEnd, const Standard_Integer theBin,
                                 const Standard_Integer theAxis, const Standard_Integer theBins) {
...
template <class T, int N> struct BVH_AxisSelector {
...
template <class T> struct BVH_AxisSelector<T, 2> {
...
// =======================================================================
template <class T, int N, int Bins>
typename BVH_QueueBuilder<T, N>::BVH_ChildNodes
BVH_BinnedBuilder<T, N, Bins>::buildNode(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH,
    ...
    // Find best split
    for (Standard_Integer anAxis = myUseMainAxis ? aMainAxis : 0; anAxis <= (myUseMainAxis ? aMainAxis : Min(N - 1, 2));
         ++anAxis) {
        ...
        for (Standard_Integer aSplit = 1; aSplit < Bins; ++aSplit) {
            ...
            Standard_Real aCost = (static_cast<Standard_Real>(aSplitPlanes[aSplit].LftVoxel.Box.Area()) /* / S(N) */) *
    ...
    if (aMinSplitNumLft == 0 || aMinSplitNumRgh == 0 || aMinSplitAxis == -1) // case of objects with the same center
    {
        ...
        aMiddle = BVH::SplitPrimitives<T, N>(theSet, anAABB, aNodeBegPrimitive, aNodeEndPrimitive, aMinSplitIndex - 1,
                                             aMinSplitAxis, Bins);
    }

    typedef typename BVH_QueueBuilder<T, N>::BVH_PrimitiveRange Range;
    return typename BVH_QueueBuilder<T, N>::BVH_ChildNodes(
        aMinSplitBoxLft, aMinSplitBoxRgh, Range(aNodeBegPrimitive, aMiddle - 1), Range(aMiddle, aNodeEndPrimitive));
}
