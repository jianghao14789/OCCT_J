...
template <class T, int N> class BVH_LinearBuilder : public BVH_Builder<T, N> {
public:
...
protected:
    ...
    Standard_Integer emitHierachy(BVH_Tree<T, N>* theBVH, const NCollection_Array1<BVH_EncodedLink>& theEncodedLinks,
                                  const Standard_Integer theBit, const Standard_Integer theShift,
                                  const Standard_Integer theStart, const Standard_Integer theFinal) const;

    //! Returns index of the first element which does not compare less than the given one.
    Standard_Integer lowerBound(const NCollection_Array1<BVH_EncodedLink>& theEncodedLinks, Standard_Integer theStart,
                                Standard_Integer theFinal, Standard_Integer theDigit) const;
};
...
Standard_Integer BVH_LinearBuilder<T, N>::lowerBound(const NCollection_Array1<BVH_EncodedLink>& theEncodedLinks,
                                                     Standard_Integer theStart, Standard_Integer theFinal,
...
Standard_Integer BVH_LinearBuilder<T, N>::emitHierachy(BVH_Tree<T, N>* theBVH,
                                                       ...
                                                       const Standard_Integer theDigit, const Standard_Integer theShift,
    ...
    if (theFinal - theStart > BVH_Builder<T, N>::myLeafNodeSize) {
        const Standard_Integer aPosition =
            theDigit < 0 ? (theStart + theFinal) / 2 : lowerBound(theEncodedLinks, theStart, theFinal, theDigit);
...
template <class T, int N> struct BoundData {
...
template <class T, int N> class UpdateBoundTask {
public:
    UpdateBoundTask(const Standard_Boolean isParallel) : myIsParallel(isParallel) {}
        ...
        if (theData.myBVH->IsOuter(theData.myNode) || theData.myLevel > 2) {
            *theData.myHeight = BVH::UpdateBounds(theData.mySet, theData.myBVH, theData.myNode);
            ...
            if (!theData.myBVH->IsOuter(aLftChild)) {
                BoundData<T, N> aBoundData = {theData.mySet, theData.myBVH, aLftChild, theData.myLevel + 1,
                                              &aLftHeight};
            ...
            if (!theData.myBVH->IsOuter(aRghChild)) {
                BoundData<T, N> aBoundData = {theData.mySet, theData.myBVH, aRghChild, theData.myLevel + 1,
                                              &aRghHeight};
                aList.push_back(aBoundData);
            } else {
...
// =======================================================================
template <class T, int N>
void BVH_LinearBuilder<T, N>::Build(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH, const BVH_Box<T, N>& theBox) const {
