...
template <class T, int N> class BVH_SweepPlaneBuilder : public BVH_QueueBuilder<T, N> {
public:
...
protected:
    ...
    typename BVH_QueueBuilder<T, N>::BVH_ChildNodes buildNode(BVH_Set<T, N>* theSet, BVH_Tree<T, N>* theBVH,
        ...
        for (Standard_Integer anAxis = 0; anAxis < (N < 4 ? N : 3); ++anAxis) {
            ...
            for (Standard_Integer aNbLft = 1, aNbRgh = aNodeNbPrimitives - 1; aNbLft < aNodeNbPrimitives;
                 ++aNbLft, --aNbRgh) {
                Standard_Real aCost =
                    (aLftSet(aNbLft) /* / aNodeArea */) * aNbLft + (aRghSet(aNbRgh) /* / aNodeArea */) * aNbRgh;
