...
template <class T, int N> class BVH_RadixSorter : public BVH_Sorter<T, N> {
public:
...
private:
    ...
    public:
        Functor(const SortRange (&aSplits)[2], const Standard_Boolean isParallel)
            : mySplits(aSplits), myIsParallel(isParallel) {}

        //! Runs sorting function for the given range.
        void operator()(const Standard_Integer theIndex) const {
            RadixSorter::Sort(mySplits[theIndex].myStart, mySplits[theIndex].myFinal, mySplits[theIndex].myDigit,
                              myIsParallel);
        }
...
public:
    static void Sort(LinkIterator theStart, LinkIterator theFinal, Standard_Integer theDigit,
        ...
        if (theDigit < 24) {
            ...
            SortRange aSplits[2] = {{theStart, anOffset, theDigit - 1}, {anOffset, theFinal, theDigit - 1}};
...
void BVH_RadixSorter<T, N>::Perform(BVH_Set<T, N>* theSet, const Standard_Integer theStart,
