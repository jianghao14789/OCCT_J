...
template <class T, int N> class BVH_Tree<T, N, BVH_BinaryTree> : public BVH_TreeBase<T, N> {
...
public: //! @name methods for accessing individual nodes
    ...
    template <int K> int Child(const int theNodeIndex) const {
        return BVH::Array<int, 4>::Value(this->myNodeInfoBuffer, theNodeIndex)[K + 1];
    }

    //! Returns index of the K-th child of the given inner node.
    //! \tparam K the index of node child (0 or 1)
    template <int K> int& ChangeChild(const int theNodeIndex) {
        return BVH::Array<int, 4>::ChangeValue(this->myNodeInfoBuffer, theNodeIndex)[K + 1];
    }

    //! Returns index of the K-th child of the given inner node.
    //! \tparam K the index of node child (0 or 1)
    template <int K> int& Child(const int theNodeIndex) {
        return BVH::Array<int, 4>::ChangeValue(this->myNodeInfoBuffer, theNodeIndex)[K + 1];
    }
...
public: //! @name methods for adding/removing tree nodes
    ...
    int AddLeafNode(const BVH_VecNt& theMinPoint, const BVH_VecNt& theMaxPoint, const int theBegElem,
    ...
    int AddInnerNode(const BVH_VecNt& theMinPoint, const BVH_VecNt& theMaxPoint, const int theLftChild,
...
void EstimateSAH(const BVH_Tree<T, N, BVH_BinaryTree>* theTree, const int theNode, T theProb, T& theSAH) {
    ...
    if (theTree->IsOuter(theNode)) {
        ...
        if (theProb > 0.0) {
            EstimateSAH(theTree, theTree->template Child<0>(theNode), theProb * aLftBox.Area() / aBox.Area(), theSAH);
        }
        ...
        if (theProb > 0.0) {
            EstimateSAH(theTree, theTree->template Child<1>(theNode), theProb * aRghBox.Area() / aBox.Area(), theSAH);
        }
    }
}
} // namespace BVH
...
// purpose  :
// =======================================================================
template <class T, int N> T BVH_Tree<T, N, BVH_BinaryTree>::EstimateSAH() const {
...
// purpose  :
// =======================================================================
template <class T, int N> BVH_Tree<T, N, BVH_QuadTree>* BVH_Tree<T, N, BVH_BinaryTree>::CollapseToQuadTree() const {
    ...
    for (int aNbNodes = 1; !aQueue.empty();) {
        ...
        BVH::Array<T, N>::Append(aQBVH->myMinPointBuffer,
                                 BVH::Array<T, N>::Value(this->myMinPointBuffer, opencascade::std::get<0>(aNode)));
        BVH::Array<T, N>::Append(aQBVH->myMaxPointBuffer,
                                 BVH::Array<T, N>::Value(this->myMaxPointBuffer, opencascade::std::get<0>(aNode)));

        BVH_Vec4i aNodeInfo;
        if (this->IsOuter(opencascade::std::get<0>(aNode))) // is leaf node
        {
            aNodeInfo = BVH_Vec4i(1 /* leaf flag */, this->BegPrimitive(opencascade::std::get<0>(aNode)),
                                  this->EndPrimitive(opencascade::std::get<0>(aNode)),
                                  opencascade::std::get<1>(aNode) /* level */);
        } else {
            ...
            aNodeInfo = BVH_Vec4i(0 /* inner flag */, aNbNodes, aGrandChildNodes.Size() - 1,
                                  opencascade::std::get<1>(aNode) /* level */);
