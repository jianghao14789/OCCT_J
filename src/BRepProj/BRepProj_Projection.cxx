...
void BRepProj_Projection::BuildSection(const TopoDS_Shape& theShape, const TopoDS_Shape& theTool) {
    ...
    if (theShape.ShapeType() == TopAbs_FACE || theShape.ShapeType() == TopAbs_SHELL ||
        ...
        for (; exp.More(); exp.Next()) {
            if (C.IsNull()) B.MakeCompound(C);
            B.Add(C, exp.Current());
        }
        aShape = C;
    }
    if (aShape.IsNull()) throw Standard_ConstructionError(__FILE__ ": target shape has no faces");
    ...
    if (anEdges->Length() <= 0) return;

    // connect edges to wires using ShapeAnalysis functionality
    ShapeAnalysis_FreeBounds::ConnectEdgesToWires(anEdges, Precision::Confusion(), Standard_True, mySection);
...
BRepProj_Projection::BRepProj_Projection(const TopoDS_Shape& Wire, const TopoDS_Shape& Shape, const gp_Dir& D)
...
BRepProj_Projection::BRepProj_Projection(const TopoDS_Shape& Wire, const TopoDS_Shape& Shape, const gp_Pnt& P)
    : myIsDone(Standard_False), myItr(0) {
    // Check the input
    Standard_NullObject_Raise_if((Wire.IsNull() || Shape.IsNull()), __FILE__ ": null input shape");
    if (Wire.ShapeType() != TopAbs_EDGE && Wire.ShapeType() != TopAbs_WIRE)
    ...
    if (Abs(Scale) < Precision::Confusion()) throw Standard_ConstructionError("Projection");
    Scale = 1. + mdis / Scale;
