...
void XmlLDrivers_DocumentStorageDriver::Write(const Handle(CDM_Document) & theDocument,
    ...
    if (aFileStream.get() != NULL && aFileStream->good()) {
        ...
        TCollection_ExtendedString aMsg =
            TCollection_ExtendedString("Error: the file ") + theFileName + " cannot be opened for writing";
...
// purpose  :
//=======================================================================
void XmlLDrivers_DocumentStorageDriver::Write(const Handle(CDM_Document) & theDocument, Standard_OStream& theOStream,
                                              const Message_ProgressRange& theRange) {
...
Standard_Boolean XmlLDrivers_DocumentStorageDriver::WriteToDomDocument(const Handle(CDM_Document) & theDocument,
    ...
    for (i = 1; i <= mySeqOfNS.Length(); i++) {
        TCollection_AsciiString aPrefix = TCollection_AsciiString("xmlns:") + mySeqOfNS(i).Prefix().ToCString();
    ...
    if (!aResourceDir.IsEmpty()) {
        ...
        if (aResourceFile.Exists()) {
            if (aToSetCSFVariable) {
                ...
                aWarn1 = (aWarn1 + aCSFVariable[0] + " has not been explicitly defined. Set to " + aResourceDir);
    ...
    TDocStd_FormatVersion aFormatVersion =
        TDocStd_Document::CurrentStorageFormatVersion(); // the last version of the format
    ...
    if (myCopyright.Length() > 0) aUserInfo.Append(TCollection_AsciiString(myCopyright, '?'));
...
// purpose  :
//=======================================================================
Standard_Integer XmlLDrivers_DocumentStorageDriver::MakeDocument(const Handle(CDM_Document) & theTDoc,
                                                                 XmlObjMgt_Element& theElement,
                                                                 const Message_ProgressRange& theRange) {
...
// purpose  :
//=======================================================================
Handle(XmlMDF_ADriverTable) XmlLDrivers_DocumentStorageDriver::AttributeDrivers(const Handle(Message_Messenger) &
                                                                                theMessageDriver) {
...
//=======================================================================
Standard_Boolean
XmlLDrivers_DocumentStorageDriver::WriteShapeSection(XmlObjMgt_Element& /*theElement*/,
