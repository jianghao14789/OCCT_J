...
static TCollection_AsciiString GetDirFromFile(const TCollection_ExtendedString& aFileName) {
    ...
    if (theCFile.SearchFromEnd("\\") > i) i = theCFile.SearchFromEnd("\\");
#endif
...
#ifdef _WIN32
    if (aRelFilePath.Search(":") == 2 || (aRelFilePath.Search("\\") == 1 && aRelFilePath.Value(2) == '\\'))
#else
...
void XmlLDrivers_DocumentRetrievalDriver::Read(const TCollection_ExtendedString& theFileName,
    ...
    if (aFileStream.get() != NULL && aFileStream->good()) {
        ...
        TCollection_ExtendedString aMsg =
            TCollection_ExtendedString("Error: the file ") + theFileName + " cannot be opened for reading";
...
void XmlLDrivers_DocumentRetrievalDriver::ReadFromDomDocument(const XmlObjMgt_Element& theElement,
                                                              const Handle(CDM_Document) & theNewDocument,
                                                              const Handle(CDM_Application) & theApplication,
                                                              const Message_ProgressRange& theRange) {
    ...
    if (anInfoElem != NULL) {
        ...
        if (aCurDocVersion > TDocStd_Document::CurrentStorageFormatVersion()) {
            TCollection_ExtendedString aMsg = TCollection_ExtendedString("error: wrong file version: ") + aDocVerStr +
                                              " while current is " + TDocStd_Document::CurrentStorageFormatVersion();
            myReaderStatus = PCDM_RS_NoVersion;
            if (!aMsgDriver.IsNull()) aMsgDriver->Send(aMsg.ToExtString(), Message_Fail);
            return;
        }

        Standard_Boolean isRef = Standard_False;
        for (LDOM_Node aNode = anInfoElem.getFirstChild(); aNode != NULL; aNode = aNode.getNextSibling()) {
                ...
                if (XmlObjMgt::GetExtendedString((LDOM_Element&)aNode, anInfo)) {
                    ...
                    if (anInfo.Search(REFERENCE_COUNTER) != -1) {
                        try {
                            ...
                            if (!aMsgDriver.IsNull()) aMsgDriver->Send(aMsg.ToExtString(), Message_Warning);
                        }
                    ...
                    if (anInfo == END_REF) isRef = Standard_False;
                    if (isRef) { // Process References
                        ...
                        if (pos != -1) {
                            ...
                            Handle(CDM_MetaData) aMetaData =
                                CDM_MetaData::LookUp(theApplication->MetaDataLookUpTable(), theFolder, theName, aPath,
                                                     aPath, UTL::IsReadOnly(aFileName));
                            ////////////
                            theNewDocument->CreateReference(aMetaData, aRefId, theApplication, aDocumentVersion,
                                                            Standard_False);
                        }
                    }
                    if (anInfo == START_REF) isRef = Standard_True;
                }
            }
        }
    }
    ...
    if (aCommentsElem != NULL) {
        for (LDOM_Node aNode = aCommentsElem.getFirstChild(); aNode != NULL; aNode = aNode.getNextSibling()) {
            if (aNode.getNodeType() == LDOM_Node::ELEMENT_NODE) {
    ...
    if (!aNSDriver.IsNull()) ::take_time(0, " +++++ Fin reading Shapes :    ", aMsgDriver);
...
// purpose  :
//=======================================================================
Standard_Boolean XmlLDrivers_DocumentRetrievalDriver::MakeDocument(const XmlObjMgt_Element& theElement,
...
// purpose  :
//=======================================================================
Handle(XmlMDF_ADriverTable) XmlLDrivers_DocumentRetrievalDriver::AttributeDrivers(const Handle(Message_Messenger) &
                                                                                  theMessageDriver) {
...
Handle(XmlMDF_ADriver) XmlLDrivers_DocumentRetrievalDriver::ReadShapeSection(
    const XmlObjMgt_Element& /*theElement*/, const Handle(Message_Messenger) & /*aMsgDriver*/,
    const Message_ProgressRange& /*theRange*/) {
...
//=======================================================================
void XmlLDrivers_DocumentRetrievalDriver::ShapeSetCleaning(const Handle(XmlMDF_ADriver) & /*theDriver*/) {}
