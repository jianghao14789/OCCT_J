...
const Handle(Standard_Transient) & XmlLDrivers::Factory(const Standard_GUID& theGUID) {
    if (theGUID == XmlLStorageDriver) {
        ...
        static Handle(Standard_Transient) model_sd =
            new XmlLDrivers_DocumentStorageDriver("Copyright: Open Cascade, 2001-2002"); // default copyright
        return model_sd;
    }
...
void XmlLDrivers::DefineFormat(const Handle(TDocStd_Application) & theApp) {
    theApp->DefineFormat("XmlLOcaf", "Xml Lite OCAF Document", "xmll", new XmlLDrivers_DocumentRetrievalDriver,
...
// purpose  :
//=======================================================================
Handle(XmlMDF_ADriverTable) XmlLDrivers::AttributeDrivers(const Handle(Message_Messenger) & theMessageDriver) {
