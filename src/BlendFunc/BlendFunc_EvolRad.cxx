...
static void FusionneIntervalles(const TColStd_Array1OfReal& I1, const TColStd_Array1OfReal& I2,
...
BlendFunc_EvolRad::BlendFunc_EvolRad(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_Surface) & S2,
                                     const Handle(Adaptor3d_Curve) & C, const Handle(Law_Function) & Law)
    : surf1(S1), surf2(S2), curv(C), tcurv(C), istangent(Standard_True), xval(1, 4), E(1, 4), DEDX(1, 4, 1, 4),
      DEDT(1, 4), D2EDX2(4, 4, 4), D2EDXDT(1, 4, 1, 4), D2EDT2(1, 4), minang(RealLast()), maxang(RealFirst()),
      lengthmin(RealLast()), lengthmax(RealFirst()), distmin(RealLast()), mySShape(BlendFunc_Rational) {
    fevol = Law;
    tevol = Law;
...
Standard_Boolean BlendFunc_EvolRad::ComputeValues(const math_Vector& X, const Standard_Integer Order,
...
void BlendFunc_EvolRad::Tangent(const Standard_Real U1, const Standard_Real V1, const Standard_Real U2,
...
void BlendFunc_EvolRad::Section(const Standard_Real Param, const Standard_Real U1, const Standard_Real V1,
...
void BlendFunc_EvolRad::GetShape(Standard_Integer& NbPoles, Standard_Integer& NbKnots, Standard_Integer& Degree,
...
//=======================================================================
void BlendFunc_EvolRad::GetTolerance(const Standard_Real BoundTol, const Standard_Real SurfTol,
...
void BlendFunc_EvolRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles, TColgp_Array1OfPnt2d& Poles2d,
...
Standard_Boolean BlendFunc_EvolRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles, TColgp_Array1OfVec& DPoles,
    ...
    if (!istgt) {
        return GeomFill::GetCircle(myTConv, ns1, ns2, dnorm1w, dnorm2w, np, dnp, pts1, pts2, tg1, tg2, Abs(ray),
                                   rayprim, Center, tgc, Poles, DPoles, Weights, DWeights);
    } else {
        GeomFill::GetCircle(myTConv, ns1, ns2, np, pts1, pts2, Abs(ray), Center, Poles, Weights);
        return Standard_False;
    }
...
Standard_Boolean BlendFunc_EvolRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles, TColgp_Array1OfVec& DPoles,
                                            TColgp_Array1OfVec& D2Poles, TColgp_Array1OfPnt2d& Poles2d,
                                            TColgp_Array1OfVec2d& DPoles2d, TColgp_Array1OfVec2d& D2Poles2d,
                                            TColStd_Array1OfReal& Weights, TColStd_Array1OfReal& DWeights,
    ...
    */
    ...
    if (!istgt) {
        return GeomFill::GetCircle(myTConv, ns1, ns2, dnorm1w, dnorm2w, d2norm1w, d2norm2w, np, dnp, d2np, pts1, pts2,
                                   tg1, tg2, dtg1, dtg2, Abs(ray), rayprim, raysecn, Center, tgc, dtgc, Poles, DPoles,
                                   D2Poles, Weights, DWeights, D2Weights);
    } else {
...
void BlendFunc_EvolRad::Resolution(const Standard_Integer IC2d, const Standard_Real Tol, Standard_Real& TolU,
