...
BlendFunc_RuledInv::BlendFunc_RuledInv(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_Surface) & S2,
                                       const Handle(Adaptor3d_Curve) & C)
    : surf1(S1), surf2(S2), curv(C), first(Standard_False) {}
...
Standard_Boolean BlendFunc_RuledInv::Derivatives(const math_Vector& X, math_Matrix& D) {
    ...
    resul1.SetLinearForm(-(grosterme * ndotns1 - nplan.Dot(temp)) / norm1, nplan, grosterme / norm1, ns1, -1. / norm1,
                         temp);
    ...
    grosterme = ncrossns1.Dot(nplan.Crossed(temp)) / norm1 / norm1;
    resul2.SetLinearForm(-(grosterme * ndotns1 - nplan.Dot(temp)) / norm1, nplan, grosterme / norm1, ns1, -1. / norm1,
                         temp);
    ...
    resul1.SetLinearForm(-(grosterme * ndotns2 - nplan.Dot(temp)) / norm2, nplan, grosterme / norm2, ns2, -1. / norm2,
                         temp);

    // Derivee par rapport a v2
    temp = d2uv2.Crossed(d1v2).Added(d1u2.Crossed(d2v2));
    grosterme = ncrossns2.Dot(nplan.Crossed(temp)) / norm2 / norm2;
    resul2.SetLinearForm(-(grosterme * ndotns2 - nplan.Dot(temp)) / norm2, nplan, grosterme / norm2, ns2, -1. / norm2,
                         temp);
    ...
    resul1.SetLinearForm(-(grosterme * ndotns1 - dnplan.Dot(ns1)) / norm1, nplan, ndotns1 / norm1, dnplan,
                         grosterme / norm1, ns1);

    grosterme = ncrossns2.Dot(dnplan.Crossed(ns2)) / norm2 / norm2;
    resul2.SetLinearForm(-(grosterme * ndotns2 - dnplan.Dot(ns2)) / norm2, nplan, ndotns2 / norm2, dnplan,
                         grosterme / norm2, ns2);
