...
BlendFunc_EvolRadInv::BlendFunc_EvolRadInv(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_Surface) & S2,
...
Standard_Boolean BlendFunc_EvolRadInv::Values(const math_Vector& X, math_Vector& F, math_Matrix& D) {
    ...
    resul1.SetLinearForm(-sg1 * ray / norm1 * (grosterme * ndotns1 - nplan.Dot(temp)), nplan,
                         sg1 * ray * grosterme / norm1, ns1, -sg1 * ray / norm1, temp, d1u1);

    // Derivee par rapport a v1

    temp = d2uv1.Crossed(d1v1).Added(d1u1.Crossed(d2v1));
    grosterme = ncrossns1.Dot(nplan.Crossed(temp)) / norm1 / norm1;
    resul2.SetLinearForm(-sg1 * ray / norm1 * (grosterme * ndotns1 - nplan.Dot(temp)), nplan,
                         sg1 * ray * grosterme / norm1, ns1, -sg1 * ray / norm1, temp, d1v1);
    ...
    resul1.SetLinearForm(-sg1 / norm1 * (grosterme * ndotns1 - dnplan.Dot(ns1)), nplan, sg1 * ndotns1 / norm1, dnplan,
                         sg1 * grosterme / norm1, ns1);

    grosterme = ncrossns2.Dot(dnplan.Crossed(ns2)) / norm2 / norm2;
    resul2.SetLinearForm(sg2 / norm2 * (grosterme * ndotns2 - dnplan.Dot(ns2)), nplan, -sg2 * ndotns2 / norm2, dnplan,
    ...
    temp.SetLinearForm(sg1 * ndotns1 / norm1 - sg2 * ndotns2 / norm2, nplan, -sg1 / norm1, ns1, sg2 / norm2, ns2);
    ...
    resul1.SetLinearForm(sg2 * ray / norm2 * (grosterme * ndotns2 - nplan.Dot(temp)), nplan,
                         -sg2 * ray * grosterme / norm2, ns2, sg2 * ray / norm2, temp);
