...
BlendFunc_ConstRadInv::BlendFunc_ConstRadInv(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_Surface) & S2,
                                             const Handle(Adaptor3d_Curve) & C)
    : surf1(S1), surf2(S2), curv(C), ray1(0.0), ray2(0.0), choix(0), first(Standard_False) {}
...
Standard_Boolean BlendFunc_ConstRadInv::IsSolution(const math_Vector& Sol, const Standard_Real Tol) {
    ...
    Value(Sol, valsol);
    if (Abs(valsol(1)) <= Tol && valsol(2) * valsol(2) + valsol(3) * valsol(3) + valsol(4) * valsol(4) <= Tol * Tol) {
...
Standard_Boolean BlendFunc_ConstRadInv::Value(const math_Vector& X, math_Vector& F) {
    ...
    F(1) = (nplan.X() * (pts1.X() + pts2.X()) + nplan.Y() * (pts1.Y() + pts2.Y()) + nplan.Z() * (pts1.Z() + pts2.Z())) /
               2. +
           theD;
...
Standard_Boolean BlendFunc_ConstRadInv::Derivatives(const math_Vector& X, math_Matrix& D) {
    ...
    resul1.SetLinearForm(-ray1 / norm1 * (grosterme * ndotns1 - nplan.Dot(temp)), nplan, ray1 * grosterme / norm1, ns1,
    ...
    grosterme = ncrossns1.Dot(nplan.Crossed(temp)) / norm1 / norm1;
    resul2.SetLinearForm(-ray1 / norm1 * (grosterme * ndotns1 - nplan.Dot(temp)), nplan, ray1 * grosterme / norm1, ns1,
                         -ray1 / norm1, temp, d1v1);
    ...
    resul1.SetLinearForm(-ray1 / norm1 * (grosterme * ndotns1 - dnplan.Dot(ns1)), nplan, ray1 * ndotns1 / norm1, dnplan,
                         ray1 * grosterme / norm1, ns1);

    grosterme = ncrossns2.Dot(dnplan.Crossed(ns2)) / norm2 / norm2;
    resul2.SetLinearForm(ray2 / norm2 * (grosterme * ndotns2 - dnplan.Dot(ns2)), nplan, -ray2 * ndotns2 / norm2, dnplan,
    ...
    resul1.SetLinearForm(ray2 / norm2 * (grosterme * ndotns2 - nplan.Dot(temp)), nplan, -ray2 * grosterme / norm2, ns2,
                         ray2 / norm2, temp);
    ...
    grosterme = ncrossns2.Dot(nplan.Crossed(temp)) / norm2 / norm2;
    resul2.SetLinearForm(ray2 / norm2 * (grosterme * ndotns2 - nplan.Dot(temp)), nplan, -ray2 * grosterme / norm2, ns2,
                         ray2 / norm2, temp);
