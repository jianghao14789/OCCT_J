...
// purpose  :
//=======================================================================
BlendFunc_CSConstRad::BlendFunc_CSConstRad(const Handle(Adaptor3d_Surface) & S, const Handle(Adaptor3d_Curve) & C,
                                           const Handle(Adaptor3d_Curve) & CG)
    :

      surf(S), curv(C), guide(CG), prmc(0.0), istangent(Standard_True), ray(0.0), choix(0), normtg(0.0), theD(0.0),
...
Standard_Boolean BlendFunc_CSConstRad::IsSolution(const math_Vector& Sol, const Standard_Real Tol) {
    ...
    if (Abs(valsol(1)) <= Tol && Abs(valsol(2)) <= Tol && Abs(valsol(3)) <= Tol * Tol) {
        ...
        temp.SetLinearForm(ray / norm * (dnplan.Dot(ns) - grosterme * ndotns), nplan, ray * ndotns / norm, dnplan,
...
Standard_Boolean BlendFunc_CSConstRad::Derivatives(const math_Vector& X, math_Matrix& D) {
    ...
    resul.SetLinearForm(-ray / norm * (grosterme * ndotns - nplan.Dot(temp)), nplan, ray * grosterme / norm, ns,
                        -ray / norm, temp, d1u1);
...
const gp_Vec& BlendFunc_CSConstRad::TangentOnS() const {
    if (istangent) throw Standard_DomainError("BlendFunc_CSConstRad::TangentOnS");
    return tgs;
}
...
const gp_Vec& BlendFunc_CSConstRad::TangentOnC() const {
    if (istangent) throw Standard_DomainError("BlendFunc_CSConstRad::TangentOnC");
    return tgc;
}
...
const gp_Vec2d& BlendFunc_CSConstRad::Tangent2d() const {
    if (istangent) throw Standard_DomainError("BlendFunc_CSConstRad::Tangent2d");
    return tg2d;
}
...
void BlendFunc_CSConstRad::Tangent(const Standard_Real U, const Standard_Real V, gp_Vec& TgS, gp_Vec& NmS) const {
    gp_Pnt bid;
    gp_Vec d1u, d1v;
...
void BlendFunc_CSConstRad::Section(const Standard_Real Param, const Standard_Real U, const Standard_Real V,
                                   const Standard_Real W, Standard_Real& Pdeb, Standard_Real& Pfin, gp_Circ& C) {
    gp_Vec d1u1, d1v1;
    gp_Vec ns;
    Standard_Real norm;
    gp_Pnt Center;
...
Standard_Boolean BlendFunc_CSConstRad::Section(const Blend_Point&, TColgp_Array1OfPnt&, TColgp_Array1OfVec&,
                                               TColgp_Array1OfVec&, TColgp_Array1OfPnt2d&, TColgp_Array1OfVec2d&,
                                               TColgp_Array1OfVec2d&, TColStd_Array1OfReal&, TColStd_Array1OfReal&,
...
Standard_Boolean BlendFunc_CSConstRad::GetSection(const Standard_Real Param, const Standard_Real U,
    ...
    grosterme = ncrossns.Dot(dnplan.Crossed(ns)) / norm / norm;
    dnw.SetLinearForm((dnplan.Dot(ns) - grosterme * ndotns) / norm, nplan, ndotns / norm, dnplan, grosterme / norm, ns);
    ...
    if (Resol.IsDone()) {
        ...
        temp = d2u1.Crossed(d1v1).Added(d1u1.Crossed(d2uv1));
        grosterme = ncrossns.Dot(nplan.Crossed(temp)) / norm / norm;
        resulu.SetLinearForm(-(grosterme * ndotns - nplan.Dot(temp)) / norm, nplan, grosterme / norm, ns, -1. / norm,
                             temp);

        // Derivee de n1 par rapport a v1
        temp = d2uv1.Crossed(d1v1).Added(d1u1.Crossed(d2v1));
        grosterme = ncrossns.Dot(nplan.Crossed(temp)) / norm / norm;
        resulv.SetLinearForm(-(grosterme * ndotns - nplan.Dot(temp)) / norm, nplan, grosterme / norm, ns, -1. / norm,
                             temp);
...
void BlendFunc_CSConstRad::GetShape(Standard_Integer& NbPoles, Standard_Integer& NbKnots, Standard_Integer& Degree,
...
//=======================================================================
void BlendFunc_CSConstRad::GetTolerance(const Standard_Real BoundTol, const Standard_Real SurfTol,
...
void BlendFunc_CSConstRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles, TColgp_Array1OfPnt2d& Poles2d,
...
Standard_Boolean BlendFunc_CSConstRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles,
    ...
    if (!istgt) {
        return GeomFill::GetCircle(myTConv, ns, ns2, dnw, dn2w, nplan, dnplan, pts, ptc, tgs, tgc, Abs(ray), 0, Center,
                                   tgct, Poles, DPoles, Weights, DWeights);
    } else {
        GeomFill::GetCircle(myTConv, ns, ns2, nplan, pts, ptc, Abs(ray), Center, Poles, Weights);
        return Standard_False;
    }
...
void BlendFunc_CSConstRad::Resolution(const Standard_Integer, const Standard_Real Tol, Standard_Real& TolU,
