...
void StdSelect_BRepSelectionTool::PreBuildBVH(const Handle(SelectMgr_Selection) & theSelection) {
    for (NCollection_Vector<Handle(SelectMgr_SensitiveEntity)>::Iterator aSelEntIter(theSelection->Entities());
        ...
        for (Select3D_IndexedMapOfEntity::Iterator aSubEntitiesIter(aGroup->Entities()); aSubEntitiesIter.More();
...
// Purpose :
//==================================================
void StdSelect_BRepSelectionTool::Load(const Handle(SelectMgr_Selection) & theSelection, const TopoDS_Shape& theShape,
                                       const TopAbs_ShapeEnum theType, const Standard_Real theDeflection,
                                       const Standard_Real theDeviationAngle,
                                       const Standard_Boolean isAutoTriangulation, const Standard_Integer thePriority,
                                       const Standard_Integer theNbPOnEdge, const Standard_Real theMaxParam) {
    ...
    switch (theType) {
        ...
        case TopAbs_COMPSOLID: {
            ...
            for (Standard_Integer aShIndex = 1; aShIndex <= aSubShapes.Extent(); ++aShIndex) {
                ...
                ComputeSensitive(aSubShape, aBrepOwner, theSelection, theDeflection, theDeviationAngle, theNbPOnEdge,
                                 theMaxParam, isAutoTriangulation);
            }
            break;
        }
        default: {
            aBrepOwner = new StdSelect_BRepOwner(theShape, aPriority);
            ComputeSensitive(theShape, aBrepOwner, theSelection, theDeflection, theDeviationAngle, theNbPOnEdge,
                             theMaxParam, isAutoTriangulation);
        }
    }
}
...
void StdSelect_BRepSelectionTool::Load(const Handle(SelectMgr_Selection) & theSelection,
                                       ...
                                       const TopoDS_Shape& theShape, const TopAbs_ShapeEnum theType,
                                       const Standard_Real theDeflection, const Standard_Real theDeviationAngle,
                                       const Standard_Boolean isAutoTriangulation, const Standard_Integer thePriority,
                                       const Standard_Integer theNbPOnEdge, const Standard_Real theMaxParam) {
    Load(theSelection, theShape, theType, theDeflection, theDeviationAngle, isAutoTriangulation, thePriority,
         theNbPOnEdge, theMaxParam);
...
void StdSelect_BRepSelectionTool::ComputeSensitive(const TopoDS_Shape& theShape,
    ...
    switch (theShape.ShapeType()) {
        case TopAbs_VERTEX: {
            theSelection->Add(new Select3D_SensitivePoint(theOwner, BRep_Tool::Pnt(TopoDS::Vertex(theShape))));
            break;
        }
        case TopAbs_EDGE: {
            Handle(Select3D_SensitiveEntity) aSensitive;
            GetEdgeSensitive(theShape, theOwner, theSelection, theDeflection, theDeviationAngle, theNbPOnEdge,
                             theMaxParam, aSensitive);
        ...
        case TopAbs_WIRE: {
            ...
            while (aWireExp.More()) {
                GetEdgeSensitive(aWireExp.Current(), theOwner, theSelection, theDeflection, theDeviationAngle,
                                 theNbPOnEdge, theMaxParam, aSensitive);
                if (!aSensitive.IsNull()) {
        ...
        case TopAbs_FACE: {
            ...
            GetSensitiveForFace(aFace, theOwner, aSensitiveList, isAutoTriangulation, theNbPOnEdge, theMaxParam);
            for (Select3D_EntitySequenceIter aSensIter(aSensitiveList); aSensIter.More(); aSensIter.Next()) {
        ...
        case TopAbs_COMPSOLID: {
            ...
            {
                const TopoDS_Face* aFaces[2] = {&TopoDS::Face(aSubfacesMap.FindKey(1)),
            ...
            {
                const TopoDS_Face* aFaces[3] = {&TopoDS::Face(aSubfacesMap.FindKey(1)),
                                                &TopoDS::Face(aSubfacesMap.FindKey(2)),
                ...
                if (!aGeomCone.IsNull()) {
                    ...
                    if (!aGeomPlanes[0].IsNull() && !aGeomPlanes[1].IsNull() &&
                        aGeomPlanes[0]->Position().Direction().IsEqual(aGeomCyl->Position().Direction(),
                                                                       Precision::Angular()) &&
                        aGeomPlanes[1]->Position().Direction().IsEqual(aGeomCyl->Position().Direction(),
                                                                       Precision::Angular())) {
            ...
            for (Standard_Integer aShIndex = 1; aShIndex <= aSubfacesMap.Extent(); ++aShIndex) {
                ComputeSensitive(aSubfacesMap.FindKey(aShIndex), theOwner, theSelection, theDeflection,
                                 theDeviationAngle, theNbPOnEdge, theMaxParam, isAutoTriangulation);
            }
            break;
        }
        ...
        default: {
            ...
            for (anExp.Init(theShape, TopAbs_VERTEX, TopAbs_EDGE); anExp.More(); anExp.Next()) {
                ComputeSensitive(anExp.Current(), theOwner, theSelection, theDeflection, theDeviationAngle,
                                 theNbPOnEdge, theMaxParam, isAutoTriangulation);
            }
            // sub-edges
            ...
            TopExp::MapShapes(theShape, TopAbs_FACE, aSubfacesMap);
            for (Standard_Integer aShIndex = 1; aShIndex <= aSubfacesMap.Extent(); ++aShIndex) {
                ComputeSensitive(aSubfacesMap(aShIndex), theOwner, theSelection, theDeflection, theDeviationAngle,
                                 theNbPOnEdge, theMaxParam, isAutoTriangulation);
            }
        }
    }
...
// Purpose :
//==================================================
static Standard_Boolean FindLimits(const Adaptor3d_Curve& theCurve, const Standard_Real theLimit,
...
void StdSelect_BRepSelectionTool::GetEdgeSensitive(const TopoDS_Shape& theShape,
    ...
    switch (cu3d.GetType()) {
        case GeomAbs_Line: {
            ...
            theSensitive = new Select3D_SensitiveSegment(theOwner, cu3d.Value(aParamFirst), cu3d.Value(aParamLast));
            break;
        }
        case GeomAbs_Circle: {
            ...
            if (aCircle.Radius() <= Precision::Confusion()) {
                ...
                theSensitive =
                    new Select3D_SensitiveCircle(theOwner, aCircle, aParamFirst, aParamLast, Standard_False, 16);
            }
            break;
        }
        default: {
...
Standard_Boolean StdSelect_BRepSelectionTool::GetSensitiveForFace(
    const TopoDS_Face& theFace, const Handle(SelectMgr_EntityOwner) & theOwner,
    Select3D_EntitySequence& theSensitiveList, const Standard_Boolean /*theAutoTriangulation*/,
    const Standard_Integer NbPOnEdge, const Standard_Real theMaxParam, const Standard_Boolean theInteriorFlag) {
        ...
        if (Handle(Geom_SphericalSurface) aGeomSphere = Handle(Geom_SphericalSurface)::DownCast(aSurf)) {
            ...
            if (isFullSphere) {
                ...
                Handle(Select3D_SensitiveSphere) aSensSphere =
                    new Select3D_SensitiveSphere(theOwner, aSphere.Position().Axis().Location(), aSphere.Radius());
    ...
    if (BS.GetType() == GeomAbs_Plane) {
        ...
        const bool isInfinite =
            aFirstU == -theMaxParam && aLastU == theMaxParam && aFirstV == -theMaxParam && aLastV == theMaxParam;
        theSensitiveList.Append(new Select3D_SensitiveFace(
            theOwner, aPlanePnts, theInteriorFlag && !isInfinite ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY));
        return Standard_True;
    }
                        ...
                        BS.GetType() == GeomAbs_BSplineSurface) // beuurkk pour l'instant...
                    {
                        ...
                        if (cu3d.Circle().Radius() <= Precision::Confusion()) {
                            ...
                            theSensitiveList.Append(
                                new Select3D_SensitiveCircle(theOwner, cu3d.Circle(), theInteriorFlag, 16));
                        }
                    }
                } else {
                    Standard_Real ff = wf, ll = wl;
    ...
    if (aFacePoints->Array1().Length() == 2) {
        ...
        theSensitiveList.Append(new Select3D_SensitiveFace(
            theOwner, aFacePoints, theInteriorFlag ? Select3D_TOS_INTERIOR : Select3D_TOS_BOUNDARY));
    }
    return Standard_True;
}
