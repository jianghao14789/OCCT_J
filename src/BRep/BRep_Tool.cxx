...
Standard_Boolean BRep_Tool::IsGeometric(const TopoDS_Edge& E) {
    ...
    while (itcr.More()) {
        ...
        if (cr->IsCurve3D()) {
            ...
            if (!GC.IsNull() && !GC->Curve3D().IsNull()) return Standard_True;
...
Handle(Geom2d_Curve) BRep_Tool::CurveOnSurface(const TopoDS_Edge& E, const Handle(Geom_Surface) & S,
    ...
    if (theIsStored) *theIsStored = Standard_True;
    ...
    if (theIsStored) *theIsStored = Standard_False;
...
Standard_Boolean BRep_Tool::IsClosed(const TopoDS_Edge& E, const Handle(Geom_Surface) & S, const TopLoc_Location& L) {
    ...
    while (itcr.More()) {
        const Handle(BRep_CurveRepresentation) & cr = itcr.Value();
        if (cr->IsCurveOnSurface(S, l) && cr->IsCurveOnClosedSurface()) return Standard_True;
        itcr.Next();
    }
    return Standard_False;
}
...
Standard_Boolean BRep_Tool::IsClosed(const TopoDS_Edge& E, const Handle(Poly_Triangulation) & T,
    ...
    while (itcr.More()) {
        const Handle(BRep_CurveRepresentation) & cr = itcr.Value();
        if (cr->IsPolygonOnTriangulation(T, l) && cr->IsPolygonOnClosedTriangulation()) return Standard_True;
        itcr.Next();
    }
    return Standard_False;
}
...
Standard_Boolean BRep_Tool::HasContinuity(const TopoDS_Edge& E, const Handle(Geom_Surface) & S1,
    ...
    while (itcr.More()) {
        const Handle(BRep_CurveRepresentation) & cr = itcr.Value();
        if (cr->IsRegularity(S1, S2, l1, l2)) return Standard_True;
        itcr.Next();
    }
    return Standard_False;
}
...
GeomAbs_Shape BRep_Tool::Continuity(const TopoDS_Edge& E, const Handle(Geom_Surface) & S1,
    ...
    while (itcr.More()) {
        const Handle(BRep_CurveRepresentation) & cr = itcr.Value();
        if (cr->IsRegularity(S1, S2, l1, l2)) return cr->Continuity();
        itcr.Next();
    }
    return GeomAbs_C0;
}
...
Standard_Boolean BRep_Tool::HasContinuity(const TopoDS_Edge& E) {
    ...
    for (; itcr.More(); itcr.Next()) {
        const Handle(BRep_CurveRepresentation) & CR = itcr.Value();
        if (CR->IsRegularity()) return Standard_True;
    }
    return Standard_False;
}
...
Standard_Real BRep_Tool::Parameter(const TopoDS_Vertex& V, const TopoDS_Edge& E, const Handle(Geom_Surface) & S,
    ...
    while (itv.More()) {
        if (V.IsSame(itv.Value())) {
            if (VF.IsNull())
                ...
                if (itv.Value().Orientation() == V.Orientation()) VF = itv.Value();
            }
        }
        itv.Next();
    }
    ...
    if (orient == TopAbs_FORWARD) {
        ...
        while (itpr.More()) {
            if (itpr.Value()->IsPointOnCurveOnSurface(PC, S, L)) return itpr.Value()->Parameter();
            itpr.Next();
        }
    }
...
Standard_Boolean BRep_Tool::IsClosed(const TopoDS_Shape& theShape) {
    if (theShape.ShapeType() == TopAbs_SHELL) {
        ...
        for (; exp.More(); exp.Next()) {
            ...
            if (V.Orientation() == TopAbs_INTERNAL || V.Orientation() == TopAbs_EXTERNAL) continue;
            hasBound = Standard_True;
            if (!aMap.Add(V)) aMap.Remove(V);
        }
        return hasBound && aMap.IsEmpty();
