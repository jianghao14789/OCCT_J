...
public:
    ...
    Standard_EXPORT const Handle(Poly_Triangulation) &
        Triangulation(const Poly_MeshPurpose thePurpose = Poly_MeshPurpose_NONE) const;
    ...
    //! If theTriangulation is NULL internal list of triangulations will be cleared and active triangulation will be
    //! nullified. If theToReset is TRUE internal list of triangulations will be reset to new list with only one input
    //! triangulation that will be active. Else if input triangulation is contained in internal triangulations list it
    //! will be made active,
