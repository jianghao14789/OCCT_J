...
template <typename Base> parser::basic_symbol<Base>::basic_symbol(typename Base::kind_type t) : Base(t), value() {}
...
parser::by_kind::by_kind(by_kind&& that) : kind_(that.kind_) {
    that.clear();
}
#endif
...
const signed char parser::yypact_[] = {
    26,  5,   -24, -24, -24, 35,  29,  -24, -24, 41,  -24, 43,  -24, 36,  -24, 45,  41,  -24, -24, 3,   38,
    -24, -24, 40,  -24, 32,  45,  -24, -24, -24, -24, -24, -24, 36,  -24, -24, 9,   -24, 60,  56,  -24, -3,
    51,  -24, 17,  -24, -24, -24, 53,  44,  6,   36,  59,  -24, 0,   36,  -24, 42,  6,   2,   -24, 47,  -24,
    -24, 36,  -24, -24, 55,  2,   49,  -24, 52,  -24, -24, -24, -14, 50,  -24, -24, 55,  -24, -24, -24};

const signed char parser::yydefact_[] = {
    0,  0,  9,  10, 11, 0, 0,  1,  15, 0,  47, 0, 12, 0, 16, 0,  0,  13, 23, 0,  0, 34, 46, 0, 29, 0,  0, 21,
    22, 17, 18, 24, 27, 0, 25, 19, 0,  14, 34, 0, 30, 0, 0,  20, 0,  26, 7,  39, 0, 0,  0,  0, 0,  28, 0, 0,
    31, 44, 0,  0,  37, 4, 6,  38, 0,  35, 43, 0, 0,  0, 2,  5,  36, 40, 41, 0,  0, 33, 3,  0, 45, 32, 42};

const signed char parser::yypgoto_[] = {-24, -24, -24, -24, -24, -24, -24, -24, 62,  58,  31, -24, -24, 46,
                                        -13, -24, -23, -21, -24, -6,  -24, -2,  -24, -24, 18, -24, -5};
...
const signed char parser::yytable_[] = {
    20, 13, 40, 42, 27, 47, 13, 21, 10, 6,  79, 10, 80, 10, 28, 57, 22, 29, 27, 48, 43, 40, 30, 63, 48, 18, 31, 58,
    28, 1,  8,  29, 31, 44, 9,  7,  30, 40, 60, 18, 10, 38, 65, 55, 8,  39, 21, 14, 16, 64, 22, 72, 21, 69, 10, 22,
    52, 41, 18, 37, -8, 22, 76, 46, 10, 56, 61, 70, 66, 73, 77, 81, 78, 17, 26, 53, 68, 82, 0,  0,  0,  0,  45};

const signed char parser::yycheck_[] = {
    13, 6,  23, 26, 1,  8,  11, 1, 11, 4,  24, 11, 26, 11, 11, 9,  10, 14, 1,  22, 33, 42, 19, 23, 22, 22, 23, 50,
    11, 3,  1,  14, 23, 24, 5,  0, 19, 58, 51, 22, 11, 1,  55, 48, 1,  5,  1,  6,  5,  54, 10, 64, 1,  59, 11, 10,
    5,  25, 22, 21, 0,  10, 68, 7, 11, 21, 7,  20, 26, 14, 21, 21, 20, 11, 16, 44, 58, 79, -1, -1, -1, -1, 36};

const signed char parser::yystos_[] = {
    0,  3,  30, 31, 32, 33, 4,  0,  1,  5,  11, 34, 35, 53, 6,  36, 5,  35, 22, 39, 41, 1,  10, 43, 44, 52, 36, 1,
    11, 14, 19, 23, 37, 38, 40, 41, 42, 21, 1,  5,  44, 25, 43, 41, 24, 40, 7,  8,  22, 46, 47, 53, 5,  37, 45, 53,
    21, 9,  43, 51, 41, 7,  29, 23, 53, 41, 26, 50, 51, 46, 20, 28, 41, 14, 48, 49, 46, 21, 20, 24, 26, 21, 48};

const signed char parser::yyr1_[] = {0,  27, 28, 28, 29, 29, 30, 31, 32, 33, 33, 33, 34, 34, 35, 35,
                                     36, 37, 37, 37, 37, 37, 38, 39, 40, 41, 41, 42, 42, 43, 43, 44,
                                     44, 44, 44, 45, 45, 46, 46, 47, 48, 49, 49, 50, 51, 51, 52, 53};

const signed char parser::yyr2_[] = {0, 2, 1, 2, 1, 2, 8, 7, 6, 1, 1, 1, 1, 2, 3, 1, 1, 1, 1, 1, 2, 1, 1, 1,
                                     1, 2, 3, 1, 3, 1, 2, 4, 7, 6, 1, 2, 3, 2, 3, 1, 1, 1, 3, 1, 1, 4, 1, 1};
...
#if YYDEBUG
const unsigned char parser::yyrline_[] = {0,   98,  98,  99,  100, 101, 102, 103, 104, 105, 105, 105,
                                          108, 109, 111, 112, 114, 117, 118, 119, 120, 121, 124, 127,
                                          130, 135, 136, 138, 139, 141, 142, 144, 145, 146, 147, 149,
                                          150, 152, 153, 155, 158, 161, 162, 164, 167, 169, 174, 177};
...
parser::symbol_kind_type parser::yytranslate_(int t) {
    ...
    static const signed char translate_table[] = {
        ...
        2, 20, 2, 2, 2, 2, 2, 2, 2, 22, 23, 2, 2, 24, 2, 2, 26, 2,  2,  2,  2,  2,  2,  2,  2,  2,  2, 2, 21, 2, 25,
...
void step::parser::error(const std::string& m) {
    ...
    if (isSyntax && m.length() > 13)
        Sprintf(newmess, "Undefined Parsing: Line %d: %s: %s", scanner->lineno() + 1, "Incorrect syntax",
                m.c_str() + 14);
    else if (isSyntax)
