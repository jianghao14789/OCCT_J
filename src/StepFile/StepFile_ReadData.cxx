...
public:
    ...
    ErrorsPage* NextErrorPage() const {
        return myNext;
    }

    //! Sets the next ErrorPage
    void SetNextErrorPage(ErrorsPage* theNextErrorsPage) {
        myNext = theNextErrorsPage;
    }

    //! Returns an error message
    Standard_CString ErrorMessage() const {
        return myError.ToCString();
    }
...
StepFile_ReadData::StepFile_ReadData()
    : myMaxChar(50000), myMaxRec(5000), myMaxArg(10000), myModePrint(0), myNbRec(0), myNbHead(0), myNbPar(0),
      myYaRec(0), myNumSub(0), myErrorArg(Standard_False), myResText(NULL), myCurrType(TextValue::SubList),
      mySubArg(NULL), myTypeArg(Interface_ParamSub), myCurrArg(NULL), myFirstRec(NULL), myCurRec(NULL), myLastRec(NULL),
      myCurScope(NULL), myFirstError(NULL), myCurError(NULL) {
...
void StepFile_ReadData::CreateNewArg() {
    ...
    if (myTypeArg == Interface_ParamMisc) myErrorArg = Standard_True;
...
Standard_Boolean StepFile_ReadData::GetArgDescription(Interface_ParamType* theType, char** theValue) {
    if (myCurrArg == NULL) return Standard_False;
...
void StepFile_ReadData::GetFileNbR(Standard_Integer* theNbHead, Standard_Integer* theNbRec,
...
Standard_Boolean StepFile_ReadData::GetRecordDescription(char** theIdent, char** theType, int* theNbArg) {
    if (myCurRec == NULL) return Standard_False;
...
void StepFile_ReadData::PrintRecord(Record* theRecord) {
    ...
    while (myCurrArg != NULL) {
        ...
        Printf("  - Arg.%d[%c%c] : %s", aNumArg, TextValue::ArgType1[myCurrArg->myType],
