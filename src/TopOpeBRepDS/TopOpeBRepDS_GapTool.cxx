...
void TopOpeBRepDS_GapTool::Init(const Handle(TopOpeBRepDS_HDataStructure) & HDS) {
    ...
    for (i = 1; i <= NbCurves; i++) {
        ...
        for (TopOpeBRepDS_ListIteratorOfListOfInterference it(LI); it.More(); it.Next()) {
            if (it.Value()->GeometryType() == TopOpeBRepDS_POINT) StoreGToI(myGToI, it.Value());
        }
    }
}
...
Standard_Boolean TopOpeBRepDS_GapTool::Curve(const Handle(TopOpeBRepDS_Interference) & I, TopOpeBRepDS_Curve& C) const {
...
const TopOpeBRepDS_ListOfInterference& TopOpeBRepDS_GapTool::SameInterferences(const Handle(TopOpeBRepDS_Interference) &
                                                                               I) const {
...
TopOpeBRepDS_ListOfInterference& TopOpeBRepDS_GapTool::ChangeSameInterferences(const Handle(TopOpeBRepDS_Interference) &
                                                                               I) {
...
Standard_Boolean TopOpeBRepDS_GapTool::EdgeSupport(const Handle(TopOpeBRepDS_Interference) & I, TopoDS_Shape& E) const {
    if (I->GeometryType() == TopOpeBRepDS_POINT) {
...
Standard_Boolean TopOpeBRepDS_GapTool::FacesSupport(const Handle(TopOpeBRepDS_Interference) & I, TopoDS_Shape& F1,
...
Standard_Boolean TopOpeBRepDS_GapTool::ParameterOnEdge(const Handle(TopOpeBRepDS_Interference) & I,
                                                       const TopoDS_Shape& E, Standard_Real& U) const {
    if (I->GeometryType() == TopOpeBRepDS_POINT) {
        if (myInterToShape.IsBound(I)) {
            const TopoDS_Shape& S = myInterToShape(I);
            if (S.IsSame(E)) {
...
void TopOpeBRepDS_GapTool::SetParameterOnEdge(const Handle(TopOpeBRepDS_Interference) & I, const TopoDS_Shape& E,
...
void TopOpeBRepDS_GapTool::SetPoint(const Handle(TopOpeBRepDS_Interference) & I, const Standard_Integer IP) {
    if (IP != I->Geometry()) {
        const TopOpeBRepDS_ListOfInterference& LI = myGToI(I->Geometry());
        for (TopOpeBRepDS_ListIteratorOfListOfInterference it(LI); it.More(); it.Next()) {
