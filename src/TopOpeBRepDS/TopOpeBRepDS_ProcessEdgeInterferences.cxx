...
static Standard_Boolean FUN_keepEinterference
    ...
    if (I->IsKind(STANDARD_TYPE(TopOpeBRepDS_EdgeVertexInterference))) {
        ...
        Standard_Boolean k2 =
            !(((shab == TopAbs_EDGE) && (stab == TopAbs_ON)) || ((shaa == TopAbs_EDGE) && (staa == TopAbs_ON)));
        res = res && k2;
        ...
        */
        ...
        Handle(TopOpeBRepDS_CurvePointInterference) aCPI = Handle(TopOpeBRepDS_CurvePointInterference)::DownCast(I);
        ...
        if (Abs(par - f) < eps || Abs(par - l) < eps) res = Standard_False;
    }
...
//------------------------------------------------------
(TopOpeBRepDS_ListOfInterference& LF,const TopOpeBRepDS_ListOfInterference& LE,const TopOpeBRepDS_DataStructure&
BDS,const Standard_Integer SIX)
{
  const TopoDS_Shape& E = BDS.Shape(SIX);
...
Standard_EXPORT void FUN_resolveEUNKNOWN
    ...
    for (it1.Initialize(LI); it1.More(); it1.Next()) {
        ...
        Standard_Boolean idt =
            (tsb1 == TopAbs_FACE && tsa1 == TopAbs_FACE && GT1 == TopOpeBRepDS_POINT && ST1 == TopOpeBRepDS_FACE);
...
// ----------------------------------------------------------------------
Standard_EXPORT void FUN_purgeDSonSE(const Handle(TopOpeBRepDS_HDataStructure) & HDS, const Standard_Integer EIX,
