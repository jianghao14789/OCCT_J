...
static Standard_Boolean CheckInterferenceIsValid(const Handle(TopOpeBRepDS_Interference) & I,
                                                 const TopoDS_Edge& theEdge, const TopoDS_Edge& theSupportEdge,
...
static void FDS_reduceONFACEinterferences(TopOpeBRepDS_ListOfInterference& LI,
                                          const TopOpeBRepDS_DataStructure& /*BDS*/, const Standard_Integer)
//------------------------------------------------------
{

    TopOpeBRepDS_ListIteratorOfListOfInterference
        it1; // set hasONFACE = True if LI contains interfs with (ON,FACE) transition(s).
...
// <LI> -> <reducedLI> + <LI>
{
    ...
    while (it1.More()) {
        ...
        if (isComplex) {
            ...
            if (isevi) {
                Handle(TopOpeBRepDS_EdgeVertexInterference)
                    EVI(Handle(TopOpeBRepDS_EdgeVertexInterference)::DownCast(I1));
            ...
            if (iscpi) {
                Handle(TopOpeBRepDS_CurvePointInterference)
                    CPI(Handle(TopOpeBRepDS_CurvePointInterference)::DownCast(I1));
                newI = new TopOpeBRepDS_CurvePointInterference(T, TopOpeBRepDS_FACE, IB1, TopOpeBRepDS_POINT, G1,
                                                               CPI->Parameter());
            }
    ...
    for (tki.Init(); tki.More(); tki.Next()) {
        ...
        while (it1.More()) {
            ...
            if (complex1d) {
                ...
                Handle(TopOpeBRepDS_Interference) newI = new TopOpeBRepDS_EdgeVertexInterference(
                    T, TopOpeBRepDS_EDGE, IB1, G, EVI->GBound(), Conf, EVI->Parameter());
                reducedLI.Append(newI);
                it1.Next();
            } else {
                newLI.Append(I1);
...
//------------------------------------------------------
static void FUN_unkeepEVIonGb1(const TopOpeBRepDS_DataStructure& BDS, const Standard_Integer EIX,
...
static void FUN_keepl3dF(const Standard_Integer, const Handle(TopOpeBRepDS_HDataStructure) &,
                         const TopOpeBRepDS_ListOfInterference& l3dF, const TopOpeBRepDS_ListOfInterference& LR3dFE,
...
// purpose  :
//=======================================================================
TopOpeBRepDS_EIR::TopOpeBRepDS_EIR(const Handle(TopOpeBRepDS_HDataStructure) & HDS) : myHDS(HDS) {}
...
static void FUN_ProcessEdgeInterferences(const Standard_Integer EIX, const TopOpeBRepDS_Kind, const Standard_Integer,
