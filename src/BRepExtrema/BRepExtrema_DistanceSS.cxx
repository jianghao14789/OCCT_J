...
static void TRIM_INFINIT_EDGE(const TopoDS_Edge& S1, const TopoDS_Edge& S2, TopoDS_Edge& aResEdge,
    ...
    if (BRep_Tool::Degenerated(S1) || BRep_Tool::Degenerated(S2)) return;
    ...
    if (bIsTrim1 || bIsTrim2) {
        Bnd_Box aEdgeBox;
        if (bIsTrim1) BRepBndLib::Add(S2, aEdgeBox);
        if (bIsTrim2) BRepBndLib::Add(S1, aEdgeBox);
        ...
        if (!bUmin) Umin = arrU[0];

        if (!bUmax) Umax = arrU[0];
...
static void TRIM_INFINIT_FACE(const TopoDS_Shape& S1, const TopoDS_Shape& S2, TopoDS_Face& aResFace,
    ...
    if (Type1 == TopAbs_EDGE && Type2 == TopAbs_FACE) {
        aE = TopoDS::Edge(S1);
        if (BRep_Tool::Degenerated(aE)) return;
        aF = TopoDS::Face(S2);
    ...
    if (bIsTrim) {
        ...
        aProj.Perform(aPnt1);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[1], arrV[1]);
        aProj.Perform(aPnt2);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[2], arrV[2]);
        aProj.Perform(aPnt3);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[3], arrV[3]);
        aProj.Perform(aPnt4);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[4], arrV[4]);
        aProj.Perform(aPnt5);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[5], arrV[5]);
        aProj.Perform(aPnt6);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[6], arrV[6]);
        aProj.Perform(aPnt7);
        if (aProj.IsDone()) aProj.LowerDistanceParameters(arrU[7], arrV[7]);

        if (!bUmin) Umin = arrU[0];
        if (!bUmax) Umax = arrU[0];
        if (!bVmin) Vmin = arrV[0];
        if (!bVmax) Vmax = arrV[0];
            ...
            for (i = 1; i <= arrInter.Length(); i++) {
                            ...
                            if (fabs(Dstmin - sqrt(Ext.SquareDistance(ii))) < Eps) {
                                ...
                                if (TRI_SOLUTION(SeqSol2, Pt)) {
                                    ...
                                    if ((fabs(t - Udeb) >= epsP) && (fabs(t - Ufin) > epsP)) {
                                        if (mDstRef > Dstmin) mDstRef = Dstmin;
                                        const BRepExtrema_SolutionElem Sol1(Dstmin, aPnt, BRepExtrema_IsOnEdge, E,
                                                                            aParameter);
                                        const BRepExtrema_SolutionElem Sol2(Dstmin, Pt, BRepExtrema_IsOnEdge, Eother,
                                                                            t);
                ...
                for (Standard_Integer i2 = 1; i2 <= arrInterOther.Length(); i2++) {
                    ...
                    if ((Dst < DstRef - Eps) || (fabs(Dst - DstRef) < Eps)) {
                        if (mDstRef > Dst) mDstRef = Dst;
                        const BRepExtrema_SolutionElem Sol1(Dst, aPnt, BRepExtrema_IsOnEdge, E, aParameter);
                        const BRepExtrema_SolutionElem Sol2(Dst, aPntOther, BRepExtrema_IsOnEdge, Eother,
                                                            aParameterOther);
                        SeqSol1.Append(iE == 0 ? Sol1 : Sol2);
                        SeqSol2.Append(iE == 0 ? Sol2 : Sol1);
                    }
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Shape& S1, const TopoDS_Shape& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    SeqSolShape1.Clear();
    SeqSolShape2.Clear();
        ...
        case TopAbs_EDGE: {
            ...
            switch (S2.ShapeType()) {
                ...
                case TopAbs_EDGE: {
                    ...
                    if (bIsTrim1) E1 = aTrimEdge;
                    if (bIsTrim2) E2 = aTrimEdge;
                    Perform(E1, E2, B1, B2);
                    break;
                }
                case TopAbs_FACE: {
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Vertex& S1, const TopoDS_Edge& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    if (BRep_Tool::Degenerated(S2)) return;
                ...
                for (i = 1; i <= NbExtrema; i++) {
                    if (fabs(Dstmin - sqrt(Ext.SquareDistance(i))) < myEps) {
                        ...
                        if (TRI_SOLUTION(SeqSolShape2, Pt)) {
                            ...
                            if ((fabs(t - Udeb) >= epsP) && (fabs(t - Ufin) > epsP)) {
                                if (myDstRef > Dstmin) myDstRef = Dstmin;
                                myModif = Standard_True;
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Edge& S1, const TopoDS_Vertex& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    if (BRep_Tool::Degenerated(S1)) return;
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Vertex& S1, const TopoDS_Face& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    const Standard_Real Dst = B1.Distance(B2);
    if ((Dst < myDstRef - myEps) || (fabs(Dst - myDstRef) < myEps)) {
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Face& S1, const TopoDS_Vertex& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    const Standard_Real Dst = B1.Distance(B2);
    if ((Dst < myDstRef - myEps) || (fabs(Dst - myDstRef) < myEps)) {
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Edge& S1, const TopoDS_Edge& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    if (BRep_Tool::Degenerated(S1) || BRep_Tool::Degenerated(S2)) return;
                ...
                for (i = 1; i <= NbExtrema; i++) {
                    if (fabs(Dstmin - sqrt(Ext.SquareDistance(i))) < myEps) {
                        ...
                        if (TRI_SOLUTION(SeqSolShape1, Pt1) || TRI_SOLUTION(SeqSolShape2, Pt2)) {
                            ...
                            if ((fabs(t1 - Udeb1) >= epsP) && (fabs(t1 - Ufin1) > epsP) && (fabs(t2 - Udeb2) >= epsP) &&
                                (fabs(t2 - Ufin2) > epsP)) {
                                if (myDstRef > Dstmin) myDstRef = Dstmin;
                                myModif = Standard_True;
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Edge& S1, const TopoDS_Face& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    if (BRep_Tool::Degenerated(S1)) return;
                        ...
                        for (ii = 1; ii <= NbExtremaPF; ii++) {
                            if (fabs(Dstmin - sqrt(ExtPF.SquareDistance(ii))) < myEps) {
                                ...
                                if (classifier.State() == TopAbs_IN) {
                                    if (myDstRef > Dstmin) myDstRef = Dstmin;
                                    myModif = Standard_True;
                                    const BRepExtrema_SolutionElem Sol1(Dstmin, aPnt, BRepExtrema_IsOnEdge, S1,
                                                                        aParameter);
                                    const BRepExtrema_SolutionElem Sol2(Dstmin, ExtPF.Point(ii), BRepExtrema_IsInFace,
                                                                        S2, U, V);
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Face& S1, const TopoDS_Edge& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    if (BRep_Tool::Degenerated(S2)) return;
                        ...
                        for (ii = 1; ii <= NbExtremaPF; ii++) {
                            if (fabs(Dstmin - sqrt(ExtPF.SquareDistance(ii))) < myEps) {
                                ...
                                if (classifier.State() == TopAbs_IN) {
                                    if (myDstRef > Dstmin) myDstRef = Dstmin;
                                    myModif = Standard_True;
                                    const BRepExtrema_SolutionElem Sol2(Dstmin, aPnt, BRepExtrema_IsOnEdge, S2,
                                                                        aParameter);
                                    const BRepExtrema_SolutionElem Sol1(Dstmin, ExtPF.Point(ii), BRepExtrema_IsInFace,
                                                                        S1, U, V);
                                    SeqSolution1.Append(Sol1);
                                    SeqSolution2.Append(Sol2);
                                }
...
void BRepExtrema_DistanceSS::Perform(const TopoDS_Face& S1, const TopoDS_Face& S2, const Bnd_Box& B1,
                                     const Bnd_Box& B2) {
    const Standard_Real Dst = B1.Distance(B2);
    if ((Dst < myDstRef - myEps) || (fabs(Dst - myDstRef) < myEps)) {
