...
public:
    ...
    Standard_EXPORT Standard_Boolean MakeCurves(const Standard_Real min, const Standard_Real max,
                                                const Handle(Geom_Curve) & C3D, const Handle(Geom2d_Curve) & PC1,
                                                const Handle(Geom2d_Curve) & PC2, const TopoDS_Shape& S1,
                                                const TopoDS_Shape& S2, Handle(Geom_Curve) & C3DN,
                                                Handle(Geom2d_Curve) & PC1N, Handle(Geom2d_Curve) & PC2N,
    ...
    Standard_EXPORT static Handle(Geom2d_Curve)
        MakePCurveOnFace(const TopoDS_Shape& S, const Handle(Geom_Curve) & C, Standard_Real& TolReached2d,
                         const Standard_Real first = 0.0, const Standard_Real last = 0.0);
