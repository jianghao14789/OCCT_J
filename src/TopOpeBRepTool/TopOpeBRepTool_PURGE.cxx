...
Standard_EXPORT Standard_Boolean FUN_tool_ClosedW(const TopoDS_Wire& W) {
    ...
    while (nmap <= 2) {
        ...
        for (; itvonlyFRine.More(); itvonlyFRine.Next()) {
            ...
            if (!ovFine) return Standard_False;
            Standard_Boolean ovRine = mapvRine.IsBound(ov);
            if (!ovRine) return Standard_False;
...
Standard_Boolean TopOpeBRepTool::PurgeClosingEdges(const TopoDS_Face& Fin, const TopoDS_Face& FF,
    ...
    Standard_Real xmin = inU ? (CORRISO.GASref().FirstUParameter()) : (CORRISO.GASref().FirstVParameter());
...
// <Fref> is x-periodic
{
  ...
  FUN_DetectEdgeswithfaultyUV(Fsp,Fsp,lfyisoe,Standard_False,lffyisoe,tmpisoe); } // end lffyisoe process
  ...
  FUN_DetectEdgeswithfaultyUV(Fsp,Fsp,lfydege,Standard_False,lffydege,tmpdege); } // end lfydege process

    TopTools_ListOfShape eFsp; FUN_tool_shapes(Fsp,TopAbs_EDGE,eFsp);
    TopTools_ListOfShape lffydege; tocorrect =
  FUN_DetectEdgeswithfaultyUV(Fsp,Fsp,eFsp,Standard_False,lffydege,tmpdege); Standard_Boolean done = !tocorrect; return
  done;*/
