...
TopOpeBRepTool_REGUW::TopOpeBRepTool_REGUW(const TopoDS_Face& Fref) : myCORRISO(Fref) {
    myS.Nullify();
...
Standard_Boolean TopOpeBRepTool_REGUW::MapS() {
        ...
        for (Standard_Integer iv = 1; iv <= mymapvEds.Extent(); iv++) {
            ...
            for (Standard_Integer i = 1; i <= 5; i++) {
                ...
                for (; ite.More(); ite.Next()) {
                    ...
                    if (cloE) std::cout << "closed";
                    Standard_Boolean dgE = BRep_Tool::Degenerated(e);
                    if (dgE) std::cout << "degenerated";
    ...
    for (i = 1; i <= mymapvEds.Extent(); i++) {
        ...
        if (multiple)
            if (mymapvmultiple.Add(v)) myListVmultiple.Append(v);
    }
    return Standard_True;
} // MapS
...
Standard_Boolean TopOpeBRepTool_REGUW::NextinBlock() {
    ...
    if (trc)
        std::cout << std::endl
                  << " vcur = v" << FUN_adds(myv) << "   p2d = (" << myp2d.X() << " " << myp2d.Y() << ")" << std::endl;
#endif
...
Standard_Boolean TopOpeBRepTool_REGUW::AddNewConnexity(const TopoDS_Vertex& v, const Standard_Integer OriKey,
                                                       const TopoDS_Edge& e) {
    if (!HasInit()) throw Standard_Failure("TopOpeBRepTool_REGUW : NO INIT");
    Standard_Boolean isb = mymapvEds.Contains(v);
    if (!isb) return Standard_False;
...
Standard_Boolean TopOpeBRepTool_REGUW::RemoveOldConnexity(const TopoDS_Vertex& v, const Standard_Integer OriKey,
                                                          const TopoDS_Edge& e) {
    if (!HasInit()) throw Standard_Failure("TopOpeBRepTool_REGUW : NO INIT");
    Standard_Boolean isb = mymapvEds.Contains(v);
    if (!isb) return Standard_False;
