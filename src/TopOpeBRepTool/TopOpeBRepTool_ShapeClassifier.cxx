...
// purpose  :
//=======================================================================
TopOpeBRepTool_ShapeClassifier::TopOpeBRepTool_ShapeClassifier() : myP3Ddef(Standard_False), myP2Ddef(Standard_False) {}
...
TopAbs_State TopOpeBRepTool_ShapeClassifier::StateShapeShape(const TopoDS_Shape& S, const TopoDS_Shape& SRef,
...
TopAbs_State TopOpeBRepTool_ShapeClassifier::StateShapeShape(const TopoDS_Shape& S, const TopoDS_Shape& AvS,
...
TopAbs_State TopOpeBRepTool_ShapeClassifier::StateShapeShape(const TopoDS_Shape& S, const TopTools_ListOfShape& AvLS,
                                                             const TopoDS_Shape& SRef) {
    ClearCurrent();
    myS = S;
    myAvS.Nullify();
...
TopAbs_State TopOpeBRepTool_ShapeClassifier::StateShapeReference(const TopoDS_Shape& S, const TopoDS_Shape& AvS) {
    myS = S;
    myAvS = AvS;
    myPAvLS = NULL;
    Perform();
...
void TopOpeBRepTool_ShapeClassifier::StateEdgeReference() {
    ...
    if (tR == TopAbs_FACE) {
        ...
        if (mySameDomain) {
            ...
            if (C2D.IsNull()) throw Standard_ProgramError("StateShapeShape : no 2d curve");
            ...
            if (!C3D.IsNull()) P3D = C3D->Value(p);
#endif
            StateP2DReference(P2D);
            return;
        } else { // myEdge/myRef=face en 3d
            C3D = BRep_Tool::Curve(myEdge, f3d, l3d);

            if (C3D.IsNull()) throw Standard_ProgramError("StateShapeShape : no 3d curve");
