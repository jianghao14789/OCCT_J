...
TDataStd_DeltaOnModificationOfIntPackedMap::TDataStd_DeltaOnModificationOfIntPackedMap(
    ...
    if (Label().FindAttribute(OldAtt->ID(), CurrAtt)) {
        ...
        if (aMap1.IsNull()) std::cout << "DeltaOnModificationOfIntPackedMap:: Old Map is Null" << std::endl;
        if (aMap2.IsNull()) std::cout << "DeltaOnModificationOfIntPackedMap:: Current Map is Null" << std::endl;
#endif
...
void TDataStd_DeltaOnModificationOfIntPackedMap::Apply() {
    ...
    if (myDeletion.IsNull() && myAddition.IsNull()) return;

    if (!myDeletion.IsNull()) {
        if (myDeletion->Map().Extent()) IntMap->ChangeMap().Subtract(myDeletion->Map());
    }
    if (!myAddition.IsNull()) {
        if (myAddition->Map().Extent()) IntMap->ChangeMap().Unite(myAddition->Map());
    }
