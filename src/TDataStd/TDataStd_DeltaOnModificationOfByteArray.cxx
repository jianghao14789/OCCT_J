...
TDataStd_DeltaOnModificationOfByteArray::TDataStd_DeltaOnModificationOfByteArray(const Handle(TDataStd_ByteArray) &
                                                                                 OldAtt)
    : TDF_DeltaOnModification(OldAtt), myUp1(0), myUp2(0) {
    ...
    if (Label().FindAttribute(OldAtt->ID(), CurrAtt)) {
        {
            ...
            if (Arr1.IsNull()) std::cout << "DeltaOnModificationOfByteArray:: Old ByteArray is Null" << std::endl;
            if (Arr2.IsNull()) std::cout << "DeltaOnModificationOfByteArray:: Current ByteArray is Null" << std::endl;
#endif
            ...
            if (Arr1 != Arr2) {
                ...
                for (i = Arr1->Lower(); i <= N; i++)
                    if (Arr1->Value(i) != Arr2->Value(i)) aList.Append(i);
                if (aCase == 3) {
                    for (i = N + 1; i <= myUp1; i++)
                        aList.Append(i);
                }
        ...
        OldAtt->RemoveArray();
#ifdef OCCT_DEBUG
        if (OldAtt->InternalArray().IsNull()) std::cout << "BackUp Arr is Nullified" << std::endl;
#endif
    }
}
...
void TDataStd_DeltaOnModificationOfByteArray::Apply() {
    ...
    if (aCase == 1 && (myIndxes.IsNull() || myValues.IsNull())) return;
