...
Handle(TDataStd_TreeNode) TDataStd_TreeNode::Set(const TDF_Label& L, const Standard_GUID& explicitID) {
    Handle(TDataStd_TreeNode) TN;
...
TDataStd_TreeNode::TDataStd_TreeNode() : myFather(NULL), myPrevious(NULL), myNext(NULL), myFirst(NULL), myLast(NULL) {}
...
Standard_Boolean TDataStd_TreeNode::Append(const Handle(TDataStd_TreeNode) & TN) {
    if (!(TN->ID() == myTreeID)) throw Standard_DomainError("TDataStd_TreeNode::Append : uncompatible GUID");
...
Standard_Boolean TDataStd_TreeNode::Prepend(const Handle(TDataStd_TreeNode) & TN) {
    if (!(TN->ID() == myTreeID)) throw Standard_DomainError("TDataStd_TreeNode::Prepend : uncompatible GUID");
...
Standard_Boolean TDataStd_TreeNode::InsertBefore(const Handle(TDataStd_TreeNode) & TN) {
    if (!(TN->ID() == myTreeID)) throw Standard_DomainError("TDataStd_TreeNode::InsertBefore : uncompatible GUID");
...
Standard_Boolean TDataStd_TreeNode::InsertAfter(const Handle(TDataStd_TreeNode) & TN) {
    if (!(TN->ID() == myTreeID)) throw Standard_DomainError("TDataStd_TreeNode::InsertAfter : uncompatible GUID");

    if (HasFather() && !HasNext()) Father()->SetLast(TN);

    TN->SetFather(Father());
    TN->SetPrevious(this);
    TN->SetNext(Next());

    if (HasNext()) Next()->SetPrevious(TN);
...
Standard_Boolean TDataStd_TreeNode::IsDescendant(const Handle(TDataStd_TreeNode) & ofTN) const {
    TDataStd_TreeNode* O = (TDataStd_TreeNode*)this;
    while (O->myFather != NULL) {
        if (O->myFather == ofTN) return Standard_True;
        O = O->myFather;
    }
...
Standard_Boolean TDataStd_TreeNode::IsRoot() const {
    if (myFather == NULL && myPrevious == NULL && myNext == NULL) return Standard_True;
    return Standard_False;
}
...
Handle(TDataStd_TreeNode) TDataStd_TreeNode::Last() {
    if (myLast && !myLast->IsChild(this)) myLast = NULL;

    if (myLast == NULL) return FindLast();
...
void TDataStd_TreeNode::AfterAddition() {
    if (!IsBackuped()) {
        ...
        if (myNext) myNext->SetPrevious(this);
    }
}
...
void TDataStd_TreeNode::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & RT) const {
