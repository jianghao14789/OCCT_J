...
TDataStd_DeltaOnModificationOfIntArray::TDataStd_DeltaOnModificationOfIntArray(const Handle(TDataStd_IntegerArray) &
                                                                               OldAtt)
    ...
    if (Label().FindAttribute(OldAtt->ID(), CurrAtt)) {
        {
            ...
            Arr2 = CurrAtt->Array();
#ifdef OCCT_DEBUG
            if (Arr1.IsNull()) std::cout << "DeltaOnModificationOfIntArray:: Old IntArray is Null" << std::endl;
            if (Arr2.IsNull()) std::cout << "DeltaOnModificationOfIntArray:: Current IntArray is Null" << std::endl;
#endif
        ...
        OldAtt->RemoveArray();
#ifdef OCCT_DEBUG
        if (OldAtt->Array().IsNull()) std::cout << "BackUp Arr is Nullified" << std::endl;
#endif
    }
}
