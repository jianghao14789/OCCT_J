...
//=======================================================================
static Handle(TDataStd_RealArray)
...
TDataStd_RealArray::TDataStd_RealArray() : myIsDelta(Standard_False), myID(GetID()) {}
...
Handle(TDataStd_RealArray) TDataStd_RealArray::Set(const TDF_Label& label, const Standard_Integer lower,
...
Handle(TDataStd_RealArray) TDataStd_RealArray::Set(const TDF_Label& label, const Standard_GUID& theGuid,
...
void TDataStd_RealArray::ChangeArray(const Handle(TColStd_HArray1OfReal) & newArray,
    ...
    if (myValue.IsNull() || !aDimEqual) myValue = new TColStd_HArray1OfReal(aLower, anUpper);
...
void TDataStd_RealArray::Paste(const Handle(TDF_Attribute) & Into, const Handle(TDF_RelocationTable) &) const {
    if (!myValue.IsNull()) {
...
Handle(TDF_DeltaOnModification) TDataStd_RealArray::DeltaOnModification(const Handle(TDF_Attribute) & OldAtt) const {
    if (myIsDelta)
