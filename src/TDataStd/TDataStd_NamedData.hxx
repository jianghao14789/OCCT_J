...
public:
    ...
    Standard_EXPORT const Handle(TColStd_HArray1OfInteger) &
        GetArrayOfIntegers(const TCollection_ExtendedString& theName);
...
public: //! @name late-load deferred data interface
    ...
    //! as such elements are created in undefined state (no data) and Undo/Redo mechanism will not work until deferred
    //! data being loaded.
    //!
    //! Usage scenarios:
    ...
    //! As result, the content of the object will be overridden by data from deferred storage (which is normally
    //! read-only).
    //! @param theToKeepDeferred [in] when TRUE, the link to deferred storage will be preserved
    //!                               so that it will be possible calling UnloadDeferredData() afterwards for releasing
    //!                               memory
