...
public:
    ...
    Standard_EXPORT Handle(StepFEA_FeaAxis2Placement3d)
        GetFeaAxis2Placement3d(const Handle(StepFEA_FeaModel) & theFeaModel) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation) IdealShape(const Handle(StepBasic_Product) & Prod) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation)
        IdealShape(const Handle(StepBasic_ProductDefinitionFormation) & PDF) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation) NominShape(const Handle(StepBasic_Product) & Prod) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation)
        NominShape(const Handle(StepBasic_ProductDefinitionFormation) & PDF) const;
    ...
    Standard_EXPORT Handle(StepFEA_HSequenceOfElementRepresentation)
        GetElements1D(const Handle(StepFEA_FeaModel) & theFeaModel) const;

    Standard_EXPORT Handle(StepFEA_HSequenceOfElementRepresentation)
        GetElements2D(const Handle(StepFEA_FeaModel) & theFEAModel) const;

    Standard_EXPORT Handle(StepFEA_HSequenceOfElementRepresentation)
        GetElements3D(const Handle(StepFEA_FeaModel) & theFEAModel) const;

    //! Getting list of curve_element_section_definitions
    //! for given element_representation
    Standard_EXPORT Handle(StepElement_HSequenceOfCurveElementSectionDefinition)
        GetCurElemSection(const Handle(StepFEA_Curve3dElementRepresentation) & ElemRepr) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation)
        GetShReprForElem(const Handle(StepFEA_ElementRepresentation) & ElemRepr) const;
    ...
    Standard_EXPORT Standard_Boolean CreateAdding203Entities(const Handle(StepBasic_ProductDefinition) & PD,
    ...
    Standard_EXPORT Handle(StepShape_ShapeRepresentation)
        IdealShape(const Handle(StepBasic_ProductDefinition) & PD) const;

    Standard_EXPORT Handle(StepShape_ShapeRepresentation)
        IdealShape(const Handle(StepRepr_ProductDefinitionShape) & PDS) const;

protected:
    Standard_EXPORT Handle(StepFEA_HSequenceOfElementRepresentation)
        GetFeaElements(const Handle(StepFEA_FeaModel) & theFeaModel, const Handle(Standard_Type) & theType) const;
