...
StepAP209_Construct::StepAP209_Construct(const Handle(XSControl_WorkSession) & WS) : STEPConstruct_Tool(WS) {}
...
Handle(StepElement_HSequenceOfElementMaterial) StepAP209_Construct::GetElementMaterial() const {
    Handle(StepElement_HSequenceOfElementMaterial) aSequence = new StepElement_HSequenceOfElementMaterial;
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (anEntity->IsKind(STANDARD_TYPE(StepElement_ElementMaterial))) {
            Handle(StepElement_ElementMaterial) anElement = Handle(StepElement_ElementMaterial)::DownCast(anEntity);
...
Handle(StepShape_ShapeRepresentation)
    StepAP209_Construct::GetShReprForElem(const Handle(StepFEA_ElementRepresentation) & ElemRepr) const {
...
Handle(StepFEA_FeaModel) StepAP209_Construct::FeaModel(const Handle(StepBasic_ProductDefinitionFormation) & PDF) const {
    ...
    for (subs.Start(); subs.More() && FM.IsNull(); subs.Next()) {
        Handle(StepBasic_ProductDefinition) PD = Handle(StepBasic_ProductDefinition)::DownCast(subs.Value());
    ...
    for (subs.Start(); subs.More() && FM.IsNull(); subs.Next()) {
        ...
        for (subs2.Start(); subs2.More() && FM.IsNull(); subs2.Next()) {
            ...
            for (subs3.Start(); subs3.More() && FM.IsNull(); subs3.Next()) {
                ...
                if (Repr.IsNull()) continue;
                if (Repr->IsKind(STANDARD_TYPE(StepFEA_FeaModel))) FM = Handle(StepFEA_FeaModel)::DownCast(Repr);
            }
        }
    }
    return FM;
...
Handle(StepFEA_FeaAxis2Placement3d) StepAP209_Construct::GetFeaAxis2Placement3d(const Handle(StepFEA_FeaModel) &
                                                                                theFeaModel) const {
...
Handle(StepShape_ShapeRepresentation) StepAP209_Construct::IdealShape(const Handle(StepBasic_ProductDefinition) &
                                                                      PD) const {
    Handle(StepShape_ShapeRepresentation) SR;
...
Handle(StepShape_ShapeRepresentation)
    StepAP209_Construct::IdealShape(const Handle(StepBasic_ProductDefinitionFormation) & PDF) const {
    Handle(StepShape_ShapeRepresentation) SR;
...
Handle(StepShape_ShapeRepresentation) StepAP209_Construct::IdealShape(const Handle(StepRepr_ProductDefinitionShape) &
                                                                      PDS) const {
    Handle(StepShape_ShapeRepresentation) SR;
    Interface_EntityIterator subs = Graph().Sharings(PDS);
    for (subs.Start(); subs.More() && SR.IsNull(); subs.Next()) {
...
Handle(StepShape_ShapeRepresentation)
    StepAP209_Construct::NominShape(const Handle(StepBasic_ProductDefinitionFormation) & PDF) const {
    Handle(StepShape_ShapeRepresentation) SR;
    if (PDF.IsNull()) return SR;
    Handle(StepBasic_ProductDefinitionFormation) PDF2;
    Interface_EntityIterator subs = Graph().Sharings(PDF);
...
Handle(StepFEA_HSequenceOfElementRepresentation) StepAP209_Construct::GetElements1D(const Handle(StepFEA_FeaModel) &
                                                                                    theFeaModel) const {
...
Handle(StepFEA_HSequenceOfElementRepresentation) StepAP209_Construct::GetElements2D(const Handle(StepFEA_FeaModel) &
                                                                                    theFeaModel) const {
...
Handle(StepFEA_HSequenceOfElementRepresentation) StepAP209_Construct::GetElements3D(const Handle(StepFEA_FeaModel) &
                                                                                    theFeaModel) const {
...
Handle(StepFEA_HSequenceOfElementRepresentation) StepAP209_Construct::GetFeaElements(
    const Handle(StepFEA_FeaModel) & theFeaModel, const Handle(Standard_Type) & theType) const {
    Handle(StepFEA_HSequenceOfElementRepresentation) aSequence;
    if (!theType->SubType(STANDARD_TYPE(StepFEA_ElementRepresentation))) return aSequence;

    Interface_EntityIterator anIter = Graph().Sharings(theFeaModel);
    anIter.Start();
    if (anIter.More()) aSequence = new StepFEA_HSequenceOfElementRepresentation;

    for (; anIter.More(); anIter.Next()) {
        Handle(Standard_Transient) anEntity = anIter.Value();
        if (anEntity->IsKind(theType)) {
            Handle(StepFEA_ElementRepresentation) anElement = Handle(StepFEA_ElementRepresentation)::DownCast(anEntity);
            aSequence->Append(anElement);
        }
    }
    return aSequence;
...
Handle(StepElement_HSequenceOfCurveElementSectionDefinition)
    StepAP209_Construct::GetCurElemSection(const Handle(StepFEA_Curve3dElementRepresentation) & ElemRepr) const {
...
Standard_Boolean StepAP209_Construct::CreateAnalysStructure(const Handle(StepBasic_Product) & Prod) const {
    ...
    AnaPDF->Init(new TCollection_HAsciiString(""), new TCollection_HAsciiString("analysis version"), AnaProd);
    ...
    PDFR->Init(new TCollection_HAsciiString(""), new TCollection_HAsciiString("analysis design version relationship"),
    ...
    AnaPDC->Init(new TCollection_HAsciiString("analysis"), AC, new TCollection_HAsciiString("analysis"));
    ...
    AnaPD->Init(new TCollection_HAsciiString("analysis"),
                new TCollection_HAsciiString("analysis discipline product definition"), AnaPDF, AnaPDC);
    ...
    AnaPDS->Init(new TCollection_HAsciiString(""), Standard_True, new TCollection_HAsciiString("analysis shape"),
                 ChDef);
    ...
    Handle(StepRepr_ShapeRepresentationRelationship) SRR = new StepRepr_ShapeRepresentationRelationship;
...
Standard_Boolean StepAP209_Construct::CreateFeaStructure(const Handle(StepBasic_Product) & Prod) const {
    ...
    FMD->Init(new TCollection_HAsciiString("FEA_MODEL"), new TCollection_HAsciiString("FEA_MODEL"), AnaPDS,
              StepData_LFalse);
    ...
    FA2P3D->Init(new TCollection_HAsciiString("FEA_BASIC_COORD_SYSTEM"), SGCP, Standard_True, SGD1, Standard_True, SGD2,
                 StepFEA_Cartesian, new TCollection_HAsciiString("FEA_BASIC_COORD_SYSTEM"));
    ...
    Handle(StepBasic_SiUnitAndThermodynamicTemperatureUnit) SUTTU = new StepBasic_SiUnitAndThermodynamicTemperatureUnit;
    ...
    NewGeoCtx->Init(new TCollection_HAsciiString("REP_CONTEXT_FEA"), new TCollection_HAsciiString("3D"), GeoCtx,
                    NewUnitCtx);
    ...
    FM->Init(new TCollection_HAsciiString("FEA_MODEL"), HARI, NewGeoCtx, new TCollection_HAsciiString(""), HAAS,
             new TCollection_HAsciiString("ANALYSIS_MODEL"), new TCollection_HAsciiString("LINEAR_STATIC"));
...
Standard_Boolean StepAP209_Construct::ReplaceCcDesingToApplied() const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (anEntity->IsKind(STANDARD_TYPE(StepAP203_CcDesignApproval))) {
            ...
            Handle(StepAP214_HArray1OfApprovalItem) HAAI214 = new StepAP214_HArray1OfApprovalItem(1, HAAI203->Length());
...
Standard_Boolean StepAP209_Construct::CreateAddingEntities(const Handle(StepBasic_ProductDefinition) & AnaPD) const {
    ...
    LT->Init(date.Hour(), Standard_True, date.Minute(), Standard_True, (Standard_Real)date.Second(), CUTO);
    ...
    Pers->Init(new TCollection_HAsciiString("1"), Standard_True, new TCollection_HAsciiString("last_name"),
               Standard_True, new TCollection_HAsciiString("first_name"), Standard_True, HAHAS, Standard_True, HAHAS,
               Standard_True, HAHAS);
    ...
    Org->Init(Standard_True, new TCollection_HAsciiString("1"), new TCollection_HAsciiString("organisation"),
    ...
    Handle(StepAP214_AppliedPersonAndOrganizationAssignment) APOA =
        new StepAP214_AppliedPersonAndOrganizationAssignment;
    Handle(StepAP214_HArray1OfPersonAndOrganizationItem) HAPOI = new StepAP214_HArray1OfPersonAndOrganizationItem(1, 1);
...
Handle(StepData_StepModel) StepAP209_Construct::CreateAP203Structure() const {
    ...
    PRPC->Init(new TCollection_HAsciiString("design"), Standard_True, Prod->Name(),
               HAProd); // may be Prod->Description() - ???
    nmodel->AddEntity(PRPC);
    Handle(StepBasic_ProductCategory) PCat = new StepBasic_ProductCategory;
    PCat->Init(new TCollection_HAsciiString("part"), Standard_True, Prod->Name()); // may be Prod->Description() - ???
    ...
    PCR->Init(new TCollection_HAsciiString(""), Standard_True, Prod->Name(), PCat,
    ...
    Handle(HeaderSection_FileName) FN =
        Handle(HeaderSection_FileName)::DownCast(smodel->HeaderEntity(STANDARD_TYPE(HeaderSection_FileName)));
    if (!FN.IsNull()) {
    ...
    Handle(HeaderSection_FileSchema) FS =
        Handle(HeaderSection_FileSchema)::DownCast(smodel->HeaderEntity(STANDARD_TYPE(HeaderSection_FileSchema)));
    if (!FS.IsNull()) nmodel->AddHeaderEntity(FS);
...
Standard_Boolean StepAP209_Construct::CreateAdding203Entities(const Handle(StepBasic_ProductDefinition) & PD,
    ...
    Handle(StepAP203_CcDesignSecurityClassification) DSC = new StepAP203_CcDesignSecurityClassification;
    ...
    for (subs.Start(); subs.More(); subs.Next()) {
        ...
        if (!AAA.IsNull()) {
            ...
            Handle(StepAP203_HArray1OfApprovedItem) HAAI = new StepAP203_HArray1OfApprovedItem(1, HAAI214->Length());
            ...
            for (subs2.Start(); subs2.More(); subs2.Next()) {
                Handle(StepBasic_ApprovalDateTime) ADT = Handle(StepBasic_ApprovalDateTime)::DownCast(subs2.Value());
                if (ADT.IsNull()) continue;
                aModel->AddWithRefs(ADT);
                Handle(StepBasic_DateAndTime) DT1 = Handle(StepBasic_DateAndTime)::DownCast(ADT->DateTime().Value());
    ...
    if (aModel->Number(DT) > 0) {
        ...
        for (subs.Start(); subs.More(); subs.Next()) {
            ...
            Handle(StepAP203_HArray1OfDateTimeItem) HADTI = new StepAP203_HArray1OfDateTimeItem(1, HADTI214->Length());
            ...
            Handle(StepAP203_CcDesignDateAndTimeAssignment) DDTA = new StepAP203_CcDesignDateAndTimeAssignment;
