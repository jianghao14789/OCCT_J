...
const Handle(Standard_Transient) & XmlDrivers::Factory(const Standard_GUID& theGUID) {
    if (theGUID == XmlStorageDriver) {
        ...
        static Handle(Standard_Transient) model_sd =
            new XmlDrivers_DocumentStorageDriver("Copyright: Open Cascade, 2001-2002"); // default copyright
        return model_sd;
    }
...
void XmlDrivers::DefineFormat(const Handle(TDocStd_Application) & theApp) {
    theApp->DefineFormat("XmlOcaf", "Xml OCAF Document", "xml", new XmlDrivers_DocumentRetrievalDriver,
...
// purpose  :
//=======================================================================
Handle(XmlMDF_ADriverTable) XmlDrivers::AttributeDrivers(const Handle(Message_Messenger) & theMessageDriver) {
