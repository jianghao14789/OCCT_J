...
void IGESSolid_ToolToroidalSurface::ReadOwnParams(const Handle(IGESSolid_ToroidalSurface) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "Center point", STANDARD_TYPE(IGESGeom_Point),
...
IGESData_DirChecker IGESSolid_ToolToroidalSurface::DirChecker(const Handle(IGESSolid_ToroidalSurface) & /*ent*/) const {
...
void IGESSolid_ToolToroidalSurface::OwnCheck(const Handle(IGESSolid_ToroidalSurface) & ent, const Interface_ShareTool&,
                                             Handle(Interface_Check) & ach) const {
    if (ent->MajorRadius() <= 0.0) ach->AddFail("Major Radius : Not Positive");
    if (ent->MinorRadius() <= 0.0) ach->AddFail("Minor Radius : Not Positive");
    if (ent->MinorRadius() >= ent->MajorRadius()) ach->AddFail("Minor Radius : Value not < Major radius");
