...
void IGESSolid_ToolLoop::ReadOwnParams(const Handle(IGESSolid_Loop) & ent, const Handle(IGESData_IGESReaderData) & IR,
    ...
    if (sb && (nbedges > 0)) {
        ...
        for (i = 1; i <= nbedges; i++) {
            ...
            if (PR.ReadBoolean(PR.Current(), Msg180, abool)) tempOrientation->SetValue(i, (abool ? 1 : 0)); // bool;
            ...
            if (st && anint > 0) {
                ...
                for (j = 1; j <= anint; j++) {
                    ...
                    if (PR.ReadBoolean(PR.Current(), Msg195, abool)) tmpints->SetValue(j, (abool ? 1 : 0)); // bool;
    ...
    ent->Init(tempTypes, tempEdges, tempIndex, tempOrientation, nbParameterCurves, isoparametricFlags, tempCurves);
}
...
void IGESSolid_ToolLoop::WriteOwnParams(const Handle(IGESSolid_Loop) & ent, IGESData_IGESWriter& IW) const {
    Standard_Integer i, j;
...
void IGESSolid_ToolLoop::OwnShared(const Handle(IGESSolid_Loop) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer i, j;
    Standard_Integer length = ent->NbEdges();
...
void IGESSolid_ToolLoop::OwnCopy(const Handle(IGESSolid_Loop) & another, const Handle(IGESSolid_Loop) & ent,
    ...
    Handle(IGESBasic_HArray1OfHArray1OfIGESEntity) tempCurves = new IGESBasic_HArray1OfHArray1OfIGESEntity(1, nbedges);
...
void IGESSolid_ToolLoop::OwnCheck(const Handle(IGESSolid_Loop) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolLoop::OwnDump(const Handle(IGESSolid_Loop) & ent, const IGESData_IGESDumper& dumper,
