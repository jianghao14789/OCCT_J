...
void IGESSolid_ToolBooleanTree::ReadOwnParams(const Handle(IGESSolid_BooleanTree) & ent,
    ...
    if (st && length > 0) {
        ...
        if (PR.ReadEntity(IR, PR.Current(), "Operand 1", entvalue)) tempOperands->SetValue(1, entvalue);

        // st = PR.ReadEntity(IR, PR.Current(), "Operand 2", entvalue); //szv#4:S4163:12Mar99 moved in if
        if (PR.ReadEntity(IR, PR.Current(), "Operand 2", entvalue)) tempOperands->SetValue(2, entvalue);
        ...
        if (PR.ReadInteger(PR.Current(), "Operation code", intvalue)) tempOperations->SetValue(length, intvalue);
    } else
...
IGESData_DirChecker IGESSolid_ToolBooleanTree::DirChecker(const Handle(IGESSolid_BooleanTree) & /*ent*/) const {
...
void IGESSolid_ToolBooleanTree::OwnCheck(const Handle(IGESSolid_BooleanTree) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolBooleanTree::OwnDump(const Handle(IGESSolid_BooleanTree) & ent, const IGESData_IGESDumper& dumper,
