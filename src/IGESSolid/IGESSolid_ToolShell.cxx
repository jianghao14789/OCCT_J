...
void IGESSolid_ToolShell::ReadOwnParams(const Handle(IGESSolid_Shell) & ent, const Handle(IGESData_IGESReaderData) & IR,
    ...
    if (sb && nbfaces > 0) {
        ...
        for (Standard_Integer i = 1; i <= nbfaces; i++) {
            // st = PR.ReadEntity(IR, PR.Current(),Msg201, STANDARD_TYPE(IGESSolid_Face), aface); //szv#4:S4163:12Mar99
            // moved in if st = PR.ReadEntity(IR, PR.Current(), "Faces", STANDARD_TYPE(IGESSolid_Face), aface);
            ...
            if (PR.ReadBoolean(PR.Current(), Msg180, abool)) tempOrientation->SetValue(i, (abool ? 1 : 0));
        }
    } else {
...
void IGESSolid_ToolShell::WriteOwnParams(const Handle(IGESSolid_Shell) & ent, IGESData_IGESWriter& IW) const {
...
void IGESSolid_ToolShell::OwnShared(const Handle(IGESSolid_Shell) & ent, Interface_EntityIterator& iter) const {
...
void IGESSolid_ToolShell::OwnCopy(const Handle(IGESSolid_Shell) & another, const Handle(IGESSolid_Shell) & ent,
...
void IGESSolid_ToolShell::OwnCheck(const Handle(IGESSolid_Shell) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolShell::OwnDump(const Handle(IGESSolid_Shell) & ent, const IGESData_IGESDumper& dumper,
