...
void IGESSolid_ToolEdgeList::ReadOwnParams(const Handle(IGESSolid_EdgeList) & ent,
    ...
    if (length > 0)
        ent->Init(tempCurves, tempStartVertexList, tempStartVertexIndex, tempEndVertexList, tempEndVertexIndex);
}
...
void IGESSolid_ToolEdgeList::WriteOwnParams(const Handle(IGESSolid_EdgeList) & ent, IGESData_IGESWriter& IW) const {
...
void IGESSolid_ToolEdgeList::OwnShared(const Handle(IGESSolid_EdgeList) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer length = ent->NbEdges();
    for (Standard_Integer i = 1; i <= length; i++) {
...
void IGESSolid_ToolEdgeList::OwnCopy(const Handle(IGESSolid_EdgeList) & another, const Handle(IGESSolid_EdgeList) & ent,
                                     Interface_CopyTool& TC) const {
    Standard_Integer length;

    length = another->NbEdges();
    Handle(IGESData_HArray1OfIGESEntity) tempCurves = new IGESData_HArray1OfIGESEntity(1, length);
    Handle(IGESSolid_HArray1OfVertexList) tempStartVertexList = new IGESSolid_HArray1OfVertexList(1, length);
    Handle(TColStd_HArray1OfInteger) tempStartVertexIndex = new TColStd_HArray1OfInteger(1, length);
    Handle(IGESSolid_HArray1OfVertexList) tempEndVertexList = new IGESSolid_HArray1OfVertexList(1, length);
...
IGESData_DirChecker IGESSolid_ToolEdgeList::DirChecker(const Handle(IGESSolid_EdgeList) & /* ent */) const {
...
void IGESSolid_ToolEdgeList::OwnCheck(const Handle(IGESSolid_EdgeList) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolEdgeList::OwnDump(const Handle(IGESSolid_EdgeList) & ent, const IGESData_IGESDumper& dumper,
