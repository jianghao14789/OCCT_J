...
void IGESSolid_Loop::Init(const Handle(TColStd_HArray1OfInteger) & Types,
    ...
    if (Types->Lower() != 1 || Edges->Lower() != 1 || nb != Edges->Length() || Index->Lower() != 1 ||
        nb != Index->Length() || Orient->Lower() != 1 || nb != Orient->Length() || nbParameterCurves->Lower() != 1 ||
        nb != nbParameterCurves->Length() || IsoparametricFlags->Lower() != 1 || nb != IsoparametricFlags->Length() ||
...
Handle(IGESData_IGESEntity) IGESSolid_Loop::ParametricCurve(const Standard_Integer EdgeIndex,
    ...
    if (!theCurves->Value(EdgeIndex).IsNull()) acurve = theCurves->Value(EdgeIndex)->Value(CurveIndex);
    return acurve;
}
