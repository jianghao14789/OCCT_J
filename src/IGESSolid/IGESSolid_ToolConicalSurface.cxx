...
void IGESSolid_ToolConicalSurface::ReadOwnParams(const Handle(IGESSolid_ConicalSurface) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "Point on axis", STANDARD_TYPE(IGESGeom_Point),
                  tempLocation); // szv#4:S4163:12Mar99 `st=` not needed

    PR.ReadEntity(IR, PR.Current(), "Axis direction", STANDARD_TYPE(IGESGeom_Direction),
    ...
    if (ent->FormNumber() == 1) // Parametrised surface
        PR.ReadEntity(IR, PR.Current(), "Reference direction", STANDARD_TYPE(IGESGeom_Direction),
...
IGESData_DirChecker IGESSolid_ToolConicalSurface::DirChecker(const Handle(IGESSolid_ConicalSurface) & /*ent*/) const {
...
void IGESSolid_ToolConicalSurface::OwnCheck(const Handle(IGESSolid_ConicalSurface) & ent, const Interface_ShareTool&,
                                            Handle(Interface_Check) & ach) const {
    if (ent->Radius() < 0.0) ach->AddFail("Radius : Value Negative");
    if (ent->SemiAngle() < 0.0 || ent->SemiAngle() > 90.0) ach->AddFail("Semi-angle : Value not in the range [0 - 90]");
    Standard_Integer fn = 0;
    if (ent->IsParametrised()) fn = 1;
    if (fn != ent->FormNumber()) ach->AddFail("Parametrised Status Mismatches with Form Number");
}
