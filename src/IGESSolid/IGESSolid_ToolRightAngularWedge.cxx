...
void IGESSolid_ToolRightAngularWedge::OwnShared(const Handle(IGESSolid_RightAngularWedge) & /* ent */,
...
void IGESSolid_ToolRightAngularWedge::OwnCopy(const Handle(IGESSolid_RightAngularWedge) & another,
    ...
    ent->Init(another->Size(), another->XSmallLength(), another->Corner().XYZ(), another->XAxis().XYZ(),
              another->ZAxis().XYZ());
}

IGESData_DirChecker
IGESSolid_ToolRightAngularWedge::DirChecker(const Handle(IGESSolid_RightAngularWedge) & /* ent */) const {
...
void IGESSolid_ToolRightAngularWedge::OwnCheck(const Handle(IGESSolid_RightAngularWedge) & ent,
    ...
    if (ent->XSmallLength() <= 0.0) ach->AddFail("Small X Length : Not Positive");
    if (ent->XSmallLength() >= ent->Size().X()) ach->AddFail("Small X Length : Value not < LX");
}
