...
void IGESSolid_TopoBuilder::AddEdge(const Handle(IGESData_IGESEntity) & curve, const Standard_Integer vstart,
                                    const Standard_Integer vend) {
    if (curve.IsNull() || vstart <= 0 || vend <= 0 || vstart > thepoint->Length() || vend > thepoint->Length())
...
void IGESSolid_TopoBuilder::Edge(const Standard_Integer num, Handle(IGESData_IGESEntity) & curve,
...
void IGESSolid_TopoBuilder::MakeEdge(const Standard_Integer edgetype, const Standard_Integer edge3d,
                                     const Standard_Integer orientation) {
    if (edge3d <= 0 || edge3d > thecur3d->Length()) throw Standard_DomainError("IGESSolid_TopoBuilder : MakeEdge");
...
void IGESSolid_TopoBuilder::AddCurveUV(const Handle(IGESData_IGESEntity) & curve, const Standard_Integer iso) {
    if (curve.IsNull() || thee3d->IsEmpty()) throw Standard_DomainError("IGESSolid_TopoBuilder : AddCurveUV");
...
void IGESSolid_TopoBuilder::MakeFace(const Handle(IGESData_IGESEntity) & surface) {
    if (surface.IsNull()) throw Standard_DomainError("IGESSolid_TopoBuilder : MakeFace");
    thesurf = surface;
