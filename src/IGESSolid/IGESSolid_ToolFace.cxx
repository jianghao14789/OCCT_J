...
void IGESSolid_ToolFace::ReadOwnParams(const Handle(IGESSolid_Face) & ent, const Handle(IGESData_IGESReaderData) & IR,
    ...
    */
    ...
    if (!tempLoops.IsNull()) {
        for (Standard_Integer i = 1; i <= nbloops; i++) {
            // st = PR.ReadEntity(IR, PR.Current(), Msg199, STANDARD_TYPE(IGESSolid_Loop), aloop); //szv#4:S4163:12Mar99
            // moved in if st = PR.ReadEntity(IR, PR.Current(), "Loops", STANDARD_TYPE(IGESSolid_Loop), aloop);
...
void IGESSolid_ToolFace::WriteOwnParams(const Handle(IGESSolid_Face) & ent, IGESData_IGESWriter& IW) const {
...
void IGESSolid_ToolFace::OwnShared(const Handle(IGESSolid_Face) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer upper = ent->NbLoops();
...
void IGESSolid_ToolFace::OwnCopy(const Handle(IGESSolid_Face) & another, const Handle(IGESSolid_Face) & ent,
...
void IGESSolid_ToolFace::OwnCheck(const Handle(IGESSolid_Face) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolFace::OwnDump(const Handle(IGESSolid_Face) & ent, const IGESData_IGESDumper& dumper,
