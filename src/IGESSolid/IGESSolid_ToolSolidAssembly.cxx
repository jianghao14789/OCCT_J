...
void IGESSolid_ToolSolidAssembly::ReadOwnParams(const Handle(IGESSolid_SolidAssembly) & ent,
    ...
    if (st && nbitems > 0) {
        ...
        for (i = 1; i <= nbitems; i++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "Solid assembly items", anent)) tempItems->SetValue(i, anent);
        }
        ...
        for (i = 1; i <= nbitems; i++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "Matrices", STANDARD_TYPE(IGESGeom_TransformationMatrix), amatr,
                              Standard_True))
...
void IGESSolid_ToolSolidAssembly::OwnCopy(const Handle(IGESSolid_SolidAssembly) & another,
    ...
    for (i = 1; i <= nbitems; i++) {
        DeclareAndCast(IGESGeom_TransformationMatrix, newlocalent, TC.Transferred(another->TransfMatrix(i)));
