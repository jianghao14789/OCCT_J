...
void IGESSolid_ToolSolidOfLinearExtrusion::ReadOwnParams(const Handle(IGESSolid_SolidOfLinearExtrusion) & ent,
    ...
    if (PR.DefinedElseSkip()) {
        ...
        if (PR.ReadReal(PR.Current(), "Extrusion direction (I)", tempreal)) tempDirection.SetX(tempreal);
    } else
        tempDirection.SetX(0.0);

    if (PR.DefinedElseSkip()) {
        // st = PR.ReadReal(PR.Current(), "Extrusion direction (J)", tempreal); //szv#4:S4163:12Mar99 moved in if
        if (PR.ReadReal(PR.Current(), "Extrusion direction (J)", tempreal)) tempDirection.SetY(tempreal);
    } else
        tempDirection.SetY(0.0);

    if (PR.DefinedElseSkip()) {
        // st = PR.ReadReal(PR.Current(), "Extrusion direction (K)", tempreal); //szv#4:S4163:12Mar99 moved in if
        if (PR.ReadReal(PR.Current(), "Extrusion direction (K)", tempreal)) tempDirection.SetZ(tempreal);
    } else
...
void IGESSolid_ToolSolidOfLinearExtrusion::WriteOwnParams(const Handle(IGESSolid_SolidOfLinearExtrusion) & ent,
...
void IGESSolid_ToolSolidOfLinearExtrusion::OwnShared(const Handle(IGESSolid_SolidOfLinearExtrusion) & ent,
                                                     Interface_EntityIterator& iter) const {
    iter.GetOneItem(ent->Curve());
}

void IGESSolid_ToolSolidOfLinearExtrusion::OwnCopy(const Handle(IGESSolid_SolidOfLinearExtrusion) & another,
...
IGESData_DirChecker
IGESSolid_ToolSolidOfLinearExtrusion::DirChecker(const Handle(IGESSolid_SolidOfLinearExtrusion) & /* ent */) const {
...
void IGESSolid_ToolSolidOfLinearExtrusion::OwnCheck(const Handle(IGESSolid_SolidOfLinearExtrusion) & ent,
                                                    const Interface_ShareTool&, Handle(Interface_Check) & ach) const {
    if (ent->ExtrusionLength() <= 0.0) ach->AddFail("Length of extrusion : Not Positive");
}

void IGESSolid_ToolSolidOfLinearExtrusion::OwnDump(const Handle(IGESSolid_SolidOfLinearExtrusion) & ent,
