...
void IGESSolid_ToolPlaneSurface::ReadOwnParams(const Handle(IGESSolid_PlaneSurface) & ent,
    ...
    if (!PR.ReadEntity(IR, PR.Current(), aStatus, STANDARD_TYPE(IGESGeom_Point),
    ...
    */
    if (!PR.ReadEntity(IR, PR.Current(), aStatus, STANDARD_TYPE(IGESGeom_Direction),
                       tempNormal)) { // szv#4:S4163:12Mar99 `st=` not needed
...
IGESData_DirChecker IGESSolid_ToolPlaneSurface::DirChecker(const Handle(IGESSolid_PlaneSurface) & /*ent*/) const {
...
void IGESSolid_ToolPlaneSurface::OwnCheck(const Handle(IGESSolid_PlaneSurface) & ent, const Interface_ShareTool&,
...
void IGESSolid_ToolPlaneSurface::OwnDump(const Handle(IGESSolid_PlaneSurface) & ent, const IGESData_IGESDumper& dumper,
