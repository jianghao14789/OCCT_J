...
void IGESAppli_ToolLevelFunction::ReadOwnParams(const Handle(IGESAppli_LevelFunction) & ent,
    ...
    if (PR.DefinedElseSkip()) PR.ReadText(PR.Current(), "Function description", tempFuncDescrip);
...
void IGESAppli_ToolLevelFunction::OwnCopy(const Handle(IGESAppli_LevelFunction) & another,
    ...
    if (!another->FuncDescription().IsNull()) descrip = new TCollection_HAsciiString(another->FuncDescription());
...
void IGESAppli_ToolLevelFunction::OwnCheck(const Handle(IGESAppli_LevelFunction) & ent, const Interface_ShareTool&,
                                           Handle(Interface_Check) & ach) const {
    if (ent->NbPropertyValues() != 2) ach->AddFail("Number of Property Values != 2");
}
