...
void IGESAppli_NodalConstraint::Init(const Standard_Integer aType, const Handle(IGESAppli_Node) & aNode,
                                     const Handle(IGESDefs_HArray1OfTabularData) & allTabData) {
    if (allTabData->Lower() != 1) throw Standard_DimensionMismatch("IGESAppli_NodalConstraint : Init");
    theType = aType;
    theNode = aNode;
...
Handle(IGESDefs_TabularData) IGESAppli_NodalConstraint::TabularData(const Standard_Integer Index) const {
