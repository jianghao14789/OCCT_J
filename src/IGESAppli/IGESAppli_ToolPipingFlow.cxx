...
void IGESAppli_ToolPipingFlow::ReadOwnParams(const Handle(IGESAppli_PipingFlow) & ent,
    ...
    ent->Init(tempNbContextFlags, tempTypeOfFlow, tempFlowAssocs, tempConnectPoints, tempJoins, tempFlowNames,
              tempTextDisplayTemplates, tempContFlowAssocs);
}

void IGESAppli_ToolPipingFlow::WriteOwnParams(const Handle(IGESAppli_PipingFlow) & ent, IGESData_IGESWriter& IW) const {
    Standard_Integer i, num;
...
void IGESAppli_ToolPipingFlow::OwnCopy(const Handle(IGESAppli_PipingFlow) & another,
    ...
    Handle(IGESDraw_HArray1OfConnectPoint) tempConnectPoints = new IGESDraw_HArray1OfConnectPoint(1, num);
    for (i = 1; i <= num; i++) {
    ...
    Handle(Interface_HArray1OfHAsciiString) tempFlowNames = new Interface_HArray1OfHAsciiString(1, num);
    for (i = 1; i <= num; i++)
    ...
    Handle(IGESData_HArray1OfIGESEntity) tempContFlowAssocs = new IGESData_HArray1OfIGESEntity(1, num);
    for (i = 1; i <= num; i++) {
...
void IGESAppli_ToolPipingFlow::OwnCheck(const Handle(IGESAppli_PipingFlow) & ent, const Interface_ShareTool&,
                                        Handle(Interface_Check) & ach) const {
    if (ent->NbContextFlags() != 1) ach->AddFail("Number of Context Flags != 1");
    if ((ent->TypeOfFlow() < 0) || (ent->TypeOfFlow() > 2)) ach->AddFail("Type of Flow != 0,1,2");
}

void IGESAppli_ToolPipingFlow::OwnDump(const Handle(IGESAppli_PipingFlow) & ent, const IGESData_IGESDumper& dumper,
