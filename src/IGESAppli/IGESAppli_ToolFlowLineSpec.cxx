...
void IGESAppli_ToolFlowLineSpec::ReadOwnParams(const Handle(IGESAppli_FlowLineSpec) & ent,
    ...
    if (!tempNameAndModifiers.IsNull()) PR.ReadTexts(PR.CurrentList(num), "Name and Modifiers", tempNameAndModifiers);
...
void IGESAppli_ToolFlowLineSpec::OwnCopy(const Handle(IGESAppli_FlowLineSpec) & another,
    ...
    Handle(Interface_HArray1OfHAsciiString) tempNameAndModifiers = new Interface_HArray1OfHAsciiString(1, num);
    for (Standard_Integer i = 1; i <= num; i++)
        tempNameAndModifiers->SetValue(i, new TCollection_HAsciiString(another->Modifier(i)));
    ent->Init(tempNameAndModifiers);
}

IGESData_DirChecker IGESAppli_ToolFlowLineSpec::DirChecker(const Handle(IGESAppli_FlowLineSpec) & /* ent */) const {
...
void IGESAppli_ToolFlowLineSpec::OwnCheck(const Handle(IGESAppli_FlowLineSpec) & /* ent */, const Interface_ShareTool&,
