...
void IGESAppli_ToolNodalConstraint::ReadOwnParams(const Handle(IGESAppli_NodalConstraint) & ent,
    ...
    if (!tempTabularDataProps.IsNull())
        for (i = 1; i <= num; i++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "Tabular Data Property", STANDARD_TYPE(IGESDefs_TabularData),
                              tempEntity))
...
void IGESAppli_ToolNodalConstraint::OwnCopy(const Handle(IGESAppli_NodalConstraint) & another,
    ...
    Handle(IGESDefs_HArray1OfTabularData) tempTabularDataProps = new IGESDefs_HArray1OfTabularData(1, num);
...
IGESData_DirChecker
IGESAppli_ToolNodalConstraint::DirChecker(const Handle(IGESAppli_NodalConstraint) & /* ent */) const {
...
void IGESAppli_ToolNodalConstraint::OwnCheck(const Handle(IGESAppli_NodalConstraint) & ent, const Interface_ShareTool&,
                                             Handle(Interface_Check) & ach) const {
    if ((ent->Type() != 1) && (ent->Type() != 2)) ach->AddFail("Type of Constraint != 1,2");
}
