...
void IGESAppli_ToolNodalDisplAndRot::ReadOwnParams(const Handle(IGESAppli_NodalDisplAndRot) & ent,
    ...
    if (PR.ReadInteger(PR.Current(), "No. of nodes", nbnodes)) {
        ...
        for (Standard_Integer j = 1; j <= nbnodes; j++) {
            ...
            if (PR.ReadInteger(PR.Current(), "Node number", nodeno)) tempNodeIdentifiers->SetValue(j, nodeno);
            ...
            if (tempFlag) {
                ...
                for (Standard_Integer k = 1; k <= nbcases; k++) {
                    gp_XYZ atrans, arot;
                    if (PR.ReadXYZ(PR.CurrentList(1, 3), "Translation XYZ", atrans)) tempArray->SetValue(k, atrans);
                    if (PR.ReadXYZ(PR.CurrentList(1, 3), "Rotational XYZ", arot)) tempArray1->SetValue(k, arot);
                }
            }
...
void IGESAppli_ToolNodalDisplAndRot::OwnCopy(const Handle(IGESAppli_NodalDisplAndRot) & another,
    ...
    Handle(IGESBasic_HArray1OfHArray1OfXYZ) aTransParam = new IGESBasic_HArray1OfHArray1OfXYZ(1, nbnodes);
    Handle(IGESBasic_HArray1OfHArray1OfXYZ) aRotParam = new IGESBasic_HArray1OfHArray1OfXYZ(1, nbnodes);
...
IGESData_DirChecker
IGESAppli_ToolNodalDisplAndRot::DirChecker(const Handle(IGESAppli_NodalDisplAndRot) & /* ent */) const {
            ...
            for (Standard_Integer j = 1; j <= nbnodes; j++) {
                ...
                for (Standard_Integer k = 1; k <= nbcases; k++) {
                    ...
                    S << "Translational Parameter : X=" << ent->TranslationParameter(j, k).X()
                      << ", Y=" << ent->TranslationParameter(j, k).Y() << ", Z=" << ent->TranslationParameter(j, k).Z()
                      << "\n";
                    S << "Rotational Parameter : X=" << ent->RotationalParameter(j, k).X()
                      << ", Y=" << ent->RotationalParameter(j, k).Y() << ", Z=" << ent->RotationalParameter(j, k).Z();
                    if (k == nbcases) S << "] ";
                    S << "\n";
                }
            }
