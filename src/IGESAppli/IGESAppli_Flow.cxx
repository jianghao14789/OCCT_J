...
void IGESAppli_Flow::Init(const Standard_Integer nbContextFlags, const Standard_Integer aFlowType,
                          const Standard_Integer aFuncFlag, const Handle(IGESData_HArray1OfIGESEntity) & allFlowAssocs,
...
Handle(IGESGraph_TextDisplayTemplate) IGESAppli_Flow::TextDisplayTemplate(const Standard_Integer Index) const {
    return theTextDisplayTemplates->Value(Index);
}

Handle(IGESData_IGESEntity) IGESAppli_Flow::ContFlowAssociativity(const Standard_Integer Index) const {
