...
void IGESAppli_ToolDrilledHole::ReadOwnParams(const Handle(IGESAppli_DrilledHole) & ent,
    ...
    ent->Init(tempNbPropertyValues, tempDrillDiaSize, tempFinishDiaSize, tempPlatingFlag, tempNbLowerLayer,
              tempNbHigherLayer);
}
...
void IGESAppli_ToolDrilledHole::OwnCopy(const Handle(IGESAppli_DrilledHole) & another,
    ...
    ent->Init(5, another->DrillDiaSize(), another->FinishDiaSize(), (another->IsPlating() ? 1 : 0),
              another->NbLowerLayer(), another->NbHigherLayer());
}

Standard_Boolean IGESAppli_ToolDrilledHole::OwnCorrect(const Handle(IGESAppli_DrilledHole) & ent) const {
    Standard_Boolean res = (ent->NbPropertyValues() != 5);
    if (res)
        ent->Init(5, ent->DrillDiaSize(), ent->FinishDiaSize(), (ent->IsPlating() ? 1 : 0), ent->NbLowerLayer(),
                  ent->NbHigherLayer());
...
IGESData_DirChecker IGESAppli_ToolDrilledHole::DirChecker(const Handle(IGESAppli_DrilledHole) & /*ent*/) const {
...
void IGESAppli_ToolDrilledHole::OwnCheck(const Handle(IGESAppli_DrilledHole) & ent, const Interface_ShareTool&,
