...
void IGESAppli_ToolNodalResults::ReadOwnParams(const Handle(IGESAppli_NodalResults) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "General Note describing the analysis case", STANDARD_TYPE(IGESDimen_GeneralNote),
                  tempNote);
    ...
    if (PR.ReadInteger(PR.Current(), "No. of nodes", nbnodes)) {
        ...
        for (Standard_Integer i = 1; i <= nbnodes; i++) {
            ...
            if (PR.ReadInteger(PR.Current(), "Node no. identifier", aitem)) tempNodeIdentifiers->SetValue(i, aitem);
            ...
            if (tempFlag)
                ...
                for (Standard_Integer j = 1; j <= nbval; j++) {
                    Standard_Real aval;
                    if (PR.ReadReal(PR.Current(), "Value", aval)) tempData->SetValue(i, j, aval);
                }
        }
    }
...
void IGESAppli_ToolNodalResults::OwnCheck(const Handle(IGESAppli_NodalResults) & ent, const Interface_ShareTool&,
...
void IGESAppli_ToolNodalResults::OwnDump(const Handle(IGESAppli_NodalResults) & ent, const IGESData_IGESDumper& dumper,
