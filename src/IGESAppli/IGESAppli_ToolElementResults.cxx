...
void IGESAppli_ToolElementResults::ReadOwnParams(const Handle(IGESAppli_ElementResults) & ent,
    ...
    for (Standard_Integer nume = 1; nume <= num; nume++) {
        ...
        if (PR.ReadInteger(PR.Current(), "FEM Element Identifier", ival)) allElementIdentifiers->SetValue(nume, ival);
        ...
        if (PR.ReadInteger(PR.Current(), "Nb. of layers", nl)) allNbLayers->SetValue(nume, nl);
        if (PR.ReadInteger(PR.Current(), "Data Layer Flag", ival)) allDataLayerFlag->SetValue(nume, ival);
        if (PR.ReadInteger(PR.Current(), "Nb. of result data locations", nrl)) allNbResultDataLocs->SetValue(nume, nrl);
        if (PR.ReadInts(PR.CurrentList(nrl), "Result data locations", rdrl)) allResultDataLoc->SetValue(nume, rdrl);
        PR.ReadInteger(PR.Current(), "Nb. of result data locations", numv); // szv#4:S4163:12Mar99 `st=` not needed
        if (PR.ReadReals(PR.CurrentList(numv), "Result Data", vres)) allResultData->SetValue(nume, vres);
    }

    DirChecker(ent).CheckTypeAndForm(PR.CCheck(), ent);
    ent->Init(aNote, aSubcaseNumber, aTime, nv, aResultReportFlag, allElementIdentifiers, allElements,
              allElementTopologyType, allNbLayers, allDataLayerFlag, allNbResultDataLocs, allResultDataLoc,
              allResultData);
}
...
void IGESAppli_ToolElementResults::OwnCopy(const Handle(IGESAppli_ElementResults) & another,
    ...
    for (i = 1; i <= num; i++) {
        ...
        allElements->SetValue(i, GetCasted(IGESAppli_FiniteElement, TC.Transferred(another->Element(i))));
...
IGESData_DirChecker
IGESAppli_ToolElementResults::DirChecker(const Handle(IGESAppli_ElementResults) & /* ent  */) const {
...
void IGESAppli_ToolElementResults::OwnCheck(const Handle(IGESAppli_ElementResults) & ent, const Interface_ShareTool&,
...
// UNFINISHED
{
    ...
    switch (ent->FormNumber()) {
        case 0:
            if (nv < 0) OK = Standard_False;
            break;
        case 1:
            if (nv != 1) OK = Standard_False;
            break;
        case 2:
            if (nv != 1) OK = Standard_False;
            break;
        case 3:
            if (nv != 3) OK = Standard_False;
            break;
        case 4:
            if (nv != 6) OK = Standard_False;
            break;
        case 5:
            if (nv != 3) OK = Standard_False;
            break;
    ...
    if (!OK) ach->AddFail("Incorrect count of real values in array V for FEM node");
    ...
    for (Standard_Integer i = 1; i <= ne; i++) {
        ...
        Standard_Integer nl = ent->NbLayers(i);
        Standard_Integer nrl = ent->NbResultDataLocs(i);
        if (dlf < 0 || dlf > 4) ach->AddFail("One of the Data Layer Flags not in [0-4]");
