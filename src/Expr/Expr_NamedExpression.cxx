...
Standard_Boolean Expr_NamedExpression::IsIdentical(const Handle(Expr_GeneralExpression) & theOther) const {
    ...
    if (theOther->IsKind(STANDARD_TYPE(Expr_NamedExpression))) {
        ...
        if (pOther == this || pOther->GetName().IsEqual(myName)) aResult = Standard_True;
    }
    return aResult;
}
