...
void IGESBasic_ToolName::ReadOwnParams(const Handle(IGESBasic_Name) & ent,
    ...
    PR.ReadInteger(PR.Current(), "Number of property values",
                   tempNbPropertyValues);        // szv#4:S4163:12Mar99 `st=` not needed
...
void IGESBasic_ToolName::WriteOwnParams(const Handle(IGESBasic_Name) & ent, IGESData_IGESWriter& IW) const {
...
void IGESBasic_ToolName::OwnCopy(const Handle(IGESBasic_Name) & another, const Handle(IGESBasic_Name) & ent,
...
void IGESBasic_ToolName::OwnCheck(const Handle(IGESBasic_Name) & ent, const Interface_ShareTool&,
                                  Handle(Interface_Check) & ach) const {
    if (ent->NbPropertyValues() != 1) ach->AddFail("Number of Property Values != 1");
}

void IGESBasic_ToolName::OwnDump(const Handle(IGESBasic_Name) & ent, const IGESData_IGESDumper& /* dumper */,
