...
void IGESBasic_ToolSingularSubfigure::ReadOwnParams(const Handle(IGESBasic_SingularSubfigure) & ent,
    ...
    if (!PR.ReadEntity(IR, PR.Current(), aStatus, STANDARD_TYPE(IGESBasic_SubfigureDef),
                       tempSubfigureDef)) { // szv#4:S4163:12Mar99 `st=` not needed;
...
IGESData_DirChecker IGESBasic_ToolSingularSubfigure::DirChecker(const Handle(IGESBasic_SingularSubfigure) & ent) const {
    ...
    if (ent->HierarchyStatus() == 1) DC.GraphicsIgnored(01); // GraphicsIgnored if Hierarchy = 01
    return DC;
}
