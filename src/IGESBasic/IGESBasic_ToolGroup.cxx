...
void IGESBasic_ToolGroup::ReadOwnParams(const Handle(IGESBasic_Group) & ent, const Handle(IGESData_IGESReaderData) & IR,
...
void IGESBasic_ToolGroup::WriteOwnParams(const Handle(IGESBasic_Group) & ent, IGESData_IGESWriter& IW) const {
...
void IGESBasic_ToolGroup::OwnShared(const Handle(IGESBasic_Group) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer upper = ent->NbEntities();
    for (Standard_Integer i = 1; i <= upper; i++)
        iter.GetOneItem(ent->Entity(i));
}

void IGESBasic_ToolGroup::OwnCopy(const Handle(IGESBasic_Group) & another, const Handle(IGESBasic_Group) & ent,
...
Standard_Boolean IGESBasic_ToolGroup::OwnCorrect(const Handle(IGESBasic_Group) & ent) const {
...
IGESData_DirChecker IGESBasic_ToolGroup::DirChecker(const Handle(IGESBasic_Group) & /* ent */) const {
...
void IGESBasic_ToolGroup::OwnCheck(const Handle(IGESBasic_Group) & ent, const Interface_ShareTool&,
...
void IGESBasic_ToolGroup::OwnDump(const Handle(IGESBasic_Group) & ent, const IGESData_IGESDumper& dumper,
