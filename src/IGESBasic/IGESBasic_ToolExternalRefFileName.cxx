...
void IGESBasic_ToolExternalRefFileName::ReadOwnParams(const Handle(IGESBasic_ExternalRefFileName) & ent,
...
void IGESBasic_ToolExternalRefFileName::WriteOwnParams(const Handle(IGESBasic_ExternalRefFileName) & ent,
                                                       IGESData_IGESWriter& IW) const {
    IW.Send(ent->FileId());
    IW.Send(ent->ReferenceName());
}

void IGESBasic_ToolExternalRefFileName::OwnShared(const Handle(IGESBasic_ExternalRefFileName) & /* ent */,
                                                  Interface_EntityIterator& /* iter */) const {}

void IGESBasic_ToolExternalRefFileName::OwnCopy(const Handle(IGESBasic_ExternalRefFileName) & another,
    ...
    Handle(TCollection_HAsciiString) tempRefName = new TCollection_HAsciiString(another->ReferenceName());
    ent->Init(tempFileId, tempRefName);
}

IGESData_DirChecker
IGESBasic_ToolExternalRefFileName::DirChecker(const Handle(IGESBasic_ExternalRefFileName) & /* ent */) const {
...
void IGESBasic_ToolExternalRefFileName::OwnCheck(const Handle(IGESBasic_ExternalRefFileName) & ent,
    ...
    if (ent->FormNumber() == 1) ach->AddFail("Invalid Form Number");
}
