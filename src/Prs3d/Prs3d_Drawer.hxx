...
public:
    ...
    Aspect_TypeOfDeflection TypeOfDeflection() const {
        return myHasOwnTypeOfDeflection || myLink.IsNull() ? myTypeOfDeflection : myLink->TypeOfDeflection();
    }

    //! Returns true if the drawer has a type of deflection setting active.
    Standard_Boolean HasOwnTypeOfDeflection() const {
        return myHasOwnTypeOfDeflection;
    }
    ...
    Standard_Real MaximalChordialDeviation() const {
        return myHasOwnChordialDeviation || myLink.IsNull() ? myChordialDeviation : myLink->MaximalChordialDeviation();
    }
    ...
    Standard_Boolean HasOwnMaximalChordialDeviation() const {
        return myHasOwnChordialDeviation;
    }
    ...
    Standard_Boolean HasOwnTypeOfHLR() const {
        return (myTypeOfHLR != Prs3d_TOH_NotSet);
    }
    ...
    Standard_Real MaximalParameterValue() const {
        return myHasOwnMaximalParameterValue || myLink.IsNull() ? myMaximalParameterValue
    ...
    Standard_Boolean HasOwnMaximalParameterValue() const {
        return myHasOwnMaximalParameterValue;
    }
    ...
    Standard_Boolean IsoOnTriangulation() const {
        return myHasOwnIsoOnTriangulation || myLink.IsNull() ? myIsoOnTriangulation : myLink->IsoOnTriangulation();
    }

    //! Returns true if the drawer has IsoOnTriangulation setting active.
    Standard_Boolean HasOwnIsoOnTriangulation() const {
        return myHasOwnIsoOnTriangulation;
    }
    ...
    Standard_Real DeviationCoefficient() const {
        return HasOwnDeviationCoefficient() || myLink.IsNull() ? myDeviationCoefficient
    ...
    Standard_Real DeviationAngle() const {
        return HasOwnDeviationAngle() || myLink.IsNull() ? myDeviationAngle : myLink->DeviationAngle();
    }
    ...
    Standard_Boolean IsAutoTriangulation() const {
        return myHasOwnIsAutoTriangulated || myLink.IsNull() ? myIsAutoTriangulated : myLink->IsAutoTriangulation();
    }

    //! Returns true if the drawer has IsoOnPlane setting active.
    Standard_Boolean HasOwnIsAutoTriangulation() const {
        return myHasOwnIsAutoTriangulated;
    }
    ...
    Standard_Boolean HasOwnHiddenLineAspect() const {
        return myHasOwnHiddenLineAspect;
    }
    ...
    Standard_Boolean HasOwnFreeBoundaryAspect() const {
        return myHasOwnFreeBoundaryAspect;
    }
    ...
    Standard_Boolean FreeBoundaryDraw() const {
        return myHasOwnFreeBoundaryDraw || myLink.IsNull() ? myFreeBoundaryDraw : myLink->FreeBoundaryDraw();
    }

    //! Returns true if the drawer has its own attribute for
    //! "draw free boundaries" flag that overrides the one in the link.
    Standard_Boolean HasOwnFreeBoundaryDraw() const {
        return myHasOwnFreeBoundaryDraw;
    }
    ...
    Standard_Boolean HasOwnUnFreeBoundaryAspect() const {
        return myHasOwnUnFreeBoundaryAspect;
    }
    ...
    Standard_Boolean UnFreeBoundaryDraw() const {
        return myHasOwnUnFreeBoundaryDraw || myLink.IsNull() ? myUnFreeBoundaryDraw : myLink->UnFreeBoundaryDraw();
    }

    //! Returns true if the drawer has its own attribute for
    //! "draw shared boundaries" flag that overrides the one in the link.
    Standard_Boolean HasOwnUnFreeBoundaryDraw() const {
        return myHasOwnUnFreeBoundaryDraw;
    }
    ...
    Standard_Boolean HasOwnFaceBoundaryAspect() const {
        return myHasOwnFaceBoundaryAspect;
    }
    ...
    Standard_Boolean FaceBoundaryDraw() const {
        return myHasOwnFaceBoundaryDraw || myLink.IsNull() ? myFaceBoundaryDraw : myLink->FaceBoundaryDraw();
    }
    ...
    //! Returns true if the drawer has its own attribute for face boundaries upper edge continuity class that overrides
    //! the one in the link.
    ...
    Standard_Boolean HasOwnDimensionAspect() const {
        return myHasOwnDimensionAspect;
    }
    ...
    const TCollection_AsciiString& DimLengthDisplayUnits() const {
        return myHasOwnDimLengthDisplayUnits || myLink.IsNull() ? myDimensionDisplayUnits.GetLengthUnits()
...
public:
    ...
    //! @param theToOverrideDefaults if true then non-overridden attributes using defaults will be allocated and copied
    //! from the Link;
    //!                              otherwise, only already customized attributes will be changed
    //! @return TRUE if presentation should be recomputed after creating aspects not previously customized (if
    //! theToOverrideDefaults is also TRUE)
    ...
    Standard_EXPORT bool SetShadingModel(Graphic3d_TypeOfShadingModel theModel, bool theToOverrideDefaults = false);
...
public: //! @name deprecated methods
    Standard_DEPRECATED("SetDeviationAngle() should be used instead") void SetHLRAngle(const Standard_Real theAngle) {
        SetDeviationAngle(theAngle);
    }

    Standard_DEPRECATED("DeviationAngle() should be used instead") Standard_Real HLRAngle() const {
        return DeviationAngle();
    }

    Standard_DEPRECATED("SetDeviationAngle() should be used instead") void SetHLRAngle() {
        SetDeviationAngle();
    }

    Standard_DEPRECATED("HasOwnDeviationAngle() should be used instead") Standard_Boolean
        HasOwnHLRDeviationAngle() const {
        return HasOwnDeviationAngle();
    }

    Standard_DEPRECATED("PreviousDeviationAngle() should be used instead") Standard_Real
        PreviousHLRDeviationAngle() const {
        return PreviousDeviationAngle();
    }
...
protected:
    ...
    Standard_Integer myFaceBoundaryUpperContinuity; //!< the most edge continuity class (GeomAbs_Shape) to be included
                                                    //!< to face boundaries presentation, or -1 if undefined
