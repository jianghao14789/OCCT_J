...
Prs3d_Drawer::Prs3d_Drawer()
    : myNbPoints(30), myHasOwnNbPoints(Standard_False), myMaximalParameterValue(500000.),
      myHasOwnMaximalParameterValue(Standard_False), myChordialDeviation(0.0001),
      myHasOwnChordialDeviation(Standard_False), myTypeOfDeflection(Aspect_TOD_RELATIVE),
      myHasOwnTypeOfDeflection(Standard_False), myTypeOfHLR(Prs3d_TOH_NotSet), myDeviationCoefficient(0.001),
      myHasOwnDeviationCoefficient(Standard_False), myDeviationAngle(20.0 * M_PI / 180.0),
      myHasOwnDeviationAngle(Standard_False), myIsoOnPlane(Standard_False), myHasOwnIsoOnPlane(Standard_False),
      myIsoOnTriangulation(Standard_False), myHasOwnIsoOnTriangulation(Standard_False),
      myIsAutoTriangulated(Standard_True), myHasOwnIsAutoTriangulated(Standard_False),

      myHasOwnUIsoAspect(Standard_False), myHasOwnVIsoAspect(Standard_False), myHasOwnWireAspect(Standard_False),
      myWireDraw(Standard_True), myHasOwnWireDraw(Standard_False), myHasOwnPointAspect(Standard_False),
      myHasOwnLineAspect(Standard_False), myHasOwnTextAspect(Standard_False), myHasOwnShadingAspect(Standard_False),
      myHasOwnPlaneAspect(Standard_False), myHasOwnSeenLineAspect(Standard_False), myHasOwnArrowAspect(Standard_False),
      myLineArrowDraw(Standard_False), myHasOwnLineArrowDraw(Standard_False), myHasOwnHiddenLineAspect(Standard_False),
      myDrawHiddenLine(Standard_False), myHasOwnDrawHiddenLine(Standard_False), myHasOwnVectorAspect(Standard_False),
      myVertexDrawMode(Prs3d_VDM_Inherited), myHasOwnDatumAspect(Standard_False), myHasOwnSectionAspect(Standard_False),

      myHasOwnFreeBoundaryAspect(Standard_False), myFreeBoundaryDraw(Standard_True),
      myHasOwnFreeBoundaryDraw(Standard_False), myHasOwnUnFreeBoundaryAspect(Standard_False),
      myUnFreeBoundaryDraw(Standard_True), myHasOwnUnFreeBoundaryDraw(Standard_False),
      myFaceBoundaryUpperContinuity(-1), myHasOwnFaceBoundaryAspect(Standard_False), myFaceBoundaryDraw(Standard_False),
      myHasOwnFaceBoundaryDraw(Standard_False),

      myHasOwnDimensionAspect(Standard_False), myHasOwnDimLengthModelUnits(Standard_False),
      myHasOwnDimAngleModelUnits(Standard_False), myHasOwnDimLengthDisplayUnits(Standard_False),
...
Standard_Boolean Prs3d_Drawer::SetOwnDatumAspects(const Handle(Prs3d_Drawer) & theDefaults) {
    ...
    if (!myHasOwnDatumAspect) {
        isUpdateNeeded = true;
        myDatumAspect = new Prs3d_DatumAspect();
        if (!aLink.IsNull() && aLink->HasOwnDatumAspect()) {
...
template <typename T>
inline void setAspectProgram(const Handle(Graphic3d_ShaderProgram) & theProgram, bool theHasAspect, T thePrsAspect) {
    if (!thePrsAspect.IsNull() && theHasAspect) {
...
bool Prs3d_Drawer::SetShaderProgram(const Handle(Graphic3d_ShaderProgram) & theProgram,
                                    const Graphic3d_GroupAspect theAspect, const bool theToOverrideDefaults) {
    bool isUpdateNeeded = false;
    switch (theAspect) {
...
// purpose  :
// =======================================================================
bool Prs3d_Drawer::SetShadingModel(Graphic3d_TypeOfShadingModel theModel, bool theToOverrideDefaults) {
    bool isUpdateNeeded = false;
    ...
    if (!myShadingAspect.IsNull() && myHasOwnShadingAspect) {
