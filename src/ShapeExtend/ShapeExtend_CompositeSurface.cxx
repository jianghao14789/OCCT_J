...
Standard_Boolean ShapeExtend_CompositeSurface::SetUJointValues(const TColStd_Array1OfReal& UJoints) {
    ...
    for (Standard_Integer i = 1, j = UJoints.Lower(); i <= NbU + 1; i++, j++) {
        ...
        if (i > 1 && UJoints(j) - UJoints(j - 1) < Precision::PConfusion()) return Standard_False;
    }
...
Standard_Boolean ShapeExtend_CompositeSurface::SetVJointValues(const TColStd_Array1OfReal& VJoints) {
    ...
    for (Standard_Integer i = 1, j = VJoints.Lower(); i <= NbV + 1; i++, j++) {
        ...
        if (i > 1 && VJoints(j) - VJoints(j - 1) < Precision::PConfusion()) return Standard_False;
    }
...
void ShapeExtend_CompositeSurface::LocateUVPoint(const gp_Pnt2d& pnt, Standard_Integer& i, Standard_Integer& j) const {
...
const Handle(Geom_Surface) & ShapeExtend_CompositeSurface::Patch(const Standard_Real U, const Standard_Real V) const {
...
Standard_Real ShapeExtend_CompositeSurface::ULocalToGlobal(const Standard_Integer i, const Standard_Integer j,
...
Standard_Real ShapeExtend_CompositeSurface::VLocalToGlobal(const Standard_Integer i, const Standard_Integer j,
...
gp_Pnt2d ShapeExtend_CompositeSurface::LocalToGlobal(const Standard_Integer i, const Standard_Integer j,
    ...
    return gp_Pnt2d(uv.X() * scaleu +
                        (myUJointValues->Value(i) - u1 * scaleu), // ! this formula is stable if u1 or v1 is infinite
...
Standard_Real ShapeExtend_CompositeSurface::UGlobalToLocal(const Standard_Integer i, const Standard_Integer j,
                                                           const Standard_Real U) const {
    Standard_Real u1, u2, v1, v2;
    myPatches->Value(i, j)->Bounds(u1, u2, v1, v2);
...
Standard_Real ShapeExtend_CompositeSurface::VGlobalToLocal(const Standard_Integer i, const Standard_Integer j,
                                                           const Standard_Real V) const {
    Standard_Real u1, u2, v1, v2;
    myPatches->Value(i, j)->Bounds(u1, u2, v1, v2);
...
gp_Pnt2d ShapeExtend_CompositeSurface::GlobalToLocal(const Standard_Integer i, const Standard_Integer j,
                                                     const gp_Pnt2d& UV) const {
    Standard_Real u1, u2, v1, v2;
    myPatches->Value(i, j)->Bounds(u1, u2, v1, v2);
    ...
    return gp_Pnt2d(UV.X() * scaleu +
                        (u1 - myUJointValues->Value(i) * scaleu), // ! this formula is stable if u1 or v1 is infinite
...
Handle(Geom_Geometry) ShapeExtend_CompositeSurface::Copy() const {
    ...
    Handle(TColGeom_HArray2OfSurface) patches = new TColGeom_HArray2OfSurface(1, NbUPatches(), 1, NbVPatches());
    for (Standard_Integer i = 1; i <= NbUPatches(); i++)
        for (Standard_Integer j = 1; j <= NbVPatches(); j++)
...
void ShapeExtend_CompositeSurface::Bounds(Standard_Real& U1, Standard_Real& U2, Standard_Real& V1,
...
void ShapeExtend_CompositeSurface::D1(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U,
                                      gp_Vec& D1V) const {
    Standard_Integer i = LocateUParameter(U);
    Standard_Integer j = LocateVParameter(V);
    gp_Pnt2d uv = GlobalToLocal(i, j, gp_Pnt2d(U, V));
...
void ShapeExtend_CompositeSurface::D2(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
...
void ShapeExtend_CompositeSurface::D3(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
...
gp_Vec ShapeExtend_CompositeSurface::DN(const Standard_Real U, const Standard_Real V, const Standard_Integer Nu,
...
static void GetLimitedBounds(const Handle(Geom_Surface) & surf, Standard_Real& U1, Standard_Real& U2, Standard_Real& V1,
                             Standard_Real& V2) {
