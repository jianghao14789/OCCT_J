...
ShapeExtend_WireData::ShapeExtend_WireData(const TopoDS_Wire& wire, const Standard_Boolean chained,
...
Standard_Boolean ShapeExtend_WireData::Init(const TopoDS_Wire& wire, const Standard_Boolean chained,
                                            const Standard_Boolean theManifold) {
    Clear();
...
void ShapeExtend_WireData::Add(const TopoDS_Edge& edge, const Standard_Integer atnum) {
    if (edge.Orientation() != TopAbs_REVERSED && edge.Orientation() != TopAbs_FORWARD && myManifoldMode) {
...
void ShapeExtend_WireData::Add(const Handle(ShapeExtend_WireData) & wire, const Standard_Integer atnum) {
    if (wire.IsNull()) return;
    TopTools_SequenceOfShape aNMEdges;
    Standard_Integer n = atnum;
    Standard_Integer i = 1;
...
Standard_Integer ShapeExtend_WireData::Index(const TopoDS_Edge& edge) {
    for (Standard_Integer i = 1; i <= NbEdges(); i++)
        if (Edge(i).IsSame(edge) && (Edge(i).Orientation() == edge.Orientation() || !IsSeam(i))) return i;
    return 0;
}
...
TopoDS_Wire ShapeExtend_WireData::Wire() const {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (aE.Orientation() != TopAbs_FORWARD && aE.Orientation() != TopAbs_REVERSED) ismanifold = Standard_False;
        B.Add(W, aE);
    }
    if (ismanifold) {
        TopoDS_Vertex vf, vl;
