...
Standard_Integer DNaming_Line3DDriver::Execute(Handle(TFunction_Logbook) & theLog) const {
    ...
    for (aCounter = 1; aCounter <= aLength - 1; aCounter++) {
        ...
        if (!aNS2->IsEmpty()) {
            ...
            (void)aDebPoint;
            //      std::cout << aCounter+1 << " X = " <<  aDebPoint.X() << " Y = " <<  aDebPoint.Y() << " Z = " <<
            //      aDebPoint.Z() << std::endl;
        } else
        ...
        //      std::cout << aCounter   << " X = " <<  aDebPoint1.X() << " Y = " <<  aDebPoint1.Y() << " Z = " <<
        //      aDebPoint1.Z() << std::endl; std::cout << aCounter+1 << " X = " <<  aDebPoint2.X() << " Y = " <<
        //      aDebPoint2.Y() << " Z = " <<  aDebPoint2.Z() << std::endl;
        (void)aDebPoint1;
        (void)aDebPoint2;
#endif
    ...
    // restore location
    if (!aLocation.IsIdentity()) TNaming::Displace(aResultLabel, aLocation, Standard_True);
...
//=======================================================================
void DNaming_Line3DDriver::LoadNamingDS(const TDF_Label& theResultLabel, const TopoDS_Wire& theWire,
                                        const TopTools_Array1OfShape& theArV, const Standard_Boolean isClosed) const {
    ...
    if (isClosed) {
        ...
        if (!aFound) anArE.SetValue(aLength, aShape);
    } else {
        // open
    ...
    for (Standard_Integer i1 = 1; i1 <= aLength; i1++) {
        ...
        if (!anArE.Value(i1).IsNull()) {
            ...
            if (aLab.FindAttribute(TNaming_NamedShape::GetID(), aNS)) TNaming_Builder aB(aLab);
        }
    }
    // put vertexes
    ...
    if (!aFirst.IsNull()) {
        ...
        Handle(TNaming_NamedShape) aNS;
        if (aLab1.FindAttribute(TNaming_NamedShape::GetID(), aNS)) TNaming_Builder aB(aLab1);
    }
    if (!aLast.IsNull()) {
        ...
        Handle(TNaming_NamedShape) aNS;
        if (aLab2.FindAttribute(TNaming_NamedShape::GetID(), aNS)) TNaming_Builder aB(aLab2);
    }
}
