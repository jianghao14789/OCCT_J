...
static Standard_Integer DNaming_CheckHasSame(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb < 4) return 1;
    ...
    for (; itr1.More(); itr1.Next()) {
        ...
        for (itr2.Initialize(M2); itr2.More(); itr2.Next()) {
            ...
            if (s1.IsSame(s2)) di << "Shapes " << arg[1] << " and " << arg[2] << " have SAME subshapes\n";
        }
    }
...
static Standard_Integer DNaming_TCopyShape(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
...
static Standard_Integer DNaming_TCopyTool(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb < 2) {
...
void DNaming::ToolsCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("CheckSame", "CheckSame (Shape1 Shape2 ExploMode[F|E|V])", __FILE__, DNaming_CheckHasSame, g);
}
