...
Standard_Boolean FillValidMap(const TDF_Label& theLabel, TDF_LabelMap& theValidMap) {
    ...
    for (; itr.More(); itr.Next()) {
        ...
        if (!aLabel.IsNull()) aLabel.FindAttribute(TNaming_Naming::GetID(), aNaming);
        ...
        for (TDF_MapIteratorOfAttributeMap attMItr(anExtMap); attMItr.More(); attMItr.Next()) {
            ...
            std::cout << "## References attribute dynamic type = " << att->DynamicType() << " at Label = " << entr1
                      << std::endl;
#endif
            if (att->Label().IsDifferent(aLabel) && !att->Label().IsDescendant(theLabel)) {
                ...
                if (!aNS.IsNull()) TNaming_NamingTool::BuildDescendants(aNS, theValidMap);
...
static Standard_Integer DNaming_SolveSelection(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n == 3) {
        ...
        if (!FillValidMap(L, aValidMap)) di << "Valid map is empty\n";
#ifdef OCCT_DEBUG_SELN
        ...
        if (!isSolved) di << "!!! Solver is failed\n";
...
//=======================================================================
static Standard_Integer DNaming_DumpSelection(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n == 3 || n == 4) {
        Handle(TDF_Data) DF;
        if (!DDF::GetDF(a[1], DF)) return 1;
        TDF_Label L;
...
//=======================================================================
static Standard_Integer DNaming_ArgsSelection(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n == 3) {
        Handle(TDF_Data) DF;
        if (!DDF::GetDF(a[1], DF)) return 1;
        TDF_Label L;
...
static void CollectAttachment(const TDF_Label& root, const Handle(TNaming_Naming) & naming,
...
void DNaming::SelectionCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("SelectShape", "SelectShape DF entry shape [context [Orient]]", __FILE__, DNaming_Select, g);

    theCommands.Add("SelectGeometry", "SelectGeometry DF entry shape [context]", __FILE__, DNaming_Select, g);
