...
Standard_Integer DNaming_RevolutionDriver::Execute(Handle(TFunction_Logbook) & theLog) const {
    ...
    if (aBasis.ShapeType() == TopAbs_WIRE) {
        ...
        if (aCheck->Closed(Standard_True) == BRepCheck_NoError) {
            ...
            if (aMaker.IsDone()) aBASIS = aMaker.Face();                         // aMaker.Face();
        }
    ...
    if (aFunction->GetDriverGUID() == FULREVOL_GUID) {
        ...
        if (aResult.ShapeType() == TopAbs_SOLID)
            ...
            if (aCheck->Closed() == BRepCheck_NoError) aVol = Standard_True;
        }
        if (aVol) {
            GProp_GProps aGProp;
    ...
    // restore location
    if (!aLocation.IsIdentity()) TNaming::Displace(RESPOSITION(aFunction), aLocation, Standard_True);
...
//=======================================================================
static void LoadSeamEdge(BRepPrimAPI_MakeRevol& mkRevol, const TDF_Label& ResultLabel, const TopoDS_Shape& ShapeIn)

{
    ...
    for (; ShapeExplorer.More(); ShapeExplorer.Next()) {
        ...
        for (; ShapesIterator.More(); ShapesIterator.Next()) {
            ...
            if (newShape.ShapeType() != TopAbs_FACE) continue;
...
// purpose  :
//=======================================================================
void DNaming_RevolutionDriver::LoadNamingDS(const TDF_Label& theResultLabel, BRepPrimAPI_MakeRevol& MS,
                                            const TopoDS_Shape& Basis, const TopoDS_Shape& Context) const {
    ...
    if (!StartShape.IsNull() && !EndShape.IsNull()) isFull = StartShape.IsEqual(EndShape);
            ...
            if (!isBasisClosed) {
                ...
                for (; it.More(); it.Next()) {
                    if (!BRep_Tool::Degenerated(TopoDS::Edge(it.Value()))) {
                        ...
                        if (MS.HasDegenerated()) std::cout << "mkRevol has degenerated" << std::endl;
                        std::cout << "BRep_Tool found degenerated edge (from Vfirst) TS = " << it.Value().TShape().get()
                                  << std::endl;
                    }
#endif
                }
                ...
                for (; it.More(); it.Next()) {
                    if (!BRep_Tool::Degenerated(TopoDS::Edge(it.Value()))) {
                        ...
                        if (MS.HasDegenerated()) std::cout << "mkRevol has degenerated" << std::endl;
                        std::cout << "BRep_Tool found degenerated edge (from Vlast) TS = " << it.Value().TShape().get()
                                  << std::endl;
                    }
#endif
                }
