...
static Standard_Boolean FixSameParameter(const TopoDS_Shape& theShape, BRepCheck_Analyzer& theAnalyzer,
                                         const Standard_Boolean bIgnoreNotSPErrors = Standard_False);
static void FindSPErrorEdges(const TopoDS_Shape& theShape, const BRepCheck_Analyzer& theAnalyzer,
                             TopTools_IndexedMapOfShape& theMap);

static Standard_Boolean FindOtherErrors(const TopoDS_Shape& theShape, const BRepCheck_Analyzer& theAnalyzer,
...
static Standard_Boolean IsValidSurfType(const TopoDS_Face& theFace) {
    ...
    if (aType == GeomAbs_Sphere) return Standard_True;
...
static Standard_Boolean IsWRCase(const BRepAlgoAPI_BooleanOperation& MS) {
    ...
    if (Type1 != TopAbs_FACE) {
        ...
        for (; anExp.More(); anExp.Next()) {
            if (IsValidSurfType(TopoDS::Face(anExp.Current()))) aList.Append(anExp.Current());
        }
...
void DNaming_BooleanOperationDriver::LoadNamingDS(const TDF_Label& theResultLabel,
    ...
    if (IsWRCase(MS)) {
        ...
        if (aList.Extent() > 0 && aList.Extent() < 3) theCase = Standard_True;

        TopTools_ListIteratorOfListOfShape it(aList);
        for (; it.More(); it.Next()) {
            TopoDS_Shape newShape = it.Value();
            if (SubShapes.IsBound(newShape)) newShape.Orientation((SubShapes(newShape)).Orientation());
...
//=======================================================================
Standard_Boolean DNaming_BooleanOperationDriver::CheckAndLoad(BRepAlgoAPI_BooleanOperation& theMkOpe,
    ...
    if (theMkOpe.IsDone() && !theMkOpe.Shape().IsNull()) {
        ...
        if (!aCheck.IsValid(theMkOpe.Shape())) aResIsValid = FixSameParameter(theMkOpe.Shape(), aCheck);
        if (aResIsValid) {
...
// purpose:
// ------------------------------------------------------------------------
Standard_Boolean FixSameParameter(const TopoDS_Shape& theShape, BRepCheck_Analyzer& theAnalyzer,
...
// purpose:
// ------------------------------------------------------------------------
void FindSPErrorEdges(const TopoDS_Shape& theShape, const BRepCheck_Analyzer& theAnalyzer,
                      TopTools_IndexedMapOfShape& theMap) {
    ...
    if (theShape.ShapeType() == TopAbs_FACE) {
        ...
        for (; anExpE.More(); anExpE.Next()) {
            ...
            if (aResult.IsNull() || theMap.Contains(anExpE.Current())) continue;

            for (aResult->InitContextIterator(); aResult->MoreShapeInContext(); aResult->NextShapeInContext()) {
...
// purpose:
// ------------------------------------------------------------------------
Standard_Boolean FindOtherErrors(const TopoDS_Shape& theShape, const BRepCheck_Analyzer& theAnalyzer,
                                 const TopTools_IndexedMapOfShape& theMap) {
    ...
    for (; anIt.More(); anIt.Next()) {
        if (FindOtherErrors(anIt.Value(), theAnalyzer, theMap)) return Standard_True;
    }
    Handle(BRepCheck_Result) aResult = theAnalyzer.Result(theShape);
    ...
    if (!aResult.IsNull()) {
        ...
        if (!theMap.Contains(theShape) && !aResult->Status().IsEmpty()) {
            ...
            for (; itl.More(); itl.Next()) {
                if ((itl.Value() != BRepCheck_InvalidSameParameterFlag) &&
                    (itl.Value() != BRepCheck_InvalidCurveOnSurface) && (itl.Value() != BRepCheck_NoError)) {
                    return Standard_True;
                }
            }
        }
