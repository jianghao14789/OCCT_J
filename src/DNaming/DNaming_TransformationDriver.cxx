...
Standard_Integer DNaming_TransformationDriver::Execute(Handle(TFunction_Logbook) & theLog) const {
    ...
    try {
        if (aGUID == PTXYZ_GUID) {
            ...
            if (aNS.IsNull() || aNS->IsEmpty() || aNS->Get().IsNull() || aNS->Get().ShapeType() != TopAbs_FACE)
...
//=================================================================================
static void BuildMap(const TopTools_MapOfShape& SMap, BRepBuilderAPI_Transform& Transformer,
...
//=================================================================================
static void CollectShapes(const TopoDS_Shape& SSh, TopoDS_Compound& C, TopTools_MapOfShape& SMap,
                          const TDF_Label& theLab, TopTools_DataMapOfShapeInteger& TagMap,
        ...
        case TopAbs_SHELL: {
            ...
            for (; anEx.More(); anEx.Next()) {
                ...
                if (SMap.Add(anEx.Current())) {
                    ...
                    if (isPrimitive) TagMap.Bind(anEx.Current(), aNS->Label().Tag());
                }
            }
...
void DNaming_TransformationDriver::LoadNamingDS(const TDF_Label& theResultLabel,
    ...
    if (theSourceNS.IsNull() || theSourceNS->IsEmpty()) return;
    const TopoDS_Shape& aSrcShape = theSourceNS->Get();
    if (aSrcShape.IsNull()) {
#ifdef OCCT_DEBUG
        std::cout << "DNaming_TransformationDriver::LoadNamingDS: The result of the Transform operation is null"
                  << std::endl;
#endif
        return;
    }
    ...
    // Collect  shapes
    if (aSMap.Add(aSrcShape)) aB.Add(aCompShape, aSrcShape);
    ...
    for (; it.More(); it.Next()) {
        if (it.Value() > aNextTag) aNextTag = it.Value();
    }
