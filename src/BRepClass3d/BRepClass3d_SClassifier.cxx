...
static Standard_Boolean FaceNormal(const TopoDS_Face& aF, const Standard_Real U, const Standard_Real V, gp_Dir& aDN);
...
static Standard_Boolean GetNormalOnFaceBound(const TopoDS_Edge& E, const TopoDS_Face& F, Standard_Real param,
                                             gp_Dir& OutDir);
...
// purpose  :
//=======================================================================
BRepClass3d_SClassifier::BRepClass3d_SClassifier() : myState(0) {}

//=======================================================================
// function : BRepClass3d_SClassifier
// purpose  :
//=======================================================================
BRepClass3d_SClassifier::BRepClass3d_SClassifier(BRepClass3d_SolidExplorer& S, const gp_Pnt& P,
...
// purpose  :
//=======================================================================
void BRepClass3d_SClassifier::PerformInfinitePoint(BRepClass3d_SolidExplorer& aSE, const Standard_Real /*Tol*/) {
    ...
    for (Standard_Integer itry = 0; itry < NB_MAX_POINTS_PER_FACE; itry++) {
        for (std::vector<TopoDS_Face>::iterator iFace = aFaces.begin(); iFace != aFaces.end(); ++iFace) {
            ...
            if (!bFound || !FaceNormal(aF, aU, aV, aDN)) continue;
                            ...
                            if (Intersector3d.IsDone()) {
                                if (Intersector3d.NbPnt()) {
                                    ...
                                    for (Standard_Integer i = 2; i <= Intersector3d.NbPnt(); i++)
                                        if (Intersector3d.WParameter(i) < Intersector3d.WParameter(imin)) imin = i;
...
// purpose  :
//=======================================================================
void BRepClass3d_SClassifier::Perform(BRepClass3d_SolidExplorer& SolidExplorer, const gp_Pnt& P,
                                      const Standard_Real Tol) {
    ...
    while (isFaultyLine) {
        ...
        if (SelsEVL > 0) {
            ...
            for (Standard_Integer i = 1; i <= VLInterNb; i++) {
                ...
                LVInts.Add(V);
                if (Abs(LP) < Abs(NearFaultPar)) NearFaultPar = LP;
            }
        ...
        for (SolidExplorer.InitShell(); SolidExplorer.MoreShell() && !isFaultyLine; SolidExplorer.NextShell()) {
            if (SolidExplorer.RejectShell(L) == Standard_False) {
                for (SolidExplorer.InitFace(); SolidExplorer.MoreFace() && !isFaultyLine; SolidExplorer.NextFace()) {
...
// purpose  :
//=======================================================================
Standard_Boolean FaceNormal(const TopoDS_Face& aF, const Standard_Real U, const Standard_Real V, gp_Dir& aDN) {
    gp_Pnt aPnt;
    gp_Vec aD1U, aD1V, aN;
    ...
    if (aN.Magnitude() <= gp::Resolution()) return Standard_False;
...
// purpose  :
//=======================================================================
static Standard_Boolean GetNormalOnFaceBound(const TopoDS_Edge& E, const TopoDS_Face& F, const Standard_Real param,
                                             gp_Dir& OutDir) {
    ...
    c2d->D0(param, P2d);
    if (!FaceNormal(F, P2d.X(), P2d.Y(), OutDir)) return Standard_False;
    return Standard_True;
}
...
//=======================================================================
static Standard_Integer GetTransi(const TopoDS_Face& f1, const TopoDS_Face& f2, const TopoDS_Edge e,
                                  const Standard_Real param, const gp_Lin& L,
    ...
    gp_Dir nf1, nf2;
    if (!GetNormalOnFaceBound(e, f1, param, nf1)) return -1;
    if (!GetNormalOnFaceBound(e, f2, param, nf2)) return -1;
...
static void Trans(const Standard_Real parmin, IntCurveSurface_TransitionOnCurve& tran, int& state) {
    ...
    if (parmin < 0) tran = (tran == IntCurveSurface_Out ? IntCurveSurface_In : IntCurveSurface_Out);
