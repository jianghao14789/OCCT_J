...
Handle(Transfer_ActorOfTransientProcess) XSControl_TransferReader::Actor() {
    if (myActor.IsNull() && !myController.IsNull() && !myModel.IsNull()) myActor = myController->ActorRead(myModel);
    return myActor;
}
...
void XSControl_TransferReader::SetContext(const Standard_CString name, const Handle(Standard_Transient) & ctx) {
...
Standard_Boolean XSControl_TransferReader::GetContext(const Standard_CString name, const Handle(Standard_Type) & type,
                                                      Handle(Standard_Transient) & ctx) const {
    if (myContext.IsEmpty()) return Standard_False;
    if (!myContext.Find(name, ctx)) ctx.Nullify();
...
Handle(Transfer_ResultFromModel) XSControl_TransferReader::FinalResult(const Handle(Standard_Transient) & ent) const {
...
Handle(Transfer_ResultFromModel) XSControl_TransferReader::ResultFromNumber(const Standard_Integer num) const {
    Handle(Transfer_ResultFromModel) res;
...
Handle(Standard_Transient) XSControl_TransferReader::TransientResult(const Handle(Standard_Transient) & ent) const {
...
Handle(Standard_Transient) XSControl_TransferReader::EntityFromResult(const Handle(Standard_Transient) & res,
...
Handle(Standard_Transient) XSControl_TransferReader::EntityFromShapeResult(const TopoDS_Shape& res,
                                                                           const Standard_Integer mode) const {
...
Handle(TColStd_HSequenceOfTransient) XSControl_TransferReader::EntitiesFromShapeList(
    const Handle(TopTools_HSequenceOfShape) & res, const Standard_Integer mode) const {
...
Handle(TColStd_HSequenceOfTransient) XSControl_TransferReader::CheckedList(const Handle(Standard_Transient) & ent,
...
Standard_Integer XSControl_TransferReader::TransferRoots(const Interface_Graph& G,
    ...
    if (theProgress.UserBreak()) return -1;
...
void XSControl_TransferReader::TransferClear(const Handle(Standard_Transient) & ent, const Standard_Integer level) {
...
void XSControl_TransferReader::PrintStats(Standard_OStream& sout, const Standard_Integer what,
                                          const Standard_Integer mode) const {
    //  A ameliorer ... !
    sout << "\n*******************************************************************\n";
...
Handle(TColStd_HSequenceOfTransient) XSControl_TransferReader::LastTransferList(const Standard_Boolean roots) const {
    Handle(TColStd_HSequenceOfTransient) li = new TColStd_HSequenceOfTransient();
...
const Handle(TopTools_HSequenceOfShape) & XSControl_TransferReader::ShapeResultList(const Standard_Boolean rec) {
    if (!rec) {
...
// purpose  :
//=======================================================================
static void PrintPercent(const Handle(Message_Messenger) & sout, const Standard_CString mess, const Standard_Integer nb,
...
void XSControl_TransferReader::PrintStatsProcess(const Handle(Transfer_TransientProcess) & TP,
                                                 const Standard_Integer what, const Standard_Integer mode) {
...
void XSControl_TransferReader::PrintStatsOnList(const Handle(Transfer_TransientProcess) & TP,
    ...
    if (what >= 1 && what <= 3) {
        ...
        if (!nolist) sout << "****        Nb Items (Listed)   : " << nl << std::endl;
