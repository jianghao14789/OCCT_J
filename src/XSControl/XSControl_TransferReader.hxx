...
public:
    ...
    Standard_EXPORT Standard_Boolean GetContext(const Standard_CString theName, const Handle(Standard_Type) & theType,
    ...
    Standard_EXPORT Handle(Standard_Transient)
        EntityFromResult(const Handle(Standard_Transient) & theRes, const Standard_Integer theMode = 0) const;

    //! Returns an entity from which a given shape result was produced
    //! Returns a Null Handle if <res> not recorded or not a Shape
    Standard_EXPORT Handle(Standard_Transient)
        EntityFromShapeResult(const TopoDS_Shape& theRes, const Standard_Integer theMode = 0) const;
    ...
    Standard_EXPORT Handle(TColStd_HSequenceOfTransient)
        EntitiesFromShapeList(const Handle(TopTools_HSequenceOfShape) & theRes,
    ...
    Standard_EXPORT Handle(TColStd_HSequenceOfTransient)
        CheckedList(const Handle(Standard_Transient) & theEnt,
    ...
    Standard_EXPORT void PrintStats(Standard_OStream& theStream, const Standard_Integer theWhat,
                                    const Standard_Integer theMode = 0) const;
    ...
    Standard_EXPORT static void PrintStatsProcess(const Handle(Transfer_TransientProcess) & theTP,
                                                  const Standard_Integer theWhat, const Standard_Integer theMode = 0);
