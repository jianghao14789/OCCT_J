...
void XSControl_Controller::Record(const Standard_CString theName) const {
    if (listad.IsBound(theName)) {
        ...
        if (newadapt->IsKind(thisadapt->DynamicType())) return;
...
Handle(Transfer_ActorOfTransientProcess)
    XSControl_Controller::ActorRead(const Handle(Interface_InterfaceModel) &) const {
...
void XSControl_Controller::SetModeWrite(const Standard_Integer modemin, const Standard_Integer modemax,
...
void XSControl_Controller::SetModeWriteHelp(const Standard_Integer modetrans, const Standard_CString help,
                                            const Standard_Boolean) {
    if (myModeWriteShapeN.IsNull()) return;
    if (modetrans < myModeWriteShapeN->Lower() || modetrans > myModeWriteShapeN->Upper()) return;
...
Standard_Boolean XSControl_Controller::ModeWriteBounds(Standard_Integer& modemin, Standard_Integer& modemax,
...
Standard_Boolean XSControl_Controller::IsModeWrite(const Standard_Integer modetrans, const Standard_Boolean) const {
...
Standard_CString XSControl_Controller::ModeWriteHelp(const Standard_Integer modetrans, const Standard_Boolean) const {
    if (myModeWriteShapeN.IsNull()) return "";
...
Standard_Boolean XSControl_Controller::RecognizeWriteTransient(const Handle(Standard_Transient) & obj,
...
static IFSelect_ReturnStatus
TransferFinder(const Handle(Transfer_ActorOfFinderProcess) & theActor, const Handle(Transfer_Finder) & theMapper,
               const Handle(Transfer_FinderProcess) & theFP, const Handle(Interface_InterfaceModel) & theModel,
               const Standard_Integer theModeTrans, const Message_ProgressRange& theProgress) {
...
IFSelect_ReturnStatus XSControl_Controller::TransferWriteTransient(const Handle(Standard_Transient) & theObj,
    ...
    return TransferFinder(myAdaptorWrite, new Transfer_TransientMapper(theObj), theFP, theModel, theModeTrans,
                          theProgress);
}
...
IFSelect_ReturnStatus XSControl_Controller::TransferWriteShape(const TopoDS_Shape& shape,
    ...
    IFSelect_ReturnStatus theReturnStat =
        TransferFinder(myAdaptorWrite, new TransferBRep_ShapeMapper(shape), FP, model, modetrans, theProgress);
...
void XSControl_Controller::AddSessionItem(const Handle(Standard_Transient) & theItem, const Standard_CString theName,
                                          const Standard_Boolean toApply) {
    if (theItem.IsNull() || theName[0] == '\0') return;
    myAdaptorSession.Bind(theName, theItem);
    if (toApply && theItem->IsKind(STANDARD_TYPE(IFSelect_GeneralModifier))) myAdaptorApplied.Append(theItem);
}
...
Handle(Standard_Transient) XSControl_Controller::SessionItem(const Standard_CString theName) const {
    ...
    if (!myAdaptorSession.IsEmpty()) item = myAdaptorSession.Find(theName);
    return item;
}
