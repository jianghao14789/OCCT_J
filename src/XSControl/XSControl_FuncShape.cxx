...
static Standard_Boolean XSControl_IsEqualSubShape(const TopoDS_Shape& Shape, TopoDS_Shape& sh,
...
static IFSelect_ReturnStatus XSControl_fromshape(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (argc >= 3) aLevel = atoi(pilot->Arg(2));
...
static IFSelect_ReturnStatus XSControl_trconnexentities(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    Handle(TColStd_HSequenceOfTransient) list = XSControl_ConnectedShapes::AdjacentEntities(Shape, TP, TopAbs_FACE);
...
void XSControl_FuncShape::Init() {
    ...
    IFSelect_Act::AddFunc("tpdraw",
                          "[mode:item or root]  num|*  [nomvar] Passes an ITEM to Shape Draw (Start or Result)",
                          XSControl_tpdraw);

    IFSelect_Act::AddFunc("tpcompound", "name:cstring [givelist] : -> compound with Shapes Root or from givelist",
                          XSControl_tpcompound);

    IFSelect_Act::AddFunc("trdraw", "results ->DRAW : all;  or num [name] : from ent.num -> DRAW [name/tread_num]",
                          XSControl_traccess);
    IFSelect_Act::AddFunc("trsave", "results ->files : all;  or num [name] : from ent.num -> DRAW [name/tread_num]",
                          XSControl_traccess);
    ...
    IFSelect_Act::AddFunc("trconnexent", "name of draw shape : entities -> connected shapes (when known)",
    ...
    // skl IFSelect_Act::AddFunc ("checkbrep","shapename or * [+ rootname for expurged and faulties [+ mode
    // f-s]]",XSHAPE_checkbrep); skl IFSelect_Act::AddFunc ("dblist","option : clear  nb  index  set n1  data n1
    // n2",XSHAPE_dblist);
}
...
Standard_Integer XSControl_FuncShape::MoreShapes(const Handle(XSControl_WorkSession) & session,
    ...
    if (paro && moins && parf) {
        ...
        n1 = atoi(&name[paro + 1]);
        if (n1 < 0) n1 += n2; // sinon on a n1-n2
    }
    //  liste
    if (n1 <= n2 && n1 > 0) {
...
Standard_Boolean XSControl_FuncShape::FileAndVar(const Handle(XSControl_WorkSession) & session,
    ...
    resvar.Clear();
    if (file)
        if (file[0] == '\0' || (file[0] == '.' && file[1] == '\0')) iafic = Standard_False;
    if (!iafic)
