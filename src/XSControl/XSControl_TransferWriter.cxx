...
IFSelect_ReturnStatus XSControl_TransferWriter::TransferWriteTransient(const Handle(Interface_InterfaceModel) & model,
    ...
    try {
        ...
        status = myController->TransferWriteTransient(obj, myTransferWriter, model, myTransferMode, theProgress);
...
IFSelect_ReturnStatus XSControl_TransferWriter::TransferWriteShape(const Handle(Interface_InterfaceModel) & theModel,
    ...
    try {
        ...
        sout << "                      ******" << std::endl;
        status = myController->TransferWriteShape(aShape, myTransferWriter, theModel, myTransferMode, theProgress);
    } catch (Standard_Failure const& anException) {
        sout << "****  ****  TransferWriteShape, EXCEPTION : ";
        sout << anException.GetMessageString();
        sout << std::endl;
...
Interface_CheckIterator XSControl_TransferWriter::ResultCheckList(const Handle(Interface_InterfaceModel) &
                                                                  model) const {
    Interface_CheckIterator chl;
    if (myTransferWriter.IsNull()) return chl;
    Standard_Integer i, nb = myTransferWriter->NbMapped();
