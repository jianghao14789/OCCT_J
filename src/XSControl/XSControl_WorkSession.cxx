...
XSControl_WorkSession::XSControl_WorkSession()
    : myTransferReader(new XSControl_TransferReader), myTransferWriter(new XSControl_TransferWriter),
...
Handle(Standard_Transient) XSControl_WorkSession::Result(const Handle(Standard_Transient) & ent,
    ...
    if (binder.IsNull() && ouca > 0) binder = myTransferReader->TransientProcess()->Find(ent);
...
Handle(Interface_InterfaceModel) XSControl_WorkSession::NewModel() {
    ...
    SetModel(newmod);
    if (!myTransferReader->TransientProcess().IsNull()) myTransferReader->TransientProcess()->Clear();
...
IFSelect_ReturnStatus XSControl_WorkSession::TransferWriteShape(const TopoDS_Shape& shape,
    ...
    if (theProgress.UserBreak()) return IFSelect_RetStop;
...
void XSControl_WorkSession::ClearBinders() {
    ...
    for (; i <= FP->NbMapped(); i++) {
        ...
        if (!bnd.IsNull()) aSeqBnd.Append(bnd);
