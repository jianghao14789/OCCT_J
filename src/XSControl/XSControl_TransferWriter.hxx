...
public:
    ...
    XSControl_TransferWriter() : myTransferWriter(new Transfer_FinderProcess), myTransferMode(0) {}
    ...
    TransferWriteTransient(const Handle(Interface_InterfaceModel) & theModel, const Handle(Standard_Transient) & theObj,
    ...
    //! transfer badly initialised
    Standard_EXPORT IFSelect_ReturnStatus
    TransferWriteShape(const Handle(Interface_InterfaceModel) & theModel, const TopoDS_Shape& theShape,
                       const Message_ProgressRange& theProgress = Message_ProgressRange());
