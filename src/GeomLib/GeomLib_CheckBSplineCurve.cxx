...
GeomLib_CheckBSplineCurve::GeomLib_CheckBSplineCurve(const Handle(Geom_BSplineCurve) & Curve,
    ...
    : myCurve(Curve), myDone(Standard_False), myFixFirstTangent(Standard_False), myFixLastTangent(Standard_False),
      myAngularTolerance(Abs(AngularTolerance)), myTolerance(Abs(Tolerance)), myIndSecondPole(-1),
      myIndPrelastPole(-1) {
    ...
    if ((!myCurve->IsPeriodic()) && num_poles >= 4) {
        ...
        if (tangent_magnitude > myTolerance) tangent_normalized = tangent / tangent_magnitude;
        ...
        for (ii = 3; ii <= num_poles; ii++) {
            ...
            if (tangent_magnitude > myTolerance && vector_magnitude > myTolerance) {
                ...
                if (CrossProdSqLength > CrossProdSqTol) break;
...
void GeomLib_CheckBSplineCurve::NeedTangentFix(Standard_Boolean& FirstFlag, Standard_Boolean& LastFlag) const {
...
void GeomLib_CheckBSplineCurve::FixTangent(const Standard_Boolean FirstFlag, const Standard_Boolean LastFlag) {
...
void GeomLib_CheckBSplineCurve::FixTangentOnCurve(Handle(Geom_BSplineCurve) & theCurve,
                                                  const Standard_Boolean FirstFlag, const Standard_Boolean LastFlag) {
