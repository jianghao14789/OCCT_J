...
static Standard_Boolean MinComputing(GeomLib_CheckCurveOnSurface_TargetFunc& theFunction,
                                     ...
                                     const Standard_Integer theNbParticles, Standard_Real& theBestValue,
                                     Standard_Real& theBestParameter);

static Standard_Integer FillSubIntervals(const Handle(Adaptor3d_Curve) & theCurve3d,
                                         const Handle(Adaptor2d_Curve2d) & theCurve2d, const Standard_Real theFirst,
                                         const Standard_Real theLast, Standard_Integer& theNbParticles,
...
public:
    GeomLib_CheckCurveOnSurface_TargetFunc(const Adaptor3d_Curve& theC3D, const Adaptor3d_Curve& theCurveOnSurface,
                                           const Standard_Real theFirst, const Standard_Real theLast)
        : myCurve1(theC3D), myCurve2(theCurveOnSurface), myFirst(theFirst), myLast(theLast) {}
    ...
    Standard_Boolean Value(const Standard_Real theX, Standard_Real& theFVal) const {
        try {
            OCC_CATCH_SIGNALS
            if (!CheckParameter(theX)) return Standard_False;
    ...
    Standard_Boolean Derive(const Standard_Real theX, Standard_Real& theDeriv1,
    ...
    virtual Standard_Boolean Values(const math_Vector& theX, Standard_Real& theVal, math_Vector& theGrad) {
...
class GeomLib_CheckCurveOnSurface_Local {
public:
    GeomLib_CheckCurveOnSurface_Local(const Array1OfHCurve& theCurveArray, const Array1OfHCurve& theCurveOnSurfaceArray,
                                      const TColStd_Array1OfReal& theIntervalsArr, const Standard_Real theEpsilonRange,
                                      const Standard_Integer theNbParticles)
        : myCurveArray(theCurveArray), myCurveOnSurfaceArray(theCurveOnSurfaceArray), mySubIntervals(theIntervalsArr),
          myEpsilonRange(theEpsilonRange), myNbParticles(theNbParticles),
    ...
    void operator()(Standard_Integer theThreadIndex, Standard_Integer theElemIndex) const {
        ...
        GeomLib_CheckCurveOnSurface_TargetFunc aFunc(
            *(myCurveArray.Value(theThreadIndex).get()), *(myCurveOnSurfaceArray.Value(theThreadIndex).get()),
            mySubIntervals.Value(theElemIndex), mySubIntervals.Value(theElemIndex + 1));
...
GeomLib_CheckCurveOnSurface::GeomLib_CheckCurveOnSurface()
    : myErrorStatus(0), myMaxDistance(RealLast()), myMaxParameter(0.), myTolRange(Precision::PConfusion()) {}
...
GeomLib_CheckCurveOnSurface::GeomLib_CheckCurveOnSurface(const Handle(Adaptor3d_Curve) & theCurve,
    ...
    : myCurve(theCurve), myErrorStatus(0), myMaxDistance(RealLast()), myMaxParameter(0.), myTolRange(theTolRange) {}
...
// purpose  :
//=======================================================================
void GeomLib_CheckCurveOnSurface::Init(const Handle(Adaptor3d_Curve) & theCurve, const Standard_Real theTolRange) {
    myCurve = theCurve;
    myErrorStatus = 0;
    myMaxDistance = RealLast();
    myMaxParameter = 0.0;
...
void GeomLib_CheckCurveOnSurface::Perform(const Handle(Adaptor3d_CurveOnSurface) & theCurveOnSurface,
    ...
    const Standard_Integer aNbSubIntervals = FillSubIntervals(
        myCurve, theCurveOnSurface->GetCurve(), myCurve->FirstParameter(), myCurve->LastParameter(), aNbParticles);
    ...
    try {
        ...
        FillSubIntervals(myCurve, theCurveOnSurface->GetCurve(), myCurve->FirstParameter(), myCurve->LastParameter(),
                         aNbParticles, &anIntervals);

        const Standard_Integer aNbThreads =
            isMultiThread ? Min(anIntervals.Size(), OSD_ThreadPool::DefaultPool()->NbDefaultThreadsToLaunch()) : 1;
        ...
        GeomLib_CheckCurveOnSurface_Local aComp(aCurveArray, aCurveOnSurfaceArray, anIntervals, anEpsilonRange,
                                                aNbParticles);
        if (aNbThreads > 1) {
...
Standard_Integer FillSubIntervals(const Handle(Adaptor3d_Curve) & theCurve3d,
    ...
    try {
        ...
        if (theSubIntervals) theSubIntervals->ChangeValue(aNbSubIntervals) = theFirst;

        while ((anIndex3D <= anIndMax3D) && (anIndex2D <= anIndMax2D)) {
            const Standard_Real aVal3D = anArrKnots3D->Value(anIndex3D), aVal2D = anArrKnots2D->Value(anIndex2D);
            const Standard_Real aDelta = aVal3D - aVal2D;

            if (aDelta < Precision::PConfusion()) { // aVal3D <= aVal2D
                if ((aVal3D > theFirst) && (aVal3D < theLast)) {
                    aNbSubIntervals++;

                    if (theSubIntervals) theSubIntervals->ChangeValue(aNbSubIntervals) = aVal3D;
                }
                ...
                if ((aVal2D > theFirst) && (aVal2D < theLast)) {
                    ...
                    if (theSubIntervals) theSubIntervals->ChangeValue(aNbSubIntervals) = aVal2D;
                }
        ...
        if (theSubIntervals) theSubIntervals->ChangeValue(aNbSubIntervals + 1) = theLast;
...
//=======================================================================
Standard_Boolean PSO_Perform(GeomLib_CheckCurveOnSurface_TargetFunc& theFunction, const math_Vector& theParInf,
                             const math_Vector& theParSup, const Standard_Real theEpsilon,
                             const Standard_Integer theNbParticles, Standard_Real& theBestValue,
                             math_Vector& theOutputParam) {
    const Standard_Real aDeltaParam = theParSup(1) - theParInf(1);
    if (aDeltaParam < Precision::PConfusion()) return Standard_False;
    ...
    for (Standard_Real aPrm = theParInf(1); aCount <= aNbControlPoints;
        ...
        if (!theFunction.Value(aPrm, aVal)) continue;

        PSO_Particle* aParticle = aParticles.GetWorstParticle();

        if (aVal > aParticle->BestDistance) continue;
...
Standard_Boolean MinComputing(GeomLib_CheckCurveOnSurface_TargetFunc& theFunction,
    ...
    try {
        ...
        if (!PSO_Perform(theFunction, aParInf, aParSup, theEpsilon, theNbParticles, theBestValue, anOutputParam)) {
#ifdef OCCT_DEBUG
        ...
        if (aMinSol.IsDone() &&
            (aMinSol.GetStatus() == math_OK)) { // math_NewtonMinimum has precised the value. We take it.
            aMinSol.Location(anOutputParam);
            theBestParameter = anOutputParam(1);
            theBestValue = aMinSol.Minimum();
        } else { // Use math_PSO again but on smaller range.
            ...
            if (PSO_Perform(theFunction, aParInf, aParSup, theEpsilon, theNbParticles, aValue, anOutputParam)) {
