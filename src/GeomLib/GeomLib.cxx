...
//
static Standard_Boolean CompareWeightPoles(const TColgp_Array1OfPnt& thePoles1, const TColStd_Array1OfReal* const theW1,
                                           const TColgp_Array1OfPnt& thePoles2, const TColStd_Array1OfReal* const theW2,
...
//=======================================================================
static void ComputeLambda(const math_Matrix& Constraint, const math_Matrix& Hermit, const Standard_Real Length,
    ...
    math_Vector GaussP(1, GOrdre), GaussW(1, GOrdre), pol2(1, 2 * Continuity + 1), pol4(1, 4 * Continuity + 1);
    ...
    if (EMin > Precision::Confusion()) {
        ...
        math_FunctionAllRoots Solve(FF, S, Precision::Confusion(), Precision::Confusion() * (Length + 1), 1.e-15);
        if (Solve.IsDone()) {
...
void GeomLib::RemovePointsFromArray(const Standard_Integer NumPoints, const TColStd_Array1OfReal& InParameters,
...
void GeomLib::DensifyArray1OfReal(const Standard_Integer MinNumPoints, const TColStd_Array1OfReal& InParameters,
                                  Handle(TColStd_HArray1OfReal) & OutParameters) {
...
// purpose  :
//=======================================================================
void GeomLib::FuseIntervals(const TColStd_Array1OfReal& I1, const TColStd_Array1OfReal& I2, TColStd_SequenceOfReal& Seq,
                            const Standard_Real Epspar, const Standard_Boolean IsAdjustToFirstInterval) {
...
void GeomLib::EvalMaxParametricDistance(const Adaptor3d_Curve& ACurve, const Adaptor3d_Curve& AReferenceCurve,
                                        //			       const Standard_Real  Tolerance,
                                        const Standard_Real, const TColStd_Array1OfReal& Parameters,
...
void GeomLib::EvalMaxDistanceAlongParameter(const Adaptor3d_Curve& ACurve, const Adaptor3d_Curve& AReferenceCurve,
                                            const Standard_Real Tolerance, const TColStd_Array1OfReal& Parameters,
                                            Standard_Real& MaxDistance) {
    Standard_Integer ii;
    Standard_Real max_squared = 0.0e0, tolerance_squared = Tolerance * Tolerance, other_parameter, para_tolerance,
                  local_distance_squared;
    gp_Pnt Point1;
    gp_Pnt Point2;
    ...
    if (Form != gp_Other) {
        ...
        if (TheType == STANDARD_TYPE(Geom2d_TrimmedCurve)) {
            ...
            if (TheBasisType == STANDARD_TYPE(Geom2d_BSplineCurve) ||
                ...
                return result;
            } else if (TheBasisType == STANDARD_TYPE(Geom2d_Circle) || TheBasisType == STANDARD_TYPE(Geom2d_Ellipse) ||
...
// purpose  :
//=======================================================================
void GeomLib::SameRange(const Standard_Real Tolerance, const Handle(Geom2d_Curve) & CurvePtr,
                        const Standard_Real FirstOnCurve, const Standard_Real LastOnCurve,
                        const Standard_Real RequestedFirst, const Standard_Real RequestedLast,
                        Handle(Geom2d_Curve) & NewCurvePtr) {
    if (CurvePtr.IsNull()) throw Standard_Failure();
    if (Abs(LastOnCurve - RequestedLast) <= Tolerance && Abs(FirstOnCurve - RequestedFirst) <= Tolerance) {
    ...
    if (Abs(LastOnCurve - FirstOnCurve - RequestedLast + RequestedFirst) <= Tolerance) {
        if (CurvePtr->IsKind(STANDARD_TYPE(Geom2d_Line))) {
            ...
            GeomLib::SameRange(Tolerance, TC->BasisCurve(), FirstOnCurve, LastOnCurve, RequestedFirst, RequestedLast,
                               NewCurvePtr);
...
public:
    GeomLib_CurveOnSurfaceEvaluator(Adaptor3d_CurveOnSurface& theCurveOnSurface, Standard_Real theFirst,
                                    Standard_Real theLast)
        : CurveOnSurface(theCurveOnSurface), FirstParam(theFirst), LastParam(theLast) {}

    virtual void Evaluate(Standard_Integer* Dimension, Standard_Real StartEnd[2], Standard_Real* Parameter,
...
void GeomLib_CurveOnSurfaceEvaluator::Evaluate(Standard_Integer*, /*Dimension*/
                                               Standard_Real DebutFin[2], Standard_Real* Parameter,
                                               Standard_Integer* DerivativeRequest,
                                               Standard_Real* Result, // [Dimension]
...
void GeomLib::BuildCurve3d(const Standard_Real Tolerance, Adaptor3d_CurveOnSurface& Curve,
                           const Standard_Real FirstParameter, const Standard_Real LastParameter,
                           Handle(Geom_Curve) & NewCurvePtr, Standard_Real& MaxDeviation,
                           Standard_Real& AverageDeviation, const GeomAbs_Shape Continuity,
                           const Standard_Integer MaxDegree, const Standard_Integer MaxSegment)

{
    ...
    if (!geom_adaptor_curve_ptr.IsNull() && !geom_adaptor_surface_ptr.IsNull()) {
        ...
        if (isIsoLine(TrimmedC2D, isU, aParam, isForward)) {
            NewCurvePtr = buildC3dOnIsoLine(TrimmedC2D, geom_adaptor_surface_ptr, FirstParameter, LastParameter,
    ...
    AdvApprox_ApproxAFunction anApproximator(0, 0, 1, Tolerance1DPtr, Tolerance2DPtr, Tolerance3DPtr, FirstParameter,
                                             LastParameter, Continuity, MaxDegree, MaxSegment, ev,
...
void GeomLib::AdjustExtremity(Handle(Geom_BoundedCurve) & Curve, const gp_Pnt& P1, const gp_Pnt& P2, const gp_Vec& T1,
                              const gp_Vec& T2) {
    ...
    if (!PLib::HermiteCoefficients(0., 1., 1, 1, Mat)) throw Standard_ConstructionError();
    ...
    if (aDef->NbPoles() != aIn->NbPoles()) throw Standard_ConstructionError("Inconsistent poles's number");
...
void GeomLib::ExtendCurveToPoint(Handle(Geom_BoundedCurve) & Curve, const gp_Pnt& Point,
...
//=======================================================================
static Standard_Boolean ExtendKPart(Handle(Geom_RectangularTrimmedSurface) & Surface, const Standard_Real Length,
                                    const Standard_Boolean InU, const Standard_Boolean After) {
...
// purpose  :
//=======================================================================
void GeomLib::ExtendSurfByLength(Handle(Geom_BoundedSurface) & Surface, const Standard_Real Length,
                                 const Standard_Integer Continuity, const Standard_Boolean InU,
                                 const Standard_Boolean After) {
    if (Continuity < 0 || Continuity > 3) return;
    Standard_Integer Cont = Continuity;

    // Kpart ?
    Handle(Geom_RectangularTrimmedSurface) TS = Handle(Geom_RectangularTrimmedSurface)::DownCast(Surface);
    ...
    if (InU && (BS->UDegree() < Continuity + 1)) BS->IncreaseDegree(Continuity + 1, BS->VDegree());
    if (!InU && (BS->VDegree() < Continuity + 1)) BS->IncreaseDegree(BS->UDegree(), Continuity + 1);
    ...
    for (Kount = 0, Ok = Standard_False; Kount <= 2 && !Ok; Kount++) {
        ...
        BSplCLib::Eval(Tbord, periodic_flag, derivative_request, extrap_mode[0], Cdeg, FKnots->Array1(), Cdim, *Padr,
                       *Radr);
        Ok = Standard_True;
        for (ii = 1; ii <= Cdim; ii++) {
            point(ii) = Result(ii);
    ...
    for (Kount = 1; Kount <= 5 && !ExtOk; Kount++) {
        //  extension
        BSplCLib::TangExtendToConstraint(FKnots->Array1(), lambmin, NbP, *Padr, Cdim, Cdeg, ConstraintPoint, Cont,
                                         After, NbPoles, NbKnots, *FKRadr, *PRadr);
    ...
    Handle(Geom_BSplineSurface) Res =
        new (Geom_BSplineSurface)(NewPoles->Array2(), NewWeights->Array2(), UKnots, VKnots, UMults, VMults, UDeg, VDeg,
...
// purpose  :
//=======================================================================
void GeomLib::Inertia(const TColgp_Array1OfPnt& Points, gp_Pnt& Bary, gp_Dir& XDir, gp_Dir& YDir, Standard_Real& Xgap,
                      Standard_Real& Ygap, Standard_Real& Zgap) {
...
// purpose  :
//=======================================================================
void GeomLib::AxeOfInertia(const TColgp_Array1OfPnt& Points, gp_Ax2& Axe, Standard_Boolean& IsSingular,
...

{
    ...
    if (!BSurf->IsURational() || (BSurf->IsUPeriodic()))
        ...
        if (AlreadyTreated) return Standard_False;
    }
...
public:
    law_evaluator(const GeomLib_DenominatorMultiplierPtr theDenominatorPtr) : myDenominator(theDenominatorPtr) {}
...
static Standard_Boolean CheckIfKnotExists(const TColStd_Array1OfReal& surface_knots, const Standard_Real knot)
...
static void AddAKnot(const TColStd_Array1OfReal& knots, const TColStd_Array1OfInteger& mults,
                     const Standard_Real knotinserted, const Standard_Integer deltasurface_degree,
                     const Standard_Integer finalsurfacedegree, Handle(TColStd_HArray1OfReal) & newknots,
...
static void BuildFlatKnot(const TColStd_Array1OfReal& surface_knots, const TColStd_Array1OfInteger& surface_mults,
                          const Standard_Integer deltasurface_degree, const Standard_Integer finalsurface_degree,
                          const Standard_Real knotmin, const Standard_Real knotmax,
                          Handle(TColStd_HArray1OfReal) & ResultKnots, Handle(TColStd_HArray1OfInteger) & ResultMults)

{
    ...
    if (CheckIfKnotExists(surface_knots, knotmin) && CheckIfKnotExists(surface_knots, knotmax)) {
        ...
        if ((CheckIfKnotExists(surface_knots, knotmin)) && (!CheckIfKnotExists(surface_knots, knotmax)))
            AddAKnot(surface_knots, surface_mults, knotmax, deltasurface_degree, finalsurface_degree, ResultKnots,
                     ResultMults);
        else {
            if ((!CheckIfKnotExists(surface_knots, knotmin)) && (CheckIfKnotExists(surface_knots, knotmax)))
                AddAKnot(surface_knots, surface_mults, knotmin, deltasurface_degree, finalsurface_degree, ResultKnots,
                         ResultMults);
            else {
                if ((!CheckIfKnotExists(surface_knots, knotmin)) && (!CheckIfKnotExists(surface_knots, knotmax)) &&
                    (knotmin == knotmax)) {
                    AddAKnot(surface_knots, surface_mults, knotmin, deltasurface_degree, finalsurface_degree,
                             ResultKnots, ResultMults);
                } else {
                    Handle(TColStd_HArray1OfReal) IntermedKnots;
                    Handle(TColStd_HArray1OfInteger) IntermedMults;
                    AddAKnot(surface_knots, surface_mults, knotmin, deltasurface_degree, finalsurface_degree,
                             IntermedKnots, IntermedMults);
                    AddAKnot(IntermedKnots->ChangeArray1(), IntermedMults->ChangeArray1(), knotmax, 0,
                             finalsurface_degree, ResultKnots, ResultMults);
                }
            }
        }
    }
...
static void FunctionMultiply(Handle(Geom_BSplineSurface) & BSurf, const Standard_Real knotmin,
...

{
    ...
    BuildFlatKnot(surface_u_knots, surface_u_mults, 3, BSurf->UDegree() + 3, knotmin, knotmax, newuknots, newumults);
    BuildFlatKnot(surface_v_knots, surface_v_mults, BSurf->VDegree(), 2 * (BSurf->VDegree()), 1.0, 0.0, newvknots,
                  newvmults);
    length = 0;
    ...
    BSplSLib::FunctionMultiply(ev, // multiplication
                               BSurf->UDegree(), BSurf->VDegree(), surface_u_knots, surface_v_knots, &surface_u_mults,
                               &surface_v_mults, surface_poles, &surface_weights, newuflatknots, newvflatknots,
                               BSurf->UDegree() + 3, 2 * (BSurf->VDegree()), NewNumerator, NewDenominator, status);
    if (status != 0) throw Standard_ConstructionError("GeomLib Multiplication Error");
    ...
    BSurf = new Geom_BSplineSurface(NewNumerator, NewDenominator, newuknots->ChangeArray1(), newvknots->ChangeArray1(),
                                    newumults->ChangeArray1(), newvmults->ChangeArray1(), BSurf->UDegree() + 3,
                                    2 * (BSurf->VDegree()));
}
    ...
    if (CanBeTreated(BSurf)) {
        for (i = 1; i <= BSurf->NbVPoles(); i++) { // loop on each pole function
            ...
            BCurve = new Geom_BSplineCurve(BCurvePoles, // building of a pole function
                                           BCurveWeights, BCurveKnots, BCurveMults, BSurf->UDegree());
            Hermit::Solutionbis(BCurve, x, y, Precision::Confusion(), Precision::Confusion());
            if (x < uknotmin) uknotmin = x; // uknotmin,uknotmax:extremal knots
            if ((x != 1.0) && (x > uknotmax)) uknotmax = x;
            if ((y != 0.0) && (y < uknotmin)) uknotmin = y;
            if (y > uknotmax) uknotmax = y;
        }
...
void GeomLib::CancelDenominatorDerivative(Handle(Geom_BSplineSurface) & BSurf, const Standard_Boolean udirection,
...
Standard_Integer GeomLib::NormEstim(const Handle(Geom_Surface) & S, const gp_Pnt2d& UV, const Standard_Real Tol,
                                    gp_Dir& N) {
    gp_Vec DU, DV;
    gp_Pnt DummyPnt;
        ...
        if (isDone) {
            ...
            if ((UV.Y() > Vmin + step) && (UV.Y() < Vmax - step)) {
                ...
                if ((DU.XYZ().SquareModulus() > eps) && (DV.XYZ().SquareModulus() > eps)) {
                    ...
                    if ((DU.XYZ().SquareModulus() > eps) && (DV.XYZ().SquareModulus() > eps)) {
                        aNormal2 = DU ^ DV;
                        if (aNormal1.IsOpposite(aNormal2, aConeSingularityAngleEps)) return 2;
                    }
                }
            }
            ...
            // Along V
            if (MDU < aTol2 && MDV >= aTol2) {
                if ((Vmax - UV.Y()) > (UV.Y() - Vmin)) sign = 1.0;
                S->D1(UV.X(), UV.Y() + sign * step, DummyPnt, DU, DV);
                gp_Vec Norm = DU ^ DV;
                if (Norm.SquareMagnitude() < eps) {
                    Standard_Real sign1 = -1.0;
                    if ((Umax - UV.X()) > (UV.X() - Umin)) sign1 = 1.0;
                    S->D1(UV.X() + sign1 * step, UV.Y() + sign * step, DummyPnt, DU, DV);
                    Norm = DU ^ DV;
                }
                if ((Norm.SquareMagnitude() >= eps) && (Norm.Dot(aNormal) < 0.0)) aNormal.Reverse();
            }

            // Along U
            if (MDV < aTol2 && MDU >= aTol2) {
                if ((Umax - UV.X()) > (UV.X() - Umin)) sign = 1.0;
                S->D1(UV.X() + sign * step, UV.Y(), DummyPnt, DU, DV);
                gp_Vec Norm = DU ^ DV;
                if (Norm.SquareMagnitude() < eps) {
                    Standard_Real sign1 = -1.0;
                    if ((Vmax - UV.Y()) > (UV.Y() - Vmin)) sign1 = 1.0;
            ...
            // conical
            if (aStatus == CSLib_InfinityOfSolutions) return 2;
        }
...
// purpose  :
//=======================================================================
void GeomLib::IsClosed(const Handle(Geom_Surface) & S, const Standard_Real Tol, Standard_Boolean& isUClosed,
    ...
    switch (aSType) {
        ...
        Standard_FALLTHROUGH case GeomAbs_Cylinder: {
            if (Precision::IsInfinite(v1)) v1 = 0.;
...
// purpose  :
//=======================================================================
Standard_Boolean GeomLib::IsBSplUClosed(const Handle(Geom_BSplineSurface) & S, const Standard_Real U1,
                                        const Standard_Real U2, const Standard_Real Tol) {
...
// purpose  :
//=======================================================================
Standard_Boolean GeomLib::IsBSplVClosed(const Handle(Geom_BSplineSurface) & S, const Standard_Real V1,
                                        const Standard_Real V2, const Standard_Real Tol) {
...
// purpose  :
//=======================================================================
Standard_Boolean GeomLib::IsBzUClosed(const Handle(Geom_BezierSurface) & S, const Standard_Real U1,
                                      const Standard_Real U2, const Standard_Real Tol) {
    Handle(Geom_Curve) aCUF = S->UIso(U1);
    Handle(Geom_Curve) aCUL = S->UIso(U2);
    if (aCUF.IsNull() || aCUL.IsNull()) return Standard_False;
...
// purpose  :
//=======================================================================
Standard_Boolean GeomLib::IsBzVClosed(const Handle(Geom_BezierSurface) & S, const Standard_Real V1,
                                      const Standard_Real V2, const Standard_Real Tol) {
    Handle(Geom_Curve) aCVF = S->VIso(V1);
    Handle(Geom_Curve) aCVL = S->VIso(V2);
    if (aCVF.IsNull() || aCVL.IsNull()) return Standard_False;
...
static Standard_Boolean CompareWeightPoles(const TColgp_Array1OfPnt& thePoles1, const TColStd_Array1OfReal* const theW1,
    ...
    for (i = 1; i <= thePoles1.Length(); i++) {
        ...
        if (!aPole1.IsEqual(aPole2, theTol)) return Standard_False;
    }
    //
    return Standard_True;
}
...
// purpose  :
//=============================================================================
Standard_Boolean GeomLib::isIsoLine(const Handle(Adaptor2d_Curve2d) theC2D, Standard_Boolean& theIsU,
                                    Standard_Real& theParam, Standard_Boolean& theIsForward) {
    ...
    if (aType == GeomAbs_Line) {
        ...
        if (aVec2d.SquareMagnitude() < Precision::Confusion()) return Standard_False; // Degenerated spline.
        aDir2d = aVec2d;
    ...
    if (!isAppropriateType) return Standard_False;
...
Handle(Geom_Curve) GeomLib::buildC3dOnIsoLine(const Handle(Adaptor2d_Curve2d) theC2D,
                                              const Handle(Adaptor3d_Surface) theSurf, const Standard_Real theFirst,
                                              const Standard_Real theLast, const Standard_Real theTolerance,
                                              const Standard_Boolean theIsU, const Standard_Real theParam,
    ...
    if (aGeomAdapter.IsNull()) return Handle(Geom_Curve)();

    if (theSurf->GetType() == GeomAbs_Sphere) return Handle(Geom_Curve)();
    ...
    if (theIsU) {
        ...
        if (isToTrim) aC3d = new Geom_TrimmedCurve(aC3d, aU1Param, aU2Param);
    }
    ...
    if (!theIsForward) aCurve3d->Reverse();
    ...
    if (anError3d > theTolerance) return Handle(Geom_Curve)();
