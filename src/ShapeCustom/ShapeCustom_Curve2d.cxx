...
// purpose  : static
//=======================================================================
static gp_Lin2d GetLine(const gp_Pnt2d& P1, const gp_Pnt2d& P2, const Standard_Real c1, Standard_Real& cf,
                        Standard_Real& cl) {
    gp_Vec2d avec(P1, P2);
    gp_Dir2d adir(avec);
...
Standard_Boolean ShapeCustom_Curve2d::IsLinear(const TColgp_Array1OfPnt2d& thePoles, const Standard_Real tolerance,
                                               Standard_Real& Deviation) {
    Standard_Integer nbPoles = thePoles.Length();
    if (nbPoles < 2) return Standard_False;
    ...
    if (dMax < dPreci) return Standard_False;
    ...
    for (i = 1; i <= nbPoles; i++) {
        ...
        if (dist > tol2) return Standard_False;
...
Handle(Geom2d_Line) ShapeCustom_Curve2d::ConvertToLine2d(const Handle(Geom2d_Curve) & theCurve, const Standard_Real c1,
                                                         const Standard_Real c2, const Standard_Real theTolerance,
    ...
    if (P1.SquareDistance(P2) < dPreci) return aLine2d; // it is not a line
    ...
    if (!bsc.IsNull()) {
        ...
        bsc->Poles(Poles);
        if (!ShapeCustom_Curve2d::IsLinear(Poles, theTolerance, theDeviation)) return aLine2d; // non
...
Standard_Boolean ShapeCustom_Curve2d::SimplifyBSpline2d(Handle(Geom2d_BSplineCurve) & theBSpline2d,
    ...
    while (IsToRemove && NbK > 2) {
        ...
        if (aKnotIndx == 1 || aKnotIndx == NbK) IsToRemove = Standard_False;
    }
    return (aInitNbK > NbK);
}
