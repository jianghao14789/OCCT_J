...
Handle(Geom_Curve) ShapeCustom_Curve::ConvertToPeriodic(const Standard_Boolean substitute, const Standard_Real preci) {
    ...
    if (closed && !BSpl->IsPeriodic() && BSpl->NbPoles() > 3) {
        ...
        if (BSpl->Multiplicity(1) == BSpl->Degree() + 1 && BSpl->Multiplicity(BSpl->NbKnots()) == BSpl->Degree() + 1) {
            ...
            Standard_Real a = 0.5 * (BSpl->Knot(2) - BSpl->Knot(1) + BSpl->Knot(nbKnots) - BSpl->Knot(nbKnots - 1));
            ...
            Handle(Geom_BSplineCurve) res =
                new Geom_BSplineCurve(oldPoles, oldWeights, newKnots, newMults, BSpl->Degree(), BSpl->IsPeriodic());
            BSpl = res;
        } else if (BSpl->Multiplicity(1) > BSpl->Degree() || BSpl->Multiplicity(BSpl->NbKnots()) > BSpl->Degree() + 1)
            set = Standard_False;
        if (set) {
