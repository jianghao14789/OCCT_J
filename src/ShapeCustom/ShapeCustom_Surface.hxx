...
public:
    ...
    Standard_EXPORT Handle(Geom_Surface)
        ConvertToAnalytical(const Standard_Real tol, const Standard_Boolean substitute);
    ...
    //! Else, or in case of failure, returns a Null Handle
    Standard_EXPORT Handle(Geom_Surface)
        ConvertToPeriodic(const Standard_Boolean substitute, const Standard_Real preci = -1);
