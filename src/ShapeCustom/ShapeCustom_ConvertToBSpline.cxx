...
ShapeCustom_ConvertToBSpline::ShapeCustom_ConvertToBSpline()
    : myExtrMode(Standard_True), myRevolMode(Standard_True), myOffsetMode(Standard_True), myPlaneMode(Standard_False) {}
...
Standard_Boolean ShapeCustom_ConvertToBSpline::IsToConvert(const Handle(Geom_Surface) & S,
    ...
    if (SS->IsKind(STANDARD_TYPE(Geom_SurfaceOfLinearExtrusion))) return myExtrMode;
    if (SS->IsKind(STANDARD_TYPE(Geom_SurfaceOfRevolution))) return myRevolMode;
    if (SS->IsKind(STANDARD_TYPE(Geom_Plane))) return myPlaneMode;
...
Standard_Boolean ShapeCustom_ConvertToBSpline::NewSurface(const TopoDS_Face& F, Handle(Geom_Surface) & S,
    ...
    if (surf->IsKind(STANDARD_TYPE(Geom_OffsetSurface)) && !myOffsetMode) {
        ...
        Handle(Geom_BSplineSurface) bspl = ShapeConstruct::ConvertSurfaceToBSpline(
            basis, U1, U2, V1, V2, Precision::Approximation(), surf->Continuity(), 10000, 15);
        ...
        res = ShapeConstruct::ConvertSurfaceToBSpline(surf, U1, U2, V1, V2, Precision::Approximation(), cnt, 10000, 15);
    }
    if (S->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface))) {
        Handle(Geom_RectangularTrimmedSurface) RTS = Handle(Geom_RectangularTrimmedSurface)::DownCast(S);
...
Standard_Boolean ShapeCustom_ConvertToBSpline::NewCurve(const TopoDS_Edge& E, Handle(Geom_Curve) & C,
                                                        TopLoc_Location& L, Standard_Real& Tol) {
...
Standard_Boolean ShapeCustom_ConvertToBSpline::NewPoint(const TopoDS_Vertex& /*V*/, gp_Pnt& /*P*/,
                                                        Standard_Real& /*Tol*/) {
    return Standard_False;
}
...
Standard_Boolean ShapeCustom_ConvertToBSpline::NewCurve2d(const TopoDS_Edge& E, const TopoDS_Face& F,
...
Standard_Boolean ShapeCustom_ConvertToBSpline::NewParameter(const TopoDS_Vertex& /*V*/, const TopoDS_Edge& /*E*/,
...
GeomAbs_Shape ShapeCustom_ConvertToBSpline::Continuity(const TopoDS_Edge& E, const TopoDS_Face& F1,
