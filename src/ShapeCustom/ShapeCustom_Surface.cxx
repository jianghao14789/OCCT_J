...
ShapeCustom_Surface::ShapeCustom_Surface(const Handle(Geom_Surface) & S) : myGap(0) {
    Init(S);
}
...
Handle(Geom_Surface) ShapeCustom_Surface::ConvertToAnalytical(const Standard_Real tol,
    ...
    if (aPlanar) {
        ...
        for (i = 1; i <= 3; i++) {
            ...
            p3(i).SetCoord(0.5 * (p1(i).X() + p2(i).X()), 0.5 * (p1(i).Y() + p2(i).Y()), 0.5 * (p1(i).Z() + p2(i).Z()));
            R(i) = p3(i).Distance(p1(i));
            //	std::cout<<"sphere, i="<<i<<" V="<<V<<" R="<<R(i)<<" p1="<<p1(i).X()<<","<<p1(i).Y()<<","<<p1(i).Z()<<"
        ...
        for (j = 1; (j <= 2) && !isFound; j++) {
            ...
            for (i = 1; i <= 3; i++) {
                ...
                p3(i).SetCoord(0.5 * (p1(i).X() + p2(i).X()), 0.5 * (p1(i).Y() + p2(i).Y()),
                               0.5 * (p1(i).Z() + p2(i).Z()));
                R(i) = p3(i).Distance(p1(i));
            }
            if ((Abs(R(1) - R(2)) < tol) && (Abs(R(1) - R(3)) < tol)) {
                gp_Pnt p10(0.5 * (p3(1).X() + p3(2).X()), 0.5 * (p3(1).Y() + p3(2).Y()), 0.5 * (p3(1).Z() + p3(2).Z()));
...
#ifdef OCCT_DEBUG
                if (resD1U.Dot(origD1U) < 0 && j != 2) std::cout << " Tore a inverser !" << std::endl;
#endif
                newSurf = anObject;
...
Handle(Geom_Surface) ShapeCustom_Surface::ConvertToPeriodic(const Standard_Boolean substitute,
    ...
    if (uclosed && !BSpl->IsUPeriodic() && BSpl->NbUPoles() > 3) {
        ...
        if (BSpl->UMultiplicity(1) == BSpl->UDegree() + 1 &&
            ...
            Handle(Geom_BSplineSurface) res =
                new Geom_BSplineSurface(oldPoles, oldWeights, newUKnots, oldVKnots, newUMults, oldVMults,
                                        BSpl->UDegree(), BSpl->VDegree(), BSpl->IsUPeriodic(), BSpl->IsVPeriodic());
            BSpl = res;
    ...
    if (vclosed && !BSpl->IsVPeriodic() && BSpl->NbVPoles() > 3) {
        ...
        if (BSpl->VMultiplicity(1) == BSpl->VDegree() + 1 &&
            ...
            Handle(Geom_BSplineSurface) res =
                new Geom_BSplineSurface(oldPoles, oldWeights, oldUKnots, newVKnots, oldUMults, newVMults,
                                        BSpl->UDegree(), BSpl->VDegree(), BSpl->IsUPeriodic(), BSpl->IsVPeriodic());
            BSpl = res;
