...
public:
    ...
    Standard_EXPORT virtual Standard_Boolean Value(const Standard_Integer theIndex, gp_Pnt& thePoint,
...
private:
    ...
    void splitSegment(const Handle(Geom_Surface) & theSurf, const Handle(Geom2d_Curve) & theCurve2d,
                      const Standard_Real theFirst, const Standard_Real theLast, const Standard_Integer theNbIter);

    //! Checks whether the given point lies within tolerance of the vertex.
    Standard_Boolean isInToleranceOfVertex(const gp_Pnt& thePoint, const TopoDS_Vertex& theVertex) const;
