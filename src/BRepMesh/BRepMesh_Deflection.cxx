...
// purpose  :
//=======================================================================
Standard_Real BRepMesh_Deflection::ComputeAbsoluteDeflection(const TopoDS_Shape& theShape,
...
// Purpose :
//=======================================================================
void BRepMesh_Deflection::ComputeDeflection(const IMeshData::IEdgeHandle& theDEdge, const Standard_Real theMaxShapeSize,
    ...
    if (BRepMesh_ShapeTool::Range(anEdge, aCurve, aFirstParam, aLastParam)) {
        const Standard_Real aDistF =
            aFirstVertex.IsNull() ? -1.0 : BRep_Tool::Pnt(aFirstVertex).Distance(aCurve->Value(aFirstParam));
...
void BRepMesh_Deflection::ComputeDeflection(const IMeshData::IFaceHandle& theDFace,
    ...
    if (!theParameters.ForceFaceDeflection) {
        ...
        aFaceDeflection = Max(2. * BRepMesh_ShapeTool::MaxFaceTolerance(theDFace->GetFace()), aFaceDeflection);
    }
...
// Purpose :
//=======================================================================
Standard_Boolean BRepMesh_Deflection::IsConsistent(const Standard_Real theCurrent, const Standard_Real theRequired,
    ...
    Standard_Boolean isConsistent =
        theCurrent < (1. + theRatio) * theRequired && (!theAllowDecrease || theCurrent > (1. - theRatio) * theRequired);
    return isConsistent;
}
