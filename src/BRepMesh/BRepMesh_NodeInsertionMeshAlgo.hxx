...
template <class RangeSplitter, class BaseAlgo> class BRepMesh_NodeInsertionMeshAlgo : public BaseAlgo {
public:
    //! Constructor.
...
protected:
    ...
    virtual Standard_Boolean initDataStructure() Standard_OVERRIDE {
        ...
        for (Standard_Integer aWireIt = 0; aWireIt < aDFace->WiresNb(); ++aWireIt) {
            ...
            if (aDWire->IsSet(IMeshData_SelfIntersectingWire) || (aDWire->IsSet(IMeshData_OpenWire) && aWireIt != 0)) {
                continue;
            }
        ...
        for (Standard_Integer aWireIt = 0; aWireIt < aDFace->WiresNb(); ++aWireIt) {
            ...
            if (!aWire.IsNull() && !aWire->IsEmpty()) {
                myClassifier->RegisterWire(*aWire, aTolUV, myRangeSplitter.GetRangeU(), myRangeSplitter.GetRangeV());
            }
        }
    ...
    virtual Standard_Integer addNodeToStructure(const gp_Pnt2d& thePoint, const Standard_Integer theLocation3d,
                                                const BRepMesh_DegreeOfFreedom theMovability,
                                                const Standard_Boolean isForceAdd) Standard_OVERRIDE {
        return BaseAlgo::addNodeToStructure(myRangeSplitter.Scale(thePoint, Standard_True), theLocation3d,
                                            theMovability, isForceAdd);
    }
...
private:
    ...
    void insertInternalVertex(const TopoDS_Vertex& theVertex) {
        try {
            ...
            if (myClassifier->Perform(aPnt2d) != TopAbs_IN) return;
