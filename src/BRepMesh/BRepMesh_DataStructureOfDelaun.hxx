...
public:
    ...
    Standard_EXPORT BRepMesh_DataStructureOfDelaun(const Handle(NCollection_IncAllocator) & theAllocator,
...
public: //! @name API for accessing mesh nodes.
    ...
    Standard_EXPORT Standard_Boolean SubstituteNode(const Standard_Integer theIndex, const BRepMesh_Vertex& theNewNode);
    ...
    void RemoveNode(const Standard_Integer theIndex, const Standard_Boolean isForce = Standard_False) {
        if (isForce || myNodes->FindKey(theIndex).Movability() == BRepMesh_Free) {
            if (LinksConnectedTo(theIndex).Extent() == 0) myNodes->DeleteVertex(theIndex);
        }
    }
...
public: //! @name API for accessing mesh links.
    ...
    Standard_EXPORT Standard_Boolean SubstituteLink(const Standard_Integer theIndex, const BRepMesh_Edge& theNewLink);
    ...
    //! is not Free.
    Standard_EXPORT void RemoveLink(const Standard_Integer theIndex, const Standard_Boolean isForce = Standard_False);
...
public: //! @name API for accessing mesh elements.
    ...
    Standard_EXPORT void ElementNodes(const BRepMesh_Triangle& theElement, Standard_Integer (&theNodes)[3]);
