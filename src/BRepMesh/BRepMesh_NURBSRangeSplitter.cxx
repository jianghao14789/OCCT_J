...
public:
    //! Constructor.
    AnalyticalFilter(const IMeshData::IFaceHandle& theDFace, const GeomAbs_IsoType theIsoType,
        ...
        : myDFace(theDFace), mySurface(myDFace->GetSurface()->Surface().Surface()), myIsoU(theIsoType == GeomAbs_IsoU),
          myParams(theParams), myControlParams(theControlParams), myParamsForbiddenToRemove(theParamsForbiddenToRemove),
          myControlParamsForbiddenToRemove(theControlParamsForbiddenToRemove),
          myAllocator(new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE)),
          myControlParamsToRemove(new IMeshData::MapOfReal(1, myAllocator)), myCurrParam(0.0), myCurrControlParam(0.0),
          myPrevControlParam(0.0) {}

    //! Returns map of parameters supposed to be removed.
    const Handle(IMeshData::MapOfReal) & GetControlParametersToRemove(const IMeshTools_Parameters& theParameters) {
...
private:
    ...
    Standard_Boolean checkParameterForDeflectionAndUpdateCache(const Standard_Real theParam) {
        ...
        if (aSqDist < myDFace->GetDeflection() * myDFace->GetDeflection()) {
            ...
            if (myPrevControlVec.SquareMagnitude() < gp::Resolution() || aTmpVec.SquareMagnitude() < gp::Resolution() ||
...
Standard_Boolean addParam(const Standard_Real& theParam, const std::pair<Standard_Real, Standard_Real>& theRange,
...
Standard_Boolean initParamsFromIntervals(const TColStd_Array1OfReal& theIntervals,
                                         ...
                                         const Standard_Boolean isSplitIntervals, IMeshData::IMapOfReal& theParams) {
...
Standard_Boolean toSplitIntervals(const Handle(Geom_Surface) & theSurf, const TColStd_Array1OfReal (&theIntervals)[2]) {
    ...
    for (; aIntervalU <= theIntervals[0].Upper(); ++aIntervalU) {
        ...
        if (Precision::IsInfinite(aParamU)) continue;
        ...
        for (; aIntervalV <= theIntervals[1].Upper(); ++aIntervalV) {
            ...
            if (Precision::IsInfinite(aParamV)) continue;
...
void BRepMesh_NURBSRangeSplitter::AdjustRange() {
    ...
    if (mySurfaceType == GeomAbs_BezierSurface) {
        ...
        myIsValid = !(aRangeU.first < -0.5 || aRangeU.second > 1.5 || aRangeV.first < -0.5 || aRangeV.second > 1.5);
    }
}
...
// Purpose :
//=======================================================================
Handle(IMeshData::ListOfPnt2d)
    BRepMesh_NURBSRangeSplitter::GenerateSurfaceNodes(const IMeshTools_Parameters& theParameters) const {
    if (!initParameters()) {
    ...
    const Handle(NCollection_IncAllocator) aTmpAlloc = new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE);
    ...
    Handle(IMeshData::MapOfReal)
        aFixedParams[2] = {new IMeshData::MapOfReal(1, aTmpAlloc), new IMeshData::MapOfReal(1, aTmpAlloc)};

    const Handle(IMeshData::MapOfReal) aParamsToRemove[2] = {
        AnalyticalFilter(GetDFace(), GeomAbs_IsoV, aParams[1], aParams[0], aFixedParams[1], aFixedParams[0])
            .GetControlParametersToRemove(theParameters),

        AnalyticalFilter(GetDFace(), GeomAbs_IsoU, aParams[0], aParams[1], aFixedParams[0], aFixedParams[1])
            .GetControlParametersToRemove(theParameters),
    };
...
Standard_Boolean BRepMesh_NURBSRangeSplitter::initParameters() const {
    ...
    const std::pair<Standard_Integer, Standard_Integer> aIntervalsNb(aSurface->NbUIntervals(aContinuity),
                                                                     aSurface->NbVIntervals(aContinuity));

    TColStd_Array1OfReal aIntervals[2] = {TColStd_Array1OfReal(1, aIntervalsNb.first + 1),
...
// purpose  :
//=======================================================================
Standard_Boolean BRepMesh_NURBSRangeSplitter::grabParamsOfEdges(const EdgeType theEdgeType,
...
Handle(IMeshData::SequenceOfReal) BRepMesh_NURBSRangeSplitter::computeGrainAndFilterParameters(
    const IMeshData::IMapOfReal& theSourceParams, const Standard_Real theTol2d, const Standard_Real theRangeDiff,
    const Standard_Real theDelta, const IMeshTools_Parameters& theParameters,
...
// purpose  :
//=======================================================================
Handle(IMeshData::SequenceOfReal) BRepMesh_NURBSRangeSplitter::filterParameters(const IMeshData::IMapOfReal& theParams,
    ...
    for (j = 2; j <= anInitLen; j++) {
        if ((aParamArray(j) - aParamArray(aParamLength)) > theMinDist) {
            if (++aParamLength < j) aParamArray(aParamLength) = aParamArray(j);
        }
    }
