...
public:
    ...
    BRepMesh_DelaunayDeflectionControlMeshAlgo()
        : myMaxSqDeflection(-1.), mySqMinSize(-1.), myIsAllDegenerated(Standard_False), myCircles(NULL) {}
...
protected:
    ...
    virtual void postProcessMesh(BRepMesh_Delaun& theMesher, const Message_ProgressRange& theRange) Standard_OVERRIDE {
        ...
        if (this->getParameters().ControlSurfaceDeflection && this->getStructure()->ElementsOfDomain().Extent() > 0) {
    ...
    virtual void optimizeMesh(BRepMesh_Delaun& theMesher, const Message_ProgressRange& theRange) {
        ...
        myCouplesMap =
            new IMeshData::MapOfOrientedEdges(3 * this->getStructure()->ElementsOfDomain().Extent(), aTmpAlloc);
...
private:
    ...
    struct TriangleNodeInfo {
        TriangleNodeInfo() : isFrontierLink(Standard_False) {}
    ...
    public:
        NormalDeviation(const gp_Pnt& theRefPnt, const gp_Vec& theNormal) : myRefPnt(theRefPnt), myNormal(theNormal) {}
    ...
    class LineDeviation {
    public:
        LineDeviation(const gp_Pnt& thePnt1, const gp_Pnt& thePnt2) : myPnt1(thePnt1), myPnt2(thePnt2) {}
    ...
    void getTriangleInfo(const BRepMesh_Triangle& theTriangle, const Standard_Integer (&theNodesIndices)[3],
    ...
    void splitTriangleGeometry(const BRepMesh_Triangle& theTriangle) {
        if (theTriangle.Movability() != BRepMesh_Deleted) {
            ...
            if (computeTriangleGeometry(aNodesInfo, aLinkVec, aNormal)) {
                ...
                const gp_XY aCenter2d = (aNodesInfo[0].Point2d + aNodesInfo[1].Point2d + aNodesInfo[2].Point2d) / 3.;
    ...
    Standard_Boolean computeTriangleGeometry(const TriangleNodeInfo (&theNodesInfo)[3], gp_Vec (&theLinks)[3],
                                             gp_Vec& theNormal) {
    ...
    //! Updates array of links vectors.
    //! @return False on degenerative triangle.
    Standard_Boolean checkTriangleForDegenerativityAndGetLinks(const TriangleNodeInfo (&theNodesInfo)[3],
                                                               gp_Vec (&theLinks)[3]) {
    ...
    void splitLinks(const TriangleNodeInfo (&theNodesInfo)[3], const Standard_Integer (&theNodesIndices)[3]) {
    ...
    Standard_Boolean checkDeflectionOfPointAndUpdateCache(const gp_XY& thePnt2d, const gp_Pnt& thePnt3d,
