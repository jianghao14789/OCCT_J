...
BRepMesh_CurveTessellator::BRepMesh_CurveTessellator(const IMeshData::IEdgeHandle& theEdge,
    ...
    : myDEdge(theEdge), myParameters(theParameters), myEdge(theEdge->GetEdge()), myCurve(myEdge) {
    init();
}
...
BRepMesh_CurveTessellator::BRepMesh_CurveTessellator(const IMeshData::IEdgeHandle& theEdge,
                                                     ...
                                                     const IMeshTools_Parameters& theParameters)
    : myDEdge(theEdge), myParameters(theParameters), myEdge(TopoDS::Edge(theEdge->GetEdge().Oriented(theOrientation))),
...
void BRepMesh_CurveTessellator::init() {
    ...
    if (myParameters.AdjustMinSize) {
        aMinSize = Min(aMinSize, myParameters.RelMinSize() *
                                     GCPnts_AbscissaPoint::Length(myCurve, myCurve.FirstParameter(),
                                                                  myCurve.LastParameter(), aPreciseLinDef));
    }
    ...
    myDiscretTool.Initialize(myCurve, myCurve.FirstParameter(), myCurve.LastParameter(), aPreciseAngDef, aPreciseLinDef,
                             aMinPntNb, Precision::PConfusion(), aMinSize);
...
void BRepMesh_CurveTessellator::addInternalVertices() {
    ...
    for (; aVertexIt.More(); aVertexIt.Next()) {
        ...
        myDiscretTool.AddPoint(BRep_Tool::Pnt(aVertex), BRep_Tool::Parameter(aVertex, myEdge), Standard_True);
    }
}
...
// purpose  :
//=======================================================================
Standard_Boolean BRepMesh_CurveTessellator::isInToleranceOfVertex(const gp_Pnt& thePoint,
...
// purpose  :
//=======================================================================
Standard_Boolean BRepMesh_CurveTessellator::Value(const Standard_Integer theIndex, gp_Pnt& thePoint,
...
void BRepMesh_CurveTessellator::splitSegment(const Handle(Geom_Surface) & theSurf,
                                             const Handle(Geom2d_Curve) & theCurve2d, const Standard_Real theFirst,
                                             const Standard_Real theLast, const Standard_Integer theNbIter) {
