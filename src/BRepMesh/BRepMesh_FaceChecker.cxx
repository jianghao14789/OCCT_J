...
public:
    ...
    SegmentsFiller(const IMeshData::IFaceHandle& theDFace,
        ...
        : myDFace(theDFace), myWiresSegments(theWiresSegments), myWiresBndBoxTree(theWiresBndBoxTree) {
...
public:
    ...
    BndBox2dTreeSelector(const Standard_Real theTolerance)
        : myMaxLoopSize(M_PI * theTolerance * theTolerance), mySelfSegmentIndex(-1), mySegment(0),
    ...
    void Reset(const BRepMesh_FaceChecker::Segment* theSegment, const Standard_Integer theSelfSegmentIndex) {
        myIndices.Clear();
    ...
    virtual Standard_Boolean Accept(const Standard_Integer& theSegmentIndex) {
        ...
        const BRepMesh_GeomTool::IntFlag aIntStatus =
            BRepMesh_GeomTool::IntSegSeg(mySegment->Point1->XY(), mySegment->Point2->XY(), aSegment.Point1->XY(),
                                         aSegment.Point2->XY(), Standard_False, Standard_False, aIntPnt);
        ...
        if (aIntStatus == BRepMesh_GeomTool::Cross) {
            ...
            if (mySelfSegmentIndex != -1) {
                ...
                for (Standard_Integer i = mySelfSegmentIndex; i < theSegmentIndex; ++i) {
                    ...
                    if (aCurVec.SquareModulus() < gp::Resolution()) continue;

                    if (aPrevVec.SquareModulus() > gp::Resolution()) aSumS += aPrevVec ^ aCurVec;
...
BRepMesh_FaceChecker::BRepMesh_FaceChecker(const IMeshData::IFaceHandle& theFace,
    ...
    : myDFace(theFace), myParameters(theParameters) {}
