...
inline void appendTriangle(const int theNode1, const int theNode2, const int theNode3,
                           const TColStd_SequenceOfInteger& theW, NCollection_List<Poly_Triangle>& thePolyTriangles) {
...
Handle(Poly_Triangulation) BRepMesh_Triangulator::ToPolyTriangulation(
    const TColgp_Array1OfPnt& theNodes, const NCollection_List<Poly_Triangle>& thePolyTriangles) {
...
BRepMesh_Triangulator::BRepMesh_Triangulator(const NCollection_Vector<gp_XYZ>& theXYZs,
    ...
    : myXYZs(theXYZs), myWires(theWires), myPlane(gp::Origin(), theNorm) {}
...
Standard_Boolean BRepMesh_Triangulator::checkCondition(const int (&theNodes)[4],
    ...
    return (aCross1.SquareModulus() < Precision::SquareConfusion() ||
            aCross2.SquareModulus() < Precision::SquareConfusion() || gp_Dir(aCross1).IsEqual(gp_Dir(aCross2), 0.01));
}
...
Standard_Boolean BRepMesh_Triangulator::prepareMeshStructure() {
    ...
    try {
        ...
        myMess->Send("makeTrianglesUsingBRepMesh: Exception raised during filling of BRepMesh_DataStructureOfDelaun",
                     Message_Fail);
        return Standard_False;
    }
