...
public:
    ...
    void operator()(const Standard_Integer theEdgeIndex) const {
        ...
        if (aDEdge->GetCurve()->ParametersNb() == 0) return;
...
private:
    ...
    void commitPolygons(const IMeshData::IEdgeHandle& theDEdge) const {
        ...
        for (; aPolygonIt.More(); aPolygonIt.Next()) {
            ...
            if (!aTriangulation.IsNull()) {
                ...
                if (aPCurves.Size() == 2) {
                    BRepMesh_ShapeTool::UpdateEdge(aEdge, collectPolygon(aPCurves.First(), theDEdge->GetDeflection()),
    ...
    Handle(Poly_PolygonOnTriangulation)
        collectPolygon(const IMeshData::IPCurveHandle& thePCurve, const Standard_Real theDeflection) const {
...
public:
    //! Constructor
    DeflectionEstimator(const Handle(IMeshData_Model) & theModel, const IMeshTools_Parameters& theParams)
        : myModel(theModel),
          myParams(new Poly_TriangulationParameters(theParams.Deflection, theParams.Angle, theParams.MinSize)) {}
...
// Purpose :
//=======================================================================
Standard_Boolean BRepMesh_ModelPostProcessor::performInternal(const Handle(IMeshData_Model) & theModel,
