...
public:
    ...
    BRepMesh_CircleInspector(const Standard_Real theTolerance, const Standard_Integer theReservedSize,
                             const Handle(NCollection_IncAllocator) & theAllocator)
        : mySqTolerance(theTolerance * theTolerance), myResIndices(theAllocator),
    ...
    NCollection_CellFilter_Action Inspect(const Standard_Integer theTargetIndex) {
        ...
        if (aRadius < 0.) return CellFilter_Purge;
        ...
        if ((aDX * aDX + aDY * aDY) - (aRadius * aRadius) <= mySqTolerance) myResIndices.Append(theTargetIndex);
