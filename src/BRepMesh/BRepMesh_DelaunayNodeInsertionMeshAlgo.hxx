...
public:
    //! Constructor.
    BRepMesh_DelaunayNodeInsertionMeshAlgo() : myIsPreProcessSurfaceNodes(Standard_False) {}
...
protected:
    ...
    virtual std::pair<Standard_Integer, Standard_Integer>
    getCellsCount(const Standard_Integer theVerticesNb) Standard_OVERRIDE {
        ...
        for (Standard_Integer aNodeIt = 1; aNodesIt.More(); aNodesIt.Next(), ++aNodeIt) {
            ...
            if (this->getClassifier()->Perform(aPnt2d) == TopAbs_IN) {
                aVertexIndexes.Append(
                    this->registerNode(this->getRangeSplitter().Point(aPnt2d), aPnt2d, BRepMesh_Free, Standard_False));
            }
        }
