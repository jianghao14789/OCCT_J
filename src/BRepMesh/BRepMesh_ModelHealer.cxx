...
public:
    //! Constructor.
    EdgeAmplifier(const IMeshTools_Parameters& theParameters) : myParameters(theParameters) {}
    ...
    void operator()(const IMeshData::IEdgePtr& theDEdge) const {
        ...
        Handle(IMeshTools_CurveTessellator) aTessellator =
            BRepMesh_EdgeDiscret::CreateEdgeTessellator(aDEdge, aPCurve->GetOrientation(), aDFace, myParameters);
...
Standard_Boolean isSameWithSomeOf(const TopoDS_Vertex& theRefVertex, const TopoDS_Vertex& theVertex1,
...
Standard_Boolean isInToleranceWithSomeOf(const gp_Pnt& theRefPoint, const gp_Pnt& thePoint1, const gp_Pnt& thePoint2,
                                         const Standard_Real theTol) {
    const Standard_Real aSqTol = theTol * theTol;
    return (theRefPoint.SquareDistance(thePoint1) < aSqTol || theRefPoint.SquareDistance(thePoint2) < aSqTol);
}
} // namespace
...
void BRepMesh_ModelHealer::amplifyEdges() {
    ...
    while (aAmpIt++ < aIterNb && popEdgesToUpdate(aEdgesToUpdate)) {
        ...
        OSD_Parallel::ForEach(aEdgesToUpdate.cbegin(), aEdgesToUpdate.cend(), anEdgeAmplifier,
                              !(myParameters.InParallel && aEdgesToUpdate.Size() > 1), aEdgesToUpdate.Size());
        ...
        OSD_Parallel::ForEach(aFacesToCheck.cbegin(), aFacesToCheck.cend(), *this,
                              !(myParameters.InParallel && aFacesToCheck.Size() > 1), aFacesToCheck.Size());
    ...
    for (int aWireIt = 0; aWireIt < theDFace->WiresNb(); ++aWireIt) {
        ...
        for (int aEdgeIt = 0; aEdgeIt < aDWire->EdgesNb(); ++aEdgeIt) {
            ...
            Standard_Boolean isConnected =
                !getCommonVertex(aCurrEdge, aNextEdge).IsNull() && !getCommonVertex(aPrevEdge, aCurrEdge).IsNull();
...
TopoDS_Vertex BRepMesh_ModelHealer::getCommonVertex(const IMeshData::IEdgeHandle& theEdge1,
    ...
    if (aVertex1_1.IsNull() || aVertex1_2.IsNull()) return TopoDS_Vertex();
    ...
    if (aVertex2_1.IsNull() || aVertex2_2.IsNull()) return TopoDS_Vertex();
...
// Purpose :
//=======================================================================
Standard_Boolean BRepMesh_ModelHealer::connectClosestPoints(const IMeshData::IPCurveHandle& thePrevDEdge,
    ...
    if (aPrevSqDist - aNextSqDist > gp::Resolution()) {
        adjustSamePoints(aCurrNextUV, aNextUV, aCurrPrevUV, aPrevUV, aCurrFirstUV, aCurrLastUV, aPrevFirstUV,
                         aPrevLastUV);
    } else {
        adjustSamePoints(aCurrPrevUV, aPrevUV, aCurrNextUV, aNextUV, aCurrFirstUV, aCurrLastUV, aNextFirstUV,
                         aNextLastUV);
    }
