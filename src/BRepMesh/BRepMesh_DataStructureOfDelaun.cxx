...
BRepMesh_DataStructureOfDelaun::BRepMesh_DataStructureOfDelaun(const Handle(NCollection_IncAllocator) & theAllocator,
    ...
    : myAllocator(theAllocator), myNodes(new BRepMesh_VertexTool(myAllocator)),
      myNodeLinks(theReservedNodeSize * 3, myAllocator), myLinks(theReservedNodeSize * 3, myAllocator),
      myDelLinks(myAllocator), myElements(theReservedNodeSize * 2, myAllocator) {}
...
Standard_Integer BRepMesh_DataStructureOfDelaun::AddNode(const BRepMesh_Vertex& theNode,
    ...
    if (!myNodeLinks.IsBound(aNodeId)) myNodeLinks.Bind(aNodeId, IMeshData::ListOfInteger(myAllocator));
...
Standard_Boolean BRepMesh_DataStructureOfDelaun::SubstituteNode(const Standard_Integer theIndex,
    ...
    if (myNodes->FindIndex(theNewNode) != 0) return Standard_False;
...
Standard_Boolean BRepMesh_DataStructureOfDelaun::SubstituteLink(const Standard_Integer theIndex,
    ...
    if (IndexOf(theNewLink) != 0) return Standard_False;
...
// purpose  :
//=======================================================================
void BRepMesh_DataStructureOfDelaun::RemoveLink(const Standard_Integer theIndex, const Standard_Boolean isForce) {
...
// purpose  :
//=======================================================================
void BRepMesh_DataStructureOfDelaun::cleanLink(const Standard_Integer theIndex, const BRepMesh_Edge& theLink) {
...
void BRepMesh_DataStructureOfDelaun::RemoveElement(const Standard_Integer theIndex) {
    ...
    if (aElement.Movability() == BRepMesh_Deleted) return;
...
void BRepMesh_DataStructureOfDelaun::cleanElement(const Standard_Integer theIndex,
    ...
    if (theElement.Movability() != BRepMesh_Free) return;
...
void BRepMesh_DataStructureOfDelaun::clearDeletedLinks() {
    ...
    while (!myDelLinks.IsEmpty()) {
        while (aLastLiveItem > 0) {
            if (GetLink(aLastLiveItem).Movability() != BRepMesh_Deleted) break;
        ...
        if (aDelItem > aLastLiveItem) continue;
...
void BRepMesh_DataStructureOfDelaun::clearDeletedNodes() {
    ...
    while (!aDelNodes.IsEmpty()) {
        while (aLastLiveItem > 0) {
            if (GetNode(aLastLiveItem).Movability() != BRepMesh_Deleted) break;
