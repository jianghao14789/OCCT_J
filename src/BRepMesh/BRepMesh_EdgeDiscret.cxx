...
Handle(IMeshTools_CurveTessellator) BRepMesh_EdgeDiscret::CreateEdgeTessellator(
    const IMeshData::IEdgeHandle& theDEdge, const IMeshTools_Parameters& theParameters) {
...
//=======================================================================
Handle(IMeshTools_CurveTessellator) BRepMesh_EdgeDiscret::CreateEdgeTessellator(
    const IMeshData::IEdgeHandle& theDEdge, const TopAbs_Orientation theOrientation,
    const IMeshData::IFaceHandle& theDFace, const IMeshTools_Parameters& theParameters) {
    return theDEdge->GetSameParam() ? new BRepMesh_CurveTessellator(theDEdge, theParameters)
...
Handle(IMeshTools_CurveTessellator) BRepMesh_EdgeDiscret::CreateEdgeTessellationExtractor(
    const IMeshData::IEdgeHandle& theDEdge, const IMeshData::IFaceHandle& theDFace) {
    ...
    try {
        ...
        if (!aDEdge->IsFree()) {
            ...
            if (aMinPCurveIndex != -1) {
                ...
                aEdgeTessellator = BRepMesh_EdgeDiscret::CreateEdgeTessellator(aDEdge, aPCurve->GetOrientation(),
                                                                               aDFace, myParameters);
            }
        } else {
            ...
            if (!aPoly3D.IsNull()) {
                if (aPoly3D->HasParameters() &&
                    BRepMesh_Deflection::IsConsistent(aPoly3D->Deflection(), aDEdge->GetDeflection(),
...
//=======================================================================
Standard_Real
BRepMesh_EdgeDiscret::checkExistingPolygonAndUpdateStatus(const IMeshData::IEdgeHandle& theDEdge,
    ...
    if (!aPolygon.IsNull()) {
        Standard_Boolean isConsistent =
            ...
            BRepMesh_Deflection::IsConsistent(aPolygon->Deflection(), theDEdge->GetDeflection(),
                                              myParameters.AllowQualityDecrease);
...
void BRepMesh_EdgeDiscret::Tessellate3d(const IMeshData::IEdgeHandle& theDEdge,
    ...
    if (aFirstVertex.IsNull() || aLastVertex.IsNull()) return;
    ...
    if (!theDEdge->GetDegenerated()) {
        for (Standard_Integer i = 2; i < theTessellator->PointsNb(); ++i) {
            gp_Pnt aPoint;
            Standard_Real aParam;
            if (!theTessellator->Value(i, aPoint, aParam)) continue;
...
// Purpose :
//=======================================================================
void BRepMesh_EdgeDiscret::Tessellate2d(const IMeshData::IEdgeHandle& theDEdge, const Standard_Boolean theUpdateEnds) {
    const IMeshData::ICurveHandle& aCurve = theDEdge->GetCurve();
