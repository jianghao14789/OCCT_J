...
Standard_Boolean BRepMesh_GeomTool::Value(const Standard_Integer theIndex,
    ...
    if (theIndex < 1 || theIndex > NbPoints()) return Standard_False;

    if (myEdge == NULL) return Standard_False;
...
Standard_Boolean BRepMesh_GeomTool::Normal(const Handle(BRepAdaptor_Surface) & theSurface,
    ...
    if (aOri == TopAbs_REVERSED) theNormal.Reverse();
...
BRepMesh_GeomTool::IntFlag BRepMesh_GeomTool::IntSegSeg(const gp_XY& theStartPnt1, const gp_XY& theEndPnt1,
    ...
    if (aPointHash[0] < 0 || aPointHash[1] < 0) {
        if (aPosHash == -1) {
            ...
            if (isConsiderEndPointTouch) return BRepMesh_GeomTool::EndPointTouch;
    ...
    if (aIntFlag == BRepMesh_GeomTool::NoIntersection) return BRepMesh_GeomTool::NoIntersection;
    ...
    for (Standard_Integer i = 0; i < 2; ++i) {
        if (aParam[i] < aPrec || aParam[i] > aEndPrec) return BRepMesh_GeomTool::NoIntersection;
    }
...
BRepMesh_GeomTool::CellsCount(const Handle(Adaptor3d_Surface) & theSurface, const Standard_Integer theVerticesNb,
    ...
    if (theRangeSplitter == NULL) return std::pair<Standard_Integer, Standard_Integer>(-1, -1);
...
Standard_Integer BRepMesh_GeomTool::classifyPoint(const gp_XY& thePoint1, const gp_XY& thePoint2,
    ...
    if (aDist > aPrec) {
        ...
        if (aDist > aSqPrec) return 0; // out
    }

    gp_XY aMult = aP1.Multiplied(aP2);
    if (aMult.X() < 0.0 || aMult.Y() < 0.0) return 0; // out

    if (aP1.SquareModulus() < aP2.SquareModulus()) return 0; // out
