...
BRepMesh_CircleTool::BRepMesh_CircleTool(const Handle(NCollection_IncAllocator) & theAllocator)
    : myTolerance(Precision::PConfusion()), myAllocator(theAllocator), myCellFilter(10.0, theAllocator),
...
// purpose  :
//=======================================================================
void BRepMesh_CircleTool::bind(const Standard_Integer theIndex, const gp_XY& theLocation,
...
// purpose  :
//=======================================================================
Standard_Boolean BRepMesh_CircleTool::MakeCircle(const gp_XY& thePoint1, const gp_XY& thePoint2, const gp_XY& thePoint3,
                                                 gp_XY& theLocation, Standard_Real& theRadius) {
    ...
    if (aLink1.SquareModulus() < aSqPrecision) return Standard_False;
    ...
    if (aLink2.SquareModulus() < aSqPrecision) return Standard_False;
    ...
    if (aLink3.SquareModulus() < aSqPrecision) return Standard_False;
    ...
    if (Abs(aD) < gp::Resolution()) return Standard_False;
    ...
    theLocation.ChangeCoord(1) = (aSqMod1 * aLink1.Y() + aSqMod2 * aLink2.Y() + aSqMod3 * aLink3.Y()) * aInvD;

    theLocation.ChangeCoord(2) = (aSqMod1 * aLink1.X() + aSqMod2 * aLink2.X() + aSqMod3 * aLink3.X()) * aInvD;

    theRadius = Sqrt(Max(Max((thePoint1 - theLocation).SquareModulus(), (thePoint2 - theLocation).SquareModulus()),
...
// purpose  :
//=======================================================================
Standard_Boolean BRepMesh_CircleTool::Bind(const Standard_Integer theIndex, const gp_XY& thePoint1,
                                           const gp_XY& thePoint2, const gp_XY& thePoint3) {
    gp_XY aLocation;
    Standard_Real aRadius;
    if (!MakeCircle(thePoint1, thePoint2, thePoint3, aLocation, aRadius)) return Standard_False;
...
void BRepMesh_CircleTool::Delete(const Standard_Integer theIndex) {
    ...
    if (aCircle.Radius() > 0.) aCircle.SetRadius(-1);
}
