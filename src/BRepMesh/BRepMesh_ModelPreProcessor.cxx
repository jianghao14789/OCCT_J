...
public:
    //! Constructor
    TriangulationConsistency(const Handle(IMeshData_Model) & theModel, const Standard_Boolean theAllowQualityDecrease)
        : myModel(theModel), myAllowQualityDecrease(theAllowQualityDecrease) {}
    ...
    void operator()(const Standard_Integer theFaceIndex) const {
        ...
        const Handle(Poly_Triangulation) & aTriangulation = BRep_Tool::Triangulation(aDFace->GetFace(), aLoc);
        ...
        if (!aTriangulation.IsNull()) {
            ...
            Standard_Boolean isTriangulationConsistent =
                BRepMesh_Deflection::IsConsistent(aDeflection, aDFace->GetDeflection(), myAllowQualityDecrease);
...
public:
    //! Constructor
    SeamEdgeAmplifier(const Handle(IMeshData_Model) & theModel, const IMeshTools_Parameters& theParameters)
        : myModel(theModel), myParameters(theParameters) {}
...
private:
    ...
    Standard_Boolean splitEdge(const IMeshData::IEdgePtr& theDEdge, const IMeshData::IFaceHandle& theDFace,
    ...
    Standard_Boolean splitCurve(GeomCurve& theGeomCurve, Curve& theCurve, const Standard_Real theDT) const {
        ...
        for (Standard_Integer aPointIdx = 1;; ++aPointIdx) {
            ...
            theCurve->InsertPoint(theCurve->ParametersNb() - 1, theGeomCurve->Value(aCurrParam), aCurrParam);
...
// Purpose :
//=======================================================================
Standard_Boolean BRepMesh_ModelPreProcessor::performInternal(const Handle(IMeshData_Model) & theModel,
