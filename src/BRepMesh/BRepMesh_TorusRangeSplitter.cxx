...
Handle(IMeshData::ListOfPnt2d)
    BRepMesh_TorusRangeSplitter::GenerateSurfaceNodes(const IMeshTools_Parameters& theParameters) const {
    ...
    const Standard_Real oldDv = GCPnts_TangentialDeflection::ArcAngularStep(r, GetDFace()->GetDeflection(),
    ...
    if (ru > 1.e-16) {
        Du = GCPnts_TangentialDeflection::ArcAngularStep(ru, GetDFace()->GetDeflection(), theParameters.Angle,
    ...
    const std::pair<Standard_Real, Standard_Real> aNewRangeU(aRangeU.first + Du * 0.1, aRangeU.second - Du * 0.1);

    const std::pair<Standard_Real, Standard_Real> aNewRangeV(aRangeV.first + Dv * 0.1, aRangeV.second - Dv * 0.1);
...
Handle(IMeshData::SequenceOfReal) BRepMesh_TorusRangeSplitter::fillParams(
    const IMeshData::IMapOfReal& theParams, const std::pair<Standard_Real, Standard_Real>& theRange,
