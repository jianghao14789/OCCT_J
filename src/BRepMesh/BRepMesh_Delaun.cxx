...
public:
    ComparatorOfIndexedVertexOfDelaun(const Handle(BRepMesh_DataStructureOfDelaun) & theDS) : myStructure(theDS) {}
...
BRepMesh_Delaun::BRepMesh_Delaun(const Handle(BRepMesh_DataStructureOfDelaun) & theOldMesh,
                                 const Standard_Integer theCellsCountU, const Standard_Integer theCellsCountV,
                                 const Standard_Boolean isFillCircles)
    : myMeshData(theOldMesh), myCircles(new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE)), mySupVert(3),
...
BRepMesh_Delaun::BRepMesh_Delaun(IMeshData::Array1OfVertexOfDelaun& theVertices)
    : myCircles(theVertices.Length(), new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE)), mySupVert(3),
      myInitCircles(Standard_False) {
    if (theVertices.Length() > 2) {
        myMeshData = new BRepMesh_DataStructureOfDelaun(new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE),
                                                        theVertices.Length());
        Init(theVertices);
    }
}
...
BRepMesh_Delaun::BRepMesh_Delaun(const Handle(BRepMesh_DataStructureOfDelaun) & theOldMesh,
    ...
    : myMeshData(theOldMesh),
      myCircles(theVertexIndices.Length(), new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE)),
      mySupVert(3), myInitCircles(Standard_False) {
...
//=======================================================================
BRepMesh_Delaun::BRepMesh_Delaun(const Handle(BRepMesh_DataStructureOfDelaun) & theOldMesh,
                                 IMeshData::VectorOfInteger& theVertexIndices, const Standard_Integer theCellsCountU,
...
// purpose  :
//=======================================================================
void BRepMesh_Delaun::InitCirclesTool(const Standard_Integer theCellsCountU, const Standard_Integer theCellsCountV) {
    Bnd_Box2d aBox;
    ...
    for (; aTriangleIt.More(); aTriangleIt.Next()) {
        ...
        myCircles.Bind(aTriangleIt.Key(), GetVertex(aNodesIndices[0]).Coord(), GetVertex(aNodesIndices[1]).Coord(),
...
// purpose  :
//=======================================================================
void BRepMesh_Delaun::initCirclesTool(const Bnd_Box2d& theBox, const Standard_Integer theCellsCountU,
                                      const Standard_Integer theCellsCountV) {
    ...
    myCircles.SetCellSize(aDeltaX / Max(theCellsCountU, aScaler), aDeltaY / Max(theCellsCountV, aScaler));
...
void BRepMesh_Delaun::superMesh(const Bnd_Box2d& theBox) {
    ...
    mySupVert.Append(myMeshData->AddNode(BRepMesh_Vertex((aMinX + aMaxX) / 2, aMaxY + aDeltaMax, BRepMesh_Free)));

    mySupVert.Append(myMeshData->AddNode(BRepMesh_Vertex(aMinX - aDelta, aMinY - aDeltaMin, BRepMesh_Free)));

    mySupVert.Append(myMeshData->AddNode(BRepMesh_Vertex(aMaxX + aDelta, aMinY - aDeltaMin, BRepMesh_Free)));
    ...
    for (Standard_Integer aNodeId = 0; aNodeId < 3; ++aNodeId) {
        ...
        Standard_Integer aLinkIndex =
            myMeshData->AddLink(BRepMesh_Edge(mySupVert[aFirstNode], mySupVert[aLastNode], BRepMesh_Free));
...
//=======================================================================
void BRepMesh_Delaun::deleteTriangle(const Standard_Integer theIndex, IMeshData::MapOfIntegerInteger& theLoopEdges) {
    if (myInitCircles) {
...
void BRepMesh_Delaun::compute(IMeshData::VectorOfInteger& theVertexIndexes) {
    ...
    Handle(NCollection_IncAllocator) aAllocator = new NCollection_IncAllocator(IMeshData::MEMORY_BLOCK_SIZE_HUGE);
...
void BRepMesh_Delaun::RemoveAuxElements() {
    ...
    for (; aFreeEdges.More(); aFreeEdges.Next()) {
        if (myMeshData->ElementsConnectedTo(aFreeEdges.Key()).IsEmpty()) myMeshData->RemoveLink(aFreeEdges.Key());
    }
...
//=======================================================================
void BRepMesh_Delaun::createTriangles(const Standard_Integer theVertexIndex, IMeshData::MapOfIntegerInteger& thePoly) {
    ...
    for (; anEdges.More(); anEdges.Next()) {
        ...
        if (anEdgeLen < Precision) continue;
        ...
        Standard_Integer anEdgesInfo[3] = {myMeshData->AddLink(aFirstLink), isPositive ? anEdgeId : -anEdgeId,
    ...
    while (!anExternalEdges.IsEmpty()) {
        const BRepMesh_PairOfIndex& aPair = myMeshData->ElementsConnectedTo(Abs(anExternalEdges.First()));

        if (!aPair.IsEmpty()) deleteTriangle(aPair.FirstIndex(), thePoly);

        anExternalEdges.RemoveFirst();
    }

    for (anEdges.Initialize(thePoly); anEdges.More(); anEdges.Next()) {
        if (myMeshData->ElementsConnectedTo(anEdges.Key()).IsEmpty()) myMeshData->RemoveLink(anEdges.Key());
    }
        ...
        for (; aCircleIt.More(); aCircleIt.Next()) {
            ...
            if (Contains(aCircleIt.Value(), aVertex, aSqTol, onEgdeId)) {
                if (onEgdeId != 0 && GetEdge(onEgdeId).Movability() != BRepMesh_Free) {
                    ...
                    if (aVertex.Movability() == BRepMesh_Free) continue;
...
Standard_Boolean BRepMesh_Delaun::isBoundToFrontier(const Standard_Integer theRefNodeId,
    ...
    while (!aLinkStack.empty()) {
        ...
        if (aPair.IsEmpty()) return Standard_False;
        ...
        for (Standard_Integer anElemIt = 1; anElemIt <= aNbElements; ++anElemIt) {
            ...
            for (Standard_Integer anEdgeIt = 0; anEdgeIt < 3; ++anEdgeIt) {
                ...
                if (anEdgeId == aCurrentLinkId) continue;
                ...
                if (anEdge.Movability() != BRepMesh_Free) return Standard_True;
...
void BRepMesh_Delaun::cleanupMesh() {
    ...
    for (;;) {
        ...
        for (; aFreeEdgesIt.More(); aFreeEdgesIt.Next()) {
            ...
            if (anEdge.Movability() == BRepMesh_Frontier) continue;
            ...
            for (Standard_Integer aCurEdgeIdx = 0; aCurEdgeIdx < 3; ++aCurEdgeIdx) {
                if (anEdges[aCurEdgeIdx] != aFreeEdgeId) continue;
                ...
                for (Standard_Integer anOtherEdgeIt = 1; anOtherEdgeIt <= 2 && isCanNotBeRemoved; ++anOtherEdgeIt) {
                    ...
                    if (anOtherEdgePair.Extent() < 2) {
                        ...
                        for (int aTriIdx = 1; aTriIdx <= anOtherEdgePair.Extent() && isCanNotBeRemoved; ++aTriIdx) {
                            if (anOtherEdgePair.Index(aTriIdx) == aTriId) continue;
            ...
            for (Standard_Integer aLinkNodeIt = 0; aLinkNodeIt < 2; ++aLinkNodeIt) {
                isConnected[aLinkNodeIt] =
                    isBoundToFrontier((aLinkNodeIt == 0) ? anEdge.FirstNode() : anEdge.LastNode(), aFreeEdgeId);
            }

            if (!isConnected[0] || !isConnected[1]) aDelTriangles.Add(aTriId);
        }
        ...
        if (aDeletedTrianglesNb == 0) break;
    }
}
...
void BRepMesh_Delaun::frontierAdjust() {
    ...
    for (Standard_Integer aPass = 1; aPass <= 2; ++aPass) {
        ...
        for (; aLoopEdgesIt.More(); aLoopEdgesIt.Next()) {
            ...
            if (myMeshData->ElementsConnectedTo(aLoopEdgeId).IsEmpty()) myMeshData->RemoveLink(aLoopEdgeId);
        }
        ...
        for (aFrontierIt.Reset(); aFrontierIt.More(); aFrontierIt.Next()) {
            Standard_Integer aFrontierId = aFrontierIt.Key();
            if (!myMeshData->ElementsConnectedTo(aFrontierId).IsEmpty()) continue;

            Standard_Boolean isSuccess = meshLeftPolygonOf(aFrontierId, Standard_True, aIntFrontierEdges);

            if (aPass == 2 && !isSuccess) aFailedFrontiers.Append(aFrontierId);
        }
    }
...
//=======================================================================
void BRepMesh_Delaun::fillBndBox(IMeshData::SequenceOfBndB2d& theBoxes, const BRepMesh_Vertex& theV1,
...
Standard_Boolean BRepMesh_Delaun::meshLeftPolygonOf(const Standard_Integer theStartEdgeId,
    ...
    if (!theSkipped.IsNull() && theSkipped->Contains(theStartEdgeId)) return Standard_True;
    ...
    if (aRefLinkDir.SquareMagnitude() < Precision2) return Standard_True;
    ...
    while (aPivotNode != aFirstNode) {
        ...
        Standard_Integer aNextLinkId = findNextPolygonLink(aFirstNode, aPivotNode, aPivotVertex, aRefLinkDir, aBoxes,
                                                           aPolygon, theSkipped, isSkipLeprous, aLeprousLinks,
                                                           aDeadLinks, aNextPivotNode, aNextLinkDir, aNextLinkBndBox);
        ...
        if (aNextLinkId != 0) {
            ...
            // Nothing to do
            if (aPolygon.Length() == 1) return Standard_False;
    ...
    if (aPolygon.Length() < 3) return Standard_False;
...
Standard_Integer BRepMesh_Delaun::findNextPolygonLink(
    const Standard_Integer& theFirstNode, const Standard_Integer& thePivotNode, const BRepMesh_Vertex& thePivotVertex,
    const gp_Vec2d& theRefLinkDir, const IMeshData::SequenceOfBndB2d& theBoxes,
    const IMeshData::SequenceOfInteger& thePolygon, const Handle(IMeshData::MapOfInteger) theSkipped,
    const Standard_Boolean& isSkipLeprous, IMeshData::MapOfInteger& theLeprousLinks,
    IMeshData::MapOfInteger& theDeadLinks, Standard_Integer& theNextPivotNode, gp_Vec2d& theNextLinkDir,
    ...
    for (; aLinkIt.More(); aLinkIt.Next()) {
        ...
        if (isSkipLeprous && isLeprous) continue;
        ...
        if (anOtherNode == thePivotNode) anOtherNode = aNeighbourLink.LastNode();
        ...
        if (!isLeprous) theLeprousLinks.Add(aNeighbourLinkId);
        ...
        if (anAngle <= aMaxAngle) continue;

        Standard_Boolean isCheckEndPoints = (anOtherNode != theFirstNode);

        Bnd_B2d aBox;
        Standard_Boolean isNotIntersect = checkIntersection(aNeighbourLink, thePolygon, theBoxes, isCheckEndPoints,
                                                            isCheckPointOnEdge, Standard_True, aBox);
        ...
        if (isNotIntersect) {
            ...
            aNextLinkId = (aNeighbourLink.FirstNode() == thePivotNode) ? aNeighbourLinkId : -aNeighbourLinkId;
        }
    }
...
Standard_Boolean BRepMesh_Delaun::checkIntersection(
    const BRepMesh_Edge& theLink, const IMeshData::SequenceOfInteger& thePolygon,
    const IMeshData::SequenceOfBndB2d& thePolyBoxes, const Standard_Boolean isConsiderEndPointTouch,
    const Standard_Boolean isConsiderPointOnEdge, const Standard_Boolean isSkipLastEdge, Bnd_B2d& theLinkBndBox) const {
    UpdateBndBox(GetVertex(theLink.FirstNode()).Coord(), GetVertex(theLink.LastNode()).Coord(), theLinkBndBox);

    Standard_Integer aPolyLen = thePolygon.Length();
    // Don't check intersection with the last link
    if (isSkipLastEdge) --aPolyLen;
    ...
    for (Standard_Integer aPolyIt = 1; aPolyIt <= aPolyLen; ++aPolyIt) {
        if (!theLinkBndBox.IsOut(thePolyBoxes.Value(aPolyIt))) {
            ...
            if (aPolyLink.Movability() == BRepMesh_Frontier && isFrontier) continue;

            gp_Pnt2d anIntPnt;
            BRepMesh_GeomTool::IntFlag aIntFlag =
                intSegSeg(theLink, aPolyLink, isConsiderEndPointTouch, isConsiderPointOnEdge, anIntPnt);

            if (aIntFlag != BRepMesh_GeomTool::NoIntersection) return Standard_False;
        }
    }
...
//=======================================================================
void BRepMesh_Delaun::addTriangle(const Standard_Integer (&theEdgesId)[3], const Standard_Boolean (&theEdgesOri)[3],
                                  const Standard_Integer (&theNodesId)[3]) {
    Standard_Integer aNewTriangleId = myMeshData->AddElement(BRepMesh_Triangle(theEdgesId, theEdgesOri, BRepMesh_Free));

    Standard_Boolean isAdded = Standard_True;
    if (myInitCircles) {
        isAdded = myCircles.Bind(aNewTriangleId, GetVertex(theNodesId[0]).Coord(), GetVertex(theNodesId[1]).Coord(),
                                 GetVertex(theNodesId[2]).Coord());
    }

    if (!isAdded) myMeshData->RemoveElement(aNewTriangleId);
}
...
void BRepMesh_Delaun::cleanupPolygon(const IMeshData::SequenceOfInteger& thePolygon,
    ...
    if (aPolyVertices.First() != aPolyVertices.Last()) aPolyVertices.Append(aPolyVertices.First());
    ...
    for (; aPolyVertIt < anUniqueVerticesNum; ++aPolyVertIt) {
        killTrianglesAroundVertex(aPolyVertices(aPolyVertIt), aPolyVertices, aPolyVerticesFindMap, thePolygon,
    ...
    for (; aLoopEdgesIt.More(); aLoopEdgesIt.Next()) {
        ...
        if (anIgnoredEdges.Contains(aLoopEdgeId)) continue;

        if (myMeshData->ElementsConnectedTo(aLoopEdgeId).IsEmpty()) myMeshData->RemoveLink(aLoopEdgesIt.Key());
    }
}
...
//=======================================================================
void BRepMesh_Delaun::killTrianglesAroundVertex(const Standard_Integer theZombieNodeId,
    ...
    for (; aNeighborsIt.More(); aNeighborsIt.Next()) {
        ...
        if (theSurvivedLinks.Contains(aNeighborLinkId)) continue;
        ...
        if (aNeighborLink.Movability() == BRepMesh_Frontier) {
            ...
            // take its triangles
            Bnd_B2d aBox;
            Standard_Boolean isNotIntersect = checkIntersection(aNeighborLink, thePolygon, thePolyBoxes, Standard_False,
                                                                Standard_True, Standard_False, aBox);
            ...
            if (anOtherNode == theZombieNodeId) anOtherNode = aNeighborLink.LastNode();
            ...
            if (!thePolyVerticesFindMap.Contains(anOtherNode)) {
                if (isVertexInsidePolygon(anOtherNode, thePolyVertices)) {
                    ...
                    // Let's check it!
                    killTrianglesOnIntersectingLinks(aNeighborLinkId, aNeighborLink, anOtherNode, thePolygon,
    ...
    for (; aVictimIt.More(); aVictimIt.Next()) {
        killTrianglesAroundVertex(aVictimIt.Value(), thePolyVertices, thePolyVerticesFindMap, thePolygon, thePolyBoxes,
...
//=======================================================================
Standard_Boolean BRepMesh_Delaun::isVertexInsidePolygon(const Standard_Integer& theVertexId,
                                                        const IMeshData::VectorOfInteger& thePolygonVertices) const {
    Standard_Integer aPolyLen = thePolygonVertices.Length();
    if (aPolyLen < 3) return Standard_False;
    ...
    if (aPrevVertexDir.SquareMagnitude() < Precision2) return Standard_True;
    ...
    for (Standard_Integer aPolyIt = 1; aPolyIt < aPolyLen; ++aPolyIt) {
        ...
        if (aCurVertexDir.SquareMagnitude() < Precision2) return Standard_True;

        aTotalAng += aCurVertexDir.Angle(aPrevVertexDir);
        aPrevVertexDir = aCurVertexDir;
    }

    if (Abs(Angle2PI - aTotalAng) > Precision::Angular()) return Standard_False;
...
void BRepMesh_Delaun::killTrianglesOnIntersectingLinks(
    const Standard_Integer& theLinkToCheckId, const BRepMesh_Edge& theLinkToCheck, const Standard_Integer& theEndPoint,
    const IMeshData::SequenceOfInteger& thePolygon, const IMeshData::SequenceOfBndB2d& thePolyBoxes,
    IMeshData::MapOfInteger& theSurvivedLinks, IMeshData::MapOfIntegerInteger& theLoopEdges) {
    if (theSurvivedLinks.Contains(theLinkToCheckId)) return;

    Bnd_B2d aBox;
    Standard_Boolean isNotIntersect = checkIntersection(theLinkToCheck, thePolygon, thePolyBoxes, Standard_False,
                                                        Standard_False, Standard_False, aBox);

    theSurvivedLinks.Add(theLinkToCheckId);

    if (isNotIntersect) return;
    ...
    for (; aNeighborsIt.More(); aNeighborsIt.Next()) {
        ...
        const BRepMesh_Edge& aNeighborLink = GetEdge(aNeighborLinkId);
        Standard_Integer anOtherNode = aNeighborLink.FirstNode();
        if (anOtherNode == theEndPoint) anOtherNode = aNeighborLink.LastNode();

        killTrianglesOnIntersectingLinks(aNeighborLinkId, aNeighborLink, anOtherNode, thePolygon, thePolyBoxes,
                                         theSurvivedLinks, theLoopEdges);
    }
}
...
//           its orientation.
//=======================================================================
void BRepMesh_Delaun::getOrientedNodes(const BRepMesh_Edge& theEdge, const Standard_Boolean isForward,
...
//=======================================================================
void BRepMesh_Delaun::processLoop(const Standard_Integer theLinkFrom, const Standard_Integer theLinkTo,
                                  const IMeshData::SequenceOfInteger& thePolygon,
                                  const IMeshData::SequenceOfBndB2d& thePolyBoxes) {
    Standard_Integer aNbOfLinksInLoop = theLinkTo - theLinkFrom - 1;
    if (aNbOfLinksInLoop < 3) return;
...
//           given polygon.
//=======================================================================
Standard_Integer BRepMesh_Delaun::createAndReplacePolygonLink(const Standard_Integer* theNodes, const gp_Pnt2d* thePnts,
    ...
    Standard_Integer aNewEdgeId = myMeshData->AddLink(BRepMesh_Edge(theNodes[0], theNodes[1], BRepMesh_Free));
...
// purpose  :
//=======================================================================
void BRepMesh_Delaun::meshPolygon(IMeshData::SequenceOfInteger& thePolygon, IMeshData::SequenceOfBndB2d& thePolyBoxes,
                                  Handle(IMeshData::MapOfInteger) theSkipped) {
    // Check is the source polygon elementary
    if (meshElementaryPolygon(thePolygon)) return;
    ...
    for (Standard_Integer aPolyIt = 1; aPolyIt < aPolyLen; ++aPolyIt) {
        ...
        if (aCurEdge->Movability() != BRepMesh_Frontier) continue;
        ...
        for (; aNextPolyIt <= aPolyLen; ++aNextPolyIt) {
            ...
            if (aNextEdge->Movability() != BRepMesh_Frontier) continue;
            ...
            if (aIntFlag == BRepMesh_GeomTool::NoIntersection) continue;
            ...
            if (aIntFlag == BRepMesh_GeomTool::Cross) {
                ...
                if (Abs(aLoopArea) > aSmallLoopArea) {
                    ...
                    aNextEdgeId = Abs(createAndReplacePolygonLink(aNextNodes, aNextPnts, aNextPolyIt,
                                                                  BRepMesh_Delaun::Replace, thePolygon, thePolyBoxes));
                ...
                if (isCloseToStart) --aIndexToRemoveTo;
                ...
                if (isFirstChopping) {
                    ...
                    for (; aSplitLinkIt < 2; ++aSplitLinkIt) {
                        createAndReplacePolygonLink(&aSplitLink[aSplitLinkIt], &aSplitPnts[aSplitLinkIt], aNextPolyIt,
                    ...
                    createAndReplacePolygonLink(aSplitLinkNodes, aSplitLinkPnts, aPolyIt, BRepMesh_Delaun::InsertAfter,
                                                thePolygon, thePolyBoxes);
            ...
            if (isAddReplacingEdge) {
                aCurEdgeId = Abs(createAndReplacePolygonLink(aCurNodes, aCurPnts, aPolyIt, BRepMesh_Delaun::Replace,
                                                             thePolygon, thePolyBoxes));
...
//=======================================================================
Standard_Boolean BRepMesh_Delaun::meshElementaryPolygon(const IMeshData::SequenceOfInteger& thePolygon) {
    Standard_Integer aPolyLen = thePolygon.Length();
    if (aPolyLen < 3)
        return Standard_True;
    else if (aPolyLen > 3)
...
void BRepMesh_Delaun::decomposeSimplePolygon(IMeshData::SequenceOfInteger& thePolygon,
    ...
    for (Standard_Integer aLinkIt = 3; aLinkIt <= aPolyLen; ++aLinkIt) {
        ...
        if (aPivotNode == aNodes[1]) continue;
        ...
        if (anAbsDist < Precision || aDist < 0.) continue;
    ...
    Standard_Integer aNewEdgesInfo[3] = {aFirstEdgeInfo, myMeshData->AddLink(aNewEdges[0]),
...
//=======================================================================
void BRepMesh_Delaun::AddVertices(IMeshData::VectorOfInteger& theVertices, const Message_ProgressRange& theRange) {
    ComparatorOfIndexedVertexOfDelaun aCmp(myMeshData);
...
//=======================================================================
Handle(IMeshData::MapOfInteger) BRepMesh_Delaun::getEdgesByType(const BRepMesh_DegreeOfFreedom theEdgeType) const {
...
//           triangle
//=======================================================================
Standard_Real BRepMesh_Delaun::calculateDist(const gp_XY theVEdges[3], const gp_XY thePoints[3],
                                             const BRepMesh_Vertex& theVertex, Standard_Real theDistance[3],
                                             Standard_Real theSqModulus[3], Standard_Integer& theEdgeOn) const {
    Standard_Real aMinDist = RealLast();
    for (Standard_Integer i = 0; i < 3; ++i) {
        theSqModulus[i] = theVEdges[i].SquareModulus();
        if (theSqModulus[i] <= Precision2) return -1;
...
//           of  index <theEdgeOn>
//=======================================================================
Standard_Boolean BRepMesh_Delaun::Contains(const Standard_Integer theTriangleId, const BRepMesh_Vertex& theVertex,
                                           const Standard_Real theSqTolerance, Standard_Integer& theEdgeOn) const {
    theEdgeOn = 0;
    ...
    if (aSqMinDist < 0) return Standard_False;

    const Standard_Boolean isNotFree = (anEdges[aEdgeOnId]->Movability() != BRepMesh_Free);
    if (aSqMinDist > theSqTolerance) {
        if (isNotFree && aDistance[aEdgeOnId] < (aSqModulus[aEdgeOnId] / 5.)) theEdgeOn = e[aEdgeOnId];
    } else if (isNotFree)
        return Standard_False;
    else
...
//=============================================================================
BRepMesh_GeomTool::IntFlag BRepMesh_Delaun::intSegSeg(const BRepMesh_Edge& theEdg1, const BRepMesh_Edge& theEdg2,
    ...
    return BRepMesh_GeomTool::IntSegSeg(p1, p2, p3, p4, isConsiderEndPointTouch, isConsiderPointOnEdge, theIntPnt);
}
...
Standard_CString BRepMesh_DumpPoly(void* thePolygon, void* theMeshHandlePtr, Standard_CString theFileNameStr) {
    ...
    Handle(BRepMesh_DataStructureOfDelaun) aMeshData = *(Handle(BRepMesh_DataStructureOfDelaun)*)theMeshHandlePtr;

    if (aMeshData.IsNull()) return "Error: mesh data is empty";
    ...
    try {
        ...
        for (; aLinksIt.More(); aLinksIt.Next()) {
            ...
            if (aPnt[0].SquareDistance(aPnt[1]) < Precision::SquareConfusion()) continue;

            aBuilder.Add(aMesh, BRepBuilderAPI_MakeEdge(aPnt[0], aPnt[1]));
        }

        if (!BRepTools::Write(aMesh, theFileNameStr)) return "Error: write failed";
