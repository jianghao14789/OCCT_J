...
public:
    ...
    template <class Persistent> StdObjMgt_WriteData& operator<<(const Handle(Persistent) & thePersistent) {
    ...
    template <class Type> StdObjMgt_WriteData& WriteValue(const Type& theValue) {
        *myDriver << theValue;
        return *this;
    }
...
Standard_EXPORT StdObjMgt_WriteData& operator<<(StdObjMgt_WriteData& theWriteData, const Standard_GUID& theGUID);
