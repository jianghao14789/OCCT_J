...
public:
    template <class Transient> class AbstractPersistentBase : public Standard_Transient {
    public:
        virtual Handle(Transient) Import() const = 0;
    };

    template <class TransientT, class Base = StdObjMgt_Persistent> class SharedBase : public Base {
    public:
        ...
        inline const Handle(TransientT) & Import() {
            return myTransient;
        }
...
private:
    template <class Base> class delayedSubBase : public Base {
    public:
        //! Import transient object from the persistent data.
...
public:
    ...
    private:
        template <class T1, class T2> struct DownCast {
            static Handle(T1) make(const Handle(T2) & theT2) {
                return Handle(T1)::DownCast(theT2);
            }
        };

        template <class T> struct DownCast<T, T> {
