...
StepData_StepDumper::StepData_StepDumper(const Handle(StepData_StepModel) & amodel,
                                         const Handle(StepData_Protocol) & protocol, const Standard_Integer mode)
...
Standard_Boolean StepData_StepDumper::Dump(Standard_OStream& S, const Handle(Standard_Transient) & ent,
    ...
    if (level <= 0) {
        ...
        if (thewlib.Select(ent, module, CN)) {
            ...
            S << "(Unrecognized Type for protocol) cdl = " << ent->DynamicType()->Name() << " (...);" << std::endl;
...
Standard_Boolean StepData_StepDumper::Dump(Standard_OStream& S, const Standard_Integer num,
                                           const Standard_Integer level) {
