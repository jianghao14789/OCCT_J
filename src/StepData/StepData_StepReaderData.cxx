...
//    sln 04,10.2001. BUC61003. Prevent exception which may occur during reading of complex entity (if entity's items
    ...
    for (Standard_Integer aStringInd = 1; aStringInd <= aResStringSize; ++aStringInd) {
        ...
        if (aChar == '\\' && aStringInd <= aResStringSize - 3) // can contains the control directive
        {
            ...
            // converts followed two hexadecimal character.
                ...
                aTempExtString =
                    TCollection_ExtendedString(aStrForConvert, Standard_False); // pass through without conversion
                isConverted = Standard_True;
                aStringInd += 4;
            }
            ...
            if (isConverted) // find the control directive
            {
                ...
                {
                    aResStringSize -= aStringInd - aSetCharInd - aTempExtString.Length() +
                                      1; // change the string size to remove unused symbols
        ...
        if (aStringInd <= aResStringSize - 1) {
            ...
            if (aCharNext == aChar && (aChar == '\'' || aChar == '\\')) {
                aResString.SetValue(aSetCharInd,
                                    aResString.ToExtString() + aStringInd); // move the string,removing one symbol
                ...
                if (isDirective) {
                    if (aCharNext == 'N') {
                        aResString.SetValue(aSetCharInd++, '\n');
                        aResString.SetValue(aSetCharInd, aResString.ToExtString() + aStringInd +
                                                             2); // move the string,removing two symbols
...
StepData_StepReaderData::StepData_StepReaderData(const Standard_Integer nbheader, const Standard_Integer nbtotal,
                                                 const Standard_Integer nbpar,
                                                 const Resource_FormatType theSourceCodePage)
    : Interface_FileReaderData(nbtotal, nbpar), theidents(1, nbtotal), thetypes(1, nbtotal),
...
void StepData_StepReaderData::SetRecord(const Standard_Integer num, const Standard_CString ident,
                                        const Standard_CString type, const Standard_Integer /* nbpar */) {
...
void StepData_StepReaderData::AddStepParam(const Standard_Integer num, const Standard_CString aval,
                                           const Interface_ParamType atype, const Standard_Integer nument) {
...
Standard_Integer StepData_StepReaderData::SubListNumber(const Standard_Integer num, const Standard_Integer nump,
...
void StepData_StepReaderData::ComplexType(const Standard_Integer num, TColStd_SequenceOfAsciiString& types) const {
...
Standard_Integer StepData_StepReaderData::NextForComplex(const Standard_Integer num) const {
    ...
    if (themults.IsBound(num)) next = themults.Find(num);
    return next;
}
...
Standard_Boolean StepData_StepReaderData::NamedForComplex(const Standard_CString name, const Standard_Integer num0,
...
Standard_Boolean StepData_StepReaderData::NamedForComplex(const Standard_CString theName,
    ...
    if ((n != 0) && (!strcmp(RecordType(n).ToCString(), theName) || !strcmp(RecordType(n).ToCString(), theShortName))) {
        num = n;
        return Standard_True;
    }
    ...
    for (n = num0; n > 0; n = NextForComplex(n)) {
        if (!strcmp(RecordType(n).ToCString(), theName) || !strcmp(RecordType(n).ToCString(), theShortName)) {
            num = n;
            errmess = new String("Complex Record n0.%d, member type %s not in alphabetic order");
            sprintf(txtmes, errmess->ToCString(), num0, theName);
            ach->AddWarning(txtmes, errmess->ToCString());
...
Standard_Boolean StepData_StepReaderData::CheckNbParams(const Standard_Integer num, const Standard_Integer nbreq,
...
Standard_Boolean StepData_StepReaderData::ReadSubList(const Standard_Integer num, const Standard_Integer nump,
                                                      const Standard_CString mess, Handle(Interface_Check) & ach,
                                                      Standard_Integer& numsub, const Standard_Boolean optional,
...
Standard_Integer StepData_StepReaderData::ReadSub(const Standard_Integer numsub, const Standard_CString mess,
                                                  Handle(Interface_Check) & ach, const Handle(StepData_PDescr) & descr,
...
Standard_Boolean StepData_StepReaderData::ReadMember(const Standard_Integer num, const Standard_Integer nump,
                                                     const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadField(const Standard_Integer num, const Standard_Integer nump,
                                                    const Standard_CString mess, Handle(Interface_Check) & ach,
                                                    const Handle(StepData_PDescr) & descr, StepData_Field& fild) const {
    const Interface_FileParameter& FP = Param(num, nump);
...
Standard_Boolean StepData_StepReaderData::ReadList(const Standard_Integer num, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadAny(const Standard_Integer num, const Standard_Integer nump,
                                                  const Standard_CString mess, Handle(Interface_Check) & ach,
                    ...
                    if (nbp2 > 1) {
                        if (Param(numsub2, 1).ParamType() == Interface_ParamReal) {
                            ...
                            for (Standard_Integer i = 1; i <= nbp2; i++) {
                                ...
                                if (!sm1.IsNull()) aSeq->Append(sm1->Real());
                            }
...
Standard_Boolean StepData_StepReaderData::ReadXY(const Standard_Integer num, const Standard_Integer nump,
                                                 const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadXYZ(const Standard_Integer num, const Standard_Integer nump,
                                                  const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadReal(const Standard_Integer num, const Standard_Integer nump,
                                                   const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadEntity(const Standard_Integer num, const Standard_Integer nump,
                                                     const Standard_CString mess, Handle(Interface_Check) & ach,
        ...
        if (FP.ParamType() == Interface_ParamIdent) {
            ...
            if (nent > 0) {
                ...
                if (entent.IsNull() || !entent->IsKind(atype)) {
                    ...
                    if (!entent.IsNull() && entent->IsKind(STANDARD_TYPE(StepData_UndefinedEntity))) ent = entent;
                } else
                    ent = entent;
            } else
...
Standard_Boolean StepData_StepReaderData::ReadInteger(const Standard_Integer num, const Standard_Integer nump,
                                                      const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadBoolean(const Standard_Integer num, const Standard_Integer nump,
                                                      const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadLogical(const Standard_Integer num, const Standard_Integer nump,
                                                      const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadString(const Standard_Integer num, const Standard_Integer nump,
                                                     const Standard_CString mess, Handle(Interface_Check) & ach,
...
Standard_Boolean StepData_StepReaderData::ReadEnumParam(const Standard_Integer num, const Standard_Integer nump,
                                                        const Standard_CString mess, Handle(Interface_Check) & ach,
...
void StepData_StepReaderData::FailEnumValue(const Standard_Integer /* num */, const Standard_Integer nump,
                                            const Standard_CString mess, Handle(Interface_Check) & ach) const {
...
Standard_Boolean StepData_StepReaderData::ReadEnum(const Standard_Integer num, const Standard_Integer nump,
                                                   const Standard_CString mess, Handle(Interface_Check) & ach,
                                                   const StepData_EnumTool& enumtool, Standard_Integer& val) const {
    ...
    if (nump > 0 && nump <= NbParams(num)) {
        ...
        if (FP.ParamType() == Interface_ParamEnum) {
            ...
            if (val < 0) errmess = new String("Parameter n0.%d (%s) : Undefined Enumeration not allowed");
            warn = (acceptvoid > 0);
        } else
            errmess = new String("Parameter n0.%d (%s) not an Enumeration");
    } else
...
Standard_Boolean StepData_StepReaderData::ReadTypedParam(const Standard_Integer num, const Standard_Integer nump,
                                                         const Standard_Boolean mustbetyped,
                                                         const Standard_CString mess, Handle(Interface_Check) & ach,
                                                         Standard_Integer& numr, Standard_Integer& numrp,
    ...
    if (nump > 0 && nump <= NbParams(num)) {
        ...
        numrp = 1;
        if (NbParams(numr) != 1) errmess = new String("Parameter n0.%d (%s) : SubList, not typed");
        typ = RecordType(numr);
    } else
        errmess = new String("Parameter n0.%d (%s) absent");
...
Standard_Boolean StepData_StepReaderData::CheckDerived(const Standard_Integer num, const Standard_Integer nump,
                                                       const Standard_CString mess, Handle(Interface_Check) & ach,
        ...
        for (Standard_Integer na = nba; na > 0; na--) {
            ...
            if (letype == Interface_ParamSub) {
                ...
                if (nok > 0) {
                    ...
                    //  ...  Construire le Check  ...
                    sprintf(failmess, "Unresolved Reference, Ent.n0 %d (Id.#%d) Param.n0 %d (Id.#%d)", nument, ident,
                            na, id);
                    thecheck->AddFail(failmess, "Unresolved Reference");

                    //  ...  Et sortir message un peu plus complet
                    sout << "*** ERR StepReaderData *** Entite " << nument << ", a " << (nr * 100) / nbseq
                         << "% de DATA : #" << ident << "\n    Type:" << RecordType(num) << "  Param.n0 " << na << ": #"
