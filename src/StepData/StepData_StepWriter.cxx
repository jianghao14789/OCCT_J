...
StepData_StepWriter::StepData_StepWriter(const Handle(StepData_StepModel) & amodel) : thecurr(StepLong), thefloatw(12) {
    themodel = amodel;
...
void StepData_StepWriter::SendModel(const Handle(StepData_Protocol) & protocol, const Standard_Boolean headeronly) {
    StepData_WriterLib lib(protocol);

    if (!headeronly) thefile->Append(new TCollection_HAsciiString("ISO-10303-21;"));
    SendHeader();
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (!thescopebeg.IsNull()) {
            if (thescopenext->Value(i) != 0) continue;
        }
        SendEntity(i, lib);
    }
...
void StepData_StepWriter::StartEntity(const TCollection_AsciiString& atype) {
    ...
    if (themult) {
        if (thelevel != 1)
            throw Interface_InterfaceMismatch("StepWriter : StartEntity"); // decompte de parentheses mauvais ...
...
void StepData_StepWriter::SendField(const StepData_Field& fild, const Handle(StepData_PDescr) & descr) {
...
void StepData_StepWriter::SendList(const StepData_FieldList& list, const Handle(StepData_ESDescr) & descr) {
    // start entity  ?
...
void StepData_StepWriter::EndEntity() {
    if (thelevel != 1)
        throw Interface_InterfaceMismatch("StepWriter : EndEntity"); // decompte de parentheses mauvais ...
...
void StepData_StepWriter::AddString(const TCollection_AsciiString& astr, const Standard_Integer more) {
...
void StepData_StepWriter::AddString(const Standard_CString astr, const Standard_Integer lnstr,
