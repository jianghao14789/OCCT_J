...
ChFiDS_FilSpine::ChFiDS_FilSpine(const Standard_Real Tol) : ChFiDS_Spine(Tol) {}
...
void ChFiDS_FilSpine::Reset(const Standard_Boolean AllData) {
    ...
    else // Complete parandrad
    {
        ...
        if (IsPeriodic()) parandrad(parandrad.Length()).SetY(parandrad(1).Y());
    }
}
...
void ChFiDS_FilSpine::UnSetRadius(const TopoDS_Edge& E) {
    ...
    for (Standard_Integer i = 1; i <= parandrad.Length(); i++) {
        if (Abs(parandrad(i).X() - Uf) <= gp::Resolution()) ifirst = i;
        if (Abs(parandrad(i).X() - Ul) <= gp::Resolution()) ilast = i;
    }
    if (ifirst != 0 && ilast != 0) parandrad.Remove(ifirst, ilast);
}
...
Standard_Boolean ChFiDS_FilSpine::IsConstant() const {
    if (parandrad.IsEmpty()) return Standard_False;
...
Standard_Boolean ChFiDS_FilSpine::IsConstant(const Standard_Integer IE) const {
    ...
    for (i = 1; i < parandrad.Length(); i++) {
        ...
        if (Abs(Uf - par) <= gp::Resolution() || (par < Uf && Uf < nextpar && nextpar - Uf > gp::Resolution())) {
            StartRad = rad;
            break;
        }
    }
    for (i++; i <= parandrad.Length(); i++) {
        par = parandrad(i).X();
        rad = parandrad(i).Y();
        if (Abs(rad - StartRad) > Precision::Confusion()) return Standard_False;
        if (Abs(Ul - par) <= gp::Resolution()) return Standard_True;
        if (par > Ul) return Standard_True;
    }
...
Standard_Real ChFiDS_FilSpine::Radius(const Standard_Integer IE) const {
    ...
    for (i++; i <= parandrad.Length(); i++) {
        par = parandrad(i).X();
        rad = parandrad(i).Y();
        if (Abs(rad - StartRad) > Precision::Confusion()) throw Standard_DomainError("Edge is not constant");
        if (Abs(Ul - par) <= gp::Resolution()) return StartRad;
        if (par > Ul) return StartRad;
    }
    return StartRad;
}
...
static void mklaw(Law_Laws& res, const TColgp_SequenceOfXY& pr, const Standard_Real curdeb, const Standard_Real curfin,
                  const Standard_Real Rdeb, const Standard_Real Rfin, const Standard_Boolean recadre,
                  const Standard_Real deb, const Standard_Real fin, const Standard_Real tol3d) {
...
Handle(Law_Composite) ChFiDS_FilSpine::ComputeLaw(const Handle(ChFiDS_ElSpine) & Els) {
    ...
    if (Els->IsPeriodic()) {
        ...
        if (k > len) { // no !
            if (parandrad.IsEmpty()) throw Standard_DomainError("Radius not defined");
    ...
    for (; icur <= nbed; icur++) {
        if (IsConstant(ind(icur))) {
            ...
            curfin = LastParameter(ind(icur));
            if (IsPeriodic()) curfin = ElCLib::InPeriod(curfin, spinedeb + tol3d, spinefin + tol3d);
            curfin = Min(fin, curfin);
            lawencours = Standard_True;
            if (ind(icur) == ind(nbed)) {
                // Attention the curfin can be wrong if the last edge passes
...
Standard_Real ChFiDS_FilSpine::MaxRadFromSeqAndLaws() const {
    ...
    for (Standard_Integer i = 1; i <= parandrad.Length(); i++)
        if (parandrad(i).Y() > MaxRad) MaxRad = parandrad(i).Y();
