...
ChFiDS_Spine::ChFiDS_Spine()
    : splitdone(Standard_False), myMode(ChFiDS_ClassicChamfer), indexofcurve(0), myTypeOfConcavity(ChFiDS_Other),
      firstState(ChFiDS_OnSame), lastState(ChFiDS_OnSame), tolesp(Precision::Confusion()), firstparam(0.0),
      lastparam(0.0), firstprolon(Standard_False), lastprolon(Standard_False), firstistgt(Standard_False),
      lastistgt(Standard_False), firsttgtpar(0.0), lasttgtpar(0.0), hasfirsttgt(Standard_False),
      haslasttgt(Standard_False), valref(0.0), hasref(Standard_False), errorstate(ChFiDS_Ok) {}
...
ChFiDS_Spine::ChFiDS_Spine(const Standard_Real Tol)
    : splitdone(Standard_False), myMode(ChFiDS_ClassicChamfer), indexofcurve(0), myTypeOfConcavity(ChFiDS_Other),
      firstState(ChFiDS_OnSame), lastState(ChFiDS_OnSame), tolesp(Tol), firstparam(0.0), lastparam(0.0),
      firstprolon(Standard_False), lastprolon(Standard_False), firstistgt(Standard_False), lastistgt(Standard_False),
      firsttgtpar(0.0), lasttgtpar(0.0), hasfirsttgt(Standard_False), haslasttgt(Standard_False), valref(0.0),
      hasref(Standard_False), errorstate(ChFiDS_Ok) {}
...
#ifdef OCCT_DEBUG
    if (Par >= Precision::Confusion()) std::cout << "Interior extension at the start of guideline" << std::endl;
    if (IsPeriodic()) std::cout << "WARNING!!! Extension on periodic guideline." << std::endl;
#endif
...
void ChFiDS_Spine::SetLastParameter(const Standard_Real Par) {
    ...
    if ((Par - lll) <= -Precision::Confusion()) std::cout << "Interior extension at the end of guideline" << std::endl;
    if (IsPeriodic()) std::cout << "WARNING!!! Extension on periodic guideline." << std::endl;
#endif
...
#ifdef OCCT_DEBUG
    if (W >= Precision::Confusion()) std::cout << "Interior extension at start of the guideline" << std::endl;
#endif
...
void ChFiDS_Spine::SetLastTgt(const Standard_Real W) {
    ...
    if (L <= -Precision::Confusion()) std::cout << "Interior extension at the end of guideline" << std::endl;
#endif
    // The flag is suspended if is already positioned to avoid
    // stopping d1
...
Standard_Integer ChFiDS_Spine::Index(const Standard_Real W, const Standard_Boolean Forward) const {
    ...
    if (IsPeriodic() && Abs(par) >= t && Abs(par - last) >= t) par = ElCLib::InPeriod(par, 0., last);
...
void ChFiDS_Spine::Parameter(const Standard_Real AbsC, Standard_Real& U, const Standard_Boolean Oriented) {
...
void ChFiDS_Spine::Parameter(const Standard_Integer Index, const Standard_Real AbsC, Standard_Real& U,
...
void ChFiDS_Spine::Prepare(Standard_Real& L, Standard_Integer& Ind) const {
    ...
    if (IsPeriodic() && Abs(L) >= tol && Abs(L - last) >= tol) L = ElCLib::InPeriod(L, 0., last);
