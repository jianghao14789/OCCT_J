...
XmlTObjDrivers_IntSparseArrayDriver::XmlTObjDrivers_IntSparseArrayDriver(const Handle(Message_Messenger) &
                                                                         theMessageDriver)
...
Standard_Boolean XmlTObjDrivers_IntSparseArrayDriver::Paste(const XmlObjMgt_Persistent& theSource,
    ...
    while (anItemID.IsIntegerValue() && anItemID.IntegerValue() != 0) {
        TCollection_AsciiString aStrIndex = TCollection_AsciiString(ITEM_VALUE) + TCollection_AsciiString(i);
...
void XmlTObjDrivers_IntSparseArrayDriver::Paste(const Handle(TDF_Attribute) & theSource,
                                                XmlObjMgt_Persistent& theTarget, XmlObjMgt_SRelocationTable&) const {
