...
static Standard_Boolean GetOffset(const LocOpe_PntFace& PntInfo, const Standard_Real Radius, const gp_Ax1& Axis,
...
void BRepFeat_MakeCylindricalHole::PerformThruNext(const Standard_Real Radius, const Standard_Boolean Cont) {
    //
    const TopoDS_Shape& aObject = myArguments.First();
    if (aObject.IsNull() || !myAxDef) {
        throw Standard_ConstructionError();
...
void BRepFeat_MakeCylindricalHole::PerformUntilEnd(const Standard_Real Radius, const Standard_Boolean Cont) {
    //
    const TopoDS_Shape& aObject = myArguments.First();
    if (aObject.IsNull() || !myAxDef) {
        throw Standard_ConstructionError();
...
void BRepFeat_MakeCylindricalHole::Perform(const Standard_Real Radius, const Standard_Real PFrom,
                                           const Standard_Real PTo, const Standard_Boolean Cont) {
    //
    const TopoDS_Shape& aObject = myArguments.First();
    if (aObject.IsNull() || !myAxDef) {
        throw Standard_ConstructionError();
...
void BRepFeat_MakeCylindricalHole::PerformBlind(const Standard_Real Radius, const Standard_Real Length,
...
void BoxParameters(const TopoDS_Shape& S, const gp_Ax1& Axis, Standard_Real& parmin, Standard_Real& parmax) {
...
Standard_Boolean GetOffset(const LocOpe_PntFace& PntInfo, const Standard_Real Radius, const gp_Ax1& Axis,
                           Standard_Real& outOff) {
    ...
    if (stat != CSLib_Defined) return Standard_False;
    Standard_Real angle = Axis.Direction().Angle(NormF);
    if (Abs(M_PI / 2. - angle) < Precision::Angular()) return Standard_False;
...
void CreateCyl(const LocOpe_PntFace& PntInfoFirst, const LocOpe_PntFace& PntInfoLast, const Standard_Real Radius,
    ...
    if (!GetOffset(PntInfoFirst, Radius, Axis, offF)) offF = Radius;
    if (!GetOffset(PntInfoLast, Radius, Axis, offL)) offL = Radius;
