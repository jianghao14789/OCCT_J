...
static void SetGluedFaces(const TopTools_DataMapOfShapeListOfShape& theSlmap, LocOpe_LinearForm&,
...
void BRepFeat_MakeLinearForm::Init(const TopoDS_Shape& Sbase, const TopoDS_Wire& W, const Handle(Geom_Plane) & Plane,
                                   const gp_Vec& Direc, const gp_Vec& Direc1, const Standard_Integer Mode,
...
// Rib is centre in the middle of translation
    ...
    if (Sliding) {
        ...
        if (ext1.NbExt() == 1 &&
            ext1.SquareDistance(1) <= BRep_Tool::Tolerance(FirstFace) * BRep_Tool::Tolerance(FirstFace)) {
            ...
            if (ext2.NbExt() == 1 &&
                ext2.SquareDistance(1) <= BRep_Tool::Tolerance(LastFace) * BRep_Tool::Tolerance(LastFace)) {
                Sliding = Standard_True;
            } else {
                Sliding = Standard_False;
            }
    ...
    if (!myDir1.IsEqual(nulldir, Precision::Confusion(), Precision::Confusion())) {
        if (Sliding) {
            gp_Pnt p1(myFirstPnt.X() + myDir1.X(), myFirstPnt.Y() + myDir1.Y(), myFirstPnt.Z() + myDir1.Z());
            BRepLib_MakeEdge ee1(myFirstPnt, p1);
            BRepExtrema_ExtCF ext1(ee1, FirstFace);
            if (ext1.NbExt() == 1 &&
                ext1.SquareDistance(1) <= BRep_Tool::Tolerance(FirstFace) * BRep_Tool::Tolerance(FirstFace)) {
                gp_Pnt p2(myLastPnt.X() + myDir1.X(), myLastPnt.Y() + myDir1.Y(), myLastPnt.Z() + myDir1.Z());
                BRepLib_MakeEdge ee2(myLastPnt, p2);
...
//=======================================================================
Standard_Boolean BRepFeat_MakeLinearForm::Propagate(TopTools_ListOfShape& SliList, const TopoDS_Face& fac,
                        ...
                        if (result) {
                            ...
                            if (perp.IsDone()) {
                                ...
                                if (perp.IsDone()) {
                                    ...
                                    if (pe1.Distance(pe2) <= BRep_Tool::Tolerance(rfe)) result = Standard_True;
                                }
                            }
                        }
                        break;
...
static void MajMap(const TopoDS_Shape& theB, const LocOpe_LinearForm& theP,
...
static void SetGluedFaces(const TopTools_DataMapOfShapeListOfShape& theSlmap, LocOpe_LinearForm& thePrism,
