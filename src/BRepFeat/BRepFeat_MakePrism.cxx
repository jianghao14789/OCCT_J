...
static Standard_Real HeightMax(const TopoDS_Shape& theSbase, const TopoDS_Face& theSkface, const TopoDS_Shape& theSFrom,
...
void BRepFeat_MakePrism::Init(const TopoDS_Shape& Sbase, const TopoDS_Shape& Pbase, const TopoDS_Face& Skface,
                              const gp_Dir& Direc, const Standard_Integer Mode, const Standard_Boolean Modify) {
#ifdef OCCT_DEBUG
    ...
    if (!mySkface.IsNull() || !mySlface.IsEmpty()) {
        ...
        for (; exp.More(); exp.Next()) {
            ...
            if (ToFuse(ff, FFace)) {
                ...
                if (!FFace.IsSame(myPbase) && BRepFeat::IsInside(ff, FFace)) break;
            }
        }
    }
...
static Standard_Real HeightMax(const TopoDS_Shape& theSbase, const TopoDS_Face& theSkface, const TopoDS_Shape& theSFrom,
    ...
    if (!theSFrom.IsNull()) {
        ...
        if (!FacRevolInfini) BRepBndLib::Add(theSFrom, Box);
    }
    if (!theSUntil.IsNull()) {
        Standard_Boolean FacRevolInfini = Standard_False;
        TopExp_Explorer exp;
        ...
        if (!FacRevolInfini) BRepBndLib::Add(theSUntil, Box);
    }
...
Standard_Integer SensOfPrism(const Handle(Geom_Curve) C, const TopoDS_Shape& Until) {
    ...
    if (ASI1.IsDone() && ASI1.NbPoints(1) >= 1) {
        if (ASI1.Point(1, 1).Parameter() < 0. && ASI1.Point(1, ASI1.NbPoints(1)).Parameter() < 0.) {
            sens = -1;
        }
...
static void MajMap(const TopoDS_Shape& theB, const LocOpe_Prism& theP,
...
static Standard_Boolean ToFuse(const TopoDS_Face& F1, const TopoDS_Face& F2) {
    ...
    if (typS1 == STANDARD_TYPE(Geom_Plane)) {
        ...
        // apply locations
        if (!loc1.IsIdentity()) pl1.Transform(loc1.Transformation());
        if (!loc2.IsIdentity()) pl2.Transform(loc2.Transformation());
