...

    {
        ...
        for (Standard_Integer jj = 1; jj <= scur.Length(); jj++) {
            if (ASI1.IsDone() && ASI2.IsDone()) {
                ...
                if (ASI2.NbPoints(jj) <= 0) continue;
                        ...
                        for (it.Initialize(lshape); it.More(); it.Next()) {
                            ...
                            for (expf.Init(it.Value(), TopAbs_FACE); expf.More(); expf.Next()) {
                                if (mapFuntil.Contains(expf.Current())) {
                                    ...
                                    BRepFeat::ParametricMinMax(expf.Current(), C, prmin1, prmax1, prbmin1, prbmax1,
                                                               flag3);
                                    BRepFeat::ParametricMinMax(it.Value(), C, prmin2, prmax2, prbmin2, prbmax2, flag2);
                                    if (sens == 1) {
            ...
            if (!myJustFeat) {
                ...
                for (it.Initialize(lshape); it.More(); it.Next()) {
                    if (C->IsPeriodic()) {
                        ...
                        BRepFeat::ParametricMinMax(it.Value(), C, pr, prmax1, prb, prbmax1, flag2, Standard_True);
                        if (flag2) {
...
const TopTools_ListOfShape& BRepFeat_Form::Modified(const TopoDS_Shape& F) {
    ...
    if (myMap.IsBound(F)) {
        ...
        for (; ite.More(); ite.Next()) {
            ...
            if (!sh.IsSame(F) && sh.ShapeType() == F.ShapeType()) myGenerated.Append(sh);
        }
    }
...
const TopTools_ListOfShape& BRepFeat_Form::Generated(const TopoDS_Shape& S) {
    myGenerated.Clear();
    if (!IsDone()) return myGenerated;
    if (myMap.IsBound(S) && S.ShapeType() != TopAbs_FACE) { // check if filter on face or not
        TopTools_ListIteratorOfListOfShape ite(myMap(S));
        for (; ite.More(); ite.Next()) {
            const TopoDS_Shape& sh = ite.Value();
            if (!sh.IsSame(S)) myGenerated.Append(sh);
        }
        return myGenerated;
    }
    return myGenerated;
...
// purpose  :
//=======================================================================
void BRepFeat_Form::UpdateDescendants(const BRepAlgoAPI_BooleanOperation& aBOP, const TopoDS_Shape& S,
