...
static void MajMap(const TopoDS_Shape&, const LocOpe_DPrism&,
...
void BRepFeat_MakeDPrism::Init(const TopoDS_Shape& Sbase, const TopoDS_Face& Pbase, const TopoDS_Face& Skface,
                               const Standard_Real Angle, const Standard_Integer Mode, const Standard_Boolean Modify)
    ...
    if (signature < 0) {
        ...
        if (!myShape.IsNull()) {
            ...
            for (itMap.Initialize(MapE); itMap.More(); itMap.Next()) {
                if (!BRep_Tool::Degenerated(TopoDS::Edge(itMap.Key()))) myLatEdges.Append(itMap.Key());
            }
        }
    }
}
...
static void MajMap(const TopoDS_Shape& theB, const LocOpe_DPrism& theP,
