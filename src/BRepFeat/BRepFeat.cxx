...
void BRepFeat::ParametricMinMax(const TopoDS_Shape& S, const Handle(Geom_Curve) & CC, Standard_Real& prmin,
                                Standard_Real& prmax, Standard_Real& prbmin, Standard_Real& prbmax,
                                Standard_Boolean& flag, const Standard_Boolean theOri) {
...
static Standard_Boolean IsIn(BRepTopAdaptor_FClass2d& FC, Geom2dAdaptor_Curve AC) {
...
static void PutInBoundsU(Standard_Real umin, Standard_Real umax, Standard_Real eps, Standard_Real period,
                         Standard_Real f, Standard_Real l, Handle(Geom2d_Curve) & C2d) {
...
static void PutInBoundsV(Standard_Real vmin, Standard_Real vmax, Standard_Real eps, Standard_Real period,
                         Standard_Real f, Standard_Real l, Handle(Geom2d_Curve) & C2d) {
    gp_Pnt2d Pf = C2d->Value(f);
    gp_Pnt2d Pl = C2d->Value(l);
    gp_Pnt2d Pm = C2d->Value(0.34 * f + 0.66 * l);
...
void BRepFeat::FaceUntil(const TopoDS_Shape& Sbase, TopoDS_Face& FUntil) {
    ...
    if (styp == STANDARD_TYPE(Geom_Plane)) {
        ...
        vmin -= diam;
        vmax += diam;
        str = new Geom_RectangularTrimmedSurface(s, umin, umax, vmin, vmax, Standard_True, Standard_True);
...
TopoDS_Solid BRepFeat::Tool(const TopoDS_Shape& SRef, const TopoDS_Face& Fac, const TopAbs_Orientation Orf) {
