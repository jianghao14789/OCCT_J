...
DBRep_HideData::DBRep_HideData() : myView(-1), myFocal(0.0), myAngle(0.0) {}
...
void DBRep_HideData::Set(const Standard_Integer viewID, const gp_Trsf& TProj, const Standard_Real focal,
                         const TopoDS_Shape& S, const Standard_Real ang) {
    myView = viewID;
    myTrsf = TProj;
    myFocal = focal;
    myAngle = ang;
    ...
    for (hider->InitHide(); hider->MoreHide(); hider->NextHide()) {
        ...
        for (It.InitVisible(status); It.MoreVisible(); It.NextVisible()) {
            ...
            myBiPntVis.Append(HLRBRep_BiPoint(PntX1 + sta * dx, PntY1 + sta * dy, PntZ1 + sta * dz, PntX1 + end * dx,
                                              PntY1 + end * dy, PntZ1 + end * dz, Sori, reg1, regn, outl, intl));
        }

        for (It.InitHidden(status); It.MoreHidden(); It.NextHidden()) {
            It.Hidden(sta, tolsta, end, tolend);
            myBiPntHid.Append(HLRBRep_BiPoint(PntX1 + sta * dx, PntY1 + sta * dy, PntZ1 + sta * dz, PntX1 + end * dx,
                                              PntY1 + end * dy, PntZ1 + end * dz, Sori, reg1, regn, outl, intl));
        }
    }
}
...
Standard_Boolean DBRep_HideData::IsSame(const gp_Trsf& TProj, const Standard_Real focal) const {
    if (focal > 0) {
        ...
        for (Standard_Integer i = 1; i <= 3; i++) {
            if (T1.Coord(i) != T2.Coord(i)) return Standard_False;
        }
    }
    ...
    for (Standard_Integer i = 1; i <= 3; i++) {
        ...
        for (Standard_Integer j = 1; j <= 3; j++) {
            if (M1.Value(i, j) != M2.Value(i, j)) return Standard_False;
        }
    }
...
void DBRep_HideData::DrawOn(Draw_Display& D, const Standard_Boolean withRg1, const Standard_Boolean withRgN,
                            const Standard_Boolean withHid, const Draw_Color& VisCol, const Draw_Color& HidCol) {
