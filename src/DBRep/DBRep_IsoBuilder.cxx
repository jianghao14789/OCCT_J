...
DBRep_IsoBuilder::DBRep_IsoBuilder(const TopoDS_Face& TopologicalFace, const Standard_Real Infinite,
                                   const Standard_Integer NbIsos)
    : Geom2dHatch_Hatcher(Geom2dHatch_Intersector(IntersectorConfusion, IntersectorTangency), HatcherConfusion2d,
                          HatcherConfusion3d, Standard_True, Standard_False),
      myInfinite(Infinite), myUMin(0.0), myUMax(0.0), myVMin(0.0), myVMax(0.0), myUPrm(1, NbIsos), myUInd(1, NbIsos),
      myVPrm(1, NbIsos), myVInd(1, NbIsos), myNbDom(0) {
    myUInd.Init(0);
    myVInd.Init(0);
...
void DBRep_IsoBuilder::FillGaps(const TopoDS_Face& theFace, DataMapOfEdgePCurve& theEdgePCurveMap) {
    ...
    for (; aItW.More(); aItW.Next()) {
        ...
        while (!bStop) {
            ...
            if (aPrevEdge.IsEqual(aCurrEdge) && !SingleEdge) continue;
            ...
            if (aCVOnCurr.IsNull() || !aCVOnPrev.IsSame(aCVOnCurr)) continue;
            ...
            if (bAddSegment) {
                if (aBASurf.IsUPeriodic()) bAddSegment = aSegmLen < aBASurf.UPeriod() / 4.;

                if (bAddSegment && aBASurf.IsVPeriodic()) bAddSegment = aSegmLen < aBASurf.VPeriod() / 4.;
            }
                    ...
                    if (bPointFound) {
                        ...
                        for (TopoDS_Iterator it1(aPrevEdge); it1.More(); it1.Next()) {
                            for (TopoDS_Iterator it2(aCurrEdge); it2.More(); it2.Next()) {
                                if (it1.Value().IsSame(it2.Value())) ++aNbCV;
                            }
                        }

                        // Trim PCurves only if the intersection belongs to current parameter
                        Standard_Boolean bTrim = (aNbCV == 1 || (Abs(aTPrev - aTPrevClosest) < (lp - fp) / 2. ||
                        ...
                        if (bTrim) {
                            ...
                            if (aTolV2 > Precision::Infinite() || aPOnS.SquareDistance(aPV) < aTolV2) {
                                ...
                                if (l - f > Precision::PConfusion()) aPrevC2d = new Geom2d_TrimmedCurve(aPrevC2d, f, l);
                                ...
                                if (l - f > Precision::PConfusion()) aCurrC2d = new Geom2d_TrimmedCurve(aCurrC2d, f, l);
