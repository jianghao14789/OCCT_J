...
DBRep_DrawableShape::DBRep_DrawableShape(const TopoDS_Shape& aShape, const Draw_Color& FreeCol,
                                         const Draw_Color& ConnCol, const Draw_Color& EdgeCol,
                                         const Draw_Color& IsosCol, const Standard_Real size,
                                         const Standard_Integer nbisos, const Standard_Integer discret)
    : mySize(size), myDiscret(discret), myFreeCol(FreeCol), myConnCol(ConnCol), myEdgeCol(EdgeCol), myIsosCol(IsosCol),
      myNbIsos(nbisos), myDispOr(Standard_False), mytriangulations(Standard_False), mypolygons(Standard_False),
      myHLR(Standard_False), myRg1(Standard_False), myRgN(Standard_False), myHid(Standard_False) {
    myShape = aShape;
}
...
static void PlotIso(Draw_Display& dis, Handle(DBRep_Face) & F, BRepAdaptor_Surface& S, GeomAbs_IsoType T,
                    Standard_Real& U, Standard_Real& V, Standard_Real Step, Standard_Boolean& halt) {
...
static void PlotEdge(Draw_Display& dis, Handle(DBRep_Edge) & E, const Adaptor3d_Curve& C, Standard_Real& f,
...
void DBRep_DrawableShape::DrawOn(Draw_Display& dis) const {
    ...
    if (myFaces.IsEmpty() || myEdges.IsEmpty()) updateDisplayData();
        ...
        if (!aSurf.IsNull()) {
            ...
            if (aSurf->IsUPeriodic() || aSurf->IsVPeriodic()) {
                ...
                if (aSurf->IsUPeriodic()) {
                    if (FU1 < SU1 || FU1 > SU2) restriction = Standard_True;
                    if (!restriction && (FU2 < SU1 || FU2 > SU2)) restriction = Standard_True;
                }
                if (!restriction && aSurf->IsVPeriodic()) {
                    if (FV1 < SV1 || FV1 > SV2) restriction = Standard_True;
                    if (!restriction && (FV2 < SV1 || FV2 > SV2)) restriction = Standard_True;
                }
                Standard_Boolean zeroS = (fabs(SU2 - SU1) <= 1.e-9 || fabs(SV2 - SV1) <= 1.e-9);
                Standard_Boolean zeroF = (fabs(FU2 - FU1) <= 1.e-9 || fabs(FV2 - FV1) <= 1.e-9);
                if (restriction && (zeroS || zeroF)) restriction = Standard_False;
                if (restriction && (FU1 >= FU2 || FV1 >= FV2)) restriction = Standard_False;
...
// If the type of the surface is GeomAbs_SurfaceOfExtrusion or GeomAbs_SurfaceOfRevolution
            ...
            for (i = 1; i <= N; i++) {
                ...
                for (Intrv = 1; Intrv <= nbIntv; Intrv++) {
                    ...
                    if (TI(Intrv) <= T1 && TI(Intrv + 1) <= T1) continue;
                    if (TI(Intrv) >= T2 && TI(Intrv + 1) >= T2) continue;
                    if (T == GeomAbs_IsoU) {
                        V1 = Max(T1, TI(Intrv));
                    ...
                    switch (SurfType) {
                        ...
                        case GeomAbs_BSplineSurface:
                            for (j = 1; j <= myDiscret / 2; j++) {
                                ...
                                PlotIso(dis, aLocalFace, S, T, U1, V1, (T == GeomAbs_IsoV) ? stepU * 2. : stepV * 2.,
                                        halt);
                                U1 += stepU * 2.;
                                V1 += stepV * 2.;
                            }
    ...
    for (exv.Init(myShape, TopAbs_VERTEX, TopAbs_EDGE); exv.More() && !halt; exv.Next()) {
        ...
        if (myDispOr) dis.SetColor(DBRep_ColorOrientation(exv.Current().Orientation()));
    ...
    while (it.More()) {
        if (it.Value().ViewId() == id) {
            ...
            if (recompute)
                ...
                it.Value().DrawOn(dis, myRg1, myRgN, myHid, myConnCol, myIsosCol);
                if (dis.HasPicked()) {
...
Handle(Draw_Drawable3D) DBRep_DrawableShape::Copy() const {
    Handle(DBRep_DrawableShape) D =
        new DBRep_DrawableShape(myShape, myFreeCol, myConnCol, myEdgeCol, myIsosCol, mySize, myNbIsos, myDiscret);
    return D;
}
...
void DBRep_DrawableShape::DisplayHLR(const Standard_Boolean withHLR, const Standard_Boolean withRg1,
                                     const Standard_Boolean withRgN, const Standard_Boolean withHid,
...
void DBRep_DrawableShape::GetDisplayHLR(Standard_Boolean& withHLR, Standard_Boolean& withRg1, Standard_Boolean& withRgN,
                                        Standard_Boolean& withHid, Standard_Real& ang) const {
    withHLR = myHLR;
    withRg1 = myRg1;
    withRgN = myRgN;
    withHid = myHid;
...
Handle(Draw_Drawable3D) DBRep_DrawableShape::Restore(Standard_IStream& theStream) {
    ...
    Handle(DBRep_DrawableShape) aDrawShape = new DBRep_DrawableShape(
        aTopoShape, Draw_vert, Draw_jaune, Draw_rouge, Draw_bleu, aParams.Size, aParams.NbIsos, aParams.Discretization);
...
void DBRep_DrawableShape::display(const Handle(Poly_Triangulation) & T, const gp_Trsf& tr, Draw_Display& dis) const {
    ...
    for (NCollection_Vector<NCollection_Vec2<Standard_Integer>>::Iterator anInterIter(anInternal); anInterIter.More();
         anInterIter.Next()) {
...
Standard_Boolean DBRep_DrawableShape::addMeshNormals(NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>& theNormals,
                                                     const TopoDS_Face& theFace, const Standard_Real theLength) {
...
void DBRep_DrawableShape::addMeshNormals(
    ...
    const TopoDS_Shape& theShape, const Standard_Real theLength) {
    TopLoc_Location aLoc;
...
void DBRep_DrawableShape::addSurfaceNormals(
    NCollection_DataMap<TopoDS_Face, NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>>& theNormals,
    const TopoDS_Shape& theShape, const Standard_Real theLength, const Standard_Integer theNbAlongU,
    const Standard_Integer theNbAlongV) {
    for (TopExp_Explorer aFaceIt(theShape, TopAbs_FACE); aFaceIt.More(); aFaceIt.Next()) {
        const TopoDS_Face& aFace = TopoDS::Face(aFaceIt.Current());
        NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>* aFaceNormals = theNormals.ChangeSeek(aFace);
