...
static void DumpExtent(const TopoDS_Shape& aS, TCollection_AsciiString& aStr) {
    ...
    const char* pNames[aNbTypes + 1] = {
        " SHAPE     : ", " COMPOUND  : ", " COMPSOLID : ", " SOLID     : ", " SHELL     : ",
...
// normals
//=======================================================================
static Standard_Integer normals(Draw_Interpretor& theDI, Standard_Integer theArgNum, const char** theArgs) {
    if (theArgNum < 2) {
    ...
    for (NCollection_DataMap<TopoDS_Face, NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>>::Iterator aFaceIt(aNormals);
        ...
        if (aFaceOri == TopAbs_REVERSED) bReverse = Standard_True;

        for (NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>::Iterator aNormalsIt(aFaceIt.Value()); aNormalsIt.More();
             aNormalsIt.Next()) {
...
void DBRep::Set(const Standard_CString theName, const TopoDS_Shape& theShape) {
    ...
    Handle(DBRep_DrawableShape) aDrawShape = new DBRep_DrawableShape(
        theShape, Draw_vert, Draw_jaune, Draw_rouge, Draw_bleu, aParams.Size, aParams.NbIsos, aParams.Discretization);
...
// purpose  :
//=======================================================================
TopoDS_Shape DBRep::getShape(Standard_CString& theName, TopAbs_ShapeEnum theType, Standard_Boolean theToComplain) {
...
static Standard_Integer XProgress(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    for (Standard_Integer i = 1; i < argc; i++) {
        ...
        if (argv[i][1] == 't')
            ...
            if (sscanf(argv[++i], "%p", &aPtr) == 1) Draw_ProgressIndicator::StopIndicator() = aPtr;
            return 0;
        }
    }
...
// writebrep
//=======================================================================
static Standard_Integer writebrep(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    if (isBinaryFormat) {
        ...
        if (!BinTools::Write(aShape, aFileName.ToCString(), isWithTriangles, isWithNormals, aBinToolsVersion,
                             aProgress->Start())) {
        ...
        if (!BRepTools::Write(aShape, aFileName.ToCString(), isWithTriangles, isWithNormals, aTopToolsVersion,
                              aProgress->Start())) {
            theDI << "Cannot write to the file " << aFileName;
            return 1;
        }
...
// readbrep
//=======================================================================
static Standard_Integer readbrep(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    if (theNbArgs != 3) {
        theDI << "Syntax error: wrong number of arguments";
        return 1;
    }
...
void DBRep::BasicCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("vconn", "vconn [name1 ...] , edges are colored by number of faces (see vori)", __FILE__, dispor,
                    g);
    ...
    theCommands.Add("nexplode", "stable numbered explode for vertex, edge and face: nexplode name [V/E/F]", __FILE__,
                    nexplode, g);
    ...
    theCommands.Add("nbshapes",
                    "\n nbshapes s - shows the number of sub-shapes in <s>;\n nbshapes s -t - shows the number of "
                    "sub-shapes in <s> counting the same sub-shapes with different location as different sub-shapes.",
                    __FILE__, nbshapes, g);
    ...
    theCommands.Add(
        "setflags",
        "setflags shape_name flag1[flag2...]\n sets flags for shape(free, modified, checked, orientable, closed, "
        "infinite, convex, locked), for example <setflags a free> or <setflags a -free> if necessary unflag ",
        __FILE__, setFlags, g);

    //  theCommands.Add("dumpmmgt",
    //		  "dump le contenu du gestionnaire de memoire",__FILE__,dumpmmgt,g);
    theCommands.Add("purgemmgt", "returns the free memory from the system to the memory manager", __FILE__, purgemmgt,
                    g);
    ...
    theCommands.Add("removeinternals",
