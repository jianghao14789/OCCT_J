...
//  purpose  :
//========================================================================
MAT2d_Mat2d::MAT2d_Mat2d(const Standard_Boolean IsOpenResult) : semiInfinite(Standard_False), isDone(Standard_False) {
...
void MAT2d_Mat2d::CreateMatOpen(MAT2d_Tool2d& atool) {
    ...
    for (i = 0; i < theedgelist->Number() - 1; i++) {
        ...
        bisectormap(i)->FirstVector(atool.TangentBefore(theedgelist->Current()->EdgeNumber(), myIsOpenResult));
        theedgelist->Next();
        ...
        bisectormap(i)->SecondVector(atool.TangentAfter(theedgelist->Current()->EdgeNumber(), myIsOpenResult));
    }
    ...
    while (theedgelist->Number() > 1) {
        ...
        for (;;) {
            ...
            if (EdgeNumbers(EvenNbIterBis + 1) == EdgeNumbers(1)) ToNullifyNoofbisectorstoremove = Standard_True;
            ...
            if (ToNullifyNoofbisectorstoremove) noofbisectorstoremove = 0;
            ...
            for (i = 0; i < noofbisectorstoremove; i++) {
                ...
                for (;;) {
                    ...
                    if (bisectortoremove->BisectorNumber() == bisectortwotoremove(i)->BisectorNumber()) break;
        ...
        if (currentbisector->FirstParameter() == Precision::Infinite() &&
            ...
            paramb[0] = -1;
            parama[0] = 1;
        } else if (atool.Distance(currentbisector, currentbisector->FirstParameter(),
                                  currentbisector->SecondParameter()) > toleranceofconfusion) {
            if ((currentbisector->FirstParameter() - currentbisector->SecondParameter()) * currentbisector->Sense() >
                0.) {
                parama[0] = -1;
                paramb[0] = 1;
            } else {
        ...
        for (i = 0; i < narea; i++) {
            ...
            edgetoremove->FirstBisector()->FirstParameter(edgetoremove->FirstBisector()->SecondParameter());
            ...
            for (j = 0; j < noofarea(i); j++) {
                ...
                edgetoremove->SecondBisector()->SecondParameter(edgetoremove->SecondBisector()->FirstParameter());
#ifdef OCCT_DEBUG_Mat
...
//========================================================================
void MAT2d_Mat2d::LoadBisectorsToRemove(Standard_Integer& noofbisectorstoremove, const Standard_Real distance1,
    ...
    if (index != -1) {
        ...
        for (int j = 0; j < noofbisectorstoremove; j++) {
            if (bisectoronetoremove(j)->BisectorNumber() == firstbisectortoremove[index]->BisectorNumber()) {
                found = j;
                if (bisectortwotoremove(j)->BisectorNumber() < lastbisectortoremove[index]->BisectorNumber())
                    found = -1;
                break;
            }
        }
        ...
        if (found != -1) {
#ifdef OCCT_DEBUG_Mat
            std::cout << " first last bisector to remove :" << firstbisectortoremove[index]->BisectorNumber() << " "
...
//                descendants les plus a gauche et les plus a droite.
//========================================================================v
void MAT2d_Mat2d::Intersect(MAT2d_Tool2d& atool, const Standard_Integer aside, Standard_Integer& noofbisectortoremove,
                            const Handle(MAT_Bisector) & firstbisector, const Handle(MAT_Bisector) & secondbisector) {
    ...
    LoadBisectorsToRemove(noofbisectortoremove, distance[0], distance[1], firstbisectortoremove[0],
                          firstbisectortoremove[1], lastbisectortoremove[0], lastbisectortoremove[1]);
}
