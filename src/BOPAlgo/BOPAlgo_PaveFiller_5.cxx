...
public:
    ...
    BOPAlgo_EdgeFace() : IntTools_EdgeFace(), BOPAlgo_ParallelAlgo(), myIE(-1), myIF(-1) {};
    //
...
void BOPAlgo_PaveFiller::PerformEF(const Message_ProgressRange& theRange) {
    ...
    for (; myIterator->More(); myIterator->Next()) {
        ...
        for (; aIt.More(); aIt.Next()) {
            ...
            if (!pMPB) pMPB = myFPBDone.Bound(nF, BOPDS_MapOfPaveBlock());
            pMPB->Add(aPB);
        } // for (; aIt.More(); aIt.Next()) {
    } // for (; myIterator->More(); myIterator->Next()) {
    //
    ...
    for (k = 0; k < aNbEdgeFace; ++k) {
        ...
        if (!aNbCPrts) {
            if (aEdgeFace.MinimalDistance() < RealLast() && aEdgeFace.MinimalDistance() > aTolE + aTolF) {
                ...
                if (!pList) pList = myDistances.Bound(BOPDS_Pair(nE, nF), NCollection_List<EdgeRangeDistance>());
        ...
        if (aNbCPrts) {
            ...
            bLinePlane = (aBAC.GetType() == GeomAbs_Line && myContext->SurfaceAdaptor(aF).GetType() == GeomAbs_Plane);
        }
        //
                ...
                case TopAbs_VERTEX: {
                    ...
                    for (j = 0; j < 2; ++j) {
                        if (bIsOnPave[j]) {
                            ...
                            if (!bV[j]) bIsOnPave[j] = ForceInterfVF(nV[j], nF);
                        }
                    }
                    ...
                    if (bIsOnPave[0] || bIsOnPave[1]) {
                        ...
                        Standard_Real aMinDistEF =
                            (aProjPS.IsDone() && aProjPS.NbPoints()) ? aProjPS.LowerDistance() : Precision::Infinite();
...
// purpose:
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::CheckFacePaves(const Standard_Integer nVx, const TColStd_MapOfInteger& aMIFOn,
...
// purpose:
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::CheckFacePaves(const TopoDS_Vertex& aVnew, const TColStd_MapOfInteger& aMIF) {
    Standard_Boolean bRet;
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::ForceInterfVF(const Standard_Integer nV, const Standard_Integer nF) {
    Standard_Boolean bRet;
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::ReduceIntersectionRange(const Standard_Integer theV1, const Standard_Integer theV2,
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::ForceInterfEF(const BOPDS_IndexedMapOfPaveBlock& theMPB, const Message_ProgressRange& theRange,
                                       const Standard_Boolean theAddInterf) {
    // Split progress on preparation, intersection and post-treatment stages
    Message_ProgressScope aPSOuter(theRange, NULL, 10);
    if (theMPB.IsEmpty()) return;
    ...
    for (Standard_Integer nF = 0; nF < aNbS; ++nF) {
        ...
        const BOPDS_IndexedMapOfPaveBlock* pMPBF[] = {&aFI.PaveBlocksOn(), &aFI.PaveBlocksIn(), &aFI.PaveBlocksSc()};
        for (Standard_Integer iM = 0; iM < 3; ++iM) {
        ...
        for (; itLIPB.More(); itLIPB.Next()) {
            ...
            if (pMPBF[0]->Contains(aPB) || pMPBF[1]->Contains(aPB) || pMPBF[2]->Contains(aPB)) continue;
            ...
            if (!aPB->HasEdge(nE)) {
                ...
                if (myDS->Rank(nF) == myDS->Rank(nE)) continue;
            }
            ...
            if (aVETgt.SquareMagnitude() < gp::Resolution()) continue;

            aProjPS.Perform(aPOnE);
            if (!aProjPS.NbPoints()) continue;
            ...
            if (aProjPS.LowerDistance() > aTolCheck + myFuzzyValue) continue;

            Standard_Real U, V;
            aProjPS.LowerDistanceParameters(U, V);
            if (!myContext->IsPointInFace(aF, gp_Pnt2d(U, V))) continue;
            ...
            if (aSurfAdaptor.GetType() != GeomAbs_Plane || aBAC.GetType() != GeomAbs_Line) {
                ...
                if (aVFNorm.SquareMagnitude() > gp::Resolution()) {
                    ...
                    if (Abs(aCos) > 0.4226) bUseAddTol = Standard_False;
                }
            }
            ...
            if (bUseAddTol) {
                ...
                for (Standard_Integer iP = 0; iP < 2; ++iP) {
                    ...
                    if (aProjPS.NbPoints()) {
                        ...
                        if (aDistEF < aTolCheck && aDistEF > aTolAdd) aTolAdd = aDistEF;
                    }
                }
                if (aTolAdd > 0.) {
                    aTolAdd -= (BRep_Tool::Tolerance(aE) + BRep_Tool::Tolerance(aF));
                    if (aTolAdd < 0.) aTolAdd = 0.;
                }
            }
    ...
    BOPDS_VectorOfInterfEF& aEFs = myDS->InterfEF();
    if (theAddInterf && aEFs.IsEmpty()) aEFs.SetIncrement(10);
