...
//
static Standard_Real ToleranceFF(const BRepAdaptor_Surface& aBAS1, const BRepAdaptor_Surface& aBAS2);
...
public:
    ...
    BOPAlgo_FaceFace() : IntTools_FaceFace(), BOPAlgo_ParallelAlgo(), myIF1(-1), myIF2(-1), myTolFF(1.e-7) {}
    //
...
void BOPAlgo_PaveFiller::PerformFF(const Message_ProgressRange& theRange) {
    ...
    Standard_Boolean bApprox = mySectionAttribute.Approximation(), bCompC2D1 = mySectionAttribute.PCurveOnS1(),
...
//=======================================================================
static void UpdateSavedTolerance(const BOPDS_PDS& theDS, const Standard_Integer theNE, const Standard_Real theTolNew,
    ...
    for (; itSS.More(); itSS.Next()) {
        ...
        if (pTolSaved && *pTolSaved < theTolNew) *pTolSaved = theTolNew;
    }
}
...
void BOPAlgo_PaveFiller::MakeBlocks(const Message_ProgressRange& theRange) {
    ...
    TColStd_MapOfInteger aMVOnIn(100, aAllocator), aMVCommon(100, aAllocator), aMVStick(100, aAllocator),
        aMVEF(100, aAllocator), aMI(100, aAllocator), aMVBounds(100, aAllocator);
    ...
    for (i = 0; i < aNbFF; ++i, aPS.Next()) {
        ...
        BOPTools_BoxTree aPBTree;
        {
            ...
            for (Standard_Integer iPB = 1; iPB <= aNbPB; ++iPB) {
                ...
                if (!aPB->HasEdge()) continue;

                if (myDS->ShapeInfo(aPB->OriginalEdge()).HasFlag()) continue;
        ...
        for (j = 0; j < aNbC; ++j) {
            ...
            for (; aItLPB.More(); aItLPB.Next()) {
                ...
                if (!BRepLib::FindValidRange(GeomAdaptor_Curve(aIC.Curve()), aTolR3D, aT1, BRep_Tool::Pnt(aV1),
                                             Max(aTolR3D, BRep_Tool::Tolerance(aV1)), aT2, BRep_Tool::Pnt(aV2),
                                             Max(aTolR3D, BRep_Tool::Tolerance(aV2)), aFirst, aLast)) {
                ...
                if (bExist) {
                    ...
                    if (!bInF1 || !bInF2) {
                        ...
                        if (aTolNew < aNC.Tolerance()) aTolNew = aNC.Tolerance(); // use real tolerance of intersection
                        if (aTolNew > aTolE) {
                        ...
                        if (!pFaces) pFaces = aPBFacesMap.Bound(aPBOut, TColStd_ListOfInteger());
                        // List is expected to be short, so we allow the check here
                        if (pFaces->IsEmpty() || !pFaces->Contains(nF)) pFaces->Append(nF);
                ...
                // Make p-curves
                BOPTools_AlgoTools::MakePCurve(aES, aF1, aF2, aIC, mySectionAttribute.PCurveOnS1(),
                                               mySectionAttribute.PCurveOnS2(), myContext);
                //
                ...
                ProcessExistingPaveBlocks(i, j, nF1, nF2, aES, aMPBOnIn, aPBTree, aMSCPB, aMVI, aLPBC, aPBFacesMap,
                                          aMPBAdd);
            }
            //
            aLPBC.RemoveFirst();
        ...
        for (; aItMV.More(); aItMV.Next()) {
            ...
            //
            if (aDMVLV.IsBound(nV1)) aDMVLV.UnBind(nV1);
        }
        //
...
void BOPAlgo_PaveFiller::PostTreatFF(BOPDS_IndexedDataMapOfShapeCoupleOfPaveBlocks& theMSCPB,
    ...
    for (k = aNbS; k > 0; --k) {
        ...
        for (; itV.More(); itV.Next()) {
            ...
            if (pSD) {
                ...
                if (anAddedSD.Add(aVSD)) aLS.Append(aVSD);
            }
        }
    }
    if (anExistingEdges.NbChildren() > 0) aLS.Append(anExistingEdges);
    //
    ...
    for (k = 1; k <= aNbME; ++k) {
        ...
        for (Standard_Integer i = 0; i < 2; ++i) {
            ...
            if (anAddedSD.Add(aVerts[i])) aLS.Append(aVerts[i]);
        }
        //
    ...
    for (Standard_Integer imap = 0; imap < 2; imap++) {
        ...
        for (Standard_Integer i = 1; i <= NbVer; ++i) {
            ...
            Standard_Integer iVer = myDS->Index(aVer);
            const Standard_Integer* pSD = aDMNewSD.Seek(iVer);
            if (pSD) aVer = myDS->Shape(*pSD);

            if (anAddedSD.Add(aVer)) aLS.Append(aVer);
        }
    }
    //
    ...
    NCollection_DataMap<Handle(BOPDS_CommonBlock), Standard_Real, TColStd_MapTransientHasher> aMCBTol;
            ...
            for (; aItLPB.More();) {
                ...
                if (theDME.IsBound(aPB)) {
                    ...
                    for (; itLPB.More(); itLPB.Next()) {
                        ...
                        if (!pLPBOnE) pLPBOnE = anEdgeLPB.Bound(nE, BOPDS_ListOfPaveBlock());
    ...
    for (; aItMF.More(); aItMF.Next()) {
        ...
        if (bVerts) {
            ...
            for (; aItMV.More(); aItMV.Next()) {
                ...
                nV2 = aItMV.Value();
                //
                if (aMVOn.Remove(nV1)) aMVOn.Add(nV2);
                //
                if (aMVIn.Remove(nV1)) aMVIn.Add(nV2);
            } // for (; aItMV.More(); aItMV.Next()) {
        } // if (bVerts) {
        //
        ...
        if (bEdges) {
            ...
            BOPDS_IndexedMapOfPaveBlock* pMPB[] = {&aFI.ChangePaveBlocksOn(), &aFI.ChangePaveBlocksIn(),
                ...
                for (j = 1; j <= aNbPB; ++j) {
                    ...
                    if (pLPB && !pLPB->IsEmpty()) {
                        ...
                        for (; aItLPB.More(); aItLPB.Next()) {
                            ...
                            if (aMPBFence.Add(aPBR)) pMPB[i]->Add(aPBR);
                        }
                    } else {
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::IsExistingVertex(const gp_Pnt& aP, const Standard_Real theTolR3D,
...
Standard_Boolean BOPAlgo_PaveFiller::IsExistingPaveBlock(const Handle(BOPDS_PaveBlock) & thePB,
                                                         const BOPDS_Curve& theNC, const TColStd_ListOfInteger& theLSE,
                                                         Standard_Integer& theNEOut, Standard_Real& theTolNew) {
    if (theLSE.IsEmpty()) return Standard_False;
    ...
    const TopoDS_Vertex &aV1 = TopoDS::Vertex(myDS->Shape(nV1)), &aV2 = TopoDS::Vertex(myDS->Shape(nV2));
...
Standard_Boolean BOPAlgo_PaveFiller::IsExistingPaveBlock(const Handle(BOPDS_PaveBlock) & thePB,
    ...
    if (!aSelector.Select()) return Standard_False;
    ...
    if (isVtgt1Valid) aVTgt1.Normalize();
...
// purpose  :
//=======================================================================
static void getBoundPaves(const BOPDS_DS* theDS, const BOPDS_Curve& theNC, Standard_Integer theNV[2]) {
    theNV[0] = theNV[1] = -1;
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PutBoundPaveOnCurve(const TopoDS_Face& aF1, const TopoDS_Face& aF2, BOPDS_Curve& aNC,
    ...
    if (isClosed && (aBndNV[0] > 0 || aBndNV[1] > 0)) return;
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PutPavesOnCurve(const TColStd_MapOfInteger& theMVOnIn, const TColStd_MapOfInteger& theMVCommon,
                                         BOPDS_Curve& theNC, const TColStd_MapOfInteger& theMI,
                                         const TColStd_MapOfInteger& theMVEF, TColStd_DataMapOfIntegerReal& theMVTol,
...
void BOPAlgo_PaveFiller::FilterPavesOnCurves(const BOPDS_VectorOfCurve& theVNC,
    ...
    for (i = 0; i < theVNC.Length(); ++i) {
        ...
        for (; itPaves.More(); itPaves.Next()) {
            ...
            if (aSqDist > anEps && aSqD1Mod > anEps) aSin = sqrt(aSin / aSqDist / aSqD1Mod);
            NCollection_List<PaveBlockDist>* pList = aIDMVertPBs.ChangeSeek(nV);
            if (!pList) pList = &aIDMVertPBs.ChangeFromIndex(aIDMVertPBs.Add(nV, NCollection_List<PaveBlockDist>()));
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::ExtendedTolerance(const Standard_Integer nV, const TColStd_MapOfInteger& aMI,
                                                       Standard_Real& aTolVExt, const Standard_Integer aType) {
    Standard_Boolean bFound = Standard_False;
    if (!(myDS->IsNewShape(nV))) {
        return bFound;
    }
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::GetEFPnts(const Standard_Integer nF1, const Standard_Integer nF2,
                                   IntSurf_ListOfPntOn2S& aListOfPnts) {
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PutEFPavesOnCurve(const BOPDS_VectorOfCurve& theVC, const Standard_Integer theIndex,
                                           const TColStd_MapOfInteger& aMI, const TColStd_MapOfInteger& aMVEF,
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PutStickPavesOnCurve(const TopoDS_Face& aF1, const TopoDS_Face& aF2,
                                              const TColStd_MapOfInteger& aMI, const BOPDS_VectorOfCurve& theVC,
                                              const Standard_Integer theIndex, const TColStd_MapOfInteger& aMVStick,
                                              TColStd_DataMapOfIntegerReal& aMVTol,
                                              TColStd_DataMapOfIntegerListOfInteger& aDMVLV) {
    const BOPDS_Curve& aNC = theVC.Value(theIndex);
    // Get numbers of vertices assigned to the ends of the curve
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::GetStickVertices(const Standard_Integer nF1, const Standard_Integer nF2,
                                          TColStd_MapOfInteger& aMVStick, TColStd_MapOfInteger& aMVEF,
                                          TColStd_MapOfInteger& aMI) {
    ...
    BOPDS_VectorOfInterfEF& aEFs = myDS->InterfEF();
    //
    Standard_Integer aNbLines[5] = {aVVs.Length(), aVEs.Length(), aEEs.Length(), aVFs.Length(), aEFs.Length()};
...
// purpose:
//=======================================================================
void BOPAlgo_PaveFiller::RemoveUsedVertices(const BOPDS_VectorOfCurve& aVC, TColStd_MapOfInteger& aMV) {
    if (aMV.IsEmpty()) return;
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PutPaveOnCurve(const Standard_Integer nV, const Standard_Real aTolR3D, const BOPDS_Curve& aNC,
                                        const TColStd_MapOfInteger& aMI, TColStd_DataMapOfIntegerReal& aMVTol,
                                        TColStd_DataMapOfIntegerListOfInteger& aDMVLV,
...
void BOPAlgo_PaveFiller::ProcessExistingPaveBlocks(
    const Standard_Integer theInt, const Standard_Integer theCur, const Standard_Integer nF1,
    const Standard_Integer nF2, const TopoDS_Edge& theES, const BOPDS_IndexedMapOfPaveBlock& theMPBOnIn,
    BOPTools_BoxTree& thePBTree, BOPDS_IndexedDataMapOfShapeCoupleOfPaveBlocks& theMSCPB,
    TopTools_DataMapOfShapeInteger& theMVI, BOPDS_ListOfPaveBlock& theLPBC,
    BOPAlgo_DataMapOfPaveBlockListOfInteger& thePBFacesMap, BOPDS_MapOfPaveBlock& theMPB) {
    Bnd_Box aBoxES;
    ...
    for (TColStd_ListOfInteger::Iterator itPB(aSelector.Indices()); itPB.More(); itPB.Next()) {
        ...
        if (theMPB.Contains(aPBF)) continue;

        Standard_Boolean bInF1 = (aFI1.PaveBlocksOn().Contains(aPBF) || aFI1.PaveBlocksIn().Contains(aPBF));
        Standard_Boolean bInF2 = (aFI2.PaveBlocksOn().Contains(aPBF) || aFI2.PaveBlocksIn().Contains(aPBF));
        if (bInF1 && bInF2) {
        ...
        if (aDist < RealLast()) {
            ...
            if (aDist <= aTolSum) {
                ...
                if (!pFaces) pFaces = thePBFacesMap.Bound(aPBF, TColStd_ListOfInteger());
                if (pFaces->IsEmpty() || !pFaces->Contains(nF)) pFaces->Append(nF);
            }
        }
    }
...
//=======================================================================
void BOPAlgo_PaveFiller::ProcessExistingPaveBlocks(
    const Standard_Integer theInt, const Standard_Integer nF1, const Standard_Integer nF2,
    const BOPDS_IndexedMapOfPaveBlock& aMPBOnIn, BOPTools_BoxTree& thePBTree,
    const TColStd_DataMapOfIntegerListOfInteger& aDMBV, BOPDS_IndexedDataMapOfShapeCoupleOfPaveBlocks& aMSCPB,
    TopTools_DataMapOfShapeInteger& aMVI, BOPAlgo_DataMapOfPaveBlockListOfInteger& thePBFacesMap,
        ...
        for (; aItLI.More(); aItLI.Next()) {
            ...
            for (TColStd_ListOfInteger::Iterator it(aSelector.Indices()); it.More(); it.Next()) {
                ...
                if (!iFlag) {
                    ...
                    // Add faces to PB
                    Standard_Boolean bInF1 = (aFI1.PaveBlocksOn().Contains(aPB) || aFI1.PaveBlocksIn().Contains(aPB));
                    Standard_Boolean bInF2 = (aFI2.PaveBlocksOn().Contains(aPB) || aFI2.PaveBlocksIn().Contains(aPB));
                    if (!bInF1 || !bInF2) {
                        // Face without pave block
                        const Standard_Integer nF = bInF1 ? nF2 : nF1;
                        TColStd_ListOfInteger* pFaces = thePBFacesMap.ChangeSeek(aPB);
                        if (!pFaces) pFaces = thePBFacesMap.Bound(aPB, TColStd_ListOfInteger());
                        // List is expected to be short, so we allow the check here
                        if (pFaces->IsEmpty() || !pFaces->Contains(nF)) pFaces->Append(nF);
                    }
                }
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::UpdateExistingPaveBlocks(const Handle(BOPDS_PaveBlock) & aPBf, BOPDS_ListOfPaveBlock& aLPB,
                                                  const BOPAlgo_DataMapOfPaveBlockListOfInteger& thePBFacesMap) {
    if (!aLPB.Extent()) {
        return;
    }
    ...
    for (; itLF.More(); itLF.Next()) {
        ...
        for (; aIt.More(); aIt.Next()) {
            aPB = aIt.ChangeValue();
            if (aFI.PaveBlocksOn().Contains(aPB) || aFI.PaveBlocksIn().Contains(aPB)) continue;
...
void BOPAlgo_PaveFiller::PutClosingPaveOnCurve(BOPDS_Curve& aNC) {
    ...
    const Handle(Geom_Curve) & aC3D = aIC.Curve();
    // check 3d curve
    if (aC3D.IsNull()) return;

    // check bounds
    if (!aIC.HasBounds()) return;
    ...
    if (!BRepLib::FindValidRange(GeomAdaptor_Curve(aIC.Curve()), aIC.Tolerance(), aT[0], aP[0], aNewTolV, aT[1], aP[1],
                                 aNewTolV, aFirst, aLast)) {
        // No valid range
        return;
    }
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::PreparePostTreatFF(const Standard_Integer aInt, const Standard_Integer aCur,
                                            const Handle(BOPDS_PaveBlock) & aPB,
                                            BOPDS_IndexedDataMapOfShapeCoupleOfPaveBlocks& aMSCPB,
                                            TopTools_DataMapOfShapeInteger& aMVI, BOPDS_ListOfPaveBlock& aLPBC) {
    Standard_Integer nV1, nV2;
    //
    aLPBC.Append(aPB);
    //
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::CheckPlanes(const Standard_Integer nF1, const Standard_Integer nF2) const {
...
void BOPAlgo_PaveFiller::UpdatePaveBlocks(const TColStd_DataMapOfIntegerInteger& aDMNewSD) {
    ...
    if (aMicroEdges.Extent()) RemovePaveBlocks(aMicroEdges);
}
//=======================================================================
...
void BOPAlgo_PaveFiller::RemovePaveBlocks(const TColStd_MapOfInteger theEdges) {
    ...
    for (i = 0; i < myDS->NbSourceShapes(); ++i) {
        ...
        BOPDS_IndexedMapOfPaveBlock* aIMPB[] = {&aFI.ChangePaveBlocksIn(), &aFI.ChangePaveBlocksOn(),
                                                &aFI.ChangePaveBlocksSc()};
        for (Standard_Integer k = 0; k < 3; k++) {
            Standard_Integer aNbPB = aIMPB[k]->Extent(), m;
            for (m = 1; m <= aNbPB; ++m) {
                const Handle(BOPDS_PaveBlock) & aPB = aIMPB[k]->FindKey(m);
                if (theEdges.Contains(aPB->Edge())) break;
            }
            if (m <= aNbPB) {
                ...
                for (m = 1; m <= aNbPB; ++m) {
                    ...
                    if (!theEdges.Contains(aPB->Edge())) aIMPB[k]->Add(aPB);
                }
            }
        }
    }
...
Standard_Real ToleranceFF(const BRepAdaptor_Surface& aBAS1, const BRepAdaptor_Surface& aBAS2) {
    ...
    isAna1 = (aBAS1.GetType() == GeomAbs_Plane || aBAS1.GetType() == GeomAbs_Cylinder ||
              aBAS1.GetType() == GeomAbs_Cone || aBAS1.GetType() == GeomAbs_Sphere || aBAS1.GetType() == GeomAbs_Torus);
    //
    isAna2 = (aBAS2.GetType() == GeomAbs_Plane || aBAS2.GetType() == GeomAbs_Cylinder ||
              aBAS2.GetType() == GeomAbs_Cone || aBAS2.GetType() == GeomAbs_Sphere || aBAS2.GetType() == GeomAbs_Torus);
    //
    if (!isAna1 || !isAna2) {
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::EstimatePaveOnCurve(const Standard_Integer nV, const BOPDS_Curve& aNC,
...
// purpose  :
//=======================================================================
void BOPAlgo_PaveFiller::RemoveMicroSectionEdges(BOPDS_IndexedDataMapOfShapeCoupleOfPaveBlocks& theMSCPB,
    ...
    if (aSEPBMap.Extent() != theMSCPB.Extent()) theMSCPB = aSEPBMap;
}
...
void BOPAlgo_PaveFiller::RemoveMicroEdges() {
    ...
    for (Standard_Integer i = 0; i < aNbPBP; ++i) {
        ...
        if (myDS->ShapeInfo(aLPB.First()->OriginalEdge()).HasFlag()) continue;
        ...
        for (; it.More(); it.Next()) {
            ...
            if (aMPBFence.Add(aPBR)) {
                ...
                if (nV1 == nV2) {
                    ...
                    FillShrunkData(aPBR);
                    if (!aPBR->HasShrunkData()) aMicroEdges.Add(aPBR->Edge());
                }
            }
        }
    }
