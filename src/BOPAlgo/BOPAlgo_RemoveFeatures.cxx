...
static void MakeRemoved(const TopTools_ListOfShape& theShapes, BRepTools_History& theHistory,
...
static void RemoveInternalWires(const TopTools_ListOfShape& theShapes, TopTools_ListOfShape* theRemoved = NULL);

static void GetOriginalFaces(const TopoDS_Shape& theShape, const TopTools_IndexedMapOfShape& theSolids,
                             const TopTools_MapOfShape& theFeatureFacesMap,
                             const TopTools_IndexedDataMapOfShapeListOfShape& theAdjFaces,
                             const Handle(BRepTools_History) & theHistory, TopTools_IndexedMapOfShape& theFacesToBeKept,
                             TopTools_ListOfShape& theInternalShapes, TopTools_MapOfShape& theFacesToCheckOri,
                             TopTools_IndexedMapOfShape& theSolidsToRebuild, TopTools_ListOfShape& theSharedFaces,
                             TopTools_ListOfShape& theUnTouchedSolids);

static void FindShape(const TopoDS_Shape& theSWhat, const TopoDS_Shape& theSWhere, TopoDS_Shape& theSFound);

static void GetValidSolids(BOPAlgo_MakerVolume& theMV, const TopTools_MapOfShape& theFacesToCheckOri,
...
static void FindExtraShapes(const TopTools_IndexedDataMapOfShapeListOfShape& theConnectionMap,
                            ...
                            TopTools_MapOfShape& theShapesToAvoid, TopTools_MapOfShape* theValidShapes = NULL);

static void AvoidExtraSharedFaces(TopTools_ListOfShape& theLSolids, const TopTools_ListOfShape& theLFSharedToAvoid,
                                  BOPAlgo_Builder& theBuilder, TopTools_ListOfShape& theExtraFaces);

static void FillSolidsHistory(const TopTools_IndexedMapOfShape& theSolIn, TopTools_ListOfShape& theSolidsRes,
                              const TopTools_IndexedDataMapOfShapeListOfShape& theAdjFaces, BOPAlgo_Builder& theBuilder,
                              BRepTools_History& theSolidsHistory);

static void TakeModified(const TopoDS_Shape& theS, BOPAlgo_Builder& theBuilder, TopTools_ListOfShape& theList);

static void TakeModified(const TopoDS_Shape& theS, BOPAlgo_Builder& theBuilder, TopTools_MapOfShape& theMap);
...
// purpose:
//=======================================================================
void BOPAlgo_RemoveFeatures::fillPIConstants(const Standard_Real theWhole, BOPAlgo_PISteps& theSteps) const {
...
void BOPAlgo_RemoveFeatures::Perform(const Message_ProgressRange& theRange) {
    try {
        ...
        if (HasHistory()) myHistory = new BRepTools_History();
...
void BOPAlgo_RemoveFeatures::CheckData() {
    ...
    if (aType == TopAbs_SOLID || aType == TopAbs_COMPSOLID) return; // OK
...
void BOPAlgo_RemoveFeatures::PrepareFeatures(const Message_ProgressRange& theRange) {
    ...
    for (; aIt.More(); aIt.Next(), aPS.Next()) {
        ...
        for (; anExpF.More(); anExpF.Next()) {
            ...
            if (myInputsMap.Contains(aF)) aFacesToRemove.Append(aF);
        }
    }
...
public: //! @name Setters/Getters
    ...
    void SetFeature(const TopoDS_Shape& theFeature) {
        myFeature = theFeature;
    }

    //! Returns the feature
    const TopoDS_Shape& Feature() const {
        return myFeature;
    }
    ...
    void SetRunParallel(const Standard_Boolean bRunParallel) {
        myRunParallel = bRunParallel;
    }
...
public: //! @name Obtain the result
    ...
    const TopTools_MapOfShape& FeatureFacesMap() const {
        return myFeatureFacesMap;
    }

    //! Shows whether the adjacent faces have been found for the feature
    Standard_Boolean HasAdjacentFaces() const {
        return myHasAdjacentFaces;
    }

    //! Returns the Images map of the adjacent faces
    const TopTools_IndexedDataMapOfShapeListOfShape& Faces() const {
        return myFaces;
    }

    //! Returns the initial solids participating in the feature removal
    const TopTools_IndexedMapOfShape& Solids() const {
        return mySolids;
    }
...
private: //! @name Private methods performing the operation
            ...
            for (; anExpE.More(); anExpE.Next()) {
                ...
                if (pAdjacentFaces) {
                    ...
                    for (; itLFA.More(); itLFA.Next()) {
                        ...
                        if (!myFeatureFacesMap.Contains(anAF)) theMFAdjacent.Add(anAF);
                    }
                }
            }
    ...
    void ExtendAdjacentFaces(const TopTools_IndexedMapOfShape& theMFAdjacent,
        ...
        for (Standard_Integer i = 1; i <= aNbFA && aPS.More(); ++i, aPS.Next()) {
            ...
            // Extend the face
            TopoDS_Face aFExt;
            BRepLib::ExtendFace(aF, anExtLength, Standard_True, Standard_True, Standard_True, Standard_True, aFExt);
    ...
    void TrimFace(const TopoDS_Face& theFExt, const TopoDS_Face& theFOriginal,
                  const TopTools_IndexedMapOfShape& theFeatureEdgesMap,
                  const TopTools_IndexedDataMapOfShapeListOfShape& theEFExtMap, BOPAlgo_Builder& theGFInter) {
        ...
        for (; anExpE.More(); anExpE.Next()) {
            ...
            if (BRep_Tool::Degenerated(aE) || BRep_Tool::IsClosed(aE, theFExt)) continue;
            ...
            for (; itLEIm.More(); itLEIm.Next()) {
                ...
                if (theEFExtMap.FindFromKey(aEIm).Extent() == 1) aMExtEdges.Add(aEIm);
            }
        }
        ...
        for (; anExpE.More(); anExpE.Next()) {
            ...
            if (!theFeatureEdgesMap.Contains(aE)) {
                ...
                if (!BRep_Tool::Degenerated(aE) && !BRep_Tool::IsClosed(aE, theFOriginal)) {
                    if (!aMEdgesToCheckOri.Add(aE)) aMEdgesToCheckOri.Remove(aE);
                }
            }
        }
        ...
        for (; anExpF.More(); anExpF.Next()) {
            ...
            for (; anExpE.More(); anExpE.Next()) {
                if (aMExtEdges.Contains(anExpE.Current())) break;
            }
            if (!anExpE.More()) aLFTrimmed.Append(aSp);
        }
        ...
        if (aLFTrimmed.Extent() > 1) {
            ...
            if (aLFTrimmed.Extent() - aFacesToAvoid.Extent() > 1) {
                ...
                for (; itLF.More(); itLF.Next()) {
                    if (!aFacesToAvoid.Contains(itLF.Value())) BRep_Builder().Add(aCF, itLF.Value());
                }
                ...
                if (aLCB.Extent() > 1) {
                    ...
                    for (; itLCB.More(); itLCB.Next()) {
                        ...
                        for (; itF.More(); itF.Next()) {
                            if (aValidFaces.Contains(itF.Value())) break;
                        }
                        if (!itF.More()) {
                            // Invalid block
...
void BOPAlgo_RemoveFeatures::RemoveFeatures(const Message_ProgressRange& theRange) {
    ...
    if (myHistory.IsNull()) myHistory = new BRepTools_History();
    ...
    for (Standard_Integer i = 0; i < aNbF; ++i) {
        ...
        RemoveFeature(aFG.Feature(), aFG.Solids(), aFG.FeatureFacesMap(), aFG.HasAdjacentFaces(), aFG.Faces(),
                      aFG.History(), isSolidsHistoryNeeded, aPSLoop.Next());
    }
}
...
//=======================================================================
void BOPAlgo_RemoveFeatures::RemoveFeature(const TopoDS_Shape& theFeature, const TopTools_IndexedMapOfShape& theSolids,
                                           const TopTools_MapOfShape& theFeatureFacesMap,
    ...
    GetOriginalFaces(myShape, theSolids, theFeatureFacesMap, theAdjFaces, myHistory, aFacesToBeKept, anInternalShapes,
                     aFacesToCheckOri, aSolidsToRebuild, aSharedFaces, anUnTouchedSolids);
    ...
    for (Standard_Integer i = 1; i <= aNbFK && bValid; ++i) {
        ...
        if (anAdjFacesSplits.Contains(aS)) continue;
        if (!aPS.More()) {
            return;
        }
        TopExp_Explorer anExpF(aS, TopAbs_FACE);
...
void BOPAlgo_RemoveFeatures::UpdateHistory(const Message_ProgressRange& theRange) {
    ...
    for (Standard_Integer i = 1; i <= aNbS; ++i, aPS.Next()) {
        ...
        if (!BRepTools_History::IsSupportedType(aS)) continue;

        if (myHistory->IsRemoved(aS)) continue;
        ...
        if (aLSIm.IsEmpty()) {
            if (!myMapShape.Contains(aS)) aHistory.Remove(aS);
        }

        TopTools_ListIteratorOfListOfShape itLSIm(aLSIm);
        for (; itLSIm.More(); itLSIm.Next()) {
            if (!myMapShape.Contains(itLSIm.Value())) aHistory.Remove(itLSIm.Value());
        }
    }
...
void BOPAlgo_RemoveFeatures::SimplifyResult(const Message_ProgressRange& theRange) {
    if (myShape.IsSame(myInputShape)) return;
    ...
    if (myMapShape.IsEmpty()) TopExp::MapShapes(myShape, myMapShape);
    ...
    for (Standard_Integer i = 1; i <= aNbS; ++i, aPS.Next()) {
        ...
        if (myMapShape.Contains(myInputsMap(i))) aSDTool.KeepShape(myInputsMap(i));
    }
    ...
    if (HasHistory()) myHistory->Merge(aSDTool.History());
}
...
void BOPAlgo_RemoveFeatures::PostTreat() {
    ...
    if (aResType == anInputType) return;
        ...
        for (; anExpF.More(); anExpF.Next()) {
            ...
            if (aLWToRemove.Extent()) {
                ...
                for (; itR.More(); itR.Next()) {
                    if (theRemoved) theRemoved->Append(itR.Value());
    ...
    for (; anExpS.More(); anExpS.Next()) {
        ...
        for (; itSh.More(); itSh.Next()) {
            ...
            for (; itF.More(); itF.Next()) {
                ...
                if (aFeatureFacesSplits.Contains(aF)) continue;

                // Avoid the adjacent faces
                if (theAdjFaces.Contains(aF)) continue;
...
void GetValidSolids(BOPAlgo_MakerVolume& theMV, const TopTools_MapOfShape& theFacesToCheckOri,
    ...
    if (theLSRes.Extent() > theNbSol) {
        ...
        for (; itLS.More();) {
            ...
            for (; anExpF.More(); anExpF.Next()) {
                if (anOrigFacesRes.Contains(anExpF.Current())) break;
            }
            if (!anExpF.More()) {
    ...
    for (; itM.More(); itM.Next()) {
        ...
        for (; itLSIm.More(); itLSIm.Next()) {
            ...
            for (; itSV.More(); itSV.Next()) {
                ...
                if (pValidShapes->Contains(aShapeToValidate)) continue;

                TopoDS_Face aSInShape;
                FindShape(aSIm, aShapeToValidate, aSInShape);

                Standard_Boolean bSameOri = !BOPTools_AlgoTools::IsSplitToReverse(aSInShape, aSToCheckOri, aCtx);
...
//=======================================================================
void AvoidExtraSharedFaces(TopTools_ListOfShape& theLSolids, const TopTools_ListOfShape& theLFSharedToAvoid,
                           BOPAlgo_Builder& theBuilder, TopTools_ListOfShape& theExtraFaces) {
    ...
    if (anExtraFaces.IsEmpty()) return;
...
//=======================================================================
void FillSolidsHistory(const TopTools_IndexedMapOfShape& theSolIn, TopTools_ListOfShape& theSolidsOut,
                       const TopTools_IndexedDataMapOfShapeListOfShape& theAdjFaces, BOPAlgo_Builder& theBuilder,
                       BRepTools_History& theSolidsHistory) {
...
//=======================================================================
void TakeModified(const TopoDS_Shape& theS, BOPAlgo_Builder& theBuilder, TopTools_ListOfShape& theList) {
...
//=======================================================================
void TakeModified(const TopoDS_Shape& theS, BOPAlgo_Builder& theBuilder, TopTools_MapOfShape& theMap) {
    const TopTools_ListOfShape& aModified = theBuilder.Modified(theS);
    if (aModified.IsEmpty() && !theBuilder.IsDeleted(theS))
        theMap.Add(theS);
    else {
