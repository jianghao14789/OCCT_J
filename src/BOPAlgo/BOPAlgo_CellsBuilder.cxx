...
static void CollectMaterialBoundaries(const TopTools_ListOfShape& theLS, TopTools_MapOfShape& theMapKeepBnd);
...
BOPAlgo_CellsBuilder::BOPAlgo_CellsBuilder()
    : BOPAlgo_Builder(), myIndex(100, myAllocator), myMaterials(100, myAllocator), myShapeMaterial(100, myAllocator),
...
BOPAlgo_CellsBuilder::BOPAlgo_CellsBuilder(const Handle(NCollection_BaseAllocator) & theAllocator)
    : BOPAlgo_Builder(theAllocator), myIndex(100, myAllocator), myMaterials(100, myAllocator),
      myShapeMaterial(100, myAllocator), myMapModified(100, myAllocator) {}
...
void BOPAlgo_CellsBuilder::AddToResult(const TopTools_ListOfShape& theLSToTake,
                                       const TopTools_ListOfShape& theLSToAvoid, const Standard_Integer theMaterial,
...
// purpose  :
//=======================================================================
void BOPAlgo_CellsBuilder::AddAllToResult(const Standard_Integer theMaterial, const Standard_Boolean theUpdate) {
    myShapeMaterial.Clear();
    myMaterials.Clear();
    myMapModified.Clear();
    //
...
void BOPAlgo_CellsBuilder::RemoveInternalBoundaries() {
    ...
    for (Standard_Integer iType = 0; iType < 2; ++iType) {
        if (aLSUnify[iType].IsEmpty()) continue;
        TopTools_ListOfShape aLSN;
        if (RemoveInternals(aLSUnify[iType], aLSN, aKeepMap[iType])) bChanged = Standard_True;
...
// purpose  :
//=======================================================================
void BOPAlgo_CellsBuilder::FindParts(const TopTools_ListOfShape& theLSToTake, const TopTools_ListOfShape& theLSToAvoid,
    ...
    if (aType == TopAbs_EDGE || aType == TopAbs_FACE) {
        ...
        for (i = 1; i <= aNb; ++i) {
            ...
            for (; aIt.More(); aIt.Next()) {
                ...
                if (pMaterial && !myShapeMaterial.IsBound(aSU)) myShapeMaterial.Bind(aSU, *pMaterial);
            }
        }
...
const TopTools_ListOfShape* BOPAlgo_CellsBuilder::LocModified(const TopoDS_Shape& theS) {
    ...
    if (!pLSp) {
        ...
        for (; aIt.More(); aIt.Next()) {
            ...
            if (pSU) pSp = pSU;
            if (aMFence.Add(*pSp)) myHistShapes.Append(*pSp);
        }
    }
    return &myHistShapes;
}
