...
// purpose  :
//=======================================================================
BOPAlgo_PaveFiller::BOPAlgo_PaveFiller() : BOPAlgo_Algo() {
    myDS = NULL;
    myIterator = NULL;
...
BOPAlgo_PaveFiller::BOPAlgo_PaveFiller(const Handle(NCollection_BaseAllocator) & theAllocator)
    : BOPAlgo_Algo(theAllocator), myFPBDone(1, theAllocator), myIncreasedSS(1, theAllocator),
      myVertsToAvoidExtension(1, theAllocator), myDistances(1, theAllocator) {
    myDS = NULL;
    myIterator = NULL;
    myNonDestructive = Standard_False;
    myIsPrimary = Standard_True;
...
void BOPAlgo_PaveFiller::RepeatIntersection(const Message_ProgressRange& theRange) {
    ...
    for (Standard_Integer i = 0; i < aNbS; ++i) {
        ...
        if (aSI.ShapeType() != TopAbs_VERTEX) continue;
        ...
        if (!myDS->HasShapeSD(i, nVSD)) continue;

        if (myIncreasedSS.Contains(nVSD)) anExtraInterfMap.Add(i);
    }

    if (anExtraInterfMap.IsEmpty()) return;
...
// purpose:
//=======================================================================
void BOPAlgo_PaveFiller::fillPIConstants(const Standard_Real theWhole, BOPAlgo_PISteps& theSteps) const {
    if (!myNonDestructive) {
