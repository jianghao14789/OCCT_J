...
typedef NCollection_IndexedDataMap<TopoDS_Shape, gp_Dir, TopTools_ShapeMapHasher> BOPAlgo_IndexedDataMapOfShapeDir;
typedef NCollection_IndexedDataMap<TopoDS_Shape, gp_Pln, TopTools_ShapeMapHasher> BOPAlgo_IndexedDataMapOfShapePln;

static void MakeWires(const TopTools_IndexedMapOfShape& theEdges, TopoDS_Compound& theWires,
                      const Standard_Boolean theCheckUniquePlane, BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
...
static Standard_Boolean FindPlane(const TopoDS_Shape& theWire, gp_Pln& thePlane,
                                  BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
                                  TopTools_MapOfShape& theMEdgesNoUniquePlane);

static Standard_Boolean FindEdgeTangent(const TopoDS_Edge& theEdge, BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
                                        gp_Dir& theTgt);
...
// purpose  :
//=======================================================================
void BOPAlgo_Tools::FillMap(const Handle(BOPDS_PaveBlock) & aPB, const Standard_Integer nF,
...
void BOPAlgo_Tools::PerformCommonBlocks(BOPDS_IndexedDataMapOfPaveBlockListOfPaveBlock& aMPBLPB,
                                        const Handle(NCollection_BaseAllocator) & aAllocator, BOPDS_PDS& pDS,
        ...
        for (; aItLPB.More(); aItLPB.Next()) {
            ...
            if (pDS->IsCommonBlock(aPB)) {
                ...
                for (; aItLF.More(); aItLF.Next()) {
                    ...
                    if (aMFaces.Add(nF)) aLFaces.Append(nF);
                }
                if (aCB.IsNull()) aCB = aCBx;
            }
        }

        if (aCB.IsNull()) aCB = new BOPDS_CommonBlock;
...
void BOPAlgo_Tools::PerformCommonBlocks(const BOPDS_IndexedDataMapOfPaveBlockListOfInteger& aMPBLI,
                                        const Handle(NCollection_BaseAllocator) &, // aAllocator
                                        BOPDS_PDS& pDS, const Handle(IntTools_Context) & theContext) {
...
// purpose  :
//=======================================================================
Standard_Real BOPAlgo_Tools::ComputeToleranceOfCB(const Handle(BOPDS_CommonBlock) & theCB, const BOPDS_PDS theDS,
                                                  const Handle(IntTools_Context) & theContext) {
    ...
    if (aCtx.IsNull()) aCtx = new IntTools_Context();
...
// purpose  :
//=======================================================================
Standard_Integer BOPAlgo_Tools::EdgesToWires(const TopoDS_Shape& theEdges, TopoDS_Shape& theWires,
                                             const Standard_Boolean theShared, const Standard_Real theAngTol) {
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_Tools::WiresToFaces(const TopoDS_Shape& theWires, TopoDS_Shape& theFaces,
...
//=======================================================================
void MakeWires(const TopTools_IndexedMapOfShape& theEdges, TopoDS_Compound& theWires,
               const Standard_Boolean theCheckUniquePlane, BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
               TopTools_MapOfShape& theMEdgesNoUniquePlane) {
    TopoDS_Compound aCE;
...
//=======================================================================
Standard_Boolean FindEdgeTangent(const TopoDS_Edge& theEdge, BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
                                 gp_Dir& theTgt) {
...
//=======================================================================
Standard_Boolean FindPlane(const TopoDS_Shape& theWire, gp_Pln& thePlane,
                           BOPAlgo_IndexedDataMapOfShapeDir& theDMEdgeTgt,
                           TopTools_MapOfShape& theMEdgesNoUniquePlane) {
...
public:
    BOPAlgo_PairVerticesSelector() : myVertices(NULL), myFuzzyValue(Precision::Confusion()) {}
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theID1, const Standard_Integer theID2) Standard_OVERRIDE {
        if (!RejectElement(theID1, theID2)) {
            ...
            if (aTolV1 < myVertices->FindFromIndex(anID1)) aTolV1 = myVertices->FindFromIndex(anID1);
            ...
            if (aTolV2 < myVertices->FindFromIndex(anID2)) aTolV2 = myVertices->FindFromIndex(anID2);
...
void BOPAlgo_Tools::IntersectVertices(const TopTools_IndexedDataMapOfShapeReal& theVertices,
                                      const Standard_Real theFuzzyValue, TopTools_ListOfListOfShape& theChains) {
    ...
    for (Standard_Integer i = 1; i <= aNbV; ++i) {
        ...
        if (aTol < theVertices(i)) aTol = theVertices(i);
...
private:
    ...
    void MapEdgesAndFaces(const TopoDS_Shape& theF, TopTools_IndexedDataMapOfShapeListOfShape& theEFMap,
                          const Handle(NCollection_BaseAllocator) & theAlloc);

    //! Makes the connexity block of faces using the connection map
    void MakeConnexityBlock(const TopoDS_Face& theF, const TopTools_IndexedMapOfShape& theMEToAvoid,
                            const TopTools_IndexedDataMapOfShapeListOfShape& theEFMap, TopTools_MapOfShape& theMFDone,
                            TopTools_ListOfShape& theLCB, TopoDS_Face& theFaceToClassify);
...
void BOPAlgo_FillIn3DParts::Perform() {
    ...
    for (; aItLI.More(); aItLI.Next()) {
        ...
        if (!aMSF.Contains(aFP)) aIVec.Appended() = nFP;
    }
    ...
    if (aNbFP > 1) std::sort(aIVec.begin(), aIVec.end());
    ...
    for (k = 0; k < aNbFP; ++k, aPSLoop.Next()) {
        ...
        if (aFaceToClassify.IsNull()) aFaceToClassify = aFP;
        ...
        Standard_Boolean bIsIN =
            BOPTools_AlgoTools::IsInternalFace(aFaceToClassify, mySolid, aMEFDS, Precision::Confusion(), myContext);
        if (bIsIN) {
            aItLS.Initialize(aLCBF);
            for (; aItLS.More(); aItLS.Next())
...
void BOPAlgo_FillIn3DParts::MapEdgesAndFaces(const TopoDS_Shape& theF,
    ...
    for (; myItF.More(); myItF.Next()) {
        ...
        if (aW.ShapeType() != TopAbs_WIRE) continue;
        ...
        for (; myItW.More(); myItW.Next()) {
            ...
            if (!pLF) pLF = &theEFMap(theEFMap.Add(aE, TopTools_ListOfShape(theAllocator)));
            pLF->Append(theF);
        }
    }
}
...
void BOPAlgo_FillIn3DParts::MakeConnexityBlock(const TopoDS_Face& theFStart,
                                               ...
                                               TopTools_MapOfShape& theMFDone, TopTools_ListOfShape& theLCB,
                                               TopoDS_Face& theFaceToClassify) {
    // Add start element
    theLCB.Append(theFStart);
    if (theEFMap.IsEmpty()) return;
        ...
        for (; myItF.More(); myItF.Next()) {
            ...
            for (; myItW.More(); myItW.Next()) {
                ...
                if (theMEAvoid.Contains(aE) || BRep_Tool::Degenerated(aE)) {
                    if (theFaceToClassify.IsNull()) theFaceToClassify = TopoDS::Face(aF);
                    continue;
                }
                ...
                for (; aItLF.More(); aItLF.Next()) {
                    ...
                    if (theMFDone.Add(aFToAdd)) theLCB.Append(aFToAdd);
                }
            }
        }
    }
...
// purpose  :
//=======================================================================
void BOPAlgo_Tools::ClassifyFaces(const TopTools_ListOfShape& theFaces, const TopTools_ListOfShape& theSolids,
                                  const Standard_Boolean theRunParallel, Handle(IntTools_Context) & theContext,
    ...
    for (; aItLS.More(); aItLS.Next()) {
        ...
        if (pBox)
            ...
            if (!aBox.IsWhole()) {
                if (BOPTools_AlgoTools::IsInvertedSolid(aSolid)) aBox.SetWhole();
            }
            aFIP.SetBoxS(aBox);
        }

        const TopTools_ListOfShape* pLIF = theSolidsIF.Seek(aSolid);
        if (pLIF) aFIP.SetOwnIF(*pLIF);
...
// purpose  :
//=======================================================================
void BOPAlgo_Tools::FillInternals(const TopTools_ListOfShape& theSolids, const TopTools_ListOfShape& theParts,
                                  const TopTools_DataMapOfShapeListOfShape& theImages,
                                  const Handle(IntTools_Context) & theContext) {
    if (theSolids.IsEmpty() || theParts.IsEmpty()) return;
            ...
            case TopAbs_FACE: {
                ...
                if (pIm) {
                    ...
                    for (; itIm.More(); itIm.Next()) {
                        ...
                        if (!aMSSolids.Contains(aPartIm)) aLParts.Append(aPartIm);
                    }
    ...
    for (; itLS.More(); itLS.Next()) {
        const TopoDS_Shape& aSolid = itLS.Value();
        if (aSolid.ShapeType() != TopAbs_SOLID) continue;
        ...
        for (; itLP.More();) {
            ...
            if (aState == TopAbs_IN) {
                if (aPart.ShapeType() == TopAbs_FACE) {
                    ...
                    if (!pFaces) pFaces = anINFaces.Bound(aSd, TopTools_ListOfShape());
                    pFaces->Append(aPart);
                } else {
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_Tools::TrsfToPoint(const Bnd_Box& theBox1, const Bnd_Box& theBox2, gp_Trsf& theTrsf,
                                            const gp_Pnt& thePoint, const Standard_Real theCriteria) {
    // Unify two boxes
    ...
    if (aPBDist < theCriteria) return Standard_False;

    Standard_Real aBSize = Sqrt(aBox.SquareExtent());
    if ((aBSize / aPBDist) > (1. / theCriteria)) return Standard_False;
