...
void BOPAlgo_Builder::FillImagesVertices(const Message_ProgressRange& theRange) {
    ...
    for (; aIt.More(); aIt.Next(), aPS.Next()) {
        ...
        if (!pLOr) pLOr = myOrigins.Bound(aVSD, TopTools_ListOfShape());
        pLOr->Append(aV);
    }
}
//=======================================================================
...
void BOPAlgo_Builder::FillImagesContainer(const TopoDS_Shape& theS, const TopAbs_ShapeEnum theType) {
    ...
    for (; aIt.More(); aIt.Next()) {
        ...
        if (pLFIm && ((pLFIm->Extent() != 1) || !pLFIm->First().IsSame(aSS))) break;
    }
    ...
    for (; aIt.More(); aIt.Next()) {
        ...
        for (; aItIm.More(); aItIm.Next()) {
            ...
            if (!aSSIm.IsEqual(aSS) && BOPTools_AlgoTools::IsSplitToReverseWithWarn(aSSIm, aSS, myContext, myReport)) {
                aSSIm.Reverse();
            }
            aBB.Add(aCIm, aSSIm);
        }
