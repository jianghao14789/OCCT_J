...
BOPAlgo_ArgumentAnalyzer::BOPAlgo_ArgumentAnalyzer()
    : BOPAlgo_Algo(), myStopOnFirst(Standard_False), myOperation(BOPAlgo_UNKNOWN), myArgumentTypeMode(Standard_False),
      mySelfInterMode(Standard_False), mySmallEdgeMode(Standard_False), myRebuildFaceMode(Standard_False),
      myTangentMode(Standard_False), myMergeVertexMode(Standard_False), myMergeEdgeMode(Standard_False),
      myContinuityMode(Standard_False), myCurveOnSurfaceMode(Standard_False), myEmpty1(Standard_False),
...
void BOPAlgo_ArgumentAnalyzer::Perform(const Message_ProgressRange& theRange) {
    ...
    try {
        ...
        // 4. Test small edges
        if (mySmallEdgeMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestSmallEdge();
            if (UserBreak(aPS)) {
                return;
            }
        }
        ...
        if (myRebuildFaceMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestRebuildFace();
            if (UserBreak(aPS)) {
                return;
            }
        }
        ...
        if (myMergeVertexMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestMergeVertex();
            if (UserBreak(aPS)) {
                return;
            }
        }
        ...
        // 8. Test merge edges
        if (myMergeEdgeMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestMergeEdge();
            if (UserBreak(aPS)) {
                return;
            }
        }
        ...
        if (myContinuityMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestContinuity();
            if (UserBreak(aPS)) {
                return;
            }
        }
        ...
        if (myCurveOnSurfaceMode) {
            if (!(!myResult.IsEmpty() && myStopOnFirst)) TestCurveOnSurface();
        }
    ...
    for (ii = 0; ii < 2; ii++) {
        ...
        for (; aItMPK.More(); aItMPK.Next()) {
            ...
            if (ii == 0) {
                ...
                if (!aS1.IsSame(aS2)) aResult.AddFaultyShape1(aS2);
            } else {
                aResult.SetShape2(myShape2);
                aResult.AddFaultyShape2(aS1);
                if (!aS1.IsSame(aS2)) aResult.AddFaultyShape2(aS2);
            }
...
void BOPAlgo_ArgumentAnalyzer::TestRebuildFace() {
    if ((myOperation == BOPAlgo_SECTION) || (myOperation == BOPAlgo_UNKNOWN)) return;
    Standard_Integer i = 0;
...
void BOPAlgo_ArgumentAnalyzer::TestMergeSubShapes(const TopAbs_ShapeEnum theType) {
    if (myShape1.IsNull() || myShape2.IsNull()) return;

    if (myEmpty1 || myEmpty2) return;
    ...
    for (; anExp1.More(); anExp1.Next()) {
        ...
        if (aMap1.Contains(aS1)) continue;
        aSeq1.Append(aS1);
        aMap1.Add(aS1);
    }
    ...
    for (; anExp2.More(); anExp2.Next()) {
        ...
        if (aMap2.Contains(aS2)) continue;
        aSeq2.Append(aS2);
        aMap2.Add(aS2);
    }
