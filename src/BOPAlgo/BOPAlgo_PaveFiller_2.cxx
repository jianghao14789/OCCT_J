...
public:
    ...
    BOPAlgo_VertexEdge() : BOPAlgo_ParallelAlgo(), myIV(-1), myIE(-1), myFlag(-1), myT(-1.), myTolVNew(-1.) {};
    //
...
void BOPAlgo_PaveFiller::PerformVE(const Message_ProgressRange& theRange) {
    ...
    for (; myIterator->More(); myIterator->Next()) {
        ...
        if (!pLV) pLV = &aMVEPairs(aMVEPairs.Add(aPB, TColStd_ListOfInteger()));
        pLV->Append(nV);
    }
    //
...
void BOPAlgo_PaveFiller::IntersectVE(const BOPDS_IndexedDataMapOfPaveBlockListOfInteger& theVEPairs,
                                     const Message_ProgressRange& theRange, const Standard_Boolean theAddInterfs) {
    ...
    for (i = 1; i <= aNbVE; ++i) {
        ...
        for (; aItLV.More(); aItLV.Next()) {
            ...
            //
            if (aMVPB.Contains(nVSD)) continue;
    ...
    for (i = 0; i < aNbVE; ++i) {
        ...
        for (; itPB.More(); itPB.Next()) {
            ...
            aPB->Range(aT1, aT2);
            if (aT > aT1 && aT < aT2) break;
        }
        if (!itPB.More()) continue;
...
//=======================================================================
static void MakeNewCommonBlock(const BOPDS_ListOfPaveBlock& theLPB, const TColStd_ListOfInteger& theLFaces,
                               BOPDS_PDS& theDS) {
...
// purpose:
//=======================================================================
void BOPAlgo_PaveFiller::SplitPaveBlocks(const TColStd_MapOfInteger& theMEdges, const Standard_Boolean theAddInterfs) {
    // Fence map to avoid unification of the same vertices twice
    BOPDS_MapOfPair aMPairs;
    // Map to treat the Common Blocks
    NCollection_IndexedDataMap<Handle(BOPDS_CommonBlock), BOPDS_ListOfPaveBlock, TColStd_MapTransientHasher> aMCBNewPB;
    //
...
// purpose:
//=======================================================================
void BOPAlgo_PaveFiller::AddIntersectionFailedWarning(const TopoDS_Shape& theS1, const TopoDS_Shape& theS2) {
