...
static void AddToShape(const TopoDS_Shape& theWhat, TopoDS_Shape& theWhere) {
    if (theWhere.IsNull()) BRep_Builder().MakeCompound(TopoDS::Compound(theWhere));
...
void BOPAlgo_MakePeriodic::Trim() {
    ...
    if (IsInputXTrimmed() && IsInputYTrimmed() && IsInputZTrimmed()) return;
...
void BOPAlgo_MakePeriodic::MakeIdentical() {
    if (myShape.IsNull()) myShape = myInputShape;

    if (mySplitHistory.IsNull()) mySplitHistory = new BRepTools_History;
...
//=======================================================================
static void AddTwin(const TopoDS_Shape& theS, const TopoDS_Shape& theTwin, TopTools_DataMapOfShapeListOfShape& theMap) {
    ...
    for (; itLT.More(); itLT.Next()) {
        if (theTwin.IsSame(itLT.Value())) break;
    }

    if (!itLT.More()) aTwins->Append(theTwin);
}
...
void BOPAlgo_MakePeriodic::SplitPositive() {
    ...
    for (Standard_Integer i = 0; i < 3; ++i) {
        ...
        for (Standard_Integer j = 1; j <= aNbS; ++j) {
            ...
            if (BRepTools_History::IsSupportedType(aS)) {
                ...
                if (!pTS) pTS = &aTranslationHistMap(aTranslationHistMap.Add(aS, TopTools_ListOfShape()));
    ...
    Handle(BRepTools_History) aSplitShapeHist = new BRepTools_History, aSplitToolsHist = new BRepTools_History;
...
void BOPAlgo_MakePeriodic::SplitShape(const TopTools_ListOfShape& theTools,
    ...
    if (!theSplitShapeHistory.IsNull()) theSplitShapeHistory->Merge(anObj, aSplitter);
    if (!theSplitToolsHistory.IsNull()) theSplitToolsHistory->Merge(theTools, aSplitter);
}
...
//=======================================================================
const TopoDS_Shape& BOPAlgo_MakePeriodic::RepeatShape(const Standard_Integer theDir, const Standard_Integer theTimes) {
    if (myRepeatedShape.IsNull()) myRepeatedShape = myShape;

    if (!IsPeriodic(theDir)) return myRepeatedShape;

    if (theTimes == 0) return myRepeatedShape;

    // Get the shape's period in the required direction
    const Standard_Integer id = BOPAlgo_MakePeriodic::ToDirectionID(theDir);
    if (myRepeatPeriod[id] < Precision::Confusion()) myRepeatPeriod[id] = Period(id);
    ...
    for (Standard_Integer i = 1; i <= aNbS; ++i) {
        ...
        if (BRepTools_History::IsSupportedType(aS)) aTranslationHistory.AddGenerated(aS, aS);
    }
...
void BOPAlgo_MakePeriodic::UpdateTwins(const BRepTools_History& theTranslationHistory,
    ...
    if (myTwins.IsEmpty()) return;

    if (myRepeatedTwins.IsEmpty()) myRepeatedTwins = myTwins;
    ...
    for (; itDMap.More(); itDMap.Next()) {
        ...
        for (; itLT.More(); itLT.Next()) {
            if (aMTwinsDone.Contains(itLT.Value())) break;
        }
        if (itLT.More())
