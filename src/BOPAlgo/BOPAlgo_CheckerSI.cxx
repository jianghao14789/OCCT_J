...
public:
    ...
    BOPAlgo_FaceSelfIntersect() : IntTools_FaceFace(), BOPAlgo_ParallelAlgo(), myIF(-1), myTolF(1.e-7) {}
    //
...
// purpose  :
//=======================================================================
BOPAlgo_CheckerSI::BOPAlgo_CheckerSI() : BOPAlgo_PaveFiller() {
...
void BOPAlgo_CheckerSI::Perform(const Message_ProgressRange& theRange) {
    try {
        ...
        if (!HasErrors()) PerformVZ(aPSZZ.Next());
        //
        if (!HasErrors()) PerformEZ(aPSZZ.Next());
        //
        if (!HasErrors()) PerformFZ(aPSZZ.Next());
        //
        if (!HasErrors()) PerformZZ(aPSZZ.Next());
        //
        if (HasErrors()) return;
...
void BOPAlgo_CheckerSI::CheckFaceSelfIntersection(const Message_ProgressRange& theRange) {
    ...
    for (Standard_Integer i = 0; i < aNbS; i++) {
        ...
        if (aSurfType == GeomAbs_Torus) {
            ...
            if (aMajorRadius > aMinorRadius + Precision::Confusion()) continue;
        }
