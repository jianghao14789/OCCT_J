...
static Standard_Boolean IsInside(const TopoDS_Shape&, const TopoDS_Shape&, Handle(IntTools_Context) &);
static void MakeInternalWires(const TopTools_IndexedMapOfShape&, TopTools_ListOfShape&);

//=======================================================================
// function :
// purpose  :
//=======================================================================
BOPAlgo_BuilderFace::BOPAlgo_BuilderFace() : BOPAlgo_BuilderArea() {
...
void BOPAlgo_BuilderFace::PerformAreas(const Message_ProgressRange& theRange) {
    ...
    if (myLoops.IsEmpty()) {
        if (myContext->IsInfiniteFace(myFace)) {
            ...
            if (BRep_Tool::NaturalRestriction(myFace)) aBB.NaturalRestriction(aFace, Standard_True);
    ...
    for (; aItLS.More(); aItLS.Next(), aPSHoles.Next()) {
        ...
        for (; aItLI.More(); aItLI.Next()) {
            ...
            if (!IsInside(aHole, aFace, myContext)) continue;
    ...
    for (i = 1; i <= aNbH; ++i) {
        ...
        if (!pLHoles) pLHoles = &aFaceHolesMap(aFaceHolesMap.Add(aFace, TopTools_ListOfShape()));
    ...
    if (aHoleFaces.Extent() != aHoleFaceMap.Extent()) {
        ...
        if (aBoxF.IsOpenXmin() || aBoxF.IsOpenXmax() || aBoxF.IsOpenYmin() || aBoxF.IsOpenYmax() ||
            ...
            for (i = 1; i <= aNbH; ++i) {
                ...
                if (!aHoleFaceMap.Contains(aHole)) anUnUsedHoles.Append(aHole);
            }
            // Save it
            aNewFaces.Append(aFace);
        }
...
void BOPAlgo_BuilderFace::PerformInternalShapes(const Message_ProgressRange& theRange) {
    ...
    for (; itLF.More(); itLF.Next(), aPSClass.Next()) {
        ...
        for (; itLI.More(); itLI.Next()) {
            ...
            if (aMEDone.Contains(nE)) continue;
        ...
        if (anEdgesInside.IsEmpty()) continue;
    ...
    for (Standard_Integer i = 1; i <= anEdgesMap.Extent(); ++i) {
        if (!aMEDone.Contains(i)) anEdgesUnUsed.Add(anEdgesMap(i));
    }
...
// purpose  :
//=======================================================================
void MakeInternalWires(const TopTools_IndexedMapOfShape& theME, TopTools_ListOfShape& theWires) {
...
// purpose  :
//=======================================================================
Standard_Boolean IsInside(const TopoDS_Shape& theWire, const TopoDS_Shape& theF,
...
// purpose  :
//=======================================================================
Standard_Boolean IsGrowthWire(const TopoDS_Shape& theWire, const TopTools_IndexedMapOfShape& theMHE) {
    if (theMHE.Extent()) {
        TopoDS_Iterator aIt(theWire);
        for (; aIt.More(); aIt.Next()) {
            if (theMHE.Contains(aIt.Value())) return Standard_True;
        }
    }
