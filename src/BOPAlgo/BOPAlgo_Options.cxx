...
void BOPAlgo_LoadMessages() {
    if (BOPAlgo_InitMessages) return;
...
BOPAlgo_Options::BOPAlgo_Options()
    : myAllocator(NCollection_BaseAllocator::CommonBaseAllocator()), myReport(new Message_Report),
      myRunParallel(myGlobalRunParallel), myFuzzyValue(Precision::Confusion()), myUseOBB(Standard_False) {
...
BOPAlgo_Options::BOPAlgo_Options(const Handle(NCollection_BaseAllocator) & theAllocator)
    : myAllocator(theAllocator), myReport(new Message_Report), myRunParallel(myGlobalRunParallel),
      myFuzzyValue(Precision::Confusion()), myUseOBB(Standard_False) {
    BOPAlgo_LoadMessages();
}
