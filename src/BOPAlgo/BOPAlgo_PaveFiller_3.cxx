...
public:
    ...
    //
    BOPAlgo_EdgeEdge() : IntTools_EdgeEdge(), BOPAlgo_ParallelAlgo() {};
    //
...
void BOPAlgo_PaveFiller::PerformEE(const Message_ProgressRange& theRange) {
    ...
    for (k = 0; k < aNbEdgeEdge; ++k) {
        ...
        Standard_Boolean bAnalytical = Standard_False;
        {
            ...
            //
            bAnalytical = (((aType1 == GeomAbs_Line) && (aType2 == GeomAbs_Line || aType2 == GeomAbs_Circle)) ||
                           ((aType2 == GeomAbs_Line) && (aType1 == GeomAbs_Line || aType1 == GeomAbs_Circle)));
        }
        //
        for (i = 1; i <= aNbCPrts; ++i) {
            ...
            switch (aType) {
                ...
                case TopAbs_EDGE: {
                    ...
                    myDS->AddInterf(nE1, nE2);
                    //
                    BOPAlgo_Tools::FillMap<Handle(BOPDS_PaveBlock), TColStd_MapTransientHasher>(aPB1, aPB2, aMPBLPB,
                                                                                                aAllocator);
                } // case TopAbs_EDGE
                break;
                default:
...
// purpose:
//=======================================================================
void BOPAlgo_PaveFiller::AnalyzeShrunkData(const Handle(BOPDS_PaveBlock) & thePB, const IntTools_ShrunkRange& theSR) {
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::ForceInterfVE(const Standard_Integer nV, Handle(BOPDS_PaveBlock) & aPB,
...
// purpose  :
//=======================================================================
Standard_Boolean BOPAlgo_PaveFiller::GetPBBox(const TopoDS_Edge& theE, const Handle(BOPDS_PaveBlock) & thePB,
...
void BOPAlgo_PaveFiller::UpdateVerticesOfCB() {
    ...
    for (Standard_Integer i = 0; i < aNbPBP; ++i) {
        ...
        for (; itPB.More(); itPB.Next()) {
            ...
            if (aCB.IsNull()) continue;

            const Handle(BOPDS_PaveBlock) & aPBR = aCB->PaveBlock1();
            if (!aMPBFence.Add(aPBR)) continue;
...
void BOPAlgo_PaveFiller::ForceInterfEE(const Message_ProgressRange& theRange) {
    ...
    for (Standard_Integer i = 0; i < aNbS; ++i) {
        ...
        if (aSI.ShapeType() == TopAbs_VERTEX) {
            if (myDS->HasInterf(i)) myDS->InitPaveBlocksForVertex(i);
        }
        if (UserBreak(aPSOuter)) {
            return;
        }
    ...
    NCollection_IndexedDataMap<BOPDS_Pair, BOPDS_ListOfPaveBlock, BOPDS_PairMapHasher> aPBMap(1, anAlloc);
    ...
    for (Standard_Integer i = 0; i < aNbS; ++i) {
        ...
        for (; aItLPB.More(); aItLPB.Next()) {
            ...
            if (!pList) pList = &aPBMap(aPBMap.Add(aPair, BOPDS_ListOfPaveBlock(anAlloc)));
            pList->Append(aPBR);
        }
    }
    ...
    BOPDS_VectorOfInterfEE& aEEs = myDS->InterfEE();
    if (aEEs.IsEmpty()) aEEs.SetIncrement(10);
    ...
    for (Standard_Integer i = 0; i < aNbPairs; ++i) {
        ...
        for (Standard_Integer j = 0; j < 2; ++j) {
            if (myDS->IsCommonBlock(aPB[j])) {
                ...
                for (; aItLPB.More(); aItLPB.Next())
                    BOPAlgo_Tools::FillMap<Handle(BOPDS_PaveBlock), TColStd_MapTransientHasher>(aPB[j], aItLPB.Value(),
                                                                                                aMPBLPB, anAlloc);
            }
        }
        BOPAlgo_Tools::FillMap<Handle(BOPDS_PaveBlock), TColStd_MapTransientHasher>(aPB[0], aPB[1], aMPBLPB, anAlloc);
    }
