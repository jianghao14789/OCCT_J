...
#include <algorithm>
//
static TopoDS_Face BuildDraftFace(const TopoDS_Face& theFace, const TopTools_DataMapOfShapeListOfShape& theImages,
                                  Handle(IntTools_Context) & theCtx, const Handle(Message_Report) & theReport);
...
public:
    ...
    BOPAlgo_PairOfShapeBoolean() : BOPAlgo_ParallelAlgo(), myFlag(Standard_False) {}
    //
...
public:
    ...
    BOPAlgo_VFI() : BOPAlgo_ParallelAlgo(), myIsInternal(Standard_False) {}
    //
    virtual ~BOPAlgo_VFI() {}
    //
...
void BOPAlgo_Builder::BuildSplitFaces(const Message_ProgressRange& theRange) {
    ...
    for (i = 0; i < aNbS; ++i) {
        ...
        Standard_Boolean isUClosed = Standard_False, isVClosed = Standard_False, isChecked = Standard_False;
        //
        ...
        if (!aNbPBIn && !aNbPBSc) {
            ...
            if (!aNbAV) {
                ...
                if (!hasInternals && !hasModified) continue;
            }
    ...
    for (k = 1; k <= aNbBF; ++k) {
        ...
        for (; aIt.More(); aIt.Next()) {
            ...
            if (anOriF == TopAbs_REVERSED) aFR.Orientation(TopAbs_REVERSED);
            pLFIm->Append(aFR);
        }
    }
    //
...
// purpose  :
//=======================================================================
typedef NCollection_IndexedDataMap<BOPTools_Set, TopTools_ListOfShape, BOPTools_SetMapHasher>
    BOPAlgo_IndexedDataMapOfSetListOfShape;

static void AddEdgeSet(const TopoDS_Shape& theS, BOPAlgo_IndexedDataMapOfSetListOfShape& theMap,
    ...
    if (!pLF) pLF = &theMap(theMap.Add(aSE, TopTools_ListOfShape(theAllocator)));
    pLF->Append(theS);
}
...
void BOPAlgo_Builder::FillSameDomainFaces(const Message_ProgressRange& theRange) {
    ...
    for (Standard_Integer i = 0; i < aNbFFs; ++i) {
        ...
        for (Standard_Integer j = 0; j < 2; ++j) {
            if (!myDS->HasFaceInfo(nF[j])) continue;

            if (!aMFence.Add(nF[j])) continue;
    ...
    NCollection_IndexedDataMap<BOPTools_Set, TopTools_ListOfShape, BOPTools_SetMapHasher> anESetFaces(1, aAllocator);
    ...
    for (Standard_Integer i = 0; i < aNbF; ++i) {
        ...
        Standard_Boolean bCheckPlanar = Standard_False;
        {
            ...
            if (myContext->SurfaceAdaptor(TopoDS::Face(aF)).GetType() == GeomAbs_Plane) {
                ...
                bCheckPlanar = !(aBox.IsOpenXmin() || aBox.IsOpenXmax() || aBox.IsOpenYmin() || aBox.IsOpenYmax() ||
        ...
        if (pLFSp) {
            ...
            for (; aItLF.More(); aItLF.Next()) {
                ...
                if (bCheckPlanar) aMFPlanar.Add(aItLF.Value());
            }
        } else {
            AddEdgeSet(aF, anESetFaces, aAllocator);
            if (bCheckPlanar) aMFPlanar.Add(aF);
        }
    }
    ...
    for (Standard_Integer i = 0; i < aNbPairs; ++i) {
        ...
        if (aPSB.Flag())
            BOPAlgo_Tools::FillMap<TopoDS_Shape, TopTools_ShapeMapHasher>(aPSB.Shape1(), aPSB.Shape2(), aDMSLS,
                                                                          aAllocator);
    }
    aVPSB.Clear();
    ...
    for (Standard_Integer i = 0; i < aNbS; ++i) {
        ...
        for (; aItLFIm.More(); aItLFIm.Next()) {
            ...
            if (!pLFOr) pLFOr = myOrigins.Bound(aFIm, TopTools_ListOfShape());
            pLFOr->Append(aF);
        }
    }
...
//=======================================================================
static Standard_Boolean HasMultiConnected(const TopoDS_Edge& theEdge, TopTools_DataMapOfShapeListOfShape& theMap) {
    ...
    for (; itV.More(); itV.Next()) {
        ...
        if (!pList) {
            ...
            if (!pList->Contains(theEdge)) pList->Append(theEdge);

            if (pList->Extent() > 2) return Standard_True;
        }
    }
...
//=======================================================================
TopoDS_Face BuildDraftFace(const TopoDS_Face& theFace, const TopTools_DataMapOfShapeListOfShape& theImages,
                           Handle(IntTools_Context) & theCtx, const Handle(Message_Report) & theReport) {
    BRep_Builder aBB;
    ...
    for (; aItW.More(); aItW.Next()) {
        ...
        for (; aItE.More(); aItE.Next()) {
            ...
            if (!pLEIm) {
                ...
                if (!bIsDegenerated && HasMultiConnected(aE, aVerticesCounter)) return TopoDS_Face();

                // Check edges unification
                if (!bIsClosed && !aMEdges.Add(aE)) return TopoDS_Face();
            ...
            for (; aItLEIm.More(); aItLEIm.Next()) {
                ...
                if (!bIsDegenerated && HasMultiConnected(aSp, aVerticesCounter)) return TopoDS_Face();

                // Check edges unification
                if (!bIsClosed && !aMEdges.Add(aSp)) return TopoDS_Face();
                ...
                if (BOPTools_AlgoTools::IsSplitToReverseWithWarn(aSp, aE, theCtx, theReport)) aSp.Reverse();
    ...
    if (theFace.Orientation() == TopAbs_REVERSED) aDraftFace.Reverse();
