...
BOPAlgo_Builder::BOPAlgo_Builder()
    : BOPAlgo_BuilderShape(), myArguments(myAllocator), myMapFence(100, myAllocator), myPaveFiller(NULL), myDS(NULL),
      myEntryPoint(0), myImages(100, myAllocator), myShapesSD(100, myAllocator), myOrigins(100, myAllocator),
      myInParts(100, myAllocator), myNonDestructive(Standard_False), myGlue(BOPAlgo_GlueOff),
...
BOPAlgo_Builder::BOPAlgo_Builder(const Handle(NCollection_BaseAllocator) & theAllocator)
    : BOPAlgo_BuilderShape(theAllocator), myArguments(myAllocator), myMapFence(100, myAllocator), myPaveFiller(NULL),
      myDS(NULL), myEntryPoint(0), myImages(100, myAllocator), myShapesSD(100, myAllocator),
      myOrigins(100, myAllocator), myInParts(100, myAllocator), myNonDestructive(Standard_False),
      myGlue(BOPAlgo_GlueOff), myCheckInverted(Standard_True) {}
//=======================================================================
// function : ~
// purpose  :
//=======================================================================
...
// purpose  :
//=======================================================================
void BOPAlgo_Builder::BuildBOP(const TopTools_ListOfShape& theObjects, const TopAbs_State theObjState,
                               const TopTools_ListOfShape& theTools, const TopAbs_State theToolsState,
                               const Message_ProgressRange& theRange, Handle(Message_Report) theReport) {
    if (HasErrors()) return;
        ...
        for (; itLS.More(); itLS.Next()) {
            ...
            for (; expS.More(); expS.Next()) {
                ...
                for (; expF.More(); expF.Next()) {
                    ...
                    if (aF.Orientation() != TopAbs_FORWARD && aF.Orientation() != TopAbs_REVERSED) continue;
                    ...
                    if (pLFIm) {
                        ...
                        for (; itLFIm.More(); itLFIm.Next()) {
                            ...
                            if (BOPTools_AlgoTools::IsSplitToReverse(aFIm, aF, myContext)) aFIm.Reverse();
                            aMapOri.Add(aFIm);
                            aMap.Add(aFIm);
                        }
                    } else {
    ...
    Standard_Boolean isObjectsIN = (theObjState == TopAbs_IN), isToolsIN = (theToolsState == TopAbs_IN);
    ...
    for (Standard_Integer i = 0; i < 2; ++i) {
        ...
        for (Standard_Integer j = 1; j <= aNbF; ++j) {
            ...
            if (!aMFence.Add(aFIm)) {
                if (!anOppositeMap.Contains(aFIm)) {
                    ...
                    if (isSameOriNeeded == isSameOri) {
                        ...
                        if (aMResFacesFence.Add(aFIm)) aMResFacesOri.Add(aFIm);
                    } else
                        // Remove the face
                        aMFToAvoid.Add(aFIm);
            ...
            if (!aMFenceOri.Add(aFIm)) continue;
    ...
    for (Standard_Integer i = 1; i <= aNbRF; ++i) {
        ...
        if (!aMFToAvoid.Contains(aRF)) aResFaces.Append(aRF);
    }
    ...
    if (!aBS.HasErrors()) {
        ...
        for (; itA.More(); itA.Next()) {
            ...
            for (; expF.More(); expF.Next()) {
                const TopoDS_Shape& aF = expF.Current();
                if (aMObjFacesOri.Contains(aF) || aMToolFacesOri.Contains(aF)) break;
            }
            if (expF.More()) {
    ...
    for (; itLF.More(); itLF.Next()) {
        if (aMFence.Add(itLF.Value())) aBB.Add(anUnUsedFaces, itLF.Value());
    }
                ...
                for (; expS.More(); expS.Next()) {
                    ...
                    for (TopoDS_Iterator it(aS); it.More(); it.Next()) {
                        ...
                        if (aSInt.Orientation() == TopAbs_INTERNAL)
                            ...
                            if (itInt.More() && itInt.Value().Orientation() == TopAbs_INTERNAL) anInParts.Append(aSInt);
                        }
                    }
                }
            }
