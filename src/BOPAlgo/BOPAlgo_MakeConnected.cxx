...
void BOPAlgo_MakeConnected::MakeConnected() {
    ...
    if (myGlueHistory.IsNull()) myGlueHistory = new BRepTools_History;
    ...
    for (Standard_Integer i = 1; i <= aNbS; ++i) {
        ...
        for (Standard_Integer j = 0; j < 2; ++j) {
            ...
            for (; itLH.More(); itLH.Next()) {
                ...
                if (!pLOr) pLOr = myOrigins.Bound(aHS, TopTools_ListOfShape());
                if (!pLOr->Contains(aS)) pLOr->Append(aS);
            }
        }
    }
}
...
void BOPAlgo_MakeConnected::AssociateMaterials() {
    ...
    for (; itLS.More(); itLS.Next()) {
        ...
        for (; anExp.More(); anExp.Next()) {
            ...
            if (!pLM) pLM = myMaterials.Bound(anElement, TopTools_ListOfShape());
            pLM->Append(aSOr);
        }
    }
}
...
void BOPAlgo_MakeConnected::Update() {
    // Update history
    myHistory->Clear();
    if (!myGlueHistory.IsNull()) myHistory->Merge(myGlueHistory);
    if (!myPeriodicityMaker.History().IsNull()) myHistory->Merge(myPeriodicityMaker.History());
...
//=======================================================================
void BOPAlgo_MakeConnected::RepeatShape(const Standard_Integer theDirectionID, const Standard_Integer theTimes) {
    if (HasErrors()) return;
