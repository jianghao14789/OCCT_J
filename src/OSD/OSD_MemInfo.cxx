...
void OSD_MemInfo::Update() {
    ...
    if (IsActive(MemPrivate) || IsActive(MemWorkingSet) || IsActive(MemWorkingSetPeak) || IsActive(MemSwapUsage) ||
...
#define HAS_MALLINFO2
    ...
    if (!IsActive(MemVirtual) && !IsActive(MemWorkingSet) && !IsActive(MemWorkingSetPeak) && !IsActive(MemPrivate)) {
        return;
    }
    ...
    if (IsActive(MemVirtual) || IsActive(MemWorkingSet) || IsActive(MemHeapUsage)) {
        ...
        if (task_info(mach_task_self(), TASK_BASIC_INFO, (task_info_t)&aTaskInfo, &aTaskInfoCount) == KERN_SUCCESS) {
...
TCollection_AsciiString OSD_MemInfo::ToString() const {
    ...
    if (hasValue(MemWorkingSet)) {
        anInfo +=
            TCollection_AsciiString("  Working Set:        ") + Standard_Integer(ValueMiB(MemWorkingSet)) + " MiB";
...
Standard_Size OSD_MemInfo::ValueMiB(const OSD_MemInfo::Counter theCounter) const {
    ...
    return (myCounters[theCounter] == Standard_Size(-1)) ? Standard_Size(-1) : (myCounters[theCounter] / (1024 * 1024));
}
...
Standard_Real OSD_MemInfo::ValuePreciseMiB(const OSD_MemInfo::Counter theCounter) const {
    ...
    return (myCounters[theCounter] == Standard_Size(-1)) ? -1.0
