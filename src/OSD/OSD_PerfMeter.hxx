...
//! This class enables measuring the CPU time between two points of code execution, regardless of the scope of these
//! points of code. A meter is identified by its name (string). So multiple objects in various places of user code may
//! point to the same meter. The results will be printed on stdout upon finish of the program. For details see
//! OSD_PerfMeter.h
class OSD_PerfMeter {
...
public:
    ...
    OSD_PerfMeter(const char* theMeter, const bool theToAutoStart = true) : myIMeter(perf_get_meter(theMeter, 0, 0)) {
    ...
    //! Starts the meter
    void Start() const {
        perf_start_imeter(myIMeter);
    }

    //! Stops the meter
    void Stop() const {
        perf_stop_imeter(myIMeter);
    }

    //! Increments the counter w/o time measurement
    void Tick() const {
        perf_tick_imeter(myIMeter);
    }

    //! Outputs the meter data and resets it to initial state
    void Flush() const {
        perf_close_imeter(myIMeter);
    }
