...
public:
    ...
    //! Note that registering protocols is not thread-safe operation and expected to be done once at application
    //! startup.
...
public:
    ...
    //! @param theMode      [in] flags describing the requested input mode for the stream (std::ios_base::in will be
    //! implicitly added)
    //! @param theOffset    [in] expected stream position from the beginning of the file (beginning of the stream by
    //! default);
    ...
    Standard_EXPORT virtual opencascade::std::shared_ptr<std::istream> OpenIStream(
        const TCollection_AsciiString& theUrl, const std::ios_base::openmode theMode, const int64_t theOffset = 0,
    ...
    //! Default implementation create a stream from file buffer returned by OSD_FileSystem::OpenFileBuffer().
    //! @param theUrl       [in] path to open
    //! @param theMode      [in] flags describing the requested output mode for the stream (std::ios_base::out will be
    //! implicitly added)
    ...
    OpenOStream(const TCollection_AsciiString& theUrl, const std::ios_base::openmode theMode);

    //! Opens stream buffer for specified file URL.
    //! @param theUrl        [in]  path to open
    //! @param theMode       [in]  flags describing the requested input mode for the stream
    //! @param theOffset     [in]  expected stream position from the beginning of the buffer (beginning of the stream
    //! buffer by default)
