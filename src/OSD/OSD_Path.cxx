...
static OSD_SysType whereAmI() {
    ...
    return OSD_UnixBSD;
#elif defined(sgi) || defined(IRIX) || defined(__sun) || defined(SOLARIS) || defined(__sco__) || defined(__hpux) ||    \
    defined(HPUX)
...
static void VmsExtract(const TCollection_AsciiString& what, TCollection_AsciiString& node,
                       TCollection_AsciiString& username, TCollection_AsciiString& password,
                       TCollection_AsciiString& disk, TCollection_AsciiString& trek, TCollection_AsciiString& name,
    ...
    if (buffer.Search("\"") != -1) { // a username to extract
        ...
        if (buffer.Search("::") != -1) buffer.Remove(1, 2); // Removes <<::>>
...
// purpose  :
//=======================================================================
static void UnixExtract(const TCollection_AsciiString& what, TCollection_AsciiString& node,
                        TCollection_AsciiString& username, TCollection_AsciiString& password,
                        TCollection_AsciiString& trek, TCollection_AsciiString& name, TCollection_AsciiString& ext) {
...
// purpose  :
//=======================================================================
static void DosExtract(const TCollection_AsciiString& what, TCollection_AsciiString& disk,
                       TCollection_AsciiString& trek, TCollection_AsciiString& name, TCollection_AsciiString& ext) {
...
// purpose  :
//=======================================================================
static void MacExtract(const TCollection_AsciiString& what, TCollection_AsciiString&, TCollection_AsciiString& trek,
                       TCollection_AsciiString& name, TCollection_AsciiString&) {
...
OSD_Path::OSD_Path(const TCollection_AsciiString& Nod, const TCollection_AsciiString& UsrNm,
                   const TCollection_AsciiString& Passwd, const TCollection_AsciiString& Dsk,
                   const TCollection_AsciiString& Trk, const TCollection_AsciiString& Nam,
...
void OSD_Path::Values(TCollection_AsciiString& Nod, TCollection_AsciiString& UsrNm, TCollection_AsciiString& Passwd,
                      TCollection_AsciiString& Dsk, TCollection_AsciiString& Trk, TCollection_AsciiString& Nam,
...
void OSD_Path::SetValues(const TCollection_AsciiString& Nod, const TCollection_AsciiString& UsrNm,
                         const TCollection_AsciiString& Passwd, const TCollection_AsciiString& Dsk,
                         const TCollection_AsciiString& Trk, const TCollection_AsciiString& Nam,
                         const TCollection_AsciiString& ext) {
    myNode = Nod;
...
void OSD_Path::RemoveATrek(const Standard_Integer thewhere) {
    ...
    if (posit != -1) myTrek.Remove(posit);
}
...
TCollection_AsciiString OSD_Path::TrekValue(const Standard_Integer thewhere) const {
    ...
    if (result == "") throw Standard_NumericError("OSD_Path::TrekValue : where is invalid");
...
void OSD_Path::SystemName(TCollection_AsciiString& FullName, const OSD_SysType aType) const {
    ...
    switch (pType) {
        ...
        case OSD_WindowsNT: // MSDOS-like syntax
        {
            ...
            if (length != 1)
                ...
                if (myDisk.Length() != 0) {
                    ...
                    if ((length == 1 && IsAlphabetic(myDisk.Value(1))) ||                        // 'A/a' to 'Z/z'
                        (length == 2 && IsAlphabetic(myDisk.Value(1)) && myDisk.Value(2) == ':') // 'A:' to 'Z:'
            ...
            if (Way.Length() != 0) FullName = FullName + Way + "\\";
        ...
        default: // UNIX-like syntax
            ...
            if (myUserName.Length() != 0 && myNode.Length() != 0) { // If USER name
                ...
                if (myPassword.Length() != 0) FullName = FullName + "\"" + myPassword + "\""; // a password if not empty
...
void OSD_Path::SetSystemName(const TCollection_AsciiString& aDependentName, const OSD_SysType aSysType) {
    UnixExtract(aDependentName, myNode, myUserName, myPassword, myTrek, myName, myExtension);
}
#endif
...
OSD_Path::OSD_Path() : myUNCFlag(Standard_False), mySysDep(OSD_WindowsNT) {} // end constructor ( 1 )
...
OSD_Path::OSD_Path(const TCollection_AsciiString& aNode, const TCollection_AsciiString& aUsername,
...
void OSD_Path::Values(TCollection_AsciiString& aNode, TCollection_AsciiString& aUsername,
    ...
    aDisk = myDisk;
    aTrek = myTrek;
    if (!aTrek.IsEmpty() && aTrek.Value(aTrek.Length()) != '|') aTrek += "|"; // (LD)
    aName = myName;
    anExtension = myExtension;

} // end OSD_Path :: Values

void OSD_Path::SetValues(const TCollection_AsciiString& aNode, const TCollection_AsciiString& aUsername,
                         const TCollection_AsciiString& aPassword, const TCollection_AsciiString& aDisk,
                         const TCollection_AsciiString& aTrek, const TCollection_AsciiString& aName,
                         const TCollection_AsciiString& anExtension) {
    ...
    if (myExtension.Length() && myExtension.Value(1) != '.') myExtension.Insert(1, '.');
...
void OSD_Path::DownTrek(const TCollection_AsciiString& aName) {
    ...
    if (!aName.IsEmpty() && aName.Value(1) != '|' && pos && myTrek.Value(pos) != '|') myTrek += "|";
...
Standard_Integer OSD_Path::TrekLength() const {
    ...
    if (myTrek.IsEmpty() || (myTrek.Length() == 1 && myTrek.Value(1) == '|')) return retVal;

    for (;;) {

        if (myTrek.Token("|", i++).IsEmpty()) break;
...
void OSD_Path::RemoveATrek(const Standard_Integer thewhere) {
    ...
    if (i) {
        ...
        if (j == 0) j = myTrek.Length() + 1;

        myTrek.Remove(i, j - i);

    } // end if

    if (flag) myTrek.Remove(1);
...
void OSD_Path::RemoveATrek(const TCollection_AsciiString& aName) {
    ...
    if (tmp.Value(1) != '|') tmp.Insert(1, '|');

    if (tmp.Value(tmp.Length()) != '|') tmp += '|';

    i = myTrek.Search(tmp);

    if (i != -1) myTrek.Remove(i + 1, tmp.Length() - 1);

    if (flag) myTrek.Remove(1);

    if (myTrek.Value(myTrek.Length()) == '|') myTrek.Trunc(myTrek.Length() - 1);
...
TCollection_AsciiString OSD_Path::Trek() const {
    ...
    TCollection_AsciiString retVal;
    retVal = myTrek;
    if (!retVal.IsEmpty() && retVal.Value(retVal.Length()) != '|') retVal += "|"; // (LD)
    return retVal;
...
// purpose  :
// =======================================================================
Standard_Boolean OSD_Path::IsValid(const TCollection_AsciiString& theDependentName, const OSD_SysType theSysType) {
...
static Standard_Integer RemoveExtraSeparator(TCollection_AsciiString& aString) {
    ...
    len = aString.Length();
#ifdef _WIN32
    if (len > 1 && aString.Value(1) == '/' && aString.Value(2) == '/') start = 2;
#endif
...
TCollection_AsciiString OSD_Path::RelativePath(const TCollection_AsciiString& aDirPath,
    ...
    if (aDirPath.Search(":") == 2) { // Cas WNT
        ...
        FilePath.ChangeAll('\\', '/');
        if (FilePath.Search("/") != 3) return EmptyString;
    } else { // Cas Unix
        if (aDirPath.Value(1) != '/' || FilePath.Value(1) != '/') return EmptyString;
    }
    ...
    for (i = n = 1;; n++) {
        ...
        if (DirToken.IsEmpty()) return FilePath;

        if (!Sibling) {
            len = FilePath.Length();
            i = FilePath.Search("/");
            if (i > 0) {
                if (i == len) return EmptyString;
...
TCollection_AsciiString OSD_Path::AbsolutePath(const TCollection_AsciiString& aDirPath,
                                               const TCollection_AsciiString& aRelFilePath) {
    TCollection_AsciiString EmptyString = "";
    if (aRelFilePath.Search("/") == 1 || aRelFilePath.Search(":") == 2) return aRelFilePath;
    TCollection_AsciiString DirPath = aDirPath, RelFilePath = aRelFilePath;
    Standard_Integer i, len;

    if (DirPath.Search("/") != 1 && DirPath.Search(":") != 2) return EmptyString;

    if (DirPath.Search(":") == 2) DirPath.ChangeAll('\\', '/');
    RelFilePath.ChangeAll('\\', '/');
    RemoveExtraSeparator(DirPath);
    len = RemoveExtraSeparator(RelFilePath);

    while (RelFilePath.Search("../") == 1) {
        if (len == 3) return EmptyString;
        RelFilePath = RelFilePath.SubString(4, len);
        len -= 3;
        if (DirPath.IsEmpty()) return EmptyString;
        i = DirPath.SearchFromEnd("/");
        if (i < 0) return EmptyString;
        DirPath.Trunc(i - 1);
    }
    DirPath += '/';
    DirPath += RelFilePath;
...
// purpose  :
// =======================================================================
void OSD_Path::FolderAndFileFromPath(const TCollection_AsciiString& theFilePath, TCollection_AsciiString& theFolder,
...
// purpose  :
// =======================================================================
void OSD_Path::FileNameAndExtension(const TCollection_AsciiString& theFilePath, TCollection_AsciiString& theName,
