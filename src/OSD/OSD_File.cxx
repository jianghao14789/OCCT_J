...
static int OSD_File_getBuffer(HANDLE theChannel, char* theBuffer, DWORD theSize, BOOL theIsPeek, BOOL theIsSocket) {
    if (theIsSocket) {
...
Standard_Integer __fastcall _get_file_type(Standard_CString theFileName, HANDLE theFileHandle) {
    const int aFileType = theFileHandle == INVALID_HANDLE_VALUE ? FILE_TYPE_DISK : GetFileType(theFileHandle);
    switch (aFileType) {
...
static HANDLE OSD_File_openFile(const TCollection_AsciiString& theFileName, OSD_OpenMode theOpenMode, DWORD theOptions,
                                bool* theIsNew = NULL) {
...
#ifndef OCCT_UWP
    HANDLE aFileHandle = CreateFileW(aFileNameW.ToWideString(), dwDesiredAccess, FILE_SHARE_READ | FILE_SHARE_WRITE,
    ...
    HANDLE aFileHandle = CreateFile2(aFileNameW.ToWideString(), dwDesiredAccess, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                     dwCreationDistribution, &pCreateExParams);
#endif
    if (aFileHandle != INVALID_HANDLE_VALUE || theOptions != OPEN_APPEND || GetLastError() != ERROR_FILE_NOT_FOUND) {
        return aFileHandle;
    }

    dwCreationDistribution = CREATE_ALWAYS;
#ifndef OCCT_UWP
    aFileHandle = CreateFileW(aFileNameW.ToWideString(), dwDesiredAccess, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
    ...
    aFileHandle = CreateFile2(aFileNameW.ToWideString(), dwDesiredAccess, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
#else
      myFileChannel(-1), myFILE(NULL),
#endif
      myIO(0), myLock(OSD_NoLock), myMode(OSD_ReadWrite), ImperativeFlag(Standard_False) {
    //
}
...
void OSD_File::BuildTemporary() {
    ...
    for (int aKeyIter = 0; aKeyIter < 2; ++aKeyIter) {
        HKEY aRegKey = NULL;
        if (RegOpenKeyExW(TheRegKeys[aKeyIter].hKey, TheRegKeys[aKeyIter].keyPath, 0, KEY_QUERY_VALUE, &aRegKey) !=
            ERROR_SUCCESS) {
            continue;
        }
...
// purpose  :
// =======================================================================
void OSD_File::ReadLine(TCollection_AsciiString& theBuffer, const Standard_Integer theNbBytes,
    ...
    if (myIO & FLAG_FILE) {
...
#ifndef OCCT_UWP
        aNbBytesRead = (DWORD)OSD_File_getBuffer(myFileHandle, &aBuffer.ChangeFirst(), (DWORD)theNbBytes, TRUE,
                                                 myIO & FLAG_SOCKET);
        ...
        } else if (aNbBytesRead == 0) // connection closed - set end-of-file flag
        {
            ...
            theNbBytesRead = OSD_File_getLine(&aBuffer.ChangeFirst(), aNbBytesRead, aSeekPos);
            if (theNbBytesRead ==
                -1) // last character in the buffer is <CR> - peek next character to see if it is a <LF>
            {
...
// purpose  :
// =======================================================================
void OSD_File::Read(const Standard_Address theBuffer, const Standard_Integer theNbBytes,
...
void OSD_File::Write(const Standard_Address theBuffer, const Standard_Integer theNbBytes) {
    ...
    DWORD aNbWritten = 0;
    if (!WriteFile(myFileHandle, theBuffer, (DWORD)theNbBytes, &aNbWritten, NULL) || aNbWritten != (DWORD)theNbBytes) {
        _osd_wnt_set_error(myError, OSD_WFile);
    }
#else
...
// purpose  :
// =======================================================================
Standard_Boolean OSD_File::ReadLastLine(TCollection_AsciiString& theLine, const Standard_Integer theDelay,
...
PSECURITY_DESCRIPTOR __fastcall _osd_wnt_protection_to_sd(const OSD_Protection& theProtection, BOOL theIsDir,
    ...
    retry:
        ...
        dwACLsize +=
            (((GetLengthSid(pSIDowner) + ACE_HEADER_SIZE) << j) + ((GetLengthSid(pSIDadmin) + ACE_HEADER_SIZE) << j) +
    ...
    __try {
        ...
        for (DWORD anAceIter = 0; anAceIter < pACL->AceCount; ++anAceIter) {
            ...
            if (GetAce(pACL, anAceIter, &pACE)) {
                const DWORD dwAccess = ((PACE_HEADER)pACE)->AceType == ACCESS_DENIED_ACE_TYPE ? 0 : *GET_MSK(pACE);
        ...
        theProtection.SetValues(aGetProtFunc(dwAccessAdmin), aGetProtFunc(dwAccessOwner), aGetProtFunc(dwAccessGroup),
