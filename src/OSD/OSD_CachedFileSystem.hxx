...
//! File system keeping last stream created by linked file system (OSD_FileSystem::DefaultFileSystem() by default) to be
//! reused for opening a stream with the same URL. Note that as file is kept in opened state, application will need
//! destroying this object to ensure all files being closed. This interface could be handy in context of reading
//! numerous objects pointing to the same file (at different offset). Make sure to create a dedicated
...
public:
    ...
    IsSupportedPath(const TCollection_AsciiString& theUrl) const Standard_OVERRIDE;

    //! Returns TRUE if current input stream is opened for reading operations.
    Standard_EXPORT virtual Standard_Boolean
    IsOpenIStream(const opencascade::std::shared_ptr<std::istream>& theStream) const Standard_OVERRIDE;

    //! Returns TRUE if current output stream is opened for writing operations.
    Standard_EXPORT virtual Standard_Boolean
    IsOpenOStream(const opencascade::std::shared_ptr<std::ostream>& theStream) const Standard_OVERRIDE;

    //! Opens stream for specified file URL for reading operations or returns previously created stream pointing to the
    //! same URL.
    Standard_EXPORT virtual opencascade::std::shared_ptr<std::istream>
    OpenIStream(const TCollection_AsciiString& theUrl, const std::ios_base::openmode theParams, const int64_t theOffset,
    ...
    Standard_EXPORT virtual opencascade::std::shared_ptr<std::ostream>
    OpenOStream(const TCollection_AsciiString& theUrl, const std::ios_base::openmode theMode) Standard_OVERRIDE;

    //! Opens stream buffer for specified file URL.
    Standard_EXPORT virtual opencascade::std::shared_ptr<std::streambuf>
    OpenStreamBuffer(const TCollection_AsciiString& theUrl, const std::ios_base::openmode theMode,
                     const int64_t theOffset = 0, int64_t* theOutBufSize = NULL) Standard_OVERRIDE;
