...
int __cdecl MyAllocHook(int nAllocType, void* pvData, size_t nSize, int nBlockUse, long lRequest,
...
OSD_MAllocHook::LogFileHandler::LogFileHandler() : myBreakSize(0) {
...
struct StorageInfo {
    ...
    StorageInfo(Standard_Size theSize = 0) : size(theSize), nbAlloc(0), nbFree(0), nbLeftPeak(0), alive() {}
...
Standard_Boolean OSD_MAllocHook::LogFileHandler::MakeReport(const char* theLogFile, const char* theOutFile,
                                                            const Standard_Boolean theIncludeAlive) {
    // open log file
    FILE* aLogFile = fopen(theLogFile, "r");
    if (aLogFile == NULL) return Standard_False;
    ...
    while (fgets(aStr, MAX_STR - 1, aLogFile) != NULL) {
        ...
        *pStr++ = '\0';
        ...
        if (aFound == aStMap.end()) aFound = aStMap.insert(aSuchInfo).first;
        StorageInfo& aInfo = const_cast<StorageInfo&>(*aFound);
        if (isAlloc) {
            if (aInfo.nbAlloc + 1 > 0) aInfo.nbAlloc++;
            aTotalLeftSize += aSize;
            if (aTotalLeftSize > aTotalPeakSize) aTotalPeakSize = aTotalLeftSize;
            int nbLeft = aInfo.nbAlloc - aInfo.nbFree;
            if (nbLeft > aInfo.nbLeftPeak) aInfo.nbLeftPeak = nbLeft;
            ...
            if (aInfo.nbAlloc + 1 > 0) aInfo.nbFree++;
        }
    }
    fclose(aLogFile);
    ...
    aRepFile << std::setw(20) << "BlockSize " << std::setw(10) << "NbAlloc " << std::setw(10) << "NbLeft "
             << std::setw(10) << "NbLeftPeak " << std::setw(20) << "AllocSize " << std::setw(20) << "LeftSize "
    ...
    for (std::set<StorageInfo>::const_iterator it = aStMap.begin(); it != aStMap.end(); ++it) {
        ...
        aRepFile << std::setw(20) << aInfo.size << ' ' << std::setw(10) << aInfo.nbAlloc << ' ' << std::setw(10)
                 << nbLeft << ' ' << std::setw(10) << aInfo.nbLeftPeak << ' ' << std::setw(20) << aSizeAlloc << ' '
                 << std::setw(20) << aSizeLeft << ' ' << std::setw(20) << aSizePeak << std::endl;
        ...
        if (theIncludeAlive && !aInfo.alive.empty()) {
            for (std::set<unsigned long>::const_iterator it1 = aInfo.alive.begin(); it1 != aInfo.alive.end(); ++it1)
                aRepFile << std::setw(10) << *it1;
        }
    }
    aRepFile << std::setw(20) << "Total:" << std::setw(10) << "" << ' ' << std::setw(10) << "" << ' ' << std::setw(10)
             << "" << ' ' << (aTotAlloc == SIZE_MAX ? '>' : ' ') << std::setw(20) << aTotAlloc << ' ' << std::setw(20)
             << aTotalLeftSize << ' ' << std::setw(20) << aTotalPeakSize << std::endl;
...
void OSD_MAllocHook::LogFileHandler::AllocEvent(size_t theSize, long theRequestNum) {
    if (myLogFile.is_open()) {
        myMutex.Lock();
        myLogFile << "alloc " << std::setw(10) << theRequestNum << std::setw(20) << theSize << std::endl;
        if (myBreakSize == theSize) place_for_breakpoint();
        myMutex.Unlock();
    }
}
...
void OSD_MAllocHook::LogFileHandler::FreeEvent(void* /*theData*/, size_t theSize, long theRequestNum) {
    if (myLogFile.is_open()) {
        myMutex.Lock();
...
OSD_MAllocHook::CollectBySize::CollectBySize()
    : myArray(NULL), myTotalLeftSize(0), myTotalPeakSize(0), myBreakSize(0), myBreakPeak(0) {
    Reset();
}
...
Standard_Boolean OSD_MAllocHook::CollectBySize::MakeReport(const char* theOutFile) {
    // print the report
    std::ofstream aRepFile(theOutFile);
    if (!aRepFile.is_open()) return Standard_False;
    std::locale aCLoc("C");
    aRepFile.imbue(aCLoc);

    aRepFile << std::setw(10) << "BlockSize " << std::setw(10) << "NbAlloc " << std::setw(10) << "NbLeft "
             << std::setw(10) << "NbLeftPeak " << std::setw(20) << "AllocSize " << std::setw(20) << "LeftSize "
             << std::setw(20) << "PeakSize " << std::endl;
    ...
    for (int i = 0; i < MAX_ALLOC_SIZE; i++) {
        if (myArray[i].nbAlloc > 0 || myArray[i].nbFree > 0) {
            ...
            aRepFile << std::setw(10) << aSize << ' ' << std::setw(10) << myArray[i].nbAlloc << ' ' << std::setw(10)
                     << nbLeft << ' ' << std::setw(10) << myArray[i].nbLeftPeak << ' ' << std::setw(20) << aSizeAlloc
                     << ' ' << std::setw(20) << aSizeLeft << ' ' << std::setw(20) << aSizePeak << std::endl;
    ...
    aRepFile << std::setw(10) << "Total:" << ' ' << std::setw(10) << "" << ' ' << std::setw(10) << "" << ' '
             << std::setw(10) << "" << ' ' << (aTotAlloc == SIZE_MAX ? '>' : ' ') << std::setw(20) << aTotAlloc << ' '
             << std::setw(20) << myTotalLeftSize << ' ' << std::setw(20) << myTotalPeakSize << std::endl;
    aRepFile.close();
    return Standard_True;
}
...
void OSD_MAllocHook::CollectBySize::AllocEvent(size_t theSize, long /*theRequestNum*/) {
    ...
    if (theSize > 0) {
        ...
        if (myTotalLeftSize > (ptrdiff_t)myTotalPeakSize) myTotalPeakSize = myTotalLeftSize;
        myMutex.Unlock();
    }
}
...
void OSD_MAllocHook::CollectBySize::FreeEvent(void* /*theData*/, size_t theSize, long /*theRequestNum*/) {
    if (theSize > 0) {
        myMutex.Lock();
        int ind = (theSize > MAX_ALLOC_SIZE ? MAX_ALLOC_SIZE - 1 : (int)(theSize - 1));
        myArray[ind].nbFree++;
