...
//! This class predefines specific list of supported devices, which does not depend on 3rdparty library provided by
//! mouse vendor. Supported input chunks:
...
//!    if (aSize == 0 || ::GetRawInputData ((HRAWINPUT )theLParam, RID_INPUT, aRawData, &aSize, sizeof(RAWINPUTHEADER))
//!    != aSize)
//!    {
//!      break;
//!    }
...
public:
    //! Main constructor.
    Standard_EXPORT WNT_HIDSpaceMouse(unsigned long theProductId, const Standard_Byte* theData, Standard_Size theSize);
    ...
    bool IsKeyState() const {
        return myData[0] == SpaceRawInput_KeyState;
    }

    //! Return new keystate.
    uint32_t KeyState() const {
        return *reinterpret_cast<const uint32_t*>(myData + 1);
    }
...
private:
    ...
    Graphic3d_Vec3d fromRawVec3(bool& theIsIdle, const Standard_Byte* theData, bool theIsTrans,
