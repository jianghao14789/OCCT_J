...
public:
    ...
    Standard_EXPORT WNT_Window(const Standard_CString theTitle, const Handle(WNT_WClass) & theClass,
                               const WNT_Dword& theStyle, const Standard_Integer thePxLeft,
                               const Standard_Integer thePxTop, const Standard_Integer thePxWidth,
                               const Standard_Integer thePxHeight,
                               const Quantity_NameOfColor theBackColor = Quantity_NOC_MATRAGRAY,
                               const Aspect_Handle theParent = 0, const Aspect_Handle theMenu = 0,
    ...
    Standard_EXPORT void SetPos(const Standard_Integer X, const Standard_Integer Y, const Standard_Integer X1,
    ...
    Standard_EXPORT virtual void Position(Standard_Integer& X1, Standard_Integer& Y1, Standard_Integer& X2,
    ...
    //! Invalidate entire window content by calling InvalidateRect() WinAPI function, resulting in WM_PAINT event put
    //! into window message loop. Method can be called from non-window thread, and system will also automatically
    //! aggregate multiple events into single one.
    Standard_EXPORT virtual void InvalidateContent(
        const Handle(Aspect_DisplayConnection) & theDisp = Handle(Aspect_DisplayConnection)()) Standard_OVERRIDE;
