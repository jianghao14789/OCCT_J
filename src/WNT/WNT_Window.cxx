...
public:
    ...
    typedef BOOL(WINAPI* GetTouchInputInfo_t)(HTOUCHINPUT hTouchInput, UINT cInputs, PTOUCHINPUT pInputs, int cbSize);
...
public:
    //! Main constructor.
    TouchInputHelper()
        : myRegisterTouchWindow(NULL), myUnregisterTouchWindow(NULL), myGetTouchInputInfo(NULL),
          myCloseTouchInputHandle(NULL), myIsRegistered(false) {
    ...
    public:
        ...
        TouchInputInfo(const TouchInputHelper& theHelper, const MSG& theMsg)
            : InnerTouchArray(0), myHelper(&theHelper), myHInput((HTOUCHINPUT)theMsg.lParam) {
...
// purpose  :
// =======================================================================
WNT_Window::WNT_Window(const Standard_CString theTitle, const Handle(WNT_WClass) & theClass, const WNT_Dword& theStyle,
                       ...
                       const Quantity_NameOfColor theBackColor, const Aspect_Handle theParent,
                       const Aspect_Handle theMenu, const Standard_Address theClientStruct)
    : Aspect_Window(), myWClass(theClass), myHWindow(NULL), myHParentWindow(NULL), myXLeft(thePxLeft), myYTop(thePxTop),
      myXRight(thePxLeft + thePxWidth), myYBottom(thePxTop + thePxHeight), myIsForeign(Standard_False) {
    ...
    myHWindow = CreateWindowW(aClassNameW.ToWideString(), aTitleW.ToWideString(), aStyle, myXLeft, myYTop,
                              (myXRight - myXLeft), (myYBottom - myYTop), (HWND)theParent, (HMENU)theMenu,
                              (HINSTANCE)myWClass->Instance(), theClientStruct);
    if (!myHWindow) {
...
WNT_Window::WNT_Window(const Aspect_Handle theHandle, const Quantity_NameOfColor theBackColor)
    : myHWindow(theHandle), myHParentWindow(GetParent((HWND)theHandle)), myXLeft(0), myYTop(0), myXRight(0),
      myYBottom(0), myIsForeign(Standard_True) {
    SetBackground(theBackColor);
...
// purpose  :
// =======================================================================
void WNT_Window::Position(Standard_Integer& theX1, Standard_Integer& theY1, Standard_Integer& theX2,
...
// purpose  :
// =======================================================================
void WNT_Window::SetPos(const Standard_Integer theX, const Standard_Integer theY, const Standard_Integer theX1,
...
bool WNT_Window::ProcessMessage(Aspect_WindowInputListener& theListener, MSG& theMsg) {
    ...
    switch (theMsg.message) {
        ...
        case WM_INPUT: {
            ...
            if (aSize == 0 || ::GetRawInputData((HRAWINPUT)theMsg.lParam, RID_INPUT, aRawData, &aSize,
                                                sizeof(RAWINPUTHEADER)) != aSize) {
                return true;
            }
            ...
            if (::GetRawInputDeviceInfoW(aRawInput->header.hDevice, RIDI_DEVICEINFO, &aDevInfo, &aDevInfoSize) !=
        ...
        case WM_TOUCH: {
            ...
            for (size_t aTouchIter = 0; aTouchIter < aSrcTouches.Size(); ++aTouchIter) {
                ...
                const Graphic3d_Vec2d aNewPos2d =
                    Graphic3d_Vec2d(double(aTouchSrc.x), double(aTouchSrc.y)) * 0.01 - Graphic3d_Vec2d(aWinTopLeft);
