...
public:
    ...
    Standard_EXPORT void Init(const Handle(TCollection_HAsciiString) & aName,
                              const Handle(StepShape_EdgeLoop) & aPathElement, const Standard_Boolean aOrientation);
    ...
    Standard_EXPORT virtual void SetEdgeList(const Handle(StepShape_HArray1OfOrientedEdge) &
                                             aEdgeList) Standard_OVERRIDE;

    Standard_EXPORT virtual Handle(StepShape_HArray1OfOrientedEdge) EdgeList() const Standard_OVERRIDE;

    Standard_EXPORT virtual Handle(StepShape_OrientedEdge)
        EdgeListValue(const Standard_Integer num) const Standard_OVERRIDE;
