...
XmlMNaming_NamedShapeDriver::XmlMNaming_NamedShapeDriver(const Handle(Message_Messenger) & theMessageDriver)
    : XmlMDF_ADriver(theMessageDriver, NULL), myShapeSet(Standard_False) // triangles mode
{}
...
Standard_Boolean XmlMNaming_NamedShapeDriver::Paste(const XmlObjMgt_Persistent& theSource,
    ...
    if (aVerString != NULL) aVerString.GetInteger(aVersion);
    ...
    if (NewPShapes.Length() == 0 && OldPShapes.Length() == 0) return Standard_True;
...
void XmlMNaming_NamedShapeDriver::Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
    ...
    if (aVersion != 0) theTarget.Element().setAttribute(::VersionString(), aVersion);
}
...
static const XmlObjMgt_DOMString& EvolutionString(const TNaming_Evolution i) {
    switch (i) {
        ...
        case TNaming_REPLACE:
            return ::EvolModifyString(); //    case TNaming_REPLACE      : return ::EvolReplaceString(); for
                                         //    compatibility
        default:
            throw Standard_DomainError("TNaming_Evolution; enum term unknown");
    }
...
static void doTranslate(const TopoDS_Shape& theShape, XmlMNaming_Shape1& theResult, BRepTools_ShapeSet& theShapeSet) {
...
static int doTranslate(const XmlMNaming_Shape1& thePShape, TopoDS_Shape& theResult, BRepTools_ShapeSet& theShapeSet) {
...
void XmlMNaming_NamedShapeDriver::ReadShapeSection(const XmlObjMgt_Element& theElement,
    ...
    if (anElement != NULL) {
        for (LDOM_Node aNode = anElement.getFirstChild(); aNode != NULL; aNode = anElement.getNextSibling()) {
