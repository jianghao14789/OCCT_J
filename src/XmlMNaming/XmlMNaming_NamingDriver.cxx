            ...
            if (aNg->Label().FindAttribute(TNaming_NamedShape::GetID(), aNS)) {
                ...
                for (; itL.More(); itL.Next()) {
                    ...
                    if (aNS->Evolution() == TNaming_SELECTED) {
                        if (itL.More() && itL.NewShape().ShapeType() != TopAbs_VERTEX && !itL.OldShape().IsNull() &&
                            itL.OldShape().ShapeType() == TopAbs_VERTEX) { // OR-N
...
//=======================================================================
void XmlMNaming_NamingDriver::Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
    ...
    // context label
    TCollection_AsciiString anEntry;
    if (!aNgName.ContextLabel().IsNull()) TDF_Tool::Entry(aNgName.ContextLabel(), anEntry);
