...
Standard_Boolean IGESData_IGESReaderTool::Recognize(const Standard_Integer num, Handle(Interface_Check) & ach,
...
void IGESData_IGESReaderTool::ReadDir(const Handle(IGESData_IGESEntity) & ent,
                                      const Handle(IGESData_IGESReaderData) & IR, const IGESData_DirPart& DP,
                                      Handle(Interface_Check) & ach) const {

    Standard_Integer v[17] = {};
    Standard_Character nom[9] = {};
    Standard_Character snum[9] = {}, theRes1[9] = {}, theRes2[9] = {};
    DP.Values(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13], v[14], v[15],
              v[16], theRes1, theRes2, nom, snum);
...
void IGESData_IGESReaderTool::ReadProps(const Handle(IGESData_IGESEntity) & ent,
    ...
    if (PR.ReadEntList(IR, PR.CurrentList(nbprops), Msg38, props, Standard_False)) ent->LoadProperties(props);
}
...
void IGESData_IGESReaderTool::ReadAssocs(const Handle(IGESData_IGESEntity) & ent,
    ...
    if (PR.ReadEntList(IR, PR.CurrentList(nbassocs), Msg37, assocs, Standard_False)) ent->LoadAssociativities(assocs);
}
