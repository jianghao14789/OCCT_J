...
void IGESData_IGESDumper::PrintDNum(const Handle(IGESData_IGESEntity) & ent, Standard_OStream& S) const {
...
void IGESData_IGESDumper::PrintShort(const Handle(IGESData_IGESEntity) & ent, Standard_OStream& S) const {
    //  PrintDNum(ent,S);
    if (!ent.IsNull()) {
        Standard_Integer num = 0;
...
void IGESData_IGESDumper::Dump(const Handle(IGESData_IGESEntity) & ent, Standard_OStream& S, const Standard_Integer own,
    ...
    if (own >= 2)
        S << "**      Status Number :   Blank:" << ent->BlankStatus() << "    Subordinate:" << ent->SubordinateStatus()
          << "    UseFlag:" << ent->UseFlag() << "    Hierarchy:" << ent->HierarchyStatus() << "\n";
    if (own >= 1) {
...
void IGESData_IGESDumper::OwnDump(const Handle(IGESData_IGESEntity) & ent, Standard_OStream& S,
    ...
    if (thelib.Select(ent, module, CN))
        ...
        S << "  ****  Dump Impossible, n0:id:" << themodel->Number(ent) << ":D" << themodel->DNum(ent) << " Type "
