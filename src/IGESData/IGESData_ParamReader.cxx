...
IGESData_ParamReader::IGESData_ParamReader(const Handle(Interface_ParamList) & list,
                                           const Handle(Interface_Check) & ach, const Standard_Integer base,
                                           const Standard_Integer nbpar, const Standard_Integer /*num*/) {
    Clear();
    theparams = list;
    thecheck = ach;
...
void IGESData_ParamReader::NextStage() {
    if (thestage != IGESData_ReadEnd) thestage = (IGESData_ReadStage)(((long)thestage) + 1);
}
...
Standard_Boolean IGESData_ParamReader::PrepareRead(const IGESData_ParamCursor& PC, const Standard_Boolean several,
...
Standard_Boolean IGESData_ParamReader::PrepareRead(const IGESData_ParamCursor& PC, const Standard_CString mess,
...
Standard_Boolean IGESData_ParamReader::ReadInteger(const IGESData_ParamCursor& PC, Standard_Integer& val) {
...
Standard_Boolean IGESData_ParamReader::ReadInteger(const IGESData_ParamCursor& PC, const Standard_CString mess,
...
Standard_Boolean IGESData_ParamReader::ReadBoolean(const IGESData_ParamCursor& PC, const Message_Msg& amsg,
...
Standard_Boolean IGESData_ParamReader::ReadBoolean(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                   Standard_Boolean& val, const Standard_Boolean exact) {
    if (!PrepareRead(PC, mess, Standard_False)) return Standard_False;
    const Interface_FileParameter& FP = theparams->Value(theindex + thebase);
    if (FP.ParamType() != Interface_ParamInteger) {
...
Standard_Boolean IGESData_ParamReader::ReadReal(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                Standard_Real& val) {
    if (!PrepareRead(PC, mess, Standard_False)) return Standard_False;
...
Standard_Boolean IGESData_ParamReader::ReadXY(const IGESData_ParamCursor& PC, Message_Msg& /*amsg*/, gp_XY& val) {
...
Standard_Boolean IGESData_ParamReader::ReadXY(const IGESData_ParamCursor& PC, const Standard_CString mess, gp_XY& val) {
...
Standard_Boolean IGESData_ParamReader::ReadXYZ(const IGESData_ParamCursor& PC, Message_Msg& /*amsg*/, gp_XYZ& val) {
    if (!PrepareRead(PC, Standard_False, 3)) return Standard_False;
    Standard_Real X, Y = 0., Z = 0.;
    Standard_Boolean stat = (ReadingReal(theindex, X) && ReadingReal(theindex + 1, Y) && ReadingReal(theindex + 2, Z));
...
Standard_Boolean IGESData_ParamReader::ReadXYZ(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                               gp_XYZ& val) {
    if (!PrepareRead(PC, mess, Standard_False, 3)) return Standard_False;
    Standard_Real X, Y = 0., Z = 0.;
    Standard_Boolean stat =
        (ReadingReal(theindex, mess, X) && ReadingReal(theindex + 1, mess, Y) && ReadingReal(theindex + 2, mess, Z));
    if (stat) val.SetCoord(X, Y, Z);
    return Standard_True;
}
...
Standard_Boolean IGESData_ParamReader::ReadText(const IGESData_ParamCursor& PC, const Message_Msg& amsg,
...
Standard_Boolean IGESData_ParamReader::ReadText(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                Handle(TCollection_HAsciiString) & val) {
    if (!PrepareRead(PC, mess, Standard_False)) return Standard_False;
    const Interface_FileParameter& FP = theparams->Value(theindex + thebase);
    if (FP.ParamType() != Interface_ParamText) {
...
Standard_Boolean IGESData_ParamReader::ReadEntity(const Handle(IGESData_IGESReaderData) & IR,
                                                  const IGESData_ParamCursor& PC, IGESData_Status& aStatus,
...
Standard_Boolean IGESData_ParamReader::ReadEntity(const Handle(IGESData_IGESReaderData) & IR,
                                                  const IGESData_ParamCursor& PC, IGESData_Status& aStatus,
                                                  const Handle(Standard_Type) & type, Handle(IGESData_IGESEntity) & val,
...
Standard_Boolean IGESData_ParamReader::ReadInts(const IGESData_ParamCursor& PC, const Message_Msg& amsg,
...
Standard_Boolean IGESData_ParamReader::ReadInts(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                Handle(TColStd_HArray1OfInteger) & val, const Standard_Integer index) {
...
Standard_Boolean IGESData_ParamReader::ReadReals(const IGESData_ParamCursor& PC, Message_Msg& /*amsg*/,
...
Standard_Boolean IGESData_ParamReader::ReadReals(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                 Handle(TColStd_HArray1OfReal) & val, const Standard_Integer index) {
    if (!PrepareRead(PC, mess, Standard_True)) return Standard_False;
    if (thenbitem == 0) return Standard_True; // vide : retour Null ...
    val = new TColStd_HArray1OfReal(index, index + thenbitem * thetermsz - 1);
...
Standard_Boolean IGESData_ParamReader::ReadTexts(const IGESData_ParamCursor& PC, const Message_Msg& amsg,
...
Standard_Boolean IGESData_ParamReader::ReadTexts(const IGESData_ParamCursor& PC, const Standard_CString mess,
                                                 Handle(Interface_HArray1OfHAsciiString) & val,
                                                 const Standard_Integer index) {
    if (!PrepareRead(PC, mess, Standard_True)) return Standard_False;
    if (thenbitem == 0) return Standard_True; // vide : retour Null ...
...
Standard_Boolean IGESData_ParamReader::ReadingReal(const Standard_Integer num, const Standard_CString mess,
                                                   Standard_Real& val) {
    const Interface_FileParameter& FP = theparams->Value(num + thebase);
    if (FP.ParamType() == Interface_ParamInteger) {
        if (!pbrealint) {
...
Standard_Boolean IGESData_ParamReader::ReadingEntityNumber(const Standard_Integer num, Standard_Integer& val) {
    const Interface_FileParameter& FP = theparams->Value(num + thebase);
    val = ParamNumber(num);
    if (val == 0) {
...
Standard_Boolean IGESData_ParamReader::ReadingEntityNumber(const Standard_Integer num, const Standard_CString mess,
                                                           Standard_Integer& val) {
    const Interface_FileParameter& FP = theparams->Value(num + thebase);
    val = ParamNumber(num);
    if (val == 0) {
...
void IGESData_ParamReader::AddFail(const Standard_CString idm, const Handle(TCollection_HAsciiString) & afail,
...
void IGESData_ParamReader::AddFail(const Standard_CString idm, const Standard_CString afail,
...
void IGESData_ParamReader::AddWarning(const Standard_CString idm, const Handle(TCollection_HAsciiString) & aw,
...
void IGESData_ParamReader::AddWarning(const Standard_CString idm, const Standard_CString awarn,
