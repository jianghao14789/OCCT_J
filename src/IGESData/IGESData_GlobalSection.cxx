...
static void MakeHollerith(const Handle(TCollection_HAsciiString) & astr, char* text, Standard_Integer& lt) {
    lt = 0;
    text[0] = '\0';
...
IGESData_GlobalSection::IGESData_GlobalSection()
    : theSeparator(','), theEndMark(';'), theIntegerBits(32), // simple = entier = 32b, double = 64
      theMaxPower10Single(38), theMaxDigitsSingle(6), theMaxPower10Double(308), theMaxDigitsDouble(15), theScale(1.0),
      theCascadeUnit(1.0), theUnitFlag(0), theLineWeightGrad(1), theMaxLineWeight(0.0), theResolution(0.0),
      theMaxCoord(0.0), hasMaxCoord(Standard_False), theIGESVersion(11), // IGES 5.3 by default
...
Handle(TCollection_HAsciiString)
    IGESData_GlobalSection::TranslatedFromHollerith(const Handle(TCollection_HAsciiString) & astr) const {
...
void IGESData_GlobalSection::Init(const Handle(Interface_ParamSet) & params, Handle(Interface_Check) & ach) {
    ...
    for (Standard_Integer i = 1; i <= nbp; i++) {
        ...
        if (fpt == Interface_ParamInteger) {
            ...
            if (i == 13 || i == 17 || i == 19 || i == 20) realval = Atof(val);
            intval = atoi(val);
        }
        ...
        // if the param is a text
            ...
            if (val[0] != '\0') {
                ...
                if (nhol > 1 && nhol < lhol) strval = strval->SubString(nhol + 1, lhol);
            }
        }
        ...
        switch (i) {
            case 1:
                if (sepend != '\0') theSeparator = sepend;
                break;
            case 2:
                if (sepend != '\0') theEndMark = sepend;
                break;
            case 3:
                theSendName = strval;
                break;
...
Handle(Interface_ParamSet) IGESData_GlobalSection::Params() const {
    ...
    if (hasMaxCoord) Interface_FloatWriter::Convert(theMaxCoord, nombre, Standard_True, 0., 0., "%f", "%f");
...
Handle(TCollection_HAsciiString) IGESData_GlobalSection::NewDateString(
    const Standard_Integer annee, const Standard_Integer mois, const Standard_Integer jour,
    const Standard_Integer heure, const Standard_Integer minute, const Standard_Integer seconde,
...
Handle(TCollection_HAsciiString) IGESData_GlobalSection::NewDateString(const Handle(TCollection_HAsciiString) & date,
                                                                       const Standard_Integer mode) {
...
void IGESData_GlobalSection::MaxMaxCoord(const Standard_Real val) {
    ...
    if (hasMaxCoord) {
        if (aval > theMaxCoord) theMaxCoord = aval;
    } else
        SetMaxCoord(aval);
}
