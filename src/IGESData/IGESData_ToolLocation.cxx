...
IGESData_ToolLocation::IGESData_ToolLocation(const Handle(IGESData_IGESModel) & amodel,
    ...
    : thelib(protocol), therefs(0, amodel->NbEntities()), theassocs(0, amodel->NbEntities()) {
    theprec = 1.e-05;
    themodel = amodel;
    therefs.Init(0);
    theassocs.Init(0);
...
Handle(IGESData_IGESEntity) IGESData_ToolLocation::Parent(const Handle(IGESData_IGESEntity) & ent) const {
...
Standard_Boolean IGESData_ToolLocation::ConvertLocation(const Standard_Real prec, const gp_GTrsf& loc, gp_Trsf& result,
    ...
    if (Abs(m1 - mm) > prec * mm || Abs(m2 - mm) > prec * mm || Abs(m3 - mm) > prec * mm) return Standard_False;
    v1.Divide(m1);
    v2.Divide(m2);
    v3.Divide(m3);
    if (Abs(v1.Dot(v2)) > prec || Abs(v2.Dot(v3)) > prec || Abs(v3.Dot(v1)) > prec) return Standard_False;
    ...
    if (v1.X() != 1. || v1.Y() != 0. || v1.Z() != 0. || v2.X() != 0. || v2.Y() != 1. || v2.Z() != 0. || v3.X() != 0. ||
        v3.Y() != 0. || v3.Z() != 1.) {
