...
void IGESData_VerifyDate(const Handle(TCollection_HAsciiString) & str, Handle(Interface_Check) & ach,
...
void IGESData_IGESModel::DumpHeader(Standard_OStream& S, const Standard_Integer) const {
    ...
    S << std::endl;
    S << "[ 7]  Integer Bits          : " << theheader.IntegerBits() << "          Features for Reals : " << std::endl;
    ...
    S << "[23]  IGES Version Number   : " << num << "   -> Name : " << IGESData_BasicEditor::IGESVersionName(num);
...
Standard_CString IGESData_IGESModel::StartLine(const Standard_Integer num) const {
    if (num > 0 && num <= thestart->Length()) return thestart->Value(num)->ToCString();
    return voidline;
}
...
void IGESData_IGESModel::VerifyCheck(Handle(Interface_Check) & ach) const {
    ...
    for (int i = 0; i <= 1; i++) {
        if (del[i] <= 32 || del[i] == 43 || del[i] == 45 || del[i] == 46 || (del[i] >= 48 && del[i] <= 57) ||
            del[i] == 68 || del[i] == 69 || del[i] == 72 || del[i] >= 127) {
    ...
    if (theheader.IGESVersion() < 1 || theheader.IGESVersion() > IGESData_BasicEditor::IGESVersionMax()) {
        Message_Msg Msg53("XSTEP_53");
        ach->SendWarning(Msg53);
    }

    // Sending of message : Drafting Standard Flag parameter is incorrect.
    if (theheader.DraftingStandard() < 0 || theheader.DraftingStandard() > IGESData_BasicEditor::DraftingMax()) {
        Message_Msg Msg54("XSTEP_54");
...
void IGESData_IGESModel::PrintLabel(const Handle(Standard_Transient) & ent, Standard_OStream& S) const {
...
void IGESData_IGESModel::PrintToLog(const Handle(Standard_Transient) & ent, Standard_OStream& S) const {
    DeclareAndCast(IGESData_IGESEntity, igesent, ent);
    if (!igesent.IsNull()) {
        Standard_Integer num = Number(ent);
        if (num == 0)
