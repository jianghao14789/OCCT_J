...
IGESData_IGESWriter::IGESData_IGESWriter(const Handle(IGESData_IGESModel) & amodel)
    : thedirs(0, amodel->NbEntities()), thepnum(1, amodel->NbEntities() + 1), thecurr(MaxcarsG + 1), themodew(0),
      thefloatw(9) {
    themodel = amodel;
...
IGESData_IGESWriter::IGESData_IGESWriter() : thedirs(0, 0), thepnum(1, 1), thecurr(MaxcarsG + 1), thefloatw(9) {}
...
void IGESData_IGESWriter::SendModel(const Handle(IGESData_Protocol) & protocol) {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (lib.Select(cnt, module, CN))
            ...
            sout << " -- IGESWriter : Not Processed for n0." << i << " in file,  Type " << cnt->TypeNumber()
                 << "  Form " << cnt->FormNumber() << std::endl;
...
void IGESData_IGESWriter::DirPart(const Handle(IGESData_IGESEntity) & anent) {
    if (thesect != 3 && thestep != IGESData_ReadEnd) throw Interface_InterfaceError("IGESWriter : DirPart");
    ...
    DP.Init(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13], v[14], v[15], v[16],
            res1, res2, label, snum);
...
void IGESData_IGESWriter::OwnParams(const Handle(IGESData_IGESEntity) & anent) {
    char text[20];
    if (thesect != 3 && thestep != IGESData_ReadDir) throw Interface_InterfaceError("IGESWriter : OwnParams");
...
void IGESData_IGESWriter::Properties(const Handle(IGESData_IGESEntity) & anent) {
    if (thesect != 3 && thestep != IGESData_ReadOwn) throw Interface_InterfaceError("IGESWriter : Properties");
...
void IGESData_IGESWriter::Associativities(const Handle(IGESData_IGESEntity) & anent) {
    if (thesect != 3 && thestep != IGESData_ReadOwn) throw Interface_InterfaceError("IGESWriter : Associativities");
...
void IGESData_IGESWriter::EndEntity() {
    if (thesect != 3 && thestep != IGESData_ReadOwn) throw Interface_InterfaceError("IGESWriter : EndEntity");
    AddChar(theendm);
...
void IGESData_IGESWriter::AddString(const Handle(TCollection_HAsciiString) & val, const Standard_Integer more) {
    if (val.IsNull()) return;
    AddString(val->ToCString(), val->Length(), more);
}

void IGESData_IGESWriter::AddString(const Standard_CString val, const Standard_Integer lnval,
...
void IGESData_IGESWriter::Send(const Handle(IGESData_IGESEntity) & val, const Standard_Boolean negative) {
...
Handle(TColStd_HSequenceOfHAsciiString) IGESData_IGESWriter::SectionStrings(const Standard_Integer num) const {
...
Standard_Boolean IGESData_IGESWriter::Print(Standard_OStream& S) const {
    ...
    for (i = 1; i <= nbd && isGood; i++) {
        ...
        thedirs.Value(i).Values(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13],
                                v[14], v[15], v[16], res1, res2, lab, num);
        v[1] = thepnum.Value(i);                         // debut en P
        v[15] = thepnum.Value(i + 1) - thepnum.Value(i); // nb de lignes en P
        sprintf(ligne, "%8d%8d%8d%8d%8d%8d%8d%8d%2.2d%2.2d%2.2d%2.2dD%7.7d", v[0], v[1], v[2], v[3], v[4], v[5], v[6],
                v[7], v[8], v[9], v[10], v[11], 2 * i - 1);
        if (fnes)
            writefnes(S, ligne);
        else
            S << ligne;
        S << "\n";
        sprintf(ligne, "%8d%8d%8d%8d%8d%8s%8s%8s%8sD%7.7d", v[0], v[13], v[14], v[15], v[16], res1, res2, lab, num,
                2 * i);
        if (fnes)
            writefnes(S, ligne);
        else
...
//  Parameter Section
#ifdef PATIENCELOG
    std::cout << " Parameter Section : " << thepnum.Value(nbd) - 1 << " lines (* = 1000 lines) " << std::flush;
#endif
    ...
    sprintf(ligne, "S%7dG%7dD%7dP%7d                                        T0000001", nbs, nbg, nbd * 2,
