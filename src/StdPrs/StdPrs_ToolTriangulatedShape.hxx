...
public:
    ...
    Standard_EXPORT static void ComputeNormals(const TopoDS_Face& theFace, const Handle(Poly_Triangulation) & theTris,
    ...
    Standard_EXPORT static void Normal(const TopoDS_Face& theFace, Poly_Connect& thePolyConnect,
    ...
    //! </ul>
    //! In case of the type of deflection in theDrawer computed relative deflection for shape is stored as absolute
    //! deflection. It is necessary to use it later on for sub-shapes. This function should always be used to compute
    //! the deflection value for building discrete representations of the shape (triangualtion, wireframe) to avoid
    //! incosistencies between different representations of the shape and undesirable visual artifacts.
