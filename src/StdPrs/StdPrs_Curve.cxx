...
// purpose:
//==================================================================
static void DrawCurve(const Adaptor3d_Curve& aCurve, const Handle(Graphic3d_Group) aGroup, const Standard_Integer NbP,
                      const Standard_Real U1, const Standard_Real U2, TColgp_SequenceOfPnt& Points,
    ...
    switch (aCurve.GetType()) {
        ...
        default: {
            ...
            if (drawCurve) aPrims = new Graphic3d_ArrayOfPolylines(N);

            for (Standard_Integer i = 1; i <= N; i++) {
                p = aCurve.Value(U1 + (i - 1) * DU);
                Points.Append(p);
                if (drawCurve) aPrims->AddVertex(p);
            }
            if (drawCurve) aGroup->AddPrimitiveArray(aPrims);
        }
    }
}
...
static Standard_Boolean MatchCurve(const Standard_Real X, const Standard_Real Y, const Standard_Real Z,
                                   ...
                                   const Standard_Real TheDeflection, const Standard_Integer NbP,
...
// purpose:
//==================================================================
void StdPrs_Curve::Add(const Handle(Prs3d_Presentation) & aPresentation, const Adaptor3d_Curve& aCurve,
                       const Handle(Prs3d_Drawer) & aDrawer, const Standard_Boolean drawCurve) {
    ...
    if (aDrawer->LineArrowDraw()) {
        ...
        Prs3d_Arrow::Draw(aPresentation->CurrentGroup(), Location, gp_Dir(Direction), aDrawer->ArrowAspect()->Angle(),
...
//==================================================================
void StdPrs_Curve::Add(const Handle(Prs3d_Presentation) & aPresentation, const Adaptor3d_Curve& aCurve,
                       const Handle(Prs3d_Drawer) & aDrawer, TColgp_SequenceOfPnt& Points,
                       const Standard_Boolean drawCurve) {
    Standard_Real V1, V2;
    FindLimits(aCurve, aDrawer->MaximalParameterValue(), V1, V2);
...
void StdPrs_Curve::Add(const Handle(Prs3d_Presentation) & aPresentation, const Adaptor3d_Curve& aCurve,
                       const Standard_Real U1, const Standard_Real U2, TColgp_SequenceOfPnt& Points,
                       const Standard_Integer NbPoints, const Standard_Boolean drawCurve) {
...
// purpose:
//==================================================================
Standard_Boolean StdPrs_Curve::Match(const Standard_Real X, const Standard_Real Y, const Standard_Real Z,
                                     const Standard_Real aDistance, const Adaptor3d_Curve& aCurve,
    ...
    const Standard_Integer NbPoints = aDrawer->Discretisation();
    return MatchCurve(X, Y, Z, aDistance, aCurve, aDrawer->MaximalChordialDeviation(), NbPoints, V1, V2);
}
...
Standard_Boolean StdPrs_Curve::Match(const Standard_Real X, const Standard_Real Y, const Standard_Real Z,
    ...
    return MatchCurve(X, Y, Z, aDistance, aCurve, aDrawer->MaximalChordialDeviation(), aDrawer->Discretisation(), V1,
                      V2);
}
