...
void StdPrs_WFRestrictedFace::Add(const Handle(Prs3d_Presentation) & thePresentation,
                                  const Handle(BRepAdaptor_Surface) & theFace, const Standard_Boolean theDrawUIso,
                                  const Standard_Boolean theDrawVIso, const Standard_Integer theNbUIso,
                                  const Standard_Integer theNBVIso, const Handle(Prs3d_Drawer) & theDrawer,
    ...
    else { // No pcurves -- take natural bounds
    ...
    for (anI = 1; anI <= aNbLines; ++anI) {
        ...
        for (Standard_Integer aJ = 1; aJ <= NumberOfIntervals; aJ++) {
            ...
            if (b1 == RealFirst() || b2 == RealLast()) continue;
            ...
            if (!aGeomBSurface.IsNull()) {
                ...
                // Note that the isos are the part of the shape, it will be displayed after a computation the whole
                // shape NbPoints = 30 - default parameter for computation of such curves
                StdPrs_Curve::Add(thePresentation, GeomAdaptor_Curve(aBCurve), b1, b2, aPoints->ChangeSequence(), 30,
                                  Standard_False);
...
Standard_Boolean StdPrs_WFRestrictedFace::Match(const Standard_Real theX, const Standard_Real theY,
                                                const Standard_Real theZ, const Standard_Real theDistance,
                                                const Handle(BRepAdaptor_Surface) & theFace,
                                                const Standard_Boolean theDrawUIso, const Standard_Boolean theDrawVIso,
                                                const Standard_Real theDeflection, const Standard_Integer theNbUIso,
    ...
    for (anI = 1; anI <= aNbLines; anI++) {
        ...
        for (Standard_Integer j = 1; j <= aNbIntervals; j++) {
            ...
            if (StdPrs_Curve::Match(theX, theY, theZ, theDistance, anIso, theDeflection, aLimit, aNbPoints))
...
void StdPrs_WFRestrictedFace::Add(const Handle(Prs3d_Presentation) & thePresentation,
                                  const Handle(BRepAdaptor_Surface) & theFace, const Handle(Prs3d_Drawer) & theDrawer) {
    Prs3d_NListOfSequenceOfPnt aCurves;
    StdPrs_WFRestrictedFace::Add(thePresentation, theFace, Standard_True, Standard_True,
                                 theDrawer->UIsoAspect()->Number(), theDrawer->VIsoAspect()->Number(), theDrawer,
                                 aCurves);
}
...
void StdPrs_WFRestrictedFace::AddUIso(const Handle(Prs3d_Presentation) & thePresentation,
                                      ...
                                      const Handle(Prs3d_Drawer) & theDrawer) {
    Prs3d_NListOfSequenceOfPnt aCurves;
    StdPrs_WFRestrictedFace::Add(thePresentation, theFace, Standard_True, Standard_False,
                                 theDrawer->UIsoAspect()->Number(), theDrawer->VIsoAspect()->Number(), theDrawer,
                                 aCurves);
}
...
void StdPrs_WFRestrictedFace::AddVIso(const Handle(Prs3d_Presentation) & thePresentation,
                                      ...
                                      const Handle(Prs3d_Drawer) & theDrawer) {
    Prs3d_NListOfSequenceOfPnt aCurves;
    StdPrs_WFRestrictedFace::Add(thePresentation, theFace, Standard_False, Standard_True,
                                 theDrawer->UIsoAspect()->Number(), theDrawer->VIsoAspect()->Number(), theDrawer,
                                 aCurves);
}
...
Standard_Boolean StdPrs_WFRestrictedFace::Match(const Standard_Real theX, const Standard_Real theY,
                                                ...
                                                const Handle(BRepAdaptor_Surface) & theFace,
                                                const Handle(Prs3d_Drawer) & theDrawer) {
    return StdPrs_WFRestrictedFace::Match(theX, theY, theZ, theDistance, theFace, Standard_True, Standard_True,
                                          theDrawer->MaximalChordialDeviation(), theDrawer->UIsoAspect()->Number(),
                                          theDrawer->VIsoAspect()->Number(), theDrawer);
}
...
Standard_Boolean StdPrs_WFRestrictedFace::MatchUIso(const Standard_Real theX, const Standard_Real theY,
                                                    const Standard_Real theZ, const Standard_Real theDistance,
                                                    const Handle(BRepAdaptor_Surface) & theFace,
                                                    const Handle(Prs3d_Drawer) & theDrawer) {
    return StdPrs_WFRestrictedFace::Match(theX, theY, theZ, theDistance, theFace, Standard_True, Standard_False,
                                          theDrawer->MaximalChordialDeviation(), theDrawer->UIsoAspect()->Number(),
                                          theDrawer->VIsoAspect()->Number(), theDrawer);
}
...
Standard_Boolean StdPrs_WFRestrictedFace::MatchVIso(const Standard_Real theX, const Standard_Real theY,
                                                    const Standard_Real theZ, const Standard_Real theDistance,
                                                    const Handle(BRepAdaptor_Surface) & theFace,
                                                    const Handle(Prs3d_Drawer) & theDrawer) {
    return StdPrs_WFRestrictedFace::Match(theX, theY, theZ, theDistance, theFace, Standard_False, Standard_True,
                                          theDrawer->MaximalChordialDeviation(), theDrawer->UIsoAspect()->Number(),
                                          theDrawer->VIsoAspect()->Number(), theDrawer);
}
