...
static void sortSegments(const SeqOfVecOfSegments& theSegments, Prs3d_NListOfSequenceOfPnt& thePolylines) {
...
static void findLimits(const Adaptor3d_Curve& theCurve, const Standard_Real theLimit, Standard_Real& theFirst,
...
// purpose  :
//==================================================================
void StdPrs_Isolines::AddOnTriangulation(const Handle(Prs3d_Presentation) & thePresentation, const TopoDS_Face& theFace,
...
// purpose  :
//==================================================================
void StdPrs_Isolines::AddOnTriangulation(const TopoDS_Face& theFace, const Handle(Prs3d_Drawer) & theDrawer,
    ...
    UVIsoParameters(theFace, aNbIsoU, aNbIsoV, theDrawer->MaximalParameterValue(), aUIsoParams, aVIsoParams, aUmin,
                    aUmax, aVmin, aVmax);
    ...
    if (!aLocTriangulation.IsEqual(aLocSurface)) {
        aSurface =
            Handle(Geom_Surface)::DownCast(aSurface->Transformed((aLocSurface / aLocTriangulation).Transformation()));
    }
    ...
    addOnTriangulation(aTriangulation, aSurface, aLocTriangulation, aUIsoParams, aVIsoParams, theUPolylines,
                       theVPolylines);
}
...
void StdPrs_Isolines::AddOnTriangulation(const Handle(Prs3d_Presentation) & thePresentation,
                                         ...
                                         const Handle(Geom_Surface) & theSurface, const TopLoc_Location& theLocation,
                                         const Handle(Prs3d_Drawer) & theDrawer,
                                         const TColStd_SequenceOfReal& theUIsoParams,
                                         const TColStd_SequenceOfReal& theVIsoParams) {
    Prs3d_NListOfSequenceOfPnt aUPolylines, aVPolylines;
    addOnTriangulation(theTriangulation, theSurface, theLocation, theUIsoParams, theVIsoParams, aUPolylines,
                       aVPolylines);
    Prs3d::AddPrimitivesGroup(thePresentation, theDrawer->UIsoAspect(), aUPolylines);
    Prs3d::AddPrimitivesGroup(thePresentation, theDrawer->VIsoAspect(), aVPolylines);
}
    ...
    for (Standard_Integer anUVIter = 0; anUVIter < 2; ++anUVIter) {
        ...
        for (Standard_Integer anIsoIdx = 1; anIsoIdx <= aNbIsolines; ++anIsoIdx) {
            ...
            for (Standard_Integer aTriIter = 1; aTriIter <= theTriangulation->NbTriangles(); ++aTriIter) {
                ...
                const gp_Pnt aNodesXYZ[3] = {theTriangulation->Node(aNodeIdxs[0]), theTriangulation->Node(aNodeIdxs[1]),
...
// purpose  :
//==================================================================
void StdPrs_Isolines::AddOnSurface(const Handle(Prs3d_Presentation) & thePresentation, const TopoDS_Face& theFace,
...
// purpose  :
//==================================================================
void StdPrs_Isolines::AddOnSurface(const TopoDS_Face& theFace, const Handle(Prs3d_Drawer) & theDrawer,
                                   const Standard_Real theDeflection, Prs3d_NListOfSequenceOfPnt& theUPolylines,
                                   Prs3d_NListOfSequenceOfPnt& theVPolylines) {
    const Standard_Integer aNbIsoU = theDrawer->UIsoAspect()->Number();
    const Standard_Integer aNbIsoV = theDrawer->VIsoAspect()->Number();
    if (aNbIsoU < 1 && aNbIsoV < 1) {
    ...
    addOnSurface(new BRepAdaptor_Surface(aSurface), theDrawer, theDeflection, aUIsoParams, aVIsoParams, theUPolylines,
                 theVPolylines);
}
        ...
        for (Standard_Integer anI = 1; anI <= aHatcher.NbLines(); anI++) {
            ...
            for (Standard_Integer aJ = 1; aJ <= aHatcher.NbIntervals(anI); aJ++) {
                ...
                Adaptor3d_Curve* aCurve =
                    aBSurface.IsNull() ? (Adaptor3d_Curve*)&aCanonicalCurve : (Adaptor3d_Curve*)&aBSurfaceCurve;

                Handle(TColgp_HSequenceOfPnt) aPoints = new TColgp_HSequenceOfPnt();
                StdPrs_DeflectionCurve::Add(Handle(Prs3d_Presentation)(), *aCurve, aSegmentP1, aSegmentP2,
                                            theDeflection, aPoints->ChangeSequence(), theDrawer->DeviationAngle(),
                                            Standard_False);
                if (aPoints->IsEmpty()) {
                    continue;
                }
...
// purpose  :
//==================================================================
void StdPrs_Isolines::UVIsoParameters(const TopoDS_Face& theFace, const Standard_Integer theNbIsoU,
                                      const Standard_Integer theNbIsoV, const Standard_Real theUVLimit,
                                      TColStd_SequenceOfReal& theUIsoParams, TColStd_SequenceOfReal& theVIsoParams,
    ...
    if (Precision::IsInfinite(aUmax)) aUmax = theUVLimit;
    if (Precision::IsInfinite(aVmin)) aVmin = -theUVLimit;
    if (Precision::IsInfinite(aVmax)) aVmax = theUVLimit;
...
// purpose  :
//==================================================================
Standard_Boolean StdPrs_Isolines::findSegmentOnTriangulation(const Handle(Geom_Surface) & theSurface, const bool theIsU,
                                                             const gp_Lin2d& theIsoline, const gp_Pnt* theNodesXYZ,
                                                             const gp_Pnt2d* theNodesUV, SegOnIso& theSegment) {
