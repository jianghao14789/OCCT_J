...
static void FindLimits(const Handle(Adaptor3d_Surface) & surf, const Standard_Real aLimit, Standard_Real& UFirst,
                       Standard_Real& ULast, Standard_Real& VFirst, Standard_Real& VLast) {
...
void StdPrs_WFSurface::Add(const Handle(Prs3d_Presentation) & aPresentation, const Handle(Adaptor3d_Surface) & aSurface,
    ...
    if (VIsoCurves.Size() > 0) {
        ...
        for (It.Init(VIsoCurves); It.More(); It.Next())
            nbVertices += It.Value()->Length();
        Handle(Graphic3d_ArrayOfPolylines) VIsoArray = new Graphic3d_ArrayOfPolylines(nbVertices, nbBounds);
        for (It.Init(VIsoCurves); It.More(); It.Next()) {
            const Handle(TColgp_HSequenceOfPnt) & Pnts = It.Value();
    ...
    if (freeCurves.Size() > 0) {
        ...
        for (It.Init(freeCurves); It.More(); It.Next())
            nbVertices += It.Value()->Length();
        Handle(Graphic3d_ArrayOfPolylines) freeArray = new Graphic3d_ArrayOfPolylines(nbVertices, nbBounds);
        for (It.Init(freeCurves); It.More(); It.Next()) {
            const Handle(TColgp_HSequenceOfPnt) & Pnts = It.Value();
