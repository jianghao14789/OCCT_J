...
void StdPrs_WFDeflectionSurface::Add(const Handle(Prs3d_Presentation) & aPresentation,
                                     const Handle(Adaptor3d_Surface) & aSurface, const Handle(Prs3d_Drawer) & aDrawer) {
    ...
    if (TOD == Aspect_TOD_RELATIVE) {
        ...
        if (!(Total.IsOpenXmin() || Total.IsOpenXmax())) m = Min(m, Abs(aXmax - aXmin));
        if (!(Total.IsOpenYmin() || Total.IsOpenYmax())) m = Min(m, Abs(aYmax - aYmin));
        if (!(Total.IsOpenZmin() || Total.IsOpenZmax())) m = Min(m, Abs(aZmax - aZmin));
