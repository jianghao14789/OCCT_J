...
//! Single instance initialize single font for sequential glyphs rendering with implicit caching of already rendered
//! glyphs. Thus position of each glyph in the text is specified by shape location.
//!
...
public:
    ...
    Standard_EXPORT static Handle(StdPrs_BRepFont)
        FindAndCreate(const TCollection_AsciiString& theFontName, const Font_FontAspect theFontAspect,
                      const Standard_Real theSize, const Font_StrictLevel theStrictLevel = Font_StrictLevel_Any);
    ...
    Standard_EXPORT StdPrs_BRepFont(const NCollection_String& theFontPath, const Standard_Real theSize,
    ...
    //! @param theSize        the face size in model units
    //! @param theStrictLevel search strict level for using aliases and fallback
    Standard_EXPORT StdPrs_BRepFont(const NCollection_String& theFontName, const Font_FontAspect theFontAspect,
    ...
    //! @param theFaceId   face id within the file (0 by default)
    //! @return true on success
    Standard_EXPORT bool Init(const NCollection_String& theFontPath, const Standard_Real theSize,
    ...
    //! @param theStrictLevel search strict level for using aliases and fallback
    //! @return true on success
    Standard_EXPORT bool FindAndInit(const TCollection_AsciiString& theFontName, const Font_FontAspect theFontAspect,
                                     const Standard_Real theSize,
                                     const Font_StrictLevel theStrictLevel = Font_StrictLevel_Any);
...
protected:
    ...
    Standard_EXPORT Standard_Boolean renderGlyph(const Standard_Utf32Char theChar, TopoDS_Shape& theShape);
...
private:
    ...
    bool to3d(const Handle(Geom2d_Curve) & theCurve2d, const GeomAbs_Shape theContinuity,
    ...
    Standard_Boolean buildFaces(const NCollection_Sequence<TopoDS_Wire>& theWires, TopoDS_Shape& theRes);
...
protected:                                                         //! @name Protected fields
    ...
    Standard_Boolean
        myIsCompositeCurve; //!< flag to merge C1 curves of each contour into single C0 curve, OFF by default
