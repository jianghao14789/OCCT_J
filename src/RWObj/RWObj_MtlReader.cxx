...
static bool findRelativePath(const TCollection_AsciiString& theAbsolutePath, const TCollection_AsciiString& theNewFoler,
...
// purpose  :
// =======================================================================
bool RWObj_MtlReader::Read(const TCollection_AsciiString& theFolder, const TCollection_AsciiString& theFile) {
    ...
    for (; ::feof(myFile) == 0 && ::fgets(aLine, 255, myFile) != NULL;) {
        ...
        if (::memcmp(aPos, "newmtl", 6) == 0) {
            ...
            if (!RWObj_Tools::ReadName(aPos, aMatName)) {
                Message::SendWarning(TCollection_AsciiString("Empty OBJ material at line ") + myNbLines + " in file " +
                                     myPath);
            }
...
void RWObj_MtlReader::processTexturePath(TCollection_AsciiString& theTexturePath,
    ...
    if (OSD_Path::IsAbsolutePath(theTexturePath.ToCString())) {
        Message::SendWarning(TCollection_AsciiString("OBJ file specifies absolute path to the texture image file which "
...
bool RWObj_MtlReader::validateScalar(const Standard_Real theValue) {
    if (theValue < 0.0 || theValue > 1.0) {
        Message::SendWarning(TCollection_AsciiString("Invalid scalar in OBJ material at line ") + myNbLines +
                             " in file " + myPath);
        return false;
    }
    return true;
...
bool RWObj_MtlReader::validateColor(const Graphic3d_Vec3& theVec) {
    if (theVec.r() < 0.0f || theVec.r() > 1.0f || theVec.g() < 0.0f || theVec.g() > 1.0f || theVec.b() < 0.0f ||
        theVec.b() > 1.0f) {
        Message::SendWarning(TCollection_AsciiString("Invalid color in OBJ material at line ") + myNbLines +
                             " in file " + myPath);
        return false;
    }
    return true;
