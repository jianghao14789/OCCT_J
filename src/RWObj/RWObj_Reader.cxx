...
struct RWObj_ReaderFile {
    ...
    RWObj_ReaderFile(const TCollection_AsciiString& theFile)
        : File(OSD_OpenFile(theFile.ToCString(), "rb")), FileLen(0) {
        if (this->File != NULL) {
...
RWObj_Reader::RWObj_Reader()
    : myMemLimitBytes(Standard_Size(-1)), myMemEstim(0), myNbLines(0), myNbProbeNodes(0), myNbProbeElems(0),
      myNbElemsBig(0), myToAbort(false) {
    //
}
...
// Purpose  :
// ================================================================
Standard_Boolean RWObj_Reader::read(const TCollection_AsciiString& theFile, const Message_ProgressRange& theProgress,
    ...
    for (NCollection_DataMap<TCollection_AsciiString, RWObj_Material>::Iterator aMatIter(myMaterials); aMatIter.More();
         aMatIter.Next()) {
