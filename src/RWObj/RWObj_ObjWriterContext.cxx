...
RWObj_ObjWriterContext::RWObj_ObjWriterContext(const TCollection_AsciiString& theName)
    : NbFaces(0), myFile(OSD_OpenFile(theName.ToCString(), "wb")), myName(theName), myElemPosFirst(1, 1, 1, 1),
      myElemNormFirst(1, 1, 1, 1), myElemUVFirst(1, 1, 1, 1), myHasNormals(false), myHasTexCoords(false) {
    if (myFile == NULL) {
...
// Purpose  :
// ================================================================
bool RWObj_ObjWriterContext::WriteHeader(const Standard_Integer theNbNodes, const Standard_Integer theNbElems,
...
bool RWObj_ObjWriterContext::WriteTriangle(const Graphic3d_Vec3i& theTri) {
    ...
    if (myHasNormals) {
        ...
        if (myHasTexCoords) {
            ...
            return Fprintf(myFile, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", aTriPos[0], aTriUv[0], aTriNorm[0], aTriPos[1],
                           aTriUv[1], aTriNorm[1], aTriPos[2], aTriUv[2], aTriNorm[2]) != 0;
        } else {
            return Fprintf(myFile, "f %d//%d %d//%d %d//%d\n", aTriPos[0], aTriNorm[0], aTriPos[1], aTriNorm[1],
                           aTriPos[2], aTriNorm[2]) != 0;
        }
    }
    if (myHasTexCoords) {
        const Graphic3d_Vec3i aTriUv = theTri + myElemUVFirst.xyz();
        return Fprintf(myFile, "f %d/%d %d/%d %d/%d\n", aTriPos[0], aTriUv[0], aTriPos[1], aTriUv[1], aTriPos[2],
                       aTriUv[2]) != 0;
    } else {
...
bool RWObj_ObjWriterContext::WriteQuad(const Graphic3d_Vec4i& theQuad) {
    ...
    if (myHasNormals) {
        ...
        if (myHasTexCoords) {
            ...
            return Fprintf(myFile, "f %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d\n", aQPos[0], aQTex[0], aQNorm[0], aQPos[1],
                           aQTex[1], aQNorm[1], aQPos[2], aQTex[2], aQNorm[2], aQPos[3], aQTex[3], aQNorm[3]) != 0;
        } else {
            return Fprintf(myFile, "f %d//%d %d//%d %d//%d %d//%d\n", aQPos[0], aQNorm[0], aQPos[1], aQNorm[1],
                           aQPos[2], aQNorm[2], aQPos[3], aQNorm[3]) != 0;
        }
    }
    if (myHasTexCoords) {
        const Graphic3d_Vec4i aQTex = theQuad + myElemUVFirst;
        return Fprintf(myFile, "f %d/%d %d/%d %d/%d %d/%d\n", aQPos[0], aQTex[0], aQPos[1], aQTex[1], aQPos[2],
                       aQTex[2], aQPos[3], aQTex[3]) != 0;
    } else {
