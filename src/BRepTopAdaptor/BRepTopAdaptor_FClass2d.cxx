...
BRepTopAdaptor_FClass2d::BRepTopAdaptor_FClass2d(const TopoDS_Face& aFace, const Standard_Real TolUV)
    : Toluv(TolUV), Face(aFace), U1(0.0), V1(0.0), U2(0.0), V2(0.0) {
    ...
    for (FaceExplorer.Init(Face, TopAbs_WIRE); (FaceExplorer.More() && BadWire == 0); FaceExplorer.Next()) {
        ...
        if (NbEdges) { //-- on compte ++ with a normal explorer and with the Wire Explorer
                       /*
                       #ifdef OCCT_DEBUG
...
TopAbs_State BRepTopAdaptor_FClass2d::Perform(const gp_Pnt2d& _Puv, const Standard_Boolean RecadreOnPeriodic) const {
#if LBRCOMPT
    STAT.NbPerform++;
#endif
...
TopAbs_State BRepTopAdaptor_FClass2d::TestOnRestriction(const gp_Pnt2d& _Puv, const Standard_Real Tol,
