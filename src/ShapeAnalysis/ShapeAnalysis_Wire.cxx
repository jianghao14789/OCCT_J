...
ShapeAnalysis_Wire::ShapeAnalysis_Wire(const TopoDS_Wire& wire, const TopoDS_Face& face,
...
ShapeAnalysis_Wire::ShapeAnalysis_Wire(const Handle(ShapeExtend_WireData) & sbwd, const TopoDS_Face& face,
                                       const Standard_Real precision) {
...
void ShapeAnalysis_Wire::Init(const TopoDS_Wire& wire, const TopoDS_Face& face, const Standard_Real precision) {
...
void ShapeAnalysis_Wire::Init(const Handle(ShapeExtend_WireData) & sbwd, const TopoDS_Face& face,
                              const Standard_Real precision) {
    Load(sbwd);
    SetFace(face);
...
void ShapeAnalysis_Wire::SetFace(const TopoDS_Face& face) {
    myFace = face;
    if (!face.IsNull()) mySurf = new ShapeAnalysis_Surface(BRep_Tool::Surface(myFace));
}
...
void ShapeAnalysis_Wire::SetSurface(const Handle(Geom_Surface) & surface, const TopLoc_Location& location) {
    BRep_Builder B;
    TopoDS_Face face;
...
void ShapeAnalysis_Wire::ClearStatuses() {
    myStatusOrder = myStatusConnected = myStatusEdgeCurves = myStatusDegenerated = myStatusClosed = myStatusLacking =
        myStatusSelfIntersection = myStatusSmall = myStatusGaps3d = myStatusGaps2d = myStatusCurveGaps = myStatusLoop =
            myStatus = 0;
...
Standard_Boolean ShapeAnalysis_Wire::CheckOrder(const Standard_Boolean isClosed, const Standard_Boolean mode3d) {
...
Standard_Boolean ShapeAnalysis_Wire::CheckEdgeCurves() {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (SAE.Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
        if (SAE.Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);

        SAE.CheckVerticesWithPCurve(E, myFace);
        if (SAE.Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
        if (SAE.Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);

        SAE.CheckVerticesWithCurve3d(E);
        if (SAE.Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE3);
        if (SAE.Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL3);

        CheckSeam(i);
        if (LastCheckStatus(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE4);
        if (LastCheckStatus(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL4);

        CheckGap3d(i);
        if (LastCheckStatus(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE5);
        if (LastCheckStatus(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL5);

        CheckGap2d(i);
        if (LastCheckStatus(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE6);
        if (LastCheckStatus(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL6);

        Standard_Real maxdev = 0.0;
        SAE.CheckSameParameter(myWire->Edge(i), maxdev);
        if (SAE.Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE7);
        if (SAE.Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL7);
    }
...
Standard_Boolean ShapeAnalysis_Wire::CheckSelfIntersection() {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (LastCheckStatus(ShapeExtend_DONE)) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
        if (LastCheckStatus(ShapeExtend_FAIL)) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);

        CheckIntersectingEdges(i);
        if (LastCheckStatus(ShapeExtend_DONE)) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
        if (LastCheckStatus(ShapeExtend_FAIL)) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
    }
    ...
    for (Standard_Integer num1 = 1; num1 < nb - 1; num1++) {
        ...
        if (CheckClosed(Precision::Confusion()) && 1 == num1) fin = nb - 1;
    ...
    if (isFail) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL3);
    if (isDone) myStatusSelfIntersection |= ShapeExtend::EncodeStatus(ShapeExtend_DONE3);
...
Standard_Boolean ShapeAnalysis_Wire::CheckClosed(const Standard_Real prec) {
    ...
    if (LastCheckStatus(ShapeExtend_DONE)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (LastCheckStatus(ShapeExtend_FAIL)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);

    CheckDegenerated(1);
    if (LastCheckStatus(ShapeExtend_DONE)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
    if (LastCheckStatus(ShapeExtend_FAIL)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
...
Standard_Boolean ShapeAnalysis_Wire::CheckOrder(ShapeAnalysis_WireOrder& sawo, const Standard_Boolean isClosed,
...
Standard_Boolean ShapeAnalysis_Wire::CheckSmall(const Standard_Integer num, const Standard_Real precsmall) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if (!IsLoaded() || NbEdges() <= 1) return Standard_False;
...
Standard_Boolean ShapeAnalysis_Wire::CheckSeam(const Standard_Integer num, Handle(Geom2d_Curve) & C1,
                                               Handle(Geom2d_Curve) & C2, Standard_Real& cf, Standard_Real& cl) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if (!IsReady()) return Standard_False;
...
Standard_Boolean ShapeAnalysis_Wire::CheckDegenerated(const Standard_Integer num, gp_Pnt2d& p2d1, gp_Pnt2d& p2d2) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if (!IsReady() || NbEdges() < 1) return Standard_False;
    ...
    if (Vp.IsNull() || V0.IsNull() || V1.IsNull() || V2.IsNull()) return Standard_False;
    ...
    if (lack ||
        n1 != n2) { //: i8 abv 18 Sep 98: ProSTEP TR9 r0501-ug.stp #182180: single degedge is a wire at apex of a cone
        Standard_Real a, b;
        Handle(Geom2d_Curve) c2d;
...
Standard_Boolean ShapeAnalysis_Wire::CheckGap3d(const Standard_Integer num) {
    ...
    if (myMin3d > myPrecision) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    return LastCheckStatus(ShapeExtend_DONE);
}
...
static gp_Pnt GetPointOnEdge(const TopoDS_Edge& edge, const Handle(ShapeAnalysis_Surface) & surf,
                             const Geom2dAdaptor_Curve& Crv2d, const Standard_Real param) {
    if (BRep_Tool::SameParameter(edge)) {
        Standard_Real f, l;
        TopLoc_Location L;
        const Handle(Geom_Curve) ConS = BRep_Tool::Curve(edge, L, f, l);
        if (!ConS.IsNull()) return ConS->Value(param).Transformed(L.Transformation());
    }
...
Standard_Boolean ShapeAnalysis_Wire::CheckSelfIntersectingEdge(const Standard_Integer num,
    ...
    for (Standard_Integer i = 1; i <= Inter.NbPoints(); i++) {
        ...
        if (Tr1.PositionOnCurve() != IntRes2d_Middle && Tr2.PositionOnCurve() != IntRes2d_Middle) continue;
...
Standard_Boolean ShapeAnalysis_Wire::CheckIntersectingEdges(const Standard_Integer num,
    ...
    if (Abs(a1 - b1) <= ::Precision::PConfusion() || Abs(a2 - b2) <= ::Precision::PConfusion())
        return Standard_False; //: f7 abv 6 May 98: BUC50070 on #42276
...
Standard_Boolean ShapeAnalysis_Wire::CheckIntersectingEdges(const Standard_Integer num1, const Standard_Integer num2,
                                                            IntRes2d_SequenceOfIntersectionPoint& points2d,
                                                            TColgp_SequenceOfPnt& points3d,
                                                            TColStd_SequenceOfReal& errors) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    ...
    if (Abs(a1 - b1) <= ::Precision::PConfusion() || Abs(a2 - b2) <= ::Precision::PConfusion()) return Standard_False;
    ...
    for (Standard_Integer i = 1; i <= NbPoints + NbSegments; i++) {
        ...
        for (Standard_Integer j = 1; (j <= 2) && !OK1; j++) {
            ...
            if (di1 < vertexTolers(j) * vertexTolers(j)) OK1 = Standard_True;
        }

        for (Standard_Integer j = 3; (j <= 4) && !OK2; j++) {
            Standard_Real di2 = pi2.SquareDistance(vertexPoints(j));
            if (di2 < vertexTolers(j) * vertexTolers(j)) OK2 = Standard_True;
        }
...
Standard_Boolean ShapeAnalysis_Wire::CheckLacking(const Standard_Integer num, const Standard_Real Tolerance,
...
static Standard_Real ProjectInside(const Adaptor3d_CurveOnSurface AD, const gp_Pnt pnt, const Standard_Real preci,
                                   gp_Pnt& proj, Standard_Real& param,
...
Standard_Boolean ShapeAnalysis_Wire::CheckNotchedEdges(const Standard_Integer num, Standard_Integer& shortNum,
                                                       Standard_Real& param, const Standard_Real Tolerance) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if (!IsReady()) return Standard_False;
    ...
    if (BRep_Tool::Degenerated(E1) || BRep_Tool::Degenerated(E2)) return Standard_False;
    ...
    if (v2.Magnitude() < gp::Resolution() || v1.Magnitude() < gp::Resolution()) return Standard_False;

    if (Abs(v2.Angle(v1)) > 0.1 || p2d1.Distance(p2d2) > Tolerance) return Standard_False;
    ...
    if (dist1 > Tolerance && dist2 > Tolerance) return Standard_False;
...
Standard_Boolean ShapeAnalysis_Wire::CheckShapeConnect(Standard_Real& tailhead, Standard_Real& tailtail,
    ...
    TopoDS_Vertex vfirst = SAE.FirstVertex(myWire->Edge(1)), vlast = SAE.LastVertex(myWire->Edge(NbEdges()));
    ...
    if (myMin3d > Max(myPrecision, prec)) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
    return LastCheckStatus(ShapeExtend_DONE);
}
...
// purpose  :
//=======================================================================
Standard_Boolean isMultiVertex(const TopTools_ListOfShape& alshape, const TopTools_MapOfShape& aMapSmallEdges,
...
// purpose  :
//=======================================================================
static Standard_Real Project(const Handle(Geom_Curve) & theCurve, const Standard_Real theFirstParameter,
                             const Standard_Real theLastParameter, const gp_Pnt& thePoint,
                             const Standard_Real thePrecision, Standard_Real& theParameter, gp_Pnt& theProjection) {
    const Standard_Real aDist = ShapeAnalysis_Curve().Project(theCurve, thePoint, thePrecision, theProjection,
                                                              theParameter, theFirstParameter, theLastParameter);
...
// purpose  :
//=======================================================================
Standard_Boolean ShapeAnalysis_Wire::CheckTail(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
                                               const Standard_Real theMaxSine, const Standard_Real theMaxWidth,
                                               const Standard_Real theMaxTolerance, TopoDS_Edge& theEdge11,
                                               TopoDS_Edge& theEdge12, TopoDS_Edge& theEdge21, TopoDS_Edge& theEdge22) {
    ...
    gp_Pnt aVPs[2];
    {
        for (Standard_Integer aEI = 0; aEI < 2; ++aEI) {
            if (!ShapeAnalysis_Edge().Curve3d(aEs[aEI], aCs[aEI], aLs[aEI][0], aLs[aEI][1], Standard_False)) {
                return Standard_False;
            }
    ...
    if (theMaxSine >= 0) {
        ...
        for (Standard_Integer aEI = 0; aEI < 2; ++aEI) {
            ...
            GCPnts_AbscissaPoint aAP(0.25 * Precision::Confusion(), aCA,
                                     0.5 * Precision::Confusion() * (1 - 2 * aVIs[aEI]), aLs[aEI][aVIs[aEI]]);
            if (!aAP.IsDone()) {
                return Standard_False;
            }
    ...
    for (Standard_Integer aEI = 0; aEI < 2; ++aEI) {
        ...
        aDists[aEI] = Project(aCs[1 - aEI], aLs[1 - aEI][0], aLs[1 - aEI][1], aPs[aEI], 0.25 * Precision::Confusion(),
                              aParams2[aEI], aPrjs[aEI]);
        if (aDists[aEI] <= aTol2) {
            continue;
        }
        ...
        for (;;) {
            ...
            const Standard_Real aDist = Project(aCs[1 - aEI], aLs[1 - aEI][0], aLs[1 - aEI][1], aPs[aEI],
                                                0.25 * Precision::Confusion(), aParams2[aEI], aPrjs[aEI]);
            if (aDist <= aTol2) {
                aParam1 = aParam;
            } else {
                aParams1[aEI] = aParam;
    ...
    for (Standard_Integer aEI = 0; aEI < 2; ++aEI) {
        ...
        for (Standard_Integer aStepN = 1; aStepN < 23; ++aStepN) {
            ...
            if (Project(aCs[1 - aEI], aLs[1 - aEI][0], aLs[1 - aEI][1], aP, 0.25 * Precision::Confusion(), aParam,
                        aPrj) > aTol4) {
                return Standard_False;
            }
        }
    ...
    for (Standard_Integer aEI = 0; aEI < 2; ++aEI) {
        ...
        TopoDS_Edge aEParts2[] = {ShapeBuild_Edge().CopyReplaceVertices(
                                      aFE, TopoDS_Vertex(), TopoDS::Vertex(aSplitV.Oriented(TopAbs_REVERSED))),
