...
ShapeAnalysis_Surface::ShapeAnalysis_Surface(const Handle(Geom_Surface) & S)
    : mySurf(S), myExtOK(Standard_False), //: 30
      myNbDeg(-1), myIsos(Standard_False), myIsoBoxes(Standard_False), myGap(0.), myUDelt(0.01), myVDelt(0.01),
      myUCloseVal(-1), myVCloseVal(-1) {
...
void ShapeAnalysis_Surface::Init(const Handle(ShapeAnalysis_Surface) & other) {
    ...
    for (Standard_Integer i = 0; i < myNbDeg; i++) {
        other->Singularity(i + 1, myPreci[i], myP3d[i], myFirstP2d[i], myLastP2d[i], myFirstPar[i], myLastPar[i],
                           myUIsoDeg[i]);
    }
}
...
Standard_Boolean ShapeAnalysis_Surface::Singularity(const Standard_Integer num, Standard_Real& preci, gp_Pnt& P3d,
                                                    gp_Pnt2d& firstP2d, gp_Pnt2d& lastP2d, Standard_Real& firstpar,
                                                    Standard_Real& lastpar, Standard_Boolean& uisodeg) {
...
Standard_Boolean ShapeAnalysis_Surface::IsDegenerated(const gp_Pnt& P3d, const Standard_Real preci) {
    ...
    for (Standard_Integer i = 0; i < myNbDeg && myPreci[i] <= preci; i++) {
        ...
        // rln S4135
        if (myGap <= preci) return Standard_True;
    }
...
Standard_Boolean ShapeAnalysis_Surface::DegeneratedValues(const gp_Pnt& P3d, const Standard_Real preci,
                                                          gp_Pnt2d& firstP2d, gp_Pnt2d& lastP2d,
                                                          Standard_Real& firstPar, Standard_Real& lastPar,
...
Standard_Boolean ShapeAnalysis_Surface::ProjectDegenerated(const gp_Pnt& P3d, const Standard_Real preci,
                                                           const gp_Pnt2d& neighbour, gp_Pnt2d& result) {
    if (myNbDeg < 0) ComputeSingularities();
    ...
    for (Standard_Integer i = 0; i < myNbDeg && myPreci[i] <= preci; i++) {
        ...
        if (gap2 > preci * preci) gap2 = Min(gap2, myP3d[i].SquareDistance(Value(result)));
        // rln S4135
...
Standard_Boolean ShapeAnalysis_Surface::ProjectDegenerated(const Standard_Integer nbrPnt,
                                                           ...
                                                           TColgp_SequenceOfPnt2d& pnt2d, const Standard_Real preci,
    ...
    for (Standard_Integer i = 0; i < myNbDeg && myPreci[i] <= preci; i++) {
        ...
        if (gap2 > prec2) gap2 = Min(gap2, myP3d[i].SquareDistance(Value(pnt2d(j))));
    ...
    for (k = j + step; k <= nbrPnt && k >= 1; k += step) {
        pk = pnt2d(k);
        gp_Pnt P1 = points(k);
        if (myP3d[indMin].SquareDistance(P1) > prec2 && myP3d[indMin].SquareDistance(Value(pk)) > prec2) break;
    }
...
Standard_Boolean ShapeAnalysis_Surface::IsDegenerated(const gp_Pnt2d& p2d1, const gp_Pnt2d& p2d2,
                                                      const Standard_Real tol, const Standard_Real ratio) {
...
static Handle(Geom_Curve)
    ComputeIso(const Handle(Geom_Surface) & surf, const Standard_Boolean utype, const Standard_Real par) {
    ...
    if (myUCloseVal < 0) {
        ...
        switch (surftype) {
            ...
            case GeomAbs_SurfaceOfExtrusion: { //: c8 abv 03 Mar 98: UKI60094 #753: process
                ...
                Handle(Geom_SurfaceOfLinearExtrusion) extr = Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurf);
...
//=======================================================================
Standard_Integer ShapeAnalysis_Surface::SurfaceNewton(const gp_Pnt2d& p2dPrev, const gp_Pnt& P3D,
                                                      const Standard_Real preci, gp_Pnt2d& sol) {
    GeomAdaptor_Surface& SurfAdapt = *Adaptor3d();
    Standard_Real uf, ul, vf, vl;
    Bounds(uf, ul, vf, vl);
    ...
    for (Standard_Integer i = 0; i < 25; i++) {
        ...
        Standard_Real D = -nrm2 + rv2 * rSuu + ru2 * rSvv - 2 * rSuv * (ru * rv) + rSuv * rSuv - rSuu * rSvv;
...
gp_Pnt2d ShapeAnalysis_Surface::NextValueOfUV(const gp_Pnt2d& p2dPrev, const gp_Pnt& P3D, const Standard_Real preci,
            ...
            if (surftype == GeomAbs_BSplineSurface) {
                ...
                if (SurfAdapt.UContinuity() == GeomAbs_C0) {
                    ...
                    for (Standard_Integer anIdx = aMinIndex; anIdx <= aMaxIndex; ++anIdx) {
                        ...
                        if (Abs(aKnot - p2dPrev.X()) < Precision::Confusion()) return ValueOfUV(P3D, preci);
                    }
                }
                ...
                if (SurfAdapt.VContinuity() == GeomAbs_C0) {
                    ...
                    for (Standard_Integer anIdx = aMinIndex; anIdx <= aMaxIndex; ++anIdx) {
                        ...
                        if (Abs(aKnot - p2dPrev.Y()) < Precision::Confusion()) return ValueOfUV(P3D, preci);
                    }
                }
            }
...
gp_Pnt2d ShapeAnalysis_Surface::ValueOfUV(const gp_Pnt& P3D, const Standard_Real preci) {
            ...
            switch (surftype) {
                ...
                case GeomAbs_OffsetSurface: //: d0 abv 3 Mar 98: UKI60107-1 #350
                {
                    ...
                    if (nPSurf > 0) {
                        ...
                        if (disSurf > preci) {
                            gp_Pnt2d pp(UU, VV);
                            if (SurfaceNewton(pp, P3D, preci, pp) != 0) { //: q2 abv 16 Mar 99: PRO7226 #412920
    ...
    if (computed) {
        if (myGap <= 0) myGap = P3D.Distance(SurfAdapt.Value(S, T));
    } else {
        myGap = -1.;
        S = 0.;
        T = 0.;
...
Standard_Real ShapeAnalysis_Surface::UVFromIso(const gp_Pnt& P3d, const Standard_Real preci, Standard_Real& U,
                                               Standard_Real& V) {
        ...
        for (Standard_Integer num = 0; num < 6; num++) {
            ...
            if (!(Adaptor3d()->GetType() == GeomAbs_OffsetSurface)) {
                ...
                if (!Precision::IsInfinite(par) && !iso.IsNull()) {
                    if (anIsoBox && anIsoBox->Distance(aPBox) > theMin) continue;
...
void ShapeAnalysis_Surface::SetDomain(const Standard_Real U1, const Standard_Real U2, const Standard_Real V1,
...
void ShapeAnalysis_Surface::ComputeBoxes() {
    ...
    ComputeBoundIsos();
    if (!myIsoUF.IsNull()) BndLib_Add3dCurve::Add(GeomAdaptor_Curve(myIsoUF), Precision::Confusion(), myBndUF);
    if (!myIsoUL.IsNull()) BndLib_Add3dCurve::Add(GeomAdaptor_Curve(myIsoUL), Precision::Confusion(), myBndUL);
    if (!myIsoVF.IsNull()) BndLib_Add3dCurve::Add(GeomAdaptor_Curve(myIsoVF), Precision::Confusion(), myBndVF);
    if (!myIsoVL.IsNull()) BndLib_Add3dCurve::Add(GeomAdaptor_Curve(myIsoVL), Precision::Confusion(), myBndVL);
}
