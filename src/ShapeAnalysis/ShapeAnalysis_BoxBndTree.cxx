...
Standard_Boolean ShapeAnalysis_BoxBndTreeSelector::Accept(const Standard_Integer& theObj) {
    ...
    if (myList.Contains(theObj)) return Standard_False;
    enum { First = 1, Last = 2 };
    ...
    if (myShared) {
        ...
        if (IsAccept) {
            SetNb(theObj);
            if (myArrIndices(Last)) myStop = Standard_True;
            return Standard_True;
        } else
        ...
        if (min3d == 0 && minInd == Last) myStop = Standard_True;
