...
ShapeAnalysis_WireOrder::ShapeAnalysis_WireOrder(const Standard_Boolean mode3d, const Standard_Real tol)
...
static Standard_Boolean IsBetter(const Standard_Integer first, const Standard_Integer second) {
...
void ShapeAnalysis_WireOrder::Perform(const Standard_Boolean /*closed*/) {
    ...
    if (nb == 0) return; // no edges loaded, nothing to do -- return with status OK
    ...
    if (myKeepLoops) {
        ...
        for (Standard_Integer ii = 1; ii <= loops.Length(); ii++) {
            Handle(TColStd_HSequenceOfInteger) subLoop = Handle(TColStd_HSequenceOfInteger)::DownCast(loops(ii));
        ...
        while (loops.Length()) {
            ...
            for (i = 1; i <= loops.Length(); i++) {
                ...
                for (Standard_Integer ibegin = 1; ibegin <= loop->Length(); ibegin++) {
                    ...
                    gp_Pnt loopFirst =
                        (loop->Value(ibegin) > 0 ? debs(loop->Value(ibegin)) : fins(-loop->Value(ibegin)));
            ...
            Handle(TColStd_HSequenceOfInteger) loop =
                Handle(TColStd_HSequenceOfInteger)::DownCast(loops.Value(loopNum));
            ...
            for (Standard_Integer ii = 1; ii <= loop->Length(); ii++) {
                Standard_Integer num =
                    (ii + loopShift - 1 > loop->Length() ? ii + loopShift - 1 - loop->Length() : ii + loopShift - 1);
    ...
    if (stTmp == 0) {
        ...
        for (i = 1; i <= length - 1; i++) {
            ...
            if (!(tmpSecond - tmpFirst == 1 || (tmpFirst == length && tmpSecond == 1))) isShiftForward = Standard_False;
            if (!(tmpFirst - tmpSecond == 1 || (tmpSecond == length && tmpFirst == 1))) isShiftReverse = Standard_False;
        }
        tmpFirst = mainSeq->Value(length);
        tmpSecond = mainSeq->Value(1);
        if (!(tmpSecond - tmpFirst == 1 || (tmpFirst == length && tmpSecond == 1))) isShiftForward = Standard_False;
        if (!(tmpFirst - tmpSecond == 1 || (tmpSecond == length && tmpFirst == 1))) isShiftReverse = Standard_False;
        if (isShiftForward || isShiftReverse) stTmp = 3;
        myStat = stTmp;
        return;
    }
}
...
Standard_Real ShapeAnalysis_WireOrder::Gap(const Standard_Integer num) const {
    ...
    return DISTABS(myXYZ->Value((n0 > 0 ? 2 * n0 : -2 * n0 - 1)), myXYZ->Value((n1 > 0 ? 2 * n1 - 1 : -2 * n1)));
