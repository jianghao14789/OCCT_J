...
Standard_Boolean ShapeAnalysis_Edge::BoundUV(const TopoDS_Edge& edge, const TopoDS_Face& face, gp_Pnt2d& first,
                                             gp_Pnt2d& last) const {
    TopLoc_Location L;
...
Standard_Boolean ShapeAnalysis_Edge::BoundUV(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
                                             const TopLoc_Location& location, gp_Pnt2d& first, gp_Pnt2d& last) const {
...
Standard_Boolean ShapeAnalysis_Edge::Curve3d(const TopoDS_Edge& edge, Handle(Geom_Curve) & C3d, Standard_Real& cf,
                                             Standard_Real& cl, const Standard_Boolean orient) const {
    TopLoc_Location L;
...
Standard_Boolean ShapeAnalysis_Edge::HasPCurve(const TopoDS_Edge& edge, const TopoDS_Face& face) const {
    TopLoc_Location L;
    const Handle(Geom_Surface) & S = BRep_Tool::Surface(face, L);
...
Standard_Boolean ShapeAnalysis_Edge::HasPCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
...
Standard_Boolean ShapeAnalysis_Edge::PCurve(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                            Handle(Geom2d_Curve) & C2d, Standard_Real& cf, Standard_Real& cl,
...
Standard_Boolean ShapeAnalysis_Edge::PCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
                                            const TopLoc_Location& location, Handle(Geom2d_Curve) & C2d,
                                            Standard_Real& cf, Standard_Real& cl, const Standard_Boolean orient) const {
...
Standard_Boolean ShapeAnalysis_Edge::IsSeam(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
                                            const TopLoc_Location& location) const {
...
Standard_Boolean
ShapeAnalysis_Edge::GetEndTangent2d(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                    const Standard_Boolean atend1, /* skl : change "atend" to "atend1" */
                                    gp_Pnt2d& pnt, gp_Vec2d& v, const Standard_Real dparam) const {
    TopLoc_Location L;
    const Handle(Geom_Surface) S = BRep_Tool::Surface(face, L);
...
Standard_Boolean
ShapeAnalysis_Edge::GetEndTangent2d(const TopoDS_Edge& edge, const Handle(Geom_Surface) & S, const TopLoc_Location& L,
        ...
        if (v.SquareMagnitude() < Precision::PConfusion() * Precision::PConfusion()) {
            ...
            if (v.SquareMagnitude() < Precision::PConfusion() * Precision::PConfusion()) {
                ...
                if (v.SquareMagnitude() < Precision::PConfusion() * Precision::PConfusion()) {
                    ...
                    v = p2.XY() - pnt.XY();
                    if (v.SquareMagnitude() < Precision::PConfusion() * Precision::PConfusion()) return Standard_False;
                }
            }
        }
...
Standard_Boolean ShapeAnalysis_Edge::CheckCurve3dWithPCurve(const TopoDS_Edge& edge, const TopoDS_Face& face) {
    TopLoc_Location L;
    const Handle(Geom_Surface) & S = BRep_Tool::Surface(face, L);
...
Standard_Boolean ShapeAnalysis_Edge::CheckCurve3dWithPCurve(const TopoDS_Edge& edge,
    ...
    if (surface->IsKind(STANDARD_TYPE(Geom_Plane))) return Standard_False;
    ...
    if (aFirstVert.IsNull() || aLastVert.IsNull()) return Standard_False;
    ...
    return CheckPoints(c3d->Value(f3d) /*.Transformed (location.Transformation())*/,
                       ...
                       surface->Value(p2d2.X(), p2d2.Y()).Transformed(location.Transformation()), preci1, preci2);
}
...
Standard_Boolean ShapeAnalysis_Edge::CheckPoints(const gp_Pnt& P1A, const gp_Pnt& P1B, const gp_Pnt& P2A,
                                                 const gp_Pnt& P2B, const Standard_Real preci1,
...
Standard_Boolean ShapeAnalysis_Edge::CheckVerticesWithCurve3d(const TopoDS_Edge& edge, const Standard_Real preci,
    ...
    if (vtx != 2) {
        ...
        if (p1v.Distance(p13d) > (preci < 0 ? BRep_Tool::Tolerance(V1) : preci)) myStatus |= ShapeExtend_DONE1;
    }
    ...
    if (vtx != 1) {
        ...
        // szv#4:S4163:12Mar99 optimized
        if (p2v.Distance(p23d) > (preci < 0 ? BRep_Tool::Tolerance(V2) : preci)) myStatus |= ShapeExtend_DONE2;
    }
...
Standard_Boolean ShapeAnalysis_Edge::CheckVerticesWithPCurve(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                                             const Standard_Real preci, const Standard_Integer vtx) {
    TopLoc_Location L;
    const Handle(Geom_Surface) & S = BRep_Tool::Surface(face, L);
...
Standard_Boolean ShapeAnalysis_Edge::CheckVerticesWithPCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surf,
                                                             const TopLoc_Location& loc, const Standard_Real preci,
                                                             const Standard_Integer vtx) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    ...
    if (vtx != 2) { //  1er VTX
        ...
        if (!loc.IsIdentity()) p12d.Transform(loc.Transformation());
        // szv#4:S4163:12Mar99 optimized
        if (p1v.Distance(p12d) > (preci < 0 ? BRep_Tool::Tolerance(V1) : preci)) myStatus |= ShapeExtend_DONE1;
    }

    if (vtx != 1) { //  2me VTX
        gp_Pnt2d p2uv = c2d->Value(cl);
        gp_Pnt p22d = surf->Value(p2uv.X(), p2uv.Y());
        if (!loc.IsIdentity()) p22d.Transform(loc.Transformation());
        // szv#4:S4163:12Mar99 optimized
        if (p2v.Distance(p22d) > (preci < 0 ? BRep_Tool::Tolerance(V2) : preci)) myStatus |= ShapeExtend_DONE2;
    }
...
static Standard_Integer CheckVertexTolerance(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                             const Standard_Boolean checkAll, Standard_Real& toler1,
    ...
    if (!sae.Curve3d(edge, c3d, a, b, Standard_True)) {
        if (!BRep_Tool::Degenerated(edge)) Status |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
        toler1 = toler2 = 0.;
        //    return Standard_False;
    } else {
    ...
    if (toler1 > old1) Status |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (toler2 > old2) Status |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
...
Standard_Boolean ShapeAnalysis_Edge::CheckVertexTolerance(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                                          Standard_Real& toler1, Standard_Real& toler2) {
...
Standard_Boolean ShapeAnalysis_Edge::CheckVertexTolerance(const TopoDS_Edge& edge, Standard_Real& toler1,
                                                          Standard_Real& toler2) {
    TopoDS_Face F;
...
Standard_Boolean ShapeAnalysis_Edge::CheckSameParameter(const TopoDS_Edge& edge, Standard_Real& maxdev,
...
Standard_Boolean ShapeAnalysis_Edge::CheckSameParameter(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                                        Standard_Real& maxdev, const Standard_Integer NbControl) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    ...
    if (!face.IsNull()) aFaceSurf = BRep_Tool::Surface(face, aFaceLoc);
    ...
    for (;;) {
        ...
        Handle(Geom_Surface) aST = Handle(Geom_Surface)::DownCast(aS->Transformed(aLoc.Transformation()));
    ...
    if (maxdev > TE->Tolerance()) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (!SameParameter) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
...
static Standard_Boolean IsOverlapPartEdges(const TopoDS_Edge& theFirstEdge, const TopoDS_Edge& theSecEdge,
                                           const Standard_Real& theTolerance, const Standard_Real& theStep,
                                           const Standard_Real& theStartLength, const Standard_Real& theEndLength) {
    ...
    for (Standard_Real aS = theStartLength; aS <= theEndLength; aS += theStep / 2) {
        ...
        aMinDist.LoadS2(aV);
        aMinDist.Perform();
        if (aMinDist.IsDone() && aMinDist.Value() >= theTolerance) return Standard_False;
    }
    return Standard_True;
}
...
Standard_Boolean ShapeAnalysis_Edge::CheckOverlapping(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
                                                      Standard_Real& theTolOverlap, const Standard_Real theDomainDist) {
    ...
    if (theDomainDist == 0.0) return isOverlap;
    ...
    if (isOverlap) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE4);
...
Standard_Boolean ShapeAnalysis_Edge::CheckPCurveRange(const Standard_Real theFirst, const Standard_Real theLast,
