...
static void ContourProperties(TopoDS_Wire wire, Standard_Real& countourArea, Standard_Real& countourLength) {
...
void ShapeAnalysis_FreeBoundsProperties::Init(const TopoDS_Shape& shape, const Standard_Real tolerance,
...
void ShapeAnalysis_FreeBoundsProperties::Init(const TopoDS_Shape& shape, const Standard_Boolean splitclosed,
                                              const Standard_Boolean splitopen) {
    myShape = shape;
...
Standard_Boolean ShapeAnalysis_FreeBoundsProperties::CheckNotches(Handle(ShapeAnalysis_FreeBoundData) & fbData,
    ...
    if (swd.NbEdges() > 1)
        for (Standard_Integer j = 1; j <= swd.NbEdges(); j++) {
            TopoDS_Wire notch;
            Standard_Real dMax;
            if (CheckNotches(fbData->FreeBound(), j, notch, dMax, prec)) fbData->AddNotch(notch, dMax);
        }
...
Standard_Boolean ShapeAnalysis_FreeBoundsProperties::CheckNotches(const TopoDS_Wire& wire, const Standard_Integer num,
    ...
    if (!sae.Curve3d(E1, c3d1, First1, Last1) || !sae.Curve3d(E2, c3d2, First2, Last2)) return Standard_False;
