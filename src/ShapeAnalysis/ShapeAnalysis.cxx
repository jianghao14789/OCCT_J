...
// purpose  :
//=======================================================================
Standard_Real ShapeAnalysis::AdjustByPeriod(const Standard_Real Val, const Standard_Real ToVal,
...
Standard_Real ShapeAnalysis::AdjustToPeriod(const Standard_Real Val, const Standard_Real ValMin,
...
// purpose  : auxiliary
//=======================================================================
template <class HSequence> static inline void ReverseSeq(HSequence& Seq) {
...
Standard_Real ShapeAnalysis::TotCross2D(const Handle(ShapeExtend_WireData) & sewd, const TopoDS_Face& aFace) {
    ...
    for (i = 1; i <= sewd->NbEdges(); i++) {
        ...
        if (!c2d.IsNull()) {
            ...
            if (edge.Orientation() == 1) ReverseSeq(SeqPnt);
            if (nbc == 1) {
                fuv = SeqPnt.Value(1);
                uv0 = fuv;
            }
...

{
    ...
    for (; aIte.More(); aIte.Next()) {
        ...
        if (!c3d.IsNull()) {
            ...
            if (!ShapeAnalysis_Curve::GetSamplePoints(c3d, first, last, aSeqPnt)) continue;
            nbc++;
            if (edge.Orientation() == TopAbs_REVERSED) ReverseSeq(aSeqPnt);
            if (nbc == 1) {
                fuv = aSeqPnt.Value(1);
                uv0 = fuv;
            }
...
TopoDS_Wire ShapeAnalysis::OuterWire(const TopoDS_Face& face) {
    ...
    while (anIt.More()) {
        ...
        if (aWire.ShapeType() != TopAbs_WIRE) continue;

        // if current wire is the last one, return it without analysis
        if (!anIt.More()) return TopoDS::Wire(aWire);

        TopoDS_Shape aTestFace = F.EmptyCopied();
        B.Add(aTestFace, aWire);
        if (ShapeAnalysis::IsOuterBound(TopoDS::Face(aTestFace))) return TopoDS::Wire(aWire);
    }
    return TopoDS_Wire();
}
...
void ShapeAnalysis::GetFaceUVBounds(const TopoDS_Face& F, Standard_Real& UMin, Standard_Real& UMax, Standard_Real& VMin,
                                    Standard_Real& VMax) {
    TopoDS_Face FF = F;
