...
public:
    ...
    Standard_EXPORT Standard_Real Tolerance(const TopoDS_Shape& shape, const Standard_Integer mode,
    ...
    Standard_EXPORT Handle(TopTools_HSequenceOfShape)
        OverTolerance(const TopoDS_Shape& shape, const Standard_Real value,
                      const TopAbs_ShapeEnum type = TopAbs_SHAPE) const;

    //! Determines which shapes have a tolerance within a given interval
    //! <type> is interpreted as in the method Tolerance
    Standard_EXPORT Handle(TopTools_HSequenceOfShape)
        InTolerance(const TopoDS_Shape& shape, const Standard_Real valmin, const Standard_Real valmax,
                    const TopAbs_ShapeEnum type = TopAbs_SHAPE) const;
