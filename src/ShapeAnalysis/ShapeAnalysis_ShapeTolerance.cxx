...
static void AddTol(const Standard_Real tol, Standard_Integer& nbt, Standard_Real& cmin, Standard_Real& cmoy,
                   Standard_Real& cmax) {
    nbt++;
    if (nbt == 1)
        cmin = cmoy = cmax = tol;
...
Standard_Real ShapeAnalysis_ShapeTolerance::Tolerance(const TopoDS_Shape& shape, const Standard_Integer mode,
    ...
    if (type == TopAbs_SHELL) {
        ...
        for (; myExp.More(); myExp.Next()) {
            ...
            if (tol >= valmin && (over || (tol <= valmax)))
                ...
                // les edges contenues ?
                Handle(TopTools_HSequenceOfShape) fl = InTolerance(myExp.Current(), valmin, valmax, TopAbs_EDGE);
                if (fl->Length() > 0)
                    iaface = Standard_True;
                else {
