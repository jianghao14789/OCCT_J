...
// purpose  :
//=======================================================================
ShapeAnalysis_TransferParametersProj::ShapeAnalysis_TransferParametersProj() : myPrecision(0.0) {
...
ShapeAnalysis_TransferParametersProj::ShapeAnalysis_TransferParametersProj(const TopoDS_Edge& E, const TopoDS_Face& F) {
    myMaxTolerance = 1; // Precision::Infinite(); ?? pdn
    myForceProj = Standard_False;
    Init(E, F);
}
...
Handle(TColStd_HSequenceOfReal) ShapeAnalysis_TransferParametersProj::Perform(const Handle(TColStd_HSequenceOfReal) &
                                                                                  Knots,
                                                                              const Standard_Boolean To2d) {
    // pdn
    if (!myInitOK || (!myForceProj && myPrecision < myMaxTolerance && BRep_Tool::SameParameter(myEdge)))
    ...
    for (j = 1; j <= len; j++) {
        ...
        prevPar = par;
        if (prevPar > lastPar) prevPar -= preci;
        resKnots->Append(par);
...
Standard_Real ShapeAnalysis_TransferParametersProj::PreformSegment(const Standard_Real Param,
    ...
    if (!myInitOK || (!myForceProj && myPrecision < myMaxTolerance && BRep_Tool::SameParameter(myEdge))) return linPar;
    ...
    if (linDev <= projDev || (linDev < myPrecision && linDev <= 2 * projDev)) ppar = linPar;
    return ppar;
}
...
Standard_Real ShapeAnalysis_TransferParametersProj::Perform(const Standard_Real Knot, const Standard_Boolean To2d) {
    if (!myInitOK || (!myForceProj && myPrecision < myMaxTolerance && BRep_Tool::SameParameter(myEdge)))
...
static Standard_Real CorrectParameter(const Handle(Geom2d_Curve) crv, const Standard_Real param) {
    if (crv->IsKind(STANDARD_TYPE(Geom2d_TrimmedCurve))) {
        ...
        for (Standard_Integer j = bspline->FirstUKnotIndex(); j <= bspline->LastUKnotIndex(); j++) {
            ...
            if (Abs(valknot - param) < Precision::PConfusion()) return valknot;
        }
    }
    return param;
}
...
void ShapeAnalysis_TransferParametersProj::TransferRange(TopoDS_Edge& newEdge, const Standard_Real prevPar,
                                                         const Standard_Real currPar, const Standard_Boolean Is2d) {
    if (!myInitOK || (!myForceProj && myPrecision < myMaxTolerance && BRep_Tool::SameParameter(myEdge))) {
    ...
    if (Is2d) {
        ...
        if (Precision::IsInfinite(p1.X()) || Precision::IsInfinite(p1.Y()) || Precision::IsInfinite(p1.Z())) {
            B.SameRange(newEdge, Standard_False);
            return;
        }
        p2 = myAC3d.Value(lastPar).Transformed(myLocation);
        if (Precision::IsInfinite(p2.X()) || Precision::IsInfinite(p2.Y()) || Precision::IsInfinite(p2.Z())) {
            B.SameRange(newEdge, Standard_False);
            return;
        }
    ...
    for (BRep_ListIteratorOfListOfCurveRepresentation toitcr(tolist); toitcr.More(); toitcr.Next()) {
        ...
        if (toGC->IsCurve3D()) {
            ...
            if (ppar1 != firstPar || ppar2 != lastPar) samerange = Standard_False;
            ...
            if (isFirstOnEnd && !localLinearFirst) localLinearFirst = Standard_True;
            if (isLastOnEnd && !localLinearLast) localLinearLast = Standard_True;
...
TopoDS_Vertex ShapeAnalysis_TransferParametersProj::CopyNMVertex(const TopoDS_Vertex& theV, const TopoDS_Edge& toedge,
                                                                 const TopoDS_Edge& fromedge) {
    TopoDS_Vertex anewV;
    if (theV.Orientation() != TopAbs_INTERNAL && theV.Orientation() != TopAbs_EXTERNAL) return anewV;
    ...
    BRep_ListIteratorOfListOfPointRepresentation itpr((*((Handle(BRep_TVertex)*)&theV.TShape()))->Points());
    ...
    for (; itpr.More(); itpr.Next()) {
        ...
        if (pr->IsPointOnCurve(C1, fromLoc)) {
            ...
            Handle(BRep_PointOnSurface) aPS =
                new BRep_PointOnSurface(aOld->Parameter(), aOld->Parameter2(), aOld->Surface(), aOld->Location());
            alistrep.Append(aPS);
            continue;
    ...
    BRep_ListIteratorOfListOfCurveRepresentation toitcr((*((Handle(BRep_TEdge)*)&toedge.TShape()))->ChangeCurves());
    ...
    if (needUpdate) aB.UpdateVertex(anewV, aTol);
    return anewV;
}
...
TopoDS_Vertex ShapeAnalysis_TransferParametersProj::CopyNMVertex(const TopoDS_Vertex& theV, const TopoDS_Face& toFace,
    ...
    if (!hasRepr || (fromSurf != toSurf || fromLoc != toLoc)) {
        ...
        if (aTol < aSurfTool->Gap()) aTol = aSurfTool->Gap() + 0.1 * Precision::Confusion();
