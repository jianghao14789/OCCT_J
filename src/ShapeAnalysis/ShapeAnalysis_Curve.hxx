...
public:
    ...
    //! computed one.
    Standard_EXPORT Standard_Real Project(const Handle(Geom_Curve) & C3D, const gp_Pnt& P3D, const Standard_Real preci,
    ...
    //! Returned value is the distance between the given point and
    //! computed one.
    Standard_EXPORT Standard_Real Project(const Adaptor3d_Curve& C3D, const gp_Pnt& P3D, const Standard_Real preci,
                                          gp_Pnt& proj, Standard_Real& param,
                                          const Standard_Boolean AdjustToEnds = Standard_True) const;
    ...
    Standard_EXPORT Standard_Real ProjectAct(const Adaptor3d_Curve& C3D, const gp_Pnt& P3D, const Standard_Real preci,
    ...
    //! If AdjustToEnds is True, point will be adjusted to the end
    //! of the curve if distance is less than <preci>
    Standard_EXPORT Standard_Real NextProject(const Standard_Real paramPrev, const Handle(Geom_Curve) & C3D,
    ...
    //! <paramPrev> is taken as the first approximation of solution.
    //! If Newton algorithm fails the method Project() is used.
    Standard_EXPORT Standard_Real NextProject(const Standard_Real paramPrev, const Adaptor3d_Curve& C3D,
                                              const gp_Pnt& P3D, const Standard_Real preci, gp_Pnt& proj,
    ...
    Standard_EXPORT Standard_Boolean ValidateRange(const Handle(Geom_Curve) & Crv, Standard_Real& First,
                                                   Standard_Real& Last, const Standard_Real prec) const;
    ...
    Standard_EXPORT void FillBndBox(const Handle(Geom2d_Curve) & C2d, const Standard_Real First,
    ...
    Standard_EXPORT static Standard_Boolean IsPlanar(const TColgp_Array1OfPnt& pnts, gp_XYZ& Normal,
                                                     const Standard_Real preci = 0);

    //! Checks if curve is planar with given preci. If Normal has not zero
    //! modulus, checks with given normal
    Standard_EXPORT static Standard_Boolean IsPlanar(const Handle(Geom_Curve) & curve, gp_XYZ& Normal,
                                                     const Standard_Real preci = 0);
    ...
    Standard_EXPORT static Standard_Boolean GetSamplePoints(const Handle(Geom_Curve) & curve, const Standard_Real first,
