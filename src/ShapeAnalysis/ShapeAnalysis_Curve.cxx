...
// purpose  :
//=======================================================================
static void ProjectOnSegments(const Adaptor3d_Curve& AC, const gp_Pnt& P3D, const Standard_Integer nbseg,
                              Standard_Real& uMin, Standard_Real& uMax, Standard_Real& distmin, gp_Pnt& proj,
                              Standard_Real& param) {
    ...
    if (aHasChanged) distmin = Sqrt(distmin2);
#ifdef OCCT_DEBUG
...
Standard_Real ShapeAnalysis_Curve::Project(const Handle(Geom_Curve) & C3D, const gp_Pnt& P3D, const Standard_Real preci,
...
Standard_Real ShapeAnalysis_Curve::Project(const Handle(Geom_Curve) & C3D, const gp_Pnt& P3D, const Standard_Real preci,
                                           gp_Pnt& proj, Standard_Real& param, const Standard_Real cf,
                                           const Standard_Real cl, const Standard_Boolean AdjustToEnds) const {
    ...
    if (C3D->IsKind(STANDARD_TYPE(Geom_BoundedCurve))) {
        Standard_Real prec =
            (AdjustToEnds ? preci : Precision::Confusion()); //: j8 abv 10 Dec 98: tr10_r0501_db.stp #9423: protection
                                                             //: against densing of points near one end
...
Standard_Real ShapeAnalysis_Curve::Project(const Adaptor3d_Curve& C3D, const gp_Pnt& P3D, const Standard_Real preci,
                                           gp_Pnt& proj, Standard_Real& param,
                                           const Standard_Boolean AdjustToEnds) const

{
    ...
    if (Precision::IsInfinite(uMin) && Precision::IsInfinite(uMax)) return ProjectAct(C3D, P3D, preci, proj, param);
    ...
    if (distProj < distmin_L + Precision::Confusion() && distProj < distmin_H + Precision::Confusion()) return distProj;
...
Standard_Real ShapeAnalysis_Curve::ProjectAct(const Adaptor3d_Curve& C3D, const gp_Pnt& P3D, const Standard_Real preci,
                                              gp_Pnt& proj, Standard_Real& param) const
        ...
        switch (C3D.GetType()) {
            ...
            default: {
                ...
                if (aProjector.IsDone()) {
                    ...
                    if (aDistNewton < aModMin) return aDistNewton;
                }
...
Standard_Real ShapeAnalysis_Curve::NextProject(const Standard_Real paramPrev, const Handle(Geom_Curve) & C3D,
                                               const gp_Pnt& P3D, const Standard_Real preci, gp_Pnt& proj,
                                               Standard_Real& param, const Standard_Real cf, const Standard_Real cl,
                                               const Standard_Boolean AdjustToEnds) const {
    Standard_Real uMin = (cf < cl ? cf : cl);
    Standard_Real uMax = (cf < cl ? cl : cf);
...
Standard_Real ShapeAnalysis_Curve::NextProject(const Standard_Real paramPrev, const Adaptor3d_Curve& C3D,
                                               const gp_Pnt& P3D, const Standard_Real preci, gp_Pnt& proj,
...
Standard_Boolean ShapeAnalysis_Curve::ValidateRange(const Handle(Geom_Curve) & theCurve, Standard_Real& First,
                                                    Standard_Real& Last, const Standard_Real preci) const {
    ...
    if (ShapeAnalysis_Curve::IsPeriodic(theCurve)) {
        ElCLib::AdjustPeriodic(cf, cl, Precision::PConfusion(), First, Last); //: a7 abv 11 Feb 98: preci ->
                                                                              //: PConfusion()
    } else if (First < Last) {
        // nothing to fix
...
static Standard_Integer SearchForExtremum(const Handle(Geom2d_Curve) & C2d, const Standard_Real First,
                                          const Standard_Real Last, const gp_Vec2d& dir, Standard_Real& par,
                                          gp_Pnt2d& res) {
    ...
    for (Standard_Integer i = 0; i < 10; i++) {
        ...
        if (par < First) {
            if (nbOut++ > 2 || prevpar == First) return Standard_False;
            par = First;
        }
        if (par > Last) {
            if (nbOut++ > 2 || prevpar == Last) return Standard_False;
            par = Last;
        }
    }
    return Standard_True;
...
void ShapeAnalysis_Curve::FillBndBox(const Handle(Geom2d_Curve) & C2d, const Standard_Real First,
                                     const Standard_Real Last, const Standard_Integer NPoints,
                                     const Standard_Boolean Exact, Bnd_Box2d& Box) const {
    if (!Exact) {
...
Standard_Boolean ShapeAnalysis_Curve::IsPlanar(const TColgp_Array1OfPnt& pnts, gp_XYZ& Normal,
    ...
    if (noNorm) {
        ...
        for (i = 1; i < pnts.Length(); i++) {
            ...
            if (aTmpDir.SquareModulus() > aMaxDir.SquareModulus()) aMaxDir = aTmpDir;

            gp_XYZ aDelta = (pnts(i).XYZ() - aCenter) ^ (pnts(i + 1).XYZ() - aCenter);
            if (Normal * aDelta < 0) aDelta *= -1;
            Normal += aDelta;
        }
    }
...
Standard_Boolean ShapeAnalysis_Curve::IsPlanar(const Handle(Geom_Curve) & curve, gp_XYZ& Normal,
                                               const Standard_Real preci) {
    Standard_Real precision = (preci > 0.0) ? preci : Precision::Confusion();
    Standard_Boolean noNorm = (Normal.SquareModulus() == 0);
...
Standard_Boolean ShapeAnalysis_Curve::GetSamplePoints(const Handle(Geom_Curve) & curve, const Standard_Real first,
                                                      const Standard_Real last, TColgp_SequenceOfPnt& seq) {
    Standard_Real adelta = curve->LastParameter() - curve->FirstParameter();
    if (!adelta) return Standard_False;
...
Standard_Boolean ShapeAnalysis_Curve::GetSamplePoints(const Handle(Geom2d_Curve) & curve, const Standard_Real first,
                                                      const Standard_Real last, TColgp_SequenceOfPnt2d& seq) {
...
Standard_Boolean ShapeAnalysis_Curve::IsClosed(const Handle(Geom_Curve) & theCurve, const Standard_Real preci) {
    if (theCurve->IsClosed()) return Standard_True;
    ...
    if (Precision::IsInfinite(f) || Precision::IsInfinite(l)) return Standard_False;
