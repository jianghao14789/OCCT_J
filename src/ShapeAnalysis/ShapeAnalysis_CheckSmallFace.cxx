...
static void MinMaxPnt(const gp_Pnt& p, Standard_Integer& nb, Standard_Real& minx, Standard_Real& miny,
                      Standard_Real& minz, Standard_Real& maxx, Standard_Real& maxy, Standard_Real& maxz) {
    Standard_Real x, y, z;
    p.Coord(x, y, z);
    if (nb < 1) {
        minx = maxx = x;
...
static Standard_Boolean MinMaxSmall(const Standard_Real minx, const Standard_Real miny, const Standard_Real minz,
                                    const Standard_Real maxx, const Standard_Real maxy, const Standard_Real maxz,
...
Standard_Integer ShapeAnalysis_CheckSmallFace::IsSpotFace(const TopoDS_Face& F, gp_Pnt& spot, Standard_Real& spotol,
    ...
    Standard_Real minx = 0, miny = 0, minz = 0, maxx = Precision::Infinite(), maxy = Precision::Infinite(),
    ...
    for (TopExp_Explorer iv(F, TopAbs_VERTEX); iv.More(); iv.Next()) {
        ...
        if (V0.IsNull())
            ...
            if (!V0.IsSame(V)) same = Standard_False;
        }
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::CheckStripEdges(const TopoDS_Edge& E1, const TopoDS_Edge& E2,
                                                               const Standard_Real tol, Standard_Real& dmax) const {
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::FindStripEdges(const TopoDS_Face& F, TopoDS_Edge& E1, TopoDS_Edge& E2,
                                                              const Standard_Real tol, Standard_Real& dmax) {
    E1.Nullify();
    E2.Nullify();
    Standard_Integer nb = 0;
    for (TopExp_Explorer ex(F, TopAbs_EDGE); ex.More(); ex.Next()) {
        TopoDS_Edge E = TopoDS::Edge(ex.Current());
        if (nb == 1 && E.IsSame(E1)) continue; // ignore seam edge
        TopoDS_Vertex V1, V2;
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::CheckSingleStrip(const TopoDS_Face& F, TopoDS_Edge& E1, TopoDS_Edge& E2,
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::CheckStripFace(const TopoDS_Face& F, TopoDS_Edge& E1, TopoDS_Edge& E2,
                                                              const Standard_Real tol) {
...
Standard_Integer ShapeAnalysis_CheckSmallFace::CheckSplittingVertices(const TopoDS_Face& F,
    ...
    if (nbp != 0) myStatusSplitVert = ShapeExtend::EncodeStatus(ShapeExtend_DONE);
    return nbp;
}

static Standard_Integer IsoStat(const TColgp_Array2OfPnt& poles, const Standard_Integer uorv,
                                const Standard_Integer rank, const Standard_Real tolpin, const Standard_Real toler) {
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::CheckPin(const TopoDS_Face& F, Standard_Integer& whatrow,
...
static Standard_Real TwistedNorm(const Standard_Real x1, const Standard_Real y1, const Standard_Real z1,
...
Standard_Boolean ShapeAnalysis_CheckSmallFace::CheckPinEdges(const TopoDS_Edge& theFirstEdge,
                                                             ...
                                                             const Standard_Real coef1, const Standard_Real coef2,
    ...
    if (dist <= tol) {
        ...
        //       std::cout << "angle1 "   << angle1<< std::endl;
        //       std::cout << "angle2 "   << angle2<< std::endl;
        if ((angle1 <= 0.001 && angle2 <= 0.01) || ((M_PI - angle2) <= 0.001 && (M_PI - angle2) <= 0.01))
            return Standard_True;
        else
            return Standard_False;
    }
