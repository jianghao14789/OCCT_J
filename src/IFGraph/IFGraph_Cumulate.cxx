...
IFGraph_Cumulate::IFGraph_Cumulate(const Interface_Graph& agraph) : thegraph(agraph) {}
...
Interface_EntityIterator IFGraph_Cumulate::Overlapped() const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (thegraph.IsPresent(i) && thegraph.Status(i) > 2) iter.GetOneItem(thegraph.Entity(i));
    }
    return iter;
}
...
Interface_EntityIterator IFGraph_Cumulate::Forgotten() const {
    ...
    Standard_Integer nb = thegraph.Size();
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (!thegraph.IsPresent(i)) iter.GetOneItem(thegraph.Model()->Value(i));
    }
    return iter;
}
...
Interface_EntityIterator IFGraph_Cumulate::PerCount(const Standard_Integer count) const {
    ...
    Standard_Integer nb = thegraph.Size();
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (thegraph.IsPresent(i) && thegraph.Status(i) == (count + 1)) iter.GetOneItem(thegraph.Model()->Value(i));
    }
    return iter;
}
