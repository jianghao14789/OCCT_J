...
IFGraph_ExternalSources::IFGraph_ExternalSources(const Interface_Graph& agraph) : thegraph(agraph) {}
...
Standard_Boolean IFGraph_ExternalSources::IsEmpty() {
    ...
    Standard_Integer nb = thegraph.Size();
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (thegraph.IsPresent(i) || thegraph.Status(i) == 1) return Standard_False;
    }
