...
IFGraph_SubPartsIterator::IFGraph_SubPartsIterator(IFGraph_SubPartsIterator& other) : thegraph(other.Graph()) {
...
void IFGraph_SubPartsIterator::Evaluate() {} // par defaut, ne fait rien; redefinie par les sous-classes
...
Interface_Graph IFGraph_SubPartsIterator::LoadedGraph() const {
    ...
    Standard_Integer nb = thegraph.Size();
    for (Standard_Integer i = 1; i <= nb; i++) {
        if (thegraph.IsPresent(i) && thegraph.Status(i) == 0) G.GetFromEntity(thegraph.Entity(i), Standard_False);
    }
    return G;
}
...
Standard_Integer IFGraph_SubPartsIterator::EntityPartNum(const Handle(Standard_Transient) & ent) const {
    Standard_Integer num = thegraph.EntityNumber(ent);
...
void IFGraph_SubPartsIterator::Next() {
    thecurr++;
    if (thecurr > theparts->Length()) return;
...
Interface_EntityIterator IFGraph_SubPartsIterator::Entities() const {
    ...
    for (Standard_Integer i = nument; i <= nb; i++) {
        if (thegraph.Status(i) == thecurr && thegraph.IsPresent(i)) iter.GetOneItem(thegraph.Entity(i));
    }
    return iter;
}
