...
PCDM_ReaderStatus CDF_Application::CanRetrieve(const TCollection_ExtendedString& theFolder,
                                               const TCollection_ExtendedString& theName, const bool theAppendMode) {
...
PCDM_ReaderStatus CDF_Application::CanRetrieve(const TCollection_ExtendedString& theFolder,
                                               ...
                                               const TCollection_ExtendedString& theVersion, const bool theAppendMode) {
...
Handle(CDM_Document) CDF_Application::Retrieve(const Handle(CDM_MetaData) & aMetaData,
    ...
    if (IsComponent) {
        ...
        switch (myRetrievableStatus) {
            ...
            case PCDM_RS_PermissionDenied:
                aMsg << "Could not find the referenced document: " << aMetaData->Path() << "; permission denied. "
                     << (char)0 << std::endl;
                break;
        ...
        if (myRetrievableStatus != PCDM_RS_OK) throw Standard_Failure(aMsg.str().c_str());
        myRetrievableStatus = PCDM_RS_DriverFailure;
    }
    ...
    if (!AlreadyRetrieved || Modified || isAppendMode) {
        ...
        if (Modified || isAppendMode) {
            ...
            if (!isAppendMode) aDocument->RemoveAllReferences();
        } else {
            NewDocument(aFormat, aDocument);
            SetReferenceCounter(aDocument,
...
// purpose  :
//=======================================================================
void CDF_Application::Read(Standard_IStream& theIStream, Handle(CDM_Document) & theDocument,
                           const Handle(PCDM_ReaderFilter) & theFilter, const Message_ProgressRange& theRange) {
...
Handle(PCDM_Reader) CDF_Application::ReaderFromFormat(const TCollection_ExtendedString& theFormat) {
    ...
    if (myReaders.FindFromKey(theFormat, aReader)) return aReader;
    ...
    if (strPluginId.Search(' ') != -1) strPluginId.RemoveAll(' ');
...
Handle(PCDM_StorageDriver) CDF_Application::WriterFromFormat(const TCollection_ExtendedString& theFormat) {
    ...
    if (myWriters.FindFromKey(theFormat, aDriver)) return aDriver;
...
Handle(CDF_MetaDataDriver) CDF_Application::MetaDataDriver() const {
    Standard_NoSuchObject_Raise_if(
        myMetaDataDriver.IsNull(),
        "no metadatadriver has been provided; this application is not able to store or retrieve files.");
