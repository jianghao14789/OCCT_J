...
static void CAUGHT(const Standard_Failure& theException, TCollection_ExtendedString& status,
            ...
            try {
                ...
                if (anApp.IsNull()) {
                    ...
                    if (aDocumentStorageDriver.IsNull()) {
                        ...
                        // It has sense in multi-threaded access to the storage driver - this way we reset the status
                        // for each call.
                        ...
                        if (!theMetaDataDriver->FindFolder(theDocument->RequestedFolder())) {
                            ...
                            for (; it.More(); it.Next())
                                theMetaDataDriver->CreateReference(aMetaData, it.Document()->MetaData(),
                                                                   it.ReferenceIdentifier(), it.DocumentVersion());
                        }
                    }
                }
