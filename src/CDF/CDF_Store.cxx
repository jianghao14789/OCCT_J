...
CDF_Store::CDF_Store()
    : myHasSubComponents(Standard_False), myIsMainDocument(Standard_False), myStatus(PCDM_SS_No_Obj) {}
...
#else
        if (theFolder.Value(l) == '/' || theFolder.Value(l) == '\\') theFolder.Trunc(l - 1);
#endif
    }
...
CDF_StoreSetNameStatus CDF_Store::SetName(const TCollection_ExtendedString& aName) {
    ...
    if (myCurrentDocument->IsStored()) {
        ...
        if (E->Folder() == myCurrentDocument->RequestedFolder() && E->Name() == theName) return CDF_SSNS_OK;
    }
...
Handle(TCollection_HExtendedString) CDF_Store::MetaDataPath() const {
    if (myCurrentDocument->IsStored()) return new TCollection_HExtendedString(myCurrentDocument->MetaData()->Path());
    return blank;
}
Handle(TCollection_HExtendedString) CDF_Store::Description() const {
    if (myMainDocument->FindDescription()) return new TCollection_HExtendedString(myMainDocument->Description());
    return blank;
}
...
Standard_Boolean CDF_Store::CurrentIsConsistent() const {
    if (!myCurrentDocument->IsStored()) return myCurrentDocument->HasRequestedFolder();
    return Standard_True;
}
...
Standard_Boolean CDF_Store::SetPreviousVersion(const Standard_ExtString aPreviousVersion) {
    if (theMetaDataDriver->HasVersionCapability()) {
        if (myCurrentDocument->HasRequestedFolder()) {
            if (theMetaDataDriver->Find(myCurrentDocument->RequestedFolder(), myCurrentDocument->RequestedName(),
                                        aPreviousVersion)) {
