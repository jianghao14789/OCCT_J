...
Standard_Boolean ChFiKPart_MakeChamfer(TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_SurfData) & Data,
                                       const ChFiDS_ChamfMode theMode, const gp_Pln& Pl1, const gp_Pln& Pl2,
                                       const TopAbs_Orientation Or1, const TopAbs_Orientation Or2,
                                       const Standard_Real theDis1, const Standard_Real theDis2, const gp_Lin& Spine,
                                       const Standard_Real First, const TopAbs_Orientation Of1) {
    ...
    if (VecTransl1.Dot(D2) <= 0.) VecTransl1.Reverse();

    gp_Dir VecTransl2 = LinAx1.Crossed(D2);
    if (VecTransl2.Dot(D1) <= 0.) VecTransl2.Reverse();
    ...
    gp_Pnt P1(P.X() + Dis1 * VecTransl1.X(), P.Y() + Dis1 * VecTransl1.Y(), P.Z() + Dis1 * VecTransl1.Z());

    // Point on the plane Pl2 and on the chamfer
    gp_Pnt P2(P.X() + Dis2 * VecTransl2.X(), P.Y() + Dis2 * VecTransl2.Y(), P.Z() + Dis2 * VecTransl2.Z());
    ...
    gp_Pnt P22(Pp.X() + Dis2 * VecTransl2.X(), Pp.Y() + Dis2 * VecTransl2.Y(), Pp.Z() + Dis2 * VecTransl2.Z());
    ...
    Data->ChangeInterferenceOnS1().SetInterference(ChFiKPart_IndexCurveInDS(GLinPln1, DStr), trans, GLin2dPln1,
                                                   GLin2dPlnCh1);
    ...
    Data->ChangeInterferenceOnS2().SetInterference(ChFiKPart_IndexCurveInDS(GLinPln2, DStr), trans, GLin2dPln2,
                                                   GLin2dPlnCh2);
