...
void HLRTopoBRep_DSFiller::Insert(const TopoDS_Shape& S, Contap_Contour& FO, HLRTopoBRep_Data& DS,
    ...
    while (ex.More()) {
        if (ShapeMap.Add(ex.Current())) {
            ...
            if (FO.IsDone()) {
                if (!FO.IsEmpty()) InsertFace(f, S1, FO, DS, withPCurve);
            }
...
void HLRTopoBRep_DSFiller::InsertFace(const Standard_Integer /*FI*/, const TopoDS_Face& F, Contap_Contour& FO,
                                      HLRTopoBRep_Data& DS, const Standard_Boolean withPCurve) {
    ...
    */
                            ...
                            case Contap_Walking: {
                                ...
                                */
                                    ...
                                    if (!Approx.IsDone()) {
                                        ...
                                        C = new Geom_BSplineCurve(poles3d, AppVal.Knots(), AppVal.Multiplicities(),
                                                                  AppVal.Degree());
                                        ...
                                        C2d = new Geom2d_BSplineCurve(poles2d, AppVal2.Knots(),
                                                                      AppVal2.Multiplicities(), AppVal2.Degree());
    ...
    for (; itl1.More(); itl1.Next()) {
        ...
        for (; itl2.More(); itl2.Next()) {
            ...
            if (anIntLine2.IsSame(anIntLine)) continue;
            TopoDS_Vertex aVer2[2];
            TopExp::Vertices(anIntLine2, aVer2[0], aVer2[1]);
            for (Standard_Integer i = 0; i < 2; i++) {
                if (i == 1 && aVer[0].IsSame(aVer[1])) continue;
                gp_Pnt Pnt1 = BRep_Tool::Pnt(aVer[i]);
                for (Standard_Integer j = 0; j < 2; j++) {
                    if (aVer[i].IsSame(aVer2[j])) continue;
...
TopoDS_Vertex HLRTopoBRep_DSFiller::MakeVertex(const Contap_Point& P, const Standard_Real tol, HLRTopoBRep_Data& DS) {
    BRep_Builder B;
    TopoDS_Vertex V;
    if (P.IsVertex()) {
...
void HLRTopoBRep_DSFiller::InsertVertex(const Contap_Point& P, const Standard_Real tol, const TopoDS_Edge& E,
                                        HLRTopoBRep_Data& DS) {
    BRep_Builder B;
    TopoDS_Vertex V;
