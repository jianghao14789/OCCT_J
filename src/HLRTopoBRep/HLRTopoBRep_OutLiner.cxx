...
HLRTopoBRep_OutLiner::HLRTopoBRep_OutLiner(const TopoDS_Shape& OriS) : myOriginalShape(OriS) {}
...
HLRTopoBRep_OutLiner::HLRTopoBRep_OutLiner(const TopoDS_Shape& OriS, const TopoDS_Shape& OutS)
    : myOriginalShape(OriS), myOutLinedShape(OutS) {}
...
void HLRTopoBRep_OutLiner::Fill(const HLRAlgo_Projector& P, BRepTopAdaptor_MapOfShapeTool& MST,
...
void HLRTopoBRep_OutLiner::ProcessFace(const TopoDS_Face& F, TopoDS_Shape& S, BRepTopAdaptor_MapOfShapeTool& MST) {
    BRep_Builder B;
...
void HLRTopoBRep_OutLiner::BuildShape(BRepTopAdaptor_MapOfShapeTool& MST) {
    ...
    for (exshell.Init(myOriginalShape, TopAbs_SHELL); exshell.More();
        ...
        for (exface.Init(exshell.Current(), TopAbs_FACE); exface.More(); exface.Next()) {
            if (ShapeMap.Add(exface.Current())) ProcessFace(TopoDS::Face(exface.Current()), theShell, MST);
        }
        B.Add(myOutLinedShape, theShell);
    }

    for (exface.Init(myOriginalShape, TopAbs_FACE, TopAbs_SHELL); exface.More();
         exface.Next()) { // faces not in a shell
        if (ShapeMap.Add(exface.Current())) ProcessFace(TopoDS::Face(exface.Current()), myOutLinedShape, MST);
    }

    for (exedge.Init(myOriginalShape, TopAbs_EDGE, TopAbs_FACE); exedge.More(); exedge.Next()) // edges not in a face
