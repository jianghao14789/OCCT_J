...
void IGESDefs_ToolUnitsData::ReadOwnParams(const Handle(IGESDefs_UnitsData) & ent,
    ...
    if (!unitTypes.IsNull())
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadText(PR.Current(), "Type of Unit", unitType)) unitTypes->SetValue(i, unitType);

            // st = PR.ReadText(PR.Current(), "Value of Unit", unitValue); //szv#4:S4163:12Mar99 moved in if
            if (PR.ReadText(PR.Current(), "Value of Unit", unitValue)) unitValues->SetValue(i, unitValue);

            // st = PR.ReadReal(PR.Current(), "Scale of Unit", unitScale); //szv#4:S4163:12Mar99 moved in if
            if (PR.ReadReal(PR.Current(), "Scale of Unit", unitScale)) unitScales->SetValue(i, unitScale);
        }
...
void IGESDefs_ToolUnitsData::WriteOwnParams(const Handle(IGESDefs_UnitsData) & ent, IGESData_IGESWriter& IW) const {
...
void IGESDefs_ToolUnitsData::OwnCopy(const Handle(IGESDefs_UnitsData) & another, const Handle(IGESDefs_UnitsData) & ent,
    ...
    for (Standard_Integer i = 1; i <= nbval; i++) {
        ...
        Handle(TCollection_HAsciiString) unitValue = new TCollection_HAsciiString(another->UnitValue(i));
...
IGESData_DirChecker IGESDefs_ToolUnitsData::DirChecker(const Handle(IGESDefs_UnitsData) & /* ent */) const {
...
void IGESDefs_ToolUnitsData::OwnCheck(const Handle(IGESDefs_UnitsData) & /* ent */, const Interface_ShareTool&,
                                      Handle(Interface_Check) & /* ach */) const {}

void IGESDefs_ToolUnitsData::OwnDump(const Handle(IGESDefs_UnitsData) & ent, const IGESData_IGESDumper& /* dumper */,
