...
void IGESDefs_ToolAssociativityDef::ReadOwnParams(const Handle(IGESDefs_AssociativityDef) & ent,
    ...
    if (!requirements.IsNull())
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            // st = PR.ReadInteger(PR.Current(), "Back Pointer Requirement", requirement); //szv#4:S4163:12Mar99 moved
            // in if
            ...
            if (PR.ReadInteger(PR.Current(), "Ordered/Unordered Class", order)) orders->SetValue(i, order);
            ...
            if (PR.ReadInteger(PR.Current(), "No. of items per entry", numItem)) {
                ...
                for (Standard_Integer j = 1; j <= numItem; j++) {
                    ...
                    if (PR.ReadInteger(PR.Current(), "Item", temp)) item->SetValue(j, temp);
                }
...
IGESData_DirChecker
IGESDefs_ToolAssociativityDef::DirChecker(const Handle(IGESDefs_AssociativityDef) & /* ent */) const {
