...
void IGESDefs_ToolAttributeTable::ReadOwnParams(const Handle(IGESDefs_AttributeTable) & ent,
    ...
    if (ent->FormNumber() == 1) st = PR.ReadInteger(PR.Current(), "No. of rows", nr);
            ...
            switch (atype) {
                ...
                case 1: {
                    ...
                    for (j = 1; j <= avc; j++) {
                        ...
                        if (PR.ReadInteger(PR.Current(), "Value", item)) attrInt->SetValue(j, item);
                    }
                } break;
                case 2: {
                    ...
                    for (j = 1; j <= avc; j++) {
                        ...
                        if (PR.ReadReal(PR.Current(), "Value", item)) attrReal->SetValue(j, item);
                    }
                } break;
                case 3: {
                    ...
                    for (j = 1; j <= avc; j++) {
                        ...
                        if (PR.ReadText(PR.Current(), "Value", item)) attrStr->SetValue(j, item);
                    }
                } break;
                case 4: {
                    ...
                    for (j = 1; j <= avc; j++) {
                        ...
                        if (PR.ReadEntity(IR, PR.Current(), "Value", item)) attrEnt->SetValue(j, item);
                    }
                } break;
                case 5:
                    for (j = 1; j <= avc; j++)
        ...
        for (Standard_Integer i = 1; i <= na; i++) {
            ...
            switch (atype) {
                ...
                case 4: {
                    ...
                    list2->SetValue(i, k, attrEnt);
                    for (j = 1; j <= avc; j++)
                        attrEnt->SetValue(j, GetCasted(IGESData_IGESEntity, TC.Transferred(otherEnt->Value(j))));
                } break;
...
IGESData_DirChecker IGESDefs_ToolAttributeTable::DirChecker(const Handle(IGESDefs_AttributeTable) & /* ent */) const {
...
void IGESDefs_ToolAttributeTable::OwnCheck(const Handle(IGESDefs_AttributeTable) & ent, const Interface_ShareTool&,
    ...
    if (ent->FormNumber() == 0 && ent->NbRows() != 1) ach->AddFail("Form 0 with several Rows");
