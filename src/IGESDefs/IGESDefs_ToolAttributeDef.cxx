...
void IGESDefs_ToolAttributeDef::ReadOwnParams(const Handle(IGESDefs_AttributeDef) & ent,
    ...
    if (st && nbval > 0) {
        ...
        if (fn > 0) attrValues = new TColStd_HArray1OfTransient(1, nbval);
        if (fn > 1) attrValuePointers = new IGESDefs_HArray1OfHArray1OfTextDisplayTemplate(1, nbval);
    } else
    ...
    if (!attrTypes.IsNull())
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadInteger(PR.Current(), "Attribute Type", attrType)) attrTypes->SetValue(i, attrType);
            ...
            if (st) {
                ...
                if (fn > 1) attrValuePointer = new IGESGraph_HArray1OfTextDisplayTemplate(1, avc);
            }
            ...
            if (!attrValues.IsNull())
                if (fn > 0) {
                    ...
                    for (Standard_Integer j = 1; j <= avc; j++) {
                        switch (attrValueDataType) {
                            ...
                            case 1: {
                                ...
                                // moved in if
                                if (PR.ReadInteger(PR.Current(), "Attribute Value", temp)) attrInt->SetValue(j, temp);
                            } break;
                            case 2: {
                                Standard_Real temp;
                                // st = PR.ReadReal(PR.Current(), "Attribute Value", temp); //szv#4:S4163:12Mar99 moved
                                // in if
                                if (PR.ReadReal(PR.Current(), "Attribute Value", temp)) attrReal->SetValue(j, temp);
                            } break;
                            case 3: {
                                Handle(TCollection_HAsciiString) temp;
                                // st = PR.ReadText(PR.Current(), "Attribute Value", temp); //szv#4:S4163:12Mar99 moved
                                // in if
                                if (PR.ReadText(PR.Current(), "Attribute Value", temp)) attrStr->SetValue(j, temp);
                            } break;
                            case 4: {
                                Handle(IGESData_IGESEntity) temp;
                                // st = PR.ReadEntity(IR, PR.Current(), "Attribute Value", temp); //szv#4:S4163:12Mar99
                                // moved in if
                        ...
                        if (fn == 2) {
                            ...
                            // STANDARD_TYPE(IGESGraph_TextDisplayTemplate), tempText); //szv#4:S4163:12Mar99 moved in
                            // if
                    ...
                    if (fn == 2) attrValuePointers->SetValue(i, attrValuePointer);
                }
        }

    DirChecker(ent).CheckTypeAndForm(PR.CCheck(), ent);
    ent->Init(aName, aListType, attrTypes, attrValueDataTypes, attrValueCounts, attrValues, attrValuePointers);
}
    ...
    for (Standard_Integer i = 1; i <= upper; i++) {
        ...
        if (ent->FormNumber() > 0) {
            for (Standard_Integer j = 1; j <= count; j++) {
                ...
                if (ent->FormNumber() == 2) IW.Send(ent->AttributeTextDisplay(i, j));
            }
        }
    }
}
    ...
    for (Standard_Integer i = 1; i <= upper; i++) {
        ...
        if (ent->FormNumber() > 0) {
            for (Standard_Integer j = 1; j <= count; j++) {
                ...
                if (ent->FormNumber() == 2) iter.GetOneItem(ent->AttributeTextDisplay(i, j));
            }
        }
    }
}
...
void IGESDefs_ToolAttributeDef::OwnCopy(const Handle(IGESDefs_AttributeDef) & another,
    ...
    Handle(TCollection_HAsciiString) aName;
    if (!another->TableName().IsNull()) aName = new TCollection_HAsciiString(another->TableName());
    ...
    attrValueDataTypes = new TColStd_HArray1OfInteger(1, nbval);
    attrValueCounts = new TColStd_HArray1OfInteger(1, nbval);
    if (another->HasValues()) attrValues = new TColStd_HArray1OfTransient(1, nbval);
    if (another->HasTextDisplay()) attrValuePointers = new IGESDefs_HArray1OfHArray1OfTextDisplayTemplate(1, nbval);
    ...
    for (Standard_Integer i = 1; i <= nbval; i++) {
        ...
        if (another->HasTextDisplay()) attrValuePointer = new IGESGraph_HArray1OfTextDisplayTemplate(1, avc);
        ...
        if (another->HasValues()) {
            ...
            for (Standard_Integer j = 1; j <= avc; j++) {
                switch (attrValueDataType) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4: {
                        DeclareAndCast(IGESData_IGESEntity, Ent, TC.Transferred(another->AttributeAsEntity(i, j)));
            ...
            if (another->HasTextDisplay()) attrValuePointers->SetValue(i, attrValuePointer);
        }
    }
    ent->Init(aName, aListType, attrTypes, attrValueDataTypes, attrValueCounts, attrValues, attrValuePointers);
}
...
void IGESDefs_ToolAttributeDef::OwnCheck(const Handle(IGESDefs_AttributeDef) & ent, const Interface_ShareTool&,
...
void IGESDefs_ToolAttributeDef::OwnDump(const Handle(IGESDefs_AttributeDef) & ent, const IGESData_IGESDumper& dumper,
