...
void IGESDefs_ToolMacroDef::ReadOwnParams(const Handle(IGESDefs_MacroDef) & ent,
    ...
    for (nbval = 0; PR.CurrentNumber() != PR.NbParams(); nbval++, PR.SetCurrentNumber(PR.CurrentNumber() + 1))
        ;
    ...
    if (!langStatements.IsNull()) {
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadText(PR.Current(), "Language Statement", langStat)) langStatements->SetValue(i, langStat);
        }
    }
...
void IGESDefs_ToolMacroDef::WriteOwnParams(const Handle(IGESDefs_MacroDef) & ent, IGESData_IGESWriter& IW) const {
    IW.Send(ent->MACRO());
...
void IGESDefs_ToolMacroDef::OwnCopy(const Handle(IGESDefs_MacroDef) & another, const Handle(IGESDefs_MacroDef) & ent,
    ...
    for (Standard_Integer i = 1; i <= nbval; i++) {
        Handle(TCollection_HAsciiString) langStat = new TCollection_HAsciiString(another->LanguageStatement(i));
        langStatements->SetValue(i, langStat);
    }
    ent->Init(macro, entityTypeID, langStatements, endMacro);
}

IGESData_DirChecker IGESDefs_ToolMacroDef::DirChecker(const Handle(IGESDefs_MacroDef) & /* ent */) const {
...
void IGESDefs_ToolMacroDef::OwnCheck(const Handle(IGESDefs_MacroDef) & /* ent */, const Interface_ShareTool&,
                                     Handle(Interface_Check) & /* ach */) const {}

void IGESDefs_ToolMacroDef::OwnDump(const Handle(IGESDefs_MacroDef) & ent, const IGESData_IGESDumper& /* dumper */,
