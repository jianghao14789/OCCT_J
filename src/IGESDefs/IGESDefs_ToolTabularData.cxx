...
void IGESDefs_ToolTabularData::ReadOwnParams(const Handle(IGESDefs_TabularData) & ent,
    ...
    if (st && nbDeps > 0) valuesDep = new IGESBasic_HArray1OfHArray1OfReal(1, nbDeps);
    ...
    PR.ReadInts(PR.CurrentList(nbIndeps), "Type of independent variables",
                typesInd); // szv#4:S4163:12Mar99 `st=` not needed

    PR.ReadInts(PR.CurrentList(nbIndeps), "No. of values of independent variables",
                nbValuesInd); // szv#4:S4163:12Mar99 `st=` not needed
...
void IGESDefs_ToolTabularData::WriteOwnParams(const Handle(IGESDefs_TabularData) & ent, IGESData_IGESWriter& IW) const {
...
void IGESDefs_ToolTabularData::OwnCopy(const Handle(IGESDefs_TabularData) & another,
    ...
    Handle(IGESBasic_HArray1OfHArray1OfReal) valuesInd = new IGESBasic_HArray1OfHArray1OfReal(1, nbIndeps);
    Handle(IGESBasic_HArray1OfHArray1OfReal) valuesDep = new IGESBasic_HArray1OfHArray1OfReal(1, nbDeps);
    Standard_Integer i;
    for (i = 1; i <= nbIndeps; i++) {
...
IGESData_DirChecker IGESDefs_ToolTabularData::DirChecker(const Handle(IGESDefs_TabularData) & /* ent */) const {
...
void IGESDefs_ToolTabularData::OwnCheck(const Handle(IGESDefs_TabularData) & /* ent */, const Interface_ShareTool&,
