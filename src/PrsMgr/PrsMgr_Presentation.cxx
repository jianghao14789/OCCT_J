...
namespace {
enum BeforeHighlightState { State_Empty, State_Hidden, State_Visible };

static BeforeHighlightState StructureState(const Graphic3d_Structure* theStructure) {
    return !theStructure->IsDisplayed() ? State_Empty : !theStructure->IsVisible() ? State_Hidden : State_Visible;
}
} // namespace
...
PrsMgr_Presentation::PrsMgr_Presentation(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
    ...
    : Graphic3d_Structure(thePrsMgr->StructureManager()), myPresentationManager(thePrsMgr),
      myPresentableObject(thePrsObject.get()), myBeforeHighlightState(State_Empty), myMode(theMode),
...
void PrsMgr_Presentation::Compute() {
    ...
    for (PrsMgr_Presentations::Iterator aPrsIter(myPresentableObject->myPresentations); aPrsIter.More();
         aPrsIter.Next()) {
