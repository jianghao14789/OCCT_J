...
Standard_DEPRECATED("Deprecated alias to PrsMgr_PresentationManager") typedef PrsMgr_PresentationManager
    PrsMgr_PresentationManager3d;

//! A framework to supply the Graphic3d structure of the object to be presented.
//! On the first display request, this structure is created by calling the appropriate algorithm and retaining this
//! framework for further display. This abstract framework is inherited in Application Interactive Services (AIS),
//! notably by AIS_InteractiveObject. Consequently, 3D presentation should be handled by the relevant daughter classes
//! and their member functions in AIS. This is particularly true in the creation of new interactive objects.
//!
...
//!
//! Warning! Methods managing standard attributes (SetColor(), SetWidth(), SetMaterial()) have different meaning for
//! objects of different type (or no meaning at all). Sub-classes might override these methods to modify Prs3d_Drawer or
//! class properties providing a convenient short-cut depending on application needs. For more sophisticated
//! configuring, Prs3d_Drawer should be modified directly, while short-cuts might be left unimplemented.
...
public:
    ...
    //! Returns true if the Interactive Object has display mode setting overriding global setting (within Interactive
    //! Context).
    Standard_Boolean HasDisplayMode() const {
        return myDrawer->DisplayMode() != -1;
    }

    //! Returns the display mode setting of the Interactive Object.
    //! The range of supported display mode indexes should be specified within object definition and filtered by
    //! AccepDisplayMode().
    //! @sa AcceptDisplayMode()
    Standard_Integer DisplayMode() const {
        return myDrawer->DisplayMode();
    }

    //! Sets the display mode for the interactive object.
    //! An object can have its own temporary display mode, which is different from that proposed by the interactive
    //! context.
    //! @sa AcceptDisplayMode()
    ...
    //! Consequently, this virtual method allowing us to get information about the class in question must be
    //! implemented. At least one display mode index should be accepted by this method. Although subclass can leave
    //! default implementation, it is highly desired defining exact list of supported modes instead, which is usually an
    //! enumeration for one object or objects class sharing similar list of display modes.
...
public: //! @name presentation attributes
    ...
    //! When not NULL, overrides both Prs3d_TypeOfHighlight_LocalSelected and Prs3d_TypeOfHighlight_Selected defined
    //! within AIS_InteractiveContext::HighlightStyle().
    ...
    //! Returns the hilight attributes settings.
    //! When not NULL, overrides both Prs3d_TypeOfHighlight_LocalDynamic and Prs3d_TypeOfHighlight_Dynamic defined
    //! within AIS_InteractiveContext::HighlightStyle().
    //! @sa AIS_InteractiveContext::HighlightStyle()
    const Handle(Prs3d_Drawer) & DynamicHilightAttributes() const {
        return myDynHilightDrawer;
    }
    ...
    //! so that modifications will take effect on already computed presentation groups (thus avoiding re-displaying the
    //! object).
    Standard_EXPORT void SynchronizeAspects();

public: //! @name object transformation
    //! Returns Transformation Persistence defining a special Local Coordinate system where this presentable object is
    //! located or NULL handle if not defined. Position of the object having Transformation Persistence is mutable and
    //! depends on camera position. The same applies to a bounding box of the object.
...
public: //! @name parent/children properties
    ...
    //! Removes theObject from children of current object in scene hierarchy with keeping the current global
    ...
    //! This method requires presentation to be already computed, since it relies on bounding box of presentation
    //! structures, which are supposed to be same/close amongst different display modes of this object.
...
protected: //! @name interface methods
    ...
    Standard_EXPORT virtual void Fill(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
                                      const Handle(PrsMgr_Presentation) & thePrs, const Standard_Integer theMode);
    ...
    Standard_EXPORT virtual void Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
                                         const Handle(Prs3d_Presentation) & thePrs, const Standard_Integer theMode) = 0;
    ...
    //! It is NOT recommended approach, because user has to fill such map and then search for each occurrence in
    //! computed groups. The recommended approach is computing presentation with necessary customized aspects, and then
    //! modify them directly followed by SynchronizeAspects() call.
...
public: //! @name simplified presentation properties API
    ...
    virtual void Color(Quantity_Color& theColor) const {
        theColor = myDrawer->Color();
    }

    //! Only the interactive object knowns which Drawer attribute is affected by the color, if any
    //! (ex: for a wire,it's the wireaspect field of the drawer, but for a vertex, only the point aspect field is
    //! affected by the color). WARNING : Do not forget to set the corresponding fields here (hasOwnColor and
    //! myDrawer->SetColor())
    ...
    Standard_Boolean IsTransparent() const {
        return myDrawer->Transparency() > 0.005f;
    }
    ...
    Standard_EXPORT virtual void PolygonOffsets(Standard_Integer& aMode, Standard_ShortReal& aFactor,
                                                Standard_ShortReal& aUnits) const;

    //! Sets up polygon offsets for this object.
    //! @sa Graphic3d_Aspects::SetPolygonOffsets()
    Standard_EXPORT virtual void SetPolygonOffsets(const Standard_Integer aMode, const Standard_ShortReal aFactor = 1.0,
...
public: //! @name deprecated methods
    ...
    Standard_DEPRECATED("This method is deprecated - UpdatePresentations() should be called instead") Standard_EXPORT
    ...
    Standard_Boolean ToPropagateVisualState() const {
        return myToPropagateVisualState;
    }
...
protected:
    ...
    Standard_DEPRECATED("This method is deprecated - SetToUpdate() + UpdatePresentations() should be called "
                        "instead") void Update(Standard_Boolean theToIncludeHidden = Standard_False) {
        SetToUpdate();
...
protected:
    ...
    Handle(Prs3d_Drawer)
        myHilightDrawer; //!< (optional) custom presentation attributes for highlighting selected object
    Handle(Prs3d_Drawer)
        myDynHilightDrawer; //!< (optional) custom presentation attributes for highlighting detected object
    ...
    Handle(TopLoc_Datum3D)
        myTransformation; //!< absolute transformation of this object (combined parents + local transformations)
    ...
    Standard_Boolean myToPropagateVisualState; //!< flag indicating if visual state (display/erase/color) should be
