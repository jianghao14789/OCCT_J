...
TDocStd_Document::TDocStd_Document(const TCollection_ExtendedString& aStorageFormat)
    : myStorageFormat(aStorageFormat), myData(new TDF_Data()), myUndoLimit(0), myUndoTransaction("UNDO"), mySaveTime(0),
      myIsNestedTransactionMode(0), mySaveEmptyLabels(Standard_False),
...
void TDocStd_Document::Update(const Handle(CDM_Document) & /*aToDocument*/, const Standard_Integer aReferenceIdentifier,
...
Standard_Boolean TDocStd_Document::CommitTransaction() {
    ...
    if (myIsNestedTransactionMode && myUndoTransaction.IsOpen()) {
        ...
        Handle(TDocStd_CompoundDelta) aCompDelta = Handle(TDocStd_CompoundDelta)::DownCast(myUndoFILO.First());
        ...
        if (myOnlyTransactionModification) {
            myData->AllowModification(myUndoTransaction.IsOpen() && myUndoLimit ? Standard_True : Standard_False);
        }
    ...
    if (isDone && IsOpened()) {
        const Handle(TDocStd_Application) anAppli = Handle(TDocStd_Application)::DownCast(Application());
        if (!anAppli.IsNull()) anAppli->OnCommitTransaction(this);
    }
    return isDone;
}
...
void TDocStd_Document::AbortTransaction() {
    ...
    if (myUndoTransaction.IsOpen())
        if (myUndoLimit != 0) myUndoTransaction.Abort();

    if (myIsNestedTransactionMode && myUndoFILO.Extent()) {
        if (!myUndoFILO.First()->IsEmpty()) myData->Undo(myUndoFILO.First(), Standard_True);
        myUndoFILO.RemoveFirst();
        if (myUndoFILO.Extent()) myUndoTransaction.Open();
    }
    // deny or allow modifications according to transaction state
    if (myOnlyTransactionModification) {
        myData->AllowModification(myUndoTransaction.IsOpen() && myUndoLimit ? Standard_True : Standard_False);
    ...
    if (IsOpened()) {
        const Handle(TDocStd_Application) anAppli = Handle(TDocStd_Application)::DownCast(Application());
        if (!anAppli.IsNull()) anAppli->OnAbortTransaction(this);
    }
}
...
void TDocStd_Document::OpenTransaction() {
    ...
    if (myIsNestedTransactionMode) {
        ...
        if (myUndoFILO.Extent()) aLastTime = myUndoFILO.First()->EndTime();
    ...
    if (IsOpened()) {
        const Handle(TDocStd_Application) anAppli = Handle(TDocStd_Application)::DownCast(Application());
        if (!anAppli.IsNull()) anAppli->OnOpenTransaction(this);
    }
}
...
void TDocStd_Document::BeforeClose() {
    ...
    AbortTransaction();
    if (myIsNestedTransactionMode) myUndoFILO.Clear();
    ClearUndos();
}
