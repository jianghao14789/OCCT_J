...
Standard_Boolean TDocStd_MultiTransactionManager::CommitCommand() {
    ...
    for (i = myDocuments.Length(); i > 0; i--) {
        ...
        while (myDocuments.Value(i)->HasOpenCommand())
            if (myDocuments.Value(i)->CommitCommand()) isCommited = Standard_True;
        if (isCommited) {
...
Standard_Boolean TDocStd_MultiTransactionManager::CommitCommand(const TCollection_ExtendedString& theName) {
    ...
    if (isCommited && myUndos.Length()) myUndos.First()->SetName(theName);
    return isCommited;
}
...
void TDocStd_MultiTransactionManager::AddDocument(const Handle(TDocStd_Document) & theDoc) {
    Standard_Integer i;
    for (i = myDocuments.Length(); i > 0; i--)
        if (myDocuments.Value(i) == theDoc) return; // the document is already added to the list
    ...
    if (myOpenTransaction) {
        if (!theDoc->HasOpenCommand()) theDoc->OpenCommand();
    } else {
        if (theDoc->HasOpenCommand()) theDoc->CommitCommand();
    }
    theDoc->ClearUndos();
    theDoc->ClearRedos();
}
...
void TDocStd_MultiTransactionManager::RemoveDocument(const Handle(TDocStd_Document) & theDoc) {
    Standard_Integer i;
    for (i = myDocuments.Length(); i > 0; i--) {
        if (myDocuments.Value(i) == theDoc) myDocuments.Remove(i);
    }
    for (i = myUndos.Length(); i > 0; i--) {
        Handle(TDocStd_ApplicationDelta) delta = myUndos.Value(i);
        ...
        for (Standard_Integer j = docs.Length(); j > 0; j--) {
            if (docs.Value(j) == theDoc) {
                docs.Remove(j);
                if (docs.Length() == 0) myUndos.Remove(i);
            }
        }
    }
    for (i = myRedos.Length(); i > 0; i--) {
        ...
        for (Standard_Integer j = docs.Length(); j > 0; j--) {
            if (docs.Value(j) == theDoc) {
                docs.Remove(j);
                if (docs.Length() == 0) myRedos.Remove(i);
            }
        }
    }
}
