...
//! This enumeration lists all versions of an OCAF document. TDocStd_FormatVersion_CURRENT value refers to the last file
//! format version. By default, Open CASCADE Technology writes new documents using the last file format version. The
//! last version of Open CASCADE Technology is able to read old documents of any version. However, a previous version of
//! Open CASCADE Technology may not be able to read a new document. In this case use the method
//! ChangeStorageFormatVersion() from TDocStd_Document to change the file format version. Then, save the document by
//! means of SaveAs() from TDocStd_Application.
//!
//! If it is necessary to improve an XML or binary file format of OCAF document, follow please the next steps:
//! - increment the file format version in this enumeration. Put a reference to the last file format version by means of
