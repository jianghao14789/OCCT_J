...
//! Also the trace level of messages can be tuned by setting trace level (SetTraceLevel (Gravity )) for the used
//! Printer. By default, trace level is Message_Info, so that all messages are output.
...
public:
    ...
    Standard_EXPORT PCDM_ReaderStatus Open(const TCollection_ExtendedString& thePath, Handle(TDocStd_Document) & theDoc,
    ...
    //! @param[in]  theRange optional progress indicator
    //! @return reading status
    PCDM_ReaderStatus Open(const TCollection_ExtendedString& thePath, Handle(TDocStd_Document) & theDoc,
                           const Message_ProgressRange& theRange = Message_ProgressRange()) {
