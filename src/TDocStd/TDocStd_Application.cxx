...
// purpose  :
//=======================================================================
TDocStd_Application::TDocStd_Application() : myIsDriverLoaded(Standard_True) {
    if (myMetaDataDriver.IsNull()) myIsDriverLoaded = Standard_False;
}
...
void TDocStd_Application::DefineFormat(const TCollection_AsciiString& theFormat,
    ...
    if (!theReader.IsNull()) theReader->SetFormat(theFormat);
    if (!theWriter.IsNull()) theWriter->SetFormat(theFormat);
...
void TDocStd_Application::ReadingFormats(TColStd_SequenceOfAsciiString& theFormats) {
    ...
    NCollection_IndexedDataMap<TCollection_ExtendedString, Handle(PCDM_RetrievalDriver)>::Iterator anIter(myReaders);
...
void TDocStd_Application::WritingFormats(TColStd_SequenceOfAsciiString& theFormats) {
    ...
    NCollection_IndexedDataMap<TCollection_ExtendedString, Handle(PCDM_StorageDriver)>::Iterator anIter(myWriters);
    for (; anIter.More(); anIter.Next()) {
...
Standard_Integer TDocStd_Application::IsInSession(const TCollection_ExtendedString& path) const {
    ...
    for (Standard_Integer i = 1; i <= nbdoc; i++) {
        ...
        if (D->IsSaved()) {
            ...
            if (unifiedPath == unifiedDocPath) return i;
        }
    }
    return 0;
}
...
PCDM_ReaderStatus TDocStd_Application::Open(const TCollection_ExtendedString& path, Handle(TDocStd_Document) & theDoc,
    ...
    try {
        ...
        if (theFilter.IsNull() || !theFilter->IsAppendMode()) CDF_Application::Open(D);
        theDoc = D;
...
PCDM_StoreStatus TDocStd_Application::SaveAs(const Handle(TDocStd_Document) & theDoc,
    ...
    if (!storer.SetFolder(directory)) {
        ...
        aMsg += directory;
        aMsg += " does not exist";
        if (!MessageDriver().IsNull()) MessageDriver()->Send(aMsg.ToExtString(), Message_Fail);
...
PCDM_StoreStatus TDocStd_Application::Save(const Handle(TDocStd_Document) & D, const Message_ProgressRange& theRange) {
    ...
    if (D->IsSaved()) {
        ...
        if (storer.StoreStatus() == PCDM_SS_OK) D->SetSaved();
