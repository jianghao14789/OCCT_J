...
static Standard_Integer IntersectionOfSets(const NCollection_List<Standard_Integer>& theSet1,
    ...
    for (NCollection_List<Standard_Integer>::Iterator anIt1 = theSet1.begin().Iterator(); anIt1.More(); anIt1.Next()) {
        const Standard_Integer aVal1 = anIt1.Value();
        for (NCollection_List<Standard_Integer>::Iterator anIt2 = theSet2.begin().Iterator(); anIt2.More();
             anIt2.Next()) {
            const Standard_Integer aVal2 = anIt2.Value();
            if (aVal1 == aVal2) {
                // theIntersectionResult.Append(aVal1);
                if (aVal1 < aRetVal) aRetVal = aVal1;
            }
        }
    }

    if (aRetVal == anIntMax) return -1;
...
static Handle(Geom2d_Curve)
    Get2DCurve(const Standard_Integer theIndex, const Standard_Real theUfirst, const Standard_Real theUlast,
    ...
    if (!theIsReverse) {
        switch (theIndex) {
            case 0:
                a2dCurv = new Geom2d_TrimmedCurve(new Geom2d_Line(gp_Pnt2d(0.0, theVfirst), gp_Dir2d(1.0, 0.0)),
                                                  theUfirst, theUlast);
                break;
            case 1:
                a2dCurv = new Geom2d_TrimmedCurve(new Geom2d_Line(gp_Pnt2d(theUlast, 0.0), gp_Dir2d(0.0, 1.0)),
                                                  theVfirst, theVlast);
                break;
            case 2:
                a2dCurv = new Geom2d_TrimmedCurve(new Geom2d_Line(gp_Pnt2d(0.0, theVlast), gp_Dir2d(1.0, 0.0)),
                                                  theUfirst, theUlast);
                break;
            case 3:
                a2dCurv = new Geom2d_TrimmedCurve(new Geom2d_Line(gp_Pnt2d(theUfirst, 0.0), gp_Dir2d(0.0, 1.0)),
                                                  theVfirst, theVlast);
                break;
            default:
                break;
        ...
        switch (theIndex) {
            case 0:
                a2dCurv = new Geom2d_TrimmedCurve(
                    new Geom2d_Line(gp_Pnt2d(theUfirst + theUlast, theVfirst), gp_Dir2d(-1.0, 0.0)), theUfirst,
                    theUlast);
                break;
            case 1:
                a2dCurv = new Geom2d_TrimmedCurve(
                    new Geom2d_Line(gp_Pnt2d(theUlast, theVfirst + theVlast), gp_Dir2d(0.0, -1.0)), theVfirst,
                    theVlast);
                break;
            case 2:
                a2dCurv = new Geom2d_TrimmedCurve(
                    new Geom2d_Line(gp_Pnt2d(theUfirst + theUlast, theVlast), gp_Dir2d(-1.0, 0.0)), theUfirst,
                    theUlast);
                break;
            case 3:
                a2dCurv = new Geom2d_TrimmedCurve(
                    new Geom2d_Line(gp_Pnt2d(theUfirst, theVfirst + theVlast), gp_Dir2d(0.0, -1.0)), theVfirst,
                    theVlast);
                break;
            default:
                break;
...
BRepBuilderAPI_FastSewing::BRepBuilderAPI_FastSewing(const Standard_Real theTol)
    : myTolerance(theTol), myStatusList(0) {}
    ...
    try {
        {
            ...
            NCollection_CellFilter<NodeInspector> aCells(Max(myTolerance, aRange / IntegerLast()), anAlloc);
...
void BRepBuilderAPI_FastSewing::UpdateEdgeInfo(const Standard_Integer theIDPrevVertex,
                                               const Standard_Integer theIDCurrVertex, const Standard_Integer theFaceID,
                                               const Standard_Integer theIDCurvOnFace) {
    // Indeed, two vertices combine into one edge only.
    const Standard_Integer anEdgeID =
        IntersectionOfSets(myVertexVec.Value(theIDPrevVertex).myEdges, myVertexVec.Value(theIDCurrVertex).myEdges);
...
void BRepBuilderAPI_FastSewing::CreateNewEdge(const Standard_Integer theIDPrevVertex,
    ...
    if (theIDPrevVertex == theIDCurrVertex) { // the Edge is degenerated
...
void BRepBuilderAPI_FastSewing::FindVertexes(const Standard_Integer theSurfID,
    ...
    const gp_Pnt aPnts[aNbPoints] = {aSurf->Value(aUf, aVf), aSurf->Value(aUl, aVf), aSurf->Value(aUl, aVl),
                                     aSurf->Value(aUf, aVl)};
...
void BRepBuilderAPI_FastSewing::FindEdges(const Standard_Integer theSurfID) {
    ...
    for (Standard_Integer i = 0; i < aNbPoints; i++) {
        ...
        if (aFirstVertID == aLastVertID) { // Edge is degenerated.
        ...
        const Standard_Integer anIntRes =
            IntersectionOfSets(myVertexVec.Value(aFirstVertID).myFaces, myVertexVec.Value(aLastVertID).myFaces);

        if ((anIntRes < 0) || (anIntRes >= theSurfID)) {
            CreateNewEdge(aFirstVertID, aLastVertID, theSurfID, i);
        } else { // if(theSurfID > anIntRes) => The edge has been processed earlier
...
// purpose  :
//=======================================================================
BRepBuilderAPI_FastSewing::FS_VARStatuses BRepBuilderAPI_FastSewing::GetStatuses(Standard_OStream* const theOS) {
    if (!theOS) return myStatusList;
    ...
    for (Standard_Integer i = 1; i <= aNumMax; i++, anIDS = static_cast<FS_Statuses>(anIDS << 1)) {
        if ((anIDS & myStatusList) == 0) continue;
...
BRepBuilderAPI_FastSewing::NodeInspector::NodeInspector(const NCollection_Vector<FS_Vertex>& theVec,
    ...
    : myVecOfVertexes(theVec), myPoint(thePnt), myResID(-1), myIsFindingEnable(Standard_False) {
...
void BRepBuilderAPI_FastSewing::FS_Face::CreateTopologicalWire(const NCollection_Vector<FS_Edge>& theEdgeVec,
    ...
    for (Standard_Integer anEdge = 0; anEdge < 4; anEdge++) {
        Standard_ProgramError_Raise_if(
            myEdges[anEdge] < 0, "BRepBuilderAPI_FastSewing::FS_Face::CreateTopologicalWire: Wire is not closed.");
