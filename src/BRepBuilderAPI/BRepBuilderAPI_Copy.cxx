...
public:
    BRepBuilderAPI_Copy_Modification(const Standard_Boolean copyGeom, const Standard_Boolean copyMesh)
        : myCopyGeom(copyGeom), myCopyMesh(copyMesh) {}

    //! Returns true to indicate the need to copy face;
    //! copies surface if requested
    Standard_Boolean NewSurface(const TopoDS_Face& F, Handle(Geom_Surface) & S, TopLoc_Location& L, Standard_Real& Tol,
                                Standard_Boolean& RevWires, Standard_Boolean& RevFace) Standard_OVERRIDE {
        S = BRep_Tool::Surface(F, L);
        Tol = BRep_Tool::Tolerance(F);
        RevWires = RevFace = Standard_False;

        if (!S.IsNull() && myCopyGeom) S = Handle(Geom_Surface)::DownCast(S->Copy());
    ...
    Standard_Boolean NewTriangulation(const TopoDS_Face& F, Handle(Poly_Triangulation) & T) Standard_OVERRIDE {
        if (!myCopyMesh) return Standard_False;
    ...
    Standard_Boolean NewCurve(const TopoDS_Edge& E, Handle(Geom_Curve) & C, TopLoc_Location& L,
        ...
        if (!C.IsNull() && myCopyGeom) C = Handle(Geom_Curve)::DownCast(C->Copy());
    ...
    Standard_Boolean NewPoint(const TopoDS_Vertex& V, gp_Pnt& P, Standard_Real& Tol) Standard_OVERRIDE {
        P = BRep_Tool::Pnt(V);
    ...
    Standard_Boolean NewCurve2d(const TopoDS_Edge& E, const TopoDS_Face& F, const TopoDS_Edge& /*NewE*/,
        ...
        if (!C.IsNull() && myCopyGeom) C = Handle(Geom2d_Curve)::DownCast(C->Copy());
    ...
    GeomAbs_Shape Continuity(const TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2, const TopoDS_Edge&,
