...
Handle(Geom2d_Curve) BRepBuilderAPI_Sewing::SameRange(const Handle(Geom2d_Curve) & CurvePtr,
    ...
    try {
        ...
        GeomLib::SameRange(Precision::PConfusion(), CurvePtr, FirstOnCurve, LastOnCurve, RequestedFirst, RequestedLast,
                           NewCurvePtr);
...
static Standard_Boolean IsClosedShape(const TopoDS_Shape& theshape, const TopoDS_Shape& v1, const TopoDS_Shape& v2) {
...
// purpose  :
//=======================================================================
static Standard_Boolean IsClosedByIsos(const Handle(Geom_Surface) & thesurf, const Handle(Geom2d_Curve) & acrv2d,
                                       const Standard_Real f2d, const Standard_Real l2d,
                                       const Standard_Boolean isUIsos) {
    Standard_Boolean isClosed = Standard_False;

    gp_Pnt2d psurf1 = (acrv2d->IsPeriodic() ? acrv2d->Value(f2d) : acrv2d->Value(Max(f2d, acrv2d->FirstParameter())));
    gp_Pnt2d psurf2 = (acrv2d->IsPeriodic() ? acrv2d->Value(l2d) : acrv2d->Value(Min(l2d, acrv2d->LastParameter())));
    ...
    aCrv2->D0(al2, p22);
    isClosed = (((p11.XYZ() - p12.XYZ()).Modulus() < (p11.XYZ() - p1m.XYZ()).Modulus() - Precision::Confusion()) &&
                ((p21.XYZ() - p22.XYZ()).Modulus() < (p21.XYZ() - p2m.XYZ()).Modulus() - Precision::Confusion()));
    return isClosed;
}
//=======================================================================
...
Standard_Boolean BRepBuilderAPI_Sewing::IsUClosedSurface(const Handle(Geom_Surface) & surf, const TopoDS_Shape& theEdge,
    ...
    if (tmpsurf->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface)))
        ...
        if (!isClosed) {
            ...
            if (!acrv2d.IsNull()) isClosed = IsClosedByIsos(tmpsurf, acrv2d, f2d, l2d, Standard_False);
        }
        return isClosed;
    }
...
Standard_Boolean BRepBuilderAPI_Sewing::IsVClosedSurface(const Handle(Geom_Surface) & surf, const TopoDS_Shape& theEdge,
                                                         const TopLoc_Location& theloc) const {
    Handle(Geom_Surface) tmpsurf = surf;
    if (tmpsurf->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface)))
        tmpsurf = Handle(Geom_RectangularTrimmedSurface)::DownCast(surf)->BasisSurface();
        ...
        if (!isClosed) {
            Standard_Real f2d, l2d;
            Handle(Geom2d_Curve) acrv2d = BRep_Tool::CurveOnSurface(TopoDS::Edge(theEdge), surf, theloc, f2d, l2d);
            if (!acrv2d.IsNull()) isClosed = IsClosedByIsos(tmpsurf, acrv2d, f2d, l2d, Standard_True);
        }
        return isClosed;
    }
...
TopoDS_Edge BRepBuilderAPI_Sewing::SameParameterEdge(const TopoDS_Shape& edge, const TopTools_SequenceOfShape& seqEdges,
    ...
    if (mySewing) {
        ...
        if (myBoundFaces.Contains(bnd1)) {
            ...
            for (; itf.More(); itf.Next())
                if (MergedFaces.Add(itf.Value())) listFaces1.Append(itf.Value());
        }
    } else {
    ...
    for (Standard_Integer i = 1; i <= seqEdges.Length(); i++) {
        ...
        if (mySewing) {
            ...
            if (!Orientation && whichSec != 1) isForward = isForward ? Standard_False : Standard_True;
            Edge1 = NewEdge;
        }
...
// purpose  : internal use
//=======================================================================
static Standard_Boolean findNMVertices(const TopoDS_Edge& theEdge, TopTools_SequenceOfShape& theSeqNMVert,
                                       TColStd_SequenceOfReal& theSeqPars) {
    TopoDS_Iterator aItV(theEdge, Standard_False);
    for (; aItV.More(); aItV.Next()) {
        if (aItV.Value().Orientation() == TopAbs_INTERNAL || aItV.Value().Orientation() == TopAbs_EXTERNAL)
            theSeqNMVert.Append(aItV.Value());
    }
    Standard_Integer nbV = theSeqNMVert.Length();
    if (!nbV) return Standard_False;
    Standard_Real first, last;
    ...
    for (Standard_Integer i = 1; i <= nbV; i++) {
        ...
        if (locProj.IsDone() && locProj.NbExt() > 0) {
            ...
            if (indMin) apar = locProj.Point(indMin).Parameter();
...
static void ComputeToleranceVertex(TopoDS_Vertex theV1, TopoDS_Vertex theV2, TopoDS_Vertex theV3,
...
TopoDS_Edge BRepBuilderAPI_Sewing::SameParameterEdge(const TopoDS_Edge& edgeFirst, const TopoDS_Edge& edgeLast,
                                                     const TopTools_ListOfShape& listFacesFirst,
                                                     const TopTools_ListOfShape& listFacesLast,
                                                     const Standard_Boolean secForward, Standard_Integer& whichSec,
    ...
    // Create and add new vertices
    {
        ...
        if (!isClosed1 && !isClosed2) {
            if (secForward) {
                if (V11.IsSame(V22) || V12.IsSame(V21)) return TopoDS_Edge();
            } else {
                if (V11.IsSame(V21) || V12.IsSame(V22)) return TopoDS_Edge();
            }
        }
        ...
        if (isClosed1 || isClosed2) {
            ...
            if (isOldFirst) V1New = V11;

            if (isOldLast) V2New = V12;
        }
    ...
    for (; itf2.More(); itf2.Next()) {
        ...
        if (!secForward) {
            if (c2d2->IsKind(STANDARD_TYPE(Geom2d_Line))) c2d2 = new Geom2d_TrimmedCurve(c2d2, firstOld, lastOld);
        ...
        for (; itf1.More() && !isSeam; itf1.Next()) {
            ...
            if (fac1.Orientation() == TopAbs_REVERSED) Ori = TopAbs::Reverse(Ori);
            ...
            if (isSeam) {
                ...
                if (!secForward) InitOri = TopAbs::Reverse(InitOri);
    ...
    try {
        if (isResEdge) SameParameter(edge);
    ...
    if (firstCall && (!isResEdge || !isSamePar || tolReached > myTolerance)) {
        ...
        TopoDS_Edge s_edge = SameParameterEdge(edgeFirst, edgeLast, listFacesFirst, listFacesLast, secForward,
                                               whichSecn, Standard_False);
        if (!s_edge.IsNull()) {
        ...
        if (!second_ok && !edge.IsNull()) {
            ...
            for (j = 1; more; j++) {
                ...
                if (more) {
                    ...
                    if (!loc2.IsIdentity()) aS = Handle(Geom_Surface)::DownCast(surf2->Transformed(loc2));
...
void BRepBuilderAPI_Sewing::EvaluateAngulars(TopTools_SequenceOfShape& sequenceSec, TColStd_Array1OfBoolean& secForward,
                                             TColStd_Array1OfReal& tabAng, const Standard_Integer indRef) const {
    tabAng.Init(-1.0);
    ...
    for (i = indRef; i <= lengSec; i++) {
        ...
        if (nbComputedAngle) tabAng(i) = cumulateAngular / ((Standard_Real)nbComputedAngle);
    }
}
...
void BRepBuilderAPI_Sewing::EvaluateDistances(TopTools_SequenceOfShape& sequenceSec,
                                              TColStd_Array1OfBoolean& secForward, TColStd_Array1OfReal& tabDst,
                                              TColStd_Array1OfReal& arrLen, TColStd_Array1OfReal& tabMinDist,
    ...
    for (i = indRef; i <= lengSec; i++) {
        ...
        for (j = 1; j <= npt; j++) {
            ...
            if (i == indRef) {
                ptsRef(j) = pt;
                if (j > 1) aLenSec2 += pt.SquareDistance(ptsRef(j - 1));
            } else {
                ptsSec(j) = pt;
                // protection to avoid merging with small sections
                if (j > 1) aLenSec2 += pt.SquareDistance(ptsSec(j - 1));
                ...
                if ((aVecRef * aVec1) * (aVecRef * aVec2) < 0.) nbFound++;
            }
        }
        ...
        if (i == indRef || (dist < myTolerance && nbFound >= npt * 0.5)) {
            ...
            for (j = 1; j <= npt; j++) {
                if (arrDist(j) < 0.) continue;
                if (dist < arrDist(j)) dist = arrDist(j);
                if (aMinDist > arrDist(j)) aMinDist = arrDist(j);
                nbFound++;
            }
            if (nbFound > 1) {
                tabDst(i) = dist;
...
Standard_Boolean BRepBuilderAPI_Sewing::IsMergedClosed(const TopoDS_Edge& Edge1, const TopoDS_Edge& Edge2,
...
void BRepBuilderAPI_Sewing::AnalysisNearestEdges(const TopTools_SequenceOfShape& sequenceSec,
    ...
    if (mySectionBound.IsBound(bnd)) bnd = mySectionBound(bnd);
    if (myBoundFaces.Contains(bnd)) workfaces = myBoundFaces.FindFromKey(bnd);
                ...
                for (lIt.Initialize(listfaces); lIt.More() && isMerged; lIt.Next()) {
                    if (mapFaces.Contains(lIt.Value())) {
                        ...
                        isMerged = ((IsUClosedSurface(surf, bnd2, loc) || IsVClosedSurface(surf, bnd2, loc)) &&
                                    IsMergedClosed(TopoDS::Edge(sequenceSec.Value(index)), TopoDS::Edge(workedge),
...
Standard_Boolean BRepBuilderAPI_Sewing::FindCandidates(TopTools_SequenceOfShape& seqSections,
    ...
    if (nbSections <= 1) return Standard_False;
    ...
    if (myNonmanifold && nbCandidates > 1) {
        ...
        while (nbCandidates) {
            ...
            Standard_Boolean isFound = FindCandidates(seqSections, mapReference, seqCandidates1, seqOrientations1);
...
BRepBuilderAPI_Sewing::BRepBuilderAPI_Sewing(const Standard_Real tolerance, const Standard_Boolean optionSewing,
...
void BRepBuilderAPI_Sewing::Init(const Standard_Real tolerance, const Standard_Boolean optionSewing,
                                 const Standard_Boolean optionAnalysis, const Standard_Boolean optionCutting,
                                 const Standard_Boolean optionNonmanifold) {
...
Standard_Boolean BRepBuilderAPI_Sewing::IsDegenerated(const TopoDS_Shape& aShape) const {
    ...
    // Degenerated face
    if (aShape.ShapeType() == TopAbs_FACE) return NewShape.IsNull();
    if (NewShape.IsNull()) return Standard_False;
    // Degenerated edge
    if (NewShape.ShapeType() == TopAbs_EDGE) return BRep_Tool::Degenerated(TopoDS::Edge(NewShape));
    // Degenerated wire
...
Standard_Boolean BRepBuilderAPI_Sewing::IsModified(const TopoDS_Shape& aShape) const {
    ...
    if (myOldShapes.Contains(aShape)) NewShape = myOldShapes.FindFromKey(aShape);
...
void BRepBuilderAPI_Sewing::FindFreeBoundaries() {
    ...
    for (; anIterEF.More(); anIterEF.Next()) {
        ...
        if (edge.Orientation() == TopAbs_INTERNAL) continue;
        Standard_Boolean isSeam = Standard_False;
        if (nbFaces == 1) {
        ...
        if (isBound || isBoundFloat) {
            ...
            if (vFirst.Orientation() == TopAbs_INTERNAL || vLast.Orientation() == TopAbs_INTERNAL) continue;
            if (isBound) {
                // Add to VertexNode
                if (!myVertexNode.Contains(vFirst)) myVertexNode.Add(vFirst, vFirst);
                if (!myVertexNode.Contains(vLast)) myVertexNode.Add(vLast, vLast);
            } else {
                // Add to VertexNodeFree
                if (!myVertexNodeFree.Contains(vFirst)) myVertexNodeFree.Add(vFirst, vFirst);
                if (!myVertexNodeFree.Contains(vLast)) myVertexNodeFree.Add(vLast, vLast);
            }
        }
    }
}
...
static Standard_Integer IsMergedVertices(const TopoDS_Shape& face1, const TopoDS_Shape& e1, const TopoDS_Shape& e2,
                                         const TopoDS_Shape& vtx1, const TopoDS_Shape& vtx2) {
    // Case of floating edges
    if (face1.IsNull()) return (!IsClosedShape(e1, vtx1, vtx2));
...
static Standard_Boolean GlueVertices(TopTools_IndexedDataMapOfShapeShape& aVertexNode,
                                     ...
                                     const Standard_Real Tolerance, const Message_ProgressRange& theProgress) {
    ...
    for (Standard_Integer i = 1; i <= nbNodes && aPS.More(); i++, aPS.Next()) {
        ...
        for (; iter1.More(); iter1.Next()) {
            ...
            for (; Ie1.More() && !Status && !isSameEdge; Ie1.Next()) {
                ...
                if (lfac1.Extent()) {
                    ...
                    for (; itf.More(); itf.Next())
                        if (!itf.Value().IsNull()) Faces1.Add(itf.Value());
                }
        ...
        if (SeqNodes.Length()) {
            ...
            if (SeqNodes.Length()) NodeNearestNode.Add(node1, SeqNodes.First());
        }
...
//=======================================================================
static void replaceNMVertices(const TopoDS_Edge& theEdge, const TopoDS_Vertex& theV1, const TopoDS_Vertex& theV2,
    ...
    for (; i <= nb; i++) {
        ...
        for (; j <= aEdParams.Length(); j++) {
            ...
            if (fabs(apar - apar2) <= Precision::PConfusion()) {
                ...
                BRep_ListOfPointRepresentation& alistrep = (*((Handle(BRep_TVertex)*)&anewV.TShape()))->ChangePoints();
...
static void ReplaceEdge(const TopoDS_Shape& oldEdge, const TopoDS_Shape& theNewShape,
...
void BRepBuilderAPI_Sewing::Merging(const Standard_Boolean /* firstTime */, const Message_ProgressRange& theProgress) {
    BRep_Builder B;
    ...
    for (; anIterB.More() && aPS.More(); anIterB.Next(), aPS.Next()) {
        ...
        if (!anIterB.Value().Extent()) {
            ...
            if (myVertexNodeFree.Contains(no1)) nno1 = myVertexNodeFree.FindFromKey(no1);
            if (myVertexNodeFree.Contains(no2)) nno2 = myVertexNodeFree.FindFromKey(no2);
            if (!no1.IsSame(nno1)) {
        ...
        if (!isPrevSplit) {
            ...
            if (MergedNearestEdges(bound, seqMergedWithBound, seqMergedWithBoundOri)) {
                ...
                while (ii <= seqMergedWithBound.Length()) {
                    ...
                    Standard_Boolean isRejected = (myMergedEdges.Contains(iedge) || MergedWithBound.Contains(iedge));
                    if (!isRejected) {
                ...
                if (nbMerged) {
                    ...
                    TopoDS_Edge MergedEdge =
                        SameParameterEdge(bound, seqMergedWithBound, seqMergedWithBoundOri, ActuallyMerged, myReShape);
                ...
                if (!nbMerged) MergedWithBound.RemoveKey(bound);
            }
        }
            ...
            for (; its.More(); its.Next()) {
                ...
                if (MergedNearestEdges(section, seqMergedWithSection, seqMergedWithSectionOri)) {
                    ...
                    if (nbMerged) {
                        // Create same parameter edge
                        TopTools_MapOfShape ActuallyMerged;
                        TopoDS_Edge MergedEdge = SameParameterEdge(
                            section, seqMergedWithSection, seqMergedWithSectionOri, ActuallyMerged, SectionsReShape);
                        Standard_Boolean isForward = Standard_False;
                        if (!MergedEdge.IsNull()) isForward = (MergedEdge.Orientation() == TopAbs_FORWARD);
                        // Process actually merged edges
                        Standard_Integer nbActuallyMerged = 0;
                    ...
                    if (!nbMerged) MergedWithSections.RemoveKey(section);
                } else if (isMerged) {
        ...
        if (isMerged && isMergedSplit && !isPrevSplit) {
            ...
            for (; anItm1.More(); anItm1.Next()) {
                ...
                if (MapSplitEdges.Contains(edge)) MapSplitEdges.RemoveKey(edge);
                if (myBoundSections.IsBound(edge)) {
                    ...
                    for (; its.More(); its.Next()) {
                        ...
                        if (MapSplitEdges.Contains(sec)) MapSplitEdges.RemoveKey(sec);
                    }
                }
            }
            ...
            for (Standard_Integer ii = 1; ii <= MapSplitEdges.Extent(); ii++) {
                const TopoDS_Edge& edge = TopoDS::Edge(MergedWithSections.FindFromKey(MapSplitEdges.FindKey(ii)));
        ...
        if (isSplitted) {
            ...
            if (!myMergedEdges.Contains(bound)) myMergedEdges.Add(bound);
        }
    }
                ...
                if (mapEdges.Add(sec)) {
                    ...
                    if (!isRejected && myBoundSections.IsBound(sec)) {
                        ...
                        for (; its.More() && !isRejected; its.Next()) {
                            ...
                            if (myMergedEdges.Contains(section)) isRejected = Standard_True;
                        }
                    }
...
void BRepBuilderAPI_Sewing::Cutting(const Message_ProgressRange& theProgress) {
    ...
    for (; anIterB.More() && aPS.More(); anIterB.Next(), aPS.Next()) {
        ...
        { // szv: Use brackets to destroy local variables
            ...
            CreateCuttingNodes(CandidateVertices, bound, V1, V2, arrDist, arrPara, arrProj, seqNode, seqPara);
        ...
        if (listSections.Extent() > 1) {
            ...
            for (; its.More(); its.Next()) {
                ...
                for (TopoDS_Iterator itv(section); itv.More(); itv.Next()) {
                    ...
                    if (myVertexNode.Contains(vertex)) vertex = TopoDS::Vertex(myVertexNode.FindFromKey(vertex));
...
static void GetSeqEdges(const TopoDS_Shape& edge, TopTools_SequenceOfShape& seqEdges,
...
void BRepBuilderAPI_Sewing::GetFreeWires(TopTools_IndexedMapOfShape& MapFreeEdges, TopTools_SequenceOfShape& seqWires) {
    TopTools_DataMapOfShapeListOfShape VertEdge;
...
static Standard_Boolean IsDegeneratedWire(const TopoDS_Shape& wire) {
    ...
    Standard_Real tol =
        BRep_Tool::Tolerance(V1) + BRep_Tool::Tolerance(V2); // Max(BRep_Tool::Tolerance(V1),BRep_Tool::Tolerance(V2));
...
static TopoDS_Edge DegeneratedSection(const TopoDS_Shape& section, const TopoDS_Shape& face) {
    ...
    if (!face.IsNull()) {
        ...
        if (aC2dn.IsNull()) aBuilder.UpdateEdge(edge, aC2dt, TopoDS::Face(face), 0);
    }
    ...
    if (!MapFreeEdges.IsEmpty()) {
        ...
        for (Standard_Integer j = 1; j <= seqWires.Length(); j++) {
            ...
            for (TopoDS_Iterator Ie(wire, Standard_False); Ie.More(); Ie.Next()) {
                ...
                TopoDS_Shape face;
                if (EdgeFace.IsBound(edge)) face = EdgeFace.Find(edge);
                TopoDS_Shape degedge = DegeneratedSection(edge, face);
                if (degedge.IsNull()) continue;
                if (!degedge.IsSame(edge)) ReplaceEdge(edge, degedge, myReShape);
                if (BRep_Tool::Degenerated(TopoDS::Edge(degedge))) myDegenerated.Add(degedge);
            }
        }
    }
}
        ...
        if (myBoundSections.IsBound(bound)) {
            ...
            for (; iter.More(); iter.Next()) {
                ...
                if (!bound.IsSame(section))
                    if (myContigousEdges.Contains(nedge)) myContigSecBound.Bind(section, bound);
            }
        }
    }
}
...
void BRepBuilderAPI_Sewing::ProjectPointsOnCurve(const TColgp_Array1OfPnt& arrPnt, const Handle(Geom_Curve) & c3d,
                                                 const Standard_Real first, const Standard_Real last,
                                                 TColStd_Array1OfReal& arrDist, TColStd_Array1OfReal& arrPara,
        ...
        try {
            ...
            if (locProj.IsDone() && locProj.NbExt() > 0) {
                Standard_Real dist2Min =
                    (isConsiderEnds || i1 == find || i1 == lind ? Min(distF2, distL2) : Precision::Infinite());
                Standard_Integer ind, indMin = 0;
                for (ind = 1; ind <= locProj.NbExt(); ind++) {
                    Standard_Real dProj2 = locProj.SquareDistance(ind);
                    if (dProj2 < dist2Min) {
...
void BRepBuilderAPI_Sewing::CreateCuttingNodes(const TopTools_IndexedMapOfShape& MapVert, const TopoDS_Shape& bound,
                                               const TopoDS_Shape& vfirst, const TopoDS_Shape& vlast,
                                               const TColStd_Array1OfReal& arrDist, const TColStd_Array1OfReal& arrPara,
                                               const TColgp_Array1OfPnt& arrPnt, TopTools_SequenceOfShape& seqVert,
                                               TColStd_SequenceOfReal& seqPara) {
...
void BRepBuilderAPI_Sewing::CreateSections(const TopoDS_Shape& section, const TopTools_SequenceOfShape& seqNode,
                                           const TColStd_SequenceOfReal& seqPara, TopTools_ListOfShape& listEdge) {
    ...
    for (; itf.More(); itf.Next()) {
        ...
        for (; ite.More(); ite.Next()) {
            ...
            if (!isSeam && c2d1New.IsNull())
                ...
                if (fac.Orientation() == TopAbs_REVERSED) Ori = TopAbs::Reverse(Ori);
...
#ifdef OCCT_DEBUG
            std::cout << "Fail: BRepBuilderAPI_Sewing::SameParameterShape exception in BRepLib::SameParameter"
                      << std::endl;
#endif
            continue;
        }
...
NCollection_CellFilter_Action BRepBuilderAPI_VertexInspector::Inspect(const Standard_Integer theTarget) {
    ...
    if ((aDx * aDx <= myTol) && (aDy * aDy <= myTol) && (aDz * aDz <= myTol)) myResInd.Append(theTarget);
