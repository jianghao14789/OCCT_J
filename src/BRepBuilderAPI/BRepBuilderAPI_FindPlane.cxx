        ...
        if (!c3d.IsNull()) {
            ...
            if (cType == STANDARD_TYPE(Geom_Line)) {
                nbPnts = 3;
                       ...
                       (cType == STANDARD_TYPE(Geom_Parabola)) || (cType == STANDARD_TYPE(Geom_Hyperbola))) {
                nbPnts = 4;
                if (!found) {
                    found = Standard_True;
    ...
    if (!found) {
        ...
        if (points.Length() > 2) {
            ...
            for (Standard_Integer i = 2; i <= points.Length(); i++) {
                ...
                if (dist > disMax) {
                    disMax = dist;
                    p1 = points(i); // it will be faster to store the point, otherwise it is necessary to find a value
                                    // in a sequence
                }
            }
