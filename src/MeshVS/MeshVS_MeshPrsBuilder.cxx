...
MeshVS_MeshPrsBuilder::MeshVS_MeshPrsBuilder(const Handle(MeshVS_Mesh) & Parent,
                                             ...
                                             const Handle(MeshVS_DataSource) & DS, const Standard_Integer Id,
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::Build(const Handle(Prs3d_Presentation) & Prs, const TColStd_PackedMapOfInteger& IDs,
                                  TColStd_PackedMapOfInteger& IDsToExclude, const Standard_Boolean IsElement,
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::BuildNodes(const Handle(Prs3d_Presentation) & Prs, const TColStd_PackedMapOfInteger& IDs,
    ...
    if (aSource.IsNull() || aDrawer.IsNull() || aNodeMark.IsNull()) return;
    ...
    if (!HasSelectFlag && !HasHilightFlag) {
        ...
        if (!aHiddenNodes.IsNull()) anIDs.Subtract(aHiddenNodes->Map());
    }
    ...
    for (; it.More(); it.Next()) {
        ...
        if (aSource->GetGeom(aKey, Standard_False, aCoords, NbNodes, aType)) {
            if (IsExcludingOn()) IDsToExclude.Add(aKey);
            k++;
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::BuildElements(const Handle(Prs3d_Presentation) & Prs, const TColStd_PackedMapOfInteger& IDs,
                                          TColStd_PackedMapOfInteger& IDsToExclude,
                                          const Standard_Integer DisplayMode) const {
    ...
    Handle(MeshVS_Drawer) aDrawer = GetDrawer();
    if (aDrawer.IsNull() || !aDrawer->GetInteger(MeshVS_DA_MaxFaceNodes, maxnodes) || maxnodes <= 0) return;
    ...
    Standard_Boolean IsWireFrame = (aDispMode == MeshVS_DMF_WireFrame), IsShading = (aDispMode == MeshVS_DMF_Shading),
                     IsShrink = (aDispMode == MeshVS_DMF_Shrink),
                     HasHilightFlag = ((DisplayMode & MeshVS_DMF_HilightPrs) != 0),
                     HasSelectFlag = ((DisplayMode & MeshVS_DMF_SelectionPrs) != 0), IsMeshReflect = Standard_False,
                     IsMeshAllowOverlap = Standard_False, IsMeshSmoothShading = Standard_False;
    ...
    Standard_Boolean IsOverlapControl = !IsMeshAllowOverlap && (IsWireFrame || IsShading) && !HasSelectFlag;
    ...
    if (!aHiddenElems.IsNull()) anIDs.Subtract(aHiddenElems->Map());
    anIDs.Subtract(IDsToExclude);
    ...
    for (it.Reset(); it.More(); it.Next()) {
        ...
        if (!aSource->GetGeom(it.Key(), Standard_True, aCoords, aNbNodes, aType)) continue;
        ...
        if (aType == MeshVS_ET_Volume) {
            if (aSource->Get3DGeom(it.Key(), aNbNodes, aTopo)) {
                for (Standard_Integer aFaceIdx = aTopo->Lower(); aFaceIdx <= aTopo->Upper(); ++aFaceIdx) {
                    ...
                    {
                        if (!HasSelectFlag) aNbVolmPrimitives += aFaceNodes.Length() - 2;
                    }
                }
            }
    ...
    for (it.Reset(); it.More(); it.Next()) {
        ...
        Standard_Integer NbNodes;
        if (!aSource->GetGeom(aKey, Standard_True, aCoords, NbNodes, aType)) continue;

        if (!aSource->GetNodesByElement(aKey, aNodes, NbNodes)) continue;
        ...
        switch (aType) {
            case MeshVS_ET_Volume: {
                ...
                if (aSource->Get3DGeom(aKey, NbNodes, aTopo)) {
                    ...
                    if (showEdges) {
                        AddVolumePrs(aTopo, aCoords, NbNodes, aEdgeSegments, IsReflect, IsShrink, HasSelectFlag,
                                     aShrinkCoef);
                    }

                    // Add shading presentation
                    if ((IsShading || IsShrink) && !HasSelectFlag) {
                        AddVolumePrs(aTopo, aCoords, NbNodes, aVolmTriangles, IsReflect, IsShrink, HasSelectFlag,
                                     aShrinkCoef);
                    }
                }
            } break;
            ...
            case MeshVS_ET_Face: {
                ...
                if (showEdges && IsOverlapControl) {
                    ...
                    for (Standard_Integer i = 1; i <= NbNodes; ++i) {
                        if (i > 1) aTwoNodes.First = aTwoNodes.Second;
                        ...
                        if (aLinkNodes.Contains(aTwoNodes)) {
                            for (Standard_Integer aNodeIdx = Last + 1; aNodeIdx < i; ++aNodeIdx) {
                                ...
                                aEdgeSegments->AddVertex(aCoords(3 * aNodeIdx - 2), aCoords(3 * aNodeIdx - 1),
                                                         aCoords(3 * aNodeIdx));
                                aEdgeSegments->AddVertex(aCoords(3 * aNextIdx - 2), aCoords(3 * aNextIdx - 1),
                                                         aCoords(3 * aNextIdx));
                            }
                    ...
                    if (NbNodes - Last > 0) {
                        for (Standard_Integer aNodeIdx = Last; aNodeIdx < NbNodes; ++aNodeIdx) {
                            ...
                            if (!aSegmentMap.Contains(aSegment)) {
                                aEdgeSegments->AddVertex(aCoords(3 * aNodeIdx + 1), aCoords(3 * aNodeIdx + 2),
                                                         aCoords(3 * aNodeIdx + 3));

                                aEdgeSegments->AddVertex(aCoords(3 * aNextIdx + 1), aCoords(3 * aNextIdx + 2),
    ...
    DrawArrays(Prs, aFaceTriangles, aEdgeSegments, aLinkSegments, aVolmTriangles, !showEdges, HasSelectFlag, aFill,
               aBeam);

    if (!aCustomElements.IsEmpty()) CustomBuild(Prs, aCustomElements, IDsToExclude, DisplayMode);

    if (IsShrink) aFill->SetEdgeColor(anOldEdgeColor);
}
...
void MeshVS_MeshPrsBuilder::BuildHilightPrs(const Handle(Prs3d_Presentation) & Prs,
    ...
    Handle(MeshVS_DataSource) aSource = GetDataSource();
    if (aSource.IsNull() || IDs.IsEmpty()) return;
    ...
    if (!aSource->GetGeom(ID, IsElement, aCoords, NbNodes, aType)) return;
...
void MeshVS_MeshPrsBuilder::AddLinkPrs(const TColStd_Array1OfReal& theCoords,
                                       ...
                                       const Standard_Boolean theIsShrinked, const Standard_Real theShrinkCoef) const {
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::AddFaceWirePrs(const TColStd_Array1OfReal& theCoords, const Standard_Integer theNbNodes,
                                           const Handle(Graphic3d_ArrayOfSegments) & theSegments,
    ...
    for (Standard_Integer aNodeIdx = 0; aNodeIdx < theNbNodes; ++aNodeIdx) {
        gp_XYZ aPnt(theCoords(3 * aNodeIdx + 1), theCoords(3 * aNodeIdx + 2), theCoords(3 * aNodeIdx + 3));
    ...
    for (Standard_Integer aNodeIdx = 0; aNodeIdx < theNbNodes; ++aNodeIdx) {
        theSegments->AddVertex(aNodes.Value(aNodeIdx).X(), aNodes.Value(aNodeIdx).Y(), aNodes.Value(aNodeIdx).Z());

        const Standard_Integer aNextIdx = (aNodeIdx + 1) % theNbNodes;

        theSegments->AddVertex(aNodes.Value(aNextIdx).X(), aNodes.Value(aNextIdx).Y(), aNodes.Value(aNextIdx).Z());
    }
}
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::AddFaceSolidPrs(const Standard_Integer theID, const TColStd_Array1OfReal& theCoords,
                                            const Standard_Integer theNbNodes, const Standard_Integer theMaxNodes,
                                            const Handle(Graphic3d_ArrayOfTriangles) & theTriangles,
                                            const Standard_Boolean theIsShaded, const Standard_Boolean theIsShrinked,
    ...
    if (theIsShaded) {
        if (theIsSmoothShading) {
            for (Standard_Integer aNodeIdx = 1; aNodeIdx <= theNbNodes; ++aNodeIdx) {
                if (!aDataSource->GetNodeNormal(aNodeIdx, theID, aNormalX, aNormalY, aNormalZ)) break;
        ...
        for (Standard_Integer aSubIdx = 0; aSubIdx < 3; ++aSubIdx) {
            if (theIsShaded) {
                ...
                theTriangles->AddVertex(aNodes.Value(aSubIdx == 0 ? 0 : (aNodeIdx + aSubIdx)).X(),
                                        ...
                                        aNodes.Value(aSubIdx == 0 ? 0 : (aNodeIdx + aSubIdx)).Z(), aNormalX, aNormalY,
                                        aNormalZ);
            } else {
                theTriangles->AddVertex(aNodes.Value(aSubIdx == 0 ? 0 : (aNodeIdx + aSubIdx)).X(),
                                        aNodes.Value(aSubIdx == 0 ? 0 : (aNodeIdx + aSubIdx)).Y(),
...
void MeshVS_MeshPrsBuilder::AddVolumePrs(const Handle(MeshVS_HArray1OfSequenceOfInteger) & theTopo,
                                         const TColStd_Array1OfReal& theNodes, const Standard_Integer theNbNodes,
                                         const Handle(Graphic3d_ArrayOfPrimitives) & theArray,
                                         const Standard_Boolean theIsShaded, const Standard_Boolean theIsShrinked,
                                         const Standard_Boolean theIsSelected, const Standard_Real theShrinkCoef) {
    Standard_Real aCenter[] = {0.0, 0.0, 0.0};

    Standard_Integer aLow = theNodes.Lower();

    if (theTopo.IsNull() || theArray.IsNull()) return;
    ...
    if (aIsPolygons) {
        ...
        for (Standard_Integer aFaceIdx = theTopo->Lower(), topoup = theTopo->Upper(); aFaceIdx <= topoup; ++aFaceIdx) {
            ...
            for (Standard_Integer aNodeIdx = 0, aNbNodes = aFaceNodes.Length(); aNodeIdx < aNbNodes; ++aNodeIdx) {
                ...
                aEdgeMap.Add(MeshVS_NodePair(aFaceNodes.Value(aNodeIdx + 1), aFaceNodes.Value(aNextIdx + 1)));
            }
        }

        // Draw edges
        for (NCollection_Map<MeshVS_NodePair, MeshVS_SymmetricPairHasher>::Iterator anIt(aEdgeMap); anIt.More();
             anIt.Next()) {
...
void MeshVS_MeshPrsBuilder::HowManyPrimitives(const Handle(MeshVS_HArray1OfSequenceOfInteger) & Topo,
                                              const Standard_Boolean AsPolygons, const Standard_Boolean IsSelect,
                                              const Standard_Integer NbNodes, Standard_Integer& Vertices,
...
void MeshVS_MeshPrsBuilder::DrawArrays(const Handle(Prs3d_Presentation) & Prs,
                                       ...
                                       const Standard_Boolean IsPolygonsEdgesOff, const Standard_Boolean IsSelected,
                                       const Handle(Graphic3d_AspectFillArea3d) & theFillAsp,
                                       const Handle(Graphic3d_AspectLine3d) & theLineAsp) const {
    if (theFillAsp.IsNull()) return;
    ...
    if (IsLinkPolylines) {
        ...
        if (!IsSelected) aBeamGroup->SetPrimitivesAspect(theFillAsp);
...
// Purpose  :
//================================================================
void MeshVS_MeshPrsBuilder::CalculateCenter(const TColStd_Array1OfReal& theCoords, const Standard_Integer NbNodes,
                                            Standard_Real& xG, Standard_Real& yG, Standard_Real& zG) {
    xG = yG = zG = 0;
    if (NbNodes < 4) {
        for (Standard_Integer k = 1; k <= NbNodes; k++) {
            xG += theCoords(3 * k - 2);
