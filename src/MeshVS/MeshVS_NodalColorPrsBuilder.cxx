...
MeshVS_NodalColorPrsBuilder::MeshVS_NodalColorPrsBuilder(const Handle(MeshVS_Mesh) & Parent,
    ...
    : MeshVS_PrsBuilder(Parent, Flags, DS, Id, Priority), myUseTexture(Standard_False),
...
// Purpose  :
//================================================================
void MeshVS_NodalColorPrsBuilder::Build(const Handle(Prs3d_Presentation) & Prs, const TColStd_PackedMapOfInteger& IDs,
    ...
    if (!(DisplayMode & GetFlags()) || !IsElement) return;
    ...
    Handle(Graphic3d_ArrayOfPolygons) aCPolyArr =
        new Graphic3d_ArrayOfPolygons(aMaxFaceNodes * aSize + PolygonVerticesFor3D, aSize + PolygonBoundsFor3D, 0,
                                      myUseTexture || IsReflect, !myUseTexture, Standard_False, myUseTexture);
    ...
    for (it.Reset(); it.More(); it.Next()) {
        ...
        if (aSource->GetGeom(aKey, Standard_True, aCoords, NbNodes, aType)) {
            ...
            if (aType == MeshVS_ET_Face) {
                for (Standard_Integer aNodeIdx = 0; aNodeIdx < NbNodes - 2; ++aNodeIdx) // triangulate polygon
                {
                    for (Standard_Integer aSubIdx = 0; aSubIdx < 3; ++aSubIdx) // generate sub-triangle
                    {
                        ...
                        if (myUseTexture) {
                            ...
                            aFaceTriangles->AddVertex(aPnt, aNorm,
                            ...
                            if (IsReflect) {
                                aNColor.SetValues(anColorRatio * aNColor.Red(), anColorRatio * aNColor.Green(),
                                                  anColorRatio * aNColor.Blue(), Quantity_TOC_RGB);
                ...
                if (!aSource->Get3DGeom(aKey, NbNodes, aTopo)) continue;

                AddVolumePrs(aTopo, aNodes, aCoords, aFaceTriangles, IsReflect, nbColors, nbTextureColors,
                             anColorRatio);

                AddVolumePrs(aTopo, aNodes, aCoords, anEdgeSegments, IsReflect, nbColors, nbTextureColors,
                             anColorRatio);
    ...
    if (myUseTexture) {
        ...
        anAsp = new Graphic3d_AspectFillArea3d(Aspect_IS_SOLID, Quantity_NOC_WHITE, anEdgeColor, anEdgeType,
                                               anEdgeWidth, aMaterial[0], aMaterial[1]);
    }
...
void MeshVS_NodalColorPrsBuilder::AddVolumePrs(const Handle(MeshVS_HArray1OfSequenceOfInteger) & theTopo,
                                               ...
                                               const Standard_Boolean theIsShaded, const Standard_Integer theNbColors,
        ...
        for (Standard_Integer aFaceIdx = theTopo->Lower(), topoup = theTopo->Upper(); aFaceIdx <= topoup; ++aFaceIdx) {
            ...
            for (Standard_Integer aNodeIdx = 0; aNodeIdx < aFaceNodes.Length() - 2; ++aNodeIdx) // triangulate polygon
            {
                for (Standard_Integer aSubIdx = 0; aSubIdx < 3; ++aSubIdx) // generate sub-triangle
                {
                    ...
                    if (myUseTexture) {
                        const Standard_Real aTexCoord =
                            myTextureCoords(theNodes(aFaceNodes(aSubIdx == 0 ? 1 : (aNodeIdx + aSubIdx + 1)) + 1));

                        theArray->AddVertex(aPnt, aNorm,
                        ...
                        if (theIsShaded) {
                            aNColor.SetValues(theColorRatio * aNColor.Red(), theColorRatio * aNColor.Green(),
                                              theColorRatio * aNColor.Blue(), Quantity_TOC_RGB);
...
// Purpose  :
//================================================================
Standard_Boolean MeshVS_NodalColorPrsBuilder::GetColor(const Standard_Integer ID, Quantity_Color& theColor) const {
    Standard_Boolean aRes = myNodeColorMap.IsBound(ID);
    if (aRes) theColor = myNodeColorMap.Find(ID);
    return aRes;
}
...
// Purpose  :
//================================================================
void MeshVS_NodalColorPrsBuilder::SetColor(const Standard_Integer theID, const Quantity_Color& theCol) {
...
//================================================================
void MeshVS_NodalColorPrsBuilder::SetTextureCoord(const Standard_Integer theID, const Standard_Real theCoord) {
...
Handle(Graphic3d_Texture2D) MeshVS_NodalColorPrsBuilder::CreateTexture() const {
    ...
    const Image_ColorRGBA aLastColor = {{Standard_Byte(255.0 * aLastColorSrc.Red()),
                                         ...
                                         Standard_Byte(255.0 * aLastColorSrc.Blue()), 0xFF}};
    ...
    const Image_ColorRGBA anInvalidColor = {{Standard_Byte(255.0 * myInvalidColor.Red()),
                                             ...
                                             Standard_Byte(255.0 * myInvalidColor.Blue()), 0xFF}};
