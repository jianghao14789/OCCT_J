...
// Purpose  :
//================================================================
MeshVS_VectorPrsBuilder::MeshVS_VectorPrsBuilder(const Handle(MeshVS_Mesh) & Parent, const Standard_Real MaxLength,
    ...
    : MeshVS_PrsBuilder(Parent, Flags, DS, Id, Priority), myIsSimplePrs(IsSimplePrs), mySimpleWidthPrm(2.5),
      mySimpleStartPrm(0.85), mySimpleEndPrm(0.95) {
...
// Purpose  :
//================================================================
const MeshVS_DataMapOfIntegerVector& MeshVS_VectorPrsBuilder::GetVectors(const Standard_Boolean IsElements) const {
    if (IsElements)
...
Standard_Boolean MeshVS_VectorPrsBuilder::HasVectors(const Standard_Boolean IsElement) const {
    ...
    if (IsElement) aRes = (myElemVectorMap.Extent() > 0);
    return aRes;
}
...
// Purpose  :
//================================================================
Standard_Boolean MeshVS_VectorPrsBuilder::GetVector(const Standard_Boolean IsElement, const Standard_Integer ID,
                                                    gp_Vec& Vect) const {
    const MeshVS_DataMapOfIntegerVector* aMap = &myNodeVectorMap;
    if (IsElement) aMap = &myElemVectorMap;

    Standard_Boolean aRes = aMap->IsBound(ID);
    if (aRes) Vect = aMap->Find(ID);
...
// Purpose  :
//================================================================
void MeshVS_VectorPrsBuilder::SetVector(const Standard_Boolean IsElement, const Standard_Integer ID,
                                        const gp_Vec& Vect) {
...
// Purpose  :
//================================================================
void MeshVS_VectorPrsBuilder::GetMinMaxVectorValue(const Standard_Boolean IsElement, Standard_Real& MinValue,
                                                   Standard_Real& MaxValue) const {
    const MeshVS_DataMapOfIntegerVector* aMap = &myNodeVectorMap;
    if (IsElement) aMap = &myElemVectorMap;

    MeshVS_DataMapIteratorOfDataMapOfIntegerVector anIt(*aMap);
    if (anIt.More()) MinValue = MaxValue = anIt.Value().Magnitude();

    Standard_Real aCurValue;

    for (; anIt.More(); anIt.Next()) {
        aCurValue = anIt.Value().Magnitude();
        if (MinValue > aCurValue) MinValue = aCurValue;
        if (MaxValue < aCurValue) MaxValue = aCurValue;
    }
}
...
void MeshVS_VectorPrsBuilder::Build(const Handle(Prs3d_Presentation) & Prs, const TColStd_PackedMapOfInteger& IDs,
    ...
    if (aSource.IsNull() || aDrawer.IsNull() || !HasVectors(IsElement) || (theDisplayMode & GetFlags()) == 0) return;
    ...
    Handle(Graphic3d_ArrayOfPrimitives) aTriangleArray =
        new Graphic3d_ArrayOfSegments(aNbVectors * 8 /* vertices per arrow */,
    ...
    for (; it.More(); it.Next()) {
        ...
        if (GetVector(IsElement, aKey, aVec)) {
            ...
            if (Abs(aValue) < Precision::Confusion()) continue;
            ...
            if (aSource->GetGeom(aKey, IsElement, aCoords, NbNodes, aType)) {
                if (aType == MeshVS_ET_Node) {
                    ...
                    if (IsElement && IsExcludingOn()) IDsToExclude.Add(aKey);
                    X = Y = Z = 0;
                ...
                aTrsf.SetDisplacement(gp_Ax3(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1)), gp_Ax3(gp_Pnt(X, Y, Z), aVec));

                DrawVector(aTrsf, Max(k * fabs(aValue) + b, aMinLength), aMaxLen, anArrowPnt, aLineArray,
                           aArrowLineArray, aTriangleArray);
            }
        }
    }
    ...
    Handle(Graphic3d_AspectLine3d) aLinAspect = new Graphic3d_AspectLine3d(aColor, Aspect_TOL_SOLID, 1.5);
    ...
    if (!aCustomElements.IsEmpty()) CustomBuild(Prs, aCustomElements, IDsToExclude, theDisplayMode);
}
...
//=======================================================================
void MeshVS_VectorPrsBuilder::DrawVector(const gp_Trsf& theTrsf, const Standard_Real theLength,
                                         const Standard_Real theMaxLength, const TColgp_Array1OfPnt& theArrowPoints,
    ...
    if (!myIsSimplePrs) {
        ...
        if (aUpper - aLower < PntNum - 1) return;
...
//=======================================================================
Standard_Real MeshVS_VectorPrsBuilder::calculateArrow(TColgp_Array1OfPnt& Points, const Standard_Real Length,
...
void MeshVS_VectorPrsBuilder::SetSimplePrsParams(const Standard_Real theLineWidthParam,
                                                 const Standard_Real theStartParam, const Standard_Real theEndParam) {
