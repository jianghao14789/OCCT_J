...
// purpose  :
//=======================================================================
MeshVS_SensitiveQuad::MeshVS_SensitiveQuad(const Handle(SelectMgr_EntityOwner) & theOwner, const gp_Pnt& thePnt1,
                                           const gp_Pnt& thePnt2, const gp_Pnt& thePnt3, const gp_Pnt& thePnt4)
    : Select3D_SensitiveEntity(theOwner) {
...
Standard_Boolean MeshVS_SensitiveQuad::Matches(SelectBasics_SelectingVolumeManager& theMgr,
    ...
    if (!theMgr.IsOverlapAllowed()) // check for inclusion
    {
        if (theMgr.GetActiveSelectionType() == SelectMgr_SelectionType_Polyline) {
            ...
            return theMgr.OverlapsTriangle(myVertices[0], myVertices[1], myVertices[2], Select3D_TOS_INTERIOR,
                                           aDummy) &&
        ...
        for (Standard_Integer aPntIdx = 0; aPntIdx < 4; ++aPntIdx) {
            if (!theMgr.OverlapsPoint(myVertices[aPntIdx])) return Standard_False;
        }
...
Select3D_BndBox3d MeshVS_SensitiveQuad::BoundingBox() {
    ...
    for (Standard_Integer aPntIdx = 0; aPntIdx < 4; ++aPntIdx) {
        aBox.Add(SelectMgr_Vec3(myVertices[aPntIdx].X(), myVertices[aPntIdx].Y(), myVertices[aPntIdx].Z()));
    }
