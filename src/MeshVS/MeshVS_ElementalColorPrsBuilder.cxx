...
void MeshVS_ElementalColorPrsBuilder::Build(const Handle(Prs3d_Presentation) & Prs,
    ...
    if (aSource.IsNull() || aDrawer.IsNull()) return;

    Standard_Integer aMaxFaceNodes;
    if (!aDrawer->GetInteger(MeshVS_DA_MaxFaceNodes, aMaxFaceNodes) && aMaxFaceNodes <= 0) return;
    ...
    if (!(DisplayMode & GetFlags()) || !IsElement || (myElemColorMap1.IsEmpty() && myElemColorMap2.IsEmpty())) return;
    ...
    for (MeshVS_DataMapIteratorOfDataMapOfIntegerColor anIterM(*anElemColorMap); anIterM.More(); anIterM.Next()) {
        ...
        for (MeshVS_DataMapIteratorOfDataMapOfColorMapOfInteger anIterC(aColorsOfElements); anIterC.More() && !IsExist;
             anIterC.Next())
    ...
    Standard_Boolean anEdgeOn = Standard_True, IsReflect = Standard_False, IsMeshSmoothShading = Standard_False;
    ...
    if (aDrawer->GetInteger(MeshVS_DA_EdgeType, anEdgeInt)) anEdgeType = (Aspect_TypeOfLine)anEdgeInt;

    Standard_Integer aLineInt = Aspect_TOL_SOLID;
    if (aDrawer->GetInteger(MeshVS_DA_BeamType, aLineInt)) aLineType = (Aspect_TypeOfLine)aLineInt;
    ...
    for (; it.More(); it.Next()) {
        ...
        if (aSource->Get3DGeom(aKey, NbNodes, aTopo))
            MeshVS_MeshPrsBuilder::HowManyPrimitives(aTopo, Standard_True, Standard_False, NbNodes,
    ...
    if (!aColorsOfElements.IsEmpty()) {
        Handle(Graphic3d_AspectFillArea3d) aGroupFillAspect = new Graphic3d_AspectFillArea3d(
            Aspect_IS_SOLID, anInteriorColor, anEdgeColor, anEdgeType, anEdgeWidth, aMaterial[0], aMaterial[1]);
    ...
    for (MeshVS_DataMapIteratorOfDataMapOfColorMapOfInteger aColIter(aColorsOfElements); aColIter.More();
        ...
        for (it.Reset(); it.More(); it.Next()) {
            ...
            if (!aColIter.Value().Contains(it.Key())) continue;
        ...
        Handle(Graphic3d_AspectFillArea3d) aFillAspect = new Graphic3d_AspectFillArea3d(
            Aspect_IS_SOLID, aColIter.Key(), anEdgeColor, anEdgeType, anEdgeWidth, aMaterial[0], aMaterial[1]);

        Handle(Graphic3d_AspectLine3d) aLinkAspect = new Graphic3d_AspectLine3d(aColIter.Key(), aLineType, aLineWidth);
        ...
        for (it.Reset(); it.More(); it.Next()) {
            ...
            if (aColIter.Value().Contains(aKey)) {
                ...
                if (aType == MeshVS_ET_Volume) {
                    ...
                    MeshVS_MeshPrsBuilder::AddVolumePrs(aTopo, aCoords, NbNodes, aFaceTriangles, IsReflect,
                                                        Standard_False, Standard_False, 1.0);

                    if (anEdgeOn) {
                        MeshVS_MeshPrsBuilder::AddVolumePrs(aTopo, aCoords, NbNodes, anEdgeSegments, IsReflect,
                                                            Standard_False, Standard_False, 1.0);
                    }
                    ...
                    if (anEdgeOn) {
                        for (Standard_Integer aNodeIdx = 0; aNodeIdx < NbNodes; ++aNodeIdx) {
                            ...
                            anEdgeSegments->AddVertex(aCoords(3 * aNodeIdx + 1), aCoords(3 * aNodeIdx + 2),
                                                      aCoords(3 * aNodeIdx + 3));

                            anEdgeSegments->AddVertex(aCoords(3 * aNextIdx + 1), aCoords(3 * aNextIdx + 2),
                    ...
                    for (Standard_Integer aNodeIdx = 0; aNodeIdx < NbNodes - 1; ++aNodeIdx) {
                        ...
                        aLinkSegments->AddVertex(aCoords(3 * aNodeIdx + 1), aCoords(3 * aNodeIdx + 2),
                                                 aCoords(3 * aNodeIdx + 3));

                        aLinkSegments->AddVertex(aCoords(3 * aNextIdx + 1), aCoords(3 * aNextIdx + 2),
                                                 aCoords(3 * aNextIdx + 3));
    ...
    if (!aTwoColorsOfElements.IsEmpty()) {
        Handle(Graphic3d_AspectFillArea3d) aGroupFillAspect2 = new Graphic3d_AspectFillArea3d(
            Aspect_IS_SOLID, anInteriorColor, anEdgeColor, anEdgeType, anEdgeWidth, aMaterial2[0], aMaterial2[1]);
        aGroup2->SetClosed(Standard_False); // ignore toSupressBackFaces
        aGroup2->SetGroupPrimitivesAspect(aGroupFillAspect2);
    }
    for (MeshVS_DataMapIteratorOfDataMapOfTwoColorsMapOfInteger aColIter2(aTwoColorsOfElements); aColIter2.More();
         aColIter2.Next()) {
        ...
        for (it.Reset(); it.More(); it.Next()) {
            ...
            if (!aColIter2.Value().Contains(it.Key())) continue;
        ...
        Handle(Graphic3d_AspectFillArea3d) anAsp = new Graphic3d_AspectFillArea3d(
            Aspect_IS_SOLID, aMyIntColor, anEdgeColor, anEdgeType, anEdgeWidth, aMaterial2[0], aMaterial2[1]);
...
// Purpose  :
//================================================================
Standard_Boolean MeshVS_ElementalColorPrsBuilder::GetColor1(const Standard_Integer ID, Quantity_Color& theColor) const {
    Standard_Boolean aRes = myElemColorMap1.IsBound(ID);
    if (aRes) theColor = myElemColorMap1.Find(ID);
...
// Purpose  :
//================================================================
void MeshVS_ElementalColorPrsBuilder::SetColor1(const Standard_Integer theID, const Quantity_Color& theCol) {
...
Standard_Boolean MeshVS_ElementalColorPrsBuilder::GetColor2(const Standard_Integer ID,
    ...
    if (aRes) theColor = myElemColorMap2.Find(ID);
...
// Purpose  :
//================================================================
Standard_Boolean MeshVS_ElementalColorPrsBuilder::GetColor2(const Standard_Integer ID, Quantity_Color& theColor1,
                                                            Quantity_Color& theColor2) const {
    MeshVS_TwoColors aTC;
    Standard_Boolean aRes = GetColor2(ID, aTC);
    if (aRes) ExtractColors(aTC, theColor1, theColor2);
    return aRes;
}
...
// Purpose  :
//================================================================
void MeshVS_ElementalColorPrsBuilder::SetColor2(const Standard_Integer theID, const Quantity_Color& theCol1,
...
// Purpose  :
//================================================================
void MeshVS_ElementalColorPrsBuilder::SetColor2(const Standard_Integer theID, const MeshVS_TwoColors& theCol) {
