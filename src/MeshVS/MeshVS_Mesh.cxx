...
void MeshVS_Mesh::Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
                          const Handle(Prs3d_Presentation) & thePresentation, const Standard_Integer theMode) {
...
// Purpose  :
//================================================================
void MeshVS_Mesh::ComputeSelection(const Handle(SelectMgr_Selection) & theSelection, const Standard_Integer theMode) {
    OSD_Timer gTimer;
    ...
    if (aSource->IsAdvancedSelectionEnabled()) {
        ...
        for (MeshVS_DataMapIteratorOfDataMapOfIntegerOwner anIt(GetOwnerMaps(Standard_False)); anIt.More();
             anIt.Next()) {
                    ...
                    case MeshVS_MSM_PRECISE: {
                        ...
                        for (TColStd_MapIteratorOfPackedMapOfInteger anElemIter(anAllElementsMap); anElemIter.More();
                            ...
                            if (IsSelectableElem(anElemIdx) &&
                                myDataSource->GetGeomType(anElemIdx, Standard_True, aType) && aType == MeshVS_ET_Link) {
                                ...
                                if (!isVertsShared) {
                                    Handle(Select3D_SensitiveEntity) aLinkEnt = new Select3D_SensitiveSegment(
                                        myWholeMeshOwner, aLinkPnts.Value(1), aLinkPnts.Value(2));
                        ...
                        for (TColStd_MapIteratorOfPackedMapOfInteger aNodesIter(anAllNodesMap); aNodesIter.More();
                            ...
                            if (IsSelectableNode(aNodeIdx) &&
                                myDataSource->GetGeom(aNodeIdx, Standard_False, aPntArrayAsCoordArray, aNbNodes,
                                                      aType) &&
                    ...
                    for (TColStd_MapIteratorOfPackedMapOfInteger anIterMG(aGroupMap); anIterMG.More();
                        ...
                        if (aGroupType == MeshVS_ET_Node) {
                            ...
                            if (aType == MeshVS_ET_Face && aNbNodes > 0) // Faces: 2D-elements
                            {
                                ...
                                if (aNbNodes == 3) {
                                    aSensFace =
                                        new Select3D_SensitiveTriangle(anOwner, aPntArray.Value(1), aPntArray.Value(2),
                                                                       aPntArray.Value(3), Select3D_TOS_INTERIOR);
                                } else if (aNbNodes == 4) {
...
// Purpose  :
//================================================================
void MeshVS_Mesh::AddBuilder(const Handle(MeshVS_PrsBuilder) & theBuilder, const Standard_Boolean TreatAsHilighter) {
    if (theBuilder.IsNull()) return;

    Standard_Integer i, n = myBuilders.Length();
    for (i = 1; i <= n; i++)
        if (myBuilders(i)->GetPriority() < theBuilder->GetPriority()) break;
    ...
    if (TreatAsHilighter) myHilighter = theBuilder;
}
...
void MeshVS_Mesh::RemoveBuilder(const Standard_Integer theIndex) {
    ...
    if (!aBuild.IsNull()) {
        if (aBuild == myHilighter) myHilighter.Nullify();
...
void MeshVS_Mesh::RemoveBuilderById(const Standard_Integer Id) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        if (!aCur.IsNull() && aCur->GetId() == Id) break;
    }
    if (i >= 1 && i <= n) {
        if (GetBuilder(i) == myHilighter) myHilighter.Nullify();
        RemoveBuilder(i);
    }
}
...
void MeshVS_Mesh::SetHiddenElems(const Handle(TColStd_HPackedMapOfInteger) & theMap) {
    ...
    if (!GetDrawer().IsNull() && GetDrawer()->GetBoolean(MeshVS_DA_SelectableAuto, AutoSelUpdate) && AutoSelUpdate)
...
const MeshVS_DataMapOfIntegerOwner& MeshVS_Mesh::GetOwnerMaps(const Standard_Boolean IsElements) {
    Handle(MeshVS_DataSource) aDS = GetDataSource();
    if (!aDS.IsNull() && aDS->IsAdvancedSelectionEnabled()) return myMeshOwners;
    if (IsElements) {
...
void MeshVS_Mesh::HilightSelected(const Handle(PrsMgr_PresentationManager) & thePM,
    ...
    if (HasPresentation()) aSelectionPrs->SetTransformPersistence(Presentation()->TransformPersistence());
    //----------------
    ...
    for (i = 1; i <= len; i++) {
        ...
        if (!anOwner.IsNull()) {
                ...
                if (GetDataSource()->GetGroup(anOwner->ID(), aGroupType, aGroupMap)) {
                    if (aGroupType == MeshVS_ET_Node) {
                        for (TColStd_MapIteratorOfPackedMapOfInteger anIt(aGroupMap); anIt.More(); anIt.Next())
                            if (IsSelectableNode /*!IsHiddenNode*/ (anIt.Key())) aSelNodes.Add(anIt.Key());
                    } else {
                        for (TColStd_MapIteratorOfPackedMapOfInteger anIt(aGroupMap); anIt.More(); anIt.Next())
                            if (IsSelectableElem /*!IsHiddenElem*/ (anIt.Key())) aSelElements.Add(anIt.Key());
                    }
                }
            } else {
            ...
            if (!aMeshOwner.IsNull()) {
                ...
                if (!aNodes.IsNull()) aSelNodes.Assign(aNodes->Map());
                if (!aElems.IsNull()) aSelElements.Assign(aElems->Map());
            }
        }
        // agv    else if( theOwners.Value ( i )==myWholeMeshOwner )
            ...
            for (; anIt.More(); anIt.Next())
                if (!IsHiddenNode(anIt.Key())) aSelNodes.Add(anIt.Key());

            anIt = TColStd_MapIteratorOfPackedMapOfInteger(GetDataSource()->GetAllElements());
            for (; anIt.More(); anIt.Next())
                if (!IsHiddenElem(anIt.Key())) aSelElements.Add(anIt.Key());
...
void MeshVS_Mesh::HilightOwnerWithColor(const Handle(PrsMgr_PresentationManager) & thePM,
    ...
    // new functionality
    if (HasPresentation()) aHilightPrs->SetTransformPersistence(Presentation()->TransformPersistence());
    //----------------
    ...
    if (isMeshEntityOwner) {
        ...
        if (theAISOwner->IsGroup()) {
            ...
            anOne.Add(anID);
            myHilighter->Build(aHilightPrs, anOne, tmp, aType != MeshVS_ET_Node, aDispMode | MeshVS_DMF_HilightPrs);
        }
    } else if (isWholeMeshOwner) {
        if (!GetDataSource().IsNull()) {
            TColStd_PackedMapOfInteger tmp;
            myHilighter->Build(aHilightPrs, GetDataSource()->GetAllElements(), tmp, Standard_True,
        ...
        if (!aMeshOwner.IsNull()) {
            ...
            if (!aNodes.IsNull()) {
                TColStd_PackedMapOfInteger tmp;
                myHilighter->Build(aHilightPrs, aNodes->Map(), tmp, Standard_False, aDispMode | MeshVS_DMF_HilightPrs);
            }
            if (!aElems.IsNull()) {
                TColStd_PackedMapOfInteger tmp;
                myHilighter->Build(aHilightPrs, aElems->Map(), tmp, Standard_True, aDispMode | MeshVS_DMF_HilightPrs);
            }
        }
    }
...
void MeshVS_Mesh::ClearSelected() {
    ...
    if (!aSelectionPrs.IsNull()) aSelectionPrs->Clear();
}
