...
MeshVS_CommonSensitiveEntity::MeshVS_CommonSensitiveEntity(const Handle(SelectMgr_EntityOwner) & theOwner,
    ...
    : Select3D_SensitiveSet(theOwner), myDataSource(theParentMesh->GetDataSource()), mySelMethod(theSelMethod) {
    myMaxFaceNodes = 0;
...
MeshVS_CommonSensitiveEntity::MeshVS_CommonSensitiveEntity(const MeshVS_CommonSensitiveEntity& theOther)
    : Select3D_SensitiveSet(theOther.myOwnerId), myDataSource(theOther.myDataSource),
      myItemIndexes(theOther.myItemIndexes), mySelMethod(theOther.mySelMethod), myMaxFaceNodes(theOther.myMaxFaceNodes),
      myCOG(theOther.myCOG), myBndBox(theOther.myBndBox) {
    //
}
...
Select3D_BndBox3d MeshVS_CommonSensitiveEntity::Box(const Standard_Integer theIdx) const {
    ...
    if (mySelMethod == MeshVS_MSM_PRECISE) {
        ...
        for (Standard_Integer aNodeIdx = 1; aNodeIdx <= aNbNodes; aNodeIdx++) {
            const SelectMgr_Vec3 aPnt(aCoords(3 * aNodeIdx - 2), aCoords(3 * aNodeIdx - 1), aCoords(3 * aNodeIdx));
            aBox.Add(aPnt);
        }
...
// purpose  :
//=======================================================================
void MeshVS_CommonSensitiveEntity::Swap(const Standard_Integer theIdx1, const Standard_Integer theIdx2) {
...
Standard_Boolean MeshVS_CommonSensitiveEntity::overlapsElement(SelectBasics_PickResult& thePickResult,
    ...
    if (mySelMethod == MeshVS_MSM_PRECISE) {
        ...
        if (aNbNodes == 3) {
            return theMgr.OverlapsTriangle(
                gp_Pnt(aCoords(1), aCoords(2), aCoords(3)), gp_Pnt(aCoords(4), aCoords(5), aCoords(6)),
                gp_Pnt(aCoords(7), aCoords(8), aCoords(9)), Select3D_TOS_INTERIOR, thePickResult);
        }

        MeshVS_Buffer aFacePntsBuf(aNbNodes * 3 * sizeof(Standard_Real));
        TColgp_Array1OfPnt aFacePnts(aFacePntsBuf, 1, aNbNodes);
        for (Standard_Integer aNodeIdx = 1; aNodeIdx <= aNbNodes; aNodeIdx++) {
            aFacePnts.SetValue(aNodeIdx,
...
Standard_Boolean MeshVS_CommonSensitiveEntity::elementIsInside(SelectBasics_SelectingVolumeManager& theMgr,
    ...
    if (mySelMethod == MeshVS_MSM_PRECISE) {
        ...
        TColgp_Array1OfPnt aFacePnts(aFacePntsBuf, 1, aNbNodes);
        for (Standard_Integer aNodeIdx = 1; aNodeIdx <= aNbNodes; ++aNodeIdx) {
            const gp_Pnt aPnt(aCoords(3 * aNodeIdx - 2), aCoords(3 * aNodeIdx - 1), aCoords(3 * aNodeIdx));
