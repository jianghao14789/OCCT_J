...
// purpose  :
//=======================================================================
static Standard_Integer incrementalmesh(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    if (theNbArgs < 3) {
        ...
        if (aNameCase == "-relative" || aNameCase == "-norelative") {
            ...
            if (anAlgoStr == "watson" || anAlgoStr == "0") {
                ...
                return 1;
                   ...
                   !isPrsDefl) {
            isPrsDefl = true;
        } else if ((aNameCase == "-angular" || aNameCase == "-angdefl" || aNameCase == "-angulardeflection" ||
                    aNameCase == "-a") &&
...
static Standard_Integer tessellate(Draw_Interpretor& /*di*/, Standard_Integer nbarg, const char** argv) {
    ...
    Handle(Poly_Triangulation) aTriangulation = new Poly_Triangulation(aNbNodes, aNbTriangles, Standard_False);
...
static Standard_Integer TrLateLoad(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 2; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anArgCase == "-load") {
        ...
        } else if (anArgIter + 1 < theNbArgs && (anArgCase == "-activatestrict" || anArgCase == "-activateexact") &&
                   TCollection_AsciiString(theArgVec[anArgIter + 1]).IsIntegerValue()) {
            Standard_Integer anIndexToActivate = TCollection_AsciiString(theArgVec[++anArgIter]).IntegerValue();
            if (anIndexToActivate < 0) {
                Message::SendWarning("Invalid negative triangulation index to be activated");
            ...
            if (anArgIter + 1 < theNbArgs && TCollection_AsciiString(theArgVec[anArgIter + 1]).IsIntegerValue()) {
...
static Standard_Integer trianglesinfo(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    struct TriangulationStat {
        TriangulationStat()
            : NbFaces(0), NbEmptyFaces(0), NbTriangles(0), NbDeferredFaces(0), NbUnloadedFaces(0),
    ...
    if (aNbLODs.Size() > 0) {
        ...
        for (NCollection_IndexedMap<Standard_Integer>::Iterator aRangeIter(aLODsRange); aRangeIter.More();
    ...
    if (!aLODsStat.IsEmpty()) {
        ...
        for (NCollection_IndexedDataMap<Standard_Integer, TriangulationStat>::Iterator anIter(aLODsStat); anIter.More();
             anIter.Next()) {
...
static Standard_Integer tri2d(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (!T.IsNull()) {
        ...
        if (T->HasUVNodes()) {
            ...
            nn = nInternal;
            for (i = 1; i <= nn; i++) {
                Seg = new Draw_Segment2D(T->UVNode(Internal[2 * i - 1]), T->UVNode(Internal[2 * i]), Draw_bleu);
                dout << Seg;
            }
        }
        dout.Flush();
...
static Standard_Integer triedgepoints(Draw_Interpretor& di, Standard_Integer nbarg, const char** argv) {
    ...
    for (Standard_Integer i = 1; i < nbarg; i++) {
        ...
        if (anEdgeMap.Extent() == 0) continue;
        ...
        *p = '_';
        ...
        for (it.Initialize(anEdgeMap); it.More(); it.Next()) {
            ...
            if (aT.IsNull() || aPoly.IsNull()) continue;
            ...
            for (Standard_Integer j = 1; j <= nbnodes; j++) {
                ...
                if (!aLoc.IsIdentity()) P3d.Transform(aLoc.Transformation());
...
//=======================================================================
static Standard_Integer correctnormals(Draw_Interpretor& theDI, Standard_Integer /*theNArg*/, const char** theArgVal) {
...
//=======================================================================
{
    ...
    theCommands.Add("tessellate", "Builds triangular mesh for the surface, run w/o args for help", __FILE__, tessellate,
                    g);
    ...
    theCommands.Add(
        ...
        "\n\t\t:   '-loadSingleExact' - make loaded and active ONLY exactly specified triangulation. All other "
        "triangulations"
