...
void MeshTest_CheckTopology::Perform(Draw_Interpretor& di) {
    ...
    for (ie = 1; ie <= aMapEF.Extent(); ie++) {
        ...
        Handle(Poly_PolygonOnTriangulation) aPoly1 = BRep_Tool::PolygonOnTriangulation(aEdge, aT1, aLoc1);
        ...
        for (; it.More(); it.Next()) {
            ...
            Handle(Poly_PolygonOnTriangulation) aPoly2 = BRep_Tool::PolygonOnTriangulation(aEdge, aT2, aLoc2);
    ...
    for (iF = 1; iF <= aMapF.Extent(); iF++) {
        ...
        for (; ex.More(); ex.Next()) {
            ...
            Handle(Poly_PolygonOnTriangulation) aPoly = BRep_Tool::PolygonOnTriangulation(aEdge, aT, aLoc);
        ...
        for (i = 1; i <= nbTri; i++) {
            ...
            const gp_Pnt aPts[3] = {aT->Node(n[0]).Transformed(aTrsf), aT->Node(n[1]).Transformed(aTrsf),
            ...
            for (j = 0; j < 3; j++) {
                if (t[j] == 0) {
                    ...
                    if (aMapBndNodes.Contains(n1) && aMapBndNodes.Contains(n2)) continue;
...
void MeshTest_CheckTopology::GetFreeLink(const Standard_Integer theFaceIndex, const Standard_Integer theLinkIndex,
                                         Standard_Integer& theNode1, Standard_Integer& theNode2) const {
...
void MeshTest_CheckTopology::GetCrossFaceError(const Standard_Integer theIndex, Standard_Integer& theFace1,
                                               Standard_Integer& theNode1, Standard_Integer& theFace2,
                                               Standard_Integer& theNode2, Standard_Real& theValue) const {
