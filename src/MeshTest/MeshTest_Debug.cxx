...
Standard_EXPORT const char* MeshTest_DrawLinks(const char* theNameStr, void* theDataStruct) {
    ...
    try {
        ...
        if (aMeshData.IsNull()) return "Null mesh data structure";
        ...
        for (Standard_Integer i = 1; i <= nbLinks; i++) {
            ...
            if (aLink.Movability() == BRepMesh_Deleted) continue;
            ...
            Handle(Draw_Segment3D) aSeg = new Draw_Segment3D(gp_Pnt(aV1.Coord().X(), aV1.Coord().Y(), 0),
                                                             gp_Pnt(aV2.Coord().X(), aV2.Coord().Y(), 0), Draw_bleu);
...
Standard_EXPORT const char* MeshTest_DrawTriangles(const char* theNameStr, void* theDataStruct) {
    ...
    try {
        ...
        for (Standard_Integer i = 1; i <= nbElem; i++) {
            ...
            if (aTri.Movability() == BRepMesh_Deleted) continue;
            ...
            gp_Pnt aP[4] = {gp_Pnt(aV1.Coord().X(), aV1.Coord().Y(), 0), gp_Pnt(aV2.Coord().X(), aV2.Coord().Y(), 0),
                            gp_Pnt(aV3.Coord().X(), aV3.Coord().Y(), 0), gp_Pnt(aV1.Coord().X(), aV1.Coord().Y(), 0)};
