...
static Standard_Integer stepread(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    Standard_Boolean modfic = XSDRAW::FileAndVar(argv[1], argv[2], "STEP", fnom, rnom);
    if (modfic)
    ...
    while (modepri) {
        ...
        if (modepri <= 2) {
            ...
            if (fromtcl) {
                ...
                if (list.IsNull()) {
                    di << "No list defined. Give a selection name or * for all transferrable roots\n";
                    continue;
                }
            } else {
...
static Standard_Integer steptrans(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (n1 > 0) ax1 = Handle(StepGeom_Axis2Placement3d)::DownCast(XSDRAW::Entity(n1));
    if (n2 > 0) ax2 = Handle(StepGeom_Axis2Placement3d)::DownCast(XSDRAW::Entity(n2));
...
static Standard_Integer stepwrite(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    switch (modeshape) {
        ...
        case '4':
            ...
            break;
        default:
            di << "1st arg = mode, incorrect [give fsmw]\n";
            return 1;
    }

    //: k8 abv 6 Jan 98: using parameter for writing mode (assemblies/shapes)
    Handle(STEPControl_ActorWrite) ActWrite = Handle(STEPControl_ActorWrite)::DownCast(ctl->ActorWrite());
    if (!ActWrite.IsNull()) ActWrite->SetGroupMode(Interface_Static::IVal("write.step.assembly"));
    ...
    switch (stat) {
        ...
        case IFSelect_RetStop:
            di << "Error on writing file: no space on disk or destination is write protected\n";
            break;
        default:
            di << "Error: File " << nomfic << " written with fail messages\n";
            break;
    }
...
static Standard_Integer countexpected(Draw_Interpretor& di, Standard_Integer /*argc*/, const char** /*argv*/) {
...
static Standard_Integer dumpassembly(Draw_Interpretor& /*di*/, Standard_Integer /*argc*/, const char** /*argv*/) {
    Handle(IFSelect_SessionPilot) pilot = XSDRAW::Pilot();
    Handle(IFSelect_WorkSession) WS = pilot->Session();
    const Interface_Graph& graph = WS->Graph();
