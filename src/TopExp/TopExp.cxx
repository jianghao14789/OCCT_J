...
void TopExp::MapShapesAndUniqueAncestors(const TopoDS_Shape& S, const TopAbs_ShapeEnum TS, const TopAbs_ShapeEnum TA,
    ...
    while (exa.More()) {
        ...
        while (exs.More()) {
            ...
            for (; it.More(); it.Next())
                if (useOrientation ? anc.IsEqual(it.Value()) : anc.IsSame(it.Value())) break;
            if (!it.More()) aList.Append(anc);
            exs.Next();
        }
        exa.Next();
    }
    ...
    while (ex.More()) {
        Standard_Integer index = M.FindIndex(ex.Current());
        if (index == 0) M.Add(ex.Current(), empty);
        ex.Next();
    }
}
...
TopoDS_Vertex TopExp::FirstVertex(const TopoDS_Edge& E, const Standard_Boolean CumOri) {
    ...
    while (ite.More()) {
        if (ite.Value().Orientation() == TopAbs_FORWARD) return TopoDS::Vertex(ite.Value());
        ite.Next();
    }
...
TopoDS_Vertex TopExp::LastVertex(const TopoDS_Edge& E, const Standard_Boolean CumOri) {
    TopoDS_Iterator ite(E, CumOri);
    while (ite.More()) {
        if (ite.Value().Orientation() == TopAbs_REVERSED) return TopoDS::Vertex(ite.Value());
        ite.Next();
    }
    return TopoDS_Vertex();
}
...
void TopExp::Vertices(const TopoDS_Edge& E, TopoDS_Vertex& Vfirst, TopoDS_Vertex& Vlast,
    ...
    if (!isFirstDefined) Vfirst.Nullify();

    if (!isLastDefined) Vlast.Nullify();
}
