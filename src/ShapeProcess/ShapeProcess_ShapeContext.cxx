...
// purpose  :
//=======================================================================
ShapeProcess_ShapeContext::ShapeProcess_ShapeContext(const Standard_CString file, const Standard_CString seq)
    : ShapeProcess_Context(file, seq), myUntil(TopAbs_FACE), myNonManifold(Standard_False) {}
...
ShapeProcess_ShapeContext::ShapeProcess_ShapeContext(const TopoDS_Shape& S, const Standard_CString file,
...
static void RecModif(const TopoDS_Shape& S, const TopTools_DataMapOfShapeShape& repl,
                     const Handle(ShapeExtend_MsgRegistrator) & msg, TopTools_DataMapOfShapeShape& map,
                     Handle(ShapeExtend_MsgRegistrator) & myMsg, const TopAbs_ShapeEnum until) {
    TopoDS_Shape r = S;
    ...
    if (!r.IsNull()) {
        ...
        if (repl.IsBound(r.Located(aShLoc, Standard_False))) {
            ...
            if (r.Orientation() != TopAbs_FORWARD) res.Reverse();
...
static void RecModif(const TopoDS_Shape& S, const Handle(ShapeBuild_ReShape) & repl,
                     const Handle(ShapeExtend_MsgRegistrator) & msg, TopTools_DataMapOfShapeShape& map,
                     Handle(ShapeExtend_MsgRegistrator) & myMsg, const TopAbs_ShapeEnum until) {
    if (S.IsNull()) return;
    // gka  -modification to keep history for shape with location (OCC21617)
    ...
    if (!r.IsNull()) {
        TopoDS_Shape res;
        if (repl->Status(r, res, Standard_True) && res != r) map.Bind(aS, res);
...
void ShapeProcess_ShapeContext::AddMessage(const TopoDS_Shape& S, const Message_Msg& msg, const Message_Gravity grv) {
...
static void ExplodeModifier(const TopoDS_Shape& S, const BRepTools_Modifier& repl, TopTools_DataMapOfShapeShape& map,
...
void ShapeProcess_ShapeContext::RecordModification(const TopoDS_Shape& S, const BRepTools_Modifier& repl,
                                                   const Handle(ShapeExtend_MsgRegistrator) & msg) {
...
Standard_Boolean ShapeProcess_ShapeContext::GetContinuity(const Standard_CString param, GeomAbs_Shape& cont) const {
...
GeomAbs_Shape ShapeProcess_ShapeContext::ContinuityVal(const Standard_CString param, const GeomAbs_Shape def) const {
    GeomAbs_Shape val;
