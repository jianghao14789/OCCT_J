...
Standard_Boolean ShapeProcess::RegisterOperator(const Standard_CString name, const Handle(ShapeProcess_Operator) & op) {
...
Standard_Boolean ShapeProcess::FindOperator(const Standard_CString name, Handle(ShapeProcess_Operator) & op) {
    if (!aMapOfOperators.IsBound(name)) {
#ifdef OCCT_DEBUG
...
Standard_Boolean ShapeProcess::Perform(const Handle(ShapeProcess_Context) & context, const Standard_CString seq,
    ...
    for (i = 1; i <= sequenceOfOperators.Length() && aPS.More(); i++) {
        ...
        try {
            OCC_CATCH_SIGNALS
            if (op->Perform(context, aRange)) isDone = Standard_True;
