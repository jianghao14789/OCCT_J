...
static Standard_Boolean directfaces(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
...
static Standard_Boolean sameparam(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
    ...
    ShapeFix::SameParameter(ctx->Result(), ctx->BooleanVal("Force", Standard_False),
                            ctx->RealVal("Tolerance3d", Precision::Confusion() /* -1 */), Message_ProgressRange(), msg);
...
static Standard_Boolean settol(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
    ...
    Standard_Real reg;
    if (ctx->GetReal("Regularity", reg)) BRepLib::EncodeRegularity(ctx->Result(), reg);
...
static Standard_Boolean splitangle(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean bsplinerestriction(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
    ...
    Handle(ShapeCustom_BSplineRestriction) LD = new ShapeCustom_BSplineRestriction(
        ModeSurf, ModeC3d, ModeC2d, aTol3d, aTol2d, aCont3d, aCont2d, aMaxDeg, aMaxSeg, ModeDeg, Rational, aParameters);
...
static Standard_Boolean torevol(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean swepttoelem(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean shapetobezier(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean converttobspline(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean splitcontinuity(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean splitclosedfaces(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean fixfacesize(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean fixwgaps(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean dropsmallsolids(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
    ...
    if (ctx->GetInteger("FixMode", aMode)) FSS.SetFixMode(aMode);
    if (ctx->GetReal("VolumeThreshold", aThreshold)) FSS.SetVolumeThreshold(aThreshold);
    if (ctx->GetReal("WidthFactorThreshold", aThreshold)) FSS.SetWidthFactorThreshold(aThreshold);
...
static Standard_Boolean mergesmalledges(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
static Standard_Boolean spltclosededges(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
...
//=======================================================================
static Standard_Boolean splitcommonvertex(const Handle(ShapeProcess_Context) & context, const Message_ProgressRange&) {
    Handle(ShapeProcess_ShapeContext) ctx = Handle(ShapeProcess_ShapeContext)::DownCast(context);
    if (ctx.IsNull()) return Standard_False;
