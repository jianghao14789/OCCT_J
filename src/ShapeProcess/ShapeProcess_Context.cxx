...
ShapeProcess_Context::ShapeProcess_Context(const Standard_CString file, const Standard_CString scope) {
    Init(file, scope);
    myMessenger = Message::DefaultMessenger();
    myTraceLev = 1;
}
...
Standard_Boolean ShapeProcess_Context::Init(const Standard_CString file, const Standard_CString scope) {
    myScope.Nullify();
...
Handle(Resource_Manager) ShapeProcess_Context::LoadResourceManager(const Standard_CString name) {
    ...
    if (!sRC.IsNull()) {
        if (sName.IsEqual(name)) {
            ...
            if (isFileModified) sRC.Nullify();
        } else
            sRC.Nullify();
    }
    if (sRC.IsNull()) {
#ifdef OCCT_DEBUG
        std::cout << "Info: ShapeProcess_Context: Reload Resource_Manager: " << sName.ToCString() << " -> " << name
                  << std::endl;
#endif
...
void ShapeProcess_Context::UnSetScope() {
    if (!myScope.IsNull() && myScope->Length() > 0) myScope->Remove(myScope->Length());
}
...
static Handle(TCollection_HAsciiString)
    MakeName(const Handle(TColStd_HSequenceOfHAsciiString) & scope, const Standard_CString param) {
    Handle(TCollection_HAsciiString) str;
...
Standard_Boolean ShapeProcess_Context::GetString(const Standard_CString param, TCollection_AsciiString& str) const {
...
Standard_Boolean ShapeProcess_Context::GetReal(const Standard_CString param, Standard_Real& val) const {
    if (myRC.IsNull()) return Standard_False;
...
Standard_Boolean ShapeProcess_Context::GetInteger(const Standard_CString param, Standard_Integer& val) const {
    if (myRC.IsNull()) return Standard_False;
...
Standard_Boolean ShapeProcess_Context::GetBoolean(const Standard_CString param, Standard_Boolean& val) const {
    if (myRC.IsNull()) return Standard_False;
    try {
        OCC_CATCH_SIGNALS
...
Standard_Real ShapeProcess_Context::RealVal(const Standard_CString param, const Standard_Real def) const {
    Standard_Real val;
...
Standard_Boolean ShapeProcess_Context::BooleanVal(const Standard_CString param, const Standard_Boolean def) const {
...
Standard_Integer ShapeProcess_Context::IntegerVal(const Standard_CString param, const Standard_Integer def) const {
...
Standard_CString ShapeProcess_Context::StringVal(const Standard_CString param, const Standard_CString def) const {
