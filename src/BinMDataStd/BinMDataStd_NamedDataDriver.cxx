...
Standard_Boolean BinMDataStd_NamedDataDriver::Paste(const BinObjMgt_Persistent& theSource,
    ...
    if (!(theSource >> aLower >> anUpper)) return Standard_False;
    ...
    if (anUpper | aLower) {
        ...
        for (i = aLower; i <= anUpper; i++) {
            ...
            if (!(theSource >> aKey >> aValue)) return Standard_False;
    ...
    if (anUpper | aLower) {
        ...
        for (i = aLower; i <= anUpper; i++) {
            TCollection_ExtendedString aKey;
            if (!(theSource >> aKey)) return Standard_False;
            Standard_Integer low, up;
            if (!(theSource >> low >> up)) return Standard_False;
            if (up < low) return Standard_False;
            if (up | low) {
                Handle(TColStd_HArray1OfInteger) aTargetArray = new TColStd_HArray1OfInteger(low, up);
                if (!theSource.GetIntArray(&(aTargetArray->ChangeArray1())(low), up - low + 1)) return Standard_False;

                Standard_Boolean Ok = anIntArrays.Bind(aKey, aTargetArray);
                aResult |= Ok;
            }
        }
        if (aResult) T->ChangeArraysOfIntegers(anIntArrays);
    }
    ...
    if (anUpper | aLower) {
        ...
        for (i = aLower; i <= anUpper; i++) {
            ...
            if (low | up) {
                ...
                if (!theSource.GetRealArray(&(aTargetArray->ChangeArray1())(low), up - low + 1)) return Standard_False;
                Standard_Boolean Ok = aRealArrays.Bind(aKey, aTargetArray);
                aResult |= Ok;
            }
        }
        if (aResult) T->ChangeArraysOfReals(aRealArrays);
    }
...
//=======================================================================
void BinMDataStd_NamedDataDriver::Paste(const Handle(TDF_Attribute) & theSource, BinObjMgt_Persistent& theTarget,
