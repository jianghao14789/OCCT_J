...
public:
    ...
    MergeNodeTool(RWStl_Reader* theReader, const Standard_Integer theNbFacets = -1)
        : Poly_MergeNodesTool(theReader->MergeAngle(), 0.0, theNbFacets), myReader(theReader),
    ...
    void AddTriangle(const gp_XYZ theElemNodes[3]) {
        ...
        // this is a waste of time for most cases of sequential index adding, but preserved for keeping RWStl_Reader
        // interface
        ...
        if (aNodesRes[0] != aNodesRes[1] && aNodesRes[1] != aNodesRes[2] && aNodesRes[2] != aNodesRes[0]) {
...
inline static gp_XYZ readStlFloatVec3(const char* theData) {
    return gp_XYZ(readStlFloat(theData), readStlFloat(theData + sizeof(float)),
...
// purpose  :
//==============================================================================
RWStl_Reader::RWStl_Reader() : myMergeAngle(M_PI / 2.0), myMergeTolearance(0.0) {
    //
}
...
// purpose  :
//==============================================================================
Standard_Boolean RWStl_Reader::ReadAscii(Standard_IStream& theStream, Standard_ReadLineBuffer& theBuffer,
                                         const std::streampos theUntilPos, const Message_ProgressRange& theProgress) {
    ...
    while (aPS.More()) {
        ...
        for (Standard_Integer i = 0; i < 3; i++) {
            ...
            gp_XYZ aReadVertex;
            if (!ReadVertex(aLine, aReadVertex.ChangeCoord(1), aReadVertex.ChangeCoord(2),
...
Standard_Boolean RWStl_Reader::ReadBinary(Standard_IStream& theStream, const Message_ProgressRange& theProgress) {
    /*
    ...
    */
    ...
    for (Standard_Integer aNbFacetRead = 0; aNbFacetRead < aNbFacets && aPS.More();
        ...
        gp_XYZ aTriNodes[3] = {readStlFloatVec3(aBufferPtr + aVec3Size), readStlFloatVec3(aBufferPtr + aVec3Size * 2),
