...
Standard_Boolean StepToTopoDS_GeometricTool::IsSeamCurve(const Handle(StepGeom_SurfaceCurve) & SurfCurve,
                                                         const Handle(StepGeom_Surface) & Surf,
                                                         const Handle(StepShape_Edge) & StepEdge,
                                                         const Handle(StepShape_EdgeLoop) & EdgeLoop) {
    if (SurfCurve->IsKind(STANDARD_TYPE(StepGeom_SeamCurve))) return Standard_True;
...
Standard_Boolean StepToTopoDS_GeometricTool::IsLikeSeam(const Handle(StepGeom_SurfaceCurve) & SurfCurve,
                                                        const Handle(StepGeom_Surface) & Surf,
                                                        const Handle(StepShape_Edge) & StepEdge,
                                                        const Handle(StepShape_EdgeLoop) & EdgeLoop) {
    if (SurfCurve->NbAssociatedGeometry() != 2) return Standard_False;
    ...
    if ((!StepPCurve1.IsNull() && !StepPCurve2.IsNull()) && (StepPCurve1->BasisSurface() == Surf) &&
        ...
        if (nbOE == 1) {
            ...
            if (!line1.IsNull() && !line2.IsNull()) {
                ...
                Standard_Real DeltaX = Abs(line1->Pnt()->CoordinatesValue(1) - line2->Pnt()->CoordinatesValue(1));
                Standard_Real DeltaY = Abs(line1->Pnt()->CoordinatesValue(2) - line2->Pnt()->CoordinatesValue(2));
...
Standard_Boolean StepToTopoDS_GeometricTool::UpdateParam3d(const Handle(Geom_Curve) & theCurve, Standard_Real& w1,
                                                           Standard_Real& w2, const Standard_Real preci) {
