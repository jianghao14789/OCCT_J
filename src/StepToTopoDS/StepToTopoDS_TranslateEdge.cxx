...
// Purpose :
// ============================================================================
static void DecodeMakeEdgeError(const BRepLib_MakeEdge& ME, const Handle(Standard_Transient) & orig,
                                const Handle(Geom_Curve) & myCurve, const TopoDS_Vertex& V1, const TopoDS_Vertex& V2,
                                const Standard_Real& U1, const Standard_Real& U2, StepToTopoDS_Tool& aTool,
    ...
    switch (ME.Error()) {
        ...
        case (BRepLib_DifferentsPointAndParameter):
            if (!ShapeConstruct_Curve().AdjustCurve(myCurve, BRep_Tool::Pnt(V1), BRep_Tool::Pnt(V2), Standard_True,
                                                    Standard_True))
...
static Handle(Geom_Curve) MakeCurve(const Handle(StepGeom_Curve) & C1, const Handle(Transfer_TransientProcess) TP) {
    Handle(Geom_Curve) C2 = Handle(Geom_Curve)::DownCast(TP->FindTransient(C1));
    if (!C2.IsNull()) return C2;
    C2 = StepToGeom::MakeCurve(C1);
    if (!C2.IsNull()) TP->BindTransient(C1, C2);
    return C2;
}

static TopoDS_Edge MakeEdge(const Handle(Geom_Curve) & C3D, const TopoDS_Vertex& V1, const TopoDS_Vertex& V2,
...
StepToTopoDS_TranslateEdge::StepToTopoDS_TranslateEdge() : myError(StepToTopoDS_TranslateEdgeOther) {
    done = Standard_False;
}
...
StepToTopoDS_TranslateEdge::StepToTopoDS_TranslateEdge(const Handle(StepShape_Edge) & E, StepToTopoDS_Tool& T,
...
void StepToTopoDS_TranslateEdge::Init(const Handle(StepShape_Edge) & aEdge, StepToTopoDS_Tool& aTool,
                                      StepToTopoDS_NMTool& NMTool) {
    Handle(Transfer_TransientProcess) TP = aTool.TransientProcess();
    ...
    if (NMTool.IsActive() && NMTool.IsBound(EC)) {
        ...
        if (!OE->Orientation()) existingShape.Reverse();
    ...
    if (NMTool.IsIDEASCase() && !anECName.IsNull() && !anECName->IsEmpty() && NMTool.IsBound(anECName->String())) {
    ...
    if (done) {
        ...
        if (NMTool.IsActive()) {
            NMTool.Bind(EC, E);
            if (NMTool.IsIDEASCase() && !anECName.IsNull() && !anECName->IsEmpty()) NMTool.Bind(anECName->String(), E);
        }
...
static void GetCartesianPoints(const Handle(StepShape_EdgeCurve) & EC, gp_Pnt& P1, gp_Pnt& P2) {
...
void StepToTopoDS_TranslateEdge::MakeFromCurve3D(const Handle(StepGeom_Curve) & C3D,
                                                 ...
                                                 const Handle(StepShape_Vertex) & Vend, const Standard_Real preci,
                                                 TopoDS_Edge& E, TopoDS_Vertex& V1, TopoDS_Vertex& V2,
...
Handle(Geom2d_Curve) StepToTopoDS_TranslateEdge::MakePCurve(const Handle(StepGeom_Pcurve) & PCU,
                                                            const Handle(Geom_Surface) & ConvSurf) const {
    ...
    try {
        ...
        if (!C2d.IsNull()) {
            ...
            C2d = GeomConvert_Units::DegreeToRadian(C2d, ConvSurf, StepData_GlobalFactors::Intance().LengthFactor(),
                                                    StepData_GlobalFactors::Intance().FactorDegreeRadian());
        }
