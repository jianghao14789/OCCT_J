...
Standard_Boolean StepToTopoDS_MakeTransformed::Compute(const Handle(StepGeom_Axis2Placement3d) & Origin,
    ...
    if (theOrig.IsNull()) return Standard_False;
    Handle(Geom_Axis2Placement) theTarg = StepToGeom::MakeAxis2Placement(Target);
    if (theTarg.IsNull()) return Standard_False;
...
Standard_Boolean StepToTopoDS_MakeTransformed::Compute(const Handle(StepGeom_CartesianTransformationOperator3d) &
                                                       Operator) {
...
TopoDS_Shape StepToTopoDS_MakeTransformed::TranslateMappedItem(const Handle(StepRepr_MappedItem) & mapit,
    ...
    Handle(StepGeom_Axis2Placement3d) Target = Handle(StepGeom_Axis2Placement3d)::DownCast(mapit->MappingTarget());
