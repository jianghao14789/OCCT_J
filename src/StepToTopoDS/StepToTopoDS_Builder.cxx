...
StepToTopoDS_Builder::StepToTopoDS_Builder() : myError(StepToTopoDS_BuilderOther) {
    done = Standard_False;
}
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_ManifoldSolidBrep) & aManifoldSolid,
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_BrepWithVoids) & aBRepWithVoids,
                                const Handle(Transfer_TransientProcess) & TP,
                                const Message_ProgressRange& theProgress) {
    Message_Messenger::StreamBuffer sout = TP->Messenger()->SendInfo();
    // Initialisation of the Tool
    ...
    for (Standard_Integer i = 1; i <= Nb && PS.More(); i++) {
        ...
        if (myTranShell.IsDone()) {
            ...
            TP->AddWarning(aCShell, " A Void from BrepWithVoids not mapped to TopoDS");
        }
    }
    myResult = S;
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_FacetedBrep) & aFB, const Handle(Transfer_TransientProcess) & TP,
                                const Message_ProgressRange& theProgress) {
    // Initialisation of the Tool
    ...
    if (myTranShell.IsDone()) {
        ...
        TP->AddWarning(aCShell, " OuterShell from FacetedBrep not mapped to TopoDS");
        myError = StepToTopoDS_BuilderOther;
        done = Standard_True;
    }
}
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_FacetedBrepAndBrepWithVoids) & aFBABWV,
                                const Handle(Transfer_TransientProcess) & TP,
                                const Message_ProgressRange& theProgress) {
    // Initialisation of the Tool
    ...
    if (myTranShell.IsDone()) {
        ...
        for (i = 1; i <= Nb && aPS.More(); i++) {
            ...
            if (myTranShell.IsDone()) {
                ...
                TP->AddWarning(aCShell, " A Void from FacetedBrepAndBrepWithVoids not mapped to TopoDS");
            }
        }
        myResult = S;
        myError = StepToTopoDS_BuilderDone;
        ...
        TP->AddWarning(aCShell, " OuterShell from FacetedBrepAndBrepWithVoids not mapped to TopoDS");
        done = Standard_False;
        myError = StepToTopoDS_BuilderOther;
    }
}
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_ShellBasedSurfaceModel) & aSBSM,
                                const Handle(Transfer_TransientProcess) & TP, StepToTopoDS_NMTool& NMTool,
                                const Message_ProgressRange& theProgress) {
    Message_Messenger::StreamBuffer sout = TP->Messenger()->SendInfo();
    // Initialisation of the Tool
    ...
    for (Standard_Integer i = 1; i <= Nb && PS.More(); i++) {
        ...
        if (!aOpenShell.IsNull()) {
            ...
            if (myTranShell.IsDone()) {
                ...
                TP->AddWarning(aClosedShell, " ClosedShell from ShellBasedSurfaceModel not mapped to TopoDS");
            }
        }
    }
    if (Nb > 1)
...
static TopoDS_Face TranslateBoundedSurf(const Handle(StepGeom_Surface) & surf, const Standard_Real TolDegen) {
    TopoDS_Face res;
    ...
    Handle(Geom_RectangularTrimmedSurface) RS = Handle(Geom_RectangularTrimmedSurface)::DownCast(theSurf);
...
void StepToTopoDS_Builder::Init(const Handle(StepShape_GeometricSet) & GCS,
                                const Handle(Transfer_TransientProcess) & TP,
                                const Handle(Transfer_ActorOfTransientProcess) & RA, const Standard_Boolean isManifold,
                                const Message_ProgressRange& theProgress) {
    // Start Mapping
    TopoDS_Compound S;
    BRep_Builder B;
    ...
    for (i = 1; i <= nbElem && aPS.More(); i++) {
        ...
        // Element should finally be a Surface
            ...
            if (ent->IsKind(STANDARD_TYPE(StepGeom_CurveBoundedSurface))) {
                Handle(StepGeom_CurveBoundedSurface) CBS = Handle(StepGeom_CurveBoundedSurface)::DownCast(aSurf);
            ...
            // try other surfs
                ...
                res = TranslateBoundedSurf(aSurf, preci);
            ...
            if (!RA.IsNull()) {
                ...
                if (!anActor.IsNull()) binder = anActor->TransferShape(GRI, TP, isManifold, Standard_False, aRange);
                if (!binder.IsNull()) {
