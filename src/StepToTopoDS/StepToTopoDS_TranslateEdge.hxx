...
public:
    ...
    Standard_EXPORT StepToTopoDS_TranslateEdge(const Handle(StepShape_Edge) & E, StepToTopoDS_Tool& T,
    ...
    Standard_EXPORT void MakeFromCurve3D(const Handle(StepGeom_Curve) & C3D, const Handle(StepShape_EdgeCurve) & EC,
                                         const Handle(StepShape_Vertex) & Vend, const Standard_Real preci,
                                         TopoDS_Edge& E, TopoDS_Vertex& V1, TopoDS_Vertex& V2, StepToTopoDS_Tool& T);

    Standard_EXPORT Handle(Geom2d_Curve)
        MakePCurve(const Handle(StepGeom_Pcurve) & PCU, const Handle(Geom_Surface) & ConvSurf) const;
