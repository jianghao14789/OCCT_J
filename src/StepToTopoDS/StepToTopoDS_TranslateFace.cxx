...
// sln 01.10.2001 BUC61003. StepToTopoDS_TranslateFace::Init function is corrected (verifying  Handle(...).IsNull() is
// added)
...
// ============================================================================
StepToTopoDS_TranslateFace::StepToTopoDS_TranslateFace() : myError(StepToTopoDS_TranslateFaceOther) {
    done = Standard_False;
}
...
StepToTopoDS_TranslateFace::StepToTopoDS_TranslateFace(const Handle(StepShape_FaceSurface) & FS, StepToTopoDS_Tool& T,
...
void StepToTopoDS_TranslateFace::Init(const Handle(StepShape_FaceSurface) & FS, StepToTopoDS_Tool& aTool,
                                      StepToTopoDS_NMTool& NMTool) {
    done = Standard_True;
    if (aTool.IsBound(FS)) {
    ...
    // sln 01.10.2001 BUC61003. If corresponding entity was read with error StepSurface may be NULL. In this case we
    // exit from function
    if (StepSurf.IsNull()) {
    ...
    for (Standard_Integer i = 1; i <= NbBnd; i++) {
        ...
        if (Loop->IsKind(STANDARD_TYPE(StepShape_VertexLoop))) {
            ...
            if (GeomSurf->IsKind(STANDARD_TYPE(Geom_SphericalSurface)) ||
                ...
                //  Modification to create natural bounds for face based on the spherical and Bspline surface and having
                //  only one bound represented by Vertex loop was made. According to the specification of ISO - 10303
                //  part 42: "If the face has only one bound and this is of type vertex_loop, then the interior of the
                //  face is the domain of the face_surface.face_geometry.
    ...
    if (NMTool.IsActive()) NMTool.Bind(StepSurf, F);
