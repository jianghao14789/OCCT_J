...
static void CheckPCurves(TopoDS_Wire& aWire, const TopoDS_Face& aFace, const Standard_Boolean isPlane,
...
StepToTopoDS_TranslateEdgeLoop::StepToTopoDS_TranslateEdgeLoop() : myError(StepToTopoDS_TranslateEdgeLoopOther) {
    done = Standard_False;
}
...
StepToTopoDS_TranslateEdgeLoop::StepToTopoDS_TranslateEdgeLoop(const Handle(StepShape_FaceBound) & FB,
                                                               ...
                                                               const Standard_Boolean sameSense, StepToTopoDS_Tool& T,
...
void StepToTopoDS_TranslateEdgeLoop::Init(const Handle(StepShape_FaceBound) & FaceBound, const TopoDS_Face& Face,
                                          const Handle(Geom_Surface) & GeomSurf,
                                          const Handle(StepGeom_Surface) & StepSurf, const Standard_Boolean sameSense,
                                          StepToTopoDS_Tool& aTool, StepToTopoDS_NMTool& NMTool) {
    done = Standard_True;
    ...
    if (GeomSurf->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface))) {
        Handle(Geom_RectangularTrimmedSurface) theRTS = Handle(Geom_RectangularTrimmedSurface)::DownCast(GeomSurf);
    ...
    for (j = 1; j <= NbEdge; j++) {
        ...
        if (OrEdge1.IsNull() || OrEdge2.IsNull()) continue;
        ...
        TopoDS_Vertex V1, V2;
        if (myTranVertex1.IsDone()) V1 = TopoDS::Vertex(myTranVertex1.Value());
        if (myTranVertex2.IsDone()) V2 = TopoDS::Vertex(myTranVertex2.Value());
    ...
    for (j = 1; j <= NbEdge; j++) {
        ...
        OrEdge1 = EL->EdgeListValue(j);
        if (OrEdge1.IsNull() || OrEdge1->EdgeElement().IsNull()) continue;
            ...
            // -----------------------------------------
                ...
                if (isSeam || isLikeSeam) {
                    ...
                    if (StepPCurve1.IsNull() || StepPCurve2.IsNull())
                        hasPcurve = Standard_False; // smh : BUC60810
                    else {
    ...
    if (!aTool.ComputePCurve())
        for (TopoDS_Iterator EdgeIt(W); EdgeIt.More(); EdgeIt.Next()) {
            ...
            if (myEdgePro->IsFirstDone() && myEdgePro->IsLastDone()) {
                if (Abs(myEdgePro->FirstParam() - myEdgePro->LastParam()) < Precision::PConfusion()) continue;
