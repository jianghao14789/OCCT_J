...
public:
    ...
    public:
        ...
        StreamBuffer(const StreamBuffer& theOther) : myMessenger(theOther.myMessenger), myGravity(theOther.myGravity) {}

        //! Wrapper for operator << of the stream
        //! 流的 operator << 的包装器
        template <typename T> StreamBuffer& operator<<(const T& theArg) {
            myStream << theArg;
            return *this;
        }
    ...
    private:
        ...
        StreamBuffer(Message_Messenger* theMessenger, Message_Gravity theGravity)
            : myMessenger(theMessenger), myGravity(theGravity) {}
...
public:
    ...
    StreamBuffer Send(Message_Gravity theGravity) {
        return StreamBuffer(this, theGravity);
    }
    ...
    void SendWarning(const TCollection_AsciiString& theMessage) {
        Send(theMessage, Message_Warning);
    }
