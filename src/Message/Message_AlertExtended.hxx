...
public:
    ...
    Standard_EXPORT static Handle(Message_Alert)
        AddAlert(const Handle(Message_Report) & theReport, const Handle(Message_Attribute) & theAttribute,
...
public:
    ...
    //! 返回警报属性的容器
    const Handle(Message_Attribute) & Attribute() const {
        return myAttribute;
    }
    ...
    void SetAttribute(const Handle(Message_Attribute) & theAttribute) {
    ...
    //! @return 实例或 NULL
    Standard_EXPORT Handle(Message_CompositeAlerts)
        CompositeAlerts(const Standard_Boolean theToCreate = Standard_False);
