...
public:
    ...
    //! 空范围的构造函数
    Message_ProgressRange() : myParentScope(0), myStart(0.), myDelta(0.), myWasUsed(false) {}

    //! Copy constructor disarms the source
    //! 拷贝构造函数，复制原对象并禁用原对象
    Message_ProgressRange(const Message_ProgressRange& theOther)
        : myParentScope(theOther.myParentScope), myStart(theOther.myStart), myDelta(theOther.myDelta),
...
private:
    ...
    Message_ProgressRange(const Message_ProgressScope& theParent, Standard_Real theStart, Standard_Real theDelta)
        : myParentScope(&theParent), myStart(theStart), myDelta(theDelta), myWasUsed(false) {}
