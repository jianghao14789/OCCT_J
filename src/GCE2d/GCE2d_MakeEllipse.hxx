...
public:
    ...
    Standard_EXPORT GCE2d_MakeEllipse(const gp_Ax2d& MajorAxis, const Standard_Real MajorRadius,
                                      const Standard_Real MinorRadius, const Standard_Boolean Sense = Standard_True);
    ...
    //! The status is "InvertRadius" if MajorRadius < MinorRadius or
    //! "NegativeRadius" if MinorRadius < 0.
    Standard_EXPORT GCE2d_MakeEllipse(const gp_Ax22d& Axis, const Standard_Real MajorRadius,
    ...
    operator const Handle(Geom2d_Ellipse) & () const {
        return Value();
    }
