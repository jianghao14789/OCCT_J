...
public:
    BOPDS_TSR() : BOPTools_BoxTreeSelector(), myHasBRep(Standard_False), myIndex(-1) {}
    //
    virtual ~BOPDS_TSR() {}
    //
...
BOPDS_Iterator::BOPDS_Iterator()
    : myAllocator(NCollection_BaseAllocator::CommonBaseAllocator()), myRunParallel(Standard_False),
...
BOPDS_Iterator::BOPDS_Iterator(const Handle(NCollection_BaseAllocator) & theAllocator)
    : myAllocator(theAllocator), myLists(0, theAllocator), myRunParallel(Standard_False), myExtLists(0, theAllocator),
      myUseExt(Standard_False) {
    Standard_Integer i, aNb;
    //
    myDS = NULL;
...
void BOPDS_Iterator::Initialize(const TopAbs_ShapeEnum aType1, const TopAbs_ShapeEnum aType2) {
    ...
    if (iX >= 0) {
        BOPDS_VectorOfPair& aPairs = (myUseExt && iX < BOPDS_Iterator::NbExtInterfs()) ? myExtLists(iX) : myLists(iX);
...
// purpose:
//=======================================================================
void BOPDS_Iterator::Prepare(const Handle(IntTools_Context) & theCtx, const Standard_Boolean theCheckOBB,
...
// purpose:
//=======================================================================
void BOPDS_Iterator::Intersect(const Handle(IntTools_Context) & theCtx, const Standard_Boolean theCheckOBB,
                               const Standard_Real theFuzzyValue) {
    ...
    for (Standard_Integer iR = 0; iR < aNbR; ++iR) {
        ...
        for (; iPair < aNbPairs; ++iPair) {
            ...
            if (theCheckOBB) {
                ...
                if (anOBB1.IsOut(anOBB2)) continue;
            }
...
void BOPDS_Iterator::IntersectExt(const TColStd_MapOfInteger& theIndices) {
    ...
    for (Standard_Integer i = 0; i < aNb; ++i) {
        const BOPDS_ShapeInfo& aSI = myDS->ShapeInfo(i);
        if (!aSI.IsInterfering() || (aSI.ShapeType() == TopAbs_SOLID)) continue;
        //
    ...
    for (Standard_Integer k = 0; k < aNbV; ++k) {
        ...
        for (; itLI.More(); itLI.Next()) {
            ...
            // avoid interfering of the shape with its sub-shapes
            if (((iTI < iTJ) && aSI.HasSubShape(j)) || ((iTI > iTJ) && aSJ.HasSubShape(i))) continue;

            BOPDS_Pair aPair(i, j);
            if (aMPFence.Add(aPair)) {
                const Standard_Integer iX = BOPDS_Tools::TypeToInteger(aTI, aTJ);
                if (iX < BOPDS_Iterator::NbExtInterfs()) myExtLists(iX).Append(aPair);
            }
        }
    }
