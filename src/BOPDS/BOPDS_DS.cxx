...
//=======================================================================
BOPDS_DS::BOPDS_DS()
    : myAllocator(NCollection_BaseAllocator::CommonBaseAllocator()), myArguments(myAllocator), myRanges(0, myAllocator),
      myLines(0, myAllocator), myMapShapeIndex(100, myAllocator), myPaveBlocksPool(0, myAllocator),
      myMapPBCB(100, myAllocator), myFaceInfoPool(0, myAllocator), myShapesSD(100, myAllocator),
      myMapVE(100, myAllocator), myInterfTB(100, myAllocator), myInterfVV(0, myAllocator), myInterfVE(0, myAllocator),
      myInterfVF(0, myAllocator), myInterfEE(0, myAllocator), myInterfEF(0, myAllocator), myInterfFF(0, myAllocator),
      myInterfVZ(0, myAllocator), myInterfEZ(0, myAllocator), myInterfFZ(0, myAllocator), myInterfZZ(0, myAllocator),
...
BOPDS_DS::BOPDS_DS(const Handle(NCollection_BaseAllocator) & theAllocator)
    : myAllocator(theAllocator), myArguments(myAllocator), myRanges(0, myAllocator), myLines(0, myAllocator),
      myMapShapeIndex(100, myAllocator), myPaveBlocksPool(0, myAllocator), myMapPBCB(100, myAllocator),
      myFaceInfoPool(0, myAllocator), myShapesSD(100, myAllocator), myMapVE(100, myAllocator),
      myInterfTB(100, myAllocator), myInterfVV(0, myAllocator), myInterfVE(0, myAllocator), myInterfVF(0, myAllocator),
      myInterfEE(0, myAllocator), myInterfEF(0, myAllocator), myInterfFF(0, myAllocator), myInterfVZ(0, myAllocator),
      myInterfEZ(0, myAllocator), myInterfFZ(0, myAllocator), myInterfZZ(0, myAllocator),
      myInterfered(100, myAllocator) {
    myNbShapes = 0;
    myNbSourceShapes = 0;
}
...
void BOPDS_DS::Init(const Standard_Real theFuzz) {
    ...
    for (nE = 0; nE < myNbSourceShapes; ++nE) {
        ...
        if (aSI.ShapeType() != TopAbs_EDGE) continue;
        ...
        for (; aIt1.More(); aIt1.Next()) {
            ...
            if (!pLE) {
                ...
                for (aIt2.Initialize(*pLE); aIt2.More(); aIt2.Next()) {
                    if (aIt2.Value() == nE) break;
                }
                if (!aIt2.More()) pLE->Append(nE);
            }
        }
    }
    //-----------------------------------------------------scope_1 t
...
// purpose  :
//=======================================================================
Standard_Boolean BOPDS_DS::HasInterfShapeSubShapes(const Standard_Integer theI1, const Standard_Integer theI2,
...
// purpose  :
//=======================================================================
Standard_Boolean BOPDS_DS::HasInterfSubShapes(const Standard_Integer theI1, const Standard_Integer theI2) const {
    Standard_Boolean bRet;
    Standard_Integer n1;
    TColStd_ListIteratorOfListOfInteger aIt;
    bRet = Standard_False;
...
// purpose  :
//=======================================================================
void BOPDS_DS::UpdateCommonBlock(const Handle(BOPDS_CommonBlock) & theCB, const Standard_Real theFuzz) {
...
// purpose  :
//=======================================================================
void BOPDS_DS::SetCommonBlock(const Handle(BOPDS_PaveBlock) & thePB, const Handle(BOPDS_CommonBlock) & theCB) {
    if (IsCommonBlock(thePB)) {
...
// purpose  :
//=======================================================================
void BOPDS_DS::FaceInfoOn(const Standard_Integer theF, BOPDS_IndexedMapOfPaveBlock& theMPB,
...
// purpose  :
//=======================================================================
void BOPDS_DS::FaceInfoIn(const Standard_Integer theF, BOPDS_IndexedMapOfPaveBlock& theMPB,
                          TColStd_MapOfInteger& theMI) {
...
void BOPDS_DS::RefineFaceInfoIn() {
    for (Standard_Integer i = 0; i < myNbSourceShapes; ++i) {
        ...
        if (aMPBIn.IsEmpty() || aMPBOn.IsEmpty()) continue;

        BOPDS_IndexedMapOfPaveBlock aMPBInNew;

        const Standard_Integer aNbPBIn = aMPBIn.Extent();
        for (Standard_Integer j = 1; j <= aNbPBIn; ++j) {
            if (!aMPBOn.Contains(aMPBIn(j))) aMPBInNew.Add(aMPBIn(j));
        }

        if (aMPBInNew.Extent() < aNbPBIn) aMPBIn = aMPBInNew;
    }
}
...
// purpose  :
//=======================================================================
void BOPDS_DS::SubShapesOnIn(const Standard_Integer theNF1, const Standard_Integer theNF2,
                             TColStd_MapOfInteger& theMVOnIn, TColStd_MapOfInteger& theMVCommon,
                             BOPDS_IndexedMapOfPaveBlock& thePBOnIn, BOPDS_MapOfPaveBlock& theCommonPB) const {
...
// purpose  :
//=======================================================================
void BOPDS_DS::SharedEdges(const Standard_Integer nF1, const Standard_Integer nF2, TColStd_ListOfInteger& theLI,
...
void BOPDS_DS::AddShapeSD(const Standard_Integer theIndex, const Standard_Integer theIndexSD) {
    if (theIndex != theIndexSD) myShapesSD.Bind(theIndex, theIndexSD);
}
//=======================================================================
// function : HasShapeSD
// purpose  :
//=======================================================================
Standard_Boolean BOPDS_DS::HasShapeSD(const Standard_Integer theIndex, Standard_Integer& theIndexSD) const {
...
// purpose:
//=======================================================================
Standard_Boolean BOPDS_DS::CheckCoincidence(const Handle(BOPDS_PaveBlock) & aPB1, const Handle(BOPDS_PaveBlock) & aPB2,
...
// purpose  :
//=======================================================================
void BOPDS_DS::BuildBndBoxSolid(const Standard_Integer theIndex, Bnd_Box& aBoxS,
    ...
    for (; aItLI.More(); aItLI.Next()) {
        ...
        for (; aItLI1.More(); aItLI1.Next()) {
            ...
            //
            if (!bIsOpenBox) {
                bIsOpenBox = (aBFc.IsOpenXmin() || aBFc.IsOpenXmax() || aBFc.IsOpenYmin() || aBFc.IsOpenYmax() ||
...
Standard_Boolean BOPDS_DS::IsValidShrunkData(const Handle(BOPDS_PaveBlock) & thePB) {
    if (!thePB->HasShrunkData()) return Standard_False;
