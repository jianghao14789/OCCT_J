...
public:
    FuncPreciseSeam(const Handle(Adaptor3d_Surface) & theQSurf, // quadric
                    ...
                    const Standard_Boolean isTheUSeam, const Standard_Real theIsoParameter)
        : myQSurf(theQSurf), myPSurf(thePSurf), mySeamCoordInd(isTheUSeam ? 1 : 0), // Defines, U- or V-seam is used
    ...
    virtual Standard_Boolean Derivatives(const math_Vector& theX, math_Matrix& theD) {
        try {
            ...
            // d/dX2
            aD1[1].Coord(theD(anIndRD, anIndCD + 1), theD(anIndRD + 1, anIndCD + 1), theD(anIndRD + 2, anIndCD + 1));

            // d/dX3
            aD2[mySeamCoordInd].Reversed().Coord(theD(anIndRD, anIndCD + 2), theD(anIndRD + 1, anIndCD + 2),
    ...
    virtual Standard_Boolean Values(const math_Vector& theX, math_Vector& theF, math_Matrix& theD) {
        ...
        if (!Derivatives(theX, theD)) return Standard_False;
...
//=======================================================================
static inline void GetTangent(const Standard_Real theConeSemiAngle, const Standard_Real theParameter,
                              gp_XYZ& theResult) {
...
//           on theSurf.
//=======================================================================
static Standard_Boolean IsPointOnSurface(const Handle(Adaptor3d_Surface) & theSurf, const gp_Pnt& thePt,
                                         const Standard_Real theTol, gp_Pnt& theProjPt, Standard_Real& theUpar,
    ...
    switch (theSurf->GetType()) {
        ...
        case GeomAbs_SurfaceOfRevolution: {
            Extrema_ExtPS anExtr(thePt, *theSurf, theSurf->UResolution(theTol), theSurf->VResolution(theTol),
                                 Extrema_ExtFlag_MIN);
...
//=======================================================================
Standard_Boolean IntPatch_SpecialPoints::AddCrossUVIsoPoint(const Handle(Adaptor3d_Surface) & theQSurf,
                                                            const Handle(Adaptor3d_Surface) & thePSurf,
                                                            const IntSurf_PntOn2S& theRefPt, const Standard_Real theTol,
                                                            IntSurf_PntOn2S& theAddedPoint,
                                                            const Standard_Boolean theIsReversed) {
    Standard_Real anArrOfPeriod[4] = {0.0, 0.0, 0.0, 0.0};
    IntSurf::SetPeriod(theIsReversed ? thePSurf : theQSurf, theIsReversed ? theQSurf : thePSurf, anArrOfPeriod);
...
Standard_Boolean IntPatch_SpecialPoints::AddPointOnUorVIso(
    const Handle(Adaptor3d_Surface) & theQSurf, const Handle(Adaptor3d_Surface) & thePSurf,
    const IntSurf_PntOn2S& theRefPt, const Standard_Boolean theIsU, const Standard_Real theIsoParameter,
    const math_Vector& theToler, const math_Vector& theInitPoint, const math_Vector& theInfBound,
    const math_Vector& theSupBound, IntSurf_PntOn2S& theAddedPoint, const Standard_Boolean theIsReversed) {
    Standard_Real anArrOfPeriod[4] = {0.0, 0.0, 0.0, 0.0};
    IntSurf::SetPeriod(theIsReversed ? thePSurf : theQSurf, theIsReversed ? theQSurf : thePSurf, anArrOfPeriod);
...
*/
//=======================================================================
Standard_Boolean IntPatch_SpecialPoints::ProcessSphere(const IntSurf_PntOn2S& thePtIso, const gp_Vec& theDUofPSurf,
                                                       const gp_Vec& theDVofPSurf, const Standard_Boolean theIsReversed,
                                                       const Standard_Real theVquad, Standard_Real& theUquad,
    ...
    if ((Abs(theDUofPSurf.Z()) < Precision::PConfusion()) && (Abs(theDVofPSurf.Z()) < Precision::PConfusion())) {
        ...
        if (Abs(theDUofPSurf.Z()) > Abs(theDVofPSurf.Z())) {
            ...
            aV1.SetCoord(theDUofPSurf.X() * aDusDvs - theDVofPSurf.X(), theDUofPSurf.Y() * aDusDvs - theDVofPSurf.Y());
        } else {
            const Standard_Real aDvsDus = theDUofPSurf.Z() / theDVofPSurf.Z();
            aV1.SetCoord(theDVofPSurf.X() * aDvsDus - theDUofPSurf.X(), theDVofPSurf.Y() * aDvsDus - theDUofPSurf.Y());
        }
...
*/
//=======================================================================
Standard_Boolean IntPatch_SpecialPoints::ProcessCone(const IntSurf_PntOn2S& thePtIso, const gp_Vec& theDUofPSurf,
                                                     const gp_Vec& theDVofPSurf, const gp_Cone& theCone,
                                                     const Standard_Boolean theIsReversed, Standard_Real& theUquad,
                                                     Standard_Boolean& theIsIsoChoosen) {
    theIsIsoChoosen = Standard_False;
    ...
    const Standard_Integer aNbTangent =
        !theIsIsoChoosen ? GetTangentToIntLineForCone(theCone.SemiAngle(), aTgPlaneZ.Divided(Sqrt(aSqModTg)), aTgILine)
                         : 0;
    ...
    if (aNbTangent == 0) {
        ...
        for (Standard_Integer anIdx = 0; anIdx < aNbTangent; anIdx++) {
            ...
            if (aDelta > M_PI) aDelta = aPeriod - aDelta;
...
Standard_Integer IntPatch_SpecialPoints::GetTangentToIntLineForCone(const Standard_Real theConeSemiAngle,
                                                                    const gp_XYZ& thePlnNormal, gp_XYZ theResult[2]) {
...
Standard_Boolean IntPatch_SpecialPoints::AddSingularPole(const Handle(Adaptor3d_Surface) & theQSurf,
                                                         const Handle(Adaptor3d_Surface) & thePSurf,
                                                         const IntSurf_PntOn2S& thePtIso, IntPatch_Point& theVertex,
                                                         IntSurf_PntOn2S& theAddedPoint,
                                                         const Standard_Boolean theIsReversed,
    ...
    {
        if (!ProcessCone(thePtIso, aVecDu, aVecDv, theQSurf->Cone(), theIsReversed, aUquad, isIsoChoosen)) {
            return Standard_False;
        }
    }
...
Standard_Boolean IntPatch_SpecialPoints::ContinueAfterSpecialPoint(
    const Handle(Adaptor3d_Surface) & theQSurf, const Handle(Adaptor3d_Surface) & thePSurf,
    const IntSurf_PntOn2S& theRefPt, const IntPatch_SpecPntType theSPType, const Standard_Real theTol2D,
    IntSurf_PntOn2S& theNewPoint, const Standard_Boolean theIsReversed) {
    if (theSPType == IntPatch_SPntNone) return Standard_False;
    ...
    const Standard_Real anArrOfPeriod[4] = {
        theIsReversed ? aUpPeriod : aUqPeriod, theIsReversed ? aVpPeriod : aVqPeriod,
        theIsReversed ? aUqPeriod : aUpPeriod, theIsReversed ? aVqPeriod : aVpPeriod};
...
void IntPatch_SpecialPoints::AdjustPointAndVertex(const IntSurf_PntOn2S& theRefPoint,
                                                  const Standard_Real theArrPeriods[4], IntSurf_PntOn2S& theNewPoint,
    ...
    if (theVertex) (*theVertex).SetParameters(aPar[0], aPar[1], aPar[2], aPar[3]);
