...
static Handle(IntPatch_WLine)
    MakeNewWLine(const Handle(IntPatch_WLine) & theWLine, NCollection_Array1<Standard_Integer>& thePointsHash,
...
static void MovePoint(const Handle(Adaptor3d_Surface) & theS1, Standard_Real& U1, Standard_Real& V1) {
    if (U1 < theS1->FirstUParameter()) U1 = theS1->FirstUParameter();

    if (U1 > theS1->LastUParameter()) U1 = theS1->LastUParameter();

    if (V1 < theS1->FirstVParameter()) V1 = theS1->FirstVParameter();

    if (V1 > theS1->LastVParameter()) V1 = theS1->LastVParameter();
}
...
//=========================================================================
static Handle(IntPatch_WLine)
    DeleteOuterPoints(const Handle(IntPatch_WLine) & theWLine, const Handle(Adaptor3d_Surface) & theS1,
                      const Handle(Adaptor3d_Surface) & theS2, const Handle(Adaptor3d_TopolTool) & theDom1,
    ...
    if (theS1->IsUPeriodic() || theS1->IsVPeriodic() || theS2->IsUPeriodic() || theS2->IsVPeriodic()) return theWLine;
    ...
    for (i = 1; i <= theWLine->NbPnts(); i++) {
        ...
        if (aState1 == TopAbs_OUT || aState2 == TopAbs_OUT) {
            ...
            if (aDelOuterPointsHash(i) == -1) aFirstGeomIdx = i; // Use data what lies in (i) point / vertex.
    ...
    for (i = theWLine->NbPnts(); i >= 1; i--) {
        ...
        if (aState1 == TopAbs_OUT || aState2 == TopAbs_OUT) {
            ...
            if (aDelOuterPointsHash(i) == -1) aLastGeomIdx = i; // Use data what lies in (i) point / vertex.
...
//=========================================================================
static Standard_Boolean IsInsideIn2d(const gp_Pnt2d& aBasePnt, const gp_Vec2d& aBaseVec, const gp_Pnt2d& aNextPnt,
...
//=========================================================================
static Standard_Boolean IsInsideIn3d(const gp_Pnt& aBasePnt, const gp_Vec& aBaseVec, const gp_Pnt& aNextPnt,
                                     const Standard_Real aSquareMaxDist) {
...
static Handle(IntPatch_WLine)
    DeleteByTube(const Handle(IntPatch_WLine) & theWLine, const Handle(Adaptor3d_Surface) & theS1,
    ...
    if (theWLine->NbPnts() <= 2) return theWLine;
    ...
    Standard_Real aResS1Tol = Min(theS1->UResolution(aBaseTolerance), theS1->VResolution(aBaseTolerance));
    Standard_Real aResS2Tol = Min(theS2->UResolution(aBaseTolerance), theS2->VResolution(aBaseTolerance));
    ...
    if (aNewPointsHash(theWLine->NbPnts() - 1) == -1 && aNewPointsHash(theWLine->NbPnts()) == -1 && aNbPnt <= 3) {
        // Delete last.
    ...
    if (aNbPnt + 2 > aMinNbBadDistr && aNbPnt + 2 < aNbSingleBezier) {
        for (Standard_Integer anIdx = 1; anIdx <= 8; anIdx++) {
            ...
            if (aNewPointsHash(aHashIdx) != -1) aNewPointsHash(aHashIdx) = 0;
        }
    }
...
//=======================================================================
static Standard_Boolean IsSeamOrBound(const IntSurf_PntOn2S& thePtf, const IntSurf_PntOn2S& thePtl,
                                      const IntSurf_PntOn2S& thePtmid, const Standard_Real theArrPeriods[4],
                                      const Standard_Real theFBound[4], const Standard_Real theLBound[4]) {
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        ...
        if (aBndR[i].IsIntersected(theFBound[i], theArrPeriods[i]) == 1) return Standard_True;

        if (aBndR[i].IsIntersected(theLBound[i], theArrPeriods[i]) == 1) return Standard_True;
    }
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        ...
        if (aBndR[i].IsIntersected(0.0, theArrPeriods[i]) == 1) return Standard_True;
    }
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        ...
        if (aBR.IsIntersected(theFBound[i], theArrPeriods[i])) return Standard_True;

        if (aBR.IsIntersected(theLBound[i], theArrPeriods[i])) return Standard_True;

        if (aBR.IsIntersected(0.0, theArrPeriods[i])) return Standard_True;
    }
...
//=======================================================================
static Standard_Boolean IsIntersectionPoint(const gp_Pnt& thePmid, const Handle(Adaptor3d_Surface) & theS1,
                                            const Handle(Adaptor3d_Surface) & theS2, const IntSurf_PntOn2S& theRefPt,
                                            const Standard_Real theTol, const Standard_Real* const theArrPeriods,
...
static void ExtendFirst(const Handle(IntPatch_WLine) & theWline, const IntSurf_PntOn2S& theAddedPt) {
    ...
    if (theAddedPt.IsSame(theWline->Point(1), Precision::Confusion())) {
        ...
        for (Standard_Integer i = 1; i <= theWline->NbVertex(); i++) {
            ...
            if (aVert.ParameterOnLine() != 1) break;
...
static void ExtendLast(const Handle(IntPatch_WLine) & theWline, const IntSurf_PntOn2S& theAddedPt) {
    ...
    for (Standard_Integer i = theWline->NbVertex(); i >= 1; i--) {
        IntPatch_Point& aVert = theWline->ChangeVertex(i);
        if (aVert.ParameterOnLine() != aNbPnts) break;
...
//=========================================================================
static Standard_Boolean IsOutOfDomain(const Bnd_Box2d& theBoxS1, const Bnd_Box2d& theBoxS2,
                                      const IntSurf_PntOn2S& thePOn2S, const Standard_Real* const theArrPeriods) {
    Standard_Real aU1 = 0.0, aV1 = 0.0, aU2 = 0.0, aV2 = 0.0;
...
CheckArgumentsToExtend(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                       const IntSurf_PntOn2S& thePtWL1, const IntSurf_PntOn2S& thePtWL2, IntSurf_PntOn2S& theNewPoint,
                       const gp_Vec& theVec1, const gp_Vec& theVec2, const gp_Vec& theVec3, const Bnd_Box2d& theBoxS1,
                       const Bnd_Box2d& theBoxS2, const Standard_Real theToler3D,
    ...
    for (Standard_Integer i = 0; i < 4; i++) {
        ...
        if (aR1.IsIntersected(aParLBC[i], theArrPeriods[i])) {
            ...
            if (aR1.IsIntersected(aParLBC[i], theArrPeriods[i]) || aR2.IsIntersected(aParLBC[i], theArrPeriods[i])) {
                return IntPatchWT_NotConnected;
            } else {
...
//=======================================================================
Standard_Boolean CheckArgumentsToJoin(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                                      const IntSurf_PntOn2S& thePnt, const gp_Pnt& theP1, const gp_Pnt& theP2,
                                      const gp_Pnt& theP3, const Standard_Real theMinRad) {
    ...
    if (aV12f.Angle(aV12l) > IntPatch_WLineTool::myMaxConcatAngle) return Standard_False;
...
//=======================================================================
static void ExtendTwoWLFirstFirst(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                                  const Handle(IntPatch_WLine) & theWLine1, const Handle(IntPatch_WLine) & theWLine2,
                                  const IntSurf_PntOn2S& thePtWL1, const IntSurf_PntOn2S& thePtWL2,
                                  const gp_Vec& theVec1, const gp_Vec& theVec2, const gp_Vec& theVec3,
                                  const Bnd_Box2d& theBoxS1, const Bnd_Box2d& theBoxS2, const Standard_Real theToler3D,
    ...
    const IntPatchWT_WLsConnectionType aCheckRes =
        CheckArgumentsToExtend(theS1, theS2, thePtWL1, thePtWL2, aPOn2S, theVec1, theVec2, theVec3, theBoxS1, theBoxS2,
    ...
    if (theHasBeenJoined || (aCheckRes == IntPatchWT_Singular)) return;
...
//=======================================================================
static void ExtendTwoWLFirstLast(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                                 const Handle(IntPatch_WLine) & theWLine1, const Handle(IntPatch_WLine) & theWLine2,
                                 const IntSurf_PntOn2S& thePtWL1, const IntSurf_PntOn2S& thePtWL2,
                                 const gp_Vec& theVec1, const gp_Vec& theVec2, const gp_Vec& theVec3,
                                 const Bnd_Box2d& theBoxS1, const Bnd_Box2d& theBoxS2, const Standard_Real theToler3D,
...
//=======================================================================
static void ExtendTwoWLLastFirst(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                                 const Handle(IntPatch_WLine) & theWLine1, const Handle(IntPatch_WLine) & theWLine2,
                                 const IntSurf_PntOn2S& thePtWL1, const IntSurf_PntOn2S& thePtWL2,
                                 const gp_Vec& theVec1, const gp_Vec& theVec2, const gp_Vec& theVec3,
                                 const Bnd_Box2d& theBoxS1, const Bnd_Box2d& theBoxS2, const Standard_Real theToler3D,
...
// purpose  :
//=======================================================================
static void ExtendTwoWLLastLast(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_Surface) & theS2,
                                const Handle(IntPatch_WLine) & theWLine1, const Handle(IntPatch_WLine) & theWLine2,
                                const IntSurf_PntOn2S& thePtWL1, const IntSurf_PntOn2S& thePtWL2, const gp_Vec& theVec1,
                                const gp_Vec& theVec2, const gp_Vec& theVec3, const Bnd_Box2d& theBoxS1,
                                const Bnd_Box2d& theBoxS2, const Standard_Real theToler3D,
                                const Standard_Real* const theArrPeriods, unsigned int& theCheckResult,
                                Standard_Boolean& theHasBeenJoined) {
    IntSurf_PntOn2S aPOn2S;
...
// purpose  :
//=========================================================================
Handle(IntPatch_WLine) IntPatch_WLineTool::ComputePurgedWLine(const Handle(IntPatch_WLine) & theWLine,
    ...
    if (nb == 2) {
        ...
        if (p1.Value().IsEqual(p2.Value(), gp::Resolution())) return aResult;
    }
    ...
    for (i = 1; i <= aLineOn2S->NbPoints(); i++) {
        ...
        if ((aStartIndex > nb) || (anEndIndex <= 1)) continue;
        ...
        while (k <= anEndIndex) {
            if (i != k) {
                ...
                for (Standard_Integer anIdx = 1; anIdx < 8; anIdx++) {
                    if (aMax < Abs(UV[anIdx])) aMax = Abs(UV[anIdx]);
                }
    ...
    if (theS1->UContinuity() == GeomAbs_C0 || theS1->VContinuity() == GeomAbs_C0 ||
        theS2->UContinuity() == GeomAbs_C0 || theS2->VContinuity() == GeomAbs_C0 || nb < aNbSingleBezier) {
        return aLocalWLine;
    }
...
// purpose  :
//=======================================================================
void IntPatch_WLineTool::JoinWLines(IntPatch_SequenceOfLine& theSlin, IntPatch_SequenceOfPoint& theSPnt,
                                    Handle(Adaptor3d_Surface) theS1, Handle(Adaptor3d_Surface) theS2,
                                    const Standard_Real theTol3D) {
    if (theSlin.Length() == 0) return;

    // For two cylindrical surfaces only
    const Standard_Real aMinRad = 1.0e-3 * Min(theS1->Cylinder().Radius(), theS2->Cylinder().Radius());

    const Standard_Real anArrPeriods[4] = {
        theS1->IsUPeriodic() ? theS1->UPeriod() : 0.0, theS1->IsVPeriodic() ? theS1->VPeriod() : 0.0,
        theS2->IsUPeriodic() ? theS2->UPeriod() : 0.0, theS2->IsVPeriodic() ? theS2->VPeriod() : 0.0};

    const Standard_Real anArrFBonds[4] = {theS1->FirstUParameter(), theS1->FirstVParameter(), theS2->FirstUParameter(),
                                          theS2->FirstVParameter()};
    const Standard_Real anArrLBonds[4] = {theS1->LastUParameter(), theS1->LastVParameter(), theS2->LastUParameter(),
    ...
    for (Standard_Integer aN1 = 1; aN1 <= theSlin.Length(); aN1++) {
        ...
        if (aWLine1.IsNull()) { // We must have failed to join not-point-lines
            continue;
        }
        ...
        for (Standard_Integer aNPt = 1; aNPt <= theSPnt.Length(); aNPt++) {
            ...
            if (aPntCur.IsSame(aPntFWL1, Precision::Confusion()) || aPntCur.IsSame(aPntLWL1, Precision::Confusion())) {
                theSPnt.Remove(aNPt);
                aNPt--;
            }
        }
        ...
        if (isFirstConnected) {
            ...
            if (!CheckArgumentsToJoin(theS1, theS2, aPntFWL1, aPt1.Value(), aPntFWL1.Value(), aPt2.Value(), aMinRad)) {
                continue;
            }
        ...
        } else // if (isLastConnected)
        {
            ...
            if (!CheckArgumentsToJoin(theS1, theS2, aPntLWL1, aPt1.Value(), aPntLWL1.Value(), aPt2.Value(), aMinRad)) {
                continue;
            }
...
//=======================================================================
void IntPatch_WLineTool::ExtendTwoWLines(IntPatch_SequenceOfLine& theSlin, const Handle(Adaptor3d_Surface) & theS1,
                                         const Handle(Adaptor3d_Surface) & theS2, const Standard_Real theToler3D,
                                         const Standard_Real* const theArrPeriods, const Bnd_Box2d& theBoxS1,
                                         const Bnd_Box2d& theBoxS2,
                                         const NCollection_List<gp_Pnt>& theListOfCriticalPoints) {
    if (theSlin.Length() < 2) return;
    ...
    for (Standard_Integer aNumOfLine1 = 1; aNumOfLine1 <= theSlin.Length(); aNumOfLine1++) {
        ...
        if (aWLine1->Vertex(1).ParameterOnLine() != 1) continue;

        if (aWLine1->Vertex(aWLine1->NbVertex()).ParameterOnLine() != aWLine1->NbPnts()) continue;
        ...
        for (Standard_Integer aNumOfLine2 = aNumOfLine1 + 1; aNumOfLine2 <= theSlin.Length(); aNumOfLine2++) {
            ...
            if (aPntLWL1.IsSame(aPntFWL2, theToler3D) || aPntLWL1.IsSame(aPntFWL2, theToler3D)) {
                aCheckResult |= IntPatchWT_DisLastFirst | IntPatchWT_DisLastLast;
            }

            if (!theListOfCriticalPoints.IsEmpty()) {
                for (NCollection_List<gp_Pnt>::Iterator anItr(theListOfCriticalPoints); anItr.More(); anItr.Next()) {
        ...
        if (aCheckResult ==
            (IntPatchWT_DisFirstFirst | IntPatchWT_DisFirstLast | IntPatchWT_DisLastFirst | IntPatchWT_DisLastLast))
            continue;
        ...
        for (Standard_Integer aNumOfLine2 = aNumOfLine1 + 1; aNumOfLine2 <= theSlin.Length(); aNumOfLine2++) {
            ...
            if (aWLine2->Vertex(1).ParameterOnLine() != 1) continue;

            if (aWLine2->Vertex(aWLine2->NbVertex()).ParameterOnLine() != aWLine2->NbPnts()) continue;
            ...
            { // First/First
                ...
                ExtendTwoWLFirstFirst(theS1, theS2, aWLine1, aWLine2, aPntFWL1, aPntFWL2, aVec1, aVec2, aVec3, theBoxS1,
                                      theBoxS2, theToler3D, theArrPeriods, aCheckResult, hasBeenJoined);
            }

            if (!(aCheckResult & IntPatchWT_DisFirstLast)) { // First/Last
                aVec1.SetXYZ(aPntFp1WL1.Value().XYZ() - aPntFWL1.Value().XYZ());
                aVec2.SetXYZ(aPntLWL2.Value().XYZ() - aPntLm1WL2.Value().XYZ());
                aVec3.SetXYZ(aPntFWL1.Value().XYZ() - aPntLWL2.Value().XYZ());

                ExtendTwoWLFirstLast(theS1, theS2, aWLine1, aWLine2, aPntFWL1, aPntLWL2, aVec1, aVec2, aVec3, theBoxS1,
                                     theBoxS2, theToler3D, theArrPeriods, aCheckResult, hasBeenJoined);
            }

            if (!(aCheckResult & IntPatchWT_DisLastFirst)) { // Last/First
                ...
                ExtendTwoWLLastFirst(theS1, theS2, aWLine1, aWLine2, aPntLWL1, aPntFWL2, aVec1, aVec2, aVec3, theBoxS1,
                                     theBoxS2, theToler3D, theArrPeriods, aCheckResult, hasBeenJoined);
            }

            if (!(aCheckResult & IntPatchWT_DisLastLast)) { // Last/Last
                aVec1.SetXYZ(aPntLWL1.Value().XYZ() - aPntLm1WL1.Value().XYZ());
                aVec2.SetXYZ(aPntLm1WL2.Value().XYZ() - aPntLWL2.Value().XYZ());
                aVec3.SetXYZ(aPntLWL2.Value().XYZ() - aPntLWL1.Value().XYZ());

                ExtendTwoWLLastLast(theS1, theS2, aWLine1, aWLine2, aPntLWL1, aPntLWL2, aVec1, aVec2, aVec3, theBoxS1,
                                    theBoxS2, theToler3D, theArrPeriods, aCheckResult, hasBeenJoined);
            }
