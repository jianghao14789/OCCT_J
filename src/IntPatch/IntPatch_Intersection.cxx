...
IntPatch_Intersection::IntPatch_Intersection()
    : done(Standard_False), empt(Standard_True), tgte(Standard_False), oppo(Standard_False), myTolArc(0.0),
      myTolTang(0.0), myUVMaxStep(0.0), myFleche(0.0), myIsStartPnt(Standard_False), myU1Start(0.0), myV1Start(0.0),
      myU2Start(0.0), myV2Start(0.0) {}
...
IntPatch_Intersection::IntPatch_Intersection(const Handle(Adaptor3d_Surface) & S1,
    ...
    : done(Standard_False), empt(Standard_True), tgte(Standard_False), oppo(Standard_False), myTolArc(TolArc),
      myTolTang(TolTang), myUVMaxStep(0.0), myFleche(0.0), myIsStartPnt(Standard_False), myU1Start(0.0), myV1Start(0.0),
      myU2Start(0.0), myV2Start(0.0) {
...
//======================================================================
IntPatch_Intersection::IntPatch_Intersection(const Handle(Adaptor3d_Surface) & S1,
                                             const Handle(Adaptor3d_TopolTool) & D1, const Standard_Real TolArc,
                                             const Standard_Real TolTang)
    : done(Standard_False), empt(Standard_True), tgte(Standard_False), oppo(Standard_False), myTolArc(TolArc),
      myTolTang(TolTang), myUVMaxStep(0.0), myFleche(0.0), myIsStartPnt(Standard_False), myU1Start(0.0), myV1Start(0.0),
      myU2Start(0.0), myV2Start(0.0) {
...
//======================================================================
void IntPatch_Intersection::SetTolerances(const Standard_Real TolArc, const Standard_Real TolTang,
                                          const Standard_Real UVMaxStep, const Standard_Real Fleche) {
    myTolArc = TolArc;
    myTolTang = TolTang;
...
// function: Perform
//======================================================================
void IntPatch_Intersection::Perform(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_TopolTool) & D1,
    ...
    switch (S1->GetType()) {
        ...
        case GeomAbs_SurfaceOfExtrusion: {
            ...
            Geom2dAdaptor_Curve AC(aPCurve, aProjectedCurve.FirstParameter(), aProjectedCurve.LastParameter());
            Geom2dInt_GInter Intersector(AC, Precision::Confusion(), Precision::Confusion());
            if (Intersector.IsDone() && Intersector.IsEmpty()) break;
        }
...
//==========================================================================
static void FUN_TrimInfSurf(const gp_Pnt& Pmin, const gp_Pnt& Pmax, const Handle(Adaptor3d_Surface) & InfSurf,
                            const Standard_Real& AlternativeTrimPrm, Handle(Adaptor3d_Surface) & TrimS) {
...
//================================================================================
static void FUN_GetUiso(const Handle(Geom_Surface) & GS, const GeomAbs_SurfaceType& T, const Standard_Real& FirstV,
                        const Standard_Real& LastV, const Standard_Boolean& IsVC, const Standard_Boolean& IsVP,
                        const Standard_Real& U, Handle(Geom_Curve) & I) {
...
//================================================================================
static void FUN_GetViso(const Handle(Geom_Surface) & GS, const GeomAbs_SurfaceType& T, const Standard_Real& FirstU,
                        const Standard_Real& LastU, const Standard_Boolean& IsUC, const Standard_Boolean& IsUP,
                        const Standard_Real& V, Handle(Geom_Curve) & I) {
    if (T != GeomAbs_OffsetSurface) {
...
//================================================================================
static void FUN_PL_Intersection(const Handle(Adaptor3d_Surface) & S1, const GeomAbs_SurfaceType& T1,
                                const Handle(Adaptor3d_Surface) & S2, const GeomAbs_SurfaceType& T2,
                                Standard_Boolean& IsOk, TColgp_SequenceOfPnt& SP, gp_Vec& DV) {
    IsOk = Standard_False;
    ...
    Standard_Boolean IsBothLines = ((isoS1isLine[0] || isoS1isLine[1]) && (isoS2isLine[0] || isoS2isLine[1]));
    if (!IsBothLines) {
        return;
    }
    ...
    if (isoS1isLine[0] && isoS2isLine[0] && derS1[1].IsParallel(derS2[1], Precision::Angular())) {
        iso = 1;
        FUN_GetViso(gs1, T1, S1->FirstUParameter(), S1->LastUParameter(), S1->IsUClosed(), S1->IsUPeriodic(), MS1[1],
                    C1);
        FUN_GetViso(gs2, T2, S2->FirstUParameter(), S2->LastUParameter(), S2->IsUClosed(), S2->IsUPeriodic(), MS2[1],
                    C2);
    } else if (isoS1isLine[0] && isoS2isLine[1] && derS1[1].IsParallel(derS2[0], Precision::Angular())) {
        iso = 1;
        FUN_GetViso(gs1, T1, S1->FirstUParameter(), S1->LastUParameter(), S1->IsUClosed(), S1->IsUPeriodic(), MS1[1],
                    C1);
        FUN_GetUiso(gs2, T2, S2->FirstVParameter(), S2->LastVParameter(), S2->IsVClosed(), S2->IsVPeriodic(), MS2[0],
                    C2);
    } else if (isoS1isLine[1] && isoS2isLine[0] && derS1[0].IsParallel(derS2[1], Precision::Angular())) {
        iso = 0;
        FUN_GetUiso(gs1, T1, S1->FirstVParameter(), S1->LastVParameter(), S1->IsVClosed(), S1->IsVPeriodic(), MS1[0],
                    C1);
        FUN_GetViso(gs2, T2, S2->FirstUParameter(), S2->LastUParameter(), S2->IsUClosed(), S2->IsUPeriodic(), MS2[1],
                    C2);
...
//================================================================================
static void FUN_NewFirstLast(const GeomAbs_CurveType& ga_ct, const Standard_Real& Fst, const Standard_Real& Lst,
                             const Standard_Real& TrVal, Standard_Real& NewFst, Standard_Real& NewLst,
...
//================================================================================
static void FUN_TrimBothSurf(const Handle(Adaptor3d_Surface) & S1, const GeomAbs_SurfaceType& T1,
                             const Handle(Adaptor3d_Surface) & S2, const GeomAbs_SurfaceType& T2,
                             const Standard_Real& TV, Handle(Adaptor3d_Surface) & NS1,
...
// purpose  :
//=======================================================================
void IntPatch_Intersection::Perform(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_TopolTool) & theD1,
                                    const Handle(Adaptor3d_Surface) & theS2, const Handle(Adaptor3d_TopolTool) & theD2,
                                    const Standard_Real TolArc, const Standard_Real TolTang,
                                    const Standard_Boolean isGeomInt, const Standard_Boolean theIsReqToKeepRLine,
                                    const Standard_Boolean theIsReqToPostWLProc) {
    myTolArc = TolArc;
    myTolTang = TolTang;
    if (myFleche <= Precision::PConfusion()) myFleche = 0.01;
    if (myUVMaxStep <= Precision::PConfusion()) myUVMaxStep = 0.01;
    ...
    //
    if (typs1 == GeomAbs_Cone || typs2 == GeomAbs_Cone || typs1 == GeomAbs_Torus || typs2 == GeomAbs_Torus) {
        gp_Ax1 aCTAx, aGeomAx;
        GeomAbs_SurfaceType aCTType;
        Standard_Boolean bToCheck;
        //
        const Handle(Adaptor3d_Surface) & aCTSurf = (typs1 == GeomAbs_Cone || typs1 == GeomAbs_Torus) ? theS1 : theS2;
        const Handle(Adaptor3d_Surface) & aGeomSurf = (typs1 == GeomAbs_Cone || typs1 == GeomAbs_Torus) ? theS2 : theS1;
        //
    ...
    if (ts1 == ts2 && ts1 == 1) {
        ...
        ListOfPnts.Clear();
        if (isGeomInt) {
            GeomGeomPerfom(theS1, theD1, theS2, theD2, TolArc, TolTang, ListOfPnts, typs1, typs2, theIsReqToKeepRLine);
        } else {
...
void IntPatch_Intersection::Perform(const Handle(Adaptor3d_Surface) & theS1, const Handle(Adaptor3d_TopolTool) & theD1,
                                    const Handle(Adaptor3d_Surface) & theS2, const Handle(Adaptor3d_TopolTool) & theD2,
                                    const Standard_Real TolArc, const Standard_Real TolTang,
                                    IntSurf_ListOfPntOn2S& ListOfPnts, const Standard_Boolean isGeomInt,
...
void IntPatch_Intersection::ParamParamPerfom(const Handle(Adaptor3d_Surface) & theS1,
                                             ...
                                             const Handle(Adaptor3d_TopolTool) & theD2, const Standard_Real TolArc,
                                             const Standard_Real TolTang, IntSurf_ListOfPntOn2S& ListOfPnts,
                                             const GeomAbs_SurfaceType typs1, const GeomAbs_SurfaceType typs2) {
    IntPatch_PrmPrmIntersection interpp;
    //
    if (!theD1->DomainIsInfinite() && !theD2->DomainIsInfinite()) {
...
void IntPatch_Intersection::GeomGeomPerfom(const Handle(Adaptor3d_Surface) & theS1,
                                           ...
                                           const Handle(Adaptor3d_TopolTool) & theD2, const Standard_Real TolArc,
                                           const Standard_Real TolTang, IntSurf_ListOfPntOn2S& ListOfPnts,
                                           const GeomAbs_SurfaceType theTyps1, const GeomAbs_SurfaceType theTyps2,
                                           const Standard_Boolean theIsReqToKeepRLine) {
    IntPatch_ImpImpIntersection interii(theS1, theD1, theS2, theD2, myTolArc, myTolTang, theIsReqToKeepRLine);
    ...
    for (Standard_Integer i = 1; i <= interii.NbLines(); i++) {
        ...
        if (line->ArcType() == IntPatch_Analytic) {
            ...
            if ((line->ArcType() != IntPatch_Restriction) || theIsReqToKeepRLine) slin.Append(line);
        }
    }
    ...
    if (isWLExist) {
        ...
        const Standard_Real anArrOfPeriod[4] = {
        ...
        IntPatch_WLineTool::ExtendTwoWLines(slin, theS1, theS2, TolTang, anArrOfPeriod, aBx1, aBx2,
...
void IntPatch_Intersection::GeomParamPerfom(const Handle(Adaptor3d_Surface) & theS1,
                                            ...
                                            const Standard_Boolean isNotAnalitical, const GeomAbs_SurfaceType typs1,
...
void IntPatch_Intersection::Perform(const Handle(Adaptor3d_Surface) & S1, const Handle(Adaptor3d_TopolTool) & D1,
                                    const Handle(Adaptor3d_Surface) & S2, const Handle(Adaptor3d_TopolTool) & D2,
                                    const Standard_Real U1, const Standard_Real V1, const Standard_Real U2,
                                    const Standard_Real V2, const Standard_Real TolArc, const Standard_Real TolTang) {
    myTolArc = TolArc;
    myTolTang = TolTang;
    if (myFleche == 0.0) {
#ifdef OCCT_DEBUG
    ...
    if (typs1 == GeomAbs_Plane || typs1 == GeomAbs_Cylinder || typs1 == GeomAbs_Sphere || typs1 == GeomAbs_Cone ||
        typs2 == GeomAbs_Plane || typs2 == GeomAbs_Cylinder || typs2 == GeomAbs_Sphere || typs2 == GeomAbs_Cone) {
...
void IntPatch_Intersection__MAJ_R(Handle(Adaptor2d_Curve2d) * R1, Handle(Adaptor2d_Curve2d) *, int* NR1, int*,
                                  Standard_Integer nbR1, Standard_Integer, const IntPatch_Point& VTX) {
...
void IntPatch_Intersection::Dump(const Standard_Integer /*Mode*/, const Handle(Adaptor3d_Surface) & /*S1*/,
                                 const Handle(Adaptor3d_TopolTool) & /*D1*/, const Handle(Adaptor3d_Surface) & /*S2*/,
...
// purpose  :
//=======================================================================
Standard_Boolean IntPatch_Intersection::CheckSingularPoints(const Handle(Adaptor3d_Surface) & theS1,
                                                            const Handle(Adaptor3d_TopolTool) & theD1,
                                                            const Handle(Adaptor3d_Surface) & theS2,
