...
static void Recadre(const Handle(Adaptor3d_Surface) & myHS1, const Handle(Adaptor3d_Surface) & myHS2, Standard_Real& u1,
...
static void Parameters(const Handle(Adaptor3d_Surface) & myHS1, const Handle(Adaptor3d_Surface) & myHS2,
...
static Standard_Real ComputeParametricTolerance(const Standard_Real theTol3d, const gp_Vec& theD1u,
                                                const gp_Vec& theD1v) {
...
static Standard_Integer AppendSameVertexA(Handle(IntPatch_ALine) & alig, const Handle(IntPatch_ALine) & L,
                                          const Standard_Integer index, Standard_Integer* TabIndex) {
    ...
    for (i = 1; i <= n; i++) {
        if (i != index) {
            ...
            if (thetol2 < thetol1) thetol2 = thetol1;
...
static Standard_Integer AppendSameVertexG(Handle(IntPatch_GLine) & glig, const Handle(IntPatch_GLine) & L,
                                          const Standard_Integer index, const Standard_Real decal,
...
static Standard_Integer AppendSameVertexW(Handle(IntPatch_WLine) & wlig, const Handle(IntPatch_WLine) & L,
                                          const Standard_Integer index, const Standard_Real par,
                                          Standard_Integer* TabIndex) {
    Standard_Integer i, a, n;
    a = 0;
    n = L->NbVertex();
...
static Standard_Integer AppendSameVertexR(Handle(IntPatch_RLine) & rlig, const Handle(IntPatch_RLine) & L,
                                          const Standard_Integer index, Standard_Integer* TabIndex) {
    Standard_Integer i, a, n;
    a = 0;
    n = L->NbVertex();
...
static void AddLine(const Handle(IntPatch_Line) & L, const Standard_Integer i, const Standard_Integer j,
                    //	     const GeomAbs_SurfaceType TypeS1,
                    const GeomAbs_SurfaceType,
                    //	     const GeomAbs_SurfaceType TypeS2,
                    const GeomAbs_SurfaceType, Standard_Integer* TabIndex, IntPatch_SequenceOfLine& slin) {
            ...
            switch (typl) {
                case IntPatch_Lin:
                    if (L->TransitionOnS1() == IntSurf_Undecided)
                        ...
                        glig =
                            new IntPatch_GLine(GLine->Line(), L->IsTangent(), L->TransitionOnS1(), L->TransitionOnS2());
                    break;
                case IntPatch_Parabola:
                    if (L->TransitionOnS1() == IntSurf_Undecided)
                        ...
                        glig =
                            new IntPatch_GLine(GLine->Parabola(), L->IsTangent(), L->SituationS1(), L->SituationS2());
                    else
                        glig = new IntPatch_GLine(GLine->Parabola(), L->IsTangent(), L->TransitionOnS1(),
                                                  L->TransitionOnS2());
                    break;
                case IntPatch_Hyperbola:
                    if (L->TransitionOnS1() == IntSurf_Undecided)
                        ...
                        glig =
                            new IntPatch_GLine(GLine->Hyperbola(), L->IsTangent(), L->SituationS1(), L->SituationS2());
                    else
                        glig = new IntPatch_GLine(GLine->Hyperbola(), L->IsTangent(), L->TransitionOnS1(),
                                                  L->TransitionOnS2());
                    break;
                case IntPatch_Circle:
                    if (L->TransitionOnS1() == IntSurf_Undecided)
                        ...
                        glig = new IntPatch_GLine(GLine->Circle(), L->IsTangent(), L->TransitionOnS1(),
                                                  L->TransitionOnS2());
                    break;
                case IntPatch_Ellipse:
                default:
                    if (L->TransitionOnS1() == IntSurf_Undecided)
                        ...
                        glig = new IntPatch_GLine(GLine->Ellipse(), L->IsTangent(), L->TransitionOnS1(),
                                                  L->TransitionOnS2());
                    break;
            }
...
static Standard_Boolean TestWLineIsARLine(const IntPatch_SequenceOfLine& slinref, const Handle(IntPatch_WLine) & wlin,
...
static Standard_Boolean TestIfWLineIsRestriction(const IntPatch_SequenceOfLine& slinref,
                                                 ...
                                                 const Handle(Adaptor3d_TopolTool) & D2, Standard_Real TolArc) {
...
static Standard_Boolean ProjectOnArc(const Standard_Real u, const Standard_Real v,
                                     const Handle(Adaptor2d_Curve2d) & arc, const Handle(Adaptor3d_Surface) & surf,
                                     const Standard_Real TolArc, Standard_Real& par, Standard_Real& dist) {
    gp_Pnt aPbid;
    gp_Vec ad1u, ad1v;
...
static void TestWLineToRLine(const IntPatch_SequenceOfLine& slinref, IntPatch_SequenceOfLine& slin,
                             const Handle(Adaptor3d_Surface) & mySurf1, const Handle(Adaptor3d_TopolTool) & myDom1,
                             const Handle(Adaptor3d_Surface) & mySurf2, const Handle(Adaptor3d_TopolTool) & myDom2,
                             const Standard_Real TolArc) {
    ...
    for (is = 0; is < 2; is++) {
        ...
        if ((onFirst && WLine->HasArcOnS1()) || (!onFirst && WLine->HasArcOnS2())) {
            ...
            for (Standard_Integer ia = 1; ia <= mapArcs.Extent(); ia++) {
                ...
                if (reverse) VtxFirst.ReverseTransition(); //-- inversion des transitions
                ...
                for (i = 1; i <= pIndVmin->Length(); i++) {
                    ...
                    if (iv != ivmin) {
                        ...
                        if (reverse) Vtx.ReverseTransition();
                        rlig->AddVertex(Vtx);
                    }
                }
                ...
                if (reverse) VtxLast.ReverseTransition();
...
void IntPatch_LineConstructor::Perform(const IntPatch_SequenceOfLine& slinref, const Handle(IntPatch_Line) & L,
                                       const Handle(Adaptor3d_Surface) & mySurf1,
                                       const Handle(Adaptor3d_TopolTool) & myDom1,
                                       const Handle(Adaptor3d_Surface) & mySurf2,
                                       const Handle(Adaptor3d_TopolTool) & myDom2, const Standard_Real TolArc) {
    ...
    if (typl == IntPatch_Analytic) {
        ...
        for (i = 1; i < nbvtx; i++) {
            ...
            if (firstp != lastp && !IsSegmentSmall(WLine, i, i + 1 /*,TolArc*/)) {
                ...
                // TopAbs_State in2 = (in1!=TopAbs_OUT)? myDom2->Classify(gp_Pnt2d(u2,v2),Tol,Standard_False) :
                // TopAbs_OUT;
                ...
                if (in1 != TopAbs_OUT && in2 != TopAbs_OUT) {
                    ...
                    if (LignetropPetite == Standard_False) {
                        ...
                        TabIndex[i] = TabIndex[i + 1] = ++numline;
                        AddLine(L, i, i + 1, typs1, typs2, TabIndex, slin);
                        TestWLineToRLine(slinref, slin, mySurf1, myDom1, mySurf2, myDom2,
                                         TolArc); //-- on teste la derniere entree de slin
                    }
        ...
        for (i = 1; i < nbvtx; i++) {
            ...
            if (Abs(firstp - lastp) > Precision::PConfusion()) {
                ...
                TopAbs_State in2 =
                    (in1 != TopAbs_OUT) ? myDom2->Classify(gp_Pnt2d(u2, v2), Tol, Standard_False) : TopAbs_OUT;
        ...
        for (i = 1; i < NbVtx; i++) {
            ...
            if (RestOnS1 && RestOnS2) {
                ...
                if (Abs(u1 - u0) > Precision::PConfusion()) {
                    ...
                    myDom2->Init();
                    if (myDom2->More()) Tol = ComputeParametricTolerance(myDom2->Tol3d(myDom2->Value()), D1u, D1v);
                ...
                if (Abs(u1 - u0) > Precision::PConfusion()) {
                    ...
                    myDom1->Init();
                    if (myDom2->More()) Tol = ComputeParametricTolerance(myDom1->Tol3d(myDom1->Value()), D1u, D1v);
