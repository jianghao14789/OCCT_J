...
static void Recadre(GeomAbs_SurfaceType typeS1, GeomAbs_SurfaceType typeS2, const Handle(IntPatch_WLine) & wlin,
                    Standard_Integer Param, Standard_Real& U1, Standard_Real& V1, Standard_Real& U2,
                    Standard_Real& V2) {
...
inline Standard_Real Tol3d(const Handle(Adaptor3d_HVertex) & vtx, const Handle(Adaptor3d_TopolTool) & Domain,
                           const Standard_Real tolDef = 0.) {
    return (Domain->Has3d() ? Domain->Tol3d(vtx) : tolDef < Confusion ? Confusion : tolDef);
}

inline Standard_Real Tol3d(const Handle(Adaptor2d_Curve2d) & arc, const Handle(Adaptor3d_TopolTool) & Domain,
                           const Standard_Real tolDef = 0.) {
    return (Domain->Has3d() ? Domain->Tol3d(arc) : tolDef < Confusion ? Confusion : tolDef);
}

static Standard_Boolean CoincideOnArc(const gp_Pnt& Ptsommet, const Handle(Adaptor2d_Curve2d) & A,
                                      const Handle(Adaptor3d_Surface) & Surf, const Standard_Real Toler,
                                      const Handle(Adaptor3d_TopolTool) & Domain, Handle(Adaptor3d_HVertex) & Vtx) {
...
static void VerifyTgline(const Handle(IntPatch_WLine) & wlin, const Standard_Integer param, const Standard_Real Tol,
                         gp_Vec& Tgl) {
...
static void GetLinePoint2d(const Handle(IntPatch_Line) & L, const Standard_Real param, const Standard_Boolean OnFirst,
...
static Standard_Boolean FindParameter(const Handle(IntPatch_Line) & L, const Handle(Adaptor3d_Surface) & OtherSurf,
                                      const Standard_Real Tol, const gp_Pnt& Ptsom, const gp_Pnt2d& Ptsom2d,
                                      Standard_Real& Param, gp_Vec& Tgl, const Standard_Integer ParamApproche,
...
inline Standard_Boolean ArePnt2dEqual(const gp_Pnt2d& p1, const gp_Pnt2d& p2, const Standard_Real tolU,
...
void IntPatch_RstInt::PutVertexOnLine(const Handle(IntPatch_Line) & L, const Handle(Adaptor3d_Surface) & Surf,
                                      const Handle(Adaptor3d_TopolTool) & Domain,
                                      const Handle(Adaptor3d_Surface) & OtherSurf, const Standard_Boolean OnFirst,
    ...
    if (possiblyClosed) {
        if (SurfaceIsUClosed) tolUClosed = (Surf->LastUParameter() - Surf->FirstUParameter()) * 0.01;
        if (SurfaceIsVClosed) tolVClosed = (Surf->LastVParameter() - Surf->FirstVParameter()) * 0.01;
        if (OSurfaceIsUClosed) tolOUClosed = (OtherSurf->LastUParameter() - OtherSurf->FirstUParameter()) * 0.01;
        if (OSurfaceIsVClosed) tolOVClosed = (OtherSurf->LastVParameter() - OtherSurf->FirstVParameter()) * 0.01;
    }
    ...
    while (Domain->More()) {
        ...
        if (Precision::IsNegativeInfinite(PFirst)) PFirst = -myInfinite;
        if (Precision::IsPositiveInfinite(PLast)) PLast = myInfinite;
                ...
                for (i = 1; i <= aNbSectionPts; i++) {
                    ...
                    if (refined) {
                        ...
                        if (!duplicate) {
                            ...
                            found = FindParameter(L, OtherSurf, edgeTol, ptsommet, gp_Pnt2d(U2, V2), paramline, tgline,
                                                  ParamApproxOnLine, OnFirst);
                            ...
                            if (!ivtx) {
                                ...
                                if (VtxOnArc) Sommet.SetVertex(OnFirst, vtxarc);
                                ...
                                if ((!ptline.IsVertexOnS1() && OnFirst) || (!ptline.IsVertexOnS2() && !OnFirst) ||
                                    ...
                                    if ((!ptline.IsOnDomS2() && !OnFirst) || (!ptline.IsOnDomS1() && OnFirst) ||
                                        ...
                                        if (VtxOnArc) ptline.SetVertex(OnFirst, vtxarc);
                                        if (typL == IntPatch_Walking) {
                                            if (OnDifferentRst) {
                                        ...
                                        for (k = 1; k <= Nbvtx; k++)
                                            if (k != ivtx) {
                                                ...
                                                if ((OnFirst && ptline.IsVertexOnS1()) ||
                                                    ...
                                                    if (Domain->Identical(vtxref, (OnFirst) ? (ptline.VertexOnS1())
                                                                                            : (ptline.VertexOnS2()))) {
