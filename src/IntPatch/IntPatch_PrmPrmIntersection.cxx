...
static void SectionPointToParameters(const Intf_SectionPoint& Sp, const IntPatch_Polyhedron& Surf1,
                                     const IntPatch_Polyhedron& Surf2, Standard_Real& u1, Standard_Real& v1,
                                     Standard_Real& u2, Standard_Real& v2);

static void AdjustOnPeriodic(const Handle(Adaptor3d_Surface) & Surf1, const Handle(Adaptor3d_Surface) & Surf2,
                             IntPatch_SequenceOfLine& aSLin);

static IntSurf_PntOn2S MakeNewPoint(const IntSurf_PntOn2S& replacePnt, const IntSurf_PntOn2S& oldPnt,
                                    const Standard_Real* Periods);

static Standard_Boolean IsPointOnLine(const IntSurf_PntOn2S& thePOn2S, const Handle(IntPatch_WLine) & theWLine,
                                      const Standard_Real Deflection);

static void AddWLine(IntPatch_SequenceOfLine& theLines, const Handle(IntPatch_WLine) & theWLine,
                     const Standard_Real Deflection);

static void SeveralWlinesProcessing(const Handle(Adaptor3d_Surface) & theSurf1,
                                    const Handle(Adaptor3d_Surface) & theSurf2, const IntPatch_SequenceOfLine& theSLin,
                                    const Standard_Real* const thePeriodsArr, const IntSurf_TypeTrans theTrans1,
                                    const IntSurf_TypeTrans theTrans2, const Standard_Real theTol,
                                    const Standard_Real theMaxStepS1, const Standard_Real theMaxStepS2,
    ...
    for (ciV = 1; ciV <= cnbV; ciV++) {
        ...
        for (iL = 1; iL <= theSLin.Length(); iL++) {
            ...
            if (aType != IntPatch_Walking) continue;
...
//=======================================================================
static void DublicateOfLinesProcessing(const IntWalk_PWalking& thePW, const Standard_Integer theWLID,
                                       IntPatch_SequenceOfLine& theLines, Standard_Boolean& theIsRejectReq) {
    ...
    if (aNbPrevPoints < aNbCurrPoints) { // Remove preview line
        ...
        for (Standard_Integer aNbPP = 1; aNbPP < aNbPrevPoints; aNbPP++) {
            const gp_Pnt aP1prev(anExistWL->Point(aNbPP).Value()), aP2prev(anExistWL->Point(aNbPP + 1).Value());
        ...
        if (aLPrev < aLCurr) { // Remove preview line
            theLines.Remove(theWLID);
            theIsRejectReq = Standard_False;
        }
    }
...
// purpose  :
//==================================================================================
IntPatch_PrmPrmIntersection::IntPatch_PrmPrmIntersection() : done(Standard_False), empt(Standard_True) {}
...
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          const Handle(Adaptor3d_TopolTool) & D1, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
...
// purpose  :
//==================================================================================
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1, const IntPatch_Polyhedron& Poly1,
                                          const Handle(Adaptor3d_TopolTool) & D1,
                                          const Handle(Adaptor3d_Surface) & Surf2,
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
                                          const Standard_Real Increment) {
...
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          const Handle(Adaptor3d_TopolTool) & D1,
                                          const Handle(Adaptor3d_Surface) & Surf2, const IntPatch_Polyhedron& Poly2,
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
                                          const Standard_Real Increment) {
        ...
        for (ls = 1; ls <= nbLigSec; ls++) {
            ...
            do {
                ...
                if (NombreDePointsDeDepartDuCheminement == 1) {
                    ...
                    if (nbp < 3) NombreDePointsDeDepartDuCheminement = 3;
                ...
                if (TabPtDep[nbps2] == 0) {
                    ...
                    if (HasStartPoint) {
                        ...
                        if (dminiPointLigne > SeuildPointLigne) {
                            PW.Perform(StartParams, UminLig1, VminLig1, UminLig2, VminLig2, UmaxLig1, VmaxLig1,
                                       UmaxLig2, VmaxLig2);
                            if (PW.IsDone()) {
                                if (PW.NbPoints() > 2) {
                                    ...
                                    if (!RejetLigne) {
                                        ...
                                        IntPatch_RstInt::PutVertexOnLine(wline, Surf1, D1, Surf2, Standard_True,
                                                                         TolTang);
                                        if (wline->NbPnts() < 2) continue;
                                        IntPatch_RstInt::PutVertexOnLine(wline, Surf2, D2, Surf1, Standard_False,
                                                                         TolTang);
                                        if (wline->NbPnts() < 2) continue;
    ...
    for (z = 1; z <= nbTanZon; z++) {
        ...
        for (pz = 1; pz <= TangentZone.NumberOfPoints(); pz++) {
            ...
            if (HasStartPoint) {
                ...
                if (dminiPointLigne > SeuildPointLigne) {
                    PW.Perform(StartParams, UminLig1, VminLig1, UminLig2, VminLig2, UmaxLig1, VmaxLig1, UmaxLig2,
                               VmaxLig2);
                    if (PW.IsDone()) {
                        if (PW.NbPoints() > 2) {
                            RejetLigne = Standard_False;
                                ...
                                if (PW.NbPoints() > 2) {
                                    ...
                                    if (!RejetLigne) {
                                        ...
                                        if (imin < imax) {
                                            ...
                                            // the method PutVertexOnLine can reduce the number of points in <wline>
                                            IntPatch_RstInt::PutVertexOnLine(wline, Surf1, D1, Surf1, Standard_True,
                                                                             TolTang);
                                            if (wline->NbPnts() < 2) continue;
                                            if (wline->NbVertex() == 0) {
                                                IntPatch_Point vtx;
...
Handle(IntPatch_Line) IntPatch_PrmPrmIntersection::NewLine(const Handle(Adaptor3d_Surface) & Surf1,
                                                           const Handle(Adaptor3d_Surface) & Surf2,
                                                           const Standard_Integer NumLine, const Standard_Integer Low,
    ...
    for (i = 2, s = ds; (i < NbPnts) && (s < smax); i++, s += ds) {
        ...
        if ((nab > ds) && (a > dsmin) && (b > dsmin)) {
            ...
            if (HasStartPoint) ResultPntOn2SLine->Add(StartPOn2S);
        } else
            s += dsmin;
    }
...
// purpose  :
//==================================================================================
void SectionPointToParameters(const Intf_SectionPoint& Sp, const IntPatch_Polyhedron& Poly1,
                              const IntPatch_Polyhedron& Poly2, Standard_Real& u1, Standard_Real& v1, Standard_Real& u2,
                              Standard_Real& v2) {
    Intf_PIType typ;
...
// purpose  :
//==================================================================================
void IntPatch_PrmPrmIntersection::RemplitLin(const Standard_Integer x1, const Standard_Integer y1,
                                             const Standard_Integer z1, const Standard_Integer x2,
                                             const Standard_Integer y2, const Standard_Integer z2,
...
// purpose  :
//==================================================================================
void IntPatch_PrmPrmIntersection::RemplitTri(const Standard_Integer x1, const Standard_Integer y1,
                                             const Standard_Integer z1, const Standard_Integer x2,
                                             const Standard_Integer y2, const Standard_Integer z2,
                                             const Standard_Integer x3, const Standard_Integer y3,
                                             const Standard_Integer z3, IntPatch_PrmPrmIntersection_T3Bits& Map) const {
...
// purpose  :
//==================================================================================
void IntPatch_PrmPrmIntersection::Remplit(const Standard_Integer a, const Standard_Integer b, const Standard_Integer c,
                                          IntPatch_PrmPrmIntersection_T3Bits& Map) const {
...
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          ...
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
                                          const Standard_Real Increment, IntSurf_ListOfPntOn2S& LOfPnts) {
    if (LOfPnts.IsEmpty()) {
        done = Standard_True;
        return;
    }
                ...
                if (PW.IsDone()) {
                    if (PW.NbPoints() > 2) {
                        ...
                        if (!RejetLigne) {
                            ...
                            SeveralWlinesProcessing(Surf1, Surf2, SLin, Periods, trans1, trans2, TolTang,
...
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          const Handle(Adaptor3d_TopolTool) & D1,
                                          const Handle(Adaptor3d_Surface) & Surf2,
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real U1Depart,
                                          const Standard_Real V1Depart, const Standard_Real U2Depart,
                                          const Standard_Real V2Depart, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
                                          const Standard_Real Increment) {
...
// purpose  :
//==================================================================================
void AdjustOnPeriodic(const Handle(Adaptor3d_Surface) & Surf1, const Handle(Adaptor3d_Surface) & Surf2,
                      IntPatch_SequenceOfLine& aSLin) {
...
// purpose  :
//==================================================================================
IntSurf_PntOn2S MakeNewPoint(const IntSurf_PntOn2S& replacePnt, const IntSurf_PntOn2S& oldPnt,
                             const Standard_Real* Periods) {
...
void IntPatch_PrmPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          ...
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real TolTangency,
                                          const Standard_Real Epsilon, const Standard_Real Deflection,
                                          const Standard_Real Increment, const Standard_Boolean ClearFlag) {
    ...
    if ((NbU1 * NbV1 <= Limit && NbV2 * NbU2 <= Limit)) {
            ...
            for (Standard_Integer ls = 1; ls <= nbLigSec; ++ls) {
                ...
                do {
                    ...
                    switch (NombreDePointsDeDepartDuCheminement) {
                        case 1:
                            nbps2 = (nbp > 1) ? nbp / 2 : 1;
                            if (nbp < 3) NombreDePointsDeDepartDuCheminement = NbDePointsDeDepartDuChmLimit;
        ...
        for (z = 1; z <= nbTanZon; z++) {
            ...
            if (HasStartPoint) {
                ...
                for (Standard_Integer l = 1; (l <= NbLigCalculee) && (dminiPointLigne >= SeuildPointLigne); l++) {
                    const Handle(IntPatch_WLine) & testwline = *((Handle(IntPatch_WLine)*)&SLin.Value(l));
...
public:
    ...
    //----------------------------------------xP1DS2
    char& xP1DS2(const Standard_Integer i, const Standard_Integer j) {
        return myP1DS2[Index(i, j)];
    };
    //----------------------------------------xP2DS1
    char& xP2DS1(const Standard_Integer i, const Standard_Integer j) {
    ...
    //----------------------------------------xP1
    gp_Pnt& xP1(const Standard_Integer i, const Standard_Integer j) {
        return myP1[Index(i, j)];
    };
    //----------------------------------------xP1
    gp_Pnt& xP2(const Standard_Integer i, const Standard_Integer j) {
...
//  purpose  :
//==================================================================================
void IntPatch_PrmPrmIntersection::PointDepart(Handle(IntSurf_LineOn2S) & LineOn2S, const Handle(Adaptor3d_Surface) & S1,
                                              const Standard_Integer SU_1, const Standard_Integer SV_1,
                                              const Handle(Adaptor3d_Surface) & S2, const Standard_Integer SU_2,
    ...
    for (i = 0; i < SU1 - 1; i += 1) {
        for (j = 0; j < SV1 - 1; j += 1) {
            if (!((aIPD.xP1DS2(i, j) & aIPD.xP1DS2(i + 1, j)) || (aIPD.xP1DS2(i, j) & aIPD.xP1DS2(i + 1, j + 1)))) {
                Remplit(aIPD.xIP1(i, j), aIPD.xIP1(i + 1, j), aIPD.xIP1(i + 1, j + 1), M1);
            }
            if (!((aIPD.xP1DS2(i, j) & aIPD.xP1DS2(i, j + 1)) || (aIPD.xP1DS2(i, j) & aIPD.xP1DS2(i + 1, j + 1)))) {
    ...
    for (i = 0; i < SU2 - 1; i += 1) {
        for (j = 0; j < SV2 - 1; j += 1) {
            if (!((aIPD.xP2DS1(i, j) & aIPD.xP2DS1(i + 1, j)) || (aIPD.xP2DS1(i, j) & aIPD.xP2DS1(i + 1, j + 1)))) {
                Remplit(aIPD.xIP2(i, j), aIPD.xIP2(i + 1, j), aIPD.xIP2(i + 1, j + 1), M2);
            }
            if (!((aIPD.xP2DS1(i, j) & aIPD.xP2DS1(i, j + 1)) || (aIPD.xP2DS1(i, j) & aIPD.xP2DS1(i + 1, j + 1)))) {
    ...
    do {
        ...
        if (ok) {
            ...
            int nb = 0;
            int LIM = 3;
            if (DansGrille(i - 1) && DansGrille(j - 1) && DansGrille(k - 1) && DansGrille(i + 1) && DansGrille(j + 1) &&
                DansGrille(k + 1)) {
                int si, sj, sk;
            ...
            if (nu1 >= 0 && nu2 >= 0) {
                ...
                POn2S.SetValue(P, S1->FirstUParameter() + nu1 * du1, S1->FirstVParameter() + nv1 * dv1,
                               S2->FirstUParameter() + nu2 * du2, S2->FirstVParameter() + nv2 * dv2);
                LineOn2S->Add(POn2S);
                Compt++;
            } else {
...
Standard_Boolean IsPointOnLine(const IntSurf_PntOn2S& thePOn2S, const Handle(IntPatch_WLine) & theWLine,
                               const Standard_Real Deflection) {
...
void AddWLine(IntPatch_SequenceOfLine& theLines, const Handle(IntPatch_WLine) & theWLine,
              const Standard_Real Deflection) {
