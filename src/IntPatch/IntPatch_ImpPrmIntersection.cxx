...
static Standard_Boolean DecomposeResult(const Handle(IntPatch_PointLine) & theLine, const Standard_Boolean IsReversed,
                                        const IntSurf_Quadric& theQuad, const Handle(Adaptor3d_TopolTool) & thePDomain,
                                        const Handle(Adaptor3d_Surface) & theQSurf,
                                        const Handle(Adaptor3d_Surface) & theOtherSurf, const Standard_Real theArcTol,
                                        const Standard_Real theTolTang, IntPatch_SequenceOfLine& theLines);
static void ComputeTangency(const IntPatch_TheSOnBounds& solrst, IntSurf_SequenceOfPathPoint& seqpdep,
                            const Handle(Adaptor3d_TopolTool) & Domain, IntPatch_TheSurfFunction& Func,
                            const Handle(Adaptor3d_Surface) & PSurf, TColStd_Array1OfInteger& Destination);
static void Recadre(const Standard_Boolean, GeomAbs_SurfaceType typeS1, GeomAbs_SurfaceType typeS2, IntPatch_Point& pt,
                    const Handle(IntPatch_TheIWLineOfTheIWalking) & iwline, Standard_Integer Param, Standard_Real U1,
                    Standard_Real V1, Standard_Real U2, Standard_Real V2);

static Standard_Boolean IsCoincide(IntPatch_TheSurfFunction& theFunc, const Handle(IntPatch_PointLine) & theLine,
                                   const Handle(Adaptor2d_Curve2d) & theArc, const Standard_Boolean isTheSurface1Using,
                                   const Standard_Real theToler3D, const Standard_Real theToler2D,
                                   const Standard_Real thePeriod);

static Standard_Real GetLocalStep(const Handle(Adaptor3d_Surface) & theSurf, const Standard_Real theStep);
...
static IntPatch_SpecPntType IsSeamOrPole(const Handle(Adaptor3d_Surface) & theQSurf,
                                         const Handle(IntSurf_LineOn2S) & theLine, const Standard_Boolean IsReversed,
                                         const Standard_Integer theRefIndex, const Standard_Real theTol3D,
                                         const Standard_Real theDeltaMax) {
    if ((theRefIndex < 1) || (theRefIndex >= theLine->NbPoints())) return IntPatch_SPntNone;
    ...
    if ((aType == GeomAbs_Cone) && (theQSurf->Cone().Apex().SquareDistance(aP3d) < theTol3D * theTol3D)) {
    ...
    if ((aType != GeomAbs_Torus) && (aDeltaU < theDeltaMax)) return IntPatch_SPntNone;
    ...
    switch (aType) {
        ...
        case GeomAbs_Torus: {
            ...
            if ((aDeltaU >= theDeltaMax) && (aDeltaV >= theDeltaMax)) return IntPatch_SPntSeamUV;

            if (aDeltaU >= theDeltaMax) return IntPatch_SPntSeamU;

            if (aDeltaV >= theDeltaMax) return IntPatch_SPntSeamV;
        }
...
IntPatch_ImpPrmIntersection::IntPatch_ImpPrmIntersection()
    : done(Standard_False), empt(Standard_False), myIsStartPnt(Standard_False), myUStart(0.0), myVStart(0.0) {}
...
IntPatch_ImpPrmIntersection::IntPatch_ImpPrmIntersection(const Handle(Adaptor3d_Surface) & Surf1,
                                                         ...
                                                         const Standard_Real TolArc, const Standard_Real TolTang,
...
// purpose  :
//=======================================================================
void ComputeTangency(const IntPatch_TheSOnBounds& solrst, IntSurf_SequenceOfPathPoint& seqpdep,
                     const Handle(Adaptor3d_TopolTool) & Domain, IntPatch_TheSurfFunction& Func,
                     const Handle(Adaptor3d_Surface) & PSurf, TColStd_Array1OfInteger& Destination) {
    ...
    for (i = 1; i <= NbPoints; i++) {
        if (Destination(i) == 0) {
            ...
            if (Func.IsTangent()) {
                ...
                if (PStart.IsNew()) {
                    if ((test < 0. && arcorien == TopAbs_FORWARD) || (test > 0. && arcorien == TopAbs_REVERSED)) {
                        vectg.Reverse();
                        dirtg.Reverse();
                    }
...
// purpose  :
//=======================================================================
void Recadre(const Standard_Boolean, GeomAbs_SurfaceType typeS1, GeomAbs_SurfaceType typeS2, IntPatch_Point& pt,
             const Handle(IntPatch_TheIWLineOfTheIWalking) & iwline, Standard_Integer Param, Standard_Real U1,
             Standard_Real V1, Standard_Real U2, Standard_Real V2) {
...
Standard_Real GetLocalStep(const Handle(Adaptor3d_Surface) & theSurf, const Standard_Real theStep) {
    ...
    if (theSurf->UContinuity() > GeomAbs_C0 && theSurf->VContinuity() > GeomAbs_C0) {
        ...
        if (aSType == GeomAbs_BezierSurface || aSType == GeomAbs_BSplineSurface) {
            ...
            aMinRes = Min(theSurf->UResolution(Precision::Confusion()), theSurf->VResolution(Precision::Confusion()));
...
void IntPatch_ImpPrmIntersection::Perform(const Handle(Adaptor3d_Surface) & Surf1,
                                          const Handle(Adaptor3d_TopolTool) & D1,
                                          const Handle(Adaptor3d_Surface) & Surf2,
                                          const Handle(Adaptor3d_TopolTool) & D2, const Standard_Real TolArc,
                                          const Standard_Real TolTang, const Standard_Real Fleche,
    ...
    if (NbPointDep || NbPointIns) {
        ...
        for (Standard_Integer j = 1; j <= Nblines - 1; j++) {
            ...
            if (dofirst || dolast) {
                ...
                if (dofirst) wlin1->Replace(indfirst, ptdeb);
                if (dolast) wlin1->Replace(indlast, ptfin);
            }
        }
    ...
    if (NbSegm) {
        for (Standard_Integer i = 1; i <= NbSegm; i++) {
            ...
            if (thesegm.HasFirstPoint()) {
                ...
                if (PStartf.IsNew() == Standard_False) _vtx = PStartf.Vertex();
                ...
                if (PStartf.IsNew() == Standard_False) _thepointAtBeg.SetVertex(reversed, _vtx);
            ...
            if (thesegm.HasLastPoint()) {
                ...
                Handle(Adaptor3d_HVertex) _vtx;
                if (PStartl.IsNew() == Standard_False) _vtx = PStartl.Vertex();
                ...
                if (PStartl.IsNew() == Standard_False) _thepointAtEnd.SetVertex(reversed, _vtx);
    ...
    for (Standard_Integer i = 1, aNbLin = slin.Length(); i <= aNbLin; i++) {
        ...
        if (aL->NbPnts() <= 2) {
            ...
            if (!aCond) aCond = (aL->Point(1).IsSame(aL->Point(2), Precision::Confusion()));
    ...
    for (Standard_Integer i = 1; i <= slin.Length(); i++) {
        ...
        for (Standard_Integer j = i + 1; j <= slin.Length(); j++) {
            ...
            if (!aRL2.IsNull()) {
                const Handle(Adaptor2d_Curve2d) & anArc2 = aRL2->IsArcOnS1() ? aRL2->ArcOnS1() : aRL2->ArcOnS2();
            ...
            if (Abs(aDir.X()) < 0.5) { // Restriction directs along V-direction
            ...
            if (isCoincide) {
                if (aRL2.IsNull()) { // Delete Walking-line
                    slin.Remove(j);
                    j--;
                } else { // Restriction-Restriction
                    const Handle(Adaptor2d_Curve2d) & anArc2 = aRL2->IsArcOnS1() ? aRL2->ArcOnS1() : aRL2->ArcOnS2();
        ...
        if (isFirstDeleted) { // Delete i-th line
            slin.Remove(i--);
        }
    } // for (Standard_Integer i = 1; i <= slin.Length(); i++)
    ...
    for (Standard_Integer i = 1; i <= slin.Length(); i++) {
        if (DecomposeResult(Handle(IntPatch_PointLine)::DownCast(slin(i)), reversed, Quad, PDomain, aQSurf, anOtherSurf,
                            TolArc, aTol3d, dslin)) {
...
static Handle(IntSurf_LineOn2S)
    GetVertices(const Handle(IntPatch_PointLine) & thePLine, const Standard_Real TOL3D, const Standard_Real TOL2D) {
    ...
    for (i = 1; i <= NbVrt; i++) {
        ...
        for (k = (i + 1); k <= NbVrt; k++) {
            ...
            if (Pi.Value().Distance(Pk.Value()) <= TOL3D) {
                ...
                if ((sameU1 && sameV1) && (sameU2 && sameV2)) anVrts(k) = -1;
            }
        }
    }
...
static void SearchVertices(const Handle(IntSurf_LineOn2S) & Line, const Handle(IntSurf_LineOn2S) & Vertices,
...
static inline void Correct2DBounds(const Standard_Real UF, const Standard_Real UL, const Standard_Real VF,
                                   const Standard_Real VL, const Standard_Real TOL2D, Standard_Real& U,
                                   Standard_Real& V) {
...
static void AdjustLine(Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean IsReversed,
                       const Handle(Adaptor3d_Surface) & QSurf, const Standard_Real TOL2D) {
...
static Standard_Boolean InsertSeamVertices(Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean IsReversed,
                                           Handle(IntSurf_LineOn2S) & Vertices, const TColStd_Array1OfInteger& PTypes,
...
static void ToSmooth(const Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean IsReversed,
                     const IntSurf_Quadric& Quad, const Standard_Boolean IsFirst, Standard_Real& D3D) {
    if (Line->NbPoints() <= 10) return;
...
static Standard_Boolean TestMiddleOnPrm(const IntSurf_PntOn2S& aP, const IntSurf_PntOn2S& aV,
                                        const Standard_Boolean IsReversed, const Standard_Real ArcTol,
...

{
    ...
    if (PosM == TopAbs_ON || PosM == TopAbs_IN) result = Standard_True;
    return result;
}

static void VerifyVertices(const Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean IsReversed,
                           const Handle(IntSurf_LineOn2S) & Vertices, const Standard_Real TOL2D,
                           const Standard_Real ArcTol, const Handle(Adaptor3d_TopolTool) & PDomain,
                           IntSurf_PntOn2S& VrtF, Standard_Boolean& AddFirst, IntSurf_PntOn2S& VrtL,
    ...
    for (iv = 1; iv <= nbv; iv++) {
        ...
        if (aPF.IsSame(aV, Precision::Confusion(), Precision::PConfusion())) {
            ...
            if (Dist < DistMinF) {
                DistMinF = Dist;
                FIndexNear = iv;
                if (FConjugated != 0) FConjugated = 0;
            }
    ...
    for (iv = 1; iv <= nbv; iv++) {
        ...
        if (aPL.IsSame(aV, Precision::Confusion(), Precision::PConfusion())) {
            ...
            if (Dist < DistMinL) {
                DistMinL = Dist;
                LIndexNear = iv;
                if (LConjugated != 0) LConjugated = 0;
            }
            if (IsSeamParameter(Uv, TOL2D)) {
                Standard_Real Ucv = (fabs(Uv) < fabs(2. * M_PI - Uv)) ? (2. * M_PI) : 0.;
                gp_Pnt2d a2DCV(Ucv, Vv);
...
static Standard_Boolean AddVertices(Handle(IntSurf_LineOn2S) & Line, const IntSurf_PntOn2S& VrtF,
                                    const Standard_Boolean AddFirst, const IntSurf_PntOn2S& VrtL,
                                    const Standard_Boolean AddLast, const Standard_Real D3DF,
...
static void PutIntVertices(const Handle(IntPatch_PointLine) & Line, Handle(IntSurf_LineOn2S) & Result,
                           Standard_Boolean theIsReversed, Handle(IntSurf_LineOn2S) & Vertices,
...
static Standard_Boolean HasInternals(Handle(IntSurf_LineOn2S) & Line, Handle(IntSurf_LineOn2S) & Vertices) {
    Standard_Integer nbp = Line->NbPoints(), nbv = Vertices->NbPoints();
    Standard_Integer ip = 0, iv = 0;
    Standard_Boolean result = Standard_False;

    if (nbp < 3) return result;
...
static Handle(IntPatch_WLine) MakeSplitWLine(Handle(IntPatch_WLine) & WLine, Standard_Boolean Tang,
                                             IntSurf_TypeTrans Trans1, IntSurf_TypeTrans Trans2, Standard_Real ArcTol,
                                             Standard_Integer ParFirst, Standard_Integer ParLast) {
...
static Standard_Boolean SplitOnSegments(Handle(IntPatch_WLine) & WLine, Standard_Boolean Tang, IntSurf_TypeTrans Trans1,
                                        IntSurf_TypeTrans Trans2, Standard_Real ArcTol,
    ...
    if (nbv > 3) {
        ...
        for (iv = 1; iv < nbv; iv++) {
            ...
            if ((lastPar - firstPar) <= 1)
                ...
                if (!result) result = Standard_True;
            }
        }
    }
    return result;
...
//=======================================================================
static Standard_Boolean IsPointOnBoundary(const Standard_Real theToler2D, const Standard_Real theBoundary,
                                          const Standard_Real thePeriod, const Standard_Real theParam) {
...
static void DetectOfBoundaryAchievement(const Handle(Adaptor3d_Surface) & theQSurf, // quadric
                                        ...
                                        const Standard_Integer thePointIndex, const Standard_Real theToler2D,
                                        Handle(IntSurf_LineOn2S) & theNewLine, Standard_Boolean& theIsOnBoundary) {
    ...
    if (IsPointOnBoundary(theToler2D, aUf, aUPeriod, aUCurr) && !IsPointOnBoundary(theToler2D, aUf, aUPeriod, aUPrev)) {
...
static Standard_Boolean DecomposeResult(const Handle(IntPatch_PointLine) & theLine, const Standard_Boolean IsReversed,
                                        ...
                                        const Standard_Real theArcTol, const Standard_Real theTolTang,
    ...
    const Standard_Real aTOL3D = 1.e-10, aTOL2D = Precision::PConfusion(), aTOL2DS = Precision::PConfusion();
    ...
    if (aSSLine->NbPoints() <= 1) return Standard_False;
    ...
    while (flNextLine) {
        ...
        if (aPrePointExist) {
            ...
            const Standard_Real aURes = theQSurf->UResolution(theArcTol), aVRes = theQSurf->VResolution(theArcTol);
            ...
            if (IntPatch_SpecialPoints::ContinueAfterSpecialPoint(theQSurf, thePSurf, aRefPt, aPrePointExist, aTol2d,
                                                                  PrePoint, IsReversed)) {
                sline->Add(PrePoint);
        ...
        for (Standard_Integer k = aFindex; k <= aLindex; k++) {
            ...
            if (aPrePointExist != IntPatch_SPntNone) {
                ...
                if (aPrePointExist == IntPatch_SPntSeamUV) {
                    ...
                    IntPatch_SpecialPoints::AddCrossUVIsoPoint(theQSurf, thePSurf, aRefPt, theTolTang, aNewPoint,
                                                               IsReversed);
                } else if (aPrePointExist == IntPatch_SPntSeamV) { // WLine goes through seam
                    aPrePointExist = IntPatch_SPntNone;
                    ...
                    IntPatch_SpecialPoints::AddPointOnUorVIso(theQSurf, thePSurf, aRefPt, Standard_False, 0.0, aTol,
                    ...
                    if (IntPatch_SpecialPoints::AddSingularPole(theQSurf, thePSurf, aRefPt, aVert, aNewPoint,
                                                                IsReversed)) {
                    ...
                    else { // Pole is not an intersection point
                        aPrePointExist = IntPatch_SPntSeamU;
                    }
                }

                if (aPrePointExist == IntPatch_SPntSeamU) { // WLine goes through seam
                    aPrePointExist = IntPatch_SPntNone;
                    ...
                    IntPatch_SpecialPoints::AddPointOnUorVIso(theQSurf, thePSurf, aRefPt, Standard_True, 0.0, aTol,
                                                              aStartPoint, anInfBound, aSupBound, aNewPoint,
                                                              IsReversed);
                }
        ...
        if (sline->NbPoints() == 1) {
            ...
            if (aFindex < aBindex) aFindex = aBindex;
        ...
        VerifyVertices(sline, IsReversed, aVLine, aTOL2DS, theArcTol, thePDomain, aVF, addVF, aVL, addVL);
        ...
        if (theLine->ArcType() == IntPatch_Walking) {
            ...
            Handle(IntPatch_WLine) wline =
                new IntPatch_WLine(sline, Standard_False, theLine->TransitionOnS1(), theLine->TransitionOnS2());
            wline->SetCreatingWayInfo(IntPatch_WLine::IntPatch_WLImpPrm);
            ...
            Standard_Boolean isSplited = SplitOnSegments(wline, Standard_False, theLine->TransitionOnS1(),
                                                         theLine->TransitionOnS2(), theArcTol, segm);
        ...
        } else { // theLine->ArcType() == IntPatch_Restriction
            ...
            const Handle(Adaptor2d_Curve2d) & anArc = aRLine->IsArcOnS1() ? aRLine->ArcOnS1() : aRLine->ArcOnS2();
...
//=======================================================================
static Standard_Boolean CheckSegmSegm(const Standard_Real theRefParF, const Standard_Real theRefParL,
                                      const Standard_Real theParF, const Standard_Real theParL) {
...
//=======================================================================
Standard_Boolean IsCoincide(IntPatch_TheSurfFunction& theFunc, const Handle(IntPatch_PointLine) & theLine,
                            const Handle(Adaptor2d_Curve2d) & theArc,
                            const Standard_Boolean isTheSurface1Using, // Surf1 is parametric?
                            const Standard_Real theToler3D, const Standard_Real theToler2D,
                            const Standard_Real thePeriod) // Period of parametric surface in direction which is
                                                           // perpendicular to theArc direction.
{
    ...
    if (theLine->ArcType() == IntPatch_Restriction) { // Restriction-restriction processing
        ...
        if ((aDist < theToler2D) || (Abs(aDist - thePeriod) < theToler2D)) {
            ...
            Standard_Real aParam1 = ElCLib::Parameter(aLin1, aP1), aParam2 = ElCLib::Parameter(aLin1, aP2);

            if (CheckSegmSegm(aRf, aRl, aParam1, aParam2)) return Standard_True;
    ...
    for (Standard_Integer aPtID = 1; aPtID <= aNbPnts; aPtID++) {
        ...
        if ((aRParam < aUAf) || (aRParam > aUAl)) return Standard_False;

        const gp_Pnt2d aPmin(ElCLib::Value(aRParam, anArcLin));

        const Standard_Real aDist = aPloc.Distance(aPmin);
        if ((aDist < theToler2D) || (Abs(aDist - thePeriod) < theToler2D)) { // Considered point is in Restriction line.
