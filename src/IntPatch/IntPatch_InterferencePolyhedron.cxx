...
IntPatch_InterferencePolyhedron::IntPatch_InterferencePolyhedron() : Intf_Interference(Standard_False), Incidence(0) {
...
IntPatch_InterferencePolyhedron::IntPatch_InterferencePolyhedron(const IntPatch_Polyhedron& FirstPol,
                                                                 const IntPatch_Polyhedron& SeconPol)
    : Intf_Interference(Standard_False), Incidence(0) {
    memset(OI, 0, sizeof(OI));
    memset(TI, 0, sizeof(TI));
    memset(dpOeT, 0, sizeof(dpOeT));
    memset(dpOpT, 0, sizeof(dpOpT));
    memset(deOpT, 0, sizeof(deOpT));
    if (!IntPatch_PolyhedronTool::Bounding(FirstPol).IsOut(IntPatch_PolyhedronTool::Bounding(SeconPol))) {
...
IntPatch_InterferencePolyhedron::IntPatch_InterferencePolyhedron(const IntPatch_Polyhedron& Objet)
    : Intf_Interference(Standard_True), Incidence(0) {
    memset(OI, 0, sizeof(OI));
    memset(TI, 0, sizeof(TI));
    memset(dpOeT, 0, sizeof(dpOeT));
    memset(dpOpT, 0, sizeof(dpOpT));
    ...
    if (gridOnFirst) {
        ...
        for (iSecon = 1; iSecon <= NbTrianglesSecondPol; iSecon++) {
            ...
            while (iLoI.More()) {
                iFirst = iLoI.Value();
                if (SelfIntf) {
                    if (iFirst < iSecon) Intersect(iFirst, FirstPol, iSecon, SeconPol);
                } else
        ...
        for (iFirst = 1; iFirst <= NbTrianglesFirstPol; iFirst++) {
            TColStd_ListIteratorOfListOfInteger iLoI(
                TheGridSecond.Compare(IntPatch_PolyhedronTool::ComponentsBounding(FirstPol)->Value(iFirst)));
...
void IntPatch_InterferencePolyhedron::Intersect(const Standard_Integer Tri1, const IntPatch_Polyhedron& FirstPol,
    ...
    if (SelfIntf) {
        if (OI[0] == TI[0] || OI[0] == TI[1] || OI[0] == TI[2] || OI[1] == TI[0] || OI[1] == TI[1] || OI[1] == TI[2] ||
            OI[2] == TI[0] || OI[2] == TI[1] || OI[2] == TI[2])
            return;
    }
    ...
    Intf::PlaneEquation(IntPatch_PolyhedronTool::Point(FirstPol, OI[0]),
                        ...
                        IntPatch_PolyhedronTool::Point(FirstPol, OI[2]), ONor, Odp);
    ...
    Intf::PlaneEquation(IntPatch_PolyhedronTool::Point(SeconPol, TI[0]),
                        ...
                        IntPatch_PolyhedronTool::Point(SeconPol, TI[2]), TNor, Tdp);
    ...
    // Otherwise line of section is calculated:
        ...
        for (iObj = 0; iObj < 3; iObj++) {
            for (iToo = 0; iToo < 3; iToo++) {
                if (dpOpT[iObj][iToo] <= floatGap) {
                    piOT.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(FirstPol, OI[iObj]), Intf_VERTEX,
                                                  OI[iObj], 0, 0., Intf_VERTEX, TI[iToo], 0, 0., Incidence));
                    ...
                    if (edTT[iToo] == 1) {
                        if (dpOeT[iObj][iToo] <= floatGap && dpOeT[iObj][iToo] >= -floatGap) {
                            if ((dpOpT[iObj][iToo] + dpOpT[iObj][inext]) < vtt[iToo].Modulus()) {
                                ...
                                piOT.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(FirstPol, OI[iObj]),
                                                              Intf_VERTEX, OI[iObj], 0, 0., Intf_EDGE,
                                                              Min(TI[iToo], TI[inext]), Max(TI[iToo], TI[inext]),
                                                              parT[iToo], Incidence));
                                parO[iObj] = 0.;
                        ...
                        if (Abs(deOpT[iObj][iToo]) <= floatGap) {
                            if ((dpOpT[iObj][iToo] + dpOpT[inext][iToo]) < voo[iObj].Modulus()) {
                                ...
                                piOT.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(SeconPol, TI[iToo]),
                                                              ...
                                                              Max(OI[iObj], OI[inext]), parO[iObj], Intf_VERTEX,
                                                              TI[iToo], 0, 0., Incidence));
                                parT[iToo] = 0.;
        ...
        for (iToo = 0; iToo < 3; iToo++) {
            if (parT[iToo] != 0.) {
                if (Abs(dfOpT[iToo]) <= floatGap) {
                    piOT.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(SeconPol, TI[iToo]), Intf_FACE, Tri1,
                                                  0, 0., Intf_VERTEX, TI[iToo], 0, 0., Incidence));
                    parT[iToo] = 0.;
        ...
        for (iObj = 0; iObj < 3; iObj++) {
            if (parO[iObj] != 0.) {
                if (Abs(dpOfT[iObj]) <= floatGap) {
                    piOT.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(FirstPol, OI[iObj]), Intf_VERTEX,
                                                  OI[iObj], 0, 0., Intf_FACE, Tri2, 0, 0., Incidence));
                    parO[iObj] = 0.;
                        ...
                        if (edTT[iToo] == 1 && (dfOpT[iToo] * dfOpT[jnext]) < 0.) {
                            ...
                            if (lg > 0. && lg < 1.) {
                                ...
                                if (Pb == Standard_False) {
                                    ...
                                    lg = piT.Modulus();
                                    if (lg <= floatGap) {
                                        piOT.Append(Intf_SectionPoint(piO, Intf_EDGE, Min(OI[iObj], OI[inext]),
                                                                      Max(OI[iObj], OI[inext]), parO[iObj], Intf_EDGE,
                                                                      Min(TI[iToo], TI[jnext]),
                                                                      Max(TI[iToo], TI[jnext]), parT[iToo], Incidence));
        ...
        for (iObj = 0; iObj < 3; iObj++) {
            ...
            if (edOT[iObj] == 1 && (dpOfT[iObj] * dpOfT[inext]) < 0.) {
                ...
                if (lg > 0. && lg < 1.) {
                    parO[iObj] = lg;
                    piO = (IntPatch_PolyhedronTool::Point(FirstPol, OI[iObj]).XYZ()) + (voo[iObj] * parO[iObj]);
                    if (OI[iObj] > OI[inext]) parO[iObj] = 1. - parO[iObj];
                    piOT.Append(Intf_SectionPoint(piO, Intf_EDGE, Min(OI[iObj], OI[inext]), Max(OI[iObj], OI[inext]),
                                                  parO[iObj], Intf_FACE, Tri2, 0, 0., Incidence));
                    nbpiOT++;
                    nbpiO++;
                }
            }
        ...
        for (iToo = 0; iToo < 3; iToo++) {
            ...
            if (edTT[iToo] == 1 && (dfOpT[iToo] * dfOpT[jnext]) < 0.) {
                ...
                if (lg > 0. && lg < 1.) {
                    parT[iToo] = lg;
                    piO = (IntPatch_PolyhedronTool::Point(SeconPol, TI[iToo]).XYZ()) + (vtt[iToo] * parT[iToo]);
                    if (TI[iToo] > TI[jnext]) parT[iToo] = 1. - parT[iToo];
                    piOT.Append(Intf_SectionPoint(piO, Intf_FACE, Tri1, 0, 0., Intf_EDGE, Min(TI[iToo], TI[jnext]),
                                                  Max(TI[iToo], TI[jnext]), parT[iToo], Incidence));
                    nbpiOT++;
                    nbpiT++;
                }
                ...
                if (ifin < 0) {
                    if (ideb != -999) {
                        ...
                        if (d < Tolerance) {
                            ...
                            //-- std::cout<<"point p"<<++nisp<<" "<<piOT(-ideb).Pnt().X()<<" "<<piOT(-ideb).Pnt().Y()<<"
                            //"<<piOT(-ideb).Pnt().Z()<<std::endl;
                        } else {
                            //-- std::cout<<"Insertion Point IntPatch_InterferencePolyhedron 1,2 d="<<d<<"
...
Standard_Boolean IntPatch_InterferencePolyhedron::TangentZoneValue(Intf_TangentZone& TheTZ,
    ...
    for (nob = 0; nob <= 2; nob++) {
        ...
        for (nou = 0; nou <= 2; nou++) {
            ...
            if (dpOpT[nob][nou] <= Tolerance) {
                Tpi.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(FirstPol, OI[nob]), Intf_VERTEX, OI[nob], 0,
                                             0., Intf_VERTEX, TI[nou], 0, 0., 1.));
                tOP[nob] = Intf_VERTEX;
                tTP[nou] = Intf_VERTEX;
                nbpInt++;
                break;
                ...
                if (dpOpT[nob][nou] + dpOpT[nob][nou2] < vtt[nou].Modulus()) {
                    ...
                    Tpi.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(FirstPol, OI[nob]), Intf_VERTEX,
                                                 OI[nob], 0, 0., Intf_EDGE, Min(TI[nou], TI[nou2]),
                                                 Max(TI[nou], TI[nou2]), par, 1.));
                    tOP[nob] = Intf_EDGE;
                    nbpInt++;
                    break;
                }
    ...
    if (nbpInt < 3) {
            ...
            if (tTP[nou] == Intf_EXTERNAL) {
                for (nob = 0; nob <= 2; nob++) {
                    ...
                    if (Abs(deOpT[nob][nou]) <= Tolerance) {
                        if (dpOpT[nob][nou] + dpOpT[nob2][nou] < voo[nob].Modulus()) {
                            ...
                            Tpi.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(SeconPol, TI[nou]), Intf_EDGE,
                                                         Min(OI[nob], OI[nob2]), Max(OI[nob], OI[nob2]), par,
                ...
                if (tTP[nou] == Intf_EXTERNAL) {
                    if (Intf::Contain(IntPatch_PolyhedronTool::Point(FirstPol, OI[0]),
                        ...
                        Tpi.Append(Intf_SectionPoint(IntPatch_PolyhedronTool::Point(SeconPol, TI[nou]), Intf_FACE, Tri1,
                                                     0, 0., Intf_VERTEX, TI[nou], 0, 0., 1.));
                        tTP[nou] = Intf_FACE;
                        nbpInt++;
                    }
                }
        ...
        if (tTP[0] != Intf_EXTERNAL && tTP[1] != Intf_EXTERNAL && tTP[2] != Intf_EXTERNAL) finished = Standard_True;
    } else
            ...
            for (nou = 0; nou < 3; nou++) {
                ...
                if (dpOeT[nob][nou] * dpOeT[nob2][nou] < 0. && deOpT[nob][nou] * deOpT[nob][nou2] < 0.) {
                    ...
                    if (nbpInt >= 5) {
                        ...
                        Tpi.Append(Intf_SectionPoint(lepi, Intf_EDGE, Min(OI[nob], OI[nob2]), Max(OI[nob], OI[nob2]),
                                                     parO[nbpInt], Intf_EDGE, Min(TI[nou], TI[nou2]),
                                                     Max(TI[nou], TI[nou2]), parT[nbpInt], Incidence));
                        nbpInt++;
...
void IntPatch_InterferencePolyhedron::CoupleCharacteristics(const IntPatch_Polyhedron& FirstPol,
    ...
    if (vnorT.Modulus() > vvec.Modulus()) vvec = vnorT;
