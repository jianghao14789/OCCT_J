...
IntPatch_WLine::IntPatch_WLine(const Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean Tang,
                               const IntSurf_TypeTrans Trans1, const IntSurf_TypeTrans Trans2)
    : IntPatch_PointLine(Tang, Trans1, Trans2), fipt(Standard_False), lapt(Standard_False), hasArcOnS1(Standard_False),
      hasArcOnS2(Standard_False), myIsPurgerAllowed(Standard_True), myCreationWay(IntPatch_WLUnknown) {
...
IntPatch_WLine::IntPatch_WLine(const Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean Tang,
                               const IntSurf_Situation Situ1, const IntSurf_Situation Situ2)
    : IntPatch_PointLine(Tang, Situ1, Situ2), fipt(Standard_False), lapt(Standard_False), hasArcOnS1(Standard_False),
      hasArcOnS2(Standard_False), myIsPurgerAllowed(Standard_True), myCreationWay(IntPatch_WLUnknown) {
    typ = IntPatch_Walking;
    curv = Line;
    u1period = v1period = u2period = v2period = 0.0;
...
IntPatch_WLine::IntPatch_WLine(const Handle(IntSurf_LineOn2S) & Line, const Standard_Boolean Tang)
    : IntPatch_PointLine(Tang), fipt(Standard_False), lapt(Standard_False), hasArcOnS1(Standard_False),
      hasArcOnS2(Standard_False), myIsPurgerAllowed(Standard_True), myCreationWay(IntPatch_WLUnknown) {
    typ = IntPatch_Walking;
    curv = Line;
    u1period = v1period = u2period = v2period = 0.0;
...
static void RecadreMemePeriode(Standard_Real& u1, Standard_Real& v1, Standard_Real& u2, Standard_Real& v2,
                               const Standard_Real anu1, const Standard_Real anv1, const Standard_Real anu2,
                               const Standard_Real anv2, const Standard_Real U1Period, const Standard_Real V1Period,
...
static void RecadreMemePeriode(IntSurf_PntOn2S& POn2S, const IntSurf_PntOn2S& RefPOn2S, const Standard_Real up1,
                               const Standard_Real vp1, const Standard_Real up2, const Standard_Real vp2) {
...
void IntPatch_WLine::SetPeriod(const Standard_Real pu1, const Standard_Real pv1, const Standard_Real pu2,
...
static Standard_Boolean CompareVerticesOnSurf(const IntPatch_Point& vtx1, const IntPatch_Point& vtx2,
                    ...
                    if (VTXi.ParameterOnLine() != VTXj.ParameterOnLine()) {
                        ...
                        if (d < maxtol || CompareVerticesOnS1(VTXi, VTXj) || CompareVerticesOnS2(VTXi, VTXj)) {
                            ...
                            if (acreer) {
                                IntPatch_Point vtx;
                                vtx = VTXj;
                                vtx.SetArc(Standard_True, VTXi.ArcOnS1(), VTXi.ParameterOnArc1(),
                            ...
                            if (acreer) {
                                IntPatch_Point vtx;
                                vtx = VTXj;
                                vtx.SetArc(Standard_False, VTXi.ArcOnS2(), VTXi.ParameterOnArc2(),
                                           VTXi.TransitionLineArc2(), VTXi.TransitionOnS2());
                                for (k = 1; encoreunefois == Standard_False && k <= nbvtx; k++) {
                                    const IntPatch_Point& VTXk = svtx.Value(k);
                                    if (SameVtxRst(VTXk, vtx)) {
                                        encoreunefois = Standard_True;
        ...
        if (indicevertexonline < 1) {
            ...
            if (Substitution == Standard_False) {
                ...
                if (indicevertexonline >= nbponline) {
                    ...
                    RecadreMemePeriode(POn2S, curv->Value(indicevertexonline + 1), U1Period(), V1Period(), U2Period(),
                                       V2Period());
        ...
        case 0:
            ...
            for (Standard_Integer i = 1; i <= aNbPoints; i++) {
                ...
                printf("%4d  [%+10.20f %+10.20f %+10.20f]  [%+10.20f %+10.20f]  [%+10.20f %+10.20f]\n", i,
                       Point(i).Value().X(), Point(i).Value().Y(), Point(i).Value().Z(), u1, v1, u2, v2);
            }
