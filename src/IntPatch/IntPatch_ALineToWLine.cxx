...
// purpose  :
//=======================================================================
static inline void AddPointIntoLine(Handle(IntSurf_LineOn2S) & theLine, const Standard_Real* const theArrPeriods,
                                    IntSurf_PntOn2S& thePoint, IntPatch_Point* theVertex = 0) {
    if (theLine->NbPoints() > 0) {
        if (thePoint.IsSame(theLine->Value(theLine->NbPoints()), Precision::Confusion())) return;

        IntPatch_SpecialPoints::AdjustPointAndVertex(theLine->Value(theLine->NbPoints()), theArrPeriods, thePoint,
                                                     theVertex);
    }
...
//=======================================================================
static void AddVertexPoint(Handle(IntSurf_LineOn2S) & theLine, IntPatch_Point& theVertex,
...
static IntPatch_SpecPntType IsPoleOrSeam(const Handle(Adaptor3d_Surface) & theS1,
                                         const Handle(Adaptor3d_Surface) & theS2, const IntSurf_PntOn2S& thePIsoRef,
                                         Handle(IntSurf_LineOn2S) & theLine, IntPatch_Point& theVertex,
                                         const Standard_Real theArrPeriods[4], const Standard_Real theTol3d,
    ...
    for (Standard_Integer i = 0; i < 2; i++) {
        ...
        switch (aType) {
            case GeomAbs_Sphere:
            case GeomAbs_Cone: {
                if (IntPatch_SpecialPoints::AddSingularPole((isReversed ? theS2 : theS1), (isReversed ? theS1 : theS2),
                                                            thePIsoRef, theVertex, anApexPoint, isReversed,
                                                            Standard_True)) {
                    anAddedPType = IntPatch_SPntPole;
                    break;
                }
            }
            Standard_FALLTHROUGH case GeomAbs_Torus:
                if (aType == GeomAbs_Torus) {
                    if (IntPatch_SpecialPoints::AddCrossUVIsoPoint((isReversed ? theS2 : theS1),
                                                                   (isReversed ? theS1 : theS2), thePIsoRef, theTol3d,
...
IntPatch_ALineToWLine::IntPatch_ALineToWLine(const Handle(Adaptor3d_Surface) & theS1,
    ...
    : myS1(theS1), myS2(theS2), myNbPointsInWline(theNbPoints), myTolOpenDomain(1.e-9), myTolTransition(1.e-8),
...
// purpose  :
//=======================================================================
void IntPatch_ALineToWLine::CorrectEndPoint(Handle(IntSurf_LineOn2S) & theLine, const Standard_Integer theIndex) const {
    ...
    for (Standard_Integer ii = 1; ii <= 2; ii++) {
        ...
        if (aQuad.TypeQuadric() == GeomAbs_Cone) {
            ...
            if (anApex.SquareDistance(aPntOn2S.Value()) > aSqTol) continue;
        } else if (aQuad.TypeQuadric() == GeomAbs_Sphere) {
            Standard_Real aU, aV;
            aPntOn2S.ParametersOnSurface(anIsOnFirst, aU, aV);
            if (Abs(aV - M_PI / 2) > aTol && Abs(aV + M_PI / 2) > aTol) continue;
        } else
            continue;
        ...
        if (Abs(aDir.Y()) < gp::Resolution()) continue;
...
// purpose  :
//=======================================================================
void IntPatch_ALineToWLine::MakeWLine(const Handle(IntPatch_ALine) & theALine, const Standard_Real theFPar,
                                      const Standard_Real theLPar, IntPatch_SequenceOfLine& theLines) const {
  ...
  ********************************** Script (end) */
    ...
    while (aParameter < theLPar) {
        ...
        if (aStep < Epsilon(theLPar)) break;
        ...
        for (; !isLast; aParameter += aStep) {
            ...
            if (aPrePointExist != IntPatch_SPntNone) {
                ...
                if (IntPatch_SpecialPoints::ContinueAfterSpecialPoint(myS1, myS2, aRPT, aPrePointExist, aTol2d,
            ...
            for (Standard_Integer i = aVertexParams.Lower(); i <= aVertexParams.Upper(); i++) {
                if (hasVertexBeenChecked(i)) continue;
            ...
            if (aVertexNumber < 0) {
                if (isPointValid) {
                    StepComputing(theALine, aPOn2S, theLPar, aParameter, aTgMagn, aStepMin, aStepMax, myTol3D, aStep);
            ...
            aPrePointExist = IsPoleOrSeam(myS1, myS2, aPrefIso, aLinOn2S, aVtx, anArrPeriods, aTol, aSingularSurfaceID);
            ...
            if ((aPrePointExist != IntPatch_SPntNone) && (aLinOn2S->NbPoints() > 1)) break;
        } // for(; !isLast; aParameter += aStep)
        ...
        if (aLinOn2S->NbPoints() >= 3) {
            if (anIsFirstDegenerated) CorrectEndPoint(aLinOn2S, 1);
            if (anIsLastDegenerated) CorrectEndPoint(aLinOn2S, aLinOn2S->NbPoints());
        }
        ...
        if (theALine->TransitionOnS1() == IntSurf_Touch) {
            aWLine =
                new IntPatch_WLine(aLinOn2S, theALine->IsTangent(), theALine->SituationS1(), theALine->SituationS2());
            ...
            const gp_Pnt &aPP0 = aLinOn2S->Value(indice1 - 1).Value(), &aPP1 = aLinOn2S->Value(indice1).Value();
...
Standard_Integer IntPatch_ALineToWLine::CheckDeflection(const gp_XYZ& theMidPt,
    ...
    if (aDist > theMaxDeflection) return 1;

    aDist = Max(Abs(myQuad2.Distance(theMidPt)), aDist);

    if (aDist > theMaxDeflection) return 1;

    if ((aDist + aDist) < theMaxDeflection) return -1;
...
// purpose  :
//=======================================================================
Standard_Boolean IntPatch_ALineToWLine::StepComputing(
    const Handle(IntPatch_ALine) & theALine, const IntSurf_PntOn2S& thePOn2S, const Standard_Real theLastParOfAline,
    const Standard_Real theCurParam, const Standard_Real theTgMagnitude, const Standard_Real theStepMin,
    const Standard_Real theStepMax, const Standard_Real theMaxDeflection, Standard_Real& theStep) const {
    if (theTgMagnitude < Precision::Confusion()) return Standard_False;
...
#if 0
  {
    ...
    if (aNbIter > aNbIterMax) return Standard_False;
