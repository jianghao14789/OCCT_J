...
//=============================================================================
GeomToIGES_GeomSurface::GeomToIGES_GeomSurface() : GeomToIGES_GeomEntity() {
...
GeomToIGES_GeomSurface::GeomToIGES_GeomSurface(const GeomToIGES_GeomEntity& GE) : GeomToIGES_GeomEntity(GE) {
    myBRepMode = Standard_False;
    myAnalytic = Standard_False;
}
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_Surface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_BSplineSurface) & start,
    ...
    if (!PeriodU) {
        ...
        if (Abs(Umin - U0) < Precision::PConfusion()) Umin = U0;
        if (Abs(Umax - U1) < Precision::PConfusion()) Umax = U1;
        uShift = ShapeAnalysis::AdjustToPeriod(Umin, U0, U1);
        Umin += uShift;
        Umax += uShift;
        if (Umax - Umin > U1 - U0) Umax = Umin + (U1 - U0);
    }
    if (!PeriodV) {
        if (Vmin < V0) Vmin = V0;
        if (V1 < Vmax) Vmax = V1;
    } else {
        if (Abs(Vmin - V0) < Precision::PConfusion()) Vmin = V0;
        if (Abs(Vmax - V1) < Precision::PConfusion()) Vmax = V1;
        vShift = ShapeAnalysis::AdjustToPeriod(Vmin, V0, V1);
        Vmin += vShift;
        Vmax += vShift;
        if (Vmax - Vmin > V1 - V0) Vmax = Vmin + (V1 - V0);
    }
    ...
    BSpline->Init(IndexU, IndexV, DegU, DegV, CloseU, CloseV, Polynom, PeriodU, PeriodV, KnotsU, KnotsV, Weights, Poles,
                  Umin, Umax, Vmin, Vmax);
    res = BSpline;
    return res;
}
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_RectangularTrimmedSurface) &
                                                                        start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_Plane) & start,
    ...
    if (Interface_Static::IVal("write.iges.plane.mode") == 0) {
        ...
        // #32 rln 19.10.98
        BSpline->Init(1, 1, 1, 1, Standard_False, Standard_False, Standard_True, Standard_False, Standard_False, KnotsU,
                      KnotsV, Weights, Poles, Udeb, Ufin, Vdeb, Vfin);
        res = BSpline;
        return res;
    }
}
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_CylindricalSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
    ...
    Handle(Geom_Line) Ligne = new Geom_Line(gp_Pnt(start->Cylinder().Radius(), 0.0, 0.0), gp_Dir(0.0, 0.0, 1.0));
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_ConicalSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
    ...
    // creation de la generatrice : Generatrix
    Handle(Geom_Line) Ligne = new Geom_Line(gp_Pnt(start->Cone().RefRadius(), 0.0, 0.0),
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_SphericalSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_ToroidalSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_SweptSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
    Handle(IGESData_IGESEntity) res;
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_SurfaceOfLinearExtrusion) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
    ...
    if (Abs(V1) > Precision::Confusion()) {
        CopyCurve = Handle(Geom_Curve)::DownCast(TheCurve->Translated(start->Value(U1, 0.), start->Value(U1, V1)));
    } else {
        CopyCurve = TheCurve;
    }
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_SurfaceOfRevolution) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSurface(const Handle(Geom_OffsetSurface) & start,
                                                                    const Standard_Real Udeb, const Standard_Real Ufin,
                                                                    const Standard_Real Vdeb,
                                                                    const Standard_Real Vfin) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferCylindricalSurface(
    const Handle(Geom_CylindricalSurface) & start, const Standard_Real /*Udeb*/, const Standard_Real /*Ufin*/,
    const Standard_Real /*Vdeb*/, const Standard_Real /*Vfin*/) {
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomSurface::TransferSphericalSurface(
    const Handle(Geom_SphericalSurface) & start, const Standard_Real /*Udeb*/, const Standard_Real /*Ufin*/,
    const Standard_Real /*Vdeb*/, const Standard_Real /*Vfin*/) {
