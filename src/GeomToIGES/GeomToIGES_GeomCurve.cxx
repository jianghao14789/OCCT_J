...
GeomToIGES_GeomCurve::GeomToIGES_GeomCurve(const GeomToIGES_GeomEntity& GE) : GeomToIGES_GeomEntity(GE) {}
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_BoundedCurve) & start,
                                                                const Standard_Real Udeb, const Standard_Real Ufin) {
    Handle(IGESData_IGESEntity) res;
    if (start.IsNull()) {
        return res;
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_BSplineCurve) & start,
                                                                const Standard_Real Udeb, const Standard_Real Ufin) {
    Handle(IGESData_IGESEntity) res;
    if (start.IsNull()) {
        return res;
    ...
    if (Umin - First > Precision::PConfusion() || Last - Umax > Precision::PConfusion()) {
        try {
            ...
            if (!bspl.IsNull()) {
                if (Abs(Umax - Umin) > Precision::PConfusion()) bspl->Segment(Umin, Umax);
                mycurve = bspl;
            }
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_BezierCurve) & start,
                                                                const Standard_Real Udeb, const Standard_Real Ufin) {
    Handle(IGESData_IGESEntity) res;
    if (start.IsNull()) {
        return res;
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_TrimmedCurve) & start,
                                                                const Standard_Real Udeb, const Standard_Real Ufin) {
    Handle(IGESData_IGESEntity) res;
    if (start.IsNull()) {
        return res;
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_Circle) & start,
    ...
    Circle->Init(Zc / GetUnit(), gp_XY(Xc / GetUnit(), Yc / GetUnit()), gp_XY(Xs / GetUnit(), Ys / GetUnit()),
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_Ellipse) & start,
    ...
    if (Abs(Ufin - Udeb - 2 * M_PI) <= Precision::PConfusion()) {
        ...
        if (Bspline.IsNull()) GeomConvert::CurveToBSplineCurve(copystart, Convert_QuasiAngular);
    ...
    gp_Elips2d E2d = gp_Elips2d(gp_Ax22d(gp::Origin2d(), gp::DX2d(), gp::DY2d()), (start->MajorRadius() / GetUnit()),
    ...
    Conic->Init(A, 2 * B, C, 2 * D, 2 * E, F, 0., // #59 rln
                gp_XY(Xs / GetUnit(), Ys / GetUnit()), gp_XY(Xe / GetUnit(), Ye / GetUnit()));
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_Hyperbola) & start,
    ...
    Build.EvalXYZ((start->Value(U1)).XYZ(), Xs, Ys, Zs);
    Build.EvalXYZ((start->Value(U2)).XYZ(), Xe, Ye, Ze);
    gp_Hypr2d H2d = gp_Hypr2d(gp_Ax22d(gp::Origin2d(), gp::DX2d(), gp::DY2d()), (start->MajorRadius() / GetUnit()),
                              (start->MinorRadius() / GetUnit()));
    Standard_Real A, B, C, D, E, F;
    H2d.Coefficients(A, C, B, D, E, F);

    Conic->Init(A, B, C, D, E, F, 0., gp_XY(Xs / GetUnit(), Ys / GetUnit()), gp_XY(Xe / GetUnit(), Ye / GetUnit()));
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_Parabola) & start,
    ...
    Build.EvalXYZ((start->Value(U1)).XYZ(), Xs, Ys, Zs);
    Build.EvalXYZ((start->Value(U2)).XYZ(), Xe, Ye, Ze);
    gp_Parab2d P2d = gp_Parab2d(gp_Ax22d(gp::Origin2d(), gp::DX2d(), gp::DY2d()), (start->Focal() * 2.));
    Standard_Real A, B, C, D, E, F;
...
Handle(IGESData_IGESEntity) GeomToIGES_GeomCurve::TransferCurve(const Handle(Geom_OffsetCurve) & start,
                                                                const Standard_Real Udeb, const Standard_Real Ufin) {
    Handle(IGESData_IGESEntity) res;
    if (start.IsNull()) {
        return res;
    ...
    OffsetC->Init(BaseCurve, 1, voident, 0, 0, (start->Offset() / GetUnit()), 0., (start->Offset() / GetUnit()), 0.,
