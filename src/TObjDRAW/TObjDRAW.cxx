...
public:
    Standard_EXPORT TObjDRAW_Model() : TObj_Model() {}
...
public:
    Standard_EXPORT TObjDRAW_Object(const TDF_Label& theLab) : TObj_Object(theLab) {}
...
static Standard_Integer getChildren(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    Handle(TObj_ObjectIterator) anItr =
        aGetSubs ? new TObj_OcafObjectIterator(tObj->GetChildLabel(), NULL, Standard_True, Standard_True)
                 : tObj->GetChildren();
...
void TObjDRAW::Init(Draw_Interpretor& di) {
    ...
    di.Add("TObjNew", "DocName \t: Create new TObj model with document named DocName", __FILE__, newModel, g);

    di.Add("TObjSave", "DocName [Path] [-stream] \t: Save Model with DocName [by file path] [into a file stream]",
           __FILE__, saveModel, g);

    di.Add("TObjLoad", "DocName Path [-stream] \t: Load model DocName from file Path [file stream]", __FILE__,
           loadModel, g);
    ...
    di.Add("TObjGetVal", "DocName ObjName1 -i | -r \t: Returns one integer or set of real values", __FILE__, getVal, g);

    di.Add("TObjSetRef", "DocName ObjName1 ObjName2 \t: Set reference from object1 to object2", __FILE__, setRef, g);

    di.Add("TObjGetRef", "DocName ObjName \t: Returns list of children objects", __FILE__, getRef, g);

    di.Add("TObjAddChild", "DocName ObjName chldName \t: Add child object to indicated object", __FILE__, addChild, g);

    di.Add("TObjGetChildren", "DocName ObjName [-all]\t: Returns list of children objects (-all to recurse)", __FILE__,
           getChildren, g);

    di.Add(
        "TObjHasModifications",
        "DocName ObjName \t: Returns status of modification of the object (if object has been modified 1, otherwise 0)",
        __FILE__, hasModifications, g);
}
