...
IntAna_Curve::IntAna_Curve()
    : Z0Cte(0.0), Z0Sin(0.0), Z0Cos(0.0), Z0SinSin(0.0), Z0CosCos(0.0), Z0CosSin(0.0), Z1Cte(0.0), Z1Sin(0.0),
      Z1Cos(0.0), Z1SinSin(0.0), Z1CosCos(0.0), Z1CosSin(0.0), Z2Cte(0.0), Z2Sin(0.0), Z2Cos(0.0), Z2SinSin(0.0),
      Z2CosCos(0.0), Z2CosSin(0.0), TwoCurves(Standard_False), TakeZPositive(Standard_False), Tolerance(0.0),
      DomainInf(0.0), DomainSup(0.0), RestrictedInf(Standard_False), RestrictedSup(Standard_False),
      firstbounded(Standard_False), lastbounded(Standard_False), typequadric(GeomAbs_OtherSurface), RCyl(0.0),
      Angle(0.0), myFirstParameter(0.0), myLastParameter(0.0) {}
//=======================================================================
// function : SetConeQuadValues
// purpose  : Description de l intersection Cone Quadrique
//=======================================================================
void IntAna_Curve::SetConeQuadValues(const gp_Cone& Cone, const Standard_Real Qxx, const Standard_Real Qyy,
                                     const Standard_Real Qzz, const Standard_Real Qxy, const Standard_Real Qxz,
                                     const Standard_Real Qyz, const Standard_Real Qx, const Standard_Real Qy,
                                     const Standard_Real Qz, const Standard_Real Q1, const Standard_Real TOL,
                                     const Standard_Real DomInf, const Standard_Real DomSup,
                                     const Standard_Boolean twocurves, const Standard_Boolean takezpositive) {
...
//=======================================================================
void IntAna_Curve::SetCylinderQuadValues(const gp_Cylinder& Cyl, const Standard_Real Qxx, const Standard_Real Qyy,
                                         const Standard_Real Qzz, const Standard_Real Qxy, const Standard_Real Qxz,
                                         const Standard_Real Qyz, const Standard_Real Qx, const Standard_Real Qy,
                                         const Standard_Real Qz, const Standard_Real Q1, const Standard_Real TOL,
                                         const Standard_Real DomInf, const Standard_Real DomSup,
...
// purpose  :
//=======================================================================
void IntAna_Curve::InternalUVValue(const Standard_Real theta, Standard_Real& Param1, Standard_Real& Param2,
                                   Standard_Real& A, Standard_Real& B, Standard_Real& C, Standard_Real& cost,
                                   Standard_Real& sint, Standard_Real& SigneSqrtDis) const {
    ...
    A = Z2Cte + sint * (Z2Sin + sint * Z2SinSin) + cost * (Z2Cos + cost * Z2CosCos) + Z2CosSin * aSin2t;

    const Standard_Real aDA =
        cost * Z2Sin - sint * Z2Cos + aSin2t * (Z2SinSin - Z2CosCos) + aCos2t * (Z2CosSin * Z2CosSin);

    B = Z1Cte + sint * (Z1Sin + sint * Z1SinSin) + cost * (Z1Cos + cost * Z1CosCos) + Z1CosSin * aSin2t;

    const Standard_Real aDB =
        Z1Sin * cost - Z1Cos * sint + aSin2t * (Z1SinSin - Z1CosCos) + aCos2t * (Z1CosSin + Z1CosSin);

    C = Z0Cte + sint * (Z0Sin + sint * Z0SinSin) + cost * (Z0Cos + cost * Z0CosCos) + Z0CosSin * aSin2t;
    ...
    if (aDiscriminant < aTolD) aDiscriminant = 0.0;
...
void IntAna_Curve::FindParameter(const gp_Pnt& theP, TColStd_ListOfReal& theParams) const {
    ...
    for (Standard_Integer i = 0; i < aMaxPar; i++) {
        if (aParams[i] > myLastParameter) break;

        if (aParams[i] < myFirstParameter) continue;

        if (i && (aParams[i] - aParams[i - 1]) < Precision::PConfusion()) continue;

        Standard_Real U = 0.0, V = 0.0, A = 0.0, B = 0.0, C = 0.0, sint = 0.0, cost = 0.0, SigneSqrtDis = 0.0;
