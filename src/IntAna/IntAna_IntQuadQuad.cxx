...
static void AddSpecialPoints(const IntAna_Quadric& theQuad, const gpSmth& theGpObj, Standard_Real& theTheta1,
                             Standard_Real& theTheta2) {
    const Standard_Real aPeriod = M_PI + M_PI;
    const NCollection_List<gp_Pnt>& aLSP = theQuad.SpecialPoints();

    if (aLSP.IsEmpty()) return;
...
public:
    TrigonometricRoots(const Standard_Real CC, const Standard_Real SC, const Standard_Real C, const Standard_Real S,
                       const Standard_Real Cte, const Standard_Real Binf, const Standard_Real Bsup);
    // IsDone
...
// purpose  :
//=======================================================================
TrigonometricRoots::TrigonometricRoots(const Standard_Real CC, const Standard_Real SC, const Standard_Real C,
                                       const Standard_Real S, const Standard_Real Cte, const Standard_Real Binf,
...
public:
    //
    MyTrigonometricFunction(const Standard_Real xCC, const Standard_Real xSS, const Standard_Real xSC,
                            const Standard_Real xC, const Standard_Real xS, const Standard_Real xCte) {
        CC = xCC;
        SS = xSS;
        SC = xSC;
        S = xS;
    ...
    Standard_Real Value(const Standard_Real& U) {
        ...
        sinus = sin(U);
        cosinus = cos(U);
        aRet = CC * cosinus * cosinus + SS * sinus * sinus + 2.0 * (sinus * (SC * cosinus + S) + cosinus * C) + Cte;
        //
        return aRet;
    }
    //
    Standard_Real Derivative(const Standard_Real& U) {
        ...
        //
        return (2.0 *
                ((sinus * cosinus) * (SS - CC) + S * cosinus - C * sinus + SC * (cosinus * cosinus - sinus * sinus)));
    }
};
...
//=======================================================================
IntAna_IntQuadQuad::IntAna_IntQuadQuad(const gp_Cylinder& Cyl, const IntAna_Quadric& Quad, const Standard_Real Tol) {
    myNbMaxCurves = 12;
    myEpsilon = 0.00000001;
    myEpsilonCoeffPolyNull = 0.00000001;
...
// purpose  : I n t e r s e c t i o n   C y l i n d r e   Q u a d r i q u e
//=======================================================================
void IntAna_IntQuadQuad::Perform(const gp_Cylinder& Cyl, const IntAna_Quadric& Quad, const Standard_Real) {
    done = Standard_True;
    identical = Standard_False;
    NbCurves = 0;
    Nbpoints = 0;
    ...
    if (Abs(Qzz) < myEpsilonCoeffPolyNull) {
        ...
        if (PolDIS.InfiniteRoots()) {
            TheCurve[0].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0., PIpPI,
                                              UN_SEUL_Z_PAR_THETA, Z_POSITIF);
            TheCurve[1].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0., PIpPI,
            ...
            if (nbsolDIS == 0) {
                ...
                if (MTF.Value(M_PI) >= -aRealEpsilon) {
                    ...
                    TheCurve[0].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0.0,
                                                      PIpPI, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                    TheCurve[1].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0.0,
                                                      PIpPI, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
                ...
                if (nbsolDIS == 1) {
                    ...
                    if (MTF.Value(PolDIS.Value(1) + M_PI) >= -aRealEpsilon) {
                        ...
                        //------------------------------------------------------------
                        TheCurve[0].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                          0.0, PIpPI, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                        TheCurve[1].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                          0.0, PIpPI, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
                        ...
                        for (i = 1; i <= nbsolDIS; i++) {
                            ...
                            if (Abs(Theta2 - Theta1) <= aRealEpsilon) {
                                ...
                                if (qwet >= 0.) {
                                    ...
                                    TheCurve[NbCurves].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy,
                                                                             Qz, Q1, myEpsilon, Theta1, aParam,
                                                                             UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                                    NbCurves++;
                                    TheCurve[NbCurves].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy,
                                                                             Qz, Q1, myEpsilon, Theta1, aParam,
                        ...
                        if (Abs(Theta2 - Theta1) <= 1e-12) {
                            ...
                            if (qwet >= 0.) {
                                ...
                                if ((Theta3 - Theta2) < 5.e-8) {
                                    ...
                                    TheCurve[NbCurves].SetCylinderQuadValues(Cyl, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy,
                                                                             Qz, Q1, myEpsilon, Theta1, Theta2,
                                                                             DEUX_Z_PAR_THETA, Z_INDIFFERENT);
                                    NbCurves++;
                                }
                            } // if(qwet >= 0.)
...
// purpose  :
//=======================================================================
IntAna_IntQuadQuad::IntAna_IntQuadQuad(const gp_Cone& Cone, const IntAna_Quadric& Quad, const Standard_Real Tol) {
    myNbMaxCurves = 12;
    myEpsilon = 0.00000001;
    myEpsilonCoeffPolyNull = 0.00000001;
...
// purpose  :
//=======================================================================
void IntAna_IntQuadQuad::Perform(const gp_Cone& Cone, const IntAna_Quadric& Quad, const Standard_Real) {
    //
    ...
    if (PolZ2.InfiniteRoots()) { // i.e A(t)=0 for any t
        if (!PolZ1.InfiniteRoots()) {
            if (!nbsol1) {
                ...
                TheCurve[0].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0., PIpPI,
                                              UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                NbCurves = 1;
            } else {
                /*
    ...
    if (Pol.InfiniteRoots()) {
        ...
        TheCurve[0].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0., PIpPI,
                                      UN_SEUL_Z_PAR_THETA, Z_POSITIF);
        TheCurve[1].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, 0., PIpPI,
                                      UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
        NbCurves = 2;
        return;
    }
    ...
    for (i = 1; i <= nbsol; ++i) {
        ...
        if (!RacinesdePolZ2DansTheta1Theta2) {
            ...
            TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, Theta1,
                                                 Theta2, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
            NbCurves++;
            TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon, Theta1,
                                                 Theta2, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
            NbCurves++;
        }
            ...
            for (i2 = 1; i2 <= (nbsolZ2 + nbsolZ2); ++i2) {
                ...
                if (to < NewMax && to > NewMin) {
                    ...
                    TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                         NewMin, to, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
                    //
                    NbCurves++;
                    TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                         NewMin, to, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                    //------------------------------------------------------------
                    //-- A == 0
            ...
            if (NoChanges) {
                TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                     Theta1, Theta2, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
                NbCurves++;
                TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                     Theta1, Theta2, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                //------------------------------------------------------------
                //-- A == 0
                //-- Si B < 0    Alors Infini sur   Z+
                //-- Sinon             Infini sur   Z-
                ...
                TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                     NewMin, Theta2, UN_SEUL_Z_PAR_THETA, Z_NEGATIF);
                NbCurves++;
                TheCurve[NbCurves].SetConeQuadValues(Cone, Qxx, Qyy, Qzz, Qxy, Qxz, Qyz, Qx, Qy, Qz, Q1, myEpsilon,
                                                     NewMin, Theta2, UN_SEUL_Z_PAR_THETA, Z_POSITIF);
                //------------------------------------------------------------
                //-- A == 0
                //-- Si B < 0    Alors Infini sur   Z+
                //-- Sinon             Infini sur   Z-
            ...
            if (TheCurve[c1].IsFirstOpen() == Standard_False) {
                if (NotLastOpenC2) {
                    if (Abs(DInfC1 - DSupC2) <= aEps &&
                        (TheCurve[c1].Value(DInfC1).Distance(TheCurve[c2].Value(DSupC2)) < aEPSILON_DISTANCE)) {
                ...
                if (NotFirstOpenC2) {
                    if (Abs(DInfC1 - DInfC2) <= aEps &&
                        (TheCurve[c1].Value(DInfC1).Distance(TheCurve[c2].Value(DInfC2)) < aEPSILON_DISTANCE)) {
                        previouscurve[c1] = -(c2 + 1);
            ...
            if (TheCurve[c1].IsLastOpen() == Standard_False) {
                if (NotLastOpenC2) {
                    if (Abs(DSupC1 - DSupC2) <= aEps &&
                        (TheCurve[c1].Value(DSupC1).Distance(TheCurve[c2].Value(DSupC2)) < aEPSILON_DISTANCE)) {
                ...
                if (NotFirstOpenC2) {
                    if (Abs(DSupC1 - DInfC2) <= aEps &&
                        (TheCurve[c1].Value(DSupC1).Distance(TheCurve[c2].Value(DInfC2)) < aEPSILON_DISTANCE)) {
                        nextcurve[c1] = c2 + 1;
                        previouscurve[c2] = c1 + 1;
                    }
                }
...
// purpose  :
//=======================================================================
Standard_Integer IntAna_IntQuadQuad::PreviousCurve(const Standard_Integer I, Standard_Boolean& theOpposite) const {
...
// purpose  :
//=======================================================================
Standard_Integer IntAna_IntQuadQuad::NextCurve(const Standard_Integer I, Standard_Boolean& theOpposite) const {
    if (HasNextCurve(I)) {
        if (nextcurve[I] > 0) {
            theOpposite = Standard_False;
...
void IntAna_IntQuadQuad::Parameters(const Standard_Integer, // i,
                                    Standard_Real&, Standard_Real&) const {
