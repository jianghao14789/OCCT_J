...
V3d_CircularGrid::V3d_CircularGrid(const V3d_ViewerPointer& aViewer, const Quantity_Color& aColor,
                                   const Quantity_Color& aTenthColor)
    : Aspect_CircularGrid(1., 8), myViewer(aViewer), myCurAreDefined(Standard_False), myToComputePrs(Standard_False),
      myCurDrawMode(Aspect_GDM_Lines), myCurXo(0.0), myCurYo(0.0), myCurAngle(0.0), myCurStep(0.0), myCurDivi(0),
      myRadius(0.5 * aViewer->DefaultViewSize()), myOffSet(THE_DEFAULT_GRID_STEP / MYFACTOR) {
    myColor = aColor;
...
void V3d_CircularGrid::DefineLines() {
    ...
    const Standard_Boolean toUpdate =
        !myCurAreDefined || myCurDrawMode != Aspect_GDM_Lines || aDivision != myCurDivi || aStep != myCurStep;
...
void V3d_CircularGrid::DefinePoints() {
    ...
    const Standard_Real aDivision = DivisionNumber();
    const Standard_Boolean toUpdate =
        !myCurAreDefined || myCurDrawMode != Aspect_GDM_Points || aDivision != myCurDivi || aStep != myCurStep;
    if (!toUpdate && !myToComputePrs) {
        return;
    } else if (!myStructure->IsDisplayed()) {
        myToComputePrs = Standard_True;
