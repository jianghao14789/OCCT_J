...
Handle(Standard_Transient)
    STEPSelections_AssemblyExplorer::FindItemWithNAUO(const Handle(StepRepr_NextAssemblyUsageOccurrence) & nauo) const {
    ...
    for (subs.Start(); subs.More() && shdefrep.IsNull(); subs.Next()) {
        itmp = subs.Value();
        if (itmp->IsKind(STANDARD_TYPE(StepShape_ContextDependentShapeRepresentation))) return itmp;
    ...
    for (Standard_Integer i = 1; i <= srep->NbItems(); i++) {
        ...
        if (repitem->IsKind(STANDARD_TYPE(StepRepr_MappedItem))) return repitem;
    }
    return item;
}

Handle(StepShape_ShapeDefinitionRepresentation)
    STEPSelections_AssemblyExplorer::FindSDRWithProduct(const Handle(StepBasic_ProductDefinition) & product) const {
...
static void PrintSubAssembly(Standard_OStream& os, const Handle(STEPSelections_AssemblyComponent) & cmp,
                             const Handle(Interface_InterfaceModel) & Model, Standard_Integer level) {
