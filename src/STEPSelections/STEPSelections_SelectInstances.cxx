...
static void AddAllSharings(const Handle(Standard_Transient) & start, const Interface_Graph& graph,
...
static void AddInstances(const Handle(Standard_Transient) & start, const Interface_Graph& graph,
                         Interface_EntityIterator& explored) {
    if (start.IsNull()) return;
    ...
    if (start->IsKind(STANDARD_TYPE(StepShape_FacetedBrep)) || start->IsKind(STANDARD_TYPE(StepShape_BrepWithVoids)) ||
        ...
        start->IsKind(STANDARD_TYPE(StepShape_GeometricSet)) || start->IsKind(STANDARD_TYPE(StepShape_FaceSurface)) ||
...
Interface_EntityIterator STEPSelections_SelectInstances::RootResult(const Interface_Graph& G) const {
    ...
    if (HasInput() || HasAlternate()) {
        ...
        for (myEntities.Start(); myEntities.More(); myEntities.Next())
            if (filter.Contains(myEntities.Value())) result.AddItem(myEntities.Value());
        return result;
    } else
        return myEntities;
}

Standard_Boolean STEPSelections_SelectInstances::Explore(const Standard_Integer, const Handle(Standard_Transient) &,
                                                         const Interface_Graph&, Interface_EntityIterator&) const {
