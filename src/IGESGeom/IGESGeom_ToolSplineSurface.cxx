...
void IGESGeom_ToolSplineSurface::ReadOwnParams(const Handle(IGESGeom_SplineSurface) & ent,
    ...
    if (!allUBreakPoints.IsNull()) {
        ...
        PR.ReadReals(PR.CurrentList(allNbUSegments + 1), Msg143,
                     allUBreakPoints); // szv#4:S4163:12Mar99 `st=` not needed
    ...
    if (!allVBreakPoints.IsNull()) {
        ...
        PR.ReadReals(PR.CurrentList(allNbVSegments + 1), Msg144,
                     allVBreakPoints); // szv#4:S4163:12Mar99 `st=` not needed
    ...
    ent->Init(aBoundaryType, aPatchType, allUBreakPoints, allVBreakPoints, allXCoeffs, allYCoeffs, allZCoeffs);
}
...
IGESData_DirChecker IGESGeom_ToolSplineSurface::DirChecker(const Handle(IGESGeom_SplineSurface) & /* ent */) const {
...
void IGESGeom_ToolSplineSurface::OwnCheck(const Handle(IGESGeom_SplineSurface) & ent, const Interface_ShareTool&,
