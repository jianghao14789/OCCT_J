...
void IGESGeom_ToolBoundary::ReadOwnParams(const Handle(IGESGeom_Boundary) & ent,
            ...
            */
            if (PR.ReadInteger(PR.Current(), tempCount) && (tempCount >= 0)) {
                ...
                if (tempCount > 0) {
                    ...
                    PR.ReadEnts(IR, PR.CurrentList(tempCount), Msg130,
                                tempParCurves); // szv#4:S4163:12Mar99 `st=` not needed
                }
                tempParameterCurves->SetValue(i, tempParCurves);
            } else {
    ...
    ent->Init(tempType, tempPreference, tempSurface, tempModelCurves, tempSenses, tempParameterCurves);
}
...
void IGESGeom_ToolBoundary::WriteOwnParams(const Handle(IGESGeom_Boundary) & ent, IGESData_IGESWriter& IW) const {
...
void IGESGeom_ToolBoundary::OwnShared(const Handle(IGESGeom_Boundary) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer i, j, num1;
...
void IGESGeom_ToolBoundary::OwnCopy(const Handle(IGESGeom_Boundary) & another, const Handle(IGESGeom_Boundary) & ent,
...
Standard_Boolean IGESGeom_ToolBoundary::OwnCorrect(const Handle(IGESGeom_Boundary) & ent) const {
    ...
    Handle(IGESBasic_HArray1OfHArray1OfIGESEntity) cv2d = new IGESBasic_HArray1OfHArray1OfIGESEntity(1, nb);
...
void IGESGeom_ToolBoundary::OwnCheck(const Handle(IGESGeom_Boundary) & ent, const Interface_ShareTool&,
...
void IGESGeom_ToolBoundary::OwnDump(const Handle(IGESGeom_Boundary) & ent, const IGESData_IGESDumper& dumper,
