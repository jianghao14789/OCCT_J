...
void IGESGeom_ToolBSplineSurface::ReadOwnParams(const Handle(IGESGeom_BSplineSurface) & ent,
    ...
    if (FlagindexU && FlagindexV) {
        ...
        for (J = 0; J <= anIndexV; J++)
            for (I = 0; I <= anIndexU; I++) {
                ...
                if (PR.ReadXYZ(PR.CurrentList(1, 3), Msg105, tempXYZ)) allPoles->SetValue(I, J, tempXYZ);
            }
    }
   ...
   */
    ...
    */
    ...
    ent->Init(anIndexU, anIndexV, aDegU, aDegV, aCloseU, aCloseV, aPolynom, aPeriodU, aPeriodV, allKnotsU, allKnotsV,
...
IGESData_DirChecker IGESGeom_ToolBSplineSurface::DirChecker(const Handle(IGESGeom_BSplineSurface) & /* ent */) const {
...
void IGESGeom_ToolBSplineSurface::OwnCheck(const Handle(IGESGeom_BSplineSurface) & ent, const Interface_ShareTool&,
    ...
    /*
      if (ent->UMin() == ent->KnotU(-ent->DegreeU()        ) &&
        ...
        Standard_Real udif = Max (ent->Pole(0, 0).SquareDistance (ent->Pole(ent->UpperIndexU(), 0)),
                                  ent->Pole(0, ent->UpperIndexV()).SquareDistance (ent->Pole(ent->UpperIndexU(),
    ent->UpperIndexV())));
    ...
    //       ach.AddWarning("U(0) != U(1) & PROP1 != 0");
      ...
      if (ent->VMin() == ent->KnotV(-ent->DegreeV()        ) &&
        ...
        Standard_Real vdif = Max (ent->Pole(0, 0).SquareDistance (ent->Pole(0, ent->UpperIndexV())),
                                  ent->Pole(ent->UpperIndexU(), 0).SquareDistance (ent->Pole(ent->UpperIndexU(),
    ent->UpperIndexV())));
...
void IGESGeom_ToolBSplineSurface::OwnDump(const Handle(IGESGeom_BSplineSurface) & ent,
    ...
    S << "In U : Upper Knot Index : " << indU << "   Degree : " << degU << "  "
      << (ent->IsClosedU() ? "Closed" : "Open") << "  " << (ent->IsPeriodicU() ? "Periodic" : "Non Periodic") << "\n"
      << "In V : Upper Knot Index : " << indV << "   Degree : " << degV << "  "
      << (ent->IsClosedV() ? "Closed" : "Open") << "  " << (ent->IsPeriodicV() ? "Periodic" : "Non Periodic") << "  "
