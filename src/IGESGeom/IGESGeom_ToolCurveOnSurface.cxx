...
void IGESGeom_ToolCurveOnSurface::ReadOwnParams(const Handle(IGESGeom_CurveOnSurface) & ent,
    ...
    if (!PR.ReadEntity(IR, PR.Current(), aStatus, aCurve3D, Standard_True)) {
        ; // szv#4:S4163:12Mar99 `st=` not needed
...
void IGESGeom_ToolCurveOnSurface::OwnCopy(const Handle(IGESGeom_CurveOnSurface) & another,
                                          const Handle(IGESGeom_CurveOnSurface) & ent, Interface_CopyTool& TC) const {
...
void IGESGeom_ToolCurveOnSurface::OwnCheck(const Handle(IGESGeom_CurveOnSurface) & ent, const Interface_ShareTool&,
