...
void IGESGeom_ToolConicArc::ReadOwnParams(const Handle(IGESGeom_ConicArc) & ent,
     ...
     */
    if ((!PR.ReadReal(PR.Current(), A)) || (!PR.ReadReal(PR.Current(), B)) || (!PR.ReadReal(PR.Current(), C)) ||
        (!PR.ReadReal(PR.Current(), D)) || (!PR.ReadReal(PR.Current(), E)) || (!PR.ReadReal(PR.Current(), F))) {
...
void IGESGeom_ToolConicArc::WriteOwnParams(const Handle(IGESGeom_ConicArc) & ent, IGESData_IGESWriter& IW) const {
...
void IGESGeom_ToolConicArc::OwnCopy(const Handle(IGESGeom_ConicArc) & another, const Handle(IGESGeom_ConicArc) & ent,
                                    Interface_CopyTool& /* TC */) const {
    Standard_Real A, B, C, D, E, F;
    another->Equation(A, B, C, D, E, F);
    ent->Init(A, B, C, D, E, F, another->ZPlane(), another->StartPoint().XY(), another->EndPoint().XY());
}
...
IGESData_DirChecker IGESGeom_ToolConicArc::DirChecker(const Handle(IGESGeom_ConicArc) & /* ent */) const {
...
void IGESGeom_ToolConicArc::OwnCheck(const Handle(IGESGeom_ConicArc) & ent, const Interface_ShareTool&,
...
void IGESGeom_ToolConicArc::OwnDump(const Handle(IGESGeom_ConicArc) & ent, const IGESData_IGESDumper& /* dumper */,
