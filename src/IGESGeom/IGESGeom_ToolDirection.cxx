...
void IGESGeom_ToolDirection::ReadOwnParams(const Handle(IGESGeom_Direction) & ent,
    ...
    if (PR.DefinedElseSkip()) {
        ...
        if (PR.ReadReal(PR.Current(), "Direction", tmpReal)) aDirection.SetZ(tmpReal);
    } else {
...
void IGESGeom_ToolDirection::WriteOwnParams(const Handle(IGESGeom_Direction) & ent, IGESData_IGESWriter& IW) const {
    IW.Send(ent->Value().X());
    IW.Send(ent->Value().Y());
    IW.Send(ent->Value().Z());
}
...
void IGESGeom_ToolDirection::OwnCopy(const Handle(IGESGeom_Direction) & another, const Handle(IGESGeom_Direction) & ent,
...
void IGESGeom_ToolDirection::OwnCheck(const Handle(IGESGeom_Direction) & ent, const Interface_ShareTool&,
                                      Handle(Interface_Check) & ach) const {
    if (ent->Value().XYZ().SquareModulus() <= 0.0) ach->AddFail("Direction : The values indicate no direction");
}

void IGESGeom_ToolDirection::OwnDump(const Handle(IGESGeom_Direction) & ent, const IGESData_IGESDumper& /* dumper */,
