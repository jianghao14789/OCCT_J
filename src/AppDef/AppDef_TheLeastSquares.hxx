...
public:
    ...
    Standard_EXPORT AppDef_TheLeastSquares(const AppDef_MultiLine& SSP, const Standard_Integer FirstPoint,
                                           const Standard_Integer LastPoint, const AppParCurves_Constraint FirstCons,
                                           const AppParCurves_Constraint LastCons, const math_Vector& Parameters,
    ...
    //! they are approximated in parallel(so with the same
    //! parameter, only the vector B changes).
    Standard_EXPORT AppDef_TheLeastSquares(const AppDef_MultiLine& SSP, const TColStd_Array1OfReal& Knots,
