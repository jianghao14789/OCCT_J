...
//=======================================================================
//
AppDef_Variational::AppDef_Variational(const AppDef_MultiLine& SSP, const Standard_Integer FirstPoint,
                                       const Standard_Integer LastPoint,
                                       const Handle(AppParCurves_HArray1OfConstraintCouple) & TheConstraints,
                                       const Standard_Integer MaxDegree, const Standard_Integer MaxSegment,
                                       const GeomAbs_Shape Continuity, const Standard_Boolean WithMinMax,
                                       const Standard_Boolean WithCutting, const Standard_Real Tolerance,
                                       const Standard_Integer NbIterations)
    : mySSP(SSP), myFirstPoint(FirstPoint), myLastPoint(LastPoint), myConstraints(TheConstraints),
      myMaxDegree(MaxDegree), myMaxSegment(MaxSegment), myNbIterations(NbIterations), myTolerance(Tolerance),
      myContinuity(Continuity), myWithMinMax(WithMinMax), myWithCutting(WithCutting) {
    // Verifications:
...

{
    ...
    if (myWithMinMax && myTolerance < myMaxError) Adjusting(mySmoothCriterion, WQuadratic, WQuality, TheCurve, Ecarts);

    //---------------------------------------------------------------------

    Standard_Integer jp2d, jp3d, index, ipole, NbElem = TheCurve->NbElements();
    ...
    {
        Handle(TColStd_HArray2OfReal) PolynomialIntervalsPtr = new TColStd_HArray2OfReal(1, NbElem, 1, 2);
        ...
        */
        Convert_CompPolynomialToPoles AConverter(NbElem, myNivCont, myDimension, myMaxDegree, NbCoeffPtr, CoeffPtr,
...
//=======================================================================
//
void AppDef_Variational::Criterium(Standard_Real& VFirstOrder, Standard_Real& VSecondOrder,
...
//=======================================================================
//
void AppDef_Variational::CriteriumWeight(Standard_Real& Percent1, Standard_Real& Percent2,
...
void AppDef_Variational::Dump(Standard_OStream& o) const {
    ...
    if (myIsDone) {
        ...
        o << (myIsOverConstr ? " The problem is overconstraint" : " Error in approximation") << std::endl;
    }
}
...
Standard_Boolean AppDef_Variational::SetWithCutting(const Standard_Boolean Cutting) {
    if (Cutting == Standard_False) {
        if (((myMaxDegree - myNivCont) * myKnots->Length() - myNbPassPoints - 2 * myNbTangPoints - 3 * myNbCurvPoints) <
            0)
            return Standard_False;
...
//=======================================================================
//
void AppDef_Variational::SetCriteriumWeight(const Standard_Real Percent1, const Standard_Real Percent2,
...
void AppDef_Variational::TheMotor(Handle(AppDef_SmoothCriterion) & J,
                                  ...
                                  const Standard_Real, const Standard_Real WQuality, Handle(FEmTool_Curve) & TheCurve,
    ...
    Standard_Real VOCRI[3] = {BigValue, BigValue, BigValue}, EROLD = BigValue, VALCRI[3], ERRMAX = BigValue, ERRMOY,
                  ERRQUA;
    ...
    while (again) {
        ...
        while (lestim) {
            ...
            CNew = new (FEmTool_Curve)(CCurrent->Dimension(), CCurrent->NbElements(), CCurrent->Base(), EpsLength);
            ...
            if (isnear) {
                ...
                Project(CNew, CurrentTi->Array1(), NewTi->ChangeArray1(), Ecarts, NumPnt, ERRMAX, ERRQUA, ERRMOY, 2);
            } else
                NewTi = CurrentTi;
            ...
            if ((EROLD > WQuality) && (ERRMAX < 0.99 * EROLD) && (ERRMAX < 1.1 * WQuality)) iprog++;
        ...
        while (ToOptim) {
            ...
            NewTi = new (TColStd_HArray1OfReal)(1, CurrentTi->Length());
            Project(CCurrent, CurrentTi->Array1(), NewTi->ChangeArray1(), Ecarts, NumPnt, ERRMAX, ERRQUA, ERRMOY, 2);
...
L8000:
    ...
    NewTi = new (TColStd_HArray1OfReal)(1, CurrentTi->Length());
    Project(CCurrent, CurrentTi->Array1(), NewTi->ChangeArray1(), Ecarts, NumPnt, ERRMAX, ERRQUA, ERRMOY, 10);
...
//=======================================================================
void AppDef_Variational::Optimization(Handle(AppDef_SmoothCriterion) & J, FEmTool_Assembly& A,
                                      const Standard_Boolean ToAssemble, const Standard_Real EpsDeg,
                                      Handle(FEmTool_Curve) & Curve, const TColStd_Array1OfReal& Parameters) const {
    Standard_Integer MxDeg = Curve->Base()->WorkDegree(), NbElm = Curve->NbElements(), NbDim = Curve->Dimension();
    ...
    if (NbConstr == 0) {
        ...
        for (el = 1; el <= NbElm; el++) {
            while ((Icnt < NbConstr) && (Parameters(p0 + myTypConstraints->Value(2 * Icnt - 1)) <= TabInt(el)))
                Icnt++;
...
void AppDef_Variational::Project(const Handle(FEmTool_Curve) & C, const TColStd_Array1OfReal& Ti,
                                 TColStd_Array1OfReal& ProjTi, TColStd_Array1OfReal& Distance,
                                 Standard_Integer& NumPoints, Standard_Real& MaxErr, Standard_Real& QuaErr,
                                 Standard_Real& AveErr, const Standard_Integer NbIterations) const

{
    ...
    TColStd_Array1OfReal ValOfC(1, myDimension), FirstDerOfC(1, myDimension), SecndDerOfC(1, myDimension);
...
void AppDef_Variational::ACR(Handle(FEmTool_Curve) & Curve, TColStd_Array1OfReal& Ti,
                             const Standard_Integer Decima) const {

    const Standard_Real Eps = 1.e-8;

    TColStd_Array1OfReal& Knots = Curve->Knots();
    Standard_Integer NbrPnt = Ti.Length(), TiFirst = Ti.Lower(), TiLast = Ti.Upper(), KFirst = Knots.Lower(),
                     KLast = Knots.Upper();
...
static Standard_Integer NearIndex(const Standard_Real T, const TColStd_Array1OfReal& TabPar, const Standard_Real Eps,
...
static void GettingKnots(const TColStd_Array1OfReal& TabPar, const Handle(FEmTool_Curve) & InCurve,
                         const Standard_Integer Deg, Standard_Integer& NbElm, TColStd_Array1OfReal& NewKnots)
...
void AppDef_Variational::SplitCurve(const Handle(FEmTool_Curve) & InCurve, const TColStd_Array1OfReal& Ti,
                                    const Standard_Real CurveTol, Handle(FEmTool_Curve) & OutCurve,
...
void AppDef_Variational::InitParameters(Standard_Real& Length) {
    ...
    if (Length <= Eps1) throw Standard_ConstructionError("AppDef_Variational::InitParameters");
...
//=======================================================================
//
void AppDef_Variational::InitCriterionEstimations(const Standard_Real Length, Standard_Real& E1, Standard_Real& E2,
                                                  Standard_Real& E3) const {
    E1 = Length * Length;

    const Standard_Real Eps1 = Precision::Confusion() * .01;

    math_Vector VTang1(1, myDimension), VTang2(1, myDimension), VTang3(1, myDimension), VScnd1(1, myDimension),
        VScnd2(1, myDimension), VScnd3(1, myDimension);
...

{
    ...
    if (ipnt == myFirstPoint) {
        ...
        Standard_Integer adr1 = (ipnt - myFirstPoint - 1) * myDimension + 1, adr2 = adr1 + 2 * myDimension;
    ...
    if (NbConstr > 0) {
        ...
        while (myTypConstraints->Value(2 * IdCnt - 1) < ipnt && IdCnt <= NbConstr)
            IdCnt++;
        if ((myTypConstraints->Value(2 * IdCnt - 1) == ipnt) && (myTypConstraints->Value(2 * IdCnt) >= 1)) {
            Wcnt = 1.;
...
//=======================================================================
//
void AppDef_Variational::EstSecnd(const Standard_Integer ipnt, const math_Vector& VTang1, const math_Vector& VTang2,
                                  const Standard_Real Length, math_Vector& VScnd) const {
    Standard_Integer i;
    ...
    if (NbConstr > 0) {
        ...
        if ((myTypConstraints->Value(2 * IdCnt - 1) == ipnt) && (myTypConstraints->Value(2 * IdCnt) >= 2)) {
            Wcnt = 1.;
...
//=======================================================================
//
void AppDef_Variational::InitCutting(const Handle(PLib_Base) & aBase, const Standard_Real CurvTol,
    ...
    if (ORCMx > myMaxDegree - myNivCont) throw Standard_ConstructionError("AppDef_Variational::InitCutting");
    ...
    if (NbElem > myMaxSegment) throw Standard_ConstructionError("AppDef_Variational::InitCutting");
    ...
    Standard_Integer IDeb = 0, IFin = NbConstr + 1, NDeb = 0, NFin = 0, IndEl = Knot.Lower(), IUpper = Knot.Upper(),
                     NbEl = 0;
    ...
    while (NbElem - NbEl > 1) {
        ...
        if (NDeb == NCnt) {
            NDeb = 0;
            if (NPlus == 1 && myParameters->Value(myTypConstraints->Value(2 * IDeb - 1)) > Knot(IndEl - 1))
...
//=======================================================================
void AppDef_Variational::Adjusting(Handle(AppDef_SmoothCriterion) & J, Standard_Real& WQuadratic,
                                   Standard_Real& WQuality, Handle(FEmTool_Curve) & TheCurve,
                                   TColStd_Array1OfReal& Ecarts) {
    ...
    while (loptim) {
        ...
        if (TheCurve->NbElements() < myMaxSegment && myWithCutting) {
            ...
            tpara =
                (TheCurve->Knots()(numint) + TheCurve->Knots()(numint + 1) + myParameters->Value(myMaxErrorIndex) * 2) /
                4;
...
static Standard_Boolean NotParallel(gp_Vec& T, gp_Vec& V) {
    V = T;
    V.SetX(V.X() + 1.);
    if (V.CrossMagnitude(T) > 1.e-12) return Standard_True;
    V.SetY(V.Y() + 1.);
    if (V.CrossMagnitude(T) > 1.e-12) return Standard_True;
    V.SetZ(V.Z() + 1.);
    if (V.CrossMagnitude(T) > 1.e-12) return Standard_True;
...
void AppDef_Variational::AssemblingConstraints(const Handle(FEmTool_Curve) & Curve,
                                               const TColStd_Array1OfReal& Parameters, const Standard_Real CBLONG,
    ...
    Standard_Integer IndexOfConstraint, Ng3d, Ng2d, NBeg2d, NPass, NgPC1, NTang3d, NTang2d, Point, TypOfConstr,
...
Standard_Boolean AppDef_Variational::InitTthetaF(const Standard_Integer ndimen, const AppParCurves_Constraint typcon,
                                                 const Standard_Integer begin, const Standard_Integer jndex) {
    if ((ndimen < 2) || (ndimen > 3)) return Standard_False;
    gp_Vec T, V;
    ...
    if ((typcon == AppParCurves_TangencyPoint) || (typcon == AppParCurves_CurvaturePoint)) {
        ...
        if (ndimen == 3)
            if (!NotParallel(T, V)) return Standard_False;
        theta1 = V ^ T;
        theta1.Normalize();
