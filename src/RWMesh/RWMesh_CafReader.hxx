...
//!    Named elements are expanded within document structure, while Compounds having no named children will remain
//!    collapsed. In addition, unnamed nodes can be filled with generated names like "Face", "Compound" via
//!    generateNames() method, and the very root unnamed node can be filled from file name like "MyModel.obj".
...
public:
    ...
    void SetRootPrefix(const TCollection_AsciiString& theRootPrefix) {
        myRootPrefix = theRootPrefix;
    }

    //! Flag indicating if partially read file content should be put into the XDE document, TRUE by default.
    //!
    //! Partial read means unexpected end of file, critical parsing syntax errors in the middle of file, or reached
    //! memory limit indicated by performMesh() returning FALSE. Partial read allows importing a model even in case of
    //! formal reading failure, so that it will be up to user to decide if processed data has any value.
    //!
...
public:
    ...
    Standard_Real SystemLengthUnit() const {
        return myCoordSysConverter.OutputLengthUnit();
    }
    ...
    Standard_Real FileLengthUnit() const {
        return myCoordSysConverter.InputLengthUnit();
    }
    ...
    const gp_Ax3& FileCoordinateSystem() const {
        return myCoordSysConverter.InputCoordinateSystem();
    }
...
public:
    ...
    //! The main purpose is collecting metadata and external references - for copying model into a new location, for
    //! example. Can be NOT implemented (unsupported by format / reader).
...
protected:
    ...
    Standard_EXPORT Standard_Boolean addShapeIntoDoc(CafDocumentTools& theTools, const TopoDS_Shape& theShape,
                                                     const TDF_Label& theLabel,
                                                     const TCollection_AsciiString& theParentName);

    //! Append new sub-shape into the document (recursively).
    Standard_EXPORT Standard_Boolean addSubShapeIntoDoc(CafDocumentTools& theTools, const TopoDS_Shape& theShape,
                                                        const TDF_Label& theParentLabel);

    //! Put name attribute onto the label.
    Standard_EXPORT void setShapeName(const TDF_Label& theLabel, const TopAbs_ShapeEnum theShapeType,
                                      const TCollection_AsciiString& theName, const TDF_Label& theParentLabel,
                                      const TCollection_AsciiString& theParentName);

    //! Put color and material attributes onto the label.
    Standard_EXPORT void setShapeStyle(const CafDocumentTools& theTools, const TDF_Label& theLabel,
                                       const XCAFPrs_Style& theStyle);

    //! Put name data (metadata) attribute onto the label.
    Standard_EXPORT void setShapeNamedData(const CafDocumentTools& theTools, const TDF_Label& theLabel,
                                           const Handle(TDataStd_NamedData) & theNameData);

    //! Generate names for root labels starting from specified index.
    Standard_EXPORT void generateNames(const TCollection_AsciiString& theFile, const Standard_Integer theRootLower,
...
protected:
    ...
    Standard_Boolean
        myToFillIncomplete; //!< fill the document with partially retrieved data even if reader has failed with error
