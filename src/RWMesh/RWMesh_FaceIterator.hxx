...
public:
    //! Main constructor.
    Standard_EXPORT RWMesh_FaceIterator(const TDF_Label& theLabel, const TopLoc_Location& theLocation,
                                        const Standard_Boolean theToMapColors = false,
                                        const XCAFPrs_Style& theStyle = XCAFPrs_Style());

    //! Auxiliary constructor.
    Standard_EXPORT RWMesh_FaceIterator(const TopoDS_Shape& theShape, const XCAFPrs_Style& theStyle = XCAFPrs_Style());

    //! Return explored shape.
    const TopoDS_Shape& ExploredShape() const {
        return myFaceIter.ExploredShape();
    }
    ...
    bool IsEmptyMesh() const {
        return myPolyTriang.IsNull() || (myPolyTriang->NbNodes() < 1 && myPolyTriang->NbTriangles() < 1);
    }
...
public:
    ...
    Standard_Integer NbTriangles() const {
        return myPolyTriang->NbTriangles();
    }
...
public:
    ...
    bool HasTexCoords() const {
        return !myPolyTriang.IsNull() && myPolyTriang->HasUVNodes();
    }
...
private:
    NCollection_DataMap<TopoDS_Shape, XCAFPrs_Style, TopTools_ShapeMapHasher> myStyles; //!< Face -> Style map
