...
public:
    struct LoadingStatistic {
        LoadingStatistic()
            : ExpectedNodesNb(0), LoadedNodesNb(0), ExpectedTrianglesNb(0), DegeneratedTrianglesNb(0),
              LoadedTrianglesNb(0) {}
...
protected: //! @name interface for filling triangulation data
    ...
    virtual bool setNbPositionNodes(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theNbNodes,
    ...
    virtual void setNodePosition(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theIndex,
    ...
    virtual void setNodeUV(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theIndex,
    ...
    virtual void setNodeNormal(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theIndex,
    ...
    //! @return TRUE in case of success operation
    virtual bool setNbTriangles(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theNbTris,
                                Standard_Boolean theToCopyData = false) const {
        if (theNbTris >= 1) {
    ...
    virtual Standard_Integer setTriangle(const Handle(Poly_Triangulation) & theMesh, Standard_Integer theIndex,
                                         const Poly_Triangle& theTriangle) const {
        if (theTriangle.Value(1) < 1 || theTriangle.Value(1) > theMesh->NbNodes() || theTriangle.Value(2) < 1 ||
            theTriangle.Value(2) > theMesh->NbNodes() || theTriangle.Value(3) < 1 ||
            theTriangle.Value(3) > theMesh->NbNodes()) {
            return 0;
        }
        if (myToSkipDegenerateTris &&
            (theTriangle.Value(1) == theTriangle.Value(2) || theTriangle.Value(1) == theTriangle.Value(3) ||
