...
loadingStatistic(const TCollection_AsciiString& thePrefix, const Standard_Integer theExpectedNodesNb,
                 const Standard_Integer theLoadedNodesNb, const Standard_Integer theExpectedTrianglesNb,
                 const Standard_Integer theDegeneratedTrianglesNb, const Standard_Integer theLoadedTrianglesNb) {
    ...
    return thePrefix +
           ("Disconformity of the expected number of nodes/triangles for deferred mesh to the loaded amount. ") +
...
void RWMesh_TriangulationReader::LoadingStatistic::PrintStatistic(const TCollection_AsciiString& thePrefix) const {
    TCollection_AsciiString aStatisticInfo = loadingStatistic(
        thePrefix, ExpectedNodesNb, LoadedNodesNb, ExpectedTrianglesNb, DegeneratedTrianglesNb, LoadedTrianglesNb);
...
RWMesh_TriangulationReader::RWMesh_TriangulationReader()
    : myLoadingStatistic(NULL), myIsDoublePrecision(false), myToSkipDegenerateTris(false),
...
bool RWMesh_TriangulationReader::finalizeLoading(const Handle(RWMesh_TriangulationSource) & theSourceMesh,
    ...
    if (myLoadingStatistic) {
        ...
        TCollection_AsciiString aStatisticInfo = loadingStatistic(
            TCollection_AsciiString("[Mesh reader. File '") + myFileName + "']. ", theSourceMesh->NbDeferredNodes(),
            theDestMesh->NbNodes(), theSourceMesh->NbDeferredTriangles(), theSourceMesh->DegeneratedTriNb(),
