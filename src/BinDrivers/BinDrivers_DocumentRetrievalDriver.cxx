...
Handle(BinMDF_ADriverTable) BinDrivers_DocumentRetrievalDriver::AttributeDrivers(const Handle(Message_Messenger) &
                                                                                 theMessageDriver) {
...

{
    ...
    if (myDrivers->GetDriver(STANDARD_TYPE(TNaming_NamedShape), aDriver)) {
        try {
            ...
            myMsgDriver->Send(aMethStr + "error of Shape Section " + anException.GetMessageString(), Message_Fail);
        }
    }
}
...
// purpose  :
//=======================================================================
void BinDrivers_DocumentRetrievalDriver::CheckShapeSection(const Storage_Position& /*ShapeSectionPos*/,
                                                           Standard_IStream& /*IS*/) {}
...
void BinDrivers_DocumentRetrievalDriver::Clear() {
    ...
    Handle(BinMDF_ADriver) aDriver;
    if (myDrivers->GetDriver(STANDARD_TYPE(TNaming_NamedShape), aDriver)) {
        Handle(BinMNaming_NamedShapeDriver) aNamedShapeDriver = Handle(BinMNaming_NamedShapeDriver)::DownCast(aDriver);
...
// purpose  :
//=======================================================================
void BinDrivers_DocumentRetrievalDriver::EnableQuickPartReading(const Handle(Message_Messenger) & theMessageDriver,
                                                                Standard_Boolean theValue) {
    if (myDrivers.IsNull()) myDrivers = AttributeDrivers(theMessageDriver);
    if (myDrivers.IsNull()) return;
    ...
    if (aShapesDriver.IsNull()) throw Standard_NotImplemented("Internal Error - TNaming_NamedShape is not found!");
