...
// purpose  :
//=======================================================================
FEmTool_Curve::FEmTool_Curve(const Standard_Integer Dimension, const Standard_Integer NbElements,
                             const Handle(PLib_Base) & TheBase, const Standard_Real)
    : myNbElements(NbElements), myDimension(Dimension), myBase(TheBase), myDegree(1, myNbElements),
      ...
      myDsecn(1, myDimension * myNbElements * (myBase->WorkDegree() - 1)), HasPoly(1, myNbElements),
      HasDeri(1, myNbElements), HasSecn(1, myNbElements), myLength(1, myNbElements), myIndex(0) {
...
// purpose  :
//=======================================================================
void FEmTool_Curve::SetElement(const Standard_Integer IndexOfElement, const TColStd_Array2OfReal& Coeffs) {
    ...
    Standard_Integer iBase = (IndexOfElement - 1) * (degBase + 1) * myDimension, i1 = iBase - myDimension,
    ...
    Standard_Real stenor = (myKnots->Value(IndexOfElement + 1) - myKnots->Value(IndexOfElement)) / 2., mfact;
...
void FEmTool_Curve::GetElement(const Standard_Integer IndexOfElement, TColStd_Array2OfReal& Coeffs) {
    ...
    Standard_Real stenor = 2. / (myKnots->Value(IndexOfElement + 1) - myKnots->Value(IndexOfElement)), mfact;
...
void FEmTool_Curve::D0(const Standard_Real U, TColStd_Array1OfReal& Pnt) {
    ...
    if (!myIndex || (U < Uf) || (U > Ul) || (myKnots->Value(myIndex) != Uf) || (myKnots->Value(myIndex + 1) != Ul)) {
        // Search the span
    ...
    S = (2 * U - USum) * Denom;
    PLib::NoDerivativeEvalPolynomial(S, deg, myDimension, deg * myDimension, myPoly(myPtr), Pnt(Pnt.Lower()));
}
...
void FEmTool_Curve::D1(const Standard_Real U, TColStd_Array1OfReal& Vec) {
    ...
    PLib::NoDerivativeEvalPolynomial(S, deg - 1, myDimension, (deg - 1) * myDimension,
                                     myDeri(1 + (myIndex - 1) * myBase->WorkDegree() * myDimension), Vec(Vec.Lower()));
...
// purpose  :
//=======================================================================
void FEmTool_Curve::Length(const Standard_Real FirstU, const Standard_Real LastU, Standard_Real& Length) {
    ...
    FirstS =
        (2 * FirstU - myKnots->Value(Low) - myKnots->Value(Low + 1)) / (myKnots->Value(Low + 1) - myKnots->Value(Low));
...
// purpose  :
//=======================================================================
void FEmTool_Curve::ReduceDegree(const Standard_Integer IndexOfElement, const Standard_Real Tol,
                                 Standard_Integer& NewDegree, Standard_Real& MaxError) {
...
void FEmTool_Curve::Update(const Standard_Integer Index, const Standard_Integer Order) {
    ...
    if (Order >= 1) {
        ...
        if ((Order >= 2) && !HasSecn.Value(Index)) {
            i1 = (Index - 1) * (degBase - 1) * myDimension - myDimension, i2 = (Index - 1) * (degBase)*myDimension;
            for (i = 1; i < deg; i++) {
                i1 += myDimension;
                i2 += myDimension;
                for (j = 1; j <= myDimension; j++)
