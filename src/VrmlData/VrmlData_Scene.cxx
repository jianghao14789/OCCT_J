...
static void dumpNode(Standard_OStream& theStream, const Handle(VrmlData_Node) & theNode,
                     const TCollection_AsciiString& theIndent);

static void dumpNodeHeader(Standard_OStream& theStream, const TCollection_AsciiString& theIndent, const char* theType,
                           const char* theName);
...
VrmlData_Scene::VrmlData_Scene(const Handle(NCollection_IncAllocator) & theAlloc)
    : myLinearScale(1.), myStatus(VrmlData_StatusOK),
      myAllocator(theAlloc.IsNull() ? new NCollection_IncAllocator : theAlloc.operator->()), myLineError(0),
      myOutput(0L), myIndent(2), myCurrentIndent(0), myAutoNameCounter(0) {
...
const Handle(VrmlData_Node) & VrmlData_Scene::AddNode(const Handle(VrmlData_Node) & theN,
    ...
    if (theN.IsNull() == Standard_False)
        if (theN->IsKind(STANDARD_TYPE(VrmlData_WorldInfo)) == Standard_False) {
            ...
            if (isTopLevel) myLstNodes.Append(aNode);
            myMutex.Unlock();
            return aNode;
        }
...
Standard_OStream& operator<<(Standard_OStream& theOutput, const VrmlData_Scene& theScene) {
    ...
    for (; anIterD.More(); anIterD.Next()) {
        ...
        if (aNode.IsNull() == Standard_False) {
            ...
            if (aStatus != VrmlData_StatusOK && aStatus != VrmlData_NotImplemented) break;
        }
    }
...
VrmlData_ErrorStatus VrmlData_Scene::readHeader(VrmlData_InBuffer& theBuffer) {
    ...
    if (aStat == VrmlData_StatusOK && !VRMLDATA_LCOMPARE(theBuffer.LinePtr, "#VRML V2.0"))
...
VrmlData_Scene& VrmlData_Scene::operator<<(Standard_IStream& theInput) {
    ...
    for (;;) {
        if (!VrmlData_Node::OK(myStatus, ReadLine(aBuffer))) {
            if (myStatus == VrmlData_EndOfFile) myStatus = VrmlData_StatusOK;
            break;
        }
        ...
        if (myStatus != VrmlData_StatusOK) break;
        if (aNode.IsNull() == Standard_False /*&&
    ...
    if (myStatus != VrmlData_StatusOK) myLineError = aBuffer.LineCount;
    myMutex.Unlock();
    return *this;
}
...
Handle(VrmlData_Node) VrmlData_Scene::FindNode(const char* theName, const Handle(Standard_Type) & /*theType*/) const {
    ...
    for (; anIter.More(); anIter.Next())
        if (!strcmp(anIter.Value()->Name(), theName)) {
            ...
            if (theType.IsNull()) break;
            if (aResult->IsKind(theType)) break;
            aResult.Nullify();
        }
#else
    const Handle(VrmlData_UnknownNode) aDummyNode = new VrmlData_UnknownNode;
    aDummyNode->myName = theName;
    if (myNamedNodes.Contains(aDummyNode)) aResult = const_cast<VrmlData_MapOfNode&>(myNamedNodes).Added(aDummyNode);
#endif
    return aResult;
}
    ...
    for (; anIter.More(); anIter.Next()) {
        ...
        if (aNode->IsKind(STANDARD_TYPE(VrmlData_Group))) {
            ...
            if (aGroup.IsNull() == Standard_False) {
                ...
                if (aResult.IsNull() == Standard_False) break;
            }
        }
    }
    return aResult;
...
VrmlData_ErrorStatus VrmlData_Scene::ReadWord(VrmlData_InBuffer& theBuffer, TCollection_AsciiString& theWord) {
...
VrmlData_ErrorStatus VrmlData_Scene::createNode(VrmlData_InBuffer& theBuffer, Handle(VrmlData_Node) & theNode,
    ...
    if (VrmlData_Node::OK(aStatus, ReadLine(theBuffer))) {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "DEF")) {
            if (VrmlData_Node::OK(aStatus, ReadWord(theBuffer, aName))) aStatus = ReadLine(theBuffer);
    ...
    if (aStatus == VrmlData_StatusOK) {
        ...
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "Appearance"))
            ...
            if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "3")) theBuffer.LinePtr++;
            ...
            if (aStatus == VrmlData_StatusOK) aNode = new VrmlData_UnknownNode(*this, strName, aTitle.ToCString());
        }
    }
    aStatus = ReadLine(theBuffer);
    if (aNode.IsNull() == Standard_False) {
        if (aNode->Name()[0] != '\0') myNamedNodes.Add(aNode);
        if (theType.IsNull() == Standard_False)
            if (aNode->IsKind(theType) == Standard_False) aStatus = VrmlData_VrmlFormatError;
    }
    if (aStatus == VrmlData_StatusOK) {
        if (theBuffer.LinePtr[0] == '{') {
            theBuffer.LinePtr++;
...
void VrmlData_Scene::createShape(TopoDS_Shape& outShape, const VrmlData_ListOfNode& lstNodes,
    ...
    for (; anIter.More(); anIter.Next()) {
        ...
        const Handle(VrmlData_ShapeNode) aNodeShape = Handle(VrmlData_ShapeNode)::DownCast(anIter.Value());
        if (aNodeShape.IsNull() == Standard_False) {
            ...
            if (aNodeGeom.IsNull() == Standard_False) {
                if (aSingleShape.IsNull() == Standard_False) isSingleShape = Standard_False;
    ...
    if (isSingleShape) outShape = aSingleShape;
}
...
VrmlData_ErrorStatus VrmlData_Scene::ReadReal(VrmlData_InBuffer& theBuffer, Standard_Real& theResult,
                                              Standard_Boolean isScale, Standard_Boolean isOnlyPositive) const {
...
VrmlData_ErrorStatus VrmlData_Scene::ReadXYZ(VrmlData_InBuffer& theBuffer, gp_XYZ& theXYZ, Standard_Boolean isScale,
                                             Standard_Boolean isOnlyPos) const {
    Standard_Real aVal[3] = {0., 0., 0.};
    VrmlData_ErrorStatus aStatus = VrmlData_StatusOK;
    for (Standard_Integer i = 0; i < 3; i++) {
        if (!VrmlData_Node::OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) break;
        char* endptr;
...
VrmlData_ErrorStatus VrmlData_Scene::ReadXY(VrmlData_InBuffer& theBuffer, gp_XY& theXY, Standard_Boolean isScale,
                                            Standard_Boolean isOnlyPos) const {
...
VrmlData_ErrorStatus VrmlData_Scene::ReadArrIndex(VrmlData_InBuffer& theBuffer, const Standard_Integer**& theArray,
    ...
    if (VrmlData_Node::OK(aStatus, ReadLine(theBuffer))) {
        if (theBuffer.LinePtr[0] != '[') // opening bracket
            ...
            while (isMore && VrmlData_Node::OK(aStatus, ReadLine(theBuffer))) {
                ...
                if (!VrmlData_Node::OK(aStatus, VrmlData_Node::ReadInteger(theBuffer, anIntValue))) break;
                // Check for valid delimiter (']' or ',')
                if (!VrmlData_Node::OK(aStatus, ReadLine(theBuffer))) break;
                if (theBuffer.LinePtr[0] == ']') {
                    theBuffer.LinePtr++;
                ...
                if ((anIntValue < 0 || isMore == Standard_False) && vecInt.Length() > 0) {
                    ...
                    Standard_Integer* bufFace =
                        static_cast<Standard_Integer*>(myAllocator->Allocate((aLen + 1) * sizeof(Standard_Integer)));
                    if (bufFace == 0L) {
                        aStatus = VrmlData_UnrecoverableError;
                        break;
                    }
...
VrmlData_ErrorStatus VrmlData_Scene::WriteArrIndex(const char* thePrefix, const Standard_Integer** theArrIndex,
    ...
    if (theNbBlocks && (IsDummyWrite() == Standard_False)) {
        if (VrmlData_Node::OK(aStatus, WriteLine(thePrefix, "[", 1))) {
            ...
            if (aStatus == VrmlData_StatusOK) aStatus = WriteLine("]", 0L, -1);
        }
    }
    return aStatus;
}
...
VrmlData_ErrorStatus VrmlData_Scene::WriteXYZ(const gp_XYZ& theXYZ, const Standard_Boolean isApplyScale,
                                              const char* thePostfix) const {
    char buf[240];
    if (IsDummyWrite() == Standard_False) {
        if (isApplyScale && myLinearScale > Precision::Confusion())
            Sprintf(buf, "%.12g %.12g %.12g%s", theXYZ.X() / myLinearScale, theXYZ.Y() / myLinearScale,
                    theXYZ.Z() / myLinearScale, thePostfix ? thePostfix : "");
        else
            Sprintf(buf, "%.12g %.12g %.12g%s", theXYZ.X(), theXYZ.Y(), theXYZ.Z(), thePostfix ? thePostfix : "");
    }
...
VrmlData_ErrorStatus VrmlData_Scene::WriteLine(const char* theLin0, const char* theLin1,
    ...
    if (IsDummyWrite())
        ...
        if (theIndent < 0) aCurrentIndent -= myIndent;
        if (aCurrentIndent < 0) aCurrentIndent = 0;
...
VrmlData_ErrorStatus VrmlData_Scene::WriteNode(const char* thePrefix, const Handle(VrmlData_Node) & theNode) const {
    VrmlData_ErrorStatus aStatus(VrmlData_StatusOK);
...
void dumpNode(Standard_OStream& theStream, const Handle(VrmlData_Node) & theNode,
              const TCollection_AsciiString& theIndent) {
    ...
    if (theNode->IsKind(STANDARD_TYPE(VrmlData_Appearance))) {
        ...
        const Handle(VrmlData_IndexedFaceSet) aNode = Handle(VrmlData_IndexedFaceSet)::DownCast(theNode);
        ...
        const Handle(VrmlData_IndexedLineSet) aNode = Handle(VrmlData_IndexedLineSet)::DownCast(theNode);
        const Standard_Integer** ppDummy;
        const Standard_Size nCoord = aNode->Coordinates()->Length();
        const Standard_Size nPoly = aNode->Polygons(ppDummy);
...
void dumpNodeHeader(Standard_OStream& theStream, const TCollection_AsciiString& theIndent, const char* theType,
                    const char* theName) {
