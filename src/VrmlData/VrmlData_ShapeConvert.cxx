        ...
        case TopAbs_FACE: {
            ...
            if (aFace.IsNull() == Standard_False) {
                ...
                if (aTri.IsNull() == Standard_False) {
                    ...
                    if (aFaceSetToReuse.IsNull() == Standard_False) aCoordToReuse = aFaceSetToReuse->Coordinates();
...
void VrmlData_ShapeConvert::Convert(const Standard_Boolean theExtractFaces, const Standard_Boolean theExtractEdges,
...
Handle(VrmlData_Geometry) VrmlData_ShapeConvert::triToIndexedFaceSet(const Handle(Poly_Triangulation) & theTri,
    ...
    const Handle(VrmlData_IndexedFaceSet) aFaceSet = new VrmlData_IndexedFaceSet(myScene,
                                                                                 0L,              // no name
                                                                                 Standard_True,   // IsCCW
    ...
    const Standard_Integer** arrPolygons =
        static_cast<const Standard_Integer**>(anAlloc->Allocate(nbTri * sizeof(const Standard_Integer*)));
    aFaceSet->SetPolygons(nbTri, arrPolygons);

    // Store the triangles
    for (i = 0; i < nbTri; i++) {
        Standard_Integer* aPolygon = static_cast<Standard_Integer*>(anAlloc->Allocate(4 * sizeof(Standard_Integer)));
        aPolygon[0] = 3;
    ...
    if (theCoord.IsNull() == Standard_False)
        ...
        const Handle(VrmlData_Coordinate) aCoordNode = new VrmlData_Coordinate(myScene, 0L, nNodes, arrNodes);
        ...
        if (aSurface->IsCNu(1) && aSurface->IsCNv(1)) {
            ...
            for (i = 0; i < nNodes; i++) {
                ...
                if (GeomLib::NormEstim(aSurface, aUV, Tol, aNormal) > 1) {
                    ...
                    if (eqPlan.SquareModulus() > gp::Resolution()) aNormal = gp_Dir(eqPlan);
                }
                if (isReverse) aNormal.Reverse();

                if (aNormal.X() * aNormal.X() < aConf2) aNormal.SetX(0.);
                if (aNormal.Y() * aNormal.Y() < aConf2) aNormal.SetY(0.);
                if (aNormal.Z() * aNormal.Z() < aConf2) aNormal.SetZ(0.);
...
Handle(VrmlData_Geometry) VrmlData_ShapeConvert::polToIndexedLineSet(const Handle(Poly_Polygon3D) & thePol) {
    Standard_Integer i;
...
Handle(VrmlData_Appearance) VrmlData_ShapeConvert::defaultMaterialFace() const {
    ...
    Handle(VrmlData_Appearance) anAppearance = Handle(VrmlData_Appearance)::DownCast(myScene.FindNode(aNodeName));
...
//=======================================================================
void VrmlData_ShapeConvert::addShape(const Handle(VrmlData_Group) & theParent, const TDF_Label& theLabel,
...
//=======================================================================
void VrmlData_ShapeConvert::addInstance(const Handle(VrmlData_Group) & theParent, const TDF_Label& theLabel,
                                        const Handle(TDocStd_Document) & theDoc) {
    Handle(XCAFDoc_ShapeTool) aShapeTool = XCAFDoc_DocumentTool::ShapeTool(theDoc->Main());
...
//=======================================================================
void VrmlData_ShapeConvert::addAssembly(const Handle(VrmlData_Group) & theParent, const TDF_Label& theLabel,
                                        const Handle(TDocStd_Document) & theDoc,
