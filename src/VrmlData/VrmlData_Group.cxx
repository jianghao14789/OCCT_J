...
VrmlData_Group::VrmlData_Group(const VrmlData_Scene& theScene, const char* theName, const Standard_Boolean isTransform)
    : VrmlData_Node(theScene, theName), myIsTransform(isTransform), myNodes(theScene.Allocator()) {}
...
Handle(VrmlData_Node) VrmlData_Group::Clone(const Handle(VrmlData_Node) & theOther) const {
    ...
    if (aResult.IsNull())
        aResult = new VrmlData_Group(theOther.IsNull() ? Scene() : theOther->Scene(), Name(), myIsTransform);
    ...
    if (&aResult->Scene() == &Scene())
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            if (aNode.IsNull() == Standard_False) aResult->myNodes.Append(aNode->Clone(aDummyNode));
        }
    }
    if (myIsTransform) aResult->SetTransform(myTrsf);
    ...
    while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "bboxCenter"))
            ...
            while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
                ...
                if (!OK(aStatus, ReadNode(theBuffer, aChildNode))) break;
                AddNode(aChildNode);
                if (isBracketed == Standard_False) break;
            }
            ...
            if (myIsTransform) {
                if (OK(aStatus, Scene().ReadXYZ(theBuffer, aRotAxis, Standard_False, Standard_False))) {
                    if (aRotAxis.SquareModulus() < Precision::Confusion()) aRotAxis.SetZ(1.0);
            ...
            if (OK(aStatus, ReadMultiString(theBuffer, lstURL))) {
                ...
                for (; anIter.More(); anIter.Next()) {
                    ...
                    for (; anAllIter.More(); anAllIter.Next()) {
                        ...
                        if (aNode->IsKind(STANDARD_TYPE(VrmlData_WorldInfo))) continue;
...
VrmlData_ErrorStatus VrmlData_Group::openFile(Standard_IStream& theStream, const TCollection_AsciiString& theFilename) {
    ...
    for (; aDirIter.More(); aDirIter.Next()) {
        if (!aDirIter.Value().IsAscii()) continue;
        const TCollection_AsciiString aFullName = TCollection_AsciiString(aDirIter.Value()) + theFilename;
    ...
    if (aStatus == VrmlData_EmptyData) {
        ...
        if (!aStream.fail()) aStatus = VrmlData_StatusOK;
    }
    if (aStatus == VrmlData_EmptyData) aStatus = VrmlData_CannotOpenFile;
    return aStatus;
}
...
VrmlData_ErrorStatus VrmlData_Group::Write(const char* thePrefix) const {
    ...
    if (myNodes.IsEmpty() == Standard_False) {
        ...
        if (isTransform && myTrsf.Form() == gp_Identity) isTransform = Standard_False;
        ...
        if (OK(aStatus, aScene.WriteLine(thePrefix, header[isTransform ? 1 : 0], GlobalIndent()))) {
            ...
            if (OK(aStatus) && aScene.IsDummyWrite() == Standard_False) {
                ...
                if (aBoxCorner[0].X() < aBoxCorner[1].X() + Precision::Confusion()) {
                    Sprintf(buf, "bboxCenter  %.9g %.9g %.9g", 0.5 * (aBoxCorner[0].X() + aBoxCorner[1].X()),
                            0.5 * (aBoxCorner[0].Y() + aBoxCorner[1].Y()),
                            0.5 * (aBoxCorner[0].Z() + aBoxCorner[1].Z()));
                    aStatus = aScene.WriteLine(buf);
                    if (OK(aStatus)) {
                        Sprintf(buf, "bboxSize    %.9g %.9g %.9g", aBoxCorner[1].X() - aBoxCorner[0].X(),
                                aBoxCorner[1].Y() - aBoxCorner[0].Y(), aBoxCorner[1].Z() - aBoxCorner[0].Z());
                        aStatus = aScene.WriteLine(buf);
                    }
                }
            }
            if (OK(aStatus) && isTransform && aScene.IsDummyWrite() == Standard_False) {
                ...
                if (myTrsf.GetRotation(anAxis, anAngle)) {
                    // output the Rotation
                    Sprintf(buf, "rotation    %.12g %.12g %.12g %.9g", anAxis.X(), anAxis.Y(), anAxis.Z(), anAngle);
                    aStatus = aScene.WriteLine(buf);
                }
            }
