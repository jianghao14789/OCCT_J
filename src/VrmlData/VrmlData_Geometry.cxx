...
Standard_Boolean VrmlData_ArrayVec3d::AllocateValues(const Standard_Size theLength) {
    myArray = reinterpret_cast<const gp_XYZ*>(Scene().Allocator()->Allocate(theLength * sizeof(gp_XYZ)));
    myLength = theLength;
    return (myArray != 0L);
}
...
const Handle(TopoDS_TShape) & VrmlData_Box::TShape() {
    if (myIsModified) {
        try {
            const TopoDS_Shell aShell = BRepPrimAPI_MakeBox(gp_Pnt(-0.5 * mySize), mySize.X(), mySize.Y(), mySize.Z());
...
Handle(VrmlData_Node) VrmlData_Box::Clone(const Handle(VrmlData_Node) & theOther) const {
    ...
    if (aResult.IsNull()) aResult = new VrmlData_Box(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
...
Handle(VrmlData_Node) VrmlData_Cone::Clone(const Handle(VrmlData_Node) & theOther) const {
    ...
    if (aResult.IsNull()) aResult = new VrmlData_Cone(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
...
VrmlData_ErrorStatus VrmlData_Cone::Read(VrmlData_InBuffer& theBuffer) {
    ...
    while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "bottomRadius"))
            ...
            if (OK(aStatus, ReadBoolean(theBuffer, hasSide))) myHasSide = hasSide;
        } else if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "bottom")) {
            if (OK(aStatus, ReadBoolean(theBuffer, hasBottom))) myHasBottom = hasBottom;
        } else
            break;
...
VrmlData_ErrorStatus VrmlData_Cone::Write(const char* thePrefix) const {
    ...
    if (OK(aStatus, Scene().WriteLine(thePrefix, header, GlobalIndent()))) {
        ...
        if (OK(aStatus) && myHasBottom == Standard_False) aStatus = Scene().WriteLine("bottom   FALSE");
        if (OK(aStatus) && myHasSide == Standard_False) aStatus = Scene().WriteLine("side     FALSE");
...
const Handle(TopoDS_TShape) & VrmlData_Cylinder::TShape() {
    if (myIsModified && (myHasBottom || myHasSide || myHasTop)) {
        try {
            ...
            if (myHasSide) aShapeBuilder.AddShellFace(aShell, aBuilder.LateralFace());
            if (myHasTop) aShapeBuilder.AddShellFace(aShell, aBuilder.TopFace());
            if (myHasBottom) aShapeBuilder.AddShellFace(aShell, aBuilder.BottomFace());
...
Handle(VrmlData_Node) VrmlData_Cylinder::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_Cylinder) aResult = Handle(VrmlData_Cylinder)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_Cylinder(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
...
VrmlData_ErrorStatus VrmlData_Cylinder::Read(VrmlData_InBuffer& theBuffer) {
    ...
    while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "radius"))
            ...
            if (OK(aStatus, ReadBoolean(theBuffer, hasTop))) myHasTop = hasTop;
        } else if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "side")) {
            if (OK(aStatus, ReadBoolean(theBuffer, hasSide))) myHasSide = hasSide;
        } else if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "bottom")) {
            if (OK(aStatus, ReadBoolean(theBuffer, hasBottom))) myHasBottom = hasBottom;
...
VrmlData_ErrorStatus VrmlData_Cylinder::Write(const char* thePrefix) const {
    ...
    if (OK(aStatus, Scene().WriteLine(thePrefix, header, GlobalIndent()))) {
        ...
        if (OK(aStatus) && myHasBottom == Standard_False) aStatus = Scene().WriteLine("bottom   FALSE");
        if (OK(aStatus) && myHasSide == Standard_False) aStatus = Scene().WriteLine("side     FALSE");
        if (OK(aStatus) && myHasTop == Standard_False) aStatus = Scene().WriteLine("top      FALSE");
...
Handle(VrmlData_Node) VrmlData_Sphere::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_Sphere) aResult = Handle(VrmlData_Sphere)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_Sphere(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    aResult->SetRadius(myRadius);
    return aResult;
}
...
Standard_Boolean VrmlData_TextureCoordinate::AllocateValues(const Standard_Size theLength) {
    myPoints = reinterpret_cast<const gp_XY*>(Scene().Allocator()->Allocate(theLength * sizeof(gp_XY)));
    myLength = theLength;
    return (myPoints != 0L);
}
...
Handle(VrmlData_Node) VrmlData_TextureCoordinate::Clone(const Handle(VrmlData_Node) & theOther) const {
    ...
    if (aResult.IsNull())
        aResult = new VrmlData_TextureCoordinate(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
...
VrmlData_ErrorStatus VrmlData_TextureCoordinate::Read(VrmlData_InBuffer& theBuffer) {
    ...
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        ...
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "point"))
            ...
            if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
                if (theBuffer.LinePtr[0] != '[') // opening bracket
                    ...
                    for (;;) {
                        gp_XY anXY;
                        if (!OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) break;
                        // closing bracket, in case that it follows a comma
                        if (theBuffer.LinePtr[0] == ']') {
                            theBuffer.LinePtr++;
                            break;
                        }
                        if (!OK(aStatus, Scene().ReadXY(theBuffer, anXY, Standard_False, Standard_False))) break;
        ...
        if (OK(aStatus) && OK(aStatus, readBrace(theBuffer))) {
            ...
            if (myLength > 0) {
                gp_XY* aPoints = reinterpret_cast<gp_XY*>(Scene().Allocator()->Allocate(myLength * sizeof(gp_XY)));
                myPoints = aPoints;
...
VrmlData_ErrorStatus VrmlData_ArrayVec3d::ReadArray(VrmlData_InBuffer& theBuffer, const char* theName,
    ...
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        ...
        if (theName) {
            ...
            while (theBuffer.LinePtr[0] != ' ' && theBuffer.LinePtr[0] != ',' && theBuffer.LinePtr[0] != '\t' &&
        ...
        if (OK(aStatus) && OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
            if (theBuffer.LinePtr[0] != '[') // opening bracket
            {
                ...
                for (;;) {
                    ...
                    // Read three numbers (XYZ value)
                    if (!OK(aStatus, Scene().ReadXYZ(theBuffer, anXYZ, isScale, Standard_False))) break;
                    vecValues.Append(anXYZ);
                    if (!OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) break;
        ...
        if (OK(aStatus) && OK(aStatus, readBrace(theBuffer))) {
            myLength = vecValues.Length();
            if (myLength > 0) {
                gp_XYZ* anArray = reinterpret_cast<gp_XYZ*>(Scene().Allocator()->Allocate(myLength * sizeof(gp_XYZ)));
                myArray = anArray;
                for (Standard_Integer i = 0; i < Standard_Integer(myLength); i++)
...
VrmlData_ErrorStatus VrmlData_ArrayVec3d::WriteArray(const char* theName, const Standard_Boolean isScale) const {
    VrmlData_ErrorStatus aStatus(VrmlData_StatusOK);
    if (myLength > 0) {
        aStatus = Scene().WriteLine(theName, "[", 2 * GlobalIndent());
        if (OK(aStatus)) {
            for (Standard_Size i = 0; i < myLength - 1; i++)
                if (!OK(aStatus, Scene().WriteXYZ(myArray[i], isScale, ","))) break;
            if (OK(aStatus)) aStatus = Scene().WriteXYZ(myArray[myLength - 1], isScale);
        }
        if (aStatus == VrmlData_StatusOK) aStatus = Scene().WriteLine("]", 0L, -2 * GlobalIndent());
    }
    return aStatus;
}
...
Handle(VrmlData_Node) VrmlData_Coordinate::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_Coordinate) aResult = Handle(VrmlData_Coordinate)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_Coordinate(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    if (&aResult->Scene() == &Scene())
...
Handle(VrmlData_Node) VrmlData_Color::Clone(const Handle(VrmlData_Node) & theOther) const {
    ...
    if (aResult.IsNull()) aResult = new VrmlData_Color(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    if (&aResult->Scene() == &Scene())
        aResult->SetValues(Length(), Values());
    else {
        aResult->AllocateValues(Length());
...
Handle(VrmlData_Node) VrmlData_Normal::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_Normal) aResult = Handle(VrmlData_Normal)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_Normal(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    if (&aResult->Scene() == &Scene())
        aResult->SetValues(Length(), Values());
    else {
        aResult->AllocateValues(Length());
