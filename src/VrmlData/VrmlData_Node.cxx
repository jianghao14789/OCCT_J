...
Standard_Boolean IsEqual(const Handle(VrmlData_Node) & theOne, const Handle(VrmlData_Node) & theTwo) {
    ...
    if (theOne->Name() != 0L && theTwo->Name() != 0L) aResult = (strcmp(theOne->Name(), theTwo->Name()) == 0);
    return aResult;
}
...
VrmlData_Node::VrmlData_Node() : myScene(&MyDefaultScene), myName(0L) {}
...
VrmlData_Node::VrmlData_Node(const VrmlData_Scene& theScene, const char* theName) : myScene(&theScene) {
    if (theName == 0L) theName = "";
    setName(theName);
}
...
Handle(VrmlData_Node) VrmlData_Node::Clone(const Handle(VrmlData_Node) & theOther) const {
    if (theOther.IsNull() == Standard_False) {
        if (theOther->IsKind(DynamicType()) == Standard_False) return NULL;
...
void VrmlData_Node::setName(const char* theName, const char* theSuffix) {
    ...
    if (theSuffix) len[1] = strlen(theSuffix);
    char* aName = (char*)Scene().Allocator()->Allocate(len[0] + len[1]);
    myName = aName;
    memcpy(aName, theName, len[0]);
    if (len[1]) memcpy(&aName[len[0] - 1], theSuffix, len[1] + 1);
}
...
VrmlData_ErrorStatus VrmlData_Node::ReadBoolean(VrmlData_InBuffer& theBuffer, Standard_Boolean& theResult) {
    VrmlData_ErrorStatus aStatus;
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
...
VrmlData_ErrorStatus VrmlData_Node::ReadString(VrmlData_InBuffer& theBuffer, TCollection_AsciiString& theResult) {
    VrmlData_ErrorStatus aStatus;
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        if (theBuffer.LinePtr[0] != '\"')
...
VrmlData_ErrorStatus VrmlData_Node::ReadMultiString(VrmlData_InBuffer& theBuffer,
    ...
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        ...
        while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
            ...
            if (!OK(aStatus, ReadString(theBuffer, aString))) break;
            theResult.Append(aString);
            if (isBracketed == Standard_False || !OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) break;
            if (theBuffer.LinePtr[0] == ',') {
                theBuffer.LinePtr++;
                continue;
...
VrmlData_ErrorStatus VrmlData_Node::ReadNode(VrmlData_InBuffer& theBuffer, Handle(VrmlData_Node) & theNode,
    ...
    if (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "USE")) {
            ...
            if (aStatus == VrmlData_StatusOK) {
                ...
                if (aNode.IsNull()) aStatus = VrmlData_NodeNameUnknown;
                //         else
        ...
        else if (OK(aStatus, const_cast<VrmlData_Scene*>(myScene)->createNode(theBuffer, aNode, theType)))
            if (aNode.IsNull() == Standard_False)
                // The node data are read here, including the final closing brace
                aStatus = aNode->Read(theBuffer);

        if (aStatus == VrmlData_StatusOK) theNode = aNode;
    }
    return aStatus;
}
...
Handle(VrmlData_Node) VrmlData_ShapeNode::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_ShapeNode) aResult = Handle(VrmlData_ShapeNode)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_ShapeNode(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    if (&aResult->Scene() == &Scene()) {
        ...
        if (myAppearance.IsNull() == Standard_False)
            aResult->SetAppearance(Handle(VrmlData_Appearance)::DownCast(myAppearance->Clone(aDummyNode)));
...
VrmlData_ErrorStatus VrmlData_ShapeNode::Read(VrmlData_InBuffer& theBuffer) {
    ...
    if (OK(aStatus)) aStatus = readBrace(theBuffer);
    return aStatus;
}
...
VrmlData_ErrorStatus VrmlData_ShapeNode::Write(const char* thePrefix) const {
    ...
    if (OK(aStatus, aScene.WriteLine(thePrefix, header, GlobalIndent()))) {
        if (myAppearance.IsNull() == Standard_False) aStatus = aScene.WriteNode("appearance", myAppearance);
        if (myGeometry.IsNull() == Standard_False && OK(aStatus)) aStatus = aScene.WriteNode("geometry", myGeometry);
...
Standard_Boolean VrmlData_ShapeNode::IsDefault() const {
    ...
    if (myGeometry.IsNull() == Standard_False) aResult = myGeometry->IsDefault();
    return aResult;
}
...
Handle(VrmlData_Node) VrmlData_Appearance::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_Appearance) aResult = Handle(VrmlData_Appearance)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_Appearance(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    if (&aResult->Scene() == &Scene()) {
        ...
        if (myTTransform.IsNull() == Standard_False)
            aResult->SetTextureTransform(Handle(VrmlData_TextureTransform)::DownCast(myTTransform->Clone(aDummyNode)));
    }
    return aResult;
}
...
VrmlData_ErrorStatus VrmlData_Appearance::Write(const char* thePrefix) const {
    ...
    const VrmlData_Scene& aScene = Scene();
    if (OK(aStatus, aScene.WriteLine(thePrefix, header, GlobalIndent()))) {
        if (myMaterial.IsNull() == Standard_False) aStatus = aScene.WriteNode("material", myMaterial);
        if (myTexture.IsNull() == Standard_False && OK(aStatus)) aStatus = aScene.WriteNode("texture", myTexture);
...
Standard_Boolean VrmlData_Appearance::IsDefault() const {
    Standard_Boolean aResult(Standard_True);
    if (myMaterial.IsNull() == Standard_False) aResult = myMaterial->IsDefault();
    if (aResult && myTexture.IsNull() == Standard_False) aResult = myTexture->IsDefault();
    if (aResult && myTTransform.IsNull() == Standard_False) aResult = myTTransform->IsDefault();
    return aResult;
}
...
VrmlData_ImageTexture::VrmlData_ImageTexture(const VrmlData_Scene& theScene, const char* theName, const char* theURL,
                                             const Standard_Boolean theRepS, const Standard_Boolean theRepT)
    : VrmlData_Texture(theScene, theName, theRepS, theRepT), myURL(theScene.Allocator()) {
...
Handle(VrmlData_Node) VrmlData_ImageTexture::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_ImageTexture) aResult = Handle(VrmlData_ImageTexture)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_ImageTexture(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
    aResult->myURL = myURL;
    return aResult;
}
...
VrmlData_ErrorStatus VrmlData_ImageTexture::Write(const char* thePrefix) const {
    ...
    if (aScene.IsDummyWrite() == Standard_False && OK(aStatus, aScene.WriteLine(thePrefix, header, GlobalIndent()))) {
