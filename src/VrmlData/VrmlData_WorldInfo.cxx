...
VrmlData_WorldInfo::VrmlData_WorldInfo(const VrmlData_Scene& theScene, const char* theName, const char* theTitle)
    : VrmlData_Node(theScene, theName), myInfo(theScene.Allocator()) {
    SetTitle(theTitle);
}
...
Handle(VrmlData_Node) VrmlData_WorldInfo::Clone(const Handle(VrmlData_Node) & theOther) const {
    Handle(VrmlData_WorldInfo) aResult = Handle(VrmlData_WorldInfo)::DownCast(VrmlData_Node::Clone(theOther));
    if (aResult.IsNull()) aResult = new VrmlData_WorldInfo(theOther.IsNull() ? Scene() : theOther->Scene(), Name());
...
VrmlData_ErrorStatus VrmlData_WorldInfo::Read(VrmlData_InBuffer& theBuffer) {
    ...
    while (OK(aStatus, VrmlData_Scene::ReadLine(theBuffer))) {
        ...
        if (VRMLDATA_LCOMPARE(theBuffer.LinePtr, "title")) {
            ...
            if (OK(aStatus, ReadString(theBuffer, aTitleString))) SetTitle(aTitleString.ToCString());
