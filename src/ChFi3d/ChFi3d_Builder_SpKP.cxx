...
extern void ChFi3d_CheckSurfData(const TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_SurfData) & Data);
#endif
//=======================================================================
// function : CompTra
...
static TopAbs_Orientation CompTra(const TopAbs_Orientation O1, const TopAbs_Orientation O2,
...
static void CompCommonPoint(ChFiDS_CommonPoint& FilPoint, const TopoDS_Edge& arc, const HatchGen_PointOnElement& PE,
    ...
    FilPoint.SetArc(Precision::PIntersection(), arc, PE.Parameter(), TopAbs::Compose(arc.Orientation(), Or));
}
...
static ChFiDS_FaceInterference CpInterf(TopOpeBRepDS_DataStructure& DStr, const ChFiDS_FaceInterference& FI) {
    ChFiDS_FaceInterference newF = FI;
    const TopOpeBRepDS_Curve& toc = DStr.Curve(FI.LineIndex());
    Handle(Geom_Curve) newC;
    if (!toc.Curve().IsNull()) newC = Handle(Geom_Curve)::DownCast(toc.Curve()->Copy());
...
static Handle(ChFiDS_SurfData) CpSD(TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_SurfData) & Data) {
...
static Standard_Boolean AdjustParam(const HatchGen_Domain& Dom, Standard_Real& f, Standard_Real& l,
                                    const Standard_Real wref, const Standard_Real period, const Standard_Real pitol) {
...
static Standard_Real ParamOnSpine(const TopOpeBRepDS_DataStructure& DStr, const Standard_Real ptg,
                                  const Handle(ChFiDS_SurfData) & CD, const Handle(ChFiDS_Spine) & Spine,
                                  const Standard_Integer iedge, const Standard_Boolean intf,
                                  const Standard_Boolean intl, const Standard_Real tol, Standard_Boolean& pok) {
    Standard_Real Nl;
    ...
    if ((Nl >= (f - tol) || intf) && (Nl <= (l + tol) || intl)) {
        ...
        while (!found && !fini) {
            ...
            Standard_Boolean First = ((O == TopAbs_FORWARD && sens == 1) || (O == TopAbs_REVERSED && sens == -1));
...
static Standard_Boolean YaUnVoisin(const Handle(ChFiDS_Spine) & Spine, const Standard_Integer iedge,
                                   Standard_Integer& ivois, const Standard_Boolean isfirst) {
...
void ChFi3d_Builder::Trunc(const Handle(ChFiDS_SurfData) & SD, const Handle(ChFiDS_Spine) & Spine,
...
static Standard_Real ResetProl(const TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_SurfData) & CD,
                               const Handle(ChFiDS_Spine) & Spine, const Standard_Integer iedge,
                               const Standard_Boolean isfirst) {
    const BRepAdaptor_Curve& bc = Spine->CurrentElementarySpine(iedge);
...
static Standard_Boolean Tri(const Geom2dHatch_Hatcher& H, const Standard_Integer iH, TColStd_Array1OfInteger& Ind,
                            const Standard_Real wref, const Standard_Real period, const Standard_Real pitol,
    ...
    if (iSansFirst != 0) {
        ...
        HatchGen_PointOnHatching* PH = ((HatchGen_PointOnHatching*)(void*)&H.Domain(iH, Ind(iSansLast)).FirstPoint());
        Standard_Real NewPar =
            H.HatchingCurve(iH).FirstParameter() - period + H.Domain(iH, Ind(iSansLast)).FirstPoint().Parameter();
...
static void FillSD(TopOpeBRepDS_DataStructure& DStr, Handle(ChFiDS_SurfData) & CD, ChFiKPart_RstMap& M,
                   const HatchGen_Domain& Dom, const Standard_Real ponH, const Standard_Boolean isFirst,
                   const Standard_Integer ons, const Standard_Real pitol, const TopoDS_Vertex bout)
...
Standard_Boolean ChFi3d_Builder::SplitKPart(const Handle(ChFiDS_SurfData) & Data, ChFiDS_SequenceOfSurfData& SetData,
                                            const Handle(ChFiDS_Spine) & Spine, const Standard_Integer Iedge,
            ...
            for (Standard_Integer icoup1 = 1; icoup1 <= nbcoup1; icoup1++) {
                for (Standard_Integer j = 1; j <= Nb2; j++) {
                    ...
                    for (Standard_Integer icoup2 = 1; icoup2 <= nbcoup2; icoup2++) {
                        if (f2 <= l1 && f1 <= l2) {
                            if (f1 >= f2 - tol2d) FillSD(DStr, CD, M1, Dom1, f1, 1, 1, pitol, bout1);
                            if (f2 >= f1 - tol2d) FillSD(DStr, CD, M2, Dom2, f2, 1, 2, pitol, bout1);
                            if (l1 >= l2 - tol2d) FillSD(DStr, CD, M2, Dom2, l2, 0, 2, pitol, bout2);
                            if (l2 >= l1 - tol2d) FillSD(DStr, CD, M1, Dom1, l1, 0, 1, pitol, bout2);
                            SetData.Append(CD);
                            CD = CpSD(DStr, CD);
                            ion1.Append(i);
                            ion2.Append(j);
