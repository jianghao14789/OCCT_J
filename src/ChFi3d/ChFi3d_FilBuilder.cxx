...
static void SimulParams(const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_FilSpine) & fsp,
                        Standard_Real& MaxStep, Standard_Real& Fleche) {
    Standard_Real la = HGuide->LastParameter(), fi = HGuide->FirstParameter();
...
ChFi3d_FilBuilder::ChFi3d_FilBuilder(const TopoDS_Shape& S, const ChFi3d_FilletShape FShape, const Standard_Real Ta)
    : ChFi3d_Builder(S, Ta) {
...
void ChFi3d_FilBuilder::SetRadius(const Handle(Law_Function) & C, const Standard_Integer IC,
...
void ChFi3d_FilBuilder::SetRadius(const Standard_Real Radius, const Standard_Integer IC, const TopoDS_Edge& E) {
    if (IC <= NbElements()) {
        Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Value(IC));
...
void ChFi3d_FilBuilder::SetRadius(const Standard_Real Radius, const Standard_Integer IC, const TopoDS_Vertex& V) {
    if (IC <= NbElements()) {
        Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Value(IC));
...
void ChFi3d_FilBuilder::SetRadius(const gp_XY& UandR, const Standard_Integer IC, const Standard_Integer IinC) {
    if (IC <= NbElements()) {
        Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Value(IC));
...
Standard_Boolean ChFi3d_FilBuilder::GetBounds(const Standard_Integer IC, const TopoDS_Edge& E, Standard_Real& F,
                                              Standard_Real& L) {
    if (IC <= NbElements()) {
        Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Value(IC));
...
void ChFi3d_FilBuilder::SetLaw(const Standard_Integer IC, const TopoDS_Edge& E, const Handle(Law_Function) & L) {
...
Handle(ChFiDS_SecHArray1) ChFi3d_FilBuilder::Sect(const Standard_Integer IC, const Standard_Integer IS) const {
    ChFiDS_ListIteratorOfListOfStripe itel;
    Standard_Integer i = 1;
...
void ChFi3d_FilBuilder::SimulKPart(const Handle(ChFiDS_SurfData) & SD) const {
    ...
    gp_Pnt2d p1f = SD->InterferenceOnS1().PCurveOnSurf()->Value(SD->InterferenceOnS1().FirstParameter());
    gp_Pnt2d p1l = SD->InterferenceOnS1().PCurveOnSurf()->Value(SD->InterferenceOnS1().LastParameter());
    gp_Pnt2d p2f = SD->InterferenceOnS2().PCurveOnSurf()->Value(SD->InterferenceOnS2().FirstParameter());
    gp_Pnt2d p2l = SD->InterferenceOnS2().PCurveOnSurf()->Value(SD->InterferenceOnS2().LastParameter());
...
Standard_Boolean ChFi3d_FilBuilder::SimulSurf(
    Handle(ChFiDS_SurfData) & Data, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Standard_Integer Choix, const Handle(BRepAdaptor_Surface) & S1, const Handle(Adaptor3d_TopolTool) & I1,
    const Handle(BRepAdaptor_Surface) & S2, const Handle(Adaptor3d_TopolTool) & I2, const Standard_Real TolGuide,
    ...
    const math_Vector& Soldep, Standard_Integer& intf, Standard_Integer& intl) {
...
//  TopAbs_Orientation Or = S1->Face().Orientation();
    ...
    if (fsp->IsConstant()) {
        ...
        Func.Set(myShape);
        done = SimulData(Data, HGuide, EmptyHGuide, lin, S1, I1, S2, I2, Func, FInv, PFirst, MaxStep, locfleche,
                         TolGuide, First, Last, Inside, Appro, Forward, Soldep, 4, RecOnS1, RecOnS2);
    ...
    ChFi3d_FilCommonPoint(lin->StartPointOnFirst(), lin->TransitionOnS1(), Standard_True, Data->ChangeVertexFirstOnS1(),
                          tolesp);
    ChFi3d_FilCommonPoint(lin->EndPointOnFirst(), lin->TransitionOnS1(), Standard_False, Data->ChangeVertexLastOnS1(),
                          tolesp);
    ChFi3d_FilCommonPoint(lin->StartPointOnSecond(), lin->TransitionOnS2(), Standard_True,
                          Data->ChangeVertexFirstOnS2(), tolesp);
    ChFi3d_FilCommonPoint(lin->EndPointOnSecond(), lin->TransitionOnS2(), Standard_False, Data->ChangeVertexLastOnS2(),
                          tolesp);
...
void ChFi3d_FilBuilder::SimulSurf(
    Handle(ChFiDS_SurfData) & Data, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Standard_Integer Choix, const Handle(BRepAdaptor_Surface) & HS1, const Handle(Adaptor3d_TopolTool) & I1,
    const Handle(BRepAdaptor_Curve2d) & PC1, const Handle(BRepAdaptor_Surface) & HSref1,
    const Handle(BRepAdaptor_Curve2d) & PCref1, Standard_Boolean& Decroch1, const Handle(BRepAdaptor_Surface) & HS2,
    ...
    if (fsp->IsConstant()) {
        ...
        done = SimulData(Data, HGuide, lin, HS2, I2, HS1, PC1, I1, Decroch1, func, finv, finvp, finvc, PFirst, MaxStep,
                         locfleche, TolGuide, First, Last, Soldep, 4, Inside, Appro, Forward, RecP, RecS, RecRst);
        if (!done) {
    ...
    ChFi3d_FilCommonPoint(lin->StartPointOnFirst(), lin->TransitionOnS1(), Standard_True, Data->ChangeVertexFirstOnS2(),
                          tolesp);
    ChFi3d_FilCommonPoint(lin->EndPointOnFirst(), lin->TransitionOnS1(), Standard_False, Data->ChangeVertexLastOnS2(),
                          tolesp);
    ChFi3d_FilCommonPoint(lin->StartPointOnSecond(), lin->TransitionOnS2(), Standard_True,
                          Data->ChangeVertexFirstOnS1(), tolesp);
    ChFi3d_FilCommonPoint(lin->EndPointOnSecond(), lin->TransitionOnS2(), Standard_False, Data->ChangeVertexLastOnS1(),
                          tolesp);
}
...
void ChFi3d_FilBuilder::SimulSurf(
    Handle(ChFiDS_SurfData) & Data, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Standard_Integer Choix, const Handle(BRepAdaptor_Surface) & HS1, const Handle(Adaptor3d_TopolTool) & I1,
    const TopAbs_Orientation Or1, const Handle(BRepAdaptor_Surface) & HS2, const Handle(Adaptor3d_TopolTool) & I2,
    const Handle(BRepAdaptor_Curve2d) & PC2, const Handle(BRepAdaptor_Surface) & HSref2,
    const Handle(BRepAdaptor_Curve2d) & PCref2, Standard_Boolean& Decroch2, const Standard_Real /*Arrow*/,
    const Standard_Real TolGuide, Standard_Real& First, Standard_Real& Last, const Standard_Boolean Inside,
    const Standard_Boolean Appro, const Standard_Boolean Forward, const Standard_Boolean RecP,
    const Standard_Boolean RecS, const Standard_Boolean RecRst, const math_Vector& Soldep) {
    Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Spine);
    ...
    if (fsp->IsConstant()) {
        ...
        done = SimulData(Data, HGuide, lin, HS1, I1, HS2, PC2, I2, Decroch2, func, finv, finvp, finvc, PFirst, MaxStep,
                         locfleche, TolGuide, First, Last, Soldep, 4, Inside, Appro, Forward, RecP, RecS, RecRst);
...
void ChFi3d_FilBuilder::SimulSurf(
    ...
    const Handle(BRepAdaptor_Surface) & HS2, const Handle(Adaptor3d_TopolTool) & I2,
    const Handle(BRepAdaptor_Curve2d) & PC2, const Handle(BRepAdaptor_Surface) & HSref2,
    const Handle(BRepAdaptor_Curve2d) & PCref2, Standard_Boolean& Decroch2, const TopAbs_Orientation Or2,
    const Standard_Real /*Fleche*/, const Standard_Real TolGuide, Standard_Real& First, Standard_Real& Last,
    const Standard_Boolean Inside, const Standard_Boolean Appro, const Standard_Boolean Forward,
    const Standard_Boolean RecP1, const Standard_Boolean RecRst1, const Standard_Boolean RecP2,
    const Standard_Boolean RecRst2, const math_Vector& Soldep) {
    Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Spine);
    if (fsp.IsNull()) throw Standard_ConstructionError("PerformSurf : it is not the spine of a fillet");
    Handle(BRepBlend_Line) lin;
    ...
    if (fsp->IsConstant()) {
        ...
        done = SimulData(Data, HGuide, lin, HS1, PC1, I1, Decroch1, HS2, PC2, I2, Decroch2, func, finv1, finvp1, finv2,
                         finvp2, PFirst, MaxStep, locfleche, TolGuide, First, Last, Soldep, 4, Inside, Appro, Forward,
                         RecP1, RecRst1, RecP2, RecRst2);
        if (!done) throw Standard_Failure("SimulSurf : Failed processing!");
        Standard_Integer nbp = lin->NbPoints();
        sec = new ChFiDS_SecHArray1(1, nbp);
        for (Standard_Integer i = 1; i <= nbp; i++) {
...
Standard_Boolean
ChFi3d_FilBuilder::PerformFirstSection(const Handle(ChFiDS_Spine) & Spine, const Handle(ChFiDS_ElSpine) & HGuide,
                                       ...
                                       const Handle(Adaptor3d_TopolTool) & I2, const Standard_Real Par,
                                       math_Vector& SolDep, TopAbs_State& Pos1, TopAbs_State& Pos2) const {
    Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Spine);
    ...
    if (fsp->IsConstant()) {
        ...
        return TheWalk.PerformFirstSection(Func, Par, SolDep, tolesp, TolGuide, Pos1, Pos2);
    } else {
        BRepBlend_EvolRad Func(S1, S2, HGuide, fsp->Law(HGuide));
        Func.Set(Choix);
        Func.Set(myShape);
...
Standard_Boolean ChFi3d_FilBuilder::PerformSurf(
    ChFiDS_SequenceOfSurfData& SeqData, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Standard_Integer Choix, const Handle(BRepAdaptor_Surface) & S1, const Handle(Adaptor3d_TopolTool) & I1,
    const Handle(BRepAdaptor_Surface) & S2, const Handle(Adaptor3d_TopolTool) & I2, const Standard_Real MaxStep,
    const Standard_Real Fleche, const Standard_Real TolGuide, Standard_Real& First, Standard_Real& Last,
    const Standard_Boolean Inside, const Standard_Boolean Appro, const Standard_Boolean Forward,
    const Standard_Boolean RecOnS1, const Standard_Boolean RecOnS2, const math_Vector& Soldep, Standard_Integer& intf,
    ...
    if (fsp->IsConstant()) {
        ...
        ChFi3d_InitChron(ch); // init perf ComputeData
            ...
            ComputeData(Data, HGuide, Spine, lin, S1, I1, S2, I2, Func, FInv, PFirst, MaxStep, Fleche, TolGuide, First,
                        Last, Inside, Appro, Forward, Soldep, intf, intl, gd1, gd2, gf1, gf2, RecOnS1, RecOnS2);
...
void ChFi3d_FilBuilder::PerformSurf(ChFiDS_SequenceOfSurfData& SeqData, const Handle(ChFiDS_ElSpine) & HGuide,
                                    const Handle(ChFiDS_Spine) & Spine, const Standard_Integer Choix,
                                    const Handle(BRepAdaptor_Surface) & HS1, const Handle(Adaptor3d_TopolTool) & I1,
                                    const Handle(BRepAdaptor_Curve2d) & PC1, const Handle(BRepAdaptor_Surface) & HSref1,
                                    const Handle(BRepAdaptor_Curve2d) & PCref1, Standard_Boolean& Decroch1,
                                    const Handle(BRepAdaptor_Surface) & HS2, const Handle(Adaptor3d_TopolTool) & I2,
                                    const TopAbs_Orientation Or2, const Standard_Real MaxStep,
                                    const Standard_Real Fleche, const Standard_Real TolGuide, Standard_Real& First,
                                    Standard_Real& Last, const Standard_Boolean Inside, const Standard_Boolean Appro,
                                    const Standard_Boolean Forward, const Standard_Boolean RecP,
    ...
    if (fsp->IsConstant()) {
        ...
        done = ComputeData(Data, HGuide, lin, HS2, I2, HS1, PC1, I1, Decroch1, func, finv, finvp, finvc, PFirst,
                           MaxStep, Fleche, TolGuide, First, Last, Soldep, Inside, Appro, Forward, RecP, RecS, RecRst);
        if (!done) {
...
void ChFi3d_FilBuilder::PerformSurf(
    ChFiDS_SequenceOfSurfData& SeqData, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Standard_Integer Choix, const Handle(BRepAdaptor_Surface) & HS1, const Handle(Adaptor3d_TopolTool) & I1,
    const TopAbs_Orientation Or1, const Handle(BRepAdaptor_Surface) & HS2, const Handle(Adaptor3d_TopolTool) & I2,
    const Handle(BRepAdaptor_Curve2d) & PC2, const Handle(BRepAdaptor_Surface) & HSref2,
    const Handle(BRepAdaptor_Curve2d) & PCref2, Standard_Boolean& Decroch2, const Standard_Real MaxStep,
    const Standard_Real Fleche, const Standard_Real TolGuide, Standard_Real& First, Standard_Real& Last,
    const Standard_Boolean Inside, const Standard_Boolean Appro, const Standard_Boolean Forward,
    ...
    if (fsp->IsConstant()) {
        ...
        done = ComputeData(Data, HGuide, lin, HS1, I1, HS2, PC2, I2, Decroch2, func, finv, finvp, finvc, PFirst,
                           MaxStep, Fleche, TolGuide, First, Last, Soldep, Inside, Appro, Forward, RecP, RecS, RecRst);
        if (!done) {
            Spine->SetErrorStatus(ChFiDS_WalkingFailure);
            throw Standard_Failure("PerformSurf : Failed processing!");
...
void ChFi3d_FilBuilder::PerformSurf(
    ...
    Standard_Real& Last, const Standard_Boolean Inside, const Standard_Boolean Appro, const Standard_Boolean Forward,
    const Standard_Boolean RecP1, const Standard_Boolean RecRst1, const Standard_Boolean RecP2,
    const Standard_Boolean RecRst2, const math_Vector& Soldep) {
    Handle(ChFiDS_SurfData) Data = SeqData(1);
    Handle(ChFiDS_FilSpine) fsp = Handle(ChFiDS_FilSpine)::DownCast(Spine);
    ...
    if (fsp->IsConstant()) {
        ...
        done = ComputeData(Data, HGuide, lin, HS1, PC1, I1, Decroch1, HS2, PC2, I2, Decroch2, func, finv1, finvp1,
                           finv2, finvp2, PFirst, MaxStep, Fleche, TolGuide, First, Last, Soldep, Inside, Appro,
                           Forward, RecP1, RecRst1, RecP2, RecRst2);
        if (!done) {
            Spine->SetErrorStatus(ChFiDS_WalkingFailure);
            throw Standard_Failure("PerformSurf : Failed processing!");
        }
...
void ChFi3d_FilBuilder::SplitSurf(ChFiDS_SequenceOfSurfData& SeqData, const Handle(BRepBlend_Line) & Line) {
...
void ChFi3d_FilBuilder::ExtentOneCorner(const TopoDS_Vertex& V, const Handle(ChFiDS_Stripe) & S) {
    ...
    if (Spine->IsTangencyExtremity((Sens == 1))) return; // No extension in the queue
