...
static void ReorderFaces(TopoDS_Face& theF1, TopoDS_Face& theF2, const TopoDS_Face& theFirstFace,
                         const TopoDS_Edge& thePrevEdge, const TopoDS_Vertex& theCommonVertex,
...
static void ConcatCurves(TColGeom_SequenceOfCurve& theCurves, TColGeom_SequenceOfCurve& theNewCurves) {
    while (!theCurves.IsEmpty()) {
        ...
        for (Standard_Integer i = 1; i <= theCurves.Length(); i++) {
            ...
            if (!Success) Success = Concat.Add(aBoundedCurve, 1.e-5, Standard_False);
            if (Success) {
                theCurves.Remove(i);
                i--;
            }
...
static TopoDS_Edge MakeOffsetEdge(const TopoDS_Edge& theEdge, const Standard_Real Distance,
    ...
    Handle(Geom_Surface) TrGS1 = new Geom_RectangularTrimmedSurface(GS1, S1.FirstUParameter(), S1.LastUParameter(),
                                                                    S1.FirstVParameter(), S1.LastVParameter());
    Standard_Real Offset = -Distance;
    if (F1.Orientation() == TopAbs_REVERSED) Offset = Distance;
    ...
    if (OffsetTrGS1.IsNull()) OffsetTrGS1 = MakeOffsetSurf;
    Handle(Geom_Surface) GS2 = BRep_Tool::Surface(F2);
    Handle(Geom_Surface) TrGS2 = new Geom_RectangularTrimmedSurface(GS2, S2.FirstUParameter(), S2.LastUParameter(),
    ...
    if (Intersector.NbLines() > 1) {
        ...
        for (Standard_Integer i = 1; i <= NewCurves.Length(); i++) {
            ...
            if (!Projector.IsDone() || Projector.NbExt() == 0) continue;
    ...
    for (Standard_Integer ind_end = 0; ind_end < 2; ind_end++) {
        if (ind_end == 1 && aBAcurve.IsClosed() /*HGuide->IsPeriodic()*/ /*HGuide->IsClosed()*/) break;
        ...
        if (Projector.IsDone() && Projector.NbExt() > 0) {
            ...
            for (Standard_Integer i = 2; i <= Projector.NbExt(); i++)
                if (Projector.SquareDistance(i) < Projector.SquareDistance(imin)) imin = i;
    ...
    if (Params[0] > Params[1]) {
        ...
        if (fpnt.SquareDistance(lpnt) <= Precision::SquareConfusion()) IsClosed = Standard_True;
        if (IsClosed)
    ...
    if (aBAcurve.IsClosed() /*HGuide->IsPeriodic()*/ /*HGuide->IsClosed()*/) // check the direction of closed curve
    {
        ...
        if (Tangent * OffsetTangent < 0) IntCurve->Reverse();
    }
...
static TopOpeBRepDS_BuildTool mkbuildtool() {
    TopOpeBRepTool_GeomTool GT2(TopOpeBRepTool_BSPLINE1, Standard_True, Standard_False, Standard_False);
...
// purpose  :
//=======================================================================
ChFi3d_Builder::ChFi3d_Builder(const TopoDS_Shape& S, const Standard_Real Ta) : done(Standard_False), myShape(S) {
...
void ChFi3d_Builder::SetParams(const Standard_Real Tang, const Standard_Real Tesp, const Standard_Real T2d,
                               const Standard_Real TApp3d, const Standard_Real TolApp2d, const Standard_Real Fleche) {
    angular = Tang;
    tolesp = Tesp;
    tol2d = T2d;
    tolapp3d = TApp3d;
...
void ChFi3d_Builder::SetContinuity(const GeomAbs_Shape InternalContinuity, const Standard_Real AngularTolerance) {
...
Handle(Geom_Surface) ChFi3d_Builder::ComputedSurface(const Standard_Integer IC, const Standard_Integer IS) const {
    ChFiDS_ListIteratorOfListOfStripe itel;
    Standard_Integer k = 0;
    Handle(ChFiDS_Stripe) st;
    for (itel.Initialize(myListStripe); itel.More(); itel.Next()) {
...
Standard_Boolean ChFi3d_Builder::FaceTangency(const TopoDS_Edge& E0, const TopoDS_Edge& E1,
    ...
    for (It.Initialize(myEFMap(E1)), Nbf = 0; It.More(); It.Next(), Nbf++) {
        if (Nbf > 1) throw Standard_ConstructionError("ChFi3d_Builder:only 2 faces");
...
//=======================================================================
static Standard_Boolean TangentExtremity(const TopoDS_Vertex& V, const TopoDS_Edge& E,
    ...
    if (f1.IsSame(f2) && BRep_Tool::IsClosed(e1, f1)) e2.Orientation(TopAbs_REVERSED);
...
//=======================================================================
static Standard_Boolean TangentOnVertex(const TopoDS_Vertex& V, const TopoDS_Edge& E, const ChFiDS_Map& EFMap,
                                        const Standard_Real tang) {
    TopoDS_Face ff1, ff2;
    ...
    for (Standard_Integer ii = 1; ii <= 2; ii++) {
        ...
        if (sst == ChFiDS_BreakPoint) {
            ...
            for (It.Initialize(myVEMap(V)); It.More(); It.Next()) {
                ...
                {
                    if (!F1.IsSame(F2)) NbG1Connections++;
                    continue;
                }
                ...
                if (Edges.Add(anEdge)) {
                    ...
                    if (V1.IsSame(V2)) // edge is closed - two ends of the edge in the vertex
                    {
                        ...
                        if (anInd == 0) anInd = EdgesOfV.FindIndex(anEdge.Reversed());
            ...
            if (EdgesOfV.Extent() != 3) sommetpourri = Standard_True;
...
Standard_Boolean ChFi3d_Builder::PerformElement(const Handle(ChFiDS_Spine) & Spine, const Standard_Real Offset,
    ...
    // Define concavity
    ChFiDS_TypeOfConcavity TypeOfConcavity = ChFi3d::DefineConnectType(Ec, ff1, ff2, 1.e-5, Standard_True);
...
Standard_Integer ChFi3d_Builder::Contains(const TopoDS_Edge& E, Standard_Integer& IndexInSpine) const {
    Standard_Integer i = 1, j;
    IndexInSpine = 0;
    ChFiDS_ListIteratorOfListOfStripe itel(myListStripe);
    for (; itel.More(); itel.Next(), i++) {
...
Standard_Real ChFi3d_Builder::RelativeAbscissa(const Standard_Integer IC, const TopoDS_Vertex& V) const {
    if (IC <= NbElements()) {
