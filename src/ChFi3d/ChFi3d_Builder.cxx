...
OSD_Chronometer cl_total, cl_extent, cl_perfsetofsurf, cl_perffilletonvertex, cl_filds, cl_reconstruction, cl_setregul,
    cl_perform1corner, cl_perform2corner, cl_performatend, cl_perform3corner, cl_performmore3corner;

Standard_EXPORT Standard_Real t_total, t_extent, t_perfsetofsurf, t_perffilletonvertex, t_filds, t_reconstruction,
    t_setregul, t_perfsetofkgen, t_perfsetofkpart, t_makextremities, t_performatend, t_startsol, t_performsurf,
    t_perform1corner, t_perform2corner, t_perform3corner, t_performmore3corner, t_batten, t_inter, t_sameinter, t_same,
    t_plate, t_approxplate, t_t2cornerinit, t_perf2cornerbyinter, t_chfikpartcompdata, t_cheminement, t_remplissage,
    t_t3cornerinit, t_spherique, t_torique, t_notfilling, t_filling, t_sameparam, t_computedata, t_completedata,
    t_t2cornerDS, t_t3cornerDS;
    ...
    for (Standard_Integer iv = 1; iv <= myVDataMap.Extent(); iv++) {
        ...
        switch (nbs) {
            case 1:
                ...
                break;
            case 2:
                if (nbedges <= 3) ExtentTwoCorner(Vtx, myVDataMap.FindFromIndex(iv));
                break;
            case 3:
                if (nbedges <= 3) ExtentThreeCorner(Vtx, myVDataMap.FindFromIndex(iv));
                break;
            default:
                break;
        }
...
void ChFi3d_Builder::Compute() {
    ...
    if (myListStripe.IsEmpty()) throw Standard_Failure("There are no suitable edges for chamfer or fillet");
    ...
    for (itel.Initialize(myListStripe); itel.More(); itel.Next()) {
        ...
        try {
            ...
            if (itel.Value()->Spine()->ErrorStatus() == ChFiDS_Ok) itel.Value()->Spine()->SetErrorStatus(ChFiDS_Error);
        }
    ...
    for (It.Initialize(myVDataMap(Index)), i = 0; It.More(); It.Next(), i++) {
        ...
        if (CV1.Point().IsEqual(CV2.Point(), 0)) {
            ...
            ChFi3d_ComputeArete(CV1, VOnS1, CV2, VOnS2, DStr.Surface(Fd->Surf()).Surface(), C3d, PCurv, Pardeb, Parfin,
                                tolapp3d, tolapp2d, tolreached, 0);
...
const TopTools_ListOfShape& ChFi3d_Builder::Generated(const TopoDS_Shape& EouV) {
    ...
    if (EouV.ShapeType() != TopAbs_EDGE && EouV.ShapeType() != TopAbs_VERTEX) return myGenerated;
