...
static Standard_Real recadre(const Standard_Real p, const Standard_Real ref, const Standard_Boolean isfirst,
...
static Standard_Boolean Update(const Handle(Adaptor3d_Surface) & fb, const Handle(Adaptor2d_Curve2d) & pcfb,
                               const Handle(Adaptor3d_Surface) & surf, ChFiDS_FaceInterference& fi,
                               ChFiDS_CommonPoint& cp, gp_Pnt2d& p2dbout, const Standard_Boolean isfirst,
                               Standard_Real& pared, Standard_Real& wop, const Standard_Real tol) {
...
static Standard_Boolean Update(const Handle(Adaptor3d_Surface) & fb, const Handle(Adaptor3d_Curve) & ct,
                               ChFiDS_FaceInterference& fi, ChFiDS_CommonPoint& cp, gp_Pnt2d& p2dbout,
                               const Standard_Boolean isfirst, Standard_Real& wop) {
...
static Standard_Boolean IntersUpdateOnSame(Handle(GeomAdaptor_Surface) & HGs, Handle(BRepAdaptor_Surface) & HBs,
                                           const Handle(Geom_Curve) & c3dFI, const TopoDS_Face& Fop,
                                           const TopoDS_Face& Fprol, const TopoDS_Edge& Eprol, const TopoDS_Vertex& Vtx,
                                           const Standard_Boolean isFirst, const Standard_Real Tol,
                                           ChFiDS_FaceInterference& FIop, ChFiDS_CommonPoint& CPop, gp_Pnt2d& FprolUV,
                                           Standard_Real& c3dU) {
    ...
    if (Update(HBs, Hc3df, FIop, CPop, FprolUV, isFirst, c3dU)) return Standard_True;

    if (!ChFi3d::IsTangentFaces(Eprol, Fprol, Fop)) return Standard_False;

    Handle(Geom2d_Curve) gpcprol = BRep_Tool::CurveOnSurface(Eprol, Fprol, uf, ul);
    if (gpcprol.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
...
static Standard_Boolean Update(const Handle(Adaptor3d_Surface) & face, const Handle(Adaptor2d_Curve2d) & edonface,
                               const Handle(Adaptor3d_Surface) & surf, ChFiDS_FaceInterference& fi,
                               ChFiDS_CommonPoint& cp, const Standard_Boolean isfirst) {
    ...
    if (ext.IsDone()) {
        ...
        if ((parltg > f) && (parltg < l)) {
            ...
            if (!PConF.IsNull()) {
                ...
                if (!aTrCurve.IsNull()) PConF = aTrCurve->BasisCurve();
                if (!PConF->IsPeriodic()) {
                    if (isfirst) {
                        Standard_Real fpar = PConF->FirstParameter();
                        if (parltg < fpar) parltg = fpar;
                    } else {
                        Standard_Real lpar = PConF->LastParameter();
                        if (parltg > lpar) parltg = lpar;
                    }
                }
            }
            /////////////////////////////////////////////////////
...
static void ChFi3d_ExtendSurface(Handle(Geom_Surface) & S, Standard_Integer& prol) {
    ...
    prol = (S->IsKind(STANDARD_TYPE(Geom_BSplineSurface)) ? 1 : S->IsKind(STANDARD_TYPE(Geom_BezierSurface)) ? 2 : 0);
    if (!prol) return;
...
static void ComputeCurve2d(const Handle(Geom_Curve) & Ct, TopoDS_Face& Face, Handle(Geom2d_Curve) & C2d) {
    TopoDS_Edge E1;
...
static void ChFi3d_Recale(BRepAdaptor_Surface& Bs, gp_Pnt2d& p1, gp_Pnt2d& p2, const Standard_Boolean refon1) {
...
Standard_Boolean ChFi3d_SelectStripe(ChFiDS_ListIteratorOfListOfStripe& It, const TopoDS_Vertex& Vtx,
...
void ChFi3d_Builder::PerformOneCorner(const Standard_Integer Index, const Standard_Boolean thePrepareOnSame) {
    ...
    if (isfirst) {
        ...
        for (; num > 1 && ((SeqFil.Value(num)->IndexOfS1() == 0) || (SeqFil.Value(num)->IndexOfS2() == 0));) {
            SeqFil.Remove(num); // The surplus is removed
            num--;
        }
    }
    ...
    if (inters) {
        ...
        if (onsame && IFopArc == 1)
            ...
            if (Hc1.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
        ...
        if (onsame && IFopArc == 2)
            ...
            if (Hc2.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
        ...
        if (!ChFi3d_ComputeCurves(HGs, HBs, Pardeb, Parfin, Cc, Ps, Pc, tolesp, tol2d, tolreached))
    ...
    if (!intcouture) {
        ...
        Handle(TopOpeBRepDS_SurfaceCurveInterference) Interfc = ChFi3d_FilCurveInDS(ICurve, IShape, Pc, Et);
        ...
        for (; aStrIt.More(); aStrIt.Next()) {
            ...
            for (iPart = 1; iPart <= aSeqData->Length(); iPart++) {
                ...
                if (IShape == aData->IndexOfS1()) {
                    ...
                    anOtherPCurve.Load(aPCurve, aData->InterferenceOnS1().FirstParameter(),
                    ...
                    anOtherPCurve.Load(aPCurve, aData->InterferenceOnS2().FirstParameter(),
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            if (anOtherIntrf.IsNull()) continue;
            ...
            if (anOtherCur.IsNull()) continue;
            Geom2dAdaptor_Curve anOtherPCurve(anOtherIntrf->PCurve(), anOtherCur->FirstParameter(),
        ...
        for (Standard_Integer i = 0; i < 2; i++) {
            if (CV[i].IsOnArc() && ChFi3d_IsPseudoSeam(CV[i].Arc(), Fv)) {
                ...
                if (Hc.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                ...
                if (HcR.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                ...
                if (DegenExist) {
                    ...
                    if (Cd.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                    Handle(Geom2d_TrimmedCurve) tCd = Handle(Geom2d_TrimmedCurve)::DownCast(Cd);
                    if (!tCd.IsNull()) Cd = tCd->BasisCurve();
...
#define VARIANT1
        ...
        Handle(TopOpeBRepDS_CurvePointInterference) interfv = ChFi3d_FilVertexInDS(OVtx, IArcspine, IVtx, parVtx);
        ...
        if (!ChFi3d_ComputeCurves(HBop, HBs, Pardeb, Parfin, zob3d, zob2dop, zob2dv, tolesp, tol2d, tolreached))
            throw Standard_Failure("OneCorner : echec calcul intersection");
    ...
    if (!CV1.IsVertex()) ChFi3d_SetPointTolerance(DStr, box1, stripe->IndexPoint(isfirst, 1));
    if (!CV2.IsVertex()) ChFi3d_SetPointTolerance(DStr, box2, stripe->IndexPoint(isfirst, 2));
...
static void cherche_face(const TopTools_ListOfShape& map, const TopoDS_Edge& E, const TopoDS_Face& F1,
                         const TopoDS_Face& F2, const TopoDS_Face& F3, TopoDS_Face& F) {
    TopoDS_Face Fcur;
...
static Standard_Boolean IsShrink(const Geom2dAdaptor_Curve PC, const Standard_Real Pf, const Standard_Real Pl,
                                 const Standard_Real Param, const Standard_Boolean isU, const Standard_Real tol) {
    switch (PC.GetType()) {
        case GeomAbs_Line: {
        ...
        case GeomAbs_BSplineCurve: {
            ...
            for (i = 1; i <= aSample.NbPoints(); i++) {
                ...
                if (Abs(P.Coord(isU ? 1 : 2) - Param) > tol) return Standard_False;
            }
            return Standard_True;
        }
        default:;
...
void ChFi3d_Builder::PerformIntersectionAtEnd(const Standard_Integer Index) {
    ...
    if (nbsurf != nbedge && nbsurf != 1) {
        ...
        if (reg1 || reg2) {
            ...
            if (cp1.IsOnArc()) {
                if (cp1.Arc().IsSame(Eadj1) || cp1.Arc().IsSame(Eadj2)) compoint1 = Standard_True;
            }
            if (cp2.IsOnArc()) {
                if (cp2.Arc().IsSame(Eadj1) || cp2.Arc().IsSame(Eadj2)) compoint2 = Standard_True;
            }
    ...
    if (CV1.IsVertex()) {
        ...
        for (It3.Initialize(myVEMap(V)); It3.More() && !trouve; It3.Next()) {
            ...
            if (!E.IsSame(Edge[0]) && (containE(F1, E))) trouve = Standard_True;
        }
    ...
    if (CV2.IsVertex()) {
        ...
        for (It3.Initialize(myVEMap(V)); It3.More() && !trouve; It3.Next()) {
            E = TopoDS::Edge(It3.Value());
            if (!E.IsSame(Edge[2]) && (containE(F2, E))) trouve = Standard_True;
        }
        TopoDS_Vertex Vt, V3, V4;
    ...
    if (!onecorner) {
        ...
        if (nface == 3) {
            ...
            if (!findonf1) {
                ...
                if (MapV.Extent() == 2)
                    if (!MapV(1).IsSame(Vtx) && !MapV(2).IsSame(Vtx)) findonf1 = Standard_True;
            }
            if (!findonf2) {
                TopTools_IndexedMapOfShape MapV;
                TopExp::MapShapes(Edge[2], TopAbs_VERTEX, MapV);
                if (MapV.Extent() == 2)
                    if (!MapV(1).IsSame(Vtx) && !MapV(2).IsSame(Vtx)) findonf2 = Standard_True;
            }
    ...
    if (!isOnSame1 && !isOnSame2) checkShrink = Standard_False;
    ...
    for (; nb <= nbface; nb++) {
            ...
            if (inters.IsDone()) {
                ...
                if (nbp == 0) {
                    ...
                    if (!extend) {
                        ...
                        if (cfacemoins1.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                        cface = BRep_Tool::CurveOnSurface(E2, Face[nb], u2, v2);
                        if (cface.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
        ...
        if (!ChFi3d_ComputeCurves(HGs, HBs, Pardeb, Parfin, Cc, Ps, Pc, tolesp, tol2d, tolreached, nbface == 1)) {
            PerformMoreThreeCorner(Index, 1);
            return;
        }
        ...
        Interfp1 = ChFi3d_FilPointInDS(TopAbs_FORWARD, indcurve[nb - 1], indpoint1, Cc->FirstParameter(), Isvtx1);
        Interfp2 = ChFi3d_FilPointInDS(TopAbs_REVERSED, indcurve[nb - 1], indpoint2, Cc->LastParameter(), Isvtx2);
        ...
        if (nb == 1) {
            ...
            Interfedge[0] = ChFi3d_FilPointInDS(CV1.TransitionOnArc(), Iarc1, indpoint1, paredge1, Isvtx1);
            // DStr.ChangeShapeInterferences(Edge[0]).Append(Interfp1);
        }
        if (nb == nbface) {
            Standard_Integer Iarc2 = DStr.AddShape(Edge[nb]);
            Interfedge[nb] = ChFi3d_FilPointInDS(CV2.TransitionOnArc(), Iarc2, indpoint2, paredge2, Isvtx2);
            ...
            if (!extend && !(oneintersection1 || oneintersection2)) {
                ...
                if (oneintersection1 || oneintersection2) {
                    ...
                    if (extend) {
                        ...
                        if (edgesau.Orientation() != orient) aLocalEdge.Reverse();
                ...
                if (oneintersection1 || oneintersection2) {
                    ...
                    if (facesau.Orientation() == Face[0].Orientation()) orient = TopAbs::Reverse(orient);
                    if (extend) {
                    ...
                    if (Face[nb].Orientation() == Face[nb - 1].Orientation()) orient = TopAbs::Reverse(orient);
                }
                if (!bordlibre) {
        ...
        if (checkShrink && IsShrink(Ps, p1, p2, checkShrParam, isUShrink, Precision::Parametric(tolreached))) {
            shrink[nb - 1] = 1;
            ...
            ChFi3d_ComputePCurv(Cc, UV1, UV2, Ps, DStr.Surface(SDprev->Surf()).Surface(), p1, p2, tolesp, aTolreached);
        ...
        if (!shrink[nb - 1]) InterfPS[nb - 1] = ChFi3d_FilCurveInDS(indcurve[nb - 1], Isurf, Ps, orcourbe);
        indpoint1 = indpoint2;
    ...
    for (nb = 1; nb <= nbface; nb++) {
        ...
        if (!shrink[nb - 1]) DStr.ChangeSurfaceInterferences(Isurf).Append(InterfPS[nb - 1]);
        if (!proledge[nb - 1]) DStr.ChangeShapeInterferences(Edge[nb - 1]).Append(Interfedge[nb - 1]);
    }
...
void ChFi3d_Builder::PerformMoreSurfdata(const Standard_Integer Index) {
    ...
    for (; anIter.More() && !isFound; anIter.Next()) {
        ...
        if (containE(aFace, anArc1)) isFound = Standard_True;
    }
    ...
    for (; anIter.More() && !isFound; anIter.Next()) {
        ...
        if (containE(aFace, anArc2) && !anArc2.IsSame(anArc1)) isFound = Standard_True;
    }
    ...
    if (isFirst)
        ChFi3d_ComputeArete(
            aSurfData->VertexLastOnS1(),
            aSurfData->InterferenceOnS1().PCurveOnSurf()->Value(aSurfData->InterferenceOnS1().LastParameter()),
            aSurfData->VertexLastOnS2(),
            aSurfData->InterferenceOnS2().PCurveOnSurf()->Value(aSurfData->InterferenceOnS2().LastParameter()),
            DStr.Surface(aSurfData->Surf()).Surface(), aCracc, aPCurv1, aParf, aParl, aTol3d, tol2d, aTolReached, 0);
    else
        ChFi3d_ComputeArete(
            aSurfData->VertexFirstOnS1(),
            aSurfData->InterferenceOnS1().PCurveOnSurf()->Value(aSurfData->InterferenceOnS1().FirstParameter()),
            aSurfData->VertexFirstOnS2(),
            aSurfData->InterferenceOnS2().PCurveOnSurf()->Value(aSurfData->InterferenceOnS2().FirstParameter()),
            DStr.Surface(aSurfData->Surf()).Surface(), aCracc, aPCurv1, aParf, aParl, aTol3d, tol2d, aTolReached, 0);
    ...
    for (i = 1; i <= anInterSS.NbLines() && !isFound; i++) {
        ...
        if (aPext1.Distance(aCP1onArc.Point()) <= aTol3d || aPext2.Distance(aCP1onArc.Point()) <= aTol3d)
    ...
    } else if (aPext2.Distance(aCP2onArc.Point()) > aTol3d && aPext2.Distance(aCP1onArc.Point()) > aTol3d) {
        aPext = aPext2;
        isPextFound = Standard_True;
    } else {
    ...
    if (isDoSecondSection) {
        ...
        for (i = 1; i <= anInterSS2.NbLines() && !isFound; i++) {
            ...
            if (aPext1.Distance(aCP2onArc.Point()) <= aTol3d || aPext2.Distance(aCP2onArc.Point()) <= aTol3d)
                isFound = Standard_True;
        }
    ...
    anInterfp1 = ChFi3d_FilPointInDS(aCP1onArc.TransitionOnArc(), indArc1, indCP1onArc, aCP1onArc.ParameterOnArc());
    ...
    Handle(TopOpeBRepDS_SolidSurfaceInterference) SSI = new TopOpeBRepDS_SolidSurfaceInterference(
        TopOpeBRepDS_Transition(anOrSD1), TopOpeBRepDS_SOLID, indSol, TopOpeBRepDS_SURFACE, indSurf1);
    ...
    if (isToReverse) anOrSurf = TopAbs::Reverse(anOrSurf);
    ...
    if (anOrSD1 == anOrSD2) anOrSurf = TopAbs::Reverse(anOrSurf);
    ...
    if (aP1.Distance(aP4) > aTol3d && aP2.Distance(aP3) > aTol3d) anOrSurf = TopAbs::Reverse(anOrSurf);
    ...
    Handle(Geom_Curve) aTrCline = new Geom_TrimmedCurve(aCline, aCline->FirstParameter(), aCline->LastParameter());
...
Standard_Boolean ChFi3d_Builder::FindFace(const TopoDS_Vertex& V, const ChFiDS_CommonPoint& P1,
    ...
    if (nbsurf != 1 && inters) {
        ...
        if (CV3.IsOnArc()) {
            if (CV3.Arc().IsSame(arc1)) {
                if (CV1.Point().Distance(CV3.Point()) < 1.e-4) oksurf = Standard_True;
            } else if (CV3.Arc().IsSame(arc2)) {
                if (CV2.Point().Distance(CV3.Point()) < 1.e-4) oksurf = Standard_True;
            }
        }
        ...
        if (CV4.IsOnArc()) {
            if (CV1.Point().Distance(CV4.Point()) < 1.e-4)
                ...
                if (CV2.Point().Distance(CV4.Point()) < 1.e-4) oksurf = Standard_True;
            }
        }
    }
    return oksurf;
...
void ChFi3d_Builder::IntersectMoreCorner(const Standard_Integer Index) {
    ...
    gp_Pnt2d p2dbout;
    {
        ...
        if (!inters && ChFi3d::IsTangentFaces(Arcprol, Fv, Fop)) {
            ...
            inters = Update(HBs, pcprol, HGs, FiopArc, CPopArc, p2dbout, isfirst, partemp, wop, 10 * tolesp);
        }
        Handle(BRepAdaptor_Curve2d) pced = new BRepAdaptor_Curve2d();
        pced->Initialize(CPadArc.Arc(), Fv);
        Update(HBs, pced, HGs, FiadArc, CPadArc, isfirst);
    ...
    if (inters) {
        ...
        // Modif of lvt to find the suite of Arcprol in the other face
        {
            ...
            for (It.Initialize(myVEMap(Vtx)); It.More(); It.Next()) {
                if (!(Arcprol.IsSame(It.Value()) || Arcspine.IsSame(It.Value()) || Arcpiv.IsSame(It.Value()))) {
