...
// purpose  :
//=======================================================================
static Standard_Integer SearchPivot(Standard_Integer* s, Standard_Real u[3][3], const Standard_Real t) {
...
// purpose  :
//=======================================================================
static Standard_Boolean SearchFD(TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_Stripe) & cd1,
...
static Standard_Boolean ToricCorner(const TopoDS_Face& F, const Standard_Real rd, const Standard_Real rf,
                                    const gp_Vec& v) {
...
void ChFi3d_FilBuilder::PerformThreeCorner(const Standard_Integer Jndex) {
    ...
    oksea[2] = SearchFD(DStr, CD[0], CD[1], sens[0], sens[1], i[0][1], i[1][0], p[0][1], p[1][0], Index[0], Index[1],
                        face[2], sameside[2], jf[0][1], jf[1][0]);
    oksea[1] = SearchFD(DStr, CD[0], CD[2], sens[0], sens[2], i[0][2], i[2][0], p[0][2], p[2][0], Index[0], Index[2],
                        face[1], sameside[1], jf[0][2], jf[2][0]);
    oksea[0] = SearchFD(DStr, CD[1], CD[2], sens[1], sens[2], i[1][2], i[2][1], p[1][2], p[2][1], Index[1], Index[2],
                        face[0], sameside[0], jf[1][2], jf[2][1]);
    //
    ...
    if (oksea[2] && oksea[1] && !sameside[2] && !sameside[1]) {
        ...
        if (Abs(p[0][1] - p[0][2]) <= tol2d && Abs(p[1][0] - p[1][2]) <= tol2d && Abs(p[2][0] - p[2][1]) <= tol2d) {
    ...
    ChFi3d_ExtrSpineCarac(DStr, CD[deb], i[deb][pivot], p[deb][pivot], jjjd, sens[deb], Pdeb, Vdeb, Rdeb);
    ChFi3d_ExtrSpineCarac(DStr, CD[fin], i[fin][pivot], p[fin][pivot], jjjf, sens[fin], Pfin, Vfin, Rfin);
    ChFi3d_ExtrSpineCarac(DStr, CD[pivot], i[pivot][deb], p[pivot][deb], 0, sens[pivot], Pdp, Vdp, Rdp);
    ChFi3d_ExtrSpineCarac(DStr, CD[pivot], i[pivot][fin], p[pivot][fin], 0, sens[pivot], Pfp, Vfp, Rfp);
    ...
    if (!pivdif) {
        ...
        gp_Vec bidvec;
        Standard_Real bidr;
        ChFi3d_ExtrSpineCarac(DStr, CD[deb], i[deb][pivot], p[deb][pivot], jf[deb][fin], sens[deb], ptestdeb, bidvec,
                              bidr);
        ChFi3d_ExtrSpineCarac(DStr, CD[fin], i[fin][pivot], p[fin][pivot], jf[fin][deb], sens[fin], ptestfin, bidvec,
                              bidr);
    ...
    if (!c1pointu) {
        if (!pivdif)
            c1pointu = (Abs(p[deb][pivot] - p[deb][fin]) <= tol2d && Abs(p[fin][pivot] - p[fin][deb]) <= tol2d);
        if (Abs(p[pivot][deb] - p[pivot][fin]) <= tol2d) c1toric = ToricCorner(face[pivot], Rdeb, Rfin, Vdp);
    }
    ...
    gp_Pnt2d ppp1, ppp2;
    const ChFiDS_FaceInterference& bid1 = CD[pivot]->SetOfSurfData()->Value(i[pivot][deb])->InterferenceOnS1();
    ppp1 = bid1.PCurveOnSurf()->Value(bid1.FirstParameter());
    const ChFiDS_FaceInterference& bid2 = CD[pivot]->SetOfSurfData()->Value(i[pivot][deb])->InterferenceOnS2();
    ppp2 = bid2.PCurveOnSurf()->Value(bid2.LastParameter());
    Standard_Real uu1 = ppp1.X(), uu2 = ppp2.X(), vv1 = ppp1.Y(), vv2 = ppp2.Y();
    GeomAdaptor_Surface gasurf((DStr.Surface(CD[pivot]->SetOfSurfData()->Value(i[pivot][deb])->Surf())).Surface(), uu1,
                               uu2, vv1, vv2);
    ...
    if (styp == GeomAbs_Cylinder) {
        ...
        vv1 -= 0.5 * h;
        vv2 += 0.5 * h;
        gasurf.Load((DStr.Surface(CD[pivot]->SetOfSurfData()->Value(i[pivot][deb])->Surf())).Surface(), uu1, uu2, vv1,
                    vv2);
    ...
    if (c1toric) {
        ...
        // Direct Construction.
        // ---------------------
        done = ChFiKPart_ComputeData::ComputeCorner(DStr, coin, Fac, Surf, oo1, oo2, o1, o2, Rdeb, Rdp, pfac1, pfac2,
                                                    psurf1, psurf2);
        ...
        ChFi3d_InitChron(ch); // init perf case sphere
            ...
            ChFiKPart_ComputeData::ComputeCorner(DStr, coin, Fac, Surf, oo1, oo2, o1, o2, Rdp, pfac1, psurf1, psurf2);
    ...
    if (!done) {
        if (!filling) {
            ...
            Handle(Geom_Curve) spinecoin = ChFi3d_CircularSpine(WFirst, WLast, Pdeb, Vdeb, Pfin, Vfin, radpondere);
            if (spinecoin.IsNull()) {
            ...
            if (Abs(Rdeb - Rfin) <= tolesp) {
                ...
                Standard_Integer intf = 3, intl = 3;
                    ...
                    ComputeData(coin, cornerspine, NullSpine, lin, Fac, IFac, Surf, ISurf, func, finv, ffi, pasmax,
                                locfleche, TolGuide, ffi, lla, 0, 0, 1, Soldep, intf, intl, Gd1, Gd2, Gf1, Gf2, 0, 1);
#ifdef OCCT_DEBUG
        ...
        if (filling) {
            ...
            if (!c1pointu)
                Bfac =
                    ChFi3d_mkbound(Fac, PCurveOnFace, sens[deb], pfac1, vfac1, sens[fin], pfac2, vfac2, tolesp, 2.e-4);
            Standard_Integer kkk;
            gp_Pnt ppbid;
            gp_Vec vp1, vp2;
            kkk = CD[deb]->SetOfSurfData()->Value(i[deb][pivot])->Interference(jf[deb][pivot]).LineIndex();
            DStr.Curve(kkk).Curve()->D1(p[deb][pivot], ppbid, vp1);
            kkk = CD[fin]->SetOfSurfData()->Value(i[fin][pivot])->Interference(jf[fin][pivot]).LineIndex();
            ...
            gp_Pnt2d pfin2 = CD[fin]
                                 ...
                                 .PCurveOnSurf()
                                 ->Value(parfin2);
            Handle(Geom_Surface) sdeb = DStr.Surface(CD[deb]->SetOfSurfData()->Value(i[deb][pivot])->Surf()).Surface();
            Handle(Geom_Surface) sfin = DStr.Surface(CD[fin]->SetOfSurfData()->Value(i[fin][pivot])->Surf()).Surface();
            ...
            Surfcoin->VReverse(); // revert to direction face surface;
                ...
                CompleteData(coin, Surfcoin, Fac, PCurveOnFace, Surf, PCurveOnPiv, fdpiv->Orientation(), 0, 0, 0, 0, 0);
    ...
    if (done) {
        ...
        ChFi3d_ComputeArete(Pf1, pp1, Pf2, pp2, DStr.Surface(coin->Surf()).Surface(), C3d, corner->ChangeFirstPCurve(),
                            P1deb, P2deb, tolesp, tol2d, tolreached, 0);
        ...
        ChFi3d_ComputeArete(Pl1, pp1, Pl2, pp2, DStr.Surface(coin->Surf()).Surface(), C3d, corner->ChangeLastPCurve(),
                            P1fin, P2fin, tolesp, tol2d, tolreached, 0);
        ...
        ChFi3d_ComputePCurv(crefdeb, pp1, pp2, CD[deb]->ChangePCurve(isfirst), DStr.Surface(fddeb->Surf()).Surface(),
        ...
        ChFi3d_ComputePCurv(creffin, pp1, pp2, CD[fin]->ChangePCurve(isfirst), DStr.Surface(fdfin->Surf()).Surface(),
        ...
        Standard_Real tolr;
        ChFi3d_SameParameter(Ccoinpiv, C2dOnPiv, Spiv, fi.FirstParameter(), fi.LastParameter(), tolesp, tolr);
