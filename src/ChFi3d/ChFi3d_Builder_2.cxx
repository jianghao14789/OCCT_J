...
//
//====================================================================
static void ChFi3d_CoupeParPlan(const ChFiDS_CommonPoint& compoint1, const ChFiDS_CommonPoint& compoint2,
                                Handle(GeomAdaptor_Surface) & HConge, const gp_Pnt2d& UV1, const gp_Pnt2d& UV2,
                                const Standard_Real tol3d, const Standard_Real tol2d, Handle(Geom_Curve) & C3d,
                                Handle(Geom2d_Curve) & pcurve, Standard_Real& tolreached, Standard_Real& Pardeb,
                                Standard_Real& Parfin, Standard_Boolean& plane) {
    plane = Standard_True;
    if (compoint1.IsOnArc() && compoint2.IsOnArc()) {
        ...
        if (scal < 0.01) {
            ...
            Pfin(3) = u1;
            Pfin(4) = v1;
            if (ChFi3d_ComputeCurves(HConge, HPlan, Pdeb, Pfin, C3d, pcurve, C2dint2, tol3d, tol2d, tolreached)) {
...
static Standard_Boolean SortieTangente(const ChFiDS_CommonPoint& CP, const TopoDS_Face& /*F*/,
                                       const Handle(ChFiDS_SurfData) & /*SD*/, const Standard_Integer /*OnS*/,
...
static Standard_Boolean BonVoisin(const gp_Pnt& Point, Handle(BRepAdaptor_Surface) & HS, TopoDS_Face& F,
                                  Handle(GeomAdaptor_Surface) & plane, const TopoDS_Edge& cured, Standard_Real& XDep,
                                  Standard_Real& YDep, const ChFiDS_Map& EFMap, const Standard_Real tolesp) {
...
static Standard_Boolean Projection(Extrema_ExtPC& PExt, const gp_Pnt& P, const Adaptor3d_Curve& C, Standard_Real& W,
                                   Standard_Real Tol) {
    Standard_Real Dist2, daux2;
    Dist2 = C.Value(W).SquareDistance(P);

    // It is checked if it is not already a solution
    if (Dist2 < Tol * Tol) return Standard_True;
...
static void TgtKP(const Handle(ChFiDS_SurfData) & CD, const Handle(ChFiDS_Spine) & Spine, const Standard_Integer iedge,
                  const Standard_Boolean isfirst, gp_Pnt& ped, gp_Vec& ded) {
...
Standard_Boolean IsG1(const ChFiDS_Map& TheMap, const TopoDS_Edge& E, const TopoDS_Face& FRef, TopoDS_Face& FVoi) {
    TopTools_ListIteratorOfListOfShape It;
...
//========================================================================
static Standard_Integer SearchFaceOnV(const ChFiDS_CommonPoint& Pc, const TopoDS_Face& FRef, const ChFiDS_Map& VEMap,
                                      const ChFiDS_Map& EFMap, TopoDS_Face& F1, TopoDS_Face& F2) {
...
//========================================================================
static void ChangeTransition(const ChFiDS_CommonPoint& Precedant, ChFiDS_CommonPoint& Courant,
                             Standard_Integer FaceIndex, const Handle(TopOpeBRepDS_HDataStructure) & DS) {
    ...
    if (tochange)
        Courant.SetArc(Precision::Confusion(), Arc, Precedant.ParameterOnArc(),
...
void ChFi3d_Builder::CallPerformSurf(
    Handle(ChFiDS_Stripe) & Stripe, const Standard_Boolean Simul, ChFiDS_SequenceOfSurfData& SeqSD,
    Handle(ChFiDS_SurfData) & SD, const Handle(ChFiDS_ElSpine) & HGuide, const Handle(ChFiDS_Spine) & Spine,
    const Handle(BRepAdaptor_Surface) & HS1, const Handle(BRepAdaptor_Surface) & HS3, const gp_Pnt2d& pp1,
    const gp_Pnt2d& pp3, const Handle(Adaptor3d_TopolTool) & It1, const Handle(BRepAdaptor_Surface) & HS2,
    const Handle(BRepAdaptor_Surface) & HS4, const gp_Pnt2d& pp2, const gp_Pnt2d& pp4,
    const Handle(Adaptor3d_TopolTool) & It2, const Standard_Real MaxStep, const Standard_Real Fleche,
    ...
    Standard_Integer Choix =
        ChFi3d::NextSide(Or1, Or2, Stripe->OrientationOnFace1(), Stripe->OrientationOnFace2(), Stripe->Choix());
    Soldep(1) = pp1.X();
    Soldep(2) = pp1.Y();
    Soldep(3) = pp2.X();
    Soldep(4) = pp2.Y();
    ...
    if (Simul) {
        isdone = SimulSurf(SD, HGuide, Spine, Choix, HS1, It1, HS2, It2, tolesp, First, Last, Inside, Inside, forward,
        ...
        isdone = PerformSurf(SeqSD, HGuide, Spine, Choix, HS1, It1, HS2, It2, MaxStep, Fleche, tolesp, First, Last,
    ...
    if (!isdone) {
        ...
        if (reprise) {
            Choix =
                ChFi3d::NextSide(Or1, Or2, Stripe->OrientationOnFace1(), Stripe->OrientationOnFace2(), Stripe->Choix());
            if (Simul) {
                isdone = SimulSurf(SD, HGuide, Spine, Choix, HSon1, It1, HSon2, It2, tolesp, First, Last, Inside,
                                   Inside, forward, RecOnS1, RecOnS2, Soldep, intf, intl);
            } else {
                ...
                isdone = PerformSurf(SeqSD, HGuide, Spine, Choix, HSon1, It1, HSon2, It2, MaxStep, Fleche, tolesp,
                                     First, Last, Inside, Inside, forward, RecOnS1, RecOnS2, Soldep, intf, intl);
#ifdef OCCT_DEBUG
                ChFi3d_ResultChron(ch1, t_performsurf); // result perf for PerformSurf
#endif
            }
...
Standard_Boolean ChFi3d_Builder::StripeOrientations(const Handle(ChFiDS_Spine) & Spine, TopAbs_Orientation& Or1,
...
void ChFi3d_Builder::ConexFaces(const Handle(ChFiDS_Spine) & Spine, const Standard_Integer IEdge,
...
void ChFi3d_Builder::StartSol(const Handle(ChFiDS_Stripe) & Stripe, const Handle(ChFiDS_ElSpine) & HGuide,
    ...
    Handle(Geom2d_Curve) PC;
    Extrema_ExtPC PExt;
    PExt.Initialize(els, Spine->FirstParameter(1), Spine->LastParameter(nbed), Precision::Confusion());
    ...
    for (nbessai = 0; nbessai <= nbessaimax; nbessai++) {
        ...
        if (iedge != ie) {
            ...
            Or1 = f1.Orientation();
            Or2 = f2.Orientation();
            Choix = ChFi3d::NextSide(Or1, Or2, Stripe->OrientationOnFace1(), Stripe->OrientationOnFace2(), RC);
        }
    ...
    for (nbessai = 0; nbessai <= nbessaimax; nbessai++) {
        ...
        if (Projection(PExt, pnt, els, w, tolesp)) {
            ...
            for (NbChangement = 1; bonvoisin && (!found) && (NbChangement < 5); NbChangement++) {
                if (Pos1 != TopAbs_IN) {
                    bonvoisin = BonVoisin(P, HS1, f1, plane, cured, SolDep(1), SolDep(2), myEFMap, tolesp);
                }
                if (Pos2 != TopAbs_IN && bonvoisin) {
                    bonvoisin = BonVoisin(P, HS2, f2, plane, cured, SolDep(3), SolDep(4), myEFMap, tolesp);
                }
                if (bonvoisin) {
                    f1 = HS1->Face();
                    f2 = HS2->Face();
...
static void ChFi3d_BuildPlane(TopOpeBRepDS_DataStructure& DStr, Handle(BRepAdaptor_Surface) & HS, gp_Pnt2d& pons,
...
Standard_Boolean ChFi3d_Builder::StartSol(const Handle(ChFiDS_Spine) & Spine,
                                          ...
                                          Standard_Real& W, const Handle(ChFiDS_SurfData) & SD,
                                          const Standard_Boolean isfirst, const Standard_Integer ons,
                                          Handle(BRepAdaptor_Surface) & HSref, // The other representation
                                          Handle(BRepAdaptor_Curve2d) & HCref, // of the obstacle
                                          Standard_Boolean& RecP, Standard_Boolean& RecS, Standard_Boolean& RecRst,
                                          Standard_Boolean& c1obstacle,
                                          Handle(BRepAdaptor_Surface) & HSBis, // Face of support
                                          gp_Pnt2d& PBis,                      // and its point
                                          const Standard_Boolean decroch, const TopoDS_Vertex& Vref) const {
...
Standard_Boolean ChFi3d_Builder::SearchFace(const Handle(ChFiDS_Spine) & Spine, const ChFiDS_CommonPoint& Pc,
...
//=======================================================================
static void ChFi3d_SingularExtremity(Handle(ChFiDS_Stripe) & stripe, TopOpeBRepDS_DataStructure& DStr,
                                     const TopoDS_Vertex& Vtx, const Standard_Real tol3d, const Standard_Real tol2d) {
    ...
    if (CV1.Point().IsEqual(CV2.Point(), 0)) {
        ...
        ChFi3d_ComputeArete(CV1, VOnS1, CV2, VOnS2, DStr.Surface(Fd->Surf()).Surface(), C3d, PCurv, Pardeb, Parfin,
...
static void ChFi3d_MakeExtremities(Handle(ChFiDS_Stripe) & Stripe, TopOpeBRepDS_DataStructure& DStr,
                                   const ChFiDS_Map& EFMap, const Standard_Real tol3d, const Standard_Real tol2d) {
    ...
    if (sp->IsPeriodic()) {
        ...
        if (!CV1.Point().IsEqual(CV2.Point(), 0)) {
            ChFi3d_ComputeArete(
                CV1, SDF->InterferenceOnS1().PCurveOnSurf()->Value(SDF->InterferenceOnS1().FirstParameter()), CV2,
                SDF->InterferenceOnS2().PCurveOnSurf()->Value(SDF->InterferenceOnS2().FirstParameter()),
                DStr.Surface(SDF->Surf()).Surface(), C3d, Stripe->ChangeFirstPCurve(), Pardeb, Parfin, tol3d, tol2d,
                tolreached, 0);
            ...
            ChFi3d_ComputePCurv(
                C3d, SDL->InterferenceOnS1().PCurveOnSurf()->Value(SDL->InterferenceOnS1().LastParameter()),
                SDL->InterferenceOnS2().PCurveOnSurf()->Value(SDL->InterferenceOnS2().LastParameter()),
                Stripe->ChangeLastPCurve(), DStr.Surface(SDL->Surf()).Surface(), Pardeb, Parfin, tol3d, tolreached);
            ...
            if (!CV1.IsVertex()) ChFi3d_SetPointTolerance(DStr, b1, Stripe->IndexFirstPointOnS1());
            if (!CV2.IsVertex()) ChFi3d_SetPointTolerance(DStr, b2, Stripe->IndexFirstPointOnS2());
        } else {
    ...
    if (freedeb) {
        ...
        if (!cpdeb1.Point().IsEqual(cpdeb2.Point(), 0)) {
            ...
            gp_Pnt2d UV1, UV2;
            UV1 = SDdeb->InterferenceOnS1().PCurveOnSurf()->Value(SDdeb->InterferenceOnS1().FirstParameter());
            UV2 = SDdeb->InterferenceOnS2().PCurveOnSurf()->Value(SDdeb->InterferenceOnS2().FirstParameter());
            // The intersection of the fillet by a plane is attempted

            Handle(GeomAdaptor_Surface) HConge = ChFi3d_BoundSurf(DStr, SDdeb, 1, 2);
            ChFi3d_CoupeParPlan(cpdeb1, cpdeb2, HConge, UV1, UV2, tol3d, tol2d, C3d, Stripe->ChangeFirstPCurve(),
                                tolreached, Pardeb, Parfin, plane);
            if (!plane)
                ChFi3d_ComputeArete(
                    cpdeb1, SDdeb->InterferenceOnS1().PCurveOnSurf()->Value(SDdeb->InterferenceOnS1().FirstParameter()),
                    cpdeb2, SDdeb->InterferenceOnS2().PCurveOnSurf()->Value(SDdeb->InterferenceOnS2().FirstParameter()),
                    DStr.Surface(SDdeb->Surf()).Surface(), C3d, Stripe->ChangeFirstPCurve(), Pardeb, Parfin, tol3d,
                    tol2d, tolreached, 0);
            TopOpeBRepDS_Curve Crv(C3d, tolreached);
            Stripe->ChangeFirstCurve(DStr.AddCurve(Crv));
            Stripe->ChangeFirstParameters(Pardeb, Parfin);
            ...
            if (!cpdeb1.IsVertex()) ChFi3d_SetPointTolerance(DStr, b1, Stripe->IndexFirstPointOnS1());
            if (!cpdeb2.IsVertex()) ChFi3d_SetPointTolerance(DStr, b2, Stripe->IndexFirstPointOnS2());
    ...
    if (freefin) {
        ...
        if (!cpfin1.Point().IsEqual(cpfin2.Point(), 0)) {
            Standard_Boolean plane;
            gp_Pnt2d UV1, UV2;
            UV1 = SDfin->InterferenceOnS1().PCurveOnSurf()->Value(SDfin->InterferenceOnS1().LastParameter());
            UV2 = SDfin->InterferenceOnS2().PCurveOnSurf()->Value(SDfin->InterferenceOnS2().LastParameter());
            // Intersection of the fillet by a plane is attempted

            Handle(GeomAdaptor_Surface) HConge = ChFi3d_BoundSurf(DStr, SDfin, 1, 2);
            ChFi3d_CoupeParPlan(cpfin1, cpfin2, HConge, UV1, UV2, tol3d, tol2d, C3d, Stripe->ChangeLastPCurve(),
                                tolreached, Pardeb, Parfin, plane);
            if (!plane)
                ChFi3d_ComputeArete(
                    cpfin1, SDfin->InterferenceOnS1().PCurveOnSurf()->Value(SDfin->InterferenceOnS1().LastParameter()),
                    cpfin2, SDfin->InterferenceOnS2().PCurveOnSurf()->Value(SDfin->InterferenceOnS2().LastParameter()),
                    DStr.Surface(SDfin->Surf()).Surface(), C3d, Stripe->ChangeLastPCurve(), Pardeb, Parfin, tol3d,
                    tol2d, tolreached, 0);
            TopOpeBRepDS_Curve Crv(C3d, tolreached);
            ...
            if (!cpfin1.IsVertex()) ChFi3d_SetPointTolerance(DStr, b1, Stripe->IndexLastPointOnS1());
            if (!cpfin2.IsVertex()) ChFi3d_SetPointTolerance(DStr, b2, Stripe->IndexLastPointOnS2());
...
static void ChFi3d_Purge(Handle(ChFiDS_Stripe) & Stripe, Handle(ChFiDS_SurfData) & SD, const ChFiDS_CommonPoint& VRef,
                         const Standard_Boolean isfirst, const Standard_Integer ons, Standard_Integer& intf,
...
static void InsertAfter(Handle(ChFiDS_Stripe) & Stripe, Handle(ChFiDS_SurfData) & Ref, Handle(ChFiDS_SurfData) & Item) {
    if (Ref == Item) throw Standard_Failure("InsertAfter : twice the same surfdata.");
...
static void RemoveSD(Handle(ChFiDS_Stripe) & Stripe, Handle(ChFiDS_SurfData) & Prev, Handle(ChFiDS_SurfData) & Next) {
    ChFiDS_SequenceOfSurfData& Seq = Stripe->ChangeSetOfSurfData()->ChangeSequence();
...
static void InsertBefore(Handle(ChFiDS_Stripe) & Stripe, Handle(ChFiDS_SurfData) & Ref,
                         Handle(ChFiDS_SurfData) & Item) {
    if (Ref == Item) throw Standard_Failure("InsertBefore : twice the same surfdata.");
...
void ChFi3d_Builder::PerformSetOfSurfOnElSpine(const Handle(ChFiDS_ElSpine) & HGuide, Handle(ChFiDS_Stripe) & Stripe,
                                               Handle(BRepTopAdaptor_TopolTool) & It1,
                                               Handle(BRepTopAdaptor_TopolTool) & It2, const Standard_Boolean Simul) {
#ifdef OCCT_DEBUG
    OSD_Chronometer ch1;
#endif
    ...
    if (Spine->Mode() == ChFiDS_ConstThroatWithPenetrationChamfer) {
        ...
        for (; ILES.More(); ILES.Next(), ILES_offset.Next()) {
            ...
            if (aHElSpine == HGuide) OffsetHGuide = ILES_offset.Value();
        }
    }
    ...
    if (ref.IsNull() && raf.IsNull()) {
        ...
        Ok1 = StartSol(Spine, HS1, pp1, HC1, w1, ref, !forward, 1, HSref1, HCref1, RecP1, RecS1, RecRst1, obstacleon1,
                       HS3, pp3, decroch1, Vref);
        Ok2 = StartSol(Spine, HS2, pp2, HC2, w2, ref, !forward, 2, HSref2, HCref2, RecP2, RecS2, RecRst2, obstacleon2,
        ...
        if (Ok1 == 1 && Ok2 == 1) {
            if (forward) {
                ...
                if (!OffsetHGuide.IsNull()) OffsetHGuide->FirstParameter(wf);
            } else {
                Guide.LastParameter(wl);
                if (!OffsetHGuide.IsNull()) OffsetHGuide->LastParameter(wl);
            }
        }
    }
    ...
    while (!fini) {
        ...
        if (!Spine->IsPeriodic()) {
            ...
            if (Spine->IsTangencyExtremity(Standard_True)) {
                ...
                if (!OffsetHGuide.IsNull()) OffsetHGuide->FirstParameter(wfsav);
            }
            ...
            if (Spine->IsTangencyExtremity(Standard_False)) {
                ...
                if (!OffsetHGuide.IsNull()) OffsetHGuide->LastParameter(wlsav);
            }
        }
        ...
        if (!ref.IsNull()) {
            ...
            Ok1 = StartSol(Spine, HS1, pp1, HC1, w1, ref, !forward, 1, HSref1, HCref1, RecP1, RecS1, RecRst1,
                           obstacleon1, HS3, pp3, decroch1, Vref);
            Ok2 = StartSol(Spine, HS2, pp2, HC2, w2, ref, !forward, 2, HSref2, HCref2, RecP2, RecS2, RecRst2,
                           obstacleon2, HS4, pp4, decroch2, Vref);
        ...
        if (obstacleon1 && obstacleon2) {
            ...
            SoldepCC(2) = w2;
            if (Simul) {
                SimulSurf(SD, HGuide, Spine, Choix, HS1, It1, HC1, HSref1, HCref1, decroch1, Or1, HS2, It2, HC2, HSref2,
                          HCref2, decroch2, Or2, locfleche, tolesp, First, Last, Inside, Inside, forward, RecP1,
                          RecRst1, RecP2, RecRst2, SoldepCC);
            } else {
#ifdef OCCT_DEBUG
                ChFi3d_InitChron(ch1); // init perf for PerformSurf
#endif
                PerformSurf(SeqSD, HGuide, Spine, Choix, HS1, It1, HC1, HSref1, HCref1, decroch1, Or1, HS2, It2, HC2,
                            HSref2, HCref2, decroch2, Or2, MaxStep, locfleche, tolesp, First, Last, Inside, Inside,
                            forward, RecP1, RecRst1, RecP2, RecRst2, SoldepCC);
#ifdef OCCT_DEBUG
                ChFi3d_ResultChron(ch1, t_performsurf); // result  perf for PerformSurf
#endif
            }
            ...
            TopAbs_Orientation Or1 = HSref1->Face().Orientation();
            TopAbs_Orientation Or2 = HS2->Face().Orientation();
            Standard_Integer Choix = ChFi3d::NextSide(Or1, Or2, Stripe->OrientationOnFace1(),
                                                      Stripe->OrientationOnFace2(), -Stripe->Choix());
            if (Choix % 2 == 1)
                Choix++;
            else
                Choix--;
            ...
            SoldepCS(2) = pp2.Y();
            if (Simul) {
                SimulSurf(SD, HGuide, Spine, Choix, HS1, It1, HC1, HSref1, HCref1, decroch1, HS2, It2, Or2, locfleche,
                          tolesp, First, Last, Inside, Inside, forward, RecP1, RecS2, RecRst1, SoldepCS);
            } else {
#ifdef OCCT_DEBUG
                ChFi3d_InitChron(ch1); // init perf for PerformSurf
#endif
                PerformSurf(SeqSD, HGuide, Spine, Choix, HS1, It1, HC1, HSref1, HCref1, decroch1, HS2, It2, Or2,
                            MaxStep, locfleche, tolesp, First, Last, Inside, Inside, forward, RecP1, RecS2, RecRst1,
                            SoldepCS);
#ifdef OCCT_DEBUG
                ChFi3d_ResultChron(ch1, t_performsurf); // result  perf for PerformSurf
#endif
            ...
            SoldepCS(2) = pp1.Y();
            if (Simul) {
                SimulSurf(SD, HGuide, Spine, Choix, HS1, It1, Or1, HS2, It2, HC2, HSref2, HCref2, decroch2, locfleche,
                          tolesp, First, Last, Inside, Inside, forward, RecP2, RecS1, RecRst2, SoldepCS);
            } else {
#ifdef OCCT_DEBUG
                ChFi3d_InitChron(ch1); // init perf for PerformSurf
#endif
                PerformSurf(SeqSD, HGuide, Spine, Choix, HS1, It1, Or1, HS2, It2, HC2, HSref2, HCref2, decroch2,
                            MaxStep, locfleche, tolesp, First, Last, Inside, Inside, forward, RecP2, RecS1, RecRst2,
                            SoldepCS);
#ifdef OCCT_DEBUG
                ChFi3d_ResultChron(ch1, t_performsurf); // result  perf for PerformSurf
#endif
            ...
            CallPerformSurf(Stripe, Simul, SeqSD, SD, HGuide, Spine, HS1, HS3, pp1, pp3, aTT1, HS2, HS4, pp2, pp4, aTT2,
                            MaxStep, locfleche, tolesp, First, Last, Inside, Inside, forward, RecS1, RecS2, Soldep,
                            intf, intl, HS1, HS2);
            decroch1 = decroch2 = 0;
        }
        ...
        if (forward) {
            fini = ((wl - Last) <= 10. * tolesp || (intl && !(obstacleon1 || obstacleon2))); // General case
    ...
    if (!Simul) {
        ...
        for (i = 1; i <= len; i++) {
            ...
            if (tw1) {
                ...
                if (samesurfon1) {
                    ...
                    if (ChFi3d_IntTraces(prevsd, prevpar1, nprevpar1, 1, 1, nextsd, nextpar1, nnextpar1, 1, -1, p2d,
                        ...
                        Bon1 = ChFi3d_mkbound(S1, PC1, -1, pdeb1, vdeb1, 1, pfin1, vfin1, tolesp, 2.e-4);
                    }
                } else {
            ...
            if (tw2) {
                ...
                if (samesurfon2) {
                    ...
                    if (ChFi3d_IntTraces(prevsd, prevpar2, nprevpar2, 2, 1, nextsd, nextpar2, nnextpar2, 2, -1, p2d,
                                         Standard_False, Standard_True)) {
                        //  Modified by Sergey KHROMOV - Wed Feb  5 12:03:17 2003 End
                        ...
                        Bon2 = ChFi3d_mkbound(S2, PC2, -1, pdeb2, vdeb2, 1, pfin2, vfin2, tolesp, 2.e-4);
                    }
                } else {
                    // here the base is on 3D tangents of neighbors.
        ...
        for (i = 1; i < last; i++) {
            ...
            if (!curp1.IsOnArc() && nextp1.IsOnArc()) {
                ...
                nextp1 = curp1;
                if ((curs1 == nexts1) && !cursd->IsOnCurve1()) ChangeTransition(curp1, nextp1, curs1, myDS);
            }

            if (!curp2.IsOnArc() && nextp2.IsOnArc()) {
                curp2 = nextp2;
                if ((curs2 == nexts2) && !nextsd->IsOnCurve2()) ChangeTransition(nextp2, curp2, curs2, myDS);
            } else if (curp2.IsOnArc() && !nextp2.IsOnArc()) {
                nextp2 = curp2;
                if ((curs2 == nexts2) && !cursd->IsOnCurve2()) ChangeTransition(curp2, nextp2, curs2, myDS);
            }
