...
extern Standard_Real t_t2cornerinit, t_perf2cornerbyinter, t_chfikpartcompdata, t_cheminement, t_remplissage,
    t_t2cornerDS;
...
static Standard_Boolean ToricRotule(const BRepAdaptor_Surface& fac, const BRepAdaptor_Surface& s1,
                                    const BRepAdaptor_Surface& s2, const Handle(ChFiDS_Stripe) & c1,
...

{
    ...
    if (!sp1->IsConstant() || !sp2->IsConstant()) return Standard_False;
    if ((fac.GetType() != GeomAbs_Plane) || (s1.GetType() != GeomAbs_Plane) || (s2.GetType() != GeomAbs_Plane))
        return Standard_False;
    gp_Dir df = fac.Plane().Position().Direction();
    gp_Dir ds1 = s1.Plane().Position().Direction();
    gp_Dir ds2 = s2.Plane().Position().Direction();
    if (Abs(df.Dot(ds1)) >= tolesp || Abs(df.Dot(ds2)) >= tolesp) return Standard_False;
...
void ChFi3d_FilBuilder::PerformTwoCorner(const Standard_Integer Index) {
    ...
    OkinterCC = ChFi3d_IsInFront(DStr, st1, st2, Isd1, Isd2, Sens1, Sens2, UIntPC1, UIntPC2, FaCo, SameSide, IFaCo1,
                                 IFaCo2, Okvisavis, Vtx, Standard_True);
    ...
    if (!Okvisavis) {
        ...
        for (i1 = deb1; i1 <= fin1 && !trouve; i1++) {
            ...
            for (i2 = deb2; i2 <= fin2 && !trouve; i2++) {
                ...
                OkinterCC = ChFi3d_IsInFront(DStr, st1, st2, Isd1, Isd2, Sens1, Sens2, UIntPC1, UIntPC2, FaCo, SameSide,
        ...
        if (SameSide) {
            if (ToricRotule(BRFaCo, BRS1, BRS2, st1, st2)) {
                ...
                Standard_Real radius = Handle(ChFiDS_FilSpine)::DownCast(st1->Spine())->Radius();
                    ...
                    ChFiKPart_ComputeData::ComputeCorner(DStr, coin, HFaCo, HBRS1, HBRS2, OFaCo, ori, op1, op2, radius);
#ifdef OCCT_DEBUG
                ...
                Bfac = ChFi3d_mkbound(HFaCo, PCurveOnFace, Sens1, p2dfac1, v2dfac1, Sens2, p2dfac2, v2dfac2, tolesp,
                                      2.e-4);
                ...
                if (sameparam) {
                    ...
                    if (pcpivot.GetType() != GeomAbs_BSplineCurve) {
                        ...
                        PCurveOnPiv =
                            Geom2dConvert::SplitBSplineCurve(Handle(Geom2d_BSplineCurve)::DownCast(pcpivot.Curve()),
                ...
                done = CompleteData(coin, Surfcoin, HFaCo, PCurveOnFace, HFaPiv, PCurveOnPiv, OFaCo, 1, 0, 0, 0, 0);
#ifdef OCCT_DEBUG
            ...
            if (done) {
                ...
                Standard_Real tolr1;
                ChFi3d_ComputePCurv(C3d, pp1, pp2, st1->ChangePCurve(isfirst1), DStr.Surface(sd1->Surf()).Surface(),
                ...
                Standard_Real tolr2;
                ChFi3d_ComputePCurv(C3d, pp1, pp2, st2->ChangePCurve(isfirst2), DStr.Surface(sd2->Surf()).Surface(),
            ...
            const Handle(Geom_Curve) & curvopsam = DStr.Curve(sdsam->Interference(ifaopsam).LineIndex()).Curve();
            ...
            Handle(Geom2d_Curve) pcFopsam = ChFi3d_BuildPCurve(HBRFopsam, ppfacsam, VVfacsam, ppfacdif, VVfacdif, 1);
	...
	fil.CheckResult(ib);
            ...
            done = CompleteData(coin, Surfcoin, Hsurfsam, pcsurfsam, HBRFopsam, pcnul, Osurfsam, 1, 0, 0, 0, 0);
#ifdef OCCT_DEBUG
            ChFi3d_ResultChron(ch, t_remplissage); // result perf filling
#endif
