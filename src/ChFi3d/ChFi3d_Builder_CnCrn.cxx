...
static void Indices(const Standard_Integer n, const Standard_Integer ic, Standard_Integer& icplus,
...
static void Calcul_Param(const Handle(ChFiDS_Stripe) & stripe, const Standard_Integer jfposit,
                         const Standard_Integer indice, const Standard_Boolean isfirst, Standard_Real& param) {
    if (jfposit == 2)
...
static void Calcul_P2dOnSurf(const Handle(ChFiDS_Stripe) & stripe, const Standard_Integer jfposit,
                             const Standard_Integer indice, const Standard_Real param, gp_Pnt2d& p2)
...
static void Calcul_C2dOnFace(const Handle(ChFiDS_Stripe) & stripe, const Standard_Integer jfposit,
                             const Standard_Integer indice, Handle(Geom2d_Curve) & c2d)
...
static void Calcul_Orientation(const Handle(ChFiDS_Stripe) & stripe, const Standard_Integer jfposit,
                               const Standard_Integer indice, TopAbs_Orientation& orient) {
    if (jfposit == 1)
...
static void RemoveSD(Handle(ChFiDS_Stripe) & Stripe, const Standard_Integer num1, const Standard_Integer num2) {
...
static void CurveHermite(const TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_Stripe) & CDicmoins,
                         const Standard_Integer jficmoins, const Standard_Integer icmoins, const Standard_Real picmoins,
                         const Standard_Integer sensicmoins, const Standard_Boolean sharpicmoins,
                         const TopoDS_Edge& Eviveicmoins, const Handle(ChFiDS_Stripe) & CDicplus,
                         const Standard_Integer jficplus, const Standard_Integer icplus, const Standard_Real picplus,
                         const Standard_Integer sensicplus, const Standard_Boolean sharpicplus,
                         const TopoDS_Edge& Eviveicplus, const Standard_Integer nbface, TopTools_SequenceOfShape& Ecom,
                         const TopTools_SequenceOfShape& Face, TColGeom2d_SequenceOfCurve& proj2d,
                         TColGeom_SequenceOfCurve& cproj, TopTools_SequenceOfShape& Eproj,
                         TColStd_SequenceOfReal& param, Standard_Real& error) {
    gp_Pnt p01, p02;
    gp_Vec d11, d12;
    ...
    if (c1.IsNull()) throw Standard_ConstructionError("Failed to get 3D curve of edge");
    if (c2.IsNull()) throw Standard_ConstructionError("Failed to get 3D curve of edge");
        ...
        if (OrtProj.IsDone()) {
            ...
            if (MapE1.Extent() != 0) {
                if (MapE1.Extent() != 1) {
                    ...
                    OrtProj.Build();
                    MapE1.Clear();
                    if (OrtProj.IsDone()) TopExp::MapShapes(OrtProj.Projection(), TopAbs_EDGE, MapE1);
                }
                if (MapE1.Extent() != 0) {
                    Standard_Boolean trouve = Standard_False;
                    ...
                    if (proj1.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                    proj2d.Append(new Geom2d_TrimmedCurve(proj1, up1, up2));
                    proj1c = BRep_Tool::Curve(E1, up1, up2);
                    if (proj1c.IsNull()) throw Standard_ConstructionError("Failed to get 3D curve of edge");
...
static void CalculDroite(const gp_Pnt2d& p2d1, const Standard_Real xdir, const Standard_Real ydir,
...
static void CalculBatten(const Handle(GeomAdaptor_Surface) ASurf, const TopoDS_Face Face, const Standard_Real xdir,
                         const Standard_Real ydir, const gp_Pnt2d& p2d1, const gp_Pnt2d& p2d2,
                         const Standard_Boolean contraint1, const Standard_Boolean contraint2,
                         Handle(Geom2d_Curve) & curv2d1, Handle(Geom2d_Curve) & curv2d2, const Standard_Real picicplus,
                         const Standard_Real picplusic, const Standard_Boolean inverseic,
                         const Standard_Boolean inverseicplus, Handle(Geom2d_Curve) & pcurve) {
    ...
    if (isplane && (Abs(ang1) > M_PI / 2 || Abs(ang2) > M_PI / 2)) isplane = Standard_False;
    if (anglebig && !isplane) {
...
static void OrientationIcNonVive(const Handle(ChFiDS_Stripe) & CDic, const Standard_Integer jfic,
                                 const Standard_Integer icicplus, const Standard_Integer sensic,
...
static void OrientationIcplusNonVive(const Handle(ChFiDS_Stripe) & CDicplus, const Standard_Integer jficplus,
                                     const Standard_Integer icplusic, const Standard_Integer sensicplus,
                                     TopAbs_Orientation& orien) {
    TopAbs_Orientation orinterf;
...
static void OrientationAreteViveConsecutive(const TopoDS_Shape& Fviveicicplus, const TopoDS_Shape& Eviveic,
...
static void PerformTwoCornerSameExt(TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_Stripe) & stripe1,
                                    const Standard_Integer index1, const Standard_Integer sens1,
                                    const Handle(ChFiDS_Stripe) & stripe2, const Standard_Integer index2,
                                    const Standard_Integer sens2, Standard_Boolean& trouve)
...
//    stripe2->SetOfSurfData()->Value(index2)->ChangeVertex (isfirst,2);
    ...
    if (trouve) {
        ...
        const Handle(Geom_Surface) Stemp = BRep_Tool::Surface(TopoDS::Face(DStr.Shape(Fd1->IndexOfS1())));
...
static void CpOnEdge(const Handle(ChFiDS_Stripe) & stripe, Standard_Integer num, Standard_Boolean isfirst,
                     const TopoDS_Edge& Eadj1, const TopoDS_Edge& Eadj2, Standard_Boolean& compoint)

{
    ...
    if (cp1.IsOnArc()) {
        if (cp1.Arc().IsSame(Eadj1) || cp1.Arc().IsSame(Eadj2)) compoint = Standard_True;
    }
    if (cp2.IsOnArc()) {
        if (cp2.Arc().IsSame(Eadj1) || cp2.Arc().IsSame(Eadj2)) compoint = Standard_True;
    }
}
...
static void RemoveSurfData(const ChFiDS_StripeMap& myVDataMap, const ChFiDS_Map& myEFMap,
                           const TopoDS_Edge& edgecouture, const TopoDS_Face& facecouture, const TopoDS_Vertex& V1) {
...
static void ParametrePlate(const Standard_Integer n3d, const GeomPlate_BuildPlateSurface& PSurf,
                           const Handle(Geom_Surface) & Surf, const gp_Pnt& point, const Standard_Real apperror,
                           gp_Pnt2d& uv) {
    Standard_Integer ip;
    gp_Pnt P1;
    Standard_Real par;
...
static void SummarizeNormal(const TopoDS_Vertex& V1, const TopoDS_Face& Fcur, const TopoDS_Edge& Ecur,
...
static Standard_Integer SurfIndex(const ChFiDS_StripeArray1& StripeArray1, const Standard_Integer StripeIndex,
                                  const Standard_Integer SurfDataIndex, const ChFi3d_SurfType SurfType) {
    const Handle(ChFiDS_SurfData) & aSurfData = StripeArray1(StripeIndex)->SetOfSurfData()->Value(SurfDataIndex);
    switch (SurfType) {
        case ChFiSURFACE:
...
void ChFi3d_Builder::PerformMoreThreeCorner(const Standard_Integer Jndex, const Standard_Integer nconges) {
//    ========================================
//             Initialisations
//     ========================================
#ifdef OCCT_DEBUG
...
//  Standard_Integer nface=ChFi3d_nbface(myVFMap(V1));
        ...
        for (ic = 0; ic < nedge; ic++) {
            if (sharp.Value(ic)) {
                ...
                if (!sharp.Value(icplus)) {
                    ...
                    cp1 = CD.Value(icplus)->SetOfSurfData()->Value(Index.Value(icplus))->ChangeVertex(isfirst, jfp);
                    if (cp1.IsOnArc()) {
    ...
    for (ic = 0; ic < nedge; ic++) {
        ...
        if (sharp.Value(ic) || sharp.Value(icplus)) {
            ...
            if (Abs(angedg - M_PI) > 0.01)
                ok = ChFi3d_SearchFD(DStr, CD.Value(ic), CD.Value(icplus), sens.Value(ic), sens.Value(icplus), i1, i2,
                                     pa1, pa2, Index.Value(ic), Index.Value(icplus), face, sameside, jf1, jfp);
            else
                ok = Standard_False;
        ...
        if (!deuxconges)
            for (ic = 0; ic < nedge; ic++) {
                ...
                if (sharp.Value(ic)) {
                    ...
                    if (dist2.Value(ic) > distance) distance = dist2.Value(ic);
                }
            }
        ...
        for (ic = 0; ic < nedge; ic++) {
            if (sharp.Value(ic)) {
                ...
                if (!sharp.Value(icplus)) {
                    isfirst = (sens.Value(icplus) == 1);
                    jfp = 3 - jf.Value(icplus);
                    cp1 = CD.Value(icplus)->SetOfSurfData()->Value(i.Value(icplus, ic))->ChangeVertex(isfirst, jfp);
    ...
    for (ic = 0; ic < nedge; ic++) {
        if (TopoDS::Edge(Evive.Value(ic)).IsSame(edgelibre1) || TopoDS::Edge(Evive.Value(ic)).IsSame(edgelibre2)) {
    ...
    if (!deuxconges)
        for (ic = 0; ic < nedge; ic++) {
            ...
            if (regul.Value(ic)) {
                ...
                CurveHermite(DStr, CD.Value(icmoins), jf.Value(icmoins), i.Value(icmoins, ic), p.Value(icmoins, ic),
                             sens.Value(icmoins), sharp.Value(icmoins), TopoDS::Edge(Evive.Value(icmoins)),
                             CD.Value(indfin), jf.Value(indfin), i.Value(indfin, indfinmoins),
                             p.Value(indfin, indfinmoins), sens.Value(indfin), sharp.Value(indfin),
                             TopoDS::Edge(Evive.Value(indfin)), nbface, Ledge, Lface, pr, cr, Epj, seqpr, error);
                ii = ic;
        ...
        for (ic = 0; ic < nedge; ic++) {
            ...
            if (!oksea.Value(ic)) {
                ...
                if (!face1.IsSame(face2)) {
                    ...
                    CurveHermite(DStr, CD.Value(ic), jf.Value(ic), i.Value(ic, icplus), p.Value(ic, icplus),
                                 sens.Value(ic), sharp.Value(ic), TopoDS::Edge(Evive.Value(ic)), CD.Value(icplus),
                                 jf.Value(icplus), i.Value(icplus, ic), p.Value(icplus, ic), sens.Value(icplus),
                                 sharp.Value(icplus), TopoDS::Edge(Evive.Value(icplus)), nbface, Ecom, Fproj, pr, cr,
                                 Eproj, parcom, error);
    ...
    if (nconges == 2 && !deuxconges) {
        ...
        if (intersection) {
            PerformTwoCornerSameExt(DStr, CD.Value(ic1), Index.Value(ic1), sens.Value(ic1), CD.Value(ic2),
                                    Index.Value(ic2), sens.Value(ic2), introuve);
            if (introuve) return;
        }
    }
    ...
    for (ic = 0; ic < nedge; ic++) {
        ...
        if (!sharp.Value(ic)) {
            ...
            isfirst = (sens.Value(ic) == 1);
            GeomLib::BuildCurve3d(tolapp, CurvOnS, CurvOnS.FirstParameter(), CurvOnS.LastParameter(), Curv3d, maxapp,
                                  avedev);
            ...
            if (ic == 0) {
                ...
                for (ii = 0; ii < ic && (!trouve); ii++) {
                    if (!sharp.Value(ii)) {
                        ...
                        if (point1.Distance(tpt.Point()) < 1.e-4) trouve = Standard_True;
                    }
                }
                if (trouve)
                ...
                for (ii = 0; ii < ic && (!trouve); ii++) {
                    if (!sharp.Value(ii)) {
                        ...
                        if (point2.Distance(tpt.Point()) < 1.e-4) trouve = Standard_True;
                    }
                }
                if (trouve)
            ...
            cp1.SetPoint(point1);
            cp2.SetPoint(point2);
            CD.Value(ic)->SetOfSurfData()->Value(i.Value(ic, icmoins))->ChangeVertex(isfirst, isurf1) = cp1;
            CD.Value(ic)->SetOfSurfData()->Value(i.Value(ic, icmoins))->ChangeVertex(isfirst, isurf2) = cp2;
            CD.Value(ic)
                ->SetOfSurfData()
    ...
    for (ic = 0; ic < nedge; ic++) {
        if (sharp.Value(ic)) {
            ...
            if (!sharp.Value(icplus)) {
                isfirst = (sens.Value(icplus) == 1);
                jfp = 3 - jf.Value(icplus);
                cp = CD.Value(icplus)->SetOfSurfData()->Value(i.Value(icplus, ic))->ChangeVertex(isfirst, jfp);
        ...
        for (ic = 0; ic < nedge; ic++) {
            ...
            if (!oksea.Value(ic) && !moresurf.Value(ic) && !libre.Value(ic)) {
                ...
                if (!regul.Value(ic)) {
                    ...
                    if (regul.Value(icplus)) raccordbatten = Standard_False;
                }
                n3d++;
                gp_Pnt2d p2d1, p2d2;
                ...
                if (curv2d1.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                ...
                if (curv2d2.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                ...
                if (l0 < 1.e-7 || ponctuel.Value(ic)) {
                    ...
                    if (raccordbatten) {
                        ...
                        CalculBatten(Asurf, TopoDS::Face(Fvive(ic, icplus)), xdir, ydir, p2d1, p2d2, contraint1,
                                     contraint2, curv2d1, curv2d2, p.Value(ic, icplus), p.Value(icplus, ic), inverseic,
                                     inverseicplus, pcurve);
#ifdef OCCT_DEBUG
                    ...
                    if (!sharp.Value(ic) && !sharp.Value(icplus)) Order.SetValue(n3d, 1);
                    if (!contraint1 && !sharp.Value(icplus)) Order.SetValue(n3d, 1);
                    if (!contraint2 && !sharp.Value(ic)) Order.SetValue(n3d, 1);
                    if (tangentregul(ic) || tangentregul(icplus)) Order.SetValue(n3d, 1);
                    if (isG1.Value(ic)) Order.SetValue(n3d, 1);
                    Handle(GeomPlate_CurveConstraint) Cont =
                    ...
                    if (curveint.IsNull()) {
                        GeomLib::BuildCurve3d(tolapp, CurvOnS, CurvOnS.FirstParameter(), CurvOnS.LastParameter(),
                                              Curv3d, maxapp1, avedev);
                    ...
                    Interfp1 = ChFi3d_FilPointInDS(TopAbs_FORWARD, indcurve3d.Value(n3d), indpoint.Value(ic, 1), pardeb,
                                                   IsVt1);
                    Interfp2 =
                        ChFi3d_FilPointInDS(TopAbs_REVERSED, indcurve3d.Value(n3d), indpoint(icplus, 0), parfin, IsVt2);
                    ...
                    if (!sharp.Value(ic)) {
                        ...
                        OrientationIcplusNonVive(CD.Value(icplus), jf.Value(icplus), i.Value(icplus, ic),
                                                 sens.Value(icplus), orinterf);
                    } else {
    ...
    for (ic = 0; ic < nedge; ic++) {
        if (moresurf.Value(ic)) {
            ...
            for (nb = 1; nb <= nbface; nb++) {
                ...
                if (Pf.Distance(BRep_Tool::Pnt(V1)) < Pl.Distance(BRep_Tool::Pnt(V1))) orvt = TopAbs_FORWARD;
                if (!Eproj.Value(nb).IsNull()) {
                    n3d++;
                    proj = BRep_Tool::CurveOnSurface(TopoDS::Edge(Eproj.Value(nb)), TopoDS::Face(Fproj.Value(nb)), up1,
                                                     up2);
                    if (proj.IsNull()) throw Standard_ConstructionError("Failed to get p-curve of edge");
                    proj2d = new Geom2d_TrimmedCurve(proj, up1, up2);
                    projc = BRep_Tool::Curve(TopoDS::Edge(Eproj.Value(nb)), up1, up2);
                    if (projc.IsNull()) throw Standard_ConstructionError("Failed to get 3D curve of edge");
    ...
    if (droit)
        for (ic = 0; ic < nedge; ic++) {
            ...
            if (ecur.IsSame(edgelibre1) || ecur.IsSame(edgelibre2)) {
                ...
                indcurve3d.SetValue(n3d, DStr.AddCurve(tcurv3d));
                Interfp1 = ChFi3d_FilPointInDS(TopAbs_FORWARD, indcurve3d.Value(n3d), indpoint1, ufirst, isvt1);
                Interfp2 = ChFi3d_FilPointInDS(TopAbs_REVERSED, indcurve3d.Value(n3d), indpoint2, ulast, isvt2);
                DStr.ChangeCurveInterferences(indcurve3d.Value(n3d)).Append(Interfp1);
                DStr.ChangeCurveInterferences(indcurve3d.Value(n3d)).Append(Interfp2);
            }
        }
...
//    Standard_Real ang1=PSurf.G1Error();
        ...
        Handle(TopOpeBRepDS_SolidSurfaceInterference) SSI = new TopOpeBRepDS_SolidSurfaceInterference(
            TopOpeBRepDS_Transition(orplate), TopOpeBRepDS_SOLID, SolInd, TopOpeBRepDS_SURFACE, Isurf);
        ...
        if (!droit)
            for (ic = 0; ic <= nedge; ic++) {
                if (libre.Value(ic)) {
                    ...
                    ChFi3d_ComputeArete(CP1, UV1, CP2, UV2, Surf, C3d, C2d, param1, param2, to3d, to2d, tolreached, 0);
        ...
        for (ic = 0; ic < nedge; ic++) {
            if (!sharp.Value(ic)) {
                ...
                Interfp1 = ChFi3d_FilPointInDS(TopAbs_FORWARD, indcurve3d.Value(n3d), indpoint.Value(ic, 0), pardeb);
                Interfp2 = ChFi3d_FilPointInDS(TopAbs_REVERSED, indcurve3d.Value(n3d), indpoint.Value(ic, 1), parfin);
                DStr.ChangeCurveInterferences(indcurve3d.Value(n3d)).Append(Interfp1);
                DStr.ChangeCurveInterferences(indcurve3d.Value(n3d)).Append(Interfp2);
                ...
                Interfc = ChFi3d_FilCurveInDS(indcurve3d.Value(n3d), Isurf, PSurf.Curves2d()->Value(n3d), orien);
                DStr.ChangeSurfaceInterferences(Isurf).Append(Interfc);
