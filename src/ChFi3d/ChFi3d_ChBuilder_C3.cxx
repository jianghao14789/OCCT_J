...
//=======================================================================
static Standard_Boolean CoPlanar(const gp_Pnt PntA, const gp_Pnt PntB, const gp_Pnt PntC, const gp_Pnt PntD) {
...
static Handle(GeomAdaptor_Surface)
    BoundSurf(const Handle(Geom_Surface) & S, const gp_Pnt2d& Pdeb, const gp_Pnt2d& Pfin) {
...
static Standard_Boolean ComputeIntersection(TopOpeBRepDS_DataStructure& DStr, const Handle(ChFiDS_SurfData) & SD,
                                            const Handle(ChFiDS_SurfData) & SDCoin, const gp_Pnt& pdeb,
                                            const gp_Pnt2d& p2ddeb, const gp_Pnt& pfin, const gp_Pnt2d& p2dfin,
                                            Handle(Geom_Curve) & gc, Handle(Geom2d_Curve) & pc1,
                                            Handle(Geom2d_Curve) & pc2, gp_Vec& derudeb, gp_Vec& dervdeb,
                                            gp_Pnt2d& ptcoindeb, const Standard_Real tol3d, const Standard_Real tol2d,
...
void ChFi3d_ChBuilder::PerformThreeCorner(const Standard_Integer Jndex) {
    ...
    oksea[2] = ChFi3d_SearchFD(DStr, CD[0], CD[1], sens[0], sens[1], i[0][1], i[1][0], p[0][1], p[1][0], Index[0],
                               Index[1], face[2], sameside[2], jf[0][1], jf[1][0]);
    oksea[1] = ChFi3d_SearchFD(DStr, CD[0], CD[2], sens[0], sens[2], i[0][2], i[2][0], p[0][2], p[2][0], Index[0],
                               Index[2], face[1], sameside[1], jf[0][2], jf[2][0]);
    oksea[0] = ChFi3d_SearchFD(DStr, CD[1], CD[2], sens[1], sens[2], i[1][2], i[2][1], p[1][2], p[2][1], Index[1],
                               Index[2], face[0], sameside[0], jf[1][2], jf[2][1]);
    //
    ...
    if (oksea[2] && oksea[1] && !sameside[2] && !sameside[1]) {
        ...
        if (!oksea[0])
            okinter = ChFi3d_IsInFront(DStr, CD[1], CD[2], i[1][2], i[2][1], sens[1], sens[2], p[1][2], p[2][1],
                                       face[0], sameside[0], jf[1][2], jf[2][1], visavis, Vtx, Standard_False, 1);
    } else if (oksea[2] && oksea[0] && !sameside[2] && !sameside[0]) {
        pivot = 1;
        deb = 2;
        fin = 0;
        if (!oksea[1])
            okinter = ChFi3d_IsInFront(DStr, CD[0], CD[2], i[0][2], i[2][0], sens[0], sens[2], p[0][2], p[2][0],
                                       face[1], sameside[1], jf[0][2], jf[2][0], visavis, Vtx, Standard_False, 1);
    } else if (oksea[1] && oksea[0] && !sameside[1] && !sameside[0]) {
        pivot = 2;
        deb = 0;
        fin = 1;
        if (!oksea[2])
            okinter = ChFi3d_IsInFront(DStr, CD[0], CD[1], i[0][1], i[1][0], sens[0], sens[1], p[0][1], p[1][0],
                                       face[2], sameside[2], jf[0][1], jf[1][0], visavis, Vtx, Standard_False, 1);
    ...
    ChFi3d_ComputesIntPC(fdpiv->Interference(jf[pivot][deb]), fddeb->Interference(jf[deb][pivot]), HSpiv, HSdeb,
                         p[pivot][deb], p[deb][pivot], p3d[fin]);
    ChFi3d_ComputesIntPC(fdpiv->Interference(jf[pivot][fin]), fdfin->Interference(jf[fin][pivot]), HSpiv, HSfin,
                         p[pivot][fin], p[fin][pivot], p3d[deb]);
    ChFi3d_ComputesIntPC(fddeb->Interference(jf[deb][fin]), fdfin->Interference(jf[fin][deb]), HSdeb, HSfin,
                         p[deb][fin], p[fin][deb], PSom);
    ...
    if (CornerAllSame) c1triangle = (DistDebFin < 0.3 * DistMin);

    if (c1triangle) p3d[pivot] = PSom;
    ...
    if (c1plan) {
        ...
        if (!ComputeIntersection(DStr, fdpiv, coin, p3d[fin], p2d[fin], p3d[deb], p2d[deb], gcpiv, pivpc1, pivpc2, deru,
                                 derv, ptbid, tolesp, tol2d, tolrcoinpiv))
        ...
        if (!ComputeIntersection(DStr, fddeb, coin, p3d[pivot], p2d1, p3d[fin], p2d2, gcdeb, debpc1, debpc2, deru, derv,
                                 ptbid, tolesp, tol2d, tolrcoindeb))
        ...
        if (!ComputeIntersection(DStr, fdfin, coin, p3dface, p2d1, p3d[deb], p2d2, gcfin, finpc1, finpc2, deru, derv,
                                 ptbid, tolesp, tol2d, tolrcoinfin))
        ...
        if (issmooth) {
            ...
            // tangentes donnees
            Bfac =
                ChFi3d_mkbound(Fac, PCurveOnFace, sens[deb], p2d[pivot], Tgpiv, sens[fin], p2d[3], Tg3, tolesp, 2.e-4);
            Bpiv = ChFi3d_mkbound(Surf, PCurveOnPiv, sens[deb], p2d[fin], vpfin, sens[fin], p2d[deb], vpdeb, tolesp,
                                  2.e-4);
        } else {
            // les bords de coin sont des segments
            //      Bfac = ChFi3d_mkbound(Fac,PCurveOnFace,p2d[pivot],
            //			    p2d[3],tolesp,2.e-4);
            Bfac = ChFi3d_mkbound(Fac, PCurveOnFace, p2d[pivot], p2d[3], tolesp, 2.e-4);
            Bpiv = ChFi3d_mkbound(Surf, PCurveOnPiv, p2d[fin], p2d[deb], tolesp, 2.e-4);
        }
        ...
        if (issmooth) {
            ...
            Bdeb = ChFi3d_mkbound(DStr.Surface(fddeb->Surf()).Surface(), pdeb1, pdeb2, tolesp, 2.e-4, Standard_True);
            Bfin = ChFi3d_mkbound(DStr.Surface(fdfin->Surf()).Surface(), pfin1, pfin2, tolesp, 2.e-4, Standard_True);
        }
        ...
        done = CompleteData(coin, Surfcoin, Fac, PCurveOnFace, Surf, PCurveOnPiv, fdpiv->Orientation(), 0, 0, 0, 0, 0);
    }
    ...
    if (done) {
        ...
        if (c1triangle) Pl1 = coin->ChangeVertexFirstOnS1();
        ...
        if (c1plan) {
            ...
            Standard_Real tolreached;
            ChFi3d_ComputeArete(Pf1, pp1, Pf2, pp2, DStr.Surface(coin->Surf()).Surface(), C3d,
                                corner->ChangeFirstPCurve(), P1deb, P2deb, tolesp, tol2d, tolreached, 0);
        ...
        if (c1plan) {
            ...
            Handle(Geom_Curve) C3d;
            Standard_Real tolreached;
            ChFi3d_ComputeArete(Pl1, pp1, Pl2, pp2, DStr.Surface(coin->Surf()).Surface(), C3d,
                                corner->ChangeLastPCurve(), P1fin, P2fin, tolesp, tol2d, tolreached, 0);
            TopOpeBRepDS_Curve Tcurv(C3d, tolreached);
