...
// purpose  :
//=======================================================================
ChFiDS_TypeOfConcavity ChFi3d::DefineConnectType(const TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2,
                                                 const Standard_Real SinTol, const Standard_Boolean CorrectPoint) {
    ...
    TopoDS_Edge EE = E;
    if (F1.IsSame(F2)) EE.Reverse();
    Handle(Geom2d_Curve) C2 = BRep_Tool::CurveOnSurface(EE, F2, f, l);
    if (C1.IsNull() || C2.IsNull()) return ChFiDS_Other;
    ...
    if (CorrectPoint) Correct2dPoint(F1, P);
    //
    ...
    P = C2->Value(ParOnC);
    if (CorrectPoint) Correct2dPoint(F2, P);
    ...
    if (NormProVec < SinTol) {
        ...
        if (NormProVec > gp::Resolution()) ProVec /= NormProVec;
...
// purpose  :
//=======================================================================
Standard_Boolean ChFi3d::IsTangentFaces(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace1,
                                        const TopoDS_Face& theFace2, const GeomAbs_Shape theOrder) {
    ...
    if (!theFace1.IsSame(theFace2) && BRep_Tool::IsClosed(theEdge, theFace1) &&
        ...
        if (anEdgeInFace1.IsNull()) return Standard_False;
        ...
        if (theFace1.IsSame(theFace2)) EE.Reverse();
        aC2d2 = BRep_Tool::CurveOnSurface(EE, theFace2, aFirst, aLast);
    }

    if (aC2d1.IsNull() || aC2d2.IsNull()) return Standard_False;

    // Obtaining of two surfaces from adjacent faces.
    Handle(Geom_Surface) aSurf1 = BRep_Tool::Surface(theFace1);
    Handle(Geom_Surface) aSurf2 = BRep_Tool::Surface(theFace2);

    if (aSurf1.IsNull() || aSurf2.IsNull()) return Standard_False;
    ...
    for (i = 1, aPar = aFirst; i <= aNbSamples; i++, aPar += aDelta) {
        ...
        LocalAnalysis_SurfaceContinuity aCont(aC2d1, aC2d2, aPar, aSurf1, aSurf2, theOrder, 0.001, TolC0, 0.1, 0.1,
                                              0.1);
        if (!aCont.IsDone()) {
            if (theOrder == GeomAbs_C2 && aCont.StatusError() == LocalAnalysis_NullSecondDerivative) continue;
        ...
        if (theOrder == GeomAbs_G1) {
            if (!aCont.IsG1()) return Standard_False;
        } else if (!aCont.IsG2())
            return Standard_False;
    }

    if (nbNotDone == aNbSamples) return Standard_False;
    ...
    if (dot < 0.) return Standard_False;
    return Standard_True;
}
...
//           2 faces.
//=======================================================================
Standard_Integer ChFi3d::ConcaveSide(const BRepAdaptor_Surface& S1, const BRepAdaptor_Surface& S2, const TopoDS_Edge& E,
                                     TopAbs_Orientation& Or1, TopAbs_Orientation& Or2)

{
    ...
    ns1 = DU1.Crossed(DV1);
    ns1.Normalize();
    if (F1.Orientation() == TopAbs_REVERSED) ns1.Reverse();
    S2.D1(p2d2.X(), p2d2.Y(), pt2, DU2, DV2);
    ns2 = DU2.Crossed(DV2);
    ns2.Normalize();
    if (F2.Orientation() == TopAbs_REVERSED) ns2.Reverse();
...
Standard_Integer ChFi3d::NextSide(TopAbs_Orientation& Or1, TopAbs_Orientation& Or2, const TopAbs_Orientation OrSave1,
                                  const TopAbs_Orientation OrSave2, const Standard_Integer ChoixSave) {
    if (Or1 == TopAbs_FORWARD) {
        Or1 = OrSave1;
    } else {
...
void ChFi3d::NextSide(TopAbs_Orientation& Or, const TopAbs_Orientation OrSave, const TopAbs_Orientation OrFace) {
    if (Or == OrFace) {
        Or = OrSave;
    } else {
...
Standard_Boolean ChFi3d::SameSide(const TopAbs_Orientation Or, const TopAbs_Orientation OrSave1,
                                  const TopAbs_Orientation OrSave2, const TopAbs_Orientation OrFace1,
