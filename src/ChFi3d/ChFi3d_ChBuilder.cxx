...
void SearchCommonFaces(const ChFiDS_Map& EFMap, const TopoDS_Edge& E, TopoDS_Face& F1, TopoDS_Face& F2) {
    TopoDS_Face Fc;
    ...
    if (!F1.IsNull() && F2.IsNull() && BRepTools::IsReallyClosed(E, F1)) F2 = F1;
}
...
void ExtentSpineOnCommonFace(Handle(ChFiDS_Spine) & Spine1, Handle(ChFiDS_Spine) & Spine2, const TopoDS_Vertex& V,
                             const Standard_Real dis1, const Standard_Real dis2, const Standard_Boolean isfirst1,
    ...
    tg1.Normalize();
    tg2.Normalize();
    if (isfirst1) tg1.Reverse();
    if (isfirst2) tg2.Reverse();
...
ChFi3d_ChBuilder::ChFi3d_ChBuilder(const TopoDS_Shape& S, const Standard_Real Ta) : ChFi3d_Builder(S, Ta) {
...
void ChFi3d_ChBuilder::SetDist(const Standard_Real Dis, const Standard_Integer IC, const TopoDS_Face& F) {
...
void ChFi3d_ChBuilder::Add(const Standard_Real Dis1, const Standard_Real Dis2, const TopoDS_Edge& E,
                           const TopoDS_Face& F) {
    if (!Contains(E) && myEFMap.Contains(E)) {
...
void ChFi3d_ChBuilder::SetDists(const Standard_Real Dis1, const Standard_Real Dis2, const Standard_Integer IC,
                                const TopoDS_Face& F) {
...
void ChFi3d_ChBuilder::Dists(const Standard_Integer IC, Standard_Real& dis1, Standard_Real& dis2) const {
    Handle(ChFiDS_ChamfSpine) chsp = Handle(ChFiDS_ChamfSpine)::DownCast(Value(IC));
...
void ChFi3d_ChBuilder::AddDA(const Standard_Real Dis1, const Standard_Real Angle, const TopoDS_Edge& E,
                             const TopoDS_Face& F) {
    if (!Contains(E) && myEFMap.Contains(E)) {
...
void ChFi3d_ChBuilder::SetDistAngle(const Standard_Real Dis, const Standard_Real Angle, const Standard_Integer IC,
                                    const TopoDS_Face& F) {
...
void ChFi3d_ChBuilder::GetDistAngle(const Standard_Integer IC, Standard_Real& Dis, Standard_Real& Angle) const {
    Handle(ChFiDS_ChamfSpine) chsp = Handle(ChFiDS_ChamfSpine)::DownCast(Value(IC));
...
Handle(ChFiDS_SecHArray1) ChFi3d_ChBuilder::Sect(const Standard_Integer IC, const Standard_Integer IS) const {
    ChFiDS_ListIteratorOfListOfStripe itel;
    Standard_Integer i = 1;
...
Standard_Boolean ChFi3d_ChBuilder::SimulSurf(
...

{
    ...
    if (chsp.IsNull()) throw Standard_ConstructionError("SimulSurf : this is not the spine of a chamfer");
    ...
    if (chsp->IsChamfer() == ChFiDS_Sym) {
        ...
        done = SimulData(Data, HGuide, OffsetHGuide, lin, S1, I1, S2, I2, *pFunc, *pFInv, PFirst, MaxStep, locfleche,
        ...
        done = SimulData(Data, HGuide, OffsetHGuide, lin, S1, I1, S2, I2, Func, FInv, PFirst, MaxStep, locfleche,
                         TolGuide, First, Last, Inside, Appro, Forward, Soldep, 4, RecOnS1, RecOnS2);
...
void ChFi3d_ChBuilder::SimulSurf(Handle(ChFiDS_SurfData) &, const Handle(ChFiDS_ElSpine) &,
                                 const Handle(ChFiDS_Spine) &, const Standard_Integer,
                                 const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                 const Handle(BRepAdaptor_Curve2d) &, const Handle(BRepAdaptor_Surface) &,
                                 const Handle(BRepAdaptor_Curve2d) &, Standard_Boolean&,
                                 const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                 const TopAbs_Orientation, const Standard_Real, const Standard_Real, Standard_Real&,
                                 Standard_Real&, const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
...
void ChFi3d_ChBuilder::SimulSurf(Handle(ChFiDS_SurfData) &, const Handle(ChFiDS_ElSpine) &,
                                 const Handle(ChFiDS_Spine) &, const Standard_Integer,
                                 const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                 const TopAbs_Orientation, const Handle(BRepAdaptor_Surface) &,
                                 const Handle(Adaptor3d_TopolTool) &, const Handle(BRepAdaptor_Curve2d) &,
                                 const Handle(BRepAdaptor_Surface) &, const Handle(BRepAdaptor_Curve2d) &,
                                 Standard_Boolean&, const Standard_Real, const Standard_Real, Standard_Real&,
                                 Standard_Real&, const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
                                 const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
                                 const math_Vector&) {
    throw Standard_Failure("SimulSurf Not Implemented");
}
void ChFi3d_ChBuilder::SimulSurf(
    Handle(ChFiDS_SurfData) &, const Handle(ChFiDS_ElSpine) &, const Handle(ChFiDS_Spine) &, const Standard_Integer,
    const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &, const Handle(BRepAdaptor_Curve2d) &,
    const Handle(BRepAdaptor_Surface) &, const Handle(BRepAdaptor_Curve2d) &, Standard_Boolean&,
    const TopAbs_Orientation, const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
    const Handle(BRepAdaptor_Curve2d) &, const Handle(BRepAdaptor_Surface) &, const Handle(BRepAdaptor_Curve2d) &,
    Standard_Boolean&, const TopAbs_Orientation, const Standard_Real, const Standard_Real, Standard_Real&,
    Standard_Real&, const Standard_Boolean, const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
    const Standard_Boolean, const Standard_Boolean, const Standard_Boolean, const math_Vector&) {
    throw Standard_Failure("SimulSurf Not Implemented");
}
//------------------------MODIFS---------------------------------------
//=======================================================================
...
Standard_Boolean
ChFi3d_ChBuilder::PerformFirstSection(const Handle(ChFiDS_Spine) & Spine, const Handle(ChFiDS_ElSpine) & HGuide,
    ...
    if (chsp.IsNull()) throw Standard_ConstructionError("PerformSurf : this is not the spine of a chamfer");
    ...
    if (chsp->IsChamfer() == ChFiDS_Sym) {
        ...
        return TheWalk.PerformFirstSection(*pFunc, Par, SolDep, tolesp, TolGuide, Pos1, Pos2);
    } else if (chsp->IsChamfer() == ChFiDS_TwoDist) {
        Standard_Real dis1, dis2;
        chsp->Dists(dis1, dis2);
...
Standard_Boolean ChFi3d_ChBuilder::PerformSurf(
...

{
    ...
    if (chsp->IsChamfer() == ChFiDS_Sym) {
        ...
        pFInv->Set(dis, dis, Choix);
            ...
            ComputeData(Data, HGuide, Spine, lin, S1, I1, S2, I2, *pFunc, *pFInv, PFirst, MaxStep, Fleche, TolGuide,
                        First, Last, Inside, Appro, Forward, Soldep, intf, intl, gd1, gd2, gf1, gf2, RecOnS1, RecOnS2);
...
void ChFi3d_ChBuilder::PerformSurf(ChFiDS_SequenceOfSurfData&, const Handle(ChFiDS_ElSpine) &,
                                   const Handle(ChFiDS_Spine) &, const Standard_Integer,
                                   const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                   const Handle(BRepAdaptor_Curve2d) &, const Handle(BRepAdaptor_Surface) &,
                                   const Handle(BRepAdaptor_Curve2d) &, Standard_Boolean&,
                                   const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                   const TopAbs_Orientation, const Standard_Real, const Standard_Real,
                                   const Standard_Real, Standard_Real&, Standard_Real&, const Standard_Boolean,
                                   const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
                                   const Standard_Boolean, const Standard_Boolean, const math_Vector&) {
...
void ChFi3d_ChBuilder::PerformSurf(ChFiDS_SequenceOfSurfData&, const Handle(ChFiDS_ElSpine) &,
                                   ...
                                   const Handle(Adaptor3d_TopolTool) &, const Handle(BRepAdaptor_Curve2d) &,
                                   const Handle(BRepAdaptor_Surface) &, const Handle(BRepAdaptor_Curve2d) &,
                                   Standard_Boolean&, const Standard_Real, const Standard_Real, const Standard_Real,
                                   Standard_Real&, Standard_Real&, const Standard_Boolean, const Standard_Boolean,
                                   const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
                                   const Standard_Boolean, const math_Vector&) {
    throw Standard_Failure("PerformSurf Not Implemented");
}
void ChFi3d_ChBuilder::PerformSurf(ChFiDS_SequenceOfSurfData&, const Handle(ChFiDS_ElSpine) &,
                                   const Handle(ChFiDS_Spine) &, const Standard_Integer,
                                   const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                   const Handle(BRepAdaptor_Curve2d) &, const Handle(BRepAdaptor_Surface) &,
                                   const Handle(BRepAdaptor_Curve2d) &, Standard_Boolean&, const TopAbs_Orientation,
                                   const Handle(BRepAdaptor_Surface) &, const Handle(Adaptor3d_TopolTool) &,
                                   const Handle(BRepAdaptor_Curve2d) &, const Handle(BRepAdaptor_Surface) &,
                                   const Handle(BRepAdaptor_Curve2d) &, Standard_Boolean&, const TopAbs_Orientation,
                                   const Standard_Real, const Standard_Real, const Standard_Real, Standard_Real&,
                                   Standard_Real&, const Standard_Boolean, const Standard_Boolean,
                                   const Standard_Boolean, const Standard_Boolean, const Standard_Boolean,
                                   const Standard_Boolean, const Standard_Boolean, const math_Vector&) {
    throw Standard_Failure("PerformSurf Not Implemented");
}
...
void ChFi3d_ChBuilder::ExtentTwoCorner(const TopoDS_Vertex& V, const ChFiDS_ListOfStripe& LS) {
    ...
    for (; itel.More(); itel.Next(), i++) {
        ...
        if (!FF)
            if (Stripe[1] == itel.Value()) Sens = -Sens;

        Stripe[i] = itel.Value();
        isfirst[i] = (Sens == 1);
        Spine[i] = Stripe[i]->Spine();
        if (!isfirst[i]) Iedge[i] = Spine[i]->NbEdges();
        FF = Standard_False;
    }
...
void ChFi3d_ChBuilder::ConexFaces(const Handle(ChFiDS_Spine) & Spine, const Standard_Integer IEdge, TopoDS_Face& F1,
