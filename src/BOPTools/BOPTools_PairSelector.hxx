...
public: //! @name Constructor
    //! Empty constructor
    BOPTools_PairSelector() : mySameBVHs(Standard_False) {}
...
public: //! @name Rejection/Acceptance rules
    ...
    virtual Standard_Boolean RejectNode(const BVH_VecNd& theCMin1, const BVH_VecNd& theCMax1, const BVH_VecNd& theCMin2,
                                        const BVH_VecNd& theCMax2, Standard_Real&) const Standard_OVERRIDE {
    ...
    Standard_Boolean RejectElement(const Standard_Integer theID1, const Standard_Integer theID2) {
        return (mySameBVHs && theID1 >= theID2) || this->myBVHSet1->Box(theID1).IsOut(this->myBVHSet2->Box(theID2));
    }
    ...
    virtual Standard_Boolean Accept(const Standard_Integer theID1, const Standard_Integer theID2) Standard_OVERRIDE {
        if (!RejectElement(theID1, theID2)) {
            myPairs.push_back(PairIDs(this->myBVHSet1->Element(theID1), this->myBVHSet2->Element(theID2)));
