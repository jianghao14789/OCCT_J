...
void Extrema_ExtElCS::Perform(const gp_Circ& C, const gp_Sphere& S) {
    ...
    for (Standard_Integer i = 1; i <= aNbExt; ++i) {
        ...
        if (aDiff < Precision::SquareConfusion()) aSols.Append(i);
    }
...
void Extrema_ExtElCS::Points(const Standard_Integer N, Extrema_POnCurv& P1, Extrema_POnSurf& P2) const {
    if (N < 1 || N > NbExt()) {
        throw Standard_OutOfRange();
    }
