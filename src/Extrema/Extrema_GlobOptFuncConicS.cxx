...
// purpose  :
//=======================================================================
Standard_Boolean Extrema_GlobOptFuncConicS::checkInputData(const math_Vector& X, Standard_Real& su, Standard_Real& sv) {
...
// purpose  : Constructor
//=======================================================================
Extrema_GlobOptFuncConicS::Extrema_GlobOptFuncConicS(const Adaptor3d_Surface* S, const Standard_Real theUf,
                                                     const Standard_Real theUl, const Standard_Real theVf,
                                                     const Standard_Real theVl)
    : myS(S), myUf(theUf), myUl(theUl), myVf(theVf), myVl(theVl) {}
...
Extrema_GlobOptFuncConicS::Extrema_GlobOptFuncConicS(const Adaptor3d_Surface* S)
    : myS(S), myUf(S->FirstUParameter()), myUl(S->LastUParameter()), myVf(S->FirstVParameter()),
      myVl(S->LastVParameter()) {}
//=======================================================================
// function : Extrema_GlobOptFuncConicS
// purpose  : Constructor
//=======================================================================
Extrema_GlobOptFuncConicS::Extrema_GlobOptFuncConicS(const Adaptor3d_Curve* C, const Adaptor3d_Surface* S)
    : myS(S), myUf(S->FirstUParameter()), myUl(S->LastUParameter()), myVf(S->FirstVParameter()),
      myVl(S->LastVParameter()) {
...
// purpose  :
//=======================================================================
void Extrema_GlobOptFuncConicS::LoadConic(const Adaptor3d_Curve* C, const Standard_Real theTf,
    ...
    if (myC->IsPeriodic()) {
        ...
        if (myTf > aTMax || myTf < -Precision::PConfusion() || Abs(myTl - myTf) > aTMax) {
            ElCLib::AdjustPeriodic(0., 2. * M_PI, Min(Abs(myTl - myTf) / 2, Precision::PConfusion()), myTf, myTl);
        }
    }
...
Standard_Boolean Extrema_GlobOptFuncConicS::Value(const math_Vector& X, Standard_Real& F) {
    Standard_Real su, sv;
    if (!checkInputData(X, su, sv)) return Standard_False;
