        ...
        case GeomAbs_Line: {
            ...
            if (N.IsParallel(L.Direction(), Precision::Angular())) {
                ...
                if (OO.Magnitude() <= gp::Resolution()) {
                    ...
                    if (N.IsParallel(OO, Precision::Angular())) return gp_Ax2(); // Line and axe of revolution coincide
                }
                N ^= OO;
            } else {
                N ^= L.Direction();
...
static void PerformExtPElC(Extrema_ExtPElC& E, const gp_Pnt& P, const Handle(Adaptor3d_Curve) & C,
...
static Standard_Boolean IsCaseAnalyticallyComputable(const GeomAbs_CurveType& theType, const gp_Ax2& theCurvePos,
    ...
    if ((pl.SquareDistance(p1) < aThreshold) && (pl.SquareDistance(p2) < aThreshold)) return Standard_True;
    return Standard_False;
...
static Standard_Boolean IsOriginalPnt(const gp_Pnt& P, const Extrema_POnSurf* Points, const Standard_Integer NbPoints) {
...
static Standard_Boolean IsExtremum(const Standard_Real U, const Standard_Real V, const gp_Pnt& P,
                                   const Adaptor3d_Surface* S, gp_Pnt& E, Standard_Real& Dist2,
                                   const Standard_Boolean IsVSup, const Standard_Boolean IsMin) {
    E = S->Value(U, V);
    Dist2 = P.SquareDistance(E);
    if (IsMin)
        return (Dist2 < P.SquareDistance(S->Value(U + 1, V)) && Dist2 < P.SquareDistance(S->Value(U - 1, V)) &&
...
Extrema_ExtPRevS::Extrema_ExtPRevS(const gp_Pnt& theP, const Handle(GeomAdaptor_SurfaceOfRevolution) & theS,
                                   const Standard_Real theUmin, const Standard_Real theUsup,
                                   const Standard_Real theVmin, const Standard_Real theVsup,
...
void Extrema_ExtPRevS::Initialize(const Handle(GeomAdaptor_SurfaceOfRevolution) & theS, const Standard_Real theUmin,
                                  const Standard_Real theUsup, const Standard_Real theVmin, const Standard_Real theVsup,
                                  const Standard_Real theTolU, const Standard_Real theTolV) {
    myvinf = theVmin;
    myvsup = theVsup;
    mytolv = theTolV;
...
void Extrema_ExtPRevS::Perform(const gp_Pnt& P) {
    ...
    if (U != M_PI / 2) {
        ...
        if (OPpp.AngleWithRef(OPq, Dir) < 0) U += M_PI;
    }
    ...
    if (anExt.IsDone()) {
        ...
        for (i = 1; i <= anExt.NbExt(); i++) {
            ...
            if (V > myvsup) {
                ...
                if (!IsExtremum(U, V, P, myS.get(), E, Dist2, Standard_False, anExt.IsMin(i))) continue;
            } else {
                E = myS->Value(U, V);
                Dist2 = P.SquareDistance(E);
            }
    ...
    if (anExt.IsDone()) {
        ...
        for (i = 1; i <= anExt.NbExt(); i++) {
            ...
            if (V > myvsup) {
                ...
                if (!IsExtremum(U, V, P, myS.get(), E, Dist2, Standard_True, anExt.IsMin(i))) continue;
            } else if (V < myvinf) {
                // 	if ( !IsExtremum (U, V = myvinf, P, myS, E, Dist2,
                // 			  Standard_False, anExt.IsMin(i))) continue;
                Standard_Real newV = myvinf;
