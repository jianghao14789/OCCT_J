...
Extrema_FuncPSNorm::Extrema_FuncPSNorm() : myS(NULL), myU(0.0), myV(0.0) {
    myPinit = Standard_False;
    mySinit = Standard_False;
}

//=============================================================================
Extrema_FuncPSNorm::Extrema_FuncPSNorm(const gp_Pnt& P, const Adaptor3d_Surface& S) : myU(0.0), myV(0.0) {
    myP = P;
    myS = &S;
...
Standard_Integer Extrema_FuncPSNorm::GetStateNumber() {
    ...
    for (; i <= nbSol; i++) {
        ...
        if (((myU - aU) * (myU - aU) + (myV - aV) * (myV - aV)) <= tol2d) break;
    }
    if (i <= nbSol) return 0;
