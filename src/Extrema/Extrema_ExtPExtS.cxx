...
void Extrema_ExtPExtS::MakePreciser(Standard_Real& U, const gp_Pnt& P, const Standard_Boolean isMin,
...
Extrema_ExtPExtS::Extrema_ExtPExtS()
    : myuinf(0.0), myusup(0.0), mytolu(0.0), myvinf(0.0), myvsup(0.0), mytolv(0.0),
      myIsAnalyticallyComputable(Standard_False), myDone(Standard_False), myNbExt(0) {
...
Extrema_ExtPExtS::Extrema_ExtPExtS(const gp_Pnt& theP, const Handle(GeomAdaptor_SurfaceOfLinearExtrusion) & theS,
    ...
    : myuinf(theUmin), myusup(theUsup), mytolu(theTolU), myvinf(theVmin), myvsup(theVsup), mytolv(theTolV), myS(theS),
      myIsAnalyticallyComputable(Standard_False), myDone(Standard_False), myNbExt(0) {
    for (size_t anIdx = 0; anIdx < sizeof(mySqDist) / sizeof(mySqDist[0]); anIdx++) {
        mySqDist[anIdx] = RealLast();
    }
...
Extrema_ExtPExtS::Extrema_ExtPExtS(const gp_Pnt& theP, const Handle(GeomAdaptor_SurfaceOfLinearExtrusion) & theS,
                                   const Standard_Real theTolU, const Standard_Real theTolV)
    : myuinf(theS->FirstUParameter()), myusup(theS->LastUParameter()), mytolu(theTolU), myvinf(theS->FirstVParameter()),
      myvsup(theS->LastVParameter()), mytolv(theTolV), myS(theS), myIsAnalyticallyComputable(Standard_False),
      myDone(Standard_False), myNbExt(0) {
    for (size_t anIdx = 0; anIdx < sizeof(mySqDist) / sizeof(mySqDist[0]); anIdx++) {
        mySqDist[anIdx] = RealLast();
    }
...
void Extrema_ExtPExtS::Initialize(const Handle(GeomAdaptor_SurfaceOfLinearExtrusion) & theS,
                                  const Standard_Real theUinf, const Standard_Real theUsup, const Standard_Real theVinf,
                                  const Standard_Real theVsup, const Standard_Real theTolU,
...
void Extrema_ExtPExtS::Perform(const gp_Pnt& P) {
    ...
    Standard_Real U, V;
    Standard_Boolean isMin, isSimpleCase = myDirection.IsParallel(myPosition.Direction(), Precision::Angular());
