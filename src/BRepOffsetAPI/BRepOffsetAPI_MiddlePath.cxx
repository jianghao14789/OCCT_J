...
static GeomAbs_CurveType TypeOfEdge(const TopoDS_Edge& anEdge) {
    gp_Lin aLin;
    if (IsLinear(anEdge, aLin)) return GeomAbs_Line;
...
static gp_Vec TangentOfEdge(const TopoDS_Shape& aShape, const Standard_Boolean OnFirst) {
    ...
    if (anOr == TopAbs_REVERSED) theTangent.Reverse();
    ...
    for (; Explo.More(); Explo.Next()) {
        ...
        if (DistMini.Value() <= Tol) {
            for (i = 1; i <= DistMini.NbSolution(); i++) {
                ...
                if (theType == BRepExtrema_IsOnEdge) return Standard_False;
                // theType is "IsVertex"
                TopoDS_Shape aVertex = DistMini.SupportOnShape2(i);
                if (!(aVertex.IsSame(V1) || aVertex.IsSame(V2))) return Standard_False;
            }
        }
    }
...
// purpose  :
//=======================================================================
static TopoDS_Wire GetUnifiedWire(const TopoDS_Wire& theWire, ShapeUpgrade_UnifySameDomain& theUnifier) {
    ...
    for (; wexp.More(); wexp.Next()) {
        ...
        if (!aLS.IsEmpty()) {
            ...
            for (; anIt.More(); anIt.Next()) {
                ...
                if (aGeneratedEdges.Add(aShape)) aWMaker.Add(TopoDS::Edge(aShape));
            }
        } else {
...
BRepOffsetAPI_MiddlePath::BRepOffsetAPI_MiddlePath(const TopoDS_Shape& aShape, const TopoDS_Shape& StartShape,
...
void BRepOffsetAPI_MiddlePath::Build(const Message_ProgressRange& /*theRange*/) {
    ...
    if (!myClosedSection) StartVertices.Append(wexp.CurrentVertex());
    ...
    if (!myClosedSection) EndVertices.Add(wexp.CurrentVertex());
    ...
    for (;;) {
        for (i = 1; i <= myPaths.Length(); i++) {
            ...
            for (itl.Initialize(LE); itl.More(); itl.Next()) {
                ...
                if (!CurVertices.Contains(NextVertex)) NextEdgeCandidates.Add(anEdge);
            }
    ...
    */
    ...
    for (;;) {
        ...
        for (j = 2; j <= NbVer; j++) {
            if (!EdgeSeq(j - 1).IsNull()) continue;
            ...
            TopoDS_Edge E1, E2;
            if (myPaths(j - 1)(i).ShapeType() == TopAbs_EDGE) E1 = TopoDS::Edge(myPaths(j - 1)(i));
            if (myPaths((j <= NbPaths) ? j : 1)(i).ShapeType() == TopAbs_EDGE)
            ...
            TopoDS_Vertex PrevPrevVer = (E1.IsNull()) ? PrevVertex : TopExp::FirstVertex(E1, Standard_True);
            ...
            if (ProperEdge.IsNull()) // no connection between these two vertices
            {
                ...
                if (E1.IsNull() || E2.IsNull()) {
                    if (E1.IsNull()) E1 = TopoDS::Edge(myPaths(j - 1)(i - 1));
                    if (E2.IsNull()) E2 = TopoDS::Edge(myPaths((j <= NbPaths) ? j : 1)(i - 1));
                    ...
                    TopoDS_Edge NewEdge = BRepLib_MakeEdge(theLine, theSurf, PrevVertex, CurVertex, 0., len_ne);
                ...
                } else // E1 is edge
                {
                    ...
                    TopoDS_Edge NewEdge1 = BRepLib_MakeEdge(Line1, theSurf, PrevPrevVer, CurVertex, 0., len_ne1);
                    BRepLib::BuildCurve3d(NewEdge1);
                    Standard_Real len_ne2 = FirstPnt2d.Distance(PrevLastPnt2d);
                    TopoDS_Edge NewEdge2 = BRepLib_MakeEdge(Line2, theSurf, PrevVertex, PrevCurVer, 0., len_ne2);
        ...
        for (j = 1; j <= EdgeSeq.Length(); j++)
            if (EndEdges.Contains(EdgeSeq(j))) NbEndEdges++;
        if (NbEndEdges == NbE) break;
        ...
        if (TypeOfMidEdge == GeomAbs_Line)
            ...
            for (j = 1; j <= myPaths.Length(); j++) {
                ...
                if (j == 1) {
                    ...
                    if (anEdge.Orientation() == TopAbs_REVERSED) theTangent.Reverse();
                } else {
            ...
            if (SimilarArcs) {
                ...
                */
                ...
                if (Abs(anAngle - theAngle) > AngTol) theAxis.Reverse();
        ...
        if (MidEdges(i).IsNull()) {
            ...
            for (k = i; k <= j; k++) {
                ...
                for (Standard_Integer indp = 1; indp <= myPaths.Length(); indp++) {
                    gp_Vec aTangent;
                    if (k == i) {
                        if (myPaths(indp)(k).ShapeType() == TopAbs_VERTEX) continue;
                        aTangent = TangentOfEdge(myPaths(indp)(k), Standard_True); // at begin
                    } else if (k == j) {
                        if (myPaths(indp)(k - 1).ShapeType() == TopAbs_VERTEX) continue;
    ...
    for (i = 1; i < NbSecFaces; i++)
        if (!MidEdges(i).IsNull()) MakeFinalWire.Add(TopoDS::Edge(MidEdges(i)));
