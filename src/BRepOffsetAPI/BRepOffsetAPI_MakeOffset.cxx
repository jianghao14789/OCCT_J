...
BRepOffsetAPI_MakeOffset::BRepOffsetAPI_MakeOffset()
    : myIsInitialized(Standard_False), myJoin(GeomAbs_Arc), myIsOpenResult(Standard_False) {}
...
BRepOffsetAPI_MakeOffset::BRepOffsetAPI_MakeOffset(const TopoDS_Face& Spine, const GeomAbs_JoinType Join,
...
void BRepOffsetAPI_MakeOffset::Init(const TopoDS_Face& Spine, const GeomAbs_JoinType Join,
                                    const Standard_Boolean IsOpenResult) {
    myFace = Spine;
...
BRepOffsetAPI_MakeOffset::BRepOffsetAPI_MakeOffset(const TopoDS_Wire& Spine, const GeomAbs_JoinType Join,
                                                   const Standard_Boolean IsOpenResult) {
...
void BRepOffsetAPI_MakeOffset::Init(const GeomAbs_JoinType Join, const Standard_Boolean IsOpenResult) {
    myJoin = Join;
    myIsOpenResult = IsOpenResult;
}
...
static void BuildDomains(TopoDS_Face& myFace, TopTools_ListOfShape& WorkWires, BRepFill_ListOfOffsetWire& myAlgos,
                         GeomAbs_JoinType myJoin, Standard_Boolean myIsOpenResult, Standard_Boolean isPositive,
    ...
    if (myFace.IsNull()) {
        ...
        if (myFace.IsNull()) throw StdFail_NotDone("BRepOffsetAPI_MakeOffset : the wire is not planar");
    }
        ...
        if (Offset <= 0.) {
            ...
            for (itOW.Initialize(myLeft); itOW.More(); itOW.Next()) {
                ...
                if (Algo.IsDone() && !Algo.Shape().IsNull()) {
                    ...
                    if (i == 1) myShape = isWasReversed ? Algo.Shape().Reversed() : Algo.Shape();
