...
void BRepOffsetAPI_DraftAngle::Add(const TopoDS_Face& F, const gp_Dir& D, const Standard_Real Angle,
                                   const gp_Pln& Plane, const Standard_Boolean Flag) {
    // POP-DPF : protection
    if (Abs(Angle) <= 1.e-04) return;
    Standard_NullObject_Raise_if(myInitialShape.IsNull(), "BRepOffsetAPI_DraftAngle::Add() - initial shape is not set");
...
void BRepOffsetAPI_DraftAngle::CorrectWires() {
        ...
        for (; wit.More(); wit.Next()) {
            ...
            if (!aWire.IsSame(CurWire)) {
                ...
                for (; eit.More(); eit.Next()) {
                    ...
                    //
                    Wadd = Standard_True;
                    if (!WFmap.IsBound(aWire)) WFmap.Bind(aWire, CurFace);
    ...
    for (i = 1; i <= NonSeam.Length(); i++) {
        ...
        for (j = 1; j <= Eseq.Length(); j++)
            if (SeamEdge.IsSame(Eseq(j))) break;
    ...
    for (; iter.More(); iter.Next()) {
        ...
        fpar = Seq(i);
        lpar = LastPar;
        if (Abs(fpar - lpar) <= Precision::Confusion()) continue;
        TopoDS_Shape aLocalShape = anEdge.EmptyCopied();
        NewE = TopoDS::Edge(aLocalShape);
        // NewE = TopoDS::Edge( anEdge.EmptyCopied() );
        BB.Range(NewE, fpar, lpar);
    ...
    if (aSub.IsCopied(myShape)) {
        ...
        if (!listSh.IsEmpty()) myShape = listSh.First();
    }
    ...
    for (; itww.More(); itww.Next()) {
        ...
        for (; itl.More(); itl.Next()) {
            ...
            for (; eit.More(); eit.Next()) {
                ...
                if (Pfirst.X() - 2. * M_PI > Precision::Confusion() || Plast.X() - 2. * M_PI > Precision::Confusion() ||
...
void BRepOffsetAPI_DraftAngle::CorrectVertexTol() {
    ...
    for (; anExp.More(); anExp.Next()) {
        ...
        if (anInitEdges.Contains(anE)) continue;
        //
        // Skip processed edges
        if (aNewEdges.Contains(anE)) continue;
        //
        aNewEdges.Add(anE);
        //
