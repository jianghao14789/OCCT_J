...
//           to this U-knot
//=======================================================================
static Standard_Real PreciseUpar(const Standard_Real anUpar, const Handle(Geom_BSplineSurface) & aSurface) {
...
static Standard_Boolean PerformPlan(const TopoDS_Wire& W, const Standard_Real presPln, TopoDS_Face& theFace) {
    ...
    if (isDegen) return Standard_True;
...
static TopoDS_Solid MakeSolid(TopoDS_Shell& shell, const TopoDS_Wire& wire1, const TopoDS_Wire& wire2,
                              const Standard_Real presPln, TopoDS_Face& face1, TopoDS_Face& face2) {
    if (shell.IsNull()) throw StdFail_NotDone("Thrusections is not build");
    ...
    if (!B) {
        ...
        if (B) {
            ...
            if (B) {
                if (!face1.IsNull() && !IsSameOriented(face1, shell)) face1.Reverse();
                if (!face2.IsNull() && !IsSameOriented(face2, shell)) face2.Reverse();

                if (!face1.IsNull()) BB.Add(shell, face1);
                if (!face2.IsNull()) BB.Add(shell, face2);
...
BRepOffsetAPI_ThruSections::BRepOffsetAPI_ThruSections(const Standard_Boolean isSolid, const Standard_Boolean ruled,
    ...
    : myNbEdgesInSection(0), myIsSolid(isSolid), myIsRuled(ruled), myPres3d(pres3d), myDegen1(Standard_False),
...
void BRepOffsetAPI_ThruSections::Build(const Message_ProgressRange& /*theRange*/) {
    ...
    if (myWCheck) {
        ...
        if (Georges.IsDone()) {
            ...
            if (Georges.IsDegeneratedFirstSection()) IndFirstSec = 2;
    ...
    else // no check
    {
        ...
        for (Standard_Integer ii = 1; ii <= myWires.Length(); ii++) {
            ...
            for (; Explo.More(); Explo.Next(), inde++) {
                ...
                if (!myVertexIndex.IsBound(V1)) myVertexIndex.Bind(V1, inde);
                if (!myVertexIndex.IsBound(V2)) myVertexIndex.Bind(V2, -inde);
            }
            inde--;
            if (inde > myNbEdgesInSection) myNbEdgesInSection = inde;
...
void BRepOffsetAPI_ThruSections::CreateSmoothed() {
    ...
    for (i = 1; i <= nbEdges; i++) {
        ...
        if (edge.Orientation() == TopAbs_REVERSED) TopExp::Vertices(edge, v1l, v1f);
        firstEdge = edge;

        edge = TopoDS::Edge(shapes((nbSects - 1) * nbEdges + i));
        TopExp::Vertices(edge, v2f, v2l);
        if (edge.Orientation() == TopAbs_REVERSED) TopExp::Vertices(edge, v2l, v2f);
        ...
        if (vClosed) {
            B.UpdateEdge(edge1, new Geom2d_Line(gp_Pnt2d(0, f2), gp_Dir2d(1, 0)),
                         new Geom2d_Line(gp_Pnt2d(0, l2), gp_Dir2d(1, 0)), face, Precision::Confusion());
            B.Range(edge1, face, f1, l1);
        } else {
            B.UpdateEdge(edge1, new Geom2d_Line(gp_Pnt2d(0, f2), gp_Dir2d(1, 0)), face, Precision::Confusion());
            B.Range(edge1, face, f1, l1);
            B.UpdateEdge(edge2, new Geom2d_Line(gp_Pnt2d(0, l2), gp_Dir2d(1, 0)), face, Precision::Confusion());
            B.Range(edge2, face, f1, l1);
        }

        if (uClosed && nbEdges == 1) {
            B.UpdateEdge(edge3, new Geom2d_Line(gp_Pnt2d(l1, 0), gp_Dir2d(0, 1)),
                         new Geom2d_Line(gp_Pnt2d(f1, 0), gp_Dir2d(0, 1)), face, Precision::Confusion());
            B.Range(edge3, face, f2, l2);

        } else {
            B.UpdateEdge(edge3, new Geom2d_Line(gp_Pnt2d(f1, 0), gp_Dir2d(0, 1)), face, Precision::Confusion());
            B.Range(edge3, face, f2, l2);
            B.UpdateEdge(edge4, new Geom2d_Line(gp_Pnt2d(l1, 0), gp_Dir2d(0, 1)), face, Precision::Confusion());
    ...
    if (uClosed && w1Point && w2Point) shell.Closed(Standard_True);
...
static Handle(Geom_BSplineCurve) EdgeToBSpline(const TopoDS_Edge& theEdge) {
    ...
    if (BRep_Tool::Degenerated(theEdge)) {
        ...
        if (aCurve.IsNull()) throw Standard_NullObject("Null 3D curve in edge");
        ...
        if (aTrimCurve->BasisCurve()->IsKind(STANDARD_TYPE(Geom_Conic))) {
            ...
            if (anAppr.HasResult()) aBSCurve = anAppr.Curve();
        }

        // general case
        if (aBSCurve.IsNull()) aBSCurve = GeomConvert::CurveToBSplineCurve(aTrimCurve);
    ...
    if (theEdge.Orientation() == TopAbs_REVERSED) aBSCurve->Reverse();
...
Handle(Geom_BSplineSurface) BRepOffsetAPI_ThruSections::TotalSurf(
    const TopTools_Array1OfShape& shapes, const Standard_Integer NbSects, const Standard_Integer NbEdges,
    const Standard_Boolean w1Point, const Standard_Boolean w2Point, const Standard_Boolean vClosed) const {
    ...
    if (anApprox.IsDone()) {
        surface = new Geom_BSplineSurface(anApprox.SurfPoles(), anApprox.SurfWeights(), anApprox.SurfUKnots(),
                                          anApprox.SurfVKnots(), anApprox.SurfUMults(), anApprox.SurfVMults(),
...
const TopTools_ListOfShape& BRepOffsetAPI_ThruSections::Generated(const TopoDS_Shape& S) {
    ...
    if (S.ShapeType() == TopAbs_EDGE) {
        if (!myEdgeNewIndices.IsBound(S)) return myGenerated;
        ...
        if (!myVertexIndex.IsBound(S)) return myGenerated;
        ...
        if (myDegen1 || myDegen2) {
            ...
            for (Standard_Integer i = 0; i < 2; i++) {
                if (i == 0 && !myDegen1) continue;
                if (i == 1 && !myDegen2) continue;
        ...
        if (IsDegen[0] || IsDegen[1]) {
            ...
            for (; itl.More(); itl.Next()) {
                ...
                if (!BRep_Tool::Degenerated(anEdge)) {
                    ...
                    if ((IsDegen[0] && S.IsSame(VV[0])) || (IsDegen[1] && S.IsSame(VV[1]))) Emap.Add(anEdge);
                }
            }
            for (Standard_Integer j = 1; j <= Emap.Extent(); j++) {
                ...
                if (myIsRuled) {
                    ...
                    for (i = 2, k = myWires.Length() - 1; i < myWires.Length(); i++, k--) {
                        ...
                        TopoDS_Vertex aVertex = (IsDegen[0]) ? TopExp::LastVertex(anEdge) : TopExp::FirstVertex(anEdge);
                        ...
                        for (itl.Initialize(EElist); itl.More(); itl.Next()) {
                            ...
                            if (!NextEdge.IsSame(anEdge) && !EmapOfSection.Contains(NextEdge)) break;
                        }
        ...
        if (myIsRuled)
            ...
            for (Standard_Integer i = 2; i < myWires.Length(); i++) {
                ...
                FirstEdge =
                    (anEdge.IsSame(Elist1.First())) ? TopoDS::Edge(Elist1.Last()) : TopoDS::Edge(Elist1.First());
                ...
                anEdge =
                    (FirstEdge.IsSame(Elist2.First())) ? TopoDS::Edge(Elist2.Last()) : TopoDS::Edge(Elist2.First());
                myGenerated.Append(anEdge);
            }
    }
...
void BRepOffsetAPI_ThruSections::SetCriteriumWeight(const Standard_Real W1, const Standard_Real W2,
