...
static TopoDS_Vertex UpdateClosedEdge(const TopoDS_Edge& E, TopTools_SequenceOfShape& SV) {
    ...
    if (OnStart && OnEnd) {
        if (!VB[0].IsSame(VB[1])) {
#ifdef OCCT_DEBUG_ALGO
            if (AffichLoop) std::cout << "Two different vertices on the closing vertex" << std::endl;
#endif
        } else {
            SV.Remove(1);
...
static Standard_Boolean SelectEdge(const TopoDS_Face& F, const TopoDS_Edge& CE, const TopoDS_Vertex& CV,
                                   TopoDS_Edge& NE, TopTools_ListOfShape& LE) {
    TopTools_ListIteratorOfListOfShape itl;
    NE.Nullify();
#ifdef OCCT_DEBUG_ALGO
    if (AffichLoop) {
...
static void PurgeNewEdges(TopTools_DataMapOfShapeListOfShape& NewEdges, const TopTools_MapOfShape& UsedEdges) {
...
static void StoreInMVE(const TopoDS_Face& F, TopoDS_Edge& E, TopTools_IndexedDataMapOfShapeListOfShape& MVE,
                       Standard_Boolean& YaCouture, TopTools_DataMapOfShapeShape& VerticesForSubstitute) {
            ...
            if (P.IsEqual(P1, Tol) && !V.IsSame(V1)) {
                ...
                if (VerticesForSubstitute.IsBound(V1)) {
                    ...
                    if (VerticesForSubstitute.IsBound(V)) {
                        ...
                        if (!NewNewV.IsSame(V1)) VerticesForSubstitute.Bind(V1, NewNewV);
                    } else {
                        VerticesForSubstitute.Bind(V1, V);
                        TopTools_DataMapIteratorOfDataMapOfShapeShape mapit(VerticesForSubstitute);
                        for (; mapit.More(); mapit.Next())
                            if (mapit.Value().IsSame(V1)) VerticesForSubstitute(mapit.Key()) = V;
                    }
                }
...
void BRepAlgo_Loop::Perform() {
    ...
    for (itl.Initialize(myConstEdges); itl.More(); itl.Next()) {
        ...
        if (DejaVu.Add(E)) StoreInMVE(myFace, E, MVE, YaCouture, myVerticesForSubstitute);
    }
    ...
    while (MVE.Extent() > 0) {
        ...
        if (VF.IsSame(CV) && SamePnt2d(VF, EF, CE, myFace)) {
            ...
            if (AffichLoop) std::cout << "OpenWire is : NW_" << NbLoops << "_" << NbWires << std::endl;
        }
#endif
#ifdef DRAW
        if (AffichLoop) {
...
void BRepAlgo_Loop::CutEdge(const TopoDS_Edge& E, const TopTools_ListOfShape& VOnE, TopTools_ListOfShape& NE) const {
...
void BRepAlgo_Loop::UpdateVEmap(TopTools_IndexedDataMapOfShapeListOfShape& theVEmap) {
    ...
    for (Standard_Integer ii = 1; ii <= VerLver.Extent(); ii++) {
        ...
        for (itl.Initialize(aVlist); itl.More(); itl.Next()) {
            ...
            if (!aConstVertex.IsNull()) break;
        }
        if (aConstVertex.IsNull()) aConstVertex = TopoDS::Vertex(aVlist.First());
        aBB.UpdateVertex(aConstVertex, aCentre, aMaxTol);

        for (itl.Initialize(aVlist); itl.More(); itl.Next()) {
            const TopoDS_Vertex& aVertex = TopoDS::Vertex(itl.Value());
            if (aVertex.IsSame(aConstVertex)) continue;
