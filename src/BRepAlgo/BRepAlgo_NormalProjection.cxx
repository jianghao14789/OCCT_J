...
BRepAlgo_NormalProjection::BRepAlgo_NormalProjection()
    : myIsDone(Standard_False), myMaxDist(-1.), myWith3d(Standard_True), myFaceBounds(Standard_True) {
    BRep_Builder BB;
...
void BRepAlgo_NormalProjection::SetParams(const Standard_Real Tol3D, const Standard_Real Tol2D,
                                          const GeomAbs_Shape InternalContinuity, const Standard_Integer MaxDegree,
...
void BRepAlgo_NormalProjection::Build() {
        ...
        for (j = 1; j <= NbFaces; j++) {
            ...
            for (k = 1; k <= HProjector->NbCurves(); k++) {
                if (HProjector->IsSinglePnt(k, P2d)) {
                    ...
                    if (Only2d && Only3d) {
                        BRepLib_MakeEdge MKed(GeomAdaptor::MakeCurve(hcur->Curve()), Udeb, Ufin);
                        prj = MKed.Edge();
                        ...
                        if (appr.MaxError3d() > 1.e3 * myTol3d) continue;
                        ...
                        if (!Only2d) std::cout << "MaxError3d = " << appr.MaxError3d() << std::endl << std::endl;
                        if (!Only3d) {
                        ...
                        BB.UpdateEdge(TopoDS::Edge(prj), PCur2d, TopoDS::Face(Faces->Value(j)), appr.MaxError3d());
                    ...
                    if (myFaceBounds) {
                        ...
                        if (!Degenerated) {
                            ...
                            BRepTopAdaptor_FClass2d classifier(TopoDS::Face(Faces->Value(j)), Precision::Confusion());
                            gp_Pnt2d Puv;
                            Standard_Real f = PCur2d->FirstParameter();
                            Standard_Real l = PCur2d->LastParameter();
                            Standard_Real pmil = (f + l) / 2;
    ...
    if (init_count != 0) t_init /= init_count;
    if (init_point_count != 0) t_init_point /= init_point_count;
    if (dicho_bound_count != 0) t_dicho_bound /= dicho_bound_count;
    if (approx_count != 0) t_approx /= approx_count;
    if (booltool_count != 0) t_booltool /= booltool_count;
