        ...
        for (Standard_Integer i = 1; i < last; i++) {
            ...
            if (S.IsNull()) {
                ...
                try {
                    ...
                    Message::SendWarning()
                        << "Operation is not done: " << aName << " is not a valid transformation - scale = " << aScale;
                    return 0;
                }
            }
        }
...
static Standard_Boolean parseMinMax(const char** theArgVec, Bnd_Box& theBox) {
    ...
    if (!aMin[0].IsRealValue() || !aMin[1].IsRealValue() || !aMin[2].IsRealValue() || !aMax[0].IsRealValue() ||
        !aMax[1].IsRealValue() || !aMax[2].IsRealValue()) {
...
// purpose  :
//=======================================================================
static Standard_Integer BoundBox(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgVal) {
    // 1. Parse arguments
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNArg; ++anArgIter) {
        ...
        } else if (anAABB.IsVoid() && anArgIter + 5 < theNArg && parseMinMax(theArgVal + anArgIter, anAABB)) {
            anArgIter += 5;
        } else {
    ...
    if (anAABB.IsVoid() && aShape.IsNull()) {
        ...
        Message::SendFail()
            << "Syntax error: Options -obb, -optimal and -extToler cannot be used for explicitly defined AABB";
        return 1;
    ...
    if (isOBB) {
        ...
        if (anOBB.IsVoid()) {
            ...
            const gp_XYZ &aXDir = anOBB.XDirection(), &aYDir = anOBB.YDirection(), &aZDir = anOBB.ZDirection();
    ...
    } else // if(!isOBB)
    {
        ...
        if (anAABB.IsVoid()) {
            ...
            if (doPrint) {
                if (useOldSyntax) {
                    theDI << aMin.X() << " " << aMin.Y() << " " << aMin.Z() << " " << aMax.X() << " " << aMax.Y() << " "
                          << aMax.Z() << "\n";
                } else {
...
// purpose  :
//=======================================================================
static Standard_Integer IsBoxesInterfered(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgVal) {
    if (theNArg < 2) {
...
static Standard_Integer gbounding(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n != 2 && n != 3) {
        ...
        Standard_Boolean IsOptimal = Standard_False;
        if (n == 3 && !strcmp(a[2], "-o")) IsOptimal = Standard_True;
        ...
        if (!S.IsNull()) {
            ...
            if (!C.IsNull()) {
                ...
                if (!C2d.IsNull()) {
                    ...
                    Is3d = Standard_False;
                    if (IsOptimal)
                        BndLib_Add2dCurve::AddOptimal(C2d, C2d->FirstParameter(), C2d->LastParameter(),
                                                      Precision::Confusion(), B2d);
                    else
                        BndLib_Add2dCurve::Add(C2d, C2d->FirstParameter(), C2d->LastParameter(), Precision::Confusion(),
                                               B2d);
                } else {
                    di << "Wrong argument \n";
                    return 1;
...
static Standard_Integer maxtolerance(Draw_Interpretor& theCommands, Standard_Integer n, const char** a) {
    if (n < 2) return (1);
    ...
    sss << "\n## Tolerances on the shape " << a[1] << "  (nbFaces:" << nbF << "  nbEdges:" << nbE << " nbVtx:" << nbV
        << ")\n";
    sss.precision(5);
...
static Standard_Integer vecdc(Draw_Interpretor& di, Standard_Integer, const char**) {
    ...
    Handle(Draw_Marker3D) D0 = new Draw_Marker3D(gp_Pnt(P1.X(), P1.Y(), P1.Z()), Draw_Square, Draw_blanc, 1);
...
static Standard_Integer nproject(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n > arg)
        if (!strcmp(a[arg], "-d")) {
            arg++;
            if (n > arg) MaxDistance = Draw::Atof(a[arg++]);
...
static Standard_Integer scalexyz(Draw_Interpretor& /*di*/, Standard_Integer n, const char** a) {
    ...
    if (!aBRepGTrsf.IsDone()) throw Standard_ConstructionError("Scaling not done");
...
void BRepTest::BasicCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("addpcurve", "addpcurve edge 2dcurve face [tol (default 1.e-7)]", __FILE__, addpcurve, g);

    theCommands.Add("reset", "reset name1 name2 ..., remove location", __FILE__, transform, g);

    theCommands.Add("tmove", "tmove name1 name2 ... name, set location from name [-copy]", __FILE__, transform, g);

    theCommands.Add("ttranslate", "ttranslate name1 name2 ... dx dy dz [-copy]", __FILE__, transform, g);

    theCommands.Add("trotate", "trotate name1 name2 ... x y z dx dy dz angle [-copy]", __FILE__, transform, g);

    theCommands.Add("tmirror", "tmirror name x y z dx dy dz [-copy]", __FILE__, transform, g);

    theCommands.Add("tscale", "tscale name x y z scale [-copy]", __FILE__, transform, g);

    theCommands.Add("tcopy", "tcopy [-n(ogeom)] [-m(esh)] name1 result1 [name2 result2 ...]", __FILE__, tcopy, g);

    theCommands.Add("bmove", "bmove name1 name2 ... name, set location from name", __FILE__, transform, g);

    theCommands.Add("fmove", "fmove name1 name2 ... name, set location from name", __FILE__, transform, g);

    theCommands.Add("btranslate", "btranslate name1 name2 ... dx dy dz", __FILE__, transform, g);

    theCommands.Add("brotate", "brotate name1 name2 ... x y z dx dy dz angle", __FILE__, transform, g);
    ...
    theCommands.Add("fsameparameter",
                    ...
                    __FILE__, sameparameter, g);

    theCommands.Add("sameparameter", "sameparameter [result] shape [tol]", __FILE__, sameparameter, g);

    theCommands.Add("updatetolerance",
                    "updatetolerance [result] shape [param] \n  if [param] is absent - not verify of face tolerance, "
                    "else - perform it",
                    __FILE__, updatetol, g);

    theCommands.Add("solidorientation", "orientsolid myClosedSolid", __FILE__, orientsolid, g);

    theCommands.Add("getcoords", "getcoords vertex1 vertex 2... ; shows coords of input vertices", __FILE__, getcoords,
                    g);
    ...
    theCommands.Add("nurbsconvert", "nurbsconvert result name [result name]", __FILE__, nurbsconvert, g);
    ...
    theCommands.Add("reperageshape", "reperage shape -> list of shape (result of interstion shape , line)", __FILE__,
                    reperageshape, g);
    ...
    theCommands.Add("scalexyz", "scalexyz res shape factor_x factor_y factor_z", __FILE__, scalexyz, g);

    theCommands.Add("compare", "Compare shapes. Usage: compare shape1 shape2", __FILE__, compareshapes, g);
