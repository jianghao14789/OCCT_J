...
Standard_Integer props(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n < 2) {
        ...
        di << "If epsilon is given, exact geometry (curves, surfaces) are used for calculations independently of using "
              "key -tri\n\n";
        return 1;
    }
...
void BRepTest::GPropCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("lprops", "lprops name [x y z] [-skip] [-full] [-tri]: compute linear properties", __FILE__, props,
                    g);
    ...
    theCommands.Add("vpropsgk", "vpropsgk name epsilon closed span mode [x y z] [-skip] : compute volumic properties",
                    __FILE__, vpropsgk, g);
}
