...
static Standard_Integer vertex(Draw_Interpretor&, Standard_Integer n, const char** a) {
    if (n < 3) return 1;
    if (n >= 5) {
        DBRep::Set(a[1], BRepBuilderAPI_MakeVertex(gp_Pnt(Draw::Atof(a[2]), Draw::Atof(a[3]), Draw::Atof(a[4]))));
    } else if (n == 4) {
...
static Standard_Integer profile(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (!TheLocation.IsIdentity()) S.Move(TheLocation);
    ...
    while (not_done) {
        ...
        if (b == 2 || b == 3) {
            if (num_points == 2) {
                ...
                if (isplanar) {
                    MW.Add(BRepBuilderAPI_MakeEdge(curve3d_ptr, curve3d_ptr->FirstParameter(),
...
Standard_Integer mkoffset(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n >= 6 && strcmp(a[5], "i") == 0) theJoinType = GeomAbs_Intersection;
...
static Standard_Integer concatwire(Draw_Interpretor&, Standard_Integer n, const char** c) {
    ...
    if (n == 4) // check if it's C1 or G1
        if (!strcmp(c[3], "G1")) Option = GeomAbs_G1;
...
void BRepTest::CurveCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("mkoffset", "mkoffset result face/compound of wires  nboffset stepoffset [jointype(a/i) [alt]]",
                    __FILE__, mkoffset);

    theCommands.Add("openoffset", "openoffset result face/wire nboffset stepoffset [jointype(a/i)]", __FILE__,
                    openoffset);

    theCommands.Add("mkedge", "mkedge edge curve [surface] [pfirst plast] [vfirst [pfirst] vlast [plast]] ", __FILE__,
                    mkedge, g);
    ...
    theCommands.Add("edgeintersector", "edgeintersector r E1 E2 F [Tol]", __FILE__, edgeintersector, g);

    theCommands.Add("build3d", "build3d S [tol]", build3d, g);

    theCommands.Add("reducepcurves", "reducepcurves shape1 shape2 ...", reducepcurves, g);
