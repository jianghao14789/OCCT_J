...
static Standard_Integer plate(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    for (i = 1; i <= NbCurFront; i++) {
        ...
        if (Henri.Sense()->Value(iInOrder) == 1) {
            BRepBuilderAPI_MakeEdge ME(Henri.Curves2d()->Value(iInOrder), Henri.Surface(),
    ...
    for (i = 1; i <= NbPointConstraint; i++) {
        ...
        if (DrawTrSurf::GetPoint(a[Indice], P1)) {
            ...
            Handle(GeomPlate_PointConstraint) PCont =
                new GeomPlate_PointConstraint(u, v, BRep_Tool::Surface(HF->Face()), Conti, 0.001, 0.001, 0.001);
            Henri.Add(PCont);
        }
    }
    Handle(Draw_ProgressIndicator) aProgress = new Draw_ProgressIndicator(di, 1);
...
static Standard_Integer approxplate(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    for (i = 1; i <= NbCurFront; i++) {
        ...
        TopoDS_Edge E;
        if (Henri.Sense()->Value(iInOrder) == 1) {
            BRepBuilderAPI_MakeEdge ME(Henri.Curves2d()->Value(iInOrder), support,
                                       Fronts->Value(iInOrder)->LastParameter(),
                                       Fronts->Value(iInOrder)->FirstParameter());
            E = ME.Edge();
        } else {
...
static Standard_Integer filling(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    BRepOffsetAPI_MakeFilling MakeFilling(Degree, NbPtsOnCur, NbIter, Anisotropie, Tol2d, Tol3d, TolAng, TolCurv,
                                          MaxDeg, MaxSegments);
    TopoDS_Face InitFace = TopoDS::Face(DBRep::Get(a[5], TopAbs_FACE));
    if (!InitFace.IsNull()) MakeFilling.LoadInitSurface(InitFace);
    ...
    for (k = 1; k <= NbBounds; k++) {
        ...
        // History
        if (!E.IsNull()) ListForHistory.Append(E);
    }
    ...
    Standard_Real dmax = MakeFilling.G0Error(), angmax = MakeFilling.G1Error(), curvmax = MakeFilling.G2Error();
    ...
    // History
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(ListForHistory, MakeFilling);
...
void BRepTest::FillingCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("plate", "plate result nbrpntoncurve nbrcurfront edge face tang (0:vif;1:tang) ...", __FILE__,
                    plate, g);

    theCommands.Add("gplate",
                    "gplate result nbrcurfront nbrpntconst [SurfInit] [edge 0] [edge tang (1:G1;2:G2) surf]... [point] "
                    "[u v tang (1:G1;2:G2) surf] ...",
                    __FILE__, gplate, g);

    theCommands.Add(
        "approxplate",
        "approxplate result nbrpntoncurve nbrcurfront edge face tang (0:vif;1:tang) ... tol nmax degmax crit", __FILE__,
        approxplate, g);
