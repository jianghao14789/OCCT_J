...
static Standard_Integer CONTROL(Draw_Interpretor& theCommands, Standard_Integer narg, const char** a) {
    if (narg >= 2) {
...
static Standard_Integer SPLS(Draw_Interpretor&, Standard_Integer narg, const char** a) {
    ...
    if (isSplittingEdges) Spls.Add(aSplitEdges);
...
Standard_Integer thickshell(Draw_Interpretor& theCommands, Standard_Integer n, const char** a) {
    ...
    if (n > 4) {
        if (!strcmp(a[4], "i")) JT = GeomAbs_Intersection;
        if (!strcmp(a[4], "t")) JT = GeomAbs_Tangent;
    }
...
Standard_Integer offsetload(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    TheOffset.Initialize(S, Of, TheTolerance, BRepOffset_Skin, TheInter, 0, TheJoin, Standard_False, RemoveIntEdges);
    //------------------------------------------
    // recuperation et chargement des bouchons.
    //----------------------------------------
...
Standard_Integer offsetperform(Draw_Interpretor& theCommands, Standard_Integer theNArg, const char** a) {
...
static Standard_Integer DEFIN(Draw_Interpretor& theCommands, Standard_Integer narg, const char** a) {
    ...
    if (strcasecmp(a[0], "FEATPRISM") && strcasecmp(a[0], "FEATDPRISM") && strcasecmp(a[0], "FEATREVOL") &&
        strcasecmp(a[0], "FEATPIPE") && strcasecmp(a[0], "FEATLF") && strcasecmp(a[0], "FEATRF")) {
        return 1;
    }

    if ((!strcasecmp(a[0], "FEATPRISM") && narg != 9) || (!strcasecmp(a[0], "FEATREVOL") && narg != 12) ||
        (!strcasecmp(a[0], "FEATDPRISM") && narg != 7) || (!strcasecmp(a[0], "FEATPIPE") && narg != 7) ||
        (!strcasecmp(a[0], "FEATLF") && narg != 12) || (!strcasecmp(a[0], "FEATRF") && narg != 14)) {
...
static Standard_Integer BOSS(Draw_Interpretor& theCommands, Standard_Integer narg, const char** a) {
    ...
    if ((!strcasecmp(a[0], "ENDEDGES") && narg != 5) || (!strcasecmp(a[0], "FILLET") && (narg < 5 || narg % 2 != 1)) ||
...
//=============================================================================
static Standard_Integer ComputeSimpleOffset(Draw_Interpretor& theCommands, Standard_Integer narg, const char** a) {
    if (narg < 4) {
...
void BRepTest::FeatureCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("hole", " Performs a hole : hole result shape Or.X Or.Y Or.Z Dir.X Dir.Y Dir.Z Radius [Pfrom Pto]",
                    __FILE__, HOLE1, g);
    ...
    theCommands.Add("holecontrol", "Sets/Unsets or display controls on holes : holecontrol [0/1]", __FILE__, CONTROL,
                    g);

    theCommands.Add("wprism",
                    "Prisms wires on a face : wprism f[use]/c[ut] result shape [[FaceFrom] FaceUntil] VecX VecY VecZ  "
                    "SkecthFace wire1 [wire2 ....]",
                    __FILE__, PRW, g);

    theCommands.Add("fprism",
                    "Prisms a set of faces of a shape : fprism f[use]/c[ut] result shape [[FaceFrom] FaceUntil] VecX "
                    "VecY VecZ face1 [face2...]",
                    __FILE__, PRF, g);

    theCommands.Add("wrotate",
                    "Rotates wires on a face : wrotate f[use]/c[ut] result shape Angle/[FFrom] FUntil OX OY OZ DX DY "
                    "DZ SkecthFace wire1 [wire2 ....]",
                    __FILE__, ROW, g);

    theCommands.Add("frotate",
                    "Rotates a set of faces of a shape : frotate f[use]/c[ut] result shape Angle/[FaceFrom] FaceUntil "
                    "OX OY OZ DX DY DZ face1 [face2...]",
                    __FILE__, ROF, g);

    theCommands.Add(
        "splitshape",
        "splitshape result shape [splitedges] [face wire/edge/compound [wire/edge/compound ...][face "
        "wire/edge/compound [wire/edge/compound...] ...] [@ edgeonshape edgeonwire [edgeonshape edgeonwire...]]",
        __FILE__, SPLS, g);

    theCommands.Add("thickshell", "thickshell r shape offset [jointype [tol] ]", __FILE__, thickshell, g);

    theCommands.Add("offsetshape", "offsetshape r shape offset [tol] [face ...]", __FILE__, offsetshape, g);

    theCommands.Add("offsetcompshape", "offsetcompshape r shape offset [face ...]", __FILE__, offsetshape, g);

    theCommands.Add("offsetparameter", "offsetparameter Tol Inter(c/p) JoinType(a/i/t) [RemoveInternalEdges(r/k)]",
                    __FILE__, offsetparameter);

    theCommands.Add("offsetload", "offsetload shape offset bouchon1 bouchon2 ...", __FILE__, offsetload, g);

    theCommands.Add("offsetonface", "offsetonface face1 offset1 face2 offset2 ...", __FILE__, offsetonface, g);

    theCommands.Add("offsetperform", "offsetperform result", __FILE__, offsetperform, g);

    theCommands.Add("glue",
                    "glue result shapenew shapebase facenew facebase [facenew facebase...] [edgenew edgebase [edgenew "
                    "edgebase...]]",
                    __FILE__, GLU, g);
    ...
    theCommands.Add("featlf",
                    "Defines the arguments for a linear rib or slot : featlf shape wire plane DirX DirY DirZ DirX DirY "
                    "DirZ Fuse(0/1/2) Modify(0/1)",
                    __FILE__, DEFIN);

    theCommands.Add("featrf",
                    "Defines the arguments for a rib or slot of revolution : featrf shape wire plane X Y Z DirX DirY "
                    "DirZ Size Size Fuse(0/1/2) Modify(0/1)",
                    __FILE__, DEFIN);

    theCommands.Add("addslide", " Adds sliding elements : addslide prism/revol/pipe edge face [edge face...]", __FILE__,
                    ADD);

    theCommands.Add("featperform",
                    " Performs the prism revol dprism linform or pipe :featperform prism/revol/pipe/dprism/lf result "
                    "[[Ffrom] Funtil]",
                    __FILE__, PERF);
    ...
    theCommands.Add(
        "endedges",
        " Return top and bottom edges of dprism :endedges dprism shapetop shapebottom First/LastShape (1/2)", __FILE__,
        BOSS);

    theCommands.Add("fillet", " Perform fillet on compounds of edges :fillet result object rad1 comp1 rad2 comp2 ...",
                    __FILE__, BOSS);

    theCommands.Add("bossage",
                    " Perform fillet on top and bottom edges of dprism :bossage dprism result radtop radbottom "
                    "First/LastShape (1/2)",
                    __FILE__, BOSS);

    theCommands.Add("offsetshapesimple", "offsetshapesimple result shape offsetvalue [solid] [tolerance=1e-7]",
