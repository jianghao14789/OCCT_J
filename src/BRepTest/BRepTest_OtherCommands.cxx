...
static TopoDS_Face NextFaceForPrism(const TopoDS_Shape& shape, const TopoDS_Shape& basis, const gp_Ax1& ax1);
...
void BRepTest::OtherCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("shape", "shape name V/E/W/F/Sh/So/CS/C; make a empty shape", __FILE__, emptyshape, g);

    theCommands.Add("subshape", "subshape name V/E/W/F/Sh/So/CS/C index; get subsshape <index> of given type", __FILE__,
                    subshape, g);
...
Standard_Integer brepintcs(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (indshape < n - 1) {
        ...
        if (preci >= Precision::Confusion()) tol = preci;
    }
    int nbpi = 0;
    gp_Pnt curp;
    ...
    if (!nbpi) di << "Points of intersections are not found\n";
    if (indshape < n - 1) DBRep::Set(a[n - 1], aComp);
    // POP pour NT
    return 0;
}
//=======================================================================
...
Standard_Integer MakeShell(Draw_Interpretor& theDI, Standard_Integer, const char** a) {
    ...
    Offset.Initialize(aShape, Off, 1.0e-3, BRepOffset_Skin, Standard_True, Standard_False, GeomAbs_Arc);
    Offset.AddFace(F);
...
//=======================================================================
TopoDS_Face NextFaceForPrism(const TopoDS_Shape& shape, const TopoDS_Shape& basis, const gp_Ax1& ax1) {
