...
static Standard_Integer chamf_throat_with_penetration(Draw_Interpretor& di, Standard_Integer narg, const char** a) {
    if (narg < 7) return 1;
    ...
    while (i + NbArg <= narg) {
        ...
        if (!E.IsNull() && !F.IsNull() && (aMCh.Contour(E) == 0)) {
            ...
            if (offset > Precision::Confusion() && throat > offset) aMCh.Add(offset, throat, E, F);
        }
        i += NbArg;
    }
...
static Standard_Integer chamf_throat(Draw_Interpretor& di, Standard_Integer narg, const char** a) {
    ...
    while (i + 1 < narg) {
        ...
        if (!E.IsNull() && (aMCh.Contour(E) == 0)) {
            ...
            if (throat > Precision::Confusion()) aMCh.Add(throat, E);
        }
        i += 2;
    }
...
static Standard_Integer chamfer(Draw_Interpretor& di, Standard_Integer narg, const char** a) {
    ...
    if (narg == 1) {
        ...
        while (i + 1 < narg) {
            TopoDS_Shape aLocalEdge(DBRep::Get(a[i], TopAbs_EDGE));
            if (aLocalEdge.IsNull()) return 1;
            E = TopoDS::Edge(aLocalEdge);
            TopoDS_Shape aLocalFace(DBRep::Get(a[i + 1], TopAbs_FACE));
            if (aLocalFace.IsNull()) {
                // symmetric chamfer (one distance)
                d1 = atof(a[i + 1]);
                if (aMCh.Contour(E) == 0 && d1 > Precision::Confusion()) aMCh.Add(d1, E);
                i += 2;
            } else {
                F = TopoDS::Face(aLocalFace);
                ...
                if (i + 3 < narg) {
                    if (!strcasecmp(a[i + 2], "A") && i + 4 < narg) {
                        ...
                        angle *= M_PI / 180.;
                        if (aMCh.Contour(E) == 0 && d1 > Precision::Confusion() && angle > Precision::Confusion() &&
...
void BRepTest::ChamferCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("chamf_throat", "chamf_throat result shape edge throat", __FILE__, chamf_throat, g);

    theCommands.Add("chamf_throat_with_penetration",
                    "chamf_throat_with_penetration result shape edge face offset throat", __FILE__,
                    chamf_throat_with_penetration, g);
}
