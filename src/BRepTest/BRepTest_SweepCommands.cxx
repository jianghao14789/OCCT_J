...
static Standard_Integer geompipe(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (n > k) ByACR = (Draw::Atoi(a[k++]) == 1);
    if (n > k) rotate = (Draw::Atoi(a[k++]) == 1);
    ...
    TopoDS_Face F;
    if (!Sur.IsNull()) F = BRepBuilderAPI_MakeFace(Sur, Precision::Confusion());
    DBRep::Set(a[1], F);
    return 0;
}
...
Standard_Integer evolved(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    TopoDS_Shape Volevo = BRepOffsetAPI_MakeEvolved(Base, Prof, GeomAbs_Arc, !hasToComputeAxes, Solid, Standard_False,
...
Standard_Integer thrusections(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (Generator->IsDone()) {
        ...
        if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(Generator->Wires(), *Generator);
    } else {
...
//  mksweep
//=======================================================================
static Standard_Integer mksweep(Draw_Interpretor&, Standard_Integer n, const char** a) {
    if (n != 2) return 1;
...
static Standard_Integer setsweep(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    } else if (!strcmp(a[1], "-G")) // contour guide
    {
        if (n != 5) {
            ...
            Sweep->SetMode(TopoDS::Wire(Guide), CurvilinearEquivalence, (BRepFill_TypeOfContact)KeepContact);
        }
...
static Standard_Integer addsweep(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (Section.IsNull() || (Section.ShapeType() != TopAbs_WIRE && Section.ShapeType() != TopAbs_VERTEX)) {
...
void BRepTest::SweepCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("pipe", "pipe result Wire_spine Profile [Mode [Approx]], no args to get help", __FILE__, pipe, g);
    ...
    theCommands.Add("thrusections",
                    "thrusections [-N] result issolid isruled shape1 shape2 [..shape..], the option -N means no check "
                    "on wires, shapes must be wires or vertices (only first or last)",
                    __FILE__, thrusections, g);

    theCommands.Add("mksweep", "mksweep wire", __FILE__, mksweep, g);

    theCommands.Add("setsweep", "setsweep  no args to get help", __FILE__, setsweep, g);

    theCommands.Add("addsweep", "addsweep wire [vertex] [-M ] [-C] [auxiilaryshape]:no args to get help", __FILE__,
                    addsweep, g);

    theCommands.Add("deletesweep", "deletesweep wire, To delete a section", __FILE__, deletesweep, g);

    theCommands.Add("buildsweep", "builsweep [r] [option] [Tol] , no args to get help", __FILE__, buildsweep, g);
    ...
    theCommands.Add("geompipe", "geompipe r spineedge profileedge radius [byACR [byrotate]]", __FILE__, geompipe, g);

    theCommands.Add("middlepath", "middlepath res shape startshape endshape", __FILE__, middlepath, g);
}
