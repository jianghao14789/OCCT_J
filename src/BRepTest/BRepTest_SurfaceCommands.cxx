...
static Standard_Integer mkface(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (n == 3) {
        ...
        if (mkface)
            res = BRepBuilderAPI_MakeFace(S, Draw::Atof(a[3]), Draw::Atof(a[4]), Draw::Atof(a[5]), Draw::Atof(a[6]),
                                          Precision::Confusion());
        else
...
static Standard_Integer pcurve(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (n == 2) {
        ...
        for (Standard_Integer i = 1; ex.More(); ex.Next(), i++) {
            const Handle(Geom2d_Curve) c = BRep_Tool::CurveOnSurface(TopoDS::Edge(ex.Current()), TopoDS::Face(S), f, l);
            if (c.IsNull()) {
        ...
        Draw_Color col, savecol = DrawTrSurf_CurveColor(Draw_rouge);
        Standard_Real f, l;
        const Handle(Geom2d_Curve) c = BRep_Tool::CurveOnSurface(TopoDS::Edge(SE), TopoDS::Face(SF), f, l);
        Standard_Real fr = c->FirstParameter(), lr = c->LastParameter();
        Standard_Boolean IsPeriodic = c->IsPeriodic();
        if (c->DynamicType() == STANDARD_TYPE(Geom2d_TrimmedCurve)) {
            const Handle(Geom2d_Curve) & aC = Handle(Geom2d_TrimmedCurve)::DownCast(c)->BasisCurve();
...
static Standard_Integer sewing(Draw_Interpretor& theDi, Standard_Integer theArgc, const char** theArgv) {
    ...
    for (Standard_Integer i = 2; i < theArgc; i++) {
        if (theArgv[i][0] == '-' || theArgv[i][0] == '+') {
            ...
            if (!aShape.IsNull()) {
                ...
                if (Draw::Atof(theArgv[i])) aTol = Draw::Atof(theArgv[i]);
            }
        }
    }
    ...
    if (!aSetMinTol) aMinTol = aTol * 1e-4;
    if (aTol < Precision::Confusion()) aTol = Precision::Confusion();
    if (aMinTol < Precision::Confusion()) aMinTol = Precision::Confusion();
    if (aMinTol > aTol) {
    ...
    if (!aRes.IsNull()) DBRep::Set(theArgv[1], aRes);
    return 0;
}
...
// purpose  :
//=======================================================================
Standard_Integer fastsewing(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgVal) {
    if (theNArg < 3) {
        //                0         1       2     3         4
    ...
    if (aStatus) {
        theDI << "Error: There are some problems while adding (" << (static_cast<Standard_Integer>(aStatus)) << ")\n";
    ...
    if (aStatus) {
        theDI << "Error: There are some problems while performing (" << (static_cast<Standard_Integer>(aStatus))
              << ")\n";
        aFS.GetStatuses(&std::cout);
    }
...
void BRepTest::SurfaceCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("mkface", "mkface facename surfacename [ufirst ulast vfirst vlast] [wire [norient]]", __FILE__,
                    mkface, g);

    theCommands.Add("mkshell", "mkshell shellname surfacename [ufirst ulast vfirst vlast] [segment 0/1]", __FILE__,
                    mkface, g);
    ...
    theCommands.Add("sewing", "sewing result [tolerance] shape1 shape2 ... [min tolerance] [max tolerance] [switches]",
                    __FILE__, sewing, g);

    theCommands.Add("continuity", "continuity [tolerance] shape1 shape2 ...", __FILE__, continuity, g);

    theCommands.Add("encoderegularity", "encoderegularity shape [tolerance (in degree)]", __FILE__, encoderegularity,
                    g);

    theCommands.Add("fastsewing", "fastsewing result [-tol <value>] <list_of_faces>", __FILE__, fastsewing, g);
