...
static Standard_Boolean Contains(const Handle(TDataStd_ExtStringArray) & arr, const TCollection_ExtendedString& c) {
...
XmlMDataStd_ExtStringArrayDriver::XmlMDataStd_ExtStringArrayDriver(const Handle(Message_Messenger) & theMsgDriver)
...
//=======================================================================
Standard_Boolean XmlMDataStd_ExtStringArrayDriver::Paste(const XmlObjMgt_Persistent& theSource,
    ...
    if (aFirstIndex == NULL)
        ...
        TCollection_ExtendedString aMessageString = TCollection_ExtendedString("Cannot retrieve the first index"
    ...
    if (!anElement.getAttribute(::LastIndexString()).GetInteger(aLastInd)) {
        TCollection_ExtendedString aMessageString = TCollection_ExtendedString("Cannot retrieve the last index"
                                                                               " for ExtStringArray attribute as \"") +
                                                    aFirstIndex + "\"";
        myMessageDriver->Send(aMessageString, Message_Fail);
        return Standard_False;
    ...
    if (aSeparator.Type() != XmlObjMgt_DOMString::LDOM_NULL) separator = aSeparator.GetString();

    Handle(TDataStd_ExtStringArray) aExtStringArray = Handle(TDataStd_ExtStringArray)::DownCast(theTarget);
...
//=======================================================================
void XmlMDataStd_ExtStringArrayDriver::Paste(const Handle(TDF_Attribute) & theSource, XmlObjMgt_Persistent& theTarget,
                                             XmlObjMgt_SRelocationTable& theRelocTable) const {
    Handle(TDataStd_ExtStringArray) aExtStringArray = Handle(TDataStd_ExtStringArray)::DownCast(theSource);
    ...
    if (found) {
        ...
        if (!len) len++; // for end of line \0 symbol
        ...
        if (xstr.SearchFromEnd(c) == isym - 1) isym--; // replace the last separator by '\0'
