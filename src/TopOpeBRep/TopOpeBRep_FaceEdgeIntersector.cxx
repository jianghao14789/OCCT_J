...
#ifdef OCCT_DEBUG
    if (TopOpeBRep_GettraceFITOL()) std::cout << "ForceTolerance : myTol = " << myTol << std::endl;
#endif
}
...
Standard_Boolean TopOpeBRep_FaceEdgeIntersector::IsVertex(const TopoDS_Shape& S, const gp_Pnt& P,
...
Standard_Boolean TopOpeBRep_FaceEdgeIntersector::IsVertex(const Standard_Integer I, TopoDS_Vertex& VR) {
    Standard_Boolean isv = Standard_False;
    gp_Pnt P = Value();
    if (I == 1)
...
Standard_Real TopOpeBRep_FaceEdgeIntersector::ToleranceMax(const TopoDS_Shape& S, const TopAbs_ShapeEnum T) const {
