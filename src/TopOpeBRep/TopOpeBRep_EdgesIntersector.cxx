...
// purpose  :
//=======================================================================
void TopOpeBRep_EdgesIntersector::SetFaces(const TopoDS_Shape& F1, const TopoDS_Shape& F2, const Bnd_Box& B1,
                                           const Bnd_Box& B2) {
...
static Standard_Boolean TransitionEqualAndExtremity(const IntRes2d_Transition& T1, const IntRes2d_Transition& T2) {
...
static Standard_Boolean IsTangentSegment(const IntRes2d_IntersectionPoint& P1, const IntRes2d_IntersectionPoint& P2,
                                         const Geom2dAdaptor_Curve& aC1, const Geom2dAdaptor_Curve& aC2,
    ...
    if (aTrans1.TransitionType() == IntRes2d_Touch || aTrans2.TransitionType() == IntRes2d_Touch) {
        ...
        if (aSqrDistPP <= aTolConf) {
            ...
            if (aParDist1 * aParDist1 <= aResol1 && aParDist2 * aParDist2 <= aResol2) return Standard_True;
        }
    }
...
// purpose  :
//=======================================================================
void TopOpeBRep_EdgesIntersector::Perform(const TopoDS_Shape& E1, const TopoDS_Shape& E2,
    ...
    if (PC1.IsNull()) throw Standard_Failure("EdgesIntersector::Perform : no 2d curve");
    ...
    */
    ...
    if (nofilter) filter = Standard_False;
#endif
...
Standard_Boolean TopOpeBRep_EdgesIntersector::ComputeSameDomain() {
    ...
    if (!myHasSegment) return SetSameDomain(Standard_False);

    Standard_Boolean tt = (t1 == t2);
    if (!tt) return SetSameDomain(Standard_False);

    if (t1 == GeomAbs_Line) return SetSameDomain(Standard_True);
    ...
    Standard_Real u2, v2;
    p2.Coord(u2, v2);
    gp_Pnt P2 = BAS1.Value(u2, v2); // recall myCurve2=C2d(myEdge2,myFace1);
...
// purpose  :
//=======================================================================
Standard_Boolean TopOpeBRep_EdgesIntersector::ReduceSegment(TopOpeBRep_Point2d& psa, TopOpeBRep_Point2d& psb,
    ...
    if (cond) {
        ...
        gp_Pnt P3Dn((P3Dpsa.X() + P3Dpsb.X()) / 2, (P3Dpsa.Y() + P3Dpsb.Y()) / 2, (P3Dpsa.Z() + P3Dpsb.Z()) / 2);
        Pn.SetValue(P3Dn);
        ...
        if (isvpsa1) Pn.SetVertex(1, psa.Vertex(1));
        Standard_Boolean isvpsa2 = psa.IsVertex(2);
        if (isvpsa2) Pn.SetVertex(2, psa.Vertex(2));
        Standard_Boolean isvpsb1 = psb.IsVertex(1);
        if (isvpsb1) Pn.SetVertex(1, psb.Vertex(1));
        Standard_Boolean isvpsb2 = psb.IsVertex(2);
        if (isvpsb2) Pn.SetVertex(2, psb.Vertex(2));
    }
...
#else
void TopOpeBRep_EdgesIntersector::Dump(const TCollection_AsciiString& str, const Standard_Integer E1index,
