...
#ifdef OCCT_DEBUG
extern Standard_Boolean TopOpeBRep_GettraceNVP(Standard_Integer a, Standard_Integer b, Standard_Integer c,
                                               Standard_Integer d, Standard_Integer e);

Standard_Boolean GLOBAL_bvpr = Standard_False;

void debvpr() {};
void debvprmess(Standard_Integer f1, Standard_Integer f2, Standard_Integer il, Standard_Integer vp,
                Standard_Integer si) {
    std::cout << "f1,f2,il,vp,si : " << f1 << "," << f2 << "," << il << "," << vp << "," << si << std::endl;
    std::cout.flush();
    debvpr();
}
...
Standard_EXPORT Standard_Boolean FDS_LOIinfsup(const TopOpeBRepDS_DataStructure& BDS, const TopoDS_Edge& E,
                                               const Standard_Real pE, const Standard_Integer GIP,
                                               const TopOpeBRepDS_ListOfInterference& LOI, Standard_Real& pbef,
                                               Standard_Real& paft, Standard_Boolean& isonboundper);
Standard_EXPORT Standard_Boolean FUNBREP_topokpart(const Handle(TopOpeBRepDS_Interference) & Ifound,
                                                   ...
                                                   const TopoDS_Shape& F, const Standard_Real toluv,
...
Standard_EXPORT Standard_Boolean FUN_GetGonParameter(TopOpeBRepDS_ListIteratorOfListOfInterference& it,
...
void TopOpeBRep_FacesFiller::ProcessVPIonR(TopOpeBRep_VPointInterIterator& VPI, const TopOpeBRepDS_Transition& Trans,
...
//-----------------------------------------------------------------------
static void FUN_transForWL(const TopOpeBRep_LineInter& L, const Standard_Integer iVP, const Standard_Integer ShapeIndex,
...
//-----------------------------------------------------------------------
{
    ...
    for (VPIbis.Init(L); VPIbis.More(); VPIbis.Next()) {
        ...
        transLinebis = TopOpeBRep_FFTransitionTool::ProcessLineTransition(VPbis, shapeindexbis, edgeoribis);
...
//-----------------------------------------------------------------------
static void
FUN_VPgeometryfound(TopOpeBRep_FacesFiller& FF, const TopOpeBRep_LineInter& L, const TopOpeBRep_VPointInter& VP,
                    const Standard_Integer ShapeIndex, const Handle(TopOpeBRepDS_HDataStructure) & HDS,
                    const TopOpeBRepDS_ListOfInterference& DSCIL, TopOpeBRepDS_Kind& PVKind, Standard_Integer& PVIndex,
                    Standard_Boolean& EPIfound, Handle(TopOpeBRepDS_Interference) & IEPI, Standard_Boolean& CPIfound,
                    Handle(TopOpeBRepDS_Interference) & ICPI, Standard_Boolean& OOEPIfound,
...
FUN_VPIndex(TopOpeBRep_FacesFiller& FF, const TopOpeBRep_LineInter& L, const TopOpeBRep_VPointInter& VP,
            const Standard_Integer ShapeIndex, const Handle(TopOpeBRepDS_HDataStructure) & HDS,
            const TopOpeBRepDS_ListOfInterference& DSCIL, TopOpeBRepDS_Kind& PVKind, Standard_Integer& PVIndex, // out
...
//-----------------------------------------------------------------------
static Standard_Boolean FUN_LineRestF(const TopoDS_Face& F, const TopOpeBRep_LineInter& L,
...
Standard_EXPORT Standard_Boolean FUN_newtransEdge(const Handle(TopOpeBRepDS_HDataStructure) HDS,
                                                  const TopOpeBRep_FacesFiller& FF, const TopOpeBRep_LineInter& L,
                                                  const Standard_Boolean& Lonrest, const TopOpeBRep_VPointInter& VP,
                                                  const TopOpeBRepDS_Kind PVKind, const Standard_Integer PVIndex,
                                                  const Standard_Integer& OOShapeIndex, const TopoDS_Edge& edge,
                                                  const TopTools_ListOfShape& ERL, TopOpeBRepDS_Transition& T)
//-----------------------------------------------------------------------
{
    ...
    if (HDS->HasShape(edge)) {
        ...
        if (PVIndex == 0)
            ...
            FDS_LOIinfsup(HDS->DS(), edge, paredge, PVKind, PVIndex, HDS->DS().ShapeInterferences(edge), par1, par2,
                          isonper);
    } else
...
static Standard_Boolean FUN_sameGsameS(const TopOpeBRepDS_ListOfInterference& loI, const Standard_Integer& G,
                                       const Standard_Integer& S, TopOpeBRepDS_ListOfInterference& loIfound) {
    loIfound.Clear();
...
//-----------------------------------------------------------------------
static void
FUN_processCPI(TopOpeBRep_FacesFiller& FF, const TopOpeBRep_VPointInter& VP, const TopoDS_Shape& F,
               const Standard_Integer ShapeIndex, const TopOpeBRep_LineInter& L, TopOpeBRepDS_PDataStructure pDS,
               const TopOpeBRepDS_Transition& transLine, const TopOpeBRepDS_ListOfInterference& DSCIL,
               const Handle(TopOpeBRepDS_Interference) & Ifound, const Standard_Boolean& Gfound,
               const TopOpeBRepDS_Kind& PVKind, const Standard_Integer& PVIndex, Standard_Integer& keptVPnbr)
//-----------------------------------------------------------------------
{
    Standard_Integer OOShapeIndex = (ShapeIndex == 1) ? 2 : 1;
...
void TopOpeBRep_FacesFiller::ProcessVPonR(const TopOpeBRep_VPointInter& VP, const TopOpeBRepDS_Transition& Trans,
                                          ...
                                          const Standard_Integer ShapeIndex) // 1,2
{
    ...
    ProcessVPondgE(VP, ShapeIndex, PVKind, PVIndex, // out
                   EPIfound, IEPI,                  // out
                   CPIfound, ICPI);                 // out
    ...
    if (setlastonwl) lasttransLine = myDSCIL.Last()->Transition();
    ...
    if (trliunk) {
        ...
        if (wline) {
            ...
            if (transLine.IsUnknown()) {
                ...
                if (absindex == 3) keepvpfirst = keepvpfirst && myLastVPison0;
    ...
    if (newtransEdge) {
        ...
        if (newtransEdge) {
            ...
            TopOpeBRepDS_Transition Tr;
            Standard_Boolean ok = FUN_newtransEdge(myHDS, (*this), (*myLine), myLineIsonEdge, VP, PVKind, PVIndex,
                                                   OOShapeIndex, edge, myERL, Tr);
            if (ok) {
                transEdge.Before(Tr.Before());
                transEdge.After(Tr.After());
            }
    ...
    Standard_Boolean ismultiplekind =
        foundPVIndex && !EPIfound && !CPIfound && (SIisvertex || OOisvertex) && (PVKind == TopOpeBRepDS_POINT);

    //   if (!foundPVIndex) FUN_VPIndex ((*this),(*myLine),VP,ShapeIndex,myHDS,myDSCIL, //in
    if (!foundPVIndex || ismultiplekind)
        FUN_VPIndex((*this), (*myLine), VP, ShapeIndex, myHDS, myDSCIL, // in
    ...
    if (PVKind == TopOpeBRepDS_VERTEX) VPonedge = ::FUN_onedge(PDS, edge);
    if (myLineINL) {
        ...
        if (ok) dot = gp_Dir(tgE).Dot(gp_Dir(ntOOF));
    ...
    if (condi) {
        ...
        FUN_processCPI((*this), VP, Face, ShapeIndex, (*myLine), myDS, transLine, myDSCIL, ICPI, Gfound, PVKind,
                       PVIndex, keptVPnbr);
    ...
    if (correctON) {
        ...
        Standard_Boolean isonper;
        FDS_LOIinfsup((*myDS), OOedge, OOparedge, PVKind, PVIndex, myDS->ShapeInterferences(OOedge), OOpar1, OOpar2,
                      isonper);
        ...
        if (ok && !(interf2d && !MKT.IsT2d())) {
            ...
            if (ok) {
                ...
                if (addEPIf) {
                    TopOpeBRepDS_Transition OOT = tOOedge;
                    OOT.Index(iSIFace);
                    Handle(TopOpeBRepDS_Interference) OOEPIf =
                        MakeEPVInterference(OOT, iSIFace, PVIndex, OOparedge, PVKind, TopOpeBRepDS_FACE, OOisvertex);
