...
extern void debvprmess(Standard_Integer f1, Standard_Integer f2, Standard_Integer il, Standard_Integer vp,
                       Standard_Integer si);
...
Standard_EXPORT void FUN_VPIndex(TopOpeBRep_FacesFiller& FF, const TopOpeBRep_LineInter& L,
                                 const TopOpeBRep_VPointInter& VP, const Standard_Integer ShapeIndex,
                                 const Handle(TopOpeBRepDS_HDataStructure) & HDS,
                                 const TopOpeBRepDS_ListOfInterference& DSCIL, TopOpeBRepDS_Kind& PVKind,
...
static Standard_Boolean FUN_IwithsuppiS(const TopOpeBRepDS_ListOfInterference& loI, const Standard_Integer iS,
...
static Standard_Boolean FUN_IwithsuppkS(const TopOpeBRepDS_ListOfInterference& loI, const TopOpeBRepDS_Kind& kS,
                                        TopOpeBRepDS_ListOfInterference& loIfound) {
    TopOpeBRepDS_ListIteratorOfListOfInterference it(loI);
    for (; it.More(); it.Next()) {
        const Handle(TopOpeBRepDS_Interference) & I = it.Value();
...
static Standard_Boolean FUN_IwithToniS(const TopOpeBRepDS_ListOfInterference& loI, const Standard_Integer iS,
                                       TopOpeBRepDS_ListOfInterference& loIfound) {
    TopOpeBRepDS_ListIteratorOfListOfInterference it(loI);
    for (; it.More(); it.Next()) {
        const Handle(TopOpeBRepDS_Interference) & I = it.Value();
...
static Standard_Boolean FUN_supponF(const TopOpeBRepDS_PDataStructure pDS, const TopOpeBRepDS_ListOfInterference& loI,
                                    const Standard_Integer iF, TopOpeBRepDS_ListOfInterference& lIsupponF,
...
static Standard_Boolean FUN_IoflSsuppS(const TopOpeBRepDS_PDataStructure pDS, const Standard_Integer iS,
                                       const TColStd_ListOfInteger& lShape, TopOpeBRepDS_ListOfInterference& IsuppiS) {
...
//=======================================================================
static Standard_Boolean FUN_findTF(const TopOpeBRepDS_PDataStructure pDS, const Standard_Integer iE,
                                   const Standard_Integer, const Standard_Integer iOOF, TopOpeBRepDS_Transition& TF) {
...
static Standard_Boolean FUN_findTOOF(const TopOpeBRepDS_PDataStructure pDS, const Standard_Integer iE,
                                     const Standard_Integer iF, const Standard_Integer iOOF,
...
void TopOpeBRep_FacesFiller::ProcessRLine() {
    ...
    IFE = TopOpeBRepDS_InterferenceTool::MakeFaceEdgeInterference(T1, iF2, iErest, isedge1, TopOpeBRepDS_UNSHGEOMETRY);
    myHDS->StoreInterference(IFE, iF1);

    IFE = TopOpeBRepDS_InterferenceTool::MakeFaceEdgeInterference(T2, iF1, iErest, isedge2, TopOpeBRepDS_UNSHGEOMETRY);
    ...
    for (; VPI.More(); VPI.Next()) {
        ...
        for (Standard_Integer ShapeIndex = 1; ShapeIndex <= 2; ShapeIndex++) {
            ...
            if (PVIndex == 0) {
                //	Standard_Boolean EPIfound=Standard_False,CPIfound=Standard_False; Handle(TopOpeBRepDS_Interference)
                //IEPI,ICPI;
                FUN_VPIndex((*this), (*myLine), VP, ShapeIndex, myHDS, myDSCIL, PVKind, PVIndex, EPIfound, IEPI,
                            CPIfound, ICPI, M_GETVP);
                Standard_Boolean Efound = (EPIfound || CPIfound);
                Standard_Boolean Ifound = (PVIndex != 0);
                Standard_Boolean condmake = (!Efound && !Ifound);
                if (condmake) {
            ...
            if (Tunk) {
                if (SIErest) {
                    ...
                    Standard_Boolean ok = FUN_newtransEdge(myHDS, (*this), (*myLine), myLineIsonEdge, VP, PVKind,
                                                           PVIndex, OOShapeIndex, edge, myERL, Tr);
                    if (!ok) continue;
...
void TopOpeBRep_FacesFiller::FillLine() {
    ...
    for (VPI.Init((*myLine)); VPI.More(); VPI.Next()) {
        ...
        CPI = TopOpeBRepDS_InterferenceTool::MakeCurveInterference(transLine, TopOpeBRepDS_CURVE, 0, PVKind, PVIndex,
                                                                   parline);
        StoreCurveInterference(CPI);
...
Standard_Boolean TopOpeBRep_FacesFiller::GetGeometry(TopOpeBRepDS_ListIteratorOfListOfInterference& IT,
                                                     const TopOpeBRep_VPointInter& VP, Standard_Integer& G,
                                                     TopOpeBRepDS_Kind& K) {
...
Standard_Integer TopOpeBRep_FacesFiller::MakeGeometry(const TopOpeBRep_VPointInter& VP,
                                                      const Standard_Integer ShapeIndex, TopOpeBRepDS_Kind& K) {
    Standard_Integer G;
...
// purpose  :
//=======================================================================
Standard_Boolean TopOpeBRep_FacesFiller::GetFFGeometry(const TopOpeBRepDS_Point& DSP, TopOpeBRepDS_Kind& K,
...
// purpose  :
//=======================================================================
Standard_Boolean TopOpeBRep_FacesFiller::GetFFGeometry(const TopOpeBRep_VPointInter& VP, TopOpeBRepDS_Kind& K,
                                                       Standard_Integer& G) const {
    TopOpeBRepDS_Point DSP = TopOpeBRep_PointGeomTool::MakePoint(VP);
