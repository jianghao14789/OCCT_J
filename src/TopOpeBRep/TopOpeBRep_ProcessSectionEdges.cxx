...
Standard_EXPORT Standard_Boolean FUN_brep_sdmRE(
    const TopoDS_Edge& E1, const TopoDS_Edge& E2) { // prequesitory : E1, E2 are restriction edges of opposite rank
...
void TopOpeBRep_FacesFiller::ProcessSectionEdges() {
    ...
    for (itLES.Initialize(LES), itLOI.Initialize(LOI); itLES.More(), itLOI.More(); itLES.Next(), itLOI.Next()) {
    ...
    for (itmapELE.Initialize(mapELE); itmapELE.More(); itmapELE.Next()) {
        ...
        for (; itL.More(); itL.Next()) {
            ...
            myDS->FillShapesSameDomain(E1, E2, TopOpeBRepDS_UNSHGEOMETRY, TopOpeBRepDS_UNSHGEOMETRY, refFirst);
        }
    }
}
