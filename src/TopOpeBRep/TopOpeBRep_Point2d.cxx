...
TopOpeBRep_Point2d::TopOpeBRep_Point2d()
    : myhaspint(Standard_False), myisvertex1(Standard_False), myparameter1(0.), myisvertex2(Standard_False),
      myparameter2(0.), myispointofsegment(Standard_False), myips1(0), myips2(0), myhasancestors(Standard_False),
      mystatus(TopOpeBRep_P2DUNK), myindex(0), mykeep(Standard_True), myedgesconfig(TopOpeBRepDS_UNSHGEOMETRY),
      mytolerance(0.) {}
...
void TopOpeBRep_Point2d::Dump(const Standard_Integer E1index, const Standard_Integer E2index) const {
    ...
    TopoDS_Vertex V1;
    if (isvertex1) V1 = Vertex(1);
    Standard_Boolean isvertex2 = IsVertex(2);
    TopoDS_Vertex V2;
    if (isvertex2) V2 = Vertex(2);
    ...
    gp_Pnt P3D = Value();
#ifdef DRAW
    std::cout << FUN_tool_PRODINP() << "P" << Index() << " " << P3D.X() << " " << P3D.Y() << " " << P3D.Z()
              << "; # tol = " << tol << std::endl;
#endif
    std::cout << "     on (1) :";
    std::cout << " vertex(1) : ";
