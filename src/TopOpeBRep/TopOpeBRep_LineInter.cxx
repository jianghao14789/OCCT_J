...
//-----------------------------------------------------------------------
static void FUN_ALINETOWLINE(const Handle(IntPatch_ALine) & AL, const Handle(BRepAdaptor_Surface) surf1,
                             const Handle(BRepAdaptor_Surface) surf2, IntPatch_SequenceOfLine& theLines) {
...
void TopOpeBRep_LineInter::SetLine(const Handle(IntPatch_Line) & L, const BRepAdaptor_Surface& S1,
    ...
    if (myTypeLineCurve == TopOpeBRep_ANALYTIC) {
        ...
        if (aSLin.Length() > 0) myILW = Handle(IntPatch_WLine)::DownCast(aSLin.Value(1));
...
void TopOpeBRep_LineInter::VPBounds(Standard_Integer& f, Standard_Integer& l, Standard_Integer& n) const {
    if (!myVPBDefined) {
...
Handle(Geom_Curve) TopOpeBRep_LineInter::Curve(const Standard_Real parmin, const Standard_Real parmax) const {
...
Standard_Boolean TopOpeBRep_LineInter::HasFirstPoint() const {
    if (myILG.IsNull()) throw Standard_ProgramError("TopOpeBRep_LineInter::HasFirstPoint sur line != GLine");
...
Standard_Boolean TopOpeBRep_LineInter::HasLastPoint() const {
    if (myILG.IsNull()) throw Standard_ProgramError("TopOpeBRep_LineInter::HasLastPoint sur line != GLine");
...
#else
    (const Standard_Integer I, const TCollection_AsciiString& s1, const TCollection_AsciiString& s2) const {
...
#else
    (const TopOpeBRep_Bipoint& bip, const TCollection_AsciiString& s1, const TCollection_AsciiString& s2) const {
