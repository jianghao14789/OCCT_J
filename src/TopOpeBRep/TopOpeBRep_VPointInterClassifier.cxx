...
static TopAbs_State SlowClassifyOnBoundary(const gp_Pnt& thePointToClassify, const gp_Pnt2d& thePoint2dToClassify,
                                           BRepClass_FaceClassifier& theSlowClassifier, const TopoDS_Face& theFace);
...
TopOpeBRep_VPointInterClassifier::TopOpeBRep_VPointInterClassifier() : myState(TopAbs_UNKNOWN) {}
...
TopAbs_State TopOpeBRep_VPointInterClassifier::VPointPosition(const TopoDS_Shape& F, TopOpeBRep_VPointInter& VP,
    ...
    if (AssumeINON && FaceClassifyIndex == VPSI) {
        ...
        if (myState == TopAbs_ON) {
            ...
            if (myState == TopAbs_ON) {
                ...
                VP.EdgeON(mySlowFaceClassifier.Edge().Edge(), mySlowFaceClassifier.EdgeParameter(), FaceClassifyIndex);
            }
        ...
        if (statefast == TopAbs_OUT || statefast == TopAbs_ON) {
            ...
            if (myState == TopAbs_ON) {
                ...
                if (myState == TopAbs_ON) {
                    // modified by NIZHNY-MKK  Mon Jun 19 11:45:36 2000.END
                    VP.EdgeON(mySlowFaceClassifier.Edge().Edge(), mySlowFaceClassifier.EdgeParameter(),
                              FaceClassifyIndex);
                }
            }
        }
...
static TopAbs_State SlowClassifyOnBoundary(const gp_Pnt& thePointToClassify, const gp_Pnt2d& thePoint2dToClassify,
    ...
    if (!anEdgeCurve.IsNull()) {
        ...
        for (; anExp.More() && !samewithvertex; anExp.Next()) {
            ...
            if (thePointToClassify.IsEqual(anEdgeVertexPoint, aVertexTolerance)) samewithvertex = Standard_True;
        }
        if (samewithvertex) return TopAbs_ON;
