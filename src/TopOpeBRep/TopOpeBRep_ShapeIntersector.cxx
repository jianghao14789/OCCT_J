...
extern Standard_Boolean TopOpeBRep_GettraceEEFF(const Standard_Integer e1, const Standard_Integer e2,
                                                const Standard_Integer f1, const Standard_Integer f2);
void seteeff(const Standard_Boolean b, const Standard_Integer e1, const Standard_Integer e2, const Standard_Integer f1,
             const Standard_Integer f2) {
    std::cout << "b,e1,e2,f1,f2 : " << b << " " << e1 << "," << e2 << "," << f1 << "," << f2 << std::endl;
    TopOpeBRep_SettraceEEFF(b);
}
void seteefft(const Standard_Integer e1, const Standard_Integer e2, const Standard_Integer f1,
              const Standard_Integer f2) {
    seteeff(Standard_True, e1, e2, f1, f2);
}
void seteefff(const Standard_Integer e1, const Standard_Integer e2, const Standard_Integer f1,
              const Standard_Integer f2) {
...
// purpose  :
//=======================================================================
void TopOpeBRep_ShapeIntersector::RejectedFaces(const TopoDS_Shape& anObj, const TopoDS_Shape& aReference,
...
Standard_Boolean TopOpeBRep_ShapeIntersector::MoreIntersection() const {
    ...
    if (TopOpeBRep_GettraceSI() && res) {
        ...
        if (myEEFFDone) {
            ...
            std::cout << "    trc teeff 1 " << ie1 << " " << ie2 << " " << if1 << " " << if2 << "; # ie1 ie2 if1 if2"
                      << std::endl;
...
static Standard_Integer OneShapeIsHalfSpace(const TopoDS_Shape& S1, const TopoDS_Shape& S2) {
    ...
    if (S1.ShapeType() == TopAbs_SOLID && S2.ShapeType() == TopAbs_SOLID) {
        ...
        if ((NbFacesSol1 == 1 && NbFacesSol2 >= 2) || (NbFacesSol2 == 1 && NbFacesSol1 >= 2)) {
            ...
            if (SolidIsSphereOrTorus) return result;
            ...
            if (SecondShellOk) result = (NbFacesSol1 == 1) ? 1 : 2;
        } else {
            // ************************** IMPORTANT !!! ************************************
...
#ifdef OCCT_DEBUG
        if (result != 0) std::cout << "# one of the SOLIDs probably is a HALF SPACE" << std::endl;
#endif
    }
