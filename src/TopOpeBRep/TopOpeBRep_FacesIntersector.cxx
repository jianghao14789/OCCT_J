...
static void SAVFF(const TopoDS_Face& F1, const TopoDS_Face& F2) {
    ...
    if (SAVFFi1) an1 = an1 + SAVFFi1;
    TCollection_AsciiString an2("SAVB");
    if (SAVFFi2) an2 = an2 + SAVFFi2;
...
static Handle(IntPatch_RLine)
    BuildRLineBasedOnWLine(const Handle(IntPatch_WLine) & theWLine, const Handle(Adaptor2d_Curve2d) & theArc,
                           const Standard_Integer theRank);

static Handle(IntPatch_RLine) BuildRLine(const IntPatch_SequenceOfLine& theSeqOfWLine, const Standard_Integer theRank,
                                         const Handle(Adaptor3d_Surface) & theSurface,
                                         const Handle(Adaptor3d_TopolTool) & theDomain, const Standard_Real theTolArc);

static void TestWLinesToAnArc(IntPatch_SequenceOfLine& slin, const Handle(Adaptor3d_Surface) & theSurface1,
                              const Handle(Adaptor3d_TopolTool) & theDomain1,
                              const Handle(Adaptor3d_Surface) & theSurface2,
                              const Handle(Adaptor3d_TopolTool) & theDomain2, const Standard_Real theTolArc);
...
static void MergeWLinesIfAllSegmentsAlongRestriction(IntPatch_SequenceOfLine& theSlin,
                                                     ...
                                                     const Standard_Real theTolArc);
//------------------------------------------------------------------------------------------------
static Standard_Integer GetArc(IntPatch_SequenceOfLine& theSlin, const Standard_Integer& theRankS,
                               const Handle(Adaptor3d_Surface) & theSurfaceObj,
                               const Handle(Adaptor3d_TopolTool) & theDomainObj,
                               const Handle(Adaptor3d_Surface) & theSurfaceTool, const gp_Pnt& theTestPoint,
                               Standard_Real& theVrtxTol, Handle(IntSurf_LineOn2S) & theLineOn2S,
                               Standard_Real& theFirst, Standard_Real& theLast);
//------------------------------------------------------------------------------------------------
static Standard_Boolean IsPointOK(const gp_Pnt& theTestPnt, const Adaptor3d_Surface& theTestSurface,
                                  const Standard_Real& theTol);
//-------------------------------------------------------------------------------------------------
static Standard_Boolean GetPointOn2S(const gp_Pnt& theTestPnt, const Adaptor3d_Surface& theTestSurface,
                                     const Standard_Real& theTol, Extrema_POnSurf& theResultPoint);
//-------------------------------------------------------------------------------------------------------------------------
static Handle(IntPatch_WLine)
    GetMergedWLineOnRestriction(IntPatch_SequenceOfLine& theSlin, const Standard_Real& theVrtxTol,
...
// purpose  :
//=======================================================================
void TopOpeBRep_FacesIntersector::Perform(const TopoDS_Shape& F1, const TopoDS_Shape& F2, const Bnd_Box& B1,
                                          const Bnd_Box& B2) {
#ifdef OCCT_DEBUG
    ...
    myIntersector.Perform(mySurface1, myDomain1, mySurface2, myDomain2, myTol1, myTol2, Standard_True, Standard_True,
                          Standard_False);
...
Standard_Boolean TopOpeBRep_FacesIntersector::SameDomain() const {
    if (!myIntersectionDone) throw Standard_ProgramError("FacesIntersector : bad SameDomain");
...
void TopOpeBRep_FacesIntersector::PrepareLines() {
    ...
    if (newV) {
        ...
        IntPatch_LineConstructor** Ptr = (IntPatch_LineConstructor**)malloc(n * sizeof(IntPatch_LineConstructor*));
        for (i = 1; i <= n; i++) {
            Ptr[i - 1] = new IntPatch_LineConstructor(2);
            Ptr[i - 1]->Perform(myIntersector.SequenceOfLine(), myIntersector.Line(i), mySurface1, myDomain1,
                                mySurface2, myDomain2, myTol1);
        ...
        MergeWLinesIfAllSegmentsAlongRestriction(aSeqOfResultLines, mySurface1, myDomain1, mySurface2, myDomain2,
                                                 myTol1);
        myLineNb = aSeqOfResultLines.Length();
...
void TopOpeBRep_FacesIntersector::ForceTolerances(const Standard_Real Tol1, const Standard_Real Tol2) {
    myTol1 = Tol1;
    myTol2 = Tol2;
...
Standard_Real TopOpeBRep_FacesIntersector::ToleranceMax(const TopoDS_Shape& S, const TopAbs_ShapeEnum T) const {
...
static Handle(IntPatch_RLine)
    ...
    if ((theRank != 1) && (theRank != 2)) return anRLine;
    ...
    if (Abs(par1 - par2) < theArc->Resolution(tol)) return anRLine;
    ...
    if (par1 < par2) {
        ...
        VtxFirst.SetArc(IsOnFirst, //-- On First
                        theArc, par1, TransitionUndecided, TransitionUndecided);
        ...
        VtxLast.SetArc(IsOnFirst, //-- On First
                       theArc, par2, TransitionUndecided, TransitionUndecided);
...
static Handle(IntPatch_RLine) BuildRLine(const IntPatch_SequenceOfLine& theSeqOfWLine, const Standard_Integer theRank,
    ...
    if (aV1->IsSame(aV2)) return anRLine;
    ...
    for (theDomain->Init(); theDomain->More(); theDomain->Next()) {
        ...
        for (theDomain->InitVertexIterator(); (!foundVertex1 || !foundVertex2) && theDomain->MoreVertex();
             theDomain->NextVertex()) {

            if (!foundVertex1 && aV1->IsSame(theDomain->Vertex())) foundVertex1 = Standard_True;
            if (!foundVertex2 && aV2->IsSame(theDomain->Vertex())) foundVertex2 = Standard_True;
        }
        ...
        if (foundVertex1 && foundVertex2) {
            ...
            for (Standard_Integer i = 1; buildrline && i <= theSeqOfWLine.Length(); i++) {
                const Handle(IntPatch_WLine) & aWLine = *((Handle(IntPatch_WLine)*)&(theSeqOfWLine.Value(i)));
            ...
            if (buildrline) {
                ...
                for (Standard_Integer j = 1; j <= theSeqOfWLine.Length(); j++) {
                    const Handle(IntPatch_WLine) & atmpWLine = *((Handle(IntPatch_WLine)*)&(theSeqOfWLine.Value(j)));
                ...
                Handle(IntPatch_WLine) emulatedWLine = new IntPatch_WLine(aLineOn2S, Standard_False, trans1, trans2);
...
static void TestWLinesToAnArc(IntPatch_SequenceOfLine& slin, const Handle(Adaptor3d_Surface) & theSurface1,
    ...
    for (Standard_Integer rank = 1; rank <= 2; rank++) {
        for (Standard_Integer i = 1; i <= slin.Length(); i++) {
            if (slin.Value(i)->ArcType() != IntPatch_Walking) continue;
            ...
            if (isvertex) {
                ...
                if (appendwline) aSeqOfWLine.Append(aWLine);
            } else {
                if (aSeqOfWLine.Length() == 0) continue;
...
static void MergeWLinesIfAllSegmentsAlongRestriction(IntPatch_SequenceOfLine& theSlin,
    ...
    for (rank = 1; rank <= 2; rank++) {
        ...
        for (i = 1; i <= theSlin.Length(); i++) {
            if (theSlin.Value(i)->ArcType() != IntPatch_Walking) continue;
            NbWLines++;
            ...
            if (rank == 1) {
                ...
                if (TestWLineAlongRestriction(aWLine, rank, theSurface1, theDomain1, theTolArc)) WLOnRS1++;
            } else {
                if (TestWLineAlongRestriction(aWLine, rank, theSurface2, theDomain2, theTolArc)) WLOnRS2++;
            }
        }
    ...
    if (sqVertexPoints.Length() <= 2) return;
    if (sqVertexPoints.Value(1).IsEqual(sqVertexPoints.Value(sqVertexPoints.Length()), tol)) return;
...
#ifdef OCCT_DEBUG
        std::cout << "*** TopOpeBRep_FaceIntersector: Merge WLines on Restriction " << ((WLineRank == 1) ? "S1" : "S2")
                  << " to WLine***" << std::endl;
#endif
    }
}
...
static Standard_Boolean IsPointOK(const gp_Pnt& theTestPnt, const Adaptor3d_Surface& theTestSurface,
    ...
    if (extPS.IsDone() && extPS.NbExt() > 0) {
        ...
        if (MinDist2 <= theTol * theTol) result = Standard_True;
    }
    return result;
}
