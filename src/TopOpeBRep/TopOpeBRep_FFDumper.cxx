...
void TopOpeBRep_FFDumper::DumpLine(const TopOpeBRep_LineInter& LI) {
    ...
    if (isrest) {
        ...
        if (FIisrest) {
            ...
            if (myPFF->ChangeDataStructure().HasShape(Erest)) iErest = myPFF->ChangeDataStructure().Shape(Erest);
            std::cout << " " << iErest;
        }
        std::cout << std::endl;
    }
...
#else
void TopOpeBRep_FFDumper::DumpDSP(const TopOpeBRep_VPointInter& VP, const TopOpeBRepDS_Kind GK,
                                  const Standard_Integer G, const Standard_Boolean newinDS) const {
    std::cout << "VP " << VP.Index() << " on " << VP.ShapeIndex();
    if (newinDS) {
