...
void RWStepGeom_RWBSplineSurfaceWithKnots::ReadStep(const Handle(StepData_StepReaderData) & data,
        ...
        for (Standard_Integer i4 = 1; i4 <= nbi4; i4++) {
            ...
            if (data->ReadSubList(nsub4, i4, "sub-part(control_points_list)", ach, nsi4)) {
                for (Standard_Integer j4 = 1; j4 <= nbj4; j4++) {
                    ...
                    if (data->ReadEntity(nsi4, j4, "cartesian_point", ach, STANDARD_TYPE(StepGeom_CartesianPoint),
                                         anent4))
    ...
    if (data->ReadSubList(num, 11, "u_knots", ach, nsub11)) {
        ...
        for (Standard_Integer i11 = 1; i11 <= nb11; i11++) {
            ...
            if (data->ReadReal(nsub11, i11, "u_knots", ach, aUKnotsItem)) aUKnots->SetValue(i11, aUKnotsItem);
        }
    }
    ...
    if (data->ReadSubList(num, 12, "v_knots", ach, nsub12)) {
        ...
        for (Standard_Integer i12 = 1; i12 <= nb12; i12++) {
            ...
            if (data->ReadReal(nsub12, i12, "v_knots", ach, aVKnotsItem)) aVKnots->SetValue(i12, aVKnotsItem);
        }
    }
    ...
    ent->Init(aName, aUDegree, aVDegree, aControlPointsList, aSurfaceForm, aUClosed, aVClosed, aSelfIntersect,
              aUMultiplicities, aVMultiplicities, aUKnots, aVKnots, aKnotSpec);
}

void RWStepGeom_RWBSplineSurfaceWithKnots::WriteStep(StepData_StepWriter& SW,
                                                     const Handle(StepGeom_BSplineSurfaceWithKnots) & ent) const {
...
void RWStepGeom_RWBSplineSurfaceWithKnots::Share(const Handle(StepGeom_BSplineSurfaceWithKnots) & ent,
...
void RWStepGeom_RWBSplineSurfaceWithKnots::Check(const Handle(StepGeom_BSplineSurfaceWithKnots) & ent,
