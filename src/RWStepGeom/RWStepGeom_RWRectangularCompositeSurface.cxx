...
void RWStepGeom_RWRectangularCompositeSurface::ReadStep(const Handle(StepData_StepReaderData) & data,
                                                        const Standard_Integer num, Handle(Interface_Check) & ach,
                                                        const Handle(StepGeom_RectangularCompositeSurface) &
                                                            ent) const {
        ...
        for (Standard_Integer i2 = 1; i2 <= nbi2; i2++) {
            ...
            if (data->ReadSubList(nsub2, i2, "sub-part(segments)", ach, nsi2)) {
                for (Standard_Integer j2 = 1; j2 <= nbj2; j2++) {
                    ...
                    if (data->ReadEntity(nsi2, j2, "surface_patch", ach, STANDARD_TYPE(StepGeom_SurfacePatch), anent2))
...
void RWStepGeom_RWRectangularCompositeSurface::WriteStep(StepData_StepWriter& SW,
                                                         const Handle(StepGeom_RectangularCompositeSurface) &
                                                             ent) const {
...
void RWStepGeom_RWRectangularCompositeSurface::Share(const Handle(StepGeom_RectangularCompositeSurface) & ent,
