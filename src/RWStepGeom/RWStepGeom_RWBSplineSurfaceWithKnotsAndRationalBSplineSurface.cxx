...
void RWStepGeom_RWBSplineSurfaceWithKnotsAndRationalBSplineSurface::ReadStep(
        ...
        for (Standard_Integer i3 = 1; i3 <= nbi3; i3++) {
            ...
            if (data->ReadSubList(nsub3, i3, "sub-part(control_points_list)", ach, nsi3temp)) {
                ...
                for (Standard_Integer j3 = 1; j3 <= nbj3; j3++) {
                    ...
                    if (data->ReadEntity(nsi3, j3, "cartesian_point", ach, STANDARD_TYPE(StepGeom_CartesianPoint),
                                         anent3))
    ...
    if (data->ReadSubList(num, 3, "u_knots", ach, nsub10)) {
        ...
        for (Standard_Integer i10 = 1; i10 <= nb10; i10++) {
            ...
            if (data->ReadReal(nsub10, i10, "u_knots", ach, aUKnotsItem)) aUKnots->SetValue(i10, aUKnotsItem);
        }
    }
    ...
    if (data->ReadSubList(num, 4, "v_knots", ach, nsub11)) {
        ...
        for (Standard_Integer i11 = 1; i11 <= nb11; i11++) {
            ...
            if (data->ReadReal(nsub11, i11, "v_knots", ach, aVKnotsItem)) aVKnots->SetValue(i11, aVKnotsItem);
        }
    }
    ...
    ent->Init(aName, aUDegree, aVDegree, aControlPointsList, aSurfaceForm, aUClosed, aVClosed, aSelfIntersect,
              aUMultiplicities, aVMultiplicities, aUKnots, aVKnots, aKnotSpec, aWeightsData);
}

void RWStepGeom_RWBSplineSurfaceWithKnotsAndRationalBSplineSurface::WriteStep(
    StepData_StepWriter& SW, const Handle(StepGeom_BSplineSurfaceWithKnotsAndRationalBSplineSurface) & ent) const {
...
void RWStepGeom_RWBSplineSurfaceWithKnotsAndRationalBSplineSurface::Check(
    const Handle(StepGeom_BSplineSurfaceWithKnotsAndRationalBSplineSurface) & ent, const Interface_ShareTool& aShto,
