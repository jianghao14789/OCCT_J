...
D3DHost_View::D3DHost_View(const Handle(Graphic3d_StructureManager) & theMgr,
                           const Handle(D3DHost_GraphicDriver) & theDriver, const Handle(OpenGl_Caps) & theCaps,
                           OpenGl_StateCounter* theCounter)
    : OpenGl_View(theMgr, theDriver, theCaps, theCounter), myD3dLib(NULL), myD3dDevice(NULL),
      myD3dParams(new D3DPRESENT_PARAMETERS()), myRefreshRate(D3DPRESENT_RATE_DEFAULT), myIsD3dEx(false) {
...
// purpose  :
// =======================================================================
void D3DHost_View::SetWindow(const Handle(Aspect_Window) & theWindow, const Aspect_RenderingContext theContext) {
    if (!myD3dWglFbo.IsNull()) {
...
void D3DHost_View::DiagnosticInformation(TColStd_IndexedDataMapOfStringString& theDict,
    ...
    theDict.Add("D3Dinterop",
                myD3dWglFbo.IsNull() || myD3dWglFbo->D3dFallback() ? "Software Fallback" : "WGL_NV_DX_interop");
}
...
bool D3DHost_View::d3dInit() {
    if (!d3dInitLib()) {
        myWorkspace->GetGlContext()->PushMessage(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0,
                                                 GL_DEBUG_SEVERITY_HIGH, "Direct3DCreate9 failed!");
        return false;
    }
    ...
    HRESULT isOK =
        myD3dLib->CreateDevice(anAdapterId, D3DDEVTYPE_HAL, (HWND)myWindow->PlatformWindow()->NativeHandle(),
...
bool D3DHost_View::d3dCreateRenderTarget() {
    ...
    if (!toD3dFallback) {
        toD3dFallback = !myD3dWglFbo->InitD3dInterop(myWorkspace->GetGlContext(), myD3dDevice, myIsD3dEx,
                                                     myWindow->Width(), myWindow->Height(),
                                                     0); // do not request depth-stencil attachment since buffer will be
                                                         // flipped using addition FBO (myToFlipOutput)
    }
    if (toD3dFallback) {
        if (!myD3dWglFbo->InitD3dFallback(myWorkspace->GetGlContext(), myD3dDevice, myIsD3dEx, myWindow->Width(),
                                          myWindow->Height(), GL_DEPTH24_STENCIL8)) {
            return false;
        }
    }
