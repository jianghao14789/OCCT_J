...
D3DHost_FrameBuffer::D3DHost_FrameBuffer()
    : myD3dSurf(NULL), myD3dSurfShare(NULL), myGlD3dDevice(NULL), myGlD3dSurf(NULL), myLockCount(0),
      myD3dFallback(Standard_False), myIsSRGBReady(Standard_False) {
    //
}
...
void D3DHost_FrameBuffer::Release(OpenGl_Context* theCtx) {
    ...
    if (myGlD3dDevice != NULL) {
        const OpenGl_GlFunctions* aFuncs = (theCtx != NULL && theCtx->IsValid()) ? theCtx->Functions() : NULL;
        if (myGlD3dSurf != NULL) {
            if (aFuncs != NULL) {
...
// purpose  :
// =======================================================================
Standard_Boolean D3DHost_FrameBuffer::Init(const Handle(OpenGl_Context) & theCtx, IDirect3DDevice9* theD3DDevice,
                                           const Standard_Boolean theIsD3dEx, const Standard_Integer theSizeX,
...
Standard_Boolean D3DHost_FrameBuffer::InitD3dFallback(const Handle(OpenGl_Context) & theCtx,
                                                      IDirect3DDevice9* theD3DDevice, const Standard_Boolean theIsD3dEx,
                                                      const Standard_Integer theSizeX, const Standard_Integer theSizeY,
                                                      const Standard_Integer theDepthFormat) {
    const Standard_Boolean isGlInit =
        OpenGl_FrameBuffer::Init(theCtx, Graphic3d_Vec2i(theSizeX, theSizeY), GL_RGBA8, theDepthFormat, 0);
    myD3dFallback = Standard_True;

    const Standard_Integer aSizeX = theSizeX > 0 ? theSizeX : 2;
    const Standard_Integer aSizeY = theSizeY > 0 ? theSizeY : 2;
    if (theD3DDevice->CreateRenderTarget(aSizeX, aSizeY, D3DFMT_X8R8G8B8, D3DMULTISAMPLE_NONE, 0,
                                         theIsD3dEx ? TRUE : FALSE, &myD3dSurf,
                                         theIsD3dEx ? &myD3dSurfShare : NULL) != D3D_OK) {
        Release(theCtx.operator->());
        theCtx->PushMessage(
            GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH,
            TCollection_AsciiString("D3DHost_FrameBuffer, could not create D3DFMT_X8R8G8B8 render target ") + aSizeX +
                "x" + aSizeY);
...
Standard_Boolean D3DHost_FrameBuffer::InitD3dInterop(const Handle(OpenGl_Context) & theCtx,
    ...
    if (aDepthFormat.IsValid() &&
        ...
        theCtx->PushMessage(
            GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH,
            TCollection_AsciiString("D3DHost_FrameBuffer, could not initialize GL_DEPTH24_STENCIL8 texture ") + aSizeX +
                "x" + aSizeY);
        return Standard_False;
    }
...
Standard_Boolean D3DHost_FrameBuffer::registerD3dBuffer(const Handle(OpenGl_Context) & theCtx) {
    ...
    myGlD3dSurf = aFuncs->wglDXRegisterObjectNV(myGlD3dDevice, myD3dSurf, myColorTextures(0)->TextureId(),
...
void D3DHost_FrameBuffer::BindBuffer(const Handle(OpenGl_Context) & theCtx) {
    Standard_ProgramError_Raise_if(myLockCount < 1,
    ...
    theCtx->arbFBO->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, myColorTextures(0)->GetTarget(),
                                           myColorTextures(0)->TextureId(), 0);
    ...
    if (myDepthStencilTexture->IsValid()) {
        ...
        theCtx->arbFBO->glFramebufferTexture2D(
            GL_FRAMEBUFFER,
            aDepthFormat.PixelFormat() == GL_DEPTH_STENCIL ? GL_DEPTH_STENCIL_ATTACHMENT : GL_DEPTH_ATTACHMENT,
            myDepthStencilTexture->GetTarget(), myDepthStencilTexture->TextureId(), 0);
#else
        theCtx->arbFBO->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, myDepthStencilTexture->GetTarget(),
...
void D3DHost_FrameBuffer::UnlockSurface(const Handle(OpenGl_Context) & theCtx) {
    ...
    if (myD3dFallback) {
        ...
        Image_PixMap anImg;
        if (anImg.InitWrapper(Image_Format_BGRA, (Standard_Byte*)aLockedRect.pBits, myInitVPSizeX, myInitVPSizeY,
                              aLockedRect.Pitch)) {
