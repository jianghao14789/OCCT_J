    ...
    for (i = 1; i <= nbSegs; i++) {
        ...
        if (ibspole > bspoles.Lower()) {
            ...
            if (!(bspoles.Value(ibspole).IsEqual(bzpoles.Value(bzlow), epsgeom))) {
                ...
                bspoles.SetValue(ibspole, gp_Pnt((bspoles.Value(ibspole).X() + bzpoles.Value(bzlow).X()) / 2.,
...
Standard_Integer IGESConvGeom::IncreaseCurveContinuity(const Handle(Geom_BSplineCurve) & res,
                                                       const Standard_Real epsgeom, const Standard_Integer continuity) {
...
Standard_Integer IGESConvGeom::SplineSurfaceFromIGES(const Handle(IGESGeom_SplineSurface) & st,
    ...
    for (i = 1; i <= DegreeU + 1; i++) {
        ...
        for (j = 1; j <= DegreeV + 1; j++) {
            ...
            gp_Pnt aPoint(XPoly->Value(PolyIndex) * ParamU * ParamV, YPoly->Value(PolyIndex) * ParamU * ParamV,
    ...
    for (USeg = 2; USeg <= NbUSeg; USeg++) {
        ...
        for (i = Coef.LowerRow(); i <= Coef.UpperRow(); i++) {
            ...
            for (j = Coef.LowerCol(); j <= Coef.UpperCol(); j++) {
                Standard_Integer PolyIndex = i + 4 * (j - 1);
                gp_Pnt aPoint;
                aPoint.SetCoord(XPoly->Value(PolyIndex) * ParamU * ParamV, YPoly->Value(PolyIndex) * ParamU * ParamV,
                                ZPoly->Value(PolyIndex) * ParamU * ParamV);
                Coef.SetValue(i, j, aPoint);
                ParamV = ParamV * deltaV(VSeg);
            }
        ...
        for (jBz = BzPole.LowerCol(); jBz <= BzPole.UpperCol(); jBz++) {
            if (!BzPole.Value(iBz, jBz).IsEqual(BsPole.Value(iBsPole, jBsPole), epsgeom)) {
                ...
                gp_Pnt MidPoint;
                Standard_Real XCoord = 0.5 * (BzPole.Value(iBz, jBz).X() + BsPole.Value(iBsPole, jBsPole).X());
                Standard_Real YCoord = 0.5 * (BzPole.Value(iBz, jBz).Y() + BsPole.Value(iBsPole, jBsPole).Y());
                Standard_Real ZCoord = 0.5 * (BzPole.Value(iBz, jBz).Z() + BsPole.Value(iBsPole, jBsPole).Z());
