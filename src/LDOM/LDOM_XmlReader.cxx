...
LDOM_XmlReader::LDOM_XmlReader(const Handle(LDOM_MemManager) & theDocument, TCollection_AsciiString& theErrorString,
                               const Standard_Boolean theTagPerStep)
    : myEOF(Standard_False), myError(theErrorString), myDocument(theDocument), myElement(NULL), myLastChild(NULL),
      myPtr(&myBuffer[0]), myEndPtr(&myBuffer[0]), myTagPerStep(theTagPerStep), myBOM(LDOM_OSStream::BOM_UNDEFINED) {}
...
LDOM_XmlReader::RecordType LDOM_XmlReader::ReadRecord(Standard_IStream& theIStream, LDOM_OSStream& theData,
    ...
    for (;;) {
        ...
        if (aBytesRest < XML_MIN_BUFFER) {
            if (myEOF == Standard_True) {
                if (aBytesRest <= 0) break; // END of processing
                ...
                if (aStartData /* && aState != STATE_WAITING */) {
                    if (myPtr > aStartData) theData.rdbuf()->sputn(aStartData, myPtr - aStartData);
        ...
        if (theDocStart && !myEOF) {
            ...
            if (myBOM != LDOM_OSStream::BOM_UNDEFINED) continue;
        }
                        ...
                        switch (myPtr[1]) {
                            ...
                            default:
                                if (::isName(&myPtr[1], myEndPtr, aNameEnd)) {
                                    ...
                                    myPtr = aNameEnd;
                                    if (myPtr < myEndPtr) {
                                        myElement = &LDOM_BasicElement::Create(
                                            aStartData, (Standard_Integer)(myPtr - aStartData), myDocument);
                                        myLastChild = NULL;
                ...
                switch (myPtr[0]) {
                    ...
                    case '\r':
                        ...
                        if (aPtr) {
                            ...
                            //    Create an attribute
                            myLastChild = myElement->AddAttribute(anAttrName, anAttrValue, myDocument, myLastChild);
                            myPtr = aPtr + 1;
                            aStartData = NULL;
                            aState = STATE_ATTRIBUTE_NAME;
                        } else {
...
Standard_Boolean LDOM_XmlReader::getInteger(LDOMBasicString& theValue, const char* theStart, const char* theEnd) {
    char* ptr;
    errno = 0;
