...
Standard_Boolean LDOM_Node::operator==(const LDOM_Node& anOther) const {
    if (isNull()) return anOther.isNull();
...
LDOM_Node LDOM_Node::getFirstChild() const {
    ...
    if (aType == ELEMENT_NODE) {
        ...
        if (aChild)
            if (aChild->getNodeType() != LDOM_Node::ATTRIBUTE_NODE) return LDOM_Node(*aChild, myDocument);
    }
    return LDOM_Node();
}
...
LDOM_Node LDOM_Node::getNextSibling() const {
    ...
    if (aSibling)
        if (aSibling->getNodeType() != ATTRIBUTE_NODE) return LDOM_Node(*aSibling, myDocument);
    return LDOM_Node();
}
...
void LDOM_Node::removeChild(const LDOM_Node& aChild) {
    ...
    if (aType == ELEMENT_NODE) {
        ...
        if (aChild != NULL) anElement.RemoveChild(aChild.myOrigin);
...
void LDOM_Node::SetValueClear() const {
    ...
    if (aValue->Type() == LDOMBasicString::LDOM_AsciiDoc) aValue->myType = LDOMBasicString::LDOM_AsciiDocClear;
}
