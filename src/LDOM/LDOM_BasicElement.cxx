...
LDOM_BasicElement& LDOM_BasicElement::Create(const char* aName, const Standard_Integer aLen,
    ...
    if (aNode) {
        if (aNode->getNodeType() == LDOM_Node::ATTRIBUTE_NODE)
            ...
            while (aNode->mySibling) {
                if (aNode->mySibling->getNodeType() == LDOM_Node::ATTRIBUTE_NODE) break;
...
const LDOM_BasicAttribute& LDOM_BasicElement::GetAttribute(const LDOMBasicString& aName,
    ...
    while (aNode) {
        if (aNode->getNodeType() == LDOM_Node::ATTRIBUTE_NODE) {
            ...
            if (!strcmp(aNameStr, anAttr->GetName())) return *anAttr;
        }
        aNode = aNode->mySibling;
    }
...
const LDOM_BasicAttribute* LDOM_BasicElement::GetFirstAttribute(const LDOM_BasicNode*& theLastCh,
...
const LDOM_BasicNode* LDOM_BasicElement::RemoveAttribute(const LDOMBasicString& aName,
    ...
    const Standard_Integer aHash = LDOM_MemManager::Hash(aNameStr, (Standard_Integer)strlen(aNameStr));
    const unsigned int anAttrMaskValue = aHash & (8 * sizeof(myAttributeMask) - 1);
    const unsigned long anAttributeMask = (1 << anAttrMaskValue);
#ifdef OCCT_DEBUG_MASK
    anAttributeMask = 0xffffffff;
...
void LDOM_BasicElement::RemoveChild(const LDOM_BasicNode* aChild) const {
    ...
    while (aNode) {
        if (aNode->getNodeType() == LDOM_Node::ATTRIBUTE_NODE) break;
        if (aNode == aChild) {
...
void LDOM_BasicElement::AppendChild(const LDOM_BasicNode* aChild, const LDOM_BasicNode*& aLastChild) const {
    if (aLastChild) {
...
void LDOM_BasicElement::AddElementsByTagName(LDOM_NodeList& aList, const LDOMBasicString& aTagName) const {
    const LDOM_BasicNode* aNode = myFirstChild;
    ...
    while (aNode) {
        ...
        if (aNode->getNodeType() == LDOM_Node::ELEMENT_NODE) {
            ...
            if (strcmp(anElement.GetTagName(), aTagString) == 0) aList.Append(anElement);
...
void LDOM_BasicElement::AddAttributes(LDOM_NodeList& aList, const LDOM_BasicNode* aLastChild) const {
    ...
    while (aBNode) {
        if (aBNode->getNodeType() == LDOM_Node::ATTRIBUTE_NODE) aList.Append(*aBNode);
    ...
    for (; aBNode != NULL; aBNode = aBNode->GetSibling()) {
        ...
        switch (aNewNodeType) {
            ...
            case LDOM_Node::CDATA_SECTION_NODE: {
                ...
                aNewBNode =
                    &LDOM_BasicText::Create(aNewNodeType, LDOMString(aBNodeText.GetData(), aDocument), aDocument);
                break;
            }
            default:
                continue;
...
loop_attr:
    ...
    for (; aBNode != NULL; aBNode = aBNode->GetSibling()) {
        ...
        LDOM_BasicAttribute* aNewAtt = &LDOM_BasicAttribute::Create(aBNodeAtt->GetName(), aDocument, aHash);
