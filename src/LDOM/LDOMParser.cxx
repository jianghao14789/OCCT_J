...
inline
#endif
    LDOM_XmlReader::RecordType ReadRecord(LDOM_XmlReader& aReader, Standard_IStream& theIStream, LDOM_OSStream& aData,
...
LDOM_OSStream::BOMType LDOMParser::GetBOM() const {
    if (myReader) return myReader->GetBOM();
...
Standard_Boolean LDOMParser::parse(std::istream& anInput, const Standard_Boolean theTagPerStep,
...
Standard_Boolean LDOMParser::ParseDocument(std::istream& theIStream, const Standard_Boolean theWithoutRoot) {
    ...
    for (;;) {
        LDOM_XmlReader::RecordType aType =
            (theWithoutRoot && !isInsertFictRootElement ? LDOM_XmlReader::XML_START_ELEMENT
