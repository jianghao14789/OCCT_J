...
inline Standard_Boolean compareStrings(char* const str, const char* theString, const Standard_Integer theLength) {
...
inline LDOM_MemManager::MemBlock::MemBlock(const Standard_Integer aSize, LDOM_MemManager::MemBlock* aFirst)
...
Standard_Integer LDOM_MemManager::HashTable::Hash(const char* aString, const Standard_Integer aLen) {
    static const unsigned int wCRC16a[16] = {
        0000000, 0140301, 0140601, 0000500, 0141401, 0001700, 0001200, 0141101,
        0143001, 0003300, 0003600, 0143501, 0002400, 0142701, 0142201, 0002100,
    };

    static const unsigned int wCRC16b[16] = {
        0000000, 0146001, 0154001, 0012000, 0170001, 0036000, 0024000, 0162001,
        0120001, 0066000, 0074000, 0132001, 0050000, 0116001, 0104001, 0043000,
    };
...
const char* LDOM_MemManager::HashTable::AddString(const char* theString, const Standard_Integer theLen,
    ...
    if (aNode->str == NULL) {
        LDOM_HashValue* anAlloc = (LDOM_HashValue*)myManager.Allocate(theLen + 1 + sizeof(LDOM_HashValue));
...
LDOM_MemManager::LDOM_MemManager(const Standard_Integer aBlockSize)
    : myRootElement(NULL), myFirstBlock(NULL), myFirstWithoutRoom(NULL), myBlockSize(convertBlockSize(aBlockSize)),
      myHashTable(NULL) {}
...
LDOM_MemManager::~LDOM_MemManager() {
    ...
    if (aCount > 1)
        std::cout << ".. Destroying " << aCount << " LDOM memory allocations: " << aSomme / 256 << " kB" << std::endl;
// fclose (out);
#endif
    delete myFirstBlock;
    if (myHashTable) delete myHashTable;
}
...
const char* LDOM_MemManager::HashedAllocate(const char* theString, const Standard_Integer theLen,
...
void LDOM_MemManager::HashedAllocate(const char* aString, const Standard_Integer theLen, LDOMBasicString& theResult) {
    ...
    if (aHashedString != NULL) theResult.myVal.ptr = (void*)aHashedString;
}
...
Standard_Boolean LDOM_MemManager::CompareStrings(const char* theString, const Standard_Integer theHashValue,
                                                 const char* theHashedStr) {
    if (((LDOM_HashValue*)theHashedStr)[-1] == LDOM_HashValue(theHashValue))
        if (!strcmp(theString, theHashedStr)) return Standard_True;
