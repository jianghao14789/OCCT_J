...
#define IS_EQUAL(_ptr, _string) (!memcmp(_ptr, _string, sizeof(_string) - 1))
    ...
    for (;;) {
        ...
        Standard_Integer aByteCount = (Standard_Integer)(aPtr - aSrcPtr);
        if (aByteCount > 0 && aDstPtr != aSrcPtr) memmove(aDstPtr, aSrcPtr, aByteCount);
        aSrcPtr = aPtr;
        if (aSrcPtr[1] == '#') {
            unsigned long aChar;
            char* aNewPtr;
...
char* LDOM_CharReference::Encode(const char* theSrc, Standard_Integer& theLen, const Standard_Boolean isAttribute) {
    // Initialising the constants
    static const struct entityRef entity_ref[6] = {entityRef(NULL, 0),     entityRef("&amp;", 5),
                                                   entityRef("&lt;", 4),   entityRef("&gt;", 4),
                                                   entityRef("&quot;", 6), entityRef("&apos;", 6)};
    ...
    for (;;) {
        ...
        if (myTab[iSrc] != NORMAL_C)
            if (isAttribute || myTab[iSrc] != ENTI_QUOT) aCount++;
        ptrSrc++;
    }
