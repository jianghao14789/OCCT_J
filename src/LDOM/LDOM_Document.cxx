...
LDOM_NodeList LDOM_Document::getElementsByTagName(const LDOMString& theTagName) const {
    ...
    if (anElem) {
        ...
        if (strcmp(anElem->GetTagName(), aTagString) == 0) aList.Append(*anElem);
...
LDOM_Document LDOM_Document::createDocument(const LDOMString& theQualifiedName) {
    ...
    if (strlen(aString) == 0) aString = "document";
...
LDOM_CDATASection LDOM_Document::createCDATASection(const LDOMString& theData) {
    LDOM_BasicText& aBasicText =
        LDOM_BasicText::Create(LDOM_Node::CDATA_SECTION_NODE, LDOMString(theData, myMemManager), myMemManager);
...
LDOM_Comment LDOM_Document::createComment(const LDOMString& theData) {
    LDOM_BasicText& aBasicText =
        LDOM_BasicText::Create(LDOM_Node::COMMENT_NODE, LDOMString(theData, myMemManager), myMemManager);
