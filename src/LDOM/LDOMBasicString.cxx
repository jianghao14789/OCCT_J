...
LDOMBasicString::LDOMBasicString(const char* aValue, const Standard_Integer aLen,
...
LDOMBasicString::LDOMBasicString(const LDOMBasicString& anOther) : myType(anOther.Type()) {
    switch (myType) {
        case LDOM_AsciiFree:
        ...
        Standard_FALLTHROUGH case LDOM_AsciiDoc:
...
LDOMBasicString::~LDOMBasicString() {
    if (myType == LDOM_AsciiFree) {
        if (myVal.ptr) delete[] (char*)myVal.ptr;
    }
}
...
LDOMBasicString& LDOMBasicString::operator=(const LDOM_NullPtr*) {
    if (myType == LDOM_AsciiFree && myVal.ptr) delete[] (char*)myVal.ptr;
    myType = LDOM_NULL;
    myVal.ptr = NULL;
    return *this;
}
...
LDOMBasicString::operator TCollection_ExtendedString() const {
    switch (myType) {
        ...
        case LDOM_AsciiHashed: {
            ...
            if (ptr[0] != '#' || ptr[1] != '#') return TCollection_ExtendedString(ptr);
            buf[0] = ptr[2];
            buf[1] = ptr[3];
            buf[2] = ptr[4];
            buf[3] = ptr[5];
            if (strtol(&buf[0], NULL, 16) != aUnicodeHeader) return TCollection_ExtendedString(ptr);
...
Standard_Boolean LDOMBasicString::GetInteger(Standard_Integer& aResult) const {
    switch (myType) {
        ...
        case LDOM_AsciiHashed: {
            ...
            if (ptr == myVal.ptr || errno == ERANGE || errno == EINVAL) return Standard_False;
