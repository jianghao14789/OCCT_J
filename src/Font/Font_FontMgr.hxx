...
public:
    ...
    Standard_EXPORT Handle(Font_SystemFont)
        GetFont(const Handle(TCollection_HAsciiString) & theFontName, const Font_FontAspect theFontAspect,
    ...
    //!                                  can be modified if specified font alias refers to another style (compatibility
    //!                                  with obsolete aliases)
    //! @param theDoFailMsg   [in]       put error message on failure into default messenger
    Standard_EXPORT Handle(Font_SystemFont)
        FindFont(const TCollection_AsciiString& theFontName, Font_StrictLevel theStrictLevel,
                 Font_FontAspect& theFontAspect, Standard_Boolean theDoFailMsg = Standard_True) const;
    ...
    Standard_EXPORT Handle(Font_SystemFont)
        FindFallbackFont(Font_UnicodeSubset theSubset, Font_FontAspect theFontAspect) const;
    ...
    Standard_Boolean RegisterFonts(const NCollection_Sequence<Handle(Font_SystemFont)>& theFonts,
        ...
        for (NCollection_Sequence<Handle(Font_SystemFont)>::Iterator aFontIter(theFonts); aFontIter.More();
             aFontIter.Next()) {
...
public:
    ...
    //! and they are resolved dynamically on request - first existing font is returned in case of multiple aliases to
    //! the same name.
    //!
...
private:
    ...
    struct Font_FontAlias {
        ...
        Font_FontAlias(const TCollection_AsciiString& theFontName,
            ...
            : FontName(theFontName), FontAspect(theFontAspect) {}
    ...
    //! Register font alias.
    void addFontAlias(const TCollection_AsciiString& theAliasName, const Handle(Font_FontAliasSequence) & theAliases,
