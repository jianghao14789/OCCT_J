...
// list of supported extensions
static Standard_CString Font_FontMgr_Extensions[] = {"ttf", "otf", "ttc", "pfa", "pfb",
#ifdef __APPLE__
...
static Standard_CString myFontServiceConf[] = {"/etc/X11/fs/config", "/usr/X11R6/lib/X11/fs/config",
                                               "/usr/X11/lib/X11/fs/config", NULL};
...
static void addDirsRecursively(const OSD_Path& thePath, NCollection_Map<TCollection_AsciiString>& theDirsMap) {
...
static bool checkFont(NCollection_Sequence<Handle(Font_SystemFont)>& theFonts, const Handle(Font_FTLibrary) & theFTLib,
                      ...
                      signed long theFaceId = -1) // FT_Long
{
    ...
    // To include other non-standard Styles, their names can be appended to Family Name; for this, names of normal
...
void Font_FontMgr::GetAllAliases(TColStd_SequenceOfHAsciiString& theAliases) const {
    for (NCollection_DataMap<TCollection_AsciiString, Handle(Font_FontAliasSequence)>::Iterator anAliasIter(
             myFontAliases);
...
void Font_FontMgr::addFontAlias(const TCollection_AsciiString& theAliasName,
                                const Handle(Font_FontAliasSequence) & theAliases, Font_FontAspect theAspect) {
...
// purpose  :
// =======================================================================
Font_FontMgr::Font_FontMgr() : myToTraceAliases(Standard_False) {
...
void Font_FontMgr::InitFontDataBase() {
    ...
    for (DWORD anIter = 0; RegEnumValueA(aFontsKey, anIter, aNameBuff, &aNameSize, NULL, NULL, (LPBYTE)aPathBuff,
                                         &aPathSize) != ERROR_NO_MORE_ITEMS;
    ...
    for (NCollection_Map<TCollection_AsciiString>::Iterator anIter(aMapOfFontsDirs); anIter.More(); anIter.Next()) {
#if defined(HAVE_FREETYPE) && !defined(__ANDROID__) && !defined(__APPLE__) && !defined(__EMSCRIPTEN__)
...
Handle(Font_SystemFont) Font_FontMgr::GetFont(const Handle(TCollection_HAsciiString) & theFontName,
    ...
    return (aFont.IsNull() || theFontAspect == Font_FontAspect_UNDEFINED || aFont->HasFontAspect(theFontAspect))
               ? aFont
...
Handle(Font_SystemFont) Font_FontMgr::FindFont(const TCollection_AsciiString& theFontName,
                                               Font_StrictLevel theStrictLevel, Font_FontAspect& theFontAspect,
    ...
    for (int aPass = 0; aPass < 2; ++aPass) {
        ...
        for (Font_FontAliasSequence::Iterator anAliasIter(*anAliases); anAliasIter.More(); anAliasIter.Next()) {
            ...
            if (Handle(Font_SystemFont) aFont2 = myFontMap.Find(anAlias.FontName)) {
                ...
                if ((anAlias.FontAspect != Font_FontAspect_UNDEFINED && aFont2->HasFontAspect(anAlias.FontAspect))) {
                    ...
                    break;
                           ...
                           (theFontAspect == Font_FontAspect_UNDEFINED || aFont2->HasFontAspect(theFontAspect))) {
                    isBestAlias = true;
                    break;
                }
            }
    ...
    if ((theFontAspect != Font_FA_Undefined && !aFont->HasFontAspect(theFontAspect)) ||
        ...
        Message::SendWarning(TCollection_AsciiString("Font_FontMgr, warning: unable to find font ") + aDesc + "; " +
                             aFont->ToString() + " is used instead");
    }
    return aFont;
}
...
Handle(Font_SystemFont) Font_FontMgr::Font_FontMap::Find(const TCollection_AsciiString& theFontName) const {
    ...
    TCollection_AsciiString aFontName(theFontName);
    aFontName.LowerCase();
    for (IndexedMapNode* aNodeIter = (IndexedMapNode*)myData1[::HashCode(aFontName, NbBuckets())]; aNodeIter != NULL;
         aNodeIter = (IndexedMapNode*)aNodeIter->Next()) {
...
#ifdef HAVE_FREETYPE
    return new NCollection_Buffer(Handle(NCollection_BaseAllocator)(), Font_DejavuSans_Latin_woff_size,
