...
struct Font_FTFontParams {
    ...
    Font_Hinting
        FontHinting; //!< request hinting (exclude FT_LOAD_NO_HINTING flag), Font_Hinting_Off by default;
    ...
    //! Empty constructor.
    Font_FTFontParams()
        : PointSize(0), Resolution(72u), FontHinting(Font_Hinting_Off), ToSynthesizeItalic(false),
          IsSingleStrokeFont(false) {}

    //! Constructor.
    Font_FTFontParams(unsigned int thePointSize, unsigned int theResolution)
        : PointSize(thePointSize), Resolution(theResolution), FontHinting(Font_Hinting_Off), ToSynthesizeItalic(false),
          IsSingleStrokeFont(false) {}
};
...
public:
    ...
    Standard_EXPORT static Handle(Font_FTFont)
        FindAndCreate(const TCollection_AsciiString& theFontName, const Font_FontAspect theFontAspect,
                      const Font_FTFontParams& theParams, const Font_StrictLevel theStrictLevel = Font_StrictLevel_Any);
    ...
    static bool IsCharFromCJK(Standard_Utf32Char theUChar) {
        return (theUChar >= 0x03400 && theUChar <= 0x04DFF) || (theUChar >= 0x04E00 && theUChar <= 0x09FFF) ||
               (theUChar >= 0x0F900 && theUChar <= 0x0FAFF) || (theUChar >= 0x20000 && theUChar <= 0x2A6DF) ||
    ...
    static bool IsCharFromKorean(Standard_Utf32Char theUChar) {
        return (theUChar >= 0x01100 && theUChar <= 0x011FF) || (theUChar >= 0x03130 && theUChar <= 0x0318F) ||
...
public:
    ...
    //! @param theFaceId   face id within the file (0 by default)
    //! @return true on success
    Standard_EXPORT bool Init(const Handle(NCollection_Buffer) & theData, const TCollection_AsciiString& theFileName,
                              const Font_FTFontParams& theParams, const Standard_Integer theFaceId = 0);
    ...
    //! @param theStrictLevel search strict level for using aliases and fallback
    //! @return true on success
    Standard_EXPORT bool FindAndInit(const TCollection_AsciiString& theFontName, Font_FontAspect theFontAspect,
                                     const Font_FTFontParams& theParams,
                                     Font_StrictLevel theStrictLevel = Font_StrictLevel_Any);

    //! Return flag to use fallback fonts in case if used font does not include symbols from specific Unicode subset;
    //! TRUE by default.
    //! @sa Font_FontMgr::ToUseUnicodeSubsetFallback()
    Standard_Boolean ToUseUnicodeSubsetFallback() const {
        return myToUseUnicodeSubsetFallback;
    }
    ...
    //! Such fonts define single-line glyphs instead of closed contours, so that they are rendered incorrectly by normal
    //! software.
    bool IsSingleStrokeFont() const {
        return myFontParams.IsSingleStrokeFont;
    }
    ...
    bool ToSynthesizeItalic() const {
        return myFontParams.ToSynthesizeItalic;
    }
...
public:
    ...
    //! @return true on success
    Standard_DEPRECATED("Deprecated method, Font_FTFontParams should be used for passing parameters") bool Init(
    ...
    //! @return true on success
    Standard_DEPRECATED("Deprecated method, Font_FTFontParams should be used for passing parameters") bool Init(
        const NCollection_String& theFontName, Font_FontAspect theFontAspect, unsigned int thePointSize,
...
protected:
    ...
    template <typename theInput_t> int32_t toFTPoints(const theInput_t thePointSize) const {
...
protected:
    ...
    Standard_EXPORT bool getKerning(FT_Vector& theKern, Standard_Utf32Char theUCharCurr,
...
protected:
    ...
    Standard_Boolean
        myToUseUnicodeSubsetFallback; //!< use default fallback fonts for extended Unicode sub-sets (Korean, CJK, etc.)
};
