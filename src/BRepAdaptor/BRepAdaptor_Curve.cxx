...
Handle(Adaptor3d_Curve) BRepAdaptor_Curve::Trim(const Standard_Real First, const Standard_Real Last,
...
void BRepAdaptor_Curve::D3(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3) const {
    if (myConSurf.IsNull())
...
Handle(Geom_BezierCurve) BRepAdaptor_Curve::Bezier() const {
    ...
    return myTrsf.Form() == gp_Identity ? BC : Handle(Geom_BezierCurve)::DownCast(BC->Transformed(myTrsf));
}
...
Handle(Geom_BSplineCurve) BRepAdaptor_Curve::BSpline() const {
    ...
    return myTrsf.Form() == gp_Identity ? BS : Handle(Geom_BSplineCurve)::DownCast(BS->Transformed(myTrsf));
}
...
Handle(Geom_OffsetCurve) BRepAdaptor_Curve::OffsetCurve() const {
    ...
    return myTrsf.Form() == gp_Identity ? anOffC : Handle(Geom_OffsetCurve)::DownCast(anOffC->Transformed(myTrsf));
}
