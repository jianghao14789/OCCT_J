...
static Standard_Integer DDocStd_fsdwrite(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgs) {
    if (theArgNb < 3) {
...
static Standard_Integer DDocStd_fsdread(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgs) {
    if (theArgNb < 3) {
    ...
    if (strcmp(theArgs[2], "restore_with_names") == 0) rflag = Standard_True;
...
void DDocStd::ShapeSchemaCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("fsdread", "fsdread filename shape [name | or key 'restore_with_names']", __FILE__, DDocStd_fsdread,
                    g);
}
