...
static Standard_Integer DDocStd_ListDocuments(Draw_Interpretor& di, Standard_Integer nb, const char** /*a*/) {
    if (nb == 1) {
...
static Standard_Integer DDocStd_NewDocument(Draw_Interpretor& di, Standard_Integer nb, const char** a) {
    Handle(TDocStd_Document) D;
...
static Standard_Integer DDocStd_SaveAs(Draw_Interpretor& di, Standard_Integer nb, const char** a) {
    if (nb >= 3) {
        ...
        if (anUseStream) {
            const Handle(OSD_FileSystem) & aFileSystem = OSD_FileSystem::DefaultFileSystem();
            opencascade::std::shared_ptr<std::ostream> aFileStream =
...
static Standard_Integer DDocStd_Close(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIt = 1; anArgIt < theArgNb; ++anArgIt) {
        ...
        if (anArgCase == "*" || anArgCase == "-all" || anArgCase == "all") {
            for (NCollection_Map<Handle(Draw_Drawable3D)>::Iterator anIter(Draw::Drawables()); anIter.More();
                 anIter.Next()) {
    ...
    for (NCollection_List<TCollection_AsciiString>::Iterator aDocNameIter(aDocNames); aDocNameIter.More();
         aDocNameIter.Next()) {
        ...
        if (DDocStd::GetDocument(aDocName, aDoc, toComplain)) {
            ...
            if (TPrsStd_AISViewer::Find(aRoot, aDocViewer) && !aDocViewer->GetInteractiveContext().IsNull()) {
...
static Standard_Integer DDocStd_IsInSession(Draw_Interpretor& di, Standard_Integer nb, const char** a) {
    if (nb == 2) {
        Handle(TDocStd_Application) A = DDocStd::GetApplication();
...
// purpose  :
//=======================================================================
static Standard_Integer DDocStd_AddComment(Draw_Interpretor& di, Standard_Integer nb, const char** a) {
    if (nb == 3) {
        Handle(TDocStd_Document) D;
        if (!DDocStd::GetDocument(a[1], D)) return 1;
...
// purpose  :
//=======================================================================
static Standard_Integer DDocStd_PrintComments(Draw_Interpretor& di, Standard_Integer nb, const char** a) {
    if (nb == 2) {
        Handle(TDocStd_Document) D;
        if (!DDocStd::GetDocument(a[1], D)) return 1;
...
// purpose  :
//=======================================================================
static Standard_Integer DDocStd_StorageFormatVersion(Draw_Interpretor& theDI, Standard_Integer theNbArgs,
...
void DDocStd::ApplicationCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("SaveAs", "SaveAs DOC path [saveEmptyLabels: 0|1] [-stream]", __FILE__, DDocStd_SaveAs, g);
    ...
    theCommands.Add("StorageFormatVersion", THE_SET_VER_HELP.ToCString(), __FILE__, DDocStd_StorageFormatVersion, g);
    ...
    theCommands.Add("SetStorageFormatVersion", "\n\t\t: Alias to StorageFormatVersion", __FILE__,
                    DDocStd_StorageFormatVersion, g);
}
