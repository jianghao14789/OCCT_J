...
// purpose  :
//=======================================================================
Transfer_TransientProcess::Transfer_TransientProcess(const Standard_Integer nb) : Transfer_ProcessForTransient(nb) {
...
void Transfer_TransientProcess::SetContext(const Standard_CString name, const Handle(Standard_Transient) & ctx) {
...
Standard_Boolean Transfer_TransientProcess::GetContext(const Standard_CString name, const Handle(Standard_Type) & type,
                                                       Handle(Standard_Transient) & ctx) const {
    if (thectx.IsEmpty()) return Standard_False;
    if (!thectx.Find(name, ctx)) ctx.Nullify();
...
void Transfer_TransientProcess::PrintTrace(const Handle(Standard_Transient) & start, Standard_OStream& S) const {
    if (!start.IsNull()) {
        if (!themodel.IsNull()) {
            S << "Entity ";
...
Standard_Boolean Transfer_TransientProcess::IsDataLoaded(const Handle(Standard_Transient) & start) const {
...
Standard_Boolean Transfer_TransientProcess::IsDataFail(const Handle(Standard_Transient) & start) const {
...
void Transfer_TransientProcess::PrintStats(const Standard_Integer /*mode*/, Standard_OStream& S) const {
    S << "\n*******************************************************************\n";
    ...
    if (nbr > nbroots) S << "****      ( Itermediate Results : " << nbr - nbroots << " )\n";
