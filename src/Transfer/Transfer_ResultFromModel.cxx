...
Standard_Boolean Transfer_ResultFromModel::Fill(const Handle(Transfer_TransientProcess) & TP,
    ...
    themlab.Clear();
    if (themnum > 0) themlab.AssignCat(themodel->StringLabel(ent)->ToCString());
...
void Transfer_ResultFromModel::SetMainResult(const Handle(Transfer_ResultFromTransient) & amain) {
    ...
    themlab.Clear();
    if (themnum > 0) themlab.AssignCat(themodel->StringLabel(themain->Start())->ToCString());
}
...
Handle(Transfer_ResultFromTransient) Transfer_ResultFromModel::ResultFromKey(const Handle(Standard_Transient) &
                                                                             start) const {
...
Interface_CheckIterator Transfer_ResultFromModel::CheckList(const Standard_Boolean erronly,
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (stat == Interface_CheckOK || (stat == Interface_CheckWarning && erronly)) continue;
