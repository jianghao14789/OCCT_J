...
public:
    ...
    Standard_EXPORT void AddFail(const Handle(Standard_Transient) & start, const Standard_CString mess,
                                 const Standard_CString orig = "");

    //! (other name of AddFail, maintained for compatibility)
    Standard_EXPORT void AddError(const Handle(Standard_Transient) & start, const Standard_CString mess,
                                  const Standard_CString orig = "");
    ...
    Standard_EXPORT void AddWarning(const Handle(Standard_Transient) & start, const Standard_CString mess,
                                    const Standard_CString orig = "");
    ...
    Standard_EXPORT Handle(Transfer_Binder)
        Transferring(const Handle(Standard_Transient) & start,
    ...
    Standard_EXPORT void StartTrace(const Handle(Transfer_Binder) & binder, const Handle(Standard_Transient) & start,
    ...
    Standard_EXPORT Transfer_IteratorOfProcessForTransient
    ResultOne(const Handle(Standard_Transient) & start, const Standard_Integer level,
    ...
    Standard_EXPORT void RemoveResult(const Handle(Standard_Transient) & start, const Standard_Integer level,
...
private:
    ...
    Standard_EXPORT Handle(Transfer_Binder)
        TransferProduct(const Handle(Standard_Transient) & start,
                        const Message_ProgressRange& theProgress = Message_ProgressRange());
