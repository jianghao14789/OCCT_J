...
void Transfer_TransferOutput::Transfer(const Handle(Standard_Transient) & obj,
    ...
    if (themodel->Number(obj) == 0)
        throw Transfer_TransferFailure("TransferOutput : Transfer, entities do not come from same initial model");
...
void Transfer_TransferOutput::TransferRoots(const Interface_Graph& G, const Message_ProgressRange& theProgress) {
    theproc->SetRootManagement(Standard_False);
...
Interface_EntityIterator Transfer_TransferOutput::ListForStatus(const Standard_Boolean normal,
    ...
    for (Standard_Integer i = 1; i <= max; i++) {
        ...
        if (ok == normal) list.AddItem((roots ? theproc->Root(i) : theproc->Mapped(i)));
    }
    return list;
}
