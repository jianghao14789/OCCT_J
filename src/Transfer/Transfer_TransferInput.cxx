...
Interface_EntityIterator Transfer_TransferInput::Entities(Transfer_TransferIterator& list) const {
    ...
    for (list.Start(); list.More(); list.Next()) {
        ...
        if (!transb.IsNull()) {
            ...
            throw Transfer_TransferFailure("TransferInput : Entities, one of the Results is not Transient Handle");
    }
    return iter;
}
...
void Transfer_TransferInput::FillModel(const Handle(Transfer_TransientProcess) & proc,
                                       ...
                                       const Handle(Interface_Protocol) & proto, const Standard_Boolean roots) const {
