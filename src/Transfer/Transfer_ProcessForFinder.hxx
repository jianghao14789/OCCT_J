...
public:
    ...
    Standard_EXPORT void AddFail(const Handle(Transfer_Finder) & start, const Standard_CString mess,
                                 const Standard_CString orig = "");

    //! (other name of AddFail, maintained for compatibility)
    Standard_EXPORT void AddError(const Handle(Transfer_Finder) & start, const Standard_CString mess,
                                  const Standard_CString orig = "");
    ...
    Standard_EXPORT void AddWarning(const Handle(Transfer_Finder) & start, const Standard_CString mess,
                                    const Standard_CString orig = "");
    ...
    template <class T>
    Standard_Boolean FindTypedTransient(const Handle(Transfer_Finder) & start, const Handle(Standard_Type) & atype,
                                        Handle(T) & val) const {
    ...
    //! Safe variant for arbitrary type of argument
    template <class T>
    Standard_Boolean GetTypedTransient(const Handle(Transfer_Binder) & start, const Handle(Standard_Type) & atype,
                                       Handle(T) & val) const {
        Handle(Standard_Transient) aVal = val;
    ...
    Standard_EXPORT Handle(Transfer_Binder)
        Transferring(const Handle(Transfer_Finder) & start,
    ...
    Standard_EXPORT void StartTrace(const Handle(Transfer_Binder) & binder, const Handle(Transfer_Finder) & start,
                                    const Standard_Integer level, const Standard_Integer mode) const;
    ...
    Standard_EXPORT Transfer_IteratorOfProcessForFinder
    ResultOne(const Handle(Transfer_Finder) & start, const Standard_Integer level,
    ...
    Standard_EXPORT void RemoveResult(const Handle(Transfer_Finder) & start, const Standard_Integer level,
...
private:
    ...
    Standard_EXPORT Handle(Transfer_Binder)
        TransferProduct(const Handle(Transfer_Finder) & start,
                        const Message_ProgressRange& theProgress = Message_ProgressRange());
