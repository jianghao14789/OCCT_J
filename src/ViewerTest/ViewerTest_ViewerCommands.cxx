...
static struct {
    ...
    void SetDefaultGradient() {
        for (NCollection_DoubleMap<TCollection_AsciiString, Handle(AIS_InteractiveContext)>::Iterator aCtxIter(
...
} ViewerTest_DefaultBackground = {Quantity_NOC_BLACK, Quantity_NOC_BLACK, Quantity_NOC_BLACK,
...
#if defined(_WIN32)
    if (theWClass.IsNull()) {
        theWClass = new WNT_WClass("GW3D_Class", (Standard_Address)AdvViewerWindowProc, CS_VREDRAW | CS_HREDRAW, 0, 0,
...
TCollection_AsciiString CreateName(const NCollection_DoubleMap<TCollection_AsciiString, ObjectType>& theObjectMap,
    ...
    if (theObjectMap.IsEmpty()) return theDefaultString + TCollection_AsciiString(1);
    ...
    ViewerTest_Names(const TCollection_AsciiString& theInputString) {
        ...
        if (theInputString.IsEmpty()) {
            ...
            if (ViewerTest_myDrivers.IsEmpty())
                myDriverName = CreateName<Handle(Graphic3d_GraphicDriver)>(ViewerTest_myDrivers,
                                                                           TCollection_AsciiString("Driver"));
            else
                myDriverName = ViewerTest_myDrivers.Find2(ViewerTest::GetAISContext()->CurrentViewer()->Driver());
            ...
            myViewName =
                CreateName<Handle(V3d_View)>(ViewerTest_myViews, TCollection_AsciiString(myViewerName + "/View"));
        } else {
...
Handle(AIS_InteractiveContext) FindContextByView(const Handle(V3d_View) & theView) {
    ...
    for (NCollection_DoubleMap<TCollection_AsciiString, Handle(AIS_InteractiveContext)>::Iterator anIter(
             ViewerTest_myContexts);
         anIter.More(); anIter.Next()) {
        if (anIter.Value()->CurrentViewer() == theView->Viewer()) return anIter.Key2();
    }
    return anAISContext;
}
...
Standard_Boolean IsWindowOverlapped(const Standard_Integer thePxLeft, const Standard_Integer thePxTop,
                                    const Standard_Integer thePxRight, const Standard_Integer thePxBottom,
...
void ViewerTest::InitViewName(const TCollection_AsciiString& theName, const Handle(V3d_View) & theView) {
...
TCollection_AsciiString ViewerTest::ViewerInit(const Standard_Integer thePxLeft, const Standard_Integer thePxTop,
    ...
    if (!ViewerTest_myViews.IsEmpty() && thePxLeft == 0 && thePxTop == 0) {
        Standard_Integer aTop = 0, aLeft = 0, aRight = 0, aBottom = 0, aScreenWidth = 0, aScreenHeight = 0;
        ...
        while (IsWindowOverlapped(aPxLeft, aPxTop, aPxLeft + aPxWidth, aPxTop + aPxHeight, anOverlappedViewId)) {
            ...
            if (IsWindowOverlapped(aRight + 20, aPxTop, aRight + 20 + aPxWidth, aPxTop + aPxHeight,
                                   anOverlappedViewId) &&
    ...
    if (ViewerTest_myViews.IsEmpty() && !ViewerTest_myContexts.IsEmpty()) {
        NCollection_DoubleMap<TCollection_AsciiString, Handle(AIS_InteractiveContext)>::Iterator anIter(
            ViewerTest_myContexts);
        if (anIter.More()) ViewerTest::SetAISContext(anIter.Value());
    ...
    // AIS context setup
    if (ViewerTest::GetAISContext().IsNull() || !(ViewerTest_myContexts.IsBound1(aViewNames.GetViewerName()))) {
    ...
    // Create window
#if defined(_WIN32)
    VT_GetWindow() = new WNT_Window(aTitle.ToCString(), WClass(), isVirtual ? WS_POPUP : WS_OVERLAPPEDWINDOW, aPxLeft,
                                    aPxTop, aPxWidth, aPxHeight, Quantity_NOC_BLACK);
    VT_GetWindow()->RegisterRawInputDevices(WNT_Window::RawInputMask_SpaceMouse);
#elif defined(HAVE_XLIB)
    VT_GetWindow() =
        new Xw_Window(aGraphicDriver->GetDisplayConnection(), aTitle.ToCString(), aPxLeft, aPxTop, aPxWidth, aPxHeight);
#elif defined(__APPLE__)
    VT_GetWindow() = new Cocoa_Window(aTitle.ToCString(), aPxLeft, aPxTop, aPxWidth, aPxHeight);
    ViewerTest_SetCocoaEventManagerView(VT_GetWindow());
#elif defined(__EMSCRIPTEN__)
    // current EGL implementation in Emscripten supports only one global WebGL canvas returned by Module.canvas
    // property; the code should be revised for handling multiple canvas elements (which is technically also possible)
...
static int VInit(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIt = 1; anArgIt < theArgsNb; ++anArgIt) {
        ...
        if (anArgIt + 1 < theArgsNb && anArgCase == "-name") {
            ...
            isVirtual = true;
            if (anArgIt + 1 < theArgsNb && Draw::ParseOnOff(theArgVec[anArgIt + 1], isVirtual)) {
                ++anArgIt;
            }
            ...
            bool toEnable = true;
            if (anArgIt + 1 < theArgsNb && Draw::ParseOnOff(theArgVec[anArgIt + 1], toEnable)) {
                ++anArgIt;
            }
            is2dMode = toEnable ? 1 : 0;
        } else if (anArgIt + 1 < theArgsNb && (anArgCase == "-disp" || anArgCase == "-display")) {
            aDisplayName = theArgVec[++anArgIt];
        } else if (!ViewerTest::CurrentView().IsNull() && aCopyFrom.IsNull() &&
    ...
    TCollection_AsciiString aViewId =
        ViewerTest::ViewerInit(aPxLeft, aPxTop, aPxWidth, aPxHeight, aViewName, aDisplayName, aCopyFrom, isVirtual);
    if (is2dMode != -1) {
        ViewerTest_V3dView::SetCurrentView2DMode(is2dMode == 1);
    }
    theDi << aViewId;
...
static TCollection_AsciiString FindViewIdByWindowHandle(Aspect_Drawable theWindowHandle) {
    for (NCollection_DoubleMap<TCollection_AsciiString, Handle(V3d_View)>::Iterator anIter(ViewerTest_myViews);
        ...
        if (aWindowHandle == theWindowHandle) return anIter.Key1();
    }
...
void ActivateView(const TCollection_AsciiString& theViewName, Standard_Boolean theToUpdate = Standard_True) {
...
// purpose  :
//==============================================================================
void ViewerTest::RemoveView(const Handle(V3d_View) & theView, const Standard_Boolean theToRemoveContext) {
...
static int VClose(Draw_Interpretor& /*theDi*/, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (NCollection_List<TCollection_AsciiString>::Iterator anIter(aViewList); anIter.More(); anIter.Next()) {
...
static int VActivate(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (toUpdate && anArg == "-noupdate") {
            ...
            ViewerTest::CurrentView()->Window()->SetTitle(TCollection_AsciiString("3D View - ") +
...
static int VViewList(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    if (ViewerTest_myContexts.Size() < 1) return 0;
    ...
    for (NCollection_DoubleMap<TCollection_AsciiString, Handle(Graphic3d_GraphicDriver)>::Iterator aDriverIter(
             ViewerTest_myDrivers);
         aDriverIter.More(); aDriverIter.Next()) {
        if (isTreeView) theDi << aDriverIter.Key1() << ":\n";

        for (NCollection_DoubleMap<TCollection_AsciiString, Handle(AIS_InteractiveContext)>::Iterator aContextIter(
                 ViewerTest_myContexts);
             aContextIter.More(); aContextIter.Next()) {
            if (aContextIter.Key1().Search(aDriverIter.Key1()) != -1) {
                ...
                for (NCollection_DoubleMap<TCollection_AsciiString, Handle(V3d_View)>::Iterator aViewIter(
                         ViewerTest_myViews);
    ...
    if (aCmdName == "vfront") {
        ...
        for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
            ...
            if (anArgCase == "-zup") {
                ...
                isYup = Standard_True;
            } else if (anArgCase == "-front" || anArgCase == "front" || anArgCase == "-f" || anArgCase == "f") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Front : V3d_TypeOfOrientation_Zup_Front, isYup);
            } else if (anArgCase == "-back" || anArgCase == "back" || anArgCase == "-b" || anArgCase == "b") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Back : V3d_TypeOfOrientation_Zup_Back, isYup);
            } else if (anArgCase == "-top" || anArgCase == "top" || anArgCase == "-t" || anArgCase == "t") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Top : V3d_TypeOfOrientation_Zup_Top, isYup);
            } else if (anArgCase == "-bottom" || anArgCase == "bottom" || anArgCase == "-bot" || anArgCase == "bot" ||
                       anArgCase == "-b" || anArgCase == "b") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Bottom : V3d_TypeOfOrientation_Zup_Bottom, isYup);
            } else if (anArgCase == "-left" || anArgCase == "left" || anArgCase == "-l" || anArgCase == "l") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Left : V3d_TypeOfOrientation_Zup_Left, isYup);
            } else if (anArgCase == "-right" || anArgCase == "right" || anArgCase == "-r" || anArgCase == "r") {
                aView->SetProj(isYup ? V3d_TypeOfOrientation_Yup_Right : V3d_TypeOfOrientation_Zup_Right, isYup);
...
static LRESULT WINAPI AdvViewerWindowProc(HWND theWinHandle, UINT theMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (theMsg) {
        ...
        case WM_ACTIVATE: {
            if (LOWORD(wParam) == WA_CLICKACTIVE || LOWORD(wParam) == WA_ACTIVE || ViewerTest::CurrentView().IsNull()) {
...
static void VProcessEvents(ClientData theDispX, int) {
    ...
    // Sending a dummy event in this case is a simple workaround (still, it is possible that new event will be queued
    // in-between).
    if (aNbRemain != 0) {
        XEvent aDummyEvent;
...
static bool parseGradientMode(const TCollection_AsciiString& theName, Aspect_GradientFillMethod& theMode) {
    TCollection_AsciiString aName = theName;
    aName.LowerCase();
    if (aName == "none") {
...
static int VBackground(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            isDefault = true;
                   ...
                   (anArg == "-imagefile" || anArg == "-imgfile" || anArg == "-image" || anArg == "-img")) {
            ...
            anArgIter += 6;
        } else if (anArg == "-invertedz" || anArg == "-noinvertedz" || anArg == "-invz" || anArg == "-noinvz") {
            ...
            Standard_Integer aNbParsed = Draw::ParseColor(theNbArgs - (anArgIter + 1), theArgVec + (anArgIter + 1),
            ...
            ++aNbColors;
                   ...
                   (anArg == "-gradientmode" || anArg == "-gradmode" || anArg == "-gradmd" || anArg == "-grmode" ||
                    anArg == "-grmd") &&
                   parseGradientMode(theArgVec[anArgIter + 1], aGradientMode)) {
            ++anArgIter;
            hasGradientMode = true;
        } else if (anArgIter + 1 < theNbArgs &&
                   (anArg == "-imagemode" || anArg == "-imgmode" || anArg == "-imagemd" || anArg == "-imgmd") &&
            ...
            Standard_Integer aNbParsed1 = Draw::ParseColor(theNbArgs - (anArgIter + 1), theArgVec + (anArgIter + 1),
                                                           aColors[aNbColors].ChangeRGB());
            ...
            Standard_Integer aNbParsed2 = Draw::ParseColor(theNbArgs - (anArgIter + 1), theArgVec + (anArgIter + 1),
                                                           aColors[aNbColors].ChangeRGB());
            ...
            hasGradientMode = true;
        } else if (aNbColors < 2 && (Quantity_ColorRGBA::ColorFromName(theArgVec[anArgIter], aColors[aNbColors]) ||
    ...
    if (!aCubeMapSeq.IsEmpty()) {
        ...
        if (aCubeMapSeq.Size() == 1) {
            ...
            for (NCollection_Sequence<TCollection_AsciiString>::Iterator aFileIter(aCubeMapSeq); aFileIter.More();
...
static int VZBuffTrihedron(Draw_Interpretor& /*theDI*/, Standard_Integer theArgNb, const char** theArgVec) {
    Handle(V3d_View) aView = ViewerTest::CurrentView();
    if (aView.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(aFlag)) {
            ...
            } else if (aPosName == "left_lower" || aPosName == "lower_left" || aPosName == "leftlower" ||
                       aPosName == "lowerleft") {
                aPosition = Aspect_TOTP_LEFT_LOWER;
            } else if (aPosName == "left_upper" || aPosName == "upper_left" || aPosName == "leftupper" ||
                       aPosName == "upperleft") {
                aPosition = Aspect_TOTP_LEFT_UPPER;
            } else if (aPosName == "right_lower" || aPosName == "lower_right" || aPosName == "rightlower" ||
                       aPosName == "lowerright") {
                aPosition = Aspect_TOTP_RIGHT_LOWER;
            } else if (aPosName == "right_upper" || aPosName == "upper_right" || aPosName == "rightupper" ||
                       aPosName == "upperright") {
            ...
            Standard_Integer aNbParsed = Draw::ParseColor(theArgNb - anArgIter - 1, theArgVec + anArgIter + 1, aColor);
            if (aNbParsed == 0) {
                Message::SendFail() << "Error: wrong syntax at '" << anArg << "'";
                return 1;
            }
...
static int VColorScale(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    if (aColorScale.IsNull()) {
        ...
        aContext->SetTransformPersistence(aColorScale,
    ...
    for (Standard_Integer anArgIter = 2; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(aFlag)) {
        ...
        } else if (aFlag == "-reversed" || aFlag == "-inverted" || aFlag == "-topdown" || aFlag == "-bottomup") {
            ...
            anArgIter += 2;
        } else if (aFlag == "-labelat" || aFlag == "-labat" || aFlag == "-labelatborder" || aFlag == "-labatborder" ||
                   aFlag == "-labelatcenter" || aFlag == "-labatcenter") {
            Standard_Boolean toEnable = Standard_True;
            ...
            aColorScale->SetLabelAtBorder(aFlag == "-labelatcenter" || aFlag == "-labatcenter" ? !toEnable : toEnable);
            ...
            if (aSeq.Length() != aColorScale->GetNumberOfIntervals()) {
                Message::SendFail() << "Error: not enough arguments! You should provide color names or RGB color "
                                       "values for every interval of the "
            ...
            Standard_Integer aNbLabels = aColorScale->IsLabelAtBorder() ? aColorScale->GetNumberOfIntervals() + 1
                                                                        : aColorScale->GetNumberOfIntervals();
            if (aFlag == "-freelabels") {
                ++anArgIter;
...
static int VGraduatedTrihedron(Draw_Interpretor& /*theDi*/, Standard_Integer theArgNum, const char** theArgs) {
    ...
    // Check parameters
    for (NCollection_DataMap<TCollection_AsciiString, Handle(TColStd_HSequenceOfAsciiString)>::Iterator aMapIt(
             aMapOfArgs);
        ...
        // One argument
        if ((aKey.IsEqual("xname") || aKey.IsEqual("yname") || aKey.IsEqual("zname")) && anArgs->Length() == 1) {
            continue;
        }
...
static int VZLayer(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            for (ViewerTest_DoubleMapIteratorOfDoubleMapOfInteractiveAndName anObjIter(GetMapOfAIS()); anObjIter.More();
                 anObjIter.Next()) {
...
public:
    ...
    // constructor
    Standard_EXPORT V3d_LineItem(Standard_Real X1, Standard_Real Y1, Standard_Real X2, Standard_Real Y2,
                                 Aspect_TypeOfLine theType = Aspect_TOL_SOLID, Standard_Real theWidth = 0.5,
...
private:
    ...
    virtual void ComputeSelection(const Handle(SelectMgr_Selection) &, const Standard_Integer) Standard_OVERRIDE {}
...
V3d_LineItem::V3d_LineItem(Standard_Real X1, Standard_Real Y1, Standard_Real X2, Standard_Real Y2,
                           Aspect_TypeOfLine theType, Standard_Real theWidth, Standard_Real theTransp)
    : myX1(X1), myY1(Y1), myX2(X2), myY2(Y2), myType(theType), myWidth(theWidth) {
    SetTransparency(1 - theTransp);
}

// render line
void V3d_LineItem::Compute(const Handle(PrsMgr_PresentationManager) &,
                           const Handle(Prs3d_Presentation) & thePresentation, const Standard_Integer) {
...
static int VLayerLine(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    // has width
    if (argc > 5) aWidth = Draw::Atof(argv[5]);
    ...
    if (argc > 7) {
        ...
        if (aTransparency < 0 || aTransparency > 1.0) aTransparency = 1.0;
    }
...
static int VGrid(Draw_Interpretor& /*theDI*/, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(theArgVec[anArgIter])) {
            ...
            hasOrigin = true;
            aNewOriginXY.SetValues(Draw::Atof(theArgVec[anArgIter + 1]), Draw::Atof(theArgVec[anArgIter + 2]));
            anArgIter += 2;
        } else if (anArgIter + 2 < theArgNb && anArg == "-step") {
            hasStep = true;
            aNewStepXY.SetValues(Draw::Atof(theArgVec[anArgIter + 1]), Draw::Atof(theArgVec[anArgIter + 2]));
            ...
            hasSize = true;
            aNewSizeXY.SetValues(Draw::Atof(theArgVec[anArgIter + 1]), Draw::Atof(theArgVec[anArgIter + 2]));
            if (aNewStepXY.x() <= 0.0 || aNewStepXY.y() <= 0.0) {
...
static int VPriviledgedPlane(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
...
static int VConvert(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    if (aCoord.Length() == 2) {
        switch (aMode) {
            ...
            case Ray:
                aView->ConvertWithProj((Standard_Integer)aCoord(1), (Standard_Integer)aCoord(2), aXYZ[0], aXYZ[1],
                                       aXYZ[2], aXYZ[3], aXYZ[4], aXYZ[5]);
...
static int VFps(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (;; ++aFrameIter) {
        aView->Redraw();
        if ((aFramesNb > 0 && aFrameIter >= aFramesNb) || (aDuration > 0.0 && aTimer.ElapsedTime() >= aDuration)) {
            break;
        }
    }
    aTimer.Stop();
...
static int VReadPixel(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    if (anX < 0 || anX >= aWidth || anY < 0 || anY > aHeight) {
        Message::SendFail() << "Error: pixel coordinates (" << anX << "; " << anY << ") are out of view (" << aWidth
                            << " x " << aHeight << ")";
        return 1;
    }
    ...
    if (toShowName) {
        ...
        switch (aBufferType) {
            ...
            case Graphic3d_BT_RGBA: {
                const Graphic3d_Vec4 aVec4 = toShow_sRGB
                                                 ? Quantity_ColorRGBA::Convert_LinearRGB_To_sRGB((Graphic3d_Vec4)aColor)
...
public:
    //! Main constructor.
    ViewerTest_ImagePrs(const Handle(Image_PixMap) & theImage, const Standard_Real theWidth,
                        const Standard_Real theHeight, const TCollection_AsciiString& theLabel)
    ...
    virtual void Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePrs,
...
static int VDiffImage(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (; anArgIter < theArgNb; ++anArgIter) {
        TCollection_AsciiString anArg(theArgVec[anArgIter]);
        anArg.LowerCase();
        if (anArgIter + 1 < theArgNb && (anArg == "-toleranceofcolor" || anArg == "-tolerancecolor" ||
                                         anArg == "-tolerance" || anArg == "-toler")) {
    ...
    if (aDiffColorsNb > 0 && (!aDiffImagePath.IsEmpty() || !aPrsNameDiff.IsEmpty())) {
        ...
        if (!aDiff->InitTrash(Image_Format_Gray, anImgRef->SizeX(), anImgRef->SizeY())) {
            Message::SendFail() << "Error: cannot allocate memory for diff image " << anImgRef->SizeX() << "x"
                                << anImgRef->SizeY();
            return 1;
        }
    ...
    Standard_Integer aWinSizeY =
        !aDiff.IsNull() && !aPrsNameDiff.IsEmpty() ? int(anImgRef->SizeY() * 2) : int(anImgRef->SizeY());
    TCollection_AsciiString aDisplayName;
    TCollection_AsciiString aViewId =
        ViewerTest::ViewerInit(aPxLeft, aPxTop, aWinSizeX, aWinSizeY, aViewName, aDisplayName);
    ...
    if (!aDiff.IsNull()) {
        anImgDiffPrs = new ViewerTest_ImagePrs(aDiff, aSizeX, aSizeY,
                                               TCollection_AsciiString() + "Difference: " + aDiffColorsNb + " pixels");
        gp_Trsf aTrsfDiff;
...
//=======================================================================
static Standard_Integer VSelect(Draw_Interpretor&, Standard_Integer theNbArgs, const char** theArgVec) {
    const Handle(AIS_InteractiveContext) & aCtx = ViewerTest::GetAISContext();
    if (aCtx.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
...
//=======================================================================
static Standard_Integer VMoveTo(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    ViewerTest::CurrentEventManager()->UpdateMousePosition(aMousePos, Aspect_VKeyMouse_NONE, Aspect_VKeyFlags_NONE,
                                                           false);
...
// purpose  :
//=======================================================================
static Standard_Integer VSelectByAxis(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    const Handle(AIS_InteractiveContext) & aContext = ViewerTest::GetAISContext();
    const Handle(V3d_View) & aView = ViewerTest::CurrentView();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
    ...
    if (!aName.IsEmpty()) {
        ...
        Handle(Geom_Axis2Placement) anAx2Axis = new Geom_Axis2Placement(gp_Ax2(anAxisLocation, anAxisDirection));
        ...
        for (NCollection_Sequence<gp_Pnt>::Iterator aPntIter(aPoints); aPntIter.More(); aPntIter.Next(), anIndex++) {
            ...
            if (toShowNormal) {
                ...
                if (aNormal.SquareModulus() > ShortRealEpsilon()) {
                    gp_Dir aNormalDir((Standard_Real)aNormal.x(), (Standard_Real)aNormal.y(),
...
public:
    //! Main constructor.
    ViewerTest_AnimationProc(const TCollection_AsciiString& theAnimationName, Draw_Interpretor* theDI,
                             const TCollection_AsciiString& theCommand)
        : AIS_Animation(theAnimationName), myDrawInter(theDI), myCommand(theCommand) {
        //
    }
...
protected:
    ...
    void replace(TCollection_AsciiString& theCmd, const TCollection_AsciiString& theKey,
...
public:
    ViewerTest_AnimationHolder(const Handle(AIS_Animation) & theAnim, const Handle(V3d_View) & theView,
    ...
    //! Start playback.
    virtual void StartTimer(const Standard_Real theStartPts, const Standard_Real thePlaySpeed,
...
static void replaceAnimation(const Handle(AIS_Animation) & theParentAnimation, Handle(AIS_Animation) & theAnimation,
...
static int VViewParams(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    if (theArgsNb == 1) {
        ...
        Sprintf(aText,
                ...
                "              %12g %12g %12g %12g\n"
                "              %12g %12g %12g %12g\n",
                aViewScale, aViewAspect, aViewProj.X(), aViewProj.Y(), aViewProj.Z(), aViewUp.X(), aViewUp.Y(),
                aViewUp.Z(), aViewAt.X(), aViewAt.Y(), aViewAt.Z(), aViewEye.X(), aViewEye.Y(), aViewEye.Z(),
                anOrientMat.GetValue(0, 0), anOrientMat.GetValue(0, 1), anOrientMat.GetValue(0, 2),
                anOrientMat.GetValue(0, 3), anOrientMat.GetValue(1, 0), anOrientMat.GetValue(1, 1),
                anOrientMat.GetValue(1, 2), anOrientMat.GetValue(1, 3), anOrientMat.GetValue(2, 0),
                anOrientMat.GetValue(2, 1), anOrientMat.GetValue(2, 2), anOrientMat.GetValue(2, 3),
                anOrientMat.GetValue(3, 0), anOrientMat.GetValue(3, 1), anOrientMat.GetValue(3, 2),
                anOrientMat.GetValue(3, 3), aProjMat.GetValue(0, 0), aProjMat.GetValue(0, 1), aProjMat.GetValue(0, 2),
                aProjMat.GetValue(0, 3), aProjMat.GetValue(1, 0), aProjMat.GetValue(1, 1), aProjMat.GetValue(1, 2),
                aProjMat.GetValue(1, 3), aProjMat.GetValue(2, 0), aProjMat.GetValue(2, 1), aProjMat.GetValue(2, 2),
                aProjMat.GetValue(2, 3), aProjMat.GetValue(3, 0), aProjMat.GetValue(3, 1), aProjMat.GetValue(3, 2),
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            Sprintf(aText,
                    ...
                    "-up %g %g %g "
                    "-at %g %g %g\n",
                    aViewScale, aViewProj.X(), aViewProj.Y(), aViewProj.Z(), aViewUp.X(), aViewUp.Y(), aViewUp.Z(),
...
// purpose  :
//==============================================================================
static Standard_Integer VAnimation(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (; anArgIter < theArgNb; ++anArgIter) {
        ...
        // video recodring options
            ...
            if (anArgIter + 2 < theArgNb && *theArgVec[anArgIter + 1] != '-' && *theArgVec[anArgIter + 2] != '-') {
        ...
        // animation definition options
        ...
        } else if (anArg == "-objecttrsf" || anArg == "-objectransformation" || anArg == "-objtransformation" ||
                   anArg == "-objtrsf" || anArg == "-object" || anArg == "-obj") {
            if (++anArgIter >= theArgNb) {
                Message::SendFail() << "Syntax error at " << anArg;
                return 1;
            }
            ...
            for (; aTrsfArgIter < theArgNb; ++aTrsfArgIter) {
                ...
                if (aTrsfArg.StartsWith("-rotation") || aTrsfArg.StartsWith("-rot")) {
                    ...
                    isTrsfSet = Standard_True;
                    if (aTrsfArgIter + 3 >= theArgNb || !parseXYZ(theArgVec + aTrsfArgIter + 1, aLocPnts[anIndex])) {
                        Message::SendFail() << "Syntax error at " << aTrsfArg;
                        return 1;
                    }
                    aTrsfArgIter += 3;
            ...
            Handle(AIS_AnimationObject) anObjAnimation =
    ...
    int64_t aNbFrames = 0;
    Message_ProgressScope aPS(Message_ProgressIndicator::Start(aProgress), "Video recording, sec",
    ...
    for (; aPts <= anUpperPts && aPS.More();) {
        ...
        if (aRecParams.FpsNum > 0) {
            aRecPts = aPlaySpeed * ((Standard_Real(aRecParams.FpsDen) / Standard_Real(aRecParams.FpsNum)) *
...
//=======================================================================
static Standard_Integer VChangeSelected(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc != 2) {
...
// purpose  :
//=======================================================================
static Standard_Integer VTranslateView(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        di << "use 'vinit' command before " << argv[0] << "\n";
        return 1;
...
public:
    ...
    void SetTextureParameters(const Standard_Boolean theRepeatFlag, const Standard_Boolean theModulateFlag,
                              const Graphic3d_TypeOfTextureFilter theFilter, const Standard_ShortReal theXScale,
                              const Standard_ShortReal theYScale, const Standard_ShortReal theXShift,
                              const Standard_ShortReal theYShift, const Standard_ShortReal theAngle);
...
OCC_TextureEnv::OCC_TextureEnv(const Standard_CString theFileName) : Graphic3d_TextureEnv(theFileName) {}

OCC_TextureEnv::OCC_TextureEnv(const Graphic3d_NameOfTextureEnv theTexId) : Graphic3d_TextureEnv(theTexId) {}

void OCC_TextureEnv::SetTextureParameters(const Standard_Boolean theRepeatFlag, const Standard_Boolean theModulateFlag,
                                          const Graphic3d_TypeOfTextureFilter theFilter,
                                          const Standard_ShortReal theXScale, const Standard_ShortReal theYScale,
                                          const Standard_ShortReal theXShift, const Standard_ShortReal theYShift,
...
static int VTextureEnv(Draw_Interpretor& /*theDI*/, Standard_Integer theArgNb, const char** theArgVec) {
        ...
        if (isOk) {
            ...
            if (isOk && theArgNb == 11) {
                TCollection_AsciiString aRepeatOpt(theArgVec[3]), aModulateOpt(theArgVec[4]), aFilterOpt(theArgVec[5]),
                    aSScaleOpt(theArgVec[6]), aTScaleOpt(theArgVec[7]), aSTransOpt(theArgVec[8]),
                    aTTransOpt(theArgVec[9]), anAngleOpt(theArgVec[10]);
    ...
    if (!isOk) {
        Message::SendFail()
            ...
            << theArgVec[0] << " off\n"
            << theArgVec[0]
            << " on {index_of_std_texture(0..7)|texture_file_name} [{clamp|repeat} {decal|modulate} "
               "{nearest|bilinear|trilinear} scale_s scale_t translation_s translation_t rotation_degrees]";
        return 1;
    }
    ...
    if (anEnableFlag) {
        ...
        Handle(OCC_TextureEnv) aTexEnv =
            aTextureOpt.IsIntegerValue() ? new OCC_TextureEnv((Graphic3d_NameOfTextureEnv)aTextureOpt.IntegerValue())
                                         : new OCC_TextureEnv(theArgVec[2]);

        if (theArgNb == 11) {
            TCollection_AsciiString aRepeatOpt(theArgVec[3]), aModulateOpt(theArgVec[4]), aFilterOpt(theArgVec[5]);
            aTexEnv->SetTextureParameters(
                aRepeatOpt.IsEqual("repeat"), aModulateOpt.IsEqual("modulate"),
                ...
                (Standard_ShortReal)Draw::Atof(theArgVec[6]), (Standard_ShortReal)Draw::Atof(theArgVec[7]),
                (Standard_ShortReal)Draw::Atof(theArgVec[8]), (Standard_ShortReal)Draw::Atof(theArgVec[9]),
...
static void removePlane(MapOfPlanes& theRegPlanes, const TCollection_AsciiString& theName) {
    ...
    for (ViewerTest_DoubleMapIteratorOfDoubleMapOfInteractiveAndName anIObjIt(GetMapOfAIS()); anIObjIt.More();
         anIObjIt.Next()) {
...
static int VClipPlane(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    if (aCommand == "-maxplanes" || aCommand == "maxplanes") {
        theDi << anActiveView->Viewer()->Driver()->InquirePlaneLimit() << " plane slots provided by driver.\n";
        return 0;
    }
    ...
    for (; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (Draw::ParseOnOff(aChangeArgs[0], toEnable)) {
            ...
            aBndBox.Add(
                Graphic3d_Vec3d(Draw::Atof(aChangeArgs[1]), Draw::Atof(aChangeArgs[2]), Draw::Atof(aChangeArgs[3])));
            aBndBox.Add(
                Graphic3d_Vec3d(Draw::Atof(aChangeArgs[4]), Draw::Atof(aChangeArgs[5]), Draw::Atof(aChangeArgs[6])));
            anArgIter += 6;

            Standard_Integer aNbSubPlanes = 6;
            const Graphic3d_Vec3d aDirArray[6] = {
                Graphic3d_Vec3d(-1, 0, 0), Graphic3d_Vec3d(1, 0, 0),  Graphic3d_Vec3d(0, -1, 0),
                Graphic3d_Vec3d(0, 1, 0),  Graphic3d_Vec3d(0, 0, -1), Graphic3d_Vec3d(0, 0, 1),
            };
            Handle(Graphic3d_ClipPlane) aSubPln = aClipPlane;
        ...
        } else if (aChangeArg == "-useobjectmaterial" || aChangeArg == "-useobjectmat" || aChangeArg == "-useobjmat" ||
...
static int VCamera(Draw_Interpretor& theDI, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    if (theArgsNb < 2) {
        ...
        theDI << "ZFocusType: "
              << (aCamera->ZFocusType() == Graphic3d_Camera::FocusType_Absolute ? "absolute" : "relative") << "\n";
        theDI << "ZNear:      " << aCamera->ZNear() << "\n";
        theDI << "ZFar:       " << aCamera->ZFar() << "\n";
        return 0;
    }
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (anArgCase == "-proj" || anArgCase == "-projection" || anArgCase == "-projtype" ||
            ...
            aCamera->SetProjectionType(Graphic3d_Camera::Projection_Stereo);
                   ...
                   anArgCase == "-monolefteye" || anArgCase == "-perpsleft" || anArgCase == "-perpslefteye") {
        ...
        } else if (anArgCase == "-fov" || anArgCase == "-fovy" || anArgCase == "-fovx" || anArgCase == "-fov2d") {
            Standard_CString anArgValue = (anArgIter + 1 < theArgsNb) ? theArgVec[anArgIter + 1] : NULL;
            if (anArgValue != NULL && *anArgValue != '-') {
                ++anArgIter;
                if (anArgCase == "-fov2d") {
...
inline Standard_Boolean parseAnaglyphFilter(Standard_CString theArg, Graphic3d_RenderingParams::Anaglyph& theFilter) {
    TCollection_AsciiString aFlag(theArg);
    aFlag.LowerCase();
...
static int VStereo(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        } else if (aFlag == "-reverse" || aFlag == "-reversed" || aFlag == "-swap") {
            Standard_Boolean toEnable = Standard_True;
            if (++anArgIter < theArgNb && !Draw::ParseOnOff(theArgVec[anArgIter], toEnable)) {
                --anArgIter;
            }
            aParams->ToReverseStereo = toEnable;
        } else if (aFlag == "-noreverse" || aFlag == "-noswap") {
            Standard_Boolean toDisable = Standard_True;
            if (++anArgIter < theArgNb && !Draw::ParseOnOff(theArgVec[anArgIter], toDisable)) {
                --anArgIter;
            }
...
static int VDefaults(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        TCollection_AsciiString anArg(theArgVec[anArgIter]);
        anArg.UpperCase();
        if (anArg == "-ABSDEFL" || anArg == "-ABSOLUTEDEFLECTION" || anArg == "-DEFL" || anArg == "-DEFLECTION") {
...
static bool parseLightSourceType(const TCollection_AsciiString& theTypeName, Graphic3d_TypeOfLightSource& theType) {
...
static int VLight(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
            ...
            switch (aLight->Type()) {
                ...
                case V3d_DIRECTIONAL: {
                    theDi << "  Type:       Directional\n"
                          ...
                          << "  Direction:  " << aLight->PackedDirection().x() << " " << aLight->PackedDirection().y()
    ...
    for (Standard_Integer anArgIt = 1; anArgIt < theArgsNb; ++anArgIt) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            continue;
        } else if (anArgCase == "-new" || anArgCase == "-add" || anArgCase == "-create" || anArgCase == "-type" ||
                   (anArgCase == "-reset" && !aLightNew.IsNull()) ||
                   (anArgCase == "-defaults" && !aLightNew.IsNull()) || anArgCase == "add" || anArgCase == "new" ||
                   anArgCase == "create") {
            ...
            if (!ViewerTest::ParseZLayer(theArgVec[++anArgIt], aLayer) || aLayer == Graphic3d_ZLayerId_UNKNOWN) {
                Message::SendFail() << "Error: wrong syntax at '" << theArgVec[anArgIt] << "'";
                return 1;
            }
        } else if (anArgCase == "-glob" || anArgCase == "-global" || anArgCase == "-loc" || anArgCase == "-local") {
            ...
            aLightNew.Nullify();
            aLightPrs.Nullify();
        } else if (!aLightNew.IsNull() && (anArgCase == "-display" || anArgCase == "-disp" ||
                                           anArgCase == "-presentation" || anArgCase == "-prs")) {
            ...
            theDi << aLightName << " ";
        } else if (!aLightNew.IsNull() && (anArgCase == "-disable" || anArgCase == "-disabled" ||
                                           anArgCase == "-enable" || anArgCase == "-enabled")) {
        ...
        } else if (!aLightNew.IsNull() && (anArgCase == "-color" || anArgCase == "-colour" || anArgCase == "color")) {
            Quantity_Color aColor;
            Standard_Integer aNbParsed = Draw::ParseColor(theArgsNb - anArgIt - 1, theArgVec + anArgIt + 1, aColor);
            anArgIt += aNbParsed;
            if (aNbParsed == 0) {
                Message::SendFail() << "Syntax error at argument '" << anArg << "'";
                return 1;
            ...
            aLightNew->SetAngle(anAngle);
                   ...
                   (aLightNew->Type() == Graphic3d_TypeOfLightSource_Positional ||
                    aLightNew->Type() == Graphic3d_TypeOfLightSource_Spot) &&
                   (anArgCase == "-constatten" || anArgCase == "-constattenuation") && anArgIt + 1 < theArgsNb) {
            ...
            aLightNew->SetAttenuation(aConstAtten, aLightNew->LinearAttenuation());
                   ...
                   (aLightNew->Type() == Graphic3d_TypeOfLightSource_Positional ||
                    aLightNew->Type() == Graphic3d_TypeOfLightSource_Spot) &&
                   (anArgCase == "-linatten" || anArgCase == "-linearatten" || anArgCase == "-linearattenuation") &&
                   anArgIt + 1 < theArgsNb) {
            ...
            aLightNew->SetConcentration((Standard_ShortReal)Atof(theArgVec[++anArgIt]));
                   ...
                   aLightNew->Type() != Graphic3d_TypeOfLightSource_Directional && anArgCase == "-range" &&
                   anArgIt + 1 < theArgsNb) {
            ...
            aLightNew->SetName(aName);
                   ...
                   (anArgCase == "-showzoomable" || anArgCase == "-prszoomable" || anArgCase == "-zoomable")) {
            const bool isZoomable = Draw::ParseOnOffIterator(theArgsNb, theArgVec, anArgIt);
            aLightPrs->SetZoomable(isZoomable);
        } else if (!aLightPrs.IsNull() &&
                   (anArgCase == "-showdraggable" || anArgCase == "-prsdraggable" || anArgCase == "-draggable")) {
            ...
            aLightPrs->SetArcSize(aSize);
                   ...
                   (anArgCase == "-castshadow" || anArgCase == "-castshadows" || anArgCase == "-shadows")) {
        ...
        } else if (anArgCase == "-del" || anArgCase == "-delete" || anArgCase == "-remove" || anArgCase == "del" ||
                   anArgCase == "delete" || anArgCase == "remove") {
            if (aLightOld.IsNull()) {
                if (!aLightNew.IsNull()) {
                    aLightNew.Nullify();
                    continue;
    ...
    if (!aPrsList.IsEmpty()) {
        ...
        for (std::vector<Handle(AIS_LightSource)>::iterator aPrsIter = aLightPrsVec.begin();
             aPrsIter != aLightPrsVec.end(); ++aPrsIter) {
            Handle(AIS_LightSource) aLightPrs2 = *aPrsIter;
            if (!aLightPrs2->TransformPersistence().IsNull() && aLightPrs2->TransformPersistence()->IsTrihedronOr2d()) {
...
static Standard_Boolean parsePerfStatsFlag(const TCollection_AsciiString& theValue, Standard_Boolean& theToReset,
...
static Standard_Integer VRenderParams(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    Handle(V3d_View) aView = ViewerTest::CurrentView();
    if (aView.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(aFlag)) {
            ...
            if (aParam == "peeling" || aParam == "peel") {
                ...
                if (anArgIter + 1 < theArgNb && TCollection_AsciiString(theArgVec[anArgIter + 1]).IsIntegerValue()) {
                    ++anArgIter;
                    const Standard_Integer aNbLayers = TCollection_AsciiString(theArgVec[anArgIter]).IntegerValue();
                    if (aNbLayers < 2) {
                        Message::SendFail()
                            << "Syntax error: invalid layers number specified for Depth Peeling OIT " << aNbLayers;
                        return 1;
                    }
                ...
                if (anArgIter + 1 < theArgNb && TCollection_AsciiString(theArgVec[anArgIter + 1]).IsRealValue()) {
                    ++anArgIter;
                    ...
                    if (aWeight < 0.f || aWeight > 1.f) {
                        Message::SendFail() << "Syntax error: invalid value of Weighted Order-Independent Transparency "
                                               "depth weight factor "
                ...
                if (aWeight < 0.f || aWeight > 1.f) {
                    Message::SendFail()
                        << "Syntax error: invalid value of Weighted Order-Independent Transparency depth weight factor "
                        << aWeight << ". Should be within range [0.0; 1.0]";
                    return 1;
                }
            ...
            if (anArgIter + 1 < theArgNb && Draw::ParseOnOff(theArgVec[anArgIter + 1], aParams.ToEnableDepthPrepass)) {
                ++anArgIter;
            }
            ...
            if (aPbrEnvBakingDiffNbSamples < 1) {
                Message::SendFail(
                    "Syntax error: 'BakingDiffSamplesNumber' of PBR Environment has to be greater or equal 1");
                return 1;
            }
            ...
            if (aPbrEnvBakingSpecNbSamples < 1) {
                Message::SendFail(
                    "Syntax error: 'BakingSpecSamplesNumber' of PBR Environment has to be greater or equal 1");
                return 1;
            }
        ...
        } else if (aFlag == "-performancestats" || aFlag == "-performancecounters" || aFlag == "-perfstats" ||
                   aFlag == "-perfcounters" || aFlag == "-stats") {
            if (++anArgIter >= theArgNb) {
                Message::SendFail() << "Syntax error at argument '" << anArg << "'";
                return 1;
            }
    ...
    if (toSyncAllViews) {
        for (V3d_ListOfViewIterator aViewIter = ViewerTest::GetViewerFromContext()->DefinedViewIterator();
             aViewIter.More(); aViewIter.Next()) {
...
// purpose  :
//=======================================================================
static Standard_Integer VStatProfiler(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    Handle(V3d_View) aView = ViewerTest::CurrentView();
    if (aView.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (aFlag == "-noredraw") {
            ...
            else if (aFlag == "allstructs" || aFlag == "allstructures" || aFlag == "structs" || aFlag == "structures")
            ...
            else if (aFlag == "allarrays" || aFlag == "fillarrays" || aFlag == "linearrays" || aFlag == "pointarrays" ||
                     aFlag == "textarrays")
...
static int VManipulator(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    TCollection_AsciiString aName;
    // parameters
    Standard_Integer toAutoActivate = -1, toFollowTranslation = -1, toFollowRotation = -1, toFollowDragging = -1,
                     isZoomable = -1;
    ...
    for (; anArgIter < theArgsNb; ++anArgIter) {
        ...
        //
        ...
        } else if (anArg == "-gap" && anArgIter + 1 < theArgsNb && Draw::ParseReal(theArgVec[anArgIter + 1], aGap) &&
                   aGap >= 0.0) {
            ++anArgIter;
        } else if (anArg == "-size" && anArgIter + 1 < theArgsNb && Draw::ParseReal(theArgVec[anArgIter + 1], aSize) &&
                   aSize > 0.0) {
            ++anArgIter;
            ...
            if (anArg == "-part") {
                if (!Draw::ParseInteger(theArgVec[++anArgIter], aPart.Axis) || aPart.Axis < 0 || aPart.Axis > 3) {
            ...
            aParts.Append(aPart);
        } else if (anArg == "-pos" && anArgIter + 3 < theArgsNb && parseXYZ(theArgVec + anArgIter + 1, aLocation)) {
            anArgIter += 3;
        ...
        } else if (anArg == "-enablemodes" || anArg == "-enablemodes") {
        ...
        //
        else if (anArg == "-move" && anArgIter + 3 < theArgsNb && parseXYZ(theArgVec + anArgIter + 1, aTmpXYZ)) {
            anArgIter += 3;
        ...
        } else if (anArg == "-rotate" && anArgIter + 7 < theArgsNb && parseXYZ(theArgVec + anArgIter + 1, aRotPnt) &&
        ...
        //
            ...
            for (ViewerTest_DoubleMapIteratorOfDoubleMapOfInteractiveAndName anIter(aMapAIS); anIter.More();
                 anIter.Next()) {
    ...
    if (anAttachPos != ManipAjustPosition_Off && aManipulator->IsAttached() &&
        ...
        switch (anAttachPos) {
            ...
            case ManipAjustPosition_Center: {
                Bnd_Box aBox;
                for (AIS_ManipulatorObjectSequence::Iterator anObjIter(*aManipulator->Objects()); anObjIter.More();
                     anObjIter.Next()) {
                    Bnd_Box anObjBox;
...
// purpose  :
//===============================================================================================
static int VSelectionProperties(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    const Handle(AIS_InteractiveContext) & aCtx = ViewerTest::GetAISContext();
    if (aCtx.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    if (anArgIter < theArgsNb) {
        ...
        ++anArgIter;
        if (anArgFirst == "dynhighlight" || anArgFirst == "dynhilight" || anArgFirst == "dynamichighlight" ||
        ...
        } else if (anArgFirst == "selhighlight" || anArgFirst == "selhilight" || anArgFirst == "selectedhighlight" ||
    ...
    for (; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (anArg == "-help") {
        ...
        } else if (anArg == "-automatichighlight" || anArg == "-automatichilight" || anArg == "-autohighlight" ||
                   anArg == "-autohilight") {
            Standard_Boolean toEnable = Standard_True;
            if (anArgIter + 1 < theArgsNb && Draw::ParseOnOff(theArgVec[anArgIter + 1], toEnable)) {
                ++anArgIter;
            ...
            Quantity_Color aColor;
            Standard_Integer aNbParsed = Draw::ParseColor(theArgsNb - anArgIter - 1, theArgVec + anArgIter + 1, aColor);
            if (aNbParsed == 0) {
                Message::SendFail("Syntax error: need more arguments");
                return 1;
            }
            ...
            toRedraw = Standard_True;
        } else if ((anArg == "-transp" || anArg == "-transparency" || anArg == "-hitransp" || anArg == "-seltransp" ||
                    anArg == "-hitransplocal" || anArg == "-seltransplocal") &&
...
// purpose  :
//===============================================================================================
static int VDumpSelectionImage(Draw_Interpretor& /*theDi*/, Standard_Integer theArgsNb, const char** theArgVec) {
    const Handle(AIS_InteractiveContext) & aContext = ViewerTest::GetAISContext();
    const Handle(V3d_View) & aView = ViewerTest::CurrentView();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (aParam == "-type") {
            ...
            } else if (aValue == "selectionmodecolor" || aValue == "selectionmode" || aValue == "selmodecolor" ||
                       aValue == "selmode") {
...
static int VViewCube(Draw_Interpretor&, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
        ...
        } else if (anArg == "-color" || anArg == "-boxcolor" || anArg == "-boxsidecolor" || anArg == "-sidecolor" ||
                   anArg == "-boxedgecolor" || anArg == "-edgecolor" || anArg == "-boxcornercolor" ||
                   anArg == "-cornercolor" || anArg == "-innercolor" || anArg == "-textcolor" ||
                   anArg == "-xaxistextcolor" || anArg == "-yaxistextcolor" || anArg == "-zaxistextcolor") {
            Standard_Integer aNbParsed =
            ...
            } else if (anArg == "-xaxistextcolor" || anArg == "-yaxistextcolor" || anArg == "-zaxistextcolor") {
                Prs3d_DatumParts aDatum =
                    anArg.Value(2) == 'x' ? Prs3d_DatumParts_XAxis
        ...
        } else if (anArgIter + 1 < theNbArgs && (anArg == "-boxfacet" || anArg == "-boxfacetextension" ||
                                                 anArg == "-facetextension" || anArg == "-extension")) {
        ...
        } else if (anArgIter + 1 < theNbArgs && (anArg == "-boxcornerminsize" || anArg == "-cornerminsize")) {
...
void ViewerTest::ViewerCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("vhelp", "vhelp            : display help on the viewer commands", __FILE__, VHelp, group);
    theCommands.Add("vviewproj",
                    ...
                    "\n\t\t:             for example '+X+Z' will show front of the model with Z-up." __FILE__,
                    VViewProj, group);
    theCommands.Add("vtop", "vtop or <T>      : Top view. Orientation +X+Y", __FILE__, VViewProj, group);
    theCommands.Add("vbottom", "vbottom          : Bottom view. Orientation +X-Y", __FILE__, VViewProj, group);
    theCommands.Add("vleft", "vleft            : Left view. Orientation -Y+Z", __FILE__, VViewProj, group);
    theCommands.Add("vright", "vright           : Right view. Orientation +Y+Z", __FILE__, VViewProj, group);
    theCommands.Add("vaxo", " vaxo or <A>     : Axonometric view. Orientation +X-Y+Z", __FILE__, VViewProj, group);
    theCommands.Add("vfront", "vfront           : Front view. Orientation +X+Z", __FILE__, VViewProj, group);
    theCommands.Add("vback", "vback            : Back view. Orientation -X+Z", __FILE__, VViewProj, group);
    theCommands.Add("vpick", "vpick           : vpick X Y Z [shape subshape] ( all variables as string )", VPick,
                    group);
    ...
    theCommands.Add(
        ...
        "\n\t\t:            [-gradientMode "
        "{NONE|HORIZONTAL|VERTICAL|DIAG1|DIAG2|CORNER1|CORNER2|CORNER3|ELLIPTICAL}]=VERT]"
        ...
        __FILE__, VBackground, group);
    theCommands.Add("vsetbg", "\n\t\t: Alias for 'vbackground -imageFile ImageFile [-imageMode FillType]'.", __FILE__,
                    VBackground, group);
    theCommands.Add("vsetbgmode", "\n\t\t: Alias for 'vbackground -imageMode FillType'.", __FILE__, VBackground, group);
    theCommands.Add("vsetgradientbg",
                    "\n\t\t: Alias for 'vbackground -gradient Color1 Color2 -gradientMode FillMethod'.", __FILE__,
                    VBackground, group);
    theCommands.Add("vsetgrbgmode", "\n\t\t: Alias for 'vbackground -gradientMode FillMethod'.", __FILE__, VBackground,
                    group);
    theCommands.Add("vsetcolorbg", "\n\t\t: Alias for 'vbackground -color Color'.", __FILE__, VBackground, group);
    theCommands.Add("vsetdefaultbg",
                    ...
                    __FILE__, VBackground, group);
    theCommands.Add("vscale", "vscale          : vscale X Y Z", __FILE__, VScale, group);
    ...
    theCommands.Add("vzoom", "vzoom           : vzoom coef", __FILE__, VZoom, group);
    theCommands.Add("vpan", "vpan            : vpan dx dy", __FILE__, VPan, group);
    ...
    theCommands.Add("vlayerline", "vlayerline : vlayerline x1 y1 x2 y2 [linewidth=0.5] [linetype=0] [transparency=1.0]",
    ...
    theCommands.Add("vfps", "vfps [framesNb=100] [-duration seconds] : estimate average frame rate for active view",
                    __FILE__, VFps, group);
    ...
    theCommands.Add(
        ...
        "     If the flag is set to 1, both sensitives that were included completely and overlapped partially by "
        "defined \n"
        "     rectangle or polygon will be detected, otherwise algorithm will chose only fully included sensitives.\n"
        "     Default behavior is to detect only full inclusion. (partial inclusion - overlap - is not allowed by "
        "default)\n"
    ...
    theCommands.Add("vanimation", "Alias for vanim", __FILE__, VAnimation, group);
    ...
    theCommands.Add("vautozfit",
                    "command to enable or disable automatic z-range adjusting\n"
    ...
    theCommands.Add("vzrange",
                    "command to manually access znear and zfar values\n"
                    "   vzrange                - without parameters shows current values\n"
                    "   vzrange [znear] [zfar] - applies provided values to view",
                    __FILE__, VZRange, group);
    theCommands.Add("vsetviewsize", "vsetviewsize size", __FILE__, VSetViewSize, group);
    theCommands.Add("vmoveview", "vmoveview Dx Dy Dz [Start = 1|0]", __FILE__, VMoveView, group);
    theCommands.Add("vtranslateview", "vtranslateview Dx Dy Dz [Start = 1|0)]", __FILE__, VTranslateView, group);
    theCommands.Add("vturnview", "vturnview Ax Ay Az [Start = 1|0]", __FILE__, VTurnView, group);
    theCommands.Add(
        "vtextureenv",
        ...
        "                  vtextureenv on {std_texture|texture_file_name} [rep mod flt ss st ts tt rot] - enables "
        "environment mapping\n"
        "                              std_texture = (0..7)\n"
        "                              rep         = {clamp|repeat}\n"
        "                              mod         = {decal|modulate}\n"
    ...
    theCommands.Add("vlight",
                    ...
                    "\n\t\t:   -arcSize     sets arc presentation size(in pixels) for rotation directional light "
                    "source; 25 by default"
                    "\n\t\t: Examples:"
    ...
    theCommands.Add("vxrotate", "vxrotate", __FILE__, VXRotate, group);
    ...
    theCommands.Add("vhighlightselected", "vhighlightselected [0|1]: alias for vselprops -highlightSelected.\n",
                    __FILE__, VSelectionProperties, group);
    ...
    theCommands.Add("vcolordiff",
                    "vcolordiff R1 G1 B1 R2 G2 B2: returns CIEDE2000 color difference between two RGB colors", __FILE__,
                    VColorDiff, group);
