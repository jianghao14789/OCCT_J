...
// purpose  :
//=======================================================================
Standard_Integer ViewerTest::ParseColor(const Standard_Integer theArgNb, const char* const* const theArgVec,
...
// purpose  :
//=======================================================================
Standard_Boolean ViewerTest::ParseLineType(Standard_CString theArg, Aspect_TypeOfLine& theType, uint16_t& thePattern) {
...
// purpose  :
//=======================================================================
Standard_Boolean ViewerTest::ParseMarkerType(Standard_CString theArg, Aspect_TypeOfMarker& theType,
...
// purpose  :
//=======================================================================
Standard_Boolean ViewerTest::ParseShadingModel(Standard_CString theArg, Graphic3d_TypeOfShadingModel& theModel) {
    TCollection_AsciiString aTypeStr(theArg);
    aTypeStr.LowerCase();
    ...
    } else if (aTypeStr == "phong" || aTypeStr == "fragment" || aTypeStr == "frag" || aTypeStr == "pixel") {
...
// purpose  :
//=======================================================================
Standard_Boolean ViewerTest::parseZLayer(Standard_CString theArg, Standard_Boolean theToAllowInteger,
...
static const char** GetTypeNames() {
    static const char* names[14] = {"Point",           "Axis",       "Trihedron",  "PlaneTrihedron", "Line",
                                    "Circle",          "Plane",      "Shape",      "ConnectedShape", "MultiConn.Shape",
                                    "ConnectedInter.", "MultiConn.", "Constraint", "Dimension"};
...
// purpose  :
//=======================================================================
static void GetTypeAndSignfromString(const char* theName, AIS_KindOfInteractive& theType, Standard_Integer& theSign) {
...
Standard_Boolean ViewerTest::Display(const TCollection_AsciiString& theName,
                                     ...
                                     const Standard_Boolean theToUpdate, const Standard_Boolean theReplaceIfExists) {
...
// purpose  :
//=======================================================================
static Standard_Boolean getCtxAndView(Handle(AIS_InteractiveContext) & theCtx, Handle(V3d_View) & theView) {
...
void ViewerTest::Clear() {
    ...
    if (aListRemoved.Size() == GetMapOfAIS().Extent()) {
        ...
        for (NCollection_Sequence<Handle(AIS_InteractiveObject)>::Iterator anObjIter(aListRemoved); anObjIter.More();
             anObjIter.Next()) {
...
//==============================================================================
static Handle(Prs3d_IsoAspect)
    CopyIsoAspect(const Handle(Prs3d_IsoAspect) & theIsoAspect, const Standard_Integer theNbIsos) {
...
static int VDir(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    for (ViewerTest_DoubleMapIteratorOfDoubleMapOfInteractiveAndName anIter(GetMapOfAIS()); anIter.More();
         anIter.Next()) {
        if (!aMatch.IsEmpty()) {
...
enum ViewerTest_StereoPair { ViewerTest_SP_Single, ViewerTest_SP_SideBySide, ViewerTest_SP_OverUnder };

//==============================================================================
// function : VDump
// purpose  : To dump the active view snapshot to image file
//==============================================================================
static Standard_Integer VDump(Draw_Interpretor& theDI, Standard_Integer theArgNb, Standard_CString* theArgVec) {
            ...
            if (anXRArg == "base") {
                ...
                if (aView->View()->IsActiveXR()) {
                    ...
                    aView->View()->ComputeXRPosedCameraFromBase(
                        *aCustomCam, anXRArg == "handleft" ? aView->View()->XRSession()->LeftHandPose()
    ...
    switch (aStereoPair) {
        case ViewerTest_SP_Single: {
            if (!aView->ToPixMap(aPixMap, aParams)) {
                ...
                theDI << "Fail: dumped dimensions " << (Standard_Integer)aPixMap.SizeX() << "x"
                      << (Standard_Integer)aPixMap.SizeY() << " are lesser than requested " << aParams.Width << "x"
                      << aParams.Height << "\n";
            }
            break;
        }
        case ViewerTest_SP_SideBySide: {
            ...
            aPixMapL.InitWrapper(aPixMap.Format(), aPixMap.ChangeData(), aParams.Width, aParams.Height,
...
enum TypeOfDispOperation { TypeOfDispOperation_SetDispMode, TypeOfDispOperation_UnsetDispMode };

//! Displays,Erase...
static void VwrTst_DispErase(const Handle(AIS_InteractiveObject) & thePrs, const Standard_Integer theMode,
                             const TypeOfDispOperation theType, const Standard_Boolean theToUpdate) {
    Handle(AIS_InteractiveContext) aCtx = ViewerTest::GetAISContext();
    switch (theType) {
...
static int VDispMode(Draw_Interpretor&, Standard_Integer argc, const char** argv) {
    ...
    TypeOfDispOperation aType = TCollection_AsciiString(argv[0]) == "vunsetdispmode" ? TypeOfDispOperation_UnsetDispMode
    ...
    if (aType == TypeOfDispOperation_UnsetDispMode) {
        if (argc == 1) {
            if (aCtx->NbSelected() == 0) {
                VwrTst_DispErase(Handle(AIS_InteractiveObject)(), -1, TypeOfDispOperation_UnsetDispMode,
                                 Standard_False);
            } else {
                for (aCtx->InitSelected(); aCtx->MoreSelected(); aCtx->NextSelected()) {
                    VwrTst_DispErase(aCtx->SelectedInteractive(), -1, TypeOfDispOperation_UnsetDispMode,
                                     Standard_False);
                }
            }
        ...
        if (aCtx->NbSelected() == 0 && aType == TypeOfDispOperation_SetDispMode) {
            VwrTst_DispErase(Handle(AIS_InteractiveObject)(), aDispMode, TypeOfDispOperation_SetDispMode,
                             Standard_True);
        }
        for (aCtx->InitSelected(); aCtx->MoreSelected(); aCtx->NextSelected()) {
...
public:
    ...
    ViewTest_PrsIter(const TCollection_AsciiString& theName) : mySource(IterSource_All) {
        NCollection_Sequence<TCollection_AsciiString> aNames;
        if (!theName.IsEmpty()) aNames.Append(theName);
        Init(aNames);
    }

    //! Create and initialize iterator object.
    ViewTest_PrsIter(const NCollection_Sequence<TCollection_AsciiString>& theNames) : mySource(IterSource_All) {
        Init(theNames);
    }
...
private:
    enum IterSource { IterSource_All, IterSource_List, IterSource_Selected };

private:
    Handle(AIS_InteractiveContext) mySelIter; //!< iterator for current (selected) objects (IterSource_Selected)
    ViewerTest_DoubleMapIteratorOfDoubleMapOfInteractiveAndName
        myMapIter; //!< iterator for map of all objects (IterSource_All)
...
static bool parseInteriorStyle(const TCollection_AsciiString& theArg, Aspect_InteriorStyle& theStyle) {
...
struct ViewerTest_AspectsChangeSet {
    ...
    ViewerTest_AspectsChangeSet()
        : ToSetVisibility(0), Visibility(1), ToSetColor(0), Color(DEFAULT_COLOR), ToSetBackFaceColor(0),
          BackFaceColor(DEFAULT_COLOR), ToSetLineWidth(0), LineWidth(1.0), ToSetTypeOfLine(0),
          StippleLinePattern(0xFFFF), StippleLineFactor(1), ToSetTypeOfMarker(0), TypeOfMarker(Aspect_TOM_PLUS),
          ToSetMarkerSize(0), MarkerSize(1.0), ToSetTransparency(0), Transparency(0.0), ToSetAlphaMode(0),
          AlphaMode(Graphic3d_AlphaMode_BlendAuto), AlphaCutoff(0.5f), ToSetMaterial(0),
          Material(Graphic3d_NameOfMaterial_DEFAULT), ToSetShowFreeBoundary(0), ToSetFreeBoundaryWidth(0),
          FreeBoundaryWidth(1.0), ToSetFreeBoundaryColor(0), FreeBoundaryColor(DEFAULT_FREEBOUNDARY_COLOR),
          ToEnableIsoOnTriangulation(0),
          //
          ToSetFaceBoundaryDraw(0), ToSetFaceBoundaryUpperContinuity(0), FaceBoundaryUpperContinuity(GeomAbs_CN),
          ToSetFaceBoundaryColor(0), FaceBoundaryColor(Quantity_NOC_BLACK), ToSetFaceBoundaryWidth(0),
          FaceBoundaryWidth(1.0f), ToSetTypeOfFaceBoundaryLine(0), TypeOfFaceBoundaryLine(Aspect_TOL_SOLID),
          //
          ToSetMaxParamValue(0), MaxParamValue(500000), ToSetSensitivity(0), SelectionMode(-1), Sensitivity(-1),
          ToSetHatch(0), StdHatchStyle(-1), ToSetShadingModel(0), ShadingModel(Graphic3d_TypeOfShadingModel_DEFAULT),
          ToSetInterior(0), InteriorStyle(Aspect_IS_SOLID), ToSetDrawSilhouette(0), ToSetDrawEdges(0),
          ToSetQuadEdges(0), ToSetEdgeColor(0), ToSetEdgeWidth(0), EdgeWidth(1.0), ToSetTypeOfEdge(0),
          TypeOfEdge(Aspect_TOL_SOLID) {}

    //! @return true if no changes have been requested
    Standard_Boolean IsEmpty() const {
        return ToSetVisibility == 0 && ToSetLineWidth == 0 && ToSetTransparency == 0 && ToSetAlphaMode == 0 &&
               ToSetColor == 0 && ToSetBackFaceColor == 0 && ToSetMaterial == 0 && ToSetShowFreeBoundary == 0 &&
               ToSetFreeBoundaryColor == 0 && ToSetFreeBoundaryWidth == 0 && ToEnableIsoOnTriangulation == 0 &&
               ToSetFaceBoundaryDraw == 0 && ToSetFaceBoundaryUpperContinuity == 0 && ToSetFaceBoundaryColor == 0 &&
               ToSetFaceBoundaryWidth == 0 && ToSetTypeOfFaceBoundaryLine == 0 && ToSetMaxParamValue == 0 &&
               ToSetSensitivity == 0 && ToSetHatch == 0 && ToSetShadingModel == 0 && ToSetInterior == 0 &&
               ToSetDrawSilhouette == 0 && ToSetDrawEdges == 0 && ToSetQuadEdges == 0 && ToSetEdgeColor == 0 &&
    ...
    Standard_Boolean Validate() const {
        ...
        if (Visibility != 0 && Visibility != 1) {
            Message::SendFail()
                << "Error: the visibility should be equal to 0 or 1 (0 - invisible; 1 - visible) (specified "
                << Visibility << ")";
            isOk = Standard_False;
        }
        ...
        if (FreeBoundaryWidth <= 0.0 || FreeBoundaryWidth > 10.0) {
            Message::SendFail() << "Error: the free boundary width should be within [1; 10] range (specified "
                                << FreeBoundaryWidth << ")";
            isOk = Standard_False;
        }
        if (MaxParamValue < 0.0) {
            Message::SendFail() << "Error: the max parameter value should be greater than zero (specified "
                                << MaxParamValue << ")";
            isOk = Standard_False;
        }
        ...
        if (ToSetShadingModel == 1 && (ShadingModel < Graphic3d_TypeOfShadingModel_DEFAULT ||
    ...
    bool Apply(const Handle(Prs3d_Drawer) & theDrawer) {
        ...
        if (ToSetTypeOfMarker != 0) {
            if (ToSetTypeOfMarker != -1 || theDrawer->HasOwnPointAspect()) {
                ...
                theDrawer->PointAspect()->Aspect()->SetMarkerImage(
                    MarkerImage.IsNull() ? Handle(Graphic3d_MarkerImage)() : new Graphic3d_MarkerImage(MarkerImage));
            }
        }
        ...
        if (ToSetInterior != 0) {
            if (ToSetInterior != -1 || theDrawer->HasOwnShadingAspect()) {
                ...
                if (InteriorStyle == Aspect_IS_HATCH && theDrawer->ShadingAspect()->Aspect()->HatchStyle().IsNull()) {
...
// purpose  :
//==============================================================================
static Standard_Integer VAspects(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    if (aCmdName == "vsetwidth") {
        ...
        if (aNames.Size() >= 2 && aNames.Value(2).IsIntegerValue()) {
            ...
            if (aNames.Size() == 5 && aNames.Value(3).IsIntegerValue() && aNames.Value(4).IsIntegerValue() &&
                ...
                aChangeSet->FaceBoundaryColor =
                    Quantity_Color(aNames.Value(3).IntegerValue() / 255.0, aNames.Value(4).IntegerValue() / 255.0,
                                   aNames.Value(5).IntegerValue() / 255.0, Quantity_TOC_sRGB);
                aNames.Remove(5);
                aNames.Remove(4);
                aNames.Remove(3);
            }
    ...
    for (; anArgIter < theArgNb; ++anArgIter) {
        TCollection_AsciiString anArg = theArgVec[anArgIter];
        anArg.LowerCase();
        if (anArg == "-setwidth" || anArg == "-width" || anArg == "-setlinewidth" || anArg == "-linewidth" ||
            anArg == "-setedgewidth" || anArg == "-setedgeswidth" || anArg == "-edgewidth" || anArg == "-edgeswidth" ||
            anArg == "-setfaceboundarywidth" || anArg == "-setboundarywidth" || anArg == "-faceboundarywidth" ||
            anArg == "-boundarywidth") {
            if (++anArgIter >= theArgNb) {
                Message::SendFail() << "Error: wrong syntax at " << anArg;
                return 1;
            ...
            if (anArg == "-setedgewidth" || anArg == "-setedgeswidth" || anArg == "-edgewidth" ||
                ...
                aChangeSet->EdgeWidth = aWidth;
                       ...
                       anArg == "-faceboundarywidth" || anArg == "-boundarywidth" || aCmdName == "vshowfaceboundary") {
                aChangeSet->ToSetFaceBoundaryWidth = 1;
            ...
            aChangeSet->Transparency = 0.0;
                   ...
                   anArg == "-setfaceboundarycolor" || anArg == "-setboundarycolor" || anArg == "-faceboundarycolor" ||
                   anArg == "-boundarycolor") {
            ...
            if (aCmdName == "vsetedgetype") {
                ...
                aChangeSet->EdgeColor = Quantity_ColorRGBA(aColor);
                       ...
                       anArg == "-setboundarycolor" || anArg == "-faceboundarycolor" || anArg == "-boundarycolor") {
                aChangeSet->ToSetFaceBoundaryColor = 1;
                ...
                aChangeSet->Color = aColor;
                   ...
                   anArg == "-setfaceboundarystyle" || anArg == "-faceboundarystyle" || anArg == "-boundarystyle" ||
                   anArg == "-setfaceboundarytype" || anArg == "-faceboundarytype" || anArg == "-setboundarytype" ||
            ...
            if (anArg == "-setedgetype" || anArg == "-setedgestype" || anArg == "-edgetype" || anArg == "-edgestype" ||
                aCmdName == "vsetedgetype") {
                aChangeSet->TypeOfEdge = Graphic3d_Aspects::DefaultLineTypeForPattern(aLinePattern);
                aChangeSet->ToSetTypeOfEdge = 1;
            } else if (anArg == "-setfaceboundarystyle" || anArg == "-faceboundarystyle" || anArg == "-boundarystyle" ||
                       anArg == "-setfaceboundarytype" || anArg == "-faceboundarytype" || anArg == "-setboundarytype" ||
        ...
        } else if (anArg == "-setstipplelinefactor" || anArg == "-setstipplefactor" || anArg == "-setlinefactor" ||
                   anArg == "-stipplelinefactor" || anArg == "-stipplefactor" || anArg == "-linefactor") {
        ...
        } else if (anArg == "-setmat" || anArg == "-mat" || anArg == "-setmaterial" || anArg == "-material") {
            if (++anArgIter >= theArgNb) {
                Message::SendFail() << "Error: wrong syntax at " << anArg;
                return 1;
            }
        ...
        } else if (anArg == "-setfreeboundary" || anArg == "-freeboundary" || anArg == "-setfb" || anArg == "-fb") {
            bool toEnable = true;
        ...
        } else if (anArg == "-setfreeboundarywidth" || anArg == "-freeboundarywidth" || anArg == "-setfbwidth" ||
                   anArg == "-fbwidth") {
            if (++anArgIter >= theArgNb) {
                Message::SendFail() << "Error: wrong syntax at " << anArg;
                return 1;
        ...
        } else if (anArg == "-setfreeboundarycolor" || anArg == "-freeboundarycolor" || anArg == "-setfbcolor" ||
                   anArg == "-fbcolor") {
            Standard_Integer aNbParsed =
                Draw::ParseColor(theArgNb - anArgIter - 1, theArgVec + anArgIter + 1, aChangeSet->FreeBoundaryColor);
            if (aNbParsed == 0) {
                Message::SendFail() << "Syntax error at '" << anArg << "'";
                return 1;
            }
        ...
        } else if (anArg == "-setisoontriangulation" || anArg == "-isoontriangulation" || anArg == "-setisoontriang" ||
                   anArg == "-isoontriang") {
            bool toEnable = true;
            if (!Draw::ParseOnOff(anArgIter + 1 < theArgNb ? theArgVec[anArgIter + 1] : "", toEnable)) {
                Message::SendFail() << "Error: wrong syntax at " << anArg;
            ...
            aChangeSet->ToEnableIsoOnTriangulation = toEnable ? 1 : -1;
                   ...
                   anArg == "-setshowfaceboundaries" || anArg == "-setdrawfaceedges" || anArg == "-faceboundarydraw" ||
                   anArg == "-drawfaceboundary" || anArg == "-drawfaceboundaries" || anArg == "-showfaceboundary" ||
                   anArg == "-showfaceboundaries" || anArg == "-drawfaceedges" || anArg == "-faceboundary" ||
                   anArg == "-faceboundaries" || anArg == "-faceedges") {
            bool toEnable = true;
            if (!Draw::ParseOnOff(anArgIter + 1 < theArgNb ? theArgVec[anArgIter + 1] : "", toEnable)) {
                Message::SendFail() << "Error: wrong syntax at " << anArg;
                return 1;
            ...
            if (aNames.IsEmpty()) {
                Message::SendFail()
                    << "Error: object and selection mode should specified explicitly when -setSensitivity is used!";
                return 1;
            }
        ...
        } else if (anArg == "-setquadedges" || anArg == "-setquads" || anArg == "-quads" || anArg == "-skipfirstedge") {
            ...
            aChangeSet->ToSetQuadEdges = isQuadMode ? 1 : -1;
                   ...
                   anArg == "-edgescolor") {
            Standard_Integer aNbParsed =
                Draw::ParseColor(theArgNb - anArgIter - 1, theArgVec + anArgIter + 1, aChangeSet->EdgeColor);
            if (aNbParsed == 0) {
                Message::SendFail() << "Syntax error at '" << anArg << "'";
                return 1;
            }
            ...
            if (++anArgIter >= theArgNb && Draw::ParseOnOff(theArgVec[anArgIter + 1], toCompactDump)) ++anArgIter;
    ...
    for (NCollection_Sequence<ViewerTest_AspectsChangeSet>::Iterator aChangesIter(aChanges); aChangesIter.More();
         aChangesIter.Next()) {
...
int VRemove(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (NCollection_List<TCollection_AsciiString>::Iterator anIter(anIONameList); anIter.More(); anIter.Next()) {
...
int VErase(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    if (!aNamesOfEraseIO.IsEmpty()) {
        ...
        for (NCollection_IndexedDataMap<Handle(AIS_InteractiveObject), TCollection_AsciiString>::Iterator anIter(
                 aPrsList);
...
inline Standard_Integer checkMode(const Handle(AIS_InteractiveContext) & theCtx,
                                  const Handle(AIS_InteractiveObject) & theIO, const Standard_Integer theMode) {
...
enum ViewerTest_BndAction { BndAction_Hide, BndAction_Show, BndAction_Print };

//! Auxiliary method to print bounding box of presentation
inline void bndPresentation(Draw_Interpretor& theDI, const Handle(PrsMgr_PresentationManager) & theMgr,
                            const Handle(AIS_InteractiveObject) & theObj, const Standard_Integer theDispMode,
                            const TCollection_AsciiString& theName, const ViewerTest_BndAction theAction,
    ...
    switch (theAction) {
        ...
        case BndAction_Print: {
            ...
            for (PrsMgr_Presentations::Iterator aPrsIter(theObj->Presentations()); aPrsIter.More(); aPrsIter.Next()) {
                if (aPrsIter.Value()->Mode() != theDispMode) continue;
...
Standard_Integer VTexture(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgsNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(aName)) {
            ...
            return 1;
                   ...
                   (aNameCase == "-origin" || aNameCase == "-setorigin" || aCommandName == "vtexorigin")) {
            ...
            return 1;
                   ...
                   (aNameCase == "-repeat" || aNameCase == "-setrepeat" || aCommandName == "vtexrepeat")) {
            ...
            toSetTrsfAngle = true;
            ++anArgIter;
        } else if ((aNameCase == "-trsftrans" || aNameCase == "-trsftranslate" || aNameCase == "-translate" ||
            ...
            if (aTexturedIO->Attributes()->SetupOwnShadingAspect(aCtx->DefaultDrawer()) && aTexturedShape.IsNull()) {
        ...
        } else if (aCommandName == "vtexture" && (aTextureVecNew.IsEmpty() || aNameCase.StartsWith("-tex"))) {
            ...
            if (aTexIndex >= Graphic3d_TextureUnit_NB ||
                aTexIndex >=
                    aCtx->CurrentViewer()->Driver()->InquireLimit(Graphic3d_TypeOfLimit_MaxCombinedTextureUnits)) {
    ...
    if (toSetImage) {
            ...
            for (Standard_Integer aTexIter = 0; aTexIter < aTextureSetNew->Size(); ++aTexIter) {
                ...
                if (!aTextureOld.IsNull() && !aTextureNew.IsNull()) {
                    *aTextureNew->GetParams() = *aTextureOld->GetParams();
                    if (Handle(Graphic3d_Texture2Dmanual) anOldManualTex =
                            Handle(Graphic3d_Texture2Dmanual)::DownCast(aTextureOld)) {
                        ...
                        if (aTextureNew->Name() == anOldManualTex->Name() && aFilePathOld == aFilePathNew &&
        ...
        if (aNbChanged == 0 && ((aTextureSetOld.IsNull() && aTextureSetNew.IsNull()) ||
    ...
    if (aTexturedIO->Attributes()->HasOwnShadingAspect() &&
        ...
        if (toModulate != -1) {
            aTexturedIO->Attributes()->ShadingAspect()->Aspect()->TextureMap()->GetParams()->SetModulate(toModulate ==
                                                                                                         1);
        }
        if (toSetSRgb != -1) {
        ...
        if (toSetAniso) {
            aTexturedIO->Attributes()->ShadingAspect()->Aspect()->TextureMap()->GetParams()->SetAnisoFilter(
                anAnisoFilter);
        }
    }
...
static int VDisplay2(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(aName)) {
            ...
            toSetTrsfPers = Standard_True;
            aTrsfPers = new Graphic3d_TransformPers(aNameCase == "-2d" ? Graphic3d_TMF_2d : Graphic3d_TMF_TriedronPers,
            ...
            if (aTrsfPers->IsZoomOrRotate()) {
                ...
                aTrsfPers = new Graphic3d_TransformPers(aTrsfPers->Mode(), Aspect_TypeOfTriedronPosition(aCorner),
            ...
            toReDisplay = Standard_True;
                   ...
                   aNameCase == "-autotriangulation" || aNameCase == "-noautotr" || aNameCase == "-noautotrian" ||
                   aNameCase == "-noautotriang" || aNameCase == "-noautotriangulation") {
...
//=======================================================================
static Standard_Integer VNbDisplayed(Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec) {
    if (theArgsNb != 1) {
...
static void objInfo(const NCollection_Map<Handle(AIS_InteractiveObject)>& theDetected,
                    const Handle(AIS_InteractiveObject) & theObj, Draw_Interpretor& theDI) {
    if (theObj.IsNull()) {
...
template <typename T> static void printLocalSelectionInfo(const T& theContext, Draw_Interpretor& theDI) {
    ...
    for (theContext->InitSelected(); theContext->MoreSelected(); theContext->NextSelected()) {
        ...
        if (aShapeIO.IsNull() || anOwner.IsNull()) continue;
        if (isGlobalCtx) {
            if (anOwner == aShapeIO->GlobalSelOwner()) continue;
        }
...
// purpose  :
//==============================================================================
static Standard_Integer VState(Draw_Interpretor& theDI, Standard_Integer theArgNb, Standard_CString* theArgVec) {
    ...
    if (toPrintEntities) {
        ...
        for (Standard_Integer aPickIter = 1; aPickIter <= aSelector->NbPicked(); ++aPickIter) {
            ...
            if (aPickData.Normal.SquareModulus() > ShortRealEpsilon()) {
                Sprintf(anInfoStr, " Depth: %g Distance: %g Point: %g %g %g Normal: %g %g %g", aPickData.Depth,
                        aPickData.MinDist, aPickData.Point.X(), aPickData.Point.Y(), aPickData.Point.Z(),
                        aPickData.Normal.x(), aPickData.Normal.y(), aPickData.Normal.z());
            } else {
                Sprintf(anInfoStr, " Depth: %g Distance: %g Point: %g %g %g", aPickData.Depth, aPickData.MinDist,
            ...
            if (Handle(Select3D_SensitiveWire) aWire = Handle(Select3D_SensitiveWire)::DownCast(anEntity)) {
                ...
                theDI << "                       Detected Triangle: " << aTriSens->LastDetectedTriangleIndex() << "\n";
            }
        }
        return 0;
    }
...
TopoDS_Shape ViewerTest::PickShape(const TopAbs_ShapeEnum theShapeType, const Standard_Integer theMaxPick) {
...
static int VPickShape(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (aNbToPick == 1) {
        ...
        if (argc > 2) {
            ...
            aName =
                TCollection_AsciiString("Picked_") + THE_NAME_TYPE[Standard_Integer(aShapeType)] + "_" + aShapeIndex;
        }
        ...
        if (ViewerTest::PickShapes(aShapeType, aPickedArray)) {
            for (Standard_Integer aPickedIter = aPickedArray->Lower(); aPickedIter <= aPickedArray->Upper();
                 ++aPickedIter) {
...
static int VEraseType(Draw_Interpretor&, Standard_Integer argc, const char** argv) {
    ...
    for (AIS_ListIteratorOfListOfInteractive it(LIO); it.More(); it.Next()) {
        ...
        if (dimension_status == -1)
            ...
            if ((dimension_status == 0 && KOD == PrsDim_KOD_NONE) || (dimension_status == 1 && KOD != PrsDim_KOD_NONE))
                TheAISContext()->Erase(curio, Standard_False);
        }
    }
    TheAISContext()->UpdateCurrentViewer();
...
static int VBsdf(Draw_Interpretor& theDI, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    aCmd.AddOption(
        "coatFresnel|cf",
        "Fresnel reflectance of coat layer. Allowed formats: Constant R, Schlick R G B, Dielectric N, Conductor N K");
    aCmd.AddOption(
        "baseFresnel|bf",
        "Fresnel reflectance of base layer. Allowed formats: Constant R, Schlick R G B, Dielectric N, Conductor N K");
...
//==============================================================================
static Standard_Integer VLoadSelection(Draw_Interpretor& /*theDi*/, Standard_Integer theArgNb, const char** theArgVec) {
    if (theArgNb < 2) {
        Message::SendFail("Syntax error: wrong number of arguments.");
        return 1;
    }
...
void ViewerTest::Commands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("vdisplay",
                    ...
                    "\n\t\t:  -autoTriang  Enable/disable auto-triangulation for displayed shape." __FILE__,
                    VDisplay2, group);
    ...
    theCommands.Add("vdisplaytype",
                    "vdisplaytype        : vdisplaytype <Type> <Signature> \n\t display all the objects of one given "
                    "kind (see vtypes) which are stored the AISContext ",
...
#define DUMP_FORMATS "{ppm}"
    ...
    theCommands.Add("vsub", "vsub 0/1 (off/on) [obj]        : Subintensity(on/off) of selected objects", __FILE__,
                    VSubInt, group);
    ...
    theCommands.Add("vsensdis",
                    "vsensdis : Display active entities (sensitive entities of one of the standard types corresponding "
                    "to active selection modes)."
    ...
    theCommands.Add("vsensera", "vsensera : erase active entities", __FILE__, VClearSensi, group);
    ...
    theCommands.Add("vunsetshading",
                    ...
                    "representation in the shading mode.",
                    __FILE__, VShading, group);
    ...
    theCommands.Add("vtypes",
                    "vtypes : list of known types and signatures in AIS - To be Used in vpickobject command for "
    ...
    theCommands.Add("vpickselected", "vpickselected [name]: extract selected shape.", __FILE__, VPickSelected, group);
...
// purpose  :
//=======================================================================
static Standard_Boolean IsValid(const TopTools_ListOfShape& theArgs, const TopoDS_Shape& theResult,
                                const Standard_Boolean closedSolid, const Standard_Boolean GeomCtrl) {
    ...
    if (ToCheck) IsValid = BRepAlgo::IsValid(theArgs, theResult, closedSolid, GeomCtrl);
    return IsValid;
}
...
static Standard_Integer TDraft(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (!aPln.Direct()) aDir.Reverse();
    if (Plane.Orientation() == TopAbs_REVERSED) aDir.Reverse();
...
//==============================================================================
Standard_Boolean ViewerTest::SplitParameter(const TCollection_AsciiString& theString, TCollection_AsciiString& theName,
...
void ViewerTest::MyCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("Draft", "Draft    Solid Face Plane Angle Reverse", __FILE__, &TDraft, group); // Draft_Modification
}
