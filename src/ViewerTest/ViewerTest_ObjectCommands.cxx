...
namespace {
static bool convertToColor(const Handle(TColStd_HSequenceOfAsciiString) & theColorValues, Quantity_Color& theColor) {
...
static bool convertToDatumPart(const TCollection_AsciiString& theValue, Prs3d_DatumParts& theDatumPart) {
...
static void convertToDatumParts(const TCollection_AsciiString& theValue, NCollection_List<Prs3d_DatumParts>& theParts) {
    TCollection_AsciiString aValue = theValue;
...
static bool convertToDatumAttribute(const TCollection_AsciiString& theValue, Prs3d_DatumAttribute& theAttribute) {
    TCollection_AsciiString aValue = theValue;
    aValue.LowerCase();
...
static bool convertToDatumAxes(const TCollection_AsciiString& theValue, Prs3d_DatumAxes& theDatumAxes) {
    TCollection_AsciiString aValue = theValue;
    aValue.LowerCase();
    if (aValue == "x")
...
static Standard_Boolean setTrihedronParams(Standard_Integer theArgsNb, const char** theArgVec,
    ...
    if (aMapOfArgs.Find("origin", aValues)) {
        aComponent->SetLocation(
            gp_Pnt(aValues->Value(1).RealValue(), aValues->Value(2).RealValue(), aValues->Value(3).RealValue()));
    }
    Handle(TColStd_HSequenceOfAsciiString) aXValues, aZValues;
    if (aMapOfArgs.Find("xaxis", aXValues) && aMapOfArgs.Find("zaxis", aZValues)) {
        gp_Dir aXDir(aXValues->Value(1).RealValue(), aXValues->Value(2).RealValue(), aXValues->Value(3).RealValue());

        gp_Dir aZDir(aZValues->Value(1).RealValue(), aZValues->Value(2).RealValue(), aZValues->Value(3).RealValue());

        if (!aZDir.IsNormal(aXDir, M_PI / 180.0)) {
            Message::SendFail(
                "Syntax error - parameters 'xaxis' and 'zaxis' are not applied as VectorX is not normal to VectorZ");
            return Standard_False;
        }
    ...
    if (aMapOfArgs.Find("dispmode", aValues)) {
        ...
        if (aValue.IsEqual("sh") || aValue.IsEqual("shading")) isWireframe = false;
    ...
    if (aMapOfArgs.Find("attribute", aValues)) {
        ...
        if (!theTrihedron->Attributes()->HasOwnDatumAspect())
            theTrihedron->Attributes()->SetDatumAspect(new Prs3d_DatumAspect());
        for (NCollection_List<Prs3d_DatumAttribute>::Iterator anIterator(anAttributes); anIterator.More();
             anIterator.Next()) {
    ...
    if (aMapOfArgs.Find("labels", aValues) || aMapOfArgs.Find("label", aValues)) {
        Prs3d_DatumParts aDatumPart = Prs3d_DatumParts_None;
        if (aValues->Size() >= 2 && convertToDatumPart(aValues->Value(1), aDatumPart) &&
            aDatumPart >= Prs3d_DatumParts_XAxis && aDatumPart <= Prs3d_DatumParts_ZAxis) // labels are set to axes only
        {
...
static Standard_Boolean parseFontStyle(const TCollection_AsciiString& theArg, Font_FontAspect& theAspect) {
    if (theArg == "regular" || *theArg.ToCString() == 'r') {
        theAspect = Font_FA_Regular;
        return Standard_True;
    } else if (theArg == "bolditalic" || theArg == "bold-italic" || theArg == "italic-bold" || theArg == "italicbold") {
...
static Standard_Integer parseFontStrictLevel(const Standard_Integer theArgNb, const char** theArgVec,
...
//==============================================================================
static int VTrihedron2D(Draw_Interpretor& /*theDI*/, Standard_Integer theArgsNum, const char** theArgVec) {
...
static int VTrihedron(Draw_Interpretor&, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    if (GetMapOfAIS().Find2(aName, anObject)) {
        ...
        Handle(Geom_Axis2Placement) aPlacement = new Geom_Axis2Placement(gp_Pnt(0.0, 0.0, 0.0), gp::DZ(), gp::DX());
...
static int VSize(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (NCollection_Sequence<Handle(AIS_Trihedron)>::Iterator anObjIter(aTrihedrons); anObjIter.More();
         anObjIter.Next()) {
...
static Standard_Integer VPlaneBuilder(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    // Declarations
    Standard_Boolean hasArg;
    TCollection_AsciiString aName;
    ...
    if (hasArg) {
        ...
        if (!aShapeA.IsNull() && aShapeA->Type() == AIS_KindOfInteractive_Datum && aShapeA->Signature() == 1) {
            ...
            if (aShapeB.IsNull() || !(aShapeB->Type() == AIS_KindOfInteractive_Datum && aShapeB->Signature() == 1)) {
            ...
            if (aShapeC.IsNull() || !(aShapeC->Type() == AIS_KindOfInteractive_Datum && aShapeC->Signature() == 1)) {
    ...
    // There are no arguments
        ...
        // test the constructor AIS_Plane::AIS_Plane(Geom_Plane,gp_Pnt,gp_Pnt,gp_Pnt)
            ...
            // Compare to heights
            if (fabs(sqrt(aHeightA.SquareDistance(1)) - sqrt(aHeightB.SquareDistance(1))) > Precision::Confusion()) {
...
void FilledCircle::Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePrs,
...
void DisplayCircle(Handle(Geom_Circle) theGeomCircle, TCollection_AsciiString theName, Standard_Boolean isFilled) {
...
static int VDrawText(Draw_Interpretor& theDI, Standard_Integer theArgsNb, const char** theArgVec) {
    ...
    for (; anArgIt < theArgsNb; ++anArgIt) {
        ...
        if (anAutoUpdater.parseRedrawMode(aParam)) {
            continue;
        } else if (aParam == "-pos" || aParam == "-position") {
        ...
        } else if (aParam == "-disptype" || aParam == "-displaytype") {
            if (++anArgIt >= theArgsNb) {
                Message::SendFail() << "Syntax error: wrong number of values for parameter '" << aParam << "'";
                return 1;
            }
...
static int VDrawSphere(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    ...
    if (toPrintInfo) {
        std::cout << "NumberOfPoints:    " << aNumberPoints << "\n"
                  ...
                  << "Amount of memory required for PolyTriangulation without Normals: " << (aTotalSize - aNormalsSize)
                  << " Mb\n"
    ...
    Handle(Graphic3d_AspectFillArea3d) anAspect = new Graphic3d_AspectFillArea3d(
        Aspect_IS_SOLID, Quantity_NOC_WHITE, Quantity_NOC_YELLOW, Aspect_TOL_SOLID, 1.0, aMat, aMat);
...
static int VComputeHLR(Draw_Interpretor&, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        anArgCase.LowerCase();
        if (anArgIter + 1 < theArgNb && (anArgCase == "-algotype" || anArgCase == "-algo" || anArgCase == "-type")) {
        ...
        } else if (anArgCase == "-showhiddenedges" || anArgCase == "-hiddenedges" || anArgCase == "-hidden") {
            toShowHiddenEdges = true;
            if (anArgIter + 1 < theArgNb && Draw::ParseOnOff(theArgVec[anArgIter + 1], toShowHiddenEdges)) {
                ++anArgIter;
            }
        } else if (anArgCase == "-showtangentedges" || anArgCase == "-tangentedges" || anArgCase == "-tangent") {
            toShowCNEdges = true;
            ...
            hasViewDirArg = true;
            gp_Dir anUp;
            anEye.SetCoord(Draw::Atof(theArgVec[anArgIter + 0]), Draw::Atof(theArgVec[anArgIter + 1]),
                           Draw::Atof(theArgVec[anArgIter + 2]));
            aDir.SetCoord(Draw::Atof(theArgVec[anArgIter + 3]), Draw::Atof(theArgVec[anArgIter + 4]),
                          Draw::Atof(theArgVec[anArgIter + 5]));
            anUp.SetCoord(Draw::Atof(theArgVec[anArgIter + 6]), Draw::Atof(theArgVec[anArgIter + 7]),
...
public:
    ...
    MyPArrayObject(Graphic3d_TypeOfPrimitiveArray thePrimType, const Handle(TColStd_HArray1OfAsciiString) & theDesc,
    ...
    Standard_Boolean Init(Graphic3d_TypeOfPrimitiveArray thePrimType,
                          ...
                          const Handle(Graphic3d_AspectMarker3d) & theMarkerAspect, Standard_Boolean theToPatch);
...
private:
    ...
    bool CheckInputCommand(const TCollection_AsciiString theCommand,
                           const Handle(TColStd_HArray1OfAsciiString) & theArgsArray, Standard_Integer& theArgIndex,
                           Standard_Integer theArgCount, Standard_Integer theMaxArgs);
    ...
    //! Replaces shading aspect from myDrawer->Link() with the own shading aspect of myDrawer for this interactive
    //! object
...
void MyPArrayObject::Compute(const Handle(PrsMgr_PresentationManager) &, const Handle(Prs3d_Presentation) & thePrs,
                             const Standard_Integer theMode) {
...
Standard_Boolean MyPArrayObject::Init(Graphic3d_TypeOfPrimitiveArray thePrimType,
    ...
    if (myPArray.IsNull()) {
        ...
        if (myPArray->Type() != thePrimType || aVertexNum > myPArray->VertexNumberAllocated() ||
            aEdgeNum > myPArray->EdgeNumberAllocated() || aBoundNum > myPArray->BoundNumberAllocated() ||
    ...
    for (Standard_Integer anArgIndex = theDesc->Lower(); anArgIndex <= theDesc->Upper();) {
        ...
        // bounds command
            ...
            if (CheckInputCommand("c", theDesc, anArgIndex, 3, anArgsCount))
                myPArray->AddBound(aVertCount, theDesc->Value(anArgIndex - 3).RealValue(),
                                   theDesc->Value(anArgIndex - 2).RealValue(),
                                   theDesc->Value(anArgIndex - 1).RealValue());
    ...
    if (Handle(Graphic3d_ArrayOfTriangles) aTris = Handle(Graphic3d_ArrayOfTriangles)::DownCast(myPArray)) {
        ...
        if (aSegs->EdgeNumber() > 0) {
            for (Standard_Integer aPntIter = 1; aPntIter <= aSegs->EdgeNumber(); aPntIter += 2) {
                Handle(Select3D_SensitiveSegment) aSeg = new Select3D_SensitiveSegment(
                    anOwner, aSegs->Vertice(aSegs->Edge(aPntIter)), aSegs->Vertice(aSegs->Edge(aPntIter + 1)));
            ...
            for (Standard_Integer aPntIter = 1; aPntIter <= aSegs->VertexNumber(); aPntIter += 2) {
                Handle(Select3D_SensitiveSegment) aSeg =
                    new Select3D_SensitiveSegment(anOwner, aSegs->Vertice(aPntIter), aSegs->Vertice(aPntIter + 1));
                aSeg->SetSensitivityFactor(4);
                theSelection->Add(aSeg);
            }
        }
...
bool MyPArrayObject::CheckInputCommand(const TCollection_AsciiString theCommand,
                                       ...
                                       Standard_Integer& theArgIndex, Standard_Integer theArgCount,
                                       Standard_Integer theMaxArgs) {
    // check if there is more elements than expected
    if (theArgIndex >= theMaxArgs) return false;

    TCollection_AsciiString aStrCommand = theArgsArray->Value(theArgIndex);
    aStrCommand.LowerCase();
    if (aStrCommand.Search(theCommand) != 1 || theArgIndex + (theArgCount - 1) >= theMaxArgs) return false;
    ...
    for (int aElement = 0; aElement < theArgCount; aElement++, theArgIndex++) {
        ...
        if (!aStrCommand.IsRealValue(Standard_True)) return false;
    }
...
static Standard_Integer parseTranslationVec(Standard_Integer theArgNb, const char** theArgVec, gp_Vec& theVec) {
    if (theArgNb < 2) {
        return 0;
    }
...
static Standard_Integer VSetLocation(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            gp_Trsf aTrsf;
            aTrsf.SetRotation(gp_Ax1(gp_Pnt(Draw::Atof(theArgVec[anArgIter + 1]), Draw::Atof(theArgVec[anArgIter + 2]),
                                            Draw::Atof(theArgVec[anArgIter + 3])),
                                     gp_Vec(Draw::Atof(theArgVec[anArgIter + 4]), Draw::Atof(theArgVec[anArgIter + 5]),
            ...
            gp_Trsf aTrsf;
            aTrsf.SetMirror(gp_Ax2(gp_Pnt(Draw::Atof(theArgVec[theArgNb - 6]), Draw::Atof(theArgVec[theArgNb - 5]),
                                          Draw::Atof(theArgVec[theArgNb - 4])),
                                   gp_Vec(Draw::Atof(theArgVec[theArgNb - 3]), Draw::Atof(theArgVec[theArgNb - 2]),
            ...
            if (anArgIter + 4 < theArgNb) {
                ...
                if (aQuatArgIter == 4) {
                    anArgIter += 4;
                    const gp_Quaternion aQuat(aQuatArgs[0].RealValue(), aQuatArgs[1].RealValue(),
                                              aQuatArgs[2].RealValue(), aQuatArgs[3].RealValue());
                    gp_Trsf aTrsf = anObj->LocalTransformation();
    ...
    Sprintf(aText,
            ...
            "Rotation: %g %g %g %g\n"
            "Scale:    %g\n",
            aLoc.X(), aLoc.Y(), aLoc.Z(), aRot.X(), aRot.Y(), aRot.Z(), aRot.W(), aTrsf.ScaleFactor());
    theDI << aText;
    return 0;
}
...
static Standard_Integer VConnect(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    ...
    gp_Trsf aTrsf;
    aTrsf.SetTranslationPart(
        gp_Vec(Draw::Atof(argv[anArgIter + 0]), Draw::Atof(argv[anArgIter + 1]), Draw::Atof(argv[anArgIter + 2])));
...
// purpose  : Creates and displays AIS_ConnectedInteractive object from input object and location
//===============================================================================================
static Standard_Integer VConnectTo(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    ViewerTest_AutoUpdater anUpdateTool(aContext, ViewerTest::CurrentView());
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
...
// purpose  :
//=======================================================================
static Standard_Integer VListConnected(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
...
// purpose  :
//=======================================================================
static Standard_Integer VChild(Draw_Interpretor&, Standard_Integer theNbArgs, const char** theArgVec) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            toAdd = 0;
            continue;
        } else if (anArg == "-inheritparenttrsf" || anArg == "-inheritparentloc" || anArg == "-inheritparentlocation" ||
                   anArg == "-inheritparent" || anArg == "-noinheritparenttrsf" || anArg == "-noinheritparentloc" ||
                   anArg == "-noinheritparentlocation" || anArg == "-noinheritparent" || anArg == "-ignoreparenttrsf" ||
                   anArg == "-ignoreparentloc" || anArg == "-ignoreparentlocation" || anArg == "-ignoreparent") {
            bool aVal = true;
...
// purpose  :
//=======================================================================
static Standard_Integer VParent(Draw_Interpretor&, Standard_Integer theNbArgs, const char** theArgVec) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    for (Standard_Integer anArgIter = 2; anArgIter < theNbArgs; ++anArgIter) {
        TCollection_AsciiString anArg(theArgVec[anArgIter]);
        anArg.LowerCase();
        if (anArg == "-ignorevisu") aParent->SetPropagateVisualState(Standard_False);
    }
    return 0;
}
...
// purpose  : vselmode
//===============================================================================================
static Standard_Integer VSetSelectionMode(Draw_Interpretor& /*di*/, Standard_Integer theNbArgs, const char** theArgv) {
    // Check errors
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        anArgCase.LowerCase();
        if (anArgCase == "-set" || anArgCase == "-replace" || anArgCase == "-single" || anArgCase == "-exclusive") {
    ...
    for (NCollection_Sequence<TCollection_AsciiString>::Iterator anObjIter(anObjNames); anObjIter.More();
         anObjIter.Next()) {
...
// purpose  :
//===============================================================================================
static Standard_Integer VSelectionNext(Draw_Interpretor& /*theDI*/, Standard_Integer /*theArgsNb*/,
...
// purpose  :
//===============================================================================================
static Standard_Integer VSelectionPrevious(Draw_Interpretor& /*theDI*/, Standard_Integer /*theArgsNb*/,
                                           const char** /*theArgVec*/) {
    // Check errors
    Handle(AIS_InteractiveContext) anAISContext = ViewerTest::GetAISContext();
    Handle(V3d_View) aView = ViewerTest::CurrentView();
...
//===========================================================================
static Standard_Integer VTriangle(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    ...
    for (Standard_Integer aPntIter = 0; aPntIter < aPrims->VertexNumberAllocated(); ++aPntIter) {
        ...
        if (Handle(AIS_Point) aPntPrs =
                Handle(AIS_Point)::DownCast(GetMapOfAIS().IsBound2(aName) ? GetMapOfAIS().Find2(aName) : NULL)) {
...
//===========================================================================
static Standard_Integer VTorus(Draw_Interpretor& /*di*/, Standard_Integer theNbArgs, const char** theArgVec) {
    if (ViewerTest::GetAISContext().IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    }
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            ++anArgIter;
                   ...
                   (anArg == "-segmentanglefrom" || anArg == "-seganglefrom" || anArg == "-segmentangle1" ||
                    anArg == "-segangle1") &&
    ...
    if (aMajorRad <= 0 || aMinorRad <= 0 || aNbSlices <= 0 || aNbStacks <= 0 ||
        Abs(aSegAngle2 - aSegAngle1) <= Precision::Angular() || Abs(aPipeAngle) <= Precision::Angular()) {
        Message::SendFail("Syntax error: wrong parameters");
        return 1;
    }

    Handle(Graphic3d_ArrayOfTriangles) aTriangles = Prs3d_ToolTorus::Create(
        aMajorRad, aMinorRad, aSegAngle1, aSegAngle2, aPipeAngle, aNbSlices, aNbStacks, gp_Trsf());
...
//===========================================================================
static Standard_Integer VCylinder(Draw_Interpretor& /*di*/, Standard_Integer theNbArgs, const char** theArgVec) {
    if (ViewerTest::GetAISContext().IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    }
...
//===========================================================================
static Standard_Integer VSphere(Draw_Interpretor& /*di*/, Standard_Integer theNbArgs, const char** theArgVec) {
    if (ViewerTest::GetAISContext().IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    }
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
        ...
        if (anUpdateTool.parseRedrawMode(anArg)) {
            ...
            ++anArgIter;
                   ...
                   Draw::ParseInteger(theArgVec[anArgIter + 1], aNbStacks)) {
            ++anArgIter;
        } else if (anArgIter + 1 < theNbArgs && anArg == "-radius" && Draw::ParseReal(theArgVec[anArgIter + 1], aRad)) {
            ++anArgIter;
        } else if (aName.IsEmpty()) {
            aName = theArgVec[anArgIter];
...
static Standard_Integer VObjZLayer(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc >= 2) aOperation = TCollection_AsciiString(argv[1]);
...
//=======================================================================
static Standard_Integer VPolygonOffset(Draw_Interpretor& /*di*/, Standard_Integer argc, const char** argv) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
...
public:
    ViewerTest_MarkersArrayObject(const gp_XYZ& theStartPoint, const Standard_Integer& thePointsOnSide,
...
void ViewerTest_MarkersArrayObject::Compute(const Handle(PrsMgr_PresentationManager) &,
                                            const Handle(Prs3d_Presentation) & aPresentation, const Standard_Integer) {
        ...
        for (Standard_Real i = 1; i <= myPointsOnSide; i++) {
            for (Standard_Real j = 1; j <= myPointsOnSide; j++) {
                for (Standard_Real k = 1; k <= myPointsOnSide; k++) {
                    ...
                    anArray->SetVertexColor(anArray->VertexNumber(), i / myPointsOnSide, j / myPointsOnSide,
                                            k / myPointsOnSide);
                }
            }
        }
...
//=======================================================================
static Standard_Integer VMarkersTest(Draw_Interpretor&, Standard_Integer theArgNb, const char** theArgVec) {
    Handle(AIS_InteractiveContext) aContext = ViewerTest::GetAISContext();
    if (aContext.IsNull()) {
        Message::SendFail("Error: no active viewer");
        return 1;
    ...
    gp_XYZ aPnt(Atof(theArgVec[anArgIter]), Atof(theArgVec[anArgIter + 1]), Atof(theArgVec[anArgIter + 2]));
    anArgIter += 3;
    ...
    if ((aMarkerType == Aspect_TOM_USERDEFINED || aMarkerType < 0) && !aFileName.IsEmpty()) {
        ...
        anAspect = new Graphic3d_AspectMarker3d(aMarkerType >= 0 ? (Aspect_TypeOfMarker)aMarkerType : Aspect_TOM_POINT,
                                                aColor, aScale);
    }
...
//=======================================================================
static int TextToBRep(Draw_Interpretor& /*theDI*/, Standard_Integer theArgNb, const char** theArgVec) {
    // Check arguments
    if (theArgNb < 3) {
    ...
    for (; anArgIt < theArgNb; ++anArgIt) {
        ...
        if (aParam == "-pos" || aParam == "-position") {
            ...
            anArgIt += parseFontStrictLevel(theArgNb - anArgIt - 1, theArgVec + anArgIt + 1, aStrictLevel);
...
static int VFont(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        anArgCase.LowerCase();
        if (anArgCase == "-strict") {
            anArgIter += parseFontStrictLevel(theArgNb - anArgIter - 1, theArgVec + anArgIter + 1, aStrictLevel);
            ...
            toPrintList = true;
            toPrintNames = true;
        } else if (anArgIter + 1 < theArgNb && (anArgCase == "-find" || anArgCase == "-findinfo" ||
                                                anArgCase == "-findall" || anArgCase == "find")) {
            ...
            if (toFindAll || aFontName.Search("*") != -1) {
                ...
                for (std::vector<Handle(Font_SystemFont)>::iterator aFontIter = aFontsSorted.begin();
                     aFontIter != aFontsSorted.end(); ++aFontIter) {
            ...
            if (!aResult.IsEmpty()) {
                ...
                Message::SendFail() << "Error: font '" << aFontName << "' is not found";
                   ...
                   (anArgCase == "-add" || anArgCase == "add" || anArgCase == "-register" || anArgCase == "register")) {
            ++anArgIter;
...
static int VVertexMode(Draw_Interpretor& theDI, Standard_Integer theArgNum, const char** theArgs) {
    ...
    if (aParam == "-set") {
        ...
        Prs3d_VertexDrawMode aNewMode =
            aModeStr == "isolated" ? Prs3d_VDM_Isolated : (aModeStr == "all" ? Prs3d_VDM_All : Prs3d_VDM_Inherited);
        ...
        if (aRedrawNeeded) ViewerTest::CurrentView()->Redraw();
...
//=======================================================================
static Standard_Integer VPointCloud(Draw_Interpretor& theDI, Standard_Integer theArgNum, const char** theArgs) {
    Handle(AIS_InteractiveContext) anAISContext = ViewerTest::GetAISContext();
    if (anAISContext.IsNull()) {
    ...
    if (aCmd == CloudForShape) {
        ...
        for (TopExp_Explorer aFaceIt(aShape, TopAbs_FACE); aFaceIt.More(); aFaceIt.Next()) {
            ...
            for (Standard_Integer aNodeIter = 1; aNodeIter <= aTriangulation->NbNodes(); ++aNodeIter) {
                ...
                if (toRandColors) {
                    Quantity_Color aColor(360.0 * Standard_Real(anIndexOfPoint) / Standard_Real(aNbPoints), 1.0, 0.5,
                                          Quantity_TOC_HLS);
                ...
                if (hasUV && aTriangulation->HasUVNodes()) {
        ...
        for (Standard_Integer aPntIt = 0; aPntIt < aNbPoints; ++aPntIt) {
            ...
            Standard_Real aDistance = isSurface ? aRadius : (Standard_Real(rand() % aNbPoints) / aNbPoints) * aRadius;
            ...
            if (hasUV) {
                anArrayPoints->SetVertexTexel(anIndexOfPoint, gp_Pnt2d(anAlpha / 2.0 * M_PI, aBeta / 2.0 * M_PI));
            }
        }
    }
...
public:
    ...
    MyShapeWithNormals(const TopoDS_Shape& theShape)
        : AIS_Shape(theShape), NormalLength(10), NbAlongU(1), NbAlongV(1), ToUseMesh(Standard_False),
...
protected:
    ...
    virtual void Compute(const Handle(PrsMgr_PresentationManager) & thePrsMgr,
        ...
        for (NCollection_DataMap<TopoDS_Face, NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>>::Iterator aFaceIt(
                 aNormalMap);
            ...
            for (NCollection_Vector<std::pair<gp_Pnt, gp_Pnt>>::Iterator aPntIt(aFaceIt.Value()); aPntIt.More();
                 aPntIt.Next()) {
                ...
                Prs3d_Arrow::Draw(aPrsGroup, aPair.second, gp_Vec(aPair.first, aPair.second), aArrowAngle,
                                  aArrowLength);
            }
...
void ViewerTest::ObjectCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("vaxis",
                    ...
                    "selection of two vertices or one edge",
    ...
    theCommands.Add("vaxisortho",
                    "vaxisortho name "
                    "\n\t\t: Creates an axis by interactive selection of an edge and a vertex. The axis will be "
                    "orthogonal to the selected edge.",
                    __FILE__, VAxisBuilder, group);
    ...
    theCommands.Add("vplane",
                    "vplane  PlaneName [AxisName/PlaneName/PointName] [PointName/PointName/PointName] "
                    "[Nothing/Nothing/PointName] [TypeOfSensitivity {0|1}]"
    ...
    theCommands.Add("vsetlocation", "alias for vlocation", __FILE__, VSetLocation, group);
    ...
    theCommands.Add("vdisconnect",
                    ...
                    "  Disconnects all objects from assembly or disconnects object by name or number (use "
                    "vlistconnected to enumerate assembly children).",
    ...
    theCommands.Add("vselnext", "vselnext : hilight next detected", __FILE__, VSelectionNext, group);

    theCommands.Add("vselprev", "vselnext : hilight previous detected", __FILE__, VSelectionPrevious, group);
    ...
    theCommands.Add("vpolygonoffset",
                    "vpolygonoffset : [object [mode factor units]] - sets/gets polygon offset parameters for an "
                    "object, without arguments prints the default values",
    ...
    theCommands.Add("vfont",
                    ...
                    "\n\t\t:        [-strict {any|aliases|strict}] [-find fontName "
                    "[regular,bold,italic,boldItalic=undefined]] [-verbose {on|off}]"
    ...
    theCommands.Add("vvertexmode",
                    ...
                    "vvertexmode -set {isolated | all | inherited} - sets the default vertex draw mode and updates the "
                    "mode for all displayed objects\n"
                    "vvertexmode -set {isolated | all | inherited} name1 name2 ... - sets the vertex draw mode for the "
                    "specified object(s)\n",
    ...
    theCommands.Add(
        "vpriority",
        "vpriority [-noupdate|-update] name [value]\n\t\t  prints or sets the display priority for an object", __FILE__,
        VPriority, group);
