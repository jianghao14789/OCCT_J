...
//           in the plane of the view
//=======================================================================
static Standard_Boolean Get3DPointAtMousePosition(const gp_Pnt& theFirstPoint, const gp_Pnt& theSecondPoint,
...
//=======================================================================
static int ParseDimensionParams(Standard_Integer theArgNum, const char** theArgVec, Standard_Integer theStartIndex,
                                const Handle(Prs3d_DimensionAspect) & theAspect, Standard_Boolean& theIsCustomPlane,
                                gp_Pln& thePlane,
    ...
    for (Standard_Integer anIt = theStartIndex; anIt < theArgNum; ++anIt) {
        ...
        if (aParam.IsEqual("-shape") || aParam.IsEqual("-shapes")) {
            ...
            Standard_Integer aNbParsed = Draw::ParseColor(theArgNum - anIt - 1, theArgVec + anIt + 1, aColor);
            anIt += aNbParsed;
            if (aNbParsed == 0) {
...
//=======================================================================
static void
SetDimensionParams(const Handle(PrsDim_Dimension) & theDim,
...
//=======================================================================
static int
ParseAngleDimensionParams(Standard_Integer theArgNum, const char** theArgVec, Standard_Integer theStartIndex,
                          NCollection_DataMap<TCollection_AsciiString, TCollection_AsciiString>& theStringParams) {
    theStringParams.Clear();
...
//=======================================================================
static void
SetAngleDimensionParams(const Handle(PrsDim_Dimension) & theDim,
                        const NCollection_DataMap<TCollection_AsciiString, TCollection_AsciiString>& theStringParams) {
...
static int VDimBuilder(Draw_Interpretor& /*theDi*/, Standard_Integer theArgsNb, const char** theArgs) {
    ...
    if (ParseDimensionParams(theArgsNb, theArgs, 3, anAspect, isPlaneCustom, aWorkingPlane, aRealParams, aStringParams,
                             &aShapes)) {
        return 1;
    }
    ...
    switch (aKindOfDimension) {
            ...
            switch (aShapes.Extent()) {
                ...
                case 2: {
                    ...
                    if (!aShape1.IsNull() && !aShape2.IsNull() && aShape1->Shape().ShapeType() == TopAbs_EDGE &&
                        ...
                        aDim =
                            new PrsDim_AngleDimension(TopoDS::Edge(aShape1->Shape()), TopoDS::Edge(aShape2->Shape()));
                    } else {
                ...
                case 3: {
                    ...
                    for (NCollection_List<Handle(AIS_InteractiveObject)>::Iterator aPntIter(aShapes); aPntIter.More();
                         aPntIter.Next()) {
        ...
        case PrsDim_KOD_RADIUS: // radius of the circle
        {
            ...
            for (NCollection_List<Handle(AIS_InteractiveObject)>::Iterator aShapeIter(aShapes); aShapeIter.More();
                 aShapeIter.Next()) {
...
//=======================================================================
static int VRelationBuilder(Draw_Interpretor& /*theDi*/, Standard_Integer theArgsNb, const char** theArgs) {
    if (theArgsNb < 2) {
    ...
    switch (aKindOfRelation) {
        ...
        case PrsDim_KOR_EQUALDISTANCE: {
            ...
            aRelation = new PrsDim_EqualDistanceRelation(aSelectedShapes[0], aSelectedShapes[1], aSelectedShapes[2],
                                                         aSelectedShapes[3], aMkPlane.Value());
...
static int VDimParam(Draw_Interpretor& theDi, Standard_Integer theArgNum, const char** theArgVec) {
    ...
    if (ParseDimensionParams(theArgNum, theArgVec, 2, anAspect, isCustomPlane, aWorkingPlane, aRealParams,
                             aStringParams)) {
        return 1;
    }
...
static int VMoveDim(Draw_Interpretor& theDi, Standard_Integer theArgNum, const char** theArgVec) {
    ...
    } else // Pick dimension or relation
    {
        ...
        while (!isPicked && aPickNum < aMaxPickNum) {
            ...
            isPicked = (!aPickedObj.IsNull() && (aPickedObj->Type() == AIS_KindOfInteractive_Dimension ||
                                                 aPickedObj->Type() == AIS_KindOfInteractive_Relation));
            if (isPicked) {
                break;
            }
...
void ViewerTest::RelationCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add(
        "vrelation",
        "vrelation name "
        "{-concentric|-equaldistance|-equalradius|-fix|-identic|-offset|-parallel|-perpendicular|-tangent|-symmetric}"
