...
ViewerTest_AutoUpdater::ViewerTest_AutoUpdater(const Handle(AIS_InteractiveContext) & theContext,
    ...
    : myContext(theContext), myView(theView), myToUpdate(RedrawMode_Auto), myWasAutoUpdate(Standard_False) {
    if (!theView.IsNull()) {
...
void ViewerTest_AutoUpdater::Update() {
    ...
    switch (myToUpdate) {
        ...
        Standard_FALLTHROUGH case ViewerTest_AutoUpdater::RedrawMode_Forced: {
            if (!myContext.IsNull()) {
