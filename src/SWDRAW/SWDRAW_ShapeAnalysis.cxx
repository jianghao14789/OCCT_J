...
static Standard_Integer projcurve(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    di << "Precision (BRepBuilderAPI) : " << BRepBuilderAPI::Precision() << "  Projection : " << X << "  " << Y << "  "
       << Z << "\n";
...
static Standard_Integer XSHAPE_comptoledge(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 2) {
        di << "Use:\n"
...
//=======================================================================
static void PrintProps(Standard_Integer i, const Handle(ShapeAnalysis_FreeBoundData) & fbd, Draw_Interpretor& di) {
    char str[100];
...
static Standard_Integer checkselfintersection(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 2) {
...
static Standard_Integer checkedge(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (analyser.HasCurve3d(TopoDS::Edge(edge))) {
        ...
        if (analyser.CheckVerticesWithCurve3d(TopoDS::Edge(edge))) {
            isOk = Standard_False;
            di << "Vertices of the edge don't coincide with start/end points of 3d-curve (using tolerance of the "
                  "vertices).\n";
        }
    } else {
        isOk = Standard_False;
    ...
    if (!face.IsNull()) {
        ...
        if (analyser.HasPCurve(TopoDS::Edge(edge), TopoDS::Face(face))) {
            ...
            if (analyser.CheckVerticesWithPCurve(TopoDS::Edge(edge), TopoDS::Face(face))) {
                isOk = Standard_False;
                di << "Vertices of the edge don't coincide with start/end points of 2d-curve (using tolerance of the "
                      "vertices).\n";
            }
        } else {
            isOk = Standard_False;
...
void SWDRAW_ShapeAnalysis::InitCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("projcurve", "nom_edge | curve3d | curve3d first last + X Y Z", __FILE__, projcurve, g);
    ...
    theCommands.Add("statshape", "shape [particul] : stats/particularites", __FILE__, XSHAPE_statshape, g);
    ...
    theCommands.Add("fbprops",
                    "shp [toler [splitclosed [splitopen]]] - free bounds properties; toler <= 0 or not specified - for "
                    "shells (no sewing call)",
                    __FILE__, FreeBoundsProps, groupold);
    theCommands.Add("fbclose",
                    "shp sewtoler closetoler [splitclosed [splitopen]] - closes free bounds; use sewtoler <= 0 for "
                    "shells (no sewing call)",
