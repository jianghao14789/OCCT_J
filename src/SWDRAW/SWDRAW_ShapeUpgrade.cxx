...
static Standard_Integer DT_ShapeConvertRev(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (c2d) tool.Set2dConversion(Standard_True);
    if (c3d) {
        tool.Set3dConversion(Standard_True);
        if (n > 5) tool.Set3dLineConversion(Standard_False);
        if (n > 6) tool.Set3dCircleConversion(Standard_False);
        if (n > 7) tool.Set3dConicConversion(Standard_False);
    }
    tool.Perform();
    TopoDS_Shape res = tool.Result();
...
static Standard_Integer DT_ShapeConvert(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    tool.SetSurfaceConversion(Standard_True);
    if (c2d) tool.Set2dConversion(Standard_True);
    if (c3d) tool.Set3dConversion(Standard_True);
    tool.Perform();
    TopoDS_Shape res = tool.Result();
...
//
//=======================================================================
static Standard_Integer offset2dcurve(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 4) {
...
static Standard_Integer splitface(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    Handle(TColGeom_HArray2OfSurface) AS = new TColGeom_HArray2OfSurface(1, uval.Length() + 1, 1, vval.Length() + 1);
    for (i = 0; i <= uval.Length(); i++) {
        ...
        for (Standard_Integer j = 0; j <= vval.Length(); j++) {
            ...
            Handle(Geom_RectangularTrimmedSurface) rect = new Geom_RectangularTrimmedSurface(S, umin, umax, vmin, vmax);
...
static Standard_Integer converttobspline(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 3) {
    ...
    TopoDS_Shape revsh = ShapeCustom::ConvertToRevolution(inputShape);
    TopoDS_Shape res = ShapeCustom::ConvertToBSpline(revsh, strchr(options, 'e') != 0, strchr(options, 'r') != 0,
                                                     strchr(options, 'o') != 0, strchr(options, 'p') != 0);
    ShapeFix::SameParameter(res, Standard_False);
...
static Standard_Integer removeinternalwires(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 4) {
        di << "bad number of arguments\n";
        return 1;
    }
    ...
    for (; k < argc; k++) {
        if (isShape) {
            ...
            if (isShape) {
                if (aShape.ShapeType() == TopAbs_FACE || aShape.ShapeType() == TopAbs_WIRE) aSeqShapes.Append(aShape);
            }
        }
        if (!isShape) aModeRemoveFaces = (Draw::Atoi(argv[k]) == 1);
    }
...
static Standard_Integer removeloc(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc > 3) aRemLoc.SetRemoveLevel((TopAbs_ShapeEnum)Draw::Atoi(argv[3]));
...
static Standard_Integer unifysamedom(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(Unifier().History());

    DBRep::Set(a[1], Result);
    return 0;
}

static Standard_Integer copytranslate(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 6) {
...
static Standard_Integer reshape(Draw_Interpretor& /*theDI*/, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc < 4) {
...
void SWDRAW_ShapeUpgrade::InitCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("DT_ShapeDivide", "DT_ShapeDivide Result Shape Tol: Divides shape with C1 Criterion", __FILE__,
                    DT_ShapeDivide, g);
    ...
    theCommands.Add("DT_SplitAngle",
                    ...
                    __FILE__, DT_SplitAngle, g);

    theCommands.Add("DT_ShapeConvert", "DT_ShapeConvert Result Shape convert2d convert3d: Converts curves to beziers",
                    __FILE__, DT_ShapeConvert, g);

    theCommands.Add("DT_ShapeConvertRev",
                    "DT_ShapeConvert Result Shape convert2d convert3d: Converts curves to beziers", __FILE__,
    ...
    theCommands.Add("DT_SplitCurve2d", "DT_SplitCurve2d Curve Tol: Splits the curve with C1 criterion", __FILE__,
                    DT_SplitCurve2d, g);

    theCommands.Add("DT_SplitCurve", "DT_SplitCurve Curve Tol: Splits the curve with C1 criterion", __FILE__,
                    DT_SplitCurve, g);

    theCommands.Add("DT_SplitSurface",
                    "DT_SplitSurface Result Surface/GridSurf Tol: Splits the surface with C1 criterion", __FILE__,
                    DT_SplitSurface, g);
    ...
    theCommands.Add("splitface", "result face [u usplit1 usplit2...] [v vsplit1 vsplit2 ...]", __FILE__, splitface, g);
    ...
    theCommands.Add("RemoveIntWires", "result minarea wholeshape [faces or wires] [moderemoveface ]", __FILE__,
                    removeinternalwires, g);

    theCommands.Add("removeloc", "result shape [remove_level(see ShapeEnum)]", __FILE__, removeloc, g);

    theCommands.Add("unifysamedom",
                    "unifysamedom result shape [s1 s2 ...] [-f] [-e] [-nosafe] [+b] [+i] [-t val] [-a val]", __FILE__,
                    unifysamedom, g);
