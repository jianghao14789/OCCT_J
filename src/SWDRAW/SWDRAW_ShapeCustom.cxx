...
static Standard_Integer ckeckKnots(const TColStd_Array1OfReal& theKnots, Standard_Real theFirst,
    ...
    for (; i < nb; i++) {
        if (!aNum) {
            ...
            if (fabs(aDL) <= Precision::PConfusion() || aDL > Precision::PConfusion()) break;
            aNum++;
        }
    }
    return aNum;
...
static void expcurv2d(const Handle(Geom2d_Curve) & aCurve, TColStd_Array2OfInteger& Nb, const Standard_Integer Degree,
                      const Standard_Integer MaxSeg, const Standard_Integer theCont, Standard_Real theFirst,
                      Standard_Real theLast) {
...
static void expcurv(const Handle(Geom_Curve) & aCurve, TColStd_Array2OfInteger& Nb, const Standard_Integer Degree,
    ...
    if (aCurve->IsKind(STANDARD_TYPE(Geom_BezierCurve))) {
        ...
        if (Bs->Degree() > Degree) Nb.ChangeValue(2, 1)++;
        if (Bs->IsRational()) Nb.ChangeValue(2, 3)++;
        if (aCont < theCont) Nb.ChangeValue(2, 4)++;
        return;
    } else
        Nb.ChangeValue(1, 5)++;
    return;
}
static void expsurf(const Handle(Geom_Surface) & aSurface, TColStd_Array2OfInteger& NbSurf,
                    const Standard_Integer Degree, const Standard_Integer MaxSeg, const Standard_Integer theCont) {
    ...
    if (aSurface->IsKind(STANDARD_TYPE(Geom_SweptSurface))) {
        ...
        expcurv(BasCurve, NbSurf, Degree, MaxSeg, theCont, BasCurve->FirstParameter(), BasCurve->LastParameter());
        return;
    }
    ...
    if (aSurface->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) {
        ...
        if (aCont < theCont) NbSurf.ChangeValue(1, 4)++;
        return;
    }
    if (aSurface->IsKind(STANDARD_TYPE(Geom_BezierSurface))) {
        ...
        if (aCont < theCont) NbSurf.ChangeValue(2, 4)++;
        return;
    }
...
static Standard_Integer expshape(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (Ex.Init(Shape, TopAbs_FACE); Ex.More(); Ex.Next(), nbF++) {
        ...
        for (exp.Init(F, TopAbs_EDGE); exp.More(); exp.Next(), nbE++) {
            ...
            if (BRep_Tool::IsClosed(E, F)) nbSeam++;
...
static Standard_Integer BSplRes(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    TopoDS_Shape result = ShapeCustom::BSplineRestriction(
        Shape, Draw::Atof(argv[3]), Draw::Atof(argv[4]), Draw::Atoi(argv[5]), Draw::Atoi(argv[6]), aCont3, aCont2,
        Draw::Atoi(argv[9]) != 0, Draw::Atoi(argv[10]) != 0, aParameters);
    if (result.IsNull()) {
        di << "NO RESULT\n";
        return 1;
    } else if (result == Shape) {
...
void SWDRAW_ShapeCustom::InitCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("bsplres",
                    "BSplineRestriction result shape tol3d tol2d reqdegree reqnbsegments continuity3d continuity2d "
