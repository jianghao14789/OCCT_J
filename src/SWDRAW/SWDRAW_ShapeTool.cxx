...
static Standard_Integer XSHAPE_explorewire(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    char nomsh[30];
    if (argc < 2) {
...
static Standard_Integer XSHAPE_ssolid(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 3) {
...
void SWDRAW_ShapeTool::InitCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("samerange", "{ shape | result curve2d first last newfirst newlast }", __FILE__, samerange, g);
}
