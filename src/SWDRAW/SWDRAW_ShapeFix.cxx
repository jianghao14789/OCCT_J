...
static Standard_Integer stwire(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (oq) {
        ...
        for (i = 1; i <= nb; i++) {
            ...
            if (Shape.ShapeType() == TopAbs_FACE && sae.IsSeam(E, TopoDS::Face(Shape))) di << " SEAM_FACE";
        ...
        for (i = 1; i <= nb; i++) {
            ...
            switch (stat) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    di << "End of " << i << " at U=" << upre << ", Start of " << (i == nb ? 1 : i + 1) << " OK";
                    break;
                case 5:
                    di << "Intersection, End of " << i << " at U=" << upre << ", Start of " << (i == nb ? 1 : i + 1)
                       << " at U=" << ufol;
                    break;
                default:
                    di << "Disjoined\n";
...
static Standard_Integer fixshape(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (mess) {
        ...
        aSStream << " ------------------------------------------------------------\n";
        for (TColStd_DataMapIteratorOfDataMapOfAsciiStringInteger anIter(aMapOfNumberOfFixes); anIter.More();
             anIter.Next()) {
...
static Standard_Integer fixsmalledges(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n > k) tol = Draw::Atof(a[k++]);

    if (n > k) mode = Draw::Atoi(a[k++]);

    if (n > k) tolang = Draw::Atof(a[k++]);
    ...
    if (mode == 2) aModeDrop = Standard_False;
...
static Standard_Integer checkoverlapedges(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    Standard_Integer k = 3;
    if (k < n) aTol = Draw::Atof(a[k++]);
    if (k < n) aDistDomain = Draw::Atof(a[k++]);
...
static Standard_Integer connectedges(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    Standard_Real aTol = Precision::Confusion();
    if (n > 3) aTol = Draw::Atof(a[3]);

    Standard_Boolean shared = Standard_True;
    if (n > 4) shared = (Draw::Atoi(a[4]) == 1);
...
void SWDRAW_ShapeFix::InitCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("edgesameparam", "nom shape draw ou * [+ option force]", __FILE__, edgesameparam, g);
    theCommands.Add("settolerance", "shape [mode=v-e-f-a] val(fix value) or tolmin tolmax", __FILE__, settolerance, g);
    ...
    theCommands.Add("fixsmalledges", "result shape [toler mode amxangle]", __FILE__, fixsmalledges, g);
    theCommands.Add("fixsmallfaces", "result shape [toler=1.]", __FILE__, fixsmallfaces, g);
    theCommands.Add("checkoverlapedges", "edge1 edge2 [toler domaindist]", __FILE__, checkoverlapedges, g);
