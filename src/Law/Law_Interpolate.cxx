...
// purpose  :
//=======================================================================
static void BuildParameters(const Standard_Boolean PeriodicFlag, const TColStd_Array1OfReal& PointsArray,
...
static void BuildPeriodicTangent(const TColStd_Array1OfReal& PointsArray, TColStd_Array1OfReal& TangentsArray,
    ...
    if (PointsArray.Length() < 2) {
        ...
        Standard_Real period =
            (ParametersArray.Value(ParametersArray.Upper()) - ParametersArray.Value(ParametersArray.Lower()));
        ...
        TangentFlags.SetValue(1, Standard_True);
        PLib::EvalLagrange(parameter_array[1], 1, degree, 1, point_array[0], parameter_array[0], eval_result[0]);
...
static void BuildTangents(const TColStd_Array1OfReal& PointsArray, TColStd_Array1OfReal& TangentsArray,
                          TColStd_Array1OfBoolean& TangentFlags, const TColStd_Array1OfReal& ParametersArray) {
    ...
    if (!TangentFlags.Value(1)) {
        ...
        TangentFlags.SetValue(1, Standard_True);
        PLib::EvalLagrange(ParametersArray.Value(1), 1, degree, 1, point_array[0], parameter_array[0], eval_result[0]);
        TangentsArray.SetValue(1, eval_result[1]);
    }
    if (!TangentFlags.Value(TangentFlags.Upper())) {
        ...
        PLib::EvalLagrange(ParametersArray.Value(ParametersArray.Upper()), 1, degree, 1, point_array[0],
                           parameter_array[0], eval_result[0]);
...
Law_Interpolate::Law_Interpolate(const Handle(TColStd_HArray1OfReal) & PointsPtr, const Standard_Boolean PeriodicFlag,
...
void Law_Interpolate::PerformPeriodic() {
    ...
    BSplCLib::Interpolate(degree, flatknots, parameters, contact_order_array, 1, poles(1), inversion_problem);
...
void Law_Interpolate::PerformNonPeriodic() {
    Standard_Integer degree, ii,
        // jj,
        index, index1, index2, index3, mult_index, inversion_problem, num_points, num_distinct_knots, num_poles;
    ...
    switch (degree) {
        ...
        case 2:
            ...
            BSplCLib::Interpolate(degree, flatknots, myParameters->Array1(), contact_order_array, 1, poles(1),
                                  inversion_problem);
            if (!inversion_problem) {
