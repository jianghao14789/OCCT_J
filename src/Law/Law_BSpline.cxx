...
static void SetPoles(const TColStd_Array1OfReal& Poles, const TColStd_Array1OfReal& Weights, TColStd_Array1OfReal& FP) {
...
static void GetPoles(const TColStd_Array1OfReal& FP, TColStd_Array1OfReal& Poles, TColStd_Array1OfReal& Weights)
...
static void CheckCurveData(const TColStd_Array1OfReal& CPoles, const TColStd_Array1OfReal& CKnots,
                           const TColStd_Array1OfInteger& CMults, const Standard_Integer Degree,
    ...
    if (CPoles.Length() != BSplCLib::NbPoles(Degree, Periodic, CMults)) throw Standard_ConstructionError();
}
...
static void KnotAnalysis(const Standard_Integer Degree, const Standard_Boolean Periodic,
...
Handle(Law_BSpline) Law_BSpline::Copy() const {
    ...
    if (IsRational())
        C = new Law_BSpline(poles->Array1(), weights->Array1(), knots->Array1(), mults->Array1(), deg, periodic);
    else
...
Law_BSpline::Law_BSpline(const TColStd_Array1OfReal& Poles, const TColStd_Array1OfReal& Knots,
                         const TColStd_Array1OfInteger& Mults, const Standard_Integer Degree,
                         const Standard_Boolean Periodic)
...
Law_BSpline::Law_BSpline(const TColStd_Array1OfReal& Poles, const TColStd_Array1OfReal& Weights,
...

{
    ...
    if (Weights.Length() != Poles.Length()) throw Standard_ConstructionError("Law_BSpline");

    Standard_Integer i;
    for (i = Weights.Lower(); i <= Weights.Upper(); i++) {
        if (Weights(i) <= gp::Resolution()) throw Standard_ConstructionError("Law_BSpline");
    }
...
void Law_BSpline::IncreaseDegree(const Standard_Integer Degree) {
    ...
    Handle(TColStd_HArray1OfReal) npoles = new TColStd_HArray1OfReal(1, poles->Length() + Step * (ToK2 - FromK1));

    Standard_Integer nbknots = BSplCLib::IncreaseDegreeCountKnots(deg, Degree, periodic, mults->Array1());
    ...
    if (IsRational()) {
        ...
        BSplCLib::IncreaseDegree(deg, Degree, periodic, 2, adimpol, knots->Array1(), mults->Array1(), adimnpol,
                                 nknots->ChangeArray1(), nmults->ChangeArray1());
        GetPoles(adimnpol, npoles->ChangeArray1(), nweights->ChangeArray1());
    } else {
        BSplCLib::IncreaseDegree(deg, Degree, periodic, 1, poles->Array1(), knots->Array1(), mults->Array1(),
                                 npoles->ChangeArray1(), nknots->ChangeArray1(), nmults->ChangeArray1());
    }
...
void Law_BSpline::IncreaseMultiplicity(const Standard_Integer I1, const Standard_Integer I2, const Standard_Integer M) {
...
void Law_BSpline::IncrementMultiplicity(const Standard_Integer I1, const Standard_Integer I2,
...
void Law_BSpline::InsertKnot(const Standard_Real U, const Standard_Integer M, const Standard_Real ParametricTolerance,
...
void Law_BSpline::InsertKnots(const TColStd_Array1OfReal& Knots, const TColStd_Array1OfInteger& Mults,
                              const Standard_Real Epsilon, const Standard_Boolean Add) {
    // Check and compute new sizes
    Standard_Integer nbpoles, nbknots;

    if (!BSplCLib::PrepareInsertKnots(deg, periodic, knots->Array1(), mults->Array1(), Knots, &Mults, nbpoles, nbknots,
                                      Epsilon, Add))
    ...
    if (rational) {
        ...
        SetPoles(poles->Array1(), weights->Array1(), adimpol);
        TColStd_Array1OfReal adimnpol(1, 2 * npoles->Upper());
        BSplCLib::InsertKnots(deg, periodic, 2, adimpol, knots->Array1(), mults->Array1(), Knots, &Mults, adimnpol,
                              nknots->ChangeArray1(), nmults->ChangeArray1(), Epsilon, Add);
        GetPoles(adimnpol, npoles->ChangeArray1(), nweights->ChangeArray1());
        weights = nweights;
    } else {
        BSplCLib::InsertKnots(deg, periodic, 1, poles->Array1(), knots->Array1(), mults->Array1(), Knots, &Mults,
                              npoles->ChangeArray1(), nknots->ChangeArray1(), nmults->ChangeArray1(), Epsilon, Add);
    }
...
Standard_Boolean Law_BSpline::RemoveKnot(const Standard_Integer Index, const Standard_Integer M,
    ...
    if (IsRational()) {
        ...
        SetPoles(poles->Array1(), weights->Array1(), adimpol);
        TColStd_Array1OfReal adimnpol(1, 2 * npoles->Upper());
        if (!BSplCLib::RemoveKnot(Index, M, deg, periodic, 2, adimpol, knots->Array1(), mults->Array1(), adimnpol,
                                  nknots->ChangeArray1(), nmults->ChangeArray1(), Tolerance))
            return Standard_False;
        GetPoles(adimnpol, npoles->ChangeArray1(), nweights->ChangeArray1());
        weights = nweights;
    } else {
        if (!BSplCLib::RemoveKnot(Index, M, deg, periodic, 1, poles->Array1(), knots->Array1(), mults->Array1(),
                                  npoles->ChangeArray1(), nknots->ChangeArray1(), nmults->ChangeArray1(), Tolerance))
            return Standard_False;
    }
...
void Law_BSpline::Reverse() {
    ...
    if (rational) BSplCLib::Reverse(weights->ChangeArray1(), last);
    UpdateKnots();
}
...
void Law_BSpline::Segment(const Standard_Real U1, const Standard_Real U2) {
    ...
    index = 0;
    BSplCLib::LocateParameter(deg, knots->Array1(), mults->Array1(), U1, periodic, knots->Lower(), knots->Upper(),
                              index, NewU1);
    index = 0;
    BSplCLib::LocateParameter(deg, knots->Array1(), mults->Array1(), U2, periodic, knots->Lower(), knots->Upper(),
                              index, NewU2);
    ...
    if (periodic) { // set the origine at NewU1
        ...
        BSplCLib::LocateParameter(deg, knots->Array1(), mults->Array1(), U1, periodic, knots->Lower(), knots->Upper(),
                                  index0, U);
        if (Abs(knots->Value(index0 + 1) - U) < Eps) index0++;
        SetOrigin(index0);
        SetNotPeriodic();
    }
    ...
    BSplCLib::LocateParameter(deg, knots->Array1(), mults->Array1(), NewU1, periodic, FromU1, ToU2, index1, U);
    BSplCLib::LocateParameter(deg, knots->Array1(), mults->Array1(), NewU1 + delta, periodic, FromU1, ToU2, index2, U);
    if (Abs(knots->Value(index2 + 1) - U) < Eps) index2++;
...
void Law_BSpline::SetKnot(const Standard_Integer Index, const Standard_Real K, const Standard_Integer M) {
...
void Law_BSpline::SetNotPeriodic() {
    if (periodic) {
        ...
        if (IsRational()) {
            ...
            SetPoles(poles->Array1(), weights->Array1(), adimpol);
            TColStd_Array1OfReal adimnpol(1, 2 * npoles->Upper());
            BSplCLib::Unperiodize(deg, 1, mults->Array1(), knots->Array1(), adimpol, nmults->ChangeArray1(),
                                  nknots->ChangeArray1(), adimnpol);
            GetPoles(adimnpol, npoles->ChangeArray1(), nweights->ChangeArray1());
        } else {

            BSplCLib::Unperiodize(deg, 1, mults->Array1(), knots->Array1(), poles->Array1(), nmults->ChangeArray1(),
...
void Law_BSpline::SetPole(const Standard_Integer Index, const Standard_Real P, const Standard_Real W) {
    SetPole(Index, P);
    SetWeight(Index, W);
}
...
void Law_BSpline::SetWeight(const Standard_Integer Index, const Standard_Real W) {
    ...
    if (rat) {
        if (rat && !IsRational()) weights = new TColStd_HArray1OfReal(1, poles->Length(), 1.);
...
void Law_BSpline::UpdateKnots() {
    ...
    if (knotSet == GeomAbs_Uniform && !periodic) {
        ...
        flatknots = new TColStd_HArray1OfReal(1, BSplCLib::KnotSequenceLength(mults->Array1(), deg, periodic));

        BSplCLib::KnotSequence(knots->Array1(), mults->Array1(), deg, periodic, flatknots->ChangeArray1());
    }
...
void Law_BSpline::D2(const Standard_Real U, Standard_Real& P, Standard_Real& V1, Standard_Real& V2) const {
    Standard_Real NewU = U;
    PeriodicNormalization(NewU);
    if (rational) {
...
void Law_BSpline::D3(const Standard_Real U, Standard_Real& P, Standard_Real& V1, Standard_Real& V2,
    ...
    if (rational) {
        ...
        BSplCLib::D3(NewU, 0, deg, periodic, POLES, BSplCLib::NoWeights(), FKNOTS, FMULTS, P, V1, V2, V3);
    }
}
...
Standard_Real Law_BSpline::LocalValue(const Standard_Real U, const Standard_Integer FromK1,
...
void Law_BSpline::LocalD0(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
...
void Law_BSpline::LocalD1(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
                          Standard_Real& P, Standard_Real& V1) const {
...
void Law_BSpline::LocalD2(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
                          Standard_Real& P, Standard_Real& V1, Standard_Real& V2) const {
...
void Law_BSpline::LocalD3(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
                          Standard_Real& P, Standard_Real& V1, Standard_Real& V2, Standard_Real& V3) const {
...
Standard_Real Law_BSpline::LocalDN(const Standard_Real U, const Standard_Integer FromK1, const Standard_Integer ToK2,
...
void Law_BSpline::LocateU(const Standard_Real U, const Standard_Real ParametricTolerance, Standard_Integer& I1,
...
void Law_BSpline::MovePointAndTangent(const Standard_Real U, const Standard_Real P, const Standard_Real Tangent,
    ...
    BSplCLib::MovePointAndTangent(
        U, dimension, delta, delta_derivative, Tolerance, deg, StartingCondition, EndingCondition, poles_array[0],
        rational ? &weights->Array1() : BSplCLib::NoWeights(), flatknots->Array1(), new_poles_array[0], ErrorStatus);
    if (!ErrorStatus) {
...
void Law_BSpline::Resolution(const Standard_Real Tolerance3D, Standard_Real& UTolerance) const {
    ...
    if (rational) {
        BSplCLib::Resolution(*bidr, 1, poles->Length(), &weights->Array1(), FKNOTS, deg, Tolerance3D, UTolerance);
    } else {

        BSplCLib::Resolution(*bidr, 1, poles->Length(), BSplCLib::NoWeights(), FKNOTS, deg, Tolerance3D, UTolerance);
    }
}
