...
//=============================================================================
Standard_Real Bisector_BisecAna::Distance(const gp_Pnt2d& apoint, const Handle(GccInt_Bisec) & abisector,
                                          const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                                          const gp_Vec2d& VecRef, const Standard_Real adirection,
                                          Standard_Real& aparameter, Standard_Boolean& asense,
                                          Standard_Boolean& astatus, Standard_Boolean IsBisecOfTwoLines) {
    ...
    if (asense) {
        ...
        if (!IsBisecOfTwoLines) {
            ...
            if (VecRef.SquareMagnitude() != 0) {
                ...
                if (tangdir * DirRef < 0.) asense = Standard_False;
            }
            ///////////////////
...
//===========================================================================
void Bisector_BisecAna::Perform(const Handle(Geom2d_Curve) & afirstcurve, const Handle(Geom2d_Curve) & asecondcurve,
                                const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                                const Standard_Real adirection, const GeomAbs_JoinType ajointype,
                                const Standard_Real tolerance, const Standard_Boolean oncurve) {
    ...
    switch (cas) {
        ...
        case 1: {
            ...
            //-----------------------------------------------------
            if (circle1.Location().IsEqual(circle2.Location(), PreConf) && (Abs(radius1 - radius2) <= PreConf)) {
                ...
                if (!circle1.Location().IsEqual(PMil, PreConf)) {
                    ...
                    line =
                        gp_Lin2d(PMil, gp_Dir2d(circle1.Location().X() - PMil.X(), circle1.Location().Y() - PMil.Y()));
                ...
                if (oncurve)
                    ...
                    distanceptsol = Distance(apoint, solution, afirstvector, asecondvector, LineBisVec, adirection,
                                             parameter, sense, ok);
            ...
            if (Bisector.IsDone()) {
                ...
                for (Standard_Integer i = 1; i <= nbsolution; i++) {
                    ...
                    if (ok) {
                        ...
                        if (oncurve)
                            ...
                            distanceptsol = Distance(apoint, solution, afirstvector, asecondvector, LineBisVec,
                                                     adirection, parameter, sense, ok);
                        //  Modified by skv - Tue Feb 15 17:51:29 2005 Integration End
                ...
                if (!TheSol.IsNull()) {
                    ...
                    if (type == GccInt_Lin) {
                        ...
                        if (!thesense)
                            thebisector = new Geom2d_TrimmedCurve(bisectorcurve, firstparameter - 2.0 * M_PI,
                                                                  firstparameter, thesense);
                        else
                            thebisector = new Geom2d_TrimmedCurve(bisectorcurve, firstparameter,
                                                                  firstparameter + 2.0 * M_PI, thesense);
                    } else if (type == GccInt_Hpr) {
        ...
        case 3: {
            ...
            if (Direc1.IsParallel(Direc2, 1.e-8)) {
                ...
                if (oncurve)
                    ...
                    distanceptsol = Distance(apoint, solution, afirstvector, asecondvector, LineBisVec, adirection,
                                             parameter, sense, isOk, Standard_True);
                //  Modified by skv - Tue Feb 15 17:51:29 2005 Integration End
                if (isOk || !oncurve) {
                    thesense = sense;
                    distancemini = distanceptsol;
...
void Bisector_BisecAna::Perform(const Handle(Geom2d_Curve) & afirstcurve, const Handle(Geom2d_Point) & asecondpoint,
                                const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                                const Standard_Real adirection, const Standard_Real tolerance,
        ...
        case 1: {
            ...
            if (Bisector.IsDone()) {
                ...
                for (Standard_Integer i = 1; i <= nbsolution; i++) {
                    ...
                    Degenerate(solution, tolerance);
                    sense = Standard_False;
                    distanceptsol = Distance(apoint, solution, afirstvector, asecondvector, VecRef, adirection,
                                             parameter, sense, ok);
...
void Bisector_BisecAna::Perform(const Handle(Geom2d_Point) & afirstpoint, const Handle(Geom2d_Curve) & asecondcurve,
                                const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                                const Standard_Real adirection,
                                //				const Standard_Real         tolerance    ,
                                const Standard_Real, const Standard_Boolean oncurve)

{
    Standard_Real adirectionreverse = -adirection;
    Perform(asecondcurve, afirstpoint, apoint, asecondvector, afirstvector, adirectionreverse,
            //  Modified by skv - Tue Feb 15 17:51:29 2005 Integration Begin
            0.,
            //  Modified by skv - Tue Feb 15 17:51:29 2005 Integration End
            oncurve);
...
void Bisector_BisecAna::Perform(const Handle(Geom2d_Point) & afirstpoint, const Handle(Geom2d_Point) & asecondpoint,
                                const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                                const Standard_Real adirection,
                                //				const Standard_Real         tolerance    ,
                                const Standard_Real, const Standard_Boolean oncurve) {
...
// purpose  :
//=============================================================================
void Bisector_BisecAna::D3(const Standard_Real U, gp_Pnt2d& P, gp_Vec2d& V1, gp_Vec2d& V2, gp_Vec2d& V3) const {
