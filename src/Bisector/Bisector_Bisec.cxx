...
static void ReplaceByLineIfIsToSmall(Handle(Bisector_Curve) & Bis, Standard_Real& UFirst, Standard_Real& ULast);
//=============================================================================
...
void Bisector_Bisec::Perform(const Handle(Geom2d_Curve) & afirstcurve, const Handle(Geom2d_Curve) & asecondcurve,
    ...
    if (Type1 == STANDARD_TYPE(Geom2d_BSplineCurve)) {
        ...
        if (afirstcurve->DynamicType() == STANDARD_TYPE(Geom2d_TrimmedCurve)) {
            aBS =
                Handle(Geom2d_BSplineCurve)::DownCast(Handle(Geom2d_TrimmedCurve)::DownCast(afirstcurve)->BasisCurve());
        } else {
    ...
    if ((Type1 == STANDARD_TYPE(Geom2d_Circle) || Type1 == STANDARD_TYPE(Geom2d_Line)) &&
        ...
        BisAna->Perform(afirstcurve1, asecondcurve1, apoint, afirstvector, asecondvector, adirection, ajointype,
                        tolerance, oncurve);
        ...
        if (oncurve) {
            ...
            if (Fd.Dot(Sd) < Sqrt(2. * Precision::Angular()) - 1.) IsLine = Standard_True;
        }
        if (IsLine) {
            //------------------------------------------------------------------
            // Half-Staight.
...
void Bisector_Bisec::Perform(const Handle(Geom2d_Curve) & afirstcurve, const Handle(Geom2d_Point) & asecondpoint,
                             const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
    ...
    if (Type1 == STANDARD_TYPE(Geom2d_Circle) || Type1 == STANDARD_TYPE(Geom2d_Line)) {
        ...
        //------------------------------------------------------------------
        Handle(Bisector_BisecAna) BisAna = new Bisector_BisecAna();
        BisAna->Perform(afirstcurve, asecondpoint, apoint, afirstvector, asecondvector, adirection, tolerance, oncurve);
        UFirst = BisAna->ParameterOfStartPoint();
        ULast = BisAna->ParameterOfEndPoint();
        Bis = BisAna;
    } else {
        ...
        if (oncurve) {
            if (Bisector::IsConvex(afirstcurve, adirection) || IsMaxRC(afirstcurve, afirstcurve->LastParameter(), RC)) {
    ...
    if (UFirst < Bis->FirstParameter()) UFirst = Bis->FirstParameter();
    if (ULast > Bis->LastParameter()) ULast = Bis->LastParameter();
    thebisector = new Geom2d_TrimmedCurve(Bis, UFirst, ULast);
...
void Bisector_Bisec::Perform(const Handle(Geom2d_Point) & afirstpoint, const Handle(Geom2d_Curve) & asecondcurve,
                             const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                             const Standard_Real adirection, const Standard_Real tolerance,
                             const Standard_Boolean oncurve)

{
    ...
    if (Type1 == STANDARD_TYPE(Geom2d_Circle) || Type1 == STANDARD_TYPE(Geom2d_Line)) {
        ...
        //------------------------------------------------------------------
        Handle(Bisector_BisecAna) BisAna = new Bisector_BisecAna();
        BisAna->Perform(afirstpoint, asecondcurve, apoint, afirstvector, asecondvector, adirection, tolerance, oncurve);
        UFirst = BisAna->ParameterOfStartPoint();
        ULast = BisAna->ParameterOfEndPoint();
        Bis = BisAna;
    } else {
...
void Bisector_Bisec::Perform(const Handle(Geom2d_Point) & afirstpoint, const Handle(Geom2d_Point) & asecondpoint,
                             const gp_Pnt2d& apoint, const gp_Vec2d& afirstvector, const gp_Vec2d& asecondvector,
                             const Standard_Real adirection, const Standard_Real tolerance,
                             const Standard_Boolean oncurve) {
    Handle(Bisector_BisecAna) Bis = new Bisector_BisecAna();

    Bis->Perform(afirstpoint, asecondpoint, apoint, afirstvector, asecondvector, adirection, tolerance, oncurve);
    thebisector = new Geom2d_TrimmedCurve(Bis, Bis->ParameterOfStartPoint(), Bis->ParameterOfEndPoint());
