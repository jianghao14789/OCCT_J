...
BRepPrimAPI_MakeRevol::BRepPrimAPI_MakeRevol(const TopoDS_Shape& S, const gp_Ax1& A, const Standard_Real D,
                                             const Standard_Boolean Copy)
    : myRevol(check(S), A, D, Copy), myIsBuild(Standard_False)
...
BRepPrimAPI_MakeRevol::BRepPrimAPI_MakeRevol(const TopoDS_Shape& S, const gp_Ax1& A, const Standard_Boolean Copy)
    :

      myRevol(check(S), A, 2. * M_PI, Copy), myIsBuild(Standard_False) {
...
static Standard_Boolean IsIntersect(const Handle(Adaptor3d_Curve) & theC, const gp_Ax1& theAxe) {
    ...
    const Standard_Real aParF = theC->FirstParameter() + aParTol, aParL = theC->LastParameter() - aParTol;
...
Standard_Boolean BRepPrimAPI_MakeRevol::CheckValidity(const TopoDS_Shape& theShape, const gp_Ax1& theA) {
