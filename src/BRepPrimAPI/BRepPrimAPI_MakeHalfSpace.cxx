...
static gp_Dir getNormalOnFace(const TopoDS_Face& theFace, const Standard_Real theU, const Standard_Real theV) {
    ...
    if (theFace.Orientation() == TopAbs_REVERSED) aNormal.Reverse();
    return aNormal;
}
...
static Standard_Boolean getNormalFromEdge(const TopoDS_Shape& theShape, const TopoDS_Edge& theEdge,
                                          const Standard_Real thePar, gp_Dir& theNormal) {
    gp_XYZ aSum;
...
static Standard_Boolean getNormalFromVertex(const TopoDS_Shape& theShape, const TopoDS_Vertex& theVer,
                                            gp_Dir& theNormal) {
    gp_XYZ aSum;
    TopExp_Explorer ex(theShape, TopAbs_FACE);
    for (; ex.More(); ex.Next()) {
...
//=======================================================================
static Standard_Boolean FindExtrema(const gp_Pnt& thePnt, const TopoDS_Shape& theShape, gp_Pnt& theMinPnt,
                                    gp_Dir& theNormal) {
    TopoDS_Vertex aRefVertex = BRepBuilderAPI_MakeVertex(thePnt);

    BRepExtrema_DistShapeShape ext(aRefVertex, theShape);

    if (!ext.IsDone() || ext.NbSolution() == 0) return Standard_False;
    ...
    for (Standard_Integer iext = 1; iext <= nbext; iext++) {
        if (ext.SupportTypeShape2(iext) == BRepExtrema_IsOnEdge) {
            ...
            if (getNormalFromEdge(theShape, aE, aPar, theNormal)) return Standard_True;
        } else if (ext.SupportTypeShape2(iext) == BRepExtrema_IsVertex) {
            theMinPnt = ext.PointOnShape2(iext);
            TopoDS_Vertex aV = TopoDS::Vertex(ext.SupportOnShape2(iext));
            if (getNormalFromVertex(theShape, aV, theNormal)) return Standard_True;
        }
    }
    return Standard_False;
}
...
static Standard_Boolean isOutside(const gp_Pnt& thePnt, const gp_Pnt& thePonF, const gp_Dir& theNormal) {
...
BRepPrimAPI_MakeHalfSpace::BRepPrimAPI_MakeHalfSpace(const TopoDS_Face& theFace, const gp_Pnt& theRefPnt) {
...
BRepPrimAPI_MakeHalfSpace::BRepPrimAPI_MakeHalfSpace(const TopoDS_Shell& theShell, const gp_Pnt& theRefPnt) {
    // Set the flag is <IsDone> to False.
    NotDone();
