...
public:
    ...
    SelectBasics_PickResult() : myObjPickedPnt(RealLast(), 0.0, 0.0), myDepth(RealLast()), myDistToCenter(RealLast()) {}

    //! Constructor with initialization.
    SelectBasics_PickResult(Standard_Real theDepth, Standard_Real theDistToCenter, const gp_Pnt& theObjPickedPnt)
        : myObjPickedPnt(theObjPickedPnt), myDepth(theDepth), myDistToCenter(theDistToCenter) {}

public:
    //! Return TRUE if result was been defined.
    Standard_Boolean IsValid() const {
        return myDepth != RealLast();
    }
    ...
    Standard_Boolean HasPickedPoint() const {
        return myObjPickedPnt.X() != RealLast();
    }
...
private:
    ...
    Standard_Real
        myDistToCenter; //!< Distance from 3d projection user-picked selection point to entity's geometry center
};
