...
static Standard_Boolean BBPP(const Standard_Real param, Blend_SurfRstFunction& Func, const math_Vector& sol,
                             const Standard_Real tol, Blend_Point& BP) {
...
Standard_Integer BRepBlend_SurfRstLineBuilder::ArcToRecadre(const math_Vector& theSol, const Standard_Integer PrevIndex,
                                                            gp_Pnt2d& lastpt2d, gp_Pnt2d& pt2d, Standard_Real& ponarc) {
    ...
    while (domain1->More()) {
        ...
        ok = 0;
        if (byinter) {
            ok = okinter = BRepBlend_BlendTool::Inters(pt2d, lastpt2d, surf1, domain1->Value(), prm, dist);
        }
        if (!ok) ok = BRepBlend_BlendTool::Project(pt2d, surf1, domain1->Value(), prm, dist);
...
BRepBlend_SurfRstLineBuilder::BRepBlend_SurfRstLineBuilder(const Handle(Adaptor3d_Surface) & Surf1,
    ...
    : done(Standard_False), sol(1, 3), surf1(Surf1), domain1(Domain1), surf2(Surf2), rst(Rst), domain2(Domain2),
      tolesp(0.0), tolgui(0.0), pasmax(0.0), fleche(0.0), param(0.0), rebrou(Standard_False),
      iscomplete(Standard_False), comptra(Standard_False), sens(0.0), decrochdeb(Standard_False),
...
void BRepBlend_SurfRstLineBuilder::Perform(Blend_SurfRstFunction& Func, Blend_FuncInv& Finv,
                                           Blend_SurfPointFuncInv& FinvP, Blend_SurfCurvFuncInv& FinvC,
                                           const Standard_Real Pdep, const Standard_Real Pmax,
                                           const Standard_Real MaxStep, const Standard_Real TolGuide,
                                           const math_Vector& ParDep, const Standard_Real Tolesp,
                                           const Standard_Real Fleche, const Standard_Boolean Appro) {
    done = Standard_False;
    ...
    BRepBlend_Extremity ptf1(previousP.PointOnS(), U, V, previousP.Parameter(), tolesp);
    BRepBlend_Extremity ptf2(previousP.PointOnC(), U, V, previousP.Parameter(), tolesp);
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::PerformFirstSection(
    ...
    const Standard_Real Pdep, const Standard_Real Pmax, const math_Vector& ParDep, const Standard_Real Tolesp,
    const Standard_Real TolGuide, const Standard_Boolean RecRst, const Standard_Boolean RecP,
    const Standard_Boolean RecS, Standard_Real& Psol, math_Vector& ParSol) {
    done = Standard_False;
    iscomplete = Standard_False;
    comptra = Standard_False;
    line = new BRepBlend_Line();
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::Complete(Blend_SurfRstFunction& Func, Blend_FuncInv& Finv,
                                                        Blend_SurfPointFuncInv& FinvP, Blend_SurfCurvFuncInv& FinvC,
...
void BRepBlend_SurfRstLineBuilder::InternalPerform(Blend_SurfRstFunction& Func, Blend_FuncInv& Finv,
                                                   Blend_SurfPointFuncInv& FinvP, Blend_SurfCurvFuncInv& FinvC,
        ...
        switch (State) {
            case Blend_OK: {
                ...
                if (param == Bound) {
                    ...
                    Exts.SetValue(previousP.PointOnS(), sol(1), sol(2), previousP.Parameter(), tolesp);
            ...
            case Blend_StepTooLarge: {
                ...
                if (Abs(stepw) < tolgui) {
                    previousP.ParametersOnS(U, V);
                    Exts.SetValue(previousP.PointOnS(), U, V, previousP.Parameter(), tolesp);
                    Extrst.SetValue(previousP.PointOnC(), previousP.ParameterOnC(), previousP.Parameter(), tolesp);
                    Arrive = Standard_True;
                    if (line->NbPoints() >= 2) {
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::Recadre(Blend_SurfCurvFuncInv& FinvC, math_Vector& Solinv,
                                                       Handle(Adaptor2d_Curve2d) & Arc, Standard_Boolean& IsVtx,
    ...
    if (recadre) {
        ...
        while (!IsVtx) {
            Vtx = domain1->Vertex();
            if (Abs(BRepBlend_BlendTool::Parameter(Vtx, Arc) - Solinv(3)) <= BRepBlend_BlendTool::Tolerance(Vtx, Arc)) {
                IsVtx = Standard_True;
            } else {
                domain1->NextVertex();
                IsVtx = !domain1->MoreVertex();
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::Recadre(Blend_SurfRstFunction& Func, Blend_FuncInv& Finv,
                                                       math_Vector& Solinv, Standard_Boolean& IsVtx,
                                                       Handle(Adaptor3d_HVertex) & Vtx) {
    ...
    if (Finv.IsSolution(Solinv, tolesp)) {
        ...
        while (!IsVtx) {
            Vtx = domain2->Vertex();
            if (Abs(BRepBlend_BlendTool::Parameter(Vtx, rst) - Solinv(1)) <= BRepBlend_BlendTool::Tolerance(Vtx, rst)) {
                IsVtx = Standard_True;
            } else {
                domain2->NextVertex();
                IsVtx = !domain2->MoreVertex();
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::Recadre(Blend_SurfPointFuncInv& FinvP, math_Vector& Solinv,
                                                       Standard_Boolean& IsVtx, Handle(Adaptor3d_HVertex) & Vtx) {
    // Le point.
    gp_Pnt2d p2drst;
    ...
    if (FinvP.IsSolution(Solinv, tolesp)) {
        ...
        while (!IsVtx) {
            Vtx = domain2->Vertex();
            if (Abs(BRepBlend_BlendTool::Parameter(Vtx, rst) - wpoint) <= BRepBlend_BlendTool::Tolerance(Vtx, rst)) {
                IsVtx = Standard_True;
            } else {
                domain2->NextVertex();
                IsVtx = !domain2->MoreVertex();
...
void BRepBlend_SurfRstLineBuilder::Transition(const Standard_Boolean OnFirst, const Handle(Adaptor2d_Curve2d) & Arc,
                                              const Standard_Real Param, IntSurf_Transition& TLine,
...
void BRepBlend_SurfRstLineBuilder::MakeExtremity(BRepBlend_Extremity& Extrem, const Standard_Boolean OnFirst,
                                                 const Handle(Adaptor2d_Curve2d) & Arc, const Standard_Real Param,
                                                 const Standard_Boolean IsVtx, const Handle(Adaptor3d_HVertex) & Vtx) {
    IntSurf_Transition Tline, Tarc;
    Standard_Real prm;
    Handle(Adaptor3d_TopolTool) Iter;
    if (OnFirst) {
        Extrem.SetValue(previousP.PointOnS(), sol(1), sol(2), previousP.Parameter(), tolesp);
        if (!previousP.IsTangencyPoint()) Extrem.SetTangent(previousP.TangentOnS());
        Iter = domain1;
    } else {
        Extrem.SetValue(previousP.PointOnC(), sol(3), previousP.Parameter(), tolesp);
        if (!previousP.IsTangencyPoint()) Extrem.SetTangent(previousP.TangentOnC());
        Iter = domain2;
    }
...
Blend_Status BRepBlend_SurfRstLineBuilder::TestArret(Blend_SurfRstFunction& Func, const Standard_Boolean TestDeflection,
    ...
    if (Func.IsSolution(sol, tolesp)) {
        ...
        if (curpointistangent) {
            ...
            curpoint.SetValue(pts, ptrst, param, sol(1), sol(2), pt2drst.X(), pt2drst.Y(), sol(3), tgs, tgrst, tg2ds,
                              tg2drst);
        }
        if (TestDeflection) {
...
Standard_Boolean BRepBlend_SurfRstLineBuilder::CheckInside(Blend_SurfRstFunction& Func, TopAbs_State& SituOnC,
                                                           TopAbs_State& SituOnS, Standard_Boolean& Decroch) {
