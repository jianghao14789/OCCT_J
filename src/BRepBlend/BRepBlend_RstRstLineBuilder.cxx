...
static Standard_Boolean BBPP(const Standard_Real param, Blend_RstRstFunction& Func, const math_Vector& sol,
    ...
    BP = Blend_Point(pntrst1, pntrst2, param, p2drst1.X(), p2drst1.Y(), p2drst2.X(), p2drst2.Y(), w1, w2);
    return 1;
}
...
BRepBlend_RstRstLineBuilder::BRepBlend_RstRstLineBuilder(const Handle(Adaptor3d_Surface) & Surf1,
    ...
    : done(Standard_False), sol(1, 2), surf1(Surf1), domain1(Domain1), surf2(Surf2), domain2(Domain2), rst1(Rst1),
      rst2(Rst2), tolesp(0.0), tolgui(0.0), pasmax(0.0), fleche(0.0), param(0.0), rebrou(Standard_False),
      iscomplete(Standard_False), comptra(Standard_False), sens(0.0), decrochdeb(Blend_NoDecroch),
...
void BRepBlend_RstRstLineBuilder::Perform(Blend_RstRstFunction& Func, Blend_SurfCurvFuncInv& Finv1,
                                          Blend_CurvPointFuncInv& FinvP1, Blend_SurfCurvFuncInv& Finv2,
                                          Blend_CurvPointFuncInv& FinvP2, const Standard_Real Pdep,
                                          const Standard_Real Pmax, const Standard_Real MaxStep,
                                          const Standard_Real TolGuide, const math_Vector& ParDep,
    ...
    BRepBlend_Extremity ptf1(previousP.PointOnC1(), U, previousP.Parameter(), tolesp);
    BRepBlend_Extremity ptf2(previousP.PointOnC2(), V, previousP.Parameter(), tolesp);
...
Standard_Boolean BRepBlend_RstRstLineBuilder::PerformFirstSection(
    ...
    Blend_SurfCurvFuncInv& Finv2, Blend_CurvPointFuncInv& FinvP2, const Standard_Real Pdep, const Standard_Real Pmax,
    const math_Vector& ParDep, const Standard_Real Tolesp, const Standard_Real TolGuide, const Standard_Boolean RecRst1,
    const Standard_Boolean RecP1, const Standard_Boolean RecRst2, const Standard_Boolean RecP2, Standard_Real& Psol,
    math_Vector& ParSol) {
    done = Standard_False;
    iscomplete = Standard_False;
    comptra = Standard_False;
...
Standard_Boolean BRepBlend_RstRstLineBuilder::Complete(Blend_RstRstFunction& Func, Blend_SurfCurvFuncInv& Finv1,
                                                       Blend_CurvPointFuncInv& FinvP1, Blend_SurfCurvFuncInv& Finv2,
                                                       Blend_CurvPointFuncInv& FinvP2, const Standard_Real Pmin) {
    if (!done) {
...
void BRepBlend_RstRstLineBuilder::InternalPerform(Blend_RstRstFunction& Func, Blend_SurfCurvFuncInv& Finv1,
                                                  Blend_CurvPointFuncInv& FinvP1, Blend_SurfCurvFuncInv& Finv2,
                                                  Blend_CurvPointFuncInv& FinvP2, const Standard_Real Bound) {
        ...
        switch (State) {
            case Blend_OK: {
                ...
                if (param == Bound) {
                    ...
                    Extrst1.SetValue(previousP.PointOnC1(), previousP.ParameterOnC1(), previousP.Parameter(), tolesp);
            ...
            case Blend_StepTooLarge: {
                ...
                if (Abs(stepw) < tolgui) {
                    Extrst1.SetValue(previousP.PointOnC1(), previousP.ParameterOnC1(), previousP.Parameter(), tolesp);
                    Extrst2.SetValue(previousP.PointOnC2(), previousP.ParameterOnC2(), previousP.Parameter(), tolesp);
                    Arrive = Standard_True;
#ifdef OCCT_DEBUG
                    if (line->NbPoints() >= 2) {
...
Standard_Boolean BRepBlend_RstRstLineBuilder::Recadre1(Blend_RstRstFunction& Func, Blend_SurfCurvFuncInv& Finv,
...
Standard_Boolean BRepBlend_RstRstLineBuilder::Recadre2(Blend_RstRstFunction& Func, Blend_SurfCurvFuncInv& Finv,
                                                       math_Vector& Solinv, Standard_Boolean& IsVtx,
                                                       Handle(Adaptor3d_HVertex) & Vtx) {
    math_Vector toler(1, 3), infb(1, 3), supb(1, 3);
    Finv.GetTolerance(toler, tolesp);
...
Standard_Boolean BRepBlend_RstRstLineBuilder::Recadre1(Blend_CurvPointFuncInv& FinvP, math_Vector& Solinv,
    ...
    if (FinvP.IsSolution(Solinv, tolesp)) {
        ...
        while (!IsVtx) {
            Vtx = domain1->Vertex();
            if (Abs(BRepBlend_BlendTool::Parameter(Vtx, rst1) - upoint) <= BRepBlend_BlendTool::Tolerance(Vtx, rst1)) {
                IsVtx = Standard_True;
            } else {
                domain1->NextVertex();
                IsVtx = !domain1->MoreVertex();
...
Standard_Boolean BRepBlend_RstRstLineBuilder::Recadre2(Blend_CurvPointFuncInv& FinvP, math_Vector& Solinv,
                                                       Standard_Boolean& IsVtx, Handle(Adaptor3d_HVertex) & Vtx) {
    // One is located on the last or the first point, following the
    // direction of processing.
    gp_Pnt2d p2drst2;
    ...
    if (FinvP.IsSolution(Solinv, tolesp)) {
        ...
        while (!IsVtx) {
            Vtx = domain2->Vertex();
            if (Abs(BRepBlend_BlendTool::Parameter(Vtx, rst2) - vpoint) <= BRepBlend_BlendTool::Tolerance(Vtx, rst2)) {
                IsVtx = Standard_True;
            } else {
                domain2->NextVertex();
                IsVtx = !domain2->MoreVertex();
...
void BRepBlend_RstRstLineBuilder::Transition(const Standard_Boolean OnFirst, const Handle(Adaptor2d_Curve2d) & Arc,
...
void BRepBlend_RstRstLineBuilder::MakeExtremity(BRepBlend_Extremity& Extrem, const Standard_Boolean OnFirst,
    ...
    if (OnFirst) {
        Extrem.SetValue(previousP.PointOnC1(), sol(1), previousP.Parameter(), tolesp);
        if (!previousP.IsTangencyPoint()) Extrem.SetTangent(previousP.TangentOnC1());
        Iter = domain1;
    } else {
        Extrem.SetValue(previousP.PointOnC2(), sol(2), previousP.Parameter(), tolesp);
        if (!previousP.IsTangencyPoint()) Extrem.SetTangent(previousP.TangentOnC1());
        Iter = domain2;
    }
...
Blend_Status BRepBlend_RstRstLineBuilder::TestArret(Blend_RstRstFunction& Func, const Standard_Boolean TestDeflection,
    ...
    if (Func.IsSolution(sol, tolesp)) {
        ...
        if (curpointistangent) {
            curpoint.SetValue(ptrst1, ptrst2, param, pt2drst1.X(), pt2drst1.Y(), pt2drst2.X(), pt2drst2.Y(), sol(1),
                              sol(2));
        } else {
            ...
            curpoint.SetValue(ptrst1, ptrst2, param, pt2drst1.X(), pt2drst1.Y(), pt2drst2.X(), pt2drst2.Y(), sol(1),
                              sol(2), tgrst1, tgrst2, tg2drst1, tg2drst2);
        }
        if (TestDeflection) {
...
Standard_Boolean BRepBlend_RstRstLineBuilder::CheckInside(Blend_RstRstFunction& Func, TopAbs_State& SituOnC1,
                                                          TopAbs_State& SituOnC2, Blend_DecrochStatus& Decroch) {
