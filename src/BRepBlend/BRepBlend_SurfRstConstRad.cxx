...
static void t3dto2d(Standard_Real& a, Standard_Real& b, const gp_Vec& A, const gp_Vec& B, const gp_Vec& C) {
...
BRepBlend_SurfRstConstRad::BRepBlend_SurfRstConstRad(const Handle(Adaptor3d_Surface) & Surf,
    ...
    : surf(Surf), surfrst(SurfRst), rst(Rst), cons(Rst, SurfRst), guide(CGuide), tguide(CGuide), prmrst(0.0),
      istangent(Standard_True), ray(0.0), choix(0), normtg(0.0), theD(0.), maxang(RealFirst()), minang(RealLast()),
...
Standard_Boolean BRepBlend_SurfRstConstRad::Values(const math_Vector& X, math_Vector& F, math_Matrix& D) {
    gp_Vec d1u1, d1v1, d1;
...
Standard_Boolean BRepBlend_SurfRstConstRad::IsSolution(const math_Vector& Sol, const Standard_Real Tol)
...
Standard_Boolean BRepBlend_SurfRstConstRad::Decroch(const math_Vector& Sol, gp_Vec& NS, gp_Vec& TgS) const {
...
void BRepBlend_SurfRstConstRad::Section(const Standard_Real Param, const Standard_Real U, const Standard_Real V,
...
void BRepBlend_SurfRstConstRad::GetShape(Standard_Integer& NbPoles, Standard_Integer& NbKnots, Standard_Integer& Degree,
...
void BRepBlend_SurfRstConstRad::GetTolerance(const Standard_Real BoundTol, const Standard_Real SurfTol,
...
void BRepBlend_SurfRstConstRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles, TColgp_Array1OfPnt2d& Poles2d,
...
Standard_Boolean BRepBlend_SurfRstConstRad::Section(const Blend_Point& P, TColgp_Array1OfPnt& Poles,
    ...
    if (!istgt) {
        return GeomFill::GetCircle(myTConv, ns, ns2, dnw, dn2w, nplan, dnplan, pts, ptrst, tgs, tgrst, Abs(ray), 0,
                                   Center, tgct, Poles, DPoles, Weights, DWeights);
    } else {
        GeomFill::GetCircle(myTConv, ns, ns2, nplan, pts, ptrst, Abs(ray), Center, Poles, Weights);
        return Standard_False;
    }
...
Standard_Boolean BRepBlend_SurfRstConstRad::Section(const Blend_Point&, TColgp_Array1OfPnt&, TColgp_Array1OfVec&,
...
void BRepBlend_SurfRstConstRad::Resolution(const Standard_Integer IC2d, const Standard_Real Tol, Standard_Real& TolU,
