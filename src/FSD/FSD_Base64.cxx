...
Standard_Size FSD_Base64::Encode(char* theEncodedStr, const Standard_Size theStrLen, const Standard_Byte* theData,
                                 const Standard_Size theDataLen) {
    // Base64字符集：包含大小写字母、数字和+/两个特殊字符
    // 共64个字符（0-63对应到字符集中的位置）
    static const char aBase64Chars[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    ...
    for (Standard_Size i = 0; i < theDataLen - aPad; i += 3) {
        ...
        //   第2字节: 位15-8
        //   第3字节: 位7-0
        uint32_t aWord = (uint32_t(theData[i]) << 16) + (uint32_t(theData[i + 1]) << 8) + theData[i + 2];
...
TCollection_AsciiString FSD_Base64::Encode(const Standard_Byte* theData, const Standard_Size theDataLen) {
...
// Base64解码是编码的逆过程
Standard_Size FSD_Base64::Decode(Standard_Byte* theDecodedData, const Standard_Size theDataLen,
    ...
    for (Standard_Size i = 0; i < aNbIter; i += 4) {
        ...
        //   第4字符：位5-0
        unsigned aWord =
            (aBase64Codes[unsigned(theEncodedStr[i])] << 18) + (aBase64Codes[unsigned(theEncodedStr[i + 1])] << 12) +
            (aBase64Codes[unsigned(theEncodedStr[i + 2])] << 6) + aBase64Codes[unsigned(theEncodedStr[i + 3])];
...
// 便利函数：直接返回解码后的缓冲区对象
// 这个版本自动处理内存分配和数据解码
Handle(NCollection_Buffer) FSD_Base64::Decode(Standard_CString theEncodedStr, const Standard_Size theStrLen) {
