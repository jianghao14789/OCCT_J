...
static Standard_Integer ComputeBox(const gp_Hypr& aHypr, const Standard_Real aT1, const Standard_Real aT2,
                                   Bnd_Box& aBox);

namespace {
//! Compute method
template <class PointType, class BndBoxType>
void Compute(const Standard_Real theP1, const Standard_Real theP2, const Standard_Real theRa, const Standard_Real theRb,
             const PointType& theXd, const PointType& theYd, const PointType& theO, BndBoxType& theB) {
...
#define addPoint7 theB.Add(PointType(theO.Coord() + aRam * PI4 * theXd.Coord() - aRbm * PI4 * theYd.Coord()))
    ...
    switch (aDeb) {
        ...
        Standard_FALLTHROUGH case 2: {
            addPoint2;
            if (aFin <= 2) break;
        }
        Standard_FALLTHROUGH case 3: {
            addPoint3;
            if (aFin <= 3) break;
        }
        Standard_FALLTHROUGH case 4: {
            addPoint4;
            if (aFin <= 4) break;
        }
        Standard_FALLTHROUGH case 5: {
            addPoint5;
            if (aFin <= 5) break;
        }
        Standard_FALLTHROUGH case 6: {
            addPoint6;
            if (aFin <= 6) break;
        }
        Standard_FALLTHROUGH case 7: {
            addPoint7;
            if (aFin <= 7) break;
        }
        Standard_FALLTHROUGH case 8: {
            addPoint0;
            if (aFin <= 8) break;
        }
        Standard_FALLTHROUGH case 9: {
            addPoint1;
            if (aFin <= 9) break;
        }
        Standard_FALLTHROUGH case 10: {
            addPoint2;
            if (aFin <= 10) break;
        }
        Standard_FALLTHROUGH case 11: {
            addPoint3;
            if (aFin <= 11) break;
        }
        Standard_FALLTHROUGH case 12: {
            addPoint4;
            if (aFin <= 12) break;
        }
        Standard_FALLTHROUGH case 13: {
            addPoint5;
            if (aFin <= 13) break;
        }
        Standard_FALLTHROUGH case 14: {
            addPoint6;
            if (aFin <= 14) break;
        }
        Standard_FALLTHROUGH case 15: {
            addPoint7;
            if (aFin <= 15) break;
        }
    }
...
void BndLib::Add(const gp_Lin& L, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol, Bnd_Box& B) {
...
void BndLib::Add(const gp_Lin2d& L, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box2d& B) {
...
void BndLib::Add(const gp_Circ& C, const Standard_Real U1, const Standard_Real U2, const Standard_Real Tol,
                 Bnd_Box& B) {
...
void BndLib::Add(const gp_Circ2d& C, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box2d& B) {

    Compute(P1, P2, C.Radius(), C.Radius(), gp_Pnt2d(C.XAxis().Direction().XY()), gp_Pnt2d(C.YAxis().Direction().XY()),
            C.Location(), B);
    B.Enlarge(Tol);
}
...
void BndLib::Add(const gp_Elips& C, const Standard_Real U1, const Standard_Real U2, const Standard_Real Tol,
                 Bnd_Box& B) {
    Standard_Real period = 2. * M_PI - Epsilon(2. * M_PI);
...
void BndLib::Add(const gp_Elips2d& C, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box2d& B) {

    Compute(P1, P2, C.MajorRadius(), C.MinorRadius(), gp_Pnt2d(C.XAxis().Direction().XY()),
            gp_Pnt2d(C.YAxis().Direction().XY()), C.Location(), B);
    B.Enlarge(Tol);
}

void BndLib::Add(const gp_Parab& P, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box& B) {
...
void BndLib::Add(const gp_Parab2d& P, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box2d& B) {
...
// purpose  :
//=======================================================================
void BndLib::Add(const gp_Hypr& H, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box& B) {
    if (Precision::IsNegativeInfinite(P1)) {
        if (Precision::IsNegativeInfinite(P2)) {
            throw Standard_Failure("BndLib::bad parameter");
...
void BndLib::Add(const gp_Hypr2d& H, const Standard_Real P1, const Standard_Real P2, const Standard_Real Tol,
                 Bnd_Box2d& B) {
...
static void ComputeCyl(const gp_Cylinder& Cyl, const Standard_Real UMin, const Standard_Real UMax,
                       const Standard_Real VMin, const Standard_Real VMax, Bnd_Box& B) {
...
void BndLib::Add(const gp_Cylinder& S, const Standard_Real UMin, const Standard_Real UMax, const Standard_Real VMin,
...
void BndLib::Add(const gp_Cylinder& S, const Standard_Real VMin, const Standard_Real VMax, const Standard_Real Tol,
                 Bnd_Box& B) {

    BndLib::Add(S, 0., 2. * M_PI, VMin, VMax, Tol, B);
}

static void ComputeCone(const gp_Cone& Cone, const Standard_Real UMin, const Standard_Real UMax,
                        const Standard_Real VMin, const Standard_Real VMax, Bnd_Box& B) {
...
void BndLib::Add(const gp_Cone& S, const Standard_Real UMin, const Standard_Real UMax, const Standard_Real VMin,
                 const Standard_Real VMax, const Standard_Real Tol, Bnd_Box& B) {
...
void BndLib::Add(const gp_Cone& S, const Standard_Real VMin, const Standard_Real VMax, const Standard_Real Tol,
                 Bnd_Box& B) {

    BndLib::Add(S, 0., 2. * M_PI, VMin, VMax, Tol, B);
}

static void ComputeSphere(const gp_Sphere& Sphere, const Standard_Real UMin, const Standard_Real UMax,
                          const Standard_Real VMin, const Standard_Real VMax, Bnd_Box& B) {
...
void BndLib::Add(const gp_Sphere& S, const Standard_Real UMin, const Standard_Real UMax, const Standard_Real VMin,
                 const Standard_Real VMax, const Standard_Real Tol, Bnd_Box& B) {
...
void BndLib::Add(const gp_Torus& S, const Standard_Real UMin, const Standard_Real UMax, const Standard_Real VMin,
                 const Standard_Real VMax, const Standard_Real Tol, Bnd_Box& B) {
...
#define SC 0.71
#define addP0                                                                                                          \
    (Compute(UMin, UMax, Ra + Ri, Ra + Ri, gp_Pnt(S.XAxis().Direction().XYZ()), gp_Pnt(S.YAxis().Direction().XYZ()),   \
             S.Location(), B))
#define addP1                                                                                                          \
    (Compute(UMin, UMax, Ra + Ri * SC, Ra + Ri * SC, gp_Pnt(S.XAxis().Direction().XYZ()),                              \
             gp_Pnt(S.YAxis().Direction().XYZ()), gp_Pnt(S.Location().XYZ() + (Ri * SC) * S.Axis().Direction().XYZ()), \
             B))
#define addP2                                                                                                          \
    (Compute(UMin, UMax, Ra, Ra, gp_Pnt(S.XAxis().Direction().XYZ()), gp_Pnt(S.YAxis().Direction().XYZ()),             \
             gp_Pnt(S.Location().XYZ() + Ri * S.Axis().Direction().XYZ()), B))
#define addP3                                                                                                          \
    (Compute(UMin, UMax, Ra - Ri * SC, Ra - Ri * SC, gp_Pnt(S.XAxis().Direction().XYZ()),                              \
             gp_Pnt(S.YAxis().Direction().XYZ()), gp_Pnt(S.Location().XYZ() + (Ri * SC) * S.Axis().Direction().XYZ()), \
             B))
...
// purpose  :
//=======================================================================
Standard_Integer ComputeBox(const gp_Hypr& aHypr, const Standard_Real aT1, const Standard_Real aT2, Bnd_Box& aBox) {
