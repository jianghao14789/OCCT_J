...
//
static Standard_Integer NbSamples(const Adaptor3d_Curve& C, const Standard_Real Umin, const Standard_Real Umax);

static Standard_Real AdjustExtr(const Adaptor3d_Curve& C, const Standard_Real UMin, const Standard_Real UMax,
                                const Standard_Real Extr0, const Standard_Integer CoordIndx, const Standard_Real Tol,
...
//=======================================================================
static void reduceSplineBox(const Adaptor3d_Curve& theCurve, const Bnd_Box& theOrigBox, Bnd_Box& theReducedBox) {
    ...
    // Left bound.
    if (aPolesXMin > x) x = aPolesXMin;
    if (aPolesYMin > y) y = aPolesYMin;
    if (aPolesZMin > z) z = aPolesZMin;

    // Right bound.
    if (aPolesXMax < X) X = aPolesXMax;
    if (aPolesYMax < Y) Y = aPolesYMax;
    if (aPolesZMax < Z) Z = aPolesZMax;
...
// OCC566(apo)->
static Standard_Real FillBox(Bnd_Box& B, const Adaptor3d_Curve& C, const Standard_Real first, const Standard_Real last,
...
void BndLib_Add3dCurve::Add(const Adaptor3d_Curve& C, const Standard_Real U1, const Standard_Real U2,
    ...
    switch (C.GetType()) {
        ...
        case GeomAbs_BSplineCurve: {
            ...
            if (Abs(Bs->FirstParameter() - U1) > Precision::Parametric(Tol) ||
                ...
                if (Bsaux->IsPeriodic())
                    ElCLib::AdjustPeriodic(Bsaux->FirstParameter(), Bsaux->LastParameter(), Precision::PConfusion(), u1,
                                           u2);
                else {
                    ////////////////////////////////////////////////
                ...
                if (Abs(u2 - u1) < aSegmentTol) aSegmentTol = Abs(u2 - u1) * 0.01;
...
void BndLib_Add3dCurve::AddOptimal(const Adaptor3d_Curve& C, const Standard_Real U1, const Standard_Real U2,
                                   const Standard_Real Tol, Bnd_Box& B) {
    switch (C.GetType()) {
...
// purpose  :
//=======================================================================
void BndLib_Add3dCurve::AddGenCurv(const Adaptor3d_Curve& C, const Standard_Real UMin, const Standard_Real UMax,
                                   const Standard_Real Tol, Bnd_Box& B) {
...
public:
    CurvMaxMinCoordMVar(const Adaptor3d_Curve& theCurve, const Standard_Real UMin, const Standard_Real UMax,
...
public:
    CurvMaxMinCoord(const Adaptor3d_Curve& theCurve, const Standard_Real UMin, const Standard_Real UMax,
                    const Standard_Integer CoordIndx, const Standard_Real Sign)
        : myCurve(theCurve), myUMin(UMin), myUMax(UMax), myCoordIndx(CoordIndx), mySign(Sign) {}
...
Standard_Real AdjustExtr(const Adaptor3d_Curve& C, const Standard_Real UMin, const Standard_Real UMax,
                         const Standard_Real Extr0, const Standard_Integer CoordIndx, const Standard_Real Tol,
                         const Standard_Boolean IsMin) {
...
Standard_Integer NbSamples(const Adaptor3d_Curve& C, const Standard_Real Umin, const Standard_Real Umax) {
    Standard_Integer N;
