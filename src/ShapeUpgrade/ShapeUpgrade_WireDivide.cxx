...
// purpose  :
//=======================================================================
ShapeUpgrade_WireDivide::ShapeUpgrade_WireDivide() : ShapeUpgrade_Tool(), myStatus(0) {
...
void ShapeUpgrade_WireDivide::Load(const TopoDS_Edge& E) {
    ...
    if (MakeWire.IsDone()) Load(MakeWire.Wire());
}
...
static void CorrectSplitValues(const Handle(TColStd_HSequenceOfReal) orig3d,
                               const Handle(TColStd_HSequenceOfReal) orig2d, Handle(TColStd_HSequenceOfReal) new2d,
    ...
    for (i = 1; i <= len3d; i++) {
        ...
        for (Standard_Integer j = 1; j <= len2d && !index; j++)
            if (Abs(par - orig2d->Value(j)) < preci) index = j;
    ...
    for (i = 1; i <= len2d; i++) {
        ...
        for (Standard_Integer j = 1; j <= len3d && !index; j++)
            if (Abs(par - orig3d->Value(j)) < preci) index = j;
...
void ShapeUpgrade_WireDivide::Perform() {
    ...
    if (!myFace.IsNull()) Surf = BRep_Tool::Surface(myFace, Loc);

    Standard_Boolean isSplit3d = Standard_True;
    switch (myEdgeMode) {
        case 0:
            if (!myFace.IsNull()) isSplit3d = Standard_False;
            break;
        case 1:
            if (myFace.IsNull()) isSplit3d = Standard_False;
            break;
    ...
    if (isSplit3d) myEdgeDivide->SetSplitCurve3dTool(GetSplitCurve3dTool());
    ...
    for (TopoDS_Iterator ItW(myWire, Standard_False); ItW.More(); ItW.Next()) {
        ...
        for (TopExp_Explorer exp(sh, TopAbs_EDGE); exp.More(); exp.Next()) {
            ...
            if (!myFace.IsNull()) isSeam = BRep_Tool::IsClosed(E, myFace);
            Handle(TColGeom2d_HArray1OfCurve) theSegments2d;
            if (myEdgeDivide->HasCurve2d()) theSegments2d = theSplit2dTool->GetCurves();
            ...
            if (isSeam) {
                ...
                if (sae.PCurve(erev, myFace, c2, f2, l2, Standard_False)) {
                    ...
                    if (!theKnots2d.IsNull()) theSplit2dTool->SetSplitValues(theKnots2d);
...
#ifdef OCCT_DEBUG
                        std::cout << "Error: ShapeUpgrade_WireDivide: seam has different splitting values on pcurvesd"
                                  << std::endl;
#endif
                    } else
            ...
            if (nbc <= 1 && !theSplit3dTool->Status(ShapeExtend_DONE) && !theSplit2dTool->Status(ShapeExtend_DONE)) {
                B.Add(newWire, E);
                continue;
            }
            ...
            for (Standard_Integer icurv = 1; icurv <= nbc; icurv++) {
                ...
                if (isSeam) revPCurve = theSegments2dR->Value(icurv);
                ...
                if (icurv <= nbc && nbc != 1 && !isDeg) {
                    ...
                    if (Small) {
                        if (P.Distance(P1) > MinTolerance() || P.Distance(PM) > MinTolerance()) {
                            ...
                            if (FixSmallCurveTool->Approx(atmpCurve, atmpCurve2d1, atmprepcurve, SavParf,
                                                          par)) { // BRepTools
                ...
                // if(!Savnum) Savnum = icurv;
                if (!theNewCurve3d.IsNull())
                    theTransferParamTool->TransferRange(newEdge, theKnots3d->Value(Savnum),
                                                        theKnots3d->Value(icurv + 1), Standard_False);
                else
                    theTransferParamTool->TransferRange(newEdge, theKnots2d->Value(Savnum),
                                                        theKnots2d->Value(icurv + 1), Standard_True);
                /*
                ...
                if (!theNewCurve3d.IsNull()) sbe.SetRange3d(newEdge, f3d, l3d);
                if (!theNewPCurve1.IsNull()) {
