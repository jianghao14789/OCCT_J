...
Standard_Boolean ShapeUpgrade_RemoveInternalWires::Perform() {
    ...
    if (myRemoveFacesMode) removeSmallFaces();
...
Standard_Boolean ShapeUpgrade_RemoveInternalWires::Perform(const TopTools_SequenceOfShape& theSeqShapes) {
    ...
    for (; i <= nb; i++) {
        ...
        if (aS.ShapeType() == TopAbs_FACE)
            ...
            if (!aWireFaces.Extent()) TopExp::MapShapesAndAncestors(myShape, TopAbs_WIRE, TopAbs_FACE, aWireFaces);
...
void ShapeUpgrade_RemoveInternalWires::removeSmallWire(const TopoDS_Shape& theFace, const TopoDS_Shape& theWire) {
    TopoDS_Face aF = TopoDS::Face(theFace);
    TopoDS_Wire anOutW = ShapeAnalysis::OuterWire(aF);
    TopoDS_Iterator aIt(aF);
    for (; aIt.More(); aIt.Next()) {
        if (aIt.Value().ShapeType() != TopAbs_WIRE || aIt.Value().IsSame(anOutW)) continue;
        // Handle(ShapeExtend_WireData) asewd = new  ShapeExtend_WireData();
        TopoDS_Wire aW = TopoDS::Wire(aIt.Value());
        if (!theWire.IsNull() && !theWire.IsSame(aW)) continue;
...
void ShapeUpgrade_RemoveInternalWires::removeSmallFaces() {
        ...
        for (; aIte.More(); aIte.Next()) {
            ...
            for (; aliter.More(); aliter.Next()) {
                ...
                if (!isFind) {
                    ...
                    if (isOuter) aFaceCandidates.Add(aF);
                }
            }
        }
            ...
            for (; n <= nbE; n++) {
                ...
                if (!myRemoveEdges.IsBound(aE)) {
                    ...
                    for (; aliter3.More(); aliter3.Next()) {
                        ...
                        if (aF2.IsNull()) continue;
                        if (!aF.IsSame(aF2) && !aFaceCandidates.Contains(aF2)) nbNotRemoved++;
                    }
                }
            }
    ...
    if (myRemovedFaces.Length()) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
}
