...
static Handle(Geom2d_BezierCurve)
    MakeBezier2d(const Handle(Geom2d_Curve) & theCurve2d, const Standard_Real theFirst, const Standard_Real theLast) {
...
void ShapeUpgrade_ConvertCurve2dToBezier::Compute() {
    ...
    if (myCurve->IsKind(STANDARD_TYPE(Geom2d_BSplineCurve)) || myCurve->IsKind(STANDARD_TYPE(Geom2d_BezierCurve))) {
        // static function`s code getted from ShapeConvert
        Standard_Real tmpF, tmpL, aDeviation;
        Handle(Geom2d_Line) aTmpLine2d = ShapeCustom_Curve2d::ConvertToLine2d(
            myCurve, First, Last, Precision::Approximation(), tmpF, tmpL, aDeviation);
    ...
    if (myCurve->IsKind(STANDARD_TYPE(Geom2d_TrimmedCurve))) {
        ...
        if (myCurve->IsKind(STANDARD_TYPE(Geom2d_Conic))) {
            Handle(Geom2d_Curve) tcurve =
                new Geom2d_TrimmedCurve(myCurve, First, Last); // protection against parabols ets
        ...
        if (Abs(First - bf) < precision) First = bf;
        if (Abs(Last - bl) < precision) Last = bl;
        if (First < bf) {
#ifdef OCCT_DEBUG
        ...
        for (j = 1; j <= nbArcs; j++) {
            ...
            if (nextKnot - mySplitParams->Value(mySplitParams->Length()) > precision) {
                ...
                if (aCrv2d->IsKind(STANDARD_TYPE(Geom2d_BezierCurve))) {
                    ...
                    Standard_Real tmpF, tmpL, aDeviation;
                    Handle(Geom2d_Line) aTmpLine2d = ShapeCustom_Curve2d::ConvertToLine2d(
                        aCrv2d, newFirst, newLast, Precision::Approximation(), tmpF, tmpL, aDeviation);
                    if (!aTmpLine2d.IsNull() && (aDeviation <= Precision::Approximation())) {
...
void ShapeUpgrade_ConvertCurve2dToBezier::Build(const Standard_Boolean /*Segment*/) {
    ...
    for (Standard_Integer i = 2; i <= nb; i++) {
        ...
        Handle(Geom2d_BezierCurve) bes = Handle(Geom2d_BezierCurve)::DownCast(mySegments->Value(j - 1)->Copy());
