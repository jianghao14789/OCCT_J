...
static Standard_Boolean IsOnSingularity(const TopTools_ListOfShape& theEdgeList) {
    ...
    for (; anItl.More(); anItl.Next()) {
        ...
        if (BRep_Tool::Degenerated(anEdge)) return Standard_True;
    }

    return Standard_False;
}

static void SplitWire(const TopoDS_Wire& theWire, const TopoDS_Face& theFace, const TopTools_IndexedMapOfShape& theVmap,
...
static Standard_Real TrueValueOfOffset(const Standard_Real theValue, const Standard_Real thePeriod) {
    if (theValue > 0.) return thePeriod;
...
// purpose  : auxiliary
//=======================================================================
static void UpdateBoundaries(const Handle(Geom2d_Curve) & thePCurve, const Standard_Real theFirst,
                             const Standard_Real theLast, Standard_Real& theUfirst, Standard_Real& theUlast) {
    ...
    for (Standard_Integer i = 0; i <= NbSamples; i++) {
        ...
        if (aPoint.X() < theUfirst) theUfirst = aPoint.X();
        if (aPoint.X() > theUlast) theUlast = aPoint.X();
    }
}

static Standard_Boolean TryMakeLine(const Handle(Geom2d_Curve) & thePCurve, const Standard_Real theFirst,
                                    const Standard_Real theLast, Handle(Geom2d_Line) & theLine) {
    ...
    if (Abs(aSqLen - aSqParamLen) > Precision::Confusion()) return Standard_False;
    ...
    for (Standard_Integer i = 1; i < NbSamples; i++) {
        ...
        if (aDist > Precision::Confusion()) return Standard_False;
    }
...
static void RemoveEdgeFromMap(const TopoDS_Edge& theEdge, TopTools_IndexedDataMapOfShapeListOfShape& theVEmap) {
    TopoDS_Vertex VV[2];
...
static Standard_Real ComputeMinEdgeSize(const TopTools_SequenceOfShape& theEdges, const TopoDS_Face& theRefFace,
    ...
    for (Standard_Integer ind = 1; ind <= theEdges.Length(); ind++) {
        ...
        if (aSqDist < MinSize) MinSize = aSqDist;
    }
    MinSize = Sqrt(MinSize);
    return MinSize;
}

static void FindFaceWithMaxUbound(const TopTools_SequenceOfShape& theFaces, const TopoDS_Face& theRefFace,
                                  const TopTools_MapOfShape& theEdgesMap, Standard_Real& theUmax,
    ...
    for (Standard_Integer ii = 1; ii <= theFaces.Length(); ii++) {
        ...
        for (; Explo.More(); Explo.Next()) {
            ...
            if (!theEdgesMap.Contains(anEdge)) continue;
...
static void RelocatePCurvesToNewUorigin(const TopTools_SequenceOfShape& theEdges, const TopoDS_Shape& theFirstFace,
                                        const TopoDS_Face& theRefFace, const Standard_Real theCoordTol,
    ...
    for (;;) // walk by contours
    {
        ...
        for (Standard_Integer ii = 1; ii <= theEdges.Length(); ii++) {
            ...
            if (theUsedEdges.Contains(anEdge)) continue;
        ...
        TopoDS_Vertex StartVertex = (anOr == TopAbs_FORWARD) ? TopExp::FirstVertex(StartEdge, Standard_True)
        ...
        for (;;) // collect pcurves of a contour
        {
            ...
            TopoDS_Vertex CurVertex = (anOr == TopAbs_FORWARD) ? TopExp::LastVertex(CurEdge, Standard_True)
                                                               : TopExp::FirstVertex(CurEdge, Standard_True);

            const TopTools_ListOfShape& Elist = theVEmap.FindFromKey(CurVertex);
            if (Elist.IsEmpty()) break; // end of contour in 3d

            TopTools_ListIteratorOfListOfShape itl(Elist);
            for (; itl.More(); itl.Next()) {
                const TopoDS_Edge& anEdge = TopoDS::Edge(itl.Value());
                if (anEdge.IsSame(CurEdge)) continue;
                TopoDS_Vertex aFirstVertex = (anOr == TopAbs_FORWARD) ? TopExp::FirstVertex(anEdge, Standard_True)
                ...
                if (!(Abs(anOffset) < theCoordTol || Abs(Abs(anOffset) - theUperiod) < theCoordTol))
...
static void InsertWiresIntoFaces(const TopTools_SequenceOfShape& theWires, const TopTools_SequenceOfShape& theFaces,
...
static TopoDS_Face FindCommonFace(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
                                  const TopTools_IndexedDataMapOfShapeListOfShape& theVFmap,
                                  TopAbs_Orientation& theOrOfE1OnFace, TopAbs_Orientation& theOrOfE2OnFace) {
    ...
    for (; itl.More(); itl.Next()) {
        ...
        for (; Explo.More(); Explo.Next()) {
            ...
            if (e1found && e2found) return aFace;
        }
    }
...
static Standard_Boolean FindClosestPoints(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
                                          const TopTools_IndexedDataMapOfShapeListOfShape& theVFmap,
                                          TopoDS_Face& theCommonFace, Standard_Real& theMinSqDist,
                                          TopAbs_Orientation& OrOfE1OnFace, TopAbs_Orientation& OrOfE2OnFace,
                                          Standard_Integer& theIndOnE1, Standard_Integer& theIndOnE2,
                                          gp_Pnt2d* PointsOnEdge1, gp_Pnt2d* PointsOnEdge2)

{
    theCommonFace = FindCommonFace(theEdge1, theEdge2, theVFmap, OrOfE1OnFace, OrOfE2OnFace);
    if (theCommonFace.IsNull()) return Standard_False;
...
//=======================================================================
static void
ReconstructMissedSeam(const TopTools_SequenceOfShape& theEdges, const TopTools_SequenceOfShape& theRemovedEdges,
                      const TopTools_MapOfShape& theUsedEdges, const TopoDS_Face& theFrefFace,
                      const TopoDS_Vertex& theCurVertex, const gp_Pnt2d& theCurPoint, const Standard_Real thePeriod,
                      const Standard_Real theFaceCoordMin, const Standard_Boolean theIsU,
                      const Standard_Real theCoordTol, TopoDS_Edge& theNextEdge, TopoDS_Wire& theNewWire,
                      gp_Pnt2d& theNextPoint, gp_Pnt2d& theStartOfNextEdge, TopoDS_Vertex& theLastVertexOfSeam,
                      TopTools_IndexedDataMapOfShapeListOfShape& theVEmap)

{
    ...
    for (Standard_Integer ind = 1; ind <= theEdges.Length(); ind++) {
        ...
        if (theUsedEdges.Contains(aCandidate)) continue;
        BRepAdaptor_Curve2d BAcurve2d(aCandidate, theFrefFace);
        Standard_Real CandParam =
            (aCandidate.Orientation() == TopAbs_FORWARD) ? BAcurve2d.FirstParameter() : BAcurve2d.LastParameter();
        ...
        if (DeltaCoord > theCoordTol) continue;
    ...
    if (aRemovedEdge.IsNull()) {
        ...
        if (SeamDir > 0) {
            V1 = theCurVertex;
            V2 = theLastVertexOfSeam;
            Param1 = theCurPoint.Coord(3 - IndCoord);
            Param2 = theStartOfNextEdge.Coord(3 - IndCoord);
        } else {
            V1 = theLastVertexOfSeam;
            V2 = theCurVertex;
            Param1 = theStartOfNextEdge.Coord(3 - IndCoord);
            Param2 = theCurPoint.Coord(3 - IndCoord);
        }
    } else {
    ...
    if (aRemovedEdge.IsNull()) {
        ...
        if ((Param1 != InitialParam1 || Param2 != InitialParam2) && MissedCurve->IsPeriodic()) {
        ...
        if (SeamDir < 0) MissedSeam.Reverse();
    } else {
        MissedSeam = aRemovedEdge;
        MissedSeam.Orientation(TopAbs_FORWARD);
        if (!theCurVertex.IsSame(V1)) MissedSeam.Reverse();
    }
    ...
    Standard_Real ParamOfNextPoint =
        (theNextEdge.Orientation() == TopAbs_FORWARD) ? BAcurve2d.LastParameter() : BAcurve2d.FirstParameter();
...
// purpose  : auxiliary
//=======================================================================
static void TransformPCurves(const TopoDS_Face& theRefFace, const TopoDS_Face& theFace,
    ...
    Standard_Boolean ToModify = Standard_False, ToTranslate = Standard_False, ToRotate = Standard_False,
                     X_Reverse = Standard_False, Y_Reverse = Standard_False, ToProject = Standard_False;
    ...
    if (!ElemSurfFace.IsNull() && !ElemRefSurf.IsNull()) {
        ...
        if (Abs(aParam) > Precision::PConfusion()) aTranslation = -aParam;
        ...
        if (CrossProd1 * CrossProd2 < 0.) X_Reverse = Standard_True;

        Standard_Real ScalProd = VdirSurfFace * VdirRefSurf;
        if (ScalProd < 0.) Y_Reverse = Standard_True;

        if (!X_Reverse && !Y_Reverse) {
            gp_Dir DirRef = VdirRefSurf;
            if (!AxisOfRefSurf.Direct()) DirRef.Reverse();
    ...
    for (; Explo.More(); Explo.Next()) {
        ...
        if (BRep_Tool::Degenerated(anEdge) && ToModify) continue;

        if (ToProject && BRep_Tool::IsClosed(anEdge, theFace)) continue;

        if (!aEmap.Add(anEdge)) continue;

        Standard_Real fpar, lpar;
        Handle(Geom2d_Curve) PCurveOnRef = BRep_Tool::CurveOnSurface(anEdge, theRefFace, fpar, lpar);
        if (!PCurveOnRef.IsNull() && !(ToModify || ToProject)) continue;
        ...
        for (Standard_Integer ii = 0; ii < NbPcurves; ii++) {
            ...
            if (ToTranslate) NewPCurves[ii]->Translate(gp_Vec2d(0., aTranslation));
            if (Y_Reverse) NewPCurves[ii]->Mirror(gp::OX2d());
            if (X_Reverse) {
                NewPCurves[ii]->Mirror(gp::OY2d());
                NewPCurves[ii]->Translate(gp_Vec2d(2 * M_PI, 0.));
            }
            if (ToRotate) NewPCurves[ii]->Translate(gp_Vec2d(anAngle, 0.));
        }
...
static Standard_Boolean AddOrdinaryEdges(TopTools_SequenceOfShape& edges, const TopoDS_Shape aShape,
                                         Standard_Integer& anIndex, TopTools_SequenceOfShape& theRemovedEdges) {
    // map of edges
...
static Standard_Boolean getCylinder(Handle(Geom_Surface) & theInSurface, gp_Cylinder& theOutCylinder) {
    ...
    if (theInSurface->IsKind(STANDARD_TYPE(Geom_CylindricalSurface))) {
        ...
        Handle(Geom_SurfaceOfLinearExtrusion) aLES = Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(theInSurface);
...
//=======================================================================
static Standard_Boolean GetNormalToSurface(const TopoDS_Face& theFace, const TopoDS_Edge& theEdge,
                                           const Standard_Real theP, gp_Dir& theNormal) {
    Standard_Real f, l;
    ...
    if (BRep_Tool::IsClosed(theEdge, theFace)) {
        ...
        if (anEdgeInFace.IsNull()) return Standard_False;
...
// purpose  :
//=======================================================================
static Standard_Boolean IsSameDomain(const TopoDS_Face& aFace, const TopoDS_Face& aCheckedFace,
                                     const Standard_Real theLinTol, const Standard_Real theAngTol,
    ...
    if (S1 == S2 && L1 == L2) return Standard_True;
    ...
    if (S1->IsKind(STANDARD_TYPE(Geom_ElementarySurface)) && S2->IsKind(STANDARD_TYPE(Geom_ElementarySurface))) {
        ...
        try {
            ...
            if (!anIIInt.IsDone() || anIIInt.IsEmpty()) return Standard_False;
    ...
    if ((S1->IsKind(STANDARD_TYPE(Geom_CylindricalSurface)) || S1->IsKind(STANDARD_TYPE(Geom_SweptSurface))) &&
        (S2->IsKind(STANDARD_TYPE(Geom_CylindricalSurface)) || S2->IsKind(STANDARD_TYPE(Geom_SweptSurface)))) {
...
// purpose  :
//=======================================================================
static void UpdateMapOfShapes(TopTools_MapOfShape& theMapOfShapes, Handle(ShapeBuild_ReShape) & theContext) {
    for (TopTools_MapIteratorOfMapOfShape it(theMapOfShapes); it.More(); it.Next()) {
        ...
        if (!aContextShape.IsSame(aShape)) theMapOfShapes.Add(aContextShape);
    }
}
...
//=======================================================================
static TopoDS_Edge GlueEdgesWithPCurves(const TopTools_SequenceOfShape& aChain, const TopoDS_Vertex& FirstVertex,
    ...
    for (i = 1; i <= nb_curve; i++) {
        ...
        if (Tol1 > MaxTol) MaxTol = Tol1;
        if (Tol2 > MaxTol) MaxTol = Tol2;
        ...
        if (ToReverse) tab_c3d(i - 1)->Reverse();
    ...
    GeomConvert::ConcatC1(tab_c3d, tabtolvertex, ArrayOfIndices, concatcurve, closed_flag,
    ...
    for (j = 1; j <= SurfSeq.Length(); j++) {
        ...
        for (i = 1; i <= nb_curve; i++) {
            ...
            Handle(Geom2d_Curve) aPCurve = BRep_Tool::CurveOnSurface(anEdge, SurfSeq(j), LocSeq(j), fpar, lpar);
            if (aPCurve.IsNull()) continue;
            ...
            if (ToReverse) tab_c2d(i - 1)->Reverse();
            PrevVertex = (ToReverse) ? VF : VL;
            PrevEdge = anEdge;
        }
        ...
        Geom2dConvert::ConcatC1(tab_c2d, tabtolvertex, ArrayOfInd2d, concatc2d, closed_flag,
                                Precision::Confusion()); // C1 concatenation
    ...
    ResEdge =
        BRepLib_MakeEdge(ResCurve, FirstVertex, LastVertex, ResCurve->FirstParameter(), ResCurve->LastParameter());
...
void ShapeUpgrade_UnifySameDomain::UnionPCurves(const TopTools_SequenceOfShape& theChain, TopoDS_Edge& theEdge) {
    ...
    for (; anItl.More(); anItl.Next()) {
        ...
        if (myFacePlaneMap.IsBound(aFace)) continue;

        if (myFaceNewFace.IsBound(aFace)) aFace = TopoDS::Face(myFaceNewFace(aFace));

        BRepAdaptor_Surface aBAsurf(aFace, Standard_False);
        if (aBAsurf.GetType() == GeomAbs_Plane) continue;
    ...
    for (Standard_Integer j = 1; j <= aFaceSeq.Length(); j++) {
        ...
        for (Standard_Integer i = 1; i <= theChain.Length(); i++) {
            ...
            Handle(Geom2d_Curve) aPCurve = BRep_Tool::CurveOnSurface(anEdge, TopoDS::Face(aFaceSeq(j)), aFirst, aLast);
            if (aPCurve.IsNull()) continue;
        ...
        if (aPCurveSeq.Length() == 1) {
            ...
            Geom2dConvert::ConcatC1(tab_c2d, tabtolvertex, ArrayOfInd2d, concatc2d, aClosedFlag,
                                    Precision::Confusion()); // C1 concatenation
    ...
    for (Standard_Integer ii = 1; ii <= ResPCurves.Length(); ii++) {
        ...
        if (Abs(aRange3d - aRange) > aMaxTol) IsBadRange = Standard_True;
    }
...
Standard_Boolean ShapeUpgrade_UnifySameDomain::MergeSubSeq(const TopTools_SequenceOfShape& theChain,
    ...
    for (j = 1; j < theChain.Length(); j++) {
        ...
        if (BRep_Tool::Degenerated(edge1) && BRep_Tool::Degenerated(edge2)) {
            ...
            if (!FindClosestPoints(edgeFirst, edgeLast, theVFmap, CommonFace, MinSqDist, OrOfE1OnFace, OrOfE2OnFace,
            ...
            if ((OrOfE1OnFace == TopAbs_FORWARD && IndOnE1 == 1) || (OrOfE1OnFace == TopAbs_REVERSED && IndOnE1 == 0)) {
        ...
        if (c3d1->IsKind(STANDARD_TYPE(Geom_Line)) && c3d2->IsKind(STANDARD_TYPE(Geom_Line))) {
            ...
            if (!Dir1.IsParallel(Dir2, myAngTol)) IsUnionOfLinesPossible = Standard_False;
        } else
        ...
        if (c3d1->IsKind(STANDARD_TYPE(Geom_Circle)) && c3d2->IsKind(STANDARD_TYPE(Geom_Circle))) {
            ...
            if (P01.Distance(P02) > Precision::Confusion()) IsUnionOfCirclesPossible = Standard_False;
        } else
            IsUnionOfCirclesPossible = Standard_False;
    }
    if (IsUnionOfLinesPossible && IsUnionOfCirclesPossible) return Standard_False;
    ...
    if (theChain.Length() > 1 && myConcatBSplines) {
        ...
        for (j = 1; j <= theChain.Length(); j++) {
            ...
            if ((c3d->IsKind(STANDARD_TYPE(Geom_BSplineCurve)) || c3d->IsKind(STANDARD_TYPE(Geom_BezierCurve))))
                continue;
...
static Standard_Boolean IsMergingPossible(const TopoDS_Edge& edge1, const TopoDS_Edge& edge2, double theAngTol,
                                          double theLinTol, const TopTools_MapOfShape& AvoidEdgeVrt,
                                          const bool theLineDirectionOk, const gp_Pnt& theFirstPoint,
    ...
    if (IsDegE1 && IsDegE2) {
        ...
        Standard_Integer IndOnE1, IndOnE2;
        gp_Pnt2d PointsOnEdge1[2], PointsOnEdge2[2];
        if (!FindClosestPoints(edge1, edge2, theVFmap, CommonFace, MinSqDist, OrOfE1OnFace, OrOfE2OnFace, IndOnE1,
                               IndOnE2, PointsOnEdge1, PointsOnEdge2))
            return Standard_False;

        if (MinSqDist <= Precision::SquareConfusion()) return Standard_True;
    ...
    if (CV.IsNull() || AvoidEdgeVrt.Contains(CV)) return Standard_False;
    ...
    if (t1 == GeomAbs_Circle && t2 == GeomAbs_Circle) {
        if (ade1.Circle().Location().Distance(ade2.Circle().Location()) > Precision::Confusion()) return Standard_False;
    }
    ...
    if (theLineDirectionOk && t2 == GeomAbs_Line) {
        ...
        if (theDirectionVec.Angle(aCurV) > theAngTol || Diff2.Angle(aCurV) > theAngTol) return Standard_False;
    }
...
// purpose  :
//=======================================================================
static Standard_Boolean GetLineEdgePoints(const TopoDS_Edge& theInpEdge, gp_Pnt& theFirstPoint,
                                          gp_Vec& theDirectionVec) {
    double f, l;
    Handle(Geom_Curve) aCur = BRep_Tool::Curve(theInpEdge, f, l);
    if (aCur.IsNull()) return Standard_False;

    Handle(Geom_TrimmedCurve) aTC = Handle(Geom_TrimmedCurve)::DownCast(aCur);
    if (!aTC.IsNull()) aCur = aTC->BasisCurve();

    if (aCur->DynamicType() != STANDARD_TYPE(Geom_Line)) return Standard_False;
...
void ShapeUpgrade_UnifySameDomain::generateSubSeq(const TopTools_SequenceOfShape& anInpEdgeSeq,
    ...
    for (int i = 1; i < anInpEdgeSeq.Length(); i++) {
        ...
        isOk = IsMergingPossible(edge1, edge2, theAngTol, theLinTol, AvoidEdgeVrt, isLineDirectionOk, aFirstPoint,
                                 aDirectionVec, theVFmap);
        if (!isOk) {
    ...
    if (IsClosed && SeqOfSubSeqOfEdges.Length() > 1) {
        ...
        if (IsMergingPossible(edge1, edge2, theAngTol, theLinTol, AvoidEdgeVrt, Standard_False, aFirstPoint,
                              aDirectionVec, theVFmap)) {
...
Standard_Boolean ShapeUpgrade_UnifySameDomain::MergeEdges(TopTools_SequenceOfShape& SeqEdges,
    ...
    for (j = 1; j <= aNbE; j++) {
        ...
        for (TopoDS_Iterator it(anEdge.Oriented(TopAbs_FORWARD)); it.More(); it.Next()) {
            ...
            if (aV.Orientation() == TopAbs_FORWARD || aV.Orientation() == TopAbs_REVERSED) {
                if (!aMapVE.Contains(aV)) aMapVE.Add(aV, TopTools_ListOfShape());
    ...
    for (Standard_Integer iE = 1; iE <= aNbE; ++iE) {
        ...
        if (!aUsedEdges.Add(edge)) continue;
        ...
        if (aChain.Length() < 2) continue;

        Standard_Boolean IsClosed = Standard_False;
        if (V[0].IsSame(V[1])) IsClosed = Standard_True;
    ...
    for (int i = 1; i <= SeqOfSubSeqOfEdges.Length(); i++) {
        TopoDS_Edge UE;
        if (SeqOfSubSeqOfEdges(i).SeqsEdges.Length() < 2) continue;
        if (MergeSubSeq(SeqOfSubSeqOfEdges(i).SeqsEdges, theVFmap, UE)) SeqOfSubSeqOfEdges(i).UnionEdges = UE;
    }
    return Standard_True;
}
...
Standard_Boolean ShapeUpgrade_UnifySameDomain::MergeSeq(TopTools_SequenceOfShape& SeqEdges,
    ...
    if (MergeEdges(SeqEdges, theVFmap, SeqOfSubsSeqOfEdges, nonMergVert)) {
        for (Standard_Integer i = 1; i <= SeqOfSubsSeqOfEdges.Length(); i++) {
            if (SeqOfSubsSeqOfEdges(i).UnionEdges.IsNull()) continue;
...
static void CheckSharedVertices(const TopTools_SequenceOfShape& theSeqEdges,
                                ...
                                const TopTools_MapOfShape& theMapKeepShape, TopTools_MapOfShape& theShareVertMap) {
    ShapeAnalysis_Edge sae;
    ...
    for (Standard_Integer k = 1; k <= theSeqEdges.Length(); k++) {
        ...
        if (!MapVertexes.Add(aV1)) SeqVertexes.Append(aV1);
        if (!MapVertexes.Add(aV2)) SeqVertexes.Append(aV2);
    }
    ...
    for (Standard_Integer k = 1; k <= SeqVertexes.Length() /* && !IsSharedVertexPresent*/; k++) {
        ...
        if (ListEdgesV1.Extent() > 2 || theMapKeepShape.Contains(SeqVertexes(k))) theShareVertMap.Add(SeqVertexes(k));
    }
...
ShapeUpgrade_UnifySameDomain::ShapeUpgrade_UnifySameDomain()
    : myLinTol(Precision::Confusion()), myAngTol(Precision::Angular()), myUnifyFaces(Standard_True),
      myUnifyEdges(Standard_True), myConcatBSplines(Standard_False), myAllowInternal(Standard_False),
      mySafeInputMode(Standard_True), myHistory(new BRepTools_History) {
...
ShapeUpgrade_UnifySameDomain::ShapeUpgrade_UnifySameDomain(const TopoDS_Shape& aShape,
    ...
    : myInitShape(aShape), myLinTol(Precision::Confusion()), myAngTol(Precision::Angular()), myUnifyFaces(UnifyFaces),
      myUnifyEdges(UnifyEdges), myConcatBSplines(ConcatBSplines), myAllowInternal(Standard_False),
      mySafeInputMode(Standard_True), myShape(aShape), myHistory(new BRepTools_History) {
    myContext = new ShapeBuild_ReShape;
}
...
void ShapeUpgrade_UnifySameDomain::Initialize(const TopoDS_Shape& aShape, const Standard_Boolean UnifyEdges,
                                              const Standard_Boolean UnifyFaces,
                                              const Standard_Boolean ConcatBSplines) {
    myInitShape = aShape;
    myShape = aShape;
...
void ShapeUpgrade_UnifySameDomain::KeepShape(const TopoDS_Shape& theShape) {
    if (theShape.ShapeType() == TopAbs_EDGE || theShape.ShapeType() == TopAbs_VERTEX) myKeepShapes.Add(theShape);
}
...
void ShapeUpgrade_UnifySameDomain::UnifyFaces() {
    ...
    for (; anExplo.More(); anExplo.Next()) {
        ...
        for (Standard_Integer ii = 1; ii <= aEFmap.Extent(); ii++) {
            ...
            if (!BRep_Tool::Degenerated(anEdge) && aFaceList.Extent() == 1) aFreeBoundMap.Add(anEdge);
        }
    }
    ...
    if (nbf > 0) IntUnifyFaces(aCmp, aGMapEdgeFaces, aFreeBoundMap);
...
void ShapeUpgrade_UnifySameDomain::IntUnifyFaces(const TopoDS_Shape& theInpShape,
    ...
    for (exp.Init(theInpShape, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        if (aProcessed.Contains(aFace)) continue;
        ...
        for (i = 1; i <= edges.Length(); i++) {
            ...
            for (; anIter.More(); anIter.Next()) {
                ...
                if (aCheckedFace.IsSame(aFace)) continue;

                if (aProcessed.Contains(aCheckedFace)) continue;
                            ...
                            for (; ex.More() && !hasConnectAnotherFaces; ex.Next()) {
                                ...
                                if (aLF.Extent() > 1) {
                                    for (it.Init(aLF); it.More() && !hasConnectAnotherFaces; it.Next()) {
                                        if (!anAvoidFaces.Contains(it.Value())) hasConnectAnotherFaces = Standard_True;
                                    }
                                }
                            }
        ...
        for (Standard_Integer ii = 1; ii <= edges.Length(); ii++) {
            ...
            if (anEdge.Orientation() != TopAbs_INTERNAL) edges(ii) = aMapEF.FindKey(indE);
        }
        ...
        if (faces.Length() > 1) {
            ...
            if (myConcatBSplines && !EdgeWith2pcurves.IsNull() && !SeamFound) {
                ...
                GeomAbs_Shape anOrderOfCont = BRepLib::ContinuityOfFaces(EdgeWith2pcurves, aFace1, aFace2, myAngTol);
            ...
            if (aIsEdgeWith2pcurvesSmooth) {
                ...
                if (anIsUclosed && Uperiod == 0.) aToMakeUPeriodic = Standard_True;
                if (!anIsUclosed && Vperiod == 0.) aToMakeVPeriodic = Standard_True;
                ...
                if (aToMakeUPeriodic || aToMakeVPeriodic) {
                    ...
                    if (aBSplineSurface.IsNull()) {
                        ...
                        GeomConvert_ApproxSurface Approximator(aBaseSurface, aTol, aUCont, aVCont, degU, degV, nmax,
                                                               aPrec);
            ...
            if (Uperiod != 0.) {
                ...
                if (!SeamFound) {
                    ...
                    RelocatePCurvesToNewUorigin(edges, faces(i_face_max), F_RefFace, CoordTol, Uperiod, VEmap,
                                                EdgeNewPCurve, UsedEdges);
                    ...
                    for (Standard_Integer ii = 1; ii <= edges.Length(); ii++) {
                        ...
                        UpdateBoundaries(aPCurve, aPCurve->FirstParameter(), aPCurve->LastParameter(), NewUmin,
                                         NewUmax);
                    }
                    ...
                    if (NewUmax - NewUmin < Uperiod - CoordTol &&
                        ...
                        if (NewUorigin < umin) NewUorigin = umin;
                        Handle(Geom_Surface) NewSurf;
                        if (NewUorigin == umin)
                            NewSurf = aBaseSurface;
                        else
                            NewSurf = new Geom_RectangularTrimmedSurface(aBaseSurface, NewUorigin, NewUorigin + Uperiod,
            ...
            if (Uperiod == 0 || Vperiod == 0) {
                ...
                if (Uperiod == 0 && aSurf->IsUClosed()) Uperiod = Ulast - Ufirst;
                if (Vperiod == 0 && aSurf->IsVClosed()) Vperiod = Vlast - Vfirst;
            }
            ...
            for (Standard_Integer ii = 1; ii <= edges.Length(); ii++) {
                ...
                if (aFirstPoint.X() < FaceUmin) FaceUmin = aFirstPoint.X();
                if (aLastPoint.X() < FaceUmin) FaceUmin = aLastPoint.X();

                if (aFirstPoint.Y() < FaceVmin) FaceVmin = aFirstPoint.Y();
                if (aLastPoint.Y() < FaceVmin) FaceVmin = aLastPoint.Y();
            }
            ...
            while (!edges.IsEmpty()) {
                ...
                for (;;) // loop till the end of current new wire
                {
                    ...
                    if (Elist.IsEmpty()) {
                        if (CurVertex.IsSame(StartVertex)) {
                                ...
                                Abs(StartPoint.X() - CurPoint.X()) > Uperiod / 2) // end of parametric space
                            {
                                ...
                                ReconstructMissedSeam(edges, RemovedEdges, UsedEdges, F_RefFace, CurVertex, CurPoint,
                                                      Uperiod, FaceUmin, Standard_True, CoordTol, NextEdge, aNewWire,
                                                      NextPoint, StartOfNextEdge, LastVertexOfSeam, VEmap);
                            } else if (Vperiod != 0. &&
                                       Abs(StartPoint.Y() - CurPoint.Y()) > Vperiod / 2) // end of parametric space
                            {
                                ...
                                TopoDS_Vertex LastVertexOfSeam;
                                ReconstructMissedSeam(edges, RemovedEdges, UsedEdges, F_RefFace, CurVertex, CurPoint,
                                                      Vperiod, FaceVmin, Standard_False, CoordTol, NextEdge, aNewWire,
                                                      NextPoint, StartOfNextEdge, LastVertexOfSeam, VEmap);
                            } else {
                                break; // end of wire
                            }
                    ...
                    if (NextEdge.IsNull()) {
                        ...
                        if (!anIsOnSingularity && Elist.Extent() > 1) SplittingVertices.Add(CurVertex);
                        ...
                        for (itl.Initialize(Elist); itl.More(); itl.Next()) {
                            const TopoDS_Edge& anEdge = TopoDS::Edge(itl.Value());
                            if (UsedEdges.Contains(anEdge)) continue;
                            TopoDS_Vertex aFirstVertex = TopExp::FirstVertex(anEdge, Standard_True);
                            if (!aFirstVertex.IsSame(CurVertex)) continue;
                        ...
                        if (TmpElist.Extent() <= 1 || (Uperiod != 0. || Vperiod != 0))
                            ...
                            CurDir.Normalize();
                            if (CurEdge.Orientation() == TopAbs_REVERSED) CurDir.Reverse();
                            for (itl.Initialize(TmpElist); itl.More(); itl.Next()) {
                                ...
                                aDir.Normalize();
                                if (anEdge.Orientation() == TopAbs_REVERSED) aDir.Reverse();
                    ...
                    if (NextEdge.IsNull()) {
                        // throw Standard_ConstructionError("Construction of unified wire failed: no proper edge
                        // found");
                        return;
                    } else {
                        CurPoint = NextPoint;
                ...
                for (;;) {
                    ...
                    for (Standard_Integer ii = 0; ii < 2; ii++) {
                        ...
                        for (; itl.More(); itl.Next()) {
                            ...
                            if (anEdge.IsSame(EndEdges[ii])) continue;
                            found = Standard_True;
            ...
            if (NewFaces.IsEmpty()) {
                ...
                for (Standard_Integer ii = 1; ii <= NewFaces.Length(); ii++) {
                    ...
                    for (; Explo.More(); Explo.Next()) {
                        const TopoDS_Edge& anEdge = TopoDS::Edge(Explo.Current());
                        if (BRep_Tool::Degenerated(anEdge) || BRep_Tool::IsClosed(anEdge, RefFace)) continue;
                        Standard_Integer jj;
                        for (jj = 1; jj <= Emaps.Length(); jj++)
                            if (Emaps(jj).Contains(anEdge)) break;
                        if (UsedFaces.Add(faces(jj))) facesForThisFace.Append(faces(jj));
                    }
...
void ShapeUpgrade_UnifySameDomain::UnifyEdges() {
    ...
    if (mySafeInputMode) UpdateMapOfShapes(myKeepShapes, myContext);
    ...
    for (Standard_Integer i = 1; i <= aChangedFaces.Extent(); i++) {
        ...
        if (mySafeInputMode) sff.SetContext(myContext);
    ...
    if (aChangedFaces.Extent() > 0) {
        ...
        if (isChanged) aRes1 = myContext->Apply(aRes1);
...
void ShapeUpgrade_UnifySameDomain::Build() {
    ...
    if (myUnifyFaces) UnifyFaces();
    if (myUnifyEdges) UnifyEdges();
...
void SplitWire(const TopoDS_Wire& theWire, const TopoDS_Face& theFace, const TopTools_IndexedMapOfShape& theVmap,
               TopTools_SequenceOfShape& theWireSeq) {
    ...
    for (; itw.More(); itw.Next()) {
        const TopoDS_Edge& anEdge = TopoDS::Edge(itw.Value());
        if (!aEmap.Add(anEdge)) continue;
        if (anEdge.Orientation() != TopAbs_FORWARD && anEdge.Orientation() != TopAbs_REVERSED) continue;
    ...
    for (Standard_Integer ii = 1; ii <= theVmap.Extent(); ii++) {
        ...
        while (anItl.More()) {
            ...
            for (;;) {
                ...
                if (aVertex.IsSame(anOrigin)) break;

                if (!aVEmap.IsBound(aVertex)) break;

                TopTools_ListOfShape& aElist = aVEmap(aVertex);
                if (aElist.Extent() == 0) break;
