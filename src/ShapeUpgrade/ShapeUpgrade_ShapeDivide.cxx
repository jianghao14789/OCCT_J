...
//    sln 29.11.01 Bug21: in method Perform(..) nullify location of compound's faces only if mode myConsiderLocation is
//    on
...
Standard_Boolean ShapeUpgrade_ShapeDivide::Perform(const Standard_Boolean newContext) {
    ...
    if (newContext || myContext.IsNull()) myContext = new ShapeBuild_ReShape;
    ...
    if (myShape.ShapeType() == TopAbs_COMPOUND) {
        ...
        for (TopoDS_Iterator it(savShape, Standard_False); it.More(); it.Next()) {
            ...
            if (myContext->ModeConsiderLocation()) myResult.Location(L);
...
void ShapeUpgrade_ShapeDivide::SendMsg(const TopoDS_Shape& shape, const Message_Msg& message,
