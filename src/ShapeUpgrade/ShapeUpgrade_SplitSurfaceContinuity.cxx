...
// purpose  :
//=======================================================================
ShapeUpgrade_SplitSurfaceContinuity::ShapeUpgrade_SplitSurfaceContinuity() : myCont(0) {
...
void ShapeUpgrade_SplitSurfaceContinuity::Compute(const Standard_Boolean Segment) {
    ...
    if (mySurface->Continuity() < myCriterion) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
    ...
    if (mySurface->IsKind(STANDARD_TYPE(Geom_RectangularTrimmedSurface))) {
        ...
        switch (myCriterion) {
            ...
            case GeomAbs_C2:
                ...
                break;
            case GeomAbs_C3: // if (ShapeUpgrade::Debug()) std::cout<<". this criterion is not suitable for a Offset
                             // Surface"<<std::endl;
#ifdef OCCT_DEBUG
    ...
    Standard_Integer UFirstInd = MyBSpline->FirstUKnotIndex() + 1, ULastInd = MyBSpline->LastUKnotIndex() - 1,
        ...
        for (Standard_Integer j = 2; j <= myUSplitValues->Length(); j++) {
            ...
            for (; iknot <= ULastInd; iknot++) {
                ...
                if (Continuity < myCont) {
                    ...
                    if (newMultiplicity >= 0) corrected = MyBSpline->RemoveUKnot(iknot, newMultiplicity, myTolerance);
        ...
        for (Standard_Integer j1 = 2; j1 <= myVSplitValues->Length(); j1++) {
            ...
            for (; iknot <= VLastInd; iknot++) {
                ...
                if (Continuity < myCont) {
                    ...
                    Standard_Boolean corrected = Standard_False;
                    if (newMultiplicity >= 0) corrected = MyBSpline->RemoveVKnot(iknot, newMultiplicity, myTolerance);
                    if (corrected && newMultiplicity > 0) {
