...
void ShapeUpgrade_SplitCurve3d::Init(const Handle(Geom_Curve) & C, const Standard_Real First,
    ...
    if (!ShapeAnalysis_Curve::IsPeriodic(C)) {
        ...
        if (Abs(firstPar - fP) < precision) firstPar = fP;
        if (Abs(lastPar - lP) < precision) lastPar = lP;
        if (firstPar < fP) {
#ifdef OCCT_DEBUG
        ...
        if ((lastPar - firstPar) < precision) lastPar = firstPar + 2 * precision;
    }
...
void ShapeUpgrade_SplitCurve3d::Build(const Standard_Boolean Segment) {
    ...
    if (mySplitValues->Length() > 2) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (myCurve->IsKind(STANDARD_TYPE(Geom_TrimmedCurve))) {
        ...
        if (myNbCurves == 1) {
            Handle(Geom_TrimmedCurve) NewTrimCurve = new Geom_TrimmedCurve(spc.GetCurves()->Value(1), First, Last);
        ...
        for (Standard_Integer i = 1; i <= myNbCurves; i++) {
            Handle(Geom_OffsetCurve) NewOffsetCurve = new Geom_OffsetCurve(spc.GetCurves()->Value(i), Offset, Direct);
    ...
    if (!ShapeAnalysis_Curve::IsPeriodic(myCurve)) {
        ...
        if (Abs(First - firstPar) < precision) First = firstPar;
        if (Abs(Last - lastPar) < precision) Last = lastPar;
        if (First < firstPar) {
#ifdef OCCT_DEBUG
            std::cout << "Warning: The range of the edge exceeds the curve domain" << std::endl;
#endif
    ...
    for (Standard_Integer i = 1; i <= myNbCurves; i++) {
        ...
        if (Segment) {
            if (myCurve->IsKind(STANDARD_TYPE(Geom_BSplineCurve)) || myCurve->IsKind(STANDARD_TYPE(Geom_BezierCurve))) {
