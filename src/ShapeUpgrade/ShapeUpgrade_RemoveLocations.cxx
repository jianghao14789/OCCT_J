...
// purpose  :
//=======================================================================
static Standard_Boolean RebuildShape(const TopoDS_Edge& theEdge, TopoDS_Edge& theNewEdge, const TopoDS_Face& theFace,
                                     TopoDS_Face& theNewFace, Standard_Boolean isBound) {
    Standard_Boolean isRebuild = Standard_False;
    BRep_Builder aB;
    if (!isBound) {
        ...
        if (BRep_Tool::Degenerated(theEdge)) aB.Degenerated(theNewEdge, Standard_True);
        isRebuild = Standard_True;
    }
    if (!theFace.IsNull()) {
        ...
        if (!aSurf->IsKind(STANDARD_TYPE(Geom_Plane))) {
            ...
            if (!c2d.IsNull() || !c2d1.IsNull()) aB.Range(theNewEdge, theNewFace, First2d, Last2d);
        }
    }
    return isRebuild;
}
...
Standard_Boolean ShapeUpgrade_RemoveLocations::MakeNewShape(const TopoDS_Shape& theShape,
                                                            const TopoDS_Shape& theAncShape, TopoDS_Shape& theNewShape,
    ...
    if (shtype == TopAbs_FACE) anAncShape = aShape;
    if (isRemoveLoc && (!aShape.Location().IsIdentity() || shtype == TopAbs_EDGE || shtype == TopAbs_FACE)) {
        ...
        if (shtype == TopAbs_FACE) {
            ...
            if (!anAncShape.IsNull()) {
                ...
                newFace = F;
                if (myMapNewShapes.IsBound(F)) newFace = TopoDS::Face(myMapNewShapes.Find(F));
            }
            if (isBound) anewEdge = TopoDS::Edge(aNewShape);
    ...
    if (!isBound) {
        ...
        if (!oldLoc.IsIdentity()) aNewShape.Location(nullloc);
        ...
        if (!theRemoveLoc && !oldLoc.IsIdentity()) aNewShape.Location(oldLoc);
    }
    theNewShape = aNewShape;
