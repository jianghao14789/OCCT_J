...
// purpose  :
//=======================================================================
ShapeUpgrade_ClosedFaceDivide::ShapeUpgrade_ClosedFaceDivide() : ShapeUpgrade_FaceDivide() {
    myNbSplit = 1;
}
...
ShapeUpgrade_ClosedFaceDivide::ShapeUpgrade_ClosedFaceDivide(const TopoDS_Face& F) : ShapeUpgrade_FaceDivide(F) {
    myNbSplit = 1;
}
...
Standard_Boolean ShapeUpgrade_ClosedFaceDivide::SplitSurface() {
    ...
    for (TopoDS_Iterator iter(face); iter.More() && !doSplit; iter.Next()) {
        ...
        for (Standard_Integer i = 1; i <= sewd->NbEdges() && !doSplit; i++)
            if (sewd->IsSeam(i)) {
                ...
                if (!sae.PCurve(edge, face, c1, f1, l1, Standard_False)) continue;
                // smh#8
                TopoDS_Shape tmpE = edge.Reversed();
                if (!sae.PCurve(TopoDS::Edge(tmpE), face, c2, f2, l2, Standard_False)) continue;
                if (c2 == c1) continue;
                // splitting
    ...
    if (!doSplit) return Standard_False;
    ...
    for (TopExp_Explorer exp(res, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        myResult = f;
        if (SplitSurface()) Context()->Replace(f, myResult);
    }
