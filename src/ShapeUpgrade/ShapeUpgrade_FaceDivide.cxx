...
// purpose  :
//=======================================================================
ShapeUpgrade_FaceDivide::ShapeUpgrade_FaceDivide() : ShapeUpgrade_Tool(), myStatus(0) {
...
ShapeUpgrade_FaceDivide::ShapeUpgrade_FaceDivide(const TopoDS_Face& F) : ShapeUpgrade_Tool(), myStatus(0) {
    mySegmentMode = Standard_True;
    mySplitSurfaceTool = new ShapeUpgrade_SplitSurface;
    myWireDivideTool = new ShapeUpgrade_WireDivide;
    Init(F);
...
Standard_Boolean ShapeUpgrade_FaceDivide::SplitSurface() {
    ...
    if (!SplitWire.IsNull()) CompShell.SetTransferParamTool(GetWireDivideTool()->GetTransferParamTool());
    CompShell.Perform();
...
Standard_Boolean ShapeUpgrade_FaceDivide::SplitCurves() {
    ...
    for (TopExp_Explorer explf(myResult, TopAbs_FACE); explf.More(); explf.Next()) {
        ...
        for (TopoDS_Iterator wi(F, Standard_False); wi.More(); wi.Next()) {
            ...
            SplitWire->Perform();
            if (SplitWire->Status(ShapeExtend_FAIL)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);
