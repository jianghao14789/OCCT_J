...
void ShapeUpgrade_SplitCurve2d::Init(const Handle(Geom2d_Curve) & C, const Standard_Real First,
...
void ShapeUpgrade_SplitCurve2d::Build(const Standard_Boolean Segment) {
    ...
    if (myCurve->IsKind(STANDARD_TYPE(Geom2d_TrimmedCurve))) {
        ...
        if (myNbCurves == 1) {
            Handle(Geom2d_TrimmedCurve) NewTrimCurve = new Geom2d_TrimmedCurve(spc.GetCurves()->Value(1), First, Last);
