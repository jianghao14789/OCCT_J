...
Geom2dGcc_Lin2d2Tan::Geom2dGcc_Lin2d2Tan(const Geom2dGcc_QualifiedCurve& Qualified1,
                                         const Geom2dGcc_QualifiedCurve& Qualified2, const Standard_Real Tolang)
    ...
    if (Type1 == GeomAbs_Circle && Type2 == GeomAbs_Circle) {
        ...
        for (i = 0; i <= aNbSamples1 && NbrSol < 4; i++) {
            ...
            for (j = 0; j <= aNbSamples2 && NbrSol < 4; j++) {
                ...
                if (Lin.IsDone()) {
                    if (Add(NbrSol + 1, Lin, Tolang, C1, C2)) NbrSol++;
                }
                Param2 += aStep2;
            }
            Param1 += aStep1;
...
Geom2dGcc_Lin2d2Tan::Geom2dGcc_Lin2d2Tan(const Geom2dGcc_QualifiedCurve& Qualified1, const gp_Pnt2d& ThePoint,
    ...
    if (Type1 == GeomAbs_Circle) {
        ...
        for (i = 0; i <= aNbSamples && NbrSol < 2; i++) {
            ...
            if (Lin.IsDone()) {
                if (Add(NbrSol + 1, Lin, Tolang, C1, Geom2dAdaptor_Curve())) NbrSol++;
            }
...
Geom2dGcc_Lin2d2Tan::Geom2dGcc_Lin2d2Tan(const Geom2dGcc_QualifiedCurve& Qualified1, const gp_Pnt2d& ThePoint,
                                         const Standard_Real Tolang, const Standard_Real Param1)
    : linsol(1, 2), qualifier1(1, 2), qualifier2(1, 2), pnttg1sol(1, 2), pnttg2sol(1, 2), par1sol(1, 2), par2sol(1, 2),
      pararg1(1, 2), pararg2(1, 2) {
    Geom2dAdaptor_Curve C1 = Qualified1.Qualified();
    Handle(Geom2d_Curve) CC1 = C1.Curve();
...
void Geom2dGcc_Lin2d2Tan::WhichQualifier(const Standard_Integer Index, GccEnt_Position& Qualif1,
...
void Geom2dGcc_Lin2d2Tan::Tangency1(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
...
void Geom2dGcc_Lin2d2Tan::Tangency2(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
                                    gp_Pnt2d& PntSol) const {
    if (!WellDone) {
        throw StdFail_NotDone();
    } else if (Index <= 0 || Index > NbrSol) {
...
Standard_Boolean Geom2dGcc_Lin2d2Tan::Add(const Standard_Integer theIndex, const Geom2dGcc_Lin2d2TanIter& theLin,
                                          const Standard_Real theTol, const Geom2dAdaptor_Curve& theC1,
    ...
    for (i = 1; i < theIndex; i++) {
        if (Abs(aPar1arg - pararg1(i)) <= theTol && Abs(aPar2arg - pararg2(i)) <= theTol) return Standard_False;
    }
    ...
    if (Abs(aLinDir.Crossed(gp_Dir2d(aVTan))) > theTol) return Standard_False;
