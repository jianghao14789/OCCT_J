...
//                      and the curve.                                    +
//=========================================================================
Geom2dGcc_FunctionTanCirCu::Geom2dGcc_FunctionTanCirCu(const gp_Circ2d& Circ, const Geom2dAdaptor_Curve& Curv) {
    Curve = Curv;
    TheCirc = Circ;
...
Standard_Boolean Geom2dGcc_FunctionTanCirCu::Value(const Standard_Real X, Standard_Real& Fval) {
    ...
    Fval = squaredir - R * R - (TheDirection.Dot(Vect1)) * (TheDirection.Dot(Vect1)) / (NormeD1 * NormeD1);
...
Standard_Boolean Geom2dGcc_FunctionTanCirCu::Derivative(const Standard_Real X, Standard_Real& Deriv) {
    ...
    Deriv = -2. * (cp1dott / NormeD1) * ((TheDirection.Dot(Vect2)) - cp1dott * Vect1.Dot(Vect2) / NormeD1);
...
Standard_Boolean Geom2dGcc_FunctionTanCirCu::Values(const Standard_Real X, Standard_Real& Fval, Standard_Real& Deriv) {
    gp_Pnt2d Point;
    gp_Vec2d Vect1, Vect2;
    Geom2dGcc_CurveTool::D2(Curve, X, Point, Vect1, Vect2);
    Standard_Real NormeD1 = Vect1.SquareMagnitude();
