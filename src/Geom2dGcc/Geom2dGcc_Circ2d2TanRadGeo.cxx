...
Geom2dGcc_Circ2d2TanRadGeo::Geom2dGcc_Circ2d2TanRadGeo(const GccEnt_QualifiedLin& Qualified1,
                                                       const Geom2dGcc_QCurve& Qualified2, const Standard_Real Radius,
    ...
    :
      ...
      cirsol(1, aNbSolMAX), qualifier1(1, aNbSolMAX), qualifier2(1, aNbSolMAX), TheSame1(1, aNbSolMAX),
      TheSame2(1, aNbSolMAX), pnttg1sol(1, aNbSolMAX), pnttg2sol(1, aNbSolMAX), par1sol(1, aNbSolMAX),
      par2sol(1, aNbSolMAX), pararg1(1, aNbSolMAX), pararg2(1, aNbSolMAX) {
...
Geom2dGcc_Circ2d2TanRadGeo::Geom2dGcc_Circ2d2TanRadGeo(const GccEnt_QualifiedCirc& Qualified1,
    ...
    if (Radius < 0.0) {
        ...
        for (Standard_Integer jcote1 = 1; jcote1 <= nbrcote1 && NbrSol < aNbSolMAX; jcote1++) {
            ...
            IntRes2d_Domain D1(ElCLib::Value(0., Circ), 0., Tol, ElCLib::Value(2. * M_PI, Circ), 2. * M_PI, Tol);
...
Geom2dGcc_Circ2d2TanRadGeo::Geom2dGcc_Circ2d2TanRadGeo(const Geom2dGcc_QCurve& Qualified1, const gp_Pnt2d& Point2,
                                                       const Standard_Real Radius, const Standard_Real Tolerance)
    :
    ...
    if (Radius < 0.0) {
        ...
        IntRes2d_Domain D1(ElCLib::Value(0., Circ), 0., Tol, ElCLib::Value(M_PI + M_PI, Circ), M_PI + M_PI, Tol);
        ...
        for (Standard_Integer jcote1 = 1; jcote1 <= nbrcote1 && NbrSol < aNbSolMAX; jcote1++) {
            ...
            IntRes2d_Domain D2(Cu2.Value(firstparam), firstparam, Tol, Cu2.Value(lastparam), lastparam, Tol);
...
//            be equal to zero).
//=======================================================================
static void PrecRoot(const Adaptor2d_OffsetCurve& theC1, const Adaptor2d_OffsetCurve& theC2, const Standard_Real theU0,
                     const Standard_Real theV0, Standard_Real& theUfinal, Standard_Real& theVfinal) {
    /*
    ...
    */
    ...
    do {
        ...
        if (IsEqual(aG1, 0.0) || IsEqual(aG2, 0.0)) { // Here we do not processing singular cases.
            break;
        }
...
Geom2dGcc_Circ2d2TanRadGeo::Geom2dGcc_Circ2d2TanRadGeo(const Geom2dGcc_QCurve& Qualified1,
    ...
    if (Radius < 0.0) {
        ...
        for (Standard_Integer jcote1 = 1; jcote1 <= nbrcote1; jcote1++) {
            ...
            IntRes2d_Domain D2C1(C1.Value(firstparam), firstparam, Tol, C1.Value(lastparam), lastparam, Tol);
#endif
            for (Standard_Integer jcote2 = 1; jcote2 <= nbrcote2 && NbrSol < aNbSolMAX; jcote2++) {
                Handle(Geom2dAdaptor_Curve) HCu2 = new Geom2dAdaptor_Curve(Cu2);
                // Adaptor2d_OffsetCurve C2(HCu2,cote2(jcote2));
                ...
                firstparam = Max(C2.FirstParameter(), thefirst);
                lastparam = Min(C2.LastParameter(), thelast);
                IntRes2d_Domain D2C2(C2.Value(firstparam), firstparam, Tol, C2.Value(lastparam), lastparam, Tol);
#endif
                Intp.Perform(C1, C2, Tol, Tol);
                if (Intp.IsDone()) {
                    if (!Intp.IsEmpty()) {
                        const Standard_Real aSQApproxTol = Precision::Approximation() * Precision::Approximation();
                        for (Standard_Integer i = 1; i <= Intp.NbPoints() && NbrSol < aNbSolMAX; i++) {
...
void Geom2dGcc_Circ2d2TanRadGeo::WhichQualifier(const Standard_Integer Index, GccEnt_Position& Qualif1,
...
void Geom2dGcc_Circ2d2TanRadGeo::Tangency1(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
...
void Geom2dGcc_Circ2d2TanRadGeo::Tangency2(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
                                           gp_Pnt2d& PntSol) const {
    if (!WellDone) {
        throw StdFail_NotDone();
    } else if (Index <= 0 || Index > NbrSol) {
