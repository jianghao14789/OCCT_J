...
public:
    ...
    //! Edit Mode
    Standard_EXPORT void SetValue(const Standard_Integer num, const Handle(Interface_TypedValue) & typval,
    ...
    Standard_EXPORT virtual Handle(IFSelect_EditForm)
        Form(const Standard_Boolean readonly, const Standard_Boolean undoable = Standard_True) const;
    ...
    Standard_EXPORT virtual Handle(TColStd_HSequenceOfHAsciiString)
    ...
    //! editor
    Standard_EXPORT virtual Standard_Boolean Load(const Handle(IFSelect_EditForm) & form,
    ...
    Standard_EXPORT virtual Standard_Boolean Update(const Handle(IFSelect_EditForm) & form, const Standard_Integer num,
    ...
    Standard_EXPORT virtual Standard_Boolean UpdateList(const Handle(IFSelect_EditForm) & form,
    ...
    //! And both <ent> and <model> may be Null, for a full static
    //! editor
    Standard_EXPORT virtual Standard_Boolean Apply(const Handle(IFSelect_EditForm) & form,
                                                   const Handle(Standard_Transient) & ent,
                                                   const Handle(Interface_InterfaceModel) & model) const = 0;
