...
IFSelect_SessionFile::IFSelect_SessionFile(const Handle(IFSelect_WorkSession) & WS, const Standard_CString filename) {
    ClearLines();
...
Standard_Integer IFSelect_SessionFile::ReadSession() {
    ...
    while (rubr) {
        ...
        if (ungen.IsEqual("ErrorHandle")) {
            ...
            if (theline.Value(2).IsEqual("0"))
                ...
                sout << "Lineno." << thenl << " : ErrorHandle Incorrect : " << theline.Value(2) << std::endl;
                continue;
            }
            continue;
        } else
            sout << "Lineno." << thenl << " : Unknown General Parameter : " << ungen << " , ignored" << std::endl;
    }
    ...
    for (Standard_Integer formod = 1; formod >= 0; formod--) {
        rubr = (theline.Length() == 1 && ((formod == 1 && theline.Value(1).IsEqual("!MODELMODIFIERS")) ||
...
Standard_Integer IFSelect_SessionFile::ReadEnd() {
    Message_Messenger::StreamBuffer sout = Message::SendInfo();
    if (theline.Length() != 2 || !theline.Value(1).IsEqual("!XSTEP") || !theline.Value(2).IsEqual("END")) {
...
void IFSelect_SessionFile::AddItem(const Handle(Standard_Transient) & item, const Standard_Boolean active) {
    Message_Messenger::StreamBuffer sout = Message::SendInfo();
    ...
    if (!item.IsNull()) {
        ...
        sout << "Lineno." << thenl << " -- Name : " << name << " : Item could not be defined" << std::endl;
...
void IFSelect_SessionFile::NewItem(const Standard_Integer ident, const Handle(Standard_Transient) & par) {
    char laligne[100];
...
Handle(Standard_Transient) IFSelect_SessionFile::ItemValue(const Standard_Integer num) const {
    ...
    if (!thenames.Find(name, id)) {
        sout << " -- Item Unknown in File : " << name << " lineno " << thenl << " param." << nm << std::endl;
        id = 0;
    }
