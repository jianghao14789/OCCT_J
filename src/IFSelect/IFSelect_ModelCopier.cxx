...
Standard_Boolean IFSelect_ModelCopier::NameFile(const Standard_Integer num, const TCollection_AsciiString& filename) {
    Standard_Integer nb = thefilenames.Length();
...
Interface_CheckIterator IFSelect_ModelCopier::Copying(IFSelect_ShareOutResult& eval,
    ...
    for (eval.Evaluate(); eval.More(); eval.Next()) {
        ...
        CopiedModel(G, WL, protocol, eval.PacketRoot(), filename, dispnum, numod, TC, model, curapp, checks);
...
Interface_CheckIterator IFSelect_ModelCopier::SendAll(const Standard_CString filename, const Interface_Graph& G,
                                                      const Handle(IFSelect_WorkLibrary) & WL,
                                                      const Handle(Interface_Protocol) & protocol) {
    Interface_CheckIterator checks;
    ...
    CopiedModel(G, WL, protocol, pipo, TCollection_AsciiString(filename), 0, 0, TC, newmod, applied, checks);
...
Interface_CheckIterator IFSelect_ModelCopier::SendSelected(const Standard_CString filename, const Interface_Graph& G,
                                                           const Handle(IFSelect_WorkLibrary) & WL,
                                                           const Handle(Interface_Protocol) & protocol,
    ...
    for (Standard_Integer ic = TC.LastCopiedAfter(0, ent1, ent2); ic > 0; ic = TC.LastCopiedAfter(ic, ent1, ent2)) {
        if (ic <= theremain->Upper()) theremain->SetValue(ic, theremain->Value(ic) + 1);
    }
...
void IFSelect_ModelCopier::CopiedModel(const Interface_Graph& G, const Handle(IFSelect_WorkLibrary) & WL,
                                       const Handle(Interface_Protocol) & protocol,
                                       const Interface_EntityIterator& tocopy, const TCollection_AsciiString& filename,
    ...
    for (i = 1; i <= nbmod; i++) {
        ...
        if (sel.IsNull())
            ...
            for (list.Start(); list.More(); list.Next()) {
                if (TC.Search(list.Value(), newent)) applied->AddNum(newmod->Number(newent));
            }
        }
    }
}

void IFSelect_ModelCopier::CopiedRemaining(const Interface_Graph& G, const Handle(IFSelect_WorkLibrary) & WL,
                                           Interface_CopyTool& TC, Handle(Interface_InterfaceModel) & newmod) {
    Handle(Interface_InterfaceModel) original = G.Model();
    //  Interface_CopyTool TC(original,protocol);
    newmod = original->NewEmptyModel();
    ...
    if (newmod->NbEntities() == 0)
        ...
        Handle(Standard_Transient) ent1, ent2;
        for (Standard_Integer ic = TC.LastCopiedAfter(0, ent1, ent2); ic > 0; ic = TC.LastCopiedAfter(ic, ent1, ent2)) {
            if (ic <= theremain->Upper()) theremain->SetValue(ic, 1);
        }
...
void IFSelect_ModelCopier::BeginSentFiles(const Handle(IFSelect_ShareOut) & sho, const Standard_Boolean record) {
...
void IFSelect_ModelCopier::AddSentFile(const Standard_CString filename) {
    if (!thesentfiles.IsNull()) thesentfiles->Append(new TCollection_HAsciiString(filename));
}
