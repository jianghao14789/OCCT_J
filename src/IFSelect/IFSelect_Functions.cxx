...
static void SplitFileName(const Standard_CString filename, TCollection_AsciiString& prefix,
                          TCollection_AsciiString& fileroot, TCollection_AsciiString& suffix) {
...
static IFSelect_ReturnStatus fun5(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    Handle(TColStd_HSequenceOfTransient) result = IFSelect_Functions::GiveList(WS, pilot->CommandPart(2));
    if (result.IsNull()) {
...
static IFSelect_ReturnStatus fun8(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    for (i = model->NextNumberForLabel(arg1, 0, exact); i != 0; i = model->NextNumberForLabel(arg1, i, exact)) {
        cnt++;
        sout << " **  Found n0/id:";
...
static IFSelect_ReturnStatus funsigncase(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (signcase.IsNull())
        ...
        if (caselist.IsNull())
            sout << "Signature " << arg1 << " : no predefined case, see command  count " << arg1 << std::endl;
        else {
...
static IFSelect_ReturnStatus fundumpent(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (argc < 2 || (argc == 2 && levmax < 0)) {
        ...
        if (levmax < 0)
            ...
            sout << "  + optional, dump level in [0 - " << levmax << "] , default = " << levdef << std::endl;
...
static IFSelect_ReturnStatus fun20c(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    Handle(TColStd_HSequenceOfTransient) result = IFSelect_Functions::GiveList(WS, pilot->CommandPart(1));
    if (result.IsNull()) return IFSelect_RetError;
...
static IFSelect_ReturnStatus fun56(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (disp->HasRootName()) sout << "-- Racine nom de fichier : " << disp->RootName()->ToCString() << std::endl;
    return IFSelect_RetVoid;
}
...
static IFSelect_ReturnStatus fun_evaladisp(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    Message_Messenger::StreamBuffer sout = Message::SendInfo();
    if (argc < 3) {
        sout << "evaladisp mode(=0-1-2-3) disp [givelist] :  Mode + Dispatch [+ GiveList]\n  If GiveList not given, "
                "computed from Selection of the Dispatch. Mode:\n"
             << "  0 brief  1 +forgotten ents  2 +duplicata  3 1+2" << std::endl
...
static IFSelect_ReturnStatus fun_writedisp(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (argc < 3) {
        sout << "writedisp filename disp [givelist] :  FileName + Dispatch [+ GiveList]\n  If GiveList not given, "
                "computed from Selection of the Dispatch.\n"
...
static IFSelect_ReturnStatus fun85(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    return pilot->RecordItem(new IFSelect_SelectSignature(new IFSelect_SignType, arg1, Standard_True));
}
...
static IFSelect_ReturnStatus fun89(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    return pilot->RecordItem(new IFSelect_SelectSignature(new IFSelect_SignType, arg1, Standard_False));
}
...
static IFSelect_ReturnStatus fun_editvalue(const Handle(IFSelect_SessionPilot) & pilot) {
    ...
    if (islist) {
        ...
        if (argval[0] == '.') {
            ...
            if (numset > 0)
                stated = listed->AddValue(new TCollection_HAsciiString(pilot->CommandPart(numarg + 1)), numset);
        }
...
Handle(TColStd_HSequenceOfTransient) IFSelect_Functions::GiveList(const Handle(IFSelect_WorkSession) & WS,
...
void IFSelect_Functions::Init() {
    ...
    IFSelect_Act::AddFunc("setapplied", "modif:Modifier [name:un item sinon sortie fichier]  : Applique un Modifier",
                          fun43);
    IFSelect_Act::AddFunc("resetapplied", "modif:Modifier  : Enleve un Modifier de la sortie fichier", fun44);
    IFSelect_Act::AddFunc("modifmove",
    ...
    IFSelect_Act::AddFunc("evaladisp", "mode=[0-3]  disp:Dispatch [givelist]  : Evaluates a Dispatch (on a GiveList)",
                          fun_evaladisp);
    IFSelect_Act::AddFunc("writedisp",
    ...
    IFSelect_Act::AddFunc("runcopy", "modif:ModelModifier [givelist] : Run <modif> via TransformStandard option Copy",
                          fun6465);
    IFSelect_Act::AddFunc(
        "runonthespot", "modif:ModelModifier [givelist] : Run <modif> via TransformStandard option OnTheSpot", fun6465);
