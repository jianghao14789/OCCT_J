...
static Standard_Integer multsign(const TCollection_AsciiString& signtext, TColStd_SequenceOfAsciiString& signlist,
...
Standard_Boolean IFSelect_SelectSignature::SortInGraph(const Standard_Integer, const Handle(Standard_Transient) & ent,
    ...
    for (Standard_Integer i = 1; i <= theexact; i++) {
        ...
        if (jmod == 0) {
            if (!thematcher.IsNull())
                ...
                quid = IFSelect_Signature::MatchValue(thecounter->ComputedSign(ent, G), signtext, (imod > 2));
        } else {
            if (!thematcher.IsNull())
...
Standard_Boolean IFSelect_SelectSignature::Sort(const Standard_Integer, const Handle(Standard_Transient) & /*ent*/,
