...
const TCollection_AsciiString& IFSelect_SessionPilot::Word(const Standard_Integer num) const {
    if (num < thenbwords) return thewords(num);
    return nulword;
}
...
IFSelect_ReturnStatus IFSelect_SessionPilot::Do(const Standard_Integer number,
    ...
    switch (number) {
        ...
        Standard_FALLTHROUGH case 0: { //        ****     HELP
            ...
            if (thenbwords <= 1) {
                ...
                for (Standard_Integer i = 1; i <= nb; i++) {
                    ...
                    if (IFSelect_Activator::Select(list->Value(i).ToCString(), num, actor)) {
                        if (IFSelect_Activator::Mode(list->Value(i).ToCString()) == 1) std::cout << "[xsnew name] ";
        ...
        case 3: { //        ****     COMMAND
            if (argc < 2) {
                std::cout << "Donner une option :\n"
                          ...
                          << "l : list recorded  c : clear  f nom : sauver dans fichier de nom" << std::endl;
                return IFSelect_RetVoid;
            }
            switch (arg1[0]) {
        ...
        case 6: { //        ****    XSNEW(variable)
            if (argc < 3) {
                ...
                //////      { std::cout<<"Command with name:"<<thewords(0)<<", already taken"<<std::endl; return
                ///IFSelect_RetFail; }
                RemoveWord(0);
                RemoveWord(0);
