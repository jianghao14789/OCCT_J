...
void IFSelect_WorkSession::SetModel(const Handle(Interface_InterfaceModel) & model,
    ...
    if (myModel != model) theloaded.Clear();
    myModel = model;
...
IFSelect_ReturnStatus IFSelect_WorkSession::ReadStream(const Standard_CString theName, std::istream& theIStream) {
    if (thelibrary.IsNull()) return IFSelect_RetVoid;
    if (theprotocol.IsNull()) return IFSelect_RetVoid;
    Handle(Interface_InterfaceModel) model;
    IFSelect_ReturnStatus status = IFSelect_RetVoid;
...
Standard_Integer IFSelect_WorkSession::NumberFromLabel(const Standard_CString val,
    ...
    for (i = myModel->NextNumberForLabel(val, after, exact); i != 0; i = myModel->NextNumberForLabel(val, i, exact)) {
        cnt++;
        if (num <= 0) num = i;
    }
    if (cnt == 1) return num;
...
Handle(Standard_Transient) IFSelect_WorkSession::NamedItem(const Standard_CString name) const {
    ...
    if (!thenames.Find(name, res)) res.Nullify();
    return res;
}
...
Handle(Standard_Transient) IFSelect_WorkSession::NamedItem(const Handle(TCollection_HAsciiString) & name) const {
    Handle(Standard_Transient) res;
...
Standard_Integer IFSelect_WorkSession::AddItem(const Handle(Standard_Transient) & item, const Standard_Boolean active) {
    if (item.IsNull()) return 0;
    Standard_Integer id = theitems.FindIndex(item);
    if (id > 0) {
...
Standard_Integer IFSelect_WorkSession::AddNamedItem(const Standard_CString name,
    ...
    if (name[0] != '\0') thenames.Bind(name, item);
...
Standard_Boolean IFSelect_WorkSession::SetActive(const Handle(Standard_Transient) & item, const Standard_Boolean mode) {
...
Standard_Boolean IFSelect_WorkSession::RemoveItem(const Handle(Standard_Transient) & item) {
    ...
    if (att->IsKind(STANDARD_TYPE(TCollection_HAsciiString))) {
        if (!thenames.UnBind(GetCasted(TCollection_HAsciiString, att)->ToCString())) return Standard_False;
    }
...
Handle(TColStd_HSequenceOfHAsciiString) IFSelect_WorkSession::ItemNames(const Handle(Standard_Type) & type) const {
    ...
    for (; IT.More(); IT.Next()) {
        if (IT.Value()->IsKind(type)) list->Append(new TCollection_HAsciiString(IT.Key()));
    }
    return list;
}
...
Handle(TColStd_HSequenceOfHAsciiString) IFSelect_WorkSession::ItemNamesForLabel(const Standard_CString label) const {
    Handle(TColStd_HSequenceOfHAsciiString) list = new TColStd_HSequenceOfHAsciiString();
...
Standard_Integer IFSelect_WorkSession::NextIdentForLabel(const Standard_CString label, const Standard_Integer id,
    ...
    for (Standard_Integer i = id + 1; i <= nb; i++) {
        ...
        switch (mode) {
            case 0:
                if (!strcmp(lab->ToCString(), label)) return i;
                break; // switch
            case 1:
                if (lab->Search(label) == 1) return i;
                break;
            case 2:
                if (lab->Search(label) > 0) return i;
                break;
            default:
...
IFSelect_SelectionIterator IFSelect_WorkSession::Sources(const Handle(IFSelect_Selection) & sel) const {
...
Handle(TColStd_HSequenceOfTransient) IFSelect_WorkSession::SelectionResult(const Handle(IFSelect_Selection) &
                                                                           sel) const {
...
Handle(TColStd_HSequenceOfTransient) IFSelect_WorkSession::SelectionResultFromList(
    const Handle(IFSelect_Selection) & sel, const Handle(TColStd_HSequenceOfTransient) & list) const {
...
Handle(IFSelect_Selection) IFSelect_WorkSession::ItemSelection(const Handle(Standard_Transient) & item) const {
...
Standard_Boolean IFSelect_WorkSession::ComputeCounterFromList(const Handle(IFSelect_SignCounter) & counter,
...
Handle(TColStd_HSequenceOfInteger) IFSelect_WorkSession::FinalModifierIdents(const Standard_Boolean formodel) const {
...
Standard_Boolean IFSelect_WorkSession::SetAppliedModifier(const Handle(IFSelect_GeneralModifier) & modif,
                                                          const Handle(Standard_Transient) & item) {
    if (ItemIdent(modif) == 0) return Standard_False;
...
Standard_Boolean IFSelect_WorkSession::ResetAppliedModifier(const Handle(IFSelect_GeneralModifier) & modif) {
    if (ItemIdent(modif) == 0) return Standard_False;
...
Handle(Standard_Transient) IFSelect_WorkSession::UsesAppliedModifier(const Handle(IFSelect_GeneralModifier) &
                                                                     modif) const {
    Handle(Standard_Transient) res;
...
Handle(TCollection_HAsciiString) IFSelect_WorkSession::FileRoot(const Handle(IFSelect_Dispatch) & disp) const {
...
Standard_CString IFSelect_WorkSession::GiveFileComplete(const Standard_CString file) const {
    ...
    if (!ext.IsNull()) {
        char val0 = '\0';
        if (ext->Length() > 0) val0 = ext->Value(1);
        for (i = nb; i > 0; i--)
    ...
    if (!pre.IsNull()) {
        char val1 = '\0';
        if (pre->Length() > 0) val1 = pre->Value(pre->Length());
        j = 0;
        for (i = nb; i > 0; i--)
...
Interface_EntityIterator IFSelect_WorkSession::SentList(const Standard_Integer newcount) const {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if ((stat > 0 && newcount < 0) || stat == newcount) iter.GetOneItem(G.Entity(i));
    }
    return iter;
}
...
Standard_Boolean IFSelect_WorkSession::SetRemaining(const IFSelect_RemainMode mode) {
    ...
    if (mode == IFSelect_RemainForget) {
        ...
        sout << " -- Maximum Sending Count (i.e. duplication in files) " << MaxSendingCount() << std::endl;
...
Standard_Integer IFSelect_WorkSession::NbSources(const Handle(IFSelect_Selection) & sel) const {
    ...
    if (sel->IsKind(STANDARD_TYPE(IFSelect_SelectExtract)) || sel->IsKind(STANDARD_TYPE(IFSelect_SelectDeduct)))
        return 1;
    if (sel->IsKind(STANDARD_TYPE(IFSelect_SelectControl))) return 2;
    if (sel->IsKind(STANDARD_TYPE(IFSelect_SelectCombine))) return GetCasted(IFSelect_SelectCombine, sel)->NbInputs();
    return 0;
}
...
Standard_Boolean IFSelect_WorkSession::IsReversedSelectExtract(const Handle(IFSelect_Selection) & sel) const {
...
Handle(IFSelect_Selection) IFSelect_WorkSession::NewSelectPointed(const Handle(TColStd_HSequenceOfTransient) & list,
                                                                  const Standard_CString name) {
...
Standard_Boolean IFSelect_WorkSession::SetSelectPointed(const Handle(IFSelect_Selection) & sel,
                                                        const Handle(TColStd_HSequenceOfTransient) & list,
                                                        const Standard_Integer mode) const {
...
static void IFSelect_QueryProp(Interface_IntList& list, TCollection_AsciiString& ana, const Standard_Integer num,
                               const int quoi) {
    list.SetNumber(num);
...
void IFSelect_WorkSession::SetParams(const NCollection_Vector<Handle(Standard_Transient)>& params,
    ...
    Handle(IFSelect_ParamEditor) editor = new IFSelect_ParamEditor(nbp + nbu + 50, "Parameter Editor");
    ...
    Handle(IFSelect_EditForm) paramsread =
        new IFSelect_EditForm(editor, listread, Standard_False, Standard_True, "Read(Transfer) Parameters");
    if (listread.Length() > 0) AddNamedItem("xst-params-read", paramsread);
    Handle(IFSelect_EditForm) paramswrite =
        new IFSelect_EditForm(editor, listwrite, Standard_False, Standard_True, "Write(Transfer) Parameters");
...
void IFSelect_WorkSession::TraceStatics(const Standard_Integer use, const Standard_Integer mode) const {
    Message_Messenger::StreamBuffer sout = Message::SendInfo();
    if (use > 0) {
...
void IFSelect_WorkSession::DumpShare() const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (sel.IsNull())
            ...
            sout << "   Final Selection : Name:" << Name(sel)->ToCString() << "  Label:" << sel->Label() << std::endl;
        else
    ...
    if (nbm > 0) sout << "  ***   " << nbm << " active Model Modifiers : see ListModifiers   ***" << std::endl;
    Standard_Integer nbf = theshareout->NbModifiers(Standard_False);
    if (nbf > 0) sout << "  ***   " << nbf << " active File  Modifiers : see ListModifiers   ***" << std::endl;
...
void IFSelect_WorkSession::ListItems(const Standard_CString lab) const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (!str.IsNull()) {
            if (label->Location(str, 1, label->Length()) == 0) continue;
        }
        sout << "#" << i;
        if (HasName(var))
...
Handle(TColStd_HSequenceOfTransient) IFSelect_WorkSession::GiveList(const Standard_CString first,
...
Handle(TColStd_HSequenceOfTransient) IFSelect_WorkSession::GiveListFromList(const Standard_CString selname,
...
Handle(TColStd_HSequenceOfTransient) IFSelect_WorkSession::GiveListCombined(
    const Handle(TColStd_HSequenceOfTransient) & l1, const Handle(TColStd_HSequenceOfTransient) & l2,
    const Standard_Integer mode) const {
    Handle(TColStd_HSequenceOfTransient) list;
...
void IFSelect_WorkSession::DumpEntity(const Handle(Standard_Transient) & ent, const Standard_Integer level,
    ...
    S << "        ********  Dumping Entity n0 " << num << " level:" << level << "  ********" << std::endl;
...
void IFSelect_WorkSession::TraceDumpEntity(const Handle(Standard_Transient) & ent, const Standard_Integer level) const {
    Message_Messenger::StreamBuffer sout = Message::SendInfo(); // should it be changed to SendTrace()?
...
void IFSelect_WorkSession::PrintEntityStatus(const Handle(Standard_Transient) & ent, Standard_OStream& S) {
    ...
    S << "    Category : " << CategoryName(ent) << "    Validity : " << ValidityName(ent) << std::endl;
...
void IFSelect_WorkSession::PrintCheckList(Standard_OStream& S, const Interface_CheckIterator& checklist,
    ...
    if (mode == IFSelect_ItemsByEntity)
        ...
        Handle(IFSelect_CheckCounter) counter = new IFSelect_CheckCounter(mode > 1 && mode != IFSelect_CountSummary);
...
void IFSelect_WorkSession::PrintSignatureList(Standard_OStream& S, const Handle(IFSelect_SignatureList) & signlist,
...
void IFSelect_WorkSession::EvaluateDispatch(const Handle(IFSelect_Dispatch) & disp, const Standard_Integer mode) const {
    Message_Messenger::StreamBuffer sout = Message::SendInfo();
    if (errhand) {
        errhand = Standard_False;
        try {
...
void IFSelect_WorkSession::ListEntities(const Interface_EntityIterator& iter, const Standard_Integer mmode,
