...
void IFSelect_Activator::Adding(const Handle(IFSelect_Activator) & actor, const Standard_Integer number,
                                const Standard_CString command, const Standard_Integer mode) {
#ifdef OCCT_DEBUG
    if (thedico.IsBound(command)) {
        std::cout << "****  XSTEP commands, name conflict on " << command << " first defined remains  ****"
                  << std::endl;
...
Standard_Boolean IFSelect_Activator::Select(const Standard_CString command, Standard_Integer& number,
...
Handle(TColStd_HSequenceOfAsciiString) IFSelect_Activator::Commands(const Standard_Integer mode,
    ...
    for (; iter.More(); iter.Next()) {
        if (!iter.Key().StartsWith(command)) continue;
        if (mode < 0) {
            DeclareAndCast(IFSelect_Activator, acti, theacts(iter.Value()));
            if (acti.IsNull()) continue;
            if (command[0] == '\0' || !strcmp(command, acti->Group())) list->Append(iter.Key());
        } else {
            num = iter.Value();
...
IFSelect_Activator::IFSelect_Activator() : thegroup("XSTEP") {}
