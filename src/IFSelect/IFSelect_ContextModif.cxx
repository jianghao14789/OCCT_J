...
// purpose  :
//=======================================================================
IFSelect_ContextModif::IFSelect_ContextModif(const Interface_Graph& graph, const Interface_CopyTool& TC,
                                             const Standard_CString filename)
    : thegraf(graph, Standard_False), thefile(filename), thelist(graph.Size(), ' ') {
    themap = TC.Control();
...
IFSelect_ContextModif::IFSelect_ContextModif(const Interface_Graph& graph, const Standard_CString filename)
    : thegraf(graph, Standard_False), thefile(filename), thelist(graph.Size(), ' ') {
    thesel = Standard_False;
    thecurr = thecurt = 0;
    Standard_Integer nb = thelist.Length();
...
void IFSelect_ContextModif::Trace(const Standard_CString mess) {
    ...
    if (ValueOriginal() == ValueResult())
        ...
        sout << "--  ContextModif. Entity in Original, n0 " << thecurr << " in Result, n0 " << thecurt << std::endl;
...
void IFSelect_ContextModif::AddWarning(const Handle(Standard_Transient) & start, const Standard_CString mess,
...
void IFSelect_ContextModif::AddFail(const Handle(Standard_Transient) & start, const Standard_CString mess,
                                    const Standard_CString orig) {
