...
IFSelect_ContextWrite::IFSelect_ContextWrite(const Handle(Interface_InterfaceModel) & model,
    ...
    : themodel(model), theproto(proto), thefile(filename), theapply(applieds), thenumod(0), thenbent(0), thecurr(0) {}
...
IFSelect_ContextWrite::IFSelect_ContextWrite(const Handle(Interface_HGraph) & hgraph,
                                             ...
                                             const Handle(IFSelect_AppliedModifiers) & applieds,
                                             const Standard_CString filename)
    : themodel(hgraph->Graph().Model()), theproto(proto), thefile(filename), theapply(applieds), thehgraf(hgraph),
      thenumod(0), thenbent(0), thecurr(0) {}
...
Handle(Standard_Transient) IFSelect_ContextWrite::Value() const {
    if (thecurr < 1 || thecurr > thenbent) throw Standard_NoSuchObject("IFSelect_ContextWrite:Value");
...
void IFSelect_ContextWrite::AddWarning(const Handle(Standard_Transient) & start, const Standard_CString mess,
...
void IFSelect_ContextWrite::AddFail(const Handle(Standard_Transient) & start, const Standard_CString mess,
                                    const Standard_CString orig) {
