...
IFSelect_ListEditor::IFSelect_ListEditor() : themax(0), thetouc(0) {}

IFSelect_ListEditor::IFSelect_ListEditor(const Handle(Interface_TypedValue) & def, const Standard_Integer max)
...
static Standard_Boolean CheckValue(const Handle(TCollection_HAsciiString) & val,
    ...
    if (pty == Interface_ParamIdent && !val.IsNull()) {
        if (modl->NextNumberForLabel(val->ToCString(), 0) <= 0) return Standard_False;
    }
...
Standard_Boolean IFSelect_ListEditor::LoadEdited(const Handle(TColStd_HSequenceOfHAsciiString) & list) {
...
Standard_Boolean IFSelect_ListEditor::Remove(const Standard_Integer num, const Standard_Integer howmany) {
    if (theedit.IsNull()) return Standard_False;
