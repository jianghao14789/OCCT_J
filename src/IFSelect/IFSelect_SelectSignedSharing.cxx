...
IFSelect_SelectSignedSharing::IFSelect_SelectSignedSharing(const Handle(IFSelect_Signature) & matcher,
                                                           ...
                                                           const Standard_Boolean exact, const Standard_Integer level)
    : IFSelect_SelectExplore(level), thematcher(matcher), thesigntext(signtext), theexact(exact) {}
...
Standard_Boolean IFSelect_SelectSignedSharing::Explore(const Standard_Integer level,
                                                       const Handle(Standard_Transient) & ent, const Interface_Graph& G,
    ...
    for (list.Start(); list.More(); list.Next()) {
        if (thematcher->Matches(list.Value(), G.Model(), thesigntext, theexact)) explored.AddItem(list.Value());
    }
    return Standard_True;
}
