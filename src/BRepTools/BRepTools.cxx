...
// purpose  :
//=======================================================================
void BRepTools::UVBounds(const TopoDS_Face& F, Standard_Real& UMin, Standard_Real& UMax, Standard_Real& VMin,
                         Standard_Real& VMax) {
    Bnd_Box2d B;
    AddUVBounds(F, B);
    if (!B.IsVoid()) {
...
void BRepTools::UVBounds(const TopoDS_Face& F, const TopoDS_Wire& W, Standard_Real& UMin, Standard_Real& UMax,
...
void BRepTools::UVBounds(const TopoDS_Face& F, const TopoDS_Edge& E, Standard_Real& UMin, Standard_Real& UMax,
                         Standard_Real& VMin, Standard_Real& VMax) {
    Bnd_Box2d B;
    AddUVBounds(F, E, B);
    if (!B.IsVoid()) {
...
void BRepTools::AddUVBounds(const TopoDS_Face& aF, const TopoDS_Edge& aE, Bnd_Box2d& aB) {
    ...
    if (aS->DynamicType() == STANDARD_TYPE(Geom_RectangularTrimmedSurface)) {
        const Handle(Geom_RectangularTrimmedSurface) aSt = Handle(Geom_RectangularTrimmedSurface)::DownCast(aS);
        aS = aSt->BasisSurface();
    }
    ...
    if (expw.More()) {
        ...
        if (expw.More()) {
            ...
            while (expw.More()) {
                ...
                if (((umin - UMin) <= Precision::PConfusion()) && ((umax - UMax) >= -Precision::PConfusion()) &&
                    ((vmin - VMin) <= Precision::PConfusion()) && ((vmax - VMax) >= -Precision::PConfusion())) {
                    Wres = W;
                    UMin = umin;
                    UMax = umax;
                    VMin = vmin;
...
void BRepTools::Map3DEdges(const TopoDS_Shape& S, TopTools_IndexedMapOfShape& M) {
    ...
    for (Ex.Init(S, TopAbs_EDGE); Ex.More(); Ex.Next()) {
        if (!BRep_Tool::Degenerated(TopoDS::Edge(Ex.Current()))) M.Add(Ex.Current());
    }
}
...
// purpose  :
//=======================================================================
void BRepTools::Write(const TopoDS_Shape& theShape, Standard_OStream& theStream,
                      const Standard_Boolean theWithTriangles, const Standard_Boolean theWithNormals,
                      const TopTools_FormatVersion theVersion, const Message_ProgressRange& theProgress) {
...
void BRepTools::Read(TopoDS_Shape& Sh, std::istream& S, const BRep_Builder& B,
...
// purpose  :
//=======================================================================
Standard_Boolean BRepTools::Write(const TopoDS_Shape& theShape, const Standard_CString theFile,
                                  const Standard_Boolean theWithTriangles, const Standard_Boolean theWithNormals,
                                  const TopTools_FormatVersion theVersion, const Message_ProgressRange& theProgress) {
...
Standard_Boolean BRepTools::Read(TopoDS_Shape& Sh, const Standard_CString File, const BRep_Builder& B,
                                 const Message_ProgressRange& theProgress) {
    const Handle(OSD_FileSystem) & aFileSystem = OSD_FileSystem::DefaultFileSystem();
...
void BRepTools::CleanGeometry(const TopoDS_Shape& theShape) {
    ...
    for (TopExp_Explorer aFaceIt(theShape, TopAbs_FACE); aFaceIt.More(); aFaceIt.Next()) {
        ...
        for (TopExp_Explorer aEdgeIt(aFace, TopAbs_EDGE); aEdgeIt.More(); aEdgeIt.Next()) {
            const TopoDS_Edge& anEdge = TopoDS::Edge(aEdgeIt.Current());
            aBuilder.UpdateEdge(anEdge, Handle(Geom2d_Curve)(), aSurface, aLocation, BRep_Tool::Tolerance(anEdge));
        }
    ...
    for (TopExp_Explorer aEdgeIt2(theShape, TopAbs_EDGE); aEdgeIt2.More(); aEdgeIt2.Next()) {
        ...
        aBuilder.UpdateEdge(anEdge, Handle(Geom_Curve)(), TopLoc_Location(), BRep_Tool::Tolerance(anEdge));
    }
}
    ...
    for (i = 1; i <= Emap.Extent(); i++) {
        ...
        while (itrep.More()) {
            ...
            if (CurveRep->IsCurveOnSurface()) {
                ...
                if (!UsedSurfaces.Contains(aSurface)) ToRemove = Standard_True;
...
Standard_Boolean BRepTools::Triangulation(const TopoDS_Shape& theShape, const Standard_Real theLinDefl,
...
// purpose  :
//=======================================================================
Standard_Boolean BRepTools::LoadTriangulation(const TopoDS_Shape& theShape, const Standard_Integer theTriangulationIdx,
...
Standard_Boolean BRepTools::LoadAllTriangulations(const TopoDS_Shape& theShape,
    ...
    for (TopExp_Explorer aFaceIter(theShape, TopAbs_FACE); aFaceIter.More(); aFaceIter.Next()) {
        const TopoDS_Face& aFace = TopoDS::Face(aFaceIter.Current());
        for (Poly_ListOfTriangulation::Iterator anIter(BRep_Tool::Triangulations(aFace, aDummyLoc)); anIter.More();
             anIter.Next()) {
...
void BRepTools::DetectClosedness(const TopoDS_Face& theFace, Standard_Boolean& theUclosed,
...
Standard_Real BRepTools::EvalAndUpdateTol(const TopoDS_Edge& theE, const Handle(Geom_Curve) & C3d,
                                          const Handle(Geom2d_Curve) C2d, const Handle(Geom_Surface) & S,
    ...
    if (CT.IsDone()) {
        ...
        if (CT.ErrorStatus() == 3 || (CT.ErrorStatus() == 2 && (C3d->IsPeriodic() || C2d->IsPeriodic()))) {
...
//=======================================================================
static void findInternalsToKeep(const TopoDS_Shape& theS, TopTools_MapOfShape& theAllNonInternals,
                                TopTools_MapOfShape& theAllInternals, TopTools_MapOfShape& theShapesToKeep) {
    for (TopoDS_Iterator it(theS, Standard_True); it.More(); it.Next()) {
        ...
        if (theAllNonInternals.Contains(aSS) && theAllInternals.Contains(aSS)) theShapesToKeep.Add(aSS);
    }
}
...
//=======================================================================
static Standard_Boolean removeInternals(TopoDS_Shape& theS, const TopTools_MapOfShape* theShapesToKeep) {
    TopTools_ListOfShape aLRemove;
    for (TopoDS_Iterator it(theS, Standard_True); it.More(); it.Next()) {
        const TopoDS_Shape& aSS = it.Value();
        if (aSS.Orientation() == TopAbs_INTERNAL) {
            if (!theShapesToKeep || !theShapesToKeep->Contains(aSS)) aLRemove.Append(aSS);
        } else {
            if (removeInternals(*(TopoDS_Shape*)&aSS, theShapesToKeep)) aLRemove.Append(aSS);
        }
    }
...
void BRepTools::RemoveInternals(TopoDS_Shape& theS, const Standard_Boolean theForce) {
    ...
    if (!theForce) {
        ...
        if (aMKeep.Extent()) pMKeep = &aMKeep;
    }
