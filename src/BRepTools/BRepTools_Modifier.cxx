...
BRepTools_Modifier::BRepTools_Modifier(const TopoDS_Shape& S, const Handle(BRepTools_Modification) & M)
    : myShape(S), myDone(Standard_False), myMutableInput(Standard_False) {
    Put(S);
    Perform(M);
}
...
void BRepTools_Modifier::Perform(const Handle(BRepTools_Modification) & M, const Message_ProgressRange& theProgress) {
    if (myShape.IsNull()) {
    ...
    if (!myMutableInput) CreateOtherVertices(aMVE, aMEF, M);
...
Standard_Boolean BRepTools_Modifier::Rebuild(const TopoDS_Shape& S, const Handle(BRepTools_Modification) & M,
                                             Standard_Boolean& theNewGeom, const Message_ProgressRange& theProgress) {
#ifdef DEBUG_Modifier
    ...
    switch (ts) {
        case TopAbs_FACE: {
            ...
            if (rebuild) {
                ...
                B.MakeFace(TopoDS::Face(result), aNSinfo.mySurface, aNSinfo.myLoc.Predivided(S.Location()),
                           aNSinfo.myToler);
                result.Location(S.Location(), Standard_False);
                if (aNSinfo.myRevFace) ResOr = TopAbs_REVERSED;
            ...
            if (M->NewTriangulation(TopoDS::Face(S), aTriangulation)) {
                ...
                else { // create new face with bare triangulation
        ...
        case TopAbs_EDGE: {
            ...
            if (rebuild) {
                ...
                if (aNCinfo.myCurve.IsNull()) {
                    ...
                    B.MakeEdge(TopoDS::Edge(result), aNCinfo.myCurve, aNCinfo.myLoc.Predivided(S.Location()),
                               aNCinfo.myToler);
            ...
            if (M->NewPolygon(TopoDS::Edge(S), aPolygon)) {
                ...
                else { // create new edge with bare polygon
    ...
    if (theNewGeom) myHasNewGeom.Add(S);
                ...
                if (theNewGeom &&
                    ...
                    if (BRep_Tool::IsClosed(edge, face)) {
                        ...
                        if (!isClosed) {
                            ...
                            if (resface.IsNull()) resface = face;
                            ...
                            for (; aExpF.More() && !isClosed; aExpF.Next()) {
                                ...
                                if (anOther.IsSame(face)) continue;
                                TopoDS_Shape resface2 = (myMap.IsBound(anOther) ? myMap(anOther) : anOther);
                                if (resface2.IsNull()) resface2 = anOther;
                    ...
                    if (isClosed) {
                        ...
                        Standard_Real f, l;
                        if ((!RevWires && fcor != edge.Orientation()) || (RevWires && fcor == edge.Orientation())) {
    ...
    for (int i = 1; i <= theMVE.Extent(); i++) {
        ...
        if (aNewV.IsNull()) {
            ...
            for (; it.More() && !toReplace; it.Next()) {
                ...
                if (myNCInfo.IsBound(anE) && !myNCInfo(anE).myCurve.IsNull()) toReplace = Standard_True;
