...
//=======================================================================
static Standard_Boolean SelectDouble(TopTools_MapOfShape& Doubles, TopTools_ListOfShape& L, TopoDS_Edge& E);

static Standard_Boolean SelectDegenerated(TopTools_ListOfShape& L, TopoDS_Edge& E);

static Standard_Real GetNextParamOnPC(const Handle(Geom2d_Curve) & aPC, const gp_Pnt2d& aPRef, const Standard_Real& fP,
                                      const Standard_Real& lP, const Standard_Real& tolU, const Standard_Real& tolV,
...
// purpose  :
//=======================================================================
BRepTools_WireExplorer::BRepTools_WireExplorer() : myReverse(Standard_False), myTolU(0.0), myTolV(0.0) {}
...
void BRepTools_WireExplorer::Init(const TopoDS_Wire& W, const TopoDS_Face& F) {
    ...
    if (!F.IsNull()) {
        ...
        if (aSurfType == GeomAbs_Cone || aSurfType == GeomAbs_BSplineSurface || aSurfType == GeomAbs_BezierSurface) {
...
// purpose  :
//=======================================================================
void BRepTools_WireExplorer::Init(const TopoDS_Wire& W, const TopoDS_Face& F, const Standard_Real UMin,
    ...
    while (it.More()) {
        ...
        if (!V1.IsNull()) {
            if (!myMap.IsBound(V1)) myMap.Bind(V1, empty);
            myMap(V1).Append(E);
        ...
        if (V1.IsNull() || V2.IsNull()) {
            ...
            if (Eori == TopAbs_FORWARD) {
                if (aF == -Precision::Infinite()) anInfEmap.Add(E);
            } else { // Eori == TopAbs_REVERSED
                if (aL == Precision::Infinite()) anInfEmap.Add(E);
            }
        }
        it.Next();
    }
    ...
    while (it2.More()) {
        if (!emap.Add(it2.Value())) myDoubles.Add(it2.Value());
        it2.Next();
    }
...
TopAbs_Orientation BRepTools_WireExplorer::Orientation() const {
    ...
    while (it.More()) {
        if (myVertex.IsSame(it.Value())) return it.Value().Orientation();
        it.Next();
    }
...
// purpose  :
//=======================================================================
Standard_Real GetNextParamOnPC(const Handle(Geom2d_Curve) & aPC, const gp_Pnt2d& aPRef, const Standard_Real& fP,
                               const Standard_Real& lP, const Standard_Real& tolU, const Standard_Real& tolV,
                               const Standard_Boolean& reverse) {
    ...
    if (reverse) {
        ...
        if (!nextPntOnEdge) result = lP;

        if (result > lP) result = lP;
    } else {
