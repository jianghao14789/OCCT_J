...
public: //! @name Constructors for History creation
    ...
    template <class TheAlgo> BRepTools_History(const TopTools_ListOfShape& theArguments, TheAlgo& theAlgo) {
        ...
        for (; aIt.More(); aIt.Next()) {
            if (!aIt.Value().IsNull()) TopExp::MapShapes(aIt.Value(), anArgsMap);
        }
        ...
        for (i = 1; i <= aNb; ++i) {
            ...
            if (!IsSupportedType(aS)) continue;

            if (theAlgo.IsDeleted(aS)) Remove(aS);
...
public:
    ...
    static Standard_Boolean IsSupportedType(const TopoDS_Shape& theShape) {
        ...
        return aType == TopAbs_VERTEX || aType == TopAbs_EDGE || aType == TopAbs_FACE || aType == TopAbs_SOLID;
    }

public: //! Methods to set the history.
    //! Set the second shape as generated one from the first shape.
    Standard_EXPORT void AddGenerated(const TopoDS_Shape& theInitial, const TopoDS_Shape& theGenerated);
    ...
    Standard_EXPORT void ReplaceGenerated(const TopoDS_Shape& theInitial, const TopoDS_Shape& theGenerated);

    //! Set the second shape as the only modified one from the first one.
    Standard_EXPORT void ReplaceModified(const TopoDS_Shape& theInitial, const TopoDS_Shape& theModified);
...
public: //! A method to merge a next history to this history.
    ...
    //! @param theArguments [in] Arguments of the algorithm;
    //! @param theAlgo [in] The algorithm.
    template <class TheAlgo> void Merge(const TopTools_ListOfShape& theArguments, TheAlgo& theAlgo) {
...
private:
    ...
    Standard_Boolean prepareGenerated(const TopoDS_Shape& theInitial, const TopoDS_Shape& theGenerated);
