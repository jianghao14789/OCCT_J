...
template <typename TMap> void Add(TMap& theMap, const TopoDS_Shape& theShape) {
...
static void CopyRanges(const TopoDS_Shape& toedge, const TopoDS_Shape& fromedge, const Standard_Real alpha,
...
BRepTools_ReShape::BRepTools_ReShape() : myStatus(-1) {
...
void BRepTools_ReShape::replace(const TopoDS_Shape& ashape, const TopoDS_Shape& anewshape,
    ...
    // protect against INTERNAL or EXTERNAL shape
        ...
        newshape.Orientation((newshape.Orientation() == shape.Orientation()) ? TopAbs_FORWARD : TopAbs_REVERSED);
...
Standard_Integer BRepTools_ReShape::Status(const TopoDS_Shape& ashape, TopoDS_Shape& newsh,
    ...
    if (res > 0) {
        if (newsh.IsNull())
            ...
            res = 0;
                 ...
                 ((myConsiderLocation && !newsh.IsPartner(shape)) || (!myConsiderLocation && !newsh.IsSame(shape)))) {
    ...
    if (myConsiderLocation && !newsh.IsNull()) {
        TopLoc_Location aResLoc = (res > 0 && !newsh.Location().IsIdentity() ? aLocSh * newsh.Location() : aLocSh);
...
TopoDS_Shape BRepTools_ReShape::Apply(const TopoDS_Shape& shape, const TopAbs_ShapeEnum until) {
    ...
    if ((myConsiderLocation && !newsh.IsPartner(shape)) || (!myConsiderLocation && !newsh.IsSame(shape))) {
    ...
    if (st == TopAbs_VERTEX || st == TopAbs_SHAPE) return shape;
    ...
    */
    ...
    for (TopoDS_Iterator it(shape, Standard_False); it.More(); it.Next()) {
        ...
        if (isEmpty) isEmpty = Standard_False;
        locStatus |= EncodeStatus(3); // ShapeExtend::EncodeStatus ( ShapeExtend_DONE3 );
        if (st == TopAbs_COMPOUND ||
            newsh.ShapeType() ==
                sh.ShapeType()) { // fix for SAMTECH bug OCC322 about abcense internal vertices after sewing.
            B.Add(result, newsh);
            continue;
        }
        ...
        for (TopoDS_Iterator subit(newsh); subit.More(); subit.Next(), nitems++) {
            ...
            if (subsh.ShapeType() == sh.ShapeType())
                B.Add(result, subsh); // fix for SAMTECH bug OCC322 about abcense internal vertices after sewing.
            else
                locStatus |= EncodeStatus(10); // ShapeExtend::EncodeStatus ( ShapeExtend_FAIL1 );
        }
...
TopoDS_Vertex BRepTools_ReShape::CopyVertex(const TopoDS_Vertex& theV, const gp_Pnt& theNewPos,
    ...
    if (!isRecorded) Replace(theV, aVertexCopy);
