...
// purpose  :
//=======================================================================
BRepTools_ShapeSet::BRepTools_ShapeSet(const Standard_Boolean theWithTriangles, const Standard_Boolean theWithNormals)
    : myWithTriangles(theWithTriangles), myWithNormals(theWithNormals) {}

//=======================================================================
// function : BRepTools_ShapeSet
// purpose  :
//=======================================================================
BRepTools_ShapeSet::BRepTools_ShapeSet(const BRep_Builder& theBuilder, const Standard_Boolean theWithTriangles,
                                       const Standard_Boolean theWithNormals)
    : myBuilder(theBuilder), myWithTriangles(theWithTriangles), myWithNormals(theWithNormals) {}
    ...
    if (S.ShapeType() == TopAbs_VERTEX) {
        ...
        while (itrc.More()) {
            ...
            if (CR->IsCurve3D()) {
                ...
                ChangeLocations().Add(CR->Location());
                if (CR->IsCurveOnClosedSurface()) myCurves2d.Add(CR->PCurve2());
                ...
                if (CR->IsPolygon3D()) {
                    ...
                    myTriangulations.Add(CR->Triangulation(),
                                         Standard_False); // edge triangulation does not need normals
                    myNodes.Add(CR->PolygonOnTriangulation());
                    ChangeLocations().Add(CR->Location());
                    if (CR->IsPolygonOnClosedTriangulation()) myNodes.Add(CR->PolygonOnTriangulation2());
                } else if (CR->IsPolygonOnSurface()) {
                    mySurfaces.Add(CR->Surface());
                    myPolygons2D.Add(CR->Polygon());
                    ChangeLocations().Add(CR->Location());
                    if (CR->IsPolygonOnClosedSurface()) myPolygons2D.Add(CR->Polygon2());
                }
            }
            itrc.Next();
        }
...
void BRepTools_ShapeSet::DumpGeometry(const TopoDS_Shape& S, Standard_OStream& OS) const {
    ...
    if (S.ShapeType() == TopAbs_VERTEX) {
        ...
        while (itrp.More()) {
            ...
            if (!PR->Location().IsIdentity()) OS << " location " << Locations().Index(PR->Location());
            OS << "\n";
        ...
        while (itrc.More()) {
            ...
            if (CR->IsCurve3D()) {
                ...
                if (!CR->Curve3D().IsNull()) {
                    ...
                    if (!CR->Location().IsIdentity()) OS << " location " << Locations().Index(CR->Location());
                    OS << ", range : " << first << " " << last << "\n";
                }
            } else if (CR->IsCurveOnSurface()) {
                Handle(BRep_GCurve) GC = Handle(BRep_GCurve)::DownCast(itrc.Value());
                ...
                if (!CR->Location2().IsIdentity()) OS << " location " << Locations().Index(CR->Location2());
                OS << "\n";
        ...
        if (BRep_Tool::NaturalRestriction(F)) OS << "NaturalRestriction\n";
        ...
        if (!TF->Surface().IsNull()) {
            ...
            if (!S.Location().IsIdentity()) OS << " location " << Locations().Index(S.Location());
            OS << "\n";
        }
        if (!(TF->Triangulation()).IsNull()) {
            // Dump the triangulation
            OS << "    - Triangulation : " << myTriangulations.FindIndex(TF->Triangulation());
            if (!S.Location().IsIdentity()) OS << " location " << Locations().Index(TF->Location());
            OS << "\n";
        }
    }
...
void BRepTools_ShapeSet::ReadGeometry(const TopAbs_ShapeEnum T, Standard_IStream& IS, TopoDS_Shape& S) {
    // Read the geometry
            ...
            do {
                ...
                switch (val) {
                    ...
                    case 1: {
                        ...
                        if (myCurves.Curve(c).IsNull()) break;
                    ...
                    case 2: {
                        ...
                        //  Modified by Sergey KHROMOV - Wed Apr 24 13:59:09 2002 Begin
                        if (myCurves2d.Curve2d(pc).IsNull() || mySurfaces.Surface(s).IsNull()) break;
                        //  Modified by Sergey KHROMOV - Wed Apr 24 13:59:13 2002 End
                    ...
                    case 3: {
                        ...
                        //  Modified by Sergey KHROMOV - Wed Apr 24 13:59:09 2002 Begin
                        if (mySurfaces.Surface(s).IsNull()) break;
                        //  Modified by Sergey KHROMOV - Wed Apr 24 13:59:13 2002 End

                        Handle(BRep_PointOnSurface) POC = new BRep_PointOnSurface(p1, p2, mySurfaces.Surface(s), L);
                        PR = POC;
                    } break;
                }
                ...
                do {
                    ...
                    switch (val) {
                        ...
                        case 3: // -3- Curve on closed surf
                            ...
                            if (closed) {
                                if (FormatNb() == TopTools_FormatVersion_VERSION_2)
                                    myBuilder.UpdateEdge(E, myCurves2d.Curve2d(pc), myCurves2d.Curve2d(pc2),
                                                         mySurfaces.Surface(s), Locations().Location(l), tol, aPf, aPl);
                                else
                                    myBuilder.UpdateEdge(E, myCurves2d.Curve2d(pc), myCurves2d.Curve2d(pc2),
                                                         mySurfaces.Surface(s), Locations().Location(l), tol);

                                myBuilder.Continuity(E, mySurfaces.Surface(s), mySurfaces.Surface(s),
                                                     Locations().Location(l), Locations().Location(l), reg);
                            } else {
                                if (FormatNb() == TopTools_FormatVersion_VERSION_2)
                                    myBuilder.UpdateEdge(E, myCurves2d.Curve2d(pc), mySurfaces.Surface(s),
                                                         Locations().Location(l), tol, aPf, aPl);
                                else
                                    myBuilder.UpdateEdge(E, myCurves2d.Curve2d(pc), mySurfaces.Surface(s),
                        ...
                        case 4: // -4- Regularity
                            ...
                            if (mySurfaces.Surface(s).IsNull() || mySurfaces.Surface(s2).IsNull()) break;
                            //  Modified by Sergey KHROMOV - Wed Apr 24 12:39:14 2002 End
                            myBuilder.Continuity(E, mySurfaces.Surface(s), mySurfaces.Surface(s2),
                                                 Locations().Location(l), Locations().Location(l2), reg);
                            break;
                        ...
                        case 7:
                            ...
                            if (closed) {
                                if (t > 0 && t <= myTriangulations.Extent() && pt > 0 && pt <= myNodes.Extent() &&
                                    ...
                                    myBuilder.UpdateEdge(E, Handle(Poly_PolygonOnTriangulation)::DownCast(myNodes(pt)),
                                                         Handle(Poly_PolygonOnTriangulation)::DownCast(myNodes(pt2)),
                                                         myTriangulations.FindKey(t), Locations().Location(l));
                            } else {
                                if (t > 0 && t <= myTriangulations.Extent() && pt > 0 && pt <= myNodes.Extent())
                                    myBuilder.UpdateEdge(E, Handle(Poly_PolygonOnTriangulation)::DownCast(myNodes(pt)),
                                                         myTriangulations.FindKey(t), Locations().Location(l));
        ...
        case TopAbs_FACE: {
            ...
            if (val == 0 || val == 1) {
                ...
                if (!mySurfaces.Surface(s).IsNull()) {
                    //  Modified by Sergey KHROMOV - Wed Apr 24 12:39:14 2002 End
                    myBuilder.UpdateFace(TopoDS::Face(S), mySurfaces.Surface(s), Locations().Location(l), tol);
...
void BRepTools_ShapeSet::WritePolygonOnTriangulation(Standard_OStream& OS, const Standard_Boolean Compact,
...
void BRepTools_ShapeSet::ReadPolygonOnTriangulation(Standard_IStream& IS, const Message_ProgressRange& theProgress) {
    char buffer[255];
    IS >> buffer;
...
void BRepTools_ShapeSet::WritePolygon3D(Standard_OStream& OS, const Standard_Boolean Compact,
                                        const Message_ProgressRange& theProgress) const {
...
void BRepTools_ShapeSet::WriteTriangulation(Standard_OStream& OS, const Standard_Boolean Compact,
                                            const Message_ProgressRange& theProgress) const {
    ...
    for (i = 1; i <= nbtri && aPS.More(); i++, aPS.Next()) {
        ...
        if (Compact) {
            ...
            OS << "  " << i << " : Triangulation with " << T->NbNodes() << " Nodes and " << T->NbTriangles()
               << " Triangles\n";
