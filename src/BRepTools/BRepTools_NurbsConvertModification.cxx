...
//
static void GeomLib_ChangeUBounds(Handle(Geom_BSplineSurface) & aSurface, const Standard_Real newU1,
...
static void GeomLib_ChangeVBounds(Handle(Geom_BSplineSurface) & aSurface, const Standard_Real newV1,
...
Standard_Boolean BRepTools_NurbsConvertModification::NewSurface(const TopoDS_Face& F, Handle(Geom_Surface) & S,
    ...
    if ((TheTypeSS == STANDARD_TYPE(Geom_BSplineSurface)) || (TheTypeSS == STANDARD_TYPE(Geom_BezierSurface))) {
    ...
    if (Abs(U1 - surfU1) <= TolPar) U1 = surfU1;
    if (Abs(U2 - surfU2) <= TolPar) U2 = surfU2;
    if (Abs(V1 - surfV1) <= TolPar) V1 = surfV1;
    if (Abs(V2 - surfV2) <= TolPar) V2 = surfV2;
    ...
    if (IsUp) {
        ...
        if (U2 - U1 > Up) U2 = U1 + Up;
    }
    if (IsVp) {
        Standard_Real Vp = S->VPeriod();
        if (V2 - V1 > Vp) V2 = V1 + Vp;
    }
    ...
    */
    ...
    if (Abs(surfU1 - U1) > Tol || Abs(surfU2 - U2) > Tol || Abs(surfV1 - V1) > Tol || Abs(surfV2 - V2) > Tol)
...
Standard_Boolean BRepTools_NurbsConvertModification::NewCurve(const TopoDS_Edge& E, Handle(Geom_Curve) & C,
                                                              TopLoc_Location& L, Standard_Real& Tol) {
...
Standard_Boolean BRepTools_NurbsConvertModification::NewPoint(const TopoDS_Vertex&, gp_Pnt&, Standard_Real&) {
    return Standard_False;
}
...
Standard_Boolean BRepTools_NurbsConvertModification::NewCurve2d(const TopoDS_Edge& E, const TopoDS_Face& F,
                                                                const TopoDS_Edge& newE, const TopoDS_Face& newF,
                                                                Handle(Geom2d_Curve) & Curve2d, Standard_Real& Tol) {
...
Standard_Boolean BRepTools_NurbsConvertModification::NewParameter(const TopoDS_Vertex& V, const TopoDS_Edge& E,
    ...
    if (!myMap.Contains(gc)) return Standard_False;

    Handle(Geom_BSplineCurve) gcc = Handle(Geom_BSplineCurve)::DownCast(myMap.FindFromKey(gc));
...
GeomAbs_Shape BRepTools_NurbsConvertModification::Continuity(const TopoDS_Edge& E, const TopoDS_Face& F1,
