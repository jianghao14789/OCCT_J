...
public:
    ...
    Standard_EXPORT static void UVBounds(const TopoDS_Face& F, Standard_Real& UMin, Standard_Real& UMax,
                                         Standard_Real& VMin, Standard_Real& VMax);

    //! Returns in UMin,  UMax, VMin,  VMax  the  bounding
    //! values of the wire in the parametric space of F.
    Standard_EXPORT static void UVBounds(const TopoDS_Face& F, const TopoDS_Wire& W, Standard_Real& UMin,
                                         Standard_Real& UMax, Standard_Real& VMin, Standard_Real& VMax);

    //! Returns in UMin,  UMax, VMin,  VMax  the  bounding
    //! values of the edge in the parametric space of F.
    Standard_EXPORT static void UVBounds(const TopoDS_Face& F, const TopoDS_Edge& E, Standard_Real& UMin,
                                         Standard_Real& UMax, Standard_Real& VMin, Standard_Real& VMax);
...
public:
    //! Verifies that each Face from the shape has got a triangulation with a deflection smaller or equal to specified
    //! one and the Edges a discretization on this triangulation.
    ...
    Standard_EXPORT static Standard_Boolean Triangulation(const TopoDS_Shape& theShape, const Standard_Real theLinDefl,
    ...
    LoadTriangulation(const TopoDS_Shape& theShape, const Standard_Integer theTriangulationIdx = -1,
    ...
    //! @return TRUE if at least one triangulation is loaded.
    Standard_EXPORT static Standard_Boolean
    LoadAllTriangulations(const TopoDS_Shape& theShape,
                          const Handle(OSD_FileSystem) & theFileSystem = Handle(OSD_FileSystem)());
...
public:
    ...
    Standard_EXPORT static void DetectClosedness(const TopoDS_Face& theFace, Standard_Boolean& theUclosed,
    ...
    static void Write(const TopoDS_Shape& theShape, Standard_OStream& theStream,
        ...
        Write(theShape, theStream, Standard_True, Standard_False, TopTools_FormatVersion_CURRENT, theProgress);
    }

    //! Writes the shape to the stream in an ASCII format of specified version.
    //! @param theShape [in]         the shape to write
    //! @param theStream [in][out]   the stream to output shape into
    //! @param theWithTriangles [in] flag which specifies whether to save shape with (TRUE) or without (FALSE)
    //! triangles;
    //!                              has no effect on triangulation-only geometry
    //! @param theWithNormals [in]   flag which specifies whether to save triangulation with (TRUE) or without (FALSE)
    //! normals;
    //!                              has no effect on triangulation-only geometry
    ...
    static Standard_Boolean Write(const TopoDS_Shape& theShape, const Standard_CString theFile,
                                  const Message_ProgressRange& theProgress = Message_ProgressRange()) {
        return Write(theShape, theFile, Standard_True, Standard_False, TopTools_FormatVersion_CURRENT, theProgress);
    }
    ...
    Standard_EXPORT static Standard_Real EvalAndUpdateTol(const TopoDS_Edge& theE, const Handle(Geom_Curve) & theC3d,
                                                          const Handle(Geom2d_Curve) theC2d,
                                                          const Handle(Geom_Surface) & theS, const Standard_Real theF,
                                                          const Standard_Real theL);

    //! returns the cumul  of the orientation  of <Edge>
    //! and thc containing wire in <Face>
    Standard_EXPORT static TopAbs_Orientation OriEdgeInFace(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace);
    ...
    Standard_EXPORT static void RemoveInternals(TopoDS_Shape& theS, const Standard_Boolean theForce = Standard_False);
    ...
    Standard_EXPORT static void CheckLocations(const TopoDS_Shape& theS, TopTools_ListOfShape& theProblemShapes);
