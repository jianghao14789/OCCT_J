...
public:
    RWGltf_SubBuffer(const Handle(NCollection_Buffer) & theBase, Standard_Size theOffset, Standard_Size theLength)
...
inline const RWGltf_JsonValue* findObjectMember(const RWGltf_JsonValue& theObject, const RWGltf_JsonValue& theName) {
...
//! Find member of the object in a safe way.
inline const RWGltf_JsonValue* findObjectMember(const RWGltf_JsonValue& theObject, const char* theName) {
...
void RWGltf_GltfJsonParser::GltfElementMap::Init(const TCollection_AsciiString& theRootName,
    ...
    if (theRoot->IsObject()) {
        // glTF 1.0
        for (ConstMemberIterator aChildIter = theRoot->MemberBegin(); aChildIter != theRoot->MemberEnd();
             ++aChildIter) {
        ...
        // glTF 2.0
        int aChildIndex = 0;
        for (rapidjson::Value::ConstValueIterator aChildIter = theRoot->Begin(); aChildIter != theRoot->End();
...
// purpose  :
// =======================================================================
void RWGltf_GltfJsonParser::reportGltfSyntaxProblem(const TCollection_AsciiString& theMsg, Message_Gravity theGravity) {
...
RWGltf_GltfJsonParser::RWGltf_GltfJsonParser(TopTools_SequenceOfShape& theRootShapes)
    : myRootShapes(&theRootShapes), myAttribMap(NULL), myExternalFiles(NULL), myMetadata(NULL), myBinBodyOffset(0),
      myBinBodyLen(0), myIsBinary(false), myIsGltf1(false), myToSkipEmptyNodes(true), myToLoadAllScenes(false),
      myUseMeshNameAsFallback(true), myToProbeHeader(false) {
    ...
    for (ConstMemberIterator aRootIter = MemberBegin(); aRootIter != MemberEnd(); ++aRootIter) {
        for (int aRootNameIter = 0; aRootNameIter < RWGltf_GltfRootElement_NB; ++aRootNameIter) {
            if (myGltfRoots[aRootNameIter].IsNull() && aNames[aRootNameIter] == aRootIter->name) {
                ...
                myGltfRoots[aRootNameIter].Init(RWGltf_GltfRootElementName((RWGltf_GltfRootElement)aRootNameIter),
                                                &aRootIter->value);
                break;
            }
        }
...
void RWGltf_GltfJsonParser::gltfParseMaterials() {
    ...
    if (aMatList == NULL) {
        ...
        // glTF 2.0
        int aMatIndex = 0;
        for (rapidjson::Value::ConstValueIterator aMatIter = aMatList->Begin(); aMatIter != aMatList->End();
             ++aMatIter, ++aMatIndex) {
            ...
            if (gltfParseCommonMaterial(aMatCommon, aMatNode) || gltfParseStdMaterial(aMatCommon, aMatNode)) {
                if (aNameVal != NULL && aNameVal->IsString()) {
...
bool RWGltf_GltfJsonParser::gltfParseStdMaterial(Handle(RWGltf_MaterialCommon) & theMat,
    ...
    if (anAmbVal == NULL && aDiffVal == NULL && anEmiVal == NULL && aSpecVal == NULL && aShinVal == NULL) {
        return false;
    }
...
bool RWGltf_GltfJsonParser::gltfParsePbrMaterial(Handle(RWGltf_MaterialMetallicRoughness) & theMat,
    ...
    // TODO ADOBE_materials_thin_transparency extension can be used to read IOR (Index of Refraction for transparent
    // materials)
...
// purpose  :
// =======================================================================
bool RWGltf_GltfJsonParser::gltfParseTexture(Handle(Image_Texture) & theTexture, const RWGltf_JsonValue* theTextureId) {
    ...
    if (anUriVal == NULL) {
        ...
        return gltfParseTextureInBufferView(theTexture, getKeyString(*aSrcVal), getKeyString(*aBufferViewName),
                                            *aBufferView);
    }
...
bool RWGltf_GltfJsonParser::gltfParseTexturInGlbBuffer(Handle(Image_Texture) & theTexture,
    ...
    if (aBufferName != NULL && aBufferName->IsString() && !IsEqual(aBufferName->GetString(), "binary_glTF")) {
        reportGltfError("BufferView '" + theBufferViewId + "' does not define binary_glTF buffer.");
        return false;
    }

    RWGltf_GltfBufferView aBuffView;
    aBuffView.ByteOffset = aByteOffset != NULL && aByteOffset->IsNumber() ? (int64_t)aByteOffset->GetDouble() : 0;
    aBuffView.ByteLength = aByteLength != NULL && aByteLength->IsNumber() ? (int64_t)aByteLength->GetDouble() : 0;
...
bool RWGltf_GltfJsonParser::gltfParseTextureInBufferView(Handle(Image_Texture) & theTexture,
    ...
    if (::strncmp(anUriData, "data:application/octet-stream;base64,", 37) == 0) {
        ...
        Handle(RWGltf_SubBuffer) aSubBuffer =
            new RWGltf_SubBuffer(aBaseBuffer, (Standard_Size)aBuffView.ByteOffset, (Standard_Size)aBuffView.ByteLength);
...
bool RWGltf_GltfJsonParser::gltfParseScene(const Message_ProgressRange& theProgress) {
    ...
    if (myToLoadAllScenes && !myIsGltf1 && aScenes->IsArray() && aScenes->Size() > 1) {
        ...
        for (rapidjson::Value::ConstValueIterator aSceneIter = aScenes->Begin(); aSceneIter != aScenes->End();
             ++aSceneIter) {
            if (!aPS.More()) {
                return false;
            }
    ...
    if (!myGltfRoots[RWGltf_GltfRootElement_Scene].IsNull()) {
        aDefScene =
            myGltfRoots[RWGltf_GltfRootElement_Scenes].FindChild(*myGltfRoots[RWGltf_GltfRootElement_Scene].Root());
    } else if (!myIsGltf1) {
...
bool RWGltf_GltfJsonParser::gltfParseSceneNode(TopoDS_Shape& theNodeShape,
    ...
    if (aMeshes_1 != NULL && aMeshes_1->IsArray()) {
        // glTF 1.0
        for (rapidjson::Value::ConstValueIterator aMeshIter = aMeshes_1->Begin(); aMeshIter != aMeshes_1->End();
             ++aMeshIter) {
...
// purpose  :
// =======================================================================
bool RWGltf_GltfJsonParser::gltfParseMesh(TopoDS_Shape& theMeshShape, const TCollection_AsciiString& theMeshId,
    ...
    int aNbFaces = 0;
    for (rapidjson::Value::ConstValueIterator aPrimArrIter = aPrims->Begin(); aPrimArrIter != aPrims->End();
         ++aPrimArrIter) {
        TopoDS_Shape aFace;
...
bool RWGltf_GltfJsonParser::gltfParsePrimArray(TopoDS_Shape& thePrimArrayShape,
    ...
    const RWGltf_JsonValue* aDracoBuf = aDracoVal != NULL ? findObjectMember(*aDracoVal, "bufferView") : NULL;
    ...
    if (myShapeMap[ShapeMapGroup_PrimArray].Find(aPrimArrayIdWithMat, thePrimArrayShape)) {
        ...
        if (myAttribMap != NULL) {
            ...
            // Handle(RWGltf_GltfLatePrimitiveArray) aLateData = Handle(RWGltf_GltfLatePrimitiveArray)::DownCast
            // (BRep_Tool::Triangulation (TopoDS::Face (thePrimArrayShape), aDummy)); TopoDS_Face aFaceCopy;
            // BRep_Builder().MakeFace (aFaceCopy, aLateData);
...
bool RWGltf_GltfJsonParser::gltfParseAccessor(const Handle(RWGltf_GltfLatePrimitiveArray) & theMeshData,
                                              ...
                                              const RWGltf_JsonValue& theAccessor, const RWGltf_GltfArrayType theType,
    ...
    const RWGltf_JsonValue* aByteOffset = theCompBuffView == NULL ? findObjectMember(theAccessor, "byteOffset") : 0;
    const RWGltf_JsonValue* aByteStride =
        findObjectMember(theAccessor, "byteStride"); // byteStride was part of bufferView in glTF 1.0
    ...
    aStruct.ByteOffset = aByteOffset != NULL && aByteOffset->IsNumber() ? (int64_t)aByteOffset->GetDouble() : 0;
...
bool RWGltf_GltfJsonParser::gltfParseBufferView(const Handle(RWGltf_GltfLatePrimitiveArray) & theMeshData,
    ...
    const RWGltf_JsonValue* aByteOffset = findObjectMember(theBufferView, "byteOffset");
    const RWGltf_JsonValue* aByteStride =
        findObjectMember(theBufferView, "byteStride"); // byteStride is part of bufferView since glTF 2.0
...
bool RWGltf_GltfJsonParser::gltfParseBuffer(const Handle(RWGltf_GltfLatePrimitiveArray) & theMeshData,
                                            const TCollection_AsciiString& theName, const RWGltf_JsonValue& theBuffer,
                                            const RWGltf_GltfAccessor& theAccessor,
                                            const RWGltf_GltfBufferView& theView, const RWGltf_GltfArrayType theType) {
...
// purpose  :
// =======================================================================
void RWGltf_GltfJsonParser::bindNamedShape(TopoDS_Shape& theShape, ShapeMapGroup theGroup,
                                           const TopLoc_Location& theLoc, const TCollection_AsciiString& theId,
    ...
    if (myAttribMap != NULL) {
        ...
        if (theShape.ShapeType() == TopAbs_FACE) {
            ...
            if (myAttribMap->Find(aShape, aFaceAttribs)) {
                ...
                } else if (!aFaceAttribs.Name.IsEmpty() && theLoc.IsIdentity() && theGroup == ShapeMapGroup_Nodes) {
            ...
            for (TopExp_Explorer aFaceIter(theShape, TopAbs_FACE); aFaceIter.More(); aFaceIter.Next()) {
                if (Handle(RWGltf_GltfLatePrimitiveArray) aLateData = Handle(RWGltf_GltfLatePrimitiveArray)::DownCast(
...
#else
    Message::SendFail(
        "Error: glTF reader is unavailable - OCCT has been built without RapidJSON support [HAVE_RAPIDJSON undefined]");
    return false;
#endif
}
