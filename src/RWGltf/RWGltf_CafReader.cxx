...
public:
    ...
    CafReader_GltfBaseLoadingFunctor(NCollection_Vector<TopoDS_Face>& theFaceList,
        ...
        : myFaceList(&theFaceList), myProgress(theProgress, "Loading glTF triangulation", Max(1, theFaceList.Size())),
...
protected:
    ...
    virtual Handle(Poly_Triangulation)
        loadData(const Handle(RWGltf_GltfLatePrimitiveArray) & theLateData, int theThreadIndex) const = 0;
...
public:
    ...
    //! Main constructor.
    CafReader_GltfFullDataLoadingFunctor(RWGltf_CafReader* myCafReader, NCollection_Vector<TopoDS_Face>& theFaceList,
                                         const Message_ProgressRange& theProgress,
                                         const OSD_ThreadPool::Launcher& theThreadPool)
        : CafReader_GltfBaseLoadingFunctor(theFaceList, theProgress, theThreadPool), myCafReader(myCafReader),
...
RWGltf_CafReader::RWGltf_CafReader()
    : myToParallel(false), myToSkipEmptyNodes(true), myToLoadAllScenes(false), myUseMeshNameAsFallback(true),
      myIsDoublePrecision(false), myToSkipLateDataLoading(false), myToKeepLateData(true),
...
Standard_Boolean RWGltf_CafReader::performMesh(const TCollection_AsciiString& theFile,
    ...
    if (isBinaryFile) {
        aRes = aDoc.ParseStream<rapidjson::kParseStopWhenDoneFlag, rapidjson::UTF8<>, rapidjson::IStreamWrapper>(
            aFileStream);
    } else {
