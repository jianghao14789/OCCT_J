...
bool RWGltf_TriangulationReader::LoadStreamData(const Handle(RWMesh_TriangulationSource) & theSourceMesh,
    ...
    Standard_ASSERT_RETURN(!theDestMesh.IsNull(),
                           "The destination mesh should be initialized before loading data to it", false);
    theDestMesh->Clear();
...
bool RWGltf_TriangulationReader::readFileData(const Handle(RWGltf_GltfLatePrimitiveArray) & theSourceGltfMesh,
    ...
    opencascade::std::shared_ptr<std::istream> aSharedStream =
        aFileSystem->OpenIStream(theGltfData.StreamUri, std::ios::in | std::ios::binary, theGltfData.StreamOffset);
    if (aSharedStream.get() == NULL) {
        reportError(TCollection_AsciiString("Buffer '") + theSourceGltfMesh->Id() + "' refers to invalid file '" +
...
bool RWGltf_TriangulationReader::loadStreamData(const Handle(RWMesh_TriangulationSource) & theSourceMesh,
    ...
    if (aSourceGltfMesh.IsNull() || aSourceGltfMesh->PrimitiveMode() == RWGltf_GltfPrimitiveMode_UNKNOWN) {
        return false;
    }
    bool wasLoaded = false;
    for (NCollection_Sequence<RWGltf_GltfPrimArrayData>::Iterator aDataIter(aSourceGltfMesh->Data()); aDataIter.More();
         aDataIter.Next()) {
...
bool RWGltf_TriangulationReader::readDracoBuffer(const Handle(RWGltf_GltfLatePrimitiveArray) & theSourceGltfMesh,
    ...
    if (!aSharedStream->good()) {
        reportError(TCollection_AsciiString("Buffer '") + aName + "' refers to file that cannot be read '" +
                    theGltfData.StreamUri + "'.");
        return false;
    }
    ...
    if (!aDracoStat.ok() || aDracoStat.value().get() == NULL) {
        reportError(TCollection_AsciiString("Buffer '") + aName + "' refers to Draco data that cannot be decoded '" +
                    theGltfData.StreamUri + "'.");
        return false;
    }
            ...
            case RWGltf_GltfArrayType_Normal: {
                if (aWrapCompType != RWGltf_GltfAccessorCompType_Float32 ||
                    ...
                    Message::SendTrace(
                        ...
                        "Vertex normals in unsupported format have been skipped while reading glTF triangulation '" +
                        aName + "'");
                    break;
                }
            ...
            case RWGltf_GltfArrayType_TCoord0: {
                if (aWrapCompType != RWGltf_GltfAccessorCompType_Float32 ||
                    ...
                    Message::SendTrace(TCollection_AsciiString() +
                                       "Vertex UV coordinates in unsupported format have been skipped while reading "
                                       "glTF triangulation '" +
                                       aName + "'");
                    break;
                }
...
bool RWGltf_TriangulationReader::readBuffer(const Handle(RWGltf_GltfLatePrimitiveArray) & theSourceMesh,
                                            const Handle(Poly_Triangulation) & theDestMesh, std::istream& theStream,
                                            const RWGltf_GltfAccessor& theAccessor, RWGltf_GltfArrayType theType) const
    ...
    switch (theType) {
        case RWGltf_GltfArrayType_Indices: {
            ...
            if (theAccessor.ComponentType == RWGltf_GltfAccessorCompType_UInt16) {
                ...
                const size_t aStride = theAccessor.ByteStride != 0 ? theAccessor.ByteStride : sizeof(uint16_t);
                ...
                const size_t aStride = theAccessor.ByteStride != 0 ? theAccessor.ByteStride : sizeof(uint32_t);
                Standard_ReadBuffer aBuffer(theAccessor.Count * aStride, aStride);
                Standard_Integer aLastTriIndex = 0;
                for (Standard_Integer aTriIter = 0; aTriIter < aNbTris; ++aTriIter) {
                ...
                const size_t aStride = theAccessor.ByteStride != 0 ? theAccessor.ByteStride : sizeof(uint8_t);
                Standard_ReadBuffer aBuffer(theAccessor.Count * aStride, aStride);
                Standard_Integer aLastTriIndex = 0;
                for (Standard_Integer aTriIter = 0; aTriIter < aNbTris; ++aTriIter) {
        ...
        case RWGltf_GltfArrayType_Position: {
            ...
            const size_t aStride = theAccessor.ByteStride != 0 ? theAccessor.ByteStride : sizeof(Graphic3d_Vec3);
            const Standard_Integer aNbNodes = (Standard_Integer)theAccessor.Count;
            if (!setNbPositionNodes(theDestMesh, aNbNodes)) {
                return false;
            }

            Standard_ReadBuffer aBuffer(theAccessor.Count * aStride - (aStride - sizeof(Graphic3d_Vec3)), aStride,
                                        true);
            if (!myCoordSysConverter.IsEmpty()) {
                ...
                for (Standard_Integer aVertIter = 0; aVertIter < aNbNodes; ++aVertIter) {
                    ...
                    setNodePosition(theDestMesh, THE_LOWER_NODE_INDEX + aVertIter,
        ...
        case RWGltf_GltfArrayType_TCoord0: {
            ...
            const size_t aStride = theAccessor.ByteStride != 0 ? theAccessor.ByteStride : sizeof(Graphic3d_Vec2);
            const Standard_Integer aNbNodes = (Standard_Integer)theAccessor.Count;
            if (!setNbUVNodes(theDestMesh, aNbNodes)) {
                return false;
            }

            Standard_ReadBuffer aBuffer(theAccessor.Count * aStride - (aStride - sizeof(Graphic3d_Vec2)), aStride,
                                        true);
            for (int aVertIter = 0; aVertIter < aNbNodes; ++aVertIter) {
