...
// Purpose  :
//================================================================
RWGltf_CafWriter::RWGltf_CafWriter(const TCollection_AsciiString& theFile, Standard_Boolean theIsBinary)
    : myFile(theFile), myTrsfFormat(RWGltf_WriterTrsfFormat_Compact),
      myNodeNameFormat(RWMesh_NameFormat_InstanceOrProduct), myMeshNameFormat(RWMesh_NameFormat_Product),
      myIsBinary(theIsBinary), myIsForcedUVExport(false), myToEmbedTexturesInGlb(true), myToMergeFaces(false),
      myToSplitIndices16(false), myBinDataLen64(0) {
...
// Purpose  :
//================================================================
TCollection_AsciiString RWGltf_CafWriter::formatName(RWMesh_NameFormat theFormat, const TDF_Label& theLabel,
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::saveNodes(RWGltf_GltfFace& theGltfFace, std::ostream& theBinFile,
                                 const RWMesh_FaceIterator& theFaceIter, Standard_Integer& theAccessorNb) const {
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::saveNormals(RWGltf_GltfFace& theGltfFace, std::ostream& theBinFile,
                                   RWMesh_FaceIterator& theFaceIter, Standard_Integer& theAccessorNb) const {
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::saveTextCoords(RWGltf_GltfFace& theGltfFace, std::ostream& theBinFile,
                                      const RWMesh_FaceIterator& theFaceIter, Standard_Integer& theAccessorNb) const {
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::saveIndices(RWGltf_GltfFace& theGltfFace, std::ostream& theBinFile,
                                   const RWMesh_FaceIterator& theFaceIter, Standard_Integer& theAccessorNb) {
    if (theGltfFace.Indices.Id == RWGltf_GltfAccessor::INVALID_ID) {
        ...
        const int64_t aPos =
            ...
            theGltfFace.Indices.Count * (theGltfFace.Indices.ComponentType == RWGltf_GltfAccessorCompType_UInt32
                                             ? sizeof(uint32_t)
                                             : sizeof(uint16_t));
...
// purpose  :
// =======================================================================
bool RWGltf_CafWriter::Perform(const Handle(TDocStd_Document) & theDocument, const TDF_LabelSequence& theRootLabels,
...
bool RWGltf_CafWriter::writeBinData(const Handle(TDocStd_Document) & theDocument,
    ...
    opencascade::std::shared_ptr<std::ostream> aBinFile =
    ...
    const RWGltf_GltfArrayType anArrTypes[4] = {RWGltf_GltfArrayType_Position, RWGltf_GltfArrayType_Normal,
                                                RWGltf_GltfArrayType_TCoord0, RWGltf_GltfArrayType_Indices};
    ...
    for (XCAFPrs_DocumentExplorer aDocExplorer(theDocument, theRootLabels, XCAFPrs_DocumentExplorerFlags_OnlyLeafNodes);
        ...
        if (myToMergeFaces) {
            ...
            for (; aFaceIter.More() && aPSentryBin.More(); aFaceIter.Next()) {
                ...
                } else if (myToSplitIndices16 && aGltfFace->NbIndexedNodes < std::numeric_limits<uint16_t>::max() &&
    ...
    for (Standard_Integer aTypeIter = 0; aTypeIter < 4; ++aTypeIter) {
        ...
        for (ShapeToGltfFaceMap::Iterator aBinDataIter(myBinDataMap); aBinDataIter.More() && aPSentryBin.More();
             aBinDataIter.Next()) {
            ...
            for (RWGltf_GltfFaceList::Iterator aGltfFaceIter(*aGltfFaceList);
                 aGltfFaceIter.More() && aPSentryBin.More(); aGltfFaceIter.Next()) {
                ...
                for (RWMesh_FaceIterator aFaceIter(aGltfFace->Shape, aGltfFace->Style);
                     aFaceIter.More() && aPSentryBin.More(); aFaceIter.Next()) {
                    switch (anArrType) {
                        case RWGltf_GltfArrayType_Position: {
    ...
    if (myIsBinary && myToEmbedTexturesInGlb) {
        ...
        for (XCAFPrs_DocumentExplorer aDocExplorer(theDocument, theRootLabels,
...
// Purpose  :
//================================================================
bool RWGltf_CafWriter::writeJson(const Handle(TDocStd_Document) & theDocument, const TDF_LabelSequence& theRootLabels,
                                 const TColStd_MapOfAsciiString* theLabelFilter,
                                 const TColStd_IndexedDataMapOfStringString& theFileInfo,
                                 const Message_ProgressRange& theProgress) {
#ifdef HAVE_RAPIDJSON
    ...
    const Handle(OSD_FileSystem) & aFileSystem = OSD_FileSystem::DefaultFileSystem();
    opencascade::std::shared_ptr<std::ostream> aGltfContentFile =
    ...
    if (aFullLen64 < std::numeric_limits<uint32_t>::max()) {
        {
            opencascade::std::shared_ptr<std::istream> aBinFile =
...
void RWGltf_CafWriter::writeAsset(const TColStd_IndexedDataMapOfStringString& theFileInfo) {
    ...
    for (TColStd_IndexedDataMapOfStringString::Iterator aKeyValueIter(theFileInfo); aKeyValueIter.More();
         aKeyValueIter.Next()) {
        if (!anIsStarted) {
            myWriter->Key("extras");
            myWriter->StartObject();
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::writePrimArray(const RWGltf_GltfFace& theGltfFace, const TCollection_AsciiString& theName,
...
void RWGltf_CafWriter::writeMeshes(const RWGltf_GltfSceneNodeMap& theSceneNodeMap) {
    ...
    for (RWGltf_GltfSceneNodeMap::Iterator aSceneNodeIter(theSceneNodeMap); aSceneNodeIter.More();
        ...
        if (myToMergeFaces) {
            ...
            for (RWMesh_FaceIterator aFaceIter(aDocNode.RefLabel, TopLoc_Location(), true, aDocNode.Style);
                 aFaceIter.More(); aFaceIter.Next(), ++aNbFacesInNode) {
                if (toSkipFaceMesh(aFaceIter)) {
                    continue;
                }
...
// purpose  :
// =======================================================================
void RWGltf_CafWriter::writeNodes(const Handle(TDocStd_Document) & theDocument, const TDF_LabelSequence& theRootLabels,
                                  const TColStd_MapOfAsciiString* theLabelFilter,
    ...
    for (RWGltf_GltfSceneNodeMap::Iterator aSceneNodeIter(aSceneNodeMapWithChildren); aSceneNodeIter.More();
         aSceneNodeIter.Next()) {
        const XCAFPrs_DocumentNode& aDocNode = aSceneNodeIter.Value();
        ...
        if (!aDocNode.LocalTrsf.IsIdentity()) {
            ...
            if (aTrsf.Form() != gp_Identity) {
                ...
                const bool hasRotation =
                    Abs(aQuaternion.X()) > gp::Resolution() || Abs(aQuaternion.Y()) > gp::Resolution() ||
...
void RWGltf_CafWriter::writeScenes(const NCollection_Sequence<Standard_Integer>& theSceneRootNodeInds) {
    ...
    myWriter->StartArray();
    {
        ...
        myWriter->Key("nodes");
        myWriter->StartArray();
        for (NCollection_Sequence<Standard_Integer>::Iterator aRootIter(theSceneRootNodeInds); aRootIter.More();
             aRootIter.Next()) {
