...
public:
    ...
    void SetSkipEmptyNodes(bool theToSkip) {
        myToSkipEmptyNodes = theToSkip;
    }

    //! Set flag to flag to load all scenes in the document, FALSE by default which means only main (default) scene will
    //! be loaded.
    void SetLoadAllScenes(bool theToLoadAll) {
        myToLoadAllScenes = theToLoadAll;
    }

    //! Set flag to use Mesh name in case if Node name is empty, TRUE by default.
    void SetMeshNameAsFallback(bool theToFallback) {
        myUseMeshNameAsFallback = theToFallback;
    }
...
protected:
    ...
    Standard_EXPORT bool gltfParseTexture(Handle(Image_Texture) & theTexture, const RWGltf_JsonValue* theTextureId);
...
protected:
    ...
    Standard_EXPORT bool gltfParseSceneNode(TopoDS_Shape& theNodeShape, const TCollection_AsciiString& theSceneNodeId,
                                            const RWGltf_JsonValue& theSceneNode,
                                            const Message_ProgressRange& theProgress);

    //! Parse mesh element.
    Standard_EXPORT bool gltfParseMesh(TopoDS_Shape& theMeshShape, const TCollection_AsciiString& theMeshId,
                                       const RWGltf_JsonValue& theMesh);

    //! Parse primitive array.
    Standard_EXPORT bool gltfParsePrimArray(TopoDS_Shape& thePrimArrayShape, const TCollection_AsciiString& theMeshId,
    ...
    Standard_EXPORT bool gltfParseAccessor(const Handle(RWGltf_GltfLatePrimitiveArray) & theMeshData,
                                           const TCollection_AsciiString& theName, const RWGltf_JsonValue& theAccessor,
                                           const RWGltf_GltfArrayType theType, const RWGltf_JsonValue* theCompBuffView);
    ...
    Standard_EXPORT bool gltfParseBuffer(const Handle(RWGltf_GltfLatePrimitiveArray) & theMeshData,
                                         ...
                                         const RWGltf_GltfAccessor& theAccessor, const RWGltf_GltfBufferView& theView,
                                         const RWGltf_GltfArrayType theType);
...
protected:
    ...
    static bool validateColor4(const Graphic3d_Vec4d& theVec) {
        return theVec.r() >= 0.0 && theVec.r() <= 1.0 && theVec.g() >= 0.0 && theVec.g() <= 1.0 && theVec.b() >= 0.0 &&
               theVec.b() <= 1.0 && theVec.a() >= 0.0 && theVec.a() <= 1.0;
    }
...
protected:
    ...
    //! Bind name attribute.
    void bindNodeShape(TopoDS_Shape& theShape, const TopLoc_Location& theLoc, const TCollection_AsciiString& theNodeId,
                       const RWGltf_JsonValue* theUserName) {
        bindNamedShape(theShape, ShapeMapGroup_Nodes, theLoc, theNodeId, theUserName);
    }

    //! Bind name attribute.
    void bindMeshShape(TopoDS_Shape& theShape, const TCollection_AsciiString& theMeshId,
                       const RWGltf_JsonValue* theUserName) {
    ...
    //! Bind name attribute.
    Standard_EXPORT void bindNamedShape(TopoDS_Shape& theShape, ShapeMapGroup theGroup, const TopLoc_Location& theLoc,
                                        const TCollection_AsciiString& theId, const RWGltf_JsonValue* theUserName);

    //! Find named shape.
    bool findNamedShape(TopoDS_Shape& theShape, ShapeMapGroup theGroup, const TCollection_AsciiString& theId) const {
...
protected:
    ...
    public:
        ...
        //! Access this node.
        const RWGltf_JsonValue* Root() const {
            return myRoot;
        }
...
protected:
    ...
    NCollection_IndexedMap<TCollection_AsciiString>* myExternalFiles; //!< list of external file references
