...
public:
    ...
    //! Return TRUE if all scenes in the document should be loaded, FALSE by default which means only main (default)
    //! scene will be loaded.
    bool ToLoadAllScenes() const {
        return myToLoadAllScenes;
    }
    ...
    bool ToUseMeshNameAsFallback() {
        return myUseMeshNameAsFallback;
    }
...
protected:
    ...
    //! Can be overridden by sub-class to read triangulation into application-specific data structures instead of
    //! Poly_Triangulation. Default implementation creates RWGltf_TriangulationReader.
...
protected:
    ...
    Standard_Boolean
        myToKeepLateData; //!< flag to keep information about deferred storage to load/unload triangulation later
