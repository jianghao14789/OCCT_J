...
Standard_Integer projonplane(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if ((n == 5 && Draw::Atoi(a[4]) == 0) || (n == 8 && Draw::Atoi(a[7]) == 0)) Param = Standard_False;
...
static Standard_Real CompLocalDev(const Adaptor3d_Curve& theCurve, const Standard_Real u1, const Standard_Real u2);

static void ComputeDeviation(const Adaptor3d_Curve& theCurve, const Handle(Geom_BSplineCurve) & thePnts,
                             Standard_Real& theDmax, Standard_Real& theUfMax, Standard_Real& theUlMax,
...
Standard_Real CompLocalDev(const Adaptor3d_Curve& theCurve, const Standard_Real u1, const Standard_Real u2) {
...
static Standard_Integer crvtpoints(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n > 4) angle = Draw::Atof(a[4]);

    if (n > 5) aMinPntsNb = Draw::Atoi(a[5]);
...
static Standard_Integer surfpoints(Draw_Interpretor& /*di*/, Standard_Integer /*n*/, const char** a) {
    ...
    Handle(Geom_BSplineSurface) aPnts = new Geom_BSplineSurface(aPoles, anUKnots, aVKnots, anUMults, aVMults, 1, 1);
...
static Standard_Integer intersection(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (GC1.IsNull()) {
        ...
        if ((nblines + nbpoints) >= 2) {
            ...
            if (nblines >= 1) std::cout << "   Lines: " << std::endl;
            ...
            if (nbpoints >= 1) std::cout << "   Points: " << std::endl;
...
//=======================================================================
static Standard_Integer GetCurveContinuity(Draw_Interpretor& theDI, Standard_Integer theNArg, const char** theArgv) {
    if (theNArg != 2) {
...
void GeometryTest::CurveCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("projonplane", "projonplane r C3d Plane [dx dy dz] [0/1]", projonplane);
    ...
    theCommands.Add("intersect",
                    ...
                    __FILE__, intersection, g);

    theCommands.Add("crvpoints", "crvpoints result <curve or wire> deflection", __FILE__, crvpoints, g);

    theCommands.Add("crvtpoints",
                    "crvtpoints result <curve or wire> deflection angular deflection - tangential deflection points",
                    __FILE__, crvtpoints, g);

    theCommands.Add("uniformAbscissa", "uniformAbscissa Curve nbPnt", __FILE__, uniformAbscissa, g);

    theCommands.Add("uniformAbscissaEl", "uniformAbscissaEl maxR minR nbPnt", __FILE__, EllipsUniformAbscissa, g);
    ...
    theCommands.Add("getcurvcontinuity",
                    "getcurvcontinuity {curve or 2dcurve}: \n\tReturns the continuity of the given curve", __FILE__,
