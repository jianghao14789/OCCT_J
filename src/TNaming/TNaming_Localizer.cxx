...
static void LWriteNSOnLabel(const Handle(TNaming_NamedShape) & NS, const Standard_CString filename) {
    if (!NS.IsNull() && !NS->IsEmpty()) {
        ...
        for (; it.More(); it.Next(), i++) {
            ...
            if (!oldShape.IsNull()) LWrite(oldShape, aName1.ToCString());
            if (!newShape.IsNull()) LWrite(newShape, aName2.ToCString());
        }
    }
}
#endif
...
void TNaming_Localizer::FindFeaturesInAncestors(const TopoDS_Shape& S, const TopoDS_Shape& Context,
...
const TopTools_MapOfShape& TNaming_Localizer::SubShapes(const TopoDS_Shape& In, const TopAbs_ShapeEnum TS) {
...
const TopTools_IndexedDataMapOfShapeListOfShape& TNaming_Localizer::Ancestors(const TopoDS_Shape& In,
...
Standard_Boolean TNaming_Localizer::IsNew(const TopoDS_Shape& S, const Handle(TNaming_NamedShape) & NS) {
...
void TNaming_Localizer::GoBack(const TopoDS_Shape& S, const TDF_Label& Lab, const TNaming_Evolution Evol,
                               TopTools_ListOfShape& LBS, TNaming_ListOfNamedShape& LBNS) {
    ...
    if (!it.More()) {
        ...
        if (itLab.More()) Sol = itLab.OldShape();
        //-------------------------------------------
...
void TNaming_Localizer::Backward(const Handle(TNaming_NamedShape) & NS, const TopoDS_Shape& S,
                                 TNaming_MapOfNamedShape& Primitives, TopTools_MapOfShape& Shapes) {
...
void TNaming_Localizer::FindNeighbourg(const TopoDS_Shape& Sol, const TopoDS_Shape& S,
...
void TNaming_Localizer::FindGenerator(const Handle(TNaming_NamedShape) & NS, const TopoDS_Shape& S,
...
void TNaming_Localizer::FindShapeContext(const Handle(TNaming_NamedShape) & NS, const TopoDS_Shape& S, TopoDS_Shape& SC)
