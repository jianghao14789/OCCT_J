...
//=======================================================================
static void DbgTools_WriteNSOnLabel(const Handle(TNaming_NamedShape) & NS, const Standard_CString filename) {
    if (!NS.IsNull() && !NS->IsEmpty()) {
        ...
        for (; it.More(); it.Next(), i++) {
            ...
            if (!oldShape.IsNull()) DbgTools_Write(oldShape, aName1.ToCString());
            if (!newShape.IsNull()) DbgTools_Write(newShape, aName2.ToCString());
        }
    }
}
#endif
...
TNaming_Name::TNaming_Name() : myType(TNaming_UNKNOWN), myIndex(-1) {}
...
static TopoDS_Shape ShapeWithType(const TopoDS_Shape theShape, const TopAbs_ShapeEnum theType) {
    ...
    if (aType < theType) {
        ...
        switch (aType) {
            ...
            Standard_FALLTHROUGH case TopAbs_WIRE: {
            ...
            Standard_FALLTHROUGH case TopAbs_SHELL: {
            ...
            Standard_FALLTHROUGH case TopAbs_SOLID: {
...
static Standard_Boolean FindModifUntil(TNaming_NewShapeIterator& it, TopTools_IndexedMapOfShape& MS,
                                       const TopoDS_Shape& S, const Handle(TNaming_NamedShape) & Context) {
#ifdef OCCT_DEBUG_MODUN
    if (!Context.IsNull()) PrintEntry(Context->Label());
#endif
    Standard_Boolean found = Standard_False;
    for (; it.More(); it.Next()) {
        if (!it.Shape().IsNull()) {
#ifdef OCCT_DEBUG_MODUN
            if (!it.NamedShape().IsNull()) PrintEntry(it.NamedShape()->Label());
#endif
...
static void SearchModifUntil(const TDF_LabelMap& /*Valid*/, const Handle(TNaming_NamedShape) & Target,
                             const TNaming_ListOfNamedShape& theListOfGenerators, TopTools_IndexedMapOfShape& theMS) {
...
//=======================================================================
static Standard_Boolean ModifUntil(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
...
//=======================================================================
static Standard_Boolean ConstShape(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
                                   const Handle(TNaming_NamedShape) & Stop, const Standard_Integer Index) {
    TopTools_IndexedMapOfShape MS;
    TDF_LabelMap Forbiden;
    if (!ValidArgs(Args)) return Standard_False;
...
//=======================================================================
static Standard_Boolean Intersection(const TDF_Label& L, const TDF_LabelMap& Valid,
                                     const TNaming_ListOfNamedShape& Args, const Handle(TNaming_NamedShape) & Stop,
                                     const TopAbs_ShapeEnum ShapeType, const Standard_Integer Index) {
...
#ifdef OCCT_DEBUG_INT
    if (!CS.IsNull()) DbgTools_Write(CS, "Int_CS_1.brep");
...
#ifdef OCCT_DEBUG_INT
        std::cout << "Kept: indxE = " << indxE << " maxENum = " << nbE << " indxW = " << indxW << " nbW = " << nbW
                  << std::endl;
#endif
...
static void KeepInList(const TopoDS_Shape& CS, const TopAbs_ShapeEnum Type, TopTools_ListOfShape& aList) {
    ...
    if (Type == TopAbs_SHAPE) {
        if (CS.ShapeType() == TopAbs_SOLID || CS.ShapeType() == TopAbs_FACE || CS.ShapeType() == TopAbs_EDGE ||
...
//=======================================================================
static Standard_Boolean Union(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
                              const Handle(TNaming_NamedShape) & Stop, const TopAbs_ShapeEnum ShapeType,
    ...
    */
    ...
    TNaming_NamingTool::CurrentShape(Valid, Forbiden, it.Value(),
                                     MS); // fill MS with last modifications of the first argument
    TopoDS_Shape CS = MakeShape(MS);

    TopTools_ListOfShape aListS;
    if (isOr) KeepInList(CS, ShapeType, aListS);
    ...
    for (; it.More(); it.Next()) {
        ...
        MS.Clear();
        TNaming_NamingTool::CurrentShape(Valid, Forbiden, it.Value(),
                                         MS); // fill MS with last modifications of the it.Value()
        CS = MakeShape(MS);
        if (isOr) KeepInList(CS, ShapeType, aListS);
        TNaming_ShapesSet OS(CS, ShapeType);
    ...
    if (!ContextLabel.IsNull()) {
        ...
        for (int i = 1; it1.More(); it1.Next(), i++) {
            std::cout << "Map(" << i << "): TShape = " << it1.Key().TShape() << " Orient = " << it1.Key().Orientation()
                      << std::endl;
        ...
        for (; itl.More(); itl.Next()) {
            ...
            for (; anExpl.More(); anExpl.Next()) {
                if (S.Map().Contains(anExpl.Current())) num--;
            }
            if (num == 0) {
                found = Standard_True;
                break;
...
static Standard_Boolean Generated(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args) {
    if (Args.Extent() < 2) {
    ...
    if (!aNaming.IsNull()) aSelection = aNaming->GetName().Shape();
#ifdef OCCT_DEBUG_GEN
    ...
    if (!anOldNS.IsNull()) aVer = anOldNS->Version();
    ...
    for (Standard_Integer anItMS = 1; anItMS <= aMS.Extent(); ++anItMS) {
        ...
        if (aMapDM.Extent()) aDM.Bind(OS, aMapDM);
    }
...
#ifdef OCCT_DEBUG_GEN
            if (!aShape.IsNull()) DbgTools_Write(aShape, "G_FindShape.brep");
#endif
            ...
            if (found) {
                ...
                if (aGenerators.Extent() != aMS.Extent()) { // missed generators
                    ...
                    for (TNaming_ListIteratorOfListOfNamedShape itg(aGenerators); itg.More(); itg.Next()) {
                        if (!aMS.Contains(itg.Value()->Get())) aLM.Append(itg.Value()->Get());
                    }
                    if (aLM.Extent() == 1) { // lost 1
                        ...
                        // <<std::endl;break;} 		  else 		    if(aShape.IsSame(aV2)) {aShape2 =  aV1;	std::cout << "##### => V1 "
                        // <<std::endl;break;}
                        // 		}
                        // 	      }
                        // 	    }
...
//=======================================================================
static Standard_Boolean Identity(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
...
//=======================================================================
static Standard_Boolean FilterByNeighbourgs(const TDF_Label& L, const TDF_LabelMap& Valid,
    ...
    TNaming_NamingTool::CurrentShape(Valid, Forbiden, Cand,
                                     SCand); // fills SCand with last modifications of Cand. CandNS should be at the
                                             // same level (before) as NS of FilterByNBS
    ...
    if (ShapeType == TopAbs_VERTEX)
        TC = TopAbs_VERTEX; // szy 31.03.10 - to process case when Candidate is of type Vertex
    ...
    for (Standard_Integer anItSCand = 1; anItSCand <= SCand.Extent(); ++anItSCand) { // 1
        ...
        if (S.ShapeType() == TopAbs_VERTEX) // # szy 31.03.10
            ...
            for (TopExp_Explorer exp(S, TC); exp.More();
                 exp.Next()) { // put boundaries of each candidate (from SCand) to the Boundaries map
        ...
        for (; it.More(); it.Next()) { // 2 ==> for each Arg
            ...
            TNaming_NamingTool::CurrentShape(Valid, Forbiden, NSVois,
                                             SVois); // fills SVois with last modifications of NSVois
...
static const TopoDS_Shape FindSubShapeInAncestor(const TopoDS_Shape& Selection, const TopoDS_Shape& Context) {
    ...
    for (; expl1.More(); expl1.Next()) {
        if (expl1.Current().IsSame(Selection)) {
            i++;
            std::cout << "FindSubShape:  = " << expl1.Current().ShapeType() << " TS = " << expl1.Current().TShape()
                      << std::endl;
...
#endif
            if (anExpl.Current().IsSame(Selection)) return anExpl.Current();
        }
    }
...
//==========================================================================
static Standard_Boolean ORientation(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
                                    const Handle(TNaming_NamedShape) & Stop, const Standard_Integer Index) {
...
// <=== end 21.10.2009
#ifdef OCCT_DEBUG_OR
                std::cout << "ORIENTATION: Selection TShape = " << CS.TShape() << " Orientation = " << CS.Orientation()
                          << std::endl;
                std::cout << "ORIENTATION: Context ShapeType = " << AS.ShapeType() << " TShape = " << AS.TShape()
                          << std::endl;
                ...
                if (!CS.IsNull()) {
                    ...
                    if (!Aggregation(aShape, AS, B)) return Standard_False;
                }
            }
        } else {
...
//=======================================================================
static Standard_Boolean WireIN(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
                               const Handle(TNaming_NamedShape) & Stop, Standard_Integer Index) {
    Standard_Boolean aResult(Standard_False);
    if (!ValidArgs(Args)) return aResult;
    TopTools_IndexedMapOfShape aMapOfSh;
    TDF_LabelMap aForbiden;
    if (Args.Extent() < 1) throw Standard_ConstructionError("TNaming_Name::Solve");
    const Handle(TNaming_NamedShape) & A = Args.First();
    ...
    if (Index == 1) { // Outer wire case
        ...
        TNaming_NamingTool::BuildDescendants(Stop, Forbiden); // fill Forbidden
        TNaming_NamingTool::CurrentShape(Valid, Forbiden, anIter.Value(),
                                         MS); // fill MS with last modifications of the first additional argument
        TopoDS_Shape CS = MakeShape(MS);
        ...
        for (int i = 1; it1.More(); it1.Next(), i++) {
            std::cout << "Map(" << i << "): TShape = " << it1.Key().TShape() << " Orient = " << it1.Key().Orientation()
                      << " Type = " << it1.Key().ShapeType() << std::endl;
        }
...
//===========================================================================
static Standard_Boolean ShellIN(const TDF_Label& L, const TDF_LabelMap& Valid, const TNaming_ListOfNamedShape& Args,
                                const Handle(TNaming_NamedShape) & Stop, Standard_Integer Index) {
    Standard_Boolean aResult(Standard_False);
    if (!ValidArgs(Args)) return aResult;
    TopTools_IndexedMapOfShape aMapOfSh;
