...
// purpose  :
//=======================================================================
static void LastModif(TNaming_NewShapeIterator& it, const TopoDS_Shape& S, TopTools_IndexedMapOfShape& MS,
                      const TDF_LabelMap& Updated, TDF_LabelList& Deleted) {
...
static void LastModif(TNaming_NewShapeIterator& it, TopTools_IndexedMapOfShape& MS, const TopoDS_Shape& S,
...
//=======================================================================
static void ApplyOrientation(TopTools_IndexedMapOfShape& MS, const TopAbs_Orientation OrientationToApply) {
    for (Standard_Integer anItMS = 1; anItMS <= MS.Extent(); ++anItMS) {
...
// purpose  :
//=======================================================================
TopoDS_Shape TNaming_Tool::CurrentShape(const Handle(TNaming_NamedShape) & Att, const TDF_LabelMap& Updated) {
...
static void FindModifUntil(TNaming_NewShapeIterator& it, TopTools_IndexedMapOfShape& MS,
...
// purpose  :
//=======================================================================
TopoDS_Shape TNaming_Tool::GeneratedShape(const TopoDS_Shape& S, const Handle(TNaming_NamedShape) & Generation)
...
// purpose  :
//=======================================================================
void TNaming_Tool::FirstOlds(const Handle(TNaming_UsedShapes) & US, const TopoDS_Shape& S, TNaming_OldShapeIterator& it,
                             TopTools_IndexedMapOfShape& MS, TDF_LabelList& Labels) {
...
TopoDS_Shape TNaming_Tool::InitialShape(const TopoDS_Shape& S, const TDF_Label& Acces, TDF_LabelList& Labels) {
    Handle(TNaming_UsedShapes) US;
...
static void Back(const Handle(TNaming_NamedShape) & NS, TNaming_MapOfNamedShape& MNS) {
...
void TNaming_Tool::Collect(const Handle(TNaming_NamedShape) & NS, TNaming_MapOfNamedShape& MNS,
...
// purpose  :
//=======================================================================
void TNamingTool_Write(const TopoDS_Shape& S, const Standard_CString File) {
...
void TNaming_Tool::FindShape(const TDF_LabelMap& Valid, const TDF_LabelMap& /*Forbiden*/,
                             const Handle(TNaming_NamedShape) & Arg, TopoDS_Shape& S) {
...
#ifdef OCCT_DEBUG
//    std::cout<<"TNaming_Tool::FindShape(): Naming attribute hasn't been found attached at the Argument
//    label"<<std::endl;
#endif
        return;
    }
...
//      Standard_Integer nbExtArgs = extArgs.Extent();
            ...
            if (!aCurrentExtArg.IsNull() && !aCurrentExtArg->IsEmpty()) extArgs.Append(aCurrentExtArg);
#ifdef OCCT_DEBUG
...
//      }
    ...
    for (; extArgsIterator.More(); extArgsIterator.Next()) {
        ...
        if (anExtArg->Label().Father().IsNull() ||
            ...
            aContextNS = anExtArg;
            // #ifdef OCCT_DEBUG
            //       std::cout<<"TNaming_Tool::FindShape(): A context shape hasn't been found at the father label of the
            //       external argument"<<std::endl;
            // #endif
            //       continue;
        }
...
#ifdef OCCT_DEBUG
    if (S.IsNull()) {
        std::cout << "TNaming_Tool::FindShape(): There hasn't been found a sub shape of the context shape coinciding "
                     "with the sub shapes of naming"
                  << std::endl;
    }
#endif
}
