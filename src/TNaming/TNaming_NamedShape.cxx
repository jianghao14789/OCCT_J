...
public:
    TNaming_Node(TNaming_PtrRefShape Old, TNaming_PtrRefShape New)
        : myOld(Old), myNew(New), myAtt(0L), nextSameAttribute(0L), nextSameOld(0L), nextSameNew(0L) {}
...
static void RemoveNode(Standard_Boolean MapExist, TNaming_DataMapOfShapePtrRefShape& M, TNaming_Node*& N) {
    ...
    if (pos != 0L) {
        if (pos->FirstUse() == N) {
            ...
            if (nextOld != 0L)
                ...
                // le shape disparait
                if (MapExist) M.UnBind(pos->Shape());
                N->myOld = 0L;
                if (pos != N->myNew) {
                    delete pos;
                    pos = 0L;
    ...
    if (pns != 0L) {
        if (pns->FirstUse() == N) {
            ...
            if (nextNew != 0L)
                ...
                // le shape disparait
                if (MapExist) M.UnBind(pns->Shape());
...
Handle(TDF_DeltaOnModification) TNaming_NamedShape::DeltaOnModification(const Handle(TDF_Attribute) &
                                                                        anOldAttribute) const {
...
void TNaming_NamedShape::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & Tab) const {
...
TNaming_Iterator::TNaming_Iterator(const Handle(TNaming_NamedShape) & Att) : myTrans(-1) {
    myNode = Att->myNode;
}
...
TNaming_Iterator::TNaming_Iterator(const TDF_Label& Lab) : myTrans(-1) {
...
TNaming_Iterator::TNaming_Iterator(const TDF_Label& Lab, const Standard_Integer Trans) : myTrans(Trans) {
...
Standard_Boolean TNaming_Iterator::IsModification() const {
    ...
    return (myNode->myAtt->myEvolution == TNaming_MODIFY || myNode->myAtt->myEvolution == TNaming_DELETE);
}
...
static void SelectSameShape(TNaming_Node*& myNode, Standard_Boolean Old, TNaming_RefShape*& RS,
...
TNaming_NewShapeIterator::TNaming_NewShapeIterator(const TopoDS_Shape& aShape, const Standard_Integer Trans,
...
TNaming_NewShapeIterator::TNaming_NewShapeIterator(const TNaming_Iterator& anIterator) : myTrans(anIterator.myTrans) {
    Standard_NoSuchObject_Raise_if(anIterator.myNode == 0L, "TNaming_NewShapeIterator::TNaming_NewShapeIterator");
...
TNaming_NewShapeIterator::TNaming_NewShapeIterator(const TopoDS_Shape& aShape, const TDF_Label& access) : myTrans(-1) {
    Handle(TNaming_UsedShapes) Shapes;
    if (access.Root().FindAttribute(TNaming_UsedShapes::GetID(), Shapes)) {
        Standard_NoSuchObject_Raise_if(!Shapes->Map().IsBound(aShape),
                                       "TNaming_NewShapeIterator::TNaming_NewShapeIterator aShape");
...
TNaming_OldShapeIterator::TNaming_OldShapeIterator(const TopoDS_Shape& aShape, const Standard_Integer Trans,
                                                   const Handle(TNaming_UsedShapes) & Shapes)
    : myTrans(Trans) {
    Standard_NoSuchObject_Raise_if(!Shapes->Map().IsBound(aShape),
...
TNaming_OldShapeIterator::TNaming_OldShapeIterator(const TopoDS_Shape& aShape, const TDF_Label& access) : myTrans(-1) {
    Handle(TNaming_UsedShapes) Shapes;
    if (access.Root().FindAttribute(TNaming_UsedShapes::GetID(), Shapes)) {
        Standard_NoSuchObject_Raise_if(!Shapes->Map().IsBound(aShape),
                                       "TNaming_OldShapeIterator::TNaming_OldShapeIterator aShape");
...
TNaming_OldShapeIterator::TNaming_OldShapeIterator(const TNaming_Iterator& anIterator) : myTrans(anIterator.myTrans) {
    Standard_NoSuchObject_Raise_if(anIterator.myNode == 0L, "TNaming_OldShapeIterator::TNaming_OldShapeIterator");
    myNode = anIterator.myNode;
    TNaming_RefShape* RS = myNode->myNew;
    if (RS == 0L)
        myNode = 0L; // No descendant
...
TNaming_SameShapeIterator::TNaming_SameShapeIterator(const TopoDS_Shape& aShape, const TDF_Label& access) {
    Handle(TNaming_UsedShapes) Shapes;
    if (access.Root().FindAttribute(TNaming_UsedShapes::GetID(), Shapes)) {
        TNaming_RefShape* RS = Shapes->Map().ChangeFind(aShape);
        myNode = RS->FirstUse();
...
TDF_Label TNaming_Tool::Label(const TDF_Label& access, const TopoDS_Shape& S, Standard_Integer& Trans) {
...
Standard_Boolean TNaming_Tool::HasLabel(const Handle(TNaming_UsedShapes) & Shapes, const TopoDS_Shape& S) {
...
TDF_Label TNaming_Tool::Label(const Handle(TNaming_UsedShapes) & Shapes, const TopoDS_Shape& S,
...
Standard_Integer TNaming_Tool::ValidUntil(const TopoDS_Shape& S, const Handle(TNaming_UsedShapes) & US) {
