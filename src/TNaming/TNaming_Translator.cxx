...
const TopoDS_Shape TNaming_Translator::Copied(const TopoDS_Shape& aShape) const {
    TopoDS_Shape aResult;
    if (myDataMapOfResults.IsBound(aShape)) aResult = myDataMapOfResults.Find(aShape);
    return aResult;
}
...
void TNaming_Translator::Perform() {
    ...
    for (; itm.More(); itm.Next()) {
        ...
        if (!Result.IsNull()) myDataMapOfResults(itm.Key()) = Result;
        Result.Nullify();
    }
...
void TNaming_Translator::DumpMap(const Standard_Boolean isWrite) const {
    ...
    for (Standard_Integer i = 1; i <= myMap.Extent(); i++) {
        ...
        if ((T == STANDARD_TYPE(BRep_TVertex)) || (T == STANDARD_TYPE(BRep_TEdge)) || T == STANDARD_TYPE(BRep_TFace) ||
            T == STANDARD_TYPE(TopoDS_TWire) || T == STANDARD_TYPE(TopoDS_TShell) ||
            T == STANDARD_TYPE(TopoDS_TSolid) || T == STANDARD_TYPE(TopoDS_TCompSolid) ||
            T == STANDARD_TYPE(TopoDS_TCompound)) {
            if (isWrite) {
