...
static void SubstituteShape(const TopoDS_Shape& oldShape, const TopoDS_Shape& newShape,
...
void TNaming::Substitute(const TDF_Label& LSource, const TDF_Label& LCible, TopTools_DataMapOfShapeShape& M) {
...
Standard_Boolean TNaming::SubstituteSShape(const TDF_Label& Lab, const TopoDS_Shape& From, TopoDS_Shape& To) {
    ...
    TNaming_DataMapOfShapePtrRefShape& amap = US->Map();
    if (!amap.IsBound(To)) return Standard_False;
    TNaming_RefShape* pos;
...
static void BuildMap(const TDF_Label& L, BRepBuilderAPI_Transform& Transformer, TopTools_DataMapOfShapeShape& M) {
    Handle(TNaming_UsedShapes) US;
    L.Root().FindAttribute(TNaming_UsedShapes::GetID(), US);
    for (TNaming_Iterator it(L); it.More(); it.Next()) {
        if (!it.OldShape().IsNull()) {
...
static void LoadNamedShape(TNaming_Builder& B, TNaming_Evolution Evol, const TopoDS_Shape& OS, const TopoDS_Shape& NS) {
    switch (Evol) {
...
void TNaming::Displace(const TDF_Label& L, const TopLoc_Location& Loc, const Standard_Boolean WithOld) {
...
void TNaming::Replicate(const TopoDS_Shape& SH, const gp_Trsf& T, const TDF_Label& L) {
    ...
    if (SH.ShapeType() == TopAbs_FACE || SH.ShapeType() == TopAbs_WIRE) SST = TopAbs_EDGE;
...
#ifdef OCCT_DEBUG
        if (!Map.Bind(aKey, C0)) std::cout << "Not bind = " << aKey.ShapeType() << std::endl;
#else
        Map.Bind(aKey, C0);
#endif
...
static void BuildMap(const TopoDS_Shape& Context, const TopAbs_ShapeEnum StopType,
    ...
    while (anIt.More()) {
        const TopoDS_Shape& aKey = anIt.Value();
#ifdef OCCT_DEBUG
        if (!Map.Bind(aKey, Context)) std::cout << "Not bind = " << aKey.ShapeType() << std::endl;
#else
...
TopoDS_Shape TNaming::FindUniqueContext(const TopoDS_Shape& Selection, const TopoDS_Shape& Context) {
    ...
    for (; it.More(); it.Next()) {
        std::cout << "FindUniqueContext: Key - " << it.Key().ShapeType() << " " << it.Key().TShape().get()
                  << " OR = " << it.Key().Orientation() << "  Context - " << it.Value().ShapeType() << "  "
                  << it.Value().TShape().get() << " OR = " << it.Value().Orientation() << std::endl;
    }
#endif
    if (aMap.IsBound(Selection)) return aMap.Find(Selection);
    return TopoDS_Shape();
}
...
TopoDS_Shape TNaming::FindUniqueContextSet(const TopoDS_Shape& Selection, const TopoDS_Shape& Context,
    ...
    if (Selection.ShapeType() == TopAbs_COMPOUND) {
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            if (aS.ShapeType() > aStopType) aStopType = aS.ShapeType();
            Up++;
        }
        if (Up > 0) Arr = new TopTools_HArray1OfShape(1, Up);
        if (aStopType == TopAbs_SHAPE) aStopType = Selection.ShapeType();
        ...
        if (aMap.IsBound(Selection))
            ...
            for (; it.More(); it.Next(), num1++) {
                if (aMap.IsBound(it.Value())) {
                    ...
                    if (!Arr.IsNull()) Arr->SetValue(num1 + 1, aMap.Find(it.Value()));

                    if (aMap.Find(it.Value()) == Context) num2++;
                }
            }
            if (num1 == num2 && num2)
                return Context;
...
Standard_Boolean TNaming::OuterShell(const TopoDS_Solid& theSolid, TopoDS_Shell& theShell) {
    ...
    for (; aIt.More(); aIt.Next()) {
        ...
        if (aSx.ShapeType() != TopAbs_SHELL) continue;
        if (IsInternal(aSx)) continue;
        //
