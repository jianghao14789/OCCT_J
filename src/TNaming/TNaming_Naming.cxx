...
static void Write(const TopoDS_Shape& shape, const Standard_CString filename) {
    ...
    if (!save) std::cout << "File " << buf << " was not created: rdstate = " << save.rdstate() << std::endl;
...
//==========================================================================================
static Handle(TNaming_NamedShape)
    BuildName(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& S, const TopoDS_Shape& Context,
              const Handle(TNaming_NamedShape) & Stop, const Standard_Boolean Geometry);

//=======================================================================
static Standard_Integer RepeatabilityInContext(const TopoDS_Shape& Selection, const TopoDS_Shape& Context);
...
static Handle(TNaming_NamedShape) BuildNS(const TDF_Label& F, const TopoDS_Shape& S, const TNaming_NameType& Name) {
...
static Standard_Boolean CompareInGeneration(const Handle(TNaming_NamedShape) & NS, const TopoDS_Shape& S) {
                                                        ...
                                                        const TopoDS_Shape& S)                 // target
{
    ...
    if (GetShapeEvolutions(S, aSource, aList) && aList.Extent() > 0) {
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            TNaming_Iterator aNIter2(
                aResult); // if some another shapes has oldshape from map, return namedshape with this oldshape

            for (; aNIter2.More(); aNIter2.Next()) {
                if (aNIter2.NewShape().IsSame(anIter.Value())) continue;
                if (aMap.Contains(aNIter2.OldShape())) { // if one shape was modified to the two at the shared label,
                                                         // return this one
...
static Standard_Boolean Compare(const Handle(TNaming_NamedShape) & NS, const TNaming_Scope& MDF,
                                const Handle(TNaming_NamedShape) & Stop, const TopoDS_Shape& S) {
...
static Standard_Boolean TestSolution(const TNaming_Scope& MDF, const Handle(TNaming_NamedShape) & NS,
    ...
    if ((S.ShapeType() == TopAbs_FACE || S.ShapeType() == TopAbs_EDGE || S.ShapeType() == TopAbs_VERTEX) &&
        ...
        if (Res.ShapeType() == TopAbs_SOLID || Res.ShapeType() == TopAbs_COMPSOLID ||
            ...
            if (S.IsSame(Res)) return Standard_True;
...
static void FindNewShapeInFather(const Handle(TNaming_NamedShape) & NS, TopoDS_Shape& SC) {
    ...
    if (itLab.More()) SC = itLab.NewShape();
}
...
//=======================================================================
static Standard_Boolean IsMultipleCase(const TopoDS_Shape& S, const TopoDS_Shape& Context,
    ...
    for (; it.More(); it.Next()) {
        if (aDM.Contains(it.Key())) {
            ...
            if (aMS.Extent()) aDMM.Bind(it.Key(), aMS);
        } else {
#ifdef OCCT_DEBUG
...
static Standard_Boolean Filter(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& S,
                               const TopoDS_Shape& Context, TNaming_Localizer& Localizer,
                               Handle(TNaming_NamedShape) & NS, const Standard_Integer Lev) {
    // 1. Localizer.FindNeighbourg("Context", "S", "Neighbourg") - looks for neighbourgs of  "S" with the same shape
    // type in the "Context"
    //    shape and adds them to the "Neighbourg" map;
    // 2. If "Neighbourg" map is empty, tries to do the same with the new context shape: shape from the father label of
    // the "S" named shape;
    // 3. If "Neighbourg" map is still empty, returns false;
    // 4. Adds to the new child of label "L" new TNaming_Naming attribute with the next properties name: shape type is
    // shape type of the "S",
    ...
    // 5. Adds to the Name all shapes from "Neighbourg" map: build it with the BuildName( new generated TNaming_Naming
    // attribute, MDF,
    //    argument shape from "Neighbourg", "Context", NextModif( "Until" ), true ) method.
    // 6. Creates resulting NamedShape with the "Regenerate" ( it just calls TName::Solve method ) method from
    // TNaming_Naming class.
    // 7. If Compare( result NamedShape, MDF, stop, "S" ) returns true, "Filter" method returns true, else returns
    // false.
    ...
    for (; itN.More(); itN.Next()) {
        ...
        if (allowChild && !aSNS.IsNull() && aS.ShapeType() != aSNS.ShapeType() &&
            aSNS.ShapeType() == TopAbs_COMPOUND) { // aLev < 3
#ifdef OCCT_DEBUG_NBS
            ...
            if (!aNaming.IsNull()) {
                ...
                if (aName.Type() == TNaming_GENERATION) StandardFilter = Standard_False;
                if (StandardFilter)
...
//=======================================================================
static Handle(TNaming_NamedShape)
    BuildNameInNS(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& S,
                  const Handle(TNaming_NamedShape) & Context, const Handle(TNaming_NamedShape) & Stop,
...
#ifdef OCCT_DEBUG_INNS
    if (NewStop.IsNull()) std::cout << "BuildNameInNS:: NewStop shape is  NULL" << std::endl;
#endif
...
static Handle(TNaming_NamedShape)
    BuildName(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& Selection, const TopoDS_Shape& Context,
              const Handle(TNaming_NamedShape) & Stop, const Standard_Boolean Geom) {
...
#endif
                if (theName.Type() == TNaming_GENERATION)
                    theName.Append(BuildNameInNS(Naming->Label(), MDF, Ident.ShapeArg(), Ident.NamedShapeOfGeneration(),
                                                 Stop, Geom));
                else
    ...
    if (OnlyOne) {
        ...
        if (itNS.More()) {
            ...
            if (IsGeneration) {
                ...
                Print_Entry(Ident.Type() == TNaming_MODIFUNTIL ? NS->Label()
                                                               : Naming->ChangeName().Arguments().First()->Label());
                std::cout << "Selection type = " << Selection.ShapeType() << " TS = " << Selection.TShape()->This()
                          << std::endl;
#endif
                Handle(TNaming_NamedShape) NewNS = CompareInModification(
                    Ident.Type() == TNaming_MODIFUNTIL ? NS : Naming->ChangeName().Arguments().First(), Selection);
                if (!NewNS.IsNull()) { // there is need to describe name in detail: modification with type 1:n in the
                                       // same label
...
//=======================================================================
static Standard_Boolean HasAncFace(const TopoDS_Shape& Context, const TopoDS_Shape& W, TopoDS_Shape& Face,
                                   Standard_Boolean& isOuter) {
    Standard_Boolean hasFace(Standard_False);
    if (W.ShapeType() != TopAbs_WIRE) return hasFace;
...
static Handle(TNaming_NamedShape)
    BuildNameWire(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& Selection, const TopoDS_Shape& Context,
                  const Handle(TNaming_NamedShape) & Stop, const Standard_Boolean Geom) {
    Handle(TNaming_NamedShape) NS;
...
static Standard_Boolean IsAllIn(const TopoDS_Shape& S, const TopoDS_Shape& Context) {
    ...
    if (num1 == num2) found = Standard_True;
#ifdef OCCT_DEBUG_CC
    else
        ...
        if (Context.ShapeType() < Selection.ShapeType()) {
            if (Selection.ShapeType() != TopAbs_SHELL) {
                for (TopExp_Explorer exp(Context, Selection.ShapeType()); exp.More(); exp.Next()) {
                    if (exp.Current().IsSame(Selection)) aNum++;
                }
            }
...
//=======================================================================
static Standard_Boolean HasAncSolid(const TopoDS_Shape& Context, const TopoDS_Shape& Sh, TopoDS_Shape& Solid,
                                    Standard_Boolean& isOuter) {
    Standard_Boolean hasSolid(Standard_False);
    if (Sh.ShapeType() != TopAbs_SHELL) return hasSolid;
...
static Handle(TNaming_NamedShape)
    BuildNameShell(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& Selection, const TopoDS_Shape& Context,
                   const Handle(TNaming_NamedShape) & Stop, const Standard_Boolean Geom) {
    Handle(TNaming_NamedShape) NS;
    Standard_Boolean found(Standard_False);
    Handle(TNaming_Naming) Naming;
    ...
    if (hasSolid && Selection.ShapeType() > Context.ShapeType()) {
        ...
        if (Context.ShapeType() == TopAbs_SOLID) {
            ...
            if (found) {
                ...
                if (!aNS.IsNull()) theName.ContextLabel(aNS->Label());
                theName.Append(aNS);
                if (isOuter) {
                    theName.Index(1);
                } else { // not OuterShell
            ...
            if (isOuter) {
                ...
                Handle(TNaming_NamedShape) aNS = TNaming_Tool::NamedShape(Context, F);
                if (!aNS.IsNull()) theNameSo.ContextLabel(aNS->Label());
...
// purpose  :
//=======================================================================
static void BuildAggregationName(const TDF_Label& F, TNaming_Scope& MDF, const TopoDS_Shape& S,
                                 const TopoDS_Shape& Context, const Handle(TNaming_NamedShape) & Stop,
    ...
    for (TopoDS_Iterator itc(S); itc.More(); itc.Next()) {
        ...
        if ((aS.ShapeType() == TopAbs_SOLID && !TNaming_Tool::NamedShape(aS, Naming->Label()).IsNull()) ||
            aS.ShapeType() == TopAbs_FACE || aS.ShapeType() == TopAbs_EDGE || aS.ShapeType() == TopAbs_VERTEX) {
            ...
            if (found2) {
                ...
                if (!aNS.IsNull()) aNaming->ChangeName().ContextLabel(aNS->Label());
            }
...
Handle(TNaming_NamedShape) TNaming_Naming::Name(const TDF_Label& F, const TopoDS_Shape& S, const TopoDS_Shape& Context,
                                                const Standard_Boolean Geom, const Standard_Boolean KeepOrientation,
...

{
                        ...
                        for (; itw.More(); itw.Next()) {
                            ...
                            for (int i = 1; it.More(); it.Next(), i++) {
                                if (it.Value().IsEqual(S)) {
                                    theName.Index(i); // We use this field to save a Seam Shape Index; Before this field
                                                      // was used for GENERATED only
                                    found = Standard_True;
#ifdef MDTV_OR
    ...
    if ((S.ShapeType() == TopAbs_SOLID && !TNaming_Tool::NamedShape(S, F).IsNull()) ||
        ...
        S.ShapeType() == TopAbs_FACE || S.ShapeType() == TopAbs_EDGE || S.ShapeType() == TopAbs_VERTEX) {
        //---------------------------------------
        ...
        theName.Shape(S);
        theName.Orientation(S.Orientation());
        if (S.ShapeType() != TopAbs_WIRE) theName.Type(TNaming_UNION);
        ...
        Standard_Boolean found(Standard_False);
        if (!Context.IsNull()) {
            if (Context.ShapeType() < S.ShapeType()) found = IsOneIn(S, Context);
            if (found) {
                NS = TNaming_Tool::NamedShape(Context, F);
                if (!NS.IsNull()) theName.ContextLabel(NS->Label());
            }
        }
...
void TNaming_Naming::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & RT) const {
...
void TNaming_Naming::ExtendedDump(Standard_OStream& anOS, const TDF_IDFilter& /*aFilter*/,
