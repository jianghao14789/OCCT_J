...
void Intf_Tool::Lin2dBox(const gp_Lin2d& L2d, const Bnd_Box2d& domain, Bnd_Box2d& boxLin) {
    ...
    if (L2d.Direction().XY().X() > 0.) {
        ...
        if (L2d.Location().XY().X() < xmin || xmax < L2d.Location().XY().X()) return;
    ...
    if (L2d.Direction().XY().Y() > 0.) {
        ...
        if (L2d.Location().XY().Y() < ymin || ymax < L2d.Location().XY().Y()) return;
...
void Intf_Tool::Parab2dBox(const gp_Parab2d& theParab2d, const Bnd_Box2d& domain, Bnd_Box2d& boxParab2d) {
    nbSeg = 0;
    boxParab2d.SetVoid();
    if (domain.IsWhole()) {
        boxParab2d.SetWhole();
...
void Intf_Tool::LinBox(const gp_Lin& L, const Bnd_Box& domain, Bnd_Box& boxLin) {
    ...
    if (L.Direction().XYZ().X() > 0.) {
        ...
        if (L.Location().XYZ().X() < xmin || xmax < L.Location().XYZ().X()) return;
    ...
    if (L.Direction().XYZ().Y() > 0.) {
        ...
        if (L.Location().XYZ().Y() < ymin || ymax < L.Location().XYZ().Y()) return;
    ...
    if (L.Direction().XYZ().Z() > 0.) {
        ...
        if (L.Location().XYZ().Z() < zmin || zmax < L.Location().XYZ().Z()) return;
