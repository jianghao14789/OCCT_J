...
Standard_Boolean Intf_TangentZone::IsEqual(const Intf_TangentZone& Other) const {
    if (Result.Length() != Other.Result.Length()) return Standard_False;
    Standard_Integer i;
    for (i = 1; i <= Result.Length(); i++) {
        if (!Result(i).IsEqual(Other.Result(i))) return Standard_False;
    }
...
Standard_Boolean Intf_TangentZone::Contains(const Intf_SectionPoint& ThePI) const {
    Standard_Integer i;
    for (i = 1; i <= Result.Length(); i++)
        if (ThePI.IsEqual(Result(i))) return Standard_True;
...
void Intf_TangentZone::InfoFirst(Standard_Integer& segMin, Standard_Real& paraMin, Standard_Integer& segMax,
...
void Intf_TangentZone::InfoSecond(Standard_Integer& segMin, Standard_Real& paraMin, Standard_Integer& segMax,
                                  Standard_Real& paraMax) const {
