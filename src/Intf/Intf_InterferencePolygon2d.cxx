...
Intf_InterferencePolygon2d::Intf_InterferencePolygon2d()
    : Intf_Interference(Standard_False), oClos(Standard_False), tClos(Standard_False), nbso(0) {}
...
Intf_InterferencePolygon2d::Intf_InterferencePolygon2d(const Intf_Polygon2d& Obje1, const Intf_Polygon2d& Obje2)
    : Intf_Interference(Standard_False), oClos(Standard_False), tClos(Standard_False), nbso(0) {
...
Intf_InterferencePolygon2d::Intf_InterferencePolygon2d(const Intf_Polygon2d& Obje)
    : Intf_Interference(Standard_True), oClos(Standard_False), tClos(Standard_False), nbso(0) {
...
void Intf_InterferencePolygon2d::Interference(const Intf_Polygon2d& Obje1, const Intf_Polygon2d& Obje2) {
    Bnd_Box2d bSO;
    Bnd_Box2d bST;
    ...
    for (iObje1 = 1; iObje1 <= n1; iObje1++) {
        ...
        if (!Obje2.Bounding().IsOut(bSO)) {
            for (iObje2 = 1; iObje2 <= n2; iObje2++) {
                ...
                bST.Add(p2e);
                bST.Enlarge(d2);
                if (!bSO.IsOut(bST)) Intersect(iObje1, iObje2, p1b, p1e, p2b, p2e);
            }
        }
    }
}
...
void Intf_InterferencePolygon2d::Intersect(const Standard_Integer iObje1, const Standard_Integer iObje2,
                                           const gp_Pnt2d& BegO, const gp_Pnt2d& EndO, const gp_Pnt2d& BegT,
                                           const gp_Pnt2d& EndT) {
    if (SelfIntf) {
    ...
    if (Abs(dbOT) <= Tolerance) {
        if (dbObT <= Tolerance) {
            ...
            parO[nbpi] = 0.;
            parT[nbpi] = 0.;
            thePi.Append(Intf_SectionPoint(BegO, Intf_VERTEX, iObje1, 0., Intf_VERTEX, iObje2, 0., sinTeta));
        }
        if (dbOeT <= Tolerance) {
            nbpi++;
            parO[nbpi] = 0.;
            parT[nbpi] = 1.;
            thePi.Append(Intf_SectionPoint(BegO, Intf_VERTEX, iObje1, 0., Intf_VERTEX, iObje2 + 1, 0., sinTeta));
        }
        if (dbObT > Tolerance && dbOeT > Tolerance && dbObT + dbOeT <= (lgT + Tolerance)) {
            nbpi++;
            parO[nbpi] = 0.;
            parT[nbpi] = dbObT / lgT;
            thePi.Append(Intf_SectionPoint(BegO, Intf_VERTEX, iObje1, 0., Intf_EDGE, iObje2, parT[nbpi], sinTeta));
        }
    }
    ...
    if (Abs(deOT) <= Tolerance) {
        if (deObT <= Tolerance) {
            ...
            parO[nbpi] = 1.;
            parT[nbpi] = 0.;
            thePi.Append(Intf_SectionPoint(EndO, Intf_VERTEX, iObje1 + 1, 0., Intf_VERTEX, iObje2, 0., sinTeta));
        }
        if (deOeT <= Tolerance) {
            nbpi++;
            parO[nbpi] = 1.;
            parT[nbpi] = 1.;
            thePi.Append(Intf_SectionPoint(EndO, Intf_VERTEX, iObje1 + 1, 0., Intf_VERTEX, iObje2 + 1, 0., sinTeta));
        }
        if (deObT > Tolerance && deOeT > Tolerance && deObT + deOeT <= (lgT + Tolerance)) {
            nbpi++;
            parO[nbpi] = 1.;
            parT[nbpi] = deObT / lgT;
            thePi.Append(Intf_SectionPoint(EndO, Intf_VERTEX, iObje1 + 1, 0., Intf_EDGE, iObje2, parT[nbpi], sinTeta));
        }
    }
    ...
    if (Abs(dbTO) <= Tolerance) {
        if (dbObT > Tolerance && deObT > Tolerance && dbObT + deObT <= (lgO + Tolerance)) {
            ...
            parO[nbpi] = dbObT / lgO;
            parT[nbpi] = 0.;
            thePi.Append(Intf_SectionPoint(BegT, Intf_EDGE, iObje1, parO[nbpi], Intf_VERTEX, iObje2, 0., sinTeta));
        }
    }
    ...
    if (Abs(deTO) <= Tolerance) {
        if (dbOeT > Tolerance && deOeT > Tolerance && dbOeT + deOeT <= (lgO + Tolerance)) {
            ...
            parO[nbpi] = dbOeT / lgO;
            parT[nbpi] = 1.;
            thePi.Append(Intf_SectionPoint(EndT, Intf_EDGE, iObje1, parO[nbpi], Intf_VERTEX, iObje2 + 1, 0., sinTeta));
        }
    }
    ...
    if (Abs(dbOT - deOT) > floatgap && Abs(dbTO - deTO) > floatgap) {
        ...
        if (nbpi == 0 && sinTeta > PRCANG) {
            ...
            parO[nbpi] = parOSP;
            parT[nbpi] = parTSP;
            thePi.Append(Intf_SectionPoint(gp_Pnt2d(BegO.X() + (segO.X() * parOSP), BegO.Y() + (segO.Y() * parOSP)),
                                           Intf_EDGE, iObje1, parOSP, Intf_EDGE, iObje2, parTSP, sinTeta));
        }
        ...
        // Otherwise it is required to check if there is no other
            ...
            if (nbpi == 0) {
                ...
                while (nbpi < 2) {
                    ...
                    thePi.Append(Intf_SectionPoint(gp_Pnt2d(x, y), Intf_EXTERNAL, iObje1, parO[nbpi], Intf_EXTERNAL,
                                                   iObje2, parT[nbpi], sinTeta));
                }
            } else { // nbpi>0
                if (nbpi == 1) {
                    ...
                    if (ok) {
                        ...
                        if (thePi(1).Pnt().Distance(gp_Pnt(x, y, 0)) >= (Tolerance / 4.)) {
                            nbpi++;
                            thePi.Append(Intf_SectionPoint(gp_Pnt2d(x, y), Intf_EXTERNAL, iObje1, parO[nbpi],
                                                           Intf_EXTERNAL, iObje2, parT[nbpi], sinTeta));
                        }
                    }
                    ...
                    if ((parOdeb < parOmin && parOmin > 0.) || (sigPS > 0. && parTdeb < parTmin && parTmin > 0.) ||
                    ...
                    if ((parOfin > parOmax && parOmax < 1.) || (sigPS < 0. && parTfin < parTmin && parTmin > 0.) ||
    ...
    if (nbpi == 1) {
        if (edgeSP) {
            thePi(1) = Intf_SectionPoint(gp_Pnt2d(BegO.X() + (segO.X() * parOSP), BegO.Y() + (segO.Y() * parOSP)),
                                         Intf_EDGE, iObje1, parOSP, Intf_EDGE, iObje2, parTSP, sinTeta);
            parO[1] = parOSP;
            parT[1] = parTSP;
        }
        if (!SelfIntf) {
            ...
            if (!contains) mySPoins.Append(thePi(1));
        ...
        if (edgeSP)
            TheTZ.PolygonInsert(
                Intf_SectionPoint(gp_Pnt2d(BegO.X() + (segO.X() * parOSP), BegO.Y() + (segO.Y() * parOSP)), Intf_EDGE,
                                  iObje1, parOSP, Intf_EDGE, iObje2, parTSP, sinTeta));
