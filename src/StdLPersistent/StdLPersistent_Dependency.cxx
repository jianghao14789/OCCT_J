...
static void ImportName(const Handle(TDataStd_Expression) & theAttribute, const TCollection_ExtendedString& theName) {
    theAttribute->SetExpression(theName);
}

static void ImportName(const Handle(TDataStd_Relation) & theAttribute, const TCollection_ExtendedString& theName) {
...
void StdLPersistent_Dependency::instance<AttribClass>::Import(const Handle(AttribClass) & theAttribute) const {
    if (myName) ImportName(theAttribute, myName->Value()->String());
    ...
    if (myVariables) {
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            if (aPersistent) theAttribute->GetVariables().Append(aPersistent->GetAttribute());
        }
    }
}
