...
public:
    //! Empty constructor.
    StdLPersistent_Real() : myValue(0.0), myDimension(0) {}
    ...
    Standard_CString PName() const {
        return "PDataStd_Real";
    }

    //! Import transient attribute from the persistent data.
    void Import(const Handle(TDataStd_Real) & theAttribute) const {
        theAttribute->Set(myValue);
        Standard_DISABLE_DEPRECATION_WARNINGS theAttribute->SetDimension(static_cast<TDataStd_RealEnum>(myDimension));
