...
void PCDM_ReadWriter_1::WriteReferences(const Handle(Storage_Data) & aData, const Handle(CDM_Document) & aDocument,
...
Standard_Integer PCDM_ReadWriter_1::ReadReferenceCounter(const TCollection_ExtendedString& aFileName,
    ...
    if (PCDM::FileDriverType(aFileNameU, theFileDriver) == PCDM_TOFD_Unknown) return theReferencesCounter;
        ...
        for (i = 1; i <= refUserInfo.Length(); i++) {
            if (refUserInfo(i).Search(REFERENCE_COUNTER) != -1) {
                try {
                    ...
                    if (!theMsgDriver.IsNull()) theMsgDriver->Send(aMsg.ToExtString());
                }
            }
        }
    ...
    for (Standard_Integer i = 1; i <= ReadReferences.Length(); i++) {
        ...
        if (pos != -1) {
            ...
            if (!theMsgDriver.IsNull()) {
                //      std::cout << "reference found; ReferenceIdentifier: " << theReferenceIdentifier << "; File:" <<
                //      thePath << ", version:" << theDocumentVersion;
                TCollection_ExtendedString aMsg("Warning: ");
...
void PCDM_ReadWriter_1::ReadUserInfo(const TCollection_ExtendedString& aFileName, const TCollection_AsciiString& Start,
                                     const TCollection_AsciiString& End, TColStd_SequenceOfExtendedString& theUserInfo,
                                     const Handle(Message_Messenger) &) {
    Standard_Integer i;
    Handle(Storage_BaseDriver) theFileDriver;
    TCollection_AsciiString aFileNameU(aFileName);
    if (PCDM::FileDriverType(aFileNameU, theFileDriver) == PCDM_TOFD_Unknown) return;
...
Standard_Integer PCDM_ReadWriter_1::ReadDocumentVersion(const TCollection_ExtendedString& aFileName,
    ...
    Handle(Storage_BaseDriver) theFileDriver;
    TCollection_AsciiString aFileNameU(aFileName);
    if (PCDM::FileDriverType(aFileNameU, theFileDriver) == PCDM_TOFD_Unknown) return theVersion;
