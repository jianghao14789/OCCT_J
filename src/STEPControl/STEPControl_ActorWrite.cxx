...
static Standard_Boolean IsManifoldShape(const TopoDS_Shape& theShape) {
    ...
    for (; anIt.More(); anIt.Next()) {
        ...
        if (aDirectChild.ShapeType() != TopAbs_COMPOUND) aBrepBuilder.Add(aDirectShapes, aDirectChild);
    }
...
STEPControl_ActorWrite::STEPControl_ActorWrite() : mygroup(0), mytoler(-1.) {
...
Handle(StepShape_NonManifoldSurfaceShapeRepresentation) STEPControl_ActorWrite::getNMSSRForGroup(
    const Handle(TopTools_HSequenceOfShape) & shapeGroup, const Handle(Transfer_FinderProcess) & FP,
...
void STEPControl_ActorWrite::mergeInfoForNM(const Handle(Transfer_FinderProcess) & theFP,
    ...
    for (; aShapeShapeIt.More(); aShapeShapeIt.Next()) {
        ...
        if (anOrig.ShapeType() != TopAbs_FACE) continue;
        ...
        if (anOrigBinder.IsNull()) continue;
...
static Standard_Real UsedTolerance(const Standard_Real mytoler, const TopoDS_Shape& theShape) {
    ...
    if (Tol <= 0 && tolmod == 2) Tol = Interface_Static::RVal("write.precision.val");
    if (Tol <= 0) {
...
Standard_Boolean STEPControl_ActorWrite::IsAssembly(TopoDS_Shape& S) const {
    ...
    if (Interface_Static::IVal("write.step.vertex.mode") == 0) { // bug 23950
        if (S.ShapeType() == TopAbs_COMPOUND) {
            ...
            if (IsOnlyVertices) return Standard_False;
        }
    }
...
static Standard_Boolean transferVertex(const Handle(Transfer_FinderProcess) & FP,
                                       ...
                                       const TopoDS_Shape& aShVrtx, const Standard_Integer theNum) {
...
Handle(Transfer_Binder) STEPControl_ActorWrite::TransferShape(
    const Handle(Transfer_Finder) & start, const Handle(StepShape_ShapeDefinitionRepresentation) & SDR0,
    const Handle(Transfer_FinderProcess) & FP, const Handle(TopTools_HSequenceOfShape) & shapeGroup,
    ...
    if (!binder.IsNull()) {
        if (!binder->HasResult()) binder.Nullify();
    }
    if (!binder.IsNull()) {
    ...
    if (IsAssembly(theShape)) return TransferCompound(start, SDR0, FP, theProgress);
    ...
    if (isNMMode && !GroupMode() && theShape.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (aNMItemsNb > 0) {
            ...
            if (isOnlyNonManifold) return aNMBinder;
        }
    }
    // [END] Separate manifold topology from non-manifold in group mode 0 (ssv; 18.11.2010)

    if (aPSRoot.UserBreak()) return Handle(Transfer_Binder)();

    // create a list of items to translate
    Handle(TopTools_HSequenceOfShape) RepItemSeq = new TopTools_HSequenceOfShape();

    Standard_Boolean isSeparateVertices = Interface_Static::IVal("write.step.vertex.mode") == 0; // bug 23950
    ...
    if (theShape.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (isSeparateVertices) {
            ...
            // replace the shapes
            if (countSh) theShape = aNewShape;
            if (countVrtx) RepItemSeq->Append(aCompOfVrtx);
            if (countSh == 0) isOnlyVertices = Standard_True;
        }
    }
    ...
    if (theShape.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (mymode != STEPControl_GeometricCurveSet) {
            ...
            if (!isOnlyVertices) RepItemSeq->Append(theShape); //: j1
        }
    ...
    for (Standard_Integer i = 1; i <= nbs && aPS.More(); i++) {
        ...
        TopoDS_Shape aShape;
        aShape = XSAlgo::AlgoContainer()->ProcessShape(xShape, Tol, maxTol, "write.step.resource.name",
                                                       "write.step.sequence", info, aPS1.Next());
        if (aPS1.UserBreak()) return Handle(Transfer_Binder)();
        ...
        switch (trmode) {
            ...
            case STEPControl_FacetedBrepAndBrepWithVoids: {
                ...
                if (aShape.ShapeType() == TopAbs_SOLID) {
                    TopoDS_Solid aSolid = TopoDS::Solid(aShape);
                    TopoDSToStep_MakeFacetedBrepAndBrepWithVoids MkFacetedBrepAndBrepWithVoids(aSolid, FP, aPS1.Next());
            ...
            case STEPControl_ShellBasedSurfaceModel: {
                if (aShape.ShapeType() == TopAbs_SOLID) {
                    TopoDS_Solid aSolid = TopoDS::Solid(aShape);
                    TopoDSToStep_MakeShellBasedSurfaceModel MkShellBasedSurfaceModel(aSolid, FP, aPS1.Next());
                    ...
                    TopoDSToStep_MakeShellBasedSurfaceModel MkShellBasedSurfaceModel(aFace, FP, aPS1.Next());
                    MkShellBasedSurfaceModel.Tolerance() = Tol;
                    if (MkShellBasedSurfaceModel.IsDone()) {
                        item = MkShellBasedSurfaceModel.Value();
                    }
            ...
            case STEPControl_GeometricCurveSet: {
                ...
                // modified by PTV 16.09.2002 OCC725
                    ...
                    for (; anExp.More(); anExp.Next()) {
                        if (anExp.Current().ShapeType() != TopAbs_VERTEX) continue;
                        aNbVrtx++;
                    }
                    if (aNbVrtx) {
                        ...
                        for (anExp.ReInit(); anExp.More(); anExp.Next()) {
                            ...
                            if (aVertex.ShapeType() != TopAbs_VERTEX) continue;
                            curNb++;
        ...
        if (xShape.IsSame(mapper->Value())) submapper = Handle(TransferBRep_ShapeMapper)::DownCast(start);
    ...
    Handle(StepRepr_HArray1OfRepresentationItem) items = new StepRepr_HArray1OfRepresentationItem(1, nCc1);

    for (Standard_Integer rep = 1; rep <= nCc1; rep++) {
        Handle(StepRepr_RepresentationItem) repit = GetCasted(StepRepr_RepresentationItem, ItemSeq->Value(rep));
    ...
    if (ap == 3 && nbs > 1) {
        ...
        if (items->Value(j)->IsKind(STANDARD_TYPE(StepGeom_Axis2Placement3d))) {
            Handle(StepRepr_HArray1OfRepresentationItem) axis = new StepRepr_HArray1OfRepresentationItem(1, 1);
    ...
    if (!useExistingNMSSR) {
        ...
        if (!aNMBinder.IsNull()) resbind->AddResult(aNMBinder);
...
Handle(Transfer_Binder) STEPControl_ActorWrite::TransferCompound(const Handle(Transfer_Finder) & start,
                                                                 const Handle(StepShape_ShapeDefinitionRepresentation) &
                                                                     SDR0,
                                                                 const Handle(Transfer_FinderProcess) & FP,
                                                                 const Message_ProgressRange& theProgress) {
    Handle(TransferBRep_ShapeMapper) mapper = Handle(TransferBRep_ShapeMapper)::DownCast(start);
...
#ifdef OCCT_DEBUG
    if (!isManifold) std::cout << "Exploding Solids to Shells if any..." << std::endl;
#endif
    ...
    for (TopoDS_Iterator iter(theShape); iter.More(); iter.Next()) {
        ...
        if (aSubShape.ShapeType() != TopAbs_VERTEX || !isSeparateVertices) {
            ...
            } else if (!isManifold && (aSubShape.ShapeType() == TopAbs_SHELL || aSubShape.ShapeType() == TopAbs_FACE)) {
                RepItemSeq->Append(aSubShape);
                NonManifoldGroup->Append(aSubShape);
            } else
                RepItemSeq->Append(aSubShape);
    ...
    if (nbFreeVrtx) RepItemSeq->Append(aCompOfVrtx);
    ...
    for (i = 1; i <= nbs && aPS.More(); i++) {
        ...
        while (!bnd.IsNull()) {
            Handle(Transfer_SimpleBinderOfTransient) bx = Handle(Transfer_SimpleBinderOfTransient)::DownCast(bnd);
            if (!bx.IsNull()) {
    ...
    Handle(StepRepr_HArray1OfRepresentationItem) items = new StepRepr_HArray1OfRepresentationItem(1, nsub);
...
Handle(Transfer_Binder) STEPControl_ActorWrite::TransferSubShape(
    const Handle(Transfer_Finder) & start, const Handle(StepShape_ShapeDefinitionRepresentation) & SDR0,
    Handle(StepGeom_Axis2Placement3d) & AX1, const Handle(Transfer_FinderProcess) & FP,
    const Handle(TopTools_HSequenceOfShape) & shapeGroup, const Standard_Boolean isManifold,
    const Message_ProgressRange& theProgress) {
    Handle(TransferBRep_ShapeMapper) mapper = Handle(TransferBRep_ShapeMapper)::DownCast(start);
    if (mapper.IsNull()) return NullResult();
    TopoDS_Shape shape = mapper->Value();
    ...
    if (!iasdr || resbind.IsNull()) {
        ...
        if (resbind1.IsNull() || sdr->UsedRepresentation().IsNull()) return Handle(Transfer_Binder)();
        resbind = resbind1;
    ...
    if (!iasdr) {
        ...
        if (resprod != resbind) resbind->AddResult(TransientResult(sdr)); // KA - OCC7141(skl 10.11.2004)
        roots->Append(myContext.GetRootsForPart(SDRTool));
    }
    for (Standard_Integer i = 1; i <= roots->Length(); i++) {
        resprod->AddResult(TransientResult(roots->Value(i)));
        if (resprod != resbind) resbind->AddResult(TransientResult(roots->Value(i))); // KA - OCC7141(skl 10.11.2004)
    }
