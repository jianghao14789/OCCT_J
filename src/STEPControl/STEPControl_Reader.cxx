        ...
        if (ent->IsKind(STANDARD_TYPE(StepBasic_ProductDefinition))) {
            ...
            if (ent->IsKind(STANDARD_TYPE(StepBasic_ProductDefinitionWithAssociatedDocuments))) {
                ...
                for (PDWADsubs.Start(); PDWADsubs.More(); PDWADsubs.Next()) {
                    if (!PDWADsubs.Value()->IsKind(STANDARD_TYPE(StepBasic_ProductDefinitionFormation))) continue;
            ...
            if (IsRoot) {
                ...
                if (ICS > 1) {
                    ...
                    for (subs.Start(); subs.More(); subs.Next()) {
                        ...
                        if (!(strcasecmp(str1, str2) == 0 || strcasecmp(str1, str3) == 0)) IsRoot = Standard_False;
                    }
                }
            }
                ...
                for (subs.Start(); subs.More() && IsRoot; subs.Next()) {
                    ...
                    if (IsRoot) {
                        ...
                        if (!RR.IsNull()) {
                            ...
                            if (SR == SR2) SR2 = Handle(StepShape_ShapeRepresentation)::DownCast(RR->Rep2());
                            if (!SR2.IsNull()) {
...
void STEPControl_Reader::FileUnits(TColStd_SequenceOfAsciiString& theUnitLengthNames,
    ...
    if (!nbroots) return;
    enum { LENGTH = 0, ANLGE = 1, SOLID_ANGLE = 2 };
    const Interface_Graph& graph = WS()->Graph();
    ...
    for (Standard_Integer i = 1; i <= nbroots; i++) {
        ...
        if (!num) continue;
        Handle(StepBasic_ProductDefinition) aProdDef = Handle(StepBasic_ProductDefinition)::DownCast(anEnt);
        ...
        if (!aProdDef.IsNull()) {
            ...
            for (subsPD.Start(); subsPD.More() && aShapeDefRepr.IsNull(); subsPD.Next()) {
                ...
                if (aProdDefShape.IsNull()) continue;
...
Standard_Boolean STEPControl_Reader::findUnits(const Handle(StepRepr_RepresentationContext) & theRepCont,
    ...
    if (theRepCont->IsKind(STANDARD_TYPE(StepGeom_GeometricRepresentationContextAndGlobalUnitAssignedContext))) {
        aContext = Handle(StepGeom_GeometricRepresentationContextAndGlobalUnitAssignedContext)::DownCast(theRepCont)
    ...
    if (theRepCont->IsKind(STANDARD_TYPE(StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx))) {
        aContext = Handle(StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx)::DownCast(theRepCont)
                       ->GlobalUnitAssignedContext();
    }
    if (aContext.IsNull()) return Standard_False;
    // Start Computation
    ...
    for (Standard_Integer i = 1; i <= nbU; i++) {
        ...
        Handle(StepBasic_ConversionBasedUnit) aConvUnit = Handle(StepBasic_ConversionBasedUnit)::DownCast(aNamedUnit);
        ...
        if (!aConvUnit.IsNull()) {
            ...
            if (aMeasWithUnit.IsNull()) continue;
            ...
            if (aSiUnit->IsKind(STANDARD_TYPE(StepBasic_SiUnitAndLengthUnit)))
                ...
                anInd = 3;
            if (!anInd) continue;
            anUnitFact = (!aSiUnit->HasPrefix() ? 1. : STEPConstruct_UnitContext::ConvertSiPrefix(aSiUnit->Prefix()));
