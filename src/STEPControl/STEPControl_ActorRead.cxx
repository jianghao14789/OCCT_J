...
static void DumpWhatIs(const TopoDS_Shape& S) {
    ...
    Standard_Integer nbSolids = 0, nbShells = 0, nbOpenShells = 0, nbFaces = 0, nbWires = 0, nbEdges = 0,
                     nbVertexes = 0, nbCompounds = 0;

    if (S.ShapeType() == TopAbs_COMPOUND) nbCompounds++;

    for (; itL.More(); itL.Next()) {
        TopoDS_Iterator it(itL.Value());
        for (; it.More(); it.Next()) {
            TopoDS_Shape aSubShape = it.Value();
            if (!aMapOfShape.Add(aSubShape)) continue;
            aListOfShape.Append(aSubShape);
            if (aSubShape.ShapeType() == TopAbs_COMPOUND) nbCompounds++;
            if (aSubShape.ShapeType() == TopAbs_SOLID) nbSolids++;
            if (aSubShape.ShapeType() == TopAbs_SHELL) {
                if (!aSubShape.Closed()) nbOpenShells++;
                nbShells++;
            }
            if (aSubShape.ShapeType() == TopAbs_FACE) nbFaces++;
            if (aSubShape.ShapeType() == TopAbs_WIRE) nbWires++;
            if (aSubShape.ShapeType() == TopAbs_EDGE) nbEdges++;
            if (aSubShape.ShapeType() == TopAbs_VERTEX) nbVertexes++;
        }
    }
...
STEPControl_ActorRead::STEPControl_ActorRead() : myPrecision(0.0), myMaxTol(0.0) {}
// ============================================================================
...
// ============================================================================
static Handle(StepRepr_Representation)
    FindContext(const Handle(Standard_Transient) & start, const Handle(Transfer_TransientProcess) & TP,
...
static Standard_Integer FindShapeReprType(const Handle(Standard_Transient) & start) {
    if (start->IsKind(STANDARD_TYPE(StepShape_AdvancedBrepShapeRepresentation))) return 2;
    if (start->IsKind(STANDARD_TYPE(StepShape_ManifoldSurfaceShapeRepresentation))) return 3;
    if (start->IsKind(STANDARD_TYPE(StepShape_GeometricallyBoundedSurfaceShapeRepresentation))) return 4;
    if (start->IsKind(STANDARD_TYPE(StepShape_FacetedBrepShapeRepresentation))) return 5;
    if (start->IsKind(STANDARD_TYPE(StepShape_EdgeBasedWireframeShapeRepresentation))) return 6;
    if (start->IsKind(STANDARD_TYPE(StepShape_GeometricallyBoundedWireframeShapeRepresentation))) return 7;
    return 8;
}
...
static void getListSDR(const Handle(StepRepr_ShapeAspect) & sa, Handle(TColStd_HSequenceOfTransient) & listSDR,
                       const Handle(Transfer_TransientProcess) & TP) {
    const Interface_Graph& graph = TP->Graph();
    ...
    for (subs5.Start(); subs5.More(); subs5.Next()) {
        if (subs5.Value()->IsKind(STANDARD_TYPE(StepDimTol_GeometricTolerance))) return;
    }
    ...
    for (subs5.Start(); subs5.More(); subs5.Next()) {
        Handle(StepRepr_PropertyDefinition) propd = Handle(StepRepr_PropertyDefinition)::DownCast(subs5.Value());
        ...
        for (subs6.Start(); subs6.More(); subs6.Next()) {
            ...
            if (!sdr.IsNull()) listSDR->Append(sdr);
        }
    }
}
...
static void getSDR(const Handle(StepRepr_ProductDefinitionShape) & PDS, Handle(TColStd_HSequenceOfTransient) & listSDR,
                   Handle(TColStd_HSequenceOfTransient) & listNAUO,
                   Handle(TColStd_HSequenceOfTransient) & listSDRAspect, const Handle(Transfer_TransientProcess) & TP) {
    ...
    for (subs4.Start(); subs4.More(); subs4.Next()) {
        ...
        if (!sdr.IsNull()) {
            ...
            if (!rep.IsNull()) {
                ...
                if (ICS == 1) // translate all SDRs
                    ...
                    if (iDiff <= delta) listSDR->Append(sdr);
                }
            }
            continue;
        }
        ...
        if (!NAUO.IsNull()) {
            if (PDS->Definition().ProductDefinition() == NAUO->RelatingProductDefinition()) listNAUO->Append(NAUO);
            continue;
        }
    }
}
...

{
    ...
    for (subs3.Start(); subs3.More(); subs3.Next()) {
        ...
        Handle(StepRepr_ProductDefinitionShape) PDS = Handle(StepRepr_ProductDefinitionShape)::DownCast(subs3.Value());
        if (!PDS.IsNull()) {
        ...
        if (!NAUO.IsNull()) {
            if (PD == NAUO->RelatingProductDefinition()) listNAUO->Append(NAUO);
            continue;
        }
    }
    ...
    for (Standard_Integer nbNauo = 1; nbNauo <= listNAUO->Length() && PS.More(); nbNauo++) {
        ...
        if (!TP->IsBound(NAUO))
            binder = TransferEntity(NAUO, TP, aRange);
        else
            binder = TP->Find(NAUO);
    ...
    for (Standard_Integer i = 1; i <= listSDR->Length() && PS.More(); i++) {
        ...
        if (binder.IsNull()) binder = TransferEntity(rep, TP, isBound, useTrsf, aPS1.Next());
        ...
        if (i > nbNotAspect && isBound) continue;
        ...
        if (readSRR && /*theResult.IsNull() &&*/ i <= nbNotAspect) {
            ...
            for (subs1.Start(); subs1.More(); subs1.Next()) {
                ...
                if (!anitem->IsKind(STANDARD_TYPE(StepRepr_RepresentationRelationship))) continue;
                ...
                } else if (readConstructiveGeomRR &&
                           anitem->IsKind(STANDARD_TYPE(StepRepr_ConstructiveGeometryRepresentationRelationship))) {
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(
    const Handle(StepRepr_NextAssemblyUsageOccurrence) & NAUO, const Handle(Transfer_TransientProcess) & TP,
    const Message_ProgressRange& theProgress) {
    Handle(TransferBRep_ShapeBinder) shbinder;
    ...
    for (subs1.Start(); subs1.More(); subs1.Next()) {
        Handle(StepRepr_ProductDefinitionShape) PDS = Handle(StepRepr_ProductDefinitionShape)::DownCast(subs1.Value());
    ...
    if (IsDepend) {
        ...
        if (theResult.IsNull() && !SRR.IsNull()) {
            ...
            if (binder.IsNull()) {
                ...
                theResult = TransferBRep::ShapeResult(binder);
                if (!theResult.IsNull()) shbinder = new TransferBRep_ShapeBinder(theResult);
            }
        }
    }
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(const Handle(StepShape_ShapeRepresentation) & sr,
    ...
    Handle(TransferBRep_ShapeBinder) shbinder;
    if (!Recognize(sr)) return shbinder;
    ...
    Message_Messenger::StreamBuffer sout = TP->Messenger()->SendInfo();
#ifdef TRANSLOG
    if (TP->TraceLevel() > 2) sout << " -- Actor : case  ShapeRepr. NbItems=" << nb << std::endl;
#endif
...
#ifdef TRANSLOG
        if (TP->TraceLevel() > 2) sout << " -- Actor, shape_representation.item n0. " << i << std::endl;
#endif
    ...
    if (!isManifold) {
        ...
        TopoDS_Shape fixedResult =
            XSAlgo::AlgoContainer()->ProcessShape(comp, myPrecision, myMaxTol, "read.step.resource.name",
                                                  "read.step.sequence", info, aPS1.Next(), Standard_True);
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(
    const Handle(StepShape_ContextDependentShapeRepresentation) & CDSR, const Handle(Transfer_TransientProcess) & TP,
    const Message_ProgressRange& theProgress) {
    Handle(TransferBRep_ShapeBinder) shbinder;
    ...
    if (SRRReversed) TP->AddWarning(SRR, "SRR reverses relation defined by NAUO; NAUO definition is taken");
    ...
    if (!TP->IsBound(rep))
        binder = TransferEntity(rep, TP, isBound, Standard_False, theProgress);
    else
        binder = TP->Find(rep);
    theResult = TransferBRep::ShapeResult(binder);
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(
    const Handle(StepRepr_ShapeRepresentationRelationship) & und, const Handle(Transfer_TransientProcess) & TP,
    const Standard_Integer nbrep, const Standard_Boolean theUseTrsf, const Message_ProgressRange& theProgress) {
    ...
    for (Standard_Integer i = 1; i <= 2 && aPS.More(); i++) {
        ...
        Standard_Boolean isBound = Standard_False;
        if (!TP->IsBound(anitem))
            binder = TransferEntity(anitem, TP, isBound, theUseTrsf, aRange);
        else
            binder = TP->Find(anitem);
        TopoDS_Shape theResult = TransferBRep::ShapeResult(binder);
        if (!theResult.IsNull()) {
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(
    ...
    Handle(TransferBRep_ShapeBinder) shbinder;
    if (theCGRR.IsNull()) return shbinder;
    ...
    for (Standard_Integer i = 1; i <= 2; i++) {
        ...
        for (; j <= aCRepr->NbItems(); j++) {
            ...
            Handle(StepGeom_Axis2Placement3d) aStepAxis = Handle(StepGeom_Axis2Placement3d)::DownCast(anItem);
            if (!aStepAxis.IsNull()) {
                Handle(Geom_Axis2Placement) anAxis = StepToGeom::MakeAxis2Placement(aStepAxis);
                if (anAxis.IsNull()) continue;
    ...
    if (oldSRContext.IsNull() || resetUnits) PrepareUnits(oldSRContext, theTP);
...
static Standard_Boolean IsNeedRepresentation(const Handle(StepRepr_ShapeAspect) & sa,
    ...
    for (subs7.Start(); !PDSA.IsNull() && subs7.More(); subs7.Next()) {
        ...
        for (subs8.Start(); subs8.More(); subs8.Next()) {
            Handle(StepRepr_Representation) rep2 = Handle(StepRepr_Representation)::DownCast(subs8.Value());
            if (rep2.IsNull()) {
                Handle(StepRepr_ShapeRepresentationRelationship) SRR =
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::OldWay(const Handle(Standard_Transient) & start,
    ...
    if (!PD.IsNull()) {
        ...
        if (!SA.IsNull()) {
            if (!IsNeedRepresentation(SA, rep, TP)) return shbinder;
        }
    }
...
#ifdef TRANSLOG
    if (TP->TraceLevel() > 2) sout << " -- Actor : case  shape_definition_representation." << std::endl;
#endif
    Handle(Transfer_Binder) binder = TP->Find(rep);
    {
    ...
    if (aPSRoot.UserBreak()) return shbinder;
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(
    const Handle(StepGeom_GeometricRepresentationItem) & start, const Handle(Transfer_TransientProcess) & TP,
    const Standard_Boolean isManifold, const Message_ProgressRange& theProgress) {
    Message_Messenger::StreamBuffer sout = TP->Messenger()->SendInfo();
    Handle(TransferBRep_ShapeBinder) shbinder;
    ...
    if (aPS.UserBreak()) return shbinder;
    ...
    if (found && myShapeBuilder.IsDone()) {
        ...
        if (isManifold) {
            Handle(Standard_Transient) info;
            mappedShape = XSAlgo::AlgoContainer()->ProcessShape(
                mappedShape, myPrecision, myMaxTol, "read.step.resource.name", "read.step.sequence", info, aPS.Next());
            XSAlgo::AlgoContainer()->MergeTransferInfo(TP, info, nbTPitems);
        }
    }
...
#ifdef TRANSLOG
    chrono.Stop();
    if (TP->TraceLevel() > 2) sout << "End transfer STEP -> CASCADE :" << (found ? "OK" : " : no result") << std::endl;
    if (TP->TraceLevel() > 2) chrono.Show();
#endif
...
Handle(TransferBRep_ShapeBinder) STEPControl_ActorRead::TransferEntity(const Handle(StepShape_FaceSurface) & fs,
    ...
    try {
        ...
        if (!sb.IsNull() && !sb->Result().IsNull()) {
            ...
            Handle(Standard_Transient) info;
            TopoDS_Shape shape = XSAlgo::AlgoContainer()->ProcessShape(
                S, myPrecision, myMaxTol, "read.step.resource.name", "read.step.sequence", info, theProgress);
            //      TopoDS_Shape shape = XSAlgo::AlgoContainer()->PerformFixShape( S, TP, myPrecision, myMaxTol );
            if (shape != S) sb->SetResult(shape);
...
Handle(Transfer_Binder) STEPControl_ActorRead::TransferShape(const Handle(Standard_Transient) & start,
    ...
    TCollection_AsciiString aProdMode = Interface_Static::CVal("read.step.product.mode");
    if (!aProdMode.IsEqual("ON") && start->IsKind(STANDARD_TYPE(StepShape_ShapeDefinitionRepresentation)))
...
void STEPControl_ActorRead::PrepareUnits(const Handle(StepRepr_Representation) & rep,
    ...
    if (theRepCont->IsKind(STANDARD_TYPE(StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx))) {
        DeclareAndCast(StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx, theGRCAGAUC, theRepCont);
        theGUAC = theGRCAGAUC->GlobalUnitAssignedContext();
    ...
    if (!theGUAC.IsNull()) {
        ...
        Standard_Real angleFactor = (anglemode == 0 ? myUnit.PlaneAngleFactor() : anglemode == 1 ? 1. : M_PI / 180.);
        StepData_GlobalFactors::Intance().InitializeFactors(myUnit.LengthFactor(), angleFactor,
...
Standard_Boolean STEPControl_ActorRead::ComputeSRRWT(const Handle(StepRepr_RepresentationRelationship) & SRR,
                                                     const Handle(Transfer_TransientProcess) & TP, gp_Trsf& Trsf) {
    Trsf = gp_Trsf(); // init
    ...
    Handle(StepGeom_Axis2Placement3d) Ax1 = Handle(StepGeom_Axis2Placement3d)::DownCast(ItemDef->TransformItem1());
    Handle(StepGeom_Axis2Placement3d) Ax2 = Handle(StepGeom_Axis2Placement3d)::DownCast(ItemDef->TransformItem2());
...
void STEPControl_ActorRead::computeIDEASClosings(const TopoDS_Compound& comp,
    ...
    for (; shellExpA.More(); shellExpA.Next()) {
        ...
        for (; shellExpB.More(); shellExpB.Next()) {
            ...
            if (shellA.IsSame(shellB)) continue;
            // Check whether ShellB is non-manifold and adjacent to ShellA.
            // If so, ShellA has a chance to be closed with ShellB
            if (myNMTool.IsSuspectedAsClosing(shellA, shellB)) closingShells.Append(shellB);
        }

        if (!closingShells.IsEmpty()) shellClosingsMap.Add(shellA, closingShells);
    }
}
