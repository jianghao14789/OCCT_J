...
STEPControl_Controller::STEPControl_Controller() : XSControl_Controller("STEP", "step") {
    static Standard_Boolean init = Standard_False;
    if (!init) {
        RWHeaderSection::Init();
        RWStepAP214::Init();

        Interface_Static::Init("step", "write.step.product.name", 't',
        ...
        // for axis placements representing axis for suplemented geometry. Axis placements are translated to planar
        // faces with CS equal to translated axis placements
        ...
        Interface_Static::Init("step", "read.step.codepage", '&',
                               "eval NoConversion");                            // Resource_FormatType_NoConversion
        ...
        Interface_Static::Init("step", "read.step.codepage", '&',
                               "eval SystemLocale");                                 // Resource_FormatType_SystemLocale
        ...
        Standard_STATIC_ASSERT((int)Resource_FormatType_CP850 - (int)Resource_FormatType_CP1250 ==
                               18); // "Error: Invalid Codepage Enumeration"
...
IFSelect_ReturnStatus STEPControl_Controller::TransferWriteShape(const TopoDS_Shape& shape,
    ...
    Handle(STEPControl_ActorWrite) ActWrite = Handle(STEPControl_ActorWrite)::DownCast(myAdaptorWrite);
