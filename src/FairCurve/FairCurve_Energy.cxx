...
//=======================================================================
FairCurve_Energy::FairCurve_Energy(const Handle(TColgp_HArray1OfPnt2d) & Poles, const Standard_Integer ContrOrder1,
                                   const Standard_Integer ContrOrder2, const Standard_Boolean WithAuxValue,
                                   const Standard_Real Angle1, const Standard_Real Angle2,
                                   const Standard_Integer Degree, const Standard_Real Curvature1,
                                   const Standard_Real Curvature2)
    //=======================================================================
    : MyPoles(Poles), MyContrOrder1(ContrOrder1), MyContrOrder2(ContrOrder2), MyWithAuxValue(WithAuxValue),
      MyNbVar(2 * (Poles->Length() - 2) - ContrOrder2 - ContrOrder1 + WithAuxValue),
      MyNbValues(2 * Poles->Length() + WithAuxValue), MyLinearForm(0, 1), MyQuadForm(0, 1), MyGradient(0, MyNbValues),
...
//=======================================================================
{
    ...
    if (MyContrOrder1 == 2) {
        Standard_Real Lambda0 = MyPoles->Value(MyPoles->Lower()).Distance(MyPoles->Value(MyPoles->Lower() + 1));
        gp_XY DPole(Vect(Vdeb), Vect(Vdeb + 1));
    ...
    if (MyContrOrder2 == 2) {
        Standard_Real Lambda1 = MyPoles->Value(MyPoles->Upper()).Distance(MyPoles->Value(MyPoles->Upper() - 1));
...
//=======================================================================
Standard_Boolean FairCurve_Energy::Values(const math_Vector& X, Standard_Real& E, math_Vector& G, math_Matrix& H)
//=======================================================================
{
    Standard_Boolean IsDone;
...
//=======================================================================
{
    ...
    Standard_Integer Vdeb = 3 + 2 * MyContrOrder1, Vfin = 2 * MyPoles->Length() - 2 * (MyContrOrder2 + 1),
    ...
    Standard_Real Cos0 = pow(MyLinearForm(0).X(), 2), Sin0 = pow(MyLinearForm(0).Y(), 2),
                  CosSin0 = 2 * MyLinearForm(0).X() * MyLinearForm(0).Y(), Cos1 = pow(MyLinearForm(1).X(), 2),
                  Sin1 = pow(MyLinearForm(1).Y(), 2), CosSin1 = 2 * MyLinearForm(1).X() * MyLinearForm(1).Y();
    ...
    if (MyContrOrder1 >= 1) {
        ...
        if (MyContrOrder1 >= 2) {
            ...
            H(1, 1) += 2 * ((MyQuadForm(0).X() * Vect(5) + MyQuadForm(0).Y() * Vect(6)) +
    ...
    if (MyContrOrder2 >= 1) {
        ...
        if (MyContrOrder2 >= 2) {
            ...
            H(FinH + 1, FinH + 1) +=
                2 * ((MyQuadForm(1).X() * Vect(jj) + MyQuadForm(1).Y() * Vect(jj + 1)) +
...
//=======================================================================
{
    Standard_Integer ii, IndexDeb1 = MyPoles->Lower() + 1, IndexDeb2 = X.Lower(), IndexFin1 = MyPoles->Upper() - 1,
...
//=======================================================================
{
    ...
    if (MyContrOrder1 >= 1) {
        IndexDeb1 += 1;
        IndexDeb2 += 1;
        ComputePolesG1(0, X(1), MyPoles->Value(MyPoles->Lower()), MyPoles->ChangeValue(MyPoles->Lower() + 1));
    }
    if (MyContrOrder1 == 2) {
        IndexDeb1 += 1;
        IndexDeb2 += 1;
        ComputePolesG2(0, X(1), X(2), MyPoles->Value(MyPoles->Lower()), MyPoles->ChangeValue(MyPoles->Lower() + 2));
    }
    if (MyContrOrder2 == 2) {
        IndexFin1 -= 1;
        IndexFin2 -= 1;
        ComputePolesG2(1, X(IndexFin2), X(IndexFin2 + 1), MyPoles->Value(MyPoles->Upper()),
                       MyPoles->ChangeValue(MyPoles->Upper() - 2));
    }
    if (MyContrOrder2 >= 1) {
        IndexFin1 -= 1;
        ComputePolesG1(1, X(IndexFin2), MyPoles->Value(MyPoles->Upper()), MyPoles->ChangeValue(MyPoles->Upper() - 1));
    }
...
//=======================================================================
void FairCurve_Energy::ComputePolesG1(const Standard_Integer Side, const Standard_Real Lambda, const gp_Pnt2d& P1,
                                      gp_Pnt2d& P2) const
//=======================================================================
{
...
//=======================================================================
void FairCurve_Energy::ComputePolesG2(const Standard_Integer Side, const Standard_Real Lambda, const Standard_Real Rho,
                                      const gp_Pnt2d& P1, gp_Pnt2d& P2) const
//=======================================================================
{
