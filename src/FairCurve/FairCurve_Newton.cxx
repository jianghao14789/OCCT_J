...
FairCurve_Newton::FairCurve_Newton(const math_MultipleVarFunctionWithHessian& theFunction,
                                   const Standard_Real theSpatialTolerance, const Standard_Real theCriteriumTolerance,
                                   const Standard_Integer theNbIterations, const Standard_Real theConvexity,
                                   const Standard_Boolean theWithSingularity)
    : math_NewtonMinimum(theFunction, theCriteriumTolerance, theNbIterations, theConvexity, theWithSingularity),
...
Standard_Boolean FairCurve_Newton::IsConverged() const {
    ...
    return (N <= 0.01 * mySpTol) || (N <= mySpTol && Abs(TheMinimum - PreviousMinimum) <= XTol * Abs(PreviousMinimum));
}
