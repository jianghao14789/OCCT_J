...
FairCurve_DistributionOfTension::FairCurve_DistributionOfTension(
    const Standard_Integer BSplOrder, const Handle(TColStd_HArray1OfReal) & FlatKnots,
    const Handle(TColgp_HArray1OfPnt2d) & Poles, const Standard_Integer DerivativeOrder,
    const Standard_Real LengthSliding, const FairCurve_BattenLaw& Law, const Standard_Integer NbValAux,
    const Standard_Boolean Uniform)
    : FairCurve_DistributionOfEnergy(BSplOrder, FlatKnots, Poles, DerivativeOrder, NbValAux),
      MyLengthSliding(LengthSliding), MyLaw(Law) {
    if (Uniform) {
...
Standard_Boolean FairCurve_DistributionOfTension::Value(const math_Vector& TParam, math_Vector& FTension) {
    ...
    ier = BSplCLib::EvalBsplineBasis(1, MyBSplOrder, MyFlatKnots->Array1(), TParam(TParam.Lower()), FirstNonZero, Base);
        ...
        if (MyDerivativeOrder >= 2) {
            ...
            for (ii = 2; ii <= 2 * MyBSplOrder; ii += 2) {
                ...
                for (jj = 2; jj < ii; jj += 2) {
                    ...
                    k2++;
                    FTension(k2) =
                        Facteur * (GradDifference(ii) * GradDifference(jj) + FacteurY * Produit); // derivation en YiYj
                    k2++;
                }
                ...
                FTension(k2) =
                    Facteur * (GradDifference(ii) * GradDifference(ii - 1) + FacteurXY * Produit); // derivation en XiYi
                k2++;
                FTension(k2) =
                    Facteur * (GradDifference(ii) * GradDifference(ii) + FacteurY * Produit); // derivation en YiYi
            }
            if (MyNbValAux == 1) {
                ...
                FTension(FTension.Upper()) = 2 * Hauteur * pow(NormeCPrim / MyLengthSliding, 2) / MyLengthSliding;
            }
        }
    }
