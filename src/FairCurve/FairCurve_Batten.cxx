...
// ==================================================================
FairCurve_Batten::FairCurve_Batten(const gp_Pnt2d& P1, const gp_Pnt2d& P2, const Standard_Real Height,
                                   const Standard_Real Slope)
    // ==================================================================
    : myCode(FairCurve_OK), OldP1(P1), OldP2(P2), OldAngle1(0), OldAngle2(0), OldHeight(Height), OldSlope(Slope),
      OldSlidingFactor(1), OldFreeSliding(0), OldConstraintOrder1(1), OldConstraintOrder2(1), NewP1(P1), NewP2(P2),
      NewAngle1(0), NewAngle2(0), NewHeight(Height), NewSlope(Slope), NewSlidingFactor(1), NewFreeSliding(0),
      NewConstraintOrder1(1), NewConstraintOrder2(1), Degree(9) {
    if (P1.IsEqual(P2, Precision::Confusion())) throw Standard_NullValue("FairCurve : P1 and P2 are confused");
    if (Height <= 0) throw Standard_NegativeValue("FairCurve : Height is not positive");
    //
    ...
    BSplCLib::IncreaseDegree(1, Degree, Standard_False, Ipoles->Array1(), BSplCLib::NoWeights(), Iknots->Array1(),
                             Imults->Array1(), Npoles->ChangeArray1(), &Nweight->ChangeArray1(), Nknots->ChangeArray1(),
                             Nmults->ChangeArray1());
    ...
    Flatknots = new TColStd_HArray1OfReal(1, BSplCLib::KnotSequenceLength(Mults->Array1(), Degree, Standard_False));

    BSplCLib::KnotSequence(Knots->Array1(), Mults->Array1(), Degree, Standard_False, Flatknots->ChangeArray1());
}
// ==================================================================
...
// ==================================================================
{
    if (P1.IsEqual(NewP2, Precision::Confusion())) throw Standard_NullValue("FairCurve : P1 and P2 are confused");
    Angles(P1, NewP2);
    NewP1 = P1;
}
...
// ==================================================================
{
    if (NewP1.IsEqual(P2, Precision::Confusion())) throw Standard_NullValue("FairCurve : P1 and P2 are confused");
    Angles(NewP1, P2);
    NewP2 = P2;
}

// ==================================================================
Standard_Boolean FairCurve_Batten::Compute(FairCurve_AnalysisCode& ACode, const Standard_Integer NbIterations,
...
// =============================================================================
Standard_Boolean FairCurve_Batten::Compute(const gp_Vec2d& DeltaP1, const gp_Vec2d& DeltaP2,
                                           const Standard_Real DeltaAngle1, const Standard_Real DeltaAngle2,
                                           FairCurve_AnalysisCode& ACode, const Standard_Integer NbIterations,
                                           const Standard_Real Tolerance)
// =============================================================================
{
    ...
    Handle(Geom2d_BSplineCurve) DeltaCurve = new Geom2d_BSplineCurve(HermitePoles, knots, mults, L - 1);
    ...
    gp_Vec2d Ox(1, 0), P1P2(NPoles->Value(NPoles->Upper()).Coord() - NPoles->Value(NPoles->Lower()).Coord());
    ...
    FairCurve_EnergyOfBatten EBatten(Degree + 1, Flatknots, NPoles, NewConstraintOrder1, NewConstraintOrder2, LBatten,
                                     SlidingLength, NewFreeSliding, Angle1, Angle2);
    ...
    // Minimisation
    FairCurve_Newton Newton(EBatten, Tolerance * P1P2.Magnitude() / 10, Tolerance, NbIterations, VConvex);
    ...
    if (Ok) {
        ...
        if (NewConstraintOrder1 == 0) {
            gp_Vec2d Tangente(Poles->Value(Poles->Lower() + 1).Coord() - Poles->Value(Poles->Lower()).Coord());
        ...
        if (NewConstraintOrder2 == 0) {
            gp_Vec2d Tangente(Poles->Value(Poles->Upper()).Coord() - Poles->Value(Poles->Upper() - 1).Coord());
    ...
    if (NewKnots) {
        ...
        Handle(TColStd_HArray1OfReal) FKnots = new TColStd_HArray1OfReal(1, NewBS->NbPoles() + Degree + 1);
...
// ==================================================================
Standard_Real FairCurve_Batten::SlidingOfReference(const Standard_Real Dist, const Standard_Real Angle1,
...
// ==================================================================
Standard_Real FairCurve_Batten::Compute(const Standard_Real Dist, const Standard_Real Angle1,
                                        const Standard_Real Angle2) const
// ==================================================================
{
