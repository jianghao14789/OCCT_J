...
static NCollection_Mat4<double> mat44vr2Occ(const vr::HmdMatrix44_t& theMat4) {
    ...
    for (int aRow = 0; aRow < 4; ++aRow) {
        aMat4.SetRow(aRow, NCollection_Vec4<double>(theMat4.m[aRow][0], theMat4.m[aRow][1], theMat4.m[aRow][2],
                                                    theMat4.m[aRow][3]));
    }
    return aMat4;
}
...
static gp_Trsf mat34vr2OccTrsf(const vr::HmdMatrix34_t& theMat4) {
    gp_Trsf aTrsf;
    aTrsf.SetValues(theMat4.m[0][0], theMat4.m[0][1], theMat4.m[0][2], theMat4.m[0][3], theMat4.m[1][0],
                    theMat4.m[1][1], theMat4.m[1][2], theMat4.m[1][3], theMat4.m[2][0], theMat4.m[2][1],
                    theMat4.m[2][2], theMat4.m[2][3]);
    return aTrsf;
}
...
static Aspect_TrackedDevicePose poseVr2Occ(const vr::TrackedDevicePose_t& theVrPose,
    ...
    aPose.AngularVelocity.SetCoord(theVrPose.vAngularVelocity.v[0], theVrPose.vAngularVelocity.v[1],
...
public:
    ...
    bool Load(vr::TextureID_t theTexture, const TCollection_AsciiString& theVrModelName) {
        ...
        InitWrapper(Image_Format_RGBA, (Standard_Byte*)aVrTexture->rubTextureMapData, aVrTexture->unWidth,
                    aVrTexture->unHeight);
...
// purpose  :
// =======================================================================
Aspect_OpenVRSession::Aspect_OpenVRSession() : myContext(new VRContext()) {
#ifdef HAVE_OPENVR
    ...
    myTrackedPoses.Resize(0, (Standard_Integer)vr::k_unMaxTrackedDeviceCount - 1, false);
    {
        ...
        Handle(Aspect_XRAction) aHeadsetOn =
            new Aspect_XRAction(aHeadActionSet->Id() + "/in/headset_on_head", Aspect_XRActionType_InputDigital);
    ...
    for (int aHand = 0; aHand < 2; ++aHand) {
        NCollection_Array1<Handle(Aspect_XRAction)>& aGenericSet =
            myRoleActions[aHand == 0 ? Aspect_XRTrackedDeviceRole_LeftHand : Aspect_XRTrackedDeviceRole_RightHand];
        ...
        Handle(Aspect_XRAction) anAppMenuClick =
            new Aspect_XRAction(anActionSet->Id() + "/in/appmenu_click", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aSysMenuClick =
            new Aspect_XRAction(anActionSet->Id() + "/in/sysmenu_click", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aTriggerClick =
            new Aspect_XRAction(anActionSet->Id() + "/in/trigger_click", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aPadPos =
            new Aspect_XRAction(anActionSet->Id() + "/in/trackpad_position", Aspect_XRActionType_InputAnalog);
        ...
        Handle(Aspect_XRAction) aPadTouch =
            new Aspect_XRAction(anActionSet->Id() + "/in/trackpad_touch", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aPadClick =
            new Aspect_XRAction(anActionSet->Id() + "/in/trackpad_click", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aThumbsctickPos =
            new Aspect_XRAction(anActionSet->Id() + "/in/thumbstick_position", Aspect_XRActionType_InputAnalog);
        ...
        Handle(Aspect_XRAction) aThumbsctickTouch =
            new Aspect_XRAction(anActionSet->Id() + "/in/thumbstick_touch", Aspect_XRActionType_InputDigital);
        ...
        Handle(Aspect_XRAction) aThumbsctickClick =
            new Aspect_XRAction(anActionSet->Id() + "/in/thumbstick_click", Aspect_XRActionType_InputDigital);
    ...
    for (Aspect_XRActionSetMap::Iterator aSetIter(myActionSets); aSetIter.More(); aSetIter.Next()) {
        ...
        for (Aspect_XRActionMap::Iterator anActionIter(anActionSet->Actions()); anActionIter.More();
            ...
            if (aVrError == vr::VRInputError_None) {
                ...
                hasErrors = true;
                Message::SendFail(TCollection_AsciiString("OpenVR, Unable to load action '") + anAction->Id() +
                                  "' from '" + myActionsManifest + "': " + getVRInputError(aVrError));
            }
        }
        ...
        if (aVrError == vr::VRInputError_None) {
            ...
            hasErrors = true;
            Message::SendFail(TCollection_AsciiString("OpenVR, Unable to load action set '") + anActionSet->Id() +
                              "' from '" + myActionsManifest + "': " + getVRInputError(aVrError));
        }
    }
    return !hasErrors;
    ...
    if (myContext->System != NULL) {
        ...
        switch (theDevice) {
            ...
            case Aspect_XRTrackedDeviceRole_LeftHand:
                aDevIndex =
                    myContext->System->GetTrackedDeviceIndexForControllerRole(vr::TrackedControllerRole_LeftHand);
                break;
            case Aspect_XRTrackedDeviceRole_RightHand:
                aDevIndex =
                    myContext->System->GetTrackedDeviceIndexForControllerRole(vr::TrackedControllerRole_RightHand);
                break;
...
Handle(Graphic3d_ArrayOfTriangles) Aspect_OpenVRSession::loadRenderModel(Standard_Integer theDevice,
    ...
    if (aVrError != vr::VRRenderModelError_None) {
        Message::SendFail(TCollection_AsciiString("OpenVR, Unable to load render model: ") + aRenderModelName + " - " +
                          int(aVrError));
        return Handle(Graphic3d_ArrayOfTriangles)();
    }
    ...
    Handle(Graphic3d_ArrayOfTriangles) aTris =
        new Graphic3d_ArrayOfTriangles((int)aVrModel->unVertexCount, (int)aVrModel->unTriangleCount * 3,
    ...
    for (uint32_t aVertIter = 0; aVertIter < aVrModel->unVertexCount; ++aVertIter) {
        ...
        aTris->AddVertex(aVert.vPosition.v[0] * aScale, aVert.vPosition.v[1] * aScale, aVert.vPosition.v[2] * aScale,
                         aVert.vNormal.v[0], aVert.vNormal.v[1], aVert.vNormal.v[2], aVert.rfTextureCoord[0],
                         aVert.rfTextureCoord[1]);
    }
    for (uint32_t aTriIter = 0; aTriIter < aVrModel->unTriangleCount; ++aTriIter) {
        aTris->AddEdges(aVrModel->rIndexData[aTriIter * 3 + 0] + 1, aVrModel->rIndexData[aTriIter * 3 + 1] + 1,
...
#ifdef HAVE_OPENVR
    if (myContext->System != NULL) {
        const vr::HmdMatrix34_t aMatVr =
            myContext->System->GetEyeToHeadTransform(theEye == Aspect_Eye_Right ? vr::Eye_Right : vr::Eye_Left);
...
// purpose  :
// =======================================================================
NCollection_Mat4<double> Aspect_OpenVRSession::ProjectionMatrix(Aspect_Eye theEye, double theZNear,
                                                                double theZFar) const {
#ifdef HAVE_OPENVR
    if (myContext->System != NULL) {
        const vr::HmdMatrix44_t aMat4 = myContext->System->GetProjectionMatrix(
            theEye == Aspect_Eye_Right ? vr::Eye_Right : vr::Eye_Left, (float)theZNear, (float)theZFar);
...
void Aspect_OpenVRSession::updateProjectionFrustums() {
    ...
    myIod =
        myContext->System->GetFloatTrackedDeviceProperty(vr::k_unTrackedDeviceIndex_Hmd, vr::Prop_UserIpdMeters_Float);
    myIod *= myUnitFactor;
#endif
}
...
bool Aspect_OpenVRSession::WaitPoses() {
    ...
    const vr::EVRCompositorError aVRError =
        vr::VRCompositor()->WaitGetPoses(myContext->TrackedPoses, vr::k_unMaxTrackedDeviceCount, NULL, 0);
...
Aspect_XRDigitalActionData Aspect_OpenVRSession::GetDigitalActionData(const Handle(Aspect_XRAction) & theAction) const {
    ...
    if (myContext->System != NULL && theAction->RawHandle() != 0) {
        ...
        vr::EVRInputError anInErr = vr::VRInput()->GetDigitalActionData(
            theAction->RawHandle(), &aNewData, sizeof(aNewData), vr::k_ulInvalidInputValueHandle);
...
Aspect_XRAnalogActionData Aspect_OpenVRSession::GetAnalogActionData(const Handle(Aspect_XRAction) & theAction) const {
    ...
    if (myContext->System != NULL && theAction->RawHandle() != 0) {
        ...
        vr::EVRInputError anInErr = vr::VRInput()->GetAnalogActionData(
            theAction->RawHandle(), &aNewData, sizeof(aNewData), vr::k_ulInvalidInputValueHandle);
        if (anInErr != vr::VRInputError_None) {
            Message::SendFail(TCollection_AsciiString("Input Error on '") + theAction->Id() +
                              "': " + getVRInputError(anInErr));
...
// purpose  :
// =======================================================================
Aspect_XRPoseActionData Aspect_OpenVRSession::GetPoseActionDataForNextFrame(const Handle(Aspect_XRAction) &
                                                                            theAction) const {
    ...
    if (myContext->System != NULL && theAction->RawHandle() != 0) {
        ...
        vr::EVRInputError anInErr = vr::VRInput()->GetPoseActionDataForNextFrame(
            theAction->RawHandle(), anOrigin, &aNewData, sizeof(aNewData), vr::k_ulInvalidInputValueHandle);
        if (anInErr != vr::VRInputError_None) {
            Message::SendFail(TCollection_AsciiString("Input Error on '") + theAction->Id() +
                              "': " + getVRInputError(anInErr));
            return anActionData;
...
void Aspect_OpenVRSession::triggerHapticVibrationAction(const Handle(Aspect_XRAction) & theAction,
    ...
    if (myContext->System != NULL && theAction->RawHandle() != 0) {
        ...
        vr::EVRInputError anInErr = vr::VRInput()->TriggerHapticVibrationAction(
            theAction->RawHandle(), aParams.Delay, aParams.Duration, aParams.Frequency, aParams.Amplitude,
...
void Aspect_OpenVRSession::ProcessEvents() {
        ...
        for (Aspect_XRActionMap::Iterator anActionIter(anActionSet->Actions()); anActionIter.More();
            ...
            switch (anAction->Type()) {
                ...
                case Aspect_XRActionType_InputAnalog: {
                    ...
                    // if (aData.IsChanged()) { std::cout << "  " << anAction->Id() << " changed: " << aData.VecXYZ[0]
                    // << " " << aData.VecXYZ[1] << " " << aData.VecXYZ[2] << "\n"; }
                    break;
                }
    ...
    //  vr::VRControllerState_t aCtrlState = {}; if (myContext->GetControllerState (aCtrlState, aDevIter)) {
...
// purpose  :
// =======================================================================
bool Aspect_OpenVRSession::SubmitEye(void* theTexture, Aspect_GraphicsLibrary theGraphicsLib,
                                     Aspect_ColorSpace theColorSpace, Aspect_Eye theEye) {
    if (theTexture == NULL) {
        return false;
    }
#ifdef HAVE_OPENVR
    ...
    const vr::EVRCompositorError aVRError =
        vr::VRCompositor()->Submit(theEye == Aspect_Eye_Right ? vr::Eye_Right : vr::Eye_Left, &aVRTexture);
    if (aVRError != vr::VRCompositorError_None) {
