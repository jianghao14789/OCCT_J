...
struct Aspect_XRDigitalActionData {
    ...
    bool IsPressed;        //!< Aspect_InputActionType_Digital state - The current state of this action; will be true if
                           //!< currently pressed
    bool IsChanged; //!< Aspect_InputActionType_Digital state - this is true if the state has changed since the last
                    //!< frame

    //! Empty constructor.
    Aspect_XRDigitalActionData()
        : ActiveOrigin(0), UpdateTime(0.0f), IsActive(false), IsPressed(false), IsChanged(false) {}
};
