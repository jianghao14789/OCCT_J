...
public:
    ...
    enum PerfCounters {
        ...
        PerfCounters_CPU = 0x002,    //!< CPU utilization as frames per second (number of frames within CPU utilization
                                     //!< time (rendering thread))
        PerfCounters_Layers = 0x004, //!< count layers (groups of structures)
        PerfCounters_Structures = 0x008, //!< count low-level Structures (normal unhighlighted Presentable Object is
                                         //!< usually represented by 1 Structure)
        //
        ...
        //
        PerfCounters_SkipImmediate = 0x1000, //!< do not include immediate viewer updates (e.g. lazy updates without
                                             //!< redrawing entire view content)
    ...
    enum FrustumCulling {
        ...
        FrustumCulling_On,      //!< culling is active, and the list of culled entities is automatically updated before
                                //!< redraw
...
public:
    ...
    Graphic3d_RenderingParams()
        : Method(Graphic3d_RM_RASTERIZATION), ShadingModel(Graphic3d_TypeOfShadingModel_Phong),
          TransparencyMethod(Graphic3d_RTM_BLEND_UNORDERED), Resolution(THE_DEFAULT_RESOLUTION),
          FontHinting(Font_Hinting_Off), LineFeather(1.0f),
          // PBR parameters
          PbrEnvPow2Size(9), PbrEnvSpecMapNbLevels(6), PbrEnvBakingDiffNbSamples(1024), PbrEnvBakingSpecNbSamples(256),
          PbrEnvBakingProbability(0.99f),
          //
          OitDepthFactor(0.0f), NbOitDepthPeelingLayers(4), NbMsaaSamples(0), RenderResolutionScale(1.0f),
          ShadowMapResolution(1024), ShadowMapBias(0.005f), ToEnableDepthPrepass(Standard_False),
          ToEnableAlphaToCoverage(Standard_True),
          // ray tracing parameters
          IsGlobalIlluminationEnabled(Standard_False), SamplesPerPixel(0), RaytracingDepth(THE_DEFAULT_DEPTH),
          IsShadowEnabled(Standard_True), IsReflectionEnabled(Standard_False), IsAntialiasingEnabled(Standard_False),
          IsTransparentShadowEnabled(Standard_False), UseEnvironmentMapBackground(Standard_False),
          ToIgnoreNormalMapInRayTracing(Standard_False), CoherentPathTracingMode(Standard_False),
          AdaptiveScreenSampling(Standard_False), AdaptiveScreenSamplingAtomic(Standard_False),
          ShowSamplingTiles(Standard_False), TwoSidedBsdfModels(Standard_False), RadianceClampingValue(30.0),
          RebuildRayTracingShaders(Standard_False), RayTracingTileSize(32), NbRayTracingTiles(16 * 16),
          CameraApertureRadius(0.0f), CameraFocalPlaneDist(1.0f), FrustumCullingState(FrustumCulling_On),
          ToneMappingMethod(Graphic3d_ToneMappingMethod_Disabled), Exposure(0.f), WhitePoint(1.f),
          // stereoscopic parameters
          StereoMode(Graphic3d_StereoMode_QuadBuffer), HmdFov2d(30.0f), AnaglyphFilter(Anaglyph_RedCyan_Optimized),
          ToReverseStereo(Standard_False), ToMirrorComposer(Standard_True),
          //
          StatsPosition(new Graphic3d_TransformPers(Graphic3d_TMF_2d, Aspect_TOTP_LEFT_UPPER, Graphic3d_Vec2i(20, 20))),
          ChartPosition(
              new Graphic3d_TransformPers(Graphic3d_TMF_2d, Aspect_TOTP_RIGHT_UPPER, Graphic3d_Vec2i(20, 20))),
          ChartSize(-1, -1), StatsTextAspect(new Graphic3d_AspectText3d()), StatsUpdateInterval(1.0),
          StatsTextHeight(16), StatsNbFrames(1), StatsMaxChartTime(0.1f), CollectedStats(PerfCounters_Basic),
...
public:                             //! @name general parameters
    ...
    Graphic3d_TypeOfShadingModel
        ShadingModel; //!< specified default shading model, Graphic3d_TypeOfShadingModel_Phong by default
    ...
    Font_Hinting
        FontHinting; //!< enables/disables text hinting within textured fonts, Font_Hinting_Off by default;
...
public: //! @name rendering resolution parameters
    Standard_Integer
        PbrEnvPow2Size; //!< size of IBL maps side can be calculated as 2^PbrEnvPow2Size (> 0), 9 by default
    Standard_Integer PbrEnvSpecMapNbLevels; //!< number of levels used in specular IBL map (> 1), 6 by default
    Standard_Integer
        PbrEnvBakingDiffNbSamples; //!< number of samples used in Monte-Carlo integration during diffuse IBL map's
                                   //!  spherical harmonics coefficients generation (> 0), 1024 by default
    Standard_Integer PbrEnvBakingSpecNbSamples; //!< number of samples used in Monte-Carlo integration during specular
                                                //!< IBL map's generation (> 0), 256 by default
    Standard_ShortReal PbrEnvBakingProbability; //!< controls strength of samples reducing strategy during specular IBL
                                                //!< map's generation
                                                //!  (see 'SpecIBLMapSamplesFactor' function for detail explanation)
                                                //!  [0.0, 1.0], 0.99 by default

    Standard_ShortReal OitDepthFactor; //!< scalar factor [0-1] controlling influence of depth of a fragment to its
                                       //!< final coverage (Graphic3d_RTM_BLEND_OIT), 0.0 by default
    Standard_Integer
        NbOitDepthPeelingLayers;    //!< number of depth peeling (Graphic3d_RTM_DEPTH_PEELING_OIT) layers, 4 by default
    Standard_Integer NbMsaaSamples; //!< number of MSAA samples (should be within 0..GL_MAX_SAMPLES, power-of-two
                                    //!< number), 0 by default
...
public:                                           //! @name Ray-Tracing/Path-Tracing parameters
    ...
    Standard_Boolean
        IsTransparentShadowEnabled; //!< enables/disables light propagation through transparent media, False by default
    Standard_Boolean UseEnvironmentMapBackground; //!< enables/disables environment map background
    Standard_Boolean
        ToIgnoreNormalMapInRayTracing; //!< enables/disables normal map ignoring during path tracing; FALSE by default
    Standard_Boolean CoherentPathTracingMode; //!< enables/disables 'coherent' tracing mode (single RNG seed within
                                              //!< 16x16 image blocks)
    Standard_Boolean
        AdaptiveScreenSampling; //!< enables/disables adaptive screen sampling mode for path tracing, FALSE by default
    Standard_Boolean AdaptiveScreenSamplingAtomic; //!< enables/disables usage of atomic float operations within
                                                   //!< adaptive screen sampling, FALSE by default
    Standard_Boolean ShowSamplingTiles; //!< enables/disables debug mode for adaptive screen sampling, FALSE by default
    Standard_Boolean
        TwoSidedBsdfModels; //!< forces path tracing to use two-sided versions of original one-sided scattering models
    ...
    Standard_Integer NbRayTracingTiles; //!< maximum number of screen tiles per frame, 256 by default (adaptive sampling
                                        //!< mode of path tracing);
                                        //!  this parameter limits the number of tiles to be rendered per redraw,
                                        //!  increasing Viewer interactivity, but also increasing the time for achieving
                                        //!  a good quality; -1 means no limit
    Standard_ShortReal CameraApertureRadius; //!< aperture radius of perspective camera used for depth-of-field, 0.0 by
                                             //!< default (no DOF) (path tracing only)
    Standard_ShortReal CameraFocalPlaneDist; //!< focal  distance of perspective camera used for depth-of field, 1.0 by
                                             //!< default (path tracing only)
    FrustumCulling FrustumCullingState;      //!< state of frustum culling optimization; FrustumCulling_On by default

    Graphic3d_ToneMappingMethod ToneMappingMethod; //!< specifies tone mapping method for path tracing,
                                                   //!< Graphic3d_ToneMappingMethod_Disabled by default
...
public:                              //! @name VR / stereoscopic parameters
    ...
    Standard_ShortReal
        HmdFov2d; //!< sharp field of view range in degrees for displaying on-screen 2D elements, 30.0 by default;
    Anaglyph AnaglyphFilter;           //!< filter for anaglyph output, Anaglyph_RedCyan_Optimized by default
    Graphic3d_Mat4 AnaglyphLeft;       //!< left  anaglyph filter (in normalized colorspace), Color = AnaglyphRight *
                                       //!< theColorRight + AnaglyphLeft * theColorLeft;
    Graphic3d_Mat4 AnaglyphRight;      //!< right anaglyph filter (in normalized colorspace), Color = AnaglyphRight *
                                       //!< theColorRight + AnaglyphLeft * theColorLeft;
    Standard_Boolean ToReverseStereo;  //!< flag to reverse stereo pair, FALSE by default
    Standard_Boolean ToMirrorComposer; //!< if output device is an external composer - mirror rendering results in
                                       //!< window in addition to sending frame to composer, TRUE by default
...
public:                                            //! @name on-screen display parameters
    ...
    Graphic3d_Vec2i ChartSize; //!< chart size in pixels, (-1, -1) by default which means that chart will occupy a
                               //!< portion of viewport
    Handle(Graphic3d_AspectText3d) StatsTextAspect; //!< stats text aspect
    Standard_ShortReal
        StatsUpdateInterval;          //!< time interval between stats updates in seconds, 1.0 second by default;
    ...
    Standard_ShortReal StatsMaxChartTime; //!< upper time limit within frame chart in seconds; 0.1 seconds by default
                                          //!< (100 ms or 10 FPS)
    PerfCounters CollectedStats;          //!< performance counters to collect, PerfCounters_Basic by default;
                                 ...
                                 //!  because some counters might lack caching optimization (and will require expensive
                                 //!  iteration through all data structures)
    Standard_Boolean ToShowStats; //!< display performance statistics, FALSE by default;
                                  ...
                                  //!  of visibility of widget managed by ToShowStats flag (e.g. stats can be retrieved
                                  //!  by application for displaying using other methods)
};
