...
Graphic3d_TextureRoot::Graphic3d_TextureRoot(const TCollection_AsciiString& theFileName,
    ...
    : myParams(new Graphic3d_TextureParams()), myPath(theFileName), myRevision(0), myType(theType), myIsColorMap(true),
      myIsTopDown(true) {
    generateId();
}
...
Graphic3d_TextureRoot::Graphic3d_TextureRoot(const Handle(Image_PixMap) & thePixMap,
                                             const Graphic3d_TypeOfTexture theType)
    : myParams(new Graphic3d_TextureParams()), myPixMap(thePixMap), myRevision(0), myType(theType), myIsColorMap(true),
      myIsTopDown(true) {
    generateId();
}
...
// purpose  :
// =======================================================================
Handle(Image_CompressedPixMap) Graphic3d_TextureRoot::GetCompressedImage(const Handle(Image_SupportedFormats) &
                                                                         theSupported) {
    if (!myPixMap.IsNull()) {
