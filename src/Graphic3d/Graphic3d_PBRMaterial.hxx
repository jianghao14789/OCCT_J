...
public:
    ...
    Standard_Boolean operator==(const Graphic3d_PBRMaterial& theOther) const {
        return (myMetallic == theOther.myMetallic) && (myRoughness == theOther.myRoughness) &&
               (myIOR == theOther.myIOR) && (myColor == theOther.myColor) && (myEmission == theOther.myEmission);
    }
...
public:
    ...
    //! @param [in] theIntegralSamplesCount number of importance samples in hemisphere integral calculation for every
    //! table item.
...
public:
    ...
    //! Specular IBL maps with less roughness values have higher resolution but require less samples for the same
    //! quality of baking. So that reducing samples number is good strategy to improve performance of baking. The
    //! samples number for specular IBL map with roughness of 1 (the maximum possible samples number) is expected to be
    //! defined as baking parameter. Samples number for other roughness values can be calculated by multiplication
    //! origin samples number by this factor.
    ...
    //! Be aware! It produces poor quality with small number of origin samples. In that case it is recommended to be
    //! disabled.
...
private:
    ...
    static Standard_ShortReal lutGenGeometryFactor(Standard_ShortReal theCosL, Standard_ShortReal theCosV,
                                                   Standard_ShortReal theRoughness);
    ...
    static Graphic3d_Vec3 lutGenReflect(const Graphic3d_Vec3& theVector, const Graphic3d_Vec3& theAxis);
