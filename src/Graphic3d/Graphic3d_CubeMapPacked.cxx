...
Graphic3d_CubeMapPacked::Graphic3d_CubeMapPacked(const TCollection_AsciiString& theFilePath,
    ...
    : Graphic3d_CubeMap(theFilePath), myOrder(theOrder), myTileNumberX(1) {}
...
Graphic3d_CubeMapPacked::Graphic3d_CubeMapPacked(const Handle(Image_PixMap) & theImage,
                                                 const Graphic3d_ValidatedCubeMapOrder theOrder)
    : Graphic3d_CubeMap(Handle(Image_PixMap)()), myOrder(theOrder), myTileNumberX(1) {
...
// purpose  :
// =======================================================================
Handle(Image_CompressedPixMap) Graphic3d_CubeMapPacked::CompressedValue(const Handle(Image_SupportedFormats) &
                                                                        theSupported) {
    ...
    if (myTileNumberX != 0) {
        ...
        if (!myPixMap.IsNull()) {
            ...
            if (aWrapper->InitWrapper(myPixMap->Format(),
                                      ...
                                      aTileSize, aTileSize, myPixMap->SizeRowBytes())) {
