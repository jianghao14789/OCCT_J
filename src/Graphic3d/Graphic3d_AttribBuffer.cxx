...
Graphic3d_AttribBuffer::Graphic3d_AttribBuffer(const Handle(NCollection_BaseAllocator) & theAlloc)
    : Graphic3d_Buffer(theAlloc), myIsInterleaved(Standard_True), myIsMutable(Standard_False) {}

// =======================================================================
// function : Init
// purpose  :
// =======================================================================
bool Graphic3d_AttribBuffer::Init(const Standard_Integer theNbElems, const Graphic3d_Attribute* theAttribs,
    ...
    if (mySize > (Standard_Size)IntegerLast() && myIsMutable) {
        throw Standard_OutOfRange(
            "Graphic3d_AttribBuffer::Init(), Mutable flag cannot be used for buffer exceeding 32-bit address space");
    }
    return true;
}
...
void Graphic3d_AttribBuffer::SetMutable(Standard_Boolean theMutable) {
    if (mySize > (Standard_Size)IntegerLast() && theMutable) {
        throw Standard_OutOfRange("Graphic3d_AttribBuffer::SetMutable(), Mutable flag cannot be used for buffer "
...
void Graphic3d_AttribBuffer::Invalidate() {
    if (mySize > (Standard_Size)IntegerLast()) {
        throw Standard_OutOfRange(
            "Graphic3d_AttribBuffer::Invalidate() cannot be used for buffer exceeding 32-bit address space");
    }
...
// purpose  :
// =======================================================================
void Graphic3d_AttribBuffer::Invalidate(Standard_Integer theAttributeIndex, Standard_Integer theVertexLower,
                                        Standard_Integer theVertexUpper) {
    Standard_OutOfRange_Raise_if(theAttributeIndex < 0 || theAttributeIndex >= NbAttributes || theVertexLower < 0 ||
                                     theVertexLower > theVertexUpper || theVertexUpper >= NbMaxElements(),
                                 "Graphic3d_AttribBuffer::Invalidate()");
    if (myIsInterleaved) {
...
// purpose  :
// =======================================================================
void Graphic3d_AttribBuffer::Invalidate(Standard_Integer theVertexLower, Standard_Integer theVertexUpper) {
    Standard_OutOfRange_Raise_if(theVertexLower < 0 || theVertexLower > theVertexUpper ||
                                     theVertexUpper >= NbMaxElements(),
                                 "Graphic3d_AttribBuffer::Invalidate()");
    if (myIsInterleaved) {
        invalidate(Graphic3d_BufferRange(Stride * theVertexLower, Stride * (theVertexUpper - theVertexLower + 1)));
        return;
    }
