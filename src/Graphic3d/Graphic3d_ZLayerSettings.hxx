...
struct Graphic3d_ZLayerSettings {
    ...
    Graphic3d_ZLayerSettings()
        : myCullingDistance(Precision::Infinite()), myCullingSize(Precision::Infinite()), myIsImmediate(Standard_False),
          myToRaytrace(Standard_True), myUseEnvironmentTexture(Standard_True), myToEnableDepthTest(Standard_True),
          myToEnableDepthWrite(Standard_True), myToClearDepth(Standard_True), myToRenderInDepthPrepass(Standard_True) {}
    ...
    //! NULL list (but not empty list!) means that default lights assigned to the View should be used instead of
    //! per-layer lights.
    ...
    const Handle(TopLoc_Datum3D) & OriginTransformation() const {
        return myOriginTrsf;
    }
    ...
    //! Return the distance to discard drawing of distant objects (distance from camera Eye point); by default it is
    //! Infinite (distance culling is disabled). Since camera eye definition has no strong meaning within orthographic
    //! projection, option is considered only within perspective projection. Note also that this option has effect only
...
protected:
    ...
    Standard_Boolean
        myUseEnvironmentTexture; //!< flag to allow/prevent environment texture mapping usage for specific layer
