...
public:
    ...
    Graphic3d_Buffer(const Handle(NCollection_BaseAllocator) & theAlloc)
        : NCollection_Buffer(theAlloc), Stride(0), NbElements(0), NbAttributes(0) {
        //
    }
...
public:
    ...
    template <typename Type_t> inline const Type_t& Value(const Standard_Integer theElem) const {
        return *reinterpret_cast<const Type_t*>(value(theElem));
    }

    //! Access element with specified position and type.
    template <typename Type_t> inline Type_t& ChangeValue(const Standard_Integer theElem) {
...
public:
    ...
    Standard_Byte* ChangeAttributeData(Graphic3d_TypeOfAttribute theAttrib, Standard_Integer& theAttribIndex,
    ...
    //! @param theAttribIndex  index of found attribute
    //! @param theAttribStride stride in bytes between values of this attribute within returned data pointer
    const Standard_Byte* AttributeData(Graphic3d_TypeOfAttribute theAttrib, Standard_Integer& theAttribIndex,
...
public:
    ...
    bool Init(const Standard_Integer theNbElems, const Graphic3d_Attribute* theAttribs,
...
public:
    ...
    Standard_Integer NbElements;   //!< number of the elements (@sa NbMaxElements() specifying the number of initially
