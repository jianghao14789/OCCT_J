...
public:
    ...
    //! Headlight flag means that light position/direction are defined not in a World coordinate system, but relative to
    //! the camera orientation.
    Standard_Boolean IsHeadlight() const {
        return myIsHeadlight;
    }
...
public:
    ...
    //! Distance attenuation factors of reducing positional/spot light intensity depending on the distance from its
    //! position:
    //! @code
    //!   float anAttenuation = 1.0 / (ConstAttenuation() + LinearAttenuation() * theDistance + QuadraticAttenuation() *
...
public:
    ...
    gp_Dir Direction() const {
        return gp_Dir(myDirection.x(), myDirection.y(), myDirection.z());
    }
...
public:
    ...
    //! @endcode
    //! The concentration factor determines the dispersion of the light on the surface, the default value (1.0)
    //! corresponds to a minimum of dispersion.
...
public:
    ...
    //! Returns the smoothness of light source (either smoothing angle for directional light or smoothing radius in case
    //! of positional light); 0.0 by default.
    ...
    bool HasRange() const {
        return myDirection.w() != 0.0f;
    }

    //! Returns maximum distance on which point light source affects to objects and is considered during illumination
    //! calculations. 0.0 means disabling range considering at all without any distance limits. Has sense only for point
    //! light sources (positional and spot).
    Standard_ShortReal Range() const {
        return myDirection.w();
    }

    //! Modifies maximum distance on which point light source affects to objects and is considered during illumination
    //! calculations. Positional and spot lights are only point light sources. 0.0 means disabling range considering at
    //! all without any distance limits.
