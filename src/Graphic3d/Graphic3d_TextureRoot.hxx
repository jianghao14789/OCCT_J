...
public:
    ...
    Standard_EXPORT virtual Handle(Image_CompressedPixMap)
        GetCompressedImage(const Handle(Image_SupportedFormats) & theSupported);
...
protected:
    ...
    TCollection_AsciiString myTexId; //!< unique identifier of this resource (for sharing graphic resource); should
                                     //!< never be modified outside constructor
    ...
    Standard_Size
        myRevision; //!< image revision - for signaling changes in the texture source (e.g. file update, pixmap update)
