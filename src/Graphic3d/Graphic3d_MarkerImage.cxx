...
static void getMarkerBitMapParam(const Aspect_TypeOfMarker theMarkerType, const Standard_ShortReal theScale,
                                 Standard_Integer& theWidth, Standard_Integer& theHeight, Standard_Integer& theOffset,
...
//! Aspect_TOM_O_POINT, Aspect_TOM_O_PLUS, Aspect_TOM_O_STAR, Aspect_TOM_O_X, Aspect_TOM_RING1, Aspect_TOM_RING2,
//! Aspect_TOM_RING3
static Handle(Image_PixMap)
    mergeImages(const Handle(Image_PixMap) & theImage1, const Handle(Image_PixMap) & theImage2) {
...
static Handle(Graphic3d_MarkerImage)
    getTextureImage(const Aspect_TypeOfMarker theMarkerType, const Standard_ShortReal theScale) {
...
Graphic3d_MarkerImage::Graphic3d_MarkerImage(const Handle(Image_PixMap) & theImage,
    ...
    : myImage(theImage), myImageAlpha(theImageAlpha), myMargin(1), myWidth((Standard_Integer)theImage->Width()),
    ...
    myImageAlphaId =
        TCollection_AsciiString("Graphic3d_MarkerImageAlpha_") + TCollection_AsciiString(THE_MARKER_IMAGE_COUNTER);

    if (!theImageAlpha.IsNull()) {
        if (theImageAlpha->Format() != Image_Format_Alpha && theImageAlpha->Format() != Image_Format_Gray) {
...
Graphic3d_MarkerImage::Graphic3d_MarkerImage(const TCollection_AsciiString& theId,
                                             ...
                                             const Handle(Image_PixMap) & theImageAlpha)
    : myImageId(theId), myImageAlphaId(theAlphaId), myImage(theImage), myImageAlpha(theImageAlpha), myMargin(1),
      myWidth((Standard_Integer)theImage->Width()), myHeight((Standard_Integer)theImage->Height()) {
    if (!theImageAlpha.IsNull()) {
        if (theImageAlpha->Format() != Image_Format_Alpha && theImageAlpha->Format() != Image_Format_Gray) {
            throw Standard_ProgramError("Graphic3d_MarkerImage, wrong color format of alpha image");
        }
...
Graphic3d_MarkerImage::Graphic3d_MarkerImage(const Handle(TColStd_HArray1OfByte) & theBitMap,
    ...
    : myBitMap(theBitMap), myMargin(1), myWidth(theWidth), myHeight(theHeight) {
    myImageId = TCollection_AsciiString("Graphic3d_MarkerImage_") +
                TCollection_AsciiString(Standard_Atomic_Increment(&THE_MARKER_IMAGE_COUNTER));
...
bool Graphic3d_MarkerImage::IsColoredImage() const {
    return !myImage.IsNull() && myImage->Format() != Image_Format_Alpha && myImage->Format() != Image_Format_Gray;
}
...
const Handle(Image_PixMap) & Graphic3d_MarkerImage::GetImage() {
    ...
    for (Standard_Integer aRowIter = 0; aRowIter < myHeight; aRowIter++) {
        ...
        for (Standard_Integer aColumnIter = 0; aColumnIter < myWidth; aColumnIter++) {
            Standard_Boolean aBitOn = (myBitMap->Value(aLowerIndex + aNumOfBytesInRow * aRowIter + aColumnIter / 8) &
                                       (0x80 >> (aColumnIter % 8))) != 0;
...
// purpose  :
// =======================================================================
void Graphic3d_MarkerImage::GetTextureSize(Standard_Integer& theWidth, Standard_Integer& theHeight) const {
    theWidth = myWidth;
    theHeight = myHeight;
}
...
Handle(Graphic3d_MarkerImage) Graphic3d_MarkerImage::StandardMarker(const Aspect_TypeOfMarker theMarkerType,
    ...
    TCollection_AsciiString aKey =
        TCollection_AsciiString("Graphic3d_MarkerImage_") + THE_MARKER_NAMES[theMarkerType] + "_" + aScaleInt;
    TCollection_AsciiString aKeyA =
        TCollection_AsciiString("Graphic3d_MarkerImageAlpha_") + THE_MARKER_NAMES[theMarkerType] + "_" + aScaleInt;
    if (theMarkerType == Aspect_TOM_BALL) {
        unsigned int aColor[3] = {(unsigned int)(255.0f * theColor.r()), (unsigned int)(255.0f * theColor.g()),
