...
public:
    ...
    public:
        ...
        Iterator(const Graphic3d_LightSet& theSet, IterationFilter theFilter = IterationFilter_None)
            : myIter(theSet.myLights), myFilter(theFilter) {
            skipFiltered();
        }
        ...
        Iterator(const Handle(Graphic3d_LightSet) & theSet, IterationFilter theFilter = IterationFilter_None)
            ...
            if (!theSet.IsNull()) {
                myIter =
                    NCollection_IndexedDataMap<Handle(Graphic3d_CLight), Standard_Size>::Iterator(theSet->myLights);
                skipFiltered();
            }
        }
        ...
        void skipFiltered() {
            ...
            for (; myIter.More(); myIter.Next()) {
                if ((myFilter & IterationFilter_ExcludeAmbient) != 0 &&
                    ...
                    continue;
                } else if ((myFilter & IterationFilter_ExcludeNoShadow) != 0 && !myIter.Key()->ToCastShadows()) {
                    continue;
                }
...
public:
    ...
    Standard_Integer Upper() const {
        return myLights.Extent();
    }

    //! Return TRUE if lights list is empty.
    Standard_Boolean IsEmpty() const {
        return myLights.IsEmpty();
    }
...
public:
    ...
    //! @sa UpdateRevision()
    Standard_Integer NbCastShadows() const {
    ...
    //! @sa UpdateRevision()
    const Graphic3d_Vec4& AmbientColor() const {
        return myAmbient;
    }

    //! Returns a string defining a list of enabled light sources as concatenation of letters 'd' (Directional), 'p'
    //! (Point), 's' (Spot) depending on the type of light source in the list. Example: "dppp".
    //! @sa UpdateRevision()
    const TCollection_AsciiString& KeyEnabledLong() const {
        return myKeyEnabledLong;
    }

    //! Returns a string defining a list of enabled light sources as concatenation of letters 'd' (Directional), 'p'
    //! (Point), 's' (Spot) depending on the type of light source in the list, specified only once. Example: "dp".
    //! @sa UpdateRevision()
    const TCollection_AsciiString& KeyEnabledShort() const {
        return myKeyEnabledShort;
    }
...
protected:
    ...
    TCollection_AsciiString
        myKeyEnabledShort; //!< key identifying the list of enabled light sources by the number of sources of each type
    Standard_Integer
        myLightTypes[Graphic3d_TypeOfLightSource_NB]; //!< counters per each light source type defined in the list
    Standard_Integer myLightTypesEnabled[Graphic3d_TypeOfLightSource_NB]; //!< counters per each light source type
                                                                          //!< enabled in the list
