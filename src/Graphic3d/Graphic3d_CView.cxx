...
Graphic3d_CView::Graphic3d_CView(const Handle(Graphic3d_StructureManager) & theMgr)
    : myBgColor(Quantity_NOC_BLACK), myBackgroundType(Graphic3d_TOB_NONE), myStructureManager(theMgr),
      myCamera(new Graphic3d_Camera()), myHiddenObjects(new Graphic3d_NMapOfTransient()),
      myIsInComputedMode(Standard_False), myIsActive(Standard_False), myIsRemoved(Standard_False),
      myBackfacing(Graphic3d_TypeOfBackfacingModel_Auto), myVisualization(Graphic3d_TOV_WIREFRAME), myUnitFactor(1.0) {
...
void Graphic3d_CView::SetComputedMode(const Standard_Boolean theMode) {
    ...
    for (Graphic3d_MapOfStructure::Iterator aDispStructIter(myStructsDisplayed); aDispStructIter.More();
        ...
        if (anIndex != 0) {
            ...
            const Standard_Boolean toComputeWireframe =
                myVisualization == Graphic3d_TOV_WIREFRAME && aStruct->ComputeVisual() != Graphic3d_TOS_SHADING;
            const Standard_Boolean toComputeShading =
                myVisualization == Graphic3d_TOV_SHADING && aStruct->ComputeVisual() != Graphic3d_TOS_WIREFRAME;
...
void Graphic3d_CView::ReCompute(const Handle(Graphic3d_Structure) & theStruct) {
    ...
    if (!theStruct->IsMutable() && !theStruct->CStructure()->IsForHighlight && !theStruct->CStructure()->IsInfinite) {
    ...
    const Standard_Boolean toComputeWireframe =
        myVisualization == Graphic3d_TOV_WIREFRAME && theStruct->ComputeVisual() != Graphic3d_TOS_SHADING;
    const Standard_Boolean toComputeShading =
        myVisualization == Graphic3d_TOV_SHADING && theStruct->ComputeVisual() != Graphic3d_TOS_WIREFRAME;
...
Bnd_Box Graphic3d_CView::MinMaxValues(const Standard_Boolean theToIncludeAuxiliary) const {
    ...
    for (NCollection_List<Handle(Graphic3d_Layer)>::Iterator aLayerIter(Layers()); aLayerIter.More();
        ...
        const Handle(Graphic3d_Layer) & aLayer = aLayerIter.Value();
        Bnd_Box aBox =
            aLayer->BoundingBox(Identification(), aCamera, aWinSize.x(), aWinSize.y(), theToIncludeAuxiliary);
        aResult.Add(aBox);
    }
    return aResult;
}
...
Standard_Real Graphic3d_CView::ConsiderZoomPersistenceObjects() {
    ...
    for (NCollection_List<Handle(Graphic3d_Layer)>::Iterator aLayerIter(Layers()); aLayerIter.More();
        ...
        aMaxCoef = Max(aMaxCoef,
                       aLayer->considerZoomPersistenceObjects(Identification(), aCamera, aWinSize.x(), aWinSize.y()));
    }
...
Bnd_Box Graphic3d_CView::MinMaxValues(const Graphic3d_MapOfStructure& theSet,
    ...
    for (Graphic3d_MapIteratorOfMapOfStructure aStructIter(theSet); aStructIter.More(); aStructIter.Next()) {
        ...
        if (!aStructure->TransformPersistence().IsNull()) {
            ...
            aStructure->TransformPersistence()->Apply(aCamera, aProjectionMat, aWorldViewMat, aWinWidth, aWinHeight,
                                                      aBox);
        }
        ...
        if (Abs(aBox.CornerMax().X()) >= ShortRealLast() || Abs(aBox.CornerMax().Y()) >= ShortRealLast() ||
            Abs(aBox.CornerMax().Z()) >= ShortRealLast() || Abs(aBox.CornerMin().X()) >= ShortRealLast() ||
            Abs(aBox.CornerMin().Y()) >= ShortRealLast() || Abs(aBox.CornerMin().Z()) >= ShortRealLast()) {
            continue;
        }
...
void Graphic3d_CView::Compute() {
    ...
    for (NCollection_Sequence<Handle(Graphic3d_Structure)>::Iterator aStructIter(aStructsSeq); aStructIter.More();
         aStructIter.Next()) {
...
// purpose  :
// =======================================================================
void Graphic3d_CView::Clear(Graphic3d_Structure* theStructure, const Standard_Boolean theWithDestruction) {
...
// purpose  :
// =======================================================================
void Graphic3d_CView::Connect(const Graphic3d_Structure* theMother, const Graphic3d_Structure* theDaughter) {
...
// purpose  :
// =======================================================================
void Graphic3d_CView::Disconnect(const Graphic3d_Structure* theMother, const Graphic3d_Structure* theDaughter) {
    Standard_Integer anIndexM = IsComputed(theMother);
    Standard_Integer anIndexD = IsComputed(theDaughter);
    if (anIndexM != 0 && anIndexD != 0) {
        const Handle(Graphic3d_Structure) & aStructM = myStructsComputed.Value(anIndexM);
...
void Graphic3d_CView::Display(const Handle(Graphic3d_Structure) & theStructure) {
    ...
    // Of which type will be the computed?
    const Standard_Boolean toComputeWireframe =
        myVisualization == Graphic3d_TOV_WIREFRAME && theStructure->ComputeVisual() != Graphic3d_TOS_SHADING;
    const Standard_Boolean toComputeShading =
        myVisualization == Graphic3d_TOV_SHADING && theStructure->ComputeVisual() != Graphic3d_TOS_WIREFRAME;
...
void Graphic3d_CView::SetTransform(const Handle(Graphic3d_Structure) & theStructure,
    ...
    if (anIndex != 0) {
        ...
        // a scale
        if (!theTrsf.IsNull() &&
            (theTrsf->Form() == gp_Translation || theTrsf->Form() == gp_Scale || theTrsf->Form() == gp_CompoundTrsf)) {
...
Standard_Boolean Graphic3d_CView::IsComputed(const Standard_Integer theStructId,
    ...
    if (!ComputedMode()) return Standard_False;
...
void Graphic3d_CView::ChangePriority(const Handle(Graphic3d_Structure) & theStructure,
                                     const Standard_Integer /*theOldPriority*/, const Standard_Integer theNewPriority) {
...
void Graphic3d_CView::ProcessXRInput() {
    ...
    // @endcode
    // so that Eye position is encoded into Orientation matrix, and there should be 2 Orientation matrices and 2
    // Projection matrices to make the stereo. Graphic3d_Camera historically follows different decomposition, with Eye
    // position encoded into Projection matrix, so that there is only 1 Orientation matrix (matching mono view) and 2
    // Projection matrices.
...
// purpose  :
//=======================================================================
void Graphic3d_CView::ComputeXRPosedCameraFromBase(Graphic3d_Camera& theCam, const gp_Trsf& theXRTrsf) const {
...
// purpose  :
//=======================================================================
void Graphic3d_CView::ComputeXRBaseCameraFromPosed(const Graphic3d_Camera& theCamPosed, const gp_Trsf& thePoseTrsf) {
    const gp_Ax3 anAxVr(gp::Origin(), gp::DZ(), gp::DX());
    const gp_Ax3 aCameraCS(gp::Origin(), -myBaseXRCamera->Direction(), -myBaseXRCamera->SideRight());
    gp_Trsf aTrsfCS;
    aTrsfCS.SetTransformation(aCameraCS, anAxVr);
...
void Graphic3d_CView::DiagnosticInformation(TColStd_IndexedDataMapOfStringString& theDict,
    ...
    if ((theFlags & Graphic3d_DiagnosticInfo_Device) != 0 && !myXRSession.IsNull()) {
        ...
        TCollection_AsciiString aDisplay = TCollection_AsciiString() + myXRSession->RecommendedViewport().x() + "x" +
