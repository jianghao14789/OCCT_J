...
private:
    ...
    template <typename Elem_t> struct TransformMatrices {
        ...
        Standard_Boolean IsOrientationValid() const {
            return myIsOrientationValid;
        }

        //! Return true if Projection was not invalidated.
        Standard_Boolean IsProjectionValid() const {
            return myIsProjectionValid;
        }
...
public:
    ...
    //! In this case, application might define intermediate camera positions for interpolation or implement own
    //! interpolation logic.
    //!
    ...
    Standard_EXPORT static void Interpolate(const Handle(Graphic3d_Camera) & theStart,
                                            const Handle(Graphic3d_Camera) & theEnd, const double theT,
...
public:
    ...
    Standard_EXPORT bool FitMinMax(const Bnd_Box& theBox, const Standard_Real theResolution,
    ...
    Standard_EXPORT bool ZFitAll(const Standard_Real theScaleFactor, const Bnd_Box& theMinMax,
...
public:
    ...
    //! Return offset to the view corner in NDC space within dimension X for 2d on-screen elements, which is normally
    //! 0.5. Can be clamped when FOVx exceeds FOV2d.
    Standard_Real NDC2dOffsetX() const {
        return myFOV2d >= myFOVx ? 0.5 : 0.5 * myFOV2d / myFOVx;
    }

    //! Return offset to the view corner in NDC space within dimension X for 2d on-screen elements, which is normally
    //! 0.5. Can be clamped when FOVy exceeds FOV2d.
    Standard_Real NDC2dOffsetY() const {
        return myFOV2d >= myFOVy ? 0.5 : 0.5 * myFOV2d / myFOVy;
    }
    ...
    Standard_EXPORT void Frustum(gp_Pln& theLeft, gp_Pln& theRight, gp_Pln& theBottom, gp_Pln& theTop, gp_Pln& theNear,
                                 gp_Pln& theFar) const;
...
public:
    ...
    Standard_EXPORT void StereoProjection(Graphic3d_Mat4d& theProjL, Graphic3d_Mat4d& theHeadToEyeL,
    ...
    //! @param theProjR      [out] right eye projection matrix
    //! @param theHeadToEyeR [out] right head to eye translation matrix
    Standard_EXPORT void StereoProjectionF(Graphic3d_Mat4& theProjL, Graphic3d_Mat4& theHeadToEyeL,
...
private:
    ...
    Standard_EXPORT void stereoProjection(NCollection_Mat4<Elem_t>& theProjL, NCollection_Mat4<Elem_t>& theHeadToEyeL,
    ...
    template <typename Elem_t>
    Standard_EXPORT void computeProjection(NCollection_Mat4<Elem_t>& theProjM, NCollection_Mat4<Elem_t>& theProjL,
    ...
    //! @param theMatrices [in] the matrices data container.
    template <typename Elem_t>
    Standard_EXPORT TransformMatrices<Elem_t>& UpdateOrientation(TransformMatrices<Elem_t>& theMatrices) const;
...
private:
    ...
    template <typename Elem_t>
    void orthoProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT, const Elem_t theNear,
    ...
    //! @param theFar [in] the far mapping (clipping) coordinate
    template <typename Elem_t>
    void perspectiveProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT,
    ...
    template <typename Elem_t>
    void stereoEyeProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT,
                       const Elem_t theNear, const Elem_t theFar, const Elem_t theIOD, const Elem_t theZFocus,
    ...
    template <typename Elem_t>
    static void LookOrientation(const NCollection_Vec3<Elem_t>& theEye, const NCollection_Vec3<Elem_t>& theFwdDir,
                                const NCollection_Vec3<Elem_t>& theUpDir, const NCollection_Vec3<Elem_t>& theAxialScale,
