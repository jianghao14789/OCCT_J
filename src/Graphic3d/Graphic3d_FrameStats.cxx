...
namespace {
//! Format counter.
static std::ostream& formatCounter(std::ostream& theStream, Standard_Integer theWidth, const char* thePrefix,
                                   Standard_Size theValue, const char* thePostfix = NULL) {
    if (thePrefix != NULL) {
        theStream << thePrefix;
    }
...
static std::ostream& formatCounterPair(std::ostream& theStream, Standard_Integer theWidth, const char* thePrefix,
                                       Standard_Size theValue, Standard_Size theImmValue, bool theToShowImmediate) {
...
static std::ostream& formatBytes(std::ostream& theStream, Standard_Integer theWidth, const char* thePrefix,
                                 Standard_Size theValue, const char* thePostfix = NULL) {
    if (thePrefix != NULL) {
        theStream << thePrefix;
    }
...
//! Format time.
static std::ostream& formatTime(std::ostream& theStream, Standard_Integer theWidth, const char* thePrefix,
                                Standard_Real theSeconds, const char* thePostfix = NULL) {
    if (thePrefix != NULL) {
        theStream << thePrefix;
    }
...
//! Format time.
static void addTimeInfo(TColStd_IndexedDataMapOfStringString& theDict, const TCollection_AsciiString& theKey,
...
Graphic3d_FrameStats::Graphic3d_FrameStats()
    : myFpsTimer(Standard_True), myFrameStartTime(0.0), myFrameDuration(0.0), myUpdateInterval(1.0), myFpsFrameCount(0),
      myCounters(0, 0), myLastFrameIndex(0), myIsLongLineFormat(Standard_False) {
    //
}
...
TCollection_AsciiString Graphic3d_FrameStats::FormatStats(Graphic3d_RenderingParams::PerfCounters theFlags) const {
    ...
    const Standard_Boolean isCompact =
        theFlags == Graphic3d_RenderingParams::PerfCounters_FrameRate; // only FPS is displayed
    ...
    if (myIsLongLineFormat && (theFlags & Graphic3d_RenderingParams::PerfCounters_FrameRate) != 0 &&
        ...
        aBuf << "FPS: " << std::setfill(' ') << std::setw(isCompact ? aValWidth : 9) << std::fixed
             ...
             << std::setw(isCompact ? aValWidth : 10) << std::fixed << std::setprecision(1) << aStats.FrameRateCpu()
             << "]\n";
    } else {
        if ((theFlags & Graphic3d_RenderingParams::PerfCounters_FrameRate) != 0) {
            aBuf << "FPS:     " << std::setfill(' ') << std::setw(isCompact ? aValWidth : aValWidth + 3) << std::fixed
                 << std::setprecision(1) << aStats.FrameRate();
        ...
        if ((theFlags & Graphic3d_RenderingParams::PerfCounters_CPU) != 0) {
            aBuf << "CPU FPS: " << std::setfill(' ') << std::setw(isCompact ? aValWidth : aValWidth + 3) << std::fixed
                 << std::setprecision(1) << aStats.FrameRateCpu();
    ...
    if ((theFlags & Graphic3d_RenderingParams::PerfCounters_Groups) != 0 ||
        ...
        (!myIsLongLineFormat && ((theFlags & Graphic3d_RenderingParams::PerfCounters_Structures) != 0 ||
    ...
    if (!myIsLongLineFormat && (theFlags & Graphic3d_RenderingParams::PerfCounters_Layers) != 0) {
        formatCounterPair(aBuf, aValWidth, "    Layers: ", aStats[Graphic3d_FrameStatsCounter_NbLayersNotCulled],
    ...
    if (!myIsLongLineFormat && (theFlags & Graphic3d_RenderingParams::PerfCounters_Structures) != 0) {
        formatCounterPair(aBuf, aValWidth, "   Structs: ", aStats[Graphic3d_FrameStatsCounter_NbStructsNotCulled],
                          aStats[Graphic3d_FrameStatsCounter_NbStructsImmediate], hasImmediate);
    }
    if ((theFlags & Graphic3d_RenderingParams::PerfCounters_Groups) != 0) {
        formatCounterPair(aBuf, aValWidth, "    Groups: ", aStats[Graphic3d_FrameStatsCounter_NbGroupsNotCulled],
    ...
    if ((theFlags & Graphic3d_RenderingParams::PerfCounters_GroupArrays) != 0) {
        ...
        formatCounterPair(aBuf, aValWidth, "    [fill]: ", aStats[Graphic3d_FrameStatsCounter_NbElemsFillNotCulled],
                          aStats[Graphic3d_FrameStatsCounter_NbElemsFillImmediate], hasImmediate);
        formatCounterPair(aBuf, aValWidth, "    [line]: ", aStats[Graphic3d_FrameStatsCounter_NbElemsLineNotCulled],
                          aStats[Graphic3d_FrameStatsCounter_NbElemsLineImmediate], hasImmediate);
        formatCounterPair(aBuf, aValWidth, "   [point]: ", aStats[Graphic3d_FrameStatsCounter_NbElemsPointNotCulled],
                          aStats[Graphic3d_FrameStatsCounter_NbElemsPointImmediate], hasImmediate);
        formatCounterPair(aBuf, aValWidth, "    [text]: ", aStats[Graphic3d_FrameStatsCounter_NbElemsTextNotCulled],
    ...
    if ((theFlags & Graphic3d_RenderingParams::PerfCounters_Triangles) != 0) {
        formatCounterPair(aBuf, aValWidth, " Triangles: ", aStats[Graphic3d_FrameStatsCounter_NbTrianglesNotCulled],
    ...
    if ((theFlags & Graphic3d_RenderingParams::PerfCounters_Lines) != 0) {
        formatCounterPair(aBuf, aValWidth, "     Lines: ", aStats[Graphic3d_FrameStatsCounter_NbLinesNotCulled],
...
// purpose  :
// =======================================================================
void Graphic3d_FrameStats::FrameStart(const Handle(Graphic3d_CView) & theView, bool theIsImmediateOnly) {
    const Graphic3d_RenderingParams::PerfCounters aBits =
        !theView.IsNull() ? theView->RenderingParams().CollectedStats : Graphic3d_RenderingParams::PerfCounters_NONE;
    if (theIsImmediateOnly && (aBits & Graphic3d_RenderingParams::PerfCounters_SkipImmediate) != 0) {
        return;
    }

    const Standard_Integer aNbFrames = Max(!theView.IsNull() ? theView->RenderingParams().StatsNbFrames : 1, 1);
...
void Graphic3d_FrameStats::FrameEnd(const Handle(Graphic3d_CView) & theView, bool theIsImmediateOnly) {
    ...
    if (aTime > gp::Resolution()) {
        ...
        if (theIsImmediateOnly) {
            ...
            myCountersTmp.ChangeFrameRateCpu() = aCpuSec > gp::Resolution() ? double(myFpsFrameCount) / aCpuSec : -1.0;
        }
    ...
    if (theIsImmediateOnly) {
        ...
        for (Standard_Integer aCntIter = Graphic3d_FrameStatsCounter_RENDERED_LOWER;
             aCntIter <= Graphic3d_FrameStatsCounter_RENDERED_UPPER; ++aCntIter) {
