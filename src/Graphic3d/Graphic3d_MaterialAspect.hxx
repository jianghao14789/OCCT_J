...
public:
    ...
    Graphic3d_NameOfMaterial RequestedName() const {
        return myRequestedMaterialName;
    }
    ...
    //! WARNING! This method does NOT return color for Graphic3d_MATERIAL_ASPECT material (color is defined by
    //! Graphic3d_Aspects::InteriorColor()).
    const Quantity_Color& Color() const {
        return myColors[Graphic3d_TOR_DIFFUSE];
    }
    ...
    Standard_ShortReal Transparency() const {
        return myTransparencyCoef;
    }

    //! Returns the alpha coefficient of the surface (1.0 - Transparency); 1.0 means opaque.
    Standard_ShortReal Alpha() const {
        return 1.0f - myTransparencyCoef;
    }

    //! Modifies the transparency coefficient of the surface, where 0 is opaque and 1 is fully transparent.
    //! Transparency is applicable to materials that have at least one of reflection modes (ambient, diffuse, specular
    //! or emissive) enabled. See also SetReflectionModeOn() and SetReflectionModeOff() methods.
    //!
    ...
    const Quantity_Color& SpecularColor() const {
        return myColors[Graphic3d_TOR_SPECULAR];
    }
    ...
    const Quantity_Color& EmissiveColor() const {
        return myColors[Graphic3d_TOR_EMISSION];
    }
    ...
    Standard_Boolean IsEqual(const Graphic3d_MaterialAspect& theOther) const {
        return myTransparencyCoef == theOther.myTransparencyCoef && myRefractionIndex == theOther.myRefractionIndex &&
               myBSDF == theOther.myBSDF && myPBRMaterial == theOther.myPBRMaterial &&
