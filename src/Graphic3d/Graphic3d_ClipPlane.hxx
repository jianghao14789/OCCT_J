...
public: // @name user-defined graphical attributes
    ...
    Quantity_Color CappingColor() const {
        return myAspect->FrontMaterial().MaterialType() == Graphic3d_MATERIAL_ASPECT ? myAspect->FrontMaterial().Color()
    ...
    Handle(Graphic3d_TextureMap) CappingTexture() const {
        return !myAspect->TextureSet().IsNull() && !myAspect->TextureSet()->IsEmpty() ? myAspect->TextureSet()->First()
    ...
    Standard_Boolean IsHatchOn() const {
        return myAspect->InteriorStyle() == Aspect_IS_HATCH;
    }
...
public:
    ...
    bool ToUseObjectTexture() const {
        return (myFlags & Graphic3d_CappingFlags_ObjectTexture) != 0;
    }
    ...
    bool ToUseObjectShader() const {
        return (myFlags & Graphic3d_CappingFlags_ObjectShader) != 0;
    }
    ...
    bool ToUseObjectProperties() const {
        return myFlags != Graphic3d_CappingFlags_None;
    }
...
public:
    ...
    Graphic3d_ClipState ProbePointHalfspace(const Graphic3d_Vec4d& thePoint) const {
        ...
        return aVal < 0.0 ? Graphic3d_ClipState_Out : (aVal == 0.0 ? Graphic3d_ClipState_On : Graphic3d_ClipState_In);
    }
    ...
    bool IsBoxFullOutHalfspace(const Graphic3d_BndBox3d& theBox) const {
        const Graphic3d_Vec4d aMaxPnt(myEquation.x() > 0.0 ? theBox.CornerMax().x() : theBox.CornerMin().x(),
                                      ...
                                      myEquation.z() > 0.0 ? theBox.CornerMax().z() : theBox.CornerMin().z(), 1.0);
    ...
    //! Check if the given bounding box is fully inside (or touches from inside) the half-space (e.g. NOT discarded by
    //! clipping plane).
    bool IsBoxFullInHalfspace(const Graphic3d_BndBox3d& theBox) const {
        const Graphic3d_Vec4d aMinPnt(myEquation.x() > 0.0 ? theBox.CornerMin().x() : theBox.CornerMax().x(),
                                      ...
                                      myEquation.z() > 0.0 ? theBox.CornerMin().z() : theBox.CornerMax().z(), 1.0);
