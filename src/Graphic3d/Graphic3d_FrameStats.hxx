...
public:
    ...
    void SetUpdateInterval(Standard_Real theInterval) {
        myUpdateInterval = theInterval;
    }

    //! Prefer longer lines over more greater of lines.
    Standard_Boolean IsLongLineFormat() const {
        return myIsLongLineFormat;
    }

    //! Set if format should prefer longer lines over greater number of lines.
    void SetLongLineFormat(Standard_Boolean theValue) {
        myIsLongLineFormat = theValue;
    }

    //! Frame redraw started.
    Standard_EXPORT virtual void FrameStart(const Handle(Graphic3d_CView) & theView, bool theIsImmediateOnly);

    //! Frame redraw finished.
    Standard_EXPORT virtual void FrameEnd(const Handle(Graphic3d_CView) & theView, bool theIsImmediateOnly);
...
public:
    ...
    Standard_Real FrameRate() const {
        return LastDataFrame().FrameRate();
    }

    //! Returns CPU FPS (frames per seconds, CPU time).
    //! This number indicates a PREDICTED frame rate,
    //! basing on CPU elapsed time between updates and NOT real elapsed time (which might include periods of CPU
    //! inactivity). Number is expected to be greater then actual frame rate returned by FrameRate(). Values
    //! significantly greater actual frame rate indicate that rendering is limited by GPU performance (CPU is stalled
    //! in-between), while values around actual frame rate indicate rendering being limited by CPU performance (GPU is
    //! stalled in-between).
    Standard_Real FrameRateCpu() const {
        return LastDataFrame().FrameRateCpu();
    }
    ...
    //! Returns value of specified timer for modification, should be called between ::FrameStart() and ::FrameEnd()
    //! calls. Should NOT be called between ::FrameStart() and ::FrameEnd() calls.
...
public:
    //! Returns value of specified counter for modification, should be called between ::FrameStart() and ::FrameEnd()
    //! calls.
    ...
    //! Returns currently filling data frame for modification, should be called between ::FrameStart() and ::FrameEnd()
    //! calls.
    Graphic3d_FrameStatsDataTmp& ActiveDataFrame() {
        return myCountersTmp;
    }
...
protected:
    ...
    Graphic3d_FrameStatsDataTmp
        myCountersTmp; //!< data frame values filled to be filled between FrameStart() and FrameEnd() calls
