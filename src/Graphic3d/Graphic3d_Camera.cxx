...
Graphic3d_Camera::Graphic3d_Camera()
    : myUp(0.0, 1.0, 0.0), myDirection(0.0, 0.0, 1.0), myEye(0.0, 0.0, -1500.0), myDistance(1500.0),
      myAxialScale(1.0, 1.0, 1.0), myProjType(Projection_Orthographic), myFOVy(45.0), myFOVx(45.0), myFOV2d(180.0),
      myFOVyTan(Tan(DTR_HALF * 45.0)), myZNear(DEFAULT_ZNEAR), myZFar(DEFAULT_ZFAR), myAspect(1.0),
      myIsZeroToOneDepth(false), myScale(1000.0), myZFocus(1.0), myZFocusType(FocusType_Relative), myIOD(0.05),
      myIODType(IODType_Relative), myIsCustomProjMatM(false), myIsCustomProjMatLR(false), myIsCustomFrustomLR(false) {
    myWorldViewProjState.Initialize((Standard_Size)Standard_Atomic_Increment(&THE_STATE_COUNTER),
                                    (Standard_Size)Standard_Atomic_Increment(&THE_STATE_COUNTER), this);
}
...
void Graphic3d_Camera::CopyMappingData(const Handle(Graphic3d_Camera) & theOtherCamera) {
    ...
    if (theOtherCamera->IsCustomStereoProjection()) {
        SetCustomStereoProjection(theOtherCamera->myCustomProjMatL, theOtherCamera->myCustomHeadToEyeMatL,
                                  theOtherCamera->myCustomProjMatR, theOtherCamera->myCustomHeadToEyeMatR);
...
void Graphic3d_Camera::CopyOrientationData(const Handle(Graphic3d_Camera) & theOtherCamera) {
    if (!myEye.IsEqual(theOtherCamera->Eye(), 0.0) || !myUp.IsEqual(theOtherCamera->Up(), 0.0) ||
        !myDirection.IsEqual(theOtherCamera->Direction(), 0.0) || myDistance != theOtherCamera->Distance()) {
...
static Graphic3d_Vec4d safePointCast(const gp_Pnt& thePnt) {
    ...
    if (Abs(aSafePoint.X()) > aLim) aSafePoint.SetX(aSafePoint.X() >= 0 ? aBigFloat : -aBigFloat);
    if (Abs(aSafePoint.Y()) > aLim) aSafePoint.SetY(aSafePoint.Y() >= 0 ? aBigFloat : -aBigFloat);
    if (Abs(aSafePoint.Z()) > aLim) aSafePoint.SetZ(aSafePoint.Z() >= 0 ? aBigFloat : -aBigFloat);
...
// purpose  :
// =======================================================================
void Graphic3d_Camera::Frustum(gp_Pln& theLeft, gp_Pln& theRight, gp_Pln& theBottom, gp_Pln& theTop, gp_Pln& theNear,
                               gp_Pln& theFar) const {
...
// purpose  :
// =======================================================================
void Graphic3d_Camera::StereoProjection(Graphic3d_Mat4d& theProjL, Graphic3d_Mat4d& theHeadToEyeL,
                                        Graphic3d_Mat4d& theProjR, Graphic3d_Mat4d& theHeadToEyeR) const {
...
// purpose  :
// =======================================================================
void Graphic3d_Camera::StereoProjectionF(Graphic3d_Mat4& theProjL, Graphic3d_Mat4& theHeadToEyeL,
                                         Graphic3d_Mat4& theProjR, Graphic3d_Mat4& theHeadToEyeR) const {
    stereoProjection(theProjL, theHeadToEyeL, theProjR, theHeadToEyeR);
}
...
void Graphic3d_Camera::stereoProjection(NCollection_Mat4<Elem_t>& theProjL, NCollection_Mat4<Elem_t>& theHeadToEyeL,
...
// purpose  :
// =======================================================================
void Graphic3d_Camera::SetCustomStereoProjection(const Graphic3d_Mat4d& theProjL, const Graphic3d_Mat4d& theHeadToEyeL,
...
// =======================================================================
template <typename Elem_t>
void Graphic3d_Camera::computeProjection(NCollection_Mat4<Elem_t>& theProjM, NCollection_Mat4<Elem_t>& theProjL,
                                         NCollection_Mat4<Elem_t>& theProjR, bool theToAddHeadToEye) const {
    ...
    Elem_t aIOD = myIODType == IODType_Relative ? static_cast<Elem_t>(myIOD * Distance()) : static_cast<Elem_t>(myIOD);

    Elem_t aFocus =
        myZFocusType == FocusType_Relative ? static_cast<Elem_t>(myZFocus * Distance()) : static_cast<Elem_t>(myZFocus);
    ...
    if (myTile.IsValid()) {
        ...
        anLRBT.Left =
            -aDXHalf + aDXFull * static_cast<Elem_t>(anOffset.x()) / static_cast<Elem_t>(myTile.TotalSize.x());
        anLRBT.Right = -aDXHalf + aDXFull * static_cast<Elem_t>(anOffset.x() + myTile.TileSize.x()) /
                                      static_cast<Elem_t>(myTile.TotalSize.x());
        anLRBT.Bottom =
            -aDYHalf + aDYFull * static_cast<Elem_t>(anOffset.y()) / static_cast<Elem_t>(myTile.TotalSize.y());
        anLRBT.Top = -aDYHalf + aDYFull * static_cast<Elem_t>(anOffset.y() + myTile.TileSize.y()) /
...
Graphic3d_Camera::UpdateOrientation(TransformMatrices<Elem_t>& theMatrices) const {
    ...
    NCollection_Vec3<Elem_t> anEye(static_cast<Elem_t>(myEye.X()), static_cast<Elem_t>(myEye.Y()),
                                   static_cast<Elem_t>(myEye.Z()));

    NCollection_Vec3<Elem_t> aViewDir(static_cast<Elem_t>(myDirection.X()), static_cast<Elem_t>(myDirection.Y()),
                                      static_cast<Elem_t>(myDirection.Z()));

    NCollection_Vec3<Elem_t> anUp(static_cast<Elem_t>(myUp.X()), static_cast<Elem_t>(myUp.Y()),
                                  static_cast<Elem_t>(myUp.Z()));

    NCollection_Vec3<Elem_t> anAxialScale(static_cast<Elem_t>(myAxialScale.X()), static_cast<Elem_t>(myAxialScale.Y()),
...
// =======================================================================
template <typename Elem_t>
void Graphic3d_Camera::orthoProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT,
                                 const Elem_t theNear, const Elem_t theFar) const {
    // row 0
...
// =======================================================================
template <typename Elem_t>
void Graphic3d_Camera::perspectiveProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT,
                                       const Elem_t theNear, const Elem_t theFar) const {
    // column 0
...
// =======================================================================
template <typename Elem_t>
void Graphic3d_Camera::stereoEyeProj(NCollection_Mat4<Elem_t>& theOutMx, const Aspect_FrustumLRBT<Elem_t>& theLRBT,
                                     const Elem_t theNear, const Elem_t theFar, const Elem_t theIOD,
                                     const Elem_t theZFocus, const Aspect_Eye theEyeIndex) const {
...
// purpose  :
// =======================================================================
bool Graphic3d_Camera::FitMinMax(const Bnd_Box& theBox, const Standard_Real theResolution,
    ...
    Frustum(aFrustumPlane[1], aFrustumPlane[2], aFrustumPlane[3], aFrustumPlane[4], aFrustumPlane[5], aFrustumPlane[6]);
...
// purpose  :
//=============================================================================
bool Graphic3d_Camera::ZFitAll(const Standard_Real theScaleFactor, const Bnd_Box& theMinMax,
                               const Bnd_Box& theGraphicBB, Standard_Real& theZNear, Standard_Real& theZFar) const {
    ...
    for (; aPntIt.More(); aPntIt.Next()) {
        ...
        aMeasurePnt = gp_Pnt(aMeasurePnt.X() * anAxialScale.X(), aMeasurePnt.Y() * anAxialScale.Y(),
        ...
        if (aVecToMeasurePnt.Magnitude() > gp::Resolution() && aCamDir.IsOpposite(aVecToMeasurePnt, M_PI * 0.5)) {
            aDistance *= -1;
        }
...
// purpose  :
//=============================================================================
void Graphic3d_Camera::Interpolate(const Handle(Graphic3d_Camera) & theStart, const Handle(Graphic3d_Camera) & theEnd,
                                   const double theT, Handle(Graphic3d_Camera) & theCamera) {
    ...
    // apply scaling
    if (Abs(theStart->Scale() - theEnd->Scale()) > Precision::Confusion() && theStart->IsOrthographic()) {
