...
protected:
    ...
    template <class Struct_t> class SubclassStructIterator {
    public:
        ...
        const Struct_t* Value() const {
            return (const Struct_t*)(myIter.Value());
        }
        Struct_t* ChangeValue() {
            return (Struct_t*)(myIter.Value());
        }
    ...
    template <class Group_t> class SubclassGroupIterator {
    public:
        ...
        const Group_t* Value() const {
            return (const Group_t*)(myIter.Value().get());
        }
        Group_t* ChangeValue() {
            return (Group_t*)(myIter.ChangeValue().get());
        }
...
protected:
    ...
    Handle(Graphic3d_PresentationAttributes)
        myHighlightStyle; //! Current highlight style; is set only if highlight flag is true

    Graphic3d_ZLayerId myZLayer;

    mutable Standard_Boolean
        myIsCulled; //!< A status specifying is structure needs to be rendered after BVH tree traverse
    Standard_Boolean
        myBndBoxClipCheck; //!< Flag responsible for checking of bounding box clipping before drawing of object

    Standard_Boolean myHasGroupTrsf; //!< flag specifying that some groups might have transform persistence

public:
    unsigned IsInfinite : 1;
    unsigned stick : 1; //!< displaying state - should be set when structure has been added to scene graph (but can be
                        //!< in hidden state)
