...
public:
    ...
    //! Returns the way how alpha value should be treated (Graphic3d_AlphaMode_BlendAuto by default, for backward
    //! compatibility).
    ...
    //! If the alpha value is greater than or equal to this value then it is rendered as fully opaque, otherwise, it is
    //! rendered as fully transparent.
    Standard_ShortReal AlphaCutoff() const {
        return myAlphaCutoff;
    }
    ...
    void SetPolygonOffsets(const Standard_Integer theMode, const Standard_ShortReal theFactor = 1.0f,
...
public:
    ...
    void SetTextAngle(Standard_ShortReal theAngle) {
        myTextAngle = (Standard_ShortReal)theAngle;
    }
...
public:
    ...
    bool ToDrawEdges() const {
        return myToDrawEdges && myLineType != Aspect_TOL_EMPTY;
    }
    ...
    //! Graphics hardware works mostly with triangles, so that wireframe presentation will draw triangle edges by
    //! default. This flag allows rendering wireframe presentation of quad-only array split into triangles. For this,
    //! quads should be split in specific order, so that the quad diagonal (to be NOT rendered) goes first:
    //!     1------2
    //!    /      /   Triangle #1: 2-0-1; Triangle #2: 0-2-3
    //!   0------3
    bool ToSkipFirstEdge() const {
        return myToSkipFirstEdge;
    }
    ...
    void SetSkipFirstEdge(bool theToSkipFirstEdge) {
        myToSkipFirstEdge = theToSkipFirstEdge;
    }
...
public:
    ...
    bool IsEqual(const Graphic3d_Aspects& theOther) {
        ...
        return myProgram == theOther.myProgram && myTextureSet == theOther.myTextureSet &&
               ...
               myBackInteriorColor == theOther.myBackInteriorColor && myFrontMaterial == theOther.myFrontMaterial &&
               myBackMaterial == theOther.myBackMaterial && myInteriorStyle == theOther.myInteriorStyle &&
               ...
               myToDrawEdges == theOther.myToDrawEdges && myToDrawSilhouette == theOther.myToDrawSilhouette &&
               myToMapTexture == theOther.myToMapTexture && myIsTextZoomable == theOther.myIsTextZoomable;
    }
...
public:
    Standard_DEPRECATED("Deprecated method, FaceCulling() should be used instead") bool ToSuppressBackFaces() const {
        return myFaceCulling == Graphic3d_TypeOfBackfacingModel_BackCulled ||
               myFaceCulling == Graphic3d_TypeOfBackfacingModel_Auto;
    }

    Standard_DEPRECATED("Deprecated method, SetFaceCulling() should be used instead") void SetSuppressBackFaces(
        bool theToSuppress) {
        myFaceCulling =
            theToSuppress ? Graphic3d_TypeOfBackfacingModel_Auto : Graphic3d_TypeOfBackfacingModel_DoubleSided;
    }

    Standard_DEPRECATED("Deprecated method, FaceCulling() should be used instead") bool BackFace() const {
        return myFaceCulling == Graphic3d_TypeOfBackfacingModel_BackCulled ||
               myFaceCulling == Graphic3d_TypeOfBackfacingModel_Auto;
    }

    Standard_DEPRECATED("Deprecated method, SetFaceCulling() should be used instead") void AllowBackFace() {
        myFaceCulling = Graphic3d_TypeOfBackfacingModel_DoubleSided;
    }

    Standard_DEPRECATED("Deprecated method, SetFaceCulling() should be used instead") void SuppressBackFace() {
        myFaceCulling = Graphic3d_TypeOfBackfacingModel_Auto;
    }
