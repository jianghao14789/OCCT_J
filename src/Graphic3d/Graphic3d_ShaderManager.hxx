...
protected:
    ...
    Standard_EXPORT Handle(Graphic3d_ShaderProgram)
        getStdProgramUnlit(Standard_Integer theBits, Standard_Boolean theIsOutline = false) const;

    //! Prepare standard GLSL program with per-vertex lighting.
    //! @param theLights [in] list of light sources
    //! @param theBits   [in] program bits
    Standard_EXPORT Handle(Graphic3d_ShaderProgram)
        getStdProgramGouraud(const Handle(Graphic3d_LightSet) & theLights, Standard_Integer theBits) const;

    //! Prepare standard GLSL program with per-pixel lighting.
    //! @param theLights [in] list of light sources
    //! @param theBits   [in] program bits
    //! @param theIsFlatNormal [in] when TRUE, the Vertex normals will be ignored and Face normal will be computed
    //! instead
    //! @param theIsPBR  [in] when TRUE, the PBR pipeline will be activated
    //! @param theNbShadowMaps [in] number of shadow maps
    Standard_EXPORT Handle(Graphic3d_ShaderProgram)
        getStdProgramPhong(const Handle(Graphic3d_LightSet) & theLights, const Standard_Integer theBits,
    ...
    Standard_EXPORT Handle(Graphic3d_ShaderProgram)
        getStdProgramFboBlit(Standard_Integer theNbSamples, Standard_Boolean theIsFallback_sRGB) const;
...
protected:
    ...
    Standard_EXPORT Standard_Integer defaultGlslVersion(const Handle(Graphic3d_ShaderProgram) & theProgram,
                                                        ...
                                                        Standard_Integer theBits, bool theUsesDerivates = false) const;
    ...
    //! @param theHasVertColor [in]  flag to use getVertColor() instead of Ambient and Diffuse components of active
    //! material
    ...
    Standard_EXPORT TCollection_AsciiString stdComputeLighting(
        Standard_Integer& theNbLights, const Handle(Graphic3d_LightSet) & theLights, Standard_Boolean theHasVertColor,
        Standard_Boolean theIsPBR, Standard_Boolean theHasTexColor, Standard_Integer theNbShadowMaps) const;
...
protected:
    ...
    Standard_Boolean
        myUseRedAlpha; //!< use RED channel instead of ALPHA (e.g. GAPI supports only GL_RED textures and not GL_ALPHA)
