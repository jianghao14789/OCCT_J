...
// Purpose  :
// ================================================================
Graphic3d_MediaTexture::Graphic3d_MediaTexture(const Handle(Standard_HMutex) & theMutex, Standard_Integer thePlane)
    : Graphic3d_Texture2D("", Graphic3d_TOT_2D), myMutex(theMutex), myPlane(thePlane) {
...
Handle(Image_PixMap) Graphic3d_MediaTexture::GetImage(const Handle(Image_SupportedFormats) &) {
    ...
    if (myFrame.IsNull() || myFrame->IsLocked() || myFrame->IsEmpty() || myFrame->SizeX() < 1 || myFrame->SizeY() < 1) {
    ...
    if (anOcctFmt != Image_Format_UNKNOWN) {
        if (myPlane != 0 || !myPixMapWrapper->InitWrapper(anOcctFmt, aFrame->data[0], aFrame->width, aFrame->height,
                                                          aFrame->linesize[0])) {
            return Handle(Image_PixMap)();
        }
        ...
        if (myPlane > 3 || !myPixMapWrapper->InitWrapper(Image_Format_Gray, aFrame->data[myPlane], aSize.x(), aSize.y(),
