...
Graphic3d_CStructure::Graphic3d_CStructure(const Handle(Graphic3d_StructureManager) & theManager)
    : Priority(Structure_MAX_PRIORITY / 2), PreviousPriority(Structure_MAX_PRIORITY / 2), ContainsFacet(0),
      //
      myGraphicDriver(theManager->GraphicDriver()), myZLayer(Graphic3d_ZLayerId_Default), myIsCulled(Standard_True),
      myBndBoxClipCheck(Standard_True), myHasGroupTrsf(Standard_False),
      //
      IsInfinite(0), stick(0), highlight(0), visible(1), HLRValidation(0), IsForHighlight(Standard_False),
      IsMutable(Standard_False), Is2dText(Standard_False) {
