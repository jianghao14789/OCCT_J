...
//!
//! Graphic3d_TMF_2d, Graphic3d_TMF_TriedronPers and Graphic3d_TMF_ZoomPers defines Local Coordinate system where length
//! units are pixels. Beware that Graphic3d_RenderingParams::ResolutionRatio() will be ignored! For other Persistence
//! flags, normal (world) length units will apply.
//!
//! WARNING: Graphic3d_TMF_None is not permitted for defining instance of this class - NULL handle should be used for
//! this purpose!
...
public:
    ...
    Graphic3d_TransformPers(const Graphic3d_TransModeFlags theMode) : myMode(theMode) {
    ...
    //! Throws an exception if persistence mode is not Graphic3d_TMF_ZoomPers, Graphic3d_TMF_ZoomRotatePers or
    //! Graphic3d_TMF_RotatePers.
    Graphic3d_TransformPers(const Graphic3d_TransModeFlags theMode, const gp_Pnt& thePnt) : myMode(Graphic3d_TMF_None) {
        SetPersistence(theMode, thePnt);
    }

    //! Set 2d/trihedron transformation persistence with a corner and 2D offset.
    //! 2D offset defines the origin of Local Coordinate system as projection of 2D point on screen plane into World
    //! Coordinate system. Throws an exception if persistence mode is not Graphic3d_TMF_TriedronPers or
    //! Graphic3d_TMF_2d. The offset is a positive displacement from the view corner in pixels.
    Graphic3d_TransformPers(const Graphic3d_TransModeFlags theMode, const Aspect_TypeOfTriedronPosition theCorner,
    ...
    void SetPersistence(const Graphic3d_TransModeFlags theMode, const Aspect_TypeOfTriedronPosition theCorner,
...
public:
    ...
    virtual Standard_Real persistentScale(const Handle(Graphic3d_Camera) & theCamera,
        ...
        gp_Vec aVecToObj(theCamera->Eye(),
    ...
    template <class T>
    void Apply(const Handle(Graphic3d_Camera) & theCamera, const NCollection_Mat4<T>& theProjection,
               const NCollection_Mat4<T>& theWorldView, const Standard_Integer theViewportWidth,
               const Standard_Integer theViewportHeight, Bnd_Box& theBoundingBox) const;
    ...
    void Apply(const Handle(Graphic3d_Camera) & theCamera, const NCollection_Mat4<T>& theProjection,
               const NCollection_Mat4<T>& theWorldView, const Standard_Integer theViewportWidth,
               const Standard_Integer theViewportHeight, BVH_Box<T, 3>& theBoundingBox) const;
    ...
    template <class T>
    NCollection_Mat4<T> Compute(const Handle(Graphic3d_Camera) & theCamera, const NCollection_Mat4<T>& theProjection,
                                const NCollection_Mat4<T>& theWorldView, const Standard_Integer theViewportWidth,
                                const Standard_Integer theViewportHeight) const;
    ...
    template <class T>
    void Apply(const Handle(Graphic3d_Camera) & theCamera, const NCollection_Mat4<T>& theProjection,
               NCollection_Mat4<T>& theWorldView, const Standard_Integer theViewportWidth,
               const Standard_Integer theViewportHeight, const gp_Pnt* theAnchor = NULL) const;
...
void Graphic3d_TransformPers::Apply(const Handle(Graphic3d_Camera) & theCamera,
                                    const NCollection_Mat4<T>& theProjection, NCollection_Mat4<T>& theWorldView,
                                    const Standard_Integer theViewportWidth, const Standard_Integer theViewportHeight,
    ...
    if (myMode == Graphic3d_TMF_TriedronPers) {
        ...
        if ((myParams.Params2d.Corner & (Aspect_TOTP_LEFT | Aspect_TOTP_RIGHT)) != 0) {
            aCenter.SetX(-aViewDim.X() * theCamera->NDC2dOffsetX() +
        ...
        if ((myParams.Params2d.Corner & (Aspect_TOTP_TOP | Aspect_TOTP_BOTTOM)) != 0) {
            aCenter.SetY(-aViewDim.Y() * theCamera->NDC2dOffsetY() +
        ...
        if (theAnchor != NULL) {
            ...
            Graphic3d_TransformUtils::Translate(aWorldView, myParams.Params3d.PntX, myParams.Params3d.PntY,
...
template <class T>
void Graphic3d_TransformPers::Apply(const Handle(Graphic3d_Camera) & theCamera,
                                    const NCollection_Mat4<T>& theProjection, const NCollection_Mat4<T>& theWorldView,
                                    const Standard_Integer theViewportWidth, const Standard_Integer theViewportHeight,
    ...
    theBoundingBox.Update(aBBox.CornerMin().x(), aBBox.CornerMin().y(), aBBox.CornerMin().z(), aBBox.CornerMax().x(),
                          aBBox.CornerMax().y(), aBBox.CornerMax().z());
}
...
template <class T>
NCollection_Mat4<T>
Graphic3d_TransformPers::Compute(const Handle(Graphic3d_Camera) & theCamera, const NCollection_Mat4<T>& theProjection,
                                 const NCollection_Mat4<T>& theWorldView, const Standard_Integer theViewportWidth,
                                 const Standard_Integer theViewportHeight) const {
