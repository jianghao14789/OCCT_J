...
const char THE_VARY_TexCoord_Trsf[] =
    EOL "  float aRotSin = occTextureTrsf_RotationSin();" EOL "  float aRotCos = occTextureTrsf_RotationCos();" EOL
...
const char THE_FUNC_transformNormal_view[] =
    EOL "vec3 transformNormal (in vec3 theNormal)" EOL "{" EOL "  vec4 aResult = occWorldViewMatrixInverseTranspose" EOL
        "               * occModelWorldMatrixInverseTranspose" EOL "               * vec4 (theNormal, 0.0);" EOL
        "  return normalize (aResult.xyz);" EOL "}";
...
const char THE_FUNC_transformNormal_world[] = EOL
    "vec3 transformNormal (in vec3 theNormal)" EOL "{" EOL "  vec4 aResult = occModelWorldMatrixInverseTranspose" EOL
...
//! Global shader variable for color definition with lighting enabled.
const char THE_FUNC_PBR_lightDef[] = EOL "vec3  DirectLighting;" //!< Accumulator of direct lighting from light sources
...
const char THE_FUNC_directionalLightFirst[] =
    EOL "void directionalLightFirst (in vec3 theNormal," EOL "                            in vec3 theView," EOL
        "                            in bool theIsFront," EOL "                            in float theShadow)" EOL
        "{" EOL "  vec3 aLight = occLight_Position (0);" EOL EOL "  vec3 aHalf = normalize (aLight + theView);" EOL EOL
        "  vec3  aFaceSideNormal = theIsFront ? theNormal : -theNormal;" EOL
        "  float aNdotL = max (0.0, dot (aFaceSideNormal, aLight));" EOL
        "  float aNdotH = max (0.0, dot (aFaceSideNormal, aHalf ));" EOL EOL "  float aSpecl = 0.0;" EOL
        "  if (aNdotL > 0.0)" EOL "  {" EOL "    aSpecl = pow (aNdotH, occMaterial_Shininess(theIsFront));" EOL
        "  }" EOL EOL "  Diffuse  += occLight_Diffuse(0)  * aNdotL * theShadow;" EOL
...
const char THE_FUNC_cubemap_vector_transform[] =
    EOL "vec3 cubemapVectorTransform (in vec3 theVector," EOL "                             in int  theYCoeff," EOL
        "                             in int  theZCoeff)" EOL "{" EOL "  theVector = theVector.yzx;" EOL
        "  theVector.y *= float(theYCoeff);" EOL "  theVector.z *= float(theZCoeff);" EOL "  return theVector;" EOL "}";
...
const char THE_FRAG_CLIP_PLANES_N[] =
    EOL "  for (int aPlaneIter = 0; aPlaneIter < occClipPlaneCount; ++aPlaneIter)" EOL "  {" EOL
        ...
        "    if (dot (aClipEquation.xyz, PositionWorld.xyz / PositionWorld.w) + aClipEquation.w < 0.0)" EOL "    {" EOL
        "      discard;" EOL "    }" EOL "  }";
...
const char THE_FRAG_CLIP_CHAINS_N[] =
    EOL "  for (int aPlaneIter = 0; aPlaneIter < occClipPlaneCount;)" EOL "  {" EOL
        "    vec4 aClipEquation = occClipPlaneEquations[aPlaneIter];" EOL
        "    if (dot (aClipEquation.xyz, PositionWorld.xyz / PositionWorld.w) + aClipEquation.w < 0.0)" EOL "    {" EOL
        "      if (occClipPlaneChains[aPlaneIter] == 1)" EOL "      {" EOL "        discard;" EOL "      }" EOL
        "      aPlaneIter += 1;" EOL "    }" EOL "    else" EOL "    {" EOL
        "      aPlaneIter += occClipPlaneChains[aPlaneIter];" EOL "    }" EOL "  }";
...
const char THE_FRAG_CLIP_PLANES_1[] =
    EOL "  vec4 aClipEquation0 = occClipPlaneEquations[0];" EOL
        "  if (dot (aClipEquation0.xyz, PositionWorld.xyz / PositionWorld.w) + aClipEquation0.w < 0.0)" EOL "  {" EOL
        "    discard;" EOL "  }";

//! Process 2 clipping planes in Fragment Shader.
const char THE_FRAG_CLIP_PLANES_2[] =
    EOL "  vec4 aClipEquation0 = occClipPlaneEquations[0];" EOL "  vec4 aClipEquation1 = occClipPlaneEquations[1];" EOL
        "  if (dot (aClipEquation0.xyz, PositionWorld.xyz / PositionWorld.w) + aClipEquation0.w < 0.0" EOL
        "   || dot (aClipEquation1.xyz, PositionWorld.xyz / PositionWorld.w) + aClipEquation1.w < 0.0)" EOL "  {" EOL
        "    discard;" EOL "  }";
...
const char THE_FRAG_WIREFRAME_COLOR[] =
    EOL "vec4 getFinalColor(void)" EOL "{" EOL
        ...
        "  float aMixVal = smoothstep (occLineWidth - occLineFeather * 0.5, occLineWidth + occLineFeather * 0.5, "
        "aDistance);" EOL "  vec4 aMixColor = isHollow" EOL
...
//! This code adds silhouette only for smooth surfaces of closed primitive, and produces visual artifacts on sharp
//! edges.
const char THE_VERT_gl_Position_OUTLINE[] =
    EOL "  float anOutlineDisp = occOrthoScale > 0.0 ? occOrthoScale : gl_Position.w;" EOL
        "  vec4  anOutlinePos  = occVertex + vec4 (occNormal * (occSilhouetteThickness * anOutlineDisp), 0.0);" EOL
...
Graphic3d_ShaderManager::Graphic3d_ShaderManager(Aspect_GraphicsLibrary theGapi)
    : myGapi(theGapi),
      ...
      myHasFlatShading(true), myToReverseDFdxSign(false), mySetPointSize(myGapi == Aspect_GraphicsLibrary_OpenGLES),
      myUseRedAlpha(false), myToEmulateDepthClamp(true), mySRgbState(true) {
...
bool Graphic3d_ShaderManager::hasGlslBitwiseOps() const {
    switch (myGapi) {
        case Aspect_GraphicsLibrary_OpenGL: {
            return IsGapiGreaterEqual(3, 0) || myGlslExtensions[Graphic3d_GlslExtension_GL_EXT_gpu_shader4];
        }
...
int Graphic3d_ShaderManager::defaultGlslVersion(const Handle(Graphic3d_ShaderProgram) & theProgram,
                                                const TCollection_AsciiString& theName, int theBits,
                                                bool theUsesDerivates) const {
    int aBits = theBits;
    const bool toUseDerivates = theUsesDerivates || (theBits & Graphic3d_ShaderFlags_StippleLine) != 0 ||
...
void Graphic3d_ShaderManager::defaultOitGlslVersion(const Handle(Graphic3d_ShaderProgram) & theProgram,
                                                    const TCollection_AsciiString& theName, bool theMsaa) const {
    switch (myGapi) {
        case Aspect_GraphicsLibrary_OpenGL: {
            if (theMsaa) {
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramFont() const {
    ...
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec2 TexCoord", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    ...
    TCollection_AsciiString aSrcGetAlpha =
        myUseRedAlpha ? EOL "float getAlpha(void) { return occTexture2D(occSamplerBaseColor, TexCoord.st).r; }"
                      : EOL "float getAlpha(void) { return occTexture2D(occSamplerBaseColor, TexCoord.st).a; }";

    TCollection_AsciiString aSrcFrag =
        aSrcGetAlpha + EOL "void main()" EOL "{" EOL "  vec4 aColor = occColor;" EOL "  aColor.a *= getAlpha();" EOL
    ...
    aProgramSrc->AttachShader(
        Graphic3d_ShaderObject::CreateFromSource(aSrcVert, Graphic3d_TOS_VERTEX, aUniforms, aStageInOuts));
    aProgramSrc->AttachShader(
        Graphic3d_ShaderObject::CreateFromSource(aSrcFrag, Graphic3d_TOS_FRAGMENT, aUniforms, aStageInOuts));
    return aProgramSrc;
}
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramFboBlit(
    Standard_Integer theNbSamples, Standard_Boolean theIsFallback_sRGB) const {
    Graphic3d_ShaderObject::ShaderVariableList aUniforms, aStageInOuts;
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec2 TexCoord", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));

    TCollection_AsciiString aSrcVert = EOL "void main()" EOL "{" EOL "  TexCoord    = occVertex.zw;" EOL
    ...
    if (theNbSamples > 1) {
        if (myGapi == Aspect_GraphicsLibrary_OpenGLES) {
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("highp sampler2DMS uColorSampler", Graphic3d_TOS_FRAGMENT));
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("highp sampler2DMS uDepthSampler", Graphic3d_TOS_FRAGMENT));
        } else {
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2DMS uColorSampler", Graphic3d_TOS_FRAGMENT));
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2DMS uDepthSampler", Graphic3d_TOS_FRAGMENT));
        }

        aSrcFrag = TCollection_AsciiString() + EOL "#define THE_NUM_SAMPLES " + theNbSamples +
                   ...
                   EOL "void main()" EOL "{" EOL "  ivec2 aSize  = textureSize (uColorSampler);" EOL
                       ...
                       "    vec4 aVal = texelFetch (uColorSampler, anUV, aSample);" EOL "    aColor += aVal;" EOL
                       "  }" EOL "  aColor /= float(THE_NUM_SAMPLES);" EOL "#ifdef THE_SHIFT_sRGB" EOL
        ...
        aSrcFrag = TCollection_AsciiString() + (theIsFallback_sRGB ? EOL "#define THE_SHIFT_sRGB" : "") +
                   EOL "void main()" EOL "{" EOL "  gl_FragDepth = occTexture2D (uDepthSampler, TexCoord).r;" EOL
                       "  vec4  aColor = occTexture2D (uColorSampler, TexCoord);" EOL "#ifdef THE_SHIFT_sRGB" EOL
                       "  aColor.rgb = pow (aColor.rgb, vec3 (1.0 / 2.2));" EOL "#endif" EOL
                       "  occSetFragColor (aColor);" EOL "}";
    }
    ...
    switch (myGapi) {
        ...
        case Aspect_GraphicsLibrary_OpenGLES: {
            if (IsGapiGreaterEqual(3, 1)) {
                ...
                aSrcFrag =
                    EOL "void main()" EOL "{" EOL "  occSetFragColor (occTexture2D (uColorSampler, TexCoord));" EOL "}";
            }
            break;
        }
    }
...
Handle(Graphic3d_ShaderProgram)
    Graphic3d_ShaderManager::getStdProgramOitCompositing(const Standard_Boolean theMsaa) const {
    Handle(Graphic3d_ShaderProgram) aProgramSrc = new Graphic3d_ShaderProgram();
    ...
    if (!theMsaa) {
        ...
        aSrcFrag = EOL "void main()" EOL "{" EOL "  vec4 aAccum   = occTexture2D (uAccumTexture,  TexCoord);" EOL
        ...
        aSrcFrag =
            EOL "void main()" EOL "{" EOL "  ivec2 aTexel  = ivec2 (vec2 (textureSize (uAccumTexture)) * TexCoord);" EOL
...
// =======================================================================
Handle(Graphic3d_ShaderProgram)
    Graphic3d_ShaderManager::getStdProgramOitDepthPeelingBlend(Standard_Boolean theMsaa) const {
    Handle(Graphic3d_ShaderProgram) aProgramSrc = new Graphic3d_ShaderProgram();
    TCollection_AsciiString aSrcVert, aSrcFrag;

    Graphic3d_ShaderObject::ShaderVariableList aUniforms, aStageInOuts;
    aSrcVert = EOL "void main()" EOL "{" EOL "  gl_Position = vec4 (occVertex.x, occVertex.y, 0.0, 1.0);" EOL "}";

    aUniforms.Append(Graphic3d_ShaderObject::ShaderVariable(
        theMsaa ? "sampler2DMS uDepthPeelingBackColor" : "sampler2D uDepthPeelingBackColor", Graphic3d_TOS_FRAGMENT));
    aSrcFrag = TCollection_AsciiString() + EOL "void main()" EOL "{" EOL "  #define THE_SAMPLE_ID " +
               (theMsaa ? "gl_SampleID" : "0") +
               EOL "  occFragColor = texelFetch (uDepthPeelingBackColor, ivec2 (gl_FragCoord.xy), THE_SAMPLE_ID);" EOL
                   "  if (occFragColor.a == 0.0) { discard; }" EOL "}";
...
// =======================================================================
Handle(Graphic3d_ShaderProgram)
    Graphic3d_ShaderManager::getStdProgramOitDepthPeelingFlush(Standard_Boolean theMsaa) const {
    Handle(Graphic3d_ShaderProgram) aProgramSrc = new Graphic3d_ShaderProgram();
    TCollection_AsciiString aSrcVert, aSrcFrag;
    ...
    aUniforms.Append(Graphic3d_ShaderObject::ShaderVariable(
        theMsaa ? "sampler2DMS uDepthPeelingFrontColor" : "sampler2D uDepthPeelingFrontColor", Graphic3d_TOS_FRAGMENT));
    aUniforms.Append(Graphic3d_ShaderObject::ShaderVariable(
        theMsaa ? "sampler2DMS uDepthPeelingBackColor" : "sampler2D uDepthPeelingBackColor", Graphic3d_TOS_FRAGMENT));
    aSrcFrag =
        TCollection_AsciiString() + EOL "void main()" EOL "{" EOL "  #define THE_SAMPLE_ID " +
        (theMsaa ? "gl_SampleID" : "0") +
        EOL "  ivec2 aFragCoord  = ivec2 (gl_FragCoord.xy);" EOL
            ...
            "  occFragColor = vec4 (aFrontColor.rgb + anAlphaMult * aBackColor.rgb, aFrontColor.a + aBackColor.a);" EOL
            "}";
...
TCollection_AsciiString Graphic3d_ShaderManager::pointSpriteAlphaSrc(Standard_Integer theBits) const {
    ...
    return isAlpha && myUseRedAlpha
               ...
               : EOL "float getAlpha(void) { return occTexture2D(occSamplerPointSprite, " THE_VEC2_glPointCoord
                     ").a; }";
}
...
TCollection_AsciiString Graphic3d_ShaderManager::pointSpriteShadingSrc(const TCollection_AsciiString& theBaseColorSrc,
    ...
    if ((theBits & Graphic3d_ShaderFlags_PointSpriteA) == Graphic3d_ShaderFlags_PointSpriteA) {
        aSrcFragGetColor =
            pointSpriteAlphaSrc(theBits) + EOL "vec4 getColor(void)" EOL "{" EOL "  vec4 aColor = " + theBaseColorSrc +
            ";" EOL "  aColor.a = getAlpha();" EOL "  if (aColor.a <= 0.1) discard;" EOL "  return aColor;" EOL "}";
...
static TCollection_AsciiString prepareGeomMainSrc(Graphic3d_ShaderObject::ShaderVariableList& theUnifoms,
    ...
    if ((theBits & Graphic3d_ShaderFlags_MeshEdges) != 0) {
        ...
        theStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable("vec3 EdgeDistance",
                                                                     Graphic3d_TOS_GEOMETRY | Graphic3d_TOS_FRAGMENT));

        aSrcMainGeom =
            TCollection_AsciiString() +
            EOL "vec3 ViewPortTransform (vec4 theVec)" EOL "{" EOL "  vec3 aWinCoord = theVec.xyz / theVec.w;" EOL
                "  aWinCoord    = aWinCoord * 0.5 + 0.5;" EOL
                "  aWinCoord.xy = aWinCoord.xy * occViewport.zw + occViewport.xy;" EOL "  return aWinCoord;" EOL "}" +
            aSrcMainGeom +
            EOL
            "  vec3 aSideA = ViewPortTransform (gl_in[2].gl_Position) - ViewPortTransform (gl_in[1].gl_Position);" EOL
            "  vec3 aSideB = ViewPortTransform (gl_in[2].gl_Position) - ViewPortTransform (gl_in[0].gl_Position);" EOL
            "  vec3 aSideC = ViewPortTransform (gl_in[1].gl_Position) - ViewPortTransform (gl_in[0].gl_Position);" EOL
            ...
            "  vec3 aHeightABC = vec3 (aQuadArea) / aLenABC;" EOL
            "  aHeightABC = max (aHeightABC, vec3 (10.0 * occLineWidth));" // avoid shrunk presentation disappearing at
                                                                           // distance
    ...
    for (Standard_Integer aVertIter = 0; aVertIter < 3; ++aVertIter) {
        ...
        for (Graphic3d_ShaderObject::ShaderVariableList::Iterator aVarListIter(theStageInOuts); aVarListIter.More();
             aVarListIter.Next()) {
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramUnlit(Standard_Integer theBits,
    ...
    if ((theBits & Graphic3d_ShaderFlags_IsPoint) != 0) {
        ...
        if ((theBits & Graphic3d_ShaderFlags_PointSprite) != 0) {
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerPointSprite", Graphic3d_TOS_FRAGMENT));
            if ((theBits & Graphic3d_ShaderFlags_PointSpriteA) != Graphic3d_ShaderFlags_PointSpriteA) {
                ...
                aUniforms.Append(
                    Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerBaseColor", Graphic3d_TOS_VERTEX));
                aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable(
                    "vec4 VertColor", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
            ...
            aSrcFragMainGetColor = EOL "  vec4 aColor = getColor();" EOL "  aColor.a = getAlpha();" EOL
                                       "  if (aColor.a <= 0.1) discard;" EOL "  occSetFragColor (aColor);";
        } else {
            if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0 && (theBits & Graphic3d_ShaderFlags_VertColor) == 0) {
                aProgramSrc->SetTextureSetBits(Graphic3d_TextureSetBits_BaseColor);
                aUniforms.Append(
                    Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerBaseColor", Graphic3d_TOS_VERTEX));
                aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable(
            ...
            aSrcFragMainGetColor = EOL "  vec4 aColor = getColor();" EOL "  if (aColor.a <= 0.1) discard;" EOL
                                       "  occSetFragColor (aColor);";
        }
    } else {
        if ((theBits & Graphic3d_ShaderFlags_HasTextures) != 0) {
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerBaseColor", Graphic3d_TOS_FRAGMENT));
            aStageInOuts.Append(
                Graphic3d_ShaderObject::ShaderVariable("vec4 TexCoord", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
            ...
            if ((theBits & Graphic3d_ShaderFlags_HasTextures) == Graphic3d_ShaderFlags_TextureEnv) {
                ...
                aSrcVertExtraMain +=
                    EOL "  vec4 aPosition = occWorldViewMatrix * occModelWorldMatrix * occVertex;" EOL
                        ...
                        "  aReflect.z += 1.0;" EOL "  TexCoord = vec4(aReflect.xy * inversesqrt (dot (aReflect, "
                                                   "aReflect)) * 0.5 + vec2 (0.5), 0.0, 1.0);";
    ...
    if ((theBits & Graphic3d_ShaderFlags_VertColor) != 0) {
        aStageInOuts.Append(
            Graphic3d_ShaderObject::ShaderVariable("vec4 VertColor", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    ...
    if ((theBits & Graphic3d_ShaderFlags_ClipPlanesN) != 0) {
        aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable("vec4 PositionWorld",
        ...
        if ((theBits & Graphic3d_ShaderFlags_ClipPlanesN) == Graphic3d_ShaderFlags_ClipPlanesN) {
            ...
            aSrcFragExtraMain +=
                (theBits & Graphic3d_ShaderFlags_ClipChains) != 0 ? THE_FRAG_CLIP_CHAINS_N : THE_FRAG_CLIP_PLANES_N;
            ...
            aNbClipPlanes = 2;
            aSrcFragExtraMain +=
                (theBits & Graphic3d_ShaderFlags_ClipChains) != 0 ? THE_FRAG_CLIP_CHAINS_2 : THE_FRAG_CLIP_PLANES_2;
        }
    }
    ...
    if (theIsOutline) {
        ...
        if ((aBits & Graphic3d_ShaderFlags_StippleLine) != 0) {
            if (hasGlslBitwiseOps()) {
                aUniforms.Append(
                    Graphic3d_ShaderObject::ShaderVariable("int   occStipplePattern", Graphic3d_TOS_FRAGMENT));
            } else {
                aUniforms.Append(
                    Graphic3d_ShaderObject::ShaderVariable("bool  occStipplePattern[16]", Graphic3d_TOS_FRAGMENT));
            }
            ...
            aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable("vec2 ScreenSpaceCoord",
                                                                       Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
            ...
            aSrcFragMainGetColor =
                ...
                EOL "  vec2 anAxis = vec2 (0.0, 1.0);" EOL
                    "  if (abs (dFdx (ScreenSpaceCoord.x)) - abs (dFdy (ScreenSpaceCoord.y)) > 0.001)" EOL "  {" EOL
                    "    anAxis = vec2 (1.0, 0.0);" EOL "  }" EOL
                    "  float aRotatePoint = dot (gl_FragCoord.xy, anAxis);" +
                (hasGlslBitwiseOps()
                     ? EOL "  uint aBit = uint (floor (aRotatePoint / occStippleFactor + 0.5)) & 15U;" EOL
                           "  if ((uint (occStipplePattern) & (1U << aBit)) == 0U) discard;"
                     : EOL "  int aBit = int (mod (floor (aRotatePoint / occStippleFactor + 0.5), 16.0));" EOL
    ...
    aSrcVert = aSrcVertExtraFunc + EOL "void main()" EOL "{" + aSrcVertExtraMain + THE_VERT_gl_Position +
               aSrcVertEndMain + EOL "}";
    ...
    aSrcFrag = aSrcFragGetColor + aSrcGetAlpha +
               EOL "void main()" EOL "{" EOL "  if (occFragEarlyReturn()) { return; }" + aSrcFragExtraMain +
               aSrcFragMainGetColor + EOL "}";
    ...
    aProgramSrc->AttachShader(Graphic3d_ShaderObject::CreateFromSource(aSrcVert, Graphic3d_TOS_VERTEX, aUniforms,
                                                                       aStageInOuts, "", "", aNbGeomInputVerts));
    aProgramSrc->AttachShader(Graphic3d_ShaderObject::CreateFromSource(
        aSrcGeom, Graphic3d_TOS_GEOMETRY, aUniforms, aStageInOuts, "geomIn", "geomOut", aNbGeomInputVerts));
    aProgramSrc->AttachShader(Graphic3d_ShaderObject::CreateFromSource(aSrcFrag, Graphic3d_TOS_FRAGMENT, aUniforms,
                                                                       aStageInOuts, "", "", aNbGeomInputVerts));
    return aProgramSrc;
}
...
// =======================================================================
TCollection_AsciiString
Graphic3d_ShaderManager::stdComputeLighting(Standard_Integer& theNbLights, const Handle(Graphic3d_LightSet) & theLights,
                                            Standard_Boolean theHasVertColor, Standard_Boolean theIsPBR,
                                            Standard_Boolean theHasTexColor, Standard_Integer theNbShadowMaps) const {
    ...
    if (!theLights.IsNull()) {
        ...
        if (theNbLights <= THE_NB_UNROLLED_LIGHTS_MAX) {
            ...
            if (theNbShadowMaps > 0) {
                for (Graphic3d_LightSet::Iterator aLightIter(
                         theLights, Graphic3d_LightSet::IterationFilter_ExcludeDisabledAndAmbient);
                    ...
                    if (aLightIter.Value()->Type() == Graphic3d_TypeOfLightSource_Directional &&
                        ...
                        aLightsLoop = aLightsLoop + EOL "    occDirectionalLight (" + anIndex +
                                      ...
                                      "                         occDirectionalLightShadow (occShadowMapSamplers[" +
                                      anIndex + "], " + anIndex + ", theNormal));";
                        ++anIndex;
                    }
                }
            }
            for (Graphic3d_LightSet::Iterator aLightIter(theLights,
            ...
            aLightsLoop = aLightsLoop + EOL "    for (int anIndex = 0; anIndex < occLightSourcesCount; ++anIndex)" EOL
            ...
            if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Positional) > 0) {
                ...
                isFirstInLoop = false;
                aLightsLoop +=
                    EOL "      if (aType == OccLightType_Point)" EOL "      {" EOL
            ...
            if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Spot) > 0) {
                ...
                aLightsLoop +=
                    EOL "      if (aType == OccLightType_Spot)" EOL "      {" EOL
                        "        occSpotLight (anIndex, theNormal, theView, aPoint, theIsFront);" EOL "      }";
            }
            aLightsLoop += EOL "    }";
        }
        ...
        if (theLights->NbEnabledLightsOfType(Graphic3d_TypeOfLightSource_Directional) == 1 && theNbLights == 1 &&
            !theIsPBR && theNbShadowMaps == 0) {
    ...
    if (!theIsPBR) {
        return TCollection_AsciiString() + THE_FUNC_lightDef + Shaders_PointLightAttenuation_glsl + aLightsFunc +
               EOL EOL "vec4 computeLighting (in vec3 theNormal," EOL "                      in vec3 theView," EOL
                       "                      in vec4 thePoint," EOL "                      in bool theIsFront)" EOL
                       "{" EOL "  Ambient  = occLightAmbient.rgb;" EOL "  Diffuse  = vec3 (0.0);" EOL
               ...
               EOL "  vec3 aMatAmbient  = occMaterial_Ambient(theIsFront);" EOL
                   ...
                   "  vec4 aColor = vec4(Ambient * aMatAmbient + Diffuse * aMatDiffuse.rgb + Specular * aMatSpecular, "
                   "aMatDiffuse.a);" +
               (theHasVertColor ? EOL "  aColor *= getVertColor();" : "") +
               (theHasTexColor ? EOL "#if defined(THE_HAS_TEXTURE_COLOR) && defined(FRAGMENT_SHADER)" EOL
                                     "  aColor *= occTexture2D(occSamplerBaseColor, TexCoord.st / TexCoord.w);" EOL
                                     "#endif"
                               : "") +
               EOL "  occMaterialOcclusion(aColor.rgb, TexCoord.st / TexCoord.w);" EOL
        ...
        return TCollection_AsciiString() + THE_FUNC_PBR_lightDef + Shaders_PointLightAttenuation_glsl + aLightsFunc +
               EOL EOL "vec4 computeLighting (in vec3 theNormal," EOL "                      in vec3 theView," EOL
                       "                      in vec4 thePoint," EOL "                      in bool theIsFront)" EOL
                       "{" EOL "  DirectLighting = vec3(0.0);" EOL
                       "  BaseColor           = occMaterialBaseColor(theIsFront, TexCoord.st / TexCoord.w)" +
               (theHasVertColor ? " * getVertColor()" : "") + ";" +
               EOL "  Emission            = occMaterialEmission(theIsFront, TexCoord.st / TexCoord.w);" EOL
                   ...
                   "  IOR       = occPBRMaterial_IOR (theIsFront);" EOL "  vec3 aPoint = thePoint.xyz / thePoint.w;" +
               aLightsLoop +
               EOL "  vec3 aColor = DirectLighting;" EOL
                   ...
                   "  anIndirectLightingSpec *= occTextureCubeLod (occSpecIBLMap, -reflect (theView, theNormal), "
                   "NormalizedRoughness * float (occNbSpecIBLLevels - 1)).rgb;" EOL
                   "  vec3 aRefractionCoeff = 1.0 - occPBRFresnel (BaseColor.rgb, Metallic, NormalizedRoughness, IOR, "
                   "abs(dot(theView, theNormal)));" EOL "  aRefractionCoeff *= (1.0 - Metallic);" EOL
                   ...
                   "  aColor += occLightAmbient.rgb * (anIndirectLightingDiff + anIndirectLightingSpec);" EOL
                   "  aColor += Emission;" EOL "  occMaterialOcclusion(aColor, TexCoord.st / TexCoord.w);" EOL
...
// purpose  :
// =======================================================================
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramGouraud(const Handle(Graphic3d_LightSet) &
                                                                                  theLights,
                                                                              Standard_Integer theBits) const {
    Handle(Graphic3d_ShaderProgram) aProgramSrc = new Graphic3d_ShaderProgram();
    ...
    if ((theBits & Graphic3d_ShaderFlags_IsPoint) != 0) {
        ...
        if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0 && (theBits & Graphic3d_ShaderFlags_VertColor) == 0) {
            ...
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerBaseColor", Graphic3d_TOS_VERTEX));
            aSrcVertColor =
                EOL "vec4 getVertColor(void) { return occTexture2D (occSamplerBaseColor, occTexCoord.xy); }";
        }
    } else {
        if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0) {
            ...
            aSrcFragGetColor =
                EOL "vec4 getColor(void)" EOL "{" EOL "  vec4 aColor = gl_FrontFacing ? FrontColor : BackColor;" EOL
    ...
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec4 FrontColor", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec4 BackColor", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    ...
    aSrcVert = TCollection_AsciiString() + THE_FUNC_transformNormal_world + EOL + aSrcVertColor + aLights +
               EOL "void main()" EOL "{" EOL "  vec4 aPositionWorld = occModelWorldMatrix * occVertex;" EOL
                   ...
                   "    aView = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 1.0, 0.0)).xyz;" EOL "  }" EOL "  else" EOL
                   "  {" EOL "    vec3 anEye = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 0.0, 1.0)).xyz;" EOL
    ...
    aSrcFrag = TCollection_AsciiString() + aSrcFragGetColor +
               EOL "void main()" EOL "{" EOL "  if (occFragEarlyReturn()) { return; }" + aSrcFragExtraMain +
               EOL "  occSetFragColor (getFinalColor());" + EOL "}";
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramPhong(
    const Handle(Graphic3d_LightSet) & theLights, const Standard_Integer theBits,
    const Standard_Boolean theIsFlatNormal, const Standard_Boolean theIsPBR,
    ...
    if (isFlatNormal != theIsFlatNormal) {
        ...
        Message::SendWarning(
            "Warning: applied workaround for GLSL flat shading normal computation using dFdx/dFdy on Adreno");
    }
    ...
    if ((theBits & Graphic3d_ShaderFlags_IsPoint) != 0) {
        ...
        if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0 && (theBits & Graphic3d_ShaderFlags_VertColor) == 0) {
            ...
            aUniforms.Append(
                Graphic3d_ShaderObject::ShaderVariable("sampler2D occSamplerBaseColor", Graphic3d_TOS_VERTEX));
            aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable("vec4 VertColor",
                                                                       Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
        ...
        if ((theBits & Graphic3d_ShaderFlags_TextureRGB) != 0) {
            ...
            Standard_Integer aTextureBits = Graphic3d_TextureSetBits_BaseColor | Graphic3d_TextureSetBits_Occlusion |
                                            Graphic3d_TextureSetBits_Emissive;
            if (theIsPBR) {
                aTextureBits |= Graphic3d_TextureSetBits_MetallicRoughness;
            }
            if ((theBits & Graphic3d_ShaderFlags_HasTextures) == Graphic3d_ShaderFlags_TextureNormal && !isFlatNormal) {
                if (myHasFlatShading) {
    ...
    if (isFlatNormal) {
        aSrcFragExtraMain += TCollection_AsciiString() + EOL "  Normal = " + aDFdxSignReversion +
                             "normalize (cross (dFdx (PositionWorld.xyz / PositionWorld.w), dFdy (PositionWorld.xyz / "
                             "PositionWorld.w)));" EOL "  if (!gl_FrontFacing) { Normal = -Normal; }";
    } else {
        aStageInOuts.Append(
            Graphic3d_ShaderObject::ShaderVariable("vec3 vNormal", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
        ...
        if ((theBits & Graphic3d_ShaderFlags_IsPoint) == 0 &&
            (theBits & Graphic3d_ShaderFlags_HasTextures) == Graphic3d_ShaderFlags_TextureNormal && myHasFlatShading) {
            aSrcFrag += Shaders_TangentSpaceNormal_glsl;
            // apply normal map texture
            aSrcFragExtraMain +=
                EOL "#if defined(THE_HAS_TEXTURE_NORMAL)" EOL "  vec2 aTexCoord = TexCoord.st / TexCoord.w;" EOL
                    "  vec4 aMapNormalValue = occTextureNormal(aTexCoord);" EOL "  if (aMapNormalValue.w > 0.5)" EOL
                    "  {" EOL "    mat2 aDeltaUVMatrix = mat2 (dFdx(aTexCoord), dFdy(aTexCoord));" EOL
                    "    mat2x3 aDeltaVectorMatrix = mat2x3 (dFdx (PositionWorld.xyz), dFdy (PositionWorld.xyz));" EOL
                    "    Normal = TangentSpaceNormal (aDeltaUVMatrix, aDeltaVectorMatrix, aMapNormalValue.xyz, Normal, "
                    "!gl_FrontFacing);" EOL "  }" EOL "#endif";
        }
    }

    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec4 PositionWorld", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec3 View", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    if (theNbShadowMaps > 0) {
        aUniforms.Append(Graphic3d_ShaderObject::ShaderVariable("mat4      occShadowMapMatrices[THE_NB_SHADOWMAPS]",
                                                                Graphic3d_TOS_VERTEX));
        aUniforms.Append(Graphic3d_ShaderObject::ShaderVariable("sampler2D occShadowMapSamplers[THE_NB_SHADOWMAPS]",
                                                                Graphic3d_TOS_FRAGMENT));
        aUniforms.Append(
            Graphic3d_ShaderObject::ShaderVariable("vec2      occShadowMapSizeBias", Graphic3d_TOS_FRAGMENT));

        aStageInOuts.Append(Graphic3d_ShaderObject::ShaderVariable("vec4 PosLightSpace[THE_NB_SHADOWMAPS]",
                                                                   Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
        aSrcVertExtraMain +=
            EOL "  for (int aShadowIter = 0; aShadowIter < THE_NB_SHADOWMAPS; ++aShadowIter)" EOL "  {" EOL
    ...
    aSrcVert = TCollection_AsciiString() + aSrcVertExtraFunc +
               EOL "void main()" EOL "{" EOL "  PositionWorld = occModelWorldMatrix * occVertex;" EOL
                   "  if (occProjectionMatrix[3][3] == 1.0)" EOL "  {" EOL
                   "    View = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 1.0, 0.0)).xyz;" EOL "  }" EOL "  else" EOL
                   "  {" EOL "    vec3 anEye = (occWorldViewMatrixInverse * vec4(0.0, 0.0, 0.0, 1.0)).xyz;" EOL
    ...
    aSrcFrag += TCollection_AsciiString() + EOL + aSrcFragGetVertColor + EOL "vec3  Normal;" + aLights +
                aSrcFragGetColor + EOL EOL "void main()" EOL "{" EOL "  if (occFragEarlyReturn()) { return; }" +
                aSrcFragExtraMain + EOL "  occSetFragColor (getFinalColor());" + EOL "}";

    const TCollection_AsciiString aProgId = TCollection_AsciiString(theIsFlatNormal ? "flat-" : "phong-") +
                                            (theIsPBR ? "pbr-" : "") + genLightKey(theLights, theNbShadowMaps > 0) +
                                            "-";
    ...
    switch (theStereoMode) {
        case Graphic3d_StereoMode_Anaglyph: {
            ...
            const TCollection_AsciiString aNormalize =
                mySRgbState ? EOL "#define sRgb2linear(theColor) theColor" EOL "#define linear2sRgb(theColor) theColor"
                            : EOL "#define sRgb2linear(theColor) pow(theColor, vec4(2.2, 2.2, 2.2, 1.0))" EOL
                                  "#define linear2sRgb(theColor) pow(theColor, 1.0 / vec4(2.2, 2.2, 2.2, 1.0))";
            aSrcFrag = aNormalize + EOL
                       "void main()" EOL "{" EOL "  vec4 aColorL = occTexture2D (uLeftSampler,  TexCoord);" EOL
                       "  vec4 aColorR = occTexture2D (uRightSampler, TexCoord);" EOL
                       "  aColorL = sRgb2linear (aColorL);" EOL "  aColorR = sRgb2linear (aColorR);" EOL
        ...
        case Graphic3d_StereoMode_RowInterlaced: {
            ...
            aSrcFrag = EOL "void main()" EOL "{" EOL "  vec4 aColorL = occTexture2D (uLeftSampler,  TexCoord);" EOL
                           "  vec4 aColorR = occTexture2D (uRightSampler, TexCoord);" EOL
        ...
        case Graphic3d_StereoMode_SideBySide: {
            aName = "sidebyside";
            aSrcFrag = EOL "void main()" EOL "{" EOL "  vec2 aTexCoord = vec2 (TexCoord.x * 2.0, TexCoord.y);" EOL
        ...
        case Graphic3d_StereoMode_OverUnder: {
            aName = "overunder";
            aSrcFrag = EOL "void main()" EOL "{" EOL "  vec2 aTexCoord = vec2 (TexCoord.x, TexCoord.y * 2.0);" EOL
        ...
        default: {
            aSrcFrag =
                EOL "void main()" EOL "{" EOL "  vec4 aColorL = occTexture2D (uLeftSampler,  TexCoord);" EOL
                    ...
                    "  aColorL.g = 0.0;" EOL "  aColorR.r = 0.0;" EOL "  occSetFragColor (aColorL + aColorR);" EOL "}";
            break;
        }
    }
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getStdProgramBoundBox() const {
    ...
    TCollection_AsciiString aSrcVert =
        EOL "void main()" EOL "{" EOL "  vec4 aCenter = vec4(occVertex.xyz * occBBoxSize + occBBoxCenter, 1.0);" EOL
    ...
    TCollection_AsciiString aSrcFrag = EOL "void main()" EOL "{" EOL "  occSetFragColor (occColor);" EOL "}";
    ...
    switch (myGapi) {
        ...
        case Aspect_GraphicsLibrary_OpenGLES: {
            if (IsGapiGreaterEqual(3, 0)) {
                ...
                Message::SendWarning("Warning: incomplete PBR lighting implementation due to missing OpenGL ES 3.0 or "
                                     "GL_EXT_shader_texture_lod support.");
            }
            break;
        }
    }

    static const char* THE_BAKE_NAMES[3] = {"pbr_env_baking_diffuse", "pbr_env_baking_specular",
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getBgCubeMapProgram() const {
    ...
    Graphic3d_ShaderObject::ShaderVariableList aUniforms, aStageInOuts;
    aStageInOuts.Append(
        Graphic3d_ShaderObject::ShaderVariable("vec3 ViewDirection", Graphic3d_TOS_VERTEX | Graphic3d_TOS_FRAGMENT));
    ...
    TCollection_AsciiString aSrcVert =
        ...
        EOL "void main()" EOL "{" EOL "  ViewDirection = cubemapVectorTransform (occVertex.xyz, uYCoeff, uZCoeff);" EOL
    ...
    if (myToEmulateDepthClamp) {
        ...
        if (myGapi == Aspect_GraphicsLibrary_OpenGLES) {
            if (IsGapiGreaterEqual(3, 0)) {
                ...
                aProgSrc->SetHeader("#extension GL_EXT_frag_depth : enable" EOL "#define gl_FragDepth gl_FragDepthEXT");
            } else {
                aDepthClamp.Clear();
            }
        }
...
Handle(Graphic3d_ShaderProgram) Graphic3d_ShaderManager::getColoredQuadProgram() const {
    ...
    TCollection_AsciiString aSrcFrag = TCollection_AsciiString() + EOL
                                       "void main()" EOL "{" EOL "  vec3 c1 = mix (uColor1, uColor2, TexCoord.x);" EOL
