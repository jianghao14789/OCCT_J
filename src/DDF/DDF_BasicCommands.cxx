...
static Standard_Integer DDF_SetEmptyAttribute(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n != 4) return 1;
...
static Standard_Integer DDF_ForgetAttribute(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n != 4) return 1;
    Handle(TDF_Data) DF;
    if (!DDF::GetDF(a[1], DF)) return 1;
...
void DDF::BasicCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("Children", " Returns the list of label children: Children DF label", __FILE__, DDF_Children, g);

    theCommands.Add("Attributes", " Returns the list of label attributes: Attributes DF label", __FILE__,
                    DDF_Attributes, g);
    ...
    theCommands.Add("ForgetAll", "Forgets all attributes from the label: ForgetAll DF Label", __FILE__, DDF_ForgetAll,
                    g);
