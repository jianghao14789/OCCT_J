...
static Standard_Integer CopyLabel_SCopy(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n == 4) {
        ...
        cop.Perform();
        if (!cop.IsDone()) di << "copy not done\n";
        return 0;
    }
...
static Standard_Integer DDF_CheckAttrs(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (n == 4) {
                    ...
                    for (TDF_MapIteratorOfAttributeMap attMItr2(attMap2); attMItr2.More(); attMItr2.Next()) {
                        ...
                        if (tAtt->IsInstance(sAtt->DynamicType()))
                            if (tAtt == sAtt) {
                                ...
                                // std::cout<<"\tAttribute dynamic type = "<<sAtt->DynamicType()<<",\tlocated on Label =
                                // "<<entr1<<std::endl;
        ...
        if (!Shar) di << "Shareable attributes not found\n";
        return 0;
    }
...
void DDF::DataCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("CopyDF", "Copies a label: CopyDF dfname1 entry1 [dfname2] entry2", __FILE__, CopyDF, g);

    theCommands.Add("XDumpDF", "Exented deep dump of a DF (with attributes content): DumpDF dfname", __FILE__, XDumpDF,
                    g);

    theCommands.Add("MiniDumpDF", "Mini dump of a DF (with attributes content): DumpDF dfname", __FILE__, MiniDumpDF,
                    g);
    ...
    theCommands.Add("SetAccessByEntry", "SetAccessByEntry DOC 1|0", __FILE__, DDF_SetAccessByEntry, g);
}
