...
static Standard_Boolean findPluginFile(TCollection_AsciiString& thePluginName, TCollection_AsciiString& thePluginDir) {
    ...
    if (aToSetCSFVariable) {
        ...
        aCSFVarEnv.Build();
#ifdef OCCT_DEBUG
        std::cout << "Variable " << aCSFVariable << " has not been explicitly defined. Set to " << thePluginDir
                  << std::endl;
#endif
        if (aCSFVarEnv.Failed()) {
            Message::SendFail() << "Failed to load plugin: Failed to initialize " << aCSFVariable << " with "
                                << thePluginDir;
            return Standard_False;
        }
    }
...
static Standard_Integer Pload(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
...
void Draw::PloadCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("pload", "pload [-PluginFilename] [[Key1] [Key2] ...]: Loads Draw plugins ", __FILE__, Pload, g);
    theCommands.Add("dtryload", "dtryload path : load and unload specified dynamic loaded library", __FILE__, dtryload,
                    g);
}
