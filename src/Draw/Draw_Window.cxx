...
static HPEN Draw_colorPenTab[MAXCOLOR] = {
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 255, 255)), CreatePen(PS_SOLID, PENWIDTH, RGB(255, 0, 0)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(0, 255, 0)),     CreatePen(PS_SOLID, PENWIDTH, RGB(0, 0, 255)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(0, 255, 255)),   CreatePen(PS_SOLID, PENWIDTH, RGB(255, 215, 0)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 0, 255)),   CreatePen(PS_SOLID, PENWIDTH, RGB(255, 52, 179)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 165, 0)),   CreatePen(PS_SOLID, PENWIDTH, RGB(255, 228, 225)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 160, 122)), CreatePen(PS_SOLID, PENWIDTH, RGB(199, 21, 133)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 255, 0)),   CreatePen(PS_SOLID, PENWIDTH, RGB(240, 230, 140)),
    CreatePen(PS_SOLID, PENWIDTH, RGB(255, 127, 80))};

// Correspondence mode X11 and WINDOWS NT
static const int Draw_modeTab[16] = {R2_BLACK,      R2_MASKPEN,     R2_MASKPENNOT,  R2_COPYPEN,
...
void Draw_Window::RemoveCallbackBeforeTerminate(FCallbackBeforeTerminate theCB) {
    for (NCollection_List<Draw_Window::FCallbackBeforeTerminate>::Iterator anIter(TermCallbacks()); anIter.More();
         anIter.Next()) {
...
// purpose  :
//=======================================================================
Draw_Window::Draw_Window(const char* theTitle, const NCollection_Vec2<int>& theXY, const NCollection_Vec2<int>& theSize,
                         Aspect_Drawable theParent, Aspect_Drawable theWin)
    : myWindow(0),
#if defined(_WIN32)
      myMemHbm(NULL), myCurrPen(0), myCurrMode(0),
#elif defined(HAVE_XLIB)
      myMother((Window)theParent), myImageBuffer(0), myBase(new Base_Window()),
#endif
      myCurrentColor(0), myUseBuffer(Standard_False) {
...
void Draw_Window::init(const NCollection_Vec2<int>& theXY, const NCollection_Vec2<int>& theSize) {
    ...
    if (myWindow == 0) {
        myWindow =
            XCreateWindow(Draw_WindowDisplay, myMother, theXY.x(), theXY.y(), (unsigned int)theSize.x(),
                          (unsigned int)theSize.y(), 5, DefaultDepth(Draw_WindowDisplay, Draw_WindowScreen),
                          InputOutput, DefaultVisual(Draw_WindowDisplay, Draw_WindowScreen), aSetMask, &myBase->xswa);
...
void Draw_Window::InitBuffer() {
    ...
    if (myUseBuffer) {
        ...
        myMemHbm = (HBITMAP)CreateCompatibleBitmap(hDC, aRect.right - aRect.left, aRect.bottom - aRect.top);
...
void Draw_Window::Redraw() {
    ...
    if (myUseBuffer) {
        ...
        GetClientRect(myWindow, &aRect);
        HDC aMemDC = getMemDC(hDC);
        BitBlt(hDC, aRect.left, aRect.top, aRect.right - aRect.left, aRect.bottom - aRect.top, aMemDC, 0, 0, SRCCOPY);
        releaseMemDC(aMemDC);
        ReleaseDC(myWindow, hDC);
    }
#elif defined(HAVE_XLIB)
    if (myUseBuffer) {
        XCopyArea(Draw_WindowDisplay, myImageBuffer, myWindow, myBase->gc, 0, 0, WidthWin(), HeightWin(), 0, 0);
    }
#endif
}
...
static Standard_Boolean SaveBitmap(HBITMAP theHBitmap, const char* theFileName) {
    ...
    if (!anImage.InitTrash(Image_Format_BGR, Standard_Size(aBitmap.bmWidth), Standard_Size(aBitmap.bmHeight),
                           aSizeRowBytes)) {
        return Standard_False;
    }
...
Standard_Boolean Draw_Window::Save(const char* theFileName) const {
    ...
    if (!myUseBuffer) {
        ...
        XTranslateCoordinates(Draw_WindowDisplay, myWindow, XRootWindowOfScreen(aWinAttr.screen), 0, 0, &aWinLeft,
                              &aWinTop, &aWinChildDummy);
    ...
    if (!anImage.InitTrash(isBigEndian ? Image_Format_RGB32 : Image_Format_BGR32, Standard_Size(aWinAttr.width),
                           Standard_Size(aWinAttr.height), aSizeRowBytes)) {
        return Standard_False;
    }
    anImage.SetTopDown(true);

    XImage* anXImage = XCreateImage(Draw_WindowDisplay, aVInfo.visual, 32, ZPixmap, 0, (char*)anImage.ChangeData(),
                                    aWinAttr.width, aWinAttr.height, 32, int(aSizeRowBytes));
    anXImage->bitmap_bit_order = anXImage->byte_order = (isBigEndian ? MSBFirst : LSBFirst);
    if (XGetSubImage(Draw_WindowDisplay, GetDrawable(), 0, 0, aWinAttr.width, aWinAttr.height, AllPlanes, ZPixmap,
                     anXImage, 0, 0) == NULL) {
        anXImage->data = NULL;
...
static void processXEvents(ClientData, int) {
    ...
    while (XPending(Draw_WindowDisplay)) {
        ...
        for (NCollection_List<Draw_Window*>::Iterator aWinIter(getDrawWindowList()); aWinIter.More(); aWinIter.Next()) {
...
HWND Draw_Window::createDrawWindow(HWND hWndClient, int nitem) {
    if (Draw_IsConsoleSubsystem) {
        HWND aWin = CreateWindowW(DRAWCLASS, DRAWTITLE, WS_OVERLAPPEDWINDOW, 1, 1, 1, 1, NULL, NULL,
        ...
        return CreateMDIWindowW(DRAWCLASS, DRAWTITLE, WS_CAPTION | WS_CHILD | WS_THICKFRAME, 1, 1, 0, 0, hWndClient,
                                (HINSTANCE)hInstance, nitem);
    }
}
...
void Draw_Window::SelectNoWait(HANDLE& theWindow, int& theX, int& theY, int& theButton) {
    ...
    aMsg.wParam = 1;
    GetMessageW(&aMsg, NULL, 0, 0);
    while ((aMsg.message != WM_RBUTTONDOWN && aMsg.message != WM_LBUTTONDOWN && aMsg.message != WM_MOUSEMOVE) ||
           !(Draw_IsConsoleSubsystem || IsChild(Draw_Window::hWndClientMDI, aMsg.hwnd))) {
        GetMessageW(&aMsg, NULL, 0, 0);
    }
...
static DWORD WINAPI readStdinThreadFunc(const LPVOID theThreadParameter) {
    ...
    // However, to use it, we have to care using std::wcerr/fwprintf/WriteConsoleW for non-ascii strings everywhere
    // (including Tcl itself), or otherwise we can have incomplete output issues (e.g. UNICODE string will be NOT just
    // corrupted as in case when we don't set setlocale() but will break further normal output to console due to special
    // characters being accidentally handled by console in the wrong way).
...
static DWORD WINAPI tkLoop(const LPVOID theThreadParameter) {
    ...
    if (!Draw_VirtualWindows) {
        try {
            ...
            if (res != TCL_OK) {
#if ((TCL_MAJOR_VERSION > 8) || ((TCL_MAJOR_VERSION == 8) && (TCL_MINOR_VERSION >= 5)))
                std::cout << "tkLoop: error in Tk initialization. Tcl reported: " << Tcl_GetStringResult(interp)
                          << std::endl;
#else
    ...
    while (toLoop) {
        ...
        // Unfortunately, Tcl has no API returning the number of queued events like XPending(), and only empty state can
        // be checked. Since events can be continuously fed from parallel threads, Tcl_DoOneEvent might never return
        // empty state at all.
...
void Run_Appli(HWND hWnd) {
    ...
    if (Draw_IsConsoleSubsystem) {
        ...
        if (!hThread) {
            ...
            Init_Appli(GetModuleHandleW(NULL), GetModuleHandleW(NULL), 1, hWnd); // reinit => create MDI client wnd
        }
    }
