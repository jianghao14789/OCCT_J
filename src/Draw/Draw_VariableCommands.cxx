...
// save
//=======================================================================
static Standard_Integer save(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    if (theNbArgs != 3) {
...
// read
//=======================================================================
static Standard_Integer restore(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
    if (theNbArgs != 3) {
        return 1;
    }
    ...
    if (n <= 1 || donly) {
        ...
        if (donly) {
            for (i = 1; i < n; i++) {
                Handle(Draw_Drawable3D) D = Draw::Get(a[i]);
                if (D.IsNull()) {
                    if ((a[i][0] == '.') && (a[i][1] == '\0')) std::cout << "Missed !!!" << std::endl;
                    return 0;
                }
            }
        }
        ...
        for (NCollection_Map<Handle(Draw_Drawable3D)>::Iterator aMapIt(Draw::Drawables()); aMapIt.More();
             aMapIt.Next()) {
...
// purpose  :
//=======================================================================
void Draw::Set(const Standard_CString name, const Handle(Draw_Drawable3D) & D, const Standard_Boolean displ) {
    Draw_Interpretor& aCommands = Draw::GetInterpretor();
    ...
    if ((name[0] == '.') && (name[1] == '\0')) {
        ...
        ClientData aCD =
            Tcl_VarTraceInfo(aCommands.Interp(), name, TCL_TRACE_UNSETS | TCL_TRACE_WRITES, tracevar, NULL);
        ...
        if (!D.IsNull()) {
            ...
            Tcl_TraceVar(aCommands.Interp(), name, TCL_TRACE_UNSETS | TCL_TRACE_WRITES, tracevar,
...
// purpose  :
//=======================================================================
Handle(Draw_Drawable3D) Draw::getDrawable(Standard_CString& theName, Standard_Boolean theToAllowPick) {
    const Standard_Boolean toPick = ((theName[0] == '.') && (theName[1] == '\0'));
    if (!toPick) {
        ClientData aCD = Tcl_VarTraceInfo(Draw::GetInterpretor().Interp(), theName, TCL_TRACE_UNSETS | TCL_TRACE_WRITES,
                                          tracevar, NULL);
...
// purpose  :
//=======================================================================
void Draw::LastPick(Standard_Integer& view, Standard_Integer& X, Standard_Integer& Y, Standard_Integer& button) {
    view = p_id;
    X = p_X;
    Y = p_Y;
    button = p_b;
...
void Draw::VariableCommands(Draw_Interpretor& theCommandsArg) {
    ...
    theCommandsArg.Add("dsetenv", "var [value] : set (unset if value is empty) environment variable in C subsystem",
                       __FILE__, dsetenv, g);
