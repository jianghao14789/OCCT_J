...
struct Draw_XSegment {
    ...
    NCollection_Vec2<short>& operator[](int theIndex) {
        return Points[theIndex];
    }
    const NCollection_Vec2<short>& operator[](int theIndex) const {
        return Points[theIndex];
    }

    void Init(Standard_Integer theXStart, Standard_Integer theYStart, Standard_Integer theXEnd,
...
enum console_semaphore_value { STOP_CONSOLE, WAIT_CONSOLE_COMMAND, HAS_CONSOLE_COMMAND };
...
public:
    ...
    //! @sa SetColor()
    Standard_EXPORT static Standard_Boolean DefineColor(const Standard_Integer theIndex, const char* theColorName);
...
public:
    ...
    //! Draw the string.
    Standard_EXPORT void DrawString(Standard_Integer theX, Standard_Integer theY, const char* theText);

    //! Draw array of segments.
    Standard_EXPORT void DrawSegments(const Draw_XSegment* theSegments, Standard_Integer theNumberOfElements);
...
protected:
    ...
    Standard_EXPORT Draw_Window(const char* theTitle, const NCollection_Vec2<int>& theXY,
                                const NCollection_Vec2<int>& theSize, Aspect_Drawable theParent,
                                Aspect_Drawable theWin);

    //! Initialize the window.
    Standard_EXPORT void init(const NCollection_Vec2<int>& theXY, const NCollection_Vec2<int>& theSize);

public:
#if defined(_WIN32)
    Standard_Boolean IsEqualWindows(HANDLE theWindow) {
        return myWindow == theWindow;
    }
    ...
    Standard_Boolean IsEqualWindows(Window theWindow) {
        return myWindow == theWindow;
    }
    ...
    static void GetNextEvent(Standard_Boolean theWait, Standard_Integer& theWindowNumber, Standard_Integer& theX,
                             Standard_Integer& theY, Standard_Integer& theButton);
#endif
...
private:
    ...
    Drawable GetDrawable() const {
        return myUseBuffer ? myImageBuffer : myWindow;
    }
