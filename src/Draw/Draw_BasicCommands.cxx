...
static Standard_Integer chronom(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if ((n == 1) || (*a[1] == '0') || (*a[1] == '1')) {
        if (n == 1)
        ...
        if (Draw_Chrono)
            di << "Chronometers activated.\n";
        else
            di << "Chronometers deactivated.\n";
    } else {
...
static Standard_Integer dchronom(Draw_Interpretor& theDI, Standard_Integer n, const char** a) {
    if ((n == 1) || (*a[1] == '0') || (*a[1] == '1')) {
        if (n == 1)
            Draw_Chrono = !Draw_Chrono;
        else
        ...
        if (Draw_Chrono)
            theDI << "Chronometers activated.\n";
        else
            theDI << "Chronometers deactivated.\n";
    } else {
        Handle(Draw_Drawable3D) D = Draw::Get(a[1]);
        Handle(Draw_Chronometer) C;
        if (!D.IsNull()) {
...
static Standard_Integer spy(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (Draw_Spying) Draw_Spyfile.close();
...
static Standard_Integer dversion(Draw_Interpretor& di, Standard_Integer, const char**) {
    ...
    di << "Compiler: MS Visual C++ " << (int)(_MSC_VER / 100 - 6) << "."
       << (int)((_MSC_VER / 10) - 60 - 10 * (int)(_MSC_VER / 100 - 6)) << " (_MSC_FULL_VER = " << _MSC_FULL_VER
       << ")\n";
#else
    di << "Compiler: MS Visual C++ " << (int)(_MSC_VER / 100 - 5) << "."
       << (int)((_MSC_VER / 10) - 50 - 10 * (int)(_MSC_VER / 100 - 5)) << " (_MSC_FULL_VER = " << _MSC_FULL_VER
       << ")\n";
#endif
...
static Standard_Integer cpulimit(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (status != 0) di << "status cpulimit setrlimit : " << status << "\n";
...
static int dparallel(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anIter = 1; anIter < theArgNb; ++anIter) {
        ...
        if (anIter + 1 < theArgNb && (anArg == "-nbthreads" || anArg == "-threads")) {
            ...
            aDefPool->Init(aVal);
        } else if (anIter + 1 < theArgNb && (anArg == "-nbdefthreads" || anArg == "-defthreads" ||
                                             anArg == "-nbmaxdefthreads" || anArg == "-maxdefthreads")) {
            const Standard_Integer aVal = Draw::Atoi(theArgVec[++anIter]);
        ...
        } else if (anIter + 1 < theArgNb && (anArg == "-useocct" || anArg == "-touseocct" || anArg == "-occt")) {
            const Standard_Integer aVal = Draw::Atoi(theArgVec[++anIter]);
        ...
        } else if (anIter + 1 < theArgNb && (anArg == "-usetbb" || anArg == "-tousetbb" || anArg == "-tbb")) {
            const Standard_Integer aVal = Draw::Atoi(theArgVec[++anIter]);
...
static int dsetsignal(Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 1; anArgIter < theArgNb; ++anArgIter) {
        ...
        } else if (anArgIter + 1 < theArgNb && (anArg == "-stracktracelength" || anArg == "-stracktracelen" ||
...
void Draw::BasicCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("chrono",
                    "chrono [name action [action...]] \n  Operates named timer.\n"
                    ...
                    __FILE__, chronom, g);
    theCommands.Add("dchrono", "see help of chrono command", __FILE__, dchronom, g);
    theCommands.Add("mallochook", "debug memory allocation/deallocation, w/o args for help", __FILE__, mallochook, g);
    theCommands.Add("meminfo",
                    "meminfo [virt|v] [heap|h] [wset|w] [wsetpeak] [swap] [swappeak] [private]"
                    " : memory counters for this process",
                    __FILE__, dmeminfo, g);
    theCommands.Add("dperf", "dperf [reset] -- show performance counters, reset if argument is provided", __FILE__,
                    dperf, g);
    theCommands.Add(
        "dsetsignal",
    ...
    theCommands.Add("dlog", "manage logging of commands and output; run without args to get help", __FILE__, dlog, g);
    theCommands.Add("decho", "switch on / off echo of commands to cout; run without args to get help", __FILE__, decho,
                    g);
    theCommands.Add("dtracelevel", "dtracelevel [trace|info|warn|alarm|fail]", __FILE__, dtracelevel, g);
    ...
    theCommands.Add("dbreak", "raises Tcl exception if user has pressed Control-Break key", __FILE__, dbreak, g);
    theCommands.Add("dversion",
                    "provides information on OCCT build configuration (version, compiler, OS, C library, etc.)",
                    __FILE__, dversion, g);
    theCommands.Add("dlocale", "set and / or query locate of C subsystem (function setlocale())", __FILE__, dlocale, g);
