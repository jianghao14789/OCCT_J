...
static Standard_Integer CommandCmd(ClientData theClientData, Tcl_Interp* interp, Standard_Integer argc,
                                   const char* argv[]) {
    ...
    Standard_Boolean isLogManipulation = (strcmp(argv[0], "dlog") == 0 || strcmp(argv[0], "decho") == 0);
    ...
    if (doEcho || doLog) dumpArgs(std::cout, argc, argv);
    ...
    try {
        ...
        Standard_SStream ss;
        ss << "An exception was caught " << theStdException.what() << " [" << typeid(theStdException).name() << "]"
           << std::ends;
        Tcl_SetResult(interp, (char*)(ss.str().c_str()), TCL_VOLATILE);
        code = TCL_ERROR;
    } catch (...) {
...
Draw_Interpretor::Draw_Interpretor()
    : // the tcl interpreter is not created immediately as it is kept
      ...
      myInterp(NULL), isAllocated(Standard_False), myDoLog(Standard_False), myDoEcho(Standard_False),
      myToColorize(Standard_True), myFDLog(-1) {
    //
}
...
Draw_Interpretor::Draw_Interpretor(const Draw_PInterp& theInterp)
    : myInterp(theInterp), isAllocated(Standard_False), myDoLog(Standard_False), myDoEcho(Standard_False),
      myToColorize(Standard_True), myFDLog(-1) {
    //
}
...
void Draw_Interpretor::Init() {
    if (isAllocated) Tcl_DeleteInterp(myInterp);
...
// purpose  :
//=======================================================================
void Draw_Interpretor::add(const Standard_CString theCommandName, const Standard_CString theHelp,
                           const Standard_CString theFileName, Draw_Interpretor::CallBackData* theCallback,
    ...
    Tcl_SetVar2(myInterp, "Draw_Groups", aGroup, aName, TCL_GLOBAL_ONLY | TCL_APPEND_VALUE | TCL_LIST_ELEMENT);
    ...
    if (aSrcPath.Value(1) == '/') aSrcPath.Remove(1);
...
Standard_Integer Draw_Interpretor::RecordAndEval(const Standard_CString line, const Standard_Integer flags) {
...
void Draw_Interpretor::AddLog(const Standard_CString theStr) {
    if (myFDLog < 0 || !theStr || !theStr[0]) return;
