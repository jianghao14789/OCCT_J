...
static Standard_Integer view(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if ((n >= 3) && (n != 4)) {
        ...
        if (dout.HasView(id)) dout.GetPosSize(id, X, Y, W, H);
        if (n >= 4) X = Draw::Atoi(a[3]);
        if (n >= 5) Y = Draw::Atoi(a[4]);
        if (n >= 6) W = Draw::Atoi(a[5]);
        if (n >= 7) H = Draw::Atoi(a[6]);
...
static Standard_Integer focal(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    Standard_Real df = 1.;
    if (!strcasecmp(a[0], "fu")) df = stepfocal;
    if (!strcasecmp(a[0], "fd")) df = 1. / stepfocal;
...
static Standard_Integer setfocal(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    if (n == 1) {
        for (Standard_Integer id = 0; id < MAXVIEW; id++) {
            if (!strcasecmp(dout.GetType(id), "PERS")) di << "Focal view " << id << " is " << dout.Focal(id) << "\n";
        }
    } else {
        Standard_Real f = Draw::Atof(a[1]);
        for (Standard_Integer id = 0; id < MAXVIEW; id++) {
            if (!strcasecmp(dout.GetType(id), "PERS")) dout.SetFocal(id, f);
        }
        dout.RepaintAll();
    }
    return 0;
...
static Standard_Integer xwd(Draw_Interpretor&, Standard_Integer n, const char** a) {
    ...
    if (!dout.SaveView(id, file)) return 1;
...
void Draw::GraphicCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("dfont", "dfont [name size] : set name and size of Draw font, or reset to default", __FILE__, dfont,
                    g);
}
