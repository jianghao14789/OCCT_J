...
void Draw_Viewer::MakeView(const Standard_Integer id, const char* typ, const Standard_Integer X,
                           const Standard_Integer Y, const Standard_Integer W, const Standard_Integer H) {
    if (Draw_Batch) return;
    if (id < MAXVIEW) {
        ...
        if (!myViews[id]->Init(typ)) DeleteView(id);
...
void Draw_Viewer::MakeView(const Standard_Integer id, const char* typ, const Standard_Integer X,
                           const Standard_Integer Y, const Standard_Integer W, const Standard_Integer H, HWND win,
...
void Draw_Viewer::RotateView(const Standard_Integer id, const gp_Pnt& P, const gp_Dir& D, const Standard_Real A) {
    if (Draw_Batch) return;
    if (myViews[id]) {
        gp_Trsf T;
...
void Draw_Viewer::SetFocal(const Standard_Integer id, const Standard_Real F) {
    if (Draw_Batch) return;
    if (myViews[id]) myViews[id]->SetFocalDistance(F);
}
...
void Draw_Viewer::GetTrsf(const Standard_Integer id, gp_Trsf& T) const {
    if (Draw_Batch) return;
    if (myViews[id]) T = myViews[id]->GetMatrix();
}
...
void Draw_Viewer::SetTrsf(const Standard_Integer id, gp_Trsf& T) {
    if (Draw_Batch) return;
    if (myViews[id]) myViews[id]->SetMatrix(T);
}
...
void Draw_Viewer::GetPosSize(const Standard_Integer id, Standard_Integer& X, Standard_Integer& Y, Standard_Integer& W,
                             Standard_Integer& H) {
    if (Draw_Batch) return;
    if (myViews[id] != NULL) {
...
void Draw_Viewer::GetFrame(const Standard_Integer id, Standard_Integer& xminf, Standard_Integer& yminf,
...
void Draw_Viewer::FitView(const Standard_Integer id, const Standard_Integer frame) {
    ...
    if (myViews[id]) {
        ...
        for (i = 1; i < MAXVIEW; i++) {
            if (myViews[i]) {
                if (myViews[i]->Is2D() == is2d) ++nbviews;
            }
        }
        ...
        for (i = 1; i <= n; i++) {
            ...
            if ((d3d && !is2d) || (!d3d && is2d)) {
                ...
                if (!only) DrawOnView(id, myDrawables(i));
...
void Draw_Viewer::PanView(const Standard_Integer id, const Standard_Integer DX, const Standard_Integer DY) {
    if (Draw_Batch) return;
    if (myViews[id]) {
...
void Draw_Viewer::SetPan(const Standard_Integer id, const Standard_Integer DX, const Standard_Integer DY) {
    if (Draw_Batch) return;
    if (myViews[id]) {
        myViews[id]->SetDx(DX);
        myViews[id]->SetDy(DY);
...
void Draw_Viewer::PostScriptView(const Standard_Integer id, const Standard_Integer VXmin, const Standard_Integer VYmin,
                                 const Standard_Integer VXmax, const Standard_Integer VYmax,
                                 const Standard_Integer PXmin, const Standard_Integer PYmin,
                                 const Standard_Integer PXmax, const Standard_Integer PYmax,
...
void Draw_Viewer::PostColor(const Standard_Integer icol, const Standard_Integer width, const Standard_Real gray) {
    if (Draw_Batch) return;
...
void Draw_Viewer::Repaint2D() const {
    ...
    for (Standard_Integer id = 0; id < MAXVIEW; id++)
        if (myViews[id]) {
            if (myViews[id]->Is2D()) RepaintView(id);
        }
}
...
void Draw_Viewer::Clear2D() {
    ...
    for (Standard_Integer id = 0; id < MAXVIEW; id++) {
        if (myViews[id]) {
            if (myViews[id]->Is2D()) ClearView(id);
        }
    }
}
...
void Draw_Viewer::DrawOnView(const Standard_Integer id, const Handle(Draw_Drawable3D) & D) const {
    ...
    if (myViews[id]) {
        ...
        if ((D->Is3D() && !view2d) || (!D->Is3D() && view2d)) {
            D->DrawOn(d);
            if (CurrentMode == DRAW) D->SetBounds(xmin, xmax, ymin, ymax);
            d.Flush();
        }
    }
}
...
void Draw_Viewer::HighlightOnView(const Standard_Integer id, const Handle(Draw_Drawable3D) & D,
...
// purpose  :
//=======================================================================
void Draw_Viewer::Select(Standard_Integer& theId, Standard_Integer& theX, Standard_Integer& theY,
                         Standard_Integer& theButton, Standard_Boolean theToWait) {
    if (Draw_Batch) {
        return;
    }
...
Standard_Integer Draw_Viewer::Pick(const Standard_Integer id, const Standard_Integer X, const Standard_Integer Y,
                                   const Standard_Integer Prec, Handle(Draw_Drawable3D) & D,
                                   const Standard_Integer first) const {
    if (Draw_Batch) return 0;
    if (myViews[id] == NULL) return 0;
...
void Draw_Display::DrawString(const gp_Pnt2d& ppt, const Standard_CString S, const Standard_Real moveX,
...
void Draw_Display::DrawString(const gp_Pnt& pt, const Standard_CString S, const Standard_Real moveX,
                              const Standard_Real moveY) {
    if (Draw_Batch) return;
...
// purpose  :
//=======================================================================
inline Standard_Integer CalculRegion(const Standard_Real x, const Standard_Real y, const Standard_Real x1,
                                     const Standard_Real y1, const Standard_Real x2, const Standard_Real y2) {
    Standard_Integer r;
    if (x < x1) {
        r = 1;
    } else {
...
Standard_Boolean Trim(gp_Pnt2d& P1, gp_Pnt2d& P2, Standard_Real x0, Standard_Real y0, Standard_Real x1,
                      Standard_Real y1) {
        ...
        case PICK:
            if (!found) {
                ...
                if ((x1 > xpick + precpick) || (x2 > xpick + precpick)) {
                    Standard_Real y = (Standard_Real)y1 + (Standard_Real)(y2 - y1) *
                                                              (Standard_Real)(xpick + precpick - x1) /
                                                              (Standard_Real)(x2 - x1);
                ...
                if ((y1 > ypick + precpick) || (y2 > ypick + precpick)) {
                    Standard_Real x = (Standard_Real)x1 + (Standard_Real)(x2 - x1) *
                                                              (Standard_Real)(ypick + precpick - y1) /
                                                              (Standard_Real)(y2 - y1);
