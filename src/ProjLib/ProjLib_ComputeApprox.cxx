...
static gp_Pnt2d Function_Value(const Standard_Real U, const Handle(Adaptor3d_Curve) & myCurve,
                               const Handle(Adaptor3d_Surface) & mySurface, const Standard_Real U1,
                               const Standard_Real U2, const Standard_Real V1, const Standard_Real V2,
                               const Standard_Boolean UCouture, const Standard_Boolean VCouture) {
    ...
    if (VCouture) {
        if (SType == GeomAbs_Sphere) {
            ...
            if (S > U1 || S < U2) S = ElCLib::InPeriod(S, U1, U2);
        }
        if (T < V1 || T > V2) T = ElCLib::InPeriod(T, V1, V2);
    }
...
// purpose  :
//=======================================================================
static Standard_Boolean Function_D1(const Standard_Real U, gp_Pnt2d& P, gp_Vec2d& D,
    ...
    switch (Type) {
        ...
        case GeomAbs_Torus: {
            ...
            if (Nu < Epsilon(1.) || Nv < Epsilon(1.)) return Standard_False;
...
// purpose  :
//=======================================================================
static Standard_Real Function_ComputeStep(const Handle(Adaptor3d_Curve) & myCurve, const Standard_Real R) {
...
// purpose  :
//=======================================================================
static void Function_SetUVBounds(Standard_Real& myU1, Standard_Real& myU2, Standard_Real& myV1, Standard_Real& myV2,
                                 Standard_Boolean& UCouture, Standard_Boolean& VCouture,
                                 const Handle(Adaptor3d_Curve) & myCurve, const Handle(Adaptor3d_Surface) & mySurface) {
    ...
    switch (mySurface->GetType()) {
        ...
        case GeomAbs_Cone: {
            ...
            switch (myCurve->GetType()) {
                ...
                default: {
                    ...
                    if (!(Abs(pmin - W1) <= Precision::PConfusion() || Abs(pmin - W2) <= Precision::PConfusion()))
                        myU1 -= dmax * .5;
                    if (!(Abs(pmax - W1) <= Precision::PConfusion() || Abs(pmax - W2) <= Precision::PConfusion()))
                        myU2 += dmax * .5;
        ...
        case GeomAbs_Torus: {
            ...
            if (!(Abs(pminU - W1) <= Precision::PConfusion() || Abs(pminU - W2) <= Precision::PConfusion()))
                myU1 -= dmaxU * .5;
            if (!(Abs(pmaxU - W1) <= Precision::PConfusion() || Abs(pmaxU - W2) <= Precision::PConfusion()))
                myU2 += dmaxU * .5;
            if (!(Abs(pminV - W1) <= Precision::PConfusion() || Abs(pminV - W2) <= Precision::PConfusion()))
                myV1 -= dmaxV * .5;
            if (!(Abs(pmaxV - W1) <= Precision::PConfusion() || Abs(pmaxV - W2) <= Precision::PConfusion()))
                myV2 += dmaxV * .5;
...
public:
    ...
    ProjLib_Function(const Handle(Adaptor3d_Curve) & C, const Handle(Adaptor3d_Surface) & S)
        : myCurve(C), mySurface(S), myU1(0.0), myU2(0.0), myV1(0.0), myV2(0.0), UCouture(Standard_False),
          VCouture(Standard_False) {
        myNbPnt = 0;
        myNbPnt2d = 1;
    ...
    void PeriodInformation(const Standard_Integer theDimIdx, Standard_Boolean& IsPeriodic,
    ...
    Standard_Boolean D1(const Standard_Real theT, NCollection_Array1<gp_Vec2d>& theVec2d,
        ...
        Standard_Boolean isOk =
            Function_D1(theT, aPnt2d, aVec2d, myCurve, mySurface, myU1, myU2, myV1, myV2, UCouture, VCouture);
        theVec2d(1) = aVec2d;
        return isOk;
    }
};
...
static Standard_Real ComputeTolU(const Handle(Adaptor3d_Surface) & theSurf, const Standard_Real theTolerance) {
...
static Standard_Real ComputeTolV(const Handle(Adaptor3d_Surface) & theSurf, const Standard_Real theTolerance) {
...
ProjLib_ComputeApprox::ProjLib_ComputeApprox()
    : myTolerance(Precision::PApproximation()), myDegMin(-1), myDegMax(-1), myMaxSegments(-1),
...
ProjLib_ComputeApprox::ProjLib_ComputeApprox(const Handle(Adaptor3d_Curve) & C, const Handle(Adaptor3d_Surface) & S,
                                             const Standard_Real Tol)
    : myTolerance(Max(Tol, Precision::PApproximation())), myDegMin(-1), myDegMax(-1), myMaxSegments(-1),
      myBndPnt(AppParCurves_TangencyPoint) {
    Perform(C, S);
}
...
void ProjLib_ComputeApprox::Perform(const Handle(Adaptor3d_Curve) & C, const Handle(Adaptor3d_Surface) & S) {
    ...
    if (CType == GeomAbs_BSplineCurve && SType == GeomAbs_Plane) {
        ...
        if (BS->IsRational()) {
            myBSpline = new Geom2d_BSplineCurve(Poles, Weights, Knots, Mults, BS->Degree(), BS->IsPeriodic());
        } else {
        ...
        if (Fit.IsAllApproximated()) {
            ...
            if (NbPoles <= 0 || NbPoles > 100000) return;
            if (NbKnots <= 0 || NbKnots > 100000) return;
        ...
        if (!myBSpline.IsNull()) {
            if (du != 0. || dv != 0.) myBSpline->Translate(gp_Vec2d(du, dv));
            if (ToMirror) {
...
// purpose  :
//=======================================================================
void ProjLib_ComputeApprox::SetDegree(const Standard_Integer theDegMin, const Standard_Integer theDegMax) {
    myDegMin = theDegMin;
    myDegMax = theDegMax;
}
//=======================================================================
