...
static gp_Pnt OnPlane_Value(const Standard_Real U, const Handle(Adaptor3d_Curve) & aCurvePtr, const gp_Ax3& Pl,
                            const gp_Dir& D) {
    //                   PO . Z                /  Z = Pl.Direction()
...
static gp_Vec OnPlane_DN(const Standard_Real U, const Standard_Integer DerivativeRequest,
                         const Handle(Adaptor3d_Curve) & aCurvePtr, const gp_Ax3& Pl, const gp_Dir& D) {
    //                   PO . Z                /  Z = Pl.Direction()
    // Proj(u) = P(u) + -------  * D     avec  \  O = Pl.Location()
    //                   D  . Z
...
static Standard_Boolean OnPlane_D1(const Standard_Real U, gp_Pnt& P, gp_Vec& V,
                                   const Handle(Adaptor3d_Curve) & aCurvePtr, const gp_Ax3& Pl, const gp_Dir& D) {
    Standard_Real Alpha;
    gp_Pnt Point;
    gp_Vec Vector;
...
static Standard_Boolean OnPlane_D2(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2,
                                   const Handle(Adaptor3d_Curve) & aCurvePtr, const gp_Ax3& Pl, const gp_Dir& D) {
    Standard_Real Alpha;
    gp_Pnt Point;
...
static Standard_Boolean OnPlane_D3(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3,
                                   const Handle(Adaptor3d_Curve) & aCurvePtr, const gp_Ax3& Pl, const gp_Dir& D) {
    Standard_Real Alpha;
    gp_Pnt Point;
...
public:
    ProjLib_MaxCurvature(LProp3d_CLProps& theProps) : myProps(&theProps) {}
...
static void PerformApprox(const Handle(Adaptor3d_Curve) & C, const gp_Ax3& Pl, const gp_Dir& D,
...

{
    ...
    Approx_FitAndDivide Fit(Deg1, Deg2, Precision::Approximation(), Precision::PApproximation(), Standard_True);
    ...
    for (i = 1; i <= Fit.NbMultiCurves(); i++) {
        ...
        if (MaxDeg > MC.Degree()) {
            BSplCLib::IncreaseDegree(MaxDeg, LocalPoles, BSplCLib::NoWeights(), TempPoles, BSplCLib::NoWeights());
...
ProjLib_ProjectOnPlane::ProjLib_ProjectOnPlane()
    : myKeepParam(Standard_False), myFirstPar(0.), myLastPar(0.), myTolerance(0.), myType(GeomAbs_OtherCurve),
...
ProjLib_ProjectOnPlane::ProjLib_ProjectOnPlane(const gp_Ax3& Pl)
    : myPlane(Pl), myDirection(Pl.Direction()), myKeepParam(Standard_False), myFirstPar(0.), myLastPar(0.),
      myTolerance(0.), myType(GeomAbs_OtherCurve), myIsApprox(Standard_False) {}
...
ProjLib_ProjectOnPlane::ProjLib_ProjectOnPlane(const gp_Ax3& Pl, const gp_Dir& D)
    : myPlane(Pl), myDirection(D), myKeepParam(Standard_False), myFirstPar(0.), myLastPar(0.), myTolerance(0.),
      myType(GeomAbs_OtherCurve), myIsApprox(Standard_False) {
...
void ProjLib_ProjectOnPlane::Load(const Handle(Adaptor3d_Curve) & C, const Standard_Real Tolerance,
    ...
    switch (Type) {
        case GeomAbs_Line: {
            ...
            if (Xc.Magnitude() < Precision::Confusion()) { // line orthog au plan
                ...
                //  Modified by Sergey KHROMOV - Tue Jan 29 16:57:29 2002 Begin
                GeomAdaptor_Curve aGACurve(GeomLinePtr, myCurve->FirstParameter(), myCurve->LastParameter());
                myResult = new GeomAdaptor_Curve(aGACurve);
                //  Modified by Sergey KHROMOV - Tue Jan 29 16:57:30 2002 End
            } else {
                myType = GeomAbs_Line;
                ...
                if (!myKeepParam) {
                    ...
                    BSplCLib::Reparametrize(myCurve->FirstParameter(), myCurve->LastParameter(), BsplineKnots);
        ...
        Standard_FALLTHROUGH case GeomAbs_Ellipse: {
            ...
            if (VDx.SquareMagnitude() < Tol2 || VDy.SquareMagnitude() < Tol2 || VDx.CrossSquareMagnitude(VDy) < Tol2) {
            ...
            if (!myIsApprox) {
                ...
                if (myKeepParam) {
                    ...
                    // Since it is not necessary to keep the same parameter for the point on the original and on the
                    // projected curves, we will use the following approach to find axes of the projected ellipse and
                    // provide the canonical curve:
                    ...
                    if (anEigenCalc.IsDone()) {
                        ...
                        // calculate the rotation angle for the plane axes to meet the correct axes of the projected
                        // ellipse (swap eigenvectors in respect to major and minor axes)
                        const math_Matrix& anEigenVec = anEigenCalc.Vectors();
                        gp_Trsf2d aTrsfInPlane;
                        aTrsfInPlane.SetValues(anEigenVec(1, 2), anEigenVec(1, 1), 0.0, anEigenVec(2, 2),
                                               anEigenVec(2, 1), 0.0);
                        gp_Trsf aRot;
            ...
            if (myIsApprox) {
                ...
                if (aResultCurve.IsNull()) aResultCurve = GeomEllipsePtr;
        ...
        case GeomAbs_Parabola: {
            ...
            if ((Abs(Yc.Magnitude() - 1.) < Precision::Confusion()) && (Xc.Magnitude() < Precision::Confusion())) {
                myType = GeomAbs_Line;
        ...
        case GeomAbs_Hyperbola: {
            ...
            if (Xc.Magnitude() < Precision::Confusion()) {
                ...
                myType = GeomAbs_Hyperbola;
                Hypr = gp_Hypr(gp_Ax2(P, gp_Dir(Xc ^ Yc), gp_Dir(Xc)), aR1 * Xc.Magnitude(), aR2 * Yc.Magnitude());
                GeomHyperbolaPtr = new Geom_Hyperbola(Hypr);
            } else if (Yc.Magnitude() < Precision::Confusion() || Yc.IsParallel(Xc, Precision::Angular())) {
                myIsApprox = Standard_True;
            } else if (!myKeepParam) {
...
Handle(Adaptor3d_Curve) ProjLib_ProjectOnPlane::Trim(const Standard_Real First, const Standard_Real Last,
...
void ProjLib_ProjectOnPlane::D3(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3) const {
    if (myType != GeomAbs_OtherCurve) {
...
gp_Vec ProjLib_ProjectOnPlane::DN(const Standard_Real U, const Standard_Integer DerivativeRequest) const {
    if (myType != GeomAbs_OtherCurve) {
...
gp_Lin ProjLib_ProjectOnPlane::Line() const {
    if (myType != GeomAbs_Line) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Line");
...
gp_Circ ProjLib_ProjectOnPlane::Circle() const {
    if (myType != GeomAbs_Circle) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Circle");
...
gp_Elips ProjLib_ProjectOnPlane::Ellipse() const {
    if (myType != GeomAbs_Ellipse) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Ellipse");
...
gp_Hypr ProjLib_ProjectOnPlane::Hyperbola() const {
    if (myType != GeomAbs_Hyperbola) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Hyperbola");
...
gp_Parab ProjLib_ProjectOnPlane::Parabola() const {
    if (myType != GeomAbs_Parabola) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Parabola");
...
Standard_Integer ProjLib_ProjectOnPlane::NbKnots() const {
    if (GetType() != GeomAbs_BSplineCurve) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:NbKnots");
...
Handle(Geom_BezierCurve) ProjLib_ProjectOnPlane::Bezier() const {
    if (myType != GeomAbs_BezierCurve) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:Bezier");
...
Handle(Geom_BSplineCurve) ProjLib_ProjectOnPlane::BSpline() const {
    if (myType != GeomAbs_BSplineCurve) throw Standard_NoSuchObject("ProjLib_ProjectOnPlane:BSpline");
...
void ProjLib_ProjectOnPlane::BuildByApprox(const Standard_Real theLimitParameter) {
    ...
    if (Precision::IsInfinite(myCurve->FirstParameter()) || Precision::IsInfinite(myCurve->LastParameter())) {
