...
struct aFuncStruct {
    aFuncStruct() // Empty constructor.
        : mySqProjOrtTol(0.0), myTolU(0.0), myTolV(0.0) {
...
//=======================================================================
static void computePeriodicity(const Handle(Adaptor3d_Surface) & theSurf, Standard_Real& theUPeriod,
...
//=======================================================================
static Standard_Real anOrthogSqValue(const gp_Pnt& aBasePnt, const Handle(Adaptor3d_Surface) & Surf,
    ...
    if (aSu.SquareMagnitude() > Precision::SquareConfusion()) aSu.Normalize();

    if (aSv.SquareMagnitude() > Precision::SquareConfusion()) aSv.Normalize();
...
static gp_Pnt2d Function_Value(const Standard_Real theU, const aFuncStruct& theData) {
    ...
    if (Abs(p2d.X() - Uinf) < Precision::PConfusion() || Abs(p2d.X() - Usup) < Precision::PConfusion()) {
        // V isoline.
        gp_Pnt aPnt;
        theData.mySurf->D0(p2d.X(), theU, aPnt);
        if (aPnt.SquareDistance(p) < aSurfPntDist) p2d.SetY(theU);
    }

    if (Abs(p2d.Y() - Vinf) < Precision::PConfusion() || Abs(p2d.Y() - Vsup) < Precision::PConfusion()) {
        // U isoline.
        gp_Pnt aPnt;
        theData.mySurf->D0(theU, p2d.Y(), aPnt);
        if (aPnt.SquareDistance(p) < aSurfPntDist) p2d.SetX(theU);
    }
...
public:
    ProjLib_PolarFunction(const Handle(Adaptor3d_Curve) & C, const Handle(Adaptor3d_Surface) & Surf,
                          const Handle(Adaptor2d_Curve2d) & InitialCurve2d, const Standard_Real Tol3d) {
        myNbPnt = 0;
        myNbPnt2d = 1;
...
ProjLib_ComputeApproxOnPolarSurface::ProjLib_ComputeApproxOnPolarSurface()
    : myProjIsDone(Standard_False), myTolerance(Precision::Approximation()), myTolReached(-1.0), myDegMin(-1),
      myDegMax(-1), myMaxSegments(-1), myMaxDist(-1.), myBndPnt(AppParCurves_TangencyPoint), myDist(0.) {}
...
ProjLib_ComputeApproxOnPolarSurface::ProjLib_ComputeApproxOnPolarSurface(const Handle(Adaptor2d_Curve2d) &
                                                                             theInitialCurve2d,
                                                                         const Handle(Adaptor3d_Curve) & theCurve,
                                                                         const Handle(Adaptor3d_Surface) & theSurface,
                                                                         const Standard_Real theTolerance3D)
    : myProjIsDone(Standard_False), myTolerance(theTolerance3D), myTolReached(-1.0), myDegMin(-1), myDegMax(-1),
      myMaxSegments(-1), myMaxDist(-1.), myBndPnt(AppParCurves_TangencyPoint), myDist(0.) {
...
ProjLib_ComputeApproxOnPolarSurface::ProjLib_ComputeApproxOnPolarSurface(const Handle(Adaptor3d_Curve) & theCurve,
                                                                         const Handle(Adaptor3d_Surface) & theSurface,
                                                                         const Standard_Real theTolerance3D)
    : myProjIsDone(Standard_False), myTolerance(theTolerance3D), myTolReached(-1.0), myDegMin(-1), myDegMax(-1),
      myMaxSegments(-1), myMaxDist(-1.), myBndPnt(AppParCurves_TangencyPoint), myDist(0.) {
...
ProjLib_ComputeApproxOnPolarSurface::ProjLib_ComputeApproxOnPolarSurface(
    const Handle(Adaptor2d_Curve2d) & theInitialCurve2d, const Handle(Adaptor2d_Curve2d) & theInitialCurve2dBis,
    const Handle(Adaptor3d_Curve) & theCurve, const Handle(Adaptor3d_Surface) & theSurface,
    const Standard_Real theTolerance3D)
    : myProjIsDone(Standard_False), myTolerance(theTolerance3D), myTolReached(-1.0), myDegMin(-1), myDegMax(-1),
      myMaxSegments(-1), myMaxDist(-1.), myBndPnt(AppParCurves_TangencyPoint), myDist(0.) {
...
static Handle(Geom2d_BSplineCurve) Concat(Handle(Geom2d_BSplineCurve) C1, Handle(Geom2d_BSplineCurve) C2,
                                          Standard_Real theUJump, Standard_Real theVJump) {
...
Handle(Geom2d_BSplineCurve) ProjLib_ComputeApproxOnPolarSurface::Perform(const Handle(Adaptor2d_Curve2d) &
                                                                             InitialCurve2d,
    ...
    if (typeCurve == GeomAbs_BSplineCurve) {
        ...
        if (nbInter > 1) {
            ...
            for (Standard_Integer ii = 2; ii <= NbC; ii++) {
                Handle(Geom2d_BSplineCurve) BS = Handle(Geom2d_BSplineCurve)::DownCast(LOfBSpline2d.First());
    ...
    if (InitialCurve2d.IsNull()) {
        ...
        if (!myProjIsDone) return Handle(Geom2d_BSplineCurve)();
    }
...
Handle(Adaptor2d_Curve2d) ProjLib_ComputeApproxOnPolarSurface::BuildInitialCurve2d(
    const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & Surf) {
    ...
    if ((Type != GeomAbs_BSplineSurface) && (Type != GeomAbs_BezierSurface) && (Type != GeomAbs_OffsetSurface)) {
        ...
        if (aExtPS.IsDone()) {
            for (i = 1; i <= aExtPS.NbExt(); i++) {
                ...
                if (aSqDist < aMinSqDist) aMinSqDist = aSqDist;
            }
        }
        if (aMinSqDist > DistTol3d2) // try to project with less tolerance
        {
            ...
            aExtPS.Initialize(*Surf, Surf->FirstUParameter(), Surf->LastUParameter(), Surf->FirstVParameter(),
                              Surf->LastVParameter(), TolU, TolV);
            ...
            for (i = 2; i <= NbOfPnts; i++)
                if (myProjIsDone) {
                    ...
                    if (aLocateExtPS.IsDone()) {
                        if (aLocateExtPS.SquareDistance() < DistTol3d2) { // OCC217
                            ...
                            if (aGlobalExtr.IsDone()) {
                                ...
                                if (LocalMinSqDist < DistTol3d2) {
                                    ...
                                    if ((i == 2) &&
                                        (!IsEqual(uperiod, 0.0) ||
                                         !IsEqual(vperiod, 0.0))) { // Make 1st point more precise for periodic surfaces
    ...
    if (myProjIsDone) {
        ...
        if (Abs(Offset.X()) > gp::Resolution() || Abs(Offset.Y()) > gp::Resolution()) myBSpline->Translate(Offset);
        //////////////////////////////////////////
...
Handle(Geom2d_BSplineCurve) ProjLib_ComputeApproxOnPolarSurface::ProjectUsingInitialCurve2d(
    ...
    if (TheTypeS == GeomAbs_Plane) {
        ...
        if (TheTypeC == GeomAbs_BSplineCurve) {
            ...
            if (BSC->IsRational()) {
                ...
                BSC->Weights(Weights);
                return new Geom2d_BSplineCurve(Poles2d, Weights, Knots, Mults, BSC->Degree(), BSC->IsPeriodic());
            }
        ...
        if (TheTypeC == GeomAbs_BezierCurve) {
            ...
            if (BC->IsRational()) {
                ...
                BC->Weights(Weights);
                return new Geom2d_BSplineCurve(Poles2d, Weights, Knots, Mults, BC->Degree(), BC->IsPeriodic());
            }
                ...
                if (TheTypeC == GeomAbs_BSplineCurve) {
                    ...
                    if (myProjIsDone) {
                        ...
                        if (BSC->IsRational()) {
                            TColStd_Array1OfReal Weights(1, BSC->NbPoles());
                            BSC->Weights(Weights);
                            return new Geom2d_BSplineCurve(Poles2d, Weights, Knots, Mults, BSC->Degree(),
                                                           BSC->IsPeriodic());
                        }
                        return new Geom2d_BSplineCurve(Poles2d, Knots, Mults, BSC->Degree(), BSC->IsPeriodic());
                    }
                ...
                if (TheTypeC == GeomAbs_BezierCurve) {
                    ...
                    if (myProjIsDone) {
                        ...
                        if (BC->IsRational()) {
                            TColStd_Array1OfReal Weights(1, BC->NbPoles());
                            BC->Weights(Weights);
                            return new Geom2d_BSplineCurve(Poles2d, Weights, Knots, Mults, BC->Degree(),
                                                           BC->IsPeriodic());
                        }
                        return new Geom2d_BSplineCurve(Poles2d, Knots, Mults, BC->Degree(), BC->IsPeriodic());
                    }
    ...
    Handle(Geom2d_BSplineCurve) DummyC2d = new Geom2d_BSplineCurve(DummyPoles, DummyKnots, DummyMults, 1);
#ifdef DRAW
    ...
    if (Fit.IsAllApproximated()) {
        ...
        for (i = 1; i <= NbCurves; i++) {
            ...
            if (Inc > 0) {
                ...
                BSplCLib::IncreaseDegree(MaxDeg, Poles2d, BSplCLib::NoWeights(), TempPoles, BSplCLib::NoWeights());
