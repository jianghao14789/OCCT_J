...
static Standard_Boolean IsoIsDeg(const Adaptor3d_Surface& S, const Standard_Real Param, const GeomAbs_IsoType IT,
                                 const Standard_Real TolMin, const Standard_Real TolMax) {
    ...
    if (IT == GeomAbs_IsoV) {
        ...
        if (D1NormMax > TolMax || D1NormMax < TolMin) Along = Standard_False;
    } else {
        Step = (V2 - V1) / 10;
        D1NormMax = 0.;
...
static void TrimC3d(Handle(Adaptor3d_Curve) & myCurve, Standard_Boolean* IsTrimmed, const Standard_Real dt,
                    const gp_Pnt& Pole, Standard_Integer* SingularCase, const Standard_Integer NumberOfSingularCase,
...
static void ExtendC2d(Handle(Geom2d_BSplineCurve) & aRes, const Standard_Real /*t*/, const Standard_Real /*dt*/,
                      const Standard_Real u1, const Standard_Real u2, const Standard_Real v1, const Standard_Real v2,
                      const Standard_Integer FirstOrLast, const Standard_Integer NumberOfSingularCase) {
...
ProjLib_ProjectedCurve::ProjLib_ProjectedCurve()
    : myTolerance(Precision::Confusion()), myDegMin(-1), myDegMax(-1), myMaxSegments(-1), myMaxDist(-1.),
...
ProjLib_ProjectedCurve::ProjLib_ProjectedCurve(const Handle(Adaptor3d_Surface) & S, const Handle(Adaptor3d_Curve) & C)
    : myTolerance(Precision::Confusion()), myDegMin(-1), myDegMax(-1), myMaxSegments(-1), myMaxDist(-1.),
      myBndPnt(AppParCurves_TangencyPoint) {
    Load(S);
    Perform(C);
...
ProjLib_ProjectedCurve::ProjLib_ProjectedCurve(const Handle(Adaptor3d_Surface) & S, const Handle(Adaptor3d_Curve) & C,
    ...
    : myTolerance(Max(Tol, Precision::Confusion())), myDegMin(-1), myDegMax(-1), myMaxSegments(-1), myMaxDist(-1.),
      myBndPnt(AppParCurves_TangencyPoint) {
    Load(S);
    Perform(C);
}
...
void ProjLib_ProjectedCurve::Perform(const Handle(Adaptor3d_Curve) & C) {
        ...
        case GeomAbs_BSplineSurface: {
            ...
            if (!aRes.IsNull()) {
                ...
                if ((IsTrimmed[0] || IsTrimmed[1])) {
                    ...
                    GeomLib::SameRange(Precision::PConfusion(), aRes, aRes->FirstParameter(), aRes->LastParameter(),
    ...
    if (!myResult.IsDone() && isAnalyticalSurf) {
        ...
        if (Comp.Bezier().IsNull() && Comp.BSpline().IsNull()) return; // advanced projector has been failed too
        myResult.Done();
        ...
        if ((IsTrimmed[0] || IsTrimmed[1])) {
            ...
            Handle(Geom2d_Curve) NewCurve2d;
            GeomLib::SameRange(Precision::PConfusion(), aRes, aRes->FirstParameter(), aRes->LastParameter(), FirstPar,
                               LastPar, NewCurve2d);
            aRes = Handle(Geom2d_BSplineCurve)::DownCast(NewCurve2d);
            myResult.SetBSpline(aRes);
            myResult.SetType(GeomAbs_BSplineCurve);
    ...
    if (myResult.IsDone() && (isPeriodic[0] || isPeriodic[1])) {
        ...
        const Standard_Real aSurfFirstPar[2] = {mySurface->FirstUParameter(), mySurface->FirstVParameter()};
        Standard_Real aSurfPeriod[2] = {0.0, 0.0};
        if (isPeriodic[0]) aSurfPeriod[0] = mySurface->UPeriod();
        if (isPeriodic[1]) aSurfPeriod[1] = mySurface->VPeriod();

        for (Standard_Integer anIdx = 1; anIdx <= 2; anIdx++) {
            if (!isPeriodic[anIdx - 1]) continue;
            ...
            if (myResult.GetType() == GeomAbs_BSplineCurve) {
                ...
                for (Standard_Integer aKnotIdx = aRes->FirstUKnotIndex(); aKnotIdx < aRes->LastUKnotIndex();
                    ...
                    for (Standard_Integer anIntIdx = 0; anIntIdx <= aDeg; anIntIdx++) {
                        ...
                        Standard_Integer aMapKey =
                            Standard_Integer((aPnt2d.Coord(anIdx) - aSurfFirstPar[anIdx - 1]) / aSurfPeriod[anIdx - 1]);

                        if (aPnt2d.Coord(anIdx) - aSurfFirstPar[anIdx - 1] < 0.0) aMapKey--;
...
// purpose  :
//=======================================================================
void ProjLib_ProjectedCurve::SetDegree(const Standard_Integer theDegMin, const Standard_Integer theDegMax) {
    myDegMin = theDegMin;
    myDegMax = theDegMax;
}
//=======================================================================
...
void ProjLib_ProjectedCurve::D3(const Standard_Real, gp_Pnt2d&, gp_Vec2d&, gp_Vec2d&, gp_Vec2d&) const {
...
Standard_Integer ProjLib_ProjectedCurve::Degree() const {
    Standard_NoSuchObject_Raise_if((GetType() != GeomAbs_BSplineCurve) && (GetType() != GeomAbs_BezierCurve),
...
Standard_Integer ProjLib_ProjectedCurve::NbKnots() const {
    Standard_NoSuchObject_Raise_if(GetType() != GeomAbs_BSplineCurve, "ProjLib_ProjectedCurve:NbKnots");
