...
public:
    ...
    //! Constructor, which performs projecting, using initial curve 2d InitCurve2d, which is any rough approximation of
    //! result curve. Parameter Tol is 3d tolerance of approximation.
    ...
    //! Constructor, which performs projecting, using two initial curves 2d: InitCurve2d and InitCurve2dBis that are any
    //! rough approximations of result curves. This constructor is used to get two pcurves for seem edge. Parameter Tol
    //! is 3d tolerance of approximation.
    Standard_EXPORT ProjLib_ComputeApproxOnPolarSurface(const Handle(Adaptor2d_Curve2d) & InitCurve2d,
    ...
    Standard_EXPORT Handle(Geom2d_BSplineCurve)
        Perform(const Handle(Adaptor2d_Curve2d) & InitCurve2d, const Handle(Adaptor3d_Curve) & C,
                const Handle(Adaptor3d_Surface) & S);

    //! Builds initial 2d curve as BSpline with degree = 1 using Extrema algorithm.
    //! Method is used in method Perform(...).
    Standard_EXPORT Handle(Adaptor2d_Curve2d)
        BuildInitialCurve2d(const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & S);

    //! Method, which performs projecting.
    //! Method is used in method Perform(...).
    Standard_EXPORT Handle(Geom2d_BSplineCurve)
        ProjectUsingInitialCurve2d(const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & S,
