...
struct SplitDS {
    SplitDS(const Handle(Adaptor3d_Curve) & theCurve, const Handle(Adaptor3d_Surface) & theSurface,
            NCollection_Vector<Standard_Real>& theSplits)
        : myCurve(theCurve), mySurface(theSurface), mySplits(theSplits), myPerMinParam(0.0), myPerMaxParam(0.0),
          myPeriodicDir(0), myExtCCCurve1(NULL), myExtCCLast2DParam(0.0), myExtPS(NULL) {}
...
static void BuildCurveSplits(const Handle(Adaptor3d_Curve) & theCurve, const Handle(Adaptor3d_Surface) & theSurface,
...
static void FindSplitPoint(SplitDS& theSplitDS, const Standard_Real theMinParam, const Standard_Real theMaxParam);
...
static void d1(const Standard_Real t, const Standard_Real u, const Standard_Real v, gp_Vec2d& V,
               const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & Surface) {
    gp_Pnt S, C;
...
static void d2(const Standard_Real t, const Standard_Real u, const Standard_Real v, gp_Vec2d& V1, gp_Vec2d& V2,
               const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & Surface) {
    gp_Pnt S, C;
    ...
    Standard_Real tmp;
    gp_Vec2d Row11(3 * DS1_u * DS2_u + Ort * DS3_u, tmp = 2 * DS1_u * DS2_uv + DS1_v * DS2_u + Ort * DS3_uuv);
...
static void d2CurvOnSurf(const Standard_Real t, const Standard_Real u, const Standard_Real v, gp_Vec& V1, gp_Vec& V2,
                         const Handle(Adaptor3d_Curve) & Curve, const Handle(Adaptor3d_Surface) & Surface) {
    gp_Pnt S, C;
    gp_Vec2d V12d, V22d;
    gp_Vec DS1_u, DS1_v, DS2_u, DS2_uv, DS2_v, DS3_u, DS3_v, DS3_uuv, DS3_uvv, DC1_t, DC2_t;
    ...
    gp_Vec2d S3(V12d * gp_Vec2d(Row11 * V12d, Row12 * V12d), V12d * gp_Vec2d(Row21 * V12d, Row22 * V12d));
...
static Standard_Boolean ExactBound(gp_Pnt& Sol, const Standard_Real NotSol, const Standard_Real Tol,
    ...
    while (!Seq.IsEmpty()) {
        ...
        if (Standard_Integer(P.Z()) == 2) {
            aPrjPS.Perform(t, P.X(), V0, gp_Pnt2d(Tol, TolV), gp_Pnt2d(t1, Surface->FirstVParameter()),
            ...
            aPrjPS.Perform(t, U0, P.X(), gp_Pnt2d(Tol, TolU), gp_Pnt2d(t1, Surface->FirstUParameter()),
...
static void DichExactBound(gp_Pnt& Sol, const Standard_Real NotSol, const Standard_Real Tol, const Standard_Real TolU,
                           const Standard_Real TolV, const Handle(Adaptor3d_Curve) & Curve,
                           const Handle(Adaptor3d_Surface) & Surface) {
#ifdef OCCT_DEBUG_CHRONO
    ...
    while (fabs(Sol.X() - aNotSol) > Tol) {
        ...
        aPrjPS.Perform(t, U0, V0, gp_Pnt2d(TolU, TolV),
                       ...
                       gp_Pnt2d(Surface->LastUParameter(), Surface->LastVParameter()), FuncTol, Standard_True);
...
static Standard_Boolean InitialPoint(const gp_Pnt& Point, const Standard_Real t, const Handle(Adaptor3d_Curve) & C,
                                     const Handle(Adaptor3d_Surface) & S, const Standard_Real TolU,
                                     const Standard_Real TolV, Standard_Real& U, Standard_Real& V,
                                     Standard_Real theMaxDist) {

    ProjLib_PrjResolve aPrjPS(*C, *S, 1);
    Standard_Real ParU, ParV;
    Extrema_ExtPS aExtPS;
    aExtPS.Initialize(*S, S->FirstUParameter(), S->LastUParameter(), S->FirstVParameter(), S->LastVParameter(), TolU,
                      TolV);
    ...
    if (aExtPS.IsDone() && aExtPS.NbExt()) {
        ...
        for (i = 1; i <= Nend; i++) {
            ...
            aPrjPS.Perform(t, ParU, ParV, gp_Pnt2d(TolU, TolV), gp_Pnt2d(S->FirstUParameter(), S->FirstVParameter()),
                           gp_Pnt2d(S->LastUParameter(), S->LastVParameter()), FuncTol, Standard_True);
            if (aPrjPS.IsDone())
...
ProjLib_CompProjectedCurve::ProjLib_CompProjectedCurve() : myNbCurves(0), myMaxDist(0.0), myTolU(0.0), myTolV(0.0) {}
...
ProjLib_CompProjectedCurve::ProjLib_CompProjectedCurve(const Handle(Adaptor3d_Surface) & theSurface,
                                                       ...
                                                       const Standard_Real theTolU, const Standard_Real theTolV)
    : mySurface(theSurface), myCurve(theCurve), myNbCurves(0), mySequence(new ProjLib_HSequenceOfHSequenceOfPnt()),
      myTol3d(1.e-6), myContinuity(GeomAbs_C2), myMaxDegree(14), myMaxSeg(16), myProj2d(Standard_True),
      myProj3d(Standard_False), myMaxDist(-1.0), myTolU(theTolU), myTolV(theTolV) {
    Init();
}
...
ProjLib_CompProjectedCurve::ProjLib_CompProjectedCurve(const Handle(Adaptor3d_Surface) & theSurface,
    ...
    : mySurface(theSurface), myCurve(theCurve), myNbCurves(0), mySequence(new ProjLib_HSequenceOfHSequenceOfPnt()),
      myTol3d(1.e-6), myContinuity(GeomAbs_C2), myMaxDegree(14), myMaxSeg(16), myProj2d(Standard_True),
      myProj3d(Standard_False), myMaxDist(theMaxDist), myTolU(theTolU), myTolV(theTolV) {
    Init();
}
...
ProjLib_CompProjectedCurve::ProjLib_CompProjectedCurve(const Standard_Real theTol3d,
                                                       ...
                                                       const Standard_Real theMaxDist)
    : mySurface(theSurface), myCurve(theCurve), myNbCurves(0), mySequence(new ProjLib_HSequenceOfHSequenceOfPnt()),
      myTol3d(theTol3d), myContinuity(GeomAbs_C2), myMaxDegree(14), myMaxSeg(16), myProj2d(Standard_True),
      myProj3d(Standard_False), myMaxDist(theMaxDist) {
...
void ProjLib_CompProjectedCurve::Init() {
    ...
    if (CExt.IsDone() && CExt.NbExt()) {
        ...
        if (myMaxDist > 0 &&
            ...
            if (min_val2 > myMaxDist * myMaxDist) return; // No near solution -> exit.
        }
    }
        ...
        while (t <= LastU && new_part) {
            ...
            if (!aPrjPS.IsDone()) {
                if (Step <= GlobalMinStep) {
                    ...
                    if ((Triple.X() -
                         mySequence->Value(myNbCurves)->Value(mySequence->Value(myNbCurves)->Length()).X()) > 1.e-10)
                        mySequence->Value(myNbCurves)->Append(Triple);
                    ...
                    if (t > (LastU - MinStep / 4)) {
                        Step = Step + LastU - t;
                        if (Abs(Step - SaveStep) <= Precision::PConfusion()) Step = GlobalMinStep; // to avoid looping
                        t = LastU;
                    }
                }
            }
            // Go further
                ...
                if ((Triple.X() - mySequence->Value(myNbCurves)->Value(mySequence->Value(myNbCurves)->Length()).X()) >
                    1.e-10)
                    mySequence->Value(myNbCurves)->Append(Triple);
                if (t == LastU) {
                    t = LastU + 1;
    ...
    for (i = 1; i < myNbCurves; i++) {
        if (mySequence->Value(i)->Value(mySequence->Value(i)->Length()).X() >= mySequence->Value(i + 1)->Value(1).X()) {
...
void ProjLib_CompProjectedCurve::Bounds(const Standard_Integer Index, Standard_Real& Udeb, Standard_Real& Ufin) const {
...
void ProjLib_CompProjectedCurve::D0(const Standard_Real U, gp_Pnt2d& P) const {
    ...
    if (mySequence->Value(i)->Length() < 4 ||
        ...
        if (j == 1) j = 2;
        if (j > mySequence->Value(i)->Length() - 2) j = mySequence->Value(i)->Length() - 2;
    ...
    if (aPrjPS.IsDone())
        ...
        if (aExtPS.IsDone() && aExtPS.NbExt()) {
            ...
            Nend = aExtPS.NbExt();
            for (k = 2; k <= Nend; k++)
                if (aExtPS.SquareDistance(k) < aExtPS.SquareDistance(imin)) imin = k;
...
void ProjLib_CompProjectedCurve::D2(const Standard_Real t, gp_Pnt2d& P, gp_Vec2d& V1, gp_Vec2d& V2) const {
    Standard_Real u, v;
    D0(t, P);
    u = P.X();
    v = P.Y();
...
void ProjLib_CompProjectedCurve::BuildIntervals(const GeomAbs_Shape S) const {
        ...
        for (i = 1; i <= myNbCurves; i++) {
            for (j = 1; j < mySequence->Value(i)->Length(); j++) {
                ...
                if (Abs(Ul - CutPntsU(k)) <= myTolU)
                    ...
                    Solver.Perform((Tl + Tr) / 2, CutPntsU(k), V, gp_Pnt2d(Tol, myTolV),
        ...
        for (i = 1; i <= myNbCurves; i++) {
            ...
            for (j = 1; j < mySequence->Value(i)->Length(); j++) {
                ...
                if (Abs(Vl - CutPntsV(k)) <= myTolV)
                    ...
                    Solver.Perform((Tl + Tr) / 2, U, CutPntsV(k), gp_Pnt2d(Tol, myTolV),
    ...
    if (!CArr.IsNull()) {
        GeomLib::FuseIntervals(BArr->ChangeArray1(), CArr->ChangeArray1(), Fusion, Precision::PConfusion());
    ...
    if (!UArr.IsNull()) {
        GeomLib::FuseIntervals(BArr->ChangeArray1(), UArr->ChangeArray1(), Fusion, Precision::PConfusion());
        BArr = new TColStd_HArray1OfReal(1, Fusion.Length());
        for (i = 1; i <= BArr->Length(); i++) {
            BArr->ChangeValue(i) = Fusion(i);
        }
    ...
    if (!VArr.IsNull()) {
        GeomLib::FuseIntervals(BArr->ChangeArray1(), VArr->ChangeArray1(), Fusion, Precision::PConfusion());
        BArr = new TColStd_HArray1OfReal(1, Fusion.Length());
        for (i = 1; i <= BArr->Length(); i++) {
            BArr->ChangeValue(i) = Fusion(i);
        }
...
Handle(Adaptor2d_Curve2d) ProjLib_CompProjectedCurve::Trim(const Standard_Real First, const Standard_Real Last,
...
void ProjLib_CompProjectedCurve::UpdateTripleByTrapCriteria(gp_Pnt& thePoint) const {
    ...
    if (!isProblemsPossible) return;

    Standard_Real U, V;
    Standard_Boolean isDone = InitialPoint(myCurve->Value(thePoint.X()), thePoint.X(), myCurve, mySurface,
                                           Precision::PConfusion(), Precision::PConfusion(), U, V, myMaxDist);

    if (!isDone) return;

    // Restore original position in case of period jump.
    if (mySurface->IsUPeriodic() && Abs(Abs(U - thePoint.Y()) - mySurface->UPeriod()) < Precision::PConfusion()) {
        U = thePoint.Y();
    }
    if (mySurface->IsVPeriodic() && Abs(Abs(V - thePoint.Z()) - mySurface->VPeriod()) < Precision::PConfusion()) {
        V = thePoint.Z();
    }
    thePoint.SetY(U);
    thePoint.SetZ(V);
...
// purpose  :
//=======================================================================
void BuildCurveSplits(const Handle(Adaptor3d_Curve) & theCurve, const Handle(Adaptor3d_Surface) & theSurface,
                      const Standard_Real theTolU, const Standard_Real theTolV,
                      NCollection_Vector<Standard_Real>& theSplits) {
    SplitDS aDS(theCurve, theSurface, theSplits);

    Extrema_ExtPS anExtPS;
    anExtPS.Initialize(*theSurface, theSurface->FirstUParameter(), theSurface->LastUParameter(),
                       theSurface->FirstVParameter(), theSurface->LastVParameter(), theTolU, theTolV);
    aDS.myExtPS = &anExtPS;
...
void SplitOnDirection(SplitDS& theSplitDS) {
    ...
    theSplitDS.myPerMinParam =
        !theSplitDS.myPeriodicDir ? theSplitDS.mySurface->FirstUParameter() : theSplitDS.mySurface->FirstVParameter();
    theSplitDS.myPerMaxParam =
        !theSplitDS.myPeriodicDir ? theSplitDS.mySurface->LastUParameter() : theSplitDS.mySurface->LastVParameter();
...
// purpose  :
//=======================================================================
void FindSplitPoint(SplitDS& theSplitDS, const Standard_Real theMinParam, const Standard_Real theMaxParam) {
    ...
    if (anExtCC.IsDone() && !anExtCC.IsParallel()) {
        ...
        for (Standard_Integer anIdx = 1; anIdx <= aNbExt; ++anIdx) {
            ...
            if (!theSplitDS.myExtPS->IsDone()) return;
            ...
            if (Abs(aProjParam - theSplitDS.myPerMinParam) < Precision::PConfusion() ||
                ...
                if (aParam > aCFParam + Precision::PConfusion() && aParam < aCLParam - Precision::PConfusion()) {
                    // Add only inner points.
                    theSplitDS.mySplits.Append(aParam);
                }

                const Standard_Real aDeltaCoeff = 0.01;
                const Standard_Real aDelta = (theMaxParam - theMinParam + aCLParam - aCFParam) * aDeltaCoeff;
