...
//=======================================================================
static gp_Pnt OnSurface_Value(const Standard_Real U, const Handle(Adaptor3d_Curve) & myCurve, Extrema_ExtPS* myExtPS) {
...
{
public:
    ProjLib_OnSurface(const Handle(Adaptor3d_Curve) & C, const Handle(Adaptor3d_Surface) & S) : myCurve(C) {
        myNbPnt = 1;
        myNbPnt2d = 0;
...
ProjLib_ProjectOnSurface::ProjLib_ProjectOnSurface() : myTolerance(0.0), myIsDone(Standard_False) {}
...
ProjLib_ProjectOnSurface::ProjLib_ProjectOnSurface(const Handle(Adaptor3d_Surface) & S)
    : myTolerance(0.0), myIsDone(Standard_False) {
    mySurface = S;
}
...
void ProjLib_ProjectOnSurface::Load(const Handle(Adaptor3d_Curve) & C, const Standard_Real Tolerance) {
    ...
    if (!mySurface.IsNull()) {
        ...
        Approx_FitAndDivide Fit(F, Deg1, Deg2, Precision::Approximation(), Precision::PApproximation(), Standard_True);
        Standard_Integer i;
        ...
        for (i = 1; i <= Fit.NbMultiCurves(); i++) {
            ...
            if (Inc > 0) {
                BSplCLib::IncreaseDegree(Inc, LocalPoles, BSplCLib::NoWeights(), TempPoles, BSplCLib::NoWeights());
