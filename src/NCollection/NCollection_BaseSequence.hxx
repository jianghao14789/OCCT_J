...
typedef void (*NCollection_DelSeqNode)(NCollection_SeqNode*, Handle(NCollection_BaseAllocator) & theAl);
...
public:
    ...
    public:
        ...
        void Previous() {
            ...
            if (myCurrent) myPrevious = myCurrent->Previous();
        }
...
protected:
    ...
    NCollection_BaseSequence(const Handle(NCollection_BaseAllocator) & theAllocator)
        : myFirstItem(NULL), myLastItem(NULL), myCurrentItem(NULL), myCurrentIndex(0), mySize(0) {
    ...
    Standard_EXPORT void RemoveSeq(const Standard_Integer From, const Standard_Integer To, NCollection_DelSeqNode fDel);
