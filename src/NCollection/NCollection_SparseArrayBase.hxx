...
private:
    ...
    public:
        ...
        static Standard_Size BitsPerCell() {
            return sizeof(Cell) * 8 /*BITSPERBYTE*/;
        }
    ...
    public:
        ...
        Block(const Standard_Address theAddr, const Standard_Size theNbItems, const Standard_Size theItemSize)
            : Count((Standard_Size*)theAddr), Array((char*)theAddr + sizeof(Standard_Size)),
        ...
        static Standard_Size Size(const Standard_Size theNbItems, const Standard_Size theItemSize) {
            return sizeof(Standard_Size) + sizeof(Cell) * ((theNbItems + BitsPerCell() - 1) / BitsPerCell()) +
                   theNbItems * theItemSize;
        }

        //! Returns address of array from address of block
        static char* ToArray(const Standard_Address theAddress, const Standard_Size /*theNbItems*/,
...
protected:
    ...
    NCollection_SparseArrayBase(Standard_Size theItemSize, Standard_Size theBlockSize)
        : myItemSize(theItemSize), myBlockSize(theBlockSize), myNbBlocks(0), mySize(0), myData(0) {}
...
protected:
    ...
    Standard_Address getValue(const Standard_Size theIndex) const {
        Standard_OutOfRange_Raise_if(!HasValue(theIndex), "NCollection_SparseArray::Value()") return Block::ToArray(
            myData[theIndex / myBlockSize], myBlockSize, myItemSize) +
            myItemSize * (theIndex % myBlockSize);
    }

    //! Set a value to the specified item; returns address of the set item
    Standard_EXPORT Standard_Address setValue(const Standard_Size theIndex, const Standard_Address theValue);
