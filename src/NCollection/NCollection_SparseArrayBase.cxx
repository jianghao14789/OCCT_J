...
void NCollection_SparseArrayBase::allocData(const Standard_Size iBlock) {
    ...
    if (myNbBlocks > 0) memcpy(newData, myData, myNbBlocks * sizeof(Standard_Address));
...
void NCollection_SparseArrayBase::Clear() {
    ...
    for (Standard_Size iBlock = 0; iBlock < myNbBlocks; iBlock++)
        if (myData[iBlock]) freeBlock(iBlock);
    ...
    // consistency check
    Standard_ProgramError_Raise_if(mySize != 0,
...
void NCollection_SparseArrayBase::assign(const NCollection_SparseArrayBase& theOther) {
    ...
    if (myBlockSize != theOther.myBlockSize) Clear();
    ...
    for (; iBlock < theOther.myNbBlocks; iBlock++) {
        if (!theOther.myData[iBlock]) {
            ...
            if (iBlock < myNbBlocks && myData[iBlock]) freeBlock(iBlock);
            continue;
        }

        if (iBlock >= myNbBlocks) allocData(iBlock);
...
template <class T> static inline void sswap(T& a, T& b) {
    T c = a;
    a = b;
    b = c;
}
...
Standard_Address NCollection_SparseArrayBase::setValue(const Standard_Size theIndex, const Standard_Address theValue) {
    ...
    Standard_Address& anAddr = myData[iBlock];
    if (!anAddr) anAddr = calloc(Block::Size(myBlockSize, myItemSize), sizeof(char));
...
Standard_Boolean NCollection_SparseArrayBase::HasValue(const Standard_Size theIndex) const {
    Standard_Size iBlock = theIndex / myBlockSize;
    if (iBlock >= myNbBlocks || !myData[iBlock]) return Standard_False;
...
Standard_Boolean NCollection_SparseArrayBase::UnsetValue(const Standard_Size theIndex) {
    ...
    Standard_Size anInd = theIndex % myBlockSize;
    if (!aBlock.Unset(anInd)) return Standard_False;
    ...
    if (!(*aBlock.Count)) freeBlock(iBlock);
...
NCollection_SparseArrayBase::Iterator::Iterator(const NCollection_SparseArrayBase* theArray)
    : myArr((NCollection_SparseArrayBase*)theArray), myHasMore(Standard_False), myIBlock(0), myInd(0),
      myBlock(0, 0, 0) {
    init(theArray);
}
