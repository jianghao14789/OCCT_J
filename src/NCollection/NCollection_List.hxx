 ...
 */
template <class TheItemType> class NCollection_List : public NCollection_BaseList {
public:
...
public:
    ...
    //! Copy constructor
    NCollection_List(const NCollection_List& theOther) : NCollection_BaseList(theOther.myAllocator) {
        Assign(theOther);
    }
    ...
    void Append(NCollection_List& theOther) {
        if (this == &theOther || theOther.Extent() < 1) return;
...
private:
    ...
    void appendList(const NCollection_ListNode* pCur) {
        while (pCur) {
            NCollection_ListNode* pNew = new (this->myAllocator) ListNode(((const ListNode*)(pCur))->Value());
            PAppend(pNew);
            pCur = pCur->Next();
        }
    }
