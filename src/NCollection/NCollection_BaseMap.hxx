...
public:
    ...
    protected:
        //! Empty constructor
        Iterator(void) : myNbBuckets(0), myBuckets(NULL), myBucket(0), myNode(NULL) {}

        //! Constructor
        Iterator(const NCollection_BaseMap& theMap)
            : myNbBuckets(theMap.myNbBuckets), myBuckets(theMap.myData1), myBucket(-1), myNode(NULL) {
            if (!myBuckets)
                myNbBuckets = -1;
            else
                do {
...
protected:
    ...
    NCollection_BaseMap(const Standard_Integer NbBuckets, const Standard_Boolean single,
        ...
        : myData1(NULL), myData2(NULL), myNbBuckets(NbBuckets), mySize(0), isDouble(!single) {
    ...
    //! BeginResize
    Standard_EXPORT Standard_Boolean BeginResize(const Standard_Integer NbBuckets, Standard_Integer& NewBuckets,
                                                 NCollection_ListNode**& data1, NCollection_ListNode**& data2) const;

    //! EndResize
    Standard_EXPORT void EndResize(const Standard_Integer NbBuckets, const Standard_Integer NewBuckets,
    ...
    //! Destroy
    Standard_EXPORT void Destroy(NCollection_DelMapNode fDel, Standard_Boolean doReleaseMemory = Standard_True);
