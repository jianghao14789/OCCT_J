...
//! For instance, this class allows defining an array of 32-bit or 64-bit integer values with bitness determined in
//! runtime. The element size in bytes (stride) should be specified at construction time. Indexation starts from 0
//! index. As actual type of element varies at runtime, element accessors are defined as templates. Memory for array is
//! allocated with the given alignment (template parameter).
template <int MyAlignSize = 16> class NCollection_AliasedArray {
public:
...
public:
    ...
    NCollection_AliasedArray(NCollection_AliasedArray&& theOther)
        : myData(theOther.myData), myStride(theOther.myStride), mySize(theOther.mySize),
    ...
    Standard_Size SizeBytes() const {
        return size_t(myStride) * size_t(mySize);
    }
...
public:
    ...
    //! Access element with specified position and type.
    //! This method allows wrapping element into smaller type (e.g. to alias 2-components within 3-component vector).
    template <typename Type_t> Type_t& ChangeValue2(Standard_Integer theIndex) {
        Standard_TypeMismatch_Raise_if(size_t(myStride) < sizeof(Type_t),
    ...
    template <typename Type_t> const Type_t& First() const {
        return Value<Type_t>(0);
    }

    //! Return first element
    template <typename Type_t> Type_t& ChangeFirst() {
