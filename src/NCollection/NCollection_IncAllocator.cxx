...
static void Debug_Create(Standard_Address theAlloc) {
    ...
    if (CurrentID == CATCH_ID) place_for_breakpoint();
    aMutex.Unlock();
}
...
NCollection_IncAllocator::NCollection_IncAllocator(size_t theBlockSize) : myMutex(NULL) {
    ...
    if (aBlock == NULL) throw Standard_OutOfMemory("NCollection_IncAllocator: out of memory");
...
NCollection_IncAllocator::~NCollection_IncAllocator() {
    delete myMutex;
#ifdef OCCT_DEBUG
    if (IS_DEBUG) Debug_Destroy(this);
#endif
    Clean();
    free(myFirstBlock);
}
...
void* NCollection_IncAllocator::Reallocate(void* theAddress, const size_t oldSize, const size_t newSize) {
    ...
    if (theAddress == NULL || oldSize == 0) return Allocate(newSize);
