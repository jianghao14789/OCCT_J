...
public:
    ...
    NCollection_StlIterator(const BaseIterator& theIterator) : myIterator(theIterator) {}
    ...
    NCollection_StlIterator&
    operator=(const NCollection_StlIterator<Category, BaseIterator, ItemType, false>& theIterator) {
...
protected: //! @name methods related to forward STL iterator
    ...
    template <bool Condition> typename opencascade::std::enable_if<!Condition, ItemType&>::type Reference() const {
        return myIterator.ChangeValue();
    }

    template <bool Condition> typename opencascade::std::enable_if<Condition, const ItemType&>::type Reference() const {
...
public: //! @name methods related to random access STL iterator
    ...
    typename NCollection_StlIterator::reference
    operator[](typename NCollection_StlIterator::difference_type theOffset) const {
        return *(*this + theOffset);
    }
