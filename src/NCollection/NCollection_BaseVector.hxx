...
protected:
    ...
    class Iterator {
    protected:
        Iterator() : myICurBlock(0), myIEndBlock(0), myCurIndex(0), myEndIndex(0) {}
        ...
        void nextV() {
            if (++myCurIndex >= myVector->myData[myICurBlock].Length && myICurBlock < myIEndBlock) {
                ++myICurBlock;
                myCurIndex = 0;
            }
        }
...
protected: //! @name Block initializer
    typedef void (*initMemBlocks_t)(NCollection_BaseVector& theVector, MemBlock& theBlock,
    ...
    Standard_EXPORT MemBlock* allocMemBlocks(const Standard_Integer theCapacity, MemBlock* theSource = NULL,
                                             const Standard_Integer theSourceSize = 0);

protected: //! @name protected methods
    //! Empty constructor
    NCollection_BaseVector(const Handle(NCollection_BaseAllocator) & theAllocator, initMemBlocks_t theInitBlocks,
                           const size_t theSize, const Standard_Integer theInc)
        : myItemSize(theSize), myIncrement(theInc), myLength(0), myCapacity(GetCapacity(myIncrement)), myNBlocks(0),
    ...
    NCollection_BaseVector(const Handle(NCollection_BaseAllocator) & theAllocator, initMemBlocks_t theInitBlocks,
        ...
        : myItemSize(theOther.myItemSize), myIncrement(theOther.myIncrement), myLength(theOther.myLength),
    ...
    inline void* findV(const Standard_Integer theIndex) const {
        Standard_OutOfRange_Raise_if(theIndex < 0 || theIndex >= myLength, "NCollection_BaseVector::findV");
