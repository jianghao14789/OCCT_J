...
public:
    explicit NCollection_LocalArray(const size_t theSize) : myPtr(myBuffer) {
        Allocate(theSize);
    }

    NCollection_LocalArray() : myPtr(myBuffer), mySize(0) {}
...
protected:
    void Deallocate() {
        if (myPtr != myBuffer) Standard::Free(myPtr);
    }
