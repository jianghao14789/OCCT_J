...
void TDF_ClosureTool::Closure(const Handle(TDF_DataSet) & aDataSet, const TDF_IDFilter& aFilter,
    ...
    for (; labLItr.More(); labLItr.Next()) {
        ...
        if (lab.HasAttribute()) TDF_ClosureTool::LabelAttributes(lab, labMap, attMap, aFilter, aMode);
        if (aMode.Descendants()) TDF_ClosureTool::Closure(lab, labMap, attMap, aFilter, aMode);
    }
}
...
void TDF_ClosureTool::Closure(const TDF_Label& aLabel, TDF_LabelMap& aLabMap, TDF_AttributeMap& anAttMap,
                              const TDF_IDFilter& aFilter, const TDF_ClosureMode& aMode) {
    TDF_Label upLab;
...
void TDF_ClosureTool::LabelAttributes(const TDF_Label& aLabel, TDF_LabelMap& aLabMap, TDF_AttributeMap& anAttMap,
                                      const TDF_IDFilter& aFilter, const TDF_ClosureMode& aMode) {
            ...
            if (anAttMap.Add(locAtt1)) {
                ...
                if (aMode.References()) {
                    ...
                    for (labMItr.Initialize(tmpLabMap); labMItr.More(); labMItr.Next()) {
                        ...
                        if (aLabMap.Add(locLab1)) TDF_ClosureTool::Closure(locLab1, aLabMap, anAttMap, aFilter, aMode);
                    }
                }
            }
        }
