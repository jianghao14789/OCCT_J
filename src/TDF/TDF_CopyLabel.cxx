...
// purpose  :
//=======================================================================
TDF_CopyLabel::TDF_CopyLabel() : myFilter(Standard_False), myIsDone(Standard_False) {
    mySL.Nullify();
    myTL.Nullify();
}
...
void TDF_CopyLabel::ExternalReferences(const TDF_Label& aRefLabel, const TDF_Label& aLabel,
    ...
    for (TDF_AttributeIterator itr(aLabel); itr.More(); itr.Next()) {
        ...
        //     std::cout<<"\tSource Attribute dynamic type = "<<itr.Value()->DynamicType()<<" Label = "<<entr1
        //     <<std::endl;
        for (TDF_MapIteratorOfAttributeMap attMItr(attMap); attMItr.More(); attMItr.Next()) {
            ...
            //       std::cout<<"\t\tReferences attribute dynamic type = "<<att->DynamicType()<<" Label = "<<entr1
            //       <<std::endl;
...
Standard_Boolean TDF_CopyLabel::ExternalReferences(const TDF_Label& L, TDF_AttributeMap& aExternals,
...
void TDF_CopyLabel::Perform() {
    ...
    if (mySL.Data()->Root().IsDifferent(myTL.Data()->Root()) && // TDF_Data is not the same
        ...
        return; // source label isn't self-contained
