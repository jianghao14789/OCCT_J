...
void TDF_CopyTool::Copy(const Handle(TDF_DataSet) & aSourceDataSet,
                        const Handle(TDF_RelocationTable) & aRelocationTable, const TDF_IDFilter& aPrivilegeFilter) {
    TDF_IDFilter refFilter; // Will not be used.
...
void TDF_CopyTool::Copy(const Handle(TDF_DataSet) & aSourceDataSet,
                        const Handle(TDF_RelocationTable) & aRelocationTable, const TDF_IDFilter& aPrivilegeFilter,
                        const TDF_IDFilter& /* aRefFilter */, const Standard_Boolean /* setSelfContained */) {
    ...
    for (; attItr2.More(); attItr2.Next()) {
        ...
        if (!sAtt.IsNull()) { // This condition looks superfluous; and below also.
            ...
            if ((sAtt != tAtt) && aPrivilegeFilter.IsIgnored(tAtt->ID())) sAtt->Paste(tAtt, aRelocationTable);
        }
    }
}
...
void TDF_CopyTool::CopyLabels(const TDF_Label& aSLabel, TDF_Label& aTargetLabel, TDF_LabelDataMap& aLabMap,
                              TDF_AttributeDataMap& aAttMap, const TDF_LabelMap& aSrcLabelMap,
    ...
    for (TDF_ChildIterator childItr(aSLabel); childItr.More(); childItr.Next()) {
        ...
        if (aSrcLabelMap.Contains(childSLab)) {
            ...
            TDF_CopyTool::CopyLabels(childSLab, childTIns, aLabMap, aAttMap, aSrcLabelMap, aSrcAttributeMap);
        }
    }
}
...
void TDF_CopyTool::CopyAttributes(const TDF_Label& aSLabel, TDF_Label& aTargetLabel, TDF_AttributeDataMap& aAttMap,
                                  const TDF_AttributeMap& aSrcAttributeMap) {
