...
//=======================================================================
void TDF_ComparisonTool::Compare(const Handle(TDF_DataSet) & aSourceDataSet, const Handle(TDF_DataSet) & aTargetDataSet,
                                 const TDF_IDFilter& aFilter, const Handle(TDF_RelocationTable) & aRelocationTable) {
    ...
    for (; srcRootItr.More(); srcRootItr.Next()) {
        ...
        for (trgRootItr.Initialize(trgRoots); trgRootItr.More(); trgRootItr.Next()) {
            ...
            if (srcLab.Tag() == trgLab.Tag()) {
                ...
                TDF_ComparisonTool::Compare(srcLab, trgLab, aSourceDataSet, aTargetDataSet, aFilter, aRelocationTable);
                break;
            }
        }
    }
...
void TDF_ComparisonTool::Compare(const TDF_Label& aSrcLabel, const TDF_Label& aTrgLabel,
                                 const Handle(TDF_DataSet) & aSourceDataSet, const Handle(TDF_DataSet) & aTargetDataSet,
                                 const TDF_IDFilter& aFilter, const Handle(TDF_RelocationTable) & aRelocationTable) {
    TDF_LabelDataMap& the2LabMap = aRelocationTable->LabelTable();
    ...
    for (TDF_AttributeIterator attItr(aSrcLabel); attItr.More(); attItr.Next()) {
        ...
        if (aFilter.IsKept(sAtt) && aSourceDataSet->ContainsAttribute(sAtt)) {
            if (aTrgLabel.FindAttribute(sAtt->ID(), tAtt)) {
                if (aTargetDataSet->ContainsAttribute(tAtt)) the2AttMap.Bind(sAtt, tAtt);
            }
        }
    }
            ...
            for (childItr2.Initialize(aSrcLabel); childItr2.More(); childItr2.Next()) {
                ...
                if (aTargetDataSet->ContainsLabel(childTrgLab)) {
                    if (childSrcLab.Tag() == childTrgLab.Tag()) {
                        ...
                        TDF_ComparisonTool::Compare(childSrcLab, childTrgLab, aSourceDataSet, aTargetDataSet, aFilter,
                                                    aRelocationTable);
                        break;
                    }
                }
...
Standard_Boolean TDF_ComparisonTool::SourceUnbound(const Handle(TDF_DataSet) & aRefDataSet,
...
Standard_Boolean TDF_ComparisonTool::TargetUnbound(const Handle(TDF_DataSet) & aRefDataSet,
                                                   const Handle(TDF_RelocationTable) & aRelocationTable,
                                                   const TDF_IDFilter& aFilter,
                                                   const Handle(TDF_DataSet) & aDiffDataSet,
                                                   const Standard_Integer anOption) {
...
Standard_Boolean TDF_ComparisonTool::Unbound(const Handle(TDF_DataSet) & aRefDataSet,
                                             const Handle(TDF_RelocationTable) & aRelocationTable,
                                             const TDF_IDFilter& aFilter, const Handle(TDF_DataSet) & aDiffDataSet,
                                             const Standard_Integer anOption, const Standard_Boolean theSource) {
    ...
    if ((anOption & 1) != 0) {
        ...
        for (TDF_MapIteratorOfLabelMap refLabMItr(refLabs); refLabMItr.More(); refLabMItr.Next()) {
            ...
            if (!(theSource ? the2LabMap.IsBound(refLab) : theTLabMap.Contains(refLab))) diffLabs.Add(refLab);
        }
    ...
    if ((anOption & 2) != 0) {
        ...
        for (TDF_MapIteratorOfAttributeMap refAttMItr(refAtts); refAttMItr.More(); refAttMItr.Next()) {
            ...
            if (aFilter.IsKept(refAtt)) {
                if (!(theSource ? the2AttMap.IsBound(refAtt) : theTAttMap.Contains(refAtt))) diffAtts.Add(refAtt);
            }
        }
...
Standard_Boolean TDF_ComparisonTool::IsSelfContained(const TDF_Label& aLabel, const Handle(TDF_DataSet) & aDataSet) {
    if (!aDataSet->IsEmpty()) {
