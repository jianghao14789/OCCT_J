...
#define TDF_Data_DebugModified(ACTION)                                                                                 \
    std::cout << "After " << ACTION << " #" << myTransaction + 1 << ", DF " << this << " had " << myNbTouchedAtt       \
              << " attribute(s) touched. Time = " << myTime << std::endl;                                              \
    if (!myTransaction) {                                                                                              \
        ...
        for (TDF_ChildIterator itr(Root(), Standard_True); itr.More(); itr.Next()) {                                   \
            ...
            if (lnp->AttributesModified() || lnp->MayBeModified()) {                                                   \
                ...
                if (lnp->MayBeModified())                                                                              \
                    std::cout << "MayBeModified already set in transaction 0! Please contact TDF developer.";          \
                std::cout << std::endl;                                                                                \
...
TDF_Data::TDF_Data()
    : myTransaction(0), myNbTouchedAtt(0), myNotUndoMode(Standard_True), myTime(0), myAllowModification(Standard_True),
    ...
    if (myTransaction > 0) {
        ...
        if (withDelta) {
            if (!delta->IsEmpty()) {
                ...
                if (myTransaction == 0) std::cout << "TDF_Data::Commit generated NO delta." << std::endl;
            }
#endif
        }
        myTimes.RemoveFirst();
...
Standard_Integer TDF_Data::CommitTransaction(const TDF_Label& aLabel, const Handle(TDF_Delta) & aDelta,
                                             const Standard_Boolean withDelta) {
        ...
        while (itr1.More()) {
            ...
            if (aPtrCurrentAtt->myTransaction == myTransaction) {
                ...
                // --------------------------------------------------------- Modified.
                    ...
                    if (aPtrCurrentAtt->myTransaction == backupAtt->myTransaction) aPtrCurrentAtt->RemoveBackup();
                    attMod = attMod || (aPtrCurrentAtt->myTransaction > 0);
                }
...
void TDF_Data::AbortTransaction() {
    if (myTransaction > 0) Undo(TDF_Data::CommitTransaction(Standard_True), Standard_False);
...
void TDF_Data::AbortUntilTransaction(const Standard_Integer untilTransaction) {
    if (untilTransaction > 0) Undo(TDF_Data::CommitUntilTransaction(untilTransaction, Standard_True), Standard_False);
}
...
#ifdef OCCT_DEBUG_DELTA
                std::cout << "TDF_Data::Undo, after validity correction, Delta is now available from time \t#"
