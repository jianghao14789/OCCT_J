...
TDF_Attribute::TDF_Attribute()
    : myLabelNode(NULL), myTransaction(0), mySavedTransaction(0), myFlags(1)
...
Standard_Boolean TDF_Attribute::FindAttribute(const Standard_GUID& anID, Handle(TDF_Attribute) & anAttribute) const {
...
void TDF_Attribute::Forget(const Standard_Integer aTransaction) {
    ...
    if (myLabelNode) myLabelNode->AttributesModified(Standard_True);
#endif
...
#ifdef OCCT_DEBUG
    if (myBackup.IsNull()) throw Standard_DomainError("Impossible to remove a nonexistent backup.");
#endif
...
Handle(TDF_DeltaOnModification) TDF_Attribute::DeltaOnModification(const Handle(TDF_Attribute) & anOldAttribute) const {
...
void TDF_Attribute::ExtendedDump(Standard_OStream& anOS, const TDF_IDFilter& /*aFilter*/,
