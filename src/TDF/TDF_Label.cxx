...
Standard_Boolean TDF_Label::FindAttribute(const Standard_GUID& anID, Handle(TDF_Attribute) & anAttribute) const {
...
Standard_Boolean TDF_Label::FindAttribute(const Standard_GUID& anID, const Standard_Integer aTransaction,
...
#ifdef OCCT_DEBUG
    if ((lp1 == NULL) || (lp2 == NULL)) throw Standard_NullObject("A null label has no ancestor nor descendant.");
#endif
...
TDF_Label TDF_Label::FindChild(const Standard_Integer aTag, const Standard_Boolean create) const {
    if (IsNull()) throw Standard_NullObject("A null Label has no child.");
    if (create && ((Depth() + 1) & TDF_LabelNodeFlagsMsk)) throw Standard_OutOfRange("Depth value out of range");
...
void TDF_Label::ExtendedDump(Standard_OStream& anOS, const TDF_IDFilter& aFilter, TDF_AttributeIndexedMap& aMap) const {
...
TDF_LabelNode* TDF_Label::FindOrAddChild(const Standard_Integer aTag, const Standard_Boolean create) const {
    ...
    if ((currentLnp != NULL) && (currentLnp->Tag() == aTag)) {
        ...
        if (myLabelNode->Data()->IsAccessByEntries()) myLabelNode->Data()->RegisterLabel(childLabelNode);
    }
...
void TDF_Label::InternalDump(Standard_OStream& anOS, const TDF_IDFilter& aFilter, TDF_AttributeIndexedMap& aMap,
...
void TDF_Label::AddToNode(const TDF_LabelNodePtr& toNode, const Handle(TDF_Attribute) & anAttribute,
    ...
    if (!anAttribute->Label().IsNull()) throw Standard_DomainError("Attribute to add is already attached to a label.");
...
void TDF_Label::ForgetFromNode(const TDF_LabelNodePtr& fromNode, const Handle(TDF_Attribute) & anAttribute) const {
    // check that modification is allowed
    ...
    if (!anAttribute->IsForgotten()) {
        if ((curTrans == 0) || ((anAttribute->myTransaction == curTrans) && anAttribute->myBackup.IsNull())) {
...
void TDF_Label::ResumeToNode(const TDF_LabelNodePtr& toNode, const Handle(TDF_Attribute) & anAttribute) const {
    if (anAttribute.IsNull()) throw Standard_NullObject("The attribute is a null handle.");
    if (!anAttribute->Label().IsNull())
        throw Standard_NullObject("Cannot resume an attribute already attached to a label.");
    if (!anAttribute->IsForgotten()) throw Standard_DomainError("Cannot resume an unforgotten attribute.");
