...
static void TDF_Tool_ExtendedDeepDump(Standard_OStream& anOS, const TDF_Label& aLabel, const TDF_IDFilter& aFilter,
                                      TDF_AttributeIndexedMap& aMap);

static Standard_Boolean TDF_Tool_DescendantRef(const TDF_Label& aRefLabel, const TDF_Label& aLabel,
                                               const TDF_IDFilter& aFilter, const Handle(TDF_DataSet) & ds);

static void TDF_Tool_OutReferers(const TDF_Label& aRefLabel, const TDF_Label& aLabel, TDF_AttributeMap& atts,
                                 const TDF_IDFilter& aFilterForReferers, const TDF_IDFilter& aFilterForReferences,
                                 const Handle(TDF_DataSet) & ds);

static void TDF_Tool_OutReferences(const TDF_Label& aRefLabel, const TDF_Label& aLabel, TDF_AttributeMap& atts,
                                   const TDF_IDFilter& aFilterForReferers, const TDF_IDFilter& aFilterForReferences,
                                   const Handle(TDF_DataSet) & ds);
...
Standard_Boolean TDF_Tool::IsSelfContained(const TDF_Label& aLabel, const TDF_IDFilter& aFilter) {
    ...
    if (!TDF_Tool_DescendantRef(aLabel, aLabel, aFilter, ds)) return Standard_False;

    for (TDF_ChildIterator itr(aLabel, Standard_True); itr.More(); itr.Next()) {
        if (!TDF_Tool_DescendantRef(aLabel, itr.Value(), aFilter, ds)) return Standard_False;
    }
    ...
    for (TDF_AttributeIterator itr(aLabel); itr.More(); itr.Next()) {
        ...
        if (aFilter.IsKept(labAtt)) {
            ...
            for (TDF_MapIteratorOfLabelMap labMItr(labMap); labMItr.More(); labMItr.Next()) {
                if (!labMItr.Key().IsDescendant(aRefLabel)) return Standard_False;
            }
            ...
            for (TDF_MapIteratorOfAttributeMap attMItr(attMap); attMItr.More(); attMItr.Next()) {
                ...
                if (!att.IsNull() && !att->Label().IsNull()) {
                    // ENDCLE
                    if (aFilter.IsKept(att) && !att->Label().IsDescendant(aRefLabel)) return Standard_False;
                }
            }
            ds->Clear();
        }
...
void TDF_Tool::OutReferers(const TDF_Label& aLabel, const TDF_IDFilter& aFilterForReferers,
                           const TDF_IDFilter& aFilterForReferences, TDF_AttributeMap& atts) {
    Handle(TDF_DataSet) ds = new TDF_DataSet();
...
static void TDF_Tool_OutReferers(const TDF_Label& aRefLabel, const TDF_Label& aLabel, TDF_AttributeMap& atts,
    ...
    for (TDF_AttributeIterator itr(aLabel); itr.More(); itr.Next()) {
        ...
        for (TDF_MapIteratorOfAttributeMap attMItr(attMap); attMItr.More(); attMItr.Next()) {
            ...
            Handle(TDF_Attribute) att = attMItr.Key();
            // ENDCLE
            if (aFilterForReferences.IsKept(att) && !att->Label().IsNull() && !att->Label().IsDescendant(aRefLabel)) {
                atts.Add(itr.Value());
...
void TDF_Tool::OutReferences(const TDF_Label& aLabel, const TDF_IDFilter& aFilterForReferers,
                             const TDF_IDFilter& aFilterForReferences, TDF_AttributeMap& atts) {
    Handle(TDF_DataSet) ds = new TDF_DataSet();
...
void TDF_Tool::RelocateLabel(const TDF_Label& aSourceLabel, const TDF_Label& fromRoot, const TDF_Label& toRoot,
                             TDF_Label& aTargetLabel, const Standard_Boolean create) {
...
void TDF_Tool::Label(const Handle(TDF_Data) & aDF, const TCollection_AsciiString& anEntry, TDF_Label& aLabel,
                     const Standard_Boolean create) {
    Standard_Boolean isFound = Standard_False;
    if (aDF->IsAccessByEntries()) isFound = aDF->GetLabel(anEntry, aLabel);

    if (!isFound) TDF_Tool::Label(aDF, anEntry.ToCString(), aLabel, create);
}
...
void TDF_Tool::Label(const Handle(TDF_Data) & aDF, const Standard_CString anEntry, TDF_Label& aLabel,
                     const Standard_Boolean create) {
    Standard_Boolean isFound = Standard_False;
    if (aDF->IsAccessByEntries()) isFound = aDF->GetLabel(anEntry, aLabel);
...
void TDF_Tool::Label(const Handle(TDF_Data) & aDF, const TColStd_ListOfInteger& aTagList, TDF_Label& aLabel,
                     const Standard_Boolean create) {
...
void TDF_Tool::ExtendedDeepDump(Standard_OStream& anOS, const Handle(TDF_Data) & aDF, const TDF_IDFilter& aFilter) {
    anOS << aDF;
...
void TDF_Tool::ExtendedDeepDump(Standard_OStream& anOS, const TDF_Label& aLabel, const TDF_IDFilter& aFilter) {
    TDF_AttributeIndexedMap map;
    TDF_Tool_ExtendedDeepDump(anOS, aLabel, aFilter, map);

    anOS << map.Extent() << " attribute";
    if (map.Extent() > 1) anOS << "s";
