...
template <class Coll> void printCollection(Coll& aColl, const char* str) {
    printf("%s:\n", str);
...
template <class Coll> void AssignCollection(Coll& aCollSrc, Coll& aCollDst) {
...
static void TestList(QANCollection_ListFunc& theL) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (anI.More()) {
            ...
            theL.Remove(anI);                          // #9
            if (theL.Extent() > 0) theL.RemoveFirst(); // #10
        } else {
            theL.Prepend(anItem);
...
static void TestSequence(QANCollection_SequenceFunc& theS) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (theS.Length() > 0) {
            ...
            theS.Remove(1);                        // #9
            if (theS.Length() > 0) theS.Remove(1); // #10
        } else {
            theS.Prepend(anItem);
...
static void TestMap(QANCollection_MapFunc& theM, Draw_Interpretor& theDI) {
    ...
    // Check method 'HasIntersection'.
    {
        ...
        if (!aM1.HasIntersection(aM2) || !aM2.HasIntersection(aM1) || aM1.HasIntersection(aM3) ||
            aM3.HasIntersection(aM1)) {
...
static void TestDoubleMap(QANCollection_DoubleMapFunc& theM) {
    ...
    if (!aM.AreBound(aKey1, aKey2)) {
        ...
        if (aM.UnBind2(aKey2)) printf("      successfully unbound the key, l=%d\n", aM.Size());
        if (aM.UnBind1(aKey1)) printf("Error   : unbound both keys?!\n");
    }
    // Copy constructor (including operator=)
...
// purpose  :
//=======================================================================
Standard_Integer CheckArguments1(Draw_Interpretor& di, Standard_Integer argc, const char** argv,
...
// purpose  :
//=======================================================================
Standard_Integer CheckArguments2(Draw_Interpretor& di, Standard_Integer argc, const char** argv,
                                 Standard_Integer& LowerRow, Standard_Integer& UpperRow, Standard_Integer& LowerCol,
...
static Standard_Integer QANColTestVector(Draw_Interpretor&, Standard_Integer, const char**) {
    ...
    aVec.Append(5);
    if (aVec(0) != 5) std::cout << "Error: wrong value in original vector!" << std::endl;
    aVec2.Append(5);
    if (aVec2(0) != 5) std::cout << "Error: wrong value in copy-constructed vector!" << std::endl;
    aVec3.Append(5);
    if (aVec3(0) != 5) std::cout << "Error: wrong value in copied vector!" << std::endl;
...
static Standard_Integer QATestAtof(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (aRangeMin < aRangeMax) {
        ...
        const uint64_t aMaxUInt64 =
            ~(uint64_t)0; // could be (not supported by old GCC): std::numeric_limits<uint64_t>::max()
...
#define ISFINITE std::isfinite
    ...
    for (int i = 0; i < aNbToTest; i++) {
        ...
        if (anEndOCCT != anEndStd) {
            nbErr++;
            if (nbErr < 5)
                di << "Error: different number of symbols parsed in " << aStr << ": " << (int)(anEndOCCT - aStr)
                   << " / " << (int)(anEndStd - aStr) << "\n";
        }
#endif
    }
...
#define PRINT_RES(method)                                                                                              \
    di << #method "\t" << aT_##method.UserTimeCPU() << "  \t" << aT_##method.ElapsedTime() << "\t" << aNbErr_##method  \
       << "\t" << aRes_##method << "\n"
    PRINT_RES(sscanf);
    PRINT_RES(strtod);
    PRINT_RES(atof);
...
static Standard_Integer QANColTestVec4(Draw_Interpretor& theDI, Standard_Integer /*theNbArgs*/,
    ...
    for (int aX = 0; aX < 2; ++aX) {
        for (int aY = 0; aY < 2; ++aY) {
            for (int aZ = 0; aZ < 2; ++aZ) {
                aPoints1[aX * 2 * 2 + aY * 2 + aZ] = NCollection_Vec4<float>(
                    -1.0f + 2.0f * float(aX), -1.0f + 2.0f * float(aY), -1.0f + 2.0f * float(aZ), 1.0f);
            }
        }
    }
...
void QANCollection::CommandsTest(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("QANColTestArray1", "QANColTestArray1 Lower Upper", __FILE__, QANColTestArray1, group);
    theCommands.Add("QANColTestArray2", "QANColTestArray2 LowerRow UpperRow LowerCol UpperCol", __FILE__,
                    QANColTestArray2, group);
    ...
    theCommands.Add("QANColTestArrayMove", "QANColTestArrayMove (is expected to give error)", __FILE__,
                    QANColTestArrayMove, group);
    ...
    theCommands.Add("QAOsdPathType", "QAOsdPathType path : Print file path flags deduced from path string", __FILE__,
                    QAOsdPathType, group);
    theCommands.Add("QAOsdPathPart", "QAOsdPathPart path [-folder][-fileName] : Print file path part", __FILE__,
                    QAOsdPathPart, group);
}
