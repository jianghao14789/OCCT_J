...
void IGESDraw_ToolConnectPoint::ReadOwnParams(const Handle(IGESDraw_ConnectPoint) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "Display Symbol Geometry Entity", tempDisplaySymbol,
    ...
    PR.ReadEntity(IR, PR.Current(), "Text Display Identifier Template", STANDARD_TYPE(IGESGraph_TextDisplayTemplate),
    ...
    PR.ReadEntity(IR, PR.Current(), "Text Display Function Template", STANDARD_TYPE(IGESGraph_TextDisplayTemplate),
    ...
    PR.ReadEntity(IR, PR.Current(), "Owner Network Subfigure Entity", tempOwnerSubfigure,
                  Standard_True); // szv#4:S4163:12Mar99 `st=` not needed

    DirChecker(ent).CheckTypeAndForm(PR.CCheck(), ent);
    ent->Init(tempPoint, tempDisplaySymbol, tempTypeFlag, tempFunctionFlag, tempFunctionIdentifier,
...
void IGESDraw_ToolConnectPoint::OwnCopy(const Handle(IGESDraw_ConnectPoint) & another,
    ...
    DeclareAndCast(IGESGraph_TextDisplayTemplate, tempIdentifierTemplate, TC.Transferred(another->FunctionTemplate()));
    Handle(TCollection_HAsciiString) tempFunctionName = new TCollection_HAsciiString(another->FunctionName());
    DeclareAndCast(IGESGraph_TextDisplayTemplate, tempFunctionTemplate, TC.Transferred(another->FunctionTemplate()));
    ...
    DeclareAndCast(IGESData_IGESEntity, tempOwnerSubfigure, TC.Transferred(another->OwnerSubfigure()));
...
IGESData_DirChecker IGESDraw_ToolConnectPoint::DirChecker(const Handle(IGESDraw_ConnectPoint) & ent) const {
...
void IGESDraw_ToolConnectPoint::OwnCheck(const Handle(IGESDraw_ConnectPoint) & ent, const Interface_ShareTool&,
                                         Handle(Interface_Check) & ach) const {
    if ((ent->TypeFlag() < 0) || (ent->TypeFlag() > 9999) || ((ent->TypeFlag() > 2) && (ent->TypeFlag() < 101)) ||
        ((ent->TypeFlag() > 104) && (ent->TypeFlag() < 201)) || ((ent->TypeFlag() > 203) && (ent->TypeFlag() < 5001)))
        ach->AddFail("TypeFlag has Invalid value");

    if ((ent->FunctionFlag() < 0) || (ent->FunctionFlag() > 2)) ach->AddFail("FunctionFlag has Invalid value");
    ...
    if ((ent->SwapFlag() != 0) && (ent->SwapFlag() != 1)) ach->AddFail("SwapFlag has Invalid value");
}

void IGESDraw_ToolConnectPoint::OwnDump(const Handle(IGESDraw_ConnectPoint) & ent, const IGESData_IGESDumper& dumper,
    ...
    S << "\n"
      ...
      << "Function Code : " << ent->FunctionCode() << "Swap Flag : " << (ent->SwapFlag() ? "True" : "False") << "\n"
