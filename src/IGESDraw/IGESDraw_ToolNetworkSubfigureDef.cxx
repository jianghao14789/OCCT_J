...
void IGESDraw_ToolNetworkSubfigureDef::ReadOwnParams(const Handle(IGESDraw_NetworkSubfigureDef) & ent,
    ...
    if (PR.ReadInteger(PR.Current(), "Number Of Child Entities", tempNbEntities1)) {
        ...
        if (tempNbEntities1 < 0)
            ...
            PR.ReadEnts(IR, PR.CurrentList(tempNbEntities1), "Child Entities",
                        tempEntities); // szv#4:S4163:12Mar99 `st=` not needed
    ...
    */
    ...
    if (PR.DefinedElseSkip())
        PR.ReadText(PR.Current(), "Primary Reference Designator",
                    tempDesignator); // szv#4:S4163:12Mar99 `st=` not needed
    else
    ...
    Standard_Boolean st = PR.ReadEntity(IR, PR.Current(), "Primary Reference Designator",
                                        STANDARD_TYPE(IGESGraph_TextDisplayTemplate), tempTemplate, Standard_True);
    ...
    if (!tempPointEntities.IsNull()) {
        ...
        for (I = 1; I <= tempNbEntities2; I++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "Associated Connect Point Entity", STANDARD_TYPE(IGESDraw_ConnectPoint),
    ...
    ent->Init(tempDepth, tempName, tempEntities, tempTypeFlag, tempDesignator, tempTemplate, tempPointEntities);
}
...
void IGESDraw_ToolNetworkSubfigureDef::OwnCopy(const Handle(IGESDraw_NetworkSubfigureDef) & another,
    ...
    if (!another->Designator().IsNull()) tempDesignator = new TCollection_HAsciiString(another->Designator());
    up = another->NbPointEntities();
    Handle(IGESDraw_HArray1OfConnectPoint) tempPointEntities;
    if (up > 0) tempPointEntities = new IGESDraw_HArray1OfConnectPoint(1, up);
    for (I = 1; I <= up; I++) {
        if (another->HasPointEntity(I)) {
            DeclareAndCast(IGESDraw_ConnectPoint, tempPointEntity, TC.Transferred(another->PointEntity(I)));
    ...
    if (another->HasDesignatorTemplate()) {
        ...
        ent->Init(tempDepth, tempName, tempEntities, tempTypeFlag, tempDesignator, tempDesignatorTemplate,
                  tempPointEntities);
    } else {
...
IGESData_DirChecker
IGESDraw_ToolNetworkSubfigureDef::DirChecker(const Handle(IGESDraw_NetworkSubfigureDef) & /*ent*/) const {
...
void IGESDraw_ToolNetworkSubfigureDef::OwnCheck(const Handle(IGESDraw_NetworkSubfigureDef) & ent,
    ...
    if ((ent->TypeFlag() < 0) || (ent->TypeFlag() > 2)) ach->AddFail("TypeFlag has Invalid value");
    if (ent->Designator().IsNull()) ach->AddFail("Primary Reference Designator : not defined");
}
