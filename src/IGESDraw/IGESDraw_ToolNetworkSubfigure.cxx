...
void IGESDraw_ToolNetworkSubfigure::ReadOwnParams(const Handle(IGESDraw_NetworkSubfigure) & ent,
    ...
    PR.ReadEntity(IR, PR.Current(), "Instance of NetworkSubfigureDef", STANDARD_TYPE(IGESDraw_NetworkSubfigureDef),
                  definition); // szv#4:S4163:12Mar99 `st=` not needed
    ...
    Standard_Boolean st = PR.ReadEntity(IR, PR.Current(), "Instance of TextDisplayTemplate",
                                        STANDARD_TYPE(IGESGraph_TextDisplayTemplate), textTemplate, Standard_True);
    ...
    if (st && nbval > 0) {
        ...
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "ConnectPoint entity", STANDARD_TYPE(IGESDraw_ConnectPoint),
                              tempConnectPoint, Standard_True))
...
void IGESDraw_ToolNetworkSubfigure::WriteOwnParams(const Handle(IGESDraw_NetworkSubfigure) & ent,
...
void IGESDraw_ToolNetworkSubfigure::OwnShared(const Handle(IGESDraw_NetworkSubfigure) & ent,
...
void IGESDraw_ToolNetworkSubfigure::OwnCopy(const Handle(IGESDraw_NetworkSubfigure) & another,
    ...
    DeclareAndCast(IGESDraw_NetworkSubfigureDef, definition, TC.Transferred(another->SubfigureDefinition()));
    ...
    DeclareAndCast(IGESGraph_TextDisplayTemplate, textTemplate, TC.Transferred(another->DesignatorTemplate()));

    if (nbval > 0) connectPoints = new IGESDraw_HArray1OfConnectPoint(1, nbval);
    for (Standard_Integer i = 1; i <= nbval; i++) {
        DeclareAndCast(IGESDraw_ConnectPoint, tempconnectPoint, TC.Transferred(another->ConnectPoint(i)));
        connectPoints->SetValue(i, tempconnectPoint);
    }
...
IGESData_DirChecker IGESDraw_ToolNetworkSubfigure::DirChecker(const Handle(IGESDraw_NetworkSubfigure) & /*ent*/) const {
...
void IGESDraw_ToolNetworkSubfigure::OwnCheck(const Handle(IGESDraw_NetworkSubfigure) & ent, const Interface_ShareTool&,
    ...
    if (ent->ReferenceDesignator().IsNull()) ach->AddFail("Primary Reference Designator : not defined");
}

void IGESDraw_ToolNetworkSubfigure::OwnDump(const Handle(IGESDraw_NetworkSubfigure) & ent,
