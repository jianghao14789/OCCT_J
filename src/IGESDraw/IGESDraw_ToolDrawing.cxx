...
void IGESDraw_ToolDrawing::ReadOwnParams(const Handle(IGESDraw_Drawing) & ent,
    ...
    if (st && nbval > 0) {
        ...
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadXY(PR.CurrentList(1, 2), "array viewOrigins", tempXY)) viewOrigins->SetValue(i, tempXY);
        }
    } else if (nbval < 0)
        PR.AddFail("Count of view entities : Less than zero");

    // Reading nbval(No. of Annotation Entities)
    // st = PR.ReadInteger(PR.Current(),"Count of array of Annotation entities", nbval); //szv#4:S4163:12Mar99 moved in
    // if
    if (PR.ReadInteger(PR.Current(), "Count of array of Annotation entities", nbval)) {
        if (nbval > 0)
            PR.ReadEnts(IR, PR.CurrentList(nbval), "Annotation Entities",
                        annotations); // szv#4:S4163:12Mar99 `st=` not needed
        /*
              {
...
void IGESDraw_ToolDrawing::WriteOwnParams(const Handle(IGESDraw_Drawing) & ent, IGESData_IGESWriter& IW) const {
...
void IGESDraw_ToolDrawing::OwnShared(const Handle(IGESDraw_Drawing) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer Up = ent->NbViews();
    Standard_Integer i; // svv Jan 10 2000 : porting on DEC
    for (i = 1; i <= Up; i++)
...
void IGESDraw_ToolDrawing::OwnCopy(const Handle(IGESDraw_Drawing) & another, const Handle(IGESDraw_Drawing) & ent,
...
void IGESDraw_ToolDrawing::OwnCheck(const Handle(IGESDraw_Drawing) & ent, const Interface_ShareTool&,
...
void IGESDraw_ToolDrawing::OwnDump(const Handle(IGESDraw_Drawing) & ent, const IGESData_IGESDumper& dumper,
