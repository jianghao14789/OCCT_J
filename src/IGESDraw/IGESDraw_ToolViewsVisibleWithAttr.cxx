...
void IGESDraw_ToolViewsVisibleWithAttr::ReadOwnParams(const Handle(IGESDraw_ViewsVisibleWithAttr) & ent,
    ...
    // Initialise HArray1 only if there is no error reading its Length
    if (tempNbEntity < 0) PR.AddFail("Number Of Entities Displayed : Less than Zero");
    ...
    if (!(tempViewEntities.IsNull())) {
        ...
        for (I = 1; I <= tempNbBlocks; I++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "View Entity", STANDARD_TYPE(IGESData_ViewKindEntity), tempView))
            ...
            if (PR.ReadInteger(PR.Current(), "Line Font Value", tempLineFont)) tempLineFonts->SetValue(I, tempLineFont);
            ...
            if (tempLineFont == 0 && PR.ReadEntity(IR, PR.Current(), "Line Font Definition",
                                                   STANDARD_TYPE(IGESData_LineFontEntity), tempEntity1, Standard_True))
    ...
    // Read the HArray1 only if its Length was read without any Error
    if (tempNbEntity > 0) {
        PR.ReadEnts(IR, PR.CurrentList(tempNbEntity), "Displayed Entities",
    ...
    ent->Init(tempViewEntities, tempLineFonts, tempLineDefinitions, tempColorValues, tempColorDefinitions,
              tempLineWeights, tempDisplayEntities);
}
...
void IGESDraw_ToolViewsVisibleWithAttr::OwnCopy(const Handle(IGESDraw_ViewsVisibleWithAttr) & another,
                                                const Handle(IGESDraw_ViewsVisibleWithAttr) & ent,
                                                Interface_CopyTool& TC) const {
    Standard_Integer I;
    Standard_Integer up = another->NbViews();
    Handle(IGESDraw_HArray1OfViewKindEntity) tempViewEntities = new IGESDraw_HArray1OfViewKindEntity(1, up);
    Handle(TColStd_HArray1OfInteger) tempLineFonts = new TColStd_HArray1OfInteger(1, up);
    Handle(IGESBasic_HArray1OfLineFontEntity) tempLineDefinitions = new IGESBasic_HArray1OfLineFontEntity(1, up);
    ...
    for (I = 1; I <= up; I++) {
        ...
        if (another->IsFontDefinition(I)) {
            DeclareAndCast(IGESData_LineFontEntity, tempEntity1, TC.Transferred(another->FontDefinition(I)));
            tempLineDefinitions->SetValue(I, tempEntity1);
        }
...
void IGESDraw_ToolViewsVisibleWithAttr::OwnRenew(const Handle(IGESDraw_ViewsVisibleWithAttr) & another,
                                                 const Handle(IGESDraw_ViewsVisibleWithAttr) & ent,
...
IGESData_DirChecker
IGESDraw_ToolViewsVisibleWithAttr::DirChecker(const Handle(IGESDraw_ViewsVisibleWithAttr) & /*ent*/) const {
...
Standard_Boolean IGESDraw_ToolViewsVisibleWithAttr::OwnCorrect(const Handle(IGESDraw_ViewsVisibleWithAttr) &
                                                               ent) const {
