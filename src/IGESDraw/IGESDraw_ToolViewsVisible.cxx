...
void IGESDraw_ToolViewsVisible::ReadOwnParams(const Handle(IGESDraw_ViewsVisible) & ent,
    ...
    // Initialise HArray1 only if there is no error reading its Length
    if (tempNbDisplayedEntities < 0) PR.AddFail("Number Of Entities Displayed : Less than Zero");
    ...
    if (!tempViewEntities.IsNull()) {
        ...
        for (I = 1; I <= tempNbViewsVisible; I++) {
            ...
            if (PR.ReadEntity(IR, PR.Current(), "View Entity", STANDARD_TYPE(IGESData_ViewKindEntity), tempViewEntity1))
    ...
    if (tempNbDisplayedEntities > 0) {
        PR.ReadEnts(IR, PR.CurrentList(tempNbDisplayedEntities), "Displayed Entities",
...
void IGESDraw_ToolViewsVisible::OwnCheck(const Handle(IGESDraw_ViewsVisible) & ent, const Interface_ShareTool&,
...
void IGESDraw_ToolViewsVisible::OwnDump(const Handle(IGESDraw_ViewsVisible) & ent, const IGESData_IGESDumper& dumper,
