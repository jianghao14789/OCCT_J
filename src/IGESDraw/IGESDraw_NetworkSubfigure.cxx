...
void IGESDraw_NetworkSubfigure::Init(const Handle(IGESDraw_NetworkSubfigureDef) & aDefinition,
                                     const gp_XYZ& aTranslation, const gp_XYZ& aScaleFactor,
    ...
    if (!allConnectPoints.IsNull())
        if (allConnectPoints->Lower() != 1) throw Standard_DimensionMismatch("IGESDraw_NetworkSubfigure : Init");
...
Handle(IGESDraw_ConnectPoint) IGESDraw_NetworkSubfigure::ConnectPoint(const Standard_Integer Index) const {
