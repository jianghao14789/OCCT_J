...
void IGESDraw_ToolLabelDisplay::ReadOwnParams(const Handle(IGESDraw_LabelDisplay) & ent,
    ...
    if (st && nbval > 0) {
        ...
        for (Standard_Integer i = 1; i <= nbval; i++) {
            ...
            if (PR.ReadXYZ(PR.CurrentList(1, 3), "array textLocations", tempXYZ)) textLocations->SetValue(i, tempXYZ);
            ...
            if (PR.ReadEntity(IR, PR.Current(), "Instance of LeaderArrow", STANDARD_TYPE(IGESDimen_LeaderArrow),
                              tempLeaderArrow))
            ...
            if (PR.ReadInteger(PR.Current(), "array labelLevels", tempLabel)) labelLevels->SetValue(i, tempLabel);
...
void IGESDraw_ToolLabelDisplay::OwnCheck(const Handle(IGESDraw_LabelDisplay) & /*ent*/, const Interface_ShareTool&,
                                         Handle(Interface_Check) & /*ach*/) const {}

void IGESDraw_ToolLabelDisplay::OwnDump(const Handle(IGESDraw_LabelDisplay) & ent, const IGESData_IGESDumper& dumper,
