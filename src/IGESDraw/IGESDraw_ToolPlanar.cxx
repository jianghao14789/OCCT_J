...
void IGESDraw_ToolPlanar::ReadOwnParams(const Handle(IGESDraw_Planar) & ent, const Handle(IGESData_IGESReaderData) & IR,
    ...
    if (nbMatrices != 1) PR.AddFail("No. of Transformation matrices != 1");
    ...
    st = PR.ReadEntity(IR, PR.Current(), "Instance of TransformationMatrix",
                       STANDARD_TYPE(IGESGeom_TransformationMatrix), transformationMatrix, Standard_True);

    if (nbval > 0) st = PR.ReadEnts(IR, PR.CurrentList(nbval), "Planar Entities", entities);
    /*
        {
...
void IGESDraw_ToolPlanar::WriteOwnParams(const Handle(IGESDraw_Planar) & ent, IGESData_IGESWriter& IW) const {
...
void IGESDraw_ToolPlanar::OwnShared(const Handle(IGESDraw_Planar) & ent, Interface_EntityIterator& iter) const {
    Standard_Integer Up = ent->NbEntities();
...
void IGESDraw_ToolPlanar::OwnCopy(const Handle(IGESDraw_Planar) & another, const Handle(IGESDraw_Planar) & ent,
    ...
    DeclareAndCast(IGESGeom_TransformationMatrix, transformationMatrix, TC.Transferred(another->TransformMatrix()));
...
void IGESDraw_ToolPlanar::OwnCheck(const Handle(IGESDraw_Planar) & ent, const Interface_ShareTool&,
                                   Handle(Interface_Check) & ach) const {
    if (ent->NbMatrices() != 1) ach->AddFail("No. of Transformation matrices : Value != 1");
}

void IGESDraw_ToolPlanar::OwnDump(const Handle(IGESDraw_Planar) & ent, const IGESData_IGESDumper& dumper,
