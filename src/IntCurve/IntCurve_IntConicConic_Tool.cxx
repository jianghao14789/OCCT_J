...
void Determine_Transition_LC(const IntRes2d_Position Pos1, gp_Vec2d& Tan1, const gp_Vec2d& Norm1,
                             IntRes2d_Transition& T1, const IntRes2d_Position Pos2, gp_Vec2d& Tan2,
                             const gp_Vec2d& Norm2, IntRes2d_Transition& T2, const Standard_Real) {
...
PeriodicInterval PeriodicInterval::FirstIntersection(PeriodicInterval& PInter) {
    ...
    if (PInter.isnull || isnull) {
        ...
        if (Length() >= PIpPI) return (PeriodicInterval(PInter.Binf, PInter.Bsup));
        if (PInter.Length() >= PIpPI) return (PeriodicInterval(Binf, Bsup));
        if (PInter.Bsup <= Binf) {
...
//----------------------------------------------------------------------
Interval::Interval() : Binf(0.), Bsup(0.), HasFirstBound(Standard_False), HasLastBound(Standard_False) {
...
Interval::Interval(const IntRes2d_Domain& Domain) : Binf(0.0), Bsup(0.0) {
    IsNull = Standard_False;
...
Interval::Interval(const Standard_Real a, const Standard_Boolean hf, const Standard_Real b, const Standard_Boolean hl) {
    Binf = a;
    Bsup = b;
    IsNull = Standard_False;
    HasFirstBound = hf;
...
Interval Interval::IntersectionWithBounded(const Interval& Inter) {
    ...
    if (!(HasFirstBound || HasLastBound)) return (Interval(Inter.Binf, Inter.Bsup));
    Standard_Real a, b;
    if (HasFirstBound) {
        if (Inter.Bsup < Binf) {
            Interval PourSGI;
