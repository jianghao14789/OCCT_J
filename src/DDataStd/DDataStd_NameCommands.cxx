...
static Standard_Integer DDataStd_SetName(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
...
static Standard_Integer DDataStd_GetName(Draw_Interpretor& di, Standard_Integer nb, const char** arg) {
    if (nb == 3 || nb == 4) {
        Handle(TDF_Data) DF;
        if (!DDF::GetDF(arg[1], DF)) return 1;
        TDF_Label L;
...
#ifdef DEB_DDataStd
        if (!N.IsNull()) std::cout << "String = " << N->Get() << std::endl;
#endif
        di << N->Get();
        return 0;
    }
