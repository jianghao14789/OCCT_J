...
void DDataStd_DrawPresentation::Paste(const Handle(TDF_Attribute) & Into, const Handle(TDF_RelocationTable) &) const {
...
void DDataStd_DrawPresentation::DrawDisplay(const TDF_Label& L, const Handle(DDataStd_DrawPresentation) & P) {
    if (!L.IsNull()) {
...
void DDataStd_DrawPresentation::DrawErase(const TDF_Label& L, const Handle(DDataStd_DrawPresentation) & P) {
    if (!L.IsNull()) {
        if (!P->GetDrawable().IsNull()) {
