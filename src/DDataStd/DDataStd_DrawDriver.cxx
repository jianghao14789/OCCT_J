...
static TopoDS_Shape Geometry(const Handle(TDataXtd_Constraint) & A, const Standard_Integer i, TopAbs_ShapeEnum T) {
    TopoDS_Shape S = TNaming_Tool::GetShape(A->GetGeometry(i));
    if (!S.IsNull()) {
        if (T != TopAbs_SHAPE && T != S.ShapeType()) S.Nullify();
    }
    return S;
}
...
Handle(Draw_Drawable3D) DDataStd_DrawDriver::DrawableConstraint(const Handle(TDataXtd_Constraint) & A) const {
    ...
    switch (A->GetType()) {
        ...
        case TDataXtd_ANGLE: {
            if (A->IsPlanar()) {
                Handle(DrawDim_PlanarAngle) DAng = new DrawDim_PlanarAngle(TNaming_Tool::GetShape(A->GetGeometry(1)),
                                                                           TNaming_Tool::GetShape(A->GetGeometry(2)));
        ...
        case TDataXtd_MATE: {
            ...
            //      TopoDS_Face F1 = TopoDS::Face(Geometry(A,1,TopAbs_FACE));
            //      TopoDS_Face F2 = TopoDS::Face(Geometry(A,2,TopAbs_FACE));
            if (!F1.IsNull() && !F2.IsNull()) D = new DrawDim_Distance(F1, F2);
        } break;
    ...
    if (!D.IsNull()) {
        if (!A->GetValue().IsNull()) {
            ...
            Standard_DISABLE_DEPRECATION_WARNINGS if (A->GetValue()->GetDimension() == TDataStd_ANGULAR) val =
                (180. * val) / M_PI;
...
Handle(Draw_Drawable3D) DDataStd_DrawDriver::DrawableShape(const TDF_Label& L, const Draw_ColorKind color,
...
Handle(Draw_Drawable3D) DDataStd_DrawDriver::DrawableShape(const TopoDS_Shape& s, const Draw_ColorKind color) {
