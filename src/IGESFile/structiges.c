...
void iges_newparam(int typarg, int longval, char* parval) {
    ...
    curparam->next = NULL;
    if (curlist->first == NULL)
        curlist->first = curparam;
    else
        curlist->last->next = curparam;
...
void iges_finfile(int mode) {
    ...
    if (mode == 0 || mode == 1) {
        ...
        while (oneparpage != NULL) {
            struct parpage* oldparpage;
            oldparpage = oneparpage->next;
            free(oneparpage);
    ...
    if (mode == 0 || mode == 2) {
        while (onecarpage != NULL) {
            struct carpage* oldcarpage;
            oldcarpage = onecarpage->next;
            free(onecarpage);
