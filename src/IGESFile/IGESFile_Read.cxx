...
Standard_Integer IGESFile_Read(char* nomfic, const Handle(IGESData_IGESModel) & amodel,
                               const Handle(IGESData_Protocol) & protocol) {
    Handle(IGESData_FileRecognizer) nulreco;
    return IGESFile_Read(nomfic, amodel, protocol, nulreco, Standard_False);
}

Standard_Integer IGESFile_ReadFNES(char* nomfic, const Handle(IGESData_IGESModel) & amodel,
                                   const Handle(IGESData_Protocol) & protocol) {
    Handle(IGESData_FileRecognizer) nulreco;
...
Standard_Integer IGESFile_Read(char* nomfic, const Handle(IGESData_IGESModel) & amodel,
                               const Handle(IGESData_Protocol) & protocol, const Handle(IGESData_FileRecognizer) & reco,
...
void IGESFile_ReadContent(const Handle(IGESData_IGESReaderData) & IR) {
    ...
    while ((ns = iges_lirpart(&v, &res1, &res2, &nom, &num, &nbparam)) != 0) {
        ...
        recupne = (ns + 1) / 2; // numero entite
                                ...
                                //      continue;
                                //    }
        IR->SetDirPart(recupne, v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13],
                       v[14], v[15], v[16], res1, res2, nom, num);
