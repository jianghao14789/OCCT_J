...
static int bases[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};
...
void iges_param(int* Pstat, char* ligne, char c_separ, char c_fin, int lonlin) {
    ...
    for (i = 0; (unpar = ligne[numcar + i]) != '\0'; i++) {
        ...
        if (unpar > 47 && unpar < 58) {
            ...
            if (typarg == ArgVide)
                typarg = ArgSign;
            else if (typarg != ArgExp && typarg != ArgMexp)
                typarg = ArgQuid;
        }
            ...
            if (typarg == ArgVide)
                typarg = ArgReal;
            else if (typarg == ArgInt || typarg == ArgSign)
                typarg = ArgReal;
            else
                typarg = ArgQuid;
        }
        ...
        /*   blanc : leading (facile) ou trailing (chercher la suite), sinon mauvais */
            ...
            typarg = ArgQuid; /* caractere non reconnu */
    }
