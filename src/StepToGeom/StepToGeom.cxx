...
Handle(Geom_Axis2Placement) StepToGeom::MakeAxis2Placement(const Handle(StepGeom_Axis2Placement3d) & SA) {
    ...
    if (!P.IsNull()) {
        ...
        // sln 22.10.2001. CTS23496: If problems with creation of direction occur default direction is used
        // (MakeLine(...) function)
        gp_Dir Ngp(0., 0., 1.);
        if (SA->HasAxis()) {
            Handle(Geom_Direction) D = MakeDirection(SA->Axis());
            if (!D.IsNull()) Ngp = D->Dir();
        }
        ...
        if (isDefaultDirectionUsed) gpAx2 = gp_Ax2(Pgp, Ngp);
...
Handle(Geom_Axis2Placement) StepToGeom::MakeAxis2Placement(const Handle(StepGeom_SuParameters) & theSP) {
    ...
    Standard_Real aDirX =
        cos(theSP->Gamma()) * cos(theSP->Beta()) - sin(theSP->Gamma()) * cos(theSP->Alpha()) * sin(theSP->Beta());
    Standard_Real aDirY =
        sin(theSP->Gamma()) * cos(theSP->Beta()) + cos(theSP->Gamma()) * cos(theSP->Alpha()) * sin(theSP->Beta());
...
Handle(Geom_BoundedCurve) StepToGeom::MakeBoundedCurve(const Handle(StepGeom_BoundedCurve) & SC) {
    ...
    if (SC->IsKind(STANDARD_TYPE(StepGeom_BezierCurve))) {
        ...
        if (aDegree < 1 || aDegree > Geom_BSplineCurve::MaxDegree()) return 0;
    ...
    if (SC->IsKind(STANDARD_TYPE(StepGeom_UniformCurveAndRationalBSplineCurve))) {
        ...
        RBSPL->Init(RUC->Name(), aDegree, RUC->ControlPointsList(), RUC->CurveForm(), RUC->ClosedCurve(),
                    RUC->SelfIntersect(), Kmult, Knots, StepGeom_ktUnspecified, RUC->WeightsData());
    ...
    if (SC->IsKind(STANDARD_TYPE(StepGeom_QuasiUniformCurveAndRationalBSplineCurve))) {
        ...
        // Initialize the BSplineCurveWithKnotsAndRationalBSplineCurve
        RBSPL->Init(RQUC->Name(), aDegree, RQUC->ControlPointsList(), RQUC->CurveForm(), RQUC->ClosedCurve(),
                    RQUC->SelfIntersect(), Kmult, Knots, StepGeom_ktUnspecified, RQUC->WeightsData());
...
Handle(Geom_BoundedSurface) StepToGeom::MakeBoundedSurface(const Handle(StepGeom_BoundedSurface) & SS) {
    ...
    if (SS->IsKind(STANDARD_TYPE(StepGeom_QuasiUniformSurface))) {
        const Handle(StepGeom_QuasiUniformSurface) QUS = Handle(StepGeom_QuasiUniformSurface)::DownCast(SS);
        const Handle(StepGeom_BSplineSurfaceWithKnots) BSPL = new StepGeom_BSplineSurfaceWithKnots;
    ...
    if (SS->IsKind(STANDARD_TYPE(StepGeom_UniformSurfaceAndRationalBSplineSurface))) {
        ...
        RBSPL->Init(RUS->Name(), RUS->UDegree(), RUS->VDegree(), RUS->ControlPointsList(), RUS->SurfaceForm(),
                    RUS->UClosed(), RUS->VClosed(), RUS->SelfIntersect(), UKmult, VKmult, UKnots, VKnots,
                    StepGeom_ktUnspecified, RUS->WeightsData());
    ...
    if (SS->IsKind(STANDARD_TYPE(StepGeom_QuasiUniformSurfaceAndRationalBSplineSurface))) {
        ...
        // Initialize the BSplineSurfaceWithKnotsAndRationalBSplineSurface
        RBSPL->Init(RQUS->Name(), RQUS->UDegree(), RQUS->VDegree(), RQUS->ControlPointsList(), RQUS->SurfaceForm(),
                    RQUS->UClosed(), RQUS->VClosed(), RQUS->SelfIntersect(), UKmult, VKmult, UKnots, VKnots,
                    StepGeom_ktUnspecified, RQUS->WeightsData());
        return MakeBSplineSurface(RBSPL);
    }
...
Handle(TBSplineCurve)
    MakeBSplineCurveCommon(const Handle(StepGeom_BSplineCurve) & theStepGeom_BSplineCurve,
    ...
    for (Standard_Integer i = 1; i <= NbUniqueKnots; ++i) {
        ...
        if (aCurrentVal > aDegree + 1) {
            if (i == 1) aFirstMuultypisityDifference = aCurrentVal - aDegree - 1;
            if (i == NbUniqueKnots) aLastMuultypisityDifference = aCurrentVal - aDegree - 1;
#ifdef OCCT_DEBUG
    ...
    if (theStepGeom_BSplineCurve->IsKind(STANDARD_TYPE(StepGeom_BSplineCurveWithKnotsAndRationalBSplineCurve))) {
        ...
        aBSplineCurve = new TBSplineCurve(Poles, aUniqueWeights, aUniqueKnots, aUniqueKnotMultiplicities, aDegree,
                                          shouldBePeriodic);
    } else {
...
Handle(Geom2d_BSplineCurve) StepToGeom::MakeBSplineCurve2d(const Handle(StepGeom_BSplineCurve) &
...
Handle(Geom_BSplineSurface) StepToGeom::MakeBSplineSurface(const Handle(StepGeom_BSplineSurface) & SS) {
    ...
    } else if ((UMult.Value(1) == UMult.Value(NUKnotsUnique)) && ((SumMult - UMult.Value(1)) == NUPoles)) {
    ...
    } else if ((VMult.Value(1) == VMult.Value(NVKnotsUnique)) && ((SumMult - VMult.Value(1)) == NVPoles)) {
    ...
    if (SS->IsKind(STANDARD_TYPE(StepGeom_BSplineSurfaceWithKnotsAndRationalBSplineSurface))) {
        ...
        CS =
            new Geom_BSplineSurface(Poles, W, KUn, KVn, UMult, VMult, UDeg, VDeg, shouldBeUPeriodic, shouldBeVPeriodic);
    } else
        CS = new Geom_BSplineSurface(Poles, KUn, KVn, UMult, VMult, UDeg, VDeg, shouldBeUPeriodic, shouldBeVPeriodic);
    return CS;
}
...
Handle(Geom_Line) StepToGeom::MakeLine(const Handle(StepGeom_Line) & SC) {
    ...
    if (!P.IsNull()) {
        ...
        if (!D.IsNull()) {
            if (D->Vec().SquareMagnitude() < Precision::Confusion() * Precision::Confusion()) return 0;
...
Handle(Geom_BSplineCurve) StepToGeom::MakePolyline(const Handle(StepGeom_Polyline) & SPL) {
    if (SPL.IsNull()) return Handle(Geom_BSplineCurve)();
...
Handle(Geom2d_BSplineCurve) StepToGeom::MakePolyline2d(const Handle(StepGeom_Polyline) & SPL) {
    if (SPL.IsNull()) return Handle(Geom2d_BSplineCurve)();
...
Handle(Geom_RectangularTrimmedSurface)
    ...
    if (!theBasis.IsNull()) {
        ...
        const Standard_Real AngleFact =
            StepData_GlobalFactors::Intance().PlaneAngleFactor(); // abv 30.06.00 trj4_k1_geo-tc-214.stp #1477: PI/180.;
...
Handle(Geom_Surface) StepToGeom::MakeSurface(const Handle(StepGeom_Surface) & SS) {
    ...
    try {
        ...
        if (SS->IsKind(STANDARD_TYPE(StepGeom_ElementarySurface))) {
            ...
            if (S1->Position().IsNull()) return Handle(Geom_Surface)();
...
Handle(Geom_SurfaceOfLinearExtrusion)
    StepToGeom::MakeSurfaceOfLinearExtrusion(const Handle(StepGeom_SurfaceOfLinearExtrusion) & SS) {
            ...
            if (C->IsKind(STANDARD_TYPE(Geom_Circle)) || C->IsKind(STANDARD_TYPE(Geom_Ellipse))) {
                ...
                if (rl.Distance(pc) < Precision::Confusion()) { // pc lies on A2
                    ...
                    if (dirline.IsNormal(norm, Precision::Angular()) &&
                        (dirline.IsParallel(xAxis, Precision::Angular()) || C->IsKind(STANDARD_TYPE(Geom_Circle)))) {
...
Standard_Boolean StepToGeom::MakeTransformation3d(const Handle(StepGeom_CartesianTransformationOperator3d) & SCTO,
    ...
    if (!CP.IsNull()) {
        ...
        if (isDefaultDirectionUsed) D3 = D1.Crossed(D2);
...
static Standard_Boolean ExtractParameter(const Handle(Geom_Curve) & aGeomCurve,
                                         ...
                                         const Standard_Integer nbSel, const Standard_Integer MasterRep,
                                         const Standard_Real Factor, const Standard_Real Shift, Standard_Real& aParam) {
...
Handle(Geom_TrimmedCurve) StepToGeom::MakeTrimmedCurve(const Handle(StepGeom_TrimmedCurve) & SC) {
    ...
    if (theCurve.IsNull()) return Handle(Geom_TrimmedCurve)();
    ...
    if (theSTEPCurve->IsKind(STANDARD_TYPE(StepGeom_Line))) {
        ...
        if (!ellipse.IsNull() && ellipse->SemiAxis1() - ellipse->SemiAxis2() < 0.) shift = 0.5 * M_PI;
...
Handle(Geom2d_BSplineCurve) StepToGeom::MakeTrimmedCurve2d(const Handle(StepGeom_TrimmedCurve) & SC) {
    ...
    if (theGeomBasis.IsNull()) return Handle(Geom2d_BSplineCurve)();
...
Handle(TColStd_HArray1OfReal) StepToGeom::MakeYprRotation(const StepKinematics_SpatialRotation& SR,
    ...
    if (SR.RotationAboutDirection().IsNull() ||
        SR.RotationAboutDirection()->DirectionOfAxis()->DirectionRatios()->Length() != 3 || theCntxt.IsNull()) {
        return NULL;
    }
    ...
    anAngle = (!aSiUnit->HasPrefix() ? 1. : STEPConstruct_UnitContext::ConvertSiPrefix(aSiUnit->Prefix())) * anAngle;
