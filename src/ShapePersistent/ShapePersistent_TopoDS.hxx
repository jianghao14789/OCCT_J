...
protected:
    ...
    protected:
        template <class ShapesArray> void addShapesT(TopoDS_Shape& theParent) const;
    };

private:
    template <class Target> class pTSimple : public pTBase {
    ...
    template <class Persistent, class ShapesArray> class pTObject : public Persistent {
    ...
    struct tObjectT : public Delayed<DelayedBase<TShape, TopoDS_TShape, pTBase>, pTObject<Persistent, ShapesArray>> {
        typedef pTObject<Persistent, ShapesArray> pTObjectT;
    };

protected:
    template <class Persistent> struct tObject : public tObjectT<Persistent, StdLPersistent_HArray1::Persistent> {};

    template <class Persistent> struct tObject1 : public tObjectT<Persistent, StdPersistent_HArray1::Shape1> {};
...
public:
    ...
    Standard_EXPORT static Handle(HShape)
...
template <> inline Standard_CString ShapePersistent_TopoDS::pTSimple<TopoDS_TWire>::PName() const {
    return "PTopoDS_TWire";
}

template <> inline Standard_CString ShapePersistent_TopoDS::pTSimple<TopoDS_TShell>::PName() const {
    return "PTopoDS_TShell";
}

template <> inline Standard_CString ShapePersistent_TopoDS::pTSimple<TopoDS_TSolid>::PName() const {
    return "PTopoDS_TSolid";
}

template <> inline Standard_CString ShapePersistent_TopoDS::pTSimple<TopoDS_TCompSolid>::PName() const {
    return "PTopoDS_TCompSolid";
}

template <> inline Standard_CString ShapePersistent_TopoDS::pTSimple<TopoDS_TCompound>::PName() const {
