...
class ShapePersistent_Poly : private StdObjMgt_SharedObject {
    ...
    public:
        pPolygonOnTriangulation() : myDeflection(0.0) {}
        inline void Read(StdObjMgt_ReadData& theReadData) {
            theReadData >> myDeflection >> myNodes >> myParameters;
        }
        inline void Write(StdObjMgt_WriteData& theWriteData) const {
        ...
        Standard_EXPORT void PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const;
        inline Standard_CString PName() const {
            return "PPoly_PolygonOnTriangulation";
        }
    ...
    public:
        pTriangulation() : myDeflection(0.0) {}
        inline void Read(StdObjMgt_ReadData& theReadData) {
...
public:
    ...
    Standard_EXPORT static Handle(Polygon2D)
    ...
    Standard_EXPORT static Handle(Polygon3D)
        Translate(const Handle(Poly_Polygon3D) & thePoly, StdObjMgt_TransientPersistentMap& theMap);
    ...
    Standard_EXPORT static Handle(Triangulation)
        Translate(const Handle(Poly_Triangulation) & thePolyTriang, StdObjMgt_TransientPersistentMap& theMap);
};
