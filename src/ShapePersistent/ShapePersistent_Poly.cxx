...
void ShapePersistent_Poly::pPolygon2D::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    theChildren.Append(myNodes);
}

Handle(Poly_Polygon2D) ShapePersistent_Poly::pPolygon2D::Import() const {
    if (myNodes.IsNull()) return NULL;
...
void ShapePersistent_Poly::pPolygon3D::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    theChildren.Append(myNodes);
    theChildren.Append(myParameters);
}

Handle(Poly_Polygon3D) ShapePersistent_Poly::pPolygon3D::Import() const {
    if (myNodes.IsNull() || myParameters.IsNull()) return NULL;
...
void ShapePersistent_Poly::pTriangulation::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    theChildren.Append(myNodes);
...
// Triangulation is not used
    ...
    if (myNodes && myTriangles) {
        if (myUVNodes)
            aTriangulation = new Poly_Triangulation(*myNodes->Array(), *myUVNodes->Array(), *myTriangles->Array());
        else
...
Handle(ShapePersistent_Poly::Polygon2D) ShapePersistent_Poly::Translate(const Handle(Poly_Polygon2D) & thePoly,
...
Handle(ShapePersistent_Poly::Polygon3D) ShapePersistent_Poly::Translate(const Handle(Poly_Polygon3D) & thePoly,
                                                                        StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_Poly::PolygonOnTriangulation) ShapePersistent_Poly::Translate(
    const Handle(Poly_PolygonOnTriangulation) & thePolyOnTriang, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_Poly::Triangulation) ShapePersistent_Poly::Translate(const Handle(Poly_Triangulation) &
                                                                                thePolyTriang,
                                                                            StdObjMgt_TransientPersistentMap& theMap) {
