...
enum { ParameterMask = 1, RangeMask = 2, DegeneratedMask = 4 };
...
void ShapePersistent_BRep::PointRepresentation::Import(BRep_ListOfPointRepresentation& thePoints) const {
    thePoints.Clear();
...
void ShapePersistent_BRep::PointOnCurve::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
...
Handle(BRep_PointRepresentation) ShapePersistent_BRep::PointOnCurve::import() const {
    ...
    if (myCurve) aCurve = myCurve->Import();
...
void ShapePersistent_BRep::PointsOnSurface::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    PointRepresentation::PChildren(theChildren);
...
Handle(BRep_PointRepresentation) ShapePersistent_BRep::PointOnCurveOnSurface::import() const {
    ...
    if (myPCurve) aPCurve = myPCurve->Import();

    Handle(Geom_Surface) aSurface;
    if (mySurface) aSurface = mySurface->Import();
...
void ShapePersistent_BRep::CurveRepresentation::Import(BRep_ListOfCurveRepresentation& theCurves) const {
    theCurves.Clear();
...
void ShapePersistent_BRep::Curve3D::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::Curve3D::import() const {
    ...
    if (myCurve3D) aCurve3D = myCurve3D->Import();
...
void ShapePersistent_BRep::CurveOnSurface::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    GCurve::PChildren(theChildren);
    theChildren.Append(myPCurve);
    theChildren.Append(mySurface);
}
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::CurveOnSurface::import() const {
    ...
    Handle(BRep_CurveOnSurface) aRepresentation = new BRep_CurveOnSurface(aPCurve, aSurface, myLocation.Import());
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::CurveOnClosedSurface::import() const {
    ...
    if (myPCurve2) aPCurve2 = myPCurve2->Import();
...
void ShapePersistent_BRep::Polygon3D::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::Polygon3D::import() const {
    ...
    if (myPolygon3D) aPolygon3D = myPolygon3D->Import();
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::PolygonOnTriangulation::import() const {
    ...
    if (myPolygon) aPolygon = myPolygon->Import();

    Handle(Poly_Triangulation) aTriangulation;
    if (myTriangulation) aTriangulation = myTriangulation->Import();
...
void ShapePersistent_BRep::PolygonOnClosedTriangulation::Write(StdObjMgt_WriteData& theWriteData) const {
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::PolygonOnClosedTriangulation::import() const {
    ...
    if (myPolygon2) aPolygon2 = myPolygon2->Import();

    Handle(Poly_Triangulation) aTriangulation;
    if (myTriangulation) aTriangulation = myTriangulation->Import();

    return new BRep_PolygonOnClosedTriangulation(aPolygon, aPolygon2, aTriangulation, myLocation.Import());
}
...
void ShapePersistent_BRep::PolygonOnSurface::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    CurveRepresentation::PChildren(theChildren);
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::PolygonOnSurface::import() const {
    ...
    if (myPolygon2D) aPolygon2D = myPolygon2D->Import();
...
void ShapePersistent_BRep::CurveOn2Surfaces::PChildren(StdObjMgt_Persistent::SequenceOfPersistent& theChildren) const {
    CurveRepresentation::PChildren(theChildren);
    theChildren.Append(mySurface);
...
Handle(BRep_CurveRepresentation) ShapePersistent_BRep::CurveOn2Surfaces::import() const {
    ...
    if (mySurface2) aSurface2 = mySurface2->Import();

    GeomAbs_Shape aContinuity = static_cast<GeomAbs_Shape>(myContinuity);

    return new BRep_CurveOn2Surfaces(aSurface, aSurface2, myLocation.Import(), myLocation2.Import(), aContinuity);
}
...
Handle(TopoDS_TShape) ShapePersistent_BRep::pTVertex::createTShape() const {
    ...
    if (myPoints) myPoints->Import(aTVertex->ChangePoints());
...
Handle(TopoDS_TShape) ShapePersistent_BRep::pTEdge::createTShape() const {
    ...
    if (myCurves) myCurves->Import(aTEdge->ChangeCurves());
...
Handle(TopoDS_TShape) ShapePersistent_BRep::pTFace::createTShape() const {
    ...
    if (mySurface) aTFace->Surface(mySurface->Import());

    if (myTriangulation) aTFace->Triangulation(myTriangulation->Import());
...
Handle(ShapePersistent_BRep::TVertex::pTObjectT)
    ShapePersistent_BRep::Translate(const TopoDS_Vertex& theVertex, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::TEdge::pTObjectT)
    ShapePersistent_BRep::Translate(const TopoDS_Edge& theEdge, StdObjMgt_TransientPersistentMap& theMap,
    ...
    while (itcr.More()) {
        ...
        if (!GC.IsNull()) {
            ...
            // CurveRepresentation is CurveOnSurface
                ...
                // CurveRepresentation is CurveOnClosedSurface
                    ...
                    Handle(CurveOnClosedSurface) COCS = Translate(CR->PCurve(), CR->PCurve2(), f, l, CR->Surface(),
                                                                  CR->Location(), CR->Continuity(), theMap);
                    COCS->myUV21 = Pnt21;
                    COCS->myUV22 = Pnt22;
                    COS = COCS;
                }
        ...
        // CurveRepresentation is CurveOn2Surfaces
            ...
            Handle(CurveOn2Surfaces) R =
                Translate(CR->Surface(), CR->Surface2(), CR->Location(), CR->Location2(), CR->Continuity(), theMap);
            CPCR = R;
        }
        // CurveRepresentation is Polygon or Triangulation
            ...
            // CurveRepresentation is PolygonOnSurface
                ...
                // CurveRepresentation is PolygonOnSurface
                    ...
                    Handle(PolygonOnSurface) PolOS = Translate(CR->Polygon(), CR->Surface(), CR->Location(), theMap);
                    CPCR = PolOS;
                }
            }
            // CurveRepresentation is PolygonOnTriangulation
                ...
                if (CR->IsPolygonOnClosedTriangulation()) {
                    Handle(PolygonOnClosedTriangulation) PolOCT =
                        Translate(CR->PolygonOnTriangulation(), CR->PolygonOnTriangulation2(), CR->Triangulation(),
                                  CR->Location(), theMap);
                    CPCR = PolOCT;
                }
                // CurveRepresentation is PolygonOnTriangulation
...
Handle(ShapePersistent_BRep::TFace::pTObjectT)
    ShapePersistent_BRep::Translate(const TopoDS_Face& theFace, StdObjMgt_TransientPersistentMap& theMap,
                                    ShapePersistent_TriangleMode theTriangleMode) {
...
// purpose  : Translates a shape to its persistent avatar
//=======================================================================
Handle(ShapePersistent_BRep::PointOnCurve) ShapePersistent_BRep::Translate(Standard_Real theParam,
...
Handle(ShapePersistent_BRep::PointOnCurveOnSurface) ShapePersistent_BRep::Translate(
    Standard_Real theParam, const Handle(Geom2d_Curve) & theCurve, const Handle(Geom_Surface) & theSurf,
    const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
...
// purpose  : Translates a shape to its persistent avatar
//=======================================================================
Handle(ShapePersistent_BRep::PointOnSurface) ShapePersistent_BRep::Translate(Standard_Real theParam,
...
Handle(ShapePersistent_BRep::CurveOnSurface) ShapePersistent_BRep::Translate(
    const Handle(Geom2d_Curve) & theCurve, const Standard_Real theFirstParam, const Standard_Real theLastParam,
    const Handle(Geom_Surface) & theSurf, const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::CurveOnClosedSurface) ShapePersistent_BRep::Translate(
    const Handle(Geom2d_Curve) & theCurve, const Handle(Geom2d_Curve) & theCurve2, const Standard_Real theFirstParam,
    const Standard_Real theLastParam, const Handle(Geom_Surface) & theSurf, const TopLoc_Location& theLoc,
    const GeomAbs_Shape theContinuity, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::CurveOn2Surfaces) ShapePersistent_BRep::Translate(
    const Handle(Geom_Surface) & theSurf, const Handle(Geom_Surface) & theSurf2, const TopLoc_Location& theLoc,
    const TopLoc_Location& theLoc2, const GeomAbs_Shape theContinuity, StdObjMgt_TransientPersistentMap& theMap) {
...
// purpose  : Translates a shape to its persistent avatar
//=======================================================================
Handle(ShapePersistent_BRep::Curve3D) ShapePersistent_BRep::Translate(const Handle(Geom_Curve) & theCurve,
...
// purpose  : Translates a shape to its persistent avatar
//=======================================================================
Handle(ShapePersistent_BRep::Polygon3D) ShapePersistent_BRep::Translate(const Handle(Poly_Polygon3D) & thePoly,
                                                                        const TopLoc_Location& theLoc,
                                                                        StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::PolygonOnClosedSurface) ShapePersistent_BRep::Translate(
    const Handle(Poly_Polygon2D) & thePoly, const Handle(Poly_Polygon2D) & thePoly2,
    const Handle(Geom_Surface) & theSurf, const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::PolygonOnSurface)
    ShapePersistent_BRep::Translate(const Handle(Poly_Polygon2D) & thePoly, const Handle(Geom_Surface) & theSurf,
                                    const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::PolygonOnClosedTriangulation) ShapePersistent_BRep::Translate(
    ...
    const Handle(Poly_PolygonOnTriangulation) & thePolyOnTriang2, const Handle(Poly_Triangulation) & thePolyTriang,
    const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
...
Handle(ShapePersistent_BRep::PolygonOnTriangulation) ShapePersistent_BRep::Translate(
    const Handle(Poly_PolygonOnTriangulation) & thePolyOnTriang, const Handle(Poly_Triangulation) & thePolyTriang,
    const TopLoc_Location& theLoc, StdObjMgt_TransientPersistentMap& theMap) {
