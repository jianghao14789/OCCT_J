...
public:
    ...
    Standard_EXPORT static Handle(TopTools_HSequenceOfShape)
        Shapes(const Handle(Transfer_TransientProcess) & TP, const Standard_Boolean rootsonly = Standard_True);

    //! Gets the Shapes recorded in a TransientProcess as result of a
    //! Transfer, for a given list of starting entities, returns
    //! the shapes as a HSequence
    Standard_EXPORT static Handle(TopTools_HSequenceOfShape)
        Shapes(const Handle(Transfer_TransientProcess) & TP, const Handle(TColStd_HSequenceOfTransient) & list);
    ...
    //! Null if none
    Standard_EXPORT static Handle(Transfer_Binder)
        ResultFromShape(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& shape);
    ...
    Standard_EXPORT static Handle(Standard_Transient)
        TransientFromShape(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& shape);
    ...
    Standard_EXPORT static Handle(TransferBRep_ShapeMapper)
        ShapeMapper(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& shape);
    ...
    //! TopAbs_ShapeEnum).
    Standard_EXPORT static void TransferResultInfo(const Handle(Transfer_TransientProcess) & TP,
    ...
    Standard_EXPORT static void TransferResultInfo(const Handle(Transfer_FinderProcess) & FP,
                                                   const Handle(TColStd_HSequenceOfInteger) & ShapeTypes,
                                                   Handle(TransferBRep_HSequenceOfTransferResultInfo) & InfoSeq);

    //! Prints the results of transfer to given priner with given header.
    Standard_EXPORT static void PrintResultInfo(const Handle(Message_Printer) & Printer, const Message_Msg& Header,
    ...
    Standard_EXPORT static Interface_CheckIterator ResultCheckList(const Interface_CheckIterator& chl,
    ...
    Standard_EXPORT static Handle(TColStd_HSequenceOfTransient)
        Checked(const Interface_CheckIterator& chl, const Standard_Boolean alsoshapes = Standard_False);
