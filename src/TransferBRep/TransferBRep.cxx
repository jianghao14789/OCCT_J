...
static void ShapeAppend(const Handle(Transfer_Binder) & binder, const Handle(TopTools_HSequenceOfShape) & shapes) {
...
void TransferBRep::SetShapeResult(const Handle(Transfer_TransientProcess) & TP, const Handle(Standard_Transient) & ent,
...
TopAbs_Orientation TransferBRep::ShapeState(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& shape) {
...
Handle(Standard_Transient) TransferBRep::TransientFromShape(const Handle(Transfer_FinderProcess) & FP,
                                                            const TopoDS_Shape& shape) {
...
void TransferBRep::SetTransientFromShape(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& shape,
...
static void FillInfo(const Handle(Transfer_Binder) & Binder, const Handle(Interface_Check) & Check,
...
Handle(TColStd_HSequenceOfTransient) TransferBRep::Checked(const Interface_CheckIterator& chl,
    ...
    for (chl.Start(); chl.More(); chl.Next()) {
        ...
        if (!alsoshapes) {
            if (ent->IsKind(STANDARD_TYPE(TransferBRep_BinderOfShape)) || ent->IsKind(STANDARD_TYPE(TopoDS_HShape)) ||
...
Interface_CheckIterator TransferBRep::CheckObject(const Interface_CheckIterator& chl,
    ...
    for (chl.Start(); chl.More(); chl.Next()) {
        ...
        if (S.IsNull()) {
            if (ent == obj) {
                Handle(Interface_Check) bch(ach);
                bch->SetEntity(ent);
                nchl.Add(bch, 0);
            }
        } else {
...
void TransferBRep::PrintResultInfo(const Handle(Message_Printer) & Printer, const Message_Msg& Header,
