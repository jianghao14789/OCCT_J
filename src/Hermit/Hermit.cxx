...

{
    ...
    min = 0;
    max = 0; // initialisation of the indices
...
static void PolyTest(const TColStd_Array1OfReal& Herm, const Handle(Geom_BSplineCurve) & BS, Standard_Real& U4,
                     Standard_Real& U5, Standard_Integer& boucle, const Standard_Real TolPoles,
                     //		     const Standard_Real                 TolKnots,
                     const Standard_Real, const Standard_Real Ux, const Standard_Real Uy)

{
    ...
    if (Ux != 1.0) {
        ...
        if (Uy != 0.0) BS->LocateU(Uy, 0.0, I3, I4);
    }
        ...
        if (((Polemax) >= ((1 / TolPoles) * Polemin)) || ((Polemin == 0.0) && (Polemax >= (1 / TolPoles)))) {
            ...
            if ((max == 0) || (max == 3)) {
                ...
                if ((min == 0) || (min == 3)) {
                    ...
                    if ((1 / TolPoles * Polemin > Polesinit(0).Y()) && (1 / TolPoles * Polemin > Polesinit(3).Y()) &&
                        (mark == 0)) {
                        for (i = 0; i <= 3; i++)
                            Polesinit(i).SetCoord(0.0, (Polesinit(i).Y() - 1 / TolPoles * Polemin));
                        mark = 1;
                    ...
                    if (mark == 0) {
                        ...
                        if (Pole0 < 3) {
                            ...
                            if (boucle == 1) {
                                for (i = 0; i <= 3; i++)
                                    Polesinit(i).SetCoord(0.0,
                                                          Polesinit(i).Y() -
                                                              (Pole0 * (Pow(10.0, (a / 2.0 + 0.5 * Log10(TolPoles))))));
                                dercas = 1;
                            }
                        }
                        if (Pole0 > Pole3) {
                            ...
                            if (boucle == 1) {
                                for (i = 0; i <= 3; i++)
                                    Polesinit(i).SetCoord(0.0,
                                                          Polesinit(i).Y() -
                                                              (Pole3 * (Pow(10.0, (a / 2.0 + 0.5 * Log10(TolPoles))))));
                                dercas = 1;
                            }
                        }
                    }
    ...
    if ((Polesinit(1).Y() < 0.0) && (Polesinit(2).Y() >= 0.0)) {
        ...
        if (boucle == 2) Us1 = Us1 * knots(2);
        if (boucle == 1)
            if (Ux != 0.0) Us1 = Us1 * Ux;
    ...
    if ((Polesinit(1).Y() >= 0.0) && (Polesinit(2).Y() < 0.0)) {
        ...
        if (boucle == 2) Us2 = knots(knots.Length() - 1) + Us2 * (1 - knots(knots.Length() - 1));
        if (boucle == 1)
            if (Ux != 0.0) Us2 = Uy + Us2 * (1 - Uy);
...
static void PolyTest(const TColStd_Array1OfReal& Herm, const Handle(Geom2d_BSplineCurve) & BS, Standard_Real& U4,
                     Standard_Real& U5, Standard_Integer& boucle, const Standard_Real TolPoles,
                     //		     const Standard_Real                TolKnots,
                     const Standard_Real, const Standard_Real Ux, const Standard_Real Uy)
...
static void InsertKnots(Handle(Geom2d_BSplineCurve) & BS, const Standard_Real U4, const Standard_Real U5)
...
Handle(Geom2d_BSplineCurve) Hermit::Solution(const Handle(Geom_BSplineCurve) & BS, const Standard_Real TolPoles,
                                             const Standard_Real TolKnots)

{
    ...
    PolyTest(Herm, BS, Upos1, Upos2, zeroboucle, Precision::Confusion(), Precision::Confusion(), 1.0,
             0.0);                  // computation of the positivity knots
    ...
    if ((BS2->Knot(2) < TolKnots) || (BS2->Knot(BS2->NbKnots() - 1) > (1 - TolKnots))) // checking of the knots
                                                                                       // tolerance
        throw Standard_DimensionError("Hermit Impossible Tolerance");
    else {
...
Handle(Geom2d_BSplineCurve) Hermit::Solution(const Handle(Geom2d_BSplineCurve) & BS, const Standard_Real TolPoles,
                                             const Standard_Real TolKnots)
...
void Hermit::Solutionbis(const Handle(Geom_BSplineCurve) & BS, Standard_Real& Knotmin, Standard_Real& Knotmax,
                         const Standard_Real TolPoles, const Standard_Real TolKnots)
