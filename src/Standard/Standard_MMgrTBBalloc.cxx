...
Standard_Address Standard_MMgrTBBalloc::Allocate(const Standard_Size aSize) {
    ...
    Standard_Address aPtr = (myClear ? scalable_calloc(aRoundSize, sizeof(char)) : scalable_malloc(aRoundSize));
    if (!aPtr) throw Standard_OutOfMemory("Standard_MMgrTBBalloc::Allocate(): malloc failed");
    return aPtr;
}
...
Standard_Address Standard_MMgrTBBalloc::Reallocate(Standard_Address theStorage, const Standard_Size theSize) {
    // the size is rounded up to 4 since some OCC classes
    // (e.g. TCollection_AsciiString) assume memory to be double word-aligned
    ...
    if (!newStorage) throw Standard_OutOfMemory("Standard_MMgrTBBalloc::Reallocate(): realloc failed");
