...
//! Macros have one parameter for both, key and the value. It is a field of the current class. Macro has internal
//! analyzer that uses the variable name to generate key. If the parameter has prefix symbols "&", "*" and "my", it is
//! cut.
//!
//! - OCCT_DUMP_FIELD_VALUE_NUMERICAL. Use it for fields of numerical C++ types, like int, float, double. It creates a
//! pair "key", "value",
...
//! - OCCT_DUMP_FIELD_VALUE_POINTER. Use it for pointer fields. It creates a pair "key", "value", where the value is the
//! pointer address,
//! - OCCT_DUMP_FIELD_VALUES_DUMPED. Use it for fields that has own Dump implementation. It expects the pointer to the
//! class instance.
//!     It creates "key": { result of dump of the field }
//! - OCCT_DUMP_FIELD_VALUES_DUMPED_INC. Use it for fields that has own Dump implementation. It expects the pointer to
//! the class instance.
...
//!     It creates massive of values ["value_1", "value_2", ...]
//! - OCCT_DUMP_BASE_CLASS. Use if Dump implementation of the class is virtual, to perform ClassName::Dump() of the
//! parent class,
...
//! It's possible to use it without necessity of OCCT_DUMP_CLASS_BEGIN call, but pay attention that it should be only
//! one row in the object dump.
#define OCCT_INIT_FIELD_VALUE_REAL(theOStream, theStreamPos, theField)                                                 \
    {                                                                                                                  \
        Standard_Integer aStreamPos = theStreamPos;                                                                    \
        if (!Standard_Dump::ProcessFieldName(theOStream, #theField, aStreamPos)) return Standard_False;                \
...
#define OCCT_INIT_FIELD_VALUES_DUMPED(theSStream, theStreamPos, theField)                                              \
    {                                                                                                                  \
        if ((theField) == NULL || !(theField)->InitFromJson(theSStream, theStreamPos)) return Standard_False;          \
    }
...
#define OCCT_INIT_VECTOR_CLASS(theOStream, theName, theStreamPos, theCount, ...)                                       \
    {                                                                                                                  \
        Standard_Integer aStreamPos = theStreamPos;                                                                    \
        if (!Standard_Dump::ProcessStreamName(theOStream, theName, aStreamPos)) return Standard_False;                 \
        if (!Standard_Dump::InitRealValues(theOStream, aStreamPos, theCount, __VA_ARGS__)) return Standard_False;      \
        theStreamPos = aStreamPos;                                                                                     \
    }
...
public:
    ...
    //! - for ',' append after '\n' and indent to the next value. If the current symbol is in massive container [], do
    //! nothing Covers result with opened and closed brackets on the top level, if it has no symbols there.
    //! @param theStream source value
    ...
    //!
    //! The two level stream example: 'key_1: value_1, key_2: value_2, key_3: {sublevel_key_1: sublevel_value_1}, key_4:
    //! value_4' In output values contain 'key_1: value_1', 'key_2: value_2', 'key_3: {sublevel_key_1:
    //! sublevel_value_1}' and 'key_4: value_4'. The sublevel value might be processed later using the same method.
    //!
    //! @param theStreamStr stream value
    //! @param theKeyToValues [out] container of split values. It contains key to value and position of the value in the
    //! stream text
    Standard_EXPORT static Standard_Boolean
    SplitJson(const TCollection_AsciiString& theStreamStr,
    ...
    Standard_EXPORT static void DumpKeyToClass(Standard_OStream& theOStream, const TCollection_AsciiString& theKey,
...
private:
    ...
    //! at beginning theIndex is the position of the quota before <key>, after the index is the next position after the
    //! value splitDumped(aString) gives theSplitValue = "abc", theTailValue = "defg", theKey = "key"
    Standard_EXPORT static Standard_Boolean
    splitKeyToValue(const TCollection_AsciiString& theStreamStr, Standard_Integer theStartIndex,
    ...
    Standard_EXPORT static Standard_Boolean jsonKey(const TCollection_AsciiString& theStreamStr,
                                                    Standard_Integer theStartIndex, Standard_Integer& theNextIndex,
