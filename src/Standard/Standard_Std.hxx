...
template <bool Test, class Type = void> struct enable_if { // type is undefined for assumed !_Test
};

template <class _Type> struct enable_if<true, _Type> { // type is _Type for _Test
    typedef _Type type;
};

template <bool Condition, typename TypeTrue, typename TypeFalse> struct conditional {
    typedef TypeTrue type;
};

template <typename TypeTrue, typename TypeFalse> struct conditional<false, TypeTrue, TypeFalse> {
...
template <class T1, class T2, class Dummy = void> struct is_base_but_not_same : opencascade::std::is_base_of<T1, T2> {};
...
struct is_base_but_not_same<T1, T2,
...
struct is_integer : std::integral_constant<bool, opencascade::std::is_integral<TheInteger>::value &&
...
template <typename TheType> struct disable_deduction {
