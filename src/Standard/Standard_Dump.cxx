...
void Standard_Dump::AddValuesSeparator(Standard_OStream& theOStream) {
    ...
    if (!aStreamStr.IsEmpty() && !aStreamStr.EndsWith("{") && !aStreamStr.EndsWith(", ")) theOStream << ", ";
}
...
Standard_Boolean Standard_Dump::ProcessStreamName(const TCollection_AsciiString& theStreamStr,
    ...
    if (theStreamStr.IsEmpty()) return Standard_False;

    if (theStreamStr.Length() < theStreamPos) return Standard_False;
    ...
    if (aResult) theStreamPos += aKeyName.Length();
...
Standard_Boolean Standard_Dump::InitValue(const TCollection_AsciiString& theStreamStr, Standard_Integer& theStreamPos,
    ...
    theStreamPos =
        aNextPos ? (theStreamPos + (aNextPos - aStreamPos) + JsonKeyLength(aNextKey)) : theStreamStr.Length();
    return Standard_True;
}
...
TCollection_AsciiString Standard_Dump::GetPointerInfo(const Handle(Standard_Transient) & thePointer,
    ...
    if (thePointer.IsNull()) return TCollection_AsciiString();
...
TCollection_AsciiString Standard_Dump::GetPointerInfo(const void* thePointer, const bool isShortInfo) {
    if (!thePointer) return TCollection_AsciiString();

    std::ostringstream aPtrStr;
    aPtrStr << thePointer;
    if (!isShortInfo) return aPtrStr.str().c_str();
...
Standard_Dump::SplitJson(const TCollection_AsciiString& theStreamStr,
    ...
    while (aNextIndex < theStreamStr.Length()) {
        ...
        if (!jsonKey(theStreamStr, aNextIndex, aNextIndex, aKey)) return Standard_False;
        ...
        if (!aProcessed) return Standard_False;
    }
    return Standard_True;
}
...
NCollection_List<Standard_Integer> Standard_Dump::HierarchicalValueIndices(
    ...
    for (Standard_Integer anIndex = 1; anIndex <= theValues.Extent(); anIndex++) {
        if (HasChildKey(theValues.FindFromIndex(anIndex))) anIndices.Append(anIndex);
    }
    return anIndices;
}
...
Standard_Dump::splitKeyToValue(const TCollection_AsciiString& theStreamStr, Standard_Integer theStartIndex,
    ...
    if (aCloseIndex == 0) return Standard_False;
    ...
    Standard_JsonKey aKey = Standard_JsonKey_None;
    if (!jsonKey(theStreamStr, aStartIndex, aCloseIndex, aKey)) return Standard_False;
    ...
    switch (aKey) {
        case Standard_JsonKey_OpenChild: {
            ...
            if (aCloseIndex > aStartIndex) aSplitValue = theStreamStr.SubString(aStartIndex, aCloseIndex);
            theNextIndex = aCloseIndex + 1;
            break;
        }
        case Standard_JsonKey_OpenContainer: {
            aCloseIndex = nextClosePosition(theStreamStr, aStartIndex, Standard_JsonKey_OpenContainer,
                                            Standard_JsonKey_CloseContainer);
            if (aCloseIndex > aStartIndex) aSplitValue = theStreamStr.SubString(aStartIndex, aCloseIndex - 1);
            theNextIndex = aCloseIndex + 1;
            break;
        }
        case Standard_JsonKey_Quote: {
        ...
        case Standard_JsonKey_None: {
            if (aStartIndex == theStreamStr.Length()) {
                ...
                Standard_Integer aCloseIndex1 =
                    nextClosePosition(theStreamStr, aStartIndex, Standard_JsonKey_None, Standard_JsonKey_CloseChild) -
                    1;
                Standard_Integer aCloseIndex2 = nextClosePosition(theStreamStr, aStartIndex, Standard_JsonKey_None,
...
Standard_Boolean Standard_Dump::jsonKey(const TCollection_AsciiString& theStreamStr, Standard_Integer theStartIndex,
    ...
    for (Standard_Integer aKeyId = (Standard_Integer)Standard_JsonKey_OpenChild;
        ...
        if (!aSubStreamStr.StartsWith(aKeyToStr)) continue;
