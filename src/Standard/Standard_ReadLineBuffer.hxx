...
public:
    ...
    Standard_ReadLineBuffer(size_t theMaxBufferSizeBytes)
        : myUseReadBufferLastStr(false), myIsMultilineMode(false), myToPutGapInMultiline(true), myBufferPos(0),
          myBytesLastRead(0) {
    ...
    template <typename Stream_T> const char* ReadLine(Stream_T& theStream, size_t& theLineLength) {
        int64_t aReadData = 0;
    ...
    //! @param theStream [inout] - the stream to read from.
    //! @param theLineLength [out] - output parameter defined length of returned line.
    //! @param theReadData   [out] - output parameter defined the number of elements successfully read from the stream
    //! during this call,
            ...
            while (myBufferPos < myBytesLastRead) {
                if (myIsMultilineMode && myReadBuffer[myBufferPos] == '\\') {
                    ...
                    if (myBufferPos + 1 == myBytesLastRead ||
                        ...
                        if (myUseReadBufferLastStr) {
                            myReadBufferLastStr.insert(myReadBufferLastStr.end(), myReadBuffer.begin() + aStartLinePos,
            ...
            if (isEndLineFound) {
                ...
                if (aStartLinePos != myBufferPos) {
                    if (myUseReadBufferLastStr) {
                        ...
                        myReadBufferLastStr =
                            std::vector<char>(myReadBuffer.begin() + aStartLinePos, myReadBuffer.begin() + myBufferPos);
                        myUseReadBufferLastStr = true;
                    }
                }
            }
