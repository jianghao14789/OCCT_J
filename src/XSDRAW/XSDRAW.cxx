...
void XSDRAW::LoadDraw(Draw_Interpretor& theCommands) {
    ...
    for (i = 1; i <= nb; i++) {
        ...
        char help[200];
        com = list->Value(i);
        if (!theolds.IsEmpty()) theolds.Find(com, num);
        if (num == 0) continue;
        if (!IFSelect_Activator::Select(com.ToCString(), nact, act))
            Sprintf(help, "type :  xhelp %s for help", com.ToCString());
        else if (!act.IsNull())
            strcpy(help, act->Help(nact));
        if (num < 0)
            theCommands.Add(com.ToCString(), help, XSTEPDRAWRUN, act->Group());
        else
            theCommands.Add(thenews->Value(num).ToCString(), help, XSTEPDRAWRUN, act->Group());
    }
}
...
void XSDRAW::SetTransferProcess(const Handle(Standard_Transient) & ATP) {
    ...
    if (!TP.IsNull()) {
        if (!TP->Model().IsNull() && TP->Model() != Session()->Model()) Session()->SetModel(TP->Model());
...
Handle(TColStd_HSequenceOfTransient) XSDRAW::GetList(const Standard_CString first, const Standard_CString second) {
...
Standard_Boolean XSDRAW::FileAndVar(const Standard_CString file, const Standard_CString var, const Standard_CString def,
...
Standard_Integer XSDRAW_WHAT(const Handle(Standard_Transient) & ent) {
    ...
    if (model.IsNull()) {
        std::cout << "(No model)  Type:" << ent->DynamicType()->Name() << std::endl;
        return 0;
    }
    std::cout << " Num/Id :";
