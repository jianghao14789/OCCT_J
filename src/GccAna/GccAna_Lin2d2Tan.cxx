...
//   ===============================                                      +
//=========================================================================
GccAna_Lin2d2Tan::GccAna_Lin2d2Tan(const gp_Pnt2d& ThePoint1, const gp_Pnt2d& ThePoint2, const Standard_Real Tolerance)
    : linsol(1, 1), qualifier1(1, 1), qualifier2(1, 1), pnttg1sol(1, 1), pnttg2sol(1, 1), par1sol(1, 1), par2sol(1, 1),
      pararg1(1, 1), pararg2(1, 1) {
...
GccAna_Lin2d2Tan::GccAna_Lin2d2Tan(const GccEnt_QualifiedCirc& Qualified1, const gp_Pnt2d& ThePoint,
                                   const Standard_Real Tolerance)
    : linsol(1, 2), qualifier1(1, 2), qualifier2(1, 2), pnttg1sol(1, 2), pnttg2sol(1, 2), par1sol(1, 2), par2sol(1, 2),
      pararg1(1, 2), pararg2(1, 2) {
...
GccAna_Lin2d2Tan::GccAna_Lin2d2Tan(const GccEnt_QualifiedCirc& Qualified1, const GccEnt_QualifiedCirc& Qualified2,
                                   const Standard_Real Tolerance)
    : linsol(1, 4), qualifier1(1, 4), qualifier2(1, 4), pnttg1sol(1, 4), pnttg2sol(1, 4), par1sol(1, 4), par2sol(1, 4),
      pararg1(1, 4), pararg2(1, 4) {
        ...
        if (Tol < Max(R1, R2) - dist - Min(R1, R2)) {
            ...
            if (Qualified1.IsEnclosing() && Qualified2.IsEnclosing()) {
                ...
                if (Abs(dist + Min(R1, R2) - Max(R1, R2)) <= Tol && dist >= Tol) {
                    ...
                    pnttg1sol(1) = P1;
                    pnttg2sol(1) = gp_Pnt2d(C2.Location().XY() + gp_XY(R2 * D1.Y(), -R2 * D1.X()));
                    WellDone = Standard_True;
                    NbrSol = 1;
                }
                ...
                if (R1 + R2 - dist > Tol) {
                    ...
                    qualifier2(1) = Qualified1.Qualifier();
                    pnttg1sol(1) = P1;
                    pnttg2sol(1) = gp_Pnt2d(C2.Location().XY() + signe * (gp_XY(-R2 * D1.Y(), R2 * D1.X())));
                    WellDone = Standard_True;
                    NbrSol = 1;
                }
                ...
                if ((Qualified1.IsUnqualified() && Qualified2.IsEnclosing()) ||
                    ...
                    if (Abs(dist + Min(R1, R2) - Max(R1, R2)) < Tol && dist > Tol) {
                        ...
                        if (Min(R1, R2) + Max(R1, R2) < dist) {
                            ...
                            qualifier2(1) = Qualified1.Qualifier();
                            pnttg1sol(2) = P1;
                            pnttg2sol(2) = gp_Pnt2d(C2.Location().XY() + (gp_XY(-R2 * D2.Y(), R2 * D2.X())));
                            NbrSol = 2;
                        }
                    }
                    ...
                    if (Abs(dist + Min(R1, R2) - Max(R1, R2)) <= Tol && dist >= Tol) {
                        ...
                        if (Min(R1, R2) + Max(R1, R2) < dist) {
                            ...
                            qualifier2(1) = Qualified1.Qualifier();
                            pnttg1sol(2) = P2;
                            pnttg2sol(2) = gp_Pnt2d(C2.Location().XY() + signe * (gp_XY(R2 * D2.Y(), -R2 * D2.X())));
                            NbrSol = 2;
                        }
                    }
                }
            ...
            for (Standard_Integer i = 1; i <= 2; i++) {
                ...
                pnttg1sol(NbrSol) = P1;
                pnttg2sol(NbrSol) = gp_Pnt2d(C2.Location().XY() + signe * (gp_XY(-R2 * D1.Y(), R2 * D1.X())));
                WellDone = Standard_True;
                if (Min(R1, R2) + Max(R1, R2) < dist) {
                    gp_Pnt2d P2(C2.Location().Rotated(C1.Location(), ASin(signe * (R2 + R1) / dist)));
                    gp_Dir2d D2(gp_Vec2d(C1.Location(), P2));
                    ...
                    qualifier2(NbrSol) = Qualified1.Qualifier();
                    pnttg1sol(NbrSol) = P2;
                    pnttg2sol(NbrSol) = gp_Pnt2d(C2.Location().XY() + signe * (gp_XY(-R2 * D2.Y(), R2 * D2.X())));
                }
            }
        }
    }
...
void GccAna_Lin2d2Tan::WhichQualifier(const Standard_Integer Index, GccEnt_Position& Qualif1,
...
void GccAna_Lin2d2Tan::Tangency1(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
...
void GccAna_Lin2d2Tan::Tangency2(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
                                 gp_Pnt2d& PntSol) const {
    if (!WellDone) {
        throw StdFail_NotDone();
    } else if (Index <= 0 || Index > NbrSol) {
