...
// Fill the fields.                                                      +
//========================================================================
GccAna_Circ2d2TanRad::GccAna_Circ2d2TanRad(const GccEnt_QualifiedLin& Qualified1, const gp_Pnt2d& Point2,
                                           const Standard_Real Radius, const Standard_Real Tolerance)
    : qualifier1(1, 2), qualifier2(1, 2), TheSame1(1, 2), TheSame2(1, 2), cirsol(1, 2), pnttg1sol(1, 2),
    ...
    if (Radius < 0.0) {
        ...
        if (nbsol == 1) {
            if (displ1 < 1.e-10) {
                ...
                for (Standard_Integer jcote = 1; jcote <= nbcote; jcote++) {
                    gp_Lin2d linint(gp_Pnt2d(lxloc - cote(jcote) * ydir * Radius, lyloc + cote(jcote) * xdir * Radius),
                                    L1.Direction());
                    ...
                    if (Intp.IsDone()) {
                        if (!Intp.IsEmpty()) {
                            for (Standard_Integer i = 1; i <= Intp.NbPoints() && NbrSol < 2; i++) {
                                ...
                                TheSame1(NbrSol) = 0;
                                TheSame2(NbrSol) = 0;
                                pnttg1sol(NbrSol) = gp_Pnt2d(Center.XY() + cote(jcote) * Radius * gp_XY(ydir, -xdir));
                                pnttg2sol(NbrSol) = Point2;
                            }
                        }
                        WellDone = Standard_True;
