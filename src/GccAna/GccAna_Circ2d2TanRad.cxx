...
GccAna_Circ2d2TanRad::GccAna_Circ2d2TanRad(const GccEnt_QualifiedCirc& Qualified1,
                                           const GccEnt_QualifiedCirc& Qualified2, const Standard_Real Radius,
                                           const Standard_Real Tolerance)
    : qualifier1(1, 8), qualifier2(1, 8), TheSame1(1, 8), TheSame2(1, 8), cirsol(1, 8), pnttg1sol(1, 8),
      pnttg2sol(1, 8), par1sol(1, 8), par2sol(1, 8), pararg1(1, 8), pararg2(1, 8) {
        ...
        if ((Qualified1.IsEnclosed()) && Qualified2.IsEnclosed()) {
            ...
            if ((Radius - R1 > Tol) || (dist - R2 - R1 > Tol) ||
                ...
                if (((Radius - R1 > 0.0) && (Abs(dist - R1 - R2) <= Tol)) ||
                    (Abs(R1 - R2 + dist - Radius * 2.0) <= Tol) || (Abs(R1 - R2 - dist - Radius * 2.0) <= Tol) ||
                    (Abs(dist - R1 - R2) <= Tol)) {
            ...
            if ((Tol < Max(R1, R2) - Radius) || (Tol < Max(R1, R2) - dist - Min(R1, R2)) ||
                ...
                if ((Abs(dist + Min(R1, R2) - Max(R1, R2)) <= Tol) || (Abs(R1 + R2 + dist - 2.0 * Radius) <= Tol)) {
...
void GccAna_Circ2d2TanRad::WhichQualifier(const Standard_Integer Index, GccEnt_Position& Qualif1,
...
void GccAna_Circ2d2TanRad::Tangency1(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
...
void GccAna_Circ2d2TanRad::Tangency2(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
                                     gp_Pnt2d& PntSol) const {
    if (!WellDone) {
        throw StdFail_NotDone();
    } else if (Index <= 0 || Index > NbrSol) {
