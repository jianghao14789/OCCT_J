...
//=========================================================================
GccAna_Circ2dTanOnRad::GccAna_Circ2dTanOnRad(const GccEnt_QualifiedLin& Qualified1, const gp_Circ2d& OnCirc,
    ...
    if (Radius < 0.0) {
        ...
        if ((dist1 - Radius > Tol) || (Tol < Radius - dist2)) {
            ...
            for (Standard_Integer j = 1; j <= nbsol; j++) {
                sign = -sign;
                gp_Lin2d L(gp_Pnt2d(origin1.X() - sign * Radius * dir1.Y(), origin1.Y() + sign * Radius * dir1.X()),
                           dir1);
                    ...
                    if (!Intp.IsEmpty()) {
                        for (Standard_Integer i = 1; i <= Intp.NbPoints(); i++) {
                            ...
                            pnttg1sol(NbrSol) = gp_Pnt2d(pntcen3(NbrSol).XY() +
                                                         gp_XY(sign * Radius * dir1.Y(), -sign * Radius * dir1.X()));
