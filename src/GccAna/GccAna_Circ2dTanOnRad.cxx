...
GccAna_Circ2dTanOnRad::GccAna_Circ2dTanOnRad(const GccEnt_QualifiedCirc& Qualified1, const gp_Lin2d& OnLine,
                                             const Standard_Real Radius, const Standard_Real Tolerance)
    : cirsol(1, 4), qualifier1(1, 4), TheSame1(1, 4), pnttg1sol(1, 4), pntcen3(1, 4), par1sol(1, 4), pararg1(1, 4),
      parcen3(1, 4) {
    ...
    if (Radius < 0.0) {
        ...
        if (signe != 0) {
            ...
            for (Standard_Integer j = 1; j <= nbsol; j++) {
                ...
                Roots Sol((B * B + A * A) / (A * A), 2.0 * (B * C / (A * A) + (B / A) * x0 - y0),
                ...
                if (Sol.IsDone()) {
                    for (Standard_Integer i = 1; i <= Sol.NbSolutions(); i++) {
                        ...
                        if (OnLine.Distance(Center) > Tol) continue;
                        NbrSol++;
...
void GccAna_Circ2dTanOnRad::Tangency1(const Standard_Integer Index, Standard_Real& ParSol, Standard_Real& ParArg,
