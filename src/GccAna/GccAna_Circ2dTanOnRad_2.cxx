...
GccAna_Circ2dTanOnRad::GccAna_Circ2dTanOnRad(const gp_Pnt2d& Point1, const gp_Lin2d& OnLine, const Standard_Real Radius,
    ...
    if (Radius < 0.0) {
        ...
        if (Abs(dp1lin - Radius) < Tol) {
            ...
            if (A != 0.0) {
                Roots Sol((B * B + A * A) / (A * A), 2.0 * (B * C / (A * A) + (B / A) * x1 - y1),
