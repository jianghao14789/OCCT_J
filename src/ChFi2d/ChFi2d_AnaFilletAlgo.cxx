...
ChFi2d_AnaFilletAlgo::ChFi2d_AnaFilletAlgo()
    : segment1(Standard_False), x11(0.0), y11(0.0), x12(0.0), y12(0.0), xc1(0.0), yc1(0.0), radius1(0.0),
      cw1(Standard_False), segment2(Standard_False), x21(0.0), y21(0.0), x22(0.0), y22(0.0), xc2(0.0), yc2(0.0),
      radius2(0.0), cw2(Standard_False) {}
...
ChFi2d_AnaFilletAlgo::ChFi2d_AnaFilletAlgo(const TopoDS_Wire& theWire, const gp_Pln& thePlane)
    : plane(thePlane), segment1(Standard_False), x11(0.0), y11(0.0), x12(0.0), y12(0.0), xc1(0.0), yc1(0.0),
      radius1(0.0), cw1(Standard_False), segment2(Standard_False), x21(0.0), y21(0.0), x22(0.0), y22(0.0), xc2(0.0),
      yc2(0.0), radius2(0.0), cw2(Standard_False) {
...
// - segment
// - arc of circle.
ChFi2d_AnaFilletAlgo::ChFi2d_AnaFilletAlgo(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
...
void ChFi2d_AnaFilletAlgo::Init(const TopoDS_Wire& theWire, const gp_Pln& thePlane) {
    ...
    if (v1.IsNull() || v2.IsNull()) throw Standard_Failure("An infinite edge.");
...
void ChFi2d_AnaFilletAlgo::Init(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2, const gp_Pln& thePlane) {
    // Make a wire consisting of two edges.
    ...
    if (v11.IsNull() || v12.IsNull() || v21.IsNull() || v22.IsNull()) throw Standard_Failure("An infinite edge.");
    ...
    if (is2ndReversed)
        ...
        mkWire.Add(theEdge2);
    if (!mkWire.IsDone()) throw Standard_Failure("Can't make a wire.");
...
Standard_Boolean ChFi2d_AnaFilletAlgo::Perform(const Standard_Real radius) {
    ...
    if (!bRet) return Standard_False;

    // Invert the fillet for left-handed plane.
    if (plane.Position().Direct() == Standard_False) cw = !cw;
    ...
    if (bRet) {
        ...
        if (bRet) {
            ...
            if (segment1) {
                ...
                if (mkSegment1.IsDone()) shrinke1 = mkSegment1.Edge();
            } else {
                BRepBuilderAPI_MakeEdge mkCirc1;
                mkCirc1.Init(AC1.Curve().Curve(), p1, p2);
                if (mkCirc1.IsDone()) shrinke1 = mkCirc1.Edge();
            }
            ...
            if (segment2) {
                ...
                if (mkSegment2.IsDone()) shrinke2 = mkSegment2.Edge();
            } else {
                BRepBuilderAPI_MakeEdge mkCirc2;
                mkCirc2.Init(AC2.Curve().Curve(), p1, p2);
                if (mkCirc2.IsDone()) shrinke2 = mkCirc2.Edge();
            }
...
Standard_Boolean ChFi2d_AnaFilletAlgo::SegmentFilletSegment(const Standard_Real radius, Standard_Real& xc,
                                                            Standard_Real& yc, Standard_Boolean& cw,
    ...
    // Check bisectrissa.
    if (bisec.SquareMagnitude() < Precision::SquareConfusion()) return Standard_False;
...
Standard_Boolean ChFi2d_AnaFilletAlgo::SegmentFilletArc(const Standard_Real radius, Standard_Real& xc,
                                                        Standard_Real& yc, Standard_Boolean& cw, Standard_Real& start,
                                                        Standard_Real& end, Standard_Real& xend, Standard_Real& yend) {
    ...
    if (p11.SquareDistance(p12) < gp::Resolution()) return Standard_False;
    ...
    if (fabs(angle) < Precision::Angular()) return Standard_False;
    ...
    if (bisec.SquareMagnitude() < gp::Resolution()) return Standard_False;
...
Standard_Boolean ChFi2d_AnaFilletAlgo::ArcFilletSegment(const Standard_Real radius, Standard_Real& xc,
                                                        Standard_Real& yc, Standard_Boolean& cw, Standard_Real& start,
                                                        Standard_Real& end, Standard_Real& xstart,
    ...
    // Check length of segment.
    if (p12.SquareDistance(p22) < gp::Resolution()) return Standard_False;
    ...
    if (line.Distance(nearPoint) > radius) line.Translate(-2.0 * radius * aD2Vec);

    // Make a circle of radius of the arc +/- fillet radius.
    gp_Ax2d axes(pc1, gp::DX2d());
    gp_Circ2d circ(axes, radius1 + radius);
    if (radius1 > radius && circ.Distance(nearPoint) > radius) circ.SetRadius(radius1 - radius);
...
Standard_Boolean ChFi2d_AnaFilletAlgo::ArcFilletArc(const Standard_Real radius, Standard_Real& xc, Standard_Real& yc,
                                                    Standard_Boolean& cw, Standard_Real& start, Standard_Real& end) {
    // Make points.
    ...
    if (radius1 > radius && c1.Distance(checkp) > radius) c1.SetRadius(radius1 - radius);
    // Arc 2.
    axes.SetLocation(pc2);
    gp_Circ2d c2(axes, radius2 + radius);
    if (radius2 > radius && c2.Distance(checkp) > radius) c2.SetRadius(radius2 - radius);
    ...
    if (extrema.NbExtrema()) {
        ...
        for (i = 1; i <= nb; ++i) {
            ...
            if (d < Precision::Confusion()) {
                ...
                if (fabs(l1 - param1) > Precision::Confusion() && fabs(f2 - param2) > Precision::Confusion()) {
                    found = Standard_True;
