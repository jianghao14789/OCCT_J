...
ChFi2d_FilletAlgo::ChFi2d_FilletAlgo()
    : myStart1(0.0), myEnd1(0.0), myStart2(0.0), myEnd2(0.0), myRadius(0.0), myStartSide(Standard_False),
      myEdgesExchnged(Standard_False), myDegreeOfRecursion(0) {}
...
ChFi2d_FilletAlgo::ChFi2d_FilletAlgo(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2, const gp_Pln& thePlane)
    : myEdge1(theEdge1), myEdge2(theEdge2), myStart1(0.0), myEnd1(0.0), myStart2(0.0), myEnd2(0.0), myRadius(0.0),
      myStartSide(Standard_False), myEdgesExchnged(Standard_False), myDegreeOfRecursion(0) {
...
void ChFi2d_FilletAlgo::Init(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2, const gp_Pln& thePlane) {
    ...
    if (aBAC1.GetType() < aBAC2.GetType()) { // first curve must be more complicated
        myEdge1 = theEdge2;
        myEdge2 = theEdge1;
    ...
    if (aBAC1.GetType() == aBAC2.GetType()) {
        if (myEnd2 - myStart2 < myEnd1 - myStart1) { // first curve must be parametrically shorter
...
static Standard_Boolean IsRadiusIntersected(const Handle(Geom2d_Curve) & theCurve, const Standard_Real theCurveMin,
                                            const double theCurveMax, const gp_Pnt2d theStart, const gp_Pnt2d theEnd,
    ...
    for (a = anInter.NbPoints(); a > 0; a--) {
        ...
        if (aPoint.Distance(theStart) < Precision::Confusion()) {
            if (!theStartConnected) return Standard_True;
        }
        if (aPoint.Distance(theEnd) < Precision::Confusion()) return Standard_True;
        if (gp_Vec2d(aPoint, theStart).IsOpposite(gp_Vec2d(aPoint, theEnd), Precision::Angular())) return Standard_True;
    }
    ...
    for (a = anInter.NbSegments(); a > 0; a--) {
        ...
        Geom2dAPI_ProjectPointOnCurve aProjInt(aPoint, theCurve, theCurveMin, theCurveMax);
        if (aProjInt.NbPoints() && aProjInt.LowerDistanceParameter() < Precision::Confusion()) { // point is on edge
            if (aPoint.Distance(theStart) < Precision::Confusion())
                if (!theStartConnected) return Standard_True;
            if (aPoint.Distance(theEnd) < Precision::Confusion()) return Standard_True;
...
void ChFi2d_FilletAlgo::FillPoint(FilletPoint* thePoint, const Standard_Real theLimit) {
    ...
    for (aValid = Standard_False; !aValid; aParam += aStep) {
        if ((aParam - aStep - theLimit) * (aParam - theLimit) <= 0) break; // limit was exceeded
        aStep *= 2;
        gp_Vec2d aVec;
        myCurve1->D1(aParam, aPoint, aVec);
        if (aVec.SquareMagnitude() < Precision::Confusion()) continue;
    ...
    for (a = aNB; a > 0; a--) {
        if (aPoint.SquareDistance(aProj.Point(a)) < Precision::Confusion()) continue;
        ...
        if (Abs(d - myRadius) < Precision::Confusion()) thePoint->setParam2(aParamProj);
    }
}
...
Standard_Boolean ChFi2d_FilletAlgo::Perform(const Standard_Real theRadius) {
    ...
    for (aCycle = 2, myStartSide = Standard_False; aCycle; myStartSide = !myStartSide, aCycle--) {
        ...
        for (aParam = myStart1 + aStep; aParam < myEnd1 || Abs(myEnd1 - aParam) < Precision::Confusion();
             aParam += aStep) {
            if (!aLeft) {
...
Standard_Boolean ChFi2d_FilletAlgo::ProcessPoint(FilletPoint* theLeft, FilletPoint* theRight,
...
void ChFi2d_FilletAlgo::PerformNewton(FilletPoint* theLeft, FilletPoint* theRight) {
    ...
    for (a = 1; a <= theLeft->getNBValues(); a++) {
        ...
        if (Abs(aA) < Precision::Confusion()) { // linear case
            // std::cout<<"###"<<std::endl;
            if (Abs(aB) > 10e-20) {
                Standard_Real aX0 = -aC / aB; // use extremum
                if (aX0 > theLeft->getParam() && aX0 < theRight->getParam()) ProcessPoint(theLeft, theRight, aX0);
            } else {
                ProcessPoint(theLeft, theRight, theLeft->getParam() + aDX / 2.0); // linear division otherwise
            }
        } else {
            if (Abs(aB) > Abs(aDet * 1000000.)) { // possible floating point operations accurancy errors
                // std::cout<<"*";
                ProcessPoint(theLeft, theRight, theLeft->getParam() + aDX / 2.0); // linear division otherwise
            } else {
                if (aDet > 0) { // two solutions
                    aDet = sqrt(aDet);
                    Standard_Boolean aRes = ProcessPoint(theLeft, theRight, (-aB + aDet) / aA);
                    if (!aRes) aRes = ProcessPoint(theLeft, theRight, (-aB - aDet) / aA);
                    if (!aRes)
                        ProcessPoint(theLeft, theRight, theLeft->getParam() + aDX / 2.0); // linear division otherwise
                } else {
                    // std::cout<<"%%%"<<std::endl;
...
int ChFi2d_FilletAlgo::NbResults(const gp_Pnt& thePoint) {
    ...
    for (; anIter.More(); anIter.Next(), i++) {
        ...
        if (aPoint->hasSolution(myRadius)) nb++;
        delete aPoint;
    } // for
...
TopoDS_Edge ChFi2d_FilletAlgo::Result(const gp_Pnt& thePoint, TopoDS_Edge& theEdge1, TopoDS_Edge& theEdge2,
                                      const int iSolution) {
    TopoDS_Edge aResult;
    gp_Pnt2d aTargetPoint2d;
    Standard_Real aX, aY;
    ...
    for (aNearest = NULL, a = 1; anIter.More(); anIter.Next(), a++) {
        ...
        if (iSolution == iSol) break;
        iSol++;
    } // for

    if (!aNearest) return aResult;

    // create circle edge
    gp_Pnt aCenter =
        ElSLib::PlaneValue(aNearest->getCenter().X(), aNearest->getCenter().Y(), myPlane->Pln().Position());
    ...
    Standard_Boolean aIsOut =
        ((aParam1 < aTargetParam && aParam2 < aTargetParam) || (aParam1 > aTargetParam && aParam2 > aTargetParam));
    if (aParam1 > aParam2) aIsOut = !aIsOut;
    ...
    if (fabs(aStart - anEnd) < Precision::Confusion()) anEnd = aStart + Precision::Confusion();
    // Divide edge
...
FilletPoint::FilletPoint(const Standard_Real theParam) : myParam(theParam), myParam2(0.0) {}
...
Standard_Boolean FilletPoint::calculateDiff(FilletPoint* thePoint) {
    ...
    for (a = 1; a <= myV.Length(); a++) {
        ...
        if (aDiffsSet) {
            if (Abs(aDY / aDX) < Abs(myD.Value(a))) myD.SetValue(a, aDY / aDX);
        } else {
            myD.Append(aDY / aDX);
        }
    } // for
...
void FilletPoint::FilterPoints(FilletPoint* thePoint) {
    ...
    for (a = 1; a <= myV.Length(); a++) {
        ...
        if (aNear) {
            if (myV.Value(a) * thePoint->myV.Value(aNear) > 0) { // the same sign at the same sides of the interval
                if (myV.Value(a) * myD.Value(a) > 0) {
                    if (Abs(myD.Value(a)) > Precision::Confusion()) aNear = 0;
                } else {
        ...
        if (aNear) {
            if (myV.Value(a) * thePoint->myV.Value(aNear) > 0) {
                if ((myV.Value(a) + myD.Value(a) * aDX) * myV.Value(a) > Precision::Confusion() &&
                    (thePoint->myV.Value(aNear) + thePoint->myD.Value(aNear) * aDX) * thePoint->myV.Value(aNear) >
                        Precision::Confusion()) {
                    aNear = 0;
                }
            }
        ...
        if (aNear == 0) { // there is no near: remove it from the list
            myV.Remove(a);
            myD.Remove(a);
            myValid.Remove(a);
            a--;
            ...
            for (b = 1; b <= myNear.Length(); b++) {
                if (myNear.Value(b) == aNear) {
                    if (Abs(aDiffs.Value(b)) < Abs(aDiff)) { // return this 'near'
...
int FilletPoint::hasSolution(const Standard_Real theRadius) {
    Standard_Integer a;
    for (a = 1; a <= myV.Length(); a++) {
        if (Abs(sqrt(Abs(Abs(myV.Value(a)) + theRadius * theRadius)) - theRadius) < Precision::Confusion()) return a;
    }
    return 0;
}
