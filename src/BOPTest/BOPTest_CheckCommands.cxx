...
#include <vector>
//
static void MakeShapeForFullOutput(const TCollection_AsciiString&, const Standard_Integer, const TopTools_ListOfShape&,
                                   Standard_Integer&, Draw_Interpretor&, Standard_Boolean bCurveOnSurf = Standard_False,
                                   Standard_Real aMaxDist = 0., Standard_Real aMaxParameter = 0.);
//
...
void BOPTest::CheckCommands(Draw_Interpretor& theCommands) {
    ...
    //
    theCommands.Add("bopcheck", "use bopcheck Shape [level of check: 0 - 9] [-t]", __FILE__, bopcheck, g);
...
Standard_Integer bopcheck(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (aLevel < (aNbInterfTypes - 1)) {
        di << "Info:\nThe level of check is set to " << aInterfTypes[aLevel] << ", i.e. intersection(s)\n";
            ...
            for (; anIt.More(); anIt.Next()) {
                ...
                switch (aResult.GetCheckStatus()) {
                    ...
                    case BOPAlgo_SelfIntersect: {
                        if (!aSS1.IsNull()) {
                            S1_SelfInt++;
                            if (isL1) MakeShapeForFullOutput(aS1SIBaseName, S1_SelfInt, aLS1, S1_SelfIntAll, di);
                        }
                        if (!aSS2.IsNull()) {
                            S2_SelfInt++;
                            if (isL2) MakeShapeForFullOutput(aS2SIBaseName, S2_SelfInt, aLS2, S2_SelfIntAll, di);
                        }
                    } break;
                    case BOPAlgo_TooSmallEdge: {
                        if (!aSS1.IsNull()) {
                            S1_SmalE++;
                            if (isL1) MakeShapeForFullOutput(aS1SEBaseName, S1_SmalE, aLS1, S1_SmalEAll, di);
                        }
                        if (!aSS2.IsNull()) {
                            S2_SmalE++;
                            if (isL2) MakeShapeForFullOutput(aS2SEBaseName, S2_SmalE, aLS2, S2_SmalEAll, di);
                        }
                    } break;
                    case BOPAlgo_NonRecoverableFace: {
                        if (!aSS1.IsNull()) {
                            S1_BadF++;
                            if (isL1) MakeShapeForFullOutput(aS1BFBaseName, S1_BadF, aLS1, S1_BadFAll, di);
                        }
                        if (!aSS2.IsNull()) {
                            S2_BadF++;
                            if (isL2) MakeShapeForFullOutput(aS2BFBaseName, S2_BadF, aLS2, S2_BadFAll, di);
                        }
                    } break;
                    ...
                    case BOPAlgo_InvalidCurveOnSurface: {
                        if (!aSS1.IsNull()) {
                            ...
                            if (isL1) {
                                ...
                                MakeShapeForFullOutput(aS1COnSBaseName, S1_COnS, aLS1, S1_COnSAll, di, Standard_True,
                                                       aMaxDist, aMaxParameter);
                            }
                        }
                        if (!aSS2.IsNull()) {
                            S2_COnS++;
                            if (isL2) {
                                ...
                                MakeShapeForFullOutput(aS2COnSBaseName, S2_COnS, aLS2, S2_COnSAll, di, Standard_True,
                                                       aMaxDist, aMaxParameter);
                            }
                        }
                    } break;
...
Standard_Integer bopapicheck(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    for (; itF.More(); itF.Next()) {
        ...
        if (isInv1 && isInv2 && isBadOp) break;
    }
...
// purpose  :
//=======================================================================
void MakeShapeForFullOutput(const TCollection_AsciiString& aBaseName, const Standard_Integer aIndex,
                            const TopTools_ListOfShape& aList, Standard_Integer& aCount, Draw_Interpretor& di,
                            Standard_Boolean bCurveOnSurf, Standard_Real aMaxDist, Standard_Real aMaxParameter) {
