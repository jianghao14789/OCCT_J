...
void BOPTest::PartitionCommands(Draw_Interpretor& theCommands) {
    ...
    theCommands.Add("bbuild",
                    "Builds the result of General Fuse operation. Intersection (bfillds) has to be already performed "
                    "by this moment.\n"
...
Standard_Integer bbuild(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(aPF.Arguments(), aBuilder);
...
Standard_Integer bbop(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(aPF.Arguments(), *pBuilder);
...
Standard_Integer bsplit(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(aPF.Arguments(), *pSplitter);
...
Standard_Integer buildbop(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    // Set history of Split operation into the session
    if (BRepTest_Objects::IsHistoryNeeded()) BRepTest_Objects::SetHistory(pDS->Arguments(), *pBuilder);
