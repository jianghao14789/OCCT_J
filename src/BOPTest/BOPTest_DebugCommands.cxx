...
template <class InterfType>
static void SearchNewIndex(const char* theCType, const Standard_Integer theInd,
                           const NCollection_Vector<InterfType>& theVInterf, Draw_Interpretor& di);
static Standard_Integer bopfinfo(Draw_Interpretor& di, Standard_Integer n, const char** a,
...
void BOPTest::DebugCommands(Draw_Interpretor& theCommands) {
    ...
    // Commands
    theCommands.Add("bopds", "Shows the shapes from DS. Use: bopds [v/e/w/f/sh/s/cs/c]", __FILE__, bopds, g);
    theCommands.Add("bopiterator", "Shows the pairs of interfered shapes. Use: bopiterator [type1 type2]", __FILE__,
                    bopiterator, g);
    theCommands.Add("bopinterf", "Shows interferences of given type. Use: bopinterf type1 type2", __FILE__, bopinterf,
                    g);
    theCommands.Add("bopnews", "Shows the newly created shapes. Use: bopnews [v,e,f]", __FILE__, bopnews, g);
    theCommands.Add("bopwho", "Shows where the new shape was created. Use: bopwho #", __FILE__, bopwho, g);
    theCommands.Add("bopindex", "Gets the index of the shape in the DS. Use: bopindex s", __FILE__, bopindex, g);
    ...
    theCommands.Add("boppb", "Shows information about pave blocks. Use: boppb [#e]", __FILE__, boppb, g);
    theCommands.Add("bopcb", "Shows information about common blocks. Use: bopcb [#e]", __FILE__, bopcb, g);
    theCommands.Add("bopsp", "Shows the splits of edges. Use: bopsp [#e]", __FILE__, bopsp, g);
    theCommands.Add("bopfon", "Shows ON information for the face. Use: bopfon #f", __FILE__, bopfon, g);
    theCommands.Add("bopfin", "Shows IN information for the face. Use: bopfin #f", __FILE__, bopfin, g);
    theCommands.Add("bopfsc", "Shows SC information for the face. Use: bopfsc #f", __FILE__, bopfsc, g);
    theCommands.Add("bopfav", "Shows information about alone vertices for the face. Use: bopfav #f", __FILE__, bopfav,
                    g);
    theCommands.Add("bopimage", "Shows split parts of the shape. Use: bopimage s", __FILE__, bopimage, g);
    theCommands.Add("boporigin", "Shows the original shape for the shape. Use: boporigin s", __FILE__, boporigin, g);
    theCommands.Add("bopfsd", "Shows SD faces for the face: Use: bopfsd f", __FILE__, bopfsd, g);
    theCommands.Add("bopbsolid", "Build solids from set of shared faces. Use: bopbsolid sr cx", __FILE__, bopbsolid, g);
    theCommands.Add("bopbface", "Splits the face by set of shared edges. Use: bopbface fr cx", __FILE__, bopbface, g);
}
...
Standard_Integer bopds(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    for (i = 0; i < aNbS; ++i) {
        ...
        if (n == 1) {
            if (aTypeShape == TopAbs_COMPOUND || aTypeShape == TopAbs_COMPSOLID || aTypeShape == TopAbs_SOLID ||
                aTypeShape == TopAbs_SHELL || aTypeShape == TopAbs_WIRE) {
                continue;
            }
        } else {
            if (aTypeShape != aType) {
...
Standard_Integer bopinterf(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    } else if ((aT1 == TopAbs_VERTEX && aT2 == TopAbs_EDGE) || (aT2 == TopAbs_VERTEX && aT1 == TopAbs_EDGE)) {
        di << "V/E: ";
    ...
    } else if ((aT1 == TopAbs_VERTEX && aT2 == TopAbs_FACE) || (aT2 == TopAbs_VERTEX && aT1 == TopAbs_FACE)) {
        di << "V/F: ";
    ...
    } else if ((aT1 == TopAbs_VERTEX && aT2 == TopAbs_SOLID) || (aT2 == TopAbs_VERTEX && aT1 == TopAbs_SOLID)) {
        di << "V/S: ";
        DumpInterfs<BOPDS_InterfVZ>(pDS->InterfVZ(), di);
    } else if ((aT1 == TopAbs_EDGE && aT2 == TopAbs_SOLID) || (aT2 == TopAbs_EDGE && aT1 == TopAbs_SOLID)) {
        di << "E/S: ";
        DumpInterfs<BOPDS_InterfEZ>(pDS->InterfEZ(), di);
    } else if ((aT1 == TopAbs_FACE && aT2 == TopAbs_SOLID) || (aT2 == TopAbs_FACE && aT1 == TopAbs_SOLID)) {
        di << "F/S: ";
...
Standard_Integer bopsc(Draw_Interpretor& di, Standard_Integer n, const char** a) {
    ...
    //
    nSF1 = nSF2 = -1;
    if (n > 1) nSF1 = Draw::Atoi(a[1]);
    if (n > 2) nSF2 = Draw::Atoi(a[2]);
...
// purpose  :
//=======================================================================
Standard_Integer bopfinfo(Draw_Interpretor& di, Standard_Integer n, const char** a, const Standard_Integer iPriz) {
    if (n != 2) {
...
// purpose  :
//=======================================================================
template <class InterfType> void DumpInterfs(const NCollection_Vector<InterfType>& theVInterf, Draw_Interpretor& di) {
