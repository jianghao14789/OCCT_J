...
BRepClass_FaceExplorer::BRepClass_FaceExplorer(const TopoDS_Face& F)
    : myFace(F), myCurEdgeInd(1), myCurEdgePar(Probing_Start), myMaxTolerance(0.1), myUseBndBox(Standard_False),
      myUMin(Precision::Infinite()), myUMax(-Precision::Infinite()), myVMin(Precision::Infinite()),
...
void BRepClass_FaceExplorer::ComputeFaceBounds() {
    ...
    if (Precision::IsInfinite(myUMin) || Precision::IsInfinite(myUMax) || Precision::IsInfinite(myVMin) ||
...
Standard_Boolean BRepClass_FaceExplorer::OtherSegment(const gp_Pnt2d& P, gp_Lin2d& L, Standard_Real& Par) {
            ...
            if (!aC2d.IsNull()) {
                ...
                for (; myCurEdgePar < Probing_End; myCurEdgePar += Probing_Step) {
                    ...
                    if (Par > aTolParConf2) {
                        ...
                        if (aTanMod < aTolParConf2) continue;
                        ...
                        if (Abs(aSinA) < SmallAngle) {
                            ...
                            if (myCurEdgePar + Probing_Step < Probing_End) continue;
                        }
