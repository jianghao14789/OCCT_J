...
//! This class allows the definition of an RGB color as triplet of 3 normalized floating point values (red, green,
//! blue).
//!
...
public:
    ...
    Standard_EXPORT Quantity_Color(const Standard_Real theC1, const Standard_Real theC2, const Standard_Real theC3,
    ...
    Standard_EXPORT void Values(Standard_Real& theC1, Standard_Real& theC2, Standard_Real& theC3,
                                const Quantity_TypeOfColor theType) const;

    //! Updates a color according to the mode specified by theType.
    //! Throws exception if values are out of range.
    Standard_EXPORT void SetValues(const Standard_Real theC1, const Standard_Real theC2, const Standard_Real theC3,
                                   const Quantity_TypeOfColor theType);
    ...
    Standard_Real Saturation() const {
        return Convert_LinearRGB_To_HLS(myRgb)[2];
    }
    ...
    //! Returns the distance between two colors. It's a value between 0 and the square root of 3 (the black/white
    //! distance).
    ...
    Standard_Real SquareDistance(const Quantity_Color& theColor) const {
        return (NCollection_Vec3<Standard_Real>(myRgb) - NCollection_Vec3<Standard_Real>(theColor.myRgb))
            .SquareModulus();
    }
    ...
    Standard_EXPORT void Delta(const Quantity_Color& theColor, Standard_Real& DC, Standard_Real& DI) const;
...
public:
    ...
    static TCollection_AsciiString ColorToHex(const Quantity_Color& theColor, const bool theToPrefixHash = true) {
        ...
        char aBuff[10];
        Sprintf(aBuff, theToPrefixHash ? "#%02X%02X%02X" : "%02X%02X%02X", anSRgbInt.r(), anSRgbInt.g(), anSRgbInt.b());
        return aBuff;
    }
    ...
    //! Convert linear RGB component into sRGB using OpenGL specs formula (double precision), also known as gamma
    //! correction.
    ...
    //! Convert linear RGB component into sRGB using OpenGL specs formula (single precision), also known as gamma
    //! correction.
    ...
    //! Convert sRGB component into linear RGB using OpenGL specs formula (double precision), also known as gamma
    //! correction.
    ...
    //! Convert sRGB component into linear RGB using OpenGL specs formula (single precision), also known as gamma
    //! correction.
    ...
    template <typename T> static NCollection_Vec3<T> Convert_LinearRGB_To_sRGB(const NCollection_Vec3<T>& theRGB) {
        return NCollection_Vec3<T>(Convert_LinearRGB_To_sRGB(theRGB.r()), Convert_LinearRGB_To_sRGB(theRGB.g()),
                                   Convert_LinearRGB_To_sRGB(theRGB.b()));
    }

    //! Convert sRGB components into linear RGB using OpenGL specs formula.
    template <typename T> static NCollection_Vec3<T> Convert_sRGB_To_LinearRGB(const NCollection_Vec3<T>& theRGB) {
        return NCollection_Vec3<T>(Convert_sRGB_To_LinearRGB(theRGB.r()), Convert_sRGB_To_LinearRGB(theRGB.g()),
