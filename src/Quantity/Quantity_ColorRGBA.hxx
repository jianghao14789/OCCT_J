...
public:
    ...
    Quantity_ColorRGBA(const Quantity_Color& theRgb, float theAlpha) : myRgb(theRgb), myAlpha(theAlpha) {}
    ...
    Quantity_ColorRGBA(float theRed, float theGreen, float theBlue, float theAlpha)
        : myRgb(theRed, theGreen, theBlue, Quantity_TOC_RGB), myAlpha(theAlpha) {}
    ...
    bool IsEqual(const Quantity_ColorRGBA& theOther) const {
        return myRgb.IsEqual(theOther.GetRGB()) && Abs(myAlpha - theOther.myAlpha) <= (float)Quantity_Color::Epsilon();
    }
...
public:
    ...
    Standard_EXPORT static bool ColorFromHex(const char* const theHexColorString, Quantity_ColorRGBA& theColor,
    ...
    static TCollection_AsciiString ColorToHex(const Quantity_ColorRGBA& theColor, const bool theToPrefixHash = true) {
        ...
        char aBuff[12];
        Sprintf(aBuff, theToPrefixHash ? "#%02X%02X%02X%02X" : "%02X%02X%02X%02X", anSRgbInt.r(), anSRgbInt.g(),
                anSRgbInt.b(), anSRgbInt.a());
        return aBuff;
    }
...
public:
    ...
    static NCollection_Vec4<float> Convert_LinearRGB_To_sRGB(const NCollection_Vec4<float>& theRGB) {
        return NCollection_Vec4<float>(Quantity_Color::Convert_LinearRGB_To_sRGB(theRGB.r()),
                                       ...
                                       Quantity_Color::Convert_LinearRGB_To_sRGB(theRGB.b()), theRGB.a());
    }
    ...
    static NCollection_Vec4<float> Convert_sRGB_To_LinearRGB(const NCollection_Vec4<float>& theRGB) {
        return NCollection_Vec4<float>(Quantity_Color::Convert_sRGB_To_LinearRGB(theRGB.r()),
                                       ...
                                       Quantity_Color::Convert_sRGB_To_LinearRGB(theRGB.b()), theRGB.a());
    }
...
private:
    static void myTestSize3() {
        Standard_STATIC_ASSERT(sizeof(float) * 3 == sizeof(Quantity_Color));
    }
