...
void IVtkOCC_ShapeMesher::internalBuild() {
    ...
    for (TopExp_Explorer aFaceIter(anOcctShape, TopAbs_FACE); aFaceIter.More(); aFaceIter.Next()) {
        ...
        try {
            ...
            Message::SendFail(
                TCollection_AsciiString("Error: addWireFrameFaces() wireframe presentation builder has failed (") +
...
Standard_Real IVtkOCC_ShapeMesher::GetDeflection() const {
    const TopoDS_Shape& anOcctShape = GetShapeObj()->GetShape();
    return !anOcctShape.IsNull() ? StdPrs_ToolTriangulatedShape::GetDeflection(anOcctShape, GetShapeObj()->Attributes())
                                 : 0.0;
}
...
void IVtkOCC_ShapeMesher::addEdges() {
    ...
    for (; aEdgeIt.More(); aEdgeIt.Next()) {
        ...
        if (aNbFaces == 0) {
            ...
            aType =
                (aNbFaces >= 2) && (BRep_Tool::MaxContinuity(anOcctEdge) > GeomAbs_G2) ? MT_SeamEdge : MT_SharedEdge;
        }
...
// Purpose  :
//================================================================
void IVtkOCC_ShapeMesher::addVertex(const TopoDS_Vertex& theVertex, const IVtk_IdType theShapeId,
...
// Purpose  :
//================================================================
void IVtkOCC_ShapeMesher::addEdge(const TopoDS_Edge& theEdge, const IVtk_IdType theShapeId,
                                  const IVtk_MeshType theMeshType) {
    ...
    if (!aPolyOnTriangulation.IsNull() && aPolyOnTriangulation->NbNodes() >= 2) {
        ...
        for (Standard_Integer aJ = 0; aJ < aNbNodes; aJ++) {
            ...
            IVtk_PointId anId =
                myShapeData->InsertPoint(aPoint, Graphic3d_Vec3((float)aNorm.X(), (float)aNorm.Y(), (float)aNorm.Z()));
...
// Purpose  :
//================================================================
void IVtkOCC_ShapeMesher::addWFFace(const TopoDS_Face& theFace, const IVtk_IdType theShapeId,
