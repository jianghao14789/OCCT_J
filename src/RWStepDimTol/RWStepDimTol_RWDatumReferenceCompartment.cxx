...
void RWStepDimTol_RWDatumReferenceCompartment::ReadStep(const Handle(StepData_StepReaderData) & data,
                                                        ...
                                                        const Handle(StepDimTol_DatumReferenceCompartment) &
                                                            ent) const {
    ...
    if (aType == Interface_ParamIdent) {
        ...
        if (data->ReadSubList(num, 5, "general_datum_reference.base", ach, nbSub)) {
            ...
            for (Standard_Integer i = 1; i <= nbElements; i++) {
                if (data->ReadEntity(nbSub, i, "datum_reference_element", ach,
                                     STANDARD_TYPE(StepDimTol_DatumReferenceElement), anEnt))
    ...
    if (hasModifiers) {
        ...
        for (Standard_Integer i = 1; i <= nbElements; i++) {
            ...
            if (aType == Interface_ParamIdent) {
                ...
                data->ReadEntity(nbSub, i, "datum_reference_modifier_with_value", ach,
                                 STANDARD_TYPE(StepDimTol_DatumReferenceModifierWithValue), aDRMWV);
