...
void RWStepDimTol_RWToleranceZone::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
    ...
    if (data->ReadSubList(num, 5, "defining_tolerance", ach, nbSub)) {
        ...
        for (Standard_Integer i = 1; i <= nbElements; i++) {
            if (data->ReadEntity(nbSub, i, "tolerance_zone_target", ach, anEnt)) anItems->SetValue(i, anEnt);
        }
    }
