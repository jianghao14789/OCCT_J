...
void RWStepDimTol_RWGeometricToleranceWithDefinedAreaUnit::ReadStep(
    ...
    data->ReadEntity(num, 3, "geometric_tolerance.magnitude", ach, STANDARD_TYPE(StepBasic_MeasureWithUnit),
                     aMagnitude);
    ...
    data->ReadEntity(num, 5, "geometric_tolerance_with_defined_unit.unit_size", ach,
                     STANDARD_TYPE(StepBasic_LengthMeasureWithUnit), anUnitSize);
    ...
    if (hasSecondUnitSize)
        data->ReadEntity(num, 7, "second_unit_size", ach, STANDARD_TYPE(StepBasic_LengthMeasureWithUnit),
                         aSecondUnitSize);

    // Initialize entity
    ent->Init(aName, aDescription, aMagnitude, aTolerancedShapeAspect, anUnitSize, aType, hasSecondUnitSize,
              aSecondUnitSize);
}
...
void RWStepDimTol_RWGeometricToleranceWithDefinedAreaUnit::WriteStep(
    StepData_StepWriter& SW, const Handle(StepDimTol_GeometricToleranceWithDefinedAreaUnit) & ent) const {
...
void RWStepDimTol_RWGeometricToleranceWithDefinedAreaUnit::Share(
    const Handle(StepDimTol_GeometricToleranceWithDefinedAreaUnit) & ent, Interface_EntityIterator& iter) const {
