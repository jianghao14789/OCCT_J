...
void RWStepDimTol_RWGeoTolAndGeoTolWthDatRefAndUneqDisGeoTol::ReadStep(
    ...
    // Initialize entity
    Handle(StepDimTol_UnequallyDisposedGeometricTolerance) anUDGT = new StepDimTol_UnequallyDisposedGeometricTolerance;
...
void RWStepDimTol_RWGeoTolAndGeoTolWthDatRefAndUneqDisGeoTol::WriteStep(
    StepData_StepWriter& SW, const Handle(StepDimTol_GeoTolAndGeoTolWthDatRefAndUneqDisGeoTol) & ent) const {
    ...
    SW.OpenSub();
    for (Standard_Integer i4 = 1; i4 <= ent->GetGeometricToleranceWithDatumReference()->DatumSystemAP242()->Length();
         i4++) {
...
void RWStepDimTol_RWGeoTolAndGeoTolWthDatRefAndUneqDisGeoTol::Share(
    const Handle(StepDimTol_GeoTolAndGeoTolWthDatRefAndUneqDisGeoTol) & ent, Interface_EntityIterator& iter) const {
    // Own fields of GeometricTolerance
    iter.AddItem(ent->Magnitude());
    iter.AddItem(ent->TolerancedShapeAspect().Value());
    // Own fields of GeometricToleranceWithDatumReference
    for (Standard_Integer i3 = 1; i3 <= ent->GetGeometricToleranceWithDatumReference()->DatumSystemAP242()->Length();
         i3++) {
        StepDimTol_DatumSystemOrReference Var0 =
