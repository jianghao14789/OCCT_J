...
void RWStepDimTol_RWDatumSystem::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
    ...
    if (data->ReadSubList(num, 5, "base", ach, nbSub)) {
        ...
        for (Standard_Integer i = 1; i <= nbElements; i++) {
            if (data->ReadEntity(nbSub, i, "datum_reference_compartment", ach,
                                 STANDARD_TYPE(StepDimTol_DatumReferenceCompartment), anEnt))
...
void RWStepDimTol_RWDatumSystem::WriteStep(StepData_StepWriter& SW, const Handle(StepDimTol_DatumSystem) & ent) const {
