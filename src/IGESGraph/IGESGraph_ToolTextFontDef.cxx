...
void IGESGraph_ToolTextFontDef::ReadOwnParams(const Handle(IGESGraph_TextFontDef) & ent,
    ...
    if (PR.IsParamEntity(PR.CurrentNumber())) {
        ...
        PR.ReadEntity(IR, PR.Current(), "Text Definition Entity", STANDARD_TYPE(IGESGraph_TextFontDef),
                      supersededEntity); // szv#4:S4163:12Mar99 `st=` not needed
    } else
                ...
                if (tempMotion > 0) {
                    ...
                    for (Standard_Integer j = 1; j <= tempMotion; j++) {
                        if (PR.DefinedElseSkip()) {
                            ...
                            if (PR.ReadInteger(PR.Current(), "array penFlags",
                        ...
                        if (PR.ReadInteger(PR.Current(), "array movePenTo X",
                                           tempMoveX)) // szv#4:S4163:12Mar99 `st=` not needed
                            xarray->SetValue(j, tempMoveX);
                        if (PR.ReadInteger(PR.Current(), "array movePenTo Y",
    ...
    ent->Init(fontCode, fontName, supersededFont, supersededEntity, scale, aSCIICodes, nextCharX, nextCharY, penMotions,
...
void IGESGraph_ToolTextFontDef::OwnShared(const Handle(IGESGraph_TextFontDef) & ent,
    ...
    if (ent->IsSupersededFontEntity()) iter.GetOneItem(ent->SupersededFontEntity());
}
...
void IGESGraph_ToolTextFontDef::OwnCopy(const Handle(IGESGraph_TextFontDef) & another,
    ...
    if (another->IsSupersededFontEntity())
        supersededEntity = Handle(IGESGraph_TextFontDef)::DownCast(TC.Transferred(another->SupersededFontEntity()));
    else
...
IGESData_DirChecker IGESGraph_ToolTextFontDef::DirChecker(const Handle(IGESGraph_TextFontDef) & /*ent*/) const {
...
void IGESGraph_ToolTextFontDef::OwnCheck(const Handle(IGESGraph_TextFontDef) & /*ent*/, const Interface_ShareTool&,
                                         Handle(Interface_Check) & /*ach*/) const {}

void IGESGraph_ToolTextFontDef::OwnDump(const Handle(IGESGraph_TextFontDef) & ent, const IGESData_IGESDumper& dumper,
