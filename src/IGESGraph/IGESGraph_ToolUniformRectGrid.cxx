...
void IGESGraph_ToolUniformRectGrid::ReadOwnParams(const Handle(IGESGraph_UniformRectGrid) & ent,
    ...
    if (nbPropertyValues != 9) PR.AddFail("No. of Property values : Value is not 9");
...
void IGESGraph_ToolUniformRectGrid::OwnCopy(const Handle(IGESGraph_UniformRectGrid) & another,
    ...
    ent->Init(9, (another->IsFinite() ? 1 : 0), (another->IsLine() ? 1 : 0), (another->IsWeighted() ? 0 : 1),
              another->GridPoint().XY(), another->GridSpacing().XY(), another->NbPointsX(), another->NbPointsY());
}

Standard_Boolean IGESGraph_ToolUniformRectGrid::OwnCorrect(const Handle(IGESGraph_UniformRectGrid) & ent) const {
    Standard_Boolean res = (ent->NbPropertyValues() != 9);
    if (res)
        ent->Init(9, (ent->IsFinite() ? 1 : 0), (ent->IsLine() ? 1 : 0), (ent->IsWeighted() ? 0 : 1),
                  ent->GridPoint().XY(), ent->GridSpacing().XY(), ent->NbPointsX(),
                  ent->NbPointsY()); // nbpropertyvalues=9
    return res;
}

IGESData_DirChecker IGESGraph_ToolUniformRectGrid::DirChecker(const Handle(IGESGraph_UniformRectGrid) & /*ent*/) const {
...
void IGESGraph_ToolUniformRectGrid::OwnCheck(const Handle(IGESGraph_UniformRectGrid) & ent, const Interface_ShareTool&,
                                             Handle(Interface_Check) & ach) const {
    if (ent->IsFinite() != 0 && ent->IsFinite() != 1) ach->AddFail("Finite/infinite grid flag : Value != 0/1");
    if (ent->IsLine() != 0 && ent->IsLine() != 1) ach->AddFail("Line/point grid flag : Value != 0/1");
    if (ent->IsWeighted() != 0 && ent->IsWeighted() != 1) ach->AddFail("Weighted/unweighted grid flag : Value != 0/1");
    if (ent->NbPropertyValues() != 9) ach->AddFail("No. of Property values : Value != 9");
}
...
void IGESGraph_ToolUniformRectGrid::OwnDump(const Handle(IGESGraph_UniformRectGrid) & ent,
    ...
    S << "IGESGraph_UniformRectGrid\n"
      ...
      << (ent->IsLine() ? "Lines" : "Points") << "  -  " << (ent->IsWeighted() ? "Weighted" : "Unweighted") << "\n"
      << "Grid Point   : ";
    ...
    S << "\n";
    if (ent->IsFinite())
        S << "No. of points/lines in direction :  X : " << ent->NbPointsX() << "  -  Y : " << ent->NbPointsY()
          << std::endl;
}
