...
void IGESGraph_ToolPick::WriteOwnParams(const Handle(IGESGraph_Pick) & ent, IGESData_IGESWriter& IW) const {
    IW.Send(ent->NbPropertyValues());
    IW.Send(ent->PickFlag());
}

void IGESGraph_ToolPick::OwnShared(const Handle(IGESGraph_Pick) & /*ent*/, Interface_EntityIterator& /*iter*/) const {}

void IGESGraph_ToolPick::OwnCopy(const Handle(IGESGraph_Pick) & another, const Handle(IGESGraph_Pick) & ent,
...
void IGESGraph_ToolPick::OwnCheck(const Handle(IGESGraph_Pick) & ent, const Interface_ShareTool&,
                                  Handle(Interface_Check) & ach) const {
    if (ent->NbPropertyValues() != 1) ach->AddFail("No. of Property values : Value != 1");
    if ((ent->PickFlag() != 0) && (ent->PickFlag() != 1)) ach->AddFail("Pick Flag : Value != 0/1");
}

void IGESGraph_ToolPick::OwnDump(const Handle(IGESGraph_Pick) & ent, const IGESData_IGESDumper& /*dumper*/,
