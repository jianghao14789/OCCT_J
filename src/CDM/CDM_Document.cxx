...
CDM_Document::CDM_Document()
    : myResourcesAreLoaded(Standard_False), myVersion(1), myActualReferenceIdentifier(0), myStorageVersion(0),
      myRequestedComment(""), myRequestedFolderIsDefined(Standard_False), myRequestedNameIsDefined(Standard_False),
      myRequestedPreviousVersionIsDefined(Standard_False), myFileExtensionWasFound(Standard_False),
...
void CDM_Document::Update(const Handle(CDM_Document) & /*aToDocument*/, const Standard_Integer /*aReferenceIdentifier*/,
...
Standard_Integer CDM_Document::CreateReference(const Handle(CDM_Document) & anOtherDocument) {
    ...
    for (; it.More(); it.Next()) {
        if (anOtherDocument == it.Value()->Document()) return it.Value()->ReferenceIdentifier();
    }

    Handle(CDM_Reference) r =
        new CDM_Reference(this, anOtherDocument, ++myActualReferenceIdentifier, anOtherDocument->Modifications());
    AddToReference(r);
...
TCollection_ExtendedString CDM_Document::Name(const Standard_Integer aReferenceIdentifier) const {
    if (!IsStored(aReferenceIdentifier)) throw Standard_DomainError("CDM_Document::Name: document is not stored");
...
void CDM_Document::UpdateFromDocuments(const Standard_Address aModifContext) const {
    ...
    if (StartUpdateCycle) {
        ...
        while (!aListOfDocumentsToUpdate.IsEmpty()) {
            ...
            theApplication->EndOfUpdate(theDocumentToUpdate, theDocumentToUpdate->Update(ErrorString), ErrorString);
...
Standard_Integer CDM_Document::CopyReference(const Handle(CDM_Document) & /*aFromDocument*/,
    ...
    if (!theReference.IsNull()) {
        ...
        if (!theDocument.IsNull()) {
            ...
            return CreateReference(theReference->MetaData(), theReference->Application(),
                                   theReference->DocumentVersion(), theReference->UseStorageConfiguration());
    }
    return 0; // for NT ...
}
...
Standard_ExtString CDM_Document::Comment() const {
    if (myComments.Length() < 1) return 0;
...
TCollection_ExtendedString CDM_Document::RequestedFolder() const {
    Standard_NoSuchObject_Raise_if(!myRequestedFolderIsDefined, "storage folder is undefined for this document");
...
Standard_Boolean CDM_Document::IsOpened(const Standard_Integer aReferenceIdentifier) const {
    ...
    for (; it.More(); it.Next()) {
        if (aReferenceIdentifier == it.Value()->ReferenceIdentifier()) return it.Value()->IsOpened();
    }
    return Standard_False;
}
...
void CDM_Document::CreateReference(const Handle(CDM_MetaData) & aMetaData, const Standard_Integer aReferenceIdentifier,
    ...
    if (aMetaData->IsRetrieved()) {
        ...
        Handle(CDM_Reference) r = new CDM_Reference(this, aMetaData, aReferenceIdentifier, anApplication,
                                                    aToDocumentVersion, UseStorageConfiguration);
        AddToReference(r);
    }
}
...
Standard_Integer CDM_Document::CreateReference(const Handle(CDM_MetaData) & aMetaData,
    ...
    for (; it.More(); it.Next()) {
        if (aMetaData == it.Value()->MetaData()) return it.Value()->ReferenceIdentifier();
    }
    Handle(CDM_Reference) r = new CDM_Reference(this, aMetaData, ++myActualReferenceIdentifier, anApplication,
                                                aDocumentVersion, UseStorageConfiguration);
    AddToReference(r);
    return r->ReferenceIdentifier();
}
...
static void FIND(const Handle(Resource_Manager) & theDocumentResource,
                 const TCollection_ExtendedString& theResourceName, Standard_Boolean& IsDef,
...
Handle(Resource_Manager) CDM_Document::StorageResource() {
    if (myApplication.IsNull()) {
        ...
        aMsg << "this document of format " << StorageFormat() << " has not yet been opened by any application. "
             << std::endl;
...
void CDM_Document::LoadResources() {
    if (!myResourcesAreLoaded) {
        ...
        //    std::cout << "resource Loaded: Format: " << theFormat << ", FileExtension:" << myFileExtension << ",
        //    Description:" << myDescription << std::endl;
    }
    return;
}
