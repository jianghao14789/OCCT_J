...
public:
    ...
    //! if a vector is null).
    Standard_EXPORT GeomLProp_CLProps(const Handle(Geom_Curve) & C, const Standard_Integer N,
                                      const Standard_Real Resolution);

    //! Same as previous constructor but here the parameter is
    //! set to the value <U>.
    //! All the computations done will be related to <C> and <U>.
    Standard_EXPORT GeomLProp_CLProps(const Handle(Geom_Curve) & C, const Standard_Real U, const Standard_Integer N,
                                      const Standard_Real Resolution);
