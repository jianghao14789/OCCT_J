...
// Constructors
// ----------------------------------------------------------------------------
TopoDSToStep_MakeStepWire::TopoDSToStep_MakeStepWire() : myError(TopoDSToStep_WireOther) {
    done = Standard_False;
}

TopoDSToStep_MakeStepWire::TopoDSToStep_MakeStepWire(const TopoDS_Wire& W, TopoDSToStep_Tool& T,
...
void TopoDSToStep_MakeStepWire::Init(const TopoDS_Wire& aWire, TopoDSToStep_Tool& aTool,
                                     const Handle(Transfer_FinderProcess) & FP) {
    ...
    if (aTool.Faceted()) {
        ...
        for (ItW.Init(aWire, aTool.CurrentFace()); ItW.More(); ItW.Next()) {
    ...
    // --------
        ...
        for (ie = 1; ie <= nb; ie++) {
            ...
            if (!BRep_Tool::Degenerated(edge)) cwd->Add(edge);
        }
        ...
        for (Standard_Integer nEdge = 1; nEdge <= sbwd->NbEdges(); nEdge++) {
            ...
            Standard_Real cf, cl;
            Handle(Geom2d_Curve) theC2d = BRep_Tool::CurveOnSurface(CurrentEdge, aTool.CurrentFace(), cf, cl);
