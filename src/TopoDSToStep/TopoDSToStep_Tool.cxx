...
void TopoDSToStep_Tool::Bind(const TopoDS_Shape& S, const Handle(StepShape_TopologicalRepresentationItem) & T) {
    myDataMap.Bind(S, T);
}
...
void TopoDSToStep_Tool::SetCurrentFace(const TopoDS_Face& F) {
    ...
    if (FaceTol > myLowestTol) myLowestTol = FaceTol;
    myCurrentFace = F;
}
...
void TopoDSToStep_Tool::SetCurrentEdge(const TopoDS_Edge& E) {
    ...
    if (EdgeTol > myLowestTol) myLowestTol = EdgeTol;
    myCurrentEdge = E;
}
...
void TopoDSToStep_Tool::SetCurrentVertex(const TopoDS_Vertex& V) {
    ...
    if (VertexTol > myLowestTol) myLowestTol = VertexTol;
    myCurrentVertex = V;
}
