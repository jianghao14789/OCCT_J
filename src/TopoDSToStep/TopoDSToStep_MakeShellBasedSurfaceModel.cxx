...
TopoDSToStep_MakeShellBasedSurfaceModel::TopoDSToStep_MakeShellBasedSurfaceModel(
    const TopoDS_Face& aFace, const Handle(Transfer_FinderProcess) & FP, const Message_ProgressRange& theProgress) {
    done = Standard_False;
    MoniTool_DataMapOfShapeTransient aMap;

    TopoDSToStep_Tool aTool(aMap, Standard_False);
    TopoDSToStep_Builder StepB(aFace, aTool, FP, theProgress);
    if (theProgress.UserBreak()) return;
...
TopoDSToStep_MakeShellBasedSurfaceModel::TopoDSToStep_MakeShellBasedSurfaceModel(
    const TopoDS_Shell& aShell, const Handle(Transfer_FinderProcess) & FP, const Message_ProgressRange& theProgress) {
    done = Standard_False;
    StepShape_Shell aShellSelect;
...
TopoDSToStep_MakeShellBasedSurfaceModel::TopoDSToStep_MakeShellBasedSurfaceModel(
    const TopoDS_Solid& aSolid, const Handle(Transfer_FinderProcess) & FP, const Message_ProgressRange& theProgress) {
    done = Standard_False;
    StepShape_Shell aShellSelect;
    Handle(StepShape_HArray1OfShell) aSbsmBoundary;
    Handle(StepShape_OpenShell) aOpenShell;
    ...
    for (It.Initialize(aSolid); It.More(); It.Next())
        if (It.Value().ShapeType() == TopAbs_SHELL) nbshapes++;
