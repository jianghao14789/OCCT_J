...
void LocOpe_FindEdges::Set(const TopoDS_Shape& FFrom, const TopoDS_Shape& FTo) {
    ...
    for (expf.Init(myFFrom, TopAbs_EDGE); expf.More(); expf.Next()) {
        ...
        if (Tf != STANDARD_TYPE(Geom_Line) && Tf != STANDARD_TYPE(Geom_Circle) && Tf != STANDARD_TYPE(Geom_Ellipse) &&
            Tf != STANDARD_TYPE(Geom_BSplineCurve) && Tf != STANDARD_TYPE(Geom_BezierCurve)) {
            continue;
        }
