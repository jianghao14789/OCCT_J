...
static Standard_Boolean NewPlane(const TopoDS_Face&, const gp_Dir&, const gp_Pln&, const Standard_Real, gp_Pln&,
                                 gp_Ax1&, const Standard_Boolean);

static void MakeFace(TopoDS_Face&, TopTools_ListOfShape&);

static TopoDS_Edge NewEdge(const TopoDS_Edge&, const TopoDS_Face&, const Handle(Geom_Surface) &, const TopoDS_Vertex&,
                           const TopoDS_Vertex&);
...
void LocOpe_SplitDrafts::Perform(const TopoDS_Face& F, const TopoDS_Wire& W, const gp_Dir& Extr, const gp_Pln& NPl,
...
void LocOpe_SplitDrafts::Perform(const TopoDS_Face& F, const TopoDS_Wire& W, const gp_Dir& Extrg, const gp_Pln& NPlg,
                                 const Standard_Real Angleg, const gp_Dir& Extrd, const gp_Pln& NPld,
                                 const Standard_Real Angled, const Standard_Boolean ModLeft,
...

{
    ...
    if (i2s.IsDone() && i2s.NbLines() > 0) {
        ...
        if (!thePipe.IsDone()) throw Standard_ConstructionError("GeomFill_Pipe : Cannot make a surface");
    ...
    for (index = 1; index <= Nbedges; index++) {
        for (exp.Init(MapEV.FindKey(index), TopAbs_VERTEX); exp.More(); exp.Next()) {
            ...
            if (Abs(p1 - p2) > Precision::PConfusion()) {
                ...
                if (!newCs1.IsNull()) {
                    B.UpdateEdge(NewEdge, newCs1, TopoDS::Face(MapEV.FindFromKey(edg1)), Precision::Confusion());
                }

                if (!newCs2.IsNull()) {
                    B.UpdateEdge(NewEdge, newCs2, TopoDS::Face(MapEV.FindFromKey(edg2)), Precision::Confusion());
                }
        ...
        for (exp3.Init(fac.Oriented(TopAbs_FORWARD), TopAbs_WIRE); exp3.More(); exp3.Next()) {
            ...
            for (exp.Init(wir.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next()) {
                ...
                if (MapW.IsBound(edg)) { // edge du wire d`origine
                    ...
                    if (Gvf.ShapeType() == TopAbs_VERTEX && Gvl.ShapeType() == TopAbs_VERTEX) {
                        if (!Gvf.IsSame(Vf) || !Gvl.IsSame(Vl)) {
                            TopoDS_Edge NewEdg = NewEdge(edg, GenF, NewS, TopoDS::Vertex(Gvf), TopoDS::Vertex(Gvl));
                            if (NewEdg.IsNull()) {
                                return;
                            }
...
static Standard_Boolean NewPlane(const TopoDS_Face& F, const gp_Dir& Extr, const gp_Pln& Neutr, const Standard_Real Ang,
                                 gp_Pln& Newpl, gp_Ax1& NormalF, const Standard_Boolean Modify) {
...
static TopoDS_Edge NewEdge(const TopoDS_Edge& edg, const TopoDS_Face& F, const Handle(Geom_Surface) & NewS,
