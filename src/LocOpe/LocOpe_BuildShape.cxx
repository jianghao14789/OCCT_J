...
static void Add(const Standard_Integer, TColStd_MapOfInteger&, TopTools_IndexedMapOfShape&,
                const TopTools_IndexedDataMapOfShapeListOfShape&);

static void Propagate(const TopoDS_Shape&, // face
                      TopoDS_Shape&,       // shell
                      const TopTools_IndexedMapOfShape&, TColStd_MapOfInteger&);
    ...
    do {
        ...
        if (i <= Nbedges) {
            ...
            if (!Manifold && FaceRef.IsNull()) {
                ...
                if (!Manifold) {
                    for (j = 1; j <= mapF.Extent(); j++) {
                        if (mapF(j).Orientation() == TopAbs_INTERNAL || mapF(j).Orientation() == TopAbs_EXTERNAL) {
                            B.Add(newSh, mapF(j));
                            mapIf.Remove(j);
                        }
                    }
...
static void Add(const Standard_Integer ind, TColStd_MapOfInteger& mapI, TopTools_IndexedMapOfShape& mapF,
...
static void Propagate(const TopoDS_Shape& F, TopoDS_Shape& Sh, const TopTools_IndexedMapOfShape& mapF,
