...
static Standard_Boolean Project(const TopoDS_Vertex&, const gp_Pnt2d&, const TopoDS_Face&, TopoDS_Edge&,
                                Standard_Real&);

static Standard_Real Project(const TopoDS_Vertex&, const TopoDS_Edge&);

static Standard_Real Project(const TopoDS_Vertex&, const gp_Pnt2d&, const TopoDS_Edge&, const TopoDS_Face&);
...
static void FindInternalIntersections(const TopoDS_Edge&, const TopoDS_Face&,
                                      TopTools_IndexedDataMapOfShapeListOfShape&, Standard_Boolean&);
...
LocOpe_WiresOnShape::LocOpe_WiresOnShape(const TopoDS_Shape& S)
    : myShape(S), myCheckInterior(Standard_True), myDone(Standard_False), myIndex(-1) {}
...
void LocOpe_WiresOnShape::BindAll() {
    ...
    for (Ind = 1; Ind <= myMapEF.Extent(); Ind++) {
        ...
        if (myCheckInterior) {
            ...
            if (isOverlapped) anOverlappedEdges.Add(edg);
        }
    }

    for (Ind = 1; Ind <= Splits.Extent(); Ind++) {
        TopoDS_Shape anEdge = Splits.FindKey(Ind);
        if (anOverlappedEdges.Contains(anEdge)) continue;
    ...
    for (Ind = 1; Ind <= myMapEF.Extent(); Ind++) {
        ...
        for (exp.Init(edg, TopAbs_VERTEX); exp.More(); exp.Next()) {
            ...
            gp_Pnt2d p2d = (!BAcurve2d.Curve().IsNull() ? BAcurve2d.Value(vtx_param)
            ...
            if (isProjected) {
                ...
                if (aSh.ShapeType() != TopAbs_EDGE) continue;
                Epro = TopoDS::Edge(myMap.Find(vtx));
                if (aVertParam.IsBound(vtx)) prm = aVertParam.Find(vtx);
            }
                ...
                for (exp2.Init(Epro, TopAbs_VERTEX); exp2.More(); exp2.Next()) {
                    ...
                    if (vtx2.IsSame(vtx)) {
                        ...
                        if (!BRep_Tool::Degenerated(Epro) &&
                            (Abs(prm - aF1) <= Precision::PConfusion() || Abs(prm - aL1) <= Precision::PConfusion())) {
                            myMap.Bind(vtx, vtx2);
                            theMap.Add(vtx);
                            break;
                        }
    ...
    for (ite.Initialize(myMap); ite.More(); ite.Next())
        if ((ite.Key()).ShapeType() == TopAbs_EDGE) myMapEF.Add(ite.Key(), ite.Value());
...
Standard_Boolean LocOpe_WiresOnShape::OnEdge(const TopoDS_Vertex& V, TopoDS_Edge& Ed, Standard_Real& prm) {
...
Standard_Boolean LocOpe_WiresOnShape::OnEdge(const TopoDS_Vertex& V, const TopoDS_Edge& EdgeFrom, TopoDS_Edge& Ed,
                                             Standard_Real& prm) {
    if (!myMap.IsBound(V) || myMap(V).ShapeType() == TopAbs_VERTEX) {
        return Standard_False;
    }
    ...
    Ed = TopoDS::Edge(myMap(V));
    if (!myMapEF.Contains(EdgeFrom)) return Standard_False;
...
Standard_Boolean Project(const TopoDS_Vertex& V, const gp_Pnt2d& p2d, const TopoDS_Face& F, TopoDS_Edge& theEdge,
                         Standard_Real& param) {
    ...
    if (theEdge.IsNull()) {
        ...
        for (TopExp_Explorer exp(F.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next()) {
            ...
            if (!C.IsNull()) {
                ...
                if (Precision::IsInfinite(aCurPar)) continue;
                gp_Pnt aCurPBound;
        ...
        if (theEdge.IsNull()) return Standard_False;
        ...
        if (!aCrvBound.IsNull()) {
            ...
            if ((dumin > dumax) || (dvmin > dvmax)) {
                ...
                if (aDist2d > aTol2d) {
                    ...
                    if (aDist3d1 > aDist3d) aDist3d = aDist3d1;
                }
            }
...
Standard_Real Project(const TopoDS_Vertex&, const gp_Pnt2d& p2d, const TopoDS_Edge& theEdge,
...
void PutPCurve(const TopoDS_Edge& Edg, const TopoDS_Face& Fac) {
    ...
    if (!V1.IsNull()) TolFirst = BRep_Tool::Tolerance(V1);
    if (!V2.IsNull()) TolLast = BRep_Tool::Tolerance(V2);
...
void PutPCurves(const TopoDS_Edge& Efrom, const TopoDS_Edge& Eto, const TopoDS_Shape& myShape) {
    ...
    if (!Lfaces.First().IsSame(Lfaces.Last())) {
        ...
        Handle(Geom2d_Curve) c2dff = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalE), TopoDS::Face(aLocalF), f, l);
        ...
        Handle(Geom2d_Curve) c2dfr = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalE), TopoDS::Face(aLocalF), f, l);
        ...
        aLocalF = Fac.Oriented(TopAbs_FORWARD);
        Handle(Geom2d_Curve) c2dtf = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalE), TopoDS::Face(aLocalF), f, l);
        ...
        aLocalF = Fac.Oriented(TopAbs_FORWARD);
        Handle(Geom2d_Curve) c2dtr = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalE), TopoDS::Face(aLocalF), f, l);
        ...
        if (isoU) {
            if (SameOri) {
                if (Abs(ptf.X() - p2f.X()) > Epsilon(ptf.X())) {
                    c2dff = Handle(Geom2d_Curve)::DownCast(c2dff->Translated(gp_Vec2d(ptf.X() - p2f.X(), 0.)));
                }
                if (Abs(ptr.X() - p2r.X()) > Epsilon(ptr.X())) {
                    c2dfr = Handle(Geom2d_Curve)::DownCast(c2dfr->Translated(gp_Vec2d(ptr.X() - p2r.X(), 0.)));
                }
            } else {
                if (Abs(ptr.X() - p2f.X()) > Epsilon(ptr.X())) {
                    c2dff = Handle(Geom2d_Curve)::DownCast(c2dff->Translated(gp_Vec2d(ptr.X() - p2f.X(), 0.)));
                }

                if (Abs(ptf.X() - p2r.X()) > Epsilon(ptf.X())) {
                    c2dfr = Handle(Geom2d_Curve)::DownCast(c2dfr->Translated(gp_Vec2d(ptf.X() - p2r.X(), 0.)));
                }
            }
            ...
            if (SameOri) {
                if (Abs(ptf.Y() - p2f.Y()) > Epsilon(ptf.Y())) {
                    c2dff = Handle(Geom2d_Curve)::DownCast(c2dff->Translated(gp_Vec2d(0., ptf.Y() - p2f.Y())));
                }
                if (Abs(ptr.Y() - p2r.Y()) > Epsilon(ptr.Y())) {
                    c2dfr = Handle(Geom2d_Curve)::DownCast(c2dfr->Translated(gp_Vec2d(0., ptr.Y() - p2r.Y())));
                }
            } else {
                if (Abs(ptr.Y() - p2f.Y()) > Epsilon(ptr.Y())) {
                    c2dff = Handle(Geom2d_Curve)::DownCast(c2dff->Translated(gp_Vec2d(0., ptr.Y() - p2f.Y())));
                }
                if (Abs(ptf.Y() - p2r.Y()) > Epsilon(ptf.Y())) {
                    c2dfr = Handle(Geom2d_Curve)::DownCast(c2dfr->Translated(gp_Vec2d(0., ptf.Y() - p2r.Y())));
                }
            }
...
void FindInternalIntersections(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace,
                               TopTools_IndexedDataMapOfShapeListOfShape& Splits, Standard_Boolean& isOverlapped) {
    ...
    for (; Explo.More(); Explo.Next()) {
        ...
        if (!anExtrema.IsDone() || !anExtrema.NbExt()) continue;
        ...
        anExtrema.TrimmedSquareDistances(dists[0], dists[1], dists[2], dists[3], aP11, aP12, aP21, aP22);
        for (i = 0; i < 4; ++i) {
            if (i < 2)
                j = 0;
            else
        ...
        for (i = 1; i <= aNbExt; i++) {
            ...
            {
                if (Abs(theIntPar - thePar[j]) <= aTolV2d[j]) break;
            }
            ...
            if (j >= 2) // intersection is inside "theEdge" => split
            {
                ...
                if (aPointInt.SquareDistance(thePnt[0]) > aTolVExt[0] &&
                    ...
                    aPoint.SquareDistance(thePnt[0]) > aTolVExt[0] && aPoint.SquareDistance(thePnt[1]) > aTolVExt[1]) {
                    SplitPars.Append(theIntPar);
                    if (aDist > aDistMax) aDistMax = aDist;
                }
            }
        }
    }

    if (SplitPars.IsEmpty()) return;
    ...
    if (!NewEdges.IsEmpty()) Splits.Add(theEdge, NewEdges);
}
...
Standard_Boolean LocOpe_WiresOnShape::Add(const TopTools_SequenceOfShape& theEdges) {
    ...
    for (; anExpFaces.More(); anExpFaces.Next(), numF++) {
        ...
        for (; i <= nb; i++) {
            if (anUsedEdges.Contains(i)) continue;

            if (aBoxF.IsOut(anEdgeBoxes(i))) continue;
            ...
            if (!anExtr.IsDone() || !anExtr.NbExt()) continue;
            Standard_Real aTolE = BRep_Tool::Tolerance(TopoDS::Edge(aCurE));
