...
void LocOpe_Spliter::Perform(const Handle(LocOpe_WiresOnShape) & PW) {
    ...
    for (PW->InitEdgeIterator(); PW->MoreEdge(); PW->NextEdge()) {
        ...
        if (PW->OnEdge(Ebis)) {
            ...
            if (IsFaceWithSec) theFacesWithSection.Add(fac);
    ...
    for (i = 1; i <= Emap.Extent(); i++) {
        ...
        if (BRep_Tool::Degenerated(anEdge)) DegEdges.Append(anEdge);
    }

    TopTools_SequenceOfShape DegWires;
    for (;;) {
        if (DegEdges.IsEmpty()) break;
    ...
    for (i = 1; i <= DegWires.Length(); i++) {
        ...
        for (j = 2; j <= Vmap.Extent(); j++) {
            if (!Vmap(j).IsSame(Vmap(1))) theSubs.Substitute(Vmap(j), LV);
        }
    }
    theSubs.Build(myRes);
    if (theSubs.IsCopied(myRes)) myRes = theSubs.Copy(myRes).First();
    ////
