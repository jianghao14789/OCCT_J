...
static void GetDirection(const TopoDS_Edge&, const TopoDS_Face&, gp_Pnt2d&, gp_Vec2d&, Standard_Boolean isFirstEnd);

static Standard_Boolean ChoixUV(const TopoDS_Edge&, const TopoDS_Face&, const TopTools_IndexedMapOfShape&, TopoDS_Edge&,
                                gp_Pnt2d&, gp_Vec2d&);

static TopoDS_Shape ChooseDirection(const TopoDS_Shape&, const TopoDS_Vertex&, const TopoDS_Face&,
                               ...
                               const BRepAdaptor_Surface& theBAS) // const Standard_Real tol)
{
    ...
    if (theBAS.IsUPeriodic()) isSame = (fabs(P1.X() - P2.X()) < theBAS.UPeriod() * 0.5);
    if (theBAS.IsVPeriodic()) isSame = (isSame && (fabs(P1.Y() - P2.Y()) < theBAS.VPeriod() * 0.5));
    return isSame;
...
void LocOpe_SplitShape::Add(const TopoDS_Vertex& V, const Standard_Real P, const TopoDS_Edge& E) {
    ...
    if (V.Orientation() == TopAbs_FORWARD || V.Orientation() == TopAbs_REVERSED) {
        ...
        Standard_Real aTolV1 = (BRep_Tool::Degenerated(edg) ? Max(BRep_Tool::Tolerance(aCurV1), aTolSplitV)
...
Standard_Boolean LocOpe_SplitShape::Add(const TopTools_ListOfShape& Lwires, const TopoDS_Face& F) {
    ...
    for (itl.Initialize(aLInside); itl.More(); itl.Next()) {
        ...
        for (i = 0; i < 2; i++) {
            if (VerWireMap.IsBound(Ver[i])) continue;
    ...
    for (itl.Initialize(aLInside); itl.More(); itl.Next()) {
        ...
        if (!VerSecMap.IsBound(V1)) VerSecMap.Bind(V1, LW1);
        VerSecMap(V1).Append(aWire);
        if (!VerSecMap.IsBound(V2)) VerSecMap.Bind(V2, LW2);
    ...
    for (;;) {
        ...
        wexp.Next();
        if (!wexp.More()) wexp.Init(CurWire, FaceRef);
        for (;;) {
            if (MW->Wire().Closed()) break;
        ...
        for (;;) {
            ...
            if (SectionsTimes(aSectionWire) == 0) SectionsTimes.UnBind(aSectionWire);
            if (MW->Wire().Closed()) {
                NewWires.Append(MW->Wire());
                if (SectionsTimes.IsEmpty()) break;
                theStartVertex = TopoDS::Vertex(BreakVertices.First());
                BreakVertices.RemoveFirst();
                CurWire = TopoDS::Wire(BreakOnWires.First());
                BreakOnWires.RemoveFirst();
                ...
                for (; itVW.More(); itVW.Next())
                    if (itVW.Value().IsSame(CurWire)) Times++;
                if (Times == 1) // it is inner touching wire
                {
                    ...
                    if (theDirection.IsSame(aStartEdge)) break;
                }
...
Standard_Boolean LocOpe_SplitShape::Add(const TopoDS_Wire& W, const TopoDS_Face& F) {
    ...
    try {
        ...
        if (!LocOpe::Closed(W, F)) {
            if (!AddOpenWire(W, F)) return Standard_False;
        } else {
            if (!AddClosedWire(W, F)) return Standard_False;
        }
    } catch (Standard_Failure const&) {
#ifdef OCCT_DEBUG
        std::cout << "Warning: SpliShape internal problem detected, some faces may be lost. Check input edges/wires"
                  << std::endl;
#endif
        return Standard_False;
    }
...
static Standard_Boolean checkOverlapping(const TopoDS_Edge& theEdge1, const TopoDS_Edge& theEdge2,
    ...
    for (; j < nbP; j++) {
        ...
        if (!proj.NbPoints() || proj.LowerDistance() > aMaxTol2d) return Standard_False;
        Standard_Real par1 = proj.LowerDistanceParameter();
        gp_Pnt2d aP2d1;
        gp_Vec2d aV1;
        aCrv1->D1(par1, aP2d1, aV1);

        if (!aV1.IsParallel(aV2, aTolAng)) return Standard_False;
    }
    return Standard_True;
}
...
Standard_Boolean LocOpe_SplitShape::AddOpenWire(const TopoDS_Wire& W, const TopoDS_Face& F) {
    ...
    if (wfirst.IsSame(wlast)) {
        ...
        for (exp.Init(W.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next()) {
            const TopoDS_Edge& edg = TopoDS::Edge(exp.Current());
            if (nbE > 1 && edg.IsSame(LastEdge)) continue;
            for (exp2.Init(edg, TopAbs_VERTEX); exp2.More(); exp2.Next()) {
        ...
        while (cond) {
            ...
            if (nbPoss >= 1) {
                ...
                if (MapE.Contains(aNextEdge)) break;
        ...
        if (nbAddBound < 2) return Standard_False;
        if (nbAddBound == 2 && !anE1.IsNull() && !anE2.IsNull()) {
            if (checkOverlapping(TopoDS::Edge(anE1), TopoDS::Edge(anE2), FaceRef)) return Standard_False;
        }
        ...
        for (; anItE.More(); anItE.Next()) {
            if (anItE.Value().ShapeType() != TopAbs_EDGE) continue;
            nbAddBound++;
            if (anE11.IsNull())
                anE11 = anItE.Value();
            else
        ...
        if (nbAddBound == 2 && !anE11.IsNull() && !anE12.IsNull()) {
            if (checkOverlapping(TopoDS::Edge(anE11), TopoDS::Edge(anE12), FaceRef)) return Standard_False;
        }
...
static void updateToleraces(const TopoDS_Face& theFace, const TopTools_DataMapOfShapeListOfShape& theMap) {
    ...
    for (; aExpE.More(); aExpE.Next()) {
        if (!theMap.IsBound(aExpE.Current())) continue;
        const TopTools_ListOfShape& lEdges = theMap(aExpE.Current());
        if (lEdges.Extent() <= 1) continue;
...
{
    if (S.ShapeType() == TopAbs_FACE) updateToleraces(TopoDS::Face(S), myMap);
...
static Standard_Boolean IsInside(const TopoDS_Face& F, const TopoDS_Wire& W) {
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (stat == TopAbs_ON) {
            ...
            if (nbOut > nbIn + nbOn) return Standard_False;
        }
    }
    return Standard_True;
}
...
// purpose  :
//=======================================================================
static void GetDirection(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace, gp_Pnt2d& thePnt, gp_Vec2d& theDir,
    ...
    TopoDS_Vertex aVtx;
    Standard_Boolean takeFirst = ((anOr == TopAbs_FORWARD && isFirstEnd) || (anOr == TopAbs_REVERSED && !isFirstEnd));
    ...
    if (anOr == TopAbs_REVERSED) theDir.Reverse();
}
...
Standard_Boolean ChoixUV(const TopoDS_Edge& Last, const TopoDS_Face& F, const TopTools_IndexedMapOfShape& Poss,
                         TopoDS_Edge& theResEdge, gp_Pnt2d& plst, gp_Vec2d& dlst) {
    gp_Pnt2d p2d;
    gp_Vec2d v2d;
    gp_Pnt aPCur, aPlst;
    ...
    for (index = 1; index <= Poss.Extent(); index++) {
        ...
        if (!SameUV(plst, p2d, surf)) continue;
...
static TopoDS_Shape ChooseDirection(const TopoDS_Shape& RefDir, const TopoDS_Vertex& RefVertex,
                                    const TopoDS_Face& theFace, const TopTools_ListOfShape& Ldirs) {
    ...
    if (anOr == TopAbs_FORWARD) RefVec.Reverse();
    ...
    for (; itl.More(); itl.Next()) {
        ...
        if (anOr == TopAbs_REVERSED) aVec.Reverse();
