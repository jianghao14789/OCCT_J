...
static Standard_Boolean ToFuse(const TopoDS_Edge&, const TopoDS_Face&, const TopoDS_Vertex&,
                               const TopTools_MapOfShape&);

static Standard_Real NewParameter(const TopoDS_Edge&, const TopoDS_Vertex&, const TopoDS_Edge&, const TopoDS_Vertex&);
                ...
                for (exp2.Init(wir, TopAbs_EDGE); exp2.More(); exp2.Next()) {
                    ...
                    if (!toRemove.Contains(edg) && !theEEMap.IsBound(edg)) {
                        ...
                        if (P.IsNull()) { // on met les courbes 2d si on n`est pas
                            ...
                            C2d = BRep_Tool::CurveOnSurface(TopoDS::Edge(aLocalEdge), TopoDS::Face(aLocalFace), f, l);
                                    ...
                                    if (S->IsUPeriodic()) {
                                        ...
                                        if (DontFuse.IsBound(edg)) {
                                            ...
                                            if (myModShapes.IsBound(edg)) { // edge raccourci...
                                                ...
                                                TopoDS_Shape aLocalFace = fac.Oriented(TopAbs_FORWARD);
                                                BRep_Tool::UVPoints(TopoDS::Edge(aLocalShape), TopoDS::Face(aLocalFace),
                                                                    pf, pl);
                                                //			BRep_Tool::
    ...
    */
    ...
    if (myRes.ShapeType() == TopAbs_SOLID) {
        ...
        if (exp.More() && exp.Current().Orientation() != FaceRefOri.Orientation()) {
            // 	---C++: return const&	---C++: return const&	---C++: return const&Si un seul Shell , on change son
            // orientation
            TopoDS_Solid NewSol;
            B.MakeSolid(NewSol);
...
Standard_Boolean ToFuse(const TopoDS_Edge& E, const TopoDS_Face& F, const TopoDS_Vertex& V,
...
Standard_Real NewParameter(const TopoDS_Edge& Edg, const TopoDS_Vertex& Vtx, const TopoDS_Edge& NewEdg,
