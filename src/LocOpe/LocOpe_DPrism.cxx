...
LocOpe_DPrism::LocOpe_DPrism(const TopoDS_Face& Spine, const Standard_Real Height1, const Standard_Real Height2,
    ...
    if (myDPrism.IsDone()) {
        ...
        for (; ExpS.More(); ExpS.Next()) {
            ...
            for (it.Initialize(lffs); it.More(); it.Next()) {
                if (View.Add(it.Value())) B.Add(C, it.Value());
            }
        }
        ...
        for (; ExpS.More(); ExpS.Next()) {
            ...
            for (it.Initialize(lfls); it.More(); it.Next()) {
                if (View.Add(it.Value())) B.Add(D, it.Value());
            }
        }
                ...
                for (it.Initialize(lffs); it.More(); it.Next()) {
                    if (it.Value().ShapeType() == TopAbs_FACE) {
                        ...
                        if (NewFace.IsNull()) {
                            ...
                            for (exp.Init(theWire.Oriented(TopAbs_FORWARD), TopAbs_EDGE); exp.More(); exp.Next()) {
                                if (!exp.Current().IsSame(RemovedEdge)) {
                                    if (theWire.Orientation() !=
                                        Orref) { // Les 2 faces planes ont des normales opposees
                                        B.Add(NewWire, exp.Current());
                                    } else {
...
LocOpe_DPrism::LocOpe_DPrism(const TopoDS_Face& Spine, const Standard_Real Height, const Standard_Real Angle)
    : mySpine(Spine) {
    Standard_Integer i;
    myHeight = Height;
