...
STEPConstruct_Styles::STEPConstruct_Styles(const Handle(XSControl_WorkSession) & WS) : STEPConstruct_Tool(WS) {}
...
Standard_Boolean
STEPConstruct_Styles::CreateMDGPR(const Handle(StepRepr_RepresentationContext) & Context,
    ...
    Handle(StepRepr_HArray1OfRepresentationItem) elems = new StepRepr_HArray1OfRepresentationItem(1, myStyles.Extent());
    ...
    if (Interface_Static::IVal("write.step.schema") == 3) {
        ...
        Handle(TCollection_HAsciiString) subSchema = new TCollection_HAsciiString("SHAPE_APPEARANCE_LAYER_MIM");
...
Standard_Boolean STEPConstruct_Styles::CreateNAUOSRD(const Handle(StepRepr_RepresentationContext) & Context,
    ...
    Handle(StepRepr_HArray1OfRepresentationItem) elems = new StepRepr_HArray1OfRepresentationItem(1, 1);
    // get PDS
...
#ifdef OCCT_DEBUG
//    std::cout << "Context of " << Shape.TShape()->DynamicType()->Name() << ": SR found: " << sr->DynamicType()->Name()
//    << std::endl;
#endif
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (enti->DynamicType() == tMDGPR || enti->DynamicType() == tDM) {
            ...
            for (Standard_Integer j = 1; j <= nbi; j++) {
                Handle(StepVisual_StyledItem) style = Handle(StepVisual_StyledItem)::DownCast(container->ItemsValue(j));
...
Standard_Boolean STEPConstruct_Styles::LoadInvisStyles(Handle(TColStd_HSequenceOfTransient) & theInvStyles) const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        Handle(Standard_Transient) enti = model->Value(i);
        if (enti->DynamicType() != tInvisibility) continue;
        ...
        for (Standard_Integer j = 1; j <= nbi; j++) {
            ...
            if (theInvStyles.IsNull()) theInvStyles = new TColStd_HSequenceOfTransient;
...
Handle(StepVisual_PresentationStyleAssignment) STEPConstruct_Styles::MakeColorPSA(
    const Handle(StepRepr_RepresentationItem) & /*item*/, const Handle(StepVisual_Colour) & SurfCol,
    const Handle(StepVisual_Colour) & CurveCol, const Handle(StepVisual_Colour) & RenderCol,
    const Standard_Real RenderTransp, const Standard_Boolean isForNAUO) const {
...
Handle(StepVisual_PresentationStyleAssignment) STEPConstruct_Styles::GetColorPSA(
    const Handle(StepRepr_RepresentationItem) & item, const Handle(StepVisual_Colour) & Col) {
...
Standard_Boolean STEPConstruct_Styles::GetColors(const Handle(StepVisual_StyledItem) & style,
                                                 ...
                                                 Handle(StepVisual_Colour) & RenderCol, Standard_Real& RenderTransp,
                ...
                for (Standard_Integer l = 1; l <= SSS->NbStyles(); l++) {
                    ...
                    if (!SSFA.IsNull()) {
                        ...
                        for (Standard_Integer m = 1; m <= FAS->NbFillStyles(); m++) {
                            ...
                            if (SurfCol.IsNull() ||
                                SSU->Side() != StepVisual_ssNegative) // abv 30 Mar 00: trj3_s1-pe.stp
                        ...
                        if (!SSRWP.IsNull()) {
                            ...
                            if (!HARP.IsNull()) {
                                for (Standard_Integer aPropIndex = 1; aPropIndex <= HARP->Length(); ++aPropIndex) {
                                    Handle(StepVisual_SurfaceStyleTransparent) SST =
                                        HARP->Value(aPropIndex).SurfaceStyleTransparent();
                                    if (!SST.IsNull()) {
...
Handle(StepVisual_Colour) STEPConstruct_Styles::EncodeColor(const Quantity_Color& C,
    ...
    if (cName) {
        ...
        gp_Pnt P;
        C.Values(P.ChangeCoord().ChangeData()[0], P.ChangeCoord().ChangeData()[1], P.ChangeCoord().ChangeData()[2],
                 Quantity_TOC_sRGB);
