...
Handle(StepBasic_DateAndTime) STEPConstruct_AP203Context::DefaultDateAndTime() {
    if (defDateAndTime.IsNull()) {
        ...
        Handle(StepBasic_CoordinatedUniversalTimeOffset) zone = new StepBasic_CoordinatedUniversalTimeOffset;
...
Handle(StepBasic_PersonAndOrganization) STEPConstruct_AP203Context::DefaultPersonAndOrganization() {
    if (defPersonAndOrganization.IsNull()) {
        ...
        aPerson->Init(uid, Standard_True, lname, Standard_True, fname, (!mname.IsNull()), mname, Standard_False, suffix,
                      Standard_False, prefix);
...
void STEPConstruct_AP203Context::InitPart(const STEPConstruct_Part& SDRTool) {
    if (myCreator.IsNull()) {
        ...
        Handle(StepAP203_HArray1OfPersonOrganizationItem) items = new StepAP203_HArray1OfPersonOrganizationItem(1, 2);
    ...
    if (myDesignOwner.IsNull()) {
        ...
        Handle(StepAP203_HArray1OfPersonOrganizationItem) items = new StepAP203_HArray1OfPersonOrganizationItem(1, 1);
...
void STEPConstruct_AP203Context::InitApprovalRequisites() {
    ...
    if (myApprovalDateTime.IsNull() || myApprovalDateTime->DatedApproval() != myApproval->AssignedApproval()) {
