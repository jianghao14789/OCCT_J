...
#ifdef OCCT_DEBUG
//      std::cout << Shape.TShape()->DynamicType()->Name() << ": GeomRepItem found: " << item->DynamicType()->Name() <<
//      std::endl;
#endif
...
//	     aspect->OfShape() != PDS )
                ...
                std::cout << Shape.TShape()->DynamicType()->Name() << ": PDS NOT found, fail to create SHAPE_ASPECT"
                          << std::endl;
#endif
        }
#ifdef OCCT_DEBUG
...
Standard_Boolean STEPConstruct_ValidationProps::AddProp(const StepRepr_CharacterizedDefinition& target,
    ...
    Handle(TCollection_HAsciiString) PropDefName = new TCollection_HAsciiString("geometric validation property");
...
Standard_Boolean STEPConstruct_ValidationProps::AddProp(const TopoDS_Shape& Shape,
                                                        const Handle(StepRepr_RepresentationItem) & Prop,
                                                        const Standard_CString Descr, const Standard_Boolean instance) {
    StepRepr_CharacterizedDefinition target;
    Handle(StepRepr_RepresentationContext) Context;
...
Standard_Boolean STEPConstruct_ValidationProps::AddVolume(const TopoDS_Shape& Shape, const Standard_Real Vol) {
    ...
    if (volUnit.DerivedUnit().IsNull()) {
        ...
        Handle(StepBasic_HArray1OfDerivedUnitElement) DUElems = new StepBasic_HArray1OfDerivedUnitElement(1, 1);
...
Standard_Boolean STEPConstruct_ValidationProps::AddArea(const TopoDS_Shape& Shape, const Standard_Real Area) {
    Handle(StepBasic_MeasureValueMember) Val = new StepBasic_MeasureValueMember;
    Val->SetReal(Area);
...
Standard_Boolean STEPConstruct_ValidationProps::AddCentroid(const TopoDS_Shape& Shape, const gp_Pnt& Pnt,
                                                            const Standard_Boolean instance) {
...
Standard_Boolean STEPConstruct_ValidationProps::LoadProps(TColStd_SequenceOfTransient& seq) const {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (!PD.IsNull() && !PD->Name().IsNull()) {
            ...
            // To be able to read files written using older convention, we convert all underscores to spaces for this
            // check.
            TCollection_AsciiString aName = PD->Name()->String();
            aName.ChangeAll('_', ' ', Standard_False);
            aName.LowerCase();
            if (aName != "geometric validation property") continue;
        }
...
Handle(StepBasic_ProductDefinition) STEPConstruct_ValidationProps::GetPropPD(const Handle(StepRepr_PropertyDefinition) &
                                                                             PD) const {
...
Handle(StepRepr_NextAssemblyUsageOccurrence)
    STEPConstruct_ValidationProps::GetPropNAUO(const Handle(StepRepr_PropertyDefinition) & PD) const {
    StepRepr_CharacterizedDefinition CD = PD->Definition();
...
Standard_Boolean STEPConstruct_ValidationProps::GetPropReal(const Handle(StepRepr_RepresentationItem) & item,
    ...
    // decode volume & area
    if (!item->IsKind(STANDARD_TYPE(StepRepr_MeasureRepresentationItem))) return Standard_False;

    Handle(StepRepr_MeasureRepresentationItem) mri = Handle(StepRepr_MeasureRepresentationItem)::DownCast(item);
    ...
    if (!DU.IsNull()) {
        ...
        if (!NU.IsNull()) {
            STEPConstruct_UnitContext unit;
            unit.ComputeFactors(NU);
            if (unit.AreaDone()) scale = unit.AreaFactor();
            if (unit.VolumeDone()) scale = unit.VolumeFactor();
        }
    }
...
#ifdef OCCT_DEBUG
        std::cout << "Warning: Measure " << Model()->StringLabel(M)->String() << " is neither area not volume"
                  << std::endl;
#endif
...
Standard_Boolean STEPConstruct_ValidationProps::GetPropPnt(const Handle(StepRepr_RepresentationItem) & item,
                                                           ...
                                                           gp_Pnt& Pnt) const {
    // centroid
    if (!item->IsKind(STANDARD_TYPE(StepGeom_CartesianPoint))) return Standard_False;

    Handle(StepGeom_CartesianPoint) P = Handle(StepGeom_CartesianPoint)::DownCast(item);
    if (P.IsNull() || P->NbCoordinates() != 3) {
#ifdef OCCT_DEBUG
        std::cout << "Warning: Point " << Model()->StringLabel(P)->String() << " is not valid for centroid"
                  << std::endl;
#endif
        return Standard_False;
    }
    ...
    if (!Context.IsNull()) {
        ...
        if (Context->IsKind(STANDARD_TYPE(StepGeom_GeometricRepresentationContextAndGlobalUnitAssignedContext))) {
            ...
            DeclareAndCast(StepGeom_GeomRepContextAndGlobUnitAssCtxAndGlobUncertaintyAssCtx, theGRCAGAUC, Context);
            theGUAC = theGRCAGAUC->GlobalUnitAssignedContext();
        }
        if (!theGUAC.IsNull()) {
