...
STEPConstruct_ExternRefs::STEPConstruct_ExternRefs(const Handle(XSControl_WorkSession) & WS) : STEPConstruct_Tool(WS) {}
...
static Standard_Boolean findPDWADandExcludeExcess(Handle(StepAP214_AppliedDocumentReference) & ADR,
    ...
    for (subsADR.Start(); subsADR.More(); subsADR.Next()) {
        if (!subsADR.Value()->IsKind(STANDARD_TYPE(StepBasic_Document))) continue;
        ...
        for (subsD.Start(); subsD.More(); subsD.Next()) {
            if (!subsD.Value()->IsKind(STANDARD_TYPE(StepBasic_DocumentProductEquivalence))) continue;
            ...
            for (subsDPE.Start(); subsDPE.More(); subsDPE.Next()) {
                if (!subsDPE.Value()->IsKind(STANDARD_TYPE(StepBasic_ProductDefinitionFormation))) continue;
...
Standard_Boolean STEPConstruct_ExternRefs::LoadExternRefs() {
    ...
    for (Standard_Integer adri = 1; adri <= aSeqOfADR.Length(); adri++) {
        ...
        for (subs4.Start(); subs4.More(); subs4.Next()) {
            if (!subs4.Value()->IsKind(STANDARD_TYPE(StepBasic_DocumentFile))) continue;
            ...
            for (subs.Start(); subs.More(); subs.Next()) {
                ...
                if (!Type.IsNull() && !Format.IsNull()) break;
            }
            if (!Type.IsNull() && !Format.IsNull()) break;
        }
        if (DocFile.IsNull()) continue;
...
Standard_CString STEPConstruct_ExternRefs::FileName(const Standard_Integer num) const {
    ...
    if (myDocFiles.Length() >= num && !myDocFiles.Value(num).IsNull())
        ...
        if (aPDWAD.IsNull() || aPDWAD->DocIds().IsNull()) return "";
        Standard_Integer i;
    ...
    if (!AEIA.IsNull()) {
        ...
        if (!aFilename.IsNull() && !aFilename->IsEmpty()) {
            ...
            if (aCStringFileName && aCStringFileName[0] == '#') aCStringFileName = 0;
        }
...
Standard_Integer STEPConstruct_ExternRefs::AddExternRef(const Standard_CString filename,
    ...
    if (!(np > 0)) {
        ...
        Handle(TCollection_HAsciiString) aName = new TCollection_HAsciiString("external document id and location");
        IR->SetName(aName);
        ...
        for (subs.Start(); subs.More(); subs.Next()) {
            ...
            Handle(StepRepr_ProductDefinitionShape) ProdDefSh = Handle(StepRepr_ProductDefinitionShape)::DownCast(sub);
...
Standard_Integer STEPConstruct_ExternRefs::WriteExternRefs(const Standard_Integer num) const {
    if (num == 3) {
        for (Standard_Integer i = 1; i <= myAEIAs.Length(); i++) {
            ...
            if (!myRoles(i).IsNull()) Model()->AddWithRefs(myRoles(i));
            if (!myTypes(i).IsNull()) Model()->AddWithRefs(myTypes(i));
        }
    } else {
        for (Standard_Integer i = 1; i <= myAEIAs.Length(); i++) {
            Model()->AddWithRefs(myAEIAs(i));
            if (!myRoles(i).IsNull()) Model()->AddWithRefs(myRoles(i));
            if (!myFormats(i).IsNull()) Model()->AddWithRefs(myFormats(i));
            if (!myShapes(i).IsNull()) Model()->AddWithRefs(myShapes(i));
            if (!myTypes(i).IsNull()) Model()->AddWithRefs(myTypes(i));
        }
    }
    // PTV 30.01.2003 TRJ11
    if (!myAPD.IsNull()) Model()->AddWithRefs(myAPD);
    if (!mySharedPRPC.IsNull()) Model()->AddWithRefs(mySharedPRPC);
...
Standard_Boolean STEPConstruct_ExternRefs::addAP214ExterRef(const Handle(StepAP214_AppliedDocumentReference) & ADR,
    ...
    if (!HProducts.IsNull()) nbProducts = HProducts->Length();
...
void STEPConstruct_ExternRefs::checkAP214Shared() {
    ...
    if (mySharedPDC.IsNull()) {
        ...
        Handle(TCollection_HAsciiString) aPDCname = new TCollection_HAsciiString("digital document definition");
