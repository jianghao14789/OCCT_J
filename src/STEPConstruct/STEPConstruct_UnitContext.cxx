...
STEPConstruct_UnitContext::STEPConstruct_UnitContext()
    : done(Standard_False), lengthFactor(0.0), planeAngleFactor(0.0), solidAngleFactor(0.0), areaFactor(0.0),
      volumeFactor(0.0) {
    lengthDone = planeAngleDone = solidAngleDone = hasUncertainty = areaDone = volumeDone = Standard_False;
...
void STEPConstruct_UnitContext::Init(const Standard_Real Tol3d) {
    ...
    Handle(TCollection_HAsciiString) contextType = new TCollection_HAsciiString("3D Context with UNIT and UNCERTAINTY");
    ...
    if (uName) { // for non-metric units, create conversion_based_unit
        ...
        Handle(StepBasic_ConversionBasedUnitAndLengthUnit) convUnit = new StepBasic_ConversionBasedUnitAndLengthUnit;
    ...
    Handle(TCollection_HAsciiString) TolName = new TCollection_HAsciiString("distance_accuracy_value");
...
Standard_Integer STEPConstruct_UnitContext::ComputeFactors(const Handle(StepRepr_GlobalUnitAssignedContext) &
                                                           aContext) {
...
#ifdef OCCT_DEBUG
        if (status == -1)
            std::cout << " -- STEPConstruct_UnitContext:ComputeFactor: Unit item no." << i << " is not recognized"
                      << std::endl;
#endif
    }
    return status;
...
Standard_Integer STEPConstruct_UnitContext::ComputeFactors(const Handle(StepBasic_NamedUnit) & aUnit) {
    ...
    if (aUnit->IsKind(STANDARD_TYPE(StepBasic_ConversionBasedUnit))) {
        Handle(StepBasic_ConversionBasedUnit) theCBU = Handle(StepBasic_ConversionBasedUnit)::DownCast(aUnit);
...
Standard_Integer STEPConstruct_UnitContext::ComputeTolerance(const Handle(StepRepr_GlobalUncertaintyAssignedContext) &
                                                             aContext) {
    Standard_Integer status = 0;
        ...
        if (!aUnit.IsNull()) {
            ...
            if (!aCBULU.IsNull()) {
                ...
                if (theUncertainty > LengthUncertainty)
                    theUncertainty = LengthUncertainty; // *lengthFactor; fait par appelant
                hasUncertainty = Standard_True;
            }
        }
    }
