...
public:
    ...
    Standard_EXPORT Handle(StepVisual_StyledItem) AddStyle(const Handle(StepRepr_RepresentationItem) & item,
    ...
    Standard_EXPORT Handle(StepVisual_StyledItem)
        AddStyle(const TopoDS_Shape& Shape, const Handle(StepVisual_PresentationStyleAssignment) & PSA,
                 const Handle(StepVisual_StyledItem) & Override);
    ...
    Standard_EXPORT Standard_Boolean CreateNAUOSRD(const Handle(StepRepr_RepresentationContext) & Context,
    ...
    Standard_EXPORT Handle(StepVisual_PresentationStyleAssignment)
        MakeColorPSA(const Handle(StepRepr_RepresentationItem) & item, const Handle(StepVisual_Colour) & SurfCol,
                     const Handle(StepVisual_Colour) & CurveCol, const Handle(StepVisual_Colour) & RenderCol,
                     const Standard_Real RenderTransp, const Standard_Boolean isForNAUO = Standard_False) const;
    ...
    Standard_EXPORT Handle(StepVisual_PresentationStyleAssignment)
        GetColorPSA(const Handle(StepRepr_RepresentationItem) & item, const Handle(StepVisual_Colour) & Col);
    ...
    Standard_EXPORT Standard_Boolean GetColors(const Handle(StepVisual_StyledItem) & style,
    ...
    //! one of standard colors predefined in STEP. In that case,
    //! PredefinedColour entity is created instead of RGBColour
    Standard_EXPORT static Handle(StepVisual_Colour)
        EncodeColor(const Quantity_Color& Col, STEPConstruct_DataMapOfAsciiStringTransient& DPDCs,
