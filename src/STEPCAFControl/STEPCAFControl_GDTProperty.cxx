...
void STEPCAFControl_GDTProperty::GetDimModifiers(const Handle(StepRepr_CompoundRepresentationItem) & theCRI,
    ...
    for (Standard_Integer l = 1; l <= theCRI->ItemElement()->Length(); l++) {
        ...
        if (aFound) theModifiers.Append(aModifier);
    }
}
...
void STEPCAFControl_GDTProperty::GetDimClassOfTolerance(const Handle(StepShape_LimitsAndFits) & theLAF,
    ...
    for (Standard_Integer c = 0; c <= 1 && !aFormV.IsNull(); c++) {
        ...
        if (c == 1) aCaseSens = Standard_True;
        ...
        if (c == 1 && !aFound) theHolle = Standard_True;
    }
...
Handle(TCollection_HAsciiString)
    STEPCAFControl_GDTProperty::GetDimTypeName(const XCAFDimTolObjects_DimensionType theType) {
...
//=======================================================================
Handle(TCollection_HAsciiString)
    STEPCAFControl_GDTProperty::GetDimQualifierName(const XCAFDimTolObjects_DimensionQualifier theQualifier) {
    Handle(TCollection_HAsciiString) aName;
    switch (theQualifier) {
...
//=======================================================================
Handle(TCollection_HAsciiString)
    STEPCAFControl_GDTProperty::GetDimModifierName(const XCAFDimTolObjects_DimensionModif theModifier) {
    Handle(TCollection_HAsciiString) aName;
    switch (theModifier) {
...
Handle(StepShape_LimitsAndFits) STEPCAFControl_GDTProperty::GetLimitsAndFits(
    Standard_Boolean theHole, XCAFDimTolObjects_DimensionFormVariance theFormVariance,
...
//=======================================================================
Handle(TCollection_HAsciiString)
    STEPCAFControl_GDTProperty::GetDatumTargetName(const XCAFDimTolObjects_DatumTargetType theDatumType) {
    Handle(TCollection_HAsciiString) aName;
    switch (theDatumType) {
...
STEPCAFControl_GDTProperty::GetGeomToleranceType(const XCAFDimTolObjects_GeomToleranceType theType) {
    switch (theType) {
...
STEPCAFControl_GDTProperty::GetGeomToleranceType(const StepDimTol_GeometricToleranceType theType) {
    switch (theType) {
...
// purpose  :
//=======================================================================
Handle(StepDimTol_GeometricTolerance)
    STEPCAFControl_GDTProperty::GetGeomTolerance(const XCAFDimTolObjects_GeomToleranceType theType) {
    switch (theType) {
        case XCAFDimTolObjects_GeomToleranceType_CircularityOrRoundness:
...
STEPCAFControl_GDTProperty::GetGeomToleranceModifier(const XCAFDimTolObjects_GeomToleranceModif theModifier) {
    switch (theModifier) {
...
Handle(StepDimTol_HArray1OfDatumReferenceModifier)
    STEPCAFControl_GDTProperty::GetDatumRefModifiers(const XCAFDimTolObjects_DatumModifiersSequence theModifiers,
                                                     const XCAFDimTolObjects_DatumModifWithValue theModifWithVal,
                                                     const Standard_Real theValue, const StepBasic_Unit theUnit) {
    if ((theModifiers.Length() == 0) && (theModifWithVal == XCAFDimTolObjects_DatumModifWithValue_None)) return NULL;
    Standard_Integer aModifNb = theModifiers.Length();
    if (theModifWithVal != XCAFDimTolObjects_DatumModifWithValue_None) aModifNb++;
...
//=======================================================================
Handle(TCollection_HAsciiString)
    STEPCAFControl_GDTProperty::GetTolValueType(const XCAFDimTolObjects_GeomToleranceTypeValue& theType) {
    switch (theType) {
...
Handle(StepVisual_TessellatedGeometricSet) STEPCAFControl_GDTProperty::GetTessellation(const TopoDS_Shape theShape) {
    ...
    for (TopExp_Explorer aCurveIt(theShape, TopAbs_EDGE); aCurveIt.More(); aCurveIt.Next()) {
        ...
        if (anEdgeCurve.IsNull()) continue;
        // Line
        ...
        // BSpline
            ...
            Handle(Geom_BSplineCurve) aBSCurve =
                aSCC.ConvertToBSpline(anEdgeCurve, aFirst, aLast, Precision::Confusion());
