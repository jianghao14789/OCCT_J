...
STEPCAFControl_Writer::STEPCAFControl_Writer()
    : myColorMode(Standard_True), myNameMode(Standard_True), myLayerMode(Standard_True), myPropsMode(Standard_True),
      mySHUOMode(Standard_True), myGDTMode(Standard_True), myMatMode(Standard_True) {
...
// purpose  :
//=======================================================================
void STEPCAFControl_Writer::prepareUnit(const TDF_Label& theLabel, const Handle(StepData_StepModel) & theModel) {
...
Standard_Boolean STEPCAFControl_Writer::Transfer(const Handle(TDocStd_Document) & doc,
                                                 const STEPControl_StepModelType mode, const Standard_CString multi,
...
Standard_Boolean STEPCAFControl_Writer::Transfer(const TDF_Label& L, const STEPControl_StepModelType mode,
...
Standard_Boolean STEPCAFControl_Writer::Transfer(const TDF_LabelSequence& labels, const STEPControl_StepModelType mode,
                                                 const Standard_CString multi,
                                                 const Message_ProgressRange& theProgress) {
    return Transfer(myWriter, labels, mode, multi, Standard_False, theProgress);
}
...
Standard_Boolean STEPCAFControl_Writer::Perform(const Handle(TDocStd_Document) & doc, const Standard_CString filename,
                                                const Message_ProgressRange& theProgress) {
...
STEPCAFControl_Writer::ExternFiles() const {
    return myFiles;
}
...
Standard_Boolean STEPCAFControl_Writer::ExternFile(const Standard_CString name,
    ...
    ef.Nullify();
    if (!myFiles.IsEmpty() || !myFiles.IsBound(name)) return Standard_False;
...
Standard_Boolean STEPCAFControl_Writer::Transfer(STEPControl_Writer& writer, const TDF_LabelSequence& labels,
                                                 const STEPControl_StepModelType mode, const Standard_CString multi,
        ...
        if (!multi) {
            ...
            if (XCAFDoc_ShapeTool::IsReference(L)) {
                ...
                if (XCAFDoc_ShapeTool::GetReferredShape(L, ref)) {
                    if (XCAFDoc_ShapeTool::IsAssembly(ref)) XCAFDoc_ShapeTool::GetComponents(ref, comp, Standard_True);
                }
                if (!XCAFDoc_ShapeTool::IsFree(L)) L = L.Father();
            } else {
                // fill sequence of (sub) shapes for which attributes should be written
                // and set actor to handle assemblies in a proper way
                if (XCAFDoc_ShapeTool::IsAssembly(L)) XCAFDoc_ShapeTool::GetComponents(L, comp, Standard_True);
            }
            ...
            for (Standard_Integer k = 1; k <= comp.Length(); k++) {
                ...
                if (!myLabels.IsBound(ref)) {
                    ...
                    if (XCAFDoc_ShapeTool::IsAssembly(ref)) Actor->RegisterAssembly(refS);
                }
            }
            myLabels.Bind(L, shape);
            sublabels.Append(L);

            if (XCAFDoc_ShapeTool::IsAssembly(L) || XCAFDoc_ShapeTool::IsReference(L)) Actor->RegisterAssembly(shape);
            ...
            if (aPS1.UserBreak()) return Standard_False;
            ...
            /*
                ...
                if ( model->HasHeaderEntity(STANDARD_TYPE(HeaderSection_FileSchema)) ) {
                  Handle(HeaderSection_FileSchema) fs =
                    Handle(HeaderSection_FileSchema)::DownCast (
               model->HeaderEntity(STANDARD_TYPE(HeaderSection_FileSchema)) ); Handle(TCollection_HAsciiString) str =
               fs->SchemaIdentifiersValue ( 1 ); Handle(TCollection_HAsciiString) ap214 = new TCollection_HAsciiString (
               "AUTOMOTIVE_DESIGN" ); if ( str->Search ( ap214 ) <0 ) { str->Clear(); str->AssignCat ( ap214 );
                  }
                }
                  }
            */
    ...
    if (aPS.UserBreak()) return Standard_False;

    writer.WS()->ComputeGraph(Standard_True); // added by skl 03.11.2003 since we use
                                              // writer.Transfer() without compute graph

    // write names
    if (GetNameMode()) WriteNames(writer.WS(), sublabels);

    if (!multi) {
        // write colors
        if (GetColorMode()) WriteColors(writer.WS(), sublabels);

        // write layers
        if (GetLayerMode()) WriteLayers(writer.WS(), sublabels);
        ...
        // write Materials
        if (GetMaterialMode()) WriteMaterials(writer.WS(), sublabels);
    ...
    //      Interface_Static::SetCVal ("write.step.schema", "AP214DIS");
    //  }
    if (GetPropsMode()) WriteValProps(writer.WS(), sublabels, multi);
    ...
    if (Interface_Static::IVal("write.stepcaf.subshapes.name") != 0) {
        ...
        for (int i = 1; i <= sublabels.Length(); i++) {
            ...
            for (TDF_ChildIterator it(L, Standard_True); it.More(); it.Next()) {
                ...
                if (!FP->FindTypedTransient(aShMapper, STANDARD_TYPE(StepRepr_RepresentationItem), RI)) continue;
...
TopoDS_Shape STEPCAFControl_Writer::TransferExternFiles(const TDF_Label& L, const STEPControl_StepModelType mode,
                                                        TDF_LabelSequence& labels, const Standard_CString prefix,
                                                        const Message_ProgressRange& theProgress) {
...
Standard_Boolean STEPCAFControl_Writer::WriteExternRefs(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer k = 1; k <= labels.Length(); k++) {
        ...
        if (aPropDef.IsNull()) {
#ifdef OCCT_DEBUG
            std::cout << "Warning: STEPCAFControl_Writer::WriteExternRefs StepRepr_PropertyDefinition is null for "
                      << S.TShape()->DynamicType()->Name() << std::endl;
#endif
            continue;
        }
        ...
        if (PD.IsNull()) {
#ifdef OCCT_DEBUG
            std::cout << "Warning: STEPCAFControl_Writer::WriteExternRefs StepBasic_ProductDefinition is null for "
                      << S.TShape()->DynamicType()->Name() << std::endl;
#endif
            continue;
        }
...
// purpose  : auxiliary
//=======================================================================
static Standard_Integer FindEntities(const Handle(Transfer_FinderProcess) & FP, const TopoDS_Shape& S,
                                     TopLoc_Location& L, TColStd_SequenceOfTransient& seqRI) {
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean getStyledItem(const TopoDS_Shape& S, const Handle(XCAFDoc_ShapeTool) & STool,
                                      const STEPConstruct_Styles& Styles, Handle(StepVisual_StyledItem) & resSelItem,
...
// purpose  : auxiliary
//=======================================================================
static void MakeSTEPStyles(STEPConstruct_Styles& Styles, const TopoDS_Shape& S,
                           const XCAFPrs_IndexedDataMapOfShapeStyle& settings, Handle(StepVisual_StyledItem) & override,
                           TopTools_MapOfShape& Map, const MoniTool_DataMapOfShapeTransient& myMapCompMDGPR,
                           STEPConstruct_DataMapOfAsciiStringTransient& DPDCs,
                           STEPConstruct_DataMapOfPointTransient& ColRGBs, const Handle(XCAFDoc_ColorTool) & CTool,
                           const XCAFPrs_Style* inherit = 0, const Standard_Boolean isComponent = Standard_False) {
    ...
    if (style.IsSetColorCurv()) curvColor = Styles.EncodeColor(style.GetColorCurv(), DPDCs, ColRGBs);

    Standard_Boolean hasOwn = (!surfColor.IsNull() || !curvColor.IsNull() || !style.IsVisible());
    ...
    if (hasOwn) {
        if (S.ShapeType() != TopAbs_COMPOUND || isComponent) { // skip compounds, let subshapes inherit its colors
            ...
            if (isComponent && nb) getStyledItem(S, CTool->ShapeTool(), Styles, override, myMapCompMDGPR);

            for (Standard_Integer i = 1; i <= nb; i++) {
                Handle(StepRepr_RepresentationItem) item = Handle(StepRepr_RepresentationItem)::DownCast(seqRI(i));
                Handle(StepVisual_PresentationStyleAssignment) PSA;
                if (style.IsVisible() || !surfColor.IsNull() || !curvColor.IsNull()) {
                    ...
                    if (isComponent) setDefaultInstanceColor(override, PSA);
    ...
    if (!isComponent) // PTV 10.02.2003
        for (TopoDS_Iterator it(S); it.More(); it.Next()) {
            MakeSTEPStyles(Styles, it.Value(), settings, STEPstyle, Map, myMapCompMDGPR, DPDCs, ColRGBs, CTool,
                           (hasOwn ? &style : 0));
        }
}
...
Standard_Boolean STEPCAFControl_Writer::WriteColors(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= labels.Length(); i++) {
        ...
        if (XCAFDoc_ShapeTool::IsAssembly(L)) {
            ...
            if (aSTool.IsNull()) continue;
            if (!aSTool->GetComponents(L, compLabels)) continue;
        ...
        for (Standard_Integer j = 1; j <= seq.Length(); j++) {
            ...
            if (CTool->GetColor(lab, XCAFDoc_ColorSurf, C)) style.SetColorSurf(C);
            if (CTool->GetColor(lab, XCAFDoc_ColorCurv, C)) style.SetColorCurv(C.GetRGB());
            if (!style.IsSetColorSurf()) {
        ...
        if (!isComponent) {
            ...
            if (!aMDGPR.IsNull()) myMapCompMDGPR.Bind(aTopSh, aMDGPR);
        } else {
            ...
            if (FP->FindTypedTransient(mapper, STANDARD_TYPE(StepShape_ContextDependentShapeRepresentation), CDSR)) {
                ...
                if (!oldItems.IsNull()) oldLengthlen = oldItems->Length();
                ...
                if (newItems->Length() > 0) aMDGPR->SetItems(newItems);
    ...
    for (Standard_Integer i = 1; i <= labels.Length(); i++) {
        ...
        if (XCAFDoc_ShapeTool::IsAssembly(L)) {
            ...
            for (Standard_Integer k = 1; k <= seq.Length(); k++) {
                ...
                TDF_Label Lref;
                if (!XCAFDoc_ShapeTool::GetReferredShape(lab, Lref) || !myLabels.IsBound(Lref)) continue;
...
static Standard_Boolean WritePropsForLabel(const Handle(XSControl_WorkSession) & WS,
                                           ...
                                           const STEPCAFControl_DataMapOfLabelShape& myLabels, const TDF_Label& L,
                                           const Standard_CString multi) {
...
Standard_Boolean STEPCAFControl_Writer::WriteLayers(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= LayerLS.Length(); i++) {
        ...
        for (Standard_Integer j = 1; j <= ShapeLs.Length(); j++) {
            ...
            if (XCAFDoc_ShapeTool::IsAssembly(shLabel) || XCAFDoc_ShapeTool::IsReference(shLabel)) continue;
            ...
            TopLoc_Location Loc;
            Standard_Integer nb = FindEntities(FP, oneShape, Loc, seqRI);
            if (nb <= 0)
                FP->Messenger()->SendInfo()
                    << "Warning: Cannot find RI for " << oneShape.TShape()->DynamicType()->Name() << std::endl;
        }
        ...
        if (L.FindAttribute(XCAFDoc::InvisibleGUID(), aUAttr)) {
            ...
            descr = new TCollection_HAsciiString("visible");

        // create layer entity
        Handle(StepVisual_HArray1OfLayeredItem) HArrayOfLItem = new StepVisual_HArray1OfLayeredItem(1, seqRI.Length());
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean getSHUOstyle(const TDF_Label& aSHUOlab, const Handle(XCAFDoc_ColorTool) & CTool,
                                     const Handle(XCAFDoc_VisMaterialTool) & theMatTool, XCAFPrs_Style& SHUOstyle) {
    Quantity_Color C;
    if (!CTool->IsVisible(aSHUOlab))
        ...
        if (CTool->GetColor(aSHUOlab, XCAFDoc_ColorSurf, C)) SHUOstyle.SetColorSurf(C);
        if (CTool->GetColor(aSHUOlab, XCAFDoc_ColorCurv, C)) SHUOstyle.SetColorCurv(C);
    ...
    if (!SHUOstyle.IsSetColorCurv() && !SHUOstyle.IsSetColorSurf() && SHUOstyle.IsVisible()) return Standard_False;
    return Standard_True;
}
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean getProDefinitionOfNAUO(const Handle(XSControl_WorkSession) & WS, const TopoDS_Shape& theShape,
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean writeSHUO(const Handle(XCAFDoc_GraphNode) & theSHUO, const Handle(XCAFDoc_ShapeTool) & theSTool,
                                  const Handle(XSControl_WorkSession) & WS,
                                  Handle(StepRepr_SpecifiedHigherUsageOccurrence) & theTopSHUO, TopoDS_Shape& NAUOShape,
                                  Handle(StepBasic_ProductDefinition) & theRelatingPD, Standard_Boolean& isDeepest) {
    ...
    if (theTopSHUO.IsNull()) {
        // the top SHUO
        if (aNextUsageLabs.Length() < 1) return Standard_False;
        theSTool->GetSHUO(aNextUsageLabs.Value(1), NuSHUO);
        if (NuSHUO.IsNull()) return Standard_False;
        ...
        if (!getProDefinitionOfNAUO(WS, aTopCompShape, theRelatingPD, UUNAUO, Standard_True)) return Standard_False;
        ...
        if (!getProDefinitionOfNAUO(WS, aNUShape, aRelatedPD, NUNAUO, Standard_False)) return Standard_False;
        ...
        theTopSHUO->Init(/*id*/ EmptyString, /*name*/ EmptyString,
                         /*no description*/ Standard_False, /*description*/ EmptyString, theRelatingPD, aRelatedPD,
...
#ifdef OCCT_DEBUG
        if (aNextUsageLabs.Length() > 1) std::cout << "Warning: store only one next_usage of current SHUO" << std::endl;
#endif
        theSTool->GetSHUO(aNextUsageLabs.Value(1), NuSHUO);
        Handle(StepRepr_SpecifiedHigherUsageOccurrence) aNUEntSHUO = new StepRepr_SpecifiedHigherUsageOccurrence;
        if (!writeSHUO(NuSHUO, theSTool, WS, aNUEntSHUO, NAUOShape, theRelatingPD, isDeepest)) return Standard_False;
...
static Standard_Boolean createSHUOStyledItem(const XCAFPrs_Style& style,
                                             ...
                                             const Handle(XSControl_WorkSession) & WS, const TopoDS_Shape& Sh,
    ...
    if (style.IsSetColorCurv()) curvColor = Styles.EncodeColor(style.GetColorCurv());
    ...
    Handle(StepShape_ContextDependentShapeRepresentation) CDSR;
    FP->FindTypedTransient(mapper, STANDARD_TYPE(StepShape_ContextDependentShapeRepresentation), CDSR);
    if (CDSR.IsNull()) return Standard_False;
    // find context
    ...
    if (Context.IsNull()) {
        ...
        if (aTopShL.IsNull()) return Standard_False;
        aTopSh = STool->GetShape(aTopShL);
        Context = Styles.FindContext(aTopSh);
    }
    if (Context.IsNull()) return Standard_False;
...
#ifdef OCCT_DEBUG
        std::cout << "Warning: " << __FILE__ << ": adds styled item of SHUO as root, cause cannot find MDGPR"
                  << std::endl;
#endif
    }
        ...
        if (XCAFDoc_ShapeTool::IsAssembly(L)) {
            ...
            for (Standard_Integer k = 1; k <= seq.Length(); k++) {
                ...
                for (Standard_Integer j = 1; j <= anAttrSeq.Length(); j++) {
                    Handle(XCAFDoc_GraphNode) aSHUO = Handle(XCAFDoc_GraphNode)::DownCast(anAttrSeq.Value(j));
                    // take label of SHUO
                    ...
                    if (aUpLabels.Length() > 0) continue;         // transfer only main SHUO
                    if (aMapOfMainSHUO.Contains(aSHUO)) continue; // do not try to transfer SHUO twice
                    ...
                    if (!getSHUOstyle(aSHUOlab, CTool, aMatTool, SHUOstyle)) {
                        ...
                        std::cout << "Warning: " << __FILE__
                                  << ": do not store SHUO without any style to the STEP model" << std::endl;
#endif
                        continue;
                    }
...
//=======================================================================
static Standard_Boolean FindPDSforDGT(const Interface_Graph& aGraph, const Handle(Standard_Transient) & ent,
                                      Handle(StepRepr_ProductDefinitionShape) & PDS,
                                      Handle(StepRepr_RepresentationContext) & RC, Handle(StepShape_AdvancedFace) & AF,
                                      Handle(StepShape_EdgeCurve) & EC) {
    if (ent.IsNull()) return Standard_False;
    if (!ent->IsKind(STANDARD_TYPE(StepShape_EdgeCurve)) && !ent->IsKind(STANDARD_TYPE(StepShape_AdvancedFace)))
        return Standard_False;
    ...
    for (subs.Start(); subs.More() && PDS.IsNull(); subs.Next()) {
        Handle(StepShape_ConnectedFaceSet) CFS = Handle(StepShape_ConnectedFaceSet)::DownCast(subs.Value());
        if (CFS.IsNull()) continue;
        Interface_EntityIterator subs1 = aGraph.Sharings(CFS);
        for (subs1.Start(); subs1.More() && PDS.IsNull(); subs1.Next()) {
            Handle(StepRepr_RepresentationItem) RI = Handle(StepRepr_RepresentationItem)::DownCast(subs1.Value());
...
//=======================================================================
static Handle(StepRepr_ProductDefinitionShape)
    FindPDS(const Interface_Graph& theGraph, const Handle(Standard_Transient) & theEnt,
            Handle(StepRepr_RepresentationContext) & theRC) {
    if (theEnt.IsNull()) return NULL;
...
static StepBasic_Unit GetUnit(const Handle(StepRepr_RepresentationContext) & theRC,
    ...
    if (isAngle) {
        ...
        if (aCurrentUnit.IsNull()) aCurrentUnit = new StepBasic_SiUnitAndPlaneAngleUnit;
    } else {
        Handle(StepGeom_GeometricRepresentationContextAndGlobalUnitAssignedContext) aCtx =
            Handle(StepGeom_GeometricRepresentationContextAndGlobalUnitAssignedContext)::DownCast(theRC);
        if (!aCtx.IsNull()) {
        ...
        if (aCurrentUnit.IsNull()) aCurrentUnit = new StepBasic_SiUnitAndLengthUnit;
    }
...
// purpose  : auxiliary
//======================================================================
static Handle(StepRepr_ReprItemAndMeasureWithUnit)
    CreateDimValue(const Standard_Real theValue, const StepBasic_Unit theUnit,
                   const Handle(TCollection_HAsciiString) & theName, const Standard_CString theMeasureName,
                   const Standard_Boolean isAngle, const Standard_Boolean isQualified = Standard_False,
    ...
    if (isQualified) {
        ...
        if (isAngle) {
            ...
            Handle(StepRepr_ReprItemAndLengthMeasureWithUnit) anItem = new StepRepr_ReprItemAndLengthMeasureWithUnit();
            anItem->Init(aMWU, aReprItem);
            return anItem;
        }
    }
...
Handle(StepRepr_ShapeAspect) STEPCAFControl_Writer::WriteShapeAspect(
    const Handle(XSControl_WorkSession) & WS, const TDF_Label theLabel, const TopoDS_Shape theShape,
    Handle(StepRepr_RepresentationContext) & theRC, Handle(StepAP242_GeometricItemSpecificUsage) & theGISU) {
    // Get working data
    const Handle(Interface_InterfaceModel) & Model = WS->Model();
    const Handle(XSControl_TransferWriter) & TW = WS->TransferWriter();
    const Handle(Transfer_FinderProcess) & FP = TW->FinderProcess();
    const Handle(Interface_HGraph) aHGraph = WS->HGraph();
    if (aHGraph.IsNull()) return NULL;
    ...
    if (aSeqRI.Length() <= 0) {
        FP->Messenger()->SendInfo() << "Warning: Cannot find RI for " << theShape.TShape()->DynamicType()->Name()
                                    << std::endl;
        return NULL;
    }
...
void STEPCAFControl_Writer::WritePresentation(const Handle(XSControl_WorkSession) & WS,
                                              ...
                                              const Standard_Boolean hasSemantic, const Standard_Boolean hasPlane,
                                              const gp_Ax2 theAnnotationPlane, const gp_Pnt theTextPosition,
                                              const Handle(Standard_Transient) theDimension) {
    if (thePresentation.IsNull()) return;
    // Get working data
    ...
    Handle(StepAP242_DraughtingModelItemAssociation) aDMIA = new StepAP242_DraughtingModelItemAssociation();
...
Handle(StepDimTol_Datum) STEPCAFControl_Writer::WriteDatumAP242(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= theShapeL.Length(); i++) {
        ...
        if (aSeqRI.Length() <= 0) {
            FP->Messenger()->SendInfo() << "Warning: Cannot find RI for " << aShape.TShape()->DynamicType()->Name()
                                        << std::endl;
            continue;
        }
        anEnt = aSeqRI.Value(1);
        aPDS = FindPDS(aGraph, anEnt, aRC);
        if (aPDS.IsNull()) continue;

        Handle(StepRepr_ShapeAspect) aCurrentSA = WriteShapeAspect(WS, theDatumL, aShape, aRC, aGISU);
        if (aCurrentSA.IsNull()) continue;
    ...
    Handle(TCollection_HAsciiString) aTargetId =
        (anObject->GetDatumTargetNumber() == 0 ? new TCollection_HAsciiString()
    ...
    if (anObject->GetDatumTargetType() == XCAFDimTolObjects_DatumTargetType_Area && anObject->GetDatumTarget().IsNull())
    ...
    if (isSimpleDatum) {
        if (aSASeq.Length() == 0) {
            ...
            for (Standard_Integer i = 1; i <= aSASeq.Length(); i++) {
                ...
                aSAR->Init(new TCollection_HAsciiString(), Standard_False, new TCollection_HAsciiString(), aDF,
                           aSASeq.Value(i));
    ...
    // Datum with datum targets
            ...
            if (anObject->HasDatumTargetParams()) {
                ...
                if (aDatumType == XCAFDimTolObjects_DatumTargetType_Point) {
                    ...
                    if (aDatumType == XCAFDimTolObjects_DatumTargetType_Line) {
                        ...
                        aTargetValueName = new TCollection_HAsciiString("target length");
                        // Additional value
                        Handle(StepRepr_ReprItemAndMeasureWithUnit) aTargetValue = CreateDimValue(
                            anObject->GetDatumTargetWidth(), aUnit, new TCollection_HAsciiString("target width"),
                            "POSITIVE_LENGTH_MEASURE", Standard_False);
                    ...
                    Handle(StepRepr_ReprItemAndMeasureWithUnit) aTargetValue =
                        CreateDimValue(anObject->GetDatumTargetLength(), aUnit, aTargetValueName,
                                       "POSITIVE_LENGTH_MEASURE", Standard_False);
        ...
        if (aSASeq.Length() == 1) {
            ...
            aCompSA->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(), aPDS,
                          aSASeq.Value(1)->ProductDefinitional());
            for (Standard_Integer i = 1; i <= aSASeq.Length(); i++) {
                Handle(StepRepr_ShapeAspectRelationship) aSAR = new StepRepr_ShapeAspectRelationship();
                aSAR->Init(new TCollection_HAsciiString(), Standard_False, new TCollection_HAsciiString(), aCompSA,
                           aSASeq.Value(i));
                Model->AddWithRefs(aSAR);
            }
            Handle(StepRepr_FeatureForDatumTargetRelationship) aFFDTR =
    ...
    if (isFirstDTarget) {
        ...
        aDatum->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(), aPDS, StepData_LFalse,
                     anIdentifier);
    ...
    WritePresentation(WS, anObject->GetPresentation(), anObject->GetPresentationName(), Standard_True,
                      anObject->HasPlane(), anObject->GetPlane(), anObject->GetPointTextAttach(), aSA);
...
static void WriteDimValues(const Handle(XSControl_WorkSession) & WS,
    ...
    Standard_Boolean isAngle =
        aDim->IsKind(STANDARD_TYPE(StepShape_AngularLocation)) || aDim->IsKind(STANDARD_TYPE(StepShape_AngularSize));
    ...
    if (theObject->IsDimWithRange()) aNbItems += 2;
    if (aModifiers.Length() > 0) aNbItems++;
    if (theObject->GetType() == XCAFDimTolObjects_DimensionType_Location_Oriented) aNbItems++;
    ...
    if (theObject->HasQualifier() && !isAngle) aNbQualifiers++;
    if (aLeftNbDigits > 0 || aRightNbDigits > 0) aNbQualifiers++;
    // With qualifiers
    if (aNbQualifiers > 0) {
        ...
        Handle(StepRepr_ReprItemAndMeasureWithUnit) anItem = CreateDimValue(
            aNominal, aUnit, new TCollection_HAsciiString("nominal value"), aMeasureName, isAngle, Standard_True, aQRI);
    ...
    // Without qualifiers
        ...
        Handle(StepRepr_ReprItemAndMeasureWithUnit) anItem =
            CreateDimValue(aNominal, aUnit, new TCollection_HAsciiString("nominal value"), aMeasureName, isAngle);
        aValues->SetValue(aValIt, anItem);
        aValIt++;
    }
    ...
    if (theObject->IsDimWithRange()) {
        Handle(StepRepr_ReprItemAndMeasureWithUnit) aLowerItem = CreateDimValue(
            theObject->GetLowerBound(), aUnit, new TCollection_HAsciiString("lower limit"), aMeasureName, isAngle);
        Handle(StepRepr_ReprItemAndMeasureWithUnit) anUpperItem = CreateDimValue(
            theObject->GetUpperBound(), aUnit, new TCollection_HAsciiString("upper limit"), aMeasureName, isAngle);
    ...
    if (aModifiers.Length() > 0) {
        ...
        for (Standard_Integer i = 1; i <= aModifiers.Length(); i++) {
            ...
            Handle(StepRepr_DescriptiveRepresentationItem) aModifItem = new StepRepr_DescriptiveRepresentationItem();
    ...
    if (theObject->GetType() == XCAFDimTolObjects_DimensionType_Location_Oriented) {
        ...
        anOrientation->Init(new TCollection_HAsciiString("orientation"), aLoc, Standard_True, anAxis, Standard_False,
                            NULL);
    ...
    if (theObject->IsDimWithPlusMinusTolerance()) {
        ...
        Standard_Real aLowerTolValue = -theObject->GetLowerTolValue(), anUpperTolValue = theObject->GetUpperTolValue();
        // Upper
    ...
    if (theObject->IsDimWithClassOfTolerance()) {
        ...
        if (!theObject->GetClassOfTolerance(isHole, aFormVariance, aGrade)) return;
...
static void WriteDerivedGeometry(const Handle(XSControl_WorkSession) & WS,
                                 ...
                                 Handle(StepRepr_ShapeAspect) & theFirstSA, Handle(StepRepr_ShapeAspect) & theSecondSA,
    ...
    if (theObject->HasPoint()) {
        ...
        aDSA->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(), theFirstSA->OfShape(),
                   StepData_LFalse);
        Handle(StepAP242_GeometricItemSpecificUsage) aGISU = new StepAP242_GeometricItemSpecificUsage();
        StepAP242_ItemIdentifiedRepresentationUsageDefinition aDefinition;
...
static Handle(StepDimTol_HArray1OfDatumSystemOrReference)
    WriteDatumSystem(const Handle(XSControl_WorkSession) & WS, const TDF_Label theGeomTolL,
                     const TDF_LabelSequence theDatumSeq, const STEPConstruct_DataMapOfAsciiStringTransient theDatumMap,
                     const Handle(StepRepr_RepresentationContext) & theRC) {
    // Get working data
    const Handle(Interface_InterfaceModel) & Model = WS->Model();
    const Handle(Interface_HGraph) aHGraph = WS->HGraph();
    ...
    if (!theGeomTolL.FindAttribute(XCAFDoc_GeomTolerance::GetID(), aGTAttr)) return NULL;
    ...
    for (Standard_Integer i = 1; i <= aMaxDatumNum; i++) {
        ...
        for (Standard_Integer j = 1; j <= aDatums.Length(); j++)
            if (aDatums.Value(j)->GetPosition() == i) aDatumSeqPos.Append(aDatums.Value(j));
        if (aDatumSeqPos.Length() < 1) continue;

        aConstituentsNum++;
        // Initialize Datum_Reference_Compartment
        StepDimTol_DatumOrCommonDatum aDatumRef;
        Handle(StepDimTol_DatumReferenceCompartment) aCompartment = new StepDimTol_DatumReferenceCompartment();
        ...
        if (aDatumSeqPos.Length() == 1) {
            ...
            for (Standard_Integer j = 1; j <= aDatumSeqPos.Length(); j++) {
                ...
                if (aFirstDatum.IsNull()) aFirstDatum = aDatum;
                // Modifiers
                ...
                anElement->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(), aDatum->OfShape(),
                                aDatum->ProductDefinitional(), anElemDatumRef, !anElemModifiers.IsNull(),
                                anElemModifiers);
    ...
    for (Standard_Integer i = 1; i <= aConstituents->Length(); i++)
        if (!aConstituents->Value(i).IsNull()) aNbConstituents++;
    ...
    if (anObject->HasAxis()) {
        ...
        if (aSDR.IsNull()) return aDatumSystem;

        aGISU->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(), aDefinition,
                    aSDR->UsedRepresentation(), anReprItems);
...
void STEPCAFControl_Writer::WriteToleranceZone(const Handle(XSControl_WorkSession) & WS,
    ...
    // Get working data
    const Handle(Interface_InterfaceModel) & Model = WS->Model();
    if (theEntity.IsNull() || theObject.IsNull()) return;
    ...
    aZone->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(),
                theEntity->TolerancedShapeAspect().ShapeAspect()->OfShape(), StepData_LFalse, aZoneTargetArray, aForm);
...
void STEPCAFControl_Writer::WriteGeomTolerance(const Handle(XSControl_WorkSession) & WS,
                                               const TDF_LabelSequence theShapeSeqL, const TDF_Label theGeomTolL,
    ...
    const Handle(Interface_InterfaceModel) & Model = WS->Model();
    Handle(XCAFDoc_GeomTolerance) aGTAttr;
    if (!theGeomTolL.FindAttribute(XCAFDoc_GeomTolerance::GetID(), aGTAttr)) return;
    Handle(XCAFDimTolObjects_GeomToleranceObject) anObject = aGTAttr->GetObject();
    ...
    Standard_Boolean isWithModif = Standard_False, isWithDatRef = Standard_False, isWithMaxTol = Standard_False;
    // Modifiers
    // Simple modifiers
    ...
    if (anObject->GetMaterialRequirementModifier() != XCAFDimTolObjects_GeomToleranceMatReqModif_None) aModifNb++;
    for (Standard_Integer i = 1; i <= aModifiers.Length(); i++)
    ...
    StepDimTol_GeometricToleranceType aType = STEPCAFControl_GDTProperty::GetGeomToleranceType(anObject->GetType());
    ...
    if (isWithModif) {
        if (isWithMaxTol) {
            if (isWithDatRef) {
                ...
                Handle(StepDimTol_GeoTolAndGeoTolWthMaxTol) aResult = new StepDimTol_GeoTolAndGeoTolWthMaxTol();
        ...
        if (isWithDatRef) {
            // Geometric_Tolerance & Geometric_Tolerance_With_Datum_Reference
            Handle(StepDimTol_GeoTolAndGeoTolWthDatRef) aResult = new StepDimTol_GeoTolAndGeoTolWthDatRef();
    ...
    // Annotation plane and Presentation
    WritePresentation(WS, anObject->GetPresentation(), anObject->GetPresentationName(), Standard_True,
                      anObject->HasPlane(), anObject->GetPlane(), anObject->GetPointTextAttach(), aGeomTol);
}
...
Standard_Boolean STEPCAFControl_Writer::WriteDGTs(const Handle(XSControl_WorkSession) & WS,
    ...
    const Handle(Interface_HGraph) aHGraph = WS->HGraph();
    if (aHGraph.IsNull()) return Standard_False;
    ...
    for (i = 1; i <= DGTLabels.Length(); i++) {
        ...
        Handle(StepRepr_HArray1OfRepresentationItem) HARI1 = new StepRepr_HArray1OfRepresentationItem(1, 1);
    ...
    for (i = 1; i <= DGTLabels.Length(); i++) {
        ...
        if (kind < 20) { // dimension
            ...
            if (aVal->Length() > 1) {
                ...
                // CID.SetValue(CIDM);
                Handle(StepRepr_HArray1OfRepresentationItem) HARIVR = new StepRepr_HArray1OfRepresentationItem(1, 2);
                ...
                Model->AddEntity(VR);
                Handle(StepShape_ShapeDimensionRepresentation) SDimR = new StepShape_ShapeDimensionRepresentation;
            ...
            if (kind < 35) {    // tolerance with datum system
                ...
                for (Standard_Integer j = 1; j <= NbDR; j++) {
                    ...
                    if (DatumMap.IsBound(stmp)) {
                        Handle(StepDimTol_Datum) aDatum = Handle(StepDimTol_Datum)::DownCast(DatumMap.Find(stmp));
                ...
                if (kind < 24) {
                    ...
                    if (kind == 21)
                        MGT->SetModifier(StepDimTol_MaximumMaterialCondition);
                    else if (kind == 22)
                        MGT->SetModifier(StepDimTol_LeastMaterialCondition);
                    else if (kind == 23)
                        MGT->SetModifier(StepDimTol_RegardlessOfFeatureSize);
                    ...
                    Handle(StepDimTol_CircularRunoutTolerance) aToler = new StepDimTol_CircularRunoutTolerance;
                    aToler->Init(aName, aDescription, LMWU, SA, HADR);
                    Model->AddWithRefs(aToler);
                } else if (kind == 26) {
                    ...
                    Handle(StepDimTol_PerpendicularityTolerance) aToler = new StepDimTol_PerpendicularityTolerance;
                    aToler->Init(aName, aDescription, LMWU, SA, HADR);
                    Model->AddWithRefs(aToler);
                } else if (kind == 30) {
...
Standard_Boolean STEPCAFControl_Writer::WriteDGTsAP242(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
        ...
        if (!aNameIdMap.Add(aDatumName.Cat(aDatumTargetId))) continue;
        Handle(Standard_Transient) aWrittenDatum;
        Standard_Boolean isFirstDT = !aDatumMap.Find(aDatumName, aWrittenDatum);
        Handle(StepDimTol_Datum) aDatum =
            WriteDatumAP242(WS, aShapeL, aDatumL, isFirstDT, Handle(StepDimTol_Datum)::DownCast(aWrittenDatum));
        // Add created Datum into Map
    ...
    for (Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
        ...
        if (!aDimensionL.FindAttribute(XCAFDoc_Dimension::GetID(), aDimAttr)) continue;
        ...
        if (!DGTTool->GetRefShapeLabel(aDimensionL, aFirstShapeL, aSecondShapeL)) continue;
        ...
        if (aFirstShapeL.Length() == 1) {
            ...
            if (aRC.IsNull() && !dummyRC.IsNull()) aRC = dummyRC;
        ...
        if (aSecondShapeL.Length() == 1) {
            ...
            for (Standard_Integer shIt = 1; shIt <= aSecondShapeL.Length(); shIt++) {
                ...
                if (!aSA.IsNull()) {
                    Handle(StepRepr_ShapeAspectRelationship) aSAR = new StepRepr_ShapeAspectRelationship();
                    aSAR->Init(new TCollection_HAsciiString(), Standard_False, new TCollection_HAsciiString(), aCSA,
                               aSA);
                    aModel->AddWithRefs(aSAR);
                }
                if (aRC.IsNull() && !dummyRC.IsNull()) aRC = dummyRC;
        ...
        if (STEPCAFControl_GDTProperty::IsDimensionalLocation(aDimType)) {
            ...
            Handle(StepRepr_ShapeAspect) aPathSA =
                WriteShapeAspect(WS, aDimensionL, anObject->GetPath(), dummyRC, dummyGISU);
        ...
        // Annotation plane and Presentation
        WritePresentation(WS, anObject->GetPresentation(), anObject->GetPresentationName(), Standard_True,
                          anObject->HasPlane(), anObject->GetPlane(), anObject->GetPointTextAttach(),
                          aDimension.Value());
    }
    ...
    if (aConnectionPnts.Length() > 0) {
        ...
        aCGReprRel->Init(new TCollection_HAsciiString(), new TCollection_HAsciiString(),
                         dummyGISU->UsedRepresentation(), aCGRepr);
    ...
    for (Standard_Integer i = 1; i <= aDGTLabels.Length(); i++) {
        ...
        if (!DGTTool->GetRefShapeLabel(aGeomTolL, aFirstShapeL, aNullSeqL)) continue;
        TDF_LabelSequence aDatumSeq;
        DGTTool->GetDatumWithObjectOfTolerLabels(aGeomTolL, aDatumSeq);
        Handle(StepDimTol_HArray1OfDatumSystemOrReference) aDatumSystem;
        if (aDatumSeq.Length() > 0) aDatumSystem = WriteDatumSystem(WS, aGeomTolL, aDatumSeq, aDatumMap, aRC);
        WriteGeomTolerance(WS, aFirstShapeL, aGeomTolL, aDatumSystem, aRC);
    }

    // Write Draughting model for Annotation Planes
    if (myGDTAnnotations.Length() == 0) return Standard_True;
...
// purpose  : auxiliary:
//=======================================================================
static Standard_Boolean FindPDSforRI(const Interface_Graph& aGraph, const Handle(Standard_Transient) & ent,
                                     Handle(StepRepr_ProductDefinitionShape) & PDS,
                                     Handle(StepRepr_RepresentationContext) & RC) {
    if (ent.IsNull() || !ent->IsKind(STANDARD_TYPE(StepRepr_RepresentationItem))) return Standard_False;
    Interface_EntityIterator subs = aGraph.Sharings(ent);
    for (subs.Start(); subs.More() && PDS.IsNull(); subs.Next()) {
        Handle(StepShape_ShapeRepresentation) SR = Handle(StepShape_ShapeRepresentation)::DownCast(subs.Value());
        if (SR.IsNull()) continue;
        RC = SR->ContextOfItems();
    ...
    for (Standard_Integer i = 1; i <= TopLabels.Length(); i++) {
        ...
        if (ShL.FindAttribute(XCAFDoc::MaterialRefGUID(), Node) && Node->HasFather()) {
            ...
            if (MatTool->GetMaterial(MatL, aName, aDescription, aDensity, aDensName, aDensValType)) {
                ...
                if (MapDRI.IsBound(aKey)) {
                    ...
                    if (aDensity > 0) {
                        ...
                        DUE2->Init(SLU, 2.0);
                        // other
                        Handle(StepBasic_HArray1OfDerivedUnitElement) HADUE =
            ...
            if (!RepDRI.IsNull()) {
                ...
                StepRepr_RepresentedDefinition RD1;
                RD1.SetValue(PropD1);
                Handle(StepRepr_PropertyDefinitionRepresentation) PDR1 = new StepRepr_PropertyDefinitionRepresentation;
