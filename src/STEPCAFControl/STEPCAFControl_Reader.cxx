...
STEPCAFControl_Reader::STEPCAFControl_Reader()
    : myColorMode(Standard_True), myNameMode(Standard_True), myLayerMode(Standard_True), myPropsMode(Standard_True),
      mySHUOMode(Standard_False), myGDTMode(Standard_True), myMatMode(Standard_True), myViewMode(Standard_True) {
...
STEPCAFControl_Reader::STEPCAFControl_Reader(const Handle(XSControl_WorkSession) & WS, const Standard_Boolean scratch)
    : myColorMode(Standard_True), myNameMode(Standard_True), myLayerMode(Standard_True), myPropsMode(Standard_True),
      mySHUOMode(Standard_False), myGDTMode(Standard_True), myMatMode(Standard_True), myViewMode(Standard_True) {
    STEPCAFControl_Controller::Init();
    Init(WS, scratch);
...
void STEPCAFControl_Reader::Init(const Handle(XSControl_WorkSession) & WS, const Standard_Boolean scratch) {
...
TCollection_ExtendedString STEPCAFControl_Reader::convertName(const TCollection_AsciiString& theName) const {
    ...
    return TCollection_ExtendedString(theName,
                                      myReader.StepModel()->SourceCodePage() != Resource_FormatType_NoConversion);
}
...
Standard_Boolean STEPCAFControl_Reader::TransferOneRoot(const Standard_Integer num, Handle(TDocStd_Document) & doc,
...
Standard_Boolean STEPCAFControl_Reader::Perform(const Standard_CString filename, Handle(TDocStd_Document) & doc,
                                                const Message_ProgressRange& theProgress) {
...
Standard_Boolean STEPCAFControl_Reader::Perform(const TCollection_AsciiString& filename, Handle(TDocStd_Document) & doc,
                                                const Message_ProgressRange& theProgress) {
...
Standard_Boolean STEPCAFControl_Reader::Transfer(STEPControl_Reader& reader, const Standard_Integer nroot,
                                                 Handle(TDocStd_Document) & doc, TDF_LabelSequence& Lseq,
    ...
    if (aPSRoot.UserBreak()) return Standard_False;
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (enti->IsKind(STANDARD_TYPE(StepShape_ShapeRepresentation))) {
            ...
            if (index > 0) {
                Handle(Transfer_Binder) binder = TP->MapItem(index);
                TopoDS_Shape S = TransferBRep::ShapeResult(binder);
                if (!S.IsNull() && ShapesMap.Contains(S)) NewShapesMap.Add(S);
            }
        }
    }
    ...
    for (i = 1; i <= ExtRefs.NbExternRefs() && aPSE.More(); i++) {
        ...
        if (!format.IsNull()) {
            ...
            if (!format->IsSameString(ap203, Standard_False) && !format->IsSameString(ap214, Standard_False)) {
#ifdef OCCT_DEBUG
                std::cout << "Warning: STEPCAFControl_Reader::Transfer: Extern document is neither STEP AP203 nor AP214"
                          << std::endl;
#else
                continue;
#endif
            ...
            std::cout << "Warning: STEPCAFControl_Reader::Transfer: Extern document format not defined" << std::endl;
#endif
        ...
        */
        ...
        Handle(STEPCAFControl_ExternFile) EF = ReadExternFile(filename, fullname.ToCString(), doc, aRange);
        PDFileMap.Bind(PD, EF);
    }
    ...
    // read colors
    if (GetColorMode()) ReadColors(reader.WS(), doc);

    // read names
    if (GetNameMode()) ReadNames(reader.WS(), doc, PDFileMap);

    // read validation props
    if (GetPropsMode()) ReadValProps(reader.WS(), doc, PDFileMap);

    // read layers
    if (GetLayerMode()) ReadLayers(reader.WS(), doc);

    // read SHUO entities from STEP model
    if (GetSHUOMode()) ReadSHUOs(reader.WS(), doc, PDFileMap);

    // read GDT entities from STEP model
    if (GetGDTMode()) ReadGDTs(reader.WS(), doc);

    // read Material entities from STEP model
    if (GetMatMode()) ReadMaterials(reader.WS(), doc, SeqPDS);

    // read View entities from STEP model
    if (GetViewMode()) ReadViews(reader.WS(), doc);
...
TDF_Label STEPCAFControl_Reader::AddShape(const TopoDS_Shape& S, const Handle(XCAFDoc_ShapeTool) & STool,
    ...
    if (ShapePDMap.IsBound(S) && PDFileMap.IsBound(ShapePDMap.Find(S))) {
        ...
        if (!EF.IsNull()) {
            ...
            if (!EF->GetLabel().IsNull())
                std::cout
                    << "Warning: STEPCAFControl_Reader::AddShape: Non-empty shape with external ref; ref is ignored"
                    << std::endl;
            else if (nbComponents <= 0)
                std::cout << "Warning: STEPCAFControl_Reader::AddShape: Result of reading extern ref is Null"
                          << std::endl;
#endif
        }
    }
...
static void propagateColorToParts(const Handle(XCAFDoc_ShapeTool) & theSTool,
                                  const Handle(XCAFDoc_ColorTool) & theCTool, const TDF_Label& theRoot) {
    ...
    for (TDF_LabelSequence::Iterator anIt(aComponents); anIt.More(); anIt.Next()) {
        ...
        if (!theCTool->IsVisible(theRoot)) theCTool->SetVisibility(anOriginalL, Standard_False);

        // propagate to next level children
        if (theSTool->IsAssembly(anOriginalL)) propagateColorToParts(theSTool, theCTool, anOriginalL);
    }
}
//=======================================================================
// function : ReadColors
...
Standard_Boolean STEPCAFControl_Reader::ReadColors(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        for (Standard_Integer si = 1; si <= aHSeqOfInvisStyle->Length(); si++) {
            if (style != aHSeqOfInvisStyle->Value(si)) continue;
        ...
        for (Standard_Integer itemIt = 0; itemIt < anItems.Length(); itemIt++) {
            ...
            if (isSkipSHUOstyle) continue; // skip styled item which refer to SHUO
            ...
            if (!SurfCol.IsNull() || !BoundCol.IsNull() || !CurveCol.IsNull() || !RenderCol.IsNull() || !IsVisible) {
                ...
                if (!SurfCol.IsNull() || !BoundCol.IsNull() || !CurveCol.IsNull() || !RenderCol.IsNull()) {
                    ...
                    if (!BoundCol.IsNull()) Styles.DecodeColor(BoundCol, aBCol);
                    if (!CurveCol.IsNull()) Styles.DecodeColor(CurveCol, aCCol);
                    if (!RenderCol.IsNull()) {
                        Styles.DecodeColor(RenderCol, aRCol);
                        aFullSCol = Quantity_ColorRGBA(aRCol, static_cast<float>(1.0f - RenderTransp));
                    }
                    if (isFound) {
                        if (!SurfCol.IsNull() || !RenderCol.IsNull()) CTool->SetColor(aL, aFullSCol, XCAFDoc_ColorSurf);
                        if (!BoundCol.IsNull()) CTool->SetColor(aL, aBCol, XCAFDoc_ColorCurv);
                        if (!CurveCol.IsNull()) CTool->SetColor(aL, aCCol, XCAFDoc_ColorCurv);
                    } else {
                        for (TopoDS_Iterator it(S); it.More(); it.Next()) {
                            TDF_Label aL1;
                            if (STool->SearchUsingMap(it.Value(), aL1, Standard_False, Standard_True)) {
                                ...
                                if (!BoundCol.IsNull()) CTool->SetColor(aL1, aBCol, XCAFDoc_ColorCurv);
                                if (!CurveCol.IsNull()) CTool->SetColor(aL1, aCCol, XCAFDoc_ColorCurv);
                            }
                        }
                    }
                }
                if (!IsVisible) {
                    ...
                    if (isFound) CTool->SetVisibility(aL, Standard_False);
                }
            }
        }
    }
...
static TDF_Label GetLabelFromPD(const Handle(StepBasic_ProductDefinition) & PD, const Handle(XCAFDoc_ShapeTool) & STool,
    ...
    if (ShapeLabelMap.IsBound(S)) L = ShapeLabelMap.Find(S);
    if (L.IsNull()) STool->Search(S, L, Standard_True, Standard_True, Standard_False);
    return L;
}
...
TDF_Label STEPCAFControl_Reader::FindInstance(const Handle(StepRepr_NextAssemblyUsageOccurrence) & NAUO,
                                              const Handle(XCAFDoc_ShapeTool) & STool, const STEPConstruct_Tool& Tool,
                                              const XCAFDoc_DataMapOfShapeLabel& ShapeLabelMap) {
    TDF_Label L;
            ...
            if (subs.Value()->IsKind(STANDARD_TYPE(StepRepr_ShapeAspect))) {
                ...
                for (Standard_Integer ir = 1; ir <= SR->NbItems() && binder.IsNull(); ir++) {
                    if (SR->ItemsValue(ir)->IsKind(STANDARD_TYPE(StepShape_SolidModel))) {
                        Handle(StepShape_SolidModel) SM = Handle(StepShape_SolidModel)::DownCast(SR->ItemsValue(ir));
                        binder = TP->Find(SM);
                ...
                S = TransferBRep::ShapeResult(TP, binder);
                if (S.IsNull()) continue;
                if (myMap.IsBound(S)) L = myMap.Find(S);
                if (L.IsNull()) STool->Search(S, L, Standard_True, Standard_True, Standard_True);
            }
        }
...
Standard_Boolean STEPCAFControl_Reader::ReadLayers(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (SVPLA->AssignedItems().IsNull()) continue;
        ...
        for (Standard_Integer j = 1; j <= SVPLA->NbAssignedItems(); j++) {
            ...
            if (aLayerLabel.IsNull()) aLayerLabel = LTool->AddLayer(aLayerName, isVisible);
            LTool->SetLayer(shL, aLayerLabel);
        }

        if (!aLayerLabel.IsNull()) LTool->SetVisibility(aLayerLabel, isVisible);
    }
    return Standard_True;
}
...
static Standard_Boolean findNextSHUOlevel(const Handle(XSControl_WorkSession) & WS,
    ...
    if (subSHUO.IsNull()) return Standard_False;

    Handle(StepRepr_NextAssemblyUsageOccurrence) NUNAUO = subSHUO->NextUsage();
    if (NUNAUO.IsNull()) return Standard_False;
    ...
    if (NULab.IsNull()) return Standard_False;
...
static TDF_Label setSHUOintoDoc(const Handle(XSControl_WorkSession) & WS,
...
#ifdef OCCT_DEBUG
        std::cout << "Warning: " << __FILE__ << ": Upper_usage or Next_usage of styled SHUO is null. Skip it"
                  << std::endl;
#endif
        return aMainLabel;
    }
    ...
    if (STool->SetSHUO(ShuoLabels, anSHUOAttr)) aMainLabel = anSHUOAttr->Label();
...
Standard_Boolean STEPCAFControl_Reader::ReadSHUOs(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        for (Standard_Integer si = 1; si <= aHSeqOfInvisStyle->Length(); si++) {
            ...
            // found that current style is invisible.
#ifdef OCCT_DEBUG
            std::cout << "Warning: item No " << i << "(" << style->Item()->DynamicType()->Name() << ") is invisible"
                      << std::endl;
#endif
            IsVisible = Standard_False;
            break;
...
// purpose  :
//=======================================================================
static Standard_Boolean GetMassConversionFactor(Handle(StepBasic_NamedUnit) & NU, Standard_Real& afact) {
    afact = 1.;
    ...
    if (anUnit2.CaseNum(anUnit2.Value()) == 1) {
        ...
        if (NU2->IsKind(STANDARD_TYPE(StepBasic_SiUnit))) {
            ...
            if (SU->Name() == StepBasic_sunGram) {
                if (SU->HasPrefix()) afact *= STEPConstruct_UnitContext::ConvertSiPrefix(SU->Prefix());
            }
        }
    }
    return Standard_True;
...
Standard_Boolean readPMIPresentation(const Handle(Standard_Transient) & thePresentEntity,
                                     const Handle(XSControl_TransferReader) & theTR, const Standard_Real theFact,
                                     TopoDS_Shape& thePresentation, Handle(TCollection_HAsciiString) & thePresentName,
                                     Bnd_Box& theBox) {
    if (thePresentEntity.IsNull()) return Standard_False;
    ...
    if (thePresentEntity->IsKind(STANDARD_TYPE(StepVisual_AnnotationOccurrence))) {
        ...
        for (Standard_Integer i = 1; i <= aDCallout->NbContents() && anAO.IsNull(); i++) {
            ...
            if (!aTesselation.IsNull()) anAnnotations.Append(aTesselation);
        }
    }

    if (!anAnnotations.Length()) return Standard_False;
    ...
    for (; i < anAnnotations.Length(); i++) {
        ...
        // case of tessellated entities
            ...
            if (aTessItem.IsNull()) continue;
            Handle(StepVisual_TessellatedGeometricSet) aTessSet =
                Handle(StepVisual_TessellatedGeometricSet)::DownCast(aTessItem);
            if (aTessSet.IsNull()) continue;
            ...
            for (; k < aNbC; k++) {
                ...
                for (Standard_Integer n = 1; n < anIndexes->Length(); n++) {
                    ...
                    if (ind > aPoints->Length() || indnext > aPoints->Length()) continue;
        ...
        if (!anAnnotationShape.IsNull()) {
            ...
            if (i == anAnnotations.Length() - 1) BRepBndLib::AddClose(anAnnotationShape, aBox);
        }
    }
...
//=======================================================================
Standard_Boolean readAnnotationPlane(const Handle(StepVisual_AnnotationPlane) theAnnotationPlane, gp_Ax2& thePlane) {
    if (theAnnotationPlane.IsNull()) return Standard_False;
    Handle(StepRepr_RepresentationItem) aPlaneItem = theAnnotationPlane->Item();
    if (aPlaneItem.IsNull()) return Standard_False;
    ...
    if (aA2P3D.IsNull()) return Standard_False;
...
//=======================================================================
void readAnnotation(const Handle(XSControl_TransferReader) & theTR, const Handle(Standard_Transient) & theGDT,
                    const Handle(Standard_Transient) & theDimObject) {
    if (theGDT.IsNull() || theDimObject.IsNull()) return;
    ...
    for (subs.Start(); subs.More() && aDMIA.IsNull(); subs.Next()) {
        if (!subs.Value()->IsKind(STANDARD_TYPE(StepAP242_DraughtingModelItemAssociation))) continue;
    ...
    if (aDMIA.IsNull() || aDMIA->NbIdentifiedItem() == 0) return;
    ...
    if (isHasPlane) {
        if (theDimObject->IsKind(STANDARD_TYPE(XCAFDimTolObjects_DimensionObject))) {
            ...
            Handle(XCAFDimTolObjects_DatumObject) anObj = Handle(XCAFDimTolObjects_DatumObject)::DownCast(theDimObject);
            anObj->SetPlane(aPlaneAxes);
    ...
    Bnd_Box aBox;
    if (!readPMIPresentation(aDMIAE, theTR, aFact, aResAnnotation, aPresentName, aBox)) return;
    gp_Pnt aPtext(0., 0., 0.);
...
//=======================================================================
void readConnectionPoints(const Handle(XSControl_TransferReader) & theTR, const Handle(Standard_Transient) theGDT,
    ...
    if (theGDT->IsKind(STANDARD_TYPE(StepShape_DimensionalSize))) {
        ...
        if (aGISU.IsNull() || aGISU->NbIdentifiedItem() == 0) return;
        ...
        if (aPoint.IsNull()) {
            ...
            if (anA2P3D.IsNull()) return;
            aPoint = anA2P3D->Location();
        }

        // set connection point to object
        gp_Pnt aPnt(aPoint->CoordinatesValue(1) * aFact, aPoint->CoordinatesValue(2) * aFact,
        ...
        if (aDSA1.IsNull() && aDSA2.IsNull()) return;
        ...
        if (!aGISU1.IsNull() && aGISU1->NbIdentifiedItem() > 0) {
            ...
            if (aPoint.IsNull()) {
                ...
                if (!anA2P3D.IsNull()) aPoint = anA2P3D->Location();
            }
            if (!aPoint.IsNull()) {
                // set connection point to object
                gp_Pnt aPnt(aPoint->CoordinatesValue(1) * aFact, aPoint->CoordinatesValue(2) * aFact,
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean ReadDatums(const Handle(XCAFDoc_ShapeTool) & STool, const Handle(XCAFDoc_DimTolTool) & DGTTool,
                                   const Interface_Graph& graph, const Handle(Transfer_TransientProcess) & TP,
    ...
    for (Standard_Integer idr = 1; idr <= HADR->Length(); idr++) {
        ...
        for (subs4.Start(); subs4.More(); subs4.Next()) {
            ...
            Handle(StepDimTol_DatumFeature) DF = Handle(StepDimTol_DatumFeature)::DownCast(SAR->RelatingShapeAspect());
...
static Standard_Integer FindShapeIndexForDGT(const Handle(Standard_Transient) & theEnt,
    ...
    if (anIndex > 0 || theEnt.IsNull()) return anIndex;
...
// purpose  :
//=======================================================================
static void collectShapeAspect(const Handle(StepRepr_ShapeAspect) & theSA, const Handle(XSControl_WorkSession) & theWS,
                               NCollection_Sequence<Handle(StepRepr_ShapeAspect)>& theSAs) {
    if (theSA.IsNull()) return;
    ...
    if (theSA->IsKind(STANDARD_TYPE(StepRepr_DerivedShapeAspect))) {
        ...
        if (!aSADR.IsNull()) collectShapeAspect(aSADR->RelatedShapeAspect(), theWS, theSAs);
        ...
        if (isSimple) theSAs.Append(theSA);
    }
}
...
static TDF_Label getShapeLabel(const Handle(StepRepr_RepresentationItem) & theItem,
    ...
    if (aShape.IsNull()) return aShapeL;
...
Standard_Boolean STEPCAFControl_Reader::setDatumToXCAF(const Handle(StepDimTol_Datum) & theDat, const TDF_Label theGDTL,
    ...
    for (anIterD.Start(); anIterD.More(); anIterD.Next()) {
        ...
        if (aSAR.IsNull() || aSAR->RelatingShapeAspect().IsNull()) continue;
    ...
    for (Standard_Integer i = 1; i <= aSAs.Length(); i++) {
        ...
        // Skip datum targets
        if (aSA->IsKind(STANDARD_TYPE(StepDimTol_DatumTarget))) continue;
        ...
        for (; anIter.More(); anIter.Next()) {
            ...
            for (Standard_Integer j = 1; j <= aGISU->NbIdentifiedItem(); j++) {
                ...
                if (!aShapeL.IsNull()) aShapeLabels.Append(aShapeL);
            }
        }
    }
    ...
    for (Standard_Integer i = 1; i <= aSAs.Length(); i++) {
        ...
        if (aType == XCAFDimTolObjects_DatumTargetType_Area) {
            // Area datum target
            if (aRelationship.IsNull()) continue;
            Handle(StepRepr_ShapeAspect) aSA = aRelationship->RelatingShapeAspect();
            Interface_EntityIterator aSAIter = aGraph.Sharings(aSA);
            Handle(StepAP242_GeometricItemSpecificUsage) aGISU;
            ...
            if (!aGISU.IsNull() && aGISU->NbIdentifiedItem() > 0) anItem = aGISU->IdentifiedItemValue(1);
                ...
                if (!aSDR.IsNull()) {
                    ...
                    if (!aSRWP.IsNull()) {
                        ...
                        for (Standard_Integer j = aSRWP->Items()->Lower(); j <= aSRWP->Items()->Upper(); j++) {
                            if (aSRWP->ItemsValue(j).IsNull()) continue;
        ...
        if (isValidDT) {
            ...
            if (!theXCAFModifiers.IsEmpty()) aDatTargetObj->SetModifiers(theXCAFModifiers);
    ...
    if (aShapeLabels.Length() > 0 || !isExistDatumTarget) {
        ...
        for (Standard_Integer i = 1; i <= aSAs.Length(); i++) {
            Handle(StepRepr_ShapeAspect) aSA = aSAs.Value(i);
            if (aSA.IsNull() || aSA->IsKind(STANDARD_TYPE(StepDimTol_DatumTarget))) continue;
        ...
        if (!theXCAFModifiers.IsEmpty()) aDatObj->SetModifiers(theXCAFModifiers);
        if (theXCAFModifWithVal != XCAFDimTolObjects_DatumModifWithValue_None)
                    ...
                    for (Standard_Integer i = aDRCA->Lower(); i <= aDRCA->Upper(); i++) {
                        ...
                        for (anIterDRC.Start(); anIterDRC.More(); anIterDRC.Next()) {
                            ...
                            if (anIterDRC.Value()->IsKind(STANDARD_TYPE(StepDimTol_Datum))) {
                                ...
                                setDatumToXCAF(aD, theGDTL, aPositionCounter, aXCAFModifiers, aXCAFModifWithVal,
                                               aModifValue, theDoc, theWS);
...
TDF_Label STEPCAFControl_Reader::createGDTObjectInXCAF(const Handle(Standard_Transient) & theEnt,
    ...
    if (theEnt->IsKind(STANDARD_TYPE(StepShape_DimensionalLocation))) {
        ...
        if (aDim->RelatedShapeAspect().IsNull() || aDim->RelatingShapeAspect().IsNull()) return aGDTL;
        aSemanticName = aDim->Name();
    }
            ...
            for (Standard_Integer i = aSAs.Lower(); i <= aSAs.Upper(); i++) {
                ...
                if (!PropD.IsNull()) // for old version
                {
                    ...
                    if (theEnt->IsKind(STANDARD_TYPE(StepShape_DimensionalSize))) {
                        ...
                        Handle(StepShape_DimensionalSize) DimSize = Handle(StepShape_DimensionalSize)::DownCast(theEnt);
        ...
        if (!aSh.IsNull()) {
            ...
            if (aShL.IsNull() && aSh.ShapeType() == TopAbs_WIRE) {
                ...
                while (ex.More()) {
                    ...
                    if (!edgeL.IsNull()) aShLS1.Append(edgeL);
                    ex.Next();
                }
            }
            if (!aShL.IsNull()) aShLS1.Append(aShL);
        }
    }
    if (!aSeqRI2.IsEmpty()) {
            ...
            if (!aSh.IsNull()) {
                ...
                if (aShL.IsNull() && aSh.ShapeType() == TopAbs_WIRE) {
                    ...
                    while (ex.More()) {
                        TDF_Label edgeL;
                        aSTool->Search(ex.Current(), edgeL, Standard_True, Standard_True, Standard_True);
                        if (!edgeL.IsNull()) aShLS2.Append(edgeL);
                        ex.Next();
                    }
                }
                if (!aShL.IsNull()) aShLS2.Append(aShL);
            }
        }
    }
...
// purpose  :
//=======================================================================
static void setDimObjectToXCAF(const Handle(Standard_Transient) & theEnt, const TDF_Label& aDimL,
                               const Handle(TDocStd_Document) & theDoc, const Handle(XSControl_WorkSession) & theWS) {
    Handle(XCAFDoc_ShapeTool) aSTool = XCAFDoc_DocumentTool::ShapeTool(theDoc->Main());
    Handle(XCAFDoc_DimTolTool) aDGTTool = XCAFDoc_DocumentTool::DimTolTool(theDoc->Main());
    const Handle(XSControl_TransferReader) & aTR = theWS->TransferReader();
    const Handle(Transfer_TransientProcess) & aTP = aTR->TransientProcess();
    ...
    Handle(StepShape_DimensionalLocation) aDimLocation = Handle(StepShape_DimensionalLocation)::DownCast(theEnt);
    ...
    for (anIterDim.Start(); anIterDim.More(); anIterDim.Next()) {
        ...
        Handle(StepShape_PlusMinusTolerance) aPMT = Handle(StepShape_PlusMinusTolerance)::DownCast(anIterDim.Value());
        if (!aDCR.IsNull()) {
            ...
            //! 2 -> LimitsAndFits from StepShape
            {
                ...
                if (aUpperBound.IsNull()) continue;
                ...
                if (aLowerBound.IsNull()) continue;
    ...
    if (!aCRI.IsNull() && !aCRI->ItemElement().IsNull() && aCRI->ItemElement()->Length() > 0) {
        ...
        if (aModifiers.Length() > 0) aDimObj->SetModifiers(aModifiers);
    }
    ...
    if (!STEPCAFControl_GDTProperty::GetDimType(aName, aType)) {
        if (!aDimSize.IsNull()) {
            ...
            Handle(StepShape_AngularLocation) anAngLoc = Handle(StepShape_AngularLocation)::DownCast(aDimLocation);
            if (!anAngLoc.IsNull()) {
                // get qualifier for angular value
...
static Standard_Boolean getTolType(const Handle(Standard_Transient) & theEnt,
    ...
    if (theEnt.IsNull() || !theEnt->IsKind(STANDARD_TYPE(StepDimTol_GeometricTolerance))) return Standard_False;
    ...
    if (theEnt->IsKind(STANDARD_TYPE(StepDimTol_GeoTolAndGeoTolWthDatRef))) {
        ...
        Handle(StepDimTol_GeoTolAndGeoTolWthMaxTol) anE = Handle(StepDimTol_GeoTolAndGeoTolWthMaxTol)::DownCast(theEnt);
        theType = STEPCAFControl_GDTProperty::GetGeomToleranceType(anE->GetToleranceType());
    } else if (theEnt->IsKind(STANDARD_TYPE(StepDimTol_GeoTolAndGeoTolWthMod))) {
        Handle(StepDimTol_GeoTolAndGeoTolWthMod) anE = Handle(StepDimTol_GeoTolAndGeoTolWthMod)::DownCast(theEnt);
        theType = STEPCAFControl_GDTProperty::GetGeomToleranceType(anE->GetToleranceType());
...
// purpose  :
//=======================================================================
static void setGeomTolObjectToXCAF(const Handle(Standard_Transient) & theEnt, const TDF_Label& theTolL,
                                   const Handle(TDocStd_Document) & theDoc,
                                   const Handle(XSControl_WorkSession) & theWS) {
    Handle(XCAFDoc_ShapeTool) aSTool = XCAFDoc_DocumentTool::ShapeTool(theDoc->Main());
    Handle(XCAFDoc_DimTolTool) aDGTTool = XCAFDoc_DocumentTool::DimTolTool(theDoc->Main());
...
Standard_Boolean STEPCAFControl_Reader::ReadGDTs(const Handle(XSControl_WorkSession) & theWS,
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (anEnt->IsKind(STANDARD_TYPE(StepShape_DimensionalSize)) ||
            ...
            if (!aDC.IsNull()) continue;
            // Read presentations for PMIs without semantic data.
            Handle(StepAP242_DraughtingModelItemAssociation) aDMIA;
            TDF_LabelSequence aShapesL;
            for (Interface_EntityIterator anIter = aGraph.Sharings(anEnt); anIter.More() && aDMIA.IsNull();
                 anIter.Next()) {
            ...
            if (!aDMIA.IsNull()) {
                ...
                if (!aDefinition.IsNull()) {
                    ...
                    for (Interface_EntityIterator anIter = aGraph.Sharings(aDefinition);
                         anIter.More() && !isConnectedToGDT; anIter.Next()) {
                        ...
                        if (!aSAR.IsNull()) {
                            ...
                            if (!aSA.IsNull()) {
                                ...
                                for (Interface_EntityIterator aDimIter = aGraph.Sharings(aSA);
                                     aDimIter.More() && !isConnectedToGDT; aDimIter.Next()) {
                ...
                for (Standard_Integer aSAIt = 1; aSAIt <= aSAs.Length(); aSAIt++) {
                    ...
                    for (Interface_EntityIterator anIter = aGraph.Sharings(aSAs.Value(aSAIt));
                         anIter.More() && aGISU.IsNull(); anIter.Next())
                    ...
                    for (Standard_Integer anItemIt = 1; anItemIt <= aGISU->NbIdentifiedItem(); anItemIt++) {
                        TDF_Label aLabel = getShapeLabel(aGISU->IdentifiedItemValue(anItemIt), theWS,
                                                         XCAFDoc_DocumentTool::ShapeTool(theDoc->Main()));
                        if (!aLabel.IsNull()) aShapesL.Append(aLabel);
                    }
                }
            }
            ...
            Handle(TCollection_HAsciiString) aPresentName;
            Bnd_Box aBox;
            if (!readPMIPresentation(anEnt, aTR, aFact, aPresentation, aPresentName, aBox)) continue;
            // Annotation plane
            Handle(StepVisual_AnnotationPlane) anAnPlane;
            for (Interface_EntityIterator anIter = aGraph.Sharings(anEnt); anIter.More() && anAnPlane.IsNull();
                 anIter.Next())
            ...
            gp_Ax2 aPlaneAxes;
            if (!anAnPlane.IsNull()) {
                if (readAnnotationPlane(anAnPlane, aPlaneAxes)) aDimObj->SetPlane(aPlaneAxes);
            }
...
static Handle(StepShape_SolidModel)
    FindSolidForPDS(const Handle(StepRepr_ProductDefinitionShape) & PDS, const Interface_Graph& graph) {
...
Standard_Boolean STEPCAFControl_Reader::ReadMaterials(const Handle(XSControl_WorkSession) & WS,
    ...
    for (Standard_Integer i = 1; i <= SeqPDS->Length(); i++) {
        ...
        for (subs.Start(); subs.More(); subs.Next()) {
            Handle(StepRepr_PropertyDefinition) PropD = Handle(StepRepr_PropertyDefinition)::DownCast(subs.Value());
            ...
            for (subs1.Start(); subs1.More(); subs1.Next()) {
                ...
                for (ir = 1; ir <= Repr->NbItems(); ir++) {
                    ...
                    if (RI->IsKind(STANDARD_TYPE(StepRepr_DescriptiveRepresentationItem))) {
                        ...
                        if (aName.IsNull()) aName = aDescription;
                    }
                        ...
                        if (!aUnit.IsNull()) {
                            ...
                            for (Standard_Integer idu = 1; idu <= DU->NbElements(); idu++) {
                                ...
                                if (NU->IsKind(STANDARD_TYPE(StepBasic_ConversionBasedUnitAndLengthUnit)) ||
                                    ...
                                    STEPConstruct_UnitContext anUnitCtx;
                                    anUnitCtx.ComputeFactors(NU);
                                    aDensity = aDensity / (anUnitCtx.LengthFactor() * anUnitCtx.LengthFactor() *
        ...
        if (aName.IsNull() || aName->Length() == 0) continue;
        ...
        if (!SM.IsNull()) {
            ...
            if (index > 0) {
                Handle(Transfer_Binder) binder = TP->MapItem(index);
                if (!binder.IsNull()) aSh = TransferBRep::ShapeResult(binder);
            }
        }
        if (aSh.IsNull()) continue;
        TDF_Label shL;
...
void collectViewShapes(const Handle(XSControl_WorkSession) & theWS, const Handle(TDocStd_Document) & theDoc,
                       const Handle(StepRepr_Representation) theRepr, TDF_LabelSequence& theShapes) {
    Handle(XSControl_TransferReader) aTR = theWS->TransferReader();
    Handle(Transfer_TransientProcess) aTP = aTR->TransientProcess();
    const Interface_Graph& aGraph = aTP->Graph();
    Handle(XCAFDoc_ShapeTool) aSTool = XCAFDoc_DocumentTool::ShapeTool(theDoc->Main());
    ...
    if (!aSh.IsNull()) {
        ...
        if (!aShL.IsNull()) theShapes.Append(aShL);
    }
    Interface_EntityIterator anIter = aGraph.Sharings(theRepr);
    for (; anIter.More(); anIter.Next()) {
        if (!anIter.Value()->IsKind(STANDARD_TYPE(StepRepr_RepresentationRelationship))) continue;
...
Handle(TCollection_HAsciiString)
    buildClippingPlanes(const Handle(StepGeom_GeometricRepresentationItem) & theClippingCameraModel,
                        TDF_LabelSequence& theClippingPlanes, const Handle(XCAFDoc_ClippingPlaneTool) theTool) {
    ...
    if (theClippingCameraModel->IsKind(STANDARD_TYPE(StepVisual_CameraModelD3MultiClipping))) {
        ...
        if (aCameraModel->ShapeClipping().IsNull()) return anExpression;
        ...
        if (aCameraModel->ShapeClipping()->Length() == 1) {
            ...
            if (!aCameraModelUnion.IsNull()) return buildClippingPlanes(aCameraModelUnion, theClippingPlanes, theTool);
        }
...
Standard_Boolean STEPCAFControl_Reader::ReadViews(const Handle(XSControl_WorkSession) & theWS,
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        Handle(Standard_Transient) anEnt = aModel->Value(i);
        if (!anEnt->IsKind(STANDARD_TYPE(StepVisual_CameraModelD3))) continue;
        ...
        if (aViewVolume->FrontPlaneClipping()) anObj->SetFrontPlaneDistance(aViewVolume->FrontPlaneDistance());
        if (aViewVolume->BackPlaneClipping()) anObj->SetBackPlaneDistance(aViewVolume->BackPlaneDistance());
        ...
        if (aDModel.IsNull()) return Standard_False;
        ...
        for (; anIter.More(); anIter.Next()) {
            if (anIter.Value()->IsKind(STANDARD_TYPE(StepRepr_MappedItem))) {
                ...
                for (; aDMIAIter.More(); aDMIAIter.Next()) {
                    if (!aDMIAIter.Value()->IsKind(STANDARD_TYPE(StepAP242_DraughtingModelItemAssociation))) continue;
                    Handle(StepAP242_DraughtingModelItemAssociation) aDMIA =
...
TDF_Label STEPCAFControl_Reader::SettleShapeData(const Handle(StepRepr_RepresentationItem) & theItem,
    ...
    if (hName.IsNull() || hName->IsEmpty()) return aResult;

    Handle(Transfer_Binder) aBinder = TP->Find(theItem);
    if (aBinder.IsNull()) return aResult;

    TopoDS_Shape aShape = TransferBRep::ShapeResult(aBinder);
    if (aShape.IsNull()) return aResult;
...
void collectRepresentationItems(const Interface_Graph& theGraph,
    ...
    for (entIt.Start(); entIt.More(); entIt.Next()) {
        ...
        if (aRelationship->Rep1() == theRepresentation) {
            ...
            if (!aRepr.IsNull()) collectRepresentationItems(theGraph, aRepr, theItems);
        }
    }
}
...
void STEPCAFControl_Reader::ExpandSubShapes(const Handle(XCAFDoc_ShapeTool) & ShapeTool,
    ...
    for (STEPCAFControl_DataMapIteratorOfDataMapOfShapePD it(ShapePDMap); it.More(); it.Next()) {
        ...
        if (aReprItems.Length() == 0) continue;

        if (!myMap.IsBound(aRootShape)) continue;
        ...
        if (ShapeTool->IsAssembly(aRootLab)) continue;
        ...
        for (Standard_Integer i = 1; i <= aMSBSeq.Length(); ++i) {
            ...
            if (doInsertSolidLab) SettleShapeData(aMSBSeq.Value(i), aRootLab, ShapeTool, TP);
...
void STEPCAFControl_Reader::ExpandSBSM(TDF_Label& ShapeLab, const Handle(StepRepr_RepresentationItem) & Repr,
                                       const Handle(Transfer_TransientProcess) & TP,
                                       const Handle(XCAFDoc_ShapeTool) & ShapeTool) const {
...
void STEPCAFControl_Reader::ExpandShell(const Handle(StepShape_ConnectedFaceSet) & Shell, TDF_Label& RootLab,
                                        const Handle(Transfer_TransientProcess) & TP,
                                        const Handle(XCAFDoc_ShapeTool) & ShapeTool) const {
    // Record CAF data
    ...
    for (Standard_Integer f = aFaces->Lower(); f <= aFaces->Upper(); ++f) {
        ...
        for (Standard_Integer w = aWires->Lower(); w <= aWires->Upper(); ++w) {
            ...
            if (!aWire->IsInstance(STANDARD_TYPE(StepShape_EdgeLoop))) continue;
            ...
            for (Standard_Integer e = anEdges->Lower(); e <= anEdges->Upper(); ++e) {
                ...
                if (anOrientedEdge.IsNull()) continue;
