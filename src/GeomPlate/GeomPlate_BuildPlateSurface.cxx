...
GeomPlate_BuildPlateSurface::GeomPlate_BuildPlateSurface(const Handle(TColStd_HArray1OfInteger) & NPoints,
                                                         ...
                                                         const Standard_Integer Degree, const Standard_Integer NbIter,
                                                         const Standard_Real Tol2d, const Standard_Real Tol3d,
                                                         const Standard_Real TolAng, const Standard_Real,
                                                         const Standard_Boolean Anisotropie)
    : myAnisotropie(Anisotropie), myDegree(Degree), myNbIter(NbIter), myProj(), myTol2d(Tol2d), myTol3d(Tol3d),
      myTolAng(TolAng), myNbBounds(0) {
    ...
    if (myNbIter < 1) throw Standard_ConstructionError("GeomPlate :  Number of iteration must be >= 1");

    if (NTCurve == 0) throw Standard_ConstructionError("GeomPlate : the bounds Array is null");
    if (Tang->Length() == 0) throw Standard_ConstructionError("GeomPlate : the constraints Array is null");
    ...
    if (myDegree < 2) throw Standard_ConstructionError("GeomPlate ; the degree resolution must be upper of 2");
...
GeomPlate_BuildPlateSurface::GeomPlate_BuildPlateSurface(
    const Handle(Geom_Surface) & Surf, const Standard_Integer Degree, const Standard_Integer NbPtsOnCur,
    const Standard_Integer NbIter, const Standard_Real Tol2d, const Standard_Real Tol3d, const Standard_Real TolAng,
    const Standard_Real /*TolCurv*/, const Standard_Boolean Anisotropie)
    : mySurfInit(Surf), myAnisotropie(Anisotropie), myDegree(Degree), myNbPtsOnCur(NbPtsOnCur), myNbIter(NbIter),
      myProj(), myTol2d(Tol2d), myTol3d(Tol3d), myTolAng(TolAng), myNbBounds(0) {
    if (myNbIter < 1) throw Standard_ConstructionError("GeomPlate :  Number of iteration must be >= 1");
    if (myDegree < 2) throw Standard_ConstructionError("GeomPlate : the degree must be above 2");
    myLinCont = new GeomPlate_HSequenceOfCurveConstraint;
    myPntCont = new GeomPlate_HSequenceOfPointConstraint;
...
GeomPlate_BuildPlateSurface::GeomPlate_BuildPlateSurface(const Standard_Integer Degree,
                                                         ...
                                                         const Standard_Integer NbIter, const Standard_Real Tol2d,
                                                         const Standard_Real Tol3d, const Standard_Real TolAng,
                                                         const Standard_Real /*TolCurv*/,
                                                         const Standard_Boolean Anisotropie)
    : myAnisotropie(Anisotropie), myDegree(Degree), myNbPtsOnCur(NbPtsOnCur), myNbIter(NbIter), myProj(),
      myTol2d(Tol2d), myTol3d(Tol3d), myTolAng(TolAng), myNbBounds(0) {
    if (myNbIter < 1) throw Standard_ConstructionError("GeomPlate :  Number of iteration must be >= 1");
    if (myDegree < 2) throw Standard_ConstructionError("GeomPlate : the degree resolution must be upper of 2");
    myLinCont = new GeomPlate_HSequenceOfCurveConstraint;
    myPntCont = new GeomPlate_HSequenceOfPointConstraint;
...
Handle(Adaptor2d_Curve2d) GeomPlate_BuildPlateSurface::ProjectedCurve(Handle(Adaptor3d_Curve) & Curv) {
    ...
    if (HProjector->NbCurves() != 1) {
        ...
        if (Abs(First1 - First2) <= Max(myTolU, myTolV) && Abs(Last1 - Last2) <= Max(myTolU, myTolV)) {
            HProjector =
                Handle(ProjLib_HCompProjectedCurve)::DownCast(HProjector->Trim(First2, Last2, Precision::PConfusion()));
        } else {
            HProjector.Nullify(); // No continuous solution
#ifdef OCCT_DEBUG
...
void GeomPlate_BuildPlateSurface::Perform(const Message_ProgressRange& theProgress) {
    ...
    if (myNbBounds == 0) myNbBounds = myLinCont->Length();
    ...
    if (!mySurfInitIsGive) {
        ...
        if (NTLinCont >= 2) { // Table of transformations to preserve the initial order, see TrierTab
            myInitOrder = new TColStd_HArray1OfInteger(1, NTLinCont);
            for (Standard_Integer l = 1; l <= NTLinCont; l++)
                myInitOrder->SetValue(l, l);
            if (!CourbeJointive(myTol3d)) { //    throw Standard_Failure("Curves are not joined");
#ifdef OCCT_DEBUG
    ...
    //======================================================================
    if ((NTLinCont != 0) && (myNbPtsOnCur != 0)) CalculNbPtsInit();
    ...
    do {
        ...
        if (Affich && NbBoucle) {
            ...
            if (myG1Error != 0) std::cout << "AngleMax=" << myG1Error << std::endl;
            if (myG2Error != 0) std::cout << "CourbMax=" << myG2Error << std::endl;
        }
#endif
        NbBoucle++;
        if (NTLinCont != 0) { //====================================================================
            ...
            //====================================================================
            LoadCurve(NbBoucle);
            if (myPntCont->Length() != 0) LoadPoint(NbBoucle);
            //====================================================================
...
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::EcartContraintesMil(const Standard_Integer c, Handle(TColStd_HArray1OfReal) & d,
...
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::Disc2dContour(const Standard_Integer /*nbp*/, TColgp_SequenceOfXY& Seq2d) {
#ifdef OCCT_DEBUG
    if (Seq2d.Length() != 4) std::cout << "Number of points should be equal to 4 for Disc2dContour" << std::endl;
#endif
    //  initialization
    Seq2d.Clear();
...
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::Disc3dContour(const Standard_Integer /*nbp*/, const Standard_Integer iordre,
                                                TColgp_SequenceOfXYZ& Seq3d) {
#ifdef OCCT_DEBUG
    if (Seq3d.Length() != 4) std::cout << "nbp should be equal to 4 for Disc3dContour" << std::endl;
    if (iordre != 0 && iordre != 1) std::cout << "incorrect order for Disc3dContour" << std::endl;
#endif
    //  initialization
    Seq3d.Clear();
    //  sampling in "cosine" + 3 points on each interval
...
Standard_Real GeomPlate_BuildPlateSurface::G0Error(const Standard_Integer Index) {
    ...
    for (Standard_Integer i = 1; i <= myNbPtsOnCur; i++)
        if (tdistance->Value(i) > MaxDistance) MaxDistance = tdistance->Value(i);
    return MaxDistance;
}
...
Standard_Real GeomPlate_BuildPlateSurface::G1Error(const Standard_Integer Index) {
    ...
    for (Standard_Integer i = 1; i <= myNbPtsOnCur; i++)
        if (tangle->Value(i) > MaxAngle) MaxAngle = tangle->Value(i);
    return MaxAngle;
}
...
Standard_Real GeomPlate_BuildPlateSurface::G2Error(const Standard_Integer Index) {
    ...
    for (Standard_Integer i = 1; i <= myNbPtsOnCur; i++)
        if (tcurvature->Value(i) > MaxCurvature) MaxCurvature = tcurvature->Value(i);
    return MaxCurvature;
}
    ...
    while (j <= (myNbBounds - 1)) {
        ...
        while (a < 1) {
            if (i > myNbBounds) {
                ...
                if (mySense->Value(j) == 1) Ufinal1 = Uinit1;
...
void GeomPlate_BuildPlateSurface::ComputeSurfInit(const Message_ProgressRange& theProgress) {
    ...
    if (CourbeJoint && IsOrderG1()) {
        ...
        for (i = 1; i <= NTLinCont; i++) {
            ...
            if (i > 1 && Order >= GeomAbs_G1) {
                ...
                Normal = Vec1 ^ Vec2;
                if (LastVec.IsOpposite(Normal, AngTol)) ToReverse = Standard_True;
            }

            for (Standard_Integer j = 0; j <= NbPoint; j++) { // Number of points per curve = 20
                // Linear distribution
    ...
    if (NTLinCont != 0) TrierTab(myInitOrder); // Reorder the table of transformations

    if (nopt != 3) {
        if (NTPntCont != 0)
            nopt = 1;                                       // Calculate by the method of plane of inertia
        else if (!CourbeJoint || NTLinCont != myNbBounds) { //    throw Standard_Failure("Curves are not joined");
#ifdef OCCT_DEBUG
    ...
    if (NTLinCont != 0 && myIsLinear) {
        Handle(Geom_Surface) InitPlane = (Handle(Geom_RectangularTrimmedSurface)::DownCast(mySurfInit))->BasisSurface();
...
#ifdef OCCT_DEBUG
        if (!myIsLinear) std::cout << "Metrics are too different :" << Ratio << std::endl;
#endif
        //      myIsLinear =  Standard_True; // !!
    } // comparing metrics of curves and projected curves
    ...
    if (!myIsLinear) {
        ...
        for (i = 1; i <= NTPntCont; i++) {
            gp_Pnt P;
            myPntCont->Value(i)->D0(P);
            if (!myPntCont->Value(i)->HasPnt2dOnSurf()) myPntCont->ChangeValue(i)->SetPnt2dOnSurf(ProjectPoint(P));
        }
        ...
        //====================================================================
        LoadCurve(0, 0);
        if (myPntCont->Length() != 0) LoadPoint(0, 0);
        //====================================================================
        // Construction of the surface
        //====================================================================
...
#endif
                    if (P1.Distance(P2) < myTol3d) { // 2D intersection corresponds to close 3D points.
...
#ifdef OCCT_DEBUG
                                if (Affich)
                                    std::cout << "Angle between curves " << i << "," << j << " " << Abs(Abs(A1) - M_PI)
                                              << std::endl;
#endif

                                coin = Ci.Resolution(Tol);
                                Standard_Real Par1 = int2d.ParamOnFirst() - coin, Par2 = int2d.ParamOnFirst() + coin;
...
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::Discretise(const Handle(GeomPlate_HArray1OfSequenceOfReal) & PntInter,
    ...
    for (Standard_Integer i = 1; i <= NTLinCont; i++) {
        ...
        if (Affich > 1) {
            ...
            std::cout << "  NbPnt, NbPtInter, NbPtG1G1 :" << NbPnt_i << ", " << NbPtInter << ", " << NbPtG1G1
                      << std::endl;
        }
#endif
                            ...
                            if (NbPtG1G1 != 0) {
                                ...
                                for (Standard_Integer k = 1; k <= NbPtG1G1; k += 2) {
                                    if ((Inter > PntG1G1->Value(i).Value(k)) &&
                                        ...
                                        Pdif.SetCoord(-PP.Coord(1) + P3d.Coord(1), -PP.Coord(2) + P3d.Coord(2),
...
void GeomPlate_BuildPlateSurface::CalculNbPtsInit() {
    ...
    for (i = 1; i <= NTLinCont; i++) {
        ...
        if (myLinCont->Value(i)->NbPoints() < 3) myLinCont->ChangeValue(i)->SetNbPoints(3);
    }
}
//---------------------------------------------------------
// Function : LoadCurve
...
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::LoadCurve(const Standard_Integer NbBoucle, const Standard_Integer OrderMax) {
    gp_Pnt P3d, Pdif, PP;
    gp_Pnt2d P2d;
...
void GeomPlate_BuildPlateSurface::LoadPoint(const Standard_Integer, const Standard_Integer OrderMax) {
    ...
    for (i = 1; i <= NTPntCont; i++) {
        ...
        P2d = myPntCont->Value(i)->Pnt2dOnSurf();
        mySurfInit->D0(P2d.Coord(1), P2d.Coord(2), PP);
        Pdif.SetCoord(-PP.Coord(1) + P3d.Coord(1), -PP.Coord(2) + P3d.Coord(2), -PP.Coord(3) + P3d.Coord(3));
        Plate_PinpointConstraint PC(P2d.XY(), Pdif.XYZ(), 0, 0);
        myPlate.Load(PC);
...
Standard_Boolean GeomPlate_BuildPlateSurface::VerifSurface(const Standard_Integer NbBoucle) {
    ...
    for (Standard_Integer i = 1; i <= NTLinCont; i++) {
        ...
        if (LinCont->Order() != -1) {
            ...
            for (Standard_Integer j = 1; j < NbPts_i; j++) {
                if (tdist->Value(j) > myG0Error) myG0Error = tdist->Value(j);
                if (tang->Value(j) > myG1Error) myG1Error = tang->Value(j);
                if (tcourb->Value(j) > myG2Error) myG2Error = tcourb->Value(j);
                Standard_Real U;
                ...
                if (diffDist > 0) {
                    ...
                    if (diffDist > diffDistMax) diffDistMax = diffDist;
                    // SdiffDist+=diffDist;
                    NdiffDist++;
#ifdef DRAW
                    ...
                    if (diffAng > diffAngMax) diffAngMax = diffAng;
                    // SdiffAng+=diffAng;
                    NdiffAng++;
#ifdef DRAW
                    if ((Affich) && (NbBoucle == myNbIter)) {
            ...
            if (NdiffDist > 0) { // at least one point is not acceptable in G0
                Standard_Real Coef;
                if (LinCont->Order() == 0) Coef = 0.6 * Log(diffDistMax + 7.4);
            ...
            } else if (NdiffAng > 0) // at least 1 point is not acceptable in G1
            {
                ...
                if ((LinCont->NbPoints() + 1) >= Floor(LinCont->NbPoints() * Coef)) Coef = 2;
    ...
    if (!Result) {
        if (myFree && NbBoucle == 1) myPrevPlate = myPlate;
        myPlate.Init();
    }
    return Result;
}
...
// Function : VerifPoint
//---------------------------------------------------------
void GeomPlate_BuildPlateSurface::VerifPoints(Standard_Real& Dist, Standard_Real& Ang, Standard_Real& Curv) const {
    Standard_Integer NTPntCont = myPntCont->Length();
    gp_Pnt Pi, Pf;
    gp_Pnt2d P2d;
    gp_Vec v1i, v1f, v2i, v2f, v3i, v3f;
