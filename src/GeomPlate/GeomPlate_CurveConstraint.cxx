...
GeomPlate_CurveConstraint ::GeomPlate_CurveConstraint()
    : myNbPoints(0), myOrder(0), myTang(0), myConstG0(Standard_False), myConstG1(Standard_False),
      myConstG2(Standard_False), myLProp(2, 1.e-4), myTolDist(0.0), myTolAng(0.0), myTolCurv(0.0), myTolU(0.0),
      myTolV(0.0) {}
...
GeomPlate_CurveConstraint ::GeomPlate_CurveConstraint(const Handle(Adaptor3d_Curve) & Boundary,
                                                      const Standard_Integer Tang, const Standard_Integer NPt,
                                                      const Standard_Real TolDist, const Standard_Real TolAng,
                                                      const Standard_Real TolCurv)
    : myLProp(2, TolDist), myTolDist(TolDist), myTolAng(TolAng), myTolCurv(TolCurv) {
    myOrder = Tang;
    if ((Tang < -1) || (Tang > 2)) throw Standard_Failure("GeomPlate : The continuity is not G0 G1 or G2");
    myNbPoints = NPt;
...
Standard_Real GeomPlate_CurveConstraint ::Length() const {
    ...
    } else { //  GCPnts_AbscissaPoint A(my3dCurve->Curve(),AP.Length(my3dCurve->Curve())/2,my3dCurve->FirstParameter());
             //  Standard_Real toto=A.Parameter();
             // std::cout<<toto<<std::endl;
...
void GeomPlate_CurveConstraint ::D1(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2) const {
    gp_Pnt2d P2d;
    if (!my3dCurve.IsNull()) throw Standard_Failure("GeomPlate_CurveConstraint.cxx : Curve must be on a Surface");
...
// Fonction : D2
//---------------------------------------------------------
void GeomPlate_CurveConstraint ::D2(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3, gp_Vec& V4,
                                    gp_Vec& V5) const {
    gp_Pnt2d P2d;
    if (!my3dCurve.IsNull()) throw Standard_Failure("GeomPlate_CurveConstraint.cxx : Curve must be on a Surface");
...
Handle(Geom2d_Curve) GeomPlate_CurveConstraint ::Curve2dOnSurf() const {
    if (my2dCurve.IsNull() && !myHCurve2d.IsNull()) {
        ...
        Approx_Curve2d appr(myHCurve2d, myHCurve2d->FirstParameter(), myHCurve2d->LastParameter(), myTolU, myTolV,
...
//---------------------------------------------------------
void GeomPlate_CurveConstraint ::SetProjectedCurve(const Handle(Adaptor2d_Curve2d) & Curve, const Standard_Real TolU,
...
GeomLProp_SLProps& GeomPlate_CurveConstraint::LPropSurf(const Standard_Real U) {
    if (myFrontiere.IsNull()) throw Standard_Failure("GeomPlate_CurveConstraint.cxx : Curve must be on a Surface");
