...
GeomPlate_BuildAveragePlane::GeomPlate_BuildAveragePlane(const Handle(TColgp_HArray1OfPnt) & Pts,
                                                         const Standard_Integer NbBoundPoints, const Standard_Real Tol,
                                                         const Standard_Integer POption, const Standard_Integer NOption)
    : myPts(Pts), myTol(Tol), myNbBoundPoints(NbBoundPoints) {
...
GeomPlate_BuildAveragePlane::GeomPlate_BuildAveragePlane(const TColgp_SequenceOfVec& Normals,
    ...
    for (i = 1; i <= myPts->Length(); i++) {
        ...
        if (U > myUmax) myUmax = U;
        if (U < myUmin) myUmin = U;
        if (V > myVmax) myVmax = V;
        if (V < myVmin) myVmin = V;
    }
...
Handle(Geom_Plane) GeomPlate_BuildAveragePlane::Plane() const {
    Standard_NoSuchObject_Raise_if(
        IsLine(), "Cannot use the function 'GeomPlate_BuildAveragePlane::Plane()', the Object is a 'Geom_Line'");
    return myPlane;
}
...
void GeomPlate_BuildAveragePlane::MinMaxBox(Standard_Real& Umin, Standard_Real& Umax, Standard_Real& Vmin,
...
gp_Vec GeomPlate_BuildAveragePlane::DefPlan(const Standard_Integer NOption) {
    ...
    if (NOption == 1) {
        ...
        if (myNbBoundPoints != 0 && myPts->Length() != myNbBoundPoints) {
            ...
            if (theAngle > M_PI / 2) theAngle = M_PI - theAngle;
            if (theAngle > M_PI / 3) OZ = OZ1;
        }
    }
...
Handle(Geom_Line) GeomPlate_BuildAveragePlane::Line() const {
    Standard_NoSuchObject_Raise_if(
        IsPlane(), "Cannot use the function 'GeomPlate_BuildAveragePlane::Line()', the Object is a 'Geom_Plane'");
    return myLine;
}
...
Standard_Boolean GeomPlate_BuildAveragePlane::HalfSpace(const TColgp_SequenceOfVec& NewNormals,
                                                        TColgp_SequenceOfVec& Normals, GeomPlate_SequenceOfAij& Bset,
                                                        const Standard_Real LinTol, const Standard_Real AngTol) {
    ...
    if (Normals.IsEmpty()) {
        ...
        if (Cross.SquareMagnitude() <= SquareTol) return Standard_False;
    ...
    for (; i <= NewNormals.Length(); i++) {
        ...
        for (j = 1; j <= Bset.Length(); j++)
            if ((Scal = Bset(j).Vec * NewNormals(i)) >= -LinTol) B2set.Append(Bset(j));

        Standard_Integer ii = Normals.Length() + 1;
        for (j = 1; j <= ii - 1; j++) {
            if (Normals(j).SquareMagnitude() == 0.) continue;
            // 4
            ...
            if (isNew) B1set.Append(GeomPlate_Aij(ii, j, Cross));
        ...
        for (j = 1; j <= B1set.Length(); j++) {
            ...
            for (k = 1; k <= Normals.Length(); k++) {
                if (Normals(k).SquareMagnitude() == 0.) continue;
                if (B1set(j).Vec * Normals(k) < -LinTol) {
                    isGEnull = Standard_False;
                    break;
                }
            }
            if (isGEnull) B2set.Append(B1set(j));
        }
