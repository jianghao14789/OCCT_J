...
TDataXtd_Presentation::TDataXtd_Presentation()
    : myDriverGUID("00000000-0000-0000-0000-000000000000"), myColor(Quantity_NOC_WHITE), myMaterialIndex(0), myMode(0),
      myTransparency(0.0), myWidth(0.0), myIsDisplayed(Standard_False), myHasOwnColor(Standard_False),
      myHasOwnMaterial(Standard_False), myHasOwnTransparency(Standard_False), myHasOwnWidth(Standard_False),
      myHasOwnMode(Standard_False), myHasOwnSelectionMode(Standard_False) {}

//=======================================================================
// function : Set
// purpose  :
//=======================================================================
Handle(TDataXtd_Presentation) TDataXtd_Presentation::Set(const TDF_Label& theLabel, const Standard_GUID& theDriverId) {
...
void TDataXtd_Presentation::Unset(const TDF_Label& theLabel) {
    Handle(TDataXtd_Presentation) aPresentation;
    if (theLabel.FindAttribute(TDataXtd_Presentation::GetID(), aPresentation)) theLabel.ForgetAttribute(aPresentation);
}
...
Standard_Integer TDataXtd_Presentation::SelectionMode(const Standard_Integer index) const {
    ...
    for (Standard_Integer i = 1; itr.More() && i <= index; itr.Next(), i++) {
        if (i == index) aSelectionMode = itr.Value();
    }
...
void TDataXtd_Presentation::Restore(const Handle(TDF_Attribute) & theAttribute) {
    Handle(TDataXtd_Presentation) aPresentation = Handle(TDataXtd_Presentation)::DownCast(theAttribute);
...
// purpose  :
//=======================================================================
void TDataXtd_Presentation::Paste(const Handle(TDF_Attribute) & theInto, const Handle(TDF_RelocationTable) &) const {
...
Standard_Boolean TDataXtd_Presentation::HasSelectionMode(const Standard_Integer theSelectionMode) const {
    ...
    for (; itr.More(); itr.Next()) {
        if (theSelectionMode == itr.Value()) ret = Standard_True;
    }
    return ret;
}
