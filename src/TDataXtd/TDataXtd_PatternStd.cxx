...
TDataXtd_PatternStd::TDataXtd_PatternStd()
    : mySignature(0), myAxis1Reversed(Standard_False), myAxis2Reversed(Standard_False) {}
...
void TDataXtd_PatternStd::Axis1(const Handle(TNaming_NamedShape) & Axis1) {
    // OCC2932 correction
    if (!myAxis1.IsNull())
        if (myAxis1->Get() == Axis1->Get()) return;
...
void TDataXtd_PatternStd::Axis2(const Handle(TNaming_NamedShape) & Axis2) {
    // OCC2932 correction
    if (!myAxis2.IsNull())
        if (myAxis2->Get() == Axis2->Get()) return;
...
void TDataXtd_PatternStd::Value1(const Handle(TDataStd_Real) & value) {
    // OCC2932 correction
    if (!myValue1.IsNull())
        if (myValue1->Get() == value->Get()) return;
...
void TDataXtd_PatternStd::Value2(const Handle(TDataStd_Real) & value) {
    // OCC2932 correction
    if (!myValue2.IsNull())
        if (myValue2->Get() == value->Get()) return;
...
void TDataXtd_PatternStd::NbInstances1(const Handle(TDataStd_Integer) & NbInstances1) {
    // OCC2932 correction
    if (!myNb1.IsNull())
        if (myNb1->Get() == NbInstances1->Get()) return;
...
void TDataXtd_PatternStd::NbInstances2(const Handle(TDataStd_Integer) & NbInstances2) {
    // OCC2932 correction
    if (!myNb2.IsNull())
        if (myNb2->Get() == NbInstances2->Get()) return;
...
void TDataXtd_PatternStd::Mirror(const Handle(TNaming_NamedShape) & plane) {
    // OCC2932 correction
    if (!myMirror.IsNull()) {
        if (myMirror->Get() == plane->Get()) return;
    }
...
void TDataXtd_PatternStd::Paste(const Handle(TDF_Attribute) & Into, const Handle(TDF_RelocationTable) & RT) const {
