...
TDataXtd_Constraint::TDataXtd_Constraint()
    : myType(TDataXtd_RADIUS), myIsReversed(Standard_False), myIsInverted(Standard_False), myIsVerified(Standard_True) {
}
...
void TDataXtd_Constraint::Set(const TDataXtd_ConstraintEnum type, const Handle(TNaming_NamedShape) & G1) {
    // OCC2932 correction
    if (myType == type) {
...
void TDataXtd_Constraint::Set(const TDataXtd_ConstraintEnum type, const Handle(TNaming_NamedShape) & G1,
    ...
    if (myType == type) {
        ...
        if (aShape1.IsNull() == Standard_False && G1.IsNull() == Standard_False && aShape2.IsNull() == Standard_False &&
            G2.IsNull() == Standard_False)
            if (aShape1->Get() == G1->Get() && aShape2->Get() == G2->Get()) return;
    }
...
void TDataXtd_Constraint::Set(const TDataXtd_ConstraintEnum type, const Handle(TNaming_NamedShape) & G1,
                              const Handle(TNaming_NamedShape) & G2, const Handle(TNaming_NamedShape) & G3) {
    // OCC2932 correction
    if (myType == type) {
        Handle(TNaming_NamedShape) aShape1 = Handle(TNaming_NamedShape)::DownCast(myGeometries[0]);
        Handle(TNaming_NamedShape) aShape2 = Handle(TNaming_NamedShape)::DownCast(myGeometries[1]);
        ...
        if (aShape1.IsNull() == Standard_False && G1.IsNull() == Standard_False && aShape2.IsNull() == Standard_False &&
            G2.IsNull() == Standard_False && aShape3.IsNull() == Standard_False && G3.IsNull() == Standard_False)
            if (aShape1->Get() == G1->Get() && aShape2->Get() == G2->Get() && aShape3->Get() == G3->Get()) return;
    }
...
void TDataXtd_Constraint::SetPlane(const Handle(TNaming_NamedShape) & plane) {
    // OCC2932 correction
    if (!myPlane.IsNull() && !plane.IsNull() && myPlane->Get() == plane->Get()) return;
...
void TDataXtd_Constraint::ClearGeometries() {
    // OCC2932 correction
    if (myGeometries[0].IsNull() && myGeometries[1].IsNull() && myGeometries[2].IsNull() && myGeometries[3].IsNull())
        return;
...
void TDataXtd_Constraint::SetGeometry(const Standard_Integer Index, const Handle(TNaming_NamedShape) & G) {
    // OCC2932 correction
...
void TDataXtd_Constraint::Paste(const Handle(TDF_Attribute) & into, const Handle(TDF_RelocationTable) & RT) const {
