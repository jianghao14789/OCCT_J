...
static Standard_Boolean IsOnRestriction(const TopoDS_Vertex& V, const TopoDS_Edge& CurE, const TopoDS_Face& F,
                                        TopoDS_Edge& E) {
...
static void Add(const TopoDS_Edge& E, TopTools_IndexedMapOfShape& Map, const TopoDS_Shape& S,
                const BRepOffset_Offset& OF, const BRepOffset_Analyse& Analyse,
...
static void KPartCurve3d(TopoDS_Edge Edge, Handle(Geom2d_Curve) Curve, Handle(Geom_Surface) Surf) {
...
static void Touched(const BRepOffset_Analyse&, const TopTools_MapOfShape&, const TopoDS_Shape&, TopTools_MapOfShape&) {
...
static TopoDS_Vertex FindVertex(const gp_Pnt& P, const TopTools_MapOfShape& Map, const Standard_Real Tol) {
    BRep_Builder B;
...
static TopAbs_Orientation Orientation(const TopoDS_Edge& E, const TopoDS_Face& F, const TopTools_ListOfShape& L) {
    ...
    if (F.Orientation() == TopAbs_REVERSED) Orien = TopAbs::Reverse(Orien);
...
static TopoDS_Edge FindCreatedEdge(const TopoDS_Vertex& V1, const TopoDS_Edge& E,
                                   const BRepOffset_DataMapOfShapeOffset& MapSF, TopTools_MapOfShape& MapOnV,
                                   const BRepOffset_Analyse& CenterAnalyse, Standard_Real Radius, Standard_Real Tol) {
    TopoDS_Edge E1;
...
static void CutEdge(const TopoDS_Edge& E, const TopTools_ListOfShape& VOnE, TopTools_ListOfShape& NE) {
...
BiTgte_Blend::BiTgte_Blend(const TopoDS_Shape& S, const Standard_Real Radius, const Standard_Real Tol,
...
void BiTgte_Blend::Init(const TopoDS_Shape& S, const Standard_Real Radius, const Standard_Real Tol,
                        const Standard_Boolean NUBS) {
    Clear();
    myShape = S;
    myTol = Tol;
...
void BiTgte_Blend::IndicesOfBranche(const Standard_Integer Index, Standard_Integer& From, Standard_Integer& To) const {
...
void BiTgte_Blend::ComputeCenters() {
                ...
                for (; itlet.More(); itlet.Next()) {
                    ...
                    if (!EdgeTgt.IsBound(Cur)) {
                        ...
                        if (!EdgeTgt.IsBound(V1)) {
                            ...
                            if (LE.Extent() == LA.Extent()) EdgeTgt.Bind(V1, OV1);
                        }
                        if (!EdgeTgt.IsBound(V2)) {
                            LE.Clear();
                            myAnalyse.Edges(V2, ChFiDS_Tangential, LE);
                            const TopTools_ListOfShape& LA = myAnalyse.Ancestors(V2);
                            if (LE.Extent() == LA.Extent()) EdgeTgt.Bind(V2, OV2);
                        }
                    }
                }
                // end of map created tangent

                if (OF1.Status() == BRepOffset_Reversed || OF1.Status() == BRepOffset_Degenerated) continue;
    ...
    for (i = 1; i <= myFaces.Extent(); i++) {
        ...
        if (CurS.ShapeType() == TopAbs_FACE) {
            ...
            BRepOffset_Inter2d::Compute(myAsDes, CurOF, myEdges, myTol, anEmptyMap, aDMVV, Message_ProgressRange());
        }
    }
    ...
    for (; itLOF.More(); itLOF.Next()) {
        ...
        if (!myImageOffset.HasImage(CurLOF)) continue;
    ...
    for (i = 1; i <= myEdges.Extent(); i++) {
        ...
        for (; itl.More(); itl.Next()) {
            ...
            if (OF1isEdge) {
                ...
                if (VfOnE1.IsNull()) VfOnE1 = FindVertex(P1, MapOnV1l, myTol);
                VlOnE1 = FindVertex(P2, MapOnV1l, myTol);
                if (VlOnE1.IsNull()) VlOnE1 = FindVertex(P2, MapOnV1f, myTol);
            ...
            if (OF2isEdge) {
                ...
                if (VfOnE2.IsNull()) VfOnE2 = FindVertex(P1, MapOnV1l, myTol);
                VlOnE2 = FindVertex(P2, MapOnV1l, myTol);
                if (VlOnE2.IsNull()) VlOnE2 = FindVertex(P2, MapOnV1f, myTol);
                if (P1.SquareDistance(P2) < myTol * myTol) {
...
Standard_Boolean BiTgte_Blend::Intersect(const TopoDS_Shape& Init, const TopoDS_Face& Face,
                                         const TopTools_DataMapOfShapeBox& MapSBox, const BRepOffset_Offset& OF1,
    ...
    Standard_Boolean F1surBordLibre = InitShape1.ShapeType() == TopAbs_EDGE && myStopFaces.Contains(InitShape1);
    ...
    for (; It.More(); It.Next()) {
        ...
        Standard_Boolean F2surBordLibre = InitShape2.ShapeType() == TopAbs_EDGE && myStopFaces.Contains(InitShape2);
        ...
        if (Init.ShapeType() == TopAbs_EDGE) {
            if (ItKey.ShapeType() == TopAbs_FACE && IsInFace(TopoDS::Edge(Init), TopoDS::Face(ItKey))) continue;
        }
