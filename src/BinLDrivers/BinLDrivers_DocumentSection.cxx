...
BinLDrivers_DocumentSection::BinLDrivers_DocumentSection() : myIsPostRead(Standard_False) {
    myValue[0] = 0;
    myValue[1] = 0;
}
...
BinLDrivers_DocumentSection::BinLDrivers_DocumentSection(const TCollection_AsciiString& theName,
    ...
    : myName(theName), myIsPostRead(isPostRead) {
    myValue[0] = 0;
    myValue[1] = 0;
}
...
void BinLDrivers_DocumentSection::WriteTOC(Standard_OStream& theStream,
    ...
    if (myName.IsEmpty() == Standard_False) {
        ...
        strncpy(&aBuf[sizeof(Standard_Integer)], myName.ToCString(), sizeof(aBuf) - sizeof(Standard_Integer) - 1);
        ...
        if (aBufSize < aLen) aBufSize += sizeof(Standard_Integer);
...
void BinLDrivers_DocumentSection::Write(Standard_OStream& theStream, const uint64_t theOffset,
                                        const TDocStd_FormatVersion theDocFormatVersion) {
...
void BinLDrivers_DocumentSection::ReadTOC(BinLDrivers_DocumentSection& theSection, Standard_IStream& theStream,
                                          const TDocStd_FormatVersion theDocFormatVersion) {
    char aBuf[512];
