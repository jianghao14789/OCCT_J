...
void BinLDrivers_DocumentStorageDriver::Write(const Handle(CDM_Document) & theDoc, Standard_OStream& theOStream,
    ...
    if (aDoc.IsNull()) {
        ...
        if (myDrivers.IsNull()) myDrivers = AttributeDrivers(myMsgDriver);
        Handle(TDF_Data) aData = aDoc->GetData();
        FirstPass(aData->Root());
        if (aDoc->EmptyLabelsSavingMode()) myEmptyLabels.Clear(); //
        ...
        myRelocTable.Clear();
        myPAtt.Init();
        if (aQuickPart) myPAtt.SetOStream(theOStream); // for writing shapes data into the stream directly
        ...
        if (aQuickPart) WriteSizes(theOStream);
...
#ifdef OCCT_DEBUG
    TCollection_ExtendedString aMsg("BinLDrivers_DocumentStorageDriver: warning: attribute driver for type ");
#endif
...
void BinLDrivers_DocumentStorageDriver::WriteSubTree(const TDF_Label& theLabel, Standard_OStream& theOS,
    ...
    if (theQuickPart) aPosition->StoreSize(theOS);
}
...
Handle(BinMDF_ADriverTable) BinLDrivers_DocumentStorageDriver::AttributeDrivers(const Handle(Message_Messenger) &
                                                                                theMessageDriver) {
...
Standard_Boolean BinLDrivers_DocumentStorageDriver::FirstPassSubTree(const TDF_Label& L, TDF_LabelList& ListOfEmptyL) {
    ...
    if (!isEmpty) ListOfEmptyL.Append(emptyChildrenList);
...
void BinLDrivers_DocumentStorageDriver::FirstPass(const TDF_Label& theRoot) {
    ...
    if (FirstPassSubTree(theRoot, myEmptyLabels)) myEmptyLabels.Append(theRoot);
...
void BinLDrivers_DocumentStorageDriver::WriteInfoSection(const Handle(CDM_Document) & theDoc,
    ...
    aHeader.einfo += FSD_BinaryFile::WriteInfo(theOStream, aObjNb, aDocVer, Storage_Schema::ICreationDate(),
                                               "", // schema name
                                               aShemaVer, theData->ApplicationName(), theData->ApplicationVersion(),
                                               theData->DataType(), theData->UserInfo(),
    ...
    // write info section
    FSD_BinaryFile::WriteInfo(theOStream, aObjNb, aDocVer, Storage_Schema::ICreationDate(),
                              "", // schema name
                              aShemaVer, theData->ApplicationName(), theData->ApplicationVersion(), theData->DataType(),
                              theData->UserInfo());
