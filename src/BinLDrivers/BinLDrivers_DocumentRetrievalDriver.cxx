...
void BinLDrivers_DocumentRetrievalDriver::Read(Standard_IStream& theIStream,
    ...
    for (i = 1; i <= aTypeNames.Length(); i++)
        if (myDrivers->GetDriver(i).IsNull()) myMapUnsupported.Add(i);
    if (!myMapUnsupported.IsEmpty()) {
        myMsgDriver->Send(aMethStr + "warning: "
                                     "the following attributes have no driver:",
                          Message_Warning);
        for (i = 1; i <= aTypeNames.Length(); i++)
            if (myMapUnsupported.Contains(i)) myMsgDriver->Send(aTypeNames(i), Message_Warning);
    }
    ...
    if (aQuickPart) myPAtt.SetIStream(theIStream); // for reading shapes data from the stream directly
    EnableQuickPartReading(myMsgDriver, aQuickPart);

    // read sub-tree of the root label
    if (!theFilter.IsNull()) theFilter->StartIteration();
...
Standard_Integer BinLDrivers_DocumentRetrievalDriver::ReadSubTree(Standard_IStream& theIS, const TDF_Label& theLabel,
                                                                  const Handle(PCDM_ReaderFilter) & theFilter,
    ...
    if (!theFilter.IsNull() && theFilter->IsPartTree()) aSkipAttrs = !theFilter->IsPassed();
    ...
    if (theQuickPart) {
        ...
        if (aSkipAttrs && !theFilter->IsSubPassed()) {
            ...
            if (!theFilter.IsNull()) theFilter->Up();
            return 0;
        }
    }
...
Handle(BinMDF_ADriverTable) BinLDrivers_DocumentRetrievalDriver::AttributeDrivers(const Handle(Message_Messenger) &
                                                                                  theMessageDriver) {
...
void BinLDrivers_DocumentRetrievalDriver::ReadShapeSection(BinLDrivers_DocumentSection& theSection,
                                                           Standard_IStream& /*theIS*/, const Standard_Boolean isMess,
                                                           const Message_ProgressRange& /*theRange*/)
...
// purpose  :
//=======================================================================
Standard_Boolean BinLDrivers_DocumentRetrievalDriver::CheckDocumentVersion(const Standard_Integer theFileVersion,
