...
IntPolyh_StartPoint::IntPolyh_StartPoint()
    : x(0), y(0), z(0), u1(0), v1(0), u2(0), v2(0), lambda1(-1.0), lambda2(-1.0), angle(-2.0), t1(-1), e1(-2), t2(-1),
      e2(-2), chainlist(-1) {}

IntPolyh_StartPoint::IntPolyh_StartPoint(const Standard_Real _x, const Standard_Real _y, const Standard_Real _z,
                                         const Standard_Real _u1, const Standard_Real _v1, const Standard_Real _u2,
                                         const Standard_Real _v2, const Standard_Integer _t1,
                                         const Standard_Integer _e1, const Standard_Real _lambda1,
                                         const Standard_Integer _t2, const Standard_Integer _e2,
                                         const Standard_Real _lambda2, const Standard_Integer _chainlist)
    : angle(-2.0) {
    x = _x;
    y = _y;
    z = _z;
...
Standard_Integer IntPolyh_StartPoint::GetEdgePoints(const IntPolyh_Triangle& Triangle, Standard_Integer& FirstEdgePoint,
...
void IntPolyh_StartPoint::SetXYZ(const Standard_Real XX, const Standard_Real YY, const Standard_Real ZZ) {
    x = XX;
    y = YY;
    z = ZZ;
}
...
Standard_Integer IntPolyh_StartPoint::CheckSameSP(const IntPolyh_StartPoint& SP) const {
    ...
    if (((e1 >= -1) && (e1 == SP.e1)) || ((e2 >= -1) && (e2 == SP.e2))) {
        ...
        if (((lambda1 > -MyConfusionPrecision) &&
             (Abs(lambda1 - SP.lambda1) < MyConfusionPrecision)) // lambda1!=-1 && lambda1==SP.lambda2
    ...
    if ((Test == 0) && ((e1 == -1) || (e2 == -1))) {
        /// monSP est un sommet
        if ((Abs(SP.u1 - u1) < MyConfusionPrecision) && (Abs(SP.v1 - v1) < MyConfusionPrecision)) Test = 1;
    } else if ((e1 == -2) && (e2 == -2)) {
        Dump(00200);
        SP.Dump(00201);
