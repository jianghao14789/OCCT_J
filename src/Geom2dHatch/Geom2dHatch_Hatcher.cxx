...
Geom2dHatch_Hatcher::Geom2dHatch_Hatcher(const Geom2dHatch_Intersector& Intersector, const Standard_Real Confusion2d,
                                         const Standard_Real Confusion3d, const Standard_Boolean KeepPnt,
                                         const Standard_Boolean KeepSeg)
    : myIntersector(Intersector), myConfusion2d(Confusion2d), myConfusion3d(Confusion3d), myKeepPoints(KeepPnt),
      myKeepSegments(KeepSeg), myNbElements(0), myNbHatchings(0) {}
...
void Geom2dHatch_Hatcher::Trim() {
    for (Standard_Integer IndH = 1; IndH <= myNbHatchings; IndH++)
        if (myHatchings.IsBound(IndH)) Trim(IndH);
}
...
static void IntersectionPointDump(const IntRes2d_IntersectionPoint& Pnt, const Standard_Integer Index) {
    ...
    for (Standard_Integer ISeg = 1; ISeg <= myIntersector.NbSegments(); ISeg++) {
        ...
        if (FirstPoint && LastPoint) {
            ...
            if (Conf2d || Conf3d) {
                ...
                PntE.SetIntersectionType((PntE.Position() == TopAbs_INTERNAL) ? HatchGen_TRUE : HatchGen_TOUCH);
                ...
                std::cout << "THESE TWO POINTS ARE " << (Conf2d ? "2D" : "3D") << " CONFUSED INTO THE FOLLOWING"
                          << std::endl;
                PntH.Dump();
#endif
                continue;
...
Standard_Boolean Geom2dHatch_Hatcher::GlobalTransition(HatchGen_PointOnHatching& Point) {
    ...
    printf("\n       P:%+10.5g  Tg2d:%+10.5g , %+10.5g  N2d:%+10.5g , %+10.5g  Crv:%+10.5g\n\n", Point.Parameter(),
           Tangente.X(), Tangente.Y(), Normale.X(), Normale.Y(), Courbure);
#endif
...
//--
        ...
        ComplexTransition.Compare(Precision::Angular(), Tangente, Normale, Courbure, LocalTransition,
                                  TangenteOrientation);
    }
...
const HatchGen_Domain& Geom2dHatch_Hatcher::Domain(const Standard_Integer IndH, const Standard_Integer IDom) const {
#if RAISE_IF_NOSUCHOBJECT
    Standard_NoSuchObject_Raise_if(!myHatchings.IsBound(IndH), "");
#endif
...
void Geom2dHatch_Hatcher::Dump() const {
    ...
    std::cout << "The points   are " << (myKeepPoints ? "    " : "not ") << "considered." << std::endl;
    std::cout << "The segments are " << (myKeepSegments ? "    " : "not ") << "considered." << std::endl;
