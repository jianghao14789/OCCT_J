...
void RWStepBasic_RWPerson::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
                                    Handle(Interface_Check) & ach, const Handle(StepBasic_Person) & ent) const {
    ...
    ent->Init(aId, hasAlastName, aLastName, hasAfirstName, aFirstName, hasAmiddleNames, aMiddleNames, hasAprefixTitles,
              aPrefixTitles, hasAsuffixTitles, aSuffixTitles);
}

void RWStepBasic_RWPerson::WriteStep(StepData_StepWriter& SW, const Handle(StepBasic_Person) & ent) const {
