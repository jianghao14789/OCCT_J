...
void RWStepBasic_RWSiUnit::ReadStep(const Handle(StepData_StepReaderData) & data, const Standard_Integer num,
    ...
    if (data->ParamType(num, 3) == Interface_ParamEnum) {
        ...
        if (!DecodeName(aName, text)) ach->AddFail("Enumeration si_unit_name has not an allowed value");
    } else
...
void RWStepBasic_RWSiUnit::WriteStep(StepData_StepWriter& SW, const Handle(StepBasic_SiUnit) & ent) const {
...
Standard_Boolean RWStepBasic_RWSiUnit::DecodePrefix(StepBasic_SiPrefix& aPrefix, const Standard_CString text) const {
...
Standard_Boolean RWStepBasic_RWSiUnit::DecodeName(StepBasic_SiUnitName& aName, const Standard_CString text) const {
