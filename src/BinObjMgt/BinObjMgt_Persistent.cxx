...
BinObjMgt_Persistent::BinObjMgt_Persistent()
    : myIndex(1), myOffset(BP_HEADSIZE), mySize(BP_HEADSIZE), myIsError(Standard_False), myOStream(NULL),
      myIStream(NULL), myDirectWritingIsEnabled(Standard_False) {
    Init();
}
...
Standard_OStream& BinObjMgt_Persistent::Write(Standard_OStream& theOS, const Standard_Boolean theDirectStream) {
    if (myDirectWritingIsEnabled) { // if direct writing was enabled, everything is already written, just pass this
                                    // stage
        myDirectWritingIsEnabled = Standard_False;
        return theOS;
    }
    ...
    if (theDirectStream) aData[1] = -aData[1];
#if DO_INVERSE
...
Standard_IStream& BinObjMgt_Persistent::Read(Standard_IStream& theIS) {
    ...
    if (theIS && aData[0] > 0) {
        ...
        if (myDirectWritingIsEnabled) aData[1] = -aData[1];
        if (theIS && aData[2] > 0) {
            mySize += aData[2];
            // read remaining data
...
const BinObjMgt_Persistent& BinObjMgt_Persistent::GetLabel(const Handle(TDF_Data) & theDS, TDF_Label& theValue) const {
    theValue.Nullify();
    alignOffset(BP_INTSIZE);
    if (noMoreData(BP_INTSIZE)) return *this;
    BinObjMgt_Persistent* me = (BinObjMgt_Persistent*)this;
...
const BinObjMgt_Persistent& BinObjMgt_Persistent::GetGUID(Standard_GUID& theValue) const {
    alignOffset(BP_INTSIZE);
    if (noMoreData(BP_UUIDSIZE)) return *this;
    BinObjMgt_UUID anUUID;
    ...
    anUUID.Data3 = (unsigned short)InverseExtChar(anUUID.Data3);
#endif
    theValue = Standard_GUID(anUUID.Data1, anUUID.Data2, anUUID.Data3, ((anUUID.Data4[0] << 8) | (anUUID.Data4[1])),
                             anUUID.Data4[2], anUUID.Data4[3], anUUID.Data4[4], anUUID.Data4[5], anUUID.Data4[6],
                             anUUID.Data4[7]);
    return *this;
}
...
void BinObjMgt_Persistent::getArray(const Standard_Address theArray, const Standard_Integer theSize) const {
    char* aPtr = (char*)theArray;
    Standard_Integer aLen = theSize;
    BinObjMgt_Persistent* me = (BinObjMgt_Persistent*)this;
    while (aLen > 0) {
...
void BinObjMgt_Persistent::inverseExtCharData(const Standard_Integer theIndex, const Standard_Integer theOffset,
...
void BinObjMgt_Persistent::inverseIntData(const Standard_Integer theIndex, const Standard_Integer theOffset,
                                          const Standard_Integer theSize) {
    Standard_Integer anIndex = theIndex;
    Standard_Integer anOffset = theOffset;
    Standard_Integer aLen = theSize;
...
void BinObjMgt_Persistent::inverseRealData(const Standard_Integer theIndex, const Standard_Integer theOffset,
                                           const Standard_Integer theSize) {
    Standard_Integer anIndex = theIndex;
    Standard_Integer anOffset = theOffset;
    Standard_Integer aLen = theSize;
    ...
    while (aLen > 0) {
        ...
        if (aLenInPiece % BP_REALSIZE) aPrevPtr = &aWrapUnion.aRealData[aLenInPiece / BP_REALSIZE];
        aLen -= aLenInPiece;
        anOffset += aLenInPiece;
        if (anOffset >= BP_PIECESIZE) {
            anIndex++;
...
void BinObjMgt_Persistent::inverseShortRealData(const Standard_Integer theIndex, const Standard_Integer theOffset,
                                                const Standard_Integer theSize) {
    Standard_Integer anIndex = theIndex;
    Standard_Integer anOffset = theOffset;
    Standard_Integer aLen = theSize;
