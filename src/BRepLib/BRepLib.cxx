                ...
                if (Abs(first - current_first) > Precision::Confusion() ||
                    ...
                    if (has_curve) {
                        GeomLib::SameRange(Tolerance, Curve2dPtr, geometric_representation_ptr->First(),
                                           geometric_representation_ptr->Last(), current_first, current_last,
                                           NewCurve2dPtr);
...
Standard_Boolean BRepLib::BuildCurve3d(const TopoDS_Edge& AnEdge, const Standard_Real Tolerance,
    ...
    if (!P.IsNull()) {
        ...
        if (!BRep_Tool::Degenerated(AnEdge)) {
            ...
            GeomLib::BuildCurve3d(Tolerance, CurveOnSurface, f, l, NewCurvePtr, max_deviation, average_deviation,
                                  Continuity, MaxDegree, evaluateMaxSegment(MaxSegment, CurveOnSurface));
            BRep_Builder B;
            tolerance = BRep_Tool::Tolerance(AnEdge);
            // Patch
            // max_deviation = Max(tolerance, max_deviation) ;
            max_deviation = Max(tolerance, Tolerance);
            if (NewCurvePtr.IsNull()) return Standard_False;
        ...
        if (!geometric_representation_ptr.IsNull() && curve_index != curve_on_surface_index) {
            ...
            if (has_curve || has_closed_curve) {
                ...
                if (BRep_Tool::SameParameter(AnEdge)) {
                    ...
                    GeomLib::EvalMaxDistanceAlongParameter(a_curve_on_surface, geom_reference_curve,
                                                           MinToleranceRequested, parameters_ptr->Array1(),
                                                           max_distance);
                } else {

                    GeomLib::EvalMaxDistanceAlongParameter(a_curve_on_surface, curve_on_surface_reference,
                                                           MinToleranceRequested, parameters_ptr->Array1(),
                                                           max_distance);
                }
    ...
    for (; SHToTolit.More(); SHToTolit.Next()) {
        ...
        switch (aSh.ShapeType()) {
            ...
            case TopAbs_VERTEX: {
                ...
                //
                if (aTV->Locked()) throw TopoDS_LockedShape("BRep_Builder::UpdateVertex");
                //
                if (theVForceUpdate)
                    aTV->Tolerance(aTol);
                else
        ...
        //
        if (!UseOldSh) theReshaper.Replace(aSh, aNsh);
    }
}
    ...
    while (ex.More()) {
        ...
        if (Done.Add(aCE)) {
            ...
            if (aNewTol > 0) {
                ...
                if (!aV1.IsNull()) UpdTolMap(aV1, aNewTol, aShToTol);
                if (!aV2.IsNull()) UpdTolMap(aV2, aNewTol, aShToTol);
            }
        }
        ex.Next();
    }
...
void UpdateVTol(const TopoDS_Vertex theV1, const TopoDS_Vertex& theV2, Standard_Real theTol) {
    BRep_Builder aB;
    if (!theV1.IsNull()) aB.UpdateVertex(theV1, theTol);
    if (!theV2.IsNull()) aB.UpdateVertex(theV2, theTol);
}
...
static void InternalUpdateTolerances(const TopoDS_Shape& theOldShape, const Standard_Boolean IsVerifyTolerance,
    ...
    for (iCur = 1; iCur <= parents.Extent(); iCur++) {
        ...
        if (tol > BRep_Tool::Tolerance(EK)) aShToTol.Bind(EK, tol);
    }
    ...
    for (iCur = 1; iCur <= nbV; iCur++) {
        ...
        if (anUpdTol || toAdd) aShToTol.Bind(V, tol);
    }
...
void BRepLib::UpdateInnerTolerances(const TopoDS_Shape& aShape) {
    ...
    for (Standard_Integer i = 1; i <= EFmap.Extent(); i++) {
        ...
        if (!V1.IsNull()) Pnt1 = BRep_Tool::Pnt(V1);
        if (!V2.IsNull()) Pnt2 = BRep_Tool::Pnt(V2);
        ...
        if (!BRep_Tool::Degenerated(anEdge) && EFmap(i).Extent() > 0) {
            ...
            for (Standard_Integer j = 2; j <= theRep.Length(); j++) {
                for (Standard_Integer k = 0; k <= NbSamples; k++) {
                    ...
                    if (aDist > MaxDist) MaxDist = aDist;
...
public:
    ...
    gp_Vec Derivative() {
        ...
        if (aLen < Precision::Confusion()) return aDeriv;
        anOrtho /= aLen;
        if (myIsReversed) anOrtho.Reverse();
...
GeomAbs_Shape BRepLib::ContinuityOfFaces(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace1,
    ...
    if (!theFace1.IsSame(theFace2) && BRep_Tool::IsClosed(theEdge, theFace1) &&
        ...
        if (anEdgeInFace1.IsNull()) return GeomAbs_C0;
        ...
        if (theFace1.IsSame(theFace2)) anEdgeInFace2.Reverse();
        aCurve2 = BRep_Tool::CurveOnSurface(anEdgeInFace2, theFace2, aFirst, aLast);
    }

    if (aCurve1.IsNull() || aCurve2.IsNull()) return GeomAbs_C0;
    ...
    for (Standard_Integer i = 0; i <= 20 && aCont > GeomAbs_C0; i++) {
        ...
        if (isSmoothSuspect) {
            ...
            if (theFace1.Orientation() == TopAbs_REVERSED) aNormal1.Reverse();
            gp_Dir aNormal2 = aSP2.Normal();
            if (theFace2.Orientation() == TopAbs_REVERSED) aNormal2.Reverse();

            if (aNormal1 * aNormal2 < 0.) return GeomAbs_C0;
        }
        ...
        if (aCont < GeomAbs_G2) continue; // no need further processing, because maximal continuity is less than G2
...
static void EncodeRegularity(const TopoDS_Shape& theShape, const Standard_Real theTolAng, TopTools_MapOfShape& theMap,
    ...
    if (!theMap.Add(aShape)) return; // do not need to process shape twice
    ...
    try {
        ...
        for (Standard_Integer i = 1; i <= M.Extent(); i++) {
            ...
            if (!theEdgesToEncode.IsEmpty()) {
                ...
                if (!theEdgesToEncode.Contains(aPureEdge)) continue;
            }
            ...
            if (found) BRepLib::EncodeRegularity(E, F1, F2, theTolAng);
        }
...
Standard_Boolean BRepLib::EnsureNormalConsistency(const TopoDS_Shape& theShape, const Standard_Real theAngTol,
    ...
    for (Standard_Integer anInd = 1; anInd <= aMapEF.Extent(); anInd++) {
        ...
        if (anEdgList.Extent() != 2) continue;
        ...
        if (aPT1.IsNull() || aPT2.IsNull()) continue;

        if (!aPT1->HasNormals() || !aPT2->HasNormals()) continue;
    ...
    for (; anExpFace.More(); anExpFace.Next()) {
        ...
        for (; anExpEdge.More(); anExpEdge.Next()) {
            ...
            if (BRep_Tool::Degenerated(aEdge)) {
                ...
                for (Standard_Integer aNodeIt = aPolygon->Nodes().Lower(); aNodeIt <= aPolygon->Nodes().Upper();
                     ++aNodeIt) {
...
void BRepLib::ExtendFace(const TopoDS_Face& theF, const Standard_Real theExtVal, const Standard_Boolean theExtUMin,
    ...
    if (aType == GeomAbs_Plane || aType == GeomAbs_Sphere || aType == GeomAbs_Cylinder || aType == GeomAbs_Torus ||
        ...
        if (theExtUMin || theExtUMax) anURes = aBAS.UResolution(theExtVal);
        if (theExtVMin || theExtVMax) aVRes = aBAS.VResolution(theExtVal);
    ...
    if (theF.Orientation() == TopAbs_REVERSED) theFExtended.Reverse();
}
