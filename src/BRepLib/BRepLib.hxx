...
public:
    ...
    Standard_EXPORT static Standard_Boolean BuildCurve3d(const TopoDS_Edge& E, const Standard_Real Tolerance = 1.0e-5,
    ...
    //! <MaxSegment> >= 30 in approximation
    Standard_EXPORT static Standard_Boolean BuildCurves3d(const TopoDS_Shape& S, const Standard_Real Tolerance,
                                                          const GeomAbs_Shape Continuity = GeomAbs_C1,
                                                          const Standard_Integer MaxDegree = 14,
                                                          const Standard_Integer MaxSegment = 0);
    ...
    template <class TCont> static void BuildPCurveForEdgesOnPlane(const TCont& theLE, const TopoDS_Face& theF) {
        for (typename TCont::Iterator aIt(theLE); aIt.More(); aIt.Next()) {
            ...
            if (!aE.IsNull()) BRepLib::BuildPCurveForEdgeOnPlane(aE, theF);
        }
    }
    ...
    Standard_EXPORT static Standard_Boolean UpdateEdgeTol(const TopoDS_Edge& E, const Standard_Real MinToleranceRequest,
    ...
    Standard_EXPORT static Standard_Boolean UpdateEdgeTolerance(const TopoDS_Shape& S,
    ...
    Standard_EXPORT static TopoDS_Edge SameParameter(const TopoDS_Edge& theEdge, const Standard_Real theTolerance,
                                                     Standard_Real& theNewTol, const Standard_Boolean IsUseOldEdge);

    //! Computes new 2d curve(s) for all the edges of  <S>
    //! to have the same parameter  as  the  3d curve.
    //! The algorithm is not done if the flag SameParameter
    //! was True  on an  Edge.
    Standard_EXPORT static void SameParameter(const TopoDS_Shape& S, const Standard_Real Tolerance = 1.0e-5,
    ...
    Standard_EXPORT static GeomAbs_Shape ContinuityOfFaces(const TopoDS_Edge& theEdge, const TopoDS_Face& theFace1,
    ...
    //! Warning: If the edges's regularity are coded before, nothing
    //! is done.
    Standard_EXPORT static void EncodeRegularity(const TopoDS_Shape& S, const TopTools_ListOfShape& LE,
    ...
    //! is done.
    Standard_EXPORT static void EncodeRegularity(TopoDS_Edge& E, const TopoDS_Face& F1, const TopoDS_Face& F2,
                                                 const Standard_Real TolAng = 1.0e-10);
    ...
    EnsureNormalConsistency(const TopoDS_Shape& S, const Standard_Real theAngTol = 0.001,
    ...
    //! other vertices.
    Standard_EXPORT static void BoundingVertex(const NCollection_List<TopoDS_Shape>& theLV, gp_Pnt& theNewCenter,
    ...
    Standard_EXPORT static Standard_Boolean FindValidRange(const Adaptor3d_Curve& theCurve, const Standard_Real theTolE,
                                                           const Standard_Real theParV1, const gp_Pnt& thePntV1,
                                                           const Standard_Real theTolV1, const Standard_Real theParV2,
                                                           const gp_Pnt& thePntV2, const Standard_Real theTolV2,
                                                           Standard_Real& theFirst, Standard_Real& theLast);

    //! Finds a range of 3d curve of the edge not covered by vertices tolerances.
    //! Returns false if there is no such range. Otherwise, sets theFirst and
    //! theLast as its bounds.
    Standard_EXPORT static Standard_Boolean FindValidRange(const TopoDS_Edge& theEdge, Standard_Real& theFirst,
    ...
    Standard_EXPORT static void ExtendFace(const TopoDS_Face& theF, const Standard_Real theExtVal,
