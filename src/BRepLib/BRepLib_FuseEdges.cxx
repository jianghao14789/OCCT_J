...
static void BCSmoothing(Handle(Geom_BSplineCurve) & theC, const Standard_Integer theCont, const Standard_Real theTol) {
...
static void MakeClosedCurve(Handle(Geom_Curve) & C, const gp_Pnt& PF, Standard_Real& f, Standard_Real& l) {
...
BRepLib_FuseEdges::BRepLib_FuseEdges(
    ...
    : myShape(theShape), myShapeDone(Standard_False), myEdgesDone(Standard_False), myResultEdgesDone(Standard_False),
      myNbConnexEdge(0), myConcatBSpl(Standard_False) {
...
void BRepLib_FuseEdges::BuildListEdges() {
    ...
    for (iEdg = 1; iEdg <= myMapEdgLstFac.Extent(); iEdg++) {
        ...
        if (!mapUniqEdg.Contains(edgecur) &&
            ...
            if (myAvoidEdg.Contains(edgecur)) continue; // edge is not allowed to be fused
        ...
        for (itLstEdg.Initialize(myMapLstEdg); itLstEdg.More(); itLstEdg.Next()) {
            ...
            if (!ME.IsDone()) {
                ...
                if (!ExtC.IsNull()) {
                    ...
                    ME.Init(ExtC, VF, VL);
                    if (!ME.IsDone()) throw Standard_ConstructionError("FuseEdges : Fusion failed");
                } else
            ...
            if (UpdatePCurve(OldEdge, NewEdge, LmapEdg)) myMapEdg.Bind(iLst, NewEdge);
        }
...
void BRepLib_FuseEdges::Perform() {
    ...
    if (myMapEdg.Extent() > 0) {
        ...
        for (itLstEdg.Initialize(myMapLstEdg); itLstEdg.More(); itLstEdg.Next()) {
            const Standard_Integer& iLst = itLstEdg.Key();
            if (!myMapEdg.IsBound(iLst)) continue;
            const TopTools_ListOfShape& LmapEdg = myMapLstEdg.Find(iLst);
...
void BRepLib_FuseEdges::BuildListConnexEdge(const TopoDS_Shape& theEdge, TopTools_MapOfShape& theMapUniq,
...
Standard_Boolean BRepLib_FuseEdges::NextConnexEdge(const TopoDS_Vertex& theVertex, const TopoDS_Shape& theEdge,
    ...
    if (LmapEdg.Extent() == 2) {
        ...
        if (myAvoidEdg.Contains(theEdgeConnex)) HasConnex = Standard_False; // edge is not allowed to be fused
...
Standard_Boolean BRepLib_FuseEdges::UpdatePCurve(const TopoDS_Edge& theOldEdge, TopoDS_Edge& theNewEdge,
        ...
        if (SameSurf) {
            ...
            if (BRep_Tool::IsClosed(theOldEdge, Surf, loc)) {
                ...
                if (l - f + 2. * Epsilon(l - f) < el - ef) {
                    ...
                    if (bcurve.IsNull()) bcurve = new Geom2d_TrimmedCurve(Curv2d, cf, cl);
                    ...
                    for (; iter.More(); iter.Next()) {
                        ...
                        if (BC.IsNull()) BC = new Geom2d_TrimmedCurve(C, first, last);
