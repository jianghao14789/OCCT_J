...
// purpose  :
//=======================================================================
Standard_Boolean BRepLib_MakeWire::BRepLib_BndBoxVertexSelector::Accept(const Standard_Integer& theObj) {
    if (theObj > myMapOfShape.Extent()) return Standard_False;

    const TopoDS_Vertex& aV = TopoDS::Vertex(myMapOfShape(theObj));

    if (theObj == myVInd) return Standard_False;
...
// purpose  :
//=======================================================================
void BRepLib_MakeWire::BRepLib_BndBoxVertexSelector::SetCurrentVertex(const gp_Pnt& theP, Standard_Real theTol,
...
void BRepLib_MakeWire::CollectCoincidentVertices(const TopTools_ListOfShape& theL,
    ...
    for (Standard_Integer i = 1; i <= anAllV.Extent(); i++) {
        ...
        if (myVertices.Contains(aV)) continue;
...
void BRepLib_MakeWire::CreateNewVertices(const NCollection_List<NCollection_List<TopoDS_Vertex>>& theGrVL,
    ...
    for (; itLLV.More(); itLLV.Next()) {
        ...
        for (; itLV.More(); itLV.Next()) {
            ...
            aValList.Append(aVV);
            if (myVertices.Contains(aVV)) aNewV = aVV;
        }
