...
// purpose  :
//=======================================================================
BRepLib_MakeShell::BRepLib_MakeShell() : myError(BRepLib_EmptyShell) {}
...
BRepLib_MakeShell::BRepLib_MakeShell(const Handle(Geom_Surface) & S, const Standard_Real UMin, const Standard_Real UMax,
...
void BRepLib_MakeShell::Init(const Handle(Geom_Surface) & S, const Standard_Real UMin, const Standard_Real UMax,
                             const Standard_Real VMin, const Standard_Real VMax, const Standard_Boolean Segment) {
    ...
    for (iu = 1; iu <= nu + 1; iu++) {
        ...
        if (!Precision::IsInfinite(u)) uisos(iu) = new Geom2d_Line(gp_Pnt2d(u, 0.), dv);
    }
    ...
    for (iv = 1; iv <= nv + 1; iv++) {
        ...
        if (!Precision::IsInfinite(v)) visos(iv) = new Geom2d_Line(gp_Pnt2d(0., v), du);
    }
    ...
    if (!Precision::IsInfinite(vpars(1))) {
        if (!Precision::IsInfinite(upars(1))) B.MakeVertex(vrt, S->Value(upars(1), vpars(1)), tol);
    ...
    for (iv = 1; iv <= nv; iv++) {
        ...
        if (vperiodic && iv == nv) {
            ...
            if (!Precision::IsInfinite(vpars(iv + 1))) {
                if (!Precision::IsInfinite(upars(1))) B.MakeVertex(vrt, S->Value(upars(1), vpars(iv + 1)), tol);
            }
        }
        ...
        for (iu = 1; iu <= nu; iu++) {
            ...
            if (GS.GetType() == GeomAbs_BSplineSurface && Segment) {
                Handle(Geom_BSplineSurface)::DownCast(SS)->Segment(upars(iu), upars(iu + 1), vpars(iv), vpars(iv + 1));
            }
            B.MakeFace(F, SS, tol);
