...
// purpose  :
//=======================================================================
BRepLib_MakeWire::BRepLib_MakeWire() : myError(BRepLib_EmptyWire) {}
...
BRepLib_MakeWire::BRepLib_MakeWire(const TopoDS_Edge& E1, const TopoDS_Edge& E2, const TopoDS_Edge& E3) {
    Add(E1);
    Add(E2);
    Add(E3);
}
...
void BRepLib_MakeWire::Add(const TopoDS_Wire& W) {
    for (TopoDS_Iterator it(W); it.More(); it.Next()) {
        ...
        if (myError != BRepLib_WireDone) break;
    }
}
...
void BRepLib_MakeWire::Add(const TopoDS_Edge& E, Standard_Boolean IsCheckGeometryProximity) {
    ...
    if (myEdge.IsNull()) {
        ...
        if (myError != BRepLib_NonManifoldWire) {
            if (VF.IsNull() || VL.IsNull()) myError = BRepLib_NonManifoldWire;
        }
        ...
        if (((forward == reverse) && (E.Orientation() == TopAbs_REVERSED)) || (reverse && !forward)) myEdge.Reverse();
    }
...
#ifdef OCCT_DEBUG
                if (!VF.IsSame(myVertex)) std::cout << "MakeWire : There is a PROBLEM !!" << std::endl;
#endif
    ...
    if (!VF.IsNull() && !VL.IsNull() && VF.IsSame(VL)) myShape.Closed(Standard_True);
