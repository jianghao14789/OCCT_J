...
// purpose  : auxiliary
//=======================================================================
static Standard_Real CalculVolume(const TopoDS_Shape& So, gp_Pnt& aRefPoint, Standard_Real tol,
                                  Standard_Boolean withForce, Draw_Interpretor& di) {
    ...
    for (TopExp_Explorer ex(So, TopAbs_FACE); ex.More(); ex.Next()) {
        ...
        for (Standard_Integer i = 1; i <= (facing->NbTriangles()); i++) {
            ...
            curVolume = TetraVol(aRefPoint, facing->Node(index1), facing->Node(index2), facing->Node(index3));
            myVolume += curVolume;
            curCentroid = TetraCen(aRefPoint, facing->Node(index1), facing->Node(index2), facing->Node(index3));
...
static Standard_Integer GetVolume(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (!aLabel.IsNull()) {
        ...
        // another case
        Standard_Real aVol;
        if (XCAFDoc_Volume::Get(aLabel, aVol)) di << aVol;
    }
    return 0;
}
...
static Standard_Integer GetArea(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (!aLabel.IsNull()) {
        ...
        // another case
        Standard_Real anA;
        if (XCAFDoc_Area::Get(aLabel, anA)) di << anA;
    }
    return 0;
}
...
static Standard_Integer GetCentroid(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (!aLabel.IsNull()) {
        ...
        if (aLabel.FindAttribute(XCAFDoc_Centroid::GetID(), aCentroid)) {
            ...
            // another case
            if (XCAFDoc_Centroid::Get(aLabel, aPoint)) di << aPoint.X() << " " << aPoint.Y() << " " << aPoint.Z();
        }
    }
    return 0;
}
    ...
    for (Standard_Integer i = 1; i <= seq.Length(); i++) {
        ...
        if (!aArea.IsNull()) {
            try {
                ...
                //), 	( wholeDoc ? "" : "\n" ));
                char string2[260];
                Sprintf(string2, "%s%9.1f (%3d%%)%s", (wholeDoc ? "" : "  Area defect:   "), aArea->Get() - G.Mass(),
                        (Standard_Integer)(Abs(G.Mass()) > 1e-10 ? 100. * (aArea->Get() - G.Mass()) / G.Mass() : 999.),
                        (wholeDoc ? "" : "\n"));
                di << string2;
            } catch (Standard_Failure const&) {
        ...
        if (!aVolume.IsNull() || !aCentroid.IsNull()) {
            try {
                ...
                if (!aVolume.IsNull()) {
                    ...
                    //   (Standard_Integer)( Abs ( localVolume ) > 1e-10 ? 100. * ( aVolume->Get() - localVolume ) /
                    //   localVolume : 999. ), ( wholeDoc ? "" : "\n" ));
                    char string5[260];
                ...
                if (!aCentroid.IsNull()) {
                    ...
                    if (wholeDoc) {
                        ...
                        Sprintf(string7, " %7.2f %7.2f %7.2f", p.X() - pcg.X(), p.Y() - pcg.Y(), p.Z() - pcg.Z());
                    } else {
...
static Standard_Boolean GetMassProps(const TDF_Label& aLabel, gp_XYZ& theCenterGravity, Standard_Real& theMassVal,
    ...
    if (aDensity == 0) {
        ...
        if (aLabel.FindAttribute(XCAFDoc::ShapeRefGUID(), Node) && Node->HasFather()) {
            ...
            TDF_LabelSequence comp;
            XCAFDoc_ShapeTool::GetComponents(aLabel, comp);
            if (!comp.Length()) return Standard_False;
...
static Standard_Integer ShapeMassProps(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc > 2) atol = Draw::Atof(argv[2]);
    if (Doc.IsNull()) {
        di << argv[1] << " is not a document\n";
        return 1;
    }
...
static Standard_Integer SetMaterial(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    MatTool->SetMaterial(aLabel, new TCollection_HAsciiString(argv[3]), new TCollection_HAsciiString(""),
                         Draw::Atof(argv[4]), new TCollection_HAsciiString("density measure"),
...
static Standard_Integer GetValidationProps(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc == 2)
        ...
        if (!aLabel.IsNull()) aLabels.Append(aLabel);
    }

    enum { Vol = 0, Area = 1, Centroid = 2 };
    ...
    for (; i <= aLabels.Length(); i++) {
        ...
        Handle(XCAFDoc_Centroid) aCentroid = new (XCAFDoc_Centroid);
        if (aLabel.FindAttribute(XCAFDoc_Centroid::GetID(), aCentroid)) XCAFDoc_Centroid::Get(aLabel, aP);
...
void XDEDRAW_Props::InitCommands(Draw_Interpretor& di) {
    ...
    di.Add("XSetVolume", "DocName {Label|Shape} volume \t: Setting volume to shape", __FILE__, SetVolume, g);
    ...
    di.Add("XSetCentroid", "DocName  {Label|Shape} x y z \t: Setting centroid to shape", __FILE__, SetCentroid, g);

    di.Add("XGetCentroid", "DocName {Shape|Label} \t: Getting centroid of shape ", __FILE__, GetCentroid, g);

    di.Add("XSetProps", "DocName {Shape|Label} [epsilon = 0.001] \t: Compute properties for shape ", __FILE__, SetProps,
           g);

    di.Add("XCheckProps", "DocName [ 0|deflection [Shape|Label] ]\t: Check properties recorded for shape ", __FILE__,
           CheckProps, g);
    di.Add("XShapeVolume", "Shape \t: Calculating volume of shape", __FILE__, ShapeVolume, g);
    di.Add("XShapeMassProps",
           "XShapeMassProps DocName [deflection [Shape|Label] ]\t: Get mass value and center of gravity for shape ",
           __FILE__, ShapeMassProps, g);
    di.Add("XSetMaterial", "Doc {Label|Shape} name density(g/cu sm) \t: Set material to shape given by Label", __FILE__,
           SetMaterial, g);
