...
static TDF_Label findVisMaterial(const Handle(TDocStd_Document) & theDoc, const TCollection_AsciiString& theKey) {
    ...
    for (TDF_LabelSequence::Iterator aLabIter(aLabels); aLabIter.More(); aLabIter.Next()) {
        ...
        if (aLabIter.Value().FindAttribute(TDataStd_Name::GetID(), aNodeName) && aNodeName->Get().IsEqual(theKey)) {
...
static bool parseRgbColor(Standard_Integer& theArgIter, Quantity_Color& theColor, Standard_Integer theNbArgs,
                          const char** theArgVec) {
    Standard_Integer aNbParsed = Draw::ParseColor(theNbArgs - theArgIter - 1, theArgVec + theArgIter + 1, theColor);
    if (aNbParsed == 0) {
...
// Purpose  :
// ================================================================
static Standard_Integer XGetAllVisMaterials(Draw_Interpretor& theDI, Standard_Integer theNbArgs,
                                            const char** theArgVec) {
...
static Standard_Integer XAddVisMaterial(Draw_Interpretor&, Standard_Integer theNbArgs, const char** theArgVec) {
    ...
    for (Standard_Integer anArgIter = 3; anArgIter < theNbArgs; ++anArgIter) {
        ...
        } else if (anArg == "-specular" && parseRgbColor(anArgIter, aMatCom.SpecularColor, theNbArgs, theArgVec)) {
            aMatCom.IsDefined = true;
        } else if (anArg == "-ambient" && parseRgbColor(anArgIter, aMatCom.AmbientColor, theNbArgs, theArgVec)) {
            aMatCom.IsDefined = true;
        } else if (anArg == "-emissive" && parseRgbColor(anArgIter, aMatCom.EmissiveColor, theNbArgs, theArgVec)) {
            aMatCom.IsDefined = true;
...
void XDEDRAW_Colors::InitCommands(Draw_Interpretor& di) {
    ...
    di.Add("XSetObjVisibility", "Doc {Label|Shape} (0\1) \t: Set the visibility of shape  ", __FILE__, setVisibility,
           g);

    di.Add("XGetObjVisibility", "Doc {Label|Shape} \t: Return the visibility of shape ", __FILE__, getVisibility, g);

    di.Add("XGetInstanceVisible", "Doc Shape \t: Return the visibility of shape ", __FILE__, getStyledVisibility, g);
