...
static Standard_Integer getLayerRefs(Draw_Interpretor& theDI, Standard_Integer theArgc, const char** theArgv) {
    if (theArgc != 3) {
...
void XDEDRAW_Layers::InitCommands(Draw_Interpretor& di) {
    ...
    di.Add("XSetLayer",
           "DocName {Shape|Label} StringLayer [shapeInOneLayer(0/1)] \t: Set reference between Shape and Layer (add "
           "layer if nesessary). shapeInOneLayer 0 is default",
           __FILE__, setLayer, g);

    di.Add("XGetLayers", "DocName {Shape|Label} \t: Get layers of indicated shape", __FILE__, getLayers, g);

    di.Add("XGetOneLayer", "DocName LayerLabel \t: Print name of layer.", __FILE__, getOneLayer, g);

    di.Add("XAddLayer", "DocName StringLayer \t: Adding layer in XCAFDocument.", __FILE__, addLayer, g);

    di.Add("XSetLinkLayer",
           "DocName {Shape|Label} LayerL [shapeInOneLayer(0/1)] \t: Set reference between shape and existing layer. "
           "shapeInOneLayer 0 is default",
           __FILE__, setLinkLayer, g);

    di.Add("XGetAllLayers", "DocName \t: Get all layers in XCAFDocument.", __FILE__, getAllLayers, g);

    di.Add("XUnSetLayer", "DocName {Shape|Label} stringL \t: unset shape from indicated layer.", __FILE__, unSetLayer,
           g);

    di.Add("XUnSetAllLayers", "DocName {Shape|Label} \t: unset shape from all layers.", __FILE__, unSetAllLayers, g);

    di.Add("XRemoveLayer", "DocName {Label|string} \t:remove layer from XCAFDocument.", __FILE__, removeLayer, g);

    di.Add("XRemoveAllLayers", "DocName \t: remove all layers from XCAFDocument.", __FILE__, removeAllLayers, g);

    di.Add("XFindLayer", "DocName string \t: Print label where are layer is situated.", __FILE__, findLayer, g);

    di.Add("XGetLayerLabels", "DocName \t: Print labels from layertable.", __FILE__, getLayerLabels, g);

    di.Add("XSetVisibility", "DocName {layerLable|StringLayer} [isvisible(1/0)] \t: Set visibility of layer", __FILE__,
           setVisibility, g);

    di.Add("XIsVisible", "DocName {layerLable|StringLayer} \t: Return 1 if layer is visible, 0 if not", __FILE__,
           isVisible, g);

    di.Add("XGetLayerRefs",
           "DocName Label \t: Prints layers labels which are referenced in passed label or prints labels which "
           "reference passed layer label.",
