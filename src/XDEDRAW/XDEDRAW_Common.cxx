...
static void AddWS(TCollection_AsciiString filename, const Handle(XSControl_WorkSession) & WS) {
    WS->SetVars(new XSDRAW_Vars); // support of DRAW variables
    thedictws.Bind(filename, WS);
}

static Standard_Boolean
FillDicWS(NCollection_DataMap<TCollection_AsciiString, Handle(STEPCAFControl_ExternFile)>& dicFile) {
    ClearDicWS();
    if (dicFile.IsEmpty()) {
...
static Standard_Boolean SetCurrentWS(TCollection_AsciiString filename) {
    ...
    Handle(XSControl_WorkSession) CurrentWS = Handle(XSControl_WorkSession)::DownCast(thedictws.ChangeFind(filename));
...
static Standard_Integer WriteStep(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 3) {
        di << "Use: " << argv[0]
           << " Doc filename [mode [multifile_prefix [label]]]: write document to the STEP file\n";
    ...
    if (argc > k) {
        switch (argv[k][0]) {
            ...
            case '4':
                ...
                break;
            default:
                di << "3rd arg = mode, incorrect [give fsmw]\n";
                return 1;
        }
        ...
        for (Standard_Integer i = 0; argv[k][i]; i++)
            switch (argv[3][i]) {
                ...
                case '+':
                    ...
                    break;
                case 'c':
                    writer.SetColorMode(wrmode);
                    break;
                case 'n':
                    writer.SetNameMode(wrmode);
                    break;
                case 'l':
                    writer.SetLayerMode(wrmode);
                    break;
                case 'v':
                    writer.SetPropsMode(wrmode);
                    break;
            }
        k++;
    }
    ...
    if (argc > k) {
        ...
        if (aStr.Search(":") == -1) multifile = argv[k++];
    }
    if (argc > k) {
    ...
    if (modfic) {
        ...
        switch (stat) {
            ...
            case IFSelect_RetDone: {
                ...
                NCollection_DataMap<TCollection_AsciiString, Handle(STEPCAFControl_ExternFile)> DicFile =
                    writer.ExternFiles();
                FillDicWS(DicFile);
...
static Standard_Integer Expand(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (atoi(argv[2]) != 0) recurs = Standard_True;
...
// purpose  :
//=======================================================================
static Standard_Integer Extract(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    if (argc < 4) {
...
void XDEDRAW_Common::InitCommands(Draw_Interpretor& di) {
    ...
    di.Add("ReadStep",
           ...
           __FILE__, ReadStep, g);
    di.Add("WriteStep", "Doc filename [mode=a [multifile_prefix] [label]]: Write DECAF document to STEP file", __FILE__,
           WriteStep, g);
    ...
    di.Add("WriteVrml",
           "Doc filename [version VRML#1.0/VRML#2.0 (1/2): 2 by default] [representation shaded/wireframe/both "
           "(0/1/2): 0 by default]",
