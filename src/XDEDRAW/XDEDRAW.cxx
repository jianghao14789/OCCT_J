...
static Standard_Integer openDoc(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc < 3) {
        di << "invalid number of arguments. Usage:\t XOpen filename docname [-skipAttribute] [-readAttribute] "
              "[-readPath] [-append|-overwrite]\n";
        return 1;
    }
...
static void StatAssembly(const TDF_Label L, const Standard_Integer level, Handle(TColStd_HArray1OfInteger) & HAI,
                         Standard_Integer& NbCentroidProp, Standard_Integer& NbVolumeProp, Standard_Integer& NbAreaProp,
                         Standard_Integer& NbShapesWithName, Standard_Integer& NbShapesWithColor,
                         Standard_Integer& NbShapesWithLayer, Handle(TDocStd_Document) & aDoc,
                         Standard_Boolean& PrintStructMode, Draw_Interpretor& di) {
    if (PrintStructMode) {
    ...
    if (L.HasChild()) {
        for (Standard_Integer i = 1; i <= L.NbChildren(); i++) {
            StatAssembly(L.FindChild(i), level + 1, HAI, NbCentroidProp, NbVolumeProp, NbAreaProp, NbShapesWithName,
                         NbShapesWithColor, NbShapesWithLayer, aDoc, PrintStructMode, di);
        }
    }
}
...
static Standard_Integer statdoc(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (i = 1; i <= SeqLabels.Length(); i++) {
        StatAssembly(SeqLabels.Value(i), level, HAI, NbCentroidProp, NbVolumeProp, NbAreaProp, NbShapesWithName,
                     NbShapesWithColor, NbShapesWithLayer, Doc, PrintStructMode, di);
    }
...
private:
    ...
    XDEDRAW_XDisplayTool()
        : myDispMode(-2), myHiMode(-2), myIsAutoTriang(-1), myToPrefixDocName(Standard_True),
          myToGetNames(Standard_True), myToExplore(Standard_False) {}

    //! Display single label.
    Standard_Integer displayLabel(Draw_Interpretor& theDI, const TDF_Label& theLabel,
                                  const TCollection_AsciiString& theNamePrefix, const TopLoc_Location& theLoc,
    ...
    Standard_Integer xdisplay(Draw_Interpretor& theDI, Standard_Integer theNbArgs, const char** theArgVec) {
        ...
        for (Standard_Integer anArgIter = 1; anArgIter < theNbArgs; ++anArgIter) {
            ...
            if (anAutoUpdater.parseRedrawMode(anArgCase)) {
                ...
                if (anArgIter + 1 < theNbArgs && Draw::ParseOnOff(theArgVec[anArgIter + 1], myToPrefixDocName)) {
                    ++anArgIter;
                }
        ...
        for (TDF_LabelSequence::Iterator aLabIter(myLabels); aLabIter.More(); aLabIter.Next()) {
            ...
            if (displayLabel(theDI, aLabel, myToPrefixDocName ? myDocName + ":" : "", TopLoc_Location(),
                             myOutDispList) == 1) {
                return 1;
            }
        }
...
//=======================================================================
static Standard_Integer XShowFaceBoundary(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
...
static Standard_Integer testDoc(Draw_Interpretor&, Standard_Integer argc, const char** argv) {
    ...
    Handle(Geom_Axis2Placement) aPlacement = new Geom_Axis2Placement(gp::Origin(), gp::DZ(), gp::DX());
    ...
    if (aLab.FindAttribute(TPrsStd_AISPresentation::GetID(), prs)) prs->Display();
...
void XDEDRAW::Init(Draw_Interpretor& di) {
    ...
    di.Add(
        "XOpen",
        "Path Doc [-skipAttribute] [-readAttribute] [-readPath] [-append|-overwrite]\t: Open XDE Document with name "
        "Doc from Path"
        "\n\t\t The options are:"
        ...
        "\n\t\t   -append : to read file into already existing document once again, append new attributes and don't "
        "touch existing"
        "\n\t\t   -overwrite : to read file into already existing document once again, overwriting existing attributes",
        __FILE__, openDoc, g);

    di.Add("Xdump", "Doc [int deep (0/1)] \t: Print information about tree's structure", __FILE__, dump, g);

    di.Add("XStat", "Doc \t: Print statistics of document", __FILE__, statdoc, g);

    di.Add("XSetPrs", "Doc [label1 lavbel2 ...] \t: Set presentation for given label(s) or whole doc", __FILE__, setPrs,
           g);

    di.Add("XShow", "Doc [label1 lavbel2 ...] \t: Display document (or some labels) in a graphical window", __FILE__,
           show, g);
    ...
    di.Add("XWdump", "Doc filename.{gif|xwd|bmp} \t: Dump contents of viewer window to XWD, GIF or BMP file", __FILE__,
           xwd, g);

    di.Add("XAttributeValue", "Doc label #attribute: internal command for browser", __FILE__, XAttributeValue, g);

    di.Add("XSetViewNameMode", "(1/0) \t: Set/Unset mode of displaying names.", __FILE__, setviewName, g);

    di.Add("XGetViewNameMode", "\t: Print if  mode of displaying names is turn on.", __FILE__, getviewName, g);

    di.Add("XSetTransparency",
           "Doc Transparency [label1 label2 ...]\t: Set transparency for given label(s) or whole doc", __FILE__,
           XSetTransparency, g);

    di.Add("XSetLengthUnit",
           "Doc {unit_name|scale_factor}\t: Set value of length unit"
           "\n\t\t: Possible unit_name: m, mm, km, cm, micron, mille, in(inch), min(microinch), nin(nano inch), ft, "
           "stat.mile"
