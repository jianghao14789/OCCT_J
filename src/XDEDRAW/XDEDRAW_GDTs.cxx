        ...
        for (Standard_Integer j = 1; j <= aGDTs.Length(); j++) {
            ...
            if (aGDTs.Value(j).FindAttribute(XCAFDoc_Dimension::GetID(), aDimTol)) {
                ...
                if (argc > 3) {
                    ...
                    if (aDimTolObj->HasQualifier()) di << ", Q " << aDimTolObj->GetQualifier();
...
static Standard_Integer DumpNbDGTs(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    if (argc == 3) {
        char aChar = argv[2][0];
        if (aChar == 'f') isFull = Standard_True;
    }
    Handle(TDocStd_Document) Doc;
    DDocStd::GetDocument(argv[1], Doc);
    if (Doc.IsNull()) {
...
static Standard_Integer addDatum(Draw_Interpretor& di, Standard_Integer argc, const char** argv) {
    ...
    for (Standard_Integer i = 2; i < argc; i++) {
        ...
        if (aLabel.IsNull()) {
            ...
            if (!aShape.IsNull()) aShapeTool->Search(aShape, aLabel);
...
void XDEDRAW_GDTs::InitCommands(Draw_Interpretor& di) {
    ...
    di.Add("XGetTypeOfToleranceValue", "XGetTypeOfToleranceValue Doc GTol_Label", __FILE__, getTypeOfTolVal, g);
    ...
    di.Add("XSetTolZoneModValue", "XSetTolZoneModValue Doc GTol_Label val", __FILE__, setZoneModVal, g);
    ...
    di.Add("XGetDimensionQualifier", "XGetDimensionQualifier Doc Dim_Label", __FILE__, getDimQalif, g);

    di.Add("XSetDimensionRange", "XSetDimensionRange Doc Dim_Label low_val up_val", __FILE__, setDimRange, g);

    di.Add("XGetDimensionRange", "XGetDimensionRange Doc Dim_Label", __FILE__, getDimRange, g);

    di.Add("XSetDimensionPlusMinusTol", "XSetDimensionPlusMinusTol Doc Dim_Label low_val up_val", __FILE__,
           setDimPlusMinusTol, g);

    di.Add("XGetDimensionPlusMinusTol", "XGetDimensionPlusMinusTol Doc Dim_Label", __FILE__, getDimPlusMinusTol, g);
    ...
    di.Add("XGetDimensionClassOfTol", "XGetDimensionClassOfTol Doc Dim_Label", __FILE__, getDimClassTol, g);

    di.Add("XSetDimensionNbOfDecimalPlaces", "XSetDimensionNbOfDecimalPlaces Doc Dim_Label l_val r_val", __FILE__,
           setDimNbOfDecimalPlaces, g);

    di.Add("XGetDimensionNbOfDecimalPlaces", "XGetDimensionNbOfDecimalPlaces Doc Dim_Label", __FILE__,
           getDimNbOfDecimalPlaces, g);
    ...
    di.Add("XGetDimensionModifiers", "XGetDimensionModifiers Doc Dim_Label", __FILE__, getDimModifier, g);

    di.Add("XSetDimensionPath", "XSetDimensionPath Doc Dim_Label path(edge)", __FILE__, addDimPath, g);

    di.Add("XSetDimensionPoints", "XSetDimensionPoints Doc Dim_Label v1 [v2]", __FILE__, addDimPoints, g);
    ...
    di.Add("XSetGDTPosition",
           "XSetGDTPosition Doc GDT_Label loc_x loc_y loc_z normal_x normal_y normal_z xdir_x xdir_y xdir_z"
    ...
    di.Add("XGetGDTSemanticName", "XGetGDTSemanticName Doc GDT_Label" __FILE__, getGDTSemanticName, g);
