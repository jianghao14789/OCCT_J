...
GeomEvaluator_OffsetCurve::GeomEvaluator_OffsetCurve(const Handle(Geom_Curve) & theBase, const Standard_Real theOffset,
                                                     const gp_Dir& theDirection)
    : GeomEvaluator_Curve(), myBaseCurve(theBase), myOffset(theOffset), myOffsetDir(theDirection) {}

GeomEvaluator_OffsetCurve::GeomEvaluator_OffsetCurve(const Handle(GeomAdaptor_Curve) & theBase,
                                                     const Standard_Real theOffset, const gp_Dir& theDirection)
    : GeomEvaluator_Curve(), myBaseAdaptor(theBase), myOffset(theOffset), myOffsetDir(theDirection) {}
...
void GeomEvaluator_OffsetCurve::D2(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2) const {
    gp_Vec aD3;
...
void GeomEvaluator_OffsetCurve::D3(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2,
                                   gp_Vec& theD3) const {
    gp_Vec aD4;
...
gp_Vec GeomEvaluator_OffsetCurve::DN(const Standard_Real theU, const Standard_Integer theDeriv) const {
...
void GeomEvaluator_OffsetCurve::BaseD1(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetCurve::BaseD2(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetCurve::BaseD3(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2,
                                       gp_Vec& theD3) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetCurve::BaseD4(const Standard_Real theU, gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2,
                                       gp_Vec& theD3, gp_Vec& theD4) const {
    if (!myBaseAdaptor.IsNull()) {
        myBaseAdaptor->D3(theU, theValue, theD1, theD2, theD3);
...
gp_Vec GeomEvaluator_OffsetCurve::BaseDN(const Standard_Real theU, const Standard_Integer theDeriv) const {
    if (!myBaseAdaptor.IsNull()) return myBaseAdaptor->DN(theU, theDeriv);
...
void GeomEvaluator_OffsetCurve::CalculateD1(gp_Pnt& theValue, gp_Vec& theD1, const gp_Vec& theD2) const {
    ...
    if (R3 <= gp::Resolution()) {
        if (R2 <= gp::Resolution()) throw Standard_NullValue("GeomEvaluator_OffsetCurve: Null derivative");
...
void GeomEvaluator_OffsetCurve::CalculateD2(gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2, const gp_Vec& theD3,
    ...
    if (R5 <= gp::Resolution()) {
        if (R4 <= gp::Resolution()) throw Standard_NullValue("GeomEvaluator_OffsetCurve: Null derivative");
        // We try another computation but the stability is not very good
        // dixit ISG.
        //  V2 = P" (U) :
        R4 = R2 * R2;
    ...
    theD1.Add(gp_Vec(DNdir));
    // P"(u) :
    if (theIsDirChange) theD2.Reverse();
    theD2.Add(gp_Vec(D2Ndir));
}

void GeomEvaluator_OffsetCurve::CalculateD3(gp_Pnt& theValue, gp_Vec& theD1, gp_Vec& theD2, gp_Vec& theD3,
                                            const gp_Vec& theD4, const Standard_Boolean theIsDirChange) const {
    // P(u) = p(u) + Offset * Ndir / R
    // with R = || p' ^ V|| and Ndir = P' ^ direction (local normal direction)
    ...
    if (R7 <= gp::Resolution()) {
        if (R6 <= gp::Resolution()) throw Standard_NullValue("CSLib_Offset: Null derivative");
        // V3 = P"' (U) :
    ...
    theD2.Add(gp_Vec(D2Ndir));
    // P"'(u)
    if (theIsDirChange) theD3.Reverse();
    theD3.Add(gp_Vec(D2Ndir));
}

Standard_Boolean GeomEvaluator_OffsetCurve::AdjustDerivative(const Standard_Integer theMaxDerivative,
                                                             const Standard_Real theU, gp_Vec& theD1, gp_Vec& theD2,
                                                             gp_Vec& theD3, gp_Vec& theD4) const {
