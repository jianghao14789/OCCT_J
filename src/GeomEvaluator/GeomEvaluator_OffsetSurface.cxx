...
static Standard_Boolean shiftPoint(const Standard_Real theUStart, const Standard_Real theVStart, Standard_Real& theU,
                                   Standard_Real& theV, const Handle(Geom_Surface) & theSurf,
                                   const Handle(GeomAdaptor_Surface) & theAdaptor, const gp_Vec& theD1U,
                                   const gp_Vec& theD1V) {
    ...
    //  std::cout << "Normal calculation failed at (" << theUStart << ", " << theVStart << "), shifting to (" << theU <<
    //  ", " << theV << ")" << std::endl;
...
static void derivatives(Standard_Integer theMaxOrder, Standard_Integer theMinOrder, const Standard_Real theU,
                        const Standard_Real theV, const SurfOrAdapt& theBasisSurf, const Standard_Integer theNU,
                        const Standard_Integer theNV, const Standard_Boolean theAlongU,
                        const Standard_Boolean theAlongV, const Handle(Geom_BSplineSurface) & theL,
                        TColgp_Array2OfVec& theDerNUV, TColgp_Array2OfVec& theDerSurf) {
    ...
    if (theAlongU || theAlongV) {
        ...
        for (i = 0; i <= theMaxOrder + theNU; i++)
            for (j = 0; j <= theMaxOrder + theNV; j++) {
                if (theAlongU) theDerNUV.SetValue(i, j, CSLib::DNNUV(i, j, DerSurfL, theDerSurf));
                if (theAlongV) theDerNUV.SetValue(i, j, CSLib::DNNUV(i, j, theDerSurf, DerSurfL));
            }
    } else {
...
inline Standard_Boolean IsInfiniteCoord(const gp_Vec& theVec) {
    return Precision::IsInfinite(theVec.X()) || Precision::IsInfinite(theVec.Y()) || Precision::IsInfinite(theVec.Z());
}
...
GeomEvaluator_OffsetSurface::GeomEvaluator_OffsetSurface(const Handle(Geom_Surface) & theBase,
                                                         const Standard_Real theOffset,
                                                         const Handle(Geom_OsculatingSurface) & theOscSurf)
    : GeomEvaluator_Surface(), myBaseSurf(theBase), myOffset(theOffset), myOscSurf(theOscSurf) {
    if (!myOscSurf.IsNull()) return; // osculating surface already exists

    // Create osculating surface for B-spline and Besier surfaces only
    if (myBaseSurf->IsKind(STANDARD_TYPE(Geom_BSplineSurface)) || myBaseSurf->IsKind(STANDARD_TYPE(Geom_BezierSurface)))
        myOscSurf = new Geom_OsculatingSurface(myBaseSurf, Precision::Confusion());
}

GeomEvaluator_OffsetSurface::GeomEvaluator_OffsetSurface(const Handle(GeomAdaptor_Surface) & theBase,
                                                         const Standard_Real theOffset,
                                                         const Handle(Geom_OsculatingSurface) & theOscSurf)
    : GeomEvaluator_Surface(), myBaseAdaptor(theBase), myOffset(theOffset), myOscSurf(theOscSurf) {}

void GeomEvaluator_OffsetSurface::D0(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue) const {
...
void GeomEvaluator_OffsetSurface::D1(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                     gp_Vec& theD1U, gp_Vec& theD1V) const {
    Standard_Real aU = theU, aV = theV;
    for (;;) {
...
void GeomEvaluator_OffsetSurface::D2(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                     gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V,
    ...
    for (;;) {
        ...
        try {
            CalculateD2(aU, aV, theValue, theD1U, theD1V, theD2U, theD2V, theD2UV, aD3U, aD3V, aD3UUV, aD3UVV);
            break;
        } catch (Geom_UndefinedValue&) {
            // if failed at parametric boundary, try taking derivative at shifted point
            if (!shiftPoint(theU, theV, aU, aV, myBaseSurf, myBaseAdaptor, theD1U, theD1V)) {
...
void GeomEvaluator_OffsetSurface::D3(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                     gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V, gp_Vec& theD2UV,
                                     gp_Vec& theD3U, gp_Vec& theD3V, gp_Vec& theD3UUV, gp_Vec& theD3UVV) const {
    Standard_Real aU = theU, aV = theV;
    for (;;) {
        BaseD3(aU, aV, theValue, theD1U, theD1V, theD2U, theD2V, theD2UV, theD3U, theD3V, theD3UUV, theD3UVV);

        CheckInfinite(theD1U, theD1V);

        try {
            CalculateD3(aU, aV, theValue, theD1U, theD1V, theD2U, theD2V, theD2UV, theD3U, theD3V, theD3UUV, theD3UVV);
            break;
        } catch (Geom_UndefinedValue&) {
            // if failed at parametric boundary, try taking derivative at shifted point
            if (!shiftPoint(theU, theV, aU, aV, myBaseSurf, myBaseAdaptor, theD1U, theD1V)) {
...
gp_Vec GeomEvaluator_OffsetSurface::DN(const Standard_Real theU, const Standard_Real theV,
    ...
    Standard_RangeError_Raise_if(theDerU + theDerV < 1, "GeomEvaluator_OffsetSurface::DN(): theDerU + theDerV < 1");
...
void GeomEvaluator_OffsetSurface::BaseD0(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetSurface::BaseD1(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                         gp_Vec& theD1U, gp_Vec& theD1V) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetSurface::BaseD2(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                         gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V,
                                         gp_Vec& theD2UV) const {
    if (!myBaseAdaptor.IsNull())
...
void GeomEvaluator_OffsetSurface::BaseD3(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                         gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V,
                                         gp_Vec& theD2UV, gp_Vec& theD3U, gp_Vec& theD3V, gp_Vec& theD3UUV,
                                         gp_Vec& theD3UVV) const {
    if (!myBaseAdaptor.IsNull())
        myBaseAdaptor->D3(theU, theV, theValue, theD1U, theD1V, theD2U, theD2V, theD2UV, theD3U, theD3V, theD3UUV,
                          theD3UVV);
    else
        myBaseSurf->D3(theU, theV, theValue, theD1U, theD1V, theD2U, theD2V, theD2UV, theD3U, theD3V, theD3UUV,
                       theD3UVV);
}

void GeomEvaluator_OffsetSurface::CalculateD0(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
    ...
    if (aD1UNorm2 > 1.0) aD1U /= Sqrt(aD1UNorm2);
    if (aD1VNorm2 > 1.0) aD1V /= Sqrt(aD1VNorm2);
    ...
    if (aNorm.SquareMagnitude() > the_D1MagTol * the_D1MagTol) {
        ...
        CSLib::Normal(MaxOrder, DerNUV, the_D1MagTol, theU, theV, Umin, Umax, Vmin, Vmax, NStatus, Normal, OrderU,
                      OrderV);
        ...
        if (NStatus != CSLib_Defined)
            throw Geom_UndefinedValue("GeomEvaluator_OffsetSurface::CalculateD0(): Unable to calculate normal");

        theValue.SetXYZ(theValue.XYZ() + myOffset * aSign * Normal.XYZ());
    }
}

void GeomEvaluator_OffsetSurface::CalculateD1(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                              gp_Vec& theD1U, gp_Vec& theD1V, const gp_Vec& theD2U,
                                              const gp_Vec& theD2V, const gp_Vec& theD2UV) const {
    // Check offset side.
    Handle(Geom_BSplineSurface) L;
    Standard_Boolean isOpposite = Standard_False;
    Standard_Boolean AlongU = Standard_False;
    ...
    if (!isSingular) {
        ...
        aN1U.SetX(theD2U.Y() * theD1V.Z() + theD1U.Y() * theD2UV.Z() - theD2U.Z() * theD1V.Y() -
                  theD1U.Z() * theD2UV.Y());
        aN1U.SetY(
            (theD2U.X() * theD1V.Z() + theD1U.X() * theD2UV.Z() - theD2U.Z() * theD1V.X() - theD1U.Z() * theD2UV.X()) *
            -1.0);
        ...
        aN1V.SetX(theD2UV.Y() * theD1V.Z() + theD2V.Z() * theD1U.Y() - theD2UV.Z() * theD1V.Y() -
                  theD2V.Y() * theD1U.Z());
        aN1V.SetY(
            (theD2UV.X() * theD1V.Z() + theD2V.Z() * theD1U.X() - theD2UV.Z() * theD1V.X() - theD2V.X() * theD1U.Z()) *
            -1.0);
    ...
    if (NStatus != CSLib_Defined)
        throw Geom_UndefinedValue("GeomEvaluator_OffsetSurface::CalculateD1(): Unable to calculate normal");
...
void GeomEvaluator_OffsetSurface::CalculateD2(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                              gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V,
                                              gp_Vec& theD2UV, const gp_Vec& theD3U, const gp_Vec& theD3V,
    ...
    CSLib::Normal(MaxOrder, DerNUV, the_D1MagTol, theU, theV, Umin, Umax, Vmin, Vmax, NStatus, Normal, OrderU, OrderV);
    if (NStatus != CSLib_Defined)
        throw Geom_UndefinedValue("GeomEvaluator_OffsetSurface::CalculateD2(): Unable to calculate normal");
...
void GeomEvaluator_OffsetSurface::CalculateD3(const Standard_Real theU, const Standard_Real theV, gp_Pnt& theValue,
                                              gp_Vec& theD1U, gp_Vec& theD1V, gp_Vec& theD2U, gp_Vec& theD2V,
                                              gp_Vec& theD2UV, gp_Vec& theD3U, gp_Vec& theD3V, gp_Vec& theD3UUV,
                                              gp_Vec& theD3UVV) const {
    gp_Dir Normal;
    ...
    CSLib::Normal(MaxOrder, DerNUV, the_D1MagTol, theU, theV, Umin, Umax, Vmin, Vmax, NStatus, Normal, OrderU, OrderV);
    if (NStatus != CSLib_Defined)
        throw Geom_UndefinedValue("GeomEvaluator_OffsetSurface::CalculateD3(): Unable to calculate normal");
...
gp_Vec GeomEvaluator_OffsetSurface::CalculateDN(const Standard_Real theU, const Standard_Real theV,
    ...
    CSLib::Normal(MaxOrder, DerNUV, the_D1MagTol, theU, theV, Umin, Umax, Vmin, Vmax, NStatus, Normal, OrderU, OrderV);
    if (NStatus != CSLib_Defined)
        throw Geom_UndefinedValue("GeomEvaluator_OffsetSurface::CalculateDN(): Unable to calculate normal");
...
void GeomEvaluator_OffsetSurface::Bounds(Standard_Real& theUMin, Standard_Real& theUMax, Standard_Real& theVMin,
...
Standard_Boolean GeomEvaluator_OffsetSurface::ReplaceDerivative(const Standard_Real theU, const Standard_Real theV,
    ...
    if (isReplaceDU ^ isReplaceDV) {
        ...
        if (isReplaceDV) {
            ...
            if (aStep > aUMax - aUMin) aStep = (aUMax - aUMin) / 100.;
        } else {
            aStep = Precision::Confusion() * theDV.Magnitude();
            if (aStep > aVMax - aVMin) aStep = (aVMax - aVMin) / 100.;
        }
        ...
        for (Standard_Real aStepSign = -1.0; aStepSign <= 1.0 && !isReplaced; aStepSign += 2.0) {
            if (isReplaceDV) {
                ...
                if (aU < aUMin || aU > aUMax) continue;
            } else {
                aV = theV + aStepSign * aStep;
                if (aV < aVMin || aV > aVMax) continue;
            }
