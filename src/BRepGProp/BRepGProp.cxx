...
static gp_Pnt roughBaryCenter(const TopoDS_Shape& S) {
    ...
    if (i > 0) {
        ...
        for (; ex.More(); ex.Next()) {
            ...
            const Handle(Poly_Triangulation) & aTri = BRep_Tool::Triangulation(TopoDS::Face(aF), aLocDummy);
...
static Standard_Real surfaceProperties(const TopoDS_Shape& S, GProp_GProps& Props, const Standard_Real Eps,
                                       const Standard_Boolean SkipShared, const Standard_Boolean UseTriangulation) {
    Standard_Integer i;
#ifdef OCCT_DEBUG
...
Standard_Real BRepGProp::VolumeProperties(const TopoDS_Shape& S, GProp_GProps& Props, const Standard_Real Eps,
                                          const Standard_Boolean OnlyClosed, const Standard_Boolean SkipShared) {
    // find the origin
    gp_Pnt P(0, 0, 0);
    P.Transform(S.Location());
    Props = GProp_GProps(P);
...
static Standard_Real volumePropertiesGK(const TopoDS_Shape& theShape, GProp_GProps& theProps,
                                        const Standard_Real theTol, const Standard_Boolean IsUseSpan,
                                        const Standard_Boolean CGFlag, const Standard_Boolean IFlag,
                                        const Standard_Boolean SkipShared) {
    ...
    for (; anExp.More(); anExp.Next()) {
        ...
        if (isFwd || isRvs) {
            ...
            if (aLocalError < 0.) return aLocalError;
...
Standard_Real BRepGProp::VolumePropertiesGK(const TopoDS_Shape& S, GProp_GProps& Props, const Standard_Real Eps,
                                            const Standard_Boolean OnlyClosed, const Standard_Boolean IsUseSpan,
                                            const Standard_Boolean CGFlag, const Standard_Boolean IFlag,
                                            const Standard_Boolean SkipShared) {
    gp_Pnt P(0, 0, 0);
    Standard_Real anError = 0.;
    ...
    if (OnlyClosed) {
        ...
        for (; anExp.More(); anExp.Next()) {
            ...
            if (aStatus == BRepCheck_NoError) aClosedShells.Append(aShell);
        }

        if (aClosedShells.IsEmpty()) return -1.;
...
static Standard_Real volumePropertiesGK(const TopoDS_Shape& theShape, GProp_GProps& theProps, const gp_Pln& thePln,
                                        const Standard_Real theTol, const Standard_Boolean IsUseSpan,
                                        const Standard_Boolean CGFlag, const Standard_Boolean IFlag,
                                        const Standard_Boolean SkipShared) {
    TopExp_Explorer anExp;
...
Standard_Real BRepGProp::VolumePropertiesGK(const TopoDS_Shape& S, GProp_GProps& Props, const gp_Pln& thePln,
                                            const Standard_Real Eps, const Standard_Boolean OnlyClosed,
                                            const Standard_Boolean IsUseSpan, const Standard_Boolean CGFlag,
