...
void BRepGProp_Face::Bounds(Standard_Real& U1, Standard_Real& U2, Standard_Real& V1, Standard_Real& V2) const {
...
void BRepGProp_Face::Normal(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& VNor) const {
    gp_Vec D1U, D1V;
...
static void GetRealKnots(const Standard_Real theMin, const Standard_Real theMax,
                         const Handle(TColStd_HArray1OfReal) & theKnots, Handle(TColStd_HArray1OfReal) & theRealKnots) {
    ...
    while (++i < iU) {
        if (aStartI == 0 && theKnots->Value(i) > theMin + aTol) aStartI = i;

        if (aEndI == 0 && theKnots->Value(i + 1) > theMax - aTol) aEndI = i;

        if (aStartI != 0 && aEndI != 0) break;
    }

    if (aStartI == 0) aStartI = iU;
...
static void GetCurveKnots(const Standard_Real theMin, const Standard_Real theMax, const Geom2dAdaptor_Curve& theCurve,
...
void BRepGProp_Face::GetUKnots(const Standard_Real theUMin, const Standard_Real theUMax,
...
void BRepGProp_Face::GetTKnots(const Standard_Real theTMin, const Standard_Real theTMax,
