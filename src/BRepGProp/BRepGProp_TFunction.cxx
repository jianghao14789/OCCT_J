...
// purpose  :
//=======================================================================
BRepGProp_TFunction::BRepGProp_TFunction(const BRepGProp_Face& theSurface, const gp_Pnt& theVertex,
    ...
    : mySurface(theSurface), myUFunction(mySurface, theVertex, IsByPoint, theCoeffs), myUMin(theUMin),
      myTolerance(theTolerance), myTolReached(0.), myErrReached(0.), myAbsError(0.), myValueType(GProp_Unknown),
      myIsByPoint(IsByPoint), myNbPntOuter(3) {}
...
Standard_Boolean BRepGProp_TFunction::Value(const Standard_Real X, Standard_Real& F) {
    ...
    while (i < iU) {
        ...
        if (!anIntegral.IsDone()) return Standard_False;
