...
BRepGProp_Gauss::Inertia::Inertia()
    : Mass(0.0), Ix(0.0), Iy(0.0), Iz(0.0), Ixx(0.0), Iyy(0.0), Izz(0.0), Ixy(0.0), Ixz(0.0), Iyz(0.0) {}
...
// purpose  : Constructor
//=======================================================================
BRepGProp_Gauss::BRepGProp_Gauss(const BRepGProp_GaussType theType) : myType(theType) {
    add = (::Add);
    mult = (::Mult);
}
...
// purpose  :
//=======================================================================
Standard_Integer BRepGProp_Gauss::MaxSubs(const Standard_Integer theN, const Standard_Integer theCoeff) {
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::Init(NCollection_Handle<math_Vector>& theOutVec, const Standard_Real theValue,
                           const Standard_Integer theFirst, const Standard_Integer theLast) {
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::InitMass(const Standard_Real theValue, const Standard_Integer theFirst,
                               const Standard_Integer theLast, InertiaArray& theArray) {
    if (theArray.IsNull()) return;
...
// purpose  :
//=======================================================================
Standard_Integer BRepGProp_Gauss::FillIntervalBounds(const Standard_Real theA, const Standard_Real theB,
                                                     const TColStd_Array1OfReal& theKnots,
                                                     const Standard_Integer theNumSubs, InertiaArray& theInerts,
    ...
    if (aSize - 1 > theParam1->Upper()) {
        ...
        if (theCommonError.IsNull() == Standard_False) theCommonError = new math_Vector(1, aSize, 0.0);
    }
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::computeVInertiaOfElementaryPart(const gp_Pnt& thePoint, const gp_Vec& theNormal,
                                                      const gp_Pnt& theLocation, const Standard_Real theWeight,
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::computeSInertiaOfElementaryPart(const gp_Pnt& thePoint, const gp_Vec& theNormal,
                                                      const gp_Pnt& theLocation, const Standard_Real theWeight,
                                                      BRepGProp_Gauss::Inertia& theOutInertia) {
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::checkBounds(const Standard_Real theU1, const Standard_Real theU2, const Standard_Real theV1,
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::multAndRestoreInertia(const Standard_Real theValue, BRepGProp_Gauss::Inertia& theInOutInertia) {
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::convert(const BRepGProp_Gauss::Inertia& theInertia, gp_Pnt& theOutGravityCenter,
                              gp_Mat& theOutMatrixOfInertia, Standard_Real& theOutMass) {
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::convert(const BRepGProp_Gauss::Inertia& theInertia, const Standard_Real theCoeff[],
                              const Standard_Boolean theIsByPoint, gp_Pnt& theOutGravityCenter,
                              gp_Mat& theOutMatrixOfInertia, Standard_Real& theOutMass) {
...
// purpose  :
//=======================================================================
Standard_Real BRepGProp_Gauss::Compute(BRepGProp_Face& theSurface, BRepGProp_Domain& theDomain,
                                       const gp_Pnt& theLocation, const Standard_Real theEps,
                                       const Standard_Real theCoeff[], const Standard_Boolean theIsByPoint,
                                       Standard_Real& theOutMass, gp_Pnt& theOutGravityCenter, gp_Mat& theOutInertia) {
    const Standard_Boolean isErrorCalculation = (0.0 > theEps || theEps < 0.001) ? Standard_True : Standard_False;
    const Standard_Boolean isVerifyComputation = (0.0 < theEps && theEps < 0.001) ? Standard_True : Standard_False;
    ...
    while (isNaturalRestriction || theDomain.More()) {
        ...
        const Standard_Integer aNbLSubs = isNaturalRestriction ? theSurface.SVIntSubs() : theSurface.LIntSubs();
                                        ...
                                        for (kU = 0; kU < kUEnd; ++kU) {
                                            ...
                                            for (Standard_Integer iGU = 0; iGU < aLength; ++iGU) {
                                                for (Standard_Integer iU = 1; iU <= NbUGaussP[iGU]; ++iU) {
                                                    ...
                                                    if (myType == Vinert) {
                                                        computeVInertiaOfElementaryPart(aPoint, aNormal, theLocation, w,
                                                                                        theCoeff, theIsByPoint,
                                                                                        aLocal[iGU]);
                                                    } else {
                                                        if (iGU > 0)
...
#ifndef IS_MIN_DIM
            {
                if (myType == Vinert) Eps = ErrorLMax / (Abs(anInertia.Ixx) + Abs(anInertia.Iyy) + Abs(anInertia.Izz));
            }
#endif
        } else {
            Eps = 0.0;
...
//=======================================================================
Standard_Real BRepGProp_Gauss::Compute(BRepGProp_Face& theSurface, BRepGProp_Domain& theDomain,
                                       const gp_Pnt& theLocation, const Standard_Real theEps, Standard_Real& theOutMass,
                                       gp_Pnt& theOutGravityCenter, gp_Mat& theOutInertia) {
    Standard_ASSERT_RAISE(myType == Sinert, "BRepGProp_Gauss: Incorrect type");

    return Compute(theSurface, theDomain, theLocation, theEps, NULL, Standard_True, theOutMass, theOutGravityCenter,
                   theOutInertia);
}
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::Compute(BRepGProp_Face& theSurface, BRepGProp_Domain& theDomain, const gp_Pnt& theLocation,
                              Standard_Real& theOutMass, gp_Pnt& theOutGravityCenter, gp_Mat& theOutInertia) {
    Standard_ASSERT_RAISE(myType == Sinert, "BRepGProp_Gauss: Incorrect type");
    ...
    const Standard_Integer NbUGaussgp_Pnts = Min(theSurface.UIntegrationOrder(), math::GaussPointsMax());

    const Standard_Integer NbVGaussgp_Pnts = Min(theSurface.VIntegrationOrder(), math::GaussPointsMax());
    ...
    for (; theDomain.More(); theDomain.Next()) {
        ...
        for (Standard_Integer i = 1; i <= aNbGaussgp_Pnts; ++i) {
            ...
            for (Standard_Integer j = 1; j <= aNbGaussgp_Pnts; ++j) {
                ...
                computeVInertiaOfElementaryPart(aPoint, aNormal, theLocation, aWeight, theCoeff, theIsByPoint,
                                                aLocalInertia);
            }
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::Compute(const BRepGProp_Face& theSurface, const gp_Pnt& theLocation,
                              const Standard_Real theCoeff[], const Standard_Boolean theIsByPoint,
                              Standard_Real& theOutMass, gp_Pnt& theOutGravityCenter, gp_Mat& theOutInertia) {
    ...
    for (Standard_Integer j = 1; j <= VOrder; ++j) {
        ...
        for (Standard_Integer i = 1; i <= UOrder; ++i) {
            ...
            {
                computeSInertiaOfElementaryPart(aPoint, aNormal, theLocation, aWeight, anInertiaOfElementaryPart);
            }
        }
...
// purpose  :
//=======================================================================
void BRepGProp_Gauss::Compute(const BRepGProp_Face& theSurface, const gp_Pnt& theLocation, Standard_Real& theOutMass,
                              gp_Pnt& theOutGravityCenter, gp_Mat& theOutInertia) {
    Standard_ASSERT_RAISE(myType == Sinert, "BRepGProp_Gauss: Incorrect type");

    Compute(theSurface, theLocation, NULL, Standard_True, theOutMass, theOutGravityCenter, theOutInertia);
}
