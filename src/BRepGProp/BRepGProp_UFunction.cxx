...
// purpose  :
//=======================================================================
BRepGProp_UFunction::BRepGProp_UFunction(const BRepGProp_Face& theSurface, const gp_Pnt& theVertex,
                                         const Standard_Boolean IsByPoint, const Standard_Real* theCoeffs)
    : mySurface(theSurface), myVertex(theVertex), myCoeffs(theCoeffs), myVParam(0.), myValueType(GProp_Unknown),
...
Standard_Boolean BRepGProp_UFunction::Value(const Standard_Real X, Standard_Real& F) {
    ...
    if (myValueType == GProp_CenterMassX || myValueType == GProp_CenterMassY || myValueType == GProp_CenterMassZ)
        return CenterMassValue(X, F);

    // Inertia computation
    if (myValueType == GProp_InertiaXX || myValueType == GProp_InertiaYY || myValueType == GProp_InertiaZZ ||
        myValueType == GProp_InertiaXY || myValueType == GProp_InertiaXZ || myValueType == GProp_InertiaYZ)
...
Standard_Real BRepGProp_UFunction::VolumeValue(const Standard_Real X, gp_XYZ& thePMP0, Standard_Real& theS,
                                               Standard_Real& theD1) {
    gp_Pnt aPnt;
    gp_Vec aNorm;
    ...
    if (myIsByPoint) return thePMP0.Dot(aNorm.XYZ());
...
Standard_Boolean BRepGProp_UFunction::InertiaValue(const Standard_Real X, Standard_Real& F) {
    ...
    if (myValueType == GProp_InertiaXY || myValueType == GProp_InertiaYZ || myValueType == GProp_InertiaXZ) {
        ...
        aPPar1 -= aCoeff1 * aD1;
        aPPar2 -= aCoeff2 * aD1;
        aParam1 = aPPar1 * aPPar2 * aD1 + (aPPar1 * aCoeff2 + aPPar2 * aCoeff1) * aD2 + aCoeff1 * aCoeff2 * aD3;
