...
public:
    ...
    Standard_EXPORT const uint64_t& Position() {
        return myPosition;
    }
    ...
    Standard_EXPORT void PutBools(const Standard_Boolean theValue1, const Standard_Boolean theValue2,
                                  const Standard_Boolean theValue3);

    //! Writes 7 booleans as one byte to the stream.
    Standard_EXPORT void PutBools(const Standard_Boolean theValue1, const Standard_Boolean theValue2,
                                  const Standard_Boolean theValue3, const Standard_Boolean theValue4,
                                  const Standard_Boolean theValue5, const Standard_Boolean theValue6,
                                  const Standard_Boolean theValue7);

private:
    Standard_OStream* myStream; ///< pointer to the stream
    uint64_t myPosition; ///< the current position relatively to the OStream position at the moment of creation of this
                         ///< class instance
