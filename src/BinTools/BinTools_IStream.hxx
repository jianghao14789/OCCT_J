...
public:
    ...
    Standard_EXPORT const BinTools_ObjectType& LastType() {
        return myLastType;
    }
    ...
    Standard_EXPORT uint64_t Position() {
        return myPosition;
    }
    ...
    Standard_EXPORT Standard_IStream& Stream() {
        return *myStream;
    }
    //! Makes up to date the myPosition because myStream was used outside and position is changed.
    Standard_EXPORT void UpdatePosition() {
        myPosition = uint64_t(myStream->tellg());
    }
    ...
    Standard_EXPORT Standard_Boolean ReadBool() {
        return ReadByte() != 0;
    }
