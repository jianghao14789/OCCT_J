...
GeomAbs_Shape LocalContinuity(Standard_Integer Degree, Standard_Integer Nb, TColStd_Array1OfReal& TK,
                              TColStd_Array1OfInteger& TM, Standard_Real PFirst, Standard_Real PLast,
    ...
    if ((IsPeriodic) && (Index1 == Nb)) Index1 = 1;
...
void GeomAdaptor_Surface::load(const Handle(Geom_Surface) & S, const Standard_Real UFirst, const Standard_Real ULast,
                               const Standard_Real VFirst, const Standard_Real VLast, const Standard_Real TolU,
    ...
    if (mySurface != S) {
        ...
        if (TheType == STANDARD_TYPE(Geom_RectangularTrimmedSurface)) {
            Load(Handle(Geom_RectangularTrimmedSurface)::DownCast(S)->BasisSurface(), UFirst, ULast, VFirst, VLast);
            ...
            Handle(Geom_SurfaceOfRevolution) myRevSurf = Handle(Geom_SurfaceOfRevolution)::DownCast(mySurface);
            ...
            Handle(Adaptor3d_Curve) aBaseAdaptor = new GeomAdaptor_Curve(aBaseCurve);
            // Create corresponding evaluator
            myNestedEvaluator = new GeomEvaluator_SurfaceOfExtrusion(aBaseAdaptor, myExtSurf->Direction());
            ...
            myNestedEvaluator =
                new GeomEvaluator_OffsetSurface(aBaseAdaptor, myOffSurf->Offset(), myOffSurf->OsculatingSurface());
        } else
...
GeomAbs_Shape GeomAdaptor_Surface::UContinuity() const {
    switch (mySurfaceType) {
        case GeomAbs_BSplineSurface: {
            ...
            return LocalContinuity(myBSplineSurface->UDegree(), myBSplineSurface->NbUKnots(), TK, TM, myUFirst, myULast,
                                   IsUPeriodic());
        }
...
GeomAbs_Shape GeomAdaptor_Surface::VContinuity() const {
    switch (mySurfaceType) {
        case GeomAbs_BSplineSurface: {
            ...
            return LocalContinuity(myBSplineSurface->VDegree(), myBSplineSurface->NbVKnots(), TK, TM, myVFirst, myVLast,
                                   IsVPeriodic());
        }
        case GeomAbs_OffsetSurface: {
    ...
    switch (mySurfaceType) {
        case GeomAbs_BSplineSurface: {
            GeomAdaptor_Curve myBasisCurve(
                myBSplineSurface->VIso(myBSplineSurface->VKnot(myBSplineSurface->FirstVKnotIndex())), myUFirst,
                myULast);
        ...
        case GeomAbs_SurfaceOfExtrusion: {
            ...
            if (myBasisCurve.GetType() == GeomAbs_BSplineCurve) return myBasisCurve.NbIntervals(S);
            break;
        }
        case GeomAbs_OffsetSurface: {
...
Standard_Integer GeomAdaptor_Surface::NbVIntervals(const GeomAbs_Shape S) const {
    switch (mySurfaceType) {
        case GeomAbs_BSplineSurface: {
            GeomAdaptor_Curve myBasisCurve(
                myBSplineSurface->UIso(myBSplineSurface->UKnot(myBSplineSurface->FirstUKnotIndex())), myVFirst,
                myVLast);
            return myBasisCurve.NbIntervals(S);
        }
        case GeomAbs_SurfaceOfRevolution: {
...
void GeomAdaptor_Surface::UIntervals(TColStd_Array1OfReal& T, const GeomAbs_Shape S) const {
    ...
    switch (mySurfaceType) {
        ...
        case GeomAbs_SurfaceOfExtrusion: {
            GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurface)->BasisCurve(),
                                           myUFirst, myULast);
...
Handle(Adaptor3d_Surface) GeomAdaptor_Surface::UTrim(const Standard_Real First, const Standard_Real Last,
                                                     const Standard_Real Tol) const {
    return Handle(GeomAdaptor_Surface)(new GeomAdaptor_Surface(mySurface, First, Last, myVFirst, myVLast, Tol, myTolV));
}
...
Handle(Adaptor3d_Surface) GeomAdaptor_Surface::VTrim(const Standard_Real First, const Standard_Real Last,
                                                     const Standard_Real Tol) const {
    return Handle(GeomAdaptor_Surface)(new GeomAdaptor_Surface(mySurface, myUFirst, myULast, First, Last, myTolU, Tol));
}
...
Standard_Boolean GeomAdaptor_Surface::IsUClosed() const {
    if (!mySurface->IsUClosed()) return Standard_False;

    Standard_Real U1, U2, V1, V2;
    mySurface->Bounds(U1, U2, V1, V2);
    if (mySurface->IsUPeriodic()) return (Abs(Abs(U1 - U2) - Abs(myUFirst - myULast)) < Precision::PConfusion());

    return (Abs(U1 - myUFirst) < Precision::PConfusion() && Abs(U2 - myULast) < Precision::PConfusion());
}
...
Standard_Boolean GeomAdaptor_Surface::IsVClosed() const {
    if (!mySurface->IsVClosed()) return Standard_False;

    Standard_Real U1, U2, V1, V2;
    mySurface->Bounds(U1, U2, V1, V2);
    if (mySurface->IsVPeriodic()) return (Abs(Abs(V1 - V2) - Abs(myVFirst - myVLast)) < Precision::PConfusion());

    return (Abs(V1 - myVFirst) < Precision::PConfusion() && Abs(V2 - myVLast) < Precision::PConfusion());
}
...
void GeomAdaptor_Surface::RebuildCache(const Standard_Real theU, const Standard_Real theV) const {
    if (mySurfaceType == GeomAbs_BezierSurface) {
        ...
        if (mySurfaceCache.IsNull())
            mySurfaceCache = new BSplSLib_Cache(aDegU, aBezier->IsUPeriodic(), aFlatKnotsU, aDegV,
                                                aBezier->IsVPeriodic(), aFlatKnotsV, aBezier->Weights());
        mySurfaceCache->BuildCache(theU, theV, aFlatKnotsU, aFlatKnotsV, aBezier->Poles(), aBezier->Weights());
    } else if (mySurfaceType == GeomAbs_BSplineSurface) {
        // Create cache for B-spline
        if (mySurfaceCache.IsNull())
            mySurfaceCache = new BSplSLib_Cache(myBSplineSurface->UDegree(), myBSplineSurface->IsUPeriodic(),
                                                myBSplineSurface->UKnotSequence(), myBSplineSurface->VDegree(),
                                                myBSplineSurface->IsVPeriodic(), myBSplineSurface->VKnotSequence(),
...
void GeomAdaptor_Surface::D0(const Standard_Real U, const Standard_Real V, gp_Pnt& P) const {
    switch (mySurfaceType) {
        case GeomAbs_BezierSurface:
        case GeomAbs_BSplineSurface:
            if (mySurfaceCache.IsNull() || !mySurfaceCache->IsCacheValid(U, V)) RebuildCache(U, V);
...
void GeomAdaptor_Surface::D1(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V) const {
...
void GeomAdaptor_Surface::D2(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
...
void GeomAdaptor_Surface::D3(const Standard_Real U, const Standard_Real V, gp_Pnt& P, gp_Vec& D1U, gp_Vec& D1V,
        ...
        case GeomAbs_BSplineSurface: {
            if ((USide == 0) && (VSide == 0))
                ...
                if (IfUVBound(u, v, Ideb, Ifin, IVdeb, IVfin, USide, VSide))
                    myBSplineSurface->LocalD3(u, v, Ideb, Ifin, IVdeb, IVfin, P, D1U, D1V, D2U, D2V, D2UV, D3U, D3V,
                                              D3UUV, D3UVV);
                else
                    myBSplineSurface->D3(u, v, P, D1U, D1V, D2U, D2V, D2UV, D3U, D3V, D3UUV, D3UVV);
            }
...
gp_Vec GeomAdaptor_Surface::DN(const Standard_Real U, const Standard_Real V, const Standard_Integer Nu,
...
Standard_Real GeomAdaptor_Surface::UResolution(const Standard_Real R3d) const {
    ...
    switch (mySurfaceType) {
        ...
        case GeomAbs_Torus: {
            ...
            if (R > Precision::Confusion()) Res = R3d / (2. * R);
            break;
        }
        case GeomAbs_Sphere: {
...
Standard_Real GeomAdaptor_Surface::VResolution(const Standard_Real R3d) const {
    ...
    switch (mySurfaceType) {
        case GeomAbs_SurfaceOfRevolution: {
            GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfRevolution)::DownCast(mySurface)->BasisCurve(),
                                           myUFirst, myULast);
            return myBasisCurve.Resolution(R3d);
        }
        case GeomAbs_Torus: {
...
gp_Pln GeomAdaptor_Surface::Plane() const {
    if (mySurfaceType != GeomAbs_Plane) throw Standard_NoSuchObject("GeomAdaptor_Surface::Plane");
...
gp_Cylinder GeomAdaptor_Surface::Cylinder() const {
    if (mySurfaceType != GeomAbs_Cylinder) throw Standard_NoSuchObject("GeomAdaptor_Surface::Cylinder");
...
gp_Cone GeomAdaptor_Surface::Cone() const {
    if (mySurfaceType != GeomAbs_Cone) throw Standard_NoSuchObject("GeomAdaptor_Surface::Cone");
...
gp_Sphere GeomAdaptor_Surface::Sphere() const {
    if (mySurfaceType != GeomAbs_Sphere) throw Standard_NoSuchObject("GeomAdaptor_Surface::Sphere");
...
gp_Torus GeomAdaptor_Surface::Torus() const {
    if (mySurfaceType != GeomAbs_Torus) throw Standard_NoSuchObject("GeomAdaptor_Surface::Torus");
...
Standard_Integer GeomAdaptor_Surface::UDegree() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->UDegree();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->UDegree();
...
Standard_Integer GeomAdaptor_Surface::NbUPoles() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->NbUPoles();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->NbUPoles();
    if (mySurfaceType == GeomAbs_SurfaceOfExtrusion) {
        GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurface)->BasisCurve(),
                                       myUFirst, myULast);
...
Standard_Integer GeomAdaptor_Surface::VDegree() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->VDegree();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->VDegree();
    if (mySurfaceType == GeomAbs_SurfaceOfRevolution) {
        GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfRevolution)::DownCast(mySurface)->BasisCurve(), myUFirst,
                                       myULast);
        return myBasisCurve.Degree();
    }
...
Standard_Integer GeomAdaptor_Surface::NbVPoles() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->NbVPoles();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->NbVPoles();
    if (mySurfaceType == GeomAbs_SurfaceOfRevolution) {
        GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfRevolution)::DownCast(mySurface)->BasisCurve(), myUFirst,
                                       myULast);
        return myBasisCurve.NbPoles();
...
Standard_Integer GeomAdaptor_Surface::NbUKnots() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->NbUKnots();
    if (mySurfaceType == GeomAbs_SurfaceOfExtrusion) {
        GeomAdaptor_Curve myBasisCurve(Handle(Geom_SurfaceOfLinearExtrusion)::DownCast(mySurface)->BasisCurve(),
                                       myUFirst, myULast);
...
Standard_Integer GeomAdaptor_Surface::NbVKnots() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->NbVKnots();
...
Standard_Boolean GeomAdaptor_Surface::IsURational() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->IsURational();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->IsURational();
...
Standard_Boolean GeomAdaptor_Surface::IsVRational() const {
    if (mySurfaceType == GeomAbs_BSplineSurface) return myBSplineSurface->IsVRational();
    if (mySurfaceType == GeomAbs_BezierSurface) return Handle(Geom_BezierSurface)::DownCast(mySurface)->IsVRational();
    return Standard_False;
}
...
Handle(Geom_BezierSurface) GeomAdaptor_Surface::Bezier() const {
    if (mySurfaceType != GeomAbs_BezierSurface) throw Standard_NoSuchObject("GeomAdaptor_Surface::Bezier");
...
Handle(Geom_BSplineSurface) GeomAdaptor_Surface::BSpline() const {
    if (mySurfaceType != GeomAbs_BSplineSurface) throw Standard_NoSuchObject("GeomAdaptor_Surface::BSpline");
...
gp_Dir GeomAdaptor_Surface::Direction() const {
    if (mySurfaceType != GeomAbs_SurfaceOfExtrusion) throw Standard_NoSuchObject("GeomAdaptor_Surface::Direction");
...
Handle(Adaptor3d_Surface) GeomAdaptor_Surface::BasisSurface() const {
    if (mySurfaceType != GeomAbs_OffsetSurface) throw Standard_NoSuchObject("GeomAdaptor_Surface::BasisSurface");
    return new GeomAdaptor_Surface(Handle(Geom_OffsetSurface)::DownCast(mySurface)->BasisSurface(), myUFirst, myULast,
                                   myVFirst, myVLast);
}
...
Standard_Boolean GeomAdaptor_Surface::IfUVBound(const Standard_Real U, const Standard_Real V, Standard_Integer& IOutDeb,
                                                Standard_Integer& IOutFin, Standard_Integer& IOutVDeb,
                                                Standard_Integer& IOutVFin, const Standard_Integer USide,
                                                const Standard_Integer VSide) const {
    Standard_Integer Ideb, Ifin;
    Standard_Integer anUFKIndx = myBSplineSurface->FirstUKnotIndex(), anULKIndx = myBSplineSurface->LastUKnotIndex(),
...
void GeomAdaptor_Surface::Span(const Standard_Integer Side, const Standard_Integer Ideb, const Standard_Integer Ifin,
                               Standard_Integer& OutIdeb, Standard_Integer& OutIfin, const Standard_Integer theFKIndx,
