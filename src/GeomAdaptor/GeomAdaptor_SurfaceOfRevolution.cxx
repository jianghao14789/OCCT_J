...
// purpose  :
//=======================================================================
GeomAdaptor_SurfaceOfRevolution::GeomAdaptor_SurfaceOfRevolution() : myHaveAxis(Standard_False) {}
...
GeomAdaptor_SurfaceOfRevolution::GeomAdaptor_SurfaceOfRevolution(const Handle(Adaptor3d_Curve) & C, const gp_Ax1& V)
    : myHaveAxis(Standard_False) {
    Load(C);
    Load(V);
}
...
void GeomAdaptor_SurfaceOfRevolution::Load(const gp_Ax1& V) {
    ...
    myNestedEvaluator = new GeomEvaluator_SurfaceOfRevolution(myBasisCurve, myAxis.Direction(), myAxis.Location());
...
void GeomAdaptor_SurfaceOfRevolution::VIntervals(TColStd_Array1OfReal& T, const GeomAbs_Shape S) const {
...
Handle(Adaptor3d_Surface) GeomAdaptor_SurfaceOfRevolution::UTrim(const Standard_Real First, const Standard_Real Last,
...
Handle(Adaptor3d_Surface) GeomAdaptor_SurfaceOfRevolution::VTrim(const Standard_Real First, const Standard_Real Last,
                                                                 const Standard_Real Tol) const {
    ...
    switch (myBasisCurve->GetType()) {
        case GeomAbs_Line: {
            ...
            if (myAxis.IsParallel(Axe, TolAng)) {
                ...
                if (Abs(V.DotCross(AxisDir, W)) <= TolConf && (proj >= TolConeSemiAng && proj <= 1. - TolConeSemiAng)) {
                    return GeomAbs_Cone;
                }
            }
            break;
        ...
        case GeomAbs_Circle: {
            ...
            if (!C.Position().IsCoplanar(myAxis, TolConf, TolAng))
                ...
                if (MajorRadius > aR) {
                    ...
                    if (dX < 0.) dX = -dX;
                    if (dX < TolConf) return GeomAbs_Torus;
                }
            }
            break;
        }
...
gp_Pln GeomAdaptor_SurfaceOfRevolution::Plane() const {
    Standard_NoSuchObject_Raise_if(GetType() != GeomAbs_Plane, "GeomAdaptor_SurfaceOfRevolution:Plane");
    ...
    Axe.SetLocation(P);
    if (Axe.XDirection().Dot(myBasisCurve->Line().Direction()) >= -Precision::Confusion()) Axe.XReverse();
...
gp_Cylinder GeomAdaptor_SurfaceOfRevolution::Cylinder() const {
    Standard_NoSuchObject_Raise_if(GetType() != GeomAbs_Cylinder, "GeomAdaptor_SurfaceOfRevolution::Cylinder");
...
gp_Sphere GeomAdaptor_SurfaceOfRevolution::Sphere() const {
    Standard_NoSuchObject_Raise_if(GetType() != GeomAbs_Sphere, "GeomAdaptor_SurfaceOfRevolution:Sphere");
...
gp_Torus GeomAdaptor_SurfaceOfRevolution::Torus() const {
    Standard_NoSuchObject_Raise_if(GetType() != GeomAbs_Torus, "GeomAdaptor_SurfaceOfRevolution:Torus");
