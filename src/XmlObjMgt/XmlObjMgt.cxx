...
void XmlObjMgt::SetStringValue(XmlObjMgt_Element& theElement, const XmlObjMgt_DOMString& theData,
...
Standard_Boolean XmlObjMgt::GetTagEntryString(const XmlObjMgt_DOMString& theSource,
    ...
    if (strncmp(aSource, aRefPrefix, aPrefixSize)) return Standard_False;
    ...
    *aTagEntry = '0';
    ...
    while (aSource[0] != '\0') {
        ...
        if (strncmp(aSource, aRefElem1, anElem1Size)) return Standard_False;
        aSource += anElem1Size;
        const char aQuote = aSource[0];
        if (aQuote != '\'' && aQuote != '\"') return Standard_False;
        ...
        if (strncmp(aPtr + 1, aRefElem2, anElem2Size)) return Standard_False;
...
void XmlObjMgt::SetTagEntryString(XmlObjMgt_DOMString& theTarget, const TCollection_AsciiString& theTagEntry) {
    //    Begin parsing theTagEntry
    const char* aTagEntry = (const char*)theTagEntry.ToCString() + 1;
    if (aTagEntry[-1] != '0') return;
    ...
    const size_t anElem1Size = sizeof(aRefElem1) - 1;
    const size_t anElem2Size = sizeof(aRefElem2) - 1;
    char* aTarget = (char*)Standard::Allocate(sizeof(aRefPrefix) + aTagCount * (anElem1Size + anElem2Size + 12));
    ...
    for (;;) {
        ...
        if (aTagValue < 0 || aTagSize == 0 || errno == ERANGE || errno == EINVAL) return; // error
...
// purpose  :
//=======================================================================
XmlObjMgt_Element XmlObjMgt::FindChildElement(const XmlObjMgt_Element& theSource, const Standard_Integer theId) {
...
XmlObjMgt_Element XmlObjMgt::FindChildByRef(const XmlObjMgt_Element& theSource, const XmlObjMgt_DOMString& theRefName) {
    Standard_Integer anID;
    if (theSource.getAttribute(theRefName).GetInteger(anID)) return FindChildElement(theSource, anID);
    return LDOM_Element();
}
...
// purpose  :
//=======================================================================
XmlObjMgt_Element XmlObjMgt::FindChildByName(const XmlObjMgt_Element& theSource, const XmlObjMgt_DOMString& theName) {
...
Standard_Boolean XmlObjMgt::GetInteger(Standard_CString& theString, Standard_Integer& theValue) {
    ...
    if (ptr == theString || errno == ERANGE || errno == EINVAL) return Standard_False;
...
Standard_Boolean XmlObjMgt::GetReal(Standard_CString& theString, Standard_Real& theValue) {
    ...
    if (*ptr == '#') {
        if (!strncmp(ptr, "#QNAN", 5) || !strncmp(ptr, "#SNAN", 5)) {
            ...
            theString = ptr + 4;
            theValue =
                (theValue < 0 ? -std::numeric_limits<double>::infinity() : std::numeric_limits<double>::infinity());
            return Standard_True;
        } else
            return Standard_False;
