...
static void BRepPrim_OneAxis_Check(const Standard_Boolean V[], const Standard_Boolean E[], const Standard_Boolean W[],
...
BRepPrim_OneAxis::BRepPrim_OneAxis(const BRepPrim_Builder& B, const gp_Ax2& A, const Standard_Real VMin,
                                   const Standard_Real VMax)
    : myBuilder(B), myAxes(A), myAngle(2 * M_PI), myVMin(VMin), myVMax(VMax), myMeridianOffset(0)
...
const TopoDS_Shell& BRepPrim_OneAxis::Shell() {
    if (!ShellBuilt) {
        ...
        if (HasTop()) myBuilder.AddShellFace(myShell, TopFace());
        if (HasBottom()) myBuilder.AddShellFace(myShell, BottomFace());
        if (HasSides()) {
...
const TopoDS_Face& BRepPrim_OneAxis::LateralFace() {
    ...
    if (!FacesBuilt[FLATERAL]) {
        ...
        if (MeridianClosed()) {
            // closed edge
            myBuilder.SetPCurve(myEdges[ETOP], myFaces[FLATERAL], gp_Lin2d(gp_Pnt2d(0, myVMin), gp_Dir2d(1, 0)),
                                gp_Lin2d(gp_Pnt2d(0, myVMax), gp_Dir2d(1, 0)));
        } else {
            if (!VMaxInfinite()) {
                myBuilder.SetPCurve(myEdges[ETOP], myFaces[FLATERAL], gp_Lin2d(gp_Pnt2d(0, myVMax), gp_Dir2d(1, 0)));
            ...
            if (!VMinInfinite()) {
                myBuilder.SetPCurve(myEdges[EBOTTOM], myFaces[FLATERAL], gp_Lin2d(gp_Pnt2d(0, myVMin), gp_Dir2d(1, 0)));
...
const TopoDS_Face& BRepPrim_OneAxis::TopFace() {
    ...
    if (!FacesBuilt[FTOP]) {
        ...
        if (HasSides()) {
            myBuilder.SetPCurve(myEdges[ETOPSTART], myFaces[FTOP], gp_Lin2d(gp_Pnt2d(0, 0), gp_Dir2d(1, 0)));
...
const TopoDS_Face& BRepPrim_OneAxis::BottomFace() {
    ...
    if (!FacesBuilt[FBOTTOM]) {
        ...
        if (HasSides()) {
            myBuilder.SetPCurve(myEdges[EBOTSTART], myFaces[FBOTTOM], gp_Lin2d(gp_Pnt2d(0, 0), gp_Dir2d(1, 0)));
...
const TopoDS_Face& BRepPrim_OneAxis::StartFace() {
    ...
    if (!FacesBuilt[FSTART]) {
        ...
        if (VMaxInfinite() && VMinInfinite()) myBuilder.AddFaceWire(myFaces[FSTART], AxisStartWire());
        ...
        if (EdgesBuilt[EAXIS])
            myBuilder.SetPCurve(myEdges[EAXIS], myFaces[FSTART], gp_Lin2d(gp_Pnt2d(0, 0), gp_Dir2d(0, 1)));
...
const TopoDS_Face& BRepPrim_OneAxis::EndFace() {
    ...
    if (!FacesBuilt[FEND]) {
        ...
        if (VMaxInfinite() && VMinInfinite()) myBuilder.AddFaceWire(myFaces[FEND], AxisEndWire());
...
const TopoDS_Wire& BRepPrim_OneAxis::LateralWire() {
    ...
    if (!WiresBuilt[WLATERAL]) {
        ...
        if (!VMaxInfinite()) myBuilder.AddWireEdge(myWires[WLATERAL], TopEdge(), Standard_False);
        myBuilder.AddWireEdge(myWires[WLATERAL], EndEdge(), Standard_True);
        if (!VMinInfinite()) myBuilder.AddWireEdge(myWires[WLATERAL], BottomEdge(), Standard_True);
...
const TopoDS_Wire& BRepPrim_OneAxis::StartWire() {
    ...
    if (!WiresBuilt[WSTART]) {
        ...
        if (HasBottom()) myBuilder.AddWireEdge(myWires[WSTART], StartBottomEdge(), Standard_True);

        if (!MeridianClosed()) {
            if (!VMaxInfinite() || !VMinInfinite()) myBuilder.AddWireEdge(myWires[WSTART], AxisEdge(), Standard_False);
        }

        if (HasTop()) myBuilder.AddWireEdge(myWires[WSTART], StartTopEdge(), Standard_False);
...
const TopoDS_Wire& BRepPrim_OneAxis::AxisStartWire() {
    ...
    if (!WiresBuilt[WAXISSTART]) {
        ...
        Standard_DomainError_Raise_if(MeridianClosed(), "BRepPrim_OneAxes::AxisStartWire:meridian closed");
...
const TopoDS_Wire& BRepPrim_OneAxis::EndWire() {
    ...
    if (!WiresBuilt[WEND]) {
        ...
        if (HasTop()) myBuilder.AddWireEdge(myWires[WEND], EndTopEdge(), Standard_True);
        if (!MeridianClosed()) {
            if (!VMaxInfinite() || !VMinInfinite()) {
                myBuilder.AddWireEdge(myWires[WEND], AxisEdge(), Standard_True);
            }
        }
        if (HasBottom()) myBuilder.AddWireEdge(myWires[WEND], EndBottomEdge(), Standard_False);
...
const TopoDS_Wire& BRepPrim_OneAxis::AxisEndWire() {
    ...
    if (!WiresBuilt[WAXISEND]) {
        ...
        Standard_DomainError_Raise_if(!VMaxInfinite() || !VMinInfinite(), "BRepPrim_OneAxes::AxisEndWire:not infinite");

        Standard_DomainError_Raise_if(MeridianClosed(), "BRepPrim_OneAxes::AxisEndWire:meridian closed");
...
const TopoDS_Edge& BRepPrim_OneAxis::AxisEdge() {
    ...
    if (!EdgesBuilt[EAXIS]) {
        ...
        if (!VMaxInfinite())
            myBuilder.AddEdgeVertex(myEdges[EAXIS], AxisTopVertex(), MeridianValue(myVMax).Y(), Standard_False);
        if (!VMinInfinite())
            myBuilder.AddEdgeVertex(myEdges[EAXIS], AxisBottomVertex(), MeridianValue(myVMin).Y(), Standard_True);
...
const TopoDS_Edge& BRepPrim_OneAxis::StartEdge() {
    ...
    if (!EdgesBuilt[ESTART]) {
        ...
        if (!HasSides() && EdgesBuilt[EEND])
            ...
            if (MeridianClosed()) {
                // Closed edge
                myBuilder.AddEdgeVertex(myEdges[ESTART], TopStartVertex(), myVMin + myMeridianOffset,
                                        myVMax + myMeridianOffset);
            } else {
                if (!VMaxInfinite()) {
                    myBuilder.AddEdgeVertex(myEdges[ESTART], TopStartVertex(), myVMax + myMeridianOffset,
                                            Standard_False);
                }
                if (!VMinInfinite()) {
                    myBuilder.AddEdgeVertex(myEdges[ESTART], BottomStartVertex(), myVMin + myMeridianOffset,
                                            Standard_True);
                }
            }
        }
...
const TopoDS_Edge& BRepPrim_OneAxis::EndEdge() {
    ...
    if (!EdgesBuilt[EEND]) {
        ...
        if (!HasSides() && EdgesBuilt[ESTART])
            ...
            if (MeridianClosed()) {
                // Closed edge
                myBuilder.AddEdgeVertex(myEdges[EEND], TopEndVertex(), myVMin + myMeridianOffset,
                                        myVMax + myMeridianOffset);
            } else {
                if (!VMaxInfinite()) {
                    myBuilder.AddEdgeVertex(myEdges[EEND], TopEndVertex(), myVMax + myMeridianOffset, Standard_False);
                }
                if (!VMinInfinite()) {
                    myBuilder.AddEdgeVertex(myEdges[EEND], BottomEndVertex(), myVMin + myMeridianOffset, Standard_True);
                }
            }
        }
...
const TopoDS_Edge& BRepPrim_OneAxis::StartTopEdge() {
    ...
    if (!EdgesBuilt[ETOPSTART]) {
        ...
        Standard_DomainError_Raise_if(!HasTop() || !HasSides(), "BRepPrim_OneAxis::StartTopEdge:no sides or no top");
        ...
        myBuilder.AddEdgeVertex(myEdges[ETOPSTART], TopStartVertex(), MeridianValue(myVMax).X(), Standard_False);
...
const TopoDS_Edge& BRepPrim_OneAxis::StartBottomEdge() {
    ...
    if (!EdgesBuilt[EBOTSTART]) {
        ...
        myBuilder.AddEdgeVertex(myEdges[EBOTSTART], BottomStartVertex(), MeridianValue(myVMin).X(), Standard_False);
...
const TopoDS_Edge& BRepPrim_OneAxis::EndTopEdge() {
    ...
    if (!EdgesBuilt[ETOPEND]) {
        ...
        Standard_DomainError_Raise_if(!HasTop() || !HasSides(), "BRepPrim_OneAxis::EndTopEdge:no sides or no top");
        ...
        myBuilder.AddEdgeVertex(myEdges[ETOPEND], TopEndVertex(), MeridianValue(myVMax).X(), Standard_False);
...
const TopoDS_Edge& BRepPrim_OneAxis::EndBottomEdge() {
    ...
    if (!EdgesBuilt[EBOTEND]) {
        ...
        myBuilder.AddEdgeVertex(myEdges[EBOTEND], BottomEndVertex(), MeridianValue(myVMin).X(), Standard_False);
