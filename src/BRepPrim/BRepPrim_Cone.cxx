...
// purpose  :
//=======================================================================
BRepPrim_Cone::BRepPrim_Cone(const Standard_Real Angle, const gp_Ax2& Position, const Standard_Real Height,
                             const Standard_Real Radius)
    : BRepPrim_Revolution(Position, 0, 0), myHalfAngle(Angle), myRadius(Radius) {
    if (Height < Precision::Confusion()) throw Standard_DomainError("cone with null height");
    if (myHalfAngle * Height < Precision::Confusion()) throw Standard_DomainError("cone with null angle");
...
BRepPrim_Cone::BRepPrim_Cone(const Standard_Real Angle)
    : BRepPrim_Revolution(gp::XOY(), 0, RealLast()), myHalfAngle(Angle), myRadius(0.) {
    if ((Angle < 0) || (Angle > M_PI / 2)) throw Standard_DomainError("cone with angle <0 or > PI/2");
    VMin(0.);
    SetMeridian();
}
...
BRepPrim_Cone::BRepPrim_Cone(const Standard_Real Angle, const gp_Pnt& Apex)
    : BRepPrim_Revolution(gp_Ax2(Apex, gp_Dir(0, 0, 1), gp_Dir(1, 0, 0)), 0, RealLast()), myHalfAngle(Angle),
      myRadius(0.) {
    if ((Angle < 0) || (Angle > M_PI / 2)) throw Standard_DomainError("cone with angle <0 or > PI/2");
    VMin(0.);
    SetMeridian();
...
BRepPrim_Cone::BRepPrim_Cone(const Standard_Real Angle, const gp_Ax2& Axes)
    : BRepPrim_Revolution(Axes, 0, RealLast()), myHalfAngle(Angle) {
    if ((Angle < 0) || (Angle > M_PI / 2)) throw Standard_DomainError("cone with angle <0 or > PI/2");
    VMin(0.);
    SetMeridian();
}
...
BRepPrim_Cone::BRepPrim_Cone(const gp_Pnt& Center, const Standard_Real R1, const Standard_Real R2,
...
void BRepPrim_Cone::SetMeridian() {
    ...
    Handle(Geom2d_Line) L2d = new Geom2d_Line(gp_Pnt2d(myRadius, 0), gp_Dir2d(Sin(myHalfAngle), Cos(myHalfAngle)));
    Meridian(L, L2d);
}
...
void BRepPrim_Cone::SetParameters(const Standard_Real R1, const Standard_Real R2, const Standard_Real H) {
    ...
    if (Abs(R1 - R2) < Precision::Confusion()) throw Standard_DomainError("cone with two identic radii");
    if (H < Precision::Confusion()) throw Standard_DomainError("cone with negative or null height");
