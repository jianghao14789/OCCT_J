...
static const Standard_Integer tab[6][6] = {{-1, -1, 0, 1, 8, 9}, {-1, -1, 2, 3, 10, 11}, {0, 2, -1, -1, 4, 5},
                                           {1, 3, -1, -1, 6, 7}, {8, 10, 4, 6, -1, -1},  {9, 11, 5, 7, -1, -1}};
...
static Standard_Integer BRepPrim_Wedge_NumDir2(const BRepPrim_Direction d1, const BRepPrim_Direction d2) {
...
static Standard_Integer BRepPrim_Wedge_NumDir3(const BRepPrim_Direction d1, const BRepPrim_Direction d2,
                                               const BRepPrim_Direction d3) {
    Standard_Integer i1 = BRepPrim_Wedge_NumDir1(d1);
    Standard_Integer i2 = BRepPrim_Wedge_NumDir1(d2);
    Standard_Integer i3 = BRepPrim_Wedge_NumDir1(d3);
    if ((i1 / 2 == i2 / 2) || (i2 / 2 == i3 / 2) || (i3 / 2 == i1 / 2)) throw Standard_DomainError();
...
static void BRepPrim_Wedge_Check(const Standard_Boolean V[], const Standard_Boolean E[], const Standard_Boolean W[],
...
static void BRepPrim_Wedge_Init(Standard_Boolean& S, Standard_Boolean V[], Standard_Boolean E[], Standard_Boolean W[],
                                Standard_Boolean F[]) {
    Standard_Integer i;
    S = Standard_False;
    for (i = 0; i < NBVERTICES; i++)
...
BRepPrim_GWedge::BRepPrim_GWedge()
    : XMin(0), XMax(0), YMin(0), YMax(0), ZMin(0), ZMax(0), Z2Min(0), Z2Max(0), X2Min(0), X2Max(0) {
...
BRepPrim_GWedge::BRepPrim_GWedge(const BRepPrim_Builder& B, const gp_Ax2& Axes, const Standard_Real dx,
                                 const Standard_Real dy, const Standard_Real dz)
    : myBuilder(B), myAxes(Axes), XMin(0), XMax(dx), YMin(0), YMax(dy), ZMin(0), ZMax(dz), Z2Min(0), Z2Max(dz),
      X2Min(0), X2Max(dx) {
    for (Standard_Integer i = 0; i < NBFACES; i++) {
        myInfinite[i] = Standard_False;
    }
...
BRepPrim_GWedge::BRepPrim_GWedge(const BRepPrim_Builder& B, const gp_Ax2& Axes, const Standard_Real xmin,
                                 const Standard_Real ymin, const Standard_Real zmin, const Standard_Real z2min,
                                 const Standard_Real x2min, const Standard_Real xmax, const Standard_Real ymax,
                                 const Standard_Real zmax, const Standard_Real z2max, const Standard_Real x2max)
    : myBuilder(B), myAxes(Axes), XMin(xmin), XMax(xmax), YMin(ymin), YMax(ymax), ZMin(zmin), ZMax(zmax), Z2Min(z2min),
      Z2Max(z2max), X2Min(x2min), X2Max(x2max) {
    for (Standard_Integer i = 0; i < NBFACES; i++) {
        myInfinite[i] = Standard_False;
    }
...
const TopoDS_Shell& BRepPrim_GWedge::Shell() {
    ...
    if (!ShellBuilt) {
        ...
        if (HasFace(BRepPrim_XMin)) myBuilder.AddShellFace(myShell, Face(BRepPrim_XMin));
        if (HasFace(BRepPrim_XMax)) myBuilder.AddShellFace(myShell, Face(BRepPrim_XMax));
        if (HasFace(BRepPrim_YMin)) myBuilder.AddShellFace(myShell, Face(BRepPrim_YMin));
        if (HasFace(BRepPrim_YMax)) myBuilder.AddShellFace(myShell, Face(BRepPrim_YMax));
        if (HasFace(BRepPrim_ZMin)) myBuilder.AddShellFace(myShell, Face(BRepPrim_ZMin));
        if (HasFace(BRepPrim_ZMax)) myBuilder.AddShellFace(myShell, Face(BRepPrim_ZMax));
...
Standard_Boolean BRepPrim_GWedge::HasFace(const BRepPrim_Direction d1) const {
    ...
    if (d1 == BRepPrim_YMax) state = state && (Z2Max != Z2Min) && (X2Max != X2Min);
    return state;
}
...
const TopoDS_Face& BRepPrim_GWedge::Face(const BRepPrim_Direction d1) {
    ...
    if (!FacesBuilt[i]) {
        ...
        BRepPrim_Direction dd1 = BRepPrim_ZMin, dd2 = BRepPrim_YMax, dd3 = BRepPrim_ZMax, dd4 = BRepPrim_YMin;
        ...
        if (HasEdge(d1, dd4)) {
            ...
            DU = L.Direction() * DX;
            DV = L.Direction() * DY;
            myBuilder.SetPCurve(myEdges[BRepPrim_Wedge_NumDir2(d1, dd4)], myFaces[i],
        ...
        if (HasEdge(d1, dd3)) {
            ...
            DU = L.Direction() * DX;
            DV = L.Direction() * DY;
            myBuilder.SetPCurve(myEdges[BRepPrim_Wedge_NumDir2(d1, dd3)], myFaces[i],
                                gp_Lin2d(gp_Pnt2d(U, V), gp_Dir2d(DU, DV)));
        }
        ...
        if (HasEdge(d1, dd2)) {
            ...
            DU = L.Direction() * DX;
            DV = L.Direction() * DY;
            myBuilder.SetPCurve(myEdges[BRepPrim_Wedge_NumDir2(d1, dd2)], myFaces[i],
                                gp_Lin2d(gp_Pnt2d(U, V), gp_Dir2d(DU, DV)));
        }
        ...
        if (HasEdge(d1, dd1)) {
            ...
            DU = L.Direction() * DX;
            DV = L.Direction() * DY;
            myBuilder.SetPCurve(myEdges[BRepPrim_Wedge_NumDir2(d1, dd1)], myFaces[i],
                                gp_Lin2d(gp_Pnt2d(U, V), gp_Dir2d(DU, DV)));
        }
...
const TopoDS_Wire& BRepPrim_GWedge::Wire(const BRepPrim_Direction d1) {
    ...
    if (!WiresBuilt[i]) {
        ...
        if (HasEdge(d1, dd4)) myBuilder.AddWireEdge(myWires[i], Edge(d1, dd4), Standard_False);
        if (HasEdge(d1, dd3)) myBuilder.AddWireEdge(myWires[i], Edge(d1, dd3), Standard_False);
        if (HasEdge(d1, dd2)) myBuilder.AddWireEdge(myWires[i], Edge(d1, dd2), Standard_True);
        if (HasEdge(d1, dd1)) myBuilder.AddWireEdge(myWires[i], Edge(d1, dd1), Standard_True);
...
Standard_Boolean BRepPrim_GWedge::HasEdge(const BRepPrim_Direction d1, const BRepPrim_Direction d2) const {
    Standard_Boolean state = !(myInfinite[BRepPrim_Wedge_NumDir1(d1)] || myInfinite[BRepPrim_Wedge_NumDir1(d2)]);
...
const TopoDS_Edge& BRepPrim_GWedge::Edge(const BRepPrim_Direction d1, const BRepPrim_Direction d2) {
    ...
    if (!EdgesBuilt[i]) {
        ...
        if (HasVertex(d1, d2, dd2)) {
            myBuilder.AddEdgeVertex(myEdges[i], Vertex(d1, d2, dd2), ElCLib::Parameter(L, Point(d1, d2, dd2)),
                                    Standard_False);
        }
        if (HasVertex(d1, d2, dd1)) {
            myBuilder.AddEdgeVertex(myEdges[i], Vertex(d1, d2, dd1), ElCLib::Parameter(L, Point(d1, d2, dd1)),
                                    Standard_True);
        }
...
Standard_Boolean BRepPrim_GWedge::HasVertex(const BRepPrim_Direction d1, const BRepPrim_Direction d2,
...
const TopoDS_Vertex& BRepPrim_GWedge::Vertex(const BRepPrim_Direction d1, const BRepPrim_Direction d2,
                                             const BRepPrim_Direction d3) {
    if (!HasVertex(d1, d2, d3)) throw Standard_DomainError();
