...
// purpose  :
//=======================================================================
static void Propagate(const TopTools_IndexedDataMapOfShapeListOfShape& mapEF, const TopoDS_Shape& theFace,
...
BRepCheck_Shell::BRepCheck_Shell(const TopoDS_Shell& S)
    : myNbori(0), myCdone(Standard_False), myCstat(BRepCheck_NoError), myOdone(Standard_False),
...
Standard_Integer BRepCheck_Shell::NbConnectedSet(TopTools_ListOfShape& theSets) {
    ...
    for (iCur = 1; iCur <= parents.Extent(); iCur++) {
        ...
        if (Ed.Orientation() != TopAbs_REVERSED && Ed.Orientation() != TopAbs_FORWARD) theUnOriEd.Add(Ed);
    }
