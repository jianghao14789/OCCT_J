...
static void ChoixUV(const TopoDS_Vertex&, const TopoDS_Edge&, const TopoDS_Face&, TopTools_ListOfShape&);
...
static void CurveDirForParameter(const Geom2dAdaptor_Curve& aC2d, const Standard_Real aPrm, gp_Pnt2d& Pnt,
                                 gp_Vec2d& aVec2d);

//  Modified by Sergey KHROMOV - Thu Jun 20 11:21:51 2002 OCC325 Begin
static Standard_Boolean IsClosed2dForPeriodicFace(const TopoDS_Face& theFace, const gp_Pnt2d& theP1,
                                                  const gp_Pnt2d& theP2, const TopoDS_Vertex& theVertex);

static Standard_Boolean GetPnt2d(const TopoDS_Vertex& theVertex, const TopoDS_Edge& theEdge, const TopoDS_Face& theFace,
                                 gp_Pnt2d& aPnt);
...
BRepCheck_Wire::BRepCheck_Wire(const TopoDS_Wire& W)
    : myCdone(Standard_False), myCstat(BRepCheck_NoError), myGctrl(Standard_False) {
    Init(W);
}
//=======================================================================
// function : Minimum
...
//=======================================================================
Standard_Boolean IsDistanceIn3DTolerance(const gp_Pnt& thePnt_f, const gp_Pnt& thePnt_l, const Standard_Real aTol3d) {
    Standard_Real Dist = thePnt_f.Distance(thePnt_l);

    if (Dist < aTol3d) return Standard_True;
...
// purpose  :
//=======================================================================
static Standard_Boolean IsDistanceIn2DTolerance(const BRepAdaptor_Surface& aFaceSurface, const gp_Pnt2d& thePnt,
                                                const gp_Pnt2d& thePntRef, const Standard_Real aTol3d,
#ifdef OCCT_DEBUG
...
#endif
{
    ...
    if ((dumin < dumax) && (dvmin < dvmax)) return Standard_True;
    ...
    if (Dist < aTol2d) return Standard_True;
...
BRepCheck_Status BRepCheck_Wire::Closed2d(const TopoDS_Face& theFace, const Standard_Boolean Update) {
    ...
    for (; anEdgeExp.More(); anEdgeExp.Next()) {
        if (IsOriented(anEdgeExp.Current())) aNbOrirntedEdges++;
    }
    ...
    if (aLastEdge.Orientation() == TopAbs_REVERSED) aP_last = aP_temp;
    ...
    if (!(IsDistanceIn2DTolerance(aFaceSurface, aP_first, aP_last, aTol3d))) aClosedStat = BRepCheck_NotClosed;
...
BRepCheck_Status BRepCheck_Wire::Orientation(const TopoDS_Face& F, const Standard_Boolean Update) {
    ...
    for (exp.Init(myShape, TopAbs_EDGE); exp.More(); exp.Next()) {
        ...
        if (IsOriented(edg)) {
            ...
            if (VF.IsNull() && VL.IsNull()) theOstat = BRepCheck_InvalidDegeneratedFlag;
            break;
        }
    }
    ...
    if (theOstat == BRepCheck_NoError) {
        ...
        if (VL.IsNull()) isGoFwd = Standard_False;
        ...
        while (Index < nbOriNoDegen) {
                    ...
                    for (vte.Init(edg, TopAbs_VERTEX); vte.More(); vte.Next()) {
                        ...
                        if (!VL.IsNull()) {
                            if (vto == TopAbs_FORWARD && VL.IsSame(vte.Current())) {
                                ...
                                if (!F.IsNull() || !BRep_Tool::Degenerated(edg)) ledge.Append(edg);
                                break;
                            }
                        } else { // VF is not null
                            if (vto == TopAbs_REVERSED && VF.IsSame(vte.Current())) {
            ...
            // JAG 03/07   else if (nbconnex >= 2 && !F.IsNull())  // Try to see in 2d
                ...
                //         if (nbconnex == 1 && !CheckLoopOrientation( pivot, theEdge, TopoDS::Edge(ledge.First()), F,
                //         ListOfPassedEdge ))
                //         {
                //           theOstat = BRepCheck_BadOrientationOfSubshape;
                //           if (Update)
...
// purpose  :
//=======================================================================
BRepCheck_Status BRepCheck_Wire::SelfIntersect(const TopoDS_Face& F, TopoDS_Edge& retE1, TopoDS_Edge& retE2,
        ...
        if (Inter.IsDone()) {
            ...
            for (Standard_Integer p = 1; p <= nbp; p++) {
                ...
                if (Tr1.PositionOnCurve() == IntRes2d_Middle || Tr2.PositionOnCurve() == IntRes2d_Middle) {
                    ...
                    for (ExplVtx.Init(E1, TopAbs_VERTEX); localok == Standard_False && ExplVtx.More(); ExplVtx.Next()) {
                        gp_Pnt p3dvtt;
                    ...
                    if (localok == Standard_False) {
                        ...
                        std::cout << "point p" << ++numpoint << " " << P3d.X() << " " << P3d.Y() << " " << P3d.Z()
                                  << std::endl;
                        std::cout.flush();
#endif
                        return (BRepCheck_SelfIntersectingWire);
                ...
                for (Standard_Integer p = 1; p <= nbp; p++) {
                    ...
                    if (Tr1.PositionOnCurve() == IntRes2d_Middle || Tr2.PositionOnCurve() == IntRes2d_Middle) {
                        ...
                        if (localok == Standard_False) {
                            ...
                            std::cout << "point p" << ++numpoint1 << " " << P3d.X() << " " << P3d.Y() << " " << P3d.Z()
                                      << std::endl;
                            std::cout.flush();
#endif
                            delete[] tabDom;
...
//           contained in <mapVE>
//=======================================================================
static void Propagate(const TopTools_IndexedDataMapOfShapeListOfShape& mapVE, const TopoDS_Shape& edg,
                      TopTools_MapOfShape& mapE) {
    ...
    do {
        ...
        for (; itrc.More(); itrc.Next()) {
            ...
            if (!mapE.Contains(Edge)) mapE.Add(Edge);
...
//=======================================================================
void ChoixUV(const TopoDS_Vertex& theVertex, const TopoDS_Edge& theEdge, const TopoDS_Face& theFace,
    ...
    if (aVOrientation != anEdgOrientation) aDerRef.Reverse();
    ...
    for (; It.More(); It.Next()) {
        ...
        if (!IsDistanceIn2DTolerance(aFaceSurface, aPnt, aPntRef, aTol3d, Standard_False)) continue;

        CurveDirForParameter(aCA, aParam, aPnt, aDer);

        if (aVOrientation == anE.Orientation()) aDer.Reverse();
...
// purpose  :
//=======================================================================
void CurveDirForParameter(const Geom2dAdaptor_Curve& aC2d, const Standard_Real aPrm, gp_Pnt2d& Pnt, gp_Vec2d& aVec2d) {
...
static Standard_Boolean GetPnt2d(const TopoDS_Vertex& theVertex, const TopoDS_Edge& theEdge, const TopoDS_Face& theFace,
    ...
    if (!theVertex.IsSame(aFirstVtx) && !theVertex.IsSame(aLastVtx)) return Standard_False;

    aPCurve = BRep_Tool::CurveOnSurface(theEdge, theFace, aFPar, aLPar);

    if (aPCurve.IsNull()) return Standard_False;
...
static Standard_Boolean IsClosed2dForPeriodicFace(const TopoDS_Face& theFace, const gp_Pnt2d& theP1,
    ...
    for (; anExp.More(); anExp.Next()) {
        ...
        if (NotSeams.Contains(anEdge)) continue;
        ...
        if (!ClosedEdges.Add(anEdge)) aSeamEdges.Append(anEdge);
    }

    if (aSeamEdges.Extent() == 0) return Standard_True;
    ...
    for (; anIter.More(); anIter.Next()) {
        ...
        for (; anExp.More(); anExp.Next()) {
            ...
            if (IsOriented(aVtx) && aVtx.IsSame(theVertex)) {
                ...
                if (!GetPnt2d(theVertex, aSeamEdge, theFace, aPnt1)) continue;

                aSeamEdge = TopoDS::Edge(aSeamEdge.Reversed());

                if (!GetPnt2d(theVertex, aSeamEdge, theFace, aPnt2)) continue;

                a2dTol = aPnt1.Distance(aPnt2) * 1.e-2;
                a2dTol = Max(a2dTol, aVicinity);

                if (aDistP1P2 > a2dTol) return Standard_False;
            }
        }
    }
