...
static Standard_Boolean Intersect(const TopoDS_Wire&, const TopoDS_Wire&, const TopoDS_Face&,
                                  const DataMapOfShapeBox2d&);

static Standard_Boolean IsInside(const TopoDS_Wire& wir, const Standard_Boolean Inside,
                                 const BRepTopAdaptor_FClass2d& FClass2d, const TopoDS_Face& F);
...
BRepCheck_Face::BRepCheck_Face(const TopoDS_Face& F)
    : myIntres(BRepCheck_NoError), myImbres(BRepCheck_NoError), myOrires(BRepCheck_NoError) {
    Init(F);
...
static Standard_Boolean Intersect(const TopoDS_Wire& wir1, const TopoDS_Wire& wir2, const TopoDS_Face& F,
    ...
    for (exp2.Init(wir2, TopAbs_VERTEX); exp2.More(); exp2.Next()) {
        ...
        if (MapW1.Contains(V)) CommonVertices.Append(V);
    }
        ...
        for (exp2.Init(wir2, TopAbs_EDGE); exp2.More(); exp2.Next()) {
            ...
            if (!edg1.IsSame(edg2)) {
                ...
                if (!Box1.IsOut(Box2)) {
                    ...
                    if (!Inter.IsDone()) return Standard_True;
                    if (Inter.NbSegments() > 0) {
                        if (PntSeq.IsEmpty())
                            ...
                            if (NbCoinc == Inter.NbSegments()) return Standard_False;
                            return Standard_True;
                        }
                    }
                    if (Inter.NbPoints() > 0) {
                        if (PntSeq.IsEmpty())
                            ...
                            if (NbCoinc == Inter.NbPoints()) return Standard_False;
                            return Standard_True;
                        }
                    }
                }
...
static Standard_Boolean IsInside(const TopoDS_Wire& theWire, const Standard_Boolean WireBienOriente,
                                 const BRepTopAdaptor_FClass2d& FClass2d, const TopoDS_Face& theFace) {
...
Standard_Boolean CheckThin(const TopoDS_Shape& w, const TopoDS_Shape& f) {
    ...
    if (v1.IsNull() || v2.IsNull() || v3.IsNull() || v4.IsNull()) return Standard_False;

    if (v1.IsSame(v2) || v3.IsSame(v4)) return Standard_False;
