...
public:
    ...
    static Standard_EXPORT Handle(TObj_TReference)
        Set(const TDF_Label& theLabel, const Handle(TObj_Object) & theObject, const Handle(TObj_Object) & theMaster);
...
public:
    ...
    virtual Standard_EXPORT Standard_Boolean BeforeUndo(const Handle(TDF_AttributeDelta) & theDelta,
                                                        const Standard_Boolean isForced = Standard_False)
        Standard_OVERRIDE;

    //! It is necessary for tranzaction mechanism (Undo/Redo).
    virtual Standard_EXPORT Standard_Boolean AfterUndo(const Handle(TDF_AttributeDelta) & theDelta,
                                                       const Standard_Boolean isForced = Standard_False)
        Standard_OVERRIDE;
