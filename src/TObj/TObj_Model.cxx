...
void TObj_Model::CloseDocument(const Handle(TDocStd_Document) & theDoc) {
    ...
    if (theDoc->HasOpenCommand()) theDoc->AbortCommand();
...
Handle(TCollection_HExtendedString) TObj_Model::GetFile() const {
    ...
    return !aPath.IsEmpty() ? new TCollection_HExtendedString(aPath) : Handle(TCollection_HExtendedString)();
}
...
Standard_Boolean TObj_Model::Save() {
    ...
    if (aDoc.IsNull()) return Standard_False;

    if (!aDoc->GetPath().IsEmpty()) return SaveAs(aDoc->GetPath());
...
Standard_Boolean TObj_Model::Close() {
    // OCAF document
    TDF_Label aLabel = GetLabel();
    if (aLabel.IsNull()) return Standard_False;
...
Handle(TObj_Model) TObj_Model::GetDocumentModel(const TDF_Label& theLabel) {
    ...
    if (theLabel.IsNull()) return aModel;

    Handle(TDocStd_Document) aDoc;
    Handle(TDF_Data) aData = theLabel.Data();
    TDF_Label aRootL = aData->Root();
    if (aRootL.IsNull()) return aModel;
    Handle(TDocStd_Owner) aDocOwnerAtt;
    if (aRootL.FindAttribute(TDocStd_Owner::GetID(), aDocOwnerAtt)) aDoc = aDocOwnerAtt->GetDocument();

    if (aDoc.IsNull()) return aModel;

    TDF_Label aLabel = aDoc->Main();
    Handle(TObj_TModel) anAttr;
    if (aLabel.FindAttribute(TObj_TModel::GetID(), anAttr)) aModel = anAttr->Model();
...
Handle(TObj_ObjectIterator) TObj_Model::GetChildren() const {
    ...
    if (aMainPartition.IsNull()) return 0;
...
Handle(TObj_Object) TObj_Model::FindObject(const Handle(TCollection_HExtendedString) & theName,
    ...
    if (aDictionary.IsNull()) aDictionary = GetDictionary();
...
Standard_Boolean TObj_Model::IsRegisteredName(const Handle(TCollection_HExtendedString) & theName,
    ...
    if (aDictionary.IsNull()) return Standard_False;
...
void TObj_Model::RegisterName(const Handle(TCollection_HExtendedString) & theName, const TDF_Label& theLabel,
                              const Handle(TObj_TNameContainer) & theDictionary) const {
    Handle(TObj_TNameContainer) aDictionary = theDictionary;
    if (aDictionary.IsNull()) aDictionary = GetDictionary();

    if (!aDictionary.IsNull()) aDictionary->RecordName(theName, theLabel);
}
...
void TObj_Model::UnRegisterName(const Handle(TCollection_HExtendedString) & theName,
    ...
    if (!aDictionary.IsNull()) aDictionary->RemoveName(theName);
}
...
Handle(TObj_TNameContainer) TObj_Model::GetDictionary() const {
    ...
    TDF_Label aLabel = GetLabel();
    if (!aLabel.IsNull()) aLabel.FindAttribute(TObj_TNameContainer::GetID(), A);
    return A;
}
...
Handle(TObj_Partition) TObj_Model::getPartition(const TDF_Label& theLabel, const Standard_Boolean theHidden) const {
...
Handle(TObj_Partition) TObj_Model::getPartition(const TDF_Label& theLabel, const Standard_Integer theIndex,
    ...
    if (isNew) aPartition->SetName(new TCollection_HExtendedString(theName));
    return aPartition;
}
...
void TObj_Model::updateBackReferences(const Handle(TObj_Object) & theObject) {
    ...
    Handle(TObj_LabelIterator) anIter = Handle(TObj_LabelIterator)::DownCast(theObject->GetReferences());
    ...
    for (; anIter->More(); anIter->Next()) {
        ...
        if (!anObject.IsNull()) anObject->RemoveBackReference(theObject, Standard_False);
    }
    ...
    if (!anIter.IsNull())
        for (; anIter->More(); anIter->Next()) {
            Handle(TObj_Object) anObject = anIter->Value();
            if (!anObject.IsNull()) anObject->AddBackReference(theObject);
        }
}
...
Handle(TDocStd_Document) TObj_Model::GetDocument() const {
    ...
    TDF_Label aLabel = GetLabel();
    if (!aLabel.IsNull()) D = TDocStd_Document::Get(aLabel);
    return D;
}
...
Standard_Boolean TObj_Model::Paste(Handle(TObj_Model) theModel, Handle(TDF_RelocationTable) theRelocTable) {
