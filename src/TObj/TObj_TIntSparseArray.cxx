...
TObj_TIntSparseArray::TObj_TIntSparseArray() : myVector(100), myOldMap(100), myDoBackup(Standard_True) {}
...
void TObj_TIntSparseArray::SetValue(const Standard_Size theId, const Standard_Integer theValue) {
    ...
    if (theId < 1 || theValue < 1) throw Standard_OutOfRange("TObj_TIntSparseArray::SetValue");
    ...
    if (!aLabel.IsNull()) {
        ...
        if (myDoBackup && aMyTransaction < aCurrentTransaction) backupValue(theId, anOld, theValue);
    }
}
...
void TObj_TIntSparseArray::UnsetValue(const Standard_Size theId) {
    ...
    if (theId < 1) throw Standard_OutOfRange("TObj_TIntSparseArray::UnsetValue");
    ...
    if (!aLabel.IsNull()) {
        ...
        if (myDoBackup && aMyTransaction < aCurrentTransaction) backupValue(theId, anOld, AbsentValue);
    }
}
...
void TObj_TIntSparseArray::backupValue(const Standard_Size theId, const Standard_Integer theCurrValue,
    ...
    if (!myOldMap.IsBound(theId))
        ...
        if (aUData == theNewValue) myOldMap.UnBind(theId);
    }
}
...
Handle(TDF_Attribute) TObj_TIntSparseArray::BackupCopy() const {
    ...
    if (!myOldMap.IsEmpty()) aCopy->myOldMap.Exchange((TObj_TIntSparseArray_MapOfData&)myOldMap);
...
void TObj_TIntSparseArray::Paste(const Handle(TDF_Attribute) & theInto, const Handle(TDF_RelocationTable) &) const {
    Handle(TObj_TIntSparseArray) aInto = Handle(TObj_TIntSparseArray)::DownCast(theInto);
    if (aInto.IsNull()) return;
...
Standard_Boolean TObj_TIntSparseArray::AfterUndo(const Handle(TDF_AttributeDelta) &, const Standard_Boolean) {
