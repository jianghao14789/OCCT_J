...
public:
    ...
    static Standard_EXPORT Handle(TObj_Object)
        CreateNewObject(const Standard_CString theType, const TDF_Label& theLabel);
...
#define DECLARE_TOBJOCAF_PERSISTENCE(name, ancestor)                                                                   \
    name(const TObj_Persistence* p, const TDF_Label& aLabel) : ancestor(p, aLabel) {                                   \
        initFields();                                                                                                  \
