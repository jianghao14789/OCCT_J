...
Handle(TObj_Partition) TObj_Partition::Create(const TDF_Label& theLabel, const Standard_Boolean theSetName) {
...
Handle(TCollection_HExtendedString) TObj_Partition::GetNewName(const Standard_Boolean theIsToChangeCount) {
    ...
    if (theIsToChangeCount && --aRank > saveRank) SetLastIndex(aRank);
    return aName;
}
...
Handle(TObj_Partition) TObj_Partition::GetPartition(const Handle(TObj_Object) & theObject) {
    ...
    if (!theObject.IsNull()) {
        ...
        while (aPartition.IsNull() && !aLabel.IsNull()) {
            ...
            if (aPartition.IsNull()) aLabel = aLabel.Father();
        }
    }
    return aPartition;
}
