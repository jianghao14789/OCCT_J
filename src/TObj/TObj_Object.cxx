...
TObj_Object::TObj_Object(const TDF_Label& theLabel, const Standard_Boolean theSetName) : myLabel(theLabel) {
    Handle(TObj_Object) aMe = this;
    TObj_TObject::Set(myLabel, aMe);
    if (theSetName) TObj_Model::SetNewName(aMe);
}
...
static void addObjToOrderSequence(const Handle(TObj_Object) & theObj, const Standard_Integer theOrder,
                                  Handle(TObj_HSequenceOfObject) & theHSeqOfObj, const Standard_Integer theHSeqLength,
                                  Standard_Integer& theLastIndex, Standard_Integer& theLastOrder) {
...
Handle(TObj_ObjectIterator) TObj_Object::GetChildren(const Handle(Standard_Type) & theType) const {
    Handle(TObj_ObjectIterator) anItr = new TObj_OcafObjectIterator(GetChildLabel(), theType, Standard_True);
    if (!TestFlags(ObjectState_Ordered)) return anItr;
    ...
    for (; anItr->More(); anItr->Next()) {
        ...
        if (!aLastIndex) {
            ...
            addObjToOrderSequence(anObj, anOrder, aHSeqOfObj, aHSeqOfObj->Length(), aLastIndex, aLastOrder);
    }
...
#ifdef DFBROWSE
static TDF_Label getLabelByRank(const TDF_Label& theL, const Standard_Integer theRank, const Standard_CString theName) {
...
TDF_Label TObj_Object::getChildLabel(const Standard_Integer theRank) const {
    ...
    if (theRank > 0) aLabel = aLabel.FindChild(theRank, Standard_True);
    return aLabel;
}
...
Standard_Boolean TObj_Object::SetName(const Handle(TCollection_HExtendedString) & theName) const {
    ...
    if (!anOldName.IsNull() && theName->String().IsEqual(anOldName->String())) return Standard_True;
    ...
    if (aModel->IsRegisteredName(theName, aDictionary)) return Standard_False;

    // change name and update registry
    if (!anOldName.IsNull()) aModel->UnRegisterName(anOldName, aDictionary);
    if (theName.IsNull())
...
Standard_Boolean TObj_Object::HasReference(const Handle(TObj_Object) & theObject) const {
    ...
    if (anItr.IsNull() || !anItr->More()) return Standard_False;
    for (; anItr->More(); anItr->Next())
        if (anItr->Value() == theObject) return Standard_True;
...
void TObj_Object::AddBackReference(const Handle(TObj_Object) & theObject) {
    if (myHSeqBackRef.IsNull()) myHSeqBackRef = new TObj_HSequenceOfObject;
...
void TObj_Object::RemoveBackReference(const Handle(TObj_Object) & theObject, const Standard_Boolean theSingleOnly) {
    if (myHSeqBackRef.IsNull()) // to avoid exception.
        return;

    for (Standard_Integer i = 1; i <= myHSeqBackRef->Length(); i++) {
        if (theObject != myHSeqBackRef->Value(i)) continue;

        myHSeqBackRef->Remove(i--);
        if (theSingleOnly) break;
    }
    if (myHSeqBackRef->Length() < 1) myHSeqBackRef.Nullify(); // do not need to store empty sequence.
}
//=======================================================================
...
Handle(TObj_ObjectIterator) TObj_Object::GetBackReferences(const Handle(Standard_Type) & theType) const {
...
Standard_Boolean TObj_Object::CanDetach(const TObj_DeletingMode theMode) {
    if (!IsAlive()) return Standard_False;
    ...
    if (theMode == TObj_Forced) return Standard_True;

    // check the last KeepDepending mode
    Handle(TObj_Object) aMe = this;
    for (; aRefs->More(); aRefs->Next()) {
        Handle(TObj_Object) anObject = aRefs->Value();
        if (!anObject->CanRemoveReference(aMe)) return Standard_False; // one of objects could not be unlinked
    }
...
Standard_Boolean TObj_Object::Detach(const TObj_DeletingMode theMode) {
    ...
    if (!RemoveBackReferences(theMode)) return Standard_False;
    ...
    if (!anOldName.IsNull()) {
        ...
        if (!aDictionary.IsNull() && aDictionary->IsRegistered(anOldName)) {
            ...
            if (!aRegisteredLabel.IsNull() && aRegisteredLabel == GetLabel()) aDictionary->RemoveName(anOldName);
        }
    }
...
Standard_Boolean TObj_Object::Detach(const TDF_Label& theLabel, const TObj_DeletingMode theMode) {
    ...
    if (GetObj(theLabel, anObject)) return anObject->Detach(theMode);
    return Standard_True;
}
...
Standard_Boolean TObj_Object::GetObj(const TDF_Label& theLabel, Handle(TObj_Object) & theResult,
                                     const Standard_Boolean isSuper) {
    if (theLabel.IsNull()) return Standard_False;
    ...
    if (!theResult.IsNull()) {
        if (!theResult->myLabel.IsNull()) return Standard_True;
...
Handle(TObj_Object) TObj_Object::GetFatherObject(const Handle(Standard_Type) & theType) const {
    ...
    if (myLabel.IsNull()) return aFather;
...
void TObj_Object::AfterRetrieval() {
    // Register the name
    Handle(TObj_Model) aModel = GetModel();
    if (!aModel.IsNull()) aModel->RegisterName(GetName(), GetLabel(), GetDictionary());
}
...
TDF_Label TObj_Object::getDataLabel(const Standard_Integer theRank1, const Standard_Integer theRank2) const {
    TDF_Label aLabel;
    if (theRank1 > 0) // protection
    {
        aLabel = GetDataLabel().FindChild(theRank1, Standard_True);
        if (theRank2 > 0) aLabel = aLabel.FindChild(theRank2, Standard_True);
    }
    return aLabel;
}
...
TDF_Label TObj_Object::getReferenceLabel(const Standard_Integer theRank1, const Standard_Integer theRank2) const {
    TDF_Label aLabel;
    if (theRank1 > 0) // protection
    {
...
Standard_Boolean TObj_Object::isDataAttribute(const Standard_GUID& theGUID, const Standard_Integer theRank1,
...
Standard_Real TObj_Object::getReal(const Standard_Integer theRank1, const Standard_Integer theRank2) const {
...
Standard_Boolean TObj_Object::setReal(const Standard_Real theValue, const Standard_Integer theRank1,
                                      const Standard_Integer theRank2, const Standard_Real theTolerance) const {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
...
void TObj_Object::setExtString(const Handle(TCollection_HExtendedString) & theValue, const Standard_Integer theRank1,
                               const Standard_Integer theRank2) const {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
    if (!theValue.IsNull())
...
void TObj_Object::setAsciiString(const Handle(TCollection_HAsciiString) & theValue, const Standard_Integer theRank1,
                                 const Standard_Integer theRank2) const {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
    if (!theValue.IsNull())
...
Standard_Integer TObj_Object::getInteger(const Standard_Integer theRank1, const Standard_Integer theRank2) const {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
...
Standard_Boolean TObj_Object::setInteger(const Standard_Integer theValue, const Standard_Integer theRank1,
                                         const Standard_Integer theRank2) const {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);

    // check that value is actually changed
    Handle(TDataStd_Integer) A;
    if (aLabel.FindAttribute(TDataStd_Integer::GetID(), A) && A->Get() == theValue) return Standard_False;
...
Handle(TObj_Object) TObj_Object::getReference(const Standard_Integer theRank1, const Standard_Integer theRank2) const {
...
Standard_Boolean TObj_Object::setReference(const Handle(TObj_Object) & theObject, const Standard_Integer theRank1,
                                           const Standard_Integer theRank2) {
    TDF_Label aLabel = getReferenceLabel(theRank1, theRank2);

    if (theObject.IsNull()) return aLabel.ForgetAttribute(TObj_TReference::GetID());

    // check that reference is actually changed
    Handle(TObj_TReference) A;
    if (aLabel.FindAttribute(TObj_TReference::GetID(), A) && A->Get() == theObject) return Standard_False;
...
TDF_Label TObj_Object::addReference(const Standard_Integer theRank1, const Handle(TObj_Object) & theObject) {
    TDF_Label aRefLabel = GetReferenceLabel();
    if (theRank1 > 0) aRefLabel = aRefLabel.FindChild(theRank1, Standard_True);
...
Handle(TColStd_HArray1OfReal) TObj_Object::getRealArray(const Standard_Integer theLength,
    ...
    if (!anArrAttribute.IsNull()) anArr = anArrAttribute->Array();
    return anArr;
}
...
Handle(TColStd_HArray1OfExtendedString) TObj_Object::getExtStringArray(const Standard_Integer theLength,
    ...
    if (!aLabel.FindAttribute(TDataStd_ExtStringArray::GetID(), anArrAttribute))
        if (theLength > 0) anArrAttribute = TDataStd_ExtStringArray::Set(aLabel, 1, theLength);
...
void TObj_Object::setArray(const Handle(TColStd_HArray1OfReal) & theArray, const Standard_Integer theRank1,
                           const Standard_Integer theRank2) {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
    Handle(TDataStd_RealArray) anArrAttribute;
    ...
    if (theArray.IsNull()) {
        // deletion mode
        if (!anArrAttribute.IsNull()) aLabel.ForgetAttribute(anArrAttribute);
        return;
    }
...
void TObj_Object::setArray(const Handle(TColStd_HArray1OfInteger) & theArray, const Standard_Integer theRank1,
                           const Standard_Integer theRank2) {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
    Handle(TDataStd_IntegerArray) anArrAttribute;
...
void TObj_Object::setArray(const Handle(TColStd_HArray1OfExtendedString) & theArray, const Standard_Integer theRank1,
                           const Standard_Integer theRank2) {
    TDF_Label aLabel = getDataLabel(theRank1, theRank2);
    Handle(TDataStd_ExtStringArray) anArrAttribute;
...
static void copyTagSources(const TDF_Label& theSourceLabel, const TDF_Label& theTargetLabel) {
    ...
    for (; aLI.More(); aLI.Next()) {
        ...
        if (!aSourceLabel.IsNull()) copyTagSources(aSourceLabel, aLI.Value());
    }
}
...
Handle(TObj_Object) TObj_Object::Clone(const TDF_Label& theTargetLabel, Handle(TDF_RelocationTable) theRelocTable) {
    Handle(TDF_RelocationTable) aRelocTable = theRelocTable;
    if (theRelocTable.IsNull()) aRelocTable = new TDF_RelocationTable;
    ...
    Handle(TObj_TModel) aModelAttr;
    if (aLabel.FindAttribute(TObj_TModel::GetID(), aModelAttr)) aTargetModel = aModelAttr->Model();

    if (aCurrentModel != aTargetModel) TObj_Assistant::SetCurrentModel(aTargetModel);
    // crete new object
    ...
    if (!aNewObj.IsNull()) {
        ...
        // copy the references
        if (theRelocTable.IsNull()) CopyReferences(aNewObj, aRelocTable);
    }

    // restore the model for persistence.
    if (aCurrentModel != aTargetModel) TObj_Assistant::SetCurrentModel(aCurrentModel);
...
Standard_Boolean TObj_Object::copyData(const Handle(TObj_Object) & theTargetObject) {
    ...
    if (!theTargetObject->DynamicType()->SubType(DynamicType())) return IsDone;
    ...
    if (aDataLabel.IsNull() || aNewDataLabel.IsNull()) return IsDone;
...
void TObj_Object::CopyChildren(TDF_Label& theTargetLabel, const Handle(TDF_RelocationTable) & theRelocTable) {
    TDF_Label aSourceChildLabel = GetChildLabel();
    Handle(TObj_ObjectIterator) aChildren = // GetChildren();
        new TObj_OcafObjectIterator(aSourceChildLabel, NULL,
                                    Standard_True); // to support children on sublabels of child label
    for (; aChildren->More(); aChildren->Next()) {
...
void TObj_Object::CopyReferences(const Handle(TObj_Object) & theTargetObject,
    ...
    for (; aSrcChildren->More(); aSrcChildren->Next()) {
        ...
        if (!theRelocTable->HasRelocation(aSrcL, aDestLabel)) continue;
        Handle(TObj_Object) aDstChild;
        if (!TObj_Object::GetObj(aDestLabel, aDstChild)) continue;
...
void TObj_Object::copyReferences(const TDF_Label& theSourceLabel, TDF_Label& theTargetLabel,
                                 const Handle(TDF_RelocationTable) & theRelocTable) {
...
void TObj_Object::ReplaceReference(const Handle(TObj_Object) & theOldObject, const Handle(TObj_Object) & theNewObject) {
    Handle(TObj_LabelIterator) anItr = Handle(TObj_LabelIterator)::DownCast(GetReferences());
    if (anItr.IsNull()) return;
    // iterates on references.
    for (; anItr->More(); anItr->Next()) {
        Handle(TObj_Object) anObj = anItr->Value();
        if (anObj != theOldObject) continue;
...
Standard_Boolean TObj_Object::IsAlive() const {
    if (myLabel.IsNull()) return Standard_False;

    Handle(TObj_Object) anObj;
    if (!GetObj(myLabel, anObj)) return Standard_False;
...
Standard_Boolean TObj_Object::RemoveBackReferences(const TObj_DeletingMode theMode) {
    ...
    for (; aRefs->More(); aRefs->Next()) {
        Handle(TObj_Object) anObject = aRefs->Value();
        if (anObject.IsNull() || !anObject->IsAlive()) continue;
    ...
    if (theMode == TObj_KeepDepending && aStrongs.Length() > 0) return Standard_False;
    ...
    for (i = 1; i <= aContainers.Length(); i++) {
        ...
        if (anObj.IsNull() || anObj->GetLabel().IsNull()) continue; // undead object on dead label
        Handle(TDF_Data) aData = anObj->GetLabel().Data();
        Standard_Boolean aModifMode = aData->IsModificationAllowed();
        if (anOwnData != aData) aData->AllowModification(Standard_True);
        anObj->RemoveReference(aMe);
        if (anOwnData != aData) aData->AllowModification(aModifMode);
    }
    /* PTv 21.11.2006
...
Standard_Boolean TObj_Object::RelocateReferences(const TDF_Label& theFromRoot, const TDF_Label& theToRoot,
    ...
    for (; aRefIt.More(); aRefIt.Next()) {
        ...
        if (aLabel.Data() != theFromRoot.Data() || aLabel.IsDescendant(theToRoot)) continue; // need not to relocate

        TDF_Tool::RelocateLabel(aLabel, theFromRoot, theToRoot, aNewLabel);
        if (aNewLabel.IsNull() || !TObj_Object::GetObj(aNewLabel, anObj)) return Standard_False;
        ...
        if (theUpdateBackRefs) {
            ...
            // an old object
            anObj = aRef->Get();
            if (!anObj.IsNull()) anObj->RemoveBackReference(me);
        }
...
Standard_Boolean TObj_Object::GetBadReference(const TDF_Label& theRoot, TDF_Label& theBadReference) const {
    TDF_ChildIDIterator aRefIt(GetReferenceLabel(), TObj_TReference::GetID(), Standard_True);
    Handle(TObj_Object) anObj;
    for (; aRefIt.More(); aRefIt.Next()) {
        Handle(TObj_TReference) aRef = Handle(TObj_TReference)::DownCast(aRefIt.Value());
...
Handle(TObj_TNameContainer) TObj_Object::GetDictionary() const {
    Handle(TObj_Model) aModel = GetModel();
    if (!aModel.IsNull()) return aModel->GetDictionary();
    return NULL;
}
