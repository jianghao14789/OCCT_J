...
Handle(TObj_TReference) TObj_TReference::Set(const TDF_Label& theLabel, const Handle(TObj_Object) & theObject,
    ...
    if (!theObject.IsNull()) theObject->AddBackReference(theMaster);
    return A;
}
...
void TObj_TReference::Paste(const Handle(TDF_Attribute) & theInto, const Handle(TDF_RelocationTable) & RT) const {
    ...
    if (!RT->HasRelocation(myLabel, aRefLabel)) aRefLabel = myLabel;
    aRefLabel.FindAttribute(TObj_TObject::GetID(), aObject);
    Handle(TObj_Object) anIObject;
    if (!aObject.IsNull()) anIObject = aObject->Get();
    ...
    if (aMasterLabel.IsNull() || !aMasterLabel.FindAttribute(TObj_TObject::GetID(), aMasterTObj)) return;

    // set master and referenced label
    aReference->Set(anIObject, aMasterLabel);

    // update back references
    if (!anIObject.IsNull()) anIObject->AddBackReference(aMasterTObj->Get());
}
...
void TObj_TReference::BeforeForget() {
    ...
    if (myMasterLabel.IsNull()) return;

    // removing back reference
    Handle(TObj_Object) aMasterObject;
    Handle(TObj_TObject) aTObject;
    if (!myMasterLabel.FindAttribute(TObj_TObject::GetID(), aTObject)) return;
...
Standard_Boolean TObj_TReference::BeforeUndo(const Handle(TDF_AttributeDelta) & theDelta,
    ...
    if (!theDelta->IsKind(STANDARD_TYPE(TDF_DeltaOnAddition))) return Standard_True;

    if (myMasterLabel.IsNull()) return Standard_True;

    Handle(TObj_Object) anObject = Get();
    if (anObject.IsNull()) return Standard_True;

    Handle(TObj_Object) aMasterObject;
    Handle(TObj_TObject) aTObject;
    if (!myMasterLabel.FindAttribute(TObj_TObject::GetID(), aTObject)) return Standard_True;
    aMasterObject = aTObject->Get();

    if (!anObject.IsNull()) anObject->RemoveBackReference(aMasterObject);
...
Standard_Boolean TObj_TReference::AfterUndo(const Handle(TDF_AttributeDelta) & theDelta,
                                            const Standard_Boolean /*isForced*/) {
    if (!theDelta->IsKind(STANDARD_TYPE(TDF_DeltaOnRemoval))) return Standard_True;
    ...
    if (!anObject.IsNull()) anObject->AddBackReference(aMasterObject);
...
Standard_Boolean TObj_TReference::AfterRetrieval(const Standard_Boolean /*forceIt*/) {
    ...
    Handle(TObj_Object) aMasterObject;
    Handle(TObj_TObject) aTObject;
    if (!myMasterLabel.FindAttribute(TObj_TObject::GetID(), aTObject)) return Standard_False;
