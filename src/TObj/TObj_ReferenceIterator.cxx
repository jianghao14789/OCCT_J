...
TObj_ReferenceIterator::TObj_ReferenceIterator(const TDF_Label& theLabel, const Handle(Standard_Type) & theType,
...
void TObj_ReferenceIterator::MakeStep() {
    for (; myIterator.More() && myNode.IsNull(); myIterator.Next()) {
        ...
        if (L.FindAttribute(TObj_TReference::GetID(), A)) {
            myObject = A->Get();
            if (!myType.IsNull() && !myObject.IsNull() && !myObject->IsKind(myType)) continue;
