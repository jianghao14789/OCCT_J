...
static Handle(Geom2d_BSplineCurve) BSplineCurveBuilder(
    ...
    const Handle(Geom2d_Conic) & TheConic, const Convert_ConicToBSplineCurve& Convert) {
    ...
    Handle(Geom2d_BSplineCurve) Cres = Handle(Geom2d_BSplineCurve)::DownCast(TheCurve->Transformed(T));
    return Cres;
}
...
Handle(Geom2d_BSplineCurve) Geom2dConvert::SplitBSplineCurve(
    ...
    const Handle(Geom2d_BSplineCurve) & C, const Standard_Integer FromK1, const Standard_Integer ToK2,
...
Handle(Geom2d_BSplineCurve) Geom2dConvert::SplitBSplineCurve(
    ...
    const Handle(Geom2d_BSplineCurve) & C, const Standard_Real FromU1, const Standard_Real ToU2,
...
Handle(Geom2d_BSplineCurve) Geom2dConvert::CurveToBSplineCurve(
    ...
    if (C->IsKind(STANDARD_TYPE(Geom2d_TrimmedCurve))) {
        ...
        if (!Curv->IsPeriodic()) {
            if (U1 < Curv->FirstParameter()) U1 = Curv->FirstParameter();
            if (U2 > Curv->LastParameter()) U2 = Curv->LastParameter();
        }
...
public:
    Geom2dConvert_law_evaluator(const Handle(Geom2d_BSplineCurve) & theAncore) : myAncore(theAncore) {}

    virtual void Evaluate(const Standard_Integer theDerivativeRequest, const Standard_Real* theStartEnd,
                          const Standard_Real theParameter, Standard_Real& theResult,
...
static Handle(Geom2d_BSplineCurve)
    MultNumandDenom(const Handle(Geom2d_BSplineCurve) & a, const Handle(Geom2d_BSplineCurve) & BS)

{
    ...
    BSplCLib::MergeBSplineKnots(tolerance, start_value, end_value, a->Degree(), aKnots, aMults, BS->Degree(), BSKnots,
                                BSMults, resNbPoles, resKnots, resMults);
    ...
    BSplCLib::FunctionMultiply(ev, BS->Degree(), BSFlatKnots, BSPoles, resFlatKnots, degree, resNumPoles, aStatus);
    BSplCLib::FunctionMultiply(ev, BS->Degree(), BSFlatKnots, BSWeights, resFlatKnots, degree, resDenPoles, aStatus);
...

{
    ...
    if (BS->IsRational()) {
        ...
        if ((BSplCLib::IsRational(tabWeights, 1, BS->NbPoles())) &&
            ((BS->Weight(1) < (1 - Precision::Confusion())) || (BS->Weight(1) > (1 + Precision::Confusion())) ||
             (BS->Weight(2) < (1 - Precision::Confusion())) || (BS->Weight(2) > (1 + Precision::Confusion())) ||
...
static void ReorderArrayOfG1(TColGeom2d_Array1OfBSplineCurve& ArrayOfCurves, TColStd_Array1OfReal& ArrayOfToler,
                             TColStd_Array1OfBoolean& tabG1, const Standard_Integer StartIndex,
...
static GeomAbs_Shape Continuity(const Handle(Geom2d_Curve) & C1, const Handle(Geom2d_Curve) & C2,
                                const Standard_Real u1, const Standard_Real u2, const Standard_Boolean r1,
                                const Standard_Boolean r2, const Standard_Real tl, const Standard_Real ta) {
...
public:
    ...
    virtual void Evaluate(const Standard_Integer theDerivativeRequest, const Standard_Real* /*theStartEnd*/,
        ...
        theErrorCode = 0;
        PLib::EvalPolynomial(
            theParameter, theDerivativeRequest, 2, 1,
...
void Geom2dConvert::ConcatG1(TColGeom2d_Array1OfBSplineCurve& ArrayOfCurves, const TColStd_Array1OfReal& ArrayOfToler,
                             Handle(TColGeom2d_HArray1OfBSplineCurve) & ArrayOfConcatenated,
                             Standard_Boolean& ClosedFlag, const Standard_Real ClosedTolerance)

{
    Standard_Integer nb_curve = ArrayOfCurves.Length(), nb_vertexG1, nb_group = 0, index = 0, i, ii, j, jj, indexmin,
                     nb_vertex_group0 = 0;
    ...
    for (i = 0; i < nb_curve; i++) {
        if (i >= 1) {
            ...
            if (Continuity(ArrayOfCurves(i - 1), ArrayOfCurves(i), PreLast, First, Standard_True, Standard_True) <
                GeomAbs_C0)
    ...
    while (index <= nb_curve - 1) { // determination des caracteristiques du Wire
        ...
        while (((index + nb_vertexG1) <= nb_curve - 2) && (tabG1(index + nb_vertexG1) == Standard_True))
            nb_vertexG1++;
        nb_group++;
        if (index == 0) nb_vertex_group0 = nb_vertexG1;
    ...
    if (nb_group == 1 && ClosedFlag && NeedDoubleDegRepara) {
        ...
        if (Curve1->Degree() > Geom2d_BSplineCurve::MaxDegree() / 2) ClosedFlag = Standard_False;
    }
    ...
    if ((nb_group == 1) && (ClosedFlag)) { // traitement d'un cas particulier
        ...
        for (j = 1; j <= nb_curve - 1; j++) { // boucle secondaire a l'interieur de chaque groupe
            ...
            if ((j == (nb_curve - 1)) && (NeedDoubleDegRepara)) {
                ...
                Standard_Real tmax, a, b, c, umin = Curve1->FirstParameter(), umax = Curve1->LastParameter();
                ...
                BSplCLib::FunctionReparameterise(ev, Curve1->Degree(), Curve1FlatKnots, Curve1Poles, FlatKnots,
                                                 aNewCurveDegree, NewPoles, aStatus);
                TColStd_Array1OfReal NewWeights(1, FlatKnots.Length() - (2 * Curve1->Degree() + 1));
                BSplCLib::FunctionReparameterise(ev, Curve1->Degree(), Curve1FlatKnots, Curve1Weights, FlatKnots,
                                                 aNewCurveDegree, NewWeights, aStatus);
            ...
            if (fusion == Standard_False) throw Standard_ConstructionError("Geom2dConvert Concatenation Error");
            Curve2 = C.BSplineCurve();
        }
        Curve2->SetPeriodic(); // 1 seule courbe C1
        Curve2->RemoveKnot(Curve2->LastUKnotIndex(), Curve2->Multiplicity(Curve2->LastUKnotIndex()) - 1,
                           Precision::Confusion());
...
void Geom2dConvert::ConcatC1(TColGeom2d_Array1OfBSplineCurve& ArrayOfCurves, const TColStd_Array1OfReal& ArrayOfToler,
                             Handle(TColStd_HArray1OfInteger) & ArrayOfIndices,
                             Handle(TColGeom2d_HArray1OfBSplineCurve) & ArrayOfConcatenated,
                             Standard_Boolean& ClosedFlag, const Standard_Real ClosedTolerance) {
    ConcatC1(ArrayOfCurves, ArrayOfToler, ArrayOfIndices, ArrayOfConcatenated, ClosedFlag, ClosedTolerance,
             Precision::Angular());
}
//=======================================================================
// function : ConcatC1
...

{
    ...
    for (i = 0; i < nb_curve; i++) {
        if (i >= 1) {
            First = ArrayOfCurves(i)->FirstParameter();
            if (Continuity(ArrayOfCurves(i - 1), ArrayOfCurves(i), PreLast, First, Standard_True, Standard_True,
                           ArrayOfToler(i - 1), AngularTolerance) < GeomAbs_C0)
                throw Standard_ConstructionError("Geom2dConvert curves not C0"); // renvoi d'une erreur
            else {
                if (Continuity(ArrayOfCurves(i - 1), ArrayOfCurves(i), PreLast, First, Standard_True, Standard_True,
                               ArrayOfToler(i - 1), AngularTolerance) >= GeomAbs_G1)
                    tabG1(i - 1) = Standard_True; // True=Continuite G1
                else
                    tabG1(i - 1) = Standard_False;
            }
    ...
    if ((nb_group == 1) && (ClosedFlag)) { // traitement d'un cas particulier
            ...
            if (j == 0) // initialisation en debut de groupe
                ...
                if ((j == (nb_curve - 1)) && (NeedDoubleDegRepara)) {
                    ...
                    TColStd_Array1OfReal FlatKnots(1,
                                                   Curve1FlatKnots.Length() + (Curve1->Degree() * Curve1->NbKnots()));
        ...
        for (i = 0; i <= nb_group - 1; i++) { // boucle principale sur chaque groupe de
            ...
            if ((!ClosedFlag) || (nb_group == 1)) { // remplissage du tableau des indices conserves
                ...
                if (k == nb_group) ArrayOfIndices->SetValue(k, 0);
            } else {
                k++;
                ArrayOfIndices->SetValue(k - 1, index + nb_vertex_group0 + 1);
                if (k == nb_group) ArrayOfIndices->SetValue(k, nb_vertex_group0 + 1);
            }
...
void Geom2dConvert::C0BSplineToC1BSplineCurve(Handle(Geom2d_BSplineCurve) & BS, const Standard_Real tolerance)

{
    ...
    for (i = BS->FirstUKnotIndex() + 1; i <= (BS->LastUKnotIndex() - 1); i++) {
        if (BSMults(i) == BS->Degree()) nbcurveC1++;
    }
    ...
    if (nbcurveC1 > 1) {
        ...
        if ((point1.SquareDistance(point2) < tolerance * tolerance) && (V1.IsParallel(V2, anAngularToler))) {
            closed_flag = Standard_True;
        }

        Geom2dConvert::ConcatC1(ArrayOfCurves, ArrayOfToler, ArrayOfIndices, ArrayOfConcatenated, closed_flag,
                                tolerance);
...

{
    ...
    if (nbcurveC1 > 1) {
        ...
        Geom2dConvert::ConcatC1(ArrayOfCurves, ArrayOfToler, ArrayOfIndices, tabBS, closed_flag, Tolerance,
                                AngularTolerance);
    } else {
