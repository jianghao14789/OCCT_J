...
void ShapeFix_EdgeConnect::Build() {
    ...
    for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theSIterator(myLists); theSIterator.More();
         theSIterator.Next()) {
            ...
            if (!theOldVertex.IsSame(theNewVertex)) {
                ...
                if (use_start && use_end) {
                    ...
                    theBuilder.Remove(theEdge,
                                      theOldVertex.Oriented(TopAbs_REVERSED));       // remove reversed from closed edge
