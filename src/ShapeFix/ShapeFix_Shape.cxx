...
Standard_Boolean ShapeFix_Shape::Perform(const Message_ProgressRange& theProgress) {
    ...
    if (NeedFix(myFixVertexPositionMode)) ShapeFix::FixVertexPosition(S, Precision(), Context());
    ...
    switch (st) {
        ...
        case TopAbs_COMPSOLID: {
            ...
            for (TopoDS_Iterator anIter(S); anIter.More() && aPSSubShape.More(); anIter.Next()) {
                ...
                if (Perform(aPSSubShape.Next())) status = Standard_True;
            }
            if (!aPSSubShape.More()) return Standard_False; // aborted execution
        ...
        case TopAbs_SOLID: {
            ...
            if (myFixSolid->Perform(aPS.Next())) status = Standard_True;
        ...
        case TopAbs_SHELL: {
            ...
            if (sfsh->Perform(aPS.Next())) status = Standard_True;
        ...
        case TopAbs_WIRE: {
            ...
            if (!S.Closed()) sfw->ClosedWireMode() = Standard_False;
        ...
        case TopAbs_EDGE: {
            ...
            if (sfe->FixVertexTolerance(TopoDS::Edge(S))) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
            break;
        }
        case TopAbs_VERTEX:
        case TopAbs_SHAPE:
    ...
    if (!aPS.More()) return Standard_False; // aborted execution
    ...
    if (!fft.IsNull()) fft->FixSmallAreaWireMode() = savFixSmallAreaWireMode;
