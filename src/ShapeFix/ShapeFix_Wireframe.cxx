...
Standard_Boolean ShapeFix_Wireframe::FixWireGaps() {
    ...
    for (TopExp_Explorer anExpf1(myShape, TopAbs_FACE); anExpf1.More(); anExpf1.Next()) {
        ...
        for (TopoDS_Iterator itw(face); itw.More(); itw.Next()) {
            ...
            sfw->FixReorder();
            sfw->FixGaps3d();
            if (sfw->StatusGaps3d(ShapeExtend_DONE)) myStatusWireGaps |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
            if (sfw->StatusGaps3d(ShapeExtend_FAIL)) myStatusWireGaps |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);
            sfw->FixGaps2d();
            if (sfw->StatusGaps2d(ShapeExtend_DONE)) myStatusWireGaps |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
            if (sfw->StatusGaps2d(ShapeExtend_FAIL)) myStatusWireGaps |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
...
static Standard_Boolean JoinEdges(const TopoDS_Edge& E1, const TopoDS_Edge& E2, TopoDS_Edge& E3,
    ...
    try {
        ...
        if (!ShapeConstruct::JoinCurves(c3d1, c3d2, E1.Orientation(), E2.Orientation(), cf1, cl1, cf2, cl2, CRes,
                                        isRev1, isRev2))
            return ReplaceFirst;
        //    if(isRev1 || isRev2)
        if (!isSame && (isRev1 || isRev2)) return ReplaceFirst;
        ReplaceFirst = (!isRev1);
        ...
        if (ismanifold != ismanifold2) return ReplaceFirst;

        if (ismanifold) {

            OrEdge1 =
                ((!isRev1 && E1.Orientation() == TopAbs_FORWARD) || (isRev1 && E1.Orientation() == TopAbs_REVERSED)
                     ? TopAbs_FORWARD
                     : TopAbs_REVERSED);
            OrEdge2 =
                ((!isRev2 && E2.Orientation() == TopAbs_FORWARD) || (isRev2 && E2.Orientation() == TopAbs_REVERSED)
                     ? TopAbs_FORWARD
                     : TopAbs_REVERSED);
        }
        ...
        for (TopTools_ListIteratorOfListOfShape iter(faces); iter.More(); iter.Next()) {
            ...
            if (!ShapeConstruct::JoinCurves(c2d1, c2d2, OrEdge1, OrEdge2, first1, last1, first2, last2, C2dRes, isRev12,
                                            isRev22, isSame))
                return ReplaceFirst;

            if (ismanifold && (!isSame && (isRev12 || isRev22))) return ReplaceFirst;

            if (isRev12) ReplaceFirst = Standard_False;
            ...
            if (sae.IsSeam(E1, face) && sae.IsSeam(E2, face)) {
                ...
                if (!ShapeConstruct::JoinCurves(c2d12, c2d22, OrEdge1, OrEdge2, first1, last1, first2, last2, C2dRes2,
                                                isRev12, isRev22, isSame))
                    return ReplaceFirst;
                if (!isSame && (isRev1 || isRev2)) return ReplaceFirst;
            ...
            if (!ismanifold) newedge.Orientation(ReplaceFirst ? OrEdge1 : OrEdge2);
        }
        ...
        for (TopoDS_Iterator itw(face); itw.More(); itw.Next()) {
            ...
            for (i = 1; i <= SAW.NbEdges(); i++) {
                ...
                if (EdgeMap.Find(edge) != 1) {
                    if (!SAW.WireData()->IsSeam(i)) theMultyEdges.Add(edge);
                    continue;
                }
        ...
        for (; anExpf2.More(); anExpf2.Next()) {
            if (theFaceWithSmall.IsBound(anExpf2.Current())) {
                if (theFaceWithSmall(anExpf2.Current()).Extent()) {
                    ...
                    if (!facet.IsSame(anExpf2.Current())) { // gka
                    ...
                    for (TopoDS_Iterator itw(face); itw.More(); itw.Next()) {
                        ...
                        if (face.Orientation() == TopAbs_REVERSED) face = TopoDS::Face(facet.Oriented(TopAbs_FORWARD));
                        ...
                        while (index <= SFW->WireData()->NbEdges() && SFW->NbEdges() > 1) {
                            ...
                            if (theSmallEdges.Contains(edge2)) {
                                ...
                                if (theEdgeToFaces.IsBound(edge1)) theList1 = theEdgeToFaces(edge1);
                                if (theEdgeToFaces.IsBound(edge2)) theList2 = theEdgeToFaces(edge2);
                                if (theEdgeToFaces.IsBound(edge3)) theList3 = theEdgeToFaces(edge3);
                                ...
                                if (same_set1 && same_set2) {
                                    ...
                                    if (fabs(Ang2 - Ang1) > Precision::Angular()) take_next = (Ang2 < Ang1);
                                    if (take_next) {
                                        edge1 = edge2;
                                        edge2 = edge3;
                                    }
                                ...
                                if (same_set && !isLimAngle) {
                                    ...
                                    if (edge3.IsNull()) {
                                        ...
                                        for (; aItv.More(); aItv.Next()) {
                                            if (aItv.Value().Orientation() == TopAbs_INTERNAL ||
                                                ...
                                                TopoDS_Vertex anewV =
                                                    ShapeAnalysis_TransferParametersProj::CopyNMVertex(aOldV, edge3,
                                                                                                       edge1);
                                                BRep_Builder aB;
                                                aB.Add(edge3, anewV);
                                        ...
                                        for (TopTools_ListIteratorOfListOfShape itlf(theList); itlf.More();
                                             itlf.Next()) {
                                    ...
                                    if (!tempSaw.LastCheckStatus(ShapeExtend_FAIL)) {
                                        ...
                                        if (theEdgeToFaces.IsBound(tmpedge1)) aL1 = theEdgeToFaces.Find(tmpedge1);
                                        TopTools_ListOfShape aL2;
                                        if (theEdgeToFaces.IsBound(tmpedge2)) aL2 = theEdgeToFaces.Find(tmpedge2);
                                        ...
                                        if (theSmallEdges.Contains(remedge)) theSmallEdges.Remove(remedge);
                                        ...
                                        if (theSmallEdges.Contains(tmpedge1)) {
                                            ...
                                            for (TopTools_ListIteratorOfListOfShape itlf(aL1); itlf.More();
                                                 itlf.Next()) {
                                                TopoDS_Shape curface = itlf.Value();
                                                TopTools_ListOfShape& theEdges = theFaceWithSmall(curface);
                                                TopTools_ListIteratorOfListOfShape ite(theEdges);
                                        ...
                                        if (theSmallEdges.Contains(tmpedge2)) {
                                            ...
                                            for (TopTools_ListIteratorOfListOfShape itlf(aL2); itlf.More();
                                                 itlf.Next()) {
                                                TopoDS_Shape curface = itlf.Value();
                                                TopTools_ListOfShape& theEdges = theFaceWithSmall(curface);
                                                TopTools_ListIteratorOfListOfShape ite(theEdges);
                                    ...
                                    if (take_next && theList2.Extent() == 1) { // gka
        ...
        for (TopExp_Explorer expw1(myShape, TopAbs_WIRE, TopAbs_FACE); expw1.More(); expw1.Next()) {
            ...
            while (index <= SFW->NbEdges() && SFW->NbEdges() > 1) {
                ...
                if (theSmallEdges.Contains(edge2)) {
                    ...
                    Standard_Boolean same_set1 = (theList1.Extent() == theList2.Extent() &&
                                                  ((!isSeam && !isSeam1) || (isSeam && isSeam1))); // gka
                    Standard_Boolean same_set2 =
                        (theList3.Extent() == theList2.Extent() && ((!isSeam && !isSeam2) || (isSeam && isSeam2)));
                    TopTools_MapOfShape theSetOfFaces;
                    for (TopTools_ListIteratorOfListOfShape itf1(theList2); itf1.More(); itf1.Next())
                        theSetOfFaces.Add(itf1.Value());
                    if (same_set1) {
                        // Add all faces of the first edge to the current set
                        for (TopTools_ListIteratorOfListOfShape itf2(theList1); (itf2.More() && same_set1); itf2.Next())
                            same_set1 = theSetOfFaces.Contains(itf2.Value());
                    }
                    if (same_set2) {
                        // Add all faces of the first edge to the current set
                        for (TopTools_ListIteratorOfListOfShape itf2(theList3); (itf2.More() && same_set2); itf2.Next())
                            same_set2 = theSetOfFaces.Contains(itf2.Value());
                    }
                    if (same_set1 && same_set2) {
                        same_set = Standard_True;
