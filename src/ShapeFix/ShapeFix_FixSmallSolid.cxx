...
ShapeFix_FixSmallSolid::ShapeFix_FixSmallSolid()
    : myFixMode(0), myVolumeThreshold(Precision::Infinite()), myWidthFactorThreshold(Precision::Infinite()) {}
...
static void AddToMap(TopTools_DataMapOfShapeListOfShape& theMap, const TopoDS_Shape& theKey,
...
static void MapFacesToShells(const TopoDS_Shape& theSolid, TopTools_DataMapOfShapeShape& theMap) {
    ...
    for (; aShellIter.More(); aShellIter.Next()) {
        ...
        for (; aFaceIter.More(); aFaceIter.Next()) {
            ...
            if (!theMap.Bind(aFace, aShell)) theMap.UnBind(aFace);
        }
    }
}
...
static Standard_Boolean FindMostSharedShell(const TopoDS_Shape& theSolid,
    ...
    if (aMostSharedSolidShellPtr == NULL) return Standard_False;
...
static TopoDS_Shape MergeShells(const TopoDS_Shape& theBaseShell, TopTools_ListOfShape& theShellsToMerge,
                                const TopTools_DataMapOfShapeShape& theMapFacesToOuterShells,
    ...
    for (; aBaseFaceIter.More(); aBaseFaceIter.Next()) {
        ...
        if (!aRemoveFaces.Contains(aFace)) aBuilder.Add(aNewShell, aFace);
    }

    // If there are no elements in the new shell, return null shape
    if (aNewShell.NbChildren() == 0) return TopoDS_Shape();
...
static TopoDS_Compound AddShells(const TopoDS_Shape& theBaseShell, TopTools_ListOfShape& theShellsToAdd) {
    // Create a compound
    BRep_Builder aBuilder;
    ...
    if (!theBaseShell.IsNull()) aBuilder.Add(aCompound, theBaseShell);
...
TopoDS_Shape ShapeFix_FixSmallSolid::Merge(const TopoDS_Shape& theShape,
    ...
    while (!aSmallSolids.IsEmpty()) {
        ...
        while (aSmallIter.More()) {
            ...
            if (FindMostSharedShell(aSmallSolid, *aMapFacesToShellsPtr, aNonSmallSolidShell, anAdjacentShell,
                                    aNotAdjacentShells)) {
        ...
        for (; aShellIter.More(); aShellIter.Next()) {
            ...
            if (aShellsToBeAddedPtr != NULL) aNewShell = AddShells(aNewShell, *aShellsToBeAddedPtr);
...
Standard_Boolean ShapeFix_FixSmallSolid::IsSmall(const TopoDS_Shape& theSolid) const {
    ...
    if (IsUsedVolumeThreshold() && aVolume > myVolumeThreshold) return Standard_False;
    ...
    if (IsUsedWidthFactorThreshold() && myWidthFactorThreshold < Precision::Infinite()) {
        ...
        if (aVolume > myWidthFactorThreshold * anArea * 0.5) return Standard_False;
    }
