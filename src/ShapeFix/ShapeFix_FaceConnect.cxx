...
TopoDS_Shell ShapeFix_FaceConnect::Build(const TopoDS_Shell& shell, const Standard_Real sewtoler,
    ...
    ***************************************************************/
    ...
    for (TopTools_DataMapIteratorOfDataMapOfShapeShape theFEIter(theFreeEdges); theFEIter.More(); theFEIter.Next()) {
    ...
    if (!myOriFreeEdges.IsEmpty()) {
        ...
        for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theOFIter(myOriFreeEdges); theOFIter.More();
             theOFIter.Next()) {
    ...
    ***************************************************************/
    ...
    if (!myOriFreeEdges.IsEmpty()) {
        ...
        for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theConnectedIter(myConnected); theConnectedIter.More();
                ...
                for (theConnectedListIter.Initialize(theConnectedList); theConnectedListIter.More();
                    ...
                    if (myOriFreeEdges.IsBound(theSecondFace)) {
                        ...
                        if (theProcessed.IsBound(theSecondFace)) {
                            ...
                            for (theProcCnxListIter.Initialize(theProcCnxList); theProcCnxListIter.More() && !skip_pair;
                                 theProcCnxListIter.Next())
                                if (theFirstFace.IsSame(theProcCnxListIter.Value())) skip_pair = Standard_True;
                        }
                        if (!skip_pair) {
        ...
        for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theOF2Iter(myOriFreeEdges); theOF2Iter.More();
             theOF2Iter.Next()) {
        ...
        ***************************************************************/
        ...
        for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theOEIter(myOriFreeEdges); theOEIter.More();
             theOEIter.Next()) {
            // Iterate on original free edges
            for (theOriginalIter.Initialize(theOEIter.Value()); theOriginalIter.More(); theOriginalIter.Next()) {
            ...
            if (theReShape->Status(ShapeExtend_OK)) {
                ...
                for (TopoDS_Iterator itf1(result); itf1.More(); itf1.Next()) {
                    ...
                    for (TopoDS_Iterator itw(newface); itw.More(); itw.Next()) {
                        ...
                        for (Standard_Integer i = 1; i <= sewd->NbEdges(); i++) {
                            ...
                            if (!SAE.PCurve(sewd->Edge(i), TopoDS::Face(tmpFace), c2d, f, l)) continue;
                ...
                if (!theRepVertices.IsEmpty()) {
                    ...
                    for (TopTools_DataMapIteratorOfDataMapOfShapeListOfShape theRV2Iter(theNewVertices);
                        ...
                        curtoler = thePosition.Distance(BRep_Tool::Pnt(theNewVert)) + BRep_Tool::Tolerance(theNewVert);
                        if (curtoler > theTolerance) theTolerance = curtoler;
                    ...
                    // Replace vertices
                    theReShape->Clear();
                    for (TopTools_DataMapIteratorOfDataMapOfShapeShape theNVIter(theOldVertices); theNVIter.More();
                         theNVIter.Next())
                ...
                for (TopTools_DataMapIteratorOfDataMapOfShapeShape theR2Iter(theOldVertices); theR2Iter.More();
                     theR2Iter.Next()) {
                    totvold++;
                ...
                std::cout << "TOTAL: " << toteold << " edges, " << totvold << " vertices replaced by " << totenew
                          << " edges, " << totvnew << " vertices" << std::endl
                          << std::endl;
                //-------------------------------
#endif
            }
