...
ShapeFix_Wire::ShapeFix_Wire(const TopoDS_Wire& wire, const TopoDS_Face& face, const Standard_Real prec)
    : myMaxTailAngleSine(0), myMaxTailWidth(-1) {
    myFixEdge = new ShapeFix_Edge;
    myAnalyzer = new ShapeAnalysis_Wire;
    ClearModes();
...
Standard_Boolean ShapeFix_Wire::Perform() {
    ...
    if (!Context().IsNull()) myFixEdge->SetContext(Context());
    ...
    if (NeedFix(myFixSelfIntersectionMode, myClosedMode)) {
        ...
        if (myFixIntersectingEdgesMode == -1 && !ReorderOK) myFixIntersectingEdgesMode = 0;
...
Standard_Boolean ShapeFix_Wire::FixReorder() {
    ...
    if (sawo.Status() != 0 && !myAnalyzer->Surface().IsNull() && myAnalyzer->Surface()->Surface()->IsUPeriodic() &&
    ...
    if (LastFixStatus(ShapeExtend_FAIL))
        myStatusReorder |=
            ShapeExtend::EncodeStatus(LastFixStatus(ShapeExtend_FAIL1) ? ShapeExtend_FAIL1 : ShapeExtend_FAIL2);
    ...
    if (sawo.Status() == 2 || sawo.Status() == -2) myStatusReorder |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
    if (sawo.Status() < 0) myStatusReorder |= ShapeExtend::EncodeStatus(ShapeExtend_DONE3);
    if (sawo.Status() == 3) myStatusReorder |= ShapeExtend::EncodeStatus(ShapeExtend_DONE5); // only shifted
...
Standard_Integer ShapeFix_Wire::FixSmall(const Standard_Boolean lockvtx, const Standard_Real precsmall) {
...
Standard_Boolean ShapeFix_Wire::FixEdgeCurves() {
    ...
    if (isReady && NeedFix(myFixRemovePCurveMode, Standard_False)) {
        for (i = 1; i <= nb; i++) {
            ...
            if (myFixEdge->Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
            if (myFixEdge->Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
        }
    }
    ...
    if (isReady && NeedFix(myFixAddPCurveMode)) {
        ...
        for (i = 1; i <= nb; i++) {
            myFixEdge->FixAddPCurve(sbwd->Edge(i), face, sbwd->IsSeam(i), myAnalyzer->Surface(), Precision());
            if (myFixEdge->Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE3);
            if (myFixEdge->Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL3);
                        ...
                        if (BRep_Tool::SameParameter(sbwd->Edge(i)))
                            ...
                            if (sae.HasPCurve(E, face)) {
                                ...
                                if (sae.PCurve(E, face, C2d, fp2d, lp2d)) {
                                    if (!C2d->IsKind(STANDARD_TYPE(Geom2d_TrimmedCurve))) sbe.RemovePCurve(E, face);
                                }
                            }
                        }
        ...
        if (overdegen && myAnalyzer->Surface()->IsUClosed(Precision())) {
            ...
            if (k > nb && Abs(Abs(vec.X()) - URange) < 0.1 * URange) {
                ...
                myFixEdge->FixAddPCurve(sbwd->Edge(overdegen), face, sbwd->IsSeam(overdegen), myAnalyzer->Surface(),
                                        Precision());
            }
#ifdef OCCT_DEBUG
    ...
    if (isReady && NeedFix(myFixRemoveCurve3dMode, Standard_False)) {
        for (i = 1; i <= nb; i++) {
            ...
            if (myFixEdge->Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE4);
            if (myFixEdge->Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL4);
        }
    }
    if (NeedFix(myFixAddCurve3dMode)) {
        for (i = 1; i <= nb; i++) {
            myFixEdge->FixAddCurve3d(sbwd->Edge(i));
            if (myFixEdge->Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE5);
            if (myFixEdge->Status(ShapeExtend_FAIL)) {
                ...
                if (C.IsNull() || Abs(last - first) < Precision::PConfusion()) {
                    SendWarning(
                        sbwd->Edge(i),
                        Message_Msg(
                            "FixWire.FixCurve3d.Removed")); // Incomplete edge (with no pcurves or 3d curve) removed
                    sbwd->Remove(i--);
                    nb--;
    ...
    if (isReady && NeedFix(myFixSeamMode, Standard_False)) {
        for (i = 1; i <= nb; i++) {
            FixSeam(i);
            if (LastFixStatus(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE6);
            if (LastFixStatus(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL6);
        }
    }
    ...
    if (isReady && NeedFix(myFixShiftedMode)) {
        FixShifted();
        if (LastFixStatus(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE7);
        if (LastFixStatus(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL7);
    }
        ...
        for (i = 1; i <= nb; i++) {
            ...
            if (sae.HasPCurve(sbwd->Edge(i), face)) {
                ...
                if (sae.PCurve(sbwd->Edge(i), face, C2d, fp2d, lp2d, Standard_False)) {
                    if (fabs(First - fp2d) > Precision::PConfusion() || fabs(Last - lp2d) > Precision::PConfusion()) {
                        BRep_Builder B;
                        ...
                        myFixEdge->FixAddPCurve(sbwd->Edge(i), face, sbwd->IsSeam(i), myAnalyzer->Surface(),
                                                Precision());
            ...
            if (myFixEdge->Status(ShapeExtend_DONE)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_DONE8);
            if (myFixEdge->Status(ShapeExtend_FAIL)) myStatusEdgeCurves |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL8);
        }
    }
...
Standard_Boolean ShapeFix_Wire::FixSelfIntersection() {
    ...
    if (NeedFix(myFixNonAdjacentIntersectingEdgesMode)) {
        ...
        if (StatusSelfIntersection(ShapeExtend_DONE5))
            std::cout << "Warning: ShapeFix_Wire::FixIntersection: Non-adjacent intersection fixed (split-" << NbSplit
                      << ", cut-" << NbCut << ", removed-" << NbRemoved << ")" << std::endl;
#endif
...
Standard_Boolean ShapeFix_Wire::FixClosed(const Standard_Real prec) {
    ...
    FixConnected(1, prec);
    if (LastFixStatus(ShapeExtend_DONE)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (LastFixStatus(ShapeExtend_FAIL)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);

    FixDegenerated(1);
    if (LastFixStatus(ShapeExtend_DONE)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
    if (LastFixStatus(ShapeExtend_FAIL)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);

    FixLacking(1);
    if (LastFixStatus(ShapeExtend_DONE)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_DONE3);
    if (LastFixStatus(ShapeExtend_FAIL)) myStatusClosed |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL3);
...
Standard_Boolean ShapeFix_Wire::FixSmall(const Standard_Integer num, const Standard_Boolean lockvtx,
    ...
    if (LastFixStatus(ShapeExtend_DONE2)) {
        ...
        if (LastFixStatus(ShapeExtend_FAIL)) savLastFixStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL3);
...
Standard_Boolean ShapeFix_Wire::FixConnected(const Standard_Integer num, const Standard_Real prec) {
    ...
    if (myAnalyzer->LastCheckStatus(ShapeExtend_DONE1)) { // absolutely confused
        ...
        if (V2.IsSame(sae.LastVertex(E2))) {
            V = V2;
            if (!Context().IsNull()) Context()->Replace(V1, V.Oriented(V1.Orientation()));
        } else {
            V = V1;
            if (!Context().IsNull()) Context()->Replace(V2, V.Oriented(V2.Orientation()));
        }
    } else { // on moyenne ...
    ...
    if (sbwd->NbEdges() < 2) {
        ...
        if (E2.Free() && E1.Free() && myTopoMode) {
            ...
            B.Remove(E2, sae.FirstVertex(E2));
            B.Add(E2, V.Oriented(TopAbs_FORWARD));
            if (!myAnalyzer->LastCheckStatus(ShapeExtend_DONE1) || sae.FirstVertex(E2).IsSame(sae.LastVertex(E2))) {
...
Standard_Boolean ShapeFix_Wire::FixShifted() {
    ...
    for (Standard_Integer i = 1; i <= sbwdOring->NbEdges(); i++) {
        ...
        if (BRep_Tool::Degenerated(E1) && !sae.HasPCurve(E1, Face())) continue;
    ...
    for (Standard_Integer n2 = 1, n1 = nb; !end; n1 = n2++) {
        ...
        if (surf->DegeneratedValues(p, Max(Precision(), BRep_Tool::Tolerance(V)), degP1, degP2, degT1, degT2))
        ...
        if (surf->Surface()->IsKind(STANDARD_TYPE(Geom_SurfaceOfRevolution))) {
            if (!isDeg && !vclosed) {
                ...
                if (surf->IsDegenerated(p1, p2, MaxTolerance(), 10) &&
                    !surf->IsDegenerated(
                        c2d1->Value(a1), c2d1->Value(b1), MaxTolerance(),
                        10)) // abv 31.07.00: trj4_pm1-ec-214.stp #31274: still allow work if edge already exists
                    isDeg = 1;
            }
            if (!isDeg && !uclosed) {
                if (c2d1.IsNull() && !sae.PCurve(E1, Face(), c2d1, a1, b1, Standard_True)) continue;
            ...
            if (!degn2) {
                ...
                if (pdeg.SquareDistance(p) < Precision() * Precision()) {
                    ...
                    if (rot1 * rot2 < -::Precision::PConfusion() && scld * scln < -::Precision::PConfusion() &&
                        ...
                        Standard_Real dx = ShapeAnalysis::AdjustToPeriod(deep, ::Precision::PConfusion(),
...
static Standard_Boolean TryNewPCurve(const TopoDS_Edge& E, const TopoDS_Face& face, Handle(Geom2d_Curve) & c2d,
                                     Standard_Real& first, Standard_Real& last, Standard_Real& tol) {
    Standard_Real f, l;
...
// purpose  :
//=======================================================================
static Standard_Boolean RemoveLoop(TopoDS_Edge& E, const TopoDS_Face& face, const IntRes2d_IntersectionPoint& IP,
                                   const Standard_Real tolfact, const Standard_Real prec,
    ...
    Standard_Real a, b;
    Handle(Geom2d_Curve) c2d;
    if (!sae.PCurve(E, face, c2d, a, b, Standard_False)) return Standard_False;
    ...
    if (!connect.Add(patch, ::Precision::PConfusion(), Standard_True, Standard_False)) return Standard_False;

    // last segment
    if (!connect.Add(trim, ::Precision::PConfusion(), Standard_True, Standard_False)) return Standard_False;
    // PTV OCC884
    ...
    if (!loopRemoved3d) {
        ...
        if (!connect1.Add(patch1, ::Precision::PConfusion(), Standard_True, Standard_False)) return Standard_False;

        // last segment
        if (!connect1.Add(trim1, ::Precision::PConfusion(), Standard_True, Standard_False)) return Standard_False;

        bs1 = connect1.BSplineCurve();

        if (bs1.IsNull()) return Standard_False;
    }
    //  Standard_Real oldtol = BRep_Tool::Tolerance ( E );

    if (!loopRemoved3d) B.UpdateEdge(E, bs1, L, 0.);
    B.UpdateEdge(E, bs, face, 0.);
    B.Range(E, face, f, l);
...
// purpose  :
//=======================================================================
static Standard_Boolean RemoveLoop(TopoDS_Edge& E, const TopoDS_Face& face, const IntRes2d_IntersectionPoint& IP2d,
                                   TopoDS_Edge& E1, TopoDS_Edge& E2) {
#ifdef OCCT_DEBUG
    ...
    if (!trim1.IsNull()) E1 = TopoDS::Edge(E.EmptyCopied());
    ...
    if (E.Orientation() == TopAbs_FORWARD) {
        if (!E1.IsNull()) E1 = sbe.CopyReplaceVertices(E1, Vfirst, Vmid);
        E2 = sbe.CopyReplaceVertices(E2, Vmid, Vlast);
    } else {
        if (!E1.IsNull()) E1 = sbe.CopyReplaceVertices(E1, Vmid, Vlast);
...
Standard_Boolean ShapeFix_Wire::FixSelfIntersectingEdge(const Standard_Integer num) {
    ...
    if (myRemoveLoopMode < 1) {
        for (Standard_Integer iter = 0; iter < 30; iter++) {
            ...
            for (Standard_Integer i = 1; i <= points2d.Length(); i++) {
                ...
                if (myGeomMode) {
                    if (c2d.IsNull()) sae.PCurve(E, Face(), c2d, f2d, l2d, Standard_False);
                    ...
                    if (RemoveLoop(E, Face(), points2d.Value(i), tolfact, Min(MaxTolerance(), Max(newtol, Precision())),
                                   myRemoveLoopMode == 0)) {
    ...
    if (myRemoveLoopMode == 1) {
        ...
        if (newtol > MaxTolerance()) myLastFixStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
        ...
        if (loopRemoved) myLastFixStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE8);
    }
...
// purpose  : auxiliary
//=======================================================================
static Standard_Real ComputeLocalDeviation(const TopoDS_Edge& edge, const gp_Pnt& pint, const gp_Pnt& pnt,
                                           Standard_Real f, Standard_Real l, const TopoDS_Face& face) {
    ShapeAnalysis_Edge sae;
...
Standard_Boolean ShapeFix_Wire::FixIntersectingEdges(const Standard_Integer num) {
    ...
    for (Standard_Integer i = 1; i <= nb; i++) {
        ...
        if (/*! myTopoMode &&*/ newtol > tol) {
            Standard_Real te1 = rad + ComputeLocalDeviation(E1, pint, pnt, param1, (isForward1 ? b1 : a1), Face());
            Standard_Real te2 = rad + ComputeLocalDeviation(E2, pint, pnt, (isForward2 ? a2 : b2), param2, Face());
...
#ifdef OCCT_DEBUG
                    std::cout << "Warning: ShapeFix_Wire::FixIE: edges tolerance increased: (" << te1 << ", " << te2
                              << ") / " << newtol << std::endl;
#endif
        ...
        if (locMayEdit || newtol <= MaxTolerance()) {
            ...
            if (locMayEdit) {
                ...
                //: j6 abv 7 Dec 98: ProSTEP TR10 r0601_id.stp #57676 & #58586: do not cut edges because of influence on
                //: adjacent faces
    ...
    if (cutEdge1) {
        if (!Context().IsNull()) E1 = TopoDS::Edge(Context()->Apply(E1));
        myFixEdge->FixSameParameter(E1);
    }
    if (cutEdge2 && !IsCutLine) {
        if (!Context().IsNull()) E2 = TopoDS::Edge(Context()->Apply(E2));
...
Standard_Boolean ShapeFix_Wire::FixIntersectingEdges(const Standard_Integer num1, const Standard_Integer num2) {
    myLastFixStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if (!IsReady()) return Standard_False;
    IntRes2d_SequenceOfIntersectionPoint points2d;
    TColgp_SequenceOfPnt points3d;
    ...
    for (i = 1; i <= nb; i++) {
        ...
        if (aMinDist < gp::Resolution()) continue;
        ...
        if (aMinDist < RealLast() && !aCurve1.IsNull() && !aCurve2.IsNull()) {
            ...
            for (aPointsC = 2; aPointsC < 19; aPointsC++) {
                ...
                if (d1 > tole1 && d1 > aMaxEdgeTol1) aMaxEdgeTol1 = d1;
                ...
                if (d2 > tole2 && d2 > aMaxEdgeTol2) aMaxEdgeTol2 = d2;
            }
            ...
            if (aNecessaryVtxTole > Max(aMaxEdgeTol1, tole1) || aNecessaryVtxTole > Max(aMaxEdgeTol2, tole2)) {
                aMaxEdgeTol1 = 0.0;
                aMaxEdgeTol2 = 0.0;
            }
        }
    ...
    if (aNewTolEdge1 > 0) {
        for (i = 1; i <= 2; i++)
            if (aNewTolEdge1 > Max(vertexTolers(i), newTolers(i))) newTolers(i) = aNewTolEdge1;
        B.UpdateEdge(edge1, aNewTolEdge1);
    }
    if (aNewTolEdge2 > 0) {
        for (i = 3; i <= 4; i++)
            if (aNewTolEdge2 > Max(vertexTolers(i), newTolers(i))) newTolers(i) = aNewTolEdge2;
...
static Standard_Boolean TryBendingPCurve(const TopoDS_Edge& E, const TopoDS_Face& face, const gp_Pnt2d p2d,
                                         const Standard_Boolean end, Handle(Geom2d_Curve) & c2d, Standard_Real& first,
                                         Standard_Real& last, Standard_Real& tol) {
    ShapeAnalysis_Edge sae;
...

    {
        try {
            ...
            if (fabs(bs->FirstParameter() - par) < ::Precision::PConfusion() && bs->Multiplicity(1) > bs->Degree())
                bs->SetPole(1, p2d);
...
Standard_Boolean ShapeFix_Wire::FixLacking(const Standard_Integer num, const Standard_Boolean force) {
    ...
    if (myGeomMode && !BRep_Tool::IsClosed(E1, face) && !BRep_Tool::IsClosed(E2, face)) {
        ...
        Standard_Boolean ok1 =
            TryBendingPCurve(E1, face, p2d, E1.Orientation() == TopAbs_FORWARD, bendc1, bendf1, bendl1, bendtol1);
        Standard_Boolean ok2 =
            TryBendingPCurve(E2, face, p2d, E2.Orientation() == TopAbs_REVERSED, bendc2, bendf2, bendl2, bendtol2);
        if (ok1 && !ok2) {
            bendtol2 = BRep_Tool::Tolerance(E2);
            ok1 =
                TryBendingPCurve(E1, face, p2d2, E1.Orientation() == TopAbs_FORWARD, bendc1, bendf1, bendl1, bendtol1);
        } else if (!ok1 && ok2) {
            bendtol1 = BRep_Tool::Tolerance(E1);
            ok2 =
                TryBendingPCurve(E2, face, p2d1, E2.Orientation() == TopAbs_FORWARD, bendc2, bendf2, bendl2, bendtol2);
        }
    ...
    // If increase is not OK or force, try to find other solutions (adding edge)
        ...
        if (myTopoMode) {
            ...
            if (!myAnalyzer->LastCheckStatus(
                    ShapeExtend_DONE2) && //: 81 abv 20 Jan 98: don`t add back-going edges (zigzags)
...
static void CopyReversePcurves(const TopoDS_Edge& toedge, const TopoDS_Edge& fromedge, const Standard_Boolean reverse) {
    ...
    for (BRep_ListIteratorOfListOfCurveRepresentation fromitcr(
        ...
        if (fromGC->IsCurveOnSurface()) {
            ...
            for (BRep_ListIteratorOfListOfCurveRepresentation toitcr(tolist); toitcr.More() && !found; toitcr.Next()) {
                ...
                if (toGC.IsNull() || !toGC->IsCurveOnSurface() || surface != toGC->Surface() || L != toGC->Location())
                    continue;
                found = Standard_True;
                break;
            }
...
Standard_Boolean ShapeFix_Wire::FixTails() {
    ...
    while (aECount >= 2 && aENs[1] <= aECount) {
        ...
        if (!myAnalyzer->CheckTail(aEs[0], aEs[1], aCheckAngle ? myMaxTailAngleSine : -1, myMaxTailWidth,
                                   MaxTolerance(), aEParts[0][0], aEParts[0][1], aEParts[1][0], aEParts[1][1])) {
            aENs[0] = aENs[1]++;
        ...
        Standard_Integer aSplitCounts[] = {aEParts[0][1].IsNull() ? 0 : 1, aEParts[1][1].IsNull() ? 0 : 1};
        const Standard_Integer aRemoveCount = (aEParts[0][0].IsNull() ? 0 : 1) + (aEParts[1][0].IsNull() ? 0 : 1);
