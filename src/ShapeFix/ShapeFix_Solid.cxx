    ...
    for (Standard_Integer i = 1; i <= aSeqShells.Length(); i++) {
        ...
        try {
            ...
            if (!theMapStatus.IsBound(aShell1)) {
                ...
                if (infinstatus != TopAbs_UNKNOWN && infinstatus != TopAbs_ON) st = (infinstatus == TopAbs_IN ? 1 : 2);
                theMapStatus.Bind(aShell1, st);

            } else {
                st = theMapStatus.Find(aShell1);
                if (st) infinstatus = (theMapStatus.Find(aShell1) == 1 ? TopAbs_IN : TopAbs_OUT);
            }
            if (!st) continue;
            for (Standard_Integer j = 1; j <= aSeqShells.Length(); j++) {
                ...
                for (TopExp_Explorer aExpVert(aShell2, TopAbs_VERTEX); aExpVert.More() && amapVert.Extent() < 10;
                     aExpVert.Next())
...
static Standard_Boolean CreateSolids(const TopoDS_Shape theShape, TopTools_IndexedMapOfShape& aMapSolids) {
    ...
    for (Standard_Integer i = 1; i <= aMapShellHoles.Extent(); ++i) {
        ...
        if (aMapStatus.IsBound(aShell)) {
            ...
            if (st) infinstatus = (aMapStatus.Find(aShell) == 1 ? TopAbs_IN : TopAbs_OUT);
        }
    ...
    for (Standard_Integer i = 1; i <= aMapFaceShells.Extent(); i++) {
        ...
        if (nbSol > 1) {
            for (TopTools_ListIteratorOfListOfShape lItSh1(lshells); lItSh1.More(); lItSh1.Next()) {
                if (ShellSolid.Contains(lItSh1.Value())) ShellSolid.ChangeFromKey(lItSh1.Value()) = aCompSolid;
            }
        }
    }
    for (Standard_Integer kk = 1; kk <= ShellSolid.Extent(); kk++)
        if (!aMapSolids.Contains(ShellSolid.FindFromIndex(kk))) aMapSolids.Add(ShellSolid.FindFromIndex(kk));
...
Standard_Boolean ShapeFix_Solid::Perform(const Message_ProgressRange& theProgress) {
    ...
    if (NeedFix(myFixShellMode)) {
        ...
        if (!aPSFixShell.More()) return Standard_False;
    } else {
        NbShells = aNbShells;
    }
    ...
    if (NbShells == 1) {
        ...
        if (isClosed || myCreateOpenSolidMode) {
            ...
            TopoDS_Shell aShell;
            if (itersh.More() && itersh.Value().ShapeType() == TopAbs_SHELL) aShell = TopoDS::Shell(itersh.Value());
            if (!aShell.IsNull()) {
            ...
            if (aMapSolids.Extent() == 1) {
                ...
                if (aResShape.ShapeType() == TopAbs_SHELL && myCreateOpenSolidMode) {
                    ...
                    if (aResSol.ShapeType() == TopAbs_SHELL)
                        SendFail(
                            Message_Msg("FixAdvSolid.FixShell.MSG10")); // Solid can not be created from open shell.
                }
                Context()->Replace(aResShape, mySolid);
            } else if (aMapSolids.Extent() > 1) {
                SendWarning(
                    Message_Msg("FixAdvSolid.FixOrientation.MSG30")); // Bad connected solid a few solids were created.
                BRep_Builder aB;
                ...
                if (!aPSCreatingSolid.More()) return Standard_False; // aborted execution
