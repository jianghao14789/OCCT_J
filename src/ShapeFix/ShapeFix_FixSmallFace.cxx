    ...
    if (st == TopAbs_COMPOUND || st == TopAbs_COMPSOLID || st == TopAbs_SOLID || st == TopAbs_SHELL ||
        ...
        for (TopExp_Explorer itf(myShape, TopAbs_FACE); itf.More(); itf.Next()) {
            ...
            if (myAnalyzer.CheckStripFace(F, E1, E2, Precision())) {
                if (ReplaceInCaseOfStrip(F, E1, E2, Precision())) RemoveFacesInCaseOfStrip(F);
...
Standard_Boolean ShapeFix_FixSmallFace::ReplaceInCaseOfStrip(TopoDS_Face& F, TopoDS_Edge& E1, TopoDS_Edge& E2,
...
TopoDS_Edge ShapeFix_FixSmallFace::ComputeSharedEdgeForStripFace(const TopoDS_Face& /*F*/, const TopoDS_Edge& E1,
                                                                 const TopoDS_Edge& E2, const TopoDS_Face& F1,
                                                                 const Standard_Real tol) const {
    ...
    if ((dev <= BRep_Tool::Tolerance(V1)) || (dev <= BRep_Tool::Tolerance(V3)) || (dev <= tol)) {
        ...
        //	theFirstVer = TopoDS::Vertex(V1);
            ...
            // if(Context()->Status(V1, temp) != 0) theFirstVer = TopoDS::Vertex (temp); //If this vertex already
            // recorded in map else                                                                       //take
            // recorded vertex
        ...
        if (V1.IsSame(V2) || V3.IsSame(V4))
            ...
            if (!V2.IsSame(V4)) {
                ...
                // if(theRepVert->Status(V2, temp) != 0) theSecondVer = TopoDS::Vertex (temp); //If this vertex already
                // recorded in map else
        ...
        if ((dev <= BRep_Tool::Tolerance(V1)) || (dev <= BRep_Tool::Tolerance(V4)) || (dev <= tol)) {
            ...
            //	    theFirstVer = TopoDS::Vertex(V1);
                ...
                thePosition = (p1.XYZ() + p2.XYZ()) / 2;
                theBuilder.UpdateVertex(theFirstVer, gp_Pnt(thePosition), dev);
                //    if(theRepVert->Status(V1, temp) != 0) theFirstVer = TopoDS::Vertex (temp); //If this vertex
                //    already recorded in map
                //  else
