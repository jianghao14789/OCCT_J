...
void ShapeFix_Face::Init(const Handle(Geom_Surface) & surf, const Standard_Real preci, const Standard_Boolean fwd) {
    myStatus = 0;
...
void ShapeFix_Face::Init(const Handle(ShapeAnalysis_Surface) & surf, const Standard_Real preci,
    ...
    for (i = 1; i <= sewd->NbEdges(); i++) {
        ...
        for (j = 2; j <= sewd->NbEdges() && IsConnectedEdge; j++) {
            ...
            if (V1.IsSame(V0)) {
                ...
                gp_Pnt2d v0, v1;
                if (E1.Orientation() == TopAbs_REVERSED) a1 = b1;
                if (E2.Orientation() == TopAbs_REVERSED) b2 = a2;
                curve1->D0(a1, v0);
                curve2->D0(b2, v1);
...
Standard_Boolean ShapeFix_Face::Perform() {
        ...
        */
        ...
        for (TopoDS_Iterator iter(S, Standard_False); iter.More(); iter.Next()) {
            ...
            if (theAdvFixWire->Perform()) {
                ...
                fixed =
                    (theAdvFixWire->StatusSmall(ShapeExtend_DONE) || theAdvFixWire->StatusConnected(ShapeExtend_DONE) ||
    ...
    if (NeedFix(myFixPeriodicDegenerated)) this->FixPeriodicDegenerated();
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (NeedFix(myFixWireMode)) {
            ...
            for (TopoDS_Iterator iter(S, Standard_False); iter.More(); iter.Next()) {
                ...
                if (theAdvFixWire->StatusRemovedSegment()) NeedCheckSplitWire = Standard_True;
                ...
                if (NeedFix(myFixLoopWiresMode) && FixLoopWire(aLoopWires)) {
                    if (aLoopWires.Length() > 1)
                        SendWarning(wire,
                                    Message_Msg("FixAdvFace.FixLoopWire.MSG0")); // Wire was split on several wires
        ...
        if (NeedFix(myFixOrientationMode)) {
            if (FixOrientation(MapWires)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
        }
        ...
        if (NeedFix(myFixSplitFaceMode) && NeedSplit && MapWires.Extent() > 1) {
            if (FixSplitFace(MapWires)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE8);
        }
    }
    ...
    for (exp.Init(myResult, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        if (NeedFix(myFixSmallAreaWireMode, Standard_False)) {
            ...
            if (FixSmallAreaWire(isRemoveFace)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE4);
        }
    }
...
// to vector <vec>
static void Shift2dWire(const TopoDS_Wire w, const TopoDS_Face f, const gp_Vec2d vec,
                        const Handle(ShapeAnalysis_Surface) & mySurf, Standard_Boolean recompute3d = Standard_False) {
    gp_Trsf2d tr2d;
...
static Standard_Boolean CutInterval(TColgp_SequenceOfPnt2d& intervals, const gp_Pnt2d& toAddI,
...
Standard_Boolean ShapeFix_Face::FixAddNaturalBound() {
    ...
    if (ws.IsEmpty() && !IsSurfaceUVInfinite(mySurf->Surface())) {
        ...
        if (!Context().IsNull()) Context()->Replace(myFace, aNewFace);
    ...
    if (!IsSurfaceUVPeriodic(mySurf->Adaptor3d()) || ShapeAnalysis::IsOuterBound(myFace)) return Standard_False;
    ...
    for (i = 1; i <= nb; i++) {
        ...
        gp_Pnt2d sh(0., 0.);
        if (mySurf->IsUClosed()) sh.SetX(ShapeAnalysis::AdjustByPeriod(centers(i).X(), center.X(), SUL - SUF));
        if (mySurf->IsVClosed()) sh.SetY(ShapeAnalysis::AdjustByPeriod(centers(i).Y(), center.Y(), SVL - SVF));
...
Standard_Boolean ShapeFix_Face::FixOrientation(TopTools_DataMapOfShapeListOfShape& MapWires) {
    ...
    Standard_Boolean isAddNaturalBounds =
        (NeedFix(myFixAddNaturalBoundMode) && IsSurfaceUVPeriodic(mySurf->Adaptor3d()));
    ...
    if (nb == 1) {
        ...
        if ((myFixAddNaturalBoundMode != 1 || !IsSurfaceUVPeriodic(mySurf->Adaptor3d())) &&
    ...
    //  Evidemment, en cas de peau de leopard, il peut y avoir probleme
        ...
        for (i = 1; i <= nb; i++) {
            ...
            if (isFirst) {
                ...
                if (mySurf->IsUClosed()) xShift = ShapeAnalysis::AdjustByPeriod(0.5 * (aXMin + aXMax), uMiddle, uRange);
                if (mySurf->IsVClosed()) yShift = ShapeAnalysis::AdjustByPeriod(0.5 * (aYMin + aYMax), vMiddle, vRange);
            ...
            if (sta == TopAbs_UNKNOWN) {                                         // ERREUR
                ...
                if (sta == TopAbs_OUT) {
                    ...
                    if (staout == TopAbs_IN) {
                        ...
                        SendWarning(sewd.Wire(),
                                    Message_Msg("FixAdvFace.FixOrientation.MSG5")); // Wire on face was reversed
...
#ifdef OCCT_DEBUG
            std::cout << "Wire no " << aSeqReversed.Value(k) << " of " << nb << " reversed"
                      << std::endl; // mise au point !
#endif
        }
    }
    return done;
...
static Standard_Boolean CheckWire(const TopoDS_Wire& wire, const TopoDS_Face& face, const Standard_Real dU,
                                  const Standard_Real dV, Standard_Integer& isuopen, Standard_Integer& isvopen,
    ...
    for (TopoDS_Iterator ed(wire); ed.More(); ed.Next()) {
        ...
        Handle(Geom2d_Curve) c2d;
        Standard_Real f, l;
        if (!sae.PCurve(edge, face, c2d, f, l, Standard_True)) return Standard_False;
...
Standard_Boolean ShapeFix_Face::FixMissingSeam() {
    ...
    if (mySurf->Surface()->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) {
        ...
        if (!BSpl->IsUPeriodic() && !BSpl->IsVPeriodic()) return Standard_False;
    }
    ...
    for (i = 1; i <= ws.Length(); i++) {
        ...
        Standard_Boolean isdeg;
        if (!CheckWire(wire, myFace, URange, VRange, isuopen, isvopen, isdeg)) continue;
        if (w1.IsNull()) {
            w1 = wire;
            ismodeu = isuopen;
            ismodev = isvopen;
            ...
            if (ismodeu == -isuopen && ismodev == -isvopen) {
                ...
                std::cout << "Warning: ShapeFix_Face::FixMissingSeam(): incompatible open wires" << std::endl;
#endif
        }
    ...
    for (i = 1; i <= ws.Length(); i++) {
        ...
        if (wire.IsSame(w1))
            ...
            if (ShapeAnalysis::IsOuterBound(TopoDS::Face(curface))) wire.Reverse();
        }
        B.Add(tmpF, wire);
    }
    ...
    if (uclosed && vclosed) {
        Standard_Real shiftw2 = ShapeAnalysis::AdjustByPeriod(
            ...
            0.5 * (m1[coord][0] + m1[coord][1] + isneg * (period + ::Precision::PConfusion())), period);
    ...
    for (Standard_Integer i1 = 1; i1 <= nb1 + nb2; i1++) {
        ...
        for (Standard_Integer i2 = 1; i1 <= nb1 && i2 <= nb2; i2++) {
            ...
            if (uclosed && ismodeu) {
                ...
                if (Abs(pos2.X() - pos1.X()) < ::Precision::PConfusion() && (foundU != 2 || Abs(pos1.X()) < Abs(uf))) {
                    foundU = 2;
                    uf = pos1.X();
                }
            }
            if (vclosed && !ismodeu) {
                ...
                if (Abs(pos2.Y() - pos1.Y()) < ::Precision::PConfusion() && (foundV != 2 || Abs(pos1.Y()) < Abs(vf))) {
                    foundV = 2;
                    vf = pos1.Y();
                }
            }
    ...
    if (uf < SUF || uf > SUL) uf += ShapeAnalysis::AdjustToPeriod(uf, SUF, SUF + URange);
    if (vf < SVF || vf > SVL) vf += ShapeAnalysis::AdjustToPeriod(vf, SVF, SVF + VRange);
    ...
    for (; expF.More(); expF.Next()) {
        ...
        for (; aExpW.More(); aExpW.Next()) {
            ...
            if (aSfw.NbEdges()) aSfw.FixSmall(Standard_True, Precision());
            if (!aSfw.NbEdges()) {
...
Standard_Boolean ShapeFix_Face::FixSmallAreaWire(const Standard_Boolean theIsRemoveSmallFace) {
...
#endif
        if (theIsRemoveSmallFace && !Context().IsNull()) Context()->Remove(myFace);
    ...
    if (!Context().IsNull()) Context()->Replace(myFace, aFace);
...
// purpose  :
//=======================================================================
static void FindNext(const TopoDS_Shape& aVert, const TopoDS_Shape& ainitEdge, TopTools_IndexedMapOfShape& aMapVertices,
                     TopTools_DataMapOfShapeListOfShape& aMapVertexEdges, const TopTools_MapOfShape& aMapSmallEdges,
                     const TopTools_MapOfShape& aMapSeemEdges, TopTools_MapOfShape& aMapEdges,
    ...
    if (!isFind && !aMapSmallEdges.Contains(ainitEdge)) return;
    if (isFind && aMapVertices.Contains(anextVert)) return;
    ...
    for (; liter.More() && !isFind; liter.Next()) {
        if (!aMapEdges.Contains(liter.Value()) && !liter.Value().IsSame(ainitEdge)) {
            ...
            if (aMapSeemEdges.Contains(anextEdge)) aWireData->Add(anextEdge.Reversed());
            isFind = Standard_True;
            aMapEdges.Add(anextEdge);
            FindNext(anextVert, anextEdge, aMapVertices, aMapVertexEdges, aMapSmallEdges, aMapSeemEdges, aMapEdges,
                     aWireData);
        }
    }
    return;
...
Standard_Boolean ShapeFix_Face::FixLoopWire(TopTools_SequenceOfShape& aResWires) {
    ...
    for (; i <= aMapVertices.Extent(); i++) {
        ...
        for (; liter.More(); liter.Next()) {
            ...
            if (aMapEdges.Contains(Edge)) continue;

            Handle(ShapeExtend_WireData) aWireData = new ShapeExtend_WireData;
            aWireData->Add(Edge);
            if (aMapSeemEdges.Contains(Edge)) aWireData->Add(Edge.Reversed());
            aMapEdges.Add(Edge);
            FindNext(aVert, Edge, aMapVertices, aMapVertexEdges, aMapSmallEdges, aMapSeemEdges, aMapEdges, aWireData);
            if (aWireData->NbEdges() == 1 && aMapSmallEdges.Contains(aWireData->Edge(1))) continue;
    ...
    if (aSeqWires.Length() == 1) {
        ...
        for (i = 1; i <= aSeqWires.Length(); i++) {
            ...
            if (j <= aSeqWires.Length()) aSeqWires.Remove(i--);
        }
...
Standard_Boolean ShapeFix_Face::SplitEdge(const Handle(ShapeExtend_WireData) & sewd, const Standard_Integer num,
                                          const Standard_Real param, const TopoDS_Vertex& vert,
                                          const Standard_Real preci, ShapeFix_DataMapOfShapeBox2d& boxes) {
...
Standard_Boolean ShapeFix_Face::SplitEdge(const Handle(ShapeExtend_WireData) & sewd, const Standard_Integer num,
                                          const Standard_Real param1, const Standard_Real param2,
                                          const TopoDS_Vertex& vert, const Standard_Real preci,
...
static Standard_Boolean IsPeriodicConicalLoop(const Handle(Geom_ConicalSurface) & theSurf, const TopoDS_Wire& theWire,
                                              const Standard_Real theTolerance, Standard_Real& theMinU,
                                              Standard_Real& theMaxU, Standard_Real& theMinV, Standard_Real& theMaxV,
                                              Standard_Boolean& isUDecrease) {
    if (theSurf.IsNull()) return Standard_False;
    ...
    for (; aWireIter.More(); aWireIter.Next()) {
        ...
        if (aC2d.IsNull()) return Standard_False;
        ...
        if (aCurMinU < aMinU) aMinU = aCurMinU;
        if (aCurMaxU > aMaxU) aMaxU = aCurMaxU;
        if (aCurMinV < aMinV) aMinV = aCurMinV;
        if (aCurMaxV > aMaxV) aMaxV = aCurMaxV;
...
Standard_Boolean ShapeFix_Face::FixPeriodicDegenerated() {
    ...
    if (aNbWires != 1 || aSurface.IsNull() || aSurface->DynamicType() != STANDARD_TYPE(Geom_ConicalSurface))
        return Standard_False;
    ...
    Standard_Boolean isConicLoop = IsPeriodicConicalLoop(aConeSurf, aSoleWire, Precision(), aMinLoopU, aMaxLoopU,
                                                         aMinLoopV, aMaxLoopV, isUDecrease);

    if (!isConicLoop) return Standard_False;
    ...
    if (fabs(aSemiAngle) <= Precision::Confusion()) return Standard_False; // Bad surface
    ...
    if (anApexV < aMinLoopV) {
        ...
        if (!isUDecrease) aSoleWire.Reverse();
    }
