...
static Standard_Real AdjustOnPeriodic3d(const Handle(Geom_Curve) & c, const Standard_Boolean takefirst,
...
Standard_Boolean ShapeFix_Wire::FixGap3d(const Standard_Integer num, const Standard_Boolean convert) {
    ...
    gp_Pnt vpnt =
        (V1.IsSame(V2)) ? BRep_Tool::Pnt(V1) : gp_Pnt((BRep_Tool::Pnt(V1).XYZ() + BRep_Tool::Pnt(V2).XYZ()) * 0.5);
    ...
    if (convert) {
        ...
        if (cpnt1.Distance(vpnt) < preci && cpnt2.Distance(vpnt) < preci) return Standard_False;
        ...
        for (Standard_Integer j = 1; j <= nbcurv; j++) {
            ...
            if (c->IsKind(STANDARD_TYPE(Geom_BSplineCurve))) {
                ...
                if (segment) bsp = GeomConvert::SplitBSplineCurve(bsp, fbsp, lbsp, ::Precision::Confusion());
            } else if (c->IsKind(STANDARD_TYPE(Geom_Conic))) {
                Approx_Curve3d Conv(new GeomAdaptor_Curve(c, first, last), myAnalyzer->Precision(), GeomAbs_C1, 9,
                                    1000);
...
#ifdef OCCT_DEBUG
                    std::cout << "Warning: ShapeFix_Wire_1::FixGap3d: Exception in TrimmedCurve" << first << " " << last
                              << std::endl;
        ...
        if (n1 == n2) {
            ...
            if (c1->IsKind(STANDARD_TYPE(Geom_BSplineCurve)) || c1->IsKind(STANDARD_TYPE(Geom_BezierCurve))) {
            ...
            if (c2->IsKind(STANDARD_TYPE(Geom_BSplineCurve)) || c2->IsKind(STANDARD_TYPE(Geom_BezierCurve))) {
            ...
            if (p1.Distance(p2) <= gap && Abs(cfirst1 - u1) > ::Precision::PConfusion() &&
                ...
                (((u1 > first1) && (u1 < last1)) || ((u2 > first2) && (u2 < last2)) || (cpnt1.Distance(p1) <= gap) ||
...
static Standard_Real AdjustOnPeriodic2d(const Handle(Geom2d_Curve) & pc, const Standard_Boolean takefirst,
                                        const Standard_Real first, const Standard_Real last,
                                        const Standard_Real param) {
    // 15.11.2002 PTV OCC966
    ...
    if (convert) {
        ...
        for (Standard_Integer j = 1; j <= nbcurv; j++) {
            ...
            if (pc->IsKind(STANDARD_TYPE(Geom2d_BSplineCurve))) {
                ...
                if (segment) bsp = Geom2dConvert::SplitBSplineCurve(bsp, fbsp, lbsp, ::Precision::PConfusion());
                ...
                Approx_Curve2d Conv(new Geom2dAdaptor_Curve(pc, first, last), first, last, tolu, tolv, GeomAbs_C1, 9,
                                    1000);
                if (Conv.IsDone() || Conv.HasResult()) bsp = Conv.Curve();
            } else {
                // Restore trim for pcurve
                try {
                    ...
                    // 15.11.2002 PTV OCC966
                    if (!ShapeAnalysis_Curve::IsPeriodic(pc))
                        c = new Geom2d_TrimmedCurve(pc, Max(first, pc->FirstParameter()),
        ...
        if (n1 == n2) {
            ...
            // Determine domains for extremal points locating
            Standard_Real domfirst1 = first1, domlast1 = last1;
            if (pc1->IsKind(STANDARD_TYPE(Geom2d_BSplineCurve)) || pc1->IsKind(STANDARD_TYPE(Geom2d_BezierCurve))) {
                domfirst1 = pc1->FirstParameter();
                domlast1 = pc1->LastParameter();
            } else if (pc1->IsKind(STANDARD_TYPE(Geom2d_Line)) || pc1->IsKind(STANDARD_TYPE(Geom2d_Parabola)) ||
                ...
                if (reversed1)
                    ...
                    domlast1 += 10. * diff;
            } else if (pc1->IsKind(STANDARD_TYPE(Geom2d_Circle)) || pc1->IsKind(STANDARD_TYPE(Geom2d_Ellipse))) {
                domfirst1 = 0.;
                domlast1 = 2 * M_PI;
            }
            Standard_Real domfirst2 = first2, domlast2 = last2;
            if (pc2->IsKind(STANDARD_TYPE(Geom2d_BSplineCurve)) || pc2->IsKind(STANDARD_TYPE(Geom2d_BezierCurve))) {
                domfirst2 = pc2->FirstParameter();
                domlast2 = pc2->LastParameter();
            } else if (pc2->IsKind(STANDARD_TYPE(Geom2d_Line)) || pc2->IsKind(STANDARD_TYPE(Geom2d_Parabola)) ||
                ...
                if (reversed2)
                    ...
                    domfirst2 -= 10. * diff;
            } else if (pc2->IsKind(STANDARD_TYPE(Geom2d_Circle)) || pc2->IsKind(STANDARD_TYPE(Geom2d_Ellipse))) {
                domfirst2 = 0.;
                domlast2 = 2 * M_PI;
            }
            ...
            IntRes2d_Domain dom1(pc1->Value(domfirst1), domfirst1, tolint, pc1->Value(domlast1), domlast1, tolint);
            IntRes2d_Domain dom2(pc2->Value(domfirst2), domfirst2, tolint, pc2->Value(domlast2), domlast2, tolint);
                                ...
                                if (Inter.IsDone()) {
                                    if (Inter.NbPoints() || Inter.NbSegments()) {
                                        ...
                                        for (i = 1; i <= Inter.NbPoints(); i++) {
                                            ...
                                            uu = AdjustOnPeriodic2d(pc, (j == 1 ? reversed1 : !reversed2), fpar, lpar,
