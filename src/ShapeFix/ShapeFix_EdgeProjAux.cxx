...
ShapeFix_EdgeProjAux::ShapeFix_EdgeProjAux()
    : myFirstParam(0.0), myLastParam(0.0), myFirstDone(Standard_False), myLastDone(Standard_False) {}
...
static Standard_Boolean FindParameterWithExt(const gp_Pnt& Pt1, const Adaptor3d_CurveOnSurface& COnS,
                                             const Standard_Real Uinf, const Standard_Real Usup,
                                             const Standard_Real preci, Standard_Real& w1) {
    try { // et allez donc !
        OCC_CATCH_SIGNALS
...
void ShapeFix_EdgeProjAux::Init2d(const Standard_Real preci) {
    ...
    if (Precision::IsInfinite(dist)) return;
    ...
    if (fabs(w1 - w2) < Precision::PConfusion()) {
        if (!theSurface->IsUPeriodic() && !theSurface->IsVPeriodic()) return;
    }
    ...
    if (COnS.Value(Uinf).Distance(COnS.Value(Usup)) < Precision::Confusion()) {
        ...
        if (Abs(myFirstParam - Uinf) < ::Precision::PConfusion() && Abs(myLastParam - Uinf) < ::Precision::PConfusion())
            myLastParam = w2 = Usup;
        // 18.11.2002 SKL OCC630 compare values with tolerance Precision::PConfusion() instead of "=="
...
void ShapeFix_EdgeProjAux::Init3d(const Standard_Real preci) {
    ...
    if ((!Precision::IsInfinite(w1) && !Precision::IsInfinite(w2) && theCurve2d->Continuity() != GeomAbs_C0) ||
        IsIso(theCurve2d)) {
