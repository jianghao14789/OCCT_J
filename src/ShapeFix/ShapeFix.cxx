...
Standard_Boolean ShapeFix::SameParameter(const TopoDS_Shape& shape, const Standard_Boolean enforce,
                                         const Standard_Real preci, const Message_ProgressRange& theProgress,
...

    {
        ...
        for (TopExp_Explorer exp(shape, TopAbs_FACE); exp.More() && aPS.More(); exp.Next(), aPS.Next()) {
            ...
            if (plane.IsNull()) {
                Handle(Geom_RectangularTrimmedSurface) GRTS = Handle(Geom_RectangularTrimmedSurface)::DownCast(Surf);
                if (!GRTS.IsNull()) plane = Handle(Geom_Plane)::DownCast(GRTS->BasisSurface());
...
TopoDS_Shape ShapeFix::RemoveSmallEdges(TopoDS_Shape& Shape, const Standard_Real Tolerance,
...
//=======================================================================
static TopoDS_Edge ReplaceVertex(const TopoDS_Edge& theEdge, const gp_Pnt theP, const Standard_Boolean theFwd) {
...
// purpose  : auxiliary for FixVertexPosition
//=======================================================================
static Standard_Real getNearPoint(const TColgp_SequenceOfPnt& aSeq1, const TColgp_SequenceOfPnt& aSeq2, gp_XYZ& acent) {
    ...
    for (; i <= aSeq1.Length(); i++) {
        ...
        for (; j <= aSeq2.Length(); j++) {
            ...
            if (fabs(d - mindist) <= Precision::Confusion()) continue;
            if (d < mindist) {
                mindist = d;
                ind1 = i;
                ind2 = j;
    ...
    if (ind1 && ind2) acent = (aSeq1.Value(ind1).XYZ() + aSeq2.Value(ind2).XYZ()) / 2.0;
    return mindist;
}
...
// purpose  : auxiliary for FixVertexPosition
//=======================================================================
static Standard_Boolean getNearestEdges(TopTools_ListOfShape& theLEdges, const TopoDS_Vertex theVert,
                                        TopTools_SequenceOfShape& theSuitEdges,
                                        TopTools_SequenceOfShape& theRejectEdges, const Standard_Real theTolerance,
                                        gp_XYZ& thecentersuit, gp_XYZ& thecenterreject) {
    if (theLEdges.IsEmpty()) return Standard_False;
    ...
    if (!aCurve1.IsNull()) {
        ...
        if (isSame1) p12 = aCurve1->Value(aLast1);
    } else
    ...
    for (; alIter.More();) {
        ...
        Standard_Boolean isLoop =
            ((aVert1.IsSame(aVert11) && aVert2.IsSame(aVert12)) || (aVert1.IsSame(aVert12) && aVert2.IsSame(aVert11)));
        ...
        if (!aCurve.IsNull()) {
            ...
            if (isSame) p2 = aCurve->Value(aLast);
            ...
            if (!isSame && !isSame1) {
                ...
                aSeq1.Append(p11);
                if (isSame1) aSeq1.Append(p12);
                aSeq2.Append(p1);
                if (isSame) aSeq2.Append(p2);
                aMinDist = getNearPoint(aSeq1, aSeq2, acent);
            }

            if (aMinDist > theTolerance) {
                if (!aseqreject.Length()) thecenterreject = acent;
    ...
    if (isDone) {
        if (aseqsuit.IsEmpty()) {
            ...
            getNearestEdges(theLEdges, theVert, theSuitEdges, theRejectEdges, theTolerance, thecentersuit,
                            thecenterreject);
        } else {
...
Standard_Boolean ShapeFix::FixVertexPosition(TopoDS_Shape& theshape, const Standard_Real theTolerance,
    ...
    for (; i <= aMapVertEdges.Extent(); i++) {
        ...
        if (aTolVert <= theTolerance) continue;
        ...
        if (aledges.Extent() == 1) continue;
        // if tolerance of vertex is more than specified tolerance
        //  check distance between curves and vertex

        if (!getNearestEdges(aledges, aVert, aSuitEdges, aRejectEdges, theTolerance, acenter, acenterreject)) continue;
        ...
        for (k = 1; k <= aRejectEdges.Length(); k++) {
            ...
            if (!aCurve.IsNull()) {
                ...
                if (isFirst) {
                    enew = ReplaceVertex(aEdge, p1, Standard_True);
                    if (isSame) enew = ReplaceVertex(enew, p2, Standard_False);
                } else {
                    enew = ReplaceVertex(aEdge, p2, Standard_False);
                    if (isSame) enew = ReplaceVertex(enew, p1, Standard_True);
                }
    ...
    if (isDone) theshape = thecontext->Apply(theshape);
    return isDone;
}
