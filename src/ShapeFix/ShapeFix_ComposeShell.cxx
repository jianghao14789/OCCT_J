...
ShapeFix_ComposeShell::ShapeFix_ComposeShell()
    : myOrient(TopAbs_FORWARD), myStatus(0), myUResolution(RealLast()), myVResolution(RealLast()),
      myInvertEdgeStatus(Standard_True), myClosedMode(Standard_False), myUClosed(Standard_False),
      myVClosed(Standard_False), myUPeriod(0.), myVPeriod(0.) {
...
void ShapeFix_ComposeShell::Init(const Handle(ShapeExtend_CompositeSurface) & Grid, const TopLoc_Location& L,
    ...
    if (theSurface->IsKind(STANDARD_TYPE(Geom_ElementarySurface))) {
        ...
        if (myUClosed) {
            ...
            if (P0.Distance(P1) > Precision::Confusion() * 10) myUClosed = Standard_False;
        }
        if (myVClosed) {
            ...
            if (P0.Distance(P1) > Precision::Confusion() * 10) myVClosed = Standard_False;
        }
    }
    //  DTK-CKY 100531 end
...
static Standard_Integer PointLinePosition(const gp_Pnt2d& p, const gp_Lin2d& line, Standard_Real& dev) {
...
static Standard_Real ParamPointsOnLine(const gp_Pnt2d& p1, const gp_Pnt2d& p2, const gp_Lin2d& line) {
    ...
    if (Abs(dist2) < ::Precision::PConfusion()) return ParamPointOnLine(p2, line);
    // just protection
    if (dist2 * dist1 > 0) return 0.5 * (ParamPointOnLine(p1, line) + ParamPointOnLine(p2, line));
    // else compute intersection
    return (ParamPointOnLine(p1, line) * dist2 - ParamPointOnLine(p2, line) * dist1) / (dist2 - dist1);
}
...
static Standard_Integer ApplyContext(ShapeFix_WireSegment& wire, const Standard_Integer iedge,
    ...
    if (segw->NbEdges() > 0) {
        ...
        std::cout << "Warning: ShapeFix_ComposeShell, ApplyContext: edge is to remove - not implemented" << std::endl;
#endif
...
static inline Standard_Boolean IsCoincided(const gp_Pnt2d& p1, const gp_Pnt2d& p2, const Standard_Real UResolution,
                                           const Standard_Real VResolution, const Standard_Real tol) {
    ...
    return Abs(p1.X() - p2.X()) <= Max(TOLINT, UTolerance) && Abs(p1.Y() - p2.Y()) <= Max(TOLINT, VTolerance);
}
...
static Standard_Integer GetPatchIndex(const Standard_Real Param, const Handle(TColStd_HArray1OfReal) & Params,
    ...
    if (isClosed) shift = ShapeAnalysis::AdjustToPeriod(Param, Params->Value(1), Params->Value(NP));
            ...
            if (nbMEdges) {
                ...
                if (gs->IsUPeriodic() && gs->IsVPeriodic()) {
                    ...
                    for (Standard_Integer i = 1; i <= nbMEdges; i++) {
                        ...
                        if (!sae.PCurve(sbwdM->Edge(i), TopoDS::Face(tmpF), c2d, f, l)) continue;
                ...
                sfw->FixReorder();
                if (sfw->StatusReorder(ShapeExtend_DONE3)) stat = -1;
                ...
                if (stat < 0) {
                    ...
                    if (isOuter != isOuterAfter) sbwdM->Reverse(face);
                }
...
Standard_Integer ShapeFix_ComposeShell::ComputeCode(const Handle(ShapeExtend_WireData) & wire, const gp_Lin2d& line,
                                                    const Standard_Integer begInd, const Standard_Integer endInd,
                                                    const Standard_Real begPar, const Standard_Real endPar,
    ...
    Standard_Integer special =
        (begInd == endInd && (wire->Edge(begInd).Orientation() == TopAbs_FORWARD ||
                              wire->Edge(begInd).Orientation() == TopAbs_INTERNAL) == (begPar > endPar)
             ? 1
             : 0);
    if (!special && begInd == endInd && begPar == endPar && (myClosedMode || isInternal)) special = 1;
...
static void DistributeSplitPoints(const Handle(ShapeExtend_WireData) & sbwd, const TopoDS_Face myFace,
                                  const Standard_Integer index, const Standard_Integer nsplit,
                                  TColStd_SequenceOfInteger& indexes, const TColStd_SequenceOfReal& values) {
...
// purpose  : auxiliary
//=======================================================================
static Standard_Integer CheckByCurve3d(const gp_Pnt& pos, const Handle(Geom_Curve) & c3d, const Standard_Real param,
                                       const gp_Trsf& T, const Standard_Real tol) {
...
// purpose  : auxiliary
//=======================================================================
static void DefinePatch(ShapeFix_WireSegment& wire, const Standard_Integer code, const Standard_Boolean isCutByU,
                        const Standard_Integer cutIndex, const Standard_Integer number = -1) {
...
ShapeFix_WireSegment ShapeFix_ComposeShell::SplitWire(ShapeFix_WireSegment& wire, TColStd_SequenceOfInteger& indexes,
    ...
    for (Standard_Integer i = 1; i <= wire.NbEdges(); i++) {
        ...
        for (; aIt.More(); aIt.Next()) {
            if (aIt.Value().Orientation() != TopAbs_FORWARD && aIt.Value().Orientation() != TopAbs_REVERSED)
        ...
        for (Standard_Integer j = start; j <= stop; prevPar = currPar, j++) {
            ...
            if (Abs(currPar - lastPar) < ::Precision::PConfusion()) {
                ...
                if (currPnt.Distance(lastVPnt) <= lastVTol &&
                    ...
                    CheckByCurve3d(lastVPnt, c3d, f3d + (currPar - firstPar) * (l3d - f3d) / span2d, T,
                    ...
                    if (IsCoincided(lastPnt2d, currPnt2d, uRes, vRes, lastVTol) &&
                        ...
                        doCut = Standard_False;
                           ...
                           // tolerance is covered by distance of the edge curve from vertex point.
                           // Doubled to prevent edge being fully covered by its vertices tolerance (invalid edge).
                           CheckByCurve3d(prevVPnt, c3d, f3d + (currPar - firstPar) * (l3d - f3d) / span2d, T,
            ...
            if (!ismanifold) anInitEdge.Orientation(TopAbs_FORWARD);
            ...
            if (!sp && !BRep_Tool::Degenerated(newEdge)) B.SameRange(newEdge, Standard_False);
        ...
        if (splitted) {
            ...
            if (anWireOrient == TopAbs_INTERNAL && code == 0) {
                ShapeBuild_Edge sbe;
                if (edge.Orientation() == TopAbs_INTERNAL) edge.Orientation(TopAbs_FORWARD);
...
Standard_Boolean ShapeFix_ComposeShell::SplitByLine(ShapeFix_WireSegment& wire, const gp_Lin2d& line,
    ...
    if (wire.IsVertex()) {
        ...
        if (code != IOR_UNDEF) return Standard_False;
    ...
    for (Standard_Integer iedge = 1; iedge <= nbe; iedge++) {
        ...
        if (iedge == nbe && wire.Orientation() != TopAbs_EXTERNAL && wire.Orientation() != TopAbs_INTERNAL &&
    ...
    if (myClosedMode && j > 1) {
        for (i = 1; i <= IntEdgePar.Length();) {
            if (i == j) break;
            if (IntEdgeInd(i) == IntEdgeInd(j) && Abs(IntEdgePar(i) - IntEdgePar(j)) < ::Precision::PConfusion()) {
                IntLinePar.Remove(i);
                IntEdgePar.Remove(i);
                IntEdgeInd.Remove(i);
                if (j > i) j--;
    ...
    for (i = 1; i <= IntEdgePar.Length(); i++) {
        ...
        Standard_Integer code =
            ComputeCode(sewd, line, IntEdgeInd(i), IntEdgeInd(j), IntEdgePar(i), IntEdgePar(j), isnonmanifold);
    ...
    // For real (closed) wire, analyze tangencies
        ...
        for (i = 1; i <= IntEdgePar.Length(); j = i++) {
            ...
            if (myClosedMode) {
                ...
                if (IntEdgeInd(i) == IntEdgeInd(j)) aNewSegCodes.Append(codej);
...
void ShapeFix_ComposeShell::SplitByLine(ShapeFix_SequenceOfWireSegment& wires, const gp_Lin2d& line,
                                        const Standard_Boolean isCutByU, const Standard_Integer cutIndex) {
    TColStd_SequenceOfReal SplitLinePar;
    TColStd_SequenceOfInteger SplitLineCode;
    TopTools_SequenceOfShape SplitLineVertex;
...
//      myStatus |= ShapeExtend::EncodeStatus ( ShapeExtend_FAIL4 );
        ...
        if (aMaxTol <= 2. * Precision::Confusion()) aMaxTol = Precision::Infinite();
        ...
        if (SplitLinePar(i) - SplitLinePar(i - 1) < ::Precision::PConfusion() ||
            (canbeMerged && (aD <= (aTol1 * aTol1) || aD <= (aTol2 * aTol2)))) { // BRepTools::Compare(V1, V2)) ) {
        ...
        if (!isCutByU) {
            Standard_Real shiftU = (myClosedMode && myUClosed
                                        ? ShapeAnalysis::AdjustToPeriod(SplitLinePar(i - 1) - TOLINT,
                                                                        myGrid->UJointValue(1), myGrid->UJointValue(2))
                                        : 0.);
            ...
            Standard_Real shiftV = (myClosedMode && myVClosed
                                        ? ShapeAnalysis::AdjustToPeriod(SplitLinePar(i - 1) - TOLINT,
                                                                        myGrid->VJointValue(1), myGrid->VJointValue(2))
                                        : 0.);
...
void ShapeFix_ComposeShell::SplitByGrid(ShapeFix_SequenceOfWireSegment& seqw) {
    ...
    // value of precision to define number of patch should be the same as used in the definitin position of point
    // relatively to seam edge (TOLINT)
    Standard_Real pprec = TOLINT; //::Precision::PConfusion();
    Standard_Integer i = 1;
    if (myClosedMode) {
        // for closed mode when only one patch exist and location of the splitting line is coincident with first joint
        // value Therefore in this case it is necessary to move all wire segments in the range of the patch between
        // first and last joint values. Then all wire segments are lie between -period and period in order to have valid
        // split ranges after splitting. Because for closed mode cut index always equal to 1 and parts of segments after
        // splitting always should have index either (0,1) or (1,2).
        for (i = 1; i <= seqw.Length(); i++) {
            ...
            // for closed mode it is necessary to move wire segment in the interval defined by first and last grid UV
            // values
            Standard_Real shiftU =
                (myClosedMode && myUClosed
                     ? ShapeAnalysis::AdjustToPeriod(Ul1 - pprec, myGrid->UJointValue(1), myGrid->UJointValue(2))
                     : 0.);
            Standard_Real shiftV =
                (myClosedMode && myVClosed
                     ? ShapeAnalysis::AdjustToPeriod(Vl1 - pprec, myGrid->VJointValue(1), myGrid->VJointValue(2))
                     : 0.);
            Uf1 += shiftU;
            Ul1 += shiftU;
            Vf1 += shiftV;
            ...
            // in same cases for example trj4_pm2-ug-203.stp (entity #8024) wire in 2D space has length greater then
            // period
...
void ShapeFix_ComposeShell::BreakWires(ShapeFix_SequenceOfWireSegment& seqw) {
    ...
    for (i = 1; i <= seqw.Length(); i++) {
        ...
        if (ori_wire != TopAbs_EXTERNAL && ori_wire != TopAbs_INTERNAL) continue;
    ...
    for (i = 1; i <= seqw.Length(); i++) {
        ...
        if (j > 1 && !myClosedMode && wire.IsClosed()) {
            ...
            if (!splitVertices.Contains(V)) shift = j - 1;
            // wire.SetLast ( j-1 );
        }
...
static Standard_Integer IsShortSegment(const ShapeFix_WireSegment& seg, const TopoDS_Face myFace,
                                       const Handle(Geom_Surface) & myGrid, const TopLoc_Location& myLoc,
                                       const Standard_Real UResolution, const Standard_Real VResolution) {
...
// purpose  : auxiliary
//=======================================================================
static Standard_Boolean IsSamePatch(const ShapeFix_WireSegment wire, const Standard_Integer NU,
                                    const Standard_Integer NV, Standard_Integer& iumin, Standard_Integer& iumax,
                                    Standard_Integer& ivmin, Standard_Integer& ivmax,
...
void ShapeFix_ComposeShell::CollectWires(ShapeFix_SequenceOfWireSegment& wires, ShapeFix_SequenceOfWireSegment& seqw) {
    ShapeAnalysis_Edge sae;
    Standard_Integer i; // svv #1
...
#endif
        Standard_Integer isshort = IsShortSegment(seqw(i), myFace, myGrid, myLoc, myUResolution, myVResolution);
        shorts.SetValue(i, isshort);
        if (isshort > 0 && (seqw(i).Orientation() == TopAbs_EXTERNAL ||
    ...
    for (;;) {
        ...
        for (i = 1; i <= seqw.Length(); i++) {
            ...
            for (Standard_Integer j = 0; j < 2; j++) {
                ...
                if (w1 + tail1 <= weigth + tail2) continue;
        ...
        if (index) {
            if (misoriented) myInvertEdgeStatus = Standard_True;
            ShapeFix_WireSegment seg = seqw.Value(index);
            if (sbwd.IsNull())
                sbwd = new ShapeExtend_WireData;
            else if (samepatch) { // extend patch indices
                IsSamePatch(seg, myGrid->NbUPatches(), myGrid->NbVPatches(), iumin, iumax, ivmin, ivmax, Standard_True);
            }
            ...
            if (myClosedMode) seg.GetPatchIndex(1, iumin, iumax, ivmin, ivmax);
        ...
        if (doupdate) {
            ...
            for (k = sbwd->NbEdges(); k >= 1; k--)
                ...
                if (sae.GetEndTangent2d(sbwd->Edge(k), myFace, Standard_True, endPnt, endTan, 1.e-3)) break;
        ...
        if (!index || (canBeClosed && !lastEdge.IsSame(firstEdge) && // cylinder (seam)
    ...
    for (i = 1; i <= seqw.Length(); i++) {
        ...
        for (Standard_Integer j = 1; j <= wires.Length(); j++) {
            ...
            for (Standard_Integer k = 1; k <= sbwd->NbEdges(); k++) {
                if (!V.IsSame(sae.FirstVertex(sbwd->Edge(k))))
                    continue; // pdn I suppose that short segment should be inserted into the SAME vertex.

                Standard_Boolean sp =
                    IsSamePatch(wires(j), myGrid->NbUPatches(), myGrid->NbVPatches(), iumin, iumax, ivmin, ivmax);
...
void ShapeFix_ComposeShell::MakeFacesOnPatch(TopTools_SequenceOfShape& faces, const Handle(Geom_Surface) & surf,
    ...
    if (loops.Length() == 1) {
        ...
        if (aSH.ShapeType() != TopAbs_WIRE) return;
...
void ShapeFix_ComposeShell::DispatchWires(TopTools_SequenceOfShape& faces,
    ...
    if (myClosedMode) {
            ...
            for (Standard_Integer jL = 1; jL <= sbwd->NbEdges(); jL++) {
                ...
                if (E.Orientation() == TopAbs_REVERSED && BRep_Tool::IsClosed(E, myFace)) {
                    ...
                    if (c21 == c22 || pf1.SquareDistance(pf2) < dPreci || pl1.SquareDistance(pl2) < dPreci) {
                        gp_Vec2d shift(0., 0.);
                        if (myUClosed && Abs(pf2.X() - pl2.X()) < ::Precision::PConfusion()) shift.SetX(myUPeriod);
                        if (myVClosed && Abs(pf2.Y() - pl2.Y()) < ::Precision::PConfusion()) shift.SetY(myVPeriod);
                        c22->Translate(shift);
                    }
                }
            }
        ...
        for (i = 1; i <= wires.Length(); i++) {
            ...
            ShapeBuild_Edge sbe;
            for (Standard_Integer jL = 1; jL <= sbwd->NbEdges(); jL++) {
                if (BRep_Tool::Degenerated(sbwd->Edge(jL))) sbe.RemovePCurve(sbwd->Edge(jL), myFace);
    ...
    for (i = 1; i <= nb; i++) {
        ...
        for (Standard_Integer j = 1; j <= sewd->NbEdges(); j++) {
            ...
            if (rs.IsRecorded(edge)) {
                ...
                if (!ismanifold) newEdge.Orientation(edge.Orientation());
    ...
    for (;;) {
        ...
        for (i = 1; i <= nb; i++) {
            ...
            if (aSeg.IsVertex()) {
                ...
                if (aVert.Orientation() == TopAbs_INTERNAL) loops.Append(wires(i).GetVertex());
            } else {
                Handle(ShapeExtend_WireData) aWD = aSeg.WireData();
                if (!aWD.IsNull()) loops.Append(aWD->Wire());
            }
        }
