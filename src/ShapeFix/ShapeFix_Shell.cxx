...
Standard_Boolean ShapeFix_Shell::Perform(const Message_ProgressRange& theProgress) {
    ...
    if (NeedFix(myFixOrientationMode)) FixFaceOrientation(TopoDS::Shell(newsh), Standard_True, myNonManifold);
    ...
    if (status) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (Status(ShapeExtend_DONE2)) status = Standard_True;
    return status;
}
...
//=======================================================================
static Standard_Boolean GetShells(TopTools_SequenceOfShape& Lface, const TopTools_MapOfShape& aMapMultiConnectEdges,
                                  TopTools_SequenceOfShape& aSeqShells, TopTools_DataMapOfShapeShape& aMapFaceShells,
    ...
    for (; i <= Lface.Length(); i++) {
        ...
        for (TopExp_Explorer expe(F1, TopAbs_EDGE); expe.More(); expe.Next()) {
            ...
            if (isMultiConnex && aMapMultiConnectEdges.Contains(edge)) continue;
        ...
        if (!nbbe && !nbe && dtemp.IsEmpty() && rtemp.IsEmpty()) continue;
    ...
    if (!isContains) {
        ...
        if (numFace > 1) {
            ...
            if (!isMultiConnex) nshell.Closed(BRep_Tool::IsClosed(nshell));
            aSeqShells.Append(nshell);
        } else if (numFace == 1) {
            if (aMapFaceShells.IsBound(aFace)) aMapFaceShells.UnBind(aFace);
            Lface.Append(aFace);
        }
    }
...
static Standard_Boolean AddMultiConexityFaces(TopTools_SequenceOfShape& Lface,
                                              ...
                                              TopTools_SequenceOfShape& ErrFaces, const Standard_Boolean NonManifold) {
    Standard_Boolean done = Standard_False;
    //  BRep_Builder aB;
            ...
            for (Standard_Integer kk = 1; kk <= aTmpShells.Length(); kk++) {
                ...
                if (GetFreeEdges(aSh, mapEdges)) {
                    ...
                    for (TopTools_MapIteratorOfMapOfShape amapIter(mapEdges); amapIter.More(); amapIter.Next()) {
                        if (aMapMultiConnectEdges.Contains(amapIter.Key())) nbedge++;
                    }
                    if (nbedge && nbedge == mapEdges.Extent()) AddShapes.Append(aSh);
                }
            }
        }
    }
    ...
    for (Standard_Integer k1 = 1; k1 <= AddShapes.Length(); k1++) {
        ...
        for (TopTools_ListIteratorOfListOfShape aItl(lfaces); aItl.More(); aItl.Next()) {
            ...
            if (!NonManifold && BRep_Tool::IsClosed(aOthershell)) continue;
        ...
        for (TopExp_Explorer aEt(aSh, TopAbs_FACE); aEt.More(); aEt.Next()) {
            for (Standard_Integer kk = 1; kk <= Lface.Length(); kk++) {
                if (aEt.Current().IsSame(Lface.Value(kk))) Lface.Remove(kk--);
            }
        }
    }
    return done;
...
static Standard_Integer BoxIn(const Bnd_Box& theBox1, const Bnd_Box& theBox2) {
    ...
    if (aXmin1 == aXmin2 && aXmax1 == aXmax2 && aYmin1 == aYmin2 && aYmax1 == aYmax2 && aZmin1 == aZmin2 &&
        aZmax1 == aZmax2)
        aNumIn = 0;
    else if (aXmin1 >= aXmin2 && aXmax1 <= aXmax2 && aYmin1 >= aYmin2 && aYmax1 <= aYmax2 && aZmin1 >= aZmin2 &&
             aZmax1 <= aZmax2)
        aNumIn = 1;
    else if (aXmin1 <= aXmin2 && aXmax1 >= aXmax2 && aYmin1 <= aYmin2 && aYmax1 >= aYmax2 && aZmin1 <= aZmin2 &&
             aZmax1 >= aZmax2)
...
static void GetClosedShells(TopTools_SequenceOfShape& Shells, TopTools_SequenceOfShape& aRemainShells) {
    ...
    for (Standard_Integer i1 = 1; i1 <= Shells.Length(); i1++) {
        if (!aMapNum.Contains(i1)) aRemainShells.Append(Shells.Value(i1));
    }
}
//=======================================================================
...
//=======================================================================
static void GlueClosedCandidate(TopTools_SequenceOfShape& OpenShells, const TopTools_MapOfShape& aMapMultiConnectEdges,
    ...
    for (Standard_Integer i = 1; i < OpenShells.Length(); i++) {
        ...
        if (!MapOtherShells.IsEmpty()) {
            ...
            for (NCollection_DataMap<TopoDS_Shape, Standard_Boolean, TopTools_ShapeMapHasher>::Iterator aIt(
                     MapOtherShells);
                 aIt.More(); aIt.Next()) {
...
static void CreateClosedShell(TopTools_SequenceOfShape& OpenShells, const TopTools_MapOfShape& aMapMultiConnectEdges) {
...
Standard_Boolean ShapeFix_Shell::FixFaceOrientation(const TopoDS_Shell& shell,
    ...
    for (TopoDS_Iterator iter(shell); iter.More(); iter.Next(), nbF++) {
        if (aMapAdded.Add(iter.Value())) Lface.Append(iter.Value());
    }
    if (Lface.Length() < nbF) done = Standard_True;
    ...
    for (Standard_Integer k = 1; k <= aMapEdgeFaces.Extent(); k++) {
        ...
        if (!isFreeBoundaries && aFaceCount == 1) {
            ...
            if (!BRep_Tool::Degenerated(E)) isFreeBoundaries = Standard_True;
        }
    ...
    while (isGetShells && Lface.Length()) {
        ...
        if (isGetShells) aSeqShells.Append(aTmpSeqShells);
    }
    if (!done) done = (aSeqShells.Length() > 1);
    Standard_Boolean aIsDone = Standard_False;
    if (Lface.Length() > 0 && aSeqShells.Length()) {
        for (Standard_Integer jj = 1; jj <= Lface.Length(); jj++) {
            if (aMapFaceShells.IsBound(Lface.Value(jj))) aMapFaceShells.UnBind(Lface.Value(jj));
        }

        // Addition of faces having only multiconnexity boundary to shells having holes
        //  containing only the multiconnexity edges
        aIsDone = AddMultiConexityFaces(Lface, aMapMultiConnectEdges, aSeqShells, aMapFaceShells, aMapEdgeFaces,
                                        aErrFaces, NonManifold);
    }
    ...
    if (!aErrFaces.IsEmpty()) {
        ...
        SendWarning(Message_Msg(
            "FixAdvShell.FixOrientation.MSG20")); // Impossible to orient faces in shell, several shells created
    ...
    if (!done) done = (aSeqShells.Length() > 1 || aIsDone);
    ...
    if (done) {
        ...
        if (!Context().IsNull()) Context()->Replace(shell, myShape);
        if (myNbShells == 1)
            SendWarning(Message_Msg(
                "FixAdvShell.FixOrientation.MSG0")); // Faces were incorrectly oriented in the shell, corrected
        else
