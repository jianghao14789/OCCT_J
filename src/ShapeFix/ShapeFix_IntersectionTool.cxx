...
ShapeFix_IntersectionTool::ShapeFix_IntersectionTool(const Handle(ShapeBuild_ReShape) & context,
                                                     const Standard_Real preci, const Standard_Real maxtol) {
    myContext = context;
    myPreci = preci;
    myMaxTol = maxtol;
}
...
Standard_Boolean ShapeFix_IntersectionTool::SplitEdge(const TopoDS_Edge& edge, const Standard_Real param,
                                                      const TopoDS_Vertex& vert, const TopoDS_Face& face,
                                                      TopoDS_Edge& newE1, TopoDS_Edge& newE2,
    ...
    if (V1.IsSame(vert) || V2.IsSame(vert)) return Standard_False;

    Handle(Geom2d_Curve) c2d;
    sae.PCurve(edge, face, c2d, a, b, Standard_True);
    if (Abs(a - param) < 0.01 * preci || Abs(b - param) < 0.01 * preci) return Standard_False;
    ...
    Handle(ShapeAnalysis_TransferParametersProj) transferParameters = new ShapeAnalysis_TransferParametersProj;
...
Standard_Boolean ShapeFix_IntersectionTool::CutEdge(const TopoDS_Edge& edge, const Standard_Real pend,
                                                    const Standard_Real cut, const TopoDS_Face& face,
...
Standard_Boolean ShapeFix_IntersectionTool::SplitEdge1(const Handle(ShapeExtend_WireData) & sewd,
                                                       const TopoDS_Face& face, const Standard_Integer num,
...
Standard_Boolean ShapeFix_IntersectionTool::UnionVertexes(const Handle(ShapeExtend_WireData) & sewd, TopoDS_Edge& edge1,
                                                          TopoDS_Edge& edge2, const Standard_Integer num2,
    ...
    if (d11 < d12 && d11 < d21 && d11 < d22) {
        ...
        if (!V2F.IsSame(V1F) && d11 < tolv) {
            ...
            if (V21F.IsSame(V2F)) {
                ...
                if (boxes.IsBound(edge21)) boxes.Bind(NewE, boxes.Find(edge21)); // update boxes
            ...
            if (V22F.IsSame(V2F)) {
                ...
                if (boxes.IsBound(edge22)) boxes.Bind(NewE, boxes.Find(edge22)); // update boxes
...
// purpose  : auxiliary
//=======================================================================
static Bnd_Box2d CreateBoxes2d(const Handle(ShapeExtend_WireData) & sewd, const TopoDS_Face& face,
                               ShapeFix_DataMapOfShapeBox2d& boxes) {
...
// purpose  : auxiliary
//=======================================================================
static void SelectIntPnt(const Geom2dInt_GInter& Inter, IntRes2d_IntersectionPoint& IP, IntRes2d_Transition& Tr1,
...
Standard_Boolean ShapeFix_IntersectionTool::FindVertAndSplitEdge(
    const Standard_Real param1, const TopoDS_Edge& edge1, const TopoDS_Edge& edge2, const Handle(Geom2d_Curve) & Crv1,
    Standard_Real& MaxTolVert, Standard_Integer& num1, const Handle(ShapeExtend_WireData) & sewd,
    const TopoDS_Face& face, ShapeFix_DataMapOfShapeBox2d& boxes, const Standard_Boolean aTmpKey) const {
...
Standard_Boolean ShapeFix_IntersectionTool::FixSelfIntersectWire(Handle(ShapeExtend_WireData) & sewd,
                                                                 const TopoDS_Face& face, Standard_Integer& NbSplit,
                ...
                if (Inter.NbPoints() > 0 && Inter.NbPoints() < 3) {
                    ...
                    if (Tr1.PositionOnCurve() == IntRes2d_Middle && Tr2.PositionOnCurve() == IntRes2d_Middle) {
                        ...
                        if (!ModifE1 && !ModifE2) {
                            ...
                            if (isEdgeSplit2) continue;
                        }
                    }
                    if (Tr1.PositionOnCurve() == IntRes2d_Middle && Tr2.PositionOnCurve() != IntRes2d_Middle) {
                        // find needed vertex from edge2 and split edge1 using it
                        Standard_Real param1 = IP.ParamOnFirst();
                        if (FindVertAndSplitEdge(param1, edge1, edge2, Crv1, MaxTolVert, num1, sewd, face, boxes,
                                                 Standard_False)) {
                            NbSplit++;
                            break;
                        }
                    ...
                    if (Tr1.PositionOnCurve() != IntRes2d_Middle && Tr2.PositionOnCurve() == IntRes2d_Middle) {
                        ...
                        Standard_Real param2 = IP.ParamOnSecond();
                        if (FindVertAndSplitEdge(param2, edge2, edge1, Crv2, MaxTolVert, num2, sewd, face, boxes,
                                                 Standard_False)) {
                            NbSplit++;
                            continue;
                        }
                    ...
                    if (Tr1.PositionOnCurve() != IntRes2d_Middle && Tr2.PositionOnCurve() != IntRes2d_Middle) {
                        // union vertexes
                        if (UnionVertexes(sewd, edge1, edge2, num2, boxes, B2)) nbReplaced++; // gka 06.09.04
                    }
                }
                    ...
                    if (IS.HasFirstPoint() && IS.HasLastPoint()) {
                        ...
                        if (IsModified1) {
                            ...
                            if (Abs(pend - p11) > Abs(pend - p12))
                                ...
                                cut = p11;
                            Standard_Boolean IsCutLine;
                            if (CutEdge(edge1, pend, cut, face, IsCutLine)) NbCut++;
                        ...
                        if (IsModified2) {
                            ...
                            if (Abs(pend - p21) > Abs(pend - p22))
                                ...
                                cut = p21;
                            Standard_Boolean IsCutLine;
                            if (CutEdge(edge2, pend, cut, face, IsCutLine)) NbCut++;
                            if (newtol > BRep_Tool::Tolerance(NewV)) {
                                B.UpdateVertex(NewV, newtol);
                            } else
                                newtol = BRep_Tool::Tolerance(NewV);
                    ...
                    if (!B1.IsOut(B2)) {
                        ...
                        if (Inter.NbPoints() > 0 && Inter.NbPoints() < 3) {
                            ...
                            if (Tr1.PositionOnCurve() == IntRes2d_Middle && Tr2.PositionOnCurve() == IntRes2d_Middle) {
                                ...
                                B.MakeVertex(V, P0, tolV);
                                MaxTolVert = Max(MaxTolVert, tolV);
                                Standard_Boolean isSplitEdge2 = SplitEdge1(sewd2, face, num2, param2, V, tolV, boxes2);
                                if (isSplitEdge2) {
                                    NbModif++;
                                    num2--;
                                }
                                ...
                                if (isSplitEdge2) continue;
                            }
                            if (Tr1.PositionOnCurve() == IntRes2d_Middle && Tr2.PositionOnCurve() != IntRes2d_Middle) {
                                // find needed vertex from edge2 and split edge1 using it
                                Standard_Real param1 = IP.ParamOnFirst();
                                if (FindVertAndSplitEdge(param1, edge1, edge2, Crv1, MaxTolVert, num1, sewd1, face,
                                                         boxes1, Standard_True)) {
                                    NbModif++;
                                    break;
                                }
                            }
                            if (Tr1.PositionOnCurve() != IntRes2d_Middle && Tr2.PositionOnCurve() == IntRes2d_Middle) {
                                // find needed vertex from edge1 and split edge2 using it
                                Standard_Real param2 = IP.ParamOnSecond();
                                if (FindVertAndSplitEdge(param2, edge2, edge1, Crv2, MaxTolVert, num2, sewd2, face,
                                                         boxes2, Standard_True)) {
                                    NbModif++;
                                    continue;
                                }
                            }
                            if (Tr1.PositionOnCurve() != IntRes2d_Middle && Tr2.PositionOnCurve() != IntRes2d_Middle) {
                                // union vertexes
                                if (UnionVertexes(sewd2, edge1, edge2, num2, boxes2, B2)) nbReplaced++; // gka 06.09.04
                            }
                        }
                            ...
                            if (IS.HasFirstPoint() && IS.HasLastPoint()) {
                                ...
                                if (IsModified1 || IsModified2) {
                                    ...
                                    if (Abs(p12 - p11) > Abs(b1 - a1) / 2 || Abs(p22 - p21) > Abs(b2 - a2) / 2) {
                                        ...
                                        if (P01.Distance(PV1) < Max(tolV1, BRep_Tool::Tolerance(V1))) {
                                            NewV1 = V1;
                                            if (tolV1 > BRep_Tool::Tolerance(V1)) B.UpdateVertex(NewV1, tolV1);
                                            akey1++;
                                        }
                                        if (P01.Distance(PV2) < Max(tolV1, BRep_Tool::Tolerance(V2))) {
                                            NewV1 = V2;
                                            if (tolV1 > BRep_Tool::Tolerance(V2)) B.UpdateVertex(NewV1, tolV1);
                                            akey1++;
                                        }
                                        // analysis fo P02
                                        if (P02.Distance(PV1) < Max(tolV2, BRep_Tool::Tolerance(V1))) {
                                            NewV2 = V1;
                                            if (tolV2 > BRep_Tool::Tolerance(V1)) B.UpdateVertex(NewV2, tolV2);
                                            akey2++;
                                        }
                                        if (P02.Distance(PV2) < Max(tolV2, BRep_Tool::Tolerance(V2))) {
                                            NewV2 = V2;
                                            if (tolV2 > BRep_Tool::Tolerance(V2)) B.UpdateVertex(NewV2, tolV2);
                                            akey2++;
                                        }
                                        if (akey1 > 1 || akey2 > 1) continue;
                                        // prepare vertices
                                        ...
                                        if (akey1 == 0 && akey2 == 0) {
                                            ...
                                            if (SplitEdge1(sewd1, face, num1, p11, NewV1, tolV1, boxes1)) {
                                                ...
                                                sae.PCurve(tmpE, face, c2d, a, b, Standard_False);
                                                if ((a - p12) * (b - p12) >
                                                    0) { // p12 - external for [a,b] => split next edge
                                                    num1split2++;
                                                }
                                            }
                                        ...
                                        if (akey1 == 0 && akey2 == 0) {
                                            ...
                                            if (SplitEdge1(sewd2, face, num2, p21, NewV1, tolV1, boxes2)) {
                                                ...
                                                sae.PCurve(tmpE, face, c2d, a, b, Standard_False);
                                                if ((a - p22) * (b - p22) >
                                                    0) { // p22 - external for [a,b] => split next edge
                                                    num2split2++;
                                                }
                                            }
