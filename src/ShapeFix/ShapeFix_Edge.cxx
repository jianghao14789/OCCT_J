...
Standard_Boolean ShapeFix_Edge::FixRemovePCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
...
Standard_Boolean ShapeFix_Edge::FixAddPCurve(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                             const Standard_Boolean isSeam, const Standard_Real prec) {
    TopLoc_Location L;
    const Handle(Geom_Surface) & S = BRep_Tool::Surface(face, L);
...
Standard_Boolean ShapeFix_Edge::FixAddPCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
                                             const TopLoc_Location& location, const Standard_Boolean isSeam,
...
Standard_Boolean ShapeFix_Edge::FixAddPCurve(const TopoDS_Edge& edge, const TopoDS_Face& face,
                                             ...
                                             const Handle(ShapeAnalysis_Surface) & surfana, const Standard_Real prec) {
    TopLoc_Location L;
    const Handle(Geom_Surface) & S = BRep_Tool::Surface(face, L);
...
static Handle(Geom2d_Curve)
    TranslatePCurve(const Handle(Geom_Surface) & aSurf, Handle(Geom2d_Curve) & aC2d, const Standard_Real& aTol) {
    ...
    while (an_Iterator.More()) {
        ...
        if (!geometric_representation_ptr.IsNull()) {
            ...
            if (has_curve || has_closed_curve) {
                ...
                if (Abs(first - current_first) >
                        Precision::PConfusion() ||                        //: b8 abv 20 Feb 98: Confusion -> PConfusion
                    ...
                    if (has_curve) {
                        ...
                        GeomLib::SameRange(Tolerance, Curve2dPtr, oldFirstCurve1, oldLastCurve1, current_first,
                    ...
                    if (has_closed_curve) {
                        ...
                        GeomLib::SameRange(Tolerance, Curve2dPtr2, oldFirstCurve2, oldLastCurve2, current_first,
                                           current_last, NewCurve2dPtr2);
...
Standard_Boolean ShapeFix_Edge::FixAddPCurve(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surf,
                                             const TopLoc_Location& location, const Standard_Boolean isSeam,
                                             const Handle(ShapeAnalysis_Surface) & sas, const Standard_Real prec) {
    ShapeAnalysis_Edge sae;
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);
    if ((!isSeam && sae.HasPCurve(edge, surf, location)) || (isSeam && sae.IsSeam(edge, surf, location)))
    ...
    try {
        ...
        if (!sae.HasPCurve(edge, surf, location)) {
            ...
            if (myProjector->Status(ShapeExtend_DONE4)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
            a1 = First;
            b1 = Last;
        } else {
        ...
        if (isSeam) {
            ...
            if (sas->IsUClosed(prec) &&
                ...
                /* rln S4135 sas->Surface()->IsKind(STANDARD_TYPE(Geom_SphericalSurface)) */) { //: d9 abv 17 Mar 98:
                                                                                                //: any sphere
                ...
                c2d2->Translate(tranvec);
            } else if (sas->IsUClosed() && sas->IsVClosed()) { //: q8 abv 23 Mar 99: bm4_al_eye.stp #53710: avoid
                                                               //: shifting pcurves for pseudo-seam
...
Standard_Boolean ShapeFix_Edge::FixAddCurve3d(const TopoDS_Edge& edge) {
    ...
    if (!BRep_Tool::SameRange(edge)) TempSameRange(edge, Precision::PConfusion());
...
Standard_Boolean ShapeFix_Edge::FixVertexTolerance(const TopoDS_Edge& edge, const TopoDS_Face& face) {
    ...
    if (sae.Status(ShapeExtend_DONE1)) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_DONE1);
    if (sae.Status(ShapeExtend_DONE2)) myStatus = ShapeExtend::EncodeStatus(ShapeExtend_DONE2);
    BRep_Builder B;
...
Standard_Boolean ShapeFix_Edge::FixReversed2d(const TopoDS_Edge& edge, const Handle(Geom_Surface) & surface,
                                              const TopLoc_Location& location) {
    myStatus = ShapeExtend::EncodeStatus(ShapeExtend_OK);

    ShapeAnalysis_Edge EA;
    EA.CheckCurve3dWithPCurve(edge, surface, location);
    if (EA.Status(ShapeExtend_FAIL1)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);
    if (EA.Status(ShapeExtend_FAIL2)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL2);
...
Standard_Boolean ShapeFix_Edge::FixSameParameter(const TopoDS_Edge& edge, const Standard_Real tolerance) {
...
Standard_Boolean ShapeFix_Edge::FixSameParameter(const TopoDS_Edge& edge, const TopoDS_Face& face,
    ...
    if (sae.Status(ShapeExtend_FAIL2)) myStatus |= ShapeExtend::EncodeStatus(ShapeExtend_FAIL1);
