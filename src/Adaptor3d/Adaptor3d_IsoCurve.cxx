...
// purpose  :
//=======================================================================
Adaptor3d_IsoCurve::Adaptor3d_IsoCurve() : myIso(GeomAbs_NoneIso), myFirst(0.0), myLast(0.0), myParameter(0.0) {}
...
Adaptor3d_IsoCurve::Adaptor3d_IsoCurve(const Handle(Adaptor3d_Surface) & S)
    : mySurface(S), myIso(GeomAbs_NoneIso), myFirst(0.0), myLast(0.0), myParameter(0.0) {}
...
Adaptor3d_IsoCurve::Adaptor3d_IsoCurve(const Handle(Adaptor3d_Surface) & S, const GeomAbs_IsoType theIso,
...
Adaptor3d_IsoCurve::Adaptor3d_IsoCurve(const Handle(Adaptor3d_Surface) & theS, const GeomAbs_IsoType theIso,
                                       const Standard_Real theParam, const Standard_Real theWFirst,
                                       const Standard_Real theWLast)
    : mySurface(theS), myIso(theIso), myFirst(theWFirst), myLast(theWLast), myParameter(theParam) {
...
void Adaptor3d_IsoCurve::Load(const GeomAbs_IsoType Iso, const Standard_Real Param, const Standard_Real WFirst,
    ...
    if (mySurface->IsUPeriodic()) {
        ...
        if (myIso == GeomAbs_IsoU) {
            ElCLib::AdjustPeriodic(mySurface->FirstUParameter(), mySurface->FirstUParameter() + mySurface->UPeriod(),
                                   mySurface->UResolution(Precision::Confusion()), myParameter, dummy);
        } else {
            ElCLib::AdjustPeriodic(mySurface->FirstUParameter(), mySurface->FirstUParameter() + mySurface->UPeriod(),
                                   mySurface->UResolution(Precision::Confusion()), myFirst, myLast);
        }
    }
    ...
    if (mySurface->IsVPeriodic()) {
        ...
        if (myIso == GeomAbs_IsoV) {
            ElCLib::AdjustPeriodic(mySurface->FirstVParameter(), mySurface->FirstVParameter() + mySurface->VPeriod(),
                                   mySurface->VResolution(Precision::Confusion()), myParameter, dummy);
        } else {
            ElCLib::AdjustPeriodic(mySurface->FirstVParameter(), mySurface->FirstVParameter() + mySurface->VPeriod(),
                                   mySurface->VResolution(Precision::Confusion()), myFirst, myLast);
        }
    }
}
...
Handle(Adaptor3d_Curve) Adaptor3d_IsoCurve::Trim(const Standard_Real First, const Standard_Real Last,
...
void Adaptor3d_IsoCurve::D3(const Standard_Real T, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3) const {
    gp_Vec dummy[6];
    switch (myIso) {

        case GeomAbs_IsoU:
            mySurface->D3(myParameter, T, P, dummy[0], V1, dummy[1], V2, dummy[2], dummy[3], V3, dummy[4], dummy[5]);
            break;

        case GeomAbs_IsoV:
            mySurface->D3(T, myParameter, P, V1, dummy[0], V2, dummy[1], dummy[2], V3, dummy[3], dummy[4], dummy[5]);
            break;
    ...
    switch (mySurface->GetType()) {
        ...
        case GeomAbs_Torus: {
            ...
            switch (myIso) {
                ...
                case GeomAbs_IsoU: {
                    return ElSLib::TorusUIso(tor.Position(), tor.MajorRadius(), tor.MinorRadius(), myParameter);
                }

                case GeomAbs_IsoV: {
                    return ElSLib::TorusVIso(tor.Position(), tor.MajorRadius(), tor.MinorRadius(), myParameter);
                }
        ...
        case GeomAbs_SurfaceOfExtrusion: {
            return mySurface->BasisCurve()->Circle().Translated(myParameter * gp_Vec(mySurface->Direction()));
        }
        default: {
...
gp_Elips Adaptor3d_IsoCurve::Ellipse() const {
    switch (mySurface->GetType()) {
        ...
        case GeomAbs_SurfaceOfExtrusion: {
            return mySurface->BasisCurve()->Ellipse().Translated(myParameter * gp_Vec(mySurface->Direction()));
        }
        default: {
