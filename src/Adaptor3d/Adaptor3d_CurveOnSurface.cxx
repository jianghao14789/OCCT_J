...
static gp_Vec SetLinearForm(const gp_Vec2d DW, const gp_Vec2d D2W, const gp_Vec2d D3W, const gp_Vec D1U,
                            const gp_Vec D1V, const gp_Vec D2U, const gp_Vec D2V, const gp_Vec D2UV, const gp_Vec D3U,
    ...
    V32.SetLinearForm(D2W.X() * DW.Y() + DW.X() * D2W.Y(), D2UV, DW.X() * DW.Y() * DW.Y(), D3UVV, V32);
    V33.SetLinearForm(2 * D2W.X() * DW.X(), D2U, DW.X() * DW.X() * DW.X(), D3U, DW.X() * DW.X() * DW.Y(), D3UUV);

    V34.SetLinearForm(2 * D2W.Y() * DW.Y(), D2V, DW.Y() * DW.Y() * DW.X(), D3UVV, DW.Y() * DW.Y() * DW.Y(), D3V);
...
//=======================================================================
static void
Hunt(const TColStd_Array1OfReal& Arr, const Standard_Real Coord,
     Standard_Integer& Iloc) { // Warning: Hunt is used to find number of knot which equals coordinate component,
...
static void ReverseParam(const Standard_Real In1, const Standard_Real In2, Standard_Real& Out1, Standard_Real& Out2) {
...
static void ReverseParam(const Standard_Integer In1, const Standard_Integer In2, Standard_Integer& Out1,
...
// purpose  :
//=======================================================================
static void FindBounds(const TColStd_Array1OfReal& Arr, const Standard_Real Coord, const Standard_Real Der,
                       Standard_Integer& Bound1, Standard_Integer& Bound2, Standard_Boolean& DerNull)

{
    ...
    if (N == Bound1) {
        if (Abs(Der) > Tol) DerNull = Standard_False;
...
static void Locate1Coord(const Standard_Integer Index, const gp_Pnt2d& UV, const gp_Vec2d& DUV,
                         const Handle(Geom_BSplineCurve) & BSplC, gp_Pnt2d& LeftBot, gp_Pnt2d& RightTop) {
...
static void Locate1Coord(const Standard_Integer Index, const gp_Pnt2d& UV, const gp_Vec2d& DUV,
                         const Handle(Geom_BSplineSurface) & BSplS, Standard_Boolean& DIsNull, gp_Pnt2d& LeftBot,
                         gp_Pnt2d& RightTop) {
...
static void Locate2Coord(const Standard_Integer Index, const gp_Pnt2d& UV, const gp_Vec2d& DUV, const Standard_Real I1,
                         const Standard_Real I2, gp_Pnt2d& LeftBot, gp_Pnt2d& RightTop) {
    Standard_Real Tol = Precision::PConfusion() / 10;
    Standard_Real Comp1 = 0, DComp1 = 0;
    if (Index == 1) {
        Comp1 = UV.X();
...
static void Locate2Coord(const Standard_Integer Index, const gp_Pnt2d& UV, const gp_Vec2d& DUV,
                         const Handle(Geom_BSplineSurface) & BSplS, const TColStd_Array1OfReal& Arr, gp_Pnt2d& LeftBot,
                         gp_Pnt2d& RightTop) {
...
Adaptor3d_CurveOnSurface::Adaptor3d_CurveOnSurface() : myType(GeomAbs_OtherCurve), myIntCont(GeomAbs_CN) {}
...
void Adaptor3d_CurveOnSurface::Load(const Handle(Adaptor2d_Curve2d) & C, const Handle(Adaptor3d_Surface) & S) {
    Load(C);
    Load(S);
}
...
static void AddIntervals(const Handle(TColStd_HSequenceOfReal) & theParameters, const math_FunctionRoots& theRoots,
                         Standard_Real theTol) {
    if (!theRoots.IsDone() || theRoots.IsAllNull()) return;
...
Standard_Integer Adaptor3d_CurveOnSurface::NbIntervals(const GeomAbs_Shape S) const {
    if (S == myIntCont && !myIntervals.IsNull()) return myIntervals->Length() - 1;
    ...
    if (aIntervals->Length() == 1) aIntervals->Append(aIntervals->Value(1));
...
void Adaptor3d_CurveOnSurface::Intervals(TColStd_Array1OfReal& T, const GeomAbs_Shape S) const {
    NbIntervals(S);
    Standard_ASSERT_RAISE(T.Length() == myIntervals->Length(),
...
Handle(Adaptor3d_Curve) Adaptor3d_CurveOnSurface::Trim(const Standard_Real First, const Standard_Real Last,
...
Standard_Boolean Adaptor3d_CurveOnSurface::IsPeriodic() const {
    if (myType == GeomAbs_Circle || myType == GeomAbs_Ellipse) return Standard_True;
...
Standard_Real Adaptor3d_CurveOnSurface::Period() const {
    if (myType == GeomAbs_Circle || myType == GeomAbs_Ellipse) return (2. * M_PI);
...
void Adaptor3d_CurveOnSurface::D3(const Standard_Real U, gp_Pnt& P, gp_Vec& V1, gp_Vec& V2, gp_Vec& V3) const {
...
gp_Lin Adaptor3d_CurveOnSurface::Line() const {
    Standard_NoSuchObject_Raise_if(myType != GeomAbs_Line,
...
gp_Circ Adaptor3d_CurveOnSurface::Circle() const {
    Standard_NoSuchObject_Raise_if(myType != GeomAbs_Circle,
...
Handle(Geom_BezierCurve) Adaptor3d_CurveOnSurface::Bezier() const {
    Standard_NoSuchObject_Raise_if(mySurface->GetType() != GeomAbs_Plane, "Adaptor3d_CurveOnSurface : Bezier");
...
Handle(Geom_BSplineCurve) Adaptor3d_CurveOnSurface::BSpline() const {
    Standard_NoSuchObject_Raise_if(mySurface->GetType() != GeomAbs_Plane, "Adaptor3d_CurveOnSurface : BSpline");
        ...
        if (CTy == GeomAbs_Line) {
            ...
            if (D.IsParallel(gp::DX2d(), Precision::Angular())) { // Iso V.
                ...
                if (STy == GeomAbs_Sphere) {
                    ...
                    Tr.Multiply(P.Y());
                    myLin.Translate(Tr);
                    if (D.IsOpposite(gp::DY2d(), Precision::Angular())) myLin.Reverse();
                } else if (STy == GeomAbs_Cone) {
                    myType = GeomAbs_Line;
                    gp_Cone Cone = mySurface->Cone();
                    gp_Pnt2d P = myCurve->Line().Location();
...
Standard_Boolean Adaptor3d_CurveOnSurface::LocatePart_RevExt(const gp_Pnt2d& UV, const gp_Vec2d& DUV,
...
Standard_Boolean Adaptor3d_CurveOnSurface::LocatePart_Offset(const gp_Pnt2d& UV, const gp_Vec2d& DUV,
                                                             const Handle(Adaptor3d_Surface) & S, gp_Pnt2d& LeftBot,
                                                             gp_Pnt2d& RightTop) const {
    Standard_Boolean Ok = Standard_True;
...
void Adaptor3d_CurveOnSurface::LocatePart(const gp_Pnt2d& UV, const gp_Vec2d& DUV, const Handle(Adaptor3d_Surface) & S,
