...
Adaptor3d_TopolTool::Adaptor3d_TopolTool()
    : myNbSamplesU(-1), myNbSamplesV(-1), nbRestr(0), idRestr(0), Uinf(0.0), Usup(0.0), Vinf(0.0), Vsup(0.0), nbVtx(0),
      idVtx(0) {}
...
TopAbs_State Adaptor3d_TopolTool::Classify(const gp_Pnt2d& P, const Standard_Real Tol, const Standard_Boolean)
//					 const Standard_Boolean RecadreOnPeriodic)
{
    ...
    if (nbRestr == 4) {
        ...
        if ((Abs(U - Uinf) <= Tol) || (Abs(U - Usup) <= Tol) || (Abs(V - Vinf) <= Tol) || (Abs(V - Vsup) <= Tol)) {
            return TopAbs_ON;
        }
        return TopAbs_IN;
    } else if (nbRestr == 0) {
...
Standard_Boolean Adaptor3d_TopolTool::IsThePointOn(const gp_Pnt2d& P, const Standard_Real Tol, const Standard_Boolean)
//					     const Standard_Boolean RecadreOnPeriodic)
{
...
#define myMinPnts                                                                                                      \
    4 // Absolut possible minimum of sample points
    ...
    if (nbvp > 2) {
        for (i = 2; i < nbup; i++) {
            ...
            for (j = 3; j < nbvp; j++) { //-- try
                ...
                Vip1.SetCoord(C1.X() - B1.X() - B1.X() + A1.X(), C1.Y() - B1.Y() - B1.Y() + A1.Y(),
                              C1.Z() - B1.Z() - B1.Z() + A1.Z());
...
void Adaptor3d_TopolTool::SamplePnts(const Standard_Real theDefl, const Standard_Integer theNUmin,
...
void Adaptor3d_TopolTool::BSplSamplePnts(const Standard_Real theDefl, const Standard_Integer theNUmin,
                                         const Standard_Integer theNVmin) {
    ...
    myNbSamplesU = 0;
    for (i = 1; i <= nbsu; i++)
        if (anUFlg(i) == Standard_True) myNbSamplesU++;
    ...
    myNbSamplesV = 0;
    for (i = 1; i <= nbsv; i++)
        if (aVFlg(i) == Standard_True) myNbSamplesV++;
...
Standard_Boolean Adaptor3d_TopolTool::IsUniformSampling() const {
    ...
    if (typS == GeomAbs_BSplineSurface) return Standard_False;
    return Standard_True;
}
