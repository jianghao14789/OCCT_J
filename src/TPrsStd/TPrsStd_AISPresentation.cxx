...
Handle(TPrsStd_AISPresentation) TPrsStd_AISPresentation::Set(const TDF_Label& theLabel,
    ...
    if (aData.IsNull()) throw Standard_NoMoreObject(NO_MORE_OBJECT);
...
Handle(TDataXtd_Presentation) TPrsStd_AISPresentation::getData() const {
    ...
    if (!Label().FindAttribute(TDataXtd_Presentation::GetID(), aData)) throw Standard_NoMoreObject(NO_MORE_OBJECT);
    return aData;
}
...
void TPrsStd_AISPresentation::Display(const Standard_Boolean theIsUpdate) {
    if (theIsUpdate || myAIS.IsNull()) AISUpdate();
...
void TPrsStd_AISPresentation::UnsetMaterial() {
    if (!getData()->HasOwnMaterial() && !myAIS.IsNull() && !myAIS->HasMaterial()) return;
...
void TPrsStd_AISPresentation::SetMaterial(const Graphic3d_NameOfMaterial theName) {
    ...
    if (getData()->HasOwnMode() && getData()->MaterialIndex() == theName)
        if (!myAIS.IsNull() && myAIS->HasMaterial() && myAIS->Material() == theName) return;
    ...
    if (!myAIS.IsNull()) {
        if (myAIS->HasMaterial() && myAIS->Material() == theName) return; // AIS has already had that material
...
void TPrsStd_AISPresentation::SetTransparency(const Standard_Real theValue) {
    ...
    if (getData()->HasOwnTransparency() && getData()->Transparency() == theValue)
        if (!myAIS.IsNull() && myAIS->Transparency() == theValue) return;
    ...
    if (!myAIS.IsNull()) {
        if (myAIS->Transparency() == theValue) return; // AIS has already had that transparency
...
void TPrsStd_AISPresentation::UnsetTransparency() {
    if (!getData()->HasOwnTransparency()) return;
...
void TPrsStd_AISPresentation::UnsetColor() {
    if (!getData()->HasOwnColor() && !myAIS.IsNull() && !myAIS->HasColor()) return;
    getData()->UnsetColor();
...
void TPrsStd_AISPresentation::SetWidth(const Standard_Real theWidth) {
    ...
    if (getData()->HasOwnWidth() && getData()->Width() == theWidth)
        if (!myAIS.IsNull() && myAIS->HasWidth() && myAIS->Width() == theWidth) return;

    getData()->SetWidth(theWidth);

    if (!myAIS.IsNull()) {
        if (myAIS->HasWidth() && myAIS->Width() == theWidth) return; // AIS has already had that width
...
void TPrsStd_AISPresentation::UnsetWidth() {
    if (!getData()->HasOwnWidth())
        if (!myAIS.IsNull() && !myAIS->HasWidth()) return;
...
void TPrsStd_AISPresentation::SetMode(const Standard_Integer theMode) {
    ...
    if (getData()->HasOwnMode() && getData()->Mode() == theMode)
        if (!myAIS.IsNull() && myAIS->DisplayMode() == theMode) return;
...
void TPrsStd_AISPresentation::UnsetMode() {
    if (!getData()->HasOwnMode())
        if (!myAIS.IsNull() && !myAIS->HasDisplayMode()) return;
...
// purpose  :
//=======================================================================
void TPrsStd_AISPresentation::Paste(const Handle(TDF_Attribute) & theInto, const Handle(TDF_RelocationTable) &) const {
    ...
    if (!anInto->myAIS.IsNull()) {
        ...
        if (displayed) anInto->SetDisplayed(displayed);
    }
...
void TPrsStd_AISPresentation::BeforeForget() {
    if (!myAIS.IsNull()) { // Remove AISObject from context.
...
//=======================================================================
Standard_Boolean TPrsStd_AISPresentation::BeforeUndo(const Handle(TDF_AttributeDelta) & AD, const Standard_Boolean) {
...
// purpose  : le NamedShape associe doit etre present
//=======================================================================
Standard_Boolean TPrsStd_AISPresentation::AfterUndo(const Handle(TDF_AttributeDelta) & AD, const Standard_Boolean) {
    Handle(TPrsStd_AISPresentation) P;
    AD->Label().FindAttribute(TPrsStd_AISPresentation::GetID(), P);
...
void TPrsStd_AISPresentation::AISUpdate() {
        ...
        if (TPrsStd_DriverTable::Get()->FindDriver(GetDriverGUID(), aDriver)) {
            ...
            if (myAIS.IsNull()) {
                ...
                if (aDriver->Update(Label(), anObj))
                    if (!(anObj == myAIS)) {
                        if (!aContext.IsNull()) aContext->Remove(myAIS, Standard_False);
    ...
    if (!myAIS.IsNull()) {
        ...
        if (HasOwnMode()) {
            ...
            if (myAIS->DisplayMode() != aMode) myAIS->SetDisplayMode(aMode);
        }
...
void TPrsStd_AISPresentation::AISDisplay() {
    if (!Label().IsNull()) {
        ...
        if (!myAIS.IsNull()) {
            ...
            if (aContext->IsDisplayed(myAIS)) SetDisplayed(Standard_True);
        }
    }
}
...
void TPrsStd_AISPresentation::AISErase(const Standard_Boolean theIsRemove) {
    ...
    if (!myAIS.IsNull()) {
        Backup();
        if (!Label().IsNull()) {
            if (IsAttribute(TDataXtd_Presentation::GetID())) SetDisplayed(Standard_False);
            ...
            if (theIsRemove) {
                if (!aContext.IsNull()) aContext->Remove(myAIS, Standard_False);
                if (!anOwnContext.IsNull() && anOwnContext != aContext) anOwnContext->Remove(myAIS, Standard_False);

                myAIS.Nullify();
            } else {
                if (!aContext.IsNull()) aContext->Erase(myAIS, Standard_False);
                if (!anOwnContext.IsNull() && anOwnContext != aContext) anOwnContext->Erase(myAIS, Standard_False);
            }
        } else {
            if (theIsRemove) {
...
Handle(AIS_InteractiveContext) TPrsStd_AISPresentation::getAISContext() const {
    ...
    if (TPrsStd_AISViewer::Find(Label(), aViewer)) return aViewer->GetInteractiveContext();
        ...
        if (!aContext.IsNull()) {
            ...
            if (nbSelModes == 1) {
                ...
                if (aSelectionMode == -1) {
                    ...
                    if (!isActivated)
                        aContext->SetSelectionModeActive(myAIS, aSelectionMode, Standard_True,
