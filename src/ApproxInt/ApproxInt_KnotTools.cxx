...
//=======================================================================
static Standard_Real EvalCurv(const Standard_Real dim, const Standard_Real* V1, const Standard_Real* V2) {
...
void ApproxInt_KnotTools::ComputeKnotInds(const NCollection_LocalArray<Standard_Real>& theCoords,
    ...
    for (i = aCurv.Lower() + 1; i < aCurv.Upper(); ++i) {
        Standard_Real d1 = aCurv(i) - aCurv(i - 1), d2 = aCurv(i) - aCurv(i + 1), ad1 = Abs(d1), ad2 = Abs(d2);
        ...
        for (; j <= theInds.Length() - 1;) {
            if (theInds(j) == anInd) {
                ...
                Standard_Integer ici = (anIndPrev - aCurv.Lower()) * theDim,
                                 ici1 = (anIndNext - aCurv.Lower()) * theDim, icm = (anInd - aCurv.Lower()) * theDim;
        ...
        if (theInds(i) - anIndsPrev <= theMinNbPnts) {
            if (i != theInds.Length()) {
                ...
                for (; anIdx <= theInds.Length(); ++anIdx) {
                    if (theInds(anIdx) - anIndsPrev >= theMinNbPnts) break;
                }
                anIdx--;
...
// purpose  :
//=======================================================================
Standard_Boolean ApproxInt_KnotTools::InsKnotBefI(const Standard_Integer theI, const TColStd_Array1OfReal& theCurv,
    ...
    for (j = anInd + 1; j < anInd1; ++j) {
        ...
        if (mid > 0) {
            if (ChkCurv) {
                Standard_Integer ici = (anInd - theCurv.Lower()) * theDim, ici1 = (anInd1 - theCurv.Lower()) * theDim,
...
// purpose  :
//=======================================================================
void ApproxInt_KnotTools::BuildKnots(const TColgp_Array1OfPnt& thePntsXYZ, const TColgp_Array1OfPnt2d& thePntsU1V1,
                                     const TColgp_Array1OfPnt2d& thePntsU2V2, const math_Vector& thePars,
                                     const Standard_Boolean theApproxXYZ, const Standard_Boolean theApproxU1V1,
                                     const Standard_Boolean theApproxU2V2, const Standard_Integer theMinNbPnts,
    ...
    if (theApproxXYZ) aDim += 3;
    if (theApproxU1V1) aDim += 2;
    if (theApproxU2V2) aDim += 2;
...
ApproxInt_KnotTools::DefineParType(const Handle(IntPatch_WLine) & theWL, const Standard_Integer theFpar,
                                   const Standard_Integer theLpar, const Standard_Boolean theApproxXYZ,
                                   const Standard_Boolean theApproxU1V1, const Standard_Boolean theApproxU2V2) {
    if (theLpar - theFpar == 1) return Approx_IsoParametric;

    const Standard_Integer nbp3d = theApproxXYZ ? 1 : 0, nbp2d = (theApproxU1V1 ? 1 : 0) + (theApproxU2V2 ? 1 : 0);

    GeomInt_TheMultiLineOfWLApprox aTestLine(theWL, nbp3d, nbp2d, theApproxU1V1, theApproxU2V2, 0., 0., 0., 0., 0., 0.,
                                             0., theApproxU1V1, theFpar, theLpar);
    ...
    if (aMidCurv <= eps) return aParType;
    ...
    if (aRat > aCritRat) {
        if (aRat > 5. * aCritRat)
            ...
            if (aParRat > aCritParRat) aParType = Approx_Centripetal;
        }
    }
