...
StdStorage_TypeData::StdStorage_TypeData() : myTypeId(0), myErrorStatus(Storage_VSOk) {
...
Standard_Integer StdStorage_TypeData::AddType(const Handle(StdObjMgt_Persistent) & aPObj) {
    ...
    if (IsType(aTypeName)) return Type(aTypeName);
...
StdObjMgt_Persistent::Instantiator StdStorage_TypeData::Instantiator(const Standard_Integer aTypeNum) const {
    TCollection_AsciiString aTypeName = Type(aTypeNum);
    StdObjMgt_Persistent::Instantiator anInstantiator = 0;
    if (!myMapOfPInst.Find(aTypeName, anInstantiator)) return 0;
