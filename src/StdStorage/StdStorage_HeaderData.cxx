...
StdStorage_HeaderData::StdStorage_HeaderData() : myNBObj(0), myErrorStatus(Storage_VSOk) {}
...
Standard_Boolean StdStorage_HeaderData::Write(const Handle(Storage_BaseDriver) & theDriver) {
    ...
    try {
        OCC_CATCH_SIGNALS
        theDriver->WriteInfo(myNBObj, myStorageVersion, myDate, mySchemaName, mySchemaVersion, myApplicationName,
                             myApplicationVersion, myDataType, myUserInfo);
    } catch (Storage_StreamTypeMismatchError const&) {
        myErrorStatus = Storage_VSTypeMismatch;
